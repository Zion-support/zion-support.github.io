  }}},// Run all fixes,fixFooter(),fixAccessibility(),fixAiServices(),fixApiDocs(),fixCareers(),// // // console.log(' Syntax error fixes completed'),,// Run all fixes,fixFooter(),fixAccessibility(),fixAiServices(),fixApiDocs(),fixCareers(),,console.log(' Syntax error fixes completed'),#!/usr/bin/env node;'
import fs from 'fs';'
...
function findFiles() {const files  = [];function traverse() {const items  = fs.readdirSync(currentDir)for (const item of items) {const fullPath = path.join(currentDir, item)const stat  = fs.statSync(fullPath)if (stat.isDirectory() && !item.startsWith('.') && item !== 'node_modules') {traverse(fullPath)} else if (stat.isFile() && extensions.some(ext => item.endsWith(ext))) {files.push(fullPath)}'
    }
  }traverse(dir)return files;
}// Main execution;
const srcDir = path.join(__dirname, 'src')const files  = findFiles(srcDir)console.log(` Found ${files.length} files to check...`)let fixedCount = 0;`for (const file of files) {if (fixFile(file)) {fixedCount++;
  }
