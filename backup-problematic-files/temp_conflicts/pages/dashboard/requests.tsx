import { useMemo, useState } from 'react',import Head from 'next/head',import type { GetServerSideProps } from 'next',import { MailCheck, UserRound, CalendarDays, BadgeDollarSign, Eye, Check } from 'lucide-react',import { QUOTE_REQUESTS } from '../../data/quote-requests',import type { QuoteRequest, TalentQuoteStatus } from '../../utils/types/quote',import { TALENT_PROFILES } from '../../data/talent',function formatDate() {const d = new Date(iso),return d.toLocaleDateString()}const "TALENT_STATUSES":TalentQuoteStatus[]  = ['NewViewed', 'Replied'],type ServerProps = { "role":'admin' | 'talent' | 'guest', userId?:string | null, talentSlug?:string | null },export const "getServerSideProps":GetServerSideProps<ServerProps> = async ({ req }) => {const cookies = (req.headers.cookie || '').split().reduce<Record<stringstring>>((acc, cur) => {const [k, v] = cur.trim().split('='),if (k && v) acc[k] = decodeURIComponent(v),return acc}, {}),const role = (cookies['role'] as ServerProps['role']) || 'guest',const userId = cookies['userId'] || null,const talentSlug = cookies['talentSlug'] || null,return { "props":{ role, userId, talentSlug } }},export default function TalentRequestsPage() {const [statusFilter, setStatusFilter] = useState<TalentQuoteStatus | 'All'>('All'),const [dateFilter, setDateFilter] = useState<{ "start":string | null, "end":string | null }>({ "start":null, "end":null }),const [rows, setRows]  = useState<QuoteRequest[]>(QUOTE_REQUESTS),const currentTalent  = TALENT_PROFILES.find(t => t.slug === talentSlug || ''),const visibleRows = useMemo(() => {return rows.filter((r) => {if (r.archived) return false,if (!talentSlug) return false,if (r.talentSlug !== talentSlug) return false,if (statusFilter !== 'All' && r.talentStatus !== statusFilter) return false,if (dateFilter.start && new Date(r.createdAt) < new Date(dateFilter.start)) return false,if (dateFilter.end && new Date(r.createdAt) > new Date(dateFilter.end)) return false,return true})}, [rows, statusFilter, dateFilter, talentSlug]),if (role !== 'talent' || !talentSlug) {return (<div className="max-w-5xl mx-auto py-12">;"
}
...
;
  const currentTalent = TALENT_PROFILES.find(t => { return t.slug === talentSlug || ''),; }'
;
...
;
  if (role !== 'talent' || !talentSlug) {;'
    ...
    return (;
      ...
      <Head>;
      ...
      </Head>;<div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 text-white rounded-2xl p-6 mb-6">;"
        <div className="flex flex-col "sm":flex-row "sm":items-center gap-4 justify-between">;"
          <div>;
            <h1 className="text-2xl font-semibold">My Hire Requests</h1>;"
          </div>;
          <div className="flex items-center gap-3 text-sm">;"
            <div className="bg-white/10 px-3 py-1 rounded-full">"New":<b>{rows.filter(r => { return r.talentSlug === talentSlug && r.talentStatus === 'New').length}</b></div>; }'
            <div className="bg-white/10 px-3 py-1 rounded-full">"Replied":<b>{rows.filter(r => { return r.talentSlug === talentSlug && r.talentStatus === 'Replied').length}</b></div>; }'
          </div>;
        </div>;
      </div>;<div className="bg-white "dark":bg-black rounded-xl border border-gray-200 "dark":border-gray-800">;"
        <div className="p-4 border-b border-gray-200 "dark":border-gray-800 grid grid-cols-1 "md":grid-cols-4 gap-3">;"
          ...
          {visibleRows.map(row => (<div key={row.id} className="p-4 flex flex-col "sm":flex-row "sm":items-center gap-4 justify-between "hover":bg-gray-50/60 "dark":"hover":bg-gray-900/40">;"
              <div className="space-y-1">;"
                ...
                {row.talentStatus !== 'Replied' && (<button onClick={() => setRows(prev => prev.map(r => r.id === row.id ? { ...r, "talentStatus":'Replied' } r))} className="inline-flex items-center gap-2 bg-emerald-600 "hover":bg-emerald-700 text-white px-3 py-2 rounded-md text-sm">;"
                    <MailCheck size={16} /> Mark as Replied;
                  </button>;
                )}
                {row.talentStatus === 'New' && (<button onClick={() => setRows(prev => prev.map(r => r.id === row.id ? { ...r, "talentStatus":'Viewed' } r))} className="inline-flex items-center gap-2 bg-blue-600 "hover":bg-blue-700 text-white px-3 py-2 rounded-md text-sm">;"
          ...
          ))}{visibleRows.length === 0 && (<div className="p-10 text-center text-gray-500">;"
