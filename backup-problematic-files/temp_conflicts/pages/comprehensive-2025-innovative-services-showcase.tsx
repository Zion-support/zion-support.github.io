import React from 'react',import SEO from '../components/SEO',import Layout from '../components/layout/Layout',import { motion } from 'framer-motion',import {Star, Brain, Atom, Shield, Zap, TrendingUp, Globe,Rocket, Lock, Palette, Factory, Database, Cloud,Cpu, Eye, Target, Users, BarChart3;'
} from 'lucide-react',import { innovative2025MicroSaasBatch } from '../data/innovative-2025-micro-saas-batch',const "Comprehensive2025InnovativeServicesShowcase":React.FC = () => {const categories = [;'
...
const serviceCategories = [;
  'All ServicesAI & Machine Learning','Quantum ComputingCybersecurity','Healthcare & BiotechFinance & Investment','Supply Chain & LogisticsManufacturing & Industry 4.0','Space TechnologyBlockchain & Web3','Education & LearningLegal & Compliance','Energy & SustainabilityAutonomous Vehicles','IoT & Edge ComputingMetaverse & VR','Neuroscience & BCI';'
],// Helper function to get service category;
const getServiceCategory = ("service":any) => {if (service.category) return service.category,if (service.type) return service.type,return 'Other';'
},// Helper function to get service pricing;
const getServicePricing = ("service":any) => {if (service.pricing?.starter) return service.pricing.starter,if (service.pricing?.monthly) return `$${service.pricing.monthly}/month`,if (service.price?.monthly) return `$${service.price.monthly}/month`,return 'Contact for pricing'},// Helper function to get service features;'const getServiceFeatures = ("service":any) => {if (service.features) return service.features,if (service.keyFeatures) return service.keyFeatures,return [];
},// Helper function to get service description;
const getServiceDescription = ("service":any) => {if (service.description) return service.description,if (service.tagline) return service.tagline,return '';'
},export default function Comprehensive2025InnovativeServicesShowcase() {const [searchTerm, setSearchTerm] = useState(''),const [selectedCategory, setSelectedCategory] = useState('All Services'),const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid'),const [sortBy, setSortBy]  = useState<'name' | 'price' | 'rating' | 'popularity'>('popularity'),// Filter and sort services;'
  }
  const filteredServices = allServices;
    .filter((service) => {const serviceName = (service as any).title || (service as any).name || '',const matchesSearch = serviceName.toLowerCase().includes(searchTerm.toLowerCase()) ||;'
                           }
                           service.description.toLowerCase().includes(searchTerm.toLowerCase()) ||;
                           (service as any).tagline?.toLowerCase().includes(searchTerm.toLowerCase()) || false,const matchesCategory = selectedCategory === 'All Services' || ;'
                             getServiceCategory(service).includes(selectedCategory.split(' ')[0]),return matchesSearch && matchesCategory}).sort((a, b) => {const aName = (a as any).title || (a as any).name || '',const bName = (b as any).title || (b as any).name || '',switch (sortBy) {case 'name':;'
          }
          return aName.localeCompare(bName),case 'price':;'
          return ((a as any).price?.monthly || 0) - ((b as any).price?.monthly || 0),case 'rating':;'
          return ((b as any).rating || 0) - ((a as any).rating || 0),case 'popularity':;'
        "default":return ((b as any).customers || 0) - ((a as any).customers || 0)}
    }),const containerVariants = {"hidden":{ "opacity":0 },"visible":{"opacity":1,"transition":{"staggerChildren":0.1;
      }
    }
  const getCategoryIcon = ("category":string) => {const cat = categories.find(c => { return c.name === category),return cat ? cat.icon :Star; }
  },const getCategoryColor = ("category":string) => {const cat = categories.find(c => { return c.name === category),return cat ? cat.color :'from-gray-500 to-gray-600'; }'
  },return (<Layout>;
      <SEO ;
        ...
        keywords={[;
          'innovative services 2025micro SAAS','quantum computingspace technology','AI platformsautonomous systems';'
        ]} />;{/* Hero Section */}
      <section className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">;"
        ...
        <div className="absolute inset-0 pointer-events-none">;"
          ...
          >;
            ...
            </motion.div>;<h1 className="text-6xl "md":text-8xl font-bold mb-8 bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 bg-clip-text text-transparent">;"
      ...
      <section className="py-20 px-4 bg-black/30 relative">;"
        ...
        <div className="max-w-7xl mx-auto relative z-10">;"
          ...
          >;
            ...
            {categories.map((category, index) => {const IconComponent = category.icon,return (<motion.div;
                  }
                  key={category.name}
                  initial={{ "opacity":0, "y":20 }}
                  whileInView={{ "opacity":1, "y":0 }}
                ...
                >;
                  <div className={`p-8 rounded-2xl bg-gradient-to-br ${category.color} bg-opacity-10 border border-white/10 "hover":border-white/20 transition-all duration-300 transform "hover":scale-105 "hover":shadow-2xl`}>;`                    <div className="text-center">;"
                      <div className={`w-16 h-16 mx-auto mb-6 rounded-full bg-gradient-to-br ${category.color} bg-opacity-20 flex items-center justify-center border border-white/20`}>;`                        <IconComponent className="w-8 h-8 text-white" />;"
                      </div>;
                      <h3 className="text-2xl font-bold text-white mb-4">{category.name}</h3>;"
                      <div className="text-gray-300 text-sm">;"
                        {innovative2025MicroSaasBatch.filter(service => { return service.category === category.name).length} Services; }
                      </div>;
                    </div>;
                  </div>;
                </motion.div>;
      ...
      <section className="py-20 px-4">;"
        <div className="max-w-7xl mx-auto">;"
          ...
          >;
            ...
            {innovative2025MicroSaasBatch.filter(service => service.popular).map((service, index) => {const IconComponent = getCategoryIcon(service.category),return (<motion.div;
                  }
                  key={service.id}
                  initial={{ "opacity":0, "y":20 }}
                  whileInView={{ "opacity":1, "y":0 }}
                ...
                >;
                  <div className="p-8 rounded-2xl bg-gradient-to-br from-gray-900/50 to-gray-800/50 border border-white/10 "hover":border-white/20 transition-all duration-300 transform "hover":scale-105 "hover":shadow-2xl backdrop-blur-sm">;"
                    {/* Popular Badge */}
                    {service.popular && (<div className="absolute -top-3 left-6 px-4 py-1 bg-gradient-to-r from-yellow-500 to-orange-500 text-black text-sm font-bold rounded-full">;"
                        }
                        Popular;
                      </div>;
                    )}<div className="text-center mb-6">;"
                      <div className={`w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br ${getCategoryColor(service.category)} bg-opacity-20 flex items-center justify-center border border-white/20 text-4xl`}>;`                        {service.icon}
                      </div>;
                    ...
                    <span className="text-gray-300 text-sm">;"
                      {(service as any).rating || 0} ({(service as any).reviews || 0} reviews)</span>;
                  </div>;
                </div>;{/* Service Details */}
                <div className={`p-6 bg-white/5 ${viewMode === 'list' ? '"lg":w-2/3' :''}`}>;`                  <p className="text-gray-300 mb-4">{service.description}</p>;{/* Key Benefits */}"
                  <div className="mb-4">;"
                    ...
                    <ul className="space-y-1">;"
                      {(service as any).benefits?.slice(0, 3)?.map(("benefit":string, "idx":number) => (<li key={idx} className="text-gray-300 text-sm flex items-start gap-2">;"
                  ...
                  <div className="mb-6">;"
                    ...
                    <div className="flex flex-wrap gap-2">;<div className="space-y-3 mb-6">;"
                      {service.features.slice(0, 4).map((feature, idx) => (<div key={idx} className="flex items-center space-x-3 text-sm text-gray-300">;"
      ...
      <section className="py-20 px-4 bg-black/30 relative">;"
        ...
        <div className="max-w-7xl mx-auto relative z-10">;"
          ...
          >;
            ...
            {innovative2025MicroSaasBatch.map((service, index) => {const IconComponent = getCategoryIcon(service.category),return (<motion.div;
                  }
                  key={service.id}
                  initial={{ "opacity":0, "y":20 }}
                  whileInView={{ "opacity":1, "y":0 }}
