...
const getServiceCategory = (if (service.category) return service.category,if (service.type) return service.type,return 'Other';) => {
  return $3;}
}
},// Helper function to get service pricing;
const getServicePricing = (if (service.pricing?.starter) return service.pricing.starter,if (service.pricing?.monthly) return `$${service.pricing.monthly) => {
  return $3;}
}/month`,if (service.price?.monthly) return `$${service.price.monthly}/month`,return 'Contact for pricing',},// Helper function to get service features;
const getServiceFeatures = (if (service.features) return service.features,if (service.keyFeatures) return service.keyFeatures,return [];) => {
  return $3;}
}
},// Helper function to get service description;
const getServiceDescription = (if (service.description) return service.description,if (service.tagline) return service.tagline,return '';) => {
  return $3;}
}
  ...
  const getCategoryIcon = (const cat = categories.find(c => c.name === category),return cat ? cat.icon :Star;) => {
  return $3;}
}
  },const getCategoryColor = (const cat = categories.find(c => c.name === category),return cat ? cat.color :'from-gray-500 to-gray-600';) => {
  return $3;}
}
  },return (<Layout />;
      <SEO ;
