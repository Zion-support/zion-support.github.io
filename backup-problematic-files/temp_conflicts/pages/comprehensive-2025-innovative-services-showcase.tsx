...
const getServicePricing = (service:any) => {if (service.pricing?.starter) return service.pricing.starter,if (service.pricing?.monthly) return `$${service.pricing.monthly}/month`,if (service.price?.monthly) return `$${service.price.monthly}/month`,return 'Contact for pricing',},// Helper function to get service features;
const getServiceFeatures = (service:any) => {if (service.features) return service.features,if (service.keyFeatures) return service.keyFeatures,return [];
...
const getServiceDescription = (service:any) => {if (service.description) return service.description,if (service.tagline) return service.tagline,return '';
  ...
  const getCategoryIcon = (category:string) => {const cat = categories.find(c => c.name === category),return cat ? cat.icon :Star;
  },const getCategoryColor = (category:string) => {const cat = categories.find(c => c.name === category),return cat ? cat.color :'from-gray-500 to-gray-600';
  },return (<Layout>;
      <SEO ;
          ...
          >;
            ...
            {innovative2025MicroSaasBatch.filter(service => service.popular).map((service, index) => {const IconComponent = getCategoryIcon(service.category),return (<motion.div;
                ...
                <div className={`p-6 bg-white/5 ${viewMode === 'list' ? 'lg:w-2/3' :''}`}>;
                  <p className="text-gray-300 mb-4">{service.description}</p>;{/* Key Benefits */}
                  <div className="mb-4">;
