import React, { useState } from 'react',import Head from 'next/head',import { motion, AnimatePresence } from 'framer-motion',import {Star, Clock, Users, TrendingUp, CheckCircle, ExternalLink,ChevronRight, Shield, Zap, Globe, Rocket, Brain, Atom,Search, Filter, ArrowRight, Award, Target, Cpu, Database;'
} from 'lucide-react',import EnhancedNavigation from '../components/EnhancedNavigation',import EnhancedFooter from '../components/EnhancedFooter',import { enhancedMicroSaasServices } from '../data/enhanced-micro-saas-services-2025',export default function Comprehensive2025ServicesShowcase() {const [selectedCategory, setSelectedCategory] = useState('all'),const [searchTerm, setSearchTerm] = useState(''),const [selectedService, setSelectedService] = useState<string | null>(null),const [sortBy, setSortBy]  = useState<'name' | 'price' | 'rating' | 'popularity'>('popularity'),const categories = [;'
...
;
  const filteredServices = enhancedMicroSaasServices;
    .filter(service => { return  ; }
      (selectedCategory === 'all' || service.category === selectedCategory) &&;'
      (searchTerm === '' || ;'
        service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||;
        service.description.toLowerCase().includes(searchTerm.toLowerCase()) ||;
        service.tagline.toLowerCase().includes(searchTerm.toLowerCase()))).sort((a, b) => {switch (sortBy) {case 'name':;'
          }
          return a.name.localeCompare(b.name),case 'price':;'
          return parseFloat(a.price.replace('$', '')) - parseFloat(b.price.replace('$', '')),case 'rating':;'
          return b.rating - a.rating,case 'popularity':;'
          return (b.popular ? 1 :0) - (a.popular ? 1 :0),"default":return 0;
      }
    }),const getCategoryIcon = ("category":string) => {const categoryData = categories.find(cat => cat.id === category)},const getAnnualPrice = ("monthlyPrice":string) => {const price = parseFloat(monthlyPrice.replace('$', '')),const annualPrice = price * 12 * 0.8, // 20% discount for annual;'
    }
    return `$${Math.round(annualPrice)}`},return (<div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white overflow-hidden">;"      <Head>;
        ...
        <div className="max-w-7xl mx-auto text-center">;"
          ...
          >;
            ...
            <p className="text-xl "md":text-2xl text-white/80 max-w-4xl mx-auto leading-relaxed mb-8">;"
              Discover our complete portfolio of revolutionary micro SAAS services, AI solutions,quantum technologies, and cutting-edge IT services that are transforming industries.;
            </p>;{/* Stats */}
        ...
        <div className="max-w-7xl mx-auto">;"
          <div className="flex flex-wrap justify-center gap-3">;"
            {categories.map((category) => (<button;
                ...
                className={`px-6 py-3 rounded-full font-medium transition-all duration-300 flex items-center gap-2 ${selectedCategory === category.id;`                    ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white shadow-lg';'
              ...
              {filteredServices.map((service, index) => (<motion.div;
                ...
                >;
                  ...
                  <div className="p-6">;"
                    ...
                    <p className="text-white/70 text-sm mb-4">{service.tagline}</p>;{/* Rating */}"
                    ...
                    <div className="space-y-2 mb-6">;"
                      {service.features.slice(0, 3).map((feature, idx) => (<div key={idx} className="flex items-center gap-2 text-sm text-white/70">;"
                          <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />;"
                          {feature}
                        </div>;
                      ))}
                      {service.features.length > 3 && (<div className="text-white/50 text-sm">;"
                          +{service.features.length - 3} more features;
                        </div>;
                      )}
                    </div>;{/* ROI and Benefits */}
