import React from 'react',import Head from 'next/head',import { Check, Shield, ShieldAlert, Lock, Phone, Mail, MapPin, ExternalLink } from 'lucide-react',import UltraAdvancedFuturisticBackground from '../components/ui/UltraAdvancedFuturisticBackground',import Card from '../components/ui/Card',import Button from '../components/ui/Button',export default function AIGuardrailsPage() {const contactInfo = {"mobile":'+1 302 464 0950',"email":'kleber@ziontechgroup.com',"address":'364 E Main St STE 1008 Middletown DE 19709',"website":'"https"://ziontechgroup.com';'
	},const features = [;
		'Prompt/output policy engine with allow/deny listsJailbreak and prompt injection detection','PII/PHI detection and redactionToxicity, bias and safety classifiers','Human handoff flows and escalation policiesAudit logs, alerting and exportable evidence';'
...
;
	return (;
		<UltraAdvancedFuturisticBackground>;
			<Head>;
			...
			</Head>;<section className="min-h-screen pt-28 pb-20 px-4">;"
				<div className="max-w-6xl mx-auto">;"
					...
					</div>;<div className="grid grid-cols-1 "lg":grid-cols-3 gap-6 mb-10">;"
						<Card className=""lg":col-span-2 bg-black/30 border border-rose-500/30">;"
							<div className="p-6">;"
								...
								<ul className="grid grid-cols-1 "md":grid-cols-2 gap-3">;"
									{features.map((f) => (<li key={f} className="flex items-start gap-2 text-gray-200"><Check className="w-5 h-5 text-emerald-400 mt-0.5" /> <span>{f}</span></li>;"
									))}
								</ul>;
							</div>;
						</Card>;
					...
					</div>;<div className="bg-black/20 border border-rose-500/30 rounded-2xl p-6">;"
						<h3 className="text-xl font-semibold text-white mb-4 text-center">Contact</h3>;"
						<div className="grid grid-cols-1 "md":grid-cols-3 gap-6 text-sm">;"
							<div className="flex items-center justify-center gap-2 text-cyan-400"><Phone className="w-4 h-4" /> <a href={`"tel":${contactInfo.mobile.replace(/[^+\d]/g, '')}`}>{contactInfo.mobile}</a></div>;`							<div className="flex items-center justify-center gap-2 text-purple-400"><Mail className="w-4 h-4" /> <a href={`"mailto":${contactInfo.email}`}>{contactInfo.email}</a></div>;`							<div className="flex items-center justify-center gap-2 text-emerald-400"><MapPin className="w-4 h-4" /> <a href={`"https"://maps.google.com/?q=${encodeURIComponent(contactInfo.address)}`} target="_blank" rel="noopener noreferrer" className="text-xs">{contactInfo.address}</a></div>;"
						</div>;
