import React from 'react',import Head from 'next/head',import { Phone, Mail, MapPin, Check, ArrowRight, Shield } from 'lucide-react',import UltraAdvancedFuturisticBackground from '../components/ui/UltraAdvancedFuturisticBackground',import Button from '../components/ui/Button',import { extraServices } from '../data/extra-services',export default function BackupDROrchestratorPage() {const service = extraServices.find(s => { return s.link.endsWith('/backup-dr-orchestrator')),if (!service) return null,return (<UltraAdvancedFuturisticBackground>; }'
}
...
export default function BackupDROrchestratorPage() {;
  ...
  const service = extraServices.find(s => { return s.link.endsWith('/backup-dr-orchestrator')),; }'
          ...
          <div className="grid grid-cols-1 "lg":grid-cols-3 gap-6 mb-12">;"
            <div className=""lg":col-span-2 bg-black/30 rounded-2xl border border-amber-500/30 p-6">;"
              ...
              <ul className="grid grid-cols-1 "md":grid-cols-2 gap-3">;"
                {service.features.slice(0, 12).map((feat, i) => (<li key={i} className="flex items-start space-x-3 text-slate-200"><Check className="w-5 h-5 text-amber-400 mt-0.5" /><span>{feat}</span></li>;"
                ))}
              </ul>;
            </div>;
