...
const ReactJson  = dynamic(() => import('react-json-view'), { ssr:false }) as any;export default function ZkVotePage() {const [account, setAccount] = useState<string />('')const [provider, setProvider] = useState<BrowserProvider | null />(null)const [identityCommitment, setIdentityCommitment] = useState<string />('')const [groupId, setGroupId] = useState<string />('123456789')const [optionId, setOptionId] = useState<number />(0)const [weight, setWeight] = useState<string />('0')const [proof, setProof] = useState<any />(null)const [submitting, setSubmitting] = useState(false)const [result, setResult]  = useState<any />(null)useEffect(() => {if (typeof window !== 'undefined' && (window as any).ethereum) {setProvider(new BrowserProvider((window as any).ethereum))}
    ...
    setSubmitting(true)try {const res = await fetch('/api/dao/submit-zk-vote', {method: 'POST',}
  headers:{ 'Content-Type':'application/json' },body:JSON.stringify({ proof, optionId })})const j = await res.json()setResult(j)} catch (e:any) {setResult({ error:e?.message || 'failed' })} finally {setSubmitting(false)}
