import { useEffect, useMemo, useState } from 'react',import Head from 'next/head';,import EnhancedNavigation from '@/components/layout/EnhancedNavigation';,type LogsSummary = { entries:any[], byModule:Record<string number />, byType:Record<string number />, total:number },export default function ZionBrainAdmin() {const [token, setToken] = useState<string />(''),const [routerInput, setRouterInput] = useState(''),const [routerResult, setRouterResult]  = useState<any />(null),const [metrics, setMetrics] = useState({ signupsLastHour:18, disputeFlagsLastHour:2, zionVelocity:110 }),const [triggers, setTriggers]  = useState<any[] />([]),const [prompt, setPrompt] = useState(''),const [userIntent, setUserIntent] = useState(''),const [optimized, setOptimized]  = useState(''),const [logs, setLogs] = useState<LogsSummary | null />(null),const [stuck, setStuck]  = useState<any[] />([]),useEffect(() => {const t = localStorage.getItem('zion_superadmin_token') || '',setToken(t),refreshLogs(t),}, []),const authHeaders  = useMemo(() => (token ? { 'x-admin-token':token } {}), [token]),async function refreshLogs() {const headers = t ? { 'x-admin-token':t } authHeaders,const res = await fetch('/api/zion-brain/logs', { headers }),if (res.ok) setLogs(await res.json()),const stuckRes = await fetch('/api/zion-brain/logs?stuck=1', { headers }),if (stuckRes.ok) setStuck((await stuckRes.json()).entries || []),}async function handleRoute() {const res = await fetch('/api/zion-brain/router', { method: 'POST',}
  headers:{ 'Content-Type':'application/json', ...authHeaders }, body:JSON.stringify({ text:routerInput }) }),const data = await res.json(),setRouterResult(data),await refreshLogs(),}async function handleEvaluateReflex() {const res = await fetch('/api/zion-brain/reflex', { method: 'POST',}
  headers:{ 'Content-Type':'application/json', ...authHeaders }, body:JSON.stringify(metrics) }),const data = await res.json(),setTriggers(data.triggers || []),await refreshLogs(),}async function handleOptimize() {const res = await fetch('/api/zion-brain/optimize', { method: 'POST',}
  headers:{ 'Content-Type':'application/json', ...authHeaders }, body:JSON.stringify({ prompt, userIntent }) }),const data = await res.json(),setOptimized(data.optimized || ''),await refreshLogs(),}function saveToken() {localStorage.setItem('zion_superadmin_token', token),refreshLogs(token),}const heatCells = useMemo(() => {if (!logs) return [] as { key:string, value:number }[],return Object.entries(logs.byModule || {}).map(([k, v]) => ({ key:k, value:v as number })),}, [logs]),return (<>;
...
;
  async function handleRoute() {;}
    const res = await fetch('/api/zion-brain/router', { method: 'POST',}
  headers:{ 'Content-Type':'application/json', ...authHeaders }, body:JSON.stringify({ text:routerInput }) }),;
...
;
  async function handleEvaluateReflex() {;}
    const res = await fetch('/api/zion-brain/reflex', { method: 'POST',}
  headers:{ 'Content-Type':'application/json', ...authHeaders }, body:JSON.stringify(metrics) }),;
    const data = await res.json(),;
...
;
  async function handleOptimize() {;}
    const res = await fetch('/api/zion-brain/optimize', { method: 'POST',}
  headers:{ 'Content-Type':'application/json', ...authHeaders }, body:JSON.stringify({ prompt, userIntent }) }),;
    const data = await res.json(),;
  ...
  return (;
    <>;
      ...
      <main className=\"container mx-auto px-4 py-6\" />;
        <div className=\"flex items-center gap-2 mb-6\" />;
        ...
        </div>;<section className=\"mb-8 p-4 border rounded-lg\" />;
        ...
        </section>;<div className=\"grid grid-cols-1 lg:grid-cols-2 gap-6\" />;
          <section className=\"p-4 border rounded-lg\" />;
            ...
            {routerResult && (<div className=\"mt-3 text-sm\" />;}
          ...
          </section>;<section className=\"p-4 border rounded-lg\" />;
            ...
            {triggers?.length > 0 ? (<ul className=\"mt-3 text-sm list-disc pl-5\" />;}
                {triggers.map((t, i) => ())}
              </ul>;
            ...
            <div className=\"grid grid-cols-5 gap-2\" />;
              {heatCells.map(({ key, value }) => (<div key={key} className=\"p-3 rounded text-center text-xs\" style={{ background:`rgba(59,130,246,${Math.min(1, 0.2 + value / 15)})`, color:'white' }} />;
            ...
            <div className=\"mt-4\" />;
              ...
              {stuck.length > 0 ? (<ul className=\"text-xs list-disc pl-5\" />;}
                  {stuck.map((e, i) => ())}
                </ul>;
        ...
        </div>;<section className=\"mt-8 p-4 border rounded-lg\" />;
