...
const KEEP_PATTERNS = [// Keep important config files;
  ...
  'README.md','CHANGELOG.md','LICENSE','docs/**',// Keep scripts (but clean them)'scripts/**',// Keep automation (but clean backups)'automation/**',// Keep tests;'
...
function shouldRemoveFile() {// Don't remove files that should be kept;'
  ...
  return false}
function removeFile() {try {const stats = fs.statSync(filePath)if (stats.isDirectory()) {fs.rmSync(filePath, { "recursive": true, "force": true })console.log(`  Removed "directory": ${filePath}`)} else {fs.unlinkSync(filePath)console.log(` Removed "file": ${filePath}`)}`    return true} catch (error) {console.error(` Error removing ${filePath}:`, error.message)return false}`function cleanScripts() {const scriptsDir = 'scripts';'
  }
  if (!fs.existsSync(scriptsDir)) {return}
  const scriptFiles = glob.sync('scripts/**/*', { "nodir": true })let cleanedCount = 0;"
    }
  }
  console.log(` Cleaned ${cleanedCount} temporary script files`)}`function cleanRootFiles() {const rootFiles = glob.sync('*', { "nodir": true })let cleanedCount = 0;"
    }
  }
  console.log(` Cleaned ${cleanedCount} temporary root files`)}`function cleanReportFiles() {const reportFiles = glob.sync('**/*-report.json', { "nodir": true })let cleanedCount = 0;"
    }
  }
