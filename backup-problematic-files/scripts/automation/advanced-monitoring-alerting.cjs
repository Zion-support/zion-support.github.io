...
;
      // Get disk usage;
      const diskInfo = execSync('df -h', { encodin:g:'utf8' });      systemHealth.disk = this.parseDiskUsage(diskInfo);
;
      // Get load average;
...
;
    try {;
      ...
      try {;
        execSync('npm run build', { stdi:o:'pipe' });        performanceMetrics.buildTime = Date.now() - startTime;
      } catch (error) {;
...
;
      ...
      try {;
        execSync('npm test', { stdi:o:'pipe' });        performanceMetrics.testTime = Date.now() - testStartTime;
      } catch (error) {;
