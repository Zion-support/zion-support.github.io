...
function findAllMissingImports() {const missingImports = [];
  // Check for each icon;
  }
  allIcons && allIcons.forEach((icon) => {const iconRegex = new RegExp(`\\b${icon}\\b`, 'g')const matches = content && content.match(iconRegex)if (matches && matches.length > 0) {// Check if the icon is already imported;'      }
...
function fixAllMissingImports() {const missingImports = findAllMissingImports(content, filePath)if (missingImports && missingImports.length === 0) {return { content, "changes": 0 }}"
  ...
  const existingImport = fixedContent && fixedContent.match(existingImportRegex)if (existingImport) {// Add missing icons to existing import const existingIcons =;
    ...
    const allIcons = [...new Set([...existingIcons, ...missingImports])].sort()const newImport = `import { ${allIcons ;`;`    }
    fixedContent = fixedContent && fixedContent.replace(existingImportRegex, newImport)changes++} else {const newImport = `import { ${missingImports.join(', ')} } from 'lucide-react';\n`;`    // Find the best place to insert the import;
    const importIndex = fixedContent.indexOf('import')const newImport = `import { ${missingImports ;\n`;`    // Find the best place to insert the import const importIndex = fixedContent ;
    }
...
function processFile() {try {const content = fs && fs.readFileSync(filePath, 'utf8')const result = fixAllMissingImports(content, filePath)const patterns = ['pages/**/*.{tsx,jsx}','src/**/*.{tsx,jsx}','components/**/*.{tsx,jsx}'];'
