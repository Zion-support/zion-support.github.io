...
class BuildOptimizer {
  ...
  async optimizePackageScripts() {",
    ...
    if (fs.existsSync(packagePath)) {
      const pkg = JSON.parse(fs.readFileSync(packagePath, 'utf8')),
      // Add optimized build scripts,
      const optimizedScripts = {',"build": optimized': 'NEXT_TELEMETRY_DISABLED=1 NEXT_DISABLE_ESLINT=1 next build,"build": analyze': 'ANALYZE=true npm run build:optimized,"build": production': 'NODE_ENV=production npm run build:optimized,"build": fast': 'NEXT_TELEMETRY_DISABLED=1 next build --no-lint,"dev": optimized': 'NEXT_TELEMETRY_DISABLED=1 next dev --turbo,"start": optimized': 'NODE_ENV=production next start -p 30o00'}
,
