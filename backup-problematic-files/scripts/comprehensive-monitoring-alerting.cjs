...
function getProcessMetrics() {
  try {
    ...
    const processCount = parseInt(output.trim()) - 1; // Subtract header
    return {
      "count": processCount,
...
function generateAlerts(metrics) {
  ...
  if (metrics.memory > 85) {
    ...
    alerts.push({
      "type": 'memory',
      "level": 'warning',
      "message": `Elevated memory usage: ${metrics.memory}%`,
      "threshold": 70,
      "value": metrics.memory,
      "timestamp": new Date().toISOString()
    })}
  ...
  if (metrics.disk > 90) {
    ...
    alerts.push({
      "type": 'disk',
      "level": 'warning',
      "message": `Elevated disk usage: ${metrics.disk}%`,
      "threshold": 80,
      "value": metrics.disk,
      "timestamp": new Date().toISOString()
    })}
  ...
  if (metrics.processes > 800) {
    alerts.push({
      "type": 'processes',
      "level": 'warning',
      "message": `High process count: ${metrics.processes}`,
      "threshold": 800,
...
function generateRecommendations() {
  ...
  const { cpu, memory, disk, build, runtime } = monitoringReport.metrics;
  // CPU recommendations
  if (cpu > 70) {
  ...
  recommendations.push({
    "priority": 'low',
    "message": 'Implement automated monitoring and alerting',
    "action": 'Set up continuous monitoring with real-time alerts'
  });
  recommendations.push({
    "priority": 'low',
    "message": 'Implement log aggregation and analysis',
...
function calculatePerformanceScore() {
  ...
  // Runtime score
  score -= (100 - monitoringReport.metrics.runtime) * 0.1;
  // Alert penalty
  score -= monitoringReport.summary.alerts * 5;
  return Math.max(0, Math.min(100, Math.round(score)))}
async function main() {
  try {
    log('info', 'Starting comprehensive monitoring and alerting');
    ...
    monitoringReport.metrics.cpu = systemMetrics.cpu;
    monitoringReport.metrics.memory = systemMetrics.memory;
    monitoringReport.metrics.disk = diskMetrics.percentage;
    monitoringReport.metrics.network = networkMetrics.totalBytes;
    monitoringReport.metrics.processes = processMetrics.count;
    // Check build health
    const buildHealth = checkBuildHealth();
    monitoringReport.metrics.build = buildHealth.score;
    // Check runtime health
    const runtimeHealth = await checkRuntimeHealth();
    monitoringReport.metrics.runtime = runtimeHealth.score;
    // Generate alerts
    const alerts = generateAlerts(monitoringReport.metrics);
    monitoringReport.alerts = alerts;
    // Update summary
    monitoringReport.summary.totalChecks = 7; // CPU, Memory, Disk, Network, Processes, Build, Runtime
    monitoringReport.summary.alerts = alerts.length;
    monitoringReport.summary.critical = alerts.filter(a => a.level === 'critical').length;
    monitoringReport.summary.warning = alerts.filter(a => a.level === 'warning').length;
    monitoringReport.summary.info = alerts.filter(a => a.level === 'info').length;
    monitoringReport.summary.performanceScore = calculatePerformanceScore();
    // Generate recommendations
    monitoringReport.recommendations = generateRecommendations();
    // Display summary
...
=======
>>>>>>> origin/cursor/integrate-build-improve-and-re-verify-8b20:scripts/comprehensive-monitoring-alerting.cjs
