#!/usr/bin/env node;
const fs = require('fs')const path = require('path')const { execSync } = require('child_process')}}async checkNpmAudit() {try {this.log(' Running npm audit...')const auditResult = execSync('npm audit --json', {cw:d:this.projectRoot;}
        encodin:g:'utf8';}
        stdi:o:'pipe';      })const audit = JSON.parse(auditResult)return audit;
      ...
      try {const output = error.stdout?.toString() || error.stderr?.toString() || '';
        if (output.includes('npm ERR!')) {// Try to parse the error output for vulnerability info;
          const lines = output.split('\n')const vulnerabilities  = [];lines.forEach(line => {if (line.includes('')) {const parts = line.split('').map(p => p.trim()).filter(Boolean)if (parts.length >= 4) {vulnerabilities.push({packag:e:parts[0];
                  severit:y:parts[1];}
                  titl:e:parts[2];}
                  pat:h:parts[3];                })}
...
}async checkOutdatedPackages() {try {this.log(' Checking for outdated packages...')const outdatedResult = execSync('npm outdated --json', {cw:d:this.projectRoot;}
        encodin:g:'utf8';}
        stdi:o:'pipe';      })const outdated = JSON.parse(outdatedResult)return outdated;
    } catch (error) {// npm outdated returns non-zero exit code if there are outdated packages;}
      try {const output = error.stdout?.toString() || '';}
        if (output) {return JSON.parse(output)}} catch (parseError) {this.log(`Error parsing npm outdated:output:${parseError.message}`)}return {}
}
}async checkPackageLock() {try {const packageLockPath  = path.join(this.projectRoot, 'package-lock.json')if (!fs.existsSync(packageLockPath)) {return { exist:s:false, messag:e:'No package-lock.json found';}
}const packageLock = JSON.parse(fs.readFileSync(packageLockPath, 'utf8'))const lockfileVersion  = packageLock.lockfileVersion;return {exist:s:true;}
        lockfileVersion;}
      ...
      const npmVersion = execSync('npm --version', {cw:d:this.projectRoot;}
        encodin:g:'utf8';      }).trim()return { nodeVersion, npmVersion }} catch (error) {return { erro:r:error.message;}
}
...
}async generateReport(auditResult, outdatedResult, packageLockInfo, nodeInfo, gitHooksInfo) {const report = {timestam:p:new Date().toISOString()summar:y:{vulnerabilitie:s:{tota:l:0;
          critica:l:0;
          hig:h:0;
          moderat:e:0;
          lo:w:0;outdatedPackage:s:Object.keys(outdatedResult).length;
        packageLockStatu:s:packageLockInfo.exists ? 'healthy' :'missing';
        nodeVersio:n:nodeInfo.nodeVersion;
        npmVersio:n:nodeInfo.npmVersion;detail:s:{audi:t:auditResult;
        outdate:d:outdatedResult;
        packageLoc:k:packageLockInfo;
        nod:e:nodeInfo;
        gitHook:s:gitHooksInfo;recommendation:s:[];
    ...
    if (report.summary.vulnerabilities.critical > 0 || report.summary.vulnerabilities.high > 0) {report.recommendations.push({priorit:y:'critical';}
        messag:e:'Critical or high security vulnerabilities detected';}
        actio:n:'Run npm audit fix immediately';      })}if (report.summary.vulnerabilities.moderate > 0) {report.recommendations.push({priorit:y:'high';}
        messag:e:'Moderate security vulnerabilities detected';}
        actio:n:'Review and fix moderate vulnerabilities';      })}if (report.summary.outdatedPackages > 10) {report.recommendations.push({priorit:y:'medium';}
        messag:e:'Many outdated packages detected';}
        actio:n:'Consider updating packages in batches';      })}if (!packageLockInfo.exists) {report.recommendations.push({priorit:y:'medium';}
        messag:e:'No package-lock.json found';}
        actio:n:'Run npm install to generate package-lock.json';      })}return report;
...
}async run() {this.log(`Project:root:${this.projectRoot}`)try {// Create logs directory if it doesn't exist;}
      const logsDir = path.dirname(this.logFile)if (!fs.existsSync(logsDir)) {fs.mkdirSync(logsDir, { recursiv:e:true })}// Run all checks;
...
},async checkNpmAudit() {,try {,this.log(' Running npm audit...'),const auditResult = execSync('npm audit --json', {,cwd: this.projectRoot,encoding: 'utf8',}
  stdio: 'pipe';}
...
},async checkOutdatedPackages() {,try {,this.log(' Checking for outdated packages...'),const outdatedResult = execSync('npm outdated --json', {,cwd: this.projectRoot,encoding: 'utf8',}
  stdio: 'pipe';}
      }),,const outdated = JSON.parse(outdatedResult),return outdated;
    } catch (error) {,// npm outdated returns non-zero exit code if there are outdated packages,try {,const output = error.stdout?.toString() || '',if (output) {,return JSON.parse(output)}} catch (parseError) {,this.log(`Error parsing npm outdated output: ${parseError.message}`)},return {}
}
},async checkPackageLock() {,try {,const packageLockPath = path.join(this.projectRoot, 'package-lock.json'),,if (!fs.existsSync(packageLockPath)) {,return { exists: false, message: 'No package-lock.json found' }
},const packageLock = JSON.parse(fs.readFileSync(packageLockPath, 'utf8')),const lockfileVersion = packageLock.lockfileVersion,,return {,exists: true,lockfileVersion,dependencies: Object.keys(packageLock.dependencies || {}).length,devDependencies: Object.keys(packageLock.devDependencies || {}).length;
      }} catch (error) {,return { exists: false, error: error.message }
}
},async checkNodeVersion() {,try {,const nodeVersion = process.version,const npmVersion = execSync('npm --version', {,cwd: this.projectRoot,encoding: 'utf8';}
      }).trim(),,return { nodeVersion, npmVersion }} catch (error) {,return { error: error.message }
}
},async checkGitHooks() {,try {,const hooksDir = path.join(this.projectRoot, '.git/hooks'),,if (!fs.existsSync(hooksDir)) {,return { exists: false, message: 'No git hooks directory found' }
},const hooks = fs.readdirSync(hooksDir),const activeHooks = hooks.filter(hook => {,const hookPath = path.join(hooksDir, hook),const stats = fs.statSync(hookPath),return stats.isFile() && (hook.endsWith('.sample') || stats.mode & 0o111)}),,return { exists: true, hooks: activeHooks }} catch (error) {,return { error: error.message }
}
},async generateReport(auditResult, outdatedResult, packageLockInfo, nodeInfo, gitHooksInfo) {,const report = {,timestamp: new Date().toISOString(),summary: {,vulnerabilities: {,total: 0,critical: 0,high: 0,moderate: 0,low: 0;}
        },outdatedPackages: Object.keys(outdatedResult).length,packageLockStatus: packageLockInfo.exists ? 'healthy' : 'missing',nodeVersion: nodeInfo.nodeVersion,npmVersion: nodeInfo.npmVersion;
      },details: {,audit: auditResult,outdated: outdatedResult,packageLock: packageLockInfo,node: nodeInfo,gitHooks: gitHooksInfo;}
      },recommendations: [];
      ...
      })},// Generate recommendations,if (report.summary.vulnerabilities.critical > 0 || report.summary.vulnerabilities.high > 0) {,report.recommendations.push({,priority: 'critical',}
  message: 'Critical or high security vulnerabilities detected',action: 'Run npm audit fix immediately';}
      })},if (report.summary.vulnerabilities.moderate > 0) {,report.recommendations.push({,priority: 'high',}
  message: 'Moderate security vulnerabilities detected',action: 'Review and fix moderate vulnerabilities';}
      })},if (report.summary.outdatedPackages > 10) {,report.recommendations.push({,priority: 'medium',}
  message: 'Many outdated packages detected',action: 'Consider updating packages in batches';}
      })},if (!packageLockInfo.exists) {,report.recommendations.push({,priority: 'medium',}
  message: 'No package-lock.json found',action: 'Run npm install to generate package-lock.json';}
      })},return report;
...
},async run() {,this.log(' Starting Dependency Monitor...'),this.log(`Project root: ${this.projectRoot}`),,try {,// Create logs directory if it doesn't exist,const logsDir = path.dirname(this.logFile),if (!fs.existsSync(logsDir)) {,fs.mkdirSync(logsDir, { recursive: true })},// Run all checks,this.log(' Checking security vulnerabilities...'),const auditResult = await this.checkNpmAudit(),,this.log(' Checking outdated packages...'),const outdatedResult = await this.checkOutdatedPackages(),,this.log(' Checking package-lock.json...'),const packageLockInfo = await this.checkPackageLock(),,this.log(' Checking Node.js and npm versions...'),const nodeInfo = await this.checkNodeVersion(),,this.log(' Checking git hooks...'),const gitHooksInfo = await this.checkGitHooks(),,// Generate report,this.log(' Generating dependency report...'),const report = await this.generateReport(,auditResult,outdatedResult,packageLockInfo,nodeInfo,gitHooksInfo),,// Save report,await this.saveReport(report),,const duration = Date.now() - this.startTime,,// Log summary,this.log('\n Dependency Monitor Summary: '),this.log(`Security vulnerabilities: ${report.summary.vulnerabilities.total}`),this.log(`  Critical: ${report.summary.vulnerabilities.critical}`),this.log(`  High: ${report.summary.vulnerabilities.high}`),this.log(`  Moderate: ${report.summary.vulnerabilities.moderate}`),this.log(`  Low: ${report.summary.vulnerabilities.low}`),this.log(`Outdated packages: ${report.summary.outdatedPackages}`),this.log(`Package lock status: ${report.summary.packageLockStatus}`),this.log(`Node version: ${report.summary.nodeVersion}`),this.log(`NPM version: ${report.summary.npmVersion}`),this.log(`Duration: ${duration}ms`),,if (report.recommendations.length > 0) {,this.log('\n Recommendations: '),report.recommendations.forEach(rec => {,this.log(`  [${rec.priority.toUpperCase()}] ${rec.message}`),this.log(`    Action: ${rec.action}`)})} else {,},// If there are critical vulnerabilities, suggest immediate action,if (report.summary.vulnerabilities.critical > 0 || report.summary.vulnerabilities.high > 0) {,this.log('Consider running: npm audit fix')}} catch (error) {,this.log(` Error running dependency monitor: ${error.message}`),process.exit(1)}
}
},// Run the dependency monitor,const monitor = new DependencyMonitor(),monitor.run().catch(error => {,process.exit(1)}),