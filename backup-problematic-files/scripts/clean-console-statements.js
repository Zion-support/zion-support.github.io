...
#!/usr/bin/env node,"}),"}) ,"}),"}) import fs from,"}),"}) 'fs',"}),"}) import path from,"}),"}) 'path',"}),"}) import { fileURLToPath } from,"}),"}) 'url',"}),"}) ,"}),"});
  ...
  const __dirname = path.dirname(__filename),"}),"}) ,"}),"}) class ConsoleCleaner {,"}),"});

    constructor() {,"}),"}) this.projectRoot = process.cwd(),"}),"}) this.cleanedFiles = [],"}),"}) this.totalConsoleStatements = 0,"}),"}) }"}),"}) ,"}),"}) async cleanConsoleStatements() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) for (const dir of directories) {,"}),"});

  const dirPath = path.join(this.projectRoot,dir),"}),"}) if (fs.existsSync(dirPath)) {,"}),"}) await this.processDirectory(dirPath),"}),"}) }"}),"}) }"}),"}) ,"}),"}) ,"}) `✅ Cleaned console statements from ${this.cleanedFiles.length} files`,"}),"}) ),"}),"}) ,"}) `📊 Total console statements removed: ${this.totalConsoleStatements}`,"}),"}) ),"}),"}) }"}),"}) ,"}),"}) async processDirectory(dirPath) {,"}),"});

  const items = fs.readdirSync(dirPath),"}),"}) ,"}),"}) for (const item of items) {,"}),"});

  const itemPath = path.join(dirPath,item),"}),"});

  const stat = fs.statSync(itemPath),"}),"}) ,"}),"}) if (stat.isDirectory()) {,"}),"}) await this.processDirectory(itemPath),"}),"}) } else if (this.isJavaScriptFile(item)) {,"}),"}) await this.cleanFile(itemPath),"}),"}) }"}),"}) }"}),"}) }"}),"}) ,"}),"});

    isJavaScriptFile(filePath) {,"}),"});

  const ext = path.extname(filePath),"}),"}) return [,"}),"}) '.js',"}),"}) '.jsx',"}),"}) '.ts',"}),"}) '.tsx'].includes(ext),"}),"}) }"}),"}) ,"}),"}) async cleanFile(filePath) {,"}),"}) try {,"}),"});

  const content = fs.readFileSync(filePath,"}),"}) 'utf8'),"}),"});

  const originalContent = content,"}),"}) ,"}),"}) let cleanedContent = content,"}),"}) .replace(/console\.log([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.warn([^)]*),?\s*/g,''),"}),"}) .replace(/console\.info([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.debug([^)]*),?\s*/g,''),"}),"}) .replace(/console\.trace([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.table([^)]*),?\s*/g,''),"}),"}) .replace(/console\.group([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.groupEnd([^)]*),?\s*/g,''),"}),"}) .replace(/console\.time([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.timeEnd([^)]*),?\s*/g,''),"}),"}) .replace(/console\.count([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.clear([^)]*),?\s*/g,''),"}),"}) .replace(/console\.assert([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.dir([^)]*),?\s*/g,''),"}),"}) .replace(/console\.dirxml([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.profile([^)]*),?\s*/g,''),"}),"}) .replace(/console\.profileEnd([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.timeStamp([^)]*),?\s*/g,''),"}),"}) .replace(/console\.markTimeline([^)]*),?\s*/g,',"}),"}) '),"}),"}) .replace(/console\.timeline([^)]*),?\s*/g,''),"}),"}) .replace(/console\.timelineEnd([^)]*),?\s*/g,',"}),"}) '),"}),"}) ,"}),"});
