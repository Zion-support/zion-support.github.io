...
#!/usr/bin/env node,"}),"}) ,"}),"}) import { existsSync,statSync } from,"}),"}) fs',"}),"}) import { join } from,"}),"}) 'path',"}),"}) ,"}),"}) function log(message) {,"}),"}) ,"}),"}) }"}),"}) ,"}),"}) try {,"}),"});

  const buildDir = join(process.cwd(),"}),"}) '.next'),"}),"}) ,"}),"}) if (!existsSync(buildDir) || !statSync(buildDir).isDirectory()) {,"}),"});

    log(,"}),"}) 'warning: .next directory not found. Skipping post-build checks.),"}),"}) process.exit(0),"}),"}) }"}),"}) ,"}),"});

    log(,"}),"}) 'Next.js build output verified. No additional optimizations applied.'),"}),"}) } catch (error) {,"}),"}) console.error(,"}),"}) '[optimize-build] error:',error?.message || error),"}),"}) process.exit(0),"}),"}) }"}),"}) ,"}),"}) ,"}),"}) import fs from,"}),"}) 'fs',"}),"}) import path from,"}),"}) 'path',"}),"}) import { execSync } from,"}),"}) 'child_process',"}),"}) import { fileURLToPath } from,"}),"}) 'url',"}),"}) ,"}),"});

  const __filename = fileURLToPath(import.meta.url),"}),"});

  const __dirname = path.dirname(__filename),"}),"}) ,"}),"}) class BuildOptimizer {,"}),"});

    constructor() {,"}),"}) this.projectRoot = process.cwd(),"}),"}) this.srcDir = path.join(this.projectRoot,"}),"}) 'src'),"}),"}) this.buildDir = path.join(this.projectRoot,"}),"}) '.next'),"}),"}) this.optimizationReport = {,"}),"}) timestamp: new Date().toISOString(),"}),"}) optimizations: [],"}),"}) warnings: [],"}),"}) errors: [],"}),"}) metrics: {},"}),"}) },"}),"}) }"}),"}) ,"}),"}) async optimize() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) try {,"}),"}) await this.cleanConsoleStatements(),"}),"}) await this.optimizeImages(),"}),"}) await this.optimizeCSS(),"}),"}) await this.optimizeJavaScript(),"}),"}) await this.generateSitemap(),"}),"}) await this.generateRobotsTxt(),"}),"}) await this.optimizeManifest(),"}),"}) await this.generateReport(),"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) } catch (error) {,"}),"}) console.error(,"}),"}) '❌ Build optimization failed:',error.message),"}),"}) this.optimizationReport.errors.push(error.message),"}),"}) await this.generateReport(),"}),"}) process.exit(1),"}),"}) }"}),"}) }"}),"}) ,"}),"}) async cleanConsoleStatements() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) let cleanedFiles = 0,"}),"}) ,"}),"}) for (const file of files) {,"}),"}) try {,"}),"}) let content = fs.readFileSync(file,"}),"}) 'utf8'),"}),"});

  const originalContent = content,"}),"}) ,"}),"}) if (process.env.NODE_ENV ===,"}),"}) 'production') {,"}),"}) content = content,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) }"}),"}) ,"}),"}) if (content !== originalContent) {,"}),"}) fs.writeFileSync(file,content),"}),"}) cleanedFiles++,"}),"}) }"}),"}) } catch (error) {,"}),"}) this.optimizationReport.warnings.push(,"}),"}) `Failed to clean console statements in ${file}: ${error.message}`,"}),"}) ),"}),"}) }"}),"}) }"}),"}) ,"}),"}) this.optimizationReport.optimizations.push(,"}),"}) `Cleaned console statements from ${cleanedFiles} files`,"}),"}) ),"}),"}) ,"}),"}) }"}),"}) ,"}),"}) async optimizeImages() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) if (!fs.existsSync(imageDir)) {,"}),"}) this.optimizationReport.warnings.push('Images directory not found,"}),"}) '),"}),"}) return,"}),"}) }"}),"}) ,"}),"});

  const images = fs,"}),"}) .readdirSync(imageDir),"}),"}) .filter(file => /\.(jpg|jpeg|png|gif|webp)$/i.test(file)),"}),"}) ,"}),"}) let optimizedImages = 0,"}),"}) ,"}),"}) for (const image of images) {,"}),"}) try {,"}),"});

  const imagePath = path.join(imageDir,image),"}),"});

  const stats = fs.statSync(imagePath),"}),"});

  const sizeKB = Math.round(stats.size / 1024),"}),"}) ,"}),"}) if (sizeKB > 100) {,"}),"}) this.optimizationReport.warnings.push(,"}),"}) `Large image detected: ${image} (${sizeKB}KB)`,"}),"}) ),"}),"}) }"}),"}) ,"}),"}) optimizedImages++,"}),"}) } catch (error) {,"}),"}) this.optimizationReport.warnings.push(,"}),"}) `Failed to optimize image ${image}: ${error.message}`,"}),"}) ),"}),"}) }"}),"}) }"}),"}) ,"}),"}) this.optimizationReport.optimizations.push(,"}),"}) `Processed ${optimizedImages} images`,"}),"}) ),"}),"}) ,"}),"}) }"}),"}) ,"}),"}) async optimizeCSS() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) let optimizedCSS = 0,"}),"}) ,"}),"}) for (const file of cssFiles) {,"}),"}) try {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) content = content,"}),"}) .replace(/\s+/g,',"}),"}) ') .replace(/\/\*[\s\S]*?\*\ .trim(),"}),"}) ,"}),"}) fs.writeFileSync(file,content),"}),"}) optimizedCSS++,"}),"}) } catch (error) {,"}),"}) this.optimizationReport.warnings.push(,"}),"}) `Failed to optimize CSS in ${file}: ${error.message}`,"}),"}) ),"}),"}) }"}),"}) }"}),"}) ,"}),"}) this.optimizationReport.optimizations.push(,"}),"}) `Optimized ${optimizedCSS} CSS files`,"}),"}) ),"}),"}) ,"}),"}) }"}),"}) ,"}),"}) async optimizeJavaScript() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) let optimizedJS = 0,"}),"}) ,"}),"}) for (const file of jsFiles) {,"}),"}) try {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) content = content,"}),"}) .replace(/\s+/g,"}),"}) ' ') .replace(/\/\/.*$/gm,',"}),"}) ') .replace(/\/\*[\s\S]*?\*\ .trim(),"}),"}) ,"}),"}) fs.writeFileSync(file,content),"}),"}) optimizedJS++,"}),"}) } catch (error) {,"}),"}) this.optimizationReport.warnings.push(,"}),"}) `Failed to optimize JavaScript in ${file}: ${error.message}`,"}),"}) ),"}),"}) }"}),"}) }"}),"}) ,"}),"}) this.optimizationReport.optimizations.push(,"}),"}) `Optimized ${optimizedJS} JavaScript files`,"}),"}) ),"}),"}) ,"}),"}) }"}),"}) ,"}),"}) async generateSitemap() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"});

  const sitemap = `<?xml version="1.0" encoding="UTF-8"?>,"}),"}) <urlset xmlns="http: <url>,"}),"}) <loc>https: <lastmod>${new Date().toISOString().split(,"}),"}) 'T)[0]}</lastmod>,"}),"}) <changefreq>daily</changefreq>,"}),"}) <priority>1.0</priority>,"}),"}) </url>,"}),"}) <url>,"}),"}) <loc>https: <lastmod>${new Date().toISOString().split(,"}),"}) T')[0]}</lastmod>,"}),"}) <changefreq>weekly</changefreq>,"}),"}) <priority>0.8</priority>,"}),"}) </url>,"}),"}) <url>,"}),"}) <loc>https: <lastmod>${new Date().toISOString().split(,"}),"}) 'T)[0]}</lastmod>,"}),"}) <changefreq>weekly</changefreq>,"}),"}) <priority>0.9</priority>,"}),"}) </url>,"}),"}) <url>,"}),"}) <loc>https: <lastmod>${new Date().toISOString().split(,"}),"}) 'T')[0]}</lastmod>,"}),"}) <changefreq>monthly</changefreq>,"}),"}) <priority>0.7</priority>,"}),"}) </url>,"}),"}) </urlset>`,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"});

  const robotsTxt = `User-agent: *,"}),"}) Allow: /,"}),"}) Sitemap: https: # Disallow admin and private areas,"}),"}) Disallow: /admin/,"}),"}) Disallow: /api/,"}),"}) Disallow: /_next/,"}),"}) Disallow: /private/`,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"});

  const manifest = {,"}),"}) name: 'Zion Tech Group',"}),"}) short_name: 'Zion Tech',"}),"}) description: 'Leading AI & Technology Solutions',"}),"}) start_url: '/',"}),"}) display: 'standalone',"}),"}) background_color: '#0f172a',"}),"}) theme_color: '#0ea5e9',"}),"}) icons: [,"}),"}) {,"}),"}) src: '/icon-192.png',"}),"}) sizes: '192x192',"}),"}) type: 'image/png',"}),"}) },"}),"}) {,"}),"}) src: '/icon-512.png',"}),"}) sizes: '512x512',"}),"}) type: 'image/png',"}),"}) },"}),"}) ],"}),"}) },"}),"}) ,"}),"}) fs.writeFileSync(,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) JSON.stringify(manifest,null,2),"}),"}) ),"}),"}) this.optimizationReport.optimizations.push(,"}),"}) 'Generated manifest.json'),"}),"}) ,"}) '✅ Generated manifest.json'),"}),"}) }"}),"}) ,"}),"}) async generateReport() {,"}),"}) this.optimizationReport.metrics = {,"}),"}) totalOptimizations: this.optimizationReport.optimizations.length,"}),"}) totalWarnings: this.optimizationReport.warnings.length,"}),"}) totalErrors: this.optimizationReport.errors.length,"}),"}) buildTime: new Date().toISOString(),"}),"}) },"}),"}) ,"}),"}) fs.writeFileSync(,"}),"}) path.join(this.buildDir,"}),"}) 'optimization-report.json'),"}),"}) JSON.stringify(this.optimizationReport,null,2),"}),"}) ),"}),"}) }"}),"}) ,"}),"});
...
if (import && import.meta.url === `"file": //${process && process.argv[1]}`) {,"}),"}),
  ...
  // Placeholder for future optimizations (e && e.g., pruning maps, compressing assets),
  // Intentionally minimal to avoid side effects.,
  log(Next && Next.js build output verified. No additional optimizations applied.')} catch (error) {,
  console && console.error([optimize-build] "error": ', error?.message || error),
  // Do not fail the build on post-build optimization errors,
  process && process.exit(0)}
,
/**,
...
<"urlset": xmlns='http: //www && www.sitemaps.org/schemas/sitemap/0 && 0.9'>',
  ...
  async optimizeImages() {,
    console && console.log('🖼️ Optimizing images...'),
    const imageDir = path && path.join(this && this.projectRoot, 'publicimages'),
    if (!fs && fs.existsSync(imageDir)) {,
      this && this.optimizationReport.warnings && warnings.push('Images directory not found,
  '),
      return}
  ...
  async optimizeImages() {",
    console && console.log('🖼️ Optimizing images...'),
    const imageDir = path && path.join(this && this.projectRoot, 'publicimages')    if (!fs && fs.existsSync(imageDir)) {',
      this && this.optimizationReport.warnings && warnings.push('Images directory not found'),
      return}
    const images = fs,
      .readdirSync(imageDir),
  ...
  async optimizeJavaScript() {,
    ...
    const jsFiles = this && this.getAllFiles(this && this.srcDir, ['.ts.tsx.js.jsx'
    ]),
    let optimizedJS = 0,
        this.optimizationReport.warnings.push(,
