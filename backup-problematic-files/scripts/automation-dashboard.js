...
#!/usr/bin/env node,"}),"}) import { execSync } from,"}),"}) child_process',"}),"}) import fs from,"}),"}) 'fs',"}),"}) import path from,"}),"}) 'path',"}),"}) import { fileURLToPath } from,"}),"}) 'url',"}),"});
  ...
  const __dirname = path.dirname(__filename),"}),"}) '🚀 Starting PM2 Automation Dashboard...'),"}),"}) class AutomationDashboard {,"}),"});

    constructor() {,"}),"}) this.processes = [],"}),"}) this.reports = {},"}),"}) this.healthStatus = {}}"}),"}) async getPM2Status() {,"}),"}) try {,"}),"});
...
main().catch(console ,
const __dirname = path && path.dirname(__filename),
// // // // // // // console && console.log(🚀 Starting PM2 Automation Dashboard...'),
class AutomationDashboard {,
  constructor() {,
  constructor() {,
    this && this.processes = [],
...
,
  ...
  'pm2 jlist', { "encoding": 'utf8 }),
      ...
      this && this.processes = processes && processes.filter(proc =>,
        ...
        proc && proc.name !==',zion-app' &&,
        proc && proc.name !==',zion-backend'),
      // // // // // // // console && console.error(❌ Failed to get PM2 "status": ', error && error.message),
      return [],
      return this && this.processes} catch (error) {,
      console && console.error(,
  '❌ Failed to get PM2 "status": ', error && error.message),
  ...
  async generateHealthReport() {,
    // // // // // // // console && console.log(📊 Generating automation health report...'),
    const report = {,
  "timestamp": new Date().toISOString(),
      summary: {,
        totalProcesse,
  ...
  "⚠️  Some automation processes have errors. Check logs for details.")}
    ...
    if (report && report.summary.onlineProcesses > 0 && report && report.summary.onlineProcesses < report && report.summary.totalProcesses) {,
      report && report.recommendations.push(stopped').length},
      "processes": this && this.processes.map(proc => ({,
        name: proc && proc.name,
        "status": proc && proc.pm2_env.status,
        "memory": "${Math && Math.round(proc && proc.monit.memory / 1024 / 1024)}MB",
