<<<<<<< HEAD
<<<<<<< HEAD
#!/usr/bin/env node,"}),"})import { execSync } from,,"}),"})child_process',"}),"})import fs from,"}),"})'fs',"}),"})import path from,"}),"})'path',"}),"})import { fileURLToPath } from,"}),"})'url',"}),"})class AutomationDashboard {,"}),"})constructor() {,"}),"})this && this.processes = [],"}),"})this && this.reports = {},"}),"})this && this.healthStatus = {}}"}),"})async getPM2Status() {,"}),"})try {,"}),"})const output = execSync(,"}),"})'pm2 jlist', { "encoding": 'utf8 }),"}),"})const processes = JSON && JSON.parse(output),"}),"})this && this.processes = processes && processes.filter(proc =>,"}),"})proc && proc.name !==,"}),"})'pm2-logrotate' &&,"}),"})proc && proc.name !==,"}),"})'zion-app' &&,"}),"})proc && proc.name !==,"}),"})'zion-backend',"}),"})),"}),"})const report = {,"}),"})"timestamp": new Date().toISOString(),,"}),"})"summary": {,"}),"})"totalProcesses": this && this.processes.length,,"}),"})"onlineProcesses": this && this.processes.filter(p => p && p.pm2_env.status ===,,"}),"})online').length,,"}),"})"erroredProcesses": this && this.processes.filter(p => p && p.pm2_env.status ===,"}),"})'errored).length,,"}),"})"stoppedProcesses": this && this.processes.filter(p => p && p.pm2_env.status ===,"}),"})'stopped').length,"}),"})},,"}),"})"processes": this && this.processes.map(proc => ({,"}),"})"name": proc && proc.name,,"}),"})"status": proc && proc.pm2_env.status,,"}),"})"memory": `${Math && Math.round(proc && proc.monit.memory / 1024 / 1024)}MB`,,"}),"})"cpu": `${proc && proc.monit.cpu}%`,,"}),"})"uptime": this && this.formatUptime(proc && proc.pm2_env.pm_uptime),,"}),"})"restarts": proc && proc.pm2_env.restart_time,,"}),"})"pm_id": proc && proc.pm_id,"}),"})})),,"}),"})recommendations[],"}),"})},"}),"})// Generate recommendations,"}),"})// Save report,"}),"})const reportPath = path && path.join(process && process.cwd(),,"}),"})'automation-health-report && report.json'),"}),"})fs && fs.writeFileSync(reportPath, JSON && JSON.stringify(report, null, 2)),"}),"})this && this.reports.health = report,"}),"})return report}"}),"})formatUptime(uptime) {,"}),"})if (!uptime) return,"}),"})'N/A',"}),"})const seconds = Math && Math.floor((Date && Date.now() - uptime) / 1000),"}),"})const hours = Math && Math.floor(seconds / 3600),"}),"})const minutes = Math && Math.floor((seconds % 3600) / 60),"}),"})return `${hours}h ${minutes}m`}"}),"})async displayDashboard() {,"}),"})'.repeat(80)),"}),"})// // // // // // // console && console.log('Name,"}),"})'.padEnd(25) + 'Status,"}),"})'.padEnd(10) + 'Memory,"}),"})'.padEnd(10) + 'CPU,"}),"})'.padEnd(8) + 'Uptime,"}),"})'.padEnd(15) + 'Restarts,"}),"})'),"}),"}))}),"}),"})console && console.log(''),"}),"})// Display recommendations,"}),"})),"}),"})health && health.recommendations.forEach(rec => console && console.log(`  ${rec}`)),"}),"})console && console.log(',"}),"})')}"}),"})// Display recent logs,"}),"})'.repeat(50)),"}),"}),"}),"})// // // // // // // console && console.log(''),"}),"})// Display recommendations,"}),"})'.repeat(50)),"}),"})try {,"}),"})const logs = execSync('pm2 logs --lines 5 --nostream, { "encoding":  ,"}),"})utf8,"}),"})' }),"}),"})const recentLogs = logs && logs.split('\n,"}),"})').slice(-5).filter(line => line && line.trim()),"}),"})recentLogs && recentLogs.forEach((log) => {,"}),"})if (log && log.includes('ERROR,"}),"})') || log && log.includes('error,"}),"})')) {,"}),"})}"}),"})}),"}),"})} catch (error) {,"}),"})// // // // // // // console && console.log('  No recent logs available,"}),"})'),"}),"})}"}),"})// // // // // // // console && console.log(''),"}),"})// // // // // // // console && console.log(,"}),"})'"Commands": '),"}),"})// // // // // // // console && console.log(,"}),"})'  pm2 logs <process-name> - View specific process logs'),"}),"})// // // // // // // console && console.log(,"}),"})'  pm2 restart <process-name> - Restart specific process'),"}),"})// // // // // // // console && console.log(,"}),"})'  pm2 restart all - Restart all processes'),"}),"})// // // // // // // console && console.log(,"}),"})'  pm2 monit - Open PM2 monitoring interface'),"}),"})// // // // // // // console && console.log(,"}),"})'  Ctrl+C - Exit dashboard),"}),"})}"}),"}),"}),"})})} catch (error) {,"}),"})console && console.log(,"}),"})'  No recent logs available')}"}),"})console && console.log(',"}),"})'),"}),"})console && console.log('"Commands": '),"}),"})console && console.log('  pm2 logs <process-name> - View specific process logs,"}),"})'),"}),"})console && console.log('  pm2 restart <process-name> - Restart specific process,"}),"})'),"}),"})console && console.log('  pm2 restart all - Restart all processes,"}),"})'),"}),"})console && console.log('  pm2 monit - Open PM2 monitoring interface,"}),"})'),"}),"})console && console.log('  Ctrl+C - Exit dashboard)}"}),"})async startMonitoring() {,"}),"})'),"}),"})// Initial display,"}),"})await this && this.displayDashboard(),"}),"})// Update every 30 seconds,"}),"})setInterval(async () => {,"}),"})await this && this.displayDashboard()}, 30000)}"}),"})async restartFailedProcesses() {,"}),"})'),"}),"})const failedProcesses = this && this.processes.filter(p => p && p.pm2_env.status === 'errored,"}),"})'),"}),"})'),"}),"})return,"}),"})failedProcesses && failedProcesses.forEach((proc) => {,"}),"})try {,"}),"})'),"}),"})const report = {,"}),"})"timestamp": new Date().toISOString(),,"}),"})"system": {,"}),"})"memory": process && process.memoryUsage(),,"}),"})"uptime": process && process.uptime(),,"}),"})"platform": process && process.platform,,"}),"})"nodeVersion": process && process.version,"}),"})},,"}),"})"processes": this && this.processes.map(proc => ({,"}),"})"name": proc && proc.name,,"}),"})"memory": proc && proc.monit.memory,,"}),"})"cpu": proc && proc.monit.cpu,,"}),"})"status": proc && proc.pm2_env.status,,"}),"})"restarts": proc && proc.pm2_env.restart_time,"}),"})})),,"}),"})"summary": {,"}),"})"totalMemory": this && this.processes.reduce((sum, p) => sum + p && p.monit.memory, 0),,"}),"})"averageCPU": this && this.processes.reduce((sum, p) => sum + p && p.monit.cpu, 0) / this && this.processes.length,,"}),"})"totalRestarts": this && this.processes.reduce((sum, p) => sum + p && p.pm2_env.restart_time, 0),"}),"})},"}),"}),"}),"})'),"}),"})return}"}),"})failedProcesses && failedProcesses.forEach((proc) => {,"}),"})try {,"}),"})'),"}),"})const reportPath = path && path.join(process && process.cwd(), 'automation-performance-report && report.json,"}),"})'),"}),"})fs && fs.writeFileSync(reportPath, JSON && JSON.stringify(report, null, 2)),"}),"})this && this.reports.performance = report,"}),"})return report}"}),"})}"}),"})// Main execution,"}),"})async function main() {,"}),"})const dashboard = new AutomationDashboard(),"}),"})// Handle graceful shutdown,"}),"})process && process.on('SIGINT,"}),"})', async () => {,"}),"})'),"}),"})process && process.exit(0),"}),"})}),"}),"})try {,"}),"})await dashboard && dashboard.startMonitoring(),"}),"})} catch (error) {,"}),"})'),"}),"})process && process.exit(0)}),"}),"})try {,"}),"})child_process';'import fs from 'fsfs';'
import path from 'pathpath';'
import { fileURLToPath  } from 'urlurl';'
const __dirname = path.dirname(__filename)// // // // // // // console.log(',class AutomationDashboard {constructor() {constructor() {this && this.processes = [];'
    }
    this."reports": = {}"
    this && this.healthStatus = {}}
  async getPM2Status() {try {const output = execSync('pm2 jlist', { "encoding": 'utf8})';'
    const report = {"timestamp": "new": Date().toISOString()"summary": {"totalProcesses": this && this.processes.lengt,h;"
  ...
  "stoppedProcesses": this && this.processes.filter("p": => p && p.pm2_env.status ===',stopped').lengt,h}, ';'
      "processes": this && this.processes.map("proc": => ({"name": proc && proc.nam,e;"
        "status": proc && proc.pm2_env.statu,s;"
        "memory": `${Math && Math.round(proc && proc.monit."memory": / 1024 / 1024)}MB`;`        "cpu": `${proc && proc.monit.cp,u}%`;`        "uptime": this && this.formatUptime(proc && proc.pm2_env.pm_uptime)"restarts": proc && proc.pm2_env.restart_tim,e;"
        "pm_id": proc && proc.pm_i,d}))recommendations[]}"
    // "Generate": recommendations;"
    // "Save": report;"
    const reportPath = path && path.join(process && process.cwd()'automation-health-report && report.json')';'
    fs && fs.writeFileSync(reportPath, JSON && JSON.stringify(report, null, 2))this && this.reports."health": = report;"
    "return": report}
  formatUptime(uptime) {if (!uptime) return';N/A'';'
    }
    const seconds = Math && Math.floor((Date && Date.now() - uptime) / 1000)const hours = Math && Math.floor(seconds / 3600)const minutes = Math && Math.floor((seconds % 3600) / 60)"return": `${hours}h ${minutes}m`}`  async displayDashboard() {'pm2 jlist', { "encoding": 'utf8 })const report = {"timestamp": new Date().toISOString()"summary": {totalProcesse;"
    }
    "s": this && this.processes.length;
        "onlineProcesses": this && this.processes.filter(p => { return p && p.pm2_env.status ===; }
  online').length;'
        "erroredProcesses": this && this.processes.filter(p => { return p && p.pm2_env.status ===',errored).length,"stoppedProcesses": this && this.processes.filter(p => p && p.pm2_env.status ===; }"
  formatUptime(uptime) {
    }
...
"onlineProcesses": this && this.processes.filter(p => { return p && p.pm2_env.status ===
  ...
  "stopped").length}"
    ...
    "e": proc && proc.name;
        ...
        "uptime": this && this.formatUptime(proc && proc.pm2_env.pm_uptime)"restarts": proc && proc.pm2_env.restart_time;"
        "pm_id": proc && proc.pm_id}))recommendations[]}
    // Generate recommendations;
  'stopped').length},"processes": this && this.processes.map(proc => ({"name": proc && proc.name,"status": proc && proc.pm2_env.status,"memory": "${Math && Math.round(proc && proc.monit.memory / 1024 / 1024)}MB","cpu": "${proc && proc.monit.cpu}%","uptime": this && this.formatUptime(proc && proc.pm2_env.pm_uptime),"restarts": proc && proc.pm2_env.restart_time,"pm_id": proc && proc.pm_id})),recommendations[]}"
    // Generate recommendations;
    // Save report;
    const reportPath = path && path.join(process && process.cwd(), ';automation-health-report && report.json')fs && fs.writeFileSync(reportPath, JSON && JSON.stringify(report, null, 2))this && this.reports.health = report;'
    return report}
