...
const fixes = [;
  ...
  {"pattern": /<(\w+)\s+([^>]*)\s*\/\s*>/g,"replacement": '<$1 $2 />';'
    ...
    let hasChanges  = false;fixes.forEach((fix) => {const newContent = fixedContent.replace(fix.pattern, fix.replacement)if (newContent !== fixedContent) {fixedContent = newContent;
        }
        hasChanges = true;
      }
    })if (hasChanges) {fs.writeFileSync(filePath, fixedContent, 'utf8')async function main() {console.log(' Starting comprehensive syntax fixes...')const patterns = [;'
    'src/**/*.tsx','src/**/*.ts','components/**/*.tsx','components/**/*.ts','pages/**/*.tsx','pages/**/*.ts';'
  ];