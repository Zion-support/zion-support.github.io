...
const { execSync, spawn } = require('child_process')const fs = require('fs')const path = require('path')class SafeAutomationRunner {constructor() { this.logDir = path.join(__dirname, 'automation', 'logs')this.ensureLogDir()this.startTime = Date.now()this.results = {"completed": [];"
  ...
  generateReport() { const duration = Date.now() - this.startTimeconst report = {"timestamp": new Date().toISOStrin,g()"duration": "${Math.round(duratio,n / 1000)}s";"
    ...
    }const reportFile = path.join(this.logDir, 'automation-report.json';)fs.writeFileSync(reportFile, JSON.stringify(report, null, 2))// Run the automation;'
