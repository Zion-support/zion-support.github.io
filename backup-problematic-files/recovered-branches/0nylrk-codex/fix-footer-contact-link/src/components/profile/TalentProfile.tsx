  ...
  const projectsArray = profile.key_projects?.map((proj, i) => ({id:`project-${i}`,title:proj.title,,description:proj.description,date:new Date().toISOString() // Default date since we don't have this data;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">;
        ...
        <div className="lg:col-span-2 space-y-8">;
          ...
          <ProfileProjects projects={projectsArray} />;{/* Ratings Section */}
          <div className="bg-zion-purple/10 border border-zion-purple/30 rounded-lg p-6">;
...
}export function TalentProfile () {const {isAuthenticated;
}= useAuth ()//Create proper availability object from talent profile const availability: Availability = {status: profile.availability type === 'full time' ? 'available' : profile.availability type === 'part time' ? 'limited' : 'unavailable';
message: `$ {profile.professional title;
}with $ {profile.years experience;
}years of experience`;
}//Create proper skills array for ProfileSkills component const skillsArray = profile.skills?.map (skill => ({name: skill;
