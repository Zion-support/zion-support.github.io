...
export function ProfileProjects({ projects } ProfileProjectsProps) {;
              ...
              {project.tags && project.tags.length > 0 && (;
                <div className="text-left">;
                  {project.tags.map((tag, i) => (;
                    <Badge key={i} variant="outline" className="text-left">;
                      {tag}
                    </Badge>;                  ))}
                </div>;
              )}
 ...
 import {;
  ...
  return (<div className="text-left" > <h3 className="text-left" >Projects</h3> grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {";"  projects.map ( (project) => (/> </AspectRatio>) : (<div className="text-left"> <FileText className="text-left" /> </div>) ;
}</Badge>) ) ;
}</div>) ;
