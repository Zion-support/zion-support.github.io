...
//Handle removing skill tags const handleRemoveSkill = (
  setSkillTags (skillTags.filter ( (s) => s !== skill) ) ) => {
  return $3;}
}
};
//Handle key press in skills input (add on enter) const handleSkillKeyPress = () => {
  return $3;}
}
}
};
//Handle avatar upload const handleAvatarUpload = (
  const file = e.target.files?.[0];
if (file) {
  const reader = new FileReader ();
reader.onloadend = () => {
