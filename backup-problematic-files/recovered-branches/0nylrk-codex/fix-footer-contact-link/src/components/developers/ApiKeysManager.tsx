...
const [showCreateDialog, setShowCreateDialog] = useState (false)const [showDeleteConfirm, setShowDeleteConfirm] = useState<string | null /> (null)const [showRegenerateConfirm, setShowRegenerateConfirm] = useState<string | null /> (null)//Create key form state //Load keys on mount await createApiKey (keyName, selectedScopes)setShowCreateDialog (false)//Toggle a scope selection const toggleScope = (setSelectedScopes (prev => prev.includes (scope) ? prev.filter (s => s !== scope) -H \"Authorization: Bearer $ {key;) => {
  return $3;}
}
}\" \\ -H \"Content-Type: application/json\"`;
}//Reset form when dialog closes const handleDialogClose = (setKeyName (\"\")setSelectedScopes ([])setShowCreateDialog (false)) => {
  return $3;}
