...
export function DisputeDetail() {;
  ...
  const { disputeId } = useParams() as { disputeId?:string },const navigate = useNavigate(),const { user } = useAuth(),const { getDisputeById, updateDisputeStatus, resolveDispute, getDisputeMessages, addDisputeMessage }  = useDisputes(),const [dispute, setDispute] = useState<any />(null),const [messages, setMessages] = useState<DisputeMessage[] />([]),const [isLoading, setIsLoading] = useState(true),const [message, setMessage] = useState(\"\"),const [isSending, setIsSending] = useState(false),const [resolution, setResolution] = useState({summary:\"\",resolution_type:\"compromise\"}),const [activeTab, setActiveTab]  = useState(\"overview\"),// Check if user is admin (placeholder - implement proper admin check)const isAdmin  = user?.userType === \"admin\",useEffect(() => {if (!disputeId) return,const loadDisputeData = async () => {setIsLoading(true),try {const disputeData = await getDisputeById(disputeId),if (!disputeData) {toast.error(\"Dispute not found\"),navigate(\"/dashboard/disputes\"),return,}
    ...
    },loadDisputeData(),}, [disputeId, navigate, getDisputeById, getDisputeMessages]),const handleStatusChange = async (status:DisputeStatus) => {if (!disputeId) return,const success = await updateDisputeStatus(disputeId, status),if (success && dispute) {setDispute({ ...dispute, status }),}
  },const handleResolveDispute = async () => {if (!disputeId) return,if (!resolution.summary) {toast.error(\"Please provide a resolution summary\"),return,}const success = await resolveDispute(disputeId, resolution),if (success && dispute) {setDispute({...dispute,status:\"resolved\",resolution_summary:resolution.summary,resolution_type:resolution.resolution_type,resolved_at:new Date().toISOString()}),}
    ...
    } catch (error) {console.error(\"Error sending message:\", error),} finally {setIsSending(false),}
    ...
    ),}const getStatusBadgeVariant = (switch (status) {case \"open\":return \"default\",case \"under_review\":return \"secondary\",case \"resolved\":return \"outline\", // Changed from \"success\" to \"outline\";
  ...
  return $3;}
      ...
      <div className=\"flex flex-wrap items-center justify-between gap-4\" />;
      ...
      </div>;{dispute.status === \"resolved\" && dispute.resolution_summary && (<Alert className=\"bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-900\" />;
        ...
        <div className=\"lg:col-span-2\" />;
          <Tabs value={activeTab} onValueChange={setActiveTab} />;
                ...
                <CardContent className=\"space-y-4\" />;
                  <div />;
                    ...
                    <ul className=\"space-y-2 mt-2\" />;
                      ...
                      </li>;{dispute.status !== \"open\" && (<li className=\"flex gap-2 items-center\" />;
                  ...
                  <CardContent />;}
                    <p className=\"whitespace-pre-wrap\" />{dispute.resolution_summary}</p>;{dispute.resolution_type && (<div className=\"mt-4\" />;}
                        <Badge />Resolution:{dispute.resolution_type.replace('_ ')}
                        </Badge>;
                      </div>;                    )}
                  </CardContent>;
                </Card>;
              )}
            </TabsContent>;<TabsContent value=\"messages\" className=\"space-y-6\" />;
              <Card />;
                <CardHeader />;
                ...
                <CardContent />;
                  <div className=\"space-y-6 max-h-[600px] overflow-y-auto p-2\" />;
                    {messages.length === 0 ? (<div className=\"text-center py-12\" />;
                        ...
                        .filter(msg => !msg.is_admin_note).map((msg) => {const isCurrentUser = user?.id === msg.user_id,return (<div;}
                              key={msg.id}
            ...
            </TabsContent>;{isAdmin && (<TabsContent value=\"admin\" className=\"space-y-6\" />;
                <Card />;
                  <CardHeader />;
                    ...
                    <div />;
                      ...
                      <div className=\"flex gap-2\" />;
                        ...
                        <div className=\"space-y-4\" />;
                          ...
                          </div>;<Button onClick={handleResolveDispute} />Resolve Dispute</Button>;
                        </div>;
                      </div>;
                    )}<div />;
                      ...
                      <div className=\"space-y-4 max-h-[300px] overflow-y-auto p-2\" />;
                        {messages;}
                          .filter(msg => msg.is_admin_note).map((msg) => (<div key={msg.id} className=\"bg-yellow-50 border-l-4 border-yellow-200 p-4 dark:bg-yellow-900/20 dark:border-yellow-900\" />;
                        ...
                        ))}{!messages.some(msg => msg.is_admin_note) && (<p className=\"text-sm text-muted-foreground italic\" />No admin notes yet</p>;}
                        )}
                        ...
                        <Button ;
                          variant=\"outline\" ;
                          onClick={() = /> {if (message.trim()) {addDisputeMessage(disputeId!, message, true).then(() => {getDisputeMessages(disputeId!).then(setMessages),setMessage(\"\"),}),}
                          }}
                        >;
                          Add Admin Note;
                        </Button>;
