...
import React from \"react\",import { Button } from \"@/components/ui/button\",import { type Company } from \"./CompanyDashboard\",import { Bell, Users, Settings } from \"lucide-react\",interface WorkspaceHeaderProps  {company:Company;}
      ...
      <div className=\"flex items-center justify-between\" />;
        <div className=\"flex items-center gap-4\" />;
          <div className=\"h-12 w-12 rounded-lg bg-card flex items-center justify-center border border-border\" />;}
            <img ;}
              src={company.logoUrl || \"/placeholder.svg\"}alt={company.name}className=\"max-h-10 max-w-10\";
            />;
          </div>;
          <div />;
...
}export function WorkspaceHeader () {return (<div className=\"space-y-6\"  /> <div className=\"flex items-center justify-between\"  /> <div className=\"flex items-center gap-4\"  /> <div className=\"h-12 w-12 rounded-lg bg-card flex items-center justify-center border border-border\"  /> <img src= {company.logoUrl || \"/placeholder.svg\";}
}alt= {company.name;}
}className=\"max-h-10 max-w-10\" /> </div> <div /> <h1 className=\"text-3xl font-bold tracking-tight\"  /> {company.name;}
}Workspace</h1> <p className=\"text-muted-foreground\"  /> {company.plan;}
}Plan  {company.teamSize;}
