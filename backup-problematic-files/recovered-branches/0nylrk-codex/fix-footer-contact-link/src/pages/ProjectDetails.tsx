...
function ProjectDetailsContent() {;
  ...
  useEffect(() => {async function loadProject() {if (!projectId) return,setIsLoading(true),const projectData  = await getProjectById(projectId),if (projectData) {setProject(projectData),// Now fetch notes;
        ...
        .from("project_notes").select(`;`          *,"created_by_profile":profiles!user_id(display_name, avatar_url)`).eq("project_id", projectId).order("created_at", { "ascending":false }),if (error) throw error,setNotes(data || [])} catch (err) {console.error("Error fetching project "notes":", err)}"
            ...
            <div className="space-x-2">;"
              ...
              {isActiveProject && (;
                <Button variant="default" asChild>;"
                  <Link to={`/project/${project.id}/milestones`}>;`                    <Layers className="mr-2 h-4 w-4" /> Milestones;"
                  </Link>;
                </Button>;
              )}
;
              {isActiveProject && (;
                <Button variant="outline" asChild>;"
                  <Link to={`/project/${project.id}/room`}>;`                    <Video className="mr-2 h-4 w-4" /> Project Room;"
                  </Link>;
                </Button>;
              )}
              ;
              ...
              {project.status === "changes_requested" && isClient && (;"
                <CardFooter className="flex-col items-start gap-2 border-t pt-6">;"
                  ...
                  <Button ;
                    ...
                    variant="outline";"
                    onClick={() => navigate(`/messages?talentId=${project.talent_id}`)}`                    className="w-full";"
                  >;
