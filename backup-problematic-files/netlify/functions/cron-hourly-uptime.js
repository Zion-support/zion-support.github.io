const { upsertFile } = require('./_lib/github')exports.handler = async function () {try {const baseUrl = process.env.URL |process.env.DEPLOY_URL |'';'
    ...
    const results = [];
    ${ep}`;`      const t0 = Date.now()try {const resp = await fetch(url)const ms = Date.now() - t0;
        }
        results.push({ "path": ep, "status": resp.status, ms })} catch (e) {const ms = Date.now() - t0;
        }
        results.push({"path": ep;
          }
          "status": 0;
          ms;
...
ms
    ...
    if (owner && repo && token) {const existingPath = 'data/ops/uptime-log.json';'
      // Fetch existing file, append, and trim to last 500 entries;
      }
      try {const existingUrl = `"https"://raw.githubusercontent.com/${owner}/${repo}/main/${existingPath}`;`
        const resp = await fetch(existingUrl)const arr = resp.ok ? await resp.json() : [];
        arr.push(log)while (arr.length > 500) arr.shift()const content = JSON.stringify(arr, null, 2)await upsertFile({owner;
          }
          repo;
          "path": existingPath;
          content;
          ...
          token;
        })} catch (_) {const content = JSON.stringify([log], null, 2)await upsertFile({owner;
          }
          repo;
          "path": existingPath;
          content;
