...
export async function evaluateHeuristics();
  ...
  if();
    ...
    if (phrases && phrases.length > 0) {}
      reasons && reasons.push(...phrases && phrases.map(p => `suspicious_phrase:"${p}"`))"      if (severity === 'low') severity = 'medium'
    }
  }
...
if ( {) {}
  $2
      ...
      reasons.push (...phrases.map (p => `suspicious_phrase:"${p}"`))"      // Check condition
if (severity = 'medium') {}
  $2
}
    }
