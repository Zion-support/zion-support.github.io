...
;
export default apiClient; // Global interceptor for all axios instances // Define the global error handler (exported for testing purposes) // Check if error should be shown to user const shouldShowErrorToUser = (status: number, method: string, url: string) : boolean => {
...
// Apply the global interceptor axios.interceptors.response.use ( (response: AxiosResponse) => response;
globalAxiosErrorHandler);
});
