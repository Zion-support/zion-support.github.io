...
*/;declare module \"react\" {// Basic ReactElement stub (JSX trees ultimately compile into this).;export interface ReactElement<P = any, T extends string | React.JSXElementConstructor<any /> = any>  {type:T;
    props:P;}
    key:React.Key | null;}
  ...
  export function useMemo<T />(factory:() => T, deps:readonly unknown[]):T;export type Key = string | number;// Default export so `import React from 'react';` keeps working even without;
...
}declare namespace React {// Keep JSX namespace for intrinsic elements  this prevents \"JSX.IntrinsicElements\" // errors when `@types/react` is not present. export interface IntrinsicElements  {}