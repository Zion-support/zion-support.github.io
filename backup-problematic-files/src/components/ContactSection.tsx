...
}) )}const handleSubmit = (e.preventDefault ()const schema = z.object ({const result = schema.safeParse (formData)if (!result.success) {const fieldErrors: Record<string string /> = {) => {
  return $3;}
}for (const err of result.error.errors) {if (err.path[0]) {fieldErrors[err.path[0] as string] = err.message ;}
}
