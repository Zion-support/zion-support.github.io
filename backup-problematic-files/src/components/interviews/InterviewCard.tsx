...
import { toast } from \"@/components/ui/use-toast\",import { InterviewResponseForm } from \"./InterviewResponseForm\",interface InterviewCardProps  {interview:Interview,onRefresh:() => Promise<void />;}
  ...
  const isInterviewPast  = isPast(interviewDate),const getRelativeTime = (if (isPast(interviewDate)) {return `Took place ${formatDistanceToNow(interviewDate)) => {
  return $3;}
} ago`,} else {return `Starts in ${formatDistanceToNow(interviewDate)}`,}
