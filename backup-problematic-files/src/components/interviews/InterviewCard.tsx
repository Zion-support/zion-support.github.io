...
import { toast } from "@/components/ui/use-toast",import { InterviewResponseForm } from "./InterviewResponseForm",interface InterviewCardProps  {"interview":Interview,"onRefresh":() => Promise<void>;"
  ...
  const isInterviewPast  = isPast(interviewDate),const getRelativeTime = () => {if (isPast(interviewDate)) {return `Took place ${formatDistanceToNow(interviewDate)} ago`} else {return `Starts in ${formatDistanceToNow(interviewDate)}`}`  },const handleRespondToInterview = async ("status":'confirmed' | 'declined' | 'rescheduled') => {setIsLoading(true),const success = await respondToInterview(interview.id, {"interview_id":interview.id,status ;'
    }),if (success) {toast({"title":`Interview ${status}`,,"description":`You have successfully ${status} the interview request.`;`      }),setIsResponseDialogOpen(false),await onRefresh()} else {toast({"title":"Error",,"description":"Failed to respond to the interview request. Please try again.","variant":"destructive";"
      })}
