...
import {logErrorToProduction} from '@/utils/productionLogger',interface ErrorBoundaryState  {hasError:boolean;
  error:Error | null;
  ...
  context?:string;
}export class GlobalErrorBoundary extends Component<ErrorBoundaryProps ErrorBoundaryState> {private retryTimeouts:NodeJS.Timeout[] = [];constructor(props:ErrorBoundaryProps) {super(props)this.state = {hasError:false,error:null,errorInfo:null,errorId:null,retryCount:0,userFeedback:'',showDetails:false;
    }
    ...
    const enhancedError = {...error,componentStack:errorInfo.componentStack,errorBoundary:this.props.context || 'GlobalErrorBoundary',timestamp:new Date().toISOString(),userAgent:typeof window !== 'undefined' ? navigator.userAgent :'SSR',url:typeof window !== 'undefined' ? window.location.href :'SSR',userId:this.getUserId(),buildInfo:this.getBuildInfo()}// Log to console in development;
    ...
    Sentry.withScope((scope) => {scope.setTag('errorBoundary', this.props.context || 'GlobalErrorBoundary')scope.setLevel('error')scope.setContext('errorInfo', {componentStack:errorInfo.componentStack,retryCount:this.state.retryCount;
      })Sentry.captureException(error)})// Custom error handler;
    if (this.props.onError) {this.props.onError(error, errorInfo)}this.setState({errorInfo,errorId;
    })}componentWillUnmount() {// Clear any pending retry timeouts;
    this.retryTimeouts.forEach(timeout => clearTimeout(timeout))}private generateErrorId():string {return `err_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  }private getErrorSeverity(error:Error):'low' | 'medium' | 'high' | 'critical' {const message = error.message.toLowerCase()const stack  = error.stack?.toLowerCase() || '';// Critical errors;
    if (message.includes('network') || message.includes('fetch')) {return 'medium';
  ...
  }private retry = () => {if (this.state.retryCount >= (this.props.maxRetries || 3)) {return;
    }const retryDelay  = Math.pow(2, this.state.retryCount) * 1000 // Exponential backoff;const timeout = setTimeout(() => {this.setState({hasError:false,error:null,errorInfo:null,errorId:null,retryCount:this.state.retryCount + 1,showDetails:false;
      ...
      const response = await fetch('/api/error-report', {method:'POST',headers:{'Content-Type':'application/json';
          ...
          },errorInfo:this.state.errorInfo,userFeedback:this.state.userFeedback,context:this.props.context,timestamp:new Date().toISOString()})})if (response.ok) {// Show success message;
      }
              ...
              <CardHeader className="text-center">;
                ...
                <div className="text-center">;
                  <p className="text-gray-600 dark:text-gray-300 mb-4">;
                    {suggestion}
                  </p>;{this.state.retryCount > 0 && (<p className="text-sm text-orange-600 dark:text-orange-400">;
                ...
                <AnimatePresence>;
                  {this.state.showDetails && (<motion.div;
                    ...
                    >;
                      <div className="space-y-4">;
                        <div>;
                        ...
                        </div>;{process.env.NODE_ENV === 'development' && this.state.error.stack && (<div>;
                          ...
                          <Button onClick={this.copyErrorDetails} variant="outline" size="sm">;
                            ...
                            Copy Details;
                          </Button>;{this.props.showReportButton !== false && (<Button onClick={this.reportError} variant="outline" size="sm">;
...
export const withErrorBoundary = <P extends object>(Component:React.ComponentType<P>,errorBoundaryProps?:Omit<ErrorBoundaryProps 'children'>;
  ...
  )WrappedComponent.displayName = `withErrorBoundary(${Component.displayName || Component.name})`;return WrappedComponent;}export default GlobalErrorBoundary;
'use client' import {{React, {Component,  ErrorInfo, ReactNode ';
  } from 'react';
...
}interface ErrorBoundaryProps  {children: ReactNode fallback?: ReactNode onError?: (error: Error, errorInfo: ErrorInfo) => void enableRetry?: boolean maxRetries?: number showReportButton?: boolean context?: string ;
}constructor (props: ErrorBoundaryProps) {super (props) this.state = {';
  hasError: false, error: null, errorInfo: null, errorId: null, retryCount: 0,  userFeedback: '', showDetails: false ;
}
}static getDerivedStateFromError (error: Error) : Partial<ErrorBoundaryState> {return {hasError: true, error ;
}
}componentDidCatch (error: Error, errorInfo: ErrorInfo) {const errorId = this.generateErrorId () //Enhanced error logging const enhancedError = {';
...
}) console.groupEnd ()}//Report to Sentry Sentry.withScope ( (scope) => {';
  scope.setTag ('errorBoundary', this.props.context || 'GlobalErrorBoundary') scope.setLevel ('error') scope.setContext ('errorInfo',  {componentStack: errorInfo.componentStack, retryCount: this.state.retryCount ;
}) Sentry.captureException (error)}) //Custom error handler if (this.props.onError) {this.props.onError (error, errorInfo)}this.setState ({errorInfo, errorId ;
})}componentWillUnmount () {//Clear any pending retry timeouts this.retryTimeouts.forEach (timeout => clearTimeout (timeout) )}private generateErrorId () : string {return `err $ {Date.now ()}$ {Math.random () .toString (36) .substr (2, 9)}` ;
}private getUserId () : string | null {';
  //Try to get user ID from various sources if (typeof window !== 'undefined') {try {';
...
})}, retryDelay) this.retryTimeouts.push (timeout)}private copyErrorDetails = async () => {const errorDetails = {';
...
}private reportError = async () => {if (!this.state.error || !this.state.errorId) return try {';
...
}render () {if (this.state.hasError && this.state.error) {//Use custom fallback if provided if (this.props.fallback) {return this.props.fallback ;
}const severity = this.getErrorSeverity (this.state.error) const suggestion = this.getErrorSuggestion (this.state.error) const canRetry = this.props.enableRetry !== false && this.state.retryCount < (this.props.maxRetries || 3) return (<div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-950/20 dark:to-orange-950/20" > <motion.div initial= {{opacity: 0, scale: 0.95 ;
}
}animate= {{opacity: 1, scale: 1 ;
}
}transition= {{duration: 0.3 ;
...
}> {severity.toUpperCase ()}</Badge> ID: {this.state.errorId.slice (-8)}</Badge>)}</div> </CardHeader> </p>)}</div> Try Again </Button>)}Go Home </Button> <Button </Button> </div> {/* Error Details */ ;
}<AnimatePresence> {";
  this.state.showDetails && (<motion.div > <div className="space-y-4" > <div> </code> </div> {'";
  process.env.NODE ENV === 'development' && this.state.error.stack && (<div> <h4 className="font-semibold text-sm mb-2" >Stack Trace:</h4> <pre className="p-3 bg-gray-50 dark:bg-gray-800 rounded text-xs overflow-auto max-h-32" > {this.state.error.stack ;
}</pre> </div>)}Report Issue </Button>)}</div> </div> </motion.div>)}</AnimatePresence> </CardContent> </Card> </motion.div> </div>)}return this.props.children ;
}
}//Hook for programmatic error boundary export const useErrorBoundary = () => {const [error, setError] = React.useState<Error | null> (null) React.useEffect ( () => {if (error) {throw error ;
}
}, [error]) const captureError = React.useCallback ( (error: Error) => {setError (error)}, []) return {captureError ;
}
}//Higher-order component for adding error boundaries <GlobalErrorBoundary {...errorBoundaryProps ;
}> <Component {...props ;
}/> </GlobalErrorBoundary>) WrappedComponent.displayName = `withErrorBoundary ($ {Component.displayName || Component.name ;
