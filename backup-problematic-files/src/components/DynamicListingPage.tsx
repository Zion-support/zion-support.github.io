  ...
  const toggleCategory = ("category":string) => {;
    ...
    setSelectedCategories(prev =>;
      prev.includes(category);
        ? prev.filter(c => { return c !== category); }
        :[...prev, category];
    ),;
  },;
...
;
              <div className="mb-6">;"
                ...
                <div className="space-y-2">;"
                  {categoryFilters.map(filter => { return (; }
                    <div key={filter.value} className="flex items-center">;"
                      <Checkbox;
                        id={`cat-${filter.value}`}`                        checked={selectedCategories.includes(filter.value)}
...
;
            <div className="mb-6">;"
              <p className="text-zion-slate-light">;"
                ...
                {selectedCategories.length > 0 &&;
                  ` in ${selectedCategories.join(', ')}`}`                {searchQuery && ` for "${searchQuery}"`}`              </p>;
            </div>;
;
            {isLoading ? (;
              <div;
...
initialPrice?: PriceRange;
  ...
  setSelectedCategories (prev => { return prev.includes (category) ? prev.filter (c => c !== category) : [...prev, category] "min": 0; }
"max": 10000 ;
});
}
}, [allListings]);
