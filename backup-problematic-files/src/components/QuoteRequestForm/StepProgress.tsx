...
import { cn } from \"@/lib/utils\",interface StepProgressProps  {currentStep:QuoteRequestSteps;}
  ...
  ],const getStepStatus = (const stepOrder = steps.findIndex(s => s.id === stepId),const currentStepOrder  = steps.findIndex(s => s.id === currentStep),if (stepOrder < currentStepOrder) return \"complete\",if (stepOrder === currentStepOrder) return \"current\",return \"upcoming\";) => {
  return $3;}
}
