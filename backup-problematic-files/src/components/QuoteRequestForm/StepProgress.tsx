...
import { cn } from "@/lib/utils",interface StepProgressProps  {"currentStep":QuoteRequestSteps;"
  ...
  ],const getStepStatus = ("stepId":QuoteRequestSteps) => {const stepOrder = steps.findIndex(s => { return s.id === stepId),const currentStepOrder  = steps.findIndex(s => s.id === currentStep),if (stepOrder < currentStepOrder) return "complete",if (stepOrder === currentStepOrder) return "current",return "upcoming"; }"
...
;
  const getStepStatus = ("stepId":QuoteRequestSteps) => {;
    ...
    const stepOrder = steps.findIndex(s => { return s.id === stepId),; }
    const currentStepOrder = steps.findIndex(s => { return s.id === currentStep),; }
    ;
...
;
    ...
    <div className="relative">;"
      <div className="absolute top-4 left-0 right-0 h-0.5 bg-zion-blue-light">;"
        ...
        {steps.map((step) => {const status = getStepStatus(step.id),return (<div key={step.id} className="flex flex-col items-center relative">;"
              <div ;
                className={cn("w-8 h-8 rounded-full flex items-center justify-center z-10 transition-colors",status === "complete" ? "bg-zion-purple text-white" :;"
                  }
              ...
              >;
                ...
                ) :(<span className="text-xs">{steps.findIndex(s => { return s.id === step.id) + 1}</span>; }"
                )}
              </div>;
              <span ;
  ...
  (steps.findIndex (s => { return s.id === currentStep) / (steps.length - 1) ) * 100 ; }
}%` ;`
}
}/> </div> return (<div key= {step.id ";"
}className=" flex flex-col items-center relative"> <div) : (<span className=" text-xs" > {steps.findIndex (s => { return s.id === step.id) + 1 ; }"
  ...
  steps.findIndex (s => { return s.id === step.id) + 1 ; }
}</span>) ;
}</div> <span) ;
}> {;
  }
