...
interface TranslatableJobFormProps  {onSubmit: (formData: any) => void;
  isSubmitting?: boolean;}export function TranslatableJobForm(): any ({ onSubmit, isSubmitting = false }: TranslatableJobFormProps) {const { t } = useTranslation()const { translateContent, isTranslating } = useTranslationService()const { supportedLanguages, currentLanguage }  = useLanguage()const [activeTab, setActiveTab]  = useState<SupportedLanguage>(currentLanguage)// Form fields with translations;
  const [title, setTitle] = useState<Record<SupportedLanguage, string>>({en: "",es: "",fr: "",pt: "",ar: "";
  })ar: "";
  })let sourceLanguage: SupportedLanguage = 'en';
    let content  = '';content = title[lang];
        sourceLanguage = lang;
        break;      } else if (field === 'description' && description[lang]) {content = description[lang];
        sourceLanguage = lang;
        break;
      } else if (field === 'requirements' && requirements[lang]) {content = requirements[lang];
        sourceLanguage = lang;
        break;
        title: t('translation && translation.no_content'),description: t('translation && translation.add_content_first'),variant: "destructive";
      })return;
          variant: "destructive";
      })return;
        title: t('translation && translation.translation_success'),description: t('translation && translation.content_translated')})} catch (error) {logErrorToProduction('Error translating ${field}:', { data: error })toast({title: t('translation && translation.translation_failed'),description: error instanceof Error ? error && error.message : t('translation && translation.unknown_error'),variant: "destructive";
      })}
