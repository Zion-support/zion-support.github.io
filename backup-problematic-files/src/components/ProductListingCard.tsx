import React, { useState } from 'react',import { logDebug, logErrorToProduction } from '@/utils/productionLogger',import { useRouter } from 'next/router',import { Badge } from "@/components/ui/badge",import { Button } from "@/components/ui/button",import { ProductListing } from "@/types/listings",import { DollarSign  } from 'lucide-react';
import { RatingStars } from "@/components/RatingStars",import { FavoriteButton } from "@/components/FavoriteButton",import { useDispatch } from 'react-redux',import type { AppDispatch } from '@/store',import { addItem } from '@/store/cartSlice',import { toast } from '@/hooks/use-toast',import { useCurrency } from '@/hooks/useCurrency',import Image from 'next/image', // Import next/image;interface ProductListingCardProps  {listing:ProductListing,view?:'grid' | 'list',onRequestQuote?:(id:string) => void,detailBasePath?:string;
      ...
      <div className={`flex flex-col justify-between ${isGrid ? 'p-4 flex-1' :'p-4 flex-1'}`}>;
        <div>;
          ...
          <div onClick={handleViewListing} className="block">;
            {listing.uspHeadline && (<p className="text-primary font-semibold text-sm mb-1">;                {listing.uspHeadline}
              </p>;
            )}
  ...
  ),},export const ProductListingCard = React.memo(ProductListingCardComponent),ProductListingCard.displayName = 'ProductListingCard',import Image from 'next/image';';
...
const handleImageError = () => {if (!imageError) {';
  //Prevent infinite loops if placeholder also fails setImageSrc ('/placeholder.svg')setImageError (true)}
}';
//Debug logging for development if (process.env.NODE ENV === 'development') {return;
}
}
}}
}> {/* Image */ ;
...
}<Image Featured </Badge>)}{stockStatus && (<Badge variant= {stockVariant as any ;
}className="absolute top-2 left-2" > {stockStatus ;
}</Badge>)}<FavoriteButton itemId= {listing.id ;
}/> </div> </div> {/* Content */ ;
...
}</div> <div className="flex gap-2" > <Button onClick={(e) => {e.stopPropagation (), //Prevent card click event addToCart ()}
}disabled= {loading ";
}loading ? (<> <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" > <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" ></circle> <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" ></path> </svg> Loading... </>) : ("Add to Cart")}</Button> <Button onClick={(e) => {e.stopPropagation (), //Prevent card click event //Add to cart first, then redirect to checkout dispatch (addItem ({id: listing.id,  title: listing.title, price: listing.price ?? 0 ;
}) )';
