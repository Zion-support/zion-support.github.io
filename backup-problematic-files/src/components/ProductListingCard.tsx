...
;
  ...
  return $3;}
    ...
    if (!listing.id) {logErrorToProduction('[ProductCard] Missing listing ID, cannot navigate', new Error('Missing listing ID'), { component:'ProductListingCard' }),toast({title:\"Navigation Error\",,description:\"Product information is incomplete\",variant:\"destructive\"}),return,}router.push(`${detailBasePath}/${listing.id}`),},const dispatch  = useDispatch<AppDispatch />(),const addToCart = (setLoading(true),dispatch(addItem({ id:listing.id, title:listing.title, price:listing.price ?? 0 ) => {
  return $3;}
