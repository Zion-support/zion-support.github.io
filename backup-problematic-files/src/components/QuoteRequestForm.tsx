...
import { z } from "zod",export type QuoteRequestSteps = "service" | "details" | "timeline" | "budget" | "summary",const serviceStepSchema = z.object({serviceType:z.string().min(1),specificItem:z.object({ id:z.string() })}),export function QuoteRequestForm() {const router = useRouter(),const { toast } = useToast(),const [currentStep, setCurrentStep] = useState<QuoteRequestSteps>("service"),const [isSubmitting, setIsSubmitting] = useState(false),const [autoFillLoading, setAutoFillLoading] = useState(false),const [autoFillOpen, setAutoFillOpen]  = useState(false),const [formData, setFormData] = useState<QuoteFormData>({serviceType:"",serviceCategory:"",specificItem:null,projectName:"",projectDescription:"",startDate:undefined,endDate:undefined,timeline:"flexible",budget:{amount:0,type:"fixed";
  ...
  },const handleBack = () => {switch (currentStep) {case "details":setCurrentStep("service"),break,case "timeline":;
        setCurrentStep("details"),break,case "budget":;
        setCurrentStep("timeline"),break,case "summary":;
        setCurrentStep("budget"),break,default:;
        break;
    }
      ...
      <div className="max-w-3xl mx-auto">;
        ...
        </div>;<Card className="bg-zion-blue-dark border border-zion-blue-light mb-8">;
          <CardContent className="px-6 py-8">;
            <StepProgress currentStep={currentStep} />;<div className="mt-8">;
              {renderStepContent()}
...
}const renderStepContent = () => {switch (currentStep) {default: return null;
}
}";
