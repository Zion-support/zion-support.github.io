        ...
        <Input;
          ...
          onKeyDown={(e) => {if (!focused || suggestions.length === 0) {if (e.key === 'Escape') {e.preventDefault(),setFocused(false),setHighlightedIndex(-1),inputRef.current?.blur()}'
              ...
              if (e.key === 'Enter' && value.trim()) {e.preventDefault(), // Prevent form submission if SearchBar is in a form;'
                ...
                fireEvent('search', { "search_term":value }),router.push(`/search?q=${encodeURIComponent(value)}`),setFocused(false),inputRef.current?.blur()}`              return}switch (e.key) {case 'ArrowDown':;'
                }
