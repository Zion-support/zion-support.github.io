import React, { useState, useEffect } from 'react',import { useForm } from 'react-hook-form',import { zodResolver } from '@hookform/resolvers/zod',import { z } from 'zod',import { Button } from '@/components/ui/button',import { Input } from '@/components/ui/input',import { Label } from '@/components/ui/label',import { useAuth } from '@/hooks/useAuth',import { toast } from '@/hooks/use-toast',import { CheckCircle, AlertCircle, Eye, EyeOff, Loader2  } from 'lucide-react';
import { cn } from '@/lib/utils',import { fireEvent } from '@/lib/analytics',import {logErrorToProduction} from '@/utils/productionLogger',const signupSchema = z.object({name:z.string().min(2, 'Full Name must be at least 2 characters').max(50, 'Name must be less than 50 characters'),email:z.string().email('Please enter a valid email address').min(1, 'Email is required'),password:z.string().min(8, 'Password must be at least 8 characters').regex(/[A-Z]/, 'Password must include at least one uppercase letter').regex(/[a-z]/, 'Password must include at least one lowercase letter').regex(/[0-9]/, 'Password must include at least one number').regex(/[^A-Za-z0-9]/, 'Password must include at least one special character'),confirmPassword:z.string()}).refine((data) => data.password === data.confirmPassword, {message:"Passwords don't match",path:["confirmPassword"]}),type SignupFormData = z.infer<typeof signupSchema>,interface SignupFormProps  {onSuccess?:(result:{email:string,emailVerificationRequired:boolean;
  }) => void,onError?:(error:string) => void;}interface FieldValidationState  {isValid:boolean,isValidating:boolean,error:string | null;
}export default function SignupForm() {const [isSubmitting, setIsSubmitting] = useState(false),const [showPassword, setShowPassword] = useState(false),const [showConfirmPassword, setShowConfirmPassword] = useState(false),const [fieldStates, setFieldStates] = useState<Record<string FieldValidationState>>({}),const { signUp }  = useAuth(),const {register,handleSubmit,formState:{ errors, isValid, touchedFields },setError,reset,watch,trigger} = useForm<SignupFormData>({resolver:zodResolver(signupSchema),mode:'onChange', // Enable real-time validation;
  }),const watchedFields  = watch(),// Real-time field validation with debounce;
  useEffect(() => {const timeouts:Record<string NodeJS.Timeout>  = {},Object.keys(watchedFields).forEach((fieldName) => {const typedFieldName = fieldName as keyof SignupFormData,if (touchedFields[typedFieldName]) {setFieldStates(prev => ({...prev,[fieldName]:{isValid:prev[fieldName]?.isValid ?? false,isValidating:true,error:prev[fieldName]?.error ?? null;          }
        })),timeouts[fieldName] = setTimeout(async () => {const result = await trigger(typedFieldName),const error  = errors[typedFieldName],setFieldStates(prev => ({...prev,[fieldName]:{isValid:result,isValidating:false,error:error?.message || null;
            }
          })),}, 300),}
      ...
      </div>;{/* Password Field with Strength Meter */}
      <div className="space-y-2">;
        ...
        {watchedFields.password && (<div className="space-y-2">;
...
if (state?.isValidating) {if (state?.isValid && !state?.error) {return <CheckCircle className="h-4 w-4 text-green-500" />;
}if (state?.error) {";
...
}className="space-y-6" > {/* Name Field */ ";
}<div className="space-y-2" > <Label htmlFor="name" className="text-sm font-medium" > Full Name <span className="text-red-500" >*</span> relative"> <Input {errors.name.message ;
}</p>) ";
}</div> </Label> <div className=" relative"> <Input {errors.email.message ;
}</p>) ";
}</div> </Label> <div className=" relative"> <Input) : (<Eye className=" h-4 w-4"/>)}</Button> </div> </div> <span>Password Strength</span> <span className= {';
  cn ('font-medium', passwordStrength.strength >= 4 ? 'text-green-600': passwordStrength.strength >= 3 ? 'text-blue-600': passwordStrength.strength >= 2 ? 'text-yellow-600': 'text-red-600')}> {passwordStrength.label ";
}</span> </div> <div className=" w-full bg-gray-200 rounded-full h-2"> <div className= {';
  cn ('h-2 rounded-full transition-all duration-300', passwordStrength.color)}style= {{width: `$ {passwordStrength.percentage ;
}%` ;
}
