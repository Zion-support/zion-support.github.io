...
clientName: string;}
onContractGenerated?: (contractContent: string) => void ;}
}export function ContractBuilder () {const [formValues, setFormValues] = useState<ContractFormValues | undefined /> (undefined)const [templateManagerOpen,  setTemplateManagerOpen] = useState (false)const [showSmartContractBuilder, setShowSmartContractBuilder] = useState (false)const handleLoadTemplate = (setFormValues (templateData)) => {
  return $3;}
}const handleContractGenerated = (if (onContractGenerated) {onContractGenerated (contract)) => {
  return $3;}
}
