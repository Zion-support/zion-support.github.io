...
function OnboardingTracker() {
  ...
  steps && steps.map ( (step) => (<divkey= {
  step && step.id "
}className="flex items-center"> <div className="mr-3" > {";
  step && step.completed ? (<CheckCircle className="h-5 w-5 text-zion-cyan" /> h-5 w-5 text-zion-slate-light"/>) ;
}</div> </div> {";
  !step && step.completed && step && step.action && (<Button asChild variant=" ghost"size=" sm"className=" text-zion-purple hover:text-zion-cyan hover:bg-zion-blue" > <Linkhref= {
  step && step.link 
}> </Link> </Button>) ;
}</div>) ) ;
}</div> </div>) ;
