...
import { useEffect, useState   } from 'react';export function ModeToggle() {const { theme, toggleTheme } = useTheme(),const [isClient, setIsClient]  = useState(false),// Ensure we're on the client side to avoid hydration mismatches;
  useEffect(() => {setIsClient(true),}, []),// Determine the actual resolved theme for display purposes;
  const resolvedTheme = (() => {if (!isClient) return 'light', // Default for SSR;if (theme === "system") {return window.matchMedia("(prefers-color-scheme:dark)").matches ? "dark" :"light";
    }
    return theme,})(),const isDarkMode  = resolvedTheme === "dark",const handleToggle = () => {try {// Determine the new theme we are switching TO;
  ...
  ),//Use the ThemeProvider hook directly to ensure no conflicts import {{{useTheme ;
  } from '@/components/ThemeProvider';
import {{{logIssue ";
}from "@/utils/logIssue" import {{{useEffect, useState ";
}from "react" export function ModeToggle () {const {theme, toggleTheme ;
}= useTheme ()const [isClient, setIsClient] = useState (false)//Ensure we're on the client side to avoid hydration mismatches useEffect ( () => {setIsClient (true)}, [])//Determine the actual resolved theme for display purposes const resolvedTheme = ( () => {';
  if (!isClient) return 'light', //Default for SSR ;
}return theme;
}) ()try {//Determine the new theme we are switching TO logInfo (`Theme toggle: $ {resolvedTheme ;
} $ {newTheme ;
