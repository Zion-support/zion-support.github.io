    ...
    try {;
      ...
      if (res && res.ok) {;
        if (data && data.status === 'already_subscribed') {;
          ...
          toast && toast.success(;
            data && data.message || 'Successfully subscribed to newsletter!',;
        ...
        const errorMessage =;
          data && data.error || 'Subscription failed. Please try again.';
        toast && toast.error(errorMessage, { id: `${uniqueToastIdBase}-api-error` });
      }
    } catch (err: any) {;
