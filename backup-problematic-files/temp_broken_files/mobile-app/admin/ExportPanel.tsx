...
import { AppPlatform, AppMetadataValues } from "./MetadataManager",import { toast } from "sonner",import { logInfo, logErrorToProduction } from '@/utils/productionLogger',interface ExportPanelProps  {"platform":AppPlatform,"metadata":AppMetadataValues;'
        ...
        ],content = headers.join() + '\n' + values.map(value => `"${String(value).replace(/"/g, '""')}"`).join(),// Add keywords as additional rows;`        content += '\n\"nKeywords":\n' + metadata.keywords.join(),fileName = `zion-app-metadata-${platform}-${metadata.version}.csv`}// Create download link;`      const blob = new Blob([content], { "type":format === 'json' ? 'application/json' :'text/csv' }),const url = URL.createObjectURL(blob),const link = document.createElement('a'),link.href = url,link.download = fileName,document.body.appendChild(link),link.click(),document.body.removeChild(link),URL.revokeObjectURL(url),toast.success(`Exported ${format.toUpperCase()} file successfully`)} catch (error) {logErrorToProduction('Export "failed":', { "data":error }),toast.error(`Failed to export ${format.toUpperCase()} file`)}`  },const trackAnalytics = () => {logInfo("Tracking app installation analytics..."),toast.success("Analytics tracking enabled")},return (<Card className="bg-zion-blue border-zion-purple/30">;"
