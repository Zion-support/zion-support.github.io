...
}/> {;"  /* AI Enhancement Option */ ";"}<FormField AI Profile Enhancement </FormLabel> <FormDescription className=" text-zion-slate-light"> Let AI help optimize your service description for better visibility and client engagement </FormDescription> </div> <FormControl> <Switch /> </FormControl> </FormItem>) ";"}/> <Button type=" button"variant=" outline"className=" border-zion-purple text-zion-purple hover:bg-zion-purple/10"onClick={;
  generateEnhancedProfile ;
}disabled= {;
  isGenerating ;"}> </Button> </div>) ";"}AI-Generated Content </h4> <Button type=" button"size=" sm"className=" bg-zion-purple hover:bg-zion-purple-dark text-white"onClick={;"  applyGeneratedContent ";"}> <Check className=" mr-1 h-3 w-3"/> Apply </Button> </div> <div className=" space-y-4"> <div> </div> {;
  generatedContent.services && generatedContent.services.length > 0 && (<div> <Badge key= {;"  index ";"}className=" bg-zion-purple/20 hover:bg-zion-purple/30 text-zion-purple border-none">{;
  service ;
}</Badge>) ) ;
}</div> </div>) ;"}</div> </div>) ";"}</div> <Separator className=" bg-zion-blue-light/50"/> <FormItem> <FormLabel className=" text-zion-slate-light">Services</FormLabel> <div className=" flex gap-2"> <FormControl> <Input > Add </Button> </div> <FormDescription className=" text-zion-slate"> Press Enter or click Add to include a service </FormDescription> <FormMessage className=" text-red-400"/> </FormItem>) ;
