...
import { ProfileExperience as ProfileExperienceType } from "@/types/profile",import { Briefcase   } from 'lucide-react';interface ProfileExperienceProps  {experience:ProfileExperienceType[];
...
export function ProfileExperience({ experience } ProfileExperienceProps) {;
 ...
 import {{{Briefcase ;
}from 'lucide-react' interface ProfileExperienceProps  {experience: ProfileExperienceType[] ;
}export function ProfileExperience () {return (<div className="bg-zion-blue-dark border border-zion-blue-light rounded-lg p-6 mb-8" > <h3 className="text-xl font-bold text-white mb-4" >Experience</h3> space-y-6"> {:temp_broken_files/profile/ProfileExperience.tsx;
  experience.map ( (exp) => (<div key= {"  exp.id ";"}className=" flex"> <div className=" mr-4"> <div className=" h-10 w-10 rounded-full bg-zion-purple/20 flex items-center justify-center"> <Briefcase className=" h-5 w-5 text-zion-purple" /> </div> </div> <div> </div> </div>) )}</div> </div>)"}'";
