        ...
        <AspectRatio ratio={3/1} className="bg-zion-blue-light">;
          ...
          <div className="flex flex-col md:flex-row md:items-end -mt-16 md:-mt-20 relative z-10 mb-6 md:mb-10">;
            <Avatar className="h-24 w-24 md:h-32 md:w-32 border-4 border-zion-blue-dark ring-2 ring-zion-purple/30">;
              ...
              </div>;<h2 className="text-lg md:text-xl text-zion-slate-light mb-1">{title}</h2>;<div className="flex flex-wrap items-center gap-3 mt-2">;
...
aiScore?: number;
profileType: 'service' | 'talent' ;
}export function ProfileHero () {return (<div className="w-full overflow-hidden" > <div className="relative" > <AspectRatio ratio= {"  3/1 ";"}className="bg-zion-blue-light" > {coverImageUrl ? (<img src= {coverImageUrl ;
}alt= {`$ {name ;"}cover` ";"}className="w-full h-full object-cover" loading="lazy" w-full h-full bg-gradient-to-r from-zion-blue via-zion-blue-light to-zion-blue-dark" />)}</AspectRatio> </AvatarFallback>)}</Avatar>)}> {';
  profileType === 'service' ? 'Service Provider' : 'Talent' ;
}</Badge> </div>)}</div>)}AI Match: {aiScore ;
}% </div>):temp_broken_files/profile/ProfileHero.tsx;
