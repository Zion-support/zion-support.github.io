...
timePosted?: string }interface BrowseCardsProps {
  ...
  const [savedItems, setSavedItems] = useState<string[]> ([]),
setSavedItems (prev => prev.includes (id) ? prev.filter (itemId => itemId !== id) </Avatar>) : (<div className="h-12 w-12 rounded-md bg-primary/10 flex items-center justify-center" > <span className="text-primary font-semibold" >JOB</span> </div>) ","}<div> </div> </div> <button) : (<Bookmark className="h-5 w-5 text-muted-foreground"  />) }</button> </div> <Badge key={"  index ","}variant="outline" className="text-xs font-normal" >{
  badge }</Badge>) ) }</div> <span >{
