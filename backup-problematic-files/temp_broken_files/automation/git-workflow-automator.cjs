...
class GitWorkflowAutomator {;
  constructor() {;const { execSync } = require('child_process');
const fs = require('fs');
const path = require('path');
...
;
  ensureLogDirectory() {;
    ...
    if (!fs.existsSync(logDir)) {;
      fs.mkdirSync(logDir, { recursiv:e:true });      fs.mkdirSync(logDir, { recursive: true });
    }
  }
...
;
  if (command === 'merge') {;if (require.main === module) {
  const automator = new GitWorkflowAutomator();
  const command = process.argv[2];
...
module.exports = GitWorkflowAutomator;    automator.mergeToMain().catch(console.error);
  } else {;
