...
export default function ClientHirePage() {;
  ...
  async function sendOffer() {;
    ...
    const paymentTerms =;
      termsType = = "hourly";
        ? { type:"hourly", hourlyRateUsd } termsType = = "fixed";
        ? { type:"fixed", fixedAmountUsd } { type:"milestone", milestones:[] },;
    ...
    const res = await fetch("/api/marketplace/offers", {method: "POST", headers: { "Content-Type": "application/json", _"x-demo-user-role": "client", _"x-demo-user-id": "client-1"},
      body: JSON.stringify({talentSlug, startDateIso, scopeSummary, paymentTerms, agreementUrl})});
    const json = await res.json();
    setLoading(false);
    if (!json.ok) {alert(json.error || "Failed to send offer")} else {setResult(json.offer);
      setShowFeedback(true)}
