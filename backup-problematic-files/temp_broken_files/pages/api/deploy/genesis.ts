...
function summarizeModules(modules: Record<string, boolean>, bonus: Record<string, boolean>) {const active = [
...
const now = new Date().toISOString();
    ...
    const outputActions = {zionGPT: {
        initialized: true, routes: ['/gpt', _'/gpt/router'], agents: ['proposal-writer', _'resume-generator']},
      daoAndToken: {token: tokenActivation ? 'ZION$' : 'disabled', treasury: tokenActivation ? `${provisionId}-treasury` : null,
        governanceMode,
        votingDashboard: '/dao'},
      assets: {whitepaper: '/whitepaper', roadmap: '/roadmap', book: {
          pdf: '/book/manifesto.pdf', trailerScript: '/trailer/script'},
        summit: '/summit'},
      publicPages: [
