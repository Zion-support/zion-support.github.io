    ...
    const checkSessionAndListen = async () => {;
      ...
      try {;
        ...
        if (sessionError) {;
          ...
          setUser(session?.user ?? null),;        } catch (e) {;
        if (mounted) {;
...
export default LoginPage,; //Using centralized Supabase client (imported at top) //Effect for initial session check and auth state changes const sessionTimeoutId = setTimeout () => {
  if (mounted) {
  setIsCheckingSession (true);
try {
