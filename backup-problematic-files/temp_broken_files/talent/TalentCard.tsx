import React from 'react',import { useRouter } from 'next/router',import { Badge } from "@/components/ui/badge",import { Button } from "@/components/ui/button",import { Card, CardContent, CardFooter } from "@/components/ui/card",import { MapPin, Clock, ArrowRight, CheckCircle2  } from 'lucide-react';
import { FavoriteButton } from "@/components/FavoriteButton",import { TalentProfile } from "@/types/talent",import { RatingStars } from '@/components/RatingStars',import { useAuth } from '@/context/auth/AuthProvider',import { useCart } from '@/context/CartContext',export interface TalentCardProps  {talent:TalentProfile,onViewProfile:(id:string) => void,onRequestHire:(talent:TalentProfile) => void,isAuthenticated:boolean;
      ...
      <div className="p-6">;
        <div className="flex items-start">;
          ...
          <div className="flex-1">;
            ...
            <p className="text-white font-medium">{talent.professional_title}</p>;{/* Location & Availability */}
  ...
  )},export const TalentCard = React.memo(TalentCardComponent),TalentCard.displayName = 'TalentCard',export interface TalentCardProps  {talent: TalentProfile;
...
onRequestHire;
isAuthenticated ;
}: TalentCardProps) => {const router = useRouter ()//Navigate directly to the talent profile router.push (`/talent/$ {talent.id ;
}`)//Also call the onViewProfile callback if provided </div>)}</div> <span >{talent.availability type ;
}</span> </div>)}</div> </div> </div> <span key= {index ;
}className="px-2 py-1 text-xs rounded-full bg-zion-blue-light text-zion-slate-light" >{skill ;
}</span>) )}+ {(talent.skills?.length || 0) - 5 ;
}more </span>)}</div> </div>):temp_broken_files/talent/TalentCard.tsx;
...
}</div> <Button size="sm" variant="secondary" onClick={handleRequestHire ";
