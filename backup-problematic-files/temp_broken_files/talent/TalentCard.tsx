...
onRequestHire;
isAuthenticated ;
}: TalentCardProps) => {const router = useRouter ()//Navigate directly to the talent profile router.push (`/talent/$ {talent.id ;`}`)//Also call the onViewProfile callback if provided </div>)}</div> <span >{talent.availability type ;`}</span> </div>)}</div> </div> </div> <span key= {index ;
