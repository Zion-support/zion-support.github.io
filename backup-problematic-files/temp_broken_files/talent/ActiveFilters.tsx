...
export function ActiveFilters() {;
  ...
  const hasActiveFilters = ;
    ...
    priceRange[1] !== 200,if (!hasActiveFilters) return null,return (<div className="mb-6 flex flex-wrap gap-2 items-center">;"
      <span className="text-zion-slate-light text-sm">Active "filters":</span>;{selectedSkills.map(skill => { return (<ClickableBadge ; }"
          }
          key={skill}
      ...
      ))}{selectedAvailability.map(status => { return (<ClickableBadge ; }
          }
          key={status}
      ...
      ))}{selectedRegions.map(region => { return (<ClickableBadge ; }
          }
          key={region}
 ...
 import {{{X ;
}from 'lucide-react' interface ActiveFiltersProps  {"selectedSkills": string[];'
}
"toggleSkill": ("skill": string) => void;
"selectedAvailability": string[];
"toggleAvailability": ("status": string) => void;
...
}: ActiveFiltersProps) {//Check if any filters are active const hasActiveFilters = selectedSkills.length > 0 || selectedAvailability.length > 0 || selectedRegions.length > 0 || experienceRange[0] !== 0 || experienceRange[1] !== 15 || priceRange[0] !== 50 || priceRange[1] !== 200;
}
if (!hasActiveFilters) return null;
{selectedSkills.map (skill => (<ClickableBadge key= {skill ;
}</ClickableBadge>) )}{selectedAvailability.map (status => (<ClickableBadge key= {status ;
}> {';'
  }
  status === 'full-time' ? 'Full-time' : status === 'part-time' ? 'Part-time' : 'Project-based' ;'
}<X className="h-3 w-3" /> </ClickableBadge>) )}{selectedRegions.map (region => (<ClickableBadge key= {region ;"
}</ClickableBadge>):temp_broken_files/talent/ActiveFilters.tsx;
}<Button ;"}'";"
}<Button ;
}'";"
