        ...
        <div className="flex flex-col gap-4">,
          {messages.map((message) => (
            <ChatMessage
              ...
              isUser={message.sender === &quot,user&quot}
...
timestamp: new Date () }]) };
  ...
  reply.id }text={
  reply.text }onClick={
  () => handleQuickReply (reply.text) }/>) ) ,"}</div> </div>) ","}Need more help? </p> <div className="flex gap-2" > <Button > Chat with Live Agent </Button> <Button > Email Support </Button> </div> </div>) ","}aria-label="Send message" h-4 w-4" /> </Button> </form> </div> </div>) ,"}'"]