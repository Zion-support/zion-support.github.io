...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method = = 'GET') {
    ...
    if (useSupabase) {
      ...
      if (params.format = = 'csv') {
        ...
        return res.status(200).send(toCsv(data || [])        return res.status(200).send(toCsv(data || []);
      }
      ...
      if (params.format = = 'csv') {
        res.setHeader('Content-Typetext/csv'),
        res.setHeader('Content-Disposition', `attachment, filename="${type}.csv"`),
        return res.status(200).send(toCsv(pageItems)        return res.status(200).send(toCsv(pageItems);
      }
  ...
  return res.status(405).json({ error: 'Method not allowed' })}return res.status (200) .send (toCsv (data || []);
