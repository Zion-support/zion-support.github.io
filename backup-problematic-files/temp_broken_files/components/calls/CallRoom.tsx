    ...
    <div className=&quot;min-h-screen bg-gray-950 text-gray-100 flex flex-col&quot;>
      ...
      <div className={_`flex-1 p-4 grid gap-4 ${gridCols}`}>
        {participants.map(p, idx) => (
          <ParticipantTile key={String(p as any).sid || (p as any).identity) + idx} participant={p} isLocal={p instanceof LocalParticipant} displayName={_(p as any).name || (p instanceof LocalParticipant ? 'You' : undefined)} />
        )}
      </div>
    </div>
  )}