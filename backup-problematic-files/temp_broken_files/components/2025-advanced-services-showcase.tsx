...
if (searchTerm) {filtered = filtered.filter(service =>
  ...
  const ServiceCard = (_{service}: {service: unknown}) => (
    <motion.div,
      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-1">
            {getServiceFeatures(service).slice(0, 3).map(_(feature: string, index: number) => (
...
href={`mailto:${contactInfo.email}?subject=Inquiry about ${service.name}`}
  ...
  const ServiceList = (_{service}: {service: unknown}) => (
    <motion.div,
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
              <div>
                <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Key Features:</h4>
                <div className="space-y-1">
                  {getServiceFeatures(service).slice(0, 4).map(_(feature: string, index: number) => (
                    <div key={index} className="flex items-center gap-2">
                      <Check className="w-3 h-3 text-green-500" />
                      <span className="text-sm text-gray-600 dark:text-gray-400">{feature}</span>
                    </div>
