...
{text: 'The marketplace is the new operating system.',}
  attribution: 'Founder',},],},}function fileToBase64(file: File): Promise<string /> {return new Promise((resolve, reject) => {const reader = new FileReader()reader.onload = () => resolve(reader.result as string)reader.onerror = reject;}
...
export default function BookBuilder() {
...
)}, [project])async function handleGenerateWithAI() {setBusy(true)try {const res = await fetch('/api/book/generate', {method: 'POST',}
  headers: { 'Content-Type': 'application/json' },body: JSON.stringify({meta: project.meta,chapters: project.chapters,}),})const data = await res.json()if (data?.chapters) {setProject(p => ({ ...p, chapters: data.chapters }))}
  ...
  async function handleExportEpub() {setBusy(true)try {const res = await fetch('/api/book/export/epub', {method: 'POST',}
  headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ project }),})const blob = await res.blob()const url = URL.createObjectURL(blob)const a = document.createElement('a')a.href = url;
    ...
    const arr = await Promise.all(Array.from(files).map(fileToBase64))setProject(p => ({...p,visuals: {...p.visuals,[target as any]: [...(p.visuals[target as any] as string[]), ...arr],},}))}return (<div className='space-y-8' />;
