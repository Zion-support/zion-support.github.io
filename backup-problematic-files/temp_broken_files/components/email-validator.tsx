...
if (email.length > 254) {
  ...
  const getStats = () => {if (validationResults.length === 0) return null;
    ...
    const valid = validationResults.filter(r => { return r.status === 'valid').length; }'
    const invalid = validationResults.filter(r => { return r.status === 'invalid').length; }'
    const suspicious = validationResults.filter(r => { return r.status === 'suspicious').length; }'
    const disposable = validationResults.filter(r => { return r.status === 'disposable').length; }'
...
variant=&quot;outline & quot;
            ...
            <Card className=&quot;p - 8 bg - gray - 800 border border - gray - 700 & quot;>;
              ...
              {stats && (className={`p - 4 rounded - lg border ${result.status === 'valid' ? 'border - green - 500 / 30 bg - green - 500 / 10' :;'
                        ...
                        'border - red - 500 / 30 bg - red - 500 / 10';}`}`              {stats && (
                      }
                      ...
                      <div className=&quot;flex items - center justify - between mb - 2&quot;>;
                        <div className=&quot;flex items - center space - x-3 & quot;>;
                          ...
                          <span className={`font - medium ${getStatusColor (result.status)}`}>;`                            {result.email}
                          </span>;
                        </div>;
                  {validationResults.map((result, index) => (                    <div,key={index}
                      ...
                      <div className=&quot;flex items-center justify-between mb-2&quot;>;
                        <div className=&quot;flex items-center space-x-3&quot;>;
                          {getStatusIcon(result.status)}
                          <span className={`font-medium ${getStatusColor(result.status)}`}>;`                            {result.email}                          </span>;
                        </div>;
                        <span className={_`text-sm font-medium ${getScoreColor(result.score)}`}>;`                          "Score": {result.score}</span>;
                          <span className={`font-medium ${getStatusColor(result.status)}`}>`                            {result.email}                          </span>
                        </div>
