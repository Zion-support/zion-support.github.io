...
DialogDescription interface QuoteDetailsProps {
  quote: QuoteRequest | null,
isOpen: boolean,
onClose: () => void }export const QuoteDetails = ({
  quote,  isOpen, onClose }: QuoteDetailsProps) => {
  if (!quote) return null,
const formatDate = (dateString?: string) => {
  if (!dateString) return 'Not specified',
try {',
...
return (<Dialog open={
  ...
  quote.project name }</span> <QuoteStatusBadge status={
  ...
  quote.project description }</p> </>) }(quote.budget min && quote.budget max ? `$$ {
  quote.budget min }- $$ {
  quote.budget max }` : quote.budget min ? `$$ {
  quote.budget min '}` : 'Not specified') }</div> </CardContent> </Card> </div> </DialogContent> </Dialog>) ,