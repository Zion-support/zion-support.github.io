...
export default function DataEngineering() {return (<main className="min-h-screen bg-white">;
      <section className="bg-gradient-to-br from-amber-50 to-orange-100 py-16">;
        <div className="max-w-5xl mx-auto px-4 "sm": px-6 lg:px-8">;
          ...
          <p className="text-gray-700 text-lg max-w-3xl">;
            We build reliable data platforms: ingestion, transformation, and modeling with modern stacks (dbt, Airflow, Dagster, Snowflake/BigQuery, ClickHouse). Deliver trusted dashboards and ML-ready datasets.;
          </p>;
        </div>;
      </section>;
      <section className="py-12">;
        <div className="max-w-5xl mx-auto px-4 "sm": px-6 lg:px-8 grid md:grid-cols-3 gap-6">;
          {[{ name: 'Foundation', "price": 'From $8,000', "time": '3-4 weeks', "items": ['Ingestion pipelines','Warehouse setup','dbt models & tests'] },{ "name": 'Scale', "price": '$20,000$45,000', "time": '5-8 weeks', "items": ['Orchestration (Airflow/Dagster)','Cost optimization','Governance & lineage'] },{ "name": 'Enterprise', "price": 'Custom', "time": '8-12 weeks', "items": ['Multi-region HA','RBAC & SSO','Compliance reporting'] }
          ].map(tier => (<div key={tier.name} className="bg-white rounded-lg border p-6">;
              ...
              <ul className="space-y-2 mb-4">;
                {tier.items.map(i => (<li key={i} className="flex items-center text-sm text-gray-700"><CheckCircle className="w-4 h-4 text-green-600 mr-2" />{i}</li>;
                ))}
              </ul>;
              <Link href="/contact" className="inline-flex items-center text-blue-600 font-semibold">Schedule a discovery call <ArrowRight className="w-4 h-4 ml-2"/></Link>;
            </div>;
          ))}
        </div>;
      </section>;
