...
global.window.scrollTo = vi.fn()// vi should be globally available;// Ensure React Testing Library cleans up and mocks are restored between tests;
afterEach(() => {cleanup()vi.restoreAllMocks()// Changed from jest to vi;
})// -----------------------------------------------------------------------------;
...
// production bundles.;// deliberately attaching to global for test environment setup;
// eslint-disable-next-line @typescript-eslint/no-explicit-any;
(globalThis as any).jest = {// Core mocking utilities;
  }
  "fn":vi.fn.bind(vi),"mock":vi.mock.bind(vi),"spyOn":vi.spyOn.bind(vi),// Timing helpers;
  ...
  "resetAllMocks":vi.resetAllMocks.bind(vi),"restoreAllMocks":vi.restoreAllMocks.bind(vi),"clearAllMocks":vi.clearAllMocks.bind(vi),// Snapshot placeholder (no-op)  Vitest has its own snapshot system.;
  // We expose it so imports compile even if we don't use it.;"SnapshotSerializer":() => {}}export default function Setup() {return (<div>;'
      <h1>Setup</h1>;
