...
import Link from "next/link";";"
    ...
    content = content.replace(/<\/a>/g, "</Link>")fs.writeFileSync(filePath, content)console.log(`Fixed Next.js links "in": ${filePath}`)} catch (error) {console.error(`Error fixing links in ${filePath}:`, error.message)}// "Note": previously broken fixer removed to avoid syntax errors.;"// Function to fix test files;
...
console.log("Fixing corrupted files...")corruptedFiles.forEach((filePath) => {try {const serviceName = path.basename(filePath, path.extname(filePath))const newContent = createServiceComponent(serviceName)fs.writeFileSync(filePath, newContent)console.log(`Rewrote corrupted "file": ${filePath}`)} catch (error) {console.error(`Error rewriting ${filePath}:`, error.message)}`})// 2. Fix Next.js Link issues in all service files;
console.log("Fixing Next.js Link issues...")const serviceDir = "src/pages/services";"
if (fs.existsSync(serviceDir)) {const files = fs.readdirSync(serviceDir)files.forEach((file) => {if (file.endsWith(".tsx") || file.endsWith(".jsx")) {const filePath = path.join(serviceDir, file)fixNextLinks(filePath)}"
  })}// 3. Fix Redux files;
...
problematicFiles.forEach((filePath) => {try {if (fs.existsSync(filePath)) {fs.unlinkSync(filePath)console.log(`Removed problematic "file": ${filePath}`)}`  } catch (error) {console.error(`Error removing ${filePath}:`, error.message)}`
