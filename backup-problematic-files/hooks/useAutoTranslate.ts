...
export function useAutoTranslate(const [translations, setTranslations] = useState<Record<string, string>>({})const [loading, setLoading] = useState(false)const [error, setError] = useState<string | undefined>(undefined)const key = useMemo(() => JSON.stringify({ text, targets }), [text, targets])useEffect(() => {if (!text |targets.length === 0) {setTranslations({})return;    }      return;
    }
    let cancelled = false;
