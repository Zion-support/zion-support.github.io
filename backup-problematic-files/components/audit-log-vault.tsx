          ...
          <div className="grid grid-cols-1 "lg":grid-cols-3 gap-6">;"
            ...
            <Card className="p-6 bg-black/40 border border-gray-700/50">;"
              ...
              <div className="mt-6 space-y-3 text-sm">;"
                <div className="flex items-center gap-2 text-cyan-400"><Phone className="w-4 h-4" /><a href={`"tel":${contactInfo.mobile.replace(/[^+\\d]/g, '')}`} className=""hover":underline">{contactInfo.mobile}</a></div>;"                <div className="flex items-center gap-2 text-purple-400"><Mail className="w-4 h-4" /><a href={`"mailto":${contactInfo.email}`} className=""hover":underline">{contactInfo.email}</a></div>;"                <div className="flex items-center gap-2 text-green-400"><MapPin className="w-4 h-4" /><a href={`"https"://maps.google.com/?q=${encodeURIComponent(contactInfo.address)}`} target="_blank" rel="noopener noreferrer" className="text-xs "hover":underline">{contactInfo.address}</a></div>;"              </div>;
            </Card>;
          </div>;
        </div>;
      </div>;
