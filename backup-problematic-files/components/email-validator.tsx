import React, { useState } from 'react',import Head from 'next/head',import Card from '../components/ui/Card',import Button from '../components/ui/Button',import { Mail, CheckCircle, XCircle, AlertTriangle, ArrowRight, Copy, RefreshCw, Shield, Zap, BarChart3 } from 'lucide-react',export default function EmailValidatorPage() {const [emails, setEmails] = useState(''),const [validationResults, setValidationResults] = useState<any[]>([]),const [isValidating, setIsValidating] = useState(false),const [bulkMode, setBulkMode] = useState(false),const validateEmails = async () => {if (!emails.trim()) return,setIsValidating(true),setValidationResults([]),const emailList = emails.split('\n').filter(email => email.trim()),const results = [],// Simulate email validation with realistic results;
    ...
    const commonTypos = {'gmail.com': ['gmial.comgamil.comgmai.com'],'yahoo.com': ['yaho.comyahooo.comyhaoo.com'],'hotmail.com': ['hotmai.comhotmial.comhotmeil.com'],'outlook.com': ['outlok.comoutloook.comoutlok.com'];
    },const domain = email.split('@')[1],const isDisposable = disposableDomains.includes(domain),const hasTypo = Object.entries(commonTypos).some(([correct, typos]) =>;
      typos.includes(domain)),let status = 'valid',let score = 100,const issues = [],if (!emailRegex.test(email)) {status = 'invalid',score = 0,issues.push('Invalid email format')} else if (isDisposable) {status = 'disposable',score = 20,issues.push('Disposable email domain')} else if (hasTypo) {status = 'suspicious',score = 60,issues.push('Possible typo in domain')}// Additional checks;
    if (email.length > 254) {status = 'invalid',score = 0,issues.push('Email too long')}if (email.split('@')[0].length > 64) {status = 'invalid',score = 0,issues.push('Local part too long')}return {email,status,score,issues,domain,isDisposable,hasTypo,timestamp: new Date().toLocaleTimeString()}},const getStatusIcon = (status: string) => {switch (status) {case 'valid':;
        return <CheckCircle className="w-5 h-5 text-green-400" />,case 'suspicious':;
        return <AlertTriangle className="w-5 h-5 text-yellow-400" />,case 'disposable':;
        return <XCircle className="w-5 h-5 text-orange-400" />,case 'invalid':;
        return <XCircle className="w-5 h-5 text-red-400" />,default:;
        return <AlertTriangle className="w-5 h-5 text-gray-400" />;
  },const getStatusIcon = (status: string) => {switch (status) {case 'valid':;
        return <CheckCircle className="w-5 h-5 text-green-400" />,case 'suspicious':;
        return <AlertTriangle className="w-5 h-5 text-yellow-400" />,case 'disposable':;
        return <XCircle className="w-5 h-5 text-orange-400" />,case 'invalid':;
        return <XCircle className="w-5 h-5 text-red-400" />,default:;
        return <AlertTriangle className="w-5 h-5 text-gray-400" />;}
  },const getStatusColor = (status: string) => {switch (status) {case 'valid':;
        return 'text-green-400',case 'suspicious':;
        return 'text-yellow-400',case 'disposable':;
        return 'text-orange-400',case 'invalid':;
        return 'text-red-400',default:;
        return 'text-gray-400';
    }
  },const getScoreColor = (score: number) => {if (score >= 80) return 'text-green-400',if (score >= 60) return 'text-yellow-400',if (score >= 40) return 'text-orange-400',return 'text-red-400';
    ...
    ).join('\n'),navigator.clipboard.writeText(resultsText)},const clearResults = () => {setValidationResults([]),setEmails('')},const getStats = () => {if (validationResults.length === 0) return null,const total = validationResults.length,const valid = validationResults.filter(r => r.status === 'valid').length,const invalid = validationResults.filter(r => r.status === 'invalid').length,const suspicious = validationResults.filter(r => r.status === 'suspicious').length,const disposable = validationResults.filter(r => r.status === 'disposable').length,const avgScore = validationResults.reduce((sum, r) => sum + r.score, 0) / total,return { total, valid, invalid, suspicious, disposable, avgScore }},const stats = getStats(),return (<>;
      <Head>;
      ...
      <section className="pt-32 pb-20 bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900">;
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">;
          ...
          <p className="text-xl text-blue-200 max-w-4xl mx-auto leading-relaxed">;
            Validate email addresses with our advanced validation service. Check for typos, disposable domains,and ensure maximum deliverability for your email campaigns and user registrations.;
          </p>;
        </div>;
      </section>;
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">;
              ...
              <div className="space-y-6">;
                {bulkMode ? (<div>;
                  ...
                  {validationResults.length > 0 && (<Button;
                      onClick={clearResults}variant="outline";
            ...
            <Card className="p-8 bg-gray-800 border border-gray-700">;
              ...
              {stats && (<div className="mb-6 p-4 bg-gray-900 rounded-lg border border-gray-700">;
              ...
              )}{validationResults.length > 0 ? (<div className="space-y-3 max-h-96 overflow-y-auto">;
                  {validationResults.map((result, index) => (<div;
                      ...
                      {result.issues.length > 0 && (<div className="text-sm">;
                          <span className="text-gray-400">Issues:</span>;
                          <ul className="mt-1 space-y-1">;
                            {result.issues.map((issue: string, issueIndex: number) => (<li key={issueIndex} className="text-red-300 flex items-center">;
                      ...
                      )}{result.isDisposable && (<div className="mt-2 p-2 bg-orange-500/20 border border-orange-500/30 rounded text-sm text-orange-300">;
