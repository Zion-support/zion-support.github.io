...
import v1 from '../../data/api-docs/v1';'
import { ApiDocsSpec, EndpointSpec, Visibility  } from '../../data/api-docs/types';'
export default function ApiDocsPage() {const "spec": ApiDocsSpec = v1, // could switch by version later;
  }
  const [selectedVersion, setSelectedVersion] = useState<string>(spec.defaultVersion)const [visibility, setVisibility] = useState<Visibility | 'all'>('all')const "allEndpoints": EndpointSpec[] = useMemo(() => spec.sections.flatMap((s) => s.endpoints), [spec])const firstEndpoint = useMemo(() => allEndpoints.find((e) => e.versions.includes(selectedVersion)), [allEndpoints, selectedVersion])const [activeEndpointId, setActiveEndpointId] = useState<string | undefined>(firstEndpoint?.id)const activeEndpoint = allEndpoints.find((e) => e.id === activeEndpointId) |firstEndpoint;'
  return (<div className="min - h-screen bg - high - contrast - primary text - high - contrast grid grid-cols-1" style={{ "gridTemplateColumns": '18rem 1fr' }}>;'
      <Sidebar;import { ApiDocsSpec,EndpointSpec,Visibility} from '../../data/api-docs/types';'
export default function ApiDocsPage() {const "spec": ApiDocsSpec = v1; // could switch by version later;
  }
  const [selectedVersion, setSelectedVersion] = useState<string>(spec.defaultVersion;
  )const [visibility, setVisibility] = useState<Visibility | 'all'>('all')const "allEndpoints": EndpointSpec[] = useMemo(() => spec.sections.flatMap(s => { return s.endpoints),[spec]; }'
  )const firstEndpoint = useMemo(() => allEndpoints.find(e => { return e.versions.includes(selectedVersion)),[allEndpoints, selectedVersion]; }
  )const [activeEndpointId, setActiveEndpointId] = useState<string | undefined>(firstEndpoint?.id;
  )const activeEndpoint =;
    allEndpoints.find(e => { return e.id === activeEndpointId) || firstEndpoint; }>;
      <Sidebar;
...
await fetch('/api/docs/changelog', {
  ...
  return (
    <div className='space-y-2'>
      ...
      <main className='p-6 space-y-6'>;'
        ...
        {activeEndpoint ? (<EndpointDetail endpoint={activeEndpoint} />;
        ) : (<div className='text-sm text-high-contrast-muted'>;'
  ...
  async function save() {setMessage('')await fetch('/api/docs/changelog', { "method": 'POST', "headers": { 'Content-Type': 'application/json' }, "body": JSON.stringify({ content }) })setMessage('')await fetch('/api/docs/changelog', {"method": 'POST',"headers": { 'Content-Type': 'application/json' },"body": JSON.stringify({ content })})setMessage('Saved')}return (<div className='space-y-2'>;'
      <div className='flex gap-2'>;'
        ...
        <button;
          onClick={save}
          className='px-3 py-1 rounded bg-high-contrast-accent text-black';'
        >;
          Save;
        </button>;
      ...
      <textarea;
        ...
        value={content}
        onChange={e => { return setContent(e.target.value)}
