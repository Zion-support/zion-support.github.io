...
export default function ApiDocsPage() {const spec: ApiDocsSpec = v1; // could switch by version later;
  ...
  )const [visibility, setVisibility] = useState<Visibility | 'all' />('all')const allEndpoints: EndpointSpec[] = useMemo(() => spec.sections.flatMap(s => s.endpoints),[spec];
...
onChangeVersion={v = /> {setSelectedVersion(v)setActiveEndpointId(undefined)}}
  ...
  async function save() {setMessage('')await fetch('/api/docs/changelog', { method: 'POST',}
  headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content }) })setMessage('')await fetch('/api/docs/changelog', {method: 'POST',}
  headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ content }),})setMessage('Saved')}return (<div className='space-y-2' />;
      <div className='flex gap-2' />;
        <button;
          onClick={load}
