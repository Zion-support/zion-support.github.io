...
// Import our new 2025 V2 innovative services;
                          {(service as any).pricing?.professional ?;
                            `"Professional": ${(service as any).pricing.professional} | "Enterprise": ${(service as any).pricing.enterprise}` :;`                            `${service.price} ${service.period}`;`          >;
          ...
          >;
            {serviceCategories.map((category) => (<motion.div key={category.id} variants={fadeInUp}>;
                <div className="text-center mb-12">;"
                  <div className={`inline-flex p-4 rounded-2xl bg-gradient-to-r ${category.color} mb-6`}>;`                    {category.icon}
                  </div>;
