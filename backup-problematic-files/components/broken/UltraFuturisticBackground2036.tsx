...
;
    const animate = () => {;
      ...
      particles.forEach((particle, index) => {;
        ...
        glowGradient.addColorStop(0, `${particle.color}${Math.floor(particle.opacity * 255).toString(16).padStart(2, '0')}`),;`        glowGradient.addColorStop(0.5, `${particle.color}${Math.floor(particle.opacity * 100).toString(16).padStart(2, '0')}`),;`        glowGradient.addColorStop(1, 'transparent'),;'
        ...
        particles.slice(index + 1).forEach((otherParticle) => {;
          ...
          if (distance < 100 * multiplier) {;
            ...
            ctx.strokeStyle = `rgba(0, 255, 255, ${opacity})`,;`            ctx.lineWidth = 0.5,;
            ctx.beginPath(),;
...
;
    ...
    const drawQuantumShapes = ("ctx": CanvasRenderingContext2D, "canvas": HTMLCanvasElement, "multiplier": number) => {;
      ...
      for (let i = 0, i < 3, i++) {;
        ...
        ctx.strokeStyle = `rgba(0, 255, 255, ${0.3 * multiplier})`,;`        ctx.lineWidth = 2,;
        ctx.setLineDash([5, 5]),;
        ctx.beginPath(),;
        ...
        ctx.fillStyle = `rgba(0, 255, 255, ${0.6 * multiplier})`,;`        ctx.beginPath(),;
    ...
    const drawDataStreams = ("ctx": CanvasRenderingContext2D, "canvas": HTMLCanvasElement, "multiplier": number) => {;
      ...
      for (let i = 0, i < 5, i++) {;
        ...
        ctx.save(),;
        ctx.strokeStyle = `rgba(0, 255, 0, ${0.4 * multiplier})`,;`        ctx.lineWidth = 2,;
    ...
    const drawSpaceElements = ("ctx": CanvasRenderingContext2D, "canvas": HTMLCanvasElement, "multiplier": number) => {;
      ...
      for (let i = 0, i < 50 * multiplier, i++) {;
        ...
        ctx.save(),;
        ctx.fillStyle = `rgba(255, 255, 255, ${brightness * multiplier})`,;`        ctx.beginPath(),;
...
key={i}
                ...
                style={{
                  ...
                  "left": `${(i + 1) * 12.5}%`,`                  "transform": 'translateX(-50%)''
                }}
                animate={{;
                  }
                  "opacity": [0.3, 0.8, 0.3]}}
