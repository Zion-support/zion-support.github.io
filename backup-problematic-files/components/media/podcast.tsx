import React, { useEffect, useState } from 'react',import Link from 'next/link',type EpisodeListItem = {"id": string,"title": string,"inviteeName": string,"createdAt": string,"summary": string,audio?: {mp3Url?: string,wavUrl?: string,mp4Url?: string;'
  }
    ...
    },load()}, []),if (loading) return <div>Loading episodes</div>,return (<div className="space-y-6">;"
        ...
        {episodes.map((ep) => (<li key={ep.id} className="border rounded p-4">;"
            <div className="flex items-center justify-between">;"
              ...
              <div className="flex gap-3">;"
                ...
                {ep.audio?.wavUrl && (<a href={ep.audio.wavUrl} className="px-3 py-2 bg-gray-700 text-white rounded" download>;"
                    WAV;
                  </a>;
                )}
                {ep.audio?.mp4Url && (<a href={ep.audio.mp4Url} className="px-3 py-2 bg-purple-700 text-white rounded" download>;"
                    MP4;
                  </a>;
                )}
              </div>;
