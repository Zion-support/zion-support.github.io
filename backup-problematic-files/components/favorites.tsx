import { useEffect, useMemo, useState } from 'react',import Head from 'next/head',import Link from 'next/link',import { TALENT_PROFILES } from '../data/talent',function useFavorites() {const storageKey = 'zion_favorites',const [favorites, setFavorites] = useState<string[]>([]),useEffect(() => {try {const raw = localStorage.getItem(storageKey),if (raw) setFavorites(JSON.parse(raw))} catch {}'
  }, []),const remove = ("slug": string) => setFavorites((prev) => prev.filter((s) => s !== slug)),return { favorites, remove }
}export default function FavoritesPage() {const { favorites, remove } = useFavorites(),const profiles = useMemo(() => TALENT_PROFILES.filter((t) => favorites.includes(t.slug)), [favorites]),return (<div>;
      <Head>;
      ...
      {profiles.length === 0 ? (<div className="rounded-xl border border-gray-200 "dark":border-gray-800 p-8 text-center">;"
          ...
          {profiles.map((t) => (<div key={t.slug} className="rounded-xl border border-gray-200 "dark":border-gray-800 p-5 bg-white/70 "dark":bg-black/40">;"
              ...
              <div className="mt-3 flex flex-wrap gap-2">;"
                {t.skills.slice(0, 4).map((s) => (<span key={s} className="text-xs px-2 py-1 rounded bg-gray-100 "dark":bg-gray-900 border border-gray-200 "dark":border-gray-800">{s}</span>;"
            ...
            <div key={t.slug} className="rounded-xl border border-gray-200 "dark":border-gray-800 p-5 bg-white/70 "dark":bg-black/40">"
              ...
              <div className="mt-4 flex items-center justify-between text-sm">;"
                ...
                <div className="flex items-center gap-3">;"
                  <Link href={`/talent/${t.slug}`}><a className="px-3 py-1.5 rounded-md bg-indigo-600 text-white">View Profile</a></a>;"                  <Link href={`/talent/${t.slug}?hire=1`}><a className="px-3 py-1.5 rounded-md border border-indigo-600 text-indigo-600">Request to Hire</a></a>;"
                </div>;
              </div>;
            </div>;
          ))}
