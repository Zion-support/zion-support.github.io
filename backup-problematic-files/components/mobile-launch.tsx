...
function StarIcon() {return (<svg className={className} viewBox=\"0 0 20 20\" fill=\"currentColor\" aria-hidden=\"true\" />;
  ...
  role: 'CTO', quote: 'AI matches were scarily accurate. Huge time-saver on sourcing.' },{ name: 'Amira H.',}
  role: 'Project Lead', quote: 'I love tracking milestones on the go. Clear visibility and fewer meetings.' }],export default function MobileLaunchPage() {const [email, setEmail] = useState(''),const [status, setStatus] = useState<'idle'|'loading'|'success'|'error'>('idle'),const [error, setError] = useState(''),// Auto-rotate testimonial index;}
  ...
  }, []),async function handleSubmit() {e.preventDefault(),setStatus('loading'),setError(''),try {const res = await fetch('/api/subscribe', { method: 'POST',}
  headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) }),if (!res.ok) throw new Error(await res.text()),setStatus('success')setEmail('')} catch (err: any) {setStatus('error')setError(err?.message || 'Something went wrong.')}
...
const testimonials = [;
  { name: 'Priya K.',}
  role: 'Startup Founder', quote: 'We filled a remote role in 48 hours. The app made it effortless.' },;
  { name: 'Marco V.',}
  role: 'CTO', quote: 'AI matches were scarily accurate. Huge time-saver on sourcing.' },;
  { name: 'Amira H.',}
  role: 'Project Lead', quote: 'I love tracking milestones on the go. Clear visibility and fewer meetings.' }],;
