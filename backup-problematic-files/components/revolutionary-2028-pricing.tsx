import React from 'react',import Head from 'next/head',import Link from 'next/link',import { motion } from 'framer-motion',import {CheckCircle, ArrowRight, Star, TrendingUp, Phone, Zap,DollarSign, Shield, Mail, MapPin, Rocket, Brain, Sparkles,Globe, Atom, Heart, Leaf, Car, GraduationCap, Scale,Building, Factory, Camera, Video, Music, Gamepad2,Crown, Award, Target, Users, Clock, Globe2;
  ...
  {name: 'Starter',price: '$299',period: '/month',,description: 'Perfect for small businesses and startups',features: [;
      ...
      'Custom AI developmentDedicated support teamWhite-label solutions24/7 phone supportCustom analyticsFull API accessCustom integrationsDedicated account manager','SLA guaranteesOn-premise deployment';
    ],popular: false,color: 'from-indigo-600 to-purple-600',cta: 'Contact Sales',href: '/contact';
  }
],const servicePricing = [;
  {category: ' 2028 Futuristic Innovations',services: futuristic2028Services.slice(0, 6)},{category: ' Smart City & Infrastructure',services: emergingTech2028Services.filter(s => s.category.includes('Smart City') || s.category.includes('Infrastructure')).slice(0, 4)},{category: ' Quantum & Advanced Tech',services: futuristic2028Services.filter(s => s.category.includes('Quantum')).slice(0, 4)},{category: ' AI & Machine Learning',services: emergingTech2028Services.filter(s => s.category.includes('AI')).slice(0, 4)}
