...
const allServices = $2;
  ...
  const categories = [
    { id: 'all', name: 'All Services', icon: <Target className = $2;
    { id: 'business-intelligence', name: 'Business Intelligence', icon: <TrendingUp className = $2;
    { id: 'ai-automation', name: 'AI Automation', icon: <Brain className = $2;
    { id: 'it-infrastructure', name: 'IT Infrastructure', icon: <Building className = $2;
    { id: 'micro-saas', name: 'Micro SAAS', icon: <Zap className = $2;
    { id: 'ai-services', name: 'AI Services', icon: <Cpu className = $2;
  const filteredServices = activeTab === 'all' 
    ? allServices 
    : allServices.filter(service => {
        if (activeTab === 'business-intelligence') return service.category?.includes('Business Intelligence') || service.category?.includes($2);
        if (activeTab === 'ai-automation') return service.category?.includes($2);
        if (activeTab === 'it-infrastructure') return service.category?.includes($2);
        if (activeTab === 'micro-saas') return service.category?.includes($2);
        if (activeTab === 'ai-services') return service.category?.includes($2);
        return true
      }),
