...
const getServicePricing = (if (service.pricing?.starter) return service.pricing.starter,if (service.price?.monthly) return `$${service.price.monthly) => {
  return $3;}
}/month`,if (typeof service.price === 'string') return service.price,return 'Contact for pricing';
},// Helper function to get service features;
const getServiceFeatures = (return service.keyFeatures || service.features || [];) => {
  return $3;}
}
},// Helper function to get service setup time;
const getServiceSetupTime = (if (service.setupTime) return service.setupTime,if (service.pricing?.setupTime) return service.pricing.setupTime,if (service.price?.setupTime) return service.price.setupTime,return 'N/A';) => {
  return $3;}
}
},// Helper function to get service trial days;
const getServiceTrialDays = (if (service.trialDays) return service.trialDays,if (service.pricing?.trialDays) return service.pricing.trialDays,if (service.price?.trialDays) return service.price.trialDays,return 'N/A';) => {
  return $3;}
}
...
const allServices = [;
  ...advancedAIMLServices,...advancedCybersecurityServices,...advancedCloudDevOpsServices2025,...industrySpecificSolutions,...emergingTechnologyServices;
    ...
    { id: 'all',}
  name: 'All Services', icon: <Target className=\"w-5 h-5\" /> },{ id: 'business-intelligence',}
  ...
  name: 'AI Services', icon: <Cpu className=\"w-5 h-5\" /> }
