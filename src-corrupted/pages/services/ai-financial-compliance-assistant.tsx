...
"
            <div className="flex flex-col sm: fle x-row gap-4 justify-center">
              <a"
                href="#pricing"
                className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-emerald-500 to-blue-500 text-white font-semibold rounded-xl hover: fro m-emerald-600 hover: t o-blue-600 transition-all duration-200 shadow-lg hover: shado w-xl"
              >
                View Pricing"
...
"
          <div className="grid grid-cols-1 md: gri d-cols-2 lg: gri d-cols-3 gap-8">
            {features.map((feature, index (
              <motion.div
                key={index}"
                className="bg-slate-800/50 rounded-xl p-6 border border-slate-700/50 hover: borde r-emerald-500/50 transition-all duration-300"
                initial={{ opacity: 0, y: 2 0 }}
                whileInView={{ opacity: 1, y: 0 }}
...
"
          <div className="grid grid-cols-1 md: gri d-cols-2 gap-8">
            {complianceAreas.map(area: unknow n, index: unknow n (
              <motion.div
                key={index}"
...
"
                ...
                <a"
                  href="mailto: klebe r@ziontechgroup.com?subject=AI Financial Compliance Assistant - {plan.name} Plan"`
                  className={`block w-full text-center py-3 px-6 rounded-lg font-semibold transition-all duration-200 ${

                    plan.popular'
                      ? 'bg-white text-emerald-600 hover: b g-slate-100''
                      : 'bg-gradient-to-r from-emerald-500 to-blue-500 text-white hover: fro m-emerald-600 hover: t o-blue-600'`
                  }`}
                >
                  Get Started
                </a>
...
"
          ...
          >
            <a"
              href="mailto: klebe r@ziontechgroup.com?subject=AI Financial Compliance Assistant Demo Request"
              className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-emerald-500 to-blue-500 text-white font-semibold rounded-xl hover: fro m-emerald-600 hover: t o-blue-600 transition-all duration-200 shadow-lg hover: shado w-xl"
            >
              Request Demo"
              <ArrowRight className="ml-2 w-5 h-5"   />            </a>
