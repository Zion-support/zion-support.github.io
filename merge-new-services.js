#!/usr/bin/env node; /**
 * Merge New Services Branches; * This script will merge all the new service branches that haven't been merged yet; */ import { execSync } from 'child_process'; import fs from 'fs'; // //Step 1: Ensure we're on main and up to date; // try {,
...
//Step,
  2: Get new service branches; // const newServiceBranches = [
  ...
  'cursor/add-new-services-and-deploy-updates-6 ed4']; // //Step 3: Enhanced merge function with conflict resolution; function mergeServiceBranch(branchName) {,
//   try {,
    //Fetch the branch; execSync(`git fetch origin ${branchName}`, { stdio: 'inherit' }); //Try direct merge first; execSync(`git merge origin/${branchName} --no-ff -m "Merge ${branchName} - Add new services and deploy updates"`)
      { stdio: 'inherit' }
...
// //Step,
  3: Enhanced merge function with conflict resolution; function mergeServiceBranch(branchName) {/* TODO: Fix JSX expression */}
    execSync(`git fetch origin ${branchName}`, {/* TODO: Fix JSX expression */})
  o: 'inherit' }); //Try direct merge first;`
...
//         //Abort and skip; try {
          ...
          //Continue anyway; //         return {/* TODO: Fix JSX expression */}
  d: 'ours' }
...
//Step 4: Process all new service branches; // const results = {
  summary: {
    total: 0; successful: 0; failed: 0; methods: { direct: 0, theirs: 0, ours: 0, failed: 0 },
//Step,
  4: Process all new service branches; // const results = {/* TODO: Fix JSX expression */}
  d: 0 }},
  branche,
  s: [],
  faile,
...
//Step 5: Generate report; fs.writeFileSync('new-services-merge-report.json'),
...
//Step,
  5: Generate report; fs.writeFileSync('new-services-merge-report.json')
  JSON.stringify(results, null, 2)
); //Step,
