...
async function main() {
  try {
    ...
    for (const branch of prBranches) {
      ...
      if (branchExists !== null) {
        ...
        if (mergeResult !== null) {
          ...
          console.log(
            `⚠️  Failed to merge ${branch}, trying to resolve conflicts...`
          );
          ...
          if (status && status.includes('UU')) {
            ...
            const unmergedFiles = runGitCommand(
              'git diff --name-only --diff-filter=U',
              'Getting unmerged files'
            );
            ...
            if (unmergedFiles) {
              ...
              runGitCommand(
                `git commit -m "Resolve conflicts and complete merge of ${branch}"`,
                'Completing merge'
              );
    ...
    for (const branch of prBranches) {
      runGitCommand(
        `git branch -D ${branch}`,
        `Cleaning up local branch ${branch}`
      );
    }
