#!/usr/bin/env node;
const fs = require('fs')const path  = require('path')function fixJSXQuotes() {try {let content = fs.readFileSync(filePath, 'utf8')let modified  = false;// Fix unescaped single quotes in JSX text content;
    // This regex looks for single quotes that are not already escaped and are in JSX text content;
    const singleQuoteRegex = /([^\\])'([^']*?)'/g;
    const newContent = content.replace(singleQuoteRegex, (match, before, text) => {// Skip if it's already escaped or in a string literal;
      ...
      modified = true;
      return `${before}'${text}'`;
    })if (modified) {fs.writeFileSync(filePath, newContent, 'utf8')console.log(`Fixed JSX quotes in: ${filePath}`)return true;
    }return false;
...
for (const file of tsxFiles) {if (fixJSXQuotes(file)) {fixedCount++;
  }
