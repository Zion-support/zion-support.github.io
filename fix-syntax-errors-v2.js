#!/usr/bin/env node;
import fs from 'fs';'import path from 'path';'import { execSync } from 'child_process''// Function to fix specific syntax errors in a file
...
const fixes = [
      ...
      // Fix malformed JSX attributes
      {
        ...
        pattern: /(\w+)="([^"]*)"\s*(\w+)/g,""        replacement: '$1="$2" $3''"'      },"      // Fix missing semicolons in exports;
      {
  
        pattern: /export\s+const\s+(\w+)\s*=\s*{\s*(\w+):\s*'([^']*)',?\s*}\s*(\w+):/g,''        replacement: 'export const $1 = {\n  $2: \'$3\',\n  $4:'''      },
