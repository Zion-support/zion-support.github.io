...
function fixMalformedJSX(content) {
  ...
  // Fix className="$1" patterns
  content = content.replace(/className="\$1"/g, 'className=""');
  
  // Fix duplicate return statements
  content = content.replace(/return \(\s*<div>\s*\/\* content \*\/\}\s*return \(/g, 'return (');
  
  // Fix malformed JSX closing tags
  content = content.replace(/<div>\s*\/\* content \*\/\}/g, '<div>');
  
  return content;
}

// Function to fix missing commas in objects
function fixMissingCommas(content) {
  // Fix twitter object missing comma
  content = content.replace(/twitter:\s*\n\s*card:/g, 'twitter: {\n    card:');
