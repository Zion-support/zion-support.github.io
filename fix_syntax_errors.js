...
let content = fs.readFileSync(filePath, 'utf8');';'const originalContent = content
    ...
    content = content.replace(/(\s+const\s+\w+\s*=\s*\[[\s\S]*?\])\s*$/gm, '$1;')''    // 2. Fix missing commas in arrays (look for patterns like } followed by {)
    content = content.replace(/}\s*\n\s*{/g, '},\n    {')''    // 3. Fix missing semicolons after const declarations
    content = content.replace(/(\s+const\s+\w+\s*=\s*[^;]+)\s*$/gm, (match) => {
  
      if (!match.trim().endsWith(') && !match.trim().endsWith('}') && !match.trim().endsWith(')')) {''        return match + '''      }
      return match
    })
    // 4. Fix malformed JSX fragments - ensure proper opening/closing
    content = content.replace(/<React\.Fragment>\s*$/gm, '<>')''    content = content.replace(/<\/React\.Fragment>\s*$/gm, '</>')''    // 5. Fix missing closing brackets for arrays
    content = content.replace(/(\s+const\s+\w+\s*=\s*\[[\s\S]*?)(\s*)(\n\s*const|\n\s*return|\n\s*function|\n\s*export)/gm, (match, arrayPart, spaces, nextPart) => {
  
      if (!arrayPart.trim().endsWith(']')) {''        return arrayPart + ']' + spaces + nextPart''      }
      return match
    })
    // 6. Fix missing commas in object arrays
    content = content.replace(/}\s*\n\s*]\s*$/gm, '}\n  ]')''    // 7. Fix missing semicolons after array declarations that end with ]
    content = content.replace(/(\s+const\s+\w+\s*=\s*\[[\s\S]*?\])\s*(\n\s*const|\n\s*return|\n\s*function|\n\s*export)/gm, '$1;\n$2')''    // 8. Fix malformed JSX - ensure proper structure
...
const tsFiles = getAllTsFiles('./app')''console.log(`Found ${tsFiles.length} TypeScript files to check`);
let fixedCount = 0;
let errorCount = 0
