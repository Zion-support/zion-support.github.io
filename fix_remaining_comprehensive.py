...
def fix_jsx_file(file_path):
    ...
    try:
        ...
        if ('return (\n    <>\n      <Helmet>' in content and 
            '    </>\n  );' in content and
            'Expected corresponding closing tag for JSX fragment' not in content and
            'Parsing error' not in content):
            return False
        
        # Extract the function name
        func_match = re.search(r'export default function (\w+)', content)
        if not func_match:
            # Try to find function name in other patterns
            func_match = re.search(r'const (\w+): React\.FC', content)
            if not func_match:
                return False
        ...
        title = func_name.replace('Page', '').replace('Component', '')
