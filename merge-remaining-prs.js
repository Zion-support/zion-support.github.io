...
for (const pr of prs) {
  ...
  try {
    ...
    try {
      execSync(`git merge ${branchName} --no-ff -m "Merge PR #${prNumber}: ${title}"`, { stdio: 'inherit' });
      ...
      try {
        ...
        // Commit the merge
        execSync(`git commit -m "Resolve merge conflicts for PR #${prNumber}: ${title}"`, { stdio: 'inherit' });
