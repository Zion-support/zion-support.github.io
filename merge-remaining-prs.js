...
for (const pr of prs) {
  ...
  try {
    ...
    try {
      execSync(
        `git merge ${branchName} --no-ff -m "Merge PR #${prNumber}: ${title}"`,
        { stdio: 'inherit' },
      );
      ...
      console.log(
        `❌ Merge conflict in PR #${prNumber}. Resolving conflicts...`,
      );
      ...
      try {
        ...
        // Commit the merge
        execSync(
          `git commit -m "Resolve merge conflicts for PR #${prNumber}: ${title}"`,
          { stdio: 'inherit' },
        );
        ...
        console.log(
          `❌ Could not resolve conflicts for PR #${prNumber}. Manual intervention needed.`,
        );
