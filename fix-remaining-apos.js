#!/usr/bin/env node;
const fs = require('fs')const path  = require('path')function fixRemainingApos() {try {let content = fs.readFileSync(filePath, 'utf8')let modified  = false;// Fix any remaining ' in import statements;
    if (content.includes(''')) {content = content.replace(/'/g, "'")modified = true;
    }if (modified) {fs.writeFileSync(filePath, content, 'utf8')console.log(`Fixed remaining ' in: ${filePath}`)return true;
    }return false;
...
for (const file of tsxFiles) {if (fixRemainingApos(file)) {fixedCount++;
  }
}console.log(`Fixed remaining ' in ${fixedCount} files`)