...
,
  ...
  try {
    ...
    logger.info(`Notification sent successfully.`)} catch (error) {
  ...
  if (result.latencyMs > ALERT_THRESHOLD_MS) {
    ...
    if (serviceName) {
      try {
        ...
        logger.info(`Service restart process initiated for ${serviceName} due to high latency on ${result.name}.`)} catch (restartError) {
