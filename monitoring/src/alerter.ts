...
${messageSuffix}`, // Use the dynamic message suffix
  ...
  try {
    ...
    logger.info(`Webhook notification sent successfully for ${result.name} (Service: ${result.serviceName || 'N/A'}).`); catch (error) {
    ...
    if (currentCount >= CONSECUTIVE_CHECKS_LIMIT) {
      ...
      if (serviceName) {
        ...
        exec(command, (error, stdout, stderr) => {
          ...
          logger.info(`Remediation script for ${serviceName} ('${result.name}') executed. Output: ${stdout}`);); else {
      ...
      alertConsecutiveCounts.set(result.name, 0); else {
