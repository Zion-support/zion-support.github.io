Zion Tech Group — Services Catalog
=================================

This repository includes a structured catalog of additional services for https://ziontechgroup.com. These entries are additive and do not modify existing products.

Where the data lives
--------------------
- Structured YAML: `content/services/services.yaml`

Contact
-------
- Mobile: +1 302 464 0950
- E-mail: kleber@ziontechgroup.com
- Address: 364 E Main St STE 1008 Middletown DE 19709

Notes
-----
- Links use canonical paths under `https://ziontechgroup.com/services/*`. Wire them to actual routes/templates in your web app.

