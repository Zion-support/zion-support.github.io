...
async function main() {
  try {
    ...
    runCommand('git fetch --all', 'Fetching all branches');
    ...
    for (let i = 0; i < branches.length; i++) {
      ...
      const mergedOutput = runCommand('git branch --merged main', 'Checking merged branches');
      ...
      if (successful % 25 === 0 && successful > 0) {
        ...
        runCommand('git push origin main', 'Pushing progress to main');
      }
    }

    // Final push
    console.log('📋 Step 4: Pushing final changes...');
    runCommand('git push origin main', 'Pushing final changes to main');
    ...
    // Show recent commits
    console.log('\n📝 Recent commits:');
    runCommand('git log --oneline -10', 'Showing recent commits');
