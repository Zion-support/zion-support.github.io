...
function fixFile(filePath) {
  try {
    const config = fiveGPageConfigs[filePath];
    if (config) {
      const content = fiveGPageTemplate(config.title, config.description, config.features);
