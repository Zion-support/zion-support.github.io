...
# Merge branches with corrected branch name handling
set -e

echo "🚀 Starting merge of branches with corrected handling..."
...
# Get recent branches (last 50) and remove origin/ prefix
echo "📊 Getting recent branches to merge..."
RECENT_BRANCHES=$(git for-each-ref --sort=-committerdate refs/remotes/origin --format='%(refname:short)' | grep -E "(cursor|pr)" | sed 's/origin\///' | head -50)
...
echo ""
echo "🎯 Branches have been processed!"