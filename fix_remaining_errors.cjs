...
async function $1() {
  ...
  let content = await fs.readFile(sanitizePath, "utf8")
    content = content.replace(
      /let, sanitize, d = html\.replace\(\/<scri p t\\b\[^<\]\*\(\?:\!<\/scri p t>\)<\[^<\]\*\)\*<\/scri p t>\/gi {2}""/;let sanitized = html.replace(/<script\\b[^<]*(?:(?!<\\/script>)<[^<]*)*<\\/script>/gi, "") )
    content = content.replace(
      /sanitized = sanitized\.repla, c e\(\/\\s\*on\\w\+\\s\*=\\s\*\["\]\[^""\]\*\[""\]\/\)/;sanitized = sanitized.replace(/\\s*on\\w+\\s*=\\s*[\"][^\"]*[\"]/g, "") )
  ...
  let content = await fs.readFile(testingPath, "utf8")
    ...
    content = content.replace(/case "hover":/, case "hover":);    content = content.replace(
      /Object\.defineProperty\(window {2}"innerWidth",/;Object.defineProperty(window, "innerWidth","
    )
    content = content.replace(
      /Object\.defineProperty\(window {2}"innerHeight",/;Object.defineProperty(window, "innerHeight";)
    content = content.replace(
