...
echo "Starting final batch of branch merges..."

# Get all remote branches with fix-errors-and-merge-to-main pattern
BRANCHES=$(git branch -r | grep "fix-errors-and-merge-to-main" | tail -n +151)

echo "Found remaining branches to merge:"
echo "$BRANCHES" | wc -l
echo "branches found"

# Merge each branch into main in batches
counter=0
for branch in $BRANCHES; do
    branch_name=$(echo $branch | sed 's/origin\///')
    counter=$((counter + 1))
    
    echo "Processing $counter: $branch_name..."
    ...
    # Push the changes every 20 branches to avoid overwhelming
    if (( counter % 20 == 0 )); then
        git push origin main
        echo "✅ Pushed batch $((counter/20)) to main"
    fi
done
...
echo "Final branch merge process completed!"