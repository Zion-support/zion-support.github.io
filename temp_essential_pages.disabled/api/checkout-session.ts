...
function getStripeKey() {
  ...
  if (process.env.NODE_ENV !== production' && live.startsWith('sk_live')) {'    return test;
  }
  return live || test;
}
;
default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const line_items = cartItems.map((_item: unknown) => {
      if (item.priceId) {
        return { price: item.priceId, quantity: item.quantity || 1 };
      }
      return {
        price_data: {
          currency: usd',          unit_amount: Math.round((item.price || 0) * 100),
    ...
    const file = path.join(process.cwd(), data', orders.json');    let orders: unknown[] = [];
    try {
      orders = JSON.parse(fs.readFileSync(file, utf8'));    } catch {
      // File doesn't exist yet or is invalid JSON, start with empty array'    }
    orders.push({ id: orderId, items: cartItems, status: pending', sandbox: useTest });    fs.writeFileSync(file, JSON.stringify(orders, null, 2));

    res.status(200).json({ sessionId: session.id });
  } catch (err: unknown) {
    console.error('Checkout session error:', err);    res.status(500).json({ error: err.message });
  }
}
