...
// Generate import statements for all missing pages;
  ...
  return `import ${componentName} from './app/${page}/page'`}).join('\n');
;
// Generate route statements;
    word.charAt(0).toUpperCase() + word.slice(1);
  ).join('') + 'Page';`'
  return `                  <Routepath = "/${page}", element = {<${componentName} />} />`}).join('\n');
;
...
//Generate route statements
  ...
  return`<Routepath = "/${page}", element = {<${componentName} />} />` }).join("\n")
//Add imports after the last import const lastImportIndex = appContent.lastIndexOf("import")
const lastImportLineEnd = appContent.indexOf("\n", lastImportIndex) + 1
const beforeImports = appContent.substring(0, lastImportLineEnd)
const afterImports = appContent.substring(lastImportLineEnd)
;
...
//Combine everything
const newAppContent = newImports+afterImports.replace(appContent.substring(lastImportLineEnd, routeInsertionPoint), newRoutes.substring(lastImportLineEnd, routeInsertionPoint))
//Write the updated App.tsx fs.writeFileSync("/workspace/App.tsx", newAppContent)

console.log(`Added${brokenLinks.length} routes to App.tsx`) console.log("Routes added for: """,brokenLinks.slice(0, 10).join(", "), "...and more")
