...
import { Switch  } from '@/components / ui / switch';
import { format  } from 'date - fns';
      company_name: initialData?.company_name || "",role_title: initialData?.role_title || "",start_date: initialData?.start_date ? new Date(initialData.start_date) : new Date(),end_date: initialData?.end_date ? new Date(initialData.end_date) : undefined,is_current: initialData?.is_current || false,return (<>;
      <Form {...form}>;
        <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6">;
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">;
            <FormField;
                    ...
                    <PopoverTrigger as_child>;
                      <FormControl>;
                        <Button;
                          variant={'outline'}
                          className={cn ('w - full pl - 3 text - left font - normal',!field.value && 'text - muted - foreground')}                        >;
                          {field.value ? (format (field.value, 'MMM yyyy')) : (<span > Select date</span>)}
                          <CalendarIcon;
                            className='ml - auto h - 4 w - 4 opacity - 50';
                            aria - hidden='true';/>;
                        </Button>;
                      </FormControl>;
                    </PopoverTrigger>;<CalendarIcon className="ml-auto h-4 w-4 opacity-50" aria-hidden="true" />;
                        </Button>;
                      </FormControl>;
                    </PopoverTrigger>;
            ...
            {!watchIsCurrent && (<FormField;
                ...
                render={({ field }: { field: any }) => (<FormItem className='flex flex - col'>                    <FormLabel > End Date</FormLabel>;
                    <Popover>;
                      ...
                      </PopoverTrigger>;selected={field && field.value || undefined}
                          onSelect={field && field.onChange}initialFocus;
                          captionLayout='dropdown-buttons';
                          fromYear={1990}
                          toYear={new Date().getFullYear()}
...
}</div> <FormField <FormLabel>Description</FormLabel> <div className="flex gap-2" > <AIEnhancementButton > AI Writer </Button> </div> </div> <FormControl> <Textareaplaceholder="Describe your responsibilities, achievements, and skills used in this role..." className="min-h-[150px]" {...field;
