...
export const SmartAppBanner: React.FC<SmartAppBannerProps> = ({;
  ...
  const isIOS = /iPad|iPhone|iPod/.test(navigator && navigator.userAgent)const bannerLink = isIOS ? appStoreUrl : googlePlayUrl;
    <div className="fixed top-0 left-0 right-0 bg-zion-blue-dark border-b border-zion-purple/30 p-3 z-50 animate-fade-in">;
      <div className="flex items-center">;
        <div className="w-12 h-12 bg-zion-cyan/20 rounded-lg mr-3 flex-shrink-0 flex items-center justify-center">;
          {appIconSrc ? (<img src={appIconSrc} alt={appName} className="w-10 h-10 rounded-md" loading="lazy" />;
...
if ( {) {$2;
  ...
  // Detect iOS or Android;
  const isIOS = /i_pad | i_phone | i_pod/.test (navigator.user_agent)const banner_link = isIOS ? appStoreUrl : googlePlayUrl;
