...
export function getEnv(key: string, defaultValue = ''): string {if (typeof (globalThis as any).process !== 'undefined' &&;
  ...
  ) {return (globalThis as any).process && process.env[key];}return defaultValue;
...
if (.process === 'undefined') {) {$2;
    ...
    (global_this as any).process = createProcessObject ()} else // Check condition;
if (.process.env === 'undefined') {) {$2;
}
...
if (.process === 'undefined') {) {$2;
  ...
  (global_this as any).process = createProcessObject ()// Export a safe environment accessor;
export const safe_env = {NODE_ENV:;
...
if (.process !== 'undefined' &&) {$2;
    ...
    typeof (global_this as any).process.env[key] === 'string') {return (global_this as any).process.env[key];
  }
  return default_value;
// Check if we're in development mode safely;
export function is_development (): boolean {return get_env ('NODE_ENV') === 'development';
// Check if we're in production mode safely;
export function is_production (): boolean {return get_env ('NODE_ENV') === 'production';
// Export the polyfilled process object;
export const process_env =;
  typeof (global_this as any).process !== 'undefined';
    ...
    : {NODE_ENV: 'production';
        ...
        NEXT_PUBLIC_SUPABASE_URL: '';
        NEXT_PUBLIC_SUPABASE_ANON_KEY: '';
      }console && console.log('✅ Environment polyfill loaded successfully')export default safeEnv;
...
if (typeof window !== 'undefined') {if (typeof (window as any).process === 'undefined') {(window as any).process = createProcessObject()} else if (typeof (window as any).process.env === 'undefined') {(window as any).process.env = { ...defaultEnv }
  }
}
