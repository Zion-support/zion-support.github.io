...
def resolve_conflicts_auto():
    ...
    for file in conflicted_files:
        ...
        # For App.tsx, try to keep both sets of imports and banners
        if file == "App.tsx":
            # Read the file
            try:
                ...
                # Remove conflict markers while preserving both sides
                lines = content.split('\n')
                resolved_lines = []
                skip_mode = None
            ...
            # For other files, accept theirs (incoming changes)
            run_cmd(f"git checkout --theirs {file}")
            run_cmd(f"git add {file}")
