...
function fixFile(filePath) {
  ...
  let content = fs.readFileSync(filePath, "utf8")
    ...
    content = content.replace(
      /\(key: keyof (\w+), value: \)/g;"(key: keyof $1, value: any)"
    )
    content = content.replace(
      /\(key: keyof (\w+), value: \)/g;"(key: keyof $1, value: any)"
    )
  ...
  /g, "$1: {")
    ...
    content = content.replace(
      /:\s*{(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*}/g;": { $1, $2, $3, $4, $5 }"
    )
    content = content.replace(
      /:\s*{(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*}/g;": { $1, $2, $3, $4 }"
    )
    content = content.replace(
      /:\s*{(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*}/g;": { $1, $2, $3 }"
    )
  ...
  /g;"const $1: $2 = {"
    ...
    content = content.replace(
      /const\s+(\w+)\s*:\s*(\w+)\s*=\s*{/g;"const $1: $2 = {"
    )
