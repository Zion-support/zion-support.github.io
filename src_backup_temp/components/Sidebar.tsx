...
import {Home,Zap,Brain,Shield,Users,HardDrive,TrendingUp,Building2,FileText,HelpCircle,BarChart3,Settings,ChevronRight,ChevronDown,Cloud,Target,Heart,DollarSign,Briefcase,Newspaper,Mail,Phone,Search,Star,Layers,Globe,Cpu,Database,Lock,Smartphone,Monitor,Lightbulb,Rocket;
  ...
  onClose?: () => void;
"}export function Sidebar() {const [expandedSections", setExpandedSections] = useState<string[]>(['Services', 'Solutions'])const location  = useLocation()const toggleSection = ("sectionName": "string) => {setExpandedSections(prev => { return  ; }"
      }
      prev.includes(sectionName)? prev.filter(name => { return name !== sectionName): [...prev", sectionName]; }"
...
export function Sidebar() {const [expandedSections, setExpandedSections] = useState<string[]>(['Services', 'Solutions'])const location = useLocation()const toggleSection = ("sectionName": string) => {ChevronRight,ChevronDown;'
} from 'lucide-react';'
interface SidebarProps  {isOpen?: boolean;
  }
  onClose?: () => void;
}
export default function Sidebar() {const [expandedSections, setExpandedSections] = useState<string[]>(['Services'])const location = useLocation()const toggleSection = ("props": any) => {setExpandedSections(prev =>;'
      }
      prev.includes(sectionName)? prev.filter(name => { return name !== sectionName): [...prev, sectionName]; }
  ...
  const renderSidebarItem = ("item": any, level = 0) => {const isExpanded = expandedSections.includes(item.name)const hasActiveChild = item.children?.some(("child": any) => isActive(child.href))const isItemActive = item.href ? isActive(item.href) : false;
    }
    if (item.type === 'link') {return (<Link;'
          }
          key={item.name}
          to={item.href}
          className={`flex items-center space-x-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isItemActive;`              ? 'bg-blue-600 text-white';'
    ...
    if (item.type === 'section') {return (<div key={item.name} className="space-y-1">;"
          <button;
            ...
            className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors ${hasActiveChild;`                ? 'bg-blue-600 text-white';'
                : 'text-gray-300 "hover":bg-gray-700 "hover":text-white';'
            }`}`          >;
    ...
    } "lg":translate-x-0 "lg":static "lg":inset-0`}>;`      <div className="flex h-full flex-col">;"
        {/* Header */}
...
export default function Sidebar() {';';'
  ...
  const [expandedSections, setExpandedSections] = useState<string[]>(['Services'])const location = useLocation()const toggleSection = ("props": any) => {setExpandedSections(prev =>;'
      }
      prev.includes(sectionName)? prev.filter(name => { return name !== sectionName): [...prev, sectionName]; }
    )}const isActive = ("path": string) => location.pathname === path;
  const sidebarItems = [';'
    {';';'
      ...
      "icon": Hom e,';';'
  const sidebarItems = [;
    {"name": 'Home',"href": '/',"icon": Hom e,"type": 'link';'
    },{"name": 'Services',"type": 'section',"icon": Za p,"items": [;'
    ...
    },{"name": 'Solutions',"type": 'section',"icon": Brai n,"items": [;'
    ...
    },{"name": 'Company',"type": 'section',"icon": Building 2,"items": [;'
        { "name": 'About Us', "href": '/about', "icon": User s },{ "name": 'Leadership', "href": '/about#team', "icon": User s },{ "name": 'Careers', "href": '/careers', "icon": User s },{ "name": 'Partners', "href": '/partners', "icon": User s },{ "name": 'Press', "href": '/press', "icon": FileTex t }'
      ];
    },{"name": 'Resources',"type": 'section',"icon": FileTex t,"items": [;'
        { "name": 'Blog', "href": '/blog', "icon": FileTex t },{ "name": 'Case Studies', "href": '/case-studies', "icon": FileTex t },{ "name": 'White Papers', "href": '/white-papers', "icon": FileTex t },{ "name": 'Webinars', "href": '/webinars', "icon": FileTex t },{ "name": 'Documentation', "href": '/docs', "icon": FileTex t },{ "name": 'API Reference', "href": '/api', "icon": FileTex t }'
      ];
    },{"name": 'Support',"type": 'section',"icon": HelpCircl e,"items": [;'
        { "name": 'Help Center', "href": '/help', "icon": HelpCircl e },{ "name": 'Contact Support', "href": '/contact', "icon": User s },{ "name": 'Training', "href": '/training', "icon": FileTex t },{ "name": 'Community', "href": '/community', "icon": User s },{ "name": 'System Status', "href": '/status', "icon": BarChart 3 }'
      ];
    },{"name": 'Pricing',"href": '/pricing',"icon": BarChart 3,"type": 'link';'
    },{"name": 'Contact',"href": '/contact',"icon": User s,"type": 'link';'
    }
  ];
  const renderSidebarItem = ("props": any) => {if (item.type === 'link') {return (<Link;'
          }
          key={item.name}
          to={item.href}
...
type SidebarProps = {"isOpen": boolean;
  ...
  const quickLinks = [;
    { "name": 'AI Services', "href": '/services/ai-services', "icon": Brain },{ "name": 'IT Services', "href": '/services/it-services', "icon": Zap },{ "name": 'Micro SaaS', "href": '/services/micro-saas', "icon": Code },{ "name": 'Cybersecurity', "href": '/services/it/cybersecurity', "icon": Shield },{ "name": 'About Us', "href": '/about', "icon": Users },{ "name": 'Case Studies', "href": '/case-studies', "icon": FileText }'
  ];
  const contactInfo = [;
