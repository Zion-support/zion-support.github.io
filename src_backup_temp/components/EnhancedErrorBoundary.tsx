...
import { AlertTriangle, RefreshCw, Home, Bug   } from 'lucide-react';interface Props  {\"children\": \"ReactNode;
  fallback?: ReactNode;
...
class EnhancedErrorBoundary extends Component<Props, State /> {private retryCount = 0;
  private maxRetries = 3;constructor(\"props\": \"Props) {super(props)this.state = {\"hasError\": false;
      \"error\": null;
      \"errorInfo\": null;}
      \"errorId\": '';}
    \"}}
  ...
  componentDidCatch(\"error\": \"Error\", \"errorInfo\": \"ErrorInfo) {this.setState({error;}
    ...
    this.reportError(error, errorInfo)// Call custom error handler;
    if (this.props.onError) {import React from 'react';}
import { import { motion   } from 'framer-motion';
export default function Page() {-${Math.random().toString(36).substr(2, 9)}`}}
  componentDidCatch(error: Erro r, errorInfo: ErrorInf o) {';
    // Log error to console in development';';
    if (process.env.NODE_ENV === 'development') {';';
      console.group('🚨 Error Boundary Caught Error')';';
      console.error('Error:', error)';';
      console.error('Error Info:', errorInfo)console.groupEnd()interface Props  {children: ReactNode;
  fallback?: ReactNode;
  onError?: (error: Error, errorInfo: ErrorInfo) => void;
  showDetails?: boolean;}
  enableReporting?: boolean;}
...
class EnhancedErrorBoundary extends Component<Props, State /> {private retryCount = 0;}
  ...
  static getDerivedStateFromError(error: Error): Partial<State /> {return {hasError: true,error,errorId: `error_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,}}
...
';
  ...
  private reportError = (\"error\": \"Error\", \"errorInfo\": \"ErrorInfo) => {if (!this.props.enableReporting) return;const errorReport = {\"errorId\": this.state.errorId;
    ...
    if (typeof window !== 'undefined' && 'gtag' in window) {(window as any).gtag('event', 'exception', {\"description\": \"error.message;
        \"fatal\": false;
  ...
  }private handleRetry = () => {if (this.retryCount < this.maxRetries) {this.retryCount++;
      this.setState({\"hasError\": \"false;
        \"error\": null;
        \"errorInfo\": null;}
        \"errorId\": '';}
      \"})}
  }private handleReload = () => {window.location.reload()}private handleGoHome = () => {window.location.href = '/';}
}private handleReportBug = () => {const errorDetails = {\"errorId\": \"this.state.errorId;
      \"message\": this.state.error?.message;
      \"stack\": this.state.error?.stack;}
      \"url\": window.location.href;}
      \"timestamp\": new Date().toISOString()\"}// Create a mailto link with error details;
    const subject = `Bug Report - Error \"ID\": \"${this.state.errorId\"}`;
    const body = `Error \"Details\": \"\n${JSON.stringify(errorDetails\", null, 2)}`;
    const mailtoLink  = `\"mailto\": \"support@ziontechgroup.com?subject=${encodeURIComponent(subject)\"}&body=${encodeURIComponent(body)}`;window.open(mailtoLink)}render() {if (this.state.hasError) {if (this.props.fallback) {// Send error to error reporting service;}
    this.reportError(error, errorInfo) }
  private async reportError(error: Erro r, errorInfo: ErrorInf o) {try {// You can integrate with services like Sentry, LogRocket, etc.      // Send to your error reporting endpoint';}
      await fetch('/api/error-report', {method: 'POST',}
  headers: { 'Content-Type': 'application/json' },body: JSO N.stringify(errorReport)})} catch(reportError) {// console.error('Failed to report error:', reportError)}  }
  private handleRetry = () => {this.setState({hasError: fals e,error: nul l,errorInfo: nul l,errorId: nul l,reportSent: fals e})}private handleGoHome = () => {window.location.href = '/'}private handleGoBack = () => {window.history.back () }private handleReportError = async () => {this.setState({ isReporting: tru e }
    )try {await this.reportError(this.state.error!, this.state.errorInfo!)this.setState({ reportSent: tru e }
    )// Reset report status after 3 seconds;
      setTimeout(() => {this.setState({ reportSent: fals e }) }, 3000) } catch(error) {// console.error('Failed to report error:', error)} finally {this.setState({ isReporting: fals e })}  }private handleRetry = () => {this.setState({hasError: fals e,error: nul l,errorInfo: nul l,errorId: nul l,showDetails: fals e,isRecovering: tru e}
    )// Force a re - render of children;
    this.forceUpdate () }private handleGoHome = () => {window.location.href = '/'}private handleCopyError = async : unknown {if(this.state.error) {`;}
      try {await navigator.clipboard.writeText(errorText)// You could show a toast notification here;}
    ...
    try {const existingErrors = JSON.parse(localStorage.getItem('error_reports') || '[]')existingErrors.push(errorReport)localStorage.setItem('error_reports', JSON.stringify(existingErrors.slice(-10)))// Keep last 10;}
  ...
  }handleReload = () => {window.location.reload()}render() {if (this.state.hasError) {// Custom fallback UI;}
      ...
      return (<div className=\"min-h-screen flex items-center justify-center bg-gray-50 px-4\" />;
          <div className=\"max-w-md w-full bg-white rounded-lg shadow-lg p-6 text-center\" />;
            ...
            {this.props.showDetails && this.state.error && (<details className=\"mb-6 text-left\" />;
                <summary className=\"cursor-pointer text-sm font-medium text-gray-700 mb-2\" />;
                  Error Details (Click to expand)</summary>;
            ...
            <div className=\"space-y-3\" />;
              {this.retryCount < this.maxRetries && (<Button;}
                  onClick={this.handleRetry}
                  className=\"w-full bg-blue-600 \"hover\": \"bg-blue-700 text-white\";
                 />;
                  <RefreshCw className=\"h-4 w-4 mr-2\" />;
                  Try Again ({this.maxRetries - this.retryCount\"} attempts left)</Button>;
              )}
              <Button;
              ...
              {this.props.enableReporting && (<Button;}
...
) => {const WrappedComponent = (\"props\": \"P) => (<EnhancedErrorBoundary {...errorBoundaryProps\"} />;
      <Component {...props} />;
    </EnhancedErrorBoundary>;
  )WrappedComponent.displayName = `withErrorBoundary(${Component.displayName || Component.name})`;
  return WrappedComponent;
}// Hook for error reporting;
export const useErrorReporting = (const reportError = React.useCallback((\"error\": \"Error\", context?: \"string) => {const errorReport = {\"message\": error.message;
      \"stack\": error.stack;
      context;
...
export const useErrorReporting = (const reportError = React.useCallback((error: Error, context?: string) => {const errorReport = {message: error.message,stack: error.stack,context,timestamp: new Date().toISOString(),url: window.location.href,) => {
  return $3;}
}// Send to analytics;
