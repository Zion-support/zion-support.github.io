...
';'
  ...
  private reportError = ("error": "Error", "errorInfo": "ErrorInfo) => {if (!this.props.enableReporting) return;const errorReport = {"errorId": this.state.errorId;"
    ...
    const subject = `Bug Report - Error "ID": "${this.state.errorId"}`;`    const body = `Error "Details": "\n${JSON.stringify(errorDetails", null, 2)}`;`    const mailtoLink  = `"mailto": "support@ziontechgroup.com?subject=${encodeURIComponent(subject)"}&body=${encodeURIComponent(body)}`;window.open(mailtoLink)}render() {if (this.state.hasError) {if (this.props.fallback) {// Send error to error reporting service;`    }
