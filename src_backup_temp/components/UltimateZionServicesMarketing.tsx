...
"}}
          ...
          >;
            {categories.map((category) => (<button     key={category.id}
                ...
                className={`px-6 py-3 rounded-xl border transition - all duration - 300 ${selectedCategory === category.id';';
                    ? 'bg-gradient - to - r from - purple - 600 to - pink - 600 border-purple - 400 text-white shadow-lg'';';                onClick={ () => setSelectedCategory(category.id) }
                className={`px-6 py-3 rounded-xl border transition - all duration - 300 ${selectedCategory === category.id;
 ...
 {/* Category Overview */} {selectedCategory !== 'all' && (;";
        <section className="py-12 px-4 "sm": "p x-6 "lg": p x-8">;";              return (<motion.div
...
"}}
            transition={{ "duration": "0.6 "}}
          >'`;
            {selectedCategory === 'all' ? 'All Revolutionary Services' : `${categories.find(c => c.id === selectedCategory)?.name} Services`}                  </div>"
                  ...
                  <div className="space-y-1">;
                    {service.benefits.slice(0, 3).map((benefit, idx) => (";
                      <div key={idx} className="flex items-center text-sm text-gray-300">";
                        <span className="text-green-400 mr-2">✓</span>;                {service.marketSize && ("
                  <div className="mb-6">"
