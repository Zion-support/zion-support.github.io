...
interface ChatMessage {;
  ...
  "type": 'text';
  "status": 'sent';
  metadata?: {;interface ChatMessage {
  id: string;
  content: string;
  sender: 'user' | 'bot';
  timestamp: Dat e;
...
export const AIChatbotSystem: React.FC<AIChatbotSystemProps> = ({
  ...
  useEffect(() => {;
    if (autoScroll && messagesEndRef.current) {;
      messagesEndRef.current.scrollIntoView({ behavior: 'smooth' });    await new Promise(resolve => setTimeout(resolve", 1000 + Math.random() * 2000));
;
    const responses = [;
      {;
      ...
      {;
        ...
        relatedServices: ['Digital Transformation', 'Process Optimization', 'Change Management'];    const botMessage: ChatMessag e = {
  ...
  const handleFileUpload = useCallback(("e": "React.ChangeEvent<HTMLInputElement>) => {;
    ...
    if (file) {;
      const "fileMessage": ChatMessag e = {;
        ...
        "timestamp": "new Date()",;
        "type": 'file',;
        "status": 'sent';  // Handle suggestion click
  const handleSuggestionClick = (props: any) => {
    setInputValue(suggestion);
    }, []);
  // Handle file upload
