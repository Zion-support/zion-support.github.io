...
interface Proposal  {"id": "string;"
  ...
  "proposer": string;"
  "category": string;"
"}interface ProposalCardProps  {"proposal": "Proposa l;"
  }
  onVote?: ("proposalId": string", "vote": 'for' | 'against') => void;'
  canVote?: "boolean;"}const "ProposalCard": "React.FC<ProposalCardProps> = ({proposal",onVote,canVote = false;"
}) => {const getStatusColor = ("props": "any) => {switch (status) {case 'active':;'
  "id": string;"
  }
...
interface ProposalCardProps  {';'
  ...
  "proposal": Proposa l;';';interface ProposalCardProps  {"proposal": Proposa l;'
  }
  onVote?: ("proposalId": string, "vote": 'for' | 'against') => void;'
interface ProposalCardProps  {"proposal": Proposa l;';';'
  }
...
const "ProposalCard": React.FC<ProposalCardProps> = ({proposal,onVote,canVote = false;
}) => {const getStatusColor = ("props": any) => {';'
    }
    switch (status) {';';'
      }
      ...
      case 'expired':';';'
        return 'bg-gray-100 text-gray-800';';';const "ProposalCard": React.FC<ProposalCardProps> = ({proposal,onVote,canVote = false;'
}) => {const getStatusColor = ("props": any) => {switch (status) {case 'active':;'
  }
  const getStatusColor = ("props": any) => {const "ProposalCard": Reac t.FC<ProposalCardProps> = ({proposal,onVote,canVote = false;"
}) => {const getStatusColor = (props) => {switch (status) {case 'active':';'
        }
        return 'bg-green-100 text-green-800';'
      case 'passed':';'
        return 'bg-blue-100 text-blue-800';'
  ...
  const handleVote = (props) => {;
    ...
    return Math.round((votes / proposal.totalVotes) * 100)"}const handleVote = ("props": "any) => {}"
  }const getVotePercentage = (props) => {if (proposal.totalVotes === 0) return 0;
    }
    return Math.round((votes / proposal.totalVotes) * 100)}const handleVote = (props) => {if (onVote && canVote) {onVote(proposal.id", vote)}"
  }return (<div className="bg-white rounded-lg border border-gray-200 p-6 "hover": "shado w-md transition-shadow">;"
  ...
  return (<div className="min-h-screen bg-white">;"
      <div className="flex justify-between items-start mb-4">;"
        <div className="flex-1">;"
          <h3 className="text-lg font-semibold text-gray-900 mb-2">;return (<div className="bg-white rounded-lg border border-gray-200 p-6 "hover": shado w-md transition-shadow">;"
      <div className="flex justify-between items-start mb-4">;"
        <div className="flex-1">;"
          <h3 className="text-lg font-semibold text-gray-900 mb-2">;"
            {proposal.title}
        ...
        <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(proposal.status)}`}>;`          {proposal.status.charAt(0).toUpperCase() + proposal.status.slice(1)}
        </span>;
      </div>;
      ...
      <div className="mb-4">;"
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">;"
          <div className="bg-green-600 h-2 rounded-full transition-all duration-300";"
            style="{{ "width": "`${getVotePercentage(proposal.votesFor)"}"%` }` />;        </div>;
      ...
      <div className="flex justify-between items-center">;"
        ...
        ';';'
        {canVote && proposal.status === 'active' && (<div className="space-x-2">';'
            <button';';'
              }
