...
const ProposalCard: React.FC<ProposalCardProps /> = ({proposal,onVote,canVote = false;}
  ...
  return $3;}
      ...
      \"default\": retur n 'bg-gray-100 text-gray-800';
    \"}
  }const getVotePercentage = (if (proposal.totalVotes === 0) return 0;) => {
  return $3;}
}
    return Math.round((votes / proposal.totalVotes) * 100)\"}const handleVote = () => {
  return $3;}
}
  }const getVotePercentage = (if (proposal.totalVotes === 0) return 0;) => {
  return $3;}
}
    return Math.round((votes / proposal.totalVotes) * 100)}const handleVote = (if (onVote && canVote) {onVote(proposal.id\", vote)) => {
  return $3;}
}
  ...
  return (<div className=\"min-h-screen bg-white\" />;
      ...
      <div className=\"flex justify-between items-center\" />;
        ...
        ';';
        {canVote && proposal.status === 'active' && (<divclassName=\"space-x-2\" />;}
            <button';';}
              onClick={() = /> handleVote('for')}
              className=\"px-4 py-2 bg-green-600 text-white text-sm rounded-lg \"hover\": b g-green-700 transition-colors\";
            >;
