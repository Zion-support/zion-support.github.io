...
';';
  ...
  estimatedImpact: 5;}
    ...
    setIsAnalyzing(true)return analysis?.issues.filter(issue => issue.category === selectedCategory) || []}, [analysis, selectedCategory])// Filter suggestions by priority;
  const filteredSuggestions = useMemo(() => {return analysis?.suggestions.sort((a, b) => {const priorityOrder = {\"high\": \"3\", \"medium\": \"2\",// Mock SEO analysis data(in real app, this would come from actual analysis)const mockAnalysis: SEOAnalysi s = useMemo(() => ({score: 8 7,issues[ {id: '1',
  type: 'warning',title: 'Missing Meta Description',
  description: 'The page is missing a meta description tag, which is important for search engine snippets.',impact: 'medium',}
  fixable: tru e,category: 'content';}
      },{id: '2',
  type: 'error',title: 'Slow Page Load Time',
  description: 'Page load time is above the recommended 3-second threshold.',impact: 'high',}
  fixable: tru e,category: 'performance';}
      },{id: '3',
  type: 'info',title: 'Missing Alt Text',
  description: 'Some images are missing alt text, which affects accessibility.',impact: 'low',
  fixable: tru e,category: 'accessibility';
    ],suggestions[ {id: '1',
  title: 'Optimize Images',description: 'Compress and optimize images to improve page load speed.',
...
\";
  ...
  if(!analysis && !isAnalyzing) {return ()<div className=\"text-center py-8\" />\";
        <Search className=\"w-12 h-12 text-zion-slate/40 mx-auto mb-4\"   />\"        <p className=\"text-zion-slate/60\" />No SEO analysis available</p>;}
        <button;}
          onClick={analyzeSEO}\";
          className=\"mt-4 px-6 py-2 bg-zion-cyan hover: b g-zion-cyan/80 text-white rounded-lg transition-colors\";
