...
import { import { motion, AnimatePresence    } from 'framer-motion';export default function Page() {interface SEOMetrics  {"pageSpeed": number;"
...
"}interface SEOOptimizerProps extends React.PropsWithChildren<{}> {url?: "string;"
  ...
  autoAnalyze?: boolean;
  showDetails?: boolean;
  onAnalysisComplete?: ("analysis": SEOAnalysi s) => void"}export const "SEOOptimizer": "React.FC<SEOOptimizerProps> = ({url","autoAnalyze": "tru e",export default function Page() {interface SEOMetrics  {"pageSpeed": number;"
  }
  "mobileFriendliness": number;
  "accessibility": number;
  "bestPractices": number;
...
";"
            <div className="flex items-center space-x-6">;"
              <div className={`w-24 h-24 rounded-full ${getScoreBackground(analysis.score)} flex items-center justify-center`}>`;`                <span className={`text-3xl font-bold ${getScoreColor(analysis.score)}`}>;`  if(!analysis && !isAnalyzing) {return ()<div className="text-center py-8">";"
        <Search className="w-12 h-12 text-zion-slate/40 mx-auto mb-4" />"        <p className="text-zion-slate/60">No SEO analysis available</p>;"
        <button;
          }
          onClick={analyzeSEO}";"
...
";"
            <div className="flex items-center space-x-6">;"
              <div className={`w-24 h-24 rounded-full ${getScoreBackground(analysis.score)} flex items-center justify-center`}>`;`                <span className={`text-3xl font-bold ${getScoreColor(analysis.score)}`}>;`                  {analysis.score}
                </span>;
              </div>;
";"
...
";"
          ...
          <div className="mb-8">";"
            ...
            <div className="grid grid-cols-3 gap-4">`;`              <div className={`p-4 rounded-lg border ${analysis.metrics.coreWebVitals.lcp <= 2.5 ? 'border-green-200 bg-green-50' : 'border-yellow-200 bg-yellow-50'`;`              }`}>";"
...
";"
              <div className="flex space-x-1">;"
                {['all',content',technical',performance',accessibility'].map((category) => (<button;'
                    }
                    key={category}';'
                    onClick={() => setSelectedCategory(category)}`';';'                    className={`px-3 py-1 text-xs rounded-lg transition-colors ${selectedCategory === category'';';'
                        ? 'bg-zion-cyan text-white''';';'
                        : 'bg-zion-slate/10 text-zion-slate "hover": b g-zion-slate/20'`;`                </span>;
              </div>;
";"
              <div className="flex-1">";"
                <div className="grid grid-cols-2 gap-4">";"
...
";"
              <div className="flex space-x-1">;"
                {['all',content',technical',performance',accessibility'].map((category) => (<button;'
                    ...
                    onClick={() => setSelectedCategory(category)}`;`                    className={`px-3 py-1 text-xs rounded-lg transition-colors ${selectedCategory === category';'
                        ? 'bg-zion-cyan text-white'';'
                        : 'bg-zion-slate/10 text-zion-slate "hover": b g-zion-slate/20'`;`                    }`}`                        : 'bg-zion-slate/10 text-zion-slate "hover": "b g-zion-slate/20'`;`                    "}`}onClick={() => setSelectedCategory(category)}`;`                    className={`px-3 py-1 text-xs rounded-lg transition-colors ${selectedCategory === category';'
                        ? 'bg-zion-cyan text-white'';'
                        : 'bg-zion-slate/10 text-zion-slate "hover": b g-zion-slate/20'`;`                    }`}`                    {category.charAt(0) .toUpperCase () + category.slice(1) }
                  </button>) ) }
              </div>;
            </div>;
";"
...
";"
                      <div className="flex-1">";"
                        <div className="flex items-center justify-between mb-2">";"
                          <h5 className="font-medium text-zion-slate-dark">{issue.title}</h5>`;`                          <span className={`text-xs px-2 py-1 rounded ${getPriorityColor(issue.impact)}`}>;`                            {issue.impact} impact;
                          </span>;
                        </div>";"
