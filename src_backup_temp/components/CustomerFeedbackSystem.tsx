...
';
    rating: 0,';';
    comment: '',';';
    category: 'overall' as Feedback['category']  const [stats, setStats] = useState<FeedbackStats>({;
    "totalFeedback": "0",;
    "averageRating": "0",;
    "positivePercentage": "0",;
    "responseRate": "0",;
  ...
  const [newFeedback, setNewFeedback] = useState<any>({;
    rating: 0,;
    comment: '',;
    category: 'overall' as Feedback['category'];    const sampleFeedback: Feedbac k[] = [{
        id: '1',
...
';';
    ...
    let filtered = feedback;
;
    if(selectedCategory !== 'all') {;    let filtered = feedback;
    if(selectedCategory !== 'all') {
...
"
            <div className="flex items-center justify-center gap-2 mb-2">"
              ...
              <div className="flex">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star}
                    className={`w-5 h-5 ${star <= stats.averageRating ? 'text-yellow-400 fill-current' : 'text-zinc-600'`;          <motion.div
            initial = {
  { opacity: 0,
  y: 2 0
}}
...
';';
  { height: 'auto',
  opacity: 1                        onClick={ () => setNewFeedback(prev => ({ ...prev,
  rating: sta r
}))}"
            ...
            <div className="flex items-center gap-2 mb-3"> {[1, 2, 3, 4, 5].map((star) => (;';
                <Star key={star}`';';
                  className={`w-5 h-5 ${                  key={star}`                  className={`w-5 h-5 ${'';';
                    star <= item.rating ? 'text-yellow-400 fill-current' : 'text-zinc-600'`            <div className="flex items-center gap-2 mb-3"> {[1, 2, 3, 4, 5].map((star) => (;
                <Star key={star}`
                  className={`w-5 h-5 ${                  key={star}`                  className={`w-5 h-5 ${'
