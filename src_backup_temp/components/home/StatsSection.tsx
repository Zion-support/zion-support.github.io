...
import { TrendingUp, Award, Users, Clock    } from 'lucide-react';interface Stat  {"number": "string;"
  "label": string;"
  "description": string;"
  "avatar": string;"
"icon": React.ComponentType<{ className?: string"}>;"
  "color": "string"}interface StatsSectionProps extends React.PropsWithChildren<{}> {"stats": "Sta t[]"}const "StatsSection": "React.FC<StatsSectionProps> = ({ stats "}) => {const ref = useRef(null)const isInView = useInView(ref, { "once": "tru e "})const [counts, setCounts]  = useState<any>({})useEffect(() => {if (isInView) {interface Stat  {"number": string;"
  }
      ...
      const targetNumber = parseInt(stat.number.replace(/[^0-9]/g, ''));
          ...
          const countTimer = setInterval(() => {current += increment;
            ...
            if (current >= targetNumber) {current = targetNumber;
              ...
              clearInterval(countTimer)}setCounts(prev => ({...prev,[stat.label]: "Math.floor(current)"}))setCounts(prev => ({...prev,[stat.label]: Math.floor(current)}))}, 50)}"
    )}, 500)return () => clearTimeout(timer)}
  }, [isInView, stats])return (<section className = "py-20 bg-gradient-to-br from-zion-slate-dark via-zion-slate-dark/80 to-zion-slate-dark" role="region" aria-labelledby="stats-heading">;"
      ...
      <div className="container mx-auto px-4 "sm": p x-6 "lg": p x-8">;"
        ...
        <div ref={ref} className="grid grid-cols-1 "md": gri d-cols-2 "lg": gri d-cols-4 gap-8">;"
          {stats.map((stat, index)  => (<motion.div;
              ...
              {/* Icon container */"}"
              <div className={`relative w-16 h-16 bg-gradient-to-br ${stat.color} rounded-2xl flex items-center justify-center mx-auto mb-4 group-"hover": "scal e-110 transition-transform duration-300 shadow-lg group-"hover": shado w-xl group-"hover": shado w-zion-cyan/30`"}>;"                <stat.icon className="w-8 h-8 text-white" aria-hidden="true" />;"
                {/* Glow effect */}
...
"}"
              ...
              <h3 className="relative text-lg font-semibold text-zion-cyan mb-2 group-"hover": "tex t-zion-blue transition-colors duration-300">;"
                {stat.label"}<p className="relative text-zion-slate-light text-sm leading-relaxed group-"hover": "tex t-zion-slate-light/90 transition-colors duration-300">;"
                {stat.description"}"
              </p>;
              {/* Hover indicator */}
