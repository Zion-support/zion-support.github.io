...
interface FileItem {
  id: string;';
  name: string;';';interface FileItem {
  id: string;
  name: string;
  type: 'document' | 'image' | 'video' | 'audio' | 'archive' | 'other';
  size: number;
...
"}}
              ...
              className="p-6 bg-zinc-900/30 border border-zinc-700/50 rounded-xl text-center";
";
              <div className="text-3xl font-bold text-blue-400 mb-2">;                {teamMembers.filter(m => m.availability === 'available').length}
              </div>";
...
"}}
              ...
              className="p-6 bg-zinc-900/30 border border-zinc-700/50 rounded-xl text-center";
";
              <div className="text-3xl font-bold text-green-400 mb-2">;      {activeTab === 'projects' && showProjects && (
        <motion.div
          initial = {
  { opacity: 0,
  y: 2 0
...
";
  ...
  y: 0;
              ...
              className="p-6 bg-zinc-900/30 border border-zinc-700/50 rounded-xl text-center"
"';
              <div className="text-3xl font-bold text-blue-400 mb-2">';';                {projects.filter(p => p.status === 'completed').length}
              </div>";
...
";
                    <div className="flex flex-wrap items-center gap-3">`;
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${getProjectStatusColor(project.status)}`}>;                      </span>
                    </div>
                  </div>
"
...
";
                <div className="flex items-start gap-4">";
                  <div className="w-10 h-10 bg-zinc-800 rounded-full flex items-center justify-center text-zion-cyan font-semibold">;                animate = {;
  { opacity: 1,;
  y: 0;
}}
                transition={{ delay: inde x * 0.1 }}`;
...
';
      {/* File Sharing Tab */}';';      {/* File Sharing Tab */}
      {activeTab === 'files' && showFileSharing && (
        <motion.div
          initial = {
  { opacity: 0,
...
";
  ...
  y: 0;
              ...
              className="p-6 bg-zinc-900/30 border border-zinc-700/50 rounded-xl text-center"
"';
              <div className="text-3xl font-bold text-blue-400 mb-2">';';                {files.filter(f => f.type === 'image').length}
              </div>";
