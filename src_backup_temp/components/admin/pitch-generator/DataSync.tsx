...
const DataSync: React.FC<DataSyncProps> = ({ syncedData, onNext, onBack }) => {              {syncedData?.marketplaceConversionFunnel?.visitors?.toLocaleString() || 'N/A'}
            </div>
            <div className="text-sm text-gray-600">Visitors</div>
          </div>
      ...
      <div className="flex justify-between">;
        <button;            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <span className="font-medium">{client.name}</span>
              <a
                href={client.caseStudyUrl}
                className="text-blue-600 "hover": tex t-blue-800 text-sm"
