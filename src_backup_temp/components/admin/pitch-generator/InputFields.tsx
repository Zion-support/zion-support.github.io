...
interface InputFieldsProps  {\"inputData\": \"an y;}
  \"setInputData\": (\"data\": an y) => void;}
  \"onNext\": () => void;\"}const \"InputFields\": \"React.FC<InputFieldsProps /> = ({ inputData\", setInputData, onNext }) => {const handleInputChange = (setInputData({...inputData\",[field]: \"value;) => {
  return $3;}
}
    \"})}const handleSubmit = (\"inputData\": an y;
  setInputData: (data: an y) => void;
  onNext: () => void;) => {
  return $3;}
}
}
const InputFields: React.FC<InputFieldsProps /> = ({ inputData, setInputData, onNext }) => {const handleInputChange = (setInputData({...inputData,[field]: value;) => {
  return $3;}
}
    }
    )const \"InputFields\": React.FC<InputFieldsProps /> = ({ inputData, setInputData, onNext }) => {const handleInputChange = (setInputData({...inputData,[field]: value;) => {
  return $3;}
    ...
    })}const handleSubmit = (e.preventDefault()if (inputData?.companyName && inputData?.industry) {onNext()\") => {
  return $3;}
}
      ...
      <form onSubmit={handleSubmit} className=\"space-y-4\" />;
        ...
        <div />;
          ...
          <textarea';';
            value={inputData?.description || ''}';';return (<div className=\"max-w-2xl mx-auto p-6\" />;
      <h2 className=\"text-2xl font-bold mb-6\" />Company Information</h2>;
      <form onSubmit={handleSubmit} className=\"space-y-4\" />;
        <div />;
          <label className=\"block text-sm font-medium mb-2\" />Company Name</label>;
