...
export const getPricingByServiceId = ('serviceId': string): ExpandedServicePricing | (null) => {;'
  ...
  return ALL_EXPANDED_SERVICES_PRICING.find(pricing => { return pricing.serviceId === serviceId)}; }
export const getPricingByCategory = ('category': string): ExpandedServicePricing[] => {;'
  }
  return ALL_EXPANDED_SERVICES_PRICING.filter(pricing => { return pricing.category === category)}; }
export const getPricingBySubcategory = ('subcategory': string): ExpandedServicePricing[] => {;'
  }
  return ALL_EXPANDED_SERVICES_PRICING.filter(pricing => { return pricing.subcategory === subcategory)}; }
export const searchPricing = ('query': string): ExpandedServicePricing[] => {;'
  }
  const lowercaseQuery = query.toLowerCase();
  return ALL_EXPANDED_SERVICES_PRICING.filter(pricing => { return pricing.serviceName.toLowerCase().includes(lowercaseQuery) ||
