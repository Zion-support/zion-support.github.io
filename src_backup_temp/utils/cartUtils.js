 ...
 */;
export const validateCartItem = (item) => {if (!item || typeof item !== 'object') return false;
...
export const addToCart = (cart, newItem) => {if (!validateCartItem(newItem)) {console.error('Invalid cart item:', newItem)return cart;
  ...
  const existingItemIndex = cart.findIndex(item => item.id === newItem.id)if (existingItemIndex !== -1) {// Update existing item quantity;
    ...
    updatedCart[existingItemIndex] = {...updatedCart[existingItemIndex],quantity: updatedCart[existingItemIndex].quantity + newItem.quantity;
    }return updatedCart;
  } else {// Add new item;
 ...
 */;
export const mergeCarts = (cart1, cart2) => {if (!Array.isArray(cart1) || !Array.isArray(cart2)) {return Array.isArray(cart1) ? cart1 : (Array.isArray(cart2) ? cart2 : [])}
  const merged = [...cart1];
  cart2.forEach(item2 => {if (validateCartItem(item2)) {const existingIndex = merged.findIndex(item1 => item1.id === item2.id)if (existingIndex !== -1) {// Merge quantities;
        merged[existingIndex] = {...merged[existingIndex],quantity: merged[existingIndex].quantity + item2.quantity,updatedAt: new Date().toISOString()}} else {// Add new item;
