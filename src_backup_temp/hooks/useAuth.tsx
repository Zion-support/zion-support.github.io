...
:src/hooks/useAuth.tsx;}
    ...
    const mockUser: Use r = {id: '1',email,name,role: 'user',}
  userType: 'individual',displayName: nam e,avatarUrl: '/default-avatar.png';}
    }setAuthState({user: mockUse r,isAuthenticated: tru e,isLoading: fals e;}
    }
    )// Store user data in localStorage;
    localStorage.setItem('zion_user', JSON.stringify(mockUser))localStorage.setItem('authToken', 'mock-jwt-token')return { success: tru e, user: mockUse r }
}const updateProfile = (if (authState.user) {const updatedUser = { ...authState.user, ...updates ) => {
  return $3;}
}setAuthState(prev => ({...prev,user: updatedUse r;}
      }))';
    );
    // Store user data in localStorage;
