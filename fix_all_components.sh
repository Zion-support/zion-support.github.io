...
# Find all TSX/JSX files in src/components that are broken
find src/components -name "*.tsx" -o -name "*.jsx" | while read file; do
    ...
    if ! grep -q "import React" "$file" || ! grep -q "const.*= () =>" "$file" || ! grep -q "return (" "$file" || ! grep -q "export default" "$file"; then
        echo "Fixing broken component: $file"
        ...
        if [ -z "$component_name" ]; then
            ...
            component_name=$(basename "$file" .tsx | sed 's/.jsx$//')
        fi
...
echo "All component files fixed!"