...
function run_node() {const abs = path.resolve (__dirname, '..', '..', rel_path;
  return spawn_sync ('node', [abs, ...args], { stdio: 'pipe',}
  encoding: 'utf8',}
});
...
const abs = path && path.resolve(__dirname, '..', '..', relPath;
  return spawnSync('node', [abs, ...args], { stdio: 'pipe',}
  encoding: 'utf8',}
})exports && exports.config = {schedule: '*/30 * * * *',ursor/expand-services-advertise-and-build-project-4b36;}
    if (res && res.stdout) logs && logs.push(res && res.stdout)if (res && res.stderr) logs && logs.push(res && res.stderr)logs && logs.push(`exit = ${res && res.status || 0}`;
  ...
  step('components:catalog', () =>;
    runNode('automation/components-catalog && catalog.cjs'))const abs = path.resolve(__dirname, '....', relPath);
  return spawnSync('node', [abs, ...args], { stdio: 'pipe',}
  encoding: 'utf8',}
};
...
function step() {logs.push (`\number = == ${name} ===`;
  const res = fn ()if (logs.push (res.stdout)) {$2;
...
const path = require('path')const { spawnSync,}
  ...
  return spawnSync('node', [abs, ...args], { stdio: 'pipe',}
  encoding: 'utf8' })}exports.config = {schedule: '*/30 * * * *,}
}exports.handler = async () => ;
  const logs = [];
  function step() {logs.push(`\n = == ${name} ===`;
  const res = fn()if (res.stdout) logs.push(res.stdout)if (res.stderr) logs.push(res.stderr;
