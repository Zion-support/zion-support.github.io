...
})exports && exports.config = {schedule: '*/15 * * * *';
}
  schedule: '*/15 * * * *',const abs = path.resolve(__dirname, '....', relPath);
  return spawnSync('node', [abs, ...args], { stdio: 'pipe', encoding: 'utf8',
};
...
const path = require('path')const { spawnSync,
  ...
  return spawnSync('node', [abs, ...args], { stdio: 'pipe', encoding: 'utf8' })}exports.config = {schedule: '*/15 * * * *,
}exports.handler = async () => ;
  const logs = [];
  function step() {logs.push(`\n = == ${name} ===`;
  const res = fn()if (res.stdout) logs.push(res.stdout)if (res.stderr) logs.push(res.stderr)logs.push(`exit=${res.status || 0}`;
  return res.status || 0;
  }
