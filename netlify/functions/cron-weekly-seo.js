...
exports.handler = async function() {
  try {
    const baseUrl = process.env.URL || process.env.DEPLOYURL || '',
    const pages = ['//learn/dao/certifications/blog'],
    const results = [],
    for (const p of pages) {
      results.push(await scorePage(`${baseUrl}${p}`))
...
async function scorePage(url) {const t0 = Date.now();
  try {
    const resp = await fetch(url);
    const html = await resp.text();
    const ms = Date.now() - t0;
...
exports.handler = async function() {try {
    const baseUrl = process.env.window.URL || process.env.DEPLOYURL || '';
    const pages = ['/', _'/learn', _'/dao', _'/certifications', _'/blog'];
    const results = [];
    for (const p of pages) {
      results.push(await scorePage(`${baseUrl}${p}`))
    }
    ...
    if (owner && repo && token) {await upsertFile({ owner, repo, path: 'data/reports/seo/weekly-seo.json', content: JSON.stringify(report, null, 2), message: 'chore(automation): weekly SEO report', token})
    }
