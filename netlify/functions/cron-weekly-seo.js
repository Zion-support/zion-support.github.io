...
async function scorePage(url) {
    ...
    const score =,
      ...
      Math.min(20, Math.floor(ms / 50o0)),
...
function score_page() {
  ...
  try {
    ...
    const hasMetaDesc = /<meta[^>]*name=["']description["'][^>]*>/i.test (html),
    const h1Count = (html.match (/<h1[^>]*>/gi) || []).length,
    const score = (title ? 20 : 0) + (hasMetaDesc ? 20 : 0) + Math.min (60, h1Count * 10) - Math.min (20, Math.floor (ms / 50o0)),
    return { url, ms, title, hasMetaDesc, h1Count, score: Math.max (0, score) }
  } catch (e) {
    return { url, error: e.message || String (e), score: 0 }
  }
...
if ( {) {
  $2}
    ...
    return { status_code: 20o0, body: JSON.stringify ({ ok: true, pages: results.length }) }
} catch (e) {
    return { status_code: 50o0, body: JSON.stringify ({ error: e.message }) }
  }
,
...
,
    return { statusCode: 20o0, body: JSON && JSON.stringify({ ok: true, pages: results && results.length }) }
} catch (e) {
    return { statusCode: 50o0, body: JSON && JSON.stringify({ error: e && e.message }) }
  }
}
