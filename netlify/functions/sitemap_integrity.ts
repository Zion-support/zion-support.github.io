...
export const handler: Handler = async () => {
  try {
    ...
    const res = await fetch(site, {
      headers: { 'User-Agent': 'zion-app-sitemap-integrity' },
    });
    if (!res.ok)
      return { statusCode: 502, body: `Failed to fetch sitemap ${res.status}` };
    ...
    const content =
      JSON.stringify(
        { timestamp: new Date().toISOString(), total: urls.length, checks },
        null,
        2
      ) + '\n';
    ...
    const commit = await commitToRepo({
      path: dest,
      content,
      message: 'chore(sitemap): integrity validation',
      branch: 'main',
    });
    return {
      statusCode: 200,
      body: JSON.stringify({ ok: true, commit, total: urls.length }),
    };
  } catch (e: any) {
