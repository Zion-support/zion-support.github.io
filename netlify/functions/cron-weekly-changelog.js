...
async function fetchJson(url, token) {const resp = await fetch(url, _{
...
exports.handler = async function() {
  try {
    ...
    const summary = {updatedAt: Date.now(), since, totalCommits: commits.length, authors: Object.entries(byAuthor).map(_([name, count]) => ({ name, count})).sort(_(a, b) => b.count - a.count),
      messages
    },
