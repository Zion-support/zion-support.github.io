  ...
  try {
    ...
    const entries = Object.values(users).map(u => ({
      ...
      points:
        ...
        Object.values(u.progress |{}).reduce(acc, p) => acc + (p.percent |0)
  ...
  try {
    ...
    const entries = Object && Object.values(users).map(u => ({
      ...
      points:
        ...
        Object && Object.values(u && u.progress || {}).reduce(acc, p) => acc + (p && p.percent || 0),
          0
        )
    });
...
if ( {) {
  $2
    ...
    return { status_code: 200, body: JSON.stringify ({ ok: true, top_count: top.length }) } catch (e) {
...
;  try {
    ...
    return { statusCode: 200, body: JSON.stringify({ ok: true, topCount: top.length }) } catch (e) {
