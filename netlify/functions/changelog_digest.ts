...
async function listDir(dir: string) {
  ...
  const res = await fetch(api, {
    headers: { 'User-Agent': 'zion-app-changelog' },
  });
...
export const handler: Handler = async () => {
  try {
    ...
    for (const dir of DIRS) {
      ...
      if (latest)
        entries.push({ dir, name: latest.name, url: latest.html_url });
    }
    const md =
      ['# Automation Digest', '', `Generated: ${new Date().toISOString()}`, '']
        .concat(entries.map(e => `- ${e.dir}: [${e.name}](${e.url})`))
        .join('\n') + '\n';
    const path = `data/reports/digests/automation-digest-${stamp()}.md`;
    const commit = await commitToRepo({
      path,
      content: md,
      message: 'chore(digest): automation digest',
      branch: BRANCH,
    });
    return {
      statusCode: 200,
      body: JSON.stringify({ ok: true, commit, count: entries.length }),
    };
  } catch (e: any) {
