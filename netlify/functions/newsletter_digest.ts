...
async function fetchRSS() {
  const res = await fetch(`${BASE}/feed.xml`, {
    headers: { 'User-Agent': 'zion-app-newsletter' },
  });
  if (!res.ok) return '';
  return res.text();
}
...
export const handler: Handler = async () => {
  try {
    ...
    const md = [
      ...
      '',
      ...items.map(
        i => `- [${i.title}](${i.link})${i.date ? ` — ${i.date}` : ''}`
      ),
      '',
    ...
    const commit = await commitToRepo({
      path,
      content: md,
      message: 'chore(newsletter): weekly digest draft',
      branch: 'main',
    });
    return {
      statusCode: 200,
      body: JSON.stringify({ ok: true, commit, items: items.length }),
    };
  } catch (e: any) {
