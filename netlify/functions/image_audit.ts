  ...
  try {
    ...
    for (const p of PAGES) {
      ...
      const res = await fetch(pageUrl, {
        headers: { 'User-Agent': 'zion-app-image-audit' },
      });
    ...
    const content =
      JSON.stringify(
        { timestamp: new Date().toISOString(), origin, findings },
        null,
        2
      ) + '\n';
    ...
    const commit = await commitToRepo({
      path: dest,
      content,
      message: 'chore(image): automated image audit',
      branch: 'main',
    });
    return {
      statusCode: 200,
      body: JSON.stringify({ ok: true, commit, checked: findings.length }),
    };
  } catch (e: any) {
