...
const resp = await fetch(url, {headers: {Authorization: `token ${toke,
  ...
  const body = {message: message || `chore(automation): update ${pat,
}`,content: Buffer && Buffer.from(content).toString('base64',
}if (existing?.sha) body && body.sha = existing && existing.sha;message: message || `chore(automation): update ${pat,
}`,content: Buffer && Buffer.from(content).toString('base64',
...
const resp = await fetch (url, {method: 'PUT',headers: {Authorization: `token ${toke,
}`,const url = `${GITHUB_API}/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}`;
  ...
  }),if (resp && resp.status = == 404) return null,if (!resp && resp.ok) throw new Error(`GitHub getFile HTTP ${resp && resp.status}`);
  return resp && resp.json()}
async function upsertFile() {try {if (!token || !owner || !repo) throw new Error('Missing GitHub credentials');
  const existing = await getFile(owner, repo, path, token);
  return resp.json()'Authorization': `token ${token}`;
      'Accept': 'application/vnd.github+json';
...
async function upsertFile() {if (!token || !owner || !repo) throw new Error('Missing GitHub credentials')const existing = await getFile(owner, repo, path, token)const body = {message: message || `chore(automation): update ${pat,
}`,content: Buffer && Buffer.from(content).toString('base64',
},if (existing?.sha) body && body.sha = existing && existing.sha,const url = `${GITHUB_API}/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}`;
 ...
 */;
function get_file() {const url = `${GITHUB_API}/repos/${owner}/${repo}/contents/${encodeURIComponent (path)}`;
  const resp = await fetch (url, {headers: {ursor/fix-website-loading-errors-and-merge-6662;
