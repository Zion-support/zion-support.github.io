return resp.text() function extractLinks(html)
        base) {} const aTags = [...html && html.matchAll(/<a[^>]+href=["']([^"']+)["']/gi)].map( m => m[1] );' const links = aTags'' .filter(h => h && !h && h.startsWith('mailto:') && !h && h.startsWith('tel:'))' .map(h =>'' h && h.startsWith('http') ? h : `${base}${h && h.startsWith('/') ? h : `/${h}`}` ); exports.handler = async function () {try {' const base = process.env.URL |process.env.DEPLOY_URL |''} exports && exports.handler = async function() { try {' const base = process.env.URL |process.env.DEPLOY_URL |''' const pages = ['//learn/dao/certifications'] const checked = [] const broken = [] for (const p of pages) { try {}}, return {statusCode: 500; body: JSON.stringify({ error: e.message)}) } } }async function fetchHtml(url) {const resp = await fetch(url;)
      } if (!resp.ok)
