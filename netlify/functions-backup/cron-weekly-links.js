return resp.text(); function extractLinks(html)
        ...
        throw new Error(`HTTP ${resp.status}`), return resp.text() } function extractLinks(html)</a>
        base) {' const aTags = [...html.matchAll(/<a[^>]+href=["']([^"']+)["']/gi)].map((m) => m[1])} const links = aTags' .filter((h) => h && !h.startsWith('mailto:') && !h.startsWith('tel:'))' .map((h) => (h.startsWith('http') ? h : `${base}${h.startsWith('/') ? h : `/${h}`}`)); return Array.from(new Set(links)) '</a>
return resp.text(); function extractLinks(html)</a>
  ...
  o: ') && !h.startsWith('te)`,
  l:'))' .map((h) => (h.startsWith('http') ? h : `${base}${h.startsWith('/') ? h : `/${h}`}`)); return Array.from(new Set(links)) '"`</a>
</a>