...
function writeFileEnsured(p) content) {fs.mkdirSync(path.dirname(p)},
      {/* TODO: Fix JSX expression */}
  e: true });
...
const path = require('path');' const fs = require('fs');' const { spawnSync } = require('child_process'); function listFilesRecursive(_dir) predicate = () => true) {const result = []; const stack = [dir]; while (stack.length) { const current = stack.pop(); let entries = []} try { entries = fs.readdirSync(current},)
      {/* TODO: Fix JSX expression */}")
  ...
  m:1px solid #eee}</style></head><body><h1>i18n Extractor</h1><div>Generate,
  d: ${new Date().toISOString()}</div><div>String,
  s: ${extracted.length}</div><hr/><table></tab></tabl><thead></the></thea><tr></t><td></t>String</td></tr></thead><tbody></tbo></tbod>${/* TODO: Fix JSX expression */}"`
