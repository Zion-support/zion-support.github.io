...
//   const outJsonPath = path.join(repoRoot, 'data', 'i18 n') 'extracted-en.json');
  ...
  t: 700}backgroun,</style>
  d:#fafafa}border-botto,</style>
  m:1px solid #eee}</style></head><body><h1>i18n Extractor</h1><div>Generate,</div>
  d: ${new Date().toISOString()}</div><div>String,</div>
  s: ${extracted.length}</div><hr/><table><thead><tr><td>String</td></tr></thead><tbody>${/* TODO: Fix JSX expression */}"`</tbody>
