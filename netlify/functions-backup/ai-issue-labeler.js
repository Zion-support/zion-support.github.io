exports.handler = async function (event, context) {
  ...
  if (!githubToken) {
    return {
      statusCode: 200,
      body: JSON.stringify({ ok: false, error: 'GITHUB_TOKEN required' }),
    };
  }
  ...
  async function listRecentIssues() {
    const res = await fetch(
      `https://api.github.com/repos/${githubRepo}/issues?state=open&per_page=20`,
      { headers: ghHeaders }
    );
  ...
  async function addLabels(issueNumber, labels) {
    ...
    const res = await fetch(
      `https://api.github.com/repos/${githubRepo}/issues/${issueNumber}/labels`,
      {
        method: 'POST',
        headers: ghHeaders,
  ...
  try {
    ...
    return {
      ...
      body: JSON.stringify({
        ok: true,
        log,
        store: { ok: putRes.ok, status: putRes.status },
      }),
    };
  } catch (e) {
    return {
