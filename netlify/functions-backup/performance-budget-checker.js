...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || '';' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' function abs(_url) { if(!url)
        ...
        return null}' return baseUrl + (url.startsWith('/') ? url: '/' + url} } async function headOrGetContentLength(url) {try {

' const headRes = await fetch(url} { method: 'HEAD' });' const len = headRes.headers.get('content-length'); if(len)
        return Number(len)

} catch {} try {

' const res = await fetch(url} { method: 'GET' }); const buf = await res.arrayBuffer(); return buf.byteLength

} catch {return 0} } } async function commitJson(path)
        data) {' if(!githubToken)
        return { ok: false),
        reason: 'no_token' }; const headers = { Authorization: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'netlify-performance-budget-checker' }; let sha; try {
...
} catch {} const body = { message: `chore: update performance budget report (${new Date().toISOString()})`,' content: Buffer.from(JSON.stringify(data, null)
...
const htmlRes = await fetch(baseUrl); const html = await htmlRes.text();' const scriptSrcs = Array.from(html.matchAll(/<script[^>]*src=["']([^"']+)["'][^>]*>/gi)).map(m => m[1]).map(abs).filter(Boolean);' const cssHrefs = Array.from(html.matchAll(/<link[^>]*rel=["']stylesheet["'][^>]*href=["']([^"']+)["'][^>]*>/gi)).map(m => m[1]).map(abs).filter(Boolean;)
...
exports.handler = async function(event, context) {/* TODO: Fix JSX expression */}
  ...
  l: '/' + url} } async function headOrGetContentLength(url) {try {

' const headRes = await fetch(url} {/* TODO: Fix JSX expression */})
...
' const res = await fetch(url} {/* TODO: Fix JSX expression */})
  d: 'GET' }); const buf = await res.arrayBuffer(); return buf.byteLength

} catch {return 0} } } async function commitJson(path)
        data) {/* TODO: Fix JSX expression */}
  n: 'no_token' }; const headers = {/* TODO: Fix JSX expression */}`
  n: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'netlify-performance-budget-checker' }; let sha; try {
