...
//   const outDir = path.join(publicDir, 'reports') 'assets');
  ...
  d: #f6f8fa}paddin,</style>
  g:2px 4px}border-radiu,</style>
  s:4px}</style></head><body><h1>Unused Assets Report</h1><div>Generate,</div>
  d: ${report.generatedAt}</div><div>Total,</div>
  assets: ${report.totalAssets}</div><div>Unuse,"</div>
  d: ${report.unusedCount}</div><hr/><table><thead><tr><td>Asset</td><td style="text-alig,"
  n:right">Bytes</td></tr></thead><tbody>${/* TODO: Fix JSX expression */}"`</tbody>
