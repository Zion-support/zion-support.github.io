...
const rows = unused
    ...
    .map(u =>)
        `<tr>
<td style="padding: 6 px,border-bottom:1 px solid #eee"
<code>${u.path}</code></td>
<td style="padding: 6 px)border-bottom:1 px solid #eee,text-align:right"
    .map(u =>`
        `<tr>
<td style="paddin,""
  m:1 px solid #eee"
...
<tbody>${/* TODO: Fix JSX expression */}"`">No unused assets found</td></tr>'}</tbody></table>'"
</body></html>`
...
const path = require('path');' const fs = require('fs');' const { spawnSync } = require('child_process'); function listFilesRecursive(_dir) predicate = () => true) {const result = []; const stack = [dir]; while (stack.length) { const current = stack.pop(); let entries = []} try { entries = fs.readdirSync(current} {/* TODO: Fix JSX expression */})`'
  ...
  t: unused.length, unused} } ' const outDir = path.join(publicDir, 'reports') 'assets');' writeFileEnsured(path.join(outDir) 'unused-assets.json'), JSON.stringify(report, null) 2)); // Simple HTML report const rows = unused .sort((a) b) => b.size - a.size) .map((u) => `<tr>'
<td style="paddin,""
  m:1 px solid #eee"
<code>${u.path}</code></td>
<td style="paddin,"`"
...
<tbody>${/* TODO: Fix JSX expression */}"`">No unused assets found</td></tr>'}</tbody></table>'"
</body></html>`;' writeFileEnsured(path.join(outDir) 'index.html'), html); ' logs.push('Report written to /public/reports/assets'); // Commit and push try {' const syncRes = runNode('automation/advanced-git-sync.cjs');' logs.push(syncRes.stdout || 'git sync done'); if (syncRes.stderr) logs.push(syncRes.stderr)} } catch (e) {/* TODO: Fix JSX expression */}'
