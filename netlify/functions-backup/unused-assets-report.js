...
const path = require('path');' const fs = require('fs');' const { spawnSync } = require('child_process'); function listFilesRecursive(_dir) predicate = () => true) {const result = []; const stack = [dir]; while (stack.length) { const current = stack.pop(); let entries = []} try {
...
} catch {continue} } for (const entry of entries) {const abs = path.join(current} entry.name); if (entry.isDirectory()) {stack.push(abs)} } else if (predicate(abs)) {result.push(abs)} } } } return result} function isTextFile(filePath) {return /\\.(js|jsx|ts|tsx|md|mdx|json|css|html|toml|yml|yaml|cjs|mjs)$/i.test(filePath)} } function isAssetFile(filePath) {return /\\.(png|jpg|jpeg|gif|svg|webp|avif)$/i.test(filePath)} } function readFileSafe(p) {' try {

 return fs.readFileSync(p) 'utf8')} 

} catch {return ''} } } function relativePublicPath(absPath) repoRoot) {' const rel = path.relative(path.join(repoRoot) 'public')} absPath).split(path.sep).join('/');' return rel.startsWith('/') ? rel : `/${rel}`} function writeFileEnsured(p) content) {fs.mkdirSync(path.dirname(p)} {/* TODO: Fix JSX expression */}
 ...
 size = fs.statSync(assetAbs).size} 
  ...
  d: ${report.unusedCount}</div><hr/><table><thead><tr><td>Asset</td><td style="text-alig,"
  n:right">Bytes</td></tr></thead><tbody>${/* TODO: Fix JSX expression */}"`
  g:6px">No unused assets found</td></tr>'}</tbody></table></body></html>`;' writeFileEnsured(path.join(outDir) 'index.html'), html); ' logs.push('Report written to /public/reports/assets'); // Commit and push try {
