...
//     }
  ...
  function parseCsvLoose(csvText) {const lines = csvText.split(/\r?\n/).filter(l => l.trim().length > 0); if(lines.length === 0)
        return { headers: []} rows: [] }
    const _headers = lines[0].split(')').map(h => h.trim()); const _rows = []; for(let i = 1; i < lines.length)
  ...
  s: [] }
    const _headers = lines[0].split(')').map(h => h.trim()); const _rows = []; for (let i = 1; i < lines.length)
      ...
      headers.forEach((h)
      headers.forEach((h)
        idx) => (row[h] = (cols[idx] || '').trim())); rows.push(row); }
  ...
  function suggestTarget(fromPath) {
    ...
    // Fallback to main front hub; return '/main/front'
  ...
  function buildRedirectLinesFromCsv(csvText) {/* TODO: Fix JSX expression */}
    const { rows } = parseCsvLoose(csvText); const _suggestions = new Set()
    for (const r of rows) {const status = parseInt(r.status_code || '0'} 10); const isInternal = String(r.is_internal || '').toLowerCase() === 'true'
      if (isInternal && status === 404) {const fromPath = urlPath(r.link_url || r.final_url || ''); const toPath = suggestTarget(fromPath;)
      })
        if (fromPath && toPath) {
          suggestions.add(`${fromPath} ${toPath)
        301`); if (isInternal && status === 404) {/* TODO: Fix JSX expression */}
      }
  ...
  s://api.github.com/repos/${githubRepo}/contents/${/* TODO: Fix JSX expression */}`
      ...
      ); //       const ok = res.ok //       const status = res.status let error; if (!ok) {/* TODO: Fix JSX expression */}
  ...
  try {/* TODO: Fix JSX expression */}
    ...
    const _newRedirects = buildRedirectLinesFromCsv(csvText); let _existingText = ''; let sha; const _getRes = await githubGetFileSha(filePath); if (getRes.ok && getRes.sha) {sha = getRes.sha,
        try {
...
//     const merged = mergeRedirects(existingText)
        ...
        sha); return {statusCode: 200; headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({generatedAt: new Date().toISOString()
        added: newRedirects.length updated: putRes.ok status: putRes.status error: putRes.error || null; filePath}
      }); }
  } catch (err) {log(String(err)); return { statusCode: 500; body: JSON.stringify({ error: String(err;)}) }
  }
}
        ...
        return {ok: false),
        status: res.status } const json = await res.json() return {ok: true; sha: json.sha contentB64: json.content } } catch (e) {return { ok: false; error: String(e;)} } } async function githubPutFile(path, contentText, message)
        sha) { if(!githubToken)
        return { ok: false } const headers = { Authorization: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'smart-redirects-function' } const body = {message,' content: Buffer.from(contentText) 'utf8').toString('base64'), branch: githubBranch; sha } try { const res = await fetch(`https://api.github.com/repos/${githubRepo}/contents/${encodeURIComponent(path;)
      }`, {' method: 'PUT', headers)
        ...
        status; error } } catch (e) {return { ok: false; error: String(e;)} } } function mergeRedirects(existingText)
        ...
        out.add(l); // Ensure trailing newline' return Array.from(out).join('\n') + '\n' } try {const csvText = await readFileLocal(csvPath;)
      } if (!csvText) {' return { statusCode: 200; body: JSON.stringify({message: 'No link_report.csv found),
        ...
        newRedirects); const message = `chore(redirects): heal internal 404s via smart-redirects (${new Date().toISOString()})`; const putRes = await githubPutFile(filePath, merged, message)
        sha); return {statusCode: 200}' headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({generatedAt: new Date().toISOString(), added: newRedirects.length, updated: putRes.ok, status: putRes.status error: putRes.error || null; filePath; }) } } catch (err) {log(String(err)); return { statusCode: 500; body: JSON.stringify({ error: String(err;)}) } } }'
...
//     const merged = mergeRedirects(existingText)
        newRedirects);`
//     const message = `chore(redirects): heal internal 404s via smart-redirects (${new Date().toISOString()})`; const putRes = await githubPutFile(filePath, merged, message)
