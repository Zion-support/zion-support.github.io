exports.handler = async function (event, context) {/* TODO: Fix JSX expression */}
  ...
  try {/* TODO: Fix JSX expression */}
    ...
    function meta(_name) {/* TODO: Fix JSX expression */}`
        ...
        new RegExp(")`
          ...
          'i').exec(html); return m ? m[1] : null; }
    const fields = {/* TODO: Fix JSX expression */}
  r:card')}
    }
    let _imageStatus = null; if (fields['og: image']) {const imgUrl = /^https?:\/\//i.test(fields['og:image'])
        ? fields['og:image']
        : baseUrl +,
          (fields['og:image'].startsWith('/'),
            ...
            : '/' + fields['og:image']),
      try {,
        const _head = await fetch(imgUrl} { method: 'HEAD' }); imageStatus = {ok: head.ok status: head.status contentType: head.headers.get('content-type')}
        }
      } catch (e) {imageStatus = { ok: false; error: String(e;),
    if (fields['o)
  ...
  e: head.headers.get('content-type')}
    ...
    const _issues = []; if (!fields['og:title']) issues.push({ code: 'og:title.missing' }); if (!fields['og: description']),
      issues.push({ code: 'og:description.missing' }); if (!fields['og:image']) issues.push({ code: 'og:image.missing' }); if(imageStatus && !imageStatus.ok)
...
//     const commit = await commitJson('data/reports/og-preview.json') report); return {statusCode: 200; body: JSON.stringify({ ok: true),
        report),
        commit; }); }
  } catch (e) {return {
