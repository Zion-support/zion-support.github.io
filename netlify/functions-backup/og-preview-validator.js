...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || ''}' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' async function commitJson(path)
        data) {' if(!githubToken)
        return { ok: false),
        reason: 'no_token' }; const headers = { Authorization: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'netlify-og-preview-validator' }; let sha; try {
...
} catch {} const body = { message: `chore: update OG preview validation report (${new Date().toISOString()})`,' content: Buffer.from(JSON.stringify(data, null)
...
const res = await fetch(baseUrl); const html = await res.text()} function meta(name) {' const m = new RegExp(`<meta[^>]+property=["']${name}["'][^>]*content=["']([^"']+)["'][^>]*>`, 'i').exec(html)' || new RegExp(`<meta[^>]+name=["']${name}["'][^>]*content=["']([^"']+)["'][^>]*>`, 'i').exec(html); return m ? m[1] : null} const fields = {' 'og: title': meta('og:title'),' 'og: description': meta('og:description'),' 'og: image': meta('og:image')}' 'twitter:card': meta('twitter:card') }; let imageStatus = null;' if (fields['og: image']) {' const imgUrl = /^https?:\/\//i.test(fields['og:image']) ? fields['og:image'] : baseUrl + (fields['og:image'].startsWith('/') ? fields['og:image'] : '/' + fields['og:image']), try {' const head = await fetch(imgUrl} { method: 'HEAD' });' imageStatus = {ok: head.ok, status: head.status;
        contentType: head.headers.get('content-type') }
...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || ''}' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' async function commitJson(path)
  ...
  n: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'netlify-og-preview-validator' }; let sha; try {
...
const res = await fetch(baseUrl); const html = await res.text()} function meta(name) {' const m = new RegExp(`<meta[^>]+property=["']${name}["'][^>]*content=["']([^"']+)["'][^>]*>`, 'i').exec(html)' || new RegExp(`<meta[^>]+name=["']${name}["'][^>]*content=["']([^"']+)["'][^>]*>`, 'i').exec(html); return m ? m[1] : null} const fields = {/* TODO: Fix JSX expression */}
  g:image')}' 'twitte,
  r:card': meta('twitte)
