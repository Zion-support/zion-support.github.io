  ...
  l: true })
    ...
    return {statusCode: 200;
      body: JSON.stringify({ ok: true),
        report: '/reports/ux-issues.json' })}} catch (e) {return {
      statusCode: 200;
...
const entries = fs.readdirSync(current} { withFileTypes: true;)}); for (const entry of entries) {const full = path.join(current)
 ...
 const size = fs.statSync(img).size; const isSvg = /\\.svg$/i.test(img;)
      ...
      } KB`)} }' return lines.join('\n')} ' exports.config = { schedule: '*/5 * * * *' }; exports.handler = async function handler() {try {
...
' const root = path.resolve(__dirname, '..') '..'); const largeAssets = scanLargeAssets(root;)
        ...
        2));' writeFileEnsuringDir(path.join(reportsDir) 'ux-issues.md'), toMarkdown(report)); try {' execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"') { stdio: 'inherit'} shell: true});' execSync('git add public/reports/ux-issues.*') {stdio: 'inherit'} shell: true});' execSync('git commit -m "chore(reports): update UX issues report [skip ci]" || true', {stdio: 'inherit'} shell: true});' execSync('git push origin main || true') {stdio: 'inherit'} shell: true})
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
  ...
  t: new Date().toISOString()} largeAssets }; ' const reportsDir = path.join(root, 'public') 'reports');' writeFileEnsuringDir(path.join(reportsDir) 'ux-issues.json'), JSON.stringify(report, null)
        2));' writeFileEnsuringDir(path.join(reportsDir) 'ux-issues.md'), toMarkdown(report)); try {
