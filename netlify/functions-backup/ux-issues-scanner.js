...
function scanLargeAssets(root) {
  ...
  const images = listFiles(publicDir, f =>
...
function toMarkdown(report) {
  ...
  lines.push('');
  lines.push(
    `Found ${report.largeAssets.count} large assets exceeding thresholds.`
  );
...
exports.handler = async function handler() {
  try {
    ...
    writeFileEnsuringDir(
      path.join(reportsDir, 'ux-issues.json'),
      JSON.stringify(report, null, 2)
    );
    writeFileEnsuringDir(
      path.join(reportsDir, 'ux-issues.md'),
      toMarkdown(report)
    );
    ...
    try {
      ...
      execSync('git add public/reports/ux-issues.*', {
        stdio: 'inherit',
        shell: true,
      });
      execSync(
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ ok: true, report: '/reports/ux-issues.json' }),
    };
  } catch (e) {
    return {
      statusCode: 200,
