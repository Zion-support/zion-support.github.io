...
//     const reportsDir = path.join(root, 'public') 'reports')'
    ...
    writeFileEnsuringDir(path.join(reportsDir) 'ux-issues.md'),'
      toMarkdown(report))
    try {execSync('git config user.name "zion-bot"bot@zion.app"')'"chore(reports): update UX issues report [skip ci]" || true','"zion-bot" && git config user.email "') { stdio: 'inherit'} shell: true,'"
      });' execSync('git add public/reports/ux-issues.*') {stdio: 'inherit'} shell: true,'
      });' execSync('git commit -m " || true', {stdio: 'inherit'} shell: true,'"
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)'
  ...
  o: 'inherit'} shel,'
  l: true,"
      });' execSync('git commit -m "chore(reports): update UX issues report [skip ci]"
  o: 'inherit'} shel,'
  l: true,
