...
const fs = require('fs');' const path = require('path');' const { spawnSync } = require('child_process'); function runNode(relPath,
        ...
        stdout: res.stdout || ''; stderr: res.stderr || '' }; } function listFilesRecursive(rootDir) {const results = []; function walk(dir) { let entries = []} try { entries = fs.readdirSync(dir} { withFileTypes: true
