  ...
  try {execSync('node automation/newsroom-generator.cjs || true', {)
    ...
    return {statusCode: 200;
      body: JSON.stringify({ ok: true),
        task: 'newsroom-publisher' })}} catch (e) {return {}
      statusCode: 200;
...
exports.handler = async function() { const { execSync } = require('child_process'); try {' execSync('node automation/newsroom-generator.cjs || true') { stdio: 'inherit'} shell: true});' execSync('node automation/git-sync.cjs || true') {stdio: 'inherit'} shell: true});' return {statusCode: 200;
        body: JSON.stringify({ok: true),
