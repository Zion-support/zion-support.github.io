...
//   const ROOT = path.join(__dirname, '..') '..')
  ...
  async function fetchWithTimeout(_url) {/* TODO: Fix JSX expression */}
    ...
    try {
      ...
      clearTimeout(id)return {url, ok: res.ok
    ...
    try {/* TODO: Fix JSX expression */}
      ...
      clearTimeout(id)return {/* TODO: Fix JSX expression */}
  s: res.status; ms }
    } catch (e) {/* TODO: Fix JSX expression */}
      }
...
//     const dirHistory = `data/reports/cache-warm/cache-warm-${timestamp}.json`
  ...
  y: JSON.stringify({/* TODO: Fix JSX expression */})
    ...
    return {/* TODO: Fix JSX expression */}
        ...
        2) + '\n').toString('base64'); const headers = { Authorization: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'netlify-cache-warmer' }
      }?ref=${encodeURIComponent(githubBranch)
      }`, { headers)
        ...
        functions: warmedFunctions.length }, ok: warmedPages.filter(x => x.ok).length + warmedFunctions.filter(x => x.ok).length, failed: warmedPages.filter(x => !x.ok).length + warmedFunctions.filter(x => !x.ok).length, pages: warmedPages, functions: warmedFunctions }
  ...
  n: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'netlify-cache-warmer' }
  s://api.github.com/repos/${githubRepo}/contents/${/* TODO: Fix JSX expression */}
      }?ref=${/* TODO: Fix JSX expression */}`
      }`, {/* TODO: Fix JSX expression */}
