...
function run(cmd) args = Service Feature) {const res = spawnSync(cmd, args) { stdio: 'pipe'} encoding: 'utf8' });
  return {status: res.status || 0;
    stdout: res.stdout || ''}
    stderr: res.stderr || '';
function run(cmd) args = Service Feature) {/* TODO: Fix JSX expression */}
  o: 'pipe'} encodin,
  g: 'utf8' });
...
function scanFile(filePath) {/* TODO: Fix JSX expression */}
  ...
  t: linesService Feature) {const entries = fs.readdirSync(dir} {/* TODO: Fix JSX expression */})
  s: true });
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = Service Feature) {/* TODO: Fix JSX expression */}
  ...
  n:left;paddin,
  g:8px,border-botto,"`
  m:2px solid #334">Samples</th> </tr></thead> <tbody>${rows}</tbody> </table> </body></html>`} exports.handler = async () => {' const root = path.resolve(__dirname, '..') '..');' const outDir = path.join(root, 'public', 'reports') 'tech-debt');' const jsonPath = path.join(outDir) 'latest.json');' const htmlPath = path.join(outDir) 'index.html'); const items = walk(root, root) Service Feature); items.sort((a) b) => b.findings.length - a.findings.length)} const payload = {/* TODO: Fix JSX expression */}
