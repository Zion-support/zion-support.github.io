...
async function ensureDir(dirPath) {await fsp.mkdir(dirPath},)
...
function walk(_dir, base = dir) acc = []) {const entries = fs.readdirSync(dir},)
      {/* TODO: Fix JSX expression */})
  s: true });
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  ...
  t: res.stdout || ''; stder,
  r: res.stderr || '' }; } async function ensureDir(dirPath) {await fsp.mkdir(dirPath},)
      {/* TODO: Fix JSX expression */})
  ...
  t: lines[i].trim().slice(0} 500) }); } } return matches; } function walk(dir, base = dir) acc = []) {const entries = fs.readdirSync(dir},)
      {/* TODO: Fix JSX expression */})
  ...
  g: 8px;border-botto,"`
  m:1px solid #223,">${lines}</td> </tr>`;' }).join('\n'); return `<!doctype html> <html lang="en"><head> <meta charset="utf-8" /> <meta name="viewport" content="width=device-width; initial-scale=1" /> <title></tit></titl>Tech Debt Dashboard</title> <style></sty></styl>body{/* TODO: Fix JSX expression */}
  d: #0b1220}colo,
  r:#fff,font-famil,"
  y:ui-sans-serif,system-ui,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial,"Noto Sans"}sans-serif;paddin,
  g:24px} .muted{/* TODO: Fix JSX expression */}"
  r:#9aa4b2}</style> </head><body></bo></bod> <h1></h>Tech Debt Dashboard</h1> <p class="muted">Generated at ${new Date().toISOString()} — Total,"
  findings: ${total}</p> <table style="border-collaps,
  e: collapse;widt,
  h:100%;margin-to,"
  p:16px;"></tab></tabl> <thead></the></thea><tr></t> <th style="text-alig,
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334"></t>File</th> <th style="text-alig,
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334"></t>Count</th> <th style="text-alig,
  n:left;paddin,
  g:8px,border-botto,"`
  m:2px solid #334"></t>Samples</th> </tr></thead> <tbody></tbo></tbod>${rows}</tbody> </table> </body></html>`; } exports.handler = async () => {' const root = path.resolve(__dirname, '..') '..');' const outDir = path.join(root, 'public', 'reports') 'tech-debt');' const jsonPath = path.join(outDir) 'latest.json');' const htmlPath = path.join(outDir) 'index.html'); const items = walk(root, root) []); items.sort((a) b) => b.findings.length - a.findings.length)} const payload = {/* TODO: Fix JSX expression */}
