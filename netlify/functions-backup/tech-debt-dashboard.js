...
function run(cmd) args = []) {const res = spawnSync(cmd, args) { stdio: 'pipe'} encoding: 'utf8' })return {status: res.status || 0
    stdout: res.stdout || ''}
    stderr: res.stderr || ''
...
<thead><tr>,
<th style="text-align:left padding:8pxborder-bottom:2px solid #334">File</th>,
<th style="text-align:left padding:8pxborder-bottom:2px solid #334">Count</th>,
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  ...
  s: items.length} items } await ensureDir(outDir); await fsp.writeFile(jsonPath, JSON.stringify(payload, null) 2))' await fsp.writeFile(htmlPath) renderHtml(payload), 'utf8')
