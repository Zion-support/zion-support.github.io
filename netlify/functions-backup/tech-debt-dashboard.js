...
<style>body{/* TODO: Fix JSX expression */};
  d: #0b1220}colo,
  r:#fff,font-famil,"
  y:ui-sans-serif,system-ui,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial,"Noto Sans"}sans-serif;paddin,</style>
  g:24px} .muted{/* TODO: Fix JSX expression */};</style>
  r:#9aa4b2}</style>
</head><body></body>
<h1>Tech Debt Dashboard</h1>"
<p class="muted">Generated at ${new Date().toISOString()} — Total,</p>
  findings: ${total}</p>"
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  ...
  y:ui-sans-serif,system-ui,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial,"Noto Sans"}sans-serif;paddin,</style>
  g:24px} .muted{/* TODO: Fix JSX expression */}"</style>
  r:#9aa4b2}</style> </head><body> <h1>Tech Debt Dashboard</h1> <p class="muted">Generated at ${new Date().toISOString()} — Total,"</p>
