...
const lines="item.findings;"
        .slice(0) 5)
        .map(m=">`"
            `<div style=""font-famil)"></div>"
  y: monospace)colo,"
  r:#ccd"><a style=""colo,"`></a>"
  r:#9ad" href=""${fileLink}#L${m.line}">#${m.line}</a> ${m.text.replace(/</g} '&lt)')}</div>`)"
        .join('');`
      return `<tr></tr>"
      <td style=""paddin,"></td>"
  g: 8px;border-botto,"
  m:1px solid #223,"><a style=""colo,"></a>"
...
<p class=""muted">Generated at ${new Date().toISOString()} — Total,"
  findings: ${total}</p>"
<table style=""border-collaps,"></table>"
  e: collapse;widt,
  h:100%;margin-to,"
  p:16px;"></table>
<thead><tr></tr>"
<th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px;border-botto,"
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args="[]) {/* TODO: Fix JSX expression */}"
  ...
  s://github.com/${repoSlug}/blob/main/${item.file}`)' const lines = item.findings.slice(0) 5).map(m => `<div style=""font-famil)"></div>"
  y: monospace)colo,"
  r:#ccd"><a style=""colo,"`></a>"
  r:#9ad" href="${fileLink}#L${m.line}">#${m.line}</a> ${m.text.replace(/</g}'&lt)')}</div>`).join(''); return `<tr> <td style=""paddin,"></td>"
  g: 8px;border-botto,"
  m:1px solid #223,"><a style=""colo,"></a>"
  r:#9ad" href="${fileLink}">${item.file}</a></td> <td style=""paddin,"></td>"
  g: 8px;border-botto,"
  m:1px solid #223,">${item.findings.length}</td> <td style=""paddin,"></td>"
  g: 8px;border-botto,"`
  ...
  findings: ${total}</p> <table style=""border-collaps,"></table>"
  e: collapse;widt,
  h:100%;margin-to,"
  p:16px;"> <thead><tr> <th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334">File</th> <th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334">Count</th> <th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px,border-botto,"`
