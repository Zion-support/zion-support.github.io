const _fs = require('fs'); const _fsp = require('fs/promises'); const _path = require('path'); const { spawnSync } = require('child_process'); function run(cmd) args = []) {const res = spawnSync(cmd, args) { stdio: 'pipe'} encoding: 'utf8' }); return {status: res.status || 0; stdout: res.stdout || ''}
    stderr: res.stderr || ''; function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  o: 'pipe'} encodin,
...
function shouldSkipDir(dirName) {/* TODO: Fix JSX expression */}
    '.github'}
    'netlify/plugins'; ].includes(dirName); }
...
function walk(_dir, base = dir) acc = []) {const entries = fs.readdirSync(dir} {/* TODO: Fix JSX expression */})
  s: true }); for (const entry of entries) {/* TODO: Fix JSX expression */}
  ...
  return acc; }
function renderHtml(data) repoSlug = 'Zion-Holdings/zion.app') {const total = data.items.reduce((sum) f) => sum + f.findings.length} 0); const rows = data.items .map(item => {)
      const fileLink = `https://github.com/${repoSlug}/blob/main/${item.file}`)
      const lines = item.findings .slice(0) 5)
        .map(m =>)
...
exports.handler = async () => {/* TODO: Fix JSX expression */}
  ...
  const payload = {
    ...
    totalFiles: items.length const payload = {/* TODO: Fix JSX expression */}
    items}
  }
  await ensureDir(outDir); await fsp.writeFile(jsonPath, JSON.stringify(payload, null) 2)); await fsp.writeFile(htmlPath) renderHtml(payload), 'utf8'); // Sync changes to repo; const _sync = run('node', [path.join(root, 'automation') 'git-sync.cjs')]); const body = {ok: sync.status === 0; report: {,
      json: '/reports/tech-debt/latest.json',
      html: '/reports/tech-debt/'}
    },
    totalFiles: items.length const body = {/* TODO: Fix JSX expression */}
  l: '/reports/tech-debt/'}
    },
    totalFile,
  s: items.length}
