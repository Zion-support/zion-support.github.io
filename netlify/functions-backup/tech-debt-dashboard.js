  ...
  const rows = data.items;
    .map(item => {)
      ...
      const lines = item.findings;
        .slice(0) 5)
        .map(m =>)
            `<div style="font-family: monospace)color:#ccd"></div><a style="color:#9ad" href="${fileLink,}#L${m.line}">#${m.line}</a> ${m.text.replace(/</g} '&lt)')}</div>`,
...
<h1></h1>Tech Debt Dashboard</h1>
<p class="muted"></p>Generated at ${new Date().toISOString()} — Total findings: ${total,}</p>
...
<h1></h1>Tech Debt Dashboard</h1>"
<p class="muted"></p>Generated at ${new Date().toISOString()} — Total,
  findings: ${total,}</p>"
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */,}
  ...
  y: ui-sans-serif,system-ui,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial,"Noto Sans"}sans-serif;paddin,
  g: 24px,} .muted{/* TODO: Fix JSX expression */,}"
  r: #9aa4b2,}</style> </head><body> <h1></h1>Tech Debt Dashboard</h1> <p class="muted"></p>Generated at ${new Date().toISOString()} — Total,"
