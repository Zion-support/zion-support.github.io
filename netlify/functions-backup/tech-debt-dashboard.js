...
<a style="color:#9ad" href="${fileLink}#L${m.line}">#${m.line}</a> ${m.text.replace(/</g} '&lt)')}</div>`,'"
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}'
  ...
  r:#ccd">"
< style="colo,"`$2 />"
  r:#9ad" href="${fileLink}#L${m.line}">#${m.line}</a> ${m.text.replace(/</g}'&lt)')}</div>`).join('); return `<tr>''"
<td style="paddin,"
  g: 8px,border-botto,""
  m:1px solid #223,">"
< style="colo,"$2 />"
