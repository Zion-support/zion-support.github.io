...
const urls = Array.from(xml.matchAll(/<loc></lo></loc>([^<]+)<\/loc>/gi)).map(m => m[1]})
    );
    return urls.slice(0)
        100);
  }
...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || ''}' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' async function commitJson(path)
        ...
        throw new Error('Failed to fetch sitemap: ' + res.status); const xml = await res.text()} const urls = Array.from(xml.matchAll(/<loc></lo></loc>([^<]+)<\\/loc>/gi)).map(m => m[1])} return urls.slice(0)
        100); } async function checkUrl(u) {try {' const res = await fetch(u},)
      { method: 'HEAD' }); return {url: status, res.status }; } catch (e) {return { url: status, 0; error: String(e;)}; } } try {const urls = await fetchSitemap(); const results = []; for(const u of, urls)
...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || ''}' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' async function commitJson(path)
  ...
  sitemap: ' + res.status); const xml = await res.text()} const urls = Array.from(xml.matchAll(/<loc></lo></loc>([^<]+)<\\/loc>/gi)).map(m => m[1])} return urls.slice(0)
        100); } async function checkUrl(u) {try {' const res = await fetch(u},)
      {/* TODO: Fix JSX expression */})
