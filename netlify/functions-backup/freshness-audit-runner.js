...
>>>>>>> cursor/fix-errors-and-merge-to-main-6ce7
  ...
  logStep('freshnes)
  s: audit') () => runNode('scripts/freshness-audit.js'))
  logStep('gi)
<<<<<<< HEAD
...
>>>>>>> cursor/fix-errors-and-merge-to-main-6ce7
  ...
  t: res.stdout || '', stder,`
  r: res.stderr || '' } } exports.handler = async () => {const logs = []} const logStep = (name} fn) => { logs.push(`\\n=== ${name} ===`); const {status, stdout} stderr } = fn(); if (stdout) logs.push(stdout); if (stderr) logs.push(stderr); logs.push(`exit=${status}`); return status; } ' logStep('freshnes)
  s: audit') () => runNode('scripts/freshness-audit.js'));' logStep('gi)
<<<<<<< HEAD
