exports.handler = async function (event, context) {
  ...
  if (!githubToken) {
    return json(
      { ok: false, error: 'Missing GITHUB_TOKEN env var for GitHub commit' },
      500
    );
  }
  ...
  const promos = {
    ...
    items: [
      ...
      {
        title: 'Advertise on Homepage',
        desc: 'Refreshes the front page promos continuously',
        href: '/.netlify/functions/autonomous-front-advertiser',
      },
      {
        title: 'Rapid Sync Committer',
        desc: 'Commits a stamp to keep the pipeline flowing',
        href: '/.netlify/functions/hyper-sync-committer',
      },
    ],
  };
  ...
  try {
    ...
    return json({
      ...
      file: path,
      promos,
      response: result.body.slice(0, 250),
    });
  } catch (err) {
