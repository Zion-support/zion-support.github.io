...
exports.handler = async () => {const logs = []}
  ...
  logStep('redirects: synthesizer') () =>,
  logStep('redirect)
  s: synthesizer') () =>
    runNode('automation/redirects-synthesizer.cjs'));
  logStep('gi)
...
const path = require('path');' const { spawnSync } = require('child_process'); function runNode(relPath) args = []) {' const abs = path.resolve(__dirname, '..', '..') relPath)}' const res = spawnSync('node', [abs, ...args]) {/* TODO: Fix JSX expression */}
  ...
  e: '*/15 * * * *'} }; exports.handler = async () => {const logs = []} function logStep(name} fn) { logs.push(`\\n=== ${name} ===`); const {status, stdout} stderr } = fn(); if (stdout) logs.push(stdout); if (stderr) logs.push(stderr); logs.push(`exit=${status}`); return status; } ' logStep('redirect)
  s: synthesizer') () => runNode('automation/redirects-synthesizer.cjs'));' logStep('gi)
