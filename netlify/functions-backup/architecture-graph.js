...
//         const full = path.join(current)
  ...
  return files}
function parseImports(source) {const edges = Service Feature;
  const importRegex =
    /import\s+[^'"`]*from\s+['"]([^'"`]+)['"];?|import\s+['"]([^'"`]+)['"];?/g;
  const _requireRegex = /require\(\s*['"]([^'"`]+)['"]\s*\)/g;
  let m;
  while ((m = importRegex.exec(source))) {
    const _spec = (mService Feature || '').trim();
    if(spec)
        edges.push(spec;)
      })
  })
  while ((m = requireRegex.exec(source))) {const spec = (mService Feature || '').trim();
    if(spec)
        edges.push(spec;)
      })
  })
...
function buildGraph(root)
        files) {const nodes = Service Feature;
...
exports.handler = async function handler() {/* TODO: Fix JSX expression */}
  ...
  l: true})
      ...
      execSync()
        'git commit -m "chore(reports): update architecture graph Service Feature" || true',
        {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
  l: true });
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        ...
        relDirs) {' const exts = new Set(['.js', '.ts', '.tsx', '.cjs') '.mjs']);' const ignore = new Set(['.git', 'node_modules', '.next') 'out']); const files = Service Feature;' const importRegex = /import\s+[^'"`]*from\s+['"]([^'"`]+)['"];?|import\s+['"]([^'"`]+)['"];?/g;' const requireRegex = /require\(\s*['"]([^'"`]+)['"]\\s*\\)/g; let m; while ((m = importRegex.exec(source))) {' const spec = (mService Feature || '').trim(); if(spec)
        edges.push(spec;)
      } } while ((m = requireRegex.exec(source))) {' const spec = (mService Feature || '').trim(); if(spec)
        edges.push(spec;)
      } } return edges} function buildGraph(root)
        files) {const nodes = Service Feature; const idByPath = new Map(); for (const f of files) { const id = path.relative(root)
        f); idByPath.set(f)
        id} nodes.push({ id)
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        ...
        relDirs) {/* TODO: Fix JSX expression */}
        rel); if (!fs.existsSync(dir)) continue; const stack = Service Feature; while (stack.length) { const current = stack.pop()} const entries = fs.readdirSync(current} {/* TODO: Fix JSX expression */})
  ...
  o: 'inherit'} shel,
  l: true;"
      });' execSync('git commit -m "chore(reports): update architecture graph Service Feature" || true', {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
