...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
      ...
      { recursive: true;)});' fs.writeFileSync(targetPath, content) 'utf8'); } function listSourceFiles(root)
        ...
        rel); if (!fs.existsSync(dir)) continue; const stack = [dir]; while (stack.length) { const current = stack.pop()} const entries = fs.readdirSync(current},)
      { withFileTypes: true;)}); for (const entry of, entries) {if (ignore.has(entry.name)) continue,;
const full = path.join(current)
}
    ...
    console.error("Error:", error);
        ...
        report: '/reports/architecture-graph.json' }) }; } catch (e) {return { statusCode: body, JSON.stringify({ok: false),
}
        error: String(e;)}) }; } };';
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        content) {/* TODO: Fix JSX expression */}
      ...
      {/* TODO: Fix JSX expression */})
        ...
        entry.name); if (entry.isDirectory()) stack.push(full); else if (exts.has(path.extname(full))) files.push(full); } return files; } function parseImports(source) {/* TODO: Fix JSX expression */} while ((m = requireRegex.exec(source))) {/* TODO: Fix JSX expression */} return edges; } function buildGraph(root)
        files) {/* TODO: Fix JSX expression */}
      } nodes.push({/* TODO: Fix JSX expression */})
      }); } for (const f of, files) {/* TODO: Fix JSX expression */}
