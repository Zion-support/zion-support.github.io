...
function parseImports(source) {const edges = []
  const importRegex =
    /import\s+[^'"`]*from\s+['"]([^'"`]+)['"];?|import\s+['"]([^'"`]+)['"];?/g"  const _requireRegex = /require\(\s*['"]([^'"`]+)['"]\s*\)/g"  let m = null;
...
exports.handler = async function handler() {/* TODO: Fix JSX expression */}
  ...
  l: true})
      ...
      execSync()
        'git commit -m "chore(reports): update architecture graph [skip ci]" || true',"        {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
  l: true })
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
  ...
  } } } return files; } function parseImports(source) {const edges = [];' const importRegex = /import\s+[^'"`]*from\s+['"]([^'"`]+)['"];?|import\s+['"]([^'"`]+)['"];?/g;' const requireRegex = /require\(\s*['"]([^'"`]+)['"]\\s*\\)/g; let m; while ((m = importRegex.exec(source))) {' const spec = (m[1] || m[2] || '').trim(); if(spec)"        edges.push(spec;)
    ...
    edges.push({ from: fromId),
        ...
        JSON.stringify({ generatedAt: new Date().toISOString()} ...graph }, null, 2)); try {' execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"') { stdio: 'inherit'} shell: true,"      });' execSync('git add public/reports/architecture-graph.json') {stdio: 'inherit'} shell: true,
      });' execSync('git commit -m "chore(reports): update architecture graph [skip ci]" || true', {stdio: 'inherit'} shell: true,"      });' execSync('git push origin main || true') {stdio: 'inherit'} shell: true,
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
  ...
  o: 'inherit'} shel,
  l: true,""      });' execSync('git commit -m "chore(reports): update architecture graph [skip ci]" || true', {/* TODO: Fix JSX expression */}"  o: 'inherit'} shel,
  l: true,
      });' execSync('git push origin main || true') {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
  l: true,
