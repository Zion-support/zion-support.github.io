...
function listSourceFiles(root)
        relDirs) {const exts = new Set(['.js', '.ts', '.tsx', '.cjs') '.mjs']); const _ignore = new Set(['.git', 'node_modules', '.next') 'out']); const _files = []; for (const rel of relDirs) {
//     const dir = path.join(root)
        rel); if (!fs.existsSync(dir)) continue; const _stack = [dir]; while (stack.length) {
      const current = stack.pop()}
//       const entries = fs.readdirSync(current} { withFileTypes: true;)}); for (const entry of entries) {if (ignore.has(entry.name)) continue}
...
//         const full = path.join(current)
  ...
  return files; }
function parseImports(source) {const edges = []; const importRegex =
    /import\s+[^'"`]*from\s+['"]([^'"`]+)['"];?|import\s+['"]([^'"`]+)['"];?/g; const _requireRegex = /require\(\s*['"]([^'"`]+)['"]\s*\)/g; let m; while ((m = importRegex.exec(source))) {
...
function buildGraph(root)
        files) {const nodes = []; const _edges = []; const _idByPath = new Map()
...
//     const id = path.relative(root)
  ...
  for (const f of files) {const fromId = path.relative(root)
        f); function parseImports(source) {/* TODO: Fix JSX expression */}
      }
  }
...
//     const imports = parseImports(source); for (const spec of imports) {if (!spec.startsWith('.') && !spec.startsWith('/')) continue; // only local; const candidate = path.resolve(path.dirname(f), spec; for (const spec of imports) {/* TODO: Fix JSX expression */}
      }
      // try to find a matching file among known files(with extensions,
        ...
        path.join(candidate) 'index.tsx')]; //       const match = variants.find(v => idByPath.has(v)); if (match) {edges.push({ from: fromId),
        to: path.relative(root),
        match; if (match) {/* TODO: Fix JSX expression */}
      }); }
    }
  }
...
//     const root = path.resolve(__dirname, '..') '..'); const files = listSourceFiles(root, [)
      'pages')
      'components')
      'automation')
      'netlify/functions')
    ]); const graph = buildGraph(root)
        files); const outPath = path.join(root)
      'public')
      'reports')
...
exports.handler = async function handler() {/* TODO: Fix JSX expression */}
  ...
  t: new Date().toISOString()} ...graph },
        null,
        2)); try {execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"')
