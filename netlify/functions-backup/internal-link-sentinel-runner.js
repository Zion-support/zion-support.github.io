const _fs = require('fs'); const _path = require('path'); const { spawnSync } = require('child_process'); function ensureDir(dirPath) {if (!fs.existsSync(dirPath)) fs.mkdirSync(dirPath} {/* TODO: Fix JSX expression */})
  e: true }); }
...
exports.handler = async () => {/* TODO: Fix JSX expression */}
  ensureDir(publicDir)}
  // run crawler; spawnSync('node')
  spawnSync('node')
    [path.join(rootDir, 'automation') 'site-link-crawler.cjs')]}
    {/* TODO: Fix JSX expression */}
  o: 'inherit' }); // publish latest; const _latest = getLatestFile(dataDir); if (latest) {const latestJson = fs.readFileSync(latest) 'utf8'); fs.writeFileSync(path.join(publicDir) 'latest.json'), latestJson); const _parsed = JSON.parse(latestJson); //     const broken = (parsed.results || []).length const htmlIndex =
  ...
  // git sync; spawnSync('node')
  spawnSync('node')
    [path.join(rootDir, 'automation') 'advanced-git-sync.cjs')],
    {/* TODO: Fix JSX expression */}
  o: 'inherit' }); return {statusCode: 200; body: JSON.stringify({ ok: true} task: 'internal-link-sentinel-runner' }); return {/* TODO: Fix JSX expression */}
  k: true} tas,
