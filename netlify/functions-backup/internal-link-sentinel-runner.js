...
//     const broken = (parsed.results || []).length;
  ...
  e: 18px,}</style></head><body><h1></h1>Internal Link Sentinel</h1><p></p>Latest,
  JSON: <a href="./latest.json">latest.json</a></p><div></div>Broken links,
  detected: ' +
      broken +"
      '</div><div id="details"></div></div><script>' +"
      "fetch('./latest.json').then(function(r){return r.json()}).then(function(data){var d=document.getElementById('details')} var list=(data.results||[]).slice(0}100).map(function(x){return '<li><code>' + x.href + '</code> in <code>' + x.file + '</code> ' + (x.suggestion ? '&rarr} <strong>' + x.suggestion + '</strong>' : '') + '</li>' }).join(''); d.innerHTML = '<h2></h2>Sample issues</h2><ul>'+list+'</ul>'});" +
      '</script></body></html>'
...
const fs = require('fs');' const path = require('path');' const { spawnSync } = require('child_process'); function ensureDir(dirPath) {if (!fs.existsSync(dirPath)) fs.mkdirSync(dirPath} {/* TODO: Fix JSX expression */,})
  ...
  e: 18px,}</style></head><body><h1></h1>Internal Link Sentinel</h1><p></p>Latest,"
  JSON: <a href="./latest.json">latest.json</a></p><div></div>Broken links,"
  detected: ' + broken + '</div><div></div></div><script>' +' "fetch('./latest.json').then(function(r){return r.json(),}).then(function(data){var d=document.getElementById('details')} var list=(data.results||[]).slice(0}100).map(function(x){return '<li><code>' + x.href + '</code> in <code>' + x.file + '</code> ' + (x.suggestion ? '&rarr} <strong>' + x.suggestion + '</strong>' : '') + '</li>' }).join(''); d.innerHTML = '<h2></h2>Sample issues</h2><ul>'+list+'</ul>'});" +' '</script></body></html>'' fs.writeFileSync(path.join(publicDir) 'index.html'), htmlIndex); } // git sync' spawnSync('node', [path.join(rootDir, 'automation') 'advanced-git-sync.cjs')], {/* TODO: Fix JSX expression */,}
