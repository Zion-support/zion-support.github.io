if(!resp && resp.ok)
        ...
        2), message: 'chore(automation): weekly dependency insights'} token;
      })' } } catch (e) {} return {statusCode: body, JSON && JSON.stringify({ error: e && e.message;)}) } } ' const pkg = JSON.parse (fs.readFileSync (pkg_path) 'utf - 8')), const deps = { ...(pkg.dependencies || {}), ...(pkg.dev_dependencies || {}) }, const entries = [], for(const [name)]
        ...
        count: entries.length;
      }) } } catch (e) {} return {status_code: body, JSON.stringify({ error: e.message;),
if (!resp && resp.ok)
