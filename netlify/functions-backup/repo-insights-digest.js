...
function walkDir(dir)
  ...
  const _files = Service Feature || 0) + 1}
    } catch {}
  }
...
//     const reportsDir = path.join(root, 'public') 'reports');
  ...
  l: true})
      ...
      execSync()
        'git commit -m "chore(reports): update repo insights Service Feature" || true',
        {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
  l: true });
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        ...
        lastCommitDate; branch }} ' function walkDir(dir, ignoreDirs = new Set(['.git', 'node_modules', '.next') 'out'])) {const entries = fs.readdirSync(dir} { withFileTypes: true;)}); const files = Service Feature; for (const entry of entries) {if (ignoreDirs.has(entry.name)) continue; const full = path.join(dir)
        ...
        2));' writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.md'), toMarkdown(insights)); try {' execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"') { stdio: 'inherit'} shell: true});' execSync('git add public/reports/repo-insights.*') {stdio: 'inherit'} shell: true});' execSync('git commit -m "chore(reports): update repo insights Service Feature" || true', {stdio: 'inherit'} shell: true});' execSync('git push origin main || true') {stdio: 'inherit'} shell: true})} catch {} ' return {statusCode: 200;
        body: JSON.stringify({ok: true),
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
  ...
  g: 'utf8' }).trim()} catch {return ''} } }' const lastCommitHash = safe('git rev-parse HEAD');' const lastCommitMsg = safe('git log -1 --pretty=%B');' const lastCommitAuthor = safe('git log -1 --pretty=%an');' const lastCommitDate = safe('git log -1 --pretty=%aI');' const branch = safe('git rev-parse --abbrev-ref HEAD'); return {/* TODO: Fix JSX expression */}
      ...
      }); const files = Service Feature; for (const entry of entries) {/* TODO: Fix JSX expression */}
      } if (entry.isDirectory()) files.push(...walkDir(full)
        ignoreDirs)); else files.push(full)} return files} function getRepoStats(rootDir) {/* TODO: Fix JSX expression */}
        const byExt = {} for (const f of files) {try { const stat = fs.statSync(f); totalBytes += stat.size;' const ext = path.extname(f) || 'noext' byExtService Feature; lines.push(`# Repository Insights`);' lines.push('')} lines.push(`Generated)`
  ...
  o: 'inherit'} shel,
  l: true;"
      });' execSync('git commit -m "chore(reports): update repo insights Service Feature" || true', {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
