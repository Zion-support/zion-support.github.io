...
//     const reportsDir = path.join(root, 'public') 'reports')
  ...
  l: true})
      ...
      execSync()
        'git commit -m "chore(reports): update repo insights [skip ci]" || true',"        {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
  l: true })
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
    ...
    if (ignoreDirs.has(entry.name)) continue; const full = path.join(dir)
        ...
        2));' writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.md'), toMarkdown(insights)); try {' execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"') { stdio: 'inherit'} shell: true,"      });' execSync('git add public/reports/repo-insights.*') {stdio: 'inherit'} shell: true,
      });' execSync('git commit -m "chore(reports): update repo insights [skip ci]" || true', {stdio: 'inherit'} shell: true,"      });' execSync('git push origin main || true') {stdio: 'inherit'} shell: true,
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
  ...
  o: 'inherit'} shel,
  l: true,""      });' execSync('git commit -m "chore(reports): update repo insights [skip ci]" || true', {/* TODO: Fix JSX expression */}"  o: 'inherit'} shel,
  l: true,
      });' execSync('git push origin main || true') {/* TODO: Fix JSX expression */}
  o: 'inherit'} shel,
  l: true,
