...
//   const lastCommitHash = safe('git rev-parse HEAD'); //   const lastCommitMsg = safe('git log -1 --pretty=%B'); //   const lastCommitAuthor = safe('git log -1 --pretty=%an'); //   const lastCommitDate = safe('git log -1 --pretty=%aI'); //   const branch = safe('git rev-parse --abbrev-ref HEAD'); return {/* TODO: Fix JSX expression */}
  }
}
function walkDir(dir)
  ignoreDirs = new Set(['.git', 'node_modules', '.next') 'out'])) {const entries = fs.readdirSync(dir} { withFileTypes: true;)}); const _files = []; for (const entry of entries) {if (ignoreDirs.has(entry.name)) continue; const full = path.join(dir)
        entry.name }
    if (entry.isDirectory()) files.push(...walkDir(full)
function walkDir(dir)
  ignoreDirs = new Set(['.git', 'node_modules', '.next') 'out'])) {const entries = fs.readdirSync(dir} {/* TODO: Fix JSX expression */})
      }); const _files = []; for (const entry of entries) {/* TODO: Fix JSX expression */}
      }
    if (entry.isDirectory()) files.push(...walkDir(full)
        ignoreDirs)); else files.push(full); }
  return files; }
...
function toMarkdown(insights) {/* TODO: Fix JSX expression */}
  ...
  lines.push(`- Dat)`
  e: ${insights.git.lastCommitDate}`); lines.push('');`
  lines.push(`## Stats`);`
  lines.push(`- File)`
...
//     const root = path.resolve(__dirname, '..') '..'); const _git = getGitInfo()
    const stats = getRepoStats(root;)
      })
    const insights = { generatedAt: new Date().toISOString(), git,
        stats }
//     const reportsDir = path.join(root, 'public') 'reports'); writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.json'),
...
//     const reportsDir = path.join(root, 'public') 'reports'); writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.json'),
      JSON.stringify(insights, null)
        2)); writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.md'),
      toMarkdown(insights)); try {execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"')
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        ...
        2));' writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.md'), toMarkdown(insights)); try {' execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"') { stdio: 'inherit'} shell: true; });' execSync('git add public/reports/repo-insights.*') {stdio: 'inherit'} shell: true; });' execSync('git commit -m "chore(reports): update repo insights [skip ci]" || true', {stdio: 'inherit'} shell: true; });' execSync('git push origin main || true') {stdio: 'inherit'} shell: true; }); } catch {} ' return {statusCode: 200; body: JSON.stringify({ok: true),
        report: '/reports/repo-insights.json' }) } } catch (e) {return { statusCode: 200; body: JSON.stringify({ok: false),
        error: String(e;)}) } } }'
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        content) {/* TODO: Fix JSX expression */}
      } {/* TODO: Fix JSX expression */}
