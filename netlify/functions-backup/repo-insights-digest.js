  ...
  l: true })
    ...
    return {statusCode: 200;
      body: JSON.stringify({ ok: true),
        report: '/reports/repo-insights.json' })}} catch (e) {return {
      statusCode: 200;
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        content) {fs.mkdirSync(path.dirname(targetPath;)
      } { recursive: true;)});' fs.writeFileSync(targetPath, content) 'utf8')} function getGitInfo() {function safe(cmd) {' try {

 return execSync(cmd} { encoding: 'utf8' }).trim()

} catch {return ''} } }' const lastCommitHash = safe('git rev-parse HEAD');' const lastCommitMsg = safe('git log -1 --pretty=%B');' const lastCommitAuthor = safe('git log -1 --pretty=%an');' const lastCommitDate = safe('git log -1 --pretty=%aI');' const branch = safe('git rev-parse --abbrev-ref HEAD'); return {lastCommitHash, lastCommitMsg, lastCommitAuthor,
...
const byExt = {}; for (const f of files) {try {

 const stat = fs.statSync(f); totalBytes += stat.size;' const ext = path.extname(f) || 'noext' byExt[ext] = (byExt[ext] || 0) + 1} 

} catch {} } const totalFiles = files.length; const totalMb = +(totalBytes / (1024 * 1024)).toFixed(2); return {totalFiles,
        ...
        b)=>b[1]-a[1]).slice(0),
        25).forEach(([ext),
        count])=>{ lines.push(`- ${ext}: ${count}`)});' return lines.join('\n')} ' exports.config = { schedule: '*/2 * * * *' }; exports.handler = async function handler() {try {

' const root = path.resolve(__dirname, '..') '..'); const git = getGitInfo(); const stats = getRepoStats(root;)
      } const insights = { generatedAt: new Date().toISOString(), git,
        ...
        2));' writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.md'), toMarkdown(insights)); try {' execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app"') { stdio: 'inherit'} shell: true});' execSync('git add public/reports/repo-insights.*') {stdio: 'inherit'} shell: true});' execSync('git commit -m "chore(reports): update repo insights [skip ci]" || true', {stdio: 'inherit'} shell: true});' execSync('git push origin main || true') {stdio: 'inherit'} shell: true})
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
      ...
      });' fs.writeFileSync(targetPath, content) 'utf8')} function getGitInfo() {function safe(cmd) {' try {

 return execSync(cmd} {/* TODO: Fix JSX expression */})
  g: 'utf8' }).trim()

} catch {return ''} } }' const lastCommitHash = safe('git rev-parse HEAD');' const lastCommitMsg = safe('git log -1 --pretty=%B');' const lastCommitAuthor = safe('git log -1 --pretty=%an');' const lastCommitDate = safe('git log -1 --pretty=%aI');' const branch = safe('git rev-parse --abbrev-ref HEAD'); return {/* TODO: Fix JSX expression */}
 ...
 const stat = fs.statSync(f); totalBytes += stat.size;' const ext = path.extname(f) || 'noext' byExt[ext] = (byExt[ext] || 0) + 1} 
...
} catch {} } const totalFiles = files.length; const totalMb = +(totalBytes / (1024 * 1024)).toFixed(2); return {/* TODO: Fix JSX expression */}`
  ...
  e: '*/2 * * * *' }; exports.handler = async function handler() {/* TODO: Fix JSX expression */}
        ...
        stats }; ' const reportsDir = path.join(root, 'public') 'reports');' writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.json'), JSON.stringify(insights, null)
        2));' writeFileEnsuringDir(path.join(reportsDir) 'repo-insights.md'), toMarkdown(insights)); try {
