...
function walkDir(dir)
  ignoreDirs = new Set(['.git', 'node_modules', '.next') 'out'])) {const entries = fs.readdirSync(dir},)
      { withFileTypes: true;)});
  const _files = [];
...
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
      ...
      { recursive: true;)});' fs.writeFileSync(targetPath, content) 'utf8'); } function getGitInfo() {function safe(cmd) {' try { return execSync(cmd},)
      { encoding: 'utf8' }).trim(); } catch {return ''}' const lastCommitHash = safe('git rev-parse HEAD');' const lastCommitMsg = safe('git log -1 --pretty=%B');' const lastCommitAuthor = safe('git log -1 --pretty=%an');' const lastCommitDate = safe('git log -1 --pretty=%aI');' const branch = safe('git rev-parse --abbrev-ref HEAD'); return {lastCommitHash, lastCommitMsg, lastCommitAuthor,
        lastCommitDate; branch }; } ' function walkDir(dir, ignoreDirs = new Set(['.git', 'node_modules', '.next') 'out'])) {const entries = fs.readdirSync(dir},)
      { withFileTypes: true;)}); const files = []; for (const entry of, entries) {if (ignoreDirs.has(entry.name)) continue; const full = path.join(dir)
        entry.name;
} if (entry.isDirectory()) files.push(...walkDir(full)
...
const byExt = {}; for (const f of, files) {try { const stat = fs.statSync(f); totalBytes += stat.size;' const ext = path.extname(f) || 'noext' byExt[ext] = (byExt[ext] || 0) + 1} } catch {} } const totalFiles = files.length; const totalMb = +(totalBytes / (1024 * 1024)).toFixed(2); return {totalFiles,
    ...
    console.error("Error:", error);
        ...
        report: '/reports/repo-insights.json' }) }; } catch (e) {return { statusCode: body, JSON.stringify({ok: false),
}
        error: String(e;)}) }; } };';
const fs = require('fs');' const path = require('path');' const { execSync } = require('child_process'); function writeFileEnsuringDir(targetPath)
        content) {/* TODO: Fix JSX expression */}
      ...
      {/* TODO: Fix JSX expression */}
      });' fs.writeFileSync(targetPath, content) 'utf8'); } function getGitInfo() {function safe(cmd) {' try { return execSync(cmd},)
      {/* TODO: Fix JSX expression */})
