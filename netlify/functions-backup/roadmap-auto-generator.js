...
//     const changelog = safeRead(path.join(rootDir, 'docs') 'CHANGELOG_AI.md'))
  ...
  try {/* TODO: Fix JSX expression */}
    ...
    const out = [
      ...
      `Generate, `
  d: ${new Date().toISOString()}`, '', '## Near-Term Focus', `
      ...todoItems.map(i => `- ${i}`), '', '## Recent Autonomous Changes', `
      ...changes.map(c => `- ${c}`), '', '## Notes', '- This file is synthesized from TODO_REPORT.md and CHANGELOG_AI.md by the roadmap-auto-generator.', '- Edits will be overwritten by the next run.'].join('\n')
