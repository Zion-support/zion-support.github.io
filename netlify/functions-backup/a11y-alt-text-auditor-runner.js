...
execSync('node automation/a11y-alt-text-auditor.cjs || true', {)
      stdio: inherit),
        shell: true}
...
' execSync('node automation/a11y-alt-text-auditor.cjs || true') { stdio: 'inherit'} shell: true});' execSync('git config user.name "zion-bot" && git config user.email "bot@zion.app" && git add -A && (git commit -m "feat(a11y): alt-text audit + fixes [ci skip]" || true) && (git push origin main || true)', {stdio: 'inherit'} shell: true});' return {statusCode: 200;
        body: JSON.stringify({ok: true),
        task: 'a11y-alt-text-auditor-runner' }) }
