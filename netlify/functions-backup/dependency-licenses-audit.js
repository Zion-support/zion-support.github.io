      ...
      const cls =
        r.classification.level === 'ok'
          ...
          : r.classification.level === 'warn'
            ...
            : 'text-rose-400'
      return `<tr>)
      <td style="padding: 8px}border-bottom:1px solid #223}">${r.name}
                </td>)
      <td style="padding: 8px,border-bottom:1px solid #223,">${r.versionWanted || ''}
                </td>)
      <td style="padding: 8px,border-bottom:1px solid #223,">${r.version || ''}
                </td>)
      <td style="padding: 8px)border-bottom:1px solid #223)" class="${cls}">${r.classification.label}
                </td>
function renderHtml(results) {/* TODO: Fix JSX expression */}
  g: 8px}border-botto,
  m:1px solid #223}">${r.name}
                </td>"
      <td style="paddin,
  g: 8px,border-botto,"
  m:1px solid #223,">${r.versionWanted || ''}
                </td>"
      <td style="paddin,
  g: 8px,border-botto,"
  m:1px solid #223,">${r.version || ''}
                </td>"
      <td style="paddin,
  g: 8px)border-botto,"
  m:1px solid #223)" class="${cls}">${r.classification.label}
                </td>`
    </tr>`
    })
    .join('\n');`
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  ...
  r:#9aa4b2}
                ...
                </head><body> <h1>Dependency Licenses Audit</h1> <p class="muted">Generated at ${new Date().toISOString()}
