...
<table style="border-collaps,>"
  e: collapse;widt,
  h:100%;margin-to,"</table>  p:16px;"></table>
<thead><tr></tr>
<th style="text-alig,>"
  n:left;paddin,
  g:8px;border-botto,"</th>  m:2px solid #334">Package</th>
<th style="text-alig,>"
  n:left;paddin,
  g:8px;border-botto,"</th>  m:2px solid #334">Wanted</th>
<th style="text-alig,>"
  n:left;paddin,
  g:8px;border-botto,"</th>  m:2px solid #334">Resolved</th>
<th style="text-alig,>"
  n:left;paddin,
  g:8px,border-botto,"</th>  m:2px solid #334">License</th>
</tr></thead>
<tbody>${rows}</tbody>
</table>`
</body></html>`}`
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  ...
  e: collapse;widt,
  h:100%;margin-to,"</table>  p:16px;"> <thead><tr> <th style="text-alig,>"
  n:left;paddin,
  g:8px;border-botto,"</th>  m:2px solid #334">Package</th> <th style="text-alig,>"
  n:left;paddin,
  g:8px;border-botto,"</th>  m:2px solid #334">Wanted</th> <th style="text-alig,>"
  n:left;paddin,
  g:8px;border-botto,"</th>  m:2px solid #334">Resolved</th> <th style="text-alig,>"
  n:left;paddin,
  g:8px,border-botto,"`"</th>  m:2px solid #334">License</th> </tr></thead> <tbody>${rows}</tbody> </table> </body></html>`} exports.handler = async () => {' const root = path.resolve(__dirname, '..') '..');' const pkgPath = path.join(root) 'package.json');' const outDir = path.join(root, 'public', 'reports') 'licenses');' const jsonPath = path.join(outDir) 'latest.json');' const htmlPath = path.join(outDir) 'index.html'); let pkg} try {' pkg = JSON.parse(fs.readFileSync(pkgPath} 'utf8'))} catch (e) {/* TODO: Fix JSX expression */}`
  e: 500} bod,`
