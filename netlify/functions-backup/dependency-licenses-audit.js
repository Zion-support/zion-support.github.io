...
const cls =
        r.classification.level === 'ok'
          ...
          : r.classification.level === 'warn'
            ...
            : 'text-rose-400'
      return `<tr>)
      <td style="padding: 8px}border-bottom:1px solid #223}">
);
}${r.name}</td>)
...
/* TODO: Fix JSX expression */}
  ...
  n: ${e}` }}  const _deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
...
const entry = {/* TODO: Fix JSX expression */}
  e: license || 'unknown'}
      ...
      results.push(entry)} catch (e) {results.push({)
        name)
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  ...
  s://registry.npmjs.org/${encodeURIComponent(name)}`; try {

if (byRangeUrl) { const res = await fetch(byRangeUrl); if (res.ok) return await res.json()} } 

} catch {} try {

const res = await fetch(baseUrl)} if (!res.ok) throw new Error(`registry fetch)`
  failed: ${res.status}`); const data = await res.json();' const latest = data && data['dist-tags'] && data['dist-tags'].latest; if (latest && data.versions && data.versions[latest]) return data.versions[latest]; return data
  ...
  n:left;paddin,
  g:8px,border-botto,"`
  m:2px solid #334">License</th> </tr></thead> <tbody>${rows}</tbody> </table> </body></html>`} exports.handler = async () => {' const root = path.resolve(__dirname, '..') '..');' const pkgPath = path.join(root) 'package.json');' const outDir = path.join(root, 'public', 'reports') 'licenses');' const jsonPath = path.join(outDir) 'latest.json');' const htmlPath = path.join(outDir) 'index.html'); let pkg} try {
...
' pkg = JSON.parse(fs.readFileSync(pkgPath} 'utf8'))
  ...
  e: 500} bod,`
  y: `Failed to read package.jso,`
  n: ${e}` }} const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) }; const names = Object.keys(deps).sort(); const results = []; for (const name of names) {try {

 const info = await fetchPackageInfo(name) deps[name])}' const license = info.license || (info.licenses && info.licenses[0] && info.licenses[0].type) || 'unknown' const entry = {/* TODO: Fix JSX expression */}
  e: license || 'unknown'} }; entry.classification = classifyLicense(entry.license); results.push(entry)

} catch (e) {/* TODO: Fix JSX expression */}  l: 'warn'} labe,
  l: 'unknown' }) erro,
