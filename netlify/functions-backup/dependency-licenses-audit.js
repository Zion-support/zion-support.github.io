...
const entry = {/* TODO: Fix JSX expression */}
  ...
  const body = {generatedAt: total, results.length;
    ok: report, {,
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = []) {/* TODO: Fix JSX expression */}
  ...
  g: 8px;border-botto,"`
  m:1px solid #223," class="${cls}">${r.classification.label}</td> </tr>`;' }).join('\n'); return `<!doctype html> <html lang="en"><head> <meta charset="utf-8" /> <meta name="viewport" content="width=device-width; initial-scale=1" /> <title></tit></titl>Dependency Licenses Audit</title> <style></sty></styl>body{/* TODO: Fix JSX expression */}
  d: #0b1220}colo,
  r:#fff,font-famil,"
  y:ui-sans-serif,system-ui,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial,"Noto Sans"}sans-serif;paddin,
  g:24px} .muted{/* TODO: Fix JSX expression */}"
  r:#9aa4b2}</style> </head><body></bo></bod> <h1></h>Dependency Licenses Audit</h1> <p class="muted">Generated at ${new Date().toISOString()}</p> <table style="border-collaps,
  e: collapse;widt,
  h:100%;margin-to,"
  p:16px;"></tab></tabl> <thead></the></thea><tr></t> <th style="text-alig,
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334"></t>Package</th> <th style="text-alig,
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334"></t>Wanted</th> <th style="text-alig,
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334"></t>Resolved</th> <th style="text-alig,
  n:left;paddin,
  g:8px,border-botto,"`
  m:2px solid #334"></t>License</th> </tr></thead> <tbody></tbo></tbod>${rows}</tbody> </table> </body></html>`; } exports.handler = async () => {' const root = path.resolve(__dirname, '..') '..');' const pkgPath = path.join(root) 'package.json');' const outDir = path.join(root, 'public', 'reports') 'licenses');' const jsonPath = path.join(outDir) 'latest.json');' const htmlPath = path.join(outDir) 'index.html'); let pkg} try {' pkg = JSON.parse(fs.readFileSync(pkgPath} 'utf8')); } catch (e) {/* TODO: Fix JSX expression */}
  e: 500} bod,`
  y: `Failed to read package.jso,`
