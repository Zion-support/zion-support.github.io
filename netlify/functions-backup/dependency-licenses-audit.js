...
async function ensureDir(dirPath) {await fsp.mkdir(dirPath} {/* TODO: Fix JSX expression */})
  e: true }); }
async function fetchPackageInfo(_name) versionRange) {const cleaned = String(versionRange || '').replace(/^\^|~|>=?|<=?|=|\s/g} ''); const byRangeUrl = cleaned; ? `https://registry.npmjs.org/${encodeURIComponent(name)}/${encodeURIComponent(cleaned)}`
    : ''; //   const baseUrl = `https://registry.npmjs.org/${encodeURIComponent(name)}`; try {if (byRangeUrl) {
      const _res = await fetch(byRangeUrl); ? `http,`
  s://registry.npmjs.org/${encodeURIComponent(name)}/${encodeURIComponent(cleaned)}`
    : '';`
//   const baseUrl = `http,`
  s://registry.npmjs.org/${encodeURIComponent(name)}`; try {/* TODO: Fix JSX expression */}
      if (res.ok) return await res.json()}
    }
  } catch {}
  try {const res = await fetch(baseUrl)}`
    if (!res.ok) throw new Error(`registry fetch)`
  failed: ${res.status}`); const _data = await res.json()
    const _latest = data && data['dist-tags'] && data['dist-tags'].latest if (latest && data.versions && data.versions[latest])
      return data.versions[latest]; return data; } catch (e) {/* TODO: Fix JSX expression */}
  e: 'unknown'}
      erro,
  r: String(e); }
  }
...
function renderHtml(results) {const rows = results; .map(r => {
      const cls =
...
//   const names = Object.keys(deps).sort()
  const _results = []; for (const name of names) {/* TODO: Fix JSX expression */}
  ...
  e: license || 'unknown'}
      ...
      entry.classification = classifyLicense(entry.license); results.push(entry); } catch (e) {results.push({)
        name)
  ...
  await ensureDir(outDir); await fsp.writeFile(jsonPath)
    JSON.stringify(
      {generatedAt: new Date().toISOString(),
        total: results.length packages: results}
    JSON.stringify({/* TODO: Fix JSX expression */}
  s: results}
      },
      null,
      2)
    )); await fsp.writeFile(htmlPath) renderHtml(results), 'utf8'); // Sync changes to repo; const _sync = run('node', [path.join(root, 'automation') 'git-sync.cjs')]); const body = {generatedAt: new Date().toISOString()
    total: results.length ok: sync.status === 0; report: {,
