...
//   const baseUrl = `http,`
  ...
  failed: ${res.status}`);
    ...
    if (latest && data.versions && data.versionsService Feature;
...
//   const names = Object.keys(deps).sort();
  const _results = Service Feature.type) ||
        'unknown'
  ...
  e: license || 'unknown'}
      ...
      results.push(entry)} catch (e) {results.push({)
        name)
        versionWanted: depsService Feature,
        version: 'unknown'),
        license: 'unknown'),
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = Service Feature) {/* TODO: Fix JSX expression */}
  ...
  failed: ${res.status}`); const data = await res.json();' const latest = data && data['dist-tags'] && data['dist-tags'].latest; if (latest && data.versions && data.versionsService Feature; return data} catch (e) {/* TODO: Fix JSX expression */}
  ...
  e: 500} bod,`
  y: `Failed to read package.jso,`
  n: ${e}` }} const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) } const names = Object.keys(deps).sort(); const results = Service Feature)}' const license = info.license || (info.licenses && info.licensesService Feature.type) || 'unknown' const entry = {/* TODO: Fix JSX expression */}
