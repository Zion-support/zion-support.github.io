...
const cls=""
        r.classification.level === 'ok'
          ...
          : r.classification.level === 'warn'
            ...
            : 'text-rose-400'
      return `<tr>)
      <td style=""padding: 8px}border-bottom:1px solid #223}"></td>"
=======
...
const fs = require('fs');' const fsp = require('fs/promises');' const path = require('path');' const { spawnSync } = require('child_process'); function run(cmd) args = "[]) {/* TODO: Fix JSX expression */}"
  ...
  g: 8px}border-botto,"
  m:1px solid #223}">${r.name}</td>' <td style=""paddin,"></td>"
  g: 8px;border-botto,"
  m:1px solid #223,">${r.versionWanted || ''}</td>' <td style=""paddin,"></td>"
  g: 8px;border-botto,"
  m:1px solid #223,">${r.version || ''}</td> <td style=""paddin,"></td>"
  g: 8px;border-botto,"`
  ...
  y:ui-sans-serif,system-ui,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial,"Noto Sans"}sans-serif;paddin,
  g:24px} .muted{/* TODO: Fix JSX expression */}"
  r:#9aa4b2}</style> </head><body> <h1>Dependency Licenses Audit</h1> <p class="muted">Generated at ${new Date().toISOString()}</p> <table style=""border-collaps,"></table>"
  e: collapse;widt,
  h:100%;margin-to,"
  p:16px;"> <thead><tr> <th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334">Package</th> <th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334">Wanted</th> <th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px;border-botto,"
  m:2px solid #334">Resolved</th> <th style=""text-alig,"></th>"
  n:left;paddin,
  g:8px,border-botto,"`
