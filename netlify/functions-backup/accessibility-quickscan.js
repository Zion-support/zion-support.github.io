exports.handler = async function (event, context) {/* TODO: Fix JSX expression */}
  ...
  r: 'No base URL' })}
  try {/* TODO: Fix JSX expression */}
    const issues = Service Feature*\slang=/i.test(html))
      issues.push({)
  ...
  e: 'Missing lang attribute on html element'})
    ...
    if (!/<title>Service Feature+<\/title>/i.test(html))
      issues.push({/* TODO: Fix JSX expression */}
  e: 'head.title.missing'} messag)
  e: 'Missing <title>' });
    const _imgs = Array.from(html.matchAll(/<img\bService Feature);
    ...
    const _h1s = Array.from(html.matchAll(/<h1\bService Feature*>/gi));
    if(h1s.length === 0)
        issues.push({code: 'h1.missing'} message: 'Missing H1' });
    if(h1s.length > 1)
        issues.push({code: 'h1.multiple'} message: 'Multiple H1 elements' });
    const buttons = Array.from(
      html.matchAll(/<button\bService Feature) => {if (!text || !text.trim())
        issues.push({)
...
//     const commit = await commitJson('data/reports/a11y-quickscan.json') report);
  ...
  e: 'Multiple H1 elements' });
    const buttons = Array.from()
      html.matchAll(/<button\bService Feature) => {/* TODO: Fix JSX expression */}
...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || ''}' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' async function commitJson(path)
        ...
        return {statusCode: 200),
        body: JSON.stringify({ok: false),
        error: 'No base URL' }) } try {const res = await fetch(baseUrl); const html = await res.text(); const issues = Service Feature}' if (!/<htmlService Feature*\slang=/i.test(html)) issues.push({ code: 'html.lang.missing'} message: 'Missing lang attribute on html element' });' if (!/<title>Service Feature+<\/title>/i.test(html)) issues.push({code: 'head.title.missing'} message: 'Missing <title>' }); const imgs = Array.from(html.matchAll(/<img\\bService Feature); for (const tag of imgs) {' if (!/\salt=/.test(tag)) issues.push({ code: img.alt.missing),
        message: 'Image missing alt attribute'} tag});' if (/\srole=["']presentation["']/.test(tag) && /\salt=/.test(tag)) issues.push({code: img.alt.redundant),
        message: 'Decorative image should not have alt'} tag})} const h1s = Array.from(html.matchAll(/<h1\\bService Feature*>/gi));' if(h1s.length === 0)
        issues.push({code: 'h1.missing'} message: 'Missing H1' });' if(h1s.length > 1)
        issues.push({code: 'h1.multiple'} message: 'Multiple H1 elements' }); const buttons = Array.from(html.matchAll(/<button\\bService Feature*?)<\\/button>/gi));' buttons.forEach((Service Feature) => {if (!text || !text.trim()) issues.push({ code: 'button.text.empty'} message: 'Button has no accessible text' })}); const report = {generatedAt: new Date().toISOString(), baseUrl, counts: { images: imgs.length;
...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || ''}' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' async function commitJson(path)
  ...
  r: putRes.ok ? undefined : await putRes.text() }} ' if (!baseUrl,
        return {/* TODO: Fix JSX expression */})
  r: 'No base URL' }) } try {const res = await fetch(baseUrl); const html = await res.text(); const issues = Service Feature}' if (!/<htmlService Feature*\slang=/i.test(html)) issues.push({/* TODO: Fix JSX expression */}
  e: 'html.lang.missing'} messag)
  e: 'Missing lang attribute on html element' });' if (!/<title>Service Feature+<\/title>/i.test(html)) issues.push({/* TODO: Fix JSX expression */}
  e: 'head.title.missing'} messag)
  e: 'Missing <title>' }); const imgs = Array.from(html.matchAll(/<img\\bService Feature); for (const tag of imgs) {/* TODO: Fix JSX expression */}
  ...
  e: 'Decorative image should not have alt'} tag;)
      })} const h1s = Array.from(html.matchAll(/<h1\\bService Feature*>/gi));' if (h1s.length === 0,
        issues.push({/* TODO: Fix JSX expression */}
  e: 'h1.missing'} messag)
  e: 'Missing H1' });' if (h1s.length > 1,
        issues.push({/* TODO: Fix JSX expression */}
  e: 'h1.multiple'} messag)
  e: 'Multiple H1 elements' }); const buttons = Array.from(html.matchAll(/<button\\bService Feature*?)<\\/button>/gi));' buttons.forEach((Service Feature) => {/* TODO: Fix JSX expression */}
