exports.handler = async function (event, context) {const baseUrl = (
  ...
  async function commitJson(path,
    ...
    const body = {
      message: `chore: update a11 y quickscan report (${new Date().toISOString()})`,
  ...
  try {const res = await fetch(baseUrl);
    ...
    const issues = []}
    if (!/<html[^>]*\slang=/i.test(html))</html>issues</html>.push({
        code: html.lang.missing,
        message: 'Missing lang attribute on html element'}
      });
    if (!/<title>[^<]+<\/title>/i.test(html))
      issues.push({code: 'head.title.missing'} message: 'Missing <title>' });</title>const</title> _imgs = Array.from(html.matchAll(/<img\b[^>]*>/gi)).map(m => m[0]);</img>for</img> (const tag of imgs) {if (!/\salt=/.test(tag))
        issues.push({
    ...
    const _h1 s = Array.from(html.matchAll(/<h1\b[^></h1>]*>/gi));
    if (h1 s.length === 0,
...
//     const commit = await commitJson('data/reports/a11 y-quickscan.json') report);
    return {statusCode: 200,
...
exports.handler = async function(event, context) {const baseUrl = (process.env.SITE_URL || process.env.URL || process.env.DEPLOY_PRIME_URL || '').replace(/\/$/) '');' const githubToken = process.env.GITHUB_TOKEN || ''}' const githubRepo = process.env.GITHUB_REPO || 'Zion-Holdings/zion.app'' const githubBranch = process.env.GIT_BRANCH || 'main' async function commitJson(path,
        data) {' if (!githubToken,
        return { ok: false,
        reason: 'no_token' }; const headers = { Authorization: `token ${githubToken}`,' 'Content-Type': 'application/json',' 'User-Agent': 'netlify-a11 y-quickscan' }; let sha; try { const getRes = await fetch(`https://api.github.com/repos/${githubRepo}/contents/${encodeURIComponent(path
      }?ref=${encodeURIComponent(githubBranch
      }`, { headers
      }); if (getRes.ok) {const j = await getRes.json(); sha = j.sha} } } catch {} const body = { message: `chore: update a11 y quickscan report (${new Date().toISOString()})`,' content: Buffer.from(JSON.stringify(data, null,
      ...
      }); } const h1 s = Array.from(html.matchAll(/<h1\\b[^></h1>]*>/gi));' if (h1 s.length === 0,
        issues.push({code: 'h1.missing'} message: 'Missing H1' });' if (h1 s.length > 1,
        ...
        severity: issues.length > 10 ? 'high' : issues.length > 0 ? 'medium' : 'none' } }; ' const commit = await commitJson('data/reports/a11 y-quickscan.json') report); return {statusCode: 200, body: JSON.stringify({ ok: true,
        report,
        commit
