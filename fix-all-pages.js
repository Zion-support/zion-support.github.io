...
async function main() {
  ...
  for (const file of files) {
    try {
      ...
      if (file.includes('components/') || file.includes('pages/') || file.includes('layout') || file.includes('globals') || file.includes('about/page.tsx') || file.includes('accessibility/page.tsx')) {
        continue;
      }
      
      const content = fs.readFileSync(file, 'utf8');
      
      // Check if file has JSX errors or is malformed
      if (content.includes('has no corresponding closing tag') || 
  ...
  console.log('All pages fixing completed!');
}
