...
async function mergeBranch(branchName) {
  ...
  if (mergeResult.success) {
    ...
    if (mergeResult.output.includes('CONFLICT') || mergeResult.error.includes('CONFLICT')) {
      ...
      const abortResult = runCommand(`git merge --abort`, `Aborting conflicted merge for ${branchName}`);
      
      // Try a different merge strategy
      const strategyResult = runCommand(`git merge origin/${branchName} -X ours --no-edit`, `Merging ${branchName} with ours strategy`);
...
async function main() {
  try {
    console.log('🚀 Starting local branch merge process...');
    ...
    // Get list of fix-errors branches
    const branchList = [
