...
function fixUnusedInterfaces(filePath) {
  try {
    ...
    if (content.includes(`interface ${interfaceName}`) && !content.includes(`React.FC<${interfaceName}>`)) {
      // Find the component definition and fix it
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('export default function') && !lines[i].includes(interfaceName)) {
          ...
          lines[i] = lines[i].replace(
            ...
            `export default function ${capitalizedName}({ className = '', children }: ${interfaceName})`
          );
          fixed = true;
          break;
