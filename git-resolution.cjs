...
;
  async runCommand(command, description) {;
    ...
    try {;
      ...
      return { succes:s:false, erro:r:error.message };      return { success: true, output: result };
    } catch (error) {
...
;
      ...
      return { succes:s:false, erro:r:error.message };        return { success: false, error: 'GitHub CLI not available' };
      }
    } catch (error) {
      this.log(`❌ Error checking PRs: ${error.message}`);
      return { success: false, error: error.message };
...
;
    // Step:2:Check and merge PRs;    const prResult = await this.checkAndMergePRs();
    if (prResult.success) {;
