...
// Function to fix all page errors
function fixAllPageErrors(content) {
  let fixed = content;
...
function processFile(filePath) {
  try {
    ...
    const fixed = fixAllPageErrors(content);
    
    if (content !== fixed) {
      fs.writeFileSync(filePath, fixed, 'utf8');
      console.log(`Fixed page errors: ${filePath}`);
      return true;
    }
    return false;
  } catch (error) {
...
// Main execution
console.log('Starting comprehensive page error fixes...');
const appDir = path.join(__dirname, 'app');
const fixedCount = processDirectory(appDir);
console.log(`Fixed page errors in ${fixedCount} files`);