...
function fixSyntaxErrors(text) {
  ...
  fixed = fixed.replace(/import\s*\{([^}]+)\}\s*from\s*['"]([^'"]+)['"];?/g, (match, imports, module) => {
    ...
    const cleanImports = imports.replace(/,\s*,/g, ',').replace(/\s*,\s*/g, ', ').trim();
    return `import { ${cleanImports} } from '${module}';`;
  });
  
  // Fix React import statements
  fixed = fixed.replace(/import\s*React,\s*\{([^}]+)\}\s*from\s*['"]react['"];?/g, (match, imports) => {
    const cleanImports = imports.replace(/,\s*,/g, ',').replace(/\s*,\s*/g, ', ').trim();
    return `import React, { ${cleanImports} } from 'react';`;
  });
  
  // Fix function declarations that got corrupted
  fixed = fixed.replace(/export\s*default\s*function\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\(/g, 'export default function $1(');
  fixed = fixed.replace(/const\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*\(/g, 'const $1 = (');
  fixed = fixed.replace(/function\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\(/g, 'function $1(');
  ...
  fixed = fixed.replace(/const\s*([A-Z][a-zA-Z0-9_$]*)\s*:\s*React\.FC/g, 'const $1: React.FC');
  fixed = fixed.replace(/const\s*([A-Z][a-zA-Z0-9_$]*)\s*=\s*\(/g, 'const $1 = (');
  
  // Fix object property syntax
  fixed = fixed.replace(/\{\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*([^,}]+),\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*([^,}]+)\s*\}/g, '{ $1: $2, $3: $4 }');
  ...
  // Fix for loops
  fixed = fixed.replace(/for\s*\(\s*([^;]+);\s*([^;]+);\s*([^)]+)\s*\)\s*\{/g, 'for ($1; $2; $3) {');
  ...
  fixed = fixed.replace(/class\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*extends\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\{/g, 'class $1 extends $2 {');
