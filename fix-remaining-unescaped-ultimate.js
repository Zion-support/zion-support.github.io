#!/usr/bin/env node;
const fs = require('fs')const path  = require('path')function fixUnescapedEntities() {try {let content = fs.readFileSync(filePath, 'utf8')let modified  = false;// Replace ' with single quotes;
    if (content.includes(''')) {content = content.replace(/'/g, "'")modified = true;
    }// Replace " with double quotes;
    if (content.includes('"')) {content = content.replace(/"/g, '"')modified = true;
    }if (modified) {fs.writeFileSync(filePath, content, 'utf8')console.log(`Fixed unescaped entities in: ${filePath}`)return true;
    }return false;
