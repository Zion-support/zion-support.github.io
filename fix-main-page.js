...
function fixMainPage() {
  try {
    ...
    content = content.replace(/<div(Service Feature/g, '<div$1>');
    
    // Fix specific patterns that are common issues
    content = content.replace(/<div(Service Feature*?)\s*\/>\s*\n\s*<div/g, '<div$1>\n<div');
    content = content.replace(/<div(Service Feature*?)\s*\/>\s*\n\s*<h/g, '<div$1>\n<h');
    content = content.replace(/<div(Service Feature*?)\s*\/>\s*\n\s*<p/g, '<div$1>\n<p');
    content = content.replace(/<div(Service Feature*?)\s*\/>\s*\n\s*<button/g, '<div$1>\n<button');
    content = content.replace(/<div(Service Feature*?)\s*\/>\s*\n\s*<span/g, '<div$1>\n<span');
    ...
    content = content.replace(/map\((Service Feature*)\)\s*=>\s*\(\s*<\/div>/g, 'map($1) => (');
