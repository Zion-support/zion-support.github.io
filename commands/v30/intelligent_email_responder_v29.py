...
# ② Select template + compose
        ...
        # IDEAS-5: partial-reply minimizer
        _is_partial = self._detect_partial_reply(body)
        if _is_partial and not dry_run and tid:
            _thread_text = self._get_thread_recent(tid)
            if _thread_text:
                body = f"{body}\n\n—\nContinuing from thread:\n{_thread_text}"
                self.stats['partial_reply_patched'] = self.stats.get('partial_reply_patched', 0) + 1
            else:
                self.stats['partial_reply_skipped'] = self.stats.get('partial_reply_skipped', 0) + 1
    ...
    def _full_pipeline(self, email, intent_raw, intent_label, intent_cat,
        ...
        # IDEAS-5: partial-reply minimizer (full-path)
        _is_partial_fp = self._detect_partial_reply(body)
        if _is_partial_fp and not dry_run and tid:
            _thread_text_fp = self._get_thread_recent(tid)
            if _thread_text_fp:
                body = f"{body}\n\n—\nContinuing from thread:\n{_thread_text_fp}"
                self.stats['partial_reply_patched'] = self.stats.get('partial_reply_patched', 0) + 1
            else:
        ...
        result = add_to_result(email, {"action": "send", "intent": intent_label,
                ...
                "quality": qc, "elapsed_ms": ms(), "fast_path": False,
                "wave5_cc": _wave5_cc_candidates, "kb_ctx": bool(kb_ctx),
                "partial_reply_patched": self.stats.get("partial_reply_patched", 0)})
    ...
    # ── Helpers ────────────────────────────────────────────────
    def _detect_partial_reply(self, body: str) -> bool:
        """IDEAS-5: return True if body looks like a partial/incomplete reply."""
        ...
        try:
            ...
            _substantive = len(_words) > 30 and any(p in _low for p in _phrases)
            return _substantive
        except Exception:
            return False

    def _get_thread_recent(self, thread_id: str, max_msgs: int = 3,
                           max_total_words: int = 500) -> str:
        """IDEAS-5: pull last N messages from thread via gmail_get."""
        try:
            from google_workspace import gmail_get as _gmail_get
            _info = _gmail_get(thread_id)
            _msgs = _info.get('messages', [])
            if not _msgs:
                return ''
            _recent = _msgs[-max_msgs:]
            _parts = []
            _total = 0
            import re as _re_thr
            for _m in _recent:
                _b = _m.get('body', '') or _m.get('snippet', '')
                _w = _re_thr.findall(r'\w+', _b)
                if len(_w) + _total > max_total_words:
                    break
                _parts.append(_b.strip())
                _total += len(_w)
            return '\n---\n'.join(_parts)
        except Exception:
            return ''
