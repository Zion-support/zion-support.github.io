...
try:
    ...
    check("1b_winner_detected", w.get("winner") in ("A", "B"))
...
try:
    ...
    check("2a_vip_urgent_score", r1["score"] >= 80)
    ...
    check("2b_unknown_low_score", r2["score"] <= 50)
    db.unlink()
except Exception as e:
...
try:
    ...
    check("3b_bump_works", v2["version"] == 2)
...
try:
    ...
    check("4a_pt_subject", detect_lang("Olá, preciso de ajuda") == "pt")
...
try:
    ...
    h = check_health(log_path, window_hours=24)
    check("5a_healthy_empty", h.get("status") in ("no_log", "no_recent_entries", "checked"))

    now = datetime.now(timezone.utc)
    lines = [json.dumps({"ts": now.isoformat(), "action": "escalated"}) for _ in range(8)]
    lines += [json.dumps({"ts": now.isoformat(), "action": "sent"}) for _ in range(32)]
    log_path.write_text("\n".join(lines) + "\n")
    h2 = check_health(log_path, window_hours=1)
