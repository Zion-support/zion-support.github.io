...
class Batch4Merge {
  ...
  async processBranch(branchName) {
    try {
      ...
      try {
        await this.runCommand(`git merge origin/${branchName} --no-ff -m "Merge ${branchName} into main"`, `Merging ${branchName}`);
        ...
        this.mergedBranches.push(branchName);
        this.log(`✅ Successfully merged ${branchName} with conflicts resolved`, 'success');
      }
      ...
      try {
        ...
        await this.runCommand('git reset --hard HEAD', 'Resetting to clean state');
      } catch (checkoutError) {
  ...
  async run() {
    try {
      ...
      for (const branch of branches) {
        ...
        try {
          await this.runCommand('git push origin main', 'Pushing changes to main');
        } catch (pushError) {
          this.log(`Warning: Could not push changes: ${pushError.message}`, 'warning');
        }
      }
