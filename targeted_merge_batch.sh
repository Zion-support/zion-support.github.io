...
# Targeted merge script for processing branches in smaller batches
set -e
...
# Process Netlify fix branches first
log_info "Processing Netlify fix branches..."
for branch in $(git branch -r | grep "cursor/fix-netlify" | head -20 | sed 's/origin\///'); do
    merge_single_branch "$branch"
    sleep 1
done

# Process recent cursor branches
log_info "Processing recent cursor branches..."
for branch in $(git branch -r | grep "cursor/" | grep -v "fix-netlify" | head -30 | sed 's/origin\///'); do
    merge_single_branch "$branch"
    sleep 1
done

# Process recent codex branches
log_info "Processing recent codex branches..."
for branch in $(git branch -r | grep "codex/" | head -20 | sed 's/origin\///'); do
    merge_single_branch "$branch"
    sleep 1
done

# Process recent chore branches
log_info "Processing recent chore branches..."
for branch in $(git branch -r | grep "chore/" | head -20 | sed 's/origin\///'); do
    merge_single_branch "$branch"
    sleep 1
done
...
git add . 2>/dev/null || true
git commit -m "Batch merge: processed recent branches - $(date)" 2>/dev/null || true
...
log_success "🎉 Targeted merge process completed!"
