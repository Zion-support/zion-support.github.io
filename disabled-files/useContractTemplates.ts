...
try {;
setLoading(true), // Simulate API call, await new Promise(resolve => setTimeout(resolve, 1000));
...
setError(null);
} catch (err) {setError('Failed to fetch contract templates");
} finally {
setLoading(false);
}
};
...
const lowercaseQuery = query.toLowerCase(), return templates.filter(template =>, template.name.toLowerCase().includes(lowercaseQuery) ||;
