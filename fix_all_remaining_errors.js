...
function fixFile(filePath) {
  try {
    ...
    content = content.replace(/;\s*}\s*from\s*['"][^'"]+['"];?/g, ' } from \'$&\'.replace(/;\\s*}/, \'}\').replace(/from\\s*[\'"]/, \'from \').replace(/[\'"];?$/, \'\')');
    content = content.replace(/;\s*}\s*from\s*['"][^'"]+['"];?/g, ' } from \'$&\'.replace(/;\\s*}/, \'}\').replace(/from\\s*[\'"]/, \'from \').replace(/[\'"];?$/, \'\')');
    
    // Fix malformed import statements - more specific
    content = content.replace(/(\w+);\s*}\s*from\s*['"][^'"]+['"];?/g, '$1 } from \'$&\'.replace(/;\\s*}/, \'}\').replace(/from\\s*[\'"]/, \'from \').replace(/[\'"];?$/, \'\')');
