...
function fixAllSyntaxErrors(content) {
  // Fix malformed variable destructuring
  content = content.replace(/const \[([^,\]]+)([^,\]]+)\] = useState/g, 'const [$1, $2] = useState');
  content = content.replace(/const \[([^,\]]+)([^,\]]+)\] = useState/g, 'const [$1, $2] = useState');
  
  // Fix malformed function calls
  content = content.replace(/useState<Set<number>>\(newSet\(\)\)/g, 'useState<Set<number>>(new Set())');
  
  // Fix malformed useEffect
  content = content.replace(/useEffect\(\(\) => \{\s*setIsVisible\(true\)\}\[\]\);/g, 'useEffect(() => {\n\t\tsetIsVisible(true);\n\t}, []);');
  
  // Fix malformed JSX return statements
  content = content.replace(/return \(\s*<div/g, 'return (\n\t\t<div');
  
  // Fix malformed switch cases
  content = content.replace(/case "([^"]+)":\s*return \(\s*<div/g, 'case "$1":\n\t\t\t\treturn (\n\t\t\t\t\t<div');
  
  // Fix malformed comments
  content = content.replace(/\/\/ import = \{ ErrorBoundary   \} from/g, '// import { ErrorBoundary } from');
