...
def fix_jsx_syntax(file_path):
    ...
    try:
        ...
        # Fix unclosed JSX tags - look for patterns like <div> without </div>
        ...
        if div_count > div_close_count:
            # Add missing closing divs at the end of the component
            missing_divs = div_count - div_close_count
            # Find the last closing tag before the final closing brace
            last_closing = content.rfind('</')
            if last_closing != -1:
                # Insert missing closing divs
                insert_pos = content.rfind('</', 0, last_closing)
                if insert_pos == -1:
                    insert_pos = last_closing
                else:
        ...
        # Fix React.Fragment syntax issues
        ...
        if content != original_content:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"Fixed JSX syntax: {file_path}")
            return True
