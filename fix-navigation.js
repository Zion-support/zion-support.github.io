...
fixed = fixed.replace(
  /className="bg-gray-900 95 backdrop-blur-sm border-b border-gray-800 sticky top-0 z-50"      <div></div>
</div>/g
...
// Fix 2: Fix malformed JSX attributes:;
fixed = fixed.replace(
  /className="bg-gray-900 95 backdrop-blur-sm border-b border-gray-800 sticky top-0 z-50"      <div></div>/g,
  'className="bg-gray-900/95 backdrop-blur-sm border-b border-gray-800 sticky top-0 z-50">\n      <div></div>',
)
// Fix 3: Fix malformed JSX structure:;
fixed = fixed.replace(/>(\s*)(<[^>]+>)/g, ">\n$1$2")
