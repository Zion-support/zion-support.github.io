...
function fixSpecificFileIssues(filePath, content) {
  ...
  if (content.includes("react-router-dom")) {
    ...
    content = content.replace(/import\s+{\s*useLocation\s*}\s+from\s+'react-router-dom';/g, "import { useRouter } from 'next/router';");
  }
    ...
    files.forEach(file => {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          if (content.includes("';'import") || content.includes("';'const") || content.includes("';'interface")) {
            console.log(`Found additional file with syntax errors: ${filePath}`);
            fixFile(filePath);
          }
        } catch (error) {
