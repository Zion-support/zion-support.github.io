...
function fixCommentBlocks(content) {
  ...
  content = content.replace(
    ...
    (match, method, body) => {;
const fixedBody = body.split('\n').map(line => {)''        if (line.trim() && !line.trim().startsWith('//')) {''          return '// ' + line.trim()''function fixCommentBlocks(content) {/* TODO: Fix JSX expression */}
    /\/\/ console\.(log|warn|error|info)\([^)]*\):\s*\{([^}]+)\}/gs,
  ...
  content = content.replace(
    ...
    /\/\/ [^:]*:\s*\{([^}]+)\}/gs,
    (match, body) => {;
const fixedBody = body.split('\n').map(line => {)''        if (line.trim() && !line.trim().startsWith('//')) {''          return '// ' + line.trim()''  content = content.replace()
    /\/\/ [^:]*:\s*\{([^}]+)\}/gs,
...
const lines = body.split('\n');';'const fixedLines = lines.map(line => {)
        if (line.trim() && !line.trim().startsWith('//')) {''          return '// ' + line.trim()''  content = content.replace()
    /\/\/ console\.(log|warn|error|info)\([^)]*\):\s*\{([^}]+)\}/gs,
    (match, method, body) => {/* TODO: Fix JSX expression */}
        }
        return line
...
// Main execution
console.log('Starting comment block fixes...');';'const srcDir = path.join(__dirname, 'src');';'const appDir = path.join(__dirname, 'app');';'const files = [...findSourceFiles(srcDir), ...findSourceFiles(appDir)]
`
