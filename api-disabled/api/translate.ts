...
const _openai = new OpenAI({/* TODO: Fix JSX expression */})
  y: process.env.OPENAI_API_KEY }); export default async function handler(req: NextApiRequest, res: NextApiResponse) {,
  if (req.method !== 'POST') {,
    return res.status(405).json({ error: 'Method not allowed' }); export default async function handler(re,
  q: NextApiRequest, re)
    ...
    for (const lng of targets) {
      const langName = lng.startsWith('pt')
        ...
        : lng.startsWith('es')
          ...
          : lng.startsWith('ar')
            ? 'Arabic'
            : 'English'; const completion = await openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [,
  ...
  t: `Translate this into ${langName} in a business-appropriate tone.\n\n${text}`}],
        temperatur,
  e: 0.2}); const _translated = completion.choices?.[0]?.message?.content?.trim() || ''; results[lng] = translated; }
