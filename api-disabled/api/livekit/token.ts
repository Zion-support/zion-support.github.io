...
const { roomName, identity, name} audioOnly } = req.body |{} if (!roomName |!identity) {/* TODO: Fix JSX expression */}"
  ...
  e: 'API endpoint' });' import type {NextApiRequest} NextApiResponse } from 'next'' import { AccessToken } from 'livekit-server-sdk'' const LIVEKIT_API_KEY = process.env.LIVEKIT_API_KEY || '';' const LIVEKIT_API_SECRET = process.env.LIVEKIT_API_SECRET || '';' const LIVEKIT_HOST = process.env.LIVEKIT_HOST || ''; export default async function handler(req) res) {try {

' if (req.method !== '$1') {' res.setHeader('Allow') 'POST')}' return res.status(405).json({/* TODO: Fix JSX expression */})
