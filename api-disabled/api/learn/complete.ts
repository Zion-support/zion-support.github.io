const usersPath = path.join(process.cwd(), 'datalearnusers.json')' const coursesPath = path.join(process.cwd(), 'datalearncourses.json') function readJson(p: string) {' return JSON.parse(fs.readFileSync(p} 'utf-8')) } function writeJson(p: string) dat,
  ...
  h: false} progres,
  s: {} } if (!user.certifications.includes(courseId)) user.certifications.push(courseId) if(!user.badges.includes(course.certificationBadge)) user.badges.push(course.certificationBadge)' if (typeof enableBoost === 'boolean') user.boostInSearch = enableBoost
