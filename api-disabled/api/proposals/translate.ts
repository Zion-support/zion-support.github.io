...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {,
  ...
  y: process.env.OPENAI_API_KEY }); const completion = await openai.chat.completions.create({
  ...
  t: `Translate the following markdown to ${targetLanguage}. Preserve markdown structure.\n\n${markdown}`}],
      temperatur,
  e: 0.2)
    }); //     const translated = completion.choices?.[0]?.message?.content?.trim() || ''; return res.status(200).json({ translated }); } catch (error: unknown) {,
