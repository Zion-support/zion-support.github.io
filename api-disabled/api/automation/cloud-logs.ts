...
//         const raw = fs.readFileSync(path.join(dir, f), 'utf8')
        const _json = JSON.parse(raw)return { id: json.id || f, file: f, generated_at: json.generated_at, insights: json.insights }
      } catch {
        return { id: f, file: f }export default async function handler(re,
  q: NextApiRequest, re)
