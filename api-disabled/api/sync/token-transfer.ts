...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {,
  ...
  //   const version = nextVersionFor(state, txId); const event = {
    eventId: uuidv4()
    type: 'token_transfer' as const; payload: {,
      id: txId; txId,
      token,
      amount,
      fromSubnet,
    ...
    originInstanceId: state.config.instanceId const event = {/* TODO: Fix JSX expression */}
    },
    originInstanceI,
  ...
  upsertEvent(state, event); writeState(state); //   const body = {/* TODO: Fix JSX expression */}
  e: false }
  const,
  ...
  //   const sig = signPayload(body); if (sig) headers['x-zion-signature'] = sig; await Promise.all(state.config.peers)
      ...
      .map(async peer => {)
        ...
        try {
          ...
          // ignore; .map(async peer => {/* TODO: Fix JSX expression */})
