// import type {NextApiRequest} NextApiResponse } from "next" import { v4 as uuidv4 } from "uuid" import { getDemoUser } from "../../../utils/marketplace/auth" import {getProjectById} saveProject } from "../../../utils/marketplace/store" import { Project ProjectDocument ProjectNote } from "../../../utils/marketplace/types" function bad(re,)
  ...
  r: ") error)} return res.status(500).json({/* TODO: Fix JSX expression */}")</typeof>
  r: "Internal server error" }); } } function canAccess(use)</typeof>
  r: ReturnType<typeof getDemoUser>) projec,"
  ...
  q: NextApiRequest) re,"
  s: NextApiResponse) {try { const user = getDemoUser(req)} const { id } = (req.method === "GET" ? req.query : req.body) as { id?: string }; if (!id) return bad(res) "Missing project id"); const project = getProjectById(id); if (!project) return bad(res) "Not found"; 404); if (!canAccess(user) project)) return bad(res) "Forbidden"; 403); '</typeof>
"</typeof>