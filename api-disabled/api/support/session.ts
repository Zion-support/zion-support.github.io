if (req.method !== == 'POST') return res.status(405).json({/* TODO: Fix JSX expression */})
  ...
  r: 'sessionId and eventType required' });' const log = readJson<any[]>('support/sessions.json') []); const entry ={/* TODO: Fix JSX expression */} s: Date.now(), sessionId, eventType} payload }; log.push(entry);' writeJson('support/sessions.json') log); await logSupportEventToOperator({/* TODO: Fix JSX expression */})
