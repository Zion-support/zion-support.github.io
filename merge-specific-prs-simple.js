...
const branches = execSync(
  'git branch -r | grep "enhance-and-expand-ziontechgroup-com-services-and-site" | tail -3',
  { encoding: 'utf8' },
)
  .split('\n')
...
function safeMerge(branchName) {
  try {
    ...
    try {
      ...
      execSync(
        `git merge origin/${branchName} -X theirs --no-ff -m "Auto-merge ${branchName} into main"`,
        { stdio: 'inherit' },
      );
