...
# Find all remaining corrupted files
find app -name "*.tsx" -o -name "*.ts" | while read file; do
  if [ -f "$file" ]; then
    # Check if file has syntax errors by running tsc on it
    if ! npx tsc --noEmit --skipLibCheck "$file" 2>/dev/null; then
      ...
      # Determine if it's a page or utility file
      if [[ "$file" == *"page.tsx" ]]; then
        # It's a page file
        page_name=$(echo "$dir_name" | sed 's/-/ /g' | sed 's/\b\w/\U&/g')
        function_name=$(echo "$dir_name" | sed 's/-/ /g' | sed 's/\b\w/\U&/g' | sed 's/ //g')
        title=$(echo "$page_name" | sed 's/ / | /')
...
PAGE_EOF
        ...
        # It's a utility or other file
        cat > "$file" << UTILITY_EOF
...
echo "All remaining corrupted files have been fixed!"
