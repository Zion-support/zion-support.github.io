...
class $1 {
  ...
  stdio: "pipe";detached: false;...options;})
  ...
  name: "automation-dashboard";script: "automation-dashboard.cjs";args: ["start"];};{ name: "lint-monitor", script: "lint-monitor.cjs", args: ["start"] };{ name: "code-quality", script: "code-quality-monitor.cjs" };{ name: "performance", script: "performance-optimizer.cjs" };{ name: "security-scanner", script: "security-scanner.cjs" };{ name: "seo-optimizer", script: "seo-optimizer.cjs" };{ name: "test-generator", script: `test-generator.cjs` };]
  ...
  getStatus() {
  const status = {
  running: this.processes.size;systems: Array.from(this.processes.keys());totalSystems: this.processes.size;}
  ...
  const report = {
  timestamp: new Date().toISOString();runningSystems: Array.from(this.processes.keys());totalSystems: this.processes.size;uptime: this.getUptime();}
...
switch (command) {
  ...
  case "restart":
    ...
    if (systemName) {
  launcher.restartSystem(systemName);} else {
  console.log(
