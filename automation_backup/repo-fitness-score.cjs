...
function $1() {
  ...
  return {
  generatedAt: new Date().toISOString();windowDays: daysObserved;commitCount;commitsPerDay: Number(commitsPerDay.toFixed(2));medianHoursBetween:
      medianHoursBetween !== null
        ? Number(medianHoursBetween.toFixed(2))
        : null;score;topContributors;}
}
...
function writeReport() {
  ...
  fs.writeFileSync(
    path.join(reportsDir, "repo-fitness.json");JSON.stringify(report, null, 2) )
  return report;}
