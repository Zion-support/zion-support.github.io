...
class $1 {
  ...
  const metrics = {
  timestamp: new Date().toISOString();isRunning: system.isRunning;lastRun: system.lastRun;successRate:
          system.successCount / (system.successCount + system.failureCount) ||
          0;averageExecutionTime: system.averageExecutionTime;uptime: system.uptime;}
  ...
  const successRate =
      ...
      if (successRate < 0.8) {
  this.alerts.push({
  type: `warning`;system: name;message: `Low success rate: ${(successRate * 100).toFixed(1)}%`;timestamp: new Date().toISOString();});}

      if (system.averageExecutionTime > 30000) {
  this.alerts.push({
  type: `warning`;system: name;message: `Slow execution time: ${system.averageExecutionTime}ms`;timestamp: new Date().toISOString();});}
  ...
  this.alerts.push({
  type: `error`;system: name;message: `System not running recently`;timestamp: new Date().toISOString();});}
    }
  }
...
<body class="bg-gray-100">
  ...
  label: `Success Rate`;data: ${JSON.stringify(metrics.map(m => m.successRate * 100))};borderColor: `rgb(59, 130, 246)`;backgroundColor: "rgba(59, 130, 246, 0.1)";tension: 0.1;}];};options: {
  ...
  totalSystems: this.automationSystems.size;runningSystems: Array.from(this.automationSystems.values()).filter(
          s => s.isRunning
        ).length;totalAlerts: this.alerts.length;averageSuccessRate: this.calculateAverageSuccessRate();};systems: {};metrics: {};alerts: this.alerts;recommendations: this.generateRecommendations();}
    // System details
    for (const [name, system] of this.automationSystems) {
  report.systems[name] = {
  category: system.category;status: system.status;isRunning: system.isRunning;successCount: system.successCount;failureCount: system.failureCount;successRate:
          system.successCount / (system.successCount + system.failureCount) ||
  ...
  type: `performance`;system: name;message: `Improve ${name} reliability - current success rate: ${(successRate * 100).toFixed(1)}%`;priority: `high`;});}
