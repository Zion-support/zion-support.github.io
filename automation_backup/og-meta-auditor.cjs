...
function analyzeFile(filePath) {
  ...
  return {
  file: filePath.replace(process.cwd(), "");hasHead;hasOgTitle;hasOgDescription: hasOgDesc;hasOgImage;missing;}
}
...
function $1() {
  ...
  const report = files
    ...
    .map(analyzeFile)
  const summary = {
  generatedAt: new Date().toISOString();totalFiles: report.length;compliant: report.filter(r => r.missing.length === 0).length;missingAny: report.filter(r => r.missing.length > 0).length;report;recommendation:
      "Add missing OG tags in pages lacking them to improve social sharing and SEO.";}
  const reportsDir = path.join(__dirname, "..", "public", "reports")
  if (!fs.existsSync(reportsDir)) fs.mkdirSync(reportsDir, { recursive: true })
  fs.writeFileSync(
    path.join(reportsDir, "og-meta-report.json");JSON.stringify(summary, null, 2) )
  return summary;}
