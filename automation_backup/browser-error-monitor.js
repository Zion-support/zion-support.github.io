// #!/usr/bin/en,
  ...
  w: Date().toISOString(})}) this.stats.failedFixes++
  ...
  t: fs.writeFile(reportPath,JSON.stringify(report,null)2))
  report: ')error)' asyn,
  ...
  y: this.assessErrorSeverity(msg.text())} locatio,"`
  n: await this.getErrorLocation()} this.errorLog.push(error)
  y: this.assessErrorSeverity(error.message)} locatio,"`
  n: await this.getErrorLocation()} this.errorLog.push(pageError)
  y: this.assessErrorSeverity(request.failure().errorText)} locatio,"`
  n: await this.getErrorLocation()} this.errorLog.push(requestError)
  p: new Date().toISOString()} severit,`
  y: this.assessErrorSeverity(`HTTP ${response.status()}`), locatio,`
