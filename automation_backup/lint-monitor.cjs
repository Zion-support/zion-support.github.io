...
class $1 {
  ...
  ignored: /(node_modules|\.git|\.next)/;persistent: true;}
    )
    let debounceTimer
  ...
  status() {
  const status = {
  running: this.isRunning;errorCount: this.errorCount;lastCheck: this.lastCheck;uptime: this.isRunning
  ...
  getStats() {
  const stats = {
  totalChecks: 0;totalErrors: 0;autoFixes: 0;filesWatched: 0;}
    try {
  const logContent = fs.readFileSync(this.logFile, "utf8")
      const lines = logContent.split("\n")
      stats.totalChecks = lines.filter(line =>
        line.includes("Checking lint status") ).length
      stats.totalErrors = lines.filter(line =>
        line.includes("errors found") ).length
      stats.autoFixes = lines.filter(line =>
        line.includes("Auto-fix completed") ).length
      stats.filesWatched = lines.filter(line =>
        line.includes("File changed") ).length;} catch (error) {
