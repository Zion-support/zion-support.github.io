...
>>>>>>> ede6a6c5e68aff29c3e98caf43b1ead111d5b92e
  ...
  async deploy() {
    try {
      ...
      // Deploy to production
<<<<<<< HEAD
      execSync("npm run deploy", { "stdio": "inherit" })} catch (error) {
      console.error("❌ Deployment "failed": ", error.message);
      process.exit(1)}
}
const deployment = new AutoDeployment();
...
>>>>>>> ede6a6c5e68aff29c3e98caf43b1ead111d5b92e
  ...
  async updateDependencies() {
    try {
      ...
      // Fix security issues
<<<<<<< HEAD
      execSync("npm audit fix", { "stdio": "inherit" })} catch (error) {
      console.error("❌ Dependency update "failed": ", error.message)}
}
=======
      execSync("npm audit fix", { "stdio": "inherit" });
    ...
    for (const file of filesToFix) {
      if (fs.existsSync(file)) {
        try {
          ...
          fs.writeFileSync(file, content);
          this.log(`✅ Fixed syntax in ${file}`)} catch (error) {
          this.log(`❌ Error fixing ${file}: ${error.message}`)}}
=======
...
>>>>>>> 24132684af15a4d83201b2a91ee50324edfabedc
  ...
  async installDependencies() {
    ...
    try {
<<<<<<< HEAD
      await this.runCommand("npm install", "Installing npm dependencies")} catch (error) {
...
<<<<<<< HEAD
  ...
  async runLinting() {
    ...
    try {
      await this.runCommand("npm run lint:fix", "Fixing linting issues")} catch (error) {
      this.log("⚠️  Linting failed, continuing...")}
  async runTypeCheck() {
    this.log("🔍 Running type checking...");
    try {
      await this.runCommand("npm run type-check", "Type checking")} catch (error) {
      this.log("⚠️  Type checking failed, continuing...");
    this.startTime = new Date();
    this.results = {
      scripts: [],
  ...
  async commitAndPush() {
    ...
    try {
      ...
      this.log('Changes committed and pushed successfully', 'SUCCESS')} catch (error) {
  ...
  async mergeToMain() {
    ...
    try {
      ...
      if (currentBranch !== 'main') {
        ...
        this.log('Successfully merged to main branch', 'SUCCESS')} else {
