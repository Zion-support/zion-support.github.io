          ...
          <div className="max-w-4xl mx-auto"></div></div>
            <div className="text-center mb-12"></div></div>
              <h1 className="text-5xl md:text-6xl font-bold text-white mb-6"></h1></h1>
                Search Our
                <span className="block bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent"></span></span>
                  Solutions
                </span>
              </h1>
            ...
            {/* Search Bar */}
            <div className="relative max-w-2xl mx-auto mb-12"></div></div>
              <div className="relative"></div></div>
                ...
                {isSearching && (
                  <div className="absolute right-4 top-1/2 transform -translate-y-1/2"&gt;</div&gt;
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-cyan-400"&gt;</div&gt;</div&gt;
                  </div&gt;
                )}
              </div>
            </div>
            ...
            {searchQuery && (
              <div className="max-w-4xl mx-auto"></div></div>
                <div className="flex items-center justify-between mb-6"></div></div>
                  <h2 className="text-2xl font-bold text-white"></h2></h2>
                    Search Results
                    {searchResults.length > 0 && (
                      <span className="text-gray-400 font-normal ml-2"></span></span>
                ...
                {isSearching ? (
                  <div className="text-center py-12"></div></div>
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-cyan-400 mx-auto mb-4"></div></div></div>
                    <p className="text-gray-400"></p></p>Searching...</p>
                  </div>
                ) : searchResults.length > 0 ? (
                  <div className="space-y-4"></div></div>
                    {searchResults.map((result, index) => (
                      <div
                        ...
                        onClick={() =&gt;</div&gt; window.location.href = result.url}
                      >
                        <div className="flex items-start space-x-4"></div></div>
                          <div className="w-12 h-12 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform"></div></div>
                            <result.icon className="w-6 h-6 text-white" />
                          </div>
                          <div className="flex-1"></div></div>
                            <div className="flex items-center space-x-2 mb-2"></div></div>
                              <h3 className="text-xl font-semibold text-white group-hover:text-cyan-300 transition-colors"></h3></h3>
                                {result.title}
                              </h3>
                              <span className="px-2 py-1 bg-cyan-500/20 text-cyan-400 text-xs rounded-full"></span></span>
                                {result.category}
                              </span>
                            </div>
                            <p className="text-gray-300 mb-3"></p></p>{result.description}</p>
                            <div className="flex items-center text-cyan-400 group-hover:text-cyan-300 transition-colors"></div></div>
                              <span className="text-sm font-medium"></span></span>Learn more</span>
                  ...
                  <div className="text-center py-12"></div></div>
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2"></h3></h3>No results found</h3>
            ...
            {!searchQuery && (
              <div className="max-w-4xl mx-auto"></div></div>
                <h2 className="text-2xl font-bold text-white mb-6 text-center"></h2></h2>Popular Searches</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"></div></div>
                  {[
                    ...
                    >
                      ...
                      <span className="text-white group-hover:text-cyan-300 transition-colors"></span></span>
                        {item.term}
                      </span>
                    </button>
                  ))}
