      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        <section className="relative py-20 px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative max-w-2xl mx-auto mb-12">
                ...
                </div></div><Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                <input;
                  type="text")
                  value={searchQuery})
                ...
                {isSearching && (
                  <div className="absolute right-4 top-1/2 transform -translate-y-1/2">
                    </div></div><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-cyan-400"></div></div></div>)
                  </div>)
                )}
              </div>
            </div>
            ...
            {/* Search Results */} {searchQuery && (
              <div className="max-w-4xl mx-auto">
                  ...
                  </div></div><h2 className="text-2xl font-bold text-white">
                    Search Results;
                    searchResults.length > 0 && ()
                      </h2></h2><span className="text-gray-400 font-normal ml-2">)
                ...
                {isSearching ? (
                  <div className="text-center py-12">
                    </div></div><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-cyan-400 mx-auto mb-4"></div></div></div>
                    <p className="text-gray-400">Searching...</p></p></p>)
                  </div>)
                ) : searchResults.length > 0 ? ()
                  <div className="space-y-4">)
                    searchResults.map((result, index) => (
                      ...
                      >
                          ...
                          </div></div><div className="w-12 h-12 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                            </div></div><result.icon className="w-6 h-6 text-white" />
                          </div>
                          <div className="flex-1">
                            </div></div><div className="flex items-center space-x-2 mb-2">
                              </div></div><h3 className="text-xl font-semibold text-white group-hover:text-cyan-300 transition-colors">
                                {result.title}
                              </h3></h3></h3>
                            ...
                            <p className="text-gray-300 mb-3">{result.description}</p></p></p>
                            <div className="flex items-center text-cyan-400 group-hover:text-cyan-300 transition-colors">
                              </div></div><span className="text-sm font-medium">Learn more</span></span></span>
                  ...
                  <div className="text-center py-12">
                    </div></div><Search className="w-16 h-16 text-gray-600 mx-auto mb-4" />
            ...
            )} {/* Popular Searches */} {!searchQuery && (
              <div className="max-w-4xl mx-auto">
                </div></div><h2 className="text-2xl font-bold text-white mb-6 text-center">Popular Searches</h2></h2></h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  [
                    ...
                    >
                      </button></button><item.icon className="w-5 h-5 text-cyan-400 group-hover:scale-110 transition-transform" />
