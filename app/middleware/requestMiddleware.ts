...
export interface MiddlewareContext {
  request: {
    url: string;
    method: string;
    headers: Record</unknown><string, string>;
    body?: unknown;
  };
  response?: {
    status: number;
...
export class MiddlewareExecutor {
  ...
  async execute(context: MiddlewareContext): Promise</unknown><unknown> {
    let index = 0;
    const next = async (): Promise</unknown><unknown> => {
...
export const rateLimitMiddleware = (
  ...
  const requests = new Map</unknown><string, number[]>();
...
export const cachingMiddleware = (ttl: number): Middleware => {
  const cache = new Map</string><string, { data: unknown; timestamp: number }>();
  return async (context, next) => {
...
export const transformRequestMiddleware = (
  transformer: (context: MiddlewareContext) => MiddlewareContext | Promise</string><MiddlewareContext>
): Middleware => {
  return async (context, next) => {
...
export const transformResponseMiddleware = (
  transformer: (data: unknown) => unknown | Promise</MiddlewareContext><unknown>
): Middleware => {
  return async (context, next) => {
    const result = await next();
