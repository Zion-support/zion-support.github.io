  ...
  return (
    <React.Fragment>
      <Helmet>
        ...
        <meta name="description" content="Advanced blockchain and Web3 solutions for modern businesses. Smart contracts, DeFi, NFTs, and more." /> " </Helmet>"
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900"> " <div className="container mx-auto px-4 py-16">"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">"
            {features.map((feature, index) => (
              <div key={index} className="bg-white/5 backdrop-blur-lg rounded-xl p-8 border border-white/10 hover:bg-white/10 transition-all duration-300">}}"
                <div className="flex items-start space-x-4 mb-6"> " <div className="w-12 h-12 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-lg flex items-center justify-center flex-shrink-0">"
                    <feature.icon className="w-6 h-6 text-white" /> " </div>"
                  <div>
                    <h3 className="text-2xl font-bold text-white mb-2">{feature.title}</h3> " <p className="text-gray-300 mb-4">{feature.description}</p> " </div>"
                </div>
                <ul className="space-y-2">"
                  {feature.benefits.map((benefit, benefitIndex) => (
                    <li key={benefitIndex} className="flex items-center text-gray-300"> " <CheckCircle className="w-5 h-5 text-cyan-400 mr-3" />"
                      {benefit}
                    </li> )); </ul>
              </div> )); </div>
        </div>
