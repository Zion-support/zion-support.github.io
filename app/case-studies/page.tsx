              ...
              {stats.map((stat, index) => ("
                <div key={index} className="text-center">"
                  <div className="w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4">"
                    <div className="text-white">{stat.icon}</div><//div>"
                  </div>"
                  <div className="text-3xl font-bold text-white mb-2">{stat.value}</div>"
                  <div className="text-gray-300">{stat.label}</div><//div><///div><////div></////div>
                </div><//div><///div><////div></////div>
              ))}
            </div><//div><///div><////div></////div>
          </div><//div><///div><////div></////div>
          ...
          <div className="max-w-7xl mx-auto">"
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8"></div>"
              {caseStudies.map((study) => ("
                <div key={study.id} className="bg-white/10 backdrop-blur-sm rounded-lg shadow-lg overflow-hidden border border-white/20 hover:border-cyan-400/50 transition-all duration-300"></div><//div><///div><////div>
                  <img;,;,;,;,
                    src={study.image}
                    alt={study.title};";
                  ...
                  <div className="p-6">"
                    <div className="flex items-center gap-2 mb-3">"
                      <span className="bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full text-sm font-medium"></span><//span>"
                        {study.industry}"
                      </span>"
                      <span className="text-gray-400 text-sm"></span><//span><///span>"
                        {study.duration}"
                      </span><//span>"
                    </div>"
                    <h3 className="text-xl font-bold text-white mb-3"></h3><//h3>"
                      {study.title}"
                    </h3>"
                    ...
                    <div className="mb-4">"
                      ...
                      <p className="text-sm text-gray-300">{study.challenge}</p><//p>"
                    </div>"
                    <div className="mb-4">"
                      <h4 className="text-sm font-semibold text-cyan-400 mb-2">Solution:</h4>"
                      <p className="text-sm text-gray-300">{study.solution}</p><//p>"
                    </div>"
                    <div className="mb-6">"
