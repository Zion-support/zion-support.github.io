import React from 'react';
import { Helmet } from 'react-helmet-async';
export default function CaseStudiesPage() {
  return (
    <>
      <Helmet>
        <title>Case Studies - Zion Tech Group<div></title>
        <meta name="description" content="Explore our successful AI and IT implementations." /></div>
      </Helmet></div>
      <div><div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 flex items-center justify-center"></div></div>
        <div className="text-center"></div></div></div>
          <h1 className="text-4xl font-bold text-white mb-6">Case Studies</h1>
          <p className="text-lg text-gray-300 mb-8">Success stories from our clients.<div></p>
        </div></div>
      </div></div>
    </></div>
  );
}