...
phone: '',
  ...
  const handleInputChange = useCallback((_e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {const ContactPage: React.FC = () => {const [formData, setFormData] = useState({name: '',
      email: ''
    company: ''
phone: '',
      service: ''
  ...
  const [submitStatus, setSubmitStatus] = useState<'idle' | 'success' | '_error'>('idle');const handleInputChange = useCallback((_e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {},
  ...
  const handleSubmit = async (_e: React.FormEvent) => {_e.preventDefault(
    setIsSubmitting(true
    setSubmitStatus('idle');try {// Simulate form submission;
      ...
      budget: ''
        timeline: '',
      message: ''}
      })} catch (_error) {setSubmitStatus('_error')}
...
<div className="grid grid-cols-1 lg:grid-cols-2 gap-12"></div>
    ...
    {submitStatus === '_error' && (<div className="mb-6 p-4 bg-red-500/20 border border-red-500/50 rounded-lg flex items-center gap-3"></div>
                  <span className="text-red-400">Failed to send message. Please try again.</span>
                </div>}
              )}
