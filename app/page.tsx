...
const HomePage = () => {
  ...
  return (
    <div><div><div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div></div></div>
      <Helmet></div>
        ...
        <meta name="keywords" content="AI solutions, IT services, digital transformation, business automation, technology consulting" /><div></div>
      </Helmet></div></div>
      {/* Hero Section */}
      <div><div><section className="pt-20 pb-16 px-4 sm:px-6 lg:px-8"></section></div></div>
      ...
      {/* Features Section */}
      <div><div><section className="py-16 px-4 sm:px-6 lg:px-8"></section></div></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8"><div></div></div></div></div>
            {features.map((feature, index) => (
              <div><div><div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 hover:bg-white/20 transition-all duration-300"></div></div></div>
                <div className={`w-16 h-16 rounded-lg bg-gradient-to-r ${feature.color} flex items-center justify-center mb-4 mx-auto`}><div></div></div></div></div>
                  {feature.icon}
                </div>
