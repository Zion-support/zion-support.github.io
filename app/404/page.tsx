...
import { Search, Home, Mail, FileText, LayoutGrid, Sparkles, Zap, ArrowRight, Fingerprint, AlertCircle, X } from 'lucide-react';
...
export default function NotFoundPage(): JSX.Element {
  ...
  const suggestions: ScoredItem[] = useMemo(() => fuzzyMatch(query, SITE_ITEMS), [query]);
  ...
  const popularLinks: PopularLink[] = [
    { title: 'AI Tools Directory', href: '/ai-tools', icon: <Zap className="h-4 w-4" /> },
  ...
  return (
    <main className="flex min-h-[80vh] flex-col items-center justify-center bg-slate-950 px-4 py-16 sm:px-6">
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            '@type': 'WebPage',
            name: 'Page Not Found',
            description: 'The requested page could not be found. Use the AI-powered search to discover tools, services, and resources.',
      ...
      <div className="w-full max-w-2xl space-y-8">
        <div className="text-center">
          ...
          <p className="mt-3 max-w-xl text-base leading-7 text-slate-300">
            ...
            Try searching below — our AI-powered search covers {SITE_ITEMS.length}+ pages and tools.
          </p>
        </div>
...
onBlur={() => { const t = setTimeout(() => setIsFocused(false), 200); return () => clearTimeout(t); }}
        ...
        {query && suggestions.length === 0 && (
          <div className="rounded-xl border border-slate-700/50 bg-slate-900/30 p-4 text-center">
            <p className="text-sm text-slate-400">
              No matching pages found for &ldquo;<span className="text-slate-200">{query}</span>&rdquo;
            </p>
          </div>
        )}
        ...
        <div className="rounded-2xl border border-slate-700/70 bg-slate-900/50 p-6">
          ...
          <p className="mt-1 text-sm text-slate-400">Jump to the page you need</p>
          <nav className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-5" aria-label="Popular pages">
            {popularLinks.map((link: PopularLink) => (
              <Link
                key={link.href}
                href={link.href}
