      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div>"
          ...
          <div className="container mx-auto px-4">"
            <div className="text-center max-w-4xl mx-auto">"
              <h1 className="text-5xl md:text-6xl font-bold text-white mb-6">;";,
                AI <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400">Personalized Learning</span><//span>"
              </h1>"
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">"
                <button className="bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-lg text-lg font-semibold hover:from-cyan-600 hover:to-purple-700 transition-all duration-300"></button>;,;,;";,
                  Start Learning"
                </button>"
                <button className="border-2 border-cyan-400 text-cyan-400 px-8 py-4 rounded-lg text-lg font-semibold hover:bg-cyan-400 hover:text-white transition-all duration-300"></button>;,;,;,;,
                  View Courses
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8"></div>"
              {features.map((feature, index) => ("
                <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 hover:border-cyan-500/40 transition-all duration-300 text-center">"
                  ...
                  <p className="text-gray-300 text-sm">{feature.description}</p><//p><///p><////p></////p>
                </div><//div><///div><////div></////div>
              ))}
            </div><//div><///div><////div></////div>
          </div><//div><///div><////div></////div>
          ...
          <div className="container mx-auto px-4">"
            ...
            <div className="flex flex-wrap justify-center gap-4"></div>"
              {subjects.map((subject, index) => ("
                <span key={index} className="px-6 py-3 bg-white/10 text-white rounded-lg font-semibold hover:bg-cyan-500/20 hover:text-cyan-400 transition-all duration-300"></span><//span><///span><////span>
                  {subject}
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8"></div>"
              {useCases.map((useCase, index) => ("
                <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20 hover:border-cyan-500/40 transition-all duration-300 group">"
                  <div className="text-cyan-400 mb-6 flex justify-center group-hover:scale-110 transition-transform duration-300"></div><//div>"
                    {useCase.icon}"
                  </div>"
                  <h3 className="text-xl font-semibold text-white mb-4 text-center group-hover:text-cyan-400 transition-colors"></h3><//h3>"
                    {useCase.title}"
                  </h3>"
                  <p className="text-gray-300 text-center">{useCase.description}</p><//p><///p><////p></////p>
                </div><//div><///div><////div></////div>
              ))}
            </div><//div><///div><////div></////div>
          </div><//div><///div><////div></////div>
        ...
        <section className="py-20 bg-white/5">"
          <div className="container mx-auto px-4">"
            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto"></div><//div><///div>"
