...
const ITServicesPage: React.FC = () => {
  const [activeCategory, setActiveCategory] = useState<'all' | 'infrastructure' | 'security' | 'development'>('all');

  const categories = [
    { id: 'all', name: 'All IT Services', icon: '💻', count: servicesData.itSolutions.length },
    { id: 'infrastructure', name: 'Infrastructure', icon: '☁️', count: servicesData.itSolutions.filter(s => s.title.includes('Infrastructure') || s.title.includes('Cloud')).length },
    { id: 'security', name: 'Security', icon: '🔒', count: servicesData.itSolutions.filter(s => s.title.includes('Security') || s.title.includes('Cybersecurity')).length },
    { id: 'development', name: 'Development', icon: '🚀', count: servicesData.itSolutions.filter(s => s.title.includes('Development') || s.title.includes('API')).length }
  ];

  const getFilteredServices = () => {
    switch (activeCategory) {
      case 'infrastructure':
        return servicesData.itSolutions.filter(s => s.title.includes('Infrastructure') || s.title.includes('Cloud'));
      case 'security':
        return servicesData.itSolutions.filter(s => s.title.includes('Security') || s.title.includes('Cybersecurity'));
      case 'development':
        return servicesData.itSolutions.filter(s => s.title.includes('Development') || s.title.includes('API'));
      default:
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Comprehensive IT services including infrastructure, security, development, and support solutions for modern businesses." />
        <meta name="keywords" content="IT services, infrastructure, cybersecurity, web development, cloud solutions, DevOps" />
      </Helmet>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-300 max-w-4xl mx-auto leading-relaxed mb-8">
              ...
              From infrastructure to security, we provide comprehensive technology services.
            </p>
                ...
                <div className="mb-6">
                  <div className="flex items-baseline space-x-2">
                    ...
                    <span className="text-gray-400">
                      {service.pricing.basic < 1000 ? '/month' : 'starting'}
                    </span>
                  </div>
