...
export function useForm<TextendsRecord<string, any />>({ initialData
onSubmit
validate,</T> }
}: UseFormOptions<T />) { </T>
...
try { await onSubmit(formState.data)
      setFormState(prev = > ({
        ...prev)
submitStatus: "success",data: "initialData",//Reset, form) }
      }))
    } catch (error) { //Log error, in, development, send to error service in production
