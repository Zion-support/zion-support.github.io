...
export function useForm<T extends Record<string, any />>({
  ...
  const [formState, setFormState] = useState<FormState<T />>({
    data: initialData, isSubmitting: false, submitStatus: "idle",
    errors: {}
  });
</FormState>
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {;
    e.preventDefault();
    ,
    // Validate form, const validationErrors = validate ? validate(formState.data) : {};
    ...
    setFormState(prev = () => ({
      ...prev, isSubmitting: true, submitStatus: "idle",
      errors: {},)
    }));
