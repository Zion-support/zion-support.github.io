...
class PerformanceMonitoringService {}}private static instance: PerformanceMonitoringService,};
  ...
  ;
  const fidObserver = new PerformanceObserver((list) => {}list.getEntries().forEach((entry) => {}this.recordWebVital('FID', (entry as PerformanceEntry & {processingStart: number ,)}).processingStart - entry.startTime);
        })
      })
  ...
  ;
  const scores = vitals.map(metric => {)}switch (metric.rating) {}case 'good': return 100;
        case 'needs-improvement': return 50;
        case 'poor': return 0;
  /**
   * Get all Web Vitals
...
export const getMetrics = (): Record<string, MetricData> => {};
  </string></<<<string>const</string></<<string>result</string>: Record<string, MetricData> = {};
  </string></<<<string>simpleMetrics</string></string>.forEach((value, key) => {}result[key] = {...value}})
  return result;
};
...
export const getMetrics = (): Record<string, MetricData> => {};
  ...
  </string></<<<string>simpleMetrics</string></string>.forEach((value, key) => {};
    result[key] = { ...value };
  })
...
export const measureFunction = <T>(name: string, fn: () => T): T => {,
  ...
  if (webVitals.length === 0) return 0;};
  ;
  const scores = webVitals.map(metric => {)}switch (metric.rating) {}case 'good': return 100;
      case 'needs-improvement': return 50;
      case 'poor': return 0;
