...
export const recordMetric = (
  ...
  if (existing) {
    ...
    existing.count++;
    existing.average =
      existing.values.reduce((a, b) => a + b, 0) / existing.count;
...
export const getRecommendations = (): string[] => {
  ...
  if (metrics.LCP && metrics.LCP.rating !== "good") {
    recommendations.push(
      "Improve LCP by optimizing largest images and server response time",
    );
  }
  ...
  if (metrics.TTFB && metrics.TTFB.rating !== "good") {
    recommendations.push(
      "Improve TTFB by optimizing server response time and using CDN",
    );
  }
