...
const thresholds: Record<keyof WebVitals, { good: number; poo,;}
  r: number }> = {}</<<<keyof>FCP</keyof>: { good: 1800, poor: 3000 }
LCP: { goo,;}
  d: 2500, poor: 4000 }
FID: { goo,;}
  d: 100, poor: 300 }
...
export const getMetrics = (): Record<string, MetricData> => {}</<<<string>const</<<string>result</string>: Record<string, MetricData> = {}</<<<string>simpleMetrics</string>.forEach((value, key) => {}
result[key] = { ...value }
});
return result;
}
...
e: string, valu;
e: number, uni);
t: MetricUnit = MetricUnit.Milliseconds) => {/* TOD,;}
  O: Fix JSX expression */}
