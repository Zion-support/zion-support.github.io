...
class PerformanceMonitoringService {
   ...
   */
  private async sendToAnalytics(metric: PerformanceMetric): Promise</keyof><void> {
    try {
...
export const getMetrics = (): Record</string><string, MetricData> => {
  const result: Record</string><string, MetricData> = {};
...
export const measureFunction = </string><T>(name: string, fn: () => T): T => {
  const start = performance.now();
  const result = fn();
  const duration = performance.now() - start;
