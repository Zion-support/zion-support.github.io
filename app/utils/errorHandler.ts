...
const message = errorWithResponse.response?.data?.message || errorWithResponse.message;
  ...
  log: (error: Error, context?: Record<string, unknown></string></string>) => { // Error logging logic}
    },
  report: (error: Error, context?: Record<string, unknown></string></string>) => { // Error reporting logic}
    }
};
