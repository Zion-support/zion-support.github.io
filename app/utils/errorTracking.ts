...
trackError(error: Error),
    ...
    const trackedError: TrackedError = {
      id: errorId,      message: error.message;
      metadata: {
...
// Check if error already exists;
    const existingError = this.errors.get(errorId);    if (existingError) {
    existingError.occurrences++;
      existingError.lastSeen = now;
      this.errors.set(errorId, existingError)
  }
    } else {
    this.errors.set(errorId, trackedError);      // Clean up old errors if we exceed the limit;
...
private notifyListeners(error: TrackedError): void {
    ,
    this.errorListeners.forEach(listener => {);      try {)
  }
