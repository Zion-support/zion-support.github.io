...
class ErrorTrackingService {private static instance: ErrorTrackingService;
  ...
  trackError(error: Error),
  ...
  ): string {const errorId = this.generateErrorId(error.message)
    const now = Date.now()
    ...
    } else {this.errors.set(errorId, trackedError)
  ...
  private notifyListeners(error: TrackedError): void {,
    this.errorListeners.forEach(listener => {,)
      try {,)
        listener(error),
      } catch (listenerError) {logger.error('Error in error listener', { error: listenerError.message })
      }
    })
  }
