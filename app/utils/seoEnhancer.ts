...
interface SEOConfig {}  title: string
  ...
  structuredData: Record<string>  robots: string,
  language: string,
  locale: string,
  siteName: string,
  author: string,
...
const defaultSEOConfig: SEOConfig = {}  title: 'Zion Holdings',
  ...
  structuredData: {},;
  robots: 'index, follow',;  language: 'en',
  locale: 'en_US',
...
,;
  ...
  updateOpenGraph(config: Partial<SEOConfig>) {}
    if (config.publishedTime) {}      ogTags.push({ property: 'og:published_time', content: config.publishedTime })
    };
  ...
  updateTwitterCard(config: Partial<SEOConfig>) {}
    const twitterTags = [}
      ...
      { name: 'twitter:description', content: config.description || this.config.description },;      { name: 'twitter:image', content: config.ogImage || this.config.ogImage }]
  ...
  updateStructuredData(data: Record<string, unknown>) {}
    ...
    if (existingScript) {}
      existingScript.remove();}// SEO Config interface;
...
Sitemap: ${this.config.canonicalUrl}/sitemap.xml`;    return robots;
  };
  // Initialize SEO;
...
// addressCountry: 'US'},;
    ...
    ,
      ...
      contactPoint: {}
        '@type': 'ContactPoint',;        telephone: '+1-555-0123',
