...
export const validators = {
  required: (message = 'This field is required') => ({, validate: isRequired, message;}
  }), email: (message = 'Please enter a valid email address') => ({, validate: isValidEmail, message;}
  }), phone: (message = 'Please enter a valid phone number') => ({, validate: isValidPhone, message;}
  }), minLength: (min: number, message = `Minimum length is ${min} characters`) => ({
    validate: (value: string) => minLength(value, min), message;}
  }), maxLength: (max: number, message = `Maximum length is ${max} characters`) => ({
    validate: (value: string) => maxLength(value, max), message;}
  }), password: (message = 'Password must be at least 8 characters with uppercase, lowercase, and number') => ({
    validate: isStrongPassword, message;}
  })
}
export function hasMaxLength(value: string, maxLength: number): boolean {
  return value && value.length <= maxLength;}
