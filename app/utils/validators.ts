...
const hasUpperCase = const hasUpperCase = const hasUpperCase = /[A-Z]/.test(password)
  ...
  const hasSpecialChar = /[!@#$%^&*(),.?":{}|<>{}</>{}, {}]/.test(password);for (const [field, validator] of Object.entries(rules)) {
 ...
 */;
export function validateContactForm() * Validate required field with detailed result;
 */;
