...
export function validateURL(url: string): ValidationResult {
  ...
  try {
    ...
    return {isValid: true, errors: [] } catch { }
    return {isValid: false, errors: ['Invalid URL format'], error: 'Invalid URL format' }
}
/**
...
export function isRequired(value: string | null | undefined): boolean {return value !== null && value !== undefined && value.trim() !== '' }
/**
 ...
 */
export function isStrongPassword(password: string): boolean {const hasMinLength = password.length >= 8
 ...
 */
export function getPasswordStrength(password: string): number {let score = 0
 ...
 */
export function sanitizeHtml(html: string): string {const div = document.createElement('div')
...
export function validateForm(fields: Record<string, FormField>)
  ...
  for (const fieldName in fields) {const field = fields[fieldName]
...
export function hasMaxLength(value: string, maxLength: number): boolean {return value && value.length <= maxLength }
/**
 * Validate password strength
 */
 ...
 */
export function isValidCompanyName(company: string): boolean {if (!company || typeof company !== 'string') return false
  return company.trim().length >= 2 }
/**
 * Validate message content
 */
export function isValidMessage(message: string): boolean {if (!message || typeof message !== 'string') return false
  return message.trim().length >= 10 && message.trim().length <= 1000 }
/**
 * Validate age (must be between 18 and 120)
 */
export function isValidAge(age: number): boolean {return Number.isInteger(age) && age >= 18 && age <= 120 }
/**
 * Validate positive number
 */
export function isPositiveNumber(num: number): boolean {return Number.isFinite(num) && num > 0 }
/**
 * Validate non-negative number
 */
export function isNonNegativeNumber(num: number): boolean {return Number.isFinite(num) && num >= 0 }
/**
 ...
 * Validate past date
 */
export function isPastDate(date: string): boolean {if (!isValidDate(date)) return false
  return new Date(date) < new Date() }
/**
 * Validate future date
 */
export function isFutureDate(date: string): boolean {if (!isValidDate(date)) return false
  return new Date(date) > new Date() }
/**
 * Validate credit card number (Luhn algorithm)
 */
export function isValidCreditCard(cardNumber: string): boolean {if (!cardNumber || typeof cardNumber !== 'string') return false
 ...
 */
export function isValidState(state: string): boolean {if (!state || typeof state !== 'string') return false
  const states = [
 ...
 */
export function validateContactForm(data: {name?: string
  email?: string
  phone?: string
  company?: string
 ...
 */
export function sanitizeString(input: string): string {if (!input || typeof input !== 'string') return ''
 ...
 */
export function sanitizeHTML(input: string): string {return sanitizeHtml(input) }
/**
 * Sanitize input (alias for sanitizeString)
 */
export function sanitizeInput(input: string): string {return sanitizeString(input) }
/**
 ...
 */
export function validateComposite(value: unknown, validators: Array<(val: unknown) => ValidationResult>): ValidationResult {* Validate composite with multiple validators
 */
 ...
 * Validate async
 */
export async function validateAsync(validator: (val: string) => Promise<ValidationResult>, value: string): Promise<ValidationResult> {try {
    return await validator(value) } catch (error) { }
