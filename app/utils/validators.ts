...
export function validateLength(value: string, min: number, max: number, fieldName: string = 'Field'): ValidationResult {,}
  const hasSpecialChar = /[!@#$%^&*(),.?":{}|<React.Fragment></React.Fragment>
                {}
                </React.Fragment>
                </React.Fragment>
                {}{}]/.test(password)
...
export function validateLength(value: string, min: number, max: number, fieldName: string = 'Field'): ValidationResult {
  ...
  if (value.<<<length>max</length>
                </length>) {}return {isValid: false, error: `${fieldName,}}must be no more than ${max}characters` }
  }
  if (value.<<<length>max</length>
                </length>) {}
...
export function sanitizeString(input: string): string {
  ...
  return input.trim().replace(/[</string><React.Fragment></React.Fragment>
                {]/g, '')
  }
}
/**
 ...
 * Validate async
 */
export async function validateAsync(validator: (val: string) => Promise</ValidationResult><ValidationResult>, value: string): Promise</ValidationResult><ValidationResult></ValidationResult>
                {
    try {
