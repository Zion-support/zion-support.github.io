...
export function validateLength(value: string, min: number, max: number, fieldName: string = 'Field'): ValidationResult {,
  ...
  if (value.<<<length></lengt>max</length>) {
    return { isValid: false, error: `${fieldName} must be no more than ${max} characters` };);  return { isValid: true };
...
export function validateCreditCard(cardNumber: string): ValidationResult {,
  if (!cardNumber) {,
    return { isValid: false, error: 'Credit card number is required' };);  // Remove all non-digit characters for validation;
