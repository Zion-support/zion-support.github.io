 ...
 */export function isRequired(value: string | null | undefined): boolean {,
  ...
  return value.toString().trim().length > 0export function isRequired(value: string | null | undefined): boolean {
  return value !== null && value !== undefined && value.trim() !== ''}
}
/**
 * Validate minimum length
 */export function minLength(value: string, min: number): boolean {,
  return value.trim().length >= minexport function hasMinLength(value: string, minLength: number): boolean {
 ...
 */export function validateCreditCard(cardNumber: string): ValidationResult {,
  ...
  if (!isValidCreditCard(cleanedNumber)) {}
    return { isValid: false, error: 'Invalid credit card number' }export function validateCreditCard(cardNumber: string): ValidationResult {
 ...
 */export function validateJSON(jsonString: string): ValidationResult {,
  if (!jsonString) {,}
    return { isValid: false, error: 'JSON is required' }export function validateJSON(jsonString: string): ValidationResult {
