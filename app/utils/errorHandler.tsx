...
export class ErrorHandler {/* TODO: Fix JSX expression */}
  ...
  private logError(error: AppError) {,
    if (this.config.enableConsoleLogging) {,}
      const logMessage = `[${error.severity
    }
  ...
  private notifyUser(error: AppError) {}
    ...
    notification.innerHTML = `</string>
      <div style="display: flexjustify-content: space-betweenalign-items:center"></div>
        <div></div>
          <strong>${error.severity} Error<p style="margin: 5px 0 0 0font-size:14px">${error.message}<button onclick="this.parentElement.parentElement.remove()" style="background: noneborder: nonecolor: whitefont-size: 18pxcursor: pointermargin-left:10px">×</button>`
    notification.innerHTML = `<div style="display: flexjustify-content: space-betweenalign-items:center">,</div>
        <div>,</div>
          <strong>${error.severity} Error</strong>
          <p style="margin: 5px 0 0 0font-size:14px">${error.message}</p>
        </div>
        <button onclick="this.parentElement.parentElement.remove()" style="background: noneborder: nonecolor: whitefont-size: 18pxcursor: pointermargin-left:10px">×</button>
      <div style="display: flexjustify-content: space-betweenalign-items:center"></div>
        <div></div>
          <strong>${error.severity} Error<p style="margin: 5px 0 0 0font-size:14px">${error.message}</p><button onclick="this.parentElement.parentElement.remove()" style="background: noneborder: nonecolor: whitefont-size: 18pxcursor: pointermargin-left:10px">×</button>
      </div>`
  ...
  s: center"></div>
        <div></div>
          <strong>${error.severity} Error</strong>"<p style="margi,
  n: 5px 0 0 0 font-siz,"
  e: 14px">${error.message}</p>
        </div>"<button onclick="this.parentElement.parentElement.remove()" style="
          backgroun,
  d: none,
          borde,
  r: none,
