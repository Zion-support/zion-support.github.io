...
export const usePerformanceMonitoring = useCallback((...args) => {}
  ...
  useEffect(() => {}
    if (typeof window === 'undefined') return const updateMetrics = useCallback((...args) => {}
      const navigation = performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming
      const paint = performance.getEntriesByType('paint')
      const fcp = paint.find(entry => entry.name === 'first-contentful-paint')?.startTime
      const lcp = performance.getEntriesByType('largest-contentful-paint')[0]?.startTime
  ...
  useEffect(() => {
    ...
    if (typeof window === 'undefined' || !('memory' in performance)) return const updateMemoryInfo = useCallback((...args) => {
      const memory = (performance as any).memory
      if (memory) {
        setMemoryInfo({)
...
export const preloadResource = useCallback((...args) => {
    if (typeof window === 'undefined') return // Resource preloading utility
  }
export const preloadResource = useCallback((...args) => {}
...
export const useBundleSizeMonitoring = useCallback((...args) => {
  ...
  useEffect(() => {
      ...
      let cssSize = 0;
      let imageSize = 0;
    if (typeof window === 'undefined') return const calculateBundleSize = useCallback((...args) => {
      const resources = performance.getEntriesByType('resource')
      let totalSize = 0
      let jsSize = 0
      let cssSize = 0
