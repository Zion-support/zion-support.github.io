...
class SEOOptimizer {
  ...
  generateTitle(pageTitle?: string): string {
    ...
    return title.includes(this.config.siteName) ? title : `${title} | ${this.config.siteName}`;
  }
  /**
  ...
  generateDescription(pageDescription?: string): string {
    const description =
      pageDescription || this.currentPageData?.description || this.config.defaultDescription;
  ...
  private initializeMetaTags(): void {
    ...
    */
    ...
    this.setMetaTag(
      'keywords',
      (this.currentPageData?.keywords || []).join(', ') || 'AI, IT Solutions, Technology'
    );
    // Open Graph tags
...
const defaultConfig: SEOConfig = {
  ...
  defaultDescription:
    'Zion Tech Group provides cutting-edge AI and IT solutions for businesses. Transform your operations with our innovative technology and expert consulting services.',
