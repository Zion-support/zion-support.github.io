...
class SEOOptimizer {/* TODO: Fix JSX expression */}
   ...
   */
  generateTitle(pageTitle?: string): string {const title = pageTitle || this.currentPageData?.title || this.config.defaultTitle;
   ...
   * Update meta tags;
   */
  private updateMetaTags(): void {if (!this.currentPageData) return;
...
// name: this.currentPageData.author || this.config.siteName;,)
   ...
   */
  generateRobotsTxt(): string {return `User-agent: *,
Allow: /}
