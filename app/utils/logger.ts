...
this.logs.push(logEntry)
  ...
  fatal(message: string, context?: LogContext, metadata?: Record<string, unknown>): void {</string&gt</<<<string&gtthis</string&gt</string&gt.log(LogLevel.FATAL, message, context, metadata)}
  }
  ...
  private log(level: LogLevel),
    ...
    context?: LogContext),
    metadata?: Record<string, unknown>
  ): void {if (level < this.logLevel) return const logEntry = {
      level,
      message,
      context,
      metadata,}
