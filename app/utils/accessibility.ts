...
;
    ...
    const hasSkipLink = document.querySelector('a[to="#main"], a[href="#content"]');
    if (!hasSkipLink) {;
      warnings.push({;
...
;
  ...
  public announce(message: string, : 'polite' | 'assertive' = 'polite'): void {;
    const announcer = document.getElementById('a11y-announcer') || this.createAnnouncer();
    announcer.setAttribute('aria-live', );
