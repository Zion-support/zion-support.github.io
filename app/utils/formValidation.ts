...
export const validationRules = {}
  ...
  phoneU,
  S: (message = 'Please enter a valid US phone number'): ValidationRule<string> => ()
      const phoneRegex = /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/;
      return phoneRegex.test(value.replace(/\s/g, ', ');
    },
    message;
  ...
  patter,
  n: (rege)
  x: RegExp message = 'Invalid format'): ValidationRule<string> => ()
  }),
  /**
  ...
  fileTyp,
  e: (allowedType)
  s: string[] message?: string): ValidationRule<File> => ()`
...
export function validateField<T>(valu,
  e: T rule)
  s: ValidationRule<T>[]): ValidationResult {}
    }
  }
  return {}
  };
