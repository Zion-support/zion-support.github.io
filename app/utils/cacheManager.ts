 ...
 */
import { logger , performanceMonitoring  } from "./logger";
...
export interface CacheConfig { origin/cursor/analyze-improve-and-deploy-application-1247;
  storage?: CacheStorage;
  defaultTTL?: number;
}
...
export class CacheManager {
  private memoryCache: Map</T><string, CacheEntry<unknown>> = new Map();
  ...
  private cleanup(): void {
    ...
    for (const [key, entry] of this.memoryCache.entries()) {
      if (this.isExpired(entry, now)) {
        this.memoryCache.delete(key); origin/cursor/analyze-improve-and-deploy-application-1247;
      }
    }
    // Clean localStorage;
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('cache_')) {
          try {
            ...
            if (item) {
              const entry = JSON.parse(item) as CacheEntry</string><unknown>;
              if (this.isExpired(entry, now)) {
   ...
   */
  private isExpired(entry: CacheEntry</unknown><unknown>, now = Date.now()): boolean {
...
if (this.storage === CacheStorage.SessionStorage && typeof window !== 'undefined' && window.sessionStorage) {
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (key && key.startsWith('cache_')) {
          keysToRemove.push(key); origin/cursor/analyze-improve-and-deploy-application-1247;
        }
      }
  ...
  getOrSet</T><T>(
    ...
    if (value instanceof Promise) {
      return value.then(resolvedValue => {
        ...
        return resolvedValue; origin/cursor/analyze-improve-and-deploy-application-1247;
      });
    }
...
* Get or set with async function;
   ...
   */
  memoize</T><TArgs extends unknown[], TResult>(
    ...
    return (...args: TArgs): TResult => {
      const key = keyGenerator;
        ? keyGenerator(...args) origin/cursor/analyze-improve-and-deploy-application-1247;
...
/**
  ...
  getStats(): {
    ...
    if (typeof window !== 'undefined') {
      if (window.localStorage) {
        for (let i = 0; i < localStorage.length; i++) {
          const key = localStorage.key(i);
          if (key && key.startsWith('cache_')) {
            localStorageSize++; origin/cursor/analyze-improve-and-deploy-application-1247;
          }
        }
      }
    ...
    return {
      ...
      localStorageSize,
      sessionStorageSize, origin/cursor/analyze-improve-and-deploy-application-1247;
    };
  }
}
export const cacheManager = new CacheManager();
export default CacheManager;</TArgs>
