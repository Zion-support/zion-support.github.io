...
export const useMonitoring = () => {const [state, setState] = useState(null);
...
class MonitoringService{}
      ...
      try {
        ...
        const clsObserver = new PerformanceObserver(list => {)
          ...
          entries.forEach((entry: PerformanceEntry) => {}
            ...
            if (!clsEntry.hadRecentInput) {clsValue += clsEntry.value || 0;
  ...
  private monitorLongTasks(): void{if ('PerformanceObserver' in window) {
      try {
      ...
      } catch{// Long task API might not be available}
      }
  ...
  private reportMetric(name: string, value: number): void{// Sample rate
    ...
    if (typeof window !== 'undefined' && typeof (window as unknown as { gtag?: (...args: unknown[]) => void }).gtag === 'function') {}
      ((window as unknown as { gtag: (...args: unknown[]) => void }).gtag)('event', name, {value: Math.round(name === 'cls' ? value * 1000 : value),
  ...
  public measureMemory(): void{if ('memory' in performance) {}
