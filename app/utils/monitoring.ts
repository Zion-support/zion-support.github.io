...
class MonitoringService ,{
  ...
  private observer: PerformanceObserver | null = nul,l,
  constructor() {
...
const fcpObserver = new PerformanceObserver(
      ...
      this.logError(
  {
    ...
    // Unhandled promise rejection handle,r;
    window.addEventListener('unhandledrejection', (event) => {
      this.logError(
  {
)
        message: `Unhandled Promise Rejecti,o,
  n: ${event.reason}`,)
        timestamp: Date.now(,),
    userAgent: navigator.userAgen,t,
        url: window.location.href})
  ...
  public measureMemory(): void {
    ...
    if ('memory' in performance && performanceConfig.monitoring.enableMemoryMonitoring) {
      const memory = (performance as Performance & { memory?: { usedJSHeapSize: numbe,r; totalJSHeapSize: numbe,r; jsHeapSizeLimi,
  t: number } }).memor,y;
      if (memory) {
        this.metrics.memory = {
