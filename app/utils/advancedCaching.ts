...
export interface CacheOptions {ttl?: number; // Time to live in milliseconds;
...
,
  constructor(options: CacheOptions = {}) {this.options = {
    ...
    if (this.options.storage !== 'memory') {this.loadFromStorage();
    }
  ...
  private setupCleanup(): void {if (typeof window !== 'undefined') {
...
,
    for (const [key, entry] of this.cache.entries()) {if (entry.expiry <= now) {
  ...
  set(key: string, value: T, customTTL?: number): void {}
    ...
    while (this.cache.size >= this.options.maxSize) {this.evictLRU();
    }
