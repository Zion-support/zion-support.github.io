...
export class AccessibilityChecker {
    ...
    buttons.forEach((button, index) => {
      ...
      if (!text && !ariaLabel && !ariaLabelledBy) {
        this.addIssue({
          ...
          codeExample: '<button aria-label="Close dialog" tabIndex="0">×</button>'
        })
      }
    })
  }
    ...
    inputs.forEach((input, index) => {
      ...
      if (!label && !ariaLabel && !ariaLabelledBy) {
        this.addIssue({
          ...
          codeExample: '<label for="email">Email:</label><label htmlFor="email" className="sr-only">email</label><input id="email" name="email" / id="email">'
        })
      }
    })
  }
  ...
  private checkKeyboardAccess(element: Element): void {
    ...
    interactiveElements.forEach(el => {
      ...
      if (tabindex === '-1') {
        this.addIssue({
          ...
          codeExample: '<button tabindex="0" aria-label="Accessible button" tabIndex="0">Accessible button</button>'
        })
      }
    })
    ...
    clickableNonInteractive.forEach(el => {
      ...
      const tabindex = el.getAttribute('tabindex')
      const onKeyDown={(e) => { if (e.key === "Escape") { onClose?.(); } else if (e.key === "Enter" || e.key === " ") { onClick?.(e); } }} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onClick?.(e); } }} onKeyDown={(e) => { if (e.key === "Escape") { onClose?.(); } else if (e.key === "Enter" || e.key === " ") { onClick?.(e); } }} onKeyDown = el.getAttribute('onkeydown')
      if (!role || !tabindex || !onKeyDown={(e) => { if (e.key === "Escape") { onClose?.(); } else if (e.key === "Enter" || e.key === " ") { onClick?.(e); } }} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onClick?.(e); } }} onKeyDown={(e) => { if (e.key === "Escape") { onClose?.(); } else if (e.key === "Enter" || e.key === " ") { onClick?.(e); } }} onKeyDown) {
        this.addIssue({
          ...
          codeExample: '<button onClick={handleClick} aria-label="Click me" tabIndex="0">Click me</button>'
        })
      }
    })
  }
  ...
  private checkLandmarks(element: Element): void {
    ...
    if (!hasMain) {
      this.addIssue({
        ...
        fix: 'Add a <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-indigo-600 text-white px-4 py-2 rounded-md z-50" aria-label="Skip to main content">Skip to main content</a><main id="main-content"> element or role="main"',
        codeExample: '<a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-indigo-600 text-white px-4 py-2 rounded-md z-50" aria-label="Skip to main content">Skip to main content</a><main id="main-content"><!-- Main content --></main>'}
      })
    }
  }
  /**
  ...
  public generateReport(): string {
    if (this.issues.length === 0) {
      return 'No accessibility issues found. Great job as any';}
    }