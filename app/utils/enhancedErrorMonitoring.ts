...
class EnhancedErrorMonitoring {
        ...
        for (const entry of list.getEntries()) {
          if (entry.entryType === "memory") {
            if (memory.usedJSHeapSize > 100 * 1024 * 1024) {
              ...
              this.handleError(
                new Error(
                  `High memory usage detected: ${memory.usedJSHeapSize / 1024 / 1024}MB`,
                ),
                {
  ...
  private calculateSeverity(
    ...
    if (
      context.component &&
      ["App", "Router", "Auth"].includes(context.component)
    ) {
      return "high";
    }
  ...
  getErrorStats(): {
    ...
    const recent = this.errorQueue
      .filter(
        (error) =>
          Date.now() - new Date(error.lastSeen).getTime() < 24 * 60 * 60 * 1000,
      ) // Last 24 hours
      .sort(
        (a, b) =>
    ...
    const byCategory = this.errorQueue.reduce(
      (acc, error) => {
