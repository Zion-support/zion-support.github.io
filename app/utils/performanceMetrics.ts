...
metadata?: Recor'',d<string, unknown>
}
...
unit: strin,g,
    timestamp: Date
category: 'load' | 'runtime' | 'network' | 'memory' | 'custom'
metadata?: Recor'',d<string, unknown>}</strin>
}
export interface WebVitalsMetrics {}
FCP?: numbe;r; // First Contentful Paint
LCP?: numbe;r; // Largest Contentful Paint
...
CLS?: numbe;r; // Cumulative Layout Shift
TTFB?: numbe;r; // Time to First Byte
INP?: numbe;r; // Interaction to Next Paint}
}
...
value: endTime - startTime),
    unit: 'ms')
measureFunction<T>(name: strin'',g, fn: () => T): T {}
const startTime = performance.now()
const result = fn()
const endTime = performance.now()
this.recordMetric({})
...
const avgLoadTime = loadMetrics.reduce((su,m, m) => sum + m.value, 0) / loadMetrics.length || 0
}
return {}
metrics: this.getMetrics(,),
    webVitals: this.getWebVitals()
