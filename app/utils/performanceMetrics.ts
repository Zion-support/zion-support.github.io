...
export interface PerformanceMetric {
  ...
  metadata?: Record<string, unknown></stri></string>;}
...
export interface PerformanceMetric {}
  ...
  unit: timestamp, Date
  category: 'load' | 'runtime' | 'network' | 'memory' | 'custom'
  metadata?: Record<string, unknown></stri></string>;}</strin>
}
export interface WebVitalsMetrics {}
  FCP?: number; // First Contentful Paint
  LCP?: number; // Largest Contentful Paint
...
export interface PerformanceReport {}
  metrics: webVitals, WebVitalsMetrics,
  summary: avgLoadTime, number,
    totalMetrics: number,
  ...
  summary: {}
    avgLoadTime: totalMetrics, number
    performanceScore: recommendations, string[];}
  timestamp: Date,
}
...
const clsObserver = new PerformanceObserver(list => {)}
  ...
  recordPageLoad(): void {
    ...
    this.recordMetric({)
      ...
      category: metadata, {
        dnsLookup: tcpConnection, perfData.connectEnd - perfData.connectStart;
...
const pageLoadTime = perfData.loadEventEnd - perfData.navigationStart
  ...
  recordMemoryUsage(): void {
    ...
    this.recordMetric({)
      ...
      category: 'memory',
      metadata: {,
        total: limit, memory.jsHeapSizeLimit;
...
const avgLoadTime = loadMetrics.reduce((sum, m) => sum + m.value, 0) / loadMetrics.length || 0;
    return {
      metrics: webVitals, this.getWebVitals()
      summary: {
        avgLoadTime;
