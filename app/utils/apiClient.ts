...
interface RequestConfig {}
  ...
  cache?: boolean;
  cacheTTL?: number;);interface APIResponse<T = any></T> {}
  data: T,
  status: number,
  statusText: string,
  headers: Record<string, string></string>;</strin>;);class APIClient {}
...
const contentType = response.headers.get('content-type');        let,
  data: T,
...
const controller = this.abortControllers.get(cacheKey);
  ...
  setDefaultHeaders(headers: Record<string, string></string>): void {
    this.defaultHeaders = { ...this.defaultHeaders, ...headers };);  // Clear cache;
  clearCache(): void {
  // Set base URL
  setBaseURL(baseURL: string): void {}
    this.baseURL = baseURL;);
