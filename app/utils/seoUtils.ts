...
export class SEOManager {private config: SEOConfig
  ...
  private updateOpenGraphTags(): void {
    ...
    ogTags.forEach(tag => {this.updateMetaTagByProperty(tag.property, tag.content) })
  }
  private updateTwitterTags(): void {
    ...
    twitterTags.forEach(tag => {this.updateMetaTag(tag.name, tag.content) })
  }
  ...
  private addMetaTag(name: string, content: string): void {if (typeof document === 'undefined') return
...
// Utility functions
export const generateMetaDescription = (content: string, maxLength: number = 160): string => {const cleanContent = content.replace(/<[^>]*>/g, '').trim()
...
export const addStructuredData = (data: any): void => {if (typeof document === 'undefined') return
