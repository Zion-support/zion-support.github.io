...
export type ValidationRules = Record</T><string, FieldRule[]>;
...
export function sanitizeHTML(html: string): string {
  // Remove script tags;
  let clean = html.replace(/</string><script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '');
 ...
 */
export function createCustomValidator</script><T>(
...
export const ValidationRulesBuilder = {
  required: </string><T>(): ValidationRule</T><T> => ({
    validate: (value: T) => {
      if (value === null || value === undefined) return false;
      if (typeof value === 'string' && value.trim() === '') return false;
      if (Array.isArray(value) && value.length === 0) return false;
...
class DataValidator {
  ...
  isObject(value: unknown): value is Record</T><string, unknown> {
...
export default DataValidator; origin/cursor/analyze-improve-and-deploy-application-1247</string>
