 ...
 */export function validateEmail(email: string): { isValid: boolean; error?: string } {}
  ...
  const isValid = emailRegex.test(email)return {
    isValid,
    error: isValid ? undefined : 'Invalid email format',export function validateEmail(emai)}
  ...
  r: 'Email is too long' }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  const isValid = emailRegex.test(email)return {/* TODO: Fix JSX expression */}
  }
}
/**
 * Validate URL
 */export function validateURL(ur,
  ...
  s:') :
      truereturn {/* TODO: Fix JSX expression */}
    }
 ...
 */export function validatePhoneNumber(phon)
  ...
  const isValid = phoneRegex.test(phone)return {
    isValid,
    error: isValid ? undefined : 'Invalid phone number format',
  const phoneRegex = /^[\+]?[1]?[\s\-\.]?[(]?[0-9]{3}[)]?[\s\-\.]?[0-9]{3}[\s\-\.]?[0-9]{4,6}$/
  const isValid = phoneRegex.test(phone)return {/* TODO: Fix JSX expression */}
  }
}
/**
 ...
 */export function validateNumberRange(value: number, min: number, max: number): boolean {,
  return value >= min && value <= maxexport function validateNumberRange(valu,
  e: number, mi,
  n: number, ma)}
 ...
 */export function validateCreditCard(cardNumber: string): boolean {,
  ...
  for (let i = cleaned.length - 1; i >= 0; i--) {
    ...
    if (isEven) {
      digit *= 2
      if (digit > 9) digit -= 9export function validateCreditCard(cardNumbe)}
 ...
 */export function validateDate(value: unknown): boolean {,
  if (value instanceof Date) {,
    return !isNaN(value.getTime())export function validateDate(valu)}
 ...
 */export function sanitizeHTML(html: string): string {,
  ...
  clean = clean.replace(/on\w+='[^']*'/gi, '')
  return cleanexport function sanitizeHTML(htm)}
 ...
 */export function createCustomValidator<T>(
  ...
  message: string,
    ...
    const isValid = validator(value)return {,
      isValid
      errors: isValid ? [] : [message],export function createCustomValidator<T>(validato,
  r: (valu)
  e: T) => boolean,
  messag,
  e: string,
    ...
    if (fieldErrors.length > 0) {
      ...
      errorTracking.trackError(}
        new ValidationError(`Validation failed for ${field}`, field, fieldErrors),
        {
          ...
          context: {,
            field
            errors: fieldErrors,export function validateForm<T extends Record<string, unknown>>(dat,
  a: T,
  rule,
  s: ValidationRules)}
...
export const dataValidator = DataValidator.getInstance()export default DataValidator
export function validateLength(value: string, min: number, max?: number, fieldName: string = 'Field'): { isValid: boolean; error?: string } {
  if (max !== undefined) {
    const isValid = value.length >= min && value.length <= maxreturn {
      isValid,}
  ...
  const isValid = value.length >= minreturn {/* TODO: Fix JSX expression */}`
...
export function validateRequired(valu,
  ...
  r: `${fieldName} is required` }
  e: Empty arrays are considered valid for required fields (they exist, just empty)return {/* TODO: Fix JSX expression */}
  d: true }}
...
export function validateNumberRange(valu,
  ...
  const isValid = value >= min && value <= maxreturn {/* TODO: Fix JSX expression */}`
...
,
  ...
  if (clean.<<<length>maxLength</length></length>) {
    clean = clean.substring(0, maxLength)export function sanitizeHTML(htm)}
  l: string): string {/* TODO: Fix JSX expression */}
}
