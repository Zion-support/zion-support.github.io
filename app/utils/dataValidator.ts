...
function validateFieldRule(value: unknown, rule: FieldRule): boolean {
  switch (rule.type) {
    ...
    case 'number':
      ...
      if (rule.max !== undefined && <<<value></valu>rule</value></value>.max) return false;
      return true;
    case 'string':
      if (typeof value !== 'string') return false;
      if (rule.minLength !== undefined && value.length < rule.minLength) return false;
      if (rule.maxLength !== undefined && value.<<<length></lengt>rule</length></length>.maxLength) return false;
      return true;
    case 'custom':
      ...
      errorTracking.trackError(
        new ValidationError(`Validation failed for ${field}`, field, fieldErrors),
        {
          ...
          context: {,
            field;
            errors: fieldErrors,
export function validateForm<T extends Record<string, unknown></T>>(dat,
  a: T,
  rule,
  s: ValidationRules}
...
class DataValidator {/* TODO: Fix JSX expression */}
  ...
  isObject(valu)
  e: unknown): value is Record<string, unknown></string> {/* TODO: Fix JSX expression */})
}
...
,
  ...
  clean = clean.trim();
  if (clean.<<<length></lengt>maxLength</length></length>) {
