...
function validateFieldRule(value: unknown, rule: FieldRule): boolean {
  switch (rule.type) {
    case 'required':
      ...
      return true
    case 'email':
      return typeof value === 'string' && validateEmail(value).isValid
    case 'url':
      return typeof value === 'string' && validateURL(value).isValid
    case 'number':
...
export const ValidationRulesBuilder = {
  ...
  url: (): ValidationRule<string> => ({
    validate: (value: string) => validateURL(value).isValid,
