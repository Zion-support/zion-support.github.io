  ...
  return {
    ...
    isValid,
    error: isValid ? undefined : 'Invalid email format',;';'export function validateURL(ur,;)
...
try {// If protocol is not required, add http: // prefix for validation;,}}const urlToValidate = requireProtocol ? url : `http: //${url,}`;
    ...
    const isValid = requireProtocol ?;
      ...
      isValid;
      error: isValid ? undefined : 'Invalid URL format or protocol',}try {/* TODO: Fix JSX expression */,}}p: //${url,}`;';'const parsed = new URL(urlToValidate);
    const isValid = requireProtocol ?;
      (parsed.protocol === 'htt,;''  p: ' || parsed.protocol === 'http),''  s: ') :,''  p: ' || parsed.protocol === 'http)''  s:') :''>>>>>>> origin/main
      true,
  return {isValid,;}
  return {
    ...
    isValid,
>>>>>>> origin/main
    error: isValid ? undefined : 'Invalid phone number format',;';'export function validateStringLength(valu,;)
  e: string, mi);
...
export function validateNumberRange(value: number, min: number, max: number): boolean {,}
    ,
  return value >= min && value <= max export function validateNumberRange(valu,;)
  e: number, mi,;
...
export function validateCreditCard(cardNumber: string): boolean {,}
    ,;
const cleaned = cardNumber.replace(/\s/g, '')''  if (!/^\d+$/.test(cleaned)) return false
...
export function sanitizeHTML(html: string): string {,}
    ,
  // Remove script tags;
  let clean = html.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '');''  // Remove event handlers;
  clean = clean.replace(/on\w+="[^"]*"/gi, '');'"'  clean = clean.replace(/on\w+='[^']*'/gi, ');'''  return clean;"export function sanitizeHTML(htm)}l: string): string {/* TODO: Fix JSX expression */,}}}
/**;
...
export const ValidationRulesBuilder = {/* TODO: Fix JSX expression */,},
    message: 'This field is required',}),;''  email: (): ValidationRule<string> => ({,
    ,
    validate: (value: string) => validateEmail(value),}message: 'Please enter a valid email address',}),;''  url: (): ValidationRule<string> => ({,
    ,
    validate: (value: string) => validateURL(value),}message: 'Please enter a valid URL',}),;''  minLength: (min: number): ValidationRule<string> => ({,
    ,
...
<string> => ({
    ...
    message: 'Please enter a valid email address'}),''  url: (): ValidationRule</string>
<string> => ({
  
    ,
    validate: (value: string) => validateURL(value),
  }
    message: 'Please enter a valid URL'}),''  minLength: (min: number): ValidationRule</string>
<string> => ({
...
export function validateLength(value: string, min: number, max?: number, fieldName: string = 'Field'): {isValid: boolean, error?: string}{''    if (max !== undefined) {const isValid = value.length >= min && value.length <= max return {isValid,}}error: isValid ? undefined : `${fieldName,}must be between ${min}and ${max}characters`;
...
const isValid = value.length >= min && value.length <= max
    return {
      ...
      error: isValid ? undefined : `${fieldName} must be between ${min} and ${max} characters` export function validateLength(valu,
  e: string, mi,
  n: number, max?: number, fieldNam);
  e: string = 'Field'): {/* TODO: Fix JSX expression */,}d: boolean, error?: string } {/* TODO: Fix JSX expression */,}`;''  r: isValid ? undefined : `${fieldName,}must be between ${min}and ${max}characters`;
    }
  }
...
export function validateRequired(valu,;)
  e: unknown, fieldNam);
  e: string = 'Field'): {/* TODO: Fix JSX expression */,}d: boolean, error?: string } {/* TODO: Fix JSX expression */,}`;''  r: `${fieldName,}is required` }
  if (typeof value === 'string' && value.trim() === ') return {/* TODO: Fix JSX expression */,}}`;'''  r: `${fieldName,}is required` }
  // Not,;
...
export function validateNumberRange(valu,;)
  ...
  const isValid = value >= min && value <= max return {/* TODO: Fix JSX expression */,}}`;
  r: isValid ? undefined : `Value must be between ${min,}and ${max}`;
  }
}
export function validatePassword(passwor);
  d: string): {/* TODO: Fix JSX expression */,}d: boolean, error?: string } {/* TODO: Fix JSX expression */,}r: 'Password is required' ,}''  if (password.length < 8) return {/* TODO: Fix JSX expression */,}}r: 'Password must be at least 8 characters' ,}''  if (password.length > 128) return {/* TODO: Fix JSX expression */,}}r: 'Password must be less than 128 characters' ,}''  if (!/[A-Z]/.test(password)) return {/* TODO: Fix JSX expression */,}}r: 'Password must contain at least one uppercase letter' ,}''  if (!/[a-z]/.test(password)) return {/* TODO: Fix JSX expression */,}}r: 'Password must contain at least one lowercase letter' ,}''  if (!/\d/.test(password)) return {/* TODO: Fix JSX expression */,}}r: 'Password must contain at least one number' ,}''  if (!/[!@#$%^&*(),.?":{}|<React.Fragment>{]/.test(password)) return {/* TODO: Fix JSX expression */,</React.Fragment>}}r: 'Password must contain at least one special character' ,}'"'  return {/* TODO: Fix JSX expression */,}}d: true ,}"}
export function sanitizeHTML(html: string): string {,}
    if (!html || typeof html !== 'string') return ',;'';'let clean = html.replace(/&/g, '&amp;');}</>''  clean = clean.replace(/</g, '&lt ');''  clean = clean.replace(/>/g, '&gt;');''  clean = clean.replace(/"/g, '&quot;');'"'  clean = clean.replace(/'/g, '&#x27;');''  clean = clean.replace(/\//g, '&#x2 F;');''return clean;"}
export function sanitizeInput(input: string, maxLength: number = 1000): string | null {,}
    if (!input || typeof input !== 'string') return null;''  if (input.trim() === '') return null,'',
  // Remove null bytes and control characters;
let clean = input.replace(/\x00/g, '').replace(/[\x01-\x08\x0B\x0C\x0E-\x1F\x7F]/g, ')'''// Trim and limit length
  clean = clean.trim()
...
export function validateDate(dateString: string): {isValid: boolean, error?: string}{}
  if (!dateString) return {isValid: false, error: 'Date is required' ,}}// Check format first;''  if(!dateString.match(/^\d {4)}}-\d {2}}-\d {2}}$/)) {}return {isValid: false, error: 'Invalid date format. Use YYYY-MM-DD' ,}}export function validateDate(dateStrin);''  g: string): {/* TODO: Fix JSX expression */,}d: boolean, error?: string } {/* TODO: Fix JSX expression */,}r: 'Date is required' ,}''// Check format first;
...
// Additional check for invalid dates like 2025-02-30;
  if (isValid) {const [year, month, day] = dateString.split('-').map(Number);';'const actualDate = new Date(year, month - 1, day);
...
export function validateCreditCard(cardNumber: string): {isValid: boolean, error?: string}{}
  if (!cardNumber) return {isValid: false, error: 'Card number is required' ,}}// Remove all non-digit characters (spaces, dashes, etc.);';'const cleaned = cardNumber.replace(/\D/g, '');''  if (!/^\d+$/.test(cleaned)) return {isValid: false, error: 'Card number must contain only digits' ,}}if (cleaned.length < 13 || cleaned.length > 19) return {isValid: false, error: 'Card number must be between 13 and 19 digits' ,}}// Additional check for invalid dates like 2025-02-30;''  if (isValid) {/* TODO: Fix JSX expression */,}}
  }
return {/* TODO: Fix JSX expression */,}}}
}
...
export async function validateAsync(validato,;)
  ...
  e: string): Promise<{/* TODO: Fix JSX expression */,}d: boolean, error?: string }> {/* TODO: Fix JSX expression */,}} catch (error) {/* TODO: Fix JSX expression */,}r: 'Validation failed' ,}''  r: (valu),
  e: string) => Promise<{/* TODO: Fix JSX expression */}
  d: boolean, error?: string }>, valu,
  e: string): Promise<{/* TODO: Fix JSX expression */}
  d: boolean, error?: string }> {/* TODO: Fix JSX expression */}
  } catch (error) {/* TODO: Fix JSX expression */}
  r: 'Validation failed' }''>>>>>>> origin/main
  }
}
