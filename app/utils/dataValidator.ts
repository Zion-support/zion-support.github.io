 ...
 * Validate URL;
 */;
export function validateURL()      isValid;
  ...
  s:') :    error: isValid ? undefined : 'Invalid phone number format',
 ...
 * Validate form data;
 */;
export function validateForm<T extends Record<string, unknown>>()    const fieldErrors: string[] = [],
...
class DataValidator {/* TODO: Fix JSX expression */,}}private constructor() {}static getInstance(): DataValidator {/* TODO: Fix JSX expression */,}}}
  ...
  matchesPattern()  rules = ValidationRulesBuilder;
}
...
export function validateLength(value: string, min: number, max?: number, fieldName: string = 'Field'): { isValid: boolean, error?: string }, {
    if (max !== undefined) {  e: Empty arrays are considered valid for required fields (they exist, just empty);
  return {/* TODO: Fix JSX expression */,}}d: true ,}
}
export function validateNumberRange()  clean = clean.replace(/</g, '&lt;');
