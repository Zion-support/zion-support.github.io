...
// Get interceptor counts;
  getInterceptorCounts(): {request: number,
    ...
    return {
      request: this.requestInterceptors.length    this.errorHandler = ErrorHandler.getInstance();
  }
  ...
  g: RequestConfig, attempt = 1): Promise<APIResponse<T>> {/* TODO: Fix JSX expression */,}}
      ...
      const,;
  fetchOptions: RequestInit = {/* TODO: Fix JSX expression */,}}      const response = await fetch(url, fetchOptions);
   ...
   */;
  private setInCache()  y: string, respons);
