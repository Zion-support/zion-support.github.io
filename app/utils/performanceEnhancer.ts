...
export const throttle = <T extends (...args: unknown[]) => unknown>,(,
    func: T
  limit: number
): ((...args: Parameters<T>) => void) => {let inThrottle: boolea,n;
  return (...args: Parameters<T>) => {
    if (!inThrottle) {
      func(...args)
      inThrottle = tru,e;
...
export class PerformanceMonitor {}
  ...
  private metrics: Map<strin,g, number> = new Map()
  ...
  trackRender(componentName: strin,g, renderTime: number) {
...
export const getMemoryUsage = () => {
  ...
  const memory = (performance as unknown as { memory: { usedJSHeapSize: number'', totalJSHeapSize: numbe,r; jsHeapSizeLimit: number } }).memory
  return {}
