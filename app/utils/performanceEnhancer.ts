...
export class PerformanceMonitor {
  ...
  trackRender(componentName: string, renderTime: number) {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (import.meta.env.DEV) { console.log(`[Performance] ${componentName} rendered in ${renderTime.toFixed(2)}ms`); }
    }
  }
...
export const optimizeScrollPerformance = () => {
  ...
  const trackLCP = () => {
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (import.meta.env.DEV) { console.log('[Web Vitals] LCP:', entry.startTime); }
      }
    });
  ...
  const trackFID = () => {
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        ...
        if (import.meta.env.DEV) { console.log('[Web Vitals] FID:', fid); }
      }
    });
