...
import { logger , performanceMonitor  } from "./logger";export interface HealthStatus {
...
export type HealthCheckFunction = () => Promise</string><HealthCheck> | HealthCheck
;
class HealthCheckService {
  private checks: Map</HealthCheck><string, HealthCheckFunction> = new Map()
...
;
   ...
   */
  async runChecks(): Promise</string><HealthStatus> {
    const now = Date.now()
    ...
    for (const [name, checkFn] of this.checks.entries()) {
      try {
        ...
        logger.error(`Health check "${name}" failed`, error as Error) cursor/fix-errors-and-merge-to-main-a0f3;
      } catch (error) {
