...
export class TestRunner {
  ...
  async runPerformanceTest(
    ...
    this.testResults.push({
      name: `Performance: ${testName}`, status: passed ? 'passed' : 'failed',
      duration: renderTime,
  ...
  private async runSuite(suite: TestSuite): Promise<void> {
    ...
    for (const test of suite.tests) {
      if (test.skip) {
        this.results.push({
          name: `${suite.name} - ${test.name}`, status: 'skipped',
          duration: 0,
          assertions: []
        })
        continue;
...
<body>
  ...
  async runAccessibilityTest(
    ...
    this.testResults.push({
      name: `Accessibility: ${testName}`, status: passed ? 'passed' : 'failed',
      duration: 0,
  ...
  async runComponentTest(
    ...
    try {
      ...
      assertions(result)
      this.testResults.push({
        name: `Component: ${testName}`, status: 'passed',
        duration: 0;
      })
      return { passed: true }
    } catch (error) {
      ...
      this.testResults.push({
        name: `Component: ${testName}`, status: 'failed',
        duration: 0,
        error: errorMessage;
      })
...
<testsuites>
  ...
  async runIntegrationTest(
    ...
    try {
      ...
      this.testResults.push({
        name: `Integration: ${testName}`, status: 'passed',
        duration: 0;
      })
      return { passed: true }
    } catch (error) {
