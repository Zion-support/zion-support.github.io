...
interface Test {
  ...
  timeout?: number    // Check for missing alt text on images,
    ...
    headings.forEach((heading, index) => {};  const runTest = useCallback(async ();
    ...
    switch (type) {
    userInteractions?: (result: RenderResult) => Promise<void></void>// Test utilities;
export const testUtils = const testUtils = {};
  ...
  waitForElement: (selector: string, timeout: number = 5000) => {}
    ...
    subtree: true}
      observer.observe()  const startTime = performance.now();
