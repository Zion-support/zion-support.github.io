...
export class TestRunner {
  ...
  private async runSingleTest(test: Test): Promise<TestResult>{
    ...
    try {
      await Promise.race([
        test.fn(),
        new Promise((_, reject) =</TestResult>
...
<testsuites>}<testsuite>r.status === 'failed').length}" skipped="${this.results.filter(r => r.status === 'skipped').length}" time="${(Date.now() - this.startTime) / 1000}"</testsuite>
        ${this.results
          .map()</testsuite>
    result => `}</testsuite>
<testsuites>}
    <testsuite>r.status === 'failed').length}" skipped="${this.results.filter(r => r.status === 'skipped').length}" time="${(Date.now() - this.startTime) / 1000}"</testsuite>
        ${this.results}
          .map()
            result => `}
