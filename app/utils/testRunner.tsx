...
export class TestRunner {
  ...
  public async run(): Promise<TestResult[]> {
    ...
    try {
      ...
      if (import.meta.env.DEV) { console.log('🚀 Starting test run...'); }
  ...
  private async runTest(suite: TestSuite, test: Test): Promise<void> {
    ...
    try {
      ...
      if (this.config.verbose) {
        // eslint-disable-next-line no-console
        if (import.meta.env.DEV) { console.log(`✅ ${testName} (${Date.now() - startTime}ms)`); }
      }
    } catch (error) {
      this.results.push({
        name: testName,
  ...
  private generateReport(): void {
    ...
    if (import.meta.env.DEV) { console.log('\n📊 Test Results:'); }
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(`Total: ${this.results.length}`); }
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(`Passed: ${passed}`); }
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(`Failed: ${failed}`); }
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(`Skipped: ${skipped}`); }
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(`Duration: ${duration}ms`); }
  ...
  private generateJsonReport(): void {
    ...
    if (import.meta.env.DEV) { console.log('\n📄 JSON Report:'); }
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(JSON.stringify(report, null, 2)); }
  }
...
<body>
    ...
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(html); }
  }
...
<testsuites>
    ...
    if (import.meta.env.DEV) { console.log('\n📄 JUnit Report:'); }
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(xml); }
  }
  ...
  async runVisualRegressionTest(
    ...
    // eslint-disable-next-line no-console
    if (import.meta.env.DEV) { console.log(`Visual regression test for ${testName} would run here`); }
