...
export class TestRunner {
  ...
  public async run(): Promise<TestResult[]> {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') { console.log('🚀 Starting test run...'); }
  ...
  private async runTest(suite: TestSuite, test: Test): Promise<void> {
    ...
    try {
      ...
      if (this.config.verbose) {
        // eslint-disable-next-line no-console
        if (process.env.NODE_ENV === 'development') { console.log(`✅ ${testName} (${Date.now() - startTime}ms)`); }
      }
    } catch (error) {
      this.results.push({
        name: testName,
  ...
  private generateReport(): void {
    ...
    if (process.env.NODE_ENV === 'development') { console.log('\n📊 Test Results:'); }
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(`Total: ${this.results.length}`); }
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(`Passed: ${passed}`); }
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(`Failed: ${failed}`); }
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(`Skipped: ${skipped}`); }
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(`Duration: ${duration}ms`); }
  ...
  private generateJsonReport(): void {
    ...
    if (process.env.NODE_ENV === 'development') { console.log('\n📄 JSON Report:'); }
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(JSON.stringify(report, null, 2)); }
  }
...
<body>
    ...
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(html); }
  }
...
<testsuites>
    ...
    if (process.env.NODE_ENV === 'development') { console.log('\n📄 JUnit Report:'); }
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(xml); }
  }
  ...
  async runVisualRegressionTest(
    ...
    // eslint-disable-next-line no-console
    if (process.env.NODE_ENV === 'development') { console.log(`Visual regression test for ${testName} would run here`); }
