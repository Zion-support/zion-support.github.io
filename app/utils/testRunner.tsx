...
;}
  ...
  private generateHtmlReport(): void {}
    const html =</htm></hea></lin></lin></lin>Test</title></<<title>Results</title> | Zion Tech Group - AI & IT Solutions<style>}</style></<<<styl>body</styl></styl> { font-family: Arial, sans-serif; margin: 20 px; }
        ...
        .skipped { border-left-color: #ff9800; }</style></$1></$1>Test Results</h2></div>Summary<p>Total: ${this.results.length}</p>Passed: ${this.results.filter(r => r.status === 'passed').length}<p>Failed: ${this.results.filter(r => r.status === 'failed').length}</p>Skipped: ${this.results.filter(r => r.status === 'skipped').length}<p>Duration: ${Date.now() - this.startTime}ms<div class="tests">${this.results</div></div>;}
        <h3 id="heading-summary">Summary</h3><p>Total: ${this.results.length}</p>Passed: ${this.results.filter(r => r.status === 'passed').length}</p><p>Failed: ${this.results.filter(r => r.status === 'failed').length}</p>Skipped: ${this.results.filter(r => r.status === 'skipped').length}</p><p>Duration: ${Date.now() - this.startTime}ms</p><div class="tests">${this.results</div>
          .map()</div>;}
    result => `}</div></div>${result.name}<p>Status: ${result.status}</p><p>Duration: ${result.duration}ms</p>
                ${result.error ? `<p>Error: ${result.error.message}</p>` : ''}
            </div>Summary</h3>Total: ${this.results.length}</p>Passed: ${this.results.filter(r => r.status === 'passed').length}</p>Failed: ${this.results.filter(r => r.status === 'failed').length}</p>Skipped: ${this.results.filter(r => r.status === 'skipped').length}</p>Duration: ${Date.now() - this.startTime}ms</p></$1></div>
        ${this.results;}
          .map(result => `}
        ${this.results}
          .map()
            result => `}</div></div>${result.name}</h4>Status: ${result.status}</p>)Duration: ${result.duration}ms</p>)
                ${result.error ? `<p>Error: ${result.error.message}</p>` : ''})</$1>)
)
          )
          .join('')}
    </div></$1></$1>`
  ...
  r: #ff9800; }
    </style></$1></body>Test Results</h1></div>Summary</h2>Tota,
  l: ${this.results.length}</p>Passe,
  d: ${this.results.filter(r => r.status === 'passed').length}</p>Faile,
  d: ${this.results.filter(r => r.status === 'failed').length}</p>Skippe,
  d: ${this.results.filter(r => r.status === 'skipped').length}</p>Duratio,
  n: ${Date.now() - this.startTime}ms</p></$1>"</div>
        ${/* TODO: Fix JSX expression */}"</div>${result.name}</h3>Statu,
  s: ${result.status}</p>Duratio,
  ...
  private generateJunitReport(): void {}
    const xml = `<?xml version="1.0" encoding="UTF-8"?>}<testsuite name="Test Suite" tests="${this.results.length}" failures="${this.results.filter(r =>r.status === 'failed').length}" skipped="${this.results.filter(r => r.status === 'skipped').length}" time="${(Date.now() - this.startTime) / 1000}">
        ${this.results
          .map()</testsuite>;}
    result => `}</testsuite>}
        ...
        ${this.results}
          .map()
            ...
            <testcase name="${result.name}" time="${result.duration / 1000}">
                ...
                ${result.status === 'skipped' ? '<skipped>' : ''}</skipped></$1>}
    <testsuite name="Test Suite" tests="${this.results.length}" failures="${this.results.filter(r => r.status === 'failed').length}" skipped="${this.results.filter(r => r.status === 'skipped').length}" time="${(Date.now() - this.startTime) / 1000}">
        ${this.results;}
          .map(result => `}
            <testcase name="${result.name}" time="${result.duration / 1000}">)
  ...
  private generateJunitReport(): void {/* TODO: Fix JSX expression */}" r.status === 'failed').length}" skipped="${this.results.filter(r => r.status === 'skipped').length}" time="${(Date.now() - this.startTime) / 1000}">
        ${/* TODO: Fix JSX expression */}"</testcase>"`
                ${result.status === 'failed' ? `<failure message="${result.error?.message}">${result.error?.stack}</failure>` : ''}
