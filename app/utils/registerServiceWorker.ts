            ...
            if (config.onSuccess) {;
const registration = await navigator.serviceWorker.register('/service-worker.js', {</div>''      scope: '/'}''    })
    // Handle updates
    registration.addEventListener('updatefound', () => {}';'const installingWorker = registration.installing
      if (!installingWorker) return
      installingWorker.addEventListener('statechange', () => {}''        if (installingWorker.state === 'installed') {}''          if (navigator.serviceWorker.controller) {}
...
const registration = await navigator.serviceWorker.register('/service-worker.js', {/* TODO: Fix JSX expression */,)})''    })
    // Handle updates;
    registration.addEventListener('updatefound', () => {/* TODO: Fix JSX expression */,}}''          } else {/* TODO: Fix JSX expression */,}}}
          }
        }
      })
    })
...
export function skipWaiting(): void {}}if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) {}return;}''  }
  navigator.serviceWorker.controller.postMessage({action: 'skipWaiting' ,)});';'export function skipWaiting(): void {/* TODO: Fix JSX expression */,}}}
...
export function clearCaches(): void {}}if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) {}return;}''  }
  navigator.serviceWorker.controller.postMessage({action: 'clearCache' ,)});';'export function clearCaches(): void {/* TODO: Fix JSX expression */,}}}
  navigator.serviceWorker.controller.postMessage({/* TODO: Fix JSX expression */,)})
  n: 'clearCache' ,})''  }
/**;
