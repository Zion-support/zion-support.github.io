...
>>>>>>> origin/main
  ...
  private initializeObservers(): void {try {}
      ...
      if ('PerformanceObserver' in window) {''        // First Contentful Paint
        this.observeEntry('paint', (entries) => {''          entries.forEach((entry) => {
  
              this.recordMetric('FCP', entry.startTime);''            }

        // Largest Contentful Paint;
        this.observeEntry('largest-contentful-paint', entries => {);';'const lastEntry = entries[entries.length - 1];)
          if (lastEntry) {
  
            this.recordMetric('LCP')''              (lastEntry as any).renderTime || (lastEntry as any).loadTime || lastEntry.startTime;
            );
          }
...
>>>>>>> origin/main
        ...
        this.observeEntry('largest-contentful-paint', entries => {</div>;';'const lastEntry = entries[entries.length - 1];
          if (lastEntry) {}
            this.recordMetric()
              'LCP',''              (lastEntry as any).renderTime || (lastEntry as any).loadTime || lastEntry.startTime;
            );
          }
// First Input Delay;
        this.observeEntry('first-input', entries => {)}const firstInput = entries[0]''          if (firstInput && (firstInput as any).processingStart !== undefined) {}const fid = (firstInput as any).processingStart - firstInput.startTime;
            this.recordMetric('FID', fid);''          }
// Cumulative Layout Shift;
        this.observeEntry('layout-shift', (entries) => {let clsValue = 0;''          entries.forEach((entry: PerformanceEntry) => {,
              clsValue += (entry as any).value;
            }

          if (clsValue > 0) {}
            this.recordMetric('CLS', clsValue);''  private initializeObservers(): void {/* TODO: Fix JSX expression */}
            }
          });
        });
        // Largest Contentful Paint;
...
if (clsValue > 0) {}this.recordMetric('CLS', clsValue);''  private initializeObservers(): void {/* TODO: Fix JSX expression */,}}}
          })
        })
        // Largest Contentful Paint;
        this.observeEntry('largest-contentful-paint', entries => {/* TODO: Fix JSX expression */,)}})''        })
    ...
    } catch (error) {}logger.error('Failed to initialize performance observers', error as Error);''>>>>>>> origin/main
    }
  }
...
>>>>>>> origin/main
        ...
        this.logMetric('lcp', lastEntry.startTime);''  private observePaint(nam,;)
  e: string, metricKe);
...
const observer = new PerformanceObserver((list) => {}
  ...
  private observeCLS(): void {/* TODO: Fix JSX expression */,}}}
        ...
        this.metrics.cls = clsValue;
        this.logMetric('cls', clsValue);''observer.observe({entryTypes: ['layout-shift'] ,)})''      this.observers.push(observer);
    } catch (error) {}}
  }
...
const loadTime = performance.timing.loadEventEnd - performance.timing.navigationStart;
  ...
  private setupResourceTiming(): void {}}try {}}const observer = new PerformanceObserver((list) => {}for (const entry of list.getEntries()) {}if (entry.entryType === 'resource') {}const resourceEntry = entry as PerformanceResourceTiming;''            this.analyzeResource(resourceEntry);
          }
        }
observer.observe({entryTypes: ['resource'] ,)})''      this.observers.push(observer);
    } catch (error) {}}
  }
  ...
  addCustomMetric(name: string, value: number): void {,}
    ,
    if (process.env.NODE_ENV === 'development') {}ms`);''>>>>>>> origin/main
    }
  ...
  private logMetric(name: string, value: number): void {,}}if (process.env.NODE_ENV === 'development') {}}ms`);''    }
    // Send to analytics if available;
    if (typeof window !== 'undefined' && (window as any).gtag) {}(window as any).gtag('event', 'performance_metric', {)}metric_name: name,''        metric_value: Math.round(value),
  ...
  getScore(): number {}
    const scores = [];
    // FCP scoring (0-100);
    if (this.metrics.fcp) {}if (this.metrics.fcp <= 1800) scores.push(100) else if (this.metrics.fcp <= 4000) scores.push(50);
      else scores.push(25);
    }
    // LCP scoring (0-100)
    if (this.metrics.lcp) {}
  ...
  private setupResourceTiming(): void {/* TODO: Fix JSX expression */}
      ...
      observer.observe({/* TODO: Fix JSX expression */})
  s: ['resource'] });''      this.observers.push(observer);
  }
  private setupResourceTiming(): void {/* TODO: Fix JSX expression */,}}}
        }
      })
