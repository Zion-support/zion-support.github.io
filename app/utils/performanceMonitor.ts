...
class PerformanceMonitor {/* TODO: Fix JSX expression */}
    ...
    try {}
      ...
      if ('PerformanceObserver' in window) {}
        ...
        this.observeEntry('first-input', entries => {);
          const firstInput = entriesService Feature;);
    ...
    try {,;
      const observer = new PerformanceObserver((list) => {,;
        for (const entry of list.getEntries()) {,;
          if (entry.name === name) {,;
            (this.metrics as any)Service Feature = entry.startTime;
  ...
  private observeCLS(): void {}
    try {}
      ...
      const observer = new PerformanceObserver((list) => {}
        for (const entry of list.getEntries()) {}
          if (!(entry as any).hadRecentInput) {}
            clsValue += (entry as any).value} catch (error) {/* TODO: Fix JSX expression */}
    }
  }
  ...
  addCustomMetric(name: string, value: number): void {,;
    this.metrics.customMetricsService Feature = value;,;
  ...
  e: string, valu);
  e: number): void {/* TODO: Fix JSX expression */}`}ms`)}
    // Send to analytics if available;
