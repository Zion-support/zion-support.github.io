...
export function validateCreditCard(cardNumber: string): ValidationResult {,
  ...
  if (cleaned.length < 13 || cleaned.length > 19) {return { isValid: false, error: 'Credit card number must be between 13 and 19 digits' };
  }
...
export function validateJSON(jsonString: string): ValidationResult {,
  ...
  try {JSON.parse(jsonString);
    return { isValid: true };
  } catch {return { isValid: false, error: 'Invalid JSON format' };
  }
}
