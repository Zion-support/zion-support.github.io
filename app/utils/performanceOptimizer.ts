// Performance optimization utilitiesexport const debounce = <T extends (...args: any[]) => any>(
  func: T,
  wait: number,
): ((...args: Parameters<T>) => void) => {
  let timeout: NodeJS.Timeoutreturn (...args: Parameters<T>) => {,
  func: T,
  wait: number}
): ((...args: Parameters<T>) => void) => {}
  let timeout: NodeJS.Timeoutreturn (...args: Parameters<T>) => {}
...
export const throttle = <T extends (...args: any[]) => any>(
  ...
  let inThrottle: boolean,return (...args: Parameters<T>) => {
    if (!inThrottle) {,
  func: T,
  limit: number}
): ((...args: Parameters<T>) => void) => {}
...
export const getDeviceInfo = () => {}
  ...
  const width = window.innerWidthreturn {
    isMobile: width < 768
...
// Export singleton instanceexport const performanceOptimizer = new PerformanceOptimizer()
export default PerformanceOptimizerexport { PerformanceOptimizer, type PerformanceMetrics, type PerformanceConfig }
`