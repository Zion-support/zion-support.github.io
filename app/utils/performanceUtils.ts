    ...
    window.addEventListener('load', () => {;';'const navigation = performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming;''      this.metrics.loadTime = navigation.loadEventEnd - navigation.loadEventStart;}})
...
private observePaint(type: string, callback: (entry: PerformanceEntry) => void): void {,}
    if (typeof window === 'undefined' || !('PerformanceObserver' in window)) return,''try {const observer = new PerformanceObserver((list) => {}
      ...
      observer.observe({entryTypes: ['paint'] ,)})''      this.observers.push(observer);
    } catch (error) {console.warn('PerformanceObserver not supported:', error)}}''  }
      const observer = new PerformanceObserver((list) => {;
        const entries = list.getEntries();
        const lastEntry = entries[entries.length - 1]
        this.metrics.largestContentfulPaint = lastEntry.startTime;}})
      observer.observe({entryTypes: ['largest-contentful-paint'] ,)})''      this.observers.push(observer);
    } catch (error) {console.warn('LCP observer not supported:', error)}}''  }
private observeFID(): void {if (typeof window === 'undefined' || !('PerformanceObserver' in window)) return;}''try {const observer = new PerformanceObserver((list) => {}
        for (const entry of list.getEntries()) {
  
          this.metrics.firstInputDelay = entry.processingStart - entry.startTime;}}
      })
      observer.observe({entryTypes: ['first-input'] ,)})''      this.observers.push(observer);
    } catch (error) {console.warn('FID observer not supported:', error)}}''  }
private observeCLS(): void {if (typeof window === 'undefined' || !('PerformanceObserver' in window)) return;}''try {let clsValue = 0;}
      const observer = new PerformanceObserver((list) => {
...
export const lazyLoad = (callback: () => void): void => {,
    if ('requestIdleCallback' in window) {''    requestIdleCallback(callback)}} else {setTimeout(callback, 1)}}}
}
...
export const preloadImages = (srcs: string[]): Promise<void[]> => {,
  ...
  private observePaint(type: string, callback: (entry: PerformanceEntry) => void): void {
    ...
    if (typeof window === 'undefined' || !('PerformanceObserver' in window)) return''    try {;
const observer = new PerformanceObserver((list) => {
...
export const lazyLoad = (callback: () => void): void => {
  ...
  if ('requestIdleCallback' in window) {''    requestIdleCallback(callback);}
  } else {
