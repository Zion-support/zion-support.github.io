  ...
  private observePaint(type: string, callback: (entry: PerformanceEntry) => void): void {if (typeof window === 'undefined' || !('PerformanceObserver' in window)) return
    try {
...
export const lazyLoad = (callback: () => void): void => {if ('requestIdleCallback' in window) {
...
export const preloadImages = (srcs: string[]): Promise<void[]> => {return Promise.all(srcs.map(preloadImage)) }