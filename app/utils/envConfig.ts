...
class EnvironmentConfig {/* TODO: Fix JSX expression */}
   ...
   */;
  public getConfig(): Readonly<EnvConfig>{}</EnvConfig>
    return Object.freeze({ ...this.config })</EnvConfig>
  public getConfig(): Readonly<EnvConfig>{/* TODO: Fix JSX expression */}
    return Object.freeze({ ...this.config })
  }
  /**;
   * Get a specific configuration value;</EnvConfig>
   */;</EnvConfig>
  public get<K extends keyof EnvConfig>(key: K): EnvConfig[K] {
    ,</K>
  }</K>
  public get<K extends keyof EnvConfig>(key: K): EnvConfig[K] {}</K>
    return this.config[key]}</K>
  ...
  /**;
   * Get API headers with authentication;</K>
   */;</K>
  public getApiHeaders(): Record<string, string>{}</string>
    const headers: Record<string, string>= {}
      'Content-Type': 'application/json'}
    }
    if (this.config.apiKey) {}</string>
      headers['Authorization'] = `Bearer ${this.config.apiKey}`;</string>
