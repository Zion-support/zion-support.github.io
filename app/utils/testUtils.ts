...
export const createMockPerformance = (): Performance = () => {
  ...
  return {
    ...
    getEntriesByName: (nam, e: string) => entries.filter(e = () => e.name === name),
    getEntriesByType: (typ, e: string) => entries.filter(e = () => e.entryType === type),
    getEntries: () => entries
    clearMark, s: () => {
  entries.length = 0
}
...
export interface Deferred<T> {</T>
  promise: Promise<T>,
    resolve: (valu, e: T) => void, reject: (reason?: unknown) => void}</T>
