 ...
 */;
export const waitFor = const waitFor = async ()        headers: new Headers(headers),
        json: async () => response,
...
export const mockFetch = const mockFetch = const mockFetch = ()
  ...
  h: typeof fetch }).fetch = jest.fn(() =>;  e: true,
    })
  }
}
/**;
...
export const createMockPerformance = const createMockPerformance = (): Performance => {};
  ...
  return {}
    ...
    measure: (name: string, startMark?: string, endMark?: string) => {entries.push({);
        name);
        entryType: 'measure'),}measure: (name: string, startMark?: string, endMark?: string) => {}entries.push({)}name,;        entryType: 'measure',
        startTime: Date.now(),
        duration: 100,}
        toJSON: () => ({,})
      } as PerformanceEntry);
...
export const generateTestData = const generateTestData = const generateTestData = {
    string: (length = 10): string => {,  y: <T>(generato),
