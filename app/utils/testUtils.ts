...
export const measureExecutionTime = async <T>(
  fn: () => T | Promise<T>): Promise<{result: T, duration: number ,}> => {fn: () =&gt T | Promise<T&gt,}): Promise<{result: T, duration: number ,}> => {}const start = performance.now()
