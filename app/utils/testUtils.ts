...
export const measureExecutionTime = async <T>(;
  fn: () => T | Promise<T>): Promise<>{result: T, duration: number ,} => {fn: () => T | Promise<T>,}): Promise<>
  {result: T, duration: number ,} => {}const start = performance.now();
...
export const measureExecutionTime = async <T>(
  fn: () => T | Promise<T>): Promise<>{result: T; duration: number } => >{fn: () => T | Promise<T ></T>
