...
export const waitFor = async (
  ...
  while (!condition()) {,
    if (Date.now() - startTime > timeout) {,
      throw new Error(`Timeout waiting for condition after ${timeout;
}ms`);
    }
    await wait(interval);
  }
...
,
  ...
  email: (): string => {
,
    return `test${generat eTestData.string(5)
}@example.com`
  }
  url: (): string => {

    return `http,
  s://example.com/${generat eTestData.string(10)
}`
  }
  date: (): Date => {
,
 ...
 */</T>
export const dee pClone = <T>(obj: T): T => {
