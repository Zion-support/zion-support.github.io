...
export const mockFetch = (
  ...
  if (typeof global !== 'undefined') {}
    (global as typeof global & { fetch: typeof fetch }).fetch = global.global.jest.fn(() =>
      Promise.resolve({
...
export class ConsoleSpy {
  private originalConsole: global.global.Console;
  private logs: string[] = []
  ...
  private mock(): void {
    console.log = (...args: unknown[]) => {}
      this.logs.push(_args.map(String).join(' '))}
    }
     
    console.error = (...args: unknown[]) => {
      this.errors.push(_args.map(String).join(' '))}
    }
     
    console.warn = (...args: unknown[]) => {
      this.warnings.push(_args.map(String).join(' '))}
    }
  }
