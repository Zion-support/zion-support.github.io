'use client';

import React from 'react';
import Link from 'next/link';
import Navigation from '../components/Navigation';
import Footer from '../components/Footer';

export default function AutomationWorkflowPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <Navigation />
      
      <main className="relative z-10 py-20 px-4">
        <div className="max-w-7xl mx-auto">
          {/* Hero Section */}
          <section className="text-center mb-16">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-white mb-6 quantum-gradient">
              Workflow Automation Suite
            </h1>
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">
              Automate repetitive tasks and create complex workflows without coding. 
              Boost productivity and reduce manual work with intelligent automation.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <div className="cyber-card px-6 py-3 text-white">
                <span className="text-2xl font-bold text-cyan-400">$39</span>
                <span className="text-gray-400 ml-2">/month</span>
              </div>
              <a 
                href="tel:+13024640950" 
                className="quantum-card px-6 py-3 text-white hover:text-cyan-400 transition-all duration-300"
              >
                📞 Call Now: +1 (302) 464-0950
              </a>
            </div>
          </section>

          {/* Features Section */}
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-white mb-8 text-center quantum-gradient">
              Powerful Automation Features
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                {
                  icon: '🎨',
                  title: 'Visual Workflow Builder',
                  description: 'Drag-and-drop interface to create complex workflows without coding'
                },
                {
                  icon: '🔌',
                  title: '500+ Integrations',
                  description: 'Connect with popular tools like Slack, Salesforce, HubSpot, and more'
                },
                {
                  icon: '⚡',
                  title: 'Custom Triggers',
                  description: 'Set up triggers based on time, events, or data changes'
                },
                {
                  icon: '👥',
                  title: 'Team Collaboration',
                  description: 'Share workflows with your team and collaborate in real-time'
                },
                {
                  icon: '📊',
                  title: 'Analytics & Monitoring',
                  description: 'Track workflow performance and identify optimization opportunities'
                },
                {
                  icon: '🛡️',
                  title: 'Enterprise Security',
                  description: 'Bank-level security with role-based access control'
                }
              ].map((feature, index) => (
                <div key={index} className="quantum-card p-6 hover:scale-105 transition-all duration-300">
                  <div className="text-4xl mb-4">{feature.icon}</div>
                  <h3 className="text-xl font-bold text-white mb-3">{feature.title}</h3>
                  <p className="text-gray-300">{feature.description}</p>
                </div>
              ))}
            </div>
          </section>

          {/* Use Cases Section */}
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-white mb-8 text-center quantum-gradient">
              Automation Use Cases
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="cyber-card p-8">
                <h3 className="text-2xl font-bold text-white mb-4">Sales & Marketing</h3>
                <ul className="space-y-3 text-gray-300">
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Lead scoring and qualification
                  </li>
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Email campaign automation
                  </li>
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    CRM data synchronization
                  </li>
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Social media posting
                  </li>
                </ul>
              </div>
              <div className="cyber-card p-8">
                <h3 className="text-2xl font-bold text-white mb-4">Operations & HR</h3>
                <ul className="space-y-3 text-gray-300">
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Employee onboarding
                  </li>
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Invoice processing
                  </li>
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Document approval workflows
                  </li>
                  <li className="flex items-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Inventory management
                  </li>
                </ul>
              </div>
            </div>
          </section>

          {/* Pricing Section */}
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-white mb-8 text-center quantum-gradient">
              Simple Pricing
            </h2>
            <div className="max-w-4xl mx-auto">
              <div className="hologram-card p-8 text-center">
                <h3 className="text-2xl font-bold text-white mb-4">Automation Pro</h3>
                <div className="text-5xl font-bold text-cyan-400 mb-4">$39<span className="text-xl text-gray-400">/month</span></div>
                <ul className="space-y-3 text-gray-300 mb-8">
                  <li className="flex items-center justify-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Visual workflow builder
                  </li>
                  <li className="flex items-center justify-center">
                    <span className="text-green-400 mr-3">✓</span>
                    500+ integrations
                  </li>
                  <li className="flex items-center justify-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Custom triggers
                  </li>
                  <li className="flex items-center justify-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Team collaboration
                  </li>
                  <li className="flex items-center justify-center">
                    <span className="text-green-400 mr-3">✓</span>
                    Analytics dashboard
                  </li>
                  <li className="flex items-center justify-center">
                    <span className="text-green-400 mr-3">✓</span>
                    24/7 support
                  </li>
                </ul>
                <a 
                  href="tel:+13024640950" 
                  className="quantum-card px-8 py-4 text-white hover:text-cyan-400 transition-all duration-300 inline-block"
                >
                  Start Automating Today
                </a>
              </div>
            </div>
          </section>

          {/* Contact Section */}
          <section className="text-center">
            <h2 className="text-3xl font-bold text-white mb-8 quantum-gradient">
              Ready to Automate Your Business?
            </h2>
            <p className="text-lg text-gray-300 mb-8 max-w-2xl mx-auto">
              Join thousands of businesses already saving time and money with intelligent automation. 
              Get started today and see the results.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a 
                href="tel:+13024640950" 
                className="cyber-card px-8 py-4 text-white hover:text-cyan-400 transition-all duration-300"
              >
                📞 Call: +1 (302) 464-0950
              </a>
              <a 
                href="mailto:kleber@ziontechgroup.com" 
                className="quantum-card px-8 py-4 text-white hover:text-cyan-400 transition-all duration-300"
              >
                ✉️ Email: kleber@ziontechgroup.com
              </a>
            </div>
          </section>
        </div>
      </main>

      <Footer />
    </div>
  );
}