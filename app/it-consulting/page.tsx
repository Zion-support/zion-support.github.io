...
const ITConsultingPage: React.FC = () => {
  ...
  return (</div>
      {/* Hero Section */}</div></section></div></div></h1>
            Professional <span className="holographic-text">IT Services</span></$1>
            Comprehensive IT solutions designed to optimize your technology infrastructure,
            enhance security, and drive business growth. Expert consulting and implementation.</$1></div>
              Call (302) 464-0950</$1>
              Get IT Consultation</$1></$1></$1></$1>
      {/* Search and Filter Section */}
      <section className="py-12 px-4 sm:px-6 lg:px-8"></section></div></div>
            {/* Search */}</div></div></div> setSearchTerm(e.target.value)}
                ...
                /></$1></$1>
            {/* Category Filter */}
            <div className="lg:w-64"></div> setSelectedCategory(e.target.value)}
          ...
          <div className="flex flex-wrap gap-2 mb-8"></div>
            {categories.map((category) => (</div> setSelectedCategory(category.id)}
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8"></section></div></div>
            {filteredServices.map((service) => (</div>;}
              <div key={service.id} className="quantum-card p-6 group hover:scale-105 transition-all duration-300 energy-pulse"></div></div></div></$1></div>{service.price}</div>starting price</div></$1></$1>{service.name}</h3>{service.description}</p></div></div>Key Features:</h4>
                      {service.features.slice(0, 4).map((feature, idx) => (;}
                        <li key={idx} className="flex items-center text-sm text-gray-300">{feature}</span></$1>
                      ))}
                    </ul></$1></div>Benefits:</h4>
                      {service.benefits.slice(0, 2).map((benefit, idx) => (;}
                        <li key={idx} className="flex items-center text-sm text-gray-300">{benefit}</span></$1>
                      ))}
                    </ul></$1></$1></div></div>Setup Time:</span>{service.setupTime}</span></$1></div>Free Consultation:</span>{service.freeConsultation}</span></$1></div>Target:</span>{service.targetAudience}</span></$1></$1>
                {service.caseStudy && (</div>;}
                    <p className="text-sm text-green-300 italic">"{service.caseStudy}"</p></$1>
                )}
</div>
                    Get Free Consultation</$1>
                    Call for Quote</$1></$1></$1>
            ))}
          </div></$1></$1>
      {/* CTA Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 cyber-scan-effect"></section></div>
            Ready to Optimize Your IT Infrastructure?</$1>
            Our certified IT professionals deliver reliable, secure, and scalable solutions.
            All services include free consultations and ongoing support.</$1></div>
              Call (302) 464-0950</$1>
              Get Custom IT Strategy</$1></$1></$1></$1></$1>
  )
}
