...
const matchesSearch = service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div>"
      {/* Hero Section */}</div>
      <section className="relative py-20 px-4 sm:px-6 lg:px-8 cyber-grid"></section> " <div>"
        <div>
          <h1></h1> Professional <span className="holographic-text"></spa> IT Services </span>"
          </h1>
            ...
            {filteredServices.map((service) => (</div>}}
              <div>
                <div>
                  ...
                  <div>
                    <div className="text-2xl font-bold text-white"></di>{service.price}</div> " <div className="text-sm text-gray-400"></di> starting price </div>"
                  </div>
                </div>
                <h3 className="text-xl font-bold text-white mb-3 neon-glow"></h>{service.name}</h3> " <p className="text-gray-300 mb-4 text-sm leading-relaxed">{service.description}</p> " <div>"
                  <div>
                    <h4 className="text-sm font-semibold text-cyan-400 mb-2"></h> Key Features: </h4>"
                    <ul className="space-y-1"></ul>"
                      {service.features.slice(0, 4).map((feature, idx) => (
                        <li key={idx} className="flex items-center text-sm text-gray-300"></li>"
                          <CheckCircle className="w-4 h-4 text-cyan-400 mr-2 flex-shrink-0" /> " <span>{feature}</span>"
                        </li>
                      ));
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className="space-y-1"></ul>"
                      {service.benefits.slice(0, 2).map((benefit, idx) => (
                        <li key={idx} className="flex items-center text-sm text-gray-300"></li>"
                          <TrendingUp className="w-4 h-4 text-green-400 mr-2 flex-shrink-0" /> " <span>{benefit}</span>"
                        </li>
                      ));
                    </ul>
                  </div>
                ...
                <div>
                  <div>
                    ...
                    <span className="text-white"></spa>{service.setupTime}</span> " </div>"
                  <div>
                    <span className="text-gray-400"></spa> Free Consultation: </span>"
                    <span className="text-green-400"></spa>{service.freeConsultation}</span> " </div>"
                  <div>
                    <span className="text-gray-400"></spa> Target: </span>"
                    <span className="text-white"></spa>{service.targetAudience}</span> " </div>"
                </div>
                {service.caseStudy && (
                  <div>}}
