...
return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div>
      ...
      </section>{/* Search and Filter Section */}<//section>
      <section>
          <div>
    ...
    categories.map((category) => (</div>
              <button
                ...
                className={
    `px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${</button
  }
                key={category.id}
                onClick={() =>
                  selectedCategory === category.id;</button
  }
                key={category.id}
                onClick={() =>
                    ? 'bg-cyan-500 text-white'</button
  }
                key={category.id}
                onClick={() =>: 'bg-slate-800/50 text-gray-300 hover:bg-slate-700/50 hover:text-cyan-400'
  }<//button
  }
                key={category.id}
                onClick={() =>
    ...
    filteredServices.map((service) => (</div>}<//div>
              <div key={service.id} className="quantum-card p-6 group hover:scale-105 transition-all duration-300 energy-pulse"></div>
                ...
                <div className="space-y-3 mb-6"></div>
                  <div></div>
                    ...
                    <ul>{
    service.features.slice(0, 4).map((feature, idx) => (</ul>
  }</ul>
                        <li>
                          <CheckCircle>
                          <span>{feature} ))}</span>
                  ...
                  <div></div>
                    ...
                    <ul>{
    service.benefits.slice(0, 2).map((benefit, idx) => (</ul>
  }</ul>
                        <li>
                          <TrendingUp>
                          <span>{benefit} ))}</span>
...
export default ITConsultingPage<//div>