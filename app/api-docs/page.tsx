...
export default function APIDocsPage() {
  ...
  ]</string | null>const filteredEndpoints = apiEndpoints.map(category => ({
    ...category,
    endpoints: category.endpoints.filter(endpoint => 
      endpoint.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      endpoint.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
      endpoint.path.toLowerCase().includes(searchQuery.toLowerCase())<//string | null>
    )<//string | null>
  })).filter(category => category.endpoints.length > 0)</string | null>return (<//string | null>
...
<section className="py-20 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-5xl font-bold text-white mb-6">API Documentation Complete API documentation for Zion Tech Group's AI and IT solutions. </h1 className="text-5xl font-bold text-white mb-6">
            Explore endpoints, authentication, and integration guides.</h1 className="text-5xl font-bold text-white mb-6">
          </p>
          <div className="flex flex-col sm: flex-row gap-4 justify-center">
            <button className="bg-gradient-to-r from-purple-600 to-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:from-purple-700 hover:to-blue-700 transition-all duration-300 flex items-center justify-center">
              <Code className="mr-2 h-5 w-5" />Get Started</Code className="mr-2 h-5 w-5" />
  </
            <button className="border border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white hover:text-purple-600 transition-all duration-300 flex items-center justify-center">
              <Download className="mr-2 h-5 w-5" />Download SDK,</Download className="mr-2 h-5 w-5" />
  </
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
              ...
              <button className="text-purple-400 hover:text-purple-300 font-medium">Get API Key →</button className="text-purple-400 hover:text-purple-300 font-medium">
              </button>
            </div>
            <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
              <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-blue-600 rounded-lg flex items-center justify-center mb-4">
              ...
              <button className="text-purple-400 hover:text-purple-300 font-medium">View Examples →</button className="text-purple-400 hover:text-purple-300 font-medium">
              </button>
            </div>
            <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
              <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-blue-600 rounded-lg flex items-center justify-center mb-4">
              ...
              <button className="text-purple-400 hover:text-purple-300 font-medium">Start Building →</button className="text-purple-400 hover:text-purple-300 font-medium">
              </button>
            </div>
      ...
      <section className="py-16 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="space-y-8">{filteredEndpoints.map((category, categoryIndex) => (</div className="space-y-8">
              <div key={categoryIndex} className="bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10">
                ...
                <div className="space-y-6">{category.endpoints.map((endpoint, endpointIndex) => (</div className="space-y-6">
                    <div key={endpointIndex} className="bg-white/5 rounded-xl p-6 border border-white/10">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-4">
                          ...
                          }`}>{endpoint.method}</span className={`px-3 py-1 rounded-full text-sm font-medium ${
                            endpoint.method === 'POST' 
                              ? 'bg-green-500/20 text-green-400' 
                              : 'bg-blue-500/20 text-blue-400'
                          }`}>
                        ...
                        <button
                          ...
                          onClick={() =>
                            <React.Fragment>
                              <Check className="w-4 h-4" />Copied! ) : (</Check className="w-4 h-4" />
                            <React.Fragment>
                              <Copy className="w-4 h-4" />Copy )}</Copy className="w-4 h-4" />
                        </button>
                      </div>
      ...
      <section className="py-20 px-4 bg-gradient-to-r from-purple-600 to-blue-700">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl text-purple-100 mb-8">Start building with our APIs today and create amazing applications.</p className="text-xl text-purple-100 mb-8">
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button className="bg-white text-purple-600 px-8 py-4 rounded-xl font-semibold hover:bg-gray-100 transition-colors">Get API Key View SDKs</button className="bg-white text-purple-600 px-8 py-4 rounded-xl font-semibold hover:bg-gray-100 transition-colors">
  </
          </div>
      </section>
      <Footer />
