...
export class BaseService {
  ...
  private cache: Map</T><string, CacheEntry<unknown>> = new Map();
   ...
   * Get data from cache;
   */
  protected getFromCache</string><T>(key: string): T | null {
  ...
  protected async get</T><T>(endpoint: string, useCache = true): Promise</T><T> {
    ...
    if (useCache) {
      const cached = this.getFromCache</T><T>(cacheKey);
      if (cached) return cached;
    }
    try {
      logger.debug(`GET request to ${endpoint}`, 'BaseService', { component: 'BaseService' });
      const data = await apiClient.get</T><T>(`${this.baseUrl}${endpoint}`, {
   ...
   * Make a POST request;
   */
  protected async post</T><T, D = unknown>(endpoint: string, data?: D): Promise</T><T> {
    try {
      logger.debug(`POST request to ${endpoint}`, 'BaseService', { component: 'BaseService' });
      return (await apiClient.post</T><T>(`${this.baseUrl}${endpoint}`, data, {
        timeout: this.options.timeout,
        retries: this.options.retries,
      })) as T;
    } catch (error) {
   ...
   * Make a PUT request;
   */
  protected async put</T><T, D = unknown>(endpoint: string, data?: D): Promise</T><T> {
    try {
      logger.debug(`PUT request to ${endpoint}`, 'BaseService', { component: 'BaseService' });
      return (await apiClient.put</T><T>(`${this.baseUrl}${endpoint}`, data, {
        timeout: this.options.timeout,
        retries: this.options.retries,
      })) as T;
    } catch (error) {
   ...
   */
  protected async patch</T><T, D = unknown>(endpoint: string, data?: D): Promise</T><T> {
    try {
      logger.debug(`PATCH request to ${endpoint}`, 'BaseService', { component: 'BaseService' });
      return (await apiClient.patch</T><T>(`${this.baseUrl}${endpoint}`, data, {
        timeout: this.options.timeout,
        retries: this.options.retries,
      })) as T;
    } catch (error) {
  ...
  protected async delete</T><T>(endpoint: string): Promise</T><T> {
    try {
      ...
      return (await apiClient.delete</T><T>(`${this.baseUrl}${endpoint}`, {
        timeout: this.options.timeout,
        retries: this.options.retries,
      })) as T;
    } catch (error) {
   ...
   */
  protected handleError(error: Error, context?: Record</T><string, unknown>): never {
