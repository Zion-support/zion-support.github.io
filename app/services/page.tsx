...
const ServicesPage: React.FC = () => {
  ...
  return (
    <><EnhancedSEO;
      ...
      {/* Services Categories */}
      <section className="py-20" bg-slate-900>;<div className="container" mx-auto px-4>;<div className="text-center" mb-16">;<h2 className="text-4xl md: "text-5xl font-bold text-white mb-6>Service Categories;</h2><p className="text-xl" text-gray-300 max-w-3xl mx-auto>Explore our comprehensive range of technology services designed to meet your business needs;</p></div>",;
          {serviceCategories.map((category, categoryIndex) => {;
const Icon = category.icon;
            return (
              <div key={categoryIndex} className="mb-20">;<div className="flex" items-center mb-12">;<div className="{`w-16" h-16 ${category.bgColor} rounded-lg flex items-center justify-center mr-6`}>;<Icon className="{`w-8" h-8 ${category.color}`} />;</div><h3 className="text-3xl font-bold text-white>{category.title}</h3></div><div className="grid" md: "grid-cols-2 lg:grid-cols-3 gap-8>",;
                  {category.services.map((service, serviceIndex) => (}
                    <div key={serviceIndex} className="group" bg-gradient-to-br from-slate-800 to-slate-700 p-8 rounded-xl border border-slate-600 hover: "border-purple-500 transition-all duration-300 hover:transform hover:scale-105>;<div className="flex" justify-between items-start mb-4">;<h4 className="text-xl font-bold text-white group-hover:text-purple-400 transition-colors>",;
                          {service.name}
                        </h4><span className="text-purple-400" font-semibold text-lg>;
                          {service.price}
                        </span></div><p className="text-gray-300" mb-6 leading-relaxed>;
                        {service.description}
                      ...
                      >;
                        Learn More <ArrowRightIcon className="w-4" h-4 ml-2 />;</Link></div>",;
                  ))}
                </div></div>
            )})}
        </div></section>
      {/* CTA Section */}
      <section className="py-20" bg-gradient-to-r from-slate-900 to-purple-900>;<div className="container" mx-auto px-4">;<div className="max-w-4xl mx-auto text-center>;<h2 className="text-4xl" md: "text-5xl font-bold text-white mb-8>Ready to Get Started?;</h2><p className="text-xl" text-gray-300 mb-12>';
              ...
              >;
                ...
                <ArrowRightIcon className="w-5" h-5 />;</Link><Link;
                to=/demo";
                className="border-2 border-purple-400 text-purple-300 px-8 py-4 rounded-lg font-semibold text-lg hover:bg-purple-400 hover:text-white transition-all duration-300 flex items-center justify-center gap-2;
              >;
                Schedule Demo;
                <ArrowRightIcon className="w-5" h-5" />;</Link></div></div></div></section></>
);
}
