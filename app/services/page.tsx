      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900" loading="lazy">>{/* Hero Section */}
        <section className="py-20 px-4 sm:px-6 lg:px-8" loading="lazy">
          <div className="max-w-7xl mx-auto text-center" loading="lazy">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6" loading="lazy">Our Services</h1>
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto" loading="lazy">Comprehensive AI and IT solutions designed to transform your business</p>
              and drive growth.
            </p>
          </div>
        </section>
        ...
        <section className="py-8 px-4" loading="lazy">
          <div className="max-w-6xl mx-auto" loading="lazy">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20" loading="lazy">
              <div className="flex flex-col md:flex-row gap-4 mb-6" loading="lazy">
                <div className="flex-1" loading="lazy">
                  <div className="relative" loading="lazy">
                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" loading="lazy" />
                    <input
                      type="text"
                      placeholder="Search services..."
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors" loading="lazy"
                    />
                  </div>
                </div>
                <div className="md:w-64" loading="lazy">
                  <div className="relative" loading="lazy">
                    <Filter className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" loading="lazy" />
                    <select
                      value={selectedCategory}
                      onChange={(e) => setSelectedCategory(e.target.value)}
                      className="w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors appearance-none" loading="lazy">
                      {categories.map((category) => (
                        <option key={category} value={category} className="bg-gray-800" loading="lazy">
                          {category}
                        </option>
                      ))}
                    </select>
              ...
              <div className="flex flex-wrap gap-2 justify-center" loading="lazy">{categories.slice(1).map((category) => (</div>
                  <button
                    key={category}
        ...
        <section className="py-20 px-4 sm:px-6 lg:px-8" loading="lazy">
          <div className="max-w-7xl mx-auto" loading="lazy">
            <div className="flex justify-between items-center mb-8" loading="lazy">
              <h2 className="text-3xl font-bold text-white" loading="lazy">{selectedCategory === 'All' ? 'All Services' : `${selectedCategory}`}</h2>
              <span className="text-gray-300" loading="lazy">{filteredServices.length} service{filteredServices.length !== 1 ? 's' : ''} found</span>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8" loading="lazy">{currentServices.map((service, index) => (</div>
                <div key={index} className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 hover:bg-white/20 transition-all duration-300 group relative" loading="lazy">{service.popular && (</div>
                    <div className="absolute -top-3 left-6 bg-gradient-to-r from-cyan-500 to-purple-500 text-white px-3 py-1 rounded-full text-sm font-medium" loading="lazy">Popular</div>
                  )}
                  <div className="flex items-start mb-6" loading="lazy">
                    <div className="bg-cyan-500/20 p-3 rounded-lg mr-4 group-hover:bg-cyan-500/30 transition-colors flex-shrink-0" loading="lazy">
                      <service.icon className="w-8 h-8 text-cyan-400" loading="lazy" />
                    </div>
                    <div className="flex-1" loading="lazy">
                      <h3 className="text-xl font-bold text-white mb-2" loading="lazy">{service.title}</h3>
                      <span className="inline-block bg-white/10 text-cyan-400 text-xs px-2 py-1 rounded-full mb-2" loading="lazy">{service.category}</span>
                    </div>
                  </div>
                  <p className="text-gray-300 mb-6 leading-relaxed" loading="lazy">{service.description}</p>
                  <div className="mb-6" loading="lazy">
                    <h4 className="text-sm font-semibold text-white mb-3" loading="lazy">Key Features:</h4>
                    <ul className="space-y-2" loading="lazy">
                      ...
                      {service.features.length > 4 && (
                        <li className="text-gray-400 text-sm" loading="lazy">
                  ...
                  <div className="mb-6" loading="lazy">
                    <div className="text-2xl font-bold text-cyan-400 mb-2" loading="lazy">{service.price}</div>
                    <div className="text-sm text-gray-300" loading="lazy">Contact for custom pricing</div>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3" loading="lazy">
                    <button className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-300 flex items-center justify-center group-hover:scale-105" loading="lazy" aria-label="Action button">Get Quote</button>
                      <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" loading="lazy" />
                    </button>
                    <button className="flex-1 border border-white/20 text-white font-semibold py-3 px-6 rounded-lg hover:bg-white/10 transition-all duration-300" loading="lazy" aria-label="Action button">Learn More</button>
                  </div>
                </div>
              ))}
            </div>
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="flex justify-center items-center mt-12 space-x-2" loading="lazy">
                <button
                  onClick={() = aria-label="Action button">setCurrentPage(prev => Math.max(prev - 1, 1))}</button>
                  disabled={currentPage === 1}
                  className="px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white hover:bg-white/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300" loading="lazy">
                  Previous
  </
                <div className="flex space-x-2" loading="lazy">{Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (</div>
                    <button
                      key={page}
                      onClick={() = aria-label="Action button">setCurrentPage(page)}</button>
                ...
                <button
                  onClick={() = aria-label="Action button">setCurrentPage(prev => Math.min(prev + 1, totalPages))}</button>
        ...
        <section className="py-20 px-4 sm:px-6 lg:px-8" loading="lazy">
          <div className="max-w-4xl mx-auto text-center" loading="lazy">
            ...
            <p className="text-xl text-gray-300 mb-8" loading="lazy">Let's discuss your project and find the perfect solution for your business needs.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center" loading="lazy">
              <button className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold py-4 px-8 rounded-lg transition-all duration-300 transform hover:scale-105" loading="lazy" aria-label="Action button">Get Free Consultation</button>
              <button className="border border-white/20 text-white font-semibold py-4 px-8 rounded-lg hover:bg-white/10 transition-all duration-300" loading="lazy" aria-label="Action button">View Portfolio</button>
            </div>
          </div>
        </section>
      </div>
