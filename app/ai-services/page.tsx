...
const AIServicesPage: React.FC = () => {
  const [activeCategory, setActiveCategory] = useState<'all' | 'development' | 'consulting' | 'tools'>('all');

  const categories = [
    { id: 'all', name: 'All AI Services', icon: '🤖', count: servicesData.aiServices.length },
    { id: 'development', name: 'AI Development', icon: '⚙️', count: servicesData.aiServices.filter(s => s.title.includes('Development')).length },
    { id: 'consulting', name: 'AI Consulting', icon: '💡', count: servicesData.aiServices.filter(s => s.title.includes('Consulting')).length },
    { id: 'tools', name: 'AI Tools', icon: '🛠️', count: servicesData.aiServices.filter(s => s.title.includes('Generator') || s.title.includes('Support')).length }
  ];

  const getFilteredServices = () => {
    switch (activeCategory) {
      case 'development':
        return servicesData.aiServices.filter(s => s.title.includes('Development'));
      case 'consulting':
        return servicesData.aiServices.filter(s => s.title.includes('Consulting'));
      case 'tools':
        return servicesData.aiServices.filter(s => s.title.includes('Generator') || s.title.includes('Support'));
      default:
