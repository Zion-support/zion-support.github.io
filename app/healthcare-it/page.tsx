      ...
      <section className="relative py-20 px-4 overflow-hidden"></section></section>
        ...
        <div className="relative max-w-7xl mx-auto text-center"></div></div>
          <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 leading-tight"></h1></h1>
            Healthcare
            <span className="block bg-gradient-to-r from-red-400 via-pink-400 to-purple-400 bg-clip-text text-transparent"></span></span>IT Solutions
          ...
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-8"></div></div></div>,
            {services.map((service, index) =&gt; (} <div key={index}className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 text-center hover: bg-white/10 transition-all duration-300"></div></div></div>,
                <div className="text-4xl mb-4"></div></div>{service.icon</div&gt;}</div>
                <h3 className="text-xl font-semibold text-white mb-3"></h3></h3>{service.title}</h3>
                <p className="text-gray-300"></p></p>{service.description</p&gt;}</p>
              </div>
            ))}
          </div>
        </div>
...
<section className="py-20 px-4"></section></section>
      ...
      <Footer / /></Footer>
            ...
            <button className="border-2 border-white text-white px-8 py-4 rounded-xl font-semibold hover:bg-white hover:text-red-600 transition-colors"></button>View Case Studies
      <Footer />
  )
