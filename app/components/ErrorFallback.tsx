...
const ErrorFallback: React.FC<ErrorFallbackProps /> = (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center px-4" /></div>
      <div className="max-w-2xl w-full bg-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20 text-center" /></div>
        ...
        {/* Error Details */}
        <details className="text-left mb-6 bg-black/20 rounded-lg p-4" /></details>
          <summary className="text-cyan-400 cursor-pointer mb-2 font-semibold flex items-center" /></summary>
          ...
          <div className="mt-3 space-y-2" /></div>
            <div /></div>
              ...
              <pre className="text-xs text-gray-400 bg-black/30 p-2 rounded mt-1 overflow-auto" /></pre>
                {error.message}
              </pre>
            </div>
        ...
        <div className="mt-8 pt-6 border-t border-white/10" /></div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center text-sm" /></div>
            <a 
