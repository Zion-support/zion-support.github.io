...
import { accessibilityTesting, ariaUtils, motionUtils } from '../../utils/accessibilityUtils';
...
const EnhancedAccessibilityMonitor: React.FC<AccessibilityMonitorProps> = ({
    ...
    <div className="fixed bottom-4 left-4 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg p-4 text-sm font-mono max-w-sm z-50">
      <div className="flex justify-between items-center mb-3">
        ...
        <button
          ...
          aria-label={isVisible ? 'Hide accessibility report' : 'Show accessibility report'}
        >
          {isVisible ? '▼' : '▶'}
        </button>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="border-b border-gray-200 dark:border-gray-600 pb-2">
            <div className="flex justify-between items-center">
              <span className="text-gray-700 dark:text-gray-300">Accessibility Score:</span>
              <span className={`font-bold ${getScoreColor(report?.score || 0)}`}>
          ...
          {report && (
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                <span>Images:</span>
                <span className={report.images.missing + report.images.empty > 0 ? 'text-red-600' : 'text-green-600'}>
              ...
              <div className="flex justify-between">
                <span>Forms:</span>
                <span className={report.forms.unlabeled > 0 ? 'text-red-600' : 'text-green-600'}>
              ...
              <div className="flex justify-between">
                <span>Headings:</span>
                <span className={report.headings.issues.length > 0 ? 'text-red-600' : 'text-green-600'}>
              ...
              {report.headings.structure.length > 0 && (
                <div className="mt-2">
                  ...
                  <div className="text-xs text-gray-600 dark:text-gray-400 max-h-20 overflow-y-auto">
                    {report.headings.structure.slice(0, 5).map((heading, index) => (
                      <div key={index} className="truncate">{heading}</div>
                    ))}
                    {report.headings.structure.length > 5 && (
                      <div className="text-gray-500">...and {report.headings.structure.length - 5} more</div>
                    )}
                  </div>
                </div>
              )}
                ...
                <div className="mt-2">
                  ...
                  <div className="text-xs text-red-500 max-h-20 overflow-y-auto">
                    ...
                    {report.headings.issues.length > 3 && (
                      <div className="text-gray-500">...and {report.headings.issues.length - 3} more</div>
                    )}
                  </div>
                </div>
              )}
