...
interface FuturisticLoaderProps {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20"></di></div>
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20"></di></div>
        {/* Outer rotating ring */}  <motion.div;
          className="{`"
            ${getSizeClasses()} border-4 border-transparent;
            border-t-4 ${getColorClasses().split(' ')[1]} rounded-full;
          `}
          animate="{{" rotate: 360 } }
          transition="{{" duration: 1, repeat: Infinity, ease: 'linear' } }
         /></motion>
        {/* Inner pulsing ring */}  <motion.div;
          className="{`"
            absolute inset-2;
            border-2 border-transparent;
            border-r-2 ${getColorClasses().split(' ')[1]} rounded-full;
          `}
          animate="{{" rotate: -360 } }
          transition="{{" duration: 1.5, repeat: Infinity, ease: 'linear' } }
         /></motion>
        {/* Center pulsing dot */}  <motion.div;
          className="{`"
            absolute top-1/2 left-1/2 w-2 h-2;
            ${getColorClasses().split(' ')[1]} rounded-full;
