...
import { Link, useLocation , ArrowRight, CheckCircle, Star, Brain, Zap, Globe, TrendingUp, Phone  } from "react-router-dom";
const ImprovedNavigation = () => {
  ...
  const Sidebar = () => (
    <div className={`fixed inset-0 z-50 lg:hidden ${isSidebarOpen ? 'block' : 'hidden'}`} /><div className="fixed inset-0 bg-black/50" onClick={() => setIsSidebarOpen(false)} />
      </div><div className="fixed right-0 top-0 h-full w-80 bg-slate-900/95 backdrop-blur-sm border-l border-white/10" /><div className="p-6" /><div className="flex items-center justify-between mb-8" /><h2 className="text-xl font-bold text-white">Menu</h2>
            <button;
          ...
          <div className="space-y-4">
            {navigationItems.map((item) => (
                  ...
                  onClick={() => setIsSidebarOpen(false)}
                >
                  </Link><div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${item.color} flex items-center justify-center`}>
                    {item.icon}</div>
                  </div>
                  <div /><div className="text-white font-medium">{item.label}</div>
          ...
          {/* Quick Links */}
          <div className="mt-8 pt-6 border-t border-white/10" /><h3 className="text-white font-medium mb-4">Quick Links</h3>
          ...
          {/* Contact Info */}
          <div className="mt-8 pt-6 border-t border-white/10" /><h3 className="text-white font-medium mb-4">Contact</h3>
  ...
  return (
    <React.Fragment /><header className="bg-black/20 backdrop-blur-sm border-b border-white/10 relative z-50" /><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" /><div className="flex justify-between items-center py-4">
            {/* Logo */}
              ...
              {navigationItems.map((item) => (
                ...
                >
                  ...
                  {activeDropdown === item.label && (
                    <div className="absolute top-full left-0 mt-2 w-80 bg-slate-800/95 backdrop-blur-sm border border-white/20 rounded-xl shadow-2xl z-50" /><div className="p-4" /><div className="flex items-center space-x-3 mb-4" /><div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${item.color} flex items-center justify-center`}>
                            {item.icon}</div>
                          </div>
                          <div /><h3 className="text-white font-semibold">{item.label}</h3>
                        ...
                        <div className="space-y-1 max-h-96 overflow-y-auto">
                          {item.dropdown.map((dropdownItem) => (
                             ...
                             /><div className="w-6 h-6 text-gray-400 group-hover:text-cyan-400 transition-colors">
                                {dropdownItem.icon}</div>
                              </div>
                              <div className="flex-1" /><div className="text-sm text-gray-300 group-hover:text-cyan-400 transition-colors">
            ...
            {/* Mobile Menu Buttons */}
            <div className="lg:hidden flex items-center space-x-2" /><button;
                onClick={() => setIsSidebarOpen(true)}
                className="text-gray-300 hover:text-cyan-400 transition-colors duration-300 p-2"
              >
                </button><Menu className="w-6 h-6" />
              </button>
            </div>
          </div>
          {/* Mobile Navigation */}
