...
;
interfaceContentStatisticsProps {stats?: StatItem[];
  animationDuration?: number;
  className?: string;
}
;
constContentStatistics: React.FC<ContentStatisticsProps /> = ({conststats = [{
      id: 'users',;
      value: 1200,;
      label: 'ActiveUsers',;
      icon: Users,;
  ...
  useEffect(() => {constanimateValue = (start: number, end: number, duration: number, key: string) => {
...
;
    stats.forEach(conststat = > {animateValue(0, stat.value, animationDuration, stat.id);
    });
