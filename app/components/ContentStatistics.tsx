      ...
      <div className="max-w-7xl mx-auto"></di></div>
        ...
        <div className="text-center mb-16"></di></div>
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6"></h2>
            Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400"></spa>Impact</span> in Numbers
          </h2>
        ...
        {/* Statistics Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16"></di></div>
          {statistics.map((stat, index) => (
            <div key={index} className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 hover:border-purple-400 transition-all duration-300 text-center group"></di></div>
              <div className={`w-16 h-16 ${stat.color} bg-white/10 rounded-full mx-auto mb-6 flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}></di></div>
                <stat.icon className="w-8 h-8" />
              </div>
              <div className={`text-4xl font-bold ${stat.color} mb-2`}></di></div>
                {stat.value.toLocaleString()}{stat.suffix}
              </div>
              <div className="text-lg font-semibold text-white mb-2"></di>{stat.label}</div>
              <div className="text-gray-400 text-sm"></di>{stat.description}</div>
            </div>
          ))}
        </div>

        {/* Features Section */}
        <div className="mb-16"></di></div>
          <div className="text-center mb-12"></di></div>
            <h3 className="text-2xl font-bold text-white mb-4"></h>Why We're the Right Choice</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8"></di></div>
            {features.map((feature, index) => (
              <div key={index} className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 hover:border-purple-400 transition-all duration-300 group"></di></div>
                <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-blue-500 rounded-lg mb-4 flex items-center justify-center group-hover:scale-110 transition-transform duration-300"></di></div>
                ...
                <div className="space-y-2"></di></div>
                  {feature.stats.map((stat, statIndex) => (
                    <div key={statIndex} className="flex items-center text-xs text-gray-400"></di></div>
        ...
        <div className="mb-16"></di></div>
          <div className="text-center mb-12"></di></div>
            <h3 className="text-2xl font-bold text-white mb-4"></h>Key Achievements</h3>
            <p className="text-gray-300 max-w-3xl mx-auto"></p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8"></di></div>
            {achievements.map((achievement, index) => (
              <div key={index} className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 text-center hover:border-purple-400 transition-all duration-300"></di></div>
                ...
                <div className="text-3xl font-bold text-white mb-2"></di>{achievement.value}</div>
                <div className="text-lg font-semibold text-white mb-2"></di>{achievement.title}</div>
                <div className="text-gray-400 text-sm"></di>{achievement.description}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Benefits Section */}
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 mb-16"></di></div>
          <div className="text-center mb-8"></di></div>
            <h3 className="text-2xl font-bold text-white mb-4"></h>Comprehensive Benefits</h3>
            <p className="text-gray-300 max-w-3xl mx-auto"></p>
