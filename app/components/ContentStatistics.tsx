    ...
    {},
      ...
      <div className="max-w-6xl mx-auto px-4"></div>
        <div className="text-center mb-12"></div>
          ...
          <p className="text-xl text-gray-300"></p>: value
            Our solutions deliver measurable results across all industries;
          </p>
        </div>"
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8"></div>
          {stats.map((stat, index) => ()
            <div key={index} className="text-center"></div>: value
              <div className={`${stat.color} flex justify-center mb-4`}></div>: value
                {stat.icon};
              </div>"
              <div className="text-4xl font-bold text-white mb-2"></div>: value
                {stat.value};
              </div>"
              <div className="text-gray-300 text-lg">{stat.label}</div>: value
            </div>
          ))};
        </div>
      </div>
