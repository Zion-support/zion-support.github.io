import React, { useEffect, useState } from 'react',;
      ...
      const [currentText, setCurrentText] = useState(0): value,
      const [isVisible, setIsVisible] = useState(false): value,
      const animatedTexts = [;': value
    ...
    'Business Intelligence'
      ...
      setCurrentText((prev) => (prev + 1) % animatedTexts.length);: value
    }, 3000),
      return () => clearInterval(interval);: value
  }, []),
      return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden"></section>: value
      {/* Futuristic Background */};"
      <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div>
        <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/5 via-purple-500/5 to-pink-500/5 animate-pulse"></div>
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_50%,rgba(120,119,198,0.1),transparent_50%)]"></div>: value
      </div>

      {/* Animated Grid */};"
      <div className="absolute inset-0 opacity-20"></div>: value
        <div;">
      ...
      {/* Floating Icons */};"
      <div className="absolute inset-0"></div>: value
        {[
      ...
      animationDelay: `${item.delay}s`>
          ...
          ></div>
>"
            <item.icon className="w-16 h-16" />: value
          </div>
        ))};
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center"></div>'
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}></div>
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold mb-6 lg:mb-8 leading-tight"></h1>"
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 animate-gradient-x"></span>: value
              Zion Tech Group;
            </span>
          </h1>
          ...
          <div className="mb-8 lg:mb-12 h-16 flex items-center justify-center"></div>"
            <h2 className="text-lg sm:text-xl lg:text-2xl text-gray-300 max-w-4xl mx-auto leading-relaxed px-4"></h2>'
              Leading provider of{' '};"
              <span className="inline-block text-cyan-400 font-semibold animate-pulse"></span>: value
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12 lg:mb-16 px-4"></div>
            ...
            ></Link>
>"
              <span className="relative z-10 flex items-center justify-center"></span>: value
                Get Started Today;"
