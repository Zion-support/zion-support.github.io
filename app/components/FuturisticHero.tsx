  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden neural-network-bg">{/* Animated Background Elements */}</section>
      <div className="absolute inset-0 particle-field">
        </div><div className="absolute top-20 left-10 w-2 h-2 bg-cyan-400 rounded-full animate-pulse"></div><div className="absolute top-40 right-20 w-1 h-1 bg-purple-400 rounded-full animate-pulse delay-1000"></div><div className="absolute bottom-40 left-20 w-3 h-3 bg-green-400 rounded-full animate-pulse delay-2000"></div><div className="absolute bottom-20 right-10 w-1 h-1 bg-yellow-400 rounded-full animate-pulse delay-500"></div><div className="container mx-auto px-4 relative z-10">
        </div><div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">{/* Left Column - Main Content */}</div><div className={`space-y-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>{/* Badge */}</div>
            ...
            {/* Main Heading */}<div className="space-y-4">
            ...
            {/* CTA Buttons */}<div className="flex flex-col sm:flex-row gap-4">
              <Link
                to="/contact"
            ...
            {/* Stats */}<div className="grid grid-cols-2 lg:grid-cols-4 gap-6 pt-8">{stats.map((stat, index) => (</div>
                <div key={index} className="text-center space-y-2">
                  </div><div className="w-12 h-12 bg-gradient-to-br from-cyan-500/20 to-purple-500/20 rounded-lg flex items-center justify-center mx-auto">
          ...
          {/* Right Column - Visual Elements */}<div className={`relative transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>{/* Main Visual Container */}</div><div className="relative w-full h-96 lg:h-[500px]">{/* Central Holographic Display */}</div>
              <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-purple-500/10 rounded-2xl border border-cyan-400/30 cyber-scan-line">
                </div><div className="absolute inset-4 bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-xl border border-cyan-400/20 flex items-center justify-center">
                  </div><div className="text-center space-y-4">
                    </div><div className="w-20 h-20 bg-gradient-to-br from-cyan-400 to-purple-600 rounded-full flex items-center justify-center mx-auto energy-pulse">
              ...
              {/* Neural Network Lines */}<svg className="absolute inset-0 w-full h-full" style={{ zIndex: -1 }}>
                <defs>
                ...
                </li><line x1="20%" y1="50%" x2="50%" y2="60%" stroke="url(#neuralGradient)" strokeWidth="2" className="animate-pulse delay-1000" />
                </li><line x1="80%" y1="70%" x2="50%" y2="80%" stroke="url(#neuralGradient)" strokeWidth="2" className="animate-pulse delay-2000" />
              </svg>
            </div>

            {/* Floating Elements */}<div className="absolute -top-4 -left-4 w-8 h-8 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-full animate-bounce"></div><div className="absolute -bottom-4 -right-4 w-6 h-6 bg-gradient-to-br from-purple-400 to-pink-500 rounded-full animate-bounce delay-1000"></div><div className="absolute top-1/2 -left-8 w-4 h-4 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full animate-bounce delay-2000"></div>

      {/* Scroll Indicator */}<div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
        </div><div className="w-6 h-10 border-2 border-cyan-400 rounded-full flex justify-center">
          </div><div className="w-1 h-3 bg-cyan-400 rounded-full mt-2 animate-pulse"></div>
    </section>
  );
