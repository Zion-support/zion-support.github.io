...
const AdvancedPerformanceOptimizer: React.FC<PerformanceOptimizerProps> = ({children,}
  ...
  const [optimizationMetrics, setOptimizationMetrics] = useState({imagesOptimized: 0,}
    scriptsOptimized: 0,
    cssOptimized: 0,
    totalSavings: 0
  });
  const optimizeImages = useCallback(() => {if (typeof window === 'undefined') return;}
    ...
    images.forEach((img) => {// Add lazy loading if not already present}
  ...
  const optimizeScripts = useCallback(() => {if (typeof window === 'undefined') return;}
    ...
    let optimizedCount = 0;
    scripts.forEach((script) => {// Add defer attribute if not already present}
      if (!script.hasAttribute('defer') && !script.hasAttribute('async')) {script.setAttribute('defer', '');}
        optimizedCount++;
      }
    });
    return optimizedCount;
  ...
  const optimizeCSS = useCallback(() => {if (typeof window === 'undefined') return;}
    ...
    let optimizedCount = 0;
    stylesheets.forEach((link) => {// Add media attribute for non-critical CSS}
      if (!link.hasAttribute('media') && !link.hasAttribute('data-critical')) {link.setAttribute('media', 'print');}
  ...
  const runOptimizations = useCallback(() => {if (!enableOptimizations) return;}
    ...
    setOptimizationMetrics({imagesOptimized,}
      scriptsOptimized,
      cssOptimized,
  ...
  useEffect(() => {if (typeof window === 'undefined') return;}
    const observer = new PerformanceObserver((list) => {const entries = list.getEntries();}
      entries.forEach((entry) => {if (entry.entryType === 'navigation') {}
          const navEntry = entry as PerformanceNavigationTiming;
          if (navEntry.loadEventEnd - navEntry.loadEventStart > 1000) {console.warn('Page load time exceeded 1 second');}
          }
        }
      });
    });
