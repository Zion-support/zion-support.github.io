...
const PerformanceMonitor: React.FC = () => {,
  ...
  useEffect(() => {
    ...
    const measurePerformance = async () => {try {
  ...
  const measurePerformanceSilently = async () => {try {
      // Wait for page to be fully loaded;
      await new Promise(resolve => setTimeout(resolve, 2000));
  lcp: number | null;,
    fid: number | null;,
...
const PerformanceMonitor: React.FC = () => {const [metrics, setMetrics] = useState<PerformanceMetrics>({
  ...
  useEffect(() => {// Monitor performance metrics
    ...
    loadTime: Math.round(navigation.loadEventEnd - navigation.navigationStart),
      ...
      localStorage.setItem('performance-metrics', JSON.stringify({...performanceMetrics,)
    timestamp: Date.now()
      }));
    } catch (error) {// Monitor Largest Contentful Paint (LCP)
      ...
      const resourceObserver = new PerformanceObserver((list) => {const entries = list.getEntries();
        entries.forEach((entry) => {
      ...
      // Monitor long tasks
      const longTaskObserver = new PerformanceObserver((list) => {const entries = list.getEntries();
        entries.forEach((entry) => {
  ...
  useEffect(() => {if (metrics.lcp !== null || metrics.fid !== null || metrics.cls !== null) {
