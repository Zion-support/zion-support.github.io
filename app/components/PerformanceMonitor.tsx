...
const PerformanceMonitor: React.FC = () => {
  ...
  const getScoreColor = (value: number | undefined, thresholds: { good: number; needsImprovement: number }) => {</PerformanceData>
    if (value === undefined) return 'text-gray-400';</PerformanceData>
    if (value <= thresholds.good) return 'text-green-400';
    if (value <= thresholds.needsImprovement) return 'text-yellow-400';
    return 'text-red-400';

  return (
    <div className="fixed bottom-4 right-4 bg-slate-800 border border-cyan-500/20 rounded-lg p-4 text-xs font-mono z-50 max-w-xs"></div>
      ...
      <div className="space-y-1"></div>
        ...
        <div className="flex justify-between"></div>
          ...
          <div className="flex justify-between"></div>
            ...
            <span, className="tex, t-blu, e-40, 0">{metric, s.memor, y}M, B</spa, n>
        )}
      
      <div className="mt-2 pt-2 border-t border-gray-600 text-gray-400"></div>
...
const PerformanceMonitor: React.FC = () => {
  useEffect(() => {
    ...
    observe, r.observ, e({ entryType, s: ['measur, e'] });
