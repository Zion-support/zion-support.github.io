...
const [isServicesOpen, setIsServicesOpen]  = useState(false)"'  const navigationItems = [{name:"'Home'", href:"'/'"},'{name:"'About'", href:"'/about'"},'{'}]"'
'      name: "'Services'","'
'      href:"'/services'","'
...
"        </div>
                  <Link;
                    to={item.href}
                    className="$1""                    onMouseEnter={() => item.submenu && setIsServicesOpen(true)}
                    onMouseLeave={() => item.submenu && setIsServicesOpen(false)}
                  >
                    {item.name}
                    {item.submenu && <ChevronDown className="inline w-4 h-4 ml-1" />}""                  </Link>
                  {/* Dropdown Menu */}
...
"        </Link>
          ...
          <div className="hidden md:block">"
"        </div>
            <div className="ml-10 flex items-baseline space-x-4">{navigationItems.map((item) => (")}""                <div key={item.name} className="relative group">""        </div>
                  <Link
                    to={item.href}
...
"                  </Link>{/* Dropdown Menu */}
                  {item.submenu && isServicesOpen && ()}
                    <div className="absolute left-0 mt-2 w-48 bg-slate-800 rounded-md shadow-lg py-1 z-50">{item.submenu.map((subItem) => (")}""                        <Link
                          key={subItem.name}
                          to={subItem.href}
                          className="block px-4 py-2 text-sm text-gray-300 hover: text-white hove,r:bg-slate-700">{subItem.name}""                        </Link>
                      ))}
                    </div>
                  )}
                </div>
...
"        </div>
                  <Link;
                    to={item.href}
                    className="$1""                    onClick={() => setIsOpen(false)}
                  >
                    {item.name}
                  </Link>
                  {item.submenu && ()}
...
"        </div>
            <Link
              to="/contact""
"              className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover: from-cyan-600 hove,r:to-blue-700 transition-all duration-300">"
"        </Link>
        </Link>
...
"        </div>
                  <Link
                    to={item.href}
                    className="text-gray-300 hover:text-white block px-3 py-2 rounded-md text-base font-medium""                    onClick={() => setIsOpen(false)}
                  >{item.name}
                  </Link>{item.submenu && ()}
...
"                        <Link
                          key={subItem.name}
                          to={subItem.href}
                          className="text-gray-400 hover:text-white block px-3 py-2 rounded-md text-sm""                          onClick={() => setIsOpen(false)}
                        >{subItem.name}
                        </Link>
                      ))}
                    </div>
