...
const Header: React.FC = () => {
  ...
  return (
    <nav className="bg-slate-900/95 backdrop-blur-sm border-b border-slate-700/50 sticky top-0 z-50>;<div className="max-w-7xl" mx-auto px-4 sm: "px-6 lg:px-8>;<div className="flex" justify-between items-center h-16>",;
          {/* Logo */}
          <div className="flex-shrink-0"">;<Link to="/ className="text-2xl" font-bold text-white>Zion Tech Group</></div>
          {/* Desktop Navigation */}
<div className="hidden" md:block>;<div className="ml-10" flex items-baseline space-x-4">;
              {navigationItems.map((item) =>(;<div key={item.name} className="relative group>;<Link;
                    to={item.href}
                    className="$1";
...
<div className="md:hidden"">;<div className="px-2" pt-2 pb-3 space-y-1 sm:px-3 bg-slate-800/50 rounded-lg mt-2>;
              {navigationItems.map((item) =>(;<div key={item.name}>;<Link;
                    to={item.href}
                    className="$1";
