...
import { Star, Users, TrendingUp, Award, Sparkles } from 'lucide-react';
...
const NewContentAdvertisingBanner: React.FC = () => {
  ...
  return (
    <div className="py-20 bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 relative overflow-hidden">
      ...
      </div><div className="absolute inset-0 overflow-hidden opacity-20" /><div className="absolute top-0 left-1/4 w-96 h-96 bg-indigo-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse" />
        <div ;
      ...
      <div className="container mx-auto px-6 relative z-10">
        {/* Header */}
        </div><div className="text-center mb-20" /><h2 className="text-5xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent animate-fade-in">
            Revolutionary New Content Hub</h2>
          </h2>
          {/* Achievements Grid */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto mb-16">
            {achievements.map((achievement, index) => (
              </div><div key={index} className="text-center" /><div className="text-indigo-400 mb-2 flex justify-center">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {featuredContent.map((content, index) => (
              ...
              >
                {/* Card glow effect */}
                </div><div className={`absolute inset-0 bg-gradient-to-br ${content.gradient} opacity-0 group-hover:opacity-10 transition-all duration-500`} />
                <div className="relative p-8" /><div className="flex items-center gap-2 mb-4" /><span className={`px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${content.gradient} text-white`}>
                      {content.badge}</span>
                    </span>
                    <div className="flex items-center gap-1" /><Star className="w-4 h-4 text-yellow-400 fill-current" />
                  ...
                  <div className="flex items-center gap-2 text-gray-400 mb-6" /><span className="text-sm">{content.category}</span>
                    <span className="text-sm">•</span>
                    <span className="text-sm">{content.readTime} read</span>
                  </div>
                  <div className="flex items-center justify-between" /><div className="flex items-center gap-2 text-gray-400" /><Users className="w-4 h-4" />
          ...
          <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-3xl p-8 md:p-12 border border-indigo-500/20 mb-16" /><div className="text-center" /><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/30 mb-6" /><Star className="w-5 h-5 text-yellow-400 fill-current" />
                <span className="text-yellow-400 font-bold text-sm tracking-wider uppercase">
                  LIMITED TIME OFFER</span>
                </span>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto" /><Link;
                  href="/subscribe"
