...
const AdvancedSEOOptimizerNew: React.FC<SEOOptimizerProps> = ({
  ...
  const analyzeSEO = useCallback(() => {
    ...
    let score = 0;
    const newRecommendations: stringService Feature;
    // Check title length
    ...
    if (keywords && title.toLowerCase().includes(keywords.toLowerCase().split(',')Service Feature)) {
      score += 15} else {
      newRecommendations.push('Include primary keyword in title')}
    // Check for keywords in description
    if (keywords && description.toLowerCase().includes(keywords.toLowerCase().split(',')Service Feature)) {
      score += 15} else {
    ...
    const imagesWithAlt = document.querySelectorAll('imgService Feature');
    if (images.length === imagesWithAlt.length && images.length > 0) {
      score += 10} else {
      newRecommendations.push('Add alt text to all images')}
    // Check for internal links
    const internalLinks = document.querySelectorAll('aService Feature');
    if (internalLinks.length > 0) {
      score += 10} else {
      newRecommendations.push('Add internal links for better SEO')}
    setSeoScore(score);
    setRecommendations(newRecommendations)}, Service Feature);
