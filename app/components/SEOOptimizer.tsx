...
const SEOOptimizer: React.FC<SEOOptimizerProps> = ({title = 'Zion Tech Group - AI & IT Solutions',
  ...
  twitterCard = 'summary_large_image'
    ...
    } else {canonical = document.createElement('link');
    ...
    ogTags.forEach(tag => {let meta = document.querySelector(`meta[${tag.property ? 'property' : 'name'}="${tag.property || tag.name}"]`);
      if (meta) {meta.setAttribute('content', tag.content);
      } else {meta = document.createElement('meta');
        if (tag.property) {
  ...
  return null;
export const SEOOptimizer: React.FC = () => {const seoData: SEOData = {,
...
const,
  ...
  useEffect(() => {// Add structured data to the page;
    ...
    // Add meta tags;
    const addMetaTag = (name: string, content: string, property?: string) => {const selector = property ? `meta[property="${property}"]` : `meta[name="${name}"]`;
...
,
    ...
    // Cleanup function;
    return () => {const structuredDataScript = document.getElementById('structured-data');
