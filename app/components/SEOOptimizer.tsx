...
interface SEOAuditResult {
  ...
  description: { hasDescription: boolean; descriptionLength: number; isValid: boolean };
  headings: { h1Count: number; hasH1: boolean; headingStructure: string[] };
  images: { totalImages: number; imagesWithoutAlt: number; imagesWithoutAltText: string[] };
...
const SEOOptimizer: React.FC<SEOOptimizerProps> = ({
  ...
  const runSEOAudit = useCallback(() => {
    ...
    if (audit.description.hasDescription && audit.description.isValid) score += 20;
          ...
          {auditResult && (
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                <span>Title:</span>
                <span className={auditResult.title.isValid ? 'text-green-600' : 'text-red-600'}>
              ...
              <div className="flex justify-between">
                ...
                <span className={auditResult.description.isValid ? 'text-green-600' : 'text-red-600'}>
              ...
              <div className="flex justify-between">
                <span>H1 Count:</span>
                <span className={auditResult.headings.h1Count === 1 ? 'text-green-600' : 'text-red-600'}>
              ...
              <div className="flex justify-between">
                <span>Images:</span>
                <span className={auditResult.images.imagesWithoutAlt === 0 ? 'text-green-600' : 'text-red-600'}>
              ...
              <div className="flex justify-between">
                <span>Links:</span>
                <span className={auditResult.links.internalLinks > auditResult.links.externalLinks ? 'text-green-600' : 'text-yellow-600'}>
                  {auditResult.links.internalLinks > auditResult.links.externalLinks ? '✅' : '⚠️'} 
              ...
              {auditResult.headings.headingStructure.length > 0 && (
                <div className="mt-2">
                  <div className="font-semibold text-gray-700 dark:text-gray-300">Structure:</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 max-h-16 overflow-y-auto">
                    {auditResult.headings.headingStructure.slice(0, 4).map((heading, index) => (
                      <div key={index} className="truncate">{heading}</div>
                    ))}
                    {auditResult.headings.headingStructure.length > 4 && (
                      <div className="text-gray-500">...and {auditResult.headings.headingStructure.length - 4} more</div>
                    )}
                  </div>
                </div>
              )}
