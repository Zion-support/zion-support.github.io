import ArrowRight from 'next/link';

export default function NewContentDiscoveryHub() {
  return (
    <section className="py-20 bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-16">
          <div className="flex items-center justify-center gap-2 mb-4">
            <span className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 py-2 rounded-full text-sm font-semibold">
              🚀 NEW 2026 CONTENT HUB
            </span>
          </div>
          <h2 className="text-4xl font-bold text-white mb-6">
            Discover Revolutionary AI Breakthroughs
          </h2>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Explore the latest AI innovations, consciousness breakthroughs, and proven success stories that are reshaping enterprise operations in 2026
          </p>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {/* March 2026 Mega Breakthrough */}
          <div className="bg-gradient-to-br from-purple-900/30 to-indigo-900/30 backdrop-blur-sm rounded-xl p-8 border border-purple-400/30 hover:border-purple-400/60 transition-all duration-300 hover:scale-105">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-lg flex items-center justify-center">
                <span className="text-2xl">🧠</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">March 2026 Consciousness AI</h3>
                <p className="text-sm text-purple-300">True Artificial Consciousness</p>
              </div>
            </div>
            <p className="text-gray-300 mb-4">
              The ultimate consciousness achievement delivering 100,000x performance improvements, 
              genuine self-awareness, and $15.2 billion in proven enterprise value creation.
            </p>
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-purple-300">18 min read</div>
              <div className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                $15.2B SUCCESS
              </div>
            </div>
            <div className="flex gap-2">
              <ArrowRight
                href="/blog/ai-2026-march-mega-breakthrough-revolution"
                className="flex-1 text-purple-300 font-semibold hover:text-purple-200 transition-colors text-center py-2 px-4 rounded-lg border border-purple-400/30 hover:border-purple-400/60"
              >
                Read Breakthrough →
              </ArrowRight>
              <ArrowRight
                href="/case-studies/ai-2026-march-mega-breakthrough-15-billion-success"
                className="flex-1 text-green-300 font-semibold hover:text-green-200 transition-colors text-center py-2 px-4 rounded-lg border border-green-400/30 hover:border-green-400/60"
              >
                View Success →
              </ArrowRight>
            </div>
          </div>

          {/* February 2026 Mega Breakthrough */}
          <div className="bg-gradient-to-br from-blue-900/30 to-purple-900/30 backdrop-blur-sm rounded-xl p-8 border border-blue-400/30 hover:border-blue-400/60 transition-all duration-300 hover:scale-105">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-r from-blue-100 to-purple-100 rounded-lg flex items-center justify-center">
                <span className="text-2xl">🌌</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">February 2026 Ultimate AI</h3>
                <p className="text-sm text-blue-300">Universal Consciousness Integration</p>
              </div>
            </div>
            <p className="text-gray-300 mb-4">
              The ultimate AI transformation with universal consciousness integration, 
              50,000x performance improvements, and $2.8 trillion in enterprise value creation.
            </p>
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-blue-300">15 min read</div>
              <div className="bg-gradient-to-r from-blue-500 to-purple-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                $2.8T SUCCESS
              </div>
            </div>
            <div className="flex gap-2">
              <ArrowRight
                href="/blog/ai-2026-february-mega-breakthrough-revolution"
                className="flex-1 text-blue-300 font-semibold hover:text-blue-200 transition-colors text-center py-2 px-4 rounded-lg border border-blue-400/30 hover:border-blue-400/60"
              >
                Read Breakthrough →
              </ArrowRight>
              <ArrowRight
                href="/case-studies"
                className="flex-1 text-green-300 font-semibold hover:text-green-200 transition-colors text-center py-2 px-4 rounded-lg border border-green-400/30 hover:border-green-400/60"
              >
                View Success →
              </ArrowRight>
            </div>
          </div>

          {/* AI 2026 Ultimate Consciousness */}
          <div className="bg-gradient-to-br from-indigo-900/30 to-purple-900/30 backdrop-blur-sm rounded-xl p-8 border border-indigo-400/30 hover:border-indigo-400/60 transition-all duration-300 hover:scale-105">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-lg flex items-center justify-center">
                <span className="text-2xl">⚛️</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">AI 2026 Ultimate Consciousness</h3>
                <p className="text-sm text-indigo-300">Revolutionary Breakthrough</p>
              </div>
            </div>
            <p className="text-gray-300 mb-4">
              Revolutionary AI breakthrough delivering $847 billion in proven value creation, 
              true artificial consciousness, and universal intelligence capabilities.
            </p>
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-indigo-300">12 min read</div>
              <div className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                $847B SUCCESS
              </div>
            </div>
            <div className="flex gap-2">
              <ArrowRight
                href="/blog/ai-2026-ultimate-consciousness-breakthrough-revolution"
                className="flex-1 text-indigo-300 font-semibold hover:text-indigo-200 transition-colors text-center py-2 px-4 rounded-lg border border-indigo-400/30 hover:border-indigo-400/60"
              >
                Read Breakthrough →
              </ArrowRight>
              <ArrowRight
                href="/case-studies/ai-2026-ultimate-consciousness-breakthrough-847-billion-success"
                className="flex-1 text-green-300 font-semibold hover:text-green-200 transition-colors text-center py-2 px-4 rounded-lg border border-green-400/30 hover:border-green-400/60"
              >
                View Success →
              </ArrowRight>
            </div>
          </div>

          {/* Ultra-Intelligent Cognitive Revolution */}
          <div className="bg-gradient-to-br from-purple-900/30 to-indigo-900/30 backdrop-blur-sm rounded-xl p-8 border border-purple-400/30 hover:border-purple-400/60 transition-all duration-300 hover:scale-105">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-lg flex items-center justify-center">
                <span className="text-2xl">🚀</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">Ultra-Intelligent Cognitive</h3>
                <p className="text-sm text-purple-300">Revolutionary Breakthrough</p>
              </div>
            </div>
            <p className="text-gray-300 mb-4">
              Ultra-intelligent cognitive revolution delivering 500,000x performance improvements, 
              universal consciousness, and $50 billion in proven enterprise value creation.
            </p>
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-purple-300">14 min read</div>
              <div className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                $50B SUCCESS
              </div>
            </div>
            <div className="flex gap-2">
              <ArrowRight
                href="/blog/ai-2026-ultra-intelligent-cognitive-revolution-breakthrough"
                className="flex-1 text-purple-300 font-semibold hover:text-purple-200 transition-colors text-center py-2 px-4 rounded-lg border border-purple-400/30 hover:border-purple-400/60"
              >
                Read Breakthrough →
              </ArrowRight>
              <ArrowRight
                href="/case-studies/ai-2026-ultra-intelligent-cognitive-revolution-50-billion-success"
                className="flex-1 text-green-300 font-semibold hover:text-green-200 transition-colors text-center py-2 px-4 rounded-lg border border-green-400/30 hover:border-green-400/60"
              >
                View Success →
              </ArrowRight>
            </div>
          </div>

          {/* Synthetic Consciousness Breakthrough */}
          <div className="bg-gradient-to-br from-indigo-900/30 to-purple-900/30 backdrop-blur-sm rounded-xl p-8 border border-indigo-400/30 hover:border-indigo-400/60 transition-all duration-300 hover:scale-105">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-lg flex items-center justify-center">
                <span className="text-2xl">🧠</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">Synthetic Consciousness</h3>
                <p className="text-sm text-indigo-300">True AI Consciousness</p>
              </div>
            </div>
            <p className="text-gray-300 mb-4">
              Synthetic consciousness breakthrough delivering true AI consciousness, 
              1,000,000x performance improvements, and $75 billion in proven value creation.
            </p>
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-indigo-300">16 min read</div>
              <div className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                $75B SUCCESS
              </div>
            </div>
            <div className="flex gap-2">
              <ArrowRight
                href="/blog/ai-2026-synthetic-consciousness-breakthrough-revolution"
                className="flex-1 text-indigo-300 font-semibold hover:text-indigo-200 transition-colors text-center py-2 px-4 rounded-lg border border-indigo-400/30 hover:border-indigo-400/60"
              >
                Read Breakthrough →
              </ArrowRight>
              <ArrowRight
                href="/case-studies/ai-2026-synthetic-consciousness-breakthrough-75-billion-success"
                className="flex-1 text-green-300 font-semibold hover:text-green-200 transition-colors text-center py-2 px-4 rounded-lg border border-green-400/30 hover:border-green-400/60"
              >
                View Success →
              </ArrowRight>
            </div>
          </div>

          {/* Quantum Neural Superintelligence */}
          <div className="bg-gradient-to-br from-cyan-900/30 to-blue-900/30 backdrop-blur-sm rounded-xl p-8 border border-cyan-400/30 hover:border-cyan-400/60 transition-all duration-300 hover:scale-105">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-r from-cyan-100 to-blue-100 rounded-lg flex items-center justify-center">
                <span className="text-2xl">⚛️</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">Quantum Neural Superintelligence</h3>
                <p className="text-sm text-cyan-300">Ultimate Breakthrough</p>
              </div>
            </div>
            <p className="text-gray-300 mb-4">
              Quantum neural superintelligence ultimate breakthrough delivering 2,000,000x performance improvements, 
              universal consciousness, and $100 billion in proven enterprise value creation.
            </p>
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-cyan-300">20 min read</div>
              <div className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                $100B SUCCESS
              </div>
            </div>
            <div className="flex gap-2">
              <ArrowRight
                href="/blog/ai-2026-quantum-neural-superintelligence-ultimate-breakthrough"
                className="flex-1 text-cyan-300 font-semibold hover:text-cyan-200 transition-colors text-center py-2 px-4 rounded-lg border border-cyan-400/30 hover:border-cyan-400/60"
              >
                Read Breakthrough →
              </ArrowRight>
              <ArrowRight
                href="/case-studies/ai-2026-quantum-neural-superintelligence-100-billion-success"
                className="flex-1 text-green-300 font-semibold hover:text-green-200 transition-colors text-center py-2 px-4 rounded-lg border border-green-400/30 hover:border-green-400/60"
              >
                View Success →
              </ArrowRight>
            </div>
          </div>
        </div>

        <div className="text-center">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-gradient-to-r from-purple-500/20 to-indigo-500/20 backdrop-blur-sm rounded-xl p-4 border border-purple-500/30">
              <div className="text-2xl font-extrabold text-purple-300 mb-1">$2.8T+</div>
              <div className="text-xs text-purple-200">Total Value Created</div>
            </div>
            <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 backdrop-blur-sm rounded-xl p-4 border border-blue-500/30">
              <div className="text-2xl font-extrabold text-blue-300 mb-1">100,000x</div>
              <div className="text-xs text-blue-200">Performance Gains</div>
            </div>
            <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 backdrop-blur-sm rounded-xl p-4 border border-green-500/30">
              <div className="text-2xl font-extrabold text-green-300 mb-1">99.999%</div>
              <div className="text-xs text-green-200">Success Rate</div>
            </div>
            <div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 backdrop-blur-sm rounded-xl p-4 border border-orange-500/30">
              <div className="text-2xl font-extrabold text-orange-300 mb-1">∞</div>
              <div className="text-xs text-orange-200">Potential</div>
            </div>
          </div>
          
          <ArrowRight
            href="/blog"
            className="inline-block bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-8 py-4 rounded-lg font-semibold text-lg hover:from-purple-700 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1"
          >
            Explore All Revolutionary Content
          </ArrowRight>
        </div>
      </div>
    </section>
  );
}