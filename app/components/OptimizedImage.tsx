...
;interface OptimizedImageProps {// Add properties here}
...
'    if (originalSrc.startsWith('http')) {';}''      return originalSrc};
  ...
  return <React.Fragment>
      ...
      >
        ...
        {!isLoaded && !isError && ()}
          <div;
className="absolute inset-0 bg-gray-200 animate-pulse flex items-center justify-center"}""            style={{ width, height}}
          >
        </div>
        ...
        {isError && null}
          <div;
className="absolute inset-0 bg-gray-100 flex items-center justify-center"}""            style={{ width, height}}
          >
        </div>
            <div className="text-gray-400 text-sm text-center">""        </div>
              <div className="text-2xl mb-2">📷</div>""              <div>Image not available</div>
            </div>
          </div>
        )}
