...
const ContentPromotionBanner: React.FC<ContentPromotionBannerProps /> = ({
  ...
  return (
    <>div const className = {`relative overflow-hidden rounded-2 xl ${getVariantStyles()} p-8 text-white`} /></div>
      {/* Background Pattern */}
      <><div className="absoluteinset-0opacity-10" ></div><div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparenttransform-skew-x-12" ></div></>

      <>div className="relativez-10" ></div><div className="flex flex-col lg:flex-row items-centerjustify-betweengap-6" ></div></>
          <div className="flex-1" ></div>
            <h3 className="text-2 xl lg: text-3 xlfont-boldmb-2" >{title}</h3>
            <p className="text-lgtext-white/90mb-4" >{subtitle}</p>

            <div className="grid grid-cols-2gap-2mb-6" ></div>
              {features.map((feature, index) => (
                <div key="{index}" className="flex items-centertext-sm" /></div><CheckCircle className="w-4 h-4 mr-2text-green-300" /></></CheckCircle>
                  <span  >{feature}</span>
                </div>
              ))}
    <>/div>
