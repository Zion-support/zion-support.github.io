...
const OptimizedLoading: React.FC<OptimizedLoadingProps /> = ({
  ...
  return (
    <>div className="{`${containerClasses}" ${className}`} />
      <><div className="flex flex-col items-centerspace-y-4" ></div><Loader2 className="{`${sizeClasses[size]}" text-cyan-500 animate-spin`} / /></></Loader2>
        {text && (
          <p className="text-gray-300 text-sm font-mediumanimate-pulse" ></p>
              {text}
          </p>
        )}
    <>/div>
