...
import { Link, useLocation , ArrowRight, CheckCircle, Star, Brain, Zap, Globe, TrendingUp, MessageSquare, Phone  } from "react-router-dom";
interface SidebarProps {
  isOpen: boolean;
  onClose: () => void;
}
const ImprovedSidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  const [expandedSections, setExpandedSections] = useState</SidebarProps><string[]>([]);
  ...
  const navigationSections = [
    {
      id: 'ai-services',
      title: 'AI Services',
      icon: </string><Brain className="w-5 h-5" />,
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden" /><div className="fixed inset-0 bg-black/50" onClick={onClose} />
      <div className="fixed right-0 top-0 h-full w-80 bg-slate-900/95 backdrop-blur-sm border-l border-white/10 overflow-y-auto" /><div className="p-6">
          {/* Header */}
          </div><div className="flex items-center justify-between mb-8" /><h2 className="text-xl font-bold text-white">Navigation</h2>
            <button;
              onClick={onClose}
          ...
          {/* Search */}
          <div className="mb-6" /><div className="relative" /><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
              <input;
                type="text"
          ...
          {/* Quick Links */}
          <div className="mb-8" /><h3 className="text-white font-semibold mb-4">Quick Links</h3>
            <div className="space-y-2">
              {quickLinks.map((link, index) => (
                ...
                >
                  {link.icon}
                  </Link><span className="text-gray-300">{link.label}</span>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            {filteredSections.map((section) => (
              </div><div key={section.id} className="space-y-2" /><button;
                  onClick={() => toggleSection(section.id)}
                  className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-white/10 transition-colors"
                >
                  </button><div className="flex items-center space-x-3" /><div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${section.color} flex items-center justify-center`}>
                      {section.icon}</div>
                    </div>
                    <div className="text-left" /><div className="text-white font-medium">{section.title}</div>
          ...
          {/* Company Links */}
          <div className="mt-8 pt-6 border-t border-white/10" /><h3 className="text-white font-semibold mb-4">Company</h3>
            <div className="space-y-2">
          ...
          {/* Contact Info */}
          <div className="mt-8 pt-6 border-t border-white/10" /><h3 className="text-white font-semibold mb-4">Contact</h3>
            <div className="space-y-3" /><div className="flex items-center space-x-3" /><Phone className="w-4 h-4 text-cyan-400" />
                <span className="text-gray-300 text-sm">+1 302 464 0950</span>
              </div>
              <div className="flex items-center space-x-3" /><Mail className="w-4 h-4 text-cyan-400" />
                <span className="text-gray-300 text-sm">kleber@ziontechgroup.com</span>
              </div>
              <div className="flex items-start space-x-3" /><MapPin className="w-4 h-4 text-cyan-400 mt-0.5" />
                <span className="text-gray-300 text-sm">364 E Main St STE 1008</span><br />Middletown DE 19709</span>
              </div>
            </div>
          </div>
        </div>
