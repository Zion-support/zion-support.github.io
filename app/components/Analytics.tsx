...
const Analytics: React.FC<AnalyticsProps> = ({enableGoogleAnalytics = true,}
  ...
  enableUserBehaviorTracking = true
    ...
    if (enableGoogleAnalytics) {initializeGoogleAnalytics()}
    }
  ...
  const initializeGoogleAnalytics = () => {// Load Google Analytics    const script = document.createElement('script')}
    script.async = true
    script.src = 'https://www.googletagmanager.com/gtag/js?id=GA_MEASUREMENT_ID'
    document.head.appendChild(script)
    // Initialize gtag    (window as any).dataLayer = (window as any).dataLayer || []
  ...
  const initializePerformanceMonitoring = () => {if ('PerformanceObserver' in window) {}
      ...
      const observer = new PerformanceObserver((list) => {for (const entry of list.getEntries()) {}
          if (entry.entryType === 'largest-contentful-paint') {trackEvent('web_vitals', 'LCP', Math.round(entry.startTime))}
  ...
  const initializeErrorTracking = () => {// Track JavaScript errors}
    ...
    window.addEventListener('error', (event) => {if (event.target !== window) {}
        trackEvent('error', 'resource_error', {type: (event.target as any).tagName,}
  ...
  const initializeUserBehaviorTracking = () => {// Track page views}
    ...
    // Track scroll depth    let maxScroll = 0
    ...
    // Track time on page    const startTime = Date.now()
