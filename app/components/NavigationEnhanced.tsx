...
import { Link, useLocation , ArrowRight, Star, Brain, Zap, Globe  } from "react-router-dom";
...
const NavigationEnhanced = React.memo<NavigationEnhancedProps>(({ onSidebarToggle }) => {
  ...
  const aiServices = useMemo(() => [
    { name: 'AI Video Generator', path: '/zion-ai-video-generator', icon: </NavigationEnhancedProps><Video className="w-4 h-4" />, featured: true },
  ...
  return (
    ...
    >
      {/* Contact Bar */}
      </nav><div className="bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border-b border-white/10" /><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" /><div className="flex flex-col sm:flex-row items-center justify-between py-2 text-sm" /><div className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-6" /><div className="flex items-center text-cyan-400" /><span className="w-2 h-2 bg-green-400 rounded-full mr-2 animate-pulse" />
                <span>Live Support Available</span>
              </div>
              <div className="flex items-center text-cyan-400" /><span className="mr-2">📞</span>
                <a href="tel:+13024640950" className="hover:text-cyan-300 transition-colors">
                  +1 302 464 0950</a>
                </a>
              </div>
              <div className="flex items-center text-cyan-400" /><span className="mr-2">✉️</span>
                <a href="mailto:kleber@ziontechgroup.com" className="hover:text-cyan-300 transition-colors">
                  kleber@ziontechgroup.com</a>
                </a>
              </div>
            </div>
            <div className="flex items-center space-x-4" /><FuturisticButton;
                href="/contact"
                variant="ghost"
                size="sm"
                className="text-cyan-400 hover:text-white"
              >
                Get Quote</FuturisticButton>
              </FuturisticButton>
            </div>
          </div>
        </div>
      ...
      {/* Main Navigation */}
      <div className="max-w-7xl mx-auto px-4" /><div className="flex justify-between items-center py-4" /><Link ;
            to="/" 
          ...
          <div className="hidden lg:flex space-x-8 items-center">
            ...
            {mainNavItems.map((item) => (
              ...
              >
                {item.name}
                </Link><span className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-cyan-400 to-purple-400 transition-all duration-300 ${
            ...
            <div className="relative" /><button;
              ...
              {isAiServicesOpen && (
                <div className="absolute top-full left-0 mt-2 w-80 bg-slate-800/95 backdrop-blur-xl rounded-xl shadow-2xl py-4 z-50 border border-purple-500/20" /><div className="px-4 py-2 border-b border-gray-700 mb-2" /><h3 className="text-sm font-semibold text-purple-400 uppercase tracking-wider">AI Services</h3>
                  </div>
                  {aiServices.map((service) => (
                    ...
                    >
                      </Link><div className="w-8 h-8 rounded-lg bg-gradient-to-r from-purple-500/20 to-cyan-500/20 flex items-center justify-center group-hover:from-purple-500/30 group-hover:to-cyan-500/30 transition-all duration-300">
                        {service.icon}</div>
                      </div>
                      <div className="flex-1" /><span className="font-medium">{service.name}</span>
                        {service.featured && (
                          <div className="flex items-center mt-1" /><Star className="w-3 h-3 text-yellow-400 fill-current mr-1" />
            ...
            <div className="relative" /><button;
              ...
              {isItServicesOpen && (
                <div className="absolute top-full left-0 mt-2 w-80 bg-slate-800/95 backdrop-blur-xl rounded-xl shadow-2xl py-4 z-50 border border-green-500/20" /><div className="px-4 py-2 border-b border-gray-700 mb-2" /><h3 className="text-sm font-semibold text-green-400 uppercase tracking-wider">IT Services</h3>
                  </div>
                  {itServices.map((service) => (
                    ...
                    >
                      </Link><div className="w-8 h-8 rounded-lg bg-gradient-to-r from-green-500/20 to-blue-500/20 flex items-center justify-center group-hover:from-green-500/30 group-hover:to-blue-500/30 transition-all duration-300">
                        {service.icon}</div>
                      </div>
            ...
            <div className="relative" /><button;
              ...
              {isMicroSaasOpen && (
                <div className="absolute top-full left-0 mt-2 w-80 bg-slate-800/95 backdrop-blur-xl rounded-xl shadow-2xl py-4 z-50 border border-purple-500/20" /><div className="px-4 py-2 border-b border-gray-700 mb-2" /><h3 className="text-sm font-semibold text-purple-400 uppercase tracking-wider">Micro SAAS Solutions</h3>
                  </div>
        ...
        {/* Search Bar */}
        {isSearchOpen && (
          <div className="py-4 border-t border-cyan-500/20" /><div className="relative" /><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input;
                type="text"
        ...
        {/* Mobile menu */}
        {isOpen && (
          <div className="lg:hidden py-4 border-t border-cyan-500/20" /><div className="flex flex-col space-y-2">
              {mainNavItems.map((item) => (
                </div><Link;
                  key={item.name}
                  to={item.path}
