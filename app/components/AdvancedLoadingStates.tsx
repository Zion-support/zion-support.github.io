...
return <div className="animate-pulse rounded h-12 w-12 bg-gray-400 mx-auto mb-4"></div>;
      case 'dots':;
return <div className="flex space-x-1 justify-center mb-4"><div className="animate-bounce w-2 h-2 bg-white rounded-full"></div><div className="animate-bounce w-2 h-2 bg-white rounded-full" style={{ animationDelay: '0.1s' }}></div><div className="animate-bounce w-2 h-2 bg-white rounded-full" style={{ animationDelay: '0.2s' }}></div></div>;
      default:;
...
;
return (
    <div className={`${fullScreen ? 'fixed inset-0' : ''} flex items-center justify-center bg-slate-900 text-white`}><div className="text-center">{getLoadingIcon()}`
        <p>{message || 'Loading...'}</p></div></div>);
};
;
