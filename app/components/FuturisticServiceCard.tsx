...
const FuturisticServiceCard: React.FC<FuturisticServiceCardProps> = ({
  ...
  return (
    <div
      className={`group relative cursor-pointer transition-all duration-500 ${isHovered ? 'scale-105' : 'scale-100'}`}
    ...
    >
      ...
      {/* Card */}
      <div
        className={`relative bg-gradient-to-br from-slate-800/90 to-slate-900/90 backdrop-blur-lg rounded-2xl p-8 border border-slate-700/50 transition-all duration-500 ${currentColor.hover} ${
      ...
      >
        ...
        <button
          className={`w-full py-3 px-6 rounded-lg font-semibold transition-all duration-300 ${
            isPopular
              ? `bg-gradient-to-r ${currentColor.gradient} text-white hover:shadow-lg hover:shadow-${color}-500/25`
