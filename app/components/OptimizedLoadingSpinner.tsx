...
const OptimizedLoadingSpinner: React.FC<OptimizedLoadingSpinnerProps> = memo(
  ({
    ...
    const renderSpinner = useMemo(() => {switch (variant) {
        ...
        case 'pulse':
          return (
            <div              className={`${baseClasses} rounded-full animate-pulse`}
              role='status'
              aria-label='Loading'
            /></div>)
        case 'skeleton':
        ...
        default:
          return (
            <div              className={`${baseClasses} rounded-full border-2 border-t-transparent animate-spin`}
              role='status'
              aria-label='Loading'
            /></div>)
      }
    ...
    }, [fullScreen, className])return (</div>
