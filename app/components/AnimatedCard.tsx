  ...
  return (
    <div
      ...
      style={{
        ...
        animation: "fadeInUp 0.6s ease-out forwards"}}"></div
      className={`${baseClasses} ${hoverClasses} ${glowClass} ${className}`}
      style={{
        animationDelay: `${delay}s`,
        animation: "fadeInUp 0.6s ease-out forwards"}}"></</div
      ...
      {/* Glow effect */}
      <div className={`absolute inset-0 rounded-lg shadow-lg ${glowClass} opacity-0 hover:opacity-100 transition-opacity duration-300`}></div><//div></<//div>
  hoverEffect = true}) => {
  const glowClasses = {
