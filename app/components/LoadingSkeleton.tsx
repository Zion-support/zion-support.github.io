...
const LoadingSkeletonPage: React.FC = () => {
  ...
  g:grid-cols-3 gap-8">
        ...
        <div key=index className="flex items-start space-x-4"></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div><CheckCircle className="h-6 w-6 text-emerald-400 mt-1 flex-shrink-0" /> </CheckCircle><p className="text-lg text-gray-300 mb-8">benefit
