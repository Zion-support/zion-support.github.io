...
interface ContentNewsletterSignupProps {
    ...
    <div className="bg -gradient-to-br from-slate-900 via-emerald-900 to-slate-900 py-16 px-4">">
      <div className="max-w-4xl mx-auto text-center">">
        ...
        <form onSubmit={handleSubmit} className= max-w-md mx-auto mb-8 ></form>
          <div className= fl ex flex-col sm:flex-row gap-4">
            <input type= email value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder={placeholder}
              className="fl ex-1 px-4 py-3 rounded-lg bg-slate-800 border border-slate-600 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focu,
  s:border-transparent required /"></input>
  ...
  d:bg-emerald-800 text-white px-6 py-3 rounded-lg font-semibold transition-colors duration-200 flex items-center justify-center">
        ...
        <div className= gr id grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          {features.map((feature, index) => (</div>
            <div key={index} className="fl ex items-center space-x-2 text-gray-300"></div>
              <feature.icon className="h-5 w-5  te xt-emerald-400 /"></feature>
