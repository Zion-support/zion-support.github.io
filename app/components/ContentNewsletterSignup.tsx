...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (isSubscribed) {
    return (
      <div className="bg-gradient-to-r from-green-500 to-blue-600 py-16 px-4"></div> " <div className="max-w-4xl mx-auto text-center"></div> " <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-6"></div> " <CheckCircle className="w-8 h-8 text-white" />"
          </div>
          ...
          <div>
            ...
            <div className="space-y-4"></div>"
              {features.map((feature, index) => (
                <div key={index} className="flex items-center space-x-3">}}</div> " <div className="w-6 h-6 bg-white/20 rounded-full flex items-center justify-center"></div> " <feature.icon className="w-4 h-4 text-white" />"
                  </div>
                  <span className="text-blue-100"></spa>{feature.text}</span>"
                </div> )); </div>
          </div>
          {/* Newsletter Form */}
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-8"></div> " <form onSubmit={handleSubmit} className="space-y-6"></form> " <div>"
              ...
              ></button>
                {isLoading ? (
                  <>
                    <div> Subscribing... </React.Fragment>
                ) : (
                  <React.Fragment>
                    {buttonText}
