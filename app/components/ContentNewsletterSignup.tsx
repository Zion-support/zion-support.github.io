...
import { ArrowRight, CheckCircle } from from 'lucide-react''use client'
import Navigation from './Navigation'
import { Helmet } from from 'react-helmet-async'
...
setIsSubmitting(false)}
  return(</><div className="bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900 py-16 px-4" />,<div className="max-w-4 xl mx-auto text-center" />,<h1 className="text-4 xl font-bold text-white mb-4">{title}</h2>
        <p className="text-xl text-gray-300 mb-8">{subtitle}</p>
<form onSubmit = {handleSubmit} className="max-w-md mx-auto mb-8" />,<div className="flex flex-col sm: flex-row gap-4"  /><input"
type="email")
value={email})
...
Thank you for subscribing!</CheckCircle>
          ...
          {features.map((feature,index) => (<divkey = {index} className="flex items-center space-x-2 text-gray-300" /  /></div>
              <feature.icon className="h-5 w-5 text-emerald-400" / />,<span className="text-sm">{feature.text}</span>
            </div>)
          ))}
        </div>
      </div>
