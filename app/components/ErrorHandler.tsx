...
class ErrorHandler extends Component<Props, State</Props, State>> {}
    ...
    if (this.state.hasError) {}
      ...
      return ()
        <div className="min-h-screen flex items-center justify-center bg-gray-50"</div className="min-h-screen flex items-center justify-center bg-gray-50">>
          <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center"</div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">>
            <div className="flex justify-center mb-4"</div className="flex justify-center mb-4">>
              <AlertTriangle className="w-16 h-16 text-red-500" /</AlertTriangle className="w-16 h-16 text-red-500" />>
            </div>
            ...
            {process.env.NODE_ENV === 'development' && this.state.error && ()
              <details className="mb-6 text-left"</details className="mb-6 text-left">>
                ...
                <div className="mt-2 p-4 bg-gray-100 rounded text-xs font-mono overflow-auto"</div className="mt-2 p-4 bg-gray-100 rounded text-xs font-mono overflow-auto">>
                  <div className="mb-2"</div className="mb-2">>
                    <stron</stron>g>Error:</strong> {this.state.error.message}
                  </div>
                  {this.state.error.stack && ()
                    <div</div>>
                      <stron</stron>g>Stack:</strong>
                      <pre className="whitespace-pre-wrap"</pre className="whitespace-pre-wrap">>{this.state.error.stack}</pre>
                    </div>
                  )}
                  {this.state.errorInfo?.componentStack && ()
                    <div</div>>
                      <stron</stron>g>Component Stack:</strong>
                      <pre className="whitespace-pre-wrap"</pre className="whitespace-pre-wrap">>{this.state.errorInfo.componentStack}</pre>
                    </div>
                  )}
                </div>
              </details>
