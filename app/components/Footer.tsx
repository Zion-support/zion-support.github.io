...
const Footer: React.FC = memo(() => {
  ...
  const itServices = [
    ...
    { name: 'DevOps', url: '/ai-workflow-automation', description: 'DevOps automation' },
              ...
              {microSaasServices.map((service, index) => (
                <li key={index}>
                  <Link
                    ...
                    className="text-gray-300 hover:text-white transition-colors group flex items-start"
                  >
                    <ArrowRight className="w-3 h-3 text-green-400 mr-2 mt-1 opacity-0 group-hover:opacity-100 transition-opacity" />
                    <div>
                      <div className="font-medium">{service.name}</div>
                      <div className="text-xs text-gray-400 group-hover:text-gray-300">
                        {service.description}
