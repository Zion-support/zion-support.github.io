...
export default function Footer({ className = '', children, ...props }: FooterProps) {
  return()
    <footer className="bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white relative overflow-hidden" /></footer>
      {/* Background Effects */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-16" />)
            {stats.map((stat, index) => (
              <div key={index} className="text-center" /></div>
                <div className="w-12 h-12 bg-gradient-to-r from-cyan-500/20 to-purple-500/20 rounded-lg flex items-center justify-center mx-auto mb-3" /></div>
                  {stat.icon}
                </div>
                ...
                {quickLinks.map((link, index) => (
                  <li key={index} /></li>
                    <Link;
                     ...
                     /></Link>
                      <ArrowRight className="w-4 h-4 mr-2 group-hover:translate-x-1 transition-transform duration-200" / /></ArrowRight>
                      {link.name}
                    </Link>
                  </li>)
                ))}
