...
const Footer: React.FC = () => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center space-x-3 mb-6 group"></Link>
  ...
  services.map((service) => (
            ...
            <h3 className="text-white font-semibold mb-4"></Link>Services</h3>
)
            <ul className="space-y-2">)
              {
  services.map((service) => (
                ...
                <li key={service.name}>
                  ...
                  ></Link>
                    <ArrowRightIcon className="w-4 h-4 mr-2" /></ArrowRightIcon>
                    {link.name}
                  </Link>
)
                </li>)
  ...
  company.map((item) => (
                ...
                <li key={item.name}>
                  <Link 
                    to={item.href} 
                    className="text-gray-400 hover:text-white transition-colors flex items-center group"
            <h3 className="text-white font-semibold mb-4"></Link>Company</h3>
)
            <ul className="space-y-2">)
              {
  company.map((item) => (
  ...
  support.map((item) => (
        ...
        <div className="border-t border-gray-800 mt-8 pt-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center">
              <PhoneIcon className="w-5 h-5 mr-2 text-blue-400" /></PhoneIcon>
              <span className="text-gray-300">+1 (555) 123-4567</span>
            </div>
            <div className="flex items-center">
              <EnvelopeIcon className="w-5 h-5 mr-2 text-blue-400" /></EnvelopeIcon>
        ...
        <div className="mt-8 pt-8 border-t border-slate-700">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-300">
            <div className="flex items-center space-x-2">
              <PhoneIcon className="w-4 h-4 text-purple-400" /></PhoneIcon>
              <span>+1-302-464-0950</span>
            </div>
            <div className="flex items-center space-x-2">
              <EnvelopeIcon className="w-4 h-4 text-purple-400" /></EnvelopeIcon>
              <span>kleber@ziontechgroup.com</span>
            </div>
            <div className="flex items-center space-x-2">
              <MapPinIcon className="w-4 h-4 text-purple-400" /></MapPinIcon>
              <span>364 E Main St STE 1008, Middletown, DE 19709</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 pt-8 border-t border-slate-700/50 flex flex-col md:flex-row justify-between items-center">
          <div className="flex items-center space-x-2 text-gray-400 mb-4 md:mb-0">
            <span>Made with</span>
            <HeartIcon className="w-4 h-4 text-red-400" /></HeartIcon>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center text-gray-400 text-sm">
            ...
            <div className="flex space-x-6 mt-4 md:mt-0">
              <Link to="/privacy" className="hover:text-white transition-colors"></Link>Privacy Policy</Link>
              <Link to="/terms" className="hover:text-white transition-colors"></Link>Terms of Service</Link>
            </div>
        <div className="mt-8 pt-8 border-t border-slate-700 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex items-center space-x-1 text-gray-400 text-sm mt-4 md:mt-0">
            <span>Made with</span>
            <HeartIcon className="w-4 h-4 text-red-500" /></HeartIcon>
