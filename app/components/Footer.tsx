...
export default function Footer() {
  ...
  const itServices = [
    { name: 'Cloud Migration', path: '/cloud-services', price: '$2,500/month' },
    { name: 'Cybersecurity Solutions', path: '/ai-cybersecurity', price: '$1,800/month' },
    { name: 'Mobile Development', path: '/it-services', price: '$3,000/month' },
    { name: 'Data Analytics Pro', path: '/ai-data-analytics', price: '$1,500/month' },
    { name: '5G Implementation', path: '/5g-implementation', price: '$5,000/month' },
    { name: 'Digital Transformation', path: '/digital-transformation', price: '$3,000/month' }
  ];

  const microSaasServices = [
    { name: 'AI Task Manager', path: '/ai-task-manager', price: '$19/month' },
    { name: 'AI Expense Tracker', path: '/ai-expense-tracker', price: '$15/month' },
    { name: 'AI Password Manager', path: '/ai-password-manager', price: '$12/month' },
    { name: 'AI Invoice Generator', path: '/ai-invoice-generator', price: '$25/month' },
    { name: 'AI Customer Support Bot', path: '/ai-customer-support-bot', price: '$35/month' },
    { name: 'AI Website Builder', path: '/ai-website-builder', price: '$39/month' }
  ];
              ...
              {aiServices.map((service, index) => (
                <li key={index}>
                  ...
                  >
                    ...
                    <span className="group-hover:text-purple-300 transition-colors">{service.name}</span>
                    <span className="ml-auto text-xs text-purple-400/70 group-hover:text-purple-300 transition-colors">{service.price}</span>
                  </Link>
                </li>
              ))}
            </ul>
            ...
            <ul className="space-y-3">
              {itServices.map((service, index) => (
                <li key={index}>
                  ...
                  >
                    <ArrowRight className="w-3 h-3 mr-2 group-hover:translate-x-1 transition-transform duration-300" />
                    <span className="group-hover:text-blue-300 transition-colors">{service.name}</span>
                    <span className="ml-auto text-xs text-blue-400/70 group-hover:text-blue-300 transition-colors">{service.price}</span>
                  </Link>
                </li>
              ))}
            </ul>
            ...
            <ul className="space-y-3">
              {microSaasServices.map((service, index) => (
                <li key={index}>
                  ...
                  >
                    <ArrowRight className="w-3 h-3 mr-2 group-hover:translate-x-1 transition-transform duration-300" />
                    <span className="group-hover:text-emerald-300 transition-colors">{service.name}</span>
                    <span className="ml-auto text-xs text-emerald-400/70 group-hover:text-emerald-300 transition-colors">{service.price}</span>
                  </Link>
                </li>
              ))}
            </ul>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12 pt-8 border-t border-white/10">
          ...
          <div>
            ...
            <ul className="space-y-2">
              ...
              <li><Link to="/api-docs" className="text-gray-300 hover:text-white transition-colors">API Reference</Link></li>
              <li><Link to="/status" className="text-gray-300 hover:text-white transition-colors">Status Page</Link></li>
