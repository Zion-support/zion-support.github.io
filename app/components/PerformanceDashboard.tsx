  ...
  return ("</>")"
    <div className="fixed bottom-4 right-4 bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 w-80 max-h-96 overflow-y-auto z-50 border">"</>"
      <div className="flex justify-between items-center mb-4">"</>"
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Performance Dashboard</>
        </h3></>"
        <button"
      ...
      <div className="space-y-3"></>"
        <div>"</>"
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Performance Metrics</>"
          </h4>"</>"
          <div className="space-y-1">{Object.entries(metrics).map(([key, value]) => ("</>")"
            ...
            {Object.keys(metrics).length === 0 && ("</>")"
              <p className="text-sm text-gray-500 dark:text-gray-400">No metrics available yet</>
              </p>)}</>
          </div></>"
        </div></>"
        <div>"</>"
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">System Info</>"
          </h4>"</>"
