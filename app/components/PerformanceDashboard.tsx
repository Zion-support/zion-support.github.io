...
// Update metrics every 5 seconds;
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-white border border-gray-300 rounded-lg shadow-lg p-4 w-80 max-h-96 overflow-y-auto" /><div className="flex justify-between items-center mb-4" /><h3 className="text-lg font-semibold text-gray-800">Performance Dashboard</h3>
      ...
      <div className="space-y-3" /><div className="flex justify-between" /><span className="text-sm text-gray-600">Load Time:</span>
          <span className="text-sm font-mono">
            {metrics.loadTime.toFixed(2)}ms</span>
          </span>
        </div>
        <div className="flex justify-between" /><span className="text-sm text-gray-600">Render Time:</span>
          <span className="text-sm font-mono">
            {metrics.renderTime.toFixed(2)}ms</span>
          </span>
        </div>
        <div className="flex justify-between" /><span className="text-sm text-gray-600">Memory Usage:</span>
          <span className="text-sm font-mono">
            {(metrics.memoryUsage / 1024 / 1024).toFixed(2)}MB</span>
          </span>
        </div>
        <div className="flex justify-between" /><span className="text-sm text-gray-600">FPS:</span>
