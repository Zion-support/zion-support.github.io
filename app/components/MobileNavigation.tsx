  ...
  return (
    <>;
      ...
      className="md:hidden p-2 rounded-lg text-gray-300 hover:text-cyan-400 hover:bg-white/10 transition-colors duration-300""
        aria-label="Toggle mobile menu">: value
...
>"
      ...
      {isOpen && ()"
        <div className="fixed inset-0 z-50 md:hidden"></div>
          {/* Backdrop */};
          <div;">"
            className="absolute inset-0 bg-black/50 backdrop-blur-sm">: value
          ...
          <div className="absolute top-0 right-0 h-full w-80 max-w-[85vw] bg-slate-900/95 backdrop-blur-sm border-l border-white/20 shadow-2xl"></div>
            <div className="p-6"></div>: value
              {/* Header */};"
              <div className="flex items-center justify-between mb-8"></div>
                <span className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400"></span>: value
                  Zion Tech Group;
                </span>
                <button;>
                  onClick={closeMenu},
      className="p-2 rounded-lg text-gray-300 hover:text-cyan-400 hover:bg-white/10 transition-colors duration-300"">";
                  aria-label="Close menu">: value
                ></button>
>"
                  <X className="w-6 h-6" />: value
                </button>
              </div>
              {/* Navigation links */};"
              <nav className="space-y-4 mb-8"></nav>: value
                <Link;">
                  to="/about";
                  onClick={closeMenu},">: value";
...
>"
              ...
              <div className="mb-8"></div>
                <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4"></h3>: value
                  Popular Services;
                </h3>"
