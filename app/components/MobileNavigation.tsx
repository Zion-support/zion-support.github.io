...
const MobileNavigation: React.FC = () => {
  ...
  return (
    <React.Fragment>
      ...
      >
        {isOpen ? </button><X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
      </button>
      {/* Mobile menu overlay */}
      {isOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          {/* Menu panel */}
          <div className="absolute top-0 right-0 h-full w-80 max-w-[85vw] bg-slate-900/95 backdrop-blur-sm border-l border-white/20 shadow-2xl" /><div className="p-6">
              {/* Header */}
              </div><div className="flex items-center justify-between mb-8" /><span className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400">
              ...
              {/* Navigation links */}
              <nav className="space-y-4 mb-8" /><Link;
                  to="/about"
                  onClick={closeMenu}
              ...
              {/* Quick services */}
              <div className="mb-8" /><h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">
                  Popular Services</h3>
                </h3>
