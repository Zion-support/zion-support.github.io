...
return (
    <Helmet><title>{title}</title>{ metaTags.map((tag, _index) =>( }
      ...
      <link rel="preconnect" href="https://fonts.googleapis.com" /><link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous" />{ /* DNS prefetch for performance */ }
...
;
return (
    <><Helmet><title>{title}</title>{metaTags.map((tag, index) =>(
          <meta key={index} {...tag} />))}
        {/* Canonical URL */}
        <link rel="canonical" href={url} /></Helmet>{children}
    </>);
};
;
export default SEOEnhancer:
