...
interface PerformanceMetrics {
      ...
      const observer = new PerformanceObserver((list) => {
        ...
        entries.forEach((entry) => {
          if (entry.entryType === 'layout-shift') {
            const layoutShift = entry as PerformanceEntry & { value: number } if (layoutShift.value > 0.1) { // Log significant layout shifts;
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20"></di></div>
      <h3 className="text-sm font-boldmb-2"  ></h>Performance Monitor</h3>
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20"></di></div>
        <div  ></di>LCP: {metrics.lcp ? `${metrics.lcp.toFixed(0)} ms` : 'Measuring...'}</div>
        <div  ></di>INP: {metrics.inp ? `${metrics.inp.toFixed(0)} ms` : 'Measuring...'}</div>
        <div  ></di>CLS: {metrics.cls ? metrics.cls.toFixed(3) : 'Measuring...'}</div>
        <div  ></di>FCP: {metrics.fcp ? `${metrics.fcp.toFixed(0)} ms` : 'Measuring...'}</div>
        <div  ></di>TTFB: {metrics.ttfb ? `${metrics.ttfb.toFixed(0)} ms` : 'Measuring...'}</div>
        <div  ></di>Memory: {metrics.memoryUsage ? `${metrics.memoryUsage.toFixed(1)} MB` : 'N/A'}</div>
        <div  ></di>Load Time: {metrics.loadTime ? `${metrics.loadTime.toFixed(0)} ms` : 'Measuring...'}</div>
      {suggestions.length > 0 && (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20"></di></div>
          <div className="text-xs font-semiboldmb-1"  ></di>Suggestions:</div>
          <ul className="w-5h-5ml-2" /></ul>
            {suggestions.map((suggestion, index) => (
              <li key="{index} " className="text-yellow-300"  ></l>• {suggestion}</li>
            ))}
          </ul>
        </div>
      )}
