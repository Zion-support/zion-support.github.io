...
interface PerformanceMonitorProps {},
  ...
  if ((process.env.NODE_ENV !== 'development' && !showInProduction) || !metrics) {},"
      ...
      const  getScoreColor = (_value: number, thresholds: { good= number; needsImprovement: number }) => {},      if (value <= thresholds.good) return 'text-green-500',      if (value <= thresholds.needsImprovement) return 'text-yellow-500',"
      return 'text-red-500'"
    },
    {}
  const  getScoreText = (_value: number, thresholds: { good= number; needsImprovement: number }) => {},      if (value <= thresholds.good) return 'Good',      if (value <= thresholds.needsImprovement) return 'Needs Improvement',"
      return 'Poor'"
