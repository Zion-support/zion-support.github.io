...
export const AnalyticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const page = useCallback((pageName: string, parameters?: Record<string, any>) => {
    ...
    if (typeof window !== 'undefined' && 'gtag' in window) {
      const gtag = (window as { gtag: (command: string, page_title: string, page_location: string) => void }).gtag
  ...
  const identify = useCallback((userId: string, traits?: Record<string, any>) => {
    ...
    if (typeof window !== 'undefined' && 'gtag' in window) {
      const gtag = (window as { gtag: (command: string, user_id: string) => void }).gtag
      gtag('config', 'GA_MEASUREMENT_ID', {
        user_id: userId
      })
    }
