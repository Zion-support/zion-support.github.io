...
interface AnalyticsContextType {
  track: (event: strin,
      g, properties?: Record<string, unknown>) => void
  identify: (userId: strin,
      g, traits?: Record<string, unknown>) => void
  page: (name: strin,
      g, properties?: Record<string, unknown>) => void
}
...
exportconstAnalyticsProvider: React.FC<AnalyticsProviderProp s>= ({childre,n,}) => {
  useEffect(() => {
  ...
  consttrack= (event: strin,
      g,properties?:Record<string, unknown>) => {
  if (type of windo w !=="undefined") {
      // Google Analytics
      if ((windo w as unknown as { gtag?: (...args: unknown[]) => voi,d
}).gtag) {(windo w as unknown as { gtag: (...args: unknown[]) => voi,d}).gtag('event',
          event,
          properties,
        )
      }
  ...
  constidentify= (userId: strin,
      g,traits?:Record<string, unknown>) => {
  if (type of windo w !=="undefined") {
      // Google Analytics
      if ((windo w as unknown as { gtag?: (...args: unknown[]) => voi,d
}).gtag) {(windo w as unknown as { gtag: (...args: unknown[]) => voi,d}).gtag('config',
