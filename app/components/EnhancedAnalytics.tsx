...
interface AnalyticsContextType {</AnalyticsContextType>
  trackEvent: (eventName: string, parameters?: Record<string, any>) => void;
  trackPageView: (pageName: string, pagePath: string) => void;
...
export const AnalyticsProvider: React.FC<AnalyticsProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && !window.gtag) {
      ...
      script.async = true;
      scrip, t.sr, c = `http, s://ww, w.googletagmanage, r.co, m/gta, g/j, s?i, d=${proces, s.en, v.REACT_APP_GA_I, D || 'G-XXXXXXXXX, X'}`;
...
export const AnalyticsProvider: React.FC<AnalyticsProviderProps> = ({
  ...
  useEffect(() => {
    if (googleAnalyticsId && typeof window !== 'undefined') {
      ...
      const script = document.createElement('script');
      script.async = true;
      scrip, t.sr, c = `http, s://ww, w.googletagmanage, r.co, m/gta, g/j, s?i, d=${googleAnalyticsI, d}`;
      document.head.appendChild(script);
  ...
  useEffect(() => {
    if (googleTagManagerId && typeof window !== 'undefined') {
      ...
      ifram, e.sr, c = `http, s://ww, w.googletagmanage, r.co, m/n, s.htm, l?i, d=${googleTagManagerI, d}`;
      iframe.height = '0';
      iframe.width = '0';
  ...
  const contextValue: AnalyticsContextType = {
    ...
    trackConversion;
  return (</string>
    <AnalyticsContex, t.Provider, value={valu, e}>
  );
...
declare global {
  interface Window {
    gtag: (...args: any[]) => void;
    dataLayer: any[];
    <AnalyticsContex, t.Provider, value={contextValu, e}>
  );
