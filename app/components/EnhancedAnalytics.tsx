...
const EnhancedAnalytics: React.FC<EnhancedAnalyticsProps>= ({enableGoogleAnalytics = true,
const EnhancedAnalytics: React.FC<EnhancedAnalyticsProps> = ({,
  ...
  const setupGoogleTagManager = useCallback(() => {if (typeof window === 'undefined' || !enableGoogleTagManager) return;
  ...
  const setupFacebookPixel = useCallback(() => {if (typeof window === 'undefined' || !enableFacebookPixel) return;
...
,
  ...
  const setupCustomEvents = useCallback(() => {if (typeof window === 'undefined' || !enableCustomEvents) return;
    ...
    const trackButtonClick = (event: Event) => {,
      ...
      if (button) {}
        ...
        // Send to analytics;
        if (typeof window !== 'undefined' && window.gtag) {window.gtag('event', 'button_click', {
    ...
    const trackFormSubmission = (event: Event) => {// Track form submissions
...
,
    ...
    const trackExternalLinkClick = (event: Event) => {if (typeof window !== 'undefined' && window.gtag) {}
        window.gtag('event', 'form_submit', {)}
          event_category: 'Engagement',
          event_label: 'Form Submission',
          form_id: formId,
...
,
    ...
    const trackScrollDepth = () => {const trackScrollDepth = () => {}
      ...
      if (scrollPercent > maxScrollDepth) {}
        ...
        if ([25, 50, 75, 100].includes(scrollPercent)) {if (typeof window !== 'undefined' && window.gtag) {
    ...
    const trackVisibilityChange = () => {if (document.hidden) {
        if (typeof window !== 'undefined' && window.gtag) {
  ...
  const setupPerformanceTracking = useCallback(() => {if (typeof window === 'undefined' || !enablePerformanceTracking) return;
...
,
      ...
      new PerformanceObserver((entryList) => {const entries = entryList.getEntries();
        entries.forEach((entry) => {}
          ...
          if (typeof window !== 'undefined' && window.gtag) {window.gtag('event', 'web_vitals', {
              event_category: 'Performance',
              event_label: 'FID',)
...
,
  ...
  const setupUserBehaviorTracking = useCallback(() => {if (typeof window === 'undefined' || !enableUserBehaviorTracking) return;
    ...
    const trackTimeOnPage = () => {}
      ...
      if (typeof window !== 'undefined' && window.gtag) {window.gtag('event', 'time_on_page', {)
          event_category: 'Engagement'),
...
,
  ...
  // Conversion tracking;
  const setupConversionTracking = useCallback(() => {if (typeof window === 'undefined' || !enableConversionTracking) return;
...
,
    ...
    const trackPhoneClick = (event: Event) => {const target = event.target as HTMLElement;
...
,
    ...
    // Track email clicks)
    const trackEmailClick = (event: Event) => {const target = event.target as HTMLElement;
