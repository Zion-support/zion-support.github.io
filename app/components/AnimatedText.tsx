...
interfaceAnimatedTextProps {text: string;
  className?: string;
  delay?: number;
  duration?: number;
...
;
constAnimatedText: React.FC<AnimatedTextProps /> = ({text,;
  constclassName = '',;
  delay = 0,;
  // duration = 1000,;
  type = 'fade';
...
;
  useEffect(() => {if (consttype = == 'typing' && isVisible) {
    ...
    } elseif (isVisible) {setDisplayText(text);
    }
  }, [isVisible, currentIndex, text, type]);
;
  constgetAnimationClasses = () => {constbaseClasses = 'transition-allduration-1000';
    switch (type) {
      case 'fade':;
