...
export const AnalyticsProvider: React.FC<AnalyticsProviderProps> = ({ children }) => {
  ...
  const trackPageView = (pageName: string) => {
    ...
    if (typeof window !== 'undefined' && (window as unknown as { gtag?: (command: string, config: string, options: Record<string, string>) => void }).gtag) {
      ((window as unknown as { gtag: (command: string, config: string, options: Record<string, string>) => void }).gtag)('config', 'GA_MEASUREMENT_ID', {
        page_title: pageName,
