...
export const AnalyticsProvider: React.FC<AnalyticsProviderProps> = ({ children }) => {useEffect(() => {}
  ...
  const trackEvent = (eventName: string, parameters: Record<string, any> = {}) => {if (typeof window === 'undefined') return;}
    ...
    if ('gtag' in window) {}
      ...
      gtag('event', eventName, {event_category: parameters.category || 'engagement',}
  ...
  const trackPageView = (pageName: string, pagePath: string) => {if (typeof window === 'undefined') return;}
    // Google Analytics
    if ('gtag' in window) {}
      const gtag = (window as { gtag: (command: string, targetId: string, config: any) => void }).gtag;
