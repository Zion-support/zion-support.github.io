...
const AnalyticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  useEffect(() => {
    ...
    const initAnalytics = () => {
      ...
      (window as unknown as { gtag: (..._args: unknown[]) => void }).gtag = gtag;
    ...
    const trackPageView = () => {
      if (typeof window !== 'undefined' && (window as unknown as { gtag: (..._args: unknown[]) => void }).gtag) {
        (window as unknown as { gtag: (..._args: unknown[]) => void }).gtag('config', GA_TRACKING_ID, {
          page_title: document.title,
          page_location: window.location.href,
          send_page_view: true
        });
    ...
    const trackInteractions = () => {
      ...
      document.addEventListener('click', (e) => {
        ...
        if (target.tagName === 'A' || target.tagName === 'BUTTON') {
          ...
          if ((window as unknown as { gtag: (..._args: unknown[]) => void }).gtag) {
            (window as unknown as { gtag: (..._args: unknown[]) => void }).gtag('event', 'click', {
      ...
      document.addEventListener('submit', (e) => {
        ...
        if ((window as unknown as { gtag: (..._args: unknown[]) => void }).gtag) {
          (window as unknown as { gtag: (..._args: unknown[]) => void }).gtag('event', 'form_submit', {
            event_category: 'engagement',
