...
const ImprovedPerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  const updateMetrics = useCallback((newMetrics: Partial</PerformanceMetrics><PerformanceMetrics>) => {
    setMetrics(prev => {
  ...
  return (
    </PerformanceMetrics><div className="fixed bottom-4 right-4 z-50" /><button;
      ...
      >
        </button><Activity className="w-5 h-5" />
      </button>
      {isVisible && (
        <div className="absolute bottom-16 right-0 w-80 bg-slate-800/95 backdrop-blur-sm border border-cyan-500/30 rounded-lg p-4 text-white" /><div className="flex items-center justify-between mb-4" /><h3 className="text-lg font-semibold flex items-center gap-2" /><TrendingUp className="w-5 h-5" />
              Performance;
            </h3>
            <button;
          ...
          {/* Performance Score */}
          <div className="mb-4" /><div className="flex items-center justify-between mb-2" /><span className="text-sm text-gray-300">Overall Score</span>
            ...
            <p className="text-xs text-gray-400 mt-1">
              {getScoreLabel(performanceScore)}</p>
            </p>
          </div>
          {/* Metrics */}
          <div className="space-y-2 text-sm">
            {metrics.lcp && (
              </div><div className="flex justify-between" /><span className="text-gray-300">LCP</span>
            ...
            {metrics.fid && (
              <div className="flex justify-between" /><span className="text-gray-300">FID</span>
            ...
            {metrics.cls && (
              <div className="flex justify-between" /><span className="text-gray-300">CLS</span>
            ...
            {metrics.ttfb && (
              <div className="flex justify-between" /><span className="text-gray-300">TTFB</span>
            ...
            {metrics.memory && (
              <div className="flex justify-between" /><span className="text-gray-300">Memory</span>
                <span className="text-cyan-400">
                  {metrics.memory.toFixed(1)}MB</span>
                </span>
              </div>
            )}
            {metrics.connection && (
              <div className="flex justify-between" /><span className="text-gray-300">Connection</span>
                <span className="text-cyan-400">
                  {metrics.connection}</span>
                </span>
              </div>
            )}
          </div>
