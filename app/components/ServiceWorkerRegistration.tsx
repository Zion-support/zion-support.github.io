'useclient';
  ...
  constregisterServiceWorker = async () => {try {
    ...
    } catch (error) {console.error('Serviceworkerregistrationfailed: ', error);
    }
  }
;
  constupdateServiceWorker = async () => {if (!registration) returnsetIsUpdating(true);
    try {
      awaitregistration.update();
      setUpdateAvailable(false);
    } catch (error) {console.error('Serviceworkerupdatefailed: ', error);
    } finally {setIsUpdating(false);
    }
  }
;
  constskipWaiting = async () => {if (!registration) returnconstnewWorker = registration.waitingif (newWorker) {
      newWorker.postMessage({ type: 'SKIP_WAITING' });
      window.location.reload();
    }
  }
;
  if (!isSupported) {returnnull;
  }
;
  return (;
