...
import { Link , ChevronDown, Phone, Mail, MapPin, Menu, X, Brain, Cloud, Shield, Code, BarChart, Users, Zap, ArrowRight, Sparkles, Cpu, Target, Globe, Database, Smartphone, Lock, TrendingUp, Settings, Calendar, CheckSquare, FileText, MessageCircle, Heart, DollarSign, Box, Monitor, Link as LinkIcon, Server, Package  } from "react-router-dom";const Navigation: React.FC = () => {
...
const closeAllMenus = () => {
  ...
  return(<nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${)
      isScrolled ? 'bg-slate-900/95 backdrop-blur-md shadow-lg' : 'bg-transparent'}`} /><div className="max-w-7xl mx-auto px-4 sm: px-6 lg:px-8">,</div>
        <div className="flex items-center justify-between h-16">{/* Logo */</div>} <div className="flex-shrink-0" /><div className="flex-shrink-0" /><Link to="/" className="flex items-center space-x-2" /><div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg flex items-center justify-center" /><Brain className="w-5 h-5 text-white" />
          {/* Desktop Navigation */} <div className="hidden lg: flex items-center space-x-8">,</div>
            <Link to="/" className="text-gray-300 hover: text-cyan-400 transition-colors duration-300 font-medium">,
              Home;</Link>
            </Link>
            {/* AI Services Dropdown */} <div className="relative group" /><button;
                ...
                <Brain className="w-4 h-4" />
                  <div className="grid grid-cols-1 gap-2">{aiServices.map((service) => (</div>
                      <Link;
                        ...
                        <div /><div className="text-white font-medium">{service.name</div>}</div>
                          <div className="text-xs text-gray-400">{service.description</div>}</div>
          <div className="hidden lg: block">,</div>
            <div className="ml-10 flex items-baseline space-x-8" /><Link to="/" className="text-gray-300 hover: text-white px-3 py-2 rounded-md text-sm font-medium transition-colors">,
                Home;</Link>
              </Link>
            ...
            {/* IT Services Dropdown */} <div className="relative group" /><button;
                ...
                <Cloud className="w-4 h-4" />
                  <div className="grid grid-cols-1 gap-2">{itServices.map((service) => (</div>
                      <Link;
                        ...
                        <div /><div className="text-white font-medium">{service.name</div>}</div>
                          <div className="text-xs text-gray-400">{service.description</div>}</div>
            {/* Micro SAAS Dropdown */} <div className="relative group" /><button;
                ...
                <Cpu className="w-4 h-4" />
                  <div className="grid grid-cols-1 gap-2">{microSaasServices.map((service) => (</div>
                      <Link;
                        ...
                        <div /><div className="text-white font-medium">{service.name</div>}</div>
                          <div className="text-xs text-gray-400">{service.description</div>}</div>
            {/* Emerging Technologies Dropdown */} <div className="relative group" /><button;
          ...
          {/* Mobile Menu Button */} <div className="lg: hidden">,</div>
            ...
            >
              {isOpen ? </button><X className="h-6 w-6" /> : <Menu className="h-6 w-6" />} </button>
          </div>
        </div>
        {/* Mobile Navigation */}{isOpen && (
              {/* Mobile AI Services */} <div /><button;
                  ...
                  <span className="flex items-center space-x-2" /><Brain className="w-4 h-4" />
                  <div className="mt-2 ml-4 space-y-2">{aiServices.slice(0, 8).map((service) => (</div>
                    ...
                    <Link;
                      ...
                      onClick={closeAllMenus}>
              {/* Mobile IT Services */} </Link><div /><button;
                  ...
                  <span className="flex items-center space-x-2" /><Cloud className="w-4 h-4" />
                  <div className="mt-2 ml-4 space-y-2">{itServices.map((service) => (</div>
                      <Link;
                        key={service.name}to={service.href}className="block text-sm text-gray-400 hover: text-cyan-400 transition-colors duration-300",
                        onClick={closeAllMenus}>
                        {service.name} </Link>
              {/* Mobile Micro SAAS */} <div /><button;
                  ...
                  <span className="flex items-center space-x-2" /><Cpu className="w-4 h-4" />
                  <div className="mt-2 ml-4 space-y-2">{microSaasServices.slice(0, 8).map((service) => (</div>
                    ...
                    <Link;
                      to="/micro-saas"
                      className="block text-sm text-cyan-400 hover: text-cyan-300 transition-colors duration-300 font-medium",
                      onClick={closeAllMenus} /><div className="lg: hidden">,</div>
            <div className="px-2 pt-2 pb-3 space-y-1 sm: px-3 bg-slate-800 rounded-lg mt-2">,</div>
              ...
              {/* Mobile Contact Info */} <div className="pt-4 border-t border-cyan-500/20 space-y-2" /><a;
                  href="tel: +13024640950",
                  className="flex items-center space-x-2 text-cyan-400 hover: text-cyan-300 transition-colors duration-300 font-medium">,
                  </a><Phone className="w-4 h-4" />
          ...
          <div className="lg: hidden">,</div>
            <div className="px-2 pt-2 pb-3 space-y-1 sm: px-3 bg-slate-800/95 backdrop-blur-md rounded-lg mt-2 border border-white/10">,</div>
              ...
              {/* Mobile Contact Info */} <div className="border-t border-white/10 pt-3 mt-3" /><a href="tel: +13024640950" className="flex items-center text-cyan-400 hover:text-cyan-300 px-3 py-2 rounded-md text-base font-medium">,
