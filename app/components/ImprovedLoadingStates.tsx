...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({ 
  ...
  return (
    </LoadingSpinnerProps><div className={`flex items-center justify-center ${className}`} /><Loader2 className={`${sizeClasses[size]} animate-spin text-cyan-400`} />
    </div>
  );
};
...
export const LoadingPage: React.FC<LoadingPageProps> = ({ 
  ...
  return (
        ...
        {showIcon && (
          </div><div className="mb-6" /><div className="w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4" /><Brain className="w-8 h-8 text-white animate-pulse" />
            </div>
          </div>
        )}
...
export const LoadingCard: React.FC<LoadingCardProps> = ({ 
  ...
  return (
    </LoadingCardProps><div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-8 text-center">
      {icon && (
...
export const ServiceLoading: React.FC<ServiceLoadingProps> = ({ serviceName }) => {
  const getServiceIcon = (service: string) => {
    if (service.toLowerCase().includes('ai')) return </ServiceLoadingProps><Brain className="w-8 h-8 text-cyan-400" />;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center" /><div className="text-center max-w-md mx-auto px-6" /><div className="mb-6" /><div className="w-20 h-20 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4">
            {getServiceIcon(serviceName)}</div>
          </div>
        </div>
        <LoadingSpinner size="lg" />
