...
const AIToolCard: React.FC<AIToolCardProps> = memo(({ title, description, link }) => {"
  return (<article className="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"><h3 className="text-lg font-semibold text-gray-900 mb-2">{title}</h3><p className="text-gray-600 text-sm mb-3 leading-relaxed">
        {description}
      </p><a "
import React, { memo } from 'react'
...
export default AIToolCard</article>)
  )})
AIToolCard.displayName = 'AIToolCard'
