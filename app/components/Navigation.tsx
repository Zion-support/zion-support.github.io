...
export default function Navigation() {
  ...
  const aiServices = [
    ...
    { name: 'AI Computer Vision', path: '/ai-computer-vision', icon: <Cpu className="w-4 h-4" /> },
    { name: 'AI Data Analytics', path: '/ai-data-analytics', icon: <Database className="w-4 h-4" /> }
  ]

  const itServices = [
    { name: 'Cloud Migration', path: '/cloud-services', icon: <Cloud className="w-4 h-4" /> },
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="absolute top-full left-0 mt-2 w-64 bg-slate-800 rounded-lg shadow-xl border border-white/10 py-2">
                  {aiServices.map((service, index) => (
                    <Link
                      key={index}
                      to={service.path}
                      className="flex items-center px-4 py-2 text-gray-300 hover:text-cyan-400 hover:bg-slate-700 transition-colors"
        ...
        {isOpen && (
          <div className="lg:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 bg-slate-800 rounded-lg mt-2 border border-white/10">
              <Link
                to="/"
              ...
              <div className="px-3 py-2">
                <div className="text-white font-semibold mb-2">AI Services</div>
                <div className="space-y-1 ml-4">
                  {aiServices.map((service, index) => (
                    <Link
                      key={index}
                      to={service.path}
                      className="block px-3 py-1 text-gray-300 hover:text-cyan-400 transition-colors text-sm"
                      onClick={() => setIsOpen(false)}
                    >
                      {service.name}
                    </Link>
              ...
              {/* Mobile IT Services */}
              <div className="px-3 py-2">
                <div className="text-white font-semibold mb-2">IT Services</div>
                <div className="space-y-1 ml-4">
                  {itServices.map((service, index) => (
                    <Link
                      key={index}
              ...
              {/* Mobile 5G Services */}
              <div className="px-3 py-2">
                <div className="text-white font-semibold mb-2">5G Services</div>
                <div className="space-y-1 ml-4">
                  {fiveGServices.map((service, index) => (
                    <Link
                      key={index}
              ...
              <div className="px-3 py-2">
                <Link
                  to="/contact"
                  className="block w-full bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-4 py-2 rounded-lg font-semibold text-center hover:from-cyan-600 hover:to-purple-700 transition-all duration-300"
                  onClick={() => setIsOpen(false)}
                >
                  Get Started
                </Link>
