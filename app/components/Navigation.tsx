...
import { Menu, X, ChevronDown, Brain, Cloud, Shield, Globe, Database, Smartphone, Cpu, Wifi, Zap, BarChart3, MessageSquare, Code2, Server, Lock, Monitor, DollarSign } from 'lucide-react'
...
export default function Navigation() {
  ...
  const aiServices = [
    ...
    { name: 'AI Customer Support Bot', path: '/ai-customer-support-chatbot', icon: <MessageSquare className="w-4 h-4" /> },
    { name: 'AI Analytics Dashboard', path: '/ai-analytics-dashboard', icon: <BarChart3 className="w-4 h-4" /> },
    { name: 'AI Process Automation', path: '/ai-automation', icon: <Zap className="w-4 h-4" /> },
    { name: 'AI Cybersecurity Suite', path: '/ai-cybersecurity-suite', icon: <Shield className="w-4 h-4" /> },
    { name: 'AI Data Analytics', path: '/ai-data-analytics', icon: <Database className="w-4 h-4" /> }
  ]

  const itServices = [
    { name: 'Cloud Migration & Management', path: '/cloud-services', icon: <Cloud className="w-4 h-4" /> },
    { name: 'Cybersecurity Solutions', path: '/ai-cybersecurity-suite', icon: <Shield className="w-4 h-4" /> },
    { name: 'Data Analytics & BI', path: '/ai-data-analytics', icon: <Database className="w-4 h-4" /> },
    { name: 'DevOps & CI/CD', path: '/it-services', icon: <Code2 className="w-4 h-4" /> },
    { name: 'Mobile Development', path: '/it-services', icon: <Smartphone className="w-4 h-4" /> },
    { name: '5G Implementation', path: '/5g-implementation', icon: <Wifi className="w-4 h-4" /> }
  ]

  const microSaasServices = [
    { name: 'AI Task Manager', path: '/micro-saas-services', icon: <Smartphone className="w-4 h-4" /> },
    { name: 'AI Expense Tracker', path: '/ai-expense-tracker', icon: <DollarSign className="w-4 h-4" /> },
    { name: 'AI Password Manager', path: '/micro-saas-services', icon: <Lock className="w-4 h-4" /> },
    { name: 'AI Invoice Generator', path: '/micro-saas-services', icon: <Monitor className="w-4 h-4" /> }
  ]
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link to="/" className="flex-shrink-0 flex items-center">
              <div className="w-8 h-8 bg-gradient-to-r from-cyan-400 to-purple-400 rounded-lg flex items-center justify-center mr-2">
          ...
          <div className="hidden md:flex items-center space-x-1">
              ...
              {activeDropdown === 'ai' && (
                <div className="absolute top-full left-0 mt-1 w-64 bg-white/95 backdrop-blur-lg rounded-lg shadow-lg border border-white/20 py-2 z-50">
                  ...
                  {aiServices.map((service, index) => (
                    <Link key={index} to={service.path} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-cyan-50">
                      {service.icon}
            ...
            <div className="relative">
              ...
              {activeDropdown === 'it' && (
                <div className="absolute top-full left-0 mt-1 w-64 bg-white/95 backdrop-blur-lg rounded-lg shadow-lg border border-white/20 py-2 z-50">
                  <Link to="/it-services" className="block px-4 py-2 text-sm text-gray-700 hover:bg-cyan-50 font-semibold">All IT Services</Link>
            ...
            <div className="relative">
              ...
              {activeDropdown === 'saas' && (
                <div className="absolute top-full left-0 mt-1 w-64 bg-white/95 backdrop-blur-lg rounded-lg shadow-lg border border-white/20 py-2 z-50">
                  <Link to="/micro-saas-services" className="block px-4 py-2 text-sm text-gray-700 hover:bg-cyan-50 font-semibold">All Micro SAAS</Link>
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white/10 backdrop-blur-lg rounded-lg mt-2 border border-white/20">
              ...
              <Link to="/5g-implementation" className="text-white hover:text-cyan-300 block px-3 py-2 rounded-md text-base font-medium" onClick={() => setIsOpen(false)}>
                5G Solutions
              </Link>
              <Link to="/contact" className="text-white hover:text-cyan-300 block px-3 py-2 rounded-md text-base font-medium" onClick={() => setIsOpen(false)}>
                Contact
              </Link>
              <Link 
                to="/contact" 
                className="block cyber-button px-3 py-2 rounded-lg text-base font-semibold hover:from-cyan-700 hover:to-purple-700 transition-all duration-300 text-center mt-4"
