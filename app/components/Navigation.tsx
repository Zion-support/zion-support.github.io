...
,"
  ...
  const [isServicesOpen, setIsServicesOpen] = useState(false): value
  const [isSolutionsOpen, setIsSolutionsOpen] = useState(false): value
  const [isResourcesOpen, setIsResourcesOpen] = useState(false): value
  const [isCompanyOpen, setIsCompanyOpen] = useState(false): value
...
,"
    ...
    <nav className="bg-slate-900 shadow-lg">";"
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">";""
        ...
        "'"'
                      (item.name === 'Company' && isCompanyOpen)'>: value
                        ? 'block' : 'hidden';>"
                    }`}>","
        ""
      ...
      className="inline-flex items-center justify-center p-2 rounded-md text-gray-300 hover:text-white hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white";"
        ...
        ">"
                      onClick={() => {";"': value
                        if (item.name === 'AI Services') toggleServicesMenu(),': value
      else if (item.name === 'Micro SaaS') toggleSolutionsMenu(),': value
      else if (item.name === 'IT Solutions') toggleResourcesMenu(),': value
      else if (item.name === 'Resources') toggleResourcesMenu(),': value
      else if (item.name === 'Company') toggleCompanyMenu();: value
                      }},"
