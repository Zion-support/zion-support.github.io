...
import { Link , Menu, X, ChevronDown, Zap, Cloud, Globe, Code, Smartphone, Brain, Wifi, Shield, Database, Users, BarChart, Settings, Mail, MessageSquare, FileText, Phone, MapPin, ArrowRight, Sparkles, Cpu, Target, Lock, TrendingUp, Calendar, CheckSquare, MessageCircle, Heart, DollarSign, Box, Monitor, Link as LinkIcon, Server, Package, Mic, Workflow, Eye, CheckCircle, ShoppingCart  } from "react-router-dom"; origin/cursor/analyze-improve-and-deploy-application-0e37;
...
'use client';
import React, { useState, useEffect } from 'react';
import { Link, useLocation , Menu, X, Phone, Mail, ChevronDown  } from "react-router-dom";
const Navigation: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);
...
onClick={() => setIsOpen(!isOpen)}
        ...
        {/* Mobile Menu */}
        {isOpen && (
<Link to="/" className="flex items-center space-x-2" /><div className="w-10 h-10 bg-gradient-to-r from-cyan-400 to-purple-600 rounded-lg flex items-center justify-center" /><span className="text-white font-bold text-xl">Z</span>
            </div>
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              </div><div key={item.name} className="relative group">
                {item.dropdown ? (
                  </div><div className="flex items-center space-x-1 text-gray-300 hover:text-cyan-400 transition-colors cursor-pointer" /><span>{item.name}</span>
                ...
                {item.dropdown && (
                  <div className="absolute top-full left-0 mt-2 w-48 bg-slate-800/95 backdrop-blur-md rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300" /><div className="py-2">
...
className="flex items-center text-cyan-400 hover:text-cyan-300 transition-colors"
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className="md:hidden bg-slate-900/95 backdrop-blur-md border-t border-slate-700" /><div className="px-2 pt-2 pb-3 space-y-1" /><Link;
                href="/"
              ...
              <Link;
                ...
                onClick={() => setIsOpen(false)}
              >
                AI Services origin/cursor/analyze-improve-and-deploy-application-10c6</Link>
              </Link>
              <Link;
 origin/cursor/analyze-improve-and-deploy-application-0ff3;
      </div>
      {/* Mobile menu */}
      {isOpen && (
        </Link><div className="lg:hidden" /><div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-slate-800/95 backdrop-blur-lg border-t border-purple-500/20" /><Link;
              to="/"
