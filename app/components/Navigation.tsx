...
const Navigation: React.FC<NavigationProps> = ({ onSidebarToggle }) => {
  ...
  const microSaasServices = useMemo(() => [
    ...
    { name: 'Zion AI Customer Insights', path: '/zion-ai-customer-insights', icon: <Brain className="w-4 h-4" /> },
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-sm border-b border-cyan-500/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            ...
            <div className="relative">
              ...
              {isAiServicesOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-slate-800/95 backdrop-blur-sm rounded-lg border border-cyan-500/20 shadow-xl z-50">
                  <div className="py-2">
                    {aiServices.map((service) => (
                      <Link
                        key={service.name}
                        to={service.path}
                        className="flex items-center space-x-3 px-4 py-3 text-sm text-gray-300 hover:text-cyan-400 hover:bg-white/5 transition-colors duration-200"
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="lg:hidden border-t border-cyan-500/20 mt-4 pt-4 pb-4">
