...
const Navigation: React.FC<NavigationProps> = ({ onSidebarToggle }) => {
  ...
  return (
    <nav className="bg-slate-900/95 backdrop-blur-md border-b border-slate-700 sticky top-0 z-50 cyber-glow">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <div className="flex-shrink-0">
            <Link to="/" className="flex items-center space-x-2 group">
              <div className="w-8 h-8 bg-gradient-to-r from-purple-600 to-cyan-600 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform cyber-glow">
                <span className="text-white font-bold text-lg">Z</span>
              </div>
              <span className="text-white font-bold text-xl group-hover:text-purple-400 transition-colors">Zion Tech Group</span>
            </Link>
          </div>
  ...
  onClick ={item.name === 'Services' ? toggleServicesMenu : toggleSolutionsMenu}
                        className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 ${
                          isActive(item.href) || (item.submenu && item.submenu.some(sub => isActive(sub.href)))
                            ...
                            : 'text-gray-300 hover:text-white hover:bg-slate-700/50 hover:cyber-glow'
                        }`}
                      >
                      ...
                      {/* Dropdown Menu */}
                      <div className={`absolute left-0 mt-2 w-64 bg-slate-800/95 backdrop-blur-md rounded-lg shadow-2xl border border-slate-700 py-2 z-50 transition-all duration-300 ${
  ...
  key ={subItem.name}
                          ...
                          >
                            <subItem.icon className="w-4 h-4 mr-3 group-hover/sub:text-purple-400 transition-colors" />
                            <span className="group-hover/sub:text-purple-400 transition-colors">{subItem.name}</span>
                          </Link>
                        ))}
                      </div>
                    </div>
  ...
  onClick ={onSidebarToggle}
      ...
      <div className={`lg:hidden transition-all duration-300 ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-slate-800/95 backdrop-blur-md border-t border-slate-700">
          {navigation.map((item) => (
            <div key={item.name}>
              {item.submenu ? (
                <div>
  ...
  key ={subItem.name}
                      ...
                      >
                        ...
                        <span className="group-hover:text-purple-400 transition-colors">{subItem.name}</span>
                      </Link>
                    ))}
                  </div>
                </div>
