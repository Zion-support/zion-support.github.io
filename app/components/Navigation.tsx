...
const Navigation = () => {"  const [isOpen, setIsOpen] :  useState(false);
  ...
  const services :  [
    ...
    <nav className="bg-gray-900 95 backdrop-blur-sm border-b border-gray-800 sticky top-0 z-50"      <div>"        <div>"          {/* Logo */}
          <div>
            <Link to="/ className="flex items-center">              <div>"                <span className="text-white font-bold text-sm">Z</span>              </div>"              <span className="text-xl font-bold text-white">Zion Tech Group</span>            </Link>"
      </div>"
          {/* Desktop Navigation */}
          <div>
            <div>
          ...
          <div>
            <button
              ...
              className="text-gray-300 hover text-white p-2"            >"              {isOpen  ? <X className="w-6 h-6" /> : <Menu className=w-6 h-6" />}"            </button>"
      </div>"
      </div>
        ...
        {isOpen && (
          <div>
            <div>
              <Link
                to: /className="text-gray-300 hover text-white block px-3 py-2 rounded-md text-base font-mediumonClick: {() : > setIsOpen(false)}"              >"                Home
              </Link>
              
              <Link
                to=/aboutclassName=text-gray-300 hover text-white block px-3 py-2 rounded-md text-base font-mediumonClick={() : > setIsOpen(false)}"              >"                About
              </Link>
              ...
              <div>
                <div className=text-gray-300 text-base font-medium mb-2">Services</div>"                <div>"                  {services.map((service) : > ("                    <Link
                      key: {service.name}
                      to={service.href}
                      className=text-gray-400 hover text-white block px-3 py-1 rounded-md text-smonClick={() : > setIsOpen(false)}"                    >"                      {service.name}
                    </Link>
                  ))}
                </div>
              </div>
              ...
              <Link
                to=/blogclassName=text-gray-300 hover text-white block px-3 py-2 rounded-md text-base font-mediumonClick={() : > setIsOpen(false)}"              >"                Blog
              </Link>
              
              <Link
                to=/contactclassName=text-gray-300 hover text-white block px-3 py-2 rounded-md text-base font-mediumonClick={() : > setIsOpen(false)}"              >"                Contact
              </Link>
              
              <Link
                to=/democlassName=bg-gradient-to-r from-cyan-500 to-purple-600 text-white block px-3 py-2 rounded-md text-base font-medium hover from-cyan-600 hover to-purple-700 transition-all duration-300onClick={() : > setIsOpen(false)}"              >"                Get Demo
              </Link>
