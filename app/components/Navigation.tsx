...
const Navigation: React.FC<NavigationProps> = ({ onSidebarToggle }) => {
  ...
  const [is""5GServicesOpen"", setIs""5GServicesOpen""] = useState(false);
'
  const navigation = [''
          ...
          <div className="md:hidden flex items-center space-x-2"></div>
            ...
            ></button>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"></svg>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 ""6h""""16M""4 ""12h""""16M""4 ""18h""16" /></path>
              </svg>
            </button>
          </div>
        </div>
