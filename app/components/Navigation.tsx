...
import { Menu, X, ChevronDown, Zap, Cloud, Globe, Code, Smartphone, Brain, Wifi, Phone, Mail, MapPin } from 'lucide-react';
          ...
          <div className="hidden lg:flex items-center space-x-8">
            ...
            <div className="relative">
              ...
              {activeDropdown === 'services' && (
                <div className="absolute top-full left-0 mt-2 w-80 bg-slate-900/95 backdrop-blur-xl rounded-lg shadow-lg border border-cyan-500/20 py-2 z-50">
                  <div className="py-2">
                    {services.map((service, index) => (
                      <div key={index}>
                        <Link
                          to={service.href}
                          className="flex items-center px-4 py-3 text-gray-300 hover:bg-cyan-500/10 transition-colors"
                        ...
                        {service.submenu && (
                          <div className="ml-8 pl-4 border-l border-cyan-500/20">
                            {service.submenu.map((subItem, subIndex) => (
                              <Link
                                key={subIndex}
                                to={subItem.href}
                                className="block px-4 py-2 text-sm text-gray-400 hover:text-cyan-400 hover:bg-cyan-500/5 transition-colors"
                                onClick={() => setActiveDropdown(null)}
                              >
                                {subItem.title}
                              </Link>
