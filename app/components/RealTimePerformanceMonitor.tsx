...
const RealTimePerformanceMonitor: React.FC = () => {
  ...
  return (
    </string><div className="fixed bottom-4 right-4 z-50">
      {/* Toggle Button */}
      </div><button;
      ...
      {/* Performance Panel */}
      {isVisible && (
        <div className="absolute bottom-16 right-0 bg-slate-800/95 backdrop-blur-sm border border-cyan-500/30 rounded-lg p-4 w-80 shadow-xl" /><div className="flex items-center justify-between mb-4" /><h3 className="text-white font-semibold text-sm">Performance Monitor</h3>
            <div className={`text-lg font-bold ${getScoreColor(score)}`}>
              {score}/100</div>
            </div>
          </div>
          {/* Metrics Grid */}
          <div className="grid grid-cols-2 gap-3 mb-4" /><div className="bg-slate-700/50 rounded-lg p-3" /><div className="flex items-center mb-1" /><Clock className="w-4 h-4 text-cyan-400 mr-2" />
            ...
            <div className="bg-slate-700/50 rounded-lg p-3" /><div className="flex items-center mb-1" /><Zap className="w-4 h-4 text-cyan-400 mr-2" />
                <span className="text-xs text-gray-300">FCP</span>
              </div>
              <div className="text-white font-semibold text-sm">
                {metrics.firstContentfulPaint.toFixed(0)}ms</div>
              </div>
            </div>
            <div className="bg-slate-700/50 rounded-lg p-3" /><div className="flex items-center mb-1" /><Activity className="w-4 h-4 text-cyan-400 mr-2" />
                <span className="text-xs text-gray-300">LCP</span>
              </div>
              <div className="text-white font-semibold text-sm">
                {metrics.largestContentfulPaint.toFixed(0)}ms</div>
              </div>
            </div>
            <div className="bg-slate-700/50 rounded-lg p-3" /><div className="flex items-center mb-1" /><Wifi className="w-4 h-4 text-cyan-400 mr-2" />
                <span className="text-xs text-gray-300">Network</span>
              </div>
              <div className="text-white font-semibold text-sm">
                {metrics.connectionType}</div>
              </div>
            </div>
          </div>
          {/* Memory Usage */}
          {metrics.memoryUsage && (
            <div className="bg-slate-700/50 rounded-lg p-3 mb-4" /><div className="flex items-center justify-between mb-2" /><span className="text-xs text-gray-300">Memory Usage</span>
          ...
          {metrics.batteryLevel !== null && (
            <div className="bg-slate-700/50 rounded-lg p-3 mb-4" /><div className="flex items-center justify-between mb-2" /><span className="text-xs text-gray-300">Battery</span>
                <span className="text-white font-semibold text-sm">
                  {metrics.batteryLevel.toFixed(0)}%</span>
                </span>
              </div>
              <div className="w-full bg-slate-600 rounded-full h-2" /><div ;
          ...
          {/* Alerts */}
          {alerts.length > 0 && (
            <div className="space-y-2" /><div className="flex items-center text-yellow-400 text-xs font-semibold" /><AlertTriangle className="w-4 h-4 mr-1" />
                Alerts;
              </div>
