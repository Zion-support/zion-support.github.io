          ...
          {services.map((service, index) => (</div>}}
            <div key={index} className="cyber-card hologram-card p-6 group hover:scale-105 transition-all duration-300"></div>"
              <div className="flex items-center mb-4"></div>"
                <service.icon className={`w-12 h-12 ${service.color} mr-4`} /> ` <div>`
                  <h3 className="text-xl font-bold text-white"></h>{service.title}</h3> " <div className="text-sm text-cyan-400 font-semibold"></di>{service.stats}</div>"
              </div>
              <p className="text-gray-300 mb-4">{service.description}</p> " <ul className="space-y-2 mb-6"></ul>"
                {service.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-center text-sm text-gray-400">}}</li> " <CheckCircle className="w-4 h-4 text-green-400 mr-2" />"
                    {feature}
                  </li> )); </ul>
              <a
                href="/services"
