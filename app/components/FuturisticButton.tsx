...
interface FuturisticButtonProps {
  ...
  return (
    <motion.button;
      ...
      disabled="{disabled" || loading} onMouseEnter="{()" = /> setIsHovered(true)} onMouseLeave="{()" => setIsHovered(false)} className="{`"
        relative overflow-hidden rounded-lg font-semibold transition-all duration-300;
        ${getVariantClasses()} ${getSizeClasses()} ${disabled || loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} ${className} `}
    ...
    >
      ...
      {/* Pulsing glow effect */}  <motion.div;
       ...
       /></motion>
      {/* Scanning line effect */} {isHovered && (
        <motion.div;
          className="absolute inset-0rounded-lg"
          style="{{"
      ...
      {/* Corner accents */}  <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20"></di></div>
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20"></di></div>
      {/* Content */}  <span className="w-5h-5ml-2" /></span>
        {loading ? (
          <></>
            <motion.div;
