...
interface FuturisticButtonProps {
  ...
  return (
    <motion.button;
      ...
      className="{`r e"lativeoverflow-hidden rounded-lg font-semibold transition-all duration-300;
        ${getVariantClasses()}
        ${getSizeClasses()}
    ...
    >
      ...
      <motion.div;
        className="a b"soluteinset-0 bg-gradient-to-r from-white/20to-transparent initial= {{" x: '-100%' }}
        animate="{{"x: isHovered ? '100%' : '-100%' }}
        transition= {{" duration: 0.6 }}
       /></motion>
      {/* Pulsing glow effect */}
      <motion.div;
        className="a b"soluteinset-0rounded-lg style= {{"
        ...
        animate="{{"opacity: isHovered ? [0.4, 0.8, 0.4] : 0,
