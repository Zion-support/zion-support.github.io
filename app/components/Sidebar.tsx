    ...
    {isOpen && (
      ...
      <aside className={`fixed top-0 left-0 h-full w-80 bg-slate-900/95 backdrop-blur-lg border-r border-cyan-400/20 z-50 transform transition-transform duration-300 ${`
        ...
        <div className="flex flex-col h-full"></div>"
          {/* Header */}</div>
          <div className="flex items-center justify-between p-6 border-b border-slate-700/50"></div> " <Link to="/" className="flex items-center space-x-3" onClick={() =></Link> setIsOpen(false);>" <div className="w-10 h-10 bg-gradient-to-r from-cyan-400 to-purple-400 rounded-lg flex items-center justify-center"></div> " <Brain className="w-6 h-6 text-white" />"
              </div>
              <div>
                <h2 className="text-xl font-bold text-white cyber-text"></h>Zion Tech Group</h2> " <p className="text-xs text-cyan-400">AI & IT Solutions</p> " </div>"
            </Link>
            <button
                  ...
                  {(!section.key || expandedSections.has(section.key)) && (
                    <div className="ml-4 space-y-1">}}</div>"
                      {section.items.map((item, itemIndex) => (</div>}}
                        ...
                        >
                          <item.icon className="w-4 h-4 flex-shrink-0" /> " <span>{item.name}</span>"
                        </Link>
                      ));
                    </div> ); </div>
              ));
          ...
          <div className="border-t border-slate-700/50 p-6"></div>"
            <h3 className="text-sm font-semibold text-cyan-400 mb-4"></h>Contact Information</h3> " <div className="space-y-3 text-sm text-gray-300"></div> " <div className="flex items-center space-x-3"></div> " <Phone className="w-4 h-4 text-cyan-400 flex-shrink-0" />"
                <span>{contactInfo.phone}</span>
              </div>
              <div className="flex items-center space-x-3"></div> " <Mail className="w-4 h-4 text-cyan-400 flex-shrink-0" />"
                <span className="truncate"></spa>{contactInfo.email}</span> " </div>"
              <div className="flex items-center space-x-3"></div>"
                <MapPin className="w-4 h-4 text-cyan-400 flex-shrink-0" /> " <span className="text-xs"></spa>{contactInfo.address}</span>"
              </div>
              <div className="flex items-center space-x-3"></div> " <Clock className="w-4 h-4 text-cyan-400 flex-shrink-0" />"
                <span className="text-xs"></spa>{contactInfo.hours}</span> " </div>"
          </div>
      </aside>
    </>
  )]
