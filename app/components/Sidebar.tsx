...
const Sidebar: React.FC = () =>{
  ...
  return (
    <React.Fragment>{/* Mobile Menu Button */}
      <button
...
&gt; <Menu className="w-6 h-6" />
      ...
      {/* Sidebar */}</div&gt; <aside className={`fixed top-0 left-0 h-full w-80 bg-slate-900/95 backdrop-blur-lg border-r border-cyan-400/20 z-50 transform transition-transform duration-300 ${
        ...
        <div className="flex flex-col h-full">{/* Header */}</div&gt; <div className="flex items-center justify-between p-6 border-b border-slate-700/50">
            ...
            <nav className="space-y-2 px-4">{navigationSections.map((section, sectionIndex) =>(
                <div key={sectionIndex} className="space-y-1">
                  <button
                    ...
                    <div className="ml-4 space-y-1">{section.items.map((item, itemIndex) =>(</div&gt; <Link
                          key={itemIndex}
                          to={item.path}
                          onClick={() =>setIsOpen(false)}
          ...
          <div className="border-t border-slate-700/50 p-6">
            <h3 className="text-sm font-semibold text-cyan-400 mb-4">Contact Information</h3&gt; <div className="space-y-3 text-sm text-gray-300">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-xs">{contactInfo.hours}</span&gt; </div&gt; </div&gt; </aside&gt; </React.Fragment>}
Sidebar.displayName = 'Sidebar'
export default Sidebar></div&gt; </string>