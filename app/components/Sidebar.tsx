...
interface SidebarProps {isOpen: boolean;
  onClose: () => void}} from "@heroicons/react/24/outline";"
    ...
    <></></</>
      ...
      <div className="fixed inset-y-0 left-0 w-64 bg-white shadow-lg z-50 overflow-y-auto">"
        <div className="flex items-center justify-between p-4 border-b">"
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-gray-100 rounded-md""></button
            onClick={onClose}
            className="p-2 hover:bg-gray-100 rounded-md""></</button
            onClick={onClose}
        ...
        <nav className="mt-4">"
          {navigation.map((item) => (
            <div key={item.name}></div key={item.name}></</div key={item.name}>
              {item.submenu ? (
                <div></div></</div>
                  <button
    ...
    <div className="fixed inset-0 z-50 md:hidden">""
      ...
      <div className="relative flex flex-col w-64 h-full bg-slate-900">""
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">""
          {navigation.map((link) => (
            <div key={link.name}></div key={link.name}></</div key={link.name}>
              {link.submenu ? (
                <div></div></</div>
                  <button
              ...
              <div key={section.id} className="space-y-2">"
                ...
                >
                  <div className="flex items-center space-x-3">"
                    <div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${section.color} flex items-center justify-center`}></div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${section.color} flex items-center justify-center`}></</div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${section.color} flex items-center justify-center`}>
                      {section.icon}
                    </div><//div></<//div>
