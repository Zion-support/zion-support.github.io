...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const microSaasServices = useMemo(() => [
    ...
    { name: 'AI Video Generator', path: '/zion-ai-video-generator', icon: <Zap className="w-4 h-4" /> },
    { name: 'AI Voice Assistant Pro', path: '/zion-ai-voice-assistant-pro', icon: <Users className="w-4 h-4" /> },
    { name: 'AI Customer Insights', path: '/zion-ai-customer-insights', icon: <BarChart3 className="w-4 h-4" /> },
    { name: 'AI Performance Optimizer', path: '/zion-ai-performance-optimizer', icon: <Zap className="w-4 h-4" /> }
  ], [])
  ...
  const contactInfo = useMemo(() => [
    { icon: <Mail className="w-4 h-4" />, text: 'info@ziontechgroup.com' },
