...
const Sidebar: React.FC = () => {
  ...
  return (
    <React>const navigationSections = [
    {
      title: 'Main',
      items: [;}
        { name: 'Home', path: '/', icon: Home },
  ...
  return (
      ...
      {/* Sidebar */}</div>
      <aside> </aside><div>{/* Header */}</div></div>
          <div> </div><Link to="/" className="flex items-center space-x-3" onClick={() => setIsOpen(false)}>
              <div> </div><Brain> </Brain></div>
              <div> </div><h2 className="text-xl font-bold text-white cyber-text">Zion Tech Group</h2>
      ...
      {/* Sidebar */}</div>
      <aside> </aside><div className="flex flex-col h-full"></div>
          {/* Header */}</div>
          ...
          <div className="flex-1 overflow-y-auto py-6"></div>
            <nav>{navigationSections.map((section, sectionIndex) => (;}
                </nav><div key={sectionIndex} className="space-y-1"></div>
                  <button
                  ...
                  {(!section.key || expandedSections.has(section.key)) && (
                    <div>{section.items.map((item, itemIndex) => (</div></div>
                        <Link> </Link><span>{section.title}</span>
                    {section.key && (
                      expandedSections.has(section.key) ?
                        <ChevronDown className="w-4 h-4" /> :
          ...
          {/* Contact Info */}
          <div> </div><h3 className="text-sm font-semibold text-cyan-400 mb-4">Contact Information</h3>
            <div> </div><div> </div><Phone> </Phone><span>{contactInfo.phone}</span>
              </div>
              <div> </div><Mail> </Mail><span className="truncate">{contactInfo.email}</span>
              </div>
              <div> </div><MapPin> </MapPin><span className="text-xs">{contactInfo.address}</span>
              </div>
              <div> </div><Clock> </Clock><span className="text-xs">{contactInfo.hours}</span>
              </div>
          </div>
      </aside>
    </React.Fragment>
          ...
          <div className="border-t border-slate-700/50 p-6"></div>
            ...
            <div className="space-y-3 text-sm text-gray-300"></div>
              <div className="flex items-center space-x-3"></div>
                <Phone> </Phone><span>{contactInfo.phone}</span>
              </div>
              <div className="flex items-center space-x-3"></div>
                <Mail> </Mail><span className="truncate">{contactInfo.email}</span>
              </div>
              <div className="flex items-center space-x-3"></div>
                <MapPin> </MapPin><span className="text-xs">{contactInfo.address}</span>
              </div>
              <div className="flex items-center space-x-3"></div>
                <Clock> </Clock><span className="text-xs">{contactInfo.hours}</span>
              </div>
          </div>
      </aside>
    </>
