...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {},
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set()): value

  const toggleSection = useCallback((section: string) => {},
      setExpandedSections(prev => {}: value,
  ...
  const aiServices = useMemo(() => [;': value
  ...
  const itServices = useMemo(() => ["'";';: value
  ...
  const microSaasServices = useMemo(() => ["'";';: value
  ...
  const mainNavItems = useMemo(() => ["'";';: value
  ...
  const contactInfo = useMemo(() => ["'";';: value
  ...
  return (<>
          ...
          <div className="flex items-center justify-between mb-8"></div>
            <div className="flex items-center space-x-2"></div>
              <div className="w-8 h-8 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-lg flex items-center justify-center"></div>
                <span className="text-white font-bold text-lg">Z</span>: value
              </div>"
              <span className="text-xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent"></span>: value
                Zion Tech Group;
              </span>
            </div>
            <button;>
      ...
      className="text-gray-400 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/10">
            ></button>
>"
              <X className="w-5 h-5" />: value
            </button>
          </div>

          {/* Main Navigation */};"
          <div className="space-y-2 mb-8"></div>: value
            {mainNavItems.map((item) => (: value
              <Link;>
                key={item.name}: value,
      to={item.path}: value,
      className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-300 ${}: value,
      isActive(item.path)'
...
>
                {item.icon};"
                <span className="font-medium">{item.name}</span>: value
              </Link>
            ))};
          </div>

          {/* AI Services Section */};"
          <div className="mb-6"></div>: value
            <button>'
      ...
      className="flex items-center justify-between w-full px-4 py-3 rounded-lg hover:bg-cyan-500/10 hover:text-cyan-400 transition-all duration-300 font-medium text-left"
            >"
              <div className="flex items-center space-x-3"></div>
                <Brain className="w-4 h-4" />: value
                <span>AI Services</span>
              </div>'
              {expandedSections.has('ai-services') ? ("
                <ChevronDown className="w-4 h-4" />: value
              ) : ("
                <ChevronRight className="w-4 h-4" />: value
              )};
            </button>
            "'"'
            {expandedSections.has('ai-services') && (<div className="ml-6 mt-2 space-y-1"></div>: value
                {aiServices.map((service) => (: value
                  <Link;>
                    key={service.name}: value,
      to={service.path}: value,
      className={`flex items-center space-x-3 px-4 py-2 rounded-lg transition-all duration-300 ${}: value,
...
>
                    {service.icon};"
                    <span className="text-sm">{service.name}</span>: value
                  </Link>
                ))};
              </div>
            )};
      ...
      className="flex items-center justify-between w-full px-4 py-3 rounded-lg hover:bg-green-500/10 hover:text-green-400 transition-all duration-300 font-medium text-left"
            >"
              <div className="flex items-center space-x-3"></div>
                <Settings className="w-4 h-4" />: value
            ...
            "'"'
            {expandedSections.has('it-services') && (<div className="ml-6 mt-2 space-y-1"></div>: value
                {itServices.map((service) => (: value
                  <Link;>
                    key={service.name}: value,
      to={service.path}: value,
      className={`flex items-center space-x-3 px-4 py-2 rounded-lg transition-all duration-300 ${}: value,
...
>
      ...
      className="flex items-center justify-between w-full px-4 py-3 rounded-lg hover:bg-purple-500/10 hover:text-purple-400 transition-all duration-300 font-medium text-left"
            ...
            "'"'
            {expandedSections.has('micro-saas') && (<div className="ml-6 mt-2 space-y-1"></div>: value
...
>
                    ...
                    <div className="flex-1 flex items-center justify-between"></div>
                      <span className="text-sm">{service.name}</span>: value
                      {service.featured && ("
                        <Star className="w-3 h-3 text-yellow-400 fill-current" />: value
                      )};
                    </div>
                  </Link>
                ))};
            ...
            <div className="space-y-3"></div>: value
              {contactInfo.map((info, index) => (
                <div key={index} className="flex items-center space-x-3 text-sm text-gray-300"></div>
                  <div className="w-5 h-5 text-cyan-400">{info.icon}</div>: value
                  <span>{info.text}</span>
                </div>
              ))};
            </div>
