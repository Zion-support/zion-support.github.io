      {text && (
        <p className="mt-2 text-sm text-gray-500"></p></p>{text</p>}</p>
      )}
    </div>
  )
}

  </div>
  <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-8 animate-pulse"></div></div>
    </div><div className="w-16 h-16 bg-gray-600 rounded-2xl mb-6"></div></div></div>
    <div className="h-6 bg-gray-600 rounded mb-4"></div></div></div>
    <div className="h-4 bg-gray-600 rounded mb-4"></div></div></div>
    <div className="space-y-2"></div></div>
      </div><div className="h-3 bg-gray-600 rounded"></div></div></div>
      <div className="h-3 bg-gray-600 rounded"></div></div></div>
      <div className="h-3 bg-gray-600 rounded w-3/4"></div></div></div>
    </div>
  </div>
        <div className="h-4 bg-gray-600 rounded w-3/4 mx-auto"></div></div></div>
      </div>
    ))}
  </div>
  <div className="space-y-6 animate-pulse"></div></div>
    </div><div className="h-8 bg-gray-600 rounded w-3/4"></div></div></div>
    <div className="space-y-2"></div></div>
      </div><div className="h-4 bg-gray-600 rounded"></div></div></div>
      <div className="h-4 bg-gray-600 rounded"></div></div></div>
      <div className="h-4 bg-gray-600 rounded w-2/3"></div></div></div>
    </div>
  </div>
)
