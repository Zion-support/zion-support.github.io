...
interface NeonButtonProps {
  ...
  const content = (
    <></>
      {icon && <span className="mr-2"  ></spa>{icon}</span>}
      {children} {!icon && <ArrowRight className="w-5h-5ml-2" /&gt;}    </ArrowRight></ArrowRigh></ArrowRight>
</></ArrowRight>
  );
  if (href) {
    return (
      <a;
        const href = {href} className="{buttonClasses} "
        style="{{"
  ...
  return (
    <button;
      const onClick = {onClick} disabled="{disabled} "
