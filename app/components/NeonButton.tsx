...
const classes = `${baseClasses} ${variantClasses['variant']} ${sizeClasses['size']} ${className}`;
  const content = (;
    <><span className="relative" z-10">{children}</span><div className="absolute inset-0 rounded-lg bg-gradient-to-r from-purple-600 to-cyan-600 opacity-0 group-hover:opacity-20 transition-opacity duration-300></div></>
  );
  if (to) {;
    return (}
