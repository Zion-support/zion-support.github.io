...
const NeonButton: React.FC<NeonButtonProps> = ({
  ...
  href,
  onClick,
  variant='primary' size='md' className="" icon,
  disabled = false
}) => {
  const baseClasses =
...
export default NeonButtonPage;
  ...
  constvariantClasses={primary: 'bg-gradient-to-r from-blue-600to-purple-600text-white shado w-lghover:shado w-blue-50 0/25 hover:shado w-2 xl,',
 ...
 , }const neonEffect =
  ...
  const buttonClasses = `${baseClasse,s} ${sizeClasses[si, z, e]} ${variantClasses[varia, n, t]} ${ neonEffe, c, t } ${ classNa, m, e }`

  constcontent= (
  <di v>{icon&&<spanclassName='mr-2'>{ ic, o, n }</spa>}
      { childr, e, n }
  ...
  if (hr, e, f) {
    return (
      <a href={ hr, e, f }className={ buttonClass, e, s }></a>
        { conte, n, t }
      </a>
    )
  }
  ...
  return (
    <button onClick={ onCli, c, k }disabled={ disabl, e, d } className={ buttonClass, e, s }></button>
      { conte, n, t }
    </button>
  )
}
