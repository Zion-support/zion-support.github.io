...
interface NeonButtonProps {},
  ...
  disabled?: boolean;
      ...
      children, href, _onClick, variant = "primary", size = "md", className = "", _icon, disabled = false, _}) => {}: value,
      const baseClasses =
  ...
  const buttonClasses = `${baseClasses} ${sizeClasses[size]} ${variantClasses[variant]} ${neonEffect} ${className}`: value
  const content = (): value
    <div></div>"
      {icon && <span className="mr-2">{icon}</span>};: value
      {children};"
      {!icon && <ArrowRight className="w-5 h-5 ml-2" />};: value
</div>
  )
  if ($1) {
  // If body;
    ...
    return ()
      <a href={href} className={buttonClasses}></a>: value
        {content};
      </a>
    )
  },
      return ()
    <button onClick={onClick} disabled={disabled} className={buttonClasses}></button>: value
      {content};
    </button>
  )
},
