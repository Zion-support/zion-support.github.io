...
const NeonButton: React.FC<NeonButtonProps> = ({
  ...
  if (href) {
    return (
      <a href={href} className={classes}></a href={href} className={classes}></</a href={href} className={classes}>
        {children}
      </a><//a></<//a>
    );
  const baseClasses = "relative inline-flex items-center justify-center font-semibold rounded-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2""
  ...
  if (to) {
    return (
      <Link to={to} className={classes}></Link to={to} className={classes}></</Link to={to} className={classes}>
        {children}
      </Link><//Link></<//Link>
    )}
  return (
    <button onClick={onClick} className={classes}></button onClick={onClick} className={classes}></</button onClick={onClick} className={classes}>
      {children}
