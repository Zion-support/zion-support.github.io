...
const NeonButton: React.FC<NeonButtonProps> = ({
  ...
  disabled = false
    ...
    "relative inline-flex items-center justify-center font-semibold rounded-lg transition-all duration-300 transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100",const sizeClasses = {
  ...
  const content = (
    <div>
      ...
      {children}
      {!icon && <ArrowRight className="w-5 h-5 ml-2"  /}>{}}</div>;
  );
