...
import { Link , ArrowRight, Brain, Zap, Globe  } from "react-router-dom";
const EnhancedNavigation = () => {
  ...
  const navigationItems = [
    {
      label: 'AI Services',
      href: '/ai-services',
      icon: </string><Brain className="w-4 h-4" />,
      dropdown: [
  ...
  return (
    <header className="bg-black/20 backdrop-blur-sm border-b border-white/10 relative z-50" /><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" /><div className="flex justify-between items-center py-4">
          {/* Logo */}
          </div><Link to="/" className="text-2xl font-bold text-white group" /><span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 group-hover:from-cyan-300 group-hover:to-purple-300 transition-all duration-300">
            ...
            {navigationItems.map((item) => (
              ...
              >
                ...
                {activeDropdown === item.label && (
                  <div className="absolute top-full left-0 mt-2 w-64 bg-slate-800/95 backdrop-blur-sm border border-white/20 rounded-lg shadow-xl z-50" /><div className="py-2">
          ...
          {/* Quick Links & CTA */}
          <div className="hidden md:flex items-center space-x-4" /><div className="hidden lg:flex space-x-4">
              {quickLinks.map((link) => (
                </div><Link;
                  key={link.href}
                  to={link.href}
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className="lg:hidden py-4 border-t border-white/10" /><div className="space-y-4">
              {navigationItems.map((item) => (
                </div><div key={item.label} /><Link;
                    to={item.href}
                    className="flex items-center space-x-2 text-gray-300 hover:text-cyan-400 transition-colors duration-300 py-2"
                    onClick={() => setIsOpen(false)}
                  >
                    {item.icon}
                    </Link><span className="font-medium">{item.label}</span>
                  </Link>
              ...
              <div className="pt-4 border-t border-white/10" /><div className="space-y-2">
                  {quickLinks.map((link) => (
                    </div><Link;
                      key={link.href}
                      to={link.href}
