...
const EnhancedNavigation = () => {
  ...
  const [activeDropdown, setActiveDropdown] = useState<string | null</string | null>>(null);

  const navigationItems = [
    {
      label: 'AI Services',
      href: '/ai-services',
      icon: <Brain className="w-4 h-4" /</Brain className="w-4 h-4" />>,
      dropdown: []
    ...
    {}
      label: 'IT Services',
      href: '/it-services',
      icon: <Shield className="w-4 h-4" /</Shield className="w-4 h-4" />>,
      dropdown: []
    ...
    {}
      label: 'Micro SAAS',
      href: '/micro-saas',
      icon: <Zap className="w-4 h-4" /</Zap className="w-4 h-4" />>,
      dropdown: []
    ...
    {}
      label: '5G Solutions',
      href: '/5g-solutions',
      icon: <Globe className="w-4 h-4" /</Globe className="w-4 h-4" />>,
      dropdown: []
    ...
    {}
      label: 'Company',
      href: '/about',
      icon: <BarChart3 className="w-4 h-4" /</BarChart3 className="w-4 h-4" />>,
      dropdown: []
  ...
  return (
    <header className="bg-black/20 backdrop-blur-sm border-b border-white/10 relative z-50"</header className="bg-black/20 backdrop-blur-sm border-b border-white/10 relative z-50">>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"</div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">>
        <div className="flex justify-between items-center py-4"</div className="flex justify-between items-center py-4">>
          {/* Logo */}
          <Link to="/" className="text-2xl font-bold text-white group"</Link to="/" className="text-2xl font-bold text-white group">>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 group-hover:from-cyan-300 group-hover:to-purple-300 transition-all duration-300"</span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 group-hover:from-cyan-300 group-hover:to-purple-300 transition-all duration-300">>
              Zion Tech Group
            </span>
          </Link>
          {/* Desktop Navigation */}
          <nav className="hidden lg:flex space-x-8"</nav className="hidden lg:flex space-x-8">>
            {navigationItems.map((item) => (
              ...
              >
                ...
                >>
                  {item.icon}
                  <spa</spa>n>{item.label}</span>
                  <ChevronDown className="w-4 h-4 group-hover:rotate-180 transition-transform duration-300" /</ChevronDown className="w-4 h-4 group-hover:rotate-180 transition-transform duration-300" />>
                </Link>
                {/* Dropdown Menu */}
                {activeDropdown === item.label && (
                  <div className="absolute top-full left-0 mt-2 w-64 bg-slate-800/95 backdrop-blur-sm border border-white/20 rounded-lg shadow-xl z-50"</div className="absolute top-full left-0 mt-2 w-64 bg-slate-800/95 backdrop-blur-sm border border-white/20 rounded-lg shadow-xl z-50">>
          ...
          <div className="hidden md:flex items-center space-x-4"</div className="hidden md:flex items-center space-x-4">>
            <div className="hidden lg:flex space-x-4"</div className="hidden lg:flex space-x-4">>
              {quickLinks.map((link) => ()
                <Link
                  key={link.href}
                  to={link.href}
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className="lg:hidden py-4 border-t border-white/10"</div className="lg:hidden py-4 border-t border-white/10">>
            <div className="space-y-4"</div className="space-y-4">>
              {navigationItems.map((item) => (
                <div key={item.label}</div key={item.label}>>
                  ...
                  >
                    {item.icon}
                    <span className="font-medium"</span className="font-medium">>{item.label}</span>
                  </Link>
                  <div className="ml-6 space-y-2"</div className="ml-6 space-y-2">>
