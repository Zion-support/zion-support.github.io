...
interface HeroStats {clients: number;,
    savings: string;,
    uptime: string;,
    support: string;
}
...
const EnhancedHero: React.FC = () => {,
  ...
  return (
    <motion.section;
      className="relative min-h-screen flex items-center justify-center overflow-hidden" initial="hidden"
    ...
    >{/* Animated Background */}</motion>
      <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900" >
        ...
        {/* Floating Elements */}
        <motion.div;
          className="absolute top-20 left-20 w-32 h-32 bg-cyan-500/20 rounded-full blur-xl" animate={{}
            y: [0, -20, 0],
            x: [0, 10, 0],}
            scale: [1, 1.1, 1]}
          }}
    ...
    repeat: Infinity;}
      ...
      <div className="relative z-10 container mx-auto px-4 py-16 pt-24" >
        <motion.div;
          className="text-center max-w-6xl mx-auto" variants={itemVariants}
        >{/* Main Heading */}<motion.h1
            className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 neon-text" variants={itemVariants}
...
,
          ...
          {/* Trust Indicators */}
          <motion.div;
            className="mt-16 flex flex-wrap justify-center items-center gap-8 opacity-60" variants={itemVariants}
          >
      ...
      {/* Scroll Indicator */}
      <motion.div;
        className="absolute bottom-8 left-1/2 transform -translate-x-1/2" animate={{ y: [0, 10, 0] }}
        transition={{ duration: 2, repeat: Infinity }}
      >
        <div className="w-6 h-10 border-2 border-white/30 rounded-full flex justify-center" >
          <motion.div;
            className="w-1 h-3 bg-white/60 rounded-full mt-2" animate={{ y: [0, 12, 0] }}
            transition={{ duration: 2, repeat: Infinity }}
          /></motio>
        </div>
      </motion.div>
