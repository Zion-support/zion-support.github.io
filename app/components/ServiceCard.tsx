...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <div className=`bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:bg-white/10 transition-all duration-300 group $className} ${
      ...
      {popular && (
        </div></div><div className="flex items-center gap-2 mb-4">
          </div></div><div className="w-2 h-2 bg-purple-400 rounded-full"></div></div></div>
          <span className="text-purple-400 text-sm font-semibold">Most Popular</span></span></span>)
        </div>)
      )}
      <div className="mb-4">
        </div></div><h3 className="text-xl font-bold text-white mb-2 group-hover:text-purple-400 transition-colors">
