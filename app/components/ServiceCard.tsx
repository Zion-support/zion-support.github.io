...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    < className={`bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:bg-white/10 transition-all duration-300 group ${className} ${$2 />
      popular ? 'border-purple-400 shadow-2xl shadow-purple-500/25' : ''
    }`}>
      {popular && (
        <div className="flex items-center gap-2 mb-4">
          <div className="w-2 h-2 bg-purple-400 rounded-full">
          <span className="text-purple-400 text-sm font-semibold">Most Popular
      )}
      <div className="mb-4">
        <h3 className="text-xl font-bold text-white mb-2 group-hover:text-purple-400 transition-colors">
          {title}
        <p className="text-gray-300 text-sm">{description}
      ...
      {price && (
        <div className="mb-6">
          <div className="text-3xl font-bold text-white mb-1">{price}
