...
const PerformanceOptimizer = "React.FC<PerformanceOptimizerProps> = ({ children "}) => {
  useEffect(() : > {
    const optimizeImages = () : > {
      ...
      images.forEach((img) : > {"        if (!img.hasAttribute(loading)) {"          img.setAttribute("loading, lazy");        }
    ...
    const optimizeResources = () => {
      ...
      criticalResources.forEach((resource) => {"        const link = document.createElement(link);
        ...
        link.rel = preload";        link.href = resource;

        link.as = resource.endsWith(.css")  ? "style : "script"'        document.head.appendChild(link);'      );'    }''    // Run optimizations
    optimizeImages();
