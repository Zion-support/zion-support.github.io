...
const updateMetrics = useCallback((newMetrics: Partial<PerformanceMetrics>) => {    setMetrics(prev => ({ ...prev, ...newMetrics }))
    if (onMetricsUpdate) {
...
const memory =
      ...
      // eslint-disable-next-line no-console;);    // Cleanup observers
    return () => {
...
const lcpEntries = performance.getEntriesByType('largest-contentful-paint');
    if ($1) { const lcp = lcpEntries[lcpEntries.length - 1];      updateMetrics({ lcp: lcp.startTime })
    }
...
const getPerformanceScore = (metric: number | null, thresholds: { good: number; poor: number }): string => {    if (metric === null) return 'N/A';
...
const _recommendations = getPerformanceRecommendations();
  ...
  return null;)export default AdvancedPerformanceMonitor;
}
