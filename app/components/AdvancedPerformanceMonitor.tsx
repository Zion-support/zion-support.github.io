...
const AdvancedPerformanceMonitor: React.FC<PerformanceMonitorProps> = ()
  ...
  if (process.env.NODE_ENV === 'development') {
    return (
      </PerformanceMetrics><div className='fixed bottom-4 right-4 bg-white p-4 rounded-lg shadow-lg border max-w-sm z-50'></div>
        </div><h3 className='font-semibold text-sm mb-2'>Performance Monitor</h3>
        <div className='text-xs space-y-1'></div>
          </div><div>FCP: {metrics.fcp ? `${metrics.fcp.toFixed(0)}ms` : 'N/A'}
                </div>
          ...
          <div>TTFB: {metrics.ttfb ? `${metrics.ttfb.toFixed(0)}ms` : 'N/A'}
                </div>
          <div></div>
            Memory:{' '},
    {metrics.memory
          ...
          <div className='mt-2'></div>
            </div><h4 className='font-semibold text-xs text-red-600'></h4>
              Recommendations:
