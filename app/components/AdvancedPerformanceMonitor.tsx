...
const AdvancedPerformanceMonitor: React.FC = () => {
  ...
  const calculatePerformanceScore = useCallback((currentMetrics: AdvancedMetrics): number => {
    let score = 100;
    ...
    // Memory scoring
    if (currentMetrics.memory && currentMetrics.memory.percentage > thresholds.memory) {
      score -= Math.min(20, (currentMetrics.memory.percentage / thresholds.memory) * 10);
    }
    
    return Math.max(0, Math.round(score));
  }, [thresholds]);

  const generateInsights = useCallback((currentMetrics: AdvancedMetrics): PerformanceInsights => {
    ...
    // Memory analysis
    if (currentMetrics.memory && currentMetrics.memory.percentage > thresholds.memory * 2) {
        ...
        <div className="space-y-2">
          ...
          <div className="border-b border-gray-600 pb-2">
            <div className="flex justify-between items-center">
              ...
              <span className={`font-bold ${getScoreColor(insights?.score || 0)}`}>
          ...
          <div className="space-y-1">
            <div className="flex justify-between">
              <span>CLS:</span>
              <span className={getStatusColor(metrics?.cls || 0, thresholds.cls)}>
                {getStatusIcon(metrics?.cls || 0, thresholds.cls)} {metrics?.cls?.toFixed(3) || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between">
              <span>INP:</span>
              <span className={getStatusColor(metrics?.inp || 0, thresholds.inp)}>
                {getStatusIcon(metrics?.inp || 0, thresholds.inp)} {metrics?.inp?.toFixed(1) || 'N/A'}ms
              </span>
            </div>
            <div className="flex justify-between">
              <span>FCP:</span>
              <span className={getStatusColor(metrics?.fcp || 0, thresholds.fcp)}>
                {getStatusIcon(metrics?.fcp || 0, thresholds.fcp)} {metrics?.fcp?.toFixed(1) || 'N/A'}ms
              </span>
            </div>
            <div className="flex justify-between">
              <span>LCP:</span>
              <span className={getStatusColor(metrics?.lcp || 0, thresholds.lcp)}>
                {getStatusIcon(metrics?.lcp || 0, thresholds.lcp)} {metrics?.lcp?.toFixed(1) || 'N/A'}ms
              </span>
            </div>
            <div className="flex justify-between">
              <span>TTFB:</span>
              <span className={getStatusColor(metrics?.ttfb || 0, thresholds.ttfb)}>
                {getStatusIcon(metrics?.ttfb || 0, thresholds.ttfb)} {metrics?.ttfb?.toFixed(1) || 'N/A'}ms
              </span>
            </div>
          </div>
          ...
          <div className="border-t border-gray-600 pt-2 space-y-1">
            ...
            <div className="flex justify-between">
              <span>Memory:</span>
              <span className={realTimeMetrics.memoryPercentage > thresholds.memory ? 'text-red-400' : 'text-green-400'}>
                {realTimeMetrics.memoryUsed || 0}MB ({realTimeMetrics.memoryPercentage || 0}%)
              </span>
            </div>
