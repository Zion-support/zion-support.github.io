...
const AdvancedPerformanceMonitor: React.FC = () => {"
  const [metrics, setMetrics] = useState<PerformanceMetrics>({
    ...
    cumulativeLayoutShift: 0,firstInputDelay: 0,totalBlockingTime: "0;"
  });
  ...
  useEffect() => {
    const handleKeyPress = (event: "KeyboardEvent) => {"
      if (event.ctrlKey && event.shiftKey && event.key === "P") {";
        setIsVisible(!isVisible)}if (event.ctrlKey && event.shiftKey && event.key === "R") {"
  ...
  if (process.env.NODE_ENV === "production") {"
    ...
    return null}const getScoreColor = (value: number, thresholds: { good: number; needsImprovement: number }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-90 text-white py-20">
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold mb-8">Advanced Performance Monitor</h$1>
