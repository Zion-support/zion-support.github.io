...
const memory = (performance as any).memory;
      updateMetrics({ memory: memory.usedJSHeapSize / 1024 / 1024 })
// Convert to MB
    }
  }, [updateMetrics]);
  useEffect(() => {
...
const _recommendations = getPerformanceRecommendations();
  if (process.env['NODE_ENV'] === 'development') {'
    return (
      <div>
        <h3 className='font-semibold text-sm mb-2'></h>Performance Monitor</h3> ' <div>'
          <div></di>FCP: {metrics.fcp ? `${metrics.fcp.toFixed(0); ms` : 'N/A'}</div> ' <div></di>LCP: {metrics.lcp ? `${metrics.lcp.toFixed(0); ms` : 'N/A'}</div>'
          <div></di>FID: {metrics.fid ? `${metrics.fid.toFixed(0); ms` : 'N/A'}</div> ' <div></di>CLS: {metrics.cls ? metrics.cls.toFixed(3) : 'N/A'}</div>'
          <div>
...
const clsScore = getPerformanceScore(metrics.cls, { good: 0.1, poor: 0.25 });
  return (
    <div className="performance-monitor p-4 bg-white/10 backdrop-blur-sm rounded-lg border border-white/20">"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> " <div className="metric-item">"
          <div className="flex justify-between items-center mb-1"> " <span className="text-sm text-gray-300">First Contentful Paint</span> " <span className={`text-xs px-2 py-1 rounded ${`
        ...
        <div className="metric-item">"
          <div className="flex justify-between items-center mb-1"> " <span className="text-sm text-gray-300">Largest Contentful Paint</span> " <span className={`text-xs px-2 py-1 rounded ${`
        ...
        <div className="metric-item">"
          <div className="flex justify-between items-center mb-1"> " <span className="text-sm text-gray-300">First Input Delay</span> " <span className={`text-xs px-2 py-1 rounded ${`
        ...
        <div className="metric-item">"
          <div className="flex justify-between items-center mb-1"> " <span className="text-sm text-gray-300">Cumulative Layout Shift</span> " <span className={`text-xs px-2 py-1 rounded ${`
        ...
        {metrics.memory && (
          <div className="metric-item md:col-span-2">}}"
            <div className="flex justify-between items-center mb-1"> " <span className="text-sm text-gray-300">Memory Usage</span> " <span className="text-xs text-gray-400">MB</span> " </div>"
            <div className="text-lg font-mono text-white">"
...
export default AdvancedPerformanceMonitor;
      ...
      <div className="mt-4 pt-4 border-t border-white/10"> " <div className="flex items-center justify-between">"
          <span className="text-sm text-gray-300"> Monitoring Status </span>"
          <div className="flex items-center gap-2"> " <div className={`w-2 h-2 rounded-full ${isMonitoring ? 'bg-green-400' : 'bg-gray-400'}`}></div> ` <span className="text-sm text-gray-300">"`
