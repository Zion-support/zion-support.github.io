...
existingStyle.remove();
  ...
  return (
    <><button;
      ...
      >♿ A11y;</button><div className="{`accessibility-panel" ${isVisible ? visible' : }`}>;<h3 className="text-lg" font-semibold mb-4">Accessibility Options</h3><div className="space-y-4>;<div><label className="flex" items-center space-x-2>;<input;
                type='checkbox;'
                checked={settings.highContrast}
                onChange={toggleHighContrast}
                className="rounded"";
              />;<span>High Contrast</span></label></div><div><label className="block text-sm font-medium mb-2>Font Size</label><div className="space-y-2">;
              {(['small, normal, large', extra-large] as const).map((size) =>(;<label key={size} className="flex" items-center space-x-2>;<input;
                    type=radio";
                    name="fontSize;
                    value={size}
