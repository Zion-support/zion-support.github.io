...
interface AccessibilitySettings {
  ...
  useEffect(() => {
    ...
    const handleMotionChange = (_e: MediaQueryListEvent) => {
      setSettings(prev => ({ ...prev, reducedMotion: _e.matches })
      applyAccessibilitySettings({ ...settings, reducedMotion: _e.matches })},
    const handleContrastChange = (_e: MediaQueryListEvent) => {
      setSettings(prev => ({ ...prev, highContrast: _e.matches })
      applyAccessibilitySettings({ ...settings, highContrast: _e.matches })},
  ...
  const applyAccessibilitySettings = (newSettings: AccessibilitySettings) => {
    ...
    if (skipLink) {
      skipLink.addEventListener('click', (_e) => {
        _e.preventDefault(
        const main = document.querySelector('main'
        if (main) {
          main.focus(
    ...
    document.addEventListener('focusin', (_e) => {
      ...
      label: (_e.target as HTMLElement).tagName;
      })});
  }, []);
