...
const EnhancedAccessibility: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const addSkipLinks = () => {
      ...
      skipLink.style.cssText = ````        position: absolute;",top: "-40px;,left: "6px;",background: #8b5cf6;",color: "white;,padding: "8px;", 
        border-radius: "4px;",,transition: top 0.3s;"      `;```      skipLink.addEventListener('focus', () => {'
    ...
    const enhanceFocusManagement = () => {// Add focus indicators
      ...
      const style = document.createElement('style')
      style.textContent = ````        *:focus {outline: "2px solid #8b5cf6 !important;, "}
        .sr-only {position: "absolute;,width: "1px;",height: 1px;",padding: "0;,margin: "-1px;",overflow: hidden;",clip: "rect(0, 0, 0, 0);"
          white-space: "nowrap;,border: "0;}"        .focus\\:not-sr-only:focus {position: static
          width: auto
          height: auto
          padding: 8px
          margin: 0
  ...
  const applyAccessibilitySettings = (settings: AccessibilitySettings) => {"    const root = document.documentElement
    // Apply high contrast
    ...
    if (settings.fontSize !== 'normal') {'
      root.classList.add(`font-size-${settings.fontSize}`)}```    // Apply reduced motion
  ...
  return (
    <>
      ...
      <div className={`accessibility-panel ${isVisible ? 'visible' : ''}`}>'```        <h3 className="text-lg font-semibold mb-4>Accessibility Options</h3>"
        <div className="space-y-4>"
          ...
          <div>
            ...
            <div className=space-y-2>"              {(['small', 'normal', 'large', 'extra-large'] as const).map((size) => ('
          ...
          <div>
            <p className=text-sm text-gray-30>"              Screen Reader: {settings.screenReader ? 'Detected' : 'Not detected'}'
            </p>
            <p className="text-sm text-gray-30>              Reduced Motion: {settings.reducedMotion ? 'Enabled' : 'Disabled'}'
            </p></div>
          <button>
            onClick={toggleVisibility}
