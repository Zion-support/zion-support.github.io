...
const PerformanceEnhancer: React.FC<PerformanceEnhancerProps>= ({enableWebVitals = true,
const PerformanceEnhancer: React.FC<PerformanceEnhancerProps> = ({,
  ...
  const reportError = useCallback((error: any, context?: string) => {if (!enableErrorReporting) return;
    ...
    // Send to analytics;
    if (enableAnalytics && typeof window !== 'undefined' && 'gtag' in window) {(window as any).gtag('event', 'performance_error', {)
    ...
    if (typeof window !== 'undefined' && navigator.sendBeacon) {// Send to analytics
  ...
  useEffect(() => {if (!enableWebVitals || typeof window === 'undefined') return;
    ...
    const reportMetric = (metric: any) => {}
      try {}
        ...
        // Send to analytics;
        if (enableAnalytics && typeof window !== 'undefined' && 'gtag' in window) {(window as any).gtag('event', 'web_vitals', {
            event_category: 'Performance',
    ...
    custom_parameter_2: metric.id;
        ...
        // Send to custom endpoint;
        if (typeof window !== 'undefined' && navigator.sendBeacon) {const data = JSON.stringify({
            name: metric.name,
            value: metric.value,
            delta: metric.delta,
            id: metric.id,
    ...
    if (enableLargestContentfulPaint) {getLCP(reportMetric);
    }
  ...
  useEffect(() => {if (!enableResourceTiming || typeof window === 'undefined') return;
  ...
  // Memory monitoring;
  useEffect(() => {if (!enableMemoryMonitoring || typeof window === 'undefined') return;

    const measureMemory = () => {}
      try {}
        if ('memory' in performance) {}
          const memory = (performance as any).memory;
          performanceMetrics.memoryUsage = {usedJSHeapSize: memory.usedJSHeapSize;,
    ...
    timestamp: Date.now(),
          ...
          if (memory.usedJSHeapSize > memory.jsHeapSizeLimit * 0.8) {reportError(new Error('High memory usage detected'), 'MemoryMonitoring');
          }
  ...
  // Network monitoring;
  useEffect(() => {if (!enableNetworkMonitoring || typeof window === 'undefined') return;

    const measureNetwork = () => {}
      try {}
        if ('connection' in navigator) {}
          const connection = (navigator as any).connection;
          performanceMetrics.networkInfo = {effectiveType: connection.effectiveType;,
  ...
  useEffect(() => {if (!enableLongTaskMonitoring || typeof window === 'undefined') return;
  ...
  useEffect(() => {if (!enableLayoutShiftMonitoring || typeof window === 'undefined') return;
  ...
  useEffect(() => {if (!enableUserTiming || typeof window === 'undefined') return;
  ...
  useEffect(() => {const reportPerformanceSummary = () => {
      try {
        const summary = {
    ...
    timestamp: Date.now(),
        ...
        if (enableAnalytics && typeof window !== 'undefined' && 'gtag' in window) {(window as any).gtag('event', 'performance_summary', {
            event_category: 'Performance',
            event_label: 'Summary',
            value: 1,)
