...
return null",}
return (<nav aria-label="Breadcrumb" className="bg-slate-800/50 backdrop-blur-sm border-b border-slate-700">,<div className="max-w-7 xl mx-auto px-4 py-3" /><ol className="flex items-center space-x-2 text-sm"><li><Link href="/" className="text-gray-400 hover: text-white transition-colors">
Home
            </Link></li>)
          {pathnames.map((name,index) => {"
const routeTo = `/${pathnames.slice(0, index + 1).join('/")}`"
            const isLast = index === pathnames.length - 1"
return (<li key={name,} className="flex items-center"><svg"
