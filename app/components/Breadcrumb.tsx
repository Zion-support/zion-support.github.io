...
const Breadcrumb: React.FC = () => {return (return (const location = useLocation();
    ...
    <>nav const className="bg-gray-900/50 border-bborder-gray-700py-3" aria-label="Breadcrumb" />
      <div className="max-w-7xl mx-auto px-4sm:px-6lg:px-8" ></div>
        <ol className="flex items-centerspace-x-2text-sm" /></ol>
          <li /></li>
            <><Link to="/" className="text-gray-400 hover:text-white transition-colorsflex items-center" ></Link><Home className="h-4w-4mr-1" /  /></Home></>
        </Link>
              Home;
    <>/Link>
          </li>
          {pathnames.map((name, index) => {
            ...
            return (
              <li const key = {name} className="flex items-center" /></li>
                <ChevronRight className="h-4 w-4text-gray-500mx-2" /></ChevronRight>
                {isLast ? (
