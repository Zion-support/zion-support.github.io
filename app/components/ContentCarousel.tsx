...
const ContentCarousel: React.FC = () => {const [currentSlide, setCurrentSlide] = useState(0);
  ...
  useEffect(() => {if (!isAutoPlaying) return;
  ...
  return (
    <div className="relative w-full h-96 lg:h-[500px] overflow-hidden rounded-lg cyber-card" >
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-slate-800/80 hover:bg-slate-700/80 rounded-full flex items-center justify-center text-white transition-all duration-300 hover:scale-110" aria-label="Previous slide"
      >
      ...
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-slate-800/80 hover:bg-slate-700/80 rounded-full flex items-center justify-center text-white transition-all duration-300 hover:scale-110" aria-label="Next slide"
      >
      ...
      <button
        ...
        className="absolute top-4 right-4 w-10 h-10 bg-slate-800/80 hover:bg-slate-700/80 rounded-full flex items-center justify-center text-white transition-all duration-300" aria-label={isAutoPlaying ? 'Pause slideshow' : 'Play slideshow'}
      >
        {isAutoPlaying ? (
      ...
      <div className="container mx-auto px-4" >
        ...
        <div className="relative max-w-6xl mx-auto" ></div>
          <div className="overflow-hidden rounded-2xl" ></div>
            <div
              className="flex transition-transform duration-500 ease-in-out" <h2 id="carousel-heading" className="text-3xl md: text-4xl font-bold text-white text-center mb-12 neon-text">
          Success Stories;
        </h2>)
)
        <div className="relative max-w-6xl mx-auto" >)
          ...
          <button;
            onClick={prevSlide}
            className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/20 hover: bg-white/30 text-white p-3 rounded-full transition-all duration-300 backdrop-blur-sm" aria-label="Previous slide"
          >,
            <ChevronLeft className="w-6 h-6" />,
          </button>,
,
          <button;
            onClick={nextSlide}
            className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/20 hover: bg-white/30 text-white p-3 rounded-full transition-all duration-300 backdrop-blur-sm" aria-label="Next slide",
          >,
            <ChevronRight className="w-6 h-6" />,
          </button>,
,
...
const ContentCarousel: React.FC = () => {const testimonials = [
    {
