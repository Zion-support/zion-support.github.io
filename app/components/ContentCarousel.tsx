...
;
interfaceContentCarouselProps {slides?: Slide[];
  autoPlay?: boolean;
  interval?: number;
}
;
constdefaultSlides: Slide[] = [{id: 1,;
...
;
  constnextSlide = () => {setCurrentSlide((prevSlide) => (prevSlide + 1) % slides.length);
  };
;
  constprevSlide = () => {setCurrentSlide((prevSlide) => (prevSlide - 1 + slides.length) % slides.length);
  };
;
  consttogglePlayPause = () => {setIsPlaying(!isPlaying);
  };
;
  constgoToSlide = (index: number) => {setCurrentSlide(index);
  };
;
  return (;
