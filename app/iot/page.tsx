...
const IoTPage: React.FC = () => {
  ...
  return (
    <></>IoT Services - Zion Tech Group</title></$1></div>
        {/* Hero Section */}</div></section></div></div></div></div>
                IoT Solutions</h1></$1>
                Connect, monitor, and optimize your world with intelligent Internet of Things solutions</$1></div>
                  Explore IoT Solutions</$1>
                  Schedule IoT Consultation</$1></$1></$1></$1></$1>
        {/* Key Benefits */}
        <section className="py-16 bg-white"></section></div></div>
                Why Choose Our IoT Services?</$1>
                End-to-end IoT solutions with proven expertise in connectivity, security, and analytics</$1></$1></div></div></div></$1>Connectivity Experts</h3>Multi-protocol IoT connectivity solutions</p></$1></div></div></$1>Security First</h3>Comprehensive IoT security and data protection</p></$1></div></div></$1>Edge Computing</h3>Low-latency processing at the edge</p></$1></div></div></$1>Custom Solutions</h3>Tailored IoT solutions for your specific needs</p></$1></$1></$1></$1>
        {/* Services by Category */}
        {categories.map(category => (;}
          <section key={category} className="py-16 bg-gray-50"></section></div></div>
                  {category} Solutions</$1>
                  Specialized {category.toLowerCase()} solutions powered by IoT technology</$1></$1></div>
                {iotServices
                  .filter(service => service.category === category)
                  .map((service, index) => (</div>;}
                    <div key={index} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow"></div>{service.icon}</div>{service.title}</h3>{service.description}</p></div></div>{service.price}</span>Market: {service.marketPrice}</span></$1></div>
                          Save up to 40% vs market rates</div></$1></$1></div>Key Features:</h4>
                          {service.features.map((feature, featureIndex) => (;}
                            <li key={featureIndex} className="flex items-center text-sm text-gray-600">
                              {feature}
                            </li>
                          ))}
                        </ul></$1></div>Technologies:</h4></div>
                        ...
                        </div></$1></div>Business Benefits:</h4>
        ...
        <section className="py-16 bg-white"></section></div></div>
                Our IoT Capabilities</$1>
                Comprehensive IoT technologies and methodologies for connected solutions</$1></$1></div></div>Connectivity</h3>Multi-protocol connectivity including WiFi, LoRaWAN, NB-IoT, and 5G</p></$1></div>Security</h3>End-to-end security with device authentication and data encryption</p></$1></div>Edge Computing</h3>Low-latency processing and real-time decision making at the edge</p></$1></div>Device Management</h3>Comprehensive device lifecycle management and monitoring</p></$1></div>AI Analytics</h3>Machine learning and AI for predictive analytics and insights</p></$1></div>Data Visualization</h3>Real-time dashboards and analytics for IoT data insights</p></$1></$1></$1></$1>
        {/* Contact Section */}
        <section className="py-16 bg-gradient-to-r from-green-600 to-teal-600 text-white"></section></div>
              Ready to Connect Your World?</$1>
              Contact our IoT experts for a free consultation and custom IoT strategy development</$1></div>
                📞 +1 302 464 0950</$1>
                ✉️ kleber@ziontechgroup.com</$1></$1></div>📍 364 E Main St STE 1008, Middletown DE 19709</p></$1></$1></$1></$1>
  )
}
