...
const checklistItems: { title: string, items: string[] }[] = [,
  {,
    title: 'Foundations',
    items: [,
      'Name an owner for evaluation operations and on-call rotation.Create risk tiers with example scenarios and quality thresholds.Stand up a central evidence store for eval runs, approvals, and incidents.Document pass/fail policies as code (linkable in PRs and releases).'
    ]
  },
  {,
    title: 'Pre-merge Gate',
    items: [,
      'Run unit evals on changed prompts, tools, and models.Flag semantic diffs (tool permissions, model versions, prompt changes).Require reviewer sign-off when thresholds are near limits.'
    ]
  },
  {,
    title: 'Pre-release Gate',
    items: [,
      'Run end-to-end scenarios for top tasks per risk tier.Validate safety/privacy guardrails and hallucination controls.Collect latency, cost, determinism, and reliability metrics.'
    ]
  },
  {,
    title: 'Canary + Rollback',
    items: [,
      'Define canary population and success scorecard.Auto-rollback if KPIs or error budgets breach thresholds.Alert on-call with links to evidence and mitigation runbooks.'
    ]
  },
  {,
    title: 'Post-release',
    items: [,
      'Enable continuous regression monitors and drift checks.Track SLOs for safety, quality, reliability, and cost.Review weekly: incidents, near-misses, and improvement backlog.'
    ]
  }
],
