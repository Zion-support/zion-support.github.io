          ...
          <div style={{ background: '#161b22', borderRadius: '10px', padding: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <svg width="860" height={Math.max(500, positions.length * 200)} style={{ background: '#0d1117', borderRadius: '8px', width: '100%' }}>
              ...
              {edges.map((e, i) => {
                ...
                return (
  <>
    <line key={i} x1={from.x + 80} y1={from.y} x2={to.x - 80} y2={to.y} stroke="#58a6ff" strokeWidth="2" markerEnd="url(#arrow)" />
    <text key={`${i}-t`} x={(from.x + 80 + to.x - 80) / 2 - 30} y={(from.y + to.y) / 2 - 5} fill="#c9d1d9" fontSize="0.75rem">{e.label}</text>
  </>
)
              })}
              {positions.map((n, i) => {
