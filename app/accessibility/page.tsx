...
const AccessibilityPage: React.FC = () => {
  ...
  const features = [{
      icon: Eye, title: 'Visual Accessibility', description: 'Comprehensive visual accessibility features including high contrast modes, text scaling, and screen reader compatibility.', benefits: ['High contrast mode', 'Text scaling', 'Screen reader support', 'Color blind friendly']
  );
},
    {icon: MousePointer,
    },
            ...
            <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-8">,</div>
              {standards.map((standard, index) => (
                ...
                >
                    ...
                    </div><CheckCircle className="w-8 h-8 text-white" /></CheckCircle>
                  </div>
                  ...
                  <span className="inline-block bg-green-500/20 text-green-400 px-3 py-1 rounded-full text-sm font-medium">{standard.status</span>} </span>
                </div>
              ))}
            </div>
          </div>
