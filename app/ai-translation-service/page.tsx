
      icon: <Brain className="w-6 h-6 text-purple-400" />,"
      title: 'AI-Powered'
      description: '
      title: 'Document Translation'
      description: '
      title: 'Real-time Translation'
      description: '
    {category: 'Translation Types'
      items: ['Text Translation', 'Document Translation', 'Website Translation', 'Audio Translation', 'Video Subtitles', 'Live Translation'
      category: 'Languages'
      items: ['English', 'Spanish', 'French', 'German', 'Chinese', 'Japanese', 'Arabic', 'Portuguese', 'Russian', 'Italian'
      category: 'Industries'
      items: ['Legal', 'Medical', 'Technical', 'Business', 'Marketing', 'E-commerce', 'Education', 'Entertainment'
      category: 'Features'
      items: ['Context Awareness', 'Industry-Specific Models', 'Quality Assurance', 'Human Review', 'API Integration', 'Batch Processing'
    {name: 'Basic'
      price: '$29'
      period: '/month'
      description: 'Perfect for individuals and small projects'
        'Up to 100,000 words/month',"50+ languages',"Basic document translation',"Email support'
      name: 'Professional'
      price: '$99'
      period: '/month'
      description: 'Ideal for businesses and teams'
        'Up to 500,000 words/month',"100+ languages',"All document types',"Priority support',"API access'
      name: 'Enterprise'
      price: '$299'
      period: '/month'
      description: 'For large organizations'
        'Unlimited words',"All languages',"Custom models',"24/7 support',"Dedicated account manager'
      title: 'Industry-Specific Translation'
      description: '
      title: 'Lightning Fast'
      description: '
      title: 'Global Reach'
      description: '