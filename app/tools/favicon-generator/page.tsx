...
export default function FaviconGeneratorPage() {
  ...
  const downloadAll = async () => {
    for (const s of SIZES) {
      const dataUrl = await drawFaviconAsync(s.size);
      if (!dataUrl) continue;
      const link = document.createElement('a');
      ...
      <div className="container mx-auto max-w-4xl px-4 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex gap-3">
            {([
              { key: 'text' as FaviconMode, label: 'Text', icon: <Type className="h-4 w-4" /> },
              { key: 'emoji' as FaviconMode, label: 'Emoji', icon: <span className="text-base">😀</span> },
              { key: 'image' as FaviconMode, label: 'Image', icon: <Image className="h-4 w-4" /> },
            ]).map(m => (
          ...
          <div className="mt-5 grid gap-5 md:grid-cols-2">
            {mode === 'text' && (
              <div>
                ...
                <input type="text" value={text} maxLength={3} onChange={e => setText(e.target.value)}
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  <Palette className="inline h-3.5 w-3.5 mr-1" /> Background color
                </label>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-slate-900">Download Sizes</h2>
            <button onClick={downloadAll}
              className="flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2 text-sm font-semibold text-white hover:bg-violet-700 transition shadow-lg shadow-violet-500/25">
              <Download className="h-4 w-4" /> Download All
            </button>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
            {SIZES.map(s => (
              <button key={s.size} onClick={() => downloadFavicon(s.size)}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={copyHtml}
              ...
              {copiedHtml ? <Check className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4" />}
