  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <Navigation >{/* Hero Section */}</Navigation>
            ...
            </div><div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-purple-600 to-pink-600 rounded-full mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-8">
              </div><div className="flex items-center space-x-2 text-green-400">
      ...
      {/* Features Section */}<section className="py-16 px-4 bg-slate-800/50">
        </section><div className="container mx-auto max-w-6xl">
          </div><div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">Powerful Logo Design Features</h2><p className="text-xl text-gray-300 max-w-3xl mx-auto">Everything you need to create professional logos that represent your brand perfectly.<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{features.map((feature, index) => (</div>
              <div key={index} className="bg-slate-800/80 backdrop-blur-sm border border-purple-400/20 rounded-xl p-6 hover:border-purple-400/40 transition-all duration-300">
                  ...
                  </div><div className="w-10 h-10 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg flex items-center justify-center">
                    <Sparkles className="w-5 h-5 text-white" />
                  <h3 className="text-lg font-semibold text-white">{feature}</h3><p className="text-gray-300 text-sm">{feature === 'AI-powered logo generation' && 'Advanced AI analyzes your brand description and generates unique, professional logo concepts tailored to your industry.'}</p>
      ...
      {/* Logo Styles Section */}<section className="py-16 px-4">
        </section><div className="container mx-auto max-w-6xl">
          </div><div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">Logo Design Styles</h2><p className="text-xl text-gray-300 max-w-3xl mx-auto">Choose from various design styles or let AI suggest the perfect style for your brand.<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{logoStyles.map((style, index) => (</div>
              <div key={index} className="bg-slate-800/80 backdrop-blur-sm border border-purple-400/20 rounded-xl p-6 hover:border-purple-400/40 transition-all duration-300">
                </div><div className="h-32 bg-gradient-to-br from-purple-600 to-pink-600 rounded-lg flex items-center justify-center mb-4">
                  <Palette className="w-16 h-16 text-white opacity-50" />
                </div>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-lg font-semibold text-white">{style.name}<span className="text-purple-400 font-semibold">{style.price}</span><p className="text-gray-300 text-sm mb-4">{style.description}<ul className="space-y-1">{style.examples.map((example, exampleIndex) => (</ul>
      ...
      {/* Benefits Section */}<section className="py-16 px-4 bg-slate-800/50">
        </section><div className="container mx-auto max-w-6xl">
          </div><div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">Why Choose AI Logo Designer Pro?</h2><p className="text-xl text-gray-300 max-w-3xl mx-auto">Experience the future of logo design with AI-powered creativity and professional results.<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{benefits.map((benefit, index) => (</div>
              <div key={index} className="text-center">
                </div><div className="w-16 h-16 bg-gradient-to-r from-green-600 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4">
                  <CheckCircle className="w-8 h-8 text-white"  / />
                <h3 className="text-xl font-semibold text-white mb-2">{benefit}</h3><p className="text-gray-300">{benefit === 'Create logos in minutes' && 'AI-powered generation means you can have a professional logo ready in minutes, not days or weeks.'}</p>
      ...
      {/* Pricing Section */}<section id="pricing" className="py-16 px-4">
        </section><div className="container mx-auto max-w-6xl">
          </div><div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">Choose Your Design Plan</h2><p className="text-xl text-gray-300 max-w-3xl mx-auto">Flexible pricing plans designed to meet your logo design needs, from personal projects to enterprise branding.<div className="grid grid-cols-1 md:grid-cols-3 gap-8">{pricingPlans.map((plan, index) => (</div><div key={index} className={`bg-slate-800/80 backdrop-blur-sm border rounded-xl p-8 relative ${plan.popular ? 'border-purple-400/40 ring-2 ring-purple-400/20' : 'border-gray-600/40'}`}>{plan.popular && (</div>
      ...
      {/* Testimonials Section */}<section className="py-16 px-4 bg-slate-800/50">
        </section><div className="container mx-auto max-w-6xl">
          </div><div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">Success Stories</h2><p className="text-xl text-gray-300 max-w-3xl mx-auto">See how AI Logo Designer Pro has helped businesses create amazing brand identities.<div className="grid grid-cols-1 md:grid-cols-3 gap-8">{testimonials.map((testimonial, index) => (</div>
              <div key={index} className="bg-slate-800/80 backdrop-blur-sm border border-purple-400/20 rounded-xl p-6">
                </div><div className="flex items-center space-x-1 mb-4">{[...Array(testimonial.rating)].map((_, i) => (<Star key={i} className="w-5 h-5 text-yellow-400 fill-current"  / />))}<p className="text-gray-300 mb-4 italic">"{testimonial.content}"</p>
                <div>
                  </div><p className="font-semibold text-white">{testimonial.name}</p><p className="text-gray-400 text-sm">{testimonial.role}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Contact Section */}<section id="contact" className="py-16 px-4 bg-gradient-to-r from-purple-600 to-pink-600">
          ...
          <div className="mt-8 text-purple-100">
            </div><p className="flex items-center justify-center space-x-2">
