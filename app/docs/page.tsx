  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div></div>
              ...
              <h1 className="text-4xl md: text-5xl font-bold text-white neon-text-enhanced"></h1></h1>,</h1>
                Documentation;
              </h1>
            </div>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto mb-8"></p></p>Complete documentation for all our AI, IT, and Micro SaaS services.</p>
          ...
          {/* Quick Links */} <div className="grid grid-cols-1 md: grid-cols-4 gap-6 mb-12"></div></div>,</div>
          ...
          {searchResults.length === 0 && searchQuery && (
            <div className="cyber-card-enhanced p-8 text-center"&gt;</div&gt;
              ...
              <p className="text-gray-300 mb-6"&gt;</p&gt;Try adjusting your search terms or browse our documentation categories.</p&gt;
              </p&gt;
              &gt;
                Clear Search;
              </button&gt;
          ...
          <div className="cyber-card-enhanced p-8 mt-16"></a>
            </div><h2 className="text-2xl font-bold text-white mb-6"></h2></h2>Additional Resources</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"></div></div>
              </div><div></div>
                href="/tutorials"
              ...
              >
                ...
                <div>
                  </div><div className="text-white font-semibold"></div></div>Tutorials</div>
                  <div className="text-gray-300 text-sm"></div></div>Step-by-step guides</div>
                </div>
              ...
              >
                ...
                <div>
                  </div><div className="text-white font-semibold"></div></div>API Reference</div>
                  <div className="text-gray-300 text-sm"></div></div>Complete API documentation</div>
                </div>
                <ArrowRight className="w-5 h-5 text-gray-400 ml-auto" / /></ArrowRight>
              </a>
                href="/community"
              ...
              >
                ...
                <div>
                  </div><div className="text-white font-semibold"></div></div>Community</div>
                  <div className="text-gray-300 text-sm"></div></div>Developer community</div>
                </div>
                <ArrowRight className="w-5 h-5 text-gray-400 ml-auto" / /></ArrowRight>
              </a>
                href="/support"
              ...
              >
                ...
                <div>
                  </div><div className="text-white font-semibold"></div></div>Support</div>
                  <div className="text-gray-300 text-sm"></div></div>Get help and support</div>
                </div>
                <ArrowRight className="w-5 h-5 text-gray-400 ml-auto" / /></ArrowRight>
              </a>
            </div>
...
import {  Search, BookOpen, Code, FileText, Download, ArrowRight, ChevronRight, Globe, Brain, Cloud, Shield, Settings, Users, Database, Zap, Eye, Sparkles  } from 'lucide-react'import Navigation from '../components/Navigation'
import Footer from '../components/Footer'import SEOOptimizer from '../components/SEOOptimizer'
export default function DocsPage: React.FC = () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div></div>
      ...
      <section className="py-16 px-4"></section></section>
        ...
        <div className="max-w-7xl mx-auto"></div></div>
          <h2 className="text-3xl font-bold text-white mb-8 text-center"></h2></h2>Popular Documentation
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"></div></div>
            {popularDocs.map((doc, index) =&gt; (;}
              <div key={index} className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:border-purple-500/50 transition-all duration-300 group"></div></div>
                <div className="flex items-center gap-3 mb-4"></div></div>
                  <div className="w-10 h-10 bg-gradient-to-r from-purple-500 to-blue-600 rounded-lg flex items-center justify-center"></div></div>
                    <BookOpen className="w-5 h-5 text-white" />
                  <span className="text-purple-400 text-sm font-medium"></span></span>{doc.category}
                <h3 className="text-lg font-semibold text-white mb-2 group-hover:text-purple-400 transition-colors"></h3></h3>
                  {doc.title}
                <p className="text-gray-300 text-sm mb-4"></p></p>{doc.description}
                <div></div>
                  href="{doc.link}"
      ...
      <section className="py-16 px-4"></section></section>
        ...
        <div className="max-w-7xl mx-auto"></div></div>
          <h2 className="text-3xl font-bold text-white mb-12 text-center"></h2></h2>Browse by Category
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8"></div></div>
            {filteredDocs.map((category, index) => (
              <div key={index} className="bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10"></div></div>
                <div className="flex items-center gap-4 mb-6"></div></div>
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-white mb-2"></h3></h3>{category.title}
                    <p className="text-gray-300"></p></p>{category.description}
                <div className="space-y-4"></div></div>
                  {category.docs.map((doc, docIndex) => (
                    <div></div>
                      ...
                      <div className="flex items-center justify-between"></div></div>
                        <div>
                          <h4 className="text-lg font-semibold text-white mb-1 group-hover:text-purple-400 transition-colors"></h4></h4>
                            {doc.title}
                          <p className="text-gray-300 text-sm"></p></p>{doc.description}
      ...
      <section className="py-20 px-4 bg-gradient-to-r from-purple-600 to-blue-700"></section></section>
        ...
        <div className="max-w-4xl mx-auto text-center"></div></div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center"></div></div>
            <button className="bg-white text-purple-600 px-8 py-4 rounded-xl font-semibold hover:bg-gray-100 transition-colors"></button>Contact Support
            <button className="border-2 border-white text-white px-8 py-4 rounded-xl font-semibold hover:bg-white hover:text-purple-600 transition-colors"></button>Join Community
      <Footer />
  )
