  ...
  return (
    <>"
      <Helmet></Helmet><//Helmet><///Helmet>"
        <title>Docs - Zion Tech Group</title><//title><///title>"
      </Helmet><//Helmet>"
              ...
              {popularArticles.map((article, index) => ("
                <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 hover:border-cyan-500/40 transition-all duration-300 group">"
                  <div className="flex items-center justify-between mb-4">"
                    <span className="px-3 py-1 bg-cyan-500/20 text-cyan-400 text-sm rounded-full"></span><//span>"
                      {article.category}"
                    </span>"
                    <span className="text-gray-400 text-sm">{article.readTime}</span><//span>"
                  </div>"
          ...
          <div className="container mx-auto px-4">"
            ...
            <div className="max-w-6xl mx-auto space-y-6"></div>"
              {filteredSections.map((section) => ("
                <div key={section.id} className="bg-white/10 backdrop-blur-sm rounded-xl border border-white/20"></div><//div><///div><////div></////div>
                  <button;
                  ...
                  >"
                    <div className="flex items-center">"
                      ...
                      <div>"
                        ...
                        <p className="text-gray-300">{section.description}</p><//p><///p><////p>"
                      </div><//div><///div>"
                    </div><//div>"
                        ...
                        {section.articles?.map((article: { title: string; readTime: string; description: string }, articleIndex: number) => ("
                          <div key={articleIndex} className="bg-white/5 rounded-lg p-4 hover:bg-white/10 transition-colors cursor-pointer group">"
                            <div className="flex items-center justify-between mb-2">"
                              <h4 className="text-lg font-semibold text-white group-hover:text-cyan-400 transition-colors"></h4><//h4>"
                                {article.title}"
                              </h4>"
                              <span className="text-gray-400 text-sm">{article.readTime}</span><//span>"
                            </div>"
