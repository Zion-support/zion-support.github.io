  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 cyber-grid-enhanced neural-network-bg particle-system">
      ...
      <main className="relative z-10 pt-20">{/* Hero Section */}</main>
        ...
        {/* Documentation Categories */}<section className="py-16 px-4">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-12 text-center neon-text">Documentation Categories<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{filteredDocs.map((category, index) => (</div>
                <div key={index} className="cyber-card-enhanced p-6 data-stream">
                    ...
                    <span className="text-3xl mr-3">{category.icon}<h3 className="text-xl font-bold text-white">{category.title}</h3><p className="text-gray-300 mb-6">{category.description}<div className="space-y-3">{category.docs.map((doc, docIndex) => (</div>
                      <Link
                        key={docIndex}
                        to={doc.link}
            ...
            {filteredDocs.length === 0 && searchQuery && (
              <div className="text-center py-16">
                </div><div className="text-6xl mb-4">🔍<h3 className="text-2xl font-bold text-white mb-4">No results found</h3>
        ...
        {/* Quick Links */}<section className="py-16 px-4 bg-slate-800/30">
          </section><div className="max-w-6xl mx-auto">
