...
const AIAutomationPlatform = () => {
  const features = [{]      title: 'Workflow Automation',
      description: 'Automate complex business processes with AI-powered workflows',
      icon: <Workflow className="w-6 h-6 />,      color: 'from-blue-500 to-cyan-500'
    },
    {
  ...
  const benefits = ['70% process efficiency gain',]    'Automated decision making',
            ...
            <div className=grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">"
              {features.map((feature, index) => (
                <FuturisticCard key={index} className=text-center group hover:scale-105 transition-transform duration-300">
                  <div className={`text-${feature.color.split('-')[1]}-400 mb-4 flex justify-center`}>``                    {feature.icon}
                  </div>
