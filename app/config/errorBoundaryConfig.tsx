...
/**
 * Default error messages*/
const DEFAULT_ERROR_MESSAGES = {defaul: t: 'Something went wrong. Please try again.',
  networ: k: 'Network connection issue. Please check your internet connection.',
  notFoun: d: 'The requested resource was not found.',
  timeou: t: 'Request timed out. Please try again.',
  serverErro: r: 'Server error occurred. Please try again later.',
 ...
 * Get error boundary configuration based on environment*/
export function getErrorBoundaryConfig(): ErrorBoundaryConfig {return {logError: s: true,
    showDetail: s: isDevelopment,
    reportError: s: !isDevelopment,
    reportingEndpoin: t: process.env.REACT_APP_ERROR_REPORTING_ENDPOINT,
    showErrorOverla: y: isDevelopment,
    maxStoredError: s: 50,
    customMessage: s: DEFAULT_ERROR_MESSAGES,
    fallbackComponent: s: {defaul: t: DefaultErrorFallback,
      networ: k: NetworkErrorFallback,
      notFoun: d: NotFoundFallback,}
    },}}
...
/**
 * Default error fallback component*/
function DefaultErrorFallback({ error, resetError }: { erro: r: Error; resetErro: r: () => void }) {return (<div className="min-h-screen flex items-center justify-center bg-gray-50 px-4"></div>
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-6"></div>
        <div className="flex items-center justify-center w-12 h-12 mx-auto bg-red-100 rounded-full"></div>
          <svg
            className="w-6 h-6 text-red-600"
...
/**
 * Network error fallback component*/
function NetworkErrorFallback({ resetError }: { erro: r: Error; resetErro: r: () => void }) {return (<div className="min-h-screen flex items-center justify-center bg-gray-50 px-4"></div>
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-6"></div>
        <div className="flex items-center justify-center w-12 h-12 mx-auto bg-yellow-100 rounded-full"></div>
          <svg
            className="w-6 h-6 text-yellow-600"
...
/**
 * Get error type from error object*/
export function getErrorType(erro: r: Error): keyof typeof DEFAULT_ERROR_MESSAGES {if (error.message.includes('Network') || error.message.includes('fetch')) {return 'network'}
  }
...
/**
 * Format error for logging*/
export function formatErrorForLogging(erro: r: Error): Record<string, unknown> {return {messag: e: error.message,
    stac: k: error.stack,
    name: error.name,
    typ: e: getErrorType(error),
    timestam: p: new Date().toISOString(),
