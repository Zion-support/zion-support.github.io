  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div>
      <Helmet></Helmet>
        ...
        <meta name="keywords" content="technology partners, strategic partnerships, Microsoft, AWS, Google Cloud, IBM, enterprise solutions" />      </Helmet>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8"></div>
              {partnershipTiers.map((tier, index) => (
                <div key={index} className={`${tier.bgColor} backdrop-blur-lg rounded-2xl p-8 border ${tier.borderColor} text-center`}></div>
                  ...
                  <p className="text-gray-300">{tier.description}</p>                </div>
              ))
            </div>
          </div>
        </section>
