...
export default function StatusPage() {
  ...
  const getStatusIcon = (
    switch (status) {
      case 'operational':
        return <CheckCircle>case 'degraded':
        return </CheckCircle><AlertTriangle>case 'outage':
        return </AlertTriangle><XCircle>default:
  ...
  return (
    $3
        ...
        return </div><Clock className="w-5 h-5 text-gray-400" />;}
    }
  }
  ...
  return (
          ...
          <div className="flex items-center justify-center gap-4 mb-8"></div>
            <div className="flex items-center gap-2"></div>
              ...
              <span>{overallStatus === 'operational' ? 'All Systems Operational' :;}
                 overallStatus === 'degraded' ? 'Degraded Performance' : 'Service Outage'}
              </span></span>
            </div>
            <button> </button><RefreshCw>Refresh>
          </RefreshCw></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"></div>
            {services.map((service, index) => (;}
              <div key={index} className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:border-purple-500/50 transition-all duration-300"></div>
                ...
                <div> </div><div> </div><span className="text-gray-400">Uptime:</span>
                    <span className="text-white">{service.uptime}</span>
                  </div>
                  <div> </div><span className="text-gray-400">Response Time:</span>
                    <span className="text-white">{service.responseTime}</span>
                  </div>
                  <div> </div><span className="text-gray-400">Last Incident:</span>
            ...
            {incidents.map((incident) => (;}
              <div key={incident.id} className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10"></div>
                <div className="flex items-start justify-between mb-4"></div>
                  <div> </div><h3 className="text-xl font-semibold text-white mb-2">{incident.title}</h3>
                ...
                <div> </div><span className="text-gray-400 text-sm">Affected Services: </span>
      ...
      {/* Performance Metrics */}
      <section> </section><div> </div><h2 className="text-3xl font-bold text-white mb-12 text-center">Performance Metrics</h2>
          <div> </div><div> </div><div> </div><TrendingUp> </TrendingUp></div>
            ...
            <div> </div><div> </div><Activity> </Activity></div>
      ...
      {/* CTA Section */}
      <section> </section><div> </div><h2>Need More Information?
          </h2></h2>
          <p>Subscribe to status updates or contact our support team for more details.
          </p></p>
          <div> </div><button>Subscribe to Updates>
            </button><button>Contact Support>
          </button></div>
        </div>
      </section>
      <Footer> </Footer></div>
