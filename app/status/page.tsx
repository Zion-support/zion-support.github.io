    ...
    : services.some(service => service.status === 'outage');</Clock>
      ...
      : 'degraded'</Clock>return (<//Clock>
          ...
          <div className="flex items-center justify-center gap-4 mb-8"></div>
            <div className="flex items-center gap-2"></div>{getStatusIcon(overallStatus)} {
    overallStatus === 'operational' ? 'All Systems Operational' :
  }<//div>
                 overallStatus === 'degraded' ? 'Degraded Performance' : 'Service Outage'}<//div>
              </span>
            </div>
            <button>
      ...
      </section>{/* Services Status */}<//section>
      <section>
          <div>
    ...
    services.map((service, index) => (<//div>
              ...
              <div key={index} className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:border-purple-500/50 transition-all duration-300"></div>
                <div className="flex items-center justify-between mb-4"></div>
                  <div className="flex items-center gap-3"></div>{getStatusIcon(service.status)}<//div>
                ...
                <div className="flex items-start justify-between mb-4"></div>
                  <div></div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-400"></div>
                      <span>Started: {new Date(incident.startTime).toLocaleString()} Ended: {new Date(incident.endTime).toLocaleString()}</span>
                    </div>
