...
import { Link } from 'react-router-dom';import { Search, Calendar, User, Clock, ArrowRight, Tag } from 'lucide-react'
interface BlogPost {
  id: string
  title: string
  excerpt: string
  ...
  return (
    <>
      <Helmet></Helmet>
        ...
        <meta name="keywords" content="tech blog, AI insights, blockchain news, cloud computing, IT solutions" />      </Helmet>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div>
        ...
        {/* Blog Posts */}        <section className="py-16 px-4"></section>
          <div className="max-w-7xl mx-auto"></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"></div>
              {displayPosts.map((post) => (
                <article key={post.id} className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:bg-white/10 transition-all duration-300"></article>
          ...
          <div className="max-w-7xl mx-auto"></div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8"></div>
              {blogPosts.filter(post => !post.featured).map((post) => (
                <article key={post.id} className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 hover:bg-white/20 transition-all duration-300 group"></article>
                  <div className="bg-gradient-to-br from-cyan-500/20 to-purple-500/20 rounded-lg h-48 flex items-center justify-center mb-6"></div>
                  ...
                  <div className="flex items-center justify-between"></div>
                    ...
                    <button className="flex items-center space-x-1 text-cyan-400 hover:text-cyan-300 transition-colors font-medium"></button>
                      <span>Read</span>
                      <ArrowRight className="w-4 h-4" /></ArrowRight>
                    </button>
                  </div>
        ...
        <section className="py-16 px-4"></section>
          <div className="max-w-4xl mx-auto text-center"></div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto"></div>
              ...
              /></input>
              <button className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-300"></button>
                Subscribe
              </button>
            </div>                </article>
              ))
            ...
            {searchResults.length === 0 && searchQuery && (              <div className="text-center py-12"></div>
