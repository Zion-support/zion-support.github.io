...
export default function AIAgentObservability2025Page() {return (<>
      ...
      <divclassName='min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 text-white'>
          ...
          <articleclassName='max-w-4xl mx-auto'>
            <headerclassName='mb-12'>
              <divclassName='flex items-center gap-3 mb-4'>
                <spanclassName='px-3 py-1 bg-cyan-400/20 text-cyan-300 text-sm rounded-full border border-cyan-400/30'>
                  AI Observability
                </span>
                ...
                <spanclassName='text-white/60 text-sm'>10 min read</span>
              </div>
              <h1className='text-5xl font-extrabold mb-6 bg-gradient-to-r from-cyan-400 to-indigo-400 bg-clip-text text-transparent'>
            ...
            <divclassName='prose prose-invert prose-lg max-w-none'>
              ...
              <sectionclassName='mb-12'>
                ...
                <divclassName='grid md:grid-cols-2 gap-6'>
                  <divclassName='bg-white/5 border border-white/10 rounded-xl p-6'>
                    <h3className='font-semibold mb-2'>Reliability SLO</h3>
                    <pclassName='text-white/70'>
                      P95 end-to-end latency ≤ 2.5s, success ≥ 99.5%
                    </p>
                  </div>
                  <divclassName='bg-white/5 border border-white/10 rounded-xl p-6'>
                    <h3className='font-semibold mb-2'>Safety SLO</h3>
                    <pclassName='text-white/70'>
              ...
              <sectionclassName='mb-12'>
                ...
                <preclassName='bg-black/40 rounded-xl p-4 overflow-x-auto text-sm'>
                  {`{
  episodeId: stringagentName: stringuserId?: stringsteps: Array<{
    ...
    tool?: {name: string; args: Record<stringunknown>; error?: string }safety?: {policy: string; decision:'allow' |'block'; score: number }}>outcome: {status:'success' |'failure'; evalScore?: number; costUsd?: number }}`}
            ...
            <footerclassName='mt-16 pt-8 border-t border-white/20'>
              <divclassName='flex items-center justify-between'>
                <divclassName='text-white/60 text-sm'>
