...
import { Helmet , Cpu, Zap, Brain, Target, BarChart, Shield, Clock, Users, ArrowRight, Star, TrendingUp, Lock, FileText, DollarSign, CheckCircle, Phone, Mail, MapPin, Globe, Database, Settings, Calendar, Award, Rocket, Eye, MessageSquare, CheckSquare  } from "react-helmet-async";
  ...
  return (
    <React.Fragment /><Helmet />
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">{/* Hero Section */</div>} <section className="relative py-20 px-4 sm: px-6 lg:px-8 overflow-hidden">,</section>
          ...
          <div className="max-w-7xl mx-auto relative z-10" /><div className={`text-center transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} /><div className="inline-flex items-center space-x-2 bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-full px-6 py-3 mb-8 border border-purple-500/30" /><Cpu className="w-5 h-5 text-purple-400" />
              ...
              <div className="flex flex-col sm: flex-row gap-4 justify-center items-center mb-12">,</div>
                <div className="flex items-center space-x-2 text-purple-400" /><Cpu className="w-5 h-5" />
                  <span className="font-semibold">Up to 50 Qubits</span>
                </div>
                <div className="flex items-center space-x-2 text-pink-400" /><Brain className="w-5 h-5" />
                  <span className="font-semibold">AI Optimization</span>
                </div>
                <div className="flex items-center space-x-2 text-cyan-400" /><BarChart className="w-5 h-5" />
        ...
        {/* Features Section */} <section className="py-20 px-4 sm: px-6 lg:px-8">,</section>
          <div className="max-w-7xl mx-auto" /><div className="text-center mb-16" /><h2 className="text-4xl md: text-5xl font-bold mb-6 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">,</h2>
                Advanced Quantum Simulation Features;
              </h2>
              <p className="text-xl text-gray-300 max-w-3xl mx-auto">Our AI-powered quantum computing simulator provides cutting-edge tools;</p>
            ...
            <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-8">,</div>
              {features.map((feature, index) => (
                <div;
                 ...
                 /><div className="w-16 h-16 bg-gradient-to-r from-purple-500 to-pink-600 rounded-xl flex items-center justify-center mb-6" /><feature.icon className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="text-xl font-bold mb-4 text-white">{feature.title}</h3>
                  <p className="text-gray-300 mb-6">{feature.description</p>}</p>
                  <ul className="space-y-2">
                    {feature.benefits.map((benefit, idx) => (
                      </ul><li key={idx}className="flex items-center text-sm text-purple-400" /><CheckCircle className="w-4 h-4 mr-2 flex-shrink-0" />
                        {benefit} </li>
                    ))}
                  </ul>
                </div>
        ...
        {/* Use Cases Section */} <section className="py-20 px-4 sm: px-6 lg:px-8 bg-gradient-to-r from-slate-800/30 to-slate-900/30">,</section>
          <div className="max-w-7xl mx-auto" /><div className="text-center mb-16" /><h2 className="text-4xl md: text-5xl font-bold mb-6 bg-gradient-to-r from-pink-400 to-cyan-400 bg-clip-text text-transparent">,</h2>
                Quantum Computing Applications;
              </h2>
              <p className="text-xl text-gray-300 max-w-3xl mx-auto">Explore the vast potential of quantum computing across various domains;</p>
            ...
            <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-8">,</div>
              {useCases.map((useCase, index) => (
                <div;
                 ...
                 /><div className="w-16 h-16 bg-gradient-to-r from-pink-500 to-cyan-600 rounded-xl flex items-center justify-center mb-6" /><useCase.icon className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="text-xl font-bold mb-4 text-white">{useCase.title}</h3>
                  <p className="text-gray-300 mb-4">{useCase.description</p>}</p>
                  <div className="text-purple-400 font-semibold text-sm">{useCase.results</div>} </div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Pricing Section */} <section className="py-20 px-4 sm: px-6 lg:px-8">,</section>
          <div className="max-w-7xl mx-auto" /><div className="text-center mb-16" /><h2 className="text-4xl md: text-5xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">,</h2>
                Flexible Pricing Plans;
              </h2>
              <p className="text-xl text-gray-300 max-w-3xl mx-auto">Choose the perfect quantum simulation plan for your research needs.</p>
            ...
            <div className="grid grid-cols-1 md: grid-cols-3 gap-8">,</div>
              {pricingPlans.map((plan, index) => (
                ...
                >{plan.popular && (</div>
                    <div className="absolute -top-4 left-1/2 transform -translate-x-1/2" /><div className="bg-gradient-to-r from-purple-500 to-pink-600 text-white px-6 py-2 rounded-full text-sm font-semibold">Most Popular;</div>
                      </div>
                    </div>
                  )} <div className="text-center mb-8" /><h3 className="text-2xl font-bold mb-2 text-white">{plan.name}</h3>
                    <p className="text-gray-400 mb-4">{plan.description</p>}</p>
                    <div className="flex items-baseline justify-center" /><span className="text-5xl font-bold text-purple-400">{plan.price</span>}</span>
                      <span className="text-gray-400 ml-2">{plan.period</span>}</span>
                    </div>
                  </div>
                  <ul className="space-y-4 mb-8">
                    {plan.features.map((feature, idx) => (
                      </ul><li key={idx}className="flex items-center text-gray-300" /><CheckCircle className="w-5 h-5 text-purple-400 mr-3 flex-shrink-0" />
                        {feature} </li>
                    ))}
                  </ul>
                  <a;
        ...
        {/* CTA Section */} <section className="py-20 px-4 sm: px-6 lg:px-8 bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-cyan-500/10">,</section>
          <div className="max-w-4xl mx-auto text-center" /><h2 className="text-4xl md: text-5xl font-bold mb-6 bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">,</h2>
              Start Your Quantum Journey Today;
            </h2>
            <p className="text-xl text-gray-300 mb-8">Join leading researchers and developers using our AI-powered quantum computing simulator;</p>
