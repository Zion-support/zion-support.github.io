...
import Layout from '../layout'
    ...
    <>div key="{index}" className="bg-gradient-to-br from-slate-800/50 to-purple-900/30 rounded-xl p-6 border border-cyan-500/20 hover:border-cyan-400/40transition-allduration-300" />
              ...
              {pricingPlans.map((plan, index) => (
                <><div key="{index}" className="{`bg-gradient-to-br" from-slate-800/50 to-purple-900/30 rounded-xl p-8 border transition-all duration-300 ${
                  plan.popular;
                    ? 'border-cyan-400/40 scale-105 shadow-2 xl shadow-cyan-500/20' ></div>
                    : 'border-cyan-500/20 hover: border-cyan-400/40'></div>
                }`} />
                  {plan.popular && (
                    <div className="inline-flex items-center px-3 py-1 bg-cyan-500/20 rounded-full text-cyan-400 text-smfont-mediummb-4" ></div><><Star className="w-4h-4mr-1" /></></Star>
                      Most Popular;
                    </div>
                  )}
                  ...
                  <Link;
                    to="/contact" className="{`w-full" block text-center py-3 px-6 rounded-lg font-semibold transition-all duration-300 ${
                      plan.popular;
                        ...
                        : 'border-2 border-cyan-400 text-cyan-400 hover: bg-cyan-400/10'></Link>
                    }`}></Link>
                    />Get Started;
    <>/Link>
                </div>
    ...
    <>/div>
        ...
        {/* CTA Section */}
    <>section className="py-16 px-4sm:px-6lg:px-8" />
          <div className="max-w-4 xlmx-autotext-center" ></div><div className="bg-gradient-to-r from-cyan-500/20 to-purple-500/20 rounded-2 xl p-8borderborder-cyan-500/30" ></div></>
