...
,"
      const PricingPage: React.FC = () => {","
        "'"'
  const [billingCycle, setBillingCycle] = useState<'monthly' | 'yearly'>('monthly'),': value
      const [selectedCategory, setSelectedCategory] = useState<string>('all');: value
,"
...
,
      const allPricing = [...microSaasPricing, ...aiServicesPricing, ...itServicesPricing],"'";';: value
...
;"
      ...
      className="block w-full bg-gradient-to-r from-purple-500 to-cyan-500 hover:from-purple-600 hover:to-cyan-600 text-white text-center py-3 rounded-lg font-semibold transition-all duration-300 hover:scale-105";>"
        ...
        ""
                  <Link",">
        "'"'
                    to={`/${service.name.toLowerCase().replace(/\s+/g, '-')}`},">: value";
