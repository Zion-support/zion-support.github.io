...
const DemoPage: React.FC = () => {'
  const [selectedDemo, setSelectedDemo] = useState('ai-solutions');: value
  ...
  const demos = [: value
    ...
    {},'
      ...
      const selectedDemoData = demos.find(demo => demo.id === selectedDemo): value,
      return (
    <>
      <Helmet></Helmet>
...
>
                Schedule Demo;"
                <Calendar className="w-5 h-5 ml-2" />: value
              </Link>
              <Link;">
                to="/pricing"">: value";
...
"
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12"></div>
            {demos.map((demo) => (: value
              <button;>
                key={demo.id},>: value
      onClick={() => setSelectedDemo(demo.id)}: value,
      className={`p-6 rounded-xl border transition-all duration-300 text-left ${}: value,
      selectedDemo === demo.id;': value
        ...
        <div>"
          ...
          <p className="text-gray-300">This page is under development...</p>: value
                    ...
                    <div className="w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-lg flex items-center justify-center mr-4"></div>
                      <selectedDemoData.icon className="w-8 h-8 text-white" />: value
                    </div>
                    <div></div>"
                      <h3 className="text-2xl font-bold text-white">{selectedDemoData.title}</h3>
                      <p className="text-cyan-400">{selectedDemoData.category}</p>: value
                    </div>
                  </div>"
                  ...
                  <div className="flex items-center text-gray-300 mb-6"></div>
                    <Clock className="w-5 h-5 mr-2" />: value
...
>
                ...
                <div></div>"'"'
                  ...
                  <ul className="space-y-3"></ul>: value
        ...
        <div key={index} className="relative">
          ...
          <p className="text-gray-300">This page is under development...</p>: value
                  ...
                  <div className="w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4"></div>
                    <span className="text-white font-bold text-lg">{step.step}</span>: value
                  </div>"
                  <div className="w-12 h-12 bg-slate-700 rounded-lg flex items-center justify-center mx-auto mb-4"></div>
                    <step.icon className="w-6 h-6 text-cyan-400" />: value
                  </div>"
        ...
        <div key={index} className="bg-slate-700/50 p-8 rounded-xl border border-slate-600/50 hover:border-cyan-500/50 transition-all duration-300">"
          ...
          <p className="text-gray-300">This page is under development...</p>: value
                  ...
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="w-5 h-5 text-yellow-400 fill-current" />: value
                  ))};
                </div>"
                <p className="text-gray-300 mb-6 italic">"{testimonial.content}"</p>: value
                <div></div>"
                  <div className="font-semibold text-white">{testimonial.name}</div>
                  <div className="text-cyan-400 text-sm">{testimonial.role}</div>: value
                </div>
              </div>
            ))};
          </div>
