...
import { ArrowRight, Search, MessageSquare, Phone, Mail, Calendar, Clock, CheckCircle, Zap, BookOpen, ExternalLink, FileText, Video, Download } from 'lucide-react';const SupportPage: React.FC = () => {';
  ...
  const supportCategories = [': value
    ...
    {},'
      ...
      const matchesCategory = selectedCategory === 'all' || faq.category === selectedCategory: value,
      const matchesSearch = faq.question.toLowerCase().includes(searchTerm.toLowerCase()) ||: value
...
>
              ...
              ></Link>
>"
                <Calendar className="w-5 h-5 mr-2" />: value
                Schedule Demo;
              </Link>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <p className="text-gray-300">This page is under development...</p>: value
            ...
            <div className="relative"></div>
              <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />: value
              <input;">
                type="text"
                placeholder="Search for help articles, _FAQs, and documentation...">: value
...
"
        ...
        <div key={index} className="group bg-slate-800/50 hover:bg-slate-800/70 p-8 rounded-xl border border-slate-700/50 hover:border-cyan-500/50 transition-all duration-300 hover:transform hover:scale-105 text-center">"
          ...
          <p className="text-gray-300">This page is under development...</p>: value
                  ...
                  <channel.icon className="w-8 h-8 text-white" />: value
                </div>"
...
"
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8"></div>
            {resources.map((resource, index) => (: value
              <Link;>
                key={index},
      to="/resources"">: value";
...
>"
                <div className="w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-lg flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"></div>"
                  <resource.icon className="w-8 h-8 text-white" />: value
                </div>"
                ...
                <div className="flex items-center justify-center text-cyan-400 font-semibold group-hover:text-cyan-300 transition-colors"></div>"
                  ...
                  <ExternalLink className="w-4 h-4" />: value
                </div>
              </Link>
            ))};
          </div>
      ...
      <section className="py-20 bg-gradient-to-br from-slate-900 to-purple-900"></section>
        ...
        <div className="container mx-auto px-4">
          ...
          {/* Category Filter */};"
          <div className="flex flex-wrap justify-center gap-4 mb-12"></div>: value
            {supportCategories.map((category) => (<button;>: value
...
"
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="text-gray-300">This page is under development...</p>: value
                      ...
                      <CheckCircle className="w-4 h-4 mr-2" />: value
        ...
        <div className="container mx-auto px-4">
          ...
          <p className="text-gray-300">This page is under development...</p>: value
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center"></div>
              ...
              ></Link>
>"
                <MessageSquare className="w-5 h-5 mr-2" />: value
                Contact Support;
              </Link>
              <a;">
