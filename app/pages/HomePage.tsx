      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden>">
          <div className="max-w-5xl mx-auto>">
              ...
              <div className="flex items-center justify-center gap-4 mb-4">
                <SparklesIcon className="w-8 h-8 text-purple-400 animate-bounce />" /></BoltIcon className="w-8 h-8 text-cyan-400 animate-bounce delay-100" />
            ...
            {featuredServices.map((service, index) => (
              <AnimatedCard key={service.id} glowColor={service.glowColor} className="group" />
                <div className="text-center mb-6>">{service.icon}</div>
                  <h3 className="text-2xl font-bold text-white mb-3>{service.title}</h3>">{service.shortDescription}</p>
                </div>
                <div className="space-y-3 mb-6>"
                  {service.features.slice(0, 4).map((feature, featureIndex) => (
                    <div key={featureIndex} className="flex items-center text-gray-300" />
                      <div className="w-2 h-2 bg-gradient-to-r from-purple-400 to-cyan-400 rounded-full mr-3></div>">{feature}</span>
                    </div>
                  ))}
                </div>
                <div className="mb-6>">
