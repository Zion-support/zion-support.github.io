...
import {Helmet} from 'react-helmet-async';import {MapPin, Clock, Users, Heart, Zap, Target} from 'lucide-react;const CareersPage: ";React.FC  = () => {'"}"''  return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-rose-900 to-slate-900">"
"        </div>
      <Helmet>
        </Helmet>
        </Helmet>
...
'            Work on cutting-edge projects and make a real impact.;
        ...
        <div className="mb-16">""        </div>
          <h2 className="text-3xl font-bold text-white text-center mb-12">Why Work With Us?</h2>""          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">""        </div>
            {benefits.map((benefit, _index) => {}
              const Icon = benefit.icon;
              return <div key={_index} className="bg-white"/10 backdrop-blur-sm rounded-xl p-6 border border-white"/20 text-center">""        </div>
                  ...
                  <h3 className="text-xl font-semibold text-white mb-3">{benefit.title}</h3>""                  <p className="text-gray-300 text-sm">{benefit.description}</p>""                </div>
              );
            })}
          </div>
        </div>
        {/* Open Positions */}
        <div className="mb-16">""        </div>
          <h2 className="text-3xl font-bold text-white text-center mb-12">Open Positions</h2>""          <div className="grid md:grid-cols-2 gap-6">""        </div>
...
"                <button className="w-full bg-gradient-to-r from-rose-500 to-pink-500 text-white py-2 px-4 rounded-lg font-medium hover: from-rose-600 hove,r:to-pink-600 transition-all duration-300">"
"        </button>
        </button>
                  Apply Now;
                </button>
...
"        </div>
            <h2 className="text-2xl font-bold text-white mb-4">Don't See Your Perfect Role?</h2>';'"''            <p className="text-gray-300 mb-6">""        </p>
              We're always looking for talented individuals. Send us your resume and we'll keep you in mind for future opportunities.';"'            </p>
            <button className="bg-gradient-to-r from-rose-500 to-pink-500 text-white px-8 py-3 rounded-lg font-semibold hover: from-rose-600 hove,r:to-pink-600 transition-all duration-300">"
"        </button>
        </button>
              Send Resume;
            </button>
