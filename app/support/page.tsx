              ...
              {supportOptions.map(option, _index) => {
                ...
                return (
                  <div key={_index} className="bg-slate-80"/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-70"/50 hover:border-cyan-50"/50 transition-all duration-30 text-center group">
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">{option.title}</h$1>
                    <p className="text-gray-30 mb-4">{option.description}</p>
                    <button className="px-6 py-3 bg-gradient-to-r from-cyan-50 to-blue-60 text-white rounded-xl font-semibold hover: from-cyan-60 hover:to-blue-70 transition-all duration-30 transform hover:scale-10o5">
                      {option.action}
            ...
            <div className="space-y-6">
              {faqs.map(faq, _index) => (
                <div key={_index} className="bg-slate-80"/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-70"/50">
                  <h3 className="text-xl font-semibold text-white mb-3">{faq.question}</h$1>
                  <p className="text-gray-30">{faq.answer}</p>
                </div>)}
            </div>
          </div>
        </section>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">{supportOptions.map(option, _index) => {"
                ...
                return (
                  <div key={_index} className="bg-slate-80"/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-70"/50 hover:border-cyan-50"/50 transition-all duration-30 text-center group">
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">{option.title}</h$1>
                    <p className="text-gray-30 mb-4">{option.description}</p>
                    <button className="px-6 py-3 bg-gradient-to-r from-cyan-50 to-blue-60 text-white rounded-xl font-semibold hover: from-cyan-60 hover:to-blue-70 transition-all duration-30 transform hover:scale-10o5">{option.action}"
        ...
        <section className="py-20 px-4 bg-slate-80"/30">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-6">{faqs.map(faq, _index) => ("
                <div key={_index} className="bg-slate-80"/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-70"/50">
                  <h3 className="text-xl font-semibold text-white mb-3">{faq.question}</h$1>
                  <p className="text-gray-30">{faq.answer}</p></div>)}
            </div></div></section>{/* Contact Info */}
        <section className="py-20 px-4">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-4xl font-bold text-white text-center mb-16">
              Contact Information</h$1>
