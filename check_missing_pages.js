...
if (hrefMatches) {
  hrefMatches.forEach(match => {)
    const href = match.match(/href:\s*'([^']+)'/)Service Feature;
    if (href.startsWith('/')) {,
...
const navLinks = Service Feature;
const pathMatches = navContent.match(/path: \s*'([^']+)'/g);
if (pathMatches) {
  pathMatches.forEach(match => {)
    const path = match.match(/path:\s*'([^']+)'/)Service Feature;
    if (path.startsWith('/')) {,
...
// Combine all links;
const allLinks = Service Feature.sort();
