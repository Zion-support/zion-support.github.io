import React from 'react';
import Link from 'next/link';

const knownPaths: string[] = [
  '/',
  '/about',
  '/contact',
  '/blog',
  '/faq',
  '/careers',
  '/partners',
  '/news',
  '/solutions',
  '/research-development',
  '/request-quote',
  '/green-it',
  '/services',
  '/services/ai-analytics',
  '/services/cybersecurity',
  '/services/cloud-devops',
  '/services/iot-edge',
  '/services/quantum-computing',
  '/services/blockchain',
  '/services/digital-twin',
  '/services/sustainability',
  '/services/data-analytics',
  '/services/infrastructure',
  '/services/digital-transformation',
  '/services/consulting',
  '/services/onsite-support',
  '/services/5g-solutions',
  '/micro-saas',
  '/micro-saas/ai-business-intelligence',
  '/micro-saas/customer-experience',
  '/micro-saas/quantum-computing',
  '/micro-saas/supply-chain',
  '/micro-saas/cybersecurity',
  '/micro-saas/iot-edge',
  '/micro-saas/content-creation',
  '/micro-saas/hr-platform',
  '/it-services',
  '/it-services/infrastructure',
  '/it-services/digital-transformation',
  '/it-services/consulting',
  '/it-services/onsite-support',
  '/it-services/green-it',
  '/it-services/5g-solutions',
  '/marketplace',
  '/marketplace/products',
  '/marketplace/talent',
  '/marketplace/equipment',
  '/marketplace/services',
  '/talent',
  '/equipment',
  '/it-onsite-services',
  '/team',
  '/help',
  '/security',
  '/status',
  '/privacy',
  '/terms',
  '/cookies',
  '/sitemap',
  '/login',
  '/signup',
];

export default function CatchAllPage({ params }: any) {
  const slug = params?.slug || [];
  const requestedPath = '/' + (Array.isArray(slug) ? slug.join('/') : String(slug));
  const isKnown = knownPaths.includes(requestedPath);

  return (
    <main>
      <h1>{isKnown ? 'Page' : 'Coming Soon'}</h1>
      <p>
        {isKnown
          ? 'This page is under construction. Content will be added shortly.'
          : 'The page you requested is not available yet. Please check back soon.'}
      </p>
      <p>
        <Link href="/">Return to Home</Link>
      </p>
    </main>
  );
}
"