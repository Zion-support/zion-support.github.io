...
// Usage
const content = await generateContent('Write a blog post about AI');
console.log(content);`}
                    </pre>
                  </div>
                </div>
...
# Usage
client = ZionTechClient(os.getenv('ZION_API_KEY'))
result = client.generate_content('Create a marketing email')
...
function useZionTechAPI() {
  ...
  const apiKey = process.env.REACT_APP_ZION_API_KEY;
  ...
  const makeRequest = async (endpoint, options = {}) => {
    ...
    try {
      const response = await fetch(\`https://api.ziontechgroup.com/v1\${endpoint}\`, {
        headers: {
...
function ServicesList() {
  const { data, loading, error, makeRequest } = useZionTechAPI();
...
class ZionTechAPI {
    ...
    public function generateContent($prompt) {
        ...
        return $this->makeRequest('/ai/generate', 'POST', $data);
    }
}

// Usage
$api = new ZionTechAPI($_ENV['ZION_API_KEY']);
$services = $api->getServices();
$content = $api->generateContent('Write a product description');
...
const result = await safeApiCall('/services');
if (result.success) {
