...
function fixPageFile(filePath) {
  try {
    ...
    if (content.includes('import React from "react";') && content.includes('export default function Page()')) {'      // Extract page name from file path"      const pathParts = filePath.split('/');'      const fileName = pathParts[pathParts.length - 2]; // Get directory name
      const pageName = fileName.split('-').map(word => '        word.charAt(0).toUpperCase() + word.slice(1)
      ).join('') + 'Page';'      
...
function fixPageFiles(dirPath) {
  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      if (['node_modules', '.git', 'dist', 'build', '.next', 'components', 'hooks', 'lib', 'utils'].includes(item)) {'        continue;
      }
      fixedCount += fixPageFiles(fullPath);
    } else if (stat.isFile() && item === 'page.tsx') {'      if (fixPageFile(fullPath)) {
        fixedCount++;
      }
    }
  }
...
// Main execution
console.log('Starting page file fixes...');'const fixedCount = fixPageFiles('/workspace/app');'console.log(`Fixed ${fixedCount} page files.`);