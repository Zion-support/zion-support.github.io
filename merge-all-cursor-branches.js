...
async function mergeAllCursorBranches() {
  try {
    ...
    const branchesOutput = execSync('git branch -r | grep "cursor/"', { encoding: 'utf8' });
    ...
    for (const branch of branches) {
      ...
      try {
        ...
        try {
          execSync(`git merge origin/${branchName} --no-ff -m "Merge cursor branch: ${branchName}"`, { stdio: 'pipe' });
          log(`✅ Successfully merged branch: ${branchName}`, 'green');
          successCount++;
        } catch (mergeError) {
          // If merge fails, try to resolve conflicts automatically
          log(`⚠️ Merge conflict in ${branchName}, attempting auto-resolution...`, 'yellow');
          
          try {
            // Reset to main and try again
            execSync('git reset --hard HEAD', { stdio: 'pipe' });
            execSync(`git merge origin/${branchName} --no-ff -m "Merge cursor branch: ${branchName}"`, { stdio: 'pipe' });
            log(`✅ Successfully merged branch after conflict resolution: ${branchName}`, 'green');
            successCount++;
          } catch (retryError) {
            log(`❌ Failed to merge branch ${branchName}: ${retryError.message}`, 'red');
            failCount++;
          }
        }
        
      } catch (error) {
        log(`❌ Error processing branch ${branchName}: ${error.message}`, 'red');
        failCount++;
      }
    }
    ...
    try {
      ...
      execSync('git commit -m "Merge all cursor branches into main - comprehensive merge"', { stdio: 'inherit' });
      execSync('git push origin main', { stdio: 'inherit' });
      log('🎉 All changes committed and pushed successfully!', 'green');
      
    } catch (buildError) {
      log(`❌ Build failed: ${buildError.message}`, 'red');
      log('⚠️ Please check the build errors and resolve them manually.', 'yellow');
    }
