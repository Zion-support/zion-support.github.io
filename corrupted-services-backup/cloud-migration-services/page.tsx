import { Metadata } from 'next';

export const metadata: Metadata = {/* content */}
  title: 'Cloud Migration Services | Zion Tech Group - AWS, Azure, Google Cloud',
  description: 'Expert cloud migration services to AWS, Azure, and Google Cloud. Zero-downtime migration with cost optimization and 24/7 support.',
  keywords: 'cloud migration, AWS migration, Azure migration, Google Cloud, cloud services, infrastructure migration',
};

export default function CloudMigrationServicesPage() {
  return (
    <div>/* content */}
  return (
    <div>
      <div> </div><div> </div></div>
    <div className="text-left">{/* Hero Section */}</div></div>
      <section className="text-left"> </section><div className="text-left"> </div><h1 className="text-left">
            Cloud Migration Services
          </h1>
          <p className="text-left">Seamlessly migrate your infrastructure to AWS, Azure, or Google Cloud with 
            zero downtime, cost optimization, and comprehensive support.
          </p></p>
          <div className="text-left"> </div><a
              href="tel:+13024640950"
              className="text-left">Call +1 302 464 0950
            </a></a>
            <a
              href="mailto:kleber@ziontechgroup.com?subject=Cloud Migration Services Inquiry"
              className="text-left">Get Migration Quote
            </a></a>
          </div>
      </section>

      {/* Cloud Providers Section */}
      <section className="text-left"> </section><div className="text-left"> </div><div className="text-left"> </div><h2 className="text-left">
              Expert Cloud Platform Migration
            </h2>
            <p className="text-left">We specialize in migrating businesses to the world's leading cloud platforms 
              with proven methodologies and best practices.
            </p></p>
          </div>

          <div className="text-left">{/* AWS Migration */}</div></div>
            <div className="text-left"> </div><div className="text-left"> </div><div className="text-left"> </div><svg className="text-left" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/> </p></svg>
                </div>
                <h3 className="text-left">AWS Migration</h3>
                <p className="text-left">Amazon Web Services</p>
              </div>
              <ul className="text-left">
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  EC2, S3, RDS Migration
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Lambda & Serverless
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  VPC & Security Groups
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  CloudFormation & CDK
                </li>
              </ul>
              <div className="text-left"> </div><span className="text-left">Starting at $2,999</span>
              </div>

            {/* Azure Migration */}
            <div className="text-left"> </div><div className="text-left"> </div><div className="text-left"> </div><svg className="text-left" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M11.4 24H0V12.6h11.4V24zM24 24H12.6V12.6H24V24zM11.4 11.4H0V0h11.4v11.4zM24 11.4H12.6V0H24v11.4z"/> </p></svg>
                </div>
                <h3 className="text-left">Azure Migration</h3>
                <p className="text-left">Microsoft Azure</p>
              </div>
              <ul className="text-left">
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Virtual Machines & App Services
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  SQL Database & Cosmos DB
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Active Directory Integration
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  ARM Templates & Bicep
                </li>
              </ul>
              <div className="text-left"> </div><span className="text-left">Starting at $3,499</span>
              </div>

            {/* Google Cloud Migration */}
            <div className="text-left"> </div><div className="text-left"> </div><div className="text-left"> </div><svg className="text-left" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/> </p><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/> </p><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/> </p><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/> </p></svg>
                </div>
                <h3 className="text-left">Google Cloud</h3>
                <p className="text-left">Google Cloud Platform</p>
              </div>
              <ul className="text-left">
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Compute Engine & GKE
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Cloud Storage & BigQuery
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Firebase & App Engine
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Cloud Functions & AI/ML
                </li>
              </ul>
              <div className="text-left"> </div><span className="text-left">Starting at $2,999</span>
              </div>
          </div>
      </section>

      {/* Migration Process Section */}
      <section className="text-left"> </section><div className="text-left"> </div><div className="text-left"> </div><h2 className="text-left">
              Our Proven Migration Process
            </h2>
            <p className="text-left">We follow a systematic approach to ensure successful cloud migration with minimal risk and maximum efficiency.
            </p></p>
          </div>

          <div className="text-left"> </div><div className="text-left"> </div><div className="text-left">1</div>
              <h3 className="text-left">Assessment</h3>
              <p className="text-left">Comprehensive analysis of current infrastructure, applications, and dependencies.</p>
            </div>
            <div className="text-left"> </div><div className="text-left">2</div>
              <h3 className="text-left">Planning</h3>
              <p className="text-left">Detailed migration strategy, timeline, and cost optimization plan.</p>
            </div>
            <div className="text-left"> </div><div className="text-left">3</div>
              <h3 className="text-left">Testing</h3>
              <p className="text-left">Pilot migration and thorough testing in cloud environment.</p>
            </div>
            <div className="text-left"> </div><div className="text-left">4</div>
              <h3 className="text-left">Migration</h3>
              <p className="text-left">Zero-downtime migration with real-time monitoring and rollback capability.</p>
            </div>
            <div className="text-left"> </div><div className="text-left">5</div>
              <h3 className="text-left">Optimization</h3>
              <p className="text-left">Performance tuning, cost optimization, and ongoing support.</p>
            </div>
        </div>
      </section>

      {/* Pricing Section */}
      <section className="text-left"> </section><div className="text-left"> </div><div className="text-left"> </div><h2 className="text-left">
              Cloud Migration Pricing
            </h2>
            <p className="text-left">Transparent pricing based on complexity and scale. All migrations include comprehensive support and optimization.
            </p></p>
          </div>

          <div className="text-left">{/* Small Business */}</div></div>
            <div className="text-left"> </div><div className="text-left"> </div><h3 className="text-left">Small Business</h3>
                <div className="text-left">$2,999</div>
                <p className="text-left">Up to 10 servers</p>
              </div>
              <ul className="text-left">
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Infrastructure assessment
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Zero-downtime migration
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  30 days support
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Basic monitoring setup
                </li>
              </ul>
              <a
                href="mailto:kleber@ziontechgroup.com?subject=Small Business Cloud Migration"
                className="text-left">Get Quote
              </a></a>
            </div>

            {/* Enterprise */}
            <div className="text-left"> </div><div className="text-left"> </div><span className="text-left">Most Popular</span>
              </div>
              <div className="text-left"> </div><h3 className="text-left">Enterprise</h3>
                <div className="text-left">$9,999</div>
                <p className="text-left">Up to 50 servers</p>
              </div>
              <ul className="text-left">
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Comprehensive assessment
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Zero-downtime migration
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  90 days support
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Advanced monitoring & security
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Cost optimization analysis
                </li>
              </ul>
              <a
                href="mailto:kleber@ziontechgroup.com?subject=Enterprise Cloud Migration"
                className="text-left">Get Quote
              </a></a>
            </div>

            {/* Custom Solution */}
            <div className="text-left"> </div><div className="text-left"> </div><h3 className="text-left">Custom Solution</h3>
                <div className="text-left">$15,999+</div>
                <p className="text-left">Large-scale enterprise</p>
              </div>
              <ul className="text-left">
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Custom migration strategy
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Multi-cloud architecture
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  24/7 dedicated support
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Advanced security & compliance
                </li>
                <li className="text-left">
                  <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /> </p></svg>
                  Ongoing optimization & management
                </li>
              </ul>
              <a
                href="mailto:kleber@ziontechgroup.com?subject=Custom Cloud Migration Solution"
                className="text-left">Contact Sales
              </a></a>
            </div>

          <div className="text-left"> </div><p className="text-left">All migrations include comprehensive testing, training, and documentation.
            </p></p>
            <p className="text-left"> </p><strong>Market Average:</strong> Cloud migration services range from $2,999-15,999 based on complexity
            </p>
          </div>
      </section>

      {/* CTA Section */}
      <section className="text-left"> </section><div className="text-left"> </div><h2 className="text-left">
            Ready to Migrate to the Cloud?
          </h2>
          <p className="text-left">Let our experts help you migrate to the cloud with zero downtime and maximum efficiency
          </p></p>
          <div className="text-left"> </div><a
              href="tel:+13024640950"
              className="text-left">Call +1 302 464 0950
            </a></a>
            <a
              href="mailto:kleber@ziontechgroup.com?subject=Cloud Migration Services Inquiry"
              className="text-left">Get Migration Quote
            </a></a>
          </div>
      </section>
  );
}</div></div></div></div></div></div></div></div></div></div>)