...
files.forEach(file => {
  try {
    ...
    // Fix Link components using href instead of to (but not regular <a> tags)
    ...
    if (linkMatches) {
      content = content.replace(linkHrefRegex, (match, before, href, after) => {
        // Only replace if it's not already using 'to=' and it's not a regular <a> tag
        if (!before.includes('to=') && !after.includes('to=') && match.includes('<Link')) {
          modified = true;
