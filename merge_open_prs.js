...
async function main() {
  try {
    ...
    // Get open PRs
    const openPRs = await makeGitHubRequest(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls?state=open`);
    ...
    for (const pr of openPRs) {
      ...
      const prDetails = await makeGitHubRequest(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls/${pr.number}`);
      
      if (prDetails.mergeable === false) {
        console.log(`❌ PR #${pr.number} has merge conflicts, attempting to resolve...`);
        ...
        try {
          ...
          if (mergeResult.merged) {
            ...
            console.log(`❌ Failed to merge PR #${pr.number}: ${mergeResult.message || 'Unknown error'}`);
          }
        } catch (error) {
        ...
        console.log(`⏳ PR #${pr.number} mergeable status is unknown, skipping for now`);
      }
    }
