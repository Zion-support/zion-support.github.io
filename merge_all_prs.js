...
const prsData = JSON.parse(fs.readFileSync('/workspace/current_prs.json', 'utf8'));
...
async function mergePR(pr) {
  ...
  execGitCommand('git fetch origin', `Fetching latest changes for PR #${prNumber}`);
  ...
  if (!checkoutResult) {
    console.log(`⚠️  Could not checkout branch for PR #${prNumber}, trying alternative approach`);
  ...
  if (mergeResult) {
    ...
    execGitCommand(`git branch -D temp-merge-${prNumber}`, `Cleaning up temporary branch for PR #${prNumber}`);
    console.log(`✅ Successfully merged PR #${prNumber}`);
    return true;
  } else {
    // Clean up on failure
    execGitCommand(`git branch -D temp-merge-${prNumber}`, `Cleaning up failed merge for PR #${prNumber}`);
    console.log(`❌ Failed to merge PR #${prNumber}`);
    return false;
  }
}
