...
function fixSyntaxErrors(content) {
  ...
  // Fix specific patterns for 404.tsx and similar files
  ...
  fixed = fixed.replace(/const Page = \(\) => \{';/g, 'const Page = () => {');
  ...
  fixed = fixed.replace(/return \(\s*<><\/>\s*<EnhancedSEO/g, 'return (\n    <>\n      <EnhancedSEO');
  
  // Fix missing closing tags in JSX
  fixed = fixed.replace(/<(\w+)[^>]*>\s*<\/\1>\s*<(\w+)[^>]*>\s*<\/\2>/g, '<$1>$2</$1>\n      <$2>');
