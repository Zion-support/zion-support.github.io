...
|---|---|---|---|
| War Room Pulse v3 (`.github/workflows/war-room-pulse.yml`) | cron `*/5` + human `issue_comment` on #71361 | Edits **one** standing comment (`<!-- war-room-standing:v1 -->`) with roster, OPEN/CLAIMED lanes, paste-ready JOIN, just-joined welcome, OFFLINE nudge, learned Done bullets. Also PATCHes the #71361 issue body (`<!-- war-room-issue:v1 -->`) so agents who skip the board still see OPEN lanes. If the room is empty, pings Slack `#cursor_channel` at most once / 30 min when a Slack secret exists. Cron skips unit tests so runners stay free. | Post a new comment every tick; write board JSON onto main; retrigger on `github-actions[bot]`; tell agents to STANDBY |
