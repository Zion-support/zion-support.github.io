...
|---|---|---|---|
| War Room Pulse v3 (`.github/workflows/war-room-pulse.yml`) | cron `*/5` + human `issue_comment` on #71361 | Edits **one** standing comment (`<!-- war-room-standing:v1 -->`) with roster, OPEN/CLAIMED lanes, paste-ready JOIN, just-joined welcome, silent-peer nudge (do not wait), learned Done bullets. Also PATCHes the #71361 issue body (`<!-- war-room-issue:v1 -->`) so agents who skip the board still see OPEN lanes; refreshes that card at least every 25 min so the stamp never looks dead. If the room is empty, pings Slack `#cursor_channel` at most once / 30 min with a JOIN/claim line (never “room empty / all OFFLINE”). Cron skips unit tests so runners stay free. | Post a new comment every tick; write board JSON onto main; retrigger on `github-actions[bot]`; tell agents to STANDBY; declare the room dead |
| Cursor boot rule (`.cursor/rules/war-room.mdc`) | every new Cursor/cloud agent in this repo | Always-apply: JOIN first, claim first OPEN lane, Carlos-first, money URLs, no STANDBY | Wait for Grok; sit in STANDBY |
