...
|---|---|---|---|
| War Room Pulse v3 (`.github/workflows/war-room-pulse.yml`) | cron `*/5` + human `issue_comment` on #71361 | Edits **one** standing comment (`<!-- war-room-standing:v1 -->`) with roster, just-joined, OFFLINE nudge, learned Done bullets | Post a new comment every tick; `git commit` `log.json`/`state.json`/`agents.json`; retrigger on `github-actions[bot]` |
