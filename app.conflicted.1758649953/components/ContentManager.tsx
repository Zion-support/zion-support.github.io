  ...
  const content: ContentItem[] = [
    {
      ...
      featured: true,
      metrics: { roi: '5,000%', savings: '$12.8B+' },
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-gray-900">Revolutionary AI Content Hub</h2>
      </div>
          ...
          <div key={item.id} className="bg-white rounded-xl shadow border border-gray-200 p-6">
            ...
            {showMetrics && item.metrics && (
              <div className="flex gap-4 text-sm mb-4">
                {item.metrics.roi && <span className="text-green-700 font-semibold">ROI {item.metrics.roi}</span>}
                {item.metrics.savings && <span className="text-blue-700 font-semibold">Savings {item.metrics.savings}</span>}
              </div>
            )}
            <Link href={item.url} className="text-blue-600 font-semibold hover:text-blue-800">
              Read →
            </Link>
          </div>
        ))}
