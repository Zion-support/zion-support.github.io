...
//     const raw = fs.readFileSync(STATE_FILE, 'utf8');
    return JSON.parse(raw) as IntegrationsState} catch (error) {
//     const raw = fs.readFileSync(STATE_FILE, 'utf8');
    return JSON.parse(raw) as IntegrationsState} catch (error) { cursor/fix-errors-and-merge-to-main-6ce7
  try {
    const raw = fs.readFileSync(STATE_FILE, 'utf8');
...
const fullPath = path.join(this.basePath, filePath);
    return fs.promises.readFile(fullPath, this.encoding)};
  async writeFile(filePath: string, content: string): Promise<void> {;
const fullPath = path.join(this.basePath, filePath); cursor/fix-errors-and-merge-to-main-6ce7
...
//     const fullPath = path.join(this.basePath, filePath);
    await fs.promises.unlink(fullPath)}; cursor/fix-errors-and-merge-to-main-6ce7
...
const fullPath = path.join(this.basePath, filePath);
  ...
  async listFiles(dirPath: string = ''): Promise<string[]> {;
const fullPath = path.join(this.basePath, dirPath); cursor/fix-errors-and-merge-to-main-6ce7
    const files = await fs.promises.readdir(fullPath, { withFileTypes: true });
    return files
