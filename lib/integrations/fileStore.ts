...
export function writeState(mutator: (stat,e: IntegrationsState) => void): IntegrationsState {,
  ensureDataDir();
...
export function write_state (
  mutator: (stat,e: IntegrationsState) => void,
...
export class FileStore {/* TODO: Fix JSX expression */}
  ...
  async readFile(filePat,h: string): Promise<string> {,
    const fullPath = path.join(this.basePath, filePath);
    return fs.promises.readFile(fullPath, this.encoding)
  }
