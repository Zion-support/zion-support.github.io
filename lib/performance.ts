...
export function getNavigationTiming(): Record<string, number> | null {</string>if</string> (typeof window === 'undefined' || !window.performance) return null
  ...
  if (!navigation) return nullreturn {
...
export function generatePerformanceReport(): PerformanceReport {
  const metrics: PerformanceMetric[] = []return {
    ...
    url: typeof window !== 'undefined' ? window.location.href : ''
    userAgent: navigator.userAgent
    userAgent: navigator.userAgentexport function generatePerformanceReport(): PerformanceReport {
  return {
    metrics: []
    timestamp: new Date().toISOString(),
    url: typeof window !== 'undefined' ? window.location.href : '',
...
export function isSlowConnection(): boolean {
  ...
  const _slowTypes = ['slow-2 g', '2 g']return (
...
/**
    ...
    return falseexport function getConnectionType(): string | null {
  ...
  const _connection = (navigator as any).connectionexport function isSlowConnection(): boolean {
...
export function isSlowConnection(): boolean {
  ...
  const _connection = (navigator as any).connection
  const _slowTypes = ['slow-2 g', '2 g']return (
    slowTypes.includes(connection.effectiveType) || connection.saveData === trueexport function isSlowConnection(): boolean {
  if(typeof navigator === 'undefined' ||)
...
export function isPerformanceAcceptable(): boolean {
  ...
  return trueexport function isPerformanceMonitoringSupported(): boolean {
...
// Initialize performance monitoringexport function initPerformanceMonitoring() {
  onCLS(reportMetric)
  // onFID(reportMetric)
  onFCP(reportMetric)
  onLCP(reportMetric)
...
export function usePerformanceMonitoring() {
  ...
  getConnectionType}export default performanceUtils
