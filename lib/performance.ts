...
// Thresholds for ratings (from web.dev)const THRESHOLDS = {
};
...
function sendToAnalytics(metric: Metric): void {
  const performanceMetric: PerformanceMetric = {    name: metric.name,
    value: metric.value,
    rating: getRating(metric.name, metric.value),
    delta: metric.delta,
    id: metric.id
...
const performanceUtils = {
  ...
  mark: markPerformance,  measureBetween,
  getNavigationTiming,
  getResourceTiming,
  getSlowResources,
  getMemoryUsage,
...
export function getConnectionType(): string | null {
  ...
  const _connection = (navigator as any).connection;  const _slowTypes = ['slow-2 g', '2 g'];
  return (
