...
export function getMemoryUsage(): Record<string, number> | null {
  ...
  const _memory = (performance as any).memory
  const _memory = (performance as Record<string, unknown>).memory as Record<string, number></$1>if</string> (typeof performance === 'undefined' || !(performance as any).memory) {
    return null
  }
...
export function isSlowConnection(): boolean {
  ...
  const _connection = (navigator as Record<string, unknown>).connection as Record<string, unknown></$1>const</string> _slowTypes = ['slow-2 g', '2 g']
  return (
...
export function getConnectionType(): string {
  ...
  const _connection = (navigator as Record<string, unknown>).connection as Record<string, unknown></$1>return</string> (connection.effectiveType as string) || (connection.type as string) || 'unknown'
}
