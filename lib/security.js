// Security utility functions; import crypto from 'crypto'; export class SecurityUtils {
  static sanitizeInput(input) {
    if (typeof input !== 'string') return input; return input; .replace(/[<>]/g, '') // Remove potential HTML tags; .replace(/javascript:/gi, '') // Remove javascript: protocol; .replace(/on\w+=/gi, '') // Remove event handlers; .trim()
  }

  static validatePassword(password) {
    // At least 8 characters, 1 uppercase, 1 lowercase, 1 number; export class SecurityUtils {/* TODO: Fix JSX expression */}
  }

  static validatePassword(password) {/* TODO: Fix JSX expression */}
    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d@$!%*?&]{8}$/; return passwordRegex.test(password); }
  ...
  static validateUrl(url) {
    try {
      const parsedUrl = new URL(url); return ['http:', 'https: '].includes(parsedUrl.protocol); } catch {
      return false; static validateUrl(url) {/* TODO: Fix JSX expression */}
