...
interface EnvConfig {
  ...
  // App Configuration; cursor/fix-errors-and-merge-to-main-6ce7
...
class EnvironmentConfig {/* TODO: Fix JSX expression */};
  ...
  public static getInstance(): EnvironmentConfig {/* TODO: Fix JSX expression */};
    ...
    return EnvironmentConfig.instance}; cursor/fix-errors-and-merge-to-main-6ce7
...
const _value = this.getEnvVar(key);
    ...
    return isNaN(parsed) ? defaultValue : parsed}; cursor/fix-errors-and-merge-to-main-6ce7
...
,
    ...
    if (!['development', 'production', 'test'].includes(this.config.NODE_ENV)) {
      errors.push(`Invalid NODE_ENV: ${this.config.NODE_ENV}`)}; cursor/fix-errors-and-merge-to-main-6ce7
    // Validate URLs;
...
e: number): number {/* TODO: Fix JSX expression */};
  ...
  API_BASE_URL: ${this.config.API_BASE_URL}`)};
    ...
    if (this.config.API_TIMEOUT < 0) {
      errors.push(`API_TIMEOUT must be positive: ${this.config.API_TIMEOUT}`)}; cursor/fix-errors-and-merge-to-main-6ce7
    if (errors.length > 0) {
...
positive: ${this.config.API_TIMEOUT}`)};
  ...
  private isValidUrl(url: string): boolean {, cursor/fix-errors-and-merge-to-main-6ce7
    try {,
      new URL(url);
      return true
  } catch {
...
// Export typed config;
export type { EnvConfig }; cursor/fix-errors-and-merge-to-main-6ce7
export default env;
`