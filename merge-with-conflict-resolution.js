...
function mergeBranch(branchName) {}
  try {}
    ...
    execSync(`git merge ${branchName} --no-ff -m "feat: Merge enhancements from ${branchName}"`, { stdio: 'pipe' })"    console.log(`✅ Successfully merged ${branchName}`)
    return true
  } catch (error) {}
    ...
    if (file.trim()) {}
      ...
      execSync(`git commit -m "feat: Resolve merge conflicts from ${branchName}"`, { stdio: 'inherit' })"      console.log(`✅ Successfully merged ${branchName} with conflict resolution`)
      return true
    } else {}
