...
function resolveConflicts(filePath) {try {
    ...
    if (!content.includes('') && !content.includes('') && !content.includes('>>>>>>>')) {
      return false; // No conflicts} console.log(`🔧 Resolving conflicts in $ {filePath}...`)
...
function mergeBranch(branchName) {try {
    ...
    if (conflictFiles.trim()) {console.log(`🔧 Resolving conflicts in ${conflictFiles.split('\n').length} files...`)
      const files = conflictFiles.trim().split('\n')
      let resolvedCount = 0
      for (const file of files) {
    if (file.trim()) {if (resolveConflicts(file.trim())) {
            resolvedCount++} }
      }
      ...
      return true
    } else {console.log(`❌ Could not resolve conflicts for ${branchName} `)
      return false
    }
  }
}
...
try {
  ...
  let mergedCount = 0
  let failedCount = 0
  for (const branch of branchesToMerge) {try {
      // Check if branch has unique commits
      const uniqueCommits = execSync(`git log --oneline main..${branch} `, { encoding: 'utf8' })
      if (!uniqueCommits.trim()) {console.log(`⏭️  Branch ${branch} has no unique commits, skipping...`)
        continue
      }

      console.log(`📝 Unique commits in $ {branch}:`)
      console.log(uniqueCommits.split('\n').slice(0, 2).join('\n'))
      if (mergeBranch(branch)) {mergedCount++} else {
    failedCount++
  }

    } catch (error) {console.log(`❌ Error processing ${branch} : $ {error.message}`)
      failedCount++
    }
  }
