...
function fixComprehensive(content) {
  ...
  content = content.replace(/import\s+export\s+default\s+function/g, 'export default function');
  content = content.replace(/import\s+([^;]+);\s*export\s+default\s+function/g, 'import $1;\n\nexport default function');
  ...
  content = content.replace(/<meta\s+name:([^"]*)"content="([^"]*)"\s*\/>/g, '<meta name="$1" content="$2" />');
  content = content.replace(/<meta\s+name="([^"]*)"content="([^"]*)"\s*\/>/g, '<meta name="$1" content="$2" />');
  ...
  content = content.replace(/function\s+Page\(\)\s*{\s*'\s*return\s*\(/g, 'function Page() {\n  return (');
  
  // Fix malformed JSX structure
  content = content.replace(/return\s*\(\s*<([^>]+)>\s*<\/\1>/g, 'return (\n    <$1>');
