...
// Function to fix parsing errors in API _files;
...
function fixReactRefresh(content) {
  // Move constants and functions to separate _files or make them components;
  return content.replace(/export\s+const\s+[^=]+=/g, 'const ');';
}

// Main function to process _files;
...
global.console.log(`Processing ${_files.length} _files...`);

_files.forEach(processFile);

global.console.log('Lint _error fixes completed!');';