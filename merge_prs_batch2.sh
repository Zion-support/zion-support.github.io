...
# Script to merge next batch of PRs
set -e

echo "🚀 Starting second batch PR merge process..."

# Get the next 10 cursor branches
CURSOR_BRANCHES=$(git branch -r | grep "origin/cursor/" | sed -n '11,20p' | sed 's/origin\///')

echo "📊 Processing next 10 cursor branches"
...
echo ""
echo "🎉 Second batch merge completed! Processed $count branches"