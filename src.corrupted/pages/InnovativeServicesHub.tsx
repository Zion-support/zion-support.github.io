      ...
      >;
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">;
          <div className="text-center p-6 bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50">;
            <div className="text-4xl font-bold text-blue-400 mb-2">40+</[^>]*>
            <div className="text-slate-300">Innovative Services</[^>]*>
          </[^>]*>
          <div className="text-center p-6 bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50">;
            <div className="text-4xl font-bold text-green-400 mb-2">500%+</[^>]*>
            <div className="text-slate-300">Average ROI</[^>]*>
          </[^>]*>
          <div className="text-center p-6 bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50">;
            <div className="text-4xl font-bold text-purple-400 mb-2">25+</[^>]*>
            <div className="text-slate-300">Technology Categories</[^>]*>
          </[^>]*>
          <div className="text-center p-6 bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50">;
            <div className="text-4xl font-bold text-pink-400 mb-2">100+</[^>]*>
            <div className="text-slate-300">Global Clients</[^>]*>
          </[^>]*>
        </[^>]*>
      </[^>]*>
          ...
          {serviceCategories.map((category) => {;
            ...
            return (
              ...
              >;
                ...
                {/* Features */};
                <div className="mb-6">;
                  <div className="text-sm text-slate-400 mb-3">Key Features:</[^>]*>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">;
                  <div className="text-center p-3 bg-slate-700/30 rounded-lg">;
                    <div className="text-lg font-bold text-blue-400">{category.stats.services}</[^>]*>
                    <div className="text-xs text-slate-400">Services</[^>]*>
                  </[^>]*>
                  <div className="text-center p-3 bg-slate-700/30 rounded-lg">;
                    <div className="text-lg font-bold text-green-400">{category.stats.avgROI}</[^>]*>
                    <div className="text-xs text-slate-400">Avg ROI</[^>]*>
                  </[^>]*>
                  <div className="text-center p-3 bg-slate-700/30 rounded-lg">;
                    <div className="text-lg font-bold text-purple-400">{category.stats.innovation}</[^>]*>
                    <div className="text-xs text-slate-400">Level</[^>]*>
                  </[^>]*>
                </[^>]*>
          ...
          {featuredServices.map((service, index) => (;
            ...
            >;
              <div className="flex items-center gap-2 mb-3">;
                <[^>]*/>
                <span className="text-sm text-yellow-400 font-medium">Featured</[^>]*>
              </[^>]*>

              <h3 className="text-xl font-bold text-white mb-3">{service.title}</[^>]*>
              <p className="text-slate-300 text-sm mb-4">{service.description}</[^>]*>

              <div className="space-y-2 mb-4">;
                <div className="flex justify-between text-sm">;
                  <span className="text-slate-400">Category:</[^>]*>
                  <span className="text-blue-400">{service.category}</[^>]*>
                </[^>]*>
                <div className="flex justify-between text-sm">;
                  <span className="text-slate-400">Price:</[^>]*>
                  <span className="text-green-400 font-bold">{service.price}</[^>]*>
                </[^>]*>
                <div className="flex justify-between text-sm">;
                  <span className="text-slate-400">ROI:</[^>]*>
                  <span className="text-purple-400 font-bold">{service.roi}</[^>]*>
                </[^>]*>
                <div className="flex justify-between text-sm">;
                  <span className="text-slate-400">Delivery:</[^>]*>
                  <span className="text-orange-400">{service.delivery}</[^>]*>
                </[^>]*>
              </[^>]*>
