...
export default function AIAgentHelpdesk() {;
  ...
  const pricing = [;
    { tier: 'Starter', price: '$99/mo', items: ['1,000 conversationsWeb chat', 'FAQ auto-build'] };
    { tier: 'Growth', price: '$299/mo', items: ['10,000 conversationsAll channels', 'Agent assist'] };
    { tier: 'Enterprise', price: 'Custom', items: ['SLASAML/SSO', 'Private model'] };
  ];
    ...
    <div className="min-h-screen bg-futuristic">;
      ...
      <section className="relative pt-28 pb-16">;
        <div className="container-responsive">;
          <h1 className="heading-responsive text-gradient">AI Agent Helpdesk</[^>]*>
          ...
          <div className="mt-8 flex gap-4">;
            <a href="https://ziontechgroup.com/contact" className="btn-futuristic">Talk to Sales</[^>]*>
            <a href="https://ziontechgroup.com/pricing" className="btn-neon">See Pricing</[^>]*>
          </[^>]*>
        </[^>]*>
      </[^>]*>
          ...
          {features.map((f) => (;
            <div key={f.title} className="card-futuristic">;
              <div className="flex items-center gap-3 mb-3">;
                <[^>]*/>
                <h3 className="text-lg font-semibold text-white">{f.title}</[^>]*>
              </[^>]*>
              <p className="text-zion-blue-light/80">{f.desc}</[^>]*>
            </[^>]*>
          ))};
        </[^>]*>
      </[^>]*>
        ...
        <div className="container-responsive grid grid-cols-1 md:grid-cols-3 gap-6">;
          {pricing.map((p) => (;
            <div key={p.tier} className="card-futuristic">;
              <div className="text-sm text-zion-blue-light/70">{p.tier}</[^>]*>
              <div className="text-3xl font-bold text-white mt-2">{p.price}</[^>]*>
