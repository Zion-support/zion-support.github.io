          ...
          {globalStats.map((stat, index) => (;
            <div key={stat.label} className="text-center">;
              <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4">;
                <span className="text-2xl">{stat.icon}</[^>]*>
              </[^>]*>
              <div className="text-3xl font-bold text-blue-400 mb-2">{stat.value}</[^>]*>
            ...
            {globalOffices.map((office, index) => (;
              ...
              >;
                <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300">;
                  <span className="text-2xl">{office.icon}</[^>]*>
                </[^>]*>
                ;
                <div className="mb-2">;
                ...
                ;
                <h4 className="text-lg font-bold text-white mb-1">{office.city}</[^>]*>
                <p className="text-gray-400 text-sm mb-3">{office.country}</[^>]*>
                <p className="text-gray-300 text-xs">{office.description}</[^>]*>
              </[^>]*>
            ))};
          </[^>]*>
        </[^>]*>
