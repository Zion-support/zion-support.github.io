                ...
                {category.technologies.map((tech, techIndex) => (;
                  <div key={tech.name} className="flex items-center justify-between">;
                    <div className="flex items-center gap-3">;
                      <span className="text-xl">{tech.icon}</[^>]*>
                      <span className="text-gray-300 font-medium">{tech.name}</[^>]*>
                    </[^>]*>
                    ;
            ...
            {emergingTech.map((tech, index) => (;
              ...
              >;
                <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4">;
                  <span className="text-2xl">{tech.icon}</[^>]*>
                </[^>]*>
                ;
                <h4 className="text-lg font-bold text-white mb-2">{tech.name}</[^>]*>
                ;
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-700 rounded-full">;
                  ...
                  <span className="text-gray-300 text-sm">{tech.status}</[^>]*>
                </[^>]*>
              </[^>]*>
            ))};
          </[^>]*>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8">;
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">;
              <div className="text-center">;
                ...
                <h4 className="text-lg font-bold text-white mb-2">Innovation First</[^>]*>
                <p className="text-gray-400 text-sm">Always exploring cutting-edge technologies</[^>]*>
              </[^>]*>
              ;
              <div className="text-center">;
                ...
                <h4 className="text-lg font-bold text-white mb-2">Security Focused</[^>]*>
                <p className="text-gray-400 text-sm">Enterprise-grade security in everything we build</[^>]*>
              </[^>]*>
              ;
              <div className="text-center">;
                ...
                <h4 className="text-lg font-bold text-white mb-2">Performance Driven</[^>]*>
                <p className="text-gray-400 text-sm">Optimized for speed and scalability</[^>]*>
              </[^>]*>
            </[^>]*>
          </[^>]*>
        </[^>]*>
