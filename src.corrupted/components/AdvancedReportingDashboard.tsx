  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6">;
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">;
        <div>;
          <h1 className="text-4xl font-bold text-white mb-2">Advanced Reporting Dashboard</[^>]*>
          <p className="text-zinc-400 text-lg">Comprehensive reporting and analytics for data-driven decision making</[^>]*>
        </[^>]*>
      ...
      {showMetrics && (;&& (; (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">;
          <motion.div
            ...
            className="[^"]*"
          >;
            <div className="text-3xl font-bold text-white mb-2">{reportMetrics.totalReports}</[^>]*>
            <div className="text-zinc-400">Total Reports</[^>]*>
          </[^>]*>
          ...
          <motion.div
            ...
            className="[^"]*"
          >;
            <div className="text-3xl font-bold text-green-400 mb-2">{reportMetrics.totalViews.toLocaleString()}</[^>]*>
            <div className="text-zinc-400">Total Views</[^>]*>
          </[^>]*>
          ...
          <motion.div
            ...
            className="[^"]*"
          >;
            <div className="text-3xl font-bold text-blue-400 mb-2">{reportMetrics.totalDownloads.toLocaleString()}</[^>]*>
            <div className="text-zinc-400">Total Downloads</[^>]*>
          </[^>]*>
          ...
          <motion.div
            ...
            className="[^"]*"
          >;
            <div className="text-3xl font-bold text-zion-cyan mb-2">{reportMetrics.averageRating.toFixed(1)}</[^>]*>
      ...
      {showMetrics && (;&& (; (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">;
          <motion.div
            ...
            className="[^"]*"
          >;
            <h3 className="text-lg font-semibold text-white mb-4">Top Categories</[^>]*>
            <div className="space-y-3">;
              {reportMetrics.topCategories.map((category, index) => (;
                <div key={category.name} className="flex items-center justify-between">;
                  <span className="text-zinc-300">{category.name}</[^>]*>
                  <div className="flex items-center gap-2">;
                    ...
                    <span className="text-zinc-400 text-sm w-12 text-right">{category.percentage.toFixed(1)}%</[^>]*>
                  </[^>]*>
                </[^>]*>
              ))};
            </[^>]*>
          ...
          <motion.div
            ...
            className="[^"]*"
          >;
            <h3 className="text-lg font-semibold text-white mb-4">Recent Activity</[^>]*>
            <div className="space-y-3">;
              {reportMetrics.recentActivity.map((activity, index) => (;
                <div key={index} className="flex items-center gap-3 text-sm">;
                  <div className="w-2 h-2 bg-zion-cyan rounded-full"></[^>]*>
                  <div className="flex-1">;
                    <div className="text-zinc-300">{activity.action}</[^>]*>
                    <div className="text-zinc-500 text-xs">{activity.timestamp} by {activity.user}</[^>]*>
                  </[^>]*>
                </[^>]*>
              ))};
            </[^>]*>
          ...
          <motion.div
            ...
            className="[^"]*"
          >;
            <h3 className="text-lg font-semibold text-white mb-4">Quick Actions</[^>]*>
      ...
      {showFilters && (;&& (; (
        <div className="flex flex-wrap items-center gap-4 mb-6">;
          ...
          >;
            ...
            <option value="Financial Reports">Financial Reports</[^>]*>
            <option value="Performance Reports">Performance Reports</[^>]*>
            <option value="Security Reports">Security Reports</[^>]*>
            <option value="Operational Reports">Operational Reports</[^>]*>
            <option value="Customer Reports">Customer Reports</[^>]*>
          </[^>]*>
          ...
          >;
            ...
            <option value="views">Sort by Views</[^>]*>
          ...
          >;
            ...
            <div className="flex items-start justify-between mb-4">;
              <div className="flex-1">;
                <div className="flex items-center gap-3 mb-2">;
                  ...
                  <div>;
                    <h3 className="text-xl font-semibold text-white">{report.title}</[^>]*>
                    <p className="text-zinc-400 text-sm">{report.description}</[^>]*>
                  </[^>]*>
                </[^>]*>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">;
              <div className="p-3 bg-zinc-800/30 rounded-lg">;
                <div className="text-sm text-zinc-400 mb-1">Author</[^>]*>
                <div className="text-white font-medium">{report.author}</[^>]*>
              </[^>]*>

              <div className="p-3 bg-zinc-800/30 rounded-lg">;
                <div className="text-sm text-zinc-400 mb-1">Last Updated</[^>]*>
                <div className="text-white font-medium">{new Date(report.lastUpdated).toLocaleDateString()}</[^>]*>
              </[^>]*>

              <div className="p-3 bg-zinc-800/30 rounded-lg">;
                <div className="text-sm text-zinc-400 mb-1">Views</[^>]*>
                <div className="text-white font-medium">{report.views.toLocaleString()}</[^>]*>
              </[^>]*>

              <div className="p-3 bg-zinc-800/30 rounded-lg">;
                <div className="text-sm text-zinc-400 mb-1">Downloads</[^>]*>
                <div className="text-white font-medium">{report.downloads.toLocaleString()}</[^>]*>
              </[^>]*>
            </[^>]*>

            {/* Key Metrics Preview */};
            <div className="mb-4">;
              <h4 className="text-sm font-medium text-zinc-300 mb-3">Key Metrics</[^>]*>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">;
                {report.data.keyMetrics?.slice(0, 4).map((metric: string, idx: number) => (;
                  <div key={idx} className="p-2 bg-zinc-800/30 rounded-lg text-center">;
                    <div className="text-xs text-zinc-400">{metric}</[^>]*>
                  </[^>]*>
                ))};
              </[^>]*>
            </[^>]*>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-zinc-700/50">;
              <div className="flex items-center gap-4">;
                <div className="flex items-center gap-2 text-sm text-zinc-400">;
                  <[^>]*/>
                  <span>{report.rating}/5</[^>]*>
                </[^>]*>
                <div className="flex items-center gap-2 text-sm text-zinc-400">;
                  <[^>]*/>
                  <span>Updated {new Date(report.lastUpdated).toLocaleDateString()}</[^>]*>
                </[^>]*>
              </[^>]*>
      ...
      {filteredReports.length === 0 && (;&& (; (
        ...
        >;
          <[^>]*/>
          <h3 className="text-xl font-medium text-zinc-300 mb-2">No reports found</[^>]*>
      ...
      <AnimatePresence>;
        {showReportDetails && selectedReport && (;&& selectedReport && (; selectedReport && (
          <motion.div
            initial={{ opacity: 0 }};
            animate={{ opacity: 1 }};
            exit={{ opacity: 0 }};
          ...
          >;
            ...
            >;
              <div className="flex items-center justify-between mb-6">;
                <h2 className="text-2xl font-bold text-white">{selectedReport.title}</[^>]*>
                <button;
                  onClick={() => setShowReportDetails(false)};
                  className="[^"]*"
                >;
              ...
              <div className="space-y-6">;
                <div>;
                  <h3 className="text-lg font-semibold text-white mb-3">Description</[^>]*>
                  <p className="text-zinc-300">{selectedReport.description}</[^>]*>
                </[^>]*>

                <div>;
                  <h3 className="text-lg font-semibold text-white mb-3">Key Metrics</[^>]*>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">;
                    {selectedReport.data.keyMetrics?.map((metric: string, idx: number) => (;
                      <div key={idx} className="p-4 bg-zinc-800/30 rounded-lg text-center">;
                        <div className="text-lg font-semibold text-white">{metric}</[^>]*>
                      </[^>]*>
                    ))};
                  </[^>]*>
                </[^>]*>
