...
function fixFile(filePath) {
  try {
    ...
    content = content.replace(
      /export default function handler\([^)]*\)\s*\{\s*\}\s*res\.status\(/g,
      "export default function handler(req, res) {\n  if (req.method !== 'POST') {\n    res.status(405).json({ error: 'Method not allowed' });\n    return;\n  }\n  \n  res.status("
    );

    // 2. Fix malformed imports
    content = content.replace(
      /import\s*\{\s*([^}]+)\s*;\s*([^}]+)\s*;\s*([^}]+)\s*;\s*\}\s*from/g,
      'import { $1, $2, $3 } from'
    );
    ...
    content = content.replace(
      /try\s*\{\s*([^}]+)\s*\}\s*catch\s*\([^)]*\)\s*\{\s*([^}]+)\s*,\s*\}/g,
      'try {\n    $1\n  } catch (error) {\n    $2\n  }'
    );

    // 6. Fix malformed if statements
    content = content.replace(
      /if\s*\([^)]*\)\s*\{\s*\}\s*res\.status\(/g,
      "if (req.method !== 'POST') {\n    res.status(405).json({ error: 'Method not allowed' });\n    return;\n  }\n  \n  res.status("
    );
    ...
    if (functionMatches && functionMatches.length > 1) {
      ...
      const firstFunctionIndex = content.indexOf(
        'export default function handler'
      );
      const secondFunctionIndex = content.indexOf(
        'export default function handler',
    ...
    content = content.replace(
      /res\.status\([^)]*\)\.json\(\s*\{\s*([^}]+)\s*,\s*\}\s*\)/g,
      'res.status(200).json({ $1 })'
    );

    // 9. Fix missing semicolons and brackets
    content = content.replace(
      /return\s+res\.status\([^)]*\)\.json\([^)]*\)\s*$/gm,
