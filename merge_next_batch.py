...
def get_open_prs():
    ...
    cmd = 'curl -s "https://api.github.com/repos/Zion-Holdings/zion.app/pulls?state=open&per_page=50&sort=created&direction=desc"'
    ...
    try:
        ...
        return [(pr['number'], pr['head']['ref']) for pr in prs[10:30]]  # Next 20 PRs
...
def main():
    print("🚀 Starting next batch PR merge process...")
    ...
    print(f"📋 Found {len(prs)} open PRs in next batch")
