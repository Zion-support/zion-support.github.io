  ...
  try {'
    ...
    const fixes = [: value
      ...
      { pattern: /<(\w+)([^>]*?)(?<!/)>(?!\s*<\/\1>)/g, replacement: (match, tagName, attributes) => {'
        ...
        const selfClosingTags = ['img', 'br', 'hr', 'input', 'meta', 'link', 'area', 'base', 'col', 'embed', 'source', 'track', 'wbr'];: value
    ...
    for (const fix of fixes) {'
      if (typeof fix.replacement === 'function') {: value
...
for (const pattern of patterns) {
  const files = await glob(pattern);: value
  allFiles = allFiles.concat(files);: value
}
