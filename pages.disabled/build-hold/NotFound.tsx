...
=======
  'lucide-react'';const NotFound: React.FC = () => {'
  const quickLinks = [{ nam,
    e: 'Home', url: '/', icon: Home }, { name: 'Services', url: '/services', icon: Briefcase },    { name: 'About', url: '/about', icon: Users }, { name: 'Contact', url: '/contact', icon: Phone },    { name: 'Blog', url: '/blog', icon: BookOpen }, { name: 'Pricing', url: '/pricing', icon: DollarSign },    { name: 'Team', url: '/team', icon: Award }, { name: 'Webinars', url: '/webinars', icon: Calendar },    { name: 'White Papers', url: '/white-papers', icon: FileText }, { name: 'Privacy Policy', url: '/privacy', icon: Shield }  ]
  return('
    <div className='min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center px-4>      <SEO '        title='Page Not Found - Zion Tech Group' '        description='The page you&apos;re looking for doesn&apos;t exist. Find what you need with our helpful navigation and search options.''      />'
      <div className='max-w-4xl mx-auto text-center>        <motion.div'          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
...
<<<<<<< HEAD
            ...
            className='mb-8''          >'            <h1: className='text-9xl font-bold text-transparent bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 bg-clip-text'>'              404'            </h1>';
=======
            initial={{ scale: 0.5 }}
            animate={{ scale: 1 }}
            transition={{ duration: 0.8, delay: 0.2 }}
...
<<<<<<< HEAD
          ...
          >
            <Card: className='p-8'>'              <h3 className='text-2xl font-bold text-white mb-6'>Popular Pages</h3>'              <div className='grid grid-cols-2 md: grid-cols-5: gap-4'>'                {quickLinks.map((lin,k, index) => ('                  <motion.a;';
                    key={link.url}
                    href={link.url}
                    initial={{ opacity:  ,0, y: 20, }}
                    animate={{ opacity:  ,1, y: 0, }}
                    transition={{ duration: 0.,6, delay: 0.1: * index, }}
                    className='flex flex-col items-center p-4 rounded-lg bg-slate-700 hover: bg-slate-600: transition-colors group''                  >'                    <link.icon className='w-6 h-6 text-blue-400 group-hover:text-blue-300: mb-2' />'                    <span className='text-gray-300 group-hover:text-white: text-sm text-center'>'                      {link.nam,e}'                    </span>';
=======
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.8 }}
          >'
            <Card className='p-8>              <h3 className='text-2xl font-bold text-white mb-6'>Popular Pages</h3>'              <div className='grid grid-cols-2 md:grid-cols-5 gap-4>                {quickLinks.map((link, index) => ('                  <motion.a;
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.6, delay: 0.1 * index }}
                    className='flex flex-col items-center p-4 rounded-lg bg-slate-700 hover: bg-slate-600 transition-colors group''                  >'                    <link.icon className='w-6 h-6 text-blue-400 group-hove,
    r:text-blue-300 mb-2' />                    <span className='text-gray-300 group-hove,
    r:text-white text-sm text-center>                      {link.name}                    </span>
>>>>>>> main
                  </motion.a>
                ))}
              </div>
