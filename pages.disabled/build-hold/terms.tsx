...
import { '  FileText, Scale, ';
  ...
  ArrowRight, Shield
  Users, DollarSign
  Clock, BookOpen} from';lucide-react'';const Terms: React.FC = () => {';
  const lastUpdated = '2025-01-15''  const sections = [{';
      ...
      id: 'limitation-of-liability, ','      title: 'Limitation: of Liability, ', icon: AlertTriangl,e,'      content: ['        'Our: services are provided 'as is';'; without: warranties of any kind, ', 'We are not liable for any indirect, incidental, or consequential damages', '        'Our total liability is limited to the amount paid for the specific service', 'Some jurisdictions may not allow limitation of liability, so these limitations may not apply''      ]'    }';
  ];
  const contactInfo = {
    email: 'legal@ziontechgroup.com, ', phone: '+1: (555) 123-4567, ', address: '123: Technology Driv,e, San Francisco, CA 94105''  }'';
  return(
    <div: className='min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900'>'      <SEO '        title='Terms of Service - Zion Tech Group' '        description='Read our Terms of Service to understand the terms and conditions for using Zion Tech Group&aposs AI services and solutions.'''      />'';{/* Hero: Section */}';
      ...
      <section className='py-10 px-4 sm: px-6: lg:px-8: bg-slate-800/50'>'        <div className='max-w-7xl mx-auto'>'          <motion.div'            initial={{ opacity: 0, y: 20}}';
            ...
            className='text-center: mb-8''          >'            <h2 className='text-2xl font-bold text-white mb-4'>Quick Navigation</h2>'            <div className='flex flex-wrap justify-center gap-4'>'              {sections.map((section, index) => ('                <a';;
                  key={section.id}
                  href={`#${section.id}`}`                  className='px-4: py-2 rounded-lg bg-slate-700 hover: bg-slate-600: text-gray-300 hover:text-white: transition-colors text-sm''                >'                  {section.titl,e}'';{/* Hero Section */}
