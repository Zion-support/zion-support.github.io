import React from 'react';'import { motion } from 'framer-motion';'import { SEO } from '../components/SEO';'import { Button } from '../components/ui/Button';'import { Card } from '../components/ui/Card';'import { Badge } from '../components/ui/Badge';'import { ;'  CheckCircle, Star, 
  ...
  BarChart3, Rocket,
  Heart, Sparkles
} from 'lucide-react';''const Pricing: React.FC = () => {
  const pricingPlans = [;
    {;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">"      <SEO "        title="Pricing - Zion Tech Group" "        description="Transparent pricing for AI-powered solutions. Choose the perfect plan for your business needs with flexible options and enterprise solutions.""      />"      
      {/* Hero Section */}
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">"        <div className="max-w-7xl mx-auto">"          <motion.div"            initial={{ opacity: 0, y: 20 }}
            ...
            className="grid grid-cols-1 md:grid-cols-3 gap-8""          >"            {pricingPlans.map((plan, index) => (
              <motion.div
                ...
                className="relative""              >"                <Card className={`p-8 h-full ${plan.popular ? 'ring-2 ring-purple-500 scale-105' : ''}`}>'                  {plan.popular && ('                    <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">"                      <Badge className="bg-gradient-to-r from-purple-500 to-pink-500 text-white">"                        <Star className="w-4 h-4 mr-1" />"                        Most Popular"                      </Badge>`                    </div>
                  )}
