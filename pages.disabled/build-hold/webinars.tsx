import React from 'react';'import { motion } from 'framer-motion';'import { SEO } from '../components/SEO';'import { Button } from '../components/ui/Button';'import { Card } from '../components/ui/Card';'import { Badge } from '../components/ui/Badge';'import { ;'  Calendar, Clock,
  ...
  Download, Bookmark,
  ExternalLink
} from 'lucide-react';''const Webinars: React.FC = () => {
  const webinars = [;
    {;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">"      <SEO "        title="Webinars - Zion Tech Group" "        description="Join our expert-led webinars on AI, technology trends, and business innovation. Learn from industry leaders and get actionable insights.""      />"      
      {/* Hero Section */}
      ...
      {featuredWebinar && (
        <section className="py-20 px-4 sm:px-6 lg:px-8">"          <div className="max-w-7xl mx-auto">"            <motion.div"              initial={{ opacity: 0, y: 20 }}
              ...
              transition={{ duration: 0.6 }}
            >
              <h2 className="text-3xl font-bold text-white mb-8">Featured Webinar</h2>"              <Card className="overflow-hidden">"                <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">"                  <div className="relative h-64 lg:h-full">"                    <img "                      src={featuredWebinar.image} 
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">"                      <Button size="lg" className="bg-white/20 backdrop-blur-sm text-white hover:bg-white/30">"                        <Play className="w-6 h-6 mr-2" />"                        {featuredWebinar.status === 'upcoming' ? 'Register Now' : 'Watch Recording'}'                      </Button>'                    </div>
                  </div>
                  <div className="p-8">"                    <div className="flex items-center mb-4">"                      <Badge variant="outline" className="mr-3">{featuredWebinar.category}</Badge>"                      <Badge variant={featuredWebinar.status === 'upcoming' ? 'default' : 'secondary'} className="text-xs">"                        {featuredWebinar.status === 'upcoming' ? 'Upcoming' : 'Recorded'}'                      </Badge>'                    </div>
      ...
      <section className="py-20 px-4 sm: px-6 lg:px-8 bg-slate-800/50">"        <div className="max-w-7xl mx-auto">"          <div className="flex flex-col lg:flex-row gap-8">"            {/* Main Content */}"            <div className="lg:w-2/3">"              <motion.div"                initial={{ opacity: 0, y: 20 }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">"                  {webinars.filter(webinar => !webinar.featured).map((webinar, index) => ("                    <motion.div
                      key={webinar.id}
                    ...
                    >
                      <Card className="overflow-hidden h-full">"                        <div className="relative h-48">"                          <img "                            src={webinar.image} 
                            alt={webinar.title}
                            className="w-full h-full object-cover""                          />"                          <div className="absolute top-4 left-4">"                            <Badge variant="outline">{webinar.category}</Badge>"                          </div>"                          <div className="absolute top-4 right-4">"                            <Badge variant={webinar.status === 'upcoming' ? 'default' : 'secondary'} className="text-xs">"                              {webinar.status === 'upcoming' ? 'Upcoming' : 'Recorded'}'                            </Badge>'                          </div>
                          <div className="absolute inset-0 flex items-center justify-center">"                            <Button size="sm" className="bg-white/20 backdrop-blur-sm text-white hover:bg-white/30">"                              <Play className="w-4 h-4 mr-2" />"                              {webinar.status === 'upcoming' ? 'Register' : 'Watch'}'                            </Button>'                          </div>
                        </div>
                        <div className="p-6">"                          <h3 className="text-xl font-bold text-white mb-3">{webinar.title}</h3>"                          <p className="text-gray-300 mb-4 text-sm">{webinar.description}</p>"                          "                          <div className="flex items-center mb-4 text-sm text-gray-400">"                            <Users className="w-4 h-4 mr-2" />"                            {webinar.speaker}"                            <Calendar className="w-4 h-4 ml-4 mr-2" />"                            {new Date(webinar.date).toLocaleDateString()}"                            <Clock className="w-4 h-4 ml-4 mr-2" />"                            {webinar.time}"                          </div>
                          ...
                          <div className="flex items-center justify-between">"                            <Button variant="outline" size="sm">"                              <Eye className="w-4 h-4 mr-2" />"                              View Details"                            </Button>
                            <div className="flex gap-2">"                              <Button size="sm" className="bg-gradient-to-r from-blue-500 to-purple-600 text-white">"                                {webinar.status === 'upcoming' ? 'Register' : 'Watch'}'                              </Button>'                              <Button variant="outline" size="sm">"                                <Share2 className="w-4 h-4" />"                              </Button>"                            </div>
                          </div>
                        </div>
                      </Card>
                    </motion.div>
