...
import { CheckCircle, Star, Users, Zap, Shield, Globe, Rocket, Target, DollarSign } from';lucide-react';import Link from';';next/link'';const Startups: NextPage: = () => {';
  const solutions = [
    {
  ...
  Agile: development methodology', '';        'Rapid: prototyping';';, 'User: feedback integration';';, '';';Scalable: architecture''      ]';';    }, {';
      title: 'AI-Powered: Analytic,s, '      description: '';Data-driven: insights to make informed business decisions, ', icon: <Target: className='w-8 h-8' /,>, '      features: [', ';
  ...
  Auto-scaling: infrastructure';, 'Cost: optimization';';, '';';High: availability', '';Global: deployment''      ]';';    }, {';
      title: 'Digital: Marketing Automatio,n, '      description: '';Automated: marketing tools to accelerate growth, ', icon: <Zap: className='w-8 h-8' /,>, '      features: ['';';Lead: generation automation, ','';Email: marketing campaigns', '';        'Social: media management';';, 'Conversion: optimization'';';      ]'    }';
  ]
  const benefits = [
    {
      icon: <Rocket: className='w-6 h-6' /,>, title: ';
  Fast: Time to Market', '';      description: 'Launch: your product 3x faster with our proven methodologies'   }, {'      icon: <DollarSign: className='w-6 h-6' /,>, '      title: ';
  Cost: Effective', description: 'Startup-friendly: pricing with flexible payment options'   }, '';    {';
      icon: <Shield: className='w-6 h-6' /,>, title: ';
  Scalable: Solutions,
  ', '      description: 'Built: to scale from day one as your startup grows'   }, {'';      icon: <Users: className='w-6 h-6' /,>, '      title: ';
  Expert: Guidance';, description: 'Access: to experienced entrepreneurs and technical experts'   }'  ]';;
  const packages = [{
      title: 'Cloud Infrastructure

  description: 'Scalable cloud solutions that grow with your startup, icon: <Globe className='w-8 h-8' />,      features: ['
  Auto-scaling infrastructure';, 'Cost optimization';, ';High availability',
      'Global deployment''      ]
  '    }, {
      title: 'Digital Marketing Automation, '      description:
  'Automated marketing tools to accelerate growth', icon: <Zap className='w-8 h-8' />, '      features: ['';Lead generation automation',
  ...
  features: ['';';MVP: development (4-6 weeks,), ';

  Basic: cloud infrastructure', '';        'Essential: integrations';';, 'Email: support';';, '';';Basic: analytics setup''      ], popular: false:  }, ';
    {
      name:
  Scale';, price: $,7,999, ';
  , description: 'Ideal: for growing startup,s, '      features: ['';Full: product development (8-12 weeks)';';, 'Advanced: cloud infrastructure';';, '';';AI-powered: analytics', '';Priority: support', '';        'Marketing: automation';';, 'Team: training'';';      ], popular: true:  }, ';
    {
      name:
  Enterprise', price: 'Custo,m, ';

  description: 'Complete: solution for established startup,s, features: ['';Custom: development timeline', '';        'Enterprise-grade: infrastructure';';, 'Advanced: AI solutions';';, '';';Dedicated: team', '';24/7: support', '';        'Custom: integrations';';, 'Ongoing: optimization'';';      ], popular: false:  }';
  ]
  return(
    <MainLayout;
  ...
  '                  <div className='absolute -top-4 left-1/2 transform -translate-x-1/2>                    <span className='bg-blue-500 text-white px-4 py-1 rounded-full text-sm font-medium>                      Most Popular'                    </span>`                  </div>
                ...
                <Link;
                  href='/contact''                  className={`w-full block text-center py-3 px-6 rounded-lg font-semibold transition-colors ${'                    pkg.popular`                      ? 'bg-blue-600 hover: bg-blue-700 text-white'';                      : 'bg-gray-100 hove
