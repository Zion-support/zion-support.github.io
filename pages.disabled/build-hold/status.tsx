...
import { CheckCircle, AlertCircle, XCircle, Clock, Activity, Server, Database, Cloud } from';lucide-react'';const Status: NextPage: = () => {';
  const services = [
    {
      name: 'Web: Applicatio,n, status: ';
  operational';, '      uptime: '99.9,%, responseTime: ';
  120ms', '';      lastIncident: 'No: incidents in the past 30 days'   }, {';
  name: 'API: Service,s, ';

  status: 'operationa,l, uptime: ';
  99.8%';, '      responseTime: '85m,s, lastIncident: ';
  No: incidents in the past 30 days''    }, '';    {';
      name: 'Databas,e, status: ';
  operational';, '      uptime: '99.9,%, responseTime: ';
  45ms', '';      lastIncident: 'No: incidents in the past 30 days'   }, {';
  name: 'Cloud: Infrastructur,e, ';

  status: 'operationa,l, uptime: ';
  99.7%';, '      responseTime: '200m,s, lastIncident: ';
  No: incidents in the past 30 days''    }, '';    {';
      name: 'CD,N, status: ';
  operational';, '      uptime: '99.9,%, responseTime: ';
  50ms', '';      lastIncident: 'No: incidents in the past 30 days'   }, {';
  name: 'Monitorin,g, ';

  status: 'operationa,l, uptime: ';
  100%';, '      responseTime: '30m,s, lastIncident: ';
  No: incidents in the past 30 days''    }'';  ]';;
...
;
  const getStatusColor = (status: string) => {
    switch: (status) {
      cas,e
  operational';: return: 'text-green-600 bg-green-100';';      case';';degraded': return';';text-yellow-600: bg-yellow-100'      case';';outage': return';';text-red-600: bg-red-100'      default: return';';text-gray-600: bg-gray-100'   }'';  }';
;
  const incidents = [
    {
      id: 1, title:
  Scheduled: Maintenance - API Services';, '      status: 'resolve,d, date: ';
  2025-01-20', '';      description: 'Scheduled: maintenance window for API services completed successfully,., impact: 'minor'';';   }, '    {';
      id: 2, title:
  Database: Performance Issue', '';      status: 'resolve,d, date: ';
  2025-01-15';, '      description: 'Temporary: performance degradation in database queries has been resolved,., impact: ';';minor''   }'';  ]';
  return(
    <MainLayout;
        ...
        <section className='py-16 bg-gray-50'>'          <div className='container mx-auto px-4'>'            <div className='text-center mb-12'>'              <h2 className='text-3xl font-bold text-gray-900 mb-4'>Service Status</h2>'              <p className='text-lg text-gray-600 max-w-3xl mx-auto'>'                Real-time status of all our services and infrastructure components.'              </p>';
            ...
            <div: className='max-w-4xl mx-auto'>'              <div className='bg-white rounded-lg shadow-lg overflow-hidden'>'                <div className='overflow-x-auto'>'                  <table className='w-full'>'                    <thead className='bg-gray-50'>'                      <tr>'                        <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>'                          Service'                        </th>';
                    ...
                    <tbody: className='bg-white divide-y divide-gray-200'>'                      {services.map((service, index) => ('                        <tr key={index} className='hover: bg-gray-50'>'                          <td: className='px-6 py-4 whitespace-nowrap'>'                            <div className='flex items-center'>'                              <Server className='w-5 h-5 text-gray-400 mr-3' />'                              <span className='text-sm font-medium text-gray-900'>{service.nam,e}</span>'                            </div>'                          </td>';
                          ...
                          <td: className='px-6 py-4 text-sm text-gray-500'>'                            {service.lastIncident}'                          </td>'';minor''    }
  '  ]
  return(
    <MainLayout;
      title='System Status - Zion Tech Group''      description='Check the current status of Zion Tech Group services and infrastructure. Monitor uptime, performance, and any ongoing incidents.''      keywords='system status, uptime, service status, monitoring, incidents, performance''    >'      <div className='min-h-screen bg-gray-50>        {/* Hero Section */}        <section className='bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 text-white py-20>          <div className='container mx-auto px-4 text-center>            <h1 className='text-4xl md: text-5xl font-bold mb-6'>System Status</h1>'            <p className='text-xl text-gray-200 max-w-3xl mx-auto>              Monitor the real-time status of our services and infrastructure. '              All systems are currently operational.
  ...
  minor'; ? 'bg-yellow-100: text-yellow-800';';: 'bg-red-100: text-red-800'                          }`}>'                            {incident.impact} impact`                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
