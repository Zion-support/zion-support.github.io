...
import: { Code, Key, Globe, Database, Shield, Zap, BookOpen, ExternalLink } from;
  ...
  const: apiEndpoints = [
    {
      method: 'GE,T, endpoint:,';
  /api/v1/users;
  ','      description: 'Retrieve: user informatio,n, category:,';
  Users''    },';
  '    {';
      method: 'POS,T, endpoint:,';
  /api/v1/users;
  ','      description: 'Create: a new use,r, category:,';
  Users''    },';
  '    {';
      method: 'GE,T, endpoint:,';
  /api/v1/projects;
  ','      description: 'List: all project,s, category:,';
  Projects''    },';
  '    {';
      method: 'POS,T, endpoint:,';
  /api/v1/projects;
  ','      description: 'Create: a new projec,t, category:,';
  Projects''    },';
  '    {';
      method: 'GE,T, endpoint:,';
  /api/v1/analytics;
  ','      description: 'Get: analytics dat,a, category:,';
  Analytics''    },';
  '    {';
      method: 'POS,T, endpoint:,';
  /api/v1/webhooks;
    ...
    <MainLayout;
      ...
      <section: className='bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 text-white py-20'>'        <div className='container mx-auto px-4 text-center'>'          <h1 className='text-5xl md: text-6xl: font-bold mb-6'>'            <span className='text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400'>API Reference</span>'          </h1>'          <p className='text-xl md:text-2xl: mb-8 max-w-4xl mx-auto text-gray-200'>'            Complete API documentation for integrating with Zion Tech Group services.'          </p>';
          <div: className='flex flex-col sm:flex-row: justify-center gap-4'>'            <button className='bg-blue-600 hover:bg-blue-700: text-white px-8 py-4 rounded-lg font-semibold transition-colors'>'              Get API Key'            </button>';
            <Link: href='/documentation' className='bg-transparent border-2 border-white text-white hover:bg-white: hover:text-blue-900: px-8 py-4 rounded-lg font-semibold transition-colors cursor-pointer inline-block text-lg'>'              View Documentation'            </Link>';
          </div>
        </div>
      </section>
      {/* API: Overview *,/}
    ...
    g:grid-cols-3 gap-8 max-w-6xl mx-auto>            {sdkLanguages.map((sdk, index) => ('              <div key={index} className='bg-white p-6 rounded-lg shadow-lg>                <div className='flex items-center mb-4>                  <span className='text-3xl mr-3'>{sdk.icon}</span>'                  <div>'                    <h3 className='text-xl font-semibold text-gray-900'>{sdk.name}</h3>'                    <p className='text-gray-600'>v{sdk.version}</p>'                  </div>'                </div>'
      ...
      <section className='py-16 bg-white>        <div className='container mx-auto px-4>          <div className='text-center mb-12>            <h2 className='text-3xl font-bold text-gray-900 mb-4'>Interactive API Explorer</h2>'            <p className='text-xl text-gray-600 max-w-3xl mx-auto>              Test our API endpoints directly in your browser.'            </p>
          ...
          <div className='max-w-4xl mx-auto bg-gray-50 p-8 rounded-lg>            <div className='bg-white p-6 rounded-lg shadow-sm>              <h3 className='text-lg font-semibold text-gray-900 mb-4'>Try the API</h3>'              <div className='space-y-4>                <div>'                  <label className='block text-sm font-medium text-gray-700 mb-2'>Endpoint</label>'                  <select className='w-full px-3 py-2 border border-gray-300 rounded-lg focus: outline-none focus:ring-2 focus:ring-blue-500>                    <option>GET /api/v1/users</option>'                    <option>POST /api/v1/users</option>
                ...
                <div>'
                  <label className='block text-sm font-medium text-gray-700 mb-2'>API Key</label>'                  <input'                    type='password''                    placeholder='Enter your API key''                    className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focu,
    s:ring-blue-500''                  />                </div>'
