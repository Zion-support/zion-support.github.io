...
:pages.disabled/pages/services/AISalesCopilot.tsx{features.map((feature, index()
              <motion.div;
                ...
                viewport={{ once: true }}'';
                className='group relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all duration-300 hover:border-blue-500/50'>'{features.map((feature, index(
              <motion.div
                key={feature.title}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
