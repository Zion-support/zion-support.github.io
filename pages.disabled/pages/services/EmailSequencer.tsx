...
:pages.disabled/pages/services/EmailSequencer.tsx
					...
					<h1 className="text-4xl md:text-5xl font-bold text-white">Email Sequencer</h1>"					<p className="mt-4 text-lg text-zion-slate-light max-w-2xl mx-auto">						Automate multi-step outreach with AI-assisted copy, throttling, and deep CRM/webhook integrations.
					</p>
:pages.disabled/pages/services/EmailSequencer.tsx
				</div>"
""
...
""
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
					{features.map((f) => (""
						<div key={f} className="p-6 rounded-xl border border-white/10 bg-white/5 backdrop-blur-lg">""							<Check className="w-5 h-5 text-zion-cyan mb-3"  />"							<p className="text-white text-sm">{f}</p>
						</div>
					))}
:pages.disabled/pages/services/EmailSequencer.tsx
				</div>"
...
""
				<div className="mb-16">""
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
						{plans.map((p) => (""
							<div key={p.name} className="p-6 rounded-2xl border border-white/10 bg-white/5">""								<div className="flex items-center justify-between mb-2">""
...
:pages.disabled/pages/services/EmailSequencer.tsx
								...
								<button className="mt-4 w-full py-2 rounded-lg bg-zion-cyan/20 text-white hover:bg-zion-cyan/30 transition">Start Trial</button>							</div>
						))}
					</div>
				</div>
:pages.disabled/pages/services/EmailSequencer.tsx
"
				...
				<div>""
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{references.map((ref) => (""
							<a key={ref.href} href={ref.href} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition">""								<LinkIcon className="w-4 h-4 text-zion-cyan mr-2"  />"								<span className="text-white text-sm">{ref.label}</span>;
							</a>;
						))};
					</div>;
				</div>;
