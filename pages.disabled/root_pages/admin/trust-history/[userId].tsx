...
const AdminUserTrustHistoryPage: React.FC = () => {
  ...
  const [currentScore, setCurrentScore] = useState<CurrentTrustScore | null>(
    null
  );
  ...
  useEffect(() => {
    if (!userId) {
      // userId from useParams is already a string or undefined
      setLoading(false);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!userRes.ok) {
          ...
          console.warn(
            'Failed to fetch user details, attempting to proceed:',
            userErrorData.error || userRes.status
          );
        ...
        if (!scoreRes.ok) {
          ...
          if (scoreRes.status !== 404) {
            // Don't throw for 404, just means no score yet
            throw new Error(
              scoreErrorData.error ||
                `Failed to fetch current trust score: ${scoreRes.status}`
            );
          }
        ...
        if (!historyRes.ok) {
          ...
          if (historyRes.status !== 404) {
            // Don't throw for 404
            throw new Error(
              historyErrorData.error ||
                `Failed to fetch trust history: ${historyRes.status}`
            );
          }
          setHistory([]);
        } else {
  ...
  if (loading)
    return (
      <div className="container mx-auto p-4 text-center">
        Loading user trust history...
      </div>
    );
  if (error && !currentScore && history.length === 0)
    return (
    ...
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      <header className="mb-8">
        <Link href="/admin/trust-scores" legacyBehavior>
          <a className="text-blue-600 hover:text-blue-800 hover:underline mb-2 inline-block">
            &larr; Back to All Trust Scores
          </a>
        </Link>
      ...
      <section className="mb-8 p-6 bg-white shadow-md rounded-lg">
        ...
        {currentScore ? (
          <div>
            <p className="text-xl">
              <strong>Score:</strong>{' '}
              <span className="font-bold">{currentScore.score}/100</span>
            </p>
            <p className="mt-2">
              <strong>Operator GPT Analysis:</strong>{' '}
              <em className="text-gray-600">
                {currentScore.operatorGptAnalysis || 'N/A'}
              </em>
            </p>
            <p className="text-sm text-gray-500 mt-1">
              Last Updated: {new Date(currentScore.updatedAt).toLocaleString()}
            </p>
            ...
            {currentScore.components && currentScore.components.length > 0 ? (
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {currentScore.components.map(comp => (
                  ...
                  >
                    ...
                    {comp.value.toFixed(2)} (Weight: {comp.weight * 100}%)
                    {comp.details && (
              ...
              <p className="text-gray-600">
                No active components found for the current score.
              </p>
            )}
          </div>
        ) : (
          <p className="text-gray-600">
            No current trust score data available for this user.
          </p>
        )}
      </section>
      ...
      <section className="p-6 bg-white shadow-md rounded-lg">
        ...
        {history.length > 0 ? (
          ...
          <p className="text-gray-600">
            No trust score history found for this user.
          </p>
        )}
        {error && (
          <p className="text-red-500 mt-4">
            Note: There might be additional errors fetching some data: {error}
          </p>
        )}
      </section>
