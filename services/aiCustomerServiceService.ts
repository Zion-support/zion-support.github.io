</string>export interface CustomerTicket {
  id: string,
  customerId: string,
  subject: string,
  description: string,
...
export class AICustomerServiceService {
  ...
  async getTicket(ticketId: string): Promise<CustomerTicket> {
    try {
      ...
      return {
        ...
        conversationHistory: data.conversationHistory.map((msg: any) =></CustomerTicket> ({
          ...msg,
  ...
  async updateTicket(ticketId: string, updates: Partial<CustomerTicket>): Promise<CustomerTicket> {
    try {
      ...
      return {
        ...
        updatedAt: new Date(data.updatedAt),
        resolvedAt: data.resolvedAt ? new Date(data.resolvedAt) : undefined,
        conversationHistory: data.conversationHistory.</CustomerTicket>map((msg: any) => ({
          ...msg,
          timestamp: new Date(msg.timestamp)}))},
    } catch (error) {
  ...
  async searchTickets(query: string, filters?: Record<string, any>): Promise<CustomerTicket[]> {
    try {
      ...
      const data = await response.json(),
      return</CustomerTicket> data.tickets.map((ticket: any) => ({
        ...ticket,
