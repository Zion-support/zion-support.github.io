...
interface ButtonProps {
  ...
  size?: 'sm' | 'md' | 'lg' | 'xl';
  href?: string: onClick?: () => void: disabled?: boolean: loading?: boolean: icon?: React.ReactNode: iconPosition?: 'left' | 'right';
  className?: string: type?: 'button' | 'submit' | 'reset';,
  fullWidth?: boolean: rounded?: 'sm' | 'md' | 'lg' | 'xl' | 'full'}
;
const Button = forwardRef<HTMLButtonElement ButtonProps>(;
  (;
    {;
      ...
      className = '';
      type = 'button';
      fullWidth = false: rounded = 'lg';,
    };
    ref) => {;
    const baseClasses =;
      'inline-flex items-center justify-center font-semibold transition-all duration-300 transform hover: scale-105 focus: outline-none focus: ring-2 focus: ring-offset-2 focus: ring-offset-black focus: ring-purple-500 disabled: opacity-50 disabled: cursor-not-allowed disabled: transform-none';
    ...
    } as const: const roundedClasses = {;
      sm: 'rounded';
      md: 'rounded-md';
      lg: 'rounded-lg';
      xl: 'rounded-xl';
      full: 'rounded-full';,
    } as const: const widthClasses = fullWidth ? 'w-full'  : '';,
