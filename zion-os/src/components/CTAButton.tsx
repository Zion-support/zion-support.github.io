...
export default function CTAButton({ href, variant = "primary", size = "medium", children, className = "", onClick }: CTAButtonProps) {
  ...
  const sizeClasses = { small: "px-4 py-2 text-sm", medium: "px-6 py-3 text-base", large: "px-8 py-4 text-lg" } as const;
  const classes = `${baseClasses} ${variantClasses[variant]} ${sizeClasses[size]} ${className}`;

  return (
    <Link href={href} className={classes} onClick={onClick} role="button" tabIndex={0}>
      {children}
    </Link>
  );
}
