...
export default function DeploymentNotification({updates;
  onDismiss;}
  onAction;}
}: DeploymentNotificationProps) {const [expanded, setExpanded] = useState<string | null />(null)const getUpdateIcon = (switch (type) {case 'deployment_started': return <Activity className=\"w-5 h-5 text-blue-400\" />;) => {
  return $3;}
}
  ...
  const getUpdateColor = (switch (type) {case 'deployment_started': return 'border-blue-500/30 bg-blue-500/10';
  ...
  const getVerticalIcon = (switch (vertical) {case \"HEALTH\": return <Shield className=\"w-4 h-4 text-blue-400\" />;
  ...
  const getGovernanceIcon = (switch (type) {case \"ADMIN\": return <Users className=\"w-4 h-4 text-yellow-400\" />;
  ...
  const formatTimestamp = (const date = new Date(timestamp)const now = new Date()const diffInMinutes = Math.floor((now.getTime() - date.getTime()) / (1000 * 60))if (diffInMinutes < 1) return 'Just now';) => {
  return $3;}
}
    if (diffInMinutes < 60) return `${diffInMinutes}m ago`;
    if (diffInMinutes < 1440) return `${Math.floor(diffInMinutes / 60)}h ago`;
    return date.toLocaleDateString()}
  const handleAction = (if (onAction) {onAction(updateId, action)) => {
  return $3;}
}},if (updates.length === 0) return null,return (<div className=\"fixed top-4 right-4 z-50 space-y-3 max-w-md\" />;
  ...
  const formatTimestamp = (const date = new Date(timestamp);
    const now = new Date();
  ...
  const handleAction = (if (onAction) {;
...
function DeploymentNotification() {
  ...
  },const getUpdateColor = (type: string) =>: any {switch (type) {case 'deployment_started': return 'border - blue - 500 / 30 bg - blue - 500 / 10',case 'deployment_completed': return 'border - green - 500 / 30 bg - green - 500 / 10',case 'deployment_failed': return 'border - red - 500 / 30 bg - red - 500 / 10',case 'instance_ready': return 'border - purple - 500 / 30 bg - purple - 500 / 10',case 'update_available': return 'border - yellow - 500 / 30 bg - yellow - 500 / 10',default: return 'border - white / 20 bg - white / 5';}
    }
  ...
  },const format_timestamp = (timestamp: string) =>: any {const date = new Date (timestamp),const now = new Date (),const diffInMinutes = Math.floor ((now.get_time () - date.get_time ()) / (1000 * 60)),// Check condition;}
if (return 'Just now', ) {$2;}
}
    // Check condition;
if (return `${diffInMinutes}m ago`, ) {$2;}
}
    if (return `${Math.floor (diffInMinutes / 60)}h ago`, ) {$2;}
}
    return date.toLocaleDateString ()},const handle_action = (update_id: string, action: string) =>: any {// Check condition;}
if ( {) {$2;}
}
         ...
         />;
          ...
          {/* Instance Details (if available) */}{update.instance_name && (<div className=\"mb - 3 p - 3 bg - white / 10 rounded - lg border border - white / 20\" />;
              <div className=\"flex items - center gap - 2 mb - 2\" />;}
                <div className=\"p - 1 bg - white / 20 rounded\" />;}
                  {getVerticalIcon (update.vertical || 'GENERAL')}</div>;
              ...
              <div className=\"flex items - center gap - 3 text - xs text - white / 70\" />;
                ...
                {update.governance_type && (<span className=\"inline - flex items - center gap - 1 px - 2 py - 1 rounded - full bg - blue - 500 / 20 text - blue - 300\" />;}
              ...
              </div>;</div>)}{/* Progress Bar (for deployment updates) */}
          ...
          {update.actions && update.actions.length > 0 && (<div className=\"flex gap - 2 pt - 2 border - t border - white / 20\" />;}
              {update.actions.map ((action, index) => (<button;}
                  key={index}onClick={() = /> handleAction(update.id, action.action)}className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200 ${action.action === 'deploy' || action.action === 'retry';? 'bg-blue-600 hover:bg-blue-700 text-white';className=\"p - 1 text - white / 60 hover:text - white / 80 transition-colors\";
            >;
              <X className=\"w - 4 h - 4\" />;
            </button>;}
          </div>;}
