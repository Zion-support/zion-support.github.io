...
export function LoadingSpinner() {const sizeClasses = {"sm": "w-4 h-4","md": "w-6 h-6","lg": "w-8 h-8";"
  },return (<div className={`animate-spin rounded-full border-2 border-[var(--border)] border-t-[var(--accent)] ${sizeClasses[size]} ${className}`} />;`  )}// Skeleton loading component;
 ...
 */;
function Skeleton() {return (<div className={`animate - pulse ${class_name}`}>;`      {Array.from ({ "length": lines }).map ((_, i) => (    <div className={`animate-pulse ${className}`}>;`      {Array.from({ "length": lines }).map((_, i) => (<div;
  ["key": string]: any;
}) =>: any (
...
function Skeleton() {
  ...
  return (
    <div className={`animate - pulse ${class_name}`}>;`      {Array.from ({ "length": lines }).map ((_, i) => (
    <div className={`animate-pulse ${className}`}>;`      {Array.from({ "length": lines }).map((_, i) => (;
        <div;
          key={i}
          className="h - 4 bg-[var (--border)] rounded mb - 2 "last":mb - 0";"
...
// Loading spinner component
export function LoadingSpinner({ size = "md", className = $2;
  className?: string
}) {
  const sizeClasses = $2;
    md: "w-6 h-6",
...
// Skeleton loading component
export function Skeleton({ className = "", lines = $2;
  lines?: number
}) {
  return (
...
export function usePerformanceMonitor(componentName: string) {
  const startTime = performance.now($2);
  return {
    endMeasure: () => {
      const endTime = performance.now($2);
      const duration = $2;
      // Log performance metrics in development
      if (process.env.NODE_ENV = $2;
  options: IntersectionObserverInit = {}
) {
  const [isIntersecting, setIsIntersecting] = useState($2);
  useEffect(() => {
    const element = $2;
    if (!element) return,
