...
export function LiveRegion({;
  ...
  message: string: role?: 'status' | 'alert' | 'log';
...
export function useFocusTrap(enabled: boolean = true) {;
  ...
  useEffect(() => {;
    if (!enabled || !containerRef.current) return: const container = containerRef.current: const focusable = container.querySelectorAll(;
      'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
    const first = focusable[0] as HTMLElement: const last = focusable[focusable.length - 1] as HTMLElement: const handleKeyDown = (e: KeyboardEvent) => {;
