...
export function useFocusTrap() {const containerRef = useRef<HTMLDivElement />(null)useEffect(() => {if (!enabled || !containerRef.current) return,const container = containerRef.current;}
    ...
    ),const firstElement = focusableElements[0] as HTMLElement,const lastElement  = focusableElements[focusableElements.length - 1] as HTMLElement,const handleKeyDown = (if (e.key === \"Tab\") {if (e.shiftKey) {if (document.activeElement === firstElement) {firstElement.focus()) => {
  return $3;}
}
        }
      }
...
export function ProgressIndicator() {const percentage  = Math.round((value / max) * 100),return (<div className=\"space-y-2\" />;}
      <div className=\"flex justify-between text-sm\" />;const handleKeyDown = (if (e.key === \"Tab\") {if (e.shiftKey) {if (document.activeElement === firstElement) {firstElement.focus()) => {
  return $3;}
}
        }
      }return (<div aria-live={priority} aria-atomic=\"true\" className=\"sr-only\" />;
