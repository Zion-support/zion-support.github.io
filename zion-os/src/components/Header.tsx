...
export default function Header() {_const [isMenuOpen, _setIsMenuOpen] = useState(false);
...
=======
  const _navigation = [
    ...
    {_name: 'AI Solutions', _href: '/ai-solutions'},
  ...
  const _services = [
    {_name: 'AI Autonomous Systems', _href: '/ai-autonomous-systems'},
    ...
    {_name: 'Research Automation', _href: '/ai-autonomous-research-assistant'}];
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13

  return (_<header 
      className={_`sticky top-0 z-50 transition-all duration-300 ${
        isScrolled 
          ...
          <nav className="hidden lg:flex items-center space-x-8" role="navigation" aria-label="Main menu">
            {_navigation.map((item) => (
              <Link
                ...
                href={_item.href}
                className="text-gray-300 hover:text-white transition-all duration-200 font-medium relative group"
                aria-label={_`Navigate to ${item.name} page`}
              >
                {_item.name}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-purple-400 to-blue-400 transition-all duration-300 group-hover:w-full"></span>
              </Link>
                ...
                <div className="p-6 grid grid-cols-1 gap-3">
                  {_services.map(_(service) => (
                    <Link
                      ...
                      href={_service.href}
                      className="text-gray-300 hover:text-white p-3 rounded-lg hover:bg-white/10 transition-all duration-200 group/item"
                      aria-label={_`Navigate to ${service.name} service page`}
                    >
                      <div className="flex items-center justify-between">
...
=======
              <div className="px-3 py-2">
                <div className="text-gray-400 text-sm font-medium mb-2">Services</div>
                {_services.map(_(service) => (_<Link
                    key={service.name}
                    href={_service.href}
                    className="block px-3 py-2 text-gray-300 hover:text-white hover:bg-white/5 rounded-lg transition-all duration-200"
                    onClick={_() => setIsMenuOpen(false)}
