...
;
'use client',import { useState, useEffect } from 'react',import Link from 'next / link',export default /**;'
 * Header - Function description;
 */;
function Header() {const [isMenuOpen, setIsMenuOpen] = useState (false),const [is_scrolled, setIsScrolled] = useState (false),useEffect (() => {const handle_scroll = () =>: any {setIsScrolled (window.scroll_y > 20)},window.addEventListener ('scroll', handle_scroll),return () => window.removeEventListener ('scroll', handle_scroll)}, []),const navigation = [;'
  const navigation = [;
    { "name": 'Home', "href": '/' }'
  ...
  return (;
    <header;
      className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled;`          ? 'bg-black/90 backdrop-blur-xl border-b border-white/20 shadow-2xl';'
          : 'bg-black/60 backdrop-blur-md border-b border-white/10';'
      }`}`    >;
      <div className="max-w-7xl mx-auto px-4 "sm":px-6 "lg":px-8">;"
        <div className="flex justify-between items-center py-4">;"
          {/* Logo */}<div className=&quot;flex items - center & quot;>;
    >
      <div className="max-w-7xl mx-auto px-4 "sm":px-6 "lg":px-8">"
        <div className="flex justify-between items-center py-4">"
...
className=&quot;"lg":hidden p-2 rounded-lg text-gray-400 "hover":text-white "hover":bg-white/10 transition-all duration-200&quot;
          ...
          >;
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">;"
              {isMenuOpen ? (<path strokeLinecap=&quot;round & quot; strokeLinejoin=&quot;round & quot; strokeWidth={2} d=&quot;M6 18L18 6M6 6l12 12 & quot; />) : (<path strokeLinecap=&quot;round & quot; strokeLinejoin=&quot;round & quot; strokeWidth={2} d=&quot;M4 6h16M4 12h16M4 18h16 & quot; />)}
            <Link;
              href="/contact";"
              className="bg-gradient-to-r from-purple-600 to-blue-600 "hover":from-purple-700 "hover":to-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 transform "hover":scale-105 shadow-lg "hover":shadow-xl";"
              aria-label="Get started with Zion Tech Group";"
...
href="/contact""
          ...
          {/* Mobile menu button */}
          <button;
            onClick={() => setIsMenuOpen(!isMenuOpen)}className=""lg":hidden p-2 rounded-lg text-gray-400 "hover":text-white "hover":bg-white/10 transition-all duration-200";"
            aria-expanded={isMenuOpen}
            aria-label={isMenuOpen ? "Close menu" : "Open menu"}"
            aria-controls="mobile-menu";"
          >;
...
key={service.name}
                    ...
                    on_click={() => setIsMenuOpen (false)}
                    aria - label={`Navigate to ${service.name} service page`}>;`                    {service.name}</Link>;
                ))})}>;
                    {service.name}
                <Link;
                  href="/contact";"
