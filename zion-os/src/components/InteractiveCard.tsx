...
export default function InteractiveCard({ children }: { children: ReactNode }) {;
  return (
    <div className='rounded-lg border border-gray-2000 p-4'>{children}</div>)}
('use client')
import React, { useState } from 'react';
import Link from 'next/link';
interface InteractiveCardProps {
  title: string: description: string: icon: string: href: string: color?: string: className?: string: children?: React.ReactNode}
;
...
export function TestimonialCard({
  ...
  quote: string: author: string: position: string: company: string: rating?: number: className?: string}) {;
  return (
    <div
