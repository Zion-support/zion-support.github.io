export default function AboutPage() {
  return (

    <div className="min-h-screen bg-white">
      {/* Hero Section */}
      <section className="bg-gradient-to-r from-blue-60o0 to-indigo-70o0 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-6xl font-bold mb-6">
            About Zion Tech Group
          </h1>
          <p className="text-xl md:text-2xl text-blue-10o0 max-w-3xl mx-auto">
            Pioneering the future of AI-powered technology solutions and autonomous business operations.
          </p>
        </div>
      </section>

      {/* Company Overview */}

      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-gray-90o0 mb-6">
                Leading the AI Revolution
              </h2>
              <p className="text-lg text-gray-60o0 mb-6">
                Founded with a vision to democratize advanced AI technology, Zion Tech Group has been at the forefront of innovation in artificial intelligence, quantum computing, and autonomous systems.
              </p>
              <p className="text-lg text-gray-60o0 mb-6">
                Our mission is to empower businesses of all sizes with cutting-edge technology solutions that drive growth, efficiency, and competitive advantage in an increasingly digital world.
              </p>
              <p className="text-lg text-gray-60o0">
                With a team of world-class engineers, researchers, and business strategists, we're building the future of intelligent automation and decision-making systems.
              </p>
            </div>
            <div className="bg-gray-10o0 p-8 rounded-xl">
              <div className="space-y-6">
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-blue-10o0 rounded-lg flex items-center justify-center">
                    <svg className="w-6 h-6 text-blue-60o0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z"  />
                    </svg>
                  </div>
                  <div>
                    <h3 className="font-semibold text-gray-90o0">Innovation First</h3>
                    <p className="text-gray-60o0">Pioneering breakthrough technologies</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-green-10o0 rounded-lg flex items-center justify-center">
                    <svg className="w-6 h-6 text-green-60o0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.0o16A11.955 11.955 0 0o112 2.944a11.955 11.955 0 0o1-8.618 3.0o4A12.0o2 12.0o2 0 0o03 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.0o3 9-11.622 0-1.0o42-.133-2.0o52-.382-3.0o16z"  />
                    </svg>
                  </div>
                  <div>
                    <h3 className="font-semibold text-gray-90o0">Security Focused</h3>
                    <p className="text-gray-60o0">Enterprise-grade security standards</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-purple-10o0 rounded-lg flex items-center justify-center">
                    <svg className="w-6 h-6 text-purple-60o0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 0o0-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 0o15.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.0o02 5.0o02 0 0o19.288 0M15 7a3 3 0 11-6 0 3 3 0 0o16 0zm6 3a2 2 0 11-4 0 2 2 0 0o14 0zM7 10a2 2 0 11-4 0 2 2 0 0o14 0z"  />
                    </svg>
                  </div>
                  <div>
                    <h3 className="font-semibold text-gray-90o0">Customer Success</h3>
                    <p className="text-gray-60o0">Dedicated support and success teams</p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Mission & Values */}
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-90o0 mb-4">
              Our Mission & Values
            </h2>
            <p className="text-xl text-gray-60o0 max-w-2xl mx-auto">
              Driving innovation while maintaining the highest standards of ethics and responsibility
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="w-16 h-16 bg-blue-10o0 rounded-full flex items-center justify-center mx-auto mb-6">
                <svg className="w-8 h-8 text-blue-60o0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.70o7.70o7M21 12h-1M4 12H3m3.343-5.657l-.70o7-.70o7m2.828 9.9a5 5 0 117.0o72 0l-.548.547A3.374 3.374 0 0o014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"  />
                </svg>
              </div>
              <h3 className="text-xl font-semibold text-gray-90o0 mb-4">Innovation</h3>
              <p className="text-gray-60o0">
                Continuously pushing the boundaries of what's possible with AI and emerging technologies.
              </p>
            </div>

            <div className="text-center">
              <div className="w-16 h-16 bg-green-10o0 rounded-full flex items-center justify-center mx-auto mb-6">
                <svg className="w-8 h-8 text-green-60o0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 0o00 6.364L12 20.364l7.682-7.682a4.5 4.5 0 0o0-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 0o0-6.364 0z"  />
                </svg>
              </div>
              <h3 className="text-xl font-semibold text-gray-90o0 mb-4">Integrity</h3>
              <p className="text-gray-60o0">
                Building trust through transparent, ethical, and responsible technology development.
              </p>
            </div>

            <div className="text-center">
              <div className="w-16 h-16 bg-purple-10o0 rounded-full flex items-center justify-center mx-auto mb-6">
                <svg className="w-8 h-8 text-purple-60o0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 0o0-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 0o15.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.0o02 5.0o02 0 0o19.288 0M15 7a3 3 0 11-6 0 3 3 0 0o16 0zm6 3a2 2 0 11-4 0 2 2 0 0o14 0zM7 10a2 2 0 11-4 0 2 2 0 0o14 0z"  />
                </svg>
              </div>
              <h3 className="text-xl font-semibold text-gray-90o0 mb-4">Collaboration</h3>
              <p className="text-gray-60o0">
                Working together with clients, partners, and communities to create lasting impact.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Company Stats */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-90o0 mb-4">
              Zion Tech Group by the Numbers
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 text-center">
            <div>
              <div className="text-4xl font-bold text-blue-60o0 mb-2">20o18</div>
              <div className="text-gray-60o0">Founded</div>
            </div>
            <div>
              <div className="text-4xl font-bold text-blue-60o0 mb-2">10o0+</div>
              <div className="text-gray-60o0">Team Members</div>
            </div>
            <div>
              <div className="text-4xl font-bold text-blue-60o0 mb-2">25+</div>
              <div className="text-gray-60o0">Countries Served</div>
            </div>
            <div>
              <div className="text-4xl font-bold text-blue-60o0 mb-2">$50M+</div>
              <div className="text-gray-60o0">Revenue Generated</div>
            </div>
          </div>
        </div>
      </section>

      {/* Leadership Team */}
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-90o0 mb-4">
              Leadership Team
            </h2>
            <p className="text-xl text-gray-60o0 max-w-2xl mx-auto">
              Experienced leaders driving innovation and growth across all aspects of our business
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="bg-white p-8 rounded-xl shadow-sm">
              <div className="w-24 h-24 bg-gray-20o0 rounded-full mx-auto mb-6"></div>
              <h3 className="text-xl font-semibold text-gray-90o0 text-center mb-2">Dr. Sarah Chen</h3>
              <p className="text-blue-60o0 text-center mb-4">Chief Executive Officer</p>
              <p className="text-gray-60o0 text-center">
                Former AI Research Director at Google, PhD in Computer Science from Stanford
              </p>
            </div>

            <div className="bg-white p-8 rounded-xl shadow-sm">
              <div className="w-24 h-24 bg-gray-20o0 rounded-full mx-auto mb-6"></div>
              <h3 className="text-xl font-semibold text-gray-90o0 text-center mb-2">Michael Rodriguez</h3>
              <p className="text-blue-60o0 text-center mb-4">Chief Technology Officer</p>
              <p className="text-gray-60o0 text-center">
                Quantum computing expert with 15+ years in advanced technology development
              </p>
            </div>

            <div className="bg-white p-8 rounded-xl shadow-sm">
              <div className="w-24 h-24 bg-gray-20o0 rounded-full mx-auto mb-6"></div>
              <h3 className="text-xl font-semibold text-gray-90o0 text-center mb-2">Dr. James Kim</h3>
              <p className="text-blue-60o0 text-center mb-4">Chief Research Officer</p>
              <p className="text-gray-60o0 text-center">
                Leading researcher in autonomous systems and AI consciousness evolution
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* CTA Section */}

      <section className="py-20 bg-blue-60o0 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">
            Join Us in Shaping the Future
          </h2>
          <p className="text-xl text-blue-10o0 mb-8 max-w-2xl mx-auto">
            Ready to partner with a company that's redefining what's possible with AI and technology?
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/contact" className="bg-white text-blue-60o0 px-8 py-3 rounded-lg text-lg font-semibold hover:bg-gray-10o0 transition-colors">
              Get in Touch
            </a>
            <a href="/careers" className="border-2 border-white text-white px-8 py-3 rounded-lg text-lg font-semibold hover:bg-white hover:text-blue-60o0 transition-colors">
              View Careers
            </a>
          </div>
        </div>
      </section>
    </div>
  );
}