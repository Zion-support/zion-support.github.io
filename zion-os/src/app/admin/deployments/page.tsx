...
import { Rocket, Clock, CheckCircle, AlertCircle, Activity, Pause, StopCircle, Eye, RefreshCw, Globe, MapPin, User, Users, Shield, Building2, Zap, Calendar, Settings } from "lucide-react";
...
export default function DeploymentsPage() {
  ...
  const getStatusCount = (status: DeploymentStatus) => deployments.filter(d => d.status === status).length;
    ...
    <div className="space-y-8">
      ...
      {filteredDeployments.length === 0 && (
        <div className="text-center py-16">
          <div className="w-16 h-16 mx-auto mb-4 p-4 bg-white/10 rounded-full"><Rocket className="w-8 h-8 text-white/40" /></div>
          <h3 className="text-lg font-medium text-white/60 mb-2">No deployments found</h3>
          <p className="text-white/40 mb-6">{filter === 'all' ? 'Start by creating your first Zion ecosystem deployment' : `No deployments with status "${filter}" found`}</p>
          {filter === 'all' && (
            <a href="/admin/os-deploy" className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors duration-200">
