...
import { Rocket, Clock, CheckCircle, AlertCircle, Activity, Pause, RefreshCw, Eye, Settings, Globe, MapPin, StopCircle, Play, Calendar } from "lucide-react";
...
export default function DeploymentsPage() {
	...
	const [filter, setFilter] = useState<"all" | Exclude<DeploymentStatus, "paused">>("all");
	...
	const getStatusCount = (status: Exclude<DeploymentStatus, "paused">) => deployments.filter((d) => d.status === status).length;
					...
					<div key={deployment.id} className="group relative bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:border-white/20 transition-all duration-300 hover:shadow-xl hover:shadow-white/5">
						<div className="p-6 border-b border-white/10">
							<div className="flex items-start justify-between mb-4">
								<div className="flex items-center gap-3">
									<div className="p-2 bg-white/10 rounded-lg">{/* vertical icon placeholder */}</div>
									<div>
