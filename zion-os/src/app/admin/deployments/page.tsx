...
const mockDeployments: Deployment[] = [
  {
    ...
    updatedAt: "2024-01-15T11:45:00Z",
    progress: 100,
    features: ["marketplace", "ziongpt", "kycaml", "daovoting"],
    vertical: "HEALTH",
    governanceType: "DAOFULL",
    domain: "health.zion.network",
    region: "North America",
    country: "United States"
  },
  {
    ...
    updatedAt: "2024-01-15T15:10:00Z",
    progress: 65,
    features: ["academy", "ziongpt", "incubatorgrants"],
    vertical: "EDUCATION",
    governanceType: "DAOLITE",
    subdomain: "edu",
    region: "Europe",
    country: "Germany"
  },
  {
    ...
    updatedAt: "2024-01-15T16:00:00Z",
    progress: 0,
    features: ["marketplace", "onchaincontracts", "web3login"],
    vertical: "LAW",
    governanceType: "DAOFULL",
    domain: "legal.zion.network",
    region: "Asia Pacific",
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-2">
        {_[
          ...
          {key: 'completed', label: 'Completed', count: getStatusCount('completed')},
          {key: 'failed', label: 'Failed', count: getStatusCount('failed')}
        ].map(_({key, label, count}) => (_<button,
key={key}
      ...
      {_/* Deployments Grid */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {filteredDeployments.map(_(deployment) => (
          <div key={deployment.id} className="group relative bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:border-white/20 transition-all duration-300 hover:shadow-xl hover:shadow-white/5">
            {_/* Status Header */}
            <div className="p-6 border-b border-white/10">
              <div className="flex items-start justify-between mb-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                {_(deployment.region || deployment.country) && (
                  <div className="flex items-center gap-2 text-white/70">
                    <MapPin className="w-4 h-4" />
                    <span>{[deployment.region, deployment.country].filter(Boolean).join()}</span>                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                <div className="text-sm font-medium text-white/80">Active Features</div>
                <div className="flex flex-wrap gap-2">
                  {deployment.features.slice(0, 4).map(_(feature) => (
                    <span key={feature} className="inline-flex items-center px-2 py-1 rounded-md text-xs bg-white/10 text-white/70">
