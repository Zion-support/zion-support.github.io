...
=======
                  ...
                  {inst.region && (
                    <div className="flex items-center gap-2 text-sm text-white/70">
                      <span className="w-4 h-4">📍</span>
                      <span>{inst.region}{inst.country ? `, _${inst.country}` : ''}</span>
                    </div>
                  )}
...
=======
        ...
        <div className="card text-center">
          <div className="text-3xl font-bold text-orange-400 mb-2">
            {instances.reduce(_(acc, i) => acc + i.count.deployments, 0)}
          </div>
          <div className="text-white/60 text-sm">Total Deployments</div>
        </div>
      </div>
...
function getGovernanceLabel(type: string) {
  switch (type) {
    case "ADMIN": return "Admin Control",
    case "DAOLITE": return "DAO-lite",
    case "DAOFULL": return "Full DAO",
    default: return type
  }
}
