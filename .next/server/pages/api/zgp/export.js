...
/* harmony import */ var private_next_pages_api_zgp_export_ts__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(632155);
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,next_dist_build_templates_helpers__WEBPACK_IMPORTED_MODULE_3__.hoist)(private_next_pages_api_zgp_export_ts__WEBPACK_IMPORTED_MODULE_4__, 'default'));
// Re-export config.
const config = (0,next_dist_build_templates_helpers__WEBPACK_IMPORTED_MODULE_3__.hoist)(private_next_pages_api_zgp_export_ts__WEBPACK_IMPORTED_MODULE_4__, 'config');
// Create and export the route module that will be consumed.
const routeModule = new next_dist_server_route_modules_pages_api_module_compiled__WEBPACK_IMPORTED_MODULE_2__.PagesAPIRouteModule({
    definition: {
        kind: next_dist_server_route_kind__WEBPACK_IMPORTED_MODULE_1__.RouteKind.PAGES_API,
        page: "/api/zgp/export",
        pathname: "/api/zgp/export",
        // The following aren't used in production.
        bundlePath: '',
        filename: ''
    },
    userland: private_next_pages_api_zgp_export_ts__WEBPACK_IMPORTED_MODULE_4__,
    distDir: ".next" || 0,
    relativeProjectDir:  false || ''
});
async function handler(req, res, ctx) {
    let srcPage = "/api/zgp/export";
...
function handler(req, res) {
  try {
    ...
    var proposal = (0,_utils_zgp_storage__WEBPACK_IMPORTED_MODULE_1__.getProposalById)(id);
    if (!proposal) {
      res.status(404).json({
        error: 'Not Found'
      });
    ...
    var latest = proposal.versions[proposal.latestVersion - 1];
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', "attachment; filename=\"".concat(proposal.proposalNumber, ".pdf\""));
    var doc = new (pdfkit__WEBPACK_IMPORTED_MODULE_0___default())({
      margin: 50
    });
    doc.pipe(res);
    doc.fontSize(18).text("".concat(proposal.proposalNumber, " \u2014 ").concat(proposal.title), {
      underline: true
    });
    doc.moveDown();
    doc.fontSize(12).text("Template: ".concat(proposal.templateCode));
    doc.text("Status: ".concat(proposal.status));
    doc.text("Created: ".concat(proposal.versions[0].createdAt));
    doc.text("Latest Version: v".concat(proposal.latestVersion, " @ ").concat(latest.createdAt));
    if (proposal.votingResultUrl) doc.text("Voting Result: ".concat(proposal.votingResultUrl));
    doc.moveDown();
    ...
    var funding = latest.fundingNeeded;
    if (funding && typeof funding === 'object') {
      doc.fontSize(12).text("".concat(funding.amount, " ").concat(funding.currency));
    } else {
...
var __webpack_exports__ = __webpack_require__.X(0, [618096,472076], () => (__webpack_exec__(80019)));
