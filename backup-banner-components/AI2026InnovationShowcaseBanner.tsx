import React from 'react';
import Link from 'next/link';

export default function AI2026InnovationShowcaseBanner() {
  return (
    <section className="text-left">
      {/* Innovation Pattern */}
      <div className="text-left">
        <div className="text-left"></div>
      </div>

      <div className="text-left">
        <div className="text-left">
          <div className="text-left">
            <span className="text-left">🚀<
            <span className="text-left">AI 2026 INNOVATION SHOWCASE<
          </div>
          
          <h2 className="text-left">
            The Future of AI
            <span className="text-left">
              {' '}Innovation is Now
            <
          </h2>
          
          <p className="text-left">
            Experience groundbreaking AI innovations that are reshaping industries and creating 
            unprecedented opportunities. From autonomous systems to quantum-enhanced intelligence.
          </p>

          <div className="text-left">
            <div className="text-left">
              <span className="text-left">🧠<
              <span>AGI Development<
            </div>
            <div className="text-left">
              <span className="text-left">⚛️<
              <span>Quantum AI<
            </div>
            <div className="text-left">
              <span className="text-left">🤖<
              <span>Autonomous Systems<
            </div>
          </div>
        </div>

        {/* Innovation Categories */}
        <div className="text-left">
          {/* Left Column - Core Innovations */}
          <div className="text-left">
            <div className="text-left">
              <div className="text-left">
                <div className="text-left">
                  <span className="text-left">🧠<
                </div>
                <div>
                  <h3 className="text-left">Artificial General Intelligence</h3>
                  <p className="text-left">
                    Breakthrough AGI systems that can learn, reason, and adapt across any domain. 
                    The first commercially available AGI platform for enterprise applications.
                  </p>
                </div>
              </div>
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">2026</div>
                  <div className="text-left">Launch Year</div>
                </div>
                <div className="text-left">
                  <div className="text-left">$2M</div>
                  <div className="text-left">Annual Value</div>
                </div>
              </div>
            </div>

            <div className="text-left">
              <div className="text-left">
                <div className="text-left">
                  <span className="text-left">⚛️<
                </div>
                <div>
                  <h3 className="text-left">Quantum-Enhanced AI</h3>
                  <p className="text-left">
                    Quantum computing integrated with AI for exponential problem-solving capabilities. 
                    Solve optimization problems 1 billion times faster than classical systems.
                  </p>
                </div>
              </div>
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">1Bx</div>
                  <div className="text-left">Speed Boost</div>
                </div>
                <div className="text-left">
                  <div className="text-left">∞</div>
                  <div className="text-left">Possibilities</div>
                </div>
              </div>
            </div>
          </div>

          {/* Right Column - Emerging Technologies */}
          <div className="text-left">
            <div className="text-left">
              <div className="text-left">
                <div className="text-left">
                  <span className="text-left">🤖<
                </div>
                <div>
                  <h3 className="text-left">Autonomous Enterprise Systems</h3>
                  <p className="text-left">
                    Fully autonomous AI systems that manage entire business operations without human intervention. 
                    Self-healing, self-optimizing, and self-evolving enterprise infrastructure.
                  </p>
                </div>
              </div>
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">100%</div>
                  <div className="text-left">Autonomous</div>
                </div>
                <div className="text-left">
                  <div className="text-left">24/7</div>
                  <div className="text-left">Operation</div>
                </div>
              </div>
            </div>

            <div className="text-left">
              <div className="text-left">
                <div className="text-left">
                  <span className="text-left">🌟<
                </div>
                <div>
                  <h3 className="text-left">Multimodal AI Integration</h3>
                  <p className="text-left">
                    Seamlessly integrate text, voice, image, and video processing in a single AI system. 
                    Revolutionary human-AI interaction capabilities for next-generation applications.
                  </p>
                </div>
              </div>
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">5</div>
                  <div className="text-left">Modalities</div>
                </div>
                <div className="text-left">
                  <div className="text-left">99%</div>
                  <div className="text-left">Accuracy</div>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Innovation Timeline */}
        <div className="text-left">
          <h3 className="text-left">Innovation Timeline 2026</h3>
          <div className="text-left">
            <div className="text-left">
              <div className="text-left">
                <span className="text-left">Q1<
              </div>
              <h4 className="text-left">AGI Beta Launch</h4>
              <p className="text-left">Limited enterprise access</p>
            </div>
            <div className="text-left">
              <div className="text-left">
                <span className="text-left">Q2<
              </div>
              <h4 className="text-left">Quantum AI Integration</h4>
              <p className="text-left">First quantum-enhanced solutions</p>
            </div>
            <div className="text-left">
              <div className="text-left">
                <span className="text-left">Q3<
              </div>
              <h4 className="text-left">Autonomous Systems</h4>
              <p className="text-left">Full enterprise deployment</p>
            </div>
            <div className="text-left">
              <div className="text-left">
                <span className="text-left">Q4<
              </div>
              <h4 className="text-left">Multimodal AI</h4>
              <p className="text-left">Complete integration platform</p>
            </div>
          </div>
        </div>

        <div className="text-left">
          <div className="text-left">
            <Link
              href="/services/ai-innovation-2026"
              className="text-left"
            >
              Access Innovation 2026
            </Link>
            <Link
              href="/blog/ai-innovation-showcase"
              className="text-left"
            >
              Explore Innovations
            </Link>
          </div>
          
          <div className="text-left">
            <p className="text-left">Be the first to experience the future</p>
            <p className="text-left">
              Join our exclusive innovation program and get early access to breakthrough AI technologies. 
              Limited to 100 enterprise clients in 2026.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}