import React from 'react';
import Link from 'next/link';
import { BookOpen, TrendingUp, Award, Zap, Globe, CheckCircle } from 'lucide-react';

/**
 * Content Intelligence Revolution Banner
 * Promotes the latest autonomous content intelligence blog post and case study
 * Published: October 1, 2025
 */
export default function ContentIntelligenceBanner() {
  return (
    <section className="text-left">
      {/* Animated background pattern */}
      <div className="text-left">
        <div className="text-left" />
        <div className="text-left" />
      </div>

      <div className="text-left">
        {/* Top Badge */}
        <div className="text-left">
          <div className="text-left">
            <Zap className="text-left" />
            <span>🚀 JUST PUBLISHED: OCTOBER 1, 2025 - CONTENT INTELLIGENCE BREAKTHROUGH!<
            <Zap className="text-left" />
          </div>
        </div>

        {/* Main Heading */}
        <div className="text-left">
          <h2 className="text-left">
            Autonomous Content Intelligence
            <br />
            <span className="text-left">Revolution<
          </h2>
          <p className="text-left">
            $127M Proven Value • 99.8% Accuracy • 47,000+ Automated Workflows
          </p>
          <p className="text-left">
            Transform your enterprise content operations with AI-powered autonomous systems. 
            Fortune 100 proven success across 84 countries with breakthrough results.
          </p>
        </div>

        {/* Key Metrics Grid */}
        <div className="text-left">
          <div className="text-left">
            <div className="text-left">$127M</div>
            <div className="text-left">Annual Value</div>
          </div>
          <div className="text-left">
            <div className="text-left">99.8%</div>
            <div className="text-left">Classification Accuracy</div>
          </div>
          <div className="text-left">
            <div className="text-left">47,000+</div>
            <div className="text-left">Automated Workflows</div>
          </div>
          <div className="text-left">
            <div className="text-left">95%</div>
            <div className="text-left">Cycle Time Reduction</div>
          </div>
          <div className="text-left">
            <div className="text-left">127</div>
            <div className="text-left">Languages Supported</div>
          </div>
          <div className="text-left">
            <div className="text-left">340%</div>
            <div className="text-left">Productivity Gain</div>
          </div>
        </div>

        {/* Feature Highlights */}
        <div className="text-left">
          <div className="text-left">
            <BookOpen className="text-left" />
            <h3 className="text-left">Intelligent Processing</h3>
            <ul className="text-left">
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>1,000+ documents/second processing<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>Advanced NLP & computer vision<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>Real-time quality assessment<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>Sub-100ms response time<
              </li>
            </ul>
          </div>

          <div className="text-left">
            <TrendingUp className="text-left" />
            <h3 className="text-left">Autonomous Operations</h3>
            <ul className="text-left">
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>Self-learning workflow optimization<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>24/7 unattended processing<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>Automatic compliance checking<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>Intelligent content distribution<
              </li>
            </ul>
          </div>

          <div className="text-left">
            <Globe className="text-left" />
            <h3 className="text-left">Enterprise Scale</h3>
            <ul className="text-left">
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>247,000 employees supported<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>84 countries deployment<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>99.99% uptime guarantee<
              </li>
              <li className="text-left">
                <CheckCircle className="text-left" />
                <span>Enterprise-grade security<
              </li>
            </ul>
          </div>
        </div>

        {/* CTA Buttons */}
        <div className="text-left">
          <Link
            href="/blog/ai-2025-oct-01-autonomous-content-intelligence-revolution"
            className="text-left"
          >
            <BookOpen className="text-left" />
            <span>📖 Read Complete Guide (18 min)<
            <span className="text-left">→<
          </Link>
          
          <Link
            href="/case-studies/ai-2025-oct-01-content-intelligence-127-million-success"
            className="text-left"
          >
            <Award className="text-left" />
            <span>💎 View $127M Success Story<
            <span className="text-left">→<
          </Link>
        </div>

        {/* Additional CTA */}
        <div className="text-left">
          <Link
            href="/contact"
            className="text-left"
          >
            <Zap className="text-left" />
            <span>🚀 Transform Your Content Operations Today<
          </Link>
        </div>

        {/* Social Proof */}
        <div className="text-left">
          <p className="text-left">TRUSTED BY FORTUNE 100 LEADERS</p>
          <div className="text-left">
            <div className="text-left">✅ Manufacturing</div>
            <div className="text-left">✅ Financial Services</div>
            <div className="text-left">✅ Healthcare</div>
            <div className="text-left">✅ Retail</div>
            <div className="text-left">✅ Technology</div>
            <div className="text-left">✅ Energy</div>
          </div>
        </div>
      </div>
    </section>
  );
}

/**
 * Compact Content Intelligence Showcase Banner
 * For use in content hub and other pages
 */
export function ContentIntelligenceShowcaseBanner() {
  return (
    <div className="text-left">
      <div className="text-left">
        <div className="text-left">
          🔥 NEW: OCTOBER 1, 2025 - CONTENT INTELLIGENCE BREAKTHROUGH!
        </div>
        <h2 className="text-left">
          Autonomous Content Intelligence Revolution
        </h2>
        <p className="text-left">
          $127M Proven Value • 99.8% Accuracy • 47,000+ Workflows
        </p>
        <p className="text-left">
          Transform enterprise content operations with AI-powered autonomous systems. 
          Fortune 100 proven success across 84 countries worldwide.
        </p>
        <div className="text-left">
          <Link
            href="/blog/ai-2025-oct-01-autonomous-content-intelligence-revolution"
            className="text-left"
          >
            📖 Read Revolutionary Guide →
          </Link>
          <Link
            href="/case-studies/ai-2025-oct-01-content-intelligence-127-million-success"
            className="text-left"
          >
            💎 View $127M Success Story →
          </Link>
          <Link
            href="/contact"
            className="text-left"
          >
            🚀 Get Started Today →
          </Link>
        </div>
      </div>
    </div>
  );
}
