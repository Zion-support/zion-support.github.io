import React from 'react';
import Link from 'next/link';
import { ArrowRight, Star, TrendingUp, Users } from 'lucide-react';

export default function NewContent2026ShowcaseBanner() {
  return (
    <section className="text-left">
      {/* Background Pattern */}
      <div className="text-left">
        <div className="text-left"></div>
      </div>
      
      <div className="text-left">
        <div className="text-left">
          <div className="text-left">
            <Star className="text-left" />
            EXCLUSIVE 2026 CONTENT
          </div>
          <h2 className="text-left">
            Revolutionary AI Insights for 2026
          </h2>
          <p className="text-left">
            Discover cutting-edge AI technologies, breakthrough case studies, and transformative business strategies 
            that are reshaping industries worldwide.
          </p>
        </div>

        {/* Featured Content Grid */}
        <div className="text-left">
          {/* Featured Article 1 */}
          <Link href="/blog/ai-future-workforce-2026" className="text-left">
            <div className="text-left">
              <div className="text-left">
                <span className="text-left">
                  NEW 2026
                <
                <span className="text-left">
                  FUTURE OF WORK
                <
              </div>
              <h3 className="text-left">
                AI Future Workforce 2026: Human-AI Collaboration Revolution
              </h3>
              <p className="text-left">
                Discover how AI is reshaping the workforce with 85% job transformation and 60% productivity gains.
              </p>
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">
                    <div className="text-left">85%</div>
                    <div className="text-left">Jobs Transformed</div>
                  </div>
                  <div className="text-left">
                    <div className="text-left">60%</div>
                    <div className="text-left">Productivity Gain</div>
                  </div>
                </div>
                <ArrowRight className="text-left" />
              </div>
            </div>
          </Link>

          {/* Featured Article 2 */}
          <Link href="/blog/ai-cybersecurity-2026" className="text-left">
            <div className="text-left">
              <div className="text-left">
                <span className="text-left">
                  NEW 2026
                <
                <span className="text-left">
                  CYBERSECURITY
                <
              </div>
              <h3 className="text-left">
                AI Cybersecurity 2026: Next-Generation Threat Protection
              </h3>
              <p className="text-left">
                Advanced AI security with 99.7% threat detection accuracy and autonomous incident response.
              </p>
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">
                    <div className="text-left">99.7%</div>
                    <div className="text-left">Detection Rate</div>
                  </div>
                  <div className="text-left">
                    <div className="text-left">80%</div>
                    <div className="text-left">Cost Reduction</div>
                  </div>
                </div>
                <ArrowRight className="text-left" />
              </div>
            </div>
          </Link>

          {/* Featured Case Study */}
          <Link href="/case-studies/ai-finance-automation-success-2026" className="text-left">
            <div className="text-left">
              <div className="text-left">
                <span className="text-left">
                  NEW 2026
                <
                <span className="text-left">
                  FEATURED
                <
              </div>
              <h3 className="text-left">
                AI Finance Automation: $15M ROI Success Story
              </h3>
              <p className="text-left">
                Fortune 500 company achieves 95% process automation and $15M annual ROI with AI finance solutions.
              </p>
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">
                    <div className="text-left">$15M</div>
                    <div className="text-left">Annual ROI</div>
                  </div>
                  <div className="text-left">
                    <div className="text-left">95%</div>
                    <div className="text-left">Automation</div>
                  </div>
                </div>
                <ArrowRight className="text-left" />
              </div>
            </div>
          </Link>
        </div>

        {/* Stats Section */}
        <div className="text-left">
          <div className="text-left">
            <div className="text-left">50+</div>
            <div className="text-left">New Articles</div>
          </div>
          <div className="text-left">
            <div className="text-left">25+</div>
            <div className="text-left">Case Studies</div>
          </div>
          <div className="text-left">
            <div className="text-left">$100M+</div>
            <div className="text-left">Total ROI Demonstrated</div>
          </div>
          <div className="text-left">
            <div className="text-left">95%</div>
            <div className="text-left">Average Automation</div>
          </div>
        </div>

        {/* CTA Section */}
        <div className="text-left">
          <div className="text-left">
            <h3 className="text-left">
              Stay Ahead with Cutting-Edge AI Insights
            </h3>
            <p className="text-left">
              Join 50,000+ professionals who trust Zion Tech Group for the latest AI innovations, 
              practical implementations, and proven business results.
            </p>
            <div className="text-left">
              <Link
                href="/blog"
                className="text-left"
              >
                <TrendingUp className="text-left" />
                Explore All Content
              </Link>
              <Link
                href="/case-studies"
                className="text-left"
              >
                <Users className="text-left" />
                View Case Studies
              </Link>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}