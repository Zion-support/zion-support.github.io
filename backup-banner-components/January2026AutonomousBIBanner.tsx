import React from 'react';
import Link from 'next/link';

export default function January2026AutonomousBIBanner() {
  return (
    <div className="text-left">
      {/* Background Pattern */}
      <div className="text-left"60" height="60" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg"%3E%3Cg fill="none" fill-rule="evenodd"%3E%3Cg fill="%239C92AC" fill-opacity="0.1"%3E%3Cpath d="M30 30c0-11.046-8.954-20-20-20s-20 8.954-20 20 8.954 20 20 20 20-8.954 20-20zm0 0c0 11.046 8.954 20 20 20s20-8.954 20-20-8.954-20-20-20-20 8.954-20 20z"/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')] opacity-20"></div>
      
      <div className="text-left">
        <div className="text-left">
          {/* Content */}
          <div>
            <div className="text-left">
              <span className="text-left">
                <span className="text-left"><
                Autonomous BI Revolution
              <
            </div>
            
            <h2 className="text-left">
              Self-Driving Business Intelligence
            </h2>
            
            <p className="text-left">
              Revolutionary autonomous BI systems that provide real-time insights, predictive analytics, 
              and automated decision support with 85% reduction in time-to-insight and 300% ROI.
            </p>
            
            <div className="text-left">
              <div className="text-left">
                <div className="text-left">85%</div>
                <div className="text-left">Faster Insights</div>
              </div>
              <div className="text-left">
                <div className="text-left">300%</div>
                <div className="text-left">ROI</div>
              </div>
            </div>
            
            <div className="text-left">
              <Link 
                href="/blog/ai-2026-january-autonomous-business-intelligence-revolution"
                className="text-left"
              >
                Read Full Article
              </Link>
              <Link 
                href="/contact"
                className="text-left"
              >
                Schedule Demo
              </Link>
            </div>
          </div>
          
          {/* Visual Element */}
          <div className="text-left">
            <div className="text-left">
              <div className="text-left">
                <div className="text-left">
                  <div className="text-left">
                    <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M3 3a1 1 0 000 2v8a2 2 0 002 2h2.586l-1.293 1.293a1 1 0 101.414 1.414L10 15.414l2.293 2.293a1 1 0 001.414-1.414L12.414 15H15a2 2 0 002-2V5a1 1 0 100-2H3zm11.707 4.707a1 1 0 00-1.414-1.414L10 9.586 8.707 8.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                    </svg>
                  </div>
                  <div className="text-left">Real-time</div>
                  <div className="text-left">Analytics</div>
                </div>
                <div className="text-left">
                  <div className="text-left">
                    <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                  </div>
                  <div className="text-left">Predictive</div>
                  <div className="text-left">Insights</div>
                </div>
                <div className="text-left">
                  <div className="text-left">
                    <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clipRule="evenodd" />
                    </svg>
                  </div>
                  <div className="text-left">Automated</div>
                  <div className="text-left">Decisions</div>
                </div>
                <div className="text-left">
                  <div className="text-left">
                    <svg className="text-left" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                    </svg>
                  </div>
                  <div className="text-left">Smart</div>
                  <div className="text-left">Reports</div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}