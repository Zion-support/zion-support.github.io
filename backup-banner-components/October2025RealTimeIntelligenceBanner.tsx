import React from 'react';
import Link from 'next/link';

export default function October2025RealTimeIntelligenceBanner() {
  return (
    <section className="text-left">
      {/* Animated background effects */}
      <div className="text-left">
        <div className="text-left"></div>
        <div className="text-left" style={{ animationDelay: '1s' }}></div>
      </div>

      <div className="text-left">
        {/* NEW Badge */}
        <div className="text-left">
          <div className="text-left">
            <span className="text-left">
              🚀 NEW OCTOBER 2025 BREAKTHROUGH
            <
            <span className="text-left">
              JUST RELEASED
            <
          </div>
        </div>

        {/* Main Headline */}
        <h2 className="text-left">
          Real-Time Enterprise Intelligence Platform Revolution
        </h2>

        {/* Subheadline */}
        <p className="text-left">
          Transform Business Decision-Making with AI-Powered Intelligence • 99.97% Accuracy • 3.7ms Response Time • $8.4B ROI Achievement
        </p>

        {/* Key Metrics Grid */}
        <div className="text-left">
          <div className="text-left">
            <div className="text-left">99.97%</div>
            <div className="text-left">Decision Accuracy</div>
          </div>
          <div className="text-left">
            <div className="text-left">3.7ms</div>
            <div className="text-left">Response Time</div>
          </div>
          <div className="text-left">
            <div className="text-left">$8.4B</div>
            <div className="text-left">Fortune 100 ROI</div>
          </div>
          <div className="text-left">
            <div className="text-left">89%</div>
            <div className="text-left">Decision Automation</div>
          </div>
        </div>

        {/* Feature Highlights */}
        <div className="text-left">
          <div className="text-left">
            <div className="text-left">⚡</div>
            <h3 className="text-left">Lightning-Fast Intelligence</h3>
            <p className="text-left">Real-time insights in 3.7ms average response time. Make critical decisions in milliseconds, not days.</p>
          </div>
          <div className="text-left">
            <div className="text-left">🎯</div>
            <h3 className="text-left">Near-Perfect Accuracy</h3>
            <p className="text-left">99.97% decision accuracy powered by advanced AI models and real-time analytics across 10,000+ data sources.</p>
          </div>
          <div className="text-left">
            <div className="text-left">💰</div>
            <h3 className="text-left">Massive ROI Generation</h3>
            <p className="text-left">Fortune 100 achieved $8.4B ROI in 18 months. 700x return on investment with 4-8 month payback period.</p>
          </div>
        </div>

        {/* CTA Buttons */}
        <div className="text-left">
          <Link
            href="/blog/ai-2025-oct-01-real-time-enterprise-intelligence-platform-revolution"
            className="text-left"
          >
            <span>Read Full Breakthrough Article<
            <span className="text-left">→<
          </Link>
          <Link
            href="/case-studies/ai-2025-oct-01-global-enterprise-real-time-intelligence-8-4-billion-success"
            className="text-left"
          >
            <span>View $8.4B Success Story<
            <span className="text-left">→<
          </Link>
          <Link
            href="/services/real-time-enterprise-intelligence-platform-2025"
            className="text-left"
          >
            <span>Explore Platform Services<
            <span className="text-left">→<
          </Link>
        </div>

        {/* Additional Info */}
        <div className="text-left">
          <p className="text-left">
            ✨ Featured Technologies: Real-Time Analytics • Cognitive AI • Decision Automation • Unified Data Fabric
          </p>
          <p className="text-left">
            Published: October 1, 2025 • Industry: Enterprise AI Solutions • Success Rate: 98% Implementation Success
          </p>
        </div>
      </div>
    </section>
  );
}
