...
# Script to merge existing cursor branches that actually exist
set -e
...
# Get only branches that actually exist
echo "🔍 Finding existing cursor branches to merge..."

# Get recent branches that exist
EXISTING_BRANCHES=$(git branch -r --list "origin/cursor/*" | grep -E "(fix-errors-and-merge-to-main|delete-old-data-records)" | sed 's/origin\///' | head -20)
