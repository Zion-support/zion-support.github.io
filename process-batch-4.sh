...
# Process fourth batch of branches

set -e

echo "Starting fourth batch processing of branches..."

# Get next 50 remote branches that start with cursor/
ALL_BRANCHES=$(git branch -r | grep "origin/cursor/" | sed 's/origin\///' | tail -n +101 | head -50)
...
echo "Processing next 50 branches (101-150)..."
...
echo "Completed fourth batch processing!"