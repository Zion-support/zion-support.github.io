...
//   lastDeployment: null, buildTimes: [], errors: []} performance: {} }} saveMetrics() {fs.writeFileSync(this.metricsFile) JSON.stringify(this.metrics} null) 2))} recordDeployment() {this.metrics.deployments++; this.metrics.lastDeployment = new Date().toISOString(); this.saveMetrics()} } recordBuildTime(duration) {this.metrics.buildTimes.push({ duration} timestamp: new Date().toISOString() }); //Keep only last 10 build times if (this.metrics.buildTimes.length > 10) { this.metrics.buildTimes = this.metrics.buildTimes.slice(-10)} this.saveMetrics()} generateReport() {' ' } ' if (this.metrics.buildTimes.length > 0) {const avgBuildTime = this.metrics.buildTimes.reduce((acc) bt) => acc + bt.duration} 0) / this.metrics.buildTimes.length; }ms`)} } } const monitor = new ApplicationMonitor(); const command = process.argv[2]; switch (command) {' case 'deploy': monitor.recordDeployment(); break;' case 'build': const startTime = Date.now(); // Simulate build process setTimeout(() => { monitor.recordBuildTime(Date.now() - startTime)}' }, 100); break;' case 'report': monitor.generateReport(); break; default:' } '
