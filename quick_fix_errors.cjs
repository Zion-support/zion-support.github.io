...
const appMinimalTestPath = path.join(__dirname, '__tests__/AppMinimal.test.tsx');
if (fs.existsSync(appMinimalTestPath)) {
  try {
    ...
    if (lines[0] && lines[0].includes('import') && (lines[0].match(/'/g) || []).length % 2 !== 0) {
...
const tailwindConfigPath = path.join(__dirname, 'zion-website/tailwind.config.js');
