...
export default function ServicesPage() {
  ...
  return (
    ...
    >
      ...
      <section className="bg-gradient-to-br from-gray-50 to-blue-50 py-20">
        <div className="mx-auto max-w-7xl px-6 text-center">
          ...
          <p className="text-xl lg:text-2xl text-gray-600 max-w-4xl mx-auto leading-relaxed">
            From Micro SaaS solutions to enterprise AI platforms, we provide the technology tools you need to succeed in the digital age.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {categories.map((category) => (
              <div key={category.id} className="text-center group">
                <div className={`h-24 w-24 bg-gradient-to-br ${category.color} rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300`}>
                  <category.icon className="h-12 w-12 text-white" />
                </div>
                <h3 className="text-2xl font-bold text-gray-900 mb-3">{category.name}</h3>
                <p className="text-gray-600 mb-4">{category.description}</p>
                <div className="text-sm text-gray-500">
                  {getServicesByCategory(category.name).length} services available
                </div>
              </div>
            ))}
          </div>
      ...
      {categories.map((category) => (
        <section key={category.id} className={`py-20 ${category.id === 'micro-saas' ? 'bg-gray-50' : 'bg-white'}`}>
          <div className="mx-auto max-w-7xl px-6">
            <div className="text-center mb-16">
              <div className={`inline-flex items-center space-x-3 bg-gradient-to-r ${category.color} text-white px-6 py-3 rounded-full mb-6`}>
                <category.icon className="h-6 w-6" />
                <span className="font-semibold">{category.name}</span>
              </div>
              <h2 className="text-4xl font-bold text-gray-900 mb-4">{category.name} Solutions</h2>
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {getServicesByCategory(category.name).map((service) => (
                <div key={service.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-100">
                  <div className="p-8">
                    {/* Header */}
                    <div className="flex items-start justify-between mb-6">
                      <div className="flex items-center space-x-4">
                        <div className={`h-16 w-16 bg-gradient-to-br ${getCategoryColor(service.categoryName)} rounded-xl flex items-center justify-center`}>
                          <Zap className="h-8 w-8 text-white" />
                        </div>
                        <div>
                          <h3 className="text-2xl font-bold text-gray-900 mb-2">{service.title}</h3>
                          <p className="text-blue-600 font-medium">{service.categoryName}</p>
                        </div>
                      </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              We deliver results with proven methodologies and cutting-edge technology.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="text-center">
              ...
              <p className="text-gray-600">Get up and running quickly with our pre-built solutions and expert implementation.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">Bank-grade security and compliance built into every solution we deliver.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">Leverage the latest AI and machine learning technologies for optimal results.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">Built for the cloud with scalability, reliability, and performance in mind.</p>
            </div>
          </div>
        </div>
      </section>
