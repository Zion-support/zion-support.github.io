...
export async function performDeploy(input: DeployInput): Promise<DeployResult> {
  ...
  writeTextFile(
    configPath,
    JSON.stringify(
      { ...input, instanceSlug, version, createdAt: nowIso() },
      null,
      2
    )
  );
  ...
  if (input.modules.gpt) {
    ...
    assets.push({
      kind: 'file',
      path: promptBasePath,
      description: 'ZionGPT prompt base',
    });
    logs.push({
      timestamp: nowIso(),
      level: 'info',
      action: 'zion_gpt_initialized',
    });
  }
  ...
  const trailerScriptPath = path.join(
    docsDir,
    `trailer-script-${instanceSlug}.md`
  );
  ...
  if (input.modules.bookBuilder) {
    ...
    assets.push({
      kind: 'file',
      path: bookPath,
      description: 'Zion Book (markdown source)',
    });
  }
  ...
  assets.push({
    kind: 'event',
    path: summitEventPath,
    description: 'Launch stream scheduled',
  });
  ...
  assets.push({
    kind: 'config',
    path: pagesActivationPath,
    description: 'Public pages activation record',
  });
  ...
  if (optionalModules.length > 0) {
    ...
    writeTextFile(
      optionalPath,
      JSON.stringify({ enabled: optionalModules }, null, 2)
    );
    assets.push({
      kind: 'config',
      path: optionalPath,
      description: 'Enabled optional modules',
    });
  }

  const summary = `Initialized ${input.instanceName} (${instanceSlug}) with modules: ${Object.entries(
    input.modules
  )
    .filter(([, v]) => v)
    .map(([k]) => k)
