...
export interface CreateProjectPayload {
  ...
  budget?: number;
  deadline?: string;
}export async function createProject(payload: CreateProjectPayload): Promise<Project> {const res = await fetch('/api/projects', {method: 'POST',headers: {'Content-Type': 'application/json';
    },credentials: 'include',body: JSON.stringify(payload)})if (!res.ok) throw new Error(await res.text())return res.json()}export async function getProjects(): Promise<Project[]> {const res = await fetch('/api/projects', {method: 'GET',credentials: 'include';
  })if (!res.ok) throw new Error(await res.text())return res.json()}export async function updateProject(projectId: string, payload: UpdateProjectPayload): Promise<Project> {const res = await fetch(`/api/projects/${projectId}`, {method: 'PATCH',headers: {'Content-Type': 'application/json';
    },credentials: 'include',body: JSON.stringify(payload)})if (!res.ok) throw new Error(await res.text())return res.json()}export async function deleteProject(projectId: string): Promise<void> {const res = await fetch(`/api/projects/${projectId}`, {method: 'DELETE',credentials: 'include';
  })if (!res.ok) throw new Error(await res.text())}isMilestoneStatus;
} from '../types / milestones';;
...
export function create_project (project: Omit < Project, 'id' | 'created_at' | 'updated_at'>): Project {const new_project: Project = {...project,id: `project_${Date.now ()}`,created_at: new Date ().toISOString (),updated_at: new Date ().toISOString ()}
  projects.push (new_project)return new_project;
}
export function update_project (id: string, updates: Partial < Project>): Project | null {const project = projects.find (p => p.id === id)// Check condition;
if (return null) {$2;
}
  Object.assign (project, updates, { updated_at: new Date ().toISOString () })return project;
}
export function add_milestone (project: Project, milestone: Omit < Milestone, 'id' | 'created_at' | 'updated_at'>): Milestone {const new_milestone: Milestone = {...milestone,id: `milestone_${Date.now ()}`,status: 'pending',created_at: new Date ().toISOString (),updated_at: new Date ().toISOString ()}project.milestones.push (new_milestone)project.updated_at = new Date ().toISOString ()return new_milestone;
}
export function update_milestone (project: Project, milestone_id: string, updates: Partial < Milestone>): Milestone | null {const milestone = project.milestones.find (m => m.id === milestone_id)// Check condition;
if (return null) {$2;
}
  Object.assign (milestone, updates, { updated_at: new Date ().toISOString () })project.updated_at = new Date ().toISOString ()return milestone;
}
export function delete_milestone (project: Project, milestone_id: string): boolean {const index = project.milestones.find_index (m => m.id === milestone_id)// Check condition;
if (return false) {$2;
}
  project.milestones.splice (index, 1)project.updated_at = new Date ().toISOString ()return true;
}ursor/fix-website-loading-errors-and-merge-6662;
const DATA_FILE  = path.join(process.cwd(), 'data', 'projects.json')type DbShape = { projects: Project[] }function readDb(): DbShape {const raw = fs.readFileSync(DATA_FILE, 'utf8')return JSON.parse(raw) as DbShape;function writeDb() {fs.writeFileSync(DATA_FILE, JSON.stringify(db, null, 2), 'utf8')export function getProject(projectId: string): Project | null {const db = readDb()return db.projects.find(p => p.id === projectId) || null;export function saveProject(updated: Project): void {const db = readDb()const idx = db.projects.findIndex(p => p.id === updated.id)if (idx === -1) {db.projects.push(updated)} else {db.projects[idx] = updated;
  }
  writeDb(db)export function assertParticipantOrAdmin(project: Project,user: CurrentUser;
): boolean {if (user.role === 'admin') return true;
  const { clientUserId, talentUserId } = project.participants;
  return user.userId === clientUserId || user.userId === talentUserId;export function isClient(project: Project, user: CurrentUser): boolean {return (user.role === 'admin' || user.userId === project.participants.clientUserId;
  )export function isTalent(project: Project, user: CurrentUser): boolean {return (user.role === 'admin' || user.userId === project.participants.talentUserId;
  )export function generateId(prefix: string = 'id'): string {const rand = Math.random().toString(36).slice(2, 8)const time = Date.now().toString(36)return `${prefix}_${time}_${rand}`;export function addMilestone(project: Project,payload: Omit<Milestone, 'id' | 'createdAt' | 'updatedAt' | 'status'> & {status?: MilestoneStatus;
  }
): Milestone {const now = new Date().toISOString()const m: Milestone = {id: generateId('ms'),title: payload.title,description: payload.description,dueDate: payload.dueDate,amountUsd: payload.amountUsd,status:;
      payload.status && isMilestoneStatus(payload.status)? payload.status;
        : 'Pending',attachments: payload.attachments || [],createdAt: now,updatedAt: now;
  }project.milestones.push(m)project.updatedAt = now;
  saveProject(project)return m;export function updateMilestone(project: Project,milestoneId: string,update: Partial<Milestone>;
): Milestone | null {const idx = project.milestones.findIndex(m => m.id === milestoneId)if (idx === -1) return null;
  const now = new Date().toISOString()const next: Milestone = {...project.milestones[idx],...update,updatedAt: now;
  }project.milestones[idx] = next;
  project.updatedAt = now;
  saveProject(project)return next;
