export async function connectMetaMask(): Promise<string[]></string[]>
                {
...
export async function getAccounts(): Promise<string[]></string[]>
                {
  if (typeof window === 'undefined' |!window.ethereum) {
  if (typeof window === 'undefined' || !window.ethereum) {
    return []
...
export async function getBalance(address: string): Promise<string></string>
                {
  if (typeof window === 'undefined' |!window.ethereum) {
  if (typeof window === 'undefined' || !window.ethereum) {
    throw new Error('MetaMask is not installed')
...
export async function signMessage(message: string, address: string): Promise<string></string>
                {
  if (typeof window === 'undefined' |!window.ethereum) {
  if (typeof window === 'undefined' || !window.ethereum) {
    throw new Error('MetaMask is not installed')
...
export async function connectMetaMask(): Promise<string[] | null></string[]>
                {
...
export async function getAccounts(): Promise<string[] | null></string[]>
                {
  const provider = getEthereumProvider()
  if (!provider) return null
  try {
