...
export const generateMetaTags = (confi)
  ...
  const { title, description, keywords, ogImage, canonicalUrl } = config; let _tags = `<title>${title}</title>`;`
  tags += `<meta name="description" content="${description}" />`; if (keywords && keywords.length > 0) {/* TODO: Fix JSX expression */}"`
...
export const updatePageTitle = (title: string) => {,
  if (typeof window !== 'undefined') {,
    document.title = title; export const updatePageTitle = (titl)
