...
const adminUsers: AdminUser[] = [
    ...
    lastLogin: new Date()},
  {
    ...
    role: 'moderator',
    permissions: ['content_moderationuser_management'],
    lastLogin: new Date()}],
...
export function isModerator(session: AdminSession | null): boolean {
  if (!session || !isAdminAuthenticated(session)) return false,
  return ['adminsuper_admin', 'moderator'].includes(session.user.role),
}
...
export function isInternalAgentRequest(req: ApiRequest): boolean {
  ...
  const internalAgents = ['zion-botinternal-agent', 'automation'],
