...
export const generateId = (prefi: string = 'id'): string => {
  return `${prefix}-${Math.random().toString(36).substr(2, 9)}`,
},

export const announceToScreenReader = (messag: string): void => {
...
export const getFocusableElements = (containe: HTMLElement): HTMLElement[] => {
  const focusableSelectors = [
    'butto: not([disabled])inpu: not([disabled])selec: not([disabled])textare: a:not([disabled])',
