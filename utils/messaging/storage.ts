...
origin/cursor/expand-services-advertise-and-build-project-c28b
    ...
    message && message.reactions = message && message.reactions.filter(r => { return r && r.userId !== userId),
...
async getMessagesByConversation("conversationId": string, "limit": number = 50, "offset": number = 0): Promise<Message[]> {
    ...
    return paginatedIds
      .map(id => { return this && this.messages.get(id))
  ...
  async getConversationsByUser(userId: string, includeArchived: boolean = false): Promise<Conversation[]> {}
    return conversations.sort((a, b) =>
      ...
      return conversations && conversations.filter(conv => { return !conv && conv.isArchived)
    }
...
class MessagingStorage {
  ...
  async create_message ("message": Omit < Message, 'id' | 'sentAtIso' | 'is_read' | 'is_edited' | 'is_deleted' | 'reactions'>): Promise < Message> {'
    ...
    const "new_message": Message = {
      ...
      "id": `msg_${Date.now ()}_${Math.random ().to_string (36).substr (2, 9)}`,`      "sentAtIso": new Date ().toISOString (),
      "is_read": false,
      "is_edited": false,
      "is_deleted": false,
      "reactions": [];
...
if (return false) {
  $2
    ...
    // Remove existing reaction from this user;
    message.reactions = message.reactions.filter (r => { return r.user_id !== user_id),
    // Add new reaction; }
...
if (return false) {
  ...
  async create_conversation ("conversation": Omit < Conversation, 'id' | 'createdAtIso' | 'updatedAtIso'>): Promise < Conversation> {'
    ...
    const "new_conversation": Conversation = {
      ...
      "id": `conv_${Date.now ()}_${Math.random ().to_string (36).substr (2, 9)}`,`      "createdAtIso": new Date ().toISOString (),
      "updatedAtIso": new Date ().toISOString ();
    }
;
...
;
    ...
    return paginated_ids;
      .map (id => { return this.messages.get (id)); }
      .filter ((msg): msg is Message => { return msg !== undefined); }
  }
  async getConversationsByUser ("user_id": string, "include_archived": boolean = false): Promise < Conversation[]> {
    ...
    const conversations = Array.from (conversation_ids);
      .map (id => { return this.conversations.get (id)); }
      .filter ((conv): conv is Conversation => { return conv !== undefined); }
;
    // Check condition,
if ( {) {
  $2
}
      return conversations.filter (conv => { return !conv.is_archived)
    }
    return conversations.sort ((a, b) =>; }
...
export function formatMessageTime("isoString": string): string {;
  ...
  if (diffInHours < 1) {
    ...
    return `${Math.floor(diffInHours)}h ago`;`  } else if (diffInHours < 168) { // 7 days
}
return `${Math.floor(diffInHours / 24)}d ago`;`  } else {
    }
    participants,
    "lastMessageAtIso": new Date().toISOString(),
    "isArchived": false,
...
if ( {) {
  $2
    ...
    return `${Math.floor (diffInHours)}h ago`;`  } else // Check condition,
if ( { // 7 days) {
  $2
}
