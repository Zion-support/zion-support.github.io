export function getConfig() {
  ...
  return {
    ...
    tokenName: 'Zion Token',''    tokenSymbol: 'ZION',''    decimals: 18,
    totalSupply: 1000000
  }
export interface TokenTransaction {
  ...
  userId: string
  amount: number
  type: 'issue' | 'redeem' | 'transfer'''  reason: string
  timestamp: number
}
  id: string
  user_id: string
...
import { randomUUID } from "crypto";"import { tokenStore } from "./storage";"import { TokenTransaction, WalletSummary } from "./types";";"export function getWalletSummary(userId: string): WalletSummary {;
...
export function earnTokens(
  ...
  if (amount <= 0) throw new Error("Amount must be positive") ";"const wallet = tokenStore.getWallet(userId);
...
export function burnTokens(
  ...
  if (amount <= 0) throw new Error("Amount must be positive") ";"const wallet = tokenStore.getWallet(userId)
  if (wallet.balance < amount) throw new Error("Insufficient balance");";"const newBalance = wallet.balance - amount
  tokenStore.setWalletBalance(userId, newBalance);
const tx: TokenTransaction = {
...
const amount = earnRules[action]
  if (!amount) throw new Error("Unknown action")""  return earnTokens(userId, amount, action, metadata)
}
...
const amount = burnRules[feature]
  if (!amount) throw new Error("Unknown feature")""  return burnTokens(userId, amount, feature, metadata)
}
...
const tx = burnTokens(userId, amount, "redeem_credits")""  tx.type = "redeem";"const usd = parseFloat((amount * usdPerToken).toFixed(2))
  return { tx, usd }
}
