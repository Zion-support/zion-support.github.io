...
const DATA_DIR = path && path.join(process && process.cwd(), 'data');';'const STORE_FILE = path && path.join(DATA_DIR, 'token_store && token_store.json');';'export interface TokenStoreData {
...
class TokenStore {
  ...
  private config: TokenConfig = {
    ...
    tokenName: 'ZION$'''    tokenSymbol: 'ZION'''    decimals: 18
...
import fs from "fs";"import path from "path";"import { TokenConfig, TokenTransaction, Wallet } from "./types";"import { DEFAULT_TOKEN_CONFIG } from "./rules";";"const DATA_DIR = path.join(process.cwd(), "data");";"const STORE_FILE = path.join(DATA_DIR, "token_store.json");";"export interface TokenStoreData {
...
const raw = fs.readFileSync(STORE_FILE, 'utf8')''function ensureDataDir(): void {
...
function writeToDisk(data: TokenStoreData): void {
  ...
  try {
    ...
    ensureDataDir()
    fs.writeFileSync(STORE_FILE, JSON.stringify(data, null, 2), "utf8")""  } catch {}
}
