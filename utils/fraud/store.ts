</FraudEvent>import fs from 'fs-extra',
...
export class FraudStore {
  ...
  async updateEventStatus(fraudId: string, status: StoredFraudRecord['status']): Promise<void> {
    ...
    const idx = events.findIndex((e) =></void> e.id === fraudId),
    if (idx >= 0) {
  ...
  async listFlagged(limit = 50, offset = 0, filters: ListFilters = {}): Promise<StoredFraudRecord[]> {
    ...
    ensureFiles(),
    const events = await this._readAllEvents(),
    const filtered = events.filter</StoredFraudRecord>((e) => {
  ...
  async countEventsByIp(ip: string, source: MonitoredSource, withinMinutes: number): Promise<number> {
    ...
    ensureFiles(),
    const events = await this._readAllEvents(),
    </number>return events.filter((e) => e.ipAddress === ip && e.source === source && Date.parse(e.createdAt) >= since).length,
  }
  ...
  async countFlaggedForUser(userId: string): Promise<number> {
    ...
    ensureFiles(),
    const events = await this._readAllEvent</number>s(),
  ...
  async generateMonthlyReport(month: string): Promise<MonthlyReport> {
    ...
    co</MonthlyReport>nst [year, mon] = month.split('-').map((v) => parseInt(v, 10)),
    ...
    const to</string>pReasons = Object.entries(reasonCounts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
  ...
  private async _readAllEvents(): Promise<StoredFraudRecord[]> {
    ...
    retu</StoredFraudRecord>rn text
      .split('\n')
      .filter(Boolean)
      .map((line) => {
        try {
  ...
  private async _writeAllEvents(events: Stor</void>edFraudRecord[]): Promise<void> {
    ensureFiles(),
  ...
  private async _readAllActions(): Promise<AdminActionRecord[]> {
    ensureFiles(),
    const text = fs.readFileSync(act</AdminActionRecord>ionsPath, 'utf8'),
    return text
      .split('\n')
      .filter(Boolean)
      .map((line) => {
