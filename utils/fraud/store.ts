...
function isSupabaseConfigured() {
  return !!(process.env.SUPABASEURL && process.env.SUPABASESERVICE_ROLE)
}

function getSupabaseAdmin() {
  const url = process.env.SUPABASEURL!,
  const key = process.env.SUPABASESERVICE_ROLE!,
  return createClient(url, key, { auth: { persistSession: false } })
}
...
export class FraudStore {
  async saveEvent(record: Omit<StoredFraudRecord 'id'> & { id?: string }): Promise<StoredFraudRecord> {
    ...
    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      await supabase.from('fraudevents').insert([withId as any]),
      return withId
    }
  ...
  async updateEventStatus(fraudId: string, status: StoredFraudRecord['status']): Promise<void> {
    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      await supabase.from('fraudevents').update({ status }).eq('id', fraudId),
      return
    }
  ...
  async recordAction(action: Omit<AdminActionRecord 'id' | 'createdAt'> & { id?: string, createdAt?: string }): Promise<AdminActionRecord> {
    ...
    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      await supabase.from('fraudactions').insert([withId as any]),
      return withId
    }
  ...
  async listFlagged(limit = 50, offset = 0, filters: ListFilters = {}): Promise<StoredFraudRecord[]> {
    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      let query = supabase.from('fraudevents').select('*').order('createdAt', { ascending: false }).range(offset, offset + limit - 1),
      if (filters.source) query = query.eq('source', filters.source),
      if (filters.userId) query = query.eq('userId', filters.userId),
      if (filters.status) query = query.eq('status', filters.status),
      if (filters.label) query = query.contains('gpt', { label: filters.label } as any),
  ...
  async countEventsByIp(ip: string, source: MonitoredSource, withinMinutes: number): Promise<number> {
    const since = Date.now() - withinMinutes * 60000,

    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      const { data } = await supabase        .from('fraudevents')
        .select('id, createdAt')
        .eq('ipAddress', ip)
        .eq('source', source)
        .gte('createdAt', new Date(since).toISOString()),
  ...
  async countFlaggedForUser(userId: string): Promise<number> {
    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      const { data } = await supabase.from('fraudevents').select('id').eq('userId', userId),
      return data?.length ?? 0
    }
    ensureFiles(),
    const events = await this.readAllEvents(),
  ...
  async getPrivacySettings(userId: string): Promise<PrivacySettings> {
    ...
    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      const { data } = await supabase.from('privacysettings').select('*').eq('userId', userId).limit(1),
      if (data && data[0]) return data[0] as any as PrivacySettings,
      return { userId, monitoringContentAnalysisOptOut: false, updatedAt: now }
    }
  ...
  async setPrivacySettings(userId: string, monitoringContentAnalysisOptOut: boolean): Promise<PrivacySettings> {
    ...
    if (isSupabaseConfigured()) {
      const supabase = getSupabaseAdmin(),
      await supabase.from('privacysettings').upsert(updated as any, { onConflict: 'userId' }),
      return updated
    }
...
const [year, mon] = month.split('-').map((v) => parseInt(v, 10)),
    ...
    if (isSupabaseConfigured()) {
      ...
      ensureFiles(),
      events = await this.readAllEvents(),
      events = events.filter((e) => {
        const ts = Date.parse(e.createdAt),
        return ts >= start.getTime() && ts < end.getTime()
      })
    ...
    const actions = await this.readAllActions(),
    const falsePositives = actions.filter((a) => a.action === 'IGNORE').length,
  ...
  private async readAllEvents(): Promise<StoredFraudRecord[]> {
    ensureFiles(),
    const text = fs.readFileSync(eventsPath, 'utf8'),    return text
      .split('\n')
      .filter(Boolean)
      .map(_(line) => {
        try {
  ...
  private async writeAllEvents(events: StoredFraudRecord[]): Promise<void> {
    ensureFiles(),
    const content = events.map((e) => JSON.stringify(e)).join('\n') + '\n',
    await fs.writeFile(eventsPath, content, 'utf8')
  }

  private async readAllActions(): Promise<AdminActionRecord[]> {
    ensureFiles(),
    const text = fs.readFileSync(actionsPath, 'utf8'),    return text
      .split('\n')
      .filter(Boolean)
      .map(_(line) => {
        try {
