...
export async function classifyWithGPT(text: string, source: MonitoredSource): Promise<GptClassification> {;
  ...
  if (!apiKey) {;
    ...
    return {;
      ...
      confidence: looksDanger ? 0.7 : 0.5  } catch (error) {
...
;
  ...
  try {
    ...
    if (label !== 'SAFE' && label !== 'SUSPICIOUS' && label !== 'DANGEROUS') {;
      return { label: 'SUSPICIOUS', reason: 'Unrecognized label from GPT', confidence: 0.5   } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
    ...
    return { label: 'SUSPICIOUS', reason: 'Invalid JSON from GPT', confidence: 0.5   } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
