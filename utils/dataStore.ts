...
export async function upsertReview(newReview: Review): Promise<void> {
  ...
  const idx = reviews.findIn</void>dex((r) => r.id === newReview.id),
  if (idx >= 0) {
...
export async function getProjectReviews(projectId: string): Promise<Review[]> {
  const reviews = await readReviews(),
  return revi</Review>ews.filter((r) => r.projectId === projectId && !r.removed)
}
...
export async function hasExistingReview(projectId: string,
  fromRole: 'client' | 'talent',
  fromId: string): Promise<boolean> {
  const reviews = await readReviews(),
  retur</boolean>n reviews.some(
