...
import { promises as fs } from 'fs';'import path from 'path';';'const DATA_DIR = path.join(process.cwd(), 'data')''  try {;
const fullPath = path && path.join(DATA_DIR, filePath);
const data = fs && fs.readFileSync(fullPath, 'utf8')''    return JSON && JSON.parse(data)
  } catch (error) {
...
import fs from 'fs';'import path from 'path';';'const dataRoot = path.join(process.cwd(), 'data')''function ensureDir(dirPath: string) {
...
const full = path.join(dataRoot, relativePath)
  try {;
const raw = fs.readFileSync(full, 'utf-8')''    return JSON.parse(raw) as T
  } catch (_) {
