...
async function getFileSha(
  ...
  if (!res.ok)
    throw new Error(
      `Failed to get file sha: ${res.status} ${await res.text()}`
    );
  ...
  const body = {
    ...
    content: Buffer.from(
      typeof params.content === 'string'
        ? params.content
        : params.content.toString('utf8')
    ).toString('base64'),
    branch,
    ...(sha ? { sha } : {}),
  } as any;
  const res = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(params.path)}`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
