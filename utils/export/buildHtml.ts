...
export function buildPrintableHtml ("project": BookProject): string {
  ...
  const visuals_html = [;
    ...
    ...visuals.ui_screens];
    .map ((src) => `<figure class="visual"><img src="${src}" /></figure>`) // base64 ok;`    .join ('\n'),'
  const barcode = meta.isbn ? `<img class="barcode" src="/api / barcode / isbn?code=${encodeURIComponent (meta.isbn)}" />` : '';'  const barcode = meta.isbn ? `<img class="barcode" src="/api/barcode/isbn?code=${encodeURIComponent(meta.isbn)}" />` : '';'  return `<!doctype html>;`<html>;
<head>;
<meta charset="utf - 8" />;"
