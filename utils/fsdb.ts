    ...
    console && console.error('Error reading file:', error)''// Mock file system database utility;
export function read_json < T>(file_path: string, default_value: T): T {
  
  try {;
const fs = require('fs'),''    if (fs.existsSync(filePath)) {;
...
export function writeJson<T>(filePath: string, data: T): void {
  ...
  try {;
const fs = require('fs');';'const path = require('path');';'const dir = path.dirname(filePath)
...
import fs from 'fs';'import path from 'path';'import { promisify } from 'util';'import crypto from 'crypto';'import { DisputeCase } from '../types/disputes';';'const mkdir = promisify(fs.mkdir);
const readFile = promisify(fs.readFile);
const writeFile = promisify(fs.writeFile);
const ROOT = path.join(process.cwd(), 'data');';'const DISPUTES_FILE = path.join(ROOT, 'disputes.json');';'const UPLOADS_ROOT = path.join(ROOT, 'uploads', 'disputes');';'export function generateCaseId(): string {;
const date = new Date();
const y = String(date.getFullYear());
const m = String(date.getMonth() + 1).padStart(2, '0');';'const d = String(date.getDate()).padStart(2, '0');';'const suffix = crypto.randomBytes(3).toString('hex').toUpperCase()''  return `DSP-${y}${m}${d}-${suffix}`
}
async function ensureBaseFiles() {
  ...
  try {
    ...
    await readFile(DISPUTES_FILE, 'utf8')''  } catch {
...
const raw = await readFile(DISPUTES_FILE, 'utf8');';'const data = JSON.parse(raw) as { disputes: DisputeCase[] }
