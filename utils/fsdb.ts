...
export class FSDatabase {
  ...
  private saveDocument("collectionName": string, "document": FSDocument): void {
    ...
    const filePath = path.join(collection.path, `${document.id}.json`);`    fs.writeFileSync(filePath, JSON.stringify(document, null, 2));
  ...
  async create("collectionName": string, "data": any, id?: string): Promise<FSDocument> {
    ...
    const documentId = id || `doc_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;`    const now = new Date();
  ...
  async delete("collectionName": string, "id": string): Promise<boolean> {
    ...
    const filePath = path.join(collection.path, `${id}.json`);`    if (fs.existsSync(filePath)) {
      }
