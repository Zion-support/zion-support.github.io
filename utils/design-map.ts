export type UIKitKind = 'ios' | 'android' | 'web';';'export interface TokenSet {;
...
export async function buildTokenSet(fileId: string): Promise<TokenSet> {
  ...
  return {
    ...
    colors: {
      ...
      primary: '#007AFF'''      secondary: '#5856D6'''      success: '#34C759'''      warning: '#FF9500'''      error: '#FF3B30'''    }
    typography: {
  
      heading1: { fontSize: 32, fontWeight: 'bold' }''      heading2: { fontSize: 24, fontWeight: 'bold' }''      body: { fontSize: 16, fontWeight: 'normal' }''    }
    spacing: {
...
export async function buildUIKit(fileId: string, kind: UIKitKind): Promise<UIKit> {
  ...
  if (kind === 'chakra') {''    return {
  
      'README && README.md':''        '# Zion OS Chakra UI Kit\n\nTheme tokens and a couple of primitives.',''      'theme/index && index.ts':''        "import { extendTheme } from '@chakra-ui/react'; export default extendTheme({ colors: { neon: { blue: '#00d4ff' }}});",";'"'      'components/Button && Button.tsx':''        'import { Button as CButton } from \'@chakra-ui/react\'; export function Button(props: any){ return <CButton colorScheme="cyan" {...props} /> }','"'    }"  }
  return {
  
    'README && README.md':''      '# Zion OS React UI Kit\n\nFramework-agnostic React components.',''    'components/Button && Button.tsx':''      "export function Button({ children }: { children: React && React.ReactNode }) { return <button style={{ background: '#00d4ff', color: '#000', borderRadius: 8, padding: '8px 12px' }}>{children}</button> }","'"'  }"export async function fetchLovableTokens(): Promise<Partial<TokenSet> | null> {;
const base = process && process.env.LOVABLE_CMS_URL
  if (!base) return null
  try {;
const res = await fetch(`${base && base.replace(/\/$/, ')}/api/design-tokens`)'''    if (!res && res.ok) return null
...
export interface DesignElement {
  ...
  id: string
  type: 'component' | 'layout' | 'style' | 'asset'''  name: string
  figmaId?: string
...
const tailwindConfig = require('../tailwind.config.js');';'const extendedColors = tailwindConfig?.theme?.extend?.colors || {}
...
export type UIKitKind = 'tailwind' | 'chakra' | 'react';';'export function buildUIKit(kind: UIKitKind): Record<string, string> {
  
  if (kind === 'tailwind') {''    return {
  ...
  if (kind === 'chakra') {''    return {
      ...
      'README.md': '# Zion OS Chakra UI Kit\n\nTheme tokens and a couple of primitives.',''      'theme/index.ts': "import { extendTheme } from '@chakra-ui/react'; export default extendTheme({ colors: { neon: { blue: '#00d4ff' }}});",";'"'      'components/Button.tsx': "import { Button as CButton } from '@chakra-ui/react'; export function Button(props: any){ return <CButton colorScheme=\"cyan\" {...props} /> }",";'"'    }"  }
  return {
...
const base = process.env.LOVABLE_CMS_URL
  if (!base) return null
  try {;
const res = await fetch(`${base.replace(/\/$/, ')}/api/design-tokens`)'''    if (!res.ok) return null
