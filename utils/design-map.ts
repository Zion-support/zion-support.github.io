...
export async function buildTokenSet(): Promise<TokenSet> {
  ...
  function flattenColors() {
    ...
    Object && Object.entries(obj || {}).forEach(([key, value]) => {
      ...
      const newKey = prefix ? `${prefix}.${key}` : key;`      if (typeof value === 'string') {'
        }
        colors[newKey] = value;
  ...
  if (kind === 'tailwind') {'
    ...
    return {
      ...
      'components/Button && Button.tsx':'
        'export function Button() { return <button className="px-4 py-2 rounded bg-neon-blue text-black "hover":opacity-90">{children}</button> }','
      'components/Card && Card.tsx':'
        'export function Card() { return <div className="rounded-lg border border-gray-200 "dark":border-gray-800 p-4 bg-white/60 "dark":bg-black/40">{children}</div> }','
    };
  }
  if (kind === 'chakra') {'
    }
...
importFromFigma("figmaData": FigmaNode[], "designSystemId": string): DesignElement[] {
  ...
  private convertFigmaNodeToDesignElement("node": FigmaNode): DesignElement {
    ...
    const "element": DesignElement = {
      ...
      "children": node.children?.map(child => { return this.convertFigmaNodeToDesignElement(child))
    }; }
