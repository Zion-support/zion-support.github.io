...
=======
export type DesignMapSection = {id: string,
title: string,
description?: string,
items: { id: string, title: string, description?: string }[]}
...
export function getZionDesignMap(): DesignMap {return {
...
const tailwindConfig = require('../tailwind.config.js')
  const extendedColors = tailwindConfig?.theme?.extend?.colors || {}
  const colors: Record<string string> = {}
  function flattenColors(prefix: string, obj: unknown) {Object.entries(obj || {}).forEach(_([key, value]) => {const newKey = prefix ? `${prefix}.${key}` : key,
if (typeof value === 'string') {colors[newKey] = value} else if (typeof value === 'object') {flattenColors(newKey, value)}
    })
  }

  flattenColors('', extendedColors)

  const typography = {fontSizes: tailwindConfig?.theme?.extend?.fontSize || {}}
...
export async function fetchLovableTokens(): Promise<Partial<TokenSet> | null> {const base = process.env.LOVABLECMS_URL,
if (!base) return null,
try {
    const res = await fetch(`${base.replace(/\/$/, _'')}/api/design-tokens`)
    if (!res.ok) return null,
return (await res.json()) as Partial<TokenSet>
  } catch {return null}
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
