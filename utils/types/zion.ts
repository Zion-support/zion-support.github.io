...
export interface GeneratedAsset {kind: "file" | "page" | "config" | "job" | "event";
  ...
  path: string;
  description?: string;}
  // Bonus;}export interface DeployInput  {instanceName: string;
  defaultLanguage: string;
  deploymentRegion: string;
  tokenActivation: boolean;governanceMode: GovernanceMode,branding: Branding,modules: DeployModules,requestedRoutes?: string[];
  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}export interface DeployLogEntry  {timestamp: string;
  level: "info" | "warn" | "error",action: string;
  details?: Record<string, unknown> | string;
  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}export interface GeneratedAsset  {kind: "file" | "page" | "config" | "job" | "event",path: string;
  description?: string;
  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}export interface DeployResult  {success: boolean;export interface DeployResult  {success: boolean;
  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c
  ...
  logs: DeployLogEntry[];
  summary: string;
  version: string;export type GovernanceMode = "Admin" | "DAO" | "Hybrid",export interface Branding  {logo_url?: string,primary_color?: string,secondary_color?: string,subdomain?: string;} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}}
