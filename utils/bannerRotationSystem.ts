...
export const recordBannerImpression = (
  impression: Omit<BannerImpression, 'timestamp' | 'sessionId'></BannerImpressi></BannerImpressio></BannerImpression>
) => {
...
export const recordBannerImpression = (impressio,
  n: Omit<BannerImpression, 'timestamp' | 'sessionId'></BannerImpressi></BannerImpressio></BannerImpression>)
}
  } catch (error) {
    console.error("Error:", error);
}
...
export const selectBannersForRotation = (
  allBanners: maxBanners, number = MAX_VISIBLE_BANNERS;
): BannerConfig[] => {
