...
export const recordBannerImpression = (
  ...
  try {
    const _impressions = getStoredImpressions();    const newImpression: BannerImpression = {
      ...impression,
      timestamp: Date.now(),
...
export const selectBannersForRotation = (
  ...
  // Calculate scores for all eligible banners,  const scoredBanners = eligibleBanners.map(banner => ({),
    banner),
