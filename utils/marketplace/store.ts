>>>>>>> 0fbf271b1f2a86c928092eda22ad7978eb59d0ee
  ...
  startDateIso: string
  endDateIso?: string
  status: 'DRAFT' | 'ACTIVE' | 'COMPLETED' | 'CANCELLED' | 'PAUSED'''  timeline: Array<{
  
    id: string
    title: string
    description: string
    status: 'pending' | 'in_progress' | 'completed' | 'overdue'''    due_date: string
...
export interface Offer {
  ...
  paymentTerms: {
    ...
    type: 'fixed' | 'hourly' | 'milestone'''    amount?: number
    hourlyRate?: number
    milestones?: Array<{
  ...
  agreementUrl?: string
  status: 'SENT' | 'ACCEPTED' | 'REJECTED' | 'EXPIRED' | 'CANCELLED'''  expiresAt?: string
      due_date: string,
    }>
  }
  agreement_url?: string
  status: 'SENT' | 'ACCEPTED' | 'REJECTED' | 'EXPIRED' | 'CANCELLED'''  expires_at?: string
  notes?: string,
}
export interface Application {
  ...
  talentSlug: string
  appliedAtIso: string
  status: 'PENDING' | 'ACCEPTED' | 'REJECTED' | 'WITHDRAWN'''  coverLetter?: string
  proposedRate?: number
...
): Omit<Project, 'id' | 'createdAt' | 'updatedAt'> {''  return {
...
export function createOfferData(
  ...
  talentSlug: string
  scopeSummary: string
  paymentTerms: Offer['paymentTerms']''  additionalData?: Partial<Offer>
): Omit<Offer, 'id' | 'createdAtIso'> {''  return {
...
): Omit<Application, 'id' | 'appliedAtIso'> {''  return {
  
    projectId,
    talentSlug,
    status: 'PENDING',''    ...additionalData
  }
}
...
): Omit<Message, 'id' | 'sentAtIso'> {''  return {
...
import fs from "fs";"import path from "path";"import { MarketplaceDb, Offer, Project } from "./types";";"const DATA_DIR = path.join(process.cwd(), "data", "runtime");";"const DB_PATH = path.join(DATA_DIR, "marketplace.json")""function ensureDataFile(): void {
...
export function readDb(): MarketplaceDb {
  ...
  ensureDataFile()
  try {;
const raw = fs.readFileSync(DB_PATH, "utf-8");";"const data = JSON.parse(raw) as MarketplaceDb
