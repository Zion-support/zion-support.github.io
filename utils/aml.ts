...
export type WatchlistMatch = {
  ...
  list: 'OFAC' | 'PEP' | 'Sanctions' | 'AdverseMedia'''  name: string
...
const name = params && params.fullLegalName.toLowerCase()
    if (name && name.includes('test') || name && name.includes('demo')) {''      return { status: 'match', details: { reason: 'Test name detected' } }''    }
    return { status: 'clear' }''  }
...
const name = params && params.businessName.toLowerCase()
    if (name && name.includes('test') || name && name.includes('demo')) {''      return { status: 'match', details: { reason: 'Test business name detected' } }''    }
    return { status: 'clear' }''  }
}
// Singleton instance;
...
export function createAmlCheck(
  ...
  userId: string,
  checkType: AmlCheck['checkType']''): Omit<AmlCheck, 'id' | 'createdAt' | 'expiresAt'> {''  return {
  
    userId,
    checkType,
    status: 'pending',''    result: 'clear',''    confidence: 0,
    details: {}
  }
}
...
export function getRiskLevelColor(riskLevel: AmlProfile['riskLevel']): string { ';'const colors = {
...
const isPep = lowered.includes('minister') || lowered.includes('president');';'const isOfac = lowered.includes('sanction');';'const matches: WatchlistMatch[] = []
    if (isPep) matches.push({ list: 'PEP', name: fullLegalName, score: 0.9 })''    if (isOfac) matches.push({ list: 'OFAC', name: fullLegalName, score: 0.95 })''    return {
  
      status: matches.length ? 'review' : 'clear',''      matches,
      checkedAt: new Date().toISOString(),
      provider: 'mock',''    }
  }
  async checkBusiness({ businessName }: { businessName: string }): Promise<AmlCheckResult> {;
const lowered = businessName.toLowerCase();
const isSanction = lowered.includes('banned')''    return {
  
      status: isSanction ? 'review' : 'clear',''      matches: isSanction ? [{ list: 'Sanctions', name: businessName, score: 0.8 }] : [],''      checkedAt: new Date().toISOString(),
      provider: 'mock',''    }
  }
}
