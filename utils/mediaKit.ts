...
export function buildTimeline(startDate: Date) {
  ...
  return [{ label: 'Week 1: Closed Beta Invite', date: fmt(addDays(startDate, 0)) }
...
function build_timeline() {
  ...
  return [{ label: 'Week 1: Closed Beta Invite', date: fmt (add_days (start_date, 0)) }, { label: 'Week 2: ZionGPT Core Reveal', date: fmt (add_days (start_date, 7)) }, {
      label: 'Week 3: Token Airdrop Snapshot', date: fmt (add_days (start_date, 14)), }, { label: 'Week 4: Zion Global Summit', date: fmt (add_days (start_date, 21)) }]
/**
...
export function buildTimeline(startDate: Date) {
  ...
  const fmt = (d: Date) => d.toISOString().substring(0, 10)
  return [{ label: 'Week 1: Closed Beta Invite', date: fmt(addDays(startDate, 0)) }, { label: 'Week 2: ZionGPT Core Reveal', date: fmt(addDays(startDate, 7)) }, { label: 'Week 3: Token Airdrop Snapshot', date: fmt(addDays(startDate, 14)) }, { label: 'Week 4: Zion Global Summit', date: fmt(addDays(startDate, 21)) }]
}
function titleCase(s: string) {
