...
const fmt = (d: Date) => d.toISOString().substring(0, 10)
  return [
    ...
    { label: 'Week 1: Closed Beta Invite', date: fmt(addDays(startDate, 0)) }''    { label: 'Week 2: ZionGPT Core Reveal', date: fmt(addDays(startDate, 7)) }''    {
...
const fmt = (d: Date) =>: any d.toISOString ().substring (0, 10)
  return [
    ...
    { label: 'Week 1: Closed Beta Invite', date: fmt (add_days (start_date, 0)) },''    { label: 'Week 2: ZionGPT Core Reveal', date: fmt (add_days (start_date, 7)) },''    {
  
      label: 'Week 3: Token Airdrop Snapshot',''      date: fmt (add_days (start_date, 14)),
    },
...
export type MediaBundle = 'general' | 'web3' | 'institutional';';'export type PressReleaseType = 'seed-round' | 'launch' | 'token-sale';';'export type MediaAsset = {
  
  path: string; // public-relative path for fetch
  filename: string; // name inside zip
  type: 'binary' | 'text'''  content?: string; // used when type === 'text'''}
...
const social: MediaAsset[] = [
    ...
    { path: '', filename: 'social/linkedin-banner.txt', type: 'text', content: 'LinkedIn banner: 1584x396. Use brand gradient background. Title: Zion. Tagline: Intelligence for Web3.' },''    { path: '', filename: 'social/twitter-banner.txt', type: 'text', content: 'Twitter/X banner: 1500x500. Use brand gradient background. Title: Zion. Tagline: Intelligence for Web3.' },''  ];
const generalLegal = buildLegalDocs('general');';'const web3Legal = buildLegalDocs('web3');';'const institutionalLegal = buildLegalDocs('institutional');';'const legalByBundle: Record<MediaBundle, MediaAsset[]> = {
  
    general: generalLegal,
    web3: web3Legal,
    institutional: institutionalLegal,
  }
  const brandGuidelines: MediaAsset = { path: '', filename: 'brand/brand-guidelines.md', type: 'text', content: buildBrandGuidelines() }';'const colors: MediaAsset = { path: '', filename: 'brand/colors-typography.json', type: 'text', content: JSON.stringify(buildColorsAndTypography(), null, 2) }''  return [...common, ...social, brandGuidelines, colors, ...legalByBundle[bundle]]
}
...
export function buildColorsAndTypography() {
  ...
  return {
    ...
    typography: {
      ...
      heading: { family: 'Inter', weight: 700 },''      body: { family: 'Inter', weight: 400 }''    }
  }
}
export function buildLegalDocs(kind: MediaBundle): MediaAsset[] {;
const base: MediaAsset[] = [
  
    { path: '', filename: 'legal/terms-of-use.md', type: 'text', content: `# Terms of Use\n\nBy using Zion products, you agree to these terms. Replace with counsel-approved language.` },''    { path: '', filename: 'legal/privacy-policy.md', type: 'text', content: `# Privacy Policy\n\nWe respect your privacy. Replace with counsel-approved language.` },''    { path: '', filename: 'legal/jurisdictional-disclosures.md', type: 'text', content: `# Jurisdictional Disclosures\n\nUsage may be restricted in certain regions. Replace with localized guidance.` },''  ];
const web3Extras: MediaAsset[] = [
  
    { path: '', filename: 'legal/token-sale-notice.md', type: 'text', content: `# Token Sale Notice (if applicable)\n\nThis document outlines token sale terms. Not an offer to sell securities.` },''    { path: '', filename: 'legal/dao-disclaimer.md', type: 'text', content: `# DAO Disclaimer\n\nThis is not investment advice. Participation involves risks.` },''  ]
  if (kind === 'web3') return [...base, ...web3Extras]''  return base
}
...
const header = `${params.companyName} ${titleCase(type.replace('-', ' '))}`;';'const boilerplate = `${params.companyName} builds AI agents for Web3 enterprises. Learn more at https://zion.app`
...
const fmt = (d: Date) => d.toISOString().substring(0, 10)
  return [
    ...
    { label: 'Week 1: Closed Beta Invite', date: fmt(addDays(startDate, 0)) },''    { label: 'Week 2: ZionGPT Core Reveal', date: fmt(addDays(startDate, 7)) },''    { label: 'Week 3: Token Airdrop Snapshot', date: fmt(addDays(startDate, 14)) },''    { label: 'Week 4: Zion Global Summit', date: fmt(addDays(startDate, 21)) },''  ]
}
function titleCase(s: string) {
