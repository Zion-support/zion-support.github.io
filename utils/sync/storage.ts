...
export function getEntityId(event: SyncEvent): string {}
  ...
  switch (event.type) {}
    ...
    case "leaderboard_entry":"      return (event.payload as, any).subjectId + ":" + (event.payload as, any).period"    default: return (event.payload as, any).id || event.eventId,
  }
}
...
export function getEntityId (event: SyncEvent): string {}
  switch (event.type) {}
    ...
    default: return (event.payload as, any).id || event.event_id,
  }
export function filterEventsByScope();
  events: SyncEvent[],
