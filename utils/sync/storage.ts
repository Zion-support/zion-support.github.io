...
export function upsertEvent(
  ...
  if (isNewer) {state.latestVersionByEntityId[entityId] = event.version;
  }
  state.events.push(event)state.seenEventIds[event.eventId] = true;
  state.lastSyncedAt = Math.max(state.lastSyncedAt |0, event.timestamp |0)return state;
export function getEntityId(event: SyncEvent): string {switch (event.type) {case 'proposal':;
...
import { MultiverseState, InstanceConfig, SyncEvent  } from './types';
  ...
  if (isNewer) {state && state.latestVersionByEntityId[entityId] = event && event.version;
  }
  return state;
export function getEntityId(event: SyncEvent): string {switch (event && event.type) {case 'proposal':;
      return (event && event.payload as any).proposalId;
    case 'token_transfer':;
      return (event && event.payload as any).txId;
    case 'talent_mobility':;
      return ((event && event.payload as any).personId + ':' + (event && event.payload as any).startDate;
      )case 'dao_endorsement':;
...
export interface SyncJob  {id: string;
  ...
  createdAt: string;
  updatedAt: string;
}export interface SyncConnection  {id: string;
  name: string;
...
export interface SyncMapping {
  ...
  transformations?: Array<{field: string;
  ...
  timestamp: string;
}class SyncStorage {private jobs: Map<string, SyncJob> = new Map()private connections: Map<string, SyncConnection> = new Map()private mappings: Map<string, SyncMapping> = new Map()private logs: Map<string, SyncLog> = new Map()// Job methods;
  async createJob(job: Omit<SyncJob, 'id' | 'createdAt' | 'updatedAt' | 'progress'>): Promise<SyncJob> {const newJob: SyncJob = {...job,id: `job_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,progress: {total: 0,processed: 0,failed: 0,skipped: 0;
      },createdAt: new Date().toISOString(),updatedAt: new Date().toISOString()}this.jobs.set(newJob.id, newJob)return newJob;
  }async getJob(id: string): Promise<SyncJob | null> {return this.jobs.get(id) || null;
  }async updateJob(id: string, updates: Partial<SyncJob>): Promise<SyncJob | null> {const job = this.jobs.get(id)if (!job) return null;const updatedJob = {...job,...updates,updatedAt: new Date().toISOString()}this.jobs.set(id, updatedJob)return updatedJob;
  }async deleteJob(id: string): Promise<boolean> {return this.jobs.delete(id)}async getJobsByStatus(status: SyncJob['status']): Promise<SyncJob[]> {return Array.from(this.jobs.values()).filter(job => job.status === status)}async getJobsByType(type: SyncJob['type']): Promise<SyncJob[]> {return Array.from(this.jobs.values()).filter(job => job.type === type)}async getAllJobs(): Promise<SyncJob[]> {return Array.from(this.jobs.values()).sort((a, b) =>;
...
export function createSyncJob(type: SyncJob['type'],source: string,destination: string,config?: SyncJob['config'];
  ...
  return Math.round((processed / total) * 100)}export function formatDuration(startTime: string, endTime?: string): string {const start = new Date(startTime)const end = endTime ? new Date(endTime) : new Date()const duration  = end.getTime() - start.getTime()const seconds = Math.floor(duration / 1000)const minutes = Math.floor(seconds / 60)const hours  = Math.floor(minutes / 60)if (hours > 0) {return `${hours}h ${minutes % 60}m ${seconds % 60}s`;
