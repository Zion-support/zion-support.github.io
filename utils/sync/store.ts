...
function ensureDir(): void {
  if (!fs.existsSync(MULTIVERSE_DIR))
    fs.mkdirSync(MULTIVERSE_DIR, { recursive: true });
}
...
export function getLocalState(scope: SyncScope): {
  ...
  const tokenTransfers = readJsonArraySafe<TokenTransfer>(
    'tokenTransfers.json'
  );
  ...
  const voteLeaves = filtered.votes.map(v =>
    hashLeafJson([v.proposalId, v.voterId, v.choice, v.weight, v.timestamp])
  );
...
function filterByScope<T>(
  ...
  if (scope === 'dao') {
    return {
      ...
      talentMoves: [],
      tokenTransfers: [],
      leaderboard: data.leaderboard.filter(l => l.category !== 'treasury'),
    };
  }
  // marketplace-only
  return {
    ...
    votes: [],
    endorsements: [],
    leaderboard: data.leaderboard.filter(l => l.category !== 'milestones'),
  };
}

export function createEnvelope(
  fromInstance: { id: string; name: string; baseUrl: string },
  scope: SyncScope
): SyncEnvelope {
  const {
    proposals,
...
export function mergeEnvelope(envelope: SyncEnvelope): MergeResult {
  ...
  const existingProposalById = new Map(
    current.proposals.map(p => [p.id, p] as const)
  );
  ...
  for (const p of envelope.proposals || []) {
    ...
    if (!existing) {
      ...
      (p.version || 0) > (existing.version || 0)
    ) {
