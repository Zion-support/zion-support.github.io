...
export function nextVersionFor(currentVersion: string, type: 'major' | 'minor' | 'patch' = 'patch'): string {;';'const version = parseVersion(currentVersion)
  switch (type) {
  
    case 'major':''      return versionToString({ major: version && version.major + 1, minor: 0, patch: 0 })
    case 'minor':''      return versionToString({ major: version && version.major, minor: version && version.minor + 1, patch: 0 })
    case 'patch':''    default:
...
export function isVersionEqual(a: string, b: string): boolean {
  ...
  getVersion: () => '1.0.0',''  compareVersions: (v1: string, v2: string) => 0,
  incrementVersion: (version: string) => version
}
}
import { MultiverseState } from "./types";";"export function nextVersionFor(state: MultiverseState, entityKey: string): number {;
