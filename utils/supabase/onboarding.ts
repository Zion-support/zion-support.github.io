...
export type TalentOnboarding = {profile_complete: boolean;
  skills_added: boolean;
  availability_set: boolean;
  first_job_applied: boolean;

export type ClientOnboarding = {job_posted: boolean;
  talent_invited: boolean;
  quote_received: boolean;
  first_hire_complete: boolean;

export type OnboardingRecord = {user_id: string;
  role: 'talent' | 'client';
  // talent fields;
...
export async function getCurrentUserId(): Promise<string | null> {try {;
...
export async function fetchOnboardingProgress(userId: string, role: 'talent' | 'client'): Promise<OnboardingRecord | null> {try {;
...
export function fallbackClientProgress(): ClientOnboarding {return {;
    job_posted: true;
