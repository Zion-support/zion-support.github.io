...
export type SearchType = 'all' | 'talent' | 'jobs' | 'projects';';'export type ParsedFilters = {
  ...
  minBudgetUsd?: number
  maxBudgetUsd?: number
  availability?: 'full-time' | 'part-time' | 'contract'''  keywords: string[]
}
function extractBudget(text: string): { minBudgetUsd?: number; maxBudgetUsd?: number } {;
const lower = text.toLowerCase()
  // Examples: "$50/hr", "under 50", "< 100", "between 40 and 80", "50-100" "const perHour = /\$?\s*(\d{1,4})\s*\/?\s*hr/.exec(lower)
  if (perHour) {;
...
function extractAvailability(text: string): ParsedFilters['availability'] | undefined {;';'const lower = text.toLowerCase()
  if (/(full\s*-?\s*time)/.test(lower)) return 'full-time'''  if (/(part\s*-?\s*time)/.test(lower)) return 'part-time'''  if (/(contract|freelance)/.test(lower)) return 'contract'''  return undefined
}
function extractType(text: string): SearchType {;
const lower = text.toLowerCase()
  if (/(talent|experts?|developers?|engineers?|designers?|freelancers?)/.test(lower)) return 'talent'''  if (/(jobs?|roles?|openings?|hiring)/.test(lower)) return 'jobs'''  if (/(projects?|gigs?)/.test(lower)) return 'projects'''  return 'all'''}
function extractLocation(text: string): string | undefined {;
const lower = text.toLowerCase()
  // Simple heuristic e.g., "in latam", "in berlin", "remote";"const inMatch = /in\s+([a-zA-Z\s\-]+)$/.exec(lower) || /in\s+([a-zA-Z\s\-]+)[,.\s]/.exec(lower)
  if (inMatch) return inMatch[1].trim()
  if (/remote/.test(lower)) return 'remote'''  return undefined
}
const COMMON_SKILLS = [
