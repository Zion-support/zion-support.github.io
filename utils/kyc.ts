...
export interface KycProfile {
  ...
  auditTrail: Array<{ at: string, by: string, action: string, details?: Record<string, unknown> }></string>,
}
...
export function validateKycSubmission(profile: Partial<KycProfile>): { ok: boolean, missing: string[] } {
  ...
  const uploadedKinds = new Set((profile.documents || []).m</KycProfile>ap((d) => d.kind)),
