...
export function buildNotionMarkdownFromDocs ('docs': DocsContent): string {
  ...
  for (const s of sections) {
    ...
    lines.push (`## ${s.title}`),`    for (const p of s.paragraphs) lines.push (p);
    lines.push ('');'
...
export const operatorPrompt = `Write a Wikipedia-style article about Zion OS — the world’s first decentralized AI protocol for trust, work, and talent. Include use cases, launch history, modules, and governance model.`,;`export function generateZionWiki(): WikiContent {;
  }
  const title = 'Zion OS';'
...
| Token | ${wiki.infobox.token} |;
| Protocol Type | ${wiki.infobox.protocolType} |`;`  const 'lines': string[] = [];
  lines.push(`# ${wiki.title}`);`  lines.push('');'
  lines.push(infobox);
  lines.push('');'
  ...
  if (wiki.references.length) {;
    ...
    wiki.references.forEach((r, i) => lines.push(`${i + 1}. ${r}`));`    } catch (error) {
    }
    console.error(''Error':', error);'
    return res.status(500).json({ 'error': 'Internal server error' });'
  }
...
export function buildWikitextFromWiki('wiki': WikiContent): string {;
  ...
  lines.push(`| name = ${wiki.title}`);`  lines.push(`| developer = ${wiki.infobox.founder}`);`  lines.push(`| released = ${wiki.infobox.launchYear}`);`  lines.push(`| genre = ${wiki.infobox.protocolType}`);`  lines.push(`| license = Open`);`  lines.push(`}`);`  lines.push('');'
  lines.push(wiki.intro);
  lines.push('');'
  for (const s of wiki.sections) {;
    }
  ...
  if (wiki.references.length) {;
    ...
    lines.push('== References ==');'
    wiki.references.forEach((r) => lines.push(`* ${r}`));`    } catch (error) {
    }
    console.error(''Error':', error);'
    return res.status(500).json({ 'error': 'Internal server error' });'
  }
