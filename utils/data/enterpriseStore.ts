...
const seedCompany: CompanyRecord = {id: 'cmp_acme';
  ...
  members: [;
    { id: 'mem_1',
  email: 'admin@acme.com', name: 'Avery Admin',}
  role: 'admin' }
    { id: 'mem_2',
  email: 'maria@acme.com', name: 'Maria Manager',}
  role: 'manager' }
    { id: 'mem_3',
  email: 'reid@acme.com', name: 'Reid Recruiter',}
  role: 'recruiter' }];
  activity: [;
    { id: generateId(), timestampIso: new Date().toISOString(), actorEmail: 'admin@acme.com',}
  action: 'created_company' }
    { id: generateId(), timestampIso: new Date().toISOString(), actorEmail: 'maria@acme.com',}
  action: 'posted_job', meta: { jobId: 'job_123' } }];
  invoices: [;
    { id: 'inv_001',
  companyId: 'cmp_acme', number: 'INV-1001',
  amountUsd: 499.0, periodStartIso: '2025-07-01',}
  periodEndIso: '2025-07-31', status: 'paid' }
    { id: 'inv_002',
  companyId: 'cmp_acme', number: 'INV-1002',
  amountUsd: 499.0, periodStartIso: '2025-08-01',}
  periodEndIso: '2025-08-31', status: 'open' }]}
...
export const store = {getCompanyBySlug(slug: string) {;}
  ...
  removeMember(companyId: string, memberId: string): boolean {const company = companiesById[companyId];
    ...
    if (changed) {;}
      company.plan.seatsUsed = Math.min(company.plan.seatsPurchased, company.members.length);}
      company.activity.unshift({ id: generateId(), timestampIso: new Date().toISOString(), actorEmail: 'system',}
  action: 'removed_member', meta: { memberId } });
    }
    return changed;
  }
  updateMemberRole(companyId: string, memberId: string, role: EnterpriseRole): boolean {const company = companiesById[companyId];
    ...
    member.role = role;}
    company.activity.unshift({ id: generateId(), timestampIso: new Date().toISOString(), actorEmail: 'system',}
  action: 'updated_role', meta: { memberId, role } });
    return true;
  }
  setUsageLimits(companyId: string, monthlyJobPosts: number, budgetCapUsd: number): boolean {const company = companiesById[companyId];}
    ...
    company.activity.unshift({ id: generateId(), timestampIso: new Date().toISOString(), actorEmail: 'system',}
  action: 'updated_usage_limits', meta: { monthlyJobPosts, budgetCapUsd } });
    return true;
  }
...
const seed_company: CompanyRecord = {
  ...
  role: 'recruiter' }],
  activity: [;
    { id: generate_id (), timestamp_iso: new Date ().toISOString (), actor_email: 'admin@acme.com',}
  action: 'created_company' },
    { id: generate_id (), timestamp_iso: new Date ().toISOString (), actor_email: 'maria@acme.com',}
  action: 'posted_job', meta: { job_id: 'job_123' } }],
  invoices: [;
