export type EnterprisePlanTier = 'teams' | 'business' | 'enterprise';

export interface EnterpriseUsageLimits {
...
export type EnterpriseRole = 'admin' | 'manager' | 'recruiter';
...
export interface CompanyRecord {
	...
	activity: Array<{ id: string; timestampIso: string; actorEmail: string; action: string; meta?: Record<string, unknown> }>;
...
const seedCompany: CompanyRecord = {
	...
	plan: {
		...
		seatsPurchased: 25,
		seatsUsed: 3,
		usageLimits: { monthlyJobPosts: 50, budgetCapUsd: 10000 },
	},
	members: [
...
export const store = {
	...
	createCompany(input: Partial<CompanyRecord>): CompanyRecord {
		...
		const record: CompanyRecord = {
			...
			slug,
			logoUrl: input.logoUrl || '/logo.svg',
	...
	removeMember(companyId: string, memberId: string): boolean {
		...
		company.members = company.members.filter((member) => member.id !== memberId);
