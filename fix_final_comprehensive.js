...
// All possible icons that might be used (excluding React components)
const allIcons = [
  ...
  'Calendar', 'Map', 'Target', 'Award', 'Trophy',
...
function fixFile(filePath) {
  try {
    ...
    // Check for icon usage in various patterns
    const iconPatterns = [
      /icon:\s*(\w+)/g,
      /<(\w+)\s/g,
      /\{(\w+)\}/g,
    ...
    for (const pattern of iconPatterns) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (iconName && iconName.charAt(0) === iconName.charAt(0).toUpperCase() && allIcons.includes(iconName)) {
...
console.log('All comprehensive issues fixed!');