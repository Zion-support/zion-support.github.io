...
# Batch script to merge all open PRs
set -e
...
for pr_number in $prs; do
    ...
    if git fetch origin pull/${pr_number}/head:pr-${pr_number} 2>/dev/null; then
        ...
        # Try to merge
        if git merge pr-${pr_number} --no-ff -m "Merge PR #${pr_number}: Fix errors and merge to main" 2>/dev/null; then
...
print_success "Batch PR merge process completed!"
