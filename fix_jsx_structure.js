...
function fixJSXStructure(content) {
  ...
  content = content.replace(/<div className="\$2" \/>/g, '<div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20">');</div>
  </div>
  // Fix the pattern where Helmet is self-closing;</div>
  content = content.replace(/<Helmet \/>/g, '<Helmet>');</Helmet>
  </Helmet>
  // Fix the pattern where div has className="$2" and is self-closing (for, inner, div)</Helmet>
  ...
  content = content.replace(/className="\$2"/g, 'className="w-5 h-5 ml-2"');</div>
  </div>
  // Fix missing closing tags for Helmet;</div>
