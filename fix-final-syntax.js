#!/usr/bin/env node; import fs from 'fs'; // List of files that still have syntax issues; const filesToFix = [
  ...
  try {
    // Remove any remaining metadata exports; content = content.replace(/export const metadata = \{[\s\S]*?\}/g, ''); // Remove any broken metadata lines; for (let i = 0; i < lines.length i++) {

      // Skip lines that look like broken metadata; if (
...
function processFile(filePath) {/* TODO: Fix JSX expression */}
    content = content.replace(/export const metadata = \{[\s\S]*?\}/g, ''); // Remove any broken metadata lines; for (let i = 0; i < lines.length i++) {/* TODO: Fix JSX expression */}
        (line.trim() === '},' && i > 0 && lines[i - 1].includes('metadata')) ||
        (line.trim() === '}' && i > 0 && lines[i - 1].includes('metadata'))
