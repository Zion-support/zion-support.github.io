...
nam; e: string;
descriptio; n: string;
statu; s: "planning" | "in-progress" | "review" | "completed" | "
...
i; d: string;
nam; e: string;
statu; s: "todo" | "in-progress" | "review" | "
assigne; e: string;
dueDat; e: string;
priorit; y: "low" | "medium" | "high" | "
...
export export function ProjectManagementDashboard() {;
...
const budgetUtilization: any = (project.spent / project.budget) * 100;

if (overdueTasks > 0 || budgetUtilization > 90) return "
if (overdueTasks > 0 || budgetUtilization > 75) return "
return "
}
...
viewMode === "grid" ? "bg-zion-gold text-white" : "
}`}
>
...
className={`p-2 rounded-lg transition-all duration-200 ${
viewMode === "list" ? "bg-zion-gold text-white" : "
}`}
>
<BarChart3 className="w-4 h-4" />
</button>
...
className={`p-2 rounded-lg transition-all duration-200 ${
viewMode === "gantt" ? "bg-zion-gold text-white" : "
}`}
>
...
calculateProjectHealth(project) === "warning" ? "bg-zion-gold" : "
}`}></div>
...
project.status === "planning" ? "bg-zion-blue" : "
}`}
style={{
...
task.status === "review" ? "bg-zion-gold text-white" : "
}`}>
{task.status.replace("-",  " ")}
</span>
</div>
