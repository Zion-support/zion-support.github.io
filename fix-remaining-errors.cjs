...
function fixRemainingErrors(content, filePath) {
  ...
  fixed = fixed.replace(/{\s*},\s*{\s*icon:\s*(\w+),\s*title:\s*'([^']+)',\s*}\s*\]/g, 
    `{
    icon: $1,
    title: '$2'
  }]`);
  ...
  if (fixed.includes('icon: BarChart') && !fixed.includes('const features')) {
...
const features = [`);
  ...
  // Fix missing closing brackets for benefits array
  if (fixed.includes('const benefits = [') && !fixed.includes(']')) {
    fixed = fixed.replace(/const benefits = \[\s*\]/, 'const benefits = []');
  }
  ...
  if (filePath.includes('ai-agricultural-intelligence-pro')) {
    // Fix the specific structure for this file
    fixed = fixed.replace(/import { Helmet } from 'react-helmet-async'\s*{\s*},\s*{\s*icon: BarChart,\s*title: 'Advanced Analytics',\s*}\s*\]/, 
      `import { Helmet } from 'react-helmet-async'
import { BarChart } from 'lucide-react'
