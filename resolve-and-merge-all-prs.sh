...
resolve_conflicts() {
    ...
    if grep -q "<<<<<<< HEAD" "$file"; then
        ...
        if [[ "$file" == "package.json" || "$file" == "package-lock.json" ]]; then
            ...
            log_message "📝 Documentation file detected, keeping both versions..."
    ...
    if git merge --no-commit --no-ff "origin/$branch" 2>/dev/null; then
        ...
        if git merge --no-commit --no-ff "origin/$branch" --strategy=recursive -X theirs 2>/dev/null; then
            ...
            if git merge --no-commit --no-ff "origin/$branch" --strategy=recursive -X ours 2>/dev/null; then
                ...
                log_message "⚠️  All automatic strategies failed, attempting manual conflict resolution..."
...
# Summary
echo ""
log_message "🎉 Comprehensive merge conflict resolution completed!"
log_message "📊 Summary:"
