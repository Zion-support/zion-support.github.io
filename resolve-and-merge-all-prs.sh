...
# Comprehensive script to resolve merge conflicts and merge all open PRs into main branch
set -e
...
# Get all remote branches that are not main, HEAD, backup, or merge branches
BRANCHES=$(git branch -r | grep "origin/" | grep -v "origin/main" | grep -v "origin/HEAD" | grep -v "backup" | grep -v "merge" | sed 's/origin\///' | sort)

echo "📋 Found $(echo "$BRANCHES" | wc -l) branches to process"
echo "---"

# Function to check if a branch has actual changes compared to main
has_changes() {
    local branch="$1"
    local commit_count=$(git log --oneline origin/main..origin/$branch 2>/dev/null | wc -l)
    [ "$commit_count" -gt 0 ]
}
...
resolve_conflicts() {
    ...
    if grep -q "        elif [[ "$file" == "next.config.js" || "$file" == "tsconfig.json" || "$file" == "vite.config.ts" ]]; then
...
for branch in $BRANCHES; do
    ...
    if ! can_merge_branch "$branch"; then
        echo "⏭️  Skipping $branch (already merged, doesn't exist, or no changes)"
