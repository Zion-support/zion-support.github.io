...
// Read the open PRs
const prs = JSON.parse(fs.readFileSync('open-prs.json', 'utf8'));

console.log(`Processing ${prs.length} open PRs...`);
...
for (const pr of prs) {
  ...
  try {
    ...
    try {
      execSync(`git merge ${branchName} --no-ff -m "Merge PR #${prNumber}: ${pr.title}"`, { stdio: 'inherit' });
      ...
      try {
        ...
        // Commit the merge
        execSync(`git commit -m "Resolve merge conflicts for PR #${prNumber}: ${pr.title}"`, { stdio: 'inherit' });
