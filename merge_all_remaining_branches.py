#!/usr/bin/env python3
"""
Comprehensive script to merge all remaining branches and resolve conflicts
"""
...
def get_all_branches() -> List[str]:
    ...
    result = run_command("git branch -r | grep -v 'origin/main' | grep -v 'origin/HEAD'")
...
def merge_branch_safely(branch: str) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"❌ Could not switch to main branch")
            return False
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️  Conflicts detected in {branch}, attempting auto-resolution...")
            
            # Check for conflict markers
            result = run_command("git status --porcelain")
            if "UU" in result.stdout or "<<<<<<<" in result.stdout:
                # Try to resolve conflicts by taking our version for most files
                result = run_command("git checkout --ours .", check=False)
                if result.returncode == 0:
...
def cleanup_merged_branches():
    ...
    for branch in branches:
        ...
        if branch and branch != 'main' and not branch.startswith('origin/'):
            print(f"Deleting merged branch: {branch}")
            run_command(f"git branch -d {branch}", check=False)
            cleaned_count += 1
    
    print(f"Cleaned up {cleaned_count} local branches")
...
def main():
    ...
    if not branches:
        print("ℹ️  No branches found to process")
        return
