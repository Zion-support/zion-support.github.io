...
function fixUnusedVariables(filePath) {
  try {
    ...
    content = content.replace(/(\s+)(const|let|var)\s+(\w+)\s*=\s*[^;]+;?\s*$/gm, (match, indent, declaration, varName) => {
      if (varName.startsWith('_') || varName === 'React' || varName === 'Component' || varName === 'useState' || varName === 'useEffect') {
        return match;
      }
    ...
    content = content.replace(/\((\w+),\s*(\w+),\s*(\w+)\)\s*=>/g, '(_$1, _$2, _$3) =>');
  ...
  try {
    ...
    if (lucideImports && lucideImports.length > 1) {
      ...
      lucideImports.forEach(importLine => {
        const matches = importLine.match(/import\s*{\s*([^}]+)\s*}\s*from\s*['"]lucide-react['"];?/);
        if (matches) {
