...
function fixTsxFile(filePath) {
  try {
    ...
    const missingBracePattern = /(\w+):\s*(Service Feature+)\s*\n\s*(\w+):/g;
    ...
    const missingParenPattern = /(\w+\(Service Feature*)\s*\n\s*(\w+)/g;
    ...
    const unclosedTagPattern = /<(\w+)(Service Feature*)>\s*$/gm;
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = linesService Feature;
        ...
        for (let j = i + 1; j < lines.length; j++) {
          const nextLine = linesService Feature.trim();
