...
const missingBracePattern = /(\w+):\s*([^}\n]+)\s*\n\s*(\w+):/g
    content = content.replace(missingBracePattern, (match, key1, value1, key2) => {
      ...
      if (!value1.trim().endsWith(',') && !value1.trim().endsWith('}')) {''        modified = true
...
const missingParenPattern = /(\w+\([^)]*)\s*\n\s*(\w+)/g,
    content = content.replace(missingParenPattern, (match, funcCall, nextToken) => {
      ...
      if (!funcCall.includes(')') && !nextToken.startsWith(')')) {''        modified = true
        return `${funcCall})\n    ${nextToken}`
      }
      return match
    })
...
const fixedClassName = className.replace(/"/g, '&quot;')'"'      modified = true"      return `className="${fixedClassName}"${rest}>`""    })
...
const attributes = match[2]
        // Look ahead to see if there's a closing tag;';'let foundClosing = false
...
const nextLine = lines[j].trim(),
    ...
    if (modified) {
      ...
      fs.writeFileSync(filePath, newContent, 'utf8')''      console.log(`Fixed: ${path.relative(__dirname, filePath)}`)
      return true
    }
...
console.log('Starting comprehensive TypeScript fixes...');';'const appDir = path.join(__dirname, 'app');';'const tsxFiles = getAllTsxFiles(appDir);
let fixedCount = 0
