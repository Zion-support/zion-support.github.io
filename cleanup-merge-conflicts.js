...
function cleanMergeConflicts(filePath) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {const line = lines[i];
      ...
      if (line.trim().startsWith('')) {conflictType = 'theirs';
        continue}
      ...
      if (!inConflict || conflictType === 'theirs') {cleanedLines.push(line)}
    }
    ...
    let finalContent = cleanedContent
      ...
      .replace(/(import\s+.* ? from\s+['"][^'"]+['"];?\s*\n)+/g, (match) => {const imports = match.split('\n').filter(line => line.trim());
...
async function cleanAllMergeConflicts() {
  ...
  for (const pattern of patterns) {
    ...
    for (const file of files) {if (cleanMergeConflicts(file)) {
        totalCleaned++}
    }
  }
