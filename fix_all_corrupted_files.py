#!/usr/bin/env python3
"""
Comprehensive script to fix all corrupted TypeScript/React files
"""
...
def is_corrupted_file(file_path):
    ...
    try:
        ...
        corruption_patterns = [
            r'const.*=.*\[\s*$',  # Incomplete array definitions
            r'    },\s*\n\s*},\s*\n\s*},\s*\n\s*},\s*\n\s*},\s*\n\s*},\s*\n\s*},\s*\n\s*}',  # Corrupted object arrays
            r'const.*=.*\[\s*$.*\n.*},\s*\n.*},\s*\n.*},\s*\n.*}',  # Another pattern
            r'<!--.*-->',  # HTML comments in JSX
            r'const.*Page.*=.*\(\).*=>\s*$',  # Incomplete function definitions
        ]
        ...
        if ('    },\n    \n    },\n    \n    },\n    \n    },\n    \n    },\n    \n    },\n    \n    }' in content or
            '<!--' in content or
            'const.*=.*[' in content and ']' not in content.split('const.*=.*[')[1].split('\n')[0]):
            return True
