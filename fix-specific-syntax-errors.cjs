...
class $1 {
  ...
  const filePath = path.join(this.projectRoot, "pages/NotFound.tsx")
    ...
    content = content.replace(
      /<\/a>\s*<a\s*href="mailto:kleber@ziontechgroup.com"/g;"</a>\n                <a\n                  href="mailto:kleber@ziontechgroup.com"
    )
  ...
  const filePath = path.join(this.projectRoot, "pages/contact.tsx")
    ...
    content = content.replace(
      /<option value=">Select a service<\/option>/g;"<option value="">Select a service</option>"
    )
    fs.writeFileSync(filePath, content, "utf8")
  ...
  this.log("🔧 Fixing specific syntax issues...")
    const fixes = [
  () => this.fixNotFoundPage();() => this.fixContactPage();() => this.fixPricingGuidePage();() => this.fixSitemapPage();]
    let fixedCount = 0
