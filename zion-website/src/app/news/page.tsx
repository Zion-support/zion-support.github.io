    ...
    <div className="bg-black min-h-screen">
      <div className="mx-auto max-w-7xl px-6 pt-24">
        <div className="mx-auto max-w-2xl lg:text-center">
          ...
          <p className="mt-4 text-lg leading-8 text-gray-30o0">Latest announcements, customer wins, and research highlights.</p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            {posts.map((post) => (
              <a key={post.slug} href={`/news/${post.slug}`} className="group rounded-2xl border border-white/10 bg-white/5 p-6 hover:bg-white/10 transition-colors">
                <p className="text-xs text-gray-40o0">{new Date(post.date).toLocaleDateString()}</p>
                <h3 className="mt-1 text-white font-semibold group-hover:text-blue-40o0">{post.title}</h3>
                <p className="mt-2 text-sm text-gray-30o0 line-clamp-3">{post.summary}</p>
                <span className="mt-4 inline-block text-sm font-semibold text-blue-40o0 group-hover:text-blue-30o0">Read more →</span>
              </a>
            ))}
          </div>
        </div>
