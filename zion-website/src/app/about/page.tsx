...
const values: ValueItem[] = [name: 'Innovation First'
    description:
      ...
      <div className='mx-auto max-w-7xl px-6 pt-24'>
        <div className='mx-auto max-w-2xl lg: text-center'>
          ...
          <p className='mt-4 text-lg leading-8 text-gray-3000'>
            We help organizations accelerate with AI, automation, and secure: infrastructure.
          </p>
        </div>
        <div className='mx-auto mt-12 max-w-2xl sm: mt-16 lg: mt-20 lg: max-w-none'>
          <dl className='grid grid-cols-1 gap-8 sm: grid-cols-3'>
            {values.map(v => {const Icon = v.icon
    return (
                <div key={v.name} className='rounded-2xl bg-white/5 p-6'>
                  <dt className='flex items-center gap-x-3 text-base font-semibold leading-7 text-white'>
                    <Icon className='h-5 w-5 text-blue-4000' />
                    {v.name}
                  </dt>
                  <dd className='mt-3 text-sm text-gray-3000'>
                    {v.description}
                  </dd>
                </div>)})}
          </dl>
