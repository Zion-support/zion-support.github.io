...
const navigation = [
  ...
  { name: 'Research', href: '/research', badge: 'New' },
          ...
          {navigation.map((item) => (
            ...
            >
              <span className="inline-flex items-center gap-2">
                {item.name}
                {item.badge ? (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-600/20 text-blue-300 border border-blue-500/30">{item.badge}</span>
                ) : null}
              </span>
            </a>
          ))}
