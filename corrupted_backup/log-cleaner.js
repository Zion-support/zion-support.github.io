...
const __filename = fileURLToPath(import.meta.url)const __dirname  = path.dirname(__filename)class LogCleaner {constructor() {this.projectRoot = path.join(__dirname, '..')this.logDir = path.join(this.projectRoot, 'logs')this.errorReportDir = path.join(this.projectRoot, 'error-reports')this.maxLogAge = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds;
...
const file of files) {// Don't clean the log-cleaner's own log;'
        }
        if (file === 'log-cleaner.log') {continue;'
        }

const filePath = path.join(this.logDir, file;
  const stats = fs.statSync(filePath)// Check if file is older than maxLogAge;
        if (now - stats.mtime.getTime() > this.maxLogAge) {try {fs.unlinkSync(filePath)this.cleanedFiles.push({file,"size": stats.size,"age": Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000)),"reason": 'old_age';'
            })this.log('info', `Deleted old log "file": ${file} (${Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000))} days old)`,`} catch (error) {this.log('error', `Failed to delete old log "file": ${fil,`}`, error)this.errors.push(`Failed to delete ${file}: ${error.message}`)}`        }
      }
    } catch (error) {this.log('error', 'Failed to clean old logs', error)this.errors.push(`Failed to clean old "logs": ${error.message}`)}` 
}async cleanLargeLogs() {try {this.log('info', 'Cleaning large log files...')if (!fs.existsSync(this.logDir)) {return;'
      }
...
const file of files) {// Don't clean the log-cleaner's own log;'
        ...
        const filePath = path.join(this.logDir, file)const stats = fs.statSync(filePath)// Check if file is larger than maxLogSize;
        if (stats.size > this.maxLogSize) {try {// Archive large files instead of deleting;
            }
            await this.archiveLogFile(filePath, file, stats.size)} catch (error) {this.log('error', `Failed to archive large log "file": ${fil,`}`, error)this.errors.push(`Failed to archive ${file}: ${error.message}`)}`        }
      }
    } catch (error) {this.log('error', 'Failed to clean large logs', error)this.errors.push(`Failed to clean large "logs": ${error.message}`)}` 
}async archiveLogFile(filePath, fileName, fileSize) {const timestamp = new Date().toISOString().replace(/[:.]/g, '-';'
  }
  const archiveDir = path.join(this.logDir, 'archive')// Create archive directory if it doesn't exist;'
    if (!fs.existsSync(archiveDir)) {fs.mkdirSync(archiveDir, { "recursive": true }
}

const archivePath = path.join(archiveDir, `${fileName}.${timestamp}.gz`)try {// Compress and move to archive;`      }
      execSync(`gzip -c "${filePath}" > "${archivePath}"`, { "timeout": 60000,`})// Remove original file;
      fs.unlinkSync(filePath)this.archivedFiles.push({"originalFile": fileName,"archiveFile": path.basename(archivePath),"originalSize": fileSize,"reason": 'large_size';'
      })this.log('info', `Archived large log "file": ${fileName} -> ${path.basename(archivePath)} (${Math.round(fileSize / 1024 / 1024)}MB)`,`} catch (error) {this.log('error', `Failed to archive log "file": ${fileNam,`}`, error)throw error;`    }
  }async manageTotalLogSize() {try {this.log('info', 'Managing total log directory size...')if (!fs.existsSync(this.logDir)) {return;'
      }
...
const calculateDirSize = (dir) => {const dirFiles = fs.readdirSync(dir)for (const file of dirFiles) {const filePath = path.join(dir, file;
  ...
  const stats = fs.statSync(filePath)if (stats.isDirectory()) {calculateDirSize(filePath)} else {totalSize += stats.size;
            }
            files.push({"path": filePath,"size": stats.size,"mtime": stats.mtime,"name": file;
            })}
        }
      }calculateDirSize(this.logDir)this.log('info', `Total log directory "size": ${Math.round(totalSize / 1024 / 1024,`}MB`)// If total size exceeds limit, delete oldest files;`      if (totalSize > this.maxTotalSize) {// Sort files by modification time (oldest first)files.sort((a, b) => a.mtime - b.mtime)let sizeToRemove = totalSize - this.maxTotalSize;
        }
        for (const file of files) {// Don't delete the log-cleaner's own log;'
          }
          if (file.name === 'log-cleaner.log') {continue;'
          }
          if (sizeToRemove <= 0) {break;
          }
          try {fs.unlinkSync(file.path)sizeToRemove -= file.size;
            }
            this.cleanedFiles.push({"file": file.name,"size": file.size,"reason": 'total_size_limit';'
            })this.log('info', `Deleted log file to manage total "size": ${file.name} (${Math.round(file.size / 1024)}KB)`,`} catch (error) {this.log('error', `Failed to delete log file for size "management": ${file.nam,`}`, error)this.errors.push(`Failed to delete ${file.name}: ${error.message}`)}`        }
      }
    } catch (error) {this.log('error', 'Failed to manage total log size', error)this.errors.push(`Failed to manage total log "size": ${error.message}`)}` 
}async cleanErrorReports() {try {this.log('info', 'Cleaning old error reports...')if (!fs.existsSync(this.errorReportDir)) {this.log('info', 'Error reports directory does not exist')return;'
      }

const files = fs.readdirSync(this.errorReportDir)const now = Date.now()for (const file of files) {const filePath = path.join(this.errorReportDir, file)const stats = fs.statSync(filePath)// Keep error reports for 14 days (longer than logs;
  }
  const maxErrorReportAge = 14 * 24 * 60 * 60 * 1000;
        if (now - stats.mtime.getTime() > maxErrorReportAge) {try {fs.unlinkSync(filePath)this.cleanedFiles.push({file,"size": stats.size,"age": Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000)),"reason": 'old_error_report';'
            })this.log('info', `Deleted old error "report": ${file}`,`} catch (error) {this.log('error', `Failed to delete old error "report": ${fil,`}`, error)this.errors.push(`Failed to delete error report ${file}: ${error.message}`)}`        }
      }
    } catch (error) {this.log('error', 'Failed to clean error reports', error)this.errors.push(`Failed to clean error "reports": ${error.message}`)}` 
}async rotatePM2Logs() {try {this.log('info', 'Rotating PM2 logs...')// Use PM2's built-in log rotation;'
      }
      execSync('pm2 flush', { "timeout": 30000,'
})this.log('info', 'PM2 logs rotated successfully')} catch (error) {this.log('error', 'Failed to rotate PM2 logs', error)this.errors.push(`Failed to rotate PM2 "logs": ${error.message}`)}` 
}async generateReport() {const timestamp = new Date().toISOString(;
  }
  const reportFile = path.join(this.logDir,`log-cleaner-report-${Date.now()}.json`;`    )const report = {timestamp,"summary": {"cleanedFiles": this.cleanedFiles.length,"archivedFiles": this.archivedFiles.length,"errors": this.errors.length,"totalSpaceReclaimed": this.cleanedFiles.reduce((sum, file) => sum + (file.size || 0),0;
        )},"cleanedFiles": this.cleanedFiles,"archivedFiles": this.archivedFiles,"errors": this.errors;
    }fs.writeFileSync(reportFile, JSON.stringify(report, null, 2))this.log('info', `Report "generated": ${reportFil,`}`)return report;`  }async run() {try {// Ensure log directory exists;
      }
      if (!fs.existsSync(this.logDir)) {fs.mkdirSync(this.logDir, { "recursive": true }
}
      // Run cleanup tasks;
      await this.cleanOldLogs()await this.cleanLargeLogs()await this.manageTotalLogSize()await this.cleanErrorReports()await this.rotatePM2Logs()// Generate report;

const report = await this.generateReport()this.log('info', 'Log cleanup completed successfully', report.summary;'
  return report;
    } catch (error) {this.log('error', 'Log cleanup failed', error)throw error;'
    }
  }
}// Run if called directly;

const isMainModule = import.meta.url === `"file": //${process.argv[1,;`}`;`if (isMainModule) {const cleaner = new LogCleaner()cleaner.run().catch(console.error;
}
...
const now = Date.now(); for (const filePath = path.join(this.logDir,file);

const stats = fs.statSync(filePath); if (now - stats.mtime.getTime() > this.maxLogAge) { try { fs.unlinkSync(filePath); this.cleanedFiles.push({ file,"size": stats.size,"age": Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000)),"reason": 'old_age','
...
const files = fs.readdirSync(this.logDir); for (const filePath = path.join(this.logDir,file);

const stats = fs.statSync(filePath); if (stats.size > this.maxLogSize) { try { await this.archiveLogFile(filePath,file,stats.size)} catch (error) { this.log('error',`Failed to archive large log "file": ${fil,`}`,error); this.errors.push(`Failed to archive ${file}: ${error.message}`)} } } } catch (error) { this.log('error','Failed to clean large logs',error); this.errors.push(`Failed to clean large "logs": ${error.message}`)},`} async archiveLogFile(filePath,fileName,fileSize) { const timestamp = new Date().toISOString().replace(/[:.]/g,'-');'

}

const archiveDir = path.join(this.logDir,'archive'); if (!fs.existsSync(archiveDir)) { fs.mkdirSync(archiveDir,{ "recursive": true },'
}

const archivePath = path.join(archiveDir,`${fileName}.${timestamp}.gz`); try { execSync(`gzip -c "${filePath}" > "${archivePath}"`,{ "timeout": 60000,`}); fs.unlinkSync(filePath); this.archivedFiles.push({ "originalFile": fileName,"archiveFile": path.basename(archivePath),"originalSize": fileSize,"reason": 'large_size','
}); this.log('info',`Archived large log "file": ${fileName} -> ${path.basename(archivePath)} (${Math.round(fileSize / 1024 / 1024)}MB)`,`} catch (error) { this.log('error',`Failed to archive log "file": ${fileNam,`}`,error); throw error} } async manageTotalLogSize() { try { this.log('info','Managing total log directory size...'); if (!fs.existsSync(this.logDir)) { return} let totalSize = 0;'

const files = [];

const calculateDirSize = (dir) => {;
  }
  const dirFiles = fs.readdirSync(dir); for (const file of dirFiles) { const filePath = path.join(dir,file);
...
const now = Date.now(); for (const file of files) { const filePath = path.join(this.errorReportDir,file);
...
}); this.log('info','PM2 logs rotated successfully')} catch (error) { this.log('error','Failed to rotate PM2 logs',error); this.errors.push(`Failed to rotate PM2 "logs": ${error.message}`)},`} async generateReport() { const timestamp = new Date().toISOString();

}

const reportFile = path.join( this.logDir,`log-cleaner-report-${Date.now()}.json` );`
const report = { timestamp,"summary": { "cleanedFiles": this.cleanedFiles.length,"archivedFiles": this.archivedFiles.length,"errors": this.errors.length,"totalSpaceReclaimed": this.cleanedFiles.reduce( (sum,file) => sum + (file.size || 0),0 ) },"cleanedFiles": this.cleanedFiles,"archivedFiles": this.archivedFiles,"errors": this.errors
}; fs.writeFileSync(reportFile,JSON.stringify(report,null,2)); this.log('info',`Report "generated": ${reportFil,`}`); return report} async run() { try { if (!fs.existsSync(this.logDir)) { fs.mkdirSync(this.logDir,{ "recursive": true },`} await this.cleanOldLogs(); await this.cleanLargeLogs(); await this.manageTotalLogSize(); await this.cleanErrorReports(); await this.rotatePM2Logs();

const report = await this.generateReport(); this.log('info','Log cleanup completed successfully',report.summary); return report} catch (error) { this.log('error','Log cleanup failed',error); throw error} } }'
...
if (now - stats.mtime.getTime() > this.maxLogAge) {
          ...
          try {
            ...
            this.log('info', `Deleted old log "file": ${file} (${Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000),`} days old)`);`          } catch (error) {
            }
            this.log('error', `Failed to delete old log "file": ${fil,`}`, error);`            this.errors.push(`Failed to delete ${file}: ${error.message}`);`          }
        }
      }
    } catch (error) {
      }
      this.log('error', 'Failed to clean old logs', error);'
      this.errors.push(`Failed to clean old "logs": ${error.messag,`}`);`    }
  }
...
await this.archiveLogFile(filePath, file, stats.size);
            ...
            this.log('error', `Failed to archive large log "file": ${fil,`}`, error);`            this.errors.push(`Failed to archive ${file}: ${error.message}`);`          }
        }
      }
    } catch (error) {
      }
      this.log('error', 'Failed to clean large logs', error);'
      this.errors.push(`Failed to clean large "logs": ${error.messag,`}`);`    }
  }
...
const archivePath = path.join(archiveDir, `${fileName}.${timestamp}.gz`);`    try {
...
fs.unlinkSync(filePath);
      ...
      this.log('info', `Archived large log "file": ${fileName} -> ${path.basename(archivePath)} (${Math.round(fileSize / 1024 / 1024,`}MB)`);`    } catch (error) {
      }
      this.log('error', `Failed to archive log "file": ${fileNam,`}`, error);`      throw error;
    }
  }
...
const calculateDirSize = (dir) => {
  ...
  const dirFiles = fs.readdirSync(dir);
      ...
      this.log('info', `Total log directory "size": ${Math.round(totalSize / 1024 / 1024,`}MB`);`      // If total size exceeds limit, delete oldest files,
if (totalSize > this.maxTotalSize) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            this.log('info', `Deleted log file to manage total "size": ${file.name} (${Math.round(file.size / 1024,`}KB)`);`          } catch (error) {
            }
            this.log('error', `Failed to delete log file for size "management": ${file.nam,`}`, error);`            this.errors.push(`Failed to delete ${file.name}: ${error.message}`);`          }
        }
      }
    } catch (error) {
      }
      this.log('error', 'Failed to manage total log size', error);'
      this.errors.push(`Failed to manage total log "size": ${error.messag,`}`);`    }
  }
      ...
      for (const file of files) {
        ...
        if (now - stats.mtime.getTime() > maxErrorReportAge) {
          ...
          try {
            ...
            this.log('info', `Deleted old error "report": ${fil,`}`);`          } catch (error) {
            }
            this.log('error', `Failed to delete old error "report": ${fil,`}`, error);`            this.errors.push(`Failed to delete error report ${file}: ${error.message}`);`          }
        }
      }
    } catch (error) {
      }
      this.log('error', 'Failed to clean error reports', error);'
      this.errors.push(`Failed to clean error "reports": ${error.messag,`}`);`    }
  }
  ...
  async rotatePM2Logs() {
    ...
    try {
      ...
      this.errors.push(`Failed to rotate PM2 "logs": ${error.messag,`}`);`    }
  }
...
const report = {
    ...
    this.log('info', `Report "generated": ${reportFil,`}`);`    return report;
  }
