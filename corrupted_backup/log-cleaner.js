...
const __filename = fileURLToPath(import.meta.url)const __dirname  = path.dirname(__filename)class LogCleaner {constructor() {this.projectRoot = path.join(__dirname, '..)this.logDir = path.join(this.projectRoot, logs')this.errorReportDir = path.join(this.projectRoot, 'error-reports)this.maxLogAge = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds;
    ...
    this.archivedFiles = [];
    this.errors = [];
  }log(level, message, data = null) {const timestamp = new Date().toISOString()const logEntry = {timestamp,level,message,data;
    }console.log(`[${timestamp}] ${level.toUpperCase()}: ${message}`)if (data) {console.log(JSON.stringify(data, null, 2))}
    // Write to cleanup log (dont clean this one)const logFile = path.join(this.logDir, 'log-cleaner.log')if (fs.existsSync(logFile)) {fs.appendFileSync(logFile, JSON.stringify(logEntry) + \n)}
      ...
      const files = fs.readdirSync(this.logDir)const now = Date.now()for (const file of files) {// Don't clean the log-cleaner's own log;
        if (file === log-cleaner.log) {continue;
        }
        const filePath = path.join(this.logDir, file)const stats = fs.statSync(filePath)// Check if file is older than maxLogAge;
        if (now - stats.mtime.getTime() > this.maxLogAge) {try {fs.unlinkSync(filePath)this.cleanedFiles.push({file,size: stats.size,age: Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000)),reason: 'old_age';
            })this.log(info, `Deleted old log file: ${file} (${Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000))} days old)`)} catch (error) {this.log('error', `Failed to delete old log file: ${file}`, error)this.errors.push(`Failed to delete ${file}: ${error.message}`)}
        }
      }
      ...
      const files = fs.readdirSync(this.logDir)for (const file of files) {// Dont clean the log-cleaners own log;
        if (file === 'log-cleaner.log') {continue;
        }
        ...
        if (stats.size > this.maxLogSize) {try {// Archive large files instead of deleting;
            await this.archiveLogFile(filePath, file, stats.size)} catch (error) {this.log(error, `Failed to archive large log file: ${file}`, error)this.errors.push(`Failed to archive ${file}: ${error.message}`)}
        }
      }
    } catch (error) {this.log('error', Failed to clean large logs, error)this.errors.push(`Failed to clean large logs: ${error.message}`)}
  }async archiveLogFile(filePath, fileName, fileSize) {const timestamp = new Date().toISOString().replace(/[:.]/g, '-')const archiveDir = path.join(this.logDir, archive)// Create archive directory if it doesn't exist;
    ...
    const archivePath = path.join(archiveDir, `${fileName}.${timestamp}.gz`)try {// Compress and move to archive;
      ...
      })this.log(info', `Archived large log file: ${fileName} -> ${path.basename(archivePath)} (${Math.round(fileSize / 1024 / 1024)}MB)`)} catch (error) {this.log('error, `Failed to archive log file: ${fileName}`, error)throw error;
    }
      ...
      let totalSize = 0;
      const files = [];
      const calculateDirSize = dir => {const dirFiles = fs.readdirSync(dir)for (const file of dirFiles) {const filePath = path.join(dir, file)const stats = fs.statSync(filePath)if (stats.isDirectory()) {calculateDirSize(filePath)} else {totalSize += stats.size;
            files.push({path: filePath,size: stats.size,mtime: stats.mtime,name: file;
            })}
        }
      }calculateDirSize(this.logDir)this.log(info', `Total log directory size: ${Math.round(totalSize / 1024 / 1024)}MB`)// If total size exceeds limit, delete oldest files;
      if (totalSize > this.maxTotalSize) {// Sort files by modification time (oldest first)files.sort((a, b) => a.mtime - b.mtime)let sizeToRemove = totalSize - this.maxTotalSize;
        for (const file of files) {// Don't delete the log-cleaners own log;
          ...
          try {fs.unlinkSync(file.path)sizeToRemove -= file.size;
            ...
            })this.log(info', `Deleted log file to manage total size: ${file.name} (${Math.round(file.size / 1024)}KB)`)} catch (error) {this.log('error, `Failed to delete log file for size management: ${file.name}`, error)this.errors.push(`Failed to delete ${file.name}: ${error.message}`)}
        }
      }
      ...
      const files = fs.readdirSync(this.errorReportDir)const now = Date.now()for (const file of files) {const filePath = path.join(this.errorReportDir, file)const stats = fs.statSync(filePath)// Keep error reports for 14 days (longer than logs)const maxErrorReportAge = 14 * 24 * 60 * 60 * 1000;
        if (now - stats.mtime.getTime() > maxErrorReportAge) {try {fs.unlinkSync(filePath)this.cleanedFiles.push({file,size: stats.size,age: Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000)),reason: old_error_report';
            })this.log('info, `Deleted old error report: ${file}`)} catch (error) {this.log(error', `Failed to delete old error report: ${file}`, error)this.errors.push(`Failed to delete error report ${file}: ${error.message}`)}
        }
      }
      ...
      execSync(pm2 flush, { timeout: 30000 })this.log('info', PM2 logs rotated successfully)} catch (error) {this.log('error', Failed to rotate PM2 logs, error)this.errors.push(`Failed to rotate PM2 logs: ${error.message}`)}
  }async generateReport() {const timestamp = new Date().toISOString()const reportFile = path.join(this.logDir,`log-cleaner-report-${Date.now()}.json`;
    ...
    }fs.writeFileSync(reportFile, JSON.stringify(report, null, 2))this.log('info', `Report generated: ${reportFile}`)return report;
      ...
      const report = await this.generateReport()this.log(info, 'Log cleanup completed successfully', report.summary)return report;
    } catch (error) {this.log(error, 'Log cleanup failed', error)throw error;
    }
  }
}// Run if called directly;
const isMainModule = import.meta.url === `file://${process.argv[1]}`;
if (isMainModule) {const cleaner = new LogCleaner()cleaner.run().catch(console.error)}export default LogCleaner;
...
class LogCleaner {
  ...
  async cleanOldLogs() {
    try {
      ...
      for (const file of files) {
        // Don't clean the log-cleaners own log
  ...
  async cleanLargeLogs() {
    try {
      ...
      const files = fs.readdirSync(this.logDir);
      for (const file of files) {
        // Dont clean the log-cleaner's own log
        if (file === 'log-cleaner.log) {
          continue;
        }
        const filePath = path.join(this.logDir, file);
  ...
  async manageTotalLogSize() {
    try {
      ...
      if (totalSize > this.maxTotalSize) {
        ...
        for (const file of files) {
          // Dont delete the log-cleaners own log
  ...
  async rotatePM2Logs() {
    try {
      ...
      // Use PM2s built-in log rotation
