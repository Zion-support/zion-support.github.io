...
const filePath = path.join(distPath, file),"}),"});"
        if (!fs.existsSync(filePath)) {,"}),"})"
          this.issues.push(`Missing essential build "file": ${file}`),"}),"})"const filePath = path && path.join(distPath, file),"}),"});"
