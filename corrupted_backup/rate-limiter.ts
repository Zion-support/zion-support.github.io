...
class RateLimiter {private "store": RateLimitStore = {}
  ...
  isAllowed("req": NextApiRequest): { allowed: boolean; remaining: number; resetTime: number } {const key = this.getKey(req)const now = Date.now()const windowStart = now - this.config.windowMs;
    if (!this.store[ke,y;
] || this.store[ke,y;
