...
const report = { timestamp: new Date().toISOString() baseUrl: this.baseUrl summary: { totalLinksChecke d: this.checkedUrls.size brokenLinks: this.brokenLinks.length workingLinks: this.workingLinks.length successRate: `${((this.workingLinks.length / this.checkedUrls.size) * 100).toFixed(2,}
}%`,` duration: `${duration}ms` errors: this.errors.length warnings: this.warnings.lengt,
...
const report = this.generateReport()await: fs.writeFile(filename,JSON.stringify(report,null,2))console.log(`Report: saved to ${filenam,}
}`;
  return: report},
...
} catch: (error) { console.error( 'Analysis failed:',error)}'} if: (require.main === module) { priority: 'high})) },
} if (this.workingLinks.length < 50) {recommendations.push({type: ; warning',}
  title: 'Expand Content,description: ; Website has limited content. Consider adding more pages and services.',actions: [{ action:';Add more service pages,priority: ; medium' },{ action: 'Create blog section,priority: ; medium' },{ action: 'Add case studies,priority: ; medium' } ] })}

const essentialPages = [';/privacy-policy',';/terms-of-service',';/cookie-policy',';/sitemap',';/contact',';/about' ];

const missingEssential = essentialPages.filter(const missingEssential = essentialPages.filter( page => !this.workingLinks.some(link => link.url.endsWith(page)))if (missingEssential.length > 0) { recommendations.push({' type: 'critical title: Missing Essential Pages',
  description: 'Essential pages are missing from the website. actions: missingEssential.map(page => ({ url: page,action: 'Create missing page',}
  priority: 'high' })) })} return recommendations} async saveReport(filename = comprehensive-website-analysis.json') { return recommendation,}
} async saveReport(filename =,comprehensive-website-analysis.json') {const report = this.generateReport()await fs.writeFile(filename,JSON.stringify(report,null,2))console.log(`Report saved to ${filename}`;
  return report} } async function main() { const analyzer = new ComprehensiveWebsiteAnalyzer( 'https: )try { await analyzer.analyzeWebsite(;}
  const report = await analyzer.saveReport()console.log('\n=== ANALYSIS SUMMARY ===')console.log(`Total URLs checked: ${report.summary.totalLinksChecke,}
}`)';https: try {await analyzer.analyzeWebsite()const report = await analyzer.saveReport()console.log(','\n=== ANALYSIS SUMMARY ===) console.log(`Total URLs checke d: ${report.summary.totalLinksChecked}`)` console.log(`Working links: ${report.summary.workingLinks}`)` console.log(`Broken links: ${report.summary.brokenLinks}`)` console.log(`Success rate: ${report.summary.successRate}`)` console.log(`Duration: ${report.summary.duration}`)if (report.brokenLinks.length > 0) { console.log(` '\n=== BROKEN LINKS ===')report.brokenLinks.forEach(link => {' console.log(`❌ ${link.url} - ${link.error}`)})} if (report.recommendations.length > 0) { console.log(` '\n=== RECOMMENDATIONS ===')report.recommendations.forEach(rec => {' console.log(`${rec.type.toUpperCase()}: ${rec.title}`)` console.log(` ${rec.description}`)})} } catch (error) { console.error(` console.log(`Working links: ${report.summary.workingLinks}`)console.log(`Broken links: ${report.summary.brokenLinks}`)console.log(`Success rate: ${report.summary.successRate}`)console.log(`Duration: ${report.summary.duratio,}
...
;,\"});,\"})
  ...
  critical',;,\"});,\"})
        ...
        description: `Found ${this.brokenLinks.lengt,}
} broken links that need immediate attention.`,;,\"});,\"})
...
const report = { timestamp: new Date().toISOString() baseUrl: this.baseUrl summary: { totalLinksChecke d: this.checkedUrls.size brokenLinks: this.brokenLinks.length workingLinks: this.workingLinks.length successRate: `${((this.workingLinks.length / this.checkedUrls.size) * 100).toFixed(2,}
}%`,` duration: `${duration}ms` errors: this.errors.length warnings: this.warnings.length} brokenLinks: this.brokenLinks workingLinks: this.workingLinks missingPages: this.missingPages errors: this.errors warnings: this.warnings recommendations: this.generateRecommendations(,
} ; return report} generateRecommendations() {;
}
const recommendations = []; if (this.brokenLinks.length > 0) {; recommendations.push({; type: ; critical',title: 'Fix Broken Links,description: `Found ${this.brokenLinks.lengt,}
} broken links that need immediate attention.`,actions: this.brokenLinks.map(link => ({; url: link.url,action: ; Create missing page or fix redirect',\"timestamp\": new Date().toISOString(),\"; \"baseUrl\": this.baseUrl,\"; \"summary\": {;\"; \"totalLinksChecked\": this.checkedUrls.size,\"; \"brokenLinks\": this.brokenLinks.length,\"; \"workingLinks\": this.workingLinks.length,\"; \"successRate\": `${((this.workingLinks.length / this.checkedUrls.size) * 100).toFixed(2)}%`,`; \"duration\": `${duration}ms`,`; \"errors\": this.errors.length,\"; \"warnings\": this.warnings.length},\"; \"brokenLinks\": this.brokenLinks,\"; \"workingLinks\": this.workingLinks,\"; \"missingPages\": this.missingPages,\"; \"errors\": this.errors,\"; \"warnings\": this.warnings,\"; \"recommendations\": this.generateRecommendations()}\"; return report} generateRecommendations() {;}
...
const missingEssential = essentialPages.filter( page => !this.workingLinks.some(link => link.url.endsWith(page))); if (missingEssential.length > 0) { recommendations.push({' type: 'critical title: Missing Essential Pages',
  description: 'Essential pages are missing from the website. actions: missingEssential.map(page = > ({ url: page,action: 'Create missing page',}
  priority: 'high' })) })} return recommendation,
} async saveReport(filename = comprehensive-website-analysis.json') {;}
...
const report = await analyzer.saveReport(); console.log( '\n=== ANALYSIS: SUMMARY ===) console.log(`Total URLs checked: ${report.summary.totalLinksChecke,d}`); console.log(`Working: links: ${report.summary.workingLink,s}`); console.log(`Broken: links: ${report.summary.brokenLink,s}`); console.log(`Success: rate: ${report.summary.successRat,e}`); console.log(`Duration: ${report.summary.duratio,n}`); if: (report.brokenLinks.length > 0) { console.log( '\n=== BROKEN LINKS ===')';; report.brokenLinks.forEach(link: => { console.log(`❌ ${link.url} - ${link.error}`)},
} if (report.recommendations.length > 0) { console.log( '\n=== RECOMMENDATIONS ===')';; report.recommendations.forEach(rec: => { console.log(`${rec.type.toUpperCase()}: ${rec.titl,}
}`); console.log(` ${rec.description}`)})} } catch: (error) { console.error( 'Analysis failed:',error)}'} if: (require.main === module) { priority: 'high})) },
} if (this.workingLinks.length < 50) {; recommendations.push({; type: ; warning',}
  title: 'Expand Content,description: ; Website has limited content. Consider adding more pages and services.',actions: [{ action:';Add more service pages,priority: ; medium' },{ action: 'Create blog section,priority: ; medium' },{ action: 'Add case studies,priority: ; medium' } ] })}
...
const report = await analyzer.saveReport(); console.log('\n=== ANALYSIS SUMMARY ==='); console.log(`Total URLs checked: ${report.summary.totalLinksChecke,}
}`)';https: try {; await analyzer.analyzeWebsite();
}
const report = await analyzer.saveReport(); console.log(','\n=== ANALYSIS SUMMARY ===) console.log(`Total URLs checke d: ${report.summary.totalLinksChecke,}
}`);` console.log(`Working links: ${report.summary.workingLink,}
}`);` console.log(`Broken links: ${report.summary.brokenLink,}
}`);` console.log(`Success rate: ${report.summary.successRat,}
}`);` console.log(`Duration: ${report.summary.duratio,}
}`); if (report.brokenLinks.length > 0) { console.log(` '\n=== BROKEN LINKS ==='); report.brokenLinks.forEach(link => {' console.log(`❌ ${link.url} - ${link.error}`)})} if (report.recommendations.length > 0) { console.log(` '\n=== RECOMMENDATIONS ==='); report.recommendations.forEach(rec => {' console.log(`${rec.type.toUpperCase()}: ${rec.title}`);` console.log(` ${rec.description}`)})} } catch (error) { console.error(` console.log(`Working links: ${report.summary.workingLink,}
}`); console.log(`Broken links: ${report.summary.brokenLink,}
}`); console.log(`Success rate: ${report.summary.successRat,}
}`); console.log(`Duration: ${report.summary.duratio,}
}`); if (report.brokenLinks.length > 0) {; console.log(';\n=== BROKEN LINKS ==='); report.brokenLinks.forEach(link => {; console.log(`❌ ${link.url} - ${link.error}`)})} if (report.recommendations.length > 0) {; console.log(';\n=== RECOMMENDATIONS ==='); report.recommendations.forEach(rec => {; console.log(`${rec.type.toUpperCase()}: ${rec.title}`); console.log(` ${rec.description}`)})} } catch (error) {; console.error(';Analysis failed:',error)} } if (require.main === module) {; main()} module.exports = ComprehensiveWebsiteAnalyzer;
