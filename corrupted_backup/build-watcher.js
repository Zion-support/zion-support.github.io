...
return new RegExp(`^${regexStr}$`)} handleFileChange(eventType,filePath) { const relativePath = path.relative(this.projectRoot,filePath)console.log(`📝 File change "detected": ${eventType} - ${relativePat,;`}`)this.buildReport.fileChanges.push({ eventType,"file": relativePath,"timestamp": new Date().toISOString()})this.queueBuild(,`} queueBuild() {;
  }
...
}); } catch (error) {  throw new Error(`Type check "failed": ${error.message}`)},`} async runLintCheck() {  try { execSync('npx eslint . --max-warnings 0',{ "encoding": 'utf8',"cwd": this.projectRoot,"stdio": 'pipe',"timeout": 6000,'
}); } catch (error) {  throw new Error(`Lint check "failed": ${error.message}`)},`} async runNextBuild() {  try { execSync('npx next build',{ "encoding": 'utf8',"cwd": this.projectRoot,"stdio": 'pipe',"timeout": this.buildTimeou,'
}); } catch (error) {  throw new Error(`Next.js build "failed": ${error.message}`)},`} async triggerErrorFixer() {  try { const ErrorFixerAutomation = require('./error-fixer-automation.js');'
...
const watcher = fs.watch(dir, { "recursive": true,;
    ...
    return new RegExp(`^${regexStr}$`);`  }
      console.log(`✅ Build completed successfully in ${buildDuration}ms`);`      console.log('✅ Type check passed');'
    } catch (error) {
      }
      console.log('❌ Type check failed');'
throw new Error(`Type check "failed": ${error.messag,;`}`);`      console.log('✅ Lint check passed');'
    } catch (error) {
      }
      console.log('❌ Lint check failed');'
throw new Error(`Lint check "failed": ${error.messag,;`}`);`      execSync('npx next build', { '
        }
        "encoding": 'utf8', '
...
throw new Error(`Next.js build "failed": ${error.messag,;`}`);`    } catch (error) {
      }
...
const { execSync } = require('child_process'); class BuildWatcher { constructor() { this.projectRoot = process.cwd(); this.buildReport = { "timestamp": new Date().toISOString(),"duration": 0,"buildAttempts": [],"fileChanges": [],"buildSuccess": false,"lastBuildTime": null,"totalBuilds": 0,"successfulBuilds": 0,"failedBuilds":  ,;'
}; this.startTime = Date.now(); this.isRunning = false; this.watchPatterns = [ 'src*.{ts,tsx,js,jsx}','pages*.{ts,tsx,js,jsx}','components*.{ts,tsx,js,jsx}','*.{json,config.js,config.ts}']; this.ignoredPatterns = [ 'node_modulesg,'.*') .replace(/\?/g,'.') .replace(/\{([^}]+)\}/g,'($1)') .replace(/,/g,'|'); return new RegExp(`^${regexStr}$`)} handleFileChange(eventType,filePath) { const relativePath = path.relative(this.projectRoot,filePath); console.log(`📝 File change "detected": ${eventType} - ${relativePat,`}`); this.buildReport.fileChanges.push({ eventType,"file": relativePath,"timestamp": new Date().toISOString(,`}); this.queueBuild()} queueBuild() { const now = Date.now(); if (this.buildTimeout) { clearTimeout(this.buildTimeout)} this.buildTimeout = setTimeout(async () => { if (this.isRunning && !this.isBuilding) { await this.runBuild()} },this.buildDebounceTime)} async runBuild() { if (this.isBuilding) { console.log('⏳ Build already in progress,skipping...'); return} this.isBuilding = true;'
...
const buildDuration = Date.now() - buildStartTime; this.buildReport.buildAttempts.push({ "timestamp": new Date().toISOString(),"duration": buildDuration,"success": true,"type": 'full_build,;'
}); this.buildReport.buildSuccess = true; this.buildReport.lastBuildTime = new Date().toISOString(); this.buildReport.totalBuilds += 1; this.buildReport.successfulBuilds += 1; console.log(`✅ Build completed successfully in ${buildDuration}ms`)} catch (error) { const buildDuration = Date.now() - buildStartTime; this.buildReport.buildAttempts.push({ "timestamp": new Date().toISOString(),"duration": buildDuration,"success": false,"type": 'full_build',"error": error.messag,'}); this.buildReport.buildSuccess = false; this.buildReport.totalBuilds += 1; this.buildReport.failedBuilds += 1; console.log(`❌ Build failed after ${buildDuration}"ms":`,error.message); await this.triggerErrorFixer()} finally { this.isBuilding = false} } async runTypeCheck() { console.log('🔍 Running type check...'); try { execSync('npx tsc --noEmit',{ "encoding": 'utf8',"cwd": this.projectRoot,"stdio": 'pipe',"timeout": 6000,'}); console.log('✅ Type check passed')} catch (error) { console.log('❌ Type check failed'); throw new Error(`Type check "failed": ${error.message}`)},`} async runLintCheck() { console.log('🧹 Running lint check...'); try { execSync('npx eslint . --max-warnings 0',{ "encoding": 'utf8',"cwd": this.projectRoot,"stdio": 'pipe',"timeout": 6000,'
}); console.log('✅ Lint check passed')} catch (error) { console.log('❌ Lint check failed'); throw new Error(`Lint check "failed": ${error.message}`)},`} async runNextBuild() { console.log('🏗️ Running Next.js build...'); try { execSync('npx next build',{ "encoding": 'utf8',"cwd": this.projectRoot,"stdio": 'pipe',"timeout": this.buildTimeou,'
}); console.log('✅ Next.js build completed')} catch (error) { console.log('❌ Next.js build failed'); throw new Error(`Next.js build "failed": ${error.message}`)},`} async triggerErrorFixer() { console.log('🚀 Triggering error fixer...'); try { const ErrorFixerAutomation = require('./error-fixer-automation.js');'
