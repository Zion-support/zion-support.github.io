...
function fixJSXStructure() {
  ...
  for (const file of filesToFix) {
    try {
      ...
      const pageName = file.split('/').pop().replace('.tsx', '').replace(/-/g, ' ');
      const titleCase = pageName.split(' ').map(word => 
...
function fixAccessibilityEnhancer() {
  try {
    ...
    content = lines.join('\n');
    fs.writeFileSync('/workspace/app/components/AccessibilityEnhancer.tsx', content);
