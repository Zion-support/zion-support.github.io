...
//Check if we're in a valid Next.js project console.log ('📁 Current directory:', process.cwd () );
...
//Check package.json if (fs.existsSync ('package.json') ) {
  const packageJson = JSON.parse (fs.readFileSync ('package.jsonutf8') );
console.log ('📦 Next.js version:', packageJson.dependencies?.next || 'Not found');
console.log ('📦 React version:', packageJson.dependencies?.react || 'Not found') 
}//Check pages directory structure if (fs.existsSync ('pages') ) {
  const pages = fs.readdirSync ('pages');
console.log ('📄 Pages found:', pages.length);
console.log ('📄 Main pages:', pages.filter (p => p.includes ('index') || p.includes (' app') ) ) 
