...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = contentFiles
    ...
    .filter(p => {
      ...
      const potentialPagePathTs = path.join(
        mainPagesDir,
        `${p.params.slug}.tsx`
      );
      const potentialPagePathJs = path.join(
        mainPagesDir,
        `${p.params.slug}.js`
      );
      const potentialPagePathJsx = path.join(
        mainPagesDir,
        `${p.params.slug}.jsx`
      ); // Added jsx check

      if (
        fs.existsSync(potentialPagePathTs) ||
        fs.existsSync(potentialPagePathJs) ||
        fs.existsSync(potentialPagePathJsx)
      ) {
