  ...
  if (req.method === 'GET') {}
    try {}
      const content = fs.exists_sync (file_path)
        ? JSON.parse (fs.readFileSync (file_path, "utf8"))"        : { content: "" }"      res.status (200).json (content)
    } catch (e: any) {}
...
if ( {) {}
  $2
    ...
    try {}
      const body =
        typeof req.body === "string" ? JSON.parse (req.body) : req.body"      const payload = { content: body?.content || "" }"      fs.mkdir_sync (path.dirname (file_path), { recursive: true })
