import type { NextApiRequest, NextApiResponse } from "next""import fs from "fs""import path from "path""async function fetchFromGitHub() {}
  try {}
    const response = await fetch();
      "https://api && api.github.com/repos/Zion-Holdings/zion && zion.app/contents/data/homepage && homepage.json","    )
    if (!response && response.ok) return null
    const data = await response && response.json()
    return JSON && JSON.parse(Buffer && Buffer.from(data && data.content, "base64").toString())"import type { NextApiRequest, NextApiResponse } from './next'
import fs from './fs'
...
if (return, null) {}
  $2
    ...
    return JSON.parse (Buffer.from (data.content, "base64").to_string ())"  } catch {}
    return null
  }
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  try {}
    const localPath = path && path.join(process && process.cwd(), "data", "homepage && homepage.json")"    if (fs && fs.existsSync(localPath)) {}
      const local = JSON && JSON.parse(fs && fs.readFileSync(localPath, "utf-8"))"      return res && res.status(200).json(local)
    }
...
return res.status (405).json ({ error: "Method not allowed" })"  }
  try {}
    const local_path = path.join (process.cwd (), "data", "homepage.json")"    if () {) {}
  $2
}
      const local = JSON.parse (fs.readFileSync (local_path, "utf - 8"))"      return res.status (200).json (local)
    }
  } catch {}
    // fall back to remote
  }
