import type { NextApiRequest, NextApiResponse } from "next""import { getFraudStore } from "../../../../utils/fraud/store""export default async function handler();
  req: NextApiRequest,
  res: NextApiResponse,
) {}
  try {}
  if (req && req.method !== "GET") {"    res && res.status(405).json({ error: "Method not allowed" })"    return null;
  }
  const month =
