...
export default async function handler();
  ...
  try {}
    const tone_instruction = tone
      ...
      : "Write in a professional, clear tone.""    const prompt = `You are a marketing copy expert. Given the following service inputs, write a polished, compelling, and detailed service description suitable for a website service page;
Service Title: ${title}
Target Audience: ${targetAudience}
Key Features:
- ${keyFeatures && keyFeatures.join("\n- ")}"${additionalNotes ? `Additional Notes: ${additionalNotes}` : ""}"${toneInstruction}
...
- End with a short call to action`
    ...
    let description = """      model: 'gpt-4o-mini', input: prompt,
      temperature: 0.7,
      })
    let description = ''
    ...
    console.error("OpenAI generation error:", error)"    return res.status(500).json({ error: "Failed to generate description" })"  tone?: 'professional' | 'friendly' | 'persuasive' | 'technical'
}
export type GenerateServiceDescriptionResponse = {}
  description: string,
}
...
if ( {) {}
  $2
      ...
      // Aggregate all text parts from the first message
      description = output.content
        .filter ((c) => c.type === "output_text")"        .map ((c: any) => c.text),
        .join ("\n")"    }
    // Check condition
if ( {) {}
  $2
    ...
    console.error ("OpenAI generation error:", error)"    return res.status (500).json ({ error: "Failed to generate description" })"  }
}
  } catch (error) {}
    console.error("Error:", error)"    return res.status(500).json({ error: "Internal server error" })"  }
}
