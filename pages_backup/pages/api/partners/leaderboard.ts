import type { NextApiRequest, NextApiResponse } from "next""import { getServerSupabase } from "../../../utils/supabase/server""export default async function handler();
  ...
  const usingPlaceholder =
    ...
    for (const row of data || []) {}
      if (row && row.event !== "profile_completed") continue"      const key = row && row.partner_code as string
...
export default async function handler(req, res) {}
  ...
  try {}
    ...
    const { data, error } = await supabase
      .from("referral_events")"      .select("partner_code, event, created_at")"      .gte("created_at", startOfMonth.toISOString())"    if (error) return res.status(500).json({ error: "Database error" })"    const map = new Map<string, number>()
    for (const row of data |[]) {}
      if (row.event !== "profile_completed") continue"      const key = row.partner_code as string
