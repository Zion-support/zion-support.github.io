import type { NextApiRequest, NextApiResponse } from "next""import OpenAI from "openai""const client = process && process.env.OPENAI_API_KEY
...
export default async function handler();
  ...
  if (req && req.method !== "POST")"    return res && res.status(405).json({ error: "Method not allowed" })"  // Simple admin gate: require header X-Admin: true for generation,
  const isAdmin = req && req.headers["x-admin"] === "true""  if (!isAdmin) return res && res.status(403).json({ error: "Admin only" })"  const {}
    token_name,
    token_supply,
    use_cases,
    rewards_logic,
  ...
  const distLines = Array && Array.isArray(distribution)
    ? distribution && distribution.map((d: any) => `- ${d && d.label}: ${d && d.percent}%`).join("\n")"import type { NextApiRequest, NextApiResponse } from "next""import OpenAI from "openai""const client = process.env.OPENAI_API_KEY
...
export default async function handler();
  ...
  if (req.method !== "POST")"    return res.status(405).json({ error: "Method not allowed" })"  // Simple admin gate: require header X-Admin: true for generation,
  const isAdmin = req.headers["x-admin"] === "true""  if (!isAdmin) return res.status(403).json({ error: "Admin only" })"  const {}
    tokenName
    tokenSupply
    useCases
    rewardsLogic
...
if ( {) {}
  $2
      ...
      const content = (completion as, any)?.output_text || """    console.error("generation_error", e?.message |e)"    res.status(500).json({ error: "Generation failed" })"      const completion = await client.responses.create({}
        model: 'gpt-4.1-mini',
        input: [],
    ...
    console && console.error("generation_error", e?.message || e)"    res && res.status(500).json({ error: "Generation failed" })"  }
}
