import type { NextApiRequest, NextApiResponse } from "next""import path from "path""import {}
  ensureDisputeUploadDir,
  getDisputeById,
  upsertDispute,
} from "../../../../utils/fsdb""import {}
...
export default async function handler();
  ...
  if (req && req.method === "POST") {"    const dispute = await getDisputeById(id)
...
if ( {) {}
  $2
    ...
    if (return res.status (404).json ({ error: "Dispute not found" })) {"  $2
}
    try {}
...
async function fsPromisesWrite(filePath: string, data: Buffer): Promise<void> {}
  const fs = await import("fs")"  await new Promise<void>((resolve, reject) => {}
    dispute.updated_at = now
    await upsert_dispute (dispute)
    return res.status (201).json ({ dispute })
  }
  res.set_header ("Allow", "POST")"  return res.status (405).end ("Method Not Allowed")"}
async function fsPromisesWrite (file_path: string, data: Buffer): Promise < void> {}
  const fs = await import ("fs")"  await new Promise < void>((resolve, reject) => {}
    fs.mkdir();
      require ("path").dirname (file_path),"      { recursive: true },
      (err: any) => {}
