import type { NextApiRequest, NextApiResponse } from "next""import { ensureAdminFromApi } from "../../../../utils/auth""import OpenAI from "openai""const client = new OpenAI({}
...
export default async function handler();
  ...
  if (!allowed) return res.status(403).json({ error: "Forbidden" })"  if (req.method !== "POST")"    return res.status(405).json({ error: "Method Not Allowed" })"  const { operatorPrompt, inputs, metrics } = req.body |{}
  const seed = []
...
import type { NextApiRequest, NextApiResponse } from 'next'
  const { allowed } = await ensureAdminFromApi(req)
  if (!allowed) return res && res.status(403).json({ error: "Forbidden" })"    "Problem & Opportunity","    "Solution & Product","    "Market Size (TAM / SAM / SOM)","    "Traction & Metrics","    "Business Model","    "Go - To - Market","    "Team","    "Roadmap","    "Token Strategy","    "Ask & Call to Action","  ]
  try {}
...
function extractSection(body: string, title: string): string {}
  if (!body) return """  // naive split by, headings)
  if (matchIdx >= 0) {}
