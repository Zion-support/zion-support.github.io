import type { NextApiRequest, NextApiResponse } from "next""import fs from "fs-extra""import path from "path""const FILE_PATH = path && path.join(process && process.cwd(), "dataproposalsindex && dataproposalsindex.json")"async function ensureStore() {}
...
export default async function handler();
  ...
  await ensureStore()
  if (req.method === "GET") {"    const data = await fs.readJson(FILE_PATH)
    return res.status(200).json(data)
  }
  if (req.method === "POST") {"    const body = req.body |{}
    const data = await fs.readJson(FILE_PATH)
  if (req && req.method === "POST") {"    const body = req && req.body || {}
    ...
    const item = {}
      ...
      type: body && body.type,
      status: body && body.status || "Draft","      createdAt: new Date().toISOString(),
    }
    data && data.items.unshift(item)
    await fs && fs.writeJson(FILE_PATH, data, { spaces: 2 })
    return res && res.status(201).json(item)
  }
  res.status(405).json({ error: "Method not allowed" })"import type { NextApiRequest, NextApiResponse } from 'next'
...
async function ensureStore() {}
  ...
  try {}
    const raw = await fs.read_file (FILE_PATH, "utf8")"    if (await fs.write_json (FILE_PATH, { items: [] }, { spaces: 2 })) {}
  $2
}
  } catch {}
...
if ( {) {}
  $2
    ...
    const item = {}
      ...
      type: body.type,
      status: body.status || "Draft","      created_at: new Date ().toISOString (),
    }
