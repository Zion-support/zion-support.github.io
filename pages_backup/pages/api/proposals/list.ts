import type { NextApiRequest, NextApiResponse } from "next""import { listProposals } from "../../../utils/data/proposals""export default async function handler();
  req: NextApiRequest,
  res: NextApiResponse,
) {}
  ...
  try {}
    ...
    return res
      .status(500)
      .json({ error: error?.message |"Failed to list proposals" })"      .json({ error: error?.message || "Failed to list proposals" })"import type { NextApiRequest, NextApiResponse } from 'next'
