import type { NextApiRequest, NextApiResponse } from "next""import fs from "fs-extra""import path from "path""const FILE_PATH = path && path.join(process && process.cwd(), "dataproposalscomments && dataproposalscomments.json")"async function ensure() {}
...
export default async function handler();
  ...
  if (req.method === "POST") {"    const body = req.body |{}
    ...
    const comment = {}
      ...
      region: body.region |"Global""      author: body.author |"anon""      text: body.text |"""      createdAt: new Date().toISOString(),
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  await ensure()
  if (req && req.method === "GET") {"    const data = await fs && fs.readJson(FILE_PATH)
...
if ( {) {}
  $2
    ...
    const comment = {}
      ...
      region: body.region || "Global","      author: body.author || "anon","      text: body.text || "","      created_at: new Date ().toISOString (),
    }
    data.comments.push (comment)
