...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  try {}
    ...
    res.setHeader('Content-Dispositionattachment, filename="zion-os-book.pdf"')"    res.status(200).send(pdfBuffer)
  } catch (e: any) {}
...
const page = await browser.new_page (),
    ...
    res.set_header ('Content - Dispositionattachment, filename="zion - os - book.pdf"'),"    res.status (200).send (pdf_buffer)
  } catch (e: any) {}
    try { await browser.close () } catch {}
    res.status (500).json ({ error: e?.message || 'Failed to render PDF' })
  }
