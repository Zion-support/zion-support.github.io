...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {,
  ...
  try {,
    ...
    const outDir = path.join(process.cwd(), 'datapage-metadata', 'seo');
      'AI Devs in BrazilAI Devs in Kenya', 'AI Devs in VietnamRent Servers in Kabul', 'Rent Servers in NairobiLLM Engineers in Toronto', 'Cybersecurity Experts in BerlinCloud Architects in Lisbon',
    ];
    const picks = seedTopics.sort(() => 0.5 - Math.random()).slice(0, 4);
