...
export default function handler(req: NextApiRequest, res: NextApiResponse) {}
  if (req.method !== "POST")"import type { NextApiRequest, NextApiResponse } from "next""import { writeState } from "../../../lib/integrations/fileStore""import { getProviderById } from "../../../lib/integrations/registry""export default function handler(req: NextApiRequest, res: NextApiResponse) {}
  if (req.method !== "POST")"    return res.status(405).json({ error: "Method not allowed" })"  const { providerId } = req.body as { providerId?: string }
