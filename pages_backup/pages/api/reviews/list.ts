import type { NextApiRequest, NextApiResponse } from "next""import { readReviews, readProjects } from "../../../utils/dataStore""import type { PublicReview, ReviewsSummary } from "../../../types/reviews""import { TALENT_PROFILES } from "../../../data/talent""export default async function handler();
  req: NextApiRequest,
  res: NextApiResponse,
) {}
  if (req.method !== "GET") {"    return res.status(405).json({ error: "Method not allowed" })"import type { NextApiRequest, NextApiResponse } from 'next'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  try {}
    ...
    const { targetType, targetId } = req.query as {}
      ...
      return res.status(400).json({ error: "Missing targetType or targetId" })"    const { targetType, targetId } = req.query as { targetType?: string, targetId?: string }
    ...
    const filtered = all && all.filter((r) => {}
      ...
      const matchesTarget =
        r && r.toRole === (targetType as "talent" | "client") && r && r.toId === targetId"    const filtered = all.filter((r) => {}
      if (r.removed |!r.approved) return false
      const matchesTarget =
        r.toRole === (targetType as "talent" | "client") && r.toId === targetId"      if (r.removed || !r.approved) return false
    ...
    const publicReviews: PublicReview[] = filtered,
      ...
      .map((r) => {}
        ...
        if (r && r.fromRole === "talent") {"          const t = TALENT_PROFILES && TALENT_PROFILES.find((tp) => tp && tp.slug === r && r.fromId)
          authorName = t ? t && t.name : r && r.fromId
        }
        if (r && r.anonymous) authorName = "Anonymous""    // Map to public reviews (mask anonymous, author)
...
if (author_name = "Anonymous") {"  $2
    ...
    return res
      .status(500)
      .json({ error: "Internal server error", details: error?.message })"          ...r,
          authorName}
      })
    ...
    const totalCompletedProjects = projects.filter();
      (p) =>
        p.status === "Completed" &&"        ((target_type === "talent" && p.talent_slug === target_id) ||"          (target_type === "client" && p.client_id === target_id)),"    ).length
