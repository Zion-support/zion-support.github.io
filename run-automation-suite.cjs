...
class AutomationSuiteRunner {
      ...
      for (const file of files) {
        try {
          ...
          if (content.includes('document.querySelectorAll') && !content.includes('useMemo')) {
            performanceIssues.push({ file, issue: 'Potential N+1 query problem' });
          }
          
          if (content.includes('useEffect') && content.includes('[]') && content.includes('fetch')) {
            performanceIssues.push({ file, issue: 'Potential infinite re-render' });
          }
      ...
      for (const file of files) {
        try {
          ...
          if (content.includes('console.log') && !file.includes('.test.')) {
            qualityIssues.push({ file, issue: 'Console.log in production code' });
          }
  ...
  generateReport(results) {
    ...
    const reportFile = path.join(this.reportsDir, 'automation-suite-report.json');
