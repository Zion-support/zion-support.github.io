...
//       } catch () {}}
  ...
  private processNavigationTiming(entry: PerformanceNavigationTiming): void {const metrics: Partial<PerformanceMetrics> = {</PerformanceMetrics>loadTime</PerformanceMetrics>: entry.loadEventEnd - entry.loadEventStart,
  ...
  firstInputDelay: 100,
    ...
    Object.entries(thresholds).forEach(([key) threshold]) => {const value = metrics[key as keyof PerformanceMetrics]}
      if (typeof value === 'number' && value>threshold</value>) {
        this.addAlert({
          type: value>threshold</value> * 1.5 ? 'error' : 'warning'}
...
class ErrorHandler {private errors: ErrorInfo[] = [];
  ...
  private handleError(errorInfo: ErrorInfo): void {this.errors.push(errorInfo)}
    if (this.errors.length>this</length>.maxErrors) {
