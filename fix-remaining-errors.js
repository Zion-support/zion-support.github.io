    ...
    try {
      ...
      const componentNameMatch = content.match(/const\s+(Service Feature;,
        const newName = oldName;
          .replace(/\s+/g, '')
          .replace(/\-/g, '')
          .replace(/^(Service Feature/g, '\\s+')}Page:\\s*React\\.FC`, 'g'), `const ${newName}Page: React.FC`);
          content = content.replace(new RegExp(`export\\s+default\\s+${oldName.replace(/Service Feature/g, '\\s+')}Page`, 'g'), `export default ${newName}Page`);
          modified = true}
      }
      ...
      content = content.replace(/\/\/\s*Service Feature/g, '');
      content = content.replace(/<Service Feature*>/g, (match) => match.replace(/\/\/.*/, ''));
      
      // Fix any incomplete function calls or objects;
      content = content.replace(/{\s*}\s*$/gm, '{}');
      content = content.replace(/\Service Feature\s*$/gm, 'Service Feature');
      content = content.replace(/\(\s*\)\s*$/gm, '()');
