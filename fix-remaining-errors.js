...
const fixLintIssues = (content, filePath) => {
  ...
  if (newContent.includes("import { Helmet } from 'react-helmet-async';") && !newContent.includes('<Helmet')) {
    newContent = newContent.replace(/import { Helmet } from 'react-helmet-async';\n?/g, '');
  }
  ...
  // Handle multiple imports in one line - more aggressive approach
  const multiImportRegex = /import { ([^}]+) } from '([^']+)';\n?/g;
  let match;
  const matches = [];
  ...
  matches.forEach(match => {
    ...
    const usedIcons = allIcons.filter(iconName => 
      newContent.includes(`<${iconName}`) || newContent.includes(`${iconName}`) || iconName === iconName
    );
  ...
  // Fix unused variables in error handler
  if (filePath.includes('errorHandler')) {
    newContent = newContent.replace(/error: Error, context: string/g, '_error: Error, _context: string');
    newContent = newContent.replace(/error: Error, context: string/g, '_error: Error, _context: string');
  }
