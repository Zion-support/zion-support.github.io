...
function fixRegexErrors(content) {
  ...
  // Fix regex patterns that were incorrectly inserted
  fixed = fixed.replace(/className="\[[^"]*\]"/g, 'className="text-gray-600"');
  fixed = fixed.replace(/className="[^"]*Custom Implementation[^"]*"/g, 'className="text-lg font-semibold text-purple-900 mb-2"');
  fixed = fixed.replace(/className="[^"]*24\/7 Support[^"]*"/g, 'className="text-lg font-semibold text-purple-900 mb-2"');
  fixed = fixed.replace(/className="[^"]*bg-purple-50border border-purple-20 0rounded-lgp-6[^"]*"/g, 'className="bg-purple-50 border border-purple-200 rounded-lg p-6"');
