...
function processFile(filePath) {
  try {
    // Fix <a></a> tags with 'to' prop to use Link components;
...
function processFile(filePath) {/* TODO: Fix JSX expression */}
      ...
      // Replace <a></a> tags with to prop to <Link></Li></Lin> components;
      content = content.replace(/<a\s+to=/g, '<Link to=');
      content = content.replace(/<\/a></Lin></a>/g, '</Link>');
      modified = true;
    }
    ...
    if (content.includes('next/image')) {
      ...
      content = content.replace(/<\/Image></Lin></Link>/g, '</img>');
