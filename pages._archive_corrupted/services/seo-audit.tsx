...
export default function SEOAudit() {class ErrorBoundary extends React.Component {constructor(props) {super(props)this.state = { hasError: false }}static getDerivedStateFromError(error) {return { hasError: true }}componentDidCatch(error, errorInfo) {console.error('Error caught by boundary:', error, errorInfo)}render() {if (this.state.hasError) {return <div />Something went wrong.</div>;}
    }return this.props.children;
  }
}
import { Search,, CheckCircle,, ArrowRight,, DollarSign,, Clock } from 'lucide-react';
export default function SEOAudit() {return (import React from 'react'; import Link from 'next/link'; import { Search,CheckCircle,ArrowRight,DollarSign,Clock } from 'lucide-react'; export default function SEOAudit() { return ( <main className=\"min-h-screen bg-white\" /> <section className=\"bg-gradient-to-br from-slate-50 to-slate-100 py-16\" /> <div className=\"max-w-5xl mx-auto px-4 sm:px-6 lg:px-8\" /> <div className=\"flex items-center gap-3 mb-6\" /> <div className=\"p-3 rounded-lg bg-slate-800 text-white w-fit\" /> <Search className=\"w-6 h-6\" /> </div> <h1 className=\"text-3xl font-bold text-gray-900\" />SEO Technical Audit</h1> </div> <p className=\"text-gray-700 text-lg max-w-3xl\" /> Comprehensive audit covering Core Web Vitals,crawlability,sitemap/robots,structured data,headers,accessibility,and content gaps. We provide prioritized fixes and implement quick wins. </p> </div> </section> <section className=\"py-12\" /> <div className=\"max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-3 gap-6\" /> {[ { name: 'Essentials',;
  price: 'From $1,500',time: '5-7 days',}
  items: ['CWV snapshot','Meta & schema review','Top 10 fixes implemented'] },{ name: 'Pro',
  price: '$3,500',time: '10-14 days',}
  items: ['Full audit + backlog','Schema & redirects','Link health fixes'] },{ name: 'Enterprise',
  price: 'Custom',time: '2-4 weeks',}
