...
function createBasicPageTemplate(filePath) {
  const fileName = path.basename(filePath, '.tsx');'  const pageName = fileName.charAt(0).toUpperCase() + fileName.slice(1);
...
export default function ${pageName}Page() {
  return (
    <div className="min-h-screen bg-gray-50">"      <Helmet>
        ...
        <meta name="description" content="Advanced ${pageName} solutions by Zion Tech Group" />"      </Helmet>
      
      <div className="container mx-auto px-4 py-16">"        <div className="text-center">"          <h1 className="text-4xl font-bold text-gray-900 mb-4">"            ${pageName}
          </h1>
...
function createBasicComponentTemplate(filePath) {
  const fileName = path.basename(filePath, '.tsx');'  const componentName = fileName.charAt(0).toUpperCase() + fileName.slice(1);
...
function isCorrupted(content) {
  ...
  if (content.includes('export default function') && !content.includes('return (')) {'    return true;
  }
  
  // Check for incomplete functions
  if (content.includes('export default function') && !content.includes('}')) {'    return true;
  }
...
function fixFile(filePath) {
  try {
    ...
    if (isCorrupted(content)) {
      ...
      // Determine if it's a page or component'      const isPage = filePath.includes('/page.tsx') || filePath.includes('/pages/');'      const template = isPage ? createBasicPageTemplate(filePath) : createBasicComponentTemplate(filePath);
      
      fs.writeFileSync(filePath, template, 'utf8');'      return true;
    }
