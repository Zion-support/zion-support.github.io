...
function fixFinalJSX(content) {
  ...
  content = content.replace(/<h1[^>]*>([^<]*)<\/h1>""/g, '<h1 className="text-4xl font-bold text-white mb-8">$1</h1>');
  content = content.replace(/<p[^>]*>([^<]*)<\/p>/g, '<p className="text-gray-300 text-lg">$1</p>');
  ...
  content = content.replace(/export default function Page\(\) \{\s*return\s*\(\s*<React\.Fragment>\s*<Helmet>\s*<title>([^<]*)<\/title>\s*<meta[^>]*\/><\/Helmet>\s*<div>\s*<div>\s*<h1[^>]*>([^<]*)<\/h1>\s*<p[^>]*>([^<]*)<\/p>\s*<\/div><\/div>\s*<\/React\.Fragment>\s*\);\s*\}/g, 
