...
# Simple script to merge cursor branches into main
set -e
...
# Get all cursor branches (limit to first 20 for safety)
BRANCHES=$(git branch -r | grep "origin/cursor/" | head -20 | sed 's/origin\///' | sort)

echo "📊 Processing first 20 cursor branches..."
...
resolve_conflicts() {
    ...
    if grep -q "<<<<<<< HEAD" "$file"; then
        ...
        if [[ "$file" == "next.config.js" || "$file" == "tsconfig.json" || "$file" == "package.json" ]]; then
