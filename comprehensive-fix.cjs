...
function fixSyntaxErrors(content) {
  ...
  content = content.replace(/import\s+.*?from\s+['"]([^'"]*?)$/gm, (match, str) => {
  ...
  content = content.replace(/<HelmetProvider[^>]*>(?![\s\S]*<\/HelmetProvider>)/g, (match) => {
  ...
  if (content.includes('describe(') && content.includes('it(')) {
    ...
    content = content.replace(/describe\([^)]*$/gm, 'describe("Test Suite", () => {');
    ...
    for (const filePath of files) {
      try {
        ...
        try {
          ...
          console.log(`❌ Failed to create basic component for ${filePath}: ${writeError.message}`);
        }
      }
    }
  }
