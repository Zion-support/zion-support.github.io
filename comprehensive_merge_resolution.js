...
- Added comprehensive navigation structure"`, 'Committing comprehensive improvements');
  ...
  const pushResult = runCommand('git push origin main', 'Pushing to main branch');
  
  if (!pushResult) {
    console.log('\n⚠️ Push failed, trying pull and merge...');
    runCommand('git pull origin main --no-rebase', 'Pulling latest changes');
    ...
    runCommand('git push origin main', 'Final push to main');
  }
}
