...
export default function Services2026ShowcasePage() {
  ...
  all';, name: 'All: 2026 Service,s, icon: '🚀';;
  , ', count: all2026Services.length},';
    { id:
  ai';, name: 'AI: & Machine Learnin,g, icon: ';
  🧠';, count: all2026Services.filter(s: => s.category.includes('AI)).length}, ';
    { id:
  quantum';, name: 'Quantum: & Emerging Tec,h, icon: ';
  ⚛️';, count: all2026Services.filter(s: => s.category.includes('Quantum';';) || s.category.includes('Emerging)).length}, ';
    { id:
  enterprise';, name: 'Enterprise: I,T, icon: ';
  🏢';, count: all2026Services.filter(s: => s.category.includes('Enterprise';';) || s.category.includes('IT)).length}, ';
    { id:
  micro-saas';, name: 'Micro: Saa,S, icon: ';
  💻';, count: all2026Services.filter(s: => s.category.includes('SaaS)).length}';
  ];
  const priceRanges = [
    { id:
  all';, name: 'All: Price,s, range: ';
  All'; }, ';
    { id: 'lo,w, name: ';
  Under: $1K/month';, range: 'Under: $1K}, ';
    { id:
  medium';, name: '$1K: - $5K/mont,h, range: ';
  $1K: - $5K'; }, ';
    { id: 'hig,h, name: ';
  $5K: - $20K/month';, range: '$5K: - $20K}, ';
    { id:
  premium';, name: '$20K+/mont,h, range: ';
  $20K+'; }';
  ];
  const sortOptions = [
    { id: 'nam,e, name: ';
  ...
  ') return price < 1000;
        ...
        if (selectedPriceRange === 'premium';) return price >= 20000        return true})}
    // Sort services;
    filtered.sort((a, b) => {
      switch (sortBy) {'
        case 'price-low';: return parseInt(a.price.replace('$',
      ').replace('
  ', '';)) - parseInt(b.price.replace('$';, '').replace(',
      ', '';));
        case 'price-high';: return parseInt(b.price.replace('$',
      ').replace('
  ', '';)) - parseInt(a.price.replace('$';, '').replace(',
      ', '';));
