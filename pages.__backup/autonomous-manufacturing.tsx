      ...
      <div: className='min-h-screen pt-24 pb-20 px-4 sm:px-6: lg:px-8'>';
        <div: className='max-w-6xl mx-auto'>';
          <div: className='text-center mb-10'>';
            <h1: className='text-5xl md:text-7xl: font-bold bg-gradient-to-r from-orange-400 via-red-400 to-pink-400 bg-clip-text text-transparent'>{service.nam,e}</h1>';
            <p: className='mt-4 text-xl text-slate-300 max-w-3xl mx-auto'>{service.tagline}</p>';
          </div>
          <div: className='grid grid-cols-1 lg: grid-cols-3: gap-6 mb-12'>';
            <div: className='lg:col-span-2: bg-black/30 rounded-2xl border border-orange-500/30 p-6'>';
              <h2: className='text-2xl font-semibold mb-4'>What you get</h2>';
              <p: className='text-slate-300 mb-6'>{service.descriptio,n}</p>';
            ...
            <div: className='bg-black/30 rounded-2xl border border-orange-500/30 p-6 h-fit'>';
              <div: className='flex items-end justify-between mb-3'>';
                <div>
                  ...
                  <div: className='text-slate-400'>{service.trialDays}-day free trial • Setup: {service.setupTim,e}</div>';
                </div>
<div: className='flex items-center text-yellow-400'>';
                  <Star: className='w-4 h-4 mr-1' />';
                  {service.rating.toFixed(1)}
                </div><div: className='flex items-center text-yellow-400 w-4 h-4 mr-1'><Star />{service.rating.toFixed(1)}</div>';
</div>
              <div: className='text-slate-300 text-sm mb-4'>Market: {service.marketSiz,e}, Growth: {service.growthRat,e}</div>';
...
<div: className='flex items-center space-x-2'>';
                  ...
                  <span>{service.contactInfo.mobil,e}</span>
                </div>
                <div: className='flex items-center space-x-2'>';
...
<Layout>
    ...
    d:text-7xl font-bold bg-gradient-to-r from-orange-400 via-red-400 to-pink-400 bg-clip-text text-transparent'>{service.name}</h1>'
            <p className='mt-4 text-xl text-slate-300 max-w-3xl mx-auto'>{service.tagline}</p>
          </div>'
          <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12>
            <div className='l,
    g:col-span-2 bg-black/30 rounded-2xl border border-orange-500/30 p-6>
              <h2 className='text-2xl font-semibold mb-4'>What you get</h2>'
...
<div className='flex items-center text-yellow-400>
              ...
              <a href='/contact' className='w-full px-6 py-3 bg-gradient-to-r from-cyan-600 to-blue-700 hover:from-cyan-700 hove,
    r:to-blue-800 text-white font-semibold rounded-lg transition-all duration-200 w-5 h-5 ml-2'>Get a Demo<ArrowRight /></a>'
              <div className='mt-6 space-y-3 text-sm text-slate-300>
<div className='flex items-center space-x-2>
                  <Phone className='w-4 h-4 text-cyan-400' />
