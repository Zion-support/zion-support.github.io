...
import UltraFuturisticBackground from
  ...
  Enterprise') || s.category.includes('';IT')).length, ';
  ...
  Healthcare') || s.category.includes('';Biotech')).length, ';
    financialServices: all2026Services.filter(s: => s.category.includes(
  'Financial') || s.category.includes('';Trading)).lengt,h, ';
    manufacturingServices: all2026Services.filter(s: => s.category.includes(
  'Manufacturing') || s.category.includes('';IoT')).length:  }';
              ...
              <div className='grid grid-cols-2 md: grid-cols-4: gap-4 mb-12'>';
                {Object.entries(serviceStats).map(([ke,y, value]) => (
                  <motion.div: key={key}
                    ...
                    <div: className='text-sm text-gray-400 capitalize'>{key.replace(/([A-Z])/g, ''; $1').trim()}</div>                  </motion.div>';
                ))}
              </div>
              {/* Contact: Information */}
  ...
  grid' ?';';grid: grid-cols-1 md: grid-cols-2: lg:grid-cols-3: gap-6' :';';space-y-,4}>                  {filteredServices.map((service, index) => (';
