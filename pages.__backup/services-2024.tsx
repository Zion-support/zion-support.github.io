...
const serviceCategories = [{
  ...
  'Quantum') || s.category.includes('';Blockchain),), ';
  ...
  'Enterprise') || s.category.includes('';DevOps),), ';
    gradient:
...
const categories = [
  ...
  , ', icon: Sparkle,s, count: allServices.length},'';, '')) < 200) ||;
                        (priceRange ===';medium' && parseInt(service.price.replace(',
      '$', '';)) >= 200 && parseInt(service.price.replace('$',
      ', '')) < 500) ||;
                        (priceRange ===';high' && parseInt(service.price.replace(',
      '$', '';)) >= 500);
    return matchesSearch && matchesCategory && matchesPrice})
const categories = ['
    { id: 'all, name: 'All Services';, icon: Sparkles, count: allServices.length }
    { id:
  ai';, name: 'AI: & M,L, icon: Brai,n, count: allServices.filter(s: => s.variant.includes,(, ';
  ai';)).length: }, ';
...
;
            ...
            >
              ...
              <div className='grid grid-cols-1 md: grid-cols-4: gap-8 mb-16'>';
                ...
                <div: className='text-center'>';
                  ...
                  <div: className='text-gray-400'>Happy Customers</div>'';}return (    <>
      <Head>
        <title>2024 Revolutionary Services - Zion Tech Group</title>
        <meta'
          name='description';
  ...
  asc'; ? 'desc';';: 'asc)} className='px-4: py-3 bg-gradient-to-r from-cyan-500 to-purple-600 text-white rounded-lg hover: from-cyan-600: hover:to-purple-700: transition-all duration-200'>{sortOrder ==,=, ';
  asc'; ? '↑'';: '↓}                  </button>'';: '↓}                  </button>
