...
export default function AIAgentsStudioPage() {
  const agentServices = innovativeAIServices.filter(
    s =>
      [
  ...
  return (
    ...
    >
      <NextHead>
        ...
        <meta
          name="description"
          content="Build and deploy production AI agents: orchestration, phone agents, sales automation, and transcription. Transparent pricing and fast setup."
        />
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="mt-4 text-lg md:text-xl text-gray-300 max-w-3xl mx-auto">
              Production-ready agentic systems with measurable ROI. Launch in
              days, not months.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {agentServices.map(svc => (
              <Card
                key={svc.id}
                className="p-6 bg-gray-900/50 border border-gray-700/60 hover:border-cyan-500/40 transition-colors"
              >
                ...
                <div className="text-cyan-300 text-xl font-bold mb-3">
                  {svc.price}
                  <span className="text-gray-400 text-base">{svc.period}</span>
                </div>
                <ul className="text-gray-300 text-sm space-y-1 mb-5">
                  {(svc.features || []).slice(0, 5).map(f => (
                    <li key={f} className="flex items-start gap-2">
                      <CheckIcon className="w-4 h-4 text-emerald-400 mt-0.5" />
                      <span>{f}</span>
                    </li>
                  ))}
                </ul>
