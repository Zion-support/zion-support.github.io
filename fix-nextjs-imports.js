...
const replacements = [
  ...
  { from: "import { useRouter } from 'next/router';", to: "import { useNavigate, useLocation } from 'react-router-dom';" },
  { from: "import { useRouter } from 'next/navigation';", to: "import { useNavigate, useLocation } from 'react-router-dom';" },
  ...
  { from: "import type { Metadata } from 'next';", to: "import { Helmet } from 'react-helmet-async';" },
  ...
  try {
    ...
    replacements.forEach(({ from, to, context }) => {
      if (context) {
        ...
        const regex = new RegExp(`(${context}[^>]*?)${from.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'g');
