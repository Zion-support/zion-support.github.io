export interface InvestmentPortfolio {
  ...
  currency: string, riskTolerance: 'conservative' | 'moderate' | 'aggressive',
  investmentHorizon: number, // in years
  targetReturn: number, assets: PortfolioAsset[],
  lastRebalanced: Date, performance: PortfolioPerformance,
  createdAt: Date,
  updatedAt: Date
}
...
export interface PortfolioAsset {
  ...
  name: string, type: 'stock' | 'bond' | 'etf' | 'mutual_fund' | 'crypto' | 'real_estate' | 'commodity',
  quantity: number, currentPrice: number,
  marketValue: number,
  allocation: number, // percentage of portfolio
  purchasePrice: number, purchaseDate: Date,
  performance: {
    dailyReturn: number, weeklyReturn: number,
...
export interface PortfolioPerformance {
  ...
  volatility: number, sharpeRatio: number,
  maxDrawdown: number, beta: number,
...
export interface FinancialGoal {
  ...
  priority: 'low' | 'medium' | 'high', category: 'retirement' | 'education' | 'home' | 'emergency' | 'vacation' | 'business' | 'other',
  monthlyContribution: number, expectedReturn: number,
...
export interface MarketAnalysis {
  ...
  trends: string[], risks: string[],
  opportunities: string[], recommendations: string[],
  confidence: number, lastUpdated: Date,
  nextUpdate: Date
}
...
export interface FinancialRequest {
  userId: string, requestType: 'portfolio_analysis' | 'investment_recommendation' | 'financial_planning' | 'market_analysis' | 'goal_tracking',
...
export class AIFinancialAdvisorService {
  ...
  async calculateRetirementNeeds(userId: string, targetAge: number, desiredIncome: number): Promise<{
    estimatedNeeds: number, monthlyContribution: number,
    projectedValue: number,
  ...
  async getTaxOptimizationStrategies(userId: string, taxYear: number): Promise<{
    strategies: string[], estimatedSavings: number,
    implementation: string[], risks: string[],
