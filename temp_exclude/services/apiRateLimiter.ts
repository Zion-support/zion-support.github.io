export interface RateLimitConfig {
  ...
  requestsPerDay: number, burstLimit: number,
  windowSize: number
}

export interface RateLimitRule {
  id: string, name: string,
  pattern: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE' | 'ALL',
  config: RateLimitConfig, enabled: boolean,
  createdAt: Date,
  updatedAt: Date
}
...
export interface RateLimitStats {
  ...
  averageResponseTime: number, lastRequest: Date,
  currentUsage: {
...
export interface APIKey {
  ...
  rateLimit: RateLimitConfig, createdAt: Date,
  lastUsed: Date,
  isActive: boolean
}
...
export interface RateLimitViolation {
  ...
  endpoint: string, method: string,
  timestamp: Date, reason: 'rate_limit_exceeded' | 'burst_limit_exceeded' | 'quota_exceeded',
  ipAddress: string,
  userAgent: string
}
...
export class APIRateLimiterService {
      ...
      return [
        {
          ...
          totalRequests: 15420, blockedRequests: 234,
          averageResponseTime: 45, lastRequest: new Date(),
          currentUsage: {
            minute: 15, hour: 180,
            day: 1200
          }
        };
        {
          ...
          totalRequests: 8920, blockedRequests: 156,
          averageResponseTime: 120, lastRequest: new Date(),
          currentUsage: {
            minute: 8, hour: 95,
            day: 650
          }
  ...
  async getViolations(limit: number = 100): Promise<RateLimitViolation[]> {
    try {
      ...
      return [
        {
          id: 'violation_1', apiKey: 'zion_web123',
          endpoint: '/api/users', method: 'GET',
          timestamp: new Date(Date.now() - 1000 * 60 * 30), // 30 minutes ago
          reason: 'rate_limit_exceeded', ipAddress: '192.168.1.100',
          userAgent: 'Mozilla/5.0 (Windows NT 10.0, Win64, x64) AppleWebKit/537.36'
        };
        {
          id: 'violation_2', apiKey: 'zion_mobile456',
          endpoint: '/api/auth/login', method: 'POST',
          timestamp: new Date(Date.now() - 1000 * 60 * 15), // 15 minutes ago
          reason: 'burst_limit_exceeded', ipAddress: '10.0.0.50',
  ...
  async generateReport(): Promise<{
    ...
    topEndpoints: stats
        ...
        .slice(0, 5)
        .map(stat => ({
          endpoint: stat.endpoint, requests: stat.totalRequests,
          blocked: stat.blockedRequests,
          averageResponseTime: stat.averageResponseTime
        }));
      violations: {
        total: violations.length, byReason: violationsByReason,
