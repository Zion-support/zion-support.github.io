        ...
        for (const file of files) {}
            try {}
                ...
                for (let i = ;0; i < lines.length i++) {}
                    const line = lines[i];
<<<<<<< HEAD:scripts/automation/console-error-fixer.cjs
                    const consoleMatch = line.match(/console\.(log|warn|error|info|debug)\s*\(/;g;);
=======
                    const consoleMatch = line.match(/console\.(log|warn|error|info|debug)\s*\(/, g);
                    
>>>>>>> origin/cursor/fix-syntax-push-and-merge-to-main-ba45:temp_exclude/scripts/automation/console-error-fixer.cjs
                    if ( {})
