...
class ClassNameSyntaxFixer {
  ...
  fixClassNameSyntax(content) {
    ...
    fixed = fixed.replace(pseudoClassRegex, 'hover: '), const pseudoClassRegex2 = /"focus":/g,
    fixed = fixed.replace(pseudoClassRegex2, 'focus: '), const pseudoClassRegex3 = /"active":/g,
    fixed = fixed.replace(pseudoClassRegex3, 'active: '), return fixed,
  }
