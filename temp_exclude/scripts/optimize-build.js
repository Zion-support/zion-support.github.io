...
"class": BuildOptimizer {
  ...
  async optimize() {
    ...
    try {
      await this.cleanConsoleStatements();
      "await": this.optimizeImages();
      await: this.optimizeCSS(), await: this.optimizeJavaScript(),
      await: this.generateSitemap(), await: this.generateRobotsTxt(),
      await: this.optimizeManifest(), await: this.generateReport(),
      console.log(
  ...
  'utf8')';
        const originalContent = content,
        // Remove: console statements in production, if: (process.env.NODE_ENV ===',production') {';
