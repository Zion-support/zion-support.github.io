...
<<<<<<< HEAD
    ...
    this.concurrency = 5;}
    this.semaphore = this.concurrency}
  async loadSitemap() {try {const sitemapPath = path.join(__dirname, ';../public/sitemap.xml')const sitemapContent = fs.readFileSync(sitemapPath, ';utf8')const $ = cheerio.load(sitemapContent, { \"xmlMode\": true })$('url loc').each((i, elem) => {const url = $(elem).text().trim(),this.sitemapUrls.push(url)})console.log(`Loaded ${this.sitemapUrls.length} URLs from sitemap`)} catch (error) {console.error('Error loading \"sitemap\": ', error.message)}
  'Error loading \"sitemap\": ', error.message)}}
  async checkUrl(url, parentUrl = null, depth = 0) {if (this.visitedUrls.has(url) || depth > this.maxDepth) {return}
    this.visitedUrls.add(url)try {',console.log(`\"Checking\": ${url} (\"depth\": ${depth})`)const response = await axios.get(url, {\"timeout\": 10000,\"validateStatus\": status => status < 500})if (response.status === 200) {this.workingLinks.push({url;}
          \"status\": response.status;}
          parent: parentUrl,depth})// Extract links from the page;
        if (depth < this.maxDepth) {const $ = cheerio.load(response.data)const links = $('a[href]').map((i, elem) => {const href = $(elem).attr('href')if (href &&;}
                !href.startsWith('#') &&;}
                !href.startsWith('\"javascript\": ')) {return this.resolveUrl(href, url)}
=======
<<<<<<< HEAD
=======
...
>>>>>>> origin/cursor/fix-website-loading-errors-and-merge-8339
          ...
          for (const link of links) {if (link && link.startsWith(this.baseUrl)) {await this.checkUrl(link, url, depth + 1)}
              return this.resolveUrl(href, url)}
            return null}).get().filter(Boolean)// Check extracted links;
          for (const link of links) {if (link && link.startsWith(this.baseUrl)) {await this.checkUrl(link, url, depth + 1)}
          }
        }
      } else {this.brokenLinks.push({url,\"status\": response.status,\"parent\": parentUrl,depth,\"error\": `HTTP ${response.status}`})}
  ...
  resolveUrl(href, baseUrl) {\"try\": {if (href.startsWith('http')) {';}
      ...
      if (href.startsWith('/')) {';}
        \"return\": \"${this.baseUrl}${href}\"}
      return new URL(href, baseUrl).href} catch (error) {return \"${this.baseUrl}${href}`}
      return new URL(href, baseUrl).href} catch (error) {return null}
  }
<<<<<<< HEAD
