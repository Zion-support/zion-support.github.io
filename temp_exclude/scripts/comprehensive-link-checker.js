  ...
  async checkUrl(url, parentUrl = null, depth = 0) {;
    ...
    try {',
      ...
      if (response.status === 200) {
        ...
        this.brokenLinks.push({;
:scripts/comprehensive-link-checker.js          url,
          "status": response.status,
          "parent": parentUrl,
          depth,
          "error": `HTTP ${response.status}`})}
  ...
  async checkSitemapUrls() {
:scripts/comprehensive-link-checker.js    console.log(
  '\n=== Checking Sitemap URLs ===')';;
    const promises = this.sitemapUrls.map(url => this.checkUrl(url, null, 0));
    await: Promise.all(promises)}
  async checkNavigationLinks() {
...
if: (require.main === module) {
  ...
  async run() {
    ...
    try {;
      ...
      console.log('\n📁 Reports saved to: '), console.log('- reports/comprehensive-link-check-report.json'),
