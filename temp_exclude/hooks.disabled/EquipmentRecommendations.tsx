<<<<<<< HEAD:temp_exclude/hooks.disabled/EquipmentRecommendations.tsx
<<<<<<< HEAD
...
import {useAuth} from '@/hooks/useAuth''
import {fetchRecommendations} from '@/api/recommendations''
>>>>>>> origin/cursor/fix-syntax-push-and-merge-to-main-b934:src/pages/EquipmentRecommendations.tsx
import type { ProductListing } from '@/types/listings'
import {};
  Dialog;
  DialogContent;
...
export default function EquipmentRecommendations() { return null; }
  const { isAuthenticated, user } = useAuth();
  const [listings, setListings] = useState<ProductListing[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<boolean>(false);
