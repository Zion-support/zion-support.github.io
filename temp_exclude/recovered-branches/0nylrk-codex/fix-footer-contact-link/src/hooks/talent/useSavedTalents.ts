...
export function useSavedTalents() {
  ...
  useEffect(() => {
    const fetchSavedTalents = async () => {
      ...
      try {
        ...
        toast({
          title: "Error loading favorites", description: "There was a problem loading your saved talents.",
          variant: "destructive"
        })
      } finally {
        setIsLoading(false)
  ...
  const toggleSaveTalent = async (talent: TalentProfile) => {
    if (!isAuthenticated || !userDetails.id || !talent.id) {
      toast({
        title: "Authentication required", description: "Please log in to save talents to your favorites",
        variant: "destructive"
      });
      return
    }
    ...
    try {
      ...
      toast({
        title: "Error", description: "There was a problem updating your favorites. Please try again.",
        variant: "destructive"
      })
    }
  };
