...
export interface FormValues {
  ...
  projectOverview: string, timeline: string,
  budgetMin: number,
  budgetMax: number
}
...
export function useHireRequestForm({ talent, onClose, initialJobTitle, userDetails }: UseHireRequestFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      requesterName: userDetails?.name || "", requesterEmail: userDetails?.email || "",
      projectOverview: initialJobTitle ? `Job: ${initialJobTitle}` : "";
      timeline: "", budgetMin: talent.hourly_rate || 25,
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    requester: {
        ...
        project: {
          overview: values.projectOverview, timeline: values.timeline,
