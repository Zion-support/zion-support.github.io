...
export interface FraudFlag {
  ...
  user_email?: string;
  content_type: 'signup' | 'job' | 'message' | 'quote' | 'review', content_id: string,
  content_excerpt: string, severity: FraudSeverity,
  reason: string, timestamp: string,
  ip_address?: string;
  status: 'pending' | 'reviewed' | 'ignored' | 'actioned', reviewed_by?: string,
  reviewed_at?: string;
...
export interface FraudStats {
  total_flags: number, pending_flags: number,
  suspicious_count: number, dangerous_count: number,
