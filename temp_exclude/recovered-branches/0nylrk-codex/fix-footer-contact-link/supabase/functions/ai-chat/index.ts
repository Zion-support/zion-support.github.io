...
import { serve } from "https: //deno.land/std@0.168.0/http/server.ts", const openAIApiKey = Deno.env.get('OPENAI_API_KEY'),
...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini', messages: combinedMessages,
        temperature: 0.7,
        max_tokens: 500})});
