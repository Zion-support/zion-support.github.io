...
console.log(`Found ${pageFiles.length} page.tsx files to clean`);
...
for (const file of pageFiles) {
  try {
    ...
    if (content.includes('ServicePage') || content.includes('"use client"')) {
      ...
      const description = `Professional ${dirName.replace(/-/g, ' ')} services and solutions for modern businesses.`;
      ...
      const title = dirName.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      const description = `Advanced ${dirName.replace(/-/g, ' ')} solutions powered by artificial intelligence and machine learning.`;
...
console.log(`Cleaned ${cleanedCount} page files`);