import { j as e, L as p } from './index-f98d0e43.js';
import { m as s } from './animation-vendor-e3c083d8.js';
import {
  n as h,
  c as a,
  S as u,
  o as y,
  p as l,
  P as g,
  m as b,
  A as j,
  q as N,
} from './icons-vendor-ffdb653c.js';
import './react-vendor-c5a9160c.js';
const I = () => {
  const c = 'January 15, 2025',
    r = {
      name: 'Zion Tech Group',
      address: '123 Innovation Drive, Tech Valley, CA 94000',
      email: 'privacy@ziontechgroup.com',
      phone: '+1 (555) 123-4567',
      website: 'https://ziontechgroup.com',
    },
    d = [
      {
        title: 'Personal Information',
        description: 'Name, email address, phone number, company information',
        examples: [
          'Contact forms',
          'Account registration',
          'Newsletter signup',
        ],
      },
      {
        title: 'Technical Data',
        description:
          'IP address, browser type, device information, usage analytics',
        examples: [
          'Website analytics',
          'Performance monitoring',
          'Security logs',
        ],
      },
      {
        title: 'Business Information',
        description:
          'Company details, project requirements, service preferences',
        examples: [
          'Service inquiries',
          'Project consultations',
          'Client communications',
        ],
      },
      {
        title: 'Communication Data',
        description: 'Email correspondence, chat logs, support tickets',
        examples: [
          'Customer support',
          'Project updates',
          'Marketing communications',
        ],
      },
    ],
    m = [
      {
        purpose: 'Service Delivery',
        description:
          'To provide and maintain our technology services and solutions',
      },
      {
        purpose: 'Communication',
        description: 'To respond to inquiries and provide customer support',
      },
      {
        purpose: 'Improvement',
        description: 'To enhance our services and develop new features',
      },
      {
        purpose: 'Security',
        description: 'To protect against fraud and ensure system security',
      },
      {
        purpose: 'Compliance',
        description: 'To meet legal obligations and regulatory requirements',
      },
      {
        purpose: 'Marketing',
        description:
          'To send relevant updates and promotional materials (with consent)',
      },
    ],
    x = [
      {
        category: 'Service Providers',
        description:
          'Trusted third-party vendors who assist in service delivery',
        examples: [
          'Cloud hosting providers',
          'Payment processors',
          'Analytics services',
        ],
      },
      {
        category: 'Legal Requirements',
        description: 'When required by law or to protect our rights',
        examples: ['Court orders', 'Government requests', 'Legal proceedings'],
      },
      {
        category: 'Business Transfers',
        description: 'In connection with business mergers or acquisitions',
        examples: ['Company sale', 'Asset transfer', 'Corporate restructuring'],
      },
      {
        category: 'Consent',
        description:
          'When you explicitly authorize us to share your information',
        examples: ['Partner integrations', 'Case studies', 'Testimonials'],
      },
    ];
  return e.jsxs('div', {
    className:
      'min-h-screen bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-700 pt-20',
    children: [
      e.jsxs('section', {
        className: 'relative py-20 overflow-hidden',
        children: [
          e.jsx('div', {
            className:
              'absolute inset-0 bg-gradient-to-r from-blue-600/20 to-purple-600/20',
          }),
          e.jsx('div', {
            className: 'relative container mx-auto px-4',
            children: e.jsxs(s.div, {
              initial: { opacity: 0, y: 30 },
              animate: { opacity: 1, y: 0 },
              transition: { duration: 0.8 },
              className: 'text-center max-w-4xl mx-auto',
              children: [
                e.jsxs('h1', {
                  className: 'text-5xl md:text-7xl font-bold text-white mb-6',
                  children: [
                    'Privacy',
                    e.jsxs('span', {
                      className:
                        'bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent',
                      children: [' ', 'Policy'],
                    }),
                  ],
                }),
                e.jsx('p', {
                  className: 'text-xl md:text-2xl text-gray-300 mb-8',
                  children:
                    'Your privacy is important to us. Learn how we collect, use, and protect your information.',
                }),
                e.jsxs('p', {
                  className: 'text-lg text-gray-400',
                  children: ['Last updated: ', c],
                }),
              ],
            }),
          }),
        ],
      }),
      e.jsx('section', {
        className: 'py-20',
        children: e.jsxs('div', {
          className: 'container mx-auto px-4',
          children: [
            e.jsxs(s.div, {
              initial: { opacity: 0, y: 30 },
              whileInView: { opacity: 1, y: 0 },
              transition: { duration: 0.8 },
              className: 'text-center mb-16',
              children: [
                e.jsx('h2', {
                  className: 'text-4xl md:text-5xl font-bold text-white mb-6',
                  children: 'Information We Collect',
                }),
                e.jsx('p', {
                  className: 'text-xl text-gray-300 max-w-3xl mx-auto',
                  children:
                    'We collect information to provide and improve our services, communicate with you, and ensure security.',
                }),
              ],
            }),
            e.jsx('div', {
              className: 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8',
              children: d.map((t, i) =>
                e.jsxs(
                  s.div,
                  {
                    initial: { opacity: 0, y: 30 },
                    whileInView: { opacity: 1, y: 0 },
                    transition: { duration: 0.5, delay: i * 0.1 },
                    className:
                      'bg-gradient-to-br from-zinc-800 to-zinc-900 p-6 rounded-xl border border-zinc-700',
                    children: [
                      e.jsx('div', {
                        className:
                          'bg-gradient-to-r from-blue-500 to-purple-500 p-4 rounded-lg w-16 h-16 mb-6 flex items-center justify-center',
                        children: e.jsx(h, { className: 'h-8 w-8 text-white' }),
                      }),
                      e.jsx('h3', {
                        className: 'text-xl font-semibold text-white mb-3',
                        children: t.title,
                      }),
                      e.jsx('p', {
                        className: 'text-gray-400 mb-4',
                        children: t.description,
                      }),
                      e.jsx('ul', {
                        className: 'space-y-2',
                        children: t.examples.map((n, o) =>
                          e.jsxs(
                            'li',
                            {
                              className:
                                'flex items-center text-gray-300 text-sm',
                              children: [
                                e.jsx(a, {
                                  className:
                                    'h-4 w-4 text-green-400 mr-2 flex-shrink-0',
                                }),
                                n,
                              ],
                            },
                            o
                          )
                        ),
                      }),
                    ],
                  },
                  i
                )
              ),
            }),
          ],
        }),
      }),
      e.jsx('section', {
        className: 'py-20 bg-black/20',
        children: e.jsxs('div', {
          className: 'container mx-auto px-4',
          children: [
            e.jsxs(s.div, {
              initial: { opacity: 0, y: 30 },
              whileInView: { opacity: 1, y: 0 },
              transition: { duration: 0.8 },
              className: 'text-center mb-16',
              children: [
                e.jsx('h2', {
                  className: 'text-4xl md:text-5xl font-bold text-white mb-6',
                  children: 'How We Use Your Information',
                }),
                e.jsx('p', {
                  className: 'text-xl text-gray-300 max-w-3xl mx-auto',
                  children:
                    'We use your information responsibly and only for purposes that benefit you and our services.',
                }),
              ],
            }),
            e.jsx('div', {
              className: 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8',
              children: m.map((t, i) =>
                e.jsxs(
                  s.div,
                  {
                    initial: { opacity: 0, y: 30 },
                    whileInView: { opacity: 1, y: 0 },
                    transition: { duration: 0.5, delay: i * 0.1 },
                    className:
                      'bg-gradient-to-br from-zinc-800 to-zinc-900 p-6 rounded-xl border border-zinc-700',
                    children: [
                      e.jsx('div', {
                        className:
                          'bg-gradient-to-r from-green-500 to-blue-500 p-4 rounded-lg w-16 h-16 mb-6 flex items-center justify-center',
                        children: e.jsx(a, { className: 'h-8 w-8 text-white' }),
                      }),
                      e.jsx('h3', {
                        className: 'text-xl font-semibold text-white mb-3',
                        children: t.purpose,
                      }),
                      e.jsx('p', {
                        className: 'text-gray-400',
                        children: t.description,
                      }),
                    ],
                  },
                  i
                )
              ),
            }),
          ],
        }),
      }),
      e.jsx('section', {
        className: 'py-20',
        children: e.jsxs('div', {
          className: 'container mx-auto px-4',
          children: [
            e.jsxs(s.div, {
              initial: { opacity: 0, y: 30 },
              whileInView: { opacity: 1, y: 0 },
              transition: { duration: 0.8 },
              className: 'text-center mb-16',
              children: [
                e.jsx('h2', {
                  className: 'text-4xl md:text-5xl font-bold text-white mb-6',
                  children: 'Information Sharing',
                }),
                e.jsx('p', {
                  className: 'text-xl text-gray-300 max-w-3xl mx-auto',
                  children:
                    'We do not sell your personal information. We only share information in specific, limited circumstances.',
                }),
              ],
            }),
            e.jsx('div', {
              className: 'grid grid-cols-1 md:grid-cols-2 gap-8',
              children: x.map((t, i) =>
                e.jsxs(
                  s.div,
                  {
                    initial: { opacity: 0, x: i % 2 === 0 ? -30 : 30 },
                    whileInView: { opacity: 1, x: 0 },
                    transition: { duration: 0.8 },
                    className:
                      'bg-gradient-to-br from-zinc-800 to-zinc-900 p-8 rounded-xl border border-zinc-700',
                    children: [
                      e.jsx('div', {
                        className:
                          'bg-gradient-to-r from-orange-500 to-red-500 p-4 rounded-lg w-16 h-16 mb-6 flex items-center justify-center',
                        children: e.jsx(u, { className: 'h-8 w-8 text-white' }),
                      }),
                      e.jsx('h3', {
                        className: 'text-2xl font-bold text-white mb-4',
                        children: t.category,
                      }),
                      e.jsx('p', {
                        className: 'text-gray-300 leading-relaxed mb-6',
                        children: t.description,
                      }),
                      e.jsx('ul', {
                        className: 'space-y-2',
                        children: t.examples.map((n, o) =>
                          e.jsxs(
                            'li',
                            {
                              className: 'flex items-center text-gray-300',
                              children: [
                                e.jsx(a, {
                                  className: 'w-5 h-5 text-green-400 mr-2',
                                }),
                                n,
                              ],
                            },
                            o
                          )
                        ),
                      }),
                    ],
                  },
                  i
                )
              ),
            }),
          ],
        }),
      }),
      e.jsx('section', {
        className: 'py-20 bg-black/20',
        children: e.jsx('div', {
          className: 'container mx-auto px-4',
          children: e.jsxs('div', {
            className: 'grid grid-cols-1 lg:grid-cols-2 gap-12 items-center',
            children: [
              e.jsx(s.div, {
                initial: { opacity: 0, x: -30 },
                whileInView: { opacity: 1, x: 0 },
                transition: { duration: 0.8 },
                children: e.jsxs('div', {
                  className:
                    'bg-gradient-to-br from-zinc-800 to-zinc-900 p-8 rounded-xl border border-zinc-700',
                  children: [
                    e.jsx('div', {
                      className:
                        'w-16 h-16 bg-gradient-to-br from-green-500 to-blue-500 rounded-xl flex items-center justify-center mb-6',
                      children: e.jsx(y, { className: 'w-8 h-8 text-white' }),
                    }),
                    e.jsx('h3', {
                      className: 'text-2xl font-bold text-white mb-4',
                      children: 'Data Security',
                    }),
                    e.jsx('p', {
                      className: 'text-gray-300 leading-relaxed mb-6',
                      children:
                        'We implement industry-standard security measures to protect your personal information, including encryption, access controls, and regular security audits.',
                    }),
                    e.jsxs('ul', {
                      className: 'space-y-2',
                      children: [
                        e.jsxs('li', {
                          className: 'flex items-center text-gray-300',
                          children: [
                            e.jsx(a, {
                              className: 'w-5 h-5 text-green-400 mr-2',
                            }),
                            'End-to-end encryption for sensitive data',
                          ],
                        }),
                        e.jsxs('li', {
                          className: 'flex items-center text-gray-300',
                          children: [
                            e.jsx(a, {
                              className: 'w-5 h-5 text-green-400 mr-2',
                            }),
                            'Regular security assessments and penetration testing',
                          ],
                        }),
                        e.jsxs('li', {
                          className: 'flex items-center text-gray-300',
                          children: [
                            e.jsx(a, {
                              className: 'w-5 h-5 text-green-400 mr-2',
                            }),
                            'Employee training on data protection',
                          ],
                        }),
                      ],
                    }),
                  ],
                }),
              }),
              e.jsx(s.div, {
                initial: { opacity: 0, x: 30 },
                whileInView: { opacity: 1, x: 0 },
                transition: { duration: 0.8 },
                children: e.jsxs('div', {
                  className:
                    'bg-gradient-to-br from-zinc-800 to-zinc-900 p-8 rounded-xl border border-zinc-700',
                  children: [
                    e.jsx('div', {
                      className:
                        'w-16 h-16 bg-gradient-to-br from-purple-500 to-cyan-500 rounded-xl flex items-center justify-center mb-6',
                      children: e.jsx(l, { className: 'w-8 h-8 text-white' }),
                    }),
                    e.jsx('h3', {
                      className: 'text-2xl font-bold text-white mb-4',
                      children: 'Contact Us',
                    }),
                    e.jsx('p', {
                      className: 'text-gray-300 leading-relaxed mb-6',
                      children:
                        'If you have questions about this privacy policy or want to exercise your data rights, please contact our privacy team.',
                    }),
                    e.jsxs('div', {
                      className: 'space-y-4',
                      children: [
                        e.jsxs('div', {
                          className: 'flex items-center text-gray-300',
                          children: [
                            e.jsx(l, {
                              className: 'w-5 h-5 text-purple-400 mr-3',
                            }),
                            e.jsx('span', { children: r.email }),
                          ],
                        }),
                        e.jsxs('div', {
                          className: 'flex items-center text-gray-300',
                          children: [
                            e.jsx(g, {
                              className: 'w-5 h-5 text-purple-400 mr-3',
                            }),
                            e.jsx('span', { children: r.phone }),
                          ],
                        }),
                        e.jsxs('div', {
                          className: 'flex items-center text-gray-300',
                          children: [
                            e.jsx(b, {
                              className: 'w-5 h-5 text-purple-400 mr-3',
                            }),
                            e.jsx('span', { children: r.address }),
                          ],
                        }),
                      ],
                    }),
                    e.jsxs(p, {
                      to: '/contact',
                      className:
                        'inline-flex items-center mt-6 px-6 py-3 bg-gradient-to-r from-purple-500 to-cyan-500 text-white font-semibold rounded-lg hover:shadow-lg transition-all duration-300',
                      children: [
                        'Contact Privacy Team',
                        e.jsx(j, { className: 'ml-2 w-4 h-4' }),
                      ],
                    }),
                  ],
                }),
              }),
            ],
          }),
        }),
      }),
      e.jsx('section', {
        className: 'py-20 bg-black/30',
        children: e.jsx('div', {
          className: 'container mx-auto px-4',
          children: e.jsxs(s.div, {
            className: 'text-center max-w-4xl mx-auto',
            initial: { opacity: 0, y: 30 },
            whileInView: { opacity: 1, y: 0 },
            transition: { duration: 0.8 },
            children: [
              e.jsxs('h2', {
                className: 'text-4xl md:text-5xl font-bold text-white mb-6',
                children: [
                  'Policy ',
                  e.jsx('span', {
                    className:
                      'bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent',
                    children: 'Updates',
                  }),
                ],
              }),
              e.jsx('p', {
                className: 'text-xl text-gray-300 mb-8',
                children:
                  'We may update this privacy policy from time to time to reflect changes in our practices or applicable laws. We will notify you of any material changes by posting the new policy on our website.',
              }),
              e.jsxs('div', {
                className:
                  'bg-gradient-to-br from-zinc-800 to-zinc-900 border border-zinc-700 rounded-xl p-6',
                children: [
                  e.jsxs('div', {
                    className: 'flex items-center justify-center mb-4',
                    children: [
                      e.jsx(N, { className: 'w-8 h-8 text-blue-400 mr-3' }),
                      e.jsx('span', {
                        className: 'text-blue-400 font-semibold',
                        children: 'Important Notice',
                      }),
                    ],
                  }),
                  e.jsxs('p', {
                    className: 'text-gray-300 text-center',
                    children: [
                      'This privacy policy was last updated on ',
                      e.jsx('strong', { children: c }),
                      '. Please review it periodically to stay informed about how we protect your information.',
                    ],
                  }),
                ],
              }),
            ],
          }),
        }),
      }),
    ],
  });
};
export { I as default };
