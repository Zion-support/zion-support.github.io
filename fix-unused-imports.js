...
if (match) {} const importList = match[1]; const source = match[2]; // Split imports and clean them; const imports = importList.split(',').map(imp => imp.trim()); const usedImports = imports.filter(imp => {}; // Check if this import is actually used in the file; const importName = imp.split( as ')[0].trim(); const alias = imp.includes(' as ) ? imp.split(' as ')[1].trim() : importName; // Skip if its a common unused import'; if (commonUnusedImports.includes(importName) || commonUnusedImports.includes(alias)) {} return false; } // Check if the import is used in the file (excluding the import statement itself); const contentWithoutImports = content.replace(importStatement, ); const usageRegex = new RegExp(`\\b${alias}\\b`, 'g);`;`'`'`
