...
// Function to find used identifiers in a file
function findUsedIdentifiers(content) {
  // Remove import statements and comments to avoid false positives
  const withoutImports = content.replace(/import\s+.*?from\s+['"][^'"]+['"];?\n?/g, '');
  const withoutComments = withoutImports.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');
  
  // Find all identifiers that look like component names or variables
  // This includes JSX components, destructured imports, and variable assignments
  const identifierRegex = /\b[A-Z][a-zA-Z0-9]*\b/g;
  const identifiers = new Set();
  let match;
  
  while ((match = identifierRegex.exec(withoutComments)) !== null) {
...
// Function to parse lucide-react imports
function parseLucideImports(importStatement) {
  if (!importStatement.includes('lucide-react')) return null;
...
function fixUnusedImports(filePath) {
  try {
    ...
    const usedIdentifiers = findUsedIdentifiers(content);
    
    // Find lucide-react import
    const lucideImportMatch = content.match(/import\s+\{\s*([^}]+)\s*\}\s+from\s+['"]lucide-react['"];?/);
    if (!lucideImportMatch) return false;
    
    const lucideImports = parseLucideImports(lucideImportMatch[0]);
    if (!lucideImports) return false;
    
    // Find which imports are actually used
    const usedImports = lucideImports.filter(imp => 
      usedIdentifiers.has(imp.alias) || usedIdentifiers.has(imp.original)
    );
    
    if (usedImports.length === lucideImports.length) {
