import fs from 'fs';'import path from 'path';'import { fileURLToPath } from 'url''function getRelativePath(fromPath, toPath) {/* TODO: Fix JSX expression */}
}
...
const replacements = [
  
      {
        ...
        pattern: /import\s+Link\s+from\s+'next\/link';/g''        replacement: `import Link from '${utilsPath}link';`},''      {
  
        pattern: /import\s+Image\s+from\s+'next\/image';/g''        replacement: `import Image from '${utilsPath}image';`},''      {
  
        pattern: /import\s+dynamic\s+from\s+'next\/dynamic';/g''        replacement: `import dynamic from '${utilsPath}dynamic';`,''      {/* TODO: Fix JSX expression */}
  ...
  n: /import\s+{\s*useRouter\s*}\s+from\s+'next\/navigation';/g,''        replacemen,`
  t: `import { useRouter } from '${utilsPath}navigation';`},''      {/* TODO: Fix JSX expression */}
  n: /import\s+{\s*usePathname\s*}\s+from\s+'next\/navigation';/g,''        replacemen,`
  t: `import { usePathname } from '${utilsPath}navigation';`},''      {/* TODO: Fix JSX expression */}
  n: /import\s+{\s*useSearchParams\s*}\s+from\s+'next\/navigation';/g,''        replacemen,`
  t: `import { useSearchParams } from '${utilsPath}navigation';`},''      {/* TODO: Fix JSX expression */}
  n: /import\s+{\s*Metadata\s*}\s+from\s+'next';/g,''        replacemen,`
  t: `import { Metadata } from '${typesPath}next';`},''      {/* TODO: Fix JSX expression */}
  n: /import\s+{\s*MetadataRoute\s*}\s+from\s+'next';/g,''        replacemen,`
  t: `import { MetadataRoute } from '${typesPath}next';`},''      {/* TODO: Fix JSX expression */}
  n: /import\s+type\s+{\s*Metadata\s*}\s+from\s+'next';/g,''        replacemen,`
  t: `import type { Metadata } from '${typesPath}next';`},''      {
  
        pattern: /'use client';/g''        replacement: "// 'use client'; // Removed for Vite compatibility","'"'      {/* TODO: Fix JSX expression */}"      }]
