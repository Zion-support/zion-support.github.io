...
export const globalAxiosErrorHandler = (error: unknown) => {
  ...
  const contentType = typeof error === 'object' && error && 'response' in (error as any) && (error as any).response && 'headers' in (error as any).response
    ? ((error as any).response as { headers?: Record<string, unknown> }).headers?.['content-type']
    : undefined;
