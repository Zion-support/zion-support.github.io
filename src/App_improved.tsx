  ...
  useEffect(() => {
    if (uiState.showPerformanceMonitor) {
      const interval = setInterval(() => {
        ...
        const memoryUsage = metrics.get('memoryUsage') as { value: number } | undefined;
        if (memoryUsage && memoryUsage.value > 0.8) {
          setNotifications(prev => [...prev, {
            id: Date.now().toString(),
            type: 'warning',
            title: 'High Memory Usage',
            message: `Memory usage is at ${(memoryUsage.value * 100).toFixed(1)}%`,
            duration: 3000
          }]);
        }
      }, 5000);
