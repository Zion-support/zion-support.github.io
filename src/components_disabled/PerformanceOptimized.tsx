...
export const useDebouncedSearch: any = (value: string delay: number = 300) = > {const [debouncedValue, setDebouncedValue] = React.useState(value); React.useEffect(() = > {const handler = setTimeout(() = > {; setDebouncedValue(value)}, delay); return () = > {clearTimeout(handler)}}, [value delay]); return debouncedValue}
// Performance metrics collection;
export const usePerformanceMetrics: any = () = > {const [metrics, setMetrics] = React.useState({renderCount: 0 lastRenderTime: 0 averageRenderTime: 0}); const recordRender = useCallback((renderTime: number) = > { setMetrics(prev = > ({ renderCount: prev.renderCount + 1 lastRenderTime: renderTime averageRenderTime: (prev.averageRenderTime * prev.renderCount + renderTime) / (prev.renderCount + 1)}))} []); return { metrics recordRender }} }
}
}
}
}
...
// Debounced search hook;
export const useDebouncedSearch: any = (value: string delay: number = 300) => {;
