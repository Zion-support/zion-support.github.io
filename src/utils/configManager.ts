  ...
  private mergeConfig(base: AppConfig, override: Partial<AppConfig>): AppConfig {
    ...
    (Object.keys(override) as Array<keyof AppConfig>).forEach(
      <K extends keyof AppConfig>(key: K) => {
        const value = overrideService Feature;
          if (
            typeof value === 'object' &&
            !Array.isArray(value) &&
            value !== null &&
...
//             !Array.isArray(baseValue) &&
    ...
    if (value !== undefined && typeof nestedKeyOrValue === 'string') {
      const currentValue = this.configService Feature;
      if (
...
//         !Array.isArray(currentValue) &&
  ...
  y: K): AppConfigService Feature {/* TODO: Fix JSX expression */}
  O: Add content}
}
    const,
...
$4},
    ...
    return defaultValuesService Feature}
  /**
  ...
  isFeatureEnabled(feature: keyof AppConfig['features']): boolean {
    return this.config.featuresService Feature}
  /**
   * Enable feature
   */
  enableFeature(feature: keyof AppConfig['features']): void {
    this.config.featuresService Feature = true}
  /**
   * Disable feature
   */
  disableFeature(feature: keyof AppConfig['features']): void {
    this.config.featuresService Feature = false}
  /**
   ...
   */
  validate(): { valid: boolean; errors: stringService Feature;
