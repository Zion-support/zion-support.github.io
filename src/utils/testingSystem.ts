...
class TestingSystem {
  ...
  public async runTest(testName: string, testFunction: () => Promise<void> | void, options: {
  ...
  public async runTestSuite(suiteName: string, tests: Array<{
    name: string;
    ...
    for (const testConfig of tests) {
      const result = await this.runTest(testConfig.name, testConfig.test, testConfig.options);
      suite.tests.push(result);
    }

    suite.duration = Date.now() - startTime;
    suite.status = suite.tests.every(test => test.status === 'pass') ? 'pass' : 'fail';
  ...
  private updateMetrics(testResult: TestResult): void {
    ...
    this.metrics.averageDuration = this.metrics.totalTests > 0 ?
      (this.metrics.averageDuration * (this.metrics.totalTests - 1) + testResult.duration) / this.metrics.totalTests :
      testResult.duration;
  ...
  public getTestSuites(): TestSuite[] {
    return Array.from(this.testSuites.values()).sort((a, b) => b.timestamp - a.timestamp);
  }
  ...
  public assertEqual(actual: unknown, expected: unknown, message?: string): void {
    if (actual !== expected) {
  ...
  public async assertAsync(conditionPromise: Promise<boolean>, message: string): Promise<void> {
  ...
  public async testAccessibility(): Promise<TestResult[]> {
    ...
    const headingTest = await this.runTest('Heading hierarchy', async () => {
      ...
      for (const heading of headings) {
        ...
        if (level > lastLevel + 1) {
          throw new Error(`Heading level ${level} follows level ${lastLevel} without intermediate levels`);
        }
        lastLevel = level;
      }
    ...
    const altTextTest = await this.runTest('Image alt text', async () => {
    ...
    const keyboardTest = await this.runTest('Keyboard navigation', async () => {
      ...
      for (const element of interactiveElements) {
        ...
        if (!element.hasAttribute('tabindex') && element.getAttribute('disabled') !== 'true') {
          ...
          if (computedStyle.display === 'none' || computedStyle.visibility === 'hidden') {
            throw new Error(`Focusable element is hidden: ${element.tagName}`);
          }
        }
      }
  ...
  public async testPerformance(): Promise<TestResult[]> {
    ...
    // Test page load time
    const loadTimeTest = await this.runTest('Page load time', async () => {
    ...
    const fcpTest = await this.runTest('First Contentful Paint', async () => {
...
export const runTest = (name: string, test: () => Promise<void> | void, options?: Parameters<TestingSystem['runTest']>[2]) => {
  return testingSystem.runTest(name, test, options);
};

export const runTestSuite = (name: string, tests: Parameters<TestingSystem['runTestSuite']>[1]) => {
