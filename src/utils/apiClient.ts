...
export class ApiClient {
  ...
  public async get<T = unknown>(url: string, options?: Partial<ApiRequest>): Promise<ApiResponse<T>> {
    return this.request<T>({ ...options, url, method: 'GET' });
  }

  public async post<T = unknown>(url: string, data?: unknown, options?: Partial<ApiRequest>): Promise<ApiResponse<T>> {
    return this.request<T>({ ...options, url, method: 'POST', body: data });
  }

  public async put<T = unknown>(url: string, data?: unknown, options?: Partial<ApiRequest>): Promise<ApiResponse<T>> {
    return this.request<T>({ ...options, url, method: 'PUT', body: data });
  }

  public async delete<T = unknown>(url: string, options?: Partial<ApiRequest>): Promise<ApiResponse<T>> {
    return this.request<T>({ ...options, url, method: 'DELETE' });
  }

  public async patch<T = unknown>(url: string, data?: unknown, options?: Partial<ApiRequest>): Promise<ApiResponse<T>> {
    return this.request<T>({ ...options, url, method: 'PATCH', body: data });
  }

  private async request<T = unknown>(requestConfig: ApiRequest): Promise<ApiResponse<T>> {
    ...
    try {
      const response = await this.executeRequest<T>(config, abortController.signal);
      
      // Cache successful responses
      if (config.cache && this.config.enableCaching && response.status >= 200 && response.status < 300) {
  ...
  private async executeRequest<T = unknown>(
    ...
    for (let attempt = 0; attempt <= (config.retries || this.config.retries); attempt++) {
      try {
        if (this.config.enableLogging) {
          console.log(`API Request [${attempt + 1}/${(config.retries || this.config.retries) + 1}]:`, {
    ...
    const requestTime = 0;
    throw this.createApiError(lastError || new Error('Request failed'), config, requestTime);
  }
  ...
  private async getCachedResponse<T>(requestKey: string): Promise<ApiResponse<T> | null> {
    try {
  ...
  private async cacheResponse<T>(requestKey: string, response: ApiResponse<T>): Promise<void> {
    try {
      const cacheData = {
        response,
        timestamp: Date.now(),
  ...
  private async waitForActiveRequest<T>(requestKey: string): Promise<ApiResponse<T>> {
        ...
        if (!this.activeRequests.has(requestKey)) {
          ...
          this.getCachedResponse<T>(requestKey).then(response => {
            if (response) {
              ...
              reject(new Error('Request failed and no cached response available'));
            }
          });
        } else {
  ...
  private createApiError(error: Error, config: ApiRequest, requestTime: number): ApiError {
    const errorWithResponse = error as Error & { response?: { status: number; statusText: string; data: unknown } };
    return {
...
if (typeof window !== 'undefined') {
  apiClient.configure({
    baseURL: process.env.NODE_ENV === 'production' ? 'https://api.ziontechgroup.com' : '/api',
    timeout: 30000,
    retries: 3,
    retryDelay: 1000,
    enableCaching: true,
