...
class EnhancedPerformanceMonitor {
      ...
      try {
        const observer = new PerformanceObserver((list) => {
          for (const entry of list.getEntries()) {
            this.recordMetric({
              name: entry.name,
              value: entryType === 'cumulative-layout-shift' ? (entry as PerformanceEntry & { value: number }).value : entry.duration,
  ...
  measurePerformance(name: string, startMark: string, endMark?: string): number | null {
    if (performance.measure) {
      try {
        const measureName = endMark ? `${startMark}-to-${endMark}` : `${startMark}-duration`;
  ...
  getPerformanceReport(): PerformanceReport {
    ...
    const lcpMetrics = this.metrics.filter(m => m.type === 'largest-contentful-paint');
    const frameRateMetrics = this.metrics.filter(m => m.name === 'frame-rate');
    
    const averageLoadTime = navigationMetrics.length > 0 
      ? navigationMetrics.reduce((sum, m) => sum + m.value, 0) / navigationMetrics.length 
      : 0;
    
    const averageRenderTime = paintMetrics.length > 0 
      ? paintMetrics.reduce((sum, m) => sum + m.value, 0) / paintMetrics.length 
      : 0;
    
    const averageFrameRate = frameRateMetrics.length > 0 
      ? frameRateMetrics.reduce((sum, m) => sum + m.value, 0) / frameRateMetrics.length 
      : 60;
    
    // Calculate performance score (0-100)
    const lcpScore = lcpMetrics.length > 0 ? Math.max(0, 100 - (lcpMetrics[0].value / 25)) : 100;
  ...
  private generateRecommendations(): string[] {
    ...
    if (lcpMetrics.length > 0 && lcpMetrics[0].value > 2500) {
      recommendations.push('Consider optimizing Largest Contentful Paint - currently above 2.5s');
    }
    
    if (frameRateMetrics.length > 0) {
      const avgFrameRate = frameRateMetrics.reduce((sum, m) => sum + m.value, 0) / frameRateMetrics.length;
      if (avgFrameRate < 30) {
        recommendations.push('Frame rate is below 30fps - consider optimizing animations and rendering');
      }
    }
