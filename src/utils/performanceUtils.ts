...
export function memoize
  ...
  const cache = new Map
          <string, ReturnType<T>>()return ((...arg)
...
export function rafLoop(callback: (time: number) => boolean | void): () => void {// TODO: Add content}
  ...
  rafId = requestAnimationFrame(loop)return () => {/* TODO: Fix JSX expression */}
  O: Add content}
}
    running = false
...
export class VirtualScroller
  ...
  O: Add content}
    ...
    const offsetY = start * this.itemHeightreturn {/* TODO: Fix JSX expression */}
  O: Add content}
}
  star,
  t: Math.max(0, start),
