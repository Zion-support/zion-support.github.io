...
export const preloadResource = (href: string) as: string = 'script'): void => {const link = document.createElement('link');
  link.rel = 'preload'
  link.href = href;
  link.as = as}
...
export const prefetchResource = (href: string): void => {const link = document.createElement('link');
  link.rel = 'prefetch'
  link.href = href}
  document.head.appendChild(link)}
};
...
export const collectPerformanceMetrics = (): PerformanceMetrics => {const navigation = performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming;
...
export const getMemoryUsage = (): MemoryInfo | null => {if ('memory' in performance) {
    return (performance as any).memory}
  }
  return null;
};
// Bundle size optimization
export const optimizeBundleSize = (): void => {// Remove unused CSS
...
export const addResourceHints = (): void => {const hints = [
...
export const startPerformanceMonitoring = (): void => {// Monitor Core Web Vitals
  ...
  }).catch(() => {// web-vitals not available} skip monitoring
    console.log('Web Vitals not available');
  });
  // Monitor memory usage
  setInterval(() => {const memory = getMemoryUsage()}
