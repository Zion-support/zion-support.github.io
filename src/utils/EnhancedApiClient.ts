...
;
  ...
  async request<T = any>(;
    ...
    const {
      method = "GET"headers = {};
body;timeout = this.config.timeout;retries = this.config.retries;cache = this.config.cacheEnabled;cacheTTL = this.config.cacheTTL;tags = [];
    } = options;
...
;
 ...
 timeout);const response = await fetch(url{;
    ...
    clearTimeout(timeoutId)
        ...
        if (!response.ok) {
          throw; new; ApiError({
            message: `HTTP ${response.status}: ${response.statusText}`,status: response.status,statusText: response.statusTexttimestam,p: Date.now();
retryCount: attempt,;
     });
        }
;
