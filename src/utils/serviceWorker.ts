...
self.addEventListener("install", (event: ExtendableEvent) : any => {
  event.waitUntil(
    Promise.all([
...
self.addEventListener("activate", (event: ExtendableEvent) : any => {
  event.waitUntil(
...
self.addEventListener("fetch", (event: FetchEvent) : any => {
  const { request } = event;
...
self.addEventListener("sync", (event: SyncEvent) : any => {
...
// Push notifications;
self.addEventListener("push", (event: PushEvent) : any => {
  if (event.data) {
...
self.addEventListener("notificationclick", (event: NotificationEvent) : any => {
...
self.addEventListener("periodicsync", (event: PeriodicSyncEvent) : any => {
