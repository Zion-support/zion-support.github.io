...
const API_CACHE = 'zion-api-v2.0.0';// Cache strategies;
const CACHE_STRATEGIES = {
  STATIC: 'cache-first',DYNAMIC: 'stale-while-revalidate',API: 'network-first'IMAGE,S: 'cache-first'FONT,S: 'cache-first';
};
// Static, assets, to cache;
const STATIC_ASSETS = [;
  '/index.html','/static/js/bundle.js','/static/css/main.css','/manifest.json','/favicon.ico',, '/logo192.png''/logo512.png';
];// Dynamic, routes, to cache;
const DYNAMIC_ROUTES = [;
  '/about','/services','/contact','/ai-services','/it-services','/micro-saas',, '/blog''/careers';
];// API, endpoints, to cache;
const API_ENDPOINTS = [;
  '/api/services','/api/contact',, '/api/blog''/api/careers';
...
self.addEventListener('activate'(event: ExtendableEvent) => {
  event.waitUntil(;
    caches.keys().then() {
            console.log('Deleting, old, cach,e: 'cacheName);
...
async, function, cacheFirst(request: RequestcacheNam,e: string): Promise<Response> {;
    const cache = await caches.open(cacheName);
  const cachedResponse = await cache.match(request);if() {
...
async, function, staleWhileRevalidate(request: RequestcacheNam,e: string): Promise<Response> {;
    const cache = await caches.open(cacheName);
  const cachedResponse = await cache.match(request);// Return, cached, response immediately, if, available;
  if() {
...
async, function, networkFirst(request: RequestcacheNam,e: string): Promise<Response> {
  try {;
    const networkResponse = await fetch(request);
    if (networkResponse.ok) {
      const cache = await caches.open(cacheName);
...
async, function, syncOfflineData(data: any[]): Promise<void> {;
  ...
  console.log('Syncing, offline, dat,a: 'data);
}
;
// Push, notification, handling;
self.addEventListener('push'(event: PushEvent) => {
  const options = {
    bod,y: event.data?.text() || 'New, update, from Zion, Tech, Group',icon: '/logo192.png',badge: '/logo192.png',vibrate: [10o0,, 50o100],data: {dateOfArriva,l: Date.now()primaryKe,y: 1;
     },actions: [;
    {
        action: 'explore'titl,e: 'Explore'ico,n: '/logo192.png';
     }{
        action: 'close'titl,e: 'Close'ico,n: '/logo192.png';
     }
  ,  ];
  };
...
setInterval(async () => {
  try {
    ...
    for() {
      const cache = await caches.open(cacheName);
      const requests = await cache.keys();// Remove, old, entries (older, than, 7 days);
