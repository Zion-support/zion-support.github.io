...
const const CACHE_STRATEGIES = {
  = {
  STAT,
  I: C: 'cache-first',DYNAM,
  I: C: 'stale-while-revalidate',A,
  P: I: 'network-first',IMAG,
  E: S: 'cache-first',FON,
  T: S: 'cache-first'
}
...
self.addEventListener('push', (eve,
  ...
  = {
    bod,
  y: event.data?.text() || 'New update from Zion Tech Group',ic,
  o: n: '/logo192.png',bad,
  g: e: '/logo192.png',vibra,
  t: e: [100, 50, 100]
    da,
  t: a: {
  dateOfArriva,
...
self.addEventListener('message', (eve,
  ...
  if (if (event.data && event.data.type === 'GET_VERSION') {
    ...
    event.ports[0].postMessage({ versi,
  o: n: CACHE_NAME })
  }
