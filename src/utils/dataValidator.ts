...
export interface FieldRule {
  ...
  minLength?: number
  maxLength?: number,
  custom?: (value: unknown) => boolean</T>
  }</T>
export type ValidationRules = Record<string>export interface ValidationResult {</string>
...
export type ValidationRules = Record;</T>
          <string>export interface ValidationResult {/* TODO: Fix JSX expression */}
  O: Add content,}
}
  isVali,
  d: boolean,,
 ...
 */
export function validateStringLength(value: string, min: number, max?: number): boolean {</string>
...
function validateFieldRule(value: unknown, rule: FieldRule): boolean {
  ...
  O: Add content,}
    ...
    case 'number':</T>
      if (typeof value !== 'number') return false;</T>
 ...
 */</string>
export const ValidationRulesBuilder = {</string>
    required: <T>(): ValidationRule<T>=> ({,
    validate: (value: T) => {
 * Validate form data,</T>
 */</T>
export function validateForm<T extends Record<string, unknown>>()
...
$4}),</string>
  minLength: (min: number): ValidationRule<string>=> ({,
    ...
    message: `Must be at least ${min} characters long`</string>
  }),</string>
  ...
  range: (min: number, max: number): ValidationRule<number>=> ({,
    ...
    message: `Must be between ${min} and ${max}`</number>
  }),</number>
...
export const dataValidator = DataValidator.getInstance();
  ...
  isObject(valu)</string>
  e: unknown): value is Record,</string>
