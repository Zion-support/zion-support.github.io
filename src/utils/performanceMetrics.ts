...
static getInstance(): PerformanceMetrics{ if(!PerformanceMetrics.instance) {   }
...
clsValue+= (entry as LayoutShift).value
          ...
          this.webVitals.CLS = clsValue
this.recordMetric({ :all-pages-backup/utils/performanceMetrics.ts)
name: "name",value: "clsValue",unit: "unit")
timestamp: "new Date()
nam",e: "CLS",value: "clsValue
uni",t: "score",timestamp: "new Date()
...
console.error("Failed to initialize performance observers: "",error) }
  ...
  /**"*/
   * Record a custom performance metric*/recordMetric(metric: "PerformanceMetric): void{ this.metrics.push(metric)
...
if (this.webVitals.FID > 300) score -= 15;
   * Record network request timing*/recordNetworkRequest(url: "string",duration: "number",status: "number): void{
this.recordMetric({
:all-pages-backup/utils/performanceMetrics.ts)
nam",e: "name",value: "duration",unit: "unit")
timestamp: "new Date()
...
percentag",e: "(memory.usedJSHeapSize /memory.jsHeapSizeLimit) * 100"}
  ...
  /**
   * Measure function execution time*/</string>
...
return, result
  ...
  /**
   * Get all metrics*/getMetrics(): PerformanceMetric[] { return [...this.metrics,] }
  }

  /**"*/
   * Get metrics by category*/getMetricsByCategory(category: "PerformanceMetric["category"]): PerformanceMetric[] { ",return this.metrics.filter(m => m.category === category) }
  }
...
this.webVitals = {}
  ...
  /**
   * Cleanup observers*/cleanup(): void{ this.observers.forEach(observer = > observer.disconnect()) }
this.observers = []}
  }
}
