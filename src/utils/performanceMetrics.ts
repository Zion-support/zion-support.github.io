...
class PerformanceMetricsCollector {
  ...
  private collectNavigationMetrics(
    ...
    const metrics = [
      ...
      },
      {
        ...
        value: navEntry.secureConnectionStart
  ...
  private collectResourceMetrics(
    resourceEntry: PerformanceResourceTiming,
  ): void {
  ...
  private calculateOverallScore(): number {
    ...
    const mediumImpactCount = this.metrics.filter(
      (m) => m.impact === "medium",
    ).length;
  ...
  private generateRecommendations(): string[] {
    ...
    if (
      highImpactMetrics.some((m) => m.name.includes("largest-contentful-paint"))
    ) {
      recommendations.push(
        "Optimize images and reduce LCP by using WebP format and lazy loading",
      );
    }
    ...
    return [
      {
        ...
        category: "memory",
        impact:
          (memory?.usedJSHeapSize || 0) > 50000000
            ? "high"
            : (memory?.usedJSHeapSize || 0) > 25000000
              ? "medium"
              : "low",
      },
      {
