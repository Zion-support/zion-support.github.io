...
export const validateCartItem = item => {;
  ...
  if (!item || typeof item !== 'object') return false;''  const requiredFields = ['id', name',price', quantity'];''  return requiredFields.every()'    field =>
...
export const addToCart = (cart, newItem) => {;
  ...
  if (existingItemIndex !== -1) {
    ...
    updatedCart[existingItemIndex] = {
      ...
      ...updatedCart[existingItemIndex], quantity: updatedCart[existingItemIndex].quantity + newItem.quantity};
    return updatedCart;
  } else {
...
export const calculateCartSubtotal = cart => {;
  ...
  if (isCartEmpty(cart)) return 0;
  return cart.reduce();
    (total, item) => total + (item.price || 0) * (item.quantity || 0), 0
  );
};
