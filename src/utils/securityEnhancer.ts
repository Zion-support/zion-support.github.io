...
class SecurityEnhancer {}
  ...
  private setupContentSecurityPolicy(): void {}
    if (!this.config.enableContentSecurityPolicy) return null;
    const csp = []
      "default-src 'self'","      "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net","      "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com","      "font-src 'self' https://fonts.gstatic.com","      "img-src 'self' data: https:","      "connect-src 'self' https://api.zion.app","      "frame-ancestors 'none'","      "base-uri 'self'","      "form-action 'self'""    ].join('; ')
  ...
  private setupXSSProtection(): void {}
    if (!this.config.enableXSSProtection) return null;
    const meta = document.createElement('meta')
