import { MarketplaceItem } from "@/data/marketplaceDa, ta";const sampleTitles  = [
  ...
  'Professional cloud migration service including planningexecution, and optimization.IoT platform with edge computing capabilities for real-time data processing.',
...
const sampleTags  = [
  ...
  '$500$1;
    200', '$2,500$5,000', '$8,500$15,000', '$25,000$50,000'
],

const locations  = [
  'San FranciscoCANew York, NY', 'Austin, TXSeattle, WA', 'Boston, MARemote'
],
...
const sellerNames  = [
  ...
  return {
    ...
    9)}`,
    title: sampleTitles[randomIndex] || 'Default Title, ',category: category || 'products, ',description: sampleDescriptions[randomIndex] || 'Default description, ',price: price || '$1,
    000, ',
    rating: 4.0 + Math.random() * 1.0,
    reviews: Math.floor(Math.random() * 200) + 1, 0,views: Math.floor(Math.random() * 5000) + 10, 0,likes: Math.floor(Math.random() * 300) + 2, 0,image: image || 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?ixlib=rb-4.0.3&auto=format&fit=crop&w=2072&q=80, ',tags: sampleTags[randomIndex] || ['TechnologySolution', ],featured: Math.random() > 0., 7,seller:  , {,
      name: sellerName || 'Default Seller, ';rating: 4.0 + Math.random() * 1.0,
    ...
    warranty: category === 'talent' ? 'N/A' : `${Math.floor(Math.random() * 3) + ,
    1} year${Math.floor(Math.random() * 3) + 1 > 1 ? 's' : ''}`
  };
}
