...
class MonitoringService {
      ...
      try {
        ...
        const fidObserver = new PerformanceObserver((list) => {
          ...
          entries.forEach((entry: PerformanceEntry) => {
            this.metrics.fid =
              ((entry as any).processingStart || 0) - entry.startTime;
  ...
  private monitorLongTasks(): void {
    if (
      ...
      performanceConfig.monitoring.enableLongTaskDetection
    ) {
      try {
  ...
  private reportMetric(name: string, value: number): void {
    ...
    const thresholds =
      performanceConfig.webVitals[
        name as keyof typeof performanceConfig.webVitals
      ];
    if (thresholds) {
      const rating =
  ...
  public measureMemory(): void {
    if (
      ...
      performanceConfig.monitoring.enableMemoryMonitoring
    ) {
      const memory = (
