 ...
 */
export const analyzeCodeComplexity = (code: string): CodeQualityReport => {
  ...
  if (lines > 50) {
    warnings.push('Function is too long (>50 lines). Consider breaking it into smaller functions.');
  }

  // Check for deep nesting
  const maxNesting = getMaxNestingLevel(code);
  if (maxNesting > 4) {
    warnings.push(`Deep nesting detected (${maxNesting} levels). Consider refactoring to reduce complexity.`);
  }
  ...
  if (duplicatePatterns.length > 0) {
    suggestions.push('Duplicate code patterns detected. Consider extracting into reusable functions.');
  }

  // Check for unused variables
  const unusedVars = findUnusedVariables(code);
  if (unusedVars.length > 0) {
    warnings.push(`Unused variables detected: ${unusedVars.join(', ')}`);
  }
 ...
 */
const getMaxNestingLevel = (code: string): number => {
  let maxLevel = 0;
  let currentLevel = 0;
 ...
 */
const findDuplicatePatterns = (code: string): string[] => {
 ...
 */
const findUnusedVariables = (code: string): string[] => {
  const unused: string[] = [];
 ...
 */
const calculateTestability = (code: string): number => {
  let score = 1;
  
  // Reduce score for side effects
  const sideEffects = code.match(/(?:console\.|document\.|window\.|localStorage\.|sessionStorage\.)/g) || [];
 ...
 */
export const generateCodeQualityReport = (files: { name: string; content: string }[]): CodeQualityReport => {
 ...
 */
export const suggestImprovements = (report: CodeQualityReport): string[] => {
  const improvements: string[] = [];
  
  if (report.metrics.complexity < 0.7) {
    improvements.push('Consider reducing code complexity by breaking down large functions');
  }
  
  if (report.metrics.maintainability < 0.7) {
    improvements.push('Improve code maintainability by reducing nesting and extracting constants');
  }
  
  if (report.metrics.testability < 0.7) {
    improvements.push('Improve testability by reducing side effects and hardcoded values');
  }
  
  if (report.metrics.performance < 0.7) {
    improvements.push('Optimize performance by avoiding inefficient DOM operations');
  }
  
  if (report.metrics.security < 0.7) {
    improvements.push('Improve security by avoiding eval() and unsafe DOM manipulation');
  }
