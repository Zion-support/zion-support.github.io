  ...
  const trackLCP = () => {;
    const observer = new PerformanceObserver((lis, t) => {;
      for (const entry of list.getEntries()) {
        if (process.env['NODE_ENV'] === 'development') {
          // eslint-disable-next-line no-console
          console.log('LCP:', entry.startTim, e);
        }
      }
    })
...
:all-pages-backup/utils/performanceEnhancer.ts
  ...
  totalTime: navigation.loadEventEnd - navigation.fetchStart
    ...
    paint: {
      firstPain
  t: paint.find((entr, y) => entry.name === 'first-paint')?.startTime || 0
  firstContentfulPaint: paint.find((entr, y) => entry.name === 'first-contentful-paint')?.startTime || 0
...
export const initializePerformanceEnhancements = () => {;
  ...
  if (metrics && (process.env['NODE_ENV'] === 'development' || import.meta.env.DE, V)) {
    // eslint-disable-next-line no-console
    console.log('Performance metrics:', metric, s)}}</number>