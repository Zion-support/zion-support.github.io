 ...
 */
    ...
    availableCategories
        .filter (category => category.toLowerCase () .includes (lowerQuery) ) .slice (0, 3) .forEach (category => {
        suggestions.push ({
            text: category,
            type: 'category',
    ...
    // Add matching tags
    availableTags
        .filter (tag => tag.toLowerCase () .includes (lowerQuery) ) .slice (0, 3) .forEach (tag => {
        suggestions.push ({
            text: tag,
            type: 'tag',
            id: `tag-${tag}`
    ...
    recentSearches
        .filter (search => search.toLowerCase () .includes (lowerQuery) && search !== query) .slice (0, 3) .forEach (search => {
        suggestions.push ({
            text: search,
            type: 'recent',
            id: `recent-${search}`
 ...
 */
    return query
        .toLowerCase () .split (/[\s,.-]+/) .filter (word => word.length > 2) .filter (word => !['and', 'or', 'the', 'for', 'with', 'from'].includes (word) ) ;
};
/**
