import React from 'react';'export const calculateRelevanceScore = (result, searchTerm) => {'export const calculateSearchMetrics = (results, searchTime) => {;
...
export const highlightSearchTerms = (text, searchTerm) => {;
    ...
    if (!searchTerm.trim())
        return text;
    const escaped = searchTerm.replace(/[.*+?^${}()|[\]\\]/g, \\$&');''    const regex = new RegExp(`(${escaped})`, gi');''    return text.replace(regex, <mark class="bg-yellow-200 text-black px-1 rounded">$1</mark>);"};"/**` * Check if a text contains the search term (case-insensitive)*/
 */
export const matchesSearchTerm = (text, searchTerm) => {;
 ...
 */
    switch (sortBy) {
        ...
        case 'price_asc': return sortedResults.sort((a, b) => (a.price ?? 0) - (b.price ?? 0));'        case 'price_desc': return sortedResults.sort((a, b) => (b.price ?? 0) - (a.price ?? 0));'        case 'rating': return sortedResults.sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));'        case 'date': return sortedResults.sort((a, b) => {''                const dateB = b.date ? new Date(b.date).getTime() : 0;
                return dateB - dateA;
            });
...
export const generateDynamicSuggestions = (query, recentSearches = [], availableCategories = [], availableTags = []) => {;
    ...
    if (query.trim()) {
        ...
        suggestions.push({
            ...
            text: query, type: 'recent',`'            id: `query-${query}`'        }) ;`    }
    ...
    availableCategories
        ...
        suggestions.push({
            ...
            text: category, type: 'category',`'            id: `category-${category}`'        }) ;`    }) ;
    // Add matching tags
    availableTags
    ...
    recentSearches
        ...
        suggestions.push({
            ...
            text: search, type: 'recent',`'            id: `recent-${search}`'        }) ;`    }) ;
...
export const hasActiveFilters = (filters) => {;
    ...
    return (filters.types.length > 0 ||'''        filters.category !== '' ||'        filters.minPrice > 0 ||'        filters.maxPrice < 10000 ||
        filters.minRating > 0 ||'''        filters.sort !== 'relevance');'};'/**
...
export const getActiveFilterCount = (filters) => {;
    ...
    if (filters.minRating > 0)
        count += 1;
    if (filters.sort !== 'relevance')'        count += 1;'    return count;
};
/**
...
export const getDefaultFilters = () => ({;
    ...
    filterSearchResults, generateDynamicSuggestions,
    calculateSearchMetrics, debounce,
    extractKeywords, formatSearchQuery,
