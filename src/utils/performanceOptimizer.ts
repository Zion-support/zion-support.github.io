...
/**
 * Measure page load timing (alias for compatibility)
 */
export const measurePageLoadTiming = (): Record<string, number> | null => {
  const metrics = measurePageLoad();
  if (!metrics) return null;
  
  return {
    loadTime: metrics.loadTime || 0,
    interactiveTime: metrics.interactiveTime || 0,
