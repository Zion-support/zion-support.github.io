 ...
 */
export const startRender = (componentName: string): void => {
  ...
  (window as any).__renderStartTimes = (window as any).__renderStartTimes || {};
  (window as any).__renderStartTimes[componentName] = startTime;
};

/**
 * End render timing for a component
 */
export const endRender = (componentName: string): void => {
  if (typeof window === 'undefined' || !window.performance) return;
  
  const endTime = performance.now();
  const startTimes = (window as any).__renderStartTimes || {};
  const startTime = startTimes[componentName];
