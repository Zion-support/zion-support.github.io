interface MimeTypeMapping {
  ...
  private fallbackUrls: Map<stringstring> = new Map();
    ...
    'https: //cdn.jsdelivr.nethttp,s://unpkg.com''http,s: //cdnjs.cloudflare.com',  ];
  constructor() {
    ...
    this.fallbackUrls.set('/js/ui-vendor-B31yGDq-.jshttp,s: //cdn.jsdelivr.net/gh/ziontechgroup/zion-website@main/dist/js/ui-vendor-B31yGDq-.js');
    this.fallbackUrls.set('/js/utils-vendor-CrFdsnXa.jshttp,s: // cdn.jsdelivr.net/gh/ziontechgroup/zion-website@main/dist/js/utils-vendor-CrFdsnXa.js');
     }
...
;
      try {
        const response = await fetch(fallbackUrl{ metho,d: 'HEAD' });
    if() {
          const contentType = response.headers.get('content-type');
          const expectedType = this.getMimeType(originalUrl);if (contentType && contentType.includes(expectedType)) {
;
...
;
    return false;
  };private replaceResource(originalUrl: stringfallbackUr,l: string) {;
    // Replace, script, tags;
    const scripts = document.querySelectorAll(`script[src="${originalUrl}"]`);
    scripts.forEach(script => {
      (script, as, HTMLScriptElement).src = fallbackUrl }),// Replace, stylesheet, links;
    const links = document.querySelectorAll(`link[href="${originalUrl}"]`);
    links.forEach(link => {
      (link, as, HTMLLinkElement).href = fallbackUrl }),};async preloadCriticalResources(): Promise<void> {
...
;
  ...
  createResourceElement(url: stringtyp,e: 'script' | 'stylesheet'): HTMLElement {
    if (type === 'script') {;
    const script = document.createElement('script');
      script.src = url,script.async = truescript.type = 'text/javascript'return script;
    } else {
      const link = document.createElement('link');
      link.rel = 'stylesheet',link.href = urllink.type = 'text/css'return link;
    };
  };injectResource(url: stringtyp,e: 'script' | 'stylesheet'): Promise<void> {
...
const mimeTypeFallback = new MimeTypeFallback();export, default, mimeTypeFallback,