...
export function reportWebVitals(
  ...
  if ('PerformanceObserver' in window) {
      ...
      for (const entry of entryList.getEntries()) {
        ...
        if (!(entry as any).hadRecentInput) {
          ...
          if (reportAllChanges) {
            ...
            if (debug) if (process.env.NODE_ENV === 'development') { if (import.meta.env.DEV) { console.log('CLS:', metric); } }
          }
        }
      }
    });
    ...
    addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden' && clsValue > 0) {
        ...
        onPerfEntry(metric);
        if (analyticsEndpoint) sendToAnalytics(metric, analyticsEndpoint);
        if (debug) if (process.env.NODE_ENV === 'development') { if (import.meta.env.DEV) { console.log('Final CLS:', metric); } }
      }
    });
  }
  ...
  if ('PerformanceObserver' in window) {
    const fidObserver = new PerformanceObserver((entryList) => {
      ...
      onPerfEntry(metric);
      if (analyticsEndpoint) sendToAnalytics(metric, analyticsEndpoint);
      if (debug) if (process.env.NODE_ENV === 'development') { if (import.meta.env.DEV) { console.log('FID:', metric); } }
  ...
  if ('PerformanceObserver' in window) {
    ...
    const lcpObserver = new PerformanceObserver((entryList) => {
      ...
      if (reportAllChanges || document.visibilityState === 'hidden') {
        ...
        onPerfEntry(metric);
        if (analyticsEndpoint) sendToAnalytics(metric, analyticsEndpoint);
        if (debug) if (process.env.NODE_ENV === 'development') { if (import.meta.env.DEV) { console.log('LCP:', metric); } }
      }
    });
    ...
    const fcpObserver = new PerformanceObserver((entryList) => {
      for (const entry of entryList.getEntries()) {
        if (entry.name === 'first-contentful-paint') {
          ...
          onPerfEntry(metric);
          if (analyticsEndpoint) sendToAnalytics(metric, analyticsEndpoint);
          if (debug) if (process.env.NODE_ENV === 'development') { if (import.meta.env.DEV) { console.log('FCP:', metric); } }
  ...
  if ('performance' in window && 'navigation' in performance) {
    ...
    if (navEntry) {
      ...
      onPerfEntry(metric);
      if (analyticsEndpoint) sendToAnalytics(metric, analyticsEndpoint);
      if (debug) if (process.env.NODE_ENV === 'development') { if (import.meta.env.DEV) { console.log('TTFB:', metric); } }
    }
  }
}
