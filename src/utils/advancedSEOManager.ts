...
class AdvancedSEOManager {
  ...
  private applySEO(): void {
    ...
    this.setMetaTag(
      "language",
      this.currentSEO.language || this.config.defaultLanguage,
    );
    this.setMetaTag(
      "author",
      this.currentSEO.author || this.config.defaultAuthor,
    );
  ...
  private updateOpenGraphTags(): void {
    ...
    const ogTags = [
      {
        property: "og:title",
        content: this.currentSEO.ogTitle || this.currentSEO.title,
      },
      {
        property: "og:description",
        content: this.currentSEO.ogDescription || this.currentSEO.description,
      },
      {
        property: "og:image",
        content:
          this.currentSEO.ogImage ||
          `${this.config.siteUrl}${this.config.defaultImage}`,
      },
      {
        property: "og:url",
        content: this.currentSEO.canonical || window.location.href,
      },
      { property: "og:type", content: this.currentSEO.ogType || "website" },
      { property: "og:site_name", content: this.config.siteName },
      {
        property: "og:locale",
        content: this.currentSEO.language || this.config.defaultLanguage,
      },
    ];
  ...
  private updateTwitterCardTags(): void {
    ...
    const twitterTags = [
      {
        name: "twitter:card",
        content: this.currentSEO.twitterCard || "summary_large_image",
      },
      {
        name: "twitter:title",
        content: this.currentSEO.twitterTitle || this.currentSEO.title,
      },
      {
        name: "twitter:description",
        content:
          this.currentSEO.twitterDescription || this.currentSEO.description,
      },
      {
        name: "twitter:image",
        content:
          this.currentSEO.twitterImage ||
          this.currentSEO.ogImage ||
          `${this.config.siteUrl}${this.config.defaultImage}`,
      },
    ];
  ...
  public auditPage(): SEOAuditResult {
    ...
    const internalLinks = document.querySelectorAll(
      'a[href^="/"], a[href*="' + this.config.siteUrl + '"]',
    );
    ...
    const externalLinks = document.querySelectorAll(
      'a[href^="http"]:not([href*="' + this.config.siteUrl + '"])',
    );
    externalLinks.forEach((link) => {
      if (
        !(link as HTMLAnchorElement).rel.includes("nofollow") &&
        !(link as HTMLAnchorElement).rel.includes("noopener")
      ) {
