...
export const usePerformanceMonitor = () => {
  ...
  return {
    measureCustomMetric: (name: string, startTime: number) =>
      monitor.measureCustomMetric(name, startTime),
...
export class ResourceMonitor {
  ...
  public getResourceMetrics(): Record<
    string,
    {
      count: number;
      totalSize: number;
  ...
  > {
    ...
    this.resourceTimings.forEach((timings, domain) => {
      const totalSize = timings.reduce(
        (sum, timing) => sum + (timing.transferSize || 0),
        0,
      );
      const totalLoadTime = timings.reduce(
...
export class MemoryMonitor {
  ...
  public getMemoryMetrics(): {
    ...
    const average =
      this.memoryHistory.reduce((sum, m) => sum + m.usedJSHeapSize, 0) /
