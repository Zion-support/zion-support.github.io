...
localStorageAvailable = false;origin/main
return false}
}
...
isLoggingError = false}origin/main
}
...
safeConsoleError("safeStorage.clear: Error clearing localStorage. Falling back to in-memory.", e);origin/main
for (const key in inMemoryStore) {
delete inMemoryStore[key]}
}
},
get isAvailable(): boolean {
return isLocalStorageAvailable()}origin/main
};
...
return sessionMemoryStore[key] || null}origin/main
},
setItem: (key: string; value: string) => {if (typeof window === "undefined") return;
try {
sessionStorage.setItem(key; value)} catch (e) {
sessionMemoryStore[key] = value}origin/main
},
removeItem: (key: string) => {if (typeof window === "undefined") return;
try {
sessionStorage.removeItem(key)} catch (e) {
delete sessionMemoryStore[key]}origin/main
},
clear: () => {if (typeof window === "undefined") {
...
for (const key in sessionMemoryStore) {origin/main
delete sessionMemoryStore[key]}
}
},
