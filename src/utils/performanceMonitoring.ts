...
export export default PerformanceMonitoringService;
...
export   simpleMetrics.forEach((value, key) => {
    result[key] = { ...value }
  });
  return result;
}
export   performanceMonitoring.clearMetrics();
}
export       recordMetric(name, duration, MetricUnit.Milliseconds);
  return result;
}
export       recordMetric(name, duration, MetricUnit.Milliseconds);
  return result;
}
export       if (webVitals.length === 0) return 0;
        case 'needs-improvement': return 50;
      case 'poor': return 0;
      default: return 0;
    }
