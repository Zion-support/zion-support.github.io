...
;
  ...
  e: number;
}).renderTime || (lastEntry as PerformanceEntry & { renderTime: number; loadTim,
  e: number }).loadTime)
        }
      })
...
;
        list.getEntries().forEach((entry) => {
          if (!(entry as PerformanceEntry & { hadRecentInput: boolean;
}).hadRecentInput) {
...
;
        list.getEntries().forEach((entry) => {
          this.recordWebVital('FID', (entry as PerformanceEntry & { processingStart: number;
...
:all-pages-backup/utils/performanceMonitoring.ts;
  ...
  rating: this.getCustomRating(value, unit);
    const metric: CustomMetric = {
      ...
      unit;
      ratin,
  g: this.getCustomRating(value, unit)
      timestamp: Date.now()
    }
  ...
  private ge tCustomRating(value: number, unit: string): 'good' | 'needs-improvement' | 'poor' {
