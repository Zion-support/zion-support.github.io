...
export class PerformanceMonitor {
    ...
    if (this.metrics.fid !== null) {
      scores.push(
        this.metrics.fid <= 100
          ? 100
          : Math.max(0, 100 - (this.metrics.fid - 100) / 1),
      );
    }
    ...
    if (this.metrics.fcp !== null) {
      scores.push(
        this.metrics.fcp <= 1800
          ? 100
          : Math.max(0, 100 - (this.metrics.fcp - 1800) / 18),
      );
    }
    ...
    if (this.metrics.ttfb !== null) {
      scores.push(
        this.metrics.ttfb <= 800
          ? 100
          : Math.max(0, 100 - (this.metrics.ttfb - 800) / 8),
      );
    }
  ...
  public getRecommendations(): string[] {
    ...
    if (this.metrics.cls && this.metrics.cls > 0.1) {
      recommendations.push(
        "Improve Cumulative Layout Shift - ensure stable layout and avoid dynamic content insertion",
      );
    }
    ...
    if (this.metrics.ttfb && this.metrics.ttfb > 800) {
      recommendations.push(
        "Improve Time to First Byte - optimize server response time",
      );
    }

    if (this.metrics.resourceSize > 1024 * 1024) {
      // 1MB
      recommendations.push(
        "Reduce resource size - consider code splitting and compression",
      );
    }

    if (this.metrics.resourceCount > 50) {
      recommendations.push(
        "Reduce number of resources - combine and minify files",
      );
    }
