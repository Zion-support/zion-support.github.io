...
export const debounce = <T extends (...args: any[]) => any>(fun",c: "T",wait: "number</T>
...
const [metrics, setMetrics ] = useState<{ fcp?: number
lcp?: number
fid?: number
cls?: number
ttfb?: number
...
ttf",b: "navigation?.responseStart - navigation?.requestStart)
    ...
    //Monitor performance after page load
if(document.readyState = == "complete") { 
updateMetrics() else {  window.addEventListener("load", updateMetrics) }
    }

    //Monitor Core Web Vitals if("web-vitals" in, window) { import("web-vitals").then(({ getCLS, getFID, getFCP, getLCP, getTTFB }) => { " }
getCLS((metric) => setMetrics(prev => ({ ...prev, cls: "metric.value "})))
getFID((metric) => setMetrics(prev => ({ ...prev, fid: "metric.value "})))
getFCP((metric) => setMetrics(prev => ({ ...prev, fcp: "metric.value "})))
getLCP((metric) => setMetrics(prev => ({ ...prev, lcp: "metric.value "})))
...
//Memory usage monitoring
export const useMemoryMonitoring = () => { const [memoryInfo, setMemoryInfo ] = useState<{
    usedJSHeapSize?: number
totalJSHeapSize?: number
...
if(memory) {
setMemoryInfo({ :all-pages-backup/utils/performanceOptimizations.ts
...
export const preloadCriticalResources = () => { 
  ...
  preloadResource("/fonts/inter-var.woff2", "font")
  preloadResource("/fonts/inter-var.woff", "font")
  //Preload critical images preloadResource("/images/hero-bg.webp", "image")
  preloadResource("/images/logo.svg", "image")
  //Preload critical CSS preloadResource("/styles/critical.css", "style")
) => {
 }
}

//Bundle size monitoring
export const useBundleSizeMonitoring = () => { const [bundleSize, setBundleSize ] = useState<{
    totalSize?: number
jsSize?: number
cssSize?: number
imageSize?: number
...
:all-pages-backup/utils/performanceOptimizations.ts
const performanceOptimizations = { const performanceOptimizations = {
debounce
throttle
useIntersectionObserver
useLazyImage
