  ...
  static async testBundleSize(): Promise<TestResult> {
    ...
    try {
      ...
      for (const script of scripts) {
        try {
          const response = await fetch(script.getAttribute('src')!, { method: 'HEAD' });
  ...
  static async testKeyboardNavigation(): Promise<TestResult> {
    ...
    try {
      ...
      focusableElements.forEach((element, index) => {
        ...
        if (role && !['button', 'link', 'heading', 'img', 'text', 'navigation', 'main', 'banner', 'contentinfo'].includes(role)) {
      ...
      elements.forEach((element) => {
        ...
        if (backgroundColor && color && backgroundColor !== 'rgba(0, 0, 0, 0)' && color !== 'rgba(0, 0, 0, 0)') {
          ...
          if (backgroundColor === color) {
            contrastIssues++;
            issues.push(`Element has same background and text color: ${element.tagName}`);
          }
        }
      });
      
      const contrastScore = totalElements > 0 ? ((totalElements - contrastIssues) / totalElements) * 100 : 100;
      let status: 'pass' | 'fail' | 'warning' = 'pass';
        ...
        if (element) {
          ...
          if (content) {
            if (tag.name === 'description' && content.length < 120) {
              issues.push(`${tag.name} is too short: ${content.length} characters`);
            } else if (tag.name === 'description' && content.length > 160) {
              issues.push(`${tag.name} is too long: ${content.length} characters`);
            }
          }
        } else {
