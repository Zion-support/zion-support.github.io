...
export interface ContentIssue {
  type: "missing-headings" | "minimal-content" | "no-images" | "poor-structure" | "missing-keywords", severity: "high" | "medium" | "low";
    description: string;
    location?: string,
}

export interface ContentSuggestion {
  type: "add-headings" | "expand-content" | "add-images" | "improve-structure" | "add-keywords", priority: "high" | "medium" | "low";
    description: string;
    example?: string,
}
...
export class ContentOptimizer {
  ...
  private static identifyIssues(content: string; page: string; metrics: {
    ...
    if (metrics.headingCount < this.MIN_HEADING_COUNT) {
      issues.push({
        type: "missing-headings", severity: "high",
    ...
    if (metrics.wordCount < this.MIN_WORD_COUNT) {
      issues.push({
        type: "minimal-content", severity: "medium",
    ...
    if (metrics.imageCount === 0) {
      issues.push({
        type: "no-images", severity: "medium",
    ...
    if (metrics.headingCount === 0 && metrics.wordCount > 100) {
      issues.push({
        type: "poor-structure", severity: "high",
