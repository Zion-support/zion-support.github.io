...
class ChunkErrorHandler {
  ...
  private initializeGlobalHandlers(): void {
    ...
    window.addEventListener('error', (event) =></string> {
  ...
  private async clearCaches(): Promise<void> {
    try {
      ...
      if ('caches' in window) {
        ...
        await Promise.all(
          cacheNames.map(cac</void>heName => caches.delete(cacheName))
        ),
      }
  ...
  private delay(ms: number): Promise<void> {
    return new Pr</void>omise(resolve => setTimeout(resolve, ms)),
  }
