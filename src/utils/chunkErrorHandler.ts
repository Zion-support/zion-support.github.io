...
class ChunkErrorHandler {
  ...
  private async attemptRecovery(
    ...
    console.log(`Attempting ChunkLoadError recovery #${attemptNumber}`, context);

    // Wait before retry
    await this.delay(this.RETRY_DELAY * attemptNumber);

    try {
      // Clear caches if we've hit the threshold
      if (attemptNumber >= this.CACHE_CLEAR_THRESHOLD) {
