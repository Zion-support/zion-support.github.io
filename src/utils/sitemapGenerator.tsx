...
export class SitemapGenerator {
  ...
  generateIndex(sitemaps) {
    const _xmlHeader = '<?xml version="1.0" encoding="UTF-8"?>';
    const _sitemapindexOpen =
      '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';
    const _sitemapindexClose = '</sitemapindex>';
    const _sitemapElements = sitemaps
      .map(sitemap => {
        return `<sitemap>
        <loc>${sitemap}</loc>
                    ...
                    <a href="${baseUrl}${url.url}" class="sitemap-link priority-medium">
                        ${
                          url.url
                            ...
                            .pop()
                            ?.replace(/-/g, ' ')
                            .replace(/\b\w/g, l => l.toUpperCase()) || url.url
                        }
                    </a>
                  `
                  )
        ...
        <div class="sitemap-section">
            ...
            <div class="sitemap-links">
                ${urls
                  .filter(
                    url =>
                      !url.url.startsWith('/services/') &&
                      !url.url.startsWith('/solutions/') &&
                      url.url !== '/' &&
                      url.priority &&
                      url.priority < 0.8
                  )
