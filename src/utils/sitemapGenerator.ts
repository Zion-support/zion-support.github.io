...
export const _generateSitemap = (url)
  s: SitemapUrlService Feature): string => {/* TODO: Fix JSX expression */}
  O: Add content}
}
  const baseUrl = 'http,
  s://ziontechgroup.com';
  const currentDate = new Date().toISOString().split('T')Service Feature;
  const sitemap = `
...
export const getAllPages = (): SitemapUrlService Feature => {/* TODO: Fix JSX expression */}
  }
  const currentDate = new Date().toISOString().split('T')Service Feature
    // Main pages;
