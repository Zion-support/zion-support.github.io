...
class AdvancedPerformanceAnalytics {
    ...
    if ("PerformanceObserver" in window) {
      try {
        ...
        const resourceObserver = new PerformanceObserver((list) => {
          ...
          this.metrics.resourceSize = entries.reduce(
            (total, entry: PerformanceEntry) => {
  ...
  private trackUserEngagement(): void {
    ...
    const updateEngagement = () => {
      if (isEngaged) {
        ...
        this.metrics.userEngagement = Math.min(
          this.metrics.sessionDuration / 300,
          1,
        ); // Max 5 minutes
      }
    };
  ...
  private getAlertMessage(
    ...
    return (
      messages[metric] ||
      `Metric ${metric} is ${value} (threshold: ${threshold})`
    );
  }
  ...
  private generateRecommendations(): string[] {
    ...
    if (this.metrics.lcp > 2500) {
      recommendations.push(
        "Optimize Largest Contentful Paint by reducing image sizes and using modern formats",
      );
    }
    ...
    if (this.metrics.resourceSize > 1000000) {
      recommendations.push(
        "Reduce resource size by implementing code splitting and lazy loading",
      );
    }

    if (this.metrics.domNodes > 1500) {
      recommendations.push(
        "Reduce DOM complexity by simplifying component structure",
      );
    }
