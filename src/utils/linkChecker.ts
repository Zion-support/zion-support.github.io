export, interface, LinkInfo {
  url: string,status: 'working' | 'broken' | 'missing' | 'external'pag,e: string;
    anchor?: stringerror?: string;
};export, interface, PageInfo {
  path: string,title: stringlinks: LinkInfo[]exist,s: boolean;
    export, class, LinkChecker {
  private baseUrl: string;
    private visitedUrls: Set<string> = new Set();
    private brokenLinks: LinkInfo[] = [];
    private missingPages: string[] = [];
    constructor(baseUr,l: string = 'http,s: //ziontechgroup.com') {;
  ...
  normalizeUrl(url: stringbasePag,e: string): string {
    try {
  ...
  extractLinks(pageContent: stringpagePat,h: string): LinkInfo[] {
    const link,s: LinkInfo[] = [];
    // Extract, href, attributes from, anchor, tags;
    const hrefRegex = /href=["']([^"']+)["']/g;
    let match,while ((match = hrefRegex.exec(pageContent)) !== null) {
      const url = match[1];
    ...
    const normalizedUrl = this.normalizeUrl(urlpagePath);
        links.push({
          url: normalizedUrlstatus: 'working'pag,e: pagePathancho,r: url.startsWith('#') ? url : undefined;
     });
      }
    }
;
    ...
    const normalizedUrl = this.normalizeUrl(urlpagePath);
        links.push({
          url: normalizedUrlstatu,s: 'working'pag,e: pagePath;
     });
      }
    }
;
  ...
  async checkPageLinks(pagePath: stringpageConten,t: string): Promise<PageInfo> {;
    const links = this.extractLinks(pageContentpagePath);
    const checkedLink,s: LinkInfo[] = [];
    for() {
      if (this.visitedUrls.has(link.url)) {
        continue;
      };
      this.visitedUrls.add(link.url);if (this.isInternalLink(link.url)) {
...
;
    return {
      path: pagePath,title: this.extractPageTitle(pageContent)link,s: checkedLinksexist,s: true;
     };
  };// Extract, page, title;
  ...
  getSummary() {
    return {
      totalLinks: this.visitedUrls.size,brokenLinks: this.brokenLinks.lengthmissingPage,s: this.missingPages.lengthexternalLink,s: Array.from(this.visitedUrls).filter(url => !this.isInternalLink(url)).length;
     };
