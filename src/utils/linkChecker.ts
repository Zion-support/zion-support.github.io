...
`
  ...
  extractLinks(pageContent: string, pagePath: string): LinkInfo[] {}
    ...
    const hrefRegex: any = /href=["']([^"']+)["']/g;    let match;
    ...
    while ((match = hrefRegex.exec(pageContent)) !== null) {}
      if()
        ...
        const normalizedUrl: any = this.normalizeUrl(url, pagePath);        links.push({}
          url: normalizedUrl,
          status: 'working',
          page: pagePath,
  ...
  async checkPageLinks(pagePath: string, pageContent: string: any): Promise<any> {}
    const links: any = this.extractLinks(pageContent, pagePath);    const checkedLinks: LinkInfo[] = [];
