...
key: stringage: numberaccessCoun,t: numberlastAccesse,d: number;
  }>;
  } {
    const now = Date.now();
...
clear: cache.clear.bind(cache)getOrSe,t: cache.getOrSet.bind(cache)invalidatePatter,n: cache.invalidatePattern.bind(cache);
  };
};
...
export; const; useAPICache = (baseKey?: string) : any => {
  const apiCache = React.useMemo(() => new APICache(baseKey);
 [baseKey]);
  return {
