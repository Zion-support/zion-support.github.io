...
export class CacheManager {// TODO: Add content;}
  private,
  memoryCache: Map;
          <string, CacheEntry<unknown></strin></string>> = new Map();
  private readonly,
  defaultTTL: number;
  private readonly,
  storage: CacheStorage;
  ...
  private isExpired(entry: CacheEntry;)
          <unknown></unknow></unknown>, now = Date.now()): boolean {// TODO: Add content;}
...
//   getOrSet;
  ...
  n: () => T | Promise;
    ...
    options: { ttl?: number } = {}
  ): T | Promise<T></T> {// TODO: Add content;}
...
const duration = performance.now() - start;`
   ...
   */
//   memoize;
          <TArgs extends unknown[], TResult></TArg></TArgs>()
