...
class ErrorMonitoringSystem {
  ...
  private async sendToMonitoringService(errorReport: ErrorReport): Promise<void> {
    try {
      ...
      logger.error('Failed to send error to monitoring service', 'ErrorMonitoring', { error });
    }
  }

  setUserId(userId: string): void {
    this.userId = userId;
    logger.info('User ID set for error monitoring', 'ErrorMonitoring', { userId });
  }
  ...
  getErrorStats(): ErrorStats {
    ...
    const lastError = this.errors.length > 0 ? this.errors[this.errors.length - 1].timestamp : undefined;
  ...
  private getSessionDuration(): number {
    ...
    return (Date.now() - parseInt(this.sessionId.split('_')[1])) / (1000 * 60 * 60);
  }
  ...
  clearResolvedErrors(): number {
    ...
    if (clearedCount > 0) {
      logger.info('Cleared resolved errors', 'ErrorMonitoring', { clearedCount });
    }
  ...
  static captureReactError(error: Error, errorInfo: { componentStack: string }): void {
  ...
  static captureNetworkError(url: string, status: number, message: string): void {
    errorMonitoring.captureError({
  ...
  // Custom error helper
  static captureCustomError(message: string, context?: Record<string, unknown>, severity: ErrorReport['severity'] = 'medium'): void {
    errorMonitoring.captureError({
      message,
      category: 'custom',
      severity,
