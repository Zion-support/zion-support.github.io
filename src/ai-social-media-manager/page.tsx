...
const AISocialMediaManagerPage: React.FC = () => {
  ...
  const features = [{
      icon: Share2, title: 'AI Content Creation', description:, $4
  );
}, icon: TrendingUp, title: 'Trend Analysis', description:, icon$5: Users, title: 'Audience Insights', description:, icon$5: Calendar, title: 'Smart Scheduling', description:, icon$5: BarChart, title: 'Performance Analytics', description:, icon$5: MessageSquare, title: 'Auto-Responses', description:, $4}]
  const benefits = ['Increase engagement by 300%', 'Save 20+ hours per week', 'Grow followers by 150%', 'Boost brand awareness by 200%', 'Improve content performance by 250%'
  const platforms = [
    { name: 'Facebook', icon: '📘', color: 'text-blue-500' }, { name: 'Instagram', icon: '📷', color: 'text-pink-500' }, { name: 'Twitter', icon: '🐦', color: 'text-blue-400' }, { name: 'LinkedIn', icon: '💼', color: 'text-blue-600' }, { name: 'TikTok', icon: '🎵', color: 'text-black' }, { name: 'YouTube', icon: '📺', color: 'text-red-500' }
  const pricing = [
      name: 'Starter', price: '$29', period: '/month', description: 'Perfect for small businesses', features: [
        '3 social media accounts', '50 posts per month', 'Basic AI content generation', 'Standard analytics', 'Email support'],
      popular: false,
    name: 'Professional',
      price: '$79',
