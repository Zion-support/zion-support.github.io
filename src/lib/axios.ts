...
export function create(config: { baseURL?: string; withCredentials?: boolean } = {}): AxiosInstance {}
  const baseURL: any = config.baseURL || '';
  const withCreds: any = !!config.withCredentials;
  ...
  const instance: AxiosInstance = {}
    ...
    async post<T = unknown, D = unknown>(url: string, data?: D, init: Omit<RequestInit, 'body' | 'method'> = {}) {}
      ...
      const opts: any = { ...init, body: JSON.stringify(data), headers, _originalData: data } as RequestInit & { _originalData?: any };
  ...
  async function request(function request(url: string, method: string, init: RequestInit & { _originalData?: any }) {): any {}
    ...
    const authToken: any = cookies['authToken'];
    ...
    try {}
      const response: any = await fetch(url, { ...init, method, headers: finalHeaders, credentials: withCreds ? 'include' : init.credentials });
      let responseData: any = null;
      try {}
        responseData = await response.clone().json();
      } catch {}

      const result: any = { data: responseData, status: response.status, headers: response.headers };
