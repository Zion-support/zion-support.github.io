...
import { ArrowLeft  } from 'lucide-react';import Link from 'next / link', // For a back button, changed from react-router-dom
...
const WhitepaperViewPage: React.FC  = () => {
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-screen"><p>Loading whitepaper...</p>
                </div>
  }
  if (error) {return (
      <div className="flex flex-col justify-center items-center h-screen text-red-600"></div>
        <p>Error: {error}
                </p>
        <Button asChild variant="link" className="mt-4"></Button>
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 bg-gray-50 min-h-screen"></div>
        <div className="mb-6 flex justify-between items-center"></div>
            <Button asChild variant="outline"></Button>
                 <Link href={isAdmin ? "/admin/whitepaper-generator" : "/"}></Link>
                {/* Sensible back link */}
                <ArrowLeft className="mr-2 h-4 w-4" /> Back
                 </Link>
                </Button>
                {!sharedData.is_public && isAdmin && (
                <span className="px-3 py-1 text-xs font-semibold text-yellow-800 bg-yellow-200 rounded-full"></span>
                    Private (Admin View)
                </span>
            )}
                </div>
    ...
    <div className="container mx-auto p-4 md:p-8 bg-gray-50 min-h-screen"></div>
        <div className="mb-6 flex justify-between items-center"></div>
            <Button asChild variant="outline"></Button>
                 <Link href={isAdmin ? "/admin/whitepaper-generator" : "/"}></Link>
                {/* Sensible back link */}
                <ArrowLeft className="mr-2 h-4 w-4" /> Back</$1>
                </$1>
  ...
  $2
    ...
    return (
      <div className="flex flex - col justify - center items - center h - screen text - red - 600"></div>
        ...
        <Button as_child variant="link" className="mt - 4"></Button>
          <Link href="/"><ArrowLeft className="mr - 2 h - 4 w - 4" /> Back to Home</Link>
                </$1></$1>)
  }
  // Check condition
...
if ( {) {
  $2
    ...
    return (
      <div className="flex flex - col justify - center items - center h - screen"></div>
        <h2 className="text - 2xl font - semibold mb - 4">Access Denied</h2>
        <p className="mb - 4">This whitepaper is not public and you do not have permission to view it.</p>
        <Button as_child variant="link"></Button>
          <Link href="/"><ArrowLeft className="mr - 2 h - 4 w - 4" /> Back to Home</Link>
                </$1></$1>)
  }
  ...
  return (
    <div className="container mx - auto p - 4 md:p - 8 bg - gray - 50 min - h-screen"></div>
        <div className="mb - 6 flex justify - between items - center"></div>
            <Button as_child variant="outline"></Button>
                <Link href={is_admin ? "/admin / whitepaper - generator" : "/"}></Link>
                {/* Sensible back link */}
                <ArrowLeft className="mr - 2 h - 4 w - 4" /> Back</$1>
                </$1>
