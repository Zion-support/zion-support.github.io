...
// Filter controls;
const MarketplaceFilterControls = ({sortBy, setSortBy, filterCategory, setFilterCategory, categories, showRecommended, setShowRecommended, loading;}: any,) => (
}: any) => (
...
<div className="flex items-center gap-2">
    ...
    <Button variant={showRecommended ? "default" : "outline"} size="sm" onClick={$2}>
...
<div className="text - sm text - muted - foreground">Available</div></$1></$1></$1></$1>),
// Filter controls;
const MarketplaceFilterControls = ({sort_by, setSortBy, filter_category, setFilterCategory, categories, show_recommended, setShowRecommended, loading;}: any, ) =>: any (
  <div className="flex flex - wrap gap - 4 mb - 6 p - 4 bg - muted / 30 rounded - lg relative">
    {loading && <Spinner className="absolute right - 4 top - 4 h - 4 w - 4 text - primary" />}
    </div>
<div className="flex items - center gap - 2">
...
import {toast} from '@/hooks / use - toast'
// Product card;
const MarketplaceCard = ({product, onViewDetails, onAddToCart}: {product: ProductListing, onViewDetails: (id: string) =>: any void, onAddToCart: () => void}) => {const { format_price} = use_currency ()
  return (
  <Card className="h - full hover:shadow - lg transition - shadow">
    <CardHeader className="pb - 3">
      </div>
...
<div className="text - xl font - bold text - blue - 600">{format_price (product.price ?? 0)}</div>
        ...
        <Button size="sm" variant="outline" className="flex-1" onClick={$2}>
          View Details;
        </Button>
      </div>
    </CardContent>
...
function MarketplacePageContent() {const router = useRouter()
  ...
  const fetchProducts = useCallback(async (page: number, limit: number,) => {// Simulate API delay;
...
function MarketplacePageContent() {const router = useRouter(),
  ...
  const fetchProducts = useCallback(async (page: number, limit: number) => {// Simulate API delay;
    ...
    try {
      ...
      processedDataset.sort((a, b,) => {switch (sortBy) {
          case 'price-low':
    ...
    } catch (error) {logErrorToProduction('Error in fetchProducts:', { data: error})
  ...
  const {items: products;
    ...
    refresh;
    scrollToTop;
    loadMore;} = useInfiniteScrollPagination(fetchProducts, 12)
  // Refresh when filters change;
  useEffect((,) => {const timeoutId = setTimeout((,) => {
      refresh()}, 100)
    return () => clearTimeout(timeoutId)
  }, [sortBy, filterCategory, showRecommended, refresh])
  const marketStats = useMemo((,) => {if (products.length === 0) return null;
    return {
  ...
  const categories = useMemo((,) => {return ["AI & Machine Learning", "Cloud Services", "Software Development", "Professional Services", "Hardware & Infrastructure"]}, [])
  const [showScrollTop, setShowScrollTop] = useState(false)
  useEffect((,) => {const handleScroll = () => setShowScrollTop(window.scrollY > 800)
  ...
  // Loading state;
  if (loading && products.length === 0) {data-testid="add-to-cart-listing-button">
...
function MarketplacePageContent() {const router = useRouter(),
  ...
  const fetchProducts = useCallback(async (page: number, limit: number,) => {// Simulate API delay;
    ...
    try {
      ...
      // Apply recommended filtering;
      if (showRecommended) {processedDataset = processedDataset && processedDataset.filter(p => (p && p.rating || 0) >= 4 && 4.5 || (p && p.aiScore || 0) >= 85)}

      // Sort the processed dataset;
      processedDataset && processedDataset.sort((a, b,) => {switch (sortBy) {
          case 'price-low':
 ...
 */
function MarketplacePageContent() {const router = use_router (),
  ...
  const fetch_products = useCallback (async (page: number, limit: number, ) => {// Simulate API delay;
...
if ( {) {
  $2;}
      ...
      // Sort the processed dataset;
      processed_dataset.sort ((a, b, ) => {switch (sort_by) {
          case 'price - low':
            return (a.price || 0) - (b.price || 0),
          case 'price - high':
            return (b.price || 0) - (a.price || 0),
    ...
    } catch (error) {logErrorToProduction ('Error in fetch_products:', { data: error}),
      throw new Error ('Failed to load marketplace data. Please try again.')
    }
  ...
  const {items: products,
    ...
    refresh,
    scrollToTop,
    load_more;} = useInfiniteScrollPagination (fetch_products, 12),
  // Refresh when filters change;
  useEffect ((, ) => {const timeout_id = set_timeout ((, ) => {
      refresh ()}, 100),
    return () => clear_timeout (timeout_id)
  }, [sort_by, filter_category, show_recommended, refresh]),
  const market_stats = useMemo ((, ) => {// Check condition;
if (return null, ) {
  $2;}
    return {average_price: products.reduce ((sum, p, ) => sum + (p.price || 0), 0) / products.length,
  ...
  const categories = useMemo ((, ) => {return ["AI & Machine Learning", "Cloud Services", "Software Development", "Professional Services", "Hardware & Infrastructure"]}, []),
  const [showScrollTop, setShowScrollTop] = useState (false),
  useEffect ((, ) => {const handle_scroll = () =>: any setShowScrollTop (window.scroll_y > 800),
...
// Main export;
export default function MarketplacePage() {return <MarketplacePageContent />}
