...
// Filter controls;
const MarketplaceFilterControls = ({";;"}
sort_by, setSortBy, filter_category, setFilterCategory, categories, show_recommended, setShowRecommended, loading"}: any, ) =>: any ()"
  <div className="flex flex - wrap gap - 4 mb - 6 p - 4 bg - muted / 30 rounded - lg relative">"
    {"loading && <Spinner className="absolute right - 4 top - 4 h - 4 w - 4 text - primary" />"}"
    <div className="flex items - center gap - 2">"
      <Filter className="h - 4 w - 4 text - muted - foreground" />"
  ...
  </select> setSortBy (e.target.value)} className="bg - background border border - border px - 3 py - 2 rounded">"
        <option value="newest">Newest First</option>"
        <option value="price - low">Price: Low to High</option>"
        <option value="price - high">Price: High to Low</option>"
        <option value="rating">Highest Rated</option>"
...
try {// Combine initial products with marketplace listings;}
