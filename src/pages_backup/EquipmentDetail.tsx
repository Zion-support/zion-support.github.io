...
if (storedData.name) {// Already in EquipmentDetails format;}
equipmentData = storedData} else {;}
...
name: string;,
value: string"}interface EquipmentDetails {;,"}
id: string;,
name: string;,
description: string;,
brand: string;,
...
const storedData = JSON && JSON.parse(stored);
              // Check if it's already in EquipmentDetails format or needs conversion;
let equipmentData: EquipmentDetails;
if (storedData && storedData.name) {// Already in EquipmentDetails format;}
...
}</div> </motion.div> </div> </div> </div> </React.Fragment>) 
}'"}"
</$1></$1></$1></$1></$1>
    </React.Fragment>
  )
...
</div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></p></p>