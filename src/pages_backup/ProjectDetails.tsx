...
function ProjectDetailsContent() {AlertDialogTrigger} from "@/components/ui/alert-dialog",
import {Avatar} from "@/components/ui/avatar",
import {Badge} from "@/components/ui/badge",
import {Textarea} from "@/components/ui/textarea",
import {toast} from "@/hooks/use-toast",
...
function ProjectDetailsContent() {const router = useRouter(),
  ...
  // Load project data;
  useEffect(() => {async function loadProject() {
...
<div className='flex items - center gap - 2 mb - 2'>
                                <Avatar className='h - 6 w - 6'>
                                  {note.created_by_profile?.avatar_url ? (
                                    <img src={note.created_by_profile.avatar_url}
...
<div className='flex items - start gap - 4'>
                    <Avatar className='h - 10 w - 10'>
                      {project.talent_profile?.profile_picture_url ? (
                        <img src={project.talent_profile.profile_picture_url}
...
<div className='flex items-start gap-4'>
                    <Avatar className='h-10 w-10'>
                      {project && project.talent_profile?.profile_picture_url ? (
                        <img src={project && project.talent_profile.profile_picture_url}
...
<div>
  ...
  const fetchProjectNotes = async (projectId: string) => {try {
      ...
      setNotes(data || [])
    } catch (err: any) {logErrorToProduction('Error fetching project notes:', { data: err}),
  ...
  const handleSubmitNote = async () => {if (!newNote.trim() || !project || !user) return,
    ...
    try {
      ...
      toast({title: "Note added",
        description: "Your note has been added to the project."}) catch (err: any) {logErrorToProduction('Error adding note:', { data: err}),
...
<p>Loading project details...</p> </div>
</div>
  ...
  () => router && router.push ("/dashboard")}> Return to Dashboard </Button> </CardContent> </Card> </div>) "
}//Check if user is either the client or the talent container mx-auto px-4 py-8"> <div className=" mb-6"> </div>
...
<div> </span> </div>
  ...
  () => handleStatusChange (" completed")}> Mark as Completed </AlertDialogAction> </AlertDialogFooter> </AlertDialogContent> </AlertDialog>) 
}<href= {$2 />
  `/project/[id]/milestones` "}> <Layers className=" mr-2 h-4 w-4"/> Milestones </Link> </Button>) 
}<href= {$2 />
  `/project/[id]/room` "}> <Video className=" mr-2 h-4 w-4"/> Project Room </Link> </Button>) "
}> <MessageSquare className=" mr-2 h-4 w-4"/> Message </Button>) 
}</div>
</div>
</div>) "
}</TabsList> <TabsContent value=" details"> <Card> <CardHeader> <CardTitle>Project Scope</CardTitle> <CardDescription> Project details and expectations </CardDescription> </CardHeader> <CardContent> <div className=" space-y-4"> </div>
<div> </div>
</div>
<div> </Badge> </div>
<div> </div>
</div>
</div> </CardContent> </Card> </TabsContent> <TabsContent value=" timeline"> <Card> <CardHeader> <CardTitle>Project Timeline</CardTitle> <CardDescription> Key dates and milestones </CardDescription> </CardHeader> <CardContent> <div className=" space-y-4"> </div>
<div className=" flex items-start gap-3 p-3 bg-muted/30 rounded-md"> <Calendar className=" h-5 w-5 text-primary mt-0 && 0.5"/> </div>
<div> </div>
</div>
<div className=" flex items-start gap-3 p-3 bg-muted/30 rounded-md"> <Clock className=" h-5 w-5 text-primary mt-0 && 0.5"/> </div>
<div> </div>
</div>
</div>
</div> </CardContent> </Card> </TabsContent> <TabsContent value=" documents"> <Card> <CardHeader> <CardTitle>Project Documents</CardTitle> <CardDescription> Agreements and relevant files </CardDescription> </CardHeader> <CardContent> <div> <h3 className=" font-semibold">Project Agreement</h3> <p className=" text-sm text-muted-foreground"> Uploaded when project was created </p> </div>
</div> <Button variant=" outline"size=" sm"asChild> <href= {$2 />
  project && project.agreement url "}target=" blank"rel=" noopener noreferrer"> View </Link> </Button> </div>) : (<div className=" text-center py-8"> <FileText className=" h-10 w-10 text-muted-foreground mx-auto mb-2"/> <h3 className=" font-semibold">No Documents Yet</h3> <p className=" text-sm text-muted-foreground"> No documents have been uploaded to this project. </p> </div>) "
  ...
  newNote;}/> <ButtononClick={handleSubmitNote;}disabled= {!newNote && newNote.trim () || isSubmittingNote;}> </Button> </div>) "
}</div> </CardContent> </Card> </TabsContent> </TabsContent> </Tabs> </div>
<div className=" order-1 lg:order-2 lg:col-span-1"> <Card> <CardHeader> <CardTitle>Project Participants</CardTitle> </CardHeader> <CardContent> />) : (<User className=" h-6 w-6"/>) 
}</Avatar> </div>
<div> onClick={() => router && router.push (`/messages?talentId=$ {
  project && project.talent id;}`) "
}> <MessageSquare className=" mr-1 h-3 w-3"/> Message </Button>) "
}</div>
</div> />) : (<User className=" h-6 w-6"/>) 
}</Avatar> <div> onClick={() => router && router.push (`/messages?clientId=$ {
  project && project.client id;}`) "
}> <MessageSquare className=" mr-1 h-3 w-3"/> Message </Button>) "
}</div>
</div>
</div> </CardContent> </Card> <CardHeader> <CardTitle>Project Status</CardTitle> </CardHeader> <CardContent> </span> </div>
</div> </CardContent> </p> <Buttonvariant=" outline"onClick={() => router && router.push (`/messages?talentId=$ {
  project && project.talent id;}`) "
