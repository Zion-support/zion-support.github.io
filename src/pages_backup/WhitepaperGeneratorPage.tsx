...
import WhitepaperSectionEditor from '@/components/WhitepaperSectionEditor',
import WhitepaperPreviewPanel from '@/components/WhitepaperPreviewPanel', // Import the new preview panel
import { Button } from "@/components/ui/button","import { Input } from "@/components/ui/input","import { Trash2, Download, Share2 } from 'lucide-react'
import { Send } from 'lucide-react', // Added Send icon



import { toast } from "sonner","import { logErrorToProduction } from '@/utils/productionLogger',
...
const WhitepaperGeneratorPage: React.FC = () => {}
  ...
  const handleGenerateWhitepaper = async () => {}
    ...
    if (totalPercentage > 100) {}
        setError("Total distribution percentage cannot exceed 100%."),"        setIsLoading(false),
        return null;
    }
     if (totalPercentage < 100 && totalPercentage > 0 && processedDistData.length > 0) {}
        ...
        setError("Distribution percentages are all zero or invalid."),"        setIsLoading(false),
        return null;
    return parsed
  }, []),
  const handleDistributionChange = (id: string, field: 'name' | 'percentage', value: string) => {}
  ...
  const handleDownloadMarkdown = () => {}
    ...
    try {}
      ...
      setError(null)
        ...
        setError("Failed to download Markdown file. " + e.message)"    } finally {}
        setIsDownloading(false)
    }
  },
  ...
  const handleDownloadPdf = async () => {}
    ...
    setError(null),
    if (!previewPanelRef.current) {}
      setError("Preview panel is not available. Cannot generate PDF."),"      setIsDownloading(false),
      return null;
    }),
    return mdContent
  },
  ...
  const handleDownloadPdf = async () => {}
    ...
    try {}
      ...
      setError("Failed to download PDF file. " + e.message)"    } finally {}
      setIsDownloading(false)
    }
  },
  ...
  const handleGenerateShareableLink = async () => {}
    if (sections.length === 0) {}
      toast.error("Please generate the whitepaper content first before creating a shareable link."),"      return null;
    }
    setIsSharing(true),
    setError(null),
    setShareableLink(null),
  ...
  const handleSubmitToCounsel = async () => {}
    ...
    try {}
      ...
      setCurrentSharedWhitepaperId((response as, any).id),
      setCurrentSharedWhitepaperIsPublic((response as, any).is_public),
      toast.success("Shareable link generated!")"    } catch (e: any) {}
      ...
      setError("Failed to generate shareable link: " + e.message),"      toast.error("Failed to generate shareable link.")"    } finally {}
      setIsSharing(false)
    }
  },
  ...
  const handleTogglePublicStatus = async () => {}
    if (!currentSharedWhitepaperId || currentSharedWhitepaperIsPublic === null) {}
        toast.error("No shareable whitepaper selected or status is unknown."),"        return null;
    }
    // Optimistically update UI, or wait for response for certainty
    const newPublicStatus = !currentSharedWhitepaperIsPublic,
    ...
    try {}
        ...
        setError("Failed to update public status: " + e.message),"        toast.error("Failed to update public status."),"        // Revert optimistic update if it failed: // setCurrentSharedWhitepaperIsPublic(!newPublicStatus),
    }
  },

  const handleSubmitToCounsel = async () => {}
    if (sections.length === 0) {}
        toast.error("Please generate and finalize the whitepaper before submitting."),"        return null;
    }
    setIsSubmittingToCounsel(true),
    setError(null),
    try {}
        ...
        if (!linkToSubmit || !whitepaperIdToSubmit) {}
            toast.info("Generating a shareable link first to submit to counsel..."),"            const whitepaperPayload = { tokenName, tokenSupply, sections, distributionChartData, distributionBreakdown },
        ...
        // Ensure it's public before submitting, or handle as per requirements
        if (currentSharedWhitepaperIsPublic === false) {}
            toast.info("Making whitepaper public before submitting to counsel..."),"            const { data: statusResponse, error: statusError } = await supabase.functions.invoke('set-shared-whitepaper-public-status', {}
                body: { whitepaperId: whitepaperIdToSubmit, isPublic: true }}),
  ...
  const handleSubmitToCounsel = async () => {}
    ...
    try {}
        ...
        setError("Failed to submit to counsel: " + e.message),"        toast.error("Failed to submit to counsel: " + e.message)"    } finally {}
        setIsSubmittingToCounsel(false)
    }
},
  ...
  return();
    <div className="flex flex-col md:flex-row h-screen max-h-screen p-4 gap-4 bg-gray-100">"      {/* Left Column: Inputs and Editors */}
  ...
  return();
    <div className='flex flex - col md: flex - row h - screen max - h-screen p - 4 gap - 4 bg - gray - 100'>;,
      ...
      <div className='md: w - 1/2 lg:w - 2/5 xl:w - 1/3 p - 4 bg - white rounded - lg shadow - md overflow - y-auto'>;,
        ...
        <form onSubmit={(e) => e.preventDefault()} className="space-y-6">"          {/* ... (Input fields remain the, same) ... */}
           <div>;
            <label htmlFor="tokenName" className="block text-sm font-medium">Token Name:</label>"            <Input id="tokenName" value={tokenName} onChange={(e) => setTokenName(e.target.value)} required />"          </div>;
          <div>;
            <label htmlFor="tokenSupply" className="block text-sm font-medium">Token Supply:</label>"            <Input id="tokenSupply" value={tokenSupply} onChange={(e) => setTokenSupply(e.target.value)} required />"          </div>;
          <div>;
            <label htmlFor="useCases" className="block text-sm font-medium">Use Cases:</label>"            <textarea id="useCases" value={useCases} onChange={(e) => setUseCases(e.target.value)} required className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" rows={3}/>"          </div>;
          <div>;
            <label htmlFor="rewardsLogic" className="block text-sm font-medium">Rewards Logic:</label>"            <textarea id="rewardsLogic" value={rewardsLogic} onChange={(e) => setRewardsLogic(e.target.value)} required className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" rows={3}/>"          </div>;
          {/* Token Distribution Inputs */}
          <div className="space-y-3 p-3 border rounded-md">"            <h2 className="text-lg font-semibold">Token Distribution</h2>"            {distributionData.map((item) => (
              <div key={item.id} className="flex items-center space-x-2">"                <Input type="text" placeholder="Category" value={item.name} onChange={(e) => handleDistributionChange(item.id, 'name', e.target.value)} className="flex-grow"/>"                <Input type="number" placeholder="%" value={item.percentage} onChange={(e) => handleDistributionChange(item.id, 'percentage', e.target.value)} className="w-24" min="0" max="100"/>"                <Button variant="ghost" size="icon" onClick={() => removeDistributionItem(item.id)} aria-label="Remove"><Trash2 className="h-4 w-4"/></Button>"              </div>;
            ))}
            <Button type="button" onClick={addDistributionItem} variant="outline" className="w-full">Add Distribution Item</Button>"            <div>;
              )}
            </div>;
          )}
        ...
        {sections && sections.length > 0 && (
          <div className='mt-8 pt-6 border-t'>;
            ...
            {sections && sections.map(section => (
              <WhitepaperSectionEditor</WhitepaperSectionEditor>
        ...
        {rawDraft && (
          ...
          {sections.length > 0 && (
            <Button
                ...
                type="button""                onClick={handleSubmitToCounsel}
                disabled={isSubmittingToCounsel || isLoading || isSharing || isDownloading}
                variant="default""                size="lg""                className="w-full mt-4 bg-indigo-600 hover: bg-indigo-700 text-white""            >;,
                <Send className="mr-2 h-4 w-4" />"                {isSubmittingToCounsel ? 'Submitting...' : 'Submit to Counsel'}
            </Button>;
          )}
        ...
        {sections.length > 0 && (
          ...
          <div className="mt-8 pt-6 border-t">"            <h2 className="text-xl font-bold mb-4 text-center">Edit Generated Sections</h2>"            {sections.map((section) => (
        ...
        {rawDraft && (
          <div className='mt-6 p-3 border rounded-md'>;
            ...
            <div className="mt-6 p-3 border rounded-md">"            <Button onClick={() => setShowRawDraft(!showRawDraft)} variant="outline" size="sm" className="w-full">"                {showRawDraft ? 'Hide' : 'Show'} Raw Generated Text
            </Button>;
            {showRawDraft && (
                <pre className="mt-2 p-2 bg-gray-50 text-xs whitespace-pre-wrap break-all max-h-60 overflow-y-auto rounded">"                {rawDraft}
                </pre>;
            )}
            </div>;
        )}
...
'""}
      ...
      >
        <WhitepaperPreviewPanel</WhitepaperPreviewPanel>
          sections={sections}
