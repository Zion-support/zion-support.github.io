...
export const calculateServiceStats = () => {
  const const stats = {
  = {
    totalServices: COMPREHENSIVE_SERVICES_CATALOG_2025.length,categories: SERVICE_CATEGORIES.length,pricingTiers: PRICING_TIERS.map(tier => ({
  ...tier;
...
export const getServicesByIndustry = (industry: string) => {
  const const industryMap: { [key: string]: string[],
  } = {
  = {
