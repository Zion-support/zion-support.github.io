...
} from './innovativeMicroSaasServices2025';
import {
  EXPANDED_INNOVATIVE_SERVICES_2025,
  SPECIALIZED_INDUSTRY_SOLUTIONS_2025,
} from './expandedInnovativeServices2025';
import {
  EMERGING_TECH_SERVICES_2025,
  EMERGING_TECH_SOLUTIONS_2025,
...
export const calculateServiceStats = () => {
  const stats = {
    ...
    innovationLevels: INNOVATION_LEVELS.map(level => ({
      ...level,
      count: COMPREHENSIVE_SERVICES_CATALOG_2025.filter(
        service => service.innovationLevel.toLowerCase() === level.id
      ).length,
    })),
    supportLevels: SUPPORT_LEVELS.map(level => ({
      ...level,
      count: COMPREHENSIVE_SERVICES_CATALOG_2025.filter(
        service => service.supportLevel.toLowerCase() === level.id
      ).length,
    })),
  };
...
export const getServicesByPriceRange = (minPrice: number, maxPrice: number) => {
  return COMPREHENSIVE_SERVICES_CATALOG_2025.filter(
...
export const getServicesByInnovationLevel = (level: string) => {
  return COMPREHENSIVE_SERVICES_CATALOG_2025.filter(
    service => service.innovationLevel.toLowerCase() === level.toLowerCase()
  );
};
...
export const getServicesBySupportLevel = (level: string) => {
  return COMPREHENSIVE_SERVICES_CATALOG_2025.filter(
    service => service.supportLevel.toLowerCase() === level.toLowerCase()
  );
};
...
export const getFeaturedServices = (limit: number = 10) => {
  return COMPREHENSIVE_SERVICES_CATALOG_2025.sort((a, b) => {
    const aScore =
      parseInt(a.roi.replace(/\D/g, '')) +
      (a.innovationLevel === 'Cutting-edge' ? 100 : 0);
    const bScore =
      parseInt(b.roi.replace(/\D/g, '')) +
      (b.innovationLevel === 'Cutting-edge' ? 100 : 0);
...
export const getTrendingServices = (limit: number = 10) => {
  return COMPREHENSIVE_SERVICES_CATALOG_2025.filter(
    service => service.betaAccess || service.launchDate?.includes('2025')
  )
...
export const getServiceRecommendations = (preferences: {
  ...
  if (preferences.budget) {
    recommendations = recommendations.filter(
      service => service.price <= preferences.budget!
    );
  }
  ...
  if (preferences.innovationLevel) {
    recommendations = recommendations.filter(
      service =>
        service.innovationLevel.toLowerCase() ===
        preferences.innovationLevel!.toLowerCase()
    );
  }
  ...
  if (preferences.supportLevel) {
    recommendations = recommendations.filter(
      service =>
        service.supportLevel.toLowerCase() ===
        preferences.supportLevel!.toLowerCase()
    );
  }
