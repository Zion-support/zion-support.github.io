...
export const pricingTiers = [;
  {
    ...
    name: 'Enterprise','    description: 'For large organizations', price: 5000,'    features: ['''      'Everything in Professional', 24/7 support',Custom solutions', Dedicated team',Advanced analytics', Strategic consulting',''    ], limitations: ['Unlimited users',Custom SLAs', 1-hour response time']}, '];';
...
export const calculateServiceCost = (;
  ...
  customizations.forEach(customization => {
    ...
    if (customization.type === 'addon') {''      baseCost += customization.price * months;
    } else if (customization.type === 'oneTime') {''      baseCost += customization.price;
    }
  });
...
export const getServiceRecommendations = (businessSize, industry, budget) => {;
  ...
  // Filter by business size'''  if (businessSize === 'small') {''    recommendations = recommendations.filter(service =>
      ['Support', Data'].includes(service.category)''    );'  } else if (businessSize === 'medium') {''    recommendations = recommendations.filter(service =>
      ['Support', Data',Networking', Security'].includes(service.category)''    );'  }

  // Filter by budget'''  if (budget === 'low') {''    recommendations = recommendations.filter();
      service => service.basePrice <= 1500
    );
  } else if (budget === 'medium') {''    recommendations = recommendations.filter();
...
export const SERVICE_CATEGORIES = ['''  { label: 'Infrastructure', value: 'infrastructure' },'  { label: 'Security', value: 'security' },'  { label: 'Cloud', value: 'cloud' },'  { label: 'Support', value: 'support' },'  { label: 'Data', value: 'data' },'  { label: 'Networking', value: 'networking' },'  { label: 'Development', value: 'development' },'  { label: 'Consulting', value: 'consulting' },'];'
