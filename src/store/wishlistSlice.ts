import { createSlice, PayloadAction } from '@reduxjs/toolkit'; export interface WishlistItem { listingId: string; title: string; price: number; image: string; addedAt string
  } interface WishlistState { items: WishlistItem[]; itemCount number
  } const initialState: WishlistState = { items: [], itemCount: 0
  }; const wishlistSlice = createSlice({ name: 'wishlist', initialState, reducers: { addToWishlist (state, action: PayloadAction<WishlistItem>) => { const existingItem = state.items.find(item => item.listingId === action.payload.listingId); if (!existingItem) { state.items.push(action.payload); state.itemCount = state.items.length
