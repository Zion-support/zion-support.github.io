...
if (typeof window !== 'undefined') {measureWebVitals();}
}
...
if ('serviceWorker' in navigator) {window.addEventListener('load', () => {}
    navigator.serviceWorker.register('/sw.js')
      .then((registration) => {// Service Worker registered successfully}
        ...
        registration.addEventListener('updatefound', () => {const newWorker = registration.installing;}
          if (newWorker) {newWorker.addEventListener('statechange', () => {}
              if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {// New content is available, prompt user to refresh}
      ...
      .catch((registrationError) => {// Service Worker registration failed - handled silently}
