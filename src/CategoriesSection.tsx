...
const getIcon = (iconName?: string,) => {switch (iconName) {
...
const special_services = [{title: "IT Onsite Services",
...
export function CategoriesSection({showTitle = true;
  className;
  style;
  categories: fetchedCategories, // Rename prop for clarity;}: CategoriesSectionProps) {const { t} = useTranslation()
...
export function CategoriesSection({showTitle = true,
  ...
  const displayCategories = fetchedCategories && fetchedCategories.length > 0;
    ...
    : defaultCategories.map(cat => ({title: cat.name,
...
export function CategoriesSection(): any ({showTitle = true;
  ...
  // Use fetchedCategories if provided, otherwise fallback to defaultCategories;
  const displayCategories = fetchedCategories && fetchedCategories.length > 0;
    ? fetchedCategories && fetchedCategories.map(cat => ({id: cat && cat.id,
...
<div className="flex flex-wrap justify-center gap-4">
  ...
  switch (iconName) {}
const getSpecialServices = (t: any) => [ {title: t ('categories.it onsite services')
link: "/it-onsite-services"}]
interface CategoriesSectionProps {showTitle?: boolean;
className?: string;
style?: React.CSSProperties;
...
categories: fetchedCategories, //Rename prop for clarity;}: CategoriesSectionProps) {const {
  t;}= useTranslation ()
const defaultCategories = getDefaultCategories (t)
//Use fetchedCategories if provided, otherwise fallback to defaultCategories const displayCategories = fetchedCategories && fetchedCategories.length > 0 ? fetchedCategories.map (cat => ({t;}= useTranslation ()
const defaultCategories = getDefaultCategories (t)
//Use fetchedCategories if provided, otherwise fallback to defaultCategories const displayCategories = fetchedCategories && fetchedCategories.length > 0 ? fetchedCategories.map (cat => ({id: cat.id;"
...
icon: getIcon (cat.iconName), //Get icon component link: cat.link |`/category/$ {
  cat.id "}`, //Construct link if not provided //Assign a default color or implement logic to assign colors color: defaultCategories.find (dc => dc.id === cat.id) ?.color |"from-gray-500 to-gray-600"
}) : defaultCategories.map (cat => ({...cat;
title: cat.name;
icon: getIcon (cat.iconName)})
//If fetchedCategories is an empty array, and we want to show nothing: if (fetchedCategories && fetchedCategories.length === 0) {}> <div className="container mx-auto px-4" > <h2 className="text-3xl font-semibold text-white mb-4" > {'
  t ('home.no categories title') "}</h2> <p className="text-zion-slate-light text-lg mb-2" > {'
  t ('home.no categories desc') "}</p> <p className="text-zion-slate-light text-md" > {'
  t ('home.no categories support')}return (</p> </div>
)
}</div> </Link>) )
}</div> > {service.title;}</Link>) ) "
}</div>
</div>
<div className="mt-12 flex justify-center" > <Link href="/categories/all" className="text-zion-cyan border-b border-zion-cyan hover:border-zion-cyan-dark transition-colors" > {'
  t ('home.view all categories')}</Link> </div>
</div> </section>
)
}'"}
 ...
 */
function CategoriesSection() {const { t} = use_translation ()
  ...
  const display_categories = fetched_categories && fetched_categories.length > 0;
    ? fetched_categories.map (cat => ({id: cat.id,
        title: cat.name, // Map name to title;
        description: cat.description || "No description available.",
...
<div className="flex flex - wrap justify - center gap - 4">
  ...
  switch (icon_name) {}
const getSpecialServices = (t: any) =>: any [ {title: t ('categories.it onsite services')
link: "/it - onsite - services"}]
interface CategoriesSectionProps {show_title?: boolean;
class_name?: string;
style?: React.CSSProperties;
categories?: CategoryType[], //Accept categories as a prop;}export /**
 * CategoriesSection - Function description;
 */
function CategoriesSection() {const {
  t;}= use_translation ()
const default_categories = getDefaultCategories (t)
//Use fetched_categories if provided, otherwise fallback to default_categories const display_categories = fetched_categories && fetched_categories.length > 0 ? fetched_categories.map (cat => ({id: cat.id;"
title: cat.name,  //Map name to title description: cat.description || "No description available."
icon: get_icon (cat.icon_name), //Get icon component link: cat.link || `/category/$ {
  cat.id "}`, //Construct link if not provided //Assign a default color or implement logic to assign colors color: default_categories.find (dc => dc.id === cat.id) ?.color || "from - gray - 500 to - gray - 600"
}) : default_categories.map (cat => ({...cat;
title: cat.name;
icon: get_icon (cat.icon_name)})
