...
export async function categoryCleanupExample() {
  ...
  const tempCleanupStats = await dataCleanup.cleanupByCategory(
    "temporary-data",
    {
  ...
  const sessionCleanupStats = await dataCleanup.cleanupByCategory(
    "user-sessions",
    {
...
export async function dataExportImportExample() {
  ...
  const importedCount = dataRecordManager.importRecords(
    exportedData,
    "sessionStorage",
  );
