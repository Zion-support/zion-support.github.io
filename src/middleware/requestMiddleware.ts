...
export const loggingMiddleware: Middleware = async (context, next) => {
  ...
  try {
    const result = await next();    const duration = Date.now() - startTime;
...
export const authMiddleware: Middleware = async (context, next) => {
  ...
  if (token) {
    ...
    const duration = Date.now() - startTime;    logger.error('Request failed', error as Error, 'RequestMiddleware', {
      component: 'RequestMiddleware',
      method: context.request.method,
      url: context.request.url,
      duration
...
//             'RetryMiddleware',
  ...
  component: 'RetryMiddleware'),          await new Promise(resolve => setTimeout(resolve, delay * Math.pow(2, attempt)));
    throw lastError;
 * Timeout middleware;
