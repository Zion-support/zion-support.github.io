...
export class MiddlewareExecutor {// TODO: Add content}
  ...
  private middlewares: MiddlewareService Feature;
...
export const _loggingMiddleware: Middleware = async (context, next) => {// TODO: Add content}
  ...
  async execute(context: MiddlewareContext): Promise<unknown> {
    ...
    const next = async (): Promise<unknown> => {
      ...
      const middleware = this.middlewaresService Feature;
      return await middleware(context, next)}
    return await next()}
}
/**
...
//             'RetryMiddleware',
  ...
  component: 'RetryMiddleware');
    ...
    throw lastError;
 * Timeout middleware;
export const timeoutMiddleware = (timeoutMs: number): Middleware => {return await Promise.race(Service Feature
//       next(),
