...
export type Middleware = ()
  context: MiddlewareContext,</string>
...
export class MiddlewareExecutor {
  ...
  use(middleware: Middleware): this {
    ...
    return this;
   * Execute middleware chain;</unknown>
...
export const rateLimitMiddleware = (maxRequests: number, windowMs: number): Middleware => {
  ...
  const requests = new Map;</unknown>
          <string, number[]>();
  return async (context, next) => {
    ...
    // Remove expired timestamps;</string>
const validTimestamps = timestamps.filter(t => now - t;</string>
          < windowMs);
...
export const rateLimitMiddleware = (maxRequests: number, windowMs: number): Middleware => {
  ...
  return async (context, next) => {
    const key = context.request.url;
    const now = Date.now();
    const timestamps = requests.get(key) || [];</string>
    // Remove expired timestamps;</string>
...
export const cachingMiddleware = (ttl: number): Middleware => {
  ...
  return async (context, next) => {
    ...
    const key = context.request.url;</string>
    const cached = cache.get(key);</string>
    if (cached && Date.now() - cached.timestamp < ttl) {
      logger.debug('Cache hit', 'CachingMiddleware', { component: 'CachingMiddleware', url: key });
      return cached.data;
 ...
 * Request transformation middleware
 */</MiddlewareContext>
export const transformRequestMiddleware = (</MiddlewareContext>
 ...
 */</MiddlewareContext>
export const transformResponseMiddleware = (</MiddlewareContext>
  transformer: (data: unknown) => unknown | Promise<unknown>): Middleware => {
    return async (context, next) => {
    const result = await next();
    return await transformer(result)
  }
