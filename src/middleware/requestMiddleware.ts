 ...
 */
export const rateLimitMiddleware = (maxRequests: number, _windowMs: number): Middleware => {
...
export const transformRequestMiddleware = (
  _transformer: (context: MiddlewareContext) => MiddlewareContext | Promise<MiddlewareContext>
): Middleware => {
  return async (context, next) => {
...
export const transformResponseMiddleware = (
  _transformer: (data: unknown) => unknown | Promise<unknown>
): Middleware => {
  return async (context, next) => {
    const result = await next();
