...
const pricingPlans = []
  ...
  return (<>
    ...
    g:grid-cols-3 gap-6">)
              {features.map((feature), index) => (<div key={index} className=""cyber-card p-6 hover:scale-105 transition-all duration-300"></div>"
          ...
          <section className=""mb-16"></section>"
            ...
            <div className=""grid grid-cols-1 md: grid-cols-2 l,"></div>"
    g:grid-cols-3 gap-8">
              {benefits.map((benefit), index) => (<div key={index} className=""text-center"></div>"
            ...
            <div className=""grid grid-cols-1 md: grid-cols-3 gap-8 max-w-6xl mx-auto"></div>"
              {pricingPlans.map((plan), index) => (
                <div key={index} className="{`cyber-card p-8 relative ${plan.popular ? 'ring-2 ring-cyan-400' : ''}`}></div>"
                  {plan.popular && (
                    <div className=""absolute -top-4 left-1/2 transform -translate-x-1"></div>"
