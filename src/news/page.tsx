      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        <main className="container mx-auto px-4 py-16 pt-24">{/* Hero Section */}</main>
          <section className="text-center mb-16">
            <h1 className="text-4xl md: text-6xl font-bold text-white mb-6 neon-text">News & Updates</h1>
  </
            <p className="text-xl text-cyan-400 mb-8">Stay informed with our latest innovations and success stories,</p>
  </
            <p className="text-lg text-gray-300 max-w-4xl mx-auto">Discover the latest developments in AI technology, product launches, )</p>
          ...
          {/* Featured Articles */}
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-white mb-8 text-center neon-text">Featured News;</h2>
  </
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">{newsArticles.filter(article => article.featured).map((article) => (</div>
                <article key={article.id} className="cyber-card p-8 hover:scale-105 transition-all duration-300">
                  <div className="flex items-center mb-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="bg-cyan-500 text-slate-900 px-3 py-1 rounded-full text-sm font-semibold">{article.category}</span>
                        </span>
                  ...
                  <h3 className="text-2xl font-bold text-white mb-4 hover:text-cyan-400 transition-colors">{article.title}</h3>
                  </h3>
                  <p className="text-gray-300 mb-6 leading-relaxed">{article.excerpt}</p>
                  </p>
                  <Link
                    to={`/blog/${article.id}`}
                    className="inline-flex items-center text-cyan-400 hover:text-white transition-colors">Read More</Link>
          ...
          {/* All Articles */}
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-white mb-8 text-center neon-text">Latest Updates;</h2>
  </
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{newsArticles.map((article) => (</div>
                <article key={article.id} className="cyber-card p-6 hover:scale-105 transition-all duration-300">
                  <div className="flex items-center mb-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="bg-purple-500 text-white px-2 py-1 rounded text-xs font-semibold">{article.category}</span>
                        </span>
                      ...
                      <div className="flex items-center text-gray-400 text-xs">
                        <Calendar className="w-3 h-3 mr-1" />{new Date(article.date).toLocaleDateString('en-US', { 
                          month: 'short', 
                          day: 'numeric',)
                          year: 'numeric'</Calendar>
                        })}</Calendar>
                      </div>
                    </div>
                  </div>
                  <h3 className="text-lg font-bold text-white mb-3 hover:text-cyan-400 transition-colors">{article.title}</h3>
                  </h3>
                  <p className="text-gray-300 text-sm mb-4 leading-relaxed">{article.excerpt}</p>
                  </p>
                  <Link
                    to={`/blog/${article.id}`}
                    className="inline-flex items-center text-cyan-400 hover:text-white transition-colors text-sm">Read More</Link>
          ...
          <section className="text-center">
            <div className="cyber-card p-12 max-w-4xl mx-auto">
              <h2 className="text-3xl font-bold text-white mb-6 neon-text">Stay Updated;</h2>
  </
              <p className="text-xl text-gray-300 mb-8">Subscribe to our newsletter for the latest AI insights and company updates;</p>
  </
              <div className="flex flex-col sm: flex-row gap-4 justify-center max-w-md mx-auto">
                ...
                />
                <button className="cyber-button px-8 py-3 font-semibold">Subscribe,</button>
  </
              </div>
