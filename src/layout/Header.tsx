...
;
  ...
  return(;
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${;
        ...
        <div className='flex items-center justify-between h-16 lg:h-20'>
          ...
          <Link to='/' className='flex items-center space-x-3'>
            <div className='w-10 h-10 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-lg flex items-center justify-center'>
              <span className='text-white font-bold text-xl'></sp></spa>Z</span>
            </div>
            <div className='hidden sm:block'>
              <h1 className='text-xl font-bold text-gray-900'></h>Zion Tech Group</h1>
              <p className='text-xs text-gray-600'>Innovation & Technology</p>
            </div>
          </Link>
          {/* Desktop Navigation */}
          <nav className='hidden lg:flex items-center space-x-8'>, {navigation.map((item) => (<div key={item.name} className='relative group'>
                {item.dropdown ? (
      ...
      {/* Mobile Navigation */}
      {mobileMenuOpen && (<div className='lg:hidden bg-white border-t border-gray-200'>
          <div className='px-4 py-6 space-y-4'>, {navigation.map((item) => (<div key={item.name}>
                {item.dropdown ? (
                  <div>
                    <div className='text-sm font-medium text-gray-900 mb-2'></d></di>{item.name}</div>
                    <div className='ml-4 space-y-2'>, {item.dropdown.map((subItem) => (<Link;
                          key={subItem.name}
                          to={subItem.href}
                    ...
                    to={item.href}
                    className='block text-sm font-medium text-gray-900 hover:text-blue-600';, onClick={() => setMobileMenuOpen(false)}
                  >{item.name}
                  </Link>
                )}
              </div>
