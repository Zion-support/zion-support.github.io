...
import { Menu, X, Search, User, Bell, ChevronDown, Zap, Brain, Shield, Cloud, Rocket, Globe, Cpu, Lock, Heart, Users, ShoppingCart, BookOpen, MessageCircle, HelpCircle, DollarSign, Gauge, Star, Sparkles, Flame, Thunder, Lightning, Sun, Moon, Planet, Galaxy, Universe, Infinity, Alpha, Beta, Gamma, Delta, Epsilon, Zeta, Eta, Theta, Iota, Kappa, Lambda, Mu, Nu, Xi, Omicron, Pi, Rho, Sigma, Tau, Upsilon, Phi, Chi, Psi, Omega, Home } from 'lucide-react';
...
export function AppHeader() {
  ...
  const navigation = [
    ...
    { name: 'Services', href: '/services', current: false, icon: Rocket },
    { name: 'AI Services', href: '/ai-services', current: false, icon: Brain },
    { name: 'IT Services', href: '/it-services', current: false, icon: Cpu },
    { name: 'Micro SaaS', href: '/micro-saas', current: false, icon: ShoppingCart },
    { name: 'Pricing', href: '/pricing', current: false, icon: DollarSign },
    { name: 'Partners', href: '/partners', current: false, icon: Users },
    { name: 'Blog', href: '/blog', current: false, icon: BookOpen },
    { name: 'Help', href: '/help', current: false, icon: HelpCircle },
    { name: 'About', href: '/about', current: false, icon: Star },
    { name: 'Contact', href: '/contact', current: false, icon: MessageCircle },
    { name: 'Careers', href: '/careers', current: false, icon: Users },
  ];
  ...
  const services = [
    ...
    { name: 'Digital Twin', href: '/services/digital-twin', icon: Rocket, description: 'Simulation & Monitoring', category: 'Innovation' },
    ...
    { name: 'Mobile Surveys', href: '/services/mobile-feedback-surveys', icon: Users, description: 'NPS/CSAT with AI insights', category: 'Analytics' },
    ...
    { name: 'IT Helpdesk', href: '/services/it-helpdesk', icon: HelpCircle, description: 'Tickets, SLAs, portals', category: 'Support' },
    ...
    { name: 'AI SEO Optimizer', href: '/services/ai-seo', icon: Search, description: 'Semantic SEO & content briefs', category: 'SEO' },
    { name: 'Interview Assessment AI', href: '/services/interview-assessment', icon: Users, description: 'Structured interviews & scoring', category: 'HR' },
        ...
        <div className="container-responsive relative z-10">
          <div className="flex h-20 items-center justify-between">
              ...
              <Link to="/" className="flex-shrink-0 group">
                <div className="flex items-center space-x-3">
                  <div className="relative">
                    <div className="w-10 h-10 bg-gradient-to-br from-cyan-400 via-blue-500 to-purple-600 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300 animate-glow">
                      <Zap className="w-6 h-6 text-white" />
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-br from-cyan-400 via-blue-500 to-purple-600 rounded-lg blur-lg opacity-50 group-hover:opacity-75 transition-opacity duration-300"></div>
                  </div>
                  <h1 className="text-2xl font-bold bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 bg-clip-text text-transparent group-hover:scale-105 transition-transform duration-300 text-neon">
                    Zion Tech Group
                  </h1>
                </div>
              </Link>
                          ...
                          <div key={category} className="space-y-3">
                            <div className="flex items-center space-x-2 mb-3">
                              {categoryIcons[category as keyof typeof categoryIcons] && (
                                <div className="w-6 h-6 bg-gradient-to-br from-cyan-400/20 to-blue-500/20 rounded-lg flex items-center justify-center">
                                  <categoryIcons[category as keyof typeof categoryIcons] className="w-3 h-3 text-cyan-400" />
                                </div>
                              )}
                              <h3 className="text-cyan-400 font-semibold text-sm uppercase tracking-wide">{category}</h3>
                            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link
                to="/contact"
                className="hidden md:inline-flex items-center px-4 py-2 bg-gradient-to-r from-cyan-400 to-blue-500 text-white text-sm font-medium rounded-lg hover:from-cyan-500 hover:to-blue-600 transition-all duration-200 hover:scale-105 shadow-lg hover:shadow-cyan-400/25 btn-futuristic"
              >
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div 
              ...
              className="lg:hidden bg-slate-800/95 border-t border-slate-700/50 backdrop-blur-xl overflow-hidden"
            >
              <div className="container-responsive py-6">
                ...
                <nav className="space-y-4">
                  {navigation.map((item) => (
                    <Link
                      ...
                      className="flex items-center text-slate-300 hover:text-cyan-400 py-2 text-base font-medium transition-colors duration-200"
                    >
                ...
                {/* Mobile services by category */}
                <div className="mt-6 pt-6 border-t border-slate-700/50">
                  <h3 className="text-cyan-400 text-lg font-semibold mb-4">Services by Category</h3>
                  <div className="space-y-4">
                    {Object.entries(servicesByCategory).slice(0, 6).map(([category, categoryServices]) => (
                      <div key={category} className="space-y-2">
                        <div className="flex items-center space-x-2">
                          {categoryIcons[category as keyof typeof categoryIcons] && (
                            <div className="w-5 h-5 bg-gradient-to-br from-cyan-400/20 to-blue-500/20 rounded-lg flex items-center justify-center">
                              <categoryIcons[category as keyof typeof categoryIcons] className="w-3 h-3 text-cyan-400" />
                            </div>
                          )}
                          <h4 className="text-cyan-400 font-medium text-sm uppercase tracking-wide">{category}</h4>
                        </div>
                        <div className="ml-7 space-y-1">
                          {categoryServices.slice(0, 3).map((service) => (
                            <Link
                              ...
                              to={service.href}
                              onClick={() => setMobileMenuOpen(false)}
                              className="block text-slate-300 hover:text-cyan-400 text-sm transition-colors duration-200 py-1"
                            >
                              {service.name}
                            </Link>
                          ))}
                ...
                {/* Mobile quick links */}
                <div className="mt-6 pt-6 border-t border-slate-700/50">
                  <h3 className="text-cyan-400 text-lg font-semibold mb-4">Quick Links</h3>
                ...
                <div className="mt-6 pt-6 border-t border-slate-700/50">
                  <Link
                    to="/contact"
                    onClick={() => setMobileMenuOpen(false)}
                    className="block w-full text-center py-3 px-4 bg-gradient-to-r from-cyan-400 to-blue-500 text-white font-medium rounded-lg hover:from-cyan-500 hover:to-blue-600 transition-all duration-200 btn-futuristic"
                  >
