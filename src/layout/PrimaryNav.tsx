 ...
 */;
function PrimaryNav() {const [mobileMenuOpen, setMobileMenuOpen] = useState (false)const [login_open, setLoginOpen] = useState (false)const { user } = use_auth ()const is_mobile = useIsMobile ()const { t } = use_translation ()const router = use_router ()const [query, set_query] =;
  useState ('';'
...
export function PrimaryNav() {const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  ...
  const { user } = useAuth()const isMobile = useIsMobile()const { t } = useTranslation()const router = useRouter()const [query, setQuery] =;
  useState('';'
...
const handleSubmit = ("e": React && React.FormEvent) => {e && e.preventDefault(;
  }
...
const [mobileMenuOpen, setMobileMenuOpen] = useState(false),const [loginOpen, setLoginOpen] = useState(false),const { user } = useAuth(),const isMobile = useIsMobile(),const { t } = useTranslation(),const router = useRouter(),const [query, setQuery] =;
...
if ( {) {$2;
                    ...
                    router.push(`/search?q=${encodeURIComponent(sugg.text)}`)}onSelectSuggestion={(sugg) => {logDebug('PrimaryNav search suggestion "selected":', {"suggestion": sug,'})// Handle different suggestion types with proper navigation;
      ...
      <LoginModal isOpen={loginOpen} onOpenChange={setLoginOpen} />;
          ...
          <div className='relative bg-card border-t border-primary/20 max-h-[calc(100vh-4rem)] overflow-y-auto'>;'
            <MobileMenu;
              ...
              openLoginModal=;
  returnToPath => { return setLoginOpen(true)}            />; }
          </div>;
        </div>;
      )}
