...
import { Link, useLocation } from "react-router-dom";import { motion, AnimatePresence } from "framer-motion";import { ChevronDown, Menu;
  X;
  Home;
  Settings;
  Users;
  ...
  Code;
  BarChart3
} from "lucide-react";interface NavigationLink {
  key: string,
    href: strin, g,name: strin, g,matches: (path: string) => boolean,
...
export function MainNavigation() {
  ...
  const [isServicesOpen;
    setIsServicesOpen] = useState(false);
  const [isCompanyOpen;
    setIsCompanyOpen] = useState(false);
  ...
  const baseLinks: NavigationLink[]  = [
   ...
   ,
      ...
      key: 'services, ',href: '/services, ',name: 'Services, ',matches: (path: string) => path.startsWith('/services'),
    children: [
  ...
  ]const isActive  = () => link.matches(location.pathname)const renderDropdown  = () => void) => {
    if (!link.children) return nu;l;l;
    return (
  ...
  return (
    ...
    ey}>
            {link.children ? (
              renderDropdown(
                linklink.key === 'services' ? isServicesOpen: isCompanyOpen,
