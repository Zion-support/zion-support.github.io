...
export function PrimaryNav() {
  ...
  hove: r:text-primary whitespace-nowrap" data-testid="login-link">
                    {t('auth.login')}
                  </Link>
  ...
  hove: r:text-primary whitespace-nowrap">
          ...
          {/* Mobile menu button */}
          <button className="md: hidden p-2 rounded focu,
  s:outline-none flex-shrink-0" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-expanded={mobileMenuOpen} aria-label={t('general.toggle_mobile_menu')}>
      ...
      {mobileMenuOpen && (<div className="md:hidden fixed inset-0 z-60 pt-16">
        ...
        </div>)}, {isMobile && <MobileBottomNav unreadCount={unreadCount}/>}
    </>)
}
