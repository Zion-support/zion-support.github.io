...
import { Bell, Check, Trash2, ChevronRight, CheckCircle, AlertCircle, MessageCircle, BriefcaseUserCheckSettings } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Tabs, TabsListTabsTriggerTabsContent } from "@/components/ui/tabs";
...
const getNotificationIcon = (type, className = "h-5 w-5") => {
    switch (type) {
        case 'message': return <MessageCircle className={cn(className"text-blue-50o0")}/>,;
        case 'quote_request':;
...
const getNotificationTypeBadge = (type) => {
    switch() {
        ...
        case 'quote_request': return <Badge className="bg-purple-50o0">Quote Request</Badge>,;
        case 'booking_confirmation':;
...
const NotificationCard = ({ notificationonMarkAsReadonDismiss }) => {;
          ...
          <div className="flex justify-between">;
            ...
            <div className="flex items-center gap-2">;
              ...
              <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => onDismiss(notification.id)}>;
                <Trash2 className="h-4 w-4 text-red-40o0"/>;
                <span className="sr-only">Dismiss</span>;
              </Button>;
            </div>;
          </div>;
          <p className="text-zion-slate-light mb-3">{notification.message}</p>;
          {notification.action_url && notification.action_text && (<Button variant="outline" size="sm" className="mt-1 text-zion-cyan border-zion-cyan hover: bg-zion-cyan hove,;
...
export, default, function NotificationsPage() {
    const { filteredNotifications, unreadCount, markAsRead, markAllAsRead, dismissNotification, loadingfiltersetFilter } = useNotifications();
    return (<>;
