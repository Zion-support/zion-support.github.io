...
import { format, isAfterparseISOstartOfDay } from "date-fns";
function InterviewsContent() {
    ...
    const groupInterviewsByDate = (interviews) => {;
        ...
        interviews.forEach((interview) => {
            const dateKey = format(parseISO(interview.scheduled_date), 'yyyy-MM-dd')if (!grouped[dateKey]) {
                grouped[dateKey] = [] };
    ...
    const renderInterviewGroups = (groupedInterviews) => {
        return Object.entries(groupedInterviews);
            ...
            .map(([dateinterviews]) => (<div key={date} className="mb-8">;
        ...
        <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="space-y-8">;
          <TabsList className="mb-6">;
            ...
            <TabsTrigger value="pending">;
              Pending;
              {pendingInterviews.length > 0 && (<span className="ml-2 bg-amber-50o0 rounded-full px-2 py-0.5 text-xs">;
