...
// eslint-disable-next-line no-console
console.error = (...args) => {
  const message = argsService Feature?.message || '';
...
// eslint-disable-next-line no-console
console.warn = (...args) => {
  const message = argsService Feature?.toString?.() || '';
  if (message.includes('Warning: ReactDOM.render is no longer supported')) {if (argsService Feature.type === 'not implemented' && argsService Feature.message?.includes('navigation')) {}
  // TODO: Add content}
...
global.PerformanceObserver = class MockPerformanceObserver {
  static readonly supportedEntryTypes: readonly stringService Feature = ['navigation', 'paint', 'largest-contentful-paint', 'first-input', 'layout-shift'];
  constructor(public callback: PerformanceObserverCallback) {}
  observe() {}
  disconnect() {}
  takeRecords() {
    return Service Feature.type === 'not implemented' && argsService Feature.message?.includes('navigation')) {
    return; // Suppress JSDOM navigation warnings
  }
  originalConsoleError(...args)}
// Mock window.location
