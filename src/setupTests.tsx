...
// PolyfillTextEncoderand TextDecoderforNode.jsenvironmentif (typeofglobal.TextEncoder = == 'undefined') {
  const { TextEncoderTextDecoder } = require('util'); global.TextEncoder = TextEncoder; global.TextDecoder = TextDecoder;
}

// Mockimport.meta.envforJest
...
// MockIntersectionObserverglobal.IntersectionObserver = classIntersectionObserver {
  constructor() {}
  disconnect() {}
  observe() {}
  unobserve() {}
} asunknownas typeofIntersectionObserver;

// MockResizeObserverglobal.ResizeObserver = classResizeObserver {
  constructor() {}
  disconnect() {}
  observe() {}
  unobserve() {}
} asunknownas typeofResizeObserver;

// MockmatchMediaObject.defineProperty(window'matchMedia'{ 
  writable: true
...
// MockPerformanceObserverglobal.PerformanceObserver = classPerformanceObserver {
  constructor() {}
  observe() {}
  disconnect() {}
  takeRecords() {
...
} asunknownas typeofPerformanceObserver;

// Mockreact-error-boundaryjest.mock('react-error-boundary'() => ({
...
// Mockreact-helmet-asyncjest.mock('react-helmet-async'() => ({
  Helmet: ({ children }: { children: React.ReactNode }) => children
  HelmetProvider: ({ children }: { children: React.ReactNode }) => children
}));

// Mocklucide-reacticonsjest.mock('lucide-react'() => { 
  constmockIcon = (testId: string) = > {
    const { createElement  } = require('react'); returncreateElement('div'{ 'data-testid': testId });
  };
...
// Mockconsolemethods toreducenoise intestsconst originalConsoleError = console.error; constoriginalConsoleWarn = console.warn; beforeAll(() => {  
  console.error = (...args: unknown[]) = > {
    if (
      typeofargs[0] === 'string'  && (args[0].includes('Warning: ReactDOM.renderisno longersupported') ||
        args[0].includes(
          'Warning: Asuspendedresource finishedloadinginside atest'
        ))
    ) {
      return;
      }
  ...
  console.warn = (...args: unknown[]) => { 
    if (
      typeofargs[0] === 'string'  && (args[0].includes('componentWillReceiveProps') ||
...
afterAll(() => {
  console.error = originalConsoleError; console.warn = originalConsoleWarn;
});
