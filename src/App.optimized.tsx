...
import { EnhancedHeader } from
  './components/EnhancedHeader';
import { EnhancedFooter } from
  './components/EnhancedFooter';// Optimized lazy loading with preloading hints
...
const Home = createLazyComponent(() => import(
  './pages/Home'));
const About = createLazyComponent(() => import(
  './pages/About'));
const Contact = createLazyComponent(() => import(
  './pages/Contact'));
const Careers = createLazyComponent(() => import(
  './pages/Careers'));
const Services = createLazyComponent(() => import(
  './pages/Services'));
const Enterprise = createLazyComponent(() => import(
  './pages/Enterprise'));
const RequestQuote = createLazyComponent(() => import(
  './pages/RequestQuote'));const Solutions = createLazyComponent(() => import(
  './pages/Solutions'));
const News = createLazyComponent(() => import(
  './pages/News'));
const CaseStudies = createLazyComponent(() => import(
  './pages/CaseStudies'));
const Privacy = createLazyComponent(() => import(
  './pages/Privacy'));
const Terms = createLazyComponent(() => import(
  './pages/Terms'));

// Service pages
const AIServices = createLazyComponent(() => import(
  './pages/AIServices'));
const CloudServices = createLazyComponent(() => import(
  './pages/CloudServices'));
const CybersecurityServices = createLazyComponent(() => import(
  './pages/CybersecurityServices'));
const InfrastructureServices = createLazyComponent(() => import(
  './pages/InfrastructureServices'));
const TransformationServices = createLazyComponent(() => import(
  './pages/TransformationServices'));
const ConsultingServices = createLazyComponent(() => import(
  './pages/ConsultingServices'));// Error Fallback Component
...
<button onClick={() => window.location.href =
  '/} className="w-full bg-gray-700 hover: bg-gray-600 text-white px-4 py-2 rounded-lg transition-colors">Go home</button></div>'</div>
  </div>
)
...
export default App;
React, { Suspense, lazy } from
  'react';
  'react-router-dom';'import { HelmetProvider } from'
  'react-helmet-async';''
  '// Core Components;'
{ PerformanceOptimizer } from './components/PerformanceOptimizer'
  ...
  './components/ui/loading-spinner';''
  '// Layout Components;'
{ EnhancedHeader } from './components/EnhancedHeader'
  ';'
import { EnhancedFooter } from './components/EnhancedFooter'
  ';''// Optimized lazy loading with preloading hints'const createLazyComponent = (importFn: () => Promise<any>, fallback?: React.ReactNode) => {;const LazyComponent = lazy(importFn);
  return(props: any) => (
...
// Core pages with optimized imports
Home=createLazyComponent(() => import(
  './pages/Home'));

const About = createLazyComponent(() => import(
  './pages/About'));'Contact=createLazyComponent(() => import('
  './pages/Contact'));

const Careers = createLazyComponent(() => import(
  './pages/Careers'));'Services=createLazyComponent(() => import('
  './pages/Services'));

const Solutions = createLazyComponent(() => import(
  './pages/Solutions'));'News=createLazyComponent(() => import('
  './pages/News'));

const CaseStudies = createLazyComponent(() => import(
  './pages/CaseStudies'));'Privacy=createLazyComponent(() => import('
  './pages/Privacy'));

const Terms = createLazyComponent(() => import(
  './pages/Terms'));''
  '// Service pages'
AIServices=createLazyComponent(() => import('./pages/AIServices'
  '));'

const CloudServices = createLazyComponent(() => import('./pages/CloudServices'
  '));'CybersecurityServices=createLazyComponent(() => import(
  './pages/CybersecurityServices'));

const InfrastructureServices = createLazyComponent(() => import(
  './pages/InfrastructureServices'));'TransformationServices=createLazyComponent(() => import('
  './pages/TransformationServices'));

const ConsultingServices = createLazyComponent(() => import(
  './pages/ConsultingServices'));''
  '// Error Fallback Component'
const ErrorFallback = ({ error, resetErrorBoundary }: { error: Error; resetErrorBoundary: () => void }) => (
  <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 flex items-center justify-center p-4">"    <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 max-w-md w-full text-center border border-white/20">"      <div className="text-red-400 text-6xl mb-4">⚠️</div>"      <h1 className="text-2xl font-bold text-white mb-4">Something went wrong</h1>"      <p className="text-gray-300 mb-6">"        {error.message ||, "
  An unexpected error occurred. Please try again.
  '}'      </p>
  '      <div className="space-y-3">"        <button"          onClick={resetErrorBoundary}'
          className="w-full bg-blue-600 hover: bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors""        >"          Try again</button><button
          onClick={() => window.location.href = '/}'          className="w-full bg-gray-700 hover: bg-gray-600 text-white px-4 py-2 rounded-lg transition-colors""        >"          Go home</button>
      </div>
    </div>
  </div>
)
