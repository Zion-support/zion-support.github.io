...
export default function FAQ() {;
  ...
  const filteredFAQs = Object.entries(faqData).reduce((acc, [category, questions]) => {;
    if (openCategory === 'all' || openCategory === category) {
          ...
          <div className="flex flex-wrap justify-center gap-4">;
            ...
            {categories.map((category) => (;
              ...
              >;
                <[^>]*/>
                <span>{category.name}</[^>]*>
                <span className="text-xs opacity-75">({category.count})</[^>]*>
              </[^>]*>
            ))};
          </[^>]*>
        </[^>]*>
        ...
        <div className="container-responsive">;
          ...
          {Object.keys(filteredFAQs).length === 0 && (;&& (; (
            <div className="text-center py-20">;
              <[^>]*/>
              <h3 className="text-xl font-semibold text-white mb-2">No questions found</[^>]*>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">;
              {[;
                ...
                >;
                  ...
                  ;
                  <h3 className="text-xl font-bold text-white mb-3">{option.title}</[^>]*>
                  <p className="text-gray-300 text-sm mb-6">{option.description}</[^>]*>
                  ;
                  <a;
                    href={option.link};
                    className="[^"]*"
          ...
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-8">;
            {[;
              ...
              >;
                ...
                ;
                <h3 className="text-xl font-bold text-white mb-3">{resource.title}</[^>]*>
                <p className="text-gray-300 text-sm mb-6">{resource.description}</[^>]*>
                ;
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg overflow-hidden">;
      <button;
        ...
        className="[^"]*"
      >;
        <span className="text-lg font-semibold text-white pr-4">{question}</[^>]*>
        {isOpen ? (;
          <[^>]*/>
        ) : (;
          <[^>]*/>
      ...
      {isOpen && (;&& (; (
        <div className="px-6 pb-4">;
          <p className="text-gray-300 leading-relaxed">{answer}</[^>]*>
        </[^>]*>
      )};
    </[^>]*>
  );
