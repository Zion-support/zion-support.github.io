...
export const useSearch = (data, options) => {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            setDebouncedQuery(searchState.query)}, debounceMs);
        return () => clearTimeout(timer)}, [searchState.query, debounceMs]);
    ...
    const processedData = useMemo(() => {
        ...
        return results}, [data, debouncedQuery, searchState.filters, searchState.sortBy, searchState.sortOrder, searchFields, fuzzyMatch]);
    // Update search query
    ...
    const getSuggestions = useCallback((query, maxSuggestions = 5) => {
        ...
        return Array.from(suggestions).slice(0, maxSuggestions)}, [data, searchFields]);
    // Pagination helper
