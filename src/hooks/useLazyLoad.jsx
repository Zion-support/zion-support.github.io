<<<<<<< HEAD
import { useEffect, useRef, useState, useCallback } from &apos;react&apos;&apos;' export const useLazyLoad = (options = {}) => {} const;const { threshold = 0.1, rootMargin = &apos;50px&apos;, preload = true, preloadDistance = 100 } = options&apos; const [isVisible, setIsVisible] = useState(false) const [isLoaded, setIsLoaded] = useState(false) const ref = useRef(null) const load = useCallback(() => { if (!isLoaded) { setIsLoaded(true) setIsVisible(true)} }, [isLoaded]) useEffect(() => {} const;const;const element = ref.current if (!element) return ;const observer = new IntersectionObserver((entries) => { entries.forEach((entry) => { if (entry.isIntersecting) { load()} })}, { threshold, rootMargin: preload ? `${preloadDistance}px` : rootMargin }) observer.observe(element) return () => { observer.unobserve(element)}}, [threshold, rootMargin, preload, preloadDistance, load]) return { isVisible, ref, load }}';`&apos;'
=======
<<<<<<< HEAD
