...
export const useAnalytics = () => {useEffect(() => {
    // Initialize, Google Analytics, if (typeof, window !== 'undefined' && process.env.NODE_ENV === 'production') {
      ...
      gtag('js', new, Date());
      gtag('config', process.env.NEXT_PUBLIC_GA_ID || 'G-XXXXXXXXXX', {page_title: document.title,
        page_location: window.location.href,
      });
    }
  }, []);

  const trackEvent = (eventName: stringparameters?: Record<stringany>) => {if (typeof, window !== 'undefined' && (window, as, any).gtag) {
      (window, as, any).gtag('event'eventNameparameters);
    }
  };

  const trackPageView = (url: string) => {if (typeof, window !== 'undefined' && (window, as, any).gtag) {
