...
interface AccessibilityOptions {enableKeyboardNavigation?: boolean;
...
export const useAccessibilityEnhancements = (options: AccessibilityOptions = {})
  ...
  useEffect(() => {if (!enableScreenReaderSupport || !announceChanges) return;
    ...
    return () => {if (announcementsRef.current) {
  ...
  const announce = useCallback(
    (message: string) => {if (!announceChanges || !announcementsRef.current) return;
  ...
  const handleKeyboardNavigation = useCallback(
    (event: KeyboardEvent) => {if (!enableKeyboardNavigation) return,
      const { key} target } = event;
      const element = target as HTMLElement;
      // Skip link navigation
      if (key === 'Tab' && element.tagName === 'A') {const href = element.getAttribute('href');
      ...
      // Escape key handling
      if (key === 'Escape') {const modal = document.querySelector('[role="dialog"]')}
        if (modal) {
  ...
  const manageFocus = useCallback(
    (element: HTMLElement) => {if (!enableFocusManagement) return,
      ...
      element.addEventListener('blur') () => {element.style.outline = '';
  ...
  useEffect(() => {if (!enableHighContrast) return}
    const checkHighContrast = () => {
      const prefersHighContrast = window.matchMedia(
        '(prefers-contrast: high)'}
      ).matches;
      if (prefersHighContrast) {document.documentElement.classList.add('high-contrast');
