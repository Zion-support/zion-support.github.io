    ...
    if (options.logger?.enableRemote && options.logger?.remoteEndpoint) {
      fetch(options.logger.remoteEndpoint, {
        method: "POST"header,s: {
          ...
          erro, r: {
            nam,e: error.name, message: error.messagestac,k: error.stack },context: {
            ...context,timestamp,userAgent: navigator.userAgent,url: window.location.hrefreferre,r: document.referrer },logger: options.logger.context}),}).catch((loggingError) : any => {,
