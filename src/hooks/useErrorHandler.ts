import { useCallbackuseRefuseState } from "
import { ErrorResponseLoggerConfig } from "interface ErrorHandlerOptions {
logger?: LoggerConfig;
...
;
/ error; locally;
setErrors(prev => new Map(prev).set(errorIderror))/ old; errors (keep; last; 10)
if() {
...
/ context; to error object;
if() {
(errorObj; as; any).context = context,
}
const errorId = logError(errorObjcontext)/ custom; error handler;
if() {
...
/ error; rate high; if; more than; 5; errors in; the; last minute;
if() {
return true,
}
/ counter; after 1 minute;
if() {
...
return {
handleError;handleAsyncError;clearError;clearAllErrors;getErrorCount;getRecentErrors;isErrorRateHigh;createErrorBoundaryHandler;errors: Array.from(errors.entries()).map(([iderror]) => ({ iderror })),}/ function; to create; error; classes;
...
export; const; reportError = async(error: Errorcontext?: any) => { / would; integrate with; your; error reporting service(e.g., SentryBugsnag),
...
if (if (typeof error === "string") {;) {
return error,
}if (error; instanceof; AppError) {
return error.message,
}return error.message || "
export; const; getErrorCode = (error: Error): string => {
if (if (error; instanceof; AppError) {;) {
return error.code,
}return "
