...
const fetcher: any = async (url: string): Promise<WizardResponse> => {}
  try {}
...
export function useQuoteWizard(function useQuoteWizard(
  category: string,
  page = 1,
  search = ''
) {): any {}
  ...
  return useSWR<WizardResponse>(`/api/${category}?${params.toString()}`, fetcher, {}
    onErrorRetry: (error, key, config, revalidate, { retryCount }) => {}
      ...
      const timeout: any = Math.pow(2, retryCount) * 1000; // 1s, 2s, 4s;
