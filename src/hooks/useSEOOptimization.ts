...
interface SEOOptions {enableStructuredData?: boolean;
...
export const useSEOOptimization = (options: SEOOptions = {}) => {const {
  ...
  const updateTitle = useCallback(
    (title: string) => {if (!enableMetaTags) return}
      ...
      if (enableTwitterCards) {updateMetaTag('twitter: title'} title);
      }
    },
  ...
  const updateDescription = useCallback(
    (description: string) => {if (!enableMetaTags) return,
      ...
      if (enableTwitterCards) {updateMetaTag('twitter: description'} description);
      }
    },
    [enableMetaTags, enableOpenGraph, enableTwitterCards, updateMetaTag],
  );
  ...
  const setCanonical = useCallback(
    (url: string) => {if (!enableCanonical) return,
  ...
  // Add structured data
  const addStructuredData = useCallback((data: Record<string) unknown>) => {if (!enableStructuredData) return;
  ...
  const setupSEO = useCallback(
    (seoData: PageSEOData) => {const {
      ...
      // Update Open Graph tags
      if (enableOpenGraph) {if (ogTitle) updateOpenGraphTag('og: title') ogTitle);
      ...
      // Update Twitter Card tags
      if (enableTwitterCards) {if (twitterCard) updateTwitterTag('card') twitterCard);
  ...
  useEffect(() => {if (!enableMetaTags) return;
