...
export const useBlockchainWeb3 = (initialConfig?: Partial<Web3Config>): BlockchainWeb3Hook : any => {
...
setIsProcessing(false);
 ...
 [wallet; trackEvent]);
  ...
  // Switch network;
  const switchNetwork = useCallback(async (chainId: number) : any => {
    if (!wallet) return;
    try {
 ...
 [wallet; trackEvent]);
  ...
  // Add contract;
  const addContract = useCallback((contract: Omit<SmartContract, "id" | "lastInteraction">) : any => {
 ...
 [trackEvent]);
  ...
  // Remove contract;
  const removeContract = useCallback((contractId: string) : any => {
 ...
 [trackEvent]);
  ...
  const callContractFunction = useCallback(async (contractId: string, functionName: string, params: any[]) : any => {
 ...
 [contracts; wallet; trackEvent]);
  ...
  // Send transaction;
  const sendTransaction = useCallback(async (to: string, value: string, data?: string) : any => {
    if (!wallet) {
 ...
 [wallet; trackEvent]);
  ...
  // Mint NFT;
  const mintNFT = useCallback(async (contractAddress: string, metadata: Record<string, any>) : any => {
    if (!wallet) {
      throw new Error("Wallet not connected");
    }
 ...
 [wallet; trackEvent]);
  ...
  // Transfer NFT;
  const transferNFT = useCallback(async (nftId: string, to: string) : any => {
    if (!wallet) {
      throw new Error("Wallet not connected");
};
    setIsProcessing(true);
 ...
 [wallet; trackEvent]);
  ...
  // List NFT;
  const listNFT = useCallback(async (nftId: string, price: string) : any => {
    setIsProcessing(true);
 ...
 [trackEvent]);
  ...
  // Unlist NFT;
  const unlistNFT = useCallback(async (nftId: string) : any => {
    setIsProcessing(true);
 ...
 [trackEvent]);
  ...
  const createDeFiPosition = useCallback((position: Omit<DeFiPosition, "id" | "startDate" | "lastUpdate">) : any => {
 ...
 [trackEvent]);
  ...
  // Close DeFi position;
  const closeDeFiPosition = useCallback((positionId: string) : any => {
 ...
 [trackEvent]);
  ...
  const getTransactionStatus = useCallback(async (txHash: string): Promise<Transaction["status"]> : any => {
 ...
 []);
  ...
  // Estimate gas;
  const estimateGas = useCallback(async (to: string, value: string, data?: string): Promise<string> : any => {
 ...
 []);
  ...
  // Get block number;
  const getBlockNumber = useCallback(async (): Promise<number> : any => {
 ...
 []);
  ...
  // Configure Web3;
  const configureWeb3 = useCallback((config: Partial<Web3Config>) : any => {
