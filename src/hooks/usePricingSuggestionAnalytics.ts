...
interface PricingSuggestion {
  id: string, serviceName: string, suggestedPrice: number, marketAverage: number, confidence: number, timestamp: Date,
}

interface AnalyticsData {
  totalSuggestions: number, averageConfidence: number, priceAccuracy: number,
}

export function usePricingSuggestionAnalytics() {
  const [suggestions, setSuggestions] = useState<PricingSuggestion[]>([])
  const [analytics, setAnalytics] = useState<AnalyticsData>({
    totalSuggestions: 0, averageConfidence: 0,
    priceAccuracy: 0,
  })

  const addSuggestion = (suggestion: Omit<PricingSuggestion, "id" | "timestamp">) => {
    const newSuggestion: PricingSuggestion = {
      ...suggestion, id: Math.random().toString(36).substr(2, 9),
      timestamp: new Date(),
    }
  ...
  const updateAnalytics = (currentSuggestions: PricingSuggestion[]) => {
    ...
    setAnalytics({
      totalSuggestions,
      averageConfidence: Math.round(averageConfidence * 100) / 100, priceAccuracy: Math.round(priceAccuracy * 100) / 100,
    })
  }
