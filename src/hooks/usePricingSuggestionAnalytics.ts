import { useState, useEffect } from 'react';'import { supabase } from "@/integrations/supabase/client";""interface PricingSuggestionAnalytics {
  ...
  recentSuggestions: {;
    ...
    accepted: boolean;
    createdAt: string;
    type: 'client' | 'talent';'  }[];'  isLoading: boolean;
  error: string | null;
}
...
export function usePricingSuggestionAnalytics(days = 30) {
  const [analytics, setAnalytics] = useState<PricingSuggestionAnalytics>({
    ...
    averagePriceGap: 0, suggestionsByCategory: [], recentSuggestions: [], isLoading: true,;
    error: null
  });
