...
export function usePricingSuggestionAnalytics() {
  ...
  const addSuggestion = useCallback(
    (suggestion: Omit<PricingSuggestion, 'id' | 'timestamp'>) => {
  ...
  const getSuggestionsByService = useCallback(
    (service: string) => {
  ...
  const getAveragePrice = useCallback(
    (service: string) => {
      ...
      const total = serviceSuggestions.reduce(
  ...
  const getConfidenceScore = useCallback(
    (service: string) => {
      ...
      const totalConfidence = serviceSuggestions.reduce(
