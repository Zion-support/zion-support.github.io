...
export function useWallet(function useWallet() {): any {}
  const { user } = useAuth();
  ...
  const fetchWallet: any = useCallback(async () => { // Wrapped in useCallback;
    if (!user?.id) {}
      setWallet(null);
  ...
  const fetchTransactions: any = useCallback(async () => { // Wrapped in useCallback;
    if (!user?.id) {}
      setTransactions([]);
      return;
    }
