import { useEffect, useState, useCallback } from 'react'; // Added useCallback'import { useAuth } from '@/hooks/useAuth';'import { supabase } from '@/integrations/supabase/client';'import type { Wallet, TokenTransaction } from '@/types/tokens';''export function useWallet() {
  const { user } = useAuth();
  ...
  const fetchWallet = useCallback(async () => { // Wrapped in useCallback;
    ...
    try {
      ...
      if (supabaseError && supabaseError.code !== 'PGRST116') { // PGRST116: single row not found, not an error for new users'        throw supabaseError;'      }
      ...
      console.error('Error fetching wallet: ', err);'      setError(err.message);'      setWallet(null); // Ensure wallet is null on error
    } 
  ...
  const fetchTransactions = useCallback(async () => { // Wrapped in useCallback;
    ...
    try {
      const { data, error: supabaseError } = await supabase
        .from('token_transactions')'        .select('*')'        .eq('user_id', user.id)'        .order('created_at', { ascending: false });''      if (supabaseError) throw supabaseError;
      setTransactions((data || []) as TokenTransaction[]);
    } catch (err: any) {
      console.error('Error fetching transactions: ', err);'      // setError(err.message); // Decide if this should set a general error'      setTransactions([]); // Ensure transactions are empty on error
    }
    ...
    setTransactions(prev => [
      {
        ...
        amount, transaction_type: 'earn','        reason: reason || null, created_at: new Date().toISOString(), },'      ...prev,  ]);
  ...
  transaction_type: 'burn', // or 'spend''        reason: reason || null, created_at: new Date().toISOString(), },'      ...prev,  ]);
