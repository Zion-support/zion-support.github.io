import { useState, useEffect, useCallback } from 'react'; // Added useCallback'import { supabase } from '@/integrations/supabase/client';''export function useJobDetails(jobId: string | undefined) {
  ...
  const loadJobDetails = useCallback(async () => { // Wrapped in useCallback;
    ...
    try {
      ...
      console.error('Error loading job details: ', err);'      setError(err.message);'      setJob(null); // Clear job on error
    } finally {
      setIsLoading(false);
    }
  ...
  return {
    job, isLoading,
    error, loadJobDetails, // Return refetch function as loadJobDetails for clarity,
