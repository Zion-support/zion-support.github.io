...
//       'navigation'
    ...
    const paintEntries = performance.getEntriesByType('paint');
    const metrics: PerformanceMetrics = {const _paintEntries = performance.getEntriesByType('paint');}
    const _metrics: PerformanceMetrics = {// TODO: Add content;}
};
  loadTime: navigation;
//         ? navigation.loadEventEnd - navigation.loadEventStart;
        : 0,
...
// Measure CLS;
    const clsObserver = new PerformanceObserver(list => {// TODO: Add content;}
      ...
      for (const entry of list.getEntries()) {// TODO: Add content;}
        ...
        const layoutShiftEntry = entry as PerformanceEntry & {// TODO: Add content;}
}
          hadRecentInput?: boolean;
          value?: number;
        if (!layoutShiftEntry.hadRecentInput) {// TODO: Add content;}
}
          clsValue += layoutShiftEntry.value || 0;
      metrics.cumulativeLayoutShift = clsValue;
    clsObserver.observe({ entryTypes: ['layout-shift'] });
// Measure FID;
  ...
  const optimizeImages = useCallback(() => {// TODO: Add content;}
}
    const images = document.querySelectorAll('img[data-src]');
    const imageObserver = new IntersectionObserver(entries => {// TODO: Add content;}
      ...
      entries.forEach(entry => {// TODO: Add content;}
        ...
        if (entry.isIntersecting) {// TODO: Add content;}
          ...
          imageObserver.unobserve(img);
    images.forEach(img => imageObserver.observe(img));
  const preloadCriticalResources = useCallback(() => {// TODO: Add content;}
}
    const criticalResources = ['/fonts/inter-var.woff2', '/css/critical.css'];
    criticalResources.forEach(resource => {// TODO: Add content;}
      ...
      link.href = resource;
      link.as = resource.endsWith('.woff2') ? 'font' : 'style';
      if (resource.endsWith('.woff2')) {// TODO: Add content;}
}
        link.crossOrigin = 'anonymous';
      document.head.appendChild(link);
