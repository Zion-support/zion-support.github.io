...
export function useForm<T extends Record<string, unknown>>({
  ...
  const validateSingleField = useCallback(
    (field: keyof T): void => {
      ...
      const result = validateField(fieldValue, rules</div>);
      setErrors(prev => ({
        ...prev,
        [field]: result.errors
      })</boolean>);
  ...
  const validateAllFields = useCallback((): boolean => {
    ...
    setErrors(formErrors</div>);
    return isFormValid(validationResults</ValidationRule[]>);
  }, [values, validationSchema]</keyof>);
  // Handle input change
  const handleChange = useCallback(
    (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | <HTMLSelectElement>) => {
      ...
      if (validateOnChange && touched[fieldName]) {
        setTimeout(() => validateSingleField(fieldName), 0</HTMLSelectElement>);
  ...
  const handleBlur = useCallback(
      ...
      if (validateOnBlur) {
        validateSingleField(fieldName</HTMLInputElement>);
    [validateOnBlur, validateSingleField]
  // Handle form submission
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault(</HTMLFormElement>);
  ...
  }, [validateOnChange, touched, validateSingleField]</div>);
  ...
  const isValid = Object.keys(errors).length === 0 || 
    Object.values(errors).every(errorArray => errorArray.length === 0</keyof>);
  return {
    values,
    errors,
    touched,
