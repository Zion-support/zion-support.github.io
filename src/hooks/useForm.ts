...
handleSubmit: (e: React.FormEvent<HTMLFormElement>) => void;""
  ...
  resetForm: () => void;</>validateField: (field: keyof, T) => void;</>"
  validateAllField",s: "() => boolean;""</>,}</HTMLFormElement>;</>
...
let fieldValue: "unknown = value;"</>if (type === 'checkbox' && 'checked' in e.target) {",fieldValue = (e.target, as, HTMLInputElement).checked;,
      }
      setValues(prev = > ({
        ...prev,);
...
const handleBlur = useCallback()</HTMLInputElement>;""</>"
    (e: "ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {;</>"
      const fieldName = e.target.name as keyof T;</>setTouched(prev = > ({);"
        ...prev",[fieldName,]: true);
      }));
...
async (e: "React.FormEvent<HTMLFormElement>) => {;</>"
      e.preventDefault();</>// Mark all fields as touched;</>"
