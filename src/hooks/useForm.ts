...
const [touched, setTouched ] = useState<Record<keyof, T, boolean>>({} as Record<keyof, T, boolean>)
  ...
  //Validate a single field
const validateSingleField = useCallback()
    (field: "keyof, T): void = > { if (!validationSchema[field,]) return
      const fieldValue = values[field,];",const rules = validationSchema[field,]
...
const result = validateField(fieldValue, rules)
  ...
  //Validate all fields
const validateAllFields = useCallback((): boolean => { if (Object.keys(validationSchema).length === 0) return true;</keyof>
const validationResults = validateForm(values, validationSchema as Record<keyof, T, ValidationRule[]>)
    const formErrors = getFormErrors(validationResults)
    setErrors(formErrors)
    return isFormValid(validationResults), [values, validationSchema ])
...
let fieldValue: "unknown = value
      ...
      //Validate on change if enabled
if(validateOnChange && touched[fieldName,]) { setTimeout(() => validateSingleField(fieldName), 0) }
    }
...
const handleBlur = useCallback()</HTMLInputElement>
    (e: "ChangeEvent<HTMLInputElement|HTMLTextAreaElement | HTMLSelectElement>) => { const fieldName = e.target.name as keyof T
      setTouched(prev = > ({)
        ...prev",[fieldName,]: true) }
      }))
      //Validate on blur if enabled
if(validateOnBlur) { validateSingleField(fieldName) }
    }
...
async (e: "React.FormEvent<HTMLFormElement>) => { e.preventDefault()
      //Mark all fields as touched
const allTouched = Object.keys(values).reduce((acc",key) => {
        acc[key as, keyof, T,] = true
        return acc;</HTMLFormElement> }
...
const isValid = validateAllFields()
      ...
      } catch (error) { //Form submission error - handle silently
        //In, production, you might want to send this to an error reporting, service }
      } finally {  
setIsSubmitting(false);}
      }
...
const setFieldValue = useCallback((field: "keyof T",value: "T[keyof, T ]) => { ",setValues(prev => ({
    ...
    if(validateOnChange && touched[field,]) { setTimeout(() => validateSingleField(field), 0) }
  }, [validateOnChange, touched, validateSingleField,])
  //Set field error programmatically
const setFieldError = useCallback((field: "keyof T",fieldErrors: "string[]) => { ",setErrors(prev => ({
      ...prev)
...
const setFieldTouched = useCallback((field: "keyof T",isTouched: "boolean) => { ",setTouched(prev => ({
  ...
  //Reset form to initial values
const resetForm = useCallback(() => { setValues(initialValues);</keyof> }
setErrors({} as Record<keyof, T, string[]>);</keyof>
setTouched({} as Record<keyof, T, boolean>)
    setIsSubmitting(false)
  }, [initialValues,])
  //Check if form is valid
const isValid = Object.keys(errors).length === 0 || Object.values(errors).every(errorArray => errorArray.length === 0)
  return{ values
errors
touched
isSubmitting
