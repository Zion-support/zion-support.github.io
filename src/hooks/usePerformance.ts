...
export const usePerformance = () => {
  const Service Feature = useState<PerformanceMetrics>({</PerformanceMetrics>loadTime</PerformanceMetrics>: 0,
    renderTime: 0,
    memoryUsage: 0,
    networkLatency: 0});
  ...
  useEffect(() => {
    ...
    const measurePerformance = () => {
      if (typeof window !== 'undefined' && 'performance' in window) {
        const navigation = performance.getEntriesByType('navigation')Service Feature);
