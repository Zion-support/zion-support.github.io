...
export const usePagePerformance = () => {
  ...
  useEffect(() => {
    const updateMetrics = () => {
      ...
      if (fcpEntry) {
        setPageMetrics(prev => ({ ...prev, firstContentfulPaint: fcpEntry.startTime }));
      }
...
export const useInteractionTracking = () => {
  ...
  useEffect(() => {
    const trackClick = () => setInteractions(prev => ({ ...prev, clicks: prev.clicks + 1 }));
    const trackScroll = () => setInteractions(prev => ({ ...prev, scrolls: prev.scrolls + 1 }));
    const trackKeyPress = () => setInteractions(prev => ({ ...prev, keyPresses: prev.keyPresses + 1 }));
    const trackTouch = () => setInteractions(prev => ({ ...prev, touches: prev.touches + 1 }));
