      ...
      <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16'>
        ...
        <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-8'>
          ...
          <div className='space-y-4'>
            {services.map((service, index) => (<div key={index} className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                <div className='flex items-center'>
                  <service.icon className='h-6 w-6 text-blue-600 mr-3' />
                  <span className='text-lg font-medium text-gray-900'></sp></spa>{service.name}</span>
                </div>
                <div className='flex items-center'>, {getStatusIcon(service.status)}
                  <span className='ml-2 text-gray-600 capitalize'></sp></spa>{service.status}</span>
                </div>
              </div>
            ))}
          </div>
