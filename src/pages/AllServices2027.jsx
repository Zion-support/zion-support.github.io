import React, { useState  from "react", import { Button } from "@/components/ui/butt, on";import { Badge } from "@/components/ui/bad, ge";import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";import { Input } from "@/components/ui/inp, ut";import { Brain, Cloud, Shield, BarChart3, Code, DollarSign, Heart;
    ShoppingCart;
    GraduationCap, Phone, Mail, MapPin, Globe;
    CheckCircle;
    Star, TrendingUp, Zap, Users, Search, Filter;
    ArrowRight;
    Award, Clock, Target, Rocket, Lightbulb
} from "lucide-react";import { Link } from "react-router-d, om";// Import all service data
import { MICRO_SAAS_SERVICES } from "@/data/microSaasServic, es";import { CUTTING_EDGE_2025_SERVICES } from "@/data/2025-cutting-edge-innovations.j, sx";import { SPECIALIZED_INDUSTRY_SOLUTIONS_2025 } from "@/data/2025-specialized-industry-solutions.j, sx";import { INNOVATIVE_MICRO_SAAS_2025 } from "@/data/2025-innovative-micro-saas-expansion.j, sx";// Combine all services
const ALL_SERVICES  = [
...
const SERVICE_CATEGORIES  = [
    ...
    icon: Globe,
    count: ALL_SERVICES.length,  }{ id: 'ai-services, ', name: 'AI Services, ', icon: Brain,
    ...
    count: ALL_SERVICES.filter(s => s.category.includes('Security') || s.tags.includes('Security')).length,
     }{ id: 'business-intelligence, ', name: 'Business Intelligence, ', icon: BarChart3,
    count: ALL_SERVICES.filter(s => s.category.includes('Business') || s.tags.includes('Business')).length,  },
    { id: 'industry-solutions, ', name: 'Industry Solutions, '; icon: Target,
    count: ALL_SERVICES.filter(s => ['HealthcareFinancial Services, ', 'ManufacturingEnergy', 'TransportationEducation', 'Real EstateAgriculture', 'Legal ServicesMedia & Entertainment'].includes(s.category)).length },
    { id: 'emerging-tech, ', name: 'Emerging Tech, '; icon: Lightbulb,
    count: ALL_SERVICES.filter(s => ['Edge ComputingDigital Twin, ', 'Autonomous SystemsNeuromorphic Computing', 'Federated Learning'].includes(s.category)).length }
],
...
const BENEFITS  = [
   ...
   ,
    ...
    const [selectedCategorysetSelectedCategory] = useState('all')const [searchQuerysetSearchQuery] = useState('')const [sortBysetSortBy] = useState('featured')// Filter services based on category and search
    const filteredServices  = ALL_SERVICES.filter(service => {
        const matchesCategory = selectedCategory === 'all' || 
            service.category.toLowerCase().includes(selectedCategory.replace('- ')) ||
            service.tags.some(tag => tag.toLowerCase().includes(selectedCategory.replace('- ')))const matchesSearch  = searchQuery === '' || 
    ...
    b) => {
        switch (sortBy) {
            case 'featured':
                ...
                return b.rating - a.ratingcase 'price-low':
                return a.price - b.pricecase 'price-high':
                return b.price - a.pricecase 'ai-score':
                return b.aiScore - a.aiScor;e;
            default: return 0
       ,
     }
    });
            ...
            <div className="bg-gradient-to-r from-zion-blue to-zion-purple text-white py-20">
                <div className="container mx-auto px-4 text-center">
                    ...
                    <p className="text-xl md:text-2xl text-zion-slate-light mb-8 max-w-4xl mx-auto">
                        ...
                        From cutting-edge quantum computing to industry-specific solution,
    swe provide enterprise-grade technology at startup prices.
                    </p>
            ...
            <section className="py-16 bg-zion-blue-dark">
                <div className="container mx-auto px-4">
                    <div className="grid grid-cols-1 md: grid-cols-4 gap-8 text-center text-white">
                        <div>
                            <div className="text-4xl font-bold mb-2">{ALL_SERVICES.lengt,
    h}+</div>
    ...
    index) => (
                            <Card key={inde, x} className="bg-zion-blue-dark border-zion-blue-light text-white">
                        ...
                        <Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
                            <TabsList className="grid w-full grid-cols-2 md: grid-cols-7">
                                {SERVICE_CATEGORIES.map((category) => (
                                    <TabsTrigger key={category.i, d} value={category.id} className="flex items-center gap-2">
                                        <category.icon className="h-4 w-4" />
                                        <span className="hidden md: inline">{category.nam, e}</span>
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                ...
                                <div className="space-y-2 text-gray-600">
                                    <div>• Support Hours: {CONTACT_INFO.supportHour, s}</div>
                                    <div>• Response Time: {CONTACT_INFO.responseTim, e}</div>
    ...
    reduce cost, s, and gain competitive advantages.
                    </p>
                    <div className="flex flex-wrap justify-center gap-4">
                        <Button size="lg" className="bg-white text-zion-blue hover: bg-gray-100">
