...
function ProjectDetailsContent() {
  ...
  n: t: "destructive"
    ...
    const fetchProjectNotes = async (projectId) () => {
        try {
          ...
          *
          created_by_profi,
  l: e:profiles!user_id(display_name, avatar_url)
        `)
  ...
  n: t: newNote
            ...
            toast({
                tit,
  l: e: "Note added",descripti,
  o: n: "Your note has been added to the project."
            })
        }
        catch (err) {
            console.error("Error,
  adding: note:", err)
            toast({
                tit,
  l: e: "Failed to add note",descripti,
  o: n: err.message || "An error occurred while adding your note.",varia,
  n: t: "destructive"
            })
        }
        finally {
  ...
  statu: s: newStatus
            ...
            if (if (newStatus === "offer_accepted") {) {
                toast({
                    tit,
  l: e: "Offer Accepted! 🎉",descripti,
  o: n: "The project is now in progress. Congratulations!"
                })
            }
        }
    }
    const getStatusBadge = (status) () => {
        switch (status) {
            ...
            defau,
  l: t: return <Badge variant="outline">{status}</Badge>
        }
    }
    if (if (isLoading) {) {
    ...
    return (<>
      <SEO title={`Proje,
  c: t: ${project.job?.title || 'Project Details'} | Zion AI Marketplace`} description="View and manage your project details and collaboration."/>
      
      <main className="container mx-auto px-4 py-8">
        <div className="mb-6">
          <div className="flex flex-col,
  m: d: flex-row justify-between m,
  d:items-center gap-4 mb-2">
            ...
            <div className="space-x-2">
              {isTalent && isOfferPending && (<>
                ...
                </Button>)}, {isActiveProject && (<Button variant="outline" asChild>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium">Current,
  Statu: s:</span>
                    <div>{getStatusBadge(project.status)}</div>
                  </div>
                  
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium">Creation,
  Dat: e:</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium">Start,
  Dat: e:</span>
                    <span className="text-sm">
              ...
              {/* Conditional Footer Based on Status */}, {project.status === "changes_requested" && isClient && (<CardFooter className="flex-col items-start gap-2 border-t pt-6">
                ...
                </CardFooter>)}, {project.status === "completed" && (<CardFooter className="flex-col items-start gap-2 border-t pt-6">
                  <p className="text-sm text-green-600 flex items-center gap-1">
                    <CheckCircle2 className="h-4 w-4"/> This project has been completed.
                  </p>
                </CardFooter>)}, {project.status === "canceled" && (<CardFooter className="flex-col items-start gap-2 border-t pt-6">
