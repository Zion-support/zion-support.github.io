...
setNotifications(prev: > ({;
  ...
  delay: 0.1 + index * 0.1
              ...
              <div  className={`flex items - center gap-3 mb-3 ${achievement.earned ? 'text-yellow-400' : 'text-slate - 400'
              }`}>
                {achievement.icon}
                <h4 className="font -medium">{achievement.title}</h4>
              </div>
              <p className={`text-sm ${achievement.earned ? 'text-yellow-300' : 'text-slate - 400'
              }`}>
  ...
  confirm: !prev.confirm
          ...
          <button     onClick={handlePasswordChange}
            ...
            {isLoading ? (;
              <>
                <div  className="animate - spin rounded-full h-4 w-4 border-b-2 border-white"></div > Changing Password...
              </>) : (<>
  ...
  sessionTimeout: Number (e.target.value) }) ) }
      ...
      <div  className="bg-white / 5 border border-slate - 600 / 30 rounded-2xl p - 8 backdrop - blur -md">;
        ...
        <div  className="space - y-4">
          {security.loginHistory.map((login, index) => (<div  key={index} className="flex items - center justify - between p - 4 bg-white / 5 rounded-lg border border-slate -600 / 30">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font - medium ${login.status === 'success'
  ...
  y: 0
      ...
      <div  className="bg-white / 5 border border-slate - 600 / 30 rounded-2xl p - 8 backdrop - blur -md">
        ...
        <div  className="space - y-6">
          {Object.entries(notifications) .map(([key, value]) => (<div  key={key} className="flex items - center justify - between p - 4 bg-white / 5 rounded-lg border border-slate -600 / 30">
              <div>
