      ...
      <CardHeader className='pb-3'>
        <div className='flex items-start justify-between'>
          ...
          <div className='text-right'>
            ...
            <Badge
              variant={
                product.availability === 'Available' ? 'default' : 'outline'
              }
              className='text-xs'
            >
...
function MarketplacePageContent() {
  ...
  const fetchProducts = useCallback(
    async (page: number, limit: number) => {
      ...
      try {
        ...
        const fullDataset: ProductListing[] = [
        ...
        if (showRecommended) {
          processedDataset = processedDataset.filter(
            p => (p.rating || 0) >= 4.5 || (p.aiScore || 0) >= 85
          );
        }
  ...
  const marketStats = useMemo(() => {
    ...
    return {
      ...
      availableCount: products.filter(p => p.availability === 'Available')
        .length,
    };
  }, [products]);
        ...
        {!hasMore && products.length > 0 && (
          ...
          >
            ...
            <div className='text-sm text-muted-foreground'>
              Showing {products.length} marketplace items
            </div>
          </motion.div>
        )}
