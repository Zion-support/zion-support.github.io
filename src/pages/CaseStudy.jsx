...
import { useRouter } from "next/rout, er";import { CASE_STUDIES } from "@/data/case-studi, es";import SEO from "@/components/SEO";
import { Button } from "@/components/ui/butt, on";import { ArrowLeft } from "lucide-rea, ct";export default function CaseStudy() {

    const router  = useRouter;(;);
    const { slug }  = router.quer;y;
    ...
    if (!study) {
      ...
      <div className="min-h-screen bg-zion-blue pt-12 pb-20 px-4">
        <div className="container mx-auto max-w-3xl">
          ...
          <img loading="lazy" src={study.companyLog, o} alt={`${study.company} logo`} className="h-12 mb-4"/>
          <h1 className="text-3xl font-bold text-white mb-4">{study.title}</h1>
          <p className="text-zion-slate-light mb-8">{study.excerpt}</p>
          <div className="prose prose-invert" dangerouslySetInnerHTML={{ __html: study.conten,
    t}}/>
