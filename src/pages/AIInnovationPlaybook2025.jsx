import React from 'react';

const AIInnovationPlaybook2025 = () => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 py-16">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold text-gray-900 mb-6">AI Innovation Playbook 2025</h1>
          <p className="text-xl text-gray-700">
            Your comprehensive guide to AI innovation and implementation
          </p>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="bg-white rounded-xl shadow-lg p-8">
            <div className="text-4xl mb-4">📚</div>
            <h2 className="text-2xl font-bold mb-4">Strategy</h2>
            <p className="text-gray-600">
              Strategic planning for AI implementation and innovation.
            </p>
          </div>
          
          <div className="bg-white rounded-xl shadow-lg p-8">
            <div className="text-4xl mb-4">🛠️</div>
            <h2 className="text-2xl font-bold mb-4">Implementation</h2>
            <p className="text-gray-600">
              Step-by-step implementation guides and best practices.
            </p>
          </div>
          
          <div className="bg-white rounded-xl shadow-lg p-8">
            <div className="text-4xl mb-4">📊</div>
            <h2 className="text-2xl font-bold mb-4">Analytics</h2>
            <p className="text-gray-600">
              Measuring success and optimizing AI performance.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AIInnovationPlaybook2025;