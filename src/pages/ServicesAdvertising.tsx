    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">;
      ...
      <section className="pb-16">;
        <div className="container mx-auto px-4 max-w-7xl">;
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6 text-center">Spotlight: New Expert Services</[^>]*>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">;
            {spotlight.map((h) => (;
              <div key={h.title} className="p-6 rounded-2xl bg-black/40 border border-gray-700/60">;
                <h3 className="text-white font-semibold mb-2">{h.title}</[^>]*>
                <div className="text-sm text-slate-400 mb-2">{h.price}</[^>]*>
      ...
      <section className="pb-20">;
        <div className="container mx-auto px-4 max-w-5xl">;
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6 text-center">Contact Us</[^>]*>
          <div className="mx-auto max-w-2xl rounded-2xl bg-black/40 border border-white/10 p-6">;
            <ul className="text-slate-300 space-y-1">;
              <li><strong className="text-white">Mobile:</strong> <a className="text-cyan-400 underline" href="tel:+13024640950">{contact.mobile}</a></[^>]*>
              <li><strong className="text-white">E-mail:</strong> <a className="text-cyan-400 underline" href="mailto:kleber@ziontechgroup.com">{contact.email}</a></[^>]*>
              <li><strong className="text-white">Address:</strong> {contact.address}</[^>]*>
              <li><strong className="text-white">Website:</strong> <a className="text-cyan-400 underline" href={contact.website} target="_blank" rel="noopener noreferrer">ziontechgroup.com</a></[^>]*>
            </[^>]*>
            <div className="flex flex-wrap gap-3 pt-4">;
              <a href="/contact" className="px-6 py-3 bg-cyan-600 text-white rounded-lg inline-flex">Contact Sales</[^>]*>
              <a href="/services" className="px-6 py-3 border border-gray-600 text-gray-200 rounded-lg inline-flex">Browse Services</[^>]*>
            </[^>]*>
          </[^>]*>
        </[^>]*>
      </[^>]*>
