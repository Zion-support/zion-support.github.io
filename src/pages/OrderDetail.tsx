...
export default function OrderDetailPage() {
  ...
  const handleResend = async () => {
    if (!order || !user?.email) return,    try {
      await supabase.functions.invoke('send-email', _{
        body: {
          to: user.email, subject: `Receipt for order ${order.orderId}`,
          html: `<p>Thank you for your purchase. Total ${order.total}.</p>`
        }
      }),
      toast({ title: 'Receipt sent!' })
  ...
  const handleCopySummary = async () => {
    if (!order) return,
    const summary = [      `Order #${order.orderId}`,
      `Date: ${new Date(order.date).toLocaleDateString()}`,
      '',
      'Items:',
      ...order.items.map(_(i) => `${i.name} x${i.quantity} - $${i.price.toFixed(2)}`),
      '',
      `Total: $${order.total.toFixed(2)}`,
      '',
      'Shipping Address: ',
  ...
  return (
    <div className=&quot;container max-w-3xl py-10 space-y-6&quot;>
      ...
      <div className=&quot;flex gap-3&quot;>
        ...
        <Button variant=&quot;outline&quot; onClick={handleResend}>Resend Receipt</Button>      </div>
