      ...
      <section className="relative pt-28 pb-12 px-4 sm:px-6 lg:px-8">;
        <div className="max-w-6xl mx-auto">;
          <div className="text-center">;
            <h1 className="text-4xl md:text-5xl font-extrabold text-white">Modern Managed IT</[^>]*>
            <p className="mt-4 text-slate-300 max-w-3xl mx-auto">;
              Reliable, secure, and cost-efficient IT operations with clear SLAs and predictable pricing.;
            </[^>]*>
            <div className="mt-8 flex items-center justify-center gap-3">;
              <Link to="/contact" className="btn-futuristic">Request Quote</[^>]*>
              <Link to="/privacy" className="btn-neon">Security & Compliance</[^>]*>
            </[^>]*>
          </[^>]*>
            ...
            {offerings.map((o) => (;
              <div key={o.title} className="card-futuristic">;
                <div className="flex items-center gap-3">;
                  <[^>]*/>
                  <h3 className="text-white font-semibold">{o.title}</[^>]*>
                </[^>]*>
                <p className="mt-2 text-slate-300">{o.desc}</[^>]*>
              </[^>]*>
            ))};
          </[^>]*>
          ...
          <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-6">;
            {prices.map((p) => (;
              <div key={p.name} className="card-futuristic">;
                ...
                <div className="mt-2 text-3xl font-extrabold text-white">{p.price}</[^>]*>
                <ul className="mt-4 space-y-2 text-slate-300">;
                  {p.includes.map((i) => (;
                    <li key={i}>• {i}</[^>]*>
                  ))};
                </[^>]*>
                <Link to="/contact" className="mt-6 inline-flex btn-futuristic">Talk to Sales</[^>]*>
              </[^>]*>
            ))};
          </[^>]*>
        </[^>]*>
