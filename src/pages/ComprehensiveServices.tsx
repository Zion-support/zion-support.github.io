import { SEO } from "@/components/S, EO";import { motion } from "framer-moti, on";import { ArrowRight, Cloud as CloudIcon;
    Cpu;
    ExternalLink, Globe as GlobeIcon, Mail, MapPin, MessageSquare, Phone, Search, Sparkles, Star
} from "lucide-react";import React, { useState, useMemo  from "react";
import { Link } from "react-router-d, om";import { zionCuttingEdgeServices2029 } from "../../data/zion-2029-cutting-edge-servic, es";import { zionEmergingTechServices2029 } from "../../data/zion-2029-emerging-tech-servic, es";// Section component for displaying service categories
const Section: React.FC<, {;
  icon: React.ReactNode,
    title: strin, g,description: strin, g,items: any[, ],gradient: string,
}> = () => (
  <section className={`py-20 px-4 sm: px-6 lg:px-8 bg-gradient-to-r ${gradien, t}`}>
    <div className="max-w-7xl mx-auto">
      <div className="text-center mb-16">
        <div className="inline-flex items-center justify-center w-16 h-16 bg-cyan-500/20 rounded-full mb-6">
          {icon}
        </div>
        <h2 className="text-4xl md: text-5xl font-bold text-white mb-6">{tit,
    le}</h2>
...
const ServiceCard: React.FC<{ service: any,
    index: number,  }> = () => (
  <motion.div
    initial={{ opacity:  ,
  ...
  >
    <div className="flex items-center justify-between mb-4">
      <h3 className="text-xl font-semibold text-white">{service.name || service.titl,
    e}</h3>
      {service.isPopular && (
    ...
    3).map((feature: strin, g, idx: number) => (
          <div key={id,
    x} className="flex items-center text-sm text-gray-300">
  ...
  const [searchTermsetSearchTerm] = useState('')const [selectedCategorysetSelectedCategory] = useState<string>('all')const [sortBysetSortBy] = useState<string>('popular')// Combine all services
  const allServices  = [...zionCuttingEdgeServices202,9...zionEmergingTechServices2029]// Filter services based on search and category
  const filteredServices  = allServices.filter(service => {
    const matchesSearch = service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         service.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         service.tagline.toLowerCase().includes(searchTerm.toLowerCase())const matchesCategory  = selectedCategory === 'all' || service.category.toLowerCase() === selectedCategory.toLowerCase()return matchesSearch && matchesCategory}),
  ...
  const sortedServices  = [...filteredServices].sort((;a;
    ...
    switch (sortBy) {
      case 'price-low':
        ...
        return b.rating - a.ratingcase 'newest':
        return new Date(b.launchDate).getTime() - new Date(a.launchDate).getTime()default: return b.isPopular ? 1 : -,
    1
   }
  ...
  const microSaaS  = allServices.filter(service => service.category === 'Micro SaaS').slice(06)const itServices  = allServices.filter(service => service.category === 'IT Services').slice(06)const aiSolutions  = allServices.filter(service => service.category === 'AI Solutions').slice(06)return (
    <>
      <SEO
        title="Comprehensive Services - Zion Tech Group"
        description="Micro SaaSIT services, and AI solutions with clear pricing, links, and rapid delivery."
        keywords="micro saasit services, ai solutions, soc2 automation, kubernetes, rag platform"
    ...
    /}
      <Section
        ...
        description="Subscription software for securitylegal, revenue, and operations with fast time-to-value."
        items={microSaaS}
      ...
      <Section
        ...
        title="IT Services"
        description="Cloudplatform, and security engagements by seasoned architects and SREs."
        items={itServices}
      ...
      <Section
        ...
        title="AI Solutions"
        description="Applied AI with governanceprivacy, and business outcomes at the core."
        items={aiSolutions}
          ...
          <div className="rounded-2xl border border-white/10 p-8 bg-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center p-4 rounded-lg bg-black/40 border border-white/10">
                ...
                <span className="text-white">Delaware,
    USA</span>
              </div>
            </div>
          </div>
