            ...
            {pricing.map((((((((p, index, index, index, index, index, index) => ({ ...((((((p, index, index, index, index, index, key: index })) => ({ ...(((((p, index, index, index, index, key: index })) => ({ ...((((p, index, index, index, key: index })) => ({ ...(((p, index, index, key: index })) => ({ ...((p, index, key: index })) => ({ ...(p, key: index })) => (;
              <div key={p.name} className="rounded-lg p-4 border border-zion-slate-light/30">;
                ...
                <ul className="text-sm text-zion-slate-dark space-y-1">;
                  {p.details.map((((((((d, index, index, index, index, index, index) => ({ ...((((((d, index, index, index, index, index, key: index })) => ({ ...(((((d, index, index, index, index, key: index })) => ({ ...((((d, index, index, index, key: index })) => ({ ...(((d, index, index, key: index })) => ({ ...((d, index, key: index })) => ({ ...(d, key: index })) => (<li key={d}>{d}</li>))};
                </ul>;
              </div>;
            ))};
          </div>;
...
;
      <section className="pb-16 max-w-6xl mx-auto px-6">;
        <div className="grid md:grid-cols-3 gap-6">;
          {[{icon: Ticket, t: 'Tickets & SLAs'}, {icon: Cpu, t: 'Remote Support'}, {icon: Shield, t: 'Security & SSO'}].map((((((((c, index, index, index, index, index, index) => ({ ...((((((c, index, index, index, index, index, key: index })) => ({ ...(((((c, index, index, index, index, key: index })) => ({ ...((((c, index, index, index, key: index })) => ({ ...(((c, index, index, key: index })) => ({ ...((c, index, key: index })) => ({ ...(c, key: index })) => (;
