		...
		<div className = "min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">""
			<section className="max-w-7xl mx-auto px-6 py-20">""
				<div className="text-center mb-12">""
					...
					<h1 className="text-4xl md:text-5xl font-bold text-white">Email Sequencer</h1>"					<p className="mt-4 text-lg text-zion-slate-light max-w-2xl mx-auto">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{references.map((ref) => (""
							<a key={ref.href} href={ref.href} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition">""
								<LinkIcon className="w-4 h-4 text-zion-cyan mr-2"  />"								<span className="text-white text-sm">{ref.label}</span>;
							</a>;
						))};
					</div>;
				</div>;
