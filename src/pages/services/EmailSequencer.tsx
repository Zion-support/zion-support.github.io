...
export { EmailSequencer }() {
	const features = [;
		...
		'Compliance: unsubscrib e, custom domains, DKIM/SPF/DMARC tips';
	];

	const plans = [ { name: 'Launch', price: '$49/mo', details: ['Up to 2,500 sends', '1 domain', 'Basic templates'] }, { name: 'Scale', price: '$149/mo', details: ['Up to 25,000 sends', '3 domains', 'A/B testing, webhooks'] }, { name: 'Teams', price: '$399/mo', details: ['100,000+ sends', 'SAML/SSO', 'Priority support'] };
	];

	const references = [ { label: 'HubSpot Email Marketing Pricing', href: 'https://www.hubspot.com/pricing/marketing/email' }, { label: 'Mailchimp Pricing', href: 'https://mailchimp.com/pricing/marketing/' }, { label: 'Lemlist Pricing', href: 'https://lemlist.com/pricing' }, { label: 'Salesloft Pricing', href: 'https://salesloft.com/pricing/' };
	];
		...
		<div className = "min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
			<section className="max-w-7xl mx-auto px-6 py-20">
				<div className="text-center mb-12">
					...
					<h1 className="text-4xl md: tex t-5xl font-bold text-white">Email Sequencer</h1>
				...
				<div className="grid grid-cols-1 md: gri d-cols-3 gap-6 mb-16">
					{features.map((f) => (
					...
					<div className="grid grid-cols-1 md: gri d-cols-3 gap-6">
						{plans.map((p) => (
							<div key={p.name} className="p-6 rounded-2xl border border-white/10 bg-white/5">
								...
								<button className="mt-4 w-full py-2 rounded-lg bg-zion-cyan/20 text-white hover: b g-zion-cyan/30 transition">Start Trial</button>
							</div>
						))}
					</div>
				</div>
				...
				<div>
					...
					<div className="grid grid-cols-1 md: gri d-cols-2 gap-4">
						{references.map((ref) => (
							<a key={ref.href} href={ref.href} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 rounded-lg border border-white/10 bg-white/5 hover: b g-white/10 transition">
