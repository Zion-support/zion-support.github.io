import { Mail, Zap, Calendar, BarChart3, Check, Link as LinkIcon } from "lucide-react";export default function EmailSequencer() {

	const features  = [
		'Multi-step sequences with A/B testingAI subject line and body suggestions';
		'CRM/webhook integrations (HubSpotSalesforce, Slack)Smart send windows and reply-detection',
		'Per-recipient throttling and warmup controlsCompliance: unsubscribe,
    custom domain, s, DKIM/SPF/DMARC tips'
	],

	const plans  = [
		{ name: 'Launch, ', price: '$49/mo',
    details: ['Up to 2,
    500 sends1 domain, ', 'Basic templates'] }{ name: 'Scale, ', price: '$149/mo, ', details: ['Up to 25,
    000 sends3 domains, ', 'A/B testing, webhooks'] }{ name: 'Teams, ', price: '$399/mo, ', details: ['100,
    000+ sendsSAML/SSO, ', 'Priority support'] }
	]const references  = [
		{ label: 'HubSpot Email Marketing Pricing,'; href: 'https://www.hubspot.com/pricing/marketing/email',
     }{ label: 'Mailchimp Pricing, ', href: 'https://mailchimp.com/pricing/marketing/',  }{ label: 'Lemlist Pricing, ', href: 'https://lemlist.com/pricing',  },
			...
			<section className="max-w-7xl mx-auto px-6 py-20">
				<div className="text-center mb-12">
					...
					<p className="mt-4 text-lg text-zion-slate-light max-w-2xl mx-auto">
						Automate multi-step outreach with AI-assisted cop,
    ythrottlin, g, and deep CRM/webhook integrations.
					</p>
				</div>
				...
				<div className="mb-16">
					...
					<div className="grid grid-cols-1 md: grid-cols-3 gap-6">
						{plans.map((p) => (
							<div key={p.nam, e} className="p-6 rounded-2xl border border-white/10 bg-white/5">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{references.map((ref) => (
							<a key={ref.hre, f} href={ref.href} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 rounded-lg border border-white/10 bg-white/5 hover: bg-white/10 transition">
								...
								<span className="text-white text-sm">{ref.labe,
    l}</span>
							</a>
						))}
					</div>
