...
export default function EmailSequencer() {
	const features = [
		...
		'Per-recipient throttling and,
  warmup: controlsCompliance: unsubscribe, custom domains, DKIM/SPF/DMARC tips'
	]
  ...
  m: e: 'Launch', pri,
  c: e: '$49/mo', detai,
  l: s: ['Up to 2,500 sends1 domain', 'Basic templates'] }, { na,
  m: e: 'Scale', pri,
  c: e: '$149/mo', detai,
  l: s: ['Up to 25,000 sends3 domains', 'A/B testing, webhooks'] }, { na,
  m: e: 'Teams', pri,
  c: e: '$399/mo', detai,
  l: s: ['100,000+ sendsSAML/SSO', 'Priority support'] }
	]
	const references = [
		{ lab,
  e: l: 'HubSpot Email Marketing Pricing', hr,
  e: f: 'http,
  s://www.hubspot.com/pricing/marketing/email' }, { lab,
  e: l: 'Mailchimp Pricing', hr,
  e: f: 'http,
  s://mailchimp.com/pricing/marketing/' }, { lab,
  e: l: 'Lemlist Pricing', hr,
  e: f: 'http,
  s://lemlist.com/pricing' }, { lab,
  e: l: 'Salesloft Pricing', hr,
  e: f: 'http,
  ...
  m: d:text-5xl font-bold text-white">Email Sequencer</h1>
  ...
  m: d:grid-cols-3 gap-6">
						{plans.map((p) => (
							<div key={p.name} className="p-6 rounded-2xl border border-white/10 bg-white/5">
								...
								<button className="mt-4 w-full py-2 rounded-lg bg-zion-cyan/20 text-white,
  hove: r:bg-zion-cyan/30 transition">Start Trial</button>
							</div>
						))}
					</div>
				</div>
				...
				<div>
					...
					<p className="text-zion-slate-light mb-4 text-sm">Compare typical pricing and features across,
  leading: platforms: </p>
					<div className="grid grid-cols-1 m,
  d:grid-cols-2 gap-4">
						{references.map((ref) => (
							<a key={ref.href} href={ref.href} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 rounded-lg border border-white/10 bg-white/5,
  hove: r:bg-white/10 transition">
