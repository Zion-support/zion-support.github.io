...
export default function CustomDevelopment() {;
  ...
  const technologies = [;
    ...
    {;
      title: 'Backend',description: 'Robust backend technologies for scalable and secure applications.',icon: Cpu, //[^;]*
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">;
            {technologies.map((tech, index) => (;
              ...
              >;
                ...
                <h3 className="text-xl font-bold text-white mb-3">{tech.title}</[^>]*>
                <p className="text-zion-slate-light mb-4 leading-relaxed text-sm">{tech.description}</[^>]*>

                <div className="space-y-2">;
                  {tech.stack.map((item, itemIndex) => (;
                    <div key={itemIndex} className="text-zion-cyan text-sm font-medium">{item}</[^>]*>
                  ))};
                </[^>]*>
              </[^>]*>
            ))};
