    ...
    <div className="min-h-screen bg-white">;
      <section className="bg-gradient-to-r from-zion-cyan to-zion-purple py-20 text-center text-white">;
        <div className="max-w-5xl mx-auto px-6">;
          ...
          <h1 className="text-4xl md:text-6xl font-bold mb-4">Mobile Feedback Surveys</[^>]*>
          <p className="text-white/90 text-xl">Modern, fast surveys with AI insights to improve customer experience.</[^>]*>
        </[^>]*>
      </[^>]*>
      <section className="py-16 max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-10">;
        <div>;
          <h2 className="text-3xl font-bold text-zion-slate-dark mb-4">Features</[^>]*>
          ...
          <div className="grid md:grid-cols-3 gap-4">;
            {pricing.map((p) => (;
              <div key={p.name} className="rounded-lg p-4 border border-zion-slate-light/30">;
                ...
                <ul className="text-sm text-zion-slate-dark space-y-1">{p.details.map((d) => (<li key={d}>{d}</li>))}</[^>]*>
              </[^>]*>
            ))};
          </[^>]*>
          <div className="mt-6 flex gap-3">;
            <a href="/pricing" className="px-5 py-3 bg-zion-cyan/10 text-zion-slate-dark rounded-lg hover:bg-zion-cyan/20 inline-flex items-center">View Full Pricing <ArrowRight className="w-4 h-4 ml-2"/></[^>]*>
          </[^>]*>
        </[^>]*>
      </[^>]*>
