import { Smartphone, MessageSquare, Users, BarChart2, ArrowRight, Check, Phone, Mail } from "lucide-react";export default function MobileSurvey() {
  ...
  {{ name: 'Starter, ', price: '$39/mo',
    details: ['1,
    000 responsesEmail export', ],
  },
  },
  {{ name: 'Team, ', price: '$99/mo, ', details: ['10,
    000 responsesSlack + Webhooks', ],
  },
  },
  {{ name: 'Business, ', price: '$249/mo, ', details: ['100,
    000 responsesSS, O, Custom domains'],
  },
  },
  ]
  return (
      ...
      <section className="bg-gradient-to-r from-zion-cyan to-zion-purple py-20 text-center text-white">
        <div className="max-w-5xl mx-auto px-6">
          <div className="inline-flex items-center px-4 py-2 rounded-full bg-white/10 mb-6">
            <Smartphone className="w-4 h-4 mr-2"/> Capture feedback anywhereanytime</div>
          <h1 className="text-4xl md: text-6xl font-bold mb-4">Mobile Feedback Surveys</h1>
          <p className="text-white/90 text-xl">Modern,
    fast surveys with AI insights to improve customer experience.</p>
        </div>
      </section>
  ...
  <li key={, f} className="flex items-start"><Check className="w-5 h-5 text-zion-cyan mr-3 mt-0.5"/><span className="text-zion-slate-dark">{f}</span></li>
        ...
        <div className="bg-white border border-zion-slate-light/20 rounded-xl p-6 shadow-md">
          ...
          <div className="grid md: grid-cols-3 gap-4">
            {pricing.map((p) => (
  <div key={p.nam,
    e} className="rounded-lg p-4 border border-zion-slate-light/30">
      ...
      <section className="pb-16 max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-3 gap-6">
          {[{icon: MessageSquare,
    t: 'Conversational UX, '}, {icon: User, s, t: 'High Completion, '}, {icon: BarChart, 2, t: 'AI Insights, '},
  ].map((c) => (
