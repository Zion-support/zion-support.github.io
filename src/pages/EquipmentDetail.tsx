...
export const SAMPLEEQUIPMENT: { [key: string]: EquipmentDetails } =
...
export default function EquipmentDetail() {
  ...
  useEffect_(() => {async function loadEquipment() {
      if (!id) {
        setLoading(false),
...
const equipmentFromSample = SAMPLEEQUIPMENT[id],
  ...
  const inCart = items.some(item => item.id === equipment?.id),
    ...
    try {dispatch({
        type: 'ADDITEM', payload: {
          id: equipment.id, name: equipment.name, price: equipment.price, quantity}});

      toast({title: "Added to Cart", description: `${equipment.name} has been added to your cart.`})
    } catch (error) {toast({
        title: "Error", description: "Failed to add item to cart. Please try again.", variant: "destructive"})
            ...
            >
              ...
              <p className=&quot;text-zion-slate-light mb-8 max-w-md mx-auto&quot;>
                {error === 'Equipment not found' 
                  ...
                  : error || &quot;We couldn't load the equipment details. Please try again.&quot
                }              </p>
...
onClick={() => router.push('/equipment')}
                  className="bg-zion-cyan hover: bg-zion-cyan/90 text-zion-blue"                  onClick={_() => router.push('/equipment')}
              ...
              <div className=&quot;bg-zion-blue-light rounded-lg p-4&quot;>
                ...
                <div className=&quot;flex items-center gap-2 text-sm&quot;>
                  ...
                  <span className={equipment.inStock ? 'text-green-400' : 'text-yellow-400'}>
                    {equipment.expectedShipping}                  </span>
                </div>
              </div>
                  ...
                  <div className=&quot;grid gap-2&quot;>
                    {equipment.specifications.map((spec, index) => (
                      <div key={index} className=&quot;flex justify-between py-2 border-b border-zion-blue-light&quot;>
                        ...
                        <span className=&quot;text-white&quot;>{spec.value || 'Enterprise Grade'}</span>                      </div>
                    ))}
                  </div>
                </div>
              )}
