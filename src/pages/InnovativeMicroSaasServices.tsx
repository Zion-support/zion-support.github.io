...
const InnovativeMicroSaasServices: React.FC = () => {
  ...
  const formatPrice = (price: number | { monthly?: number; yearly?: number; oneTime?: number; currency: string; pricingModel: string }, currency?: string) => {
    if (typeof price === 'object') {
      const monthlyPrice = price.monthly || price.yearly ? (price.yearly || 0) / 12 : price.oneTime || 0;
      const priceCurrency = price.currency;
      if (monthlyPrice >= 1000) {
        return `${priceCurrency}${(monthlyPrice / 1000).toFixed(1)}k`;
      }
      return `${priceCurrency}${monthlyPrice}`;
    } else {
      const priceCurrency = currency || '$';
      if (price >= 1000) {
        return `${priceCurrency}${(price / 1000).toFixed(1)}k`;
      }
              ...
              >
                ...
                <div className="p-6">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-cyan-400">
                        {formatPrice(service.price, service.currency)}
                                              ...
                                              <div className="text-gray-400 text-sm">per {typeof service.price === 'object' ? service.price.pricingModel : service.pricingModel}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`px-2 py-1 text-xs text-white rounded ${getSupportLevelColor(service.supportLevel || 'standard')}`}>
