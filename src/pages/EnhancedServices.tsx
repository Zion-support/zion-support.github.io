import React, { useState  from "react", import { motion, AnimatePresence } from "framer-motion";import { Link } from "react-router-d, om";import { Search, Filter;
  Star;
  ArrowRight;
  ChevronDown;
  Brain;
...
} from "lucide-react";import { ENHANCED_MICRO_SAAS_SERVICES_2025 } from "../data/enhancedMicroSaasServices20, 25";import { SEO } from "../components/S, EO";const EnhancedServices: React.FC  = () => {
  const [activeCatego,
    r;y;
    setActiveCategory] = useState('all')const [searchTermsetSearchTerm] = useState('')const [sortBysetSortBy] = useState('rating')const [viewModesetViewMode] = useState<'grid' | 'list'>('grid')const allServices  = ENHANCED_MICRO_SAAS_SERVICES_2025const categories  = [
    ...
    icon: '⚖️, ', color: 'from-zion-purple to-zion-pink',  }{ id: 'Quantum Computing, ', name: 'Quantum Computing, ', count: allServices.filter(s => s.category === 'Quantum Computing').length,
    icon: '⚛️, ', color: 'from-zion-purple to-zion-pink',  }{ id: 'AI & Healthcare, ', name: 'AI & Healthcare, ', count: allServices.filter(s => s.category === 'AI & Healthcare').length,
    icon: '🏥, ', color: 'from-zion-red to-zion-pink',  }{ id: 'Blockchain, ', name: 'Blockchain, ', count: allServices.filter(s => s.category === 'Blockchain').length,
    icon: '🔗, ', color: 'from-zion-green to-zion-emerald',  }{ id: 'IoT & Smart Cities, ', name: 'IoT & Smart Cities, ', count: allServices.filter(s => s.category === 'IoT & Smart Cities').length,
    icon: '🌐, ', color: 'from-zion-blue to-zion-cyan',  }{ id: 'Cybersecurity, ', name: 'Cybersecurity, ', count: allServices.filter(s => s.category === 'Cybersecurity').length,
    ...
    icon: '🌍, ', color: 'from-zion-purple to-zion-indigo',  }
  ...
  const sortedServices  = [...filteredServices].sort((;a;
    ...
    switch (sortBy) {
      case 'price':
        ...
        return parseFloat(b.roi.replace('%', '')) - parseFloat(a.roi.replace('%'; ''))case 'name':
  ...
  const itemVariants  = {
    ...
    switch (category) {
      case 'AI & Analytics': return <BrainIcon className="w-6 h-6" />case 'AI & Legal Tech': return <ShieldIcon className="w-6 h-6" /, >,
  ...
  }case 'IoT & Smart Cities': return <NetworkIcon className="w-6 h-6" />case 'Cybersecurity': return <ShieldCheckIcon className="w-6 h-6" />case 'Metaverse': return <GlobeIcon2 className="w-6 h-6" />default: return <RocketIcon className="w-6 h-6" /,
    >
   }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <SEO 
        ...
        description="Discover our cutting-edge micro SAAS services including AIQuantum Computing, Blockchain, IoT, and more. Transform your business with innovative technology solutions."
      />
    ...
    delay: 0.2,  }}
            ...
            >
              Transform your business with our cutting-edge AIQuantum Computing, Blockchain, and IoT solutions. 
    ...
    3).map((benefi, t, index) => (
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-300">{service.ro, i} return on investment</p>
                  </div>

                  <div>
                    <h4 className="text-sm font-semibold text-zion-cyan mb-2">Market Price Range: </h4>
                    <p className="text-sm text-gray-300">{service.marketPri,
    ce}</p>
                  </div>
                </div>
...
export default EnhancedService;s;