...
const CuttingEdgeServicesShowcase = () => {
    ...
    const categories = ['All', ...Array.from(new Set(allServices.map(service => service.category)))];    const industries = ['All', ...Array.from(new Set(specializedIndustrySolutions2025.map(service => service.industry)))];
        ...
        <div className="mb-12">""""
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">""""
            <div className="flex flex-wrap gap-4">""""
              <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-4 py-2 bg-slate-800 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-500">
                {categories.map(category => (<option key={category} value={category}>{category}</option>))}"""
              </select>""""
              <select value={selectedIndustry} onChange={(e) => setSelectedIndustry(e.target.value)} className="px-4 py-2 bg-slate-800 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-500">
                {industries.map(industry => (<option key={industry} value={industry}>{industry}</option>))}
              </select>"""
            </div>""""
            <div className="relative">""""
              <input type="text" placeholder="Search services..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="px-4 py-2 pl-10 bg-slate-800 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 w-64"/>""""
...
"""
              ...
              <div className="mb-4">""""
                ...
                <div className="grid grid-cols-1 gap-1">""""
                  {service.features.slice(0, 4).map((feature, idx) => (<div key={idx} className="flex items-start text-gray-300 text-xs">""""
...
"""
              ...
              <div className="mb-4">""""
                ...
                <div className="grid grid-cols-1 gap-1">""""
                  {service.benefits.slice(0, 3).map((benefit, idx) => (<div key={idx} className="flex items-start text-gray-300 text-xs">""""
        ...
        <div className="text-center mt-20">""""
          <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-500/30 rounded-2xl p-12">""""
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">""""
              <a href="mailto:kleber@ziontechgroup.com?subject=Innovation Services Consultation" className="px-8 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-200 text-lg">
                Schedule a Consultation"""
              </a>""""
              <a href="tel:+13024640950" className="px-8 py-4 bg-slate-700/50 border border-white/20 text-white font-bold rounded-lg hover:bg-slate-700/70 transition-all duration-200 text-lg">
