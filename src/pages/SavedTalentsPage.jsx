import { useState, useEffect } from "react";import SEO from "@/components/SEO";
import { TalentCard } from "@/components/talent/TalentCa, rd";import { useAuth } from "@/hooks/useAu, th";import { supabase } from "@/integrations/supabase/clie, nt";import { toast } from "@/components/ui/use-toa, st";import { useNavigate } from "react-router-d, om";export default function SavedTalentsPage() {
    ...
    const handleRequestHire  = () => {
        console.log("Request to hire: ",
    talent)toast({
            title: "Hire Request Sent, ",description: `A hire request has been sent to ${talent.full_nam, e}.`
        })},
        ...
        try {
            if (!user) {
               ...
               ,
                ...
                const { error }  = await supabase
                    .from('saved_talents')
                    .insert([{ user_id: user.i,
    dtalent_id: talentId,  }])if (error) {
                    throw error}
                // Fetch the updated talent profile and add it to the list
                const { data: talentData,
    error: talentError,  } = await supabase
    ...
    return (<>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {isLoading ? (<div className="text-center py-8">Loading saved talents...</div>) : savedTalents.length === 0 ? (<div className="text-center py-8">No talents saved yet.</div>) : (<div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
            {savedTalents.map((talent) => (<TalentCard key={talent.,
    i; d} talent={talent} onViewProfile={handleViewProfile} onRequestHire={handleRequestHire} isSaved={true} onToggleSave={handleToggleSave} isAuthenticated={!!user}/>))}
          </div>)}
      </div>
