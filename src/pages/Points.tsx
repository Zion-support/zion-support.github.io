...
export default function PointsPage() {const { isAuthenticated, user } = useAuth()const { ledger, balance, loading, fetchLedger } = usePoints()const [loginOpen, setLoginOpen] = useState(false)const [redeeming, setRedeeming]  = useState(false)async function handleRedeem(): any (reward: {id: string;
    cost: number;
    title: string;}) {if (!user?.id) return;
    setRedeeming(true)try {await fetch('/api/points/redeem', {method: 'POST';
        headers: { 'Content-Type': 'application/json' },body: JSON.stringify({userId: user.id;
          cost: reward.cost;
          reward: reward.title;
...
<CardContent className='space-y-4'>;
                {earningOpportunities.map((opportunity, index) => (<div;
                    key={index}
                  ...
                  >;
                    ...
                    <div className='flex - 1'>;
                      ...
                      <p className='text - sm text - muted - foreground'>;
                        {opportunity.description}<CardContent className="space-y-4">;
                {earningOpportunities.map((opportunity, index) => (<div key={index} className="flex items-start gap-3 p-3 rounded-lg border">;
                    <div className="text-primary mt-1">{opportunity.icon}</div>;
                      <h4 className="font-medium">{opportunity.title}</h4>;
...
<CardTitle className='flex items-center gap-2'>;
                ...
                <CardDescription>What you can redeem your points for;
                </CardDescription>;
              </CardHeader>;
                    <Badge variant="outline">{reward.cost}</Badge>;
<CardContent className='space-y-4'>;
                {upcomingRewards.map((reward, index) => (<div;
                    key={index}
...
<Card className='text-center'>;
        ...
        {!loading && balance === 0 && (<p className="text-sm text-muted-foreground mt-2">;
    )}
...
<CardTitle className='flex items-center gap-2'>;
            ...
            <CardDescription>Redeem your points for these rewards;
            </CardDescription>;
          </CardHeader>;
        </div>;
        <p className='text-xl text-muted-foreground'>Points Available</p>;
        {!loading && balance === 0 && (<p className='text-sm text-muted-foreground mt-2'>            You haven't earned any points yet. Complete the tasks below to start        </div>;
        <p className="text-xl text-muted-foreground">Points Available</p>;
        {!loading && balance === 0 && (<p className="text-sm text-muted-foreground mt-2">;
            You haven't earned any points yet. Complete the tasks below to start;
            earning and redeem rewards.;
        ...
        <Card>;
          <CardHeader>;
            ...
            <CardDescription>More ways to boost your point balance;
            </CardDescription>;
          </CardHeader>;
          <CardContent className='space-y-4'>;
            {earningOpportunities && earningOpportunities.map((opportunity, index) => (<CardContent className='space-y-4'>;
            {upcomingRewards.map(reward => (<div;
                key={index}
                className='flex items-start gap-3 p-3 rounded-lg border'>;
          ...
          <CardContent className='space-y-4'>;
            {upcomingRewards && upcomingRewards.map(reward => (<div;
                ...
                <div className='text-right space-y-1'>;
                  ...
                  {balance >= reward && reward.cost ? (<Button;
                      size='sm';
                      onClick={() => handleRedeem(reward)}
                      disabled={redeeming}<CardContent className="space-y-4">;
            {upcomingRewards.map((reward) => (<div key={reward.id} className="flex items-center justify-between p-3 rounded-lg border">;
                <div>;
                  <h4 className="font-medium">{reward.title}</h4>;
                ...
                <div className="text-right space-y-1">;
                  ...
                  {balance >= reward.cost ? (>                      {redeeming ? 'Processing...' : 'Redeem'}
                    </Button>;
                  ) : (<p className='text-xs text-muted-foreground'>;
                      Need more points;
                    </p>;
...
<CardTitle className='flex items-center gap-2'>;
        ...
        <CardContent>;
          {ledger.length === 0 ? (<p className='text-center py-8 text-muted-foreground'>;
              No points activity yet.;
            </p>;
            <ScrollArea className='h-64'>;
              <div className='space-y-2 mt-2'>;
                {ledger.map(entry => (<div key={entry.id} className="flex items-center justify-between py-2 border-b">;
                    <div>;
                      variant='outline';
