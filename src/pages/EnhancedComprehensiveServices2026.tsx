...
export default function Page(function Page() {): any {}
  ...
  const cats: any = ['All', ...Array.from(new Set (allServices.map (s => s.category) ) ) ];
    return cats.sort () ;
  }, [allServices]) ;

  const filtered: any = allServices.filter(service => {;
      const matchesSearch: any = service.name.toLowerCase () .includes(searchTerm.toLowerCase () ) ||;
