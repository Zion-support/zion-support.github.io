  ...
  const [isContactDialogOpen, setIsContactDialogOpen] = useState(false)const [isChatOpen, setIsChatOpen] = useState(false)const { user } = useAuth()const { formatPrice }  = useCurrency()const router = useRouter()const id = router.query.id as string;  const [selectedImageIndex, setSelectedImageIndex] = useState(0)const [isLoading, setIsLoading] = useState(false)const [isContactDialogOpen, setIsContactDialogOpen] = useState(false)const [isChatOpen, setIsChatOpen] = useState(false)const { user } = useAuth()const { formatPrice } = useCurrency()// Find the listing from our shared data source - now also checking equipment listings;
  const listing = MARKETPLACE_LISTINGS.find(item => item.id === id)import { useState } from "react",import { useAuth } from "@/hooks/useAuth",import { ChatWidget } from "@/components/ChatWidget",import { useRouter } from "next/router",import { Badge } from "@/components/ui/badge",import { Button } from "@/components/ui/button",import Skeleton from "@/components/ui/skeleton",import ImageWithRetry from '@/components/ui/ImageWithRetry',import { Star, MessageSquare, Brain, Shield  } from 'lucide-react';
import { cn } from "@/lib/utils",import Link from 'next/link',import { MARKETPLACE_LISTINGS } from "@/data/marketplaceData",import { toast } from "@/hooks/use-toast",import { PaymentButton } from "@/components/transactions/PaymentButton",import { ProfileContact } from "@/components/profile/ProfileContact",import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog",import { useCurrency } from '@/hooks/useCurrency',export default function ListingDetail() {// useParams may be untyped in this environment, so avoid passing a;
  // type argument and cast the result instead to prevent TS2347 errors.;
  const router = useRouter(),const id = router.query.id as string,const [selectedImageIndex, setSelectedImageIndex] = useState(0),const [isLoading, setIsLoading] = useState(false),const [isContactDialogOpen, setIsContactDialogOpen] = useState(false),const [isChatOpen, setIsChatOpen] = useState(false),const { user } = useAuth(),const { formatPrice } = useCurrency(),import { Dialog, DialogContent, DialogHeader, DialogTitle  } from '@/components/ui/dialog';
...
export default function ListingDetail() {// useParams may be untyped in this environment, so avoid passing a;
  // type argument and cast the result instead to prevent TS2347 errors.;
  const router = null;
  const [isContactDialogOpen, setIsContactDialogOpen] = useState(false)const [isChatOpen, setIsChatOpen] = useState(false)const { user } = useAuth()const { formatPrice }  = useCurrency()// Find the listing from our shared data source - now also checking equipment listings;
  const listing = MARKETPLACE_LISTINGS.find(item => item.id === id),if (!listing) {return (<div className="min-h-screen bg-zion-blue py-12 px-4">;
      ...
      )const handleContact = () => {if (user) {setIsChatOpen(true)} else {setIsContactDialogOpen(true)setIsChatOpen(true)} else {<div className='min-h-screen bg-zion-blue py-12 px-4'>;
        <div className='container mx-auto'>;
          <div className='text-center py-20'>;
    ...
    )}const handleContact = () => {if (user) {setIsChatOpen(true)} else {setIsContactDialogOpen(true)}
  }
  return (<>;
                ...
                <div className='aspect-[16/9] w-full relative'>;
                  {listing.images && listing.images.length > 0 ? (<ImageWithRetry;
                    ...
                    {listing.images.map((image, index) => (<div;
                        ...
                        className={cn(index === selectedImageIndex;
                            ? 'border-zion-purple';
                            : 'border-transparent';
                        )}
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>;
                    ...
                    <div className='flex items-start gap-3'>;
                      ...
                      <div>;
                        ...
                        className={cn ('w - 20 h - 20 flex - shrink - 0 cursor - pointer rounded overflow - hidden border - 2',index === selectedImageIndex;
                            ? 'border - zion - purple';
...
<div className='mt-8 bg-zion-blue-dark rounded-lg p-6 border border-zion-blue-light'>;
                ...
                <div className='mt-8'>;
                  ...
                  <div className='flex flex-wrap gap-2'>;
                    {listing && listing.tags.map((tag, i) => (<Badge;
                        key={i}
                        variant='outline';
                {/* Tags */}
                <div className="mt-8">;
                  <h3 className="text-xl font-bold text-white mb-4">Tags</h3>;
                  <div className="flex flex-wrap gap-2">;
                    {listing && listing.tags.map((tag, i) => (<Badge key={i} variant="outline" className="border-zion-slate-dark text-zion-slate-light py-1 px-3">;
                  </div>;
                </div>;
              </div>;
            </div>;
...
<h1 className='text-2xl font-bold text-white mb-4'>;
                  ...
                  <div className='flex items-center gap-2 mb-6'>;
                    <div className='flex items-center'>;
                      {[...Array(5)].map((_, i) => (<Star;
                          key={i}
                          className={cn(i < Math.floor(listing.rating!)? 'text-zion-cyan fill-zion-cyan';
                  ...
                  <div className="flex items-center gap-2 mb-6">;
                    ...
                    <span className="text-sm text-zion-slate-light">;
                      {listing.rating.toFixed(1)} ({listing.reviewCount} reviews)<span className='text-sm text-zion-slate-light'>;
                      {listing.rating.toFixed(1)} ({listing.reviewCount}{' '}
                      reviews)</span>;
                  </div>;
                )}
                {/* Price */}<div className="text-2xl font-bold text-white">;
                      Custom Pricing;
                    </div>;
                  )}{/* Action Buttons */}
...
buttonText='Buy Now';
                ...
                <div className="border-t border-zion-blue-light pt-6">;
                  ...
                  <div className="flex items-center gap-3">;
                    {listing.author.avatarUrl ? (<div className="relative h-12 w-12 rounded-full overflow-hidden">;
                        <ImageWithRetry;
                          src={listing.author.avatarUrl}
                          alt={listing.author.name}className="object-cover";
                          onError={(e) => {const target = e.target as HTMLImageElement,className="object-cover";
                          onError={(e) => {const target = e.target as HTMLImageElement,className='object-cover';
                          onError={e => {const target = e.target as HTMLImageElement;
                            target.src =;
                              'https://ui-avatars.com/api/?name=' +;
                              encodeURIComponent(listing.author.name)target.src = "https: //ui-avatars.com/api/?name=" + encodeURIComponent(listing.author.name)}}
                        />;
                      </div>;
...
<div className='h-12 w-12 rounded-full bg-zion-purple/20 flex items-center justify-center'>;
                ...
                {/* Additional Info */}<div className="border-t border-zion-blue-light mt-6 pt-6">;
      ...
      <ChatWidget;
                ...
                <div className='border - t border - zion - blue - light pt - 6'>;
                  ...
                  <div className='flex items - center gap - 3'>;
                    {listing.author.avatar_url ? (<div className='relative h - 12 w - 12 rounded - full overflow - hidden'>;
                        <ImageWithRetry;
                        ...
                        />;
                      </div>) : (                            target.src = "https: //ui - avatars.com / api/?name=" + encodeURIComponent (listing.author.name)target.src = "https: //ui - avatars.com / api/?name=" + encodeURIComponent (listing.author.name)}}
                        />;
                      </div>) : (<div className='h - 12 w - 12 rounded - full bg - zion - purple / 20 flex items - center justify - center'>;
                        <span className='text - lg font - medium text - zion - purple'>;
...
}/> </div>) : (<div className=" h-12 w-12 rounded-full bg-zion-purple/20 flex items-center justify-center"> <span className=" text-lg font-medium text-zion-purple"> {listing.author.name.charAt (0)}</span> </div>) ";
