const Services2027: React.FC = (): JSX.Element => {}
  ...
  const filteredAndSorted: any = useMemo(() => {}
    ...
    if(query.trim () ) {}
      ...
      const q: any = query.trim().toLowerCase();      filtered = filtered.filter(s =>
        s.title.toLowerCase().includes(q) ||
        s.description.toLowerCase().includes(q) ||
        s.category.toLowerCase().includes(q)
      )}
...
'
      ...
      case 'roi': any;
        return filtered.sort((a, b) => {}
          const aROI: any = parseInt(a.roi.split ('-') [0]) ;
          const bROI: any = parseInt(b.roi.split ('-') [0]) ;
          return bROI - aROI;
        }) ;
      default: any;
        return filtered;
