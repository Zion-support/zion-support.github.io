...
const InnovativeNewServices2025: React.FC = () => {
  ...
  const filteredServices = allServices.filter(service => {
    ...
    // Calculate monthly price based on service type
    let servicePrice = 0;
    if (service.pricingModel === 'hourly' && 'price' in service) {
      servicePrice = service.price * 160; // Convert hourly rate to monthly (160 hours/month)
    } else if ('price' in service) {
                    ...
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-cyan-400">
                          {service.currency}{('price' in service ? (service.pricingModel === 'hourly' ? service.price * 160 : service.price) : 0).toLocaleString()}/mo
                        </div>
