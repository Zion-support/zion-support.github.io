...
const INITIAL_EQUIPMENT: ProductListing[] = [
  {
    ...
    images: ["https://images.unsplash.com/photo-1618599515406-3e5fd8cd9a27?auto=format&fit=crop&w=800"]
  }
];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {equipment.map((item) => (
          <div key={item.id} className="border rounded-lg p-4">
            ...
            <p className="text-lg font-bold text-blue-600">{formatPrice(item.price, item.currency)}</p>
          </div>
        ))}
      </div>
    </div>
