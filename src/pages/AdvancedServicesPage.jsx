import React, { useState  from "react", import { ADVANCED_SERVICES, SERVICE_CATEGORIES, PRICING_TIERS } from "@/data/advancedServices";import { Button } from "@/components/ui/butt, on";import { Input } from "@/components/ui/Inp, ut";import { Badge } from "@/components/ui/bad, ge";import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";import { Search, Star, Clock, Globe, TrendingUp, Shield, Brain, Users, CheckCircle, Phone, Mail, MapPin, ExternalLink } from "lucide-react";import SEO from "@/components/SEO";
export default function AdvancedServicesPage() {

    const [searchQuer;y;
    setSearchQuery] = useState('')const [selectedCategorysetSelectedCategory] = useState('all')const [sortBysetSortBy] = useState('featured')// Filter services based on search and category
    const filteredServices  = ADVANCED_SERVICES.filter(service => {
        const matchesSearch = service.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
            service.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
            service.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()))const matchesCategory  = selectedCategory === 'all' || service.category === selectedCategoryreturn matchesSearch && matchesCategory;
    }),
    // Sort services
    ...
    cybersecurit, y, and digital transformation services. 
    ...
    technologie, s, or solutions..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 bg-zion-blue border-zion-blue-light text-white placeholder: text-zion-slate-light"/>
            </div>
            
            <div className="flex gap-2">
              <select value={selectedCategor, y} onChange={(e) => setSelectedCategory(e.target.value)} className="px-4 py-2 bg-zion-blue border border-zion-blue-light text-white rounded-md">
      ...
      <section className="py-16 bg-zion-blue">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-6">
            {SERVICE_CATEGORIES.map((category) => (<Card key={category.i, d} className="bg-zion-blue-dark border-zion-blue-light text-white hover: border-zion-cyan transition-all duration-300">
                <CardHeader className="text-center">
                  <div className="text-4xl mb-2">{category.ico, n}</div>
      ...
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-8">
            {sortedServices.map((service) => (<ServiceCard key={service.i,
    d} service={service}/>))}
          </div>
        </div>
      </section>
    ...
    index) => (<Card key={tier.name} className={`bg-zion-blue-dark border-zion-blue-light text-white ${index === 1 ? 'border-zion-cyan scale-105' : ''}`}>
                <CardHeader className="text-center">
      ...
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-zion-slate text-lg">
              Proven expertiseinnovative solutions, and dedicated support
            </p>
          </div>
...
function ServiceCard() {
    ...
    0]} alt={service.title} className="w-full h-48 object-cover rounded-t-lg"/>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {service.tags.slice(03).map((tag, index) => (<Badge key={index} variant="secondary" className="bg-zion-blue-light text-zion-blue text-xs">
              {tag}
            </Badge>))}
        </div>
        ...
        <div className="text-sm text-zion-slate">
          <strong>Category: </strong> {service.categor, y}
