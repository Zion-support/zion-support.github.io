...
import React, { useState } from "react",import { Header } from "@/components/Header",import { SEO } from "@/components/SEO",import { useAuth } from "@/hooks/useAuth",import { useRouter } from "next/router",import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card",import { Input } from "@/components/ui/input",import { Label } from "@/components/ui/label",import { Button } from "@/components/ui/button",import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs",import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select",import { toast } from "sonner",import { supabase } from "@/integrations/supabase/client",export default function TenantOnboarding() {const { user } = useAuth()const [activeTab, setActiveTab] = useState("company")const [isSubmitting, setIsSubmitting] = useState(false)const [formData, setFormData] = useState({brand_name: "";
    subdomain: "";
    logo_url: "";
    primary_color: "#9b87f5";
    theme_preset: "light";
    company_size: "";
    industry: "";
    custom_domain: "";is_co_branded: true;
  })// Check if user has admin role;
  const isAdmin = user?.role === "admin";
      })}}),// Check if user has admin role;
  const isAdmin = user?.role === "admin",if (!isAdmin) {return // Use router.push('/unauthorized') or redirect in getServerSideProps;
      ...
      })} catch (error: any) {logErrorToProduction(error instanceof Error ? error.message : String(error), error instanceof Error ? error : undefined, { message: 'Error creating tenant' }),toast.error("Failed to create tenant", {description: error.message;
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">;
                  ...
                  <TabsContent value="company" className="space-y-4">;
                    ...
                    <div className="space-y-2">;
                      ...
                      <Select;
                        name="industry";
                        value={formData.industry}onValueChange={(value) => handleSelectChange("industry", value)}
                      >;
                        <SelectTrigger>;
                    ...
                    <div className="space-y-2">;
                      ...
                      <Select;
                        name="company_size";
                        value={formData.company_size}onValueChange={(value) => handleSelectChange("company_size", value)}
                      >;
                        <SelectTrigger>;
                  ...
                  <TabsContent value="branding" className="space-y-4">;
                    ...
                    <div className="space-y-2">;
                      ...
                      <Select;
                        name="theme_preset";
                        value={formData.theme_preset}onValueChange={(value) => handleSelectChange("theme_preset", value)}
                      >;
                        <SelectTrigger>;
...
import { Switch } from '@/components/ui/switch';
import { logErrorToProduction } from '@/utils/productionLogger';
import { Switch } from "@/components/ui/switch",import { logErrorToProduction } from '@/utils/productionLogger',export default function TenantOnboarding() {const { user } = useAuth()const [activeTab, setActiveTab] = useState("company")const [isSubmitting, setIsSubmitting] = useState(false)const [formData, setFormData] = useState({brand_name: "",subdomain: "",logo_url: "",primary_color: "#9b87f5",theme_preset: "light",company_size: "",industry: "",custom_domain: "",is_co_branded: true;
  })// Check if user has admin role;
  const isAdmin  = user?.role === "admin";}
  )}import React, { useState } from './react';
import { Header  } from '@/components / Header';
