...
import { Send } from 'lucide-react', // Added Send icon,
import { toast } from "sonner",
import { logErrorToProduction } from '@/utils/productionLogger',interface WhitepaperSection {
  id: string,
  title: string,
  content: string
...
const WhitepaperGeneratorPage: React.FC = () => {const [tokenName, setTokenName] = useState('My Awesome Token');
  ...
  const [distributionData, setDistributionData] = useState<DistributionItem[]>([
    { id: window.crypto.randomUUID(), name: 'Team & Advisors', percentage: '15'},
    {id: window.crypto.randomUUID(), name: 'Private Sale Investors', percentage: '20'},
    {id: window.crypto.randomUUID(), name: 'Ecosystem Development Fund', percentage: '35'},
    {id: window.crypto.randomUUID(), name: 'Community Rewards & Airdrops', percentage: '20'},
    {id: window.crypto.randomUUID(), name: 'Public Sale Allocation', percentage: '10'}]);
...
const [rawDraft, setRawDraft] = useState<string | null>(null),
  ...
  const distributionChartData: DistributionChartItem[] = React.useMemo_(() => {return distributionData
      .map(item => ({
        name: item.name || 'Unnamed',
        value: parseFloat(item.percentage) || 0}))
      .filter(item => item.value > 0)
  ...
  const handleGenerateWhitepaper = async () => {
    ...
    try {
      ...
      setIsLoading(false)
        ...
        name: item.name || 'Unnamed', value: parseFloat(item.percentage) || 0}))
      .filter(item => item.value > 0)
  }, [distributionData]);
  ...
  const handleGenerateWhitepaper = async () => {setIsLoading(true);
    ...
    const processedDistData = distributionChartData.map(d => ({name: d.name, percentage: d.value}));
    const totalPercentage = processedDistData.reduce(_(sum, item) => sum + item.percentage, 0);
    ...
    try {const apiPayload: unknown = {
        tokenName, tokenSupply: tokenSupply.toString(), useCases, rewardsLogic, governanceLogic, legalDisclaimers, distributionBreakdown};
      ...
      setRawDraft((data as any).whitepaperDraft);
      setSections(parseWhitepaperDraft((data as any).whitepaperDraft))
    } catch (e: unknown) {logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, _{ message: 'Error generating whitepaper'});
      setError(e.message || 'An unexpected error occurred.');
      setSections([])
  ...
  const handleDownloadMarkdown = () => {
    ...
    try {
      ...
      const link = document.createElement('a'),
      link.href = url,
      link.download = `${slugify(tokenName || 'whitepaper')}whitepaper.md`,
      document.body.appendChild(link),
      link.click(),
      document.body.removeChild(link),
      URL.revokeObjectURL(url),
  ...
  const handleDownloadMarkdown = () => {setIsDownloading(true);
    try {
      const markdown = assembleMarkdownContent();
      const blob = new Blob([markdown], _{ type: 'text/markdown;charset=utf-8'});
      ...
      setError(null)
    } catch (e: unknown) {logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, _{ message: 'Error downloading Markdown'});
        setError("Failed to download Markdown file. " + e.message)
  ...
  const handleDownloadPdf = async () => {setIsDownloading(true);
    ...
    try {_// Temporarily ensure the entire preview panel content is visible for capture if it's scrollable internally.
      ...
      const html2canvas = html2canvasModule.default,
      const { default: jsPDF } = await import('jspdf'),
      const canvas = await html2canvas(previewPanelRef.current, {scale: 2, _// Increase scale for better resolution,
...
onclone: (documentClone) => {
      ...
      pdf.save(`${slugify(tokenName || 'whitepaper')}whitepaper.pdf`)
  ...
  const handleGenerateShareableLink = async () => {
    ...
    try {
      ...
      setShareableLink(link),
      setCurrentSharedWhitepaperId((response as any).id),
      setCurrentSharedWhitepaperIsPublic((response as any).ispublic),
      toast.success("Shareable link generated!")
    } catch (e: any) {
      logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, { message: 'Error generating shareable link' }),
      setError("Failed to generate shareable link: " + e.message),
      toast.error("Failed to generate shareable link.")    } finally {
      setIsSharing(false)
    }
  },
...
const newPublicStatus = !currentSharedWhitepaperIsPublic,
      while (heightLeft > 0) {position = heightLeft - imgHeight; // Or position = position - pdfHeight;
        pdf.addPage();
        pdf.addImage(imgData, _'PNG', 0, position, pdfWidth, imgHeight);
        heightLeft -= pdfHeight}

      pdf.save(`${slugify(tokenName || 'whitepaper')}whitepaper.pdf`)

    } catch (e: unknown) {logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, _{ message: 'Error downloading PDF'});
      setError("Failed to download PDF file. " + e.message)
    } finally {setIsDownloading(false)}
  };
  ...
  const handleGenerateShareableLink = async () => {if (sections.length === 0) {
    ...
    try {const whitepaperPayload = {
        tokenName, tokenSupply, sections, distributionChartData, distributionBreakdown};
      ...
      const link = `${window.location.origin}/whitepaper/view/${_(response as any).id}`;
      setShareableLink(link);
      setCurrentSharedWhitepaperId((response as any).id);
      setCurrentSharedWhitepaperIsPublic((response as any).ispublic);
      toast.success("Shareable link generated!")
    } catch (e: unknown) {logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, _{ message: 'Error generating shareable link'});
      setError("Failed to generate shareable link: " + e.message);
      toast.error("Failed to generate shareable link.")
...
const newPublicStatus = !currentSharedWhitepaperIsPublic;
    ...
    try {
        ...
        setCurrentSharedWhitepaperIsPublic((response as any).ispublic), // Update with actual status from DB,
toast.success(`Whitepaper is now ${(response as any).ispublic ? 'public' : 'private'}.`)

    } catch (e: any) {
        logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, { message: 'Error toggling public status' }),
        setError("Failed to update public status: " + e.message),
        toast.error("Failed to update public status."),    try {const { data: response, error: funcError} = await supabase.functions.invoke('set-shared-whitepaper-public-status', {body: { whitepaperId: currentSharedWhitepaperId, isPublic: newPublicStatus}});
        if (funcError) throw new Error(`Supabase function error: ${funcError.message}`);
        ...
        setCurrentSharedWhitepaperIsPublic((response as any).ispublic); // Update with actual status from DB,
toast.success(`Whitepaper is now ${_(response as any).ispublic ? 'public' : 'private'}.`)

    } catch (e: unknown) {logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, _{ message: 'Error toggling public status'});
        setError("Failed to update public status: " + e.message);
        toast.error("Failed to update public status.");
        // Revert optimistic update if it failed:
        // setCurrentSharedWhitepaperIsPublic(!newPublicStatus)
  ...
  const handleSubmitToCounsel = async () => {
    ...
    try {
        ...
        if (!linkToSubmit || !whitepaperIdToSubmit) {
            toast.info("Generating a shareable link first to submit to counsel..."),
            const whitepaperPayload = { tokenName, tokenSupply, sections, distributionChartData, distributionBreakdown },            const { data: linkResponse, error: linkFuncError } = await supabase.functions.invoke('create-shared-whitepaper', {
                body: whitepaperPayload}),
            if (linkFuncError) throw new Error(`Failed to create link for counsel: ${linkFuncError.message}`),
            if (!linkResponse) throw new Error('No response received from create-shared-whitepaper function for counsel'),
            if ((linkResponse as any).error) throw new Error(`Error from create-shared-whitepaper function: ${(linkResponse as any).error}`),
            ...
            setShareableLink(linkToSubmit),
            setCurrentSharedWhitepaperId(whitepaperIdToSubmit),
            setCurrentSharedWhitepaperIsPublic((linkResponse as any).ispublic)
        }
  ...
  const handleSubmitToCounsel = async () => {if (sections.length === 0) {
    ...
    try {let linkToSubmit = shareableLink;
        ...
        if (!linkToSubmit || !whitepaperIdToSubmit) {
            ...
            linkToSubmit = `${window.location.origin}/whitepaper/view/${_(linkResponse as any).id}`;
            whitepaperIdToSubmit = (linkResponse as any).id;
            setShareableLink(linkToSubmit);
            setCurrentSharedWhitepaperId(whitepaperIdToSubmit);
            setCurrentSharedWhitepaperIsPublic((linkResponse as any).ispublic)
...
if (currentSharedWhitepaperIsPublic === false) {toast.info("Making whitepaper public before submitting to counsel...");
        ...
        const {data: notifyResponse, error: notifyError} = await supabase.functions.invoke('notify-legal-team', {body: {
                whitepaperId: whitepaperIdToSubmit, sharableLink: linkToSubmit, _// Corrected variable name,
tokenName: tokenName}
        }),
        if (notifyError) throw new Error(`Failed to notify counsel: ${notifyError.message}`),
        if (!notifyResponse) throw new Error('No response received from notify-legal-team function'),
        ...
        logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, { message: 'Error submitting to counsel' }),
        setError("Failed to submit to counsel: " + e.message),
        toast.error("Failed to submit to counsel: " + e.message)    } finally {
        setIsSubmittingToCounsel(false)
    }
},
        ...
        <div className=&quot;flex justify-between items-center mb-6&quot;>
            ...
            <div className=&quot;flex space-x-1&quot;>
                ...
                <Button onClick={handleDownloadPdf} disabled={isDownloading || sections.length === 0 || isLoading || isSharing || isSubmittingToCounsel} variant=&quot;outline&quot; size=&quot;sm&quot; title=&quot;Download as PDF&quot;>
                 ...
                 <Button onClick={handleGenerateShareableLink} disabled={isSharing || sections.length === 0 || isLoading || isDownloading || isSubmittingToCounsel} variant=&quot;outline&quot; size=&quot;sm&quot; title=&quot;Generate Shareable Link&quot;>
                    <Share2 className=&quot;h-4 w-4&quot; /> <span className=&quot;ml-1 hidden sm:inline&quot;>Share</span>
    } catch (e: unknown) {logErrorToProduction(e instanceof Error ? e.message : String(e), e instanceof Error ? e : undefined, _{ message: 'Error submitting to counsel'});
        setError("Failed to submit to counsel: " + e.message);
        toast.error("Failed to submit to counsel: " + e.message)
      ...
      <div className="md:w-1/2 lg:w-2/5 xl:w-1/3 p-4 bg-white rounded-lg shadow-md overflow-y-auto">
        ...
        <form onSubmit={(e) => e.preventDefault()} className=&quot;space-y-6&quot;>
          ...
          <div className=&quot;space-y-3 p-3 border rounded-md&quot;>
            ...
            <div>
              <label htmlFor=&quot;distributionBreakdownDetails&quot; className=&quot;block text-sm font-medium&quot;>Additional Distribution Details (Text):</label>
              <textarea id=&quot;distributionBreakdownDetails&quot; value={distributionBreakdown} onChange={(e) => setDistributionBreakdown(e.target.value)} className=&quot;mt-1 block w-full border-gray-300 rounded-md shadow-sm&quot; rows={2}/>            </div>
          </div>
          ...
          <div>
            <label htmlFor=&quot;legalDisclaimers&quot; className=&quot;block text-sm font-medium&quot;>Legal Disclaimers:</label>
            <textarea id=&quot;legalDisclaimers&quot; value={legalDisclaimers} onChange={(e) => setLegalDisclaimers(e.target.value)} required className=&quot;mt-1 block w-full border-gray-300 rounded-md shadow-sm&quot; rows={3}/>          </div>
...
onClick={handleTogglePublicStatus}
                    ...
                    size=&quot;sm&quot; // smaller button,
disabled={isSharing} // Disable while another share operation is in progress,
className={currentSharedWhitepaperIsPublic ? &quot;bg-red-100 hover:bg-red-200&quot; : &quot;bg-green-100 hover:bg-green-200&quot}                >
                    {currentSharedWhitepaperIsPublic ? 'Make Private' : 'Make Public'}
                </Button>
              </div>
              <div className="flex items-center space-x-2 mt-1">
               ...
               {currentSharedWhitepaperIsPublic !== null && (
                <p className=&quot;text-xs mt-1 text-gray-600&quot;>
                    Currently: {currentSharedWhitepaperIsPublic ? &quot;Public&quot; : &quot;Private (Only admins can view)&quot}
                <Input type="text" value={shareableLink} readOnly className="flex-grow bg-white text-xs"/>
          ...
          <div className=&quot;mt-8 pt-6 border-t&quot;>
            ...
            {sections.map((section) => (
              <WhitepaperSectionEditor                key={section.id}
                title={section.title}
                content={section.content}
                onContentChange={_(newContent) => handleSectionContentChange(section.id, newContent)}
              />
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
            ))}
          </div>;
...
=======
      ...
      {/* Right Column: Preview Panel - Pass ref here */}
      <div id=&quot;preview-panel-content&quot; ref={previewPanelRef} className=&quot;md:w-1/2 lg:w-3/5 xl:w-2/3 p-1&quot;>        <WhitepaperPreviewPanel,
sections={sections}
            distributionChartData={distributionChartData}
            tokenName={tokenName}
            tokenSupply={tokenSupply}
