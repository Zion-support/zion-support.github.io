...
import { Badge } from "@/components/ui/bad, ge";import { Button } from "@/components/ui/butt, on";import Input from "@/components/ui/Input";
import { MICRO_SAAS_SERVICES } from "@/data/microSaasServic, es";import { motion } from "framer-moti, on";import { ArrowRight, BarChart3;
    Briefcase;
    Globe;
    Grid;
    List;
    ...
    Search;
    Star
} from "lucide-react";import { useEffect, useState } from "react";import { Link } from "react-router-d, om";// Sample data for demonstration
const MICRO_SAAS_SERVICES  = [
  {
    ...
    price: 29,
    rating: 4., 8,reviewCount: 15, 6,aiScore: 9, 5,featured: tru, e,createdAt: '2024-01-15, ',href: '/services/micro-crm, ',tags: ['CRMBusiness, ', 'Automation'],
    images: ['/images/micro-crm.jpg']
 ,  }{
    id: 'helpdesk-platform, ',title: 'Helpdesk Platform, ',description: 'Streamlined customer support and ticket management system, ',category: 'Customer Support, ',subcategory: 'Helpdesk, ',pricingModel: 'monthly, ',price: 39,
    rating: 4., 7,reviewCount: 8, 9,aiScore: 9, 2,featured: fals, e,createdAt: '2024-02-01, ',href: '/services/helpdesk-platform, ',tags: ['SupportHelpdesk, ', 'Customer Service'],
    images: ['/images/helpdesk.jpg']
 ,  }{
    id: 'website-analytics, ',title: 'Website Analytics, ',description: 'Comprehensive website performance and user behavior analytics, ',category: 'Analytics, ',subcategory: 'Web Analytics, ',pricingModel: 'monthly, ',price: 19,
    rating: 4., 9,reviewCount: 23, 4,aiScore: 9, 8,featured: tru, e,createdAt: '2024-01-10, ',href: '/services/website-analytics, ',tags: ['AnalyticsWeb, ', 'Performance'],
 ...
 ,  }
  ...
  { id: 'all,'name: 'All Pricing',
     }{ id: 'monthly, ', name: 'Monthly',  }{ id: 'yearly, ', name: 'Yearly',  }{ id: 'one-time, ', name: 'One-time',  },
  { id: 'usage-based, ', name: 'Usage-based',  }
]export default function ComprehensiveServicesOverview2027() {

  const [selectedCategorysetSelectedCategory] = useState('all')const [selectedPricingsetSelectedPricing] = useState('all')const [searchQuerysetSearchQuery] = useState('')const [filteredServicessetFilteredServices] = useState(MICRO_SAAS_SERVICES)const [sortBy;
    setSortBy] = useState<'rating' | 'price' | 'aiScore' | 'newest'>('rating')const [viewModesetViewMode] = useState<'grid' | 'list'>('grid')const [expandedServicesetExpandedService] = useState<string | null>(null)useEffect(() => {
    let filtered  = MICRO_SAAS_SERVICE;S;
    if (selectedCategory !== 'all') {
      filtered = filtered.filter(service => service.category === selectedCategory)}if (selectedPricing !== 'all') {
      filtered = filtered.filter(service => service.pricingModel === selectedPricing)}if (searchQuery.trim()) {
      const query  = searchQuery.toLowerCase()filtered = filtered.filter(service =>
    ...
    b) => {
      switch (sortBy) {
        case 'rating':
          return (b.rating || 0) - (a.rating || 0)case 'price':
          return (a.price || 0) - (b.price || 0)case 'aiScore':
          return b.aiScore - a.aiScorecase 'newest':
          return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()default: return 0
     ,
     }
    })setFilteredServices(filtered)}, [selectedCategory;
    selectedPricing, searchQuery, sortBy]),
  ...
  const ServiceCard  = () => (
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-white font-semibold">{service.pricingMode, l}</span>
          </div>
          <div className="flex items-center justify-between">
    ...
    <div className="min-h-screen bg-gradient-to-br from-zion-slate-dark via-zion-slate to-zion-slate-light">
      <SEO
        ...
        description="Explore our comprehensive portfolio of innovative AIIT, and business solutions. From AI-powered chatbots to quantum computing services, discover cutting-edge technology solutions."
        keywords="AI servicesIT solutions, business automation, cybersecurity, blockchain, quantum computing, Zion Tech Group"
      />
      {/* Hero Section */}
    ...
    y}
            ...
            <div className="flex flex-wrap gap-4">
              {categories.map((category) => (
                <Button
                  key={category.id}variant={selectedCategory === category.id ? 'default' : 'outline'}
        ...
        {filteredServices.length === 0 && (
          <div className="text-center py-16">
            ...
            <Button
              onClick={() => {
                setSelectedCategory('all')setSelectedPricing('all')setSearchQuery('')}}
              className="bg-zion-cyan hover: bg-zion-cyan-light"
            >
              Clear Filters
            </Button>
