import React, { useState  from "react", import { motion, AnimatePresence } from "framer-motion";import { Link } from "react-router-d, om";import { Brain, Cloud;
  ...
  const filteredServices  = allServices.filter(service => {
    const matchesCategory = selectedCategory === 'all' || service.category === selectedCategoryconst matchesPrice  = service.price >= priceRange[0] && service.price <= priceRange[1];
    return matchesCategory && matchesPric;e;
  });
  ...
  const sortedServices  = [...filteredServices].sort((;a;
    ...
    switch (sortBy) {
      case 'price':
        return a.price - b.pricecase 'rating':
        return b.rating - a.ratingcase 'roi':
        return parseInt(b.roi) - parseInt(a.roi)case 'name':
        return a.title.localeCompare(b.title)default: return a.price - b.pric,
    e
   }
  })// Calculate statistics
  const totalServices  = allServices.lengt;h;
  const averagePrice  = Math.round(allServices.reduce((ac;c;
    s) => acc + s.price, 0) / totalServices),
  const totalValue  = allServices.reduce((ac;c;
    s) => acc + s.price, 0),
  const averageROI  = Math.round(allServices.reduce((ac;c;
    s) => acc + parseInt(s.roi); 0) / totalServices),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <SEO 
        ...
        description="Explore our complete pricing guide for innovative AIquantum computing, space technology, and emerging tech services. Get detailed pricing, ROI analysis, and market comparisons."
        keywords="pricing guideAI services pricing, quantum computing cost, space technology pricing, emerging tech pricing, Zion Tech Group"
      />
    ...
    delay: 0.4,  }}
          ...
          >
            <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
              ...
              <div className="text-2xl font-bold text-white">{totalService,
    s}</div>
    ...
    y: 0,  }}
          ...
          >
            ...
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              ...
              detailed pricing,
    ROI projection, s, and competitive analysis.
            </p>
          </motion.div>
                ...
                <div className="space-y-4 mb-6">
                  <div className="bg-white/5 rounded-lg p-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-green-400 text-sm">{service.marketPric,
    e}</span>
                    </div>
                  </div>
