...
import { Button } from "@/components/ui/butt, on";import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";import { Badge } from "@/components/ui/bad, ge";import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";import { Star, Zap, TrendingUp, CheckCircle, Phone, Mail, Globe, Shield, Users, ArrowUpRight, Brain, Cloud, Database, Lock, Code, BarChart3, MessageSquare, FileText, ShoppingCart, Settings } from "lucide-react";import { EXPANDED_SERVICES, SERVICE_CATEGORIES } from "@/data/expandedServices";import { TrustedBySection } from "@/components/TrustedBySecti, on";import SEO from "@/components/SEO";
export default function AllServicesLandingPage() {
    ...
    const getCategoryIcon  = () => {
       ...
       ,
        ...
        switch (category) {
            case 'AI Services': return <Brain className="h-6 w-6"/>case 'Micro SAAS': return <Cloud className="h-6 w-6"/>case 'IT Services': return <Code className="h-6 w-6"/>case 'Digital Services': return <ArrowUpRight className="h-6 w-6"/>default: return <Settings className="h-6 w-6"/,
    >
       }
    }const getPricingModelColor = () => {
    switch (model) {
            case 'subscription': return 'bg-blue-100 text-blue-800'case 'project-based': return 'bg-purple-100 text-purple-800'
    ...
    switch (subcategory) {
            case 'Customer Service': return <MessageSquare className="h-5 w-5"/>case 'Analytics': return <BarChart3 className="h-5 w-5"/>,
  ...
  }case 'DevOps': return <Code className="h-5 w-5"/>case 'Transformation': return <ArrowUpRight className="h-5 w-5"/>case 'E-commerce': return <ShoppingCart className="h-5 w-5"/>default: return <Settings className="h-5 w-5"/,
    >
       }
    }return (<div className="min-h-screen bg-background">
      <SEO title="Complete Tech Solutions - AIMicro SAAS, IT & Digital Services | Zion Tech Group" description="Discover our comprehensive ecosystem of AI services, micro SAAS solutions, IT infrastructure, and digital transformation services. Expert solutions for modern businesses." keywords="AI services, micro SAAS, IT services, digital transformation, cloud migration, cybersecurity, DevOps, business automation, Zion Tech Group" canonical="https: //ziontechgroup.com/services"/>
    ...
    comprehensive IT infrastructureand digital transformation services - we provide everything your business needs to thrive in the digital age.
          </p>
          ...
          <div className="grid grid-cols-2 md: grid-cols-4 gap-6 max-w-4xl mx-auto">
            <div className="text-center">
              <div className="text-3xl font-bold text-zion-cyan mb-2">{EXPANDED_SERVICES.lengt, h}</div>
          ...
          <Tabs defaultValue="ai-services" className="w-full">
            <TabsList className="grid w-full grid-cols-4 mb-8">
              {SERVICE_CATEGORIES.map((category) => (<TabsTrigger key={category.id} value={category.id} className="flex items-center space-x-2">
                  ...
                  <span className="hidden sm: inline">{category.nam, e}</span>
                </TabsTrigger>))}
            </TabsList>
            ...
            {SERVICE_CATEGORIES.map((category) => (<TabsContent key={category.id} value={category.id} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-6">
                  {category.services.map((service) => (<Card key={service.i, d} className="h-full hover: shadow-xl transition-all duration-300 border-zion-blue-light">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm font-medium text-zion-purple">AI Score: {service.aiScor, e}</span>
                          </div>
                        </div>
    ...
    3).map((featur, e, index) => (<Badge key={index} variant="secondary" className="text-xs">
                                {feature}
                              </Badge>))}
    ...
    2).map((benefi, t, index) => (<li key={index} className="flex items-start space-x-2 text-sm text-gray-600">
                        ...
                        <div className="pt-4 border-t border-gray-200 space-y-2">
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-medium">{service.availabilit, y}</span>
                          </div>
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-600">Response Time: </span>
                            <span className="font-medium">{service.responseTim, e}</span>
                          </div>
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-600">Support Level: </span>
                            <span className="font-medium">{service.supportLeve, l}</span>
                          </div>
                        </div>
          ...
          <div className="grid grid-cols-1 md: grid-cols-3 gap-8 max-w-4xl mx-auto">
            <div className="text-center">
              ...
              <p className="text-zion-slate-light">
                Our services have helped businesses achieve significant improvements in efficiency,
    cost saving, s, and growth.
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-zion-slate-light">
                Certified professionals with years of experience in AIIT, and digital transformation across various industries.
              </p>
            </div>
      ...
      <section className="py-16 bg-gradient-to-r from-zion-purple to-zion-purple-dark">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="text-zion-slate-light">
            <p className="mb-2">Address: 364 E Main St STE 1008,
