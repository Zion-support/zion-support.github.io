...
}</div> </div> </div> </div> /> </div> </div> <ReactMarkdown> {post.content}</ReactMarkdown> </div> <span key= {
tag ";
}className=" text-xs text-zion-slate-light bg-zion-blue-dark px-3 py-1 rounded-full"> # {tag}</span>) ) ";
...
const formatDate: any = (dateString: string) => {return new Date(dateString) .toLocaleDateString("en - US", {
year: "numeric",
month: "long",
day: "numeric"})
}
...
const getCategoryName: any = (category: string) => {
...
{blogPost.relatedPosts.map((post index) => (<motion.article key={post.id}
