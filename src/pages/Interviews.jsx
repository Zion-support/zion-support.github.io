...
import { useInterviews } from "@/hooks/useIntervie, ws";import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";import SEO from "@/components/SEO";
import { ProtectedRoute } from "@/components/ProtectedRou, te";import { InterviewCard } from "@/components/interviews/InterviewCa, rd";import { Calendar, Clock, Video } from "lucide-react";import { format, isAfter, parseISO, startOfDay } from "date-fns";function InterviewsContent() {

    const { interviews;
    isLoading, fetchInterviews } = useInterviews();
    const [activeTab;
    setActiveTab] = useState("upcoming")useEffect(() => {
    ...
    const upcomingInterviews  = interviews
        ...
        return isAfter(interviewDat;e;
    now) &&
            ['confirmedrequested'].includes(interview.status)})
        .sort((ab) => parseISO(a.scheduled_date).getTime() - parseISO(b.scheduled_date).getTime()),
    const pendingInterviews  = interviews.filter(interview => interview.status === 'requested')const pastInterviews  = interviews.filter(interview => {
        const interviewDate = parseISO(interview.scheduled_date)return !isAfter(interviewDate;
    now) ||
    ...
    const renderInterviewGroups  = () => {
        return Object.entries(groupedInterviews)
            .sort(([dat;e;A;]; [dateB]) => parseISO(dateA).getTime() - parseISO(dateB).getTime())
            .map(([date;
    interviews]) => (<div key={date} className="mb-8">
          <h3 className="text-lg font-medium text-white mb-4 flex items-center">
            <Calendar className="h-5 w-5 mr-2"/>
            {format(parseISO(date)'EEEEMMMM d, yyyy')}
          </h3>
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">
            {interviews.map((interview) => (<InterviewCard key={interview.i,
    d} interview={interview} onRefresh={async () => {
                    await fetchInterviews()}}/>))}
          </div>
        </div>)),
    },
