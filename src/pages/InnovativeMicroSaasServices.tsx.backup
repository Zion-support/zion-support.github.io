...
>>>>>>> 2569ab8784f28177b60ebf1fb896001693b757b7
  ...
  const formatPrice = (price: number | { monthly?: number; yearly?: number; oneTime?: number; currency: string; pricingModel: string; }, currency?: string) => {
    if (typeof price === 'object') {
      const priceObj = price;
      const amount = priceObj.monthly || priceObj.yearly || priceObj.oneTime || 0;
      const curr = priceObj.currency;
      if (amount >= 1000) {
    ...
    const curr = currency || '$';
    if (price >= 1000) {
      return `${curr}${(price / 1000).toFixed(1)}k`;
    }
    return `${curr}${price}`;
  };

  const getServiceTitle = (service: AdvancedMicroSaasService | EmergingTechService): string => {
    return 'title' in service ? service.title : 'name' in service ? service.name : '';
  };

  const getServiceTags = (service: AdvancedMicroSaasService | EmergingTechService): string[] => {
    return 'tags' in service ? service.tags : [];
  };

  const getServicePrice = (service: AdvancedMicroSaasService | EmergingTechService) => {
    return service.price;
  };

  const getServiceCurrency = (service: AdvancedMicroSaasService | EmergingTechService): string => {
    if ('price' in service && typeof service.price === 'object') {
      return service.price.currency;
    }
    return '$'; // default for AdvancedMicroSaasService
  };

  const getServicePricingModel = (service: AdvancedMicroSaasService | EmergingTechService): string => {
    if ('pricingModel' in service) {
      return service.pricingModel;
    }
    if ('price' in service && typeof service.price === 'object') {
      return service.price.pricingModel;
    }
    return 'month';
  };

  const getServiceSupportLevel = (service: AdvancedMicroSaasService | EmergingTechService): string => {
    return 'supportLevel' in service ? service.supportLevel : 'Standard';
  };

  const getServiceEstimatedDelivery = (service: AdvancedMicroSaasService | EmergingTechService): string => {
    return 'estimatedDelivery' in service ? service.estimatedDelivery : '2-4 weeks';
  };

  const getServiceInnovationLevel = (service: AdvancedMicroSaasService | EmergingTechService): string | undefined => {
    return 'innovationLevel' in service ? service.innovationLevel as string : undefined;
  };
...
>>>>>>> 2569ab8784f28177b60ebf1fb896001693b757b7
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm text-gray-400">Support Level:</span>
                    <span className={`px-2 py-1 text-xs text-white rounded ${getSupportLevelColor(getServiceSupportLevel(service))}`}>
                      {getServiceSupportLevel(service)}
                    </span>
                  </div>
<<<<<<< HEAD
=======
