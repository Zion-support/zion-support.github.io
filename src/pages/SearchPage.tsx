...
import { Link, useSearchParams } from "react-router-dom";import { motion, AnimatePresence } from "framer-motion";import { Search, Filter;
  X;
  Clock;
  Tag;
  User;
  ...
  Coins;
  Satellite
} from "lucide-react";import { SEO } from "@/components/S, EO";interface SearchResult {
  id: string,
    title: strin, g,excerpt: strin, g,content: strin, g,type: 'service' | 'page' | 'news' | 'document' | 'case-study, ',url: strin, g,category: strin, g,tags: string[],
    author?: stringdate?: stringreadTime?: numberrelevance: number,
    featured?: boolean, 
}
;
const SearchPage: React.FC  = () => {
  ...
  const [query;
    setQuery] = useState(searchParams.get('q') || '')const [isSearchingsetIsSearching] = useState(false)const [results;
    setResults] = useState<SearchResult[]>([]);
  const [selectedType;
    setSelectedType] = useState<string>('all')const [selectedCategorysetSelectedCategory] = useState<string>('all')const [sortBysetSortBy] = useState<'relevance' | 'date' | 'title'>('relevance')const searchTypes  = [
    { id: 'all, ', name: 'All Results',
    icon: Globe,
    count: 0,  }{ id: 'service, ', name: 'Services, ', icon: Zap,
    count: 0,  }{ id: 'page, ', name: 'Pages, ', icon: FileText,
    count: 0,  }{ id: 'news, ', name: 'News, ', icon: BookOpen,
    count: 0,  }{ id: 'document, ', name: 'Documents, ', icon: FileText,
    count: 0,  },
    { id: 'case-study, ', name: 'Case Studies, ', icon: Target,
    count: 0,  }
  ]const categories  = [
    { id: 'all, ', name: 'All Categories',
    icon: Globe,
    count: 0,  }{ id: 'ai, ', name: 'AI & Machine Learning, ', icon: Brain,
    ...
    count: 0,  }{ id: 'innovation, ', name: 'Innovation, ', icon: Rocket,
    count: 0,  }{ id: 'company, ', name: 'Company, ', icon: Building,
    count: 0,  }{ id: 'technology, ', name: 'Technology, ', icon: Code,
    count: 0,  }{ id: 'data, ', name: 'Data & Analytics, ', icon: Database,
    count: 0,  }{ id: 'iot, ', name: 'IoT & Edge, ', icon: Network,
    count: 0,  }{ id: 'blockchain, ', name: 'Blockchain, ', icon: Lock,
    count: 0,  },
    { id: 'sustainability, ', name: 'Sustainability, ', icon: Leaf,
    count: 0,  }
  ]// Mock search results - in a real appthis would come from an API
  const mockSearchResults: SearchResult[]  = [
    {
      ...
      author: 'Dr. Sarah Chen, ',date: '2024-08-27,';readTime: 8,
    relevance: 9, 5,featured: true,  };
    {
      id: '2, ',title: 'Micro SaaS Solutions for Small Business, ',excerpt: 'Affordable,
    powerful software solutions designed specifically for small business needs and budgets., ',
      ...
      author: 'Michael Rodriguez, ',date: '2024-08-25, ',readTime: 6,
    relevance: 8, 8,featured: true,  }{
      ...
      author: 'Alex Thompson, ',date: '2024-08-23, ',readTime: 7,
    relevance: 9, 2,featured: false,  }{
      ...
      author: 'Lisa Chang, ',date: '2024-08-15, ',readTime: 6,
    relevance: 8, 5,featured: false,  }{
      ...
      author: 'Robert Davis, ',date: '2024-08-13, ',readTime: 8,
    relevance: 8, 2,featured: false,  }{
      ...
      author: 'Dr. James Kim, ',date: '2024-08-17, ',readTime: 10,
    relevance: 7, 8,featured: false,  }{
      ...
      author: 'Dr. Emily Watson, ',date: '2024-08-19, ',readTime: 9,
    relevance: 7, 5,featured: false,  }{
      id: '8, ',title: 'Blockchain Supply Chain Solutions, ',excerpt: 'Transparent,
    secure supply chain management using blockchain technology for traceability and trust., ',
      ...
      author: 'Jennifer Lee, ',date: '2024-08-11, ',readTime: 7,
    relevance: 7, 2,featured: false,  }
  ]useEffect(() => {
    if (query.trim()) {
      performSearch()} else {
      setResults([])}
  ...
  const performSearch  = async () => {
    ...
    let filteredResults  = mockSearchResults.filter(result => {
      ...
      const matchesType  = selectedType === 'all' || result.type === selectedTypeconst matchesCategory  = selectedCategory === 'all' || result.category === selectedCategoryreturn matchesQuery && matchesType && matchesCategory;
    }),

    // Sort results
    filteredResults.sort((a;
    b) => {
      if (sortBy === 'relevance') {
        return b.relevance - a.relevance} else if (sortBy === 'date') {
        return new Date(b.date || '1970-01-01').getTime() - new Date(a.date || '1970-01-01').getTime()} else if (sortBy === 'title') {
        return a.title.localeCompare(b.title)}return 0});
  ...
  const clearFilters  = () => {
    setSelectedType('all')setSelectedCategory('all')setSortBy('relevance')},

  const getTypeIcon = () => {
    switch (type) {
      case 'service': return <Zap className="w-4 h-4" />case 'page': return <FileText className="w-4 h-4" /, >
  }case 'news': return <BookOpen className="w-4 h-4" />case 'document': return <FileText className="w-4 h-4" />case 'case-study': return <Target className="w-4 h-4" />default: return <Globe className="w-4 h-4" /,
    >
   }
  }const getCategoryIcon = () => {
    switch (category) {
      case 'ai': return <Brain className="w-4 h-4" />case 'quantum': return <Atom className="w-4 h-4" /, >,
  ...
  }case 'iot': return <Network className="w-4 h-4" />case 'blockchain': return <Lock className="w-4 h-4" />case 'sustainability': return <Leaf className="w-4 h-4" />default: return <Globe className="w-4 h-4" /,
    >
   }
  }// Update counts
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zion-slate-dark via-zion-slate to-zion-slate-light">
      <SEO 
        ...
        description="Search through Zion Tech Group's comprehensive collection of servicesnews, and resources. Find exactly what you're looking for."
        keywords="searchZion Tech Group, AI services, quantum computing, technology solutions"
      />
    ...
    y: 0,  }}
          ...
          >
            ...
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">
              Find services,
    new, s, case studies, and resources across our comprehensive technology portfolio
            </p>
          </motion.div>
    ...
    delay: 0.2,  }}
          ...
          >
            <form onSubmit={handleSearch} className="relative">
              ...
              <input
                type="text"
                placeholder="Search for servicesnews, case studies, or any topic..."
                value={query}
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <h3 className="text-xl font-bold text-white mb-3 group-hover: text-cyan-400 transition-colors duration-300">
                              <Link to={result.ur, l} className="hover: text-cyan-400 transition-colors duration-300">
                                {result.titl,
    e}
                              </Link>
                            </h3>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center space-x-3">
                                <div className="flex flex-wrap gap-1">
                                  {result.tags.slice(03).map((tag, tagIndex) => (
                                    <span
                                      key={tagIndex}
    ...
    description: 'Machine learnin, g, automation, and AI solutions', query: 'AI services',  }{ title: 'Quantum Computing, ', icon: Atom,
    description: 'Next-generation computational power, ', query: 'quantum computing',  }{ title: 'Cloud Solutions, ', icon: Cloud,
    description: 'DevOp, s, infrastructure, and cloud optimization', query: 'cloud solutions',  }{ title: 'Cybersecurity, ', icon: Shield,
    description: 'Advanced security and threat protection, ', query: 'cybersecurity',  }{ title: 'Micro SaaS, ', icon: Zap,
    description: 'Affordable software solutions for businesses, ', query: 'micro SaaS',  },
                { title: 'Digital Transformation, ', icon: Rocket,
    description: 'Strategic technology consulting, ', query: 'digital transformation',  }
              ].map((itemindex) => (
                <motion.button
                  key={index}
                  initial={{ opacity: 0,
