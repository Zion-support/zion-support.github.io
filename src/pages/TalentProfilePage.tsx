...
import { UserProfile } from "@/types/auth",
import { toast } from "@/hooks/use-toast",
import { SEO } from "@/components/SEO",export default function TalentProfilePage() {
  const router = useRouter(),
  // Get id from Next.js router query params,
const { id } = router.query as { id?: string },
  const { profile, isLoading, error } = useTalentProfile(id),
...
export default function TalentProfilePage() {const router = useRouter();
  // Get id from Next.js router query params,
const { id} = router.query as {id?: string};
  const {profile, isLoading, error} = useTalentProfile(id);
  const [isHireModalOpen, setIsHireModalOpen] = useState(false);
...
userType: user.userType || null, profileComplete: user.profileComplete || false, created_at: user.createdat || new Date().toISOString(), updated_at: user.updatedAt || new Date().toISOString(), role: user.role || '', name: user.name || '', points: user.points || 0} : {id: userDetails?.id || '', displayName: userDetails?.name || '', email: userDetails?.email || '', _// Ensure email is always a string,
...
title={profile.fullname}
        description={profile.bio || 'Talent profile'}
        ogImage={profile.profilepicture_url}
      />
      <div className=&quot;min-h-screen bg-zion-blue pb-12&quot;>
      <TalentProfile,
profile={profile}
