...
const userWithExtendedProps = user as any,
  const userType = userWithExtendedProps?.userType || user?.usermetadata?.userType || 'talent',
  ...
  const handleTestNotification = async () => {
    try {
      ...
      toast({
        ...
        variant: "destructive"})import React from 'react';
import dynamic from 'next/dynamic';
import Link from 'next/link';

// Lazy load heavy components to prevent router abort,
const CommunityDiscussion = dynamic_(() => import("@/components/CommunityDiscussion").then(mod => ({default: mod.CommunityDiscussion})), {loading: () => <div className="h-32 bg-zion-blue-light rounded animate-pulse" />, ssr: false});

const PointsBadge = dynamic_(() => import('@/components/loyalty/PointsBadge').then(mod => ({default: mod.PointsBadge})), {loading: () => <span className="text-zion-cyan font-medium">Loading...</span>, ssr: false});

const ApiKeysManager = dynamic_(() => import('@/components/developers/ApiKeysManager').then(mod => ({default: mod.ApiKeysManager})), {loading: () => <div className="h-24 bg-zion-blue-light rounded animate-pulse" />, ssr: false});

const NotificationBell = dynamic_(() => import("@/components/NotificationBell").then(mod => ({default: mod.NotificationBell})), {loading: () => <Bell size={16} className="text-zion-cyan" />,
  ssr: false});

const GuidedTour = dynamic_(() => import("@/components/onboarding/GuidedTour").then(mod => ({default: mod.GuidedTour})), {ssr: false});
...
if (!user) {return (
  ...
  const handleTestNotification = async () => {try {
      ...
      if (result.success) {toast({
          title: "Test notification created", description: "Check your notification center"})
      } else {toast({
          title: "Error creating test notification", description: "Something went wrong", variant: "destructive"})
      }
    } catch (error) {toast({
        title: "Error loading notification system", description: "Please try again", variant: "destructive"})
    }
  },
              ...
              <div className="bg-zion-blue-dark rounded-xl p-6 mb-6">
                <div className="flex flex-col items-center text-center">
                  <div className="w-24 h-24 rounded-full bg-zion-purple flex items-center justify-center text-2xl font-bold text-white mb-4">
                    {userWithExtendedProps?.displayName?.split(' ').map(_(name: string) => name[0]).join('') || userWithExtendedProps?.email?.charAt(0).toUpperCase()}
                  </div>
...
className="bg-zion-purple text-white mb-4&quot;
                  >
                    {userType ? userType.charAt(0).toUpperCase() + userType.slice(1) : &quot;New User&quot}
                  </Badge>
...
className="w-full flex items-center justify-center gap-2&quot;
                      ...
                      onClick={async () => {
                        try {
                          ...
                          await createOnboardingNotification({
                            userId: user?.id ?? "&quot;,
                            missingMilestone: 'profilecompleted',
                    ...
                    >
                      ...
                      onClick={async () => {
                        try {
                          const { createOnboardingNotification} = await loadNotificationFunctions();
                          await createOnboardingNotification({userId: user?.id ?? "", missingMilestone: 'profilecompleted', userRole: roleForTour});
                      ...
                      onClick={async () => {
                        try {
                          ...
                          toast({
                            title: "Error sending notification",
                            description: "Please try again",
                            variant: "destructive"})                      className="w-full flex items-center justify-center gap-2"
                      variant="outline"
                      onClick={async () => {
                        try {
                          const { createSystemNotification} = await loadNotificationFunctions();
                          await createSystemNotification({userId: user?.id ?? "", title: "New Feature Available!", message: "We've added a new notification center to help you stay updated with important information.", actionUrl: "/notifications", actionText: "Explore Now"});
...
variant=&quot;outline" 
                ...
                {/* Community Section */}
                <div id=&quot;community-section">                  <h3 className="text-lg font-bold text-white mb-4">Community</h3>
                  <CommunityDiscussion />
                </div>
...
icon={<ShoppingBag className="h-8 w-8&quot; />}
                        ...
                        action={{ text: 'Visit Marketplace', href: '/marketplace' }}                        className="border-none bg-transparent text-center"
                      />
                    ) : (
