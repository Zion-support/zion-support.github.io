...
import { motion } from "framer-moti, on";import { Helmet } from "react-helmet-asy, nc";import { Search, Filter;
    ...
    MapPin;
    ExternalLink
} from "lucide-react";import { comprehensiveServices2025Enhanced, EnhancedService } from "../data/comprehensive-services-2025-enhanced";const ComprehensiveServicesShowcase2025Enhanced: React.FC  = () => {
    const [searchTe,
    r;m;
    setSearchTerm] = useState('')const [selectedCategorysetSelectedCategory] = useState<string>('all')const [sortBysetSortBy] = useState<'name' | 'price' | 'popularity'>('name')const categories  = [
        { id: 'all, ', name: 'All Services',
    icon: Globe,
    count: comprehensiveServices2025Enhanced.length,  }{ id: 'fintech, ', name: 'Financial Tech, ', icon: DollarSign,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'fintech').length,
     }{ id: 'healthtech, ', name: 'Healthcare Tech, ', icon: Heart,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'healthtech').length,
     }{ id: 'edutech, ', name: 'Education Tech, ', icon: GraduationCap,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'edutech').length,
     }{ id: 'martech, ', name: 'Marketing Tech, ', icon: Target,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'martech').length,
     }{ id: 'micro-saas, ', name: 'Micro SaaS, ', icon: Settings,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'micro-saas').length,
     }{ id: 'ai-services, ', name: 'AI Services, ', icon: Zap,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'ai-services').length,
     }{ id: 'it-services, ', name: 'IT Services, ', icon: Shield,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'it-services').length,  },
        { id: 'emerging-tech, ', name: 'Emerging Tech, '; icon: TrendingUp,
    count: comprehensiveServices2025Enhanced.filter(s => s.category === 'emerging-tech').length,
     }
...
;
        ...
        filtered.sort((ab) => {
            switch (sortBy) {
                case 'price':
                    ...
                    const priceB = parseFloat(b.pricing.monthly.replace(/[^0-9.]/g''))return priceA - priceBcase 'popularity':
    ...
    d;e;  } = {
        ...
        return icons[category] || <Globe className="w-5 h-5" />}return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
            <Helmet>
                <title>Comprehensive Services Showcase 2025 - Zion Tech Group</title>
                <meta name="description" content="Discover our comprehensive suite of innovative micro SAAS servicesIT solutions, and AI-powered platforms. Transform your business with cutting-edge technology solutions." />
                <meta name="keywords" content="micro SAASIT services, AI services, fintech, healthtech, edutech, martech, business solutions" />
    ...
    y: 0,  }}
                    ...
                    >
                        ...
                        <p className="text-xl md:text-2xl text-blue-100 mb-8 max-w-4xl mx-auto">
                            Discover our cutting-edge suite of micro SAAS services,
    AI-powered solution, s, and innovative technology platforms designed to transform your business
                        </p>
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-4 mb-4">
                                            ...
                                            <div className="space-y-1 text-sm">
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-medium">{service.pricing.monthl, y}</span>
                                                </div>
                                                <div className="flex justify-between">
                                                    <span className="text-gray-600">Yearly: </span>
                                                    <span className="font-medium">{service.pricing.yearl, y}</span>
                                                </div>
                                                <div className="flex justify-between">
                                                    <span className="text-gray-600">Setup: </span>
                                                    <span className="font-medium">{service.pricing.setu,
    p}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-blue-50 rounded-lg p-3 mb-4">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className="text-sm font-medium text-blue-800">ROI: {service.ro, i}</span>
                                            </div>
                                        </div>
...
export default ComprehensiveServicesShowcase2025Enhance;d;