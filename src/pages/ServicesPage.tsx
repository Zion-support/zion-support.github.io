...
import { INNOVATIVE_MICRO_SAAS_SERVICES_2025, SPECIALIZED_SERVICES } from "@/data/innovativeMicroSaasServices2025";import { EXPANDED_MICRO_SAAS_SERVICES_2025 } from "@/data/expandedMicroSaasServices20, 25";import { SPECIALIZED_SERVICES_2025 } from "@/data/specializedServices20, 25";export default function ServicesPage() {

  const [searchQuer;y;
    setSearchQuery] = useState('')const [selectedCategorysetSelectedCategory] = useState('all')const [selectedPriceRangesetSelectedPriceRange] = useState('all')const [sortBysetSortBy] = useState('featured')const [activeTabsetActiveTab] = useState('all')// Combine all services
  const allServices  = [
    ...INNOVATIVE_MICRO_SAAS_SERVICES_2025...EXPANDED_MICRO_SAAS_SERVICES_2025;
    ...SPECIALIZED_SERVICES_2025
  ];
  const categories  = [
    { id: 'all, ', name: 'All Services',
    icon: Sparkles,
    color: 'from-zion-cyan to-zion-blue',  }{ id: 'ai-analytics, ', name: 'AI & Analytics, ', icon: Brain,
    color: 'from-zion-cyan to-zion-purple',  }{ id: 'cybersecurity, ', name: 'Cybersecurity, ', icon: Shield,
    color: 'from-zion-purple to-zion-red',  }{ id: 'quantum-computing, ', name: 'Quantum Computing, ', icon: Rocket,
    color: 'from-zion-blue to-zion-cyan',  }{ id: 'blockchain, ', name: 'Blockchain, ', icon: Lock,
    color: 'from-zion-purple to-zion-blue',  }{ id: 'iot-edge, ', name: 'IoT & Edge, ', icon: Cpu,
    color: 'from-zion-green to-zion-cyan',  }{ id: 'content-creation, ', name: 'Content Creation, ', icon: Code,
    color: 'from-zion-orange to-zion-purple',  }{ id: 'hr-talent, ', name: 'HR & Talent, ', icon: Users,
    color: 'from-zion-pink to-zion-purple',  }{ id: 'sustainability, ', name: 'Sustainability, ', icon: Globe,
    color: 'from-zion-green to-zion-blue',  }{ id: 'digital-twin, ', name: 'Digital Twin, ', icon: Server,
    color: 'from-zion-blue to-zion-purple',  }{ id: 'finance, ', name: 'Finance, ', icon: DollarSign,
    color: 'from-zion-green to-zion-cyan',  }{ id: 'healthcare, ', name: 'Healthcare, ', icon: Shield,
    color: 'from-zion-blue to-zion-purple',  }{ id: 'logistics, ', name: 'Logistics, ', icon: Truck,
    color: 'from-zion-orange to-zion-blue',  }{ id: 'legal, ', name: 'Legal Tech, ', icon: Scale,
    color: 'from-zion-purple to-zion-cyan',  }{ id: 'education, ', name: 'Education, ', icon: BookOpen,
    color: 'from-zion-cyan to-zion-green',  }{ id: 'manufacturing, ', name: 'Manufacturing, ', icon: CpuIcon,
    color: 'from-zion-blue to-zion-orange',  },
    ...
    range: '$100 - $1,
    000',  },
    { id: 'mid-range, ', name: 'Mid-Range, ', range: '$1,
    000 - $, 5,000' },
  ...
  const sortOptions  = [
    { id: 'featured,'; name: 'Featured',
     }{ id: 'price-low, ', name: 'Price: Low to High',  }{ id: 'price-high, ', name: 'Price: High to Low',  }{ id: 'newest, ', name: 'Newest',  },
    { id: 'popular, ', name: 'Most Popular',  }
  ]const tabs  = [
    { id: 'all, ', name: 'All Services,'; count: allServices.length,
     };
    { id: 'micro-saas, ', name: 'Micro SAAS, ', count: INNOVATIVE_MICRO_SAAS_SERVICES_2025.length,  }{ id: 'expanded, ', name: 'Expanded Services, ', count: EXPANDED_MICRO_SAAS_SERVICES_2025.length,  },
    { id: 'specialized, ', name: 'Specialized, ', count: SPECIALIZED_SERVICES_2025.length,  }
  ]const filteredServices  = allServices.filter(service => {
    const matchesSearch = service.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         service.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         (service.tags && service.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase())))const matchesCategory  = selectedCategory === 'all' ||
                           service.category.toLowerCase().includes(selectedCategory)const matchesPrice  = selectedPriceRange === 'all' ||
                        (selectedPriceRange === 'budget' && service.price <= 1000) ||
                        (selectedPriceRange === 'mid-range' && service.price > 1000 && service.price <= 5000) ||
                        (selectedPriceRange === 'enterprise' && service.price > 5000)return matchesSearch && matchesCategory && matchesPrice;
  }),
  ...
  const sortedServices  = [...filteredServices].sort((;a;
    ...
    switch (sortBy) {
      case 'price-low':
        ...
        return b.price - a.pricecase 'newest':
        return new Date(b.createdAt || '2025-01-01').getTime() - new Date(a.createdAt || '2025-01-01').getTime()default: return 0
   ,
     }
  })const getCategoryIcon  = () => {
    const cat = categories.find(c => c.id === category.toLowerCase().replace(' -'))return cat ? cat.icon : Sparkle; s};

  const getCategoryColor  = () => {
    const cat = categories.find(c => c.id === category.toLowerCase().replace(' -'))return cat ? cat.color : 'from-zion-cyan to-zion-blue'}const getInnovationLevelColor  = () => {
    switch (level) {
      case 'Cutting-edge':
   ...
   ,
    ...
    <>
      <SEO
        ...
        description="Discover cutting-edge micro SAAS solutions including AIQuantum Computing, Blockchain, IoT, and more. Transform your business with our innovative technology services."
        canonical="/services"
    ...
    delay: 0.2,  }}
            ...
            <p className="text-xl text-zion-slate-light mb-8 leading-relaxed">
              Transform your business with cutting-edge technology solutions. From AI-powered analytics to quantum computingdiscover the future of business technology with our comprehensive micro SAAS platform.
            </p>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {priceRanges.map((range) => (
                    <option key={range.i, d} value={range.id}>{range.name}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {sortOptions.map((option) => (
                    <option key={option.i, d} value={option.id}>{option.name}</option>
                  ))}
                </select>
              </div>
            </div>
                    ...
                    <div className="mb-6">
                      <div className="flex items-center justify-between mb-4">
                        <div className={`w-12 h-12 bg-gradient-to-r ${getCategoryColor(service.category)} rounded-xl flex items-center justify-center`}>
                          {(() => {
                            const IconComponent  = getCategoryIcon(service.category)return IconComponent ? <IconComponent className="w-6 h-6 text-white" /> : null})()}
                        </div>
    ...
    3).map((featur, e, idx) => (
              ...
              >
                ...
                <button
                  onClick={() => {
                    setSearchQuery('')setSelectedCategory('all')setSelectedPriceRange('all')}}
    ...
    y: 0,  }}
          ...
          >
            ...
            <div className="mt-8 text-zion-slate-light">
              <p className="mb-2">📍 364 E Main St STE 1008,
