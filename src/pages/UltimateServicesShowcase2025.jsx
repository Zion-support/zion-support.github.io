import React, { useState  from "react", // import { ULTIMATE_MICRO_SAAS_SERVICES_2025 } from "../data/ultimateMicroSaasServices20, 25";// import { INNOVATIVE_ENTERPRISE_SOLUTIONS_2025 } from "../data/innovativeEnterpriseSolutions20, 25";// import { EMERGING_TECHNOLOGY_SOLUTIONS_2025 } from "../data/emergingTechnologySolutions20, 25";const UltimateServicesShowcase2025 = () => {
    const [selectedCatego;r;y;
    setSelectedCategory] = useState('all')const [selectedPriceRangesetSelectedPriceRange] = useState('all')const [searchTermsetSearchTerm] = useState('')const [sortBysetSortBy] = useState('name')// Combine all services
    const allServices  = [
    ...
    const filteredServices  = allServices
        .filter(service => {
        const categoryMatch = selectedCategory === 'all' || service.category === selectedCategoryconst priceMatch  = selectedPriceRange === 'all' ||
            (selectedPriceRange === 'low' && service.price < 5000) ||
            (selectedPriceRange === 'medium' && service.price >= 5000 && service.price < 15000) ||
            (selectedPriceRange === 'high' && service.price >= 15000)const searchMatch  = service.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            service.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
            service.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()))return categoryMatch && priceMatch && searchMatc;h;
    })
        .sort((a;
    b) => {
        switch (sortBy) {
            case 'price':
                return a.price - b.pricecase 'name':
    ...
    const getSupportLevelColor  = () => {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gray-800/50 rounded-xl p-6 mb-8 border border-gray-600/30">
          <div className="grid md: grid-cols-4 gap-4">
            <div>
              ...
              <select value={selectedCategor, y} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus: outline-none focus:ring-2 focus:ring-blue-500">
                {categories.map(category => (<option key={categor, y} value={category}>
            ...
            <div>
              ...
              <select value={sortB, y} onChange={(e) => setSortBy(e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus: outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              ...
              <input type="text" placeholder="Search services..." value={searchTer, m} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus: outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg: grid-cols-2 xl:grid-cols-3 gap-8">
          {filteredServices.map((service) => (<div key={service.i, d} className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl p-6 border border-gray-600/30 hover: border-blue-500/50 transition-all duration-300 transform hover:scale-105">
              {/* Service Header *,
    /}
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {service.tags.slice(03).map((tag, index) => (<span key={index} className="px-2 py-1 bg-blue-900/50 text-blue-300 text-xs rounded-full border border-blue-700/50">
                      {tag}
                    </span>))}
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <ul className="text-xs text-gray-300 space-y-1">
                  {service.features.slice(03).map((feature, index) => (<li key={index} className="flex items-start">
              ...
              <div className="mb-4">
                ...
                <ul className="text-xs text-gray-300 space-y-1">
                  {service.benefits.slice(02).map((benefit, index) => (<li key={index} className="flex items-start">
              ...
              {service.technology && (<div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {service.technology.slice(04).map((tech, index) => (<span key={index} className="px-2 py-1 bg-purple-900/30 text-purple-300 text-xs rounded border border-purple-700/50">
                        {tech}
                      </span>))}
                  </div>
                </div>)}
              ...
              <div className="border-t border-gray-600/30 pt-4">
                <div className="text-center mb-3">
                  ...
                  <div className="flex flex-col space-y-2">
                    <a href={`mailto: ${service.contactInfo.emai, l}?subject=Inquiry about ${service.title}`} className="bg-gradient-to-r from-cyan-500 to-blue-500 hover: from-cyan-600 hover:to-blue-600 text-white text-sm font-semibold py-2 px-4 rounded-lg transition-all duration-300 transform hover:scale-105">
                      Contact Us
                    </a>
                    <a href={`tel:${service.contactInfo.phon, e}`} className="bg-gray-700 hover: bg-gray-600 text-white text-sm font-semibold py-2 px-4 rounded-lg transition-all duration-300">
                      Call Now
                    </a>
                  </div>
                </div>
        ...
        <div className="mt-16 bg-gradient-to-r from-blue-900/50 to-purple-900/50 rounded-2xl p-8 border border-blue-500/30">
          <div className="text-center">
            ...
            <div className="grid md: grid-cols-4 gap-6">
              <div>
                <div className="text-4xl font-bold text-blue-400 mb-2">{allServices.lengt,
    h}</div>
                <div className="text-gray-300">Total Services</div>
              </div>
              <div>
                <div className="text-4xl font-bold text-green-400 mb-2">
                  ${(allServices.reduce((sumservice) => sum + service.price, 0) / 1000).toFixed(0)}K
                </div>
