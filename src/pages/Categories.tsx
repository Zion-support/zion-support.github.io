...
const Categories: React.FC = () => {'
  ...
  return("'"'"
    <div className='min-h-screen bg-gradient-to-br from-slate-50 to-blue-50'></div>"'"'"
      <div className='max-w-6xl mx-auto px-4 sm: px-6 lg:px-8 py-16'></div>"'"'"
        <div className='text-center mb-12'></div>"'"'"
          <h1 className='text-4xl font-bold text-gray-900 mb-4'></h>Service Categories</h1>"'"'"
          <p className='text-xl text-gray-600'>Explore our comprehensive range of technology services</p>""'"
        </div>"''"
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'></div>"'"
          {categories.map((category, index) => ("'"'"
            <div key={index} className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover: shadow-md transition-shadow'></div>"'"'"
              <div className='flex items-center mb-4'></div>"'"'"
                <category .icon className='h-8 w-8 text-blue-600 mr-3' /" ></category>"'"'"
                <h2 className='text-xl font-semibold text-gray-900'></h>{category.name}</h2>""'"
              </div>"''
              ...
              <a"'"'"
                ...
                className='text-blue-600 hover: text-blue-700 font-medium'></a>
                View Services →
              </a>
            </div>
          ))}
