...
interface CategoryType  {id: string;
  ...
  slug: string;
  icon: string;
      logErrorToProduction ('Categories API error:', { data: response.status_text }),return CATEGORIES as CategoryType[];
    fallback_data: initial_categories}),const categories = data || [];
...
}</p> <p>Please try again later.</p> </div>)}{categories.map ( (category) => {//Use default folder icon for all categories to avoid large bundle;
}/>)})}</div>)}</ErrorBoundary> </div> </div> </>)}'";
}
      ...
      <div className="max-w-6xl mx-auto px-4 sm: px-6 lg:px-8 py-16">,";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> {categories.map((category, index) => (";
            <div key="{index}" className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover: shadow-md transition-shadow">,";
              ...
              <p className="text-gray-600 mb-4"> {category.count} services available in this category;
import { Brain, Cloud, Shield, Code, Database, Network  } from 'lucide-react';
const Categories: React.FC = () => {const categories = [;
  ...
  return(<div className='min-h-screen bg-gradient-to-br from-slate-50 to-blue-50'>;
      <div className='max-w-6xl mx-auto px-4 sm: px-6 lg:px-8 py-16'>;
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>;
          {categories.map((category, index) => (<div key={index} className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover: shadow-md transition-shadow'>;
              <div className='flex items-center mb-4'>;
                <category.icon className='h-8 w-8 text-blue-600 mr-3' />;
                <h2 className='text-xl font-semibold text-gray-900'>{category.name}</h2>;
              </div>"";
...
";
  )}import useSWR from 'swr',import { CategoryCard } from "@/components/CategoryCard",import { GradientHeading } from "@/components/GradientHeading",import { SkeletonCard } from '@/components/ui',import ErrorBoundary from "@/components/GlobalErrorBoundary",import { CATEGORIES } from '@/data/categories',import { NextSeo } from '@/components/NextSeo',import {logErrorToProduction} from '@/utils/productionLogger',interface CategoryType  {id: string,name: string,slug: string,icon: string;
}const fetcher = async (url: string): Promise<CategoryType[]> => {try {const response = await fetch(url),if (!response.ok) {logErrorToProduction('Categories API error:', { data: response.statusText }),return CATEGORIES as CategoryType[];
    }
    const data = await response.json(),return Array.isArray(data) && data.length > 0 ? data : CATEGORIES as CategoryType[];
  } catch (err) {logErrorToProduction('Categories API fetch failed:', { data: err }),return CATEGORIES as CategoryType[];
  }
},export interface CategoriesProps  {categories?: CategoryType[];
}export default function Categories() {const { data, error } = useSWR<CategoryType[]>('/api/categories', fetcher, {fallbackData: initialCategories}),const categories = data || [],const isLoading = !data && !error,return (<>;
      <NextSeo;
      ...
      <div className="min-h-screen bg-zion-blue">;
        ...
        <ErrorBoundary>;
          {isLoading && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" data-testid="skeleton-loader">;
              {Array.from({ length: 4 }).map((_, index) => (<SkeletonCard key={index} />;
              ))}
            </div>;
          )}
          {error && (<div className="text-center text-red-500 py-8">;
              <p>Error loading categories: {error.message}</p>;
              <p>Please try again later.</p>;
            </div>;
          )}
          {!isLoading && !error && categories.length === 0 && (<div className="text-center text-zion-slate-light py-8">;
              <p>No categories yet</p>;
            </div>;
          )}
          {!isLoading && !error && categories.length > 0 && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">;
              {categories.map((category) => {// Use default folder icon for all categories to avoid large bundle;
                return (<CategoryCard;
                    key={category.id}
                    title={category.name}
