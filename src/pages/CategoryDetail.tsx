...
import ListingGridSkeleton from '@/components/skeletons/ListingGridSkeleton',
import {logErrorToProduction} from '@/utils/productionLogger',
const AUTOSERVICE_TITLES = [  "AI-Powered Customer Support",
  "Cloud Infrastructure Management",
  "Predictive Analytics Consulting",
  "Cybersecurity Automation Suite",
  "Robotic Process Automation",
...
const AUTOSERVICE_TITLES = [
  ...
  &quot;Machine Learning Model Tuning&quot;,
  &quot;IoT Device Integration Service&quot;,
  &quot;Blockchain Data Solutions&quot
];

function generateInnovationListing(index: number): ProductListing {
  const title = AUTOSERVICE_TITLES[index % AUTOSERVICE_TITLES.length] || 'AI Service',
  const price = Math.floor(Math.random() * 9500) + 500, // $500 - $10,000,
const rating = Math.floor(Math.random() * 2) + 4, // 4-5 stars,
const reviewCount = Math.floor(Math.random() * 50) + 10,
  return {
    ...
    price,
    currency: &quot;$&quot;,
    tags: [&quot;innovation&quot;, &quot;ai&quot;, &quot;service&quot],
    author: {
      name: &quot;AutoGen Solutions&quot;,
      id: &quot;autogen&quot
    },
    images: [&quot;https://source.unsplash.com/random/800x500?technology&quot],    createdAt: new Date().toISOString(),
    rating,
    reviewCount,
    location: &quot;Global&quot;,
    availability: &quot;Immediate&quot;,
...
const categoryData = {
  ...
  useEffect(() => {
    async function load() {
      setIsLoading(true),
interface CategoryDetailProps {slug?: string}

export default function CategoryDetail(_{slug: slugProp}: CategoryDetailProps = {}) {const router = useRouter();
  // Get slug from Next.js router query params,
const params = router.query as { slug?: string};
  const slug = slugProp ?? params.slug;

  // Redirect to categories list if slug is missing,
if (!slug) {router.push('/categories');
    return null}
  const [isLoading, setIsLoading] = useState(true);
  const [listings, setListings] = useState(MARKETPLACELISTINGS);
...
const currentCategory = categoryData[slug as keyof typeof categoryData] || {
          title: slug
            ...
            .join(' ') || 'Category',
          description: 'Explore our collection in this category',
          icon: <Bot className="w-6 h-6" />},            .map(_(word) => word.charAt(0).toUpperCase() + word.slice(1))
            .join(' ') || 'Category', description: 'Explore our collection in this category', icon: <Bot className="w-6 h-6" />};
    ...
    <>
      ...
      <Suspense fallback={<ListingGridSkeleton />}> 
        <div className=&quot;min-h-screen bg-zion-blue&quot;>
          <div className=&quot;container mx-auto px-4 py-12&quot;>
          <div className=&quot;mb-4&quot;>
            <Link href=&quot;/categories&quot; className=&quot;text-zion-cyan hover:text-zion-cyan-light transition-colors inline-flex items-center&quot;>              ← Back to Categories
            </Link>
          </div>
