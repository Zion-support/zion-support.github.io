...
import { Button } from "@/components/ui/button",import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card",import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs",import { useState, useEffect } from "react",import { useTranslation } from 'react-i18next',useEffect(() => {async function checkHealth() {try {const res = await fetch('/api/auth/health')setAuthServiceAvailable(res.ok)} catch (err) {import { useTranslation   } from 'react-i18next';
import Link from 'next/link',import { PartnerRegistrationForm } from "@/components/partners/PartnerRegistrationForm",import { PartnerReferralLinks } from "@/components/partners/PartnerReferralLinks",import { PartnerDashboard } from "@/components/partners/PartnerDashboard",import { PartnerLeaderboard } from "@/components/partners/PartnerLeaderboard",import { PartnerResources } from "@/components/partners/PartnerResources",import { useAuth } from "@/hooks/useAuth",import { useRouter } from 'next/router',import { logInfo, logErrorToProduction } from '@/utils/productionLogger',import { useRouter   } from 'next/router';
export default function Partners() {export default function Partners() {ursor/fix-website-loading-errors-and-merge-6662;
xport default function Partners() {logInfo('PartnersPage rendering')const [activeTab, setActiveTab] = useState('overview')const { t } = useTranslation()const { user, isAuthenticated }  = useAuth()const router = useRouter()const [authServiceAvailable, setAuthServiceAvailable] = useState(true)import { Button  } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle  } from '@/components/ui/card';
  const router = null;
        logErrorToProduction('Partner login auth health check failed', {data: err;
        })setAuthServiceAvailable(false)}
    }
    checkHealth()}, [])// If not authenticated, display partner program info and signup CTA;
  if (!isAuthenticated) {logInfo('PartnersPage rendering Unauthenticated View'),return (<CardHeader>;
...
<Card className='bg-zion-blue-dark border-zion-blue-light'>;
            <CardHeader>;
              ...
              <CardDescription>{t('partner.organizations.desc')}
              </CardDescription>;
            </CardHeader>;
            <CardContent className='space-y-4'>;
              <div className='flex items-start gap-3'>;
...
export default function Partners() {logInfo('PartnersPage rendering'),const [activeTab, setActiveTab] = useState("overview"),const { t } = useTranslation(),const { user, isAuthenticated } = useAuth(),const router = useRouter(),const [authServiceAvailable, setAuthServiceAvailable] = useState(true),useEffect(() => {async function checkHealth() {try {const res = await fetch('/api/auth/health'),setAuthServiceAvailable(res.ok)} catch (err) {logErrorToProduction('Partner login auth health check failed', { data: err }),setAuthServiceAvailable(false)}
    }
    checkHealth()}, []),// If not authenticated, display partner program info and signup CTA;
  if (!isAuthenticated) {logInfo('PartnersPage rendering Unauthenticated View')return (<div className="container max-w-6xl py-10">;
        <div className="text-center mb-8">;
        ...
        <div className="flex justify-center gap-4">;
          ...
          <Button;
            size="lg";
            variant="outline";
            className="text-zion-cyan border-zion-cyan";<div className='flex justify-center gap-4'>;
          <Button;
            size='lg';
            className='bg-zion-purple hover:bg-zion-purple-dark text-white';
            asChild;
          ...
          {!authServiceAvailable && (<p className="text-red-500 text-sm mt-2">{t('partner.login_unavailable')}</p>;
<p className='text-red-500 text-sm mt-2'>;
  ...
  logInfo('PartnersPage rendering Authenticated View. User:', { data: user }),return (<h1>DEBUG: Partners Page - Authenticated View</h1>;
  ...
  logInfo('PartnersPage rendering Authenticated View. User:', { data: user })return (<div className='container max-w-7xl py-10'>;
      ...
      >;
        ...
        <TabsContent value='earnings' className='space-y-4'>;
          <Card>;
            ...
            <CardContent>;
              {/* This will be implemented later */}<p className="text-zion-slate-light">{t('partner.earnings_placeholder')}</p>;
            </CardContent>;
          </Card>;
        </TabsContent>;
...
<p className='text-zion-slate-light'>;
...
}checkHealth ()}, [])//If not authenticated, display partner program info and signup CTA </div> <div className="grid md:grid-cols-2 gap-8 mb-12" > <Card className="bg-zion-blue-dark border-zion-blue-light" > <CardHeader> </CardHeader> <CardContent className="space-y-4" > <div className="flex items-start gap-3" > <CheckCircle className="h-5 w-5 text-zion-cyan mt-0.5" /> <div> </div> </div> <div className="flex items-start gap-3" > <CheckCircle className="h-5 w-5 text-zion-cyan mt-0.5" /> <div> </div> </div> <div className="flex items-start gap-3" > <CheckCircle className="h-5 w-5 text-zion-cyan mt-0.5" /> <div> </div> </div> </CardContent> </Card> <Card className="bg-zion-blue-dark border-zion-blue-light" > <CardHeader> </CardHeader> <CardContent className="space-y-4" > <div className="flex items-start gap-3" > <CheckCircle className="h-5 w-5 text-zion-purple mt-0.5" /> <div> </div> </div> <div className="flex items-start gap-3" > <CheckCircle className="h-5 w-5 text-zion-purple mt-0.5" /> <div> </div> </div> <div className="flex items-start gap-3" > <CheckCircle className="h-5 w-5 text-zion-purple mt-0.5" /> <div> </div> </div> </CardContent> </Card> </div> </CardContent> </Card> <Card className="bg-zion-blue-dark border-zion-blue-light" > <CardHeader className="text-center pb-2" > <div className="mx-auto bg-zion-blue-light rounded-full w-12 h-12 flex items-center justify-center mb-4" > <FileText className="h-6 w-6 text-zion-cyan" /> </div> </CardContent> </Card> <Card className="bg-zion-blue-dark border-zion-blue-light" > <CardHeader className="text-center pb-2" > <div className="mx-auto bg-zion-blue-light rounded-full w-12 h-12 flex items-center justify-center mb-4" > <PieChart className="h-6 w-6 text-zion-cyan" /> </div> </CardContent> </Card> </div> </div> <div className="flex justify-center gap-4" > <Button size="lg" className="bg-zion-purple hover:bg-zion-purple-dark text-white" asChild > > {t ('partner.login')}</Button>)}</div> </div>) ";
  ...
  t ('partner.earnings title')}</CardTitle> <CardDescription> {';
