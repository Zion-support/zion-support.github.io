...
<<<<<<< HEAD
  const filteredWebinars = (showPast ? pastWebinars: anyanyanyanyanyanyanyanyanyanyupcomingWebinars).filter(webinar            => {;
...
=======
      currentAttendees: anyanyanyanyanyanyanyanyanyany189,;
      featured: false,;
      tags: ['Cybersecurity', 'Zero-Trust', 'Security Architecture', 'Implementation'],;
      thumbnail: '/images/webinars/zero-trust-security-2025.jpg',;
      registrationRequired: true,;
...
<<<<<<< HEAD
  const formatTimeUntil = (dateString: anyanyanyanyanyanyanyanyanyanystring)           => {;
    const now = new Date();
...
=======
  const getRegistrationStatus = (webinar: anyanyanyanyanyanyanyanyanyany)           => {
    if (webinar.isLive) {
