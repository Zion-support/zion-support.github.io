import React, { useState  from "react", import { motion } from "framer-moti, on";import { SEO } from "../components/S, EO";import { Link } from "react-router-d, om";import { Store, Search;
  Filter;
  Grid;
  List;
  Star;
  ...
  Canary;
  Finch
} from "lucide-react";export default function Marketplace() {

  const [searchQuer;y;
    setSearchQuery] = useState('')const [selectedCategorysetSelectedCategory] = useState('all')const [selectedTypesetSelectedType] = useState('all')const [viewModesetViewMode] = useState<'grid' | 'list'>('grid')const [expandedItemsetExpandedItem] = useState<string | null>(null)const categories  = [
    { id: 'all, ', name: 'All Categories,'; count: 48,
     };
    { id: 'ai-ml, ', name: 'AI & Machine Learning, ', count: 12,  }{ id: 'cloud, ', name: 'Cloud Solutions, ', count: 8,  }{ id: 'security, ', name: 'Security & Compliance, ', count: 6,  }{ id: 'data, ', name: 'Data & Analytics, ', count: 7,  }{ id: 'development, ', name: 'Development Tools, ', count: 9,  },
    { id: 'hardware, ', name: 'Hardware & Equipment, ', count: 6,  }
  ]const types  = [
    { id: 'all, ', name: 'All Types,'; count: 48,
     };
    { id: 'software, ', name: 'Software, ', count: 25,  }{ id: 'service, ', name: 'Services, ', count: 15,  },
    { id: 'hardware, ', name: 'Hardware, ', count: 8,  }
  ]const marketplaceItems  = [
    // AI & Machine Learning
    {
      id: 'ai-chatbot, ',name: 'AI Chatbot Platform, ',category: 'ai-ml, ',type: 'software',
    icon: Bot,
    description: 'Intelligent chatbot platform with natural language processing, ',features: [
      ...
      pricing: '$299/month, ',rating: 4.8,
    reviews: 15, 6,downloads: 124, 7,status: 'featured, ',tags: ['AIChatbot, ', 'NLPAutomation'],
   ...
   ,  }{
      id: 'ml-pipeline, ',name: 'ML Pipeline Automation, ',category: 'ai-ml, ',type: 'software, ',icon: BarChart3,
    description: 'Automated machine learning pipeline for data scientists, ',features: [
        'AutoML capabilitiesModel versioning, ',
        'A/B testingPerformance monitoring',
        'Scalable infrastructureCloud deployment'
      ],
      pricing: '$499/month, ',rating: 4.9,
    reviews: 8, 9,downloads: 56, 7,status: 'popular, ',tags: ['Machine LearningAutomation, ', 'Data ScienceMLOps'],
   ...
   ,  }{
      id: 'computer-vision, ',name: 'Computer Vision API, ',category: 'ai-ml, ',type: 'service, ',icon: Eye,
    description: 'Advanced computer vision services for image and video analysis, ',features: [
        'Object detectionFace recognition, ',
        'Image classificationVideo analysis',
        'Real-time processingCustom model training'
      ],
      pricing: '$0.01 per image, ',rating: 4.7,
    reviews: 20, 3,downloads: 89, 2,status: 'trending, ',tags: ['Computer VisionAI, ', 'Image ProcessingVideo Analysis'],
   ...
   ,  }// Cloud Solutions
    {
      id: 'cloud-migration, ',name: 'Cloud Migration Service, ',category: 'cloud, ',type: 'service, ',icon: Cloud,
    description: 'Professional cloud migration and optimization services, ',features: [
    ...
    000, ',
      rating: 4.9,
    reviews: 6, 7,downloads: 23, 4,status: 'featured, ',tags: ['Cloud MigrationAWS, ', 'AzureGCP', 'DevOps'],
   ...
   ,  }{
      id: 'kubernetes-manager, ',name: 'Kubernetes Management Platform, ',category: 'cloud, ',type: 'software, ',icon: Server,
    description: 'Enterprise-grade Kubernetes cluster management solution, ',features: [
        'Multi-cluster managementAuto-scaling, ',
        'Monitoring & alertingSecurity policies',
        'Backup & recoveryCost optimization'
      ],
      pricing: '$199/month, ',rating: 4.8,
    reviews: 13, 4,downloads: 45, 6,status: 'popular, ',tags: ['KubernetesContainer Orchestration, ', 'DevOpsCloud Native'],
   ...
   ,  }// Security & Compliance
    ...
    description: 'AI-powered threat detection and response system, ',features: [
        'Real-time monitoringBehavioral analysis, ',
        'Threat intelligenceAutomated response',
        'Compliance reporting24/7 SOC support'
      ],
      pricing: '$399/month, ',rating: 4.9,
    reviews: 17, 8,downloads: 67, 8,status: 'featured, ',tags: ['CybersecurityThreat Detection, ', 'AI SecuritySOC'],
   ...
   ,  }{
      id: 'compliance-audit, ',name: 'Compliance Audit Service, ',category: 'security, ',type: 'service, ',icon: CheckCircle,
    description: 'Comprehensive compliance auditing and certification services, ',features: [
    ...
    000, ',
      rating: 4.8,
    reviews: 4, 5,downloads: 12, 3,status: 'popular, ',tags: ['ComplianceSecurity, ', 'AuditCertification'],
   ...
   ,  }// Data & Analytics
    {
      id: 'data-warehouse, ',name: 'Data Warehouse Solution, ',category: 'data, ',type: 'software, ',icon: Database,
    description: 'Scalable data warehouse with advanced analytics, ',features: [
        'Multi-cloud supportReal-time processing, ',
        'Advanced analyticsData governance',
        'Security & encryptionAuto-scaling'
      ],
      pricing: '$599/month, ',rating: 4.7,
    reviews: 9, 2,downloads: 34, 5,status: 'trending, ',tags: ['Data WarehouseBig Data, ', 'AnalyticsBusiness Intelligence'],
   ...
   ,  }{
      id: 'bi-dashboard, ',name: 'Business Intelligence Dashboard, ',category: 'data, ',type: 'software, ',icon: BarChart3,
    description: 'Interactive BI dashboard for data visualization, ',features: [
        'Drag & drop interfaceReal-time data, ',
        'Custom widgetsMobile responsive',
        'Export capabilitiesCollaboration tools'
      ],
      pricing: '$149/month, ',rating: 4.6,
    reviews: 16, 7,downloads: 78, 9,status: 'popular, ',tags: ['Business IntelligenceDashboard, ', 'Data VisualizationAnalytics'],
   ...
   ,  }// Development Tools
    {
      id: 'code-generator, ',name: 'AI Code Generator, ',category: 'development, ',type: 'software, ',icon: Code,
    description: 'AI-powered code generation and assistance tool, ',features: [
        'Multi-language supportCode completion, ',
        'Bug detectionDocumentation generation',
        'Code reviewIntegration with IDEs'
      ],
      pricing: '$99/month, ',rating: 4.8,
    reviews: 23, 4,downloads: 123, 4,status: 'trending, ',tags: ['Code GenerationAI, ', 'DevelopmentProductivity'],
   ...
   ,  }{
      id: 'api-gateway, ',name: 'API Gateway Platform, ',category: 'development, ',type: 'software, ',icon: Network,
    description: 'Enterprise API gateway with advanced management features, ',features: [
        'Rate limitingAuthentication, ',
        'Monitoring & analyticsVersion management',
        'DocumentationDeveloper portal'
      ],
      pricing: '$299/month, ',rating: 4.7,
    reviews: 15, 6,downloads: 56, 7,status: 'featured, ',tags: ['API GatewayMicroservices, ', 'API ManagementDeveloper Tools'],
   ...
   ,  }// Hardware & Equipment
    {
      id: 'edge-computing, ',name: 'Edge Computing Device, ',category: 'hardware, ',type: 'hardware, ',icon: Cpu,
    description: 'High-performance edge computing device for IoT applications, ',features: [
    ...
    299, ',
      rating: 4.9,
    reviews: 7, 8,downloads: 23, 4,status: 'featured, ',tags: ['Edge ComputingIoT, ', 'HardwareIndustrial'],
   ...
   ,  }{
      id: 'network-switch, ',name: 'Enterprise Network Switch, ',category: 'hardware, ',type: 'hardware, ',icon: Network,
    description: 'High-speed network switch for enterprise environments, ',features: [
        '48-port GigabitPoE+ support, ',
        'VLAN managementQoS features',
        'SNMP monitoringLifetime warranty'
      ],
      pricing: '$899, ',rating: 4.8,
    reviews: 12, 3,downloads: 45, 6,status: 'popular, ',tags: ['NetworkingSwitch, ', 'EnterprisePoE'],
   ...
   ,  }
  ]const featuredItems  = marketplaceItems.filter(item => item.status === 'featured')const popularItems  = marketplaceItems.filter(item => item.status === 'popular')const trendingItems  = marketplaceItems.filter(item => item.status === 'trending')const toggleItemExpansion  = () => {
    setExpandedItem(expandedItem === itemId ? null : itemId)};
  const filteredItems  = marketplaceItems.filter(item => {
    if (selectedCategory !== 'all' && item.category !== selectedCategory) return falseif (selectedType !== 'all' && item.type !== selectedType) return falseif (searchQuery) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <SEO 
        ...
        description="Discover innovative technology solutionssoftware, services, and hardware in our comprehensive marketplace. Find the perfect tools for your business needs."
      />
    ...
    cutting-edge softwar, e, professional servicesand enterprise hardware to accelerate your digital transformation.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col lg: flex-row gap-6 mb-8">
              ...
              <div className="lg: w-48">
                ...
                >
                  {types.map((type) => (
                    <option key={type.i, d} value={type.id}>
    ...
    featureIndex) => (
                              <li key={featureInde, x} className="flex items-center text-gray-300 text-sm">
