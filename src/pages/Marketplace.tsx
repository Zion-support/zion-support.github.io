...
import { MAXPRICE, MINPRICE } from '@/data/marketplaceData',
import { logInfo, logErrorToProduction } from '@/utils/productionLogger',
  ...
  <Card className=&quot;bg-gradient-to-r from-blue-900/20 to-purple-900/20 border-blue-700/30 mb-6&quot;>
    <CardContent className=&quot;p-6&quot;>
      ...
      <div className=&quot;grid grid-cols-2 md:grid-cols-4 gap-4&quot;>
        ...
        <div className=&quot;text-center&quot;>
          <div className=&quot;text-2xl font-bold text-yellow-400&quot;>{stats.categoriesCount}</div>
          <div className=&quot;text-sm text-muted-foreground&quot;>Categories</div>        </div>
      </div>
    </CardContent>
  </Card>
),
...
type=&quot;number"
        ...
        min={priceRange[0]}
        max={MAXPRICE}
        onChange={(e) => setPriceRange([priceRange[0], Number(e.target.value)])}        className="w-20 bg-background border border-border px-2 py-1 rounded"
      />
    </div>
...
type=&quot;number"
        ...
        min={0}
        max={100}
        onChange={(e) => setMinAiScore(Number(e.target.value))}        className="w-16 bg-background border border-border px-2 py-1 rounded"
      />
    </div>
    ...
    <div className="flex items-center gap-2">
      ...
      <select,
value={minRating}
        onChange={_(e) => setMinRating(Number(e.target.value))}
        className="bg-background border border-border px-2 py-1 rounded"
      >
        <option value={0}>Any</option>
        <option value={5}>5</option>
...
variant={showRecommended ? &quot;default&quot; : &quot;outline&quot}
    ...
    >
      <Sparkles className="h-4 w-4&quot; />
      {showRecommended ? &quot;All Products&quot; : &quot;Recommended&quot}    </Button>
  </div>
),
...
const fetchProducts = useCallback(async (page: number, limit: number) => {
    ...
    try {
      // Use static marketplace listings data for now (compatible with ProductListing type)
      const params = {
        page, limit, _...(filterCategory && { category: filterCategory}),
        sort: sortBy
      },

      logInfo('Marketplace.tsx: Fetching products using static data with params:', { data: params }),      
      // Use static data that's already of type ProductListing[]
      let items: ProductListing[] = [...MARKETPLACELISTINGS],
...
if (filterCategory) {
      ...
      items = items.filter((p) => {
        ...
        const rating = p.rating || 0,
        const location = (p.location || '').toLowerCase(),
        const availability = (p.availability || '').toLowerCase(),        return (
          price >= priceRange[0] &&
          price <= priceRange[1] &&
          ai >= minAiScore &&
          rating >= minRating &&
      ...
      items.sort(_(a, b) => {switch (sortBy) {
          case 'price-low':
            return (a.price || 0) - (b.price || 0),
          case 'price-high':
            return (b.price || 0) - (a.price || 0),
...
const startIndex = (page - 1) * limit;
      ...
      return {items: paginatedItems, hasMore: endIndex < items.length, total: items.length}
...
logErrorToProduction('Error in Marketplace fetchProducts:', _{ data: err});
      ...
      // Show more specific error messages based on the error type,
if (err.response?.status === 403) {logErrorToProduction("403 Forbidden error - authentication issue");
        // Don't show toast here, let the AuthModal handle it or rely on ProductCard's tooltip} else if (err.response?.status === 500) {logErrorToProduction("500 Server error");
        toast({
...
const marketStats = useMemo(() => {
  ...
  // Get unique categories and other filter values,
const categories = useMemo(() => {
    return Array.from(new Set(MARKETPLACELISTINGS.map((p) => p.category)))
  }, []),
  const locations = useMemo(() => {
    return Array.from(new Set(MARKETPLACELISTINGS.map((p) => p.location).filter(Boolean)))
  }, []).filter(Boolean) as string[],
  const availabilityOptions = useMemo(() => {
    return Array.from(new Set(MARKETPLACELISTINGS.map((p) => p.availability).filter(Boolean)))
  }, []).filter(Boolean) as string[],
...
if (loading && products.length === 0) {
    return (
      <div className="container py-8&quot; data-testid=&quot;marketplace-loading">        <motion.div,
initial={{ opacity: 0, y: 20}}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {Array.from({ length: 12}).map(_(_, i) => (
            <SkeletonCard key={i} />
          ))}
        </div>
      </div>
...
if (error && products.length === 0) {return (
  ...
  // Empty state (only show when not loading and no products)
  if (!loading && products.length === 0 && !error) {return (
      <div className="container py-8">
        <motion.div,
initial={{ opacity: 0, y: 20}}
          animate={_{ opacity: 1, y: 0}}
                ...
                onBuy={async () => {
                  ...
                  try {
                    ...
                    toast({
                      title: "Navigation Error",
                      description: "Could not navigate to checkout. Please try again.",
                      variant: "destructive"}),                    setIsAuthModalOpen(true);
                    return; // Stop further execution}
                  try {await router.push(`/checkout/${product.id}`)
                  } catch (error) {logErrorToProduction('Failed to navigate to checkout:', _{ data: error});
                    toast({title: "Navigation Error", description: "Could not navigate to checkout. Please try again.", variant: "destructive"});
                    // Re-throw to allow ProductCard's catch to also run if needed,
                    // though ProductCard will reset its state in .finally() regardless.
                    throw error
                  }
