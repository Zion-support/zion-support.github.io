...
export default function ProjectRoom() {_const _router = useRouter();
  const { projectId: rawProjectId} = router.query;
  const _projectId = typeof rawProjectId === 'string' ? rawProjectId : ''; // Ensure string, default to empty if not
  ...
  const _simulateUserJoining = () => {_// This is just for demo purposes - in a real app, _this would be handled by the video call service
    ...
    if (randomUser && !callParticipants.find(p => p.id === randomUser.id)) {_setCallParticipants(prev => [...prev, _randomUser]);
