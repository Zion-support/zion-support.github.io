import React, { useState  from "react", import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";import { Badge } from "@/components/ui/bad, ge";import { Button } from "@/components/ui/butt, on";import { cuttingEdgeComprehensiveServices2027 } from "../../data/2027-cutting-edge-comprehensive-servic, es";import { specializedInnovativeServices2027 } from "../../data/2027-specialized-innovative-servic, es";import { Star, CheckCircle, Phone, Mail, MapPin, ArrowRight, TrendingUp, Users, Shield, Zap } from "lucide-react";import { Link } from "react-router-d, om";const contactInfo  = {
 ...
 ,
  ...
  const [selectedCategorysetSelectedCategory] = useState<string>('all')const [selectedPriceRangesetSelectedPriceRange] = useState<string>('all')// Combine all services
  ...
  const filteredServices  = allServices.filter(service => {
    ...
    if (selectedPriceRange !== 'all') {
      const price  = parseInt(service.price.replace(/[^0-9]/g''))switch (selectedPriceRange) {
        case 'under-1000':
          matchesPrice = price < 1000breakcase '1000-3000':
          matchesPrice = price >= 1000 && price < 3000breakcase '3000-5000':
          matchesPrice = price >= 3000 && price < 5000breakcase 'over-5000':
          matchesPrice = price >= 5000break}
    }
return matchesCategory && matchesPric;e;
  ...
  const getPriceRangeLabel  = () => {
    switch (range) {
      case 'under-1000': return 'Under $1;
    000';
      case '1000-3000': return '$1000 - $, 3,000';
      case '3000-5000': return '$3000 - $5,000';
      case 'over-5000': return 'Over $5000';
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 max-w-4xl mx-auto">
            <div className="grid grid-cols-1 md: grid-cols-3 gap-6">
              <div className="flex flex-col items-center">
                ...
                <p className="text-zion-slate-light">{contactInfo.mobil,
    e}</p>
              </div>
              <div className="flex flex-col items-center">
      ...
      <div className="py-20 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-col md: flex-row gap-4 mb-12 max-w-4xl mx-auto">
            ...
            <select
              className="flex-1 px-4 py-3 rounded-lg border border-zion-slate-light bg-white text-zion-slate-dark focus: outline-none focus:ring-2 focus:ring-zion-cyan"
              value={selectedPriceRang,
    e}
              onChange={(e) => setSelectedPriceRange(e.target.value)}
            >
              <option value="all">All Prices</option>
              <option value="under-1000">Under $1000</option>
              <option value="1000-3000">$1000 - $3,000</option>
              <option value="3000-5000">$3000 - $5,000</option>
          ...
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredServices.map((service) => (
              <Card key={service.i, d} className="hover: shadow-lg transition-shadow duration-300">
                <CardHeader>
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-3xl">{service.ico,
    n}</div>
                    {service.popular && (
    ...
    3).map((featur, e, index) => (
                  ...
                  <div className="space-y-2 mb-4 text-sm bg-zion-slate-light/20 p-3 rounded-lg">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{service.marketSiz, e}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zion-slate-dark">Growth Rate: </span>
                      <span className="font-semibold">{service.growthRat, e}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zion-slate-dark">ROI: </span>
                      <span className="font-semibold">{service.ro, i}</span>
                    </div>
                  </div>
