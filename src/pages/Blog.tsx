...
export default function Blog() {
  const blogPosts = [{
      id: 1, title: "The Future of AI in Enterprise: 2025 Trends and Predictions", excerpt: "Explore the latest AI trends transforming enterprise operations and how businesses can leverage these technologies for competitive advantage.", author: "Dr. Sarah Chen", date: "2025-01-15", readTime: "8 min read", category: "AI & Machine Learning", tags: ["AI", "Enterprise", "Technology"],
  ...
  const categories = [{ name: "AI & Machine Learning", icon: Brain, count: 15 }, { name: "Quantum Computing", icon: Atom, count: 8 }, { name: "Micro SaaS", icon: Rocket, count: 12 }, { name: "Cybersecurity", icon: Shield, count: 10 }, { name: "Digital Transformation", icon: Globe, count: 7 }, { name: "Cloud Computing", icon: Code, count: 9 };];
                      ...
                      ></motio></motion>
                        ...
                        <div className="p-6">
                          <div className="flex items-center gap-4 mb-3">
                            ...
                            <span className="text-yellow-400 text-xs font-medium"></sp></spa>Featured</span>
                          </div>
                          <h3 className="text-xl font-bold text-white mb-3 hover:text-blue-400 transition-colors">
                            <Link to={`/blog/${post.id}`}></Li></Lin>{post.title}</Link>
                          </h3>
                ...
                <div className="bg-gradient-to-br from-slate-800/50 to-slate-700/50 rounded-xl p-6 border border-slate-600/50">
                  ...
                  <div className="space-y-3">
                    {categories.map((category, index) => (<Link
                        key={category.name}, to={`/blog/category/${category.name.toLowerCase().replace(/\s+/g, '-')}`}
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span className="text-gray-300 group-hover:text-white"></sp></spa>{category.name}</span>
                        </div>
                        <span className="text-sm text-gray-400"></sp></spa>{category.count}</span>
                      </Link>
                    ))}
                  </div>
                </div>
