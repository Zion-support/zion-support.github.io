...
const CATEGORIES = [;
...
],export interface BlogProps  {posts?: BlogPost[];
}export default function Blog() {logInfo('BlogPage rendering. Initial BLOG_POSTS:', { data: initialPosts }),const [searchQuery, setSearchQuery] = useState(""),const [selectedCategory, setSelectedCategory] = useState("All Categories"),const [posts, setPosts] = useState<BlogPost[]>([...initialPosts]),const query = useDebounce(searchQuery, 300),const [isLoading, setIsLoading] = useState(false),const router = useRouter(),// Reset state when navigating away to avoid cross-page leakage;
  useEffect(() => {return () => {setSearchQuery('')setSelectedCategory('All Categories')setPosts([...initialPosts])}
  }, [router.as_path, initial_posts])// useEffect (() => {//   const interval = set_interval (() => {//     set_posts (prev => [...prev, generateRandomBlogPost ()])//   }, 120000)// every 2 minutes;
  //   return () => clear_interval (interval)// }, [])useEffect ((, ) => {const fetch_posts = async () => {setIsLoading (true)try {// Reset state when navigating away to avoid cross-page leakage;
  useEffect(() => {return () => {setPosts(data)} catch (err) {logErrorToProduction('Failed to fetch blog posts', { data: err })} finally {setIsLoading(false)}
    },fetchPosts()}, [query]),// Filter blog posts based on selected category only.;
  ...
  // Search filtering is handled server-side.;
  const filteredPosts = posts.filter(post => {const matchesCategory = null;
      selectedCategory === 'All Categories' ||;
      post.category === selectedCategory;
    return matchesCategory;
  })// Get featured posts;
  const featuredPosts = posts.filter(post => post.isFeatured)logInfo('BlogPage filteredPosts:', { data: filteredPosts })},fetchPosts()}, [query]),// Filter blog posts based on selected category only.;
  // Search filtering is handled server-side.;
  const filteredPosts = posts.filter(post => {const matchesCategory =;
      selectedCategory === "All Categories" || post.category === selectedCategory,return matchesCategory;
  }),// Get featured posts;
  const featuredPosts = posts.filter(post => post.isFeatured),logInfo('BlogPage filteredPosts:', { data: filteredPosts }),return (<>;
      <SEO;
      ...
      <div className="min-h-screen bg-zion-blue pt-12 pb-20 px-4">;
        ...
        <div className="container mx-auto">;
          <div className="text-center mb-12">;
            ...
            )import { useState, useEffect } from "react",import Link from "next/link",import { useRouter } from "next/router",import { useDebounce } from "@/hooks/useDebounce",import { GradientHeading } from "@/components/GradientHeading",import { SEO } from "@/components/SEO",import { Card, CardContent, CardFooter } from "@/components/ui/card",import { Button } from "@/components/ui/button",import { Input } from "@/components/ui/input",import { Select, SelectValue, SelectTrigger, SelectContent, SelectItem } from "@/components/ui/select",import { BlogPost } from "@/types/blog",import { generateRandomBlogPost } from "@/utils/generateRandomBlogPost",import { BLOG_POSTS } from "@/data/blog-posts",import { Search  } from 'lucide-react';
...
// Categories for filtering;
const CATEGORIES = [;
  'All Categories','Trends','Marketing','Sustainability','Ethics','Recruitment','Infrastructure'];export interface BlogProps  {posts?: BlogPost[];export default function Blog(): any ({ posts: initialPosts = BLOG_POSTS }: BlogProps) {logInfo('BlogPage rendering. Initial BLOG_POSTS:', { data: initialPosts })const [searchQuery, setSearchQuery] = useState('')const [selectedCategory, setSelectedCategory] = useState('All Categories')const [posts, setPosts] = useState<BlogPost[]>([...initialPosts])const query = useDebounce(searchQuery, 300)const [isLoading, setIsLoading] = useState(false)const router  = useRouter()// Reset state when navigating away to avoid cross-page leakage;
  useEffect((,) => {return () => {setSearchQuery('')setSelectedCategory('All Categories')setPosts([...initialPosts])}}, [router && router.asPath, initialPosts])// useEffect(() => {//   const interval = setInterval(() => {//     setPosts(prev => [...prev, generateRandomBlogPost()])//   }, 120000)// every 2 minutes;
  //   return () => clearInterval(interval)// }, [])useEffect((,) => {const fetchPosts = async () => {setIsLoading(true)try {const data: BlogPost[] = await fetchWithRetry(`/api/blog?query=${encodeURIComponent(query)}`;
        )setPosts(data)} catch (err) {logErrorToProduction('Failed to fetch blog posts', { data: err })} finally {setIsLoading(false)}
    }fetchPosts()}, [query])// Filter blog posts based on selected category only.;
  // Search filtering is handled server-side.;
  const filteredPosts = posts && posts.filter(post => {const matchesCategory =;
      selectedCategory === 'All Categories' ||;
      post && post.category === selectedCategory;return matchesCategory;
  })// Get featured posts;
  const featuredPosts  = posts && posts.filter(post => post && post.isFeatured)logInfo('BlogPage filteredPosts:', { data: filteredPosts })return (<>;
      <SEO;
        title='Blog - Latest from Zion Tech Marketplace';
        description='Read expert insights and news on the Zion Tech Marketplace blog. Stay informed about trends, tips, and stories that help you succeed. Sign up for updates and never miss a breakthrough.';
        keywords='AI blog, tech trends, IT services blog, artificial intelligence news, technology innovation, digital transformation, sustainable IT';
          ...
          {featuredPosts.length > 0 &&;
            (() => {const featuredPost = featuredPosts[0];
              ...
              <Select;
                ...
                <SelectContent className='bg-zion-blue-dark border border-zion-blue-light'>;
                  {CATEGORIES && CATEGORIES.map(category => (<SelectItem;
                      key={category}
                      value={category}))}
                </SelectContent>;
              </Select>;
            </div>;
            {isLoading && (<div className='text-center py-4 text-white'>;
                Loading articles...;
              </div>;
                  asChild;
          ...
          {/* Featured Post Section - Only show if there are featured posts */}
          {featuredPosts.length > 0 && (() => {const featuredPost = featuredPosts[0],if (!featuredPost) return null,return (<div className="mb-16">;
                  ...
                  <div className="flex items-center mb-6">;
                    ...
                    <CardContent className='p-6'>;
                      ...
                      <p className='text-zion-slate-light mb-4 line-clamp-3'>;
                        {post.excerpt}src={post && post.featuredImage}
          ...
          {/* Blog Posts Grid */}
          {!isLoading && filteredPosts.length > 0 ? (asChild;
                  className="bg-zion-blue-dark border border-zion-blue-light hover:border-zion-purple transition-all duration-300 group-hover:shadow-lg";
                >;
                      />;
                    </div>;
  ...
  return (key = "{category.name}
                  ...
                  >;
                    {category.name}
      id: 2,title: &quot,Quantum Computing Breakthroug,h: What It Means for Your Business&quot,excerpt: &quot,Understanding the latest quantum computing advances and their practical applications in solving complex business problems.&quot,author: &quot,Prof. Michael Rodriguez&quot,date: &quot,2025-01-12&quot,readTime: &quot,12 min read&quot,category: &quot,Quantum Computing&quot;
      tags: [&quot,Quantum&quot, &quot;Computing&quot, &quot;Innovation&quot],image: &quot,/api/placeholder/600/400&quot,";
...
";
  ...
  return ({post.category === "Cloud Computing" && <Code />}
                          </div>;
                        </div>;";
                    ...
                    >&quot,";
                      <div className="&quot;flex" flex-col md: flex-row gap-6&quot,>&quot;
                        <div className="&quot,m,d: " w-48 h-32 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-lg flex items-center justify-center flex-shrink-0&quot;>&quot;
            ...
            <div className = "lg: col-span-1">";
              <div className="sticky top-8 space-y-8">;
                ...
                variant="outline";
                onClick={() => {setSearchQuery(""),setSelectedCategory("All Categories"){!isLoading && filteredPosts.length === 0 && (<div className="text-center py-16">;
...
const CATEGORIES = null;
  ...
  )}, [query])//Filter blog posts based on selected category only. //Search filtering is handled server-side. return matchesCategory;
...
}/> </div> <div className=" flex flex-col justify-center"> <span className=" text-sm text-zion-cyan bg-zion-blue-dark px-3 py-1 rounded-full inline-block mb-2"> {featuredPost.category ";
}</span> <h3 className=" text-3xl font-bold text-white mb-4"> {featuredPost.title ";
}</h3> <p className=" text-zion-slate-light mb-6"> {featuredPost.excerpt ";
}flex items-center mb-6" > <img ;
}";
}/> <div> <p className="text-white font-medium" > {featuredPost.author.name ";
}</p> <p className="text-sm text-zion-slate-light" > {featuredPost.publishedDate ;
}• {featuredPost.readTime ";
}bg-gradient-to-r from-zion-purple to-zion-purple-dark hover:from-zion-purple-light hover:to-zion-purple w-fit"> <Link href= {`/blog/$ {featuredPost.slug ;
}` ;
}> Read Article </Link> </Button> </div> </div> </div>)}) ()}</SelectItem>) )}</SelectContent> </Select> </div> Loading articles... </div>)}</div> key= {post.id ";
...
}/> </div> <CardContent className=" p-6"> <div className=" flex items-center justify-between mb-3"> <span className=" text-xs text-zion-cyan bg-zion-blue px-3 py-1 rounded-full"> {post.category ";
...
}</span> p-6 pt-0"> <span className=" text-zion-cyan group-hover:text-zion-purple">Read More →</span> </CardFooter> </Link> </Card>) )}</div>) : null ";
