...
export {CartPage}() {
    ...
    useEffect(() => {
        if (reduxItems.length > 0) {
            ...
            if (stored) {
                ...
                catch {dispatch(setItemsAction([]))}
            }
            else {dispatch(setItemsAction([]))}
        }
        ;
        load()}, [user, dispatch]);
    ...
    const updateQuantity = (props: any) => {
        dispatch(updateQuantityAction({ id, quantity: qty }))};
    const removeItem = (props: any) => {dispatch(removeItemAction(id))};
    const handleCheckout = (props: any) => {router.push('/checkout')};
    const applyCode = async () => {
        ...
        catch (e) {setDiscount(0)}
    };
    const subtotal = items.reduce((sum, i) => sum + i.price * i.quantity, 0);
    const total = subtotal - discount;
    if (cartLoading) {return (<div className="container py-10 space-y-4">
      ...
      <ul className="space-y-4">
        {items.map(item => (<li key={item.id} className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <input type="number" min={1} value={item.quantity} onChange={e => updateQuantity(item.id, parseInt(e.target.value || '1',
