        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {whitePapers.map((paper) => (
              <div
                key={paper.id}
                className="bg-slate-800/50 rounded-xl p-6 border border-slate-700 hover:border-slate-600 transition-all hover:transform hover:scale-105"
              >
                <div className="flex items-center justify-between mb-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    paper.category === 'Cybersecurity' ? 'bg-red-500/20 text-red-400' :
                    paper.category === 'AI & Machine Learning' ? 'bg-blue-500/20 text-blue-400' :
                    paper.category === 'Digital Transformation' ? 'bg-green-500/20 text-green-400' :
                    paper.category === 'Cloud & Infrastructure' ? 'bg-purple-500/20 text-purple-400' :
