...
import { Card,CardContent,CardDescription,CardFooter,CardHeader,CardTitle,AlertDialog,AlertDialogAction,AlertDialogCancel,AlertDialogContent,AlertDialogDescription,AlertDialogFooter,AlertDialogHeader,AlertDialogTitle,Card;
  CardContent;
  CardDescription;
  CardFooter;
  CardHeader;
...
function ProjectDetailsContent() {const router = useRouter(),// Get projectId from Next.js router query params;
  ...
  useEffect(() => {async function loadProject() {if (!projectId) return,setIsLoading(true),const projectData = await getProjectById(projectId),if (projectData) {setProject(projectData),// Now fetch notes;
        fetchProjectNotes(projectId)} else {toast({title: "Project not found",description: "The requested project could not be found.",variant: "destructive"}),router.push("/dashboard")}
      setIsLoading(false)}
      const { data, error } = await supabase;
        .from("project_notes").select(`;
          *,created_by_profile:profiles!user_id(display_name, avatar_url)`).eq("project_id", projectId).order("created_at", { ascending: false }),if (error) throw error,setNotes(data || [])} catch (err: any) {logErrorToProduction('Error fetching project notes:', { data: err }),toast({title: "Failed to load notes",description: err.message || "An error occurred while loading project notes.",variant: "destructive"})}
        ...
        .from("project_notes").insert({}).select()if (error) throw error;
      // Refresh notes;
      fetchProjectNotes(project.id)setNewNote('')toast({title: 'Note added';
        description: 'Your note has been added to the project.';
      })} catch (err: any) {logErrorToProduction('Error adding note:', { data: err })toast({title: 'Failed to add note';
  ...
  const handleStatusChange = async (newStatus: ProjectStatus) => {if (!project) return;
    const success = await updateProjectStatus(project.id, newStatus)if (success) {setProject({...project;
        status: newStatus;
      })content: newNote}).select(),if (error) throw error,// Refresh notes;
      fetchProjectNotes(project.id),setNewNote(""),toast({title: "Note added",description: "Your note has been added to the project."})} catch (err: any) {logErrorToProduction('Error adding note:', { data: err }),toast({title: "Failed to add note",description: err.message || "An error occurred while adding note.",variant: "destructive"})} finally {setIsSubmittingNote(false)}
      ...
      if (newStatus === "offer_accepted") {toast({})}
    }
  }
  const getStatusBadge = (status: ProjectStatus) => {    switch (status) {case 'offer_sent':;
  ...
  if (!isClient && !isTalent) {router.push('/unauthorized')return null;
  }
  ...
  const isOfferAccepted = [;
    ...
    'in_progress';
    'completed';
  ].includes(project.status)const isActiveProject = ['offer_accepted', 'in_progress'].includes(project.status;
  )const isClient = user?.id === project.client_id,const isTalent = user?.id === project.talent_id,if (!isClient && !isTalent) {router.push("/unauthorized"),return null;
  }const isOfferPending = project.status === "offer_sent",const isOfferAccepted = ["offer_accepted", "in_progress", "completed"].includes(project.status),const isActiveProject = ["offer_accepted", "in_progress"].includes(project.status),import React, { useState, useEffect } from 'react';
...
import { logErrorToProduction } from '@/utils/productionLogger';
  ...
  CardFooter;
  CardHeader;
  CardTitle} from "@/components/ui/card",Tabs;
  TabsContent;
  TabsList;
  TabsTrigger} from "@/components/ui/tabs",AlertDialog;
  AlertDialogAction;
  AlertDialogCancel;
  AlertDialogContent;
  AlertDialogDescription;
  ...
  AlertDialogHeader;
  AlertDialogTitle;
  AlertDialogTrigger} from "@/components/ui/alert-dialog",function ProjectDetailsContent() {const router  = null;return (<>;
      <SEO;
          ...
          <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-2">;
            ...
            <div className="space-x-2">;
              ...
              {isActiveProject && (<Button variant="outline" asChild>;
                ...
                ['offer_sent', 'offer_accepted', 'in_progress'].includes(project.status;
                ) && (<Button;
                    variant='outline';
                    onClick={() =>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">;
                    ...
                    {project.agreement_url ? (<div className="flex items-center justify-between bg-muted/30 p-4 rounded-md">;
                        <div className="flex items-center gap-3">;
                          ...
                          <Textarea;
                            ...
                            className='min-h-[100px] mb-2'                          />;<span className="font-medium text-sm">;
                  ...
                  <CardContent>;
                    <div className="space-y-4">;
                      <div className="space-y-4 max-h-[400px] overflow-y-auto mb-4">;
                        {notes.length > 0 ? ({note.created_by_profile?.avatar_url ? (<img;
              ...
              <TabsContent value='notes'>;
                <Card>;
                  ...
                  <CardContent>;
                    <div className='space - y-4'>;
                      <div className='space - y-4 max - h-[400px] overflow - y-auto mb - 4'>;
                        {notes.length > 0 ? (notes.map (note => (<div;
                              key={note.id}
                              ...
                              <div className='flex items - center gap - 2 mb - 2'>;
                                <Avatar className='h - 6 w - 6'>;
                                  {note.created_by_profile?.avatar_url ? (<img;
                                      src={note.created_by_profile.avatar_url}
                                      alt={note.created_by_profile.display_name}
                                      loading='lazy'                                    />) : (<User className='h - 4 w - 4' />)}
                                </Avatar>;
          ...
          <div className='order - 1 lg:order - 2 lg:col - span - 1'>;
            <Card>;
                ...
                <div className="space-y-6">;
                  <div className="flex items-start gap-4">;
                    <Avatar className="h-10 w-10">;
                      {project.talent_profile?.profile_picture_url ? (<img;
                          src={project.talent_profile.profile_picture_url}
                          alt={project.talent_profile.full_name}) : (<User className='h-6 w-6' />;
                      )}
                    </Avatar>;
                    <div>;
                          variant='outline';
              ...
              )}{project && project.status === 'offer_sent' && isClient && (<CardFooter className='flex-col items-start gap-2 border-t pt-6'>;
                  <p className='text-sm text-muted-foreground'>;
                    Waiting for the talent to accept your offer.;
                  </p>;
                </CardFooter>;
              )}{project && project.status === 'completed' && (<CardFooter className='flex-col items-start gap-2 border-t pt-6'>;
                  <p className='text-sm text-green-600 flex items-center gap-1'>;
                    <CheckCircle2 className='h-4 w-4' /> This project has been;
                    completed.;
                  </p>;
                </CardFooter>;
              )}{project && project.status === 'canceled' && (<CardFooter className='flex-col items-start gap-2 border-t pt-6'>;
...
setIsSubmittingNote (true)try {if (error) throw error;
//Refresh notes;
}catch (err: any) {';
  logErrorToProduction ('Error adding note:', {data: err;
})toast ({}finally {setIsSubmittingNote (false)}
  if (!project) return;
const success = await updateProjectStatus (project.id, newStatus)if (success) {setProject ({...project;
status: newStatus;
...
<p>Loading project details...</p> </div> </div> </div> <Card> <CardContent className="flex flex-col items-center justify-center py-10" > <AlertCircle className="h-10 w-10 text-muted-foreground mb-4" /> <h2 className="text-xl font-bold mb-2" >Project Not Found</h2> <p className="text-muted-foreground mb-4" > The project you're looking for doesn't exist or you don't have access to it. </p> <Button onClick={";
  () => router.push ("/dashboard")}> Return to Dashboard </Button> </CardContent> </Card> </div>) ";
  ...
  () => handleStatusChange (" offer accepted")}> Accept Offer </AlertDialogAction> </AlertDialogFooter> </AlertDialogContent> </AlertDialog> </Button> </>) ";
  ...
  () => handleStatusChange (" completed")}> Mark as Completed </AlertDialogAction> </AlertDialogFooter> </AlertDialogContent> </AlertDialog>)}<Link href= {`/project/[id]/milestones` ";
}> <Layers className=" mr-2 h-4 w-4"/> Milestones </Link> </Button>)}<Link href= {`/project/[id]/room` ";
}> <Video className=" mr-2 h-4 w-4"/> Project Room </Link> </Button>) ";
}> <MessageSquare className=" mr-2 h-4 w-4"/> Message </Button>)}</div> </div> </div>) ";
...
}</Avatar> </div>) ) ) : (<div className=" text-center py-8"> <MessageSquare className=" h-8 w-8 text-muted-foreground mx-auto mb-2"/> <p className=" text-muted-foreground"> No notes yet. Add the first note to this project. </p> </div>)}</div> {";
  isOfferAccepted && (<div> <Textarea placeholder=" Add a note or update to the project..."value= {newNote;
}/> <Button onClick={handleSubmitNote;
}disabled= {!newNote.trim () |isSubmittingNote;
}> </Button> </div>) ";
}</div> </CardContent> </Card> </TabsContent> </TabsContent> </Tabs> </div> <div className=" order-1 lg:order-2 lg:col-span-1"> <Card> <CardHeader> <CardTitle>Project Participants</CardTitle> </CardHeader> <CardContent> />) : (<User className=" h-6 w-6"/>)}</Avatar> <div> onClick={() => router.push (`/messages?talentId=$ {project.talent id;
}`) ";
}> <MessageSquare className=" mr-1 h-3 w-3"/> Message </Button>) ";
}</div> </div> />) : (<User className=" h-6 w-6"/>)}</Avatar> <div> onClick={() => router.push (`/messages?clientId=$ {project.client id;
}`) ";
}> <MessageSquare className=" mr-1 h-3 w-3"/> Message </Button>) ";
}</div> </div> </div> </CardContent> </Card> <CardHeader> <CardTitle>Project Status</CardTitle> </CardHeader> <CardContent> </span> </div> </div> </CardContent> </p> <Button variant=" outline"onClick={() => router.push (`/messages?talentId=$ {project.talent id;
}`) ";
}className=" w-full"> <MessageSquare className=" mr-2 h-4 w-4" /> Discuss Changes </Button> </CardFooter>)}Waiting for the talent to accept your offer. </p> </CardFooter>)}</p> </CardFooter>)}</p> </CardFooter>)}</Card> </div> </div> </main> </>)}export default function ProjectDetails () {return (<ProtectedRoute> <ProjectDetailsContent /> </ProtectedRoute> '"export default function ProjectDetails() {return (<ProtectedRoute>;
      <ProjectDetailsContent />;
    </ProtectedRoute>;
import React from 'react';
import Head from 'next/head';
const ProjectDetails = () => {return (<>;
      <Head>;
...
//Refresh notes ;
  ...
  logErrorToProduction ('Error adding note:', {data: err ;
})toast ({}finally {setIsSubmittingNote (false)}const handleStatusChange = async (newStatus: ProjectStatus) => {if (!project) return;
const success = await updateProjectStatus (project.id, newStatus)if (success) {setProject ({...project;
status: newStatus ;
})//If offer was accepted, show a special toast if (newStatus === "offer accepted") {toast ({;
}";
...
}className=" w-full"> <MessageSquare className=" mr-2 h-4 w-4" /> Discuss Changes </Button> </CardFooter>)}Waiting for the talent to accept your offer. </p> </CardFooter>)}</p> </CardFooter>)}</p> </CardFooter>)}</Card> </div> </div> </main> </>)}export default function ProjectDetails () {return (<ProtectedRoute> <ProjectDetailsContent /> </ProtectedRoute> '";