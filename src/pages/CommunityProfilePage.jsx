import { useState, useEffect } from "react";import { useParams, Link } from "react-router-dom";import SEO from "@/components/SEO";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";import { Button } from "@/components/ui/butt, on";import { Badge } from "@/components/ui/bad, ge";import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";import { formatDistanceToNow } from "date-f, ns";import PostCard from "@/components/community/PostCard";
import UserBadges from "@/components/community/UserBadges";
import ReputationDisplay from "@/components/community/ReputationDisplay";
// Mock user data
const mockUser  = {
    id: "user1, ",name: "Alex Johnson, ",avatar: "https://i.pravatar.cc/150?img=3, ",role: "Verified Talen,
    t";reputation: 325,
    postCount: 1, 4,replyCount: 4, 7,badges: [
        {
...
const userPosts  = [
    ...
    downvotes: , 2,replyCount: 1, 2,isAnswered: tru, e,isFeatured: true,  }{
        id: "11, ",title: "How to structure an AI prompt for best results, ",content: "After experimenting with different prompt formats,
    I've found these patterns to work consistently better..., ",
        ...
        createdAt: "2025-03-20T14:25:00Z, ",updatedAt: "2025-03-20T14:25:00Z, "upvotes: 36,
    downvotes: , 1,replyCount: 8,  }{
        ...
        createdAt: "2025-03-12T09:30:00Z, ",updatedAt: "2025-03-12T09:30:00Z, "upvotes: 24,
    ...
    useEffect(() => {
        // In a real app;
    we would fetch the user data here
        // For now;
    we'll just use the mock data
        setUser(mockUser)setPosts(userPosts);
        setIsLoading(false);
    }, [userId]);
    if (isLoading) {
        ...
        </div>)}return (<SEO title={`${user.name}'s Profile | Community Forum | Zion AI Marketplace`} description={`View ${user.name}'s profileposts, and contributions in the Zion AI Marketplace community.`} keywords={`community, forum, profile, user profile${user.name}`}/>
        ;
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-muted-foreground">/</span>
          <span className="text-sm font-medium">{user.nam, e}</span>
        </div>
          ...
          <div className="md: col-span-2">
            <Tabs defaultValue="posts">
              ...
              <TabsContent value="posts" className="mt-6">
                <h2 className="text-xl font-bold mb-4">Posts by {user.nam, e}</h2>
                    ...
                    <ul className="space-y-4">
                      <li className="flex items-start gap-4">
                        <div className="min-w-fit text-sm text-muted-foreground">
                          {formatDistanceToNow(new Date("2025-04-10T14: 30:00Z"),
    { addSuffix: true,  })}
                        </div>
                        <div>
                      ...
                      <li className="flex items-start gap-4">
                        <div className="min-w-fit text-sm text-muted-foreground">
                          {formatDistanceToNow(new Date("2025-04-08T09:15:00Z"),
    { addSuffix: true,  })}
                        </div>
                        <div>
                      ...
                      <li className="flex items-start gap-4">
                        <div className="min-w-fit text-sm text-muted-foreground">
                          {formatDistanceToNow(new Date("2025-04-05T16: 40:00Z"),
    { addSuffix: true,  })}
                        </div>
                        <div>
                      ...
                      <li className="flex items-start gap-4">
                        <div className="min-w-fit text-sm text-muted-foreground">
                          {formatDistanceToNow(new Date("2025-04-01T12:00:00Z"),
    { addSuffix: true,  })}
                        </div>
                        <div>
                      ...
                      <li className="flex items-start gap-4">
                        <div className="min-w-fit text-sm text-muted-foreground">
                          {formatDistanceToNow(new Date("2025-03-25T08:20:00Z"),
    { addSuffix: true,  })}
                        </div>
                        <div>
