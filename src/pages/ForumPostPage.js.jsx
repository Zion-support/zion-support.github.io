...
export {ForumPostPage}() {
    ...
    const handleDownvote = (props: any) => {
        if (!user) {
            toast({
                title: "Authentication required",
                description: "Please sign in to vote on posts",
    ...
    const handleMarkAsAnswer = (props: any) => {
    ...
    const handleReportPost = (props: any) => {
        if (!user) {
            toast({
                title: "Authentication required",
    ...
    const handleLockPost = (props: any) => {
        if (!isAdminOrMod)
            return;
        setPost({ ...post, isLocked: !post.isLocked });
        toast({title: post.isLocked ? "Post unlocked" : "Post locked",
            description: post.isLocked ? "Comments are now allowed" : "Comments are now disabled",})};
    const timeAgo = formatDistanceToNow(new Date(post.createdAt), {addSuffix: true});
    const formattedDate = format(new Date(post.createdAt), "MMMM d, yyyy 'at' h:mm a");
    return (<>
      <SEO title={`${post.title} | Community Forum | Zion AI Marketplace`} description = {post.content.substring(0,
  160)} keywords = {
