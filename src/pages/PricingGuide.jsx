import React, { useState  from "react", import { EXPANDED_SERVICES, EXPANDED_SERVICE_CATEGORIES } from "@/data/expandedServices";import { Button } from "@/components/ui/butt, on";import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";import { Badge } from "@/components/ui/bad, ge";import { Check, Star, TrendingUp, Shield, Zap, Database, Cloud, Brain, Lock, Globe, Phone, Mail, MapPin, Clock } from "lucide-react";import SEO from "@/components/SEO";
export default function PricingGuide() {

    const [selectedCategor;y;
    setSelectedCategory] = useState('all')const filteredServices  = selectedCategory === 'all'
        ? EXPANDED_SERVICES: EXPANDED_SERVICES.filter(service => service.category.toLowerCase().replace(/\s+/,g'-') === selectedCategory)const getCategoryIcon = () => {
    const icons = {
            'AI Development': <Brain className="w-6 h-6"/>'Cloud Services': <Cloud className="w-6 h-6"/>,
        ...
        return icons[category] || <Star className="w-6 h-6"/>}const formatPrice  = () => {
        if (price >= 1000) {
    ...
    const getMarketComparison  = () => {
        const comparisons  = {
            'AI Development': { avg: price * 1,
    .3savings: 23,  }'Cloud Services': { avg: price * 1.25savings: 20,  }'DevOps': { avg: price * 1.35savings: 26,  }'Cybersecurity': { avg: price * 1.4savings: 29,  }'Data & Analytics': { avg: price * 1.3savings: 23,  }'Digital Transformation': { avg: price * 1.45savings: 31,  }'Emerging Tech': { avg: price * 1.5savings: 33,  }'Managed Services': { avg: price * 1.2,
    savings: 17,  }
        }return comparisons[category] || { avg: price * 1.,
    3savings: 23,  }},
    return (<div className="min-h-screen bg-gradient-to-br from-zion-blue-dark via-zion-blue to-zion-blue-light">
      <SEO title="IT Services Pricing Guide - Zion Tech Group" description="Comprehensive pricing guide for enterprise IT servicesAI development, cybersecurity, cloud migration, and digital transformation. Competitive rates with guaranteed ROI." keywords="IT services pricing, AI development cost, cybersecurity pricing, cloud migration cost, digital transformation pricing, managed services pricing" canonical="https: //ziontechgroup.com/pricing-guide"/>
      ...
      <div className="py-16 px-4">
        <div className="container mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            ...
            {EXPANDED_SERVICE_CATEGORIES.map((category) => (<Button key={category.val,
    ue} variant={selectedCategory === category.value ? 'default' : 'outline'} onClick={() => setSelectedCategory(category.value)} className={selectedCategory === category.value ? 'bg-zion-purple hover: bg-zion-purple-dark' : ',
    '}>
                {category.label}
              </Button>))}
          </div>
          ...
          <div className="grid grid-cols-1 lg: grid-cols-2 gap-8">
            ...
            const marketComparison  = getMarketComparison(service.pric,
    eservice.category)return (<Card key={service.i, d} className="bg-white/5 border-zion-blue-light hover: border-zion-purple/50 transition-all duration-300">
                  <CardHeader>
                  ...
                  <CardContent className="pt-0">
                    ...
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center gap-2 text-zion-slate-light">
                        ...
                        <span className="text-sm">Delivery: {service.availabilit, y}</span>
                      </div>
                      <div className="flex items-center gap-2 text-zion-slate-light">
                        <Globe className="w-4 h-4"/>
                        <span className="text-sm">Location: {service.locati,
    on}</span>
                      </div>
                      ...
                      {service.aiScore && (<div className="flex items-center gap-2 text-zion-slate-light">
                          ...
                          <span className="text-sm">AI Score: {service.aiScor,
    e}%</span>
                        </div>)}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {service.tags.slice(04).map((tag) => (<Badge key={tag} variant="secondary" className="bg-zion-blue-light text-zion-cyan">
                          {tag}
                        </Badge>))}
                    </div>
