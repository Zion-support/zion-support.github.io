      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-slate-800/30">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {contentTypes.map((((((((contentType, index, index, index, index, index, index, index) => ({ ...((((((contentType, index, index, index, index, index, index, key: index })) => ({ ...(((((contentType, index, index, index, index, index, key: index })) => ({ ...((((contentType, index, index, index, index, key: index })) => ({ ...(((contentType, index, index, index, key: index })) => ({ ...((contentType, index, index, key: index })) => ({ ...(contentType, index, key: index })) => (<div key={index} className="bg-slate-800/50 border border-white/10 rounded-xl p-6 hover:border-emerald-500/50 transition-all duration-300">
                <div className="w-16 h-16 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full flex items-center justify-center mx-auto mb-4">
                ...
                <ul className="space-y-2">
                  {contentType.capabilities.map((((((((capability, idx, index, index, index, index, index, index) => ({ ...((((((capability, idx, index, index, index, index, index, key: index })) => ({ ...(((((capability, idx, index, index, index, index, key: index })) => ({ ...((((capability, idx, index, index, index, key: index })) => ({ ...(((capability, idx, index, index, key: index })) => ({ ...((capability, idx, index, key: index })) => ({ ...(capability, idx, key: index })) => (<li key={idx} className="text-sm text-gray-400 flex items-start">
                      <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full mr-2 mt-2 flex-shrink-0"></div>
                      {capability}
                    </li>))}
                </ul>
