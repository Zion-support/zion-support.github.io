...
const avgRating = services.reduce((sum, s) => sum + (s.rating || 0), 0) / services.length;
  ...
  m: d:grid-cols-4 gap-6 max-w-4xl mx-auto mb-8">
      ...
      <div className="bg-zion-blue-dark py-8 border-b border-zion-blue-light">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-4">
            <Button variant={selectedCategory === 'all' ? 'default' : 'outline'} onClick={() => setSelectedCategory('all')} className={selectedCategory === 'all' ? 'bg-zion-cyan text-white' : 'border-zion-cyan text-zion-cyan,
  hove: r: bg-zion-cyan/10'}>
              All Services ({EXPANDED_SERVICES.length})
            </Button>
            {SERVICE_CATEGORIES.map(category () => {
  const stats = getCategoryStats(category.name)
            return (<Button key={category.id} variant={selectedCategory === category.name ? 'default' : 'outline'} onClick={() => setSelectedCategory(category.name)} className={selectedCategory === category.name ? 'bg-zion-cyan text-white' : 'border-zion-cyan text-zion-cyan,
  hove: r: bg-zion-cyan/10'}>
  ...
  const tier = getServiceTier(service.price || 0)
            return (<Card key={service.id} className="h-full,
  hove: r:shadow-lg transition-shadow duration-300">
                ...
                <CardContent className="pt-0">
                  <CardDescription className="text-gray-600 mb-4 overflow-hidden text-ellipsis" style={ displ,
  a: y: '-webkit-box', WebkitLineCla,
  m: p: 3, WebkitBoxOrie,
  n: t: 'vertical' },
  }>
                    {service.description},
  ...
  s: m: flex-row justify-center gap-4">
