    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center py-12">
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">{/* Left Side - Signup Form */}</div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md: grid-cols-2 gap-4">
                  <div>
                    ...
                    Email Address</input>
                    <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />type="email"
                      name="email"
                      value={formData.email}
                      placeholder="Enter your email"</Mail>
                    Company Name</Mail>
                    <Building className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />name="company"
                      value={formData.company}
                      placeholder="Enter your company name"</Building>
                    Phone Number</Building>
                    <Phone className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />type="tel"
                      name="phone"
                      value={formData.phone}
                      placeholder="Enter your phone number"</Phone>
                    Password</Phone>
                    <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />type={showPassword ? 'text' : 'password'}
                      ...
                      className="w-full pl-10 pr-12 py-3 bg-slate-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20"</Lock>
                      placeholder="Create a password"</Lock>
                    <button;
                      type="button")
                ...
                <div className="space-y-4">
                  <label className="flex items-start">type="checkbox"
                      name="agreeToTerms"</label>
            {/* Left Side - Signup Form */}</label>
...
/>Last Name,
                        ...
                        placeholder="Enter your last name" Email Address;</input>
          <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />type="email" name="email"
                      value={formData.email}</Mail>
                      placeholder="Enter your email" Company Name;</Mail>
                    <Building className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />name="company" value={formData.company}</Building>
                      placeholder="Enter your company name" Phone Number;</Building>
                    <Phone className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />type="tel" name="phone"
                      value={formData.phone}</Phone>
                      placeholder="Enter your phone number" Password;</Phone>
                    <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />type={showPassword ? 'text' : 'password'}
                      name="password" value={formData.password}</Lock>
                      className="w-full pl-10 pr-12 py-3 bg-slate-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20" placeholder="Create a password"</Lock>
                    <button;
...
Confirm Password;
                      ...
                      {showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</Eye>
                <div className="space-y-4">
                  <label className="flex items-start">type="checkbox" name="agreeToTerms"
                      checked={formData.agreeToTerms}</label>
                      className="w-4 h-4 text-cyan-400 bg-slate-700 border-gray-600 rounded focus:ring-cyan-400 focus:ring-2 mt-1"</label>
                    <span className="ml-2 text-sm text-gray-300">I agree to the{' '}</span>
              ...
              <div className="mt-8 text-center">Already have an account?{' '}</div>
                  <a
                    href="/login"
            ...
            <div className="space-y-8">
                ...
                <p className="text-xl text-gray-300 mb-8">Join thousands of businesses already using our AI and IT solutions </p>
                  to achieve unprecedented growth and efficiency.</p>
              <div className="space-y-6">{features.map((feature, index) => (</div>
              ...
              <div className="bg-gradient-to-r from-indigo-900/50 to-purple-900/50 rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">{benefits.map((benefit, index) => (</div>
