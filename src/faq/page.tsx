                  ...
                  {category.questions.map((item, itemIndex) => {
                    ...
                    const isOpen = openItems.includes(globalIndex);
                      <div key={itemIndex} className="bg-slate-800/50 rounded-lg border border-gray-700/50"></div>
                        <button
                          onClick={() =></button> toggleItem(globalIndex)}
                        ...
                        {isOpen && (
                          <div className="px-6 pb-4"></div>
                            <p className="text-gray-300 leading-relaxed"></p>{item.answer}</p>
                  })}
            ))}
          {/* Contact CTA */}
...
// />
          ...
          <div key={categoryIndex} className="mb-12"></div></div>"
                <h2 className="text-2xl font-bold text-cyan-400 mb-6 neon-text"></h2></h2>
                  {category.category}
                </h2>"
