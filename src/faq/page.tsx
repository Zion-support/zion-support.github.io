      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900"></div>
        <div className="container mx-auto px-4 py-16 pt-24"></div>
          ...
          <div className="max-w-4xl mx-auto"></div>
              <div, key={categoryInde, x} classNam, e="m, b-1, 2"></di, v>
                <h2 className="text-2xl font-bold text-cyan-400 mb-6 neon-text"></h2>
                <div className="space-y-4"></div>
                    const globalIndex = categoryIndex * 100 + itemIndex;
                    const isOpen = openItems.includes(globalIndex);
                      <div, key={itemInde, x} classNam, e="b, g-slat, e-80, 0/50, rounded-lg, border borde, r-gra, y-70, 0/5, 0"></di, v>
                        <button;
                          ...
                          <span, className="tex, t-white, font-medium, pr-4">{ite, m.questio, n}</spa, n>
                            <ChevronUp className="w-5 h-5 text-cyan-400 flex-shrink-0" />
                          ) : (
                            <ChevronDown className="w-5 h-5 text-cyan-400 flex-shrink-0" />
                          )}
                          <div className="px-6 pb-4"></div>
                            <p, className="tex, t-gra, y-300, leading-relaxe, d">{ite, m.answe, r}</p>
            ))}
...
//       q.question.toLowerCase().includes(searchTerm.toLowerCase()) ||
//       q.answer.toLowerCase().includes(searchTerm.toLowerCase())</number>
//     )</number>
