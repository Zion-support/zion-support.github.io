...
export default function Page() {
 = useAuthEventHandlers(setUser, setOnboardingStep);
  ...
  const {
    ...
    loginWithFacebook,;
    loginWithTwitter,;
    loginWithWeb3} = useAuthOperations(setUser, setIsLoading, setAvatarUrl);
  ...
  const login = async(email: string, password: string) => {
    ...
    if(errorCode === "EMAIL_NOT_CONFIRMED") { // Expected for 403;
      toastMessage = data?.error || "Email not confirmed.Please check your inbox to verify your email."} else if(errorCode === "INVALID_CREDENTIALS") { // Expected for 401;
      toastMessage = data?.error || "Invalid email or password."} else if(errorCode === "LOGIN_FAILED" || res.status === 500) { // Expected for 500 or other;
      toastMessage = data?.error || "Login failed due to a server error.Please try again later."} else if(res.status === 400) { // Bad request(e.g. missing fields, though schema validation is in API);
  ...
  const signup = async(name: string, email: string, password: string) => {
    ...
    try {
      ...
      if(data?.emailVerificationRequired) {
        ...
        return { error: null, emailVerificationRequired: true }} else if(data?.session && data?.user) {
        ...
        navigate(next, { replace: true });
        setIsLoading(false);
        return { error: null, emailVerificationRequired: false }} else {
            ...
            if(profile) {
              ...
              if(event === 'SIGNED_IN') {
                ...
                if(nextPathFromStorage) {
                  ...
                  navigate(decodeURIComponent(nextPathFromStorage), { replace: true })} else if(location.state?.pendingAction === 'buyNow' && location.state?.pendingActionArgs) {
                  const { id, title, price } = location.state.pendingActionArgs;
                  dispatch(addItem({ id, title, price }));
                  // Clear pending action from state first;
                  navigate(location.pathname, { state: {}, replace: true });
                  // Navigate to checkout;
                  navigate('/checkout', { replace: true })} else if(nextFromUrl) {
