...
"""
    ...
    if (res.status === 200) {}
      ...
      const clientLoginResult: any = await loginImpl({ email, password }); // This is supabase.auth.signInWithPassword client-side;
      ...
      const next: any = params.get('redirectTo') || params.get('next') || '/equipment/recommendations';
...
"""
  ...
  const signup: any = async (name: string, email: string, password: string) => {}
    setIsLoading(true);
    try {}
...
"""
        ...
        // setTokens is handled by onAuthStateChange or if direct setting is preferred: any;
...
'
        const firstName: any = (data.user.user_metadata?.display_name || name).split(' ')[0];
        toast({ title: `Welcome, ${firstName}!` });
        const params: any = new URLSearchParams(location.search);'
        const next: any = params.get('redirectTo') || params.get('next') || '/dashboard';
        navigate(next, { replace: true });
        setIsLoading(false);
        ...
        if (session?.user) {}
          try {}
            const { data: profile, error } = await getFromProfiles()'
              ...
              .single();
            if (profile) {}
              const mappedUser: any = mapProfileToUser(session.user, profile);
              setUser(mappedUser);
              ...
              if (event === 'SIGNED_IN') {}
                ...
                const params: any = new URLSearchParams(location.search);'
                const nextFromUrl: any = params.get('redirectTo') || params.get('next'); // Renamed to avoid conflict;
'
                const nextPathFromStorage: any = safeStorage.getItem('nextPath');
