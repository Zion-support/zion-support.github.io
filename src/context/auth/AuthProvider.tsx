...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    ...
    if (res.status === 400) {
      ...
      toast({
        title: 'Login Failed',
        description: data?.error || 'Missing email or password',
        variant: 'destructive',
      });
    ...
    if (clientLoginResult?.error) {
      ...
      return {
        error: clientLoginResult.error?.message || 'Client-side login failed.',
      };
    }
  ...
  const signup = async (email, password, userData) => {
    ...
    if (!result?.error) {
      ...
      if (!loginResult.error) {
        ...
        const params = new URLSearchParams(location.search);
        const next =
          params.get('redirectTo') || params.get('next') || '/dashboard';
        navigate(next, { replace: true });
      }
    }
    return result;
        ...
        try {
          ...
          if (profile) {
            ...
            if (event === 'SIGNED_IN') {
              ...
              // --- BEGIN MODIFICATION ---
              if (
                location.state?.pendingAction === 'buyNow' &&
