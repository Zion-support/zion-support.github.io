import React, { createContext, useContext, useReducer, useEffect } from 'react';'import { safeStorage } from '@/utils/safeStorage';'import { useAuth } from '@/hooks/useAuth';'import { getCartKey, mergeCartItems } from '@/utils/cartUtils';';'const initialState = { items: [] };
;
function cartReducer(state, action) {

  switch (action.type) {

    case 'ADD_ITEM': {''      const existing = state.items.find(i => i.id === action.payload.id);
      let items;
      if (existing) {
...
export function useCart() {
  ...
  if (!ctx) {
    ...
    throw new Error('useCart must be used within a CartProvider');'  }'  return ctx;
}
;
