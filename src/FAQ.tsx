...
const FAQ: React.FC = () => {const [openItems, setOpenItems] = useState<number[]>([])
  const faqItems = [
    {
      id: 1,
  ...
  const toggleItem = (id: number) => {setOpenItems(prev => 
      prev.includes(id) 
...
export default function FAQ() {const [openItems, setOpenItems] = useState<Set<number>>(new Set())
  const [selectedCategory, setSelectedCategory] = useState<string>("All")
  const toggleItem = (index: number) => {
    const newOpenItems = new Set(openItems)
    if (newOpenItems.has(index)) {
      newOpenItems.delete(index)} else {newOpenItems.add(index)}
...
export default function FAQ() {const faqData = [
    {
      category: "General",
      icon: Search,
      questions: [
