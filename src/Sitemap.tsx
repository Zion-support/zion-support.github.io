...
const sitemapSections: SitemapSection[] = [],
  {}
    ...
    icon: <Home className="w-6 h-6" />,"    links: [],
      {}
        title: 'Home',
        href: '/',
  ...
  {}
    ...
    icon: <Zap className="w-6 h-6" />,"    links: [],
      {}
  ...
  {}
    ...
    icon: <BookOpen className="w-6 h-6" />,"    links: [],
      {}
        title: 'Help Center',
        href: '/help',
  ...
  {}
    ...
    icon: <Globe className="w-6 h-6" />,"    links: [],
      {}
        title: 'Careers',
        href: '/careers',
...
import React from 'react'
import { SEO } from "@/components/SEO""import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card""import { Link } from "react-router-dom""import {}
  Home, 
  Users, 
  Briefcase, 
  ShoppingCart, 
  ...
  const sitemapSections = []
    ...
    {}
      title: "Marketplace","      icon: <ShoppingCart className="w-5 h-5" />,"      links: [],
        { name: "Marketplace", path: "/marketplace", description: "Browse all products and services" },"        { name: "Categories", path: "/categories", description: "Browse by category" },"        { name: "Services", path: "/services", description: "Professional services" },"        { name: "Equipment", path: "/equipment", description: "Tech equipment and hardware" },"        { name: "Green IT", path: "/green-it", description: "Sustainable technology solutions" }"      ]
    },
    {}
      title: "Talent & Community","      icon: <Users className="w-5 h-5" />,"      links: [],
        { name: "Talent Directory", path: "/talent", description: "Find skilled professionals" },"        { name: "Community", path: "/community", description: "Join our community" },"        { name: "AI Matcher", path: "/match", description: "AI-powered talent matching" },"        { name: "Zion Hire AI", path: "/zion-hire-ai", description: "AI recruitment platform" }"      ]
    },
    {}
      title: "Enterprise Solutions","      icon: <Building className="w-5 h-5" />,"      links: [],
        { name: "Enterprise Plans", path: "/enterprise", description: "Enterprise solutions" },"        { name: "IT Onsite Services", path: "/it-onsite-services", description: "On-site IT support" },"        { name: "Request Quote", path: "/request-quote", description: "Get a custom quote" }"      ]
    },
    {}
      title: "Developer Resources","      icon: <Code className="w-5 h-5" />,"      links: [],
        { name: "Developer Portal", path: "/developers", description: "Developer tools and APIs" },"        { name: "API Documentation", path: "/api-docs", description: "API reference and guides" },"        { name: "Sample Code", path: "/api-samples", description: "Code examples and tutorials" }"      ]
    },
    {}
      title: "User Dashboard","      icon: <Settings className="w-5 h-5" />,"      links: [],
        { name: "Dashboard", path: "/dashboard", description: "Main user dashboard" },"        { name: "Messages", path: "/messages", description: "Communication center" },"        { name: "Notifications", path: "/notifications", description: "System alerts" },"        { name: "Profile", path: "/profile", description: "User profile management" },"        { name: "Account Settings", path: "/account-settings", description: "Account configuration" }"      ]
    },
    {}
      title: "Authentication","      icon: <Shield className="w-5 h-5" />,"      links: [],
        { name: "Login", path: "/login", description: "Sign in to your account" },"        { name: "Sign Up", path: "/signup", description: "Create new account" },"        { name: "Forgot Password", path: "/forgot-password", description: "Reset password" }"      ]
    },
    {}
      title: "Legal & Support","      icon: <FileText className="w-5 h-5" />,"      links: [],
        { name: "Privacy Policy", path: "/privacy", description: "Privacy and data protection" },"        { name: "Terms of Service", path: "/terms", description: "Terms and conditions" },"        { name: "Help Center", path: "/help", description: "Support and documentation" }"      ]
    }
  ]
import { motion } from 'framer-motion'
import {}
...
export default function Sitemap() {}
  ...
  const renderPageSection = (title: string, pages: any[], IconComponent: any) => (,
    <motion.div</motion>
      ...
      <h2 className="text-2xl font-bold text-white mb-6 flex items-center">"        <IconComponent className="w-6 h-6 text-zion-cyan mr-3" />"        {title}
      </h2>;
      <ul className="space-y-3">"        {pages.map((page) => (
          <li key={page.href}>;
            <$2 />;
              to={page.href} 
              className="text-zion-slate-light hover: text-zion-cyan transition-colors flex items-center group""            >,
              <span className="w-2 h-2 bg-zion-cyan rounded-full mr-3 group-hover:bg-zion-cyan transition-colors"></span>"              <span className="font-medium">{page.name}</span>"              <span className="ml-2 text-sm text-zion-slate-light opacity-75">- {page.description}</span>"            </Link>;
          </li>;
        ))}
      </ul>;
    </motion.div>;
  ...
  return();
    <></>
      <SEO</SEO>
        title="Sitemap | Zion AI Marketplace""        description="Human readable sitemap for all public routes""        keywords="sitemap, navigation""        canonical="https: //ziontechgroup.com/sitemap""      />;,
      <Header />;
      <main className="pt-32 pb-20">"        <div className="container mx-auto px-4">"          {/* Header Section */}
          <div className="text-center mb-16">"            <h1 className="text-4xl md: text-5xl font-bold text-gray-900 mb-6">"              Site Navigation Guide,
            </h1>;
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">"              Explore our comprehensive website structure. Find all pages, services, and resources organized by category for easy navigation;
            </p>;
          </div>;
          {/* Quick Links */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">"            {quickLinks.map((link, index) => (
              <$2 />;
                key={index}
                to={link.path}
                className={`${link.color} p-4 rounded-lg text-center hover:scale-105 transition-transform duration-200`}
              >
                <link.icon className="h-8 w-8 mx-auto mb-2" />"                <div className="font-semibold text-sm">{link.name}</div>"              </Link>;
            ))}
          </div>;
          {/* Sitemap Sections */}
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">"            {sitemapSections.map((section, index) => (
              <Card key={index} className="hover: shadow-lg transition-shadow duration-300">"                <CardHeader>;,
                  <div className="flex items-center space-x-3 mb-4">"                    <div className="w-10 h-10 bg-zion-blue/10 rounded-lg flex items-center justify-center">"                      <section.icon className="h-5 w-5 text-zion-blue" />"                    </div>;
                    <CardTitle className="text-xl">{section.title}</CardTitle>"                  </div>;
                </CardHeader>;
                <CardContent>;
                  <div className="space-y-3">"                    {section.links.map((link, linkIndex) => (
                      <div key={linkIndex} className="border-l-2 border-gray-200 pl-4">"                        <$2 />;
                          to={link.path}
                          className="block text-zion-blue hover: text-zion-blue-dark font-medium mb-1 transition-colors""                        >,
                          {link.name}
                        </Link>;
                        <p className="text-sm text-gray-600">{link.description}</p>"                      </div>;
                    ))}
                  </div>;
                </CardContent>;
              </Card>;
          ...
          <div className="mt-16 grid grid-cols-1 md: grid-cols-2 gap-8">"            <Card>;,
              <CardHeader>;
                <CardTitle className="flex items-center space-x-2">"                  <Globe className="h-5 w-5 text-zion-blue" />"                  <span>Site Information</span>;
                </CardTitle>;
              </CardHeader>;
              <CardContent>;
                <div className="space-y-3 text-sm text-gray-600">"                  <div className="flex justify-between">"                    <span>Total Pages: </span>;,
                    <span className="font-medium">50+</span>"                  </div>;
                  <div className="flex justify-between">"                    <span>Last Updated: </span>;,
                    <span className="font-medium">{new Date().toLocaleDateString()}</span>"                  </div>;
                  <div className="flex justify-between">"                    <span>Site Version: </span>;,
                    <span className="font-medium">2.0.0</span>"                  </div>;
                </div>;
              </CardContent>;
            </Card>;
            <Card>;
              <CardHeader>;
                <CardTitle className="flex items-center space-x-2">"                  <MessageSquare className="h-5 w-5 text-zion-blue" />"                  <span>Need Help?</span>;
                </CardTitle>;
              </CardHeader>;
              <CardContent>;
                <p className="text-sm text-gray-600 mb-4">"                  Can't find what you're looking for? Our support team is here to help;
                </p>;
                <div className="space-y-2">"                  <$2 />;
                    to="/contact""                    className="block text-zion-blue hover: text-zion-blue-dark text-sm font-medium""                  >,
                    Contact Support →
                  </Link>;
                  <$2 />;
                    to="/help-center""                    className="block text-zion-blue hover: text-zion-blue-dark text-sm font-medium""                  >,
                    Visit Help Center →
                  </Link>;
                </div>;
              </CardContent>;
  ...
  const renderPageSection = (title: string, pages: any[], IconComponent: any) => (,
    <div key={title} className="mb-8">"      <h3 className="text-lg font-semibold text-white mb-4 flex items-center">"        <IconComponent className="w-5 h-5 mr-2 text-cyan-400" />"        {title}
      </h3>;
      <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">"        {pages.map((page) => (,
          <$2 />;
            key={page.path}
            to={page.path}
            ...
            <h4 className="font-medium text-white mb-2 group-hover:text-cyan-400 transition-colors duration-200">"              {page.title}
            </h4>;
            <p className="text-sm text-gray-400">{page.description}</p>"          </Link>;
        ))}
      </div>;
    </div>;
  )
  return();
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-blue-900 text-white">"      {/* Hero Section */}
      <section className="relative py-20 px-4 sm:px-6 lg:px-8">"        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(0,255,255,0.1),transparent_50%)]"></div>"        <div className="relative z-10 max-w-7xl mx-auto text-center">"          <h1 className="text-4xl md: text-6xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">"            Sitemap,
          </h1>;
          <p className="text-xl md: text-2xl text-gray-300 mb-8 max-w-4xl mx-auto">"            Navigate through all the pages and sections of our website to find exactly ,
            what you're looking for;
          </p>;
        </div>;
      </section>;
      {/* Sitemap Content */}
      <section className="py-16 px-4 sm:px-6 lg:px-8">"        <div className="max-w-7xl mx-auto">"          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">"            {/* Left Column */}
            <div>;
      ...
      {/* Quick Navigation */}
      <section className="py-16 px-4 sm: px-6 lg:px-8 bg-black/20">"        <div className="max-w-7xl mx-auto">"          <div className="text-center mb-12">"            <h2 className="text-3xl font-bold text-white mb-4">"              Quick Navigation,
            </h2>;
            <p className="text-gray-400">"              Popular destinations and frequently accessed pages
            </p>;
          </div>;
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-6">"            <$2 />;,
              to="/services""              className="group p-6 bg-gray-800/50 border border-gray-700 rounded-xl hover: border-cyan-500/50 transition-all duration-200""            >,
              <div className="w-12 h-12 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg flex items-center justify-center mb-4 group-hover: scale-110 transition-transform duration-300">"                <Briefcase className="w-6 h-6 text-white" />"              </div>;,
              <h3 className="text-lg font-semibold text-white mb-2">Our Services</h3>"              <p className="text-sm text-gray-400">Explore our comprehensive service offerings</p>"            </Link>;
            <$2 />;
              to="/pricing""              className="group p-6 bg-gray-800/50 border border-gray-700 rounded-xl hover: border-cyan-500/50 transition-all duration-200""            >,
              <div className="w-12 h-12 bg-gradient-to-r from-green-500 to-emerald-500 rounded-lg flex items-center justify-center mb-4 group-hover: scale-110 transition-transform duration-300">"                <DollarSign className="w-6 h-6 text-white" />"              </div>;,
              <h3 className="text-lg font-semibold text-white mb-2">Pricing Plans</h3>"              <p className="text-sm text-gray-400">Transparent pricing for all our services</p>"            </Link>;
            <$2 />;
              to="/contact""              className="group p-6 bg-gray-800/50 border border-gray-700 rounded-xl hover: border-cyan-500/50 transition-all duration-200""            >,
              <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg flex items-center justify-center mb-4 group-hover: scale-110 transition-transform duration-300">"                <Mail className="w-6 h-6 text-white" />"              </div>;,
              <h3 className="text-lg font-semibold text-white mb-2">Get in Touch</h3>"              <p className="text-sm text-gray-400">Contact our team for inquiries</p>"            </Link>;
            <$2 />;
              to="/careers""              className="group p-6 bg-gray-800/50 border border-gray-700 rounded-xl hover: border-cyan-500/50 transition-all duration-200""            >,
              <div className="w-12 h-12 bg-gradient-to-r from-orange-500 to-yellow-500 rounded-lg flex items-center justify-center mb-4 group-hover: scale-110 transition-transform duration-300">"                <Users className="w-6 h-6 text-white" />"              </div>;,
              <h3 className="text-lg font-semibold text-white mb-2">Join Our Team</h3>"              <p className="text-sm text-gray-400">Career opportunities at Zion Tech Group</p>"            </Link>;
          </div>;
        </div>;
      </section>;
      {/* CTA Section */}
      <section className="py-20 px-4 sm: px-6 lg:px-8">"        <div className="max-w-4xl mx-auto text-center">"          <h2 className="text-3xl font-bold text-white mb-6">"            Need Help Finding Something?,
          </h2>;
          <p className="text-xl text-gray-400 mb-8">"            Can't find what you're looking for? Our team is here to help you navigate 
            our website and find the information you need;
          </p>;
          <div className="flex flex-col sm: flex-row gap-4 justify-center">"            <$2 />;,
              to="/contact""              className="inline-flex items-center space-x-2 px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg text-white font-medium hover: from-cyan-600 hover:to-blue-700 transition-all duration-200""            >,
              <span>Contact Support</span>;
              <ArrowRight className="w-5 h-5" />"            </Link>;
            <$2 />;
              to="/help""              className="inline-flex items-center space-x-2 px-8 py-4 border border-cyan-500 text-cyan-400 rounded-lg font-medium hover: bg-cyan-500 hover:text-white transition-all duration-200""            >,
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900">"      <div className="max-w-6xl mx-auto px-4 py-12">"        {/* Header */}
        <motion.div</motion>
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-center mb-12""        >;
          <h1 className="text-4xl md: text-5xl font-bold text-white mb-4">"            Zion Tech Group Sitemap,
          </h1>;
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">"            Navigate through our comprehensive range of services, solutions, and resources;
...
const Sitemap = () => {}
  ...
  return();
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-blue-900 text-white">"      {/* Header Section */}
      <div className="pt-32 pb-16 px-4 sm: px-6 lg:px-8">"        <div className="max-w-7xl mx-auto text-center">"          <h1 className="text-4xl md:text-6xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent mb-6">"            Site Map,
          </h1>;
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">"            Explore all the pages and resources available on the Zion Tech Group website;
