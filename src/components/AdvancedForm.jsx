...
''''}} className='text-center p-8 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-700'>''''
  ...
  y: 20}} animate = {}
  { opacity: 1,
  y: 0`;`}} className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden ${className}`}>'''{/* Header */}''''
...
'`{/* Submit Button */}'`''`
        <motion.button type='submit' disabled={!isFormValid() || isSubmitting} className={`w-full py-3 px-6 rounded-lg font-medium text-white transition-all duration-200 flex items-center justify-center gap-2 ${!isFormValid() || isSubmitting''`;
            ...
            : 'bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 transform hover:scale-105'}`} whileHover={isFormValid() && !isSubmitting ? { scale: 1.02 } : { /* empty */ }} whileTap={isFormValid() && !isSubmitting ? { scale: 0.98 } : { /* empty */ }}>'''{isSubmitting ? (<>''''
