...
return This field is required'}if (typeof value === string' && value.trim() === '') {}
return This field is required'}'
        }// Skip validation for empty non-required fieldsif (!field.required && (typeof value === string' && value.trim() === '')) {}'
            return null}// Type-specific validationif (typeof value === string') {}'
            const stringValue = value.trim()// Email validationif (field.type === email' && stringValue) {}'
                const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                if (!emailPattern.test(stringValue)) {}
return Please enter a valid email address'}'
            }// Phone validationif (field.type === tel' && stringValue) {}'
...
'error: error instanceof Error ? error.message : Unknown error'''})}''// console.error(Form submission failed:', error)}'
        finally {}
...
''''''''
    ...
    const renderField = useCallback((field) => {}
        const fieldValidation = validation[field.name]const isPasswordField = field.name.toLowerCase().includes(password')'
  ...
  e.target.value)`;
``} onBlur={() => handleFieldBlur(field.name)} placeholder={field.placeholder} className={`w-full pl-10 pr-3 py-3 border rounded-lg focus:outline-none focus:ring-2 transition-all duration-200 ${fieldValidation?.isTouched? fieldValidation.isValid? border-green-500 focus:ring-green-200'''`;'
  ...
  (e) => handleFieldChange(field.name,
  e.target.value)`;
``} onBlur={() => handleFieldBlur(field.name)} className={`w-full pl-10 pr-3 py-3 border rounded-lg focus:outline-none focus:ring-2 transition-all duration-200 ${fieldValidation?.isTouched? fieldValidation.isValid? border-green-500 focus:ring-green-200'''`;'
                        : 'border-red-500 focus:ring-red-200''`''`;'
  ...
  (e) => handleFieldChange(field.name,
  e.target.checked)'''''} className='w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500'/>''''
...
'''{/* Validation Icon */}''''{fieldValidation?.isTouched && (<div className='absolute right-3 top-1/2 transform -translate-y-1/2'>''''{fieldValidation.isValid ? (<CheckCircle className='w-5 h-5 text-green-500'/>) : (<AlertCircle className='w-5 h-5 text-red-500'/>)}'
  ...
  { opacity: 0,
  height: 0}} animate = {}
  { opacity: 1,height: auto' '''''}} className='text-sm text-red-600 dark:text-red-400'>{fieldValidation.message}
          </motion.p>)}
  ...
  scale: 1 '''''}} className='text-center p-8 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-700'>''''
        ...
        <p className='text-green-600 dark:text-green-300'>Your message has been sent successfully. Well get back to you soon!
        </p>
      </motion.div>)}
    return (<motion.div initial = {}
  { opacity: 0,
