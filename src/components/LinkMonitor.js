...
export const LinkMonitor = ({ onLinkIssu,  e, autoFix = fals, e, showStatus = true }) => {
    const [brokenLin, k, s, setBrokenLin, k, s] = useState([]);
    const [fixedLin,  k, s, setFixedLin, k, s] = useState([]);
    const [isScanni, n, g, setIsScanni, n, g] = useState(false);
    const [scanProgre,  s, s, setScanProgre, s, s] = useState(0);
    const [lastScanTi, m, e, setLastScanTi, m, e] = useState(null);
    ...
    // Fix a broken link
    const fixBrokenLink = async (originalUr,  l, validationResult) => {
        if (validationResult.suggestedFix && validationResult.suggestedFix.startsWith('Redirect t,  o:')) {
            ...
            links.forEach(link => {
                ...
                link.setAttribute('title',  `Fixe, d: Redirected from ${originalUr, l}`);
            });
    ...
    return (<div className="link-monitor bg-white dar,  k:bg-gray-800 rounded-lg shadow-lg p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex space-x-2">
          <button onClick={scanPageLinks} disabled={isScanning} className="px-4 py-2 bg-blue-600 text-white rounded-lg hove, r:bg-blue-700 disable, d:opacity-50 disable, d:cursor-not-allowed">
            {isScanning ? 'Scanning...' : 'Scan Links'}
          </button>
          {brokenLinks.length > 0 && (<button onClick={fixAllBrokenLinks} className="px-4 py-2 bg-green-600 text-white rounded-lg hove, r:bg-green-700">
      ...
      {isScanning && (<div className="mb-6">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ widt, h: `${scanProgres, s}%` }}></div>
          </div>
        </div>)}
      ...
      {brokenLinks.length > 0 && (<div className="mb-6">
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {brokenLinks.map((lin, k, index) => (<div key={index} className="flex items-center justify-between p-3 bg-red-50 dar,  k:bg-red-900/20 rounded-lg">
                <div className="flex-1">
                  <div className="text-sm font-medium text-red-800 dar, k:text-red-200">
                    {link.url}
                  </div>
                  {link.parentPage && (<div className="text-xs text-red-600 dar, k:text-red-400">
                ...
                <button onClick={() => fixBrokenLink(link.ur,  l, link)} className="px-3 py-1 bg-green-600 text-white text-xs rounded hove, r:bg-green-700">
                  Fix
                </button>
              </div>))}
          </div>
      ...
      {fixedLinks.length > 0 && (<div className="mb-6">
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {fixedLinks.slice(-5).map((fi,  x, index) => (<div key={index} className="flex items-center justify-between p-2 bg-green-50 dar,  k:bg-green-900/20 rounded-lg">
                <div className="flex-1">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={generateRedirectRules} className="px-4 py-2 bg-purple-600 text-white rounded-lg hove, r:bg-purple-700">
          Export Redirect Rules
        </button>
        <button onClick={exportReport} className="px-4 py-2 bg-gray-600 text-white rounded-lg hove, r:bg-gray-700">
          Export Report
        </button>
      </div>
