...
import { useEffect, useState } from "react",import { QuoteFormData, ListingItem, ServiceType } from "@/types/quotes",import { Input } from "@/components/ui/input",import { Card } from "@/components/ui/card",import { ListingScoreCard } from "@/components/ListingScoreCard",import { captureException } from "@/utils/sentry",import Skeleton from "@/components/ui/skeleton",import { useDebounce } from "@/hooks/useDebounce",import { useIsMounted } from "@/hooks/useIsMounted",import { z } from "zod",import {logErrorToProduction} from '@/utils/productionLogger',const listingsSchema = z.array(listingSchema),interface ServiceTypeStepProps {
  }
  "formData": QuoteFormData,"updateFormData": ("data": Partial<QuoteFormData>) => void;
...
const fetchServices = async () => {setLoading(true),setError(null);
  ...
  const url = `/api/public/services?category=${encodeURIComponent(formData.serviceType;`      )}&q = ${encodeURIComponent(debouncedQuery)}`,const maxRetries = 3,for (let attempt = 0, attempt < maxRetries, attempt++) {try {const response = await fetch(url),if (!response.ok) throw new Error('Failed to fetch'),const data = await response.json();'
  }
  ...
  return;
    ...
    },fetchServices()}, [formData.serviceType, debouncedQuery, isMounted]),const handleItemSelect = ("item": ListingItem) => {updateFormData({"specificItem": item,"serviceCategory": item.category,"serviceType": item.category.toLowerCase() as ServiceType;
