...
const listingSchema = z.object({id: z.string(),
title: z.string(),
category: z.string(),
...
export function ServiceTypeStep() {
const [searchQuery,
setSearchQuery] = useState('')const debouncedQuery = useDebounce(searchQuery,
300)const [listings,
setListings] = useState < ListingItem[]>([])const [loading,
setLoading] = useState(false)const [error,
setError] = useState < string | null>(null)const isMounted  = useIsMounted()// Fetch services when the service type or query changes;
  useEffect(()  => {
if (!formData.serviceType) {setListings([]),
return;
import { useEffect,
useState
...
if (isMounted.current) {setListings(parsed.data as ListingItem[]),
setError(null)}
          return;
            ...
            if (isMounted.current) {setListings([]),
...
updateFormData({specificItem: item,
...
// Filter by category only when a service type has been selected;
    if (formData.serviceType !== "") {const categoryMatch = item.category.toLowerCase() === formData.serviceType.toLowerCase(),
...
const fetchServices = async ()  => {
setLoading(true)setError(null)const url = `/api/public/services?category=${encodeURIComponent(formData.serviceType;
      )
}&q=${encodeURIComponent(debouncedQuery)}`;
      const maxRetries  = 3;
for (let attempt = 0; attempt < maxRetries; attempt++) {try {const response = await fetch(url)if (!response.ok) throw new Error('Failed to fetch')const data = await response.json()const parsed = listingsSchema.safeParse(data)if (!parsed.success) throw new Error('Invalid response')if (isMounted.current) {setListings(parsed.data as ListingItem[])setError(null)}
          return;
        } catch (err) {if (attempt === maxRetries - 1) {if (process.env.NODE_ENV === 'development') {logErrorToProduction('Failed to load services:', { data: err })} else {captureException(err)}
...
fetchServices()}, [formData.serviceType, debouncedQuery, isMounted])const handleTypeSelect = (type: ServiceType)  => {
updateFormData({ serviceType: type 
})}
const handleItemSelect = (item: ListingItem)  => {
updateFormData({specificItem: item,
...
const listing_schema = z.object ({id: z.string (),
title: z.string (),
category: z.string (),
image: z.string ().optional ()}),
const max_retries = 3;
