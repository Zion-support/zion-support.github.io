...
import { z } from "zod",
import {logErrorToProduction} from '@/utils/productionLogger',const listingSchema = z.object({
  id: z.string(),
  title: z.string(),
  category: z.string(),
...
export function ServiceTypeStep({ formData, updateFormData }: ServiceTypeStepProps) {
  ...
  const isMounted = useIsMounted(),import Skeleton from "@/components/ui/skeleton";
...
if (formData.serviceType !== "") {
  ...
  const handleTypeSelect = (type: ServiceType) => {updateFormData({ serviceType: type})
  };
  ...
  const filteredListings = sourceListings.filter(item => {_// Filter by category only when a service type has been selected,
if (formData.serviceType !== "") {
      const categoryMatch = item.category.toLowerCase() === formData.serviceType.toLowerCase();
      if (!categoryMatch) return false}
