import { motion     } from 'framer-motioninterface EnhancedLoadingSpinnerProps {size?: sm | 'md' | 'lg' text?: string className?: string}'
export default function EnhancedLoadingSpinner({
  ...
  return (<div className={`flex flex-col items-center justify-center space-y-4 ${className}`}>
      ...
      >{/* Outer ring */}
        ...
        />{/* Center dot */}
        <motion.div
          className='absolute inset-0 flex items-center justify-centeranimate={{ scale: [1, 1.2, 1] }}'
      ...
      <div className='flex space-x-1'>
        {[0, 1, 2].map((i) => (
          <motion.div
            key={i}
            className='w-2 h-2 bg-blue-600 rounded-fullanimate={{ scale: [1, 1.5, 1],'
              opacity: [0.5, 1, 0.5]
            }}
