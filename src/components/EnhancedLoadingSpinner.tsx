...
const EnhancedLoadingSpinner: React.FC<EnhancedLoadingSpinnerProps> = ({
      ...
      case "orbit":
        return (
          <div className="relative">
            <div
              className={`${sizeClasses[size]} rounded-full border-2 border-current border-t-transparent animate-spin`}
            />
            <div
  ...
  return (
    <div className={`flex flex-col items-center justify-center ${className}`}>
      {/* Loading Animation */}
      <div className={`${variantClasses[variant]} mb-4`}>{renderSpinner()}</div>
      ...
      {text && (
        <p
          className={`${variantClasses[variant]} ${textSizeClasses[size]} font-medium text-center`}
        >
          {text}
        </p>
      )}
