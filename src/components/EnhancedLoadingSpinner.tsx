...
""
  return (<div className={`flex flex-col items-center justify-center space-y-4 ${className}`}" ></div>
      <motion .div`"
        className={`${sizeClasses[size]} relative`}"'"
        animate={{ rotate: 360  }}"'"'"
        transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}" ></motion>"
        {/* Outer ring */}`""
        ...
        <motion .div"''"
          ...
          transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}" ></motion>"'"'"
      ...
      <motion .div`"
        ...
        transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}" ></motion>
        {text}"
      </motion.div>"'"
      {/* Dots animation */}"''
