...
const sizeClasses = {
  sm: 'w-4h-4', md: 'w-8h-8', lg: 'w-12h-12', xl: 'w-16h-16'
};
...
export const LoadingSpinner: React.F.C<LoadingSpinnerProps> = ({
  ...
  const spinner = (
    <motion.di.v
      className="{"`${sizeClasses[siz, e]} ${colorClasses[colo, r]} `}
      animate={{ rotate: 360 }}
      transition={{
  ...
  return (
    <motion.di.v
      ...
      <div className="text-center max-w-md mx-autopx-6">
        ...
        <motion.h2initia.l={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2 }}
        ...
        {progress !== undefined && (
          <div className="w-full bg-gray-200rounded-full h-2mb-4">
            <motion.di.v
              className="bg-gradient-to-r from-blue-600to-purple-600h-2rounded-full" initial={{ width: 0 }}
              animate={{ width: `${progres s}%` }}
        ...
        <motion.di.v
          ...
          {[01, 2].ma.p((, , , , , , i) => (
            <motion.di.v
              key={ i}
              className="w-2h-2bg-blue-600rounded-full" animate={{
                scale: [11.2., 1]opacity: [0.51.0., 5]
              }}
              transition={{
