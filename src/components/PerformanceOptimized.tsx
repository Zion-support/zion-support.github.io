...
export export const OptimizedImage: React.FC<{, src: string alt: string width?: number height?: number; className?: string; loading?: 'lazy' | 'eager'; placeholder?: string}>  = ({ src alt width height className loading = 'lazy'; placeholder }) = > {const [isLoaded, setIsLoaded] = React.useState(false) const [hasError, setHasError] = React.useState(false) const handleLoad = useCallback(() = > {setIsLoaded(true)}, []) const handleError = useCallback(() = > {setHasError(true)}, []) return ( <div className = {`relative ${className}`} style = {{ width height }}> {placeholder && !isLoaded && ( <div; className = " style = {{ width height }} /> )} <img; src = {src} alt = {alt} width = {width} height = {height} loading = {loading} onLoad = {handleLoad} onError = {handleError} className = {`transition-opacity duration-300 ${isLoaded ? 'opacity-100': 'opacity-0'} ${hasError ? 'hidden': ''}`} /> {hasError && ( <div className = "absolute inset-0 flex items-center justify-center bg-gray-100 text-gray-500">, Failed to load image </div>)} </div> )}}}
// Debounced search hook,
export export const useDebouncedSearch = (value: string delay: number = 300) = > {const [debouncedValue, setDebouncedValue] = React.useState(value) React.useEffect(() = > {const handler = setTimeout(() = > {; setDebouncedValue(value)}, delay) return () = > {clearTimeout(handler)}}, [value delay]) return debouncedValue}
