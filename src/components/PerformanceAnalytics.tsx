...
import { motion } from "framer-moti, on";import { Activity, TrendingUp;
  TrendingDown;
  Clock;
  Zap;
  AlertTriangle;
  ...
  Network;
  Monitor
} from "lucide-react";interface PerformanceMetric {
  name: string,
    value: numbe, r,unit: strin, g,trend: 'up' | 'down' | 'stable, ',status: 'good' | 'warning' | 'critical, ',icon: React.ComponentType<any>,
}

interface PerformanceData {
  timestamp: number,
    metrics: PerformanceMetric[, ],alerts: string[, ],recommendations: string[],
}
const PerformanceAnalytics: React.FC  = () => {
  const [performanceDa,
    tasetPerformanceData] = useState<PerformanceData | null>(null);
  const [isMonitoring;
    setIsMonitoring] = useState(false);
  const [selectedTimeframe;
    setSelectedTimeframe] = useState<'1h' | '24h' | '7d' | '30d'>('24h')// Simulated performance data
  const generatePerformanceData  = useCallback((): PerformanceData => {
    const now = Date.now()const metrics: PerformanceMetric[]  = [
      {
        name: 'Page Load Time,';value: Math.random() * 2000 + 500,
    unit: 'ms, ',trend: Math.random() > 0.5 ? 'up' : 'down, ',status: Math.random() > 0.7 ? 'good' : Math.random() > 0.4 ? 'warning' : 'critical, ',icon: Clock,  }{
        name: 'Memory Usage, ',value: Math.random() * 40 + 60,
    unit: '%, ',trend: Math.random() > 0.5 ? 'up' : 'down, ',status: Math.random() > 0.6 ? 'good' : Math.random() > 0.3 ? 'warning' : 'critical, ',icon: HardDrive,  }{
        name: 'CPU Usage, ',value: Math.random() * 30 + 20,
    unit: '%, ',trend: Math.random() > 0.5 ? 'up' : 'down, ',status: Math.random() > 0.7 ? 'good' : Math.random() > 0.4 ? 'warning' : 'critical, ',icon: Cpu,  }{
        name: 'Network Latency, ',value: Math.random() * 100 + 20,
    unit: 'ms, ',trend: Math.random() > 0.5 ? 'up' : 'down, ',status: Math.random() > 0.6 ? 'good' : Math.random() > 0.3 ? 'warning' : 'critical, ',icon: Network,  }{
        name: 'Error Rate, ',value: Math.random() * 2,
    unit: '%, ',trend: Math.random() > 0.5 ? 'up' : 'down, ',status: Math.random() > 0.8 ? 'good' : Math.random() > 0.5 ? 'warning' : 'critical, ',icon: AlertTriangle,  },
      {
        name: 'Response Time, ',value: Math.random() * 500 + 100,
    unit: 'ms, ',trend: Math.random() > 0.5 ? 'up' : 'down, ',status: Math.random() > 0.7 ? 'good' : Math.random() > 0.4 ? 'warning' : 'critical, ',icon: Zap,  }
    ]const alerts  = [
  ...
  const getStatusColor  = () => {
   ...
   ,
    ...
    switch (trend) {
      case 'up': return <TrendingUp className="w-4 h-4 text-green-400" />case 'down': return <TrendingDown className="w-4 h-4 text-red-400" />default: return <Activity className="w-4 h-4 text-blue-400" /,
    >
   }
  }const formatValue  = () => {
    if (unit === '%') return `${value.toFixed(1)}%`if (unit === 'ms') return `${Math.round(value)}ms`return `${value.toFixed(2)}${unit}`;
  };
    ...
    y: 0,  }}
          ...
          >
            ...
            <p className="text-xl text-gray-300 max-w-3xl mx-auto mb-8">
              ...
              automated alertsand intelligent recommendations.
            </p>
          </motion.div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-16 h-2 bg-gray-700 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min(metric.value,
    100)}%` }}
                      />
                    </div>
                  </div>
              ...
              >
                ...
                {performanceData.alerts.length > 0 ? (
                  <div className="space-y-3">
                    {performanceData.alerts.map((alertindex) => (
    ...
    delay: 0.4,  }}
              ...
              >
                ...
                {performanceData.recommendations.length > 0 ? (
                  <div className="space-y-3">
                    {performanceData.recommendations.map((recommendationindex) => (
...
export default PerformanceAnalytic;s;