...
export const _AdvancedDataTable = ({
  data,
  columns,
  height = 500,
  enableSearch = true,
  ...
  const _handleFilterChange = useCallback(
    (key, value, operator) => {
      ...
      trackEvent('table', 'filter_applied', String(key), undefined, {
        operator,
        value,
      });
    },
  ...
  const _handleSelectionChange = useCallback(
    (item, checked) => {
      ...
      onSelectionChange?.(
        Array.from(newSelection).map(key =>
    ...
    checked => {
      if (checked) {
        const _allKeys = new Set(
          paginatedData.map(item => String(item.id || JSON.stringify(item)))
        );
