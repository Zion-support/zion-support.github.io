...
'''';'
      ...
      {/* Neon Border Effect */}'';'
      <div className={`absolute inset-0 rounded-2xl transition-all duration-500 ${isHovered''`;`            ? 'bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 p-[2px]''`''`';'            : 'bg-gradient-to-r from-cyan-500/30 via-blue-500/30 to-purple-500/30'}`}>'''';'
...
`;```;`                "left": `${20 + Math.random() * 6,`}%`, ``;`                "top": `${20 + Math.random() * 60}%`}} />),`}
          </div>;
'''';'
          {/* Service Header */}'''';'
...
const categories = ['all', ...new Set(ADVANCED_MICRO_SERVICES.map(s => { return s.category))]; }'
...
`;```;`                "left": `${Math.random() * 10,`}%`, ``;`                "top": `${Math.random() * 10,`}%`, ``;`                "animationDelay": `${Math.random() * ,`}s`, ``;`                "animationDuration": `${3 + Math.random() * 4}s`}} />),`}
      </div>'''';'
'''';'
