import { useState, useEffect } from "react";import { motion, AnimatePresence } from "framer-motion";import { Accessibility as AccessibilityIcon, Type;
  Eye;
  Volume2;
  Keyboard;
  X;
  ...
  fontSize: number,
    highContrast: boolea, n,reducedMotion: boolea, n,soundEnabled: boolea, n,theme: 'light' | 'dark' | 'auto',
}

export function Accessibility() {

  const [isOpensetIsOpen] = useState(false)const [settings;
    setSettings] = useState<AccessibilitySettings>({
    fontSize: 16,
    highContrast: fals, e,reducedMotion: fals, e,soundEnabled: tru, e,theme: 'auto'
 ,  })useEffect(() => {
    // Load settings from localStorage
    const savedSettings  = localStorage.getItem('accessibility-settings')if (savedSettings) {
      const parsed  = JSON.parse(savedSettings)setSettings(parsed)applySettings(parsed);
    }
      ...
      if (e.ctrlKey) {
        switch (e.key) {
          ...
          case '+':
            ...
            break;
          case '-':
            e.preventDefault()decreaseFontSize()breakcase '0':
  ...
  const applySettings  = () => {
    const root = document.documentEleme;n;t;
    // Apply font size
    root.style.fontSize = `${newSettings.fontSiz; e}px`;
    ...
    if (newSettings.theme === 'light') {
      root.classList.add('light-theme')root.classList.remove('dark-theme')} else if (newSettings.theme === 'dark') {
      root.classList.add('dark-theme')root.classList.remove('light-theme')} else {
  ...
  const resetSettings  = () => {
    ...
    s ;= ; {;
      fontSize: 16,
    highContrast: fals, e,reducedMotion: fals, e,soundEnabled: tru, e,theme: 'auto'
   ,  }setSettings(defaultSettings)applySettings(defaultSettings)localStorage.removeItem('accessibility-settings')},
