      ...
      <div className={`bg-gradient-to-br from-indigo-700 via-purple-700 to-pink-700 rounded-2xl p-8 text-white ${className}`}>
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            <span className="w-8 h-8 text-yellow-300 animate-pulse">✨</span>
            <h2 className="text-3xl font-bold">Breakthrough Articles Just Published</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {featuredArticles.map((article) => (
              <div key={article.id} className="bg-white/10 backdrop-blur-lg rounded-xl p-6 hover:bg-white/20 transition-all hover:scale-105">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link to={article.link} className="text-yellow-300 font-semibold hover:text-yellow-200 flex items-center gap-1">
                    Read <span className="w-4 h-4">→</span>
                  </Link>
                </div>
              </div>
            ))}
      ...
      <div className={`bg-white rounded-2xl shadow-2xl overflow-hidden ${className}`}>
        ...
        <div className="p-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredArticles.map((article) => (
              <Link key={article.id} to={article.link} className="group border border-gray-200 rounded-xl p-5 hover:shadow-xl hover:border-indigo-300 transition-all">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="w-4 h-4 text-indigo-600 group-hover:translate-x-1 transition-transform">→</span>
                </div>
              </Link>
            ))}
          </div>
  ...
  return (
    <div className={`bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 md:p-8 text-white ${className}`}>
      <div className="flex items-center gap-3 mb-4">
        <span className="w-6 h-6 text-yellow-300">✨</span>
