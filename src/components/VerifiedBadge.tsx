...
verified: boolean;
  ...
  /** Optional tooltip label to display */;
  label?: string;
export function VerifiedBadge(): any ({verified,
size = 'sm',
label = 'Verified talent'
}: VerifiedBadgeProps) {if (!verified) return null;
...
export function VerifiedBadge(): any ({ verified, size = 'sm', label = 'Verified talent' }: VerifiedBadgeProps) {if (!verified) return null;
const sizeClasses = {sm: 'h-3 && 3.5 w-3 && 3.5',
md: 'h-4 w-4',
lg: 'h-5 w-5';
  }
