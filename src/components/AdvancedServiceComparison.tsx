...
export const _AdvancedServiceComparison = () => {
  ...
  const _filteredServices = useMemo(() => {
    return mockServices.filter(service => {
      ...
      const _matchesComplexity =
        filters.complexity === 'all' ||
        service.complexity === filters.complexity;
      const _matchesTechnology =
        filters.technology === 'all' ||
        service.technology.includes(filters.technology);
      const _matchesSearch = service.name
        .toLowerCase()
    ...
    <div className='max-w-7xl mx-auto p-6'>
      ...
      >
        ...
        <p className='text-xl text-zion-slate-light max-w-3xl mx-auto'>
          Compare our cutting-edge services across different tiers, complexity
          levels, and pricing to find the perfect solution for your business
          needs.
        </p>
      </motion.div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4'>
          ...
          <div>
            ...
            <select
              ...
              onChange={e =>
                setFilters(prev => ({ ...prev, complexity: e.target.value }))
              }
              className='w-full p-2 border border-zion-slate-light rounded-md'
            >
      ...
      >
        {filteredServices.map((service, index) => (
          ...
          >
            <Card
              className={`h-full transition-all duration-300 hover:shadow-xl ${selectedServices.includes(service.id) ? 'ring-2 ring-zion-cyan' : ''}`}
            >
                  ...
                  <div>
                    ...
                    <ul className='space-y-1'>
                      {service.features.slice(0, 3).map((feature, idx) => (
                        <li
                          key={idx}
                          className='text-sm text-zion-slate-light flex items-center gap-2'
                        >
        ...
        >
              ...
              <div className='text-2xl font-bold text-zion-green'>
                {
                  filteredServices
                    .filter(s => selectedServices.includes(s.id))
                    .filter(s => s.complexity === 'Enterprise').length
                }
              </div>
          ...
          <div className='mt-6 text-center'>
            <Button
              ...
              onClick={() =>
                window.open(
                  'mailto:kleber@ziontechgroup.com?subject=Service Comparison Inquiry',
                  '_blank'
                )
              }
            >
      ...
      >
        <div className='bg-gradient-to-r from-zion-purple via-zion-blue-light to-zion-cyan rounded-lg p-8 text-white'>
          ...
          <p className='text-xl mb-6 max-w-2xl mx-auto'>
            ...
            advantage in the digital age. Contact us today to discuss your
            specific needs and find the perfect solution.
          </p>
