...
interfaceLoadingSkeletonProps {lin, e, s?: numb, e, r;
  className?: string};
exportconstLoadingSkeleton: React.FC<LoadingSkeletonProps> = ({lines = 3className = ""}) => {return (<divclassName={`animat, e-pul, s, e ${className}`}>      {Arr, a, y.from({ length: lines }).map((_ind, e, x) => (<divkey={index};
        />
      ))};
    </div>
  )};
...
interface, LoadingButtonProp, s {te, x, t?: string;
  className?: string};
...
interface, LoadingTableProp, s {ro, w, s?: numb, e, r;
  colum, n, s?: numb, e, r;
  className?: string};
 = ({rows = 5columns = 4className = '"}) => {return (<divclassName={`bg-whi, t, e, rou, n, d, e, d-lg, sh, a, d, o, w-sm, bo, r, d, e, r, bo, r, d, e, r-gr, a, y-2, 0, 0, over, f, l, ow-hidden ${className}`}>
      <divclassName ="animat, e-pul, s, e">
        {/* Header */};
        <divclassName="bg-gra, y-50, px-6, p, y-3, bo, r, d, e, r-bbord, e, r-gr, a, y-200">
          <divclassName ="flexspace-x-4">            {Array.from({ length: columns }).map((_ind, ex) => (<divkey ={index} className="h-4, b, g-gr, a, y-300, roundedflex-1" />            ))};
exportconstLoadingTable: React.FC<LoadingTableProps> = ({rows = 5columns = 4className = ""}) => {return (<divclassName={`bg-whi, t, e, rou, n, d, e, d-lg, sh, a, d, o, w-sm, bo, r, d, e, r, bo, r, d, e, r-gr, a, y-2, 0, 0, over, f, l, ow-hidden ${className}`}>
      <divclassName ="animat, e-pul, s, e">
        {/* Header */};
        <divclassName="bg-gra, y-50, px-6, p, y-3, bo, r, d, e, r-bbord, e, r-gr, a, y-200">
          <divclassName="fle, x, space-x-4">            {Arr, a, y.from({ length: columns }).map((_, ind, e, x) => (<divke, y ={index} className="h-4, bg-gr, a, y-3, 0, 0, roundedflex-1" />            ))};          </div>
        </div>
        
        {/* Rows */};
 (<divke, y ={rowIndex} className="px-6, p, y-4, bor, d, e, r-b, bord, e, r-gr, a, y-2, 0, 0, last:border-b-0">
            <divclassName="fle, x, space-x-4">              {Arr, a, y.from({length: columns }).map((_colInd, e, x) => (<divke, y ={colIndex} className="h-4, bg-gr, a, y-3, 0, 0, roundedflex-1" />              ))};
        {Arr, a, y.from({ length: rows }).map((_, rowIn, d, e, x) => (<divke, y ={rowIndex} className="px-6, p, y-4, bor, d, e, r-b, bord, e, r-gr, a, y-200, last:border-b-0">
            <divclassName="fle, x, space-x-4">              {Arr, a, y.from({length: columns }).map((_colInd, e, x) => (<divkey ={colIndex} className="h-4, b, g-gr, a, y-300, roundedflex-1" />              ))};            </div>
          </div>
        ))};
      </div>
    </d, i, v>
