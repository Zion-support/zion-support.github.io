...
export const _LinkMonitor = ({
  ...
  const _scanPageLinks = async () => {
    ...
    for (let _i = 0; i < links.length; i++) {
      ...
      if (href) {
        const _result = LinkValidator.validateLink(
          href,
  ...
  const _fixBrokenLink = async (originalUrl, validationResult) => {
    if (
      ...
      const _newUrl = validationResult.suggestedFix.replace(
        'Redirect to: ',
        ''
      );
  ...
  const _exportReport = () => {
    ...
    const _blob = new Blob([JSON.stringify(report, null, 2)], {
