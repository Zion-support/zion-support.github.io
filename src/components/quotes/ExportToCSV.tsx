      ...
      headers.join()...rows.map(row =>;
        row.map(cell =>;
          ...
          typeof cell === 'string' && (cell.includes() |cell.includes('"'))? `"${cell.replace(/"/g, '""')}"`;
            : cell;
        ).join())const blob = new Blob([csvContent], { type: 'text/csv,
charset = utf-8,' })const url = URL.createObjectURL(blob)const link = document.createElement('a')link.setAttribute('href', url)link.setAttribute('download', `${filename}-${new Date().toISOString().split('T')[0]}.csv`)document.body.appendChild(link)// Download file and clean up;
...
charset = utf - 8;' })const url = URL.createObjectURL (blob)const link = document.create_element ('a')link.set_attribute ('href', url)link.set_attribute ('download',`${filename}-${new Date ().toISOString ().split ('T')[0]}.csv`)document.body.append_child (link)import { Button   } from '@/components / ui / button';
interface ExportToCSVProps {
quotes: QuoteRequest[],
filename?: string;
}
export const ExportToCSV = ({ quotes, filename = "quote - requests" }: ExportToCSVProps) =>: any {const handle_export = () =>: any {// Define CSV Headers;
    ...
    link.click ()set_timeout (()  => {
document.body.remove_child (link)URL.revokeObjectURL (url)
}, 100)}
      >;
      <Download size={16} />;
      Export CSV;
...
const handleExport = () => {// Define CSV Headers;
    ...
    const blob = new Blob([csvContent], { type: 'text/csv,
charset = utf-8,' })const url = URL && URL.createObjectURL(blob)const link = document && document.createElement('a')link && link.setAttribute('href', url)link && link.setAttribute('download', `${filename}-${new Date().toISOString().split('T')[0]}.csv`)document && document.body.appendChild(link)// Download file and clean up;
    link && link.click()setTimeout(()  => {
document && document.body.removeChild(link)URL && URL.revokeObjectURL(url)
}, 100)}>;
      <Download size={16} />;
      Export CSV;
    </Button>;
...
new Date(quote.created_at).toLocaleDateString()])// Create CSV content;
    ...
    const blob = new Blob([csvContent], { type: 'text/csv;
charset = utf-8;' })const url = URL.createObjectURL(blob)const link = document.createElement('a')link.setAttribute('href', url)link.setAttribute('download',`${filename}-${new Date().toISOString().split('T')[0]}.csv`;
    )document.body.appendChild(link)// Download file and clean up;
    link.click()setTimeout(()  => {
document.body.removeChild(link)URL.revokeObjectURL(url)
}, 100)}>;
