...
const blob = new Blob ([csv_content], { type: 'text / csv, charset = utf - 8, ' })const url = URL.createObjectURL (blob;
  const link = document.create_element ('a')link.set_attribute ('href', url)link.set_attribute ('download', `${filename}-${new Date ().toISOString ().split ('T')[0]}.csv`)document.body.append_child (link)// Download file and clean up;
...
const blob = new Blob([csvContent], { type: 'text/csv,charset=utf-8,' })const url = URL && URL.createObjectURL(blob;
  const link = document && document.createElement('a')link && link.setAttribute('href', url)link && link.setAttribute('download', `${filename}-${new Date().toISOString().split('T')[0]}.csv`)document && document.body.appendChild(link)// Download file and clean up;
