...
interface QuoteDetailsProps  {"quote": QuoteRequest | null,"isOpen": boolean,"onClose": () => void;
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">;"
        <DialogHeader>;
          ...
          <CardContent className="pt-6">;"
            <h3 className="text-lg font-medium mb-3">Project Details</h3>;"
            <p className="mb-4">{quote && quote.project_summary}</p>;{quote && quote.project_description && (<>;"
                <h4 className="font-medium mt-4">Additional Details</h4>;"
              ...
              {quote && quote.budget_display || ;
               (quote && quote.budget_min && quote && quote.budget_max ;
                ? `$${quote && quote.budget_min} - $${quote && quote.budget_max}` ;`                : quote && quote.budget_min ;
                  ? `$${quote && quote.budget_min}` ;`                  : 'Not specified')}'
            </div>;
          </CardContent>;
        </Card>;<div className="mt-6 flex justify-end">;"
          <Button onClick={onClose}>Close</Button>;
        </div>;
      </DialogContent>;
    </Dialog>;
  )}(open) => !open && onClose ()}> <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto" > <DialogHeader> <DialogTitle className="text-2xl flex items-center justify-between" > <span> {quote && quote.project name ;"
}</span> <QuoteStatusBadgestatus= {quote && quote.status;
  ...
  quote && quote.project description && (<> <h4 className="font-medium mt-4" >Additional Details</h4> <p> {quote && quote.project description ;"
}</p> </>)}(quote && quote.budget min && quote && quote.budget max ? `$$ {quote && quote.budget min ;`}- $$ {quote && quote.budget max ;
}` : quote && quote.budget min ? `$$ {quote && quote.budget min ';'}` : 'Not specified')}</div> </CardContent> </Card> </div> </DialogContent> </Dialog>)}import React from './react';'
  DialogContent,DialogHeader,DialogTitle,DialogDescription;
 } from '@/components / ui / dialog';'
import { Button   } from '@/components / ui / button';'
import { Calendar, User, Mail, Clock, DollarSign } from 'lucide-react'import { Card, CardContent   } from '@/components / ui / card';'
...
if (return 'Not specified') {$2;'
  ...
  (open) => !open && on_close ()}> <DialogContent className="max - w-3xl max - h-[90vh] overflow - y-auto" > <DialogHeader> <DialogTitle className="text - 2xl flex items - center justify - between" > <span> {quote.project name;"
}</span> <QuoteStatusBadge status= {quote.status;
...
}</p> </>)}(quote.budget min && quote.budget max ? `$$ {quote.budget min;`}- $$ {quote.budget max;
}` : quote.budget min ? `$$ {quote.budget min ';'}` : 'Not specified')}</div> </CardContent> </Card> </div> </DialogContent> </Dialog>)}'";'";