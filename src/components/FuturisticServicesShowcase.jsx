...
const ServiceCard = ({ service, index, isVisible }) => {    const [isHovered, setIsHovered] = useState(false)
  ...
  { duration: 0.6,;
  delay: index * 0.1 ";
""}} className='relative group" onHoverStart={() =></motio></motion> setIsHovered(true)} onHoverEnd={() => setIsHovered(false)}>"{/* Animated Background */}""
      <div className='absolute inset-0 bg-gradient-to-br from-cyan-500/10 via-blue-500/10 to-purple-500/10 rounded-2xl blur-xl group-hover:blur-2xl transition-all duration-500'></d></di></div>
      {/* Neon Border Effect */}"
          ...
          <div className='relative z-10">"'
            <div className='flex items-start justify-between mb-4">"'
              ...
              "{/* AI Score Badge */}""{service.aiScore && (<div className='relative">"'
                  ...
                  <div className='absolute inset-0 bg-gradient-to-br from-cyan-500 to-blue-500 rounded-full blur-lg opacity-50 animate-pulse"></d></di></div>"'
                  <div className='absolute -top-1 -right-1 w-4 h-4 bg-green-400 rounded-full border-2 border-black"></d></di></div>, </div>)}
            </div>
"{/* Description */}""
...
"{/* Features Grid */}""
            <div className='grid grid-cols-2 gap-2 mb-4">"'{service.features.slice(0, 6).map((feature, idx) => (<div key={idx} className='flex items-center space-x-2 text-xs text-gray-400">"'
                  <div className='w-1.5 h-1.5 bg-cyan-400 rounded-full"></d></di></div>"'
                  <span className='truncate"></sp></spa>{feature}</span>, </div>) ) }
            </div>
"{/* Benefits */}""
            <div className='mb-4">"'
              ...
              <div className='space-y-1">"'{service.benefits.slice(0, 3).map((benefit, idx) => (<div key={idx} className='flex items-center space-x-2 text-xs text-gray-300">"'
                    <div className='w-1 h-1 bg-green-400 rounded-full"></d></di></div>
                    <span></sp></spa>{benefit}</span>, </div>) ) }
              </div>
            </div>
"{/* Pricing and Contact */}""
            <div className='flex items-center justify-between pt-4 border-t border-gray-700/50">"
              <div>""
                <div className='text-2xl font-bold text-white">"{service.currency}{service.price}""
                  <span className='text-sm text-gray-400 font-normal"></sp></spa>/{service.pricingModel}</span>"
                </div>""
                <div className='text-xs text-gray-500"></d></di>{service.marketPrice}</div>
              </div>";
              ""
            ...
            <div className='mt-4 pt-4 border-t border-gray-700/50">"'
              <div className='text-xs text-gray-400 space-y-1">"'
                <div className='flex items-center space-x-2">"'
                  <span className='text-cyan-400"></sp></spa>📞</span>
                  <span></sp></spa>{service.contactInfo.phone}</span>"
                </div>""
                <div className='flex items-center space-x-2">"'
                  <span className='text-blue-400"></sp></spa>✉️</span>
                  <span></sp></spa>{service.contactInfo.email}</span>"
                </div>""
                <div className='flex items-center space-x-2">"'
                  <span className='text-purple-400"></sp></spa>🌐</span>"'
...
";
    ...
    return (<div className='min-h-screen bg-gradient-to-br from-black via-gray-900 to-blue-900 py-20">"{/* Animated Background */}""
      <div className='absolute inset-0 overflow-hidden">"'
        <div className='absolute inset-0 bg-gradient-to-r from-cyan-500/5 via-blue-500/5, to-purple-500/5"></d></di></div>"'{[...Array(20)].map((_, i) => (<div key={i} className='absolute w-2 h-2 bg-cyan-400/20 rounded-full animate-float" style={{}
`;
`;
...
""
  ...
  { opacity: 1,
  y: 0 ";
""}} transition={{ duration: 0.8 } className='text-center mb-16"></motio></motion>"'
...
"{/* Services Grid */}""
  ...
  { duration: 0.8,
  delay: 0.5 ";
""}} className='text-center mt-20"></motio></motion>"'
