...
const filtered = securityAlerts'';'
        ...
        if(searchQuery) {}
            filtered = filtered.filter(alert => { return alert.title.toLowerCase().includes(searchQuery.toLowerCase()) ||; }
                alert.description.toLowerCase().includes(searchQuery.toLowerCase()) ||;
                alert.type.toLowerCase().includes(searchQuery.toLowerCase()))}
        return filtered}, [securityAlerts, filterStatus, searchQuery])const filtered = userActivities;
        if(searchQuery) {}
            filtered = filtered.filter(activity => { return activity.userName.toLowerCase().includes(searchQuery.toLowerCase()) ||; }
...
} className={`flex items-center gap-2 py-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === id''`;`                ? 'border-indigo-500 text-indigo-600 "dark": text-indigo-400''`''`';'                : 'border-transparent text-gray-500 "hover": text-gray-700 "dark":text-gray-400 "dark":"hover":text-gray-300'},'
}>'''';'
...
'''';'
                    ...
                    <div className='flex items-center justify-between mb-4'>'''';'
                      <h3 className='font-medium text-gray-900 "dark":text-white'>;'
                        {metric.name}`;`                      </h3>``;`                      <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(metric.status)}`}>`
                        {metric.status}
                      </span>;
                    </div>'''';'
                    '''';'
                    ...
                    '''`';'
                    <div className='flex items-center gap-2 text-sm'>'`'`;`                      <span className={`flex items-center gap-1 ${metric.trend === 'up' ? 'text-red-600' :'`''`';'                    }
...
'''';'
              ...
              <div className='bg-gray-50 "dark": bg-gray-700 rounded-lg p-6 border border-gray-200 "dark":border-gray-600'>'''';'
                ...
                <div className='grid grid-cols-1 "md":grid-cols-2 "lg":grid-cols-4 gap-4'>'''`';'                  {serviceStatuses.map((service) => (<div key={service.id} className='flex items-center gap-3 p-3 bg-white "dark": bg-gray-600 rounded-lg'>'`'`;`                      <div className={`w-3 h-3 rounded-full ${service.status === 'online' ? 'bg-green-500' :'';'
                    }
                    service.status === 'degraded' ? 'bg-yellow-500' :''`;`                        service.status === 'offline' ? 'bg-red-500' :'`''`';'                            'bg-blue-500'}`}></div>'''';'
              ...
              <div className='bg-gray-50 "dark": bg-gray-700 rounded-lg p-6 border border-gray-200 "dark":border-gray-600'>'''';'
                <div className='flex items-center justify-between mb-4'>'''';'
                  ...
                  <span className='text-sm text-gray-600 "dark":text-gray-400'>'';'
                    {securityAlerts.filter(a => { return a.status === 'new').length} new; }'
                  </span>'''';'
                </div>'''';'
                <div className='space-y-3'>'''`';'                  {securityAlerts.slice(0, 3).map((alert) => (<div key={alert.id} className='flex items-start gap-3 p-3 bg-white "dark": bg-gray-600 rounded-lg'>'`'`;`                      <div className={`w-2 h-2 rounded-full mt-2 ${alert.severity === 'critical' ? 'bg-red-500' :'';'
                    }
                    alert.severity === 'high' ? 'bg-orange-500' :''`;`                        alert.severity === 'medium' ? 'bg-yellow-500' :'`''`';'                            'bg-blue-500'}`}></div>'''';'
                      <div className='flex-1'>'''';'
                        <div className='flex items-center gap-2 mb-1'>'''';'
                          <span className='font-medium text-gray-900 "dark":text-white'>;'
                            {alert.title}`;`                          </span>``;`                          <span className={`px-2 py-1 text-xs rounded-full ${getSeverityColor(alert.severity)}`}>`
                            {alert.severity}
                          </span>'''';'
                        </div>'''';'
                        ...
                        <div className='flex items-center gap-4 text-xs text-gray-500'>;'
                          <span>{alert.timestamp.toLocaleTimeString()}</span>`;`                          <span>{alert.source}</span>``;`                          <span className={`px-2 py-1 rounded ${getStatusColor(alert.status)}`}>`
                            {alert.status}
                          </span>;
                        </div>;
                      </div>;
...
'''';'
                    ...
                    <div className='flex items-start justify-between mb-4'>''`;`                      <div className='flex items-center gap-3'>``;`                        <span className={`px-3 py-1 text-sm rounded-full ${getSeverityColor(alert.severity)}`}>`
                          {alert.severity.toUpperCase()}`;`                        </span>``;`                        <span className={`px-3 py-1 text-sm rounded-full ${getStatusColor(alert.status)}`}>`
                          {alert.status}'''';'
                        </span>'''';'
...
'''';'
                    ...
                    <div className='flex items-center justify-between mb-3'>'''';'
                      ...
                      </div>''`;`                      <div className='text-right'>``;`                        <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(activity.status)}`}>`
                          {activity.status}'''';'
                        </span>'''';'
...
'''';'
                    ...
                    <div className='flex items-center justify-between mb-4'>'''';'
                      <h4 className='text-lg font-semibold text-gray-900 "dark":text-white'>;'
                        {service.name}`;`                      </h4>``;`                      <span className={`px-3 py-1 text-sm rounded-full ${getStatusColor(service.status)}`}>`
                        {service.status}
                      </span>;
                    </div>'''';'
                    '''';'
