import React from 'react';

export const NewContent2026PromotionalBanner = () => {
  return (
    <div className="w-full bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white">
      <div className="max-w-6xl mx-auto px-4 py-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center">
              <span className="text-xl">🚀</span>
            </div>
            <div>
              <h3 className="text-lg font-bold">NEW 2026 CONTENT</h3>
              <p className="text-sm opacity-90">Latest AI insights and breakthrough technologies</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2 text-sm">
            <a 
              href="/blog/ai-agent-evals-2026" 
              className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full transition-colors"
            >
              Agent Evals
            </a>
            <a 
              href="/blog/ai-ethical-framework-2026" 
              className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full transition-colors"
            >
              AI Ethics
            </a>
            <a 
              href="/blog/ai-multimodal-enterprise-integration-2026" 
              className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full transition-colors"
            >
              Multimodal AI
            </a>
            <a 
              href="/case-studies/ai-ethical-framework-success-2026" 
              className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full transition-colors"
            >
              Success Story
            </a>
          </div>
          <a
            href="/blog"
            className="bg-white text-indigo-700 hover:bg-gray-100 px-4 py-2 rounded-lg font-semibold transition-colors"
          >
            Explore All
          </a>
        </div>
      </div>
    </div>
  );
};

export const AIAgentEvalsBanner = () => {
  return (
    <div className="w-full bg-gradient-to-r from-blue-50 to-indigo-100 border border-blue-200">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs font-semibold">
                NEW 2026
              </span>
              <span className="text-sm text-gray-500">14 min read</span>
            </div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">
              AI Agent Evals 2026: Shipping Reliable Autonomy
            </h3>
            <p className="text-gray-600 mb-4">
              Practical eval frameworks, traces, and policy gates to ship trustworthy agents. Learn how to implement reliable AI agent evaluation systems.
            </p>
            <a
              href="/blog/ai-agent-evals-2026"
              className="inline-flex items-center text-blue-600 hover:text-blue-700 font-semibold"
            >
              Read Article →
            </a>
          </div>
          <div className="w-16 h-16 bg-blue-100 rounded-lg flex items-center justify-center">
            <span className="text-2xl">🤖</span>
          </div>
        </div>
      </div>
    </div>
  );
};

export const AIEthicsFrameworkBanner = () => {
  return (
    <div className="w-full bg-gradient-to-r from-green-50 to-emerald-100 border border-green-200">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-semibold">
                NEW 2026
              </span>
              <span className="text-sm text-gray-500">18 min read</span>
            </div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">
              AI Ethical Framework 2026: Building Responsible AI Systems
            </h3>
            <p className="text-gray-600 mb-4">
              Comprehensive guide to implementing ethical AI frameworks in enterprise environments with practical implementation strategies.
            </p>
            <a
              href="/blog/ai-ethical-framework-2026"
              className="inline-flex items-center text-green-600 hover:text-green-700 font-semibold"
            >
              Read Guide →
            </a>
          </div>
          <div className="w-16 h-16 bg-green-100 rounded-lg flex items-center justify-center">
            <span className="text-2xl">⚖️</span>
          </div>
        </div>
      </div>
    </div>
  );
};

export const MultimodalAIBanner = () => {
  return (
    <div className="w-full bg-gradient-to-r from-purple-50 to-violet-100 border border-purple-200">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-purple-100 text-purple-800 px-2 py-1 rounded-full text-xs font-semibold">
                NEW 2026
              </span>
              <span className="text-sm text-gray-500">22 min read</span>
            </div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">
              AI Multimodal Enterprise Integration 2026: Complete Guide
            </h3>
            <p className="text-gray-600 mb-4">
              Master multimodal AI integration in enterprise environments with comprehensive strategies for text, image, video, and audio AI systems.
            </p>
            <a
              href="/blog/ai-multimodal-enterprise-integration-2026"
              className="inline-flex items-center text-purple-600 hover:text-purple-700 font-semibold"
            >
              Read Complete Guide →
            </a>
          </div>
          <div className="w-16 h-16 bg-purple-100 rounded-lg flex items-center justify-center">
            <span className="text-2xl">🎯</span>
          </div>
        </div>
      </div>
    </div>
  );
};

export const EthicsSuccessCaseStudyBanner = () => {
  return (
    <div className="w-full bg-gradient-to-r from-orange-50 to-amber-100 border border-orange-200">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-orange-100 text-orange-800 px-2 py-1 rounded-full text-xs font-semibold">
                Case Study
              </span>
              <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-semibold">
                Fortune 500
              </span>
            </div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">
              AI Ethical Framework Success 2026: $5.2M ROI Case Study
            </h3>
            <p className="text-gray-600 mb-4">
              See how a Fortune 500 company achieved $5.2M ROI with comprehensive AI ethical framework implementation, achieving 99.7% compliance and zero incidents.
            </p>
            <a
              href="/case-studies/ai-ethical-framework-success-2026"
              className="inline-flex items-center text-orange-600 hover:text-orange-700 font-semibold"
            >
              View Case Study →
            </a>
          </div>
          <div className="w-16 h-16 bg-orange-100 rounded-lg flex items-center justify-center">
            <span className="text-2xl">💰</span>
          </div>
        </div>
      </div>
    </div>
  );
};

export const Comprehensive2026ContentBanner = () => {
  return (
    <div className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold mb-4">
            Revolutionary 2026 AI Content
          </h2>
          <p className="text-xl opacity-90 max-w-3xl mx-auto">
            Discover the latest breakthroughs in AI agent evaluation, ethical frameworks, and multimodal enterprise integration
          </p>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          <a href="/blog/ai-agent-evals-2026" className="group">
            <div className="bg-white/10 hover:bg-white/20 rounded-lg p-6 transition-colors">
              <div className="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center mb-4">
                <span className="text-2xl">🤖</span>
              </div>
              <h3 className="font-semibold mb-2 group-hover:text-yellow-200 transition-colors">
                Agent Evals 2026
              </h3>
              <p className="text-sm opacity-80">
                Shipping reliable autonomy with practical evaluation frameworks
              </p>
            </div>
          </a>

          <a href="/blog/ai-ethical-framework-2026" className="group">
            <div className="bg-white/10 hover:bg-white/20 rounded-lg p-6 transition-colors">
              <div className="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center mb-4">
                <span className="text-2xl">⚖️</span>
              </div>
              <h3 className="font-semibold mb-2 group-hover:text-yellow-200 transition-colors">
                AI Ethics Framework
              </h3>
              <p className="text-sm opacity-80">
                Building responsible AI systems for enterprise environments
              </p>
            </div>
          </a>

          <a href="/blog/ai-multimodal-enterprise-integration-2026" className="group">
            <div className="bg-white/10 hover:bg-white/20 rounded-lg p-6 transition-colors">
              <div className="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center mb-4">
                <span className="text-2xl">🎯</span>
              </div>
              <h3 className="font-semibold mb-2 group-hover:text-yellow-200 transition-colors">
                Multimodal AI Integration
              </h3>
              <p className="text-sm opacity-80">
                Complete guide to enterprise multimodal AI systems
              </p>
            </div>
          </a>

          <a href="/case-studies/ai-ethical-framework-success-2026" className="group">
            <div className="bg-white/10 hover:bg-white/20 rounded-lg p-6 transition-colors">
              <div className="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center mb-4">
                <span className="text-2xl">💰</span>
              </div>
              <h3 className="font-semibold mb-2 group-hover:text-yellow-200 transition-colors">
                $5.2M ROI Success
              </h3>
              <p className="text-sm opacity-80">
                Fortune 500 ethical framework implementation case study
              </p>
            </div>
          </a>
        </div>

        <div className="text-center mt-8">
          <a
            href="/blog"
            className="bg-white text-indigo-700 hover:bg-gray-100 px-8 py-3 rounded-lg font-semibold transition-colors"
          >
            Explore All 2026 Content
          </a>
        </div>
      </div>
    </div>
  );
};