...
import { Menu, X, Phone, Globe, Brain, Zap, Shield, Building, ChevronDown const Navigation = () => {}
    ...
    const navigationItems = [''
        { name: 'Home', href: '/', icon: Globe },'{ name: 'Micro SAAS Services', href: '/micro-saas-services', icon: Zap },'{ name: 'Comprehensive Services', href: '/comprehensive-services', icon: Brain },'{ name: 'Pricing', href: '/pricing', icon: Building }
    ];
...
';
            ...
            services['
                { name: 'Blockchain Solutions', href: '/comprehensive-services#blockchain-supply-chain' },'{ name: 'AR/VR Business', href: '/comprehensive-services#ar-vr-business-solutions' },'{ name: 'IoT Intelligence', href: '/comprehensive-services#iot-intelligence-hub' }
            ]},
        {}
';
            name: 'Industry Solutions',
            services['
                { name: 'Healthcare IT', href: '/comprehensive-services#healthcare-it-solutions' },'{ name: 'FinTech Solutions', href: '/comprehensive-services#fintech-solutions-pro' },'{ name: 'GreenTech Analytics', href: '/comprehensive-services#greentech-analytics' }
            ];
    ];
    const isActive = (href) => {}
';
...
''''{isServicesOpen && (<div className='absolute top-full left-0 mt-2 w-80 bg-zion-blue-dark border border-zion-blue-light rounded-xl shadow-2xl z-50'>''''
                  <div className='p-4'>''''
                    <div className='grid grid-cols-1 gap-4'>''''{serviceCategories.map((category) => (<div key={category.name} className='space-y-2'>''''
                          <h3 className='text-sm font-semibold text-zion-cyan'>{category.name}</h3>''''
                          <div className='space-y-1'>''''{category.services.map((service) => (<Link key={service.name} to={service.href} className='block px-3 py-2 text-sm text-zion-slate-light hover:text-white hover:bg-white/10 rounded-lg transition-colors' onClick={() => setIsServicesOpen(false)}>{service.name}
                              </Link>) ) }
                          </div>
                        </div>) ) }
                    </div>
...
'''{/* Mobile menu button */}''''
          <div className='lg:hidden'>''''
            <button onClick={() => setIsOpen(!isOpen)} className='text-zion-slate-light hover:text-white focus:outline-none focus:text-white'>''''{isOpen ? <X className='w-6 h-6'/> : <Menu className='w-6 h-6'/>}
            </button>
          </div>
        </div>
      </div>
'''{/* Mobile Navigation */}''''{isOpen && (<div className='lg:hidden'>''''
...
'''{/* Mobile Services */}''''
            <div className='px-3 py-2'>''''
              ...
              <div className='space-y-2'>''''{serviceCategories.map((category) => (<div key={category.name} className='ml-4'>''''
                    <h4 className='text-sm font-medium text-white mb-1'>{category.name}</h4>''''
                    <div className='space-y-1'>''''{category.services.map((service) => (<Link key={service.name} to={service.href} className='block px-3 py-1 text-sm text-zion-slate-light hover:text-white rounded transition-colors' onClick={() => setIsOpen(false)}>{service.name}
                        </Link>) ) }
                    </div>
                  </div>) ) }
              </div>
