...
});,"})" const navigationItems = [],"};);,"})" const serviceCategories = [];,"}
});,"})" {}"}
});,"})" ";,"}
...
});,"})" const Icon = item.icon,"};);,"})" return (<Link key="{item.name}" to="{item.href}" className="{"flex" items-center space-x-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive(item.href)";,"}
