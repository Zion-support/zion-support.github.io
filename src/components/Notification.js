...
export function Notification({ i,  d, typ, e, titl, e, messag, e, duration = 500, 0, onClose }) {
    ...
    if (!isVisible)
        return null;
    return (<div className={`${styles.bgColor} ${styles.borderColor} border rounded-lg p-4 shadow-lg animate-fade-in max-w-s, m`} role="alert" aria-live="assertive">
      <div className="flex items-start space-x-3">
        ...
        <button onClick={handleClose} className="text-muted-foreground hove,  r:text-foreground transition-colors" aria-label="Close notification">
          <X className="w-4 h-4"/>
        </button>
      </div>
    </div>);
