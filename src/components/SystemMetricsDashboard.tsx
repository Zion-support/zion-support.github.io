...
export const SystemMetricsDashboard: React.FC<SystemMetricsDashboardProps> = ({
  ...
  const [performanceMetrics, setPerformanceMetrics] = useState<Record<
    ...
    number
  > | null>(null);
  const [accessibilityMetrics, setAccessibilityMetrics] = useState<Record<
    string,
    number
  > | null>(null);
  const [securityMetrics, setSecurityMetrics] = useState<Record<
    string,
    number
  > | null>(null);
    ...
    if (isVisible) {
      const updateMetrics = () => {
        ...
        setAccessibilityMetrics(
          accMetrics
            ? Object.fromEntries(
                Object.entries(accMetrics).map(([k, v]) => [
                  k,
                  typeof v === "number" ? v : 0,
                ]),
              )
        ...
        setSecurityMetrics(
          secMetrics
            ? Object.fromEntries(
                Object.entries(secMetrics).map(([k, v]) => [
                  k,
                  typeof v === "number" ? v : 0,
                ]),
              )
          ...
          {activeTab === "accessibility" && (
            <div className="space-y-6">
              ...
              {accessibilityMetrics ? (
                ...
                <p className="text-gray-500">
                  No accessibility metrics available
                </p>
              )}
            </div>
          )}
