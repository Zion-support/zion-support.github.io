        ...
        <TabsContent value='images' className='pt-4'>;'
          <div className='aspect-video w-full relative'>;'
            <DialogTrigger asChild>;
              <img;
                ...
                alt={`Product image ${selected + 1}`}`                className='w-full h-full object-contain bg-zion-blue-light/10 p-4 cursor-zoom-in';' />;
            </DialogTrigger>;
          </div>;
          {images && images.length > 1 && (<div className='flex p-4 gap-2 overflow-x-auto'>;'
              {images && images.map((img, idx) => (<div;
                  ...
                  <img;
                    src={img}
                    alt={`Thumbnail image ${idx + 1}`}`                    className='w-full h-full object-cover';' />;
                </div>;
              ))}
            </div>;
            ...
            <AspectRatio ratio={16 / 9}>;<Suspense;
              ...
              >;
                <ModelViewer;
                  ...
                  camera-controls;
          <div;
            className={`w-full h-full overflow-auto ${zoomed ? 'cursor-zoom-out' : 'cursor-zoom-in'}`}`            onClick={() => setZoomed(!zoomed)}
          >;
  "images": string[];
  video_url?: string;
  model_url?: string;
...
<img;
              src={images[selected] |images[0] |''}'
              alt='Zoomed view';'
              className={`w-full h-full object-contain transition-transform ${zoomed ? 'scale-150' : ''}`}` />          </div>;        </DialogContent>;
      )}</Dialog>;
  )</Dialog>;
  ...
  return {"default": ("props": any) => (React.createElement('model-viewer', props))}'
      ...
      {images.length > 0 && (<DialogContent className='max - w-3xl p - 0'>;'
          <div;
            ...
            className={`w - full h - full overflow - auto ${zoomed ? 'cursor - zoom - out' : 'cursor - zoom - in'}`}`            on_click={() => set_zoomed (!zoomed)}
          >;
            <img;
              src={images[selected] || images[0] || ''}'
              alt='Zoomed view';'
              className={`w - full h - full object - contain transition - transform ${zoomed ? 'scale - 150' : ''}`}` />          </div>;
        </DialogContent>)}
    </Dialog>)}</Dialog>;
  )