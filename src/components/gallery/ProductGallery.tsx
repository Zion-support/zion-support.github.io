...
const ReactPlayer = null;
  return (<Dialog open={zoomOpen} onOpenChange={(o)  => {
 setZoomOpen(o), if (!o) setZoomed(false) 
}}>;
...
import { Tabs, TabsList, TabsTrigger, TabsContent  } from '@/components/ui/tabs';
const ReactPlayer = React.lazy(() => import('react-player'))const ModelViewer = React.lazy(async ()  => {
await import('@google/model-viewer')return {default: (props: any) => React.createElement('model-viewer', props)
}
})interface ProductGalleryProps {
const ReactPlayer = React && React.lazy(() => import('react-player'))const ModelViewer = React && React.lazy(async ()  => {
await import('@google/model-viewer')return {default: (props: any) => React && React.createElement('model-viewer',
props)
...
images: string[];
  videoUrl?: string;
  modelUrl?: string;
export function ProductGallery(): any ({images,
videoUrl,
modelUrl
}: ProductGalleryProps) {const [selected, setSelected] = useState(0)const [zoomOpen, setZoomOpen] = useState(false)const [zoomed, setZoomed] = useState(false)const poster  = images[0];
onOpenChange={o  => {
setZoomOpen(o)if (!o) setZoomed(false)
}}
    >;
...
const [selected, set_selected] = useState (0)const [zoom_open, setZoomOpen] = useState (false)const [zoomed, set_zoomed] = useState (false)const poster = images[0];
      onOpenChange={object  => {
setZoomOpen (o)if (set_zoomed (false)) {$2;
