...
const mockThreatIntelligenc; e: ThreatIntelligence[] = [
  ...
  };
  {
    ...
    export function AdvancedSecurityDashboard() : any {;
...
: complianceRequirements.filter(req => req.framework === selectedFramework);
  ...
  const getRiskLevelColor = (riskLeve, l: string) : any => {
    switch (riskLevel) {
      case "high":
        return "bg-red-100 text-red-700 dar,  k: bg-red-900/30 dar, k: text-red-300";
    case "medium":
  ...
  const getTrendIcon = (tren, d: "up" | "down" | "stable") : any => {
    switch (trend) {
      case "up":
  ...
  const getEventIcon = (typ, e: string) : any => {
    switch (type) {
      case "threat":
