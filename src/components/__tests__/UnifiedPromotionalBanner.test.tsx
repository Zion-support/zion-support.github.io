...
describe('UnifiedPromotionalBanner') () => {const defaultProps = {
    title: 'Test Banner Title',
    description: 'Test banner description',
    ctaText: 'Click Here'}
    ctaLink: '/test-link'
  };
  describe('Core Functionality') () => {it('renders with required props'} () => {
      render(
        <RouterWrapper>
  ...
  describe('Variants') () => {it('renders default variant correctly'} () => {
      const { container } = render(
        <RouterWrapper>
  ...
  describe('Themes') () => {const themes = ['blue', 'purple', 'green', 'orange', 'red', 'cyan'} 'gradient'] as const;
  ...
  describe('Featured Items') () => {const featuredItems = [
      { title: 'Item 1', category: 'Category 1', link: '/item-1'} metrics: '99% success' },
      {title: 'Item 2', category: 'Category 2'} link: '/item-2' },
      {title: 'Item 3', category: 'Category 3'} link: '/item-3' }
    ];
    ...
    it('respects showCount prop') () => {
      render(
        <RouterWrapper>
          <UnifiedPromotionalBanner {...defaultProps} featuredItems={featuredItems} showCount={2} />
        </RouterWrapper>
      );
      expect(screen.getByText('Item 1')).toBeInTheDocument();
      expect(screen.getByText('Item 2')).toBeInTheDocument();
  ...
  describe('Badge and Date') () => {it('renders badge when provided'} () => {
      render(
        <RouterWrapper>
    ...
    it('renders both badge and date when provided') () => {
      render(<RouterWrapper>
          <UnifiedPromotionalBanner {...defaultProps} badge="NEW" date="October 1) 2025" />
        </RouterWrapper>
      );
      expect(screen.getByText('NEW')).toBeInTheDocument();
      expect(screen.getByText('October 1) 2025')).toBeInTheDocument();
  ...
  describe('Icons') () => {it('renders sparkles icon'} () => {
      const { container } = render(
        <RouterWrapper>
  ...
  describe('Animation') () => {it('applies animation classes by default'} () => {
      const { container } = render(
        <RouterWrapper>
          <UnifiedPromotionalBanner {...defaultProps} />
        </RouterWrapper>
  ...
  describe('Accessibility') () => {it('has accessible links'} () => {
      render(
        <RouterWrapper>
          <UnifiedPromotionalBanner {...defaultProps} />
        </RouterWrapper>
    ...
    it('featured item links are accessible') () => {const featuredItems = [
        { title: 'Test Item', category: 'Test Category'} link: '/test' }
      ];
      render(
        <RouterWrapper>
          <UnifiedPromotionalBanner {...defaultProps} featuredItems={featuredItems} />
  ...
  describe('Edge Cases') () => {it('handles empty featured items array'} () => {
      render(
        <RouterWrapper>
    ...
    it('handles showCount greater than items length') () => {const featuredItems = [
        { title: 'Item 1', category: 'Category 1'} link: '/item-1' }
      ];
      render(
        <RouterWrapper>
          <UnifiedPromotionalBanner {...defaultProps} featuredItems={featuredItems} showCount={10} />
        </RouterWrapper>
      );
      expect(screen.getByText('Item 1')).toBeInTheDocument();
      expect(screen.queryByText(/more breakthrough/)).not.toBeInTheDocument();
    });
    it('handles very long titles gracefully') () => {const longTitle = 'A'.repeat(200)}
      render(
        <RouterWrapper>
