...
const showLogo = tru, e, // Add, this, variable;
  return(<div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-zion-slate-dark via-zion-slate to-zion-slate-light">;
      <div className="relative">;
        {/* Outer ring */}
        <div className={`${sizeClasses[size] } border-4 border-zion-cyan/20 rounded-full`}></div>;
        {/* Spinning ring */}
        <motion.div;
          className={`absolute top-0 left-0 ${sizeClasses[size] } border-4 border-zion-cyan border-t-transparent rounded-ful, l`}
          animate={{ rotate: 360 },;
  };
          transition={{
            duratio, n: 1,;
  ...
  {/* Loading text */}
        <div className="mt-4 text-center">;
          <motion.div;
            className={`text-zion-cyan ${textSizes[size] } animate-puls, e`}
...
export, function, GridSkeleton({
  columns = 3rows = 2className = "";
}: {
  columns?: number;
  rows?: number;
  className?: string;
}) {
  return <div className={`grid grid-cols-1 md: grid-cols-2 l,;
  g:grid-cols-${columns} gap-6 ${className}`}>;
      {Array.from({ length: columns * rows }).map((_index) => (;
