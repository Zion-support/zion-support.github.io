      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"></div>""
        <div className="flex items-center justify-between h-16 lg:h-20"></div>"
          ...
          <Link href="/" className="flex items-center space-x-2 group"></Link>""
            ...
            <span className="text-xl font-bold text-white group-hover:text-cyan-300 transition-colors"></span>
              Zion Tech Group
            </span>
          </Link>
"
              ...
              <div key={item.name} className="relative"></div>"
                {item.dropdown ? (""
                  <div className="relative"></div>
                    <button"
                      onClick={() =></button> toggleDropdown(item.name)}""
                        ...
                        ></motion>""
                          <div className="p-4"></div>
                            {item.dropdown.map((dropdownItem) => (
                              ...
                              >""
                                <div className="flex-shrink-0 w-10 h-10 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-lg flex items-center justify-center group-hover:from-cyan-400/30 group-hover:to-blue-400/30 transition-all duration-200"></div>""
                                  <dropdownItem.icon className="w-5 h-5 text-cyan-400" /></dropdownItem>"
                                </div>""
                                <div className="flex-1 min-w-0"></div>""
                                  <p className="text-sm font-medium text-white group-hover:text-cyan-300 transition-colors"></p>
                                    {dropdownItem.name}"
                                  </p>""
        ...
        {isOpen && (
          ...
          ></motion>""
            <div className="px-4 py-6 space-y-4"></div>"
              {/* Contact Information */}""
              <div className="pb-4 border-b border-gray-700"></div>""
                <h3 className="text-sm font-semibold text-cyan-400 mb-3 uppercase tracking-wide"></h3>
                  Contact Information"
                </h3>""
                <div className="space-y-2"></div>
                  {contactInfo.map((contact, index) => (
                    ...
                    ></a>""
                      <contact.icon className="w-4 h-4 text-cyan-400" /></contact>
                      <span></spa>{contact.text}</span>
                    </a>
                  ))}
                </div>
              </div>
                          ...
                          {activeDropdown === item.name && (
                            ...
                            ></motion>
                              {item.dropdown.map((dropdownItem) => (
                                ...
                                ></Link>""
                                  <dropdownItem.icon className="w-4 h-4 text-cyan-400" /></dropdownItem>
                                  <span></spa>{dropdownItem.name}</span>
                                </Link>
                              ))}
                            </motion.div>
                          )}
