...
export default Navigation;
 ...
 window.removeEventListener('scroll', handleScroll);
  ...
  // Removed specializedServices as it was unused;
          {/* Logo */}
            <span className = "text-xl font-bold text-white neon-text">+1 302 464 0950</span>;
                Get Started;
          {/* Mobile Menu Button */}
            {isMenuOpen ? <X className="w-6 h-6" /&gt;}</X></X>
        {/* Mobile Menu */}
<span>Services</span>
...
<div className="container mx-auto px-4"flex items-center justify-between h-16 lg:h-20"
          {/* Logo */}
          <Link to=" className="flex items-center space-x-2 group"w-10 h-10 bg-gradient-to-br from-cyan-400 to-purple-600 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300"
<span className="
            <div className="hidden sm:block"text-xl font-bold text-white cyber-text neon-pulse"
<p className="
</Link>
...
<Link to="/about"text-white hover:text-cyan-400 transition-colors font-medium"
              About
            </Link>
...
<$2 />
                ...
                className="flex items-center space-x-1 font-medium transition-colors hover:text-cyan-400 text-white"true""w-4 h-4"
<span>Services
  ...
  // TODO: Add parameters
                ...
                <div className="
<div className="p-6"text-lg font-bold text-white mb-4 neon-text"
                  <div className="
                    {serviceCategories.map((category, index) => (
  // TODO: Add parameters
)
                      <div key={index} className="space-y-2"flex items-center space-x-2 mb-2"
<category.icon className={`w-4 h-4 ${category.color}`} />
<span className="
                        <div className="space-y-2"block px-3 py-2 text-sm text-gray-300 hover:bg-gray-800 hover:text-cyan-400 rounded-lg transition-colors"
                              onClick={closeAllMenus}
                            >
<div className="
                              <div className="text-xs text-gray-500 group-hover:text-purple-500"block px-3 py-2 text-sm text-purple-600 hover:bg-purple-50 rounded-lg transition-colors font-medium"
                              onClick={closeAllMenus}
                            >
                              View All →
                          )}
...
<Link
                      to="/services"
                      className="block w-full text-center bg-gradient-to-r from-cyan-500 to-blue-600 text-white py-2 px-4 rounded-lg font-medium hover:from-cyan-600 hover:to-blue-700 transition-all"hidden lg:flex items-center space-x-4"
<Link
              to=" "
              className="
              onClick={closeAllMenus}
...
<Link
              to="/blog"
              className="text-white hover:text-cyan-400 transition-colors duration-200 font-medium"/contact""text-white hover:text-cyan-400 transition-colors duration-200 font-medium"
              onClick={closeAllMenus}
            >
              Contact
            </Link>
            ...
            <a
              ...
              onClick={closeAllMenus}
            >
<Phone className="w-4 h-4 mr-2"lg:hidden"
<$2 />
              onClick={toggleMenu}
              className="
  ...
  // TODO: Add parameters
          ...
          <div className="
<div className="px-2 pt-2 pb-3 space-y-1 bg-gray-900/95 backdrop-blur-sm rounded-lg mt-2"/""block px-3 py-2 text-base font-medium text-white hover:text-cyan-400 hover:bg-gray-800 rounded-md"
                onClick={closeAllMenus}
              >
                Home
              </Link>
...
<button
                  onClick={() => setServicesOpen(!servicesOpen)}
                  className="flex items-center justify-between w-full px-3 py-2 text-base font-medium text-white hover:text-cyan-400 hover:bg-gray-800 rounded-md"flex items-center space-x-2"
<Brain className="
<span>Services
  ...
  // TODO: Add parameters
                  ...
                  <div className="mt-2 ml-4 space-y-2"text-sm font-medium text-cyan-400 mb-2"
                        <div className="
  ...
  // TODO: Add parameters
                            ...
                            <$2 />
                              key={serviceIndex}
                              to={service.path}
                              className="block text-xs text-gray-300 hover:text-cyan-400 transition-colors duration-300 py-1"w-3 h-3 inline mr-1"'"/services""block text-xs text-cyan-400 hover:text-cyan-300 transition-colors duration-300 py-1 font-medium"
                          >
                            View All →
                    ))}
                  </div></div>
...
<a
                  ...
                  className="block w-full text-center bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:from-cyan-600 hover:to-blue-700 transition-all"w-4 h-4 inline mr-2"
                  (302) 464-0950
                </a></div>
</div></div>
        )}
