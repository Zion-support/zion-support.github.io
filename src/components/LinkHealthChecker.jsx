...
';
    ...
    return (<div className={`bg-white/5 backdrop-blur-sm border border-white/20 rounded-lg p-6 ${className}`}>'''{/* Header */}''''
      <div className='flex items-center justify-between mb-6'>'''
        ...
        <button onClick={checkAllLinks} disabled={isChecking} className='px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 text-white rounded-lg transition-colors duration-300 flex items-center gap-2'>'''{isChecking ? (<>''''
            ...
            return (<div key={link.url} className='flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-lg hover:border-white/20 transition-colors duration-300'>''''
              <div className='flex items-center gap-3'>
                ...
                <div>''''
                  ...
                  <div className='text-sm text-gray-400'>{link.url}</div>''''{status.error && (<div className='text-xs text-red-400 mt-1'>{status.error}</div>)}
                </div>'''
              </div>''`
              <div className='text-right'>``
                <div className={`text-sm font-medium ${getStatusColor(status.status)}`}>
                  {getStatusText(status.status)}'''
                </div>''''{status.responseTime && (<div className='text-xs text-gray-400'>
                    {status.responseTime}ms
                  </div>) }
              </div>
            </div>) })}
      </div>
'''{/* Recommendations */}''''{brokenCount > 0 && (<div className='mt-6 p-4 bg-red-500/10 border border-red-500/20 rounded-lg'>''''
          <h4 className='text-red-400 font-semibold mb-2'>⚠️ Action Required</h4>''''
          <p className='text-sm text-red-300'>'{brokenCount} link{brokenCount !== 1 ? 's' : ''} {brokenCount !== 1 ? 'are' : 'is'} broken and need attention.;
