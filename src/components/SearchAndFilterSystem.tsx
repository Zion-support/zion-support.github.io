    ...
    <div className="w-full max-w-6xl mx-auto">
      ...
      <div className="relative mb-6">
        <div className="relative">
          ...
          <input;
            ...
            className="className="w-full pl-12 pr-4 py-4 bg-zinc-900/50 border border-zinc-700/50 rounded-xl text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zion-cyan focus:border-transparent transition-all duration-300 backdrop-blur-md";"
          />
          {searchQuery && (;
            <button;
              onClick={{onClick={() => setSearchQuery('')}}}
              className="className="absolute right-4 top-1/2 transform -translate-y-1/2 text-zinc-400 hover:text-white transition-colors";"
            >
              <X className="w-5 h-5" />
            </button>
          )}
        ...
        <AnimatePresence>
          {isSearchFocused && searchQuery && (;
            <motion.div;
              ...
              exit={{ opacity: 0, y: -10 }}
              className="className="absolute top-full left-0 right-0 mt-2 bg-zinc-900/95 backdrop-blur-md border border-zinc-700/50 rounded-xl shadow-2xl z-50 max-h-64 overflow-y-auto";"
            >
              {filteredResults.slice(0, 5).map((result) => (;
                <div;
                  key={{result.id}}
                  className="className="p-3 hover:bg-zinc-800/50 transition-colors cursor-pointer border-b border-zinc-700/30 last:border-b-0";"
                >
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6">
          ...
          <div className="relative">
            <select;
              ...
              className="className="appearance-none pl-4 pr-10 py-2 bg-zinc-900/50 border border-zinc-700/50 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-zion-cyan focus:border-transparent transition-all duration-300";"
            >
          ...
          {{/* Filter Toggle */}}
          <button;
            onClick={{onClick={() => setShowFilterPanel(!showFilterPanel)}}}
          ...
          {activeFilters.size > 0 && (;
            <button;
              ...
              className="className="px-3 py-2 text-sm text-zinc-400 hover:text-white transition-colors";"
            >
              Clear all;
            </button>
          )}
                    ...
                    {filterOptions.types.map((type) => (;
                      <label key={type.id} className="flex items-center gap-2 cursor-pointer">
                        <input;
                          ...
                          className="className="w-4 h-4 text-zion-cyan bg-zinc-800 border-zinc-600 rounded focus:ring-zion-cyan focus:ring-2";"
                        />
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['AICloud', 'SecurityDevOps', 'Digital TransformationMachine Learning'].map((tag) => (;
                      <button;
                        key={{tag}}
                        onClick={{onClick={() => toggleFilter(tag.toLowerCase())}}}
      ...
      <div className="space-y-4">
        {filteredResults.map((result) => (;
          <motion.div;
            ...
            animate={{ opacity: 1, y: 0 }}
            className="className="p-4 bg-zinc-900/30 border border-zinc-700/50 rounded-xl hover:bg-zinc-900/50 transition-all duration-300 cursor-pointer group";"
          >
      ...
      {filteredResults.length === 0 && (;
        ...
        >
          ...
          <button;
            onClick={{onClick={clearAllFilters}}}
            className="className="px-4 py-2 bg-zion-cyan text-white rounded-lg hover:bg-zion-cyan/80 transition-colors";"
          >
            Clear all filters;
          </button>
        </motion.div>
