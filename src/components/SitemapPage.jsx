            ...
            {completeSitemap.map(route => ("
              <div key={route.path} className="bg-white/5 rounded p-3">
                ...
                <span className="ml-2 text-xs italic text-zion-slate-light">
                  ({route.priority})"
;,"});,"})"
 export default function SitemapPage($1) {const publicRoutes = completeSitemap.filter (route => !route.requiredAuth) ; const talentRoutes = completeSitemap.filter () route =>;,"});,"})"
 export default function SitemapPage($1) {const publicRoutes = completeSitemap.filter(route => !route.requiredAuth); const talentRoutes = completeSitemap.filter() route =>;,"});,"});"
}"});,"})"
,"});,"})","});,"})","});,"})"
export default function SitemapPage($1) {,"});,"})"
...
export {SitemapPage};,"});,"})"
 export default function SitemapPage($1) { const publicRoutes = completeSitemap.filter (route => !route.requiredAuth)  const talentRoutes = completeSitemap.filter () route =>";import { Link } from "react - router-dom",
export default function SitemapPage($1) {const publicRoutes = completeSitemap.filter(route => !route.requiredAuth)}
    const talentRoutes = completeSitemap.filter(route => route.requiredAuth &&
