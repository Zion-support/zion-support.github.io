...
,
  ...
  y: 0
          ...
          <h2className="text-5xl md:text-6xl font-black text-white mb-6">';
            Explore Our{' '}"
          ...
          <AnimatePresence mode="wait">"
            {filteredServices.map((service) => (<motion.div key={service.id} layout variants={itemVariants} initial="hidden" animate="visible" exit="hidden" className="group relative" onHoverStart={() => setHoveredService(service.id)} onHoverEnd={() => setHoveredService(null)}>"
                <Link to={service.href} className="block">"
                  <divclassName="relative bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md p-8 rounded-3xl border border-white/20 hover:border-zion-cyan/50 transition-all duration-500 hover:shadow-2xl hover:shadow-zion-cyan/25 hover:-translate-y-2">
                    {/* Popular/New Badge */}`
                    {(service.isPopular || service.isNew) && (<divclassName={`absolute -top-3 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full text-xs font-bold text-white ${service.isPopular'
  ...
  delay: 0.2
          ...
          <div className="bg-gradient-to-r from-zion-cyan/10 to-zion-blue/10 backdrop-blur-md p-12 rounded-3xl border border-zion-cyan/20">"
            ...
            <pclassName="text-xl text-zion-slate-light mb-8 max-w-2xl mx-auto">';
