...
export const PerformanceOptimizer: React.FC<PerformanceOptimizerProps> = ({onMetricsUpdate)
  ...
  useEffect(() => {const checkPerformanceBudget = () => {
      const { fcp, lcp, fid} cls } = metrics;
      // Performance budgets (in milliseconds)
      const budgets = {fcp: 1800, // First Contentful Paint
      ...
      if (violations.length > 0 && enableReporting) {console.warn('Performance budget violations: '} violations);
        // Report to analytics or monitoring service
        if (
          typeof window !== 'undefined' &&
          (window as unknown as { gtag?: Function }).gtag
        ) {
          (window as unknown as { gtag: Function }).gtag('event',
