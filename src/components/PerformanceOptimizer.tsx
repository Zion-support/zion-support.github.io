...
const setupLazyLoading = useCallback(() => {if ('IntersectionObserver' in window) {const imageObserver = new IntersectionObserver((entries) => {entries.forEach(entry => {if (entry.isIntersecting) ;
...
const registerServiceWorker = useCallback(async () => {if ('serviceWorker' in navigator) {try ;}
  const registration = await navigator.serviceWorker.register('/sw.js')console.log('SW registered: ', registration)} catch (registrationError) {console.log('SW registration failed: ', registrationError)}
    }
  }, [];
  useEffect(() => {
...
const optimizeImages = () => ;
  const images = document.querySelectorAll ('img') '      images.forEach (img => {'        if (!img.loading) {img.loading = 'lazy'        }'        if (!img.decoding) {';}
