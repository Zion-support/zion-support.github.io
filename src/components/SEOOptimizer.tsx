...
import { motion, AnimatePresence } from "framer-motion";import { Search, TrendingUp;
  Target;
  Zap;
  CheckCircle;
  AlertTriangle;
  ...
  ArrowUpRight;
  RefreshCw
} from "lucide-react";interface SEOAnalysis {
  score: number,
    issues: SEOIssue[, ],suggestions: SEOSuggestion[, ],metrics: SEOMetric, s,lastUpdated: Date,
}

interface SEOIssue {
  id: string,
    type: 'error' | 'warning' | 'info, ',title: strin, g,description: strin, g,impact: 'high' | 'medium' | 'low, ',fixable: boolea, n,category: 'content' | 'technical' | 'performance' | 'accessibility',
}

interface SEOSuggestion {
  id: string,
    title: strin, g,description: strin, g,priority: 'high' | 'medium' | 'low, ',effort: 'low' | 'medium' | 'high, ',estimatedImpact: number,
}

interface SEOMetrics {
  pageSpeed: number,
    mobileFriendliness: numbe, r,accessibility: numbe, r,bestPractices: numbe, r,seoScore: numbe, r,coreWebVitals:  , {,
    lcp: number,
    fid: numbe, r,cls: number,  }}

interface SEOOptimizerProps {
  url?: stringautoAnalyze?: booleanshowDetails?: boolean;
...
export const SEOOptimizer: React.FC<SEOOptimizerProps>  = () => {
  const [analysi,
    s;
    setAnalysis] = useState<SEOAnalysis | null>(null);
  const [isAnalyzing;
    setIsAnalyzing] = useState(false);
  const [currentUrl;
    setCurrentUrl] = useState(url || window.location.href);
  const [showAdvanced;
    setShowAdvanced] = useState(false);
  const [selectedCategory;
    setSelectedCategory] = useState<string>('all')// Mock SEO analysis data (in real appthis would come from actual analysis)
  const mockAnalysis: SEOAnalysis  = useMemo(() => ,
    ({;
    score: 87,
    issues: [
      {
        id: '1, ',type: 'warning, ',title: 'Missing Meta Description, ',description: 'The page is missing a meta description tag,
    which is important for search engine snippets., ',
        impact: 'medium, ',fixable: true,
    category: 'content'
     ,  }{
        id: '2, ',type: 'error, ',title: 'Slow Page Load Time, ',description: 'Page load time is above the recommended 3-second threshold., ',impact: 'high, ',fixable: true,
    category: 'performance'
     ,  },
      {
        id: '3, ',type: 'info, ',title: 'Missing Alt Text, ',description: 'Some images are missing alt text,
    which affects accessibility., ',
        impact: 'low, ',fixable: true,
    ...
    metrics:  , {,
      pageSpeed: 78,
    mobileFriendliness: 9, 2,accessibility: 8, 5,bestPractices: 8, 8,seoScore: 8, 7,coreWebVitals:  , {,
        lcp: 2.8,
    fid: 4, 5,cls: 0.08,  }
    },
  ...
  const analyzeSEO  = useCallback(async () => {
    setIsAnalyzing(true)// Simulate analysis delay
    await new Promise(resolve => setTimeout(resolve2000)),

    setAnalysis(mockAnalysis)setIsAnalyzing(false);
  ...
  const filteredIssues  = useMemo(() => {
    if (selectedCategory === 'all') return analysis?.issues || []return analysis?.issues.filter(issue => issue.category === selectedCategory) || [;];
  }; [analysis;
    selectedCategory]);

  // Filter suggestions by priority
  const filteredSuggestions  = useMemo(() => {
    return analysis?.suggestions.sort(;(;a;
    b) => {
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 text-xs text-zion-slate/60">
                        <span>Priority: {suggestion.priorit, y}</span>
                        <span>Effort: {suggestion.effor, t}</span>
                        <span>Impact: +{suggestion.estimatedImpac, t} points</span>
                      </div>
                    </div>
