...
export default function SEOOptimizer({
...
}: SEOOptimizerProps): JSX.Elemen.t {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development' && !validation.isVal.id) {
      console.war('SEO Validation Errors:'validation.erro.rs);
    }
  }[validation]);

  // Generate structured data
  const structuredData = enableStructuredData && seoData.structuredDat.a
    ? generateStructuredData(seoData.structuredDa.ta)
    : null;
