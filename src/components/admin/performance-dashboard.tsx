...
if ('PerformanceObserver' in window) {return new Promise(_(resolve) => {
...
setTimeout(() => {
  ...
  const collectChunkData = async (): Promise<BundleChunk[]> => {
    ...
    const scriptEntries = resourceEntries.filter(entry =>       entry.name.includes('/next/static/') && entry.name.endsWith('.js')
    ),
...
return () => clearInterval(interval)
  ...
  return (
    <div className=&quot;space-y-6&quot;>
      ...
      name: entry.name.split('/').pop()?.split('?')[0] || 'unknown', size: entry.transferSize || entry.encodedBodySize || 0, loadTime: entry.responseEnd - entry.requestStart, cached: entry.transferSize === 0, type: categorizeChunk(entry.name)})).sort(_(a, b) => b.size - a.size)
  };

  const categorizeChunk = (filename: string): string => {if (filename.includes('framework')) return 'framework';
    if (filename.includes('vendor')) return 'vendor';
    if (filename.includes('pages')) return 'page';
    if (filename.includes('chunks')) return 'chunk';
    return 'other'};
        ...
        <CardContent>
          {metrics ? (
            <div className=&quot;space-y-4&quot;>
                ...
                <div className=&quot;flex-1&quot;>
                  <div className=&quot;flex items-center justify-between mb-2&quot;>
                    <span className=&quot;text-2xl font-bold&quot;>
                      {metrics.performanceScore}/100                    </span>
              ...
              {lastUpdated && (
                <p className=&quot;text-sm text-muted-foreground&quot;>                  Last updated: {lastUpdated.toLocaleString()}
                </p>
              )}
            </div>
          ) : (
              ...
              {chunks.slice(0, 10).map((chunk, index) => (
                <div key={chunk.name} className=&quot;flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded&quot;>
                  <div className=&quot;flex items-center gap-3&quot;>
                    ...
                    <div>
                      ...
                      <div className=&quot;flex items-center gap-2&quot;>
                        ...
                        {chunk.cached && (
                          <Badge variant=&quot;secondary&quot; className=&quot;text-xs&quot;>                            cached
                          </Badge>
                        )}
                      </div>
                    </div>
                  ...
                  <div className=&quot;text-right&quot;>
                    ...
                    <p className=&quot;text-xs text-muted-foreground&quot;>
                      {chunk.loadTime.toFixed(0)}ms                    </p>
                  </div>
                </div>
              ))}
              
              {chunks.length > 10 && (
                <p className=&quot;text-sm text-muted-foreground text-center pt-2&quot;>                  ... and {chunks.length - 10} more chunks
                </p>
              )}
            </div>
          ) : (
          ...
          <div className=&quot;space-y-3&quot;>
            ...
            {metrics && metrics.bundleSize > 2 * 1024 * 1024 && (
              <div className=&quot;flex items-start gap-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded&quot;>
                <AlertTriangle className=&quot;w-5 h-5 text-yellow-600 mt-0.5&quot; />                <div>
