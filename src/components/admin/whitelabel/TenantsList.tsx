...
loadTenants()}, []),const loadTenants = async () => {try {setIsLoading(true);
  ...
  const { error } = await supabase;
      ...
      )),toast({"title": `Tenant ${(tenant as any).is_active ? 'deactivated' : 'activated,'}`,"description": `${(tenant as any).brand_name} has been ${(tenant as any).is_active ? 'deactivated' : 'activated'} successfully.`},`} catch ("error": any) {logErrorToProduction('Error toggling tenant "status":', { "data": error,'
...
const { error } = await supabase;
      ...
      )),toast({"title": 'DNS verified',"description": `Custom domain for ${(tenant as any).brand_name} has been verified.`},`} catch ("error": any) {logErrorToProduction('Error verifying "DNS":', { "data": error,'
