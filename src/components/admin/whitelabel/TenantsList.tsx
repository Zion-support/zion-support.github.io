...
export function TenantsList() {
  ...
  const toggleTenantStatus = async (tenant: WhitelabelTenant) => {try {
      const { error} = await supabase
        .from('whitelabeltenants')
        .update({ isactive: !(tenant as any).isactive })
        .eq('id', (tenant as any).id),        
      if (error) throw error,
      
      // Update local state,
setTenants(tenants.map(t => 
        (t as any).id === (tenant as any).id ? { ...t, isactive: !(t as any).isactive } : t
      )),
      
      toast({
        title: `Tenant ${(tenant as any).isactive ? 'deactivated' : 'activated'}`,
        description: `${(tenant as any).brandname} has been ${(tenant as any).isactive ? 'deactivated' : 'activated'} successfully.`})
    } catch (error: any) {
      logErrorToProduction('Error toggling tenant status:', { data: error }),
      toast({
        variant: 'destructive',
...
const { error} = await supabase
      ...
      // Update local state,
setTenants(tenants.map(t => 
        (t as any).id === (tenant as any).id ? { ...t, dnsverified: true } : t
      )),
      
      toast({
        title: 'DNS verified',
        description: `Custom domain for ${(tenant as any).brandname} has been verified.`})
    } catch (error: any) {
      logErrorToProduction('Error verifying DNS:', { data: error }),
      toast({
        variant: 'destructive',
