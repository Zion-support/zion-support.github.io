...
setTenants(data as WhitelabelTenant[])} catch (error: any) {logErrorToProduction('Error loading tenants:', { data: error }),
toast({variant: 'destructive',
title: 'Failed to load tenants',
description: error.message})} finally {setIsLoading(false)}
  },
const toggleTenantStatus = async (tenant: WhitelabelTenant)  => {
try {const { error 
} = await supabase;
        .from('whitelabel_tenants').update({ is_active: !(tenant as any).is_active }).eq('id', (tenant as any).id),
...
description: `${(tenant as any).brand_name} has been ${(tenant as any).is_active ? 'deactivated' : 'activated'} successfully.`})} catch (error: any) {logErrorToProduction('Error toggling tenant status:', { data: error }),
toast({variant: 'destructive',
...
try {// In a real implementation, this would verify DNS records;
      ...
      const { error 
        ...
        .from('whitelabel_tenants').update({ dns_verified: true }).eq('id', (tenant as any).id),
if (error) throw error,// Update local state;
      setTenants(tenants.map(t =>;
        (t as any).id === (tenant as any).id ? { ...t, dns_verified: true } : t;
      )),
toast({title: 'DNS verified',
description: `Custom domain for ${(tenant as any).brand_name} has been verified.`})} catch (error: any) {logErrorToProduction('Error verifying DNS:', { data: error }),
toast({variant: 'destructive',
...
import { format   } from 'date-fns';
export function TenantsList() {
const [tenants, setTenants] = useState < WhitelabelTenant[]>([])const [isLoading, setIsLoading] = useState(true)useEffect(()  => {
loadTenants()
