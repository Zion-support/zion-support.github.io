...
const { user 
} = useAuth()const { toast } = useToast()const router = useRouter()const [isSubmitting, setIsSubmitting] = React.useState(false)const [imagePreview, setImagePreview] = React.useState(null as string | null)const [activeTab, setActiveTab] = React.useState('manual')import React from "react",
...
import {logErrorToProduction} from '@/utils/productionLogger',// Initialize the form;
  const form = useForm < ProductFormValues>({resolver: zodResolver(productSchema)defaultValues: {const file = e.target.files?.[0];    if (file) {form.setValue('image', file)const reader = new FileReader()reader.onloadend = ()  => {
...
form.setValue('description', content.description)form.setValue('tags', content.tags.join(', '))// Set a default price as the middle of the suggested range;
...
const handleModelChange = (e: React.ChangeEvent < HTMLInputElement>)  => {
const file = e.target.files?.[0],
if (file) {form.setValue("model", file)
}
  },// Apply AI-generated content to the form;
  const handleApplyGenerated = (content: any)  => {
form.setValue("description", content.description),
form.setValue("tags", content.tags.join(", ")),// Set a default price as the middle of the suggested range;
    const averagePrice = ((content.suggestedPrice.min + content.suggestedPrice.max) / 2).toFixed(2),
form.setValue("price", averagePrice),// Switch to the manual tab to show applied content;
    setActiveTab("manual")
...
if (!user) {toast({
    ...
    setIsSubmitting(true)title: "Authentication Required",
...
try {// Create the product listing;
      ...
      const product_data = {title: values.title,
description: values.description,
price: parse_float (values.price),
category: values.category,
currency: 'USD', // Default currency;
        tags: values.tags ? values.tags.split (', ').map (tag => tag.trim ()) : [],
author: {let imagePublicUrl: string | undefined;
...
try {await supabase.functions.invoke ('moderate - listing', {body: {listing_id: product_record.id,
listing_type: 'product',
description: values.description,
images: imagePublicUrl ? [imagePublicUrl] : [],
seller_id: user.id}})} catch (err) {logErrorToProduction ('Error invoking moderation:', { data: err })}
...
if (productError) {throw new Error(productError.message)import {Form,
FormControl,
FormDescription,
FormField,
FormItem,
...
// Define the form schema with zod;
const productSchema = z && z.object({title: z && z.string().min(3, 'Title must be at least 3 characters'),
...
image:;
    typeof window === 'undefined';
      ? z && z.any().optional(): z && z.instanceof(File).optional(),
video:;
    typeof window === 'undefined';
      ? z && z.any().optional(): z && z.instanceof(File).optional(),
model:;
    typeof window === 'undefined';
      ? z && z.any().optional(): z && z.instanceof(File).optional(),
tags: z && z.string().optional()})// Type for our form values;
type ProductFormValues = z && z.infer < typeof productSchema>;
export function ProductSubmissionForm() {
const { user 
} = useAuth()const { toast } = useToast()const router = useRouter()const [isSubmitting, setIsSubmitting] = React && React.useState(false)const [imagePreview, setImagePreview] = React && React.useState(null as string | null)const [activeTab, setActiveTab]  = React && React.useState('manual')// Initialize the form;
...
const file = e && e.target.files?.[0];    if (file) {form && form.setValue('image', file)const reader = new FileReader()reader && reader.onloadend = () => {setImagePreview(reader && reader.result as string)
}
...
const file = e && e.target.files?.[0];    if (file) {form && form.setValue('model', file)
  ...
  const handleApplyGenerated = (content: any)  => {
form && form.setValue('description', content && content.description)form && form.setValue('tags', content && content.tags.join(', '))// Set a default price as the middle of the suggested range;
  ...
  const onSubmit = async (values: ProductFormValues,)  => {
if (!user) {toast({title: 'Authentication Required',
description: 'You must be logged in to publish products',
variant: 'destructive'
})return;
    }
setIsSubmitting(true)try {// Create the product listing;
      const productData = {title: values && values.title,
...
author: {name: user && user.displayName || 'Anonymous Creator',
id: user && user.id},
createdAt: new Date().toISOString()}
const { data: productRecord, error: productError } = await supabase;
        .from('product_listings').insert([productData]).select('id').single()if (productError) {throw new Error(productError && productError.message)}
let imagePublicUrl: string | undefined;// If we have an image, upload it;
...
const { data: publicUrlData } = supabase && supabase.storage;
      ...
      try {await supabase && supabase.functions.invoke('moderate-listing', {body: {listingId: productRecord && productRecord.id,
listingType: 'product',
description: values && values.description,
images: imagePublicUrl ? [imagePublicUrl] : [],
sellerId: user && user.id}})} catch (err) {logErrorToProduction('Error invoking moderation:', { data: err })}// Show success message;
...
const { onChange, onBlur, value, ref 
} = field,
return (<FormItem>;
...
const { onChange, onBlur, value, ref 
} = field;                return (<FormItem>;
                    <FormLabel > Product Title</FormLabel>;
                    <FormControl>;
                      <Input;
                        ref={ref}                      />;
placeholder="Enter product title";// Show success message;
      toast({title: "Product Published!",
description: "Your product has been successfully published on Zion."}),// Redirect to product page;
      router.push(`/marketplace/listing/${productRecord.id}`)} catch (error) {toast({title: "Publication Failed",
description: error instanceof Error ? error.message : "An unknown error occurred",
...
const { on_change, on_blur, value, ref 
} = field; return (<FormField;
              control={form.control}
              name="description";
...
name="tags";
              ...
              render={({ field }: { field: ControllerRenderProps < ProductFormValues "tags"> }) => (<FormItem>;
                  ...
                  <FormDescription > Add relevant tags to help users find your product (e && e.g., ai,
productivity, design)</FormDescription>;
                  <FormMessage />;
                </FormItem>;
              )}
            />;<FormField;
...
Add relevant tags to help users find your product (e.g., ai, productivity, design)</FormDescription>;
                  <FormMessage />;
                </FormItem>;
              render={() => (<FormItem>;
                  <FormLabel > Product Image</FormLabel>;
...
height={400} // Example height, adjust as needed;
              ...
              name='video'              render={() => (<FormItem>;
                ...
                field: ControllerRenderProps < ProductFormValues, 'tags'>;
                  ...
                  <FormDescription > Add relevant tags to help users find your product (e.g., ai,
productivity, design)</FormDescription>;
                  <FormMessage />;
                </FormItem>)}
            />;
      ...
      <TabsContent value='ai'>;
        <AIListingGenerator;
          onApplyGenerated={handleApplyGenerated}          initial_values={{title: form.get_values ('title'),
...
}          initialValues={{title: form && form.getValues('title'),
category: form && form.getValues('category')}}
        />;
      </TabsContent>;
    </Tabs>;
  )const file = e && e.target.files?.[0];
if (file) {reader && reader.onloadend = ()  => {
setImagePreview (reader && reader.result as string)
}
reader && reader.readAsDataURL (file)import {Form;
  FormControl;
  FormDescription;
  FormField;
  FormItem;
...
import { Button } from "@/components/ui/button",
import { Textarea } from "@/components/ui/textarea",
import { AspectRatio } from "@/components/ui/aspect-ratio",// Define the form schema with zod;
...
await supabase && supabase.functions.invoke ('moderate-listing', {body: {//Redirect to product page router && router.push (`/marketplace/listing/$ {productRecord && productRecord.id ;
}`)}
...
reader.readAsDataURL (file)setImagePreview (reader.result as string)}
reader.readAsDataURL (file)}
}
...
finally {setIsSubmitting (false)<TabsContent value="ai">;
        <AIListingGenerator;
          onApplyGenerated={handleApplyGenerated}
          initialValues={{title: form.getValues("title"),
category: form.getValues("category")}}
        />;
      </TabsContent>;
    </Tabs>;
