...
const handler = (e: BeforeInstallPromptEvent) => {e.preventDefault(),
setPromptEvent(e)
},
...
const onClick = async ()  => {
if (!promptEvent) {toast('Installation not available', {description: 'Your browser does not support app installation.'
}),
return;
    }
    try {setIsInstalling(true),
promptEvent.prompt(),
const result = await promptEvent.userChoice,
setIsInstalling(false),
if (result.outcome === 'accepted') {toast.success('App installed'),
setPromptEvent(null)} else {toast('Installation dismissed'),
safeStorage.setItem(DISMISS_KEY, Date.now().toString()),
setPromptEvent(null)}
    } catch (err) {setIsInstalling(false),
...
const [promptEvent, setPromptEvent] =;
  ...
  useEffect(() => {if (typeof window === 'undefined') return;
const dismissedAt = safeStorage.getItem(DISMISS_KEY)const recentlyDismissed =;
      dismissedAt && Date.now() - Number(dismissedAt) < DISMISS_MS;
    const inStandalone = window.matchMedia('(display-mode: standalone)';
    ).matches;
if (recentlyDismissed || inStandalone) return;
const handler = (e: BeforeInstallPromptEvent) => {e.preventDefault()setPromptEvent(e)
}
