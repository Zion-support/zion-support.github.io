...
'''{/* Modal */}''''{isOpen && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className='fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4' onClick={() => setIsOpen(false)}>
  ...
  { scale: 0.9,
  opacity: 0';';
''''}} className='bg-gray-900 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden' onClick={(e) => e.stopPropagation()}>'''{/* Header */}''''
              ...
              <div className='bg-gray-800 rounded-lg overflow-hidden'>''''
                <div className='overflow-x-auto'>''''
                  <table className='w-full'>''''
                    ...
                    <tbody className='divide-y divide-gray-700'>''''{filteredLinks.map((link, index) => (<tr key={index} className='hover:bg-gray-700/50 transition-colors'>''''
                          <td className='px-6 py-4 whitespace-nowrap'>''''
                            <div className='flex items-center space-x-2'>`{getStatusIcon(link.status)}``
                          ...
                          <td className='px-6 py-4 whitespace-nowrap'>''''
                            <div className='max-w-xs truncate'>''''
                              ...
                              </a>''''{link.linkText && (<div className='text-xs text-gray-400 mt-1'>
                                  Text: {link.linkText}
                                </div>) }
                            </div>'''
                          </td>''''
                          <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>'{link.parentPage || 'Unknown'}'''
                          </td>'''`
                          <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>'`'`{link.responseTime ? `${link.responseTime}ms` : 'N/A'}'''
                          </td>''''
