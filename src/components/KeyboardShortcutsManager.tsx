...
const KeyboardShortcutsManager: React.FC<KeyboardShortcutsManagerProps> = ({
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    const matchingShortcut = activeShortcuts.find(shortcut => {
      ...
      return shortcutKeys.length === currentKeys.length &&
  ...
  }, [activeShortcuts, isRecording, recordingShortcut, shortcuts, onShortcutTriggered]);
          ...
          <div className="bg-gray-900 rounded-lg shadow-2xl w-full max-w-4xl max-h-[80vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              {Object.keys(shortcutsByCategory).length === 0 ? (
                <p className="text-gray-400 text-center py-8">No keyboard shortcuts configured</p>
              ) : (
                        ...
                        {categoryShortcuts.map((shortcut) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {isRecording && recordingShortcut?.id === shortcut.id ? (
                                <div className="flex items-center gap-2">
                                  <div className="flex gap-1">
                                    {pressedKeys.map((key, index) => (
                                      <kbd key={index} className="px-2 py-1 bg-red-600 text-white text-xs rounded">
                                        {formatKeys([key])}
                                      </kbd>
                                    ))}
                                  </div>
                                ...
                                <div className="flex items-center gap-2">
                                  <div className="flex gap-1">
                                    {shortcut.keys.map((key, index) => (
                                      <kbd key={index} className="px-2 py-1 bg-gray-700 text-white text-xs rounded">
                                        {formatKeys([key])}
                                      </kbd>
                                    ))}
                                  </div>
      ...
      {isRecording && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 bg-red-600 text-white px-4 py-2 rounded-lg shadow-lg animate-pulse">
          <div className="flex items-center gap-2">
            ...
            <span>Recording shortcut for: {recordingShortcut?.description}</span>
          </div>
        </div>
      )}
    </>
