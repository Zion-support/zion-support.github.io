import React, { useState, useEffect, useRef, useCallback } from,
  ...
  'lucide-react'; ; interface SearchResult {; id: string; title: string; description: string; type:
  'service' |
  'page' |
  'article' |
  'case-study' |
  'tool; category: string; tags: string[]; url: string; relevance: number; lastUpdated: Date; views: number; rating: number; }; interface SearchFilter {; type: string[]; category: string[]; tags: string[]; dateRange:,
  all' |
  'week' |
  'month' |
  'year'; sortBy: 'relevance' |
  'date' |
  'popularity' |
  'rating; }; export function AdvancedSearch() {; const [isOpen, setIsOpen] = useState(false); const [query, setQuery] = useState('
