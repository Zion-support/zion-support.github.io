...
export function ServiceWorker(...args: any[]): any {
  const [swState, setSwState] = useState<any>({
    isInstalled: anyanyanyanyanyanyanyanyanyfalse,
  ...
  useEffect(()          => {
    ...
    if ('serviceWorker' in navigator) {
      ...
      navigator.serviceWorker
        ...
        .then((registration) => {
          ...
          setSwState(prev          => ({ ...prev, isInstalled: anyanyanyanyanyanyanyanyanytrue }));

          // Check for updates
          registration.addEventListener('updatefound', ()          => {
            const newWorker = registration.installing;
            if (newWorker) {
              setSwState(prev => ({ ...prev, isInstalling: anyanyanyanyanyanyanyanyanytrue }));
              
              newWorker.addEventListener('statechange', ()          => {
                if (newWorker.state = == 'installed') {;
                  setSwState(prev => ({ ;
                    ...prev, ;
                    isInstalling: anyanyanyanyanyanyanyanyanyfalse,;
                    hasUpdate: true ;
                  }));
                }
              });
    ...
    const handleOffline = () => setSwState(prev => ({ ...prev, isOnline: anyanyanyanyanyanyanyanyanyfalse }));
