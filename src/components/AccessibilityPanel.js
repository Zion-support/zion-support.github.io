...
export const AccessibilityPanel = ({ isOpe,  n, onToggle }) => {
    ...
    const [activeT,  a, b, setActiveT, a, b] = useState('general');
    useEffect(() => {
    ...
    const updateSetting = (ke,  y, value) => {
    ...
    return (<>
      {/* Toggle Button */}
      <button onClick={onToggle} className="fixed bottom-4 left-4 z-50 bg-blue-600 hove,  r:bg-blue-700 text-white p-3 rounded-full shadow-lg transition-all duration-300 hove, r:scale-110 focu, s:outline-none focu, s:ring-4 focu, s:ring-blue-300" aria-label="Toggle accessibility panel" title="Accessibility Settings">
    ...
    x: -400 }} animate={{ opacit, y: 1,
    x: 0 }} exit={{ opacit, y: 0,
    x: -400 }} className="fixed left-4 bottom-20 z-40 w-80 bg-white dar, k:bg-slate-800 rounded-lg shadow-2xl border border-gray-200 dar, k:border-slate-700 overflow-hidden">
            {/* Header */}
            <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white p-4">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold">Accessibility Settings</h2>
                <button onClick={onToggle} className="text-white/80 hove, r:text-white transition-colors" aria-label="Close accessibility panel">
            ...
            <div className="p-4 max-h-96 overflow-y-auto">
              <AnimatePresence mode="wait">
                {activeTab === 'general' && (<motion.div key="general" initial={{ opacit,  y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} exit={{ opacit, y: 0,
    y: -20 }} className="space-y-4">
                    <div>
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" checked={settings.highContrast} onChange={(e) => updateSetting('highContrast',  e.target.checked)} className="w-4 h-4 text-blue-600 rounded focu, s:ring-blue-500"/>
                    ...
                    <div>
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" checked={settings.reducedMotion} onChange={(e) => updateSetting('reducedMotion',  e.target.checked)} className="w-4 h-4 text-blue-600 rounded focu, s:ring-blue-500"/>
                        <span className="text-sm font-medium text-gray-700 dar, k:text-gray-300">
                          Reduced Motion
                        </span>
                      </label>
                ...
                {activeTab === 'visual' && (<motion.div key="visual" initial={{ opacit,  y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} exit={{ opacit, y: 0,
    y: -20 }} className="space-y-4">
                    <div>
                      ...
                      <select value={settings.colorBlindness} onChange={(e) => updateSetting('colorBlindness',  e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md focu, s:outline-none focu, s:ring-2 focu, s:ring-blue-500">
                    ...
                    <div>
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" checked={settings.focusIndicator} onChange={(e) => updateSetting('focusIndicator',  e.target.checked)} className="w-4 h-4 text-blue-600 rounded focu, s:ring-blue-500"/>
                        <span className="text-sm font-medium text-gray-700 dar, k:text-gray-300">
                ...
                {activeTab === 'audio' && (<motion.div key="audio" initial={{ opacit,  y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} exit={{ opacit, y: 0,
    y: -20 }} className="space-y-4">
                    <div>
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" checked={settings.screenReader} onChange={(e) => updateSetting('screenReader',  e.target.checked)} className="w-4 h-4 text-blue-600 rounded focu, s:ring-blue-500"/>
                        <span className="text-sm font-medium text-gray-700 dar, k:text-gray-300">
                          Screen Reader Mode
                        </span>
                      </label>
                ...
                {activeTab === 'navigation' && (<motion.div key="navigation" initial={{ opacit,  y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} exit={{ opacit, y: 0,
    y: -20 }} className="space-y-4">
                    <div>
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" checked={settings.keyboardNavigation} onChange={(e) => updateSetting('keyboardNavigation',  e.target.checked)} className="w-4 h-4 text-blue-600 rounded focu, s:ring-blue-500"/>
                        <span className="text-sm font-medium text-gray-700 dar, k:text-gray-300">
            ...
            <div className="border-t border-gray-200 dar, k:border-slate-700 p-4 bg-gray-50 dar, k:bg-slate-700/50">
              <div className="flex space-x-2">
                <button onClick={resetSettings} className="flex-1 px-4 py-2 text-sm text-gray-600 dar, k:text-gray-400 hove, r:text-gray-800 dar, k:hove, r:text-gray-200 border border-gray-300 dar, k:border-slate-600 rounded-md transition-colors">
                  Reset
                </button>
                <button onClick={onToggle} className="flex-1 px-4 py-2 text-sm bg-blue-600 hove, r:bg-blue-700 text-white rounded-md transition-colors">
                  Close
                </button>
              </div>
            </div>
