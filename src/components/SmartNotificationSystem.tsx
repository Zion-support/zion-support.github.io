...
import { motion, AnimatePresence } from "framer-motion";import { Bell, X;
  CheckCircle;
  AlertCircle;
  Info;
  XCircle;
  ...
  id: string,
    type: 'success' | 'error' | 'warning' | 'info' | 'achievement, ',title: strin, g,message: strin, g,timestamp: Dat, e,read: boolean,
    action?: {
    label: string,
    onClick: () => void,  },
...
export function SmartNotificationSystem() {
  ...
  const generateSampleNotifications  = useCallback(() => {
    const sampleNotifications: Notification[] = [
      {
        id: '1, ',type: 'success, ',title: 'Welcome to Zion Tech Group!, ',message: 'Your account has been successfully created. Explore our AI-powered solutions., ',timestamp: new Date(Date.now() - 1000 * 60 * ,
    5)// 5 minutes ago
        read: false,
    priority: 'high, ',category: 'onboarding, ',action:  , {,
        ...
        read: false,
    priority: 'medium, ',category: 'performance, ',action:  , {,
     ...
     ,  },
      {
        ...
        read: false,
    priority: 'high, ',category: 'security, ',action:  , {,
          label: 'Update Now, ',onClick: () => console.log('Update Now clicked'),
     }
      }
    ]setNotifications(sampleNotifications)setUnreadCount(sampleNotifications.filter(n => !n.read).length),
  ...
  const getNotificationIcon = () => {
    switch (type) {
      case 'success':
        return <CheckCircle className="w-5 h-5 text-green-500" />case 'error':
        return <XCircle className="w-5 h-5 text-red-500" /, >
  }case 'warning':
        return <AlertCircle className="w-5 h-5 text-yellow-500" />case 'info':
        return <Info className="w-5 h-5 text-blue-500" />case 'achievement':
        return <Award className="w-5 h-5 text-purple-500" />default: return <Info className="w-5 h-5 text-gray-500" /,
    >
   }
  }// Get priority color
  const getPriorityColor  = () => {
    switch (priority) {
      case 'high':
        return 'border-l-red-500'case 'medium':
  ...
  const formatTimestamp = () => {
    ...
    return `${days}d ago`,}if (!enabled) return null;
...
return (
            ...
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={clearAllNotification, s}
    ...
    l} →
                        ...
                        <div className="flex items-center gap-2 ml-3">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation()removeNotification(notification.id)}}
