...
interface TestimonialCardProps {
 ...
 = ({testimonialisVisible 
    ...
    returnArray.from({ length: 5 }(_i) => (<svg  key ={i}
  ...
  return (<divclassName={`group  p-8, bg-white  rounded-2xlshadow-lghover:shadow-2xltransition-allduration-500borderborder-gray-100hover:border-blue-200hover-lift ${isVisible?'opacity-100translate-y-0':'opacity-0translate-y-8'}`}    >
      {/* Rating */}
...
export const TestimonialCard: React.FC<TestimonialCardProps> = ({ testimonial  isVisible }) => {
  ...
  return (
    <div className={`group p-8 bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-blue-200 hover:-translate-y-1 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      ...
      {/* Quote decoration */}
      <div className="absolute top-4 right-4 text-blue-200 text-6 xl font-serif leading-none opacity-20">
        &ldquo;
      </div>
    </div>
  )};