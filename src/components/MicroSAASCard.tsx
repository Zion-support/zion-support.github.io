...
const MicroSAASCard: React.FC<MicroSAASCardProps> = ({ service }) => {
  return (
    <div className={`cyber-card p-6 hover:scale-105 transition-all duration-300 ${service.popular ? 'ring-2 ring-cyan-400' : ''}`}></div><//div>
      {service.popular && (</div><>
<//div>
<div className="absolute -top-3 left-1/2 transform -translate-x-1/2"></div><>
<//div>
<span className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-4 py-1 rounded-full text-sm font-medium"></span className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-4 py-1 rounded-full text-sm font-medium">
            Most Popular
          </span><>
<//span>
</div><//div>
...
<span className="text-2xl font-bold text-cyan-400">{service.price}</span><//span>
          {service.marketPrice && (
            <span className="text-sm text-gray-500 line-through">{service.marketPrice}</span><//span>
          )}
        </div><>
<//div>
...
<Star className="w-3 h-3 text-yellow-400 mr-2 flex-shrink-0" /></Star className="w-3 h-3 text-yellow-400 mr-2 flex-shrink-0" />
                {benefit}
              </li><//li>
            ))}
          </ul><>
...
<span key={index} className="bg-gray-800 text-gray-300 px-2 py-1 rounded text-xs"></span key={index} className="bg-gray-800 text-gray-300 px-2 py-1 rounded text-xs">
                {tech}
              </span><//span>
            ))}
          </div><>
