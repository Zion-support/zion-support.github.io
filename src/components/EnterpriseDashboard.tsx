  ...
  const [securityAlerts] = useState([
    {
      ...
      description:
        'Multiple login attempts from different locations within short time frame',
              ...
              <div className='bg-gray-50 dark:bg-gray-700 rounded-lg p-6 border border-gray-200 dark:border-gray-600'>
                ...
                <div className='space-y-3'>
                  {securityAlerts.slice(0, 3).map(alert => (
                    <div
                      key={alert.id}
                      className='flex items-start gap-3 p-3 bg-white dark:bg-gray-600 rounded-lg'
                    >
                      <div
                      ...
                      <div className='flex-1'>
                        ...
                        <div className='flex items-center gap-4 text-xs text-gray-500'>
                          ...
                          <span
                            className={`px-2 py-1 rounded ${getStatusColor(alert.status)}`}
                          >
                            {alert.status}
                          </span>
                        </div>
                ...
                {filteredSecurityAlerts.map(alert => (
                  ...
                  >
                    <div className='flex items-start justify-between mb-4'>
                      <div className='flex items-center gap-3'>
                        <span
                          className={`px-3 py-1 text-sm rounded-full ${getSeverityColor(alert.severity)}`}
                        >
                          {alert.severity.toUpperCase()}
                        </span>
                        <span
                          className={`px-3 py-1 text-sm rounded-full ${getStatusColor(alert.status)}`}
                        >
                          {alert.status}
                        </span>
                ...
                {serviceStatuses.map(service => (
                  ...
                  >
                    <div className='flex items-center justify-between mb-4'>
                      ...
                      <span
                        className={`px-3 py-1 text-sm rounded-full ${getStatusColor(service.status)}`}
                      >
                        {service.status}
                      </span>
                    </div>
                    ...
                    {service.lastIncident && (
                      <div className='mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg'>
                        ...
                        <div className='text-xs text-yellow-700 dark:text-yellow-300'>
                          {service.lastIncident.type} -{' '}
                          {service.lastIncident.timestamp.toLocaleDateString()}
                        </div>
                      </div>
                    )}
                  </motion.div>
