  ...
  useEffect(() => {
    ...
    const updateData = () => {
      try {
        ...
        const fetchedErrors = enhancedErrorHandler.getErrorReports().slice(0, 10) as unknown as ErrorReport[];
        setErrors(Array.isArray(fetchedErrors) ? fetchedErrors : []);
      } catch (error) {
                  ...
                  <div key={index} className="bg-white p-4 rounded-lg border border-red-200">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-xs text-gray-500">
                        {typeof error.lastOccurrence === 'number' ? new Date(error.lastOccurrence).toLocaleString() : error.lastOccurrence}
                      </span>
                    </div>
                    ...
                    {error.resolutionSuggestions && error.resolutionSuggestions.length > 0 && (
                      <div className="mt-2">
                        ...
                        <ul className="mt-1 space-y-1">
                          {error.resolutionSuggestions.map((suggestion, suggestionIndex) => (
