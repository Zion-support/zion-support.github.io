      ...
      <div className="relative w-full max-w-2xl bg-slate-900/95 backdrop-blur-md rounded-xl shadow-2xl border border-cyan-400/20">
        ...
        <div className="flex items-center p-4 border-b border-cyan-400/20">
          ...
          <input
            ...
            value={query} onChange={handleInputChange} className="flex-1 bg-transparent text-white placeholder-gray-400 outline-none text-lg"
          />
          <button
            onClick={onClose} className="p-2 text-gray-400 hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {recentSearches.map((search, index) => (
                    <button}
                      key={index} onClick={() => handleSuggestionClick(search)} className="px-3 py-1 bg-slate-800/50 text-gray-300 rounded-full text-sm hover:bg-cyan-400/20 hover:text-cyan-400 transition-colors"
                    >
                      {search}
                    </button>
                  ))}
              ...
              {isSearching ? (
                ...
                <div className="space-y-2">
                  {results.map((result, index) => (
                    <button}
                      key={result.id} onClick={() => handleResultClick(result)} className={`w-full text-left p-3 rounded-lg transition-colors ${
                        index === = selectedIndex
