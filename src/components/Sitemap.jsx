...
const Sitemap = () => {}
  ...
  description: 'Welcome to Zion Tech Group'},'{ path: '/about', label: 'About Us', description: 'Learn about our comp and mission' },'{ path: '/contact', label: 'Contact', description: 'Get in touch with our team' },'{ path: '/careers', label: 'Careers', description: 'Join our innovative team' },'{ path: '/blog', label: 'Blog', description: 'Latest insights and technology trends' }
        ],
        ...
        pricing['{ path: '/comprehensive-pricing-2025', label: 'Pricing 2025', description: 'Current year pricing plans' },'{ path: '/comprehensive-pricing', label: 'Comprehensive Pricing', description: 'Detailed pricing information' }
        ],
        showcase['{ path: '/services-showcase', label: 'Services Showcase', description: 'Showcase of our services' },'{ path: '/comprehensive-services-showcase', label: 'Comprehensive Services Showcase', description: 'Complete overview of all services' },'{ path: '/innovative-services-2025', label: 'Innovative Services 2025', description: 'Latest innovative services' }
        ],
        legal['
            { path: '/privacy', label: 'Privacy Policy', description: 'Our privacy policy and data handling' },'{ path: '/terms', label: 'Terms of Service', description: 'Terms and conditions of service' }
        ]}
...
''''
        <div className='grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8'>'''{/* Main Pages */}''''
...
'''{/* Core Services */}''''
          <div className='bg-white rounded-lg shadow-sm p-6'>''''
            ...
            <ul className='space-y-3'>'''{sitemapData.services.map((item) => (<li key={item.path}>''''
                  <Link to={item.path} className='block p-3 rounded-lg hover:bg-slate-50 transition-colors duration-200'>''''
                    <div className='font-medium text-slate-900'>{item.label}</div>''''
                    <div className='text-sm text-slate-600'>{item.description}</div>
                  </Link>
...
'''{/* AI Autonomous Services */}''''
          <div className='bg-white rounded-lg shadow-sm p-6'>''''
            ...
            <ul className='space-y-3'>'''{sitemapData.aiAutonomous.map((item) => (<li key={item.path}>''''
                  <Link to={item.path} className='block p-3 rounded-lg hover:bg-slate-50 transition-colors duration-200'>''''
                    <div className='font-medium text-slate-900'>{item.label}</div>''''
                    <div className='text-sm text-slate-600'>{item.description}</div>
                  </Link>
...
'''{/* Pricing & Showcase */}''''
          <div className='bg-white rounded-lg shadow-sm p-6'>''''
            ...
            <ul className='space-y-3'>'''{sitemapData.pricing.map((item) => (<li key={item.path}>''''
                  <Link to={item.path} className='block p-3 rounded-lg hover:bg-slate-50 transition-colors duration-200'>''''
                    <div className='font-medium text-slate-900'>{item.label}</div>''''
                    <div className='text-sm text-slate-600'>{item.description}</div>
                  </Link>
                </li>))}'''{sitemapData.showcase.map((item) => (<li key={item.path}>''''
                  <Link to={item.path} className='block p-3 rounded-lg hover:bg-slate-50 transition-colors duration-200'>''''
                    <div className='font-medium text-slate-900'>{item.label}</div>''''
                    <div className='text-sm text-slate-600'>{item.description}</div>
                  </Link>
...
'''{/* Legal & Resources */}''''
          <div className='bg-white rounded-lg shadow-sm p-6'>''''
            ...
            <ul className='space-y-3'>'''{sitemapData.legal.map((item) => (<li key={item.path}>''''
                  <Link to={item.path} className='block p-3 rounded-lg hover:bg-slate-50 transition-colors duration-200'>''''
                    <div className='font-medium text-slate-900'>{item.label}</div>''''
                    <div className='text-sm text-slate-600'>{item.description}</div>
                  </Link>
