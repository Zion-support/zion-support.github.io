...
export const _ContentQualityEnhancer = ({
  className = '',
  showAnalysis = true,
  autoAnalyze = true,
  targetElements = [
  ...
  const _analyzeContent = useCallback(async () => {
    ...
    const _contentElements = document.querySelectorAll(
      targetElements.join(', ')
    );
    contentElements.forEach((element, index) => {
      ...
      if (element.tagName.match(/^H[1-6]$/)) {
        ...
        const _previousHeadings = Array.from(contentElements)
          .slice(0, index)
          .filter(el => el.tagName.match(/^H[1-6]$/));
        if (previousHeadings.length > 0) {
          const _lastLevel = parseInt(
            previousHeadings[previousHeadings.length - 1].tagName.charAt(1)
          );
    ...
    if (wordCount < 300) {
      suggestions.push({
        ...
        description:
          'Consider adding more content to improve SEO and user engagement',
        priority: 'high',
        implementation:
          'Add relevant sections, expand existing content, include more examples',
        expectedImpact: 15,
      });
    }
    if (readabilityScore < 80) {
      suggestions.push({
        ...
        description:
          'Content could be more readable for better user experience',
        priority: 'medium',
        implementation:
          'Use shorter sentences, break up long paragraphs, simplify complex language',
        expectedImpact: 12,
      });
    }
    if (seoScore < 80) {
      suggestions.push({
        ...
        priority: 'high',
        implementation:
          'Fix meta descriptions, add alt text, improve heading structure',
        expectedImpact: 20,
      });
    }
    if (engagementScore < 80) {
      suggestions.push({
        ...
        priority: 'medium',
        implementation:
          'Add interactive elements, improve content structure, include calls-to-action',
        expectedImpact: 10,
      });
    }
    // Calculate overall score
    const _overallScore = Math.round(
      (readabilityScore + seoScore + engagementScore) / 3
    );
          ...
          case 'empty-content':
            if (issue.element.textContent?.trim() === '') {
              issue.element.innerHTML =
                '<em>Content placeholder - please add relevant information</em>';
              fixedCount++;
            }
            break;
          case 'missing-alt':
            ...
            <div className='p-4 max-h-96 overflow-y-auto'>
                ...
                <div className='space-y-4'>
                  {!analysis ? (
                    ...
                    <>
                      {/* Overall Score */}
                      <div
                        className={`p-4 rounded-lg ${getScoreBgColor(analysis.overallScore)}`}
                      >
                      ...
                      <div className='grid grid-cols-3 gap-3'>
                        <div
                          className={`p-3 rounded-lg ${getScoreBgColor(analysis.readabilityScore)}`}
                        >
                          <div className='text-center'>
                            <div
                              className={`text-lg font-bold ${getScoreColor(analysis.readabilityScore)}`}
                            >
                        ...
                        <div
                          className={`p-3 rounded-lg ${getScoreBgColor(analysis.seoScore)}`}
                        >
                          <div className='text-center'>
                            <div
                              className={`text-lg font-bold ${getScoreColor(analysis.seoScore)}`}
                            >
                              {analysis.seoScore}
                            </div>
                            <div className='text-xs text-gray-600 dark:text-gray-400'>
                        ...
                        <div
                          className={`p-3 rounded-lg ${getScoreBgColor(analysis.engagementScore)}`}
                        >
                          <div className='text-center'>
                            <div
                              className={`text-lg font-bold ${getScoreColor(analysis.engagementScore)}`}
                            >
                      ...
                      <div className='p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg'>
                        <div className='text-sm text-blue-800 dark:text-blue-200'>
                          ...
                          <div>
                            {analysis.suggestions.length} suggestions available
                          </div>
                        </div>
                      </div>
                    </>
                    ...
                    <div className='space-y-3'>
                      {analysis.issues.map(issue => (
                        <div
                          ...
                          onClick={() =>
                            issue.element && highlightElement(issue.element)
                          }
                        >
                        ...
                        >
                          <div className='flex items-start gap-2'>
                            ...
                            <div className='flex-1'>
                              ...
                              <div className='flex items-center gap-2 mt-2'>
                                <span
                                  className={`text-xs px-2 py-1 rounded ${getPriorityColor(suggestion.priority)}`}
                                >
              ...
              {activeTab === 'actions' && (
                <div className='space-y-4'>
                  ...
                  {/* Auto-fix Button */}
                  {analysis &&
                    analysis.issues.filter(i => i.fixable).length > 0 && (
                      <button
