...
export const ContentQualityEnhancer: React.FC<ContentQualityEnhancerProps> = ({,
  ...
  const analyzeContent = useCallback(async () => {
    ...
    contentElements.forEach((element, index) : any => {
      const text = element.textContent || "";
      const words = text.trim().split(/\s+/).filter(word => word.length > 0);
      wordCount += words.length;
    ...
    seoScore -= 3;
      ...
      Object.entries(wordFrequency).forEach(([word, count]) : any => {
        if (count > 5 && word.length > 3) {
...
=======
  ...
  const highlightElement = useCallback((element: HTMLElement) : any => {
