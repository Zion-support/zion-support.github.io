...
export const _BrokenLinkFixer = ({
  ...
  const _checkLink = useCallback(async link => {
    ...
    try {
      ...
      const _isInternal =
        link.url.startsWith('/') || link.url.startsWith(window.location.origin);
      if (isInternal) {
        ...
        if (random < 0.1) {
          // 10% chance of broken external link
          return {
            ...link,
            status: 'broken',
  ...
  const _checkAllLinks = useCallback(async () => {
    ...
    for (let _i = 0; i < allLinks.length; i += batchSize) {
      ...
      setLinks(prev =>
        prev.map(link => {
          const _checkedLink = checkedBatch.find(
            checked => checked.url === link.url
          );
  ...
  const _autoFixBrokenLinks = useCallback(() => {
    const _brokenLinks = links.filter(
      link => link.status === 'broken' && link.fixable
    );
    const _fixedCount = 0;
      ...
      if (link.element && link.url.startsWith('#')) {
        ...
        if (!targetElement) {
          ...
          placeholder.style.cssText =
            'padding: 2rem; margin: 1rem 0; background: #f3f4f6; border: 2px dashed #d1d5db; border-radius: 0.5rem; color: #6b7280;';
        ...
        const _randomPage =
          workingPages[Math.floor(Math.random() * workingPages.length)];
        if (randomPage !== link.url) {
          link.element.setAttribute('href', randomPage);
          link.element.setAttribute(
            'title',
            `Redirected from ${link.url} to working page`
          );
          fixedCount++;
        }
      }
        ...
        {isOpen && (
          ...
          >
            ...
            <div className='p-4 max-h-96 overflow-y-auto'>
              ...
              {activeTab === 'broken' && (
                <div className='space-y-4'>
                  {links.filter(link => link.status === 'broken').length ===
                  0 ? (
              ...
              {activeTab === 'healthy' && (
                <div className='space-y-4'>
                  {links.filter(link => link.status === 'healthy').length ===
                  0 ? (
                    <div className='text-center text-gray-500 dark:text-gray-400'>
                              ...
                              <div className='flex-1'>
                                ...
                                <div className='flex items-center gap-2 mt-2'>
                                  ...
                                  <span className='text-xs text-gray-500 dark:text-gray-400'>
                                    Last checked:{' '}
                                    {link.lastChecked.toLocaleTimeString()}
                                  </span>
                                </div>
                              </div>
                            </div>
