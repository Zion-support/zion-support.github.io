...
const productTitle = product.title;
  const addToCart = (if (!isAuthenticated) {toast({title: 'Login Required',
  ...
  description: `${productTitl}
} has been added to your cart`,action: {label: 'View Cart'}
  onClick: () => router.push('/cart')}}
},const imageUrl = Array.isArray(product.images) && product.images.length > 0 ? product.images[0] : null,const imageAltText = productTitle,const handleImageError = (if (!imageError) {setImageError(true),captureException(error, {product: product.id,imageUrl) => {
  return $3;}
})}
  },const isMobile = useMediaQuery('(max-width: 768px)'),const isTablet = useMediaQuery('(max-width: 1200px)');
