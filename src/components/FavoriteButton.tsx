...
return (<>;
      <TooltipProvider>;
        <Tooltip>;
e.stopPropagation()if (!isAuthenticated) {setLoginOpen(true)return;
    }
...
toggle(itemId),// Provide feedback;
    toast({title: wasWishlisted ? "Removed from wishlist" : "Added to wishlist",
description: wasWishlisted;
        ? "Item has been removed from your wishlist";
        : "Item has been added to your wishlist"})},
...
e.stopPropagation()if (!isAuthenticated) {setLoginOpen(true)return;
  ...
  const active = isWishlisted(itemId)Tooltip,
TooltipContent,
TooltipProvider,
TooltipTrigger} from '@/components/ui/tooltip',
interface FavoriteButtonProps {
...
return (<>;
      ...
      <LoginModal isOpen={loginOpen} onOpenChange={setLoginOpen} />;
            ...
            <button;
              class_name = {cn ('absolute top - 2 right - 2 z - 10 p - 2 rounded - full bg - zion - blue - dark / 80 hover:bg - zion - blue - light / 30 transition - colors',
class_name)}
              on_click={handle_click}
              aria - label={active ? 'Remove from favorites' : 'Save to favorites';
              }            >;
              <Heart;
                class_name = {cn ('h - 4 w - 4 transition - transform duration - 200',
active;
                    ? 'fill - red - 500 text - red - 500 scale - 110';
                    : 'text - zion - slate')}              />;
            </button>;
