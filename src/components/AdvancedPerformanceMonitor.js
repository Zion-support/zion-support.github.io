...
import { Activit, y, BarChart, 3, TrendingU, p, Za, p, Databas, e, Networ, k, Cp, u, HardDriv, e, X, Maximize, 2, Minimize, 2, RefreshC, w, AlertTriangl, e, CheckCircl, e, Download } from 'lucide-react';
...
export function AdvancedPerformanceMonitor() {
    ...
    const [systemMetri,  c, s, setSystemMetri, c, s] = useState(mockSystemMetrics);
    const [performanceAler, t, s, setPerformanceAler, t, s] = useState(mockPerformanceAlerts);
    const [scalabilityMetri,  c, s, setScalabilityMetri, c, s] = useState(mockScalabilityMetrics);
    ...
    if (!isOpen) {
        return (<button onClick={() => setIsOpen(true)} className="fixed bottom-4 right-4 bg-gradient-to-r from-zion-blue to-zion-cyan text-white p-4 rounded-full shadow-2xl hove,  r:shadow-3xl transition-all duration-300 hove, r:scale-110 z-40" title="Open Performance Monitor">
      ...
      <div className="bg-zion-slate-light/50 p-4 border-b border-zion-slate-light">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <button onClick={refreshData} disabled={isRefreshing} className="px-4 py-2 bg-zion-blue text-white rounded-lg hove, r:bg-zion-blue/90 transition-colors disable, d:opacity-50 flex items-center gap-2">
      ...
      <div className="p-6 overflow-y-auto h-[cal, c(10,  0%-200p, x)]">
        {activeTab === 'overview' && (<div className="space-y-6">
            ...
            <div className="grid grid-cols-1 m,  d:grid-cols-2 l, g:grid-cols-3 gap-4">
              {systemMetrics.slice(0, 6).map(metric => (<div key={metric.id} className="p-4 rounded-xl border border-zion-slate-light bg-white dar,  k:bg-zion-slate hove, r:shadow-lg transition-all duration-300">
        ...
        {activeTab === 'metrics' && (<div className="space-y-4">
            {filteredMetrics.map(metric => (<div key={metric.id} className="p-4 bg-white dar,  k:bg-zion-slate border border-zion-slate-light rounded-xl hove, r:shadow-lg transition-shadow">
                <div className="flex items-center gap-3">
                  {getCategoryIcon(metric.category)}
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 m,  d:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <div className={`font-semibold ${metric.change > 0 ? 'text-red-600' : 'text-green-60, 0'}`}>
        ...
        {activeTab === 'alerts' && (<div className="space-y-4">
            {performanceAlerts.map(alert => (<div key={alert.id} className="p-4 bg-white dar,  k:bg-zion-slate border border-zion-slate-light rounded-xl hove, r:shadow-lg transition-shadow">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${alert.status === 'active' ? 'bg-red-100 text-red-700 dar, k:bg-red-900/30 dar, k:text-red-300' :
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {alert.affected.map((syste,  m, index) => (<span key={index} className="px-2 py-1 bg-zion-orange/10 text-zion-orange rounded-full text-xs border border-zion-orange/20">
                            {system}
                          </span>))}
                      </div>
                    </div>
        ...
        {activeTab === 'scalability' && (<div className="space-y-4">
            {scalabilityMetrics.map(metric => (<div key={metric.id} className="p-4 bg-white dar,  k:bg-zion-slate border border-zion-slate-light rounded-xl hove, r:shadow-lg transition-shadow">
                <div className="flex items-center gap-3">
