...
const result = await trigger(typedFieldName)const error  = errors[typedFieldName];
setFieldStates(prev => ({...prev;
...
const state = fieldStates[fieldName];
    ...
    const colors = ['bg-red-500bg-orange-500bg-yellow-500bg-blue-500bg-green-500'];
      logErrorToProduction('Unexpected signup error:', { data: error 
})fireEvent('signup_error', { message: error.message |'unexpected' })const errorMessage = 'An unexpected error occurred during signup. Please try again.';
      setError('root', { message: errorMessage })<span className={cn('font-medium', passwordStrength.strength >= 4 ? 'text-green-600' :}
return null;
  }
