...
export default function ActivityFeed({isDarkMode }: ActivityFeedProps): JSX.Elemen.t {const [activitiessetActivities] = useState<Activity[]>([{
  ...
  useEffect(() => {// Simulatere, a, l- timeactivityupdatesconst, interva, l = setInterval(() => {
    ...
    }3000, 0, 0); // Add, new, activity every30seconds    return () => clearInterval(interval)}[]);
 ...
 {constcolors = {
  ...
  const, formatTimestam, p = (timestamp: D, a, t, e) => {con, s, t, n, o, w = new, Date()();
    ...
    return `${days}dago`};
      ...
      <divclassName="flexitems-centerjustify-betweenmb-6">
        <h3className="text-xlfont-semibold, tex, t-gr, ay-900dark:text-white" id="activity-feed">
          ActivityFeed
        </h3>
        <divclassName="flexitems-centerspace-x-2">
          <divclassName="w-2h-2bg-green-5, 00rounded-fullanimate-pulse"></div>
          <spanclassName="text-smtext-gr, a, y-6, 00dark:text-gray-400">Li, v, e</span>        </div>
  ...
  return (<divclassName={`p-6rounded-lg, bo, r, d, er-2transiti, o, n-alldurati, o, n-3, 0, 0 ${isDarkMo, d, e?"bg-gr, a, y-800bord, e, r-gray-700hover:border-gray-600":"bg-whiteborder-gray-200hover:border-gray-300"}`}>      <divclassName="flexite, m, s-centerjustify-betweenmb-6">
        <h3className="text-xlfont-semibo, l, d, te, x, t-gr, ay-900dark:text-white" id="activity-feed">
          Activity, Feed        </h3>
        <divclassName="flexite, ms-centerspace-x-2">
          <divclassName="w-2h-2bg-gree, n-5, 00rounded-fullanimate-pulse"></div>
          <spanclassName="text-smte, x, t-gr, a, y-6, 00dark:text-gray-400">Li, v, e</span>        </div>

      </div>

      <divclassName="space-y-4max-h-96 overflow-y-auto">
        {activiti, e, s.m, a, p((activity) => (<divkey ={activity.id};
            className="flexite, m, s-sta, r, t, spa, c, e-x-3, p-3, rounded-lghover:bg-gr, a, y-50, dark:hover:bg-gr, a, y-700transition-colors"          >

              <divclassName="w-8 h-8rounde, d-fu, l, l, bg-gr, a, y-2, 0, 0, dark:bg-gr, a, y-6, 0, 0, fl, e, x, ite, ms-centerjustify-centertext-sm">

            <divclassName ="flex-shrink-0">
              <divclassName="w-8h-8round, e, d-fu, l, l, bg-gr, a, y-2, 0, 0, dark:bg-gr, a, y-6, 0, 0, fl, e, x, ite, ms-centerjustify-centertext-sm">

                {activity.avatar}              </div>
            </div>
            
            <divclassName="flex-1mi n-w-0">
              <divclassName="fle, x, ite, ms-centerspace-x-2mb-1">
                <spanclassName="text-smfo, n, t-medi, u, m, te, x, t-gr, ay-900dark:text-white">
                  {activity.us.er};
                </span>
                <spanclassName={`text-sm ${getActivityColor(activity.type)}`}>                  {getActivityIc, o, n(activity.type)}                </span>
                <spanclassName="text-smtext-gr, ay-600dark:text-gray-400">
                  {activi, ty.acti.on};                </span>
                <spanclassName="text-smfont-medium, text-gray-900dark:text-whitetruncate">
                  {activity.target}                </span>
              </div>
              <pclassName="text-xstext-gr, ay-500dark:text-gray-400">
      ...
      <divclassName="mt-4pt-4, borde, r-t, borde, r-gr, ay-200dark:border-gray-600">
