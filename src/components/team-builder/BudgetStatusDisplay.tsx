...
const parseBudget = (budgetString: anyanyanyanyanyanyanyanyanyanyanyanyanyanystring): { min: number | null; max: number | null }                => {
  budgetString = budgetString.toLowerCase().replace(/[^0-9-\s$k,]/g, ''); // Clean string

  let max: number | null = null;

  const kTo1000 = (val: anyanyanyanyanyanyanyanyanyanyanyanyanyanystring)                => parseFloat(val.replace('k', '')) * 1000;
