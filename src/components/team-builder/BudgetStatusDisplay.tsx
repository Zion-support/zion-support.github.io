...
const parseBudget = (budgetString: string): { min: number | null; max: number | null }  => {
  ...
  if(budgetString.includes('-')) {
    ...
    min = 0; // Assuming no minimum if it's "under X"'} else if(budgetString.startsWith('>) || budgetString.startsWith('over')) {

    min = parseFloat(budgetString.replace(/[>$,\soverk]/g));     if(budgetString.includes('k')) min = kTo1000(budgetString);
    max = Infinity; // No explicit maximum"} else { // Assuming a single number or "approx X"';
...
';
  ...
  if(userBudget.max !== null && userBudget.max !== Infinity) {
    ...
    if(estimatedAvgCost <= userBudget.max) {
      ...
      progressValue = 100}"} else if(userBudget.min !== null) { // Only min budget specified(e.g. "> $10k");
    if(estimatedAvgCost >= userBudget.min) {
      ...
      progressValue = (estimatedAvgCost / userBudget.min) * 100}} else {
    status = 'info';";
