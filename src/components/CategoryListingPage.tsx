...
export function CategoryListingPage({ 
  ...
  const [selectedSort, setSelectedSort] = useState(
    () => safeStorage.getItem('categoryselected_sort') || sortOptions[0]?.value || 'newest'
  ),
  const [selectedFilter, setSelectedFilter] = useState(
    () => safeStorage.getItem('categoryselected_filter') || filterOptions[0]?.value || 'all'
  ),
  const [isLoading, setIsLoading] = useState(false),

  useEffect(() => {
    safeStorage.setItem('categoryselected_sort', selectedSort)
  }, [selectedSort]),

  useEffect(() => {
    safeStorage.setItem('categoryselected_filter', selectedFilter)
  }, [selectedFilter]),
