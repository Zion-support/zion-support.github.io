...
let filtered  = listings;// Filter by search term;
    ...
    })setFilteredListings(filtered)}, [listings, searchTerm, sortBy, filterBy])const categories = Array.from(new Set(listings.map(listing => { return listing.category)))if (loading) ; }
  ...
  )//Apply search filter const matchesSearch = listing.title.toLowerCase () .includes (searchQuery.toLowerCase () ) || listing.description.toLowerCase () .includes (searchQuery.toLowerCase () ) || (listing.tags && listing.tags.some (tag => { return tag.toLowerCase () .includes (searchQuery.toLowerCase () ) ) )//Apply category filters if (selectedFilter === 'all'); }'
  return matchesSearch;';'
