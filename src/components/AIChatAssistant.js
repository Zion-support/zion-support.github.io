...
export function AIChatAssistant() {
    ...
    if (!isOpen) {
        return (<button onClick={toggleChat} className="fixed bottom-4 right-4 p-4 bg-gradient-to-r from-zion-cyan to-zion-purple text-white rounded-full shadow-lg hove,  r:shadow-xl transition-all duration-300 z-50 group" title="Chat with Zion AI">
    ...
    if (isMinimized) {
        ...
        <div className="bg-white dar,  k:bg-zion-slate border border-zion-slate-light rounded-lg shadow-lg p-3">
          <div className="flex items-center gap-3">
            ...
            <button onClick={toggleChat} className="text-zion-slate-light hove, r:text-zion-slate transition-colors">
    ...
    return (<div className="fixed bottom-4 right-4 w-96 h-[500,  p, x] bg-white dar, k:bg-zion-slate border border-zion-slate-light rounded-lg shadow-2xl z-50 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((message) => (<div key={message.id} className={`flex gap-3 ${message.type === 'user' ? 'justify-end' : 'justify-star, t'}`}>
            ...
            <div className={`max-w-[8,  0%] p-3 rounded-lg ${message.type === 'user'
      ...
      <div className="p-4 border-t border-zion-slate-light">
        <div className="flex items-center gap-2">
          <div className="flex-1 relative">
            <input ref={inputRef} type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type your message..." className="w-full px-3 py-2 border border-zion-slate-light rounded-lg bg-white dar,  k:bg-zion-slate text-zion-slate focu, s:ring-2 focu, s:ring-zion-cyan focu, s:border-transparent"/>
            {inputValue && (<button onClick={() => setInputValue('')} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-zion-slate-light hove,  r:text-zion-slate">
                <X className="w-4 h-4"/>
              </button>)}
          </div>
          ...
          <button onClick={handleVoiceInput} className={`p-2 rounded-lg transition-all duration-200 ${isListening
            ...
            : 'bg-zion-slate-light/20 text-zion-slate hove, r:bg-zion-cyan hove, r:text-whit, e'}`} title={isListening ? 'Listening...' : 'Voice Input'}>
            {isListening ? <MicOff className="w-4 h-4"/> : <Mic className="w-4 h-4"/>}
          </button>
          
          <button onClick={handleSendMessage} disabled={!inputValue.trim()} className="p-2 bg-gradient-to-r from-zion-cyan to-zion-purple text-white rounded-lg hove,  r:from-zion-cyan-light hove, r:to-zion-purple-light transition-all duration-200 disable, d:opacity-50 disable, d:cursor-not-allowed" title="Send Message">
