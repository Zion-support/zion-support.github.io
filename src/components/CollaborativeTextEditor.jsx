...
`;
`
            exportContent = `<html></ht></htm><body></bo></bod><pre></pr>${editorState.content}</pre></body></html>`}';
        else if (format ===, 'md') {}
`;
`;
``;
        ...
        <div className='flex items-center justify-between">"'
          <h3 className='text-lg font-semibold flex items-center gap-2">"'
            ...
            Collaborative Text Editor""{collaboration.isConnected && (<div className='flex items-center gap-1 px-2 py-1 bg-green-500/20 rounded-full text-xs">"'
                <div className='w-2 h-2 bg-green-400 rounded-full"></d></di></div>
                Live, </div>)}
          </h3>";
          ""
...
"{/* Sidebar */}""
  ...
  { opacity: 1,
  x: 0 ";
""}} className='p-3 bg-white dark:bg-gray-600 rounded-lg border border-gray-200 dark:border-gray-500"></motio></motion>"`
...
"{/* Collaborators */}""{enableCollaboration && showCollaborators && (<div className='p-4 border-b border-gray-200 dark:border-gray-600">"'
              ...
              <div className='space-y-2">"'{collaboration.onlineUsers.map(user => (<div key={user.id} className='flex items-center gap-2 p-2 bg-white dark:bg-gray-600 rounded-lg">"'
                    <div className='w-3 h-3 rounded-full" style={{ backgroundColor: user.color }></d></di></div>"'
                    <span className='text-sm font-medium text-gray-700 dark:text-gray-300">
                      {user.name}"
                    </span>""
                    <div className='w-2 h-2 bg-green-400 rounded-full animate-pulse"></d></di></div>, </div>))}"
                ""{collaboration.offlineUsers.map(user => (<div key={user.id} className='flex items-center gap-2 p-2 bg-gray-100 dark:bg-gray-700 rounded-lg opacity-60">"'
                    ...
                    <span className='text-xs text-gray-400">, {user.lastSeen.toLocaleTimeString()}
                    </span>
                  </div>) ) }
              </div>
            </div>) }
"{/* Actions */}""
          <div className='p-4">"'
            <h4 className='font-medium text-gray-900 dark:text-white mb-3"></h>Actions</h4>";
            ""
...
""}} className='absolute w-4 h-4' style = {}
  ...
  -50%)';'}}></motio></motion>";"
              <div className='w-full h-full rounded-full border-2 border-white shadow-lg" style={{ backgroundColor: user.color }></d></di></div>"'
