...
export,
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusDisplay(resource.status).color}`}>
                    <div className="flex items-center gap-1">
                      {getStatusDisplay(resource.status).icon}, {resource.status.charAt(0).toUpperCase() + resource.status.slice(1)}
                    </div>
                  </span>
  ...
  r:bg-zion-cyan/20 rounded-lg transition-colors">
      ...
      {/* No Results */}, {filteredResources.length === 0 && (
        <motion.div
          initial={ opaci,
  t: y: 0 }
          animate={ opaci,
