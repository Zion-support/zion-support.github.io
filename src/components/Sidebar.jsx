      ...
      `}" ></div>""
        ...
        <div className="flex items-center justify-between p-4 border-b border-zion-purple/20"></div>"""
          ...
          <button onClick={onClose}"""
            className="lg:hidden p-1 text-zion-slate-light hover:text-white hover:bg-zion-purple/10 rounded"" ></button>"""
...
""
        ...
        <div className="flex-1 overflow-y-auto py-4"></div>""
          {navigationSections.map((section, sectionIndex) => ("""
            <div key={section.title} className="mb-6"></div>"""
              <h3 className="px-4 text-xs font-semibold text-zion-cyan uppercase tracking-wider mb-3"></h3>
                {section.title}"
              </h3>""
              """
                  ...
                  <div key={item.name}" ></div>
                    {item.href ? (
                      <Link to={item.href}`
                        ...
                        {item.description && ("""
                          <span className="text-xs text-zion-slate-light opacity-0 group-hover:opacity-100 transition-opacity"></span>
                            {item.description}
                          </span>
                        )}
                      </Link>"
                    ) : (""
                      <button onClick={() =" ></button> toggleSection(item.name.toLowerCase())}"""
...
""
        ...
        <div className="p-4 border-t border-zion-purple/20"></div>"""
          <div className="text-center"></div>"""
            <p className="text-xs text-zion-slate-light mb-2"></p>
