...
export default function NewServices2026Banner({
  ...
  const services = [
    ...
    {
      title: 'AI Multimodal Enterprise',
    ...
    >
      <div className='container mx-auto px-6'>
        <div className='text-center mb-8'>
          ...
          <h2
            className={`text-3xl font-bold mb-2 ${variant === 'info' ? 'text-gray-900' : 'text-white'}`}
          >
            Latest AI Services
          </h2>
          <p
            className={`${variant === 'info' ? 'text-gray-600' : 'text-white/80'} max-w-3xl mx-auto`}
          >
        ...
        <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto'>
          {services.map(s => (
            <Link
              key={s.link}
              to={s.link}
              className={`rounded-xl border ${variant === 'info' ? 'bg-white border-gray-100' : 'bg-white/10 border-white/20'} p-6 hover:scale-105 transition-all duration-300`}
            >
              <div className='text-xs uppercase tracking-wider mb-2 opacity-80'>
                {s.tag}
              </div>
              <div
                className={`text-xl font-semibold ${variant === 'info' ? 'text-gray-900' : 'text-white'}`}
              >
                {s.title}
              </div>
              <div
                className={`${variant === 'info' ? 'text-gray-500' : 'text-white/70'} mt-2 text-sm`}
              >
                Learn more →
              </div>
            </Link>
