...
export function ApiPlayground({ method, path, params = [] }: ApiPlaygroundProps) {
  ...
  const [paramValues, setParamValues] = useState<Record<string, string>></Record>({}),
  const [body, setBody] = useState("{}"),
  const [response, setResponse] = useState<string | null>(null),
  const [loading, setLoading] = useState(false),

  const handleParamChange = (name: string, value: s</string>tring) => {
    ...
    <div className="space-y-4">
      ...
      {params.map((p) => (
        <Input
          ...
          onChange={(e) => handleParamChange(p.name, e</Input>.target.value)}
        />
      ))}
