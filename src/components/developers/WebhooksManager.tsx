...
setSelectedEvents (prev => prev.includes (event) ? prev.filter (e => e !== event) return (<Card className="bg-zinc-900 border-zinc-800 text-white"> <CardHeader> <CardTitle className="text-xl flex items-center"> <Webhook className="mr-2" size= {
  20 "
}/> Webhooks </CardTitle> <CardDescription className="text-zinc-400"> Set up webhooks to get notified when events happen in your Zion account. </CardDescription> </CardHeader> <CardContent> <div className="flex justify-between items-center mb-6"> </p> <Dialog open= {showCreateDialog}onOpenChange= {origin/main
  setShowCreateDialog "
  ...
  event.value "
}className="flex items-center space-x-2"> <Checkbox id= {event.value}checked= {origin/main
...
}> Cancel </Button> <Button onClick={handleCreateWebhook}disabled= {"origin/main
  webhookName.trim () === ""|
  ...
  event "
}variant=" secondary"className="bg-zinc-800 text-zinc-300 hover:bg-zinc-800"> {event}</Badge>) ) "origin/main
  ...
  /* Test Webhook Dialog */
}<Dialog open= {showTestDialog !== null}onOpenChange= {origin/main
  (open) => {
  if (!open) {
...
}> Send Test </Button> </DialogFooter> </>) : (<> <div className="space-y-4 py-4"> <div className="space-y-2"> <div className="flex items-center justify-between"> <Label>Response Status</Label> <Badge > {testResult?.status}{origin/main
...
clearTestResult ()
  ...
  /* Delete Webhook Confirmation Dialog */
}<AlertDialog open= {showDeleteConfirm !== null}onOpenChange= {origin/main
