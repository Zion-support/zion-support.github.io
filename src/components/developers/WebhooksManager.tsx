...
const [testEventType, setTestEventType] = useState < WebhookEventType>('new_application'),// Load webhooks on mount;
  useEffect(()  => {
fetchWebhooks()import { useState, useEffect  
} from 'react';
...
} = useWebhooks()const [showCreateDialog, setShowCreateDialog] = useState(false)const [showDeleteConfirm, setShowDeleteConfirm] = useState < string | null>(null),
const [showTestDialog, setShowTestDialog] = useState < string | null>(null),
const [showTestResult, setShowTestResult] = useState(false)import { format  } from 'date-fns';
import { Globe,
MoreVertical,
PlayCircle,
Plus,
...
test_webhook,
clearTestResult,
const [showCreateDialog, setShowCreateDialog] = useState (false)const [showDeleteConfirm, setShowDeleteConfirm] = useState < string | null>(null)const [showTestDialog, setShowTestDialog] = useState < string | null>(null)const [showTestResult, setShowTestResult] = useState (false)// Create webhook form state;
...
if (=== '' ||) {$2;
    ...
    await create_webhook (webhook_name,
webhook_url,
selected_events,
webhook_secret.trim () === '' ? undefined : webhook_secret)setShowCreateDialog (false)resetWebhookForm ()}
  const handleToggleStatus = async (webhook_id: string,
current_status: boolean)  => {
await toggle_webhook (webhook_id, !current_status) 
}
  const handleDeleteWebhook = async (webhook_id: string)  => {
await delete_webhook (webhook_id)setShowDeleteConfirm (null) 
}
  const handleTestWebhook = async (webhook_id: string)  => {
await test_webhook (webhook_id, testEventType)setShowTestResult (true)const handleToggleStatus = async (webhook_id: string, current_status: boolean) => {await toggle_webhook (webhook_id, !current_status)
}
  const handleDeleteWebhook = async (webhook_id: string)  => {
await delete_webhook (webhook_id)setShowDeleteConfirm (null)setShowDeleteConfirm (null)
}
const handleTestWebhook = async (webhook_id: string)  => {
await test_webhook (webhook_id, testEventType)setShowTestResult (true)const handleToggleStatus = async (webhook_id: string, current_status: boolean, ) => {await toggle_webhook (webhook_id, !current_status)
},
...
const resetWebhookForm = () =>: any {setWebhookName ('')setWebhookUrl ('')setWebhookSecret ('')setSelectedEvents ([])}
const handleDeleteWebhook = async (webhookId: string,)  => {
await deleteWebhook(webhookId)setShowDeleteConfirm(null)
}
  const handleTestWebhook = async (webhookId: string,)  => {
await testWebhook(webhookId, testEventType)setShowTestResult(true)
}
const {    webhooks,    webhooks;
    loading;
    testResult;
...
const [showTestDialog, setShowTestDialog] = useState < string | null>(null),
const [showTestResult, setShowTestResult] = useState(false),// Create webhook form state;
...
await createWebhook(webhookName,
webhookUrl,
selectedEvents,
...
const handleTestWebhook = async (webhookId: string)  => {
await testWebhook(webhookId, testEventType)setShowTestResult(true)const handleToggleStatus = async (webhookId: string, currentStatus: boolean) => {await toggleWebhook(webhookId, !currentStatus)
}
const handleDeleteWebhook = async (webhookId: string)  => {
await deleteWebhook(webhookId)setShowDeleteConfirm(null)setShowDeleteConfirm(null)
}
const handleTestWebhook = async (webhookId: string)  => {
await testWebhook(webhookId, testEventType)setShowTestResult(true)const handleToggleStatus = async (webhookId: string, currentStatus: boolean,) => {await toggleWebhook(webhookId, !currentStatus)
},
...
setWebhookName('')setWebhookUrl('')setWebhookSecret('')setSelectedEvents([])
  ...
  const eventOptions: {value: WebhookEventType;
    ...
    {value: 'new_application',
label: 'New Application',
description: 'When a talent applies to a job'},{value: 'quote_received',
label: 'Quote Received',
description: 'When a quote is received from talent'},{value: 'milestone_approved',
label: 'Milestone Approved',
description: 'When a project milestone is approved'},{value: 'talent_hired',
label: 'Talent Hired',
description: 'When talent is hired for a project'}];// Toggle an event selection;
                ...
                <div className="space-y-2">;
                  ...
                  <div className="grid gap-2 pt-2">;
                    {eventOptions.map((event) => (<div key={event.value} className="flex items-center space-x-2">;
                        <Checkbox;
                          id={event.value}
checked={selectedEvents.includes(event.value)}<Checkbox;
...
<Dialog;
        open={showTestDialog !== null}
        onOpenChange={open  => {
if (!open) {setShowTestDialog(null)setTestEventType('new_application')if (showTestResult) {setShowTestResult(false)clearTestResult() 
}
        onOpenChange = {(open,)  => {
<Dialog;
        open={showTestDialog !== null
      ...
      <Dialog;
        open={showTestDialog !== null}
onOpenChange={(open)  => {
if (!open) {setShowTestDialog(null)setTestEventType('new_application')if (showTestResult) {setShowTestResult(false)
}
        }}
onOpenChange={(open)  => {
// Check condition;
...
<div className='space-y-2 mt-4'>;
              ...
              <div className='space - y-4 py - 4'>;
                ...
                <Button;
                  variant='default';
                  onClick={()  => {
setShowTestDialog(null)setShowTestResult(false)clearTestResult()
}}
                >                  Close;
                </Button>;
                <Button;
...
setShowTestResult(false)clearTestResult()
      ...
      </Dialog>;{/* Delete Webhook Confirmation Dialog */}
...
onCheckedChange= {() => toggleEvent (event && event.value)}/> <LabelhtmlFor= {event && event.value ";
}
...
checked= {selectedEvents.includes (event.value)}
onCheckedChange= {() => toggleEvent (event.value)}/> <Label htmlFor= {event.value ";
}
...
format (new Date (webhook && webhook.created at),  'MMM d, yyyy')}</span> {webhook && webhook.last triggered at && (<span > Last triggered: {';
...
onOpenChange= {(open)  => {
if (!open) {setShowTestDialog (null)';
