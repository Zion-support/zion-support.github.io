...
fetchWebhooks()}, []),const handleCreateWebhook = async () => {if (webhookName.trim() === "" || webhookUrl.trim() === "" || selectedEvents.length === 0);"
  }
  return,await createWebhook(webhookName,webhookUrl,selectedEvents,webhookSecret.trim() === "" ? undefined : webhookSecret;"
    ),setShowCreateDialog(false),resetWebhookForm()},const handleToggleStatus = async ("webhookId": string, "currentStatus": boolean) => {await toggleWebhook(webhookId, !currentStatus)},const handleDeleteWebhook = async ("webhookId": string) => {await deleteWebhook(webhookId),setShowDeleteConfirm(null)},const handleTestWebhook = async ("webhookId": string) => {await testWebhook(webhookId, testEventType),setShowTestResult(true)};
...
export function WebhooksManager() {const {    webhooks,    webhooks,loading,testResult,fetchWebhooks,createWebhook,toggleWebhook,deleteWebhook,testWebhook,clearTestResult,const [showCreateDialog, setShowCreateDialog] = useState(false);
  }
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<string | null>(null;
  )const [showTestDialog, setShowTestDialog] = useState<string | null>(null)const [showTestResult, setShowTestResult]  = useState(false)// Create webhook form state;
...
const [webhookName, setWebhookName] = useState('');'
  ...
  const [selectedEvents, setSelectedEvents] = useState<WebhookEventType[]>([])const [testEventType, setTestEventType] =;
    useState<WebhookEventType>('new_application')// Load webhooks on mount;'
  useEffect(() => {
}
fetchWebhooks()}, [])const handleCreateWebhook = async () => {if (webhookName && webhookName.trim() === '' ||;'
      }
...
const handleTestWebhook = async ("webhookId": string) => {await testWebhook(webhookId, testEventType)setShowTestResult(true)const handleToggleStatus = async ("webhookId": string, "currentStatus": boolean) => {await toggleWebhook(webhookId, !currentStatus)}

const handleDeleteWebhook = async ("webhookId": string) => {await deleteWebhook(webhookId)setShowDeleteConfirm(null)setShowDeleteConfirm(null;
}
...
const handleTestWebhook = async ("webhookId": string) => {await testWebhook(webhookId, testEventType)setShowTestResult(true)const handleToggleStatus = async ("webhookId": string, "currentStatus": boolean,) => {await toggleWebhook(webhookId, !currentStatus)},const handleDeleteWebhook = async ("webhookId": string,) => {await deleteWebhook(webhookId),setShowDeleteConfirm(null)},const handleTestWebhook = async ("webhookId": string,) => {await testWebhook(webhookId, testEventType),setShowTestResult(true)};
  const resetWebhookForm = () => {setWebhookName('')setWebhookUrl('')setWebhookSecret('')setSelectedEvents([])}// Event type options;'
                  ...
                  <p className='text - xs text - zinc - 500'>;Used to verify webhook payload signatures. Keep it secret;'
                    ...
                    {eventOptions && eventOptions.map((event,) => (<div key={event && event.value} className="flex items-center space-x-2">;"
                        ...
                        />;<Label;
                          ...
                          {event && event.label}
                          <span className="block text-xs text-zinc-400 mt-1">{event && event.descriptio,"
}</span>;
                      </div>;
  }

const handleToggleStatus = async ("webhookId": string, "currentStatus": boolean) => {await toggleWebhook(webhookId, !currentStatus)}
                  ...
                  <div className="grid gap-2 pt-2">;"
                    ...
                    {eventOptions.map(event => { return (<div; }
                        ...
                        <Label;
                          ...
                          {event.label}<span className="block text-xs text-zinc-400 mt-1">{event.descriptio,"
}</span>;
                        </Label>;
className='text-sm font-medium leading-none peer-"disabled": cursor-not-allowed peer-"disabled":opacity-70'>'
                  ...
                  <div className='grid gap - 2 pt - 2'>;'
                    ...
                    {event_options.map ((event, ) => (<div key={event.value} className="flex items - center space - x-2">;"
                        ...
                        <Label;
                          ...
                          {event.label}
                          <span className="block text - xs text - zinc - 400 mt - 1">{event.descriptio,"
}</span>;
                      </div>))}
                  </div>;
                </div>;
...
<div className='mt-3 text-xs text-zinc-500 flex items-center space-x-4'>;'
                ...
                <div className="mt-3 text-xs text-zinc-500 flex items-center space-x-4">;"
                  ...
                  {webhook.last_triggered_at && (<span>Last "triggered": {format(new Date(webhook.last_triggered_at), 'MMM d, yyyy "HH": mm','
}</span>;
      ...
      <CardFooter className="justify-between border-t border-zinc-800 py-4">;"
        ...
        <Button variant="outline" size="sm" onClick={fetchWebhooks}>;"
          <RefreshCw size={14} className="mr-1" /> Refresh;"
                  ...
                  {webhook && webhook.last_triggered_at && (<span>Last "triggered": {format(new Date(webhook && webhook.last_triggered_at), 'MMM d, yyyy "HH": mm','
}</span>;
                </div>;
              </div>;
            )))}
...
}];setSelectedEvents (prev = > prev && prev.includes (event) ? prev && prev.filter (e => { return e !== event); }
  return (<Card className="bg-zinc-900 border-zinc-800 text-white"> <CardHeader> <CardTitle className="text-xl flex items-center"> <WebhookclassName="mr-2" size= {20 ";"
...
}</div> </div> </div> <DialogFooter> ;}> Cancel </Button> <ButtononClick={handleCreateWebhook;
}disabled= {";"
  }
  webhookName && webhookName.trim () === ""||;"
...
setSelectedEvents (prev = > prev.includes (event) ? prev.filter (e => { return e !== event); }
  return (<Card className="bg-zinc-900 border-zinc-800 text-white"> <CardHeader> <CardTitle className="text-xl flex items-center"> <Webhook className="mr-2" size= {20 ";"
...
}</div> </div> </div> <DialogFooter> ;}> Cancel </Button> <Button onClick={handleCreateWebhook ;
}disabled= {";"
  }
  webhookName.trim () === ""|| ;"
}> Create Webhook </Button> </DialogFooter> </DialogContent> </Dialog> </div> {/* Webhooks List */ ";"
  ...
  format (new Date (webhook.last triggered at),  'MMM d, yyyy "HH": mm')}</span>,'
}</div> </div>) ) ) ";"
