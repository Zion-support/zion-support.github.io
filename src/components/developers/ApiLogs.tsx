 ...
 */;
function ApiLogs() {
const { logs, total_logs, loading, fetchApiLogs 
} = useApiKeys ()const [page_size, setPageSize] = useState (25)const [current_page, setCurrentPage] = useState (0)const hasNextPage = current_page < total_pages - 1;
...
const { logs, totalLogs, loading, fetchApiLogs 
} = useApiKeys()const [pageSize, setPageSize] = useState(25)const [currentPage, setCurrentPage] = useState(0)// Load logs on mount and when pagination changes;
  useEffect(()  => {
...
const handleRefresh = null;
        ...
        {logs.length > 0 && (<div className="mb-6">;
          ...
          <table className="w-full border-collapse">;
            ...
            <tbody>;
              ...
              ) : (logs.map((log) => (<tr key={log.id} className="border-b border-zinc-800 hover:bg-zinc-800/40">;
                    <td className="px-4 py-3 text-sm">{formatTimestamp(log.created_at)}</td>;
                    <td className="px-4 py-3">;
                      <Badge;
                        variant="outline";
                        className={log.method === 'GET';
...
ms` : '-'}
        ...
        {logs.length > 0 && (<div className="mt-4 flex justify-between items-center">;
