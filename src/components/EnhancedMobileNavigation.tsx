    ...
    return (
      <div key={item.label} className="w-full">
        ...
        >
          <Link;
            to={{item.path}}
            onClick={{onClick={() => handleNavigation(item.path)}}}
          ...
          {hasChildren && (;
            <button;
              onClick={{onClick={() => toggleExpanded(item.label)}}}
              className="className="absolute right-4 top-1/2 transform -translate-y-1/2 p-2 text-zion-slate-light hover:text-white transition-colors";"
  ...
  return (
    <>
      ...
      <button;
        ...
        className="className="lg:hidden p-2 text-white hover:text-zion-cyan transition-colors focus:outline-none focus:ring-2 focus:ring-zion-cyan/50 rounded-lg";"
      ...
      <AnimatePresence>
        {isOpen && (;
          <motion.div;
            ...
            className="className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 lg:hidden";"
          >
            <motion.div;
              ref={{menuRef}}
              initial={{ x: '100%' }}
              ...
              className="className="absolute right-0 top-0 h-full w-full max-w-sm bg-zion-slate-dark border-l border-zion-cyan/30 shadow-2xl";"
            >
              {{/* Header */}}
              <div className="flex items-center justify-between p-6 border-b border-zion-slate-light/20">
                ...
                <button;
                  ...
                  className="className="p-2 text-zion-slate-light hover:text-white transition-colors rounded-lg hover:bg-zion-slate-light/10";"
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {quickActions.map(action => (;
                      <Link;
                        key={{action.label}}
                        to={{action.path}}
                        onClick={{onClick={() => handleNavigation(action.path)}}}
