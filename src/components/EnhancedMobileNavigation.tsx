  ...
  const toggleExpanded = ("props": "any) => {;
    setExpandedItems(prev => {;
      const newSet = new Set(prev);      if(newSet.has(label)) {;
        newSet.delete(label)"} else {;  const toggleExpanded = (props: any) => {
    setExpandedItems(prev => {
      const newSet = new Set(prev);      if(newSet.has(label)) {
...
;
    setExpandedItems(new Set())};
;
    setExpandedItems(new Set())};    if(path === '/') {};
    return ()      <div key = {item.label} className="w-full">
        <motion.div
          initial={false}
          animate = {';
';';
  { backgroundColor: isItemActiv e ? 'rgba(34, 221, 210,';';            onClick={() => handleNavigation(item.path)}`
...
";
                        ...
                        <span className="text-xs font-medium">{action.label}</span>;                        key={action.label}
                        to={action.path}
                        onClick={() => handleNavigation(action.path)}`
                        className={`${action.color} p-4 rounded-lg text-white text-center hover: scal e-105 transition-transform duration-200`}
"
