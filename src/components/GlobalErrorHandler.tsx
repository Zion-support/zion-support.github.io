...
export function GlobalErrorHandler({ children }: GlobalErrorHandlerProps) {
  const [retryCount, setRetryCount] = useState<Record<string number>>({}),
  const reportError = useCallback(_(error: Error, context?: unknown) => {_// Log to console for development,
if (process.env.NODEENV === 'development') {
      logErrorToProduction('Global Error Handler:', error, context)
    }
    // Report to Sentry for production,
if (process.env.NODEENV === 'production') {Sentry.withScope(_(scope) => {
        if (context) {
          scope.setContext('errorContext', context)
        }
        scope.setLevel('error'),
...
function getErrorMessage(error: Error): string {
  ...
  // Fallback for unknown errors,
return "An unexpected error occurred. Please try again."
    return &quot;Unable to connect to our servers. Please check your internet connection.&quot
  }

  if (message.includes('auth') || message.includes('unauthorized') || message.includes('401')) {
    return &quot;Your session has expired. Please log in again.&quot
  }

  if (message.includes('forbidden') || message.includes('403')) {
    return &quot;You don't have permission to perform this action.&quot
  }

  if (message.includes('not found') || message.includes('404')) {
    return &quot;The requested information could not be found.&quot
  }

  if (message.includes('timeout')) {
    return &quot;Request timed out. Please try again.&quot
  }

  if (message.includes('validation') || message.includes('invalid')) {
    return &quot;Please check your input and try again.&quot
  }

  if (message.includes('server') || message.includes('500')) {
    return &quot;Our servers are experiencing issues. Please try again in a moment.&quot
  }

  // Fallback for unknown errors,
return &quot;An unexpected error occurred. Please try again.&quot
}
...
export function useErrorHandler() {
  ...
  const handleApiError = useCallback((error: any, retryAction?: () => void) => {
    if (error.response?.status === 401 || error.response?.status === 403) {
      showAuthError()
    } else if (error.code === 'NETWORKERROR' || !navigator.onLine) {
      showNetworkError(retryAction)
    } else {
      showRetryableError(error, retryAction)
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
...
export function useErrorHandler() {const { reportError, showRetryableError, showNetworkError, showAuthError} = useGlobalErrorHandler();
  ...
  const handleApiError = useCallback(_(error: unknown, retryAction?: () => void) => {if (error.response?.status === 401 || error.response?.status === 403) {
      showAuthError()} else if (error.code === 'NETWORKERROR' || !navigator.onLine) {showNetworkError(retryAction)} else {showRetryableError(error, retryAction)}
  }, [showRetryableError, showNetworkError, showAuthError]);

  const handleAsyncOperation = useCallback(async <T, _>(
    operation: () => Promise<T>,
    options?: {onError?: (error: Error) => void;
      ...
      if (options?.successMessage) {
        toast({
          title: "Success", description: options.successMessage})
      }
      ...
      if (options?.onError) {
        options.onError(error)} else {handleApiError(error, options?.retryAction)}
