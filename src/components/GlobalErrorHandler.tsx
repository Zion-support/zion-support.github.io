...
const currentRetryCount = retry_count[error_key] || 0;
  ...
  action: retry_action;}
          ? {label: 'Try Again',}
  on_click: () => {setRetryCount (prev = > ({...prev,[error_key]: currentRetryCount + 1}))retry_action ()}}
  ...
  const is_online = typeof navigator !== 'undefined' ? navigator.on_line : true;
    toast ({title: is_online ? 'Connection Issue' : 'No Internet Connection',description: is_online;
        ? 'Unable to connect to our servers. Please check your connection and try again.';
        : 'You appear to be offline. Please check your internet connection.',variant: 'destructive',}
  action: retry_action;}
        ? {label: 'Retry',}
  on_click: retry_action}
        : undefined},
}, [])const showAuthError = useCallback ((login_action?: (, ) => void) => {toast ({title: 'Authentication Required',
  description: 'Please log in to continue with this action.',variant: 'destructive',}
  action: login_action;}
        ? {label: 'Log In',}
...
const currentRetryCount = retryCount[errorKey] || 0;
  ...
  action: retryAction;}
          ? {label: 'Try Again',}
  onClick: () => {setRetryCount(prev => ({...prev,[errorKey]: currentRetryCount + 1}))retryAction()}}
...
export function useErrorHandler() {const { reportError, showRetryableError, showNetworkError, showAuthError } =;
    ...
    ): Promise<T | null /> => {try {const result = await operation()if (options?.successMessage) {toast({title: 'Success',}
  description: options && options.successMessag,}
});
  return result;
...
export function useErrorHandler() {const { reportError, showRetryableError, showNetworkError, showAuthError } = useGlobalErrorHandler(),const handleApiError = useCallback((error: any, retryAction?: () => void) => {if (error.response?.status === 401 || error.response?.status === 403) {showAuthError()} else if (error.code === 'NETWORK_ERROR' || !navigator.onLine) {showNetworkError(retryAction)} else {showRetryableError(error, retryAction)}
  ...
  const handleAsyncOperation = useCallback(async <T />(operation: () => Promise<T />,options?: {onError?: (error: Error) => void,retryAction?: () => void,successMessage?: string;}
    }
