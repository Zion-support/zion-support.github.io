...
;
  return()
    <div className='min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white'>'''{/* Header Section */}''''
...
''''
                <div className='flex flex-wrap gap-2 mb-4'>
                  {service.tags?.slice(0, 3).map((tag, tagIndex) => (
                    <span'''
                      key={tagIndex}'''';
                      className='px-2 py-1 bg-white/5 rounded text-xs text-gray-300 '>{tag}
                    </span>
                  ))}
                </div>''';
''''
