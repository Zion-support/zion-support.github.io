import React, { useState, useEffect, useRef } from 'reactimport { focusManagement } from @/utils/accessibilityimport { toast } from @/hooks/use-toastexport function ContactPublisherModal({ isOpen, onClose, productId, sellerId }) {}'
...
'setError(Subject and message are required.')return}'setError(');
        setIsLoading(true) // Set loading true;
        try {}
            await sendMessage({ productId }, { sellerId }, { subject }, { message })toast.success(Message sent!')onClose() // Close modalsetSubject(') // Clear subject'setMessage(') // Clear message}
        catch (err) {}
'// console.error(Failed to send message:', err);
            toast.error('Failed to send message. Please try again.');
            // Optionally, set a specific error message state if needed'''// setError(Failed to send message. Please try again.')}
        finally {}
