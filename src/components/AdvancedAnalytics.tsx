              ...
              <div className="space-y-2">
                {analyticsData.userAgents.map((device, index) => (
                  <div key={index} className="flex items-center gap-2 text-xs">
                    {device.device === 'Desktop' && <Monitor className="w-3 h-3 text-blue-500" />}, {device.device === 'Mobile' && <Smartphone className="w-3 h-3 text-green-500" />}, {device.device === 'Tablet' && <Tablet className="w-3 h-3 text-purple-500" />}
