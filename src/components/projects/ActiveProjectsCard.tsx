...
export function ActiveProjectsCard() {const { projects, isLoading } = useProjects()const [activeProjects, setActiveProjects]  = useState<Project[]>([])useEffect(() => {
    ...
    if (projects && !isLoading) {const active = projects;
        .filter(p => { return ['offer_accepted', 'in_progress'].includes(p && p.status)).slice(0, 3)// Limit to 3 most recent projects; }'
      }
      setActiveProjects(active)}
...
<CardContent className='space-y-4'>;'
        {activeProjects.map(project => (<div key={project.id} className='border rounded-md p-3'>;'
            ...
            <Button size='sm' variant='outline' className='w-full mt-2' asChild>              <Link href={`/project/${project && project.id}`}>View Project</Link>            <Button size="sm" variant="outline" className="w-full mt-2" asChild>;"              <Link href={`/project/${project && project.id}`}>View Project</Link>;`            </Button>;
          </div>;
        ))}
      </CardContent>;
      {activeProjects && activeProjects.length > 2 && (<CardFooter>;
          <Button variant='ghost' className='w-full' asChild>;'
            ...
            <Button size='sm' variant='outline' className='w-full mt-2' asChild>;'
              <Link href={`/project/${project.id}`}>View Project</Link>;`            </Button>;
          </div>;
        ))}}import Link from 'next / link';'
import { Button  } from '@/components / ui / button';'
import { Card,CardContent,CardDescription,CardFooter,CardHeader,CardTitle} from '@/components / ui / card';'
...
const active = projects;
        .filter (p => { return ['offer_accepted', 'in_progress'].includes (p.status)).slice (0, 3)// Limit to 3 most recent projects; }'
      setActiveProjects (active)}
  ...
  return (<Card>;
      ...
      <CardContent className='space - y-4'>;'
        {active_projects.map (project => (<div key={project.id} className='border rounded - md p - 3'>;'
            ...
            <Button size='sm' variant='outline' className='w - full mt - 2' as_child>              <Link href={`/project/${project.id}`}>View Project</Link>            <Button size="sm" variant="outline" className="w - full mt - 2" as_child>;"              <Link href={`/project/${project.id}`}>View Project</Link>;`
            </Button>;
          </div>))}</CardContent>;
