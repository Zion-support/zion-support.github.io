...
} = useProjects()const [activeProjects, setActiveProjects] = useState < Project[]>([]),
useEffect(()  => {
...
if (isLoading) {import { useEffect, useState  } from 'react';
...
export function ActiveProjectsCard() {
const { projects, isLoading 
} = useProjects()const [activeProjects, setActiveProjects]  = useState < Project[]>([])useEffect(()  => {
...
const [activeProjects, setActiveProjects] = useState < Project[]>([])useEffect(()  => {
if (projects && !isLoading) {const active = projects.filter(p =>;
        ['offer_acceptedin_progress'].includes(p.status)).slice(0, 3), // Limit to 3 most recent projects;
      setActiveProjects(active)
}
...
const { projects, is_loading 
} = use_projects ()const [active_projects, setActiveProjects] = useState < Project[]>([])useEffect (()  => {
    // Check condition;
if ( {) {$2;
...
<Button variant='ghost' className='w-full' asChild>;
  ...
  )}, [projects, isLoading])return (<Card> <CardHeader> <CardTitle className="flex items-center gap-2" > <BriefcaseIcon className="h-5 w-5 text-primary" /> <span > Active Projects</span> </CardTitle> <CardDescription > Your ongoing work</CardDescription> </CardHeader> <CardContent>) )}</div> </CardContent> </Card>)}
