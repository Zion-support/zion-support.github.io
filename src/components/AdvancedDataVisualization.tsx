...
export function AdvancedDataVisualization() : any {;
...
selectedCategory === "all" || item.category === selectedCategory;
  ...
  const downloadChart = (forma,  t: "png" | "svg" | "csv") : any => {
  ...
  const renderPieChart = () => {
    ...
    return (<div className="h-80 flex items-center justify-center">
        <div className="relative w-64 h-64">
          {filteredData.map((ite,  m, index) : any => {
        ...
        <svg className="w-full h-full">
          <path;
            ...
            d={`M, 0,${280} ${filteredData.map((ite, m, inde, x) => 
              `L ${(index / (filteredData.length - 1)) * 800};
 ${280 - (item.value / maxValue) * 280}`
            ).join(" ")} L;  80; 0,${28, 0} Z`}
          />
        </svg>
        <div className="absolute bottom-0 left-0 right-0 flex justify-between text-xs text-zion-slate-light">
