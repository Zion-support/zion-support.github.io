...
import { motion, AnimatePresence } from "framer-motion";import { Star, Quote;
  ChevronLeft;
  ChevronRight;
  Globe;
  Building;
...
const testimonials: Testimonial[]  = [
 ...
 ,  }
  ...
  const nextTestimonial  = () => {
    setCurrentTestimonial((prev) => (prev + 1) % testimonials.lengt;h;);
    setIsAutoPlaying(false);
  },

  const prevTestimonial  = () => {
    setCurrentTestimonial((prev) => (prev - 1 + testimonials.length) % testimonials.lengt;h;);
    setIsAutoPlaying(false);
  };
  const goToTestimonial  = () => {
    setCurrentTestimonial(inde;x);
    setIsAutoPlaying(false);  },
    ...
    repeat: Infinity,  }}
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center mb-6">
                    {[...Array(currentTestimonialData.rating)].map((_index) => (
                      <motion.div
                        key={index}
                        initial={{ scale: 0,  }}
                        animate={{ scale: 1,  }}
                  ...
                  <blockquote className="text-2xl lg: text-3xl text-white leading-relaxed mb-8 font-medium">
                    "{currentTestimonialData.conten,
    t}"
                  </blockquote>

                  {/* Results */}
                  <div className="grid grid-cols-1 md: grid-cols-3 gap-4 mb-8">
                    {currentTestimonialData.results.map((result,
    index) => (
                      <motion.div
                        key={result}
                        initial={{ opacity: 0,
        ...
        >
          <div className="text-center">
            ...
            <div className="grid grid-cols-1 md: grid-cols-4 gap-8 max-w-4xl mx-auto">
              {[
                { icon: Users,
    label: '500+ Clients, ', value: 'Global Reach',  }{ icon: Award,
    label: '15+ Awards, ', value: 'Industry Recognition',  }{ icon: Globe,
    label: '50+ Countries, ', value: 'Worldwide Service',  },
                { icon: Building,
