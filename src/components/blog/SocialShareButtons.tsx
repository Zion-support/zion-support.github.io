    ...
    <div className=&quot;mt-12 flex flex-wrap gap-2&quot;>
      {buttons.map((btn) => (
        <Button key={btn.label} variant=&quot;outline&quot; size=&quot;sm&quot; onClick={btn.onClick} aria-label={btn.label}>
          {btn.icon}
          <span>{btn.label}</span>
  const shareToFacebook = () => {window.open(`https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`, 'blank')
  };

  const shareToLinkedIn = () => {window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${shareUrl}`, 'blank')
  };

  const copyLink = () => {if (typeof window === 'undefined') return;
    navigator.clipboard
      .writeText(window.location.href)
      .then_(() => toast.success('Link copied to clipboard'))
      .catch_(() => toast.error('Failed to copy link'))};
