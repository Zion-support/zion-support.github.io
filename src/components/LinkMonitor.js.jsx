...
import { LinkValidator } from '../utils / linkValidator';
  ...
  const scanPageLinks = async () => {
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      if (href) {
        const result = LinkValidator.validateLink (href,
  ...
  return (<div className="link - monitor bg - white dark:bg - gray - 800 rounded - lg shadow - lg p - 6 max - w-4xl mx - auto">
      <div className="flex items - center justify - between mb - 6">
        ...
        <div className="flex space - x-2">
          ...
          {brokenLinks.length > 0 && (<button
              onClick={fixAllBrokenLinks}
              className="px - 4 py - 2 bg - green - 600 text - white rounded - lg hover:bg - green - 700"
            >
              Fix All ({brokenLinks.length}) </button>) }
        </div>
      </div>
      ...
      {brokenLinks.length > 0 && (<div className="mb - 6">
          ...
          <div className="space - y-3 max - h-64 overflow - y-auto">
            {brokenLinks.map ( (link, index) => (<div
                key={index}
      ...
      {fixedLinks.length > 0 && (<div className="mb - 6">
          ...
          <div className="space - y-2 max - h-32 overflow - y-auto">
            {fixedLinks.slice (-5) .map ( (fix, index) => (<div
                key={index}
