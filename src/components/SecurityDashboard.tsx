...
export const SecurityDashboard: React.FC<SecurityDashboardProps> = ({ className=""}) => {
  ...
  const generateMockEvents = useCallback((): SecurityEvent[] => {
    ...
    for (let i = 0; i < 50; i++ ) {
      ...
      mockEvents.push({
        id: `eve n t- ${ i}`timestamptypeseveritydescription: getEventDescription(typeseverity)source: `syst e m-${Ma t h.flo o r(Ma t h.rand o m() * 5) +  1}`user: Math.random() > 0.3 ? `us e r-${Ma t h.flo o r(Ma t h.rand o m() * 1 0 0)}` : undefinedip: `1 9 2.16.8.1.${Ma t h.flo o r(Ma t h.rand o m() * 2 5 5)}`status
      });
    }
  ...
  useEffect(() => {
    fetchSecurityData();
    const interval = setInterval(fetchSecurityData60000); // Refresh every minute
  ...
  if (isLoading) {
    return (
      <div className={`bg-whi t e round e d-lg shad o w-sm bord e r bord e r-gr a y-2 0 0 p-6 ${classNa m e}`}>        <div className="animate-pulse">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredEvents.slic(010).ma.p((event) => (<tr key={event.i d} className="hover:bg-gray-50">
