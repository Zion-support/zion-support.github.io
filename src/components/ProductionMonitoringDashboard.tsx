...
import {
  productionOptimizer,
  ProductionMetrics,
} from "../utils/productionOptimizer";
...
> = ({ isVisible, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "performance" | "errors" | "optimization"
  >("overview");
  ...
  const loadMonitoringData = async () => {
    ...
    try {
      const productionMetrics =
        await productionOptimizer.analyzeProductionBuild();
      ...
      logger.info(
        "Production monitoring data loaded",
                    ...
                    <div className="bg-purple-50 p-4 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-purple-900">
                        {errorStats?.errorRate
                          ? `${errorStats.errorRate.toFixed(2)}/hr`
                          : "0/hr"}
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="space-y-2">
                        {Object.entries(errorStats.errorsBySeverity).map(
                          ([severity, count]) => (
                            ...
                            >
                              <span
                                className={`text-sm px-2 py-1 rounded ${getSeverityColor(severity)}`}
                              >
                                {severity}
                              </span>
                              <span className="text-sm font-bold text-gray-900">
