...
const handleViewListing = () => {// Debug logging for development;
    ...
    if (process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating "to":', {"path": `${detailBasePath}/${listing.i,`}`;`      })logDebug('[ProductCard] Listing "ID":', { "id": listing.id,'
...
}log_debug ('[ProductCard] Navigating "to":', {"path": `${detailBasePath}/${listing.id},`})log_debug ('[ProductCard] Listing "ID":', { "id": listing.id,'
...
if ( {) {$2;
    ...
    router.push (`${detailBasePath}/${listing.id}`)}`
const dispatch = use_dispatch < AppDispatch>()const addToCart = () =>: any {set_loading (true)dispatch (add_item ({"id": listing.id,"title": listing.title,"price": listing.price ?? 0}))toast.success (`1× ${listing.titl,`} added`, {"action": {"label": 'View Cart',"on_click": () => router.push ('/cart')}})set_loading (false,'
}
;
...
const ProductListingCardComponent = ({listing,view = 'grid',onRequestQuote,detailBasePath = '/marketplace/listing';'
  ...
  const handleImageError = () => {if (!imageError) { // Prevent infinite loops if placeholder also fails;
    ...
    if (process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating "to":', { "path": `${detailBasePath}/${listing.id}`,`}),logDebug('[ProductCard] Listing "ID":', { "id": listing.id,'
}),logDebug('[ProductCard] Listing "Title":', { "title": listing.title },'
}// Validate listing ID exists before navigation;
...
const handleViewListing = () => {// Debug logging for development;
    ...
    if (process && process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating "to":', {"path": `${detailBasePath}/${listing && listing.id},`})logDebug('[ProductCard] Listing "ID":', { "id": listing && listing.id,'
...
const dispatch = useDispatch<AppDispatch>()const addToCart = () => {setLoading(true)dispatch(addItem({"id": listing && listing.id,"title": listing && listing.title,"price": listing && listing.price ?? 0}))toast && toast.success(`1× ${listing && listing.titl,`} added`, {"action": {"label": 'View Cart',"onClick": () => router && router.push('/cart'),'
})setLoading(false;
}
