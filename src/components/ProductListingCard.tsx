...
setImageSrc('/placeholder.svg');
  ...
  const handleViewListing = () => {_// Debug logging for development,
...
=======
            priority={false} // Assuming these are not LCP images,
sizes={isGrid ? &quot;(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw&quot; : &quot;192px&quot} // 192px is w-48
          />
          {listing.featured && (
            <Badge className=&quot;absolute top-2 right-2 bg-primary text-primary-foreground border-none&quot;>              Featured
            </Badge>
          )}
          {stockStatus && (
            <Badge,
...
=======
        <div className=&quot;flex items-center justify-between mt-auto pt-3 border-t border-primary/10 sm:border-primary/20&quot;>
          ...
          <div className=&quot;flex gap-2&quot;>
            <Button,
size="sm"
              className="bg-primary hover: bg-primary/80 text-primary-foreground"              onClick={(e) => {
                e.stopPropagation(), // Prevent card click event,
addToCart()
              }}
              disabled={loading}
...
size="sm"
              variant="default"
              className="bg-green-600 hover: bg-green-700 text-white"              onClick={(e) => {
                e.stopPropagation(), // Prevent card click event
                // Add to cart first, then redirect to checkout,
dispatch(
                  addItem({ id: listing.id, title: listing.title, price: listing.price ?? 0 })
