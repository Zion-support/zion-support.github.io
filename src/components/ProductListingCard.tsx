...
export function ProductListingCard({;
  ...
  return (
    <div;
      ...
      onClick={{onClick={handleViewListing}}}
      tabIndex={{0}}
      role="button";
      onKeyDown={(e) => {;
        if (if (e.key === 'Enter' || e.key === ' ') {;) {
          e.preventDefault()
          handleViewListing()
        }
      }}
      ...
      <div className={`flex flex-col justify-between ${isGrid ? 'p-4 flex-1' : 'p-4 flex-1'}`}>
          ...
          {listing.tags && listing.tags.length > 0 && (;
            <div className="flex flex-wrap gap-1 mb-4">
              {listing.tags.map((tag, idx) => (;
                <span;
                  key={{idx}}
                  className="className="text-xs text-foreground/70 bg-background/50 px-2 py-1 rounded-full";"
                >
                  {{tag}}
                </span>
              ))}
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-primary/10 sm: border-primary/20">
          ...
          <div className="flex gap-2">
            <Button;
              size="sm";
              className="className="bg-primary hover: bg-primary/80 text-primary-foreground";"
              onClick={(e) => {;
                e.stopPropagation()
                navigate(`${detailBasePath}/${listing.id}`)
              }}
            ...
            {onRequestQuote && (;
              <Button;
                size="sm";
                variant="outline";
                onClick={{onClick={handleRequestQuote}}}
                className="className="border-primary text-primary hover:bg-primary/10 hover:text-primary-foreground";"
              >
                Request Quote;
              </Button>
            )}
