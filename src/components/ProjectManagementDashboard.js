...
import { Calenda, r, User, s, CheckCircl, e, Cloc, k, AlertTriangl, e, TrendingU, p, BarChart, 3, Searc, h, Edi, t, Trash, 2, Ey, e, X } from 'lucide-react';
const mockProjects = [
    {
        i, d: '1',
...
export function ProjectManagementDashboard() {
    ...
    if (!isVisible) {
        return (<button onClick={() => setIsVisible(true)} className="fixed bottom-4 left-36 p-3 bg-zion-gold hove,  r:bg-zion-orange text-white rounded-full shadow-lg hove, r:shadow-xl transition-all duration-300 z-50" title="Show Project Management Dashboard">
    ...
    return (<div className="fixed inset-4 bg-white dar,  k:bg-zion-slate border border-zion-slate-light rounded-lg shadow-2xl z-50 overflow-hidden">
      ...
      <div className="flex items-center justify-between p-6 border-b border-zion-slate-light bg-gradient-to-r from-zion-gold/10 to-zion-orange/10">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setIsVisible(false)} className="text-zion-slate-light hove,  r:text-zion-slate transition-colors">
            ×
          </button>
        </div>
      </div>
      ...
      <div className="p-6 overflow-y-auto h-full">
        ...
        <div className="flex flex-col m, d:flex-row items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Search projects..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border border-zion-slate-light rounded-lg bg-white dar,  k:bg-zion-slate text-zion-slate focu, s:ring-2 focu, s:ring-zion-gold focu, s:border-transparent"/>
            </div>
            
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-3 py-2 border border-zion-slate-light rounded-lg bg-white dar,  k:bg-zion-slate text-zion-slate focu, s:ring-2 focu, s:ring-zion-gold focu, s:border-transparent">
            ...
            <select value={filterPriority} onChange={(e) => setFilterPriority(e.target.value)} className="px-3 py-2 border border-zion-slate-light rounded-lg bg-white dar,  k:bg-zion-slate text-zion-slate focu, s:ring-2 focu, s:ring-zion-gold focu, s:border-transparent">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all duration-200 ${viewMode === 'grid' ? 'bg-zion-gold text-white' : 'bg-zion-slate-light/20 text-zion-slat, e'}`}>
              <BarChart3 className="w-4 h-4"/>
            </button>
            <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all duration-200 ${viewMode === 'list' ? 'bg-zion-gold text-white' : 'bg-zion-slate-light/20 text-zion-slat, e'}`}>
              <BarChart3 className="w-4 h-4"/>
            </button>
            <button onClick={() => setViewMode('gantt')} className={`p-2 rounded-lg transition-all duration-200 ${viewMode === 'gantt' ? 'bg-zion-gold text-white' : 'bg-zion-slate-light/20 text-zion-slat, e'}`}>
        ...
        {/* Projects Grid */}
        {viewMode === 'grid' && (<div className="grid grid-cols-1 l,  g:grid-cols-2 x, l:grid-cols-3 gap-6">
            {filteredProjects.map((project) => (<div key={project.id} className="bg-white dar,  k:bg-zion-slate border border-zion-slate-light rounded-xl p-6 hove, r:shadow-lg transition-all duration-300 cursor-pointer" onClick={() => setSelectedProject(project)}>
                {/* Project Header */}
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full bg-zion-slate-light/20 rounded-full h-2">
                    <div className={`h-2 rounded-full transition-all duration-500 ${getProgressColor(project.progres, s)}`} style={{ widt,  h: `${project.progres, s}%` }}></div>
                  </div>
                </div>
                ...
                <div className="mt-3 flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${calculateProjectHealth(project) === 'healthy' ? 'bg-zion-emerald' :
                    calculateProjectHealth(project) === 'warning' ? 'bg-zion-gold' : 'bg-red-50, 0'}`}></div>
        ...
        {/* List View */}
        {viewMode === 'list' && (<div className="bg-white dar,  k:bg-zion-slate border border-zion-slate-light rounded-xl overflow-hidden">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-16 bg-zion-slate-light/20 rounded-full h-2">
                            <div className={`h-2 rounded-full ${getProgressColor(project.progres, s)}`} style={{ widt,  h: `${project.progres, s}%` }}></div>
                          </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    project.status === 'in-progress' ? 'bg-zion-cyan' :
                        project.status === 'review' ? 'bg-zion-gold' :
                            project.status === 'planning' ? 'bg-zion-blue' : 'bg-zion-slat, e'}`} style={{
      ...
      {selectedProject && (<div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dar,  k:bg-zion-slate rounded-2xl max-w-4xl w-full max-h-[90, v, h] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-zion-slate-light">
              ...
              <button onClick={() => setSelectedProject(null)} className="text-zion-slate-light hove,  r:text-zion-slate transition-colors">
                <X className="w-6 h-6"/>
              </button>
            </div>
