...
const [showNotes,
setShowNotes] = useState(false)const [notes,
setNotes] = useState(application.notes || "")const [showHireModal,
setShowHireModal] = useState(false)const [avatarError,
setAvatarError] = useState(false)// Check if application is stalled (no activity for 7 days)const isStalled  = null;
return (<>;
      <Draggable draggableId={application.id
...
Calendar,
AlertTriangle,
BriefcaseIcon} from 'lucide-react';// Local stub is used in place of the @hello-pangea/dnd package which isn't;
// available in this environment.;
import {DropdownMenu,
DropdownMenuContent,
DropdownMenuItem,
...
application: JobApplication;
  index: number;
export function CandidateCard(): any ({ application,
index
}: CandidateCardProps) {const [showNotes, setShowNotes] = useState(false)const [notes, setNotes] = useState(application && application.notes || '')const [showHireModal, setShowHireModal] = useState(false)const [avatarError, setAvatarError]  = useState(false)// Check if application is stalled (no activity for 7 days)const isStalled =;
...
const [show_notes,
setShowNotes] = useState (false)const [notes,
set_notes] = useState (application.notes || '')const [showHireModal,
setShowHireModal] = useState (false)const [avatar_error,
setAvatarError] = useState (false)// Check if application is stalled (no activity for 7 days)const is_stalled =;
...
}//Match h - 8 w - 8 className="rounded - full object - cover" //Ensure rounded and object - cover on_error= {() => setAvatarError (true)}
priority= {false ";
...
}//Match h-8 w-8 className="rounded-full object-cover" //Ensure rounded and object-cover onError= {() => setAvatarError (true)}
priority= {false ";
}//Avatars are usually not LCP />) : (<User className="h-4 w-4" />) ";
...
}> Message </Link> </DropdownMenuItem> {application.resume?.file url && (<DropdownMenuItem asChild> <a href= {application.resume.file url ";}
target="blank" rel="noopener noreferrer"> View Resume </Link> </DropdownMenuItem>)}</DropdownMenuContent> </DropdownMenu> </div> {/* Application Info */ ";
