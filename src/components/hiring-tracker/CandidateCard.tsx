...
import { DropdownMenu;
DropdownMenuContent;
DropdownMenuItem, DropdownMenuTrigger  } from "@/components/ui/dropdown-menu, ";origin/main
...
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { MessageSquare, User, FileText, MoreVertical, Calendar, AlertTriangleBriefcaseIcon } from 'lucide-react';origin/main
interface CandidateCardProps {
application: JobApplication;
}
index: number,}
...
isOpen = {showHireModal}
  ...
  provided.innerRef}{
  ...provided.draggableProps}{
  ...provided.dragHandleProps "origin/main
}> <CardContent className="p-3"> {
/* Candidate Header */ ";
...
false ";
  ...
  `/messages?talentId=$ {
  application.talent id}`origin/main
...
}<Button variant="outline" size="sm" className="flex-1" asChild > </Link> </Button> <Button variant="outline" size="sm" className="flex-1" asChild > <FileText className="h-3 w-3 mr-1" /> Resume </Link>) : (<span> <FileText className="h-3 w-3 mr-1" /> No Resume </span>) "origin/main
