...
import { motion  } from 'framer-motion;import { enhancedServices2025  } from ../data/enhanced-services-2025const EnhancedServicesShowcase = () => {};'
  const [selectedCategory, setSelectedCategory] = useState(all')const [searchTerm, setSearchTerm] = useState(')const categories = ['
    ...
    <div className='min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white'>'''{/* Hero Section */}''''
      <section className='py-20 px-4 sm:px-6 lg:px-8'>''''
        <div className='max-w-7xl mx-auto text-center'>'''
          ...
          <motion.p''''
            className='text-xl text-gray-300 mb-8 max-w-4xl mx-auto''initial={{ opacity: 0, y: 20 }}'
            whileInView={{ opacity: 1, y: 0 }}
          ...
          {/* Contact Information */}''
          <motion.div'''className=bg-white/10 backdrop-blur-lg rounded-xl p-6 mb-8 max-w-2xl mx-auto''initial={{ opacity: 0, y: 20 }}'
            whileInView={{ opacity: 1, y: 0 }}
          ...
          {/* Search and Filter */}'''
          <motion.div'''className=flex flex-col md:flex-row gap-4 justify-center items-center mb-12''initial={{ opacity: 0, y: 20 }}'
            whileInView={{ opacity: 1, y: 0 }}
...
selectedCategory === category.id? bg-blue-600 border-blue-400 text-white'''`;'
      ...
      <section className='py-20 px-4 sm:px-6 lg:px-8'>''''
        <div className='max-w-7xl mx-auto'>'''
          ...
          >{filteredServices.map((service, index) => (
              <motion.div;
                key={service.id}'variants={itemVariants}''className=bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 hover:border-white/40 transition-all duration-300 hover:transform hover:scale-105 group '>'''{/* Service Header */}''''
...
'''{/* CTA */}''''
                <div className='text-center'>
                  <Link'''
                    to={service.link}'''className=inline-flex items-center text-blue-400 hover:text-blue-300 transition-colors duration-300 text-sm '>
                    Learn More'''
          ...
          {filteredServices.length === 0 && ('''
            <motion.div'''className=text-center py-12''initial={{ opacity: 0 }}'
              animate={{ opacity: 1 }}''>''''
              <p className='text-gray-400 text-lg'>
                No services found matching your criteria.</p>
              <button;
                onClick={() => {}
setSelectedCategory(all')setSearchTerm(')}}'''className=mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-300 '>
                View All Services
              </button>
            </motion.div>
          )}
...
'''{/* Enhanced CTA Section */}''''
      <section className='py-20 bg-gradient-to-r from-blue-900/50 to-cyan-900/50'>''''
        <div className='max-w-4xl mx-auto text-center'>'''
          <motion.h2''''
            className='text-3xl md:text-4xl font-bold text-white mb-6''initial={{ opacity: 0, y: 20 }}'
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
          >
...
'''
          ...
          >''
            <Link''''
              to='/contact'''className=inline-flex items-center px-8 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-cyan-700 transition-all duration-300 '>'
              Contact Us Today
            </Link>'''
            <a''''
              href='tel:+13024640950'''className=inline-flex items-center px-8 py-3 border border-gray-600 text-white font-semibold rounded-lg hover:bg-gray-800 transition-all duration-300 '>'
              Call: +1 302 464 0950
            </a>
          </motion.div>
'''
...
'''
          <motion.div''''
            className='mt-8 text-sm text-gray-400''initial={{ opacity: 0 }}'
          ...
          >
            ...
            <p>;
              Website:{' '}'''
              <a''''
                href='https://ziontechgroup.com'''className=text-blue-400 hover:text-blue-300 '>'
