...
'''}''''
    return (<div className='min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white'>'''{/* Hero Section */}''''
  ...
  y: 0}} transition={{ duration: 0.6 }}>''';
            Comprehensive''''
            <span className='text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400'>''''{' '}Technology Solutions
            </span>'''
          </motion.h1>''''
...
'''}}>''''
            ...
            <div className='flex flex-wrap gap-2'>'{categories.map((category) => (<button key={category.id} onClick={() => setSelectedCategory(category.id)} className={`px-4 py-2 rounded-lg border transition-all duration-300 ${selectedCategory === category.id''`;
...
'''{/* Services Grid */}''''
      <section className='py-20 px-4 sm:px-6 lg:px-8'>''''
        <div className='max-w-7xl mx-auto'>''''
          <motion.div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8' variants={containerVariants} initial='hidden' whileInView='visible' viewport={{ once: true }}>''''{filteredServices.map((service, index) => (<motion.div key={service.id} variants={itemVariants} className='bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 hover:border-white/40 transition-all duration-300 hover:transform hover:scale-105 group'>'''{/* Service Header */}''`
...
'''{/* Key Features */}''''
                <div className='mb-4'>''''
                  ...
                  <ul className='space-y-1'>''''{service.features.slice(0, 4).map((feature, idx) => (<li key={idx} className='text-xs text-gray-300 flex items-center'>''''
...
'''{/* Benefits */}''''
                <div className='mb-4'>''''
                  ...
                  <ul className='space-y-1'>''''{service.benefits.slice(0, 3).map((benefit, idx) => (<li key={idx} className='text-xs text-gray-300 flex items-center'>''''
...
'''{/* No Results Message */}''''{filteredServices.length === 0 && (<motion.div className='text-center py-12' initial={{ opacity: 0 }} animate={{ opacity: 1 }}>''''
