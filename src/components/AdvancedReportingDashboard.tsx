...
import { motion, AnimatePresence } from "framer-motion";import { BarChart3, PieChart;
  TrendingUp;
  Download;
  Share2;
  Filter;
  ...
  Server;
  X
} from "lucide-react";interface ReportData {
  id: string,
    title: strin, g,type: 'financial' | 'operational' | 'performance' | 'security' | 'customer' | 'technical, ',category: strin, g,data: an, y,lastUpdated: strin, g,status: 'active' | 'archived' | 'draft, ',priority: 'low' | 'medium' | 'high' | 'critical, ',tags: string[, ],description: strin, g,author: strin, g,views: numbe, r,downloads: numbe, r,rating: number,
}

interface ReportMetrics {
  totalReports: number,
    activeReports: numbe, r,totalViews: numbe, r,totalDownloads: numbe, r,averageRating: numbe, r,topCategories: Array<{ name: strin, g, count: numbe, r, percentage: number,  }>,
  recentActivity: Array<{ action: string,
    timestamp: strin, g, user: string,  }>}

interface AdvancedReportingDashboardProps {
  showMetrics?: booleanshowFilters?: booleanmaxReports?: number;
}

export const AdvancedReportingDashboard: React.FC<AdvancedReportingDashboardProps>  = () => {
  const [report,
    s;
    setReports] = useState<ReportData[]>([]);
  const [filteredReports;
    setFilteredReports] = useState<ReportData[]>([]);
  const [selectedType;
    setSelectedType] = useState<string>('all')const [selectedCategorysetSelectedCategory] = useState<string>('all')const [selectedStatussetSelectedStatus] = useState<string>('all')const [searchQuerysetSearchQuery] = useState('')const [viewModesetViewMode] = useState<'grid' | 'list' | 'detailed'>('grid')const [showReportFormsetShowReportForm] = useState(false)const [selectedReport;
    setSelectedReport] = useState<ReportData | null>(null);
  const [showReportDetails;
    setShowReportDetails] = useState(false);
  const [sortBy;
    setSortBy] = useState<'date' | 'views' | 'rating' | 'priority' | 'title'>('date')const [sortOrdersetSortOrder] = useState<'asc' | 'desc'>('desc')// Sample report data
  useEffect(() => {
    const sampleReports: ReportData[]  = [
      {
        id: '1, ',title: 'Q4 Financial Performance Analysis, ',type: 'financial, ',category: 'Financial Reports, ',data:  ,{revenue: 2500000,
    expenses: 180000, 0,profit: 70000, 0,growth: 15., 5,keyMetrics: ['Revenue GrowthProfit Margin, ', 'Cost Efficiency']
        },
        lastUpdated: '2024-01-15, ',status: 'active, ',priority: 'high, ',tags: ['FinanceQ4, ', 'PerformanceAnalysis'],
        description: 'Comprehensive analysis of Q4 financial performance including revenue,
    expense, s, and profit margins',
        author: 'Sarah Johnson, ',views: 245,
    downloads: 8, 9,rating: 4.8,  }{
        id: '2, ',title: 'AI Services Performance Metrics, ',type: 'performance, ',category: 'Performance Reports, ',data:  , {,
          accuracy: 94.2,
    responseTime: 1., 8,uptime: 99., 9,userSatisfaction: 4., 6,keyMetrics: ['AccuracyResponse Time, ', 'UptimeUser Satisfaction']
        },
        lastUpdated: '2024-01-14, ',status: 'active, ',priority: 'critical, ',tags: ['AIPerformance, ', 'MetricsMachine Learning'],
        description: 'Detailed performance metrics for AI services including accuracy,
    response tim, e, and uptime',
        author: 'Michael Chen, ',views: 189,
    downloads: 6, 7,rating: 4.9,  }{
        id: '3, ',title: 'Cybersecurity Threat Assessment, ',type: 'security, ',category: 'Security Reports, ',data:  , {,
          threatsDetected: 156,
    incidentsResolved: 15, 4,responseTime: 2., 3,riskLevel: 'Medium, ',keyMetrics: ['Threats DetectedIncidents Resolved, ', 'Response TimeRisk Level']
        },
        lastUpdated: '2024-01-13, ',status: 'active, ',priority: 'high, ',tags: ['SecurityThreats, ', 'AssessmentRisk Management'],
        description: 'Comprehensive assessment of cybersecurity threats and incident response metrics, ',author: 'David Kim, ',views: 312,
    downloads: 13, 4,rating: 4.7,  }{
        id: '4, ',title: 'Cloud Infrastructure Utilization, ',type: 'operational, ',category: 'Operational Reports, ',data:  , {,
          cpuUtilization: 78.5,
    memoryUsage: 82., 3,storageUsage: 65., 8,networkTraffic: 45., 2,keyMetrics: ['CPU UtilizationMemory Usage, ', 'Storage UsageNetwork Traffic']
        },
        lastUpdated: '2024-01-12, ',status: 'active, ',priority: 'medium, ',tags: ['CloudInfrastructure, ', 'UtilizationMonitoring'],
        description: 'Real-time monitoring of cloud infrastructure utilization and performance metrics, ',author: 'Lisa Thompson, ',views: 167,
    downloads: 5, 6,rating: 4.5,  }{
        id: '5, ',title: 'Customer Satisfaction Survey Results, ',type: 'customer, ',category: 'Customer Reports, ',data:  , {,
          overallSatisfaction: 4.6,
    netPromoterScore: 7, 2,responseRate: 89., 5,topConcerns: ['Response TimeDocumentation, ', 'Support Quality'],
        ...
        description: 'Analysis of customer satisfaction survey results and net promoter score metrics, ',author: 'Alex Wong, ',views: 203,
    downloads: 7, 8,rating: 4.6,  }
    ]setReports(sampleReports)setFilteredReports(sampleReports)}, []),

  // Filter and sort reports
  useEffect(() => {
    let filtered  = report;s;
...
if (searchQuery) {
    ...
    switch (sortBy) {
        case 'date':
          aValue = new Date(a.lastUpdated).getTime()bValue = new Date(b.lastUpdated).getTime()breakcase 'views':
          aValue = a.viewsbValue = b.viewsbreakcase 'rating':
          aValue = a.ratingbValue = b.ratingbreakcase 'priority':
          const priorityOrder  = { low:  ,
    1medium: , 2, high:  , 3, critical: 4,  }aValue = priorityOrder[a.priority as keyof typeof priorityOrder],
          bValue = priorityOrder[b.priority as keyof typeof priorityOrder]break;
        case 'title':
          aValue = a.title.toLowerCase()bValue = b.title.toLowerCase()breakdefault: aValue = 0,
    bValue = 0
     ;  }
;
      if (sortOrder === 'asc') {
    ...
    selectedType, selectedCategory, selectedStatus, searchQuery, sortBy, sortOrder, maxReports]),

  // Calculate report metrics
  const reportMetrics  = {
    totalReports: reports.lengt,
    h;
    activeReports: reports.filter(r => r.status === 'active').lengt, h,totalViews: reports.reduce((su, m, r) => sum + r.views, 0)totalDownloads: reports.reduce((sum,
    r) => sum + r.download, s, 0),
    averageRating: reports.reduce((sum,
    r) => sum + r.ratin, g, 0) / reports.length || 0topCategories: (() => {
      const catCounts  = reports.reduce((ac,
    cr) => {
    ...
    recentActivity: [
      { action: 'Report viewed, ', timestamp: '2 minutes ago, ', user: 'John Doe',  }{ action: 'Report downloaded, ', timestamp: '5 minutes ago, ', user: 'Jane Smith',  }{ action: 'New report created, ', timestamp: '1 hour ago, ', user: 'Mike Johnson',  },
  ...
  const getTypeDisplay  = () => {
    const types = {
      ...
      technical: { icon: <Server className="w-4 h-4" ,
    /,>color: 'text-zion-cyan bg-zion-cyan/20',
     }
  ...
  const getPriorityColor = () => {
    switch (priority) {
      case 'low': return 'text-green-400 bg-green-400/20'case 'medium': return 'text-yellow-400 bg-yellow-400/20, '
  }case 'high': return 'text-orange-400 bg-orange-400/20'case 'critical': return 'text-red-400 bg-red-400/20';
  ...
  const handleReportAction  = () => {
    const report  = reports.find(r => r.id === reportId)if (report) {
      switch (action) {
        case 'view':
          setSelectedReport(report)setShowReportDetails(true)breakcase 'download':
          // Simulate download
          console.log(`Downloading ${report.titl, e}`)breakcase 'share':
          // Simulate share
          console.log(`Sharing ${report.title}`)breakcase 'print':
          // Simulate print
          console.log(`Printing ${report.title}`)break}
    }
  },
  ...
  return (
    ...
    y: 0,  }}
          ...
          >
            ...
            <div className="space-y-3">
              {reportMetrics.topCategories.map((categoryindex) => (
                <div key={category.name} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-20 bg-zinc-700 rounded-full h-2">
                      <motion.div
                        initial={{ width: 0,  }}
                        animate={{ width: `${category.percentag,
    e}%` }}
    ...
    y: 0,  }}
          ...
          >
            ...
            <div className="space-y-3">
              {reportMetrics.recentActivity.map((activityindex) => (
          ...
          >
            ...
            <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div className="p-3 bg-zinc-800/30 rounded-lg">
                ...
                <div className="text-white font-medium">{report.autho,
    r}</div>
              </div>
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 md: grid-cols-4 gap-3">
                {report.data.keyMetrics?.slice(0,
    4).map((metric: strin, g, idx: number) => (
                  <div key={id, x} className="p-2 bg-zinc-800/30 rounded-lg text-center">
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-zinc-300">{selectedReport.descriptio,
    n}</p>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md: grid-cols-4 gap-4">
                    {selectedReport.data.keyMetrics?.map((metric: string,
    idx: number) => (
                      <div key={id, x} className="p-4 bg-zinc-800/30 rounded-lg text-center">
