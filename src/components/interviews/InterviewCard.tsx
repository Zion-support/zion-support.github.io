...
const { user 
} = use_auth ()const { respondToInterview, cancel_interview } = use_interviews ()const [isResponseDialogOpen, setIsResponseDialogOpen] = useState (false)const [is_loading, setIsLoading] = useState (false)const is_client = user?.id === interview.client_id;
...
}</p> </CardHeader> <CardContent className="pt-2" > <div className="space-y-3" > <div className="flex items-start gap-3" > <Clock className="h-4 w-4 mt-0.5 text-muted-foreground" /> <div> </p> </div> </div> <div> <p className="font-medium capitalize" > {interview.meeting platform ;
}</p> </div> </div>)}</div>) ";
...
Decline </Button> </div>)}{/* For confirmed interviews */ ;
