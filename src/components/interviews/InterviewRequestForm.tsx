...
notes: z.string ().optional ()})export /**;
 * InterviewRequestForm - Function description;
 */;
function InterviewRequestForm() {
const { request_interview 
} = use_interviews ()const [is_submitting, setIsSubmitting] = useState (false)const form = use_form < z.infer < typeof form_schema>>({resolver: zod_resolver (form_schema),
default_values: {title: `Interview with ${talent.full_name}`,
duration: '30',
platform: 'zoom',
notes: '',
meeting_link: ''}})async /**;
...
variant: 'destructive'})return;
    ...
    setIsSubmitting (true)try {// Combine date and time;
      const dateTimeString = `${format (values.date, 'yyyy - MM - dd')}
T${values.time}:00`;
      const scheduled_date = new Date (dateTimeString)// Calculate end time based on duration;
      const duration_minutes = parse_int (values.duration)await request_interview ({import React, { useState } from "react",
import { Button } from "@/components/ui/button",
...
const form = useForm < z.infer < typeof formSchema>>({resolver: zodResolver(formSchema)defaultValues: {title: `Interview with ${talent.full_name}`,
...
meeting_link: values.meetingLink} finally {setIsSubmitting (false)}
  }
const formSchema = z && z.object({date: z;
    .date({required_error: 'Please select a date for the interview.'}).refine(date => date > new Date(), {message: 'Interview date must be in the future'}),
...
notes: z && z.string().optional()})export function InterviewRequestForm(): any ({talent,
onClose,
userDetails}: InterviewRequestFormProps) {const { requestInterview } = useInterviews()const [isSubmitting, setIsSubmitting]  = useState(false)const form = useForm < z && z.infer < typeof formSchema>>({resolver: zodResolver(formSchema),
defaultValues: {title: `Interview with ${talent && talent.full_name}`,
duration: '30',
platform: 'zoom',
notes: '',
...
setIsSubmitting(true)try {// Combine date and time;
      const dateTimeString = `${format(values && values.date, 'yyyy-MM-dd')}
T${values && values.time}:00`;
      const scheduledDate  = new Date(dateTimeString)// Calculate end time based on duration;
      const durationMinutes  = parseInt(values && values.duration)await requestInterview({talent_id: talent && talent.id,
...
defaultValues: {title: `Interview with ${talent.full_name}`,
duration: "30",
platform: "zoom",
notes: "",
meetingLink: ""}}),
async function onSubmit() {
if (!userDetails?.id) {toast({title: "Authentication required",
description: "Please log in to schedule an interview",
variant: "destructive"
}),
return;
