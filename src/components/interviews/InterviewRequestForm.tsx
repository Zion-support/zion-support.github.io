 ...
 */;
function InterviewRequestForm() {const { request_interview } = use_interviews ()const [is_submitting, setIsSubmitting] = useState (false)const form = use_form < z.infer < typeof form_schema>>({"resolver": zod_resolver (form_schema),"default_values": {"title": `Interview with ${talent.full_nam,`}`,"duration": '30',"platform": 'zoom',"notes": '',"meeting_link": '','
})async /**;
...
const formSchema = z.object({"date": z.date({"required_error": "Please select a date for the interview.,"
  ...
  }),"time": z.string().min(1, "Please select a time for the interview."),"duration": z.string().min(1, "Please select the interview duration."),"platform": z.string().min(1, "Please select a meeting platform."),"meetingLink": z.string().optional(),const formSchema = z && z.object({"date": z;
    .date({"required_error": 'Please select a date for the interview.,'
}).refine(date = > date > new Date(), {"message": 'Interview date must be in the future,'
  ...
  useState(false;
  const form = useForm<z && z.infer<typeof formSchema>>({"resolver": zodResolver(formSchema),"defaultValues": {"title": `Interview with ${talent && talent.full_nam,`}`,"duration": '30',"platform": 'zoom',"notes": '',"meetingLink": '','
...
})toast({"title": 'Interview requested',"description": `Your interview request with ${talent && talent.full_name} has been sent.`})onClose(,`} catch (error) {logErrorToProduction('Failed to schedule "interview":', { "data": error,'
...
}),export function InterviewRequestForm() {const { requestInterview } = useInterviews(),const [isSubmitting, setIsSubmitting] = useState(false),const form = useForm<z.infer<typeof formSchema>>({"resolver": zodResolver(formSchema),"defaultValues": {"title": `Interview with ${talent.full_nam,`}`,"duration": "30","platform": "zoom","notes": "","meetingLink": "","
...
const durationMinutes = parseInt(values.duration),await requestInterview({"talent_id": talent.id,"client_id": userDetails.id,"scheduled_date": scheduledDate.toISOString(),"duration_minutes": durationMinutes,"notes": values.notes,"meeting_platform": values.platform as any,"meeting_link": values.meetingLink,"interview_type": "video","title": values.title;"
      }),toast({"title": "Interview requested","description": `Your interview request with ${talent.full_name} has been sent.,`}),onClose()} catch (error) {logErrorToProduction('Failed to schedule "interview":', { "data": error,'
