...
export function HowItWorksSection() {
  ...
  const steps = [
    ...
    },
    {
      ...
      duration: "4-8 weeks",
      success: "98% quality",
      details: ["UI/UX design", "Backend development", "API integration", "Testing & QA"]
    },
    {
      title: "Testing & Deployment",
      description: "Rigorous testing ensures flawless performance before launch",
      icon: <Users className="w-12 h-12" />,
      color: "from-zion-blue to-zion-purple",
      duration: "1-2 weeks",
      success: "99.9% uptime",
      details: ["Performance testing", "Security audit", "Deployment", "Monitoring setup"]
    },
    {
      ...
      details: ["Go-live support", "Training", "Maintenance", "Updates"]
    }
  ];

  const stats = [
    { icon: <Zap className="w-6 h-6" />, value: "95%", label: "Success Rate" },
    { icon: <Clock className="w-6 h-6" />, value: "6-12", label: "Weeks Avg" },
    { icon: <Shield className="w-6 h-6" />, value: "99.9%", label: "Uptime" },
    { icon: <Users className="w-6 h-6" />, value: "24/7", label: "Support" }
  ];
...
<<<<<<< HEAD
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
            {steps.map((step, index) => (
              <motion.div key={index} variants={itemVariants} className="relative" onHoverStart={() => setHoveredStep(index)} onHoverEnd={() => setHoveredStep(null)} whileHover={{ y: -8 }} transition={{ type: "spring", stiffness: 300 }}>
=======
