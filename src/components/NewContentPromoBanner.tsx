    ...
    <div className={`${variants[variant]} ${className} relative overflow-hidden`}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-6">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              {featuredContent.map((item, index) => (
                <Link key={index} to={item.link} className="bg-white/10 backdrop-blur-sm rounded-lg p-3 hover:bg-white/20 transition-all duration-300 hover:scale-105 group">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-4 h-4 text-white/80">{item.icon}</span>
          ...
          {/* Dismiss button */}
          <button onClick={() => setIsDismissed(true)} className="ml-4 p-2 hover:bg-white/10 rounded-full transition-colors">
