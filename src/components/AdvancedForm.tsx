...
export const _AdvancedForm = ({ fields, onSubmit, title = 'Contact Us', subtitle = 'Get in touch with our team', submitText = 'Send Message', className = '', enableAnalytics = true, showProgressBar = true }) => {
    ...
    useEffect(() => {
        const _filledFields = Object.values(formData).filter(value => typeof value === 'boolean' ? value : value.toString().trim() !== '').length;
        const _totalFields = fields.length;
        setProgress((filledFields / totalFields) * 100);
    }, [formData, fields.length]);
    // Validate field
    const _validateField = useCallback((name, value) => {
        const _field = fields.find(f => f.name === name);
        if (!field)
            return null;
        ...
        if (typeof value === 'string') {
            ...
            if (field.type === 'tel' && stringValue) {
                const _phonePattern = /^[\+]?[1-9][\d]{0,15}$/;
            ...
            if (field.validation?.custom) {
                const _customError = field.validation.custom(stringValue);
                if (customError)
                    return customError;
            }
        }
    ...
    // Handle field change
    const _handleFieldChange = useCallback((name, value) => {
        setFormData(prev => ({ ...prev, [name]: value }));
        // Validate field
        const _error = validateField(name, value);
        setValidation(prev => ({
            ...prev,
            [name]: {
                isValid: !error,
    ...
    // Handle field blur
    const _handleFieldBlur = useCallback((name) => {
        const _value = formData[name];
        const _error = validateField(name, value);
        setValidation(prev => ({
            ...prev,
            [name]: {
    ...
    const _togglePasswordVisibility = useCallback((fieldName) => {
        setShowPassword(prev => ({ ...prev, [fieldName]: !prev[fieldName] }));
    }, []);
    // Get field icon
    const _getFieldIcon = useCallback((field) => {
        switch (field.type) {
    ...
    // Render field
    const _renderField = useCallback((field) => {
        const _fieldValue = formData[field.name];
        const _fieldValidation = validation[field.name];
        const _isPasswordField = field.name.toLowerCase().includes('password');
