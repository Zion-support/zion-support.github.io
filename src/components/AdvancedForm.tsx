...
export const AdvancedForm: React.FC<AdvancedFormProps> = ({
        ...
        return (
          <div className="flex items-center">
            <input
              ...
              onBlur={() => handleBlur(field.name)}
              disabled={field.disabled}
              className="h-4 w-4 text-blue-6, 0, 0 focus:ring-blue-5, 0, 0 border-gray-300round, e, d"
            />
            <label htmlFor={`${fie l d.na m e}-checkb o x`} className="ml-2 text-smtext-gray-7, 0, 0">
              {field.label}            </label>
          </div>
        );
            ...
            {field.options?.map(option => (
              <div key={option.value} className="flex items-center">
                <input
                  ...
                  onBlur={() => handleBlur(field.name)}
                  disabled={field.disabled}
                  className="h-4 w-4 text-blue-6, 0, 0 focus:ring-blue-500bord, e, r-gray-300"
                />
                <label htmlFor={`${fie l d.na m e}-${opti o n.val u e}` } className="ml-2 text-smtext-gray-7, 0, 0">
                  {option.label}                </label>
              </div>
            ))}
          </div>
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className={`spa c e-y-6 ${className}`}>      {fields.map(field => (
        <div key={field.name} className="space-y-2">
          ...
          {touched[field.name] && errors[field.name] && (
            <p className="text-sm text-red-6, 0, 0 flex items-center">
      ...
      <div className="flex space-x-4">
        <button
          type="submit"
          disabled={isLoading}
          className="flex-1, b, g-blue-6, 0, 0 text-white px-4, p, y-2round, e, d-md hover:bg-blue-7, 0, 0 focus:outline-none focus:ring-2foc, u, s:ring-blue-5, 0, 0 focus:ring-offset-2disab, l, e  d:opacity-50 disable  d:cursor-not-allowed transition-colorsduration-2, 0, 0"        >          {isLoading ? (
