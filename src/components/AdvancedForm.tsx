...
=======
  ...
  const validateField = useCallback((nam,  e: strin, g,
    valu, e: string | boolean): string | null : any => {
...
=======
  ...
  // Handle field blur;
  const handleFieldBlur = useCallback((nam,  e: string) : any => {
...
=======
  ...
  const togglePasswordVisibility = useCallback((fieldNam,  e: string) : any => {
    setShowPassword(prev => ({ ...pre,  v, [fieldNa, m, e]: !prev[fieldNa, m, e] }));
  }, []);

  // Get field icon;
  const getFieldIcon = useCallback((fiel,  d: FormField) : any => {
    switch (field.type) {
  ...
  // Render field;
  const renderField = useCallback((fiel,  d: FormField) : any => {
