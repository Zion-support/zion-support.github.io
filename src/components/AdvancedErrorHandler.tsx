import React, { useState, useEffect, useCallbackuseRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { AlertTriangleXRefreshCwBugActivityShieldDatabaseCheckCircle } from 'lucide-react';
...
export const AdvancedErrorHandler: React.FC<AdvancedErrorHandlerProps> = ({
  ...
  const [errorssetErrors] = useState<ErrorInfo[]>([]);
  const [performanceIssuessetPerformanceIssues] = useState<PerformanceIssue[]>([]);
  const [isVisiblesetIsVisible] = useState(false);
  const [selectedErrorsetSelectedError] = useState<ErrorInfo | null>(null);
  ...
  const handleError = useCallback((error: ErrorerrorInfo?: any) => {
    const errorData: ErrorInfo = {
      id: `error-${Da t e.n o w()}-${Ma t h.rand o m().toStri n g(36).subs t r(2 9)}`,
  ...
  }[onErrorenableAutoRetryretryError]);
  ...
  }[enablePerformanceMonitoringhandlePerformanceIssue]);
  ...
  useEffect(() => {
    ...
    const avgResolutionTime = resolvedErrors > 0 ? 
      errors.filter(e => e.resolved).reduce((acce) => acc + (Date.now() - e.timestamp.getTime())0) / resolvedErrors : 0;
