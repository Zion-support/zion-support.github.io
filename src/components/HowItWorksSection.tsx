import React, { useState  from "react", import { motion, AnimatePresence } from "framer-motion";import { Search, Users, FileText, Rocket, CheckCircle, ArrowRight, Clock, Target, TrendingUp, Award, Zap, Shield } from "lucide-react";const steps  = [
  {
    icon: <Search className="w-8 h-8" /, >,title: "Discove,
    r";description: "Browse our comprehensive catalog of services,
    talen, t, and solutions",
    color: "from-zion-cyan to-zion-cyan-dark, ",bgColor: "from-zion-cyan/20 to-zion-cyan-dark/20, ",details: [
 ...
 ,
    ...
    icon: <FileText className="w-8 h-8" /, >,title: "Plan, ";description: "Collaborate on project requirements,
    timelin, e, and deliverables",
 ...
 ,
  ...
  const [hoveredStepsetHoveredStep] = useState<number | null>(null);
  const [activeStep;
  ...
  const statsVariants  = {
    ...
    <section className="py-20 bg-gradient-to-br from-zion-blue via-zion-slate-dark to-zion-blue-dark relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute inset-0" style={{
          backgroundImage: `radial-gradient(circle at 50% 5,
    0%currentColor 1pxtransparent 1px)`,
    ...
    repeat: Infinit, y, ease: "easeInOut",  }}
        />
      </div>
    ...
    y: 0,  }}
        ...
        >
          {stats.map((statindex) => (
            <motion.div
              key={index}variants={statsVariants}
    ...
    index) => (
              <motion.div
                key={index}variants={containerVariants}
                className="relative group"
                onHoverStart={() => setHoveredStep(index)}
                onHoverEnd={() => setHoveredStep(null)}whileHover={{ y: -8,  }}
