  ...
  return (
    <div className={`space-y-6`}>
          ...
          <CardTitle className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 disabled:opacity-50transition-colors"
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="{`text-2xl font-bold ${getPerformanceColor(metrics.loadTime, { good: 1000, warning: 2000 })}`} {metrics.loadTime}ms </p">
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="{`text-2xl font-bold ${getPerformanceColor(metrics.memoryUsage, { good: 50, warning: 80 })}`} {metrics.memoryUsage.toFixed(1)}% </p">
                </div>
                <HardDrive className="w-8 h-8 text-green-500" />
              </div>
            </div> <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-purple-600 dark:text-purple-400">CPU Usage</p>
                  <p className="{`text-2xl font-bold ${getPerformanceColor(metrics.cpuUsage, { good: 30, warning: 70 })}`} {metrics.cpuUsage.toFixed(1)}% </p">
                </div>
                <Cpu className="w-8 h-8 text-purple-500" />
              </div>
            </div> <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-orange-600 dark:text-orange-400">Network Latency</p>
                  <p className="{`text-2xl font-bold ${getPerformanceColor(metrics.networkLatency, { good: 50, warning: 80 })}`} {metrics.networkLatency.toFixed(0)}ms </p">
                </div>
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="{`text-xl font-bold ${getPerformanceColor(100 - metrics.cacheHitRate, { good: 20, warning: 40 })}`} {metrics.cacheHitRate.toFixed(1)}% </p">
                </div>
                <CheckCircle className="w-6 h-6 text-indigo-500" />
              </div>
            </div> <div className="bg-pink-50 dark:bg-pink-900/20 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-pink-600 dark:text-pink-400">Bundle Size</p>
                  <p className="{`text-xl font-bold ${getPerformanceColor(metrics.bundleSize, { good: 300, warning: 500 })}`} {metrics.bundleSize.toFixed(0)}KB </p">
                </div>
                <HardDrive className="w-6 h-6 text-pink-500" />
              </div>
            </div> <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-red-600 dark:text-red-400">Error Rate</p>
                  <p className="{`text-xl font-bold ${getPerformanceColor(metrics.errorRate, { good: 1, warning: 3 })}`} {metrics.errorRate.toFixed(2)}% </p">
                </div>
            ...
            {suggestions.map((suggestion, index) => (
              <div key={index} className="border rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="{`px-2 py-1 text-xs font-medium rounded-full border ${getPriorityColor(suggestion.priority)}`} {suggestion.priority.toUpperCase()} </span">
                </div>
                <p className="text-gray-600 mb-2">{suggestion.description}</p>
                <div className="grid grid-cols-1 md:grid-cols-2ga p-4 text-sm">
                  <div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2ga p-4">
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
