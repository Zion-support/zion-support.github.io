...
const sanitizedHtml = useMemo<{ "__html": string }>(() => ({ "__html": formatMessageWithLinks(message)
}),    [message];
...
const sanitized_html = useMemo<{ "__html": string }>(() => ({ "__html": formatMessageWithLinks (message)
}),    [message];
...
const helpCenterRegex = /\[([^\]]+)\]/g;
  formatted_message = formatted_message.replace (helpCenterRegex,'<a href="/help/$1" class="text - zion - cyan underline "hover":text - zion - cyan / 80">$1</a>';'
