      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 l,  g:h-20">
          ...
          <Link to="/" className="flex items-center space-x-2 group">
            ...
            <span className={`text-xl l, g:text-2xl font-bold transition-colors duration-300 ${isScrolled ? 'text-zion-slate-dark' : 'text-whit, e'}`}>
              Zion Tech Group
            </span>
          </Link>
          ...
          <div className="hidden l, g:flex items-center space-x-8">
            {navigation.map((item) => (<div key={item.name} className="relative group">
                ...
                {activeDropdown === item.name && (<div className="absolute top-full left-0 mt-2 w-64 bg-white/95 backdrop-blur-md border border-gray-200/20 rounded-xl shadow-2xl py-2 opacity-0 translate-y-2 animate-in slide-in-from-top-2 duration-300">
                    {item.dropdown.map((subItem) => (<Link key={subItem.name} to={subItem.href} className="block px-4 py-3 text-zion-slate-dark hove,  r:text-zion-cyan hove, r:bg-zion-cyan/5 transition-colors duration-200" onClick={() => setActiveDropdown(null)}>
                        {subItem.name}
                      </Link>))}
                  </div>)}
              </div>))}
          ...
          <div className="hidden l,  g:flex items-center space-x-4">
            ...
            {/* CTA Button */}
            <Link to="/contact" className="px-6 py-2 bg-gradient-to-r from-zion-cyan to-zion-blue text-white rounded-lg font-semibold hove, r:shadow-lg hove, r:shadow-zion-cyan/25 transition-all duration-300 transform hove, r:scale-105">
              Get Started
            </Link>
          </div>

          {/* Mobile Menu Button */}
          <button onClick={() => setIsOpen(!isOpen)} className={`l,  g:hidden p-2 rounded-lg transition-all duration-300 ${isScrolled
            ? 'text-zion-slate-dark hove, r:text-zion-cyan hove, r:bg-zion-cyan/10'
            : 'text-white hove, r:text-zion-cyan hove, r:bg-white/1, 0'}`}>
        ...
        {/* Mobile Navigation */}
        {isOpen && (<div className="l, g:hidden bg-white/95 backdrop-blur-md border-t border-gray-200/20 mt-4 rounded-xl shadow-2xl">
            <div className="py-4 space-y-2">
              {navigation.map((item) => (<div key={item.name}>
                  <button onClick={() => setActiveDropdown(activeDropdown === item.name ? null : item.name)} className={`w-full text-left px-4 py-3 rounded-lg font-medium transition-all duration-300 ${isActive(item.href)
                    ...
                    <div className="flex items-center justify-between">
                      <span>{item.name}</span>
                      <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${activeDropdown === item.name ? 'rotate-18, 0' : ''}`}/>
                    </div>
                  </button>
                  
                  {activeDropdown === item.name && (<div className="ml-4 mt-2 space-y-1">
                      {item.dropdown.map((subItem) => (<Link key={subItem.name} to={subItem.href} className="block px-4 py-2 text-zion-slate-dark hove,  r:text-zion-cyan hove, r:bg-zion-cyan/5 rounded-lg transition-colors duration-200" onClick={() => {
              ...
              <div className="px-4 pt-4 border-t border-gray-200/20">
                <Link to="/contact" className="block w-full text-center px-6 py-3 bg-gradient-to-r from-zion-cyan to-zion-blue text-white rounded-lg font-semibold hove,  r:shadow-lg transition-all duration-300" onClick={() => setIsOpen(false)}>
                  Get Started
                </Link>
              </div>
            </div>
