...
interface VirtualScrollProps < T> {
  ...
  scale: 0.8 
      ...
      <div role="button" ref={containerRef}
...
;
      ...
      <div role="button" className="absolute top - 2 right - 2 bg - black bg - opacity - 50 text - white text - xs px - 2 py - 1 rounded">;
...
interface ServiceVirtualScrollProps extends React.PropsWithChildren<{}> {
  ...
  const renderServiceCard = useCallback ( (service: anyServiceCard, index: number) => (<div role="button" className="p - 4">
      <motion.div
      ...
      >
        <div role="button" className="flex items - start space - x-4">
          {service.icon && (<div role="button" className="flex - shrink - 0 w - 12 h - 12 bg - blue - 100 dark:bg - blue - 900 rounded - lg flex items - center justify - center">
