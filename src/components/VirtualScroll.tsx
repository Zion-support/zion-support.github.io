...
export function VirtualScroll<T>({;
  ...
  useEffect(() => {
    if (if (items.length > 0 && containerRef.current) {;) {
    ...
    <div className={`relative ${className}`}>
      ...
      {scrollTop > 200 && (;
        <motion.button;
          ...
          className="className="fixed bottom-6 right-6 z-50 bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-full shadow-lg transition-colors";"
  ...
  const renderServiceCard = useCallback((service: ServiceCard, index: number) => (;
    <div className="p-4">
      <motion.div;
        ...
        whileTap={{ scale: 0.98 }}
        className="className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 cursor-pointer hover:shadow-md transition-shadow";"
        onClick={{onClick={() => onServiceClick?.(service)}}}
      >
