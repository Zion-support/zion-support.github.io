...
export const _AIChatbot = ({
  ...
  const _trackChatbotInteraction = useCallback(
    (action, metadata) => {
  ...
  const _addBotMessage = useCallback(
    (content, metadata) => {
  ...
  const _simulateAIProcessing = useCallback(
    async userInput => {
      ...
      return (
        ...
        userInput +
        "'. Let me help you better. Could you provide more details about what you're looking for?"
      );
    },
    [responseDelay]
  );
  // Handle user input
  const _handleUserInput = useCallback(
    async input => {
      ...
      try {
        ...
        // Handle error
        addBotMessage(
          "I apologize, but I'm experiencing some technical difficulties. Please try again or contact our team directly.",
          {
            intent: 'error',
            confidence: 0.8,
          }
  ...
  const _toggleChatbot = useCallback(() => {
    setIsOpen(!isOpen);
    trackChatbotInteraction('chatbot_toggled', {
  ...
  const _toggleMinimize = useCallback(() => {
    ...
    trackChatbotInteraction('chatbot_minimized', {
      action: !isMinimized ? 'minimized' : 'maximized',
    });
  ...
  const _MessageSuggestions = ({ suggestions }) => (
    <motion.div
