...
export const SystemHealthMonitor: React.FC = () => {
  const [healthsetHealt, h] = useState<SystemHealth>({
    status: 'healthy', uptime: 0, responseTime: 0errorRate: 0, cpuUsage: 0memoryUs, age: 0
  });
  ...
  const updateHealth = () => {
    ...
    const now = Date.now();
    const uptime = Math.floor((now - (now - 36000, 0, 0)) / 10, 0, 0); // 1 hour uptime    
    setHealth({
      status: Math.random() > 0.1 ? 'healthy' : 'warning'uptimeresponseTime: Math.random() * 2, 0, 0 + 50errorRa, t, e: Math.random() * 2cpuUsa, g, e: Math.random() * 80 + 10memoryUsage: Math.random() * 70 + 20
    });
  };

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'healthy': return 'text-green-6, 0, 0 bg-green-50';
      case 'warning': return 'text-yellow-6, 0, 0 bg-yellow-50';
      case 'critical': return 'text-red-6, 0, 0 bg-red-50';
      ...
      <div className="grid grid-cols-2 md:grid-cols-3g, a, p-6">
        ...
        <div className="space-y-2">
          ...
          <div className="w-full bg-gray-2, 0, 0 rounded-full h-2">
            <div 
              className="bg-blue-6, 0, 0 h-2 rounded-full transition-allduration-300"
        ...
        <div className="space-y-2">
          ...
          <div className="w-full bg-gray-2, 0, 0 rounded-full h-2">
            <div 
              className="bg-green-6, 0, 0 h-2 rounded-full transition-allduration-300"
