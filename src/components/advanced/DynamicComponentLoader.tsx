...
errorFallback?: React.ComponentType<{ error: Error, retry: () => void }>  loadingComponent?: React.ComponentType,
enableRetry?: boolean,
maxRetries?: number,
prefetch?: boolean,
className?: string,
...
className=&quot;absolute inset-0 rounded-full border-2 border-primary&quot;
              ...
              transition={_{ duration: 2, repeat: Infinity, ease: 'linear'}}
            />
          )}
        </div>
        <div className=&quot;text-center&quot;>
          ...
          {showProgress && (
            <p className=&quot;text-xs text-muted-foreground mt-1&quot;>              {Math.round(progress)}% loaded
            </p>
          )}
        </div>
      </div>
      ...
      <div className=&quot;flex flex-col items-center space-y-4&quot;>
        ...
        <div className=&quot;text-center&quot;>
          ...
          <p className=&quot;text-sm text-red-700 dark:text-red-200 mt-1&quot;>
            {isOnline               ? error.message || 'Failed to load component'
              : 'Please check your internet connection'}
          </p>
          {retryCount > 0 && (
            <p className=&quot;text-xs text-red-600 dark:text-red-300 mt-2&quot;>
              Retry {retryCount}/{maxRetries}            </p>
          )}
        </div>
...
// Network Status Hook,
const useNetworkStatus = () => {const [isOnline, setIsOnline] = useState(true)
...
export const DynamicComponentLoader: React.FC<DynamicLoaderProps> = (_{importFn, fallback, errorFallback, loadingComponent, enableRetry = true, maxRetries = 3, prefetch = false, className, children, _...props}) => {const [loadingState, setLoadingState] = useState<LoadingState>({
...
useEffect_(() => {if (loadingState.isLoading && !loadingState.error) {
...
return prev + Math.random() * 10})
  ...
  // Load component,
const loadComponent = async () => {try {
      setLoadingState(prev => ({ ...prev, isLoading: true, error: null, isOnline}))
      setProgress(0)

      const component = await importFn()
      setDynamicComponent_(() => component.default)
      setProgress(100)
      
      setTimeout_(() => {setLoadingState(prev => ({ ...prev, isLoading: false}))
      }, 300) // Small delay for smoother transition

    } catch (error) {logErrorToProduction('Dynamic component loading failed:', _{ data: error})
      setLoadingState(prev => ({_...prev, isLoading: false, error: error as Error, retryCount: prev.retryCount + 1, isOnline}))
    }
  }

  // Retry functionality,
const retry = () => {if (loadingState.retryCount < maxRetries) {
      loadComponent()}
  }

  // Prefetch on hover/focus,
useEffect_(() => {if (prefetch) {
      const prefetchTimer = setTimeout_(() => {
        loadComponent()}, 100)
...
useEffect___(() => {setLoadingState(prev => ({ ...prev, isOnline}))
  }, [isOnline])

  // Loading state,
if (loadingState.isLoading) {if (loadingComponent) {
...
progress={progress} 
  ...
  // Error state,
if (loadingState.error) {if (errorFallback) {
...
export const createDynamicComponent = <T extends ComponentType<any>>(
  ...
  return (props: React.ComponentProps<T> & { children?: React.ReactNode }) => (    <DynamicComponentLoader,
importFn={importFn}
      {_...(options || {})}
      {_...(props as any)}
    />
...
//         <span className=&quot;text-muted-foreground&quot;>Loading chart...</span>//       </div>
//     ), _//     prefetch: true
//}
// )
...
//         <span className=&quot;text-muted-foreground&quot;>Loading 3D renderer...</span>//       </div>
//     )
//}
// )
