...
export default function UserExperienceMonitor({ isVisible, onClose }: UserExperienceMonitorProps) {
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg max-w-7xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-3xl font-bold text-gray-900">User Experience Monitor</h2>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {uxMetrics.map((metric) => (
                  <div key={metric.id} className="bg-gray-50 p-6 rounded-lg">
                    ...
                    <div className="mb-2">
                      <span className={`text-3xl font-bold ${getPerformanceColor(metric.value, metric.target)}`}>
                        ...
                        <tr key={session.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-sm">
                            <div className="flex items-center">
                              ...
                              <span className={getPerformanceColor(session.satisfaction, 4.0)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-blue-50 p-6 rounded-lg text-center">
                  <div className="text-2xl font-bold text-blue-600">{userSessions.length}</div>
                  <div className="text-sm text-gray-600">Active Sessions</div>
                </div>
                <div className="bg-green-50 p-6 rounded-lg text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {Math.round(userSessions.reduce((acc, s) => acc + s.satisfaction, 0) / userSessions.length * 10) / 10}
                  </div>
                  <div className="text-sm text-gray-600">Avg Satisfaction</div>
                </div>
                <div className="bg-purple-50 p-6 rounded-lg text-center">
                  <div className="text-2xl font-bold text-purple-600">
                    {Math.round(userSessions.reduce((acc, s) => acc + s.duration, 0) / userSessions.length / 60)}m
                  </div>
                  <div className="text-sm text-gray-600">Avg Session Duration</div>
                </div>
                <div className="bg-yellow-50 p-6 rounded-lg text-center">
                  <div className="text-2xl font-bold text-yellow-600">
                    {Math.round(userSessions.reduce((acc, s) => acc + s.pageViews, 0) / userSessions.length)}
                  </div>
