...
const mockPerformanceAlerts = [
  ...
  },
  {
    ...
    recommendations: ['Increase connection pool size', 'Implement connection pooling', 'Monitor query performance'],
    status: 'active'
  }
];
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-zion-slate-light/20">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-semibold text-zion-slate-dark dark:text-white">Performance Monitor</h3>
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <AlertTriangle className={`w-4 h-4 ${showAlerts ? 'text-yellow-500' : 'text-gray-400'}`} />
          </button>
          <button
      ...
      <div className="p-4 space-y-4 overflow-y-auto h-full">
        ...
        <div className="text-center">
          <div className="text-2xl font-bold text-zion-cyan">{performanceScore}</div>
          ...
          {mockSystemMetrics.slice(0, 4).map((metric) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-zion-slate-light">{metric.name}</span>
        ...
        {showAlerts && mockPerformanceAlerts.length > 0 && (
          <div className="space-y-2">
            <h4 className="text-sm font-semibold text-zion-slate-dark dark:text-white">Active Alerts</h4>
        ...
        {isExpanded && (
          <div className="space-y-4">
            <div className="border-t border-zion-slate-light/20 pt-4">
              <h4 className="text-sm font-semibold text-zion-slate-dark dark:text-white mb-3">System Health Overview</h4>
              <div className="space-y-2">
                {mockSystemMetrics.map((metric) => (
                  <div key={metric.id} className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={getStatusColor(metric.status)}>{metric.value}{metric.unit}</span>
                      {getStatusIcon(metric.status)}
                    </div>
                  </div>
                ))}
