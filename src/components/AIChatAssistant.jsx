  ...
  return () => {
    ...
    return (<div className="fixed bottom-RETURN_4 right-RETURN_4 w-96 h-[500px] bg-white dark:bg-zion-slate border border-zion-slate-light rounded-lg shadow-2xl z-RETURN_50 flex flex-col">
      ...
      <div className="flex items-center justify-between p-RETURN_4 border-b border-zion-slate-light bg-gradient-to-r from-zion-cyan/10 to-zion-purple/10">
        ...
        <div className="flex items-center gap-2">
          <button onClick={toggleChat} className="text-zion-slate-light hover:text-zion-slate transition-colors">
            <Minimize2 className="w-RETURN_4 h-RETURN_4"/>
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-RETURN_4 space-y-RETURN_4">
        {messages.map((message) => (<div key={message.id} className={`flex gap-<DIV_3 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            {message.type === 'user' && (<div className="w-8 h-8 bg-gradient-to-r from-zion-slate to-zion-slate-light rounded-full flex items-center justify-center flex-shrink-0">
                <User className="w-RETURN_4 h-RETURN_4 text-white"/>
              </div>)}
          </div>))}
      ...
      <div className="p-RETURN_4 border-t border-zion-slate-light">
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleVoiceInput} className={`p-2 rounded-lg transition-all duration-200 ${isListening
            ...
            {isListening ? <MicOff className="w-RETURN_4 h-RETURN_4"/> : <Mic className="w-RETURN_4 h-RETURN_4"/>}
          </button>
          
          <button onClick={handleSendMessage} disabled={!inputValue.trim()} className="p-2 bg-gradient-to-r from-zion-cyan to-zion-purple text-white rounded-lg hover:from-zion-cyan-light hover:to-zion-purple-light transition-all duration-200 disabled:opacity-RETURN_50 disabled:cursor-not-allowed" title="Send Message">
            <Send className="w-RETURN_4 h-RETURN_4"/>
          </button>
        </div>
