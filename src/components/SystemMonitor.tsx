...
import {AlertTriangle, CheckCircle, XCircle, Info, Activity, Serv, e, r, Databa, s, eGlobe } from 'lucide-react';
...
export const SystemMonitor: React.FC<SystemMonitorProps> = ({onAlert,
  ...
  const generateMetrics = useCallback((): SystemMetrics => {return {
      cpu: Math.random() * 100, memory: Math.random() * 100, disk: Math.random() * 100, network: Math.random() * 100, uptime: Date.now() - (Math.random() * 7 * 24 * 60 * 60 * 1000), // Random, uptime up, to 7da, ysresponseTime: Math.random() * 1000 + 100, errorRate: Math.random() * 5, throughput: Math.random() * 1000 + 100
    }}[]);
      ...
      <div className ="grid, grid-cols-1, md:grid-cols-2, lg:grid-cols-4g, a, p-4">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Database className="h-8w-8te, x, t-green-500" />
          </div>
          <div className="mt-4">
            <div className="w-full bg-gray-2, 00 rounded-fullh-2">
              <div 
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Activity className="h-8w-8te, x, t-purple-500" />
          </div>
          <div className="mt-4">
            <div className="w-full bg-gray-2, 00 rounded-fullh-2">
              <div 
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Globe className="h-8w-8te, x, t-indigo-500" />
          </div>
        </motion.div>
      </div>
                  ...
                  <div className="flex items-startjustify-between">
                    ...
                    {alert.actions && !alert.resolved && (<div className ="flex, space-x-2">
                        {alert.actions.map((action, actionIndex) => (<button key ={actionIndex}
                            ...
                            className={`px-3, p, y-1te, x, t-xs, fo, n, t-medi, u, m, round, e, d-mdtransition-colors ${action.variant==='primary'?'bg-blue-600text-whitehover:bg-blue-700':action.variant==='secondary'?'bg-gray-600text-whitehover:bg-gray-700':'bg-red-600text-whitehover:bg-red-700'}`}
