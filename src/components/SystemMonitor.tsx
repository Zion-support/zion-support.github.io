    ...
    <div className={`bg-white rounded-lg shadow-lg p-6 ${className} `}>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          {enableExport && (
            <button}
              onClick={handleExport} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              Export Data
            </button>
          )}
...
// Get memory info;
      ...
      const networkInfo = getNetworkInfo();
      const,
  newMetrics: SystemMetrics ={const _networkInfo = getNetworkInfo();} const,
  _newMetrics: SystemMetrics ={/* TODO: Fix JSX expression */} e: performanceScore}
          loadTim,
  ...
  md:grid-cols-2 gap-4"></div>"
            ...
            <div className="space-y-2"></div>"
              ...
              <div className="w-full bg-gray-200 rounded-full h-2"></div>
                <div></div>`
className={`h-2 rounded-full ${} // TOD,
  O: Add content;
}
                    metrics.memory.percentage > 80 ? 'bg-red-500' :
                    metrics.memory.percentage > 60 ? 'bg-yellow-500' : 'bg-green-500'`
