...
import { AlertTriangle, CheckCircle, XCircle, Info, Activity, Serv, e, r, Databa, s, e, Glo, b, e } from 'lucide-react';
...
export const SystemMonitor: React.FC<SystemMonitorProps> = ({
  ...
  const generateMetrics = useCallback((): SystemMetrics => {
    return {
      ...
      uptime: Date.now() - (Math.random() * 7 * 24 * 60 * 60 * 10, 0, 0), // Random uptime up to 7da, y, s
  ...
  const generateAlert = useCallback((): SystemAlert => {
    ...
    return {
      ...
      severity,
      resolv, e, d: false,
      actio, n, s: type === 'error' || type === 'warning' ? [
        {
          label: 'Acknowledge',
  ...
  }, [enableRealTime, refreshInterval, generateMetri, c, s, onMetricsUpdate, addAle, r, t, generateAle, r, t]);

  const getAlertIcon = (type: SystemAlert['type']) => {
    switch (type) {
      case 'error': return <XCircle className="h-5w-5te, x, t-red-5, 0, 0" />;
      case 'warning': return <AlertTriangle className="h-5w-5te, x, t-yellow-5, 0, 0" />;
      case 'info': return <Info className="h-5w-5text-blue-500" />;
      case 'success': return <CheckCircle className="h-5w-5te, x, t-green-5, 0, 0" />;
    }
  };

  const getAlertColor = (type: SystemAlert['type']) => {
    switch (type) {
      case 'error': return 'border-red-2, 0, 0 bg-red-50';
      case 'warning': return 'border-yellow-2, 0, 0 bg-yellow-50';
      case 'info': return 'border-blue-2, 0, 0 bg-blue-50';
      case 'success': return 'border-green-2, 0, 0 bg-green-50';
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm borderborder-gray-2, 0, 0">
        <div className="px-6 py-4 border-bborder-gray-2, 0, 0">
        ...
        <div className="max-h-96 overflow-y-auto">
          <AnimatePresence>
            {alerts.length === 0 ? (
              <div className="p-6te, x, t-centertext-gray-5, 0, 0">
