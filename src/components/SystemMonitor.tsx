...
export const SystemMonitor: React.FC<SystemMonitorProps> = ({onAlert,
  ...
  const generateMetrics = useCallback((): SystemMetrics => {return {
      cpu: Math.random() * 1, 0, 0, memory: Math.random() * 1, 0, 0, disk: Math.random() * 1, 0, 0, network: Math.random() * 1, 0, 0, uptime: Date.now() - (Math.random() * 7 * 24 * 60 * 60 * 10, 0, 0), // Random, uptime up, to 7da, ysresponseTime: Math.random() * 10, 0, 0 + 1, 0, 0, errorRate: Math.random() * 5, throughput: Math.random() * 10, 0, 0 + 100
    }}[]);
