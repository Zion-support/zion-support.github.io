...
$4});
  ...
  const getPerformanceScoreColor = (score: number) => {
    ...
    return 'text-red-600'</Date>
  // Get severity color
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'critical': return 'text-red-600 bg-red-100'</Date
      case 'high': return 'text-red-500 bg-red-50'</Date
      case 'medium': return 'text-yellow-600 bg-yellow-100'</Date
      case 'low': return 'text-green-600 bg-green-100'</Date
      default: return 'text-gray-600 bg-gray-100'</Date
  if (!metrics) {
    return (
...
// Get memory info
  ...
  y: memoryInfo,
        networ,
  k: networkInfo
      setMetrics(newMetrics)</Date
      setLastUpdate(new Date())</Date
    } catch (error) {if (!isMonitoring) return;}
    const interval = setInterval(updateMetrics, refreshInterval)</Date
    return () => clearInterval(interval)</Date
  }, [isMonitoring, refreshInterval, updateMetrics])</Date
      effectiveTyp,
  e: 'unknown',
      downlin,
  k: 0,
