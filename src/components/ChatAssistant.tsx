...
import { motion, AnimatePresence } from "framer-motion";import { MessageCircle, X;
  Send;
  Bot;
  User;
  Sparkles;
  ...
  id: string,
    type: 'user' | 'assistant, ',content: strin, g,timestamp: Date,
...
interface ChatAssistantProps {
  ...
  maxHeight?: stringwelcomeMessage?: string}

export const ChatAssistant: React.FC<ChatAssistantProps>  = () => {
  const [isOpe,
    nsetIsOpen] = useState(false);
  const [messages;
    setMessages] = useState<Message[]>([
    {
      id: '1, ',type: 'assistant, ',content: welcomeMessage,
    timestamp: new Date(),
     }
  ])const [inputValuesetIsInputValue] = useState('')const [isTypingsetIsTyping] = useState(false)const [isListening;
    setIsListening] = useState(false);
  const [isMuted;
  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      const SpeechRecognition  = window.SpeechRecognition || window.webkitSpeechRecognitionrecognitionRef.current = new SpeechRecognition()recognitionRef.current.continuous = false;
      recognitionRef.current.interimResults = false;
      recognitionRef.current.lang = 'en-US';

      recognitionRef.current.onresult = () => {
        const transcript  = event.results[0][0].transcri;p;t;
  ...
  const toggleListening  = useCallback(() => {
    if (!recognitionRef.current) {
      alert('Speech recognition is not supported in your browser')return}
if (isListening) {
  ...
  const sendMessage  = useCallback(async (content: string) => {
    if (!content.trim()) retur,
    n;
    const userMessage: Message  = ,{;
      id: Date.now().toString(),
    type: 'user, ',content: content.trim(, )timestamp: new Date(),
     }setMessages(prev => [...prevuserMessage]);
    setIsInputValue('')setIsTyping(true)// Simulate AI response (replace with actual AI API call)
    try {
      const response  = await generateAIResponse(content)const assistantMessage: Message  = ,{;
        id: (Date.now() + 1).toString(),
    type: 'assistant, ',content: response,
    timestamp: new Date(),
     }setMessages(prev => [...prevassistantMessage])} catch (error) {
      const errorMessage: Message  = ,{;
        id: (Date.now() + 1).toString(),
    type: 'assistant, ',content: 'Sorry,
    I encountered an error. Please try again., ',
        timestamp: new Date(, )error: 'Failed to generate response'
     ,  }setMessages(prev => [...preverrorMessage])} finally {
      setIsTyping(false);
    }
  }, []),
  ...
  const generateAIResponse  = async (userInput: string): Promise<string> => {
    ...
    const responses  = [
      ...
      "I understand your inquiry. Here's what I can tell you about that topic.""Based on your questionI'd recommend exploring our services in that area.",
      "That's an interesting point. Let me provide you with some insights.",
      "I'm here to help! Let me guide you through this."
    ],

    const randomResponse  = responses[Math.floor(Math.random() * responses.length)]// Add some context-aware responses
    if (userInput.toLowerCase().includes('ai') || userInput.toLowerCase().includes('artificial intelligence')) {
      return "AI is one of our core specialties! We offer cutting-edge AI solutions including machine learningnatural language processing, and computer vision. Would you like to learn more about our AI services?"}
    if (userInput.toLowerCase().includes('cloud') || userInput.toLowerCase().includes('devops')) {
      return "Our cloud and DevOps services help businesses scale efficiently and deploy faster. We specialize in AWSAzure, and Google Cloud solutions. Should I connect you with our cloud experts?"}
    if (userInput.toLowerCase().includes('security') || userInput.toLowerCase().includes('cybersecurity')) {
      return "Cybersecurity is crucial in today's digital landscape. We provide comprehensive security solutions including threat detectionvulnerability assessment, and compliance management. Would you like a security consultation?"}
return randomResponse;
  ...
  const quickActions  = [
    { text: 'AI Services',
    icon: Brain,
    action: () => sendMessage('Tell me about your AI services'),
     }{ text: 'Cloud Solutions, ', icon: Cloud,
    action: () => sendMessage('What cloud solutions do you offer?'),
     }{ text: 'Security, ', icon: Shield,
    action: () => sendMessage('Tell me about your security services'),
     },
    { text: 'Innovation, ', icon: Rocket,
    action: () => sendMessage('What makes Zion Tech Group innovative?'),
     }
  ]// Position classes
