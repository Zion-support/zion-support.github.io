...
'`'`'`'`"`''
    ...
    const getScoreText = (score) => {}"'
        ...
        if (score >= 40)';''
            return 'Fair;'
        return 'Poor'}';''
    return (<div className='fixed bottom-6 right-24 z-50'></div>'"
      {/* Floating Action Button */}'"'"
      <button onClick={() =" ></button> setIsOpen(!isOpen)} className='bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105' aria-label='Toggle SEO Optimizer'>'"'"
        <Search className='w-6 h-6'/" ></Search>"'"
      </button>"'
'
      {/* SEO Optimizer Panel */}''
      {isOpen && (<div className='absolute bottom-16 right-0 w-[700px] bg-white dark:bg-gray-900 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden'></div>'
          {/* Header */}''
          <div className='bg-gradient-to-r from-purple-600 to-pink-600 text-white p-4'></div>''
            <div className='flex items-center justify-between'></div>''"
              <h3 className='text-lg font-semibold flex items-center gap-2'></h3>'"'"
                <TrendingUp className='w-5 h-5'/" ></TrendingUp>"'
                SEO Optimizer'"
              </h3>'"'"
              <button onClick={() =" ></button> setIsOpen(false)} className='text-white/80 hover:text-white transition-colors'>
                ×"
              </button>"'"
            </div>"'
            '
            {/* Summary Stats */}''
            {report && (<div className='grid grid-cols-3 gap-4 mt-3'></div>''
                <div className='text-center'></div>''
                  <div className='text-2xl font-bold'></di>{report.totalPages}</div>''
                  <div className='text-sm text-purple-100'></di>Pages Analyzed</div>'
                </div>''
                <div className='text-center'></div>''
                  <div className='text-2xl font-bold'></di>{report.averageScore}%</div>''
                  <div className='text-sm text-purple-100'></di>Avg SEO Score</div>'
                </div>''
                <div className='text-center'></div>''
                  <div className='text-2xl font-bold'></di>{report.pagesWithIssues}</div>''
                  <div className='text-sm text-pink-100'></di>Need Optimization</div>
                </div>"
              </div>) }"'"
          </div>"'
'
          {/* Content */}''
          <div className='p-4 max-h-[500px] overflow-y-auto'></div>''"
            {isAnalyzing ? (<div className='flex items-center justify-center py-8'></div>'"'"
                <RefreshCw className='w-6 h-6 animate-spin text-purple-600'/" ></RefreshCw>''
                <span className='ml-2 text-gray-600'></spa>Analyzing SEO...</span>''
              </div>) : report ? (<div className='space-y-4'></div>'
                {/* Top Issues */}''
                {report.topIssues.length > 0 && (<div className='bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg'></div>''"
                    <h4 className='font-medium text-yellow-800 dark:text-yellow-200 mb-2 flex items-center gap-2'></h4>'"'"
                      <AlertTriangle className='w-4 h-4'/" ></AlertTriangle>"'
                      Top SEO Issues to Address'
                    </h4>''
                    <div className='space-y-1'></div>''
                      {report.topIssues.slice(0, 3).map((issue, index) => (<div key={index} className='text-sm text-yellow-700 dark:text-yellow-300'></div>
                          • {issue}
                        </div>) ) }"
                    </div>"'"
                  </div>) }"'
'
                {/* Pages Analysis */}''
                <div className='bg-gray-50 dark:bg-gray-800 rounded-lg overflow-hidden'></div>''
                  <div className='overflow-x-auto'></div>''
                    <table className='w-full'></table>''
                      <thead className='bg-gray-100 dark:bg-gray-700'></thead>'"
                        <tr></tr>'"'"
                      ...
                      <tbody className='bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700'></tbody>'"'"
                        {report.pageAnalyses.map((page, index) => (<tr key={index} className='hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer' onClick={() =" ></tr> setSelectedPage(page)}>''"
                            ...
                            <td className='px-4 py-3'></td>``""
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${getScoreColor(page.score)}`}" ></span>"'"
...
'
                {/* Page Details Modal */}''
                {selectedPage && (<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'></div>''
                    <div className='bg-white dark:bg-gray-900 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto'></div>''"
                      <div className='flex items-center justify-between mb-4'></div>'"'"
                        <h3 className='text-lg font-semibold text-gray-900 dark:text-white'></h3>"'
                          SEO Analysis: {selectedPage.title}'"
                        </h3>'"'"
                        <button onClick={() =" ></button> setSelectedPage(null)} className='text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'>"
                          ×"'"
                        </button>"'
                      </div>'
                      ''
                      <div className='space-y-4'></div>''
                        <div className='grid grid-cols-2 gap-4'></div>''
                          <div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'></div>''
                            <div className='text-sm text-gray-500 dark:text-gray-400'></di>SEO Score</div>''
                            <div className='text-lg font-semibold text-gray-900 dark:text-white'></di>{selectedPage.score}%</div>'
                          </div>''
                          <div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'></div>''
                            <div className='text-sm text-gray-500 dark:text-gray-400'></di>Headings</div>''
                            <div className='text-lg font-semibold text-gray-900 dark:text-white'></di>{selectedPage.headings.length}</div>'
                          </div>''
                          <div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'></div>''
                            <div className='text-sm text-gray-500 dark:text-gray-400'></di>Images</div>''
                            <div className='text-lg font-semibold text-gray-900 dark:text-white'></di>{selectedPage.images.length}</div>'
                          </div>''
                          <div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'></div>''
                            <div className='text-sm text-gray-500 dark:text-gray-400'></di>Links</div>''
                            <div className='text-lg font-semibold text-gray-900 dark:text-white'></di>{selectedPage.links.length}</div>"
                          </div>"'"
                        </div>"'
'
                        {selectedPage.issues.length > 0 && (<div></div>''
                            <h4 className='font-medium text-red-600 dark:text-red-400 mb-2'></h>SEO Issues Found</h4>''
                            <div className='space-y-2'></div>''"
                              {selectedPage.issues.map((issue, index) => (<div key={index} className='flex items-start gap-2 text-sm text-red-600 dark:text-red-400'></div>'"'"
                                  <AlertTriangle className='w-4 h-4 mt-0.5 flex-shrink-0'/" ></AlertTriangle>
                                  <span></spa>{issue}</span>
                                </div>) ) }"
                            </div>"'"
                          </div>) }"'
'
                        {selectedPage.recommendations.length > 0 && (<div></div>''
                            <h4 className='font-medium text-green-600 dark:text-green-400 mb-2'></h>SEO Recommendations</h4>''
                            <div className='space-y-2'></div>''"
                              {selectedPage.recommendations.map((rec, index) => (<div key={index} className='flex items-start gap-2 text-sm text-green-600 dark:text-green-400'></div>'"'"
                                  <Zap className='w-4 h-4 mt-0.5 flex-shrink-0'/" ></Zap>
                                  <span></spa>{rec}</span>
                                </div>) ) }
                            </div>"
                          </div>) }"'"
              ...
              </div>) : (<div className='text-center py-8 text-gray-500'></div>
                No SEO analysis data available"
              </div>) }"'"
          </div>"'
'"
          {/* Footer Actions */}`'"'"
          <div className='bg-gray-50 dark:bg-gray-800 p-3 flex gap-2'></div>'`"`'"'"
            <button onClick={analyzeSEO} disabled={isAnalyzing} className='flex-1 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-400 text-white px-3 py-2 rounded-md text-sm font-medium transition-colors flex items-center justify-center gap-2'></button>'`'`"`'"'"
              <RefreshCw className={`w-4 h-4 ${isAnalyzing ? 'animate-spin' : }`}/" ></RefreshCw>'
              {isAnalyzing ? 'Analyzing...' : 'Analyze SEO'}'
            </button>''"
            <button onClick={exportSEOReport} className='px-3 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-md transition-colors flex items-center gap-2'></button>'"'"
