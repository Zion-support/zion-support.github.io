...
const [bulkSkills, setBulkSkills] = useState('')const [error, setError] = useState < string | null>(null)const { enhanceContent, isEnhancing 
} = useResumeEnhancer()const { addSkill } = useResume()const handleCategorizeSkills = async ()  => {
if (!bulkSkills |bulkSkills.trim().length === 0) {setError('Please enter some skills to categorize')return;
