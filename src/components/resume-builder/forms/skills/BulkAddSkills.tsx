...
export const BulkAddSkills = ({ resumeId, onSuccess }: BulkAddSkillsProps) => {const [bulkSkills, setBulkSkills] =;
  }
  useState('');'
  const [error, setError] = useState<string | null>(null)const { enhanceContent, isEnhancing } = useResumeEnhancer()const { addSkill } = useResume(;
  const handleCategorizeSkills = async () => {if (!bulkSkills |bulkSkills.trim().length === 0) {setError('Please enter some skills to categorize';'
  }
  return;
    }
...
const categorizedSkills = JSON.parse(enhancedContent)// Add the categorized skills;
    ...
    } catch ("err": any) {setError(err && err.message || 'Failed to categorize skills'),'
