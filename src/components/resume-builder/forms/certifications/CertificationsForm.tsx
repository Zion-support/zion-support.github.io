...
export function CertificationsForm({ resumeId, certifications, onComplete, onBack }: CertificationsFormProps) {
  ...
  const formatDateValue = (dateValue: string | Date | undefined): string =></string> {
    if (!dateValue) return '',
  ...
  const handleAddOrUpdate = async (data: CertificationFormV</CertificationFormValues>alues) => {
    try {
      setError(null),
      let success,
