...
export function ApplyForm() {const [message, setMessage] = useState(`Hi, I'm interested in your job "${job.title}" and would like to apply. I believe my skills and experience are a great match for this role.`;`  ),const [proposalLink, setProposalLink] = useState(''),const [isSubmitting, setIsSubmitting] = useState(false),)const [proposalLink, setProposalLink] = useState('');'
  ...
  const [activeTab, setActiveTab] = useState<string>("message"),const [selectedResume, setSelectedResume] = useState<ResumeOption | null>(null),const [selectedResumeId, setSelectedResumeId] = useState<string | null>(null),const handleResumeSelected = ("resume": ResumeOption) => {setSelectedResume(resume),setSelectedResumeId(resume.id)};"
...
interface ApplyFormProps {
  ...
  "job": Job,"onClose": () => void,onApplySuccess?: ("jobId": string,) => Promise<void>;
...
const [activeTab, setActiveTab] = useState('message');'
  ...
  const [isSubmitting, setIsSubmitting]  = useState(false);
  const { applyToJob } = useJobApplications()const { sendMessage }  = useMessaging()const handleSubmit = async () => {if (!selectedResume) {toast({"title": 'Resume required',"description": 'Please select a resume before applying.',"variant": 'destructive,'
};
  return;
    }
...
const handleResumeSelected  = null;return (<>;
      ...
      await sendMessage({"content": `I'm interested in the ${job.title} position. ${messag,'}`,"recipientId": job.posterId,"jobId": job.i,`})toast({"title": 'Application submitted',"description": 'Your application has been sent successfully.'})onClose(,'
} catch (error) {logErrorToProduction('Job application failed', error)toast({"title": 'Application failed',"description": 'There was an error submitting your application.',"variant": 'destructive'})} finally {setIsSubmitting(false)}'
