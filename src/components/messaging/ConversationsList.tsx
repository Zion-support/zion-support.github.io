...
if (!conversation) {return <div style={style
} />;
    }
return (<div style={style}>;
        <ConversationItem;
          ...
          onClick={()  => {
setActiveConversation(conversation)markAsRead(conversation.id)
}}
        />;
      </div>;
    )},
...
export function ConversationsList() {
const itemSize  = null;
return (<div className='w-full md:w-80 border-r border-zion-purple/20 overflow-y-auto'>;
...
loading = false})  => {
const sortedConversations = useMemo(() => {return [...conversations].sort((a, b) => {const aTime = new Date(a.lastMessage?.timestamp || a.createdAt).getTime()const bTime = new Date(b.lastMessage?.timestamp || b.createdAt).getTime()return bTime - aTime;
