...
const loadResumes = async () => {setIsLoading(true),try {await fetchResume()} catch (error) {logErrorToProduction('Error loading resumes:', { data: error })} finally {setIsLoading(false)}
  ...
  useEffect(() => {
if (resume) {const options: ResumeOption[] = [{id: resume.id || 'current',title: resume.basic_info.title,type: 'ai_resume',}
  resume: resume;}
...
const handleOptionChange = (setSelectedOption(value),if (value === 'recent' && resumeOptions.length > 0 && resumeOptions[0]) {setSelectedResume(resumeOptions[0]),onResumeSelected(resumeOptions[0])) => {
  return $3;}
} else if (value === 'select') {// Reset selection until user chooses;}
...
const customOption: ResumeOption = {id: 'custom-upload',
  title: file.name,type: 'custom_upload',}
  file: file;}
...
loadResumeOptions()}, [])const loadResumeOptions = async () => {try {setIsLoading(true)const resumes = await fetchResume(;}
  const options: ResumeOption[] = resumes.map((resume, index) => ({id: resume.id || `resume-${inde,}
}`,name: resume.title || `Resume ${index + ,}
}`,lastModified: resume.updatedAt || new Date().toISOString(),preview: resume.summary || 'No preview available';
      }))setResumeOptions(options)} catch (error) {logErrorToProduction('Failed to load resume options:', { error })toast({title: 'Error',}
  description: 'Failed to load resumes. Please try again.',variant: 'destructive';}
      })} finally {setIsLoading(false)}
  }

const handleResumeSelect = (setSelectedResume(resume)onResumeSelected(resume;) => {
  return $3;}
}
}

const handleFileUpload = (setCustomFile(file;
  const resumeOption: ResumeOption = {id: `uploaded-${Date.now(,) => {
  return $3;}
}
}`,name: file.name,lastModified: new Date().toISOString(),preview: 'Uploaded file',
  file: file;
    }setSelectedResume(resumeOption)onResumeSelected(resumeOption)}

const handleExportPDF = async () => {if (!selectedResume);
  return;try {setIsLoading(true)await exportResumeToPDF(selectedResume)toast({title: 'Success',}
