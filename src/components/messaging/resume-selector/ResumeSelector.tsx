...
;
  ...
  useEffect(()  => {
const loadResumes = async () => {try {await fetchResume()
} catch (error) {logErrorToProduction('Error loading resumes:', { data: error })} finally {setIsLoading(false)})}
...
} = useResume(),// Fetch resume data when component mounts;
  useEffect(()  => {
const loadResumes = async () => {setIsLoading(true),
try {await fetchResume()
} catch (error) {logErrorToProduction('Error loading resumes:', { data: error })} finally {setIsLoading(false)}
    },
loadResumes()}, [fetchResume]),// Update resume options when resume data changes;
  useEffect(()  => {
if (resume) {const options: ResumeOption[] = [{id: resume.id || 'current',
...
setResumeOptions(options),// Pre-select the most recent resume;
      if (options.length > 0 && selectedOption === 'recent' && options[0]) {setSelectedResume(options[0]),
...
if (value === 'recent' && resumeOptions.length > 0 && resumeOptions[0]) {setSelectedResume(resumeOptions[0]),
...
if (e.target.files && e.target.files[0]) {const file = e.target.files[0],// Check if it's a PDF file;
...
if (!selectedResume || selectedResume.type !== 'ai_resume' || !selectedResume.resume) {return;
    
}
try {setIsLoading(true),
const pdfBlob = await exportResumeToPDF(selectedResume.resume),// Create download link;
...
toast({title: "Success!",
description: "Your resume has been downloaded."})} catch (error) {logErrorToProduction('Error downloading PDF:', { data: error }),
...
try {setIsLoading(true)const resumes = await fetchResume()const options: ResumeOption[] = resumes.map((resume, index) => ({id: resume.id || `resume-${index
}`,
...
setSelectedResume(resume)onResumeSelected(resume)
}
const handleFileUpload = (file: File)  => {
setCustomFile(file)const resumeOption: ResumeOption = {id: `uploaded-${Date.now()
}`,
name: file.name,
...
try {setIsLoading(true)await exportResumeToPDF(selectedResume)toast({title: 'Success',
...
return (<div className="space-y-6">;
      ...
      {selectedOption === 'recent' && resume && (<div className="space-y-4">;
          <ResumePreviewCard;
            resume={{id: resume.id || 'recent',
name: resume.title || 'Most Recent Resume',
lastModified: resume.updatedAt || new Date().toISOString(),
preview: resume.summary || 'No preview available';
            }}
            onSelect={() => handleResumeSelect({id: resume.id || 'recent',
name: resume.title || 'Most Recent Resume',
lastModified: resume.updatedAt || new Date().toISOString(),
preview: resume.summary || 'No preview available';
            })}
...
}}, [fetchResume])//Update resume options when resume data changes useEffect ( ()  => {
if (resume) {const options: ResumeOption[] = [ {setResumeOptions (options)//Pre-select the most recent resume ;
...
finally {setIsLoading (false)}//Handle "Generate Resume Now" button const handleGenerateResume = ()  => {
';
