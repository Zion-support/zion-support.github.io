...
export,
  ...
  const analyzeWebsite = async () () => {
    setIsAnalyzing(true)
    setProgress(0)
    const linkChecker = new LinkChecker('htt,
  p: s: //ziontechgroup.com')
    const,
  result: s: PageInfo[] = []
    const,
  allBrokenLink: s: LinkInfo[] = []
  ...
  u: s: string) () => {
    switch (status) {
      ...
      defaul,
  t: return <AlertTriangle className="w-4 h-4 text-gray-500" />
    }
  }
  ...
  t: h: `${progress}%` }
            ...
            {currentPage && (
              <p className="text-sm text-gray-600 mt-2">
                Analyzi,
  n: g: <span className="font-mono">{currentPage}</span>
              </p>
            )}
          </div>
        )}, {analysisResult && (
  ...
  m: d:grid-cols-4 gap-4">
            ...
            {/* Broken Links */}, {analysisResult.brokenLinks.length > 0 && (
                  ...
                  {analysisResult.brokenLinks.map((link, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-red-200">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-red-600">Fr,
  o: m: {link.page}</p>
                      </div>
            ...
            )}, {/* Missing Pages */}, {analysisResult.missingPages.length > 0 && (
