...
ursor/automate-test-fix-improve-and-merge-code-48f3;
    useEffect(() => {
        ...
        if (reducedMotion) {"
            ...
            root.classList.remove("reduced-motion');// Font size;"
        ...
        root.style.filter = colorBlindMode ===;"none" ?;"none' :;"
            colorBlindMode ===;"protanopia" ?;"url(#protanopia)" :;
                colorBlindMode ===;"deuteranopia" ?;"url(#deuteranopia)" :;";url(#tritanopia)}, [highContrast, reducedMotion, fontSize, colorBlindMode]);'
...
ursor/automate-test-fix-improve-and-merge-code-48f3;
    ...
    return (<>';"{/* Floating Accessibility Button */}";''
      ...
      <AnimatePresence>""        {isOpen && (<motion .div initial={{ opacity: 0 } animate={{ opacity: 1 } exit={{ opacity: 0 } className='fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4', onClick={() =" ></motion> setIsOpen(false)}>'
            <motion .div initial = {
  ...
  opacity: 0";"} className='bg-zion-blue-dark border border-zion-cyan/20 rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto' onClick={(e) =" ></motion> e.stopPropagation()}>';"{/* Header */}";'
              ...
              <div className='space-y-6'>';"{/* High Contrast */}";''
                <div className='flex items-center justify-between'>';"
                  ...
                  <Button variant={highContrast ? 'default' : 'outline'} size='sm' onClick={toggleHighContrast} className={highContrast ? 'bg-zion-cyan text-white' : 'border-zion-cyan/30 text-zion-cyan'}" >"'{highContrast ? <Eye className='w-4 h-4'/" ></Eye> : <EyeOff className='w-4 h-4'/" ></EyeOff>}"
                  </Button>"
                </div>';"{/* Reduced Motion */}";''
                <div className='flex items-center justify-between'>';"
                  <div>";''
                  ...
                  <Button variant={reducedMotion ? 'default' : 'outline'} size='sm' onClick={toggleReducedMotion} className={reducedMotion ? 'bg-zion-cyan text-white' : 'border-zion-cyan/30 text-zion-cyan'}" >"'{reducedMotion ? <VolumeX className='w-4 h-4'/" ></VolumeX> : <Volume2 className='w-4 h-4'/" ></Volume2>}"
                  </Button>"
                </div>'{/* Font Size */}";"
                <div>'";"
                  <h3 className='text-white font-medium mb-3"></h>Font Size</h3>";''
                  <div className='flex gap-2">";"{["small', 'medium","large'].map((size) => (<Button key={size} variant={fontSize === size ? 'default' : 'outline'} size='sm', onClick={() =" > setFontSize(size)} className={fontSize === size ? 'bg-zion-cyan text-white' : 'border-zion-cyan/30 text-zion-cyan'}>{size.charAt(0).toUpperCase() + size.slice(1)}"
                      </Button>))}"
                  </div>''
                </div>'{/* Color Blind Mode */}";"
                <div>'";"
                  <h3 className='text-white font-medium mb-3"></h>Color Blind Support</h3>";''
                  <div className='grid grid-cols-2 gap-2">";"{["none', 'protanopia","deuteranopia', 'tritanopia'].map((mode) => (<Button key={mode} variant={colorBlindMode === mode ? 'default' : 'outline'} size='sm', onClick={() =" > setColorBlindMode(mode)} className={colorBlindMode === mode ? 'bg-zion-cyan text-white' : 'border-zion-cyan/30 text-zion-cyan"}>{mode.charAt(0).toUpperCase() + mode.slice(1)}"
                      </Button>))}'"
                  </div>"
