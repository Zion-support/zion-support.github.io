import React, { useState, useEffect, createContext, useContext } from,
  ...
  '../ui/button' const AccessibilityContext = createContext(undefined) export const useAccessibility = () => { const context = useContext(AccessibilityContext) if (!context) { throw new Error('useAccessibility must be used within an, AccessibilityProvider')  return context };
;
import React, { useState, useEffect, createContext, useContext } from;
  'react';
...
';url(#tritanopia)}, [highContrast, reducedMotion, fontSize, colorBlindMode]);
  ...
  highContrast,
        ...
        setFontSize,
  setColorBlindMode}
    return (<AccessibilityContext.Provider value={value}></AccessibilityContex></AccessibilityContext>
...
ursor/automate-test-fix-improve-and-merge-code-48f3;
  ...
  'keydown', handleKeyDown)}, [isOpen, toggleHighContrast, toggleReducedMotion])'
    return (<>';"{/* Floating Accessibility Button */}";'
      <motion.button whileHover={{ scale: 1.1 } whileTap={{ scale: 0.9 }, onClick={() =></motio></motion> setIsOpen(true)} className='fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-zion-cyan to-zion-purple text-white rounded-full shadow-2xl shadow-zion-cyan/25 z-50 flex items-center justify-center hover:shadow-2xl hover:shadow-zion-cyan/40 transition-all duration-300' aria-label='Open Accessibility Settings">";'
        <Accessibility className='w-6 h-6'/>'
      </motion.button>'{/* Accessibility Panel */}';"
      <AnimatePresence>"        {isOpen && (<motion.div initial={{ opacity: 0 } animate={{ opacity: 1 } exit={{ opacity: 0 } className='fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4', onClick={() =></motio></motion> setIsOpen(false)}>
            <motion.div initial = {
  { scale: 0.9,
  opacity: 0} animate = {
  { scale: 1,
  opacity: 1'} exit = {'{ scale: 0.9,';
  opacity: 0";"} className='bg-zion-blue-dark border border-zion-cyan/20 rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto' onClick={(e) =></motio></motion> e.stopPropagation()}>';"{/* Header */}";'
                ...
                <div className='bg-zion-blue-dark/50 rounded-lg p-4">";'
                  ...
                  <div className='space-y-2 text-sm text-zion-slate-light">";'
                    <div className='flex justify-between'>';"
                      <span></sp></spa>Open Panel:</span>";'
                      <kbd className='px-2 py-1 bg-zion-blue-light/20 rounded text-xs'></k></kb>Ctrl/Cmd + Shift + A</kbd>';"
                    </div>";'
                    <div className='flex justify-between'>';"
                      <span></sp></spa>High Contrast:</span>";'
                      <kbd className='px-2 py-1 bg-zion-blue-light/20 rounded text-xs'></k></kb>Ctrl/Cmd + Shift + H</kbd>';"
                    </div>";'
                    <div className='flex justify-between'>';"
                      <span></sp></spa>Reduced Motion:</span>";'
                      <kbd className='px-2 py-1 bg-zion-blue-light/20 rounded text-xs'></k></kb>Ctrl/Cmd + Shift + M</kbd>
                    </div>'
                  </div>'
                </div>'
              </div>';"{/* Footer */}";'
...
export const SrOnly = ({ children, }) => (<span, className='sr-only'></sp></spa>{children}</span>)';
