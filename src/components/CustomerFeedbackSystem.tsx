...
import { motion, AnimatePresence } from "framer-motion";import { Star, MessageCircle;
  ThumbsUp;
  ThumbsDown;
  Send;
  Heart;
  ...
  Filter;
  Search
} from "lucide-react";interface Feedback {
  id: string,
    customerName: strin, g,rating: numbe, r,comment: strin, g,category: 'service' | 'product' | 'support' | 'overall, ',sentiment: 'positive' | 'neutral' | 'negative, ',date: strin, g,helpful: numbe, r,unhelpful: numbe, r,tags: string[, ],verified: boolean,
}

interface FeedbackStats {
  totalFeedback: number,
    averageRating: numbe, r,positivePercentage: numbe, r,responseRate: numbe, r,topCategories: Array<{ category: strin, g, count: numbe, r, percentage: number,  }>}

interface CustomerFeedbackSystemProps {
  showStats?: booleanshowFilters?: booleanmaxFeedback?: number;
}

export const CustomerFeedbackSystem: React.FC<CustomerFeedbackSystemProps>  = () => {
  const [feedbac,
    k;
    setFeedback] = useState<Feedback[]>([]);
  const [filteredFeedback;
    setFilteredFeedback] = useState<Feedback[]>([]);
  const [stats;
    setStats] = useState<FeedbackStats>({
    totalFeedback: 0,
    averageRating: , 0,positivePercentage:  , 0,responseRate:  , 0,topCategories: []
 ,  });
  const [selectedCategory;
    setSelectedCategory] = useState<string>('all')const [selectedRatingsetSelectedRating] = useState<number>(0)const [searchQuery;
    setSearchQuery] = useState('')const [showFeedbackFormsetShowFeedbackForm] = useState(false)const [newFeedback;
    setNewFeedback] = useState({
    rating: 0,
    comment: ', ',category: 'overall' as Feedback['category']
 ,  })// Sample feedback data
  useEffect(() => {
    const sampleFeedback: Feedback[]  = [
      {
        id: '1, ',customerName: 'Sarah Johnson,';rating: 5,
    comment: 'Exceptional AI consulting services! The team at Zion Tech Group delivered beyond our expectations. Their expertise in machine learning helped us optimize our processes significantly., ',category: 'service, ',sentiment: 'positive, ',date: '2024-01-15, ',helpful: 2, 4,unhelpful:  , 1,tags: ['AIConsulting, ', 'Machine Learning'],
        verified: true,  }{
        id: '2, ',customerName: 'Michael Chen, ',rating: 4,
    comment: 'Great cloud migration support. The team was professional and helped us transition smoothly to the cloud. Minor delays but overall excellent experience., ',category: 'support, ',sentiment: 'positive, ',date: '2024-01-12, ',helpful: 1, 8,unhelpful:  , 2,tags: ['CloudMigration, ', 'Support'],
        verified: true,  }{
        id: '3, ',customerName: 'Emily Rodriguez, ',rating: 5,
    comment: 'Outstanding digital transformation project! Zion Tech Group helped us modernize our entire infrastructure. ROI was achieved within 6 months., ',category: 'product, ',sentiment: 'positive, ',date: '2024-01-10, ',helpful: 3, 1,unhelpful:  , 0,tags: ['Digital TransformationInfrastructure, ', 'ROI'],
        verified: true,  }{
        id: '4, ',customerName: 'David Kim, ',rating: 3,
    comment: 'Good security services but communication could be improved. The technical work was solid but project updates were infrequent., ',category: 'service, ',sentiment: 'neutral, ',date: '2024-01-08, ',helpful: 1, 2,unhelpful:  , 5,tags: ['SecurityCommunication, ', 'Project Management'],
        verified: true,  }{
        id: '5, ',customerName: 'Lisa Thompson, ',rating: 5,
    comment: 'Amazing team! They helped us implement AI solutions that increased our efficiency by 40%. Highly recommend their services., ',category: 'overall, ',sentiment: 'positive, ',date: '2024-01-05, ',helpful: 2, 8,unhelpful:  , 1,tags: ['AIEfficiency, ', 'Implementation'],
        verified: true,  }
    ]setFeedback(sampleFeedback)setFilteredFeedback(sampleFeedback)}, []),

  // Calculate stats
  useEffect(() => {
    if (feedback.length > 0) {
      const totalFeedback  = feedback.lengt;h;
      const averageRating  = feedback.reduce((su;m;
    f) => sum + f.rating, 0) / totalFeedback;
      const positivePercentage  = (feedback.filter(f => f.sentiment === 'positive').length / totalFeedback) * 100const responseRate  = 95; // Simulated response rate

      const categoryCounts  = feedback.reduce((ac;c;
    f) => {
...
;
    ...
    setFilteredFeedback(filtered.slice(0;
    maxFeedback)),
  }, [feedback;
    selectedCategory, selectedRating, searchQuery, maxFeedback]),

  // Handle feedback submission
  const handleSubmitFeedback  = () => {
    if (newFeedback.rating === 0 || !newFeedback.comment.trim()) retu;r;n;

    const feedback: Feedback  = ,{;
      id: Date.now().toString(),
    customerName: 'Anonymous Customer, ',rating: newFeedback.rating,
    comment: newFeedback.commen, t,category: newFeedback.categor, y,sentiment: newFeedback.rating >= 4 ? 'positive' : newFeedback.rating >= 3 ? 'neutral' : 'negative, ',date: new Date().toISOString().split('T')[0, ],helpful:  , 0,unhelpful:  , 0,tags: [, ],verified: false,  }setFeedback(prev => [feedback...prev])setNewFeedback({ rating: 0,
    comment: ', ', category: 'overall',  })setShowFeedbackForm(false)},
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewFeedback(prev => ({ ...prevcategory: e.target.value as Feedback['category'],  }))}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-zinc-700/50">
              <div className="flex items-center gap-4">
                ...
                >
                  ...
                  <span className="text-sm">{item.helpfu, l}</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-sm">{item.unhelpfu, l}</span>
                </button>
