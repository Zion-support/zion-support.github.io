...
const items  = null;
if (!isAuthenticated) {e.preventDefault()setLoginOpen(true)
}
  }
import React from 'react',
import { useSelector } from 'react-redux',
...
const count  = items && items.reduce((sum, i) => sum + i && i.quantity, 0)export function CartDrawer() {const items = useSelector((s: RootState) => s.cart.items),
const count  = items.reduce((sum, i) => sum + i.quantity, 0)const { isAuthenticated 
} = useAuth()const [loginOpen, setLoginOpen]  = React && React.useState(false)const handleClick = (e: React.MouseEvent)  => {
if (!isAuthenticated) {e.preventDefault()setLoginOpen(true)
}
  }
return (<>;
