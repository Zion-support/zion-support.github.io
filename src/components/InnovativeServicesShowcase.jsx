import React,{useState} from 'react'; import {INNOVATIVE_MICRO_SAAS_SERVICES_2025} from "../data/innovativeMicroSaasServices2025";'; const InnovativeServicesShowcase = (props) => { const [activeTab,setActiveTab] = useState('microsaas'); const [selectedService,setSelectedService] = useState(null); const tabs = [ { id: 'microsaas',name: 'Micro SAAS Services',count: INNOVATIVE_MICRO_SAAS_SERVICES_2025.length },{id: 'it',name: 'IT Services',count: innovativeITServices2025.length},{id: 'ai',name: 'AI Services',count: innovativeAIServices2025.length}; ]; const getServicesByTab = (props) => {} switch (activeTab) {}; ' case 'microsaas': any;';'; return INNOVATIVE_MICRO_SAAS_SERVICES_2025;' case 'it': any;';'; return innovativeITServices2025;' case 'ai': any; return innovativeAIServices2025; default: any; return []} }; const handleServiceClick = (props) => {setSelectedService(service)}; const closeModal = (props) => {setSelectedService(null)};" return ( <div className="min-h-screen bg-white"> {}" <section className="py-20 px-4 sm:px-6 lg:px-8">" <div className="max-w-7xl mx-auto text-center">" <h1 className="text-4xl md:text-6xl font-bold mb-6"> Innovative" <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">" {" "}Services 2025 </span> </h1>" <p className="text-xl text-gray-300 max-w-3xl mx-auto mb-8"> Cutting-edge technology solutions designed to transform your business and drive innovation </p>" <div className="flex flex-wrap justify-center gap-4">" <div className="bg-white/10 backdrop-blur-lg rounded-lg px-6 py-3">" <span className="text-2xl font-bold text-blue-400"> {INNOVATIVE_MICRO_SAAS_SERVICES_2025.length + innovativeITServices2025.length + innovativeAIServices2025.length} </span>" <p className="text-sm text-gray-300">Total Services</p> </div>" <div className="bg-white/10 backdrop-blur-lg rounded-lg px-6 py-3">" <span className="text-2xl font-bold text-green-400">24/7</span>" <p className="text-sm text-gray-300">Support Available</p> </div>" <div className="bg-white/10 backdrop-blur-lg rounded-lg px-6 py-3">" <span className="text-2xl font-bold text-purple-400">99.9%</span>" <p className="text-sm text-gray-300">Uptime SLA</p> </div> </div> </div> </section> {}" <section className="px-4 sm:px-6 lg:px-8 mb-12">" <div className="max-w-7xl mx-auto">"; <divclassName="flex flex-wrap justify-center gap-2">'; {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-6 py-3 rounded-lg font-semibold transition-all duration-300 ${activeTab === tab.id' ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white"` : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}> {tab.name} ({tab.count}) </button>))} </div> </div> </section> {}" <section className="px-4 sm:px-6 lg:px-8 pb-20">" <div className="max-w-7xl mx-auto">" <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">" {getServicesByTab().map((service,index) => (<div key={service.id} onClick={() => handleServiceClick(service)} className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 hover:border-white/40 transition-all duration-300 hover:transform hover:scale-105 cursor-pointer group">" <div className="flex justify-between items-start mb-4">"; <divclassName="text-3xl">'; {activeTab === 'microsaas' && ''} {activeTab === 'it' && '⚡'} {activeTab === 'ai' && '🤖'} </div>"; <div className="text-right">";'; <div className="text-lg font-bold text-blue-400">'` {activeTab === 'microsaas' && `$${service.price}`}'` {activeTab === 'it' && `$${service.hourlyRate}/hr`}'` {activeTab === 'ai' && `$${service.price}`} </div>"; <divclassName="text-sm text-gray-400">'; {activeTab === 'microsaas' && service.pricingModel} {activeTab === 'it' && 'Professional'} {activeTab === 'ai' && service.pricingModel} </div> </div> </div> import React {useState} from "react";,"}
});,"})","}
});,"})" ;,"}
});,"})" const InnovativeServicesShowcase = (props) => {"};);,"})" const [activeTab,setActiveTab] = useState("microsaas");,"}
});,"})" const [selectedService,setSelectedService] = useState(null);,"}
});,"})" const tabs = [],"};);,"})" const getServicesByTab = (props) => {}"};);,"})" switch (activeTab) {}"}
});,"})" ";,"}
});,"})" case "microsaas": any,"}
