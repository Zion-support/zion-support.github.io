  ...
  return (
    <div className="fixed top-4 left-4 bg-white/90 backdrop-blur-sm border border-gray-300 rounded-lg p-4 shadow-lg z-40 max-w-sm">;
      <div className="flex items-center justify-between mb-3">;
        <h3 className="text-sm font-semibold text-gray-900">Security Monitor</[^>]*>
      ...
      <div className="bg-gray-50 p-2 rounded mb-3">;
        <h4 className="font-medium text-gray-700 mb-2 text-xs">Statistics</[^>]*>
        <div className="grid grid-cols-2 gap-2 text-xs">;
          <div>Blocked: {blockedRequests}</[^>]*>
          <div>Allowed: {allowedRequests}</[^>]*>
          <div>Events: {securityEvents.length}</[^>]*>
      ...
      <div className="bg-gray-50 p-2 rounded mb-3">;
        <h4 className="font-medium text-gray-700 mb-2 text-xs">Configuration</[^>]*>
          ...
          {Object.entries(config).map(([key, value]) => (;
            <label key={key} className="flex items-center gap-2 cursor-pointer">;
              <input;
                ...
                className="[^"]*"
              />;
              <span className="text-gray-600">{key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</[^>]*>
            </[^>]*>
          ))};
        </[^>]*>
      </[^>]*>
      ...
      <div className="bg-gray-50 p-2 rounded">;
        <h4 className="font-medium text-gray-700 mb-2 text-xs">Recent Events</[^>]*>
