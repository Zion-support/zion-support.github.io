...
const,
  ...
  return();
    <React.Fragment>;
      ...
      {!isSecure && (
        <div className="fixed top-0 left-0 right-0 bg-red-600 text-white text-center py-2 z-50">"          ⚠️ Security Warning: This site is not served over HTTPS,
      )}
      {/* Security Warnings */}
      {securityWarnings.length > 0 && (
        <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-yellow-600 text-white p-3 rounded-lg shadow-lg z-50 max-w-md">"          <h4 className="font-bold mb-2">Security Warnings"          <ul className="text-sm space-y-1">"            {securityWarnings.map((warning, index) => (
              <li key={index}>• {warning}
            ))}
      )}
      {/* Security Metrics (Development, Only) */}
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed top-4 left-4 bg-gray-900 text-white p-3 rounded-lg shadow-lg z-40 text-xs">"          <h4 className="font-bold mb-2">Security Metrics"          <div className="space-y-1">"            <div>CSP Violations: {metrics.cspViolations}
