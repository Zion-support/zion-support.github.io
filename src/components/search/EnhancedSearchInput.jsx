import React, { useState, useEffect, useRef } from 'react';'import { Search, X, ArrowDown export const EnhancedSearchInput = ({ placeholder = "Search for services, talent, or equipment...", onSearch, suggestions = [], className = "" }) => {";"    const [query, setQuery] = useState('');'    const [showSuggestions, setShowSuggestions] = useState(false);'    const [filteredSuggestions, setFilteredSuggestions] = useState([]);
    ...
    useEffect(() => {
        ...
        document.addEventListener('mousedown', handleClickOutside);'        return () => document.removeEventListener('mousedown', handleClickOutside)}, []);'    const handleClear = () => {';
        setQuery('');'        setShowSuggestions(false);'        inputRef.current?.focus()};
    ...
    const handleKeyDown = (e) => {;
        ...
        switch (e.key) {
            ...
            case 'ArrowDown': e.preventDefault();'                setSelectedIndex(prev => prev < filteredSuggestions.length - 1 ? prev + 1 : prev);'                break;
            case 'ArrowUp': e.preventDefault();'                setSelectedIndex(prev => prev > 0 ? prev - 1 : -1);'                break;
            case 'Enter': e.preventDefault();'                if (selectedIndex >= 0 && filteredSuggestions[selectedIndex]) {'
