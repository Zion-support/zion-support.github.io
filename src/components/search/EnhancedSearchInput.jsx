...
const handleClickOutside = (event) => {if (inputRef.current && !inputRef.current.contains(event.target) &&;
  ...
  return;switch (e.key) {case 'ArrowDown': e.preventDefault()setHighlightedIndex(prev =>;'
      ...
      case 'ArrowUp': e.preventDefault()setHighlightedIndex(prev => { return prev > 0 ? prev - 1 : -1)break; }'
...
return (<div className="relative" ref={inputRef}>;"
  ...
  return (<button;
                    ...
                    className={`w-full flex items-center px-3 py-2 text-sm transition-colors ${index === highlightedIndex;`                        ? 'text-white bg-zion-purple/20';'
