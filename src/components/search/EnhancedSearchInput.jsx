...
const [isOpen, setIsOpen] = useState(false)const [filteredSuggestions, setFilteredSuggestions] = useState([])const [highlightedIndex, setHighlightedIndex] = useState(-1)const inputRef = useRef(null)const suggestionsRef  = useRef(null)useEffect(()  => {
if (value.trim()) {const filtered = suggestions.filter(suggestion =>;
        suggestion.toLowerCase().includes(value.toLowerCase()))setFilteredSuggestions(filtered.slice(0, 8))setIsOpen(true)

} else {setFilteredSuggestions([])setIsOpen(false)}
...
document.addEventListener('mousedown', handleClickOutside)return () => document.removeEventListener('mousedown', handleClickOutside)}, [])const handleKeyDown = (e)  => {
if (!isOpen) return;
switch (e.key) {case 'ArrowDown': e.preventDefault()setHighlightedIndex(prev =>;
...
onSelectSuggestion?.(suggestion)setIsOpen(false)setHighlightedIndex(-1)
}
const handleClear = ()  => {
onChange('')setIsOpen(false)setHighlightedIndex(-1)inputRef.current?.focus()
}
const getSuggestionIcon = (suggestion)  => {
const lowerSuggestion = suggestion.toLowerCase()if (lowerSuggestion.includes('service') || lowerSuggestion.includes('solution')) {return Briefcase;
...
const Icon = getSuggestionIcon(suggestion)return (<button;
                    key={suggestion
}
