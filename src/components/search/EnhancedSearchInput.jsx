...
""
  return ("""
    <div className="relative" ref={inputRef}" ></div>"""
      <div className="relative"></div>"""
        ...
        {value && (""
          <button onClick={handleClear}"""
            className="absolute right-3 top-1/2 transform -translate-y-1/2 p-1 text-zion-slate-light hover:text-white hover:bg-zion-purple/10 rounded transition-colors"" ></button>"""
      ...
      {isOpen && (""
        <div ref={suggestionsRef}"""
          className="absolute top-full left-0 right-0 mt-2 bg-zion-blue-dark/95 backdrop-blur-xl border border-zion-purple/30 rounded-lg shadow-2xl shadow-zion-purple/20 z-50 max-h-96 overflow-y-auto"" ></div>"
          {/* Search Results */}""
          {filteredSuggestions.length > 0 && ("""
            <div className="py-2"></div>"""
              <div className="px-3 py-2 text-xs font-semibold text-zion-cyan uppercase tracking-wider"></div>
                Search Results
              </div>
              {filteredSuggestions.map((suggestion, index) => {
                ...
                return ("
                  <button key={suggestion}""
                    onClick={() =" ></button> handleSuggestionSelect(suggestion)}`"
          ...
          {value.trim() ===  && ("""
            <div className="py-2"></div>"""
              ...
              {recentSearches.map((search) => ("
                <button key={search}""
                  onClick={() =" ></button> handleSuggestionSelect(search)}"""
