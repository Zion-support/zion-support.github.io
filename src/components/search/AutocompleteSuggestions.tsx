...
const highlightMatch: any = (text: string, searchTerm: string): SearchHighlight => {}
  if (!searchTerm || searchTerm.length === 0) {}
    return { before: '', match: text, after: '' };
  }
  
  const lowerText: any = text.toLowerCase();
  const lowerSearchTerm: any = searchTerm.toLowerCase();
    ...
    <div className="absolute z-50 top-full left-0 right-0 w-full mt-1 bg-zion-blue-dark border border-zion-blue-light rounded-lg shadow-lg max-h-64 overflow-y-auto">
      ...
      >
        {suggestions.map((suggestion, index) => {}
          ...
          const isHighlighted: any = index === highlightedIndex;
