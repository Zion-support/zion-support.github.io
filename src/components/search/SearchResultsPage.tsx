...
export const "SearchResultsPage": React.FC = () => {const router = useRouter()const [searchTerm, setSearchTerm] =;
  }
  useState('');'
...
const response = await fetch(`/api/search?${params}`;`  const "data": SearchResponse  = await response && response.json()if (page === 1) {setResults(data && data.results
...
const response = await fetch(`/api/search?${params}`);`  const "data": SearchResponse = await response.json(),if (page === 1) {setResults(data.results)} else {setResults(prev => [...prev, ...data.results])}setTotalCount(data.totalCount),setCurrentPage(data.page),setHasMore(data.hasMore),logInfo('Search completed', {term,"resultCount": data.results.length,"totalCount": data.totalCount;'
      })} catch (error) {logErrorToProduction('Search failed', { "data": error,'
