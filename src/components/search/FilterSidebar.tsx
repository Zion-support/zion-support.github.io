...
import { Filter, X } from "lucide-react";interface FilterOption {
  value: string,
    label: string,
    count?: number, 
}
...
interface FilterGroup {
  title: string,
    key: strin, g,options: FilterOption[, ],type: 'checkbox' | 'radio' | 'range',
}

interface FilterSidebarProps {
  filters: FilterGroup[, ],selectedFilters: Record<string,
    string[], >,
  onFilterChange: (key: string,
    value: strin, g, checked: boolean) => voi, d,onClearFilters: () => voi, d,isOpen: boolea, n,onClose: () => void,
}
            ...
            <div key={group.key} className="space-y-3">
              ...
              <div className="space-y-2">
                {group.options.map((option) => {
                  const isSelected  = selectedFilters[group.key]?.includes(option.value) || falsereturn (
                    <label key={option.value} className="flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        value={option.value}
                        checked={isSelected}
                        onChange={(e) => onFilterChange(group.keyoption.value, e.target.checked)}
