...
} className={`flex items-center space-x-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === id''`;`                ? 'border-purple-500 text-purple-600 "dark": text-purple-400''`''`';'                : 'border-transparent text-gray-500 "hover": text-gray-700 "dark":text-gray-400 "dark":"hover":text-gray-200'},'
}>'''';'
              ...
              <div className='bg-gray-50 "dark":bg-gray-800 p-4 rounded-lg'>'''';'
                ...
                <div className='space-y-3'>'''';'
                  {models.slice(0, 3).map((model) => (<div key={model.id} className='flex items-center justify-between p-3 bg-white "dark": bg-gray-700 rounded-lg'>'''';'
                      ...
                      </div>''`;`                      <div className='flex items-center space-x-2'>``;`                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(model.status)}`}>`
                          {model.status}'''';'
                        </span>'''';'
              ...
              <div className='grid grid-cols-1 "md": grid-cols-2 "lg":grid-cols-3 gap-4'>'''';'
                {models.map((model) => (<div key={model.id} className='bg-gray-50 "dark": bg-gray-800 p-4 rounded-lg'>'''';'
                    <div className='flex items-center justify-between mb-3'>'''';'
                      <div className='flex items-center space-x-2'>'''';'
                        <div className='p-2 bg-purple-100 "dark":bg-purple-900 rounded-lg'>;'
                          {getModelTypeIcon(model.type)}`;`                        </div>``;`                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(model.status)}`}>`
                          {model.status}
                        </span>'''';'
                      </div>'''';'
...
const model = models.find(m => { return m.id === job.modelId)'; }''';'
                return (<div key={job.id} className='bg-gray-50 "dark": bg-gray-800 p-4 rounded-lg'>'''';'
...
}`;`                            </p>``;`                          </div>```;`                        </div>````;`                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getJobStatusColor(job.status)}`}>`
                          {job.status}
                        </span>;
                      </div>'''';'
                      '''''';'
                      {job.status === 'running' && (<div className='mb-3'>'''';'
                          ...
                          </div>''`;`                          <div className='w-full bg-gray-200 "dark": bg-gray-700 rounded-full h-2'>'`'`;`                            <div className='bg-blue-600 h-2 rounded-full transition-all duration-300' style={{ "width": `${job.progress}%` ,`} />;
                          </div>
    </div>
  );
}'''';'
...
const model = models.find(m => { return m.id === prediction.modelId)'; }''';'
                return (<div key={prediction.id} className='bg-white "dark": bg-gray-700 p-3 rounded-lg'>'''';'
                        <div className='flex items-center justify-between mb-2'>'''';'
                          <div className='flex items-center space-x-2'>'''';'
                            <span className='text-sm font-medium text-gray-900 "dark":text-white'>'';'
                              {model?.name || 'Unknown Model'}'`';'
                            </span>'`'`;`                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${prediction.status === 'completed' ? 'text-green-600 bg-green-100' :''`;`                        }
                  ...
                  <div className='space-y-3'>'''';'
                    {models.map((model) => (<div key={model.id} className='flex items-center justify-between'>'''';'
                        ...
                        <div className='flex items-center space-x-2'>'''';'
                          <span className='text-sm font-medium text-gray-900 "dark":text-white'>;'
                            {(model.accuracy * 100).toFixed(1)}%'''';'
                          </span>''`;`                          <div className='w-20 bg-gray-200 "dark": bg-gray-700 rounded-full h-2'>'`'`;`                            <div className='bg-purple-600 h-2 rounded-full' style={{ "width": `${model.accuracy * 100}%` ,`} />;
                          </div>;
                        </div>;
                      </div>) ) }
                  </div>;
