...
'
  ...
  const navigationItems = ['
    ...
    {'
      ...
      dropdown: ['
        { nam,
    e: 'AI Solutions', href: '/ai-services', icon: Brain, description: 'Cutting-edge AI services' },
  ...
  const contactInfo = ['
    ...
    l:+13024640950' },
    { icon: Mail, text: 'kleber@ziontechgroup.com', href: 'mailt,
                        ...
                        >"
                          <div className="p-4">
                            {item.dropdown.map((dropdownItem) => (
                              ...
                              >"
                                <div className="flex-shrink-0 w-10 h-10 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-lg flex items-center justify-center group-hover:from-cyan-400/30 group-hove,
    r:to-blue-400/30 transition-all duration-200">"
...
`
          <div className="hidden lg: block">
            <Link"
              to="/contact""
              className="px-6 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-medium rounded-lg hover:from-cyan-600 hover:to-blue-700 transition-all duration-300 transform hove,
    ...
    r:shadow-cyan-500/25"
          ...
          <button
            ...
            className="lg: hidden p-2 rounded-lg text-gray-300 hove,
    r:text-cyan-300 hove,
    r:bg-gray-800/50 transition-colors duration-200""
                  ...
                  <div key={item.name}>
                    {item.dropdown ? (
                      <div>
                        <button
                          onClick={() => toggleDropdown(item.name)}"
                          className="flex items-center justify-between w-full text-left px-3 py-2 text-gray-300 hover:text-cyan-300 hove,
    r:bg-gray-800/50 rounded-lg transition-colors duration-200"
                        >
                          <span>{item.name}</span>"
                          <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${`
                            activeDropdown === item.name ? 'rotate-180' : '''
                            ...
                            >
                              {item.dropdown.map((dropdownItem) => (
                                <Link
                                  ...
                                  className="flex items-center space-x-3 px-3 py-2 text-sm text-gray-400 hover:text-cyan-300 hove,
    r:bg-gray-800/50 rounded-lg transition-colors duration-200"
                                >"
                      ...
                      <Link
                        ...
                        className={`block px-3 py-2 rounded-lg transition-colors duration-200 ${`
                          location.pathname === item.href ? 'text-cyan-400 bg-cyan-400/10' : 'text-gray-300 hover:text-cyan-300 hove,
    r:bg-gray-800/50''
                        }`}
                      >
                        {item.name}
...
`
              <div className="pt-4 border-t border-gray-700">
                <Link"
                  to="/contact"
                  onClick={closeMobileMenu}"
                  className="block w-full px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-medium text-center rounded-lg hover: from-cyan-600 hover:to-blue-700 transition-all duration-300 transform hove,
    r:scale-105 shadow-lg hove,
    r:shadow-cyan-500/25"
                >
                  Get Started Today
