  ...
  return (, <motion.aside, initial={false}, animate={{ x: isOpen ? 0 : -320 }},
      ...
      className={`fixed left-0 top-0 h-full w-80 bg-white shadow-xl z-40 overflow-y-auto ${className}`}, >, <div className="p-6">, {/* Header */}, <div className="flex items-center justify-between mb-8">, <Link href="/" className="flex items-center space-x-2" onClick={() => onClose?.()}>;
            <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg flex items-center justify-center">;
              <Brain className="w-5 h-5 text-white" />;
            </div>;
            <span className="text-lg font-bold text-gray-900">Zion Tech</span>;
          </Link>;
          {onClose && (, <button, onClick={onClose}, className="p-2 rounded-lg text-gray-500 hover: bg-gray-100 transition-colors",
            >, ×, </div>, {/* Quick Links */}, <div className="mb-8">, <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Quick Actions</h3>, <div className="space-y-2">, {quickLinks.map((link) => {;
              const IconComponent = link.icon;
              return (, <button, key={link.name}, onClick={() => handleLinkClick(link.href)};
          ...
          {navigationSections.map((section) => {;
            ...
            return (, <div key={section.id}>, <button, onClick={() => toggleSection(section.id)};
                ...
                >;
                  ...
                  {isExpanded ? (, </button>, {isExpanded && (, <motion.div, initial={{ opacity: 0, height: 0 }},
