...
return (<aside className="hidden lg:block fixed left-0 top-16 bottom-0 w-80 bg-gray-100 border-r">;
      <div className="p-6">;
...
title: 'Our Services',
icon: Settings,
items: [,{ name: 'AI Services', href: '/ai-services', icon: Brain, description: 'Cutting-edge AI solutions' },{ name: 'IT Services', href: '/it-services', icon: Code, description: 'Comprehensive IT solutions' },{ name: 'Micro SaaS', href: '/micro-saas', icon: Cloud, description: 'Scalable SaaS applications' },{ name: 'Cybersecurity', href: '/cybersecurity', icon: Shield, description: 'Advanced security solutions' },{ name: 'Cloud Solutions', href: '/cloud-solutions', icon: Cloud, description: 'Cloud infrastructure & services' },{ name: 'Database Solutions', href: '/database-solutions', icon: Database, description: 'Database management & optimization' }];
      ];
      ];
    },{,
id: 'solutions',
...
,
if (onClose) onClose(),
router.push(href)router.push(href)router.push(href)
},
return (,<motion.aside;
}
...
return (<motion.aside; initial = {false}
animate = {{ x: isOpen ? 0: -320 }}
transition = {{ duration: 0.3, ease: 'easeInOut' }}
className = {`fixed left-0 top-0 h-full w-80 bg-white shadow-xl z-40 overflow-y-auto ${className}`}>; <div className = "p-6">; {/* Header */}<div className = "flex items-center justify-between mb-8">; <Link href = "/" className = "flex items-center space-x-2" onClick = {() = > onClose?.()}>; <div className = "w-8 h-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg flex items-center justify-center">; <Brain className = "w-5 h-5 text-white" />; </div>; <span className = "text-lg font-bold text-gray-900">Zion Tech</span>; </Link>; {onClose && (<button; onClick = {onClose}
className = "p-2 rounded-lg text-gray-500 hover: bg-gray-100 transition-colors">, ×, </button>; )}</div>; {/* Quick Links */}<div className = "mb-8">; <h3 className = "text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Quick Actions</h3>; <div className = "space-y-2">; {quickLinks.map((link) = > {const IconComponent = link.icon; return (<button; key = {link.name}
onClick = {() = > handleLinkClick(link.href)}
className = {`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-left transition-colors ${link.highlight; ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:shadow-lg';: 'text-gray-700 hover:bg-gray-100'}`}>; <IconComponent className = "w-5 h-5" />; <span className = "font-medium">{link.name}</span>; </button>; )})}</div>; </div>; {/* Navigation Sections */}<div className = "space-y-6">; {navigationSections.map((section) = > {const isExpanded = expandedSections.includes(section.id)const IconComponent  = section.icon;
return (<div key = {section.id}>; <button; onClick = {() = > toggleSection(section.id)}
className = "w-full flex items-center justify-between px-4 py-3 text-left text-gray-700 hover: bg-gray-100 rounded-lg transition-colors">, <div className = "flex items-center space-x-3">, <IconComponent className = "w-5 h-5" />; <span className = "font-medium">{section.title}</span>; </div>; {isExpanded ? (<ChevronDown className = "w-4 h-4" />; ): (<ChevronRight className = "w-4 h-4" />; )}</button>; {isExpanded && (<motion.div; initial = {{ opacity: 0, height: 0 }}
animate = {{ opacity: 1, height: 'auto' }}
exit = {{ opacity: 0, height: 0 }}
className = "mt-2 space-y-1">; {section.items.map((item) = > {const ItemIconComponent = item.icon; return (<button; key = {item.name}
...
animate={{ x: isOpen ? 0 : -320 }}
transition={{ duration: 0.3, ease: 'easeInOut' }}
className={`fixed left-0 top-0 h-full w-80 bg-white shadow-xl z-40 overflow-y-auto ${className}`}>,<div className="p-6">,{/* Header */}<div className="flex items-center justify-between mb-8">,<Link href="/" className="flex items-center space-x-2" onClick={() => onClose?.()}>,<div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg flex items-center justify-center">,<Brain className="w-5 h-5 text-white" />,</div>,<span className="text-lg font-bold text-gray-900">Zion Tech</span>,</Link>,{onClose && (,<button,
onClick={onClose}
className="p-2 rounded-lg text-gray-500 hover: bg-gray-100 transition-colors",>,×,</button>,)}</div>,{/* Quick Links */}<div className="mb-8">,<h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Quick Actions</h3>,<div className="space-y-2">,{quickLinks.map((link)  => {
,
...
link.highlight,? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white hover: shadow-lg',: 'text-gray-700 hover: bg-gray-100';
                      : 'text-gray-700 hover: bg-gray-100';
                      : 'text-gray-700 hover: bg-gray-100';
                  }`}>,<IconComponent className="w-5 h-5" />,<span className="font-medium">{link.name}</span>,</button>,)))})}</div>,</div>,{/* Navigation Sections */}<div className="space-y-6">,{navigationSections.map((section)  => {
,
...
className="w-full flex items-center justify-between px-4 py-3 text-left text-gray-700 hover: bg-gray-100 rounded-lg transition-colors",>,<div className="flex items-center space-x-3">,<IconComponent className="w-5 h-5" />,<span className="font-medium">{section.title}</span>,</div>,{isExpanded ? (,<ChevronDown className="w-4 h-4" />,) : (,<ChevronRight className="w-4 h-4" />,)}</button>,{isExpanded && (,<motion.div,
...
onClick={() => handleLinkClick(link.href)}
className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-left transition-colors ${link.highlight;
...
const isExpanded = expandedSections.includes(section.id)const IconComponent  = section.icon;
return (<div key={section.id
}>;
                <button;
                  onClick={() => toggleSection(section.id)}
className="w-full flex items-center justify-between px-4 py-3 text-left text-gray-700 hover: bg-gray-100 rounded-lg transition-colors", >,<div className="flex items-center space-x-3">;
                    <IconComponent className="w-5 h-5" />;
