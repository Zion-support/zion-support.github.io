...
interface EnhancedAnalyticsProps {
  ...
  useEffect ( () => {
    ...
    // Initialize Google Analytics (if tracking ID provided) if (trackingId && trackingId !== 'G - XXXXXXXXXX') {
...
function gtag (...args: any[]) {
  ...
  useEffect ( () => {
    ...
    // Listen for route changes (for SPA) window.addEventListener ('popstate', handleRouteChange) ;
  ...
  const trackEvent = useCallback ( (eventName: string, parameters: Record < string, any> = {}) => {
      ...
      // Send to analytics endpoint (if configured) if (process.env.REACT_APP_ANALYTICS_ENDPOINT) {
