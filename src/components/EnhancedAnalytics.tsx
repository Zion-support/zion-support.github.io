            ...
            <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">{analyticsData.pageViews}</div>
                  <div className="text-xs text-slate-600 dark:text-slate-400">Page Views</div>
                </div>
                <div className="text-center p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">{analyticsData.uniqueVisitors}</div>
                  <div className="text-xs text-slate-600 dark:text-slate-400">Unique Visitors</div>
                </div>
              </div>

              {/* Device Types */}
              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Device Types</h4>
              ...
              {/* User Engagement */}
              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">User Engagement</h4>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Scroll Depth</span>
                    <span className="text-sm font-medium">{analyticsData.userEngagement.scrollDepth}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Time on Page</span>
                    <span className="text-sm font-medium">{Math.round(analyticsData.userEngagement.timeOnPage)}s</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Interactions</span>
                    <span className="text-sm font-medium">{analyticsData.userEngagement.interactions}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Performance */}
              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Performance</h4>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Load Time</span>
                    <span className="text-sm font-medium">{analyticsData.performance.loadTime}ms</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">FCP</span>
                    <span className={`text-sm font-medium ${
                      analyticsData.performance.coreWebVitals.fcp <= 1800 ? 'text-green-600' : 'text-yellow-600'
                    }`}>
                      {analyticsData.performance.coreWebVitals.fcp}ms
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">LCP</span>
                    <span className={`text-sm font-medium ${
                      analyticsData.performance.coreWebVitals.lcp <= 2500 ? 'text-green-600' : 'text-yellow-600'
                    }`}>
                  ...
                  {analyticsData.topPages.map((page, index) => (
                    <div key={page.path} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-700 rounded">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-slate-700 dark:text-slate-300">{page.title}</span>
                      </div>
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{page.views}</span>
                    </div>
                  ))}
                </div>
              </div>
