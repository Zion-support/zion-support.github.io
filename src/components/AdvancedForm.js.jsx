...
''"
;'"'"
            case 'email': return <Mail className='w-4 h-4'/" ></Mail>;"'"
            case 'tel': return <Phone className='w-4 h-4'/" ></Phone>;"'"
            case 'textarea': return <MessageSquare className='w-4 h-4'/" ></MessageSquare>;"'"
            case 'select': return <Building className='w-4 h-4'/" ></Building>';'"'"
            default: return <User className='w-4 h-4'/" ></User>}
    }, [])"
    // Render field"'"
  ...
  { opacity: 1, y: 0 '
,''
}} className='space-y-2'></motion>''
        <label className='block text-sm font-medium text-gray-700 dark:text-gray-300'></label>'"
          {field.label}'"'"
          {field.required && <span className='text-red-500 ml-1'></spa>*</span>}"'
        </label>'
''
  ...
  (e) =" ></textarea> handleFieldChange(field.name, e.target.value)`"'"
``",`'
  ...
  (e) =" ></select> handleFieldChange(field.name, e.target.value)`"'"
``",`'
              ...
              <option value=></optio>Select an option</option>"'"
              {field.options?.map(option => (<option key={option.value} value={option.value}" ></option>"''
                  {option.label}''
                </option>))}''"
            </select>) : field.type === 'checkbox' ? (<div className='flex items-center space-x-3'></div>'"'"
              <input type='checkbox' name={field.name} checked={fieldValue} onChange = {}"'""
  (e) =" ></input> handleFieldChange(field.name, e.target.checked)'
,''
  ...
  (e) =" ></input> handleFieldChange(field.name, e.target.value)`"'"
``",`'
} onBlur={() => handleFieldBlur(field.name)} placeholder={field.placeholder} className={`w-full pl-10 pr-3 py-3 border rounded-lg focus:outline-none focus:ring-2 transition-all duration-200 ${fieldValidation?.isTouched'"
                    ? fieldValidation.isValid`'"'"
                        ? 'border-green-500 focus:ring-green-200'`"`'"'"
...
'"
          {/* Password Toggle */}'"'"
          {isPasswordField && (<button type='button' onClick={() =" ></button> togglePasswordVisibility(field.name)} className='absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors'>'"'"
              {showPassword[field.name] ? <EyeOff className='w-4 h-4'/" ></EyeOff> : <Eye className='w-4 h-4'/" ></Eye>}"'"
            </button>)}"'
'
          {/* Validation Icon */}''"
          {fieldValidation?.isTouched && (<div className='absolute right-3 top-1/2 transform -translate-y-1/2'></div>'"'"
              {fieldValidation.isValid ? (<CheckCircle className='w-5 h-5 text-green-500'/" ></CheckCircle>) : (<AlertCircle className='w-5 h-5 text-red-500'/" ></AlertCircle>)}
            </div>)}
        </div>
        {/* Validation Message */}
  ...
  height: 'auto' '
,''
}} className='text-sm text-red-600 dark:text-red-400'></motion>
  ...
  { opacity: 1, scale: 1 '
,''"
}} className='text-center p-8 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-700'></motion>'"'"
        <CheckCircle className='w-16 h-16 text-green-500 mx-auto mb-4'/" ></CheckCircle>'"'"
        <h3 className='text-2xl font-bold text-green-800 dark:text-green-200 mb-2'></h3>"'
          Thank You!'
        </h3>''
        <p className='text-green-600 dark:text-green-300'></p>'
  ...
  { opacity: 1, y: 0 `"'"
``",`'""
}} className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden ${className}`}" ></motion>'
      {/* Header */}''
      <div className='bg-gradient-to-r from-blue-500 to-purple-500 p-6 text-white'></div>''
        <h2 className='text-2xl font-bold mb-2'></h>{title}</h2>''"
        <p className='text-blue-100'>{subtitle}</p>"'"
      </div>"'
'
      {/* Progress Bar */}''
      {showProgressBar && (<div className='px-6 pt-4'></div>''"
          <div className='flex items-center justify-between text-sm text-gray-600 dark:text-gray-400 mb-2'></div>"'"
            <span></spa>Form Progress</span>"''"
            <span></spa>{Math.round(progress)}%</span>'`'"'"
          </div>`"`'"'"
          <div className='w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2'></div>'`'`"`'"'"
            <motion .div className='bg-gradient-to-r from-blue-500 to-purple-500 h-2 rounded-full transition-all duration-300' initial={{ width: 0 }} animate={{ width: `${progress}%` }}/" ></motion>"
          </div>"'"
        </div>) }"'
'
      {/* Form */}''
      <form onSubmit={handleSubmit} className='p-6 space-y-6'></form>''
...
'`"`'"'"
        ...
        <motion .button type='submit' disabled={!isFormValid() || isSubmitting} className={`w-full py-3 px-6 rounded-lg font-medium text-white transition-all duration-200 flex items-center justify-center gap-2 ${!isFormValid() || isSubmitting`"`'"'"
            ...
            : 'bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 transform hover:scale-105'}`} whileHover={isFormValid() && !isSubmitting ? { scale: 1.02 } : {}} whileTap={isFormValid() && !isSubmitting ? { scale: 0.98 } : {}}" ></motion>'"
          {isSubmitting ? (<>'"'"
  ...
  { opacity: 0, height: 0 '
,''
}} className='p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg'></motion>''
