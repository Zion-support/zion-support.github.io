...
export const Modal: React.FC<ModalProps> = ({
  ...
  const handleClose = useCallback(() => {
    trackEvent('click', { element: 'modal_close' });
    onClose();
  }, [onClose, trackEvent]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      trackEvent('view', { element: 'modal', title });
    } else {
  ...
  }, [isOpen, handleKeyDown, trackEvent, title]);
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
        ...
        >
          ...
          {/* Content */}
          <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pt-0">
            {children}
          </div>
        </div>
      </div>
