...
const removeFile = (index: number) => {async function onSubmit (values: z.infer<typeof formSchema>) {
  try {
...
//Future enhancement: Upload attachments //For now we just log the files that would be uploaded if (files.length > 0) {
  ...
  value}value= {value}> {label}</SelectItem>) )origin/main
...
}</div> </FormControl> <FormMessage /> </FormItem> </Button> </div> </form> </Form> </div>);origin/main
}'"};