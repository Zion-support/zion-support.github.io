...
const formSchema = z.object({"reason_code": z.string().min(1, { "message": "Please select a reason for the dispute","
  ...
  const dispute = await createDispute({"project_id": projectId,"milestone_id": milestoneId,"reason_code": values.reason_code,if (dispute && dispute.id) {// Future "enhancement": Upload attachments;
        ...
        if (files.length > 0) {// logInfo(`Would upload ${files.length} files for dispute ${dispute.id}`)}`        toast.success('Your dispute has been submitted')toast.success("Your dispute has been submitted"),if (onDisputeCreated) {if (onDisputeCreated) {onDisputeCreated(dispute.id)}"
      }
...
})const handleFileChange = ("e": React && React.ChangeEvent<HTMLInputElement>,) => {if (e && e.target.files) {const newFiles = Array && Array.from(e && e.target.files),setFiles(prev => { return [...prev, ...newFiles])form && form.setValue('attachments', [...files, ...newFiles])}'
 ; }
}
