...
const AnimatedCounter: React.FC<AnimatedCounterProps> = ({</AnimatedCounterProps>end</AnimatedCounterProps>,}
  ...
  const counterRef = useRef<HTMLDivElement>(null)</HTMLDivElement>useEffect</HTMLDivElement>(() => {const observer = new IntersectionObserver(}
      ([entry]) => {if (entry.isIntersecting && !isVisible) {}
          setIsVisible(true);
          observer.disconnect();
        }
      },
  ...
  useEffect(() => {if (!isVisible) return;}
    ...
    const animate = (currentTime: number) => {if (!startTime) startTime = currentTime;}
