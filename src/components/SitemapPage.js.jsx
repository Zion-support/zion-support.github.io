 export default function SitemapPage() { const publicRoutes = completeSitemap.filter (route => { return !route.requiredAuth)  const talentRoutes = completeSitemap.filter () route => route.requiredAuth && route.requiredRoles &&' (route.requiredRoles.includes ('jobSeeker') ||' route.requiredRoles.includes ('creator') ) )  const employerRoutes = completeSitemap.filter () route => route.requiredAuth && route.requiredRoles &&' (route.requiredRoles.includes ('employer') ||' route.requiredRoles.includes ('buyer') ) ) ; }'
  }
  const adminRoutes = completeSitemap.filter () route => { return route.requiredAuth && route.requiredRoles &&' route.requiredRoles.includes ('admin') ) ; }'
...
const publicRoutes = completeSitemap.filter(route => { return !route.requiredAuth; }
  const talentRoutes = completeSitemap.filter(route => { return route.requiredAuth &&; }
        route.requiredRoles &&;
        (route.requiredRoles.includes('jobSeeker') ||;'
            route.requiredRoles.includes('creator')))const employerRoutes = completeSitemap.filter(route => { return route.requiredAuth &&; }'
        route.requiredRoles &&;
        (route.requiredRoles.includes('employer') ||;'
            route.requiredRoles.includes('buyer')))const adminRoutes = completeSitemap.filter(route => { return route.requiredAuth &&; }'
        route.requiredRoles &&;
...
'''';'
        ...
        <div className='mt-12 bg-white/10 backdrop-blur-sm rounded-lg p-6'>'''';'
          ...
          <div className='grid grid-cols-1 "md": grid-cols-2 "lg":grid-cols-3 gap-4'>'''';'
            {completeSitemap.map(route => { return (''''; }'
