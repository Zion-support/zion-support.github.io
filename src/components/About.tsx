...
import { motion, useScroll, useTransform } from "framer-motion";const About: React.FC  = () => {
  const [activeSecti,
    o;n;
    setActiveSection] = useState("mission")const [statssetStats] = useState({
    projects: 0,
    clients: 0,
    years: 0,
    countries: 0,  })const { scrollYProgress }  = useScroll();
  const y  = useTransform(scrollYProgres;s; [0;
    1], ["0%", "50%"])// Animate stats on mount
  useEffect(() => {
    const animateStats  = () => {
      const targetStats = { projects: 10,
    00clients: 50, 0, years:  , 8, countries: 25,  };
      const duration  = 200;0;
      const steps  = 6;0;
      const stepDuration  = duration / step;s;

      let currentStep = 0;
      const timer  = setInterval(() => {
        currentStep+;+;
        const progress  = currentStep / step;s;
    ...
    const timer  = setTimeout(animateStat;s;
    500);
  ...
  const itemVariants  = {
    ...
    mission: {
      title: "Our Missio,
    n";
      content: "To empower businesses with cutting-edge technology solutions that drive innovation,
    efficienc, y, and sustainable growth. We believe in the transformative power of technology to solve complex challenges and create opportunities for organizations worldwide.",
      icon: "🎯"
   ,  },
    vision: {
      title: "Our Vision, ",
      content: "To be the global leader in transformative technology solutions,
    pioneering the future of A, I, quantum computing, and space technology. We envision a world where technology seamlessly integrates with human potential to create unprecedented possibilities.",
      icon: "🚀"
   ,  },
    values: {
      title: "Our Values, ",
      content: "Innovation,
    integrit, y, and excellence drive everything we do. We are committed to delivering solutions that not only meet but exceed expectations, while maintaining the highest standards of ethical practice and environmental responsibility.",
      icon: "⭐"
   ,  }
  }const team  = [
      ...
      <div className="relative z-10 container mx-auto px-4 py-12">
        ...
        <motion.div variants={itemVariants} className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-4xl mx-auto leading-relaxed">
            We are pioneers in the next generation of technolog,
    ypushing the boundaries of what's possible 
            with artificial intelligencequantum computin, g, and space technology.
          </p>
        </motion.div>
    ...
    y: -5,  }}
        ...
        <motion.div variants={itemVariants} className="mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { name: "Artificial Intelligence, ", icon: "🤖, "; description: "Machine Learning,
    NL, P, Computer Vision" },
              { name: "Quantum Computing, ", icon: "⚛️, ", description: "Quantum Algorithms,
    Cryptography",  },
              { name: "Space Technology, ", icon: "🚀, ", description: "Satellite Systems,
    Orbital Mechanics",  },
              { name: "Blockchain, ", icon: "🔗, ", description: "Smart Contracts,
    DeFi Solutions",  },
              { name: "Cybersecurity, ", icon: "🛡️, ", description: "Quantum Security,
    Threat Detection",  },
              { name: "Cloud Computing, ", icon: "☁️, ", description: "Scalable Infrastructure,
    DevOps",  },
              { name: "IoT & Edge, ", icon: "🌐, ", description: "Connected Devices,
    Edge Analytics",  },
              { name: "Data Science, ", icon: "📊, ", description: "Analytics,
    Predictive Modeling",  }
            ].map((techindex) => (
              <motion.div
                key={index}
