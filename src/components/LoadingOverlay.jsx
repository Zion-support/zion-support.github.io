<<<<<<< HEAD
';export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' />' </div>')  } export default LoadingOverlay';'''"'"
=======
<<<<<<< HEAD
';export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50> <Loader2 className='h - 8 w - 8 animate - spin text - white' /> </div>')  } export default LoadingOverlay';''";""
=======
<<<<<<< HEAD
';export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' />' </div>')  } export default LoadingOverlay';''";
=======
<<<<<<< HEAD
';export const LoadingOverlay = ({visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' />' </div>')  } export default LoadingOverlay';'''";'"
=======
<<<<<<< HEAD
';export const LoadingOverlay = memo(({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' />' </div>')  } export default LoadingOverlay';'''";'"
=======
<<<<<<< HEAD
';&apos;&apos;export const LoadingOverlay = ({ visible = false }) => {&apos; if (!visible) return null return ()&apos;}  <div className=&apos;fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50&apos;>'&apos;&apos; <Loader2 className=&apos;h - 8 w - 8 animate - spin text - white&apos; />&apos;&apos;' </div>&apos;)  }&apos; export;export default LoadingOverlay';&apos;&apos;'";
&quot;"
=======
<<<<<<< HEAD
';export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' /" >' </div>')  } export default LoadingOverlay';'''";'""
';import { Loader2 export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return (<div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white'/" >' </div>) }';export default LoadingOverlay';;'''"'

""
}
=======
<<<<<<< HEAD
';export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' />' </div>')  } export default LoadingOverlay';'''";'"
=======
<<<<<<< HEAD
';export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' />' </div>')  } export default LoadingOverlay';'''";'"
=======
';export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return ()  <div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white' /" >' </div>')  } export default LoadingOverlay''"
';import { Loader2 export const LoadingOverlay = ({ visible = false }) => {' if (!visible) return null return (<div className='fixed inset - 0 z - 50 flex items - center justify - center bg - black / 50'>' <Loader2 className='h - 8 w - 8 animate - spin text - white'/" >' </div>) }';export default LoadingOverlay';;'''"

"
}
>>>>>>> main
>>>>>>> main
>>>>>>> main
>>>>>>> main
>>>>>>> main
>>>>>>> main
>>>>>>> main
>>>>>>> main
>>>>>>> main
