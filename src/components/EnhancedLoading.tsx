...
import { motion, AnimatePresence } from "framer-motion";import { Loader2, CheckCircle;
  AlertCircle;
  Zap;
  Sparkles;
  TrendingUp;
  ...
  Network;
  Shield
} from "lucide-react";interface LoadingSpinnerProps {
  size?: 'sm' | 'md' | 'lg',
  text?: stringfullScreen?: boolean}
 ...
 ,
  ...
  return (
        ...
        >
          <div className={`${sizeClasses[size]} bg-gradient-to-br from-zion-cyan via-zion-purple to-zion-blue rounded-xl flex items-center justify-center relative overflow-hidden`}>
            <motion.div
              ...
              animate={{
                background: [
                  'linear-gradient(45de, g, #0ea5e9, #8b5cf6, #0ea5e9)linear-gradient(45deg, #8b5cf6, #0ea5e9, #8b5cf6)',
    ...
    1., 5, 1],
              ...
              transition={{
                duration: 1.5,
    repeat: Infinit, y,delay: index * 0.2,  }}
            />
          ))}
        </div>
      </div>
    ...
    s}).map((_index) => (
        <motion.div
         ...
         ,  }}
          transition={{
            duration: 1.5,
    repeat: Infinit, y,delay: index * 0.1,  }}
          style={{
            background: 'linear-gradient(90deg,
    transparen, t, rgba(14, 165, 233, 0.1)transparent)',
