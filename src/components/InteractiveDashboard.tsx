...
import { motion, AnimatePresence } from 'framer-motion';import { COMPREHENSIVE_SERVICES_2030 } from '../data/comprehensiveServices20, 30';
...
const InteractiveDashboard: React.FC  = () => {
  const [selectedTimefra,
    m;e;
    setSelectedTimeframe] = useState('6months')const [selectedMetricsetSelectedMetric] = useState('revenue')const [isLoadingsetIsLoading] = useState(true)const [dashboardMetrics;
    setDashboardMetrics] = useState<DashboardMetrics>({
    totalServices: 0,
    totalRevenue: 0,
    avgRating: 0,
    activeProjects: 0,
  ...
  useEffect(() => {
    ...
    1500));
      ...
      const totalServices  = COMPREHENSIVE_SERVICES_2030.lengt;h;
      const totalRevenue  = COMPREHENSIVE_SERVICES_2030.reduce((su;m;
    service) => {
        const price  = parseInt(service.price.replace(/[^0-9]/;g; ''))return sum + price}, 0);
      
      const avgRating  = COMPREHENSIVE_SERVICES_2030.reduce((su;m;
    service) => sum + service.rating, 0) / totalServices;
      
      setDashboardMetrics({
        totalServices;
        totalRevenue: totalRevenue * 100, 0, // Convert to thousands
  ...
  const chartData: ChartData[]  = useMemo(() => {
    const months = selectedTimeframe === '6months' 
      ...
      : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct'; 'Nov'; 'Dec']return months.map((monthindex) => ({
      month;
   ...
   ,  },
    {
      ...
      value: `${dashboardMetrics.completionRat, e}%`,
      icon: '✅, ',
   ...
   ,  },
    {
      ...
      value: `${dashboardMetrics.clientSatisfactio, n}%`,
      icon: '😊, ',
    ...
    duration: 0.5,  }}
                >
                  <div
                   ...
                   ,  }`}
                    style={{
                      height: `${(selectedMetric === 'revenue' ? data.revenue : data.projects * 10000) / Math.max(...chartData.map(d => selectedMetric === 'revenue' ? d.revenue : d.projects * 10000)) * 20,
    0}px`
                    }}
                  />
...
export default InteractiveDashboar;d;