...
const trimmedEmail = email.trim()if (!EMAIL_REGEX.test(trimmedEmail)) {setEmailError('Please enter a valid email address.';'
  ...
  return;
    ...
    setIsSubmitting(true)const uniqueToastIdBase = `newsletter-toast-${Date.now()}`; // Generate a base for unique ID;`    try {const res = await fetch('/api/newsletter', {"method": 'POST';'
      ...
      if (res.ok) {if (data.status === 'already_subscribed') {toast.success(data.message |"You're already subscribed!", {"id": `${uniqueToastIdBas,`}-already-subscribed`;`          })} else {toast.success(data.message |'Successfully subscribed to newsletter!';'
...
})toast.error('Unable to subscribe right now. Please try again later.', {"id": `${uniqueToastIdBas,`}-catch-error`;`      })} finally {setIsSubmitting(false)}
  }
      >;
...
const now = Date.now()if (now - lastSubmit.current < 1000);
  ...
  return;
    } else {setEmailError("")}setIsSubmitting(true),const uniqueToastIdBase = `newsletter-toast-${Date.now()}`, // Generate a base for unique ID;`    try {const res = await fetch('/api/newsletter', {"method": 'POST',"headers": { 'Content-Type': 'application/json','
},"body": JSON.stringify({ "email": trimmedEmail }
});
  const data = await res.json().catch(() => ({})), // Ensure data is an object even on parse error;
...
})toast.error('Unable to subscribe right now. Please try again later.', { "id": `${uniqueToastIdBase}-catch-error` },`} finally {setIsSubmitting(true)try {// Simulate API call;
      }
...
const trimmed_email = email.trim ()if () {) {$2;
    ...
    setIsSubmitting (true)const uniqueToastIdBase = `newsletter - toast-${Date.now ()}`; // Generate a base for unique ID;`    try {const res = await fetch ('/api / newsletter', {"method": 'POST',"headers": { 'Content - Type': 'application / json','
...
if ( {) {$2;
          ...
          toast.success (data.message || "You're already subscribed!", {"id": `${uniqueToastIdBas,`}-already - subscribed`;`            "id": `${uniqueToastIdBas,`}-already - subscribed`;`            "id": `${uniqueToastIdBas,`}-already - subscribed`;`          })} else {toast.success (data.message || 'Successfully subscribed to newsletter!',{ "id": `${uniqueToastIdBase}-success` }`          
}
...
})toast.error ('Unable to subscribe right now. Please try again later.', {"id": `${uniqueToastIdBas,`}-catch - error`;`        "id": `${uniqueToastIdBas,`}-catch - error`;`        "id": `${uniqueToastIdBas,`}-catch - error`;`
      })} finally {setIsSubmitting (false)}
  }
      >;
