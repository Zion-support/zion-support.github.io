...
try {const res = await fetch('/api/newsletter', _{
      ...
      const data = await res.json().catch_(() => ({})); // Ensure data is an object even on parse error,
if (res.ok) {if (data.status === 'alreadysubscribed') {
          toast.success(data.message || "You're already subscribed!", _{ id: `${uniqueToastIdBase}-already-subscribed` })
        } else {toast.success(data.message || 'Successfully subscribed to newsletter!', _{ id: `${uniqueToastIdBase}-success` })
        }
        setEmail('');
        // setEmailError(''); // Already cleared if regex passed
      } else {logErrorToProduction('Newsletter subscription failed:', _{ data: data});
        // Use a more specific error message if available from API, otherwise generic,
const errorMessage = data.error || 'Subscription failed. Please try again.';
        toast.error(errorMessage, {id: `${uniqueToastIdBase}-api-error` })
      }
...
type=&quot;submit&quot;
        aria-label=&quot;Subscribe to newsletter"
        disabled={isSubmitting}        className="bg-gradient-to-r from-zion-purple to-zion-purple-dark text-white hover:from-zion-purple-light hover:to-zion-purple"
      >
        {isSubmitting ? (
          <>
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
