 const Navigation = () => { const [isOpen, setIsOpen] = useState (false)  const [isServicesOpen, setIsServicesOpen] =;
  ...
  useState (false)  const location = useLocation ()  const navigationItems = [' { "name": 'Home', "href": '/', "icon": Globe,'
...
}] const serviceCategories = [ { "name": 'AI & Machine Learning', services[   { "name": 'AI - Powered CRM', "href": '/comprehensive - services#ai - powered - crm','
},
...
}, { "name": 'Emerging Technologies', services[   { "name": 'Blockchain Solutions', "href": '/comprehensive - services#blockchain - supply - chain','
},
...
const Icon = item.icon;
            return (<Link key={item.name} to={item.href} className={`flex items-center space-x-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive(item.href)'`';'                    ? 'text-zion-cyan bg-zion-cyan/10''`''`';'                    : 'text-zion-slate-light "hover": text-white "hover":bg-white/10'},'
}>'''';'
...
'''';'
            ...
            <div className='relative'>'''';'
              <button onClick={() => setIsServicesOpen(!isServicesOpen)} className='flex items-center space-x-1 px-3 py-2 rounded-lg text-sm font-medium text-zion-slate-light "hover": text-white "hover":bg-white/10 transition-colors'>'''';'
                ...
                <span>Services</span>'`'`;`                <ChevronDown className={`w-4 h-4 transition-transform ${isServicesOpen ? 'rotate-180' : ''}`} />;`              </button>'''';'
'''';'
...
'''';'
      ...
      {isOpen && (<div className='"lg":hidden'>'''';'
          <div className='px-2 pt-2 pb-3 space-y-1 bg-zion-blue-dark border-t border-zion-blue-light/30'>;'
            {navigationItems.map((item) => {}
`;`
const Icon = item.icon`;`                return (<Link key={item.name} to={item.href} className={`flex items-center space-x-2 px-3 py-2 rounded-lg text-base font-medium transition-colors ${isActive(item.href)'`';'                        ? 'text-zion-cyan bg-zion-cyan/10''`''`';'                        : 'text-zion-slate-light "hover": text-white "hover":bg-white/10'}`} onClick={() => setIsOpen(false,`
}>'''';'
