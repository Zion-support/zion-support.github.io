    ...
    return (<nav className='bg-zion-blue-dark/95 backdrop-blur-sm border-b border-zion-blue-light/30 fixed top-0 left-0 right-0 z-50">"''
      <div className='container mx-auto px-4 sm:px-6 lg:px-8">"''
        <div className='flex justify-between items-center h-16">""
          ...
          {/* Desktop Navigation */}"''
          <div className='hidden lg:flex lg:items-center lg:space-x-8'>, {navigationItems.map((item) => {}
...
"''
              {isServicesOpen && (<div className='absolute top-full left-0 mt-2 w-80 bg-zion-blue-dark border border-zion-blue-light rounded-xl shadow-2xl z-50">"''
                  <div className='p-4">"''
                    <div className='grid grid-cols-1 gap-4">"'', {serviceCategories.map((category) => (<div key={category.name} className='space-y-2">"''
                          ...
                          <div className='space-y-1">"'', {category.services.map((service) => (<Link key={service.name} to={service.href} className='block px-3 py-2 text-sm text-zion-slate-light hover:text-white hover:bg-white/10 rounded-lg transition-colors', onClick={() =" > setIsServicesOpen(false)}>
                                {service.name}
                              </Link>) ) }
                          </div>
                        </div>) ) }
      ...
      {isOpen && (<div className='lg:hidden">"''
          <div className='px-2 pt-2 pb-3 space-y-1 bg-zion-blue-dark border-t border-zion-blue-light/30'>, {navigationItems.map((item) => {}`
                const Icon = item.icon`"`
            ...
            <div className='px-3 py-2">"''
              ...
              <div className='space-y-2">"''
                {serviceCategories.map((category) => (<div key={category.name} className='ml-4">"''
                    ...
                    <div className='space-y-1">"'', {category.services.map((service) => (<Link key={service.name} to={service.href} className='block px-3 py-1 text-sm text-zion-slate-light hover:text-white rounded transition-colors', onClick={() =" > setIsOpen(false)}>
                          {service.name}
                        </Link>) ) }
                    </div>
                  </div>) ) }
