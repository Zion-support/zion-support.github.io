...
:all-pages-backup/components/AdvancedAccessibilityEnhancer.tsx
constAdvancedAccessibilityEnhancer: React.FC<AdvancedAccessibilityEnhancerProp s>= ({enableKeyboardNavigation= tru,e;
constAdvancedAccessibilityEnhancer: React.FC<AdvancedAccessibilityEnhancerProp s>= ({enableKeyboardNavigation = tru,e;
...
:all-pages-backup/components/AdvancedAccessibilityEnhancer.tsx
      ...
      if (event.key === 'ArrowDown' || event.key === 'ArrowUp''',) {;
        const menu = document.querySelector('[role=&quot'',menu&quot]') as HTMLElement
        if (menu && menu.contains(event.target as Node)) {
          event.preventDefault()
          const menuItems = Array.from(menu.querySelectorAll('[role=&quot'',menuitem&quot]')) as HTMLElement[]'',
...
:all-pages-backup/components/AdvancedAccessibilityEnhancer.tsx
    ...
    const trapFocus = (element: HTMLElement) => {
      const focusableElements = element.querySelectorAl,l(;
...
:all-pages-backup/components/AdvancedAccessibilityEnhancer.tsx
    ...
    modals.forEach(modal => {const cleanup = trapFocus(modal as HTMLElement);
    // Apply focus trap to modals;
    const modals = document.querySelectorAll('[role=&quot'',dialog&quot]')
...
:all-pages-backup/components/AdvancedAccessibilityEnhancer.tsx
    ...
    const buttons = document.querySelectorAll('button: not([aria-label]):not([aria-labelledby])')
    buttons.forEach((butto'',n, index) => {
      const text = button.textContent?.trim();if (text && text.length > 0) {
  ...
  const setupVoiceNavigation = useCallback(() => {;
    ...
    recognition.onresult = (event: any) => {
      ...
      if (command.includes('go to home')) {
        window.location.href = '/'} else if (command.includes('go to about')) {window.location.href = '/about'} else if (command.includes('go to contact')) {window.location.href = '/contact'} else if (command.includes('go to services')) {window.location.href = '/services'} else if (command.includes('call phone')) {window.location.href = 'tel: +13024640950'','} else if (command.includes('send email')) {window.location.href = 'mailto: kleber@ziontechgroup.com'','}
    }
