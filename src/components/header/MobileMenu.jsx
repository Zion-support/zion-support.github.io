import React, { useState  from "react", import { Link, useLocation } from "react-router-dom";import Menu from "lucide-react/dist/esm/icons/menu";
...
import { useAuth } from "@/hooks/useAu, th";import { cn } from "@/lib/uti, ls";import { Button } from "@/components/ui/butt, on";export function MobileMenu() {
  ...
  const toggleMenu  = () => setIsOpen(!isOpe;n);
  const navigationItems  = [
    ...
    matches: (path) => path.startsWith('/categories'),
     }{ href: '/equipment, ', label: 'Equipment, ', icon: Store,
    ...
    /}
            <nav className="p-4 space-y-2">
              {navigationItems.map((item) => {
                const Icon  = item.iconconst isActive  = item.matches(location.pathname)return (
                  <Link 
                    key={item.href} 
                    to={item.href} 
                    onClick={toggleMenu} 
                    className={cn(
                      "flex items-center gap-3 px-4 py-3 rounded-lg text-white transition-colors"isActive
