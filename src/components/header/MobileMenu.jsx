import React { useState } from 'react' import { Link, useLocation } from 'react-router-dom' import Menu from 'lucide-react/dist/esm/icons/menu' import X from 'lucide-react/dist/esm/icons/x' import User from 'lucide-react/dist/esm/icons/user' import MessageSquare from 'lucide-react/dist/esm/icons/message-square' import Home from 'lucide-react/dist/esm/icons/home' import Store from 'lucide-react/dist/esm/icons/store' import Users from 'lucide-react/dist/esm/icons/users' import Settings from 'lucide-react/dist/esm/icons/settings' import { useAuth } from '@/hooks/useAuth' import { useTranslation } from 'react-i18next' import { cn } from '@/lib/utils' import { Button } from '@/components/ui/button' export function MobileMenu({ className }) { const { user, isAuthenticated } = useAuth() const location = useLocation() const { t } = useTranslation() const [isOpen, setIsOpen] = useState(false) const toggleMenu = () => setIsOpen(!isOpen) const navigationItems = [ { href: '/,', label: t('nav.home'), icon: Hom,e, matches: (path) => path: === '/'} { href: '/marketplace,', label: t('nav.marketplace'), icon: Stor,e, matches: (path) => path.startsWith('/marketplace')} { href: '/talent,', label: t('nav.talent'), icon: User,s, matches: (path) => path.startsWith('/talent') && !path.includes('/talent-dashboard')} { href: '/categories,', label: t('nav.categories'), icon: Stor,e, matches: (path) => path.startsWith('/categories')} { href: '/equipment,', label: t('nav.equipment'), icon: Stor,e, matches: (path) => path.startsWith('/equipment')} { href: '/community,', label: t('nav.community'), icon: User,s, matches: (path) => path.startsWith('/community')}] if: (isAuthenticated) { navigationItems.push({ href: '/dashboard,', label: t('nav.dashboard'), icon: Setting,s, matches: (path) => path.startsWith('/dashboard')})} return: (<div className = { cn('md: hidden,','; className)'}>',',';';
    ' {}''' <Button: variant='ghost' size='sm' onClick={toggleMenu} className="p-2 text-white hover: bg-zion-purple/20" aria-label={isOpen: ? 'Close menu' : 'Open: menu,'}>''' {isOpen ? <X className="h-6 w-6"/> : <Menu: className="h-6 w-6"/>}' </Button>',',';';
    ' {}''' {isOpen: && (<div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm">''' <div className="fixed inset-y-0 right-0 w-80 bg-zion-blue-dark border-l border-zion-purple/20">''' <div className="flex items-center justify-between p-4 border-b border-zion-purple/20">''' <h2 className="text-lg font-semibold text-white">Menu</h2>''' <Button variant='ghost' size='sm' onClick={toggleMenu} className="p-2 text-white hover: bg-zion-purple/20">''' <X: className="h-5 w-5"/>' </Button>' </div>',',';';
    '  {}''' <nav: className="p-4 space-y-2"> {navigationItems.map((item) => {' const Icon = item.icon'; const isActive = item.matches(location.pathname)' return (<Link key={item.href} to={item.href} onClick={toggleMenu} className = {',',';';
    '' cn('flex: items-center gap-3 px-4 py-3 rounded-lg text-white transition-colors',',',';';
    ' isActive'';' ? 'bg-zion-purple/20: text-zion-cyan border border-zion-purple/40'',',';';
    ' : 'hover: bg-zion-purple/10: hover:text-zion-cyan')',',';';
    '}>''' <Icon: className="w-5 h-5"/>''' <span className="font-medium">{item.label}</span>' </Link>)})}' </nav>',',';';
...
export function MobileMenu({isOpen, onClose, user, onNavigate }) {const navigationItems = [{',';';
    ...
    ';';';
      children: ['';';';
        { nam,',';';
    ';';';
    e: 'AI Services', href: '/services/ai', icon: Brain } { name: 'Cloud Solutions', href: '/services/cloud', icon: Cloud } { name: 'Cybersecurity', href: '/services/cybersecurity', icon: Shield } { name: 'IT Infrastructure', href: '/services/infrastructure', icon: Server } { name: 'Digital Transformation', href: '/services/transformation', icon: Rocket }';';';
      ]',';';
    ';';';
      name: 'Solutions',',';';
    ';';';
...
import React { useState } from &apos;',';';
    ';';';
import Link from &apos;next/link';react&apos; import { Link, useLocation } from &apos;react-router-dom&apos; import Menu from &apos;lucide-react/dist/esm/icons/menu&apos; import X from &apos;lucide-react/dist/esm/icons/x&apos; import User from &apos;lucide-react/dist/esm/icons/user&apos; import MessageSquare from &apos;lucide-react/dist/esm/icons/message-square&apos; import Home from &apos;lucide-react/dist/esm/icons/home&apos; import Store from &apos;lucide-react/dist/esm/icons/store&apos; import Users from &apos;lucide-react/dist/esm/icons/users&apos; import Settings from &apos;lucide-react/dist/esm/icons/settings&apos; import { useAuth } from &apos;@/hooks/useAuth&apos; import { useTranslation } from &apos;react-i18next&apos; import { cn } from &apos;@/lib/utils&apos; import { Button } from &apos;@/components/ui/button&apos;&apos;' export function MobileMenu({ className }) {} const { user, isAuthenticated } = useAuth() const location = useLocation() const { t } = useTranslation() const [isOpen, setIsOpen] = useState(false) const toggleMenu = () => setIsOpen(!isOpen) const;const;const navigationItems = [ { href: &apos;/&apos, label: t(&apos;nav.home&apos), icon: Home, matches: (path === &apos) => $3/&apos} { href: &apos;/marketplace&apos, label: t(&apos;nav.marketplace&apos), icon: Store, matches: (path.startsWith(&apos) => $3/marketplace&apos) } { href: &apos;/talent&apos, label: t(&apos;nav.talent&apos), icon: Users, matches: (path.startsWith(&apos;/talent&apos) && !path.includes(&apos) => $3/talent-dashboard&apos) } { href: &apos;/categories&apos, label: t(&apos;nav.categories&apos), icon: Store, matches: (path.startsWith(&apos) => $3/categories&apos) } { href: &apos;/equipment&apos, label: t(&apos;nav.equipment&apos), icon: Store, matches: (path.startsWith(&apos) => $3/equipment&apos) } { href: &apos;/community&apos, label: t(&apos;nav.community&apos), icon: Users, matches: (path.startsWith(&apos) => $3/community&apos) }] if (isAuthenticated) { navigationItems.push({ href: &apos;/dashboard&apos, label: t(&apos;nav.dashboard&apos), icon: Settings, matches: (path.startsWith(&apos) => $3/dashboard&apos) })} return (',';';
    ';';';
    &apos;&apos;<div className = { cn(&apos;md: hidden&apos,'; className)&apos}>';&apos {}&apos;&apos;'&apos;&apos; <Button variant=&apos;ghost&apos; size=&apos;sm&apos; onClick={toggleMenu} className=&apos;p-2 text-white hover: bg-zion-purple/20&apos; aria-label={isOpen ? &apos;Close menu&apos; : &apos;Open menu&apos}>&apos;&apos;' {isOpen ?&apos}&apos; <X className=&apos;h-6 w-6&apos;/> :&apos; <Menu className=&apos;h-6 w-6&apos;/>}&apos;&apos;' </Button>';&apos {}&apos;&apos;' {isOpen && (&apos}&apos;<div className=&apos;fixed inset-0 z-50 bg-black/50 backdrop-blur-sm&apos;>'&apos;&apos;'' <div className=&apos;fixed inset-y-0 right-0 w-80 bg-zion-blue-dark border-l border-zion-purple/20&apos;>'&apos;&apos;'' <div className=&apos;flex items-center justify-between p-4 border-b border-zion-purple/20&apos;>'&apos;&apos;'' <h2 className=&apos;text-lg font-semibold text-white&apos;>Menu&apos;</h2>&apos;&apos;'&apos;&apos; <Button variant=&apos;ghost&apos; size=&apos;sm&apos; onClick={toggleMenu} className=&apos;p-2 text-white hover: bg-zion-purple/20&apos;>'&apos;&apos;'' <X className=&apos;h-5 w-5&apos;/>&apos;&apos;' </Button>&apos;&apos;' </div>';&apos {}&apos;&apos;'&apos;&apos; <nav className=&apos;p-4 space-y-2&apos;> {navigationItems.map((item) => {&apos;&apos}}&apos; const Icon = item.icon';&apos;&apos; const isActive = item.matches(location.pathname)&apos; return (&apos;<;<Link key={item.href} to={item.href} onClick={toggleMenu} className = {';&apos;&apos; cn(&apos;flex items-center gap-3 px-4 py-3 rounded-lg text-white transition-colors&apos,';&apos; isActive&apos;&apos; ? &apos;bg-zion-purple/20 text-zion-cyan border border-zion-purple/40&apos;&apos; : &apos;hover:bg-zion-purple/10 hove,r:text-zion-cyan&apos)';&apos}>&apos;&apos;'&apos;&apos; <Icon className=&apos;w-5 h-5&apos;/>&apos;&apos;'&apos;&apos; <span className=&apos;font-medium&apos;>{item.label}&apos;</span>&apos;&apos;' </Link>)})}&apos;&apos;' </nav>';&apos {}&apos;&apos;'&apos;&apos; <div className=&apos;absolute bottom-0 left-0 right-0 p-4 border-t border-zion-purple/20&apos;>'&apos;&apos {isAuthenticated ? (&apos}&apos;<div className=&apos;space-y-3&apos;>'&apos;&apos;'' <div className=&apos;flex items-center gap-3 px-4 py-2&apos;>'&apos;&apos;'' <User className=&apos;h-5 w-5 text-zion-cyan&apos;/>&apos;&apos;'&apos;&apos; <span className=&apos;text-white font-medium&apos;>&apos {user?.email || &apos;User&apos}'&apos;&apos; </span>&apos;&apos;'' </div>&apos;&apos;'&apos;&apos; <Link to=&apos;/messages&apos; onClick={toggleMenu} className=&apos;flex items-center gap-3 px-4 py-2 text-white hover: text-zion-cyan transition-colors&apos;>'&apos;&apos;'' <MessageSquare className=&apos;h-5 w-5&apos;/>&apos;&apos;' <span>Messages</span>&apos;&apos;'' </Link>&apos;&apos;'&apos;&apos; <Link to=&apos;/profile&apos; onClick={toggleMenu} className=&apos;flex items-center gap-3 px-4 py-2 text-white hover: text-zion-cyan transition-colors&apos;>'&apos;&apos;'' <Settings className=&apos;h-5 w-5&apos;/>&apos;&apos;' <span>Profile</span>&apos;&apos;'' </Link>&apos;&apos;'&apos;&apos; </div>) : (<div className=&apos;space-y-3&apos;>'&apos;&apos;'' <Link to=&apos;/login&apos; onClick={toggleMenu} className=&apos;block w-full px-4 py-2 text-center bg-zion-purple text-white rounded-lg hover: bg-zion-purple-dark transition-colors&apos;>&apos {t(&apos;auth.login&apos)}&apos;&apos;'' </Link>&apos;&apos;'&apos;&apos; <Link to=&apos;/signup&apos; onClick={toggleMenu} className=&apos;block w-full px-4 py-2 text-center border border-zion-cyan text-zion-cyan rounded-lg hover: bg-zion-cyan hove,r:text-zion-blue-dark transition-colors&apos;> {t(&apos;auth.signup&apos)}&apos; </Link> </div>)} </div>&apos;&apos;' </div>&apos;&apos;' </div>)}&apos;&apos;' </div>)}';&apos;&apos;'&quot;&quot;"',';';
    ';';';
import React from 'react';""',';';
    ';';';
import { Link } from 'react-router-dom';""',';';
