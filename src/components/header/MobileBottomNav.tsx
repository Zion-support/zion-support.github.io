...
const router = use_router ()const { user

} = use_auth ()const is_authenticated = !!user;
...
const router = useRouter()const { user 
} = useAuth()const isAuthenticated = !!user;
...
if (cartContextValue && cartContextValue.items) {cartCount = cartContextValue.items.reduce((sum, i) => sum + i.quantity, 0)} else {// logWarn("MobileBottomNav: Cart data or items not available, defaulting cartCount to 0.")}
  const navItems = [;
    {name: "Home",
href: "/",
icon: Home,{name: "Browse",
href: "/talent",
icon: Search,{name: "Community",
href: "/community",
icon: MessageCircle,{name: "Wishlist",
href: "/wishlist",
icon: Heart},{name: "Messages",
href: "/messages",
icon: MessageSquare},{name: "Cart",
href: "/cart",
icon: ShoppingCart,{name: "Dashboard",
href: "/dashboard",
icon: User,
...
interface MobileBottomNavProps {
unreadCount?: number;
export function MobileBottomNav(): any ({ unreadCount = 0
}: MobileBottomNavProps) {const router = useRouter()const { user } = useAuth()const isAuthenticated = !!user;
  const { items: wishlistItems } = useWishlist()// Renamed to avoid conflict;
...
href: '/',
icon: Home,
matches: (path: string) => path === '/'    },{name: 'Browse',
href: '/talent',
icon: Search,
matches: (path: string) =>;
...
authRequired: true},{name: 'Messages',
href: '/messages',
icon: MessageSquare,
matches: (path: string) =>;
        path && path.startsWith('/messages') || path && path.startsWith('/inbox'),      badge: unreadCount,
authRequired: true},{name: 'Cart',
href: '/cart',
icon: ShoppingCart,
matches: (path: string) => path && path.startsWith('/cart'),
badge: cartCount    },{name: 'Dashboard',
href: '/dashboard',
icon: User,
matches: (path: string) => path && path.startsWith('/dashboard'),
authRequired: true}];
  // Filter items based on auth status;
  const visibleItems = navItems && navItems.filter(item => !item && item.authRequired || (item && item.authRequired && isAuthenticated))'flex flex-col items-center justify-center w-full h-full px-1 py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary',
item && item.matches(router && router.pathname)? 'text-primary';
                : 'text-foreground/70 hover:text-foreground';
            )}>;
            <div className='relative'>;
