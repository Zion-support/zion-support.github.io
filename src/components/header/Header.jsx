...
const { user 
} = useAuth()const { isWhitelabel, primaryColor } = useWhitelabel()const navigate = useNavigate()const [query, setQuery] = useState('')const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false)const [isScrolled, setIsScrolled] = useState(false)const searchSuggestions  = generateSearchSuggestions()// If we have a white-label tenant and no specific customTheme is provided, // use the tenant&apos;
s primary color'';
...
window.addEventListener('scroll', handleScroll)return () => window.removeEventListener('scroll', handleScroll)}, [])const handleSubmit = e  => {
...
setQuery(text)navigate(`/search?q=${encodeURIComponent(text)
}`)}}
