...
import { ChevronDown, Globe export const LanguageSelector = () => {
    ...
    return (<div className="relative">
      ...
      {isOpen && (<div className="absolute top-full right-0 mt-2 w-48 bg-black/95 backdrop-blur-md rounded-lg shadow-xl border border-gray-800 z-50">
          <div className="py-2">
            {languages.map((((((((language, index, index, index, index, index, index) => ({ ...((((((language, index, index, index, index, index, key: index })) => ({ ...(((((language, index, index, index, index, key: index })) => ({ ...((((language, index, index, index, key: index })) => ({ ...(((language, index, index, key: index })) => ({ ...((language, index, key: index })) => ({ ...(language, key: index })) => (<button key={language.code} onClick={() => handleLanguageChange(language.code)} className={`w-full flex items-center gap-3 px-4 py-2 text-sm transition-colors ${currentLanguage === language.code
