...
export function MainNavigation({ className = '' }: { className?: string }) {
  ...
  const renderDropdown = (item: any, isOpen: boolean, setIsOpen: (open: boolean) => void) => {
    ...
    return (
      ...
      {/* Get Started */}
      {/* The NavLink component was removed from imports, so this block is commented out or removed if not needed */}
      {/* <NavLink
        to="/signup"
        className={({ isActive }) =>
          cn(
