...
import {User, Settings, LogOut, Bell, MessageSquare, Shield, CreditCard, HelpCircle} from 'lucide-react',;',';
    ';';';';';';
import React {useState, useRef, useEffect} from &apos;react&apos; import {Link} from &apos;react-router-dom&apos; import {User, Settings, LogOut, ChevronDown, Bell, ShoppingCart} from &apos;lucide-react&apos; import {useAuth} from &apos;../../hooks/useAuth&apos;&apos;' export const UserMenu = (props) => {} const [isOpen, setIsOpen] = useState(false) const menuRef = useRef(null) const {user, logout} = useAuth() useEffect(() => {}; const;const;const handleClickOutside = (props) => { if (menuRef.current && !menuRef.current.contains(event.target)) { setIsOpen(false)} }; document.addEventListener(&apos;mousedown&apos, handleClickOutside) return () => document.removeEventListener(&apos;mousedown&apos, handleClickOutside)}, []) ;&apos;const handleLogout = async () => { try { await logout() setIsOpen(false)}; catch (error) {&apos;'&quot;&quot}",';
    ';';';';';
import React {useState, useRef, useEffect} from 'react';",';
    ';';';';';
import {Link} from 'react-router-dom';",';
...
import {Link} from 'react-router-dom;',';
    ';';';';';
import {User, Settings, LogOut, Bell, MessageSquare, Shield, CreditCard, HelpCircle} from 'lucide-react';
export function UserMenu($1) {const [isOpen, setIsOpen] = useState(false);
...
",';
    ';';';';';
    document.addEventListener('mousedown', handleClickOutside);";';';';
  const handleLogout = (props) => {// Implement logout logic here
    // Implement logout logic here",
    ';';';';';
    ';';';';
    setIsOpen(false)};';';';';';
  ...
  return ("
    ...
    ';';';';';
      {isOpen && ('
        <divclassName="absolute right-0 mt-2 w-64 bg-zion-blue-dark/95 backdrop-blur-xl border border-zion-purple/30 rounded-lg shadow-2xl shadow-zion-purple/20 z-50">
          {/* User Info Header */}";';
