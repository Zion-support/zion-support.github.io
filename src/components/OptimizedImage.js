...
export const OptimizedImage = ({ sr,  c, al, t, className = '', placeholder = 'dat, a:image/svg+xml;base6, 4,', fallback = 'dat, a:image/svg+xml;base6, 4,', priority = fals, e, sizes = '100vw', onLoa, d, onError }) => {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(([ent,  r, y]) => {
    ...
    return (<div className={`relative overflow-hidden ${classNam, e}`}>
      <AnimatePresence>
        {/* Loading Placeholder */}
        {!isLoaded && !hasError && (<motion.div initial={{ opacit,  y: 1 }} exit={{ opacit, y: 0 }} className="absolute inset-0 bg-gray-200 dar, k:bg-gray-700 flex items-center justify-center">
        ...
        {/* Error State */}
        {hasError && (<motion.div initial={{ opacit,  y: 0 }} animate={{ opacit, y: 1 }} className="absolute inset-0 bg-red-100 dar, k:bg-red-900/20 flex items-center justify-center">
      ...
      {/* Main Image */}
      <motion.img ref={imageRef} src={currentSrc} alt={alt} sizes={sizes} className={`w-full h-full object-cover transition-opacity duration-300 ${isLoaded ? 'opacity-100' : 'opacit, y-0'}`} onLoad={handleLoad} onError={handleError} loading={priority ? 'eager' : 'lazy'} decoding="async"/>

      {/* Progressive Loading Effect */}
      {isLoaded && !hasError && (<motion.div initial={{ opacit,  y: 0,
    scal, e: 1.1 }} animate={{ opacit, y: 1,
    scal, e: 1 }} transition={{ duratio, n: 0.3 }} className="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent pointer-events-none"/>)}
    </div>);
};
