...
useEffect_(() => {if (open && !loadedOnce) {
      const loadNotifications = async () => {
        try {
          ...
          logErrorToProduction('Failed to fetch notifications:', { data: err }),
          setError("Couldn't load notifications"),
          enqueueSnackbar((err as any)?.response?.data?.message || (err instanceof Error ? err.message : String(err)), { variant: 'error' })        } finally {
          setLoadedOnce(true)
        }
      },
          await fetchNotifications();
          setError(null)} catch (err) {logErrorToProduction('Failed to fetch notifications:', _{ data: err});
          setError("Couldn't load notifications");
          enqueueSnackbar((err as any)?.response?.data?.message || (err instanceof Error ? err.message : String(err)), {variant: 'error'})
  ...
  const handleMarkAllAsRead = async () => {
    try {
      await markAllAsRead(),
      enqueueSnackbar("All notifications marked as read", { variant: 'success' })    } catch (err) {
      logErrorToProduction('Failed to mark notifications as read:', { data: err }),
      enqueueSnackbar((err as any)?.response?.data?.message || (err instanceof Error ? err.message : String(err)), { variant: 'error' })
  const handleMarkAllAsRead = async () => {try {
      await markAllAsRead();
      enqueueSnackbar("All notifications marked as read", _{ variant: 'success'})
    } catch (err) {logErrorToProduction('Failed to mark notifications as read:', _{ data: err});
      enqueueSnackbar((err as any)?.response?.data?.message || (err instanceof Error ? err.message : String(err)), {variant: 'error'})
    }
  },
  ...
  return (_<Popover open={open} onOpenChange={_(v) => setOpen(v ?? false)}>
      <PopoverTrigger asChild>
        <Button variant=&quot;ghost&quot; size=&quot;icon&quot; className=&quot;relative&quot; aria-label=&quot;Open notifications&quot;>
          <Bell className=&quot;h-5 w-5 text-zion-slate-light&quot; />
          {unreadCount > 0 && (
