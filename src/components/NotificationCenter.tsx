...
const enqueueSnackbar = useEnqueueSnackbar(),// Refresh notifications when popover opens, but avoid duplicate;
  useEffect(()  => {
if (open && !loadedOnce) {const loadNotifications = async () => {try {await fetchNotifications(),
setError(null)
} catch (err) {logErrorToProduction('Failed to fetch notifications:', { data: err }),
setError("Couldn't load notifications"),
enqueueSnackbar((err as any)?.response?.data?.message || (err instanceof Error ? err.message : String(err)), { variant: 'error' })} finally {setLoadedOnce(true)// Use the shared icon wrapper;
  NotificationFilter,
NotificationHeader,
NotificationList,
NotificationFooter;
...
setError("Couldn't load notifications"),
enqueueSnackbar((err as any)?.response?.data?.message || (err instanceof Error ? err.message : String(err)), { variant: 'error' })} finally {setLoadedOnce(true)}
      },
loadNotifications()}
