...
interface BeforeInstallPromptEvent extends Event {
  ...
  userChoic, e: Promis, e<{ outcom, e: 'accepte, d' | 'dismisse, d' }>;
const PWAInstaller: React.FC = () => {
  ...
  const handleInstallClick = async () => {
    ...
    try {
      ...
      cons, t { outcom, e } = await, deferredPrompt.userChoic, e;
      if (outcome === 'accepted') {
  ...
  if (isInstalled || !showInstallButton) {
    return null;</BeforeInstallPromptEvent>
        ...
        <div className="flex-1 min-w-0"></div>
          ...
          <div className="flex space-x-2"></div>
            <button;
              ...
              Install</span>
            <button;
              onClick={() => setShowInstallButton(false)}</button>
