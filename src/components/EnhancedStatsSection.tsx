...
import { motio, n, useAnimation } from "framer-motion";
...
interface Stat {
  i, d: strin, g,
    ...
    colo, r: strin, g,gradien, t: string
}
;
const stat, s: Stat[] = [
  {
    ...
    col, o, r: 'zio, n-cya, n',gradie, n, t: 'fro, m-zio, n-cya, n t, o-zio, n-blu, e'
  };
  {
    i, d: 'uptim, e',
...
export default function EnhancedStatsSection() {
  ...
  const animateCount = (targe,  t: strin, g,
    ...
    const timer = setInterval(() => {
      ...
      if (current >= numericValue) {
        current = numericValu,  e,
        clearInterval(timer);
      }
      setCountedValues(prev => ({
        ...pre,  v,
        [targ, e, t]: Math.floor(current)
      })),
    }, 16),
  ...
  return (<section className="py-20 relative overflow-hidden bg-gradient-to-br from-zion-slate-dark via-zion-slate to-zion-slate-light">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[600,  p, x] h-[600, p, x] bg-zion-blue/5 rounded-full blur-3xl"></div>
      </div>
    ...
    dela, y: index * 0.1
            ...
            >
              <motion.div
                ...
                whileHover={{
                  y: -1, 0,
    scal, e: 1.0, 2,borderColo, r: `rgba(3, 4, 22, 1, 2, 1, 0, 0.5)`
                }}
                transition={{ duratio, n: 0.3 }}
              >
                {/* Background gradient overlay */}
                <div className={`absolute inset-0 bg-gradient-to-br ${stat.gradient} opacity-5 group-hove, r:opacity-10 transition-opacity duration-30, 0`}></div>
    ...
    scal, e: 1.1
                ...
                >
                  {React.createElement(stat.ico,  n, { classNam, e: `w-10 h-10 text-${stat.colo, r}` })}
                </motion.div>
    ...
    dela, y: index * 0.1 }}
                ...
                <motion.div
                  className="absolute inset-0 bg-gradient-to-br from-zion-cyan/5 to-zion-purple/5 rounded-3xl opacity-0 group-hove,  r:opacity-100 transition-opacity duration-300"
                  initial={false}
                />
              </motion.div>
            </motion.div>
    ...
    dela, y: 0.6
          ...
          className="mt-16"
        >
          <div className="bg-gradient-to-r from-zion-cyan/20 to-zion-purple/20 border border-zion-cyan/30 rounded-3xl p-8 l, g:p-12 text-center">
            <motion.div
