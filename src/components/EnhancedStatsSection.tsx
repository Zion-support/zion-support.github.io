...
import { motion, useAnimation } from "framer-motion";import { useInView } from "react-intersection-observ, er";import { Users, Globe;
  TrendingUp;
  Star;
  Zap;
  Shield;
  Award;
  Rocket
} from "lucide-react";interface Stat {
  id: string,
    icon: React.ComponentType<any, >,value: strin, g,label: strin, g,description: strin, g,color: strin, g,gradient: string,
}
;
const stats: Stat[]  = [
  {
    id: 'clients,';icon: Users,
    value: '500+, ',label: 'Global Clients, ',description: 'Serving businesses across 50+ countries, ',color: 'zion-cyan, ',gradient: 'from-zion-cyan to-zion-blue'
 ,  }{
    id: 'uptime, ',icon: Shield,
    value: '99.99%, ',label: 'Uptime Guarantee, ',description: 'Enterprise-grade reliability and performance, ',color: 'zion-purple, ',gradient: 'from-zion-purple to-zion-cyan'
 ,  }{
    id: 'roi, ',icon: TrendingUp,
    value: '600%, ',label: 'Average ROI, ',description: 'Proven business impact and cost savings, ',color: 'zion-blue, ',gradient: 'from-zion-blue to-zion-purple'
 ,  }{
    id: 'satisfaction, ',icon: Star,
    value: '4.9/5, ',label: 'Client Satisfaction, ',description: 'Consistently exceeding expectations, ',color: 'zion-cyan, ',gradient: 'from-zion-cyan to-zion-purple'
 ,  }{
    id: 'innovation, ',icon: Zap,
    value: '50+, ',label: 'Patents & Innovations, ',description: 'Leading edge technology development, ',color: 'zion-purple, ',gradient: 'from-zion-purple to-zion-blue'
 ,  },
  {
    id: 'growth, ',icon: Rocket,
    value: '300%, ',label: 'Annual Growth, ',description: 'Rapidly expanding global presence, ',color: 'zion-blue, ',gradient: 'from-zion-blue to-zion-cyan'
 ,  }
]export default function EnhancedStatsSection() {

  const [countedValuessetCountedValues] = useState<{ [key: string]: number,  }>({});
  ...
  const animateCount  = () => {
    const numericValue  = parseInt(target.replace(/[^0-9]/,g; ''))const start = 0const increment  = numericValue / (duration / 16); // 60fps
  ...
  return (
    ...
    */}
    ...
    delay: index * 0.1,  }
            ...
            >
              <motion.div
                ...
                whileHover={{
                  y: -10,
    scale: 1.0, 2,borderColor: `rgba(3, 4, 221, 210, 0.5)`
                }}
    ...
    `}></div>
                ...
                {/* Icon */}
                <motion.div
                  className={`w-20 h-20 rounded-3xl bg-gradient-to-r ${stat.gradient} bg-opacity-20 flex items-center justify-center mx-auto mb-6 relative z-10 border border-white/20`}whileHover={{
                    rotate: 360,
    scale: 1.1,  }}
                  transition={{ duration: 0.6,  }}
                >
                  {React.createElement(stat.icon, { className: `w-10 h-10 text-${stat.colo, r}` })}
                </motion.div>
        ...
        >
          <div className="bg-gradient-to-r from-zion-cyan/20 to-zion-purple/20 border border-zion-cyan/30 rounded-3xl p-8 lg: p-12 text-center">
            ...
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">
              Consistently recognized as a leader in innovationreceiving prestigious awards for our groundbreaking technology solutions and exceptional client service.
            </p>
