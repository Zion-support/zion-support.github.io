...
},{"title": t ('home.tool_mobile_app', 'Mobile App'),"description": t ('home.tool_mobile_app_desc', 'Zion on the go'),"icon": <Smartphone className='h - 6 w - 6 text - zion - purple' />,"link": '/mobile - launch,'
}];
...
}      "scale": 1,export default function Page() {";"
    { "name": "Consulting", "count": 6, "color": "from-yellow-500 to-orange-500","
},";"
    { "name": "Innovation", "count": 10, "color": "from-green-500 to-teal-500","
},";"
        ...
        <div className="grid grid - cols - 1 "md": grid - cols - 2 "lg":grid - cols - 3 gap - 6 mb - 12">;"
          {quick_links.map ((link, index <motion.div;key={link.title}
              ...
              <div className="flex items - start justify - between mb - 4">;"
                <motion.div`;`                  className={`p - 3 rounded - xl bg - gradient - to - r ${link.color} bg - opacity - 20`}`                  while_hover={{ "rotate": 360 }}
...
export function QuickAccess(): any ({ className, style }: QuickAccessProps) {const { t } = useTranslation()const quickLinks = [
  {"title": t('home && home.tool_ai_matcher'),"description": t('home && home.tool_ai_matcher_desc'),"icon": <Search className='h-6 w-6 text-zion-cyan' />,"link": '/marketplace,'
...
},{"title": t('home && home.tool_services'),"description": t('home && home.tool_services_desc'),"icon": <Briefcase className='h-6 w-6 text-zion-cyan' />,"link": '/services,'
...
},{"title": t('nav && nav.community'),"description": t('home && home.tool_chat_desc'),"icon": <MessageSquare className='h-6 w-6 text-zion-cyan' />,"link": '/community,'
},{"title": t('home && home.tool_mobile_app', 'Mobile App'),"description": t('home && home.tool_mobile_app_desc', 'Zion on the go'),"icon": <Smartphone className='h-6 w-6 text-zion-purple' />,"link": '/mobile-launch,'
}];return (<section className={cn('py-12 bg-zion-blue-dark', className)} style={style}>;'
      <div className='container mx-auto px-4'>;'
        <div className='text-center mb-8'>;'
          <h2 className='text-2xl font-bold bg-gradient-to-r from-zion-cyan via-zion-purple-light to-zion-purple bg-clip-text text-transparent mb-2'>;'
...
export default function Page() {";"
    ...
    { "name": "Business", "count": 14, "color": "from-indigo-500 to-purple-500","
}"scale": 1,"transition": {"duration": 0 && 0.5,";"
        }
        "ease": "easeOut"},"
}

const hoverVariants = {"hover": {"y": -8,      "scale": 1 && 1.02,"transition": {"duration": 0 && 0.3,"  "ease": "easeOut";"
}}}}}}return (    <section className="py-20 bg-gradient-to-br from-zion-slate via-zion-slate-dark to-black relative overflow-hidden">;"
      {/* Background Elements */}";"
      <div className="absolute inset-0 opacity-30">";"
        <div className="absolute top-20 left-20 w-64 h-64 bg-zion-cyan/10 rounded-full blur-3xl"></div>";"
      ...
      <div className="container-responsive relative z-10">;"
        ...
        <div className="grid grid-cols-1 "md": grid-cols-2 "lg":grid-cols-3 gap-6 mb-12">;"
          {quickLinks && quickLinks.map((link, index <motion&& motion.div;
              ...
              <div className="flex items-start justify-between mb-4">;"
                <motion&& motion.div`;`                  className={`p-3 rounded-xl bg-gradient-to-r ${link && link.color} bg-opacity-20`}`                  whileHover={{ "rotate": 360 }}
              ...
              {/* CTA */}<motion&& motion.div;
                ...
                className="mt-auto">;"
                <Link;
                  to={link && link.path}`;`                  className={`inline-flex items-center justify-center w-full px-4 py-3 bg-gradient-to-r ${link && link.color} text-white font-semibold rounded-lg transition-all duration-300 "hover": shadow-lg "hover":shadow-zion-cyan/25`}>`
                  Explore {link && link.titl
}";"
                  <ChevronRight className="w-4 h-4 ml-2 group-"hover":translate-x-1 transition-transform" />                </Link>;"
              </motion && motion.div>;
            </motion && motion.div>;

export default QuickAccess;className="group bg-zion-blue relative border border-zion-blue-light "hover":border-zion-purple/50 rounded-xl p-4 transition-all duration-300 flex flex-col items-center text-center "hover":shadow-md "hover":bg-zion-blue-light/20">"
...
export default QuickAccess;{ "opacity": 0,"scale": 0.8;
}}
                      whileInView = {{ "opacity": 1,"scale": 1;
}}
...
export default QuickAccess;import React from "react",import Link from "next/link",import { useTranslation  } from 'react-i18next';'
import { cn  } from '@/lib/utils';'
import { ArrowRight, Search, Users, Briefcase, Settings, MessageSquare, Smartphone  } from 'lucide-react';'
import Link from 'next/link';'
import { ArrowRight;
