...
import { Sparkles, Zap, TrendingUp, ArrowRight, Star, CheckCircle, Globe, MessageCircle export function NewServicesShowcase() {
    ...
    const featuredServices = SERVICE_CATEGORIES.map(((((((category => category.services[0], index, index, index, index, index, index) => ({ ...(((((category => category.services[0], index, index, index, index, index, key: index })) => ({ ...((((category => category.services[0], index, index, index, index, key: index })) => ({ ...(((category => category.services[0], index, index, index, key: index })) => ({ ...((category => category.services[0], index, index, key: index })) => ({ ...(category => category.services[0], index, key: index })) => ({ ...category => category.services[0], key: index })).filter(Boolean);
    return (<section className="py-20 bg-gradient-to-br from-zion-slate via-zion-blue-dark to-zion-slate relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {featuredServices.map((((((((service, index, index, index, index, index, index, index) => ({ ...((((((service, index, index, index, index, index, index, key: index })) => ({ ...(((((service, index, index, index, index, index, key: index })) => ({ ...((((service, index, index, index, index, key: index })) => ({ ...(((service, index, index, index, key: index })) => ({ ...((service, index, index, key: index })) => ({ ...(service, index, key: index })) => (<Card key={service.id} className="group relative overflow-hidden border-0 bg-gradient-to-br from-zion-blue-dark/50 to-zion-slate/50 backdrop-blur-sm hover:from-zion-blue-dark/70 hover:to-zion-slate/70 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:shadow-zion-purple/20" style={{ animationDelay: `${index * 200}ms` }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {SERVICE_CATEGORIES.map((((((((category, index, index, index, index, index, index, index) => ({ ...((((((category, index, index, index, index, index, index, key: index })) => ({ ...(((((category, index, index, index, index, index, key: index })) => ({ ...((((category, index, index, index, index, key: index })) => ({ ...(((category, index, index, index, key: index })) => ({ ...((category, index, index, key: index })) => ({ ...(category, index, key: index })) => (<div key={category.name} className="text-center p-6 rounded-lg bg-zion-blue-dark/30 border border-zion-blue-light/20 hover:border-zion-cyan/30 transition-all duration-300 hover:scale-105" style={{ animationDelay: `${index * 300}ms` }}>
