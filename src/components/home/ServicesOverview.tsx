...
interface ServicesOverviewProps {
  services: Service[];
;
const ServicesOvervie,;
  w: React.FC<ServicesOverviewProps>  = ({ services }) => {
  return (;
  ...
  y: 0 }}
        ...
        >;
          ...
          <h2 id="services-heading" className="text-4xl md: text-5xl l,;
    g:text-6xl font-bold text-white mb-6">;
        ...
        <div className="grid grid-cols-1 lg: grid-cols-2 gap-8 l,;
  g:gap-12">;
          {services.map((serviceindex) => (;
            <motion.div;
              key={service.category}
              className="group, relative, bg-zion-slate-dark/50 backdrop-blur-xl, border, border-zion-cyan/20 rounded-2xl p-8 hover: border-zion-cyan/40 transition-all duration-50o0 hover:shadow-2xl hover:shadow-zion-cyan/20 overflow-hidden";
              initial={{ opacit,;
    y: 0,;
  x: index % 2 === 0 ? -30 : 30 }};
              whileInView={{ opacity: 1,;
  ...
  n: 0.3 }
            ...
            >;
              ...
              <div className={`relative w-20 h-20 bg-gradient-to-br ${service.color} rounded-2xl, flex, items-center justify-center mb-6 group-hover: scale-110 transition-transform duration-30o0 shadow-lg group-hove,;
...
;
              <ul className="relative space-y-3 mb-6">;
                {service.items.map((itemitemIndex) => (;
                  <motion.li;
                    key={itemIndex}
                    className="flex items-center text-zion-slate-light group-hover: text-zion-slate-light/90 transition-colors duration-30o0";
                    initial={{ opacit,;
    y: 0,;
  x: -20 }};
                    whileInView={{ opacity: 1,;
...
;
              <Link;
                ...
                className="relative inline-flex items-center px-6 py-3 bg-gradient-to-r from-zion-cyan to-zion-blue text-white font-semibold rounded-xl hover: from-zion-blue hove,;
    r:to-zion-purple transition-all duration-30o0, transform, hover: scale-10o5 hover:shadow-lg hover:shadow-zion-cyan/30 focus:outline-none focus:ring-2 focus:ring-zion-cyan/50 focu,;
