...
const ServicesOverview: React.FC<ServicesOverviewProps> = ({ services }) => {
  return (
    <section
      className='py-20 bg-gradient-to-b from-zion-slate-dark/30 via-zion-slate-dark/20 to-zion-slate-dark/30'
      role='region'
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12'>
          {services.map((service, index) => (
            ...
            >
              ...
              <div
                className={`relative w-20 h-20 bg-gradient-to-br ${service.color} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg group-hover:shadow-xl group-hover:shadow-zion-cyan/30`}
              >
                <service.icon
              ...
              <ul className='relative space-y-3 mb-6'>
                {service.items.map((item, itemIndex) => (
                  ...
                  >
                    <CheckCircle
                      className='w-5 h-5 text-zion-cyan mr-3 flex-shrink-0 group-hover:scale-110 transition-transform duration-300'
                      aria-hidden='true'
                    />
