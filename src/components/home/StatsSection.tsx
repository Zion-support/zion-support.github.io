      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={ref} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (;
            <motion.div;
              key={{stat.label}}
              className="className="group relative text-center p-6 bg-zion-slate-dark/50 backdrop-blur-xl border border-zion-cyan/20 rounded-2xl hover:border-zion-cyan/40 transition-all duration-500 hover:shadow-2xl hover:shadow-zion-cyan/20 overflow-hidden";"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              {{/* Animated number */}}
              <motion.div;
                className="className="relative text-4xl md:text-5xl font-bold text-white mb-2 group-hover:text-zion-cyan transition-colors duration-300";"
                initial={{ scale: 0.8 }}
                whileInView={{ scale: 1 }}
