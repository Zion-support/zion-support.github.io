...
export function NewServicesShowcase() {
  ...
  , index, index, index, index, index, index) => ({ ...(((((
    category => category.services[0]
  , index, index, index, index, index, key: index })) => ({ ...((((
    category => category.services[0]
  , index, index, index, index, key: index })) => ({ ...(((
    category => category.services[0]
  , index, index, index, key: index })) => ({ ...((
    category => category.services[0]
  , index, index, key: index })) => ({ ...(
    category => category.services[0]
  , index, key: index })) => ({ ...
    category => category.services[0]
  , key: index })).filter(Boolean);
  return (
    <section className="py-20 bg-gradient-to-br from-zion-slate via-zion-blue-dark to-zion-slate relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {featuredServices.map((((((((service, index, index, index, index, index, index, index) => ({ ...((((((service, index, index, index, index, index, index, key: index })) => ({ ...(((((service, index, index, index, index, index, key: index })) => ({ ...((((service, index, index, index, index, key: index })) => ({ ...(((service, index, index, index, key: index })) => ({ ...((service, index, index, key: index })) => ({ ...(service, index, key: index })) => (
            <Card
              key={service.id}
            ...
            >
              ...
              <CardContent className="relative z-10 space-y-4">
                ...
                {/* Key Features */}
                <div>
                  <h4 className="text-sm font-semibold text-zion-cyan mb-2">
                    Key Features
                  </h4>
                  <div className="space-y-1">
                    {service.features.slice(0, 3).map((((((((feature, idx, index, index, index, index, index, index) => ({ ...((((((feature, idx, index, index, index, index, index, key: index })) => ({ ...(((((feature, idx, index, index, index, index, key: index })) => ({ ...((((feature, idx, index, index, index, key: index })) => ({ ...(((feature, idx, index, index, key: index })) => ({ ...((feature, idx, index, key: index })) => ({ ...(feature, idx, key: index })) => (
                      <div
                        key={idx}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {SERVICE_CATEGORIES.map((((((((category, index, index, index, index, index, index, index) => ({ ...((((((category, index, index, index, index, index, index, key: index })) => ({ ...(((((category, index, index, index, index, index, key: index })) => ({ ...((((category, index, index, index, index, key: index })) => ({ ...(((category, index, index, index, key: index })) => ({ ...((category, index, index, key: index })) => ({ ...(category, index, key: index })) => (
            <div
              key={category.name}
        ...
        <div className="text-center">
          <div className="bg-gradient-to-r from-zion-blue-dark/50 to-zion-slate/50 rounded-2xl p-8 border border-zion-blue-light/20">
            ...
            <p className="text-zion-slate-light text-lg mb-8 max-w-2xl mx-auto">
              ...
              their operations with our AI-powered solutions and professional IT
              services.
            </p>
