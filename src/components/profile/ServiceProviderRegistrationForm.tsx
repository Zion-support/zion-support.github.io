...
import { logWarn, logErrorToProduction } from '@/utils/productionLogger',import {
  Form,
  FormControl,
  FormDescription,
  FormField,
...
export function ServiceProviderRegistrationForm() {const { user} = useAuth();
  ...
  // Initialize form with default values,
const form = useForm<ServiceFormValues>({resolver: zodResolver(serviceProfileSchema) as any, defaultValues: {
      name: user?.displayName || "", title: "", bio: "", location: "", services: "", hourlyRate: "", availability: "available", enhancedProfile: false, website: ""}});
...
if (data && typeof data === 'object') {
        ...
        toast({
          ...
          description: "AI has created a professional bio and suggested additional services for your profile."})      } else {
...
logWarn('Mock AI response - using fallback content');
        setGeneratedContent({
          summary: "Professional service provider with expertise in delivering high-quality solutions.", services: ["Consulting", _"Project Management", _"Technical Support"]});
        
        toast({title: "Enhanced Profile Generated", description: "AI has created a professional bio and suggested additional services for your profile."})
      }
      
    } catch (error: unknown) {logErrorToProduction('Error generating enhanced profile:', _{ data: error});
      toast({title: "Generation failed", description: error.message || "There was an error generating your enhanced profile. Please try again.", variant: "destructive"})
...
if (userEmail && values.enhancedProfile) {try {
          ...
          logErrorToProduction('Failed to send notification email:', { data: emailError }),          // Continue with submission even if email fails
        }
      }
...
setTimeout(() => {
      ...
      // Redirect to service provider dashboard or profile page,
setTimeout_(() => {router.push('/service-dashboard')}, 1500)
      
    } catch (error: unknown) {logErrorToProduction('Error creating profile:', _{ data: error});
      toast({title: "Error Creating Profile", description: error.message || "There was an error creating your profile. Please try again.", variant: "destructive"})
