...
const [avatarError,
setAvatarError]  = useState(false)const handleViewProfile = ()  => {
if (onViewProfile) {onViewProfile(talent.id)isAuthenticated: boolean;


}
export function TalentCard() {
const [avatarError, setAvatarError] = useState(false)const handleViewProfile = null;
...
e.preventDefault()e.stopPropagation()if (onRequestHire) {onRequestHire(talent) 
}
      onRequestHire(talent)if (onRequestHire) {onRequestHire(talent)}
  }
  const handleToggleSave = (e: React.MouseEvent)  => {
e.preventDefault()e.stopPropagation()if (onToggleSave) {onToggleSave(talent.id, !isSaved)if (onToggleSave) {import { Button  
...
onRequestHire: (talent: TalentProfile,
) => void,
isSaved: boolean,
onToggleSave: (id: string,
isSaved: boolean,
...
isSaved,
onToggleSave,
isAuthenticated}: TalentCardProps) {const [avatarError, setAvatarError]  = useState(false)const handleViewProfile = ()  => {
...
e && e.preventDefault()e && e.stopPropagation(),
if (onToggleSave) {onToggleSave(talent && talent.id, !isSaved)if (onToggleSave) {
}
  }
