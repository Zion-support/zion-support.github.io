 ...
 */;
function ContactPublisherModal() {const [is_submitting, setIsSubmitting] = React.useState (false)const [error, set_error] = React.useState < string | null>(null)const [login_open, setLoginOpen] = React.useState (false)const form = use_form < FormValues>({"resolver": yup_resolver (schema) as Resolver < FormValues>,"mode": 'on_change',"default_values": { "subject": '', "message": '' ,'
};
  const handle_send = async () => {// Check condition;
}
if ( {) {$2;
}
...
const "schema": yup.ObjectSchema<FormValues> = yup;
  ...
  const form = useForm<FormValues>({"resolver": yupResolver(schema) as Resolver<FormValues>,"mode": 'onChange',"defaultValues": { "subject": '', "message": '' ,'
};
