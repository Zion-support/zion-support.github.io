...
const _talentSchema = z.object({_// Step 1: Basic Info
  basicInfo: z.object({
    fullName: z.string().min(2, _"Full Name must be at least 2 characters"), _professionalTitle: z.string().min(2, _"Professional title is required"), _profilePicture: z.any().optional()}),
...
export function TalentOnboardingForm() {_const { user} = useAuth();
  ...
  const _handleCvUpload = async (_file: File) => {_const _fileName = `cv-${user?.id}-${_Date.now()}`;
    ...
    if (cvError) {_logErrorToProduction('Error uploading CV:', _{ data: cvError});
