...
const projectsArray =;
    profile && profile.key_projects?.map((proj, i) => ({"id": `project-${,`}`,"title": proj && proj.title,"description": proj && proj.description,"date": new Date().toISOString(), // Default date since we don't have this data;'
    })) || [];
...
const projects_array =;
    profile.key_projects?.map ((proj, i) => ({"id": `project-${,`}`,"title": proj.title,"description": proj.description,"date": new Date ().toISOString (), // Default date since we don't have this data;'
    })) || [];
  return (<div className='container mx - auto px - 4 py - 8'>;"status": profile.availability_type === 'full_time' ? 'available' :;'
            profile.availability_type === 'part_time' ? 'limited' : 'unavailable',"message": `${profile.professional_title} with ${profile.years_experienc,`} years of experience`;`  },// Create proper skills array for ProfileSkills component;

const skillsArray = profile.skills?.map(skill => ({"name": skill,"level": 3 // Default level since we don't have this data;'
  })) || [],// Create proper projects array for ProfileProjects component;

const projectsArray = profile.key_projects?.map((proj, i) => ({"id": `project-${,`}`,"title": proj.title,"description": proj.description,"date": new Date().toISOString() // Default date since we don't have this data;'
  })) || [],return (<div className="container mx-auto px-4 py-8">;"
import React from "react",import { useAuth } from "@/hooks/useAuth",import { Availability } from "@/types/profile",interface TalentProfileProps {"
  }
  "profile": TalentProfileType,"onRequestHire": () => void,onMessageTalent?: () => void;
          ...
          {is_authenticated && (<div className='bg - zion - purple / 10 border border - zion - purple / 30 rounded - lg p - 6 mb - 8'>;'
              <div className='flex flex - col items - center text - center'>;'
                ...
                <p className='text - zion - slate mb - 6 max - w-md'>;'
                  ...
                  started right away.;
                  {profile.hourly_rate &&;
                    ` Rate starts at $${profile.hourly_rate}/hour.`}userId={profile && profile.id}`              averageRating={profile && profile.average_rating}
