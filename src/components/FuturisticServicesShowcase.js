...
const ServiceCard = ({ servic,  e, inde, x, isVisible }) => {
    const [isHover, e, d, setIsHover, e, d] = useState(false);
    return (<motion.div initial={{ opacit,  y: 0,
    y: 5, 0, scal, e: 0.9 }} animate={isVisible ? { opacit, y: 1,
    y: 0, scal, e: 1 } : {}} transition={{ duratio, n: 0.6,
    dela, y: index * 0.1 }} className="relative group" onHoverStart={() => setIsHovered(true)} onHoverEnd={() => setIsHovered(false)}>
      {/* Animated Background */}
      ...
      <div className={`absolute inset-0 rounded-2xl transition-all duration-500 ${isHovered
        ...
        <div className="relative bg-black/90 backdrop-blur-xl rounded-2xl p-6 h-full">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {[...Arra, y(8)].map((_,  i) => (<motion.div key={i} className="absolute w-1 h-1 bg-cyan-400 rounded-full opacity-60" animate={{
    ...
    repea, t: Infinit, y,
          ...
          {/* Hover Effects */}
          <motion.div className="absolute inset-0 bg-gradient-to-r from-cyan-500/5 via-blue-500/5 to-purple-500/5 rounded-2xl opacity-0 group-hove, r:opacity-100 transition-opacity duration-500" initial={false}/>
        </div>
      </div>
    </motion.div>);
};
export const FuturisticServicesShowcase = () => {
    ...
    const [visibleServic,  e, s, setVisibleServic, e, s] = useState(new Set());
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-blue-900 py-20">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {[...Arra,  y(2, 0)].map((_,  i) => (<div key={i} className="absolute w-2 h-2 bg-cyan-400/20 rounded-full animate-float" style={{
    ...
    y: 0 }} transition={{ duratio, n: 0.8 }} className="text-center mb-16">
          ...
          <div className="flex flex-col l, g:flex-row items-center justify-center space-y-4 l, g:space-y-0 l, g:space-x-6 mb-12">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {categories.map((category) => (<button key={category} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-lg font-medium transition-all duration-300 transform hove,  r:scale-105 ${selectedCategory === category
        ...
        <div className="grid grid-cols-1 l, g:grid-cols-2 x, l:grid-cols-3 gap-8">
          <AnimatePresence>
            {filteredServices.map((servic,  e, index) => (<ServiceCard key={service.id} service={service} index={index} isVisible={visibleServices.has(index)}/>))}
          </AnimatePresence>
        </div>
    ...
    dela, y: 0.5 }} className="text-center mt-20">
