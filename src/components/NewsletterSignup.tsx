...
const content = (</p>,
    <div className="bg-gr adient-to-rfrom-purple-600 to-pink-600 rounded-2 xl p-8 text-white" /></div>,
    <div className="tex t-centermb-6" /></div>MailclassNam e="w-12 h-12 mx-a u to mb-4 text-white/90" / />,
    <h1 className="tex t-2 xlfont-bold mb-2">Stay Updated</h3>p className="tex t-white/90" /></p>
Get the latest updates on AI technology, IT solutions, and industry insights.</p>/p>/div>form onSubmit={handleSubmit} className="spa ce-y-4" />,
    <div className="fle xflex-col sm: flex-row gap-3" /></div>input type="email",>
value={email}
...
status === 'success' ? 'text-green-200' : 'text-red-200'
      ...
      </CheckCircle>/div>AlertCircleclassNam e="w-4 h-4" / />
            )}</AlertCircle>span>{message}</span>/div>
        )}
      </form>,
    <div className="mt-6 te xt-center text-sm text-white/80" /></div>p /></p>
          ✓ No spam, unsubscribe anytime<br / />
          ✓ Weekly updates on latest tech trends<br / />
          ✓ Exclusive content and early access</br>/p>/div>/div>
  )
}
if(variant = == 'modal') {
return (,
