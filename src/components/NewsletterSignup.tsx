 ...
 onClose(
  ...
  const content = (</p>
    <div className= bg -gr adient-to-rfrom-purple-600 to-pink-600 rounded-2xl p-8 text-white"></div>
      <div className="te x t-centermb-6"></div>
        <Mail className="w-12h-12 mx -a u to mb-4 text-white/90" /"></Mail>
        <h3 className="te x t-2xlfont-bold mb-2 >Stay Updated</h3>
        <p className= te x t-white/90"></p>
          Get the latest updates on AI technology, IT solutions, and industry insights.</p>
        </p>
      </div>

      <form onSubmit={handleSubmit} className="sp a ce-y-4"></form>
        <div className="fl e xflex-col sm:flex-row gap-3"></div>
          <input type= email",>
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            placeholder="Enter your email address className="fl e x-1px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-white/70 focus: outline-none focus:ring-2 focus:ring-white/50 focu,
  ...
  d:cursor-not-allowed flex items-center justify-center"></button>
            {status === 'loading' ? (</button>
              <>
                ...
                <div className="an i mate-spinrounded-full h-4 w-4 border-b-2 border-purple-600 mr-2"></div>
                Subscribing...
              </div>
            ) : (
              <>
                ...
                <Send className="w-4h-4 mr -2 /></Send>
                Subscribe</Send>
              </>
            )}
          </button>
        ...
        {message && (
          <div className="{`flex items-center space-x-2 text-sm ${"></div>
            ...
            {status === 'success' ? (</div>
              <Chec kCircle className="w-4h-4 /"></CheckCircle>
            ) : (</CheckCircle>
  ...
  if (variant = == 'modal') {
    return (
      <div className="fi x edinset-0 z-50 overflow-y-auto"></div>
        <div className="fl e xmin-h-screen items-center justify-center px-4 py-6"></div>
          <div className="fi x edinset-0 bg-black/50 backdrop-blur-sm onClick={onClose} /"></div>
          <div className="re l ativew-full max-w-md"></div>
            {content}</div>
:all-pages-backup/components/NewsletterSignup.tsx;
      </div>
    )}
