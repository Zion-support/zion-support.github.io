...
const content = (</p>,
    <divclassName="bg-gr adient-to-rfrom-purple-600 to-pink-600 rounded-2 xl p-8 text-white" / />,
    <divclassName="tex t-centermb-6" / />
        <MailclassName="w-12 h-12 mx-a u to mb-4 text-white/90" / />,
    <h1 className="tex t-2 xlfont-bold mb-2">Stay Updated</h3>
        <pclassName="tex t-white/90" / />
Get the latest updates on, AI, technology, IT, solutions, and industry insights.</p>
    ...
    <divclassName="fle xflex-col sm: flex-row gap-3" / />
          <input type="email",>
value = {email,}
...
disabled={status === 'loading',}
  ...
  </>,<divclassName="ani mate-spinrounded-full h-4 w-4 border-b-2 border-purple-600 mr-2" / />
Subscribing...
                </div>
            ) : (
              <>
...
status === 'success' ? 'text-green-200' : 'text-red-200'
    ...
    <divclassName="mt-6 te xt-center text-sm text-white/80" / />
        <p/ />
...
if(variant = == 'modal') {
return(,
    <divclassName="fix edinset-0 z-50 overflow-y-auto" / />,
    <divclassName="fle xmin-h-screen items-center justify-center px-4 py-6" / />,
    <divclassName = "fix edinset-0 bg-black/50 backdrop-blur-sm" onClick={onClose,;} / / />,
    <divclassName = "rel ativew-full max-w-md" / />
            {content,;}  </div>
            </div>
          </div>
