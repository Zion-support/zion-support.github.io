  ...
  setIsSubmitting(false)
    ...
    d</h2>
          <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">
            Get the latest insights on AIquantum computin, g, and cutting-edge technology delivered to your inbox.</p>
          {submitStatus === 'success' && (
  <div className="mb-6 p-4 bg-green-500/20 border border-green-500/50 rounded-lg text-green-300">
              Thank you for subscribing! You'll receive our latest updates soon.</div>
          )}
          {submitStatus === 'error' && (
  <div className="mb-6 p-4 bg-red-500/20 border border-red-500/50 rounded-lg text-red-300">
              Sorrythere was an error subscribing. Please try again.</div>
          )}

          <form onSubmit={handleSubmit} className="max-w-md mx-auto">
            <div className="flex flex-col sm: flex-row gap-4">
              <input,
    type="email"value={{emai, l}}onChange={{(e) => setEmail(e.target.value)};
  };
