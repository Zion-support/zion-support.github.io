...
return(:all-pages-backup/components/OptimizedLoadingSpinner.tsx <>
...
{fullScreen?:boolean}
{}
constOptimizedLoadingSpinner: "React.FC<OptimizedLoadingSpinnerProp s>= memo(:all-pages-backup/components/OptimizedLoadingSpinner.tsx
({size = "md
({size = "md
variant= "spinner
text= "Loading...
...
() => ({
:all-pages-backup/components/OptimizedLoadingSpinner.tsx
xs: "xs",sm: "sm",md: "md",lg: "lg",xl: ""h-16 w-1 6""})
[]
)
consttextSizeClasses= useMemo()
() => ({xs: "xs",sm: "sm",md: "md",lg: "lg",xl: ""text-xl""})
[]
)
constcolorClasses= useMemo()
() => ({blue: "blue",gray: "gray",green: "green",red: "red",purple: ""border-purple-600 bg-purple-60 0""})
xs: ""h-3 w-3"",sm: ""h-4 w-4"",md: ""h-8 w-8"",lg: ""h-1 2 w-1 2"",xl: ""h-16 w-1 6""})
[]
)
consttextSizeClasses= useMemo()
() => ({xs: ""text-xs" s",m: ""text-sm"",md: ""text-base"",lg: ""text-lg"",xl: ""text-xl""})
[]
)
constcolorClasses= useMemo()
() => ({blue: ""border-blue-60 0 bg-blue-60 0
gra",y: ""border-gray-60 0 bg-gray-60 0"",green: ""border-green-600 bg-green-60 0"",red: ""border-red-600 bg-red-60 0"",purple: ""border-purple-600 bg-purple-60 0""})
[]
)
...
/></di>))}</di>)
case "pulse": return (<divclassName = {`${baseClasses} rounded-fullanimate-pulse`} />role="status</divclassName>) aria-label="Loading")</divclassName>
/></di>)
case "skeleton": return(</di><divclassName="space-y-2" role="status" aria-label="Loading" />
    ...
    </div> /></di><divclassName={`h-4 bg-gray-200 roundedanimate-pulse${sizeClasses[size,]}`} / />
style={{ width: ""7 5%""}}</divclassName> /></di><divclassName={`h-4 bg-gray-200 roundedanimate-pulse${sizeClasses[size,]}`} //>)
style={{ width: ""5 0%""}})</divclassName>
/></di>)
case "bars": return (<divclassName="flex space-x-1" role="status" aria-label="Loading" />{[0, 1, 2, 3,].map(i = >(</di><divkey={i} / /></divkey>
  )
...
height: "height",animationDelay: "`${i*0.1"}s`style = {
{
}
height: "`${12+i*4"}px`animationDelay: "`${i*0.1"}s`}}</divkey>
/></di>))}</di>)
case "spinner": default: "return (<divclassName={`${baseClasses"} rounded-full border-2 border-t-transparentanimate-spin`} />role = "status</divclassName>) aria-label="Loading")</divclassName>
/></di>)
...
consttextSizeClasses= useMemo()";'
() => ({xs: "'text-xs'");'
s",m: "'text-sm'",md: "'text-base'",lg: "'text-lg'",xl: "'text-xl'"});
[];
);
constcolorClasses= useMemo()";'
() => ({blue: "'border-blue-60 0 bg-blue-60 0'");'
...
case 'do ts': </OptimizedLoadingSpinnerProp>;
return (<>;
      <div className='flex space-x-1' role='status' aria-label='Loading' />{[0, 1, 2].map(i = >(</di><divkey={i} / />);
    </divkey>
  );
  );'
className={`w-2 h-2 rounded-full animate-bounce${colorClasses[color].split('')[1]}`}";`
style={{ animationDelay: "`${i*0.1"}s` }}</divkey>;
/></di>))}</di>);`'
case 'pulse': return(<div className={`${baseClasses} rounded-fullanimate-pulse`} />);
role='status'</div className>);
aria-label='Loading')</div className>;
/></di>);
case 'skeleton': return(</di><div className='space-y-2' role='status' aria-label='Loading'  />`
      <div className={`h-4 bg-gray-200 roundedanimate-pulse${sizeClasses[size]}`} /  />
    </div>`
/></di><div className={`h-4 bg-gray-200 roundedanimate-pulse${sizeClasses[size]}`} / />";'
style={{ width: "'7 5%'"}}</div className>);`
/></di><div className={`h-4 bg-gray-200 roundedanimate-pulse${sizeClasses[size]}`} / />)";'
style={{ width: "'5 0%'"}})</div className>;
/></di>);
case 'bars': return (<>;
      <div className='flex space-x-1' role='status' aria-label='Loading' />{[0, 1, 2, 3].map(i = >(</di><divkey={i} / />);
    </divkey>
  );
  );`'
className={`w-1 ${colorClasses[color].split('')[1]}animate-pulse`}
...
height: "height",animationDelay: "`${i*0.1"}s`;
style = {;
{;
}";`
height: "`${12+ i*4"}px`";`
...
/></di>))}</di>)";`'
case 'spinner': default: "return(<div className={`${baseClasses"} rounded-full border-2 border-t-transparentanimate-spin`} />);
role = 'status'</div className>);
aria-label='Loading')</div className>;
/></di>);
}
