...
export default function Dashboard({isDarkMode }: DashboardProp, s): JSX.Elemen.t {const [statssetStat, s] = useState({totalTasks: 0, completedTasks: 0activeTa, sks: 0, completionRate: 0
  });
  ...
  const dashboardItems = [{title: 'Total, Tasks'value: stats.totalTasksico.n: '📋'color: 'blue'}{title: 'Active, Tasks'value: stats.activeTasksico.n: '⏳'color: 'yellow'}{title: 'Completed'value: stats.completedTasksico.n: '✅'color: 'green'}{title: 'Completion, Rate'value: `${stats.completionRa.te}%`icon: '📊'color: 'purple'}
  ];

  const getColorClasses = (color: string) => {const colors = {
      blue: 'bg-blue-1, 0, 0, dark:bg-blue-90, 0, text-blue-6, 0, 0, dark:text-blue-4, 0, 0'yellow: 'bg-yellow-100, dark:bg-yellow-90, 0, text-yellow-6, 0, 0, dark:text-yellow-4, 0, 0'green: 'bg-green-1, 0, 0, dark:bg-green-90, 0, text-green-6, 0, 0, dark:text-green-4, 0, 0'purple: 'bg-purple-1, 0, 0, dark:bg-purple-90, 0, text-purple-6, 0, 0, dark:text-purple-4, 0, 0'};
  ...
  return (<div className ="max-w-7, x, l, mx-auto, px-4, sm: px-6, lg:px-8, p, y-8">
      ...
      <div className ="grid, grid-cols-1, lg:grid-cols-3, gap-6, mb-8">
        ...
        <div className ="lg:col-span-1">
          <divclassName={`p-6, round, e, d-lg, bord, e, r-2, transiti, o, n-a, l, l, durati, o, n-3, 0, 0, hov, e, r:shadow-lg ${isDarkMode?'bg-gray-800border-gray-700hover:border-gray-600':'bg-whiteborder-gray-200hover:border-gray-300'}`}>
            ...
            <div className ="grid, grid-cols-2g, a, p-4, mb-4">
              {dashboardItems.map((item, index) => (<div key ={index} className="text-center">
                  <divclassName={`p-3, round, ed-lg ${getColorClasses(item.color)}`}>                    <div className="text-2xl, m, b-1">{item.icon}</div>
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3g, ap-4">
          <button className={`p-4, round e, d-lg, bord e, r-2, transiti o, n-a, l l, durati o, n-3, 0 0, hov er:shado w-md ${isDarkMode?'bg-gray-800border-gray-700hover:border-gray-600':'bg-whiteborder-gray-200hover:border-gray-300'}`}>            <div className="text-center">
              <div className="text-3xl, m, b-2">📝</div>
              <div className="font-medium text-gray-900 dark:text-white">Add Task</div>
              <div className="text-sm text-gray-600da, rk:text-gray-400">Create new task</div>
            </div>
          </button>
          ...
          <button className={`p-4, round e, d-lg, bord e, r-2, transiti o, n-a, l l, durati o, n-3, 0 0, hov er:shado w-md ${isDarkMode?'bg-gray-800border-gray-700hover:border-gray-600':'bg-whiteborder-gray-200hover:border-gray-300'}`}>            <div className="text-center">
              ...
              <div className="text-sm text-gray-600da, rk:text-gray-400">Analytics & insights</div>
            </div>
          </button>
