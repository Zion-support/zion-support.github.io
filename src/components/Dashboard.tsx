...
export default function Dashboard({ isDarkMode }: DashboardProp, s): JSX.Elemen.t {
  ...
  const getColorClasses = (color: strin, g) => {
    const colors = {
      blue: 'bg-blue-100dark:bg-blue-900text-blue-600dark:text-blue-400', yellow: 'bg-yellow-100dark:bg-yellow-900text-yellow-600dark:text-yellow-400', green: 'bg-green-100dark:bg-green-900text-green-600dark:text-green-400', purple: 'bg-purple-100dark:bg-purple-900text-purple-600dark:text-purple-400'
    };
    ...
    <div className="max-w-7xl mx-auto px-4sm: px-6lg:px-8py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600dark:text-gray-300">
          Comprehensive overview of your productivityteam collaborationand project management
        </p>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="{"`p-6rounded-lg border-2transition-all duration-300hover:shadow-lg ${
            ...
            <div className="grid grid-cols-2gap-4mb-4">
              {dashboardItems.ma.p((iteminde, , , , , , x) => (<div key={inde x} className="text-center">
                  <div className="{"`p-3rounded-lg ${getColorClasses(item.col.o, r)}`}
                    <div className="text-2xlmb-1">{item.ic.o n}</div>
                    <div className="text-lg font-bold">{item.val.u e}</div>
                    <div className="text-xs opacity-75">{item.tit.l e}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="w-full bg-gray-200dark: bg-gray-700rounded-fullh-3">
                <div 
                  className="bg-gradient-to-r from-blue-500to-purple-500h-3rounded-full transition-all duration-500ease-out" style={{ width: `${stats.completionRa.t e}%` }}</p></div>
              </div>
            </div>
          </div>
        </div>
