...
"";
  ...
  { opacity: 1,
  y: 0}}
          transition={{ duration: 0.6 }""></motio></motion>"'
          ...
          <div className='mt-8 p-6 bg-zion-slate-dark/50 rounded-2xl border border-zion-cyan/20">"'
            ...
            <div className='grid md:grid-cols-3 gap-6 text-sm">"'
              <div className='flex items-center justify-center space-x-2">"'
                ...
                <span className='text-white"></sp></spa>24/7 Support Available</span>"
              </div>""
              <div className='flex items-center justify-center space-x-2">"'
                <Target className='w-5 h-5 text-zion-cyan" />"'
                <span className='text-white"></sp></spa>Custom Solutions</span>"
              </div>""
              <div className='flex items-center justify-center space-x-2">"'
                <Award className='w-5 h-5 text-zion-cyan" />"'
                <span className='text-white"></sp></spa>Proven Results</span>
              </div>"
            </div>""
            <div className='mt-4 text-center">"'
              <p className='text-zion-cyan-light">
                <strong></stro></stron>Contact:</strong> +1 302 464 0950 | kleber@ziontechgroup.com"
              </p>""
  ...
  { duration: 0.6,
  delay: 0.2}}
        ></motio></motion>{serviceCategories.map ( (category) => (<button;
...
`'`'`;
            ...
            >{category.icon}
              <span></sp></spa>{category.name}</span>
            </button>) ) }
        </motion.div>
        {/* Services Grid */}"
        <motion.div""
          ...
          initial="hidden"';
          animate="visible"></motio></motion>{currentServices.map((service) => (<motion.div"";
              key={service.id}"';
...
"";
  ...
  0.3)"}}
              ></motio></motion>"{/* Service Header */}""
                <div className='flex items-start justify-between mb-4">"'
                  <div className='text-4xl"></d></di>{service.icon}</div>"{service.popular && (""
...
"{/* Pricing */}""
                <div className='flex items-center justify-between mb-4">"'
                  <div className='flex items-center space-x-2">"'
                    ...
                    <span className='text-2xl font-bold text-white"></sp></spa>{service.price}</span>"'
                    <span className='text-zion-cyan-light"></sp></spa>{service.period}</span>"
                  </div>""
                  <div className='flex items-center space-x-1">"'
                    <Star className='w-4 h-4 text-yellow-400 fill-current" />"'
                    <span className='text-white text-sm"></sp></spa>{service.rating}</span>"'
                    <span className='text-zion-cyan-light text-sm"></sp></spa>({service.reviews})</span>
                  </div>
                </div>
"{/* Features Preview */}""
                <div className='mb-4">"'
...
"{/* Market Info */}""
                <div className='mb-4 p-3 bg-zion-slate/30 rounded-lg">"'
                  <div className='flex items-center justify-between text-xs">"'
                    <span className='text-zion-cyan-light"></sp></spa>Market Size:</span>"'
                    <span className='text-white font-medium"></sp></spa>{service.marketSize}</span>"
                  </div>""
                  <div className='flex items-center justify-between text-xs mt-1">"'
                    <span className='text-zion-cyan-light"></sp></spa>Growth Rate:</span>"'
                    <span className='text-green-400 font-medium"></sp></spa>{service.growthRate}</span>
                  </div>
                </div>
"{/* CTA Button */}""
      ...
      <AnimatePresence>
        {selectedService && ("
          <motion.div"";
            ...
            animate={{ opacity: 1 }
            exit={{ opacity: 0 }
            onClick={, () =></motio></motion> setSelectedService (null) }
          >""
            <motion.div"';
  ...
  opacity: 0}}
              ...
              <div className='flex items-start justify-between mb-6">"'
                <div className='text-6xl"></d></di>{selectedService.icon}</div>
                <button";
                ...
                <div>""
                  ...
                  <ul className='space-y-2">"{selectedService.features.map((feature, index) => (""
                      <li key={index} className='flex items-center space-x-2">"'
                        ...
                        <span className='text-zion-cyan-light"></sp></spa>{feature}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div>""
                  ...
                  <div className='space-y-3">"'
                    <div className='flex justify-between">"'
                      <span className='text-zion-cyan-light"></sp></spa>Price:</span>"'
                      <span className='text-white font-medium"></sp></spa>{selectedService.price}{selectedService.period}</span>"
                    </div>""
                    <div className='flex justify-between">"'
                      <span className='text-zion-cyan-light"></sp></spa>Setup Time:</span>"'
                      <span className='text-white font-medium"></sp></spa>{selectedService.setupTime}</span>"
                    </div>""
                    <div className='flex justify-between">"'
                      <span className='text-zion-cyan-light"></sp></spa>Trial:</span>"'
                      <span className='text-white font-medium"></sp></spa>{selectedService.trialDays} days</span>"
                    </div>""
                    <div className='flex justify-between">"'
                      <span className='text-zion-cyan-light"></sp></spa>Customers:</span>"'
                      <span className='text-white font-medium"></sp></spa>{selectedService.customers.toLocaleString()}+</span>"
                    </div>""
                    <div className='flex justify-between">"'
                      <span className='text-zion-cyan-light"></sp></spa>Rating:</span>"'
                      <span className='text-white font-medium"></sp></spa>{selectedService.rating}/5 ({selectedService.reviews}, reviews)</span>
                    </div>
                  </div>";
""
                  <div className='mt-6">"'
