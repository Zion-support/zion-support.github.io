    ...
    useEffect ( () => {
        if (query.trim () ) {
            const filtered = mockSuggestions
                ...
                suggestion.category?.toLowerCase () .includes (query.toLowerCase () ) ) .sort ( (a, b) => b.relevance - a.relevance) .slice (0, 8) ;
    ...
    return (<div className="relative w - full max - w-2xl mx - auto" ref={searchRef}>
      ...
      {isExpanded && (<div className="mt - 4 p - 4 bg - white dark:bg - zion - slate border border - zion - slate - light rounded - lg shadow - lg">
          ...
          <div className="flex flex - wrap gap - 2">
            {categories.map (category => (<button key={category} onClick={ () => toggleFilter (category) } className={`px - 3 py - 1 text - xs rounded - full border transition - all duration - 200 ${selectedFilters.includes (category) ? 'bg - zion - cyan text - white border - zion - cyan'
