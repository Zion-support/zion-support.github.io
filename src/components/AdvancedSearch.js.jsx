...
const mockSuggestions = [{"id": '1', "text": 'AI Development Services', "type": 'service', "relevance": 95, "category": 'AI & ML'},
...
export function AdvancedSearch($1) {
    ...
    const handleSuggestionClick = (props) => {setQuery(suggestion.text);
        setShowSuggestions(false);
        // Here you would typically trigger a search};
    const toggleFilter = (props) => {setSelectedFilters(prev => prev.includes(category)
            ? prev.filter(f => f !== category)
            : [...prev, category]);};
    const clearFilters = (props) => {setSelectedFilters([]);};
    const getSuggestionIcon = (props) => {
        switch (type) {
    ...
    const getSuggestionColor = (props) => {
        switch (type) {
