...
export function SiteHealthDashboard() {
  ...
  useEffect(() => {
    ...
    const _performHealthCheck = async () => {
          ...
          metrics: [
            {
              ...
              recommendation:
                'Optimize images and enable compression to reach < 2s',
            },
            {
          ...
          metrics: [
            {
              ...
              recommendation:
                'Add comprehensive content to 32% of service pages',
            },
            {
              name: 'Broken Links',
              value: 45,
      ...
      const _totalScore = healthCategories.reduce(
        (sum, category) => sum + category.overallScore,
        0
      );
    ...
    <div className='max-w-7xl mx-auto p-6 space-y-8'>
      ...
      <div className='text-center'>
        ...
        <p className='text-gray-600 text-lg'>
          Comprehensive monitoring of website performance, SEO, accessibility,
          and user experience
        </p>
      </div>
      ...
      <div className='bg-white rounded-xl shadow-lg p-8 text-center'>
        ...
        <div
          className={`text-6xl font-bold ${getScoreColor(overallScore)} mb-4`}
        >
          {overallScore}
        </div>
        <div className='w-full bg-gray-200 rounded-full h-4 mb-4'>
          <div
            className={`h-4 rounded-full ${overallScore >= 90 ? 'bg-green-500' : overallScore >= 75 ? 'bg-blue-500' : overallScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`}
            style={{ width: `${overallScore}%` }}
          ></div>
        </div>
        <p className='text-gray-600'>
          {overallScore >= 90
            ? 'Excellent - Your site is performing at peak levels!'
            : overallScore >= 75
              ? 'Good - Minor improvements can boost performance.'
              : overallScore >= 60
            ...
            >
              <div className='p-6 border-b border-gray-100'>
                <div className='flex items-center justify-between mb-4'>
                  ...
                  <div
                    className={`text-2xl font-bold ${getScoreColor(category.overallScore)}`}
                  >
                ...
                {category.metrics.map((metric, metricIndex) => (
                  <div key={metricIndex} className='space-y-2'>
                    <div className='flex items-center justify-between'>
                      ...
                      <div
                        className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs border ${getStatusColor(metric.status)}`}
                      >
      ...
      <div className='bg-white rounded-xl shadow-lg p-8'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          ...
          <div className='text-center p-4 bg-blue-50 rounded-lg'>
            ...
            <p className='text-sm text-gray-600'>
              Improved bundle splitting implemented
            </p>
          </div>
