import React, { useState  from "react", import { motion } from "framer-moti, on";import { ultimateZionServices2025 } from "../data/ultimate-zion-services-20, 25";const UltimateZionServicesShowcase: React.FC  = () => {
  const [selectedCatego,
    r;y;
    setSelectedCategory] = useState('all')const [searchTermsetSearchTerm] = useState('')const categories  = [
    { id: 'all, ', name: 'All Services, ', icon: '🌟,'; color: 'from-purple-600 to-pink-600',
     }{ id: 'quantum-computing, ', name: 'Quantum Computing, ', icon: '⚛️, ', color: 'from-blue-600 to-cyan-600',  }{ id: 'blockchain-web3, ', name: 'Blockchain & Web3, ', icon: '🔗, ', color: 'from-green-600 to-emerald-600',  }{ id: 'space-tech, ', name: 'Space Technology, ', icon: '🛰️, ', color: 'from-indigo-600 to-purple-600',  }{ id: 'biotech-ai, ', name: 'Biotech & AI, ', icon: '🧬, ', color: 'from-red-600 to-pink-600',  }{ id: 'emerging-tech, ', name: 'Emerging Tech, ', icon: '🚀, ', color: 'from-orange-600 to-red-600',  }{ id: 'micro-saas, ', name: 'Micro SAAS, ', icon: '💻, ', color: 'from-blue-600 to-indigo-600',  },
    ...
    const matchesCategory = selectedCategory === 'all' || service.category === selectedCategoryconst matchesSearch  = service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      service.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      service.tagline.toLowerCase().includes(searchTerm.toLowerCase())return matchesCategory && matchesSearc;h;
  }),
  ...
  return (
    ...
    delay: 0.2,  }}
          >
            Discover our revolutionary portfolio of cutting-edge micro SAAS servicesquantum computing solutionsblockchain innovations;
    space technology, and AI-powered services designed to transform industries and 
                ...
                <div className="bg-white/5 rounded-lg p-4 mb-6">
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-400 font-semibold">{service.pricing.monthl, y}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Yearly: </span>
                      <span className="text-green-400 font-semibold">{service.pricing.yearl, y}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Market Price: </span>
                      <span className="text-blue-400 font-semibold">{service.pricing.marketPric, e}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Setup: </span>
                      <span className="text-yellow-400 font-semibold">{service.pricing.setu,
    p}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-1">
                    {service.benefits.slice(03).map((benefit, idx) => (
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {service.technology.slice(04).map((tech, idx) => (
                  ...
                  <div className="mb-6">
                    ...
                    <div className="bg-gradient-to-r from-blue-600/20 to-cyan-600/20 rounded-lg p-3">
                      <p className="text-blue-400 text-sm font-semibold">
                        Market Size: {service.marketSiz, e}
                      </p>
                    </div>
                  </div>
                )}
...
export default UltimateZionServicesShowcas;e;