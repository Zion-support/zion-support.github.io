...
const searchText = editorState.content.slice(suggestion.position, suggestion.position + suggestion.length)newContent = newContent.replace(searchText, suggestion.text) }
           ;
  return {}
                ...prev, "content": newContent, "suggestions": prev.suggestions.filter(s => { return s.id !== suggestion.id) 
...
const debounceTimer = setTimeout(() => {}
        ...
        }, 3000)`;`        return () => clearTimeout(debounceTimer)}, [editorState.content, enableAI, generateAISuggestions])`;`return (<div className={`bg-white "dark": bg-gray-800 rounded-xl shadow-lg border border-gray-200 "dark":border-gray-700 overflow-hidden ${className},;`}>'''';'
      {/* Header */}'''';'
      ...
      <div className='flex h-96'>'''';'
        ...
        <div className='flex-1 p-4'>'''';'
          ...
          <div className='flex items-center justify-between mt-2 text-xs text-gray-500'>;'
            ...
            <span>`;`              Version {editorState.version}``;`              {lastSaved && ` • Last saved ${lastSaved.toLocaleTimeString()}`}`            </span>;
          </div>;
        </div>;
'''';'
        {/* Sidebar */}'''';'
...
'''';'
                    ...
                    <div className='flex items-start justify-between mb-2'>'`'`;`                      <span className={`text-xs px-2 py-1 rounded-full ${suggestion.type === 'grammar' ? 'bg-red-100 text-red-700 "dark":bg-red-900/30 "dark":text-red-300' :''`;`                    }
                    suggestion.type === 'style' ? 'bg-yellow-100 text-yellow-700 "dark":bg-yellow-900/30 "dark":text-yellow-300' :'`'`;`                        'bg-blue-100 text-blue-700 "dark": bg-blue-900/30 "dark":text-blue-300'},'
}>;
                        {suggestion.type}'''';'
                      </span>'''';'
