...
interface ApplyToJobFormProps {onSuccess?: () => void}origin/main
...
id="coverLetter"
            value = {coverLetter}
            onChange = {(e) => setCoverLetter(e.target.value)}origin/main
            rows = {6}
        ...
        <div>
          ...
          ) : resumes && resumes.length > 0 ? (<Selectorigin/main
...
variant="outline"
                size="sm"
                type="button"
                onClick = {() => router.push("/dashboard/talent/portfolio")}origin/main
              >
                Create Resume
              </Button>
            </div>
...
id="cvUpload"
            ...
            accept=".pdf"
            className="mt-1"
            onChange = {(e) => setResumeFile(e.target.files?.[0] |null)}origin/main
          />
        </div>
      </div>
