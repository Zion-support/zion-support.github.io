...
export function ApplicationProgress(_{status, className}: ApplicationProgressProps) {_// Define the progress value based on status,
const getProgressValue = () => {
    switch (status) {
      case "new": return 20,
      case "viewed": return 40,
...
return <CircleDot className="h-4 w-4 text-blue-500" />
    ...
    // This step is upcoming,
return <Circle className="h-4 w-4 text-muted-foreground/50" />  }
function StatusIcon(_{status, current}: {status: ApplicationStatus, current: ApplicationStatus}) {_// Helper to determine if this step is active, completed, or inactive,
const statusRank: Record<ApplicationStatus, number> = {
    new: 1, viewed: 2, shortlisted: 3, interview: 4, hired: 5, rejected: 5};
