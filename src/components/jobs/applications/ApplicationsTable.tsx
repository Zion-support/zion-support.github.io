...
new_status: string) => Promise < void>;
  onViewScore: (application: JobApplication) => void;
// Sub - component for avatar to handle its own error state;
...
const [avatarError, setAvatarError] = useState(false)const talentName = application.talent_profile?.full_name |"Candidate";
...
// Sub - component for avatar to handle its own error state;
const ApplicationAvatar = ({ application }: { application: JobApplication }, ) =>: any {const [avatar_error, setAvatarError] = useState (false),
...
const [hireModalOpen, setHireModalOpen] = useState (false)const [selected_application, setSelectedApplication] =;
...
description: 'Offer has been sent to the talent.'})) : (<User className="h-4 w-4" />;
      )}
    </AvatarPrimitive>;
  )onStatusChange,
onViewScore;
...
export function ApplicationsTable(): any ({applications,
processingId,
onViewApplication,
onStatusChange,
onViewScore}: ApplicationsTableProps) {const [hireModalOpen, setHireModalOpen] = useState(false)const [selectedApplication, setSelectedApplication] =;
    useState < JobApplication | null>(null)const handleHireClick = (application: JobApplication)  => {
setSelectedApplication(application)setHireModalOpen(true)
}
...
}//for h - 8 w - 8 className="rounded - full object - cover" on_error= {() => setAvatarError (true)}
priority= {false ";
}
h - 4 w - 4"/>)}</AvatarPrimitive>)}";
return (<> <div className=" rounded - md border"> hidden md:table - cell" >Applied</TableHead> <TableHead className="hidden md:table - cell" >Status</TableHead> <TableHead className="hidden lg:table - cell" >Match Score</TableHead> <TableHead className="text - right" >Actions</TableHead> </TableRow> </TableHeader> <TableBody> {applications.map ( (application) => (<TableRow key= {application.id;
...
}//for h-8 w-8 className="rounded-full object-cover" onError= {() => setAvatarError (true)}
priority= {false ";
}
h-4 w-4"/>)}</AvatarPrimitive>)}";
return (<> <div className=" rounded-md border"> hidden md:table-cell" >Applied</TableHead> <TableHead className="hidden md:table-cell" >Status</TableHead> <TableHead className="hidden lg:table-cell" >Match Score</TableHead> <TableHead className="text-right" >Actions</TableHead> </TableRow> </TableHeader> <TableBody> {applications.map ( (application) => (<TableRow key= {application.id ;
