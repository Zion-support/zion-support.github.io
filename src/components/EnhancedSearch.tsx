...
import { Search, X, TrendingUp, Clock, Sparkles, Brain, ArrowRight } from 'lucide-react';
...
export function EnhancedSearch({
	...
	const handleSearch = useCallback(async (searchQuery?: string) => {
		...
		const filtered = mockSearchResults.filter(item =>
			item.title.toLowerCase().includes(queryToSearch.toLowerCase()) ||
			item.description.toLowerCase().includes(queryToSearch.toLowerCase())
		);
		...
		if (onSearch) onSearch(queryToSearch);
	}, [query, onSearch]);
	...
	return (
		<div className={`relative ${className}`} ref={searchRef}>
			<div className={`flex items-center rounded-xl ${getVariantClasses()}`}>
				<div className="pl-3">
					{getSearchIcon()}
				</div>
				<input
								...
								<div className="space-y-2">
									{suggestions.map((suggestion, index) => (
										<motion.button
											...
											initial={{ opacity: 0, x: -20 }}
											animate={{ opacity: 1, x: 0 }}
											transition={{ delay: (index + results.length) * 0.05 }}
