...
import { SearchXFilterSortAscSortDescClockStarTag } from 'lucide-react';
...
export default function EnhancedSearch({
  ...
  }[filterssortBysortOrdermaxResultsonSearchenableHistory]);
  ...
  }[performSearchgenerateSuggestionsenableSuggestionsdebounceMs]);
  ...
  return (
    <div className = "relative">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div            className="absolute z-50 mt-1 w-full bg-white rounded-lg shadow-lg border border-gray-200 max-h-96 overflow-y-auto"            initial={{ opacity: 0y: -10 }}
            ...
            {results.length > 0 && !is Loading && (              <div class Name="p-2""">                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2>                  Results ({results.length})
                ...
                {results.map((result index) => (
                  ...
                  >                    <div class Name="flex" items-start justify-between"">                      <div className="flex-1>                        <h 4 class Name="text-sm font-medium text-gray-900"" id="resulttitle">{result.title}</h4>                        <p className="text-xs text-gray-600 mt-1">{result.description}</p>                        <div className="flex items-center mt-2 space-x-2>                          <span class Name="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded"">
                          ...
                          {result.tags?.slice(02).map(tag => (                            <span key={tag} className="text-xs px-2 py-1 bg-green-100 text-green-600 rounded flex items-center">                              <Tag className="h-3 w-3 mr-1" />
                              {tag}
                            </span>
                          ))}
                        </div>
