...
interface EnhancedSearchProps { onSearch?: (query: string, results: SearchResult[]) => void;
  onResultClick?: (resul,
    t: SearchResult) => void;
  placeholder?: string;
...
>>>>>>> 7 f 723505c7d69fdcdfb649a50c1163e3919b1408:src/components/Enhanced Search.tsx
            ...
            {/* Suggestions */}
            {enable Suggestions && suggestions.length > 0 && !is Loading && (<div class Name="p-2""">                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 flex items-center">                  <Star className="h-3 w-3 mr-1"" />
                  Suggestions
                </div>
                ...
                {results.map((result, index) => (
                  <motion.div                    key={result.id}
                    className="{`p-3 rounded cursor-pointer ${
                      index === selected Index ? 'bg-blue-50 border border-blue-200' : 'hover:bg-gray-50'                    }`}
                    on Click={() => handle Result Click(result)}
                    while Hover={{ scale: 1.01 }}
                  >                    <div class Name="flex" items-start justify-between"">                      <div className="flex-1">                        <h4 className="text-sm font-medium text-gray-900"" id="resulttitle">{result.title}</h4>                        <p className="text-xs text-gray-600 mt-1"">{result.description}</p>                        <div className="flex items-center mt-2 space-x-2">                          <span className="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded"">
                            {result.type}
                          </span>
