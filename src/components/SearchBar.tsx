...
export function SearchBar({}: SearchBarProps) {  const router = useRouter()export function SearchBar() {const router = useRouter()const [suggestions, setSuggestions] = useState<SearchSuggestion[]>([])const [focused, setFocused] =;
  useState(false);
...
const controller = new AbortController()fetch(`/api/search/suggest?q=${encodeURIComponent(debounced)}`, {signal: controller.signal;
    }).then(res = > {if (!res.ok) throw new Error('Failed to fetch suggestions';
