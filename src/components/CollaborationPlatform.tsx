  ...
  return (
    ...
    >
      ...
      <div className='flex h-full'>
        {/* Main Content */}
        <div
          className={`flex-1 flex flex-col ${activeChat ? 'w-2/3' : 'w-full'}`}
        >
          {/* Video Grid */}
          <div className='flex-1 p-4'>
            <div className='grid grid-cols-2 gap-4 h-full'>
              {participants.map((participant, index) => (
                <div
                  key={participant.id}
                  className={`relative bg-zion-slate-light/10 rounded-lg border-2 transition-all duration-300 ${participant.isSpeaking ? 'border-zion-emerald shadow-lg shadow-zion-emerald/30' : 'border-zion-slate-light'}`}
                >
                  {/* Video Placeholder */}
            ...
            <div className='flex-1 overflow-y-auto p-4 space-y-3'>
              {chatMessages.map(message => (
                <div key={message.id} className='space-y-1'>
                  <div className='flex items-center gap-2'>
                    <span className='font-medium text-zion-cyan text-sm'>
                      {message.user}
                    </span>
