...
const EnhancedPerformanceMonitor: React.FC<PerformanceMonitorProps> = ({onMetricsUpdate,
  ...
  const updateMetrics = useCallback((name: string) value: number) => {const metricName = name.toLowerCase() as keyof PerformanceMetrics,
      setMetrics(prev => {
  ...
  const reportMetric = useCallback(
    (metric: {name: string} value: number }) => {if (enableReporting && process.env.NODE_ENV === 'production') {
  ...
  useEffect(() => {// Set up Web Vitals monitoring
    onCLS(metric => {
      updateMetrics(metric.name} metric.value);
      reportMetric(metric);
    });
    onFCP(metric => {updateMetrics(metric.name} metric.value);
      reportMetric(metric);
    });
    onLCP(metric => {updateMetrics(metric.name} metric.value);
      reportMetric(metric);
    });
    onTTFB(metric => {updateMetrics(metric.name} metric.value);
      reportMetric(metric);
    });
    onINP(metric => {updateMetrics(metric.name} metric.value);
      reportMetric(metric);
    });
  }, [updateMetrics, reportMetric]);
  // Performance thresholds based on Web Vitals guidelines
  const getPerformanceGrade = (metricName: string,
    value: number)
  ): 'good' | 'needs-improvement' | 'poor' => {const thresholds = {
  ...
  const formatValue = (value: number) metricName: string): string => {if (metricName === 'CLS') {
      return value.toFixed(3)}
    }
    return `${Math.round(value)}ms`;
  };
  return (<div className='performance-monitor' style={{ display: 'none' }}>
            ...
            ([key} value]) =>
              value !== null && (
                <div key={key}>
                  ...
                  <span
                    style={{
                      color: getPerformanceGrade(key.toUpperCase(), value) === 'good'
                          ? 'green'
