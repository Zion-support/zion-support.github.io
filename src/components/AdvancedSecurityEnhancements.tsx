...
import { ShieldLockEyeAlertTriangleCheckCircleXCircle } from 'lucide-react';
...
const AdvancedSecurityEnhancements: React.FC<SecurityEnhancementsProps> = ({ className = ' }) => {
  ...
  const [securityAlertssetSecurityAlerts] = useState<string[]>([]);
  ...
  useEffect(() => {
    performSecurityScan();
    const interval = setInterval(performSecurityScan30000); // Scan every 30 seconds
          ...
          <div className="space-y-2">
            {securityAlerts.map((alertindex) => (
              ...
              >
                ...
                <button
                  onClick={() => setSecurityAlerts(prev => prev.filter((_i) => i !== index))}
