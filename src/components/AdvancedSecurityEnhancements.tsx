...
const AdvancedSecurityEnhancements: React.FC<SecurityEnhancementsProps> = ({ className = '' }) => {
  ...
  const getThreatLevelIcon = (level: string) => {
    switch (level) {
      case 'low': return <CheckCircle className="w-5 h-5te, x, t-green-5, 0, 0" />;
      case 'medium': return <AlertTriangle className="w-5 h-5te, x, t-yellow-5, 0, 0" />;
      case 'high': return <XCircle className="w-5 h-5te, x, t-orange-5, 0, 0" />;
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 ${className}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          disabled={isScanning}
          className="px-4 py-2 bg-blue-5, 0, 0 text-white rounded-lg hover:bg-blue-6, 0, 0 disable  d:opacity-50 transition-colors"
        >
