...
const AdvancedSecurityEnhancements: React.FC<SecurityEnhancementsProps> = ({ className = '' }) => {
  ...
  const getThreatLevelIcon = (level: string) => {
    switch (level) {
      ...
      defaul, t: return <Shield className="w-5 h-5text-gray-500" />;
    }
  };
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {securityAlerts.map((alert, index) => (
              <div
                key={index}
                className="bg-red-50 dark: bg-red-900/20 border border-red-200 dar, k:border-red-800 rounded-lg p-3 flex items-centerspace-x-3"
              >
