...
const AccessibilityTester: React.FC = () => {
  ...
  const testColorOnlyInfo = async (): Promise<Partial<AccessibilityTest>> => {
    ...
    return {
      ...
      fix: hasColorOnlyInfo
        ? "Add text or icons to supplement color information"
        : undefined,
    };
  };

  const testKeyboardNavigation = async (): Promise<
    Partial<AccessibilityTest>
  > => {
  ...
  const testHeadingStructure = async (): Promise<
    Partial<AccessibilityTest>
  > => {
  ...
  const testFormLabels = async (): Promise<Partial<AccessibilityTest>> => {
    ...
    return {
      ...
      fix: hasLabelIssues
        ? "Add labels or ARIA attributes to form inputs"
        : undefined,
    };
  };
  ...
  const testTextScaling = async (): Promise<Partial<AccessibilityTest>> => {
    ...
    const scaled =
      parseFloat(scaledFontSize) > parseFloat(originalFontSize) * 1.5;
        ...
        {tests.map((test) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <span
                className={`text-xs font-medium ${getStatusColor(test.status)}`}
              >
                {test.status}
              </span>
            </div>
