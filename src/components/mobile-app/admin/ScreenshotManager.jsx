import React, { useState, useRef } from "react";"""import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";"""import { Button } from "@/components/ui/button";"""import { Upload, Trash2, Plus import { toast } from "sonner";"export const ScreenshotManager = ({ platform }) => {";
    ...
    const addScreenshots = (files) => {;
        ...
        const imageFiles = files.filter(file => file.type.startsWith('image/'));'        if (imageFiles.length === 0) {'"""            toast.error("Please select valid image files");"            return}"        // Limit the number of screenshots"""        const maxScreenshots = platform === "ios" ? 10 : 8;"        const availableSlots = maxScreenshots - screenshots.length;"        if (availableSlots <= 0) {
"""            toast.error(`Maximum ${maxScreenshots} screenshots allowed for ${platform === "ios" ? "iOS" : "Android"}`);"            return}"        const filesToAdd = imageFiles.slice(0, availableSlots);`        const newScreenshots = filesToAdd.map(file => ({;

            id: Math.random().toString(36).substring(2, 9), url: URL.createObjectURL(file), file
        }));
