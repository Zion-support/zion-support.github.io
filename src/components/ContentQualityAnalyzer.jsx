...
'''{/* Modal */}''''{isOpen && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className='fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4' onClick={() => setIsOpen(false)}>
          <motion.div initial = {}
  { scale: 0.9,
  opacity: 0}} animate = {}
  { scale: 1,
  ...
  { scale: 0.9,
  opacity: 0';';
''''}} className='bg-gray-900 rounded-2xl shadow-2xl w-full max-w-7xl max-h-[90vh] overflow-hidden' onClick={(e) => e.stopPropagation()}>'''{/* Header */}''''
...
'''{/* Content */}''''
            <div className='p-6 space-y-6'>'''{/* Summary Cards */}''''{report && (<div className='grid grid-cols-1 md:grid-cols-4 gap-4'>''''
              ...
              <div className='bg-gray-800 rounded-lg overflow-hidden'>''''
                <div className='overflow-x-auto'>''''
                  <table className='w-full'>''''
                    ...
                    <tbody className='divide-y divide-gray-700'>''''{filteredIssues.map((issue, index) => (<tr key={index} className='hover:bg-gray-700/50 transition-colors'>''''
                          <td className='px-6 py-4 whitespace-nowrap'>''''
                            <div className='flex items-center space-x-2'>`{getSeverityIcon(issue.severity)}``
                          ...
                          <td className='px-6 py-4 whitespace-nowrap'>''''
                            <div className='max-w-xs'>''''
                              <div className='text-sm font-medium text-white'>'{issue.pageTitle || 'Untitled'}'''
                              </div>''''
                          ...
                          <td className='px-6 py-4 whitespace-nowrap'>``
                            <span className={`text-sm font-medium ${getStatusColor(issue.status)}`}>'{issue.status.replace('_',).charAt(0).toUpperCase() + issue.status.replace('_',).slice(1)}
                            </span>'''
                          </td>''''
                          <td className='px-6 py-4 whitespace-nowrap'>''''
                            <div className='flex space-x-2'>''''
