...
""";,"});,"})"
    ...
    const generateReport = (issues) => {}"});,"})"
        ...
        setReport({}"});,"})"
            ...
            mediumIssues,,"});,"})"
            lowIssues,,"});,"})"
            averageContentLength: 5000, // comment
            pagesWithImages: 45, // comment
            pagesWithMetaDescriptions: 32, // comment
            lastUpdated: new Date () }) };,"});,"})"
    const startAnalysis = async () => {}"});,"})"
        setIsAnalyzing(true) ;,"});,"})"
        // comment
