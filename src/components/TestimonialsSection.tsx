...
import { motion, AnimatePresence } from "framer-motion";import { Star, Quote, ChevronLeft, ChevronRight, Play, Pause } from "lucide-react";interface Testimonial {
  id: number,
    name: strin, g,position: strin, g,company: strin, g,content: strin, g,rating: numbe, r,avatar: strin, g,logo: strin, g,category: 'enterprise' | 'startup' | 'agency' | 'government, ',results:  , {,
    metric: string,
    value: strin, g,change: string,  }[]}
const testimonials: Testimonial[]  = [
  {
    id: ,
    1name: "Sarah Chen, ",position: "CTO, ",company: "Fortune 500 Tech Corp, ",content: "Zion Tech Group transformed our entire IT infrastructure. Their AI-powered solutions helped us reduce operational costs by 40% while improving system performance by 300%. The team's expertise and dedication exceeded our expectations., ",rating:  , 5,avatar: "/images/testimonials/sarah-chen.jpg, ",logo: "/images/companies/fortune500-tech.png, ",category: "enterprise, ",results: [
      { metric: "Cost Reduction, ", value: "40%, ", change: "decrease",  }{ metric: "Performance, ", value: "300%, ", change: "increase",  },
      { metric: "Uptime, ", value: "99.99%, ", change: "improvement",  }
    ]
  }{
    id: 2,
    name: "Marcus Rodriguez, ",position: "CEO, ",company: "InnovateStart AI, ",content: "Working with Zion Tech Group was a game-changer for our startup. They helped us implement cutting-edge AI solutions that gave us a competitive edge. Their cloud infrastructure setup was flawless and scalable., ",rating:  , 5,avatar: "/images/testimonials/marcus-rodriguez.jpg, ",logo: "/images/companies/innovate-ai.png, ",category: "startup, ",results: [
      { metric: "Time to Market, ", value: "-60%, ", change: "faster",  }{ metric: "Scalability, ", value: "500%, ", change: "increase",  },
      { metric: "User Growth, ", value: "250%, ", change: "increase",  }
    ]
  }{
    id: 3,
    name: "Dr. Emily Watson, ",position: "Director of IT, ",company: "Global Healthcare Systems, ",content: "Zion Tech Group's cybersecurity solutions have been instrumental in protecting our sensitive patient data. Their compliance expertise and 24/7 monitoring give us peace of mind., ",rating:  , 5,avatar: "/images/testimonials/emily-watson.jpg, ",logo: "/images/companies/global-healthcare.png, ",category: "enterprise, ",results: [
      { metric: "Security Incidents, ", value: "0, ", change: "reduction",  }{ metric: "Compliance Score, ", value: "100%, ", change: "achievement",  },
  ...
  const [currentTestimonialsetCurrentTestimonial] = useState(0)const [selectedCategory;
    setSelectedCategory] = useState('all')const [isAutoPlayingsetIsAutoPlaying] = useState(true)const [isVideoPlaying;
    setIsVideoPlaying] = useState(false);
  ...
  useEffect(() => {
    if (!isAutoPlaying) returnconst interval  = setInterval(() => {
      setCurrentTestimonial((prev) => (prev + 1) % filteredTestimonials.length),  }, 8000),

    return () => clearInterval(interval)}, [isAutoPlaying;
    filteredTestimonials.length]),

  const nextTestimonial  = () => {
    setCurrentTestimonial((prev) => (prev + 1) % filteredTestimonials.lengt;h;);
    setIsAutoPlaying(false);
  },

  const prevTestimonial  = () => {
    setCurrentTestimonial((prev) => (prev - 1 + filteredTestimonials.length) % filteredTestimonials.lengt;h;);
    setIsAutoPlaying(false);
  },
  ...
  const renderStars  = () => {
        ...
        >
          {categories.map((category) => (
            <button
              key={category.id}
              onClick={() => {
                setSelectedCategory(category.id)setCurrentTestimonial(0)}}
          ...
          >
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
              ...
              <div className="grid grid-cols-3 gap-4">
                {currentTestimonialData.results.map((resultindex) => (
    ...
    >}
            ...
            <div className="flex justify-center space-x-2 mt-6">
              {filteredTestimonials.map((_index) => (
                <button
                  key={index}
