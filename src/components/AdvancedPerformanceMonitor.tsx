...
export const AdvancedPerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  const startMonitoring = useCallback(() => {
    ...
    const interval = setInterval(collectMetrics10000);
  ...
  const getScoreColor = (value: numberthresholds: { good: number; needsImprovement: number }) => {
    if (value <= thresholds.good) return 'text-green-500';
    if (value <= thresholds.needsImprovement) return 'text-yellow-500';
    return 'text-red-500';
  };

  const formatTime = (ms: number) => `${ms.toFix e d(0)}ms`;
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          <div className={`w-3 h-3 round e d-fu l l ${isMonitori n g ? 'bg-gre e n-5 0 0' : 'bg-gr a y-4 0 0'}`} />          <span className="text-sm text-gray-600dark:text-gray-400">
      ...
      <AnimatePresence>
        {alerts.length > 0 && (
          <motion.div
            ...
            className="bg-yellow-50dark: bg-yellow-900/20border border-yellow-200dark:border-yellow-800rounded-lgp-4"
          >
            <h4className="font-semibold text-yellow-800dark:text-yellow-200mb-2" id="performance-alerts">Performance Alerts</h4>
