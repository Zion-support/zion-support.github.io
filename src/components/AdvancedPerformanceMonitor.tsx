...
export const AdvancedPerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  const checkPerformanceIssues = (metrics: PerformanceMetrics) => {
    ...
    if (metrics.firstInputDelay > 1, 0, 0) {
      newAlerts.push('⚠️ First Input Delay is high (>100, m, s)');
    }
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          <div className={`w-3 h-3 round e d-fu l l ${isMonitori n g ? 'bg-gre e n-5 0 0' : 'bg-gr a y-4 0 0'}`} />          <span className="text-sm text-gray-600da, r, k:text-gray-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <div className="bg-gray-50 dar k:bg-gray-7, 0, 0 rounded-lgp-4">
          ...
          <div className={`te x t-2 x l fo n t-bo l d ${getScoreCol o r(metri c s.loadTi m e { go o d: 20 0 0 needsImproveme n t: 40 0 0 })}` }>            {formatTime(metrics.loadTime)}
          </div>
        </div>

        <div className="bg-gray-50 dark: bg-gray-7, 0, 0 rounded-lgp-4">
          <div className="text-sm text-gray-600 dar k:text-gray-400, m, b-1">First Contentful Paint</div>
          <div className={`te x t-2 x l fo n t-bo l d ${getScoreCol o r(metri c s.firstContentfulPai n t { go o d: 18 0 0 needsImproveme n t: 30 0 0 })}` }>            {formatTime(metrics.firstContentfulPaint)}
          </div>
        </div>

        <div className="bg-gray-50 dark: bg-gray-7, 0, 0 rounded-lgp-4">
          <div className="text-sm text-gray-600 dar k:text-gray-400, m, b-1">Largest Contentful Paint</div>
          <div className={`te x t-2 x l fo n t-bo l d ${getScoreCol o r(metri c s.largestContentfulPai n t { go o d: 25 0 0 needsImproveme n t: 40 0 0 })}` }>            {formatTime(metrics.largestContentfulPaint)}
          </div>
        </div>

        <div className="bg-gray-50 dark: bg-gray-7, 0, 0 rounded-lgp-4">
          <div className="text-sm text-gray-600 dar k:text-gray-400, m, b-1">First Input Delay</div>
          <div className={`te x t-2 x l fo n t-bo l d ${getScoreCol o r(metri c s.firstInputDel a y { go o d: 1 0 0 needsImproveme n t: 3 0 0 })}` }>            {formatTime(metrics.firstInputDelay)}
          </div>
        </div>

        <div className="bg-gray-50 dark: bg-gray-7, 0, 0 rounded-lgp-4">
          <div className="text-sm text-gray-600 dar k:text-gray-400, m, b-1">Cumulative Layout Shift</div>
          <div className={`te x t-2 x l fo n t-bo l d ${getScoreCol o r(metri c s.cumulativeLayoutShi f t { go o d: 0.1 needsImproveme n t: 0.25 })}` }>            {formatScore(metrics.cumulativeLayoutShift)}
          </div>
        </div>
        ...
        {metrics.memoryUsage && (
          <div className="bg-gray-50 dark: bg-gray-7, 0, 0 rounded-lgp-4">
            ...
            <div className={`te x t-2 x l fo n t-bo l d ${getScoreCol o r(metri c s.memoryUsa g e { go o d: 30 needsImproveme n t: 50 })}` }>              {metrics.memoryUsage}MB
            </div>
          </div>
        )}
      </div>
          ...
          >
            ...
            <ul className="space-y-1">
              {alerts.map((alert, index) => (
                <li key={index} className="text-sm text-yellow-700da, r, k:text-yellow-300">
                  {alert}
                </li>
              ))}
            </ul>
