...
export default function BenefitsSection() {
  ...
  const benefits = [
    ...
    },
    {
      ...
      description: "Dedicated team of technology experts providing round-the-clock support and guidance.",
      stats: "24/7 Support",
      color: "from-zion-orange to-zion-purple",
      bgColor: "from-zion-orange/20 to-zion-purple/20",
      features: [
        "Technical Expertise",
  ...
  const additionalStats = [
    { icon: <Zap className="w-8 h-8 text-zion-cyan" />, value: "500+", label: "Projects Completed" },
    { icon: <Star className="w-8 h-8 text-zion-purple" />, value: "98%", label: "Client Satisfaction" },
    { icon: <Globe className="w-8 h-8 text-zion-blue" />, value: "25+", label: "Countries Served" },
    { icon: <Cpu className="w-8 h-8 text-zion-green" />, value: "10+", label: "Years Experience" }
  ];
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {benefits.map((benefit, index) => (
            <motion.div key={index} variants={itemVariants} onHoverStart={() => setHoveredIndex(index)} onHoverEnd={() => setHoveredIndex(null)} whileHover={{ y: -8 }} transition={{ type: "spring", stiffness: 300 }}>
                  ...
                  {hoveredIndex === index && (
                    <motion.div className="space-y-2" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }}>
                      {benefit.features.map((feature, idx) => (
                        <motion.div key={idx} className="flex items-center gap-2 text-zion-slate-light/80 text-sm" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }}>
