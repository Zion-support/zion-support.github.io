...
const AdvancedServicePortfolioDashboard = () => {
  ...
  const portfolioMetrics = useMemo ( () => {
    ...
    const totalClients = portfolioData.reduce ( (sum, s) => sum + s.clientCount,
      0) ;
          ...
          <CardContent>
            <div className="overflow - x-auto">
              <table className="w - full">
                ...
                <tbody>
                  {filteredPortfolio.map ( (service, index) => (<motion.tr
                      key={service.id}
        ...
        <Card>
          ...
          <CardContent>
            <div className="space - y-3">
              {categories
                .filter (cat => cat.id !== 'all') .map ( (category, index) => (<motion.div
                    key={category.id}
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.1 * index }}
            ...
            <div className="space - y-4">
              ...
              <div className="flex items - start gap - 3 p - 3 bg - zion - purple / 10 rounded - lg">
                ...
                <div>
                  ...
                  <p className="text - sm text - zion - slate - light">
                    {
                      portfolioData.filter (s => s.technologyMaturity === 'Leading') .length
                    }{' '}
