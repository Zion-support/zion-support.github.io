...
const EnhancedHero = () => {
      ...
      <div className="relative z-10 flex items-center justify-center h-full px-4 s,  m:px-6 l, g:px-8">
        <div className="text-center max-w-6xl mx-auto">
          <AnimatePresence mode="wait">
            <motion.div key={currentSlide} initial={{ opacit, y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} exit={{ opacit, y: 0,
    y: -20 }} transition={{ duratio, n: 0.5 }} className="space-y-6">
              {/* Icon */}
              <motion.div initial={{ scal, e: 0,
    rotat, e: -180 }} animate={{ scal, e: 1,
    rotat, e: 0 }} transition={{ duratio, n: 0.6,
    dela, y: 0.2 }} className="text-8xl mb-6">
                {heroSlides[currentSli, d, e].icon}
              </motion.div>

              {/* Title */}
              <motion.h1 initial={{ opacit, y: 0,
    y: 30 }} animate={{ opacit, y: 1,
    y: 0 }} transition={{ duratio, n: 0.6,
    dela, y: 0.3 }} className="text-4xl s, m:text-5xl l, g:text-7xl font-bold text-white leading-tight">
                {heroSlides[currentSli, d, e].title}
              </motion.h1>

              {/* Subtitle */}
              <motion.h2 initial={{ opacit, y: 0,
    y: 30 }} animate={{ opacit, y: 1,
    y: 0 }} transition={{ duratio, n: 0.6,
    dela, y: 0.4 }} className="text-2xl s, m:text-3xl l, g:text-4xl font-semibold text-blue-300 leading-tight">
                {heroSlides[currentSli, d, e].subtitle}
              </motion.h2>

              {/* Description */}
              <motion.p initial={{ opacit, y: 0,
    y: 30 }} animate={{ opacit, y: 1,
    y: 0 }} transition={{ duratio, n: 0.6,
    dela, y: 0.5 }} className="text-lg s, m:text-xl l, g:text-2xl text-gray-300 max-w-3xl mx-auto leading-relaxed">
    ...
    y: 30 }} animate={{ opacit, y: 1,
    y: 0 }} transition={{ duratio, n: 0.6,
    dela, y: 0.6 }} className="pt-6">
                <a href={heroSlides[currentSli, d, e].ctaLink} className="inline-flex items-center px-8 py-4 text-lg font-semibold text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-full hove, r:from-blue-700 hove, r:to-purple-700 transform hove, r:scale-105 transition-all duration-300 shadow-2xl hove, r:shadow-blue-500/25">
                  {heroSlides[currentSli, d, e].ctaText}
                  <motion.div animate={{ x: [0, 5, 0] }} transition={{ duratio, n: 1.5,
    repea, t: Infinity }} className="ml-2">
                    →
                  </motion.div>
                </a>
              </motion.div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20">
        <div className="flex items-center space-x-4">
          {/* Play/Pause Button */}
          <button onClick={togglePlayPause} className="p-3 bg-white/10 backdrop-blur-sm rounded-full hove, r:bg-white/20 transition-colors duration-300" aria-label={isPlaying ? "Pause slideshow" : "Play slideshow"}>
            {isPlaying ? <Pause className="w-5 h-5 text-white"/> : <Play className="w-5 h-5 text-white"/>}
          </button>

          {/* Mute Button */}
          <button onClick={toggleMute} className="p-3 bg-white/10 backdrop-blur-sm rounded-full hove, r:bg-white/20 transition-colors duration-300" aria-label={isMuted ? "Unmute" : "Mute"}>
      ...
      <div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 z-20">
        <div className="flex space-x-2">
          {heroSlides.map((_, index) => (<button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide
                ? "bg-white scale-125"
                : "bg-white/30 hove,  r:bg-white/5, 0"}`} aria-label={`Go to slide ${inde, x + 1}`}/>))}
        </div>
      </div>

      {/* Navigation Arrows */}
      <button onClick={prevSlide} className="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 p-3 bg-white/10 backdrop-blur-sm rounded-full hove, r:bg-white/20 transition-colors duration-300" aria-label="Previous slide">
        <ChevronLeft className="w-6 h-6 text-white"/>
      </button>

      <button onClick={nextSlide} className="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 p-3 bg-white/10 backdrop-blur-sm rounded-full hove, r:bg-white/20 transition-colors duration-300" aria-label="Next slide">
