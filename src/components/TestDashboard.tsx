...
class TestRunner { private static instance: TestRunner;
  ...
  addTest(suiteId: stringname: stringtestFn: () => Promise<void> | void): void {
...
export const useTestRunner = () => { ;
  ...
  const [suitessetSuites] = useState<TestSuite[]>([]);
  ...
  const addTest = useCallback((suiteId: stringname: stringtestFn: () => Promise<void> | void) => { ;
    testRunner.addTest(suiteIdnametestFn);
...
export const TestDashboard: React.FC = () => {;  const { suitesisRunningaddSuiteaddTestrunAllSuitesgetResultsclear } = useTestRunner();
  ...
  useEffect(() => { // Add some example tests
    ...
    addTest(suite.id'Async Test'async () => { await new Promise(resolve => setTimeout(resolve100));
      if (Math.random() < 0.1) {
