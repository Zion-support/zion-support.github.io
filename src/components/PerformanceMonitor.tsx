...
if('web-vitals" in, window) {;"</>import('web-vitals").then(({ getCLS, getFID, getFCP, getLCP, getTTFB }) => {"
const logMetric = ("")"
...
limi",t: "Math.round(memory.jsHeapSizeLimit / 1048576) + ' MB'""</>",;});}</PerformanceMonitorProps></>
...
constfidObserver= new PerformanceObserver((list) => {constentries= list.getEntries()"
          entries.forEach((entry: "any) => {",newMetrics.firstInputDelay = entry.processingStart - entry.startTime"
         ,;})""
        ;})"
