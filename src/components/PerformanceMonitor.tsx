...
const PerformanceMonitor: React.FC = () => {
  return (
    ...
    fcp: null,
    ttfb: null});
  useEffect(() => {if (typeof window === 'undefined' || !('performance' in window)) return}
    // Web Vitals measurement
    const measureWebVitals = () => {// LCP - Largest Contentful Paint}
      ...
      const fidObserver = new PerformanceObserver((list) => {const entries = list.getEntries()}
        entries.forEach((entry: any) => {setMetrics(prev => ({}
            ...prev, 
    ...
    const sendToAnalytics = (metrics: PerformanceMetrics) => {if (typeof window !== 'undefined' && 'gtag' in window) {}
        const gtag = (window as any).gtag,
        if (metrics.lcp !== null) {gtag('event', 'web_vitals', {}
            event_category: 'Performance',
            event_label: 'LCP',
            value: Math.round(metrics.lcp)});
        }
        if (metrics.fid !== null) {gtag('event', 'web_vitals', {}
            event_category: 'Performance',
            event_label: 'FID',
            value: Math.round(metrics.fid)});
        }
        if (metrics.cls !== null) {gtag('event', 'web_vitals', {}
            event_category: 'Performance',
            event_label: 'CLS',
    ...
    const timeoutId = setTimeout(() => {sendToAnalytics(metrics)}, 5000)
    return () => {cleanup?.()}
