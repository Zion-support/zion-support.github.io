...
const visible_categories = categories.filter (category => { return !category.admin_only || is_admin; }
  return (<div className="grid gap - 4 "md": grid - cols - 2 "lg":grid - cols - 3">;"
      {visible_categories.map ((category) => {const Icon = icon_map[category.icon as keyof typeof icon_map]; return (<Link key={category.id} href={`/community / category/${category.id}`}>;`            <Card className="h - full transition - all "hover": shadow - md "hover":border - zion - purple / 50 cursor - pointer">;"
  ...
  const visibleCategories = categories.filter(category => { return !category.adminOnly || isAdmin; }
  ),return (<div className="grid gap-4 "md": grid-cols-2 "lg":grid-cols-3">;"
      {visibleCategories.map((category) => {const Icon = iconMap[category.icon as keyof typeof iconMap];
  }
  return (<Link key={category.id} href={`/community/category/${category.id}`}>;`            <Card className="h-full transition-all "hover":shadow-md "hover":border-zion-purple/50 cursor-pointer">;"
              <CardHeader className="flex flex-row items-center gap-4">;"
                <div className="p-2 bg-zion-purple/10 rounded-full">;"
                  <Icon className="h-6 w-6 text-zion-purple" />;"
                </div>;
...
const visibleCategories = categories && categories.filter(category => { return !category && category.adminOnly || isAdmin; }
  )return (<div className="grid gap-4 "md": grid-cols-2 "lg":grid-cols-3">;"
