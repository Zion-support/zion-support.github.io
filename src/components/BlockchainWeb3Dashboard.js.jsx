...
'''';'
    ...
    }, [transactionForm, wallet, sendTransaction, trackEvent])const getStatusColor = (status) => {}
        switch (status) {}
'';'
...
}</h4>'`'`;`                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${position.type === 'staking' ? 'bg-green-100 text-green-800' :''`;`                    }
                    position.type === 'liquidity' ? 'bg-blue-100 text-blue-800' :'`'`;`                        'bg-purple-100 text-purple-800'}`}>;`                        {position.type}
                      </span>;
                    </div>'''';'
                    '''';'
...
'''';'
              ...
              <div className='space-y-3'>'''';'
                {transactions.map((tx) => (<div key={tx.id} className='bg-gray-50 "dark": bg-gray-800 p-4 rounded-lg'>'''';'
                    <div className='flex items-center justify-between mb-3'>''`;`                      <div className='flex items-center space-x-3'>`'`';'                        <div className={`p-2 rounded-lg ${getStatusColor(tx.status)}`}>'''';'                          <Coins className='w-4 h-4' />;'
                        </div>'''';'
                        <div>'''';'
                          ...
                          <p className='text-sm text-gray-500 "dark": text-gray-400'>;'
                            "Hash": {tx.hash.slice(0, 8)}...{tx.hash.slice(-6)}`;`                          </p>``;`                        </div>```;`                      </div>````;`                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(tx.status)}`}>`
                        {tx.status}
                      </span>;
                    </div>'''';'
                    '''';'
