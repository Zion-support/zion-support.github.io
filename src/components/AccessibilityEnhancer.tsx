...
const AccessibilityEnhancer = React.forwardRef<any, AccessibilityEnhancerProps>(({
  ...
  mediaQuery.addEventListener('change', handleChange);
  return () => mediaQuery.removeEventListener('change', handleChange);  }  }, [enableSkipLinks, enableFocusManagement, enableScreenReaderSupport, enableHighContrastSupport, enableReducedMotionSupport]);
  // Apply accessibility styles
  useEffect(() => {
  if (isHighContrast) {
  document.documentElement.classList.add('high-contrast');  } else {
  document.documentElement.classList.remove('high-contrast');  }  }, [isHighContrast]);
  useEffect(() => {
  if (!prefersMotion) {
  document.documentElement.classList.add('reduced-motion');  } else {
  document.documentElement.classList.remove('reduced-motion');  }  }, [prefersMotion]);
