...
const AccessibilityEnhancer: React.FC<AccessibilityEnhancerProps> = ({ children }) => {
    ...
    const buttons = document.querySelectorAll('button: not([aria-label])'),      buttons.forEach(button => {
...
const AccessibilityEnhancer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const prefersHighContrast = window.matchMedia('(prefers-contrast: high)').matches,    setSettings(prev => ({
      ...prev,
  ...
  return (
    <div className="fixed top-4 right-4 bg-black/80 text-white p-4 rounded-lg text-xs z-50">
      <div className="mb-2 font-bold">Accessibility Settings</div>      <div className="space-y-2">
        ...
        <label className="flex items-center space-x-2">
          <input
            ...
            onChange={(e) => setSettings(prev => ({ ...prev, focusVisible: e.target.checked }))}            className="rounded"
          />
