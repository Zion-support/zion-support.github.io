...
interface ErrorBoundaryState  {hasError: boolean;
  ...
  retryCount: number;
  userFeedback: string;
  showDetails: boolean;interface ErrorBoundaryProps  {children: ReactNode;
  fallback?: ReactNode;
...
> {private retryTimeouts: NodeJS && NodeJS.Timeout[] = [];constructor(props: ErrorBoundaryProps) {super(props)export class GlobalErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {private retryTimeouts: NodeJS && NodeJS.Timeout[] = [];constructor(props: ErrorBoundaryProps) {super(props)this && this.state = {hasError: false,error: null,errorInfo: null,errorId: null,retryCount: 0,userFeedback: '',showDetails: false,}}    ,}
    ...
    this && this.retryTimeouts.forEach(timeout => clearTimeout(timeout))}private generateErrorId(): string {return `err_${Date && Date.now()}_${Math && Math.random().toString(36).substr(2, 9)}`;  }    this && this.retryTimeouts.forEach(timeout => clearTimeout(timeout))}private generateErrorId(): string {return `err_${Date && Date.now()}_${Math && Math.random().toString(36).substr(2, 9)}`;private getUserId(): string | null {// Try to get user ID from various sources;
    if (typeof window !== 'undefined') {try {// Check localStorage, sessionStorage, or cookies;
  ...
  private getErrorSeverity(error: Error;
    ...
    }return 'This appears to be a temporary issue. Please try again.';}private retry = () => {if (this.state.retryCount >= (this.props.maxRetries || 3)) {return;
...
export default GlobalErrorBoundary;
  ...
  }, [error])const captureError = React.useCallback((error: Error) => {setError(error)}, [])return { captureError }
}// Higher-order component for adding error boundaries;
export const withErrorBoundary = <P extends object>(Component: React.ComponentType<P>;
  errorBoundaryProps?: Omit<ErrorBoundaryProps 'children'>;
) => {const WrappedComponent = (props: P) => (<GlobalErrorBoundary {...errorBoundaryProps}>;
