...
'''''
          <div className='flex gap-2'>''''''{['overview',detailed',analytics'].map((mode) => (<Button key={mode} variant={viewMode === mode ? 'default' : 'outline'} size='sm' onClick={() => setViewMode(mode)}>''''''{mode === 'overview' && <Eye className='w-4 h-4 mr-2'/>}''''''{mode === 'detailed' && <BarChart3 className='w-4 h-4 mr-2'/>}''''''{mode === 'analytics' && <PieChart className='w-4 h-4 mr-2'/>}
...
''''}} transition={{ delay: 0.1 * index }} className='border-b border-zion-slate-light/10 hover:bg-zion-blue-light/5'>''''
                      ...
                      <td className='py-3 px-4'>''''
                        <div className='flex items-center gap-1'>'''`{service.growth > 0 ? (<ArrowUpRight className='w-4 h-4 text-green-500'/>) : (<ArrowDownRight className='w-4 h-4 text-red-500'/>)}'`'`
                          <span className={`text-sm font-medium ${service.growth > 0 ? 'text-green-600' : 'text-red-600'}`}>'{service.growth > 0 ? '+' : ''}{service.growth}%
                          </span>
                        </div>'''
                      </td>''''
                      <td className='py-3 px-4'>
  ...
  { opacity: 1,
  y: 0';';
''''}} transition={{ delay: 0.4 }} className='grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8'>{/* Category Distribution */}
        <Card>'''
          <CardHeader>''''
            <CardTitle className='flex items-center gap-2'>''''
          ...
          <CardContent>''''
            <div className='space-y-3'>'{categories.filter(cat => cat.id !== 'all').map((category, index) => (<motion.div key={category.id} initial = {}
  { opacity: 0,
  x: -20}} animate = {}
  { opacity: 1,
  x: 0';';
...
''''
              <div className='flex items-start gap-3 p-3 bg-zion-purple/10 rounded-lg'>''''
                ...
                <div>''''
                  ...
                  <p className='text-sm text-zion-slate-light'>'{portfolioData.filter(s => s.technologyMaturity === 'Leading').length} services in leading position
                  </p>
                </div>
              </div>
            </div>
