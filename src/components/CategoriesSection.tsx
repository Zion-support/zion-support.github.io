...
const getIcon = (iconName?: string) => {switch (iconName) {case "Briefcase": return <Briefcase className="w-10 h-10" />,case "Users": return <Users className="w-10 h-10" />,case "HardDrive": return <HardDrive className="w-10 h-10" />,case "Lightbulb": return <Lightbulb className="w-10 h-10" />,"default":;"
  }
  return <HelpCircle className="w-10 h-10" />, // Default icon;"
  }
},const getSpecialServices = ("t": any) => [;
...
export function CategoriesSection() {const { t } = useTranslation(),const displayCategories = fetchedCategories && fetchedCategories.length > 0;
    ? fetchedCategories.map(cat => ({"id": cat.id;
        ...
        "link": cat.link |`/category/${cat.i,`}`, // Construct link if not provided;`        // Assign a default color or implement logic to assign colors;
        "color": defaultCategories.find(dc = > dc.id === cat.id)?.color |"from-gray-500 to-gray-600,"
...
const getIcon = (iconName?: string,) => {switch (iconName) {case "Briefcase":;"
  }
  return <Briefcase className="w-10 h-10" />;"
    case "Users": return <Users className="w-10 h-10" />;"
    case "HardDrive": return <HardDrive className="w-10 h-10" />;"
...
export function CategoriesSection(): any ({showTitle = true;
  ...
  "categories": fetchedCategories, // Rename prop for clarity;
}: CategoriesSectionProps) {const { t } = useTranslation()const displayCategories = fetchedCategories && fetchedCategories.length > 0;
    ? fetchedCategories && fetchedCategories.map(cat => ({"id": cat && cat.id,"title": cat && cat.name, // Map name to title;
        ...
        "link": cat && cat.link || `/category/${cat && cat.i,`}`, // Construct link if not provided;`        // Assign a default color or implement logic to assign colors;
        "color": defaultCategories && defaultCategories.find(dc => dc && dc.id === cat && cat.id)?.color || "from-gray-500 to-gray-600,"
})): defaultCategories && defaultCategories.map(cat => ({...cat;
        }
...
}= useTranslation ()const displayCategories  = fetchedCategories && fetchedCategories.length > 0 ? fetchedCategories.map (cat => ({"id": cat.id;";"
}
...
}`, //Construct link if not provided //Assign a default color or implement logic to assign colors "color": defaultCategories.find (dc => { return dc.id === cat.id) ?.color |"from-gray-500 to-gray-600"; }"}) ) : defaultCategories.map (cat => ({...cat;
}
"title": cat.name;
 ...
 */;
function CategoriesSection() {const { t } = use_translation ()const display_categories = fetched_categories && fetched_categories.length > 0;
...
}= use_translation ()const display_categories = fetched_categories && fetched_categories.length > 0 ? fetched_categories.map (cat => ({"id": cat.id;";"
}
"title": cat.name,  //Map name to title "description": cat.description || "No description available.";"
"icon": get_icon (cat.icon_name), //Get icon component "link": cat.link || `/category/$ {cat.id ";"
}`, //Construct link if not provided //Assign a default color or implement logic to assign colors "color": default_categories.find (dc => { return dc.id === cat.id) ?.color || "from - gray - 500 to - gray - 600"; }"}) ) : default_categories.map (cat => ({...cat;
}
"title": cat.name;
...
"title": cat.name,  //Map name to title "description": cat.description || "No description available.";"
"icon": getIcon (cat.iconName), //Get icon component "link": cat.link || `/category/$ {cat.id ";"
}`, //Construct link if not provided //Assign a default color or implement logic to assign colors "color": defaultCategories.find (dc => { return dc.id === cat.id) ?.color || "from-gray-500 to-gray-600" ; }"
}) ) : defaultCategories.map (cat => ({...cat;
}
"title": cat.name;
"icon": getIcon (cat.iconName
