...
id: string;
  ...
  description?: string;
  iconName?: string,
// Example field if categories have icons;
  itemCount?: number,
// Example field for number of items in a category;
  // Add a 'link' property if your API provides it,
or construct it.;
  link?: string;
}
...
const getIcon = (iconName?: string,)  => {
switch (iconName) {title: "AI Services",
...
icon: "☁️",
link: "/micro - saas",
color: "from - cyan - 500 to - blue - 600", },{title: "IT Services",
description: "Infrastructure, security, and technical consulting",
icon: "⚡",
link: "/all - services",
color: "from - amber - 500 to - orange - 600", },{title: "Digital Transformation",
...
icon_name: "Users",
link: "/talent",
color: "from - cyan - 500 to - blue - 600"},{id: "equipment",
name: t ('categories.equipment'),
description: t ('categories.equipment_desc'),
icon_name: "HardDrive",
link: "/equipment",
color: "from - amber - 500 to - orange - 600"},{id: "innovation",
name: t ('categories.innovation'),
description: t ('categories.innovation_desc'),
icon_name: "Lightbulb",
link: "/innovation",
color: "from - emerald - 500 to - green - 600"}],// Helper to get icon component from name;
...
link: "/it-onsite-services";
import { cn } from "@/lib/utils",
import { useTranslation } from "react-i18next",// This is the type definition copied from Categories.tsx for consistency.;
// Ideally, this would be in a shared types file.;
interface CategoryType {
id: string,
name: string,
...
color: "from-emerald-500 to-green-600"}],// Helper to get icon component from name;
const getIcon = (iconName?: string)  => {
switch (iconName) {case "Briefcase": return <Briefcase className="w-10 h-10" />,
case "Users": return <Users className="w-10 h-10" />,
case "HardDrive": return <HardDrive className="w-10 h-10" />,
case "Lightbulb": return <Lightbulb className="w-10 h-10" />,
default: return <HelpCircle className="w-10 h-10" />, // Default icon;
...
const defaultCategories = getDefaultCategories(t),// Use fetchedCategories if provided, otherwise fallback to defaultCategories;
  const displayCategories = fetchedCategories && fetchedCategories.length > 0;
    ? fetchedCategories.map(cat => ({id: cat.id;
        ...
        color: defaultCategories.find(dc => dc.id === cat.id)?.color |"from-gray-500 to-gray-600"})): defaultCategories.map(cat => ({...cat,...cat,
title: cat.name,
 ...
 */;
function CategoriesSection(): any ({ showTitle = true }: CategoriesSectionProps) {const categories = [{title: "AI Services",
description: "Cutting - edge AI solutions, chatbots, and machine learning",
icon: "🤖",
link: "/ai - services",
color: "from - purple - 500 to - indigo - 600",},{title: "Micro SAAS",
description: "Cloud - based software solutions for modern businesses",
icon: "☁️",
link: "/micro - saas",
color: "from - cyan - 500 to - blue - 600",},{title: "IT Services",
...
icon: getIcon(cat && cat.iconName), // Get icon component;
        ...
        color: defaultCategories && defaultCategories.find(dc => dc && dc.id === cat && cat.id)?.color || "from-gray-500 to-gray-600"})): defaultCategories && defaultCategories.map(cat => ({...cat;
        title: cat && cat.name,
icon: getIcon(cat && cat.iconName)})),// If fetchedCategories is an empty array, and we want to show nothing:;
  if (fetchedCategories && fetchedCategories.length === 0) {return (<section className={cn("py-20 bg-zion-blue text-center", className)} style={style}>;
        <div className="container mx-auto px-4">;
...
return (</p> </div>)}</div> </Link>) )}</div> > {service.title;
}</Link>) ) ";
...
icon: get_icon (cat.icon_name), // Get icon component;
        ...
        title: cat.name,
icon: get_icon (cat.icon_name)})),// If fetched_categories is an empty array, and we want to show nothing:;
  // Check condition;
if ( {) {$2;
}
