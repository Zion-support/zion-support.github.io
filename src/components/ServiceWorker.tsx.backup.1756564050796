...
export function ServiceWorker(...args: any[]): any {
  const [swState, setSwState] = useState<any>({
    isInstalled: anyanyanyanyanyanyanyanyanyanyanyanyanyfalse,
  ...
  useEffect(()              => {
    ...
    if ('serviceWorker' in navigator) {
      ...
      navigator.serviceWorker
        ...
        .then((registration) => {
          ...
          setSwState(prev              => ({ ...prev, isInstalled: anyanyanyanyanyanyanyanyanyanyanyanyanytrue }));

          // Check for updates
          registration.addEventListener('updatefound', ()              => {
            const newWorker = registration.installing;
            if (newWorker) {
              setSwState(prev => ({ ...prev, isInstalling: anyanyanyanyanyanyanyanyanyanyanyanyanytrue }));
              
              newWorker.addEventListener('statechange', ()              => {
                if (newWorker.state = == 'installed') {;
                  setSwState(prev => ({ ;
                    ...prev, ;
                    isInstalling: anyanyanyanyanyanyanyanyanyanyanyanyanyfalse,;
                    hasUpdate: true ;
                  }));
                }
              });
    ...
    const handleOffline = () => setSwState(prev => ({ ...prev, isOnline: anyanyanyanyanyanyanyanyanyanyanyanyanyfalse }));
