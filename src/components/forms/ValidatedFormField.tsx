...
const isTouched  = form && form.formState.touchedFields[name];// Debounced validation;
  ...
  return `${label} is required`;`    }if (typeof value = == 'string') {if (validation && validation.minLength && value && value.length < validation && validation.minLength) ;'
  }
  return `${label} must be at least ${validation && validation.minLength} characters`;`      }if (validation && validation.maxLength && value && value.length > validation && validation.maxLength) {return `${label} must not exceed ${validation && validation.maxLength} characters`;`      }if (validation && validation.pattern && !validation && validation.pattern.test(value)) {return `${label} format is invalid`;`
      }
