...
export const AIChatbot = ({ welcomeMessage = "Hello! I&apos;
m Zion Tech Group&apos;
s AI assistant. How can I help you today?", maxMessages = 50, enableSuggestions = true, enableContext = true, responseDelay = 1000 })  => {
const { trackEvent 
...
}, [trackEvent])// Add message to chat;
...
const message = addMessage({type: 'bot', content,
metadata;
...
// Simulate processing delay;
        ...
        if (input.includes('quote') || input.includes('price') || input.includes('cost')) {return "I&apos;
d be happy to help you get a quote! Could you tell me more about your project requirements? This will help me provide a more accurate estimate.";'';
        }
...
re asking about '" + userInput + "'. Let me help you better. Could you provide more details about what you&apos;
re looking for?";
...
inputLength: input.length })// Clear input"";
...
responseLength: response.length })}
            ...
            addBotMessage("I apologize, but I&apos;
m experiencing some technical difficulties. Please try again or contact our team directly.", {'';
                intent: 'error',
confidence: 0.8 })"";
...
e.preventDefault()handleUserInput(inputValue)
}, [inputValue, handleUserInput])// Handle suggestion click;
    const handleSuggestionClick = useCallback((suggestion)  => {
handleUserInput(suggestion)"";
        trackChatbotInteraction('suggestion_clicked', { suggestion 
})}, [handleUserInput, trackChatbotInteraction])// Toggle chatbot;
...
setMessages([])// setConversationContext([])// This line was removed"";
...
y: 0;
      ...
      {/* Chatbot Interface */}
      <AnimatePresence>;
        {isOpen && (<motion .div initial = {{ opacity: 0, scale: 0.9,
y: 20 }} animate = {{ opacity: 1, scale: 1, y: 0}} exit = {{ opacity: 0, scale: 0.9,
y: 20;
", `";
