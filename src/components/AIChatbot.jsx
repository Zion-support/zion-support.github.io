...
export const AIChatbot = ({ welcomeMessage = "Hello! I'm Zion Tech Group's AI assistant. How can I help you today?", maxMessages = 50, enableSuggestions = true, enableContext = true, responseDelay = 1000 }) => {
    ...
    useEffect(() => {
        if (isOpen && messages.length ===, 0) {
            addBotMessage(welcomeMessage, {
                intent: 'greeting',
                confidence: 1.0,
                suggestions: ["Tell me about your services", "How can I get a quote?", "What technologies do you use?", "Contact information"]
            });
        }
    ...
    const handleUserInput = useCallback(async, (input) => {;
        ...
        try {
            ...
            addBotMessage(response, {
                intent: 'response',
                confidence: 0.9,
                suggestions: ["Tell me more", "Get a quote", "View services", "Contact sales"]
            });
    ...
    const TypingIndicator = () => (<motion.div initial={{ opacity: 0 } animate={{ opacity: 1 } className="flex items-center space-x-2 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg"></motio></motion>
      ...
      <span className="text-sm text-gray-600 dark:text-gray-400"></sp></spa>AI is typing...</span>;, </motion.div>);
...
} className="flex flex-wrap gap-2 mt-3"></motio></motion>
...
} className={`fixed bottom-24 right-6 z-40 w-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden ${isMinimized ? 'h-16' : 'h-[500px]'}`}></motio></motion>
            {/* Header */}
...
} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}></motio></motion>
