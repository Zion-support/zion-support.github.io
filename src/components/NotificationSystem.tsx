    ...
    switch (type) {
      case 'success':
        return (
          <CheckCircle
            {...iconProps}
            className={`w-5 h-5 ${priority === 'high' ? 'text-green-600' : 'text-green-500'}`}
          />
        );
      case 'warning':
        return (
          <AlertTriangle
            {...iconProps}
            className={`w-5 h-5 ${priority === 'high' ? 'text-yellow-600' : 'text-yellow-500'}`}
          />
        );
      case 'error':
        return (
          <AlertTriangle
            {...iconProps}
            className={`w-5 h-5 ${priority === 'high' ? 'text-red-600' : 'text-red-500'}`}
          />
        );
      case 'info':
        return (
          <Info
            {...iconProps}
            className={`w-5 h-5 ${priority === 'high' ? 'text-blue-600' : 'text-blue-500'}`}
          />
        );
      case 'achievement':
        return (
          <Star
            {...iconProps}
            className={`w-5 h-5 ${priority === 'high' ? 'text-purple-600' : 'text-purple-500'}`}
          />
        );
      default:
          ...
          >
            ...
            <div className='space-y-4'>
              ...
              <div className='flex items-center justify-between'>
                ...
                <button
                  onClick={() =>
                    updateSettings({ autoDismiss: !settings.autoDismiss })
                  }
                  className={`p-2 rounded-lg transition-colors ${
        ...
        {isOpen && (
          ...
          >
            ...
            <div className='flex items-center justify-between mb-4'>
              <h3 className='text-lg font-semibold text-zion-slate-dark'>
                Notifications
              </h3>
              <button
