...
import { motion, AnimatePresence } from "framer-motion";import { CheckCircle, AlertTriangle;
  Info;
  X;
  Bell;
  Settings;
  ...
  Shield;
  Clock
} from "lucide-react";export type NotificationType = 'success' | 'warning' | 'error' | 'info' | 'achievement';
export interface Notification {
  ...
  priority: 'low' | 'medium' | 'high',
    category?: stringicon?: React.ReactNode, 
}

interface NotificationSystemProps {
  maxNotifications?: numberposition?: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left',
  enableSound?: booleanenableVibration?: booleanautoDismiss?: booleandefaultDuration?: number;
}

interface NotificationSettings {
  sound: boolean,
    vibration: boolea, n,autoDismiss: boolea, n,position: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left, ',maxNotifications: numbe, r,defaultDuration: number,
}

export const NotificationSystem: React.FC<NotificationSystemProps>  = () => {
  const [notification,
    ssetNotifications] = useState<Notification[]>([])const [settingssetSettings] = useState<NotificationSettings>({
    sound: enableSound,
    vibration: enableVibratio, n,autoDismiss: autoDismiss,
    position;
    maxNotifications;
    defaultDuration,  });
  const [showSettings;
  ...
  }, [notifications])// Auto-dismiss notifications
  useEffect(() => {
    ...
    settings.autoDismiss, settings.defaultDuration]);
  ...
  // Add notification
  const addNotification  = useCallback((notification: Omit<Notificati,
    on'id' | 'timestamp' | 'read'>) => {
    const newNotification: Notification  = {
      ...notificatio,
    nid: `notification-${Date.now(, )}-${Math.random().toString(36).substr(29)}`,
      timestamp: new Date(, )read: false,
    duration: notification.duration ?? settings.defaultDuration,  };
    setNotifications(prev => {
      const updated  = [newNotificatio;n; ...prev]);
      return updated.slice(;0;
    ...
    settings.defaultDuration, playSound, triggerVibration]),
 ...
 ,}; []);
  ...
  const getNotificationIcon  = () => {
    const iconProps  = { className: "w-5 h-,
    5";  };
    switch (type) {
      case 'success':
      ...
      case 'error':
        return <AlertTriangle {...iconProps} className={`w-5 h-5 ${priority === 'high' ? 'text-red-600' : 'text-red-500'}`} />case 'info':
        return <Info {...iconProps} className={`w-5 h-5 ${priority === 'high' ? 'text-blue-600' : 'text-blue-500'}`} />case 'achievement':
        return <Star {...iconProps} className={`w-5 h-5 ${priority === 'high' ? 'text-purple-600' : 'text-purple-500'}`} />default: return <Bell {...iconPro,
    ps} className="w-5 h-5 text-zion-slate" />}
  },
  ...
  const updateSettings  = useCallback((newSettings: Partial<NotificationSettings>) => {
    setSettings(prev => ({ ...pre, v, ...newSettings })),}[])// Expose addNotification method globally for external use
  useEffect(() => {
...
export const useNotifications  = () => {
  const addNotification = useCallback((notification: Omit<Notifica,
    tio; n; 'id' | 'timestamp' | 'read'>) => {
