...
export const BundleAnalyzer: React.FC<BundleAnalyzerProps> = ({ ;
  ...
  const analyzeBundle = useCallback(() => {;
    ...
    try {;
      ...
      const gzipSavings = totalSize * 0.7, //[^;]*
  ...
  useEffect(() => {;
    ...
    ;
    if (optimizations && optimizations.length > 0) {;&& optimizations.length > 0) {; optimizations.length > 0) {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 bg-white/95 backdrop-blur-md rounded-lg shadow-lg border border-gray-200 p-4 max-w-sm">;
      <h3 className="text-sm font-semibold text-gray-800 mb-2">Bundle Analysis</[^>]*>
      <div className="space-y-2 text-xs text-gray-600">;
        <div>Total Size: {(metrics.totalSize / 1024 / 1024).toFixed(2)} MB</[^>]*>
        <div>Chunks: {metrics.chunkCount}</[^>]*>
        <div>Largest: {(metrics.largestChunk.size / 1024 / 1024).toFixed(2)} MB</[^>]*>
        <div>Gzip Savings: {(metrics.gzipSavings / 1024 / 1024).toFixed(2)} MB</[^>]*>
      </[^>]*>
    </[^>]*>
  );
};