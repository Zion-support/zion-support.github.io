    ...
    const detectDevice = () => {
      ...
      if ('getBattery' in navigator) {
        (navigator as any).getBattery().then((battery: any) => {
          setDeviceInfo(prev =>
            prev ? { ...prev, batteryLevel: battery.level * 100 } : null
          );
        });
      }
    };
                  ...
                  <div className='p-6 bg-gradient-to-r from-zion-green to-zion-blue rounded-xl text-white'>
                    <div className='text-center'>
                      ...
                      <div className='text-sm opacity-75 mt-2'>
                        {features.filter(f => f.enabled).length} of{' '}
                        ...
                        .map((gesture, index) => (
                          ...
                          >
                            <div className='flex items-center space-x-2 mb-2'>
                              ...
                              {gesture.type === 'tap' && (
                                <MousePointer className='w-4 h-4 text-zion-green' />
                              )}
                              {gesture.type === 'longPress' && (
                                <Clock className='w-4 h-4 text-zion-purple' />
                              )}
