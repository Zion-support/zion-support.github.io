...
import { useVirtualScroll } from '../hooks / useVirtualScroll';
  ...
  const handleFilterChange = useCallback ( (key, value, operator) => {
      ...
      trackEvent ('table', 'filter_applied', String (key) , null, {
        operator,
        value,
      }) ;
    },
  ...
  const renderCell = (column, item, index) => {
    ...
    return (<span
        className={`truncate ${column.align === 'center' ? 'text - center' : column.align === 'right' ? 'text - right' : 'text - left'}`}
      >
        {value}
      </span>) ;
  };
    ...
    >
          ...
          {showFilters && (<motion.div
            ...
            >
              ...
              <div className="grid grid - cols - 1 md:grid - cols - 3 gap - 4">
                {columns
                  .filter (col => col.filterable !== false) .map (column => (<div key={String (column.key) } className="space - y-2">
      ...
      <div className="overflow - hidden">
            ...
            {columns.map (column => (<div
              ...
              >
                <button
                  ...
                  className={`w - full flex items - center justify - between px - 2 py - 1 rounded hover:bg - gray - 200 dark:hover:bg - gray - 600 transition - colors ${!enableSorting || !column.sortable ? 'cursor - default' : 'cursor - pointer'}`}
                >
        ...
        <div {...containerProps} className="relative">
          <div {...listProps}>
            {virtualItems.map ( (item, index) => (<motion.div
                key={String (item.id || index) }
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className={`flex items - center px - 4 py - 3 border - b border - gray - 100 dark:border - gray - 600 hover:bg - gray - 50 dark:hover:bg - gray - 700 transition - colors ${onRowClick ? 'cursor - pointer' : ''} ${selectedItems.has (String (item.id || JSON.stringify (item) ) ) ? 'bg - blue - 50 dark:bg - blue - 900 / 20' : ''}`}
