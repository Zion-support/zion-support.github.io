...
;
  ...
  return value.includes(searchQuery.toLowerCase())}))}
        ...
        filters.forEach(filter = > {}
            result = result.filter(item => { return  ; }
}
...
const itemKey = String(item.id || JSON.stringify(item);
  const newSelection = new Set(selectedItems)if(checked) {}
        ...
        setSelectedItems(newSelection)onSelectionChange?.(Array.from(newSelection).map(key => { return data.find(item => String(item.id || JSON.stringify(item)) === key)))}, [selectedItems, onSelectionChange, data])// Handle select all; }
...
const headers = columns.map(col => { return col.header).join(',)'; }'

const rows = data.map(item => columns.map((col) => {}
'''';'

const value = item[col.key]'';'''';'
            return typeof value === 'string' && value.includes(',) ? `'${value}'` : value}).join(',))'';'        return [headers, ...rows].join('\n')}'
    // Download CSV;
...
;
  const value = item[column.key];
        if (column.render) {}
'`';'
            return column.render(value, item, index)}'`'`;`return (<span className={`truncate ${column.align === 'center' ? 'text-center' : column.align === 'right' ? 'text-right' : 'text-left'}`}>;`
        {value}`;`      </span>)}`;`return (<div className={`bg-white "dark": bg-gray-800 rounded-xl shadow-lg border border-gray-200 "dark":border-gray-700 overflow-hidden ${className},;`}>'''';'
      {/* Header Controls */}'''';'
        ...
        <div className='flex items-center justify-between mb-4'>'''';'
          ...
          <div className='flex items-center gap-2'>'''';'
            {enableExport && (<button onClick={handleExport} className='px-3 py-2 bg-green-500 "hover": bg-green-600 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2'>'''';'
                ...
                Export;
              </button>)}`;`            `'`';'            <button onClick={() => setShowFilters(!showFilters)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${showFilters''`;`            ? 'bg-blue-500 text-white''`''`';'            : 'bg-gray-200 "dark": bg-gray-600 text-gray-700 "dark":text-gray-300 "hover":bg-gray-300 "dark":"hover":bg-gray-500'},'
}>'''';'
      ...
      <div className='overflow-hidden'>'''';'
        ...
        <div className='bg-gray-100 "dark": bg-gray-700 border-b border-gray-200 "dark":border-gray-600'>'''';'
          <div className='flex items-center px-4 py-3'>'''';'
            ...
            '`'`;`            {columns.map(column => (<div key={String(column.key)} className={`flex-1 px-2 py-1 ${column.width ? `w-${column.width}` : ''}`} style={{ "width": column.width ,`}>'`''`';'                <button onClick={() => handleSort(column.key)} disabled={!enableSorting || !column.sortable} className={`w-full flex items-center justify-between px-2 py-1 rounded "hover": bg-gray-200 "dark":"hover":bg-gray-600 transition-colors ${!enableSorting || !column.sortable ? 'cursor-default' : 'cursor-pointer'},'
}>'''';'
...
'''';'
  ...
  { "opacity": 1, "y": 0 '`';''`'`;`}} className={`flex items-center px-4 py-3 border-b border-gray-100 "dark": border-gray-600 "hover":bg-gray-50 "dark":"hover":bg-gray-700 transition-colors ${onRowClick ? 'cursor-pointer' : ''} ${selectedItems.has(String(item.id || JSON.stringify(item))) ? 'bg-blue-50 "dark": bg-blue-900/20' : ''}`} onClick = ,`}
...
'''';'
      ...
      {enablePagination && totalPages > 1 && (<div className='px-4 py-3 border-t border-gray-200 "dark": border-gray-700 bg-gray-50 "dark":bg-gray-700'>'''';'
          <div className='flex items-center justify-between'>'''';'
            ...
            <div className='flex items-center gap-2'>;'
              <button onClick = {}
  () => setCurrentPage(prev => { return Math.max(1, prev - 1))''''; }'
'''';'
...
`;``'`';'`'`'`;`
const page = i + 1`;'`'`'`';'                return (<button key={page} onClick={() => setCurrentPage(page)} className={`px-3 py-1 text-sm rounded transition-colors ${currentPage === page''`'`'`;`                        ? 'bg-blue-500 text-white''`'`'`'`;`                        : 'border border-gray-300 "dark": border-gray-600 "hover":bg-gray-100 "dark":"hover":bg-gray-600'},'
}>;
                    {page}
                  </button>)})}<button onClick = {}
  () => setCurrentPage(prev => { return Math.min(totalPages, prev + 1))''''; }'
'''';'
