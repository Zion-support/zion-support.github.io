...
''';
                ...
                ease: 'backOut''';
'''}';''
    return (<motion.div variants={cardVariants} initial='hidden' whileInView='visible' whileHover='hover' viewport={{ once: true }} className='group relative h-full'>''{/* Enhanced Popular Badge */}''''{isPopular && (<motion.div className='absolute -top-3 left-1/2 transform -translate-x-1/2 z-10' initial={{ scale: 0 }} animate={{ scale: 1 }} transition = {}
''';
...
'''{/* Enhanced New Badge */}''''{isNew && (<motion.div className='absolute -top-3 right-4 z-10' initial = {}
  { scale: 0,
  rotate: -180}} animate = {}
  { scale: 1,
  rotate: 0}} transition = {}
...
''''
      <div className='relative h-full bg-gradient-to-br from-white/10 via-white/5 to-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 hover:border-zion-cyan/40 transition-all duration-500 overflow-hidden group-hover:shadow-zion-glow'>'''{/* Enhanced Background Pattern */}''''
...
'''{/* Content */}''''
        <div className='relative z-10'>'''{/* Enhanced Category */}''''
...
'''{/* Enhanced Features */}''''{features.length > 0 && (<div className='mb-6'>''''
              <ul className='space-y-3'>''''{features.slice(0, 3).map((feature, index) => (<motion.li key={index} className='flex items-center gap-3 text-sm text-zion-slate-light group-hover:text-white transition-colors duration-300' variants={featureVariants} whileHover='hover'>''''
