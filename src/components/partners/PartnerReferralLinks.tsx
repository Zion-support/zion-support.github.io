...
shareOnSocialMedia
  ...
  const baseLink  = getReferralLink()const handleCopyLink = (link: string)  => {
navigator && navigator.clipboard.writeText(link)toast({title: 'Link copied!',
description: 'The referral link has been copied to your clipboard',
variant: 'default'
})}
const handleGenerateLink = ()  => {
if (baseLink) {const url  = new URL(baseLink)// Add custom campaign parameter if selected;
      if (selectedCampaign !== 'default') {url && url.searchParams.append('campaign', selectedCampaign)
...
const csvContent = [;
      'Name,
Link',...allLinks && allLinks.map(l => `${l && l.name},${l && l.link}`)].join('\n')const blob = new Blob([csvContent], { type: 'text/csv;
charset = utf-8;' })const url = URL && URL.createObjectURL(blob)const link = document && document.createElement('a')link && link.setAttribute('href', url)link && link.setAttribute('download', 'zion_referral_links && zion_referral_links.csv')link && link.style.visibility = 'hidden';
...
export function PartnerReferralLinks() {
const { referralCode, getReferralLink, copyReferralLink, shareOnSocialMedia 
} = useReferrals(),
const [isDialogOpen, setIsDialogOpen] = useState(false),
const [selectedCampaign, setSelectedCampaign] = useState < string>("default"),
const [customParam, setCustomParam] = useState < string>(""),
const [generatedLinks, setGeneratedLinks] = useState<{name: string, link: string}[]>([]),// Get the base referral link;
...
const handleGenerateLink = ()  => {
if (baseLink) {const url = new URL(baseLink),// Add custom campaign parameter if selected;
      if (selectedCampaign !== "default") {url.searchParams.append("campaign", selectedCampaign)
}// Add custom parameter if provided;
...
const csvContent = [;
      "Name,
Link",...allLinks.map(l => `${l.name},${l.link}`)].join("\n"),
const blob = new Blob([csvContent], { type: 'text/csv,
charset = utf-8,' }),
...
Export Links;
        ...
        <CardContent>;<div className='flex space - x-2'>;
            ...
            <Button variant='outline' on_click={() => handleCopyLink (base_link)}>;
              ...
              <span className='sr - only'>Copy</span>            </Button>;</div>;
        </CardContent>;
      </Card>;
            ...
            <DialogHeader>;
              ...
              <div className='grid gap - 2'>;
                ...
                <Select;
                  ...
                  onValueChange={setSelectedCampaign}                >;<SelectTrigger id='campaign'>;
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>;
          ...
          <DialogContent className="sm:max-w-md bg-zion-blue border-zion-blue-light">;
            <DialogHeader>;
              ...
              <DialogDescription > Generate a trackable link for specific marketing campaigns;
              </DialogDescription>;
            </DialogHeader>;
...
className='bg-zion-purple hover:bg-zion-purple-dark'              >;
      ...
      <div className="grid gap-4">;
        {generatedLinks.length > 0 ? (generatedLinks.map((item, index) => (<Card key={index} className="bg-zion-blue-dark border-zion-blue-light">;
        ...
        {generatedLinks && generatedLinks.length > 0 ? (generatedLinks && generatedLinks.map((item, index) => (<Card;
              key={index}
...
...generated_links ];
link.style.visibility = 'hidden';
document.body.append_child (link)link.click ()document.body.remove_child (link)}";
  base_link ";
}
read_only className="font - mono text - sm"/> <Copy className="h - 4 w - 4"/> <span className="sr - only">Copy</span> </Button> </div> </CardContent> </Card> <div className="flex justify - between items - center"> <DialogTrigger as_child> <Button variant=" outline"size=" sm"className="flex items - center gap - 2"> <Plus className="h - 4 w - 4"/> Create New Link </Button> </DialogTrigger> <DialogContent className="sm:max - w-md bg - zion - blue border - zion - blue - light"> <DialogHeader> <DialogTitle > Create Custom Campaign Link</DialogTitle> <DialogDescription> Generate a trackable link for specific marketing campaigns </DialogDescription> </DialogHeader> <div className="grid gap - 4 py - 4"> <div className="grid gap - 2"> <Label html_for=" campaign">Campaign Type</Label> <Select value= {selected_campaign;
}
onValueChange= {setSelectedCampaign ";
}> <SelectTrigger id=" campaign"> <SelectValue placeholder=" Select campaign type"/> </SelectTrigger> <SelectContent> <SelectItem value=" default">General (Default) </SelectItem> <SelectItem value=" youtube">YouTube</SelectItem> <SelectItem value=" newsletter">Newsletter</SelectItem> <SelectItem value=" blog">Blog</SelectItem> <SelectItem value=" podcast">Podcast</SelectItem> <SelectItem value=" social">Social Media</SelectItem> </SelectContent> </Select> </div> <div className="grid gap - 2"> <Label html_for=" custom">Custom Parameter (Optional) </Label> <Input /> </div> </div> <DialogFooter> <Button > Cancel </Button> <Button className="bg - zion - purple hover:bg - zion - purple - dark"> Generate Link </Button> </DialogFooter> </DialogContent> </Dialog> </div> <div className="grid gap - 4"> {generated_links.length > 0 ? (generated_links.map ( (item, index) => (<Card key= {index ";
}
className="bg - zion - blue - dark border - zion - blue - light"> <CardHeader className="pb - 2"> <CardTitle className="text - base flex items - center justify - between"> <div className="flex items - center gap - 2"> </div> </CardTitle> </CardHeader> <CardContent className="pb - 4"> <div className="flex space - x-2"> <Input value= {item.link ";
}
...
readOnly className="font-mono text-sm"/> <Copy className="h-4 w-4"/> <span className="sr-only">Copy</span> </Button> </div> </CardContent> </Card> <div className="flex justify-between items-center"> <DialogTrigger asChild> <Button variant=" outline"size=" sm"className="flex items-center gap-2"> <Plus className="h-4 w-4"/> Create New Link </Button> </DialogTrigger> <DialogContent className="sm:max-w-md bg-zion-blue border-zion-blue-light"> <DialogHeader> <DialogTitle > Create Custom Campaign Link</DialogTitle> <DialogDescription> Generate a trackable link for specific marketing campaigns </DialogDescription> </DialogHeader> <div className="grid gap-4 py-4"> <div className="grid gap-2"> <Label htmlFor=" campaign">Campaign Type</Label> <Select value= {selectedCampaign ;
}
onValueChange= {setSelectedCampaign ";
}> <SelectTrigger id=" campaign"> <SelectValue placeholder=" Select campaign type"/> </SelectTrigger> <SelectContent> <SelectItem value=" default">General (Default) </SelectItem> <SelectItem value=" youtube">YouTube</SelectItem> <SelectItem value=" newsletter">Newsletter</SelectItem> <SelectItem value=" blog">Blog</SelectItem> <SelectItem value=" podcast">Podcast</SelectItem> <SelectItem value=" social">Social Media</SelectItem> </SelectContent> </Select> </div> <div className="grid gap-2"> <Label htmlFor=" custom">Custom Parameter (Optional) </Label> <Input /> </div> </div> <DialogFooter> <Button > Cancel </Button> <Button className="bg-zion-purple hover:bg-zion-purple-dark"> Generate Link </Button> </DialogFooter> </DialogContent> </Dialog> </div> <div className="grid gap-4"> {generatedLinks.length > 0 ? (generatedLinks.map ( (item, index) => (<Card key= {index ";
}
className="bg-zion-blue-dark border-zion-blue-light"> <CardHeader className="pb-2"> <CardTitle className="text-base flex items-center justify-between"> <div className="flex items-center gap-2"> </div> </CardTitle> </CardHeader> <CardContent className="pb-4"> <div className="flex space-x-2"> <Input value= {item.link ";
}
readOnly className="font-mono text-xs"/> <Button > <Copy className="h-4 w-4"/> <span className="sr-only">Copy</span> </Button> </div> </CardContent> </Card>) ) ) : (<Card className="bg-zion-blue/20 border-dashed border-zion-blue-light"> <CardContent className="flex flex-col items-center justify-center p-6"> <p className="text-zion-slate-light text-center mb-4"> Create custom campaign links to track different marketing efforts </p> <Button className="flex items-center gap-2"> <Plus className="h-4 w-4" /> Create First Link </Button> </CardContent> </Card>)}</div> </div>)}'";