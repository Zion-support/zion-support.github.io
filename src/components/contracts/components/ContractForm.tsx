...
export function ContractForm(_{talent, clientName, initialValues, onFormValuesChange, onContractGenerated}: ContractFormProps) {const [isGenerating, setIsGenerating] = useState(false);
  const [generatedMilestones, setGeneratedMilestones] = useState<GeneratedMilestone[]>([]);
  const { toast} = useToast();
...
if (form.getValues("paymentTerms") !== "milestone") {
  ...
  const onSubmit = async (values: ContractFormValues) => {setIsGenerating(true);
    try {
      ...
      onContractGenerated(contract)} catch (error) {logErrorToProduction('Error generating contract:', _{ data: error});
      toast({title: "Contract Generation Failed", description: error instanceof Error ? error.message : "Something went wrong. Please try again.", variant: "destructive"})
