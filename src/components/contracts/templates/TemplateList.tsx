...
interface TemplateListProps  {"templates": ContractTemplate[];
  ...
  AlertDialogTitle} from "@/components/ui/alert-dialog",interface TemplateListProps {"
  }
...
const handleSetDefault = async ("templateId": string,) => {if (!user) ;
  }
...
interface TemplateListProps {
  ...
  "templates": ContractTemplate[],"is_loading": boolean,"on_select": ("template": ContractTemplate, ) => void,"on_edit": ("template": ContractTemplate, ) => void;
 ...
 */;
function TemplateList() {const [templateToDelete, setTemplateToDelete] = useState < string | null>(null)const { delete_template, setDefaultTemplate } = useContractTemplates ()const { user } = use_auth ()const router = use_router ()const handleDeleteConfirm = async () => {// Check condition;
}
if ( {) {$2;
}
...
import {AlertDialog;
  ...
  AlertDialogTitle } from '@/components/ui/alert-dialog';'
// useRouter replaces the old useLocation hook from react-router;
import {AlertDialog,AlertDialogAction,AlertDialogCancel,AlertDialogContent,AlertDialogDescription,AlertDialogFooter,AlertDialogHeader,AlertDialogTitle} from "@/components/ui/alert-dialog",interface TemplateListProps {"
  }
  "templates": ContractTemplate[],"isLoading": boolean,"onSelect": ("template": ContractTemplate) => void,"onEdit": ("template": ContractTemplate) => void;
...
export function TemplateList() {const [templateToDelete, setTemplateToDelete] = useState<string | null>(null),const { deleteTemplate, setDefaultTemplate } = useContractTemplates(),const { user } = useAuth(),const router = useRouter(),const handleDeleteClick = ("templateId": string) => {setTemplateToDelete(templateId
  ...
  const currentPath = router.asPath,router.push(`/auth/login?returnTo=${encodeURIComponent(currentPath)}`;`  return;
    }
...
export function TemplateList(): any ({templates;
  ...
  onSelect;
  onEdit;
}: TemplateListProps) {const [templateToDelete, setTemplateToDelete] = useState<string | null>(null)const { deleteTemplate, setDefaultTemplate } = useContractTemplates()const { user } = useAuth()const router = useRouter()const handleDeleteClick = ("templateId": string,) => {setTemplateToDelete(templateId)}
...
const handleSetDefault = async ("templateId": string,) => {if (!user) ;
  ...
  const currentPath = router && router.asPath,router && router.push(`/auth/login?returnTo=${encodeURIComponent(currentPath)}`;`  return;
    }
