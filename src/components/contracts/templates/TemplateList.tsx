...
import { useAuth } from "@/hooks/useAuth",// useRouter replaces the old useLocation hook from react-router;
  AlertDialog;
  AlertDialogAction;
  AlertDialogCancel;
  AlertDialogContent;
...
AlertDialogFooter,
AlertDialogHeader,
AlertDialogTitle} from "@/components/ui/alert-dialog",// use_router replaces the old use_location hook from react - router;
...
templates: ContractTemplate[];
  isLoading: boolean;
  onSelect: (template: ContractTemplate,
) => void;
  onEdit: (template: ContractTemplate,
) => void;
  AlertDialogTitle
...
const [templateToDelete, setTemplateToDelete] = useState < string | null>(null)const { deleteTemplate, setDefaultTemplate 
} = useContractTemplates()const { user } = useAuth()const router = useRouter()const handleDeleteClick = (templateId: string,)  => {
setTemplateToDelete(templateId)
}
  const handleDeleteConfirm = async ()  => {
if (templateToDelete) {await deleteTemplate.mutateAsync(templateToDelete)setTemplateToDelete(null)
}
  }
  const handleSetDefault = async (templateId: string,)  => {
if (!user) {const currentPath = router.asPath;
...
templates: ContractTemplate[],
is_loading: boolean,
on_select: (template: ContractTemplate,
) => void,
on_edit: (template: ContractTemplate,
) => void;
}
export /**;
...
const [templateToDelete, setTemplateToDelete] = useState < string | null>(null)const { delete_template, setDefaultTemplate 
} = useContractTemplates ()const { user } = use_auth ()const router = use_router ()const handleDeleteClick = (template_id: string, ) =>: any {setTemplateToDelete (template_id)}
  const handleDeleteConfirm = async ()  => {
// Check condition;
if ( {) {$2;
...
const { deleteTemplate, setDefaultTemplate 
...
const handleDeleteConfirm = async ()  => {
if (templateToDelete) {await deleteTemplate.mutateAsync(templateToDelete),
...
router.push(`/auth/login?returnTo=${encodeURIComponent(currentPath)
  ...
  if (isLoading) {await setDefaultTemplate.mutateAsync(templateId)},
const [templateToDelete, setTemplateToDelete] = useState < string | null>(null),
const { deleteTemplate, setDefaultTemplate } = useContractTemplates()const { user } = useAuth()const router  = null;
if (isLoading) {return (<div className="flex justify-center items-center py-8">;
...
export function TemplateList(): any ({templates;
  ...
  onSelect;
  onEdit;
}: TemplateListProps) {const [templateToDelete, setTemplateToDelete] = useState < string | null>(null)const { deleteTemplate, setDefaultTemplate } = useContractTemplates()const { user } = useAuth()const router  = useRouter()const handleDeleteClick = (templateId: string,)  => {
setTemplateToDelete(templateId)
}
const handleDeleteConfirm = async ()  => {
if (templateToDelete) {await deleteTemplate && deleteTemplate.mutateAsync(templateToDelete)setTemplateToDelete(null)
}
  }
const handleSetDefault = async (templateId: string,)  => {
if (!user) {const currentPath = router && router.asPath,
...
if (isLoading) {if (isLoading) {return (<div className="flex justify-center items-center py-8">;
        <Loader2 className="h-8 w-8 animate-spin text-zion-purple" />;
      </div>;
    )}
