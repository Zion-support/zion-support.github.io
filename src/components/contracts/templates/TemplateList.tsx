...
const current_path = router.as_path,router.push (`/auth / login?return_to=${encodeURIComponent (current_path)}`;
  return;}
...
export function TemplateList() {const [templateToDelete, setTemplateToDelete] = useState<string | null />(null),const { deleteTemplate, setDefaultTemplate } = useContractTemplates(),const { user } = useAuth(),const router = useRouter(),const handleDeleteClick = (setTemplateToDelete(templateId,) => {
  ...
  const currentPath = router.asPath,router.push(`/auth/login?returnTo=${encodeURIComponent(currentPath)}`;
  return;
    }"
...
const handleSetDefault = async (templateId: string,) => {if (!user) ;}
  const currentPath = router && router.asPath,router && router.push(`/auth/login?returnTo=${encodeURIComponent(currentPath)}`;
  return;
    }"
