    ...
    return (
      <div key={item.name}>
        <div className="flex items-center">
          {hasChildren ? (;
            <button;
              onClick={{onClick={() => toggleSection(item.name.toLowerCase())}}}
        ...
        {hasChildren && isExpanded && (;
          <motion.div;
            ...
            className="className="ml-4 mt-1 space-y-1";"
          >
  ...
  return (
    <>
      ...
      <button;
        ...
        className="className="lg:hidden fixed top-4 left-4 z-50 p-2 rounded-md bg-slate-800/80 text-slate-300 hover:text-white hover:bg-slate-700/80 transition-colors";"
      >
      ...
      <AnimatePresence>
        {isOpen && (;
          <motion.div;
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="className="fixed inset-0 z-50 lg:hidden";"
          >
            {{/* Backdrop */}}
            <div ;
            ...
            <motion.div;
              ...
              className="className="absolute left-0 top-0 h-full w-80 bg-slate-900/95 border-r border-slate-700/50 backdrop-blur-xl";"
            >
              {{/* Header */}}
              <div className="flex items-center justify-between p-4 border-b border-slate-700/50">
                ...
                <button;
                  onClick={{onClick={() => setIsOpen(false)}}}
                  className="className="p-2 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 transition-colors";"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
      ...
      <div className="hidden lg:block fixed left-0 top-0 h-full w-80 bg-slate-900/95 border-r border-slate-700/50 backdrop-blur-xl z-40">
        ...
        <div className="p-4 border-t border-slate-700/50">
          <div className="text-center">
            <Link;
              to="/contact";
              className="className="w-full px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-sm font-medium rounded-lg hover: from-cyan-400 hover:to-blue-500 transition-all duration-200 hover:scale-105 shadow-lg shadow-cyan-500/20";"
            >
              Get Started;
            </Link>
          </div>
