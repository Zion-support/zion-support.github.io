import {;
  ...
  Workflow, Atom,
  Star, Building,
  BookOpen, HelpCircle,  ChevronRight, Menu,
  X, Users,
  ...
  ChevronDown, Plus,
  Scale, GraduationCap,
  Package, User} from 'lucide-react';''

export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle }) => {

  
  const [expandedSections, setExpandedSections] = useState<string[]>(['''    'main', services',''  ]);'  const navigation: SidebarItem[] = [
    {;

      id: 'main', title: 'Main Navigation','      icon: Home, links: ['''        { name: 'Home', href: '/', icon: Home, description: 'Main dashboard' }, {''          name: 'About Us','          href: '/about', icon: Building,'          description: 'Our story and mission'}, {''          name: 'Contact','          href: '/contact', icon: PhoneCall,'          description: 'Get in touch'}, {''          name: 'Get Quote','          href: '/request-quote', icon: DollarSign,'          description: 'Request pricing'}, {''          name: 'Schedule Demo','          href: '/schedule-demo', icon: Calendar,'          description: 'Book a demo'},  ]}, {''      id: 'services','      title: 'Services Portfolio', icon: Settings,'      links: ['        {;
      ...
      id: 'ai-solutions','      title: 'AI Solutions', icon: Brain,'      items: ['        {;
      ...
      id: 'enterprise-services','      title: 'Enterprise Services', icon: Building,'      links: ['        {;
      ...
      id: 'specialized-services','      title: 'Specialized Services', icon: Star,'      links: ['        {;
      ...
      id: 'micro-saas-products','      title: 'Micro SaaS Products', icon: ShoppingCart,'      links: ['        {;
      ...
      id: 'innovative-showcase','      title: 'Innovation Hub', icon: Star,'      links: ['        {;
      ...
      id: 'pricing','      title: 'Pricing & Plans', icon: DollarSign,'      links: ['        {;
      ...
      id: 'resources','      title: 'Resources & Learning', icon: BookOpen,'      links: ['        {;
      ...
      id: 'company','      title: 'Company', icon: Building,'      links: ['        {;
      ...
      id: 'platform','      title: 'Platform', icon: Settings,'      links: ['        {;
          ...
          name: 'Community', href: '/community','          icon: Users, description: 'User community'},'        { name: 'Login', href: '/login', icon: User, description: 'Sign in' },'        {';
          name: 'Sign Up', href: '/signup','          icon: Plus, description: 'Create account'},'      ]}, {';
      id: 'legal','      title: 'Legal & Policies', icon: FileText,'      links: ['        {;
          ...
          name: 'Accessibility', href: '/accessibility','          icon: Heart, description: 'Accessibility features'},'      ]},  ]}'    return location.pathname.startsWith (href) };
  if (!isOpen) {

    return();
      <div className="fixed left-0 top-16 z-40 w-16 h-full bg-slate-900/95 backdrop-blur-md border-r border-slate-700/50 lg: block hidden">"""        <div className="flex flex-col items-center py-4 space-y-4">"          {navigationSections.slice(0, 6).map(section => {"
            
            return()              <button
                key={section.id}
                onClick={onToggle}"""                className="p-3 rounded-lg text-slate-300 hover: text-white hover:bg-slate-800 transition-colors""                title={section.title}"              >"""                <Icon className="w-5 h-5"  />              </button>"            )})}"        </div>
      </div>) }
  return ("""    <div className="fixed left-0 top-16 z-40 w-80 h-full bg-slate-900/95 backdrop-blur-md border-r border-slate-700/50 overflow-y-auto">"""      <div className="p-4">"""        <div className="flex items-center justify-between mb-6">"""          <h2 className="text-lg font-semibold text-white">Navigation</h2>"          <button"            onClick={onToggle}"""            className="p-2 rounded-lg text-slate-300 hover: text-white hover:bg-slate-800 transition-colors""          >"""            <X className="w-5 h-5"  />          </button>"        </div>""""        <div className="space-y-2">"          {navigationSections.map(section => {"
            
            
            return ("              <div key={section.id} className="space-y-1">""                <button"                  onClick={() => toggleSection(section.id)}"""                  className="w-full flex items-center justify-between p-3 rounded-lg text-slate-300 hover: text-white hover:bg-slate-800 transition-colors""                >"""                  <div className="flex items-center space-x-3">"""                    <Icon className="w-5 h-5"  />"""                    <span className="font-medium">{section.title}</span>"                  </div>"                  <ChevronDown'''                    className={`w-4 h-4 transition-transform ${isExpanded ? 'rotate-180' : ''}`}'                  />                </button>'`                {isExpanded && ("""                  <div className="ml-4 space-y-1">"                    {section.links.map(link => {"
                      
                      
                      return()                        <Link
                          key={link.href}
                          to={link.href}``                          className={`block p-3 rounded-lg transition-colors group ${`
                            active'''                              ? 'bg-cyan-600 text-white''''                              : 'text-slate-300 hover:text-white hover:bg-slate-800'`'                          }`}'                        >"""                          <div className="flex items-center space-x-3">"""                            <LinkIcon className="w-4 h-4"  />"""                            <div className="flex-1 min-w-0">"                              <div className="font-medium">{link.name}</div>""                              {link.description && ("                                <div'`''                                  className={`text-xs ${active ? 'text-cyan-100' : 'text-slate-400'}`}'                                >'                                  {link.description}`                                </div>
                              )}
                            </div>"""                            {active && <ChevronRight className="w-4 h-4"  />}"                          </div>"                        </Link>) }) }
                  </div>) }
              </div>) }) }        </div>

        {/* Quick Actions */}"""        <div className="mt-8 pt-6 border-t border-slate-700">"""          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">"            Quick Actions"          </div>"""          <div className="space-y-2">"            <Link"""              to="/request-quote""              className="block p-3 rounded-lg bg-gradient-to-r from-cyan-600 to-blue-600 text-white hover: from-cyan-700 hover:to-blue-700 transition-colors""            >"""              <div className="flex items-center space-x-3">"""                <DollarSign className="w-4 h-4"  />"""                <span className="font-medium">Get Quote</span>"""                <ExternalLink className="w-4 h-4 ml-auto"  />              </div>"            </Link>"            <Link"""              to="/schedule-demo""              className="block p-3 rounded-lg border border-slate-600 text-slate-300 hover:text-white hover:bg-slate-800 transition-colors""            >"""              <div className="flex items-center space-x-3">"""                <Calendar className="w-4 h-4"  />"""                <span className="font-medium">Schedule Demo</span>"""                <ExternalLink className="w-4 h-4 ml-auto"  />              </div>"            </Link>"          </div>
        </div>

        {/* Contact Info */}"""        <div className="mt-6 pt-6 border-t border-slate-700">"""          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">"            Contact"          </div>"""          <div className="space-y-2 text-xs text-slate-400">"""            <div className="flex items-center space-x-2">"""              <Mail className="w-3 h-3"  />"              <span>contact@ziontechgroup.com</span>"            </div>"""            <div className="flex items-center space-x-2">"""              <PhoneCall className="w-3 h-3"  />              <span>+1 (555) 123-4567</span>"            </div>"          </div>
        </div>
      </div>
    </div>) };
'"`'"""`