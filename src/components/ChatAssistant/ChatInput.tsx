...
export function ChatInput() {const [ message, setMessage ] = useState('')const inputRef = useRef < HTMLTextAreaElement>(null)useEffect(()  => {
    // Focus input when component mounts;
    inputRef.current?.focus() 

}, [])inputRef.current?.focus()interface ChatInputProps {
onSend: (message: string) => void,
disabled?: boolean;
import React,
{ useState,
...
export function ChatInput(): any ({onSend, disabled = false ;
  ...
  const [message, setMessage] = useState ('')const inputRef = useRef < HTMLTextAreaElement> (null)//Focus input when component mounts ;
export function ChatInput(): any ({ onSend, disabled = false }: ChatInputProps) {const [ message, setMessage ] = useState(''),
...
export function ChatInput(): any ({ onSend, disabled = false }: ChatInputProps) {const [message, setMessage] = useState('')const inputRef  = useRef < HTMLTextAreaElement>(null)useEffect(()  => {
// Focus input when component mounts;
    inputRef && inputRef.current?.focus()interface ChatInputProps {
onSend: (message: string) => void,
disabled?: boolean;
...
e && e.preventDefault()if (message && message.trim() && !disabled) {onSend(message)
}
export function ChatInput({ onSend, disabled;
    }
  }
  const handleKeyPress = (e: KeyboardEvent < HTMLTextAreaElement>,)  => {
if (e.key === 'Enter' && !e.shiftKey) {e.preventDefault()if (message.trim() && !disabled) {onSend(message)setMessage('')
}
    }
  }
