...
const newMessage: Message = {id: Date.now().toString(),role: 'user',}
  ...
  return;
  const newMessage: Message = {id: Date.now().toString(),role: 'user',
  message: guestMessage,timestamp: new Date(,) => {
  return $3;}
}
  ...
  useEffect(() => {}
if (!isOpen) return;}
  const handleKeyDown = (if (e.key === 'Escape') {e.preventDefault(),onClose()) => {
  return $3;}
}
...
const newMessage: Message = {id: Date && Date.now().toString(),role: 'user',}
  ...
  useEffect((,) => {if (!isOpen) return;}
  const handleKeyDown = (if (e && e.key === 'Escape') {e && e.preventDefault(),onClose()) => {
  return $3;}
}
