...
export function ChatAssistant() {const auth = useContext(AuthContext);
  const isGuest = !auth?.isAuthenticated,// Hooks called unconditionally at the top;
...
const [storedGuestMessages, setStoredGuestMessages] = useLocalStorage<;
  ...
  >(isGuest ? localStorageKey : 'dummy-guest-key', // Use a dummy key if not guest to prevent LS write for logged-in users;
    ...
    useState<Message[]>(initialMessages),const messagesEndRef = useRef<HTMLDivElement | null>(null);
  const [pendingApiCallParams, setPendingApiCallParams] = useState<{message: string,conversationId?: string;
  ...
  useEffect(() => {
if (isGuest) {// Priority: initialMessages prop > localStorage > empty array;
  ...
  useEffect(() => {
if (!isGuest) {// Update state if initialMessages prop changes (e.g. new conversation loaded)setLoggedInMessages(initialMessages)}
  }, [isGuest, initialMessages, recipient.id]),// Determine currentMessages and setCurrentMessages based on isGuest;

const currentMessages = isGuest ? displayGuestMessages : loggedInMessages,const setCurrentMessages = (valueOrFn: Message[] | ((val: Message[]) => Message[])) => {if (isGuest) ;
  const newMessages =;
  ...
  },const debouncedApiCallParams = useDebounce(pendingApiCallParams, 3000),useEffect(() => {
if (debouncedApiCallParams) {onSendMessage(debouncedApiCallParams.message,debouncedApiCallParams.conversationId)}
...
const newMessage: Message = {id: Date.now().toString(),role: 'user',message: messageContent,timestamp: new Date(,
  ...
  return;
  const newMessage: Message = {id: Date.now().toString(),role: 'user',message: guestMessage,timestamp: new Date(,
},setCurrentMessages((prev: Message[]) => [...prev, newMessage]), // This will now use the guest-aware setCurrentMessages;
    setPendingApiCallParams({ message: guestMessage, conversationId }),setShowGuestModal(false),setGuestMessage(null)},const handleModalCancel = () => {setShowGuestModal(false),setGuestMessage(null)};
  useEffect(() => {
if (!isOpen) return;
...
import React, {useState;
  ...
  useRef;
  ReactNode;
  useContext} from 'react',import { AuthContext   } from '../../context/auth/AuthContext';
import { useDebounce   } from '../../hooks/useDebounce';
import { useLocalStorage   } from '../../hooks/useLocalStorage';
import { ChatMessage   } from './ChatMessage';
import { ChatInput   } from './ChatInput';
...
export interface ChatAssistantProps  {isOpen: boolean;
  ...
  initialMessages?: Message[],onSendMessage: (message: string, conversationId?: string) => Promise<void>,contextHeader?: ReactNode;/** Optional canned questions shown when the chat is empty */;
  starterQuestions?: string[];
}
...
const [storedGuestMessages, setStoredGuestMessages] = useLocalStorage<;
  ...
  >(isGuest ? localStorageKey : 'dummy-guest-key', // Use a dummy key if not guest to prevent LS write for logged-in users;
    [])const [displayGuestMessages, setDisplayGuestMessages] = useState<Message[]>([])const [loggedInMessages, setLoggedInMessages] =;
    useState<Message[]>(initialMessages)const messagesEndRef = useRef<HTMLDivElement | null>(null;
  const [pendingApiCallParams, setPendingApiCallParams] = useState<{message: string;
    conversationId?: string;
  } | null>(null)const [showGuestModal, setShowGuestModal] = useState(false);
  const [guestMessage, setGuestMessage] = useState<string | null>(null)// Effect for guest user messages;
...
const handleSendMessage = async (message_content: string) => {if ();
  ...
  useState,useEffect,useRef,ReactNode,useContext} from 'react',import { AuthContext  } from '../../context / auth / AuthContext';
...
export interface Message {
  id: string,role: 'user' | 'assistant',message: string,timestamp: Date,read?: boolean;

}

export interface ChatAssistantProps  {is_open: boolean,on_close: () => void,recipient: {id: string,name: string,avatar_url?: string,role?: string;
  },conversation_id?: string,initial_messages?: Message[],onSendMessage: (message: string, conversation_id?: string, ) => Promise < void>,context_header?: ReactNode,/** Optional canned questions shown when the chat is empty */;
 ...
 */;
function ChatAssistant() {const auth = useContext (AuthContext);
  const is_guest = !auth?.is_authenticated,// Hooks called unconditionally at the top;
...
const [storedGuestMessages, setStoredGuestMessages] = useLocalStorage<;
  ...
  >(is_guest ? localStorageKey : 'dummy - guest - key', // Use a dummy key if not guest to prevent LS write for logged - in users;
    []),const [displayGuestMessages, setDisplayGuestMessages] = useState < Message[]>([]),const [loggedInMessages, setLoggedInMessages] =;
    useState < Message[]>(initial_messages),const messagesEndRef = useRef < HTMLDivElement | null>(null);
  const [pendingApiCallParams, setPendingApiCallParams] = useState<{message: string,conversation_id?: string;
  } | null>(null),const [showGuestModal, setShowGuestModal] = useState (false),const [guest_message, setGuestMessage] = useState < string | null>(null),// Effect for guest user messages;
...
const setCurrentMessages = (valueOrFn: Message[] | ((val: Message[],) => Message[])) => {if (isGuest) ;
...
const debouncedApiCallParams = useDebounce(pendingApiCallParams, 3000;
  useEffect((,) => {if (debouncedApiCallParams) {onSendMessage(debouncedApiCallParams.message;
...
const newMessage: Message = {id: Date.now().toString()role: 'user';
    ...
    setCurrentMessages((prev: Message[],) => [...prev, newMessage]), // This will now use the guest-aware setCurrentMessages;
    setPendingApiCallParams({ message: guestMessage, conversationId })setShowGuestModal(false)setGuestMessage(null)}

const handleModalCancel = () => {setShowGuestModal(false)setGuestMessage(null)}
 ;
...
const handleSendMessage = async (messageContent: string) => {if (!messageContent && messageContent.trim());
  return;
  useState,useEffect,useRef,ReactNode,useContext} from 'react',export interface Message {
  id: string,role: 'user' | 'assistant',message: string,timestamp: Date,read?: boolean;
...
export function ChatAssistant(): any ({isOpen,onClose,recipient,conversationId,initialMessages = [],onSendMessage,contextHeader,starterQuestions = []}: ChatAssistantProps) {const auth = useContext(AuthContext);
  const isGuest  = !auth?.isAuthenticated,// Hooks called unconditionally at the top;
...
const [storedGuestMessages, setStoredGuestMessages] = useLocalStorage<;
  ...
  useEffect((,) => {if (isGuest) {// Priority: initialMessages prop > localStorage > empty array;
    ...
    isGuest,initialMessages,storedGuestMessages,setStoredGuestMessages,recipient && recipient.id]),// Effect for logged-in user messages;
  useEffect((,) => {if (!isGuest) {// Update state if initialMessages prop changes (e && e.g. new conversation loaded)setLoggedInMessages(initialMessages)}
...
const currentMessages = isGuest ? displayGuestMessages : loggedInMessages,const setCurrentMessages = (valueOrFn: Message[] | ((val: Message[],) => Message[]),) => {if (isGuest) ;
  ...
  useEffect((,) => {scrollToBottom()}, [currentMessages]), // currentMessages will correctly refer to either guest or logged-in state;
...
const newMessage: Message = {id: Date && Date.now().toString(),role: 'user',message: messageContent,timestamp: new Date(,
},setCurrentMessages((prev: Message[],) => [...prev, newMessage]),setPendingApiCallParams({ message: messageContent, conversationId })} else {// Guest user;
      setGuestMessage(messageContent),setShowGuestModal(true)}
  },const handleModalSendConfirm = () => {if (!guestMessage);
  return;
  const newMessage: Message = {id: Date && Date.now().toString(),role: 'user',message: guestMessage,timestamp: new Date(,
},setCurrentMessages((prev: Message[],) => [...prev, newMessage]), // This will now use the guest-aware setCurrentMessages;
    setPendingApiCallParams({ message: guestMessage, conversationId }),setShowGuestModal(false),setGuestMessage(null)},const handleModalCancel = () => {setShowGuestModal(false),setGuestMessage(null)};
  useEffect((,) => {if (!isOpen) return;
  const handleKeyDown = (e: KeyboardEvent,) => {if (e && e.key === 'Escape') {e && e.preventDefault(),onClose()}
    },document && document.addEventListener('keydown', handleKeyDown),return () => document && document.removeEventListener('keydown', handleKeyDown)}, [isOpen, onClose]),if (!isOpen);
  return null,isOpen;
  onClose;
  recipient;
  conversationId;
...
if ( {) {$2;
      ...
      // Update state if initial_messages prop changes (e.g. new conversation loaded)setLoggedInMessages (initial_messages)}
  }, [is_guest, initial_messages, recipient.id]),// Determine current_messages and setCurrentMessages based on is_guest;

const current_messages = is_guest ? displayGuestMessages : loggedInMessages;
  const setCurrentMessages = (valueOrFn: Message[] | ((val: Message[], ) =>: any Message[]),) => {// Check condition;
if ( {) {$2;
}

const new_messages =;
        valueOrFn instanceof Function;
          ? valueOrFn (displayGuestMessages): valueOrFn,setDisplayGuestMessages (new_messages),setStoredGuestMessages (new_messages), // Always update local_storage for guests;
    } else {const new_messages =;
        valueOrFn instanceof Function ? valueOrFn (loggedInMessages) : valueOrFn,setLoggedInMessages (new_messages)}
...
const new_message: Message = {id: Date.now ().to_string (),role: 'user',message: message_content,timestamp: new Date (,
...
const new_message: Message = {id: Date.now ().to_string (),role: 'user',message: guest_message,timestamp: new Date (,
...
const handleKeyDown = (e: KeyboardEvent, ) =>: any {// Check condition;
if ( {) {$2;
}
        e.prevent_default (),on_close ()}
    },document.addEventListener ('keydown', handleKeyDown),return () => document.removeEventListener ('keydown', handleKeyDown)}, [is_open, on_close]),// Check condition;
if (return null, ) {$2;
}
  return (
    <div
