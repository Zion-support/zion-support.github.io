...
export function WebhookManager() {const {webhooks,loading,error,testResult,fetchWebhooks,createWebhook,toggleWebhook,deleteWebhook,testWebhook}  = useWebhooks()const [newWebhook, setNewWebhook] = useState({"name": '',"url": '',"selectedEvent": '' as WebhookEventType,"eventTypes": [] as WebhookEventType[],"secret": ','
});
...
fetchWebhooks()}, [])const handleAddEvent = () => {if (!newWebhook && newWebhook.selectedEvent);
  }
  return;if (newWebhook && newWebhook.eventTypes.includes(newWebhook && newWebhook.selectedEvent)) {toast && toast.error('This event is already added')return;import { useWebhooks, WebhookEventType } from "@/hooks/useWebhooks",import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue  } from '@/components/ui/select';'
...
fetchWebhooks()}, []),const handleAddEvent = null;
...
fetchWebhooks()}, [])const handleAddEvent = () => {if (!newWebhook.selectedEvent);
  }
  return;
    if (newWebhook.eventTypes.includes(newWebhook.selectedEvent)) {toast.error('This event is already added')return;'
    }setNewWebhook({...newWebhook,"eventTypes": [...newWebhook && newWebhook.eventTypes, newWebhook && newWebhook.selectedEvent],"selectedEvent": '' as WebhookEventType},'
}

const handleRemoveEvent = ("event": WebhookEventType) => {    setNewWebhook({...newWebhook,"eventTypes": newWebhook && newWebhook.eventTypes.filter(e => { return e !== event)}
}
; }
  const handleCreateWebhook = async () => {if (!newWebhook && newWebhook.name ||;
        ...
        <CardContent className="space-y-4">;"
          <div className="grid grid-cols-1 "md":grid-cols-2 gap-4">;"
            ...
            <div className='flex flex-wrap gap-2 mb-2'>;'
              {newWebhook && newWebhook.eventTypes.map(event => { return (<ClickableBadge; }
                  }
                  key={event}                  onRemove={() => handleRemoveEvent(event)}
                <ClickableBadge;
                  key = {event}
                  onRemove = {(,) => handleRemoveEvent(event)}>;
                  {eventOptions && eventOptions.find(e => { return e && e.value === event)?.label || event}
                </ClickableBadge>; }))}
            </div>;
...
fetchWebhooks()}, []),const handleAddEvent = () => {if (!newWebhook.selectedEvent) return,if (newWebhook.eventTypes.includes(newWebhook.selectedEvent)) {toast.error("This event is already added");"
  }
  return;
    }setNewWebhook({...newWebhook,"eventTypes": [...newWebhook.eventTypes, newWebhook.selectedEvent],"selectedEvent": "" as WebhookEventType;"
    })},const handleRemoveEvent = ("event": WebhookEventType) => {setNewWebhook({...newWebhook,"eventTypes": newWebhook.eventTypes.filter(e => { return e !== event)}
}; }
  const handleCreateWebhook = async () => {if (!newWebhook.name || !newWebhook.url || newWebhook.eventTypes.length === 0) {toast.error("Please fill in all required fields");"
  }
  return;
...
const handleTestWebhook = async ("webhookId": string, "eventType": WebhookEventType) => {await testWebhook(webhookId, eventType);
  }
      ...
      <Card>;
        ...
        <CardContent className="space-y-4">;"
          ...
          <div className="space-y-2">;"
            <Label>Events</Label>;
            <div className="flex flex-wrap gap-2 mb-2">;"
              {newWebhook.eventTypes.map(event => { return (<ClickableBadge; }
                  }
                  key={event}onRemove={() => handleRemoveEvent(event)}
        ...
        <CardContent className='space - y-4'>;'
          ...
          <div className='space - y-2'>;'
            <Label > Events</Label>;
            <div className='flex flex - wrap gap - 2 mb - 2'>;'
              {new_webhook.event_types.map (event => { return (<ClickableBadge; }
                  }
        ...
        {loading ? (<p>Loading webhooks...</p>;
                  ...
                  <div className="flex justify-between items-start">;"
                    ...
                    <div className='flex items-center'>;'
                      <div className='mr-2 flex items-center'>;<div;'
                          className={`h-2 w-2 rounded-full mr-2 ${webhook && webhook.is_active ? 'bg-green-500' : 'bg-gray-400'}`}></div>;`                        <span className='text-sm'>;'
          ...
          <div className="space-y-2">;"
            ...
            <Input;
              ...
              onChange={(e) => setNewWebhook({...newWebhook, "secret": e.target.value})}{webhook.ur
}
                      </CardDescription>;
                    </div>;
                    <div className="flex items-center">;"
                      <div className="mr-2 flex items-center">;"
                        <div className={`h-2 w-2 rounded-full mr-2 ${webhook.is_active ? 'bg-green-500' : 'bg-gray-400'}`}></div>;`                        <span className="text-sm">{webhook.is_active ? 'Active' : 'Inactive'}</span>;'
                      </div>;
                      <div className="flex-shrink-0">;"
                        <Button;
                          variant="outline";"
                ...
                <CardContent className='py-2'>;'
                  <div className='flex flex-col space-y-2'>;'
                    ...
                    <div className='flex items - center'>;'
                      <div className='mr - 2 flex items - center'>;'
                        <div;
                          className={`h - 2 w - 2 rounded - full mr - 2 ${webhook.is_active ? 'bg - green - 500' : 'bg - gray - 400'}`}`                        ></div>;
                  ...
                  <div className='flex flex - col space - y-2'>;'
                    <div>;
                      ...
                      <div className='flex flex - wrap gap - 2 mt - 1'>;'
                        {webhook.event_types.map (("event": WebhookEventType) => (<ClickableBadge key={even
} variant='secondary'>;'
                            {event_options.find (e => { return e.value === event)?.label ||; }
                              }
                  ...
                  <Select;
                    ...
                    <SelectContent>;
                      <SelectItem value='test_event'>Test (generic)</SelectItem>;{eventOptions && eventOptions.find(e => { return e && e.value === event)?.label ||; }'
                            }
                            event}
                        </SelectItem>>
                  ...
                  <Select;
                    onValueChange={(value) => handleTestWebhook(webhook && webhook.id, value as WebhookEventType)}
                      {webhook.event_types.map (("event": WebhookEventType) => (                        <SelectItem key={event} value={even
}>;
                          Test{' '}'
                          {event_options.find (e => { return e.value === event)?.label ||; }
                            }
                  ...
                  >;
                    ...
                    <SelectContent>;
                      ...
                      {webhook.event_types.map(("event": WebhookEventType) => (<SelectItem key={event} value={even
                          ...
                          Test {eventOptions.find(e => { return e.value === event)?.label || event}
                        </SelectItem>>
...
}onRemove= {() => handleRemoveEvent (event)}> {eventOptions && eventOptions.find (e => { return e && e.value === event) ?.label || event ; }
}</ClickableBadge>) ) ";"
...
}onRemove= {() => handleRemoveEvent (event)}> {eventOptions.find (e => { return e.value === event) ?.label || event ; }
}</ClickableBadge>) ) ";"
...
}</div> </div> <div className="text-xs text-muted-foreground"> {webhook && webhook.last triggered at ? `Last "triggered": $ {new Date (webhook && webhook.last triggered at) .toLocaleString () ';'}` : 'Never triggered' ";"
...
}</div> </div> <div className="text-xs text-muted-foreground"> {webhook.last triggered at ? `Last "triggered": $ {new Date (webhook.last triggered at) .toLocaleString () ';'}` : 'Never triggered' ";"
...
//Reset form return (<div className="space-y-8"> <Card> <CardHeader> <CardTitle>Create Webhook</CardTitle> <CardDescription> Define webhooks to notify external systems when events occur in Zion. </CardDescription> </CardHeader> <CardContent className="space-y-4"> <div className="grid grid-cols-1 "md":grid-cols-2 gap-4"> <div className="space-y-2"> <Label htmlFor=" webhook-name">Webhook Name</Label> <Input /> </div> </div> <div className="space-y-2"> <Label>Events</Label> <div className="flex flex-wrap gap-2 mb-2"> {newWebhook.eventTypes.map (event => (<ClickableBadge key= {event;"
  ...
  testResult.responseBody |'<empty>';'
                      ...
                      {webhook.event_types.map (event => (<SelectItem key={event} value={event}>;
                          Test {event_options.find (e => { return e.value === event)?.label || event}
                        </SelectItem>))}
                    </SelectContent>; }
                  </Select>;
                </CardFooter>;
