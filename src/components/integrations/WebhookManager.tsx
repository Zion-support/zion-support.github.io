...
export function WebhookManager() {const {webhooks,loading,error,testResult,fetchWebhooks,createWebhook,toggleWebhook,deleteWebhook,testWebhook}  = useWebhooks()const [newWebhook, setNewWebhook] = useState({name: ''}
  url: '',selectedEvent: '' as WebhookEventType,eventTypes: [] as WebhookEventType[],secret: '}
});
...
const handleRemoveEvent = (    setNewWebhook({...newWebhook,eventTypes: newWebhook && newWebhook.eventTypes.filter(e => e !== event)) => {
  return $3;}
}
}
;
  const handleCreateWebhook = async () => {if (!newWebhook && newWebhook.name ||;
    ...
    setNewWebhook({name: ''}
  url: '',selectedEvent: '' as WebhookEventType,eventTypes: [],secret: ''}
}
...
fetchWebhooks()}, []),const handleAddEvent = (if (!newWebhook.selectedEvent) return,if (newWebhook.eventTypes.includes(newWebhook.selectedEvent)) {toast.error("This event is already added");
  ...
  return $3;}
    ...
    })},const handleRemoveEvent = (setNewWebhook({...newWebhook,eventTypes: newWebhook.eventTypes.filter(e => e !== event)) => {
  return $3;}
}
};"
