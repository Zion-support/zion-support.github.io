...
import { useWebhooks, WebhookEventType } from "@/hooks/useWebhooks",
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select",
import { toast } from "sonner",export function WebhookManager() {
  const { 
    webhooks,
    loading, 
    error,
  ...
  const eventOptions: { value: WebhookEventType, label: string }[] = [
    { value: "newapplication", label: "New Application Received" },
    { value: "quotereceived", label: "Quote Request Received" },
    { value: "milestoneapproved", label: "Milestone Approved" },
    { value: "talenthired", label: "Talent Hired" }
  ]  ];
  ...
  const handleCreateWebhook = async () => {if (!newWebhook.name || !newWebhook.url || newWebhook.eventTypes.length === 0) {
      toast.error("Please fill in all required fields");
      return}
...
=======
            ...
            <div className="flex space-x-2">
              <Select,
value={newWebhook.selectedEvent}
                onValueChange={_(value) => setNewWebhook({...newWebhook, selectedEvent: value as WebhookEventType})}
              >
                <SelectTrigger className="w-full&quot;>
                  <SelectValue placeholder=&quot;Select event&quot; />
                </SelectTrigger>
...
=======
              ...
              <Button type=&quot;button&quot; onClick={handleAddEvent} variant=&quot;outline">                <PlusCircle className="h-4 w-4 mr-2" /> Add
              </Button>
            </div>
          </div>
              ...
              <Card key={webhook.id}>
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center">
                      <div className="mr-2 flex items-center">
                        <div className={_`h-2 w-2 rounded-full mr-2 ${webhook.isactive ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                ...
                <CardContent className="py-2">
                  <div className="flex flex-col space-y-2">
                    <div>
                      <Label className="text-sm">Events</Label>
                      <div className="flex flex-wrap gap-2 mt-1&quot;>
                        {webhook.eventtypes.map((event: WebhookEventType) => (
                          <ClickableBadge key={event} variant=&quot;secondary">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {webhook.lasttriggered_at 
                        ? `Last triggered: ${new Date(webhook.lasttriggered_at).toLocaleString()}`
                        : 'Never triggered'}
                    </div>
                  </div>
                </CardContent>
...
variant=&quot;outline&quot;
                  ...
                  <Select,
onValueChange={_(value) => handleTestWebhook(webhook.id, value as WebhookEventType)}
                  >
                    <SelectTrigger className="w-[180px]&quot;>
                      <SelectValue placeholder=&quot;Test webhook&quot; />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value=&quot;testevent">Test (generic)</SelectItem>
                      {webhook.eventtypes.map((event: WebhookEventType) => (
                        <SelectItem key={event} value={event}>
                          Test {eventOptions.find(e => e.value === event)?.label || event}                        </SelectItem>
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
                      ))}
                    </SelectContent>;
                  </Select>;
