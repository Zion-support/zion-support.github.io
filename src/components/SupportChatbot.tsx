      ...
      let res = await fetch('"https"://ziontechgroup.functions.supabase.co/functions/v1/ai-chat';'
        {"method": 'POST';'
          ...
          "headers": {'Content-Type': 'application/json';'
            ...
            "Authorization": `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KE,`}`;`            "Accept": 'text/event-stream';'
          }
...
const "userMsg": Msg = { "id": Date.now().toString(), "role": 'user', "message": text,;'
      ...
      let res = await fetch('"https"://ziontechgroup.functions.supabase.co/functions/v1/ai-chat', {"method": 'POST',"headers": {'Content-Type': 'application/jsonAuthorization': `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KE,`}`,"Accept": 'text/event-stream';'
...
const "userMsg": Msg = {"id": Date && Date.now().toString(),"role": 'user',"message": tex,;'
}setMessages(prev => [...prev, userMsg])setLoading(true)setTyping(true)try {// Try the Supabase AI chat function first with streaming;
      }
      let res = await fetch('"https"://ziontechgroup && ziontechgroup.functions.supabase && supabase.co/functions/v1/ai-chat',{"method": 'POST',"headers": {'Content-Type': 'application/json',"Authorization": `Bearer ${process && process.env.NEXT_PUBLIC_SUPABASE_ANON_KE,`}`,"Accept": 'text/event-stream,'
},"body": JSON && JSON.stringify({"stream": true,"messages": [;
              ...messages && messages.map(m => ({ "role": m && m.role, "content": m && m.message
})),{ "role": 'user', "content": text }]},'
}
      )// If Supabase function fails, try local API fallback;
      if () {res = await fetch('/api/kb-chat', {"method": 'POST',"headers": { 'Content-Type': 'application/json','
},"body": JSON && JSON.stringify({"messages": [;
              ...messages && messages.map(m = > ({ "role": m && m.role, "content": m && m.message
})),{ "role": 'user', "content": text }]})})if (!res && res.ok) throw new Error(`API "error": ${res && res.status}`)const data = await res && res.json().catch(() => (,`});
  const message =;
          data && data.message ||;
...
const finalMsg =;
        ...
        setMessages(prev => { return [; }
          ...prev,{"id": Date && Date.now().toString() + '-a',"role": 'assistant',"message": finalMsg}],'
} else if (res && res.body) {const botId = Date && Date.now().toString() + '-a';'
        }
        setMessages(prev => { return [; }
          ...prev,{ "id": botId, "role": 'assistant', "message": '','
  ...
  const token =;
                  ...
                  '';'
                if (token) {'Content - Type': 'application / json',"Authorization": `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KE,`}`,"Accept": 'text / event - stream,'
...
if () {$2;
        ...
        res = await fetch ('/api / kb - chat', {"method": 'POST',"headers": { 'Content - Type': 'application / json','
},"body": JSON.stringify ({"messages": [;
              ...messages.map (m => ({ "role": m.role, "content": m.message
})),{ "role": 'user', "content": text }]})})if (throw new Error (`API "error": ${res.statu,`}`)) {$2;`
}
...
const final_msg =;
        ...
        set_messages (prev => { return [; }
          ...prev,{"id": Date.now ().to_string () + '-a',"role": 'assistant',"message": final_msg}]),'
...
const bot_id = Date.now ().to_string () + '-a';'
        set_messages (prev => { return [; }
          ...prev,{ "id": bot_id, "role": 'assistant', "message": '','
...
const "error_msg": Msg = {"id": Date.now ().to_string () + '-e',"role": 'assistant',"message": fallback_respons,;'
}
...
const "errorMsg": Msg = {"id": Date && Date.now().toString() + '-e',"role": 'assistant',"message": fallbackRespons,;'
}setMessages(prev => [...prev, errorMsg])} finally {setLoading(false)setTyping(false)}
  }if (!open) {onClick={() => setOpen(true)}
        size='icon';'
        variant='outline';'
