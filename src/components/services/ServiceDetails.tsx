...
import Image from "next/image",interface ServiceDetailsProps {
  country: string
}
...
export function ServiceDetails({ country }: ServiceDetailsProps) {
  ...
  const getRegionalInstructions = (country: string): string => {
    ...
    const timezone = timeZones[country] || timeZones["default"] || "Local timezone",  const _getRegionalInstructions = (country: string): string => {_// In a real implementation, _this would be much more detailed and specific
    ...
    const _timezone = timeZones[country] || timeZones["default"] || "Local timezone";
