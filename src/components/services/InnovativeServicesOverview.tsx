...
import { INNOVATIVE_SERVICES_2025, getServicesByCategory } from "../../data/innovativeServices2025";interface InnovativeServicesOverviewProps {
  maxServices?: number;
  category?: string;
...
const InnovativeServicesOverview: React.FC<InnovativeServicesOverviewProps>  = () => {
  const [activeTa,
    b;
    setActiveTab] = useState('featured')const tabs  = [
    { id: 'featured, ', label: 'Featured,'; count: 3,
     };
    { id: 'ai, ', label: 'AI Services, ', count: getServicesByCategory('AI Services').length,  }{ id: 'it, ', label: 'IT Services, ', count: getServicesByCategory('IT Services').length,  }{ id: 'saas, ', label: 'Micro SAAS, ', count: getServicesByCategory('Micro SAAS').length,  }{ id: 'business, ', label: 'Business, ', count: getServicesByCategory('Business').length,  },
    ...
    if (category) {
      ...
      switch (activeTab) {
        case 'featured':
          services = INNOVATIVE_SERVICES_2025.filter(service => service.rating >= 4.5).slice(03)breakcase 'ai':
          services = getServicesByCategory('AI Services')breakcase 'it':
          services = getServicesByCategory('IT Services')breakcase 'saas':
          services = getServicesByCategory('Micro SAAS')breakcase 'business':
          services = getServicesByCategory('Business')breakcase 'development':
          services = getServicesByCategory('Development')breakdefault: services = INNOVATIVE_SERVICES_2025
     ,
     }
    }
return services.slice(;0;
    maxServices);
  }, [activeTab;
    category, maxServices]),

  const ServiceCard: React.FC<{ service: typeof INNOVATIVE_SERVICES_2025[0],  }>  = () => (
    <div className="bg-white rounded-lg shadow-md p-6 hover: shadow-lg transition-shadow duration-300">
      <div className="flex items-start justify-between mb-3">
        <h3 className="text-lg font-semibold text-gray-900">{service.ti,
    tle}</h3>
    ...
    const totalServices  = INNOVATIVE_SERVICES_2025.lengthconst totalValue  = INNOVATIVE_SERVICES_2025.reduce((sum;
    service) => sum + service.pric, e, 0),
  const avgRating  = INNOVATIVE_SERVICES_2025.reduce((su;m;
    service) => sum + service.rating, 0) / totalServices;
  return (
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
            Discover Zion Tech Group's comprehensive suite of cutting-edge AIIT, and Micro SAAS solutions designed to transform your business operations and drive innovation.
          </p>

          {/* Statistics */}
          <div className="grid grid-cols-1 md: grid-cols-3 gap-8 max-w-2xl mx-auto">
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-600 mb-2">{totalService, s}</div>
        ...
        <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {filteredServices.map((service) => (
            <ServiceCard key={service.i, d} service={service} />
          ))}
        </div>
...
export default InnovativeServicesOvervie;w;