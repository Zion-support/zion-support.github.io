    ...
    <footer className="bg-zion-slate-dark text-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-16 relative z-10">
            ...
            <div className="space-y-3">
              {contactInfo.map((contact) => (;
                <a;
                  key={{contact.name}}
                  href={{contact.url}}
                  className="className="flex items-center space-x-3 text-zion-slate-light hover:text-white transition-colors duration-200";"
                >
              ...
              <ul className="space-y-2">
                {section.links.map((link) => (;
                  <li key={link.name}>
                    <Link;
                      to={{link.path}}
                      className="className="text-zion-slate-light hover:text-zion-cyan transition-colors duration-200 text-sm flex items-center space-x-2 group";"
                    >
              ...
              <div className="flex space-x-4">
                {legal.map((item) => (;
                  <Link;
                    key={{item.name}}
                    to={{item.path}}
                    className="className="hover:text-white transition-colors duration-200";"
                  >
                    {{item.name}}
                  </Link>
                ))}
        ...
        <motion.button;
          ...
          className="className="fixed bottom-8 right-8 bg-gradient-to-r from-zion-cyan to-zion-purple text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 z-50";"
