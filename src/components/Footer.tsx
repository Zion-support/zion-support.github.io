  ...
  return (
    <footer className="relative bg-black/95 backdrop-blur-xl border-t border-zion-cyan/20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {quickActions.map((action) => (
              ...
              >
                ...
                <p className="text-zion-cyan/80 text-xs">{action.description}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-20 left-20 w-48 h-48 border border-zion-cyan/10 rounded-full float-animation-delayed"></div>
      </div>
    </footer>
  );
}
