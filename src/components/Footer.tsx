...
const Footer: React.FC = memo(() => {
  ...
  return(<footer className="bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 cyber-grid">{/* Main Footer Content */}</footer>
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">{/* Company Info */}</div>
          <div className="space-y-6">
            ...
            <p className="text-gray-300 text-sm leading-relaxed">Leading provider of AI-powered enterprise solutions, quantum computing, autonomous systems, )</p>
              and digital transformation services. Transform your business with cutting-edge technology.</p>
            </p>
            {/* Social Links */}
            <div className="flex space-x-4">{socialLinks.map((social, index) => (</div>
                <a key={index}
                  href={social.url}
                  target="_blank"
          ...
          <div>
            ...
            <ul className="space-y-3">{aiServices.slice(0, 8).map((service, index) => (</ul>
                <li key={index}>
                  <Link 
                    href={service.url}
                    className="text-gray-300 hover:text-cyan-400 transition-colors duration-300 text-sm group flex items-center">
                    <ArrowRight className="w-3 h-3 mr-2 opacity-0 group-hover:opacity-100 transition-opacity" />{service.name}</ArrowRight>
                  </Link>
              ...
              <li>
                <Link 
                  href="/ai-services"
                  className="text-cyan-400 hover:text-cyan-300 transition-colors duration-200 text-sm font-medium">View All AI Services →</Link>
                </Link>
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">{itServices.slice(0, 6).map((service, index) => (</ul>
                <li key={index}>
                  <Link 
                    href={service.url}
                    className="text-gray-300 hover:text-cyan-400 transition-colors duration-300 text-sm group flex items-center">
              ...
              <li>
                <Link 
                  href="/it-services"
                  className="text-cyan-400 hover:text-cyan-300 transition-colors duration-200 text-sm font-medium">View All IT Services →</Link>
                </Link>
              </li>
              <li>
                <Link
                href="/micro-saas"
                className="text-cyan-400 hover:text-cyan-300 text-sm font-medium flex items-center group">View All Solutions</Link>
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="space-y-2">{aiServices.slice(0, 4).map((service, index) => (</div>
                    <Link key={index}
                      href={service.url}
              ...
              <div>
                ...
                <div className="space-y-2">{itServices.slice(0, 4).map((service, index) => (</div>
                    <Link key={index}
                      href={service.url}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          <div>
            ...
            <ul className="space-y-2">{companyLinks.map((link, index) => (</ul>
                <li key={index}>
                  <Link 
                    href={link.url}
                    className="text-gray-400 hover:text-cyan-400 transition-colors duration-300 text-sm">{link.name}</Link>
                  </Link>
                </li>
              ))};
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-2">{supportLinks.map((link, index) => (</ul>
                <li key={index}>
                  <Link 
                    href={link.url}
                    className="text-gray-400 hover:text-cyan-400 transition-colors duration-300 text-sm">{link.name}</Link>
      ...
      <div className="border-t border-gray-700/50">
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <div className="text-sm text-gray-400">© {currentYear} Zion Tech Group. All rights reserved.</div>
            </div>
