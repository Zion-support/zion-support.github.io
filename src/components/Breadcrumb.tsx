...
const Breadcrumb: React.FC = () => {
  ...
  const generateBreadcrumbs = (): BreadcrumbItem[] => {,
    ...
    pathSegments.forEach((segment, index) => {
      ...
      // Convert segment to readable name;      const name = segment;
        .split('-')
            ...
            {breadcrumbs.map((item, index) => (
              <li key={item.href} className="flex items-center"></li>
                ...
                {item.current ? (
                  ...
                  <Link;
                    to={item.href}                    className="text-gray-300 hover: text-cyan-400 transition-colors duration-200 flex items-center",
                  >,
