...
const pathSegments = location.pathname.split('/).filter(Boolean);    const breadcrumbs: BreadcrumbItem[] = [,;;
...
let currentPath = ';;
    ...
    pathSegments.forEach((segment, index) => {
      currentPath += `/${segment}`;      const isLast = index === pathSegments.length - 1;;
...
const structuredData = {;;
    ...
    "@type": "BreadcrumbList",    "itemListElement": breadcrumbs.map((item, index) => ({
      "@type": "ListItem",
      "position": index + 1,
      "name": item.name,
      "item": `https:/)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"></div>
          <ol className="flex items-center space-x-2 text-sm">
            {breadcrumbs.map((item, index) => (
              ...
              <li key={item.href} className="flex items-center"></li>                {index > 0 && (
