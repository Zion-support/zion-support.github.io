...
'
  ...
  { opacity: 1, y: 0 '
,''"
}} className='text-center mb-8'></motion>'"'"
        <h1 className='text-4xl font-bold text-zion-blue-dark mb-4'></h1>"'
...
,''
        ...
        <h2 className='text-2xl font-bold text-zion-blue-dark mb-6 flex items-center gap-2'></h2>'"'"
          <Brain className='w-6 h-6 text-zion-cyan'/" ></Brain>"'"
          Tell Us About Your Business"'
        </h2>'
        ''
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'></div>'
          <div></div>''"
            <label className='block text-sm font-medium text-zion-slate-dark mb-2'></labe>Industry</label>"'"
            <select value={clientProfile.industry} onChange = {}"'""
  (e) =" ></select> setClientProfile(prev => ({ ...prev, industry: e.target.value '
,''
}))} className='w-full p-3 border border-zion-slate-light rounded-md'>'"
              <option value=></optio>Select Industry</option>""
              {industries.map(industry => (<option key={industry} value={industry}" ></optio>{industry}</option>) ) }"
            </select>"'"
          </div>"'
          '
          <div></div>''"
            <label className='block text-sm font-medium text-zion-slate-dark mb-2'></labe>Comp Size</label>"'"
            <select value={clientProfile.compSize} onChange = {}"'""
  (e) =" ></select> setClientProfile(prev => ({ ...prev, compSize: e.target.value '
,''
}))} className='w-full p-3 border border-zion-slate-light rounded-md'>'"
              <option value=></optio>Select Comp Size</option>""
              {compSizes.map(size => (<option key={size} value={size}" ></optio>{size}</option>) ) }"
            </select>"'"
          </div>"'
          '
          <div></div>''"
            <label className='block text-sm font-medium text-zion-slate-dark mb-2'></labe>Budget Range</label>"'"
            <select value={clientProfile.budget} onChange = {}"'""
  (e) =" ></select> setClientProfile(prev => ({ ...prev, budget: e.target.value '
,''
}))} className='w-full p-3 border border-zion-slate-light rounded-md'>'"
              <option value=></optio>Select Budget Range</option>""
              {budgets.map(budget => (<option key={budget} value={budget}" ></optio>{budget}</option>) ) }"
            </select>"'"
          </div>"'
          '
          <div></div>''"
            <label className='block text-sm font-medium text-zion-slate-dark mb-2'></labe>Implementation Timeline</label>"'"
            <select value={clientProfile.timeline} onChange = {}"'""
  (e) =" ></select> setClientProfile(prev => ({ ...prev, timeline: e.target.value '
,''
}))} className='w-full p-3 border border-zion-slate-light rounded-md'>'"
              <option value=></optio>Select Timeline</option>""
              {timelines.map(timeline => (<option key={timeline} value={timeline}" ></optio>{timeline}</option>) ) }"
            </select>"'"
          </div>"'
          '
          <div></div>''"
            <label className='block text-sm font-medium text-zion-slate-dark mb-2'></labe>Technology Maturity Level</label>"'"
            <select value={clientProfile.technologyMaturity} onChange = {}"'""
  (e) =" ></select> setClientProfile(prev => ({ ...prev, technologyMaturity: e.target.value '
,''
}))} className='w-full p-3 border border-zion-slate-light rounded-md'>'"
              <option value=></optio>Select Technology Maturity</option>""
              {technologyMaturityLevels.map(level => (<option key={level} value={level}" ></optio>{level}</option>) ) }"
            </select>"'"
          </div>"'
        </div>'
''
        <div className='mt-6'></div>''
          <label className='block text-sm font-medium text-zion-slate-dark mb-2'></labe>Primary Business Goals</label>''
          <div className='grid grid-cols-2 md:grid-cols-3 gap-2'></div>''
            {primaryGoals.map(goal => (<label key={goal} className='flex items-center gap-2 cursor-pointer'></label>''"
...
'
                ...
                <span className='text-sm text-zion-slate-light'></spa>{goal}</span>"
              </label>))}"'"
          </div>"'
        </div>'
''
        <div className='mt-6'></div>''
          <label className='block text-sm font-medium text-zion-slate-dark mb-2'></labe>Current Challenges</label>''
          <div className='grid grid-cols-2 md:grid-cols-3 gap-2'></div>''
            {challenges.map(challenge => (<label key={challenge} className='flex items-center gap-2 cursor-pointer'></label>''"
...
'
                ...
                <span className='text-sm text-zion-slate-light'></spa>{challenge}</span>"
              </label>))}"'"
          </div>"'
        </div>'
''"
        <div className='mt-8 text-center'></div>'"'"
          <Button className='bg-gradient-to-r from-zion-cyan to-zion-purple hover:from-zion-cyan-dark hover:to-zion-purple-dark text-lg px-8 py-4' onClick={generateRecommendations} disabled={!isFormComplete() || isAnalyzing}" ></Button>'"
            {isAnalyzing ? (<>'"'"
                <div className='animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2'></di></div>"'
  ...
  { opacity: 1, y: 0';','"'"
}} transition={{ delay: 0.2 }}" ></motion>''"
          <h2 className='text-2xl font-bold text-zion-blue-dark mb-6 text-center'></h2>"'"
                ...
                <Card className='hover:shadow-lg transition-shadow'></Card>'
                  <CardHeader></CardHeader>''
                    <div className='flex items-start justify-between'></div>''"
                      <div className='flex-1'></div>'"'"
                        <CardTitle className='text-xl text-zion-blue-dark mb-2'></CardTitle>"'
                          {rec.service.title}'
                        </CardTitle>''"
                        <p className='text-zion-slate-light mb-3'></p>"'"
                          {rec.service.description}"'
                        </p>'
                        ''"
                        <div className='flex flex-wrap gap-2 mb-4'></div>""
                          <Badge className={getPriorityColor(rec.priority)}" ></Badge>
                            Priority: {rec.priority}"
                          </Badge>"'"
                          <Badge className={getComplexityColor (rec.implementationComplexity) }" ></Badge>"'
                            Complexity: {rec.implementationComplexity}'
                          </Badge>''
                          <Badge className='bg-zion-cyan/10 text-zion-cyan'></Badge>
                      ...
                      <div className='text-right ml-4'></div>'"'"
                        <div className='text-2xl font-bold text-zion-cyan mb-1'></div>"'
                          {rec.service.price}'
                        </div>''
                  ...
                  <CardContent></CardContent>''
                    <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'></div>'
                      <div></div>''"
                        <h4 className='font-semibold text-zion-slate-dark mb-3 flex items-center gap-2'></h4>'"'"
                          <Lightbulb className='w-4 h-4 text-zion-purple'/" ></Lightbulb>"'
                          Why This Service Matches'
                        </h4>''
                        <ul className='space-y-2'></ul>''"
                          {rec.reasoning.map((reason, idx) => (<li key={idx} className='text-sm text-zion-slate-light flex items-start gap-2'></li>'"'"
                              <CheckCircle className='w-4 h-4 text-green-500 mt-0.5 flex-shrink-0'/" ></CheckCircle>
                              {reason}
                            </li>) ) }"
                        </ul>"'"
                      </div>"'
                      ...
                      <div></div>''"
                        <h4 className='font-semibold text-zion-slate-dark mb-3 flex items-center gap-2'></h4>'"'"
                          <BarChart3 className='w-4 h-4 text-zion-cyan'/" ></BarChart3>"'
                          Implementation Details'
                        </h4>''
                        <div className='space-y-3'></div>''
                          <div className='flex justify-between'></div>''
                            <span className='text-sm text-zion-slate-light'></spa>Expected ROI:</span>''
                            <span className='text-sm font-medium text-zion-slate-dark'></spa>{rec.expectedROI}</span>'
                          </div>''
                          <div className='flex justify-between'></div>''
                            <span className='text-sm text-zion-slate-light'></spa>Time to Value:</span>''
                            <span className='text-sm font-medium text-zion-slate-dark'></spa>{rec.timeToValue}</span>'
                          </div>''
                          <div className='flex justify-between'></div>''
                            <span className='text-sm text-zion-slate-light'></spa>Category:</span>''
...
"'"
  () =" ></Button> window.open(rec.service.link,_blank')",'
}>'"
                            Learn More'"'"
...
,''
            ...
            <div className='bg-gradient-to-r from-zion-purple via-zion-blue-light to-zion-cyan rounded-lg p-8 text-white'></div>''
              <h3 className='text-3xl font-bold mb-4'></h>Ready to Get Started?</h3>''"
...
"'"
  () =" ></Button> window.open('mailto:kleber@ziontechgroup.com?subject=Service Recommendation Consultation',_blank')"''
',''"
}>'"'"
...
"'"
  () =" ></Button> window.open('tel:+13024640950',_blank')"''
',''"
}>'"'"
