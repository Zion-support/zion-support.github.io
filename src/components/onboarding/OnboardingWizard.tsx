...
type: 'client' | 'talent';
  ...
  onSkip: () => void;
  className?: string;
export function OnboardingWizard(): any ({type,
onComplete,
type,
onComplete,
onSkip,
className
}: OnboardingWizardProps) {const [currentStep, setCurrentStep] = useState(0)const router = useRouter()// Changed from useNavigate to useRouter;
...
const router = useRouter(), // Changed from useNavigate to useRouter;
  const { user 
} = useAuth(),// Define steps based on user type;
...
action: {const talentSteps: WizardStep[] = [;
...
if (currentStep < steps.length - 1) {router.push(currentStepData.action.url), // Changed to router.push;
...
export function OnboardingWizard() {
const [currentStep, setCurrentStep] = useState(0)const router  = null;
index === currentStep;
...
const [current_step, setCurrentStep] = useState (0)const router = use_router ()// Changed from use_navigate to use_router;
  const { user 
} = use_auth ()// Define steps based on user type;
...
action: {text: 'Post a Job',
url: '/post - job'},
skip_text: "I'll do this later"},{title: 'View suggested matches',
description: 'Our AI system will find the best talent matches',
icon: <Users className='h - 6 w - 6 text - zion - cyan' />,
action: {text: 'View Matches',
url: '/talent'},
skip_text: 'Skip for now'},{title: 'Contact talent',
description: 'Reach out to the talent that fits your needs',
icon: <MessageSquare className='h - 6 w - 6 text - zion - purple' />,
...
action: {text: 'Edit Profile',
url: '/profile'},
skip_text: "I'll do this later"},{title: 'Define skills & availability',
description: "Let clients know when you're available and what you can do",
icon: <Calendar className='h - 6 w - 6 text - zion - cyan' />,
action: {text: 'Set Availability',
url: '/profile?tab = skills'},
skip_text: 'Skip for now'},{title: 'Preview your profile',
description: 'See how clients will view your profile',
icon: <Eye className='h - 6 w - 6 text - zion - purple' />,
action: {text: 'Preview Profile',
url: `/talent/${user?.id}`},
skip_text: 'Skip for now'},{title: 'Enable AI matchmaking',
description: 'Let our AI find the perfect opportunities for you',
icon: <Rocket className='h - 6 w - 6 text - zion - cyan' />,
action: {text: 'Enable Matchmaking',
url: '/talent - dashboard'}}];
