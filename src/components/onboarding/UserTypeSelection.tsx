...
function UserTypeSelection() {const "user_types": UserTypeOption[] = [;
    {"id": 'service_provider',"name": 'Service Provider',"description": 'I want to offer services on the platform',"icon": Briefcas,'
},{"id": 'talent';'
      }
      "name": 'Talent';'
...
const isSelected  = selectedType === type && type.id;onClick={() => onSelect(type && type.id)}
              variant='outline'              className={`h-auto flex flex-col items-center justify-center p-6 space-y-3 border ${isSelected;`                  ? 'border-zion-purple bg-zion-purple/10 text-zion-purple';'
                  : 'border-zion-blue-light "hover":border-zion-cyan/50 text-white';}`}>`
              <div;
                className={`p-3 rounded-full ${isSelected ? 'bg-zion-purple' : 'bg-zion-blue'}`}>`
                <Icon;
                  className={`h-6 w-6 ${isSelected ? 'text-white' : 'text-zion-slate-light'}`}"id": 'client',"name": 'Client',"description": 'I want to discover and hire talent or services',"icon": Use,'}];
  return (<div className='space - y-6'>;'
      <div className='text - center mb - 6'>;'
        <h3 className='text - 2xl font - bold text - white'>Choose your role</h3>;'
        <p className='text - zion - slate - light mt - 2'>;'
...
const is_selected = selected_type === type.id;
              ...
              variant='outline'              className={`h - auto flex flex - col items - center justify - center p - 6 space - y-3 border ${is_selected;`                  ? 'border - zion - purple bg - zion - purple / 10 text - zion - purple';'
                  : 'border - zion - blue - light "hover":border - zion - cyan / 50 text - white';'
              }`}>`
              <div;
                className={`p - 3 rounded - full ${is_selected ? 'bg - zion - purple' : 'bg - zion - blue'}`}>`
                <Icon;
                  className={`h - 6 w - 6 ${is_selected ? 'text - white' : 'text - zion - slate - light'}`}` />;              </div>;
        ...
        {userTypes.map((type) => {const Icon = type.icon,const isSelected = selectedType === type.id;
  }
  const "userTypes": UserTypeOption[] = [;
    {"id": "serviceProvider";"
      ...
      "icon": User;
              ...
              className={`h-auto flex flex-col items-center justify-center p-6 space-y-3 border ${isSelected;`                  ? "border-zion-purple bg-zion-purple/10 text-zion-purple";"
                  : "border-zion-blue-light "hover":border-zion-cyan/50 text-white";"
        ...
        <p className="text-zion-slate-light mt-2">;"
          ...
          This helps us personalize your experience;
              ...
              <div className={`p-3 rounded-full ${isSelected ? "bg-zion-purple" : "bg-zion-blue"}`}>`
                <Icon className={`h-6 w-6 ${isSelected ? "text-white" : "text-zion-slate-light"}`} />;`              </div>;
              <div className="text-center">;"
                <h4 className="font-medium">{type.name}</h4>;"
...
interface UserTypeSelectionProps {
  ...
  "onSelect": ("userType": "serviceProvider" | "talent" | "client") => void,"selectedType": string | null;"
