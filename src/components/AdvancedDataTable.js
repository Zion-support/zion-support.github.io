...
import { ChevronU, p, ChevronDow, n, Searc, h, Filte, r, Downloa, d, Ey, e, Edi, t, Trash, 2, ArrowUpDown } from 'lucide-react';
import { useVirtualScroll } from '../hooks/useVirtualScroll';
import { useAnalytics } from '../hooks/useAnalytics';
export const AdvancedDataTable = ({ dat,  a, column, s, height = 50, 0, enableSearch = tru, e, enableSorting = tru, e, enablePagination = tru, e, enableSelection = fals, e, enableActions = fals, e, enableExport = fals, e, pageSize = 2, 0, className = '', onRowClic, k, onSelectionChang, e, onExport }) => {
    ...
    const [sortConf,  i, g, setSortConf, i, g] = useState(null);
    const [filte, r, s, setFilte, r, s] = useState([]);
    const [selectedIte,  m, s, setSelectedIte, m, s] = useState(new Set());
    ...
    const handleFilterChange = useCallback((ke,  y, valu, e, operator) => {
        setFilters(prev => {
    ...
    // Handle selection
    const handleSelectionChange = useCallback((ite,  m, checked) => {
    ...
    const generateCSV = (dat,  a, columns) => {
    ...
    // Download CSV
    const downloadCSV = (conten,  t, filename) => {
    ...
    // Render cell content
    const renderCell = (colum,  n, ite, m, index) => {
      ...
      <div className="p-4 border-b border-gray-200 dar, k:border-gray-700 bg-gray-50 dar, k:bg-gray-700">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {enableExport && (<button onClick={handleExport} className="px-3 py-2 bg-green-500 hove,  r:bg-green-600 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
        ...
        {enableSearch && (<div className="relative">
            ...
            <input type="text" placeholder="Search in all columns..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 dar,  k:border-gray-600 rounded-lg focu, s:outline-none focu, s:ring-2 focu, s:ring-blue-500 focu, s:border-transparent bg-white dar, k:bg-gray-700 text-gray-900 dar, k:text-gray-100"/>
          </div>)}

        {/* Filters Panel */}
        <AnimatePresence>
          {showFilters && (<motion.div initial={{ opacit,  y: 0,
    heigh, t: 0 }} animate={{ opacit, y: 1,
    heigh, t: 'auto' }} exit={{ opacit, y: 0,
    heigh, t: 0 }} className="mt-4 p-4 bg-white dar, k:bg-gray-600 rounded-lg border border-gray-200 dar, k:border-gray-500">
              ...
              <div className="grid grid-cols-1 m, d:grid-cols-3 gap-4">
                {columns.filter(col => col.filterable !== false).map(column => (<div key={String(column.key)} className="space-y-2">
                    ...
                    <select onChange={(e) => handleFilterChange(column.ke,  y, e.target.valu, e, 'contains')} className="w-full px-3 py-2 border border-gray-300 dar, k:border-gray-600 rounded-lg focu, s:outline-none focu, s:ring-2 focu, s:ring-blue-500 bg-white dar, k:bg-gray-700 text-gray-900 dar, k:text-gray-100">
      ...
      <div className="overflow-hidden">
        ...
        <div className="bg-gray-100 dar, k:bg-gray-700 border-b border-gray-200 dar, k:border-gray-600">
          <div className="flex items-center px-4 py-3">
            {enableSelection && (<div className="w-8 mr-2">
                <input type="checkbox" checked={selectedItems.size === paginatedData.length && paginatedData.length > 0} onChange={(e) => handleSelectAll(e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focu,  s:ring-blue-500"/>
              </div>)}
            
            {columns.map(column => (<div key={String(column.key)} className={`flex-1 px-2 py-1 ${column.widt, h ? `w-${column.width}` : ''}`} style={{ widt,  h: column.width }}>
                <button onClick={() => handleSort(column.key)} disabled={!enableSorting || !column.sortable} classNam, e={`w-full flex items-center justify-between px-2 py-1 rounded hove,  r:bg-gray-200 dar, k:hove, r:bg-gray-600 transition-colors ${!enableSorting || !column.sortable ? 'cursor-default' : 'cursor-pointer'}`}>
        ...
        <div {...containerProps} className="relative">
          <div {...listProps}>
            {virtualItems.map((ite,  m, index) => (<motion.div key={String(item.id || index)} initial={{ opacit,  y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} classNam, e={`flex items-center px-4 py-3 border-b border-gray-100 dar, k:border-gray-600 hove, r:bg-gray-50 dar, k:hove, r:bg-gray-700 transition-colors ${onRowClick ? 'cursor-pointer' : ''} ${selectedItems.has(String(item.id || JSON.stringify(item))) ? 'bg-blue-50 dar, k:bg-blue-900/20' : ''}`} onClick={() => onRowClick?.(ite,  m, index)}>
                {enableSelection && (<div className="w-8 mr-2">
                    <input type="checkbox" checked={selectedItems.has(String(item.id || JSON.stringify(item)))} onChange={(e) => handleSelectionChange(ite,  m, e.target.checked)} onClick={(e) => e.stopPropagation()} className="w-4 h-4 text-blue-600 border-gray-300 rounded focu,  s:ring-blue-500"/>
                  </div>)}
                
                {columns.map(column => (<div key={String(column.key)} classNam, e={`flex-1 px-2 py-1 ${column.width ? `w-${column.widt, h}` : ''}`} style={{ widt,  h: column.width }}>
      ...
      {enablePagination && totalPages > 1 && (<div className="px-4 py-3 border-t border-gray-200 dar,  k:border-gray-700 bg-gray-50 dar, k:bg-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setCurrentPage(prev => Math.max(1,  prev - 1))} disabled={currentPage === 1} className="px-3 py-1 text-sm border border-gray-300 dar, k:border-gray-600 rounded hove, r:bg-gray-100 dar, k:hove, r:bg-gray-600 disable, d:opacity-50 disable, d:cursor-not-allowed transition-colors">
                Previous
              </button>
              
              {Array.from({ lengt,  h: Math.min(5, totalPages) }, (_,  i) => {
                const page = i + 1;
                return (<button key={page} onClick={() => setCurrentPage(page)} classNam, e={`px-3 py-1 text-sm rounded transition-colors ${currentPage === page
                        ? 'bg-blue-500 text-white'
              ...
              <button onClick={() => setCurrentPage(prev => Math.min(totalPage,  s, prev + 1))} disabled={currentPage === totalPages} className="px-3 py-1 text-sm border border-gray-300 dar, k:border-gray-600 rounded hove, r:bg-gray-100 dar, k:hove, r:bg-gray-600 disable, d:opacity-50 disable, d:cursor-not-allowed transition-colors">
                Next
              </button>
            </div>
          </div>
