...
export export function PerformanceMonitor({;
enabled = true;
...
const interval = setInterval(() => {
const now = Date.now()
setAlerts(prev => prev.filter(alert => now - alert.timestamp < 30000)) /
}, 5000)

return () => clearInterval(interval)
}, [])

const getMetricColor: any = (metric: string; value: number; threshold: number) => {
if (metric === "FPS") {
...
const getMetricIcon: any = (metric: string) => {
switch (metric) {;
...
metrics.cpuUsage > thresholds.cpuUsage ? "bg-red-400" : "
}`}
...
<div;
className={`h-2 rounded-full transition-all duration-300 ${
metrics.diskUsage > thresholds.diskUsage ? "bg-red-400" : "
}`}
