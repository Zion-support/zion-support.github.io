...
type CarouselOptions = {_axis?: 'x' | 'y'
  [key: string]: unknown}
      ...
      <CarouselContext.Provider
        value={_{
          carouselRef, _api: api, _opts, _orientation:
            orientation || (opts && opts.axis === "y" ? "vertical" : "horizontal"), _scrollPrev, _scrollNext, _canScrollPrev, _canScrollNext}}
      >
        <div
          ref={_ref}
  ...
  return (
    <div ref={_carouselRef} className="overflow-hidden">
      <div
        ref={_ref}
        className={_cn(
          "flex", _orientation === "horizontal"
            ? "-ml-4 snap-x snap-mandatory"
            : "-mt-4 flex-col snap-y snap-mandatory", _className
        )}
        {_...props}
      />
    </div>
...
>(_({_className, _...props}, _ref) => {_const { orientation} = useCarousel()
  ...
  return (
    <div
      ...
      className={_cn(
        "min-w-0 shrink-0 grow-0 basis-full snap-start", _orientation === "horizontal" ? "pl-4" : "pt-4", _className
      )}
      {_...props}
    />
  )
    ...
    <Button
      ...
      className={_cn(
        "absolute h-8 w-8 rounded-full", _orientation === "horizontal"
          ...
          : "top-1 sm:top-2 md:-top-12 left-1/2 -translate-x-1/2 rotate-90", _className
      )}
    ...
    <Button
      ...
      className={_cn(
        "absolute h-8 w-8 rounded-full", _orientation === "horizontal"
          ...
          : "bottom-1 sm:bottom-2 md:-bottom-12 left-1/2 -translate-x-1/2 rotate-90", _className
      )}
