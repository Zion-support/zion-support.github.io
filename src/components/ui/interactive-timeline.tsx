...
const getStatusIcon: any = (status: TimelineEvent["status"]) => {
switch (status) {
case "completed":;
...
// Get priority color;
const getPriorityColor: any = (priority: TimelineEvent["priority"]) => {
switch (priority) {
case "critical":;
...
=======
  // Handle event selection;
  const handleEventClick = useCallback((event: TimelineEvent) : any => {
    setSelectedEvent(event),
    onEventClick?.(event);
  }, [onEventClick]);

  // Handle status change;
  const handleStatusChange = useCallback((eventId: string, newStatus: TimelineEvent["status"]) : any => {
    onStatusChange?.(eventId, newStatus);
  }, [onStatusChange]);

  // Get status icon and color;
  const getStatusIcon = (status: TimelineEvent["status"]) : any => {
    switch (status) {
      case "completed":
        return { icon: CheckCircle, color: "text-green-400", bgColor: "bg-green-400/20" },
    case "in-progress":
  ...
  // Get priority color;
  const getPriorityColor = (priority: TimelineEvent["priority"]) : any => {
    switch (priority) {
      case "critical":
        return "border-red-500/50 bg-red-500/10",
    case "high":
    ...
    <div className={`bg-zion-blue-dark/60 backdrop-blur-sm border border-zion-blue-light/30 rounded-xl p-6 ${className}`} ref={timelineRef}>
      ...
      {viewMode === "timeline" && (
        <div className="relative">
          ...
          {/* Events */}
          <div className="space-y-6">
            {filteredEvents.map((event, index) : any => {
                                        const statusInfo = getStatusIcon(event.status);
                          const CategoryIcon = getCategoryIcon(event.category);
                          const isCurrent = index === currentEventIndex;
