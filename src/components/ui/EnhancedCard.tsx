...
export function EnhancedCard({ children, className, variant = 'default', hover = true, onClick, as: Component = 'div', href }) {
    const _baseClasses = 'rounded-xl transition-all duration-300';
    ...
    const _hoverClasses = hover ? 'hover:shadow-xl hover:shadow-zion-purple/20 hover:transform hover:scale-[1.02]' : '';
    const _interactiveClasses = onClick || href ? 'cursor-pointer' : '';
    const _classes = cn(baseClasses, variantClasses[variant], hoverClasses, interactiveClasses, className);
    if (href) {
