...
export function Select({ children, className = '', value, onValueChange }) {
    return (
        <select className={`w-full px-3 py-2 rounded-md border border-zion-blue-light/30 bg-zion-blue-dark/50 text-white ${className}`} value={value} onChange={onValueChange}>
            {children}
        </select>
    );
}
