...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      <Loader2 className={cn('animate-spin', sizeClasses[size], variantClasses[variant])} />
      {showText && <span className=&quot;text-sm text-muted-foreground&quot;>{text}</span>}    </div>
  )
},
...
key={step.id}
            className={cn(
              'flex items-center gap-3 p-2 rounded-md', index === activeStep ? 'bg-primary/10' : 'opacity-50'
            )}
            initial={_{ opacity: 0, x: -20}}
            animate={_{ opacity: index <= activeStep ? 1 : 0.5, x: 0}}
          ...
          >
            ...
            <span className=&quot;text-sm font-medium&quot;>{step.label}</span>          </motion.div>
        ))}
      </div>
    </div>
  )
...
key={i}
            className={cn(
              baseClasses, variantClasses.text, animationClasses[animation], i === lines - 1 ? 'w-3/4' : 'w-full', className
...
export const ErrorState: React.FC<ErrorStateProps> = ({
  ...
  const getErrorConfig = () => {switch (variant) {
      case 'network':
        return {
          icon: isOnline ? Wifi : WifiOff, title: title || (isOnline ? 'Connection Error' : 'No Internet Connection'), description: description || (isOnline 
            ? 'Unable to connect to our servers. Please check your connection.'
            : 'You appear to be offline. Please check your internet connection.'
          ),
          color: 'text-orange-500'
  ...
  return (
    <Card className={cn('border-destructive/20', className)}>
      <CardContent className=&quot;p-8 text-center&quot;>        <motion.div,
initial={_{ scale: 0.8, opacity: 0}}
          animate={_{ scale: 1, opacity: 1}}
        ...
        >
          ...
          {error && process.env.NODEENV === 'development' && (
            <details className=&quot;mb-4 text-left&quot;>
              <summary className=&quot;text-sm text-muted-foreground cursor-pointer&quot;>                Error Details (Development)
              </summary>
              <pre className=&quot;mt-2 p-2 bg-muted rounded text-xs overflow-auto&quot;>
                {typeof error === 'string' ? error : error.message}
              </pre>
...
=======
            ...
            {action && (
              <Button onClick={action.onClick} variant={canRetry ? &quot;outline&quot; : &quot;default&quot}>
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
                {action.label}
              </Button>;
            )}
...
export const LoadingGrid: React.FC<LoadingGridProps> = (_{count = 8, columns = 4, variant = 'card', className}) => {const gridClasses = {
    card: `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-${columns} gap-6`,
    list: 'space-y-4',
    table: 'space-y-2'
  },
          ...
          <CardContent className={itemClasses[variant]}>
            ...
            {variant === 'list' && (
              <div className=&quot;flex gap-4&quot;>
                ...
                <div className=&quot;flex-1 space-y-2&quot;>
                  <Skeleton variant=&quot;text&quot; className=&quot;h-5 w-1/2&quot; />
                  <Skeleton variant=&quot;text&quot; lines={2} />                </div>
              </div>
            )}
            ...
            {variant === 'table' && (
              <div className=&quot;flex items-center gap-4&quot;>
                ...
                <Skeleton variant=&quot;text&quot; className=&quot;h-4 w-1/3&quot; />
                <Skeleton variant=&quot;text&quot; className=&quot;h-4 w-1/6&quot; />
                <Skeleton variant=&quot;text&quot; className=&quot;h-4 w-1/4&quot; />              </div>
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
            )}
          </CardContent>;
        </Card>;
...
export const PerformanceIndicator: React.FC<PerformanceIndicatorProps> = ({
  ...
  if (isLoading) {
    return (
      <Badge variant=&quot;outline&quot; className={cn('text-xs', className)}>
        <LoadingSpinner size=&quot;sm&quot; />
        <span className=&quot;ml-1&quot;>Loading...</span>      </Badge>
    )
  }
