          ...
          <div className="flex flex-col sm:flex-row gap-2 justify-center">;
            {canRetry && (;
              <Button onClick={onRetry} variant="default">;
                <RefreshCw className="h-4 w-4 mr-2" />;                Try Again {retryCount > 0 && `(${retryCount}/${maxRetries})`}
              </Button>;
            )}
            ;
            {action && (;
              <Button onClick={action.onClick} variant={canRetry ? "outline" :"default"}>;                {action.label}
              </Button>;
            )}
            ;
            {secondaryAction && (;
