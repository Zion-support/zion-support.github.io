...
export function LoadingSpinner({ size = 'md', className, text }) {
    ...
    const textSizes = {
        ...
        md: 'text-base';
        lg: 'text-lg'};
    return (_jsxs("div", { className: cn('flex flex-col items-center justify-center', className);
 children: [_jsx(motion.div, { className: cn('border-2 border-zion-blue-light border-t-zion-cyan rounded-full', sizeClasses[size]);
 animate: { rotate: 360 };
 transition: {
                    duration: 1;
                    repeat: Infinity;
 ...
 text && (_jsx(motion.p, { className: cn('mt-3 text-zion-slate-light', textSizes[size]);
 initial: { opacity: 0 };
 animate: { opacity: 1 };
...
export function LoadingDots({ className }) {
    return (_jsx("div", { className: cn('flex space-x-2', className);
 children: [0, 1, 2].map((index) => (_jsx(motion.div, { className: "w-2 h-2 bg-zion-cyan rounded-full", animate: {
                scale: [1, 1.5, 1];
                opacity: [0.5, 1, 0.5]
            };
 transition: {
...
export function LoadingPulse({ className }) {
    return (_jsx(motion.div, { className: cn('w-4 h-4 bg-zion-purple rounded-full', className);
 animate: {
            scale: [1, 1.2, 1];
            opacity: [0.5, 1, 0.5]
        };
...
export function LoadingSpinner({ size = 'md', color = 'primary', customColor, className, text = 'Loading...', showText = false }) {
    ...
    return (<div className={cn('flex flex-col items-center justify-center', className)}>
      <motion.div className={cn('border-2 border-t-transparent rounded-full animate-spin', sizeClasses[size], borderColor)} animate={{ rotate: 360 }} transition={{
            duration: 1;
            repeat: Infinity;
            ease: "linear"}} role="status" aria-label="Loading"/>
