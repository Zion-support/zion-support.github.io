...
src: string;
  ...
  height?: number;
const [isLoading,
setIsLoading] = useState(true)const [hasError,
setHasError] = useState(false)const [isInView,
setIsInView] = useState(!lazy |priority)const [currentSrc,
setCurrentSrc] = useState(src)const [retries,
setRetries] = useState(0)const [loadProgress,
setLoadProgress] = useState(0)const imgRef = useRef < HTMLImageElement>(null)const observerRef = useRef < IntersectionObserver>()const [metrics,
setMetrics] = useState < ImageMetrics | null>(null)const loadStartTime = useRef < number>(0)// Intersection Observer for lazy loading;
  class_name?: string;
...
}, [src])// Monitor image performance;
  useEffect((,)  => {
if (typeof window !== 'undefined' && 'PerformanceObserver' in window) {const observer = new PerformanceObserver(list => {const entries = list.getEntries()entries.forEach(entry => {          if (entry.name === src && entry.entryType === 'resource') {const resourceEntry = entry as PerformanceResourceTiming;
            const fileSize = null;
...
if ( {) {$2;
            ...
            set_metrics ({load_time,
file_size,
format: src.includes ('.webp')? 'webp';
                : src.includes ('.avif')? 'avif';
                  : 'other',
was_optimized: src.includes ('/_next / image')})// Log slow or large images;
            // Check condition;
if ( {) {$2;
}
              log_warn ('Slow image loading:', {src,
load_time: `${load_time.to_fixed (2)}
ms`,
size: `${(file_size / 1024).to_fixed (2)}
KB`})}
            // Check condition;
if ( {) {$2;
}
              log_warn ('Large image detected:', {src,
size: `${(file_size / 1024).to_fixed (2)}
KB`,
load_time: `${load_time.to_fixed (2)}
ms`})}
          }
        })})observer.observe({ entryTypes: ['resource'] })return () => observer.disconnect()}
return () => {} // Return empty cleanup function for the else case;
  ...
  const handleError = ()  => {
if (retries < retryCount) {setRetries(prev => prev + 1)// Retry with a slight delay;
      setTimeout(() => {setCurrentSrc (src + `?retry=${retries + 1
...
const interval = setInterval((,) => {setLoadProgress(prev => {if (prev >= 90) {clearInterval(interval)return prev;
...
observerRef.current = new IntersectionObserver((entries) => {const [entry] = entries,
...
loadStartTime.current = window.window.window.performance.now()
  ...
  useEffect(()  => {
if (typeof window !== 'undefined' && 'PerformanceObserver' in window) {const observer = new PerformanceObserver((list) => {const entries = list.getEntries(),
entries.forEach((entry) => {if (entry.name === src && entry.entryType === 'resource') {const resourceEntry = entry as PerformanceResourceTiming,
...
if (retries < retryCount) {setRetries(prev => prev + 1),// Retry with a slight delay;
      setTimeout(() => {setCurrentSrc(src + `?retry=${retries + 1
}`)}, 1000 * (retries + 1))} else if (fallbackSrc && currentSrc !== fallbackSrc) {setCurrentSrc(fallbackSrc),
setRetries(0)} else {setIsLoading(false),
setHasError(true),
onError?.()}
  },// Simulate loading progress for demo purposes;
  useEffect(()  => {
...
return () => clearInterval(interval)}, [isLoading, showLoadingProgress]),// Generate placeholder based on type;
  const generatePlaceholder = ()  => {
if (placeholder === 'none') return null,
const placeholderClassName = cn('absolute inset-0 flex items-center justify-center',
...
setLoadProgress (prev => {// Check condition;
if ( {) {$2;
...
placeholder === 'shimmer' &&;
        'bg - gradient - to - r from - gray - 200 via - gray - 300 to - gray - 200 animate - pulse',
placeholder === 'blur' && 'backdrop - blur - sm bg - gray - 200 / 50',
placeholder === 'color' && 'bg - gray - 200')// Check condition;
if ( {) {$2;
}
              />)}
...
px` : undefined}
  ...
  const containerStyle: React.CSSProperties = {aspectRatio: aspectRatio || (width && height ? `${width}/${height}` : undefined),
width: width ? `${width}
...
px` : undefined}>;
      <AnimatePresence>;
        {/* Placeholder */}{(isLoading |!isInView) && !hasError && (<motion.div;
            initial={{ opacity: 1 }}
            exit={{ opacity: 0 }}
transition={{ duration: 0.3 }}{(isLoading || !isInView) && !hasError && (<motion&& motion.div;
            initial={{ opacity: 1 }}
            exit={{ opacity: 0 }}
...
className="absolute inset-0 flex flex-col items-center justify-center bg-gray-100 text-gray-400";
        ...
        {/* Actual image */}<motion.img;
            ...
            onLoad={handleLoad}
            onError={handleError}
            className={cn('w-full h-full transition-opacity duration-300',`object-${objectFit}`,
...
export const OptimizedAvatar: React.FC < OptimizedAvatarProps> = ({src,
alt,
size = 'md',
fallback,
className})  => {
...
className="rounded-full";
        ...
        {isInView && !has_error && (<motion.img;
            ...
            className={cn ('w - full h - full transition - opacity duration - 300',`object-${object_fit}`,
...
class_name,
onImageClick})  => {
const [loaded_count, setLoadedCount] = useState (0)const handleImageLoad = () =>: any {setLoadedCount (prev => prev + 1)
}
