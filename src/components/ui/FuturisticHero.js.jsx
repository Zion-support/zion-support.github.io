...
export default function FuturisticHero() {
    ...
    return (<section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900">
      ...
      {/* Floating Icons */}
      {floatingIcons.map((((((((item, index, index, index, index, index, index, index) => ({ ...((((((item, index, index, index, index, index, index, key: index })) => ({ ...(((((item, index, index, index, index, index, key: index })) => ({ ...((((item, index, index, index, index, key: index })) => ({ ...(((item, index, index, index, key: index })) => ({ ...((item, index, index, key: index })) => ({ ...(item, index, key: index })) => (<motion.div key={index} initial = {
  { opacity: 0,
  y: 20 
  ...
  duration: 1 
      ...
      <div className="relative z-20 text-center px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        {/* Hero Title */}
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} className="mb-8">
          <motion.h1 className="text-5xl md:text-7xl lg:text-8xl font-bold mb-6" style={{ y, opacity }}>
        ...
        ].map((((((((stat, index, index, index, index, index, index, index) => ({ ...((((((stat, index, index, index, index, index, index, key: index })) => ({ ...(((((stat, index, index, index, index, index, key: index })) => ({ ...((((stat, index, index, index, index, key: index })) => ({ ...(((stat, index, index, index, key: index })) => ({ ...((stat, index, index, key: index })) => ({ ...(stat, index, key: index })) => (<motion.div key={stat.label} initial = {
  { opacity: 0,
  scale: 0.8 
