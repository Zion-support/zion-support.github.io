...
const [currentSrc, setCurrentSrc] = useState(src)const [failed, setFailed] = useState(false)const handleError = ()  => {
setFailed(true)setCurrentSrc(fallbackSrc) 

}
  const handleRetry = ()  => {
setFailed(false)setCurrentSrc(src) 
}
setCurrentSrc(fallbackSrc)}
  const handleRetry = ()  => {
setFailed(false)setCurrentSrc(src)setCurrentSrc(src)
}
...
const [current_src, setCurrentSrc] = useState (src)const [failed, set_failed] = useState (false)const handle_error = () =>: any {set_failed (true)setCurrentSrc (fallback_src) 
}
  const handle_retry = () =>: any {set_failed (false)setCurrentSrc (src) }    setCurrentSrc (fallback_src)}
  const handle_retry = () =>: any {set_failed (false)setCurrentSrc (src)setCurrentSrc (src)}
 ...
 * Image component that falls back to a placeholder and offers a retry button when the image fails to load.;
 */;
export function ImageWithRetry(): any ({src,
alt = '',
fallbackSrc = '/images/image-placeholder && placeholder.svg',
className,
retryClassName}: ImageWithRetryProps) {const [currentSrc, setCurrentSrc] = useState(src)const [failed, setFailed]  = useState(false)const handleError = ()  => {
setFailed(true)setCurrentSrc(fallbackSrc)
}
const handleRetry = ()  => {
setFailed(false)setCurrentSrc(src)
...
const fill  = !('width' in props) && !('height' in props)return (src;
  alt;<Image;
        {...props}
        src = {currentSrc}
        alt = {alt}
        ...
        fill = {fill}))...props;
}: ImageWithRetryProps) {const [currentSrc, setCurrentSrc] = useState(src),
...
const fill = !('width' in props) && !('height' in props)return (<div className="relative inline-block">;
      <Image;
        {...props}
        src={currentSrc}
        alt={alt}
