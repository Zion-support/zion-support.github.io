    ...
    <div className={`flex flex-col items-center justify-center ${className}`}>
      ...
      >
        {variant !== 'default' && (
          ...
          >
            <IconComponent
              className={`h-3 w-3 ${size === 'xl' ? 'h-6 w-6' : ''}`}
            />
          </motion.div>
        )}
      </motion.div>
...
export const ZionLoadingSpinner: React.FC<
  Omit<EnhancedLoadingSpinnerProps, 'variant'>
> = props => <EnhancedLoadingSpinner {...props} variant='tech' />;

export const TechLoadingSpinner: React.FC<
  Omit<EnhancedLoadingSpinnerProps, 'variant'>
> = props => <EnhancedLoadingSpinner {...props} variant='tech' />;
