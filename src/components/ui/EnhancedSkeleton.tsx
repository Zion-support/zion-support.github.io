...
export const TableSkeleton: React.FC<{ rows?: number; columns?: number; className?: string }> = ({
  ...
  <div className={cn('space-y-3', className)}>
    ...
    <div className="flex space-x-4">
      {Array.from({ length: anyanycolumns }).map((_, index)    => (
        <Skeleton key={index} variant="rectangular" height={20} width="100%" />
      ))}
    </div>
    {/* Rows */}
    {Array.from({ length: anyanyrows }).map((_, rowIndex)    => (
      <div key={rowIndex} className="flex space-x-4">
        {Array.from({ length: anyanycolumns }).map((_, colIndex)    => (
...
export const ListSkeleton: React.FC<{ items?: number; className?: string }> = ({
  ...
  <div className={cn('space-y-4', className)}>
    {Array.from({ length: anyanyitems }).map((_, index)    => (
...
export const GridSkeleton: React.FC<{ 
  ...
  <div className={cn('grid gap-4', className)} style={{
    gridTemplateColumns: anyany`repeat(${columns}, minmax(0, 1fr))`
  }}>
