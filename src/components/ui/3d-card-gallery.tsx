...
=======
  // Handle card click;
  const handleCardClick = useCallback((item: CardItem) : any => {
    setSelectedItem(item),
    onCardClick?.(item);
  }, [onCardClick]);

  // Handle action click;
  const handleActionClick = useCallback((itemId: string, action: string) : any => {
    onAction?.(itemId, action);
  }, [onAction]);

  // Get status color;
  const getStatusColor = (status: CardItem["status"]) : any => {
    switch (status) {
      case "active":
  ...
  const getComplexityColor = (complexity: CardItem["metadata"]["complexity"]) : any => {
    switch (complexity) {
      case "beginner":
        return "border-green-500/50 bg-green-500/10 text-green-400",
    case "intermediate":
...
<<<<<<< HEAD
// Get status color;
const getStatusColor: any = (status: CardItem["status"]) => {
switch (status) {
case "active":;
return "border-green-500/50 bg-green-500/10 text-green-400";
case "beta":
...
// Get complexity color;
const getComplexityColor: any = (complexity: CardItem["metadata"]["complexity"]) => {
switch (complexity) {
case "beginner":;
return "border-green-500/50 bg-green-500/10 text-green-400";
case "intermediate":
...
function Card3D({
  ...
  const handleMouseMove = useCallback((event: React.MouseEvent<HTMLDivElement>) : any => {
    if (!isHovered) return,
...
=======
  const handleLike = useCallback((e: React.MouseEvent) : any => {
    e.stopPropagation(),
    setIsLiked(!isLiked);
  }, [isLiked]);
