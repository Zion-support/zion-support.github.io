...
const inputRef = useRef<HTMLInputElement>(null);
  ...
  const generateSuggestions = useCallback((searchQuery: string) : any => {
 ...
 []);
  ...
  // Handle search input;
  const handleSearchInput = useCallback((value: string) : any => {
    setQuery(value);
 ...
 [generateSuggestions]);
    ...
    // Perform search;
  const performSearch = useCallback(async (searchQuery: string, searchFilters: SearchFilters) : any => {
    setIsSearching(true);
...
return matchesQuery && matchesCategory && matchesRating && matchesVerified;
    ...
    if (searchQuery.trim() && !searchHistory.includes(searchQuery.trim())) {
      setSearchHistory(prev => [searchQuery.trim();
 ...prev.slice(0, 9)]);
    }
...
onSearch?.(searchQuery; searchFilters);
 ...
 [searchHistory; onSearch]);
  ...
  // Save search;
  const saveSearch = useCallback((searchQuery: string) : any => {
 ...
 [handleSearch]);
  ...
  const handleResultClick = useCallback((result: SearchResult) : any => {
