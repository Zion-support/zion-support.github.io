...
interface SafeImageProps  {interface SafeImageProps  {src: string;
...
const [hasError,
setHasError] = useState(false)const [currentSrc,
setCurrentSrc]  = useState(src)const handleError = ()  => {
if (!hasError && fallbackSrc && currentSrc !== fallbackSrc) {setCurrentSrc(fallbackSrc)setHasError(true)
...
if (!hasError && fallbackSrc && currentSrc !== fallbackSrc) {setCurrentSrc(fallbackSrc)setHasError(true)
      ...
      const fallbackUrl = `/api/image${src}`,
setCurrentSrc(fallbackUrl)setHasError(true)} else if (!hasError) {setHasError(true)}
  }// If we have an error and no fallback, show a placeholder;
  if (hasError && (!fallbackSrc || currentSrc === fallbackSrc)) {return (<div;
className={`flex items-center justify-center bg-gray-100 text-gray-400 ${className}`}
        style={{ width, height }}
...
function SafeImage() {
const [has_error,
setHasError] = useState (false)const [current_src,
setCurrentSrc] = useState (src)const handle_error = () =>: any {// Check condition;
if ( {) {$2;
