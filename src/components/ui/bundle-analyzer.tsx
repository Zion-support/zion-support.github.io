...
const show = null;
    ...
    setIsVisible(true)collectBundleInfo()
...
} = use_auth ()const is_admin = user?.user_type === 'admin' || user?.role === 'admin';
...
if ( {) {$2;
  ...
  const [bundle_info, setBundleInfo] = useState < BundleInfo | null>(null)const [chunks, set_chunks] = useState < ChunkInfo[]>([])const [is_visible, setIsVisible] = useState (false)const [is_collecting, setIsCollecting] = useState (false)const [should_show, setShouldShow] = useState (false)useEffect ((, )  => {
// Only show in development or when explicitly enabled;
    const show =;
      process.env.NODE_ENV === 'development' ||;
...
chunkData.push({name: entry.name.split('/').pop()?.split('?')[0] |'unknown';
          size;
          loadTime;
          cached;
      ...
      setBundleInfo({setIsVisible (true)collectBundleInfo ()}, [])const collectBundleInfo = async ()  => {
if (typeof window === 'undefined') return;
    setIsCollecting(true)try {totalSize,
gzippedSize,
...
const size = entry.transfer_size || entry.encodedBodySize || 0;
        ...
        chunk_data.push ({name: entry.name.split ('/').pop ()?.split ('?')[0] || 'unknown',
size,
load_time,
cached
})})// Estimate gzipped size (roughly 70% of original)const gzipped_size = total_size * 0.7;
      const cacheHitRate =;
        chunk_data.filter (chunk => chunk.cached).length / chunk_data.length;
      setBundleInfo ({total_size,
gzipped_size,
chunk_count: chunk_data.length,
load_time: totalLoadTime / chunk_data.length,
cacheHitRate: cacheHitRate * 100})set_chunks (chunk_data.sort ((a, b) => b.size - a.size).slice (0, 5))// Top 5 largest chunks    } catch (error) {logErrorToProduction ('Failed to collect bundle info:', { data: error })} finally {setIsCollecting(false)}
  }
...
try {// Get performance entries for script resources;
...
let totalLoadTime = 0,
const chunkData: ChunkInfo[] = [],
scriptEntries.forEach(entry => {const size = entry.transferSize || entry.encodedBodySize || 0,
const loadTime = entry.responseEnd - entry.requestStart,
const cached = entry.transferSize === 0,
totalSize += size,
totalLoadTime += loadTime,
...
setIsVisible(!current)if (!current) {collectBundleInfo()
}
  }
if (!shouldShow) {return null;
  }
          ...
          {bundle_info ? (<>;
              <div className='grid grid - cols - 2 gap - 2 text - xs'>;
                ...
                <div className='flex justify - between'>;
                  <span > Avg Load:</span>;
                  <Badge variant='outline'>;
                    {bundle_info.load_time.to_fixed (0)}
ms;
                  </Badge>;
                </div>;
              </div>;
