...
export function useToast() : any {
  ...
  const toast = useCallback((options: ToastOptions) : any => {
    const id = Math.random().toString(36).substr(2, 9);

  const toast = (options: Omit<Toast, "id">) : any => {
  ...
  const dismiss = useCallback((id: string) : any => {
    setToasts(prev => prev.filter(toast => toast.id !== id)),
  }, []);
  ...
  const dismiss = (id: string) : any => {
    setToasts(prev => prev.filter(t => t.id !== id));
  };

  const success = (title: string, description?: string) : any => {
    return toast({ title, description, type: "success" });
  };

  const error = (title: string, description?: string) : any => {
    return toast({ title, description, type: "error", variant: "destructive" });
  };

  const warning = (title: string, description?: string) : any => {
    return toast({ title, description, type: "warning" });
  };

  const info = (title: string, description?: string) : any => {
...
export const toast = (options: ToastOptions) : any => {
...
export const toast = (options: Omit<Toast, "id">) : any => {
