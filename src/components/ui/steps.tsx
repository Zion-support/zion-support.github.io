...
interface StepProps  {status: "incomplete" | "current" | "complete";
  label: string;
  description?: string;className?: string;
}
export function Step() {import React from "react",import { cn } from "@/lib/utils",interface StepProps  {status: "incomplete" | "current" | "complete",label: string,description?: string,className?: string;
}
...
interface StepsProps  {current_step: number,class_name?: string;
  children: React.ReactNode;
}
export /**;
 * Steps - Function description;
 */;
function Steps() {const children_array = React.Children.to_array (children)return (<div className={cn ("w - full", class_name)}>;
      <ol className="space - y-6 md:flex md:space - y-0 md:space - x-16">;
