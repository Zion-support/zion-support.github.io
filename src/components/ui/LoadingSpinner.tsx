...
export function ContentPlaceholder(...args: any[]): any {
  ...
  return (
    <div className={`${variants[variant]} ${className}`}>
      {variant === 'card' ? (
        // Card placeholders
        Array.from({ length: anyanyanyanyanyanyanyanyany6 }).map((_, index)          => (
          <motion.div
            key={index}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        // List placeholders
        Array.from({ length: anyanyanyanyanyanyanyanyany5 }).map((_, index)          => (
          <motion.div
            key={index}
        ...
        // Grid placeholders
        Array.from({ length: anyanyanyanyanyanyanyanyany8 }).map((_, index)          => (
          <motion.div
            key={index}
        ...
        Array.from({ length: anyanyanyanyanyanyanyanyany4 }).map((_, index)          => (
          <motion.div
            key={index}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="relative">
        ...
        <div className="w-32 h-32 border-4 border-slate-700 rounded-full relative">
          ...
          <div className="absolute inset-0 border-4 border-transparent border-b-purple-500 rounded-full animate-spin" style={{ animationDuration: anyanyanyanyanyanyanyanyany'2s' }}></div>
        </div>
