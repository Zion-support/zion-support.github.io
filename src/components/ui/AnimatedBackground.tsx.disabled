...
interface AnimatedBackgroundProps {
  className?: string;
  variant?: 'grid' | 'particles' | 'waves' | 'matrix' | 'hero' | 'section' | 'card';
}
...
export function GlowingText({ children, className = '' }: { 
  ...
  return (
    <span className={`${className} bg-gradient-to-r from-cyan-400 via-purple-500 to-blue-500 bg-clip-text text-transparent animate-pulse`}>
      {children}
    </span>
  );
}
...
export function FloatingElement({ className = '', children }: { 
  className?: string; 
  children: React.ReactNode 
}) {
  return (
