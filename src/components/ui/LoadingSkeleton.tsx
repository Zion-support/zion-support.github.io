...
export const Skeleton: React.FC<SkeletonProps> = ({ 
  ...
  if (animated) {
    return (
      <>
        {Array.from({ length: anyanyanyanyanyanyanyanyanyanyanycount }).map((_, index)            => (
          <motion.div
            key={index}
            ...
            transition={{
              duration: anyanyanyanyanyanyanyanyanyanyany1.5,
              repeat: Infinity,
              ease: "easeInOut"
            }}
          />
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden ${className}`}>
      ...
      <div className="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
        <div className="flex space-x-4">
          {Array.from({ length: anyanyanyanyanyanyanyanyanyanyanycolumns }).map((_, index)            => (
      ...
      {/* Rows */}
      <div className="divide-y divide-gray-200 dark: anyanyanyanyanyanyanyanyanyanyanydivide-gray-600">
        {Array.from({ length: rows }).map((_, rowIndex)            => (
          <div key={rowIndex} className="px-6 py-4">
            <div className="flex space-x-4">
              {Array.from({ length: anyanyanyanyanyanyanyanyanyanyanycolumns }).map((_, colIndex)            => (
                <Skeleton 
                  key={colIndex} 
...
export function ListSkeleton(...args: any[]): any {
  return (
    <div className={`space-y-4 ${className}`}>
      {Array.from({ length: anyanyanyanyanyanyanyanyanyanyanyitems }).map((_, index)            => (
...
export function GridSkeleton(...args: any[]): any {
  return (
    <div className={`grid grid-cols-1 sm: anyanyanyanyanyanyanyanyanyanyanygrid-cols-2 lg:grid-cols-${columns} gap-6 ${className}`}>
