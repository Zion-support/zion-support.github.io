...
export function NotificationProvider({ children, maxNotifications = 5, position = 'top-right' }: PropsWithChildren<{ maxNotifications?: number; position?: string }>) {
...
function NotificationItem({ notification }: { notification: Notification }) {
	...
	const getIcon = (type: NotificationType) => {
		switch (type) {
			case 'success': return <CheckCircle className="w-5 h-5 text-green-4 00" />;
	...
	return (
		...
		>
			{notification.duration && notification.duration > 0 && (
				<motion.div className={`absolute top-0 left-0 h-1 ${getProgressColor(notification.type)}`} initial={{ width: '100%' }} animate={{ width: '0%' }} transition={{ duration: (notification.duration || 0) / 1000, ease: 'linear' }} />
			)}
...
export function showSuccess(title: string, message?: string, options?: Partial<Notification>) { return { type: 'success', title, message, ...options } as Notification; }
export function showError(title: string, message?: string, options?: Partial<Notification>) { return { type: 'error', title, message, ...options } as Notification; }
export function showWarning(title: string, message?: string, options?: Partial<Notification>) { return { type: 'warning', title, message, ...options } as Notification; }
export function showInfo(title: string, message?: string, options?: Partial<Notification>) { return { type: 'info', title, message, ...options } as Notification; }
