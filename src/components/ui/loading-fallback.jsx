 import { motion } from 'framer-motion' import { Loader2 export function LoadingFallback() {}"
;
  const sizeClasses = {}';'; "sm": 'w-6 h-6', ''' "md": 'w-8 h-8', ''' "lg": 'w-12 h-12' }'

const textSizes = ,;
}';'; "sm": 'text-sm', ''' "md": 'text-base', ''' "lg": 'text-lg' } return (<div className={`flex flex-col items-center justify-center p-8 space-y-4 ${className}`}>" <motion.div animate={{ "rotate": 360 }} transition = ,"}"';""" { "duration": 1, "repeat": Infinity,"""" "ease": "linear"""`;""`;"`"" }} className={`${sizeClasses[size]} text-zion-purple`}>"""" <Loader2 className="w-full h-full" /> </motion.div> <motion.p initial = {} { "opacity": 0, "y": 10 }} animate = ,"} { "opacity": 1, "y": 0 }} transition = 
