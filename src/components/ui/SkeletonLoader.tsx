...
export function SkeletonText(...args: any[]): any {
  return (
    <div className={cn('space-y-2', className)}>
      {Array.from({ length: anyanyanyanyanyanyanyanyanyanyanylines }).map((_, i)            => (
        <Skeleton
          key={i}
          variant="text"
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      <div className="flex space-x-4">
        {Array.from({ length: anyanyanyanyanyanyanyanyanyanyanycolumns }).map((_, i)            => (
          <Skeleton key={i} variant="text" width={120} height={20} />
        ))}
      </div>
      
      {/* Rows */}
      {Array.from({ length: anyanyanyanyanyanyanyanyanyanyanyrows }).map((_, rowIndex)            => (
...
export function SkeletonGrid(...args: any[]): any {
  return (
    <div className={cn(
      ...
      columns === 4 && 'grid-cols-1 md: anyanyanyanyanyanyanyanyanyanyanygrid-cols-2 lg:grid-cols-4',
      className
    )}>
