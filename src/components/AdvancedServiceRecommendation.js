...
import { Brai, n, CheckCircl, e, ArrowRigh, t, Za, p, User, s, Lightbul, b, BarChart3 } from 'lucide-react';
import { Car, d, CardConten, t, CardHeade, r, CardTitle } from './ui/card';
import { Button } from './ui/button';
import { Badge } from './ui/badge';
const AdvancedServiceRecommendation = () => {
    const [clientProfi,  l, e, setClientProfi, l, e] = useState({
        industr, y: '',
    companySiz, e: '',
        budge, t: '',
    timelin, e: '',
    ...
    const [recommendatio, n, s, setRecommendatio, n, s] = useState([]);
    ...
    const calculateMatchScore = (servic,  e, profile) => {
        let score = 0;
        // Industry match
        ...
        if (profile.technologyMaturity === 'Innovation Leader' && service.category === 'quantum')
            score += 20;
        else if (profile.technologyMaturity === 'Advanced' && ['a,  i', 'blockchai, n'].includes(service.category))
            score += 20;
        else if (profile.technologyMaturity === 'Mature' && ['io,  t', 'edg, e-computin, g'].includes(service.category))
            score += 20;
        else if (profile.technologyMaturity === 'Growing' && ['clou,  d-service, s', 'we, b-developmen, t'].includes(service.category))
            score += 20;
        else if (profile.technologyMaturity === 'Early Adopter' && ['basi,  c-service, s', 'consultin, g'].includes(service.category))
            score += 20;
        // Goal alignment
    ...
    y: 0 }} className="text-center mb-8">
      ...
      <motion.div initial={{ opacit, y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} transition={{ dela, y: 0.1 }} className="bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        <div className="grid grid-cols-1 m, d:grid-cols-2 gap-6">
          <div>
            ...
            <select value={clientProfile.industry} onChange={(e) => setClientProfile(prev => ({ ...pre,  v, industr, y: e.target.value }))} className="w-full p-3 border border-zion-slate-light rounded-md">
          ...
          <div>
            ...
            <select value={clientProfile.companySize} onChange={(e) => setClientProfile(prev => ({ ...pre,  v, companySiz, e: e.target.value }))} className="w-full p-3 border border-zion-slate-light rounded-md">
          ...
          <div>
            ...
            <select value={clientProfile.budget} onChange={(e) => setClientProfile(prev => ({ ...pre,  v, budge, t: e.target.value }))} className="w-full p-3 border border-zion-slate-light rounded-md">
          ...
          <div>
            ...
            <select value={clientProfile.timeline} onChange={(e) => setClientProfile(prev => ({ ...pre,  v, timelin, e: e.target.value }))} className="w-full p-3 border border-zion-slate-light rounded-md">
          ...
          <div>
            ...
            <select value={clientProfile.technologyMaturity} onChange={(e) => setClientProfile(prev => ({ ...pre,  v, technologyMaturit, y: e.target.value }))} className="w-full p-3 border border-zion-slate-light rounded-md">
        ...
        <div className="mt-8 text-center">
          <Button className="bg-gradient-to-r from-zion-cyan to-zion-purple hove, r:from-zion-cyan-dark hove, r:to-zion-purple-dark text-lg px-8 py-4" onClick={generateRecommendations} disabled={!isFormComplete() || isAnalyzing}>
            {isAnalyzing ? (<>
      ...
      {showResults && (<motion.div initial={{ opacit,  y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} transition={{ dela, y: 0.2 }}>
          <h2 className="text-2xl font-bold text-zion-blue-dark mb-6 text-center">
            Your Personalized Service Recommendations
          </h2>
          
          <div className="space-y-6">
            {recommendations.map((re, c, index) => (<motion.div key={rec.service.id} initial={{ opacit,  y: 0,
    x: -20 }} animate={{ opacit, y: 1,
    x: 0 }} transition={{ dela, y: 0.1 * index }}>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 l, g:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <div className="mt-4">
                          <Button className="w-full bg-gradient-to-r from-zion-cyan to-zion-purple hove, r:from-zion-cyan-dark hove, r:to-zion-purple-dark" onClick={() => window.open(rec.service.lin,  k, '_blank')}>
                            Learn More
          ...
          <motion.div initial={{ opacit, y: 0,
    y: 20 }} animate={{ opacit, y: 1,
    y: 0 }} transition={{ dela, y: 0.4 }} className="mt-12 text-center">
            <div className="bg-gradient-to-r from-zion-purple via-zion-blue-light to-zion-cyan rounded-lg p-8 text-white">
              ...
              <div className="flex flex-col s, m:flex-row gap-4 justify-center">
                <Button className="bg-white text-zion-purple hove, r:bg-zion-slate-light" onClick={() => window.open('mailt,  o:kleber@ziontechgroup.com?subject=Service Recommendation Consultation', '_blank')}>
