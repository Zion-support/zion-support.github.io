...
const ServiceCard: React.FC<ServiceCardProps>= ({ service, index }) => {
  const [isHovered, setIsHovered] = useState(false)</ServiceCardProps>
  const formatPrice = (pricing: typeof service.pricing) => {
    if (pricing.custom) {
      return 'Custom Pricing'</ServiceCardProps
    }
    return `$${pricing.starting.toLocaleString()}/${pricing.period === 'month' ? 'mo' : pricing.period === 'year' ? 'yr' : 'one-time'}`</ServiceCardProps
  }</ServiceCardProps
...
const EnhancedServicesShowcase: React.FC = () => {
  ...
  const [filteredServices, setFilteredServices] = useState(services)</string>
  useEffect(() => {
    if (selectedCategory === 'all') {
      setFilteredServices(services)</string
    } else {
      setFilteredServices(services.filter(service => service.category === selectedCategory))</string
    }
  }, [selectedCategory])</string
  const popularServices = services.filter(service => service.popular)</string
