...
import { Phone, Mail, ExternalLink, Star, CheckCircle, ArrowRight, Zap, Brain, Cloud, Rocket, TrendingUp, Users, Award, Shield, Clock, DollarSign, Target, BarChart, Sparkles  } from 'lucide-react'import { services, serviceCategories, contactInfo  } from '../data/services'
interface ServiceCardProps {
  service: typeof services[0]
  index: number
}
const ServiceCard: React.FC<ServiceCardProps> = ({ service, index }) => {
  const [isHovered, setIsHovered] = useState(false)const formatPrice  = () => {
    if (pricing.custom) {
  ...
  const newServices = services.filter(service => service.new)return (
