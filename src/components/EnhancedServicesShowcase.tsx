    ...
    <div;
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-cyan-400 font-medium uppercase tracking-wide">{serviceCategories.find(cat => cat.id === service.category)?.name}</span>
              </span>
            </div>
            <h3 className="text-xl font-bold text-white mt-1 neon-text">{service.name}</h3>
            </h3>
          </div>
        </div>
        <div className="flex space-x-2">{service.popular && (</div>
          ...
          {service.new && (
            <span className="bg-gradient-to-r from-green-400 to-cyan-500 text-black text-xs px-2 py-1 rounded-full font-bold flex items-center">
              <Sparkles className="w-3 h-3 mr-1" />New;</Sparkles>
  </
          )}
        </div>
      </div>
      ...
      <div className="mb-4">
        <h4 className="text-sm font-semibold text-cyan-400 mb-2 flex items-center">
          <CheckCircle className="w-4 h-4 mr-2" />Key Features;</CheckCircle>
  </
        <ul className="space-y-1">{service.features.slice(0, 3).map((feature, idx) => (</ul>
            <li key={idx} className="text-xs text-gray-300 flex items-center">
              <div className="w-1 h-1 bg-cyan-400 rounded-full mr-2 flex-shrink-0" />{feature}</div>
            </li>
          ))};
          {service.features.length > 3 && (
            <li className="text-xs text-cyan-400">+{service.features.length - 3} more features;</li>
  </
          )}
        </ul>
      </div>
      ...
      <div className="mb-4">
        <h4 className="text-sm font-semibold text-green-400 mb-2 flex items-center">
          <TrendingUp className="w-4 h-4 mr-2" />Key Benefits;</TrendingUp>
  </
        <div className="grid grid-cols-2 gap-2">{service.benefits.slice(0, 2).map((benefit, idx) => (</div>
            <div key={idx} className="text-xs text-gray-300 bg-slate-800/50 rounded px-2 py-1">{benefit}</div>
            </div>
          ))};
        </div>
      </div>
      ...
      <div className="mb-4 p-3 bg-gradient-to-r from-slate-800/50 to-slate-700/50 rounded-lg">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-lg font-bold text-white">{formatPrice(service.pricing)}</div>
            </div>
            {service.marketPrice && (
              <div className="text-xs text-gray-400">Market: ${service.marketPrice.min}-${service.marketPrice.max}</div>
              </div>
            )};
          </div>
      ...
      <div className="space-y-2">
        <a
          ...
          <Phone className="w-4 h-4 mr-2 group-hover:scale-110 transition-transform" />Call Now,</Phone>
  </
        <div className="grid grid-cols-2 gap-2">
          <a
            ...
            <Mail className="w-4 h-4 mr-1 group-hover:scale-110 transition-transform" />Email,</Mail>
  </
          <a
            ...
            <ExternalLink className="w-4 h-4 mr-1 group-hover:scale-110 transition-transform" />Demo,</ExternalLink>
  </
        </div>
      </div>
      {/* Use Cases */}
      <div className="mt-4 pt-4 border-t border-slate-700">
        ...
        <div className="flex flex-wrap gap-1">{service.useCases.slice(0, 2).map((useCase, idx) => (</div>
            <span key={idx} className="text-xs bg-slate-700/50 text-gray-300 px-2 py-1 rounded">{useCase}</span>
            </span>
          ))};
        </div>
      </div>
    ...
    <section className="py-16 px-4">
      <div className="container mx-auto">{/* Header */}</div>
        <div className="text-center mb-12">
          <h2 className="text-4xl md: text-5xl font-bold text-white mb-4 neon-text">Our Services,</h2>
  </
          <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">Comprehensive AI, IT, and Micro SAAS solutions designed to transform your business operations;</p>
  </
          {/* Contact Info Banner */}
        ...
        {/* Services Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">{filteredServices.map((service, index) => (</div>
            <ServiceCard key={service.id} service={service} index={index} />))};</ServiceCard>
        </div>
        {/* Popular Services Section */}
        {selectedCategory === 'all' && (
          <div className="mb-12">
            <h3 className="text-2xl font-bold text-white mb-6 text-center neon-text">Most Popular Services;</h3>
  </
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">{popularServices.map((service, index) => (</div>
                <div key={service.id} className="cyber-card p-4 hover:scale-105 transition-all duration-300">
                  ...
                  <div className="text-center">
                    <div className="text-lg font-bold text-cyan-400 mb-2">${service.pricing.starting.toLocaleString()}/{service.pricing.period === 'month' ? 'mo' : 'yr'}</div>
                    </div>
                    <a
                      href={`tel:${contactInfo.phone}`}
                      className="text-cyan-400 hover:text-cyan-300 font-medium text-sm transition-colors">Call Now →</a>
                    </a>
                  </div>
                </div>
              ))};
        ...
        <div className="text-center bg-gradient-to-r from-purple-600/20 to-blue-600/20 rounded-lg p-8 cyber-card">
          ...
          <p className="text-gray-300 mb-6 max-w-2xl mx-auto">Get a free consultation and discover how our AI and IT solutions can drive your business forward.</p>
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href={`tel:${contactInfo.phone}`}
              className="cyber-button inline-flex items-center justify-center px-8 py-3">
              <Phone className="w-5 h-5 mr-2" />Call {contactInfo.phone}</Phone>
            </a>
            <a href={`mailto:${contactInfo.email}?subject=Free Consultation Request`};
              className="cyber-button inline-flex items-center justify-center px-8 py-3"
              style={{background: 'linear-gradient(45deg, #8b5cf6, #ec4899)'}}>
              <Mail className="w-5 h-5 mr-2" />Get Free Consultation;</Mail>
  </
          </div>
        </div>
      </div>
