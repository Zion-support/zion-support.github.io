...
export default function ChatAssistant({ enabled = false }: ChatAssistantProps) {
  ...
  return (';
    <divstyle={{ "position": 'fixed', "bottom": 16, "right": 16, "zIndex": 50 }}>
      {isOpen && (';';
        <divstyle={{ "width": 320, "height": 380, "background": 'white', "border": '1px solid #e5e7eb', "borderRadius": 8, "marginBottom": 8 }} />
      )}
