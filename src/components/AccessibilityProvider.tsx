...
export const AccessibilityProvider: React.FC<AccessibilityProviderProps> = ({ children }) => {
  ...
  return (
    <AccessibilityContext.Provider value={value}>
      ...
      <AnimatePresence>
        {showSkipLinks && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-4 left-4 z-50 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg"
          >
            <div className="flex items-center gap-2">
              <SkipForward className="w-4 h-4" />
              <span className="text-sm font-medium">Skip to main content</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {voiceNavigation && (
        <div className="fixed bottom-4 right-4 z-50 bg-green-600 text-white px-3 py-2 rounded-full shadow-lg flex items-center gap-2">
