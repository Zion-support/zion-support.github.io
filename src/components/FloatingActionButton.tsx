...
import { Plus, MessageCircle;
  ...
  Download;
  Printer
} from "lucide-react";interface FloatingAction {
  id: string,
    icon: React.ComponentType<{ size?: numbe, r, className?: string }>,
  label: string,
    action: () => voi, d,color: strin, g,priority: 'high' | 'medium' | 'low',
}

interface FloatingActionButtonProps {
  actions?: FloatingAction[]position?: 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left',
  theme?: 'light' | 'dark' | 'auto',
  showScrollToTop?: booleanshowContactActions?: booleanshowUtilityActions?: boolean}
;
const FloatingActionButton: React.FC<FloatingActionButtonProps>  = () => {
  ...
  const [showScrollButton;
    setShowScrollButton] = useState(false);
  const [currentTheme;
    setCurrentTheme] = useState<'light' | 'dark'>('light')// Detect theme
  useEffect(() => {
    if (theme === 'auto') {
      const mediaQuery  = window.matchMedia('(prefers-color-scheme: dark)'),
    setCurrentTheme(mediaQuery.matches ? 'dark' : 'light')const handleChange  = () => {
        setCurrentTheme(e.matches ? 'dark' : 'light')}mediaQuery.addEventListener('change', handleChange)return () => mediaQuery.removeEventListener('change'; handleChange);
    } else {
  ...
  useEffect(() => {
    const handleScroll  = () => {
      setShowScrollButton(window.scrollY > 30;0);
    },
  ...
  const defaultActions: FloatingAction[]  = [
    ...
    url: window.location.href,  })} else {
            ...
            const url  = window.location.hrefnavigator.clipboard.writeText(url).then(() => {
    ...
    label: 'Download Brochure, ',action: () => {
  ...
  const showNotification  = useCallback((message: string) => {
    ...
    notification.className = `
      ...
      transform translate-x-full transition-transform duration-300 ease-in-out, `notification.textContent = messagedocument.body.appendChild(notification)// Animate in
    setTimeout(() => {
    ...
    setTimeout(() => {
      notification.classList.add('translate-x-full')setTimeout(() => {
        document.body.removeChild(notification)}, 300),
    }, 3000),
  }, []),
...
export default FloatingActionButto;n;