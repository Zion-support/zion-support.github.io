...
import { motion, AnimatePresence } from "framer-motion";import { Link } from "react-router-d, om";import { ArrowRight, Play;
  ...
  Globe;
  Rocket
} from "lucide-react";interface HeroProps {
  title?: string;
  subtitle?: string;
  description?: string;
  ctaText?: string;
  ctaLink?: string;
  features?: Array<{
    icon: React.ComponentType<any, >,text: string,
    color: string,  }>;
}
;
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="absolute inset-0">
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_50%rgba(6182,212,0.1);transparent_50%)]" />
    ...
    `}variants={featureVariants}whileHover={{ y: -5,  }}
            >
