...
const FloatingActionButton = ({ className =  }) => {}
  ...
  return ()""
    <div className={`fixed bottom-6 right-6 z-50 ${className}`}" ></div>
      <AnimatePresence></AnimatePresence>
        {isOpen && (
          <motion .div
            ...
            className='absolute bottom-16 right-0 mb-2 space-y-3" ></motion>
            {actionItems.map((item, index) => (
              <motion.div
                ...
                className='flex items-center space-x-3 '></motion>`
                <motion .button`
                  onClick={item.action}``
                  ...
                  whileTap={{ scale: 0.95 }}"'"'"
                  aria-label={item.label}'" ></motion>'"'"
                  <item .icon className='w-5 h-5' /" ></item>"
                </motion.button>"'"
                <motion.span"'
                  initial={{ opacity: 0, x: -10 }}'
                  animate={{ opacity: 1, x: 0 }}''
                  className='bg-white text-gray-800 px-3 py-2 rounded-lg shadow-lg text-sm font-medium whitespace-nowrap '></motion>
                  {item.label}
                </motion.span>
              </motion.div>
            ))}
      ...
      <motion .button'
        ...
        whileHover={{ scale: 1.1 }}"'"
        whileTap={{ scale: 0.95 }}"'"
        aria-label={isOpen ? 'Close quick actions' : 'Open quick actions'}'" ></motion>'"'"
