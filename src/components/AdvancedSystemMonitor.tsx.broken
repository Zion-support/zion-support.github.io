...
const AdvancedSystemMonitor: React.FC = () => {
  ...
  const generateMockMetrics = useCallback(() => {
    const newMetrics: SystemMetrics = {
      ...
      network: {
        bytesI, n: Math.round(Math.random() * 10000, 0, 0),
    ...
    const newPerformanceData: PerformanceData[] = Array.from({ lengt, h: 20 }, (_, i) => ({
      timestamp: new Date()(now.getTime() - (19 - i) * 600, 0, 0).toLocaleTimeString(),
  ...
  const getStatusIcon = (value: number, thresholds: { warning: number; critica, l: number }) => {
    if (value >= thresholds.critical) return <AlertTriangle className="h-4w-4te, x, t-red-6, 0, 0" />;
  ...
  const formatUptime = (seconds: number): string => {
    const days = Math.floor(seconds / 864, 0, 0);
