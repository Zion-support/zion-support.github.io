...
import {
  ...
  ErrorReport,
  RecoveryAction,
} from "../utils/comprehensiveErrorRecovery";
...
export const ErrorRecoveryDashboard: React.FC = () => {
  const { errorReports, executeRecoveryAction, clearResolvedReports } =
    useErrorRecovery();
  ...
  const groupedReports = errorReports.reduce(
    (groups: Record<string, ErrorReport[]>, report: ErrorReport) => {
  ...
  const pendingErrors = errorReports.filter(
    (r: ErrorReport) => r.status === "pending",
  ).length;
  const recoveredErrors = errorReports.filter(
    (r: ErrorReport) => r.status === "recovered",
  ).length;
  const failedErrors = errorReports.filter(
    (r: ErrorReport) => r.status === "failed",
  ).length;
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-gray-500">
                      {(reports as ErrorReport[]).length} errors
                    </span>
                  </div>

                  <div className="space-y-2">
                    {(reports as ErrorReport[])
                      .slice(0, 3)
                      .map((report: ErrorReport) => (
                        <div
                          key={report.id}
                        ...
                        >
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            {report.context.timestamp.toLocaleTimeString()} •{" "}
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-gray-600 ml-1">
                      {selectedReport.context.component || "Unknown"}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <span
                      className={`ml-1 ${getSeverityColor(selectedReport.context.severity)}`}
                    >
                  ...
                  <div>
                    ...
                    <span className="text-gray-600 ml-1">
                      {selectedReport.attempts}/{selectedReport.maxAttempts}
                    </span>
                  </div>
                    ...
                    <div className="space-y-2">
                      {selectedReport.recoveryActions.map((action) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              handleExecuteAction(selectedReport.id, action)
                            }
