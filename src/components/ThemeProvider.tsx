...
export const ThemeToggle: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  const getIcon = () => {
    if (theme === 'system') {
      return (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17, L, 9 2, 0, l-1 1h, 8, l-1-1-.75-3, M, 3 13h18, M, 5 17h14, a, 2 2 0 0, 0, 2-2V5, a, 2 2 0 00-2-2H5, a, 2 2 0 00-2 2v10, a, 2 2 0 0, 0, 2 2z" />        </svg>
      );
    }
    ...
    return (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1, m, 0 16v1, m, 9-9h-1, M, 4 12H3m, 1, 5.3, 6, 4 6.36, 4, l-.7, 0, 7-.707, M, 6.3, 4, 3 6.34, 3, l-.7, 0, 7-.707m, 1, 2.7, 2, 8 0l-.7, 0, 7.707, M, 6.3, 4, 3 17.65, 7, l-.7, 0, 7.707M, 1, 6 12, a, 4 4 0 11-8 0 4 4 0 0, 1, 8 0z" />      </svg>
    );
  };
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`p-2 round e d-lg bg-gr a y-1 0 0 da r k: bg-gr a y-8 0 0 te x t-gr a y-7 0 0 da r k:te x t-gr a y-3 0 0 hov e r:bg-gr a y-2 0 0 da r k:ho v e r:bg-gr a y-7 0 0 transiti o n-colo r s durati o n-2 0 0 ${className}`}      title={getTooltip()}      aria-label={getTooltip()}
    >
      {getIcon()}
    </button>
  );
