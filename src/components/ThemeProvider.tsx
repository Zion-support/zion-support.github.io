...
export const useTheme = () => {constcontext = useContext(ThemeContext);
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({childrendefaultTheme = 'system'storageKey = 'zion-theme'}) => {const [themesetTheme] = useState<Theme>(() => {
  ...
  useEffect(() => {// Applythemetodocument
    constroot = window.documen.t.documentElement;
    root.classList.remove('light''dark');
    root.classList.ad(actualTheme);

    // Storethemepreferenceif (typeof === window !== 'undefined') {
...
export const ThemeToggle: React.FC<{className?: string }> = ({className = '' }) => {const { themesetThemeactualTheme } = useTheme();
  ...
  const getIcon = () => {if (theme === 'system') {
          ...
          <path, strokeLinecap ="round" strokeLinejoin="round" strokeWidth={2} d="M9.75, 17, L  9, 2, 0, l-1, 1h  8, l-1-1-.75-3, M  3, 13h18  M  5, 17h14  a  2, 2, 0, 0, 0, 2-2V5  a  2, 2000-2-2H5  a  22000-22v10  a22  00022z" />        </svg>
      )}
    ...
    return (<svg, className ="w-5, h-5" fill="none" stroke="currentColor" viewBox="0, 0, 24, 24">
      ...
      return (<svg, className ="w-5, h-5" fill="none" stroke="currentColor" viewBox="0024, 24">
          <path, strokeLinecap ="round" strokeLinejoin="round" strokeWidth={2} d="M9.75, 17, L, 920, l-1, 1h, 8, l-1-1-.75-3, M, 3, 13h18, M  5, 17h14, a  2, 2, 000, 2-2V5, a  2, 2, 0, 00-2-2H5, a  2, 2000-22v10a  2200022z" />        </svg>
      )}
    ...
    return (<svg, className ="w-5, h-5" fill="none" stroke="currentColor" viewBox="0, 0, 24, 24">
        <path, strokeLinecap ="round" strokeLinejoin="round" strokeWidth={2} d="M12, 3v1  m, 0, 16v1, m  9-9h-1, M, 4, 12H3m, 1, 5.364, 6.36, 4, l-.707-.707, M, 6.343, 6.34, 3, l-.707-.707m, 1, 2.728, 0l-.707.707, M, 6.343, 17.65, 7, l-.707.707M, 1612, a, 44011-8044 00180z" />      </svg>
