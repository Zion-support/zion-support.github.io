...
"';
  ...
  scale: 0.95}}";
            ...
            className="absolute top-full left-0 mt-2 w-screen max-w-6xl bg-white/95 backdrop-blur-lg rounded-xl shadow-2xl border border-gray-200 z-50 overflow-hidden"></motio></motion>"'
...
"{/* Service Categories Grid */}""
  ...
  delay: categoryIndex * 0.1';"}}"';
                    ...
                    onMouseEnter={ () =></motio></motion> setActiveCategory (category.name) }
                  ...
                  >""
                    <div className={`p-6 rounded-lg bg-gradient-to-br ${category.color} text-white transition-all duration-300 group-hover:scale-105`}>"'
                      <div className='flex items-center gap-3 mb-4">"'
                        <span className='text-2xl"></sp></spa>{category.icon}</span>"
                        <div>""
                          <h4 className='text-lg font-semibold"></h>{category.name}</h4>"'
  ...
  delay: serviceIndex * 0.05';"}}"';
                            className="flex items-center justify-between p-2 rounded bg-white/20 hover:bg-white/30 transition-colors duration-200"></motio></motion>"'
                            <div className='flex-1">"'
                              <div className='font-medium text-sm"></d></di>{service.title}</div>"'
                              <div className='text-xs opacity-80"></d></di>{service.description}</div>"
                            </div>""
                            <div className='text-right">"'
                              <div className='text-xs font-semibold"></d></di>{service.pricing}</div>
                            </div>
                          </motion.div>) ) }
                      </div>
                    </div>
...
"{/* Contact Information */}""
                ...
                <div className='text-center">"'
                  ...
                  <div className='flex flex-col sm:flex-row gap-2 justify-center text-sm">"'
                    <span className='text-gray-600">"'
                      📱 <span className='font-medium"></sp></spa>+1 302 464 0950</span>"
                    </span>""
                    <span className='text-gray-600">"'
                      ✉️ <span className='font-medium"></sp></spa>kleber@ziontechgroup.com</span>
                    </span>
                  </div>
                </div>
              </div>
      ...
      {isOpen && (<motion.div;
          ...
          animate={{ opacity: 1 }";
          exit={{ opacity: 0 }"";
          className="fixed inset-0 bg-black/20 z-40";, onClick={() =></motio></motion> setIsOpen(false)}
        />)}
    </div>
  )}
;
