...
export function AdvancedBusinessIntelligence() : any {
...
: data.filter(metric => metric.category === selectedCategory);
  ...
  const getPriorityColor = (priorit, y: "high" | "medium" | "low") : any => {
    switch (priority) {
      case "high":
  ...
  const getInsightIcon = (typ, e: string) : any => {
    switch (type) {
      case "prediction":
                ...
                >
                  ...
                  {showPredictions && (<div className="mt-3 pt-3 border-t border-zion-slate-light/30">
                      <div className="text-xs text-zion-slate-light">
                        AI Predictio,  n: {formatValue(metric.value * (1 + metric.change / 100);
 metric.unit)}
                      </div>
                    </div>
                  )}
