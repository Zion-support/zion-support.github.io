...
} = useAuth()const router  = useRouter()useEffect(()  => {
// Don't redirect while auth is still loading;
...
const returnTo = encodeURIComponent(router.asPath)router.push(`${redirectTo}?returnTo=${returnTo}`)return;
...
redirectTo,
showToast,
allowGuest])// Show loading state while auth is being determined;
  ...
  requireRole.join ('or ')}
privileges. </p> </div> </div>)// Hook for programmatic auth checks;
export function useAuthGuard() {
const { user, isAuthenticated, isLoading 
} = useAuth()const router  = useRouter()const requireAuth = (options?: {redirectTo?: string;
    showToast?: boolean;
    returnUrl?: string;
  })  => {
if (isLoading) return false;
if (!isAuthenticated) {const redirectTo = options?.redirectTo || '/login';
      const returnUrl  = options?.returnUrl || router.asPath;
if (options?.showToast !== false) {toast({title: 'Authentication Required',
...
if (!isAuthenticated || !user) return false;// Simple permission check - can be extended based on your permission system;
...
isAuthenticated,
user,
isLoading}}//Hook for programmatic auth checks ;
}
