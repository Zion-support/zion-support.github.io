...
const AdvancedAnalyticsTracker2025: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Track page view
    trackEvent('page_view', 'engagement', 'view', window.location.pathname);
    // Track scroll depth
    ...
    const trackScroll = () => {
      ...
      if (scrollPercent > maxScrollDepth) {
        ...
        trackEvent('scroll', 'engagement', 'depth', `${scrollPercent}%`);
      }
    };
    // Track time on page
    ...
    const trackInteraction = (event: Event) => {
      ...
      trackEvent('user_interaction', 'engagement', action, label);
    ...
    const trackFormSubmission = (event: Event) => {
      ...
      trackEvent('form_submission', 'conversion', 'submit', formId);
    // Track external links
    const trackExternalLink = (event: Event) => {
      const link = event.target as HTMLAnchorElement;
      if (link.href && !link.href.includes(window.location.hostname)) {
        trackEvent('external_link', 'engagement', 'click', link.href);
    // Add event listeners
    ...
    const handleBeforeUnload = () => {
      trackEvent('page_exit', 'engagement', 'before_unload', '', sessionData.timeOnPage);
