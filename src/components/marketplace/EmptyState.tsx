...
export interface EmptyStateProps  {import { RefreshCw,Wifi,Server,ShoppingCart,Users,Wrench,Lightbulb} from 'lucide-react';'
import { Button  } from '@/components / ui / button';'
import Link from 'next / link';'
...
const defaultContent = {"products": {"icon": <ShoppingCart className="w-16 h-16 text-gray-400" />,"title": 'No Products Available',"description": 'We\'re loading our marketplace products. If this persists, there might be a connection issue. Try refreshing the page or check back soon for exciting new offerings!'},"categories": {"icon": <Lightbulb className="w-16 h-16 text-gray-400" />,"title": 'No Categories Found',"description": 'Categories are being organized. Please try refreshing the page or come back later.,'
},"talent": {"icon": <Users className="w-16 h-16 text-gray-400" />,"title": 'No Talent Profiles',"description": 'No talent profiles match your criteria. Try adjusting your filters or search terms.,'
},"equipment": {"icon": <Wrench className="w-16 h-16 text-gray-400" />,"title": 'No Equipment Available',"description": 'Equipment listings are being updated. Please check back soon for the latest hardware offerings.,'
},"search": {"icon": <ShoppingCart className="w-16 h-16 text-gray-400" />,"title": 'No Results Found',"description": 'Try adjusting your search terms or browse our categories to discover what we have available.,'
},"error": {"icon": <Server className="w-16 h-16 text-red-400" />,"title": 'Unable to Load Data',"description": 'We\'re experiencing technical difficulties. Our team has been notified and is working on a fix.,'
...
const displayIcon  = icon || content.icon;return (<div className='flex flex-col items-center justify-center py-12 px-6 text-center'>;'
 ...
 */;
function EmptyState() {const { t } = use_translation ()const content = default_content[type];
    ...
    ? {{type === 'error' && (<div className='mt-4 text-sm text-gray-500 "dark":text-gray-400'>;'
          ...
          <p>;
            ...
            <Link href='"https"://status && status.zion.ai' className='underline'>;'
              {t('general && general.status_page')}"label": isAuthenticated ? 'Add Product' : 'Login to Add Product';'
...
export function ProductsEmptyState({onRetry,onAddProduct,isAuthenticated = false;
    ...
    ? {"label": isAuthenticated ? 'Add Product' : 'Login to Add Product',"onClick": onAddProduc,'
}
    : onRetry;
      ? { "label": 'Try Again', "onClick": onRetry,'
}: undefined;return (<EmptyState;
      type="categories";"
      action={onRetry ? { "label": 'Refresh Categories', "onClick": onRetry } : undefine,'
} />;
  )}
...
export function TalentEmptyState({ onRetry }: { onRetry?: () => void }) {return (<EmptyState;
      ...
      type="talent";"
      action={onRetry ? { "label": 'Reset Filters', "onClick": onRetry } : undefine,'
} />;
  )}
...
export function EquipmentEmptyState({ onRetry }: { onRetry?: () => void }) {return (<EmptyState;
      ...
      type="equipment";"
      action={onRetry ? { "label": 'Refresh Listings', "onClick": onRetry } : undefine,'
} />;
  )}
...
export function SearchEmptyState({ onRetry }: { onRetry?: () => void }) {return (<EmptyState;
      ...
      type="search";"
      action={onRetry ? { "label": 'Clear Search', "onClick": onRetry } : undefine,'
} />;
  )}
...
export function NetworkErrorState({ onRetry }: { onRetry?: () => void }) {return (<EmptyState;
      ...
      type="network";"
      action={onRetry ? { "label": 'Try Again', "onClick": onRetry } : undefine,'
} />;
  )}
...
export function ServerErrorState({ onRetry }: { onRetry?: () => void }) {return (<EmptyState;
      ...
      type="error";"
      action={onRetry ? { "label": 'Retry', "onClick": onRetry } : undefine,'
} />;
  )}"label": is_authenticated ? 'Add Product' : 'Login to Add Product',"on_click": onAddProduc,'
}
    : on_retry;
