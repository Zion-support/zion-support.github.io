          ...
          registration.addEventListener('updatefound', () => {
            ...
            if (if (newWorker) {;) {
              ...
              newWorker.addEventListener('statechange', () => {
                if (if (newWorker.state === 'installed') {;) {
                  setSwState(prev => ({ ;
                    ...prev, ;
    ...
    <AnimatePresence>
      {swState.hasUpdate && (;
        <motion.div;
          ...
          exit={{ opacity: 0, y: -50 }}
          className="className="fixed top-4 right-4 z-50 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-4 max-w-sm";"
        >
          <div className="flex items-center space-x-3">
            ...
            {swState.hasUpdate && (;
              <button;
                ...
                className="className="flex-shrink-0 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-md text-sm font-medium transition-colors";"
              >
                Update;
              </button>
            )}
