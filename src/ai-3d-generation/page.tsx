...
const AI3DGenerationPage: React.FC = () => {
  ...
  const industries = [{
      title: 'Architecture & Real Estate', description: 'Create 3D visualizations and virtual tours', icon: Building, features: ['3D building models', 'Virtual walkthroughs', 'Interior design', 'Landscape visualization']
  ...
  const features = [{
      icon: Cube, title: 'AI-Powered 3D Modeling', description: 'Generate complex 3D models from text descriptions or 2D images', benefits: ['90% faster modeling', 'Unlimited creativity', 'Consistent quality', 'Cost effective']
    },
    {
      icon: Palette,
  ...
  const benefits = ['Reduce 3D creation time by 80%', 'Lower production costs by 60%', 'Unlimited design iterations', 'Professional quality output', 'Scalable solutions', '24/7 availability'];

  const technologies = ['Neural Networks', 'Computer Vision', '3D Geometry Processing', 'Texture Synthesis', 'Real-time Rendering', 'Cloud Computing', 'GPU Acceleration', 'Machine Learning'];
                ...
                <div key={index} className="bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20 hover: bg-white/20 transition-all duration-300">
                  ...
                  <ul className="space-y-2">
                    {industry.features.map((feature, idx) => (
                      <li key={idx} className="text-sm text-gray-400 flex items-center gap-2">
                        <CheckCircle className="w-4 h-4 text-purple-400" /></CheckCircle>
                        {feature}
                      </li>
                    ))}
                  </ul>
