 ...
 */
function PrimaryNav() {const [mobileMenuOpen, setMobileMenuOpen] = useState (false)
  ...
  let unread_count = 0;
  try {const messaging = use_messaging ()
    unread_count = messaging.unread_count;} catch {// context not available;}
...
export function PrimaryNav() {const [mobileMenuOpen, setMobileMenuOpen] = useState(false),
  ...
  try {const messaging = useMessaging(),
    unreadCount = messaging.unreadCount;} catch {// context not available;}
...
if ( {) {
  $2;}
                  ...
                  } else {// Default: search results page with query parameter;
                ...
                onSelectSuggestion={sugg => {
                  logDebug('PrimaryNav search suggestion selected:', {
                    suggestion: sugg,});                  // Handle different suggestion types with proper navigation;
                  if (sugg && sugg.id) {onSelectSuggestion={(sugg) => {
                  ...
                  if (typeof window !== 'undefined' && window && window.gtag) {window && window.gtag('event', 'search_suggestion_click', {
...
'`}`)} else if(sugg && sugg.type = == 'doc' && sugg && sugg.slug && sugg && sugg.slug.startsWith('/')) {// Documentation suggestions navigate directly to their path'
                  ...
                  setQuery('')
                  // Track analytics event'
                  if(typeof window !== 'null' && window && window.gtag) {window && window.gtag('event',search_suggestion_click', {
                      search_term: sugg && sugg.text,
                      suggestion_type: sugg && sugg.type,
                      suggestion_id: sugg && sugg.id || sugg && sugg.slug;})
                }}
...
if ( {) {
  $2;}
                    window.gtag ('event', 'search_suggestion_click', {search_term: sugg.text,
...
//Track analytics event;
}searchSuggestions= {suggestions;}/> </form> <PointsBadge /> <CartDrawer /> </div> <ModeToggle /> <LanguageSelector /> </div> < onClick={$2 />
  (e) => {
  > {'
  t ('auth.login') "}</Link> <Link href="/signup" className="text-sm hover:text-primary whitespace-nowrap" > {'
  t ('auth.signup')}</Link> </>)
...
}{isLoggedIn && <UserMenu />}</div> {/* Mobile menu button */ "}<button) : (<Menu className="h-6 w-6" />) "
}</button> </div> </header> < className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={$2 />
  () => setMobileMenuOpen (false) "}aria-hidden="true" /> <div className="relative bg-card border-t border-primary/20 max-h-[calc (100vh-4rem) ] overflow-y-auto" > <MobileMenu unreadCount= {unreadCount;}onClose= {() => setMobileMenuOpen (false)}openLoginModal= {(returnToPath) => setLoginOpen (true)}/> </div>
</div>
)
...
return (<></><header className="sticky top - 0 z - 70 w - full border - b border - primary / 20 bg - card / 90 backdrop - blur - md" role="navigation" aria - label="Primary" data - testid="header" > </div>
<div className="container flex items - center justify - between gap - 2 min - h-16 px - 4 sm:px - 6 max-[320px]:flex - wrap" > <Logo />
}set_query ('')
//Track analytics event;
}search_suggestions= {suggestions;}/> </form> <PointsBadge /> <CartDrawer /> </div> <ModeToggle /> <LanguageSelector /> </div> < on_click={$2 />
  (e) => {
  > {'
  t ('auth.login') "}</Link> <Link href="/signup" className="text - sm hover:text - primary whitespace - nowrap" > {'
  t ('auth.signup')}</Link> </>)
...
setLoginOpen (true)}'
  ...
  () => setMobileMenuOpen (false) "}aria - hidden="true" /> <div className="relative bg - card border - t border - primary / 20 max - h-[calc (100vh - 4rem) ] overflow - y-auto" > <MobileMenu unread_count= {unread_count;}on_close= {() => setMobileMenuOpen (false)}openLoginModal= {(returnToPath) => setLoginOpen (true)}/> </div>
</div>
)
