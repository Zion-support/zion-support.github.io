...
;
return (<React.Fragment> <header className="sticky top-0 z-70 w-full border-b border-primary/20 bg-card/90 backdrop-blur-md" role="navigation" aria-label="Primary" data-testid="header" > <div className="container flex items-center justify-between gap-2 min-h-16 px-4 sm:px-6 max-[320px]:flex-wrap" > <Logo />;"
}setQuery ('')
//Track analytics event
}searchSuggestions={";"}
...
t ('auth.signup')}</Link> </React.Fragment>)
}{";"}
isLoggedIn && <UserMenu />"}</div> </div> <ModeToggle /> <LanguageSelector /> {"}
...
isLoggedIn && <UserMenu />"}</div> {"/* Mobile menu button */ ""}<button) : (<Menu className="h-6 w-6" />) "
  ...
  () => setMobileMenuOpen (false) "}aria-hidden="true" /> <div className="relative bg-card border-t border-primary/20 max-h-[calc (100vh-4rem) ] overflow-y-auto" > <MobileMenu unreadCount={";"}
...
isLoggedIn && <UserMenu />"}</div> {"/* Mobile menu button */ ""}<button) : (<Menu className="h - 6 w - 6" />) "
  ...
  () => setMobileMenuOpen (false) "}aria - hidden="true" /> <div className="relative bg - card border - t border - primary / 20 max - h-[calc (100vh - 4rem) ] overflow - y-auto" > <MobileMenu unread_count={";"}
unread_count"}on_close={() => setMobileMenuOpen (false)}openLoginModal={(returnToPath) => setLoginOpen (true)}/> </div> </div>)"
}{;}
is_mobile && <MobileBottomNav unread_count={;}
unread_count}/>
}<LoginModal is_open={";"}
login_open"}onOpenChange={";"}
setLoginOpen"}/> </React.Fragment>)"
}'"  {"opacity: 0,;,"}
height: 0"}}"
              animate={{ opacity: 1}},;,
