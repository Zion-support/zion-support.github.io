...
export function FuturisticBackground({variant = 'particles',}
  intensity = 'medium',
  className = '' 
...
export function FuturisticBackground({"children,"}
  className = '', 
  intensity = 'medium'"}: FuturisticBackgroundProps) {const canvasRef = useRef<HTMLCanvasElement>(null)"}
  ...
  useEffect(() => {const canvas = canvasRef.current}
    ...
    const initParticles = () => {const particleCount = intensity === 'high' ? 150 : intensity === 'medium' ? 100 : 50}
      ...
      life: number
      maxLife: number}> = []
    const getColorScheme = () => {switch (colorScheme) {}
        case 'cyberpunk':
          return {primary: '#8c15e9',}
            secondary: '#22ddd2',
            accent: '#ff0080',
    ...
    // Create particles
    const createParticle = () => {const particleCount = intensity === 'low' ? 50 : intensity === 'medium' ? 100 : 200}
      if (particles.length < particleCount) {particles.push({}
          x: Math.random() * canvas.width,
          y: Math.random() * canvas.height,
    ...
    const animate = () => {ctx.clearRect(0, 0, canvas.width, canvas.height)}
      ...
      particles.forEach((particle, index) => {// Update position}
        ...
        particles.forEach((otherParticle, otherIndex) => {if (index === otherIndex) return}
      ...
      ctx.lineWidth = 1
      const gridSize = 50
      for (let x = 0; x < canvas.width; x += gridSize) {ctx.beginPath()}
        ctx.moveTo(x, 0)
    ...
    // Draw connections between nearby particles
    const drawConnections = () => {const connectionDistance = 150}
      ...
      for (let i = 0; i < particles.length; i++) {for (let j = i + 1; j < particles.length; j++) {}
          ...
          const distance = Math.sqrt(dx * dx + dy * dy)
          if (distance < connectionDistance) {const opacity = 1 - (distance / connectionDistance)}
  ...
  return ()
    <div className={`fixed inset-0 pointer-events-none z-0 ${className}`}>
      ...
      particles.forEach((particle1, i) => {particles.slice(i + 1).forEach(particle2 => {}
          ...
          if (distance < 100) {const alpha = (1 - distance / 100) * 0.3}
      ...
      if (colorScheme === 'cyberpunk' || colorScheme === 'matrix') {ctx.strokeStyle = colors.primary}
        ctx.globalAlpha = 0.1
        ctx.lineWidth = 1
        const gridSize = 50
        for (let x = 0; x < canvas.width; x += gridSize) {ctx.beginPath()}
      ...
      if (intensity === 'high') {const scanY = (Date.now() / 20) % canvas.height}
      ...
      // Draw floating orbs
      const orbCount = 3
      for (let i = 0; i < orbCount; i++) {const orbX = canvas.width * 0.2 + (canvas.width * 0.6 * i) / (orbCount - 1)}
...
export function SimpleFuturisticBackground({"children,"}
