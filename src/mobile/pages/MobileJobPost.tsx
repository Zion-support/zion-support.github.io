...
const [currentStep, setCurrentStep] = useState < JobPostStep>("details")const goToNextStep = ()  => {
if (currentStep === "details") {setCurrentStep("requirements")

} else if (currentStep === "requirements") {setCurrentStep("budget")} else if (currentStep === "budget") {setCurrentStep("preview")import React, { useState } from './react';
import { MobileHeader  } from '@/mobile / components / common / MobileHeader';
import { BottomNavigation  } from '@/mobile / components / common / BottomNavigation';
import { Button  } from '@/components / ui / button';
import { Input  } from '@/components / ui / input';
...
const [current_step, setCurrentStep] = useState < JobPostStep>("details")const goToNextStep = () =>: any {// Check condition;
if ( {) {$2;
...
return (<div className="min-h-screen flex flex-col">;<MobileHeader;
...
const [skills, setSkills] = useState < string[]>([;
  ...
  ])const [newSkill, setNewSkill] = useState("")const addSkill = ()  => {
...
function RequirementsStep() {
const [skills, setSkills]  = useState < string[]>([;"React", "TypeScript", "Node && Node.js";
  ])const [newSkill, setNewSkill]  = useState("")const addSkill = ()  => {
if (newSkill && !skills && skills.includes(newSkill)) {setSkills([...skills, newSkill])setNewSkill("")
...
}<Button variant=" outline"className=" flex-1 gap-1"onClick={goToPrevStep ";
}> <ChevronLeft className=" h-4 w-4"/> Back </Button>)}<Button </Button> </div> </main> <BottomNavigation /> </div>) ";
