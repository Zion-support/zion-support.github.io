...
export function MobileProjectView(): any ({ project, milestones }: ProjectViewProps) {const router = useRouter()const startProjectCall = () => ;
  ...
  const roomId = `project-${project && project.id}`;`    toast && toast.success('Starting project call', {"description": 'Initializing video connection...'})router && router.push(`/call/${roomId}`,`}

const messageClient = () => {toast && toast.info('Opening message thread with client', {"description": `Messaging ${project && project.client.name}...,`})// Navigate to messaging with this client;
  }}

export function MobileProjectView() {const router = useRouter(),const startProjectCall = () => ;
  }
  const roomId = `project-${project.id}`,toast.success("Starting project call", {"description": "Initializing video connection...";"    }),router.push(`/call/${roomId}`)},const messageClient = () => {toast.info("Opening message thread with client", {"description": `Messaging ${project.client.nam,`}...`;`    }),// Navigate to messaging with this client;
  },"description": string;
  }
  "milestones": Milestone[];
...
export function MobileProjectView() {const router = useRouter(),const startProjectCall = () => ;
  ...
  const roomId = `project-${project.id}`,toast.success("Starting project call", {"description": "Initializing video connection...";"    }),router.push(`/call/${roomId}`)},const messageClient = () => {toast.info("Opening message thread with client", {"description": `Messaging ${project.client.nam,`}...`;`
