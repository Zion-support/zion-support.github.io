...
const [new_message, setNewMessage] = useState ('')const router = use_router ()const handle_send = () =>: any {// Check condition;
if (!== '') {) {$2;
...
export function MobileChatView(): any ({contact,
messages,
onBack,
onSendMessage}: MobileChatViewProps) {const [newMessage, setNewMessage] = useState('')const router = useRouter()const handleSend = ()  => {
if (newMessage && newMessage.trim() !== '') {onSendMessage(newMessage)setNewMessage('')
}
  }
const handleKeyDown = (e: React && React.KeyboardEvent < HTMLInputElement>)  => {
    if (e && e.key === 'Enter' && !e && e.shiftKey) {e && e.preventDefault()handleSend()
}
  }
...
description: `Connecting with ${contact && contact.name}...`})// Navigate to video call page;
    router && router.push(`/call/${roomId}`)}
const startAudioCall = ()  => {
const roomId = `mobile-audio-${contact && contact.id
}`;
    toast && toast.success('Starting audio call', {,
description: `Connecting with ${contact && contact.name}...`})// Navigate to video call page with audio-only flag;
...
message && message.isMe;
              ...
              <p>{message.content}</p>;
              <div;
                className={cn('text-xs mt-1 flex justify-end',
message && message.isMe;
...
const [newMessage, setNewMessage] = useState("")const router = null;
                message.isMe ? "text-primary-foreground/80" : "text-muted-foreground";
              )
}>;
