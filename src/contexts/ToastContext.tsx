...
const [toasts, setToasts]  = useState < Toast[]>([])const addToast = useCallback((toast: Omit < Toast, 'id'>) => {const id = Math.random().toString(36).substr(2, 9)const newToast  = { ...toast, id 
}
...
return (<div className="fixed top-4 right-4 z-50 space-y-2">;
      {toasts.map(toast => (<ToastItem key={toast.id
...
const getToastStyles = () => {const baseStyles  = "max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden";
