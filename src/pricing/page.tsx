...
import SEOOptimizer  from "../components/SEOOptimizer";import { CheckCircle, Phone, Mail }  from "lucide-react";'use client'
constPricingPage: React.FC= () =>{constpricingPlans= [
  ...
  return(<><SEOOptimizertitle ="Pricing Plans - ZionTechGroup"
        ...
        keywords={
  ['pricing', 'AI services pricing', 'IT services pricing', 'business plans', 'enterprisepricing']
}
  ...
  plan.popular && (
                ...
                <divclassName="text-centermb-8"><h3className="text-2 xl font-boldtext-whitemb-2">{plan.name}</h><pclassName="text-gray-300mb-4">{plan.description}</p><divclassName="flexitems-baselinejustify-center"><spanclassName="text-5 xl font-boldtext-cyan-400">{plan.price}</spa><spanclassName="text-gray-400ml-2">{plan.period}</spa></di></di><ulclassName="space-y-4mb-8">{plan.features.map((featurefeatureIndex) => (
                   <likey={featureIndex}className="flex items-centertext-gray-300"><CheckCircleclassName="w-5 h-5text-green-400mr-3flex-shrink-0" />{
  feature
}
                    </l>))}
                </u><ahref ={
  plan.popular ?"mailto:kleber@ziontechgroup.com" :"tel:+13024640950"
}
