...
// Mock components
AdvancedErrorBoundary = ({ children }: { children: React.ReactNode }) => {
  return <div data-testid="error-boundary">{children}</div>}
AdvancedSEOOptimizer = ({ title, description }: { title?: string; description?: string }) => {
...
describe('Advanced Components', () => {
  ...
  it('retries when retry button is clicked', async () => {
    ...
    TestComponent = () => <ThrowError shouldThrow={shouldThrow} />
    render(
      <MemoryRouter>
        <AdvancedErrorBoundary enableRetry={true}>
          <TestComponent />
    ...
    retryButton = screen.getByText('Try Again (3 attempts left)')
