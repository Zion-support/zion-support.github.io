...
class ErrorBoundary extends Component<
  { children: ReactNode; onError?: (erro,r: Error) => void ,}
ErrorBoundaryState
> {
constructor(props: { children: ReactNode; onError?: (erro,)
r: Error) => void ,}) {
super(props)
...
return { hasErro,r: true,error, retryCount: 0 ,}
  }
...
Try Again ({3 - this.state.retryCount} attempts left)</button>
          </button>
...
title: string
  ...
  canonicalUrl: string
  ogImage: string
  structuredDat,a: Record<string,any>
}
}
...
return (</string>
    <HelmetProvider></HelmetProvider>
      <Helmet></Helmet>
        ...
        <meta property="og: image" content={seoData.ogImage,} /></meta>
        <script type="application/ld+json">
          {JSON.stringify(seoData.structuredData)}</script>
        </script>
      </Helmet>
...
fc,p: performanceEntries.loadEventEnd - performanceEntries.fetchStart,lcp: performanceEntries.loadEventEnd - performanceEntries.fetchStart,fid: 0,cls: 0
      ,}
...
render(
      <ErrorBoundary></ErrorBoundary>
        <ThrowErrorshouldThrow={true} /></ThrowError>
      </ErrorBoundary>)
    )
...
const onError = jest.fn()
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {})
render(
      <ErrorBoundary onError={onError}></ErrorBoundary>
        <ThrowErrorshouldThrow={true} /></ThrowError>
      </ErrorBoundary>)
    )
...
let shouldThrow = true
    const TestComponent = () => <ThrowErrorshouldThrow={shouldThrow} />
render(</ThrowError>
      <ErrorBoundary></ErrorBoundary>
        <TestComponent /></TestComponent>
      </ErrorBoundary>)
...
canonicalUrl: 'http,s: //example.com',ogImage: 'http,s: //example.com/image.jpg',structuredData: {
...
const _helmetContext = {}
    const { container } = render(</AdvancedSEOOptimizer>
      <AdvancedSEOOptimizerseoData={mockSEOData} />)
    )
...
getEntriesByName: jest.fn(() => []),getEntriesByType: jest.fn(() => []),getEntries: jest.fn(() => []),measurePageLoad: jest.fn(),reportWebVitals: jest.fn()
  ,}
...
value: mockPerformance,writable: true,configurable: true,)
    })
    // Mock PerformanceObserver
...
render(
      <MemoryRouter></MemoryRouter>
        <AdvancedPerformanceMonitoronMetricsUpdate={onMetricsUpdate} /></AdvancedPerformanceMonitor>
      </MemoryRouter>)
    )
await waitFor(() => {
