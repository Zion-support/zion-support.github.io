...
const { describe,it,expect } from "vitest"; describe("Login",() =>
                { it("should work",() =>
                { expect(true).toBe(true)})});''"
import { describe,it,expect } from 'vitest'; describe('Login',() =>
                { it('should work',() =>
                { expect(true).toBe(true)})}
});''
});''
...
describe('/api/auth/login API Handler', () =>
  ...
  it('should return 405 if method is not POST', async () =>;
                {'    const req = mockApiReq({}, GET');    const res = mockApiRes()
    await loginHandler(req, res)
    expect(res.status).toHaveBeenCalledWith(405)
    expect(res.end).toHaveBeenCalled()
  })
  it('should successfully log in a verified user and set authToken cookie', async () =>
                {'    const testEmail = verified@example.com';    const testPassword = password123';    const mockAuthToken = mock-access-token';    const mockSessionData = {
  ...
  it('should return 403 for login with unconfirmed email', async () =>
                {'    const testEmail = unconfirmed@example.com';    const testPassword = password123'
    mockSignInWithPassword.mockResolvedValueOnce({
  ...
  it('should return 401 for invalid credentials', async () =>
                {'    const testEmail = wrong@example.com';    const testPassword = wrongpassword'
    mockSignInWithPassword.mockResolvedValueOnce({
      data: { session: null, user: null },
  ...
  it('should return 401 for AuthApiError with status 400', async () =>
                {'    const testEmail = authapierror@example.com';    const testPassword = password123'
    mockSignInWithPassword.mockResolvedValueOnce({
      data: null,
  ...
  it('should return 500 for generic error with status', async () =>
                {'    const testEmail = servererror@example.com';    const testPassword = password123'
    mockSignInWithPassword.mockResolvedValueOnce({
      data: null,
  ...
  it('should return 500 for generic error without status (defaults to 500), async () =>
                {'    const testEmail = genericerror@example.com';    const testPassword = password123'
    mockSignInWithPassword.mockResolvedValueOnce({
      data: null,
  ...
  it('should also return 401 if Supabase returns no error but no session (fallback), async () =>
                {'    const testEmail = noerrornosession@example.com';    const testPassword = password123'
    mockSignInWithPassword.mockResolvedValueOnce({
...
describe('loginUser Service', () =>
  ...
  it('should handle "Invalid credentials" (401) from API', async () =>
                {'    const mockErrorResponse = { error: Invalid credentials', code: INVALID_CREDENTIALS' };    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 401,
      json: async () => mockErrorResponse,
      headers: new Headers()
  ...
  it('should handle other errors (e.g., 500) from API', async () =>
                {'    const mockErrorResponse = { error: Server error', code: LOGIN_FAILED' };    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 500,
      json: async () => mockErrorResponse,
      headers: new Headers()
  ...
  it('should handle network errors during fetch', async () =>
                {'    global.fetch = vi.fn().mockRejectedValue(new Error('Network failed'));    try {
