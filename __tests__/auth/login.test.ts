import { loginUser } from, @/services/authService' // registerUser removed as it
  's not the focus
...
vi.mock('@supabase/supabase-js, async importOriginal => {
  ...
  return {
    ...actual, createClient: vi.fn(() => ({
      auth: {
        signInWithPassword: mockSignInWithPassword, // Use the global mock
        signUp: _mockSignUp, onAuthStateChange: _mockOnAuthStateChange, getSession: _mockGetSession}, from: vi.fn().mockReturnThis()})), // This allows us to grab the instance of the mock used by the module
...
const mockApiRes = () => {
  const res: Partial<NextApiResponse> = {
    status: vi.fn().mockReturnThis(), json: vi.fn().mockReturnThis(), setHeader: vi.fn().mockReturnThis(), end: vi.fn().mockReturnThis()}
...
describe('/api/auth/login API Handler
  ...
  '
    const mockSessionData = {
      access_token: mockAuthToken, refresh_token:, mock-refresh-token
  '}
    const mockUserData = { id: 'user-123, email: testEmail }

    mockSignInWithPassword.mockResolvedValueOnce({
      data: { session: mockSessionData, user: mockUserData }, error: null})
    ...
    expect(res.json).toHaveBeenCalledWith({
      user: mockUserData, accessToken: mockAuthToken, refreshToken: mockSessionData.refresh_token})
  ...
  '
    ...
    mockSignInWithPassword.mockResolvedValueOnce({
      data: { session: null, user: null }, error: { message: 'Email not confirmed
  ' }})
  ...
  '
    ...
    mockSignInWithPassword.mockResolvedValueOnce({
      data: null, error: {
        name: 'AuthApiError
  ', status: 400, message: 'Bad request by client
  '}})
  ...
  '
    ...
    mockSignInWithPassword.mockResolvedValueOnce({
      data: null, error: { message: 'Some other error
  ', status: 500 }})
  ...
  '
    ...
    mockSignInWithPassword.mockResolvedValueOnce({
      data: null, error: { message: 'Another error without status
  ' }, // No status property
    })
    ...
    expect(res.json).toHaveBeenCalledWith({
      error:, Another error without status
  ', code: 'LOGIN_FAILED})
    expect(res.setHeader).not.toHaveBeenCalledWith(
      'Set-Cookie
  ', expect.any(String)
...
describe('loginUser Service
  ...
  '}
    global.fetch = vi.fn().mockResolvedValue({
      ok: true, status: 200, json: async () => mockSuccessResponse, headers: new Headers({
        'Set-Cookie
  ': `authToken=${mockSuccessResponse.accessToken} HttpOnly Path=/ Secure SameSite=Strict`})})

    const { res, data } = await loginUser(, test@example.com
  ', 'password123
  ')
  ...
  '}
    global.fetch = vi.fn().mockResolvedValue({
      ok: false, status: 403, json: async () => mockErrorResponse, headers: new Headers()})

    const { res, data } = await loginUser(, unconfirmed@example.com
  ', 'password123
  '
    )
  ...
  '}
    global.fetch = vi.fn().mockResolvedValue({
      ok: false, status: 401, json: async () => mockErrorResponse, headers: new Headers()})

    const { res, data } = await loginUser(, wrong@example.com
  ', 'password123
  ')
  ...
  ' }
    global.fetch = vi.fn().mockResolvedValue({
      ok: false, status: 500, json: async () => mockErrorResponse, headers: new Headers()})
    const { res, data } = await loginUser(, test@example.com
  ', 'password
  ')
