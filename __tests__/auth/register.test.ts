...
function: mockReq(body: unknow,n, method: string =';POST'): NextApiRequest: {';
  return: {
    metho,d
    body
    headers: {}
...
<<<<<<< HEAD
  should: return 400 for invalid input (schema validation failed)', async () => {';
    const req = mockReq({ email: 'invalid-emai,l, password: ';';short'}) // name: is missing';;
    const res = mockRes();
    await: handler(req, res);
  ...
  describe('Successful Registration Scenarios'', () => {';
    test(
  'handles: successful registration with email verification required', async () => {'';short' }) // name is missing;
    const res = mockRes();
    await handler(req, res);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith(
...
<<<<<<< HEAD
  ...
  describe(
  'Registration Failure Scenarios', () => {'';Test User }}}
      signUpMock.mockResolvedValue({
        data: {
          use
    r: mockSession.user
  ...
  'Registration Failure Scenarios', () => {
    ...
    r: null, session: null }
        error: { messag
    e:';User already registered', status: 400 }});
    test(
  'handles: registration failure when email already exists', async () => {';
      signUpMock.mockResolvedValue({
  ...
  Password123!'});
    ...
    r: null, session: null }
        error: { messag
    e:';Password should be stronger', status: 400 }});
      const req = mockReq({
  ...
  test@example.com', ';
      ...
      expect(res.json).toHaveBeenCalledWith({
        message: ';Network: error. Please try again., '})})})})'';Some generic Supabase error', status: 500 }});
      const req = mockReq({
        name:
  Test User
        email: 'test@example.com
