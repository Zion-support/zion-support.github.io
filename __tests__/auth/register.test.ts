...
describe('/api/auth/register'', () => {
  ...
  'should return 405 if method is not POST', async () => {
    ...
    const req = {
      method: 'GET, body: undefined, headers: {}, query: {}, cookies: {}} as NextApiRequest
    const res = mockRes()
      ...
      signUpMock.mockResolvedValue({
        data: {
          user: {
            id:
  'user-id-123, email:
  'test@example.com', identities: [], user_metadata: { display_name:
  'Test User' }}, session: null}, error: null})
      ...
      expect(res.json).toHaveBeenCalledWith({
        message: 'Registration successful. Please check your email to verify your account., emailVerificationRequired: true, user: {
          email:, test@example.com', id: 'user-id-123, display_name:, Test User'}})
  ...
  'handles successful registration with auto-confirmation (email already verified)', async () => {
      const mockSession = {
        access_token: 'mock-access-token, refresh_token:, mock-refresh-token', user: {
          id:
  'user-id-123, email:, test@example.com', user_metadata: { display_name:
  'Test User }}}
      signUpMock.mockResolvedValue({
        data: {
          user: mockSession.user, session: mockSession}, error: null})
  ...
  'Password123!'})
      ...
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({
        user: mockSession.user, session: mockSession})
  ...
  'handles registration failure when email already exists', async () => {
      signUpMock.mockResolvedValue({
        data: { user: null, session: null }, error: { message:
