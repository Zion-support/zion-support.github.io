...
jest.mock('@/services/authService');jest.mock('@/hooks/useAuthOperations');jest.mock('@/utils/safeStorage', () => ({'  safeStorage: { setItem: jest.fn(), getItem: jest.fn(), removeItem: jest.fn() },
    ...
    render(
      <AuthProvider>
        <AuthContext.Consumer>
          {({ login }) => <button onClick={() => login('a@b.c', pw')}>LoginBtn</button>}        </AuthContext.Consumer>
      </AuthProvider>
    );
