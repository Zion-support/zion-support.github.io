...
const mockFetchImplementation = (
  responseData: UserProfile | { success: boolean } | Partial<UserProfile>, ok = true, status = 200
) =>
  vi.fn().mockResolvedValue({
    ok, status, json: () => Promise.resolve(responseData), text: () => Promise.resolve(JSON.stringify(responseData)), // Add text() method for completeness
  })
...
describe('Profile Page
  ...
  ', async () => {
    const mockProfileData: UserProfile = {
      id: '1, name:, Jane
  ', email: 'jane@example.com, avatarUrl: '', notifications: { email: true, push: false }}
  ...
  'saves updated profile', async () => {
    const initialProfileData: UserProfile = {
      id:
  '1, name:, Jane', email: 'jane@example.com, avatarUrl: '
  ', notifications: { email: true, push: false }}
    const updatedProfileData: UserProfile = {
      ...initialProfileData, name: 'New Name
  '}
  ...
  ') // Ensure initial data is loaded
    ...
    (global.fetch as vi.Mock).mockResolvedValueOnce({
      ok: true, status: 200, json: () => Promise.resolve(updatedProfileData), // This is the response for the PUT
    })
  ...
  ': 'application/json }, body: JSON.stringify({ name:, New Name
  ' }), // Only 'name
