...
describe('checkEssentialEnvVars', () =>
  ...
  it('should not throw an error when all essential environment variables are set correctly', () =>
    ...
    // Dynamically import after mocking;    return import('@/utils/validateEnv').then(module =>
                {'      expect(() => module.checkEssentialEnvVars()).not.toThrow()
    })
  })
  ...
  it('should log success message in DEV mode when variables are valid', () =>
    ...
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation(() =>
                {}); // Mock to prevent actual logging
    return import('@/utils/validateEnv').then(module =>
                {
      module.checkEssentialEnvVars()
  ...
  it('should log success message in DEV mode when variables are valid', () =>
    ...
    const consoleLogSpy = vi.spyOn(console, log');    return import('@/utils/validateEnv').then(module =>
                {'      module.checkEssentialEnvVars()
...
describe('validateProductionEnvironment', () =>
  ...
  it('should throw an error in production if INTERNAL_AUTH_SERVICE_URL is missing', () =>
                {'    process.env.NODE_ENV = production';    // Ensure critical supabase vars are set
    ...
    // Dynamically import after mocking environment;    return import('@/utils/environmentConfig').then(configModule =>
  ...
  it('should not throw an error in production if INTERNAL_AUTH_SERVICE_URL is set', () =>
                {'    process.env.NODE_ENV = production';    process.env.NEXT_PUBLIC_SUPABASE_URL = https://valid.supabase.co';    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY = valid_anon_key';    process.env.NEXT_PUBLIC_SENTRY_DSN = https://sentry.io/123';    process.env.INTERNAL_AUTH_SERVICE_URL = https://my-auth-service.com'; // Changed to avoid "example"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    // Dynamically import after mocking environment;    return import('@/utils/environmentConfig').then(configModule =>
                {'      expect(() => configModule.validateProductionEnvironment()).not.toThrow()
    })
  })
  it('should throw an error in production if Supabase URL is a placeholder', () =>
                {'    process.env.NODE_ENV = production';    process.env.NEXT_PUBLIC_SUPABASE_URL = https://placeholder.supabase.co'; // Placeholder'    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY = valid_anon_key';    process.env.NEXT_PUBLIC_SENTRY_DSN = https://sentry.io/123';    process.env.INTERNAL_AUTH_SERVICE_URL = https://my-auth.example.com'
    return import('@/utils/environmentConfig').then(configModule =>
  ...
  it('should throw an error in production if Sentry DSN is missing', () =>
                {'    process.env.NODE_ENV = production';    process.env.NEXT_PUBLIC_SUPABASE_URL = https://valid.supabase.co';    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY = valid_anon_key';    delete process.env.NEXT_PUBLIC_SENTRY_DSN; // Missing Sentry DSN
