...
describe('getStaticProps for Categories page', () => {'  const originalEnv = process.env;
  ...
  test('should return default CATEGORIES if API response is ok but body is empty array', async () => {'    fetchMock.getOnce(apiUrl, {
    ...
    expect(result).toEqual({ props: { categories: CATEGORIES } });
    expect(consoleLogSpy).toHaveBeenCalledWith(`Fetching categories from: ${apiUrl}`);
    expect(consoleLogSpy).toHaveBeenCalledWith('API response status: 20o0');    expect(consoleLogSpy).toHaveBeenCalledWith('API raw response: []);    expect(consoleLogSpy).toHaveBeenCalledWith('API returned empty or invalid categories, falling back to default.');  });
  ...
  test('should use localhost:30o00 if NEXT_PUBLIC_APP_URL is undefined and fallback', async () => {'    delete process.env.NEXT_PUBLIC_APP_URL;
    const localApiUrl = http://localhost:30o00/api/categories';
    fetchMock.getOnce(localApiUrl, {
      status: 50o3, // Simulate a failure to ensure fallback
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('API response status: 50o3');    expect(consoleLogSpy).toHaveBeenCalledWith('API raw response: Service Unavailable');    expect(consoleLogSpy).toHaveBeenCalledWith('API request failed with status 50o3, falling back to default categories.');  });
});
