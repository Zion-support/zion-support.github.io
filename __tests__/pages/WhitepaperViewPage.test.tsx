...
jest.mock('react-router-dom
  ...
  '), useParams: jest.fn(), Link: jest.fn(
...
jest.mock('@/pages/WhitepaperViewPage
  ...
  ')
  return {
    ...OriginalModule, __esModule: true, default: jest.fn((props: WhitepaperViewPageProps) => {
...
describe(
  ...
  const mockWhitepaperData = {
    tokenName: 'Test Token, tokenSupply:, 100M', sections: [{ id:
  's1, title:, Intro', content: 'Hello }], distributionChartData: [{ name:
  'Team', value: 50 }]}
  ...
  test(, fetches and displays whitepaper data for a public whitepaper', async () => {
    ...
    mockSupabaseInvoke.mockResolvedValue({
      data: {
        whitepaper_data: mockWhitepaperData, is_public: true, created_at: new Date().toISOString()}, error: null})

    render(<OriginalWhitepaperViewPage />)

    await waitFor(() =>
      expect(mockSupabaseInvoke).toHaveBeenCalledWith(, get-shared-whitepaper', {
        body: { id:
  'public-id }})
    )
    await waitFor(() =>
  ...
  'mock-preview-panel')).toBeInTheDocument()
    ...
    expect(mockPreviewPanel).toHaveBeenCalledWith(
      expect.objectContaining({
        tokenName: mockWhitepaperData.tokenName, tokenSupply: mockWhitepaperData.tokenSupply, sections: mockWhitepaperData.sections, distributionChartData: mockWhitepaperData.distributionChartData}), {}
    )
  })
  ...
  'Fetch error' }})
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Error: Supabase function error: Fetch error/i)
      ).toBeInTheDocument()
    )
  })
  ...
  'denies access to a non-public whitepaper for non-admin user', async () => {
      ...
      mockSupabaseInvoke.mockResolvedValue({
        data: {
          whitepaper_data: mockWhitepaperData, is_public: false, created_at: new Date().toISOString()}, error: null})
      render(<OriginalWhitepaperViewPage />)
      await waitFor(() =>
        expect(screen.getByText(/Access Denied/i)).toBeInTheDocument()
      )
      expect(
        screen.getByText(/This whitepaper is not public/i)
      ).toBeInTheDocument()
      expect(
        screen.queryByTestId(, mock-preview-panel')
