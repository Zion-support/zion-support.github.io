...
const mockFetch = <T,>(data: T, ok = true, status = 200) => {
  ...
  global.fetch = vi.fn().mockResolvedValue({
    ok, status, json: () => Promise.resolve(data), text: () =>
...
describe('TalentPage'', () => {
  ...
  'should fetch and display talent profile successfully', async () => {
    ...
    (useRouter as jest.Mock).mockReturnValue({
      query: { id: mockProfile.id }, isReady: true})
  ...
  't-001 }, isReady: true})
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Failed to load talent profile./i)
      ).toBeInTheDocument()
    })
