import React from react',import { render, screen, waitFor } from @testing-library/react',import userEvent from @testing-library/user-event',import Login from @/src/pages/auth/login', // Assuming Login page is now at pages/auth/login.tsx'import { useRouter } from next/router',// useAuth hook is mocked below - no import needed;
import { safeStorage } from @/utils/safeStorage', // To be mocked'import * as Sentry from @sentry/nextjs', // To be mocked'import { fireEvent as fireAnalyticsEvent } from @/lib/analytics', // To be mocked'import @testing-library/jest-dom';
// Mock next/router,
...
describe('Login Page', () => {'  let _mockRouterReplace: jest.Mock;
  ...
  beforeEach(() => {,
    ...
    (useRouter as jest.Mock).mockImplementation(() => ({,
      ...
      push: mockRouterPush;
      asPath: /auth/login', // Default asPath'      pathname: /auth/login',      query: {};
      isReady: true;
      events: {,
        on: jest.fn();
        off: jest.fn(),}
  ...
  test('login button has correct styling classes', () => {'    render(<Login />);
    ...
    const loginButton = screen.getByRole('button', { name: /login/i ,}),    expect(loginButton).toHaveClass('inline-flex'),    expect(loginButton).toHaveClass('px-4'),    expect(loginButton).toHaveClass('py-2'),    expect(loginButton).toHaveClass('rounded-md'),    // From previous subtask, the full class list for the button included: // "w-full inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-gradient-to-r from-zion-purple to-zion-purple-dark hover:from-zion-purple-light hover:to-zion-purple focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zion-purple-light""    expect(loginButton).toHaveClass('from-zion-purple'), // Existing gradient'    expect(loginButton).toHaveClass('shadow-sm'),    expect(loginButton).toHaveClass('font-medium'),    expect(loginButton).toHaveClass('text-white'),    // Add other specific classes if necessary, but this covers the new additions and some core ones.,
  });
  test('successful login updates auth state and redirects to dashboard', async () => {'    // Initial auth state: not authenticated,
    ...
    // Let's assume the redirect comes from a useEffect in the Login page component itself.'    // If not, this part of the test needs adjustment.,
    // Based on AuthProvider logic, redirection happens within AuthProvider's onAuthStateChange.'    // So, if Login page relies purely on useAuth().isAuthenticated, it might not directly redirect.,
    ...
    // It might just stop rendering the form, or render a "redirecting..." message."    // Let's test for router.replace, assuming the Login page *does* have a useEffect for this.'    // If this fails, it means the redirect logic is not in Login.jsx.,
    await waitFor(() => {,
  ...
  test('failed login with invalid credentials shows specific error message and calls Sentry', async () => {'    // This is the error object that AuthProvider's login method would return to the form'    mockLogin.mockResolvedValue({ error: Invalid email or password.' ,});
    ...
    // AuthProvider's login catch block calls Sentry.captureException.'    // If mockLogin itself is the function from AuthProvider, then this test is implicitly,
    // testing that AuthProvider's wrapper around the actual API call correctly'    // processes the error and it's passed to the form.'    // The original test checked Sentry.captureException. Let's assume an error'    // that AuthProvider would report to Sentry.,
    // If mockLogin *is* the login function from useAuth (which comes from AuthProvider);
    // and it internally calls Sentry.captureException for the error it's returning,    // then we could check Sentry.captureException.,
    // However, the { error: message' ,} pattern is usually for *handled* errors shown to user.'    // Sentry is more for *unhandled* or unexpected errors.,
