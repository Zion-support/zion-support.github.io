...
describe('ResetPassword Page
  ...
  test(, calls resetPassword service and navigates on successful submission (mocked)
  ', async () => {
  ...
  ', { name: /reset password/i }))
    ...
    // await waitFor(() => {
    //   expect(authService.resetPassword).toHaveBeenCalledWith(, testuid
  ''testtoken
  ''newpassword123
  ')
    // })
  ...
  '))
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/password reset successfully \(mocked\)/i)
      ).toBeInTheDocument()
    })
  })

  test(, shows error message on failed submission (mocked)
  ', async () => {
