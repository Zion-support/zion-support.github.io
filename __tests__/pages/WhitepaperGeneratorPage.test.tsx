...
jest.mock(, html2canvas', () =>
  jest.fn(() =>
    Promise.resolve({
      toDataURL: () =>
  'mockImageDataUri, width: 200, // Example width
...
jest.mock(
  ...
  return jest.fn(
    (): MockJsPDF => ({
      ...
      addImage: mockAddImage, save: mockSave, addPage: mockAddPage, internal: { pageSize: { getWidth: () => 210, getHeight: () => 297 } }, getImageProperties: () => ({ width: 200, height: 100 })})
  )
})
...
describe('WhitepaperGeneratorPage'', () => {
  ...
  'button', { name: /Share/i })).toBeInTheDocument()
    expect(
      screen.queryByRole(
  'button', { name: /Submit to Counsel/i })
  ...
  '## Section 1\nContent 1\n## Section 2\nContent 2'
      mockSupabaseInvoke.mockResolvedValueOnce({
        data: { whitepaperDraft: mockGeneratedDraft }, error: null})
      render(<WhitepaperGeneratorPage />)
  ...
  'generate-whitepaper', {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId(
  'mock-section-editor-section-1')
        ).toBeInTheDocument()
        expect(
          screen.getByTestId(
  'mock-section-editor-section-2')
        ).toBeInTheDocument()
      })
      expect(
        screen.getByRole(
  ...
  'button', { name: /Generate Whitepaper Draft/i })
      ...
      await waitFor(() =>
        expect(
          screen.getByText(/Supabase function error: Generation failed/i)
        ).toBeInTheDocument()
      )
    })
  })
  ...
  'mockImageDataUri''PNG', 0, 0, expect.any(Number), expect.any(Number)
        )
      )
      await waitFor(() =>
  ...
  'test-share-id'
      ...
      mockSupabaseInvoke.mockResolvedValueOnce({
        data: { id: shareId, is_public: initialPublicStatus }, error: null})
  ...
  'button', { name: /Copy/i })).toBeInTheDocument()
      expect(
        screen.getByRole(
  'button', {
          name: initialPublicStatus ? /Make Private/i : /Make Public/i})
      ).toBeInTheDocument()
  ...
  'toggle-id'
      mockSupabaseInvoke.mockResolvedValueOnce({
        data: { id: shareId, is_public: false }, error: null}) // Initial create
  ...
  'button', { name: /Make Public/i })
      ...
      mockSupabaseInvoke.mockResolvedValueOnce({
        data: { id: shareId, is_public: true }, error: null}) // For set to public
  ...
  'set-shared-whitepaper-public-status', {
      ...
      await waitFor(() =>
        expect(
          screen.getByRole(
  'button', { name: /Make Private/i })
        ).toBeInTheDocument()
      )
      expect(screen.getByText(/Currently: Public/i)).toBeInTheDocument()

      mockSupabaseInvoke.mockResolvedValueOnce({
        data: { id: shareId, is_public: false }, error: null}) // For set to private
  ...
  'counsel-share-id'
      ...
      mockSupabaseInvoke
        .mockResolvedValueOnce({
          data: { id: newShareId, is_public: false }, error: null})
        .mockResolvedValueOnce({
          data: { id: newShareId, is_public: true }, error: null})
        .mockResolvedValueOnce({
          data: { message:
  'Notification sent' }, error: null})
      ...
      await waitFor(() =>
        expect(mockSupabaseInvoke).toHaveBeenCalledWith(
  'create-shared-whitepaper', expect.any(Object)
        )
      )
      await waitFor(() =>
        expect(mockSupabaseInvoke).toHaveBeenCalledWith(
  ...
  'notify-legal-team', {
          body: expect.objectContaining({
            whitepaperId: newShareId, tokenName:
  'My Test Token'})})
      )
  ...
  'existing-link-id'
      ...
      mockSupabaseInvoke.mockResolvedValueOnce({
        data: { id: existingShareId, is_public: true }, error: null})
      fireEvent.click(screen.getByRole(
  'button', { name: /Share/i }))
      await waitFor(() =>
        expect(
  ...
  'notify-legal-team', {
          body: expect.objectContaining({
            whitepaperId: existingShareId, tokenName:
  'My Test Token'})})
      )
  ...
  'set-shared-whitepaper-public-status'
      ...
      ).toBe(0) // Not called again for this specific submit
    })
  })
})
