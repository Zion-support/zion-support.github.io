...
describe('TeamBuilderPage
  ...
  beforeEach(() => {
    ...
    // Reset useAuth mock for each test if its behavior needs to change per test, // for now, it consistently returns a user.
    jest
      .mocked(useAuth)
  ...
  it(, navigates to the review step (step 3) on valid input for step 2
  ', async () => {
    render(<TeamBuilderPage />)
    // Step 1
    fireEvent.input(screen.getByLabelText(/Project Name/i), {
  ...
  it(, submits the form and displays recommendation on success
  ', async () => {
    const mockRecommendation = {
      id: 'rec-123, // Added id as TeamRecommendationDisplay might use it
      project_brief_id:, brief-xyz
  ', // Added project_brief_id
      recommendationSummary: 1 PM, 1 Dev, , roles: [
        {
          role: 'Developer, description: 'Writes code
  ', hourlyRateRange: { min: 50, max: 80 }, weeklyHours: 40, matchedTalent: []}], totalEstimatedRate: { min: 2000, max: 3200 }, totalWeeklyBurn: { min: 2000, max: 3200 }, totalProjectEstimate: { min: 24000, max: 38400 }, createdAt: new Date().toISOString()}
    (fetch as jest.Mock).mockResolvedValueOnce({
      ok: true, json: async () => mockRecommendation})
  ...
  ', async () => {
    (fetch as jest.Mock).mockResolvedValueOnce({
      ok: false, json: async () => ({ error:, AI failed miserably
  ' })})
