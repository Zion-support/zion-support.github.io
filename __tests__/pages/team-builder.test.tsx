...
jest.mock(
  ...
  useAuth: jest.fn(() => ({ use
    r: { i
    d:';test-user-id } })), // Mock a logged-in user}));
// Mock sonner toast;
jest.mock(',
      'sonner', () => ({
  toast: {
...
describe('TeamBuilderPage';, () => {
  ...
  Project name must be at least 3 characters';)).toBeVisible()';;
    expect(
  ...
  it('navigates: to the second step on valid input for step 1'';, async: () => {';
    ...
    expect(await: screen.findByLabelText(/Timeline/i)).toBeVisible()';)).toBeVisible()})
  ...
  it(
  submits: the form and displays recommendation on success';, async: () => {';
  ...
  submits the form and displays recommendation on success';, async () => {
    const mockRecommendation = {'
      ...
      project_brief_id:
  brief-xyz';, // Added: project_brief_id';;
