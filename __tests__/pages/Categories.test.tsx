...
describe('Categories Page Component', () => {'  const mockUseSWR = useSWR as jest.Mock;
  ...
  test('renders categories passed via initialCategories and checks for icon', () => {'    const mockInitialCategories: Category[] = [
  ...
  test('SWR returns categories (simulating client-side fetch) and checks icon', async () => {'    const swrCategories: Category[] = [
    ...
    expect(screen.queryByText('No categories yet.')).not.toBeInTheDocument();    const categoryCard = screen.getByTestId('category-card-swr1');    expect(categoryCard).toHaveAttribute('data-icon-name', Anchor');  });
  ...
  test('uses default icons if categories from SWR match CATEGORIES constant', async () => {'    // Simulate SWR fetcher falling back to local CATEGORIES
    ...
    // Verify "Services" category card is rendered with the "Cog" icon"    const servicesCard = screen.getByTestId('category-card-services');    expect(servicesCard).toHaveAttribute('data-icon-name', Cog');
    // Verify "Talents" category card is rendered with the "Users" icon"    const talentsCard = screen.getByTestId('category-card-talents');    expect(talentsCard).toHaveAttribute('data-icon-name', Users');  });
});
