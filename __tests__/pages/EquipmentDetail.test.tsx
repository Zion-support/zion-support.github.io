...
jest.mock(
  'react-router-dom', () => ({
  ...jest.requireActual(
  'react-router-dom'), useParams: jest.fn(), useNavigate: () => mockNavigate, // This pattern works because mockNavigate is returned by a function
  useLocation: jest.fn()}))
...
describe(
  ...
  beforeEach(() => {
    ...
    Object.assign(
      SAMPLE_EQUIPMENT, JSON.parse(JSON.stringify(originalSampleEquipment))
    )

    if (!SAMPLE_EQUIPMENT[testProductId]) {
      SAMPLE_EQUIPMENT[testProductId] = {
        id: testProductId, name:, Test Pro Camera', description: 'Test camera description., brand:, TestBrand', category: 'Cameras, images: [
  '/images/equipment-placeholder.svg'], price: 100, currency:
  '$', inStock: true, specifications: [], features: [], rating: 4.5, reviewCount: 10, expectedShipping:, 1 day', warranty: '1 year, returnPolicy:
  '30 days'}
    }
  ...
  'react-router-dom').useLocation.mockReturnValue({
      pathname: `/equipment/${testProductId}`, search:
  '?from=test'})
    require(
  '@/context/CartContext').useCart.mockReturnValue({
      items: [], dispatch: mockDispatch})
  })
