...
describe('Composite Validation', () => {
  test('combines multiple validators successfully', () => {
    const validators = [
      ...
      (val: string) => validateLength(val, 10, 50),
    ];
  ...
  test('fails on first invalid validator', () => {
    const validators = [
      ...
      (val: string) => validateLength(val, 5, 15),
    ];
...
describe('Async Validation', () => {
  test('handles successful async validation', () => {
    const asyncValidator = async (value: string) => {
      // Simulate async validation
      await new Promise(resolve => setTimeout(resolve, 10));
      return validateLength(value, 3, 10);
    };

    return validateAsync(asyncValidator, 'test').then(result => {
      expect(result.isValid).toBe(true);
    });
  });

  test('handles failed async validation', () => {
    const asyncValidator = async (value: string) => {
      await new Promise(resolve => setTimeout(resolve, 10));
      return validateLength(value, 10, 20);
    };

    return validateAsync(asyncValidator, 'short').then(result => {
      expect(result.isValid).toBe(false);
    });
  });
});