...
describe('InputFields Component', () => {'  const mockOnSubmit = jest.fn();
  ...
  test('calls onSubmit with form data when all fields are valid', async () => {'    render(<InputFields onSubmit={mockOnSubmit} />);
    ...
    fireEvent.change(screen.getByLabelText(/Company Mission/i), { target: { value: Test Mission' } });    fireEvent.change(screen.getByLabelText(/Current Funding Stage/i), { target: { value: Seed' } });    fireEvent.change(screen.getByLabelText(/Vision\/Goals/i), { target: { value: Test Vision' } });    fireEvent.change(screen.getByLabelText(/Target Raise Amount/i), { target: { value: 100000' } });    // fireEvent.change(screen.getByLabelText(/Round Type/i), { target: { value: Series A' } }); // Already has default
    fireEvent.click(screen.getByRole('button', { name: /Next: Data Sync/i }));
    await waitFor(() => {
      expect(mockOnSubmit).toHaveBeenCalledWith({
        companyMission: Test Mission',        currentFundingStage: Seed',        visionGoals: Test Vision',        roundType: Seed', // Default'        targetRaiseAmount: 100000',        logos: null,
        photos: null
      });
    });
  });
