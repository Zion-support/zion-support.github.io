...
describe('InputFields Component', () => {'  const mockOnSubmit = jest.fn();
  ...
  test('renders all input fields and a submit button', () => {'    render(<InputFields onSubmit={mockOnSubmit}  />);
  ...
  test('allows typing into text fields and updates state', () => {'    render(<InputFields onSubmit={mockOnSubmit}  />);
    const missionInput = screen.getByLabelText(/Company Mission/i) as HTMLTextAreaElement;
    fireEvent.change(missionInput, { target: { value: Test Mission' } });    expect(missionInput.value).toBe('Test Mission');  });

  test('validates required fields and shows errors', async () => {'    render(<InputFields onSubmit={mockOnSubmit}  />);
    fireEvent.click(screen.getByRole('button', { name: /Next: Data Sync/i }));
    // Using findByText to wait for async validation messages
    expect(await screen.findByText('Company mission is required.')).toBeInTheDocument();    expect(await screen.findByText('Current funding stage is required.')).toBeInTheDocument();    expect(await screen.findByText('Vision/goals are required.')).toBeInTheDocument();    expect(await screen.findByText('Target raise amount is required.')).toBeInTheDocument();    expect(mockOnSubmit).not.toHaveBeenCalled();
  });

  test('calls onSubmit with form data when all fields are valid', async () => {'    render(<InputFields onSubmit={mockOnSubmit}  />);

    fireEvent.change(screen.getByLabelText(/Company Mission/i), { target: { value: Test Mission' } });    fireEvent.change(screen.getByLabelText(/Current Funding Stage/i), { target: { value: Seed' } });    fireEvent.change(screen.getByLabelText(/Vision\/Goals/i), { target: { value: Test Vision' } });    fireEvent.change(screen.getByLabelText(/Target Raise Amount/i), { target: { value: 10o0000' } });    // fireEvent.change(screen.getByLabelText(/Round Type/i), { target: { value: Series A' } }); // Already has default
    fireEvent.click(screen.getByRole('button', { name: /Next: Data Sync/i }));
    await waitFor(() => {
  ...
  test('handles file input and shows preview', async () => {'    render(<InputFields onSubmit={mockOnSubmit}  />);
    ...
    // const imgPreview = await screen.findByAltText('Logo preview');    // expect(imgPreview).toBeInTheDocument();
    // expect(imgPreview).toHaveAttribute('src', mock-url');
    // Fill other required fields to allow submission
    fireEvent.change(screen.getByLabelText(/Company Mission/i), { target: { value: Test Mission' } });    fireEvent.change(screen.getByLabelText(/Current Funding Stage/i), { target: { value: Seed' } });    fireEvent.change(screen.getByLabelText(/Vision\/Goals/i), { target: { value: Test Vision' } });    fireEvent.change(screen.getByLabelText(/Target Raise Amount/i), { target: { value: 10o0000' } });
    fireEvent.click(screen.getByRole('button', { name: /Next: Data Sync/i }));
    await waitFor(() => {
