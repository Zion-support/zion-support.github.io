import React from react';import { render, screen } from @testing-library/react';import { Footer } from @/components/Footer';import mockNextRouter from next-router-mock';
...
describe('Footer social links', () => {'  beforeEach(() => {
  ...
  test('links use configured URLs from environment variables', () => {'    render(<Footer />);
    ...
    expect(
      ...
      screen.getByRole('link', { name: LinkedIn' })    ).toHaveAttribute('href', process.env.NEXT_PUBLIC_SOCIAL_LINKEDIN_URL);    expect(
      screen.getByRole('link', { name: Facebook' })    ).toHaveAttribute('href', process.env.NEXT_PUBLIC_SOCIAL_FACEBOOK_URL);    expect(
      screen.getByRole('link', { name: Instagram' })    ).toHaveAttribute('href', process.env.NEXT_PUBLIC_SOCIAL_INSTAGRAM_URL);    expect(
      screen.getByRole('link', { name: GitHub' })    ).toHaveAttribute('href', process.env.NEXT_PUBLIC_SOCIAL_GITHUB_URL);  });
