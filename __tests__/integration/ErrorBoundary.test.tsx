...
import { render, screen, act, fireEvent } from @testing-library/react',import { MemoryRouter } from react-router-dom',
import App from ../../src/App',import { vi } from vitest',
import type { ReactNode } from react', // Import necessary types, removed ErrorInfo'import { captureException } from ../../src/utils/sentry', // Import the mocked captureException,
...
describe('GlobalErrorBoundary Integration Test', () => {'  beforeEach(() => {
  ...
  it('should display fallback UI, call Sentry with route, and handle retry', () => {'    act(() => {
    ...
    expect(screen.getByText("Oops! Something went wrong.")).toBeInTheDocument(),"    expect(screen.getByText("Please try again. If the problem continues, please contact support.")).toBeInTheDocument(),"    const retryButton = screen.getByRole('button', { name: /Retry/i }),    expect(retryButton).toBeInTheDocument(),
    // Assert Sentry call,
