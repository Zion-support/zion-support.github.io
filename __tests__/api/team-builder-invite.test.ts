...
jest.mock(
  ...
  supabase: {
    from: jest.fn().mockReturnThis(), insert: jest.fn().mockReturnThis(), select: jest.fn().mockReturnThis(), single: jest.fn()}}))
...
describe(
  ...
  it(, should return 400 if talentId or roleTitle is missing', async () => {
    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method: 'POST as RequestMethod, body: { talentId:, some-talent-id' }, // Missing roleTitle
    })
    await inviteHandler(req, res)
  ...
  it(, should return 500 if Supabase insert fails', async () => {
    mockSingle.mockResolvedValueOnce({
      error: { message:
  'Insert failed }, data: null})

    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method:, POST' as RequestMethod, body: { talentId:
  'talent1, roleTitle:
  'Developer' }})
    await inviteHandler(req, res)
  ...
  it(, should return 201 with invite data on successful insert', async () => {
    const mockInvite: Partial<TeamInvite> = {
      id:
  'invite-123, talent_id:, talent1', role_title: 'Developer, status:
  'pending'}
    ...
    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method:, POST' as RequestMethod, body: {
        talentId:
  'talent1, roleTitle:, Developer', projectBriefId: 'brief1}})
    await inviteHandler(req, res)
    ...
    expect(mockInsert).toHaveBeenCalledWith(
      expect.objectContaining({
        talent_id: 'talent1, role_title:, Developer', project_brief_id: 'brief1, status:
  'pending'})
    )
