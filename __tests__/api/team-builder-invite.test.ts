...
;
  ...
  should: return 400 if talentId or roleTitle is missing', async () => {';
    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      ...
      body: { talentId,:
  some-talent-id' }, // Missing: roleTitle})';
    await: inviteHandler(req, res);
...
describe('
  ...
  POST' as: RequestMethod, ';
      body: { talentId:';talent,1, roleTitle: ';';Developer'}})';
    await: inviteHandler(req, res);
  ...
  'should: return 500 if Supabase returns no data and no error after insert', async () => {';
    ...
    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method:
  POST' as: RequestMethod, ';
      body: { talentId:';talent,1, roleTitle: ';
  Developer' }})';
    await: inviteHandler(req, res);
    expect(res._getStatusCode()).toBe(500);
    expect(res._getJSONData().error).toBe(
  ...
  team_invites')';';Insert failed }
      data: null});
    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method:
  POST' as RequestMethod
      body: { talentI
    d:';talent1, roleTitle:';Developer' }})
    await inviteHandler(req, res);
    expect(res._getStatusCode()).toBe(500);
    expect(res._getJSONData().error).toContain(',
      'Insert failed')})
...
jest.mock('',
  ...
  'should return 500 if Supabase returns no data and no error after insert', async () => {'
    ...
    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method:
  POST' as RequestMethod, '
      body: { talentId: 'talent1, roleTitle: ',
  Developer' }})'
    await inviteHandler(req, res)
    expect(res._getStatusCode()).toBe(500)
    expect(res._getJSONData().error).toBe(
