...
describe('/api/team-builder/generate API Endpoint', () => {
  ...
  it('should return 500 if Supabase function call fails', async () => {
    ...
    expect(res._getJSONData().error).toBe('Failed to generate team recommendations');
  });
  ...
  it('should return 200 with team recommendations on success', async () => {
    ...
    expect(responseData.recommendations[0].role).toBe('Frontend Developer');
    expect(responseData.recommendations[1].role).toBe('Backend Developer');
  });
});