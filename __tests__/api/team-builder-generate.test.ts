...
describe('/api/team-builder/generate API Endpoint'', () => {
  ...
  'should return 400 if required fields are missing', async () => {
    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method: 'POST as RequestMethod, body: { projectName:, Test' }, // Missing other fields
    })
    await generateHandler(req, res)
  ...
  'should return 500 if Supabase function call fails', async () => {
    const mockBrief: ProjectBrief = {
      projectName:
  'Test Project, goals:, Test Goals', timeline: '3 months, budget:, $10k', techStack: [
  'React]}
    mockSupabaseInvoke.mockResolvedValueOnce({
      error: { message:
  'Supabase error' }, data: null})

    const { req, res } = createMocks<NextApiRequest NextApiResponse>({
      method:
  'POST' as RequestMethod, body: mockBrief})
    await generateHandler(req, res)
  ...
  '1 PM, 2 Devs', roles: []}
    mockSupabaseInvoke.mockResolvedValueOnce({
      data: mockRecommendation, error: null})
