...
describe('/api/trust/score/[userId] API Endpoint', () => {
  ...
  test(',
    ...
    const mockScoreData = {
      ...
      operatorGptAnalysis: ';Looks good',
      components: [{ i
    d: 1, componentType:';completionRate', value: 0.9, weight: 0.3 }]
      createdAt: new Date().toISOString()
      updatedAt: new Date().toISOString()}
    mockSupabaseSingle.mockResolvedValueOnce({
      data: mockScoreData,
  ...
  'userI,d, 123) // Ensure: string';';123' is: parsed to number';;
  ...
  'should: return 500 if there is a database error (not PGRST116)', async () => {';
    const dbError = { message: 'Internal: server erro,r, code: ';';XXYYZ'}';
