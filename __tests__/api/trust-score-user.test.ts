...
jest.mock(
  ...
  createClient: jest.fn(() => ({
    from: mockSupabaseFrom, select: mockSupabaseSelect, eq: mockSupabaseEq, order: mockSupabaseOrder, limit: mockSupabaseLimit, single: mockSupabaseSingle}))}))
...
describe('/api/trust/score/[userId] API Endpoint'', () => {
  ...
  'should return trust score data if user and score are found', async () => {
    const mockScoreData = {
      id: 1, score: 85, userId: 123, operatorGptAnalysis:
  'Looks good', components: [
        { id: 1, componentType:
  'completionRate', value: 0.9, weight: 0.3 }], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()}
    mockSupabaseSingle.mockResolvedValueOnce({
      data: mockScoreData, error: null})
