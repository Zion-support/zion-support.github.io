...
describe('/api/auth/register API Endpoint', () => {'  const ORIGINAL_ENV = { ...process.env };
  ...
  it('should return 405 if method is not POST', async () => {'    const { req, res } = createMocks({
      method: GET',    });
  ...
  it('should return 400 if name is missing', async () => {'    const { req, res } = createMocks({
  ...
  it('should return 400 if email is invalid', async () => {'    const { req, res } = createMocks({
  ...
  it('should return 400 if password is too short', async () => {'    const { req, res } = createMocks({
  ...
  it('should return 500 if AUTH0_ISSUER_BASE_URL is not set', async () => {'    delete process.env.AUTH0_ISSUER_BASE_URL; // Ensure it's not set
  ...
  describe('Auth0 Management Token Retrieval and User Creation', () => {'    const validRequestBody = {
    ...
    it('should return 201 on successful registration', async () => {'      mockFetch
    ...
    it('should return 500 if AUTH0_CLIENT_ID is missing (config error before fetch), async () => {'      delete process.env.AUTH0_CLIENT_ID;
      const { req, res } = createMocks({
        method: POST',        body: validRequestBody
      });
      await registerHandler(req as NextApiRequest, res as NextApiResponse);
    ...
    it('should handle 401 from token endpoint (non-retryable), async () => {'      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 401,
    ...
    it('should handle 403 from token endpoint (non-retryable), async () => {'        mockFetch.mockResolvedValueOnce({
          ok: false,
          status: 403,
    ...
    it('should handle 404 from token endpoint (non-retryable), async () => {'      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 404,
    ...
    it('should retry on 500 from token endpoint then succeed', async () => {'      mockFetch
    ...
    it('should exhaust retries on consistent 503 from token endpoint', async () => {'      mockFetch
    ...
    it('should return 400 if Auth0 password policy fails', async () => {'        mockFetch
          .mockResolvedValueOnce({ // Token success
            ok: true,
            json: async () => ({ access_token: fake-management-token' }),          })
