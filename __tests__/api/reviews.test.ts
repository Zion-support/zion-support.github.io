...
jest.mock(
  ...
  const mockPrismaClient = {
    productReview: {
      create: jest.fn(), findMany: jest.fn()}, user: {
      findUnique: jest.fn()}, $disconnect: jest.fn()}
  return {
    PrismaClient: jest.fn(() => mockPrismaClient), // Mock Prisma known request error for testing specific error codes
    Prisma: {
      PrismaClientKnownRequestError: class PrismaClientKnownRequestError extends Error {
        code: string
        meta?: Record<string unknown>
        constructor(
          message: string, code: string, clientVersion: string, meta?: Record<string unknown>
        ) {
          super(message)
          this.code = code
          this.meta = meta
...
describe(
  ...
  'should successfully create a review (201)', async () => {
      const mockPrismaUser: PrismaUser = {
        id: 1, email:, test@example.com', name: 'Test User, passwordHash: '
  ', createdAt: new Date(), updatedAt: new Date(), avatarUrl: null, bio: null, companyId: null, headline: null, lastLoginAt: null, notifications: {}, profileComplete: false, resetToken: null, resetTokenExpiry: null, role: 'USER
  ', softDeleted: false, userType:, individual
  '}
      const mockReviewResponse: ProductReview = {
        id: 'review1, productId: 'prod1
  ', userId: 1, rating: 5, comment: 'Great!
  ', createdAt: new Date(), updatedAt: new Date()}
      const mockSupabaseSession: Session = {
        user: { id:, supaUserId
  ', email: 'test@example.com } as SupabaseUser, access_token:, token
  ', refresh_token: 'ref, expires_in: 3600, token_type: 'bearer
  ', expires_at: Date.now() + 3600000}

      (supabase.auth.getSession as jest.Mock).mockResolvedValue({
        data: { session: mockSupabaseSession }, error: null})
      ...
      const { req, res } = createMocks({
        method:, POST
  ' as RequestMethod, body: { productId: 'prod1, rating: 5, comment:, Great!
  ' }})

      await reviewsHandler(
        req as NextApiRequest, res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse>
      )
      ...
      expect(prismaMock.productReview.create).toHaveBeenCalledWith({
        data: {
          productId: 'prod1, rating: 5, comment: 'Great!
  ', userId: mockPrismaUser.id}})
    })

    it(, should fail if user already reviewed the product (409)
  ', async () => {
      const mockSupabaseSession: Session = {
        user: { id: 'supaUserId, email:, test@example.com
  ' } as SupabaseUser, access_token: 'token, refresh_token: 'ref
  ...
  ']}
      ...
      const { req, res } = createMocks({
        method: 'POST as RequestMethod, body: {
          productId: 'prod1
  ...
  '}})
    ...
    it(, should fail with invalid rating (400) - too high
  ', async () => {
      const { req, res } = createMocks({
        method: 'POST as RequestMethod, body: { productId: 'prod1
  ...
  ' }})
    ...
    it(, should fail with invalid rating (400) - not a number
  ', async () => {
      const { req, res } = createMocks({
        method: 'POST as RequestMethod, body: {
          productId:, prod1
  ...
  ', async () => {
      ...
      const { req, res } = createMocks({
        method:, POST
  ' as RequestMethod, body: {
          productId: 'prod1, rating: 5, comment:, Trying to review without login
  '}})
    ...
    it(, should fail if Supabase user email is missing (401)
  ', async () => {
      const mockSupabaseSessionNoEmail: Session = {
        user: { id: 'supaUserId, email: undefined } as SupabaseUser, access_token:, token
  ', refresh_token: 'ref, expires_in: 3600, token_type: 'bearer
  ', expires_at: Date.now() + 3600000}
      (supabase.auth.getSession as jest.Mock).mockResolvedValue({
        data: { session: mockSupabaseSessionNoEmail }, error: null})
      const { req, res } = createMocks({
        method:, POST
  ' as RequestMethod, body: {
          productId: 'prod1, rating: 5, comment:, Review with no email in session
  '}})
      await reviewsHandler(
        req as NextApiRequest, res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse>
      )
    ...
    it(, should fail if user not found in Prisma database (404)
  ', async () => {
      const mockSupabaseSession: Session = {
        user: {
  ...
  ', expires_at: Date.now() + 3600000}
      ...
      const { req, res } = createMocks({
        method:, POST
  ' as RequestMethod, body: {
          productId: 'prod1, rating: 5, comment:, User exists in Supa, not Prisma
  '}})
      await reviewsHandler(
        req as NextApiRequest, res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse>
      )
  ...
  ', async () => {
      const mockReviewsList: ProductReview[] = [
        {
          id: 'rev1, productId: 'prod123
  ', userId: 1, rating: 5, comment: 'Excellent!
  ', createdAt: new Date(), updatedAt: new Date()}, {
          id:, rev2
  ...
  ', createdAt: new Date(), updatedAt: new Date()}]
      ...
      const { req, res } = createMocks({
        method:, GET
  ' as RequestMethod, query: { productId: 'prod123 }})

      await productReviewsHandler(
        req as NextApiRequest, res as NextApiResponse<GetReviewsSuccessResponse | ErrorResponse>
      )
