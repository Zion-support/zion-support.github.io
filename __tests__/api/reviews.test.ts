...
describe('/api/reviews API Endpoint';
  describe('POST /api/reviews';
    it('should successfully create a review (201)';
        ...
        \"error\";
        \"error\";}
  PrismaClient,ProductReview,User as PrismaUser,Prisma;}
} from '@prisma/client'; // Import Prisma types;
  User as SupabaseUser,Session,AuthError;
} from '@supabase/supabase-js'; // Supabase types;
ursor/automate-test-improve-and-merge-code-646c;
// Mock Prisma Client;
  Prisma;
...
// Mock Prisma Client;jest.mock('@prisma/client', () => {const mockPrismaClient = {\"productReview\": {create: jest.fn(),\"findMany\": jest.fn()},\"user\": {findUnique: jest.fn(,}
},$\"disconnect\": jest.fn()findMany: jest.fn(,
},user: {findUnique: jest.fn(,}
},$disconnect: jest.fn();
  return {\"PrismaClient\": jest.fn(() => mockPrismaClient),// Mock Prisma known request error for testing specific error codes;}
...
describe('/api/reviews API Endpoint', () => {let \"prismaMock\": PrismaClient; // Use PrismaClient type for the mock;}
  ...
  name: 'Test User',passwordHash: 'hashedpassword',
  createdAt: new Date(),updatedAt: new Date(),avatarUrl: null,bio: null,companyId: null,headline: null,lastLoginAt: null,notifications:  ,
},profileComplete: false,resetToken: null,resetTokenExpiry: null,role: 'USER',
  softDeleted: false,userType: 'individual,
}
...
const mockSupabaseSession: Session = {user: { id: 'supaUserId',}
  email: 'test@example.com',}
} as SupabaseUser,access_token: 'token',
  refresh_token: 'ref',expires_in: 3600,token_type: 'bearer',
...
expect(prismaMock.productReview.create).toHaveBeenCalledWith({
  ...
  userId: mockPrismaUser.id;
      ...
      })})it('should fail if user already reviewed the product (409)', async () => {const mockSupabaseSession: Session = {user: { id: 'supaUserId',}
  email: 'test@example.com',}
} as SupabaseUser,access_token: 'token',
  refresh_token: 'ref',expires_in: 3600,token_type: 'bearer',
  expires_at: Date.now() + 3600000;
      ...
      })})it('should fail with invalid rating (400) - too high', async () => {const { req, res } = createMocks({method: 'POST' as RequestMethod,body: { productId: 'prod1',}
  ...
  rating: 'five-stars',comment: 'Text rating!';}
        }
      })await reviewsHandler(req as NextApiRequest,res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse />;
      )expect(res._getStatusCode()).toBe(400)expect(res._getJSONData()).toEqual({\"error\": 'Rating is required and must be a number between 1 and 5.';}
  ...
  rating: 5,comment: 'Trying to review without login';}
        }
      })await reviewsHandler(req as NextApiRequest,res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse />;
...
comment: 'Trying to review without login'}
    ...
    it('should fail if Supabase user email is missing (401)', async () => {}
      ...
      })})it('should fail if Supabase user email is missing (401)', async () => {const mockSupabaseSessionNoEmail: Session = {user: { id: 'supaUserId',}
  email: undefined,}
} as SupabaseUser,access_token: 'token',
  refresh_token: 'ref',expires_in: 3600,token_type: 'bearer',
  expires_at: Date.now() + 3600000;
  ...
  rating: 5,comment: 'Review with no email in session';}
        }
      })await reviewsHandler(req as NextApiRequest,res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse />;
  ...
  rating: 5,comment: 'User exists in Supa, not Prisma';}
        }
      })await reviewsHandler(req as NextApiRequest,res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse />;
     ...
     ,
        ...
        {id: 'rev1',
  productId: 'prod123',userId: 1,rating: 5,comment: 'Excellent!',}
  createdAt: new Date(),updatedAt: new Date(,}
},{id: 'rev2',
  productId: 'prod123',userId: 2,rating: 4,comment: 'Very good.',}
  createdAt: new Date(),updatedAt: new Date(,}
}
...
const mockReviewResponse: ProductReview = { id: 'review1',
  productId: 'prod1',userId: 1,rating: 5,comment: 'Great!',}
  createdAt: new Date(),updatedAt: new Date();}
}
...
const mockSupabaseSession: Session = { user: { id: 'supaUserId',}
  ...
  refresh_token: 'ref',expires_in: 3600,token_type: 'bearer',
  expires_at: Date.now() + 3600000 } (supabase.auth.getSession as jest.Mock).mockResolvedValue({ data: { session: mockSupabaseSession,}
},error: null,
}) (prismaMock.user.findUnique as jest.Mock).mockResolvedValue(mockPrismaUser) (prismaMock.productReview.create as jest.Mock).mockResolvedValue(mockReviewResponse) const { req,res } = createMocks({ method: 'POST' as RequestMethod,body: { productId: 'prod1',}
  rating: 5,comment: 'Great!' },
}) await reviewsHandler( req as NextApiRequest,res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse /> ) expect(res._getStatusCode()).toBe(201) expect(res._getJSONData()).toEqual(mockReviewResponse)expect(prismaMock.productReview.create).toHaveBeenCalledWith({ data: { productId: 'prod1',
  rating: 5,comment: 'Great!',}
  userId: mockPrismaUser.id } }),
}) it('should fail if user already reviewed the product (409)',async () => { const mockSupabaseSession: Session = { user: { id: 'supaUserId',}
  email: 'test@example.com',}
...
}) (prismaMock.productReview.create as jest.Mock).mockRejectedValue( new Prisma.PrismaClientKnownRequestError( 'Unique constraint failed','P2002','client-version',{ target: ['productId,userId'] } ) ) const { req,res } = createMocks({ method: 'POST' as RequestMethod,body: { productId: 'prod1',}
  rating: 5,comment: 'Another great review!' },
  ...
  refresh_token: 'ref',expires_in: 3600,token_type: 'bearer',
  expires_at: Date.now() + 3600000 } (supabase.auth.getSession as jest.Mock).mockResolvedValue({ data: { session: mockSupabaseSessionNoEmail } error: null,
}) const { req,res } = createMocks({ method: 'POST' as RequestMethod,body: { productId: 'prod1',}
  rating: 5 comment: 'Review with no email in session' },
  ...
  rating: 5 comment: 'User exists in Supa,not Prisma' } }) await reviewsHandler( req as NextApiRequest,res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse /> ) expect(res._getStatusCode()).toBe(404) expect(res._getJSONData()).toEqual({ error: 'User not found in our database.' }) }),
}) describe('GET /api/reviews/[productId]',() => { it('should successfully fetch reviews for a product (200)',async () => { const mockReviewsList: ProductReview[] = [ { id: 'rev1',
  productId: 'prod123' userId: 1 rating: 5 comment: 'Excellent!',}
  createdAt: new Date() updatedAt: new Date() } { id: 'rev2',
  productId: 'prod123' userId: 2 rating: 4 comment: 'Very good.',}
  createdAt: new Date() updatedAt: new Date(),}
...
}) (prismaMock.user.findUnique as jest.Mock).mockResolvedValue(mockPrismaUser) (prismaMock.productReview.create as jest.Mock).mockResolvedValue(mockReviewResponse);
  const { req,res } = createMocks({ method: 'POST' as RequestMethod,body: { productId: 'prod1',}
  rating: 5,comment: 'Great!' },
}) await reviewsHandler( req as NextApiRequest,res as NextApiResponse<PostReviewSuccessResponse | ErrorResponse /> ) expect(res._getStatusCode()).toBe(201) expect(res._getJSONData()).toEqual(mockReviewResponse); expect(prismaMock.productReview.create).toHaveBeenCalledWith({ data: { productId: 'prod1',
  rating: 5,comment: 'Great!',}
