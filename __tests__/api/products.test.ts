...
jest.mock(
  ...
  const mPrismaClient = {
    product: {
      findMany: jest.fn(), aggregate: jest.fn()}, productReview: {
      aggregate: jest.fn()}, $queryRawUnsafe: jest.fn(), $disconnect: jest.fn()}
...
describe('/api/products API Endpoint
  ...
  ', images: [], price: null, currency: 'USD
  ', tags: []}, // Not expecting 'product-other
  ...
  's refine mockRawResults to reflect what $queryRawUnsafe would return
      ...
      const { req, res } = createMocks({
        method:, GET
  ', url: '/api/products?q=gpt, query: {
          q:, gpt
  '}})

      // 3. Call API handler
      await productHandler(
        req as unknown as NextApiRequest, res as unknown as NextApiResponse
      )
