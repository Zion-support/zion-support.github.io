...
describe('/api/products API Endpoint', () => {let req: ReturnType<typeof createRequest>;
  ...
  beforeEach(() => {jest.clearAllMocks()prisma = new PrismaClient()// Get the mocked instance;
    ...
    (prisma.productReview.aggregate as jest.Mock).mockResolvedValue({_avg: { rating: null },_count: { id: 0 }
      ...
      const expectedProductIds = filteredMockRawResults.map(p => p.id)(prisma.product.findMany as jest.Mock).mockImplementation(async ({ where }: { where: { id: { in: string[] } } }) => {return mockProductsData.filter(p => where.id.in.includes(p.id))}
