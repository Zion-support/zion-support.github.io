...
export default function TalentDashboardSuggestedJobs() {const [userId, setUserId] = useState<string | null>(null);
  ...
  useEffect(() => {;
    ...
    async function init() {;
      try {;
        ...
        if (!currentUserId) {setSuggestions([]);
          setLoading(false);
          return;
        }
  ...
  const handleDecline = async (s: JobSuggestion) => {await supabase;
      .from(SUGGESTION_TABLE_ENV);
