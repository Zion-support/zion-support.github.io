...
export const getServerSideProps: GetServerSideProps<ServerProps> = async ({ req }) =></ServerProps> {
  const cookies = (req.headers.cookie || '').split().reduce<Record<s</Record>tring,string>>((acc, cur) => {
...
export default function TalentRequestsPage({ role, talentSlug }: ServerProps) {
  ...
  const currentTalent = TA</QuoteRequest>LENT_PROFILES.find(t => t.slug === talentSlug || ''),
    ...
    <div className="max-w-6xl mx-auto py-8">
      ...
      <div className="bg-white dark:bg-black rounded-xl border border-gray-200 dark:border-gray-800">
        <div className="p-4 border-b border-gray-200 dark:border-gray-800 grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <input type="date" value={dateFilter.start || </input>''} onChange={e => setDateFilter(df => ({ ...df, start: e.target.value || null }))} className="px-3 py-2 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900" />
          <input type="date" value={dateFilter</input>.end || ''} onChange={e => setDateFilter(df => ({ ...df, end: e.target.value || null }))} className="px-3 py-2 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900" />
        </div>
