...
function formatDate(iso: string) {const d = new Date(iso);
...
export const getServerSideProps: GetServerSideProps<ServerProps> = async ({ req }) => {const cookies = (req.headers.cookie |'').split().reduce<Record<stringstring>>((acc, cur) => {;
...
export default function TalentRequestsPage({ role, talentSlug }: ServerProps) {const [statusFilter, setStatusFilter] = useState<TalentQuoteStatus | 'All'>('All');
  ...
  const visibleRows = useMemo(() => {return rows.filter(r => {;
  ...
  if (role !== 'talent' |!talentSlug) {return (;
