...
function useToast() {const [message, setMessage] = useState<string | null>(null);
...
export default function ClientShortlistPage() {const router = useRouter();
  ...
  useEffect(() => {supabase.auth.getSession().then((res) => {;
      if (!res.data.session) {;
  ...
  useEffect(() => {if (!isAuthChecked) return;
    setLoading(true);
  ...
  const analytics = useMemo(() => {const total = applications.length;
    ...
    const stageCounts: Record<CandidateStatus number> = {applied: applications.filter((a) => a.status === "applied").length;
  ...
  const handleMove = async (id: string, status: CandidateStatus) => {if (await updateApplicationStatus(id, status)) {;
  ...
  const handleNotes = async (id: string, notes: string) => {const ok = await updateApplicationNotes(id, notes);
    if (ok) {;
      setApplications((prev) => prev.map((a) => (a.id === id ? { ...a, notes } : a)));
      show("Notes saved", "success");
    } else {show("Failed to save notes", "error");
    }
  }
  const filteredList = useMemo(() => {let list = shortlistedOnly;
