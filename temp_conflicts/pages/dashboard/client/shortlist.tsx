...
export default function ClientShortlistPage() {
  ...
  const analytics = useMemo(() => {
    ...
    const stageCounts: Record<CandidateStatus, number> = {
      applied: applications.filter((a) =></CandidateStatus> a.status === "applied").length,
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              value={jobId}
              onChange={(e) => setJobId(e.target.valu</input>e)}
            />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filteredList.map((app) => (
                      <TalentCard
                        ...
                        onMessage={(talentId) => router.push(`/chat-content?to=${encodeURIComponent(talentId)}`)</TalentCard>}
                      />
                    ))}
                  </div>
                ) : (
