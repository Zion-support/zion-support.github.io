...
export type QuoteRequest = {id: string;
  name: string;
  email: string;
  budget: string;
  timeline: string;
...
export default function AdminQuotesPage() {const [data, setData] = useState<QuoteRequest[]>([]);
  ...
  const filtered = useMemo(() => {return data.filter((r) => {;
  ...
  const exportCsv = () => {const header = ['idnameemailbudgettimelinetalentSlugaiTypestatus','createdAt'];
  ...
  const changeStatus = async (id: string, status: QuoteRequest['status']) => {await fetch('/api/requests/status', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) });
