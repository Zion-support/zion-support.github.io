...
export const getServerSideProps: GetServerSideProps = async (ctx) => {return requireSuperadminSSR(ctx as any);
}
...
export default function IpoPortal({ user }: Props) {const [activeTab, setActiveTab] = useState<'dashboard' | 'dataroom' | 'updates' | 'dealroom'>('dashboard');
  return (;
    <EnhancedLayout>;
...
function Dashboard() {const [metrics, setMetrics] = useState<Metric[]>([]);
  useEffect(() => {;
...
function DataRoom() {const sections: DataRoomSection[] = useMemo(() => [;
  ...
  async function onUpload(e: React.ChangeEvent<HTMLInputElement>) {const file = e.target.files?.[0];
    if (!file) return;
  ...
  function onOpen(fileName: string) {window.open(`/api/ipo/dataroom/file?section=${encodeURIComponent(active)}&file=${encodeURIComponent(fileName)}`, '_blank');
  }
  return (;
    <div>;
...
function InvestorUpdates() {const [updates, setUpdates] = useState<any[]>([]);
  ...
  useEffect(() => { refresh() }, []);
  async function save() {await fetch('/api/ipo/updates/create', {;
      method: 'POST';
...
function DealRoom() {const [terms, setTerms] = useState<any>({});
  const [softCommit, setSoftCommit] = useState('');
  const [offerings, setOfferings] = useState({ safe: true, equity: true, token: false });
  useEffect(() => {fetch('/api/ipo/deal/terms').then((r) => r.json()).then(setTerms).catch(() => setTerms({}));
    fetch('/api/ipo/deal/offerings').then((r) => r.json()).then(setOfferings).catch(() => {});
  }, []);
  async function saveOfferings() {await fetch('/api/ipo/deal/offerings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(offerings) });
  }
  async function submitSoftCommit() {await fetch('/api/ipo/deal/commit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount: softCommit }) });
    setSoftCommit('');
  }
  return (;
