...
export default function DocsAdmin() {const [token, setToken] = useState('');
  const [value, setValue] = useState('');
  const [status, setStatus] = useState<string | null>(null);
  const fetchContent = async () => {;
    setStatus('Loading...');
    const res = await fetch('/api/admin/docs/get', { headers: { 'x-admin-token': token } });
    if (!res.ok) {setStatus('Failed to load');
      return;
    }
  ...
  const saveContent = async () => {setStatus('Saving...');
    try {;
    ...
    } catch (e) {setStatus('Invalid JSON or save error');
    }
  }
  return (;
