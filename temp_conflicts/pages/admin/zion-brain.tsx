...
export default function ZionBrainAdmin() {const [token, setToken] = useState<string>('');
  ...
  useEffect(() => {const t = localStorage.getItem('zion_superadmin_token') |'';
    setToken(t);
    refreshLogs(t);
  }, []);
  const authHeaders = useMemo(() => (token ? { 'x-admin-token': token } : {}), [token]);
  async function refreshLogs(t?: string) {const headers = t ? { 'x-admin-token': t } : authHeaders;
  ...
  async function handleRoute() {const res = await fetch('/api/zion-brain/router', { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders }, body: JSON.stringify({ text: routerInput }) });
    const data = await res.json();
    setRouterResult(data);
    await refreshLogs();
  }
  async function handleEvaluateReflex() {const res = await fetch('/api/zion-brain/reflex', { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders }, body: JSON.stringify(metrics) });
    const data = await res.json();
    setTriggers(data.triggers |[]);
    await refreshLogs();
  }
  async function handleOptimize() {const res = await fetch('/api/zion-brain/optimize', { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders }, body: JSON.stringify({ prompt, userIntent }) });
    const data = await res.json();
    setOptimized(data.optimized |'');
    await refreshLogs();
  }
  function saveToken() {localStorage.setItem('zion_superadmin_token', token);
    refreshLogs(token);
  }
  const heatCells = useMemo(() => {if (!logs) return [] as { key: string, value: number }[];
