...
const emptyPost: Partial<BlogPost> = {title: '';
  slug: '';
  author: '';
  ...
  body: '';
  status: 'draft'}
export default function AdminBlog() {const [token, setToken] = useState('');
  ...
  const saveToken = () => {localStorage.setItem('adminToken', token);
  }
  const save = async (publish = false) => {const payload = { ...editing, status: publish ? 'published' : (editing.status |'draft') } as Partial<BlogPost>;
    const isCreate = !payload.id;
    const res = await fetch(isCreate ? '/api/blog/posts' : `/api/blog/posts/${payload.id}`, {method: isCreate ? 'POST' : 'PUT';
      headers: { 'Content-Type': 'application/jsonx-admin-token': token }
      body: JSON.stringify(payload)});
    if (res.ok) {const saved = await res.json();
      setEditing(saved);
      // refresh list;
  ...
  const startEdit = (p?: BlogPost) => {if (p) setEditing(p);
