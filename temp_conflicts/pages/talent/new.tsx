...
export default function NewTalentPage() {const [form, setForm] = useState({;
    name: '';
    title: '';
    location: '';
    timezone: '';
  ...
  const onGenerate = async (e: FormEvent) => {e.preventDefault();
    setError(null);
    setLoading(true);
    try {;
  ...
  const onSave = async () => {if (!generated) return;
    setSaving(true);
    setError(null);
    try {;
      ...
      const res = await fetch('/api/talent', {method: 'POST';
        headers: { 'Content-Type': 'application/json' }
