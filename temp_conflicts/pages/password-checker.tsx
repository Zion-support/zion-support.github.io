...
export default function PasswordCheckerPage() {const [password, setPassword] = useState('');
  ...
  const analyzePassword = (pass: string) => {const newChecks = {;
    ...
    if (score >= 90) {setStrengthText('Very Strong');
      setStrengthColor('text-green-400');
    } else if (score >= 70) {setStrengthText('Strong');
      setStrengthColor('text-green-400');
    } else if (score >= 50) {setStrengthText('Moderate');
      setStrengthColor('text-yellow-400');
    } else if (score >= 30) {setStrengthText('Weak');
      setStrengthColor('text-orange-400');
    } else {setStrengthText('Very Weak');
      setStrengthColor('text-red-400');
    }
    // Generate suggestions;
    generateSuggestions(newChecks, pass);
  }
  const isCommonPassword = (pass: string) => {const commonPasswords = [;
  ...
  const hasSequentialChars = (pass: string) => {const sequences = ['123abcqweasdzxc789456'];
    return sequences.some(seq => pass.toLowerCase().includes(seq));
  }
  const generateSuggestions = (checks: any, pass: string) => {const suggestions: string[] = [];
  ...
  const resetAnalysis = () => {setStrength(0);
    setStrengthText('');
    setStrengthColor('');
    setChecks({;
      length: false;
  ...
  const generateStrongPassword = () => {const length = 16;
  ...
  const getStrengthBarColor = () => {if (strength >= 90) return 'bg-green-500';
