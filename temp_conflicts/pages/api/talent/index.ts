...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {if (req.method === 'GET') {;
  ...
  if (req.method === 'POST') {try {;
      ...
      for (const lang of SUPPORTED_LANGS) {if (!lang |lang === originalLang) continue;
      ...
      if (hasSupabase) {const { error } = await supabaseClient.from('talent_profiles').insert({id: item.id;
          slug: item.slug;
          name: item.name;
          title: item.title;
