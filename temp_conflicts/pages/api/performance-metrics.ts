...
export default async function handler(;
  req: NextApiRequest;
  res: NextApiResponse;
): Promise<void> {if (req.method === 'POST') {;
    try {;
      ...
      if (poorMetrics.length > 0) {console.warn('⚠️ Poor Performance Metrics Detected:', poorMetrics.map(m =>;
          `${m.name}: ${m.value}ms`;
        ));
      }
      ...
      if (process.env['NODE_ENV'] === 'production' && process.env['ANALYTICS_ENDPOINT']) {try {;
        ...
        } catch (error) {console.error('Error sending to analytics:', error);
        }
      }
  ...
  if (req.method === 'GET') {try {;
      ...
      if (type === 'performance') {const limitedMetrics = performanceMetrics;
          .slice(-Number(limit));
          .map(metric => ({;
            ...metric;
      ...
      } else if (type === 'error') {const limitedErrors = errorLogs;
          .slice(-Number(limit));
          .map(error => ({;
            ...error;
      ...
      } else if (type === 'summary') {res.status(200).json({;
          success: true;
          summary: {;
            performance: {;
              total: performanceMetrics.length;
  ...
  } else {res.setHeader('Allow', ['POSTGET']);
    res.status(405).json({;
      success: false;
...
function calculateAverages(metrics: PerformanceData[]) {if (metrics.length === 0) return null;
  ...
  return {fcp: Math.round(sums.fcp / metrics.length);
