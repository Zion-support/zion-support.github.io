...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {if (req.method !== 'POST') {;
  ...
  try {const body = req.body as Partial<ClientBudgetRequest>;
    if (!body |!body.title |!body.category) {;
      return res.status(400).json({ error: 'Missing required fields: title, category' });
    }
    const input: ClientBudgetRequest = {title: String(body.title);
  ...
  } catch (error) {return res.status(500).json({ error: 'Failed to generate budget suggestion' });
  }
}