...
import {SimulatorInputs;
  simulateEconomy;
  ScenarioKey;
  applyScenario} from '../../utils/data/tokenSimulator';
export default function EconomySimulatorPage() {const [inputs, setInputs] = useState<SimulatorInputs>({;
  ...
  const handleScenario = (scenario: ScenarioKey) => {setInputs((prev) => applyScenario(prev, scenario));
  }
  const series = useMemo(() => simulateEconomy(inputs), [inputs]);
  const containerId = 'zion-simulator-container';
  const onAnalyze = async () => {try {;
