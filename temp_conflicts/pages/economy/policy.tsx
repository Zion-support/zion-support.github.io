...
export async function getStaticProps() {
  ...
  if (fs.existsSync(recDir)) {const latestPath = path.join(recDir, 'latest.json');
    if (fs.existsSync(latestPath)) {;
      try { latest = JSON.parse(fs.readFileSync(latestPath, 'utf8')).latest } catch {}
    }
    if (latest) {const p = path.join(recDir, `${latest}.json`);
      if (fs.existsSync(p)) {try { recommendation = JSON.parse(fs.readFileSync(p, 'utf8')) } catch {}
      }
    }
  }
  ...
  if (fs.existsSync(propDir)) {const files = fs.readdirSync(propDir).filter((f) => f.endsWith('.md')).sort().reverse();
    proposal = files[0] |null;
  }
  return { props: { latest, recommendation, proposal } }
}
export default function PolicyPage({ latest, recommendation, proposal }: any) {return (;
    <EnhancedLayout>;
