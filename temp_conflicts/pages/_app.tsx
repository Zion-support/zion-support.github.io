...
import {validateProductionEnvironment;
...
const WhitelabelProvider = dynamic(() =>;
  import('@/context/WhitelabelContext').then(mod => ({ default: mod.WhitelabelProvider })), {ssr: true;
  loading: () => null;
});
const WalletProvider = dynamic(() =>;
  import('@/context/WalletContext').then(mod => ({ default: mod.WalletProvider })), {ssr: false, // Wallet is client-side only;
  loading: () => null;
});
const AnalyticsProvider = dynamic(() =>;
  import('@/context/AnalyticsContext').then(mod => ({ default: mod.AnalyticsProvider })), {ssr: false, // Analytics is client-side only;
  loading: () => null;
});
const CartProvider = dynamic(() =>;
  import('@/context/CartContext').then(mod => ({ default: mod.CartProvider })), {ssr: true;
  loading: () => null;
});
const FeedbackProvider = dynamic(() =>;
  import('@/context/FeedbackContext').then(mod => ({ default: mod.FeedbackProvider })), {ssr: false, // Feedback is client-side only;
  loading: () => null;
});
const ThemeProvider = dynamic(() =>;
  import('@/components/ThemeProvider').then(mod => ({ default: mod.ThemeProvider })), {ssr: true;
  loading: () => null;
});
const AppLayout = dynamic(() =>;
  import('@/layout/AppLayout').then(mod => ({ default: mod.AppLayout })), {ssr: true;
  loading: () => (;
...
const ToastContainer = dynamic(() =>;
  import('@/components/ToastContainer').then(mod => ({ default: mod.ToastContainer })), {ssr: false;
  loading: () => null;
});
const OfflineIndicator = dynamic(() =>;
  import('@/components/OfflineIndicator').then(mod => ({ default: mod.OfflineIndicator })), {ssr: false;
  loading: () => null;
});
const RouteChangeHandler = dynamic(() =>;
  import('@/components/RouteChangeHandler').then(mod => ({ default: mod.RouteChangeHandler })), {ssr: false;
  loading: () => null;
});
const RouteSEO = dynamic(() =>;
  import('@/components/RouteSEO'), {ssr: true;
  loading: () => null;
});
...
// =====================================================const inter = Inter({subsets: ['latin'];
  ...
  variable: '--font-inter';
  preload: true});
const poppins = Poppins({weight: ['400600700'];
  subsets: ['latin'];
  display: 'swap';
  fallback: ['system-uiarial'];
  adjustFontFallback: true;
...
// =====================================================const LanguageProviderWrapper: React.FC<{ children: React.ReactNode }> = ({children}) => {const { user, isAuthenticated } = useAuth();
...
// =====================================================function MyApp({ Component, pageProps }: AppProps) {const router = useRouter();
  ...
  React.useEffect(() => {const forceInitTimeout = setTimeout(() => {;
      if (!isInitialized) {;
  ...
  React.useEffect(() => {let isMounted = true;
    const initializeApp = async () => {;
      try {;
        ...
        // Critical path only;
        try {initializeGlobalErrorHandlers();
        } catch (error) {console.warn('[App] Global error handlers failed:', error);
        }
        try {validateProductionEnvironment();
        } catch (error) {console.warn('[App] Environment validation warning:', error);
        }
        ...
        setTimeout(() => {if (!isMounted) return;
          try {;
            initializeServices().catch((err) =>;
              console.warn('Service initialization failed:', err));
          } catch (error) {console.warn('Services init error:', error);
          }
          // Lazy load performance monitoring;
          if (typeof window !== 'undefined' && process.env.PERFORMANCE_MONITORING === 'true') {import('@/utils/performance').then(perf => {;
          ...
          if (process.env.NODE_ENV === 'development') {import('@/utils/consoleLogCapture').then(console => {;
              console.initConsoleLogCapture();
            }).catch(err => console.warn('Console capture failed:', err));
          }
        }, 200);
      } catch (error) {console.error('[App] Critical initialization error:', error);
        ...
        setTimeout(() => {try {;
          ...
          } catch (sentryError) {console.warn('[App] Could not send error to Sentry:', sentryError);
          }
        }, 0);
      }
    }
  ...
  React.useEffect(() => {try {;
    ...
    } catch (error) {console.warn('Sentry context update failed:', error);
    }
  }, [router.pathname, router.query]);
  // Lazy service worker registration with error handling;
  React.useEffect(() => {if (process.env.NODE_ENV === 'production') {;
      setTimeout(() => {;
                    ...
                    <ApiErrorBoundary>;
                      <ReduxProvider store={store}>;
                        <I18nextProvider i18n={i18n}>;
                          <ErrorProvider>;
                            <AuthProvider>;
                              <ErrorBoundary fallback={<div className="flex items-center justify-center min-h-screen">;
                                  <div className="text-center">;
...
if (process.env.NODE_ENV === 'development') {// // // console.log('[App] MyApp component initialized with loading fix');
}
