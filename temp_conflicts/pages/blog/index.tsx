...
const BlogHome: NextPage<Props> = ({ posts, authors, topics, tags }) => {const [filters, setFilters] = useState<{ author?: string, topic?: string, tag?: string }>({});
  const filtered = useMemo(() => {return posts.filter((p) => {;
...
export const getServerSideProps: GetServerSideProps = async () => {const posts = listPublishedPosts();
