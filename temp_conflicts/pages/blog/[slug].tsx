...
const PostPage: NextPage<Props> = ({ post, all }) => {const pageUrl = typeof window === 'undefined' ? '' : window.location.href;
  useEffect(() => {;
    // track view;
    fetch(`/api/blog/metrics/${post.id}/views`, { method: 'POST' }).catch(() => {});
  }, [post.id]);
  const toc = useMemo(() => {const headings: { level: number, text: string, id: string }[] = [];
    const lines = post.body.split('\n');
    lines.forEach((line) => {const m = /^(#{1,3})\s+(.*)$/.exec(line);
      if (m) {const level = m[1].length;
        const text = m[2].trim();
        const id = text;
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {const slug = ctx.params?.slug as string;
