...
function StarIcon({ className = 'w-5 h-5 text-yellow-500' }: { className?: string }) {return (;
...
function AppleBadge({ href }: { href: string }) {return (;
...
function GoogleBadge({ href }: { href: string }) {return (;
...
function SmartBanner({ iosUrl, androidUrl, deepLink }: { iosUrl: string, androidUrl: string, deepLink: string }) {const [visible, setVisible] = useState(false);
  useEffect(() => {;
...
export default function MobileLaunchPage() {const [email, setEmail] = useState('');
  ...
  const qrHref = useMemo(() => {const target = SITE_BASE_URL ? `${SITE_BASE_URL}/download` : (typeof window !== 'undefined' ? `${window.location.origin}/download` : '/download');
