...
export default function SalaryInsightsPage() {const [roleTitle, setRoleTitle] = useState('Senior AI Engineer');
  ...
  async function fetchInsights() {setLoading(true);
    setError(null);
    try {;
  ...
  function saveInsight() {const payload = { createdAt: new Date().toISOString(), input: { roleTitle, skills, region, experienceLevel, remote, employmentType }, output: data }
    (async () => {try {;
        const { supabase } = await import('../utils/supabase/client');
        const user = await supabase.auth.getUser();
        if (user.data.user) {// Attempt to save to Supabase if table exists;
      ...
      try {const key = 'zion.salary-insights.history';
  ...
  const donutData = useMemo(() => {if (!data) return [] as { label: string, value: number }[];
