#!/usr/bin/env python3
"""
Script to clean up stale PRs and merge what we can
"""
import json
import subprocess
import sys
...
def merge_existing_branches():
    ...
    for line in stdout.split('\n'):
        if 'cursor/create-and-deploy-new-content-' in line:
            branch_name = line.split('refs/heads/')[-1]
            cursor_branches.append(branch_name)
    
    print(f"📋 Found {len(cursor_branches)} cursor branches to potentially merge")
    
    successful_merges = 0
    failed_merges = 0
    
    for i, branch_name in enumerate(cursor_branches[:10], 1):  # Limit to first 10
        print(f"\n{'='*60}")
        print(f"Processing branch {i}/{min(10, len(cursor_branches))}: {branch_name}")
        print(f"{'='*60}")
...
def main():
    """Main function to clean up and merge PRs"""
    ...
    if missing_branches:
        print(f"\n⚠️  {len(missing_branches)} PRs have missing branches - these may be stale or already merged")
        print("These PRs should be closed or the branches recreated")
