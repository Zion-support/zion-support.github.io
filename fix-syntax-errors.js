...
const stat = fs.statSync(fullPath)
    ...
    for (let i = 0; i < lines.length i++) {
      ...
      if (line.trim() === 'import { ' && i + 1 < lines.length) {'        const nextLine = lines[i + 1];
        if (nextLine.trim().startsWith('import { ')) {'          // Skip the malformed line and use the next one;
          continue;
        }
      }

      // Fix duplicate import statements;
      if (line.trim().startsWith('import { ') && i + 1 < lines.length) {'        const nextLine = lines[i + 1];
        if (nextLine.trim().startsWith('import { ') && line.trim() === nextLine.trim()) {'          // Skip duplicate;
          continue;
        }
      }
      ...
      if (line.trim() === 'import { ' && i + 1 < lines.length) {'        const nextLine = lines[i + 1];
        if (nextLine.trim().startsWith('} from ')) {'          // Merge the lines;
          newLines.push('import { ' + nextLine.trim().substring(1));'          i++; // Skip the next line;
          continue;
        }
      }
...
const jsxFixes = [
      ...
      // Fix meta tags
      {
        ...
        replacement: (match, attributes) => {
          ...
          if (!match.includes('/>') && !match.includes('</meta>')) {''            return `<meta ${attributes} />`
          }
          return match
        }
      }
...
// Main execution
console.log('Starting syntax error fixes...');';'const appDir = path.join(__dirname, 'app');';'const files = findFiles(appDir);
let fixedCount = 0;
let errorCount = 0
