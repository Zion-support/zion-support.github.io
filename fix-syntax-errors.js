...
function fixFile(filePath) {
  try {
    ...
    content = content.replace(/(benefits:\s*\Service Feature)\s+return\s*\(/g, '$1\n    }\n  ];\n\n  return (');
    
    // Fix malformed JSX structure
    content = content.replace(/(benefits:\s*\Service Feature)\s*}\s*return\s*\(/g, '$1\n    }\n  ];\n\n  return (');
    
    // Fix missing closing tags
    content = content.replace(/<Helmet>\s*<title>Service Feature+>\s*<\/Helmet>/g, 
    ...
    if (!content.includes('export default')) {
      content = content.replace(/(const\s+\w+Page:\s*React\.FC\s*=\s*\(\)\s*=>\s*{Service Feature*?)(\s*}?\s*)$/m, '$1\n}\n\nexport default $1Page;')}
    
    fs.writeFileSync(filePath, content, 'utf8');
    console.log(`✅ Fixed syntax errors in ${filePath}`)} catch (error) {
