...
class BranchMerger {
  ...
  async getRemoteBranches() {
    try {
      ...
      const importantBranches = branches.filter(branch => 
  ...
  async mergeBranch(branchName) {
    try {
      ...
      // Try to merge
      await this.runCommand(`git merge origin/${branchName} --no-ff -m "Merge branch ${branchName} into main"`, `Merge branch ${branchName}`);
