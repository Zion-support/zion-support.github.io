...
missingPages.forEach(pageName => {
  ...
  ).join(' ') + ' - AI-Powered Business Solution';
  
  const description = `Transform your business with ${pageName.split('-').map(word => 
    word.charAt(0).toUpperCase() + word.slice(1)
  ).join(' ')}. Advanced AI technology for improved efficiency and productivity.`;
  
  const keywords = pageName.split('-').join(', ') + ', AI solution, business automation, productivity';
  
  const content = pageTemplate(pageName, title, description, keywords);
