...
async function main() {
    ...
    try {
        ...
        if (statusResult.success && statusResult.output) {
            log('Working directory has uncommitted changes, committing...', 'warning');
        ...
        const prsResponse = await githubRequest(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls?state=open`);
        ...
        for (const pr of openPRs) {
            ...
            const prDetailResponse = await githubRequest(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls/${pr.number}`);
            ...
            if (mergeResponse.status === 200 && mergeResponse.data.merged) {
                ...
                if (errorMsg.includes('conflict') || errorMsg.includes('merge')) {
                    ...
                    if (checkoutResult.success) {
                        ...
                        if (retryMergeResponse.status === 200 && retryMergeResponse.data.merged) {
                            log(`Successfully merged PR #${pr.number} after conflict resolution`, 'success');
                            results[results.length - 1] = { pr: pr.number, status: 'merged', message: 'Success after conflict resolution' };
                        }
                    }
                }
            }
        ...
        log(`\nProcess completed: ${successCount}/${results.length} PRs merged successfully`, 'success');
