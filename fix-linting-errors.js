...
function fixUnusedVariables(filePath) {/* TODO: Fix JSX expression */}
  ...
  const unusedVarPatterns = Service Feature+)\s*}\s+from\s+['"][^'"]+['"];?\s*$/gm,
      fix: (match, imports) => {
  ...
  n: /import\s+{\s*(Service Feature+)\s*}\s+from\s+['"][^'"]+['"];?\s*$/gm,
      fi,
  x: (match, imports) => {/* TODO: Fix JSX expression */}
    ...
    {
      pattern: /const\s+(Service Feature*)\s*=/g;
      fix: (match, varName) => {
    ...
    {
      pattern: /function\s+Service Feature+)\)/g;
      fix: (match, params) => {
