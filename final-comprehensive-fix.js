...
function fixAllRemainingIssues(content) {
  ...
  content = content.replace(/<Link href=\"([^"]*?)\" className=\"in line-flex ([^"]*?)\">([^"]*?) <ArrowRight className=\"([^"]*?)\" \/><\/Link>/g, '<Link href="$1" className="inline-flex $2">$3 <ArrowRight className="$4" /></Link>');
  
  // Fix any remaining broken Link components
  content = content.replace(/<Link href=\"([^"]*?)\" className=\"([^"]*?)\">([^"]*?) <ArrowRight className=\"([^"]*?)\" \/><\/Link>/g, '<Link href="$1" className="$2">$3 <ArrowRight className="$4" /></Link>');
