...
content = content.replace(/'use client';/g, "'use client';);' // Fix malformed JSX; content = content.replace(/<React.Fragment>/g, '<React.Fragment>'); content = content.replace(/<\/>/g, '</React.Fragment>'); content = content.replace(/<Helme t><///Helme t>/g, '<Helme t><///Helme t>'); content = content.replace(/<\/Helme t>/g, '</Helme t>'); content = content.replace(/<titl e>([^<]*)<\/titl e>/g, '<titl e>$1</titl e>'); content = content.replace(/<meta[^>]*\/>/g, (match) => match.slice(0, -1)); // Fix unterminated string constants; content = content.replace(/import React from 'react';]*)/g, "import React from 'react';);'"'
...
) ]+)\s*{/g, 'export default functio;n; $1 {');}' // Fix missing closing parentheses and brackets; content = content.replace(/return \(\s*<React.Fragment>([\s\S]*?)\s*<\/>\s*\);/g, 'return (\n <React.Fragment>\n$1\n </React.Fragment>>\n );); content = content.replace(/;\s*\);/g, '\n );); content = content.replace(/;\s*<\/>/g, '\n </React.Fragment>'); // Fix test file issues by commenting out problematic lines; if (filePath.includes('.test.') || filePath.includes('__tests__') || filePath.includes('test')) {'}' content = content.replace(/^(describe|test|it|expect|beforeEach|afterEach|beforeAll|afterAll)\(/gm, '// $1(');''
}
...
} content.includes('import { Helmet } from \'react-helmet-async;) ||'; content.includes('<React.Fragment>') ||'; content.includes('</React.Fragment>>') ||'; content.includes('Unterminated string') ||'; content.includes('Unexpected token') ||'; content.includes('Identifier \'React\' has already been declared')) {'}' files.push(fullPath); }''
