...
console.log('⚡ Direct Merge Resolver - Critical Branches');
console.log('===========================================');

class DirectMergeResolver {
  constructor() {
    this.mergedBranches = [];
    this.failedBranches = [];
  ...
  async mergeCriticalBranches() {
    // Focus on the most important branches
  ...
  generateReport() {
    ...
    fs.writeFileSync('direct-merge-report.json', JSON.stringify(report, null, 2));
    this.log(`Report saved to direct-merge-report.json`, 'info');
  ...
  async run() {
    try {
      this.log('Starting direct merge resolution...', 'info');
      ...
      this.log('Direct merge resolution completed!', 'success');
...
// Run the direct merger
const merger = new DirectMergeResolver();
