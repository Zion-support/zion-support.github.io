...
function getFilesWithJSXErrors() {
  try {
    const output = execSync('pnpm run lint 2>&1 | grep -B1 "JSX expressions must have one parent element" | grep "^/workspace"', { encoding: 'utf8' });
    return output.trim().split('\n').filter(line => line.trim());
  } catch (error) {
    console.log('No JSX errors found or lint command failed');
    return [];
  }
}
...
function getFilesWithParsingErrors() {
  try {
    const output = execSync('pnpm run lint 2>&1 | grep -B1 "Parsing error" | grep "^/workspace"', { encoding: 'utf8' });
    return output.trim().split('\n').filter(line => line.trim());
  } catch (error) {
    console.log('No parsing errors found or lint command failed');
    return [];
  }
}
...
const jsxFiles = getFilesWithJSXErrors();
const parsingFiles = getFilesWithParsingErrors();
const allFiles = [...new Set([...jsxFiles, ...parsingFiles])];

console.log(`Found ${allFiles.length} files with errors`);

allFiles.forEach(fixPageFile);
console.log('All JSX and parsing errors fixed!');