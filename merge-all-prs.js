...
function mergePRBranch(branchName, prNumber) {
  try {
    ...
    try {
      ...
      console.log(
        `⚠️  Merge conflicts detected in PR #${prNumber}, resolving...`,
      );
      ...
      if (conflictedFiles.length > 0) {
        console.log(
          `📁 Resolving ${conflictedFiles.length} conflicted files...`,
        );
        ...
        // Commit the merge
        execSync(
          `git commit -m "Merge PR #${prNumber}: ${branchName} (conflicts resolved)"`,
          { stdio: "pipe" },
        );
    ...
    if (fs.existsSync(file)) {
      try {
        ...
        content = content.replace(
          /^\s*console\.(warn|error|info|debug)\([^)]*\);\s*$/gm,
          "",
        );
...
async function main() {
  try {
    ...
    execSync(
      'git commit -m "Final optimizations: clean console.logs, optimize build, resolve all conflicts"',
      { stdio: "pipe" },
    );
