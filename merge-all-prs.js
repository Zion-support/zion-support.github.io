...
async function isPRMergeable(prNumber) {
  try {
    const response = await fetch(`https://api.github.com/repos/Zion-Holdings/zion.app/pulls/${prNumber}`);
    ...
    return { mergeable: false, mergeable_state: 'unknown', draft: true, head: '', title: '' };
  }
}
...
async function main() {
  ...
  for (const pr of prs) {
    ...
    if (prInfo.mergeable && prInfo.mergeable_state === 'clean' && !prInfo.draft) {
      mergeablePRs.push({
        number: pr.number,
        title: prInfo.title,
        branch: prInfo.head
      ...
      console.log(`⚠️  PR #${pr.number} is not mergeable (mergeable: ${prInfo.mergeable}, state: ${prInfo.mergeable_state}, draft: ${prInfo.draft})`);
    }
  }
  ...
  console.log(`✅ Successfully merged: ${mergeablePRs.length - failedPRs.length} PRs`);
