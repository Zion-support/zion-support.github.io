...
const fs = require('fs';
  const path  = require('path')function fixUnescapedEntities() {try {let content = fs.readFileSync(filePath, 'utf8')let modified  = false;// Replace ' with single quotes;}
    ...
    if (content.includes('\"')) {content = content.replace(/\"/g, '\"')modified = true;}
    }if (modified) {fs.writeFileSync(filePath, content, 'utf8')console.log(`Fixed unescaped entities in: ${filePat,}
}`)return true;
    }return false;
...
const workspaceDir = process.cwd(;
  const tsxFiles  = findTSXFiles(workspaceDir)console.log(`Found ${tsxFiles.length} TSX/JSX files to check`)let fixedCount = 0;
