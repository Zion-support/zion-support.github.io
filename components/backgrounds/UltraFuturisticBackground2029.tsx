...
const gradient = ctx.createRadialGradient(canvas.width / 2,
  ...
  0.8)');
      gradient.addColorStop(0.3, 'rgba(6, 182, 212,,,
  0.1)');
      gradient.addColorStop(0.6, 'rgba(139, 92, 246,,,
  0.05)');
...
if (index % 10 = == 0) {
  ...
  const partner =
              ...
              if (partner && partner.type = == 'quantum') {
               ;
  const distance = Math.sqrt(Math.pow(particle.x - partner.x,,,
  2) +
                    Math.pow(particle.y - partner.y,,,
  2)
                );
                if (distance < 100) {) => {
  return $3;}
...
const hue = (time * 50 + index * 10) % 360;
            ...
            ctx && ctx.beginPath();
            ctx && ctx.arc(particle && particle.x, particle && particle.y, particle && particle.size, 0,,,
  Math && Math.PI * 2);
            ctx && ctx.fill();
            break;
...
if (index % 15 = == 0) {
  ...
  const connections = particlesRef.current.filter(p =>
                  p.type === 'neural' &&
                  Math.sqrt(
                    Math.pow(particle.x - p.x,,,
  2) +
                      Math.pow(particle.y - p.y,,,
  2)
                  ) < 80;
              );
  ...
  const distance = Math.sqrt(
                  Math.pow(particle.x - connection.x,,,
  2) +
                    Math.pow(particle.y - connection.y,,,
  2)}
                );}
...
ctx.stroke();
          ...
          case 'cyberpunk':;
            ...
            ctx.shadowColor = particle.color;
            ctx.shadowBlur = 25;
            ctx.beginPath()ctx.moveTo(particle.x - particle.size,,,
  particle.y - particle.size)ctx.lineTo(particle.x + particle.size,,,
  particle.y + particle.size)ctx.moveTo(particle.x + particle.size,,,
  particle.y - particle.size)ctx.lineTo(particle.x - particle.size,,,
  particle.y + particle.size)ctx.stroke()break;
        }ctx.restore()})// Draw floating geometric shapes;
      drawGeometricShapes(ctx, canvas.width,,,
  canvas.height)// Draw energy waves;
      drawEnergyWaves(ctx, canvas.width,,,
  canvas.height)animationRef.current = requestAnimationFrame(animate)}animate(;,,
  return () => {window.removeEventListener('resize',,,
  resizeCanvas)if (animationRef.current) {cancelAnimationFrame(animationRef.current)}
    }
  }, [];
  const drawGeometricShapes = (const time = Date.now() * 0.0005;
    // Floating hexagons;
...
const px = x + size * Math.cos(angle;) => {
  ...
  const py = y + size * Math.sin(angle)if (j === 0) ctx.moveTo(px,,,
  py)else ctx.lineTo(px,,,
  py)}
...
const px = x + size * Math.cos(angle)const py = y + size * Math.sin(angle)if (j === 0) ctx.moveTo(px,,,
  py)else ctx.lineTo(px,,,
  py)}
      ctx.closePath()ctx.stroke()ctx.restore()}
  }
;
  const drawEnergyWaves = (const time = Date.now() * 0.001;
...
className='absolute inset-0 bg-gradient-to-br from-black via-cyan-900/20 to-purple-900/30'
        animate={{
          background: [
            ...
            'linear-gradient(45deg, rgba(0,0,0,,,
  1) 0%, rgba(6,182,212,,,
  0.2) 50%, rgba(139,92,246,,,
  0.3) 100%)'}
            'linear-gradient(45deg, rgba(0,0,0
             ,
  1) 0%, rgba(139,92,246,,
  0.3) 50%, rgba(6,182,212,,
  0.2) 100%)'
            'linear-gradient(45deg, rgba(0,0,0,,,
  1) 0%, rgba(6,182,212,,,
  0.2) 50%, rgba(139,92,246,,,
  ...
  i) => (
      ...
      <div className=\"absolute inset-0 pointer-events-none\" />
          ...
          <motion&& motion.div;
            key={i"
            className=\"absolute w-2 h-2 rounded-full bg-cyan-400\"}
            style={{scale: [1, 1.2, 1],opacity: [0.1, 0.3
            0.1]          
          transition={{duration: 4}
            repeat: Infinity
            ease: 'easeInOut'
...
className='absolute top-1/4 left-1/4 w-96 h-96 rounded-full border border-cyan-500/20'
        ...
        <motion.div;
          className='absolute top-3/4 right-1/4 w-80 h-80 rounded-full border border-purple-500/20';
          animate={{scale: [1.2, 1, 1.2],opacity: [0.3, 0.1}
            0.3]
          transition={{duration: 5}
            repeat: Infinity
            ease: 'easeInOut'
        ...
        <motion.div;
          className='absolute top-1/2 left-1/2 w-64 h-64 rounded-full border border-green-500/20';
          animate={{scale: [1, 1.1, 1],opacity: [0.2, 0.4}
            0.2]
          transition={{duration: 6}
            repeat: Infinity
            ease: 'easeInOut'
  ...
  i) => (
          <motion.div;}
            ...
            style={{
              ...
              filter: 'blur(1px)',
  boxShadow: '0 0 20px rgba(0, 255, 255,,,
  0.6)'
            }}
            animate={{
              y: [0, -20}
            ...
            animate={{y: [0, -20, 0],opacity: [0.3, 0.8, 0.3],scale: [1, 1.2}
            1]duration: 4,repeat: Infinity,transition={{delay: i * 0.3;}
