...
import { motion  } from 'lucide-react';import { Shield, AlertTriangle, CheckCircle, Lock, Database, Activity  } from 'lucide-react'';interface SecurityMetrics {';
  ...
  lastScan: Date;
}const SecurityDashboard: React.FC = () => {const [security, setSecurity] = useState<SecurityMetrics | null>(null)const [isLoading, setIsLoading] = useState(true)useEffect(() => {// Mock security data;
    const mockData: SecurityMetrics = {overallScore: Math.floor(Math.random() * 20) + 80, vulnerabilities: {critical: Math.floor(Math.random() * 2), high: Math.floor(Math.random() * 5), medium: Math.floor(Math.random() * 8), low: Math.floor(Math.random() * 15)}, lastScan: new Date()}
    setTimeout(() => {useEffect(() => {// Mock security data;
const mockData: SecurityMetrics = {overallScore: Math.floor(Math.random() * 20) + 80, vulnerabilities: {critical: Math.floor(Math.random() * 2), high: Math.floor(Math.random() * 5), medium: Math.floor(Math.random() * 8), low: Math.floor(Math.random() * 15)}, lastScan: new Date()}setTimeout(() => {setSecurity(mockData)setIsLoading(false)}, 1000)}, [])if (isLoading || !security) {return (<div className="flex items-center justify-center p-8">;
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>;
      </div>;
    )}const getScoreColor = (score: number): string => {if (score >= 90) return 'text-green-500';
