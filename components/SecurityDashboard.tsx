...
const getScoreColor = ("score": number): (string) => {if (score >= 90);
  ...
  return (<div className='space-y-6'>'      <div className='flex items-center justify-between'>'        <h2 className='text-2xl font-bold text-gray-900 "dark": text-white flex items-center'>'          <Shield className='w-6 h-6 mr-2 text-blue-500' />'          Security Dashboard'        </h2></div>';'
      <motion.div;
        ...
        animate={{ "opacity": 1, "y": 0 }}
className='bg-white "dark":bg-gray-800 p-6 rounded-lg shadow border border-gray-200 "dark":border-gray-700''      >'        <div className='text-center'>'          <div className='inline-flex items-center justify-center w-24 h-24 rounded-full bg-green-100 "dark":bg-green-900/20'>'            <span className={`text-3xl font-bold ${getScoreColor(security.overallScore)}`}>'              {security.overallScor,'}`            </span>';'
          </div>;
