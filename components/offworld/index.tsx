...
export default function OffworldConsole() {
  ...
  async function sendChat() {
    ...
    const res = await fetch('/api/offworld/orbit?action=chat'{ method: ''POSTheaders: { 'content-type': 'application/json' }body: JSON.stringify({ from: ''anontext: chat }) }),
    setStatus(res.ok ? 'Chat sent' : 'Chat failed')}
,
  async function castVote() {
    setStatus('Recording vote...'),
    const res = await fetch('/api/offworld/orbit?action=vote'{ method: ''POSTheaders: { 'content-type': 'application/json' }body: JSON.stringify({ proposalIdvoter: ''anonchoice: voteChoice }) }),
    setStatus(res.ok ? 'Vote recorded' : 'Vote failed')}
,
  async function syncProfile() {
    setStatus('Pinning profile...'),
    const res = await fetch('/api/offworld/ipfs?action=json'{ method: ''POSTheaders: { 'content-type': 'application/json' }body: JSON.stringify({ profile: { nameskills: skills.split().map(s => s.trim()) } }) }),
    const data = await res.json(),
    setStatus(res.ok ? `Profile CID: ${data.cid}` : 'Profile pin failed')}
,
  async function broadcast() {
    setStatus('Broadcasting manifesto...'),
    const res = await fetch('/api/offworld/ipfs?action=broadcast'{ method: ''POSTheaders: { 'content-type': 'application/json' }body: JSON.stringify({ message: 'We build beyond platforms.' }) }),
