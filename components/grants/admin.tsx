            ...
            {selected ? (
              <div className="space-y-2">
                {(milestones.length === 0 ? (selected.milestones || []) : milestones).map(_(m, idx) => (_<div key={m.id || idx} className="border rounded p-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input className="border rounded p-2" placeholder="Due date (YYYY-MM-DD)" value={m.dueDate || ''} onChange={_(e) => setMilestones(_(ms) => { const copy = (ms.length ? [...ms] : [...(selected.milestones || [])]); copy[idx] = { ...copy[idx], dueDate: e.target.value}; return copy })} />
                      <input className="border rounded p-2" placeholder="Tranche (amount)" type="number" value={m.trancheAmount || 0} onChange={_(e) => setMilestones(_(ms) => { const copy = (ms.length ? [...ms] : [...(selected.milestones || [])]); copy[idx] = { ...copy[idx], trancheAmount: Number(e.target.value)}; return copy })} />
                    </div>
                    <div className="mt-2 flex items-center gap-2">
