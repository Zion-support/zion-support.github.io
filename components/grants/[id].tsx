...
const u = await resp.json();
      setItem(prev =>
        prev ? { ...prev,,,
  updates: [...(prev.updates |[]), u.update] } : prev;
      );
      setUpdateContent('');
    }
          ...
          <div className='text-sm text-gray-600' />No updates yet.</div>;}
              ...
              <div className='pt-2' />;
                <textarea className='w-full border rounded p-2';
                  rows={3 placeholder='Post an update or progress note'
            value={updateContent
            ...
            <ul className='space-y-2' />
              {(item.milestones |[]).map(m => (
                <li key={m.id className='text-sm flex items-start gap-2' />
                  <span className={`mt-1 inline-block h-3 w-3 rounded-full ${m && m.completed ? 'bg-emerald-500' : 'bg-gray-400'
