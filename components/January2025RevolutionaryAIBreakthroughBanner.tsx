import React from 'react';

const January2025RevolutionaryAIBreakthroughBanner: React.FC = () => {
  return (
    <section className="py-20 bg-gradient-to-br from-slate-900 via-purple-900 to-indigo-900 relative overflow-hidden">
      {/* Background Effects */}
      <div className="absolute inset-0 overflow-hidden opacity-30">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse"></div>
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-indigo-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
      </div>
      
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-purple-500/20 to-indigo-500/20 border border-purple-500/30 mb-8 animate-pulse">
            <span className="text-purple-400 font-bold text-2xl tracking-wider uppercase">
              🚀 BREAKING: JANUARY 2025 REVOLUTIONARY AI BREAKTHROUGHS
            </span>
          </div>
          
          <h1 className="text-5xl md:text-7xl font-extrabold mb-8 bg-gradient-to-r from-purple-400 via-indigo-400 to-blue-400 bg-clip-text text-transparent leading-tight">
            The Future of Enterprise AI is Here
          </h1>
          
          <p className="text-2xl md:text-3xl text-gray-300 mb-12 font-semibold max-w-4xl mx-auto">
            Revolutionary AI systems delivering unprecedented performance: 10,000x optimization, 
            99.8% uptime, and $2.7B in proven enterprise value
          </p>
        </div>

        {/* Key Metrics */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          <div className="bg-gradient-to-r from-purple-500/20 to-purple-600/20 backdrop-blur-sm rounded-2xl p-6 border border-purple-500/30 text-center">
            <div className="text-4xl font-extrabold text-purple-400 mb-2">10,000x</div>
            <div className="text-sm text-purple-300">Quantum Performance</div>
          </div>
          <div className="bg-gradient-to-r from-indigo-500/20 to-indigo-600/20 backdrop-blur-sm rounded-2xl p-6 border border-indigo-500/30 text-center">
            <div className="text-4xl font-extrabold text-indigo-400 mb-2">99.8%</div>
            <div className="text-sm text-indigo-300">Autonomous Uptime</div>
          </div>
          <div className="bg-gradient-to-r from-blue-500/20 to-blue-600/20 backdrop-blur-sm rounded-2xl p-6 border border-blue-500/30 text-center">
            <div className="text-4xl font-extrabold text-blue-400 mb-2">95%</div>
            <div className="text-sm text-blue-300">Automation Rate</div>
          </div>
          <div className="bg-gradient-to-r from-green-500/20 to-green-600/20 backdrop-blur-sm rounded-2xl p-6 border border-green-500/30 text-center">
            <div className="text-4xl font-extrabold text-green-400 mb-2">$2.7B</div>
            <div className="text-sm text-green-300">Proven Value</div>
          </div>
        </div>

        {/* Content Highlights */}
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-purple-500/50 transition-all duration-300 group">
            <div className="text-6xl mb-6 group-hover:scale-110 transition-transform duration-300">⚡</div>
            <h3 className="text-2xl font-bold text-white mb-4">Advanced AI Automation</h3>
            <p className="text-gray-300 mb-6">
              Complete enterprise transformation framework achieving 95% operational automation 
              with $2.3B in proven cost savings across Fortune 500 implementations.
            </p>
            <div className="space-y-2 mb-6">
              <div className="flex items-center gap-2 text-sm text-purple-300">
                <span className="w-2 h-2 bg-purple-500 rounded-full"></span>
                <span>95% Operational Automation</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-blue-300">
                <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
                <span>87% Cost Reduction</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-green-300">
                <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                <span>340% Efficiency Gains</span>
              </div>
            </div>
            <a 
              href="/blog/ai-2025-january-advanced-ai-automation-enterprise-transformation"
              className="inline-block bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 w-full text-center"
            >
              Master AI Automation →
            </a>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-indigo-500/50 transition-all duration-300 group">
            <div className="text-6xl mb-6 group-hover:scale-110 transition-transform duration-300">🌌</div>
            <h3 className="text-2xl font-bold text-white mb-4">Quantum AI Optimization</h3>
            <p className="text-gray-300 mb-6">
              Revolutionary quantum-enhanced algorithms solving previously intractable optimization 
              problems with 10,000x performance improvements and $340M in savings.
            </p>
            <div className="space-y-2 mb-6">
              <div className="flex items-center gap-2 text-sm text-indigo-300">
                <span className="w-2 h-2 bg-indigo-500 rounded-full"></span>
                <span>10,000x Performance</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-cyan-300">
                <span className="w-2 h-2 bg-cyan-500 rounded-full"></span>
                <span>94% Optimization Rate</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-purple-300">
                <span className="w-2 h-2 bg-purple-500 rounded-full"></span>
                <span>$340M in Savings</span>
              </div>
            </div>
            <a 
              href="/blog/ai-2025-january-quantum-ai-optimization-revolution"
              className="inline-block bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-400 hover:to-blue-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 w-full text-center"
            >
              Explore Quantum AI →
            </a>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-blue-500/50 transition-all duration-300 group">
            <div className="text-6xl mb-6 group-hover:scale-110 transition-transform duration-300">🎯</div>
            <h3 className="text-2xl font-bold text-white mb-4">Autonomous AI Systems</h3>
            <p className="text-gray-300 mb-6">
              Self-managing intelligence systems operating independently with 99.8% uptime 
              and autonomous decision-making delivering $2.7B in enterprise value.
            </p>
            <div className="space-y-2 mb-6">
              <div className="flex items-center gap-2 text-sm text-blue-300">
                <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
                <span>99.8% System Uptime</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-green-300">
                <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                <span>95% Automation Rate</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-orange-300">
                <span className="w-2 h-2 bg-orange-500 rounded-full"></span>
                <span>$2.7B Proven Value</span>
              </div>
            </div>
            <a 
              href="/blog/ai-2025-january-autonomous-ai-systems-enterprise-mastery"
              className="inline-block bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-400 hover:to-purple-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 w-full text-center"
            >
              Master Autonomous AI →
            </a>
          </div>
        </div>

        {/* Call to Action */}
        <div className="text-center">
          <div className="bg-gradient-to-r from-purple-500/10 to-indigo-500/10 backdrop-blur-sm rounded-3xl p-12 border border-purple-500/20">
            <h3 className="text-3xl md:text-4xl font-extrabold mb-6 bg-gradient-to-r from-purple-400 to-indigo-400 bg-clip-text text-transparent">
              Ready to Lead the AI Revolution?
            </h3>
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">
              Join the Fortune 500 companies already achieving unprecedented results with our revolutionary AI frameworks. 
              Get your free consultation and discover how to implement these breakthrough technologies.
            </p>
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <a
                href="/contact"
                className="bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-purple-500/50 transform hover:-translate-y-1"
              >
                Get Free AI Consultation →
              </a>
              <a
                href="/case-studies"
                className="bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-400 hover:to-blue-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-indigo-500/50 transform hover:-translate-y-1"
              >
                View Success Stories →
              </a>
              <a
                href="/services"
                className="border-2 border-purple-500 text-purple-400 hover:bg-purple-500 hover:text-white font-bold py-4 px-8 rounded-xl transition-all duration-300"
              >
                AI Implementation Services
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default January2025RevolutionaryAIBreakthroughBanner;