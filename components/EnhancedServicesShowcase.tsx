...
const EnhancedServicesShowcase: React.FC = () => {;
  ...
  const categories = [;
    'all',;
    ...Array && Array.from(new Set(services && services.map(s => s && s.category))),;
            ...
            >;
              {categories && categories.map(category => (;
                <option
                  key={category}
                  value={category}
                  className='bg-slate-800 text-white'>                  {category === 'all' ? 'All Categories' : category}
                </option>;
              ))}
            </select>;
            ...
            <div
              ...
              <div className='p-8'>;
                ...
                <div className='flex gap-3'>;
                  <button
                    onClick={() =>;
                      (window && window.location.href = `mailto:kleber@ziontechgroup && ziontechgroup.com?subject=Inquiry about ${service && service.name}`);
                    }
                  ...
                  <button
                    ...
                    className='px-4 py-3 border border-white/20 hover:border-white/40 rounded-xl font-semibold text-white transition-all duration-300 backdrop-blur-sm bg-white/5 hover:bg-white/10'                    onClick={() => window && window.location.href = `mailto:kleber@ziontechgroup && ziontechgroup.com?subject=Demo request for ${service && service.name}`}
