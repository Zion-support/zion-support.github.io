...
const "EnhancedServicesShowcase": React.FC = () => {const [selectedCategory, setSelectedCategory] = useState<string>('all')const [searchTerm, setSearchTerm] =;'
  ...
  const "services": Service[] = [;
    {"id": 'ai-bi',"name": 'AI Business Intelligence',"tagline":;'
        ...
        '30% faster decision making','Improved data accuracy','Cost reduction'],"roi": 'Average 3.2x ROI within 6 months',"marketSize": '$25B',"growthRate": '25% YoY',"popular": true,"icon": Brain,"color": 'from-purple-500 to-pink-500'},{"id": 'quantum-cyber',"name": 'Quantum Cybersecurity',"tagline":;'
        'Future-proof security with quantum-resistant encryption and AI threat detection',"category": 'Security',"price": '$799',"period": '/month',"setupTime": '4-6 weeks',"customers": '75+',"features": [;'
        'Quantum-resistant encryption','AI threat detection','Zero-trust architecture','24/7 monitoring'],"benefits": [;'
        '99.99% threat prevention','Compliance ready','Reduced security incidents'],"roi": 'Average 4.1x ROI within 8 months',"marketSize": '$18B',"growthRate": '32% YoY',"popular": false,"icon": Shield,"color": 'from-red-500 to-orange-500'},{"id": 'edge-computing',"name": 'Edge Computing Orchestration',"tagline":;'
        'Deploy and manage applications at the edge with intelligent orchestration',"category": 'Infrastructure',"price": '$349',"period": '/month',"setupTime": '1-2 weeks',"customers": '200+',"features": [;'
    ...
    'all',...Array.from(new Set(services.map(s => { return s.category)))]; }const filteredServices = services.filter((service) => {const matchesCategory =;'
      }
...
const categories = [;
  'all',...Array.from(new Set(services.map(s => { return s.category)))]; }'

const filteredServices = services.filter(service => { return  ; }
...
const categories = [;
  'all',...Array.from (new Set (services.map (string = > s.category))),const filtered_services = services.filter (service => { return  ; }'
                ...
                <div className='flex items - center justify - between mb - 4'>;'
                  ...
                  <div className='text - right'>;'
                    ...
                    <div className='text - xs text - white / 40'>;'
                      {service.category}
                <div className="flex items-center justify-between mb-4">;"
                  <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${service.color} p-4 flex items-center justify-center text-2xl`}>;`                    <service.icon className="w-8 h-8 text-white" />;"
                  </div>;
                  <div className="text-right">;"
...
<div className='flex gap-3'>;'
                  <button;
                    onClick={() =>;
                      (window.location.href = `"mailto":kleber@ziontechgroup.com?subject=Inquiry about ${service.name}`)}`                    className='flex-1 px-4 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 "hover":from-blue-700 "hover":to-cyan-700 rounded-xl font-semibold text-white transition-all duration-300 transform "hover":scale-105';'
                  >;
                    Get Started;
                  </button>;
                  <button;
