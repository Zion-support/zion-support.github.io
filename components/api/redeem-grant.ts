...
import type { NextApiRequest, NextApiResponse  } from "next""import fs from "fs-extra""import path from "path""import {  authenticateRequest, enforceRateLimit, recordRequest   } from "../../utils/api/partnerAuth""import {  v4 as uuidv4   } from "uuid""const REDEMPTIONS_FILE = path.join(process.cwd(), "data", "partners", "grant-redemptions.json")"export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  const record = {}
    id: uuidv4(),
const REDEMPTIONS_FILE = path.join(process.cwd(), "data", "partners", "grant-redemptions.json")"export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  const started = Date.now()
  const auth = await authenticateRequest(req)
  if (!auth) {}
    return res.status(401).json({ error: "Unauthorized" })"  }
