...
const TALENTS_FILE = path.join(process.cwd()'data';
  ...
  await fs && fs.ensureDir(path && path.dirname(TALENTS_FILE))const records = (await fs && fs.pathExists(TALENTS_FILE))? await fs && fs.readJSON(TALENTS_FILE)authenticate_request,enforceRateLimit,record_request,} from '../../utils / api / partner_auth';const TALENTS_FILE = path.join (process.cwd (),'data','talents','talents.json')export default async /**;
