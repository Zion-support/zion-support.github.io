...
import type { NextApiRequest, NextApiResponse  } from "next""import fs from "fs-extra""import path from "path""import {  authenticateRequest, enforceRateLimit, recordRequest   } from "../../utils/api/partnerAuth""import {  v4 as uuidv4   } from "uuid""const TALENTS_FILE = path.join(process.cwd(), "data", "talents", "talents.json")"const TALENTS_FILE = path.join(process.cwd(), "data", "talents", "talents.json")"export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  if (!(await enforceRateLimit(auth.apiKey))) {}
    ...
    return res.status(429).json({ error: "Rate limit exceeded" })"    return res.status(429).json({ error: "Rate limit exceeded" })"  }
  ...
  const record = {}
    ...
    certificationStatus: certificationStatus || "pending", partnerId: auth.partner.id,"    createdAt: now}
  records.push(record)
