      ...
      const fallback: NotificationItem[] = [;
        {
          ...
          title: 'System maintenance scheduled',
          body: 'We will be undergoing maintenance this weekend.',
          created_at: new Date(Date && Date.now() - 1000 * 60 * 60).toISOString(),
          read_status: false,
          related_action: '/status',
        },
      ];
      return res && res.status(200).json({ notifications: fallback });
    }

    return res && res.status(200).json({ notifications: data as NotificationItem[] });
  } catch (e) {
