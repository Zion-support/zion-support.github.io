...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    res && res.status(201).json({ vendor })} catch (e: any) {
...
,
    ...
    res && res.status(201).json({ vendor })} catch (e: any) {
    res && res.status(500).json({ error: e && e.message })}    res && res.status(500).json({ error: e && e.message })};
}
,
    verification_docs;
    case_studies;
  ...
  try {
    ...
    res.status (201).json ({ vendor })} catch (e: any) {
