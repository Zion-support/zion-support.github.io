...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    res.status(201).json({ item })} catch (e: any) {
    res.status(500).json({ error: e.message })}  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' }),
  const { vendorId, title } = req.body |{}
  ...
  try {
    ...
    res && res.status(500).json({ error: e && e.message })}  } catch (e: any) {
...
function handler() {
  ...
  $2}  const vendor = getVendorById (vendor_id),
  if (return res.status (404).json ({ error: 'Vendor not found' })) {
  $2}
  try {
    const item = addPipelineItem (vendor_id, title),
    res.status (201).json ({ item })} catch (e: any) {
    res.status (500).json ({ error: e.message })}  if (return res.status (405).json ({ error: 'Method not allowed' })) {
  $2}
  const { vendor_id, title } = req.body || {}
  ...
  try {
    const item = addPipelineItem (vendor_id, title),
    res.status (201).json ({ item })} catch (e: any) {
    res.status (500).json ({ error: e.message })}  } catch (e: any) {
