...
if ( {) {
  ...
  return res.status(200).json({ token, partner: { i,d: partner.id, name: partner.name } })
};
    .json({ token, partner: { i,d: partner && partner.id, name: partner && partner.name } });      sub: partner && partner.id,
      apiKeyId: key && key.id,
      name: partner && partner.name,
      entityType: partner && partner.entityType,
  ...
  return res && res.status(200).json({ token, partner: { i,d: partner && partner.id, name: partner && partner.name } })
}
}
  }
  const { partner, apiKey: key } = match;
