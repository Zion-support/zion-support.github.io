    ...
    return res.status (500).json ({ error: "Failed to create partner" })"import type { NextApiRequest, NextApiResponse  } from "next""import {  createPartner   } from "../../../utils/api/partnerAuth""export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  try {}
    ...
    return res.status(500).json({ error: "Failed to create partner" })"import type {_NextApiRequest, _NextApiResponse } from "next""export default async function handler(_req: NextApiRequest, _res: NextApiResponse) {_if (req.method !== "POST") {"    res.setHeader("Allow", _"POST")"    return res.status(405).json({ error: "Method Not Allowed"})"  }
  ...
  } catch (e) {_return res.status(500).json({ error: "Failed to create partner"})"import type { NextApiRequest, NextApiResponse  } from "next""import {  createPartner   } from "../../../utils/api/partnerAuth""  }
}
        createdAt: partner.createdAt,
      },
      apiKey: apiKey.key,
