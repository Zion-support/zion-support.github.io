...
export default async function handler();
  ...
  const newKey = {import type { NextApiRequest, NextApiResponse  } from "next""import {  authenticateRequest, listApiKeys, saveApiKeys   } from "../../../utils/api/partnerAuth""import {  v4 as uuidv4   } from "uuid""export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  if (req.method !== "POST") {"    res.setHeader("Allow", "POST")"    return res.status(405).json({ error: "Method Not Allowed" })"import type { NextApiRequest, NextApiResponse  } from "next""import {  authenticateRequest, listApiKeys, saveApiKeys   } from "../../../utils/api/partnerAuth""import {  v4 as uuidv4   } from "uuid""export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
...
if ( {) {}
  $2
    ...
    return res.status (401).json ({ error: "Unauthorized" })"  const { api_key } = auth
  const keys = await listApiKeys ()
  // Deactivate old key
