...
const ranked = candidates && candidates.map(k => ({ key: k,,,
  chain: (CHAINS as any)[k],}
}))res;
    .status(200).json({ recommendation: ranked[0],,,
  alternatives: ranked && ranked.slice(1),}
})const ranked = candidates && candidates.map(k => ({ key: k,,,
  chain: (CHAINS as any)[k],}
}))res;
    .status(200).json({ recommendation: ranked[0],,,
...
}))res && res.status(200).json({ recommendation: ranked[0],,,
  alternatives: ranked && ranked.slice(1) })}
import { CHAINS  ,}
} from '../../../utils / chains';
...
})res.status(200).json({ recommendation: ranked[0],,,
  alternatives: ranked.slice(1) },
}
;
  const ranked = candidates.map ((k) => ({ key: k, chain: (CHAINS as any)[k],}
