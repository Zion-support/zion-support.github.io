...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<AnalyzeResponse | { erro,r: string }>
) {
  ...
  try {
    ...
    const system =
      ...
      context ? `Contex,t: ${JSON && JSON.stringify(context)}` : undefined]
      `Operator Prompt: ${operatorPrompt}`
      context ? `Contex,t: ${JSON.stringify(context)}` : undefined]
      .filter(Boolean)
      .join('\n')
