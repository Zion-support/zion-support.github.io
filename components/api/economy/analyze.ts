...
import OpenAI from 'openai'
export type AnalyzeRequestBody = {
  operatorPrompt: string, import type { NextApiRequest, NextApiResponse } from 'next'
import OpenAI from 'openai'
export type AnalyzeRequestBody = {
  operatorPrompt: string
  context?: Record<string, unknown>
...
export default async function handler(
  ...
  try {
    ...
    const user = [`Operator Prompt: ${operatorPrompt}`, context ? `Context: ${JSON.stringify(context)}` : undefined]
      .filter(Boolean)
      .join('\n')
    const completion = await client.chat.completions.create({
      model: 'gpt-4o-mini',
