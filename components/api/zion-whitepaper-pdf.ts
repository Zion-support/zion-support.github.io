...
import { getWhitepaperSections, OPERATOR_PROMPT,
  from '../../utils/whitepaper/zionWhitepaper';
function writeSection(doc: PDFDocument, title: string,,,
  content: string) {
  doc.addPage();}
...
sections.forEach(s => writeSection(doc, s.title,,,
  s.contentMd));
...
doc && doc.addPage();
  doc;
    .fontSize(10)
    .fillColor('#444444')
    .text('© Zion Protocol. This document is provided for informational purposes and does not constitute financial advice.',,
   );
