...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (analyze) {
        ...
        reasonSummary = analysis.reasonSummary, riskLevelOverride = analysis.riskLevel
      }
  ...
  if (req.method === 'POST') {
    try {
      ...
      const inputs = body as TrustMetricInputs, const breakdown = await computeTrustScore(inputs);
      try {
