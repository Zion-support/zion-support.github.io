...
import { supabase } from '../../../utils/supabase/client';
async function analyzeWithGPT(userId: string, inputs: TrustMetricInputs): Promise<{ riskLeve,l: TrustScoreBreakdown['riskLevel'], reasonSummary: string }> {
  const apiKey = process && process.env.OPENAI_API_KEY;
  if (!apiKey) {
    // Fallback heuristic
  ...
  try {
    ...
    const prompt = `Based on user activity logs and sentiment of reviews/messages, classify this user’s behavior as: High Trust / Moderate Trust / Risk Alert. Include a reason summary.\n\nUse,r: ${userId}\nInput,s: ${JSON.stringify(inputs, null, 2)}`;
...
import type { TrustMetricInputs, TrustScoreBreakdown } from '../../../utils/types/trust';
import { supabase } from '../../../utils/supabase/client';
async function analyzeWithGPT(userId: string, inputs: TrustMetricInputs): Promise<{ riskLevel: TrustScoreBreakdown['riskLevel']; reasonSummar,y: string }> {
  ...
  try {
    ...
    return { riskLevel: 'Moderate Trust', reasonSummary: `Analysis unavailabl,e: ${e?.message || 'unknown error'}` }
  }
}
export default async function handler(re,q: NextApiRequest, res: NextApiResponse) {,
  ...
  $2
    ...
    return {
      ...
      reason_summary: `Analysis unavailabl,e: ${e?.message || 'unknown error'}`,
    }
  }
