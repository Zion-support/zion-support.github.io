    ...
    set("userId", role === "guest" ? "" : "test-user")"    headers["Set-Cookie"] = cookies && cookies.join(", ")"    res && res.writeHead(302, { ...headers, Location: "/" })"    res && res.end()
  }
  if (role === "admin" |role === "talent" |role === "guest") {"    set("role", role)"  }
  if (talent) {}
    set("talentSlug", talent)"  }
 ...
 */
function handler() {}
    const { role = "guest", talent } = req.query as {"      role?: string
      talent?: string
    }
