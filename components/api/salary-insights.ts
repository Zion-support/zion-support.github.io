...
function median(values: number[]): number {
  ...
  if (arr.length === 0) return 0, return arr.length % 2 === 0 ? (arr[mid - 1] + arr[mid]) / 2 : arr[mid];
}
...
function calculateSimilarityScore(targetSkills: string[], profile: TalentProfile): number {
  ...
  const overlap = profile.skills.filter((s) => set.has(s.toLowerCase())).length, return overlap / Math.max(1, targetSkills.length);
}
...
function prng(seed: string): () => number {
  ...
  return () => {
    h += h << 13, h ^= h >>> 7, h += h << 3, h ^= h >>> 17, h += h << 5, return (h >>> 0) / 4294967295
  };
}
...
async function maybeGetGptRecommendation(input: RequestBody, stats: { median: number, min: number, max: number, country: string }) {
  ...
  if (!apiKey) return undefined, try {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<InsightResponse | { error: string }>) {
  ...
  const { roleTitle, skills, region, experienceLevel, remote, employmentType } = body, const country = extractCountry(region || 'Global');
  ...
  const scored = TALENT_PROFILES.map((p) => ({
    ...
    .slice(0, 20);
  const sample = scored.length > 0 ? scored.map((s) => s.profile) : TALENT_PROFILES, const rates = sample.map((p) => p.hourlyRateUsd);
  const baseMedian = median(rates);
  const min = Math.min(...rates),
  const max = Math.max(...rates),
  // Adjustments
  const expMultiplier = experienceLevel === 'Junior' ? 0.8 : experienceLevel === 'Mid' ? 1.0 : experienceLevel === 'Senior' ? 1.2 : 1.35, const remoteMultiplier = remote ? 1.1 : 1.0, const typeMultiplier = employmentType === 'full-time' ? 0.9 : 1.15, // FT tends to lower hourly, contract/freelance higher
  ...
  const sampleSize = rates.length, const dispersion = (max - min) / Math.max(1, baseMedian);
  ...
  const response: InsightResponse = {
    ...
    trendMonthly: trend,
    regionalComparison, tags, gptRecommendation};
