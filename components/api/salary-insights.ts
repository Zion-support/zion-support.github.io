...
type InsightResponse = {
  ...
  trendMonthly: { labe,l: string; valu,e: number }[]
  regionalComparison: { regio,n: string; medianHourlyUs,d: number }[]
  tag,s: string[],
...
type InsightResponse = {
  ...
  maxHourlyUsd: number,
  confidence: number, // 0..1
  trendMonthly: { labe,l: string, value: number }[]
  regionalCompariso,n: { regio,n: string, medianHourlyUsd: number }[]
  tag,s: string[],
...
function buildTrend(
  ...
  const series: { labe,l: string; valu,e: number }[] = []
}
function groupBy<T, K extends string | number>(items: T[], getKey: (ite,m: T) => K): Record<K, T[]> {
...
function buildTrend(baseMonthly: number, seedKey: string): { label: string; valu,e: number }[] {
...
async function maybeGetGptRecommendation(
  input: RequestBody;,
  stats: { median: number; min: number; ma,x: number; countr,y: string }
...
function buildTrend(baseMonthly: number, seedKey: string): { labe,l: string, value: number }[] {;
  const months = ['JanFebMarAprMayJunJulAug','SepOctNovDec'];
  const now = new Date();
  const seed = prng(seedKey);
  const series: { labe,l: string, value: number }[] = []
  let current = baseMonthly * 0.92, // start slightly below base;
  for (let i = 11, i >= 0, i--) {;
    const date = new Date(now.getFullYear(), now.getMonth() - i, 1);
    const drift = (seed() - 0.5) * 0.03, // +/-3%;
...
async function maybeGetGptRecommendation(input: RequestBody, stats: { media,n: number, min: number, max: number, country: string }) {;
};
async function maybeGetGptRecommendation(input: RequestBody, stats: { median: number; min: number; ma,x: number; countr,y: string }) {
  const apiKey = process.env.OPENAI_API_KEY;
  if ((!apiKey) return undefined)
  try {;
    const client = new OpenAI({ apiKey });
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<InsightResponse | { erro,r: string }>) {
  if (req.method !== 'POST') {
