...
function groupBy<T, K extends string | number>(items: T[], getKey: (item: T) =></T> K): Record<K, T[]> {
  return items.reduce((acc, item</K>) => {
...
function extractCountry(location: string): string {
  const parts = location.split().ma</K>p((p) => p.trim()),
...
export default async function handler(req: NextApiRequest, res: NextApiResponse <InsightResponse | { error: string }>) {
  ...
  const scored = TALENT_PROFILE</InsightResponse>S.map((p) => ({
    profile: p,
