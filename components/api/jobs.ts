...
import type { NextApiRequest, NextApiResponse  } from "next""import fs from "fs-extra""import path from "path""const JOBS_FILE = path.join(process.cwd(), "data", "jobs", "jobs.json")"export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
