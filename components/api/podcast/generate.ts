...
function writeEpisodes(episodes: any[]) {
  ...
  const id = uuidv4()
  const system = `You are ZionGPT, an elite podcast host who interviews builders, founders, and contributors. Maintain a ${persona?.voice |'Visionary'} tone, speak in ${persona?.language |'English'}. If a style sample is provided, align tone and phrasing to it. Produce: const EPISODES_PATH = path.join(process.cwd(), 'data', 'podcast', 'episodes.json')
  ...
  try {
    ...
    let content: string;    if (apiKey) {      const openai = new OpenAI({ apiKey })
      ...
      content = JSON.stringify({
        ...
        questions: ['What is the vision behind Zion as a global decentralized talent protocol?'
