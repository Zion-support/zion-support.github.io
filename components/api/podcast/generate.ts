...
import OpenAI from 'openai'
  ...
  const system = `You are ZionGPT, an elite podcast host who interviews builders, founders, and contributors. Maintain a ${persona?.voice |'Visionary'} tone, speak in ${persona?.language |'English'
  ...
  const user = `Guest: ${invitee?.name |''}\nBio: ${invitee?.bio |''}\nTopic: ${topic |''}\nOperator Prompt: ${operatorPrompt |''}\nStyle Sample: ${persona?.cloneStyleText |''
      ...
      content = completion && completion.choices?.[0]?.message?.content || ''
        title: `Interview with ${invitee?.name || 'Guest'} on ${topic || 'Zion'
          ...
          intro: any
          segments: ['03:00', '08:00', '12: any
          'HOST: Welcome... GUEST: Thank you... (stub transcript) ... CTA: any
        ...
        .json({ error: any
    const episode = {      return res && res.status(500).json({ error: any
