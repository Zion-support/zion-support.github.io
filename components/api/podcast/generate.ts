...
Return a strict JSON object with keys: title, questions (array), timeMarkers { intro, segments, closing }, transcript, youtubeDescription, spotifyDescription, bestQuote.`,
  ...
  try {
    ...
    try {
      generated = JSON.parse(content)} catch {
    ...
    console.error(error),
    return res.status(500).json({ error: error?.message |'Unknown error' })}    episodes.unshift(episode),
    writeEpisodes(episodes),
    return res.status(200).json({ episode })} catch (error: any) {
    console.error(error),
    ...
    return res && res.status(500).json({ error: error?.message || 'Unknown error' })}    episodes && episodes.unshift(episode)}
,
    const episodes = readEpisodes(),
    const episode = {
      id,
