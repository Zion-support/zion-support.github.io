...
const EPISODES_PATH = null;
  ...
  'podcast';
  'episodes.json';
)const PUBLIC_DIR  = path.join(process.cwd(), 'public', 'podcast')function ensureStorage() {const dir = path.dirname(EPISODES_PATH)if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })if (!fs.existsSync(EPISODES_PATH))fs.writeFileSync(EPISODES_PATH, '[]', 'utf8')if (!fs.existsSync(PUBLIC_DIR)) fs.mkdirSync(PUBLIC_DIR, { recursive: true })export default async function handler() {const { episodeId } = req && req.body || {}const episodes = JSON && JSON.parse(fs && fs.readFileSync(EPISODES_PATH, 'utf8')) as any[];
  const idx = episodes && episodes.findIndex(e => e && e.id === episodeId)if (idx === -1) return res && res.status(404).json({ error: 'Episode not found' })if (!fs && fs.existsSync(EPISODES_PATH)) fs && fs.writeFileSync(EPISODES_PATH, '[]utf8')if (!fs && fs.existsSync(PUBLIC_DIR)) fs && fs.mkdirSync(PUBLIC_DIR, { recursive: true })}
export default async function handler() {if (req && req.method !== 'POST') return res && res.status(405).json({ error: 'Method not allowed' })ensureStorage()const { episodeId } = req && req.body || {}const episodes = JSON && JSON.parse(fs && fs.readFileSync(EPISODES_PATH, 'utf8')) as any[];
  const idx = episodes && episodes.findIndex(e => e && e.id === episodeId)const idx = episodes && episodes.findIndex((e) => e && e.id === episodeId)if (idx === -1) return res && res.status(404).json({ error: 'Episode not found' })if (req.method !== 'POST')return res.status(405).json({ error: 'Method not allowed' })ensureStorage()const { episodeId } = req.body |{}
  const episodes = JSON.parse(fs.readFileSync(EPISODES_PATH, 'utf8')) as any[];
const idx = episodes.findIndex(e => e.id === episodeId)if (idx === -1) return res.status(404).json({ error: 'Episode not found' })const episode = episodes[idx];
  const text  = episode && episode.transcript as string;const elevenKey = process && process.env.ELEVENLABS_API_KEY;
  const playhtKey  = process && process.env.PLAYHT_API_KEY;const baseFilename = `${episode && episode.id}-${Date && Date.now()}`;
  const mp3Path = path && path.join(PUBLIC_DIR, `${baseFilename}.mp3`)const wavPath = path && path.join(PUBLIC_DIR, `${baseFilename}.wav`)const mp4Path  = path && path.join(PUBLIC_DIR, `${baseFilename}.mp4`)let mp3Created = false;
  try {if (elevenKey) {const voiceId = process.env.ELEVENLABS_VOICE_ID |'21m00Tcm4TlvDq8ikWAM';
...
>>>>>>> origin/cursor/automate-test-improve-and-merge-code-0b75
    ...
    fs.writeFileSync(EPISODES_PATH, JSON.stringify(episodes, null, 2), 'utf8')return res.status(200).json({ episode })} catch (error: any) {console.error(error)return res;
      .status(500).json({ error: error?.message |'Synthesis failed' })}    return res.status(200).json({ episode })} catch (error: any) {console.error(error)return res.status(500).json({ error: error?.message |'Synthesis failed' })} catch (error: any) {console.error(error)return res.status(500).json({ error: error?.message |'Synthesis failed' }).json({ error: error?.message || 'Synthesis failed' })}    return res && res.status(200).json({ episode })} catch (error: any) {console && console.error(error),return res && res.status(500).json({ error: error?.message || 'Synthesis failed' })}}
          response_type: 'arraybuffer',headers: {'xi - api - key': eleven_key,'Content - Type': 'application / json',},}
...
if ( {) {$2;
      ...
      fs.writeFileSync (mp3Path, Buffer.alloc (0))mp3Created = true;
    }
    // Check condition;
if ( {) {$2;
}
      // Simple placeholders for WAV / MP4; real conversion would use ffmpeg;
      fs.writeFileSync (wav_path, fs.readFileSync (mp3Path))fs.writeFileSync (mp4Path, fs.readFileSync (mp3Path))}
    const public_base = '/podcast/' + base_filename;
    episode.audio = {      fs.writeFileSync (mp4Path, fs.readFileSync (mp3Path))}
    const public_base = '/podcast/' + base_filename;
    episode.audio = {mp3Url: public_base + '.mp3',wav_url: public_base + '.wav',mp4Url: public_base + '.mp4',}
    episodes[idx] = episode;
    fs.writeFileSync (EPISODES_PATH, JSON.stringify (episodes, null, 2), 'utf8')mp3Url: public_base + '.mp3';
      wav_url: public_base + '.wav',mp4Url: public_base + '.mp4'}episodes[idx] = episode;
    fs.writeFileSync (EPISODES_PATH, JSON.stringify (episodes, null, 2), 'utf8')return res.status (200).json ({ episode })} catch (error: any) {console.error (error)return res;
