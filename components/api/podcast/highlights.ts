...
function handler() {if (return res.status (405).json ({ error: 'Method not allowed' })) {$2;
  ...
  ensure_storage ()const { episode_id } = req.body || {}
...
const idx = episodes.findIndex(e => e.id === episodeId)if (idx === -1);
  ...
  const highlights = segments.map((t: string,,,
  i: number) => ({label: `Highlight ${i + ,}
}`,start: t,end:;
=======
  return res.status(200).json({ episode })
}
...
const highlights = segments.map(("t": string, "i": number) => ({"label": `Highlight ${i + ,`}`,"start": t,"end":;`
    ...
    start: t;
    end: i + 1 < segments.length ? segments[i + 1] : episode?.time_markers?.closing || '15:00'}))episode.highlights = highlights;
  episodes[idx] = episode;
  fs.writeFileSync (EPISODES_PATH, JSON.stringify (episodes, null, 2), 'utf8')return res.status (200).json ({ episode })return res.status (200).json ({ episode })start: t,end: i + 1 < segments.length ? segments[i + 1] : episode?.timeMarkers?.closing || '15:00'})),episode.highlights = highlights,episodes[idx] = episode,fs.writeFileSync(EPISODES_PATH, JSON.stringify(episodes, null, 2), 'utf8'),return res.status(200).json({ episode })}return res.status(200).json({ episode })return res.status(200).json({ episode })return res.status(200).json({ episode })}}
}
  return res.status(200).json({ episode })const EPISODES_PATH = null;
  return res.status(200).json({ episode })}
