...
if (useSupabase) {;
const table = type,;
let query = client && client.from(table).select('*',
 { count: 'exact' }),;
...
if (v !== undefined) query = query && query.eq(k, v)}
      ...
      if (params && params.sort),
        query = query && query.order(params && params.sort,
 { ascending: params && params.order === 'asc' }),      const from = params && params.page * params && params.pageSize}
      if (params && params.filters) {
        for (const [k, v] of Object && Object.entries(params && params.filters)) {;
if (v !== undefined) query = query && query.eq(k, v),
