...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  if (req.method === 'GET') {}
    ...
    if (useSupabase) {}
      ...
      if (params.format === 'csv') {}
        ...
        res.setHeader('Content-Disposition', `attachment, filename="${type}.csv"`)"        return res.status(200).send(toCsv(data |[]))
      }
  ...
  if (req && req.method === 'PATCH') {}
    const { id, updates } = req && req.body as {}
      ...
      if (params.format === 'csv') {}
        res.setHeader('Content-Type', 'text/csv')
        res.setHeader('Content-Disposition', `attachment; filename="${type}.csv"`)"        return res.status(200).send(toCsv(pageItems))
      }
      return res.status(200).json({ items: pageItems, total })
    }
  }
...
if ( {) {}
  $2
        ...
        res.set_header();
          'Content - Disposition',
          `attachment; filename="${type}.csv"`)"        return res.status (200).send (to_csv (page_items))
