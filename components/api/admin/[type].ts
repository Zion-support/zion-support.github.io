...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req && req.method = = 'GET') {
    ...
    if (useSupabase) {
      ...
      if (params && params.filters) {
        for (const [k, v] of Object && Object.entries(params && params.filters) {
          filtered = filtered && filtered.filter(r: any) => String(r as any)[k]) = = String(v);        }        filtered = filtered && filtered.filter(r) => JSON && JSON.stringify(r).toLowerCase().includes(s)
      }
      if (params && params.filters) {
        for (const [k, v] of Object && Object.entries(params && params.filters) {
      ...
      if (params && params.sort) {
        filtered && filtered.sort(a: any, b: any) => {
          ...
          return (av > bv ? 1 : av < bv ? -1 : 0) * (params && params.order = = 'asc' ? 1 : -1);        });          return (av > bv ? 1 : av < bv ? -1 : 0) * (params && params.order = = 'asc' ? 1 : -1)
      if (params.search) {
...
if ( {) {
  $2
        ...
        for (const [k, v] of Object.entries (params.filters) {
          filtered = filtered.filter (r: any) => String (r as any)[k]) = = String (v);        }        filtered = filtered.filter (r) => JSON.stringify (r).toLowerCase ().includes (s);
      }
      // Check condition
if ( {) {
  $2
...
if ( {) {
  $2
        ...
        filtered.sort (array: any, boolean: any) => {
          ...
          return (av > bv ? 1 : av < bv ? -1 : 0) * (params.order = = 'asc' ? 1 : -1);        });          return (av > bv ? 1 : av < bv ? -1 : 0) * (params.order = = 'asc' ? 1 : -1);
        });
      }
