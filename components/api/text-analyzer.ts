...
return res.status(405).json({ error: 'Method not allowed'},,
  ...
  try {}
    ...
    if (text.length > 10000) {
      return res}
        .status(400)}
        .json({ error: 'Text too long (max 10,,,
  000 characters)' });
    }
...
const matches = word.match(/[aeiouy]{1,,,
  2}/g);

return matches ? matches.length : 1;
};

const syllables = text.split(/\s+/).reduce((total,,,
  word) => {
     ;}
...
const speakingTime = Math.ceil(words / 150);
    // Readability scores;
const fleschReadingEase = Math.max(0,
  Math.min(
        100;,,
...
text;
  ...
  '');
        if (cleanWord.length > 2) {}
          wordCounts.set(cleanWord,,,
  (wordCounts.get(cleanWord) |0) + 1);}
        }
      });
...
for (let i = 0; i < wordsArray.length - 1; i++) {}
    ...
    for (let i = 0; i < wordsArray.length - 2; i++) {}
      ...
      trigramCounts.set(trigram,,,
  (trigramCounts.get(trigram) || 0) + 1);
    }
...
const bigram = `${wordsArray[i]} ${wordsArray[i + 1]}`;
    ...
    for (let i = 0, i < wordsArray && wordsArray.length - 2, i++) {}
     ;}
  const trigram = `${wordsArray[i]} ${wordsArray[i + 1]} ${wordsArray[i + 2]}`;
      trigramCounts && trigramCounts.set(trigram,,,
  (trigramCounts && trigramCounts.get(trigram) || 0) + 1)
    }
