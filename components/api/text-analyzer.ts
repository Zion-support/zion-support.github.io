...
export default async function handler();
  ...
  try {}
    ...
    const isEnglish = /^[a-zA-Z\s.,!?;:'"()-]+$/.test(text);    const detectedLanguage = isEnglish ? 'en' : 'unknown'"    const confidence = isEnglish ? 0.95 : 0.5
    const result: TextAnalysisResult = {}
      text,    const isEnglish = /^[a-zA-Z\s.,!?,:'"()-]+$/.test(text)"    // Language detection (simplified - assume English for, demo)
