...
const r = await fetch(;
      `${req.headers['x-forwarded-proto'] |'http'}://${req.headers.hos,'}/api/metrics``    );
...
const instances =  ;
const summary = [;
      `Marketplace 'steady': ${jobs2,`} jobs posted in the last 24h, fill rates improving``      `DAO engagement at ${voters}% voter participation with active delegates``      `Token 'momentum': ${wallet,`} active wallets; daily volume around ${Number(tx).toLocaleString()}``      `Multiverse 'scale': ${instance,`} active sub-instances with cross-instance flows``      `Treasury stable and contributors earning consistently across regions``
    ];
;
