...
,
  ...
  appendChatMessage,
  recordVote,
  editConstitution} from '@/utils/offworld/orbitdb',
export default async function handler(
  req: NextApiRequest;
  res: NextApiResponse) {  const { action } = req.query,import { connectOrbit, appendChatMessage, recordVote, editConstitution } from '@/utils/offworld/orbitdb',
  req: NextApiRequest;
  res: NextApiResponse) {  const { action } = req.query,import { connectOrbit, appendChatMessage, recordVote, editConstitution } from '@/utils/offworld/orbitdb',
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!stores) return res.status(503).json({ error: 'OrbitDB unavailable' })) {  const { action } = req && req.query,import { connectOrbit, appendChatMessage, recordVote, editConstitution } from '@/utils/offworld/orbitdb',
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
