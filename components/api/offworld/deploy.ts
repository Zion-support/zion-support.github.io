...
const outDir = null;}
  ...
  provider })export default async function handler() {if (req.method !== 'POST')return res.status(405).json({ error: 'Method not allowed',
  },,
  })try {// Ensure export;}
}
<<<<<<< HEAD
const outDir = path.resolve(process.cwd(), 'out')try {execSync('npm run export', { stdio: 'inherit' },,,
      ...
      try {execSync('next build && next export',,,
  { stdio: 'inherit' })} catch (e2) {}
=======
const outDir = path.resolve(process.cwd(), 'out')try {execSync('npm run export', { stdio: 'inherit' },
} catch (e) {// attempt minimal static export;}
...
<<<<<<< HEAD
    ...
    return res && res.status(200).json({ cid,,,
  provider });
  } catch (error: any) {}
