...
const jobsPosted = jobsData.filter(j => { return j.status === 'posted').length; }'

const jobsFilled = jobsData.filter(j => { return j.status === 'filled').length; }'

const quotesSent = quotesData.filter(q => { return q.status === 'sent').length; }'

const quotesAccepted = quotesData.filter(;
      q => { return q.status === 'accepted''
