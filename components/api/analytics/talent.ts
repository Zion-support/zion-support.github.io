...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const talentId = (req.query.talentId as string) || null, const [viewsR, invitesR, appsR, tagsR] = await Promise.allSettled([
    ...
    const successRate = ((apps.filter(a => a.status === 'accepted').length || 3) / Math.max(jobApplications, 1)) * 100, const tagCounts: Record<string, number> = {};
    (tags.length ? tags : [
      { tag: 'react' },
      { tag: 'node' },
      { tag: 'ai' },
    ...
    res.status(200).json({
      profileViews, quoteInvites, jobApplications, successRate, topTags: Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([label, value]) => ({ label, value }))})
  } catch (e) {
    res.status(200).json({
      profileViews: 27,
      quoteInvites: 6,
