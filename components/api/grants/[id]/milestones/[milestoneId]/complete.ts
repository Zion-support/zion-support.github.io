...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  ms[idx].completed = true, ms[idx].completedAt = new Date().toISOString();
  const tranche = ms[idx].trancheAmount || 0, existing.fundsReleased = (existing.fundsReleased || 0) + tranche, existing.milestones = ms, existing.updatedAt = new Date().toISOString();
  writeGrant(existing);
