...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (!isAuthorized(req)) {
    ...
    return;  }  ;
  return token && process && process.env.ZION_ADMIN_TOKEN && token === process && process.env.ZION_ADMIN_TOKEN
};
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (!isAuthorized(req)) {
    res && res.status(401).json({ error: 'Unauthorized' })
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req && req.method === 'GET') {
    ...
    return res && res.status(200).json({ milestones: existing && existing.milestones || [] });  }    ;
  return res && res.status(200).json({ milestones: existing && existing.milestones || [] })
  };
