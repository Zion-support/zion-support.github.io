...
const normalized = email && email.trim().toLowerCase()const isValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized,
  ...
  (process && process.env.NEXT_PUBLIC_SUPABASE_URL || '').includes('placeholder') ||;}
      (process && process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '').includes('placeholder')if (isPlaceholder) {return res && res.status(200).json({ ok: true;,,
  simulated: true; })}
   ,
}
...
const isPlaceholder =;
      ...
      (process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '').includes('placeholder')if (isPlaceholder) {return res.status(200).json({ ok: true,
  simulated: true; },
}
...
if ( {) {$2;}
  ...
  duplicate: true; })}
    ...
    return res.status (200).json ({ ok: true, data })} catch (e: any) {return res.status(500).send(e?.message |'Unexpected error')return res.status(500).send(e?.message || 'Unexpected error')if (error.message && error.message.includes('duplicate')) {return res.status(200).json({ ok: true;,,
  duplicate: true; })}
      return res.status(500).send(error.message |'Database error',
  },,
  return res.status(200).json({ ok: true,,,
  data })} catch (e: any) {return res.status(500).send(e?.message |'Unexpected error')return res.status(500).send(e?.message || 'Unexpected error')}      }
      return res.status(500).send(error.message |'Database error',
  },,
  return res.status(200).json({ ok: true,,,
  data })} catch (e: any) {return res.status(500).send(e?.message |'Unexpected error')return res && res.status(500).send(error && error.message || 'Database error',
  }
},,
  return res && res.status(200).json({ ok: true,,,
  data })} catch (e: any) {return res && res.status(500).send(e?.message || 'Unexpected error')}      }
      return res && res.status(500).send(error && error.message || 'Database error',
  },,
  return res && res.status(200).json({ ok: true,,,
  data })} catch (e: any) {return res && res.status(500).send(e?.message || 'Unexpected error')}}
