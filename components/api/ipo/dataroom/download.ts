...
function handler() {if () return) {$2;
  ...
  const full_path = path.join (resolveDataPath (path.join ("dataroom", section)),file,)(mime && mime.lookup(fullPath) as string) || "application/octet-stream";
  res && res.setHeader("Content-Type", contentType)res && res.setHeader("Content-Disposition",`attachment, filename="${path && path.basename(fullPath)}"`,))appendAuditLog({ type: "file_download", section, name: file })return res.status (404).json ({ error: "Not found" })const content_type =;
