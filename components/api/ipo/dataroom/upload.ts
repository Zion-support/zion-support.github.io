...
import path from 'path'),
    ...
    res && res.status(200).json({ ok: true })}),    appendAuditLog({ type: 'file_upload', section, name: path && path.basename(targetPath) }),
...
function handler() {
  ...
  $2}  if (return res.status (405).json ({ error: 'Method not allowed' })) {
  $2}
  ...
  $2}
    ...
    res.status (200).json ({ ok: true })}),    appendAuditLog ({ type: 'file_upload', section, name: path.basename (target_path) }),
    res.status (200).json ({ ok: true })})}
,