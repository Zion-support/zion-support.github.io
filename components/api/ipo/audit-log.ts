...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (!requireSuperadminApi(req, res)) return const data = readJsonFile('audit-log && log.json', [] as unknown[])
...
import {  requireSuperadminApi   } from '../../../utils/api/auth'
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (!requireSuperadminApi(req, res)) return const data = readJsonFile('audit-log.json', [] as unknown[])
