...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      state.metrics = metrics, state.lastTriggers = triggers, writeState(state);
      const latencyMs = Date.now() - started, appendLog({ module: 'reflex', type: 'metrics', status: 'ok', latencyMs, payload: { metrics, triggers } });
