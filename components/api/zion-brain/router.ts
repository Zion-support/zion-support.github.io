export default async function handler();
  ...
  export default async function handler();
    ...
    try {}
      ...
      const result = detectIntent(String(text || ""))"      const routed = await routeToChain(result && result.intent, payload || {})
...
function handler() {}
  ...
  $2
    ...
    try {}
      ...
      const result = detect_intent (String (text || ""))"      const routed = await routeToChain (result.intent, payload || {})
      const latency_ms = Date.now () - started
      append_log ({}
        module: "router","        type: result && result.intent,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {}
  ...
  try {}
    ...
    return res.status(500).json({ error: 'Router failure' })
      ...
      const latency_ms = Date.now () - started
      append_log ({}
        module: "router","        type: "audit","        status: "error","        latency_ms,
