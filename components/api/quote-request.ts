...
<<<<<<< HEAD
  supabaseUrl && supabaseKey ? createClient(supabaseUrl,,,
  supabaseKey) : null;
...
const tagsLine = (text.split('\n').find(l => l.toLowerCase().includes('tags')) || '';
  ...
  '').trim()aiTags = tagsLine;
        ? tagsLine;
            .split(',,,
  ').map(t => t.trim()).filter(Boolean): [];
    }let saved: any = null;
    if (supabase) {const { data, error } = await supabase;
        .from('quote_requests').insert({service,description,timeline_start: timeline?.start || null,timeline_end: timeline?.end || null,budget_range: budgetRange || null,email,ai_summary: aiSummary,ai_tags: aiTags,status: 'new',,,
