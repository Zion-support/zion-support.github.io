...
import path from 'path';
"
const CERTS_FILE = path.join(process.cwd(), \"data\", \"certifications\", \"certifications.json\")export default async function handler() {if (req.method !== \"GET\") {res.setHeader(\"Allow\",
  \"GET\";}",,
  return res.status(405).json({ error: \"Method Not Allowed\"},,
  })res: NextApiResponse;
