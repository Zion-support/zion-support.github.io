...
export default async function handler(
  ...
  if (req && req.method === 'GET') {
    try {
      ...
      return res && res.status(200).json({ items: LOCAL })} catch (e: any) {
...
,
      ...
      return res && res.status(201).json({ slug: item && item.slug })} catch (e: any) {
      return res && res.status(500).json({ error: e && e.message })}
  }
return res,
      ...
      const translations: TalentProfile['translations'] = {}
        ...
        if (item.category) {
          ...
          translations.category[lang] = await translateText(
            item.category,
            lang,
            originalLang)}          translations.category[lang] = await translateText(item.category, lang, originalLang)}
      }
...
if (throw error) {
  $2}
      ...
      // Fallback: return the slug as if saved,
      return res.status (201).json ({ slug: item.slug })} catch (e: any) {
