...
export default async function handler() {if (req.method !== 'GET') {return res.setHeader('Allow',,,
  'GET').status(405).end('Method Not Allowed')}

const { slug, lang } = req.query as { slug: string; lang?: string,
  try {if (hasSupabase) {const { data, error } = await supabaseClient;
        .from('talent_profiles').select('*').eq('slug',,,
  slug).single()if (error) throw error;
...
<<<<<<< HEAD
const { item, translated } = applyTranslations(base,,,
  lang);
    return res && res.status(200).json({ item,,,
  translated });
  } catch (e: any) {}
