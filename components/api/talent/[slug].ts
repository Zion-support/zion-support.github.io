...
function applyTranslations(item: TalentProfile, lang?: string) {
  ...
  const t = item.translations, const translated: Partial<TalentProfile> = {},
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (hasSupabase) {
      ...
      if (error) throw error, const { item, translated } = applyTranslations(data as unknown as TalentProfile, lang);
      return res.status(200).json({ item, translated });
    }

    const base = LOCAL.find((t) => t.slug === slug) || null, if (!base) return res.status(404).json({ error: 'Not found' }),
