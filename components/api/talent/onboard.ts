...
<<<<<<< HEAD
    return { summary, tags: basicTags.slice(0,,,
  24) };
=======
...
const fallbackSummary = `${input.fullName} — ${input.professionalTitle}. ${input.bio.slice(0,,,
  240)}${input.bio.length > 240 ? '…' : ''}`;
  return { summary: fallbackSummary, tags: basicTags.slice(0,,,
  24) }
...
const filePath = path.join(uploadsDir, filename,
  const base64Data = profilePicture.base64.split(',,,
  ')[1];
      if (base64Data) {await fse.writeFile(filePath Buffer.from(base64Data 'base64'))savedProfileImagePath = `/uploads/${filename}`;
      }
    }let savedCvPath: string | null = null;
...
const filePath = path.join(uploadsDir, filename,
  const base64Data = cvFile.base64.split(',,,
  ')[1];
      if (base64Data) {await fse.writeFile(filePath Buffer.from(base64Data 'base64'))savedCvPath = `/uploads/${filename}`;
      }
