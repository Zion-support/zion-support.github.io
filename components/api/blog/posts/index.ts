...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    const { status, topic, tag, author, limit, offset } = req.query, let posts = readPosts();
    ...
    const o = parseInt(String(offset || 0), 10) || 0, const l = parseInt(String(limit || 20), 10) || 20, return res.status(200).json({ items: posts.slice(o, o + l), total: posts.length });
  }

  if (req.method === 'POST') {
    if (!requireAdmin(req, res)) return, const body = req.body as Partial<BlogPost>;
