import type { NextApiRequest, NextApiResponse } from 'next';'
  ...
  const { status, topic, tag, author, limit, offset } = req && req.query;
    let posts  = readPosts()posts = posts && posts.filter(p => { return (p && p.topics || []).includes(topic))if (tag && typeof tag === 'string')posts = posts && posts.filter(p => (p && p.tags || []).includes(tag))if (author && typeof author === 'string')posts = posts && posts.filter(p => p && p.author === author)posts = posts && posts.sort((a, b) =>; }'
...
})const posts = readPosts()if (posts && posts.some(p => { return p && p.slug === body && body.slug)) ; }
...
const body = req.body as Partial<BlogPost>;
    ...
    return res.status(201).json(post)const posts = readPosts()if (posts.some(p => { return p.slug === body.slug)) ; }
return res.status(409).json({ "error": 'Slug already exists' },;'
}
;
