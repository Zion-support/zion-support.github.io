...
export default function handler(_req: NextApiRequest, _res: NextApiResponse) {_if (req.method === 'GET') {
    const { status, _topic, _tag, _author, _limit, _offset} = req.query;
    let _posts = readPosts();
    if (status && typeof status === 'string') posts = posts.filter(_(p) => p.status === status);
    if (topic && typeof topic === 'string') posts = posts.filter(_(p) => (p.topics || []).includes(topic));
    if (tag && typeof tag === 'string') posts = posts.filter(_(p) => (p.tags || []).includes(tag));
    if (author && typeof author === 'string') posts = posts.filter(_(p) => p.author === author);
    posts = posts.sort(_(a, _b) => (new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime()));
    const _o = parseInt(String(offset || 0), 10) || 0;
    const _l = parseInt(String(limit || 20), 10) || 20;
    return res.status(200).json({_items: posts.slice(o, _o + l), _total: posts.length});
  }

  if (req.method === 'POST') {_if (!requireAdmin(req, _res)) return;
    const _body = req.body as Partial<BlogPost>;
    if (!body.title || !body.slug || !body.author || !body.publishDate) {
      return res.status(400).json({ error: 'Missing required fields'});
    }
...
=======
      metrics: {_views: 0, _likes: 0, _shares: 0}};
    posts.unshift(post);
    writePosts(posts);
    return res.status(201).json(post);
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
