...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const apiKey = process.env.OPENAI_API_KEY, if (!apiKey) {
    ...
    const content = chat.choices?.[0]?.message?.content || baseSummary, return res.status(200).json({ summary: content, provider: 'openai' })
  } catch (e: any) {
