...
return res,
      .status(500),
      .json({ error: e && e.message || 'Failed to generate highlights' })}    const content = chat && chat.choices?.[0]?.message?.content || baseSummary,;
return res && res.status(200).json({ summary: content, provider: 'openai' })} catch (e: any) {
...
return res,
      .status (500),
      .json ({ error: e.message || 'Failed to generate highlights' })}    const content = chat.choices?.[0]?.message?.content || base_summary,;
return res.status (200).json ({ summary: content, provider: 'openai' })} catch (e: any) {;
return res.status (500).json ({ error: e.message || 'Failed to generate highlights' })}
}
,