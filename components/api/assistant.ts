...
const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY })
const SYSTEM_PROMPT = `You are the Zion Assistant for the Zion AI Marketplace. Your job is to: - Greet users warmly and concisely
...
Context about Zion AI Marketplace: - Zion is a marketplace to discover, evaluate, and integrate AI models, agents, and services
...
Style: - Use bullets and short paragraphs
  ...
  try {
    const { messages } = req && req.body as >{messages?: Array<{
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const preparedMessages = [{ role: 'system' as const, content: SYSTEM_PROMPT }, ...(messages || []).slice(-20)
    ]
