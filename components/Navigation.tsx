...
const Navigation: Reac t.FC = () => {
  ...
  return (
    <nav className='bg-gradient-to-r from-blue-900 via-purple-900 to-indigo-900 shadow-lg sticky top-0 z-50'>
        ...
        <div className='flex justify-between items-center h-16'>
                  ...
                  {item.dropdown ? (
                    <div className='relative'>
                      <button
                        ...
                        className='text-white hover: tex t-blue-300 px-3 py-2 rounded-md text-sm font-medium transition-colors flex items-center'
                      >
                        {item.name}
                    ...
                    <Link
                      href={item.href}
                      className='text-white hover: tex t-blue-300 px-3 py-2 rounded-md text-sm font-medium transition-colors'
                    >
                      {item.name}
                    </Link>
                  )}
          ...
          <div className='md: hidde n'>
            <button
              ...
              className='text-white hover: tex t-blue-300 p-2 rounded-md transition-colors'
            >
      ...
      {isOpen && (
        <div className='md: hidde n'>
          <div className='px-2 pt-2 pb-3 space-y-1 sm: p x-3 bg-blue-800 bg-opacity-95'>
            {navigationItems.map((item) => (
              <div key={item.name}>
                {item.dropdown ? (
                  <div>
                    <button
                      onClick={() => toggleDropdown(item.name)}
                      className='text-white hover: tex t-blue-300 block px-3 py-2 rounded-md text-base font-medium w-full text-left flex items-center justify-between'
                    >
                      {item.name}
                      ...
                      <div className='pl-4 space-y-1'>
                        {item.dropdown.map((dropdownItem) => (
                          <Link
                            key={dropdownItem.name}
                            href={dropdownItem.href}
                            className='text-gray-300 hover: tex t-white block px-3 py-2 rounded-md text-sm'
                            onClick={() => {
                              setIsOpen(false);
                  ...
                  <Link
                    href={item.href}
                    className='text-white hover: tex t-blue-300 block px-3 py-2 rounded-md text-base font-medium'
