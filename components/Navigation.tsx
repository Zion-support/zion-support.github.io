...
const Navigation: React.FC = () => {
  ...
  const navigationItems = [
    { name: 'Home', href: '/' },
    {
      ...
      dropdown: [{ name: 'AI Development', href: '/services/ai-model-development-chat' },
              ...
              {navigationItems.map((item) => (
                <div key={item.name} className='relative'>
                  {item.dropdown ? (
                    ...
                    <Link
                      href={item.href}
                      className='text-white hover:text-blue-300 px-3 py-2 rounded-md text-sm font-medium transition-colors'>{item.name}
                    </Link>
                  )}
                </div>
              ))}
