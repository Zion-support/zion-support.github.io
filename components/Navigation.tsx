...
const Navigation: React.FC = () => {const [isOpen, setIsOpen] = useState(false);
  ...
  return (<nav className="bg-slate-900/95 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" />
        <div className="flex justify-between items-center h-16">}
          {/* Logo */}</div>
          <div className="flex-shrink-0" />
            <Link href="/" className="text-2xl font-bold text-emerald-400" />Zion Tech Group</Link>
            </Link>
          </div>
          ...
          <div className="md:hidden" />
            ...
            ></button>
              {isOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</Menu>
            </button>
          </div>
        </div>
