        ...
        <div className='flex gap-2 mt-2 flex-wrap'>
          {spec.versions.map(v => (
            <button
              ...
              className={`px-2 py-1 rounded border text-xs ${selectedVersion === v ? 'bg-high-contrast-tertiary border-high-contrast-accent' : 'bg-high-contrast-tertiary border-high-contrast-secondary'}`}>{v}
            </button>
  spec: ApiDocsSpec,
  activeEndpointId?: string;
  onSelectEndpoint: (endpointId: string) => void,
                ...
                .map((e) => (
                  <li key={e.id}>
                    <button
                      ...
                      onClick={() => onSelectEndpoint(e.id)}><span className='mr-2 inline-block w-10 text-center text-[10px] opacity-80'>
                        {e.method}
                      </span>
