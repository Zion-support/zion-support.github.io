import React from 'react';
import { Link } from 'react-router-dom';

// AI Trends 2026 Revolutionary Banner
export function NewContent2026MegaBanner() {
  return (
    <section className="py-16 bg-gradient-to-r from-purple-900 via-blue-900 to-indigo-900 text-white relative overflow-hidden">
      <div className="absolute inset-0 bg-black/20"></div>
      <div className="relative max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          <div className="inline-block bg-gradient-to-r from-yellow-400 to-orange-500 text-black px-4 py-2 rounded-full text-sm font-bold mb-4">
            🚀 NEW 2026 CONTENT
          </div>
          <h2 className="text-4xl md:text-5xl font-bold mb-6 leading-tight">
            Revolutionary AI Trends 2026: The Future is Here
          </h2>
          <p className="text-xl opacity-90 max-w-3xl mx-auto mb-8">
            Discover the groundbreaking AI technologies that will reshape industries and create unprecedented opportunities for growth.
          </p>
        </div>
        
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            <div className="text-3xl mb-4">🧠</div>
            <h3 className="text-xl font-bold mb-3">Quantum AI Computing</h3>
            <p className="text-sm opacity-90 mb-4">
              10,000x faster optimization and unbreakable security for enterprise applications.
            </p>
            <Link
              href="/blog/ai-trends-2026-future-tech"
              className="inline-flex items-center text-yellow-400 font-semibold hover:text-yellow-300 transition-colors"
            >
              Read Article →
            </Link>
          </div>
          
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            <div className="text-3xl mb-4">🔗</div>
            <h3 className="text-xl font-bold mb-3">Neural Interfaces</h3>
            <p className="text-sm opacity-90 mb-4">
              Direct brain-computer communication for seamless human-AI collaboration.
            </p>
            <Link
              href="/blog/ai-trends-2026-future-tech"
              className="inline-flex items-center text-yellow-400 font-semibold hover:text-yellow-300 transition-colors"
            >
              Learn More →
            </Link>
          </div>
          
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            <div className="text-3xl mb-4">🤖</div>
            <h3 className="text-xl font-bold mb-3">Autonomous AI</h3>
            <p className="text-sm opacity-90 mb-4">
              Self-managing AI systems that learn, adapt, and optimize continuously.
            </p>
            <Link
              href="/case-studies/enterprise-ai-transformation-2026"
              className="inline-flex items-center text-yellow-400 font-semibold hover:text-yellow-300 transition-colors"
            >
              View Case Study →
            </Link>
          </div>
        </div>
        
        <div className="text-center">
          <Link
            href="/blog/ai-trends-2026-future-tech"
            className="inline-block bg-gradient-to-r from-yellow-400 to-orange-500 text-black px-8 py-4 rounded-lg font-bold text-lg hover:from-yellow-300 hover:to-orange-400 transition-all shadow-lg hover:shadow-xl"
          >
            Explore 2026 AI Trends
          </Link>
        </div>
      </div>
    </section>
  );
}

// AI Revolutionary 2026 Banner
export function AIRevolutionary2026Banner() {
  return (
    <section className="py-16 bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-100">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          <div className="inline-block bg-gradient-to-r from-cyan-600 to-blue-600 text-white px-4 py-2 rounded-full text-sm font-bold mb-4">
            🌟 REVOLUTIONARY 2026
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6 leading-tight">
            The AI Revolution 2026: Transform Your Business Today
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Join the AI revolution with cutting-edge technologies that deliver 400%+ ROI and market leadership.
          </p>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="text-3xl mb-3">⚡</div>
            <h3 className="text-lg font-bold text-gray-900 mb-2">Edge AI</h3>
            <p className="text-sm text-gray-600 mb-4">Sub-10ms response times</p>
            <div className="text-2xl font-bold text-blue-600">99.9%</div>
            <div className="text-xs text-gray-500">Uptime</div>
          </div>
          
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="text-3xl mb-3">🎨</div>
            <h3 className="text-lg font-bold text-gray-900 mb-2">GenAI 3.0</h3>
            <p className="text-sm text-gray-600 mb-4">Human-level creativity</p>
            <div className="text-2xl font-bold text-purple-600">10x</div>
            <div className="text-xs text-gray-500">Faster Development</div>
          </div>
          
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="text-3xl mb-3">🛡️</div>
            <h3 className="text-lg font-bold text-gray-900 mb-2">AI Security</h3>
            <p className="text-sm text-gray-600 mb-4">Quantum-grade protection</p>
            <div className="text-2xl font-bold text-green-600">99.7%</div>
            <div className="text-xs text-gray-500">Threat Detection</div>
          </div>
          
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="text-3xl mb-3">🚀</div>
            <h3 className="text-lg font-bold text-gray-900 mb-2">Autonomous AI</h3>
            <p className="text-sm text-gray-600 mb-4">Self-managing systems</p>
            <div className="text-2xl font-bold text-orange-600">80%</div>
            <div className="text-xs text-gray-500">Cost Reduction</div>
          </div>
        </div>
        
        <div className="text-center">
          <Link
            href="/services"
            className="inline-block bg-gradient-to-r from-cyan-600 to-blue-600 text-white px-8 py-4 rounded-lg font-bold text-lg hover:from-cyan-500 hover:to-blue-500 transition-all shadow-lg hover:shadow-xl"
          >
            Start Your AI Revolution
          </Link>
        </div>
      </div>
    </section>
  );
}

// Revolutionary 2026 Content Banner
export function Revolutionary2026ContentBanner() {
  return (
    <section className="py-16 bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 text-white">
      <div className="max-w-6xl mx-auto px-4">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <div className="inline-block bg-white/20 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm font-bold mb-4">
              🎯 NEW CASE STUDY
            </div>
            <h2 className="text-4xl font-bold mb-6 leading-tight">
              $50M ROI Enterprise AI Transformation
            </h2>
            <p className="text-xl opacity-90 mb-8">
              See how a Fortune 500 company achieved unprecedented growth with comprehensive AI transformation, 
              resulting in $50M ROI and market leadership.
            </p>
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">90%</div>
                <div className="text-sm opacity-90">Efficiency Gain</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">$50M</div>
                <div className="text-sm opacity-90">Total ROI</div>
              </div>
            </div>
            <Link
              href="/case-studies/enterprise-ai-transformation-2026"
              className="inline-block bg-white text-teal-600 px-8 py-4 rounded-lg font-bold text-lg hover:bg-gray-100 transition-colors shadow-lg"
            >
              View Case Study →
            </Link>
          </div>
          
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20">
            <h3 className="text-2xl font-bold mb-6">Key Results</h3>
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center">
                  <span className="text-white font-bold">✓</span>
                </div>
                <span>400% ROI in 12 months</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center">
                  <span className="text-white font-bold">✓</span>
                </div>
                <span>90% operational efficiency gain</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center">
                  <span className="text-white font-bold">✓</span>
                </div>
                <span>75% cost reduction across processes</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center">
                  <span className="text-white font-bold">✓</span>
                </div>
                <span>Market leadership position achieved</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

// Latest AI Innovations 2026 Banner
export function LatestAIInnovations2026Banner() {
  return (
    <section className="py-16 bg-gradient-to-br from-violet-50 via-purple-50 to-pink-50">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          <div className="inline-block bg-gradient-to-r from-violet-600 to-purple-600 text-white px-4 py-2 rounded-full text-sm font-bold mb-4">
            🔮 FUTURE TECH 2026
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6 leading-tight">
            Latest AI Innovations Shaping 2026
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Explore the breakthrough technologies that will define the next era of business transformation.
          </p>
        </div>
        
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-purple-600 rounded-lg flex items-center justify-center">
                <span className="text-white text-xl">⚛️</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-gray-900">Quantum AI</h3>
                <p className="text-sm text-gray-500">Next-gen computing</p>
              </div>
            </div>
            <p className="text-gray-600 mb-4">
              Quantum-enhanced AI systems solving complex optimization problems in seconds, 
              delivering 10,000x performance improvements.
            </p>
            <Link
              href="/blog/ai-trends-2026-future-tech"
              className="text-violet-600 font-semibold hover:text-violet-700 transition-colors"
            >
              Learn More →
            </Link>
          </div>
          
          <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-br from-pink-500 to-rose-600 rounded-lg flex items-center justify-center">
                <span className="text-white text-xl">🧠</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-gray-900">Neural Interfaces</h3>
                <p className="text-sm text-gray-500">Brain-computer integration</p>
              </div>
            </div>
            <p className="text-gray-600 mb-4">
              Direct neural communication enabling seamless human-AI collaboration 
              with 90% faster decision making and zero latency.
            </p>
            <Link
              href="/blog/ai-trends-2026-future-tech"
              className="text-pink-600 font-semibold hover:text-pink-700 transition-colors"
            >
              Explore Technology →
            </Link>
          </div>
          
          <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg flex items-center justify-center">
                <span className="text-white text-xl">🤖</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-gray-900">Autonomous Systems</h3>
                <p className="text-sm text-gray-500">Self-managing AI</p>
              </div>
            </div>
            <p className="text-gray-600 mb-4">
              Fully autonomous AI ecosystems that learn, adapt, and optimize 
              continuously, reducing operational costs by 80%.
            </p>
            <Link
              href="/case-studies/enterprise-ai-transformation-2026"
              className="text-emerald-600 font-semibold hover:text-emerald-700 transition-colors"
            >
              View Implementation →
            </Link>
          </div>
        </div>
        
        <div className="text-center">
          <Link
            href="/blog"
            className="inline-block bg-gradient-to-r from-violet-600 to-purple-600 text-white px-8 py-4 rounded-lg font-bold text-lg hover:from-violet-500 hover:to-purple-500 transition-all shadow-lg hover:shadow-xl"
          >
            Explore All 2026 Innovations
          </Link>
        </div>
      </div>
    </section>
  );
}