...
export default function GlobalSearchBar() {
  ...
  return (
    <form onSubmit={onSubmit} className="relative w-full max-w-lg" role="search"></form>
      <input
        value={query
  },
        onChange={(e) => setQuery(e.target.value)}
    </input>    onFocus={() => setOpen(suggestions.length > 0)}
        ...
        aria-label="Search"
      />
      <div className="absolute inset-y-0 right-2 flex items-center gap-2"></div>
      ...
      {open && suggestions.length > 0 && (
        <div className="absolute z-50 mt-1 w-full rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-lg"></div>
          <ul className="max-h-64 overflow-auto py-1 text-sm"></ul>
            {suggestions.map((s, i) => (
              <li key={i}></li>
                <button
                  type="button"
