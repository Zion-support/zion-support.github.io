<<<<<<< HEAD
export default function ReportsIndex() {export default function ReportsIndex() {return (
    <div className='min-h-screen bg-slate-950 text-white' />;
=======
export default function ReportsIndex() {export default function ReportsIndex() {return (<div className='min-h-screen bg-slate-950 text-white' />;
>>>>>>> origin/main
export default function ReportsIndex() {;
export default function ReportsIndex() {
  return (
    <div className='min-h-screen bg-slate-950 text-white' />;}
      <main className='container mx-auto px-4 py-12 max-w-7xl' />;}
        {/* Header */}
        <div className='mb-8' />;
          <h1 className='text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent' />;
            Autonomous Content Hub;
          </h1>;
          <p className='text-white/70 text-lg' />;
            Live audit artifacts, insights, and updates generated by autonomous;
<<<<<<< HEAD
            agents.          </p>export default function ReportsIndex() {return (
    <div className=\"min-h-screen bg-slate-950 text-white\" />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\" />;}"
        {/* Header */}<div className=\"mt-12 text-center\" />;"
        {/* Back to Home */}<div className=\"mt-12 text-center\" />;"
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
}
        <ContentCategorizer />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
=======
            agents.          </p>export default function ReportsIndex() { return null; }
        {/* Header */}

<<<<<<< HEAD

        {/* Back to Home */}


pr-12243
        <div className="mt-12 text-center">

        {/* Content Categorizer Component */  } catch (error) {
    console.error("Error: ";, error);
    return res.status(500).json({ error: "Internal server error" ;});
  }
            agents.          </p>export default function ReportsIndex() {return (<div className=\"min-h-screen bg-slate-950 text-white\" />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\" />;}"
        {/* Header */}<div className=\"mt-12 text-center\" />;"
        {/* Back to Home */}<div className=\"mt-12 text-center\" />;"
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error: \";, error)return res.status(500).json({ error: \"Internal server error\" ;})}
}
=======
export default function ReportsIndex() {export default function ReportsIndex() {return (<div className='min-h-screen bg-slate-950 text-white'    />;
>>>>>>> origin/chore/fix-lint-and-merge

export default function ReportsIndex() {;

export default function ReportsIndex() {
  return (
    <div className='min-h-screen bg-slate-950 text-white'    />;}
      <main className='container mx-auto px-4 py-12 max-w-7xl'    />;}
        {/* Header */}
        <div className='mb-8'    />;
          <h1 className='text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent'    />;
            Autonomous Content Hub;
          </h1>;
          <p className='text-white/70 text-lg'    />;
            Live audit artifacts, insights, and updates generated by autonomous;
<<<<<<< HEAD
            agents.          </p>export default function ReportsIndex() { return null; }
        {/* Header */}



        <div className="mt-12 text-center>

        {/* Content Categorizer Component */  } catch (error) {
    console.error(Error: ";, error);
    return res.status(500).json({ error: "Internal server error ;});
  }
            agents.          </p>export default function ReportsIndex() {return (<div className=\min-h-screen bg-slate-950 text-white\" />;}"
      <main className=\container mx-auto px-4 py-12 max-w-7xl\ />;}"
        {/* Header */}<div className=\"mt-12 text-center\ />;
        {/* Back to Home */}<div className=\"mt-12 text-center\" />;
        {/* Content Categorizer Component */  } catch (error) {console.error(\Error: \";, error)return res.status(500).json({ error: \"Internal server error\ ;})}
}
        <ContentCategorizer />;
        {/* Back to Home */  } catch (error) {console.error(\"Error: \";, error)return res.status(500).json({ error: \Internal server error\ ;})}
>>>>>>> origin/main
}"
        <div className=\"mb-8\ />;
          <h1 className=\"text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent\" />;
            Autonomous Content Hub;
          </h1>;
          <p className=text-white/70 text-lg>;
=======
            agents.          </p>export default function ReportsIndex() {return (<div className=\"min-h-screen bg-slate-950 text-white\"    />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\"    />;}"
        {/* Header */}<div className=\"mt-12 text-center\"    />;"
        {/* Back to Home */}<div className=\"mt-12 text-center\"    />;"
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error:\", error)return res.status(500).json({ error: \"Internal server error\" })}
}
        <ContentCategorizer    />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error:\", error)return res.status(500).json({ error: \"Internal server error\" })}
}"
        <div className=\"mb-8\"    />;"
          <h1 className=\"text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent\"    />;
            Autonomous Content Hub;
          </h1>;"
          <p className=\"text-white/70 text-lg\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {/* Content Categorizer Component */}
<<<<<<< HEAD
        <ContentCategorizer />;
pr-12243
=======
        <ContentCategorizer    />;
        {/* Back to Home */}
        <div className='mt-12 text-center'    />;
          <Link;
            href='/';
            agents.          </p    />export default function ReportsIndex() {;
  return ("
    <div className=\"min-h-screen bg-slate-950 text-white\"    />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\"    />;}
        {/* Header */}


>>>>>>> origin/chore/fix-lint-and-merge
        {/* Back to Home */}


        {/* Back to Home */}
<<<<<<< HEAD
        {/* Back to Home */}'
        <div className='mt-12 text-center'>;
          <Link'
            href='/''"
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200'>            ← Back to home        <div className="mt-12 text-center">;"
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">;'
import Link from 'next / link';'
import ContentCategorizer from '../../components / ContentCategorizer';'
=======
        <div className='mt-12 text-center'    />;
          <Link;
href='/'"
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200'    />            ← Back to home        <div className=\"mt-12 text-center\"    />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />;
import Link from 'next / link';
import ContentCategorizer from '../../components / ContentCategorizer';
>>>>>>> origin/chore/fix-lint-and-merge

export default /**;
 * ReportsIndex - Function description;
 */;
function ReportsIndex() {return (<div className='min - h-screen bg - slate - 950 text - white'    />;

export default /**
 * ReportsIndex - Function description;
 */
<<<<<<< HEAD

            Live audit artifacts, insights, and updates generated by autonomous;
            agents.          </p > export default /**;
 * ReportsIndex - Function description;
 */
  return ("
    <div className="min - h-screen bg - slate - 950 text - white>;
      <main className="container mx - auto px - 4 py - 12 max - w-7xl">;
        {/* Header */}
        <div className=mb - 8">;"
          <h1 className=text - 4xl font - extrabold mb - 2 bg - gradient - to - r from - white to - blue - 100 bg - clip - text text - transparent>;
            Autonomous Content Hub;
          </h1>;"
          <p className="text - white / 70 text - lg>;
          </h1>;
          <p className=\"text-white/70 text-lg\" />;
            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {/* Content Categorizer Component */}
        <ContentCategorizer />;


        {/* Back to Home */}'
        <div className='mt - 12 text - center'>;
          <Link;'
            href='/';'
            className='inline - flex items - center gap - 2 text - cyan - 300 hover:text - cyan - 200 transition - colors duration - 200';"
          >            ← Back to home        <div className="mt - 12 text - center">;"
          <Link href="/" className="inline - flex items - center gap - 2 text - cyan - 300 hover: text - cyan - 200 transition - colors duration - 200">;
pr-12243


        {/* Back to Home */}
<<<<<<< HEAD
        <div className='mt-12 text-center' />;
          <Link href='/';
            agents.          </p />export default function ReportsIndex() {;
  return ("
    <div className=\"min-h-screen bg-slate-950 text-white\" />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\" />;}
=======
        <div className='mt-12 text-center'>;
          <Link;
            href='/';
            agents.          </p>export default function ReportsIndex() {;
  return (
    <div className="min-h-screen bg-slate-950 text-white">;
      <main className="container mx-auto px-4 py-12 max-w-7xl">;
>>>>>>> origin/main
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">
            Autonomous Content Hub
          </h1>
          <p className="text-white/70 text-lg">
            Live audit artifacts, insights, and updates generated by autonomous agents.
          </p>
        </div>

<<<<<<< HEAD
=======
        {/* Content Categorizer Component */}
        <ContentCategorizer />

>>>>>>> origin/main
        {/* Back to Home */}
        <div className="mt-12 text-center">
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">
            className='inline - flex items - center gap - 2 text - cyan - 300 hover:text - cyan - 200 transition - colors duration - 200';
          >            ← Back to home        <div className="mt - 12 text - center">;
          <Link href="/" className="inline - flex items - center gap - 2 text - cyan - 300 hover: text - cyan - 200 transition - colors duration - 200">;

<<<<<<< HEAD
        {/* Back to Home */}
        <div className='mt-12 text-center' />;
          <Link href='/'"
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200' />            ← Back to home        <div className=\"mt-12 text-center\" />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />;
import Link from 'next / link';
=======




            ← Back to home;
          </Link>;
        </div>;
      </main>;

  );
}
        <div className="mt-12 text-center">

        {/* Content Categorizer Component */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
        <div className="mt-12 text-center">;
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">;
  )})}
        <div className="mt-12 text-center">;
        {/* Content Categorizer Component */  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
  );
}
        <div className="mt-12 text-center">


          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">

  );
}

  );
}
pr-12243
        <div className="mt-12 text-center">


          <Link href=/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200>

        {/* Content Categorizer Component */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: Internal server error" });
  }
}
        <ContentCategorizer />;
        {/* Back to Home */  } catch (error) {"
    console.error("Error:", error);"
    return res.status(500).json({ error: "Internal server error" });
        <div className="mt-12 text-center">

        {/* Content Categorizer Component */  } catch (error) {
    console.error("Error: ;, error);
    return res.status(500).json({ error: Internal server error" ;});
  }
}
        <ContentCategorizer />;
        {/* Back to Home */  } catch (error) {
    console.error("Error: ";, error);
    return res.status(500).json({ error: "Internal server error" ;});
  }
}
        <div className="mt-12 text-center">

          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">
        {/* Header */}
        <div className=&quot;mb-8&quot;>
          <h1 className=&quot;text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent&quot;>
            Autonomous Content Hub
          </h1>

  }

          <p className=&quot;text-white/70 text-lg&quot;>
            Live audit artifacts, insights, and updates generated by autonomous agents.
          </p>
        </div>

        {_/* Content Categorizer Component */}
            agents.          </p />export default function ReportsIndex() {;
  return ("
    <div className=\min-h-screen bg-slate-950 text-white\ />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\ />;}
        {/* Header */}

        {/* Back to Home */}

        {/* Back to Home */}
        <div className='mt-12 text-center' />;
          <Link;
href=/
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200' />            ← Back to home        <div className=\"mt-12 text-center\" />;
          <Link href=\/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\ />;
import Link from next / link;
>>>>>>> origin/main
import ContentCategorizer from '../../components / ContentCategorizer';
export default /**;
 * ReportsIndex - Function description;
 */;
<<<<<<< HEAD
function ReportsIndex() {return (
    <div className='min - h-screen bg - slate - 950 text - white' />;
export default /**
 * ReportsIndex - Function description;
 */
=======

=======
>>>>>>> origin/main
function ReportsIndex() {
  return (
    <div className='min - h-screen bg - slate - 950 text - white'    />;}
      <main className='container mx - auto px - 4 py - 12 max - w-7xl'    />;}
        {/* Header */}
        <div className='mb - 8'    />;
          <h1 className='text - 4xl font - extrabold mb - 2 bg - gradient - to - r from - white to - blue - 100 bg - clip - text text - transparent'    />;
            Autonomous Content Hub;
          </h1>;
          <p className='text - white / 70 text - lg'    />;
            Live audit artifacts, insights, and updates generated by autonomous;
            agents.          </p > export default /**;
 * ReportsIndex - Function description;
 */;"
<<<<<<< HEAD
function ReportsIndex() {return (
    <div className=\"min - h-screen bg - slate - 950 text - white\" />;
=======
function ReportsIndex() {return (<div className=\"min - h-screen bg - slate - 950 text - white\"    />;
>>>>>>> origin/main
            agents.          </p > export default /**
 * ReportsIndex - Function description;
 */
function ReportsIndex() {
  return ("
    <div className=\"min - h-screen bg - slate - 950 text - white\"    />;}"
      <main className=\"container mx - auto px - 4 py - 12 max - w-7xl\"    />;}
        {/* Header */}"
        <div className=\"mb - 8\"    />;"
          <h1 className=\"text - 4xl font - extrabold mb - 2 bg - gradient - to - r from - white to - blue - 100 bg - clip - text text - transparent\"    />;
            Autonomous Content Hub;
          </h1>;"
          <p className=\"text - white / 70 text - lg\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {/* Content Categorizer Component */}
        <ContentCategorizer    />;
        {/* Back to Home */}
<<<<<<< HEAD
        <div className='mt - 12 text - center' />;
          <Link href='/';
=======
        <div className='mt - 12 text - center'    />;
          <Link;
<<<<<<< HEAD

=======
            href='/';
>>>>>>> origin/main
            className='inline - flex items - center gap - 2 text - cyan - 300 hover:text - cyan - 200 transition - colors duration - 200';"
              />            ← Back to home        <div className=\"mt - 12 text - center\"    />;"
          <Link href=\"/\" className=\"inline - flex items - center gap - 2 text - cyan - 300 hover: text - cyan - 200 transition - colors duration - 200\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
            ← Back to home;
          </Link>;
        </div>;
      </main>;"
<<<<<<< HEAD
        <div className=\"mt-12 text-center\" />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />;
  )})}"
        <div className=\"mt-12 text-center\" />;"
<<<<<<< HEAD
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
}
        <ContentCategorizer />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
=======
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error: \";, error)return res.status(500).json({ error: \"Internal server error\" ;})}
}
        <ContentCategorizer />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error: \";, error)return res.status(500).json({ error: \"Internal server error\" ;})}
>>>>>>> origin/main
}"
        <div className=\"mb-8\" />;"
          <h1 className=\"text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent\" />;
            Autonomous Content Hub;
          </h1>;"
          <p className=\"text-white/70 text-lg\" />;
            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {/* Content Categorizer Component */}"
        <ContentCategorizer />;<div className=\"mt-12 text-center\" />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />;"
        <div className=\"mt-12 text-center\" />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />;
        {/* Header */}
        <div className=&quot;mb-8&quot />

          <h1 className=&quot;text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent&quot />

            Autonomous Content Hub;
          </h1>;
          <p className=&quot;text-white/70 text-lg&quot />

            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {_/* Content Categorizer Component */}
import Link from 'next/link';
import Link from next/link;
import ContentCategorizer from '../../components/ContentCategorizer';
<<<<<<< HEAD
"
export default function ReportsIndex() {return (
    <div className=\"min-h-screen bg-slate-950 text-white\" />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\" />;}"
        {/* Header */  } catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
=======

export default function ReportsIndex() {
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <main className="container mx-auto px-4 py-12 max-w-7xl">
        {/* Header */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
        <ContentCategorizer />;
        {/* Back to Home */  } catch (error) {console.error(""Error":", error)return res.status(500).json({ "error": "Internal server error" })},"
}
        <div className="mb-8">
          <h1 className="text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">
            Autonomous Content Hub
          </h1>
          <p className="text-white/70 text-lg">
            Live audit artifacts, insights, and updates generated by autonomous agents.
          </p>
        </div>
        <div className="mb-8">;
          <h1 className="text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">;
            Autonomous Content Hub;
          </h1>;
          <p className=text-white/70 text-lg">;
            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {/* Content Categorizer Component */}

=======
        <div className=\"mt-12 text-center\"    />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />;
  )})}"
        <div className=\"mt-12 text-center\"    />;"
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error:\", error)return res.status(500).json({ error: \"Internal server error\" })}
}
        <ContentCategorizer    />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error:\", error)return res.status(500).json({ error: \"Internal server error\" })}
>>>>>>> origin/main
}"
        <div className=\"mb-8\"    />;"
          <h1 className=\"text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent\"    />;
            Autonomous Content Hub;
          </h1>;"
          <p className=\"text-white/70 text-lg\"    />;
            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {/* Content Categorizer Component */}"
        <ContentCategorizer    />;<div className=\"mt-12 text-center\"    />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />;"
        <div className=\"mt-12 text-center\"    />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
        {/* Header */}
        <div className=&quot;mb-8&quot    />

          <h1 className=&quot;text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent&quot    />

            Autonomous Content Hub;
          </h1>;
          <p className=&quot;text-white/70 text-lg&quot    />

            Live audit artifacts, insights, and updates generated by autonomous agents.;
          </p>;
        </div>;
        {_/* Content Categorizer Component */}
<<<<<<< HEAD

=======
import Link from 'next/link';
import ContentCategorizer from '../../components/ContentCategorizer';
"
export default function ReportsIndex() {return (<div className=\"min-h-screen bg-slate-950 text-white\"    />;}"
      <main className=\"container mx-auto px-4 py-12 max-w-7xl\"    />;}"
        {/* Header */  } catch (error) {console.error(\"Error:\", error)return res.status(500).json({ error: \"Internal server error\" })}
}"
        <div className=\"mb-8\"    />;"
          <h1 className=\"text-4xl font-extrabold mb-2 bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent\"    />;
            Autonomous Content Hub;
          </h1>;"
          <p className=\"text-white/70 text-lg\"    />;
            Live audit artifacts, insights, and updates generated by autonomous agents.;
          <p className='text-white/70 text-lg'    />;
>>>>>>> origin/chore/fix-lint-and-merge
            Live audit artifacts, insights, and updates generated by autonomous;
            agents.;
          </p>;
        </div>;
<<<<<<< HEAD


=======
>>>>>>> origin/chore/fix-lint-and-merge
        {/* Content Categorizer Component */}
        <ContentCategorizer    />;
        {/* Back to Home */}
<<<<<<< HEAD
<div className='mt-12 text-center'>;
        {/* Content Categorizer Component */  } catch (error) {
    console.error("Error: ;, error);
    return res.status(500).json({ error: Internal server error" ;});
<div className=mt-12 text-center />;
          <Link;
            href='/';
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200';"
           />            ← Back to home        <div className=\mt-12 text-center\ />;
  )}"
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error: \;, error)return res.status(500).json({ error: \Internal server error\" ;})}
}
        <ContentCategorizer />;"
        {/* Back to Home */  } catch (error) {console.error(\Error: \;, error)return res.status(500).json({ error: \"Internal server error\" ;})}
}
        <div className="mt-12 text-center">;
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">;
          >;
<div className='mt-12 text-center'>
          <Link
            href='/'
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200'
          >            ← Back to home        <div className="mt-12 text-center">
  );
}
pr-12243
        {/* Content Categorizer Component */  } catch (error) {
    console.error("Error: ";, error);
    return res.status(500).json({ error: "Internal server error" ;});
<div className='mt-12 text-center' />;
          <Link href='/';
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200';"
           />            ← Back to home        <div className=\"mt-12 text-center\" />;
  )}"
<<<<<<< HEAD
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
}
        <ContentCategorizer />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
=======
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error: \";, error)return res.status(500).json({ error: \"Internal server error\" ;})}
}
        <ContentCategorizer />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error: \";, error)return res.status(500).json({ error: \"Internal server error\" ;})}
>>>>>>> origin/main
}"
        <div className=\"mt-12 text-center\" />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />>
=======
<div className='mt-12 text-center'    />;
          <Link;
            href='/';
            className='inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors duration-200';"
              />            ← Back to home        <div className=\"mt-12 text-center\"    />;
  )}"
        {/* Content Categorizer Component */  } catch (error) {console.error(\"Error:\", error)return res.status(500).json({ error: \"Internal server error\" })}
}
        <ContentCategorizer    />;"
        {/* Back to Home */  } catch (error) {console.error(\"Error:\", error)return res.status(500).json({ error: \"Internal server error\" })}
}"
        <div className=\"mt-12 text-center\"    />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />>
>>>>>>> origin/chore/fix-lint-and-merge

            ← Back to home;
          </Link>;
        </div>;
      </main>
    </div>
  );
}ursor/fix-website-loading-errors-and-merge-6662;
    </div>;
    </div>;
  )}
<<<<<<< HEAD
    </div>;
  )})<div className=\mt-12 text-center\" />;"
          <Link href=\/\ className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />;

  );
}
        <div className=\mt-12 text-center\" />

"
          <Link href=\/\ className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />
        {/* Content Categorizer Component */ }
} catch (error) {
    console.error(\Error: \";, error);}"
    return res.status(500).json({ error: \Internal server error\;}
=======
    </div>;"
<<<<<<< HEAD
  )})}
      <div className=\"mt-12 text-center\" />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\" />;
=======
  )})<div className=\"mt-12 text-center\"    />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />;
>>>>>>> origin/main

  );
}"
        <div className=\"mt-12 text-center\"    />

"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />
        {/* Content Categorizer Component */ }
} catch (error) {"
<<<<<<< HEAD
    console.error(\"Error:\",,,
  error);}"
    return res.status(500).json({ error: \"Internal server error\"},,
  });
=======
    console.error(\"Error:\", error);}"
    return res.status(500).json({ error: \"Internal server error\"}
>>>>>>> origin/chore/fix-lint-and-merge
});
>>>>>>> origin/main
  }
}
        <ContentCategorizer    />;
        {/* Back to Home */  } catch (error) {"
<<<<<<< HEAD
    console.error(\"Error:\",,,
  error);}"
    return res.status(500).json({ error: \"Internal server error\"},,
  });
=======
<<<<<<< HEAD
    console.error(\"Error: \;, error);}
    return res.status(500).json({ error: \"Internal server error\";}
});
  }
}
        <div className="mt-12 text-center">
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">
          >
origin/cursor/automate-test-improve-and-merge-code-2533
pr-12243

          >
origin/cursor/automate-test-improve-and-merge-code-2533
          >
origin/cursor/automate-test-improve-and-merge-code-2533
pr-12243
            ← Back to home
          </Link>
        </div>
      </main>
pr-12243

            ← Back to home;
          </Link>
        </div>
      </main>


pr-12243


pr-12243
=======
    console.error(\"Error:\", error);}"
    return res.status(500).json({ error: \"Internal server error\"}
});
>>>>>>> origin/main
  }
}"
        <div className=\"mt-12 text-center\"    />"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />
            ← Back to home;
          </Link>
        </div>
      </main>

    </div>);
}

    </div>
    </div>
  );
}
>>>>>>> origin/chore/fix-lint-and-merge
    </div>;
  );
}

  )"
<<<<<<< HEAD
        <div className="mt-12 text-center">;"
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">;
=======
        <div className=\"mt-12 text-center\"    />;"
          <Link href=\"/\" className=\"inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
            ← Back to home;
          </Link>;
        </div>;
      </main>;
<<<<<<< HEAD
    </div>)}ursor/fix-website-loading-errors-and-merge-6662;
    </div>;
    </div>;
  )}
    </div>;

  }
}

  )})<div className="mt-12 text-center>;
          <Link href=/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200>;

  );
}
        <div className=mt-12 text-center">


          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">
          >            ← Back to home        <div className="mt-12 text-center">

        <div className="mt-12 text-center">

          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">

        {/* Content Categorizer Component */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
    </div>;"
  )} catch (error) {console.error(\"Error: \";, error)return res.status(500).json({ error: \"Internal server error\" ;})}
}</div>)}</div>
}
  );
  } catch (error) {"
    console.error("Error:", error);"
    return res.status(500).json({ error: "Internal server error" });
  }
}
;
pr-12243
  }
}
        <ContentCategorizer />;
        {/* Back to Home */  } catch (error) {
    }
    console.error(""Error":", error);"
return res.status(500).json({ "error": "Internal server error",;"
});
  }
}
        <div className="mt-12 text-center">
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 hover: text-cyan-200 transition-colors duration-200">
          >

          >
origin/cursor/automate-test-improve-and-merge-code-2533


            ← Back to home
          </Link>
        </div>
      </main>
    </div>
  )
        <div className="mt-12 text-center">;"
          <Link href="/" className="inline-flex items-center gap-2 text-cyan-300 "hover": text-cyan-200 transition-colors duration-200">;"
            ← Back to home;
          </Link>;
        </div>;
      </main>;
    </div>;
  )} catch (error) {console.error(""Error":", error)return res.status(500).json({ "error": "Internal server error" })}"
}</div>)}</div>
}


origin/cursor/automate-test-improve-and-merge-code-2533





=======
    </div>;"
  )} catch (error) {console.error(\"Error:\",,,
  error)return res.status(500).json({ error: \"Internal server error\" })}
}</div>)}</div>
}
  );
  } catch (error) {"
    console.error(\"Error:\",,,
  error);}"
    return res.status(500).json({ error: \"Internal server error\"},,
  });
  }
}
;
>>>>>>> origin/chore/fix-lint-and-merge

    </div>);
}

    </div>);
}
<<<<<<< HEAD
"
=======
<<<<<<< HEAD
"
origin/cursor/automate-test-improve-and-merge-code-2533
pr-12243


origin/cursor/automate-test-improve-and-merge-code-2533


origin/cursor/automate-test-improve-and-merge-code-2533
pr-12243
"
=======
"
>>>>>>> origin/chore/fix-lint-and-merge
>>>>>>> origin/main
