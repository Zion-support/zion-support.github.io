...
import React, { useState } from 'react';
  Settings,Trash2,} from 'lucide-react';import { Link, Copy, BarChart3, ArrowRight, RefreshCw, CheckCircle, ExternalLink, QrCode, Settings, Trash2  } from 'lucide-react';
import { import { Link, Copy, BarChart3, ArrowRight, RefreshCw, CheckCircle, ExternalLink, QrCode, Settings, Trash2  } from 'lucide-react';
export default function URLShortenerPage() {} from 'lucide-react';import { Link, Copy, BarChart3, ArrowRight, RefreshCw, CheckCircle, ExternalLink, QrCode, Settings, Trash2  } from 'lucide-react';
export default function URLShortenerPage() {const [longUrl, setLongUrl] = useState('')const [customAlias, setCustomAlias] = useState('')const [shortenedUrls, setShortenedUrls] = useState<any[]>([]),const [isShortening, setIsShortening]  = useState(false)setIsShortening(true)// Simulate API call delay;
    ...
    }setShortenedUrls(prev => [newShortUrl, ...prev])setLongUrl('')setCustomAlias('')setIsShortening(false)'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    let result = '';
  ...
  const getTotalUrls = () => {return shortenedUrls.length;  }setIsShortening(true)// Simulate API call delay;
    await new Promise(resolve => setTimeout(resolve, 1000))const alias = customAlias.trim() |generateRandomAlias()const shortUrl = `https://zion.tech/${alias}`;
    const newShortUrl = {id: Date.now()longUrl: longUrl.trim()shortUrl;
      alias;
      clicks: 0;
...
import {Link,Copy,BarChart3,ArrowRight,RefreshCw,CheckCircle,ExternalLink,QrCode,const newShortUrl = {id: Date.now (),long_url: long_url.trim (),short_url;
      alias;
      clicks: 0,const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    let result = '';
      ...
      <section className='pt-32 pb-20 bg-gradient-to-br from-orange-900 via-red-900 to-pink-900'>;
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center'>;
          ...
          <p className='text-xl text-orange-200 max-w-4xl mx-auto leading-relaxed'>;
            ...
            professional shortening service. Track clicks, generate QR codes,and manage your links efficiently with advanced analytics.;
          </p>;
        </div>;
      </section>;
      {/* URL Shortening Tool */}</h2>;
...
<Card className='p-8 bg-gray-800 border border-gray-700'>;
              ...
              <div className='space-y-6'>;
                ...
                {shortenedUrls && shortenedUrls.length === 0 ? (<div className='bg-gray-900 p-6 rounded-lg border border-gray-700 text-center'>;
                    ...
                    {shortenedUrls && shortenedUrls.slice(0, 3).map(url => (<div key={url && url.id} className='p-3 bg-gray-700 rounded-lg'>;
                ...
                {shortenedUrls.length === 0 ? (<div className='bg-gray-900 p-6 rounded-lg border border-gray-700 text-center'>;
                    <div className='text-6xl mb-4'>📊</div>;
                    <p className='text-gray-400'>;
                      Your URL statistics will appear here. Create your first;
                      short URL to get started.;
                    ...
                    {shortenedUrls.slice(0, 3).map(url => (<div key={url.id} className='p-3 bg-gray-700 rounded-lg'>;
                        <div className='flex items-center justify-between'>;
                          <div className='flex-1 min-w-0'>;
                            <div className='text-sm text-orange-400 font-medium truncate'>;
                              {url.shortUrl}
...
<div className='flex space-x-2'>;
                        <Button;
                          onClick={() =>;
                            setShowQR(showQR === url && url.id ? null : url && url.id)}variant='outline';
                          size='sm';
                          className='flex-1 border-gray-600 text-gray-300 hover:bg-gray-600';
                        >;
