...
<<<<<<< HEAD
    ...
    const dao = includeDAOIntegration
      ? `6. DAO and Community Governance\n- The Foundation supports the ${name} DAO as the primary governance mechanism for the protocol.\n- DAO proposals are subject to transparent voting and execution.\n- The Foundation provides technical and legal support for DAO operations.`
      : '',
    const grants = includeGrants
      ? `7. Multiverse Grants and R&D\n- The Foundation may issue cross-multiverse grants across chains, rollups, and alternate compute realms.\n- Priority to public goods: safety tooling, evaluations, datasets, compute schedulers, agents, and interoperability.\n- Quarterly open calls with transparent selection criteria and milestone-based vesting.`
      : '',
    ...
    const amendments = `10. Amendments and Evolution\n- Charter amendments require 2/3 majority of Core Stewards and community ratification.\n- Technical upgrades follow the ${name} Protocol's governance process.\n- Emergency provisions: temporary measures with 30-day sunset clauses.`;
...
const liability = `11. Limitations and Disclaimers\n- No warranties; the ${name Protocol is experimental.\n- The Foundation does not offer securities and does not guarantee token value.\n- Jurisdictional compliance: local obligations remain with individual actors.`;return [;
      ...
      .filter(Boolean).join('\n')}, [;
<<<<<<< HEAD
    legalStructure,protocolName,tokenSymbol,includeContributorRules,includeGrants,includeDAOIntegration])return (
    <div className='space-y-6' />;
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6' />;
        <div className='space-y-4 p-4 rounded-lg border border-gray-200 dark:border-gray-800' />;
          <h2 className='text-lg font-semibold' />Charter Settings</h2>;
          <label className='block text-sm' />;
            Protocol Name;
            <input className='mt-1 w-full rounded border border-gray-300 dark:border-gray-700 bg-transparent px-3 py-2';
              value={protocolName}
...
>>>>>>> origin/main
          ...
          <label className='block text-sm'    />;
            Legal Structure;
<<<<<<< HEAD
            <select className='mt-1 w-full rounded border border-gray-300 dark:border-gray-700 bg-transparent px-3 py-2';
...
<<<<<<< HEAD
        <div className='space-y-4 p-4 rounded-lg border border-gray-200 dark:border-gray-800' />;
          <h2 className='text-lg font-semibold' />Export</h2>}
            <div className='flex gap-3' />
            <button className='px-3 py-2 rounded bg-cyan-600 text-white hover:bg-cyan-700'
            onClick = {() = /> 
...
<<<<<<< HEAD
            <button className='px-3 py-2 rounded border border-gray-300 dark:border-gray-700';
              onClick={() = /> navigator.clipboard.writeText(charterText)}
=======
            <button;
className='px-3 py-2 rounded border border-gray-300 dark:border-gray-700';
...
<<<<<<< HEAD
        <textarea className='w-full h-96 rounded border border-gray-300 dark:border-gray-700 bg-transparent p-3 font-mono text-sm';
          value={charterText,
  readOnly;
        />;
      </div>;
