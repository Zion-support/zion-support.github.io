...
export type LegalStructure =;
  ...
  | 'Swiss Verein';
  | 'US 501(c)(6)';
  | 'DAO-native Wrapper';export default function CharterGenerator() {const [legalStructure, setLegalStructure] =;
    useState<LegalStructure>('Cayman Foundation')const [protocolName, setProtocolName] = useState('Zion')const [tokenSymbol, setTokenSymbol] = useState('ZION$')const [includeContributorRules, setIncludeContributorRules] = useState(true)const [includeGrants, setIncludeGrants] = useState(true)const [includeDAOIntegration, setIncludeDAOIntegration]  = useState(true)const charterText = useMemo(() => {const name = protocolName.trim() || 'Zion';
    const symbol  = tokenSymbol.trim() || 'ZION$';const header = `${name} Foundation Charter`;
    const preamble  = `This Charter establishes a legally-neutral foundation (the "Foundation") to steward the ${name} Protocol, an open, decentralized AI protocol dedicated to sovereignty, open knowledge, and verifiable trust.`;const registry  = `1. Legal Neutrality and Registry\n- Structure: ${legalStructure}.\n- Purpose: Maintain global neutrality, operate outside the control of any single jurisdiction, and coordinate public-good infrastructure for the ${name} Protocol.\n- Scope: Non-custodial oversight, standard-setting, and community enablement.`;const mission  = `2. Mission and Scope\n- Protect, evolve, and safeguard the ${name} Protocol as a public good.\n- Champion open knowledge, reproducible research, and transparent governance.\n- Promote sovereignty for users, developers, and nation/community DAOs.`;const neutrality  = `3. Political and Jurisdictional Neutrality\n- The Foundation is non-partisan and non-sovereign.\n- No nation-state, corporation, or private actor may control governance outcomes.\n- All critical processes are transparent, documented, and reproducible.`;const treasury  = `4. Treasury and Token Oversight\n- The Foundation monitors the on-chain treasury associated with ${symbol} for policy compliance and program execution.\n- The Foundation does not custody private keys for third parties and does not provide financial advice.\n- Treasury actions (grants, bounties, RFPs) are executed via on-chain governance with public reporting.`;const contributors = includeContributorRules;
        ...
        <div className='space-y-4 p-4 rounded-lg border border-gray-200 dark:border-gray-800'>;
          ...
          <label className='block text-sm'>;
            ...
            <select;
              ...
              onChange={e =>;
                setLegalStructure(e.target.value as LegalStructure)}
            >;
              ...
              <option value='DAO-native Wrapper'>;
                DAO-native legal wrapper (e.g., Otoco)</option>;
            </select>;
          </label>;
          <div className='flex items-center gap-3'>;
            <input;
              id='contrib';
              type='checkbox';
              checked={includeContributorRules}
              onChange={e => setIncludeContributorRules(e.target.checked)}
            />;
            <label htmlFor='contrib' className='text-sm'>;
              Include contributor program rules;
            </label>;
          </div>;
          <div className='flex items-center gap-3'>;
            <input;
              id='dao';
              type='checkbox';
              checked={includeDAOIntegration}
              onChange={e => setIncludeDAOIntegration(e.target.checked)}
            />;
            <label htmlFor='dao' className='text-sm'>;
              Include DAO integration;
            </label>;
          </div>;
          <div className='flex items-center gap-3'>;
            <input;
        ...
        <div className='space-y-4 p-4 rounded-lg border border-gray-200 dark:border-gray-800'>;
          ...
          <div className='flex gap-3'>;
            <button;
              ...
              onClick={() => {const blob = new Blob([charterText], {type: 'text/markdown;charset=utf-8',})const url = URL.createObjectURL(blob)const a = document.createElement('a')a.href = url;
