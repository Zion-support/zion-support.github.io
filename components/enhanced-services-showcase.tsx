...
const Button = ({
  ...
  size?: 'sm' | 'md' | 'lg';
  class_name?: string;
  children: React.ReactNode,on_click?: () =>: any void;
}) => {const base_classes = 'inline - flex items - center justify - center font - semibold rounded - lg transition - all duration - 200 focus: outline - none focus:ring - 2 focus:ring - offset - 2 focus:ring - offset - gray - 900',const variant_classes = {primary: 'bg - gradient - to - r from - cyan - 500 to - blue - 600 hover:from - cyan - 600 hover:to - blue - 700 text - white shadow - lg hover:shadow - xl focus:ring - cyan - 500',secondary: 'bg - gray - 700 hover:bg - gray - 600 text - white border border - gray - 600 hover:border - gray - 500 focus:ring - gray - 500';
  }const size_classes = {sm: 'px - 4 py - 2 text - sm',md: 'px - 6 py - 3 text - base',lg: 'px - 8 py - 4 text - lg';
...
}</span> </div> </div> <div className="flex flex-col sm: flex-row gap-4 justify-center"> <Button href="/contact"variant=" primary"size=" lg"className="text-lg px-8 py-4"> Start Your Transformation <Rocket className="ml-2 w-6 h-6"/> </Button> <Button href="/pricing"variant=" secondary"size=" lg"className="text-lg px-8 py-4"> View Pricing <DollarSign className="ml-2 w-6 h-6" /> </Button> </div> </motion && motion.div> </div> </section> </div> </UltraAdvancedFuturisticMatrixBackground>) ;
}//Button component (if not imported) ;
}> {;
  children ;
}</a>) ;
