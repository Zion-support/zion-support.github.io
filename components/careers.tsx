...
}<motion.div </div>) )}</div> </motion.div> {/* Benefits */;
}<motion.div </div>) )}</div> </div> </motion.div> {/* Job Openings */;
}<motion.div > <h2 className="text-3xl font-bold text-white text-center mb-8" >Open Positions</h2> key= {dept.id;
}onClick= {() => setSelectedDepartment (dept.id)}className= {`px-4 py-2 rounded-lg font-medium transition-all duration-300 $ {selectedDepartment === dept.id ? 'bg-blue-500 text-white' : 'bg-gray-800/50 text-gray-300 hover:bg-gray-700/50';
}`;
}> {dept.name;
}</button>) )}</div> > <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4" > <div> </span> </div> </div> <a > Apply Now <ArrowRight className="w-5 h-5 ml-2" /> </a> </div> </li>) )}</ul> </div> </motion.div>) )}</div> We don't have any open positions in this department at the moment. Check back later or send us your resume for future opportunities. </p> </div>)}</motion.div> {/* General Application */;
}<motion.div > <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 p-8 rounded-2xl border border-blue-500/20 max-w-4xl mx-auto" > <h2 className="text-2xl font-bold text-white mb-4" >Don't See the Right Fit?</h2> <p className="text-gray-300 mb-6" > We're always looking for talented individuals to join our team. Send us your resume and let's discuss how you can contribute to our mission. </p> <a href="/contact" className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-semibold rounded-lg hover:from-blue-600 hover:to-purple-700 transition-all duration-300 transform hover:scale-105" > Send Your Resume <ArrowRight className="w-5 h-5 ml-2" /> </a> <a href="/about" className="inline-flex items-center px-6 py-3 border-2 border-blue-500 text-blue-400 font-semibold rounded-lg hover:bg-blue-500 hover:text-white transition-all duration-300" > Learn More About Us </a> </div> </div> </motion.div> </div> </Layout>)}export default CareersPage;import React, { useState } from 'react';
import React, { useState } from 'react';
...
export default function Careers() {const [selectedDepartment, setSelectedDepartment] = useState('all')const departments = [;
  ...
  const filteredJobs = selectedDepartment === 'all';
    ? jobOpenings;
    : jobOpenings.filter(job => job.department === selectedDepartment)return (<UltraFuturisticBackground>;
...
export default function Careers() {const [selectedDepartment, setSelectedDepartment]  = useState('all')const departments = [;
    { id: 'all', name: 'All Departments' },{ id: 'engineering', name: 'Engineering' },{ id: 'ai-research', name: 'AI Research' },{ id: 'quantum', name: 'Quantum Computing' },{ id: 'space-tech', name: 'Space Technology' },{ id: 'sales', name: 'Sales & Business Development' },{ id: 'marketing', name: 'Marketing' },{ id: 'operations', name: 'Operations' }
  ];const jobOpenings = [;
    {id: 1,title: 'Senior AI Engineer',department: 'ai-research',location: 'Remote / Middletown, DE';
      type: 'Full-time',experience: '5+ years',,description: 'Lead the development of cutting-edge AI solutions and machine learning models.',requirements: [;
        'Advanced degree in Computer Science, AI, or related fieldExpertise in Python, TensorFlow, PyTorchExperience with large language models and neural networksStrong problem-solving and analytical skills';
      ];
}{id: 2,title: 'Quantum Computing Researcher',department: 'quantum',location: 'Remote / Middletown, DE';
      type: 'Full-time',experience: '3+ years',,description: 'Research and develop quantum algorithms and quantum computing solutions.',requirements: [;
...
}{id: 3,title: 'Space Technology Engineer',department: 'space-tech',location: 'Remote / Middletown, DE';
      type: 'Full-time',experience: '4+ years',,description: 'Design and develop innovative space technology solutions and satellite systems.',requirements: [;
...
}{id: 4,title: 'Full Stack Developer',department: 'engineering',location: 'Remote / Middletown, DE';
      type: 'Full-time',experience: '3+ years',,description: 'Build scalable web applications and cloud-based solutions.',requirements: [;
...
}{id: 5,title: 'Business Development Manager',department: 'sales',location: 'Remote / Middletown, DE';
      type: 'Full-time',experience: '5+ years',,description: 'Drive business growth through strategic partnerships and client relationships.',requirements: [;
    ...
    {icon: <Brain className="w-8 h-8" />,title: 'Innovation First',,description: 'We push boundaries and explore new technologies to solve complex problems.';
}{icon: <Heart className="w-8 h-8" />,title: 'People Matter',,description: 'Our team is our greatest asset, and we invest in their growth and well-being.';
}{icon: <Globe className="w-8 h-8" />,title: 'Global Impact',,description: 'We create solutions that make a difference in the world.';
}{icon: <Star className="w-8 h-8" />,title: 'Excellence',,description: 'We strive for the highest quality in everything we do.';
    }
  ];const benefits = [;
    'Competitive salary and equity packagesComprehensive health, dental, and vision insuranceFlexible work arrangements and remote work optionsProfessional development and training programsGenerous paid time off and holidays401(k) with company matchingModern technology and equipmentCollaborative and inclusive work environment';
  ];const filteredJobs = selectedDepartment === 'all' ;
    ? jobOpenings ;
    : jobOpenings && jobOpenings.filter(job => job && job.department === selectedDepartment)return (<UltraFuturisticBackground>;
      <div className="min-h-screen">;
        <Head>;
          <title>Careers - Zion Tech Group | Join Our Revolutionary Technology Team</title>;
          <meta name="description" content="Join Zion Tech Group's team of innovators. Explore career opportunities in AI, quantum computing, and emerging technologies. Remote work, competitive benefits, and cutting-edge projects." />;
        </Head>;<div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">;
        {/* Hero Section */}
        <motion&& motion.div;
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        {/* Company Culture */}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">;
            {companyValues && companyValues.map((value, index) => (<div key={index} className="bg-gradient-to-br from-gray-900/40 to-blue-900/20 p-6 rounded-2xl border border-gray-600/20 text-center">;
        ...
        <motion&& motion.div;
          ...
          <div className="bg-gradient-to-br from-gray-900/40 to-purple-900/20 p-8 rounded-2xl border border-gray-600/20 max-w-4xl mx-auto">;
            <div className="grid md:grid-cols-2 gap-6">;
              {benefits && benefits.map((benefit, index) => (<div key={index} className="flex items-center">;
        ...
        >;
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-8">;
            {departments && departments.map((dept) => (<button;
                key={dept && dept.id}
                onClick={() => setSelectedDepartment(dept && dept.id)}
                className={`px-4 py-2 rounded-lg font-medium transition-all duration-300 ${selectedDepartment === dept && dept.id;
          ...
          <div className="space-y-6">;
            {filteredJobs && filteredJobs.map((job) => (<motion&& motion.div;
                key={job && job.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
