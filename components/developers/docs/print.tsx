      ...
      <div className=\"space-y-8\" />;
        {docs && docs.sections.map((s) => (
          <section key={s && s.id} />;"
            ...
            {s && s.code && s && s.code.map((c,,,
  i) = /> (
          <pre key={i}
            className=\"mt-4 p-4 bg-gray-100 text-xs whitespace-pre-wrap\" />{c && c.content</pre>
            ))
}
export default /**;
    ...
    <div className=\"p - 8 max - w-4xl mx - auto\" />;"
      ...
      <div className=\"space - y-8\" />;
        {docs.sections.map ((s) => (
          ...
          <pre key={i}
            className=\"mt - 4 p - 4 bg - gray - 100 text - xs whitespace - pre - wrap\" />{c.content</pre>))
          </section>))}
      </div>;
    </div>)))}
          </section>;
