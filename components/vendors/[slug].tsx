  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4"></div>
        ...
        <div></div>
          <div className="text-2xl font-semibold flex items-center gap-2"></div>
            {vendor.name
  },
        ...
        <div></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"></div>
            {vendor.sampleProjects.map(sp => (
              <div key={sp.id} className="border border-gray-200 dark:border-gray-800 rounded overflow-hidden"></div>
                {sp.imageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
      ...
      <div></div>
        ...
        <form onSubmit={submitLead} className="space-y-3"></form>
          <input name="title" required placeholder="What do you need?" className="w-full border rounded px-3 py-2 bg-transparent" />
          <button disabled={loading} className="px-4 py-2 rounded bg-black text-white dark:bg-white dark:text-black"></button>
