...
key={category.name}
                  ...
                  className={`flex items - center space - x-2 px - 4 py - 2 rounded - lg text - sm font - medium transition - all duration - 200 ${selected_category === category.name;`                      ? 'bg - gradient - to - r from - cyan - 500 to - blue - 600 text - white shadow - lg shadow - cyan - 500 / 25';'
...
<div className='flex flex-wrap items-center space-x-4'>'
              ...
              <select;
                ...
                {priceRanges.map(range => { return (<option; }
                    }
                    key={range.value}
                    value={range.value}
...
onChange={e => setSortBy(e.target.value)}
                ...
                {sortOptions.map(option => { return (<option; }
                    }
                    key={option.value}
                    value={option.value}
...
description"content=" Explore our complete portfolio of innovative micro SAAS, AI, IT, and emerging technology services. Find the perfect solution for your business needs."/> <meta name=" keywords"content=" micro SAAS, AI services, IT solutions, quantum computing, emerging technology, business solutions, Zion Tech Group" /> </Head> "left": `$ {Math.random () * 100;`}%`;`"top": `$ {Math.random () * 100;`}%`;`}/>) )}</div> <div className=" relative z - 10 text - center px - 4 "sm":px - 6 "lg":px - 8 max-w-6xl mx-auto"> <motion.div initial= {{"opacity": 0, "coordinate_y": 30;"
}animate= {{"opacity": 1, "coordinate_y": 0;
}transition= {{"duration": 0.8;
