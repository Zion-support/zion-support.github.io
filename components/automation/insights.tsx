...
return (
    <div /> <Head /><title  /> Automation Insights — Zion</title></Head>) );
}</ul> </div> </div>);
}</section>) : (
          <pre className=\"mt - 3 whitespace - pre - wrap text - sm bg - gray - 50 dark:bg - gray - 900 p - 3 rounded border border - gray - 200 dark:border - gray - 800\"  /> {}
  talent_md;}
}</pre>);
}</section> </div>) import { useEffect, useState,
  from 'react';
import Head from 'next / head';
export default /**;
 * AutomationInsightsPage - Function description;
 */;
function AutomationInsightsPage() {const [intel, set_intel] = useState < any>(null)const [talent_md, setTalentMd]  = useState < string>('')useEffect (() => {fetch ('/data / automation / market - intel.json').then ((r) => r.json ()).then (set_intel).catch (() => {})fetch ('/data / automation / talent - highlights.md').then ((r) => r.text ()).then (setTalentMd).catch (() => {})}, [])return (
    <div />;
          ...
          <div className=\"mt-3 text-sm\" />}"
            ...
            <div className=\"mt-3\" />"
              ...
              <ul className=\"list-disc list-inside\" />
                {Object.entries(intel.insights.keywordCounts).map(([k,,,
  v]: any) => (}
