...
const PerformanceEnhancer: React.FC = () => {
  ...
  useEffect(() => {
    if (metrics.fcp) sendToAnalytics('FCP', metrics.fcp);
    if (metrics.lcp) sendToAnalytics('LCP', metrics.lcp);
    if (metrics.fid) sendToAnalytics('FID', metrics.fid);
    if (metrics.cls) sendToAnalytics('CLS', metrics.cls);
    if (metrics.ttfb) sendToAnalytics('TTFB', metrics.ttfb);
    if (metrics.loadTime) sendToAnalytics('LoadTime', metrics.loadTime);
  }, [metrics, sendToAnalytics]);

  if (!isVisible) return null;

  return (
    <div className="fixed bottom-4 left-4 bg-black bg-opacity-90 text-white p-4 rounded-lg text-xs font-mono max-w-xs">
