...
interface State  {hasError: boolean;
  ...
  return {hasError: true,error,errorInfo: null,}}override componentDidCatch(error: Error, errorInfo: ErrorInfo) {this.setState({error,errorInfo,})// Log error to console in development;
    ...
    try {// Example: Send to analytics;}
      if (typeof gtag !== 'undefined') {gtag('event', 'exception', {description: error.message,fatal: false,})}// Example: Send to custom endpoint;
      fetch('/api/error-reporting', {method: 'POST'}
  headers: {'Content-Type': 'application/json',},body: JSON.stringify({error: {message: error.message,stack: error.stack,name: error.name,},errorInfo: {componentStack: errorInfo.componentStack},timestamp: new Date().toISOString(),userAgent: navigator.userAgent,url: window.location.href,}),}).catch(() => {// Silently fail if error reporting fails;}
      })} catch (reportingError) {// Silently fail if error reporting fails;}
    }
  }private handleRetry = () => {this.setState({hasError: false,error: null,errorInfo: null,})}private handleReload = () => {window.location.reload()}override render() {if (this.state.hasError) {// Custom fallback UI;}
      ...
      return (<div className="min-h-screen flex items-center justify-center bg-gray-50" />;
          <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6" />;
            ...
            {this.props.showDetails && this.state.error && (<div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md" />;
                ...
                {this.state.error.stack && (<details className="mt-2" />;
