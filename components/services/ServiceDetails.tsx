...
export function ServiceDetails({ country }: ServiceDetailsProps) {
  ...
  const getDatacenters = (country: string): string[] => {
    ...
    return datacenterMap[country] || ['Multiple locations available'];
  };

  const getRegionalInstructions = (country: string): string => {
    return `Our technicians operate during business hours (8AM-6PM local time). Response times are typically within 4 hours for metropolitan areas. Please have site access permissions and contact details ready for our technicians. For remote locations additional travel fees may apply.`;
  };
