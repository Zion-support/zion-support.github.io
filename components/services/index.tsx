...
import { realOperationalServices,
  from '../../data/real-operational-services';
import { verified2025Additions,
  from '../../data/verified-2025-additions';
import { realServicesQ12025,
  from '../../data/real-services-q1-2025'
import { newVerifiedServicesQ22025,
  from '../../data/real-verified-services-q2-2025'
...
const mapLocalToServiceItem = (item: any): ServiceItem => ({slug: item.slug,title: item.name,description: item.description,provider: 'Zion Provider,priceRangeUSD: item.priceRangeUSD,categories: [item.category],rating: Math.round ((3.8 + Math.random () * 1.2) * 10) / 10}),const ServicesPage: NextPage = () => {const [services, set_services] = React.useState < ServiceItem[]>([])const [filtered, set_filtered] = React.useState < ServiceItem[]>([])const [filters, set_filters] = React.useState < Filters>({ categories: [] }),const [modal_open, setModalOpen] = React.useState (false)const [selected, set_selected]  = React.useState < ServiceItem | null>(null)export default /**;
 * ServicesIndexPage - Function description;
 */;
function ServicesIndexPage() {const all = (enhancedRealMicroSaasServices as unknown[]).concat (extra_services as any[];
  ...
  const category_aliases: Record < string, string> = {AI & Data': 'AI & DataAI & Machine Learning: AI & DataGenAI': 'AI & DataCloud & FinOps: Cloud & FinOpsCloud & Data': 'Cloud & FinOpsPlatform Engineering: Cloud & FinOpsObservability': 'ObservabilityObservability & Telemetry: ObservabilityQuality & Monitoring': 'Quality & MonitoringSecurity & Reliability: Quality & MonitoringSecurity & Compliance': 'Quality & MonitoringDeveloper Tools: Developer ToolsGrowth & Marketing': 'Developer Tools;
  }
  for (const s of all) {const service = s as { category?: string }
    const raw_cat = (service.category || ').trim ()const mapped = category_aliases[raw_cat] || (categories.includes (raw_cat) ? raw_cat : 'Developer Tools)by_category[mapped].push (s)}
  React.useEffect (() => {const next = services.filter ((s) => {// Category;
      if (=> filters.categories.includes (c))) return false) {$2;
}
      // Price;
