    ...
    <div className="fixed bottom-4 right-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4 max-w-sm z-50">
      ...
      <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
        <div className="text-xs text-gray-500 dark:text-gray-400">
          {metrics.fcp && (
            <div>FCP: {getScoreLabel('fcp', metrics.fcp)}</div>
          )}
          {metrics.lcp && (
            <div>LCP: {getScoreLabel('lcp', metrics.lcp)}</div>
          )}
          {metrics.fid && (
            <div>FID: {getScoreLabel('fid', metrics.fid)}</div>
          )}
          {metrics.cls && (
            <div>CLS: {getScoreLabel('cls', metrics.cls)}</div>
          )}
        </div>
      </div>
    </div>
