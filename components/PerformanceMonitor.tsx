...
case 'paint': if (entry.name === 'first-contentful-paint') {'              setMetrics(prev => ({ ...prev, fcp: entry.startTime }))'            }
    ...
    // Observe different types of performance entries
    try {
observer.observe({ entryTypes: ['paintlargest-contentful-paintfirst-input', 'layout-shift', 'navigation'] })'    } catch (e) {'      // Fallback for browsers that don&apost support all entry types''      observer.observe({ entryTypes: ['paint', 'largest-contentful-paint'] })'    }'
>>>>>>> cursor/automate-test-fix-improve-and-merge-code-48f3
