...
if (return null) {$2;
  ...
  }, [])function completeOnboarding() {try {if (typeof window !== 'undefined') {window.localStorage.setItem('zion_has_onboarded',,,
  '1')}
    } catch {}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4>
      <div className=w-full max-w-xl rounded-lg bg-white dark:bg-zinc-900 border border-gray-200 dark:border-gray-800 shadow-xl">
        ...
        <div className=p-4 space-y-4">
          ...
          <div className=flex gap-2">
            <button onClick={() => setRole(client)} className={`px-3 py-1.5 rounded-md border ${role === 'client' ? bg-blue-600 text-white border-blue-600 : 'border-gray-300 dark:border-gray-700'}`}>Im a Client</button>
            <button onClick={() => setRole(talent')} className={`px-3 py-1.5 rounded-md border ${role === 'talent ? bg-blue-600 text-white border-blue-600' : 'border-gray-300 dark:border-gray-700}`}>Im Talent</button>
          </div>
          {role === 'client' ? (
            <div className="space-y-2>
              <div className=text-base font-medium">Next steps</div>
