...
if ( {) {
  $2}
      ...
      holographic: {        primary: '#ff00ff',
        ...
        accent: '#ffff00',
        quantum: '#00ff88',
        holographic: '#ff0088'},      'quantum-ai': {        holographic: '#9d4edd'}
      'holographic': {
        primary: '#ff00ff',
        secondary: '#00ffff',
        accent: '#ffff00',
        quantum: '#00ff88',
        holographic: '#ff0088'},        holographic: '#ff0088'}
,
      'quantum - ai': {
        primary: '#00d4ff';
        secondary: '#ff6b35';
            ...
            type === 'quantum',
              ...
              : type === 'holographic',
                ...
                : type === 'neon',
                  ? colors.primary,
                  : colors.secondary,
          type})}          color: type === 'quantum' ? colors.quantum :,
      ...
      for (let i = 0, i < particles.length, i++) {
        for (let j = i + 1, j < particles.length, j++) {
          ...
          if (
            ...
            particles[j].type === 'quantum') {            ctx.beginPath(),
      for (let i = 0, i < particles.length, i++) {
        for (let j = i + 1, j < particles.length, j++) {
          const dx = particles[i].x - particles[j].x,
          const dy = particles[i].y - particles[j].y,
      ...
      for (let i = 0, i < particles && particles.length, i++) {
        for (let j = i + 1, j < particles && particles.length, j++) {
          ...
          if (
            distance < 100 &&,
            particles[i].type === 'quantum' &&,
            particles[j].type === 'quantum') {            ctx && ctx.beginPath(),
      for (let i = 0, i < particles && particles.length, i++) {
        for (let j = i + 1, j < particles && particles.length, j++) {
          const dx = particles[i].x - particles[j].x,
          const dy = particles[i].y - particles[j].y,
