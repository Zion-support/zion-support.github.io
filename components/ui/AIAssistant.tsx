    ...
    try {;
      const res = await fetch('/api/ai/operator', {;
        ...
        headers:{;
          ...
          ...(authorizationToken;
            ? { Authorization:`Bearer ${authorizationToken}` } process.env.NEXT_PUBLIC_OPERATOR_TOKEN;
...
export default function AIAssistant({
  ...
  const callOperator = useCallback(async () => {
    setLoading(true),
    setError(null),    try {
      const res = await fetch('/api/ai/operator', _{
        method: 'POST', headers: {
          'Content-Type': 'application/json', _...(authorizationToken
            ? { Authorization: `Bearer ${authorizationToken}` }
            : process.env.NEXTPUBLIC_OPERATOR_TOKEN
            ? {Authorization: `Bearer ${process.env.NEXTPUBLIC_OPERATOR_TOKEN}` }
            : {})},
        body: JSON.stringify({ prompt, system: systemPrompt })
      }),
      const data = await res.json(),
