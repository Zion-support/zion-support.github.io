...
const InteractiveNavigation: React.FC<InteractiveNavigationProps> = (_{items, className = '', _}) => {const [isOpen, setIsOpen] = useState(false);
...
onClick={toggleMenu}
        ...
        {isOpen && (_<div className="md:hidden py-4 border-t border-gray-200">
            {items.map(item => (
              <div key={item.label}>
                {item.children ? (
                  <div>
...
onClick={() => toggleDropdown(item.label)}
                    ...
                    {activeDropdown === item.label && (_<div className="pl-4">
                        {item.children.map(child => (
                          <Link,
key={child.href}
                            href={child.href}
