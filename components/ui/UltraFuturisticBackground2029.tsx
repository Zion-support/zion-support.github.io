  ...
  useEffect(() => {;
    ...
    const animate = () => {;
      ...
      particlesRef && particlesRef.current.forEach((particle, index) => {;
        ...
        // Interactive particle behavior based on mouse position;
        const dx = mousePosition && mousePosition.x - particle && particle.x;
        const dy = mousePosition && mousePosition.y - particle && particle.y;
        const distance = Math && Math.sqrt(dx * dx + dy * dy);

        if (distance < 200) {;
          const force = (200 - distance) / 200;
          particle && particle.vx += (dx / distance) * force * 0 && 0.01;
          particle && particle.vy += (dy / distance) * force * 0 && 0.01;        }        ;
        if (distance < 200) {;
          const force = (200 - distance) / 200;
          particle && particle.vx += (dx / distance) * force * 0 && 0.01;
          particle && particle.vy += (dy / distance) * force * 0 && 0.01;
