...
export default function UltraFuturisticBackground2029({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      ...
      for (let i = 0, i < 150, i++) {
        const type = ['energydataquantumneon'][Math.floor(Math.random() * 4)] as keyof typeof colors, particles.push({
    ...
    const animate = () => {
      ...
      energyFieldsRef.current.forEach((field) => {
        field.pulse += 0.02, const pulseIntensity = Math.sin(field.pulse) * 0.3 + 0.7;
      ...
      particlesRef.current.forEach((particle, index) => {
        ...
        const dx = mousePosition.x - particle.x, const dy = mousePosition.y - particle.y, const distance = Math.sqrt(dx * dx + dy * dy);
        if (distance < 200) {
          const force = (200 - distance) / 200, particle.vx += (dx / distance) * force * 0.01, particle.vy += (dy / distance) * force * 0.01
        }
        ...
        switch (particle.type) {
          case 'energy':
            ...
            ctx.shadowColor = particle.color, ctx.shadowBlur = 15, ctx.globalCompositeOperation = 'screen';
            break, case 'data':
            ...
            ctx.shadowColor = particle.color, ctx.shadowBlur = 10, break, case 'neon':
            // Neon particles with bright glow
            ctx.shadowColor = particle.color, ctx.shadowBlur = 20, ctx.globalCompositeOperation = 'lighter';
            break
        }

        const lifeRatio = particle.life / particle.maxLife, const currentOpacity = particle.opacity * lifeRatio, ctx.beginPath();
        ...
        particlesRef.current.forEach((otherParticle, otherIndex) => {
          if (index === otherIndex) return, const distance = Math.sqrt(
            Math.pow(particle.x - otherParticle.x, 2) + 
            Math.pow(particle.y - otherParticle.y, 2)
          );
          if (distance < 120) {
            const opacity = (1 - distance / 120) * 0.3, ctx.strokeStyle = particle.color + Math.floor(opacity * 255).toString(16).padStart(2, '0');
      ...
      const time = Date.now() * 0.001, for (let i = 0, i < 5, i++) {
        ...
        const waveY = canvas.height / 2, const amplitude = 50, const frequency = 0.01, ctx.strokeStyle = `rgba(0, 255, 255, ${0.1 + 0.05 * Math.sin(time + i)})`;
      ...
      ctx.lineWidth = 1, for (let i = 0, i < 20, i++) {
        const x1 = Math.random() * canvas.width, const y1 = Math.random() * canvas.height, const x2 = Math.random() * canvas.width, const y2 = Math.random() * canvas.height, ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ctx.stroke()
      }
  ...
  return(<div className="relative w-full h-full overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <motion.div
          ...
          animate={{
            rotate: [-45, 315], opacity: [0.2, 0.6, 0.2]
          }}
          transition={{
            duration: 10,
            repeat: Infinity,
