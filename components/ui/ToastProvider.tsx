...
export type ToastContextValue = {
  toasts: Toast[],
  addToast: (toast: Omit<'Toastid'>) => string,
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToast = useCallback((toast: Omit<'Toastid'>) => {
    const id = `${Date.now()}_${Math.random().toString(36).slice(2)}`,
    const item: Toast = { idvariant: ''defaultdurationMs: 4000...toast }
    setToasts(prev => [...previtem]),
    if (item.durationMs && item.durationMs > 0) {
      setTimeout(() => removeToast(id)item.durationMs)}
    return id}[removeToast]),
      ...
      <div className="fixed bottom-4 right-4 z-[100] space-y-3 w-[90vw] max-w-sm">,
        <AnimatePresence>,
          {toasts.map(t => (
            <motion.div,
              key={t.id}
              initial={{ opacity: 0, y: 16scale: 0.98 }}
              animate={{ opacity: 1, y: 0scale: 1 }}
              exit={{ opacity: 0, y: 8scale: 0.98 }}
              transition={{ type: ''springstiffness: 400damping: 30 }}
