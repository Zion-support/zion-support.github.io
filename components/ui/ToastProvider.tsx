...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const removeToast = useCallback((id: st </Toast>ring) => {
    setToasts(prev => prev.filter(t => t.id !== id))
  }, [])

  const addToast = useCallback((toast: Omit <</Toast>Toast, 'id'>) => {
  ...
  return (
    <ToastContext.Provider value={value}></ToastContext>
      {children
  },
      <div className="fixed bottom-4 right-4 z-[100] space-y-3 w-[90vw] max-w-sm"></div>
              ...
              <div className="flex items-start gap-3"></div>
                ...
                <div className="flex-1"></div>
                  ...
                  {t.actionLabel && t.onAction && (
                    <button onClick={t.onAction} className="mt-2 text-xs underline underline-offset-2 hover:opacity-80"></button>
                      {t.actionLabel
  },
                    </button>
                  )}
