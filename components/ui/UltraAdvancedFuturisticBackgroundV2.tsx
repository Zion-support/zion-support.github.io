...
const updateParticles = (ctx.clearRect(0, 0, canvas.width,,,
  canvas.height)// Create gradient background;

const gradient = ctx.createRadialGradient(canvas.width / 2,canvas.height / 2,0,canvas.width / 2,canvas.height / 2,
  canvas.width / 2;,,
   )gradient.addColorStop(0, 'rgba(0, 0, 0,,,
  0.8)')gradient.addColorStop(0.5, 'rgba(20, 20, 40,,,
  0.6)')gradient.addColorStop(1, 'rgba(0, 0, 0,,,
  0.9)')ctx.fillStyle = gradient;
...
ctx.stroke();
      ...
      ctx.lineWidth = 0.5;
// Vertical lines;
      for (let x = 0; x < canvas.width; x += 50) {ctx.beginPath()ctx.moveTo(x,,,
  0)ctx.lineTo(x,,,
  canvas.height)ctx.stroke()}
      // Horizontal lines;
      for (let y = 0; y < canvas.height; y += 50) {ctx.beginPath()ctx.moveTo(0,,,
  y)ctx.lineTo(canvas.width,,,
  y)ctx.stroke()}// Draw holographic circles;
...
const radius = 100 + Math.sin(Date.now() * 0.002 + i) * 50;}
        ...
        ctx.lineWidth = 2;
        ctx.beginPath()ctx.arc(centerX, centerY, radius, 0,,,
  Math.PI * 2)ctx.stroke()}animationFrameId = requestAnimationFrame(updateParticles)}// Handle resize;

const handleResize = (canvas.width = window.innerWidth;
      canvas.height = window.innerHeight;) => {
  return $3;}
}
initParticles()}window.addEventListener('resize',,,
  handleResize)initParticles()updateParticles()return () => {window.removeEventListener('resize',,,
  handleResize)if (animationFrameId) {cancelAnimationFrame(animationFrameId)}
    }}, [])return (
    <div className={`relative min-h-screen overflow-hidden ${className}`} />
      ...
      <canvas ref={canvasRef}
            ...
            style={{background:
            'radial-gradient(circle at center, rgba(0,0,0,,,
  0.8) 0%, rgba(20,20,40,}
            0.6) 50%, rgba(0,0,0,
  0.9) 100%)';
...
<div className='fixed inset-0 pointer-events-none z-20' />;
        {[...Array(20)].map((_, i) => (
          <motion.div;}
            key={i className='absolute w - 2 h - 2 bg - cyan - 400 rounded - full'}
