...
const initParticles = (particles = [];) => {
  return $3;}
  for($2) {particles.push({x: Math.random() * canvas.width,y: Math.random() * canvas.height,vx: (Math.random() - 0.5) * 2,vy: (Math.random() - 0.5) * 2,size: Math.random() * 3 + 1,color: colors[Math.floor(Math.random() * colors.length)],life: Math.random() * 100,maxLife: 100})}
...
ctx.stroke()
      ...
      ctx.lineWidth = 0.5
// Vertical lines
  for($2) {ctx.beginPath()ctx.moveTo(x, 0)ctx.lineTo(x, canvas.height)ctx.stroke()}
      // Horizontal lines
  for($2) {ctx.beginPath()ctx.moveTo(0, y)ctx.lineTo(canvas.width, y)ctx.stroke()}// Draw holographic circles
