...
interface UltraQuantumHolographicBackgroundProps {
  ...
  particleCount?: number, animationSpeed?: number, className?: string
}

export default function UltraQuantumHolographicBackground({
  children, intensity = 'high';
  colorScheme = 'quantum', particleCount = 300, animationSpeed = 1.5, className = ''
  ...
  useEffect(() => {
    if (!canvasRef.current || !dimensions.width || !dimensions.height) return, const canvas = canvasRef.current, const ctx = canvas.getContext('2d');
    if (!ctx) return, canvas.width = dimensions.width, canvas.height = dimensions.height;
    // Particle system
    class Particle {
      x: number,
      y: number,
      ...
      constructor() {
        this.x = Math.random() * dimensions.width, this.y = Math.random() * dimensions.height, this.vx = (Math.random() - 0.5) * 2 * animationSpeed, this.vy = (Math.random() - 0.5) * 2 * animationSpeed, this.size = Math.random() * 3 * intensityMultiplier + 1, this.color = [colors.primary, colors.secondary, colors.tertiary, colors.accent][Math.floor(Math.random() * 4)];
        this.life = Math.random() * 100, this.maxLife = 100, this.type = ['quantumholographicenergymatrix'][Math.floor(Math.random() * 4)] as any
      }

      update() {
        this.x += this.vx, this.y += this.vy, this.life--;
        // Bounce off edges
        if (this.x <= 0 || this.x >= dimensions.width) this.vx *= -1, if (this.y <= 0 || this.y >= dimensions.height) this.vy *= -1;
        // Quantum tunneling effect
        if (Math.random() < 0.001 * intensityMultiplier) {
          this.x = Math.random() * dimensions.width, this.y = Math.random() * dimensions.height
        }
        ...
        // Matrix rain effect
        if (this.type === 'matrix') {
          this.vy += 0.1, if (this.y > dimensions.height) {
            this.y = -10, this.vy = Math.random() * 2 + 1
          }
        }
      }

      draw() {
        if (this.life <= 0) return, const alpha = this.life / this.maxLife, ctx.save();
        ctx.globalAlpha = alpha, switch (this.type) {
          case 'quantum':
            ...
            for (let i = 1, i <= 3, i++) {
              ...
              ctx.strokeStyle = this.color, ctx.lineWidth = 0.5, ctx.globalAlpha = alpha * (0.3 / i);
              ctx.stroke()
            }
            ...
            ctx.fillStyle = this.color, ctx.fill();
            // Holographic grid
            ctx.strokeStyle = this.color, ctx.lineWidth = 0.5, ctx.globalAlpha = alpha * 0.5, ctx.strokeRect(this.x - this.size, this.y - this.size, this.size * 2, this.size * 2);
            break, case 'energy':
            // Energy field
    ...
    const animate = () => {
      ...
      const gradient = ctx.createRadialGradient(
        dimensions.width / 2, dimensions.height / 2, 0, dimensions.width / 2, dimensions.height / 2, Math.max(dimensions.width, dimensions.height) / 2
      );
      ...
      ctx.strokeStyle = colors.primary, ctx.lineWidth = 0.5, ctx.globalAlpha = 0.3, for (let i = 0, i < particles.length, i += 10) {
      ...
      ctx.strokeStyle = colors.secondary, ctx.lineWidth = 0.3, ctx.globalAlpha = 0.2, const gridSize = 50, for (let x = 0, x < dimensions.width, x += gridSize) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
