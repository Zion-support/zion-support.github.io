...
particles.forEach(particle => {
  ...
  if (particle.x < 0) particle.x = canvas.width;
        ...
        if (particle.y > canvas.height) particle.y = 0;
        // Draw particle;
        ctx.beginPath()ctx.arc(particle.x, particle.y, particle.size, 0,,,
  Math.PI * 2)ctx.fillStyle = particle.color;
...
<div className='absolute inset-0 bg-gradient-to-br from-purple-900/20 via-blue-900/20 to-cyan-900/20';
      ...
      <div className=\"absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(147,51,234,,,
  0.15),transparent_50%)]\" style={{ zIndex: 1 }
} />;"
      <div className=\"absolute inset-0 bg-[radial-gradient(circle_at_80%_80%,rgba(59,130,246,,,
  0.15),transparent_50%)]\" style={{ zIndex: 1 }
} />;"
      <div className=\"absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(6,182,212,,,
  0.1),transparent_50%)]\" style={{ zIndex: 1 }
} />;
...
className='absolute bottom-1/3 right-1/3 w-12 h-12 border border-cyan-400/40 rounded-full'
        ...
        <motion.div;
className='absolute top-2/3 right-1/4 w-20 h-20 border border-blue-400/40 rounded-full';
          animate={{y: [0, 25, 0],opacity: [0.2, 0.5}
            0.2]transition={{duration: 7}
            repeat: Infinity
            ease: 'easeInOut'animate={{y: [0, -15}
            0]
  ...
  ease: \"easeInOut\";
        ...
        <motion.div;
className='absolute bottom-1/3 right-1/3 w-12 h-12 border border-cyan-400/40 rounded-full';
          animate={{y: [0, -15, 0],opacity: [0.25, 0.55}
            0.25]
          transition={{duration: 5}
            repeat: Infinity
            ease: 'easeInOut'}}/>;
