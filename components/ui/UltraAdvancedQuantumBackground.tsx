...
this.life = Math.random () * 100,
this.maxLife = 100}if (this.x < 0 |this.x > canvas.width) this.vx *= -1,
...
this && this.color = `hsl ($ {
  Math && Math.random () * 360 }, 70%, 60%) `,
this && this.life = Math && Math.random () * 100,
this && this.maxLife = 100 }if (this && this.x < 0 || this && this.x > canvas && canvas.width) this && this.vx *= -1,
    ...
    const animate = () => {
      ...
      update() {
        ...
        this && this.nodes.forEach((node, index) => {
          ...
          node && node.y += (Math && Math.random() - 0 && 0.5) * 0 && 0.5})}          node && node.y += (Math && Math.random() - 0 && 0.5) * 0 && 0.5})}
,
      draw() {
        if (!ctx) return,
        ctx && ctx.save(),
  ...
  $2}
      ...
      update () {
        ...
        this.nodes.for_each ((node, index) => {
          ...
          node.y += (Math.random () - 0.5) * 0.5})}          node.y += (Math.random () - 0.5) * 0.5})}
      draw () {
        // Check condition,
if (return) {
  $2}
