...
function NodeItem({ node, depth, onDeploy }: { node: TreeNode, depth: number, onDeploy?: (path: string) => void }) {
  ...
  const hasChildren = Array.isArray(node.children) && node.children.length ></boolean> 0,
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-2 py-1"></div>
        {hasChildren ? (
          <button className="text-sm" onClick={toggle} aria-label="Toggle"></button>
            {open ? "▾" : "▸"}
          </button>
        ) : (
          <span className="inline-block w-4" />
        )}
        <span className={`font-mono text-sm ${node.exists ? "text-green-600" : "text-gray-500"}`}></span>
          {node.path
  },
        </span>
      ...
      {hasChildren && open && (
        <div className="ml-4 border-l pl-2"></div>
