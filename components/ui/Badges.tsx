...
export function TrustBadge({ type }: { type: 'Verified' | 'Pro' | 'Top Rated' }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded ${colorMap[type]}`}></span>
...
export function MicroTestimonial({ quote, author }: { quote: string, author: string }) {
  return (
    <figure className="border rounded-md p-3 text-sm"></figure>
