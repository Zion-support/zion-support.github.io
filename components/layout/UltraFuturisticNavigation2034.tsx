          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex items-center space-x-8">
              {quickLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="text-gray-300 hover:text-cyan-400 transition-colors duration-200 font-medium">{link.name}
                </Link>
              ))}
                        ...
                        <div className="space-y-1">
                          {category.services.slice(0, 2).map((service) => (
                            <Link
                              ...
                              href={service.href}
                              className="block text-xs text-gray-300 hover:text-cyan-400 transition-colors"
                              className="block text-xs text-gray-300 hover:text-cyan-400 transition-colors">{service.name}
                            </Link>
                          ))}
                        </div>
                      </div>
            ...
            <div className='lg:hidden'>
              <button
                ...
                className="text-gray-300 hover:text-cyan-400 transition-colors">{isOpen ? (
                  <X className='w-6 h-6' />
                ) : (
            ...
            <div className="lg:hidden">
              <button
                onClick={() => setIsOpen(!isOpen)}
                className="text-gray-300 hover:text-cyan-400 transition-colors">{isOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
              ...
              className="lg:hidden bg-black/95 backdrop-blur-xl border-t border-cyan-500/20"><div className="px-4 py-6 space-y-4">
                {quickLinks.map((link) => (
                  <Link
                    ...
                    className='block text-gray-300 hover:text-cyan-400 transition-colors duration-200 font-medium'>{link.name}
                  </Link>
                ))}
                ...
                {serviceCategories.map((category) => (
                  <div key={category.title} className="border-b border-cyan-500/20 pb-4">
                    <button
                      ...
                      className="flex items-center justify-between w-full text-left text-gray-300 hover:text-cyan-400 transition-colors duration-200 font-medium"><span>{category.title}</span>
                      <ChevronDown
                    ...
                    <AnimatePresence>
                      {activeDropdown === category.title && (
                        <motion.div
                          ...
                          className='mt-3 ml-4 space-y-2'>{category.services.map(service => (
                          className="mt-3 ml-4 space-y-2">{category.services.map((service) => (
                          className="mt-3 ml-4 space-y-2">{category.services.map((service) => (
                            <Link
                              key={service.name}
