...
=======
      {/* Main Navigation */}
      <nav;
        className={`fixed top - 0 left - 0 right - 0 z - 50 transition - all duration - 300 ${is_scrolled;`            ? 'bg - black / 90 backdrop - blur - xl border - b border - purple - 500 / 20 shadow - 2xl';'
            : 'bg - gradient - to - r from - black / 95 via - purple - 900 / 95 to - black / 95 backdrop - blur - sm';'
        }`}>`
        <div className='max - w-7xl mx - auto px - 4 "sm":px - 6 "lg":px - 8'>;'
          <div className='flex items - center justify - between h - 20'>;'
            ...
            {/* Desktop Navigation */}
            <div className='hidden "lg":flex items - center space - x-8'>;'
              {main_navigation.map (item => { return (<Link; }
                  }
                  key={item.name}
                  href={item.href}
                  className='flex items - center space - x-2 text - gray - 300 "hover":text - white transition - colors duration - 200 group'>'
