        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <motion.div
              ...
              className="flex items-center space-x-2"><Link href="/" className="flex items-center space-x-2 group">
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
                ...
                <div key={item.name} className="relative group">
                  {item.dropdown ? (
                    <div
                      ...
                      className='flex items-center space-x-1 cursor-pointer text-gray-300 hover:text-white transition-colors duration-300'><span className='font-medium'>{item.name}</span>
                      ...
                      <AnimatePresence>
                        {activeDropdown === item.name && (
                          <motion.div
                            ...
                            className='absolute top-full left-0 mt-2 w-80 bg-gray-900/95 backdrop-blur-xl rounded-2xl border border-cyan-500/20 shadow-2xl shadow-cyan-500/25 p-4'><div className="grid grid-cols-1 gap-3">
                    ...
                    <Link
                      href={item.href}
                      className='flex items-center space-x-2 text-gray-300 hover:text-white transition-colors duration-300 group'><item.icon className='w-4 h-4 group-hover:scale-110 transition-transform duration-300' />
            ...
            <div className='lg:hidden'>
              <motion.button
                ...
                className="text-gray-300 hover:text-white transition-colors duration-300">{isOpen ? (
                  <X className='w-6 h-6' />
                ) : (
            ...
            <div className="lg:hidden">
              <motion.button
                whileTap={{ scale: 0.95 }}
                onClick={() => setIsOpen(!isOpen)}
                className="text-gray-300 hover:text-white transition-colors duration-300">{isOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className='fixed inset-0 z-40 lg:hidden'>{/* Backdrop */}
            <div 
            ...
            <motion.div
              ...
              exit={{ x: '100%' }}
              transition={{ duration: 0.3 }}
              className='absolute right-0 top-0 h-full w-80 bg-gray-900/95 backdrop-blur-xl border-l border-cyan-500/20 shadow-2xl'><div className="p-6">
                {/* Close Button */}
                <div className="flex justify-end mb-6">
                  <motion.button
                    whileTap={{ scale: 0.95 }}
                    onClick={() => setIsOpen(false)}
                    className="text-gray-300 hover:text-white transition-colors duration-300"><X className='w-6 h-6' />
              className="absolute right-0 top-0 h-full w-80 bg-gray-900/95 backdrop-blur-xl border-l border-cyan-500/20 shadow-2xl"><div className="p-6">
                {/* Close Button */}
                <div className="flex justify-end mb-6">
                  <motion.button
                            ...
                            {item.dropdown.map((dropdownItem) => {
                              ...
                              return (
                                <Link
                                  ...
                                  href={dropdownItem.href}
                                  onClick={() => setIsOpen(false)}
                                  className="flex items-center space-x-3 p-3 rounded-lg hover: bg-cyan-500/10 transition-all duration-300"><Icon className="w-5 h-5 text-cyan-400" />
                                  <div>
                    ...
                    <div key={item.name}>
                      {item.dropdown ? (
                        ...
                        <Link
                          href={item.href}
                          onClick={() => setIsOpen(false)}
                          className='flex items-center space-x-3 p-3 rounded-lg hover:bg-cyan-500/10 transition-all duration-300'><item.icon className='w-5 h-5 text-cyan-400' />
