...
import { motion, AnimatePresence,
  from 'framer-motion';
import { Menu, X, ChevronDown, Search, Phone, Mail, MapPin, Rocket, Brain, Atom, Shield,
  from 'lucide-react';
...
{ name: 'Pricing'}
  href: '/pricing'}
      ...
      { name: 'Pricing'}
  href: '/pricing'];name: 'View All Services',
...
className='absolute top-full left-0 mt-2 w-80 bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 rounded-2xl shadow-2xl overflow-hidden' />
  ...
  idx) => (
          <Link key={dropdownItem.name href={dropdownItem.href className='flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-800/50 transition-all duration-300 group/item' />
...
className='ml-4 mt-2 space-y-2'
  ...
  <Link key={dropdownItem.name href={dropdownItem.href className='flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-800/50 transition-all duration-300'} onClick={() = /> setIsOpen(false)

                            >
                              <div className={`w-8 h-8 bg-gradient-to-r ${dropdownItem.color}
        ...
        {is_open && (
          ...
          <div />;
                      <button on_click={() = />;}
                        ...
                        className='flex items - center justify - between w - full text - left text - gray - 300 hover:text - white transition - colors duration - 300 py - 3 px - 4 rounded - lg hover:bg - gray - 800 / 50';
                        <span />{item.name}</span>;
                        <ChevronDown className={`w - 4 h - 4 transition - transform duration - 300 ${active_dropdown === item.name ? 'rotate - 180' : '';}
                          }`}
                        />;
                      </button>;
          ...
          <motion.div;}
            ...
            coordinate_y: 0
                          ...
                          exit={{ opacity: 0
            coordinate_y: -10 transition={{ duration: 0.2 
}
                          className='ml - 4 mt - 2 space - y-2' />

                          {item.dropdown_items?.map (dropdown_item => (
          <Link key={dropdown_item.name href={dropdown_item.href className='flex items - center space - x-3 p - 3 rounded - lg hover:bg - gray - 800 / 50 transition - all duration - 300'}
            on_click={() = /> setIsOpen (false)
                            >}
            <div className={`w - 8 h - 8 bg - gradient - to - r ${dropdown_item.color
            rounded - lg flex items - center justify - center` />
