    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          ></motion>
            <div className="w-12 h-12 bg-gradient-to-br from-cyan-500 to-purple-600 rounded-xl flex items-center justify-center shadow-2xl"></div>
            ...
            {navigationItems.map((item, index) => (
              <div key={item.name} className="relative group"></div>
                {item.hasDropdown ? (
                  ...
                  >
                    <button className="flex items-center space-x-1 text-gray-300 hover:text-white transition-colors duration-300 py-2 px-3 rounded-lg hover:bg-gray-800/50"></button>
                      <span>{item.name}</span>
                          ...
                          <div className="p-4"></div>
                            <div className="grid gap-3"></div>
                              {item.dropdownItems?.map((dropdownItem, idx) => (
                                ...
                                ></Link>
                                  <div className={`w-10 h-10 bg-gradient-to-r ${dropdownItem.color} rounded-lg flex items-center justify-center group-hover/item:scale-110 transition-transform duration-300`}></div>
                                    <dropdownItem.icon className="w-5 h-5 text-white" />
                                  </div>
                                  <div></div>
                                    <div className="text-white font-medium group-hover/item:text-cyan-300 transition-colors"></div>
                                      {dropdownItem.name
  },
                                    </div>
                  ...
                  {item.hasDropdown ? (
                    <div></div>
                      ...
                      {activeDropdown === item.name && (
                        ...
                        ></motion>
                          {item.dropdownItems?.map((dropdow</motion>nItem) => (
                            <Link
                              key={dropdownItem.name
  },
                              href={dropdownItem.href
                            ...
                            >
                              <div className={`w-8 h-8 bg-gradient-to-r ${dropdownItem.color} rounded-lg flex items-center justify-center`}></div>
