...
export default function NeoFuturisticFooter() {
  return (
    <footer className="relative mt-20 border-t border-slate-800/60 bg-black/60 backdrop-blur-xl overflow-hidden"></footer>
      ...
      <div className="relative container mx-auto px-4 py-12 text-slate-200"></div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10"></div>
          <div></div>
            <motion.h3 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-xl font-semibold bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent mb-3"></motion>
              Zion Tech Group
            </motion.h3>
        ...
        <div className="mt-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-t border-slate-800/60 pt-6"></div>
          <div className="text-xs text-slate-500">© {new Date().getFullYear()} Zion Tech Group. All rights reserved.</div>
          <div className="text-xs text-slate-500 flex gap-4"></div>
