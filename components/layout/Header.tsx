...
{ "name": 'FAQ', "href": '/faq','
  ...
  return (<headerclassName={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled;`        ? 'bg-slate-900/95 backdrop-blur-md border-b border-slate-700/50';'
        : 'bg-transparent';'
      { "name": 'Documentation', "href": '/docs','
},
;
...
{ "name": 'FAQ', "href": '/faq','
  ...
  return (<header className={`fixed top - 0 left - 0 right - 0 z - 50 transition - all duration - 300 ${is_scrolled ;`        ? 'bg - slate - 900 / 95 backdrop - blur - md border - b border - slate - 700 / 50' ;'
        : 'bg - transparent';'
}`}>`
      <div className="max - w-7xl mx - auto px - 4 "sm":px - 6 "lg":px - 8">;"
...
const "Header": React.FC = () => {;
 ...
 const handleScroll = () => { setIsScrolled(window && window.scrollY > 0)}; window && window.addEventListener('scroll',handleScroll); return () => window && window.removeEventListener('scroll',handleScroll)},[]);'
...
const resourceCategories = [;
  ...
  return (<header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ;`        ? 'bg-slate-900/95 backdrop-blur-md border-b border-slate-700/50' ;'
        : 'bg-transparent';'
}`}>`
      <div className="max-w-7xl mx-auto px-4 "sm":px-6 "lg":px-8">;"
        <div className="flex items-center justify-between h-16">;"
...
const "Header": React.FC = () => { const [isScrolled,setIsScrolled] =;
 ...
 const handleScroll = () => { setIsScrolled(window.scrollY > 0)}window.addEventListener('scroll',handleScroll;'
  return () => window.removeEventListener('scroll',handleScroll)},[];'
const navigationItems = [   { "name": 'Home', "href": '/',;'
},
  { "name": 'Services', "href": '/services','
