...
const Header: React.FC = () => {
  const [mobileMenuOp,e,n, setMobileMenuOpen] = useState(false);

  const navigation = [;
    { name: 'Hom,e,', href: '/', },;
    { name: 'All Service,s,', href: '/service,s', },;
    { name: 'Catalo,g,', href: '/services-catalo,g', },;
    { name: 'Cloud DevOp,s,', href: '/cloud-devop,s', },;
    { name: 'Cybersecurit,y,', href: '/cybersecurit,y', },;
    { name: 'Quantu,m,', href: '/quantum-computin,g', },;
    { name: 'Doc,s,', href: '/doc,s', },;
      ...
      <nav className="max-w-7xl mx-auto px-4 sm: px-6 lg:px-8">";
        <div className="flex items-center justify-between h-16">;
          ...
          <div className="md: hidden">;
            <button;
              onClick={() => setMobileMenuOpen(!mobileMenuOpe,n,)}";
              className="text-gray-300 hover: text-white p-2"";
              aria-label="Toggle mobile menu";
            >;
              {mobileMenuOpen ? <X siz,e={2,4} /> : <Menu size={24} />}
            </button>;
          </div>;
        </div>;
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-slate-800/50 backdrop-blur-sm rounded-lg mt-2">;
              {navigation.map((item) => (;
                <Link;
                  ...
                  onClick={() => setMobileMenuOpen(fals,e,)}
                >;
                  {item.name}
                </Link>;
              ))}";
