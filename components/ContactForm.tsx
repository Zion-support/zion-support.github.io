...
interface FormData {
  ...
  phone: '',service: '',}
  message: '';}
  })const [isSubmitting, setIsSubmitting] = useState(false)const [submitStatus, setSubmitStatus]  = useState<'idle' | 'success' | 'error'>('idle')const handleInputChange = (const { name, value ) => {
  return $3;}
} = e.target;
  ...
  })const [isSubmitting, setIsSubmitting] = useState(false)const [isSubmitted, setIsSubmitted] = useState(false)const handleChange = (const { name, value ) => {
  return $3;}
} = e.target;
    setFormData(prev => ({...prev,[name]: value;}
...
const handleSubmit = async (e: React.FormEvent) => {e.preventDefault()setIsSubmitting(true)setSubmitStatus('idle')try {// Simulate form submission;
  ...
  const handleChange = (const { name, value ) => {
  return $3;}
} = e.target;
    setFormData(prev => ({...prev,[name]: value;}
    }))}
