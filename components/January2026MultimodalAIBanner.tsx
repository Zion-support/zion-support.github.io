import React from 'react';

const January2026MultimodalAIBanner: React.FC = () => {
  return (
    <section className="py-16 bg-gradient-to-r from-emerald-900/40 via-teal-900/40 to-cyan-900/40 backdrop-blur-sm border-b border-emerald-500/20">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border border-emerald-500/30 mb-6 animate-pulse">
            <span className="text-emerald-400 font-bold text-xl tracking-wider uppercase">
              🔗 BREAKING: January 2026 Multimodal AI Integration Breakthrough
            </span>
          </div>
          <h2 className="text-4xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 bg-clip-text text-transparent">
            Unified Intelligence Revolution
          </h2>
          <p className="text-xl text-gray-300 max-w-4xl mx-auto mb-8">
            Revolutionary multimodal AI systems that seamlessly integrate text, images, audio, video, and sensor data 
            for comprehensive enterprise intelligence and automation.
          </p>
        </div>

        {/* Data Modalities */}
        <div className="grid md:grid-cols-5 gap-6 mb-12">
          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-emerald-500/50 transition-all duration-300">
            <div className="text-4xl mb-4">📝</div>
            <h3 className="text-xl font-bold text-white mb-2">Text Intelligence</h3>
            <p className="text-gray-300 text-sm">Advanced NLP and understanding</p>
          </div>
          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-teal-500/50 transition-all duration-300">
            <div className="text-4xl mb-4">🖼️</div>
            <h3 className="text-xl font-bold text-white mb-2">Visual Recognition</h3>
            <p className="text-gray-300 text-sm">Image and video analysis</p>
          </div>
          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-cyan-500/50 transition-all duration-300">
            <div className="text-4xl mb-4">🎵</div>
            <h3 className="text-xl font-bold text-white mb-2">Audio Processing</h3>
            <p className="text-gray-300 text-sm">Speech and emotion detection</p>
          </div>
          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-blue-500/50 transition-all duration-300">
            <div className="text-4xl mb-4">📹</div>
            <h3 className="text-xl font-bold text-white mb-2">Video Analysis</h3>
            <p className="text-gray-300 text-sm">Real-time video intelligence</p>
          </div>
          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-green-500/50 transition-all duration-300">
            <div className="text-4xl mb-4">📊</div>
            <h3 className="text-xl font-bold text-white mb-2">Sensor Data</h3>
            <p className="text-gray-300 text-sm">IoT and sensor integration</p>
          </div>
        </div>

        {/* Key Benefits */}
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 rounded-xl p-6 border border-emerald-500/20">
            <h3 className="text-xl font-bold text-white mb-4">🎯 Unified Customer Understanding</h3>
            <ul className="text-gray-300 space-y-2">
              <li>• Omnichannel customer profiles</li>
              <li>• Emotion recognition across channels</li>
              <li>• Predictive customer analytics</li>
              <li>• Personalized experiences</li>
            </ul>
          </div>
          <div className="bg-gradient-to-br from-teal-500/10 to-cyan-500/10 rounded-xl p-6 border border-teal-500/20">
            <h3 className="text-xl font-bold text-white mb-4">⚡ Intelligent Automation</h3>
            <ul className="text-gray-300 space-y-2">
              <li>• Cross-modal process optimization</li>
              <li>• Real-time decision making</li>
              <li>• Predictive maintenance</li>
              <li>• Quality assurance</li>
            </ul>
          </div>
          <div className="bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-xl p-6 border border-cyan-500/20">
            <h3 className="text-xl font-bold text-white mb-4">🚀 Innovation Acceleration</h3>
            <ul className="text-gray-300 space-y-2">
              <li>• Cross-modal insights</li>
              <li>• Creative problem solving</li>
              <li>• Market intelligence</li>
              <li>• Competitive advantage</li>
            </ul>
          </div>
        </div>

        {/* Success Metrics */}
        <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 rounded-2xl p-8 border border-emerald-500/20 mb-12">
          <h3 className="text-2xl font-bold text-white mb-6 text-center">Proven Enterprise Results</h3>
          <div className="grid md:grid-cols-4 gap-6">
            <div className="text-center">
              <div className="text-3xl font-extrabold text-emerald-400 mb-2">$75B</div>
              <div className="text-sm text-emerald-300">Value Creation</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-extrabold text-teal-400 mb-2">91%</div>
              <div className="text-sm text-teal-300">Customer Satisfaction</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-extrabold text-cyan-400 mb-2">145%</div>
              <div className="text-sm text-cyan-300">Process Efficiency</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-extrabold text-blue-400 mb-2">234%</div>
              <div className="text-sm text-blue-400">Sales Conversion</div>
            </div>
          </div>
        </div>

        {/* Industry Applications */}
        <div className="mb-12">
          <h3 className="text-2xl font-bold text-white mb-6 text-center">Industry Applications</h3>
          <div className="grid md:grid-cols-4 gap-4">
            <div className="bg-white/5 backdrop-blur-lg rounded-lg p-4 border border-white/10 text-center">
              <div className="text-2xl mb-2">🏥</div>
              <div className="text-white font-semibold">Healthcare</div>
              <div className="text-gray-400 text-sm">Medical intelligence platform</div>
            </div>
            <div className="bg-white/5 backdrop-blur-lg rounded-lg p-4 border border-white/10 text-center">
              <div className="text-2xl mb-2">🏭</div>
              <div className="text-white font-semibold">Manufacturing</div>
              <div className="text-gray-400 text-sm">Production intelligence</div>
            </div>
            <div className="bg-white/5 backdrop-blur-lg rounded-lg p-4 border border-white/10 text-center">
              <div className="text-2xl mb-2">🏦</div>
              <div className="text-white font-semibold">Financial Services</div>
              <div className="text-gray-400 text-sm">Risk & fraud detection</div>
            </div>
            <div className="bg-white/5 backdrop-blur-lg rounded-lg p-4 border border-white/10 text-center">
              <div className="text-2xl mb-2">🛒</div>
              <div className="text-white font-semibold">Retail</div>
              <div className="text-gray-400 text-sm">Customer experience</div>
            </div>
          </div>
        </div>

        {/* Call to Action */}
        <div className="text-center">
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <a
              href="/blog/ai-2026-january-multimodal-ai-enterprise-integration-breakthrough"
              className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-emerald-500/50 transform hover:-translate-y-1"
            >
              Read Full Article →
            </a>
            <a
              href="/case-studies/ai-2026-january-multimodal-ai-enterprise-75-billion-success"
              className="bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-400 hover:to-cyan-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-teal-500/50 transform hover:-translate-y-1"
            >
              View $75B Success Story →
            </a>
            <a
              href="/services/ai-2026-january-multimodal-ai-enterprise-solutions"
              className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-cyan-500/50 transform hover:-translate-y-1"
            >
              Get Enterprise Solutions →
            </a>
          </div>
          <p className="text-gray-400 text-sm max-w-2xl mx-auto">
            Transform your enterprise with unified intelligence. Our multimodal AI solutions seamlessly integrate all your 
            data sources to deliver comprehensive insights and automated decision-making.
          </p>
        </div>
      </div>
    </section>
  );
};

export default January2026MultimodalAIBanner;