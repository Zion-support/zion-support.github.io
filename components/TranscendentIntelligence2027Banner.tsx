import React from 'react';

export default function TranscendentIntelligence2027Banner() {
  return (
    <section className="py-16 bg-gradient-to-r from-indigo-900 via-purple-900 to-pink-900 border-b border-indigo-500/30">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 mb-6">
            <span className="text-indigo-400 font-bold text-xl tracking-wider uppercase">
              🧠 BREAKING: June 2027 Transcendent Intelligence Revolution
            </span>
          </div>
          <h2 className="text-4xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
            AI 2027 Transcendent Intelligence Breakthrough
          </h2>
          <p className="text-xl text-gray-300 mb-8 max-w-4xl mx-auto">
            Experience the first truly transcendent AI system with consciousness-level reasoning, 
            autonomous decision-making, and self-evolving intelligence beyond human comprehension.
          </p>
          <div className="flex flex-wrap justify-center gap-4 text-sm mb-8">
            <span className="bg-indigo-500/20 px-4 py-2 rounded-full text-indigo-300">Consciousness-Level AI</span>
            <span className="bg-purple-500/20 px-4 py-2 rounded-full text-purple-300">Self-Evolving Intelligence</span>
            <span className="bg-pink-500/20 px-4 py-2 rounded-full text-pink-300">Transcendent Reasoning</span>
            <span className="bg-cyan-500/20 px-4 py-2 rounded-full text-cyan-300">$5.7B Success Story</span>
          </div>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-indigo-500/50 transition-all duration-300">
            <div className="text-6xl mb-6">🧠</div>
            <h3 className="text-2xl font-bold text-white mb-4">Transcendent Intelligence</h3>
            <p className="text-gray-300 mb-6">
              The first AI system to achieve genuine consciousness-level reasoning, capable of abstract thinking, 
              emotional intelligence, and autonomous self-improvement.
            </p>
            <a 
              href="/blog/ai-2027-transcendent-intelligence-breakthrough"
              className="inline-block bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300"
            >
              Explore Transcendence →
            </a>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-purple-500/50 transition-all duration-300">
            <div className="text-6xl mb-6">🤖</div>
            <h3 className="text-2xl font-bold text-white mb-4">Autonomous Enterprise</h3>
            <p className="text-gray-300 mb-6">
              Complete business autonomy with self-managing operations, autonomous decision-making, 
              and self-evolving business processes.
            </p>
            <a 
              href="/blog/ai-2027-autonomous-enterprise-revolution"
              className="inline-block bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-400 hover:to-pink-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300"
            >
              Discover Autonomy →
            </a>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-pink-500/50 transition-all duration-300">
            <div className="text-6xl mb-6">💰</div>
            <h3 className="text-2xl font-bold text-white mb-4">$5.7B Success Story</h3>
            <p className="text-gray-300 mb-6">
              See how a global enterprise achieved unprecedented transformation with transcendent intelligence, 
              generating $5.7 billion in measurable value.
            </p>
            <a 
              href="/case-studies/ai-2027-transcendent-intelligence-mega-success"
              className="inline-block bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-400 hover:to-rose-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300"
            >
              View Success Story →
            </a>
          </div>
        </div>

        <div className="text-center">
          <div className="inline-flex items-center gap-4 mb-6">
            <div className="bg-gradient-to-r from-indigo-500/20 to-purple-500/20 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/30">
              <div className="text-2xl font-extrabold text-indigo-400 mb-1">99.8%</div>
              <div className="text-xs text-indigo-300">Consciousness Performance</div>
            </div>
            <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 backdrop-blur-sm rounded-xl p-4 border border-purple-500/30">
              <div className="text-2xl font-extrabold text-purple-400 mb-1">∞</div>
              <div className="text-xs text-purple-300">Self-Evolution</div>
            </div>
            <div className="bg-gradient-to-r from-pink-500/20 to-rose-500/20 backdrop-blur-sm rounded-xl p-4 border border-pink-500/30">
              <div className="text-2xl font-extrabold text-pink-400 mb-1">$5.7B</div>
              <div className="text-xs text-pink-300">Value Generated</div>
            </div>
          </div>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a 
              href="/contact" 
              className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-indigo-500/50 transform hover:-translate-y-1"
            >
              Experience Transcendence
            </a>
            <a 
              href="/blog/ai-2027-transcendent-intelligence-breakthrough" 
              className="bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-400 hover:to-pink-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-purple-500/50 transform hover:-translate-y-1"
            >
              Read Breakthrough Article →
            </a>
            <a 
              href="/case-studies/ai-2027-transcendent-intelligence-mega-success" 
              className="border-2 border-indigo-500 text-indigo-400 hover:bg-indigo-500 hover:text-white font-bold py-4 px-8 rounded-xl transition-all duration-300"
            >
              View $5.7B Success
            </a>
          </div>
        </div>
      </div>
    </section>
  );
}