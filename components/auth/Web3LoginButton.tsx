...
=======
  ...
  return (<>;
      ...
      {open && <Web3LoginModal isOpen={open} onClose={() => setOpen(false)} onLoggedIn={onLoggedIn} />}
import dynamic from 'next / dynamic';const Web3LoginModal = dynamic (() => import ('./Web3LoginModal'), {ssr: false,})const Web3LoginModal = dynamic (() => import ('./Web3LoginModal'), { ssr: false }),async function resolveDisplayName (addr: string): Promise < string | null> {try {const r = await fetch (`/api / did / get?address=${encodeURIComponent (addr)}`)const { data } = await r.json ()const did = data?.payload || {}
    return did.lens || did.ens || null;
  } catch {return null;
  }
export default /**;
 ...
 */;
function Web3LoginButton() {const [open, set_open] = useState (false)const [user, set_user] = useState<{address: string;
    chain: 'evm' | 'sol';
  } | null>(null)const [display_name, setDisplayName] = useState < string | null>(null)const [displayWeb3, setDisplayWeb3]  = useState < boolean>(false)useEffect (() => {    return did.lens || did.ens || null;
  } catch { return null }
}
export default /**;
 * Web3LoginButton - Function description;
 */;
function Web3LoginButton() {const [open, set_open] = useState (false)const [user, set_user] = useState<{ address: string, chain: 'evm' | 'sol' } | null>(null),const [display_name, setDisplayName] = useState < string | null>(null)const [displayWeb3, setDisplayWeb3]  = useState < boolean>(false)useEffect (() => {const saved =;
      typeof window !== 'undefined';
    ...
    setDisplayWeb3 (pref === 'true')}, [])useEffect (() => {(async () => {    const saved = typeof window !== 'undefined' ? window.local_storage.get_item ('zion - web3 - user') : null;
    if (set_user (JSON.parse (saved))) {$2;
}
    const pref = typeof window !== 'undefined' ? window.local_storage.get_item ('zion - web3 - display') : null;
    setDisplayWeb3 (pref === 'true')useEffect (() => {(async () => {if (setDisplayName (await resolveDisplayName (user.address))) {$2;
}
      else setDisplayName (null)})()const onLoggedIn = (u: { address: string; chain: 'evm' | 'sol' }) =>: any {window.local_storage.set_item ('zion - web3 - user', JSON.stringify (u))set_user (u)}const disconnect = async () => {window.local_storage.remove_item ('zion - web3 - user')set_user (u)}const disconnect = async () => {window.local_storage.remove_item ('zion - web3 - user')try {await fetch ('/api / auth / logout', { method: 'POST' })} catch {}
    set_user (null)// Check condition;
if ( {) {$2;
}
    const base =;
