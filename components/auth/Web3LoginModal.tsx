...
function ModalInner({ isOpen, onClose, onLoggedIn }: Web3LoginModalProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="w-full max-w-md rounded-xl bg-white dark:bg-neutral-900 p-6 shadow-2xl"></div>
        ...
        <div className="space-y-3"></div>
          <button onClick={handleEvmConnect} disabled={loading} className="w-full rounded-lg bg-black text-white py-2.5 dark:bg-white dark:text-black"></button>
            {loading ? 'Connecting…' : 'Connect MetaMask / WalletConnect'}
          </button>
          <button onClick={handlePhantomConnect} disabled={loading} className="w-full rounded-lg bg-purple-600 text-white py-2.5"></button>
