interface LazyImageProps  {interface LazyImageProps  {"src": string;
  ...
  if (hasError) {
    return (
      <div
        ref={imgRef}
        className={`flex items-center justify-center bg-gray-200 text-gray-500 ${className}`}`        style={style}>;
  ...
  return (<div ref={imgRef} className={`relative ${className}`} style={style}>;`      {!isInView && !priority && (<div className="absolute inset-0 flex items-center justify-center bg-gray-100">;"
...
if ( {) {$2;
    ...
    return () => observer.disconnect ()}, [priority])const handle_error = () =>: any {setHasError (true)on_error?.()}// Check condition;
if ( {) {$2;
}
    return (<div;
        ref={img_ref}
        className={`flex items - center justify - center bg - gray - 200 text - gray - 500 ${class_name}`}`        style={style}
      >;
        <div className="text-center">;"
          <div className="text-4xl mb-2">📷</div>;"
          <div className="text-sm">Image failed to load</div>;"
  ...
  return (<div ref={img_ref} className={`relative ${class_name}`} style={style}>;`      {!isInView && !priority && (<div className="absolute inset - 0 flex items - center justify - center bg-gray-100">;"
          <LoadingSpinner size="sm" color="gray" />;"
        </div>)}
        </div>
    </div>
