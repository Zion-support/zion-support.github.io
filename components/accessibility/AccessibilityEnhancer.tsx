    ...
    if (keyboardNavigation) {
      const handleKeyboardNavigation = (e: KeyboardEvent) => {
        ...
        if (e.key === "ArrowDown" || e.key === "ArrowUp") {
          ...
          if (currentElement && currentElement.getAttribute("data-navigable") === "true") {
            e.preventDefault();
            const navigableElements = document.querySelectorAll("[data-navigable='true']");
            const currentIndex = Array.from(navigableElements).indexOf(currentElement);
            
            if (e.key === "ArrowDown" && currentIndex < navigableElements.length - 1) {
              (navigableElements[currentIndex + 1] as HTMLElement).focus();
