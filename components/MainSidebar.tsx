...
const navigation = {
    ...
    href: '/contact', icon: Phone },'    ], services: ['      { name: 'AI Solutions',;
    href: '/services/ai-solutions', icon: Brain, description: 'Autonomous AI systems' }, { name: 'Cloud & DevOps',;
    href: '/services/cloud', icon: Cloud, description: 'Infrastructure & automation' }, { name: 'Cybersecurity',;
    href: '/services/cybersecurity', icon: Shield, description: 'Zero-trust security' }, { name: 'Data & Analytics',;
    href: '/services/data', icon: Database, description: 'Data pipelines & ML ops' }, { name: 'Micro SaaS',;
    href: '/services/micro-saas', icon: Store, description: 'Rapid product development' }, { name: 'Digital Transformation',;
    href: '/services/transformation', icon: Rocket, description: 'Business modernization' }], solutions: ['      { name: 'Enterprise',;
    href: '/solutions/enterprise', icon: Building2, description: 'Large-scale implementations' }, { name: 'SMB',;
    href: '/solutions/smb', icon: Store, description: 'Small business focused' }, { name: 'Startup',;
    href: '/solutions/startup', icon: Rocket, description: 'Growth acceleration' }, { name: 'Government',;
    href: '/solutions/government', icon: Building, description: 'Public sector expertise' }, { name: 'Healthcare',;
    href: '/solutions/healthcare', icon: Heart, description: 'Health tech solutions' }, { name: 'Financial Services',;
    href: '/solutions/financial', icon: DollarSign, description: 'Fintech & compliance' }], resources: ['      { name: 'Documentation',;
    href: '/docs', icon: FileText, description: 'Technical guides' }, { name: 'API Reference',;
    ...
    href: '/pricing', icon: DollarSign, description: 'Service pricing' }], company: ['      { name: 'About Us',;
    ...
    href: '/careers', icon: Briefcase, description: 'Join our team' }, { name: 'News',;
    href: '/news', icon: Newspaper, description: 'Company updates' }, { name: 'Press',;
    href: '/press', icon: Quote, description: 'Media resources' }]'  };';
  const quickActions = [;
    { name: 'Request Quote',;
    href: '/request-quote', icon: Quote, color: 'from-cyan-500 to-blue-600' },'    { name: 'Get Support',;
    ...
    href: '/docs', icon: FileText, color: 'from-purple-500 to-pink-600' },'  ];';
  const contactInfo = [;
        ...
        <div className="mt-2 space-y-1>          {items.map((item) => ("            <Link;
              ...
              className={`flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition-all duration-200 ${;
                isActive(item.href);
                  ...
                  : 'text-gray-400 hover:text-white hover:bg-white/5}}'            >`              <item.icon className="w-4 h-4 flex-shrink-0 />              <div className="flex-1 min-w-0">"                <div className="font-medium>{item.name}</div>                {item.description && ("                  <div className="text-xs text-gray-500 truncate>{item.description}</div>                )}"              </div></Link>;
          ))}
        </div>;
      )}
    </div>;
...
<div className="space-y-1>            {navigation.main.map((item) => ("              <Link;
                ...
                className={`flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition-all duration-200 ${`;
                  isActive(item.href);
                    ...
                    ? 'bg-zion-cyan/20 text-zion-cyan border-l-2 border-zion-cyan;
                    : 'text-gray-300 hover:text-white hover:bg-white/5}}'              >                <item.icon className="w-4 h-4 flex-shrink-0 />                <span className="font-medium">{item.name}</span>"              </Link>"            ))}
          </div>;
