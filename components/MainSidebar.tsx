...
interface SidebarProps extends React.PropsWithChildren<{}> {"isOpen": boolean;
  ...
  interface NavItem  {"name": string;
    ...
    description?: string;
      ...
      }, {"name": 'SaaS Solutions',"icon": Cloud, "children": [;'
          { "name": 'Micro SaaS', "href": '/services/micro-saas' }  ];'
      }  ], "solutions": [;
      {"name": 'Automation Solutions',"icon": Zap, "children": [;'
      ...
      }, {"name": 'Business Solutions',"icon": Target, "children": [;'
  ...
  "onClose": ()               => void}const "MainSidebar": React.FC<SidebarProps> = ({ isOpen, onClose }) => {const router = useRouter()const [expandedSections, setExpandedSections] = useState<any>([])const toggleSection = ("section": anyanyanyanyanyanyanyanyanyanyanyanyanystring)              => {setExpandedSections(prev =>;
      }
      prev.includes(section)? prev.filter(s => { return s !== section): [...prev, section]; }
...
const renderNavSection = ("title": string, "items": NavItem[], "sectionKey": string) => (<div key={sectionKe
      ...
      {expandedSections.includes(sectionKey) && (<div className="mt-2 space-y-1">;"
          {items.map((item) => (<Link;
              ...
              href={item.href}
              onClick={onClose}
              className={`flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition-all duration-200 ${isActive(item.href)? 'bg-zion-cyan/20 text-zion-cyan border-l-2 border-zion-cyan';'                  : 'text-gray-400 "hover":text-white "hover":bg-white/5';'
              }`}`            >;
              }`}>`
...
className = 'fixed inset-0 bg-black/50 z-40 "lg": hidden''          onClick={onClose}' />;'
      ...
      <div className={`fixed top-0 left-0 h-full w-80 bg-gradient-to-b from-zion-blue-dark via-zion-blue to-zion-blue-dark border-r border-zion-blue-light z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full';'      } "lg":translate-x-0 "lg":relative "lg":z-auto`}>;`        {/* Header */}
className = 'fixed inset-0 bg-black/50 z-40 "lg": hidden''          onClick={onClos,'
          ...
          <div className="mb-6">;"
            ...
            <div className="grid grid-cols-2 gap-2">;"
              {quickActions.map((action) => (<Link;
                ...
                >;
                  <div className={`w-8 h-8 bg-gradient-to-r ${action.color} rounded-lg flex items-center justify-center group-"hover":scale-110 transition-transform duration-200`}>;`                  className="flex flex-col items-center gap-2 p-3 bg-white/5 rounded-lg "hover": bg-white/10 transition-all duration-200 group">"
        ...
        <div className="p-4 border-t border-zion-blue-light">;"
          ...
          <div className="flex items-center gap-3 mb-4">;"
            {socialLinks.map((social) => (<a;
                ...
                className={`p-2 "hover": bg-white/10 rounded-lg transition-all duration-200 ${social.color}`}`                aria-label={social.name}
  ...
  ': '-translate-x-full'      } "lg":translate-x-0 "lg":relative "lg":z-auto`}>'`        {/* Header */}`  ': '-translate-x-full'      } "lg": translate-x-0 "lg":relative "lg":z-auto`}>'`        {/* Header *,`}
...
<div className='space-y-1'>'            {navigation.main.map((item) => ('              <Link;'
                ...
                onClick={onClose}
                className={`flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition-all duration-200 ${`;`                  }
                  isActive(item.href)?, bg-zion-cyan/20 text-zion-cyan border-l-2 border-zion-cyan'': 'text-gray-300 "hover":text-white "hover":bg-white/5'                }`}`                  isActive(item.href)?, bg-zion-cyan/20 text-zion-cyan border-l-2 border-zion-cyan'': 'text-gray-300 "hover": text-white "hover":bg-white/5'                },'
}
...
, rel={contact.href.startsWith('http';'
  ...
  ': '}'                className='flex items-center gap-2 text-xs text-zion-slate-light "hover": text-zion-cyan transition-colors duration-200''              >'                <contact.icon className='w-3 h-3' />'                <span className='truncate'>{contact.text}</span>'              </a>'            ),'
          ...
          <div className='flex items-center gap-3 mb-4'>'            {socialLinks.map((social) => ('              <a;'
                ...
                key={social.name}
                href={social.href}
                target='_blank''                rel='noopener noreferrer''                className={`p-2 "hover": bg-white/10 rounded-lg transition-all duration-200 ${social.color}`}'                aria-label={social.name}`              >';'                target='_blank''                rel='noopener noreferrer''                className={`p-2 "hover": bg-white/10 rounded-lg transition-all duration-200 ${social.color}`}'                aria-label={social.nam,'}`              >';'
