    ...
    const generateSmartSuggestions = () => {
      ...
      const scoredItems = allItems.map(item => {
        ...
        if (
          ...
          item.tags.includes('automation')
        )
          score += 0.2;
        if (currentPath.includes('reports') && item.tags.includes('reports'))
          score += 0.2;
        if (
          currentPath.includes('components') &&
          item.tags.includes('components')
        )
          score += 0.2;
                  ...
                  <div className='flex items-start gap-3'>
                    ...
                    <div className='flex-1 min-w-0'>
                      ...
                      <div className='flex items-center justify-between'>
                        <span
                          className={`text-xs px-2 py-1 rounded ${getCategoryColor(item.category)}`}
                        >
                          {item.category}
                        </span>
