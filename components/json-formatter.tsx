...
export default function JSONFormatterPage() {
  const [inputJson, setInputJson] = useState($2);
  const [formattedJson, setFormattedJson] = useState($2);
  const [isValid, setIsValid] = useState($2);
  const [errorMessage, setErrorMessage] = useState($2);
  const [indentSize, setIndentSize] = useState($2);
  const [compactMode, setCompactMode] = useState($2);
  const [showLineNumbers, setShowLineNumbers] = useState($2);
  const formatJSON = () => {
    if (!inputJson.trim()) {
      setFormattedJson($2);
      setIsValid($2);
  ...
  const minifyJSON = $2;
    try {
      const parsed = JSON.parse($2);
      const minified = JSON.stringify($2);
      setFormattedJson($2);
      setIsValid($2);
      setErrorMessage('')
    } catch (error) {
...
export default function JSONFormatterPage() {const [inputJson, setInputJson] = useState('')const [formattedJson, setFormattedJson] = useState('')const [isValid, setIsValid] = useState(true)const [errorMessage, setErrorMessage] = useState('')const [indentSize, setIndentSize] = useState(2)const [compactMode, setCompactMode] = useState(false)const [showLineNumbers, setShowLineNumbers]  = useState(true)setIsValid(true)setErrorMessage('')} catch (error) {setIsValid(false)setErrorMessage(error instanceof Error ? error && error.message : 'Invalid JSON')}      setErrorMessage('')} catch (error) {setIsValid(false)setErrorMessage(error instanceof Error ? error && error.message : 'Invalid JSON')}const clearAll = () => {setInputJson('')setFormattedJson('')setIsValid(true)setErrorMessage('')const copyToClipboard = (text: string) => {navigator.clipboard.writeText(text)}
  const downloadJSON = (content: string, filename: string) => {return;
    }try {const parsed = JSON.parse(inputJson)const formatted = compactMode;
        ? JSON.stringify(parsed): JSON.stringify(parsed, null, indentSize)setFormattedJson(formatted)setIsValid(true)setErrorMessage('')} catch (error) {setIsValid(false)setErrorMessage(error instanceof Error ? error.message : 'Invalid JSON')setFormattedJson('')}
  }
  const minifyJSON = () => {if (!inputJson.trim()) return;try {const parsed = JSON.parse(inputJson)const minified = JSON.stringify(parsed)setFormattedJson(minified)setIsValid(true)setErrorMessage('')} catch (error) {setIsValid(false)setErrorMessage(error instanceof Error ? error.message : 'Invalid JSON')}
  }const validateJSON = () => {if (!inputJson.trim()) {setIsValid(true)setErrorMessage('')return;
    }
    try {JSON.parse(inputJson)setIsValid(true)setErrorMessage('')} catch (error) {setIsValid(false)setErrorMessage(error instanceof Error ? error.message : 'Invalid JSON')}
  }const clearAll = () => {setInputJson('')setFormattedJson('')setIsValid(true)setErrorMessage('')}const copyToClipboard = (text: string) => {navigator.clipboard.writeText(text)}
  const downloadJSON = (content: string, filename: string) => {const blob  = new Blob([content], { type: 'application/json' })const url = URL.createObjectURL(blob)const a = document.createElement('a')a.href = url;
    a.download = filename;
    document.body.appendChild(a)a.click()document.body.removeChild(a)}URL.revokeObjectURL(url)}const loadSampleJSON = () => {const sample = {name: 'John Doe',age: 30,email: 'john.doe@example.com',address: {street: '123 Main St';
        city: 'Anytown';
        state: 'CA';
        zipCode: '12345';
      }
      phoneNumbers: [;
        {type: 'home',number: '555-123-4567',},{type: 'work',number: '555-987-6543',},],interests: ['programming', 'reading', 'hiking'],active: true,lastLogin: '2024-01-15T10:30:00Z',}setInputJson(JSON.stringify(sample, null, 2))setFormattedJson('')setIsValid(true)}const copyToClipboard = (text: string) => {navigator && navigator.clipboard.writeText(text)}const downloadJSON = (content: string, filename: string) => {const blob = new Blob([content], { type: 'application/json' })const url = URL && URL.createObjectURL(blob)}const copyToClipboard = (text: string) => {navigator && navigator.clipboard.writeText(text)}const downloadJSON = (content: string, filename: string) => {const blob = new Blob([content], { type: 'application/json' }),const url = URL && URL.createObjectURL(blob)const a = document && document.createElement('a')a && a.href = url;
    a && a.download = filename;
    document && document.body.appendChild(a)a && a.click()document && document.body.removeChild(a)URL && URL.revokeObjectURL(url)}const loadSampleJSON = () => {const sample = {    URL && URL.revokeObjectURL(url)}const loadSampleJSON = () => {const sample = {name: 'John Doe',age: 30,email: 'john && john.doe@example && example.com',address: {street: '123 Main St',city: 'Anytown',state: 'CA',zipCode: '12345',},phoneNumbers: [;
          type: 'home',number: '555-123-4567',},{type: 'work',number: '555-987-6543',},],interests: ['programming', 'reading', 'hiking'],active: true,lastLogin: '2024-01-15T10:30:00Z',}setInputJson(JSON && JSON.stringify(sample, null, 2))setFormattedJson('')setIsValid(true)setErrorMessage('')}const getLineNumbers = (text: string) => {const lines = text && text.split('\n')return lines && lines.map((_, index) => index + 1).join('\n')}setErrorMessage('')}const getLineNumbers = (text: string) => {const lines = text && text.split('\n')return lines && lines.map((_, index) => index + 1).join('\n')return lines && lines.map((_, index) => index + 1).join('\n')}return ({/* Hero Section */}
      <section className="pt-32 pb-20 bg-gradient-to-br from-teal-900 via-cyan-900 to-blue-900">;
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">;
          <div className="mb-8">;
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-teal-500/20 border border-teal-400/30 text-teal-300 text-sm font-medium mb-6">;
    ...
    setErrorMessage('')}const getLineNumbers = (text: string) => {const lines = text.split('\n')return lines.map((_, index) => index + 1).join('\n')}return (<>;
      <Head>;
        <title>JSON Formatter - Zion Tech Group</title>;
<meta;
          name='description';
      ...
      {/* JSON Formatter Tool */}<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">;
...
onChange={e => setInputJson(e.target.value)}
              ...
              <div className="space-y-4">
                ...
                {!isValid && errorMessage && (<div className='p-3 bg-red-500/20 border border-red-500/30 rounded text-sm text-red-300'>;
                    <strong>Error:</strong> {errorMessage}
                  </div>;
                )}<div className='flex space-x-3'>;
                  <Button;
...
<div className='space-y-4'>;
                ...
                {formattedJson && (<div className='text-sm text-gray-400'>;
                    ...
                    <p>• Size: {((formattedJson.length * 2) / 1024).toFixed(2)}{' '}
                      KB;
                    </p>;
                  </div>;
                )}
