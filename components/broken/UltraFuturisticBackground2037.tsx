...
export default function UltraFuturisticBackground2037({
  ...
  useEffect(() => {
    ...
    const drawQuantumNeonParticle = (particle: any) => {
      const { x, y, size, color, quantumState, neonIntensity, rotation } =
        particle;
      ...
      gradient.addColorStop(
        0,
        `${color}${Math.floor(neonIntensity * 255)
          .toString(16)
          .padStart(2, '0')}`
      );
      gradient.addColorStop(
        0.5,
        `${color}${Math.floor(neonIntensity * 128)
          .toString(16)
          .padStart(2, '0')}`
      );
      ...
      for (let i = 0; i < 3; i++) {
        ...
        ctx.fillStyle = `${color}${Math.floor(neonIntensity * 200)
          .toString(16)
          .padStart(2, '0')}`;
        ctx.fill();
      }
    ...
    const drawHologramParticle = (particle: any) => {
      ...
      ctx.strokeStyle = `${color}${Math.floor(hologramOpacity * 255)
        .toString(16)
        .padStart(2, '0')}`;
      ctx.lineWidth = 1;
      ...
      // Holographic center
      ctx.fillStyle = `${color}${Math.floor(hologramOpacity * 100)
        .toString(16)
        .padStart(2, '0')}`;
      ctx.beginPath();
    ...
    const drawWaveParticle = (particle: any) => {
      const { x, y, size, color, waveFrequency, waveAmplitude, opacity } =
        particle;
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const waveY =
          y + Math.sin(i * waveFrequency + Date.now() * 0.01) * waveAmplitude;
    ...
    const drawQuantumParticle = (particle: any) => {
      ...
      ctx.lineTo(
        Math.cos(spinAngle + Math.PI) * size,
        Math.sin(spinAngle + Math.PI) * size
      );
      ctx.stroke();
