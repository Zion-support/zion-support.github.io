...
const offset = (time * 20) % gridSize;
<<<<<<< HEAD
      for (let x = offset; x < canvas.width; x += gridSize) {ctx.beginPath()ctx.moveTo(x,,,
  0)ctx.lineTo(x,,,
  canvas.height)ctx.stroke()}
      for (let y = offset; y < canvas.height; y += gridSize) {ctx.beginPath()ctx.moveTo(0,,,
  y)ctx.lineTo(canvas.width,,,
  y)ctx.stroke()}// Draw floating geometric shapes;
=======
>>>>>>> origin/chore/fix-lint-and-merge
...
>>>>>>> origin/chore/fix-lint-and-merge
          ...
          ctx && ctx.beginPath();
<<<<<<< HEAD
          ctx && ctx.arc(0, 0, shape && shape.size / 2, 0,,,
  Math && Math.PI * 2);}
=======
<<<<<<< HEAD
...
<<<<<<< HEAD
          ctx && ctx.moveTo(0,,,
  -shape && shape.size / 2);
          ctx && ctx.lineTo(shape && shape.size / 2,,,
  shape && shape.size / 2);
          ctx && ctx.lineTo(-shape && shape.size / 2,,,
  shape && shape.size / 2);
=======
...
origin/cursor/automate-test-improve-and-merge-code-2533
        ...
        ctx.strokeStyle = `rgba(236, 72, 153, ${0.1 + Math.sin(time + i) * 0.05})`;`        ctx.lineWidth = 3;
        ctx.beginPath();
        {x: canvas.width * 0.6,y: canvas.height * 0.2,size: 50,rotation: time * 0.7,},];
...
<<<<<<< HEAD
  ...
  handleResize)return () => {window.removeEventListener('resize',,,
  handleResize)cancelAnimationFrame(animationFrameId)}}, [])return (
...
<<<<<<< HEAD
          animate={{y: [0, -25, 0],opacity: [0.4, 0.9}
            0.4],
