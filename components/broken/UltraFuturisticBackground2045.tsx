...
<<<<<<< HEAD
  ...
  x: Math.random() * canvas.width,
        ...
        vx: (Math.random() - 0.5) * 2 vy: (Math.random() - 0.5) * 2 size: Math.random() * 3 + 1,}
=======
>>>>>>> origin/chore/fix-lint-and-merge
      particles.push({
        x: Math.random() * canvas.width,
...
>>>>>>> origin/chore/fix-lint-and-merge
  ...
  Math.PI * 2)ctx.fill()// Add glow effect;
...
if (particle.life <= 0) {
          particles[index] = {
            ...
            y: Math.random() * canvas.height,
            vx: (Math.random() - 0.5) * 2,
            vy: (Math.random() - 0.5) * 2 size: Math.random() * 3 + 1 color: `hsl(${Math.random() * 36,) => {
  return $3;}
}
}, 70%, 60%)`,
            alpha: Math.random() * 0.8 + 0.2,
...
className='absolute top-1/4 left-1/3 w-8 h-8 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full shadow-lg shadow-cyan-400/50';
<<<<<<< HEAD
          animate={{scale: [1, 1.5, 1],opacity: [0.5, 1, 0.5],y: [0, -10}
            0],
