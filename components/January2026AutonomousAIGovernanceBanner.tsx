import React from 'react';

export default function January2026AutonomousAIGovernanceBanner() {
  return (
    <section className="py-16 bg-gradient-to-r from-emerald-900/40 via-teal-900/40 to-cyan-900/40 backdrop-blur-lg border-b border-emerald-500/30">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-emerald-500/30 to-teal-500/30 border border-emerald-500/40 mb-6 animate-pulse">
            <span className="text-emerald-300 font-bold text-xl tracking-wider uppercase">
              🧠 BREAKING: JANUARY 2026 AUTONOMOUS AI GOVERNANCE REVOLUTION
            </span>
          </div>
          
          <h2 className="text-4xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 bg-clip-text text-transparent leading-tight">
            Self-Regulating AI Systems
          </h2>
          
          <p className="text-xl md:text-2xl text-gray-200 max-w-4xl mx-auto mb-8 font-semibold">
            The Future of AI Governance: Autonomous Systems That Monitor, Regulate, and Adapt 
            Their Own Behavior to Maintain Ethical Standards
          </p>
          
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12 max-w-4xl mx-auto">
            <div className="bg-gradient-to-r from-emerald-500/20 to-emerald-600/20 backdrop-blur-sm rounded-xl p-4 border border-emerald-500/30">
              <div className="text-2xl font-extrabold text-emerald-400 mb-1">94%</div>
              <div className="text-xs text-emerald-300">Bias Reduction</div>
            </div>
            <div className="bg-gradient-to-r from-teal-500/20 to-teal-600/20 backdrop-blur-sm rounded-xl p-4 border border-teal-500/30">
              <div className="text-2xl font-extrabold text-teal-400 mb-1">100%</div>
              <div className="text-xs text-teal-300">Compliance</div>
            </div>
            <div className="bg-gradient-to-r from-cyan-500/20 to-cyan-600/20 backdrop-blur-sm rounded-xl p-4 border border-cyan-500/30">
              <div className="text-2xl font-extrabold text-cyan-400 mb-1">67%</div>
              <div className="text-xs text-cyan-300">Faster Processes</div>
            </div>
            <div className="bg-gradient-to-r from-blue-500/20 to-blue-600/20 backdrop-blur-sm rounded-xl p-4 border border-blue-500/30">
              <div className="text-2xl font-extrabold text-blue-400 mb-1">$12.8M</div>
              <div className="text-xs text-blue-300">Annual Savings</div>
            </div>
          </div>
        </div>
        
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-emerald-500/50 transition-all duration-300 group">
            <div className="text-6xl mb-6 group-hover:scale-110 transition-transform duration-300">⚖️</div>
            <h3 className="text-2xl font-bold text-white mb-4">Ethical Decision Engine</h3>
            <p className="text-gray-300 mb-6">
              Advanced moral reasoning framework that integrates utilitarian, deontological, and virtue ethics 
              for context-aware ethical decision making.
            </p>
            <ul className="text-sm text-gray-400 space-y-2 mb-6">
              <li>• Stakeholder impact assessment</li>
              <li>• Long-term consequence evaluation</li>
              <li>• Cultural and contextual adaptation</li>
              <li>• Consensus building with human values</li>
            </ul>
            <a 
              href="/blog/ai-2026-january-autonomous-ai-governance-revolution"
              className="inline-block bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-emerald-500/50"
            >
              Read Full Article →
            </a>
          </div>
          
          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-teal-500/50 transition-all duration-300 group">
            <div className="text-6xl mb-6 group-hover:scale-110 transition-transform duration-300">🔍</div>
            <h3 className="text-2xl font-bold text-white mb-4">Real-Time Compliance</h3>
            <p className="text-gray-300 mb-6">
              Continuous monitoring and automated compliance verification across jurisdictions 
              with predictive risk assessment capabilities.
            </p>
            <ul className="text-sm text-gray-400 space-y-2 mb-6">
              <li>• Cross-jurisdictional compliance</li>
              <li>• Industry-specific requirements</li>
              <li>• Predictive risk modeling</li>
              <li>• Automated policy updates</li>
            </ul>
            <a 
              href="/services/autonomous-ai-governance"
              className="inline-block bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-400 hover:to-cyan-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-teal-500/50"
            >
              Explore Services →
            </a>
          </div>
          
          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-cyan-500/50 transition-all duration-300 group">
            <div className="text-6xl mb-6 group-hover:scale-110 transition-transform duration-300">🔄</div>
            <h3 className="text-2xl font-bold text-white mb-4">Self-Correction</h3>
            <p className="text-gray-300 mb-6">
              Autonomous behavioral modification and algorithmic bias correction with 
              continuous learning and adaptation capabilities.
            </p>
            <ul className="text-sm text-gray-400 space-y-2 mb-6">
              <li>• Automated parameter adjustment</li>
              <li>• Performance optimization</li>
              <li>• Safety constraint enforcement</li>
              <li>• Knowledge transfer between systems</li>
            </ul>
            <a 
              href="/case-studies/fortune-500-autonomous-governance-success"
              className="inline-block bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-cyan-500/50"
            >
              View Case Study →
            </a>
          </div>
        </div>
        
        <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 backdrop-blur-lg rounded-2xl p-8 border border-emerald-500/20 mb-12">
          <div className="text-center">
            <h3 className="text-3xl font-bold text-white mb-4">
              🏆 Fortune 500 Success Story: Autonomous Governance in Action
            </h3>
            <p className="text-lg text-gray-200 mb-6 max-w-3xl mx-auto">
              A multinational corporation implemented autonomous AI governance for their recruitment system, 
              ensuring fairness across diverse global markets while maintaining compliance with varying local regulations.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
              <div className="bg-emerald-500/20 rounded-lg p-4">
                <div className="text-xl font-bold text-emerald-400">94%</div>
                <div className="text-sm text-emerald-300">Bias Reduction</div>
              </div>
              <div className="bg-teal-500/20 rounded-lg p-4">
                <div className="text-xl font-bold text-teal-400">100%</div>
                <div className="text-sm text-teal-300">Compliance</div>
              </div>
              <div className="bg-cyan-500/20 rounded-lg p-4">
                <div className="text-xl font-bold text-cyan-400">67%</div>
                <div className="text-sm text-cyan-300">Faster Hiring</div>
              </div>
              <div className="bg-blue-500/20 rounded-lg p-4">
                <div className="text-xl font-bold text-blue-400">$12.8M</div>
                <div className="text-sm text-blue-300">Annual Savings</div>
              </div>
            </div>
          </div>
        </div>
        
        <div className="text-center">
          <div className="inline-flex items-center gap-4 px-8 py-4 rounded-full bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border border-emerald-500/30 mb-6">
            <span className="text-emerald-300 font-bold text-lg">
              🚀 Ready to Implement Autonomous AI Governance?
            </span>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a 
              href="/contact"
              className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-emerald-500/50 transform hover:-translate-y-1"
            >
              Start Governance Journey
            </a>
            <a 
              href="/blog/ai-2026-january-autonomous-ai-governance-revolution"
              className="bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-400 hover:to-cyan-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-teal-500/50 transform hover:-translate-y-1"
            >
              Read Full Article →
            </a>
            <a 
              href="/services/ai-governance-consulting"
              className="border-2 border-emerald-500 text-emerald-400 hover:bg-emerald-500 hover:text-white font-bold py-4 px-8 rounded-xl transition-all duration-300"
            >
              Get Consultation
            </a>
          </div>
        </div>
      </div>
    </section>
  );
}