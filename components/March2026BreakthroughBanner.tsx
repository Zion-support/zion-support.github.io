// import React from 'react';

const March2026BreakthroughBanner: React.FC = () => {
  return (
    <section className="py-12 bg-gradient-to-r from-purple-900/60 via-blue-900/60 to-indigo-900/60 border-b border-purple-500/40">
      <div className="container mx-auto px-6">
        <div className="text-center">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-purple-500/30 to-blue-500/30 border border-purple-500/40 mb-6">
            <span className="text-purple-300 font-bold text-xl tracking-wider uppercase">
              🚀 March 2026 AI Breakthrough Revolution Now Live!
            </span>
          </div>
          
          <h2 className="text-4xl md:text-5xl font-extrabold mb-6 bg-gradient-to-r from-purple-400 via-blue-400 to-cyan-400 bg-clip-text text-transparent">
            The Next Frontier of Enterprise Intelligence
          </h2>
          
          <p className="text-xl text-gray-200 mb-8 max-w-4xl mx-auto">
            Discover revolutionary AI breakthroughs delivering <span className="text-purple-400 font-bold">1000x processing speed</span>, 
            <span className="text-blue-400 font-bold"> 99.7% decision accuracy</span>, and 
            <span className="text-cyan-400 font-bold"> $5+ billion value generation</span>
          </p>
          
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-gradient-to-r from-purple-500/20 to-purple-600/20 backdrop-blur-sm rounded-xl p-4 border border-purple-500/30">
              <div className="text-2xl font-extrabold text-purple-400 mb-1">1000x</div>
              <div className="text-xs text-purple-300">Processing Speed</div>
            </div>
            <div className="bg-gradient-to-r from-blue-500/20 to-blue-600/20 backdrop-blur-sm rounded-xl p-4 border border-blue-500/30">
              <div className="text-2xl font-extrabold text-blue-400 mb-1">99.7%</div>
              <div className="text-xs text-blue-300">Decision Accuracy</div>
            </div>
            <div className="bg-gradient-to-r from-cyan-500/20 to-cyan-600/20 backdrop-blur-sm rounded-xl p-4 border border-cyan-500/30">
              <div className="text-2xl font-extrabold text-cyan-400 mb-1">$5B+</div>
              <div className="text-xs text-cyan-300">Value Generated</div>
            </div>
            <div className="bg-gradient-to-r from-green-500/20 to-green-600/20 backdrop-blur-sm rounded-xl p-4 border border-green-500/30">
              <div className="text-2xl font-extrabold text-green-400 mb-1">500%</div>
              <div className="text-xs text-green-300">Average ROI</div>
            </div>
          </div>
          
          <div className="flex flex-wrap justify-center gap-4">
            <a
              href="/blog/ai-2026-march-breakthrough-revolution"
              className="bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-400 hover:to-blue-500 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-purple-500/50 transform hover:-translate-y-1"
            >
              Read Full Breakthrough Guide →
            </a>
            <a
              href="/case-studies/ai-2026-march-breakthrough-5-billion-success"
              className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-cyan-500/50 transform hover:-translate-y-1"
            >
              View $5B Success Story →
            </a>
            <a
              href="/services/ai-2026-march-breakthrough-consulting"
              className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-green-500/50 transform hover:-translate-y-1"
            >
              Get Free Consultation →
            </a>
          </div>
        </div>
      </div>
    </section>
  );
};

export default March2026BreakthroughBanner;