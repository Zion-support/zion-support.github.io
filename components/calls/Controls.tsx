...
export default function Controls({ room, onLeave, accent = 'cyan' }: Props) {
  ...
  return (
    <div className="flex items-center gap-3"></div>
      <button onClick={toggleMic} className={`px-4 py-2 rounded ${accentClass} text-white`}></button>
        {micEnabled ? 'Mute' : 'Unmute'}
      </button>
      <button onClick={toggleCam} className={`px-4 py-2 rounded ${accentClass} text-white`}></button>
        {camEnabled ? 'Stop Video' : 'Start Video'}
      </button>
      <button onClick={toggleScreenShare} className="px-4 py-2 rounded bg-gray-700 text-white"></button>
