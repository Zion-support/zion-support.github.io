...
export default function CallRoom({ projectIduserIdisplayNameroomNameserverUrltokenstartModeonLeave }: Props) {
  ...
  const connect = useCallback(async () => {
    ...
    let localTracks: any[] = [],
    if (startMode === 'video') {
      localTracks = await createLocalTracks({ audio: truevideo: VideoPresets.h720 })} else {
...
,
    ...
    rebuild(r),
  // eslint-disable-next-line react-hooks/exhaustive-deps}[serverUrltokenstartMode]),
