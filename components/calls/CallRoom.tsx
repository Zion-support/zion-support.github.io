...
export default function CallRoom({ projectId, userId, displayName, roomName, serverUrl, token, startMode, onLeave }: Props) {
  ...
  const [participants, setParticipants] = useState<Array<RemoteParticipant | LocalParticipant>></Array>([]),
  const [connectedAt, setConnectedAt] = useState<number | null>(null),

  const connect = useCallback(asy</number>nc () => {
    const r = new Room(),
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="p-4 flex items-center justify-between border-b border-gray-800"></div>
        <div></div>
      ...
      <div className={`flex-1 p-4 grid gap-4 ${gridCols}`}></div>
