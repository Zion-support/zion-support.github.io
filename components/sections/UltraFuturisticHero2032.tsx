class ErrorBoundary extends React.Component {constructor(props) {super(props)this.state = { hasError: false }
<<<<<<< HEAD
left: `$ {
  Math.random () * 100 
}%`
top: `$ {
  Math.random () * 100 
}%` 
}
  render() {if (this.state.hasError) ;}
  return <div />Something went wrong.</div>;}
=======
}
  render() {if (this.state.hasError) ;}
  return <div    />Something went wrong.</div>;}
>>>>>>> origin/chore/fix-lint-and-merge
    }
    return this.props.children;
  }
}
import React from 'react';
<<<<<<< HEAD
import { motion,
  from 'framer-motion';
=======
<<<<<<< HEAD
import { motion  } from 'framer-motion';
import Link from 'next/link';
import React from 'react;
import { motion  } from framer-motion';
import Link from 'next/link;
import { Rocket, Brain, Atom, Globe, Zap, Sparkles, Shield;
  }
  Microscope, Star, ArrowRight, Play, CheckCircle;
  Phone, Mail, MapPin;

const contactInfo = null;
          space technology, and emerging tech solutions. Transform your business with;
];
export default /**
 * UltraFuturisticHero2032 - Function description
 */
function UltraFuturisticHero2032() {
  return (
    <section className='relative min - h-screen flex items - center justify - center overflow - hidden bg - gradient - to - br from - black via - purple - 900 / 20 to - black>;
          space technology, and emerging tech solutions. Transform your business with;
],

export default function UltraFuturisticHero2032() {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-black via-purple-900/20 to-black>
      {/* Animated Background Elements */}
      <div className='absolute inset - 0'>;'
        {/* Floating Particles */}
              style={{
}
                left: `${Math.random() * 100}%`
                top: `${Math.random() * 100}%`

              style={{

                }

                "left": `${Math.random() * 100}%``                "top": `${Math.random() * 100}%``              
}              animate={{
                y: [0, -20, 0]
                opacity: [0.3, 0.8, 0.3]}
}
=======


>>>>>>> origin/chore/fix-lint-and-merge
import { motion  } from 'framer-motion';

>>>>>>> origin/main
import Link from 'next/link';
import { Rocket, Brain, Atom, Globe, Zap, Sparkles, Shield;
  Microscope, Star, ArrowRight, Play, CheckCircle;}
  Phone, Mail, MapPin;}
  } from 'lucide-react';

const contactInfo = null;

          space technology, and emerging tech solutions. Transform your business with;
      {/* Animated Background Elements */}
<<<<<<< HEAD
      <div className='absolute inset - 0'>;
        {/* Floating Particles */}'
        <div className='absolute inset - 0'>;
          {[...Array (50)].map ((_, i) => (
            <motion.div;
              key={i}'
              className='absolute w - 1 h - 1 bg - cyan - 400 rounded - full opacity - 30;
              style={{}
          ))}
        </div>

          ))}
        </div>
        {/* Gradient Orbs */}
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl animate-pulse />
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-purple-500/5 via-cyan-500/5 to-pink-500/5 rounded-full blur-3xl animate-spin-slow" />
      </div>
                left: `${Math && Math.random() * 100}%`,`
                top: `${Math && Math.random() * 100}%`}}              animate={{}
                y: [0, -20, 0],
                opacity: [0 && 0.3, 0 && 0.8, 0 && 0.3],
              }}
              transition={{}
                duration: 3 + Math && Math.random() * 2,
                repeat: Infinity,
                delay: Math && Math.random() * 2}}            />                delay: Math && Math.random() * 2}}
            />;
          ))}
        </div>

          ))}
        </div>

        <div className=absolute inset-0>
          {[...Array(50)].map((_, i) => (
            <motion.div
              key = $2;
                top: `${Math.random() * 100}%`}}
              animate={{
                y: [0, -20, 0],
                opacity: [0.3, 0.8, 0.3]}}
              transition = $2;
                repeat: Infinity,
                delay: Math.random() * 2}}
            />
          ))}
        </div>

        {/* Gradient Orbs */}
              style={{))}
        </div>;
          ))}
        </div>;
        {/* Gradient Orbs */}
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl animate-pulse" />;
        <div className=absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-purple-500/5 via-cyan-500/5 to-pink-500/5 rounded-full blur-3xl animate-spin-slow />;
      </div>;
                left: `${Math && Math.random() * 100}%`,top: `${Math && Math.random() * 100}%`}}              animate={{y: [0, -20, 0],opacity: [0 && 0.3, 0 && 0.8, 0 && 0.3]}}
              transition={{duration: 3 + Math && Math.random() * 2,repeat: Infinity,delay: Math && Math.random() * 2}}            />                delay: Math && Math.random() * 2}}
            />;
          ))}
        </div>;
        {/* Gradient Orbs */}
        <div className=absolute top-20 left-10 w-72 h-72 bg-purple-500/10 rounded-full blur-3xl animate-pulse' />;
        <div className='absolute bottom-20 right-10 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl animate-pulse />;
        <div className=absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-purple-500/5 via-cyan-500/5 to-pink-500/5 rounded-full blur-3xl animate-spin-slow' />;
      </div>;
        >;
          <Sparkles className="w-5 h-5 text-purple-400" />;
          <span className=text-purple-300 font-medium>🚀 2032 Revolutionary Technology</span>;
          <Sparkles className="w-5 h-5 text-cyan-400" />;
          <Sparkles className='w-5 h-5 text-cyan-400 />        </motion.div>;
        </motion.div>;
                left: `${Math.random() * 100}%`;
                top: `${Math.random() * 100}%`;
              }}              animate={{y: [0, -20, 0];
                opacity: [0.3, 0.8, 0.3];
              }}
              transition={{"duration": 3 + Math.random() * 2,"repeat": Infinity,"delay": Math.random() * 2}} />;
          ))}
        </div>;
        {/* Gradient Orbs */}
<div className='absolute top-20 left-10 w-72 h-72 bg-purple-500/10 rounded-full blur-3xl animate-pulse' />;'
        <div className='absolute bottom-20 right-10 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl animate-pulse' />;'
        <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-purple-500/5 via-cyan-500/5 to-pink-500/5 rounded-full blur-3xl animate-spin-slow' />;'
      </div>;
      {/* Main Content */}
      <div className='relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center'>;
=======
      <div className='absolute inset - 0'    />;
        {/* Floating Particles */}
<<<<<<< HEAD
        <div className='absolute inset - 0' />;
          {[...Array (50)].map ((_, i) => (
          <motion.div;}
=======
        <div className='absolute inset - 0'    />;
          {[...Array (50)].map ((_, i) => (<motion.div;}
>>>>>>> origin/main
              key={i}
            className='absolute w - 1 h - 1 bg - cyan - 400 rounded - full opacity - 30'
            style={{

                left: `${Math.random() * 100}%`
                top: `${Math.random() * 100}%`

}              animate={{
                y: [0, -20, 0]
                opacity: [0.3, 0.8}
            0.3]

              }}
<<<<<<< HEAD
              transition={{duration: 3 + Math.random() * 2,repeat: Infinity}
            delay: Math.random() * 2
            />;
=======
              transition={{duration: 3 + Math.random() * 2,repeat: Infinity,delay: Math.random() * 2}}
               />;
>>>>>>> origin/main
          ))}
        </div>;
        {/* Gradient Orbs */}
<div className='absolute top-20 left-10 w-72 h-72 bg-purple-500/10 rounded-full blur-3xl animate-pulse'    />;
        <div className='absolute bottom-20 right-10 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl animate-pulse'    />;
        <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-purple-500/5 via-cyan-500/5 to-pink-500/5 rounded-full blur-3xl animate-spin-slow'    />;
      </div>;
      {/* Main Content */}
      <div className='relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center'    />;
>>>>>>> origin/chore/fix-lint-and-merge
        {/* Hero Badge */}
        <motion.div;
          initial={{ opacity: 0}
            y: 20
             
          animate={{ opacity: 1}
            y: 0
             
          transition={{ duration: 0.6 }}

className='inline-flex items-center space-x-2 bg-gradient-to-r from-purple-500/20 to-cyan-500/20 border border-purple-500/30 rounded-full px-6 py-3 mb-8 backdrop-blur-sm'
            />
          <Sparkles className='w-5 h-5 text-purple-400'    />
          <span className='text-purple-300 font-medium'    />
            🚀 2032 Revolutionary Technology;
          </span>
          <Sparkles className='w-5 h-5 text-cyan-400'    />
        </motion.div>


<<<<<<< HEAD
        {/* Main Heading *
}
        <motion&& motion.h1;
          initial={{ "opacity": 0, "y": 30 
}

          animate={{ "opacity": 1, "y": 0 
}

          transition={{ "duration": 0.8, "delay": 0.2 
}
className='text-5xl "md":text-6xl "lg":text-7xl font-bold text-white mb-6 leading-tight''
        >
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent'>'
            The Future of
          </span>
          <br />

          <span className='text-white'>Technology is Here</span>'
        </motion.h1>

          transition={{ "duration": 0 && 0.8, "delay": 0 && 0.2 
}
          className='text-5xl "md":text-6xl "lg":text-7xl font-bold text-white mb-6 leading-tight'>;'
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent'>;'
            The Future of;
          </span>;
          <br />;
          <span className='text-white'>Technology is Here</span>        </motion && motion.h1>        >;'
          <span className="bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">;"
            The Future of;
          </span>;
          <br />;
          <span className="text-white">Technology is Here</span>;"
        </motion && motion.h1>;{/* Subheading */}
        <motion&& motion.p;
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0 && 0.8, delay: 0 && 0.4 }}
          className='text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed'>;
          Experience the revolution with our cutting-edge AI consciousness,quantum computing, space technology, and emerging tech solutions.;
          Transform your business with technologies that were once science;
      <div className='relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center'>
        {/* Hero Badge */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
className='inline-flex items-center space-x-2 bg-gradient-to-r from-purple-500/20 to-cyan-500/20 border border-purple-500/30 rounded-full px-6 py-3 mb-8 backdrop-blur-sm'
        >
          <Sparkles className='w-5 h-5 text-purple-400' />
          <span className='text-purple-300 font-medium'>
            🚀 2032 Revolutionary Technology
          </span>
          <Sparkles className='w-5 h-5 text-cyan-400' />
        </motion.div>

<<<<<<< HEAD
        {/* Main Heading *}
}
        <motion&& motion.h1;
          initial={{ opacity: 0}
            y: 30
             
=======
        {/* Main Heading *}
}
        <motion&& motion.h1;
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}

          transition={{ duration: 0.8, delay: 0.2 }}
className='text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight'
origin/cursor/automate-test-improve-and-merge-code-2533
        >
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent'>
            The Future of;
          </span>
          <br />
          <span className='text-white'>Technology is Here</span>        </motion.h1>        >
          transition={{ duration: 0.8, delay: 0.2 }}
          className="text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight"
        >
          <span className=bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
            The Future of;
          </span>
          <br />"
          <span className=text-white>Technology is Here</span>
          <span className=text-white>Technology is Here</span>
origin/cursor/automate-test-improve-and-merge-code-2533
        </motion.h1>
          transition={{ duration: 0 && 0.8, delay: 0 && 0.2 }}'
          className='text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight>;
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent'>;
            The Future of;
          </span>;
          <br />;
          <span className=text-white'>Technology is Here</span>        </motion && motion.h1>        >;"
          <span className="bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent>;
            The Future of;
          </span>;
          <br />;
          <span className="text-white">Technology is Here</span>;
        </motion && motion.h1>;

        {/* Subheading */}
        <motion&& motion.p;
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0 && 0.8, delay: 0 && 0.4 }}'
          className=text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed>;
          Experience the revolution with our cutting-edge AI consciousness,
          quantum computing, space technology, and emerging tech solutions.;
          Transform your business with technologies that were once science;"
          fiction.        </motion && motion.p>          className="text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed";
        >;
          Experience the revolution with our cutting-edge AI consciousness, quantum computing, space technology, and emerging tech solutions. Transform your business with ;
          technologies that were once science fiction.;
          transition={{ duration: 0.8, delay: 0.4 }}className="text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed";
        >;
          Experience the revolution with our cutting-edge AI consciousness, quantum computing, space technology, and emerging tech solutions. Transform your business with;
          technologies that were once science fiction.;
        </motion.p>;
        </motion.p>;
className='text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed';
        >;
          Experience the revolution with our cutting-edge AI consciousness;
          quantum computing, space technology, and emerging tech solutions.;
          Transform your business with technologies that were once science;
          fiction.;
        </motion.p>;
          technologies that were once science fiction.;
          transition={{ duration: 0.8, delay: 0.4 }}
          className="text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed"
        >
          Experience the revolution with our cutting-edge AI consciousness, quantum computing, space technology, and emerging tech solutions. Transform your business with 
          technologies that were once science fiction.
        </motion.p>



        </motion.p>

          fiction.        </motion && motion.p>          className=text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed";
        >;
          Experience the revolution with our cutting-edge AI consciousness, quantum computing, space technology, and emerging tech solutions. Transform your business with ;
          technologies that were once science fiction.;

          technologies that were once science fiction.



        {/* CTA Buttons */}
        <motion.div
          initial={{ opacity: 0, y: 30}}
          animate={{ opacity: 1, y: 0}}
          transition={{ duration: 0.8, delay: 0.6 }}
          className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16"
        >
          <Link
            href="/services"
            className="group inline-flex items-center space-x-3 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-2xl hover:shadow-purple-500/25"
          >
            <span>Explore All Services</span>
            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </Link>
          
          <Link
            href={`tel:${contactInfo.mobile}`}
            className="group inline-flex items-center space-x-3 bg-transparent border-2 border-purple-500/50 text-purple-400 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-purple-500/10 hover:border-purple-400 transition-all duration-300"
          >
            <Phone className="w-5 h-5" />
            <span>Call {contactInfo.mobile}</span>
          </Link>
        </motion.div>
        {/* CTA Buttons */}
          technologies that were once science fiction.



        </motion.p>

          transition={{ "duration": 0.8, "delay": 0.4 
=======
        {/* Main Heading *}
}
        <motion&& motion.h1;
          initial={{ opacity: 0, y: 30 }
>>>>>>> origin/chore/fix-lint-and-merge
}
>>>>>>> origin/main

          animate={{ opacity: 1}
            y: 0
             

<<<<<<< HEAD
          transition={{ duration: 0.8,
  delay: 0.2,
  className='text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight'
         />
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent' />
            The Future of}
            </span>
          <br />
=======
          transition={{ duration: 0.8, delay: 0.2 }
}
className='text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight'
            />
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent'    />
            The Future of;
          </span>
          <br    />
>>>>>>> origin/main

          <span className='text-white'    />Technology is Here</span>
        </motion.h1>

<<<<<<< HEAD
          transition={{ duration: 0 && 0.8
            delay: 0 && 0.2
            className='text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight'>;
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent' />;
=======
          transition={{ duration: 0 && 0.8, delay: 0 && 0.2 }
}
          className='text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight'>;
          <span className='bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent'    />;
>>>>>>> origin/main
            The Future of;
          </span>;
          <br    />;
          <span className='text-white'    />Technology is Here</span>        </motion && motion.h1>        >;
          <span className=\"bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent\"    />;
            The Future of;
          </span>;
<<<<<<< HEAD
          <br />;"
          <span className=\"text-white\" />Technology is Here</span>;
        </motion && motion.h1>
            {/* Subheading */
        <motion&& motion.p;
          initial={{ opacity: 0}
            y: 30
             
          animate={{ opacity: 1}
            y: 0
             
          transition={{ duration: 0 && 0.8,
  delay: 0 && 0.4,
  className='text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed' />;
=======
          <br    />;"
          <span className=\"text-white\"    />Technology is Here</span>;
        </motion && motion.h1>;{/* Subheading */}
        <motion&& motion.p;
          initial={{ opacity: 0, y: 30 }
}
          animate={{ opacity: 1, y: 0 }
}
          transition={{ duration: 0 && 0.8, delay: 0 && 0.4 }
}
          className='text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed'    />;
>>>>>>> origin/main
          Experience the revolution with our cutting-edge AI consciousness,quantum computing, space technology, and emerging tech solutions.;
          Transform your business with technologies that were once science;"
          fiction.        </motion && motion.p>          className=\"text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed\">

          Experience the revolution with our cutting-edge AI consciousness, quantum computing, space technology, and emerging tech solutions. Transform your business with ;
          technologies that were once science fiction.}
            transition={{ duration: 0.8,
  delay: 0.4
            className='text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto mb-12 leading-relaxed'
        >
          Experience the revolution with our cutting-edge AI consciousness;
quantum computing, space technology
            and emerging tech solutions.
          Transform your business with technologies that were once science
            fiction.
        </motion.p>

          technologies that were once science fiction.
<<<<<<< HEAD
=======

<<<<<<< HEAD
        {/* CTA Buttons */
=======
>>>>>>> origin/chore/fix-lint-and-merge
        {/* CTA Buttons */}
>>>>>>> origin/main
        <motion&& motion.div;
          initial={{ opacity: 0}
            y: 30
             

          animate={{ opacity: 1}
            y: 0
             

<<<<<<< HEAD
          transition={{ duration: 0.8,
  delay: 0.6}
            className='flex flex-col sm:flex-row gap-4 justify-center items-center mb-16'
         />
          transition={{ duration: 0 && 0.8
            delay: 0 && 0.6 className='flex flex-col sm:flex-row gap-4 justify-center items-center mb-16'>
            <Link href='/services'
          <Link href={`tel: ${contactInfo.mobile`}
=======
          transition={{ duration: 0.8, delay: 0.6 }
}
<<<<<<< HEAD
className='flex flex-col "sm":flex-row gap-4 justify-center items-center mb-16''
        >
          transition={{ duration: 0 && 0.8, delay: 0 && 0.6 }}'
          className='flex flex-col sm:flex-row gap-4 justify-center items-center mb-16'>;
          <Link'
            href='/services'
          <Link;`
            href={`tel:${contactInfo && contactInfo.mobile}`}'
            className='group inline-flex items-center space-x-3 bg-transparent border-2 border-purple-500/50 text-purple-400 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-purple-500/10 hover:border-purple-400 transition-all duration-300'>;'
            <Phone className='w-5 h-5' />            <span>Call {contactInfo && contactInfo.mobile}</span>        >;
          <Link"
            href="/services"

          <Link;`
            href={`tel:${contactInfo && contactInfo.mobile}`}"
            className="group inline-flex items-center space-x-3 bg-transparent border-2 border-purple-500/50 text-purple-400 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-purple-500/10 hover:border-purple-400 transition-all duration-300">;"
            <Phone className="w-5 h-5" />;
            <span>Call {contactInfo && contactInfo.mobile}</span>;
          </Link>;
        </motion && motion.div>;

            href={`tel:${contactInfo.mobile}`}
        <motion&& motion.div;
          initial={{ opacity: 0, y: 30 }
=======
className='flex flex-col sm:flex-row gap-4 justify-center items-center mb-16'
            />
          transition={{ duration: 0 && 0.8, delay: 0 && 0.6 }
>>>>>>> origin/chore/fix-lint-and-merge
}
          className='flex flex-col sm:flex-row gap-4 justify-center items-center mb-16'>;
          <Link;
href='/services'
          <Link;
href={`tel: ${contactInfo.mobile}`}
>>>>>>> origin/main
            className='group inline-flex items-center space-x-3 bg-transparent border-2 border-purple-500/50 text-purple-400 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-purple-500/10 hover:border-purple-400 transition-all duration-300'
              />
            <Phone className='w-5 h-5'    />
            <span    />Call {contactInfo.mobile}</span>
          </Link>
        </motion.div>

        {/* Stats Grid *}
}
        <motion&& motion.div;
<<<<<<< HEAD
          initial={{ opacity: 0}
            y: 30
             

          animate={{ opacity: 1}
            y: 0
             

          transition={{ duration: 0.8}
            delay: 0.8 className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-16'
         />
          {heroStats.map((stat index) => (
            <motion.div;
=======
<<<<<<< HEAD

=======
          initial={{ opacity: 0, y: 30 }
}

          animate={{ opacity: 1, y: 0 }
}
>>>>>>> origin/chore/fix-lint-and-merge

          transition={{ duration: 0.8, delay: 0.8 }
}
className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-16'
            />
          {heroStats.map((stat, index) => (
<<<<<<< HEAD
            <motion.div
                left: `${Math.random () * 100}%`,
                top: `${Math.random () * 100}%`,
              }}              animate={{
                coordinate_y: [0, -20, 0],
                opacity: [0.3, 0.8, 0.3],
            <motion.div;`
                left: `${Math.random () * 100}%`,`
                top: `${Math.random () * 100}%`,
              }}              animate={{}
                coordinate_y: [0, -20, 0],
                opacity: [0.3, 0.8, 0.3],
              }}
              transition={{}
                duration: 3 + Math.random () * 2,
                repeat: Infinity,
                delay: Math.random () * 2,
              }}            />                delay: Math.random () * 2}}
        {/* Stats Grid */}
        <motion&& motion.div;
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}transition={{ duration: 0.8, delay: 0.8 }}
className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-16';
        >;
          {heroStats.map((stat, index) => (<motion.div;
                left: `${Math.random () * 100}%`,top: `${Math.random () * 100}%`,}}              animate={{coordinate_y: [0, -20, 0],opacity: [0.3, 0.8, 0.3],}}
              transition={{duration: 3 + Math.random () * 2,repeat: Infinity,delay: Math.random () * 2,}}            />                delay: Math.random () * 2}}
            />))}
        </div>;
        {/* Gradient Orbs */}
        <div className='absolute top - 20 left - 10 w - 72 h - 72 bg - purple - 500 / 10 rounded - full blur - 3xl animate - pulse' />;'
        <div className='absolute bottom - 20 right - 10 w - 96 h - 96 bg - cyan - 500 / 10 rounded - full blur - 3xl animate - pulse' />;'
        <div className='absolute top - 1/2 left - 1/2 transform -translate - x-1 / 2 -translate - y-1 / 2 w-[800px] h-[800px] bg - gradient - to - r from - purple - 500 / 5 via - cyan - 500 / 5 to - pink - 500 / 5 rounded - full blur - 3xl animate - spin - slow' />;'
      </div>;
      {/* Main Content */}'"
      <div className='relative z - 10 max - w-7xl mx - auto px - 4 sm:px - 6 lg:px - 8 text - center'>        {/* Hero Badge */}        <div className="absolute top - 20 left - 10 w - 72 h - 72 bg - purple - 500 / 10 rounded - full blur - 3xl animate - pulse" />;"
        <div className="absolute bottom - 20 right - 10 w - 96 h - 96 bg - cyan - 500 / 10 rounded - full blur - 3xl animate - pulse" />;"
        <div className="absolute top - 1/2 left - 1/2 transform -translate - x-1 / 2 -translate - y-1 / 2 w-[800px] h-[800px] bg - gradient - to - r from - purple - 500 / 5 via - cyan - 500 / 5 to - pink - 500 / 5 rounded - full blur - 3xl animate - spin - slow" />;
      </div>;
      {/* Main Content */}"
      <div className="relative z - 10 max - w-7xl mx - auto px - 4 sm:px - 6 lg:px - 8 text - center">;
      {/* Main Content */}
      <div className="relative z - 10 max - w-7xl mx - auto px - 4 sm:px - 6 lg:px - 8 text - center">;
        <motion.div;
          initial={{ opacity: 0, coordinate_y: 20 }}
          animate={{ opacity: 1, coordinate_y: 0 }}
          transition={{ duration: 0.6 }}
          transition={{ duration: 0.6 }}'
          className='inline - flex items - center space - x-2 bg - gradient - to - r from - purple - 500 / 20 to - cyan - 500 / 20 border border - purple - 500 / 30 rounded - full px - 6 py - 3 mb - 8 backdrop - blur - sm';
          transition={{ duration: 0 && 0.8, delay: 0 && 0.8 }}'"
          className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-16'>          className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16";
        >;
          {heroStats && heroStats.map((stat, index) => (;
            <motion&& motion.div;
              key={stat && stat.label}
              initial={{ opacity: 0, scale: 0 && 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0 && 0.6, delay: 1 + index * 0 && 0.1 }}'
              className='text-center group'>;
              <div;`
                className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat && stat.color} rounded-2xl transform transition-all duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg`}>;'
                <stat && stat.icon className='w-8 h-8 text-white' />;
              </div>;'
              <div className='text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2'>;
                {stat && stat.number}
              </div>;'
              <div className='text-gray-400 text-sm'>{stat && stat.label}</div>            </motion && motion.div>            >;`
              <div className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat && stat.color} rounded-2xl transform transition-all duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg`}>;"
                <stat && stat.icon className="w-8 h-8 text-white" />;
              </div>;"
              <div className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2">;
                {stat && stat.number}
              </div>;"
              <div className="text-gray-400 text-sm">{stat && stat.label}</div>;
          className='inline - flex items - center space - x-2 bg - gradient - to - r from - purple - 500 / 20 to - cyan - 500 / 20 border border - purple - 500 / 30 rounded - full px - 6 py - 3 mb - 8 backdrop - blur - sm';
          transition={{ duration: 0 && 0.8, delay: 0 && 0.8 }}
          className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-16'>          className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16";
        >;
      {/* Main Content */}
      <div className="relative z - 10 max - w-7xl mx - auto px - 4 "sm":px - 6 "lg":px - 8 text - center">;"
        <motion.div;
          initial={{ opacity: 0, coordinate_y: 20 }}
          animate={{ opacity: 1, coordinate_y: 0 }}

          className='inline - flex items - center space - x-2 bg - gradient - to - r from - purple - 500 / 20 to - cyan - 500 / 20 border border - purple - 500 / 30 rounded - full px - 6 py - 3 mb - 8 backdrop - blur - sm';
          transition={{ duration: 0 && 0.8, delay: 0 && 0.8 }}
          className=grid grid-cols-2 md:grid-cols-4 gap-6 mb-16>          className=grid grid-cols-2 md:grid-cols-4 gap-6 mb-16;
        >;
          {heroStats && heroStats.map((stat, index) => (<motion&& motion.div;
              }
              key={stat && stat.label}
              initial={{ "opacity": 0, "scale": 0 && 0.8 
}
              animate={{ "opacity": 1, "scale": 1 
}
              transition={{ "duration": 0 && 0.6, "delay": 1 + index * 0 && 0.1 
}
              className='text-center group'>;'
              <div;

              </div>;
              <div className='text-3xl "md":text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2'>;'
                {stat && stat.number}
              </div>;

                <stat && stat.icon className="w-8 h-8 text-white" />;"
              </div>;
              <div className="text-3xl "md":text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2">;"
                {stat && stat.number}
              </div>;
              <div className="text-gray-400 text-sm">{stat && stat.label}</div>;
            </motion && motion.div>;
=======
            <motion.div;}
>>>>>>> origin/main
left: `${Math.random () * 10}
}%`,
                top: `${Math.random () * 10}
}%`
              }}              animate={{
                coordinate_y: [0, -20}
            0]
                opacity: [0.3, 0.8
            0.3]
              }}
              transition={{
<<<<<<< HEAD
                duration: 3 + Math.random () * 2}
            repeat: Infinity
            delay: Math.random () * 2
                          />                delay: Math.random () * 2}}
=======
                duration: 3 + Math.random () * 2,
>>>>>>> origin/chore/fix-lint-and-merge
                repeat: Infinity}
                delay: Math.random () * 2}
              }}               />                delay: Math.random () * 2}}
>>>>>>> origin/main

            />)
}
        </div>;
        {/* Gradient Orbs */}
<<<<<<< HEAD
        <div className='absolute top - 20 left - 10 w - 72 h - 72 bg - purple - 500 / 10 rounded - full blur - 3xl animate - pulse'    />
        <div className=absolute bottom - 20 right - 10 w - 96 h - 96 bg - cyan - 500 / 10 rounded - full blur - 3xl animate - pulse    />
        <div className='absolute top - 1/2 left - 1/2 transform -translate - x-1 / 2 -translate - y-1 / 2 w-[800px] h-[800px] bg - gradient - to - r from - purple - 500 / 5 via - cyan - 500 / 5 to - pink - 500 / 5 rounded - full blur - 3xl animate - spin - slow'    />
      </div>
        <motion.div
=======
        <div className='absolute top - 20 left - 10 w - 72 h - 72 bg - purple - 500 / 10 rounded - full blur - 3xl animate - pulse'    />;
        <div className='absolute bottom - 20 right - 10 w - 96 h - 96 bg - cyan - 500 / 10 rounded - full blur - 3xl animate - pulse'    />;
        <div className='absolute top - 1/2 left - 1/2 transform -translate - x-1 / 2 -translate - y-1 / 2 w-[800px] h-[800px] bg - gradient - to - r from - purple - 500 / 5 via - cyan - 500 / 5 to - pink - 500 / 5 rounded - full blur - 3xl animate - spin - slow'    />;
      </div>;
      {/* Main Content */}
      <div className='relative z - 10 max - w-7xl mx - auto px - 4 sm: px - 6 lg:px - 8 text - center'    />        {/* Hero Badge *}"
}        <div className=\"absolute top - 20 left - 10 w - 72 h - 72 bg - purple - 500 / 10 rounded - full blur - 3xl animate - pulse\"    />;"
        <div className=\"absolute bottom - 20 right - 10 w - 96 h - 96 bg - cyan - 500 / 10 rounded - full blur - 3xl animate - pulse\"    />;"
        <div className=\"absolute top - 1/2 left - 1/2 transform -translate - x-1 / 2 -translate - y-1 / 2 w-[800px] h-[800px] bg - gradient - to - r from - purple - 500 / 5 via - cyan - 500 / 5 to - pink - 500 / 5 rounded - full blur - 3xl animate - spin - slow\"    />;
      </div>;
      {/* Main Content */}"
      <div className=\"relative z - 10 max - w-7xl mx - auto px - 4 sm:px - 6 lg:px - 8 text - center\"    />;
        <motion.div;
<<<<<<< HEAD
          initial={{ opacity: 0}
            coordinate_y: 20
             
          animate={{ opacity: 1}
            coordinate_y: 0
             
          transition={{ duration: 0.6 }
}
          className='inline - flex items - center space - x-2 bg - gradient - to - r from - purple - 500 / 20 to - cyan - 500 / 20 border border - purple - 500 / 30 rounded - full px - 6 py - 3 mb - 8 backdrop - blur - sm';
          transition={{ duration: 0 && 0.8}
            delay: 0 && 0.8 
"
          className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-16' />          className=\"grid grid-cols-2 md:grid-cols-4 gap-6 mb-16\">
=======
>>>>>>> origin/chore/fix-lint-and-merge
          initial={{ opacity: 0, coordinate_y: 20 }
}
          animate={{ opacity: 1, coordinate_y: 0 }}
<<<<<<< HEAD

            </motion && motion.div>
              key={stat.label}
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.6, delay: 1 + index * 0.1 }}</motion && motion.div>;className='text-center group';
className='text-center group';
            >;
              <div;`
                className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat.color} rounded-2xl transform transition-all duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg`}
              >'
                <stat.icon className='w-8 h-8 text-white' />
              </div>'
              <div className='text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2'>
              <div;
              </div>;
              <div className='text-gray-400 text-sm'>{stat.label}</div>;
            </motion.div>;
              transition={{ duration: 0.6, delay: 1 + index * 0.1 }}
        </motion && motion.div>;
                {stat.number}


              className=text-center group

=======
          transition={{ duration: 0.6 }
}
          className='inline - flex items - center space - x-2 bg - gradient - to - r from - purple - 500 / 20 to - cyan - 500 / 20 border border - purple - 500 / 30 rounded - full px - 6 py - 3 mb - 8 backdrop - blur - sm';
          transition={{ duration: 0 && 0.8, delay: 0 && 0.8 }
}"
          className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-16'    />          className=\"grid grid-cols-2 md:grid-cols-4 gap-6 mb-16\">
>>>>>>> origin/main

          {heroStats && heroStats.map((stat,,,
  index) => (
          <motion&& motion.div;}
              key={stat && stat.label initial={{ opacity: 0 scale: 0 && 0.8 }
}
<<<<<<< HEAD
              animate={{ opacity: 1}
            scale: 1
             
              transition={{ duration: 0 && 0.6}
            delay: 1 + index * 0 && 0.1 className='text-center group' />;
              <div className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat && stat.color
            rounded-2xl transform transition-all duration-300 group-hover: shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg
 />
=======
              animate={{ opacity: 1, scale: 1 }
}
              transition={{ duration: 0 && 0.6, delay: 1 + index * 0 && 0.1 }
}
              className='text-center group'    />;
              <div;
                className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat && stat.color} rounded-2xl transform transition-all duration-300 group-hover: shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg
}    />
>>>>>>> origin/main

                <stat && stat.icon className='w-8 h-8 text-white'    />;
              </div>;
<<<<<<< HEAD
              <div className='text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2' />
            {stat && stat.number
              </div>
            <div className='text-gray-400 text-sm' />{stat && stat.label</div>            </motion && motion.div>            >}
            <div className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat && stat.color
            rounded-2xl transform transition-all duration-300 group-hover: shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg
 />
=======
              <div className='text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2'    />;
                {stat && stat.number}
              </div>;
              <div className='text-gray-400 text-sm'    />{stat && stat.label}</div>            </motion && motion.div>            >;
              <div className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat && stat.color} rounded-2xl transform transition-all duration-300 group-hover: shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg
}    />
>>>>>>> origin/main
"
                <stat && stat.icon className=\"w-8 h-8 text-white\"    />;
              </div>;"
<<<<<<< HEAD
              <div className=\"text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2\" />
            {stat && stat.number
              </div>
            "
              <div className=\"text-gray-400 text-sm\" />{stat && stat.label</div>;
=======
              <div className=\"text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2\"    />;
                {stat && stat.number}
              </div>;"
              <div className=\"text-gray-400 text-sm\"    />{stat && stat.label}</div>;
>>>>>>> origin/main
            </motion && motion.div>;
              key={stat.label initial={{ opacity: 0 scale: 0.8 }
}
<<<<<<< HEAD
              animate={{ opacity: 1}
            scale: 1
             
=======
>>>>>>> origin/chore/fix-lint-and-merge
>>>>>>> origin/main

              transition={{ duration: 0.6}
            delay: 1 + index * 0.1 className='text-center group'
            >
<<<<<<< HEAD
              <div className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat.color
            rounded-2xl transform transition-all duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg
 />

                <stat.icon className='w-8 h-8 text-white' />}
            </div>
            <div className='text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2' />
            {stat.number
=======
              <div;
className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat.color} rounded-2xl transform transition-all duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg
}    />

                <stat.icon className='w-8 h-8 text-white'    />;
              </div>;
              <div className='text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2'    />;
                {stat.number}
>>>>>>> origin/main

              </div>
<<<<<<< HEAD
              <div className='text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2'>
                {stat.number}
              </div>;
              <div className='text-gray-400 text-sm'>{stat.label}</div>;
            </motion.div>;
          transition={{ duration: 0.8, delay: 1.2 }}
className='mb-16'
origin/cursor/automate-test-improve-and-merge-code-2533
        >
          <h2 className='text-2xl "md":text-3xl font-bold text-white mb-8'>'
            Featured Revolutionary Services
          </h2>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        >'
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-8'>
            Featured Revolutionary Services;
          </h2>'
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>            {featuredServices.map((service, index) => (        >"
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-8">
            Featured Revolutionary Services;
          </h2>"
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featuredServices.map((service, index) => (
              <motion.div;
                key={service.name}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: 1.4 + index * 0.1 }}'
                className='group'

<<<<<<< HEAD
              <div className='text-gray-400 text-sm' />{stat.label</div>
=======
className='group'
origin/cursor/automate-test-improve-and-merge-code-2533
              >
          transition={{ duration: 0 && 0.8, delay: 1 && 1.2 }}'
          className='mb-16'>;'
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-8'>;
            Featured Revolutionary Services;
          </h2>;'
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>            {featuredServices && featuredServices.map((service, index) => (        >;"
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-8">;
            Featured Revolutionary Services;
          </h2>;"
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">;
            {featuredServices && featuredServices.map((service, index) => (;
              <motion&& motion.div;
                key={service && service.name}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0 && 0.6, delay: 1 && 1.4 + index * 0 && 0.1 }}'
                className='group'>;
                <Link;
                  href={service && service.href}'
                  className='block relative bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 h-full transform transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-2xl hover:shadow-purple-500/20 hover:border-purple-500/50'>;
                  <div"

        {/* Stats Grid */}
        <motion.div
          initial={{ opacity: 0, y: 30}}
          animate={{ opacity: 1, y: 0}}
          transition={{ duration: 0.8, delay: 0.8 }}
          className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16"
        >
          {heroStats.map((stat, index) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1}}
              transition={{ duration: 0.6, delay: 1 + index * 0.1 }}
              className="text-center group"
            >
              <div className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-r ${stat.color} rounded-2xl transform transition-all duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40 group-hover:shadow-lg`}>
                <stat.icon className="w-8 h-8 text-white" />
              </div>
              <div className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2">
                {stat.number}
              </div>
              <div className="text-gray-400 text-sm">{stat.label}</div>
            </motion.div>
          ))}
        </motion.div>

        {/* Featured Services */}
          animate={{ opacity: 1, y: 0 }}


        <motion.div
          initial={{ opacity: 0, y: 30}}
          animate={{ opacity: 1, y: 0}}
          transition={{ duration: 0.8, delay: 1.2 }}
          className="mb-16"
        >
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-8">
            Featured Revolutionary Services
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featuredServices.map((service, index) => (
              <motion.div
                key={service.name}
                initial={{ opacity: 0, y: 20}}
                animate={{ opacity: 1, y: 0}}
                transition={{ duration: 0.6, delay: 1.4 + index * 0.1 }}
                className=group

className='group''
              >
          animate={{ opacity: 1, y: 0 }}transition={{ duration: 0.8, delay: 1.2 }}
className='mb-16';
        >;
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-8'>;
=======

              <div className='text-gray-400 text-sm'    />{stat.label}</div>
>>>>>>> origin/main
            </motion.div>

          ))
        {/* Featured Services */}
        <motion&& motion.div;
          initial={{ opacity: 0}
            y: 30
             

          animate={{ opacity: 1}
            y: 0
             

<<<<<<< HEAD
          transition={{ duration: 0.8}
            delay: 1.2
            className='mb-16'
         />
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-8' />
            Featured Revolutionary Services
            </h2>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6' />
            {featuredServices.map((service,,,
  index) => (
=======
          transition={{ duration: 0.8, delay: 1.2 }
}
className='mb-16'
            />
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-8'    />
            Featured Revolutionary Services;
          </h2>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'    />
            {featuredServices.map((service, index) => (}
>>>>>>> origin/main
              <motion.div;}
key={service.name,
  initial={{ opacity: 0,
  y: 20
            

<<<<<<< HEAD
                animate={{ opacity: 1
            y: 0,
  transition={{ duration: 0.6,
  delay: 1.4 + index * 0.1,
  className='group'
               />

          transition={{ duration: 0 && 0.8,
  delay: 1 && 1.2}
            className='mb-16'>;
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-8' />;
            Featured Revolutionary Services;
          </h2>;
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6' />            {featuredServices && featuredServices.map((service,,
  index) => (        >;"
          <h2 className=\"text-2xl md:text-3xl font-bold text-white mb-8\" />;
            Featured Revolutionary Services;
          </h2>
            "
          <div className=\"grid grid-cols-1 md:grid-cols-3 gap-6\" />;
            {featuredServices && featuredServices.map((service,,,
  index) => (
          <motion&& motion.div;}
                key={service && service.name,
  initial={{ opacity: 0,
  y: 20
            

                animate={{ opacity: 1
            y: 0,
  transition={{ duration: 0 && 0.6,
  delay: 1 && 1.4 + index * 0 && 0.1}
            className='group' />;
                <Link href={service && service.href
            className='block relative bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 h-full transform transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-2xl hover:shadow-purple-500/20 hover:border-purple-500/50' />
            <div className={`w-16 h-16 bg-gradient-to-r ${service.color rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover: shadow-lg hover:shadow-cyan-400/40
 />

                    <service.icon className='w-8 h-8 text-white' />
            </div>
            <h3 className='text-xl font-bold text-white mb-3 group-hover: text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300' />                    {service.nam

                  </h3>;
                  <p className='text-gray-400 text-sm mb-4 leading-relaxed' />}
            {service.description
=======
className='group'
                  />

          transition={{ duration: 0 && 0.8, delay: 1 && 1.2 }
}
          className='mb-16'>;
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-8'    />;
>>>>>>> origin/chore/fix-lint-and-merge
            Featured Revolutionary Services;
          </h2>;
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'    />            {featuredServices && featuredServices.map((service, index) => (        >;"
          <h2 className=\"text-2xl md:text-3xl font-bold text-white mb-8\"    />;
            Featured Revolutionary Services;
          </h2>;"
          <div className=\"grid grid-cols-1 md:grid-cols-3 gap-6\"    />;}
            {featuredServices && featuredServices.map((service, index) => (<motion&& motion.div;}
                key={service && service.name}
                initial={{ opacity: 0, y: 20 }
}
                animate={{ opacity: 1, y: 0 }
}
                transition={{ duration: 0 && 0.6, delay: 1 && 1.4 + index * 0 && 0.1 }
}
                className='group'    />;
                <Link;
                  href={service && service.href}
<<<<<<< HEAD
                  className='block relative bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 h-full transform transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-2xl hover:shadow-purple-500/20 hover:border-purple-500/50'>;
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                    className={`w-16 h-16 bg-gradient-to-r ${service.color} rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40`}
                  >
                    <service.icon className='w-8 h-8 text-white' />
                  </div>
                  <h3 className='text-xl font-bold text-white mb-3 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300'>                    {service.name}
                  </h3>
                  <p className='text-gray-400 text-sm mb-4 leading-relaxed'>
                  className='block relative bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 h-full transform transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-2xl hover:shadow-purple-500/20 hover:border-purple-500/50' />;

                  <div;
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />;

                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                    className={`w-16 h-16 bg-gradient-to-r ${service.color} rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40`}
                  >;
                    <service.icon className='w-8 h-8 text-white' />;
                  </div>;
                  <h3 className=text-xl font-bold text-white mb-3 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300>                    {service.name}
=======
                  className='block relative bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 h-full transform transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-2xl hover:shadow-purple-500/20 hover:border-purple-500/50'    />;

                  <div;
className={`w-16 h-16 bg-gradient-to-r ${service.color} rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover: shadow-lg hover:shadow-cyan-400/40
}    />

                    <service.icon className='w-8 h-8 text-white'    />;
                  </div>;
                  <h3 className='text-xl font-bold text-white mb-3 group-hover: text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300'    />                    {service.nam}
}
>>>>>>> origin/chore/fix-lint-and-merge
                  </h3>;
                  <p className='text-gray-400 text-sm mb-4 leading-relaxed'    />;
                    {service.description}
>>>>>>> origin/main
                  </p>;
<<<<<<< HEAD

                      {service.price}
                  <div
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                </Link>
              </motion.div>
            ))}
          </div>
        </motion.div>`
                    className={`w-16 h-16 bg-gradient-to-r ${service && service.color} rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40`}>;'
                    <service && service.icon className='w-8 h-8 text-white' />;
                  </div>;
'
                  <h3 className='text-xl font-bold text-white mb-3 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300'>                    {service && service.name}
                  </h3>;
'
                  <p className='text-gray-400 text-sm mb-4 leading-relaxed'>;
                    {service && service.description}
                  </p>;
'
                  <div className='flex items-center justify-between'>;'
                    <div className='text-2xl font-bold text-cyan-400'>;
                      {service && service.price}
                    </div>;'
                    <div className='text-purple-400 group-hover:text-cyan-400 transition-colors'>;'
                      <ArrowRight className='w-5 h-5 group-hover:translate-x-1 transition-transform' />;
                    </div>;
                  </div>;
'
                  <div className='absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none' />                </Link>                  ;"
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />;
                    </div>;
                    <div className='text-purple-400 group-hover:text-cyan-400 transition-colors'>;
                      <ArrowRight className='w-5 h-5 group-hover:translate-x-1 transition-transform' />;
                    </div>;
                  </div>;
                  <div className='absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none' />;
                </Link>;
              </motion.div>;
            ))}
          </div>;
        </motion.div>;
                    className={`w-16 h-16 bg-gradient-to-r ${service && service.color} rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40`}>;
                    <service && service.icon className='w-8 h-8 text-white' />;
                  </div>;<h3 className='text-xl font-bold text-white mb-3 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300'>                    {service && service.name}
                  </h3>;<p className='text-gray-400 text-sm mb-4 leading-relaxed'>;
                    {service && service.description}
                  </p>;<div className='flex items-center justify-between'>;
                    <div className='text-2xl font-bold text-cyan-400'>;
=======
                  <div className='flex items-center justify-between'    />;
                    <div className='text-2xl font-bold text-cyan-400'    />;
                      {service.price}

                    </div>
                    <div className='text-purple-400 group-hover: text-cyan-400 transition-colors'    />
                      <ArrowRight className='w-5 h-5 group-hover:translate-x-1 transition-transform'    />
                    </div>
                  </div>

                  <div className='absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none'    />
                </Link>
              </motion.div>

            )
}
          </div>;
        </motion.div>;
                    className={`w-16 h-16 bg-gradient-to-r ${service && service.color}
            rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover: shadow-lg hover:shadow-cyan-400/40
>

                    <service && service.icon className='w-8 h-8 text-white' />
            </div>
            <h3 className='text-xl font-bold text-white mb-3 group-hover: text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300' />                    {service && service.nam

<<<<<<< HEAD
                  </h3>;<p className='text-gray-400 text-sm mb-4 leading-relaxed' />;
=======
                    <service && service.icon className='w-8 h-8 text-white'    />;
                  </div>;<h3 className='text-xl font-bold text-white mb-3 group-hover: text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300'    />                    {service && service.nam}
}
                  </h3>;<p className='text-gray-400 text-sm mb-4 leading-relaxed'    />;
>>>>>>> origin/main
                    {service && service.description}
                  </p>;<div className='flex items-center justify-between'    />;
                    <div className='text-2xl font-bold text-cyan-400'    />;
>>>>>>> origin/chore/fix-lint-and-merge
                      {service && service.price}
                    </div>;
                    <div className='text-purple-400 group-hover:text-cyan-400 transition-colors'    />;
                      <ArrowRight className='w-5 h-5 group-hover:translate-x-1 transition-transform'    />;
                    </div>;
<<<<<<< HEAD
                  </div>;<div className='absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none' />                </Link>                  ;
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />;
=======
                  </div>;<div className='absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none'    />                </Link>                  ;"
                  <div className=\"absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
                </Link>;
              </motion && motion.div>;
            ))}
          </div>;
        </motion && motion.div>;{/* Contact Information */}
        <motion&& motion.div;
<<<<<<< HEAD
          initial={{ opacity: 0}
            y: 30
             
=======
<<<<<<< HEAD

                </Link>
              </motion && motion.div>
            ))}
          </div>
        </motion.div>
        {/* Contact Information */}
        <motion&& motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
        {/* Contact Information */}
        <motion&& motion.div;
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}

          transition={{ duration: 0.8, delay: 1.6 }}
className='bg-gradient-to-r from-purple-900/50 to-cyan-900/50 backdrop-blur-xl border border-purple-500/30 rounded-3xl p-8 max-w-4xl mx-auto'
origin/cursor/automate-test-improve-and-merge-code-2533
        >
        >'
          <h3 className='text-2xl font-bold text-white mb-6 text-center'>
          animate={{ opacity: 1, y: 0 }
=======
          initial={{ opacity: 0, y: 30 }
>>>>>>> origin/chore/fix-lint-and-merge
}
>>>>>>> origin/main

          animate={{ opacity: 1}
            y: 0
             

<<<<<<< HEAD
          transition={{ duration: 0.8,
  delay: 1.6}
            className='bg-gradient-to-r from-purple-900/50 to-cyan-900/50 backdrop-blur-xl border border-purple-500/30 rounded-3xl p-8 max-w-4xl mx-auto'
         />
          <h3 className='text-2xl font-bold text-white mb-6 text-center' />
=======
          transition={{ duration: 0.8, delay: 1.6 }
}
<<<<<<< HEAD
className='bg-gradient-to-r from-purple-900/50 to-cyan-900/50 backdrop-blur-xl border border-purple-500/30 rounded-3xl p-8 max-w-4xl mx-auto''
        >

=======
className='bg-gradient-to-r from-purple-900/50 to-cyan-900/50 backdrop-blur-xl border border-purple-500/30 rounded-3xl p-8 max-w-4xl mx-auto'
            />
          <h3 className='text-2xl font-bold text-white mb-6 text-center'    />
>>>>>>> origin/chore/fix-lint-and-merge
>>>>>>> origin/main
            Ready to Transform Your Business?
          </h3>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'    />
            <div className='flex items-center space-x-3 text-gray-300'    />
              <Phone className='w-5 h-5 text-cyan-400'    />

<<<<<<< HEAD
          transition={{ duration: 0 && 0.8
            delay: 1 && 1.6 className='bg-gradient-to-r from-purple-900/50 to-cyan-900/50 backdrop-blur-xl border border-purple-500/30 rounded-3xl p-8 max-w-4xl mx-auto'>;
          <h3 className='text-2xl font-bold text-white mb-6 text-center' />;
            Ready to Transform Your Business?;
          </h3>;<div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6' />;
            <div className='flex items-center space-x-3 text-gray-300' />}
            <Phone className='w-5 h-5 text-cyan-400' />
            <a href={`tel: ${contactInfo && contactInfo.mobile

                className='hover:text-cyan-400 transition-colors' />;
=======
          transition={{ duration: 0 && 0.8, delay: 1 && 1.6 }
}
          className='bg-gradient-to-r from-purple-900/50 to-cyan-900/50 backdrop-blur-xl border border-purple-500/30 rounded-3xl p-8 max-w-4xl mx-auto'>;
          <h3 className='text-2xl font-bold text-white mb-6 text-center'    />;
            Ready to Transform Your Business?;
          </h3>;<div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'    />;
            <div className='flex items-center space-x-3 text-gray-300'    />;
              <Phone className='w-5 h-5 text-cyan-400'    />;
              <a;
<<<<<<< HEAD
                href={`"tel": ${contactInfo && contactInfo.mobile},`}
                className='"hover":text-cyan-400 transition-colors'>;'
>>>>>>> origin/main
                {contactInfo && contactInfo.mobile}
              </a>;
            </div>;
            <div className='flex items-center space-x-3 text-gray-300'>;
              <Mail className='w-5 h-5 text-purple-400' />;
          <h3 className="text-2xl font-bold text-white mb-6 text-center">
            Ready to Transform Your Business?
          </h3>"
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">"
            <div className="flex items-center space-x-3 text-gray-300">"
              <Phone className="w-5 h-5 text-cyan-400" />"`
              <a href={`tel:${contactInfo.mobile}`} className="hover:text-cyan-400 transition-colors">
                {contactInfo.mobile}
              </a>
            </div>"
            <div className="flex items-center space-x-3 text-gray-300">"
              <Mail className="w-5 h-5 text-purple-400" />"`
              <a href={`mailto:${contactInfo.email}`} className="hover:text-purple-400 transition-colors">

<<<<<<< HEAD
              <a href={`mailto: ${contactInfo.email}`}
                className='hover:text-purple-400 transition-colors' />
=======
              <a;
          <h3 className="text-2xl font-bold text-white mb-6 text-center">;
            Ready to Transform Your Business?;
          </h3>;
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">;
            <div className="flex items-center space-x-3 text-gray-300">;
              <Phone className="w-5 h-5 text-cyan-400" />;
              <a href={`tel:${contactInfo.mobile}`} className="hover:text-cyan-400 transition-colors">;
                {contactInfo.mobile}
              </a>;
            </div>;
            <div className="flex items-center space-x-3 text-gray-300">;
              <Mail className="w-5 h-5 text-purple-400" />;
              <a href={`mailto:${contactInfo.email}`} className="hover:text-purple-400 transition-colors">;
                {contactInfo.email}
              </a>;
            </div>;
            <div className=flex items-center space-x-3 text-gray-300">;
              <MapPin className="w-5 h-5 text-green-400 />;
              <span>{contactInfo.address}</span>;
            </div>;
          </div>;
          <div className=text-center">;
            <Link;
              href="/contact;
              className=inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25";
            >;
              <span>Get Started Today</span>;
              <ArrowRight className="w-5 h-5 />;
                href={`mailto:${contactInfo.email}`}
                className='hover:text-purple-400 transition-colors;
              >;
                {contactInfo.email}
              <a
                className=group"
              >
                <Link
                  href={service.href}
                  className="block relative bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 h-full transform transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-2xl hover:shadow-purple-500/20 hover:border-purple-500/50
                >
                  <div className={`w-16 h-16 bg-gradient-to-r ${service.color} rounded-2xl flex items-center justify-center mb-6 transform transition-transform duration-300 group-hover:shadow-lg hover:shadow-cyan-400/40`}>
                    <service.icon className=w-8 h-8 text-white" />
                  </div>
                  
                  <h3 className="text-xl font-bold text-white mb-3 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-purple-400 group-hover:bg-clip-text transition-all duration-300>
                    {service.name}
                  </h3>
                  
                  <p className=text-gray-400 text-sm mb-4 leading-relaxed">
                    {service.description}
                  </p>
                  
                  <div className="flex items-center justify-between>
                    <div className=text-2xl font-bold text-cyan-400">{service.price}</div>
                    <div className="text-purple-400 group-hover:text-cyan-400 transition-colors>
                      <ArrowRight className=w-5 h-5 group-hover:translate-x-1 transition-transform" />
                    </div>
                  </div>
                  
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-cyan-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none />
                </Link>
              </motion.div>
            ))}
          </div>
        </motion.div>

        {/* Contact Information */}
        <motion.div
          initial={{ opacity: 0, y: 30}}
          animate={{ opacity: 1, y: 0}}
          transition={{ duration: 0.8, delay: 1.6 }}
          className=bg-gradient-to-r from-purple-900/50 to-cyan-900/50 backdrop-blur-xl border border-purple-500/30 rounded-3xl p-8 max-w-4xl mx-auto"
        >
          <h3 className="text-2xl font-bold text-white mb-6 text-center>
            Ready to Transform Your Business?
          </h3>
          
          <div className=grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="flex items-center space-x-3 text-gray-300>
              <Phone className=w-5 h-5 text-cyan-400" />
              <a href={`tel:${contactInfo.mobile}`} className="hover:text-cyan-400 transition-colors>
                {contactInfo.mobile}
              </a>
            </div>
            <div className="flex items-center space-x-3 text-gray-300">
              <Mail className="w-5 h-5 text-purple-400" />
              <a href={`mailto:${contactInfo.email}`} className="hover:text-purple-400 transition-colors">
                {contactInfo.email}
              </a>
            </div>
            <div className='flex items-center space-x-3 text-gray-300'>'
              <MapPin className='w-5 h-5 text-green-400' />'
              <span>{contactInfo.address}</span>
            </div>
          </div>
          <div className='text-center'>'
            <Link,
href='/contact''
              className='inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 "hover":shadow-xl "hover":shadow-cyan-500/30 "hover":shadow-xl "hover":shadow-purple-500/25''
            >
              <span>Get Started Today</span>
              <ArrowRight className="w-5 h-5" />
            </Link>
          </div>
        </motion.div>
      </div>`
                href={`mailto:${contactInfo && contactInfo.email}`}'
                className='hover:text-purple-400 transition-colors'>;
                {contactInfo && contactInfo.email}
              </a>;
            </div>;'
            <div className='flex items-center space-x-3 text-gray-300'>;'
            <div className='flex items-center space-x-3 text-gray-300'>;
              <MapPin className='w-5 h-5 text-green-400' />;
              <span>{contactInfo && contactInfo.address}</span>;
            </div>;
          </div>;<div className='text-center'>;
          </div>;
'
          <div className='text-center'>;
            <Link'
              href='/contact''
              className='inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25'>;
              <span>Get Started Today</span>;'
              <ArrowRight className='w-5 h-5' />            </Link>        >;"
          <h3 className="text-2xl font-bold text-white mb-6 text-center">;
            Ready to Transform Your Business?;
          </h3>;
"
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">;"
            <div className="flex items-center space-x-3 text-gray-300">;"
              <Phone className="w-5 h-5 text-cyan-400" />;"`
              <a href={`tel:${contactInfo && contactInfo.mobile}`} className="hover:text-cyan-400 transition-colors">;
                {contactInfo && contactInfo.mobile}
              </a>;
            </div>;"
            <div className="flex items-center space-x-3 text-gray-300">;"
              <Mail className="w-5 h-5 text-purple-400" />;"`
              <a href={`mailto:${contactInfo && contactInfo.email}`} className="hover:text-purple-400 transition-colors">;
                {contactInfo && contactInfo.email}
              </a>;
            </div>;"
            <div className="flex items-center space-x-3 text-gray-300">;"
              <MapPin className="w-5 h-5 text-green-400" />;
              <span>{contactInfo && contactInfo.address}</span>;
            </div>;
          </div>;
"
          <div className="text-center">;
            <Link"
              href="/contact""
              className="inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25">;
              <span>Get Started Today</span>;"
              <ArrowRight className="w-5 h-5" />;
            <Link;
              href='/contact;
              className=inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25'>;
              <span>Get Started Today</span>;
              <ArrowRight className='w-5 h-5 />            </Link>        >;
          <h3 className="text-2xl font-bold text-white mb-6 text-center>;
            Ready to Transform Your Business?;
          </h3>;<div className=grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">;
            <div className="flex items-center space-x-3 text-gray-300>;
              <Phone className=w-5 h-5 text-cyan-400" />;
              <a href={`tel:${contactInfo && contactInfo.mobile}`} className="hover:text-cyan-400 transition-colors>;
                {contactInfo && contactInfo.mobile}
              </a>;
            </div>;
            <div className=flex items-center space-x-3 text-gray-300">;
              <Mail className="w-5 h-5 text-purple-400 />;
              <a href={`mailto:${contactInfo && contactInfo.email}`} className=hover:text-purple-400 transition-colors">;
                {contactInfo && contactInfo.email}
              </a>;
            </div>;
            <div className="flex items-center space-x-3 text-gray-300">;"
              <MapPin className="w-5 h-5 text-green-400" />;"
              <span>{contactInfo && contactInfo.address}</span>;
            </div>;
          </div>;<div className="text-center">;"
            <Link;
              href="/contact";"
              className="inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 "hover":shadow-xl "hover":shadow-cyan-500/30 "hover":shadow-xl "hover":shadow-purple-500/25">;"
              <span>Get Started Today</span>;
              <ArrowRight className="w-5 h-5" />;
            </Link>;
          </div>;
        </motion && motion.div>;
      </div>;{/* Floating Action Button */}
      <motion&& motion.div;
                className='group';
              >;

      {/* Floating Action Button */}
      <motion&& motion.div'
                className='group';
              >;
                <Link;
                  href={service.href}'
                  className='block relative bg - gradient - to - br from - gray - 900 / 80 to - gray - 800 / 80 backdrop - blur - xl border border - gray - 700 / 50 rounded - 2xl p - 6 h - full transform transition - all duration - 500 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - 2xl hover:shadow - purple - 500 / 20 hover:border - purple - 500 / 50';
                >;
                  <div;`
                    className={`w - 16 h - 16 bg - gradient - to - r ${service.color} rounded - 2xl flex items - center justify - center mb - 6 transform transition - transform duration - 300 group - hover:shadow - lg hover:shadow - cyan - 400 / 40`}
                  >;'
                    <service.icon className='w - 8 h - 8 text - white' />;
                  </div>;'
                  <h3 className='text - xl font - bold text - white mb - 3 group - hover:text - transparent group - hover:bg - gradient - to - r group - hover:from - cyan - 400 group - hover:to - purple - 400 group - hover:bg - clip - text transition - all duration - 300'>                    {service.name}
                  </h3>;'
                  <p className='text - gray - 400 text - sm mb - 4 leading - relaxed'>;
                    {service.description}
                  </p>;'
                  <div className='flex items - center justify - between'>;'
                    <div className='text - 2xl font - bold text - cyan - 400'>;
                      {service.price}
                    </div>;'
                    <div className='text - purple - 400 group - hover:text - cyan - 400 transition - colors'>;'
                      <ArrowRight className='w - 5 h - 5 group - hover:translate - x-1 transition - transform' />;
                    </div>;
                  </div>;'
                  <div className='absolute inset - 0 bg - gradient - to - br from - purple - 500 / 5 to - cyan - 500 / 5 rounded - 2xl opacity - 0 group - hover:opacity - 100 transition - opacity duration - 300 pointer - events - none' />                </Link>;"
                  <div className="absolute inset - 0 bg - gradient - to - br from - purple - 500 / 5 to - cyan - 500 / 5 rounded - 2xl opacity - 0 group - hover:opacity - 100 transition - opacity duration - 300 pointer - events - none" />;
                <Link;
                  href={service.href}

                  <div;
                    className={`w - 16 h - 16 bg - gradient - to - r ${service.color} rounded - 2xl flex items - center justify - center mb - 6 transform transition - transform duration - 300 group - "hover": shadow - lg "hover":shadow - cyan - 400 / 40,`}>

                    <service.icon className='w - 8 h - 8 text - white' />;'
                  </div>;
                  <h3 className='text - xl font - bold text - white mb - 3 group - "hover": text - transparent group - "hover":bg - gradient - to - r group - "hover":from - cyan - 400 group - "hover":to - purple - 400 group - "hover":bg - clip - text transition - all duration - 300'>                    {service.nam,'
}
                  </h3>;
                  <p className='text - gray - 400 text - sm mb - 4 leading - relaxed'>;'
                    {service.description}
                  </p>;
                  <div className='flex items - center justify - between'>;'
                    <div className='text - 2xl font - bold text - cyan - 400'>;'
                      {service.price}
                    </div>;
                    <div className='text - purple - 400 group - "hover":text - cyan - 400 transition - colors'>;'
                      <ArrowRight className='w - 5 h - 5 group - "hover":translate - x-1 transition - transform' />;'
                    </div>;
                  </div>;
                  <div className='absolute inset - 0 bg - gradient - to - br from - purple - 500 / 5 to - cyan - 500 / 5 rounded - 2xl opacity - 0 group - hover:opacity - 100 transition - opacity duration - 300 pointer - events - none' />                </Link>;
                  <div className="absolute inset - 0 bg - gradient - to - br from - purple - 500 / 5 to - cyan - 500 / 5 rounded - 2xl opacity - 0 group - hover:opacity - 100 transition - opacity duration - 300 pointer - events - none" />;
                </Link>;

          <h3 className="text-2xl font-bold text-white mb-6 text-center>
            Ready to Transform Your Business?
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className=flex items-center space-x-3 text-gray-300">"
              <Phone className=w-5 h-5 text-cyan-400 />"`
              <a href={`tel:${contactInfo.mobile}`} className="hover:text-cyan-400 transition-colors>
                {contactInfo.mobile}
              </a>
            </div>
            <div className="flex items-center space-x-3 text-gray-300">
              <Mail className=w-5 h-5 text-purple-400" />"`
              <a href={`mailto:${contactInfo.email}`} className=hover:text-purple-400 transition-colors>

              <a
=======
                href={`tel: ${contactInfo && contactInfo.mobile}
}
                className='hover:text-cyan-400 transition-colors'    />;
                {contactInfo && contactInfo.mobile}
              </a>;
            </div>;
            <div className='flex items-center space-x-3 text-gray-300'    />;
              <Mail className='w-5 h-5 text-purple-400'    />;

              <a;
>>>>>>> origin/chore/fix-lint-and-merge
href={`mailto: ${contactInfo.email}`}
                className=hover:text-purple-400 transition-colors'    />
>>>>>>> origin/main

                {contactInfo.email}

              </a>
            </div>
            <div className='flex items-center space-x-3 text-gray-300    />
              <MapPin className=w-5 h-5 text-green-400'    />
              <span    />{contactInfo.address}</span>
            </div>
          </div>
<<<<<<< HEAD
          <div className='text-center' />
            <Link href='/contact'
=======
<<<<<<< HEAD
          <div className='text-center    />
            <Link
href=/contact'
              className='inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25
=======
          <div className='text-center'    />
            <Link;
href='/contact'
>>>>>>> origin/main
              className='inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25'
>>>>>>> origin/chore/fix-lint-and-merge
                />
              <span    />Get Started Today</span>
              <ArrowRight className=w-5 h-5'    />
            </Link>
          </div>
        </motion.div>
      </div>

<<<<<<< HEAD
              <MapPin className='w-5 h-5 text-green-400 />
              <span />{contactInfo && contactInfo.address}</span>
            </div>
            </Link>
          </div>
        </motion && motion.div>
      </div>;{/* Floating Action Button */}
      <motion&& motion.div
                className=group'    />

                </Link>
=======
                href={`mailto:${contactInfo && contactInfo.email}
}
                className='hover:text-purple-400 transition-colors'>;
                {contactInfo && contactInfo.email}
              </a>;
            </div>;
            <div className='flex items-center space-x-3 text-gray-300'    />;
              <MapPin className='w-5 h-5 text-green-400'    />;
              <span    />{contactInfo && contactInfo.address}</span>;
            </div>;
<<<<<<< HEAD
          </div>;<div className='text-center' />;
            <Link href='/contact';
              className='inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25' />;
              <span />Get Started Today</span>;
              <ArrowRight className='w-5 h-5' />            </Link>        >;"
          <h3 className=\"text-2xl font-bold text-white mb-6 text-center\" />;
=======
          </div>;<div className='text-center'    />;
            <Link;
              href='/contact';
              className='inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25'    />;
              <span    />Get Started Today</span>;
              <ArrowRight className='w-5 h-5'    />            </Link>        >;"
          <h3 className=\"text-2xl font-bold text-white mb-6 text-center\"    />;
>>>>>>> origin/main
            Ready to Transform Your Business?;"
          </h3>;<div className=\"grid grid-cols-1 md:grid-cols-3 gap-6 mb-6\"    />;"
            <div className=\"flex items-center space-x-3 text-gray-300\"    />;"
              <Phone className=\"w-5 h-5 text-cyan-400\"    />;
              <a href={`tel: ${contactInfo && contactInfo.mobile},"
} className=\"hover:text-cyan-400 transition-colors\"    />;
                {contactInfo && contactInfo.mobile}
              </a>;
            </div>;"
            <div className=\"flex items-center space-x-3 text-gray-300\"    />;"
              <Mail className=\"w-5 h-5 text-purple-400\"    />;
              <a href={`mailto: ${contactInfo && contactInfo.email},"
} className=\"hover:text-purple-400 transition-colors\"    />;
                {contactInfo && contactInfo.email}
              </a>;
            </div>;"
            <div className=\"flex items-center space-x-3 text-gray-300\"    />;"
              <MapPin className=\"w-5 h-5 text-green-400\"    />;
              <span    />{contactInfo && contactInfo.address}</span>;
            </div>;"
<<<<<<< HEAD
          </div>;<div className=\"text-center\" />;
            <Link"
              href=\"/contact\";"
              className=\"inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25\" />;
              <span />Get Started Today</span>;"
              <ArrowRight className=\"w-5 h-5\" />;
=======
          </div>;<div className=\"text-center\"    />;
            <Link;"
              href=\'/contact\';"
              className=\"inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transform transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/30 hover:shadow-xl hover:shadow-purple-500/25\"    />;
              <span    />Get Started Today</span>;"
              <ArrowRight className=\"w-5 h-5\"    />;
>>>>>>> origin/main
            </Link>;
          </div>;
        </motion && motion.div>;
      </div>;{/* Floating Action Button */}
      <motion&& motion.div;
                className='group'    />

<<<<<<< HEAD
                <Link href={service.href className='block relative bg - gradient - to - br from - gray - 900 / 80 to - gray - 800 / 80 backdrop - blur - xl border border - gray - 700 / 50 rounded - 2xl p - 6 h - full transform transition - all duration - 500 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - 2xl hover:shadow - purple - 500 / 20 hover:border - purple - 500 / 50' />

                  <div className={`w - 16 h - 16 bg - gradient - to - r ${service.color}
            rounded - 2xl flex items - center justify - center mb - 6 transform transition - transform duration - 300 group - hover: shadow - lg hover:shadow - cyan - 400 / 40
 />

                    <service.icon className='w - 8 h - 8 text - white' />
            </div>
            <h3 className='text - xl font - bold text - white mb - 3 group - hover: text - transparent group - hover:bg - gradient - to - r group - hover:from - cyan - 400 group - hover:to - purple - 400 group - hover:bg - clip - text transition - all duration - 300' />                    {service.nam

=======
                <Link;
                  href={service.href}
                  className='block relative bg - gradient - to - br from - gray - 900 / 80 to - gray - 800 / 80 backdrop - blur - xl border border - gray - 700 / 50 rounded - 2xl p - 6 h - full transform transition - all duration - 500 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - 2xl hover:shadow - purple - 500 / 20 hover:border - purple - 500 / 50'    />

                  <div;
                    className={`w - 16 h - 16 bg - gradient - to - r ${service.color} rounded - 2xl flex items - center justify - center mb - 6 transform transition - transform duration - 300 group - hover: shadow - lg hover:shadow - cyan - 400 / 40
}    />

                    <service.icon className='w - 8 h - 8 text - white'    />;
                  </div>;
                  <h3 className='text - xl font - bold text - white mb - 3 group - hover: text - transparent group - hover:bg - gradient - to - r group - hover:from - cyan - 400 group - hover:to - purple - 400 group - hover:bg - clip - text transition - all duration - 300'    />                    {service.nam}
}
>>>>>>> origin/main
                  </h3>;
                  <p className='text - gray - 400 text - sm mb - 4 leading - relaxed'    />;
                    {service.description}
                  </p>;
                  <div className='flex items - center justify - between'    />;
                    <div className='text - 2xl font - bold text - cyan - 400'    />;
                      {service.price}
                    </div>;
                    <div className='text - purple - 400 group - hover:text - cyan - 400 transition - colors'    />;
                      <ArrowRight className='w - 5 h - 5 group - hover:translate - x-1 transition - transform'    />;
                    </div>;
                  </div>;
                  <div className='absolute inset - 0 bg - gradient - to - br from - purple - 500 / 5 to - cyan - 500 / 5 rounded - 2xl opacity - 0 group - hover:opacity - 100 transition - opacity duration - 300 pointer - events - none'    />                </Link>;"
                  <div className=\"absolute inset - 0 bg - gradient - to - br from - purple - 500 / 5 to - cyan - 500 / 5 rounded - 2xl opacity - 0 group - hover:opacity - 100 transition - opacity duration - 300 pointer - events - none\"    />;
                </Link>;
>>>>>>> origin/chore/fix-lint-and-merge
              </motion.div>))}
          </div>;
        </motion.div>;
        {/* Contact Information */}
        <motion.div;
<<<<<<< HEAD
          initial={{ opacity: 0}
            coordinate_y: 30
             
          animate={{ opacity: 1}
            coordinate_y: 0
             
          transition={{ duration: 0.8}
            delay: 1.6 className='bg - gradient - to - r from - purple - 900 / 50 to - cyan - 900 / 50 backdrop - blur - xl border border - purple - 500 / 30 rounded - 3xl p - 8 max - w-4xl mx - auto' />
=======
<<<<<<< HEAD
          initial={{ opacity: 0, coordinate_y: 30 }}
          animate={{ opacity: 1, coordinate_y: 0 }}
          transition={{ duration: 0.8, delay: 1.6 }}
          className='bg - gradient - to - r from - purple - 900 / 50 to - cyan - 900 / 50 backdrop - blur - xl border border - purple - 500 / 30 rounded - 3xl p - 8 max - w-4xl mx - auto';
        >;
          <h3 className='text - 2xl font - bold text - white mb - 6 text - center'>;
          initial={{ opacity: 0, coordinate_y: 30 }}
          animate={{ opacity: 1, coordinate_y: 0 }}
          transition={{ duration: 0.8, delay: 1.6 }}'
          className='bg - gradient - to - r from - purple - 900 / 50 to - cyan - 900 / 50 backdrop - blur - xl border border - purple - 500 / 30 rounded - 3xl p - 8 max - w-4xl mx - auto';
        >;'
          <h3 className='text - 2xl font - bold text - white mb - 6 text - center'>;
            Ready to Transform Your Business?;
          </h3>;'
          <div className='grid grid - cols - 1 md:grid - cols - 3 gap - 6 mb - 6'>;'
            <div className='flex items - center space - x-3 text - gray - 300'>;'
              <Phone className='w - 5 h - 5 text - cyan - 400' />;
              <a;`
                href={`tel:${contact_info.mobile}`}'
                className='hover:text - cyan - 400 transition - colors';
              >;
                {contact_info.mobile}
              </a>;
            </div>;'
            <div className='flex items - center space - x-3 text - gray - 300'>;'
              <Mail className='w - 5 h - 5 text - purple - 400' />;
              <a;`
                href={`mailto:${contact_info.email}`}'
                className='hover:text - purple - 400 transition - colors';
              >;
                {contact_info.email}
              </a>;
            </div>;'
            <div className='flex items - center space - x-3 text - gray - 300'>;'
=======
          initial={{ opacity: 0, coordinate_y: 30 }
}
          animate={{ opacity: 1, coordinate_y: 0 }
}
          transition={{ duration: 0.8, delay: 1.6 }
}
          className='bg - gradient - to - r from - purple - 900 / 50 to - cyan - 900 / 50 backdrop - blur - xl border border - purple - 500 / 30 rounded - 3xl p - 8 max - w-4xl mx - auto'    />
>>>>>>> origin/main

          <h3 className='text - 2xl font - bold text - white mb - 6 text - center'    />;
>>>>>>> origin/chore/fix-lint-and-merge
            Ready to Transform Your Business?;
          </h3>;
<<<<<<< HEAD
          <div className='grid grid - cols - 1 md:grid - cols - 3 gap - 6 mb - 6' />;
            <div className='flex items - center space - x-3 text - gray - 300' />;
              <Phone className='w - 5 h - 5 text - cyan - 400' />
            <a href={`tel: ${contact_info.mobile`}
                className='hover:text - cyan - 400 transition - colors' />
=======
          <div className='grid grid - cols - 1 md:grid - cols - 3 gap - 6 mb - 6'    />;
            <div className='flex items - center space - x-3 text - gray - 300'    />;
              <Phone className='w - 5 h - 5 text - cyan - 400'    />;
              <a;
<<<<<<< HEAD
                href={`tel:${contact_info.mobile}`}
                className='hover:text - cyan - 400 transition - colors';
              >;
              <MapPin className='w - 5 h - 5 text - green - 400' />;
              <span />{contact_info.address}</span>;
            </div>;
          </div>;'
          <div className='text - center'>;
            <Link;'
              href='/contact';'
              className='inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25';
            >;
              <span > Get Started Today</span>;'
              <ArrowRight className='w - 5 h - 5' />            </Link>        >;"
          <h3 className="text - 2xl font - bold text - white mb - 6 text - center">;
            Ready to Transform Your Business?;
          </h3>;"
          <div className="grid grid - cols - 1 md:grid - cols - 3 gap - 6 mb - 6">;"
            <div className="flex items - center space - x-3 text - gray - 300">;"
              <Phone className="w - 5 h - 5 text - cyan - 400" />;"`
              <a href={`tel:${contact_info.mobile}`} className="hover:text - cyan - 400 transition - colors">;
                {contact_info.mobile}
              </a>;
            </div>;"
            <div className="flex items - center space - x-3 text - gray - 300">;"
              <Mail className="w - 5 h - 5 text - purple - 400" />;"`
              <a href={`mailto:${contact_info.email}`} className="hover:text - purple - 400 transition - colors">;
                {contact_info.email}
              </a>;
            </div>;"
            <div className="flex items - center space - x-3 text - gray - 300">;"
              <MapPin className="w - 5 h - 5 text - green - 400" />;
              <span>{contact_info.address}</span>;
            </div>;
          </div>;"
          <div className="text - center">;
            <Link;"
              href="/contact";"
              className="inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25";
            >;
              <span > Get Started Today</span>;"
              <ArrowRight className="w - 5 h - 5" />;
                {contact_info.mobile}
              </a>;
            </div>;
            <div className='flex items - center space - x-3 text - gray - 300'>;
              <Mail className='w - 5 h - 5 text - purple - 400' />;
              <a;
                href={`mailto:${contact_info.email}`}
                className='hover:text - purple - 400 transition - colors';
              >;
                {contact_info.email}
              </a>;
            </div>;
            <div className='flex items - center space - x-3 text - gray - 300'>;
              <MapPin className='w - 5 h - 5 text - green - 400' />;
              <span>{contact_info.address}</span>;
            </div>;
          </div>;
          <div className='text - center'>;
            <Link;
              href='/contact';
              className='inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25';
            >;
              <span > Get Started Today</span>;
              <ArrowRight className='w - 5 h - 5' />            </Link>        >;'
          <h3 className="text - 2xl font - bold text - white mb - 6 text - center">;"
=======
                href={`tel: ${contact_info.mobile}`}
                className='hover:text - cyan - 400 transition - colors'    />
>>>>>>> origin/main

                {contact_info.mobil}
}
              </a>;
            </div>;
<<<<<<< HEAD
            <div className='flex items - center space - x-3 text - gray - 300' />;
              <Mail className='w - 5 h - 5 text - purple - 400' />;
              <a href={`mailto: ${contact_info.email}`}
                className='hover:text - purple - 400 transition - colors' />
=======
            <div className='flex items - center space - x-3 text - gray - 300'    />;
              <Mail className='w - 5 h - 5 text - purple - 400'    />;
              <a;
                href={`mailto: ${contact_info.email}`}
                className='hover:text - purple - 400 transition - colors'    />
>>>>>>> origin/main

                {contact_info.emai}
}
              </a>;
            </div>;
            <div className='flex items - center space - x-3 text - gray - 300'    />;
              <MapPin className='w - 5 h - 5 text - green - 400'    />;
              <span    />{contact_info.address}</span>;
            </div>;
          </div>;
<<<<<<< HEAD
          <div className='text - center' />;
            <Link href='/contact';
              className='inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25' />
=======
          <div className='text - center'    />;
            <Link;
              href='/contact';
              className='inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25'    />
>>>>>>> origin/main

              <span     /> Get Started Today</span>;
              <ArrowRight className='w - 5 h - 5'    />            </Link>        >;"
          <h3 className=\"text - 2xl font - bold text - white mb - 6 text - center\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
            Ready to Transform Your Business?;
          </h3>;"
          <div className=\"grid grid - cols - 1 md:grid - cols - 3 gap - 6 mb - 6\"    />;"
            <div className=\"flex items - center space - x-3 text - gray - 300\"    />;"
              <Phone className=\"w - 5 h - 5 text - cyan - 400\"    />;
              <a href={`tel: ${contact_info.mobile},"
} className=\"hover:text - cyan - 400 transition - colors\"    />;
                {contact_info.mobile}
              </a>;
            </div>;"
            <div className=\"flex items - center space - x-3 text - gray - 300\"    />;"
              <Mail className=\"w - 5 h - 5 text - purple - 400\"    />;
              <a href={`mailto: ${contact_info.email},"
} className=\"hover:text - purple - 400 transition - colors\"    />;
                {contact_info.email}
              </a>;
            </div>;"
            <div className=\"flex items - center space - x-3 text - gray - 300\"    />;"
              <MapPin className=\"w - 5 h - 5 text - green - 400\"    />;
              <span    />{contact_info.address}</span>;
            </div>;
<<<<<<< HEAD
            <div className="flex items - center space - x-3 text - gray - 300">;"
              <MapPin className="w - 5 h - 5 text - green - 400" />;"
              <span>{contact_info.address}</span>;
            </div>;
          </div>;
          <div className="text - center">;"
            <Link;
              href="/contact";
              className="inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25";
            >;
              <span > Get Started Today</span>;
              <ArrowRight className="w - 5 h - 5" />;
=======
          </div>;"
<<<<<<< HEAD
          <div className=\"text - center\" />;
            <Link"
              href=\"/contact\";"
              className=\"inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25\" />
=======
          <div className=\"text - center\"    />;
            <Link;"
              href=\'/contact\';"
              className=\"inline - flex items - center space - x-2 bg - gradient - to - r from - cyan - 500 to - purple - 600 text - white px - 8 py - 4 rounded - xl font - semibold text - lg transform transition - all duration - 300 hover:shadow - xl hover:shadow - cyan - 500 / 30 hover:shadow - xl hover:shadow - purple - 500 / 25\"    />
>>>>>>> origin/main

              <span     /> Get Started Today</span>;"
              <ArrowRight className=\"w - 5 h - 5\"    />;
>>>>>>> origin/chore/fix-lint-and-merge
            </Link>;
          </div>;
        </motion.div>;
      </div>;
<<<<<<< HEAD
        <motion.div
              <MapPin className=w - 5 h - 5 text - green - 400' />
              <span />{contact_info.address}</span>
            </div>
            </Link>
          </div>
        </motion.div>
      </div>
      {/* Floating Action Button */}
      <motion.div;
        initial={{ opacity: 0, scale: 0 }}
        animate={{ opacity: 1, scale: 1 }}}href={`tel:${contactInfo && contactInfo.mobile}`}
        animate={{ opacity: 1, scale: 1 }}


}
`
          href={`tel:${contactInfo && contactInfo.mobile}`}'
          className='group w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/25 transform transition-all duration-300 hover:shadow-lg hover:shadow-cyan-400/40 hover:shadow-2xl hover:shadow-purple-500/40'>;'
          <Phone className='w-7 h-7 text-white group-hover:rotate-12 transition-transform' />;
        </Link>;
      </motion && motion.div>;
    </section>;
}'
        className='fixed bottom - 8 right - 8 z - 50';
      >;
        <Link;`
          href={`tel:${contact_info.mobile}`}'
          className='group w - 16 h - 16 bg - gradient - to - r from - cyan - 500 to - purple - 600 rounded - full flex items - center justify - center shadow - 2xl shadow - purple - 500 / 25 transform transition - all duration - 300 hover:shadow - lg hover:shadow - cyan - 400 / 40 hover:shadow - 2xl hover:shadow - purple - 500 / 40';
        >;'
          <Phone className='w - 7 h - 7 text - white group - hover:rotate - 12 transition - transform' />;
        </Link>;
      </motion.div>;
    </section>);      >;
        <Link;`
          href={`tel:${contact_info.mobile}`}"
          className="group w - 16 h - 16 bg - gradient - to - r from - cyan - 500 to - purple - 600 rounded - full flex items - center justify - center shadow - 2xl shadow - purple - 500 / 25 transform transition - all duration - 300 hover: shadow - lg hover:shadow - cyan - 400 / 40 hover:shadow - 2xl hover:shadow - purple - 500 / 40";
        >;"
          <Phone className="w - 7 h - 7 text - white group - hover:rotate - 12 transition - transform" />;
        </Link>;
      </motion.div>;
    </section>);
}'
        className='fixed bottom-8 right-8 z-50'
        className='fixed bottom - 8 right - 8 z - 50';
      >;


        transition={{ "duration": 0.8, "delay": 2 }}
className='fixed bottom-8 right-8 z-50''
      >
        <Link;`
          href={`tel:${contactInfo.mobile}`}'
          className='group w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/25 transform transition-all duration-300 hover:shadow-lg hover:shadow-cyan-400/40 hover:shadow-2xl hover:shadow-purple-500/40'
        >'
        <Link;
          href={`tel:${contact_info.mobile}`}
          className='group w - 16 h - 16 bg - gradient - to - r from - cyan - 500 to - purple - 600 rounded - full flex items - center justify - center shadow - 2xl shadow - purple - 500 / 25 transform transition - all duration - 300 hover:shadow - lg hover:shadow - cyan - 400 / 40 hover:shadow - 2xl hover:shadow - purple - 500 / 40';
        >;
          <Phone className='w - 7 h - 7 text - white group - hover:rotate - 12 transition - transform' />;
        </Link>;
      </motion.div>;
    </section>)>;
        <Link;
          href={`tel:${contact_info.mobile}`}
          className="group w - 16 h - 16 bg - gradient - to - r from - cyan - 500 to - purple - 600 rounded - full flex items - center justify - center shadow - 2xl shadow - purple - 500 / 25 transform transition - all duration - 300 hover: shadow - lg hover:shadow - cyan - 400 / 40 hover:shadow - 2xl hover:shadow - purple - 500 / 40";
        >;
          <Phone className="w - 7 h - 7 text - white group - hover:rotate - 12 transition - transform" />;
        </Link>;
      </motion.div>;
    </section>)}
        className='fixed bottom-8 right-8 z-50';
        transition={{ duration: 0.8, delay: 2 }}
className='fixed bottom-8 right-8 z-50';
      >;
        <Link;
          href={`tel:${contactInfo.mobile}`}
          className='group w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/25 transform transition-all duration-300 hover:shadow-lg hover:shadow-cyan-400/40 hover:shadow-2xl hover:shadow-purple-500/40';
        >;
          <Phone className='w-7 h-7 text-white group-hover:rotate-12 transition-transform' />;
        </Link>;
      </motion.div>;
  )>;
        <Link;
    </section>);
}
        className='fixed bottom-8 right-8 z-50'


      >
        <Link
          href={`tel:${contactInfo.mobile}`}
          className='group w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/25 transform transition-all duration-300 hover:shadow-lg hover:shadow-cyan-400/40 hover:shadow-2xl hover:shadow-purple-500/40'
        >
          <Phone className='w-7 h-7 text-white group-hover:rotate-12 transition-transform' />
        </Link>
      </motion.div>
  );      >
        <Link
          href={`tel:${contactInfo.mobile}`}
          className="group w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/25 transform transition-all duration-300 hover: shadow-lg hover:shadow-cyan-400/40 hover:shadow-2xl hover:shadow-purple-500/40";
        >;
          <Phone className="w-7 h-7 text-white group-hover:rotate-12 transition-transform" />;
        </Link>;
      </motion.div>;
    </section>;
  )}
    </section>;
  ))</section>;
  )</section>;
        </Link>
      </motion.div>
  );      >

}
    </section>
  );
  );
    </section>
  );


  )

      {/* Floating Action Button */}
      <motion.div
        initial={{ opacity: 0, scale: 0}}
        animate={{ opacity: 1, scale: 1}}
        transition={{ duration: 0.8, delay: 2}}
        className="fixed bottom-8 right-8 z-50"
      >
        <Link
          href={`tel:${contactInfo.mobile}`}
          className="group w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/25 transform transition-all duration-300 hover:shadow-lg hover:shadow-cyan-400/40 hover:shadow-2xl hover:shadow-purple-500/40"
        >
          <Phone className="w-7 h-7 text-white group-hover:rotate-12 transition-transform" />
        </Link>
      </motion.div>
    </section>
  )
}
  );

"
    </section>
  );


    </section>
  );
origin/cursor/automate-test-improve-and-merge-code-2533
=======
      {/* Floating Action Button */}
      <motion.div;
        initial={{ opacity: 0}
            scale: 0
             

        animate={{ opacity: 1}
            scale: 1
             

<<<<<<< HEAD
        transition={{ duration: 0.8}
            delay: 2 className='fixed bottom-8 right-8 z-50'
       />
        <Link href={`tel:${contactInfo.mobile
=======
        transition={{ duration: 0.8, delay: 2 }}
className='fixed bottom-8 right-8 z-50'
          />
        <Link;
href={`tel:${contactInfo.mobile}
>>>>>>> origin/main
}
          className='group w-16 h-16 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/25 transform transition-all duration-300 hover:shadow-lg hover:shadow-cyan-400/40 hover:shadow-2xl hover:shadow-purple-500/40'
            />
          <Phone className='w-7 h-7 text-white group-hover:rotate-12 transition-transform'    />
        </Link>
      </motion.div>
>>>>>>> origin/chore/fix-lint-and-merge

    </section>
  );

<<<<<<< HEAD
"
=======
<<<<<<< HEAD
"
=======
"
>>>>>>> origin/chore/fix-lint-and-merge
>>>>>>> origin/main
