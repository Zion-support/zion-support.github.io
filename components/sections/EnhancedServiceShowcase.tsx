...
>>>>>>> origin/cursor/fix-syntax-push-and-merge-to-main-dbb7
  reviews: number;
}interface EnhancedServiceShowcaseProps  {title: string;
  subtitle: string;
  show_filters?: boolean;
...
>>>>>>> origin/cursor/expand-services-advertise-and-build-project-961d
  ...
  maxServices = 12;
  maxServices?: number;
}const EnhancedServiceShowcase: React.FC<EnhancedServiceShowcaseProps> = ({title;
  subtitle;
  showFilters;const [selectedCategory, setSelectedCategory] = useState<string>('all')const [selectedPriceRange, setSelectedPriceRange] = useState<string>('all')const [sortBy, setSortBy] = useState<string>('popular')const categories = [;
  const filteredServices = useMemo(() => {const filtered = services.filter(service => {const matchesCategory = selectedCategory === 'all' |;
                             (selectedCategory === 'ai' && (service.category.includes('AI') |service.category.includes('Machine Learning'))) |;
                             (selectedCategory === 'quantum' && (service.category.includes('Quantum') |service.category.includes('Space'))) |;
                             (selectedCategory === 'blockchain' && (service.category.includes('Blockchain') |service.category.includes('DeFi') |service.category.includes('NFT'))) |;
                             (selectedCategory === 'enterprise' && (service.category.includes('Enterprise') |service.category.includes('IT'))) |;
                             (selectedCategory === 'emerging' && (service.category.includes('Neural') |service.category.includes('Autonomous') |service.category.includes('Space') |service.category.includes('Biotech')))const matchesPrice = selectedPriceRange === 'all' |;
                          (selectedPriceRange === 'low' && parseFloat(service.price.replace(/[$]/g, '')) < 1000) |;
                          (selectedPriceRange === 'medium' && parseFloat(service.price.replace(/[$]/g, '')) >= 1000 && parseFloat(service.price.replace(/[$]/g, '')) < 5000) |;
                          (selectedPriceRange === 'high' && parseFloat(service.price.replace(/[$]/g, '')) >= 5000 && parseFloat(service.price.replace(/[$]/g, '')) < 20000) |;
                          (selectedPriceRange === 'premium' && parseFloat(service.price.replace(/[$]/g, '')) >= 20000)return matchesCategory && matchesPrice;
    { id: 'all', name: 'All Services', icon: '🚀' },{ id: 'ai', name: 'AI & ML', icon: '🧠' },{ id: 'quantum', name: 'Quantum', icon: '⚛️' },{ id: 'blockchain', name: 'Blockchain', icon: '⛓️' },{ id: 'enterprise', name: 'Enterprise', icon: '🏢' },{ id: 'emerging', name: 'Emerging Tech', icon: '🌟' },];
  const priceRanges = [;
    { id: 'all', name: 'All Prices' },{ id: 'low', name: 'Under $1K/month' },{ id: 'medium', name: '$1K - $5K/month' },{ id: 'high', name: '$5K - $20K/month' },{ id: 'premium', name: '$20K+/month' },];
  const sortOptions = [;
    { id: 'popular', name: 'Most Popular' },{ id: 'rating', name: 'Highest Rated' },{ id: 'roi', name: 'Best ROI' },{ id: 'price-low', name: 'Price Low to High' },{ id: 'price-high', name: 'Price High to Low' },];
  const filteredServices = useMemo(() => {let filtered = services.filter(service => {const matchesCategory =;
        selectedCategory === 'all' |;
        (selectedCategory === 'ai' &&;
          (service.category.includes('AI') |;
            service.category.includes('Machine Learning'))) |;
        ...
        (selectedCategory === 'emerging' &&;
          (service.category.includes('Neural') |;
            service.category.includes('Autonomous') |;
            service.category.includes('Space') |;
            service.category.includes('Biotech')))const matchesPrice =;
        selectedPriceRange === 'all' |;
        (selectedPriceRange === 'low' &&;
          parseFloat(service.price.replace(/[$]/g, '')) < 1000) |;
        (selectedPriceRange === 'medium' &&;
        ...
        (selectedPriceRange === 'high' &&;
          ...
          parseFloat(service.price.replace(/[$]/g, '')) < 20000) |;
        (selectedPriceRange === 'premium' &&;
          parseFloat(service.price.replace(/[$]/g, '')) >= 20000)return matchesCategory && matchesPrice;})// Sort services;
    switch (sortBy) {case 'popular':;
        filtered.sort((a, b) => (b.popular ? 1 : 0) - (a.popular ? 1 : 0))break;
      case 'rating':;
        filtered.sort((a, b) => (b.rating |0) - (a.rating |0))break;
      case 'roi':;
        filtered.sort((a, b) => {subtitle;
  showFilters = false;
  services = [],maxServices = 12;
  const [selectedCategory, setSelectedCategory] = useState<string>('all')const [selectedPriceRange, setSelectedPriceRange] = useState<string>('all')const [sortBy, setSortBy] = useState<string>('popular')const categories = [;
    { id: 'all', name: 'All Services', icon: '🚀' },{ id: 'ai', name: 'AI & ML', icon: '🧠' },{ id: 'quantum', name: 'Quantum', icon: '⚛️' },{ id: 'blockchain', name: 'Blockchain', icon: '⛓️' },{ id: 'enterprise', name: 'Enterprise', icon: '🏢' },{ id: 'emerging', name: 'Emerging Tech', icon: '🌟' },  ];    { id: 'emerging', name: 'Emerging Tech', icon: '🌟' }
  ];
  const priceRanges = [;
  ...
  const sortOptions = [;
    { id: 'popular', name: 'Most Popular' },{ id: 'rating', name: 'Highest Rated' },{ id: 'roi', name: 'Best ROI' },{ id: 'price-low', name: 'Price Low to High' },{ id: 'price-high', name: 'Price High to Low' },];
  const filteredServices = useMemo(() => {let filtered = services && services.filter(service => {const matchesCategory =;
        selectedCategory === 'all' ||;
        (selectedCategory === 'ai' &&;
        ...
        (selectedCategory === 'emerging' &&;
          (service && service.category.includes('Neural') ||;
            ...
            service && service.category.includes('Space') ||;
            service && service.category.includes('Biotech')))const matchesPrice =;
        selectedPriceRange === 'all' ||;
        (selectedPriceRange === 'low' &&;
        ...
        (selectedPriceRange === 'premium' &&;
          parseFloat(service && service.price.replace(/[$]/g, '')) >= 20000)const filteredServices = useMemo(() => {const filtered = services && services.filter(service => {const matchesCategory = selectedCategory === 'all' || ;
                             ...
                             (selectedCategory === 'emerging' && (service && service.category.includes('Neural') || service && service.category.includes('Autonomous') || service && service.category.includes('Space') || service && service.category.includes('Biotech')))const matchesPrice = selectedPriceRange === 'all' ||;
                          ...
                          (selectedPriceRange === 'premium' && parseFloat(service && service.price.replace(/[$]/g, '')) >= 20000)// Sort services;
    switch (sortBy) {case 'popular':;
        filtered && filtered.sort((a, b) => (b && b.popular ? 1 : 0) - (a && a.popular ? 1 : 0))break;
      case 'rating':;
        filtered && filtered.sort((a, b) => (b && b.rating || 0) - (a && a.rating || 0))break;
      case 'roi':;
<<<<<<< HEAD
<<<<<<< HEAD
...
<<<<<<< HEAD
                      ...
                      <span className='text-cyan-400'>
=======
                      Email:{' '}<span className='text-cyan-400'>;
=======
                      Email:{' '}
                      <span className='text-cyan-400'>;
>>>>>>> origin/cursor/expand-services-advertise-and-build-project-961d
...
=======
>>>>>>> origin/cursor/fix-syntax-push-and-merge-to-main-dbb7
        </motion && motion.div>;<span className='text-cyan-400'>;
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c
