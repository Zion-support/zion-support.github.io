export default function October2025AutonomousEnterpriseMeshBanner() {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-cyan-900/30 via-blue-900/30 to-indigo-900/30 backdrop-blur-sm border-b border-cyan-500/20 py-16">
      <div className="absolute inset-0 overflow-hidden opacity-20">
        <div className="absolute top-0 right-1/4 w-96 h-96 bg-cyan-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse"></div>
        <div className="absolute bottom-0 left-1/4 w-96 h-96 bg-blue-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
      </div>

      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 mb-6 animate-pulse">
            <span className="text-cyan-400 font-bold text-xl tracking-wider uppercase">
              🚀 BREAKING: OCTOBER 1, 2025 - AUTONOMOUS ENTERPRISE MESH REVOLUTION!
            </span>
          </div>
          
          <h2 className="text-4xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-cyan-400 via-blue-400 to-indigo-400 bg-clip-text text-transparent leading-tight">
            Autonomous Enterprise Mesh: $4.2B Success Story
          </h2>
          
          <p className="text-xl md:text-2xl text-gray-300 mb-8 max-w-4xl mx-auto font-semibold">
            Fortune 100 Manufacturing Achieves 99.99% Operational Autonomy • $4.2B Value Creation • 15x ROI • 87% Cost Reduction
          </p>
        </div>

        {/* Key Metrics */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12 max-w-5xl mx-auto">
          <div className="bg-gradient-to-br from-cyan-500/20 to-cyan-600/20 backdrop-blur-lg rounded-xl p-6 border border-cyan-500/30">
            <div className="text-4xl font-extrabold text-cyan-400 mb-2">99.99%</div>
            <div className="text-sm text-cyan-300">Operational Autonomy</div>
          </div>
          <div className="bg-gradient-to-br from-blue-500/20 to-blue-600/20 backdrop-blur-lg rounded-xl p-6 border border-blue-500/30">
            <div className="text-4xl font-extrabold text-blue-400 mb-2">$4.2B</div>
            <div className="text-sm text-blue-300">Value Creation</div>
          </div>
          <div className="bg-gradient-to-br from-indigo-500/20 to-indigo-600/20 backdrop-blur-lg rounded-xl p-6 border border-indigo-500/30">
            <div className="text-4xl font-extrabold text-indigo-400 mb-2">15x</div>
            <div className="text-sm text-indigo-300">ROI in 24 Months</div>
          </div>
          <div className="bg-gradient-to-br from-purple-500/20 to-purple-600/20 backdrop-blur-lg rounded-xl p-6 border border-purple-500/30">
            <div className="text-4xl font-extrabold text-purple-400 mb-2">87%</div>
            <div className="text-sm text-purple-300">Cost Reduction</div>
          </div>
        </div>

        {/* Key Features */}
        <div className="grid md:grid-cols-3 gap-6 mb-12 max-w-6xl mx-auto">
          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-cyan-500/50 transition-all duration-300">
            <div className="text-5xl mb-4">🎯</div>
            <h3 className="text-xl font-bold text-white mb-3">Self-Healing Systems</h3>
            <p className="text-gray-300 text-sm mb-4">
              Automatic detection and resolution of system failures with zero human intervention.
              94% faster incident resolution.
            </p>
            <ul className="text-sm text-gray-400 space-y-1">
              <li>✓ Predictive failure detection</li>
              <li>✓ Automatic remediation</li>
              <li>✓ Zero-touch operations</li>
              <li>✓ 99.99% uptime guaranteed</li>
            </ul>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-blue-500/50 transition-all duration-300">
            <div className="text-5xl mb-4">🧠</div>
            <h3 className="text-xl font-bold text-white mb-3">Intelligent Orchestration</h3>
            <p className="text-gray-300 text-sm mb-4">
              AI-driven resource allocation and workload management across 450 global facilities.
              12x faster decision-making.
            </p>
            <ul className="text-sm text-gray-400 space-y-1">
              <li>✓ Real-time optimization</li>
              <li>✓ Predictive analytics</li>
              <li>✓ Autonomous decisions</li>
              <li>✓ 95% accuracy rate</li>
            </ul>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 hover:border-indigo-500/50 transition-all duration-300">
            <div className="text-5xl mb-4">⚡</div>
            <h3 className="text-xl font-bold text-white mb-3">Unified Data Fabric</h3>
            <p className="text-gray-300 text-sm mb-4">
              Seamless integration of all enterprise data sources with real-time analytics.
              Sub-50ms response time.
            </p>
            <ul className="text-sm text-gray-400 space-y-1">
              <li>✓ Real-time data access</li>
              <li>✓ Unified visibility</li>
              <li>✓ Advanced analytics</li>
              <li>✓ 360-degree insights</li>
            </ul>
          </div>
        </div>

        {/* Call to Actions */}
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <a
            href="/blog/ai-2025-oct-01-autonomous-enterprise-mesh-revolution"
            className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-cyan-500/50 transform hover:-translate-y-1"
          >
            Read Complete Guide →
          </a>
          <a
            href="/case-studies/ai-2025-oct-global-manufacturing-autonomous-mesh-4-2-billion-success"
            className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-400 hover:to-indigo-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-blue-500/50 transform hover:-translate-y-1"
          >
            View $4.2B Success Story →
          </a>
          <a
            href="/services/autonomous-enterprise-mesh-consulting"
            className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-indigo-500/50 transform hover:-translate-y-1"
          >
            Get Started Today →
          </a>
        </div>

        {/* Additional Info */}
        <div className="mt-12 text-center">
          <p className="text-gray-400 text-sm mb-4">
            <span className="text-cyan-400 font-semibold">NEW OCTOBER 2025:</span> The most comprehensive guide to achieving operational autonomy at enterprise scale
          </p>
          <div className="flex flex-wrap justify-center gap-4 text-xs">
            <span className="bg-cyan-500/20 px-3 py-1 rounded-full text-cyan-300">450 Global Facilities</span>
            <span className="bg-blue-500/20 px-3 py-1 rounded-full text-blue-300">250,000+ Employees</span>
            <span className="bg-indigo-500/20 px-3 py-1 rounded-full text-indigo-300">$87B Enterprise</span>
            <span className="bg-purple-500/20 px-3 py-1 rounded-full text-purple-300">12-Month Implementation</span>
            <span className="bg-pink-500/20 px-3 py-1 rounded-full text-pink-300">Industry Leader</span>
          </div>
        </div>
      </div>
    </section>
  );
}
