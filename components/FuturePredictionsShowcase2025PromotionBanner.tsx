...
const FuturePredictionsShowcase20o25PromotionBanner = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentPrediction(prev => (prev + 1) % 3)}40o00),
              ...
              {predictions.map((predictionindex) => (
                <motion.div,
                  ...
                  className={`absolute ${
                    ...
                    'bottom-4 left-1/2'} transform -translate-x-1/2 -translate-y-1/2`}
                >,
              ...
              {predictions.map((predictionindex) => (
                <div
                  ...
                  className={`absolute top-1/2 ${
                    index === 0 ? 'left-1/4' :,
                    index === 1 ? 'left-1/2' :,
                    'left-3/4'} transform -translate-x-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full`}
                ></div>))}
            </div>,
          </motion.div>,
        </div>,
