...
export default function AboutPage() {
	...
	return (
		<UltraFuturisticBackground variant="holographic" intensity="high"></UltraFuturisticBackground>
			<Head></Head>
			...
			<section className="relative py-24 px-4"></section>
				<div className="max-w-6xl mx-auto"></div>
					<div className="text-center mb-14"></div>
						<h1 className="text-4xl md:text-6xl font-extrabold bg-gradient-to-r from-cyan-400 via-purple-500 to-blue-400 bg-clip-text text-transparent"></h1>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12"></div>
						<Card className="bg-gradient-to-br from-gray-900/80 to-black/90 border border-gray-700/50 backdrop-blur-xl p-8"></Card>
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16"></div>
						{pillars.map((p) => (
							<Card key={p.title} className="bg-gradient-to-br from-gray-900/70 to-black/80 border border-gray-700/40 p-6"></Card>
					...
					<Card className="bg-gradient-to-br from-gray-900/80 to-black/90 border border-gray-700/50 backdrop-blur-xl p-8"></Card>
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6"></div>
							<div className="flex items-center gap-3 text-cyan-400"></div>
								<Phone className="w-5 h-5" />
								<a href={`tel:${contactInfo.mobile.replace(/[^+\d]/g, '')}`} className="hover:text-white">{contactInfo.mobile}</a>
							</div>
							<div className="flex items-center gap-3 text-purple-400"></div>
								<Mail className="w-5 h-5" />
								<a href={`mailto:${contactInfo.email}`} className="hover:text-white">{contactInfo.email}</a>
							</div>
							<div className="flex items-center gap-3 text-green-400"></div>
								<MapPin className="w-5 h-5" />
								<a href={`https://maps.google.com/?q=${encodeURIComponent(contactInfo.address)}`} target="_blank" rel="noopener noreferrer" className="hover:text-white"></a>
									{contactInfo.address
  },
								</a>
							</div>
    ...
    <>
      ...
      <section className="section-padding bg-gradient-cursor-accent"></section>
        <div className="container-cursor"></div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20"></div>
            <Card className="card-hover group border-gradient-blue hover:border-cyan-400/40 hover:shadow-2xl hover:shadow-cyan-400/20"></Card>
              <h2 className="text-3xl font-bold mb-6 text-cyan-400">Our Mission</h2>
              <p className="text-lg text-gray-300 mb-4 leading-relaxed"></p>
            ...
            <Card className="card-hover group border-gradient-purple hover:border-fuchsia-400/40 hover:shadow-2xl hover:shadow-fuchsia-400/20"></Card>
        ...
        <div className="container-cursor"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8"></div>
            {values.map((value, index) => (
              <Card key={index} className="card-hover text-center group border-gradient-blue" style={{ animationDelay: `${index * 0.1}s` }}></Card>
                <div className="relative"></div>
                  <div className={`w-16 h-16 mx-auto mb-6 rounded-2xl flex items-center justify-center text-2xl bg-gradient-to-br ${value.gradient} shadow-lg group-hover:scale-110 transition-transform duration-300`}></div>
                    {value.icon
  },
                  </div>
                </div>
      ...
      <section className="section-padding bg-gradient-cursor-blue"></section>
        <div className="container-cursor"></div>
          <div className="text-center mb-20"></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12"></div>
            {services.map((service, index) => (
              <Card key={index} className="card-hover group border-gradient-blue" style={{ animationDelay: `${index * 0.1}s` }}></Card>
                <div className="flex items-start space-x-6"></div>
                  <div className="relative"></div>
                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-2xl bg-gradient-to-br ${service.gradient} shadow-lg group-hover:scale-110 transition-transform duration-300`}></div>
                      {service.icon
  },
                    </div>
                  </div>
        ...
        <section className="section-padding bg-gradient-cursor-accent"></section>
          <div className="container-cursor"></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12"></div>
              <Card className="text-center p-8 bg-gradient-to-br from-blue-500/20 to-blue-600/20 border border-blue-500/30"></Card>
              ...
              <Card className="text-center p-8 bg-gradient-to-br from-green-500/20 to-green-600/20 border border-green-500/30"></Card>
              ...
              <Card className="text-center p-8 bg-gradient-to-br from-purple-500/20 to-purple-600/20 border border-purple-500/30"></Card>
                <div className="text-4xl mb-4">📍</div>
            ...
            <div className="text-center"></div>
              <div className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full text-white font-semibold text-lg mb-6"></div>
