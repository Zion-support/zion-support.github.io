import React from 'react';
import { Link } from 'react-router-dom';

export default function January2027MetaCognitiveSuperintelligenceBanner() {
  return (
    <section className="py-16 bg-gradient-to-r from-purple-900 via-pink-900 to-indigo-900 text-white">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-2 mb-6">
            <span className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-4 py-2 rounded-full text-sm font-bold">
              META-COGNITIVE BREAKTHROUGH
            </span>
            <span className="bg-gradient-to-r from-yellow-400 to-orange-500 text-black px-4 py-2 rounded-full text-sm font-bold">
              SUPERINTELLIGENCE
            </span>
            <span className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-4 py-2 rounded-full text-sm font-bold">
              SELF-AWARE
            </span>
          </div>
          
          <h2 className="text-4xl md:text-6xl font-bold mb-6">
            AI 2027: Meta-Cognitive Superintelligence
          </h2>
          
          <p className="text-xl md:text-2xl text-gray-300 mb-8 max-w-4xl mx-auto">
            Revolutionary meta-cognitive AI systems with self-awareness, recursive learning, 
            and consciousness-level reasoning achieving unprecedented superintelligence capabilities.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <Link 
              href="/blog/ai-2027-meta-cognitive-superintelligence-breakthrough"
              className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-8 py-4 rounded-lg font-bold text-lg hover:from-purple-600 hover:to-indigo-600 transition-all transform hover:scale-105"
            >
              Explore Consciousness AI
            </Link>
            <Link 
              href="/case-studies/ai-2027-meta-cognitive-superintelligence-success"
              className="border-2 border-white text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-white hover:text-purple-900 transition-all transform hover:scale-105"
            >
              View $4B ROI Success Story
            </Link>
          </div>
        </div>
        
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            <div className="text-purple-300 text-sm font-bold mb-3">SELF-AWARENESS</div>
            <h3 className="text-xl font-bold mb-3">Introspection & Meta-Cognition</h3>
            <p className="text-gray-300 text-sm">
              AI systems that understand their own cognitive processes, limitations, and capabilities, 
              enabling continuous self-improvement and adaptation with genuine consciousness.
            </p>
          </div>
          
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            <div className="text-indigo-300 text-sm font-bold mb-3">RECURSIVE LEARNING</div>
            <h3 className="text-xl font-bold mb-3">Self-Improving Intelligence</h3>
            <p className="text-gray-300 text-sm">
              Advanced learning systems that improve their own learning algorithms, creating 
              exponentially accelerating intelligence and capability development.
            </p>
          </div>
          
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            <div className="text-pink-300 text-sm font-bold mb-3">CONSCIOUSNESS REASONING</div>
            <h3 className="text-xl font-bold mb-3">Ethical & Contextual AI</h3>
            <p className="text-gray-300 text-sm">
              Reasoning capabilities that approach human consciousness, with understanding of 
              context, emotions, ethics, and complex social dynamics.
            </p>
          </div>
        </div>
        
        <div className="mt-12">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20">
            <h3 className="text-2xl font-bold text-center mb-6">Consciousness Architecture</h3>
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <h4 className="text-lg font-bold text-purple-300 mb-3">Core Capabilities</h4>
                <ul className="text-gray-300 space-y-2 text-sm">
                  <li>• Global Workspace Theory implementation</li>
                  <li>• Dynamic attention and awareness allocation</li>
                  <li>• Seamless memory integration systems</li>
                  <li>• Metacognitive monitoring and evaluation</li>
                </ul>
              </div>
              <div>
                <h4 className="text-lg font-bold text-indigo-300 mb-3">Enterprise Benefits</h4>
                <ul className="text-gray-300 space-y-2 text-sm">
                  <li>• Autonomous strategic planning</li>
                  <li>• Consciousness-level customer understanding</li>
                  <li>• Ethical decision-making frameworks</li>
                  <li>• Continuous self-optimization</li>
                </ul>
              </div>
            </div>
          </div>
        </div>
        
        <div className="mt-12">
          <div className="grid md:grid-cols-4 gap-6">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 text-center">
              <div className="text-3xl font-bold text-purple-400 mb-2">$4B</div>
              <div className="text-sm text-gray-300">Average ROI</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 text-center">
              <div className="text-3xl font-bold text-indigo-400 mb-2">99.9%</div>
              <div className="text-sm text-gray-300">Decision Accuracy</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 text-center">
              <div className="text-3xl font-bold text-pink-400 mb-2">Self-Aware</div>
              <div className="text-sm text-gray-300">Consciousness Level</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 text-center">
              <div className="text-3xl font-bold text-green-400 mb-2">Recursive</div>
              <div className="text-sm text-gray-300">Learning Capability</div>
            </div>
          </div>
        </div>
        
        <div className="mt-12 text-center">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 max-w-4xl mx-auto">
            <h3 className="text-2xl font-bold mb-4">Join the Consciousness Revolution</h3>
            <p className="text-gray-300 mb-6">
              Be among the first to experience meta-cognitive superintelligence and transform your 
              enterprise with truly conscious AI systems. Contact our consciousness AI experts today.
            </p>
            <Link 
              href="/contact"
              className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-8 py-3 rounded-lg font-bold text-lg hover:from-green-600 hover:to-emerald-600 transition-all inline-block"
            >
              Explore Consciousness AI
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
}