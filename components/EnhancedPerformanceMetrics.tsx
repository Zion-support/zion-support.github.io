...
interface PerformanceMetric  {"id": string;
  ...
  "description": string;
}interface PerformanceMetricsProps  {title?: string;
  }
  subtitle?: string;
  showCharts?: boolean;
  autoRefresh?: boolean;
...
const interval = setInterval(() => {setMetrics(prevMetrics =>;
        }
        prevMetrics.map(metric => ({...metric, "value": metric.value + (Math.random() - 0.5) * metric.value * 0.1, "change": metric.change + (Math.random() - 0.5) * 2;
        ...
        >;
          ...
          <div className="bg-white rounded-2xl shadow-lg p-2 border border-gray-200">;"
            {(['1h', '6h', '24h', '7d'] as const).map((range) => (<button;'
                ...
                className={`px-6 py-3 rounded-xl font-medium transition-all duration-200 ${timeRange === range;`                    ? 'bg-blue-600 text-white shadow-lg';'
  ...
  return (<motion.div;
              ...
              >;
                <div className="flex items-center justify-between mb-4">;"
                  ...
                  <div className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(metric.status)}`}>;`                  <div className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(metric.status)}`}>`
                    {metric.status}
                  </div>;
                </div>;
                ...
                <div className="flex items-center gap-2 mb-4">;"
                  <ChangeIcon className={`w-4 h-4 ${getChangeColor(metric.changeType)}`} />;`                  <span className={`text-sm font-medium ${getChangeColor(metric.changeType)}`}>;`                  <span className={`text-sm font-medium ${getChangeColor(metric.changeType)}`}>`
                ...
                <div className="mb-4">;"
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">;"
                    <div;
                      className={`h-2 rounded-full transition-all duration-500 ${metric.value / metric.target > 0.9;`                          ? 'bg-red-500';'
                    ...
                    >;
                      ...
                      <div className="space-y-3">;"
                        ...
                        <div className="flex justify-between text-sm">;"
                          ...
                          <span className={`font-medium ${getChangeColor(metric.changeType)}`}>;`                          <span className={`font-medium ${getChangeColor(metric.changeType)}`}>`
          ...
          <div className="grid grid-cols-1 "md":grid-cols-3 gap-6">;"
            <div>;
              <div className="text-3xl font-bold mb-2">;"
                {metrics.filter(m => { return m.status === 'excellent').length}'
              </div>; }
              <div className="text-blue-100">Excellent Metrics</div>;"
            </div>;
            <div>;
              <div className="text-3xl font-bold mb-2">;"
                {metrics.filter(m => { return m.changeType === 'decrease').length}'
              </div>; }
