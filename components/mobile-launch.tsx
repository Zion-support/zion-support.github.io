...
function StarIcon({ className = 'w-5 h-5 text-yellow-500' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"></svg>
...
function AppleBadge({ href }: { href: string }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-3 rounded-lg bg-black text-white px-4 py-2 shadow hover: opacity-90"></a>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden></svg>
...
function GoogleBadge({ href }: { href: string }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-3 rounded-lg bg-[#121212] text-white px-4 py-2 shadow hover: opacity-90"></a>
      <svg width="24" height="24" viewBox="0 0 24 24" aria-hidden></svg>
        <defs></defs>
          <linearGradient id="g1" x1="100%" y1="0%"></linearGradient>
            <stop offset="0%" stopColor="#00A0FF" />
            <stop offset="100%" stopColor="#00E3FF" />
          </linearGradient>
          <linearGradient id="g2" x1="100%" y1="0%"></linearGradient>
            <stop offset="0%" stopColor="#FFE000" />
            <stop offset="100%" stopColor="#FFA000" />
          </linearGradient>
          <linearGradient id="g3" x1="100%" y1="0%"></linearGradient>
            <stop offset="0%" stopColor="#FF3A44" />
            <stop offset="100%" stopColor="#C31162" />
          </linearGradient>
          <linearGradient id="g4" x1="100%" y1="0%"></linearGradient>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mx-auto max-w-5xl"></div>
        <div className="m-2 rounded-xl bg-white/95 dark:bg-black/80 border border-gray-200 dark:border-gray-800 shadow flex items-center gap-3 px-4 py-3"></div>
...
export default function MobileLaunchPage() {
  ...
  return (
    <EnhancedLayout></EnhancedLayout>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 text-white p-6 md:p-10"></section>
        <div className="grid md:grid-cols-2 gap-10 items-center"></div>
          ...
          <div className="flex items-center justify-center"></div>
            {/* Phone mockup */}
            <div className="relative h-96 w-48 md:h-[28rem] md:w-64 rounded-[2rem] bg-white/95 text-gray-900 shadow-2xl ring-4 ring-white/30"></div>
              <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 h-6 w-28 rounded-b-2xl bg-gray-200" />
              <div className="h-full w-full rounded-[2rem] overflow-hidden"></div>
                <div className="h-full w-full bg-gradient-to-b from-gray-50 to-gray-200 p-4 flex flex-col"></div>
                  <div className="text-xs font-semibold">Zion</div>
                  <div className="mt-2 flex-1 rounded-xl bg-white shadow-inner p-3"></div>
      ...
      {/* Key features */}
      <section className="mt-10 grid md:grid-cols-3 gap-6"></section>
        <div className="rounded-2xl border border-gray-200 dark:border-gray-800 p-6 bg-white dark:bg-gray-950"></div>
      ...
      <section className="mt-12"></section>
        ...
        <div className="relative mt-4 overflow-hidden"></div>
          <div className="flex transition-transform duration-700" style={{ transform: `translateX(-${idx * 100}%)`, width: `${testimonials.length * 100}%` }}></div>
            {testimonials.map((t) => (
              <div key={t.name} className="w-full md:w-1/3 flex-shrink-0 pr-4"></div>
                <div className="rounded-2xl border border-gray-200 dark:border-gray-800 p-5 bg-white dark:bg-gray-950 h-full"></div>
      ...
      {/* Email opt-in */}
      <section className="mt-12 rounded-3xl border border-gray-200 dark:border-gray-800 p-6 bg-white dark:bg-gray-950"></section>
        ...
        <form onSubmit={handleSubmit} className="mt-4 flex flex-col sm:flex-row gap-3"></form>
          <input
            type="email"
            required
            value={email
      ...
      {/* Helper links */}
      <section className="mt-10 text-sm opacity-80"></section>
