<<<<<<< HEAD
import React, { useEffect, useState, useCallback } from 'react';'import { motion, AnimatePresence } from 'framer-motion';'import { Activity, Zap, Shield, Globe, TrendingUp, AlertTriangle, CheckCircle } from 'lucide-react';''interface PerformanceMetrics {
=======
...
<<<<<<< HEAD
  showDetails = false, autoRefresh = true,
=======
  showDetails = false,
  autoRefresh = true,
>>>>>>> origin/cursor/install-dependencies-and-fix-errors-827a
...
<<<<<<< HEAD
            case 'paint': if (entry.name === 'first-contentful-paint') {'                metrics.fcp = Math.round(entry.startTime);'              }
              break;
            case 'largest-contentful-paint': metrics.lcp = Math.round(entry.startTime);'              break;'            case 'first-input': metrics.fid = Math.round(entry.processingStart - entry.startTime);'              break;'            case 'layout-shift': if (!(entry as any).hadRecentInput) {'                metrics.cls = (metrics.cls || 0) + (entry as any).value;'              }
=======
            case 'paint':
...
=======
      ...
      if (newMetrics.lcp > 4000) newAlerts.push('Largest Contentful Paint is slow');
      if (newMetrics.fid > 300) newAlerts.push('First Input Delay is high');
      if (newMetrics.cls > 0.25) newAlerts.push('Cumulative Layout Shift is high');
      if (newMetrics.score < 50) newAlerts.push('Overall performance score is low');
      ...
      <div className="fixed bottom-4 right-4 z-50">
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            {metrics && (
              <span className={`text-sm font-bold ${getScoreColor(metrics.score)}`}>
                {metrics.score}
>>>>>>> origin/cursor/install-dependencies-and-fix-errors-827a
              </span>
            )}
...
<<<<<<< HEAD
    <div className="bg-white dark: bg-gray-800 rounded-lg shadow-lg p-6 border border-gray-200 dark:border-gray-700">"      <div className="flex items-center justify-between mb-4">"        <h3 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center">"          <Activity className="w-5 h-5 mr-2 text-blue-500" />"          Performance Monitor"        </h3>
        <div className="flex items-center space-x-2">"          <button"            onClick={updateMetrics}
            disabled={isLoading}
            className="px-3 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50""          >"            {isLoading ? 'Updating...' : 'Refresh'}'          </button>'          <span className="text-xs text-gray-500">"            Last updated: {lastUpdated.toLocaleTimeString()}"          </span>
=======
...
<<<<<<< HEAD
        <div className="space-y-4">"          {/* Overall Score */}"          <div className="text-center">"            <div className={`inline-flex items-center justify-center w-20 h-20 rounded-full ${getScoreBgColor(metrics.score)}`}>"              <span className={`text-2xl font-bold ${getScoreColor(metrics.score)}`}>`                {metrics.score}
          ...
          {/* Additional Metrics */}
          <div className="grid grid-cols-3 gap-3">"            <div className="text-center">"              <div className="text-lg font-bold text-gray-900 dark: text-white">{metrics.ttfb}ms</div>"              <div className="text-xs text-gray-500">TTFB</div>"            </div>"            <div className="text-center">"              <div className="text-lg font-bold text-gray-900 dark:text-white">{metrics.fmp}ms</div>"              <div className="text-xs text-gray-500">FMP</div>"            </div>"            <div className="text-center">"              <div className="text-lg font-bold text-gray-900 dark:text-white">{metrics.tti}ms</div>"              <div className="text-xs text-gray-500">TTI</div>"            </div>"          </div>
=======
        <div className="space-y-4">
          {/* Overall Score */}
          <div className="text-center">
            <div className={`inline-flex items-center justify-center w-20 h-20 rounded-full ${getScoreBgColor(metrics.score)}`}>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center">
              <div className="text-lg font-bold text-gray-900 dark:text-white">{metrics.ttfb}ms</div>
              <div className="text-xs text-gray-500">TTFB</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-gray-900 dark:text-white">{metrics.fmp}ms</div>
              <div className="text-xs text-gray-500">FMP</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-gray-900 dark:text-white">{metrics.tti}ms</div>
...
<<<<<<< HEAD
                animate={{ opacity: 1, height: 'auto' }}'                exit={{ opacity: 0, height: 0 }}'                className="space-y-2""              >"                <h4 className="text-sm font-medium text-red-600 dark:text-red-400 flex items-center">"                  <AlertTriangle className="w-4 h-4 mr-1" />"                  Performance Alerts"                </h4>
=======
...
>>>>>>> origin/cursor/install-dependencies-and-fix-errors-827a
          ...
          {alerts.length === 0 && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
<<<<<<< HEAD
              className="flex items-center justify-center text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 p-3 rounded""            >"              <CheckCircle className="w-4 h-4 mr-2" />"              <span className="text-sm">All performance metrics are within acceptable ranges</span>"            </motion.div>"          )}
=======
