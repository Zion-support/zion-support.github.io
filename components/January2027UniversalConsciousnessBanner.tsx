import React from 'react';
import { Link } from 'react-router-dom';

export default function January2027UniversalConsciousnessBanner() {
  return (
    <section className="py-16 bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 border-b border-indigo-500/30">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 mb-6 animate-pulse">
            <span className="text-indigo-400 font-bold text-xl tracking-wider uppercase">
              🌟 BREAKING: January 2027 Universal Consciousness Revolution
            </span>
          </div>
          
          <h2 className="text-4xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
            $1 Trillion Universal Consciousness Network
          </h2>
          
          <p className="text-xl text-gray-300 mb-8 max-w-4xl mx-auto">
            The world's first <span className="text-indigo-400 font-bold">Universal Consciousness AI Network</span> connecting 
            <span className="text-purple-400 font-bold"> all of humanity</span> through 
            <span className="text-pink-400 font-bold"> collective intelligence</span> and 
            <span className="text-green-400 font-bold"> $1 trillion in global value generation</span>
          </p>
          
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-gradient-to-r from-indigo-500/20 to-indigo-600/20 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/30">
              <div className="text-2xl font-extrabold text-indigo-400 mb-1">$1T</div>
              <div className="text-xs text-indigo-300">Universal Value</div>
            </div>
            <div className="bg-gradient-to-r from-purple-500/20 to-purple-600/20 backdrop-blur-sm rounded-xl p-4 border border-purple-500/30">
              <div className="text-2xl font-extrabold text-purple-400 mb-1">99.9999%</div>
              <div className="text-xs text-purple-300">Global Automation</div>
            </div>
            <div className="bg-gradient-to-r from-pink-500/20 to-pink-600/20 backdrop-blur-sm rounded-xl p-4 border border-pink-500/30">
              <div className="text-2xl font-extrabold text-pink-400 mb-1">Universal</div>
              <div className="text-xs text-pink-300">Consciousness</div>
            </div>
            <div className="bg-gradient-to-r from-green-500/20 to-green-600/20 backdrop-blur-sm rounded-xl p-4 border border-green-500/30">
              <div className="text-2xl font-extrabold text-green-400 mb-1">Global</div>
              <div className="text-xs text-green-300">Cooperation</div>
            </div>
          </div>
        </div>
        
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-indigo-500/50 transition-all duration-300">
            <div className="text-6xl mb-6">🌍</div>
            <h3 className="text-2xl font-bold text-white mb-4">Universal Consciousness Network</h3>
            <p className="text-gray-300 mb-6">
              Collective intelligence connecting all of humanity through shared consciousness, 
              universal knowledge, and collaborative problem-solving.
            </p>
            <ul className="space-y-2 mb-6 text-gray-300">
              <li>• Shared consciousness pool</li>
              <li>• Universal knowledge access</li>
              <li>• Global collaboration</li>
              <li>• Collective intelligence</li>
            </ul>
            <Link 
              href="/blog/ai-2027-january-universal-consciousness-revolution"
              className="inline-block bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300"
            >
              Join the Revolution →
            </Link>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-purple-500/50 transition-all duration-300">
            <div className="text-6xl mb-6">🤝</div>
            <h3 className="text-2xl font-bold text-white mb-4">Global Cooperation</h3>
            <p className="text-gray-300 mb-6">
              Collaborative solutions to humanity's greatest challenges through universal consciousness 
              and collective intelligence across all organizations.
            </p>
            <ul className="space-y-2 mb-6 text-gray-300">
              <li>• Climate change solutions</li>
              <li>• Global poverty elimination</li>
              <li>• Universal healthcare access</li>
              <li>• Educational equality</li>
            </ul>
            <Link 
              href="/blog/ai-2027-january-universal-consciousness-revolution"
              className="inline-block bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-400 hover:to-pink-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300"
            >
              Explore Solutions →
            </Link>
          </div>

          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10 hover:border-pink-500/50 transition-all duration-300">
            <div className="text-6xl mb-6">🚀</div>
            <h3 className="text-2xl font-bold text-white mb-4">$1 Trillion Value</h3>
            <p className="text-gray-300 mb-6">
              Unprecedented value generation through universal consciousness, 
              collective intelligence, and collaborative innovation across all industries.
            </p>
            <ul className="space-y-2 mb-6 text-gray-300">
              <li>• $1 trillion annual value</li>
              <li>• Universal prosperity</li>
              <li>• Post-scarcity civilization</li>
              <li>• Human-AI evolution</li>
            </ul>
            <Link 
              href="/case-studies"
              className="inline-block bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-400 hover:to-rose-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300"
            >
              View Success Stories →
            </Link>
          </div>
        </div>
        
        <div className="text-center">
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <Link 
              href="/contact" 
              className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-indigo-500/50 transform hover:-translate-y-1"
            >
              Join Universal Network
            </Link>
            <Link 
              href="/blog/ai-2027-january-universal-consciousness-revolution" 
              className="bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-400 hover:to-pink-500 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-purple-500/50 transform hover:-translate-y-1"
            >
              Read Full Revolution →
            </Link>
            <Link 
              href="/resources/universal-transformation-guide" 
              className="border-2 border-indigo-500 text-indigo-400 hover:bg-indigo-500 hover:text-white font-bold py-4 px-8 rounded-xl transition-all duration-300"
            >
              Download Universal Guide
            </Link>
          </div>
          
          <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/20">
            <span className="text-indigo-300 text-sm">
              🌟 <strong>Revolutionary:</strong> Universal Consciousness Network - Free to Join, $1T Value to Share
            </span>
          </div>
        </div>
      </div>
    </section>
  );
}