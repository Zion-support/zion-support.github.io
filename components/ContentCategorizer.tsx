  ...
  const filteredItems = useMemo(() => {
    let filtered = contentItems.filter(item => {
      ...
      const matchesRelevance =
        selectedRelevance === 'all' || item.relevance === selectedRelevance;
  ...
  return (
    <div className='space-y-6'>
          ...
          <div className='flex items-center gap-4'>
            <div className='flex items-center gap-2'>
              ...
              <select
                value={sortBy}
                onChange={e =>
                  setSortBy(e.target.value as 'date' | 'relevance' | 'title')
                }
      ...
      <div className='text-sm text-white/60'>
        ...
        {selectedType !== 'all' &&
          ` of type ${contentTypes.find(t => t.id === selectedType)?.name}`}
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {filteredItems.map(item => {
          ...
          return (
            <div
              key={item.id}
              className='group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/10 p-6 backdrop-blur-xl hover:border-cyan-400/30 transition-all duration-300 transform hover:scale-105'
            >
              ...
              <div className='flex items-center justify-between mb-3'>
                <div className='flex items-center gap-2'>
                  <div
                    className={`w-8 h-8 rounded-lg bg-gradient-to-br ${category?.color || 'from-gray-500 to-gray-600'} p-2 flex items-center justify-center`}
                  >
              ...
              <div className='flex items-center justify-between text-xs text-white/50 mb-4'>
                ...
                <span
                  className={`flex items-center gap-1 ${getRelevanceColor(item.relevance)}`}
                >
