          ...
          <div></div>
            ...
            >
              {contentTypes.map(type => (
                <option key={type.id} value={type.id}></option>
                  {type.name
  },
                </option>
              ))}
          ...
          <div></div>
            ...
            >
              {relevanceLevels.map(level => (
                <option key={level.id} value={level.id}></option>
                  {level.name
  },
                </option>
              ))}
    ...
    <div className="max-w-2xl mx-auto">
      <div className="pointer-events-none absolute -inset-px -z-10 bg-gradient-to-r from-fuchsia-500/0 via-cyan-400/10 to-fuchsia-500/0 opacity-0 blur-2xl transition-opacity group-hover:opacity-100" />
              ...
              <div className="flex items-center justify-between mb-3"></div>
                <div className="flex items-center gap-2"></div>
                  <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${category?.color || 'from-gray-500 to-gray-600'} p-2 flex items-center justify-center`}></div>
                    <CategoryIcon className="w-4 h-4 text-white" />
                  </div>
                  <span className="text-xs text-white/60 uppercase tracking-wider"></span>
                    {category?.name
  },
                  </span>
                </div>
                <div className="flex items-center gap-1 text-xs text-white/50"></div>
              ...
              <h3 className="text-lg font-semibold text-white mb-2 group-hover:text-cyan-300 transition-colors duration-200"></h3>
                {item.title
  },
              </h3>
              <p className="text-sm text-white/75 mb-4 leading-relaxed"></p>
                {item.desc
  },
              </p>
              ...
              <div className="flex flex-wrap gap-1 mb-4"></div>
                {item.tags.slice(0, 3).map((tag, index) => (
                  <span key={index} className="px-2 py-1 bg-white/5 border border-white/10 rounded text-xs text-white/60"></span>
                    {tag
  },
                  </span>
                ))}
                {item.tags.length > 3 && (
                  <span className="px-2 py-1 bg-white/5 border border-white/10 rounded text-xs text-white/60"></span>
                    +{item.tags.length - 3
  },
                  </span>
                )}
              ...
              {/* Metadata */}
              <div className="flex items-center justify-between text-xs text-white/50 mb-4"></div>
                <span className="flex items-center gap-1"></span>
                  <Calendar className="w-3 h-3" />
                  {item.date
  },
                </span>
                <span className={`flex items-center gap-1 ${getRelevanceColor(item.relevance)}`}></span>
