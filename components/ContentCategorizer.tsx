...
<div>;
            <label className=&quot;block text - sm font - medium text - white / 70 mb - 2&quot;>Content Type</label>;<select;
              ...
              className=&quot;w - full px - 3 py - 2 bg - white / 5 border border - white / 10 rounded - lg text - white focus:outline - none focus:ring - 2 focus:ring - cyan - 500 / 50 focus:border - cyan - 500 / 50 transition - all duration - 200 & quot;
            >;
              {content_types.map (type => (<option key={type.id} value={type.id}>;{type.name}
                </option>))}
            </select>;
          </div>;
          {/* Relevance Filter */}<div>;
            <label className=&quot;block text - sm font - medium text - white / 70 mb - 2&quot;>Relevance</label>;<select;
              ...
              className=&quot;w - full px - 3 py - 2 bg - white / 5 border border - white / 10 rounded - lg text - white focus:outline - none focus:ring - 2 focus:ring - cyan - 500 / 50 focus:border - cyan - 500 / 50 transition - all duration - 200 & quot;
            >;
              {relevance_levels.map (level => (<option key={level.id} value={level.id}>;{level.name}
                </option>))}
            </select>;
          </div>;
        </div>;
        {/* Sort Controls and Clear Filters */}<div className=&quot;flex flex - wrap items - center justify - between gap - 4&quot;>;
      ...
      <div>;
        <div className=&quot;flex flex - wrap gap - 2&quot;>;
          {categories.map (category => (<button;key={category.id}
              ...
              {category.name}
              <span className=&quot;text - xs bg - white / 10 px - 2 py - 1 rounded - full & quot;>;{category.count}
              </span>;
            </button>))}
        </div>;
      </div>;
...
<div className=&quot;flex flex - wrap gap - 1 mb - 4&quot;>;
                {item.tags.slice (0, 3).map ((tag, index) => (<span key={index} className=&quot;px - 2 py - 1 bg - white / 5 border border - white / 10 rounded text - xs text - white / 60 & quot;>;
                    {tag}
                  </span>))}
{item.tags.length > 3 && (<span className=&quot;px - 2 py - 1 bg - white / 5 border border - white / 10 rounded text - xs text - white / 60 & quot;>;+{item.tags.length - 3}
                  </span>)}
              </div>;
              {/* Metadata */}<div className=&quot;flex items - center justify - between text - xs text - white / 50 mb - 4&quot;>;
