...
export default function HolographicAICommunicationPage() {_const _contactInfo = {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
            <div className="lg:col-span-2 bg-black/30 rounded-2xl border border-fuchsia-500/30 p-6">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {_features.map((feat) => (
                  <li key={feat} className="flex items-start space-x-3 text-slate-200"><Check className="w-5 h-5 text-fuchsia-400 mt-0.5" /><span>{_feat}</span></li>
                ))}
              </ul>
            </div>
