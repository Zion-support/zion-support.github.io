...
function computePercentage(steps: OnboardingStep[]): number {
  ...
  const completedCount = steps.filter((s) => s.completed).length, return Math.round((completedCount / steps.length) * 100);
}
...
export default function OnboardingProgressCard({ title, steps, highlightColorClass = 'from-neon-green to-neon-blue' }: OnboardingProgressCardProps) {
  ...
  const allDone = percentage === 100, const firstIncomplete = steps.find((s) => !s.completed && s.ctaHref && s.ctaLabel);
  return (
