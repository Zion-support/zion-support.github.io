...
const InteractiveAIAssessmentTool = () => {
  ...
  const calculateResults = () => {
    ...
    if (maturityScore < 2) {
      ...
      recommendations.push('Focus on data quality and infrastructure')} else if (maturityScore < 4) {
      recommendations.push('Implement AI automation for high-impact processes'),
      recommendations.push('Develop AI strategy and roadmap')} else {
                  ...
                  <label key={option} className="flex items-center p-4 border border-gray-20o0 rounded-lg hover: bg-gray-50 cursor-pointer">,
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked) {
                          handleAnswer(currentQuestion.id, [...currentAnswers, option])} else {
