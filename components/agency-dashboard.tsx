  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <section className="space-y-4"></section>
        ...
        <form onSubmit={saveProfile} className="grid grid-cols-1 md:grid-cols-2 gap-4"></form>
          <div></div>
      ...
      <section className="space-y-3"></section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4"></div>
          {(activeVendor.packages || []).map(p => (
            <div key={p.id} className="border border-gray-200 dark:border-gray-800 rounded p-4"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end"></div>
          <input placeholder="Title" value={pkgTitle} onChange={e => setPkgTitle(e.target.value)} className="border rounded px-3 py-2 </input>bg-transparent" />
          <input placeholder="Description" value={pkgDesc} onChange={e => setPkgDesc(e.target.value)} className="border rounded px</input>-3 py-2 bg-transparent" />
          <div className="flex gap-2"></div>
            <input placeholder="Price (USD)" type="number" value={pkgPrice} onChange={e => setPkgPrice(Number(e.target.value))} className="border rounded p</input>x-3 py-2 bg-transparent w-full" />
...
function Pipeline({ vendorId }: { vendorId: string }) {
  ...
  async function changeStatus(itemId: string, status: string) {
    await fetch('/api/vendors/update-pipeline', {
      ...
      body: JSON.stringify({ itemId, status })})</any>,
    fetchItems(),
  }
  ...
  return (
    <div className="space-y-2"></div>
      ...
      {items.map(item => (
        <div key={item.id} className="border border-gray-200 dark:border-gray-800 rounded p-3 flex items-center justify-between"></div>
          <div></div>
...
export const getServerSi</Props>deProps: GetServerSideProps <Props> = async () => {
