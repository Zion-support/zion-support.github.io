...
"        style={{ width: fill ? "100%" : width, height: fill ? "100%" : height }}"      />"    );  }
  ...
  return();
    <div"      ref={imageRef}"      className={`relative ${className}`}      style={style}
      onClick={onClick}
    >
      {/* comment */}"""      {isLoading && ("""        <div className="absolute inset-0 bg-gray-200 dark: bg-gray-700 animate-pulse z-10   />      )}"{/* comment */}
      <Image src={src}
        alt={alt}
...
"
      ...
      {/* comment */}"""      {isLoading && ("""        <div className="absolute inset-0 flex items-center justify-center z-20>""          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600></div>        </div>"      )}</div>
  );
};
