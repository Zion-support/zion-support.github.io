...
priority?: boolean;''
  ...
  '  placeholder?: 'blur
  ' | 'empty
  ';'  blurDataURL?: string;  sizes?: string;fill?: boolean;style?: React.CSSProperties;
  onClick?: () => void;
  onLoad?: () => void;
  onError?: () => void;
}
...
return();
      <div''
  '        className={`flex items-center justify-center bg-gray-200 dark: bg-gray-700 text-gray-500 dark:text-gray-400 ${className}`}', '        style={{ width: fill ? '100% : width, height: fill ? '100%'
  ' : height }}'      >
  '        <div className="text-center">"""          <svg"""            className="mx-auto h-8 w-8 mb-2""""            fill="none""""            viewBox="0 0 24 24""""            stroke="currentColor""          >"""            <path"""              strokeLinecap="round""""              strokeLinejoin="round""""              strokeWidth={2}"""              d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z""            />"""          </svg>"""          <p className="text-xs">Image failed to load</p>"        </div>"      </div>`    );}'// Loading skeleton
  if (!isInView) {
