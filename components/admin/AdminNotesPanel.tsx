...
const res = await fetch(},,
  ...
  text })} finally {setAdding(false)}      if (!res.ok) {method: 'POST'}
  headers: {'Content-Type': 'application/json','X-Admin': isAdmin ? 'true' : 'false','X-Admin-User': adminId},body: JSON.stringify({ targetType, targetId,,,
  text })})if (!res.ok) {alert('Failed to add note';}
  return;}
      }
<<<<<<< HEAD
,,
    ...
    <div className='rounded border p-4 space-y-3' />;
      <div className='flex items-center justify-between' />;
        ...
        <textarea className='w-full border rounded-md px-3 py-2'
            rows={3
            placeholder='Write a private note (abuse}
            spam
            special support)'
            value={text</div />
            </div>
...
<<<<<<< HEAD
      <div className='space - y-2' />;
        ...
        />
            <button disabled={!text.trim () || adding
            on_click={add_note
            className='px - 3 py - 2 rounded - md bg - gray - 900 text - white disabled: opacity - 50' />
...
<<<<<<< HEAD
      <div className='border - t pt - 3' />;
        <div className='text - sm opacity - 70 mb - 2' />;
          Notes are private, time - stamped
            and include author ID.;
        </div>;
        {loading ? (
...
<<<<<<< HEAD
    ...
    <div className=&quot;rounded border p-4 space-y-3&quot />
=======
...
<<<<<<< HEAD
  ...
  onChange={(e) = /> setIsAdmin(e.target.checked) />;
          ...
          <input className=&quot;border rounded px-2 py-1&quot; value={adminId,
  onChange={(e) = /> setAdminId(e.target.value) placeholder=&quot;Admin ID&quot; />;
        </div>;
      </div>}
            <div className=&quot;space-y-2&quot />

        <textarea className=&quot;w-full border rounded-md px-3 py-2&quot; rows={3,
  placeholder=&quot;Write a private note (abuse, spam, special support)&quot; value={text,
  onChange={(e) = /> setText(e.target.value) />
            <button disabled={!text.trim() || adding
            onClick={addNote
            className=&quot;px-3 py-2 rounded-md bg-gray-900 text-white disabled:opacity-50&quot; />{adding ? 'Adding…' : 'Add Note'</button>;
=======
            <input type=&quot;checkbox&quot; checked={isAdmin} onChange={(e) =    /> setIsAdmin(e.target.checked)} />;
            <span    />Admin</span>;
          </label>;
...
<<<<<<< HEAD
        <div className=&quot;text-sm opacity-70 mb-2&quot; />Notes are private, time-stamped
            and include author ID.</div>;
        {loading ? (
          <div className=&quot;text-sm&quot; />Loading…</div>;
        ) : notes.length === 0 ? (
          <div className=&quot;text-sm opacity-70&quot; />No notes yet.</div>;
          ...
          <ul className=&quot
            ...
            {notes.map((n) => (
          <li key={n.id
            className=&quot;rounded border p-2 text-sm&quot />

                <div className=&quot
            opacity-60 text-xs mb-1&quot
            />{new Date(n.createdAt).toLocaleString() • {n.authorId</div>}
            <div />{n.text</div>;
          onChange={e => setText(e.target.value)}
        />;
        <button disabled={!text.trim() |adding}
