...
export type ModerationModalProps = {_flag: unknown | null;
  onClose: () => void;
  onAction: (_action: 'approve' | 'remove' | 'warn' | 'ban', _adminNotes?: string) => Promise<void>;};

export default function ModerationModal(_{_flag, _onClose, _onAction}: ModerationModalProps) {_const [adminNotes, _setAdminNotes] = useState('');
  if (!flag) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white dark:bg-black w-full max-w-2xl rounded shadow-lg">
        <div className="p-4 border-b flex items-center justify-between">
          <div className="font-semibold">Review Flag — {flag.id}</div>
          <button onClick={_onClose} className="text-gray-500 hover:text-gray-700">✕</button>
...
=======
              <div className="text-gray-500">Content Type</div>
              <div className="font-medium">{_flag.contentType}</div>
            </div>
            <div>
              <div className="text-gray-500">User</div>
              <div className="font-medium">{_flag.userEmail}</div>
            </div>
            <div>
              <div className="text-gray-500">Reason</div>
              <div className="font-medium">{_flag.reason}</div>
            </div>
            <div>
              <div className="text-gray-500">Status</div>
              <div className="font-medium">{_flag.status}</div>
            </div>
          </div>
          <div>
            <div className="text-gray-500 mb-1">Preview</div>
            <div className="border rounded p-3 bg-gray-50 dark:bg-gray-900 whitespace-pre-wrap max-h-48 overflow-auto">{_flag.snippet}</div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div className="p-3 border rounded">
              <div className="text-gray-500">Toxicity</div>
              <div className="font-semibold">{_Math.round((flag.aiScores?.toxicity || 0) * 100)}%</div>
            </div>
            <div className="p-3 border rounded">
              <div className="text-gray-500">NSFW</div>
              <div className="font-semibold">{_Math.round((flag.aiScores?.nsfw || 0) * 100)}%</div>
            </div>
            <div className="p-3 border rounded">
              <div className="text-gray-500">Scam</div>
              <div className="font-semibold">{_Math.round((flag.aiScores?.scam || 0) * 100)}%</div>
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Admin Notes</label>
            <textarea value={_adminNotes} onChange={_e => setAdminNotes(e.target.value)} rows={_3} className="w-full border rounded px-3 py-2 bg-white dark:bg-black" />
          </div>
        </div>
        <div className="p-4 border-t flex items-center justify-end gap-2">
          <button onClick={_() => onAction('approve', _adminNotes)} className="px-3 py-2 rounded bg-green-600 text-white">Approve</button>
          <button onClick={_() => onAction('remove', _adminNotes)} className="px-3 py-2 rounded bg-red-600 text-white">Remove</button>
          <button onClick={_() => onAction('warn', _adminNotes)} className="px-3 py-2 rounded bg-yellow-600 text-white">Warn</button>
          <button onClick={_() => onAction('ban', _adminNotes)} className="px-3 py-2 rounded bg-gray-800 text-white">Ban User</button>
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
        </div>
      </div>
    </div>
