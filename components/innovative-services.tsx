...
const [sortBy, setSortBy] = useState('innovation')
  ...
  // Category filter;}
  if($2) {filteredServices = getInnovativeServicesByCategory(selectedCategory)}// Price range filter
  if($2) {const [min, max] = priceRange;}
      .split('-').map(p => (p === '+' ? Infinity : parseInt(p)))filteredServices = getInnovativeServicesByPriceRange(min, max)}
  // Search filter
  if($2) {filteredServices = filteredServices.filter(service =>
