...
} />;<meta property='og:image' content='/images / og / topic - default.jpg' />;<meta property='og:image' content='/images / og / topic - default.jpg' />;<meta property='og:image' content='/images / og / topic - default.jpg' />;
...
},body: JSON && JSON.stringify({url: window && window.location.href,title: `${topi,
} - Zion Blog`,network,utm:;
                    'utm_source=' +;
                    network +;
      ...
      <div className='mx - auto max - w-6xl'>;
        ...
        <div className='mb - 6'>;
          <PageShareButtons;
            ...
            on_share={network =>;
              fetch ('/api / analytics / share', {method: 'POST',headers: { 'Content - Type': 'application / json',
},body: JSON.stringify ({url: window.location.href,title: `${topi,
} - Zion Blog`,network,utm:;
                    'utm_source=' +;
                    network +;
                    '&utm_medium = share & utm_campaign = category';
...
} - Zion Blog`, network, utm: 'utm_source=' + network + '&utm_medium=share&utm_campaign=category' }) }).catch(() => {},
}
          />;
        </div>;
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">;
...
export default TopicPage;      </Head>;
          ...
          <PageShareButtons
            ...
            onShare={(network) => fetch('/api/analytics/share', { method: 'POST', headers: { 'Content-Type': 'application/json',
}, body: JSON && JSON.stringify({ url: window && window.location.href, title: `${topi,
} - Zion Blog`, network, utm: 'utm_source=' + network + '&utm_medium=share&utm_campaign=category' }) }).catch(() => {},
}
          />;
      ...
      <div className=&quot;mx-auto max-w-6xl&quot;>
        ...
        <div className=&quot;mb-6&quot;>
          <PageShareButtons
            ...
            onShare={_(_network) => fetch('/api/analytics/share', _{ method: 'POST', _headers: { 'Content-Type': 'application/json,
}, body: JSON.stringify({_url: window.location.href, _title: `${topi,
} - Zion Blog`, network, utm: 'utm_source=' + network + '&utm_medium=share&utm_campaign=category' }) }).catch__(() => {},
}
          />;
        </div>;
        <div className=&quot;grid grid-cols-1 md:grid-cols-3 gap-6&quot>

import type { GetServerSideProps, NextPage } from 'next',type Props = { topic: string; posts: BlogPost[],
}
...
export const getServerSideProps: GetServerSideProps = async ctx => {const topic = String(ctx.params?.topic || '')const posts = listPublishedPosts().filter(p => p.topics.includes(topic))const posts = listPublishedPosts().filter(p => p.topics.includes(topic);
  return { props: { topic, posts } };
}
