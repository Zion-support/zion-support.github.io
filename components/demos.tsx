...
export default function DemosPage() {} from 'lucide-react;
...
import { Mail, Lock, Link, FileText, CheckCircle, Zap, BarChart3, Code, Play   } from lucide-react';
export default function DemosPage() {const [activeDemo, setActiveDemo] = useState<string>('email-validator)const demos  = null;export default function DemosPage() {const [activeDemo, setActiveDemo] = useState<string>(email-validator')const demos = [;
    {id: 'email-validator,title: Email Validator Pro',description:;
        'Advanced email validation with domain checking and deliverability scoring,icon: <Mail className=w-6 h-6' />,color: 'from-blue-500 to-cyan-600,component: <EmailValidatorDemo />},{id: password-strength',title: 'Password Strength Analyzer,description:;
        Comprehensive password security analysis with entropy calculation',icon: <Lock className='w-6 h-6 />,color: from-green-500 to-emerald-600',component: <PasswordStrengthDemo />},{id: 'url-shortener,title: Smart URL Shortener',description:;
        'Professional URL shortening with analytics and custom codes,icon: <Link className=w-6 h-6' />,color: 'from-purple-500 to-pink-600,component: <UrlShortenerDemo />},{id: text-analyzer',title: 'Text Analysis Suite,description:;
        Advanced text analysis with readability and sentiment analysis',icon: <FileText className='w-6 h-6 />,color: from-orange-500 to-red-600',component: <TextAnalyzerDemo />}];
  const activeDemoData  = demos.find(demo => demo.id === activeDemo)export default function DemosPage() {const [activeDemo, setActiveDemo]  = useState<string>('email-validator)<>;
      <Head>;
        <title>Interactive Demos | Zion Tech Group - Test Our Micro SaaS Services;
        </title>;return (<>;
      <Head>;
<title>Interactive Demos | Zion Tech Group - Test Our Micro SaaS Services;
        </title>;
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4>;
            {demos.map(demo => (<button;
              ...
              >;
                ...
                {activeDemo === demo && demo.id && (<div className=mt-3 flex items-center text-blue-400 text-sm'>;
...
<section className='py-16 bg-black relative overflow-hidden>;
      ...
      {/* Features */}              Live Demo;
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4>;{activeDemoData?.title}
            </h2>;
            <p className=text-lg text-gray-400 max-w-2xl mx-auto">;
              {activeDemoData?.description}
              {activeDemoData?.title}
