...
const handleKeyDown = ("e": React.KeyboardEvent) =>: any {// Check condition;
}
if ( {) {$2;
}
...
const handleRecentClick = ("search": string) =>: any {set_query (search
}// Check condition;
if (return null) {$2;}
}
  return (<div className="fixed inset - 0 z-50 overflow-y-auto">;"
          ...
          <div className="max-h-96 overflow-y-auto">;"
            {query.trim () ? (results.length > 0 ? (<div className="p-2">;"
                  {results.map ((result, index) => (<div;
                      ...
                      key={index}
                      className={`p - 3 rounded - lg cursor - pointer transition - colors ${index === selected_index;`                          ? "bg - blue - 50 border border - blue - 200";"
                          : ""hover":bg - gray - 50";"
                      }`}`
                      <button;
                        key={term}
