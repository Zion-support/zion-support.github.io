...
const searchData: SearchResult[] = [;
  ...
  const [query, setQuery] = useState("")const [results, setResults] = useState<SearchResult[]>([])const [recentSearches, setRecentSearches] = useState<string[]>([])const [selectedIndex, setSelectedIndex] = useState(0)const inputRef = useRef<HTMLInputElement>(null))setResults(filtered)} else {setResults([])export default /**;
 * SearchModal - Function description;
 */;
function SearchModal() {const [query, set_query] = useState ("")const [results, set_results] = useState < SearchResult[]>([])const [recent_searches, setRecentSearches] = useState < string[]>([])const [selected_index, setSelectedIndex] = useState (0)const input_ref  = useRef < HTMLInputElement>(null)useEffect (() => {// Check condition;
if ( {) {$2;
}
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-lg shadow-xl">;
          ...
          <div className="max-h-96 overflow-y-auto">;
            {query && query.trim() ? (results && results.length > 0 ? (<div className="p-2">;
                  {results && results.map((result, index) => (<div;const handleKeyDown = (e: React.KeyboardEvent) =>: any {// Check condition;
if ( {) {$2;
}
      on_close ()} else // Check condition;
if ( {) {$2;
}
      e.prevent_default ()setSelectedIndex ((prev) => Math.min (prev + 1, results.length - 1))} else // Check condition;
if ( {) {$2;
}
      e.prevent_default ()setSelectedIndex ((prev) => Math.max (prev - 1, 0))} else // Check condition;
if ( {) {$2;
}
      handleResultClick (results[selected_index])}
  }const handleResultClick = (result: SearchResult) =>: any {// Add to recent searches;
    const new_recent = [;
      result.title,...recent_searches.filter ((s) => s !== result.title),].slice (0, 5)setRecentSearches (new_recent)local_storage.set_item ("recent - searches", JSON.stringify (new_recent))// Navigate to result;
    window.location.href = result.url;
  }const handleRecentClick = (search: string) =>: any {set_query (search)}// Check condition;
if (return null) {$2;
}
  return (<div className="fixed inset - 0 z-50 overflow-y-auto">;
      <div className="flex min - h-screen items - start justify - center p-4 pt-16">;
        ...
        <div className="relative w - full max - w-2xl bg - white rounded-lg shadow-xl">;
          ...
          <div className="max-h-96 overflow-y-auto">;
            {query.trim () ? (results.length > 0 ? (<div className="p-2">;
                ...
                {recent_searches.length > 0 && (<div className="mb-6">;
                    <div className="flex items - center text - sm text - gray-500 mb-3">;
                      <Clock className="w - 4 h-4 mr-2" />;
                      Recent Searches;
                    </div>;
                    <div className="space-y-2">;
                      {recent_searches.map ((search, index) => (<button;
                          key={index}
                ...
                <div>;
                  ...
                  <div className="grid grid - cols-2 gap-2">;
                    {[;
                      "AI Services","Cloud Solutions","Cybersecurity","Digital Transformation",].map ((term) => (<button;
                        key={term}
