import React, { useEffect, useState } from 'react';''interface AccessibilityEnhancerProps {
  children: React.ReactNode;
}

const AccessibilityEnhancer: React.FC<AccessibilityEnhancerProps> = ({ children }) => {
  const [isHighContrast, setIsHighContrast] = useState(false);
  const [fontSize, setFontSize] = useState('normal');'  const [reducedMotion, setReducedMotion] = useState(false);'
  useEffect(() => {
    ...
    const savedHighContrast = localStorage.getItem('highContrast') === 'true';'    const savedFontSize = localStorage.getItem('fontSize') || 'normal';'    '    setIsHighContrast(savedHighContrast);
  ...
  const applyAccessibilityStyles = (highContrast: boolean, fontSize: string, reducedMotion: boolean) => {;
    ...
    root.classList.remove('font-small', 'font-normal', 'font-large', 'font-extra-large');'    root.classList.add(`font-${fontSize}`);'`    // Reduced motion
    if (reducedMotion) {
  ...
  const toggleHighContrast = () => {;
    ...
    localStorage.setItem('highContrast', newValue.toString());'    applyAccessibilityStyles(newValue, fontSize, reducedMotion);'  };

  const changeFontSize = (newSize: string) => {;
    setFontSize(newSize);
    localStorage.setItem('fontSize', newSize);'    applyAccessibilityStyles(isHighContrast, newSize, reducedMotion);'  };
  ...
  return (
    <>
        ...
        <div className="space-y-2">"          <button"            onClick={toggleHighContrast}
          ...
          <div className="flex gap-1">"            {['small', 'normal', 'large', 'extra-large'].map((size) => ('              <button'                key={size}
                ...
                className={`px-2 py-1 text-xs rounded ${`                  fontSize === size
                    ? 'bg-blue-600 text-white''                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300''                }`}'                aria-label={`Set font size to ${size}`}`              >
      ...
      <div className="sr-only">"        <h1>Zion Tech Group - Technology Solutions Provider</h1>"        <p>
      ...
      <style jsx global>{``        /* High contrast mode */
        .high-contrast {
          --tw-bg-opacity: 1;
          --tw-text-opacity: 1;
        }
