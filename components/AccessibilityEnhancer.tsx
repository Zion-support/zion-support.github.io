...
const AccessibilityEnhancer: React.FC<AccessibilityEnhancerProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const savedFontSize = (typeof window !== 'undefined' && (localStorage.getItem('fontSize') as 'small' | 'normal' | 'large' | 'extra-large')) || 'normal';

    setIsHighContrast(Boolean(savedHighContrast));
    setFontSize(savedFontSize);
  }, []);

  const applyAccessibilityStyles = (highContrast: boolean, size: 'small' | 'normal' | 'large' | 'extra-large', motionReduced: boolean) => {
  ...
  const toggleHighContrast = () => {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('highContrast', newValue.toString());
  ...
  return (
    <>
      ...
      <div className="sr-only">
        ...
        <p>
          Leading technology solutions provider helping businesses transform their digital presence with cutting-edge AI, quantum computing, blockchain infrastructure, and innovative development services.
        </p>
      </div>
