...
import React, { useEffect } from, react'';const SecurityHeaders: React.FC = () => {
  useEffect(() => {
    ...
    const addSecurityHeaders = () => {
      const securityMetaTags = [
        { name: 'referrer, content:, strict-origin-when-cross-origin
  ' },'        { name: 'x-content-type-options, content:, nosniff' }, '        { name: 'x-frame-options, content:, DENY
  ' },'        { name: 'x-xss-protection, content:, 1 mode=block' }, { name: 'permissions-policy, content:, camera=(), microphone=(), geolocation=(), interest-cohort=()' }, { name: 'content-security-policy, content: 'default-src, self' script-src
  'self'
  'unsafe-inline'
