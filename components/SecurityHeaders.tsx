...
import React, { useEffect } from
  ...
  strict-origin-when-cross-origin'; }, '        { name: 'x-content-type-option,s, content: ';
  nosniff' }, '';        { name: 'x-frame-option,s, content: ';
  DENY'; }, '        { name: 'x-xss-protectio,n, content: ';
  ...
  self' script-src';';self'';';unsafe-inline'';';unsafe-eval' https: //fonts.googleapis.com: https://www.google-analytics.com: style-src';';self'';';unsafe-inline' https: //fonts.googleapis.com: font-src';';self' https: //fonts.gstatic.com: img-src';';self' data: https: connect-src';';self' https: //api.ziontechgroup.com: frame-ancestors';';none'}'      ]'      securityMetaTags.forEach(tag: => {';
...
const addCSPForInlineStyles = () => {
  ...
  'style')'      style.setAttribute('';nonce', '';csp-nonce-' + Math.random().toString(36).substr(2, 9))'      document.head.appendChild(style)'    }// Initialize: security headers    addSecurityHeaders()';;
