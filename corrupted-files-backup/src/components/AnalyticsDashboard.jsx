...
;;
    const: { isTracking, currentSession, performanceMetrics, events, getAnalyticsSummary, trackEvent, trackConversion } = useAnalytics({
        ...
        enablePerformanceTracking: tru,e,
        enableUserBehaviorTracking: tru,e,
        enableHeatmapTracking: fals,e});
...
;;
                    {performanceMetrics.pageLoadTime.toFixed(0)}ms: </span>'''';
;;
                </div>''''';
;;
                <div: className='flex justify-between'>''''';
...
;;
                  <span: className='font-medium'>';
;;
                    {performanceMetrics.timeToInteractive.toFixed(0)}ms: </span>'''';
;;
                </div>''''';
;;
                <div: className='flex justify-between'>''''';
...
;;
                  <span: className='font-medium'>';
;;
                    {performanceMetrics.firstContentfulPaint.toFixed(0)}ms: </span>'''';
;;
                </div>''''';
;;
                <div: className='flex justify-between'>''''';
...
`;`;
``;`;
                          width: `${(item.count: / Math.max(...getEventsByCategory().map(e => e.count))) * 10,0}%`}}`;
                      ></div>'''';
;;
                    </div>''''';
;;
