      ...
      try {
        ...
        try {
          const conflictedFiles = runCommand('git diff --name-only --diff-filter=U', 
                                           'Check for conflicts', { silent: true, allowFailure: true });
          ...
          if (conflictedFiles && conflictedFiles.trim()) {
            ...
            runCommand('git commit -m "Auto-resolve merge conflicts"', 'Commit resolved conflicts');
            runCommand(`git branch -d ${branch}`, `Delete merged branch ${branch}`, { allowFailure: true });
          }
        } catch (resolveError) {
          console.log(`❌ Could not resolve conflicts in ${branch}`);
          // Abort the merge
          runCommand('git merge --abort', 'Abort merge', { allowFailure: true });
        }
      }
    }
...
function cleanupBranches() {
  ...
  try {
    ...
    // Get all branches
    const allBranches = runCommand('git branch -a', 'Get all branches', { silent: true });
