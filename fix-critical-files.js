...
interface ImportMetaEnv {
  readonly VITE_APP_TITLE: string; readonly VITE_APP_DESCRIPTION: string; readonly VITE_APP_URL: string; readonly VITE_APP_API_URL: string; readonly DEV: boolean; readonly PROD: boolean; readonly MODE: string; }
...
interface ImportMeta {
  readonly env: ImportMetaEnv; }`
    },
    {
      ...
      content: `import React from 'react'; import Navigation from '../components/Navigation'; import Footer from '../components/Footer'; import Analytics from './components/Analytics'; export default function RootLayout({
  children}: {
  ...
  for (const file of criticalFiles) {
    try {
      fs.writeFileSync(file.path, file.content, 'utf8'); console.log(`✓ Fixed: ${file.path}`); } catch (error) {
  ...
  const problematicFiles = [
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        let content = ''; if (filePath.endsWith('.tsx')) {
