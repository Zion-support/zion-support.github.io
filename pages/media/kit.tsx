  ...
  const PressReleaseCard = ({ type, title }: { type: PressReleaseType, title: string }) => {
    ...
    return (
      <div className=&quot;p-4 border rounded-lg space-y-2&quot;>
        ...
        <pre className=&quot;whitespace-pre-wrap text-xs bg-gray-50 p-3 rounded max-h-60 overflow-auto&quot;>{text}</pre>      </div>
    )
  },
      ...
      <div className=&quot;space-y-8&quot;>
        <header className=&quot;flex items-center justify-between&quot;>
          ...
          <div className=&quot;flex gap-2&quot;>
            ...
            <button onClick={onGeneratePdf} className=&quot;px-4 py-2 rounded bg-gray-900 text-white hover:bg-black&quot;>Generate PDF</button>          </div>
        </header>
        ...
        <section className="grid md:grid-cols-3 gap-6">
          <div className="p-4 border rounded-lg">
            ...
            <div className="flex gap-2">
              {(['generalweb3institutional'] as MediaBundle[]).map(b => (                <button key={b} onClick={() => setBundle(b)} className={`px-3 py-1 rounded border ${bundle===b ? 'bg-blue-600 text-white border-blue-600':'bg-white'}`}>{b}</button>
              {_(['general', _'web3', _'institutional'] as MediaBundle[]).map(b => (
                <button key={b} onClick={_() => setBundle(b)} className={_`px-3 py-1 rounded border ${bundle===b ? 'bg-blue-600 text-white border-blue-600':'bg-white'}`}>{b}</button>
              ))}
            </div>
          ...
          <div className=&quot;p-4 border rounded-lg&quot;>
            ...
            {timeline.length>0 && (
              <ul className=&quot;mt-3 text-sm list-disc list-inside space-y-1&quot;>
                {timeline.map((t)=> (<li key={t.label}><span className=&quot;font-medium&quot;>{t.label}:</span> {t.date}</li>))}              </ul>
            )}
          </div>
        </section>
          ...
          <ul className=&quot;grid md:grid-cols-2 gap-3&quot;>
            {assets.map(a => (
              <li key={a.filename} className=&quot;flex items-center justify-between border rounded p-2&quot;>
                ...
                {a.path ? <a href={a.path} download className=&quot;text-blue-600 text-sm&quot;>Download</Link> : <span className=&quot;text-gray-400 text-xs&quot;>generated</span>}              </li>
            ))}
          </ul>
        </section>
        ...
        <section className=&quot;p-4 border rounded-lg space-y-4&quot;>
          ...
          <div className=&quot;grid md:grid-cols-3 gap-4&quot;>
            ...
            {bundle === 'web3' && <PressReleaseCard type=&quot;token-sale&quot; title=&quot;Token sale&quot; />}          </div>
        </section>
      </div>
    </div>
  )
