export default function AISolutions(){return (
<main style={{padding:20,fontFamily:'sans-serif',maxWidth:920,margin:'0 auto'}}>
  <h1 style={{fontSize:32,marginBottom:8}}>AI Solutions</h1>
  <p style={{color:'#374151',marginBottom:20}}>Explore production-ready AI services we implement and operate. No mockups—real, secure deployments with measurable outcomes.</p>

  <section style={{marginTop:16}}>
    <h2 style={{fontSize:22}}>Featured Offerings</h2>
    <ul>
      <li><a href="/services" style={{textDecoration:'underline'}}>Agentic Customer Support Suite</a> — resolve tickets end-to-end with tools and approvals.</li>
      <li><a href="/services" style={{textDecoration:'underline'}}>AI Marketing Content Studio</a> — on-brand content at scale with SEO and approvals.</li>
      <li><a href="/services" style={{textDecoration:'underline'}}>Agentic Sales Engineer</a> — technical answers, diagrams, and demo scaffolds 24/7.</li>
      <li><a href="/services" style={{textDecoration:'underline'}}>AI Governance & Risk Suite</a> — policies, evals, monitoring, and audit reporting.</li>
    </ul>
  </section>

  <section style={{marginTop:24}}>
    <h2 style={{fontSize:22}}>What you get</h2>
    <ul>
      <li>Secure architecture with observability and guardrails</li>
      <li>Human-in-the-loop approvals for sensitive actions</li>
      <li>Integrations with your stack (Zendesk, HubSpot, Salesforce, etc.)</li>
      <li>Time-to-value in days or weeks, not months</li>
    </ul>
  </section>

  <section style={{marginTop:24}}>
    <h2 style={{fontSize:22}}>Typical pricing</h2>
    <p style={{color:'#374151'}}>Implementation from $5,000–$25,000 one-time, managed run from $499–$4,999/month depending on scope and seats. We price competitively based on outcomes.</p>
  </section>

  <section style={{marginTop:24,background:'#F9FAFB',padding:16,borderRadius:8}}>
    <h2 style={{fontSize:20}}>Ready to get started?</h2>
    <p>Call <a href="tel:+13024640950" style={{textDecoration:'underline'}}>+1 302 464 0950</a> · Email <a href="mailto:kleber@ziontechgroup.com" style={{textDecoration:'underline'}}>kleber@ziontechgroup.com</a> · Visit <a href="https://ziontechgroup.com" style={{textDecoration:'underline'}}>ziontechgroup.com</a></p>
    <p>Address: 364 E Main St STE 1008 Middletown DE 19709</p>
  </section>
</main>
)}