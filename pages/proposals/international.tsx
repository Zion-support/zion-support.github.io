...
import React, { useEffect, useState } from 'react',import EnhancedLayout from '../../components/layout/EnhancedLayout',type ProposalListItem = {id: string;
  title: string;
...
},type ProposalListItem = any;export default function InternationalProposalsPage() {const [items, setItems] = useState<ProposalListItem[]>([]),const [filter, setFilter] = useState('All'),useEffect__(() => {fetch('/api/proposals').then((r) => r.json()).then((d) => setItems(d.items |[])).catch(() => setItems([]))}, []),const filtered = items.filter((i) => (filter === 'All' ? true : i.regionalScope === filter)).then((r) => r.json()).then((d) => setItems(d.items |[])).catch(() => setItems([])).then(r => r.json()).then(d => setItems(d.items || [])).catch(() => setItems([]))}, [])const filtered = items.filter(i =>;
  ...
  return (<EnhancedLayout>;
      <div className="space-y-4">;
        ...
        <div className="divide-y border rounded">;
          {filtered.map((i) => (<div key={i.id} className="p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">;
              <div>;
          ...
          {filtered.length === 0 && <div className="p-4 text-sm text-gray-600">No proposals yet.</div>  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}</div>;
...
type ProposalListItem = {id: string;
  title: string;
  targetInstitution: string;
  regionalScope: string;
  type: string;
 ...
 */;
function InternationalProposalsPage() {const [items, set_items] = useState < ProposalListItem[]>([]),const [filter, set_filter] = useState ('All'),useEffect (() => {fetch ('/api / proposals').then ((r) => r.json ()).then ((d) => set_items (d.items || [])).catch (() => set_items ([]))}, []),const filtered = items.filter ((i) => (filter === 'All' ? true : i.regional_scope === filter)),return (<EnhancedLayout>;
      <div className="space - y-4">;
        <h1 className="text - 2xl font - semibold">International Proposals</h1>;
        <div className="flex items - center gap - 2 text - sm">;
          <span > Filter by region:</span>;
