import React from 'react';
import Head from 'next/head';

export default function WhitePapers() {
  return (
    <>
      <Head>
        <title>White Papers | Zion Tech Group</title>
        <meta name="description" content="Technical white papers and research from Zion Tech Group." />
        <link rel="canonical" href="https://ziontechgroup.com/white-papers" />
      </Head>
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 py-16">
          <h1 className="text-4xl font-bold text-center mb-8">White Papers</h1>
          <p className="text-xl text-center text-gray-600 mb-12">
            Technical research and insights from our team of experts.
          </p>
          <div className="text-center">
            <a href="mailto:kleber@ziontechgroup.com" className="inline-block bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700">
              Contact Us
            </a>
          </div>
        </div>
      </div>
    </>
  );
}