...
;
  const productResults = MARKETPLACE_LISTINGS.filter(;
    ...
    rating:p.rating,;
    author:p.author;
      ? { name:p.author.name, avatar:p.author.avatarUrl } undefined,;
    tags:p.tags,;
    category:p.category,;
    date:p.createdAt})),;
;
...
function offlineSearch(
  ...
  const term = query.toLowerCase().trim(),
  const match = (text?: string) => text?.toLowerCase().includes(term),
  const productResults = MARKETPLACELISTINGS.filter(_(p) =>
      match(p.title) ||
      match(p.description) ||
      match(p.category) ||
      p.tags?.some((t) => match(t))).map((p) => ({
  ...
  const talentResults = TALENTPROFILES.filter(_(t) =>
      match(t.fullname) ||
      match(t.professionaltitle) ||
      match(t.bio) ||
      t.skills?.some((s) => match(s))).map((t) => ({
    id: t.id,
    title: t.fullname,
    description: t.professionaltitle || '',
    type: 'talent' as const,
    slug: t.id,
    image: t.profilepicture_url,
    rating: t.averagerating,
    author: { name: t.fullname, avatar: t.profilepicture_url },    tags: t.skills,
    category: t.location,
    date: undefined})),

  const blogResults = BLOGPOSTS.filter(_(b) =>
      match(b.title) ||
      match(b.excerpt) ||
      match(b.content) ||
      b.tags?.some((t) => match(t))).map((b) => ({
  ...
  if (typeof filters.maxPrice === 'number') {all = all.filter(r => {
      if (r.type === 'product') {
        return (r.price ?? 0) <= filters.maxPrice!
      }
      return true
    })  }
  if (typeof filters.minRating === 'number') {all = all.filter(r => {
      if (r.type === 'product' || r.type === 'talent') {
        return (r.rating ?? 0) >= filters.minRating!
      }
      return true
  ...
  if (filters.sortBy && filters.sortBy !== 'relevance') {switch (filters.sortBy) {
      case 'priceasc':
        all.sort((a, b) => {
          const aPrice = a.type === 'product' ? (a.price ?? 0) : 0,
          const bPrice = b.type === 'product' ? (b.price ?? 0) : 0,
...
const loadMore = () => {
  ...
  const categories = Array.from(
    new Set(results.map((r) => r.category).filter(Boolean))),
  const filteredResults = results.filter(_(r) => {if (
      categoryFilter !== 'all' &&
      categoryFilter &&
      r.category !== categoryFilter
    ) {
...
const groupedResults = filteredResults.reduce(
    ...
    {} as Record<string SearchResult[]>),
  const renderResultCard = (result: SearchResult) => {switch (result.type) {
      case 'product':
      case 'equipment':
        return (
          <div key={result.id} data-testid=&quot;result-card&quot;>
...
talent={{
                ...
                professionaltitle: result.description || '',
                profilepicture_url: result.image,
...
className=&quot;container mx-auto px-4 py-8&quot;
        ...
        >
          ...
          <div className=&quot;mb-8&quot;>
            <div className=&quot;flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4&quot;>
              <div className=&quot;flex-1&quot;>
                <h1 className=&quot;text-3xl font-bold text-gray-900 dark:text-white mb-2&quot;>                  Search Results
                </h1>
                <p,
className=&quot;text-gray-600 dark:text-gray-200&quot;
                  data-testid=&quot;results-count&quot;
...
=======
          ...
          {filteredResults.length > 0 && (
            <div className="space-y-8">
              {Object.entries(groupedResults).map(_([type, typeResults]) => (
                <div key={type}>
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4 capitalize">
                    {type}s ({typeResults.length})
                  </h2>
...
const query = slug ? slug.replace(/-/g, ' ') : '',
  ...
  try {
    // In production, replace with your actual API base URL,
const apiBaseUrl =
      process.env.NEXTPUBLIC_API_URL || 'http: //localhost:3000',
