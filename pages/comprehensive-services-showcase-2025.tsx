...
;
  // Filter services based on selections;
  const filtered_services = comprehensiveRealServices2025.filter (service => {
...
if ( {) {
  $2
      ...
      switch (selectedPriceRange) {
        case 'Under $300':;
          price_match = price < 300;
          break;
        case '$300 - $500':;
          price_match = price >= 300 && price <= 500;
          break;
        case '$500 - $800':;
          price_match = price > 500 && price <= 800;
          break;
        case '$800+':;
          price_match = price > 800;
          break;
      }
    }
    const search_match =;
...
=======
    ...
    return res.status(500).json({ error: "Internal server error" });
        ...
        {/* Filters and Search Section */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
    ...
    return res.status(500).json({ error: "Internal server error" });
            ...
            <div className="mb-8">
              <p className="text-gray-400">
                Showing <span className="text-cyan-400 font-semibold">{sortedServices.length}</span> of{' '  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
...
;
    ...
    return res.status(500).json({ error: "Internal server error" });
                        ...
                        <span className="text-gray-400">
                          Setup: {service.setupTime  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
...
=======
    ...
    return res.status(500).json({ error: "Internal server error" });
                  ...
                  >
                    <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl p-6 border border-gray-700 hover:border-cyan-500/50 transition-all duration-300">
                      <div className="flex flex-col lg:flex-row gap-6 items-start">
                        {/* Service Icon and Basic Info */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
    ...
    return res.status(500).json({ error: "Internal server error" });
                          ...
                          <span className="text-sm text-gray-400">
                            {getPriceRange(service.price)  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
                      ...
                      {/* Features and Benefits */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
    ...
    return res.status(500).json({ error: "Internal server error" });
                      ...
                      {/* CTA and Additional Info */  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
}
