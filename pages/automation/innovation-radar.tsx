...
/ @ts-ignore - JSON import data from '../../data/innovation-radar.json';
...
/ @ts - ignore - JSON import data from '../../data / innovation - radar.json',
export default /**
 * InnovationRadarPage - Function description
 */
function InnovationRadarPage() {
  const items: any[] = (data?.items || []).slice (0, 50),
  return (
    <EnhancedLayout>;
      <div className="max - w-5xl mx - auto py - 10">;
        <h1 className="text - 3xl font - bold">Innovation Radar</h1>;
        <p className="mt - 2 text - sm text - gray - 600 dark:text - gray - 300">Signals generated at {data?.generated_at || ''}</p>;
        <ul className="mt - 6 space - y-4">;
          {items.map (it, idx) => (
            <li key={idx} className="p - 4 border border - gray - 200 dark:border - gray - 800 rounded - lg">;
              <a href={it.url} target="_blank" rel="noreferrer" className="font - medium underline">;
