...
export default function StackExchangeInsightsPage() {_const items: unknown[] = (data?.items || []).slice(0, _50);
        ...
        <ul className="mt-6 space-y-4">
          {_items.map((it, _idx) => (
            <li key={idx} className="p-4 border border-gray-200 dark:border-gray-800 rounded-lg">
              ...
              <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">Score {_it.score} · Answers {_it.answer_count} · Tags: {_(it.tags || []).join(', _')}</div>
