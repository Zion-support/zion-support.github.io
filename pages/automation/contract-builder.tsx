...
export default function ContractBuilderPage() {
  ...
  const canSubmit = useMemo_(() => {return (
  ...
  async function submitForm(event: React.FormEvent) {
    ...
    try {
      const body = {
        talentName, projectName, scopeSummary, startDate: startDate?.toISOString().slice(0, 10), endDate: endDate?.toISOString().slice(0, 10), payment:
          paymentType === 'hourly'
            ? {
        ...
        clauses: {nda, ipTransfer},
        governingLaw,
        revisionRounds},

      const res = await fetch('/api/ai-contract', {method: 'POST', headers: {
      ...
      <form onSubmit={submitForm} className=&quot;grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-neutral-900 p-6 rounded-lg border border-gray-200 dark:border-neutral-800&quot;>
          ...
          {paymentType === 'hourly' ? (
            ...
            <div className=&quot;grid grid-cols-1 md:grid-cols-3 gap-4&quot;>
              ...
              <div className=&quot;md:col-span-3&quot;>
                <label className=&quot;block text-sm font-medium mb-1&quot;>Payment schedule</label>
                <input className=&quot;w-full input input-bordered&quot; value={paymentSchedule} onChange={(e) => setPaymentSchedule(e.target.value)} placeholder=&quot;e.g., 50% upfront, 50% on delivery&quot; />              </div>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className=&quot;space-y-2&quot;>
            ...
            <label className=&quot;inline-flex items-center gap-2&quot;>
              <input type=&quot;checkbox&quot; checked={ipTransfer} onChange={(e) => setIpTransfer(e.target.checked)} /> IP Transfer / Assignment            </label>
          </div>
        </div>
