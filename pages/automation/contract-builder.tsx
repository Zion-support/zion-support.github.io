...
export default function ContractBuilderPage() {
  ...
  const [milestoneSummary, setMilestoneSummary] = useState($2);
  const [paymentSchedule, setPaymentSchedule] = useState($2);
  const [nda, setNda] = useState($2);
  const [ipTransfer, setIpTransfer] = useState($2);
  const [governingLaw, setGoverningLaw] = useState($2);
  ...
  async function submitForm(event: React.FormEvent) {
    ...
    try {
      const body = $2;
        projectName,
        scopeSummary,
        startDate: startDate ?.toISOString().slice($2);
        endDate: endDate ?.toISOString().slice($2);
        payment: paymentType = $2;
                currency,
                hourlyRate,
                weeklyHourCap: typeof weeklyHourCap = $2;
                paymentSchedule}
            : {
                type: 'fixed',
                currency,
