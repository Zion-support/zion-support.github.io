  ...
  const items = [
    {
      ...
      title: 'Top Talents (Auto)',
      desc: 'Aggregated from approved reviews, updated on schedule',
    },
    {
      href: '/automations/sitemap-status',
      title: 'Sitemap Status (Auto)',
      desc: 'Nightly sitemap and weekly pings to search engines',
    },
  ];
  ...
  return (
    <main className='space-y-6'>
      ...
      <p className='text-gray-600'>
        These run in the cloud on a schedule, with no human interaction.
      </p>
        ...
        {items.map(it => (
          <Link key={it.href} href={it.href}>
            <div className='enhanced-card hover:shadow-lg cursor-pointer'>
              ...
              <p className='text-sm text-gray-600 mt-1'>{it.desc}</p>            </div>          </Link>
        ))}
      </div>
    </main>
  );};
