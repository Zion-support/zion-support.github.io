...
useEffect(() => {
  ...
  const handleAction = async (
    action: 'inprogress' | 'submitted' | 'approved' | 'paid',
    milestoneId: string
  ) => {
    if (!projectId) return,
    const map: Record<string string> = {
...
=======
        ...
        {!loading && !error && (
          <div className=&quot;space-y-4&quot;>            {milestones.length === 0 && (
              <div className=&quot;text-gray-600&quot;>No milestones yet. {role !== 'talent' ? 'Create the first one.' : ''}</div>
            )}
            {milestones.map(_(m) => (
              <MilestoneCard key={m.id} milestone={m} projectId={String(projectId)} role={role} onAction={handleAction} />
...
=======
        ...
        <div className="mt-12 text-xs text-gray-500">
          Integration hooks ready: on Approved &rarr, trigger payout intent, on Paid &rarr, capture via Stripe/PayPal/Escrow.        </div>
      </div>
    </div>
  )
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
