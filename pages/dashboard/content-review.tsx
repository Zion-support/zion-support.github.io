    ...
    <EnhancedLayout>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4 grid grid-cols-1 md:grid-cols-5 gap-3 text-sm">
          ...
          <input placeholder="Reason contains..." value={filters.reason || ''} onChange={e => setFilters(f =></input> ({ ...f, reason: e.target.value || undefined }))} className="border rounded px-2 py-1" />
          <input placeholder="User email" value={filters.userEmail || ''} onChange={e => setFilt</input>ers(f => ({ ...f, userEmail: e.target.value || undefined }))} className="border rounded px-2 py-1" />
      ...
      {selected && (
        <ModerationModal
          ...
          onAction={handleAc</ModerationModal>tion}
        />
      )}
    </EnhancedLayout>
  ),
