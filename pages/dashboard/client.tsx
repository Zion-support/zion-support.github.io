          ...
          {STEPS.map(s => (
            <li key={s.key} className='flex items-center justify-between'>
              <div className='flex items-center gap-2'>
                <span
                  className={`inline-flex h-5 w-5 items-center justify-center rounded-full border ${completed[s.key] ? 'bg-emerald-500 text-white border-emerald-500' : 'border-gray-300 dark:border-gray-700'}`}>{completed[s.key] ? '✓' : ''}
                </span>
...
ursor/integrate-build-improve-and-re-verify-b76c
              ...
              {completed[s.key] ? (
                ...
                <EnhancedButton
                  ...
                  variant='secondary'
                  className='text-xs py-1 px-2'>{s.key === 'job' ? 'Post a Job' : 'Mark done'}
                </EnhancedButton>
        <h2 className="font-semibold mb-2">Checklist</h2>
        <ul className="space-y-2">
          {STEPS.map((s) => (
