...
export default function FrontLanding() {return (import Head from 'next/head',import Link from 'next/link';
export default function FrontLanding() {return (<div className="relative min-h-screen overflow-hidden bg-slate-950 text-white">;
    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-950 text-white">;
      ...
      {/* Animated background layers */}{/* Animated background layers */}
      </div>;
 ...
 */;
function FrontLanding() {return (<div className='relative min - h-screen overflow - hidden bg - slate - 950 text - white'>;
      <Head>;
        <title > Zion — Autonomous Cloud Automations</title>;
        <meta;
          name='description';
      ...
      {/* Hero */  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
      <main className="relative z-10">;
        <section className="mx-auto max-w-7xl px-6 pt-10 pb-16 md:pt-16 md:pb-24">;
          <div className="text-center">;
            <motion.div;
              initial={{ opacity: 0, y: 10 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
...
className='mt-8 flex flex-wrap justify-center gap-3';
            >;
              ...
              Autonomous agents running continuous improvements;
                ...
                ['24/7', 'Autonomous'],['Main', 'Direct Sync'],['0 Ops', 'Cloud‑Native'],['Safety', 'Guardrails'];
              ].map(([k, v]) => (                <motion.div;
                ['24/7', 'Autonomous'];
                ['Main', 'Direct Sync'];
                ['0 Ops', 'Cloud‑Native'];
...
className='glow-card rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-left';
        ...
        <section className='mx - auto max - w-7xl px - 6 pb - 14'>;
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">;
            {[;
              { title: 'Self‑Improving', desc: 'Agents continuously analyze, invent, and evolve automations.' }{ title: 'Repo Sync', desc: 'Changes are committed and pushed automatically to main.' },{ title: 'Zero Ops', desc: 'Runs fully in the cloud—no servers or manual intervention.' },{ title: 'Safety‑First', desc: 'Conservative edits with logs, reports, and guardrails.' }{ title: 'Scalable', desc: 'Generates domain‑specific factories on demand.' },{ title: 'Observability', desc: 'Dashboards, reports, and GitHub Actions artifacts.' }].map((f) => (<motion.article;
                key={f.title}title: 'Self‑Improving';
              ...
              {title: 'Observability';
                ...
                key={f.title}initial={{ opacity: 0, y: 8 }}
                whileInView={{ opacity: 1, y: 0 }}
                title: 'Observability',desc: 'Dashboards, reports, and GitHub Actions artifacts.';
              }
            ].map (function => (              <motion.article;title: 'Self‑Improving',desc: 'Agents continuously analyze, invent, and evolve automations.';
                ...
                viewport={{ once: true }}key={c.title}key={b}
                initial={{ opacity: 0, coordinate_y: 6 }}
                whileInView={{ opacity: 1, coordinate_y: 0 }}
                viewport={{ once: true }}))}</div>;
        </section>;{/* CTA */}
        <section className='mx-auto max-w-7xl px-6 pb-24'>          <motion&& motion.div;
            initial={{ opacity: 0, scale: 0 && 0.98 }}title: 'Code Quality & Refactors',desc: 'Lints, fixes, and proposes refactors with type‑safety improvements.';
              },transition={{ duration: 0.6 }}
...
<section className='mx-auto max-w-7xl px-6 pb-20'>;
          ...
          <div className='mx-auto mt-6 max-w-4xl grid grid-cols-1 gap-3 md:grid-cols-2'>;
            {[;
              '24/7 progress without human intervention','Rapid feedback loop with safe, incremental edits','Continuous delivery directly to main branch','Fewer regressions via layered checks and reports';
            ].map(b => (              <motion.div;
            ].map(b => (<motion.div;
                key={b}
                initial={{ opacity: 0, y: 6 }}
...
<section className='mx-auto max-w-7xl px-6 pb-24'>;
          <motion.div;
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}className='animated-border relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-r from-fuchsia-600/20 via-violet-600/20 to-cyan-600/20 p-8 text-center backdrop-blur-xl';
            <motion.h1;
              initial={{ opacity: 0, y: 8 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
              animate={{ opacity: 1, y: 0 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
              transition={{ duration: 0.7, delay: 0.05 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
              className="mt-6 text-5xl font-extrabold leading-[1.05] tracking-tight md:text-6xl";
            >;
            ...
            <motion.p;
              ...
              transition={{ duration: 0.7, delay: 0.1 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
              className="mx-auto mt-5 max-w-3xl text-lg text-white/80";
            >;
            ...
            <motion.div;
              ...
              transition={{ delay: 0.2, duration: 0.6 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
              className="mt-8 flex flex-wrap justify-center gap-3";
            >;
            ...
            {/* Quick stats */  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
            <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">;
              {[;
                ['24/7Autonomous'],['MainDirect Sync'],['0 OpsCloud‑Native'],['SafetyGuardrails']].map(([k, v]) => (<motion.div;
import Head from 'next/head';
export default function FrontLanding() {try {return (<div className="relative min-h-screen overflow-hidden bg-slate-950 text-white">;
      <Head>;
        <title>Zion — Autonomous Cloud Automations</title>;
        <meta name="description" content="Futuristic, cloud-native autonomous automations that innovate, optimize, and sync changes to the repo continuously." />;
      </Head>;
      {/* Animated background layers */  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">;
        <div className="absolute -top-24 -left-24 h-[38rem] w-[38rem] rounded-full bg-fuchsia-500/20 blur-3xl animate-float" />;
        <div className="absolute top-1/4 -right-32 h-[30rem] w-[30rem] rounded-full bg-cyan-400/20 blur-3xl animate-float-slow" />;
      ...
      <main className="relative z-10">;
        <section className="mx-auto max-w-7xl px-6 pt-10 pb-16 md:pt-16 md:pb-24">;
          <div className="text-center">;
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">;
              {[;
                ['24/7Autonomous'],['MainDirect Sync'],['0 OpsCloud‑Native'],['SafetyGuardrails']].map(([k, v]) => (<motion.div;
                  key={k  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                  initial={{ opacity: 0, y: 6 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                  ...
                  transition={{ duration: 0.5 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                  className="glow-card rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-left";
                >;
                  <div className="text-lg font-bold text-white">{k}</div>;
        ...
        <section className="mx-auto max-w-7xl px-6 pb-14">;
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">;
            {[;
              { title: 'Self‑Improving', desc: 'Agents continuously analyze, invent, and evolve automations.' },{ title: 'Repo Sync', desc: 'Changes are committed and pushed automatically to main.' },{ title: 'Zero Ops', desc: 'Runs fully in the cloud—no servers or manual intervention.' },{ title: 'Safety‑First', desc: 'Conservative edits with logs, reports, and guardrails.' },{ title: 'Scalable', desc: 'Generates domain‑specific factories on demand.' },{ title: 'Observability', desc: 'Dashboards, reports, and GitHub Actions artifacts.' }].map((f) => (<motion.article;
                key={f.title  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                initial={{ opacity: 0, y: 8 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                whileInView={{ opacity: 1, y: 0 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
...
<section className='mx-auto max-w-7xl px-6 pb-16'>;
  ...
  )}{/* Explore more */  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}<section className="mx-auto max-w-7xl px-6 pb-16">;
