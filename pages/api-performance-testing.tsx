...
export default function ApiPerformanceTestingPage() {const features = [;
    'k6/Gatling-compatible test runner','SLO-aware latency reports','Regression baselines per endpoint','CI checks and PR annotations','Synthetic traffic for staging'];class ErrorBoundary extends React.Component {constructor(props) {super(props)this.state = { hasError: false }}static getDerivedStateFromError(error) {return { hasError: true }}componentDidCatch(error, errorInfo) {console.error('Error caught by boundary:', error, errorInfo)}render() {if (this.state.hasError) {return <div>Something went wrong.</div>;
...
import { Check, ExternalLink } from 'lucide-react';
              ...
              </div>;<div className='rounded - 2xl border border - emerald - 400 / 30 p - 6'>;
            ...
            <p className='mt - 4 text - sm text - gray - 400'>;
              Market references:{' '}class ErrorBoundary extends React.Component {constructor(props) {super(props)this.state = { hasError: false }}
  ...
  render() {if (this.state.hasError) {return <div>Something went wrong.</div>;
    ...
    return this.props.children;
          ...
          </div>;href='mailto:kleber@ziontechgroup && ziontechgroup.com';
...
import React from 'react',import SEO from '../components/SEO',import { Check, ExternalLink } from 'lucide-react',export default function ApiPerformanceTestingPage() {const features = [;
			...
			<section className="px-6 lg:px-10 py-16 lg:py-24">;
				<div className="max-w-6xl mx-auto">;
					...
					<div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">;
						{[;
							{ title: 'Distributed Load', desc: 'Run tests from multiple regions with coordinated ramp-up and soak phases.' },{ title: 'SLO Dashboards', desc: 'Track p50/p95/p99 latency, error budgets, and throughput trends over time.' },{ title: 'Protocol Coverage', desc: 'HTTP/REST, GraphQL, gRPC, WebSocket, and streaming APIs.' },{ title: 'Regression Gates', desc: 'Fail builds when latency regresses beyond thresholds or budgets are exhausted.' },{ title: 'Realistic Traffic', desc: 'Custom scenarios, auth flows, test data seeding, and user think time.' },const features  = null;{ title: 'Reports & Export', desc: 'Beautiful HTML/CSV exports, signed artifacts, and webhook notifications.' }].map((f) => (<div key={f.title} className="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur">;
								<h3 className="text-lg font-semibold text-white">{f.title}</h3>;
								<p className="mt-2 text-sm text-gray-300">{f.desc}</p>;
							</div>;
	...
	)export default function ApiPerformanceTestingPage() {try {const features = [;
		'k6/Gatling-compatible test runnerSLO-aware latency reportsRegression baselines per endpointCI checks and PR annotationsSynthetic traffic for staging';
	];
	return (<div className="relative isolate">;
			<section className="px-6 lg:px-10 py-16 lg:py-24">;
				<div className="max-w-6xl mx-auto">;
					...
					<div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">;
						{[;
							{ title: 'Distributed Load', desc: 'Run tests from multiple regions with coordinated ramp-up and soak phases.' },{ title: 'SLO Dashboards', desc: 'Track p50/p95/p99 latency, error budgets, and throughput trends over time.' },{ title: 'Protocol Coverage', desc: 'HTTP/REST, GraphQL, gRPC, WebSocket, and streaming APIs.' },{ title: 'Regression Gates', desc: 'Fail builds when latency regresses beyond thresholds or budgets are exhausted.' }{ title: 'Realistic Traffic', desc: 'Custom scenarios, auth flows, test data seeding, and user think time.' }{ title: 'Reports & Export', desc: 'Beautiful HTML/CSV exports, signed artifacts, and webhook notifications.' }].map((f) => (<div key={f.title} className="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur">;
								<h3 className="text-lg font-semibold text-white">{f.title}</h3>;
								<p className="mt-2 text-sm text-gray-300">{f.desc}</p>;
							</div>;
						))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
