...
export default function TokenomicsWhitepaperBuilder() {
  ...
  const totalPercent = useMemo() => distribution.reduce(acc, d) => acc + (Number(d.percent) |0), 0)
    [distribution]
  );
  ...
  const total_percent = useMemo () => distribution.reduce (acc, d) => acc + (Number (d.percent) || 0), 0),
    [distribution]);
