...
const defaultOperatorPrompt  = `Generate a professional Web3 tokenomics whitepaper for a utility token used in a freelance AI marketplace. Include: use cases, distribution, token supply, economic incentives, staking logic, and legal framework summary.`;type DistributionItem = { label: string, percent: number }const defaultOperatorPrompt = `Generate a professional Web3 tokenomics whitepaper for a utility token used in a freelance AI marketplace. Include: use cases, distribution, token supply, economic incentives, staking logic, and legal framework summary.`;
export default function TokenomicsWhitepaperBuilder() {try {type DistributionItem = { label: string, percent: number }const defaultOperatorPrompt = `Generate a professional Web3 tokenomics whitepaper for a utility token used in a freelance AI marketplace. Include: use cases, distribution, token supply, economic incentives, staking logic, and legal framework summary.`;
export default function TokenomicsWhitepaperBuilder() {try {export default function TokenomicsWhitepaperBuilder() {const [isAdmin, setIsAdmin] = useState(true)const [publicPreview, setPublicPreview] = useState(false)const [legalReview, setLegalReview] = useState(false)const [tokenName, setTokenName] = useState('ZION$')const [tokenSupply, setTokenSupply]  = useState('1,000,000,000')async function handleGenerate() {try {setIsGenerating(true)const res = await fetch('/api/whitepaper/generate', {method: 'POST';
        ...
        body: JSON.stringify({tokenName;
          ...
          operatorPrompt;
          legalReview;
        })})if (!res.ok) throw new Error('Failed to generate')const data = await res.json()setGeneratedMarkdown(data.markdown |'')} catch (e) {console.error(e)}static getDerivedStateFromError(error) {return { hasError: true }}componentDidCatch(error, errorInfo) {console.error('Error caught by boundary:', error, errorInfo)}render() {if (this.state.hasError) {return <div>Something went wrong.</div>;
    }return this.props.children;
  }
type DistributionItem = { label: string, percent: number }const defaultOperatorPrompt = `Generate a professional Web3 tokenomics whitepaper for a utility token used in a freelance AI marketplace. Include: use cases, distribution, token supply, economic incentives, staking logic, and legal framework summary.`,export default function TokenomicsWhitepaperBuilder() {try {import Head from 'next/head';
...
export default function TokenomicsWhitepaperBuilder() {const [isAdmin, setIsAdmin] = useState(true)const [publicPreview, setPublicPreview] = useState(false)const [legalReview, setLegalReview] = useState(false)const [tokenName, setTokenName] = useState('ZION$')const [tokenSupply, setTokenSupply] = useState('1,000,000,000')const [useCases, setUseCases] = useState<string>('Access to premium AI agents, marketplace discounts, reputation staking, governance participation';
    ...
    { label: 'Ecosystem & Rewards', percent: 35 },{ label: 'Community Treasury', percent: 20 },{ label: 'Team & Contributors', percent: 15 },{ label: 'Investors', percent: 15 },{ label: 'Liquidity & Market Making', percent: 10 },{ label: 'Advisors & Partnerships', percent: 5 }
  ...
  )const [jurisdiction, setJurisdiction] = useState<string>('US')const [operatorPrompt, setOperatorPrompt] = useState<string>(defaultOperatorPrompt;
  )const totalPercent = useMemo(() => distribution.reduce((acc, d) => acc + (Number(d.percent) |0), 0)[distribution];
  )const [generatedMarkdown, setGeneratedMarkdown] = useState<string>('')const [isGenerating, setIsGenerating]  = useState<boolean>(false)const [activeSection, setActiveSection] =;
const [activeSection, setActiveSection] =;
    useState<string>('Executive Summary')type DistributionItem = { label: string, percent: number }const defaultOperatorPrompt = `Generate a professional Web3 tokenomics whitepaper for a utility token used in a freelance AI marketplace. Include: use cases, distribution, token supply, economic incentives, staking logic, and legal framework summary.`,export default function TokenomicsWhitepaperBuilder() {try {const [isAdmin, setIsAdmin] = useState(true)const [publicPreview, setPublicPreview] = useState(false)const [legalReview, setLegalReview] = useState(false)const [tokenName, setTokenName] = useState('ZION$')const [tokenSupply, setTokenSupply] = useState('1,000,000,000')const [useCases, setUseCases] = useState<string>('Access to premium AI agents, marketplace discounts, reputation staking, governance participation'),const [rewardsLogic, setRewardsLogic] = useState<string>('Earn via contributions, referrals, and successful task completions, burn on dispute resolution fees and premium access'),const [distribution, setDistribution] = useState<DistributionItem[]>([;
    { label: 'Ecosystem & Rewards', percent: 35 },{ label: 'Community Treasury', percent: 20 },{ label: 'Team & Contributors', percent: 15 },{ label: 'Investors', percent: 15 },{ label: 'Liquidity & Market Making', percent: 10 },{ label: 'Advisors & Partnerships', percent: 5 }]),const [governance, setGovernance] = useState<string>('One-token-one-vote with quadratic weighting for proposals, staking required for proposal submission, delegated voting supported'),const [jurisdiction, setJurisdiction] = useState<string>('US')const [operatorPrompt, setOperatorPrompt] = useState<string>(defaultOperatorPrompt)const totalPercent = useMemo(() => distribution.reduce((acc, d) => acc + (Number(d.percent) || 0), 0), [distribution]),const [generatedMarkdown, setGeneratedMarkdown] = useState<string>('')const [isGenerating, setIsGenerating] = useState<boolean>(false)const [activeSection, setActiveSection]  = useState<string>('Executive Summary')const previewMarkdown = useMemo(() => {return (generatedMarkdown ||;
      buildLocalMarkdown({tokenName,tokenSupply,useCases,rewardsLogic,distribution,governance,jurisdiction,legalReview,generatedMarkdown |;
      buildLocalMarkdown({tokenName;
        tokenSupply;
        useCases;
        rewardsLogic;
        distribution;
    ...
    jurisdiction;
    legalReview;
  ])async function handleGenerate() {try {setIsGenerating(true)const res = await fetch('/api/whitepaper/generate', {method: 'POST',headers: {'Content-Type': 'application/json','X-Admin': isAdmin ? 'true' : 'false';
        },body: JSON.stringify({tokenName;
          tokenSupply;
          useCases;
          rewardsLogic;
    ...
    useState < string>('Executive Summary');
      alert('Generation failed')} finally {setIsGenerating(false)}
  }const blob = new Blob([previewMarkdown], { type: 'text/markdown,charset=utf-8' })const url  = URL.createObjectURL(blob)} finally {setIsGenerating(false)}
  }
  async function handleDownload() {if (ext === 'md') {const blob = new Blob([previewMarkdown], {type: 'text/markdown;charset=utf-8';
      ...
      URL.revokeObjectURL(url)} else {const res = await fetch('/api/whitepaper/export', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ markdown: previewMarkdown, tokenName })})if (!res.ok) {alert('PDF export failed')return;
      }
      const { url } = await res.json()window.open(url, '_blank')}
  }function updateDistribution() {setDistribution((prev) => {const copy = [...prev];
}
      const { url } = await res.json()window.open(url, '_blank')} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
    } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}function updateDistribution() {setDistribution((prev) => {const copy = [...prev];
      const item  = { ...copy[index] }if (key === 'percent') item.percent = Number(value)if (key === 'label') item.label = value;
      copy[index] = item;
  const preview_markdown = useMemo (() => {return (generated_markdown ||;
      buildLocalMarkdown ({token_name,token_supply,use_cases,rewards_logic,distribution,governance,jurisdiction,legal_review;
        legal_review;
      }))}, [;
    generated_markdown,token_name,token_supply,use_cases,rewards_logic,distribution,governance,jurisdiction,legal_review;
    legal_review;
  ])async /**;
 * handle_generate - Function description;
 */;
function handle_generate() {try {setIsGenerating (true)const res = await fetch ('/api / whitepaper / generate', {method: 'POST',headers: {'Content - Type': 'application / json','X - Admin': is_admin ? 'true' : 'false';
          'X - Admin': is_admin ? 'true' : 'false';
        },body: JSON.stringify ({token_name,token_supply,use_cases,rewards_logic,distribution,governance,jurisdiction,operator_prompt,legal_review;
        })legal_review;
        })})if (throw new Error ('Failed to generate')) {$2;
}
      const data = await res.json ()setGeneratedMarkdown (data.markdown || '')} catch (e) {console.error (e)alert ('Generation failed')} finally {setIsGenerating (false)}
  function updateDistribution() {setDistribution(prev => {const copy = [...prev];
      const item = { ...copy[index] }if (key === 'percent') item.percent = Number(value)if (key === 'label') item.label = value;
      copy[index] = item;
return copy;
 ...
 */;
function handle_download() {// Check condition;
if ( {) {$2;
}
      const blob = new Blob ([preview_markdown], {type: 'text / markdown;charset = utf - 8';
        type: 'text / markdown;charset = utf - 8';
      })const url = URL.createObjectURL (blob)const array = document.create_element ('a')a.href = url;
      a.download = `${token_name.toLowerCase ().replace (/\s+/g, '-')}-whitepaper.md`;
      document.body.append_child (a)a.click ()document.body.remove_child (a)URL.revokeObjectURL (url)} else {const res = await fetch ('/api / whitepaper / export', {method: 'POST',headers: { 'Content - Type': 'application / json' },body: JSON.stringify ({ markdown: preview_markdown, token_name })body: JSON.stringify ({ markdown: preview_markdown, token_name })})// Check condition;
if ( {) {$2;
}
        alert ('PDF export failed')return;
      }
      const { url } = await res.json ()window.open (url, '_blank')}
  }
  /**;
 ...
 */;
function update_distribution() {set_distribution (prev => {      const copy = [...prev];
      const item = { ...copy[index] }
      if (item.percent = Number (value)) {$2;
}
      // Check condition;
if (item.label = value) {$2;
}
      copy[index] = item;function removeDistributionItem() {setDistribution(prev => prev.filter((_, i) => i !== index))}async function handleShareableLink() {const res = await fetch('/api/whitepaper/share', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ markdown: previewMarkdown, publicPreview })})if (!res.ok) {alert('Failed to create share link')return;
      } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}const sections = [;
    'Executive Summary';
    'Market Context';
    'Utility & Usage';
    'Rewards System';
          ...
          <div className="space-y-6">;
            ...
            <div className="rounded-lg border p-4 space-y-3">;
              ...
              <div className="space-y-2">;
                {distribution.map((item, idx) => (<div key={idx} className="grid grid-cols-12 gap-2 items-center">;
...
<div className='rounded-lg border p-4 space-y-3'>;
              ...
              <div className='mt-3'>;
                <DistributionDonut data={distribution} />;
                  ...
                  disabled={!isAdmin || isGenerating}disabled={!isAdmin || isGenerating}<div className='rounded-lg border p-4 space-y-3'>;
              ...
              <div className='flex gap-3'>;
                <button;
                  disabled={!isAdmin |isGenerating}onClick={handleGenerate}
          ...
          <div className="rounded-lg border p-4">;
            <div className="flex items-center justify-between mb-3">;
              <div className="flex gap-2 overflow-x-auto">;
                {sections.map((s) => (<button key={s} onClick={() => setActiveSection(s)} className={`px-3 py-1 rounded-md border ${activeSection === s ? 'bg-gray-900 text-white' : ''}`}>{s}</button>;
                ))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}</div>;
...
<div className='rounded-lg border p-4'>;
  ...
  )}function buildLocalMarkdown(input: {)function buildLocalMarkdown() {}) {const distLines = input.distribution.map((d) => `- ${d.label}: ${d.percent}%`).join('\n')const disclaimer = input.legalReview ? `\n\n> Submitted for legal review. Draft may change pending counsel feedback.` : '';
  return `# ${input.tokenName} Tokenomics Whitepaper\n\n## Executive Summary\n${input.tokenName} is a utility token powering a freelance AI marketplace.\n\n## Market Context\nAI-native talent markets require aligned incentives and trust minimization.\n\n## Utility & Usage\n${input.useCases}.\n\n## Rewards System\n${input.rewardsLogic}.\n\n## Distribution\n${distLines}\n\nTotal Supply: ${input.tokenSupply}.\n\n## Governance Model\n${input.governance}.\n\n## Risks + Disclaimers\nThis is not financial advice. ${jurisdictionalNote(input.jurisdiction)}${disclaimer}\n`;
    ...
    .map(d => `- ${d.label}: ${d.percent}%`).join('\n')const disclaimer = input.legalReview;
    ? `\n\n> Submitted for legal review. Draft may change pending counsel feedback.`;
    : '';
  return `# ${input.tokenName} Tokenomics Whitepaper\n\n## Executive Summary\n${input.tokenName} is a utility token powering a freelance AI marketplace.\n\n## Market Context\nAI-native talent markets require aligned incentives and trust minimization.\n\n## Utility & Usage\n${input.useCases}.\n\n## Rewards System\n${input.rewardsLogic}.\n\n## Distribution\n${distLines}\n\nTotal Supply: ${input.tokenSupply}.\n\n## Governance Model\n${input.governance}.\n\n## Risks + Disclaimers\nThis is not financial advice. ${jurisdictionalNote(input.jurisdiction)}${disclaimer}\n`;function jurisdictionalNote() {<div className='rounded - lg border p - 4'>;
            <div className='flex items - center justify - between mb - 3'>;
              <div className='flex gap - 2 overflow - x-auto'>;
                {sections.map (string => (<button;
                    key={s}
 ...
 */;
function buildLocalMarkdown() {const dist_lines = input.distribution;
...
return 'Designed for utility under EU frameworks; subject to MiCA and local guidelines as applicable.';
  ...
  }function DistributionDonut() {// Simple textual donut placeholder until a chart lib is added;
  const total = data.reduce((a, b) => a + b.percent, 0) || 1,return (<div className="space-y-1 text-sm">;
<div className='space-y-1 text-sm'>;
      {data.map((d, idx) => (<div key={idx} className="flex items-center gap-2">;
  ...
  const parts = useMemo(() => {const sections = markdown.split(/\n## /g)const map: Record<string, string> = {}
    sections.forEach((s, i) => {if (i === 0) return; // first is H1;
      const [titleLine, ...rest] = s.split('\n')map[titleLine.trim()] = rest.join('\n')}) {  // Very lightweight section filter: split by headings;
  const parts = useMemo(() => {const sections = markdown && markdown.split(/\n## /g),const map: Record<string, string> = {}sections && sections.forEach((s, i) => {if (i === 0) return; // first is H1;
      const [titleLine, ...rest] = s && s.split('\n')map[titleLine && titleLine.trim()] = rest && rest.join('\n')</div>;
}) {  // Very lightweight section filter: split by headings;
    })return map;  }, [markdown])const content  = parts[activeSection] |'';return (case 'EU':;
      return 'Designed for utility under EU frameworks; subject to MiCA and local guidelines as applicable.';
    case 'SG':;
      return 'Intended utility token under MAS guidance; prospective purchasers should not view it as capital markets products.';
    case 'AE':;
...
function MarkdownPreview() {  // Very lightweight section filter: split by headings;
  const parts = useMemo (() => {const sections = markdown.split (/\n## /g),const map: Record < string, string> = {}
    sections.for_each ((s, i) => {// Check condition;
if (return) {$2;
} // first is H1;
      const [title_line, ...rest] = s.split ('\n')map[title_line.trim ()] = rest.join ('\n')})return map;  }, [markdown])const content  = parts[active_section] || '';return (<pre className='whitespace - pre - wrap text - sm leading - 6'>;
      {content || markdown}
    </pre>);
      ))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
...
function MarkdownPreview() {// Very lightweight section filter: split by headings;
  const parts = useMemo(() => {const sections = markdown.split(/\n## /g),const map: Record<string, string> = {},sections.forEach((s, i) => {if (i === 0) return, // first is H1;
      const [titleLine, ...rest] = s.split('\n'),map[titleLine.trim()] = rest.join('\n')}),return map;
  }, [markdown]),const content = parts[activeSection] || '',return (<pre className="whitespace-pre-wrap text-sm leading-6">{content || markdown}</pre>;
  )function MarkdownPreview() {// Very lightweight section filter: split by headings;
  const parts = useMemo(() => {const sections = markdown.split(/\n## /g)const map: Record<string, string> = {}sections.forEach((s, i) => {if (i === 0) return, // first is H1;
      const [titleLine, ...rest] = s.split('\n')map[titleLine.trim()] = rest.join('\n')}),return map;
  }, [markdown])const content = parts[activeSection] || '';
  return (<pre className="whitespace-pre-wrap text-sm leading-6">{content || markdown}</pre>;
  )} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}return map;
  }, [markdown])const content  = parts[activeSection] || '';return (<pre className='whitespace-pre-wrap text-sm leading-6'>;
      {content || markdown}
    </pre>;
  )return (<pre className='whitespace-pre-wrap text-sm leading-6'>;
      {content |markdown}
    </pre>;
  )