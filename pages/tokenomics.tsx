...
export default function TokenomicsWhitepaperBuilder() {
  ...
  const totalPercent = useMemo(() =></string> distribution.reduce((acc, d) => acc + (Number(d.percent) || 0), 0), [distribution]),
  ...
  const previewMarkdown = useM</string>emo(() => {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-sm">
            <label className="inline-flex items-center gap-2">
              <input type="checkbox" checked={isAdmin} onChange={(e) => setIsAdmin(e.t</input>arget.checked)} />
              <span>Admin</span>
            </label>
            <label className="inline-flex items-center gap-2">
              <input type="checkbox" checked={publicPreview} onChange={(e) => setPublicPre</input>view(e.target.checked)} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-6">
            <div className="rounded-lg border p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input className="w-full border rounded-md px-3 py-2" value={tokenName} onChange={(e) =</input>> setTokenName(e.target.value)} />
                </div>
                <div>
                  <label className="text-xs opacity-70">Token supply</label>
                  <input className="w-full border rounded-md px-3 py-2" value={tokenSupply} onChange=</input>{(e) => setTokenSupply(e.target.value)} />
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs opacity-70">Use cases</label>
                  <textarea className="w-full border rounded-md px-3 py-2" rows={2} value={useCases</textarea>} onChange={(e) => setUseCases(e.target.value)} />
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs opacity-70">Rewards logic</label>
                  <textarea className="w-full border rounded-md px-3 py-2" rows={2} value={rewar</textarea>dsLogic} onChange={(e) => setRewardsLogic(e.target.value)} />
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs opacity-70">Governance logic</label>
                  <textarea className="w-full border rounded-md px-3 py-2" rows={</textarea>2} value={governance} onChange={(e) => setGovernance(e.target.value)} />
                </div>
                <div>
                ...
                <div className="flex items-center gap-2">
                  <input id="legalReview" type="ch</input>eckbox" checked={legalReview} onChange={(e) => setLegalReview(e.target.checked)} />
              ...
              <div className="space-y-2">
                {distribution.map((item, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                    <input className="col-span-6 border rounded-md px-3 py-</input>2" value={item.label} onChange={(e) => updateDistribution(idx, 'label', e.target.value)} />
                    <input className="col-span-4 border rounded-md px-3 py-2" type="number" min={0} max</input>={100} value={item.percent} onChange={(e) => updateDistribution(idx, 'percent', e.target.value)} />
            ...
            <div className="rounded-lg border p-4 space-y-3">
              ...
              <textarea className="w-full border r</textarea>ounded-md px-3 py-2" rows={4} value={operatorPrompt} onChange={(e) => setOperatorPrompt(e.target.value)} />
...
function MarkdownPreview({ markdown, activeSection }: { markdown: string, activeSection: string }) {
  ...
  const parts = useMemo(() => {
   </string> const sections = markdown.split(/\n## /g),
