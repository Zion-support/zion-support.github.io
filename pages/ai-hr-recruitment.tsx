...
export default function AIHRRecruitmentPage() {const service  = null;import {Phone;
  Mail;
  MapPin;
  Check;
  ArrowRight;
...
import React from 'react';
  Phone,Mail,MapPin,Check,ArrowRight,Star,Users} from 'lucide-react';
 ...
 */;
function AIHRRecruitmentPage() {const service = enhancedRealMicroSaasServices.find (string =>;
...
if (return null) {$2;
  ...
  return (<Layout>;
      ...
      </main>;export default function AIHRRecruitmentPage() {const service = enhancedRealMicroSaasServices.find(s =>;
    s.link.endsWith('/ai-hr-recruitment'))if (!service) return null;export default function AIHRRecruitmentPage() {const service = enhancedRealMicroSaasServices.find(s => s.link.endsWith('/ai-hr-recruitment'))if (!service) return null;import { Phone, Mail, MapPin, Check, ArrowRight, Star, Users } from 'lucide-react',export default function AIHRRecruitmentPage() {const service = enhancedRealMicroSaasServices.find(s => s.link.endsWith('/ai-hr-recruitment'))if (!service) return null,s.link.endsWith('/ai-hr-recruitment'))if (!service) return null;return (<Layout>;
      <Head>;
        <title>{service.name} - Zion Tech Group</title>;
        ...
        <div className="max-w-6xl mx-auto">;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">;
            <div className="lg:col-span-2 bg-black/30 rounded-2xl border border-teal-500/30 p-6">;
              ...
              <p className="text-slate-300 mb-6">{service.description}</p>;
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">;
                {service.features.slice(0, 12).map((feat, i) => (<li key={i} className="flex items-start space-x-3 text-slate-200 w-5 h-5 text-teal-400 mt-0.5"><Check /><span>{feat}</span></li>;
}</div>;
    </Layout>;
  )}<li;
                    key={i}
