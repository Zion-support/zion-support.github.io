import React from 'react';'import { motion } from 'framer-motion';'import { SEO } from '../components/SEO';'import { Button } from '../components/ui/Button';'import { Card } from '../components/ui/Card';'import { ;'  Home, ArrowLeft,
  Search, HelpCircle,
  Phone, Mail,
  Globe, FileText,
  Briefcase, Users,
  ...
  Calendar, Award,
  Shield
} from 'lucide-react';''const NotFound: React.FC = () => {
  const quickLinks = [;
      ...
      <div className="max-w-4xl mx-auto text-center">"        <motion.div"          initial={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <motion.div
            ...
            <p className="text-gray-400">"              Don&apos;t worry, we&apos;ll help you find what you&apos;re looking for.''            </p>'          </motion.div>
          ...
          <motion.div
            ...
            <Button 
              size="lg" "              variant="outline""              onClick={() => window.location.href = '/'}'            >'              <Home className="w-5 h-5 mr-2" />"              Go Home"            </Button>
          </motion.div>
          ...
          >
            <Card className="p-8">"              <h3 className="text-2xl font-bold text-white mb-6">Popular Pages</h3>"              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">"                {quickLinks.map((link, index) => ("                  <motion.a
                    ...
                    className="flex flex-col items-center p-4 rounded-lg bg-slate-700 hover:bg-slate-600 transition-colors group""                  >"                    <link.icon className="w-6 h-6 text-blue-400 group-hover:text-blue-300 mb-2" />"                    <span className="text-gray-300 group-hover:text-white text-sm text-center">"                      {link.name}"                    </span>
                  </motion.a>
                ))}
              </div>
            </Card>
