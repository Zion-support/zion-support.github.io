        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[ 'Cluster-wide RBAC inventoryOver-permission & wildcard detectionUnused binding analysisLeast-privilege recommendations' ].map((f) => (
              <div key={f} className="p-4 rounded-xl bg-black/40 border border-sky-500/30 text-slate-200">{f}</div>            {[ 'Cluster-wide RBAC inventory', _'Over-permission & wildcard detection', _'Unused binding analysis', _'Least-privilege recommendations' ].map((f) => (
