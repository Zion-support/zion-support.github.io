...
import React, { useState } from 'react';
    ...
    supportingMultiverses: 'Digital Labor, AI Ethics',promptAssist: 'Write a proposal for the UN Development Program on integrating Zion into their Digital Labor Initiative. Include metrics, social outcomes, and DAO-based governance logic.',language: 'en'})const [loading, setLoading] = useState(false)const [result, setResult] = useState<any>(null),const [translated, setTranslated] = useState<string>(''),const onChange = null;
      setResult((r: any) => ({ ...r, meta: data.meta }))} finally { setLoading(false) }
    ...
    language: 'en';
  })const [loading, setLoading] = useState(false)const [result, setResult] = useState<any>(null)const [translated, setTranslated]  = useState<string>('')const onChange = (e: React.ChangeEvent<;
      HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
    >;
  ) => {const { name, value } = e.target;
    setForm(f => ({ ...f, [name]: value }))}
    setForm(f => ({ ...f, [name]: value }))}async function generate() {setLoading(true)try {const res = await fetch('/api/proposals/generate', {method: 'POST';
        ...
        <div className='space-y-3'>;
          ...
          <label className='block'>;
            <span className='text-sm'>;
              Supporting multiverse(s) (comma separated)</span>;
            <input;
          ...
          <div className='flex gap-3'>;
            <button;
              ...
              {loading ? 'Working…' : 'Generate Proposal'}</button>;
          </div>;
        </div>;
        <div className='space-y-3'>;
          ...
          {translated && (<div className='border rounded p-3 h-60 overflow-auto whitespace-pre-wrap bg-gray-50'>;{translated}
            </div>;
          )}
          <div className='flex items-center gap-2'>;
            <button;
              onClick={exportArtifacts}disabled={loading || !result}
              className='px-3 py-2 border rounded'>;
          ...
          {result?.meta && (<div className='text-sm space-y-1'>;
              <div>;<span className='font-medium'>Status:</span>{' '}
                {result && result.meta.status}
              </div>;
              {result && result.meta.artifacts?.markdownPath && (<div>;
                  <a;
              ...
              {result && result.meta.artifacts?.pdfPath && (<div>;
                  <a;
                    className='text-blue-600 underline';
              ...
              {result && result.meta.artifacts?.ipfsCid && (<div>IPFS CID: {result && result.meta.artifacts && artifacts.ipfsCid}</div>;
              )}
              {result && result.meta.artifacts?.signature && (<div>Signature: {result && result.meta.artifacts && artifacts.signature.slice(0, 30)}…;
                </div>              )}
            </div>;
          )}</div>;
      </div>;
...
<label className='block'>;
          ...
          {translated && (<div className="border rounded p-3 h-60 overflow-auto whitespace-pre-wrap bg-gray-50">;
              {translated  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
            </div>;
          ...
          {result?.meta && (<div className="text-sm space-y-1">;
              ...
              {result.meta.artifacts?.markdownPath && (<div><a className="text-blue-600 underline" href={result.meta.artifacts.pdfPath} target="_blank" rel="noreferrer">PDF</Link></div>;
              )  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
  return (<div className='space - y-6'>;
      <h1 className='text - 2xl font - semibold'>Global Outreach: UN Bridge</h1>;
      <div className='grid md:grid - cols - 2 gap - 6'>;
        ...
        <div className='space - y-3'>;
          ...
          {result?.meta && (<div className='text - sm space - y-1'>;
              ...
              {result.meta.artifacts?.markdown_path && (<div>;
                  <a;
                    className='text - blue - 600 underline';
              ...
              {result.meta.artifacts?.pdf_path && (<div>;
                  <a;
                    className='text - blue - 600 underline';
              ...
              {result.meta.artifacts?.ipfs_cid && (<div > IPFS CID: {result.meta.artifacts.ipfs_cid}</div>)}
              {result.meta.artifacts?.signature && (<div>Signature: {result.meta.artifacts.signature.slice (0, 30)}…;
                </div>              )}
            </div>)}
        </div>;
      </div>;
              </div>;
              {result.meta.artifacts?.markdownPath && (<div>;
                  <a;
                    className='text-blue-600 underline';
              ...
              {result.meta.artifacts?.pdfPath && (<div>;
                  <a;
                    className='text-blue-600 underline';
              ...
              {result.meta.artifacts?.ipfsCid && (<div>IPFS CID: {result.meta.artifacts.ipfsCid}</div>;
              )}
              {result.meta.artifacts?.signature && (<div>Signature: {result.meta.artifacts.signature.slice(0, 30)}…;
                </div>;
              )}
