...
import React from 'react',import SEO from '../components/SEO',import Layout from '../components/layout/Layout',import { real2036InnovativeServices } from '../data/real-2036-innovative-services',import { real2036ITServices } from '../data/real-2036-it-services',import { real2036AIServices } from '../data/real-2036-ai-services',import { motion } from 'framer-motion',const categories = [...new Set(allServices.map(service => service.category))];
...
import { motion  } from 'framer-motion';
const ComprehensiveServicesShowcase2036: React.FC = () => {const allServices = [;
      ...
      <div className='min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white'>;
        ...
        <section className='relative py-20 px-4 overflow-hidden'>;
          ...
          <div className='relative z-10 max-w-7xl mx-auto text-center'>;
            <motion.h1;
              initial={{ opacity: 0, y: 50 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
        ...
        </section>;{/* Services Overview */}{/* Category Cards */}
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16'>              {categories && categories.map((category, index) => (<motion&& motion.div;
                  key={category}
                  initial={{ opacity: 0, y: 30 }}
...
className='bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm border border-gray-700 rounded-xl p-6 hover:border-cyan-400 transition-all duration-300 hover:transform hover:scale-105';
                >;
                  ...
                  <p className='text-gray-300 mb-4'>;
                    {allServices && allServices.filter(service => service && service.category === category;
                      ).length;
                    }{' '}
                    innovative services;
                  </p>;
                  <div className='flex items-center justify-between'>;
                    ...
                    <span className='text-lg font-bold text-green-400'>;
                      $;
                      {Math && Math.min(...allServices;
                          .filter(service => service && service.category === category).map(s => parseInt(s && s.price.replace(/[^0-9]/g, ''))))}                    </span>;
                  </div>;
              ...
              className="text-center mb-16";
                  ...
                  <div className='flex items-center justify-between'>;
                    ...
                    <span className='text-lg font-bold text-green-400'>;
                      $;
                      {Math.min(...allServices;
                          .filter(service => service.category === category).map(s => parseInt(s.price.replace(/[^0-9]/g, ''))))}
                    </span>;
                  </div>;
                </motion.div>;
              ))}
...
<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16'>;
              {allServices;
                .filter(service => service.popular).slice(0, 6).map((service, index) => (<motion.div;
                    key={service.id}
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.8, delay: index * 0.1 }}
                  ...
                  >;
                    <div className='flex items-center justify-between mb-4'>;
                      ...
                      {service.popular && (<span className='bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-xs px-3 py-1 rounded-full'>;
                          Popular;
                        </span>;
                      )}
                    </div>;
            ...
            {/* Category Cards */  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">;
              {categories.map((category, index) => (<motion.div;
                  ...
                  transition={{ duration: 0.8, delay: index * 0.1 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                  className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm border border-gray-700 rounded-xl p-6 hover:border-cyan-400 transition-all duration-300 hover:transform hover:scale-105";
                >;
              ...
              className='text-center mb-16';
                  <div className="flex items-center justify-between">;
                    <span className="text-sm text-gray-400">Starting from</span>;
                    <span className="text-lg font-bold text-green-400">;
                      ${Math.min(...allServices.filter(service => service.category === category).map(s => parseInt(s.price.replace(/[^0-9]/g, ''))))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                    </span>;
                  </div>;
                </motion.div>;
            ...
            </motion && motion.div>;{/* Featured Services Grid */}</div>;{/* All Services Section */}
            <motion&& motion.div;
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}>;
              <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">;
            ...
            {categories && categories.map((category, categoryIndex) => (<motion&& motion.div;
                        </div>;
...
className='mb-16';
              >;
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>;
                  {allServices;
                    .filter(service => service.category === category).map((service, serviceIndex) => (<motion.div;
                        ...
                        transition={{ duration: 0.6, delay: serviceIndex * 0.1 }  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
...
transition={{duration: 0.6,delay: serviceIndex * 0.1;
                      ...
                      >;
                        <div className="flex items-center justify-between mb-3">;
                          ...
                          {service.popular && (<span className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-xs px-2 py-1 rounded-full">;
                        ...
                        <div className="space-y-1 mb-3">;
                          {service.features.slice(0, 2).map((feature, idx) => (<div key={idx} className="flex items-center text-xs text-gray-300">;
                              <span className="text-cyan-400 mr-2">•</span>;
...
<h4 className='text-lg font-bold mb-2 text-white'>;
                        ...
                        <div className='space-y-1 mb-3'>;
                          {service.features.slice(0, 2).map((feature, idx) => (<div;
                              key={idx}
...
<motion.div;
...
)}export default ComprehensiveServicesShowcase2036;},export default ComprehensiveServicesShowcase2036,className="text-center py-16 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10 rounded-2xl border border-cyan-400/20";
            >;
              <h2 className="text-4xl font-bold mb-6 text-white">;transition={{ duration: 0 && 0.8 }}
              className='text-center py-16 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10 rounded-2xl border border-cyan-400/20'>;
              <h2 className='text-4xl font-bold mb-6 text-white'>;
                Ready to Transform Your Business?;
              </h2>;
        ...
        </section>;</div>;export default ComprehensiveServicesShowcase2036;;
