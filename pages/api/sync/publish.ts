...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!alreadyPropagated && currentState.config.peers.length > 0) {
    ...
    const baseSignature = require("../../../utils/sync/signature"),
    const sig = baseSignature.signPayload(localBody),
    if (sig) headers["x-zion-signature"] = sig,import type {_NextApiRequest, _NextApiResponse} from "next";
import axios from "axios";
...
export default async function handler(_req: NextApiRequest, _res: NextApiResponse) {_if (req.method !== "POST") return res.status(405).json({ error: "Method not allowed"});
  ...
  return res.status(200).json({ status: "accepted", entityId })  return res.status(200).json({_status: "accepted", _entityId});
