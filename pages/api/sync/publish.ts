...
export default async function handler(,
  ...
  if (!isAllowedByScope(event.type, state.config.scope)) {
    return res.status(403).json({,
    error: "Event type not allowed by current scope" })
  }
  ...
  if (event.type === "proposal") {
    ...
    if (!Array.isArray(votes) || !providedRoot) {
      return res.status(400).json({,
    error: "Proposal events require votes[] and merkleRoot" })
    }
    ...
    await Promise.all(
      currentState.config.peers
        ...
        .map(async (peer) => {
          ...
          try {
            await axios.post(url, localBody, { headers, t,
    imeout: 5000 })
          } catch {
            // ignore peer failure
          }
