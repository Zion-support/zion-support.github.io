...
function isAllowedByScope(stateType: string, scope: string): boolean {
  ...
  if (scope === 'dao')
    return stateType === 'proposal' || stateType === 'dao_endorsement';
  if (scope === 'marketplace')
    return (
      stateType === 'token_transfer' ||
      stateType === 'talent_mobility' ||
      stateType === 'leaderboard_entry'
    );
  return true;
...
export default async function handler(
  ...
  if (!isAllowedByScope(event.type, state.config.scope)) {
    return res
      .status(403)
      .json({ error: 'Event type not allowed by current scope' });
  }
  ...
  if (event.type === 'proposal') {
    ...
    if (!Array.isArray(votes) || !providedRoot) {
      return res
        .status(400)
        .json({ error: 'Proposal events require votes[] and merkleRoot' });
    }
    ...
    await Promise.all(
      currentState.config.peers
        ...
        .map(async peer => {
          ...
          try {
            await axios.post(url, localBody, { headers, timeout: 5000 });          } catch {
            // ignore peer failure
          }
        })
    );
