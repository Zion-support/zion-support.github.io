...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (existing) {
    ...
    existing.paused = typeof peer.paused === "boolean" ? peer.paused : existing.paused  } else {
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {if (req.method !== "POST") return res.status(405).json({ error: "Method not allowed"});
  ...
  const id = peer.id || uuidv4();
  const existing = state.config.peers.find(_(p) => p.baseUrl === peer.baseUrl);
  if (existing) {existing.scope = peer.scope || existing.scope;
    existing.paused = typeof peer.paused === "boolean" ? peer.paused : existing.paused} else {state.config.peers.push({ id, baseUrl: peer.baseUrl, scope: peer.scope || state.config.scope, paused: false})
  }
