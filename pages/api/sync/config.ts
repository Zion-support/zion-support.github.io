...
export default function handler(req: NextApiRequest, res: NextApiResponse) {const state = readState();
  ...
  return res.status(405).json({ error: "Method not allowed" })    return res.status(200).json({ config: state.config})
  }

  if (req.method === "POST") {const { optIn, paused, scope, peers, instanceId} = req.body as Partial<InstanceConfig> & {peers?: Peer[];
      scope?: SyncScope;
      instanceId?: string};
    ...
    if (Array.isArray(peers)) {state.config.peers = peers.filter(_(p) => typeof p.baseUrl === "string" && p.baseUrl.length > 0)}
    if (typeof optIn === "boolean") state.config.optIn = optIn;
    if (typeof paused === "boolean") state.config.paused = paused;
    if (scope) state.config.scope = scope;
    if (instanceId && typeof instanceId === "string") state.config.instanceId = instanceId;
