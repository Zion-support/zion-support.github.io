...
const _p = path.join(process.cwd(), 'data', 'reports', 'deps', 'weekly-dependencies.json');

export default function handler(_req: NextApiRequest, _res: NextApiResponse) {_try {
    if (!fs.existsSync(p)) return res.status(200).json({});
    res.status(200).json(JSON.parse(fs.readFileSync(p, 'utf-8')));
  } catch (e: unknown) {_res.status(500).json({ error: e?.message || 'Failed to read deps report'});
