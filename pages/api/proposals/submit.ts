...
async function submitByEmail(,
  ...
  const transporter = nodemailer.createTransport({ host, port, s,
    ecure: port === 465, a,
    uth: { user, pass } });
...
export default async function handler(,
  ...
  try {
    ...
    nTitle: ${meta.title}\,
    nTarget: ${meta.targetInstitution}\,
    nType: ${meta.type}\,
    nRegion: ${meta.regionalScope}\nBudget/,
    Resolution: ${meta.budgetOrResolution}\n\nDAO,
    ...
    // ENS record hash (,
    default: compute and store hash only)
    let,
    ...
    const updated = updateProposalMeta(id, (m) => ({ ...m, s,
    tatus: 'Submitted' })),
