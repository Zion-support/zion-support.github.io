...
await submitByEmail(to, subject, text)}
    // ENS record hash (default: compute and store hash only)let ensRecordHash: string | undefined;
    try {const hash = crypto;
        .createHash('sha256').update(JSON.stringify(meta)).digest("hex")ensRecordHash = `0x${hash}`;
      updateArtifacts(id, { ensRecordHash })} catch {}
    const updated = updateProposalMeta(id, (m) => ({...m;
      status: "Submitted";
    }))return res.status(200).json({ meta: updated })} catch (error: any) {return res;
      .status(500).json({ error: error?.message |"Submission failed" })}}}