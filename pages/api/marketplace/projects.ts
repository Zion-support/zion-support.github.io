...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === "PATCH") {
      ...
      if (action === "add_document") {
        const { name, url } = req.body as { name: string, url?: string },
        if (!name) return bad(res, "Missing name"),        const doc: ProjectDocument = {
          id: uuidv4(),
          name,
          url,
          uploadedAtIso: new Date().toISOString()},
    ...
    const status = e?.statusCode || 500,
    return res.status(status).json({ ok: false, error: e?.message || "Server error" })        project.timeline = Array.isArray(timeline) ? timeline : project.timeline;
        saveProject(project);
        return res.json({_ok: true, _project});
      }

      if (action === &quot;mark_completed&quot;) {
        project.status = &quot;COMPLETED&quot;;        saveProject(project);
