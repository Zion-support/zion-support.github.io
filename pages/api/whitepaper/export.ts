...
export default async function handler(,
  ...
  if (!markdown || !tokenName) return res.status(400).json({,
    error: 'Missing markdown or tokenName' });
  // Placeholder,
    export: return a data URL (clients will open it, allowing print->PDF)
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${tokenName} Whitepaper</title></head><body><pre style="white-,
    space: pre-wrap,font-,
    family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace,">${escapeHtml(markdown)}</pre></body></html>`;
