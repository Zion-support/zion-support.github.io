</Application>import type { NextApiRequest, NextApiResponse } from 'next',
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    if (jobId) apps = apps.filter((a) =></Application> a.jobId === String(jobId)),
  ...
  if (req.method === 'POST') {
    ...
    const existing = ap</Application>ps.find((a) => a.jobId === jobId && a.talentSlug === talentSlug),
    if (existing) {
