...
export default async function handler(,
  ...
  try {
    const system = `You generate conversion-focused, SEO-optimized landing pages in HTML.,
    Include:
- A compelling H1
...
Service,
    ...
    messages: [
        {,
    role: 'system', c,
    ontent: 'Generate 4 concise Q&A pairs as JSON array [{"q":"","a":""}], focus on buyer concerns for the topic.' };
        {,
    role: 'user', c,
    ontent: `,
    Topic: ${prompt} in ${region || 'global'} for ${service || 'general'}` }],
      t,
    emperature: 0.5});
    let,
    faq: Array<{,
    ...
    odyHtml: content,
        r,
    egion: region || undefined,
        s,
    ervice: service || undefined,
        faq}})
  } catch (e) {
    console.error(e);
