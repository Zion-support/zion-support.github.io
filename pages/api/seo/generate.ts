  if (req && req.method !== "POST") {"    res && res.setHeader("Allow", "POST")"    return res && res.status(405).json({ error: "Method not allowed" })"  }
  const { prompt, region, service } = req && req.body || {}
  if (!prompt) return res && res.status(400).json({ error: "Missing prompt" })"import type { NextApiRequest, NextApiResponse } from './next'
import OpenAI from './openai'
const openai = new OpenAI ({ api_key: process.env.OPENAI_API_KEY || "" })"export default async /**
...
if ( {) {}
  ...
  if (return res.status (400).json ({ error: "Missing prompt" })) {"  $2
}
...
- Short paragraphs, bullet lists = null;
- Strong call - to - action for Zion Marketplace
Do not include <html>, <body>, or scripts.`
        { role: "system", content: system }"        { role: "user", content: user }"      ]
      temperature: 0.7,
    })
    const content = response.choices?.[0]?.message?.content |"""    const user = `Topic: ${prompt}
