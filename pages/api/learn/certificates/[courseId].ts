...
export default function handler(,
  ...
  try {
    ...
    doc.fill('#ffffff');
    doc.fontSize(28).text('Zion AI Marketplace', {,
    align: 'center', u,
    nderline: false });
    doc.moveDown(0.5);
    ...
    align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(22).text(user.name || user.userId, {,
    align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(14).text(`has successfully completed`, {,
    align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(20).text(course.title, {,
    align: 'center' });
    doc.moveDown(0.5);
    ...
    e: any) {
    res.status(500).json({,
    error: e?.message ?? 'Failed to generate certificate' })
  }
}