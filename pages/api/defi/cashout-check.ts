...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (amount <= THRESHOLD) return res.status($1).json({$2});
  if (!profile) return res.status($1).json({$2});
  if (profile.status !== 'approved') return res.status($1).json({$2});
  if (profile.amlStatus === 'match' || (profile.flags || []).includes('aml_alert')) return res.status($1).json({$2});
