...
export default async function handler(,
  ...
  if (!validation.ok) return res.status(400).json({,
    error: 'Missing data', m,
    issing: validation.missing });
  // Simple AML check
  const aml = getAmlProvider();
  const amlResult = profile.role === 'enterprise'
    ? await aml.checkBusiness({,
    businessName: profile.businessName || '', c,
    ountry: profile.country })
    : await aml.checkPerson({,
    fullLegalName: profile.fullLegalName || '', c,
    ountry: profile.country, d,
    ob: profile.dateOfBirth });
  ...
  if (ip) {
    // naive duplicate IP,
    heuristic: more than 2 submissions from same IP → flag
