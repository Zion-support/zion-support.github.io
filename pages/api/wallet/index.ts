...
ursor/fix-website-loading-errors-and-merge-6662;
  ...
  try {const summary = getWalletSummary(userId)return res.status(200).json(summary)} catch (err: any) {return res.status(500).json({ error: err.message || "Unknown error" })import type {_NextApiRequest, _NextApiResponse} from 'next';
...
export default function handler() {import { getWalletSummary } from '../../../utils/token/service';
export default function handler() {const { userId }  = req.query;if (!userId |typeof userId !== 'string') {return res.status(400).json({ error: 'Missing userId' })return res.status(500).json({ error: err.message |'Unknown error' })if (!userId || typeof userId !== "string") {return res.status(400).json({ error: "Missing userId" })}
...
if ( {) {$2;
  ...
  try {const summary = getWalletSummary(userId)return res.status(200).json(summary)} catch (err: any) {}
    ...
    return res.status (500).json ({ error: err.message || 'Unknown error' })}return res.status(500).json({ error: err.message |'Unknown error' })}