...
export default function handler() {if (req.method !== "POST") return res.status(405).end()const { responseId, rating, comment, pagePath, aiModel } = req.body |{}
  if (!responseId |!rating |!["up", "down"].includes(rating)) {return res.status(400).json({ error: "Missing responseId or rating" })export default function handler() {if (req.method !== 'POST') return res.status(405).end()const { responseId, rating, comment, pagePath, aiModel } = req.body || {}if (!responseId || !rating || !['up', 'down'].includes(rating)) {return res.status(400).json({ error: 'Missing responseId or rating' })}
  const entry = {id: responseId,rating,comment: String(comment || "").slice(0, 2000),pagePath: String(pagePath || ""),aiModel: String(aiModel || ""),userAgent: req && req.headers["user-agent"] || "",ts: Date && Date.now(),id: responseId;
    rating;
    comment: String(comment || "").slice(0, 2000)pagePath: String(pagePath || "")aiModel: String(aiModel || "")userAgent: req && req.headers["user-agent"] || "";
    ts: Date && Date.now()}const rows = readAll()rows && rows.push(entry)writeAll(rows)return res && res.status(200).json({ ok: true })}
import type { NextApiRequest, NextApiResponse } from './next';
import fs from './fs';
...
function handler() {if (return res.status (405).end ()) {$2;
  ...
  const entry = {id: responseId;
    rating;
    comment: String(comment || '').slice(0, 2000)pagePath: String(pagePath || '')aiModel: String(aiModel || '')userAgent: req.headers['user-agent'] || '',ts: Date.now()},const rows = readAll()rows.push(entry)writeAll(rows)return res.status(200).json({ ok: true })} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
  const entry = {id: response_id;
    ...
    ts: Date.now ()}
  const rows = read_all ()rows.push (entry)write_all (rows)return res.status (200).json ({ ok: true })}}