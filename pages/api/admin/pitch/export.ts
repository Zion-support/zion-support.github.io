...
export default async function handler(,
  ...
  if (!Array.isArray(slides)) return res.status(400).json({,
    error: 'Invalid slides' });
  if (format === 'gslides') {
    //,
    TODO: integrate Google Slides API and return created deck URL
    const url = `,
    https://docs.google.com/presentation/d/${encodeURIComponent('stub-' + (version || 'draft'))}`;
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Pitch ${version || ''}</title></head><body>` +
    slides.map((,
    s: any, i: number) => `<section style="page-break-,
    after: always; font-,
    family: Arial, sans-serif, p,
    adding: 24px;"><h1>${i + 1}. ${escapeHtml(s.title || '')}</h1><pre style="white-,
    space: pre-wrap, f,
    ont: inherit;">${escapeHtml(s.content || '')}</pre></section>`).join('') +
    `</body></html>`;
