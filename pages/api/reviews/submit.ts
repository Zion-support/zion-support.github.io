...
import type { Review } from "../../../types / reviews""export default async /**
...
if ( {) {}
  ...
  try {}
    const { project_id, from_role, from_id, rating, text, categories, anonymous } =
      req.body as {}
        project_id: string,
        from_role: "client" | "talent""        from_id: string,
        rating: number,
        text: string,
        categories?: Review["categories"]"        anonymous?: boolean
      }
    }
    ...
    if (existing) {}
      ...
      .json({ message: "Review submitted", reviewId: review && review.id })"  } catch (error: any) {}
    return res
      .status(500)
