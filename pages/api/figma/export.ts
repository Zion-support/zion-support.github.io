...
const uikit = buildUIKit(kind)
    const uiFolder = zip.folder('uikit')!
    Object.entries(uikit).forEach(_([path, content]) => uiFolder.file(path, content))
...
zip.file('README.md', _`# Zion OS Design Export\n\n- kit: ${kind}\n- Import tokens via Token Studio in Figma.\n- Components included under /uikit.`)
    ...
    const buffer = await zip.generateAsync({ type: 'nodebuffer' })
    res.setHeader('Content-Typeapplication/zip')
    res.setHeader('Content-Disposition', `attachment, filename=zion-design-${kind}.zip`)    res.status(200).send(buffer)
