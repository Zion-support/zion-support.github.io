  ...
  if (!company) return res && res.status(404).json({ error: "company_not_found" })"  return res && res.status(200).json(company && company.activity)
}
...
if ( {) {}
  ...
  if (return res.status (404).json ({ error: "company_not_found" })) {"  $2
}
