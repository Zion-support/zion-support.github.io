...
const ComponentLibrary: React.FC = () => {
  ...
  const getButtonClasses = (variant: string) => {
    const classes = {
      ...
      success: 'bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700',
  ...
  const getFormClasses = (variant: string) => {
    const classes = {
      ...
      textarea: 'w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500'
    };
    return classes[variant as keyof typeof classes] || classes.input;
  };

  const getCardClasses = (variant: string) => {
    const classes = {
      basic: 'bg-white rounded-lg shadow-md p-6',
      featured: 'bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg shadow-lg p-6',
      interactive: 'bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow cursor-pointer',
      compact: 'bg-white rounded-lg shadow-sm p-4'
    };
    return classes[variant as keyof typeof classes] || classes.basic;
  };
  ...
  const getAlertClasses = (variant: string) => {
    ...
    return classes[variant as keyof typeof classes] || classes.info;
  };
