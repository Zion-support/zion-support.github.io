  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Browse platform reports, live updates, and performance analytics." />
      </Head>
        ...
        <main className="container mx-auto px-6 py-12">
          ...
          <header className="text-center mb-12">
            <h1 className="text-5xl font-extrabold mb-4 bg-gradient-to-r from-cyan-400 to-fuchsia-400 bg-clip-text text-transparent">
              Platform Reports & Updates
            </h1>
            <p className="text-white/80 max-w-3xl mx-auto">
              The latest autonomous updates, performance metrics, and system analytics.
            </p>
          </header>
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {updates.map((slug) => (
              <Link key={slug} href={`/reports/updates/${slug}`} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-6 backdrop-blur-xl hover:border-cyan-400/30">
                ...
                <h3 className="text-lg font-semibold">Autonomous Update — {slug.replace('update-', '').replace(/-/g, ': ')}</h3>
