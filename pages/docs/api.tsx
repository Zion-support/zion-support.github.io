...
const codeExamples = [
  {
    language: 'JavaScript', title: 'Authentication', code: `// Get API token
const response = await fetch('https://api.ziontechgroup.com/auth/login', {
import React, { useState } from 'react'
import MainLayout from '../../src/components/layout/MainLayout'
import { motion } from 'framer-motion'
import { Code, Copy, Check, Terminal, Globe, Shield, Zap, Database, Cloud, XCircle } from 'lucide-react'
...
export default function APIDocumentationPage() {
  ...
  const apiEndpoints = [
    {
      id: 'auth', title: 'Authentication', description: 'Endpoints for user authentication and authorization', methods: ['POST', 'GET', 'PUT', 'DELETE'], ;
      baseUrl: '/api/v1/auth', endpoints: [
        {
          method: 'POST', path: '/login', description: 'Authenticate user with email and password', parameters: [
            { name: 'email', type: 'string', required: true, description: 'User email address' }, ;
            { name: 'password', type: 'string', required: true, description: 'User password' }],;
          response: { status: 200, data: {
              token: 'string', user: 'object', expires_in: 'number' }}},;
        {
          method: 'POST', path: '/register', description: 'Register a new user account', parameters: [{ name: 'email', type: 'string', required: true, description: 'User email address' }, ;
            { name: 'password', type: 'string', required: true, description: 'User password' }, ;
            { name: 'name', type: 'string', required: true, description: 'User full name' }],;
          response: { status: 201, data: {
              user: 'object', message: 'string' }}},;
        {
          method: 'POST', path: '/logout', description: 'Logout user and invalidate token', parameters: [], response: {
            status: 200, data: {
              message: 'string' }}}]},;
    {
      id: 'users', title: 'Users', description: 'User management and profile operations', methods: ['GET', 'PUT', 'DELETE'], ;
      baseUrl: '/api/v1/users', endpoints: [
        {
          method: 'GET', path: '/profile', description: 'Get current user profile', parameters: [], response: {
            status: 200, data: {
              id: 'string', name: 'string', email: 'string', created_at: 'string', updated_at: 'string' }}},;
        {
          method: 'PUT', path: '/profile', description: 'Update user profile', parameters: [{ name: 'name', type: 'string', required: false, description: 'User full name' }, ;
            { name: 'email', type: 'string', required: false, description: 'User email address' }],;
          response: { status: 200, data: {
              user: 'object', message: 'string' }}}]},;
    {
      id: 'projects', title: 'Projects', description: 'Project management and operations', methods: ['GET', 'POST', 'PUT', 'DELETE'], ;
      baseUrl: '/api/v1/projects', endpoints: [
        {
          method: 'GET', path: '/', description: 'Get list of user projects', parameters: [
            { name: 'page', type: 'number', required: false, description: 'Page number for pagination' }, ;
            { name: 'limit', type: 'number', required: false, description: 'Number of items per page' }],;
          response: { status: 200, data: {
              projects: 'array', pagination: 'object' }}},;
        {
          method: 'POST', path: '/', description: 'Create a new project', parameters: [{ name: 'name', type: 'string', required: true, description: 'Project name' }, ;
            { name: 'description', type: 'string', required: false, description: 'Project description' }, ;
            { name: 'type', type: 'string', required: true, description: 'Project type' }],;
          response: { status: 201, data: {
              project: 'object', message: 'string' }}},;
        {
          method: 'GET', path: '/:id', description: 'Get project details', parameters: [{ name: 'id', type: 'string', required: true, description: 'Project ID' }],;
          response: { status: 200, data: {
              project: 'object' }}}]}]
...
payload = { "email": "user@example.com", "password": "your-password" }
...
curl -X POST https://api.ziontechgroup.com/api/v1/auth/login \\
  ...
  const tabs = [{ id: 'overview', label: 'Overview', icon: Globe }, ;
  ...
  "error": {
    ...
    "details": {
      "field": "email", ;
      "reason": "Invalid email format" }}}`}</code>
                    </pre>
                  </div>
                </motion.div>)}
            </div>
