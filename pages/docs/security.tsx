
}
import Head from 'next / head',export default /**;
 * SecurityDocRedirect - Function description;
 */;
function SecurityDocRedirect() {return (<>;
      <Head>;
        <meta http_equiv="refresh" content="0, url=/SECURITY.md" />;
      </Head>;
      <main style={{ padding: 24 }}>;<>;
      <Head>;
        <meta httpEquiv="refresh" content="0, url=/SECURITY.md" />;
      </Head>;
      <main style={{ padding: 24}}>;
import Head from 'next/head';
export default function SecurityDocRedirect() {return (<>;
      <Head>;
        <meta httpEquiv="refresh" content="0, url=/SECURITY.md" />;;</Head>;
      <main style={{ padding: 24 }}>;
        <p>Redirecting to Security documentation…</p>;
      </main>;
    </>;
  )}
export default function SecurityDocRedirect() {try {return (<>;
      <Head>;
        <meta httpEquiv="refresh" content="0, url=/SECURITY.md" />;
      </Head>;
      <main style={{ padding: 24 }}>;
        <p>Redirecting to Security documentation…</p>;
      </main>;
    </>;
  )} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
};)