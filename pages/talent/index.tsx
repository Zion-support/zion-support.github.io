  ...
  return (
    <div className='space-y-6 pb-16'>      <Head>;
      ...
      <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-4'>;
        {pageItems && pageItems.map(t => (;
          <article key={t && t.id} className='border rounded-md p-4 space-y-2'>;
            <div className='flex items-center justify-between'>;
              <h3 className='font-semibold'>{t && t.name}</h3>;
              <div className='flex gap-1'>;
                {t && t.badges.map(b => (                  <TrustBadge key={b} type={b} />;
                ))}
              </div>;
            </div>;
