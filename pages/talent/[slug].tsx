...
import type { NextPage } from 'next',import Head from 'next/head',import { useRouter } from 'next/router',import { useState } from 'react',import EnhancedButton from '../../components/ui/EnhancedButton',import { TrustBadge, MicroTestimonial } from '../../components/ui/Badges',const TalentProfile: NextPage = () => {const router = useRouter(),const { slug } = router.query,return (<div className="space-y-6 pb-20">;
      ...
      <header className="flex items-start justify-between gap-4">;
        ...
        <div className="flex gap-2">;
          ...
          <EnhancedButton;
            variant="ghost";
            onClick={() => handleRespond("Negotiate")}const [responseLog, setResponseLog] = useState<Array<{ action: 'Accept' | 'Decline' | 'Negotiate', at: string }>>([]),const handleRespond = (action: 'Accept' | 'Decline' | 'Negotiate') => {setResponseLog((prev) => [...prev, { action, at: new Date().toISOString() }])},return (<div className="space-y-6 pb-20">;
  return (<div className="space-y-6 pb-20">;
const [responseLog, setResponseLog] = useState<;
    Array<{ action: 'Accept' | 'Decline' | 'Negotiate'; at: string }>;
  >([])const handleRespond = (action: 'Accept' | 'Decline' | 'Negotiate') => {setResponseLog(prev => [...prev, { action, at: new Date().toISOString() }])}return (<div className='space-y-6 pb-20'>;
      <Head>;
        <title>{slug} - Talent - Zion</title>;
      </Head>;
      <header className="flex items-start justify-between gap-4">;
...
<header className='flex items-start justify-between gap-4'>;
      ...
      </section>;},export default TalentProfile;<aside className='space-y-4'>;
          <div className='border rounded-md p-4'>;
            <h3 className='font-semibold mb-2'>Response Log</h3>;
            <ul className='space-y-1 text-sm'>;
              {responseLog.length === 0 && (<li className='opacity-70'>No responses yet.</li>;
              )}
              {responseLog.map((r, idx) => (<li key={idx} className='flex items-center justify-between'>;
                  <span>{r.action}</span>;
