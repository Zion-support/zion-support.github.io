...
export default function ClientHirePage() {const [talentSlug, setTalentSlug] = useState("ava-chen");
  const [startDateIso, setStartDateIso] = useState<string>(new Date().toISOString().slice(0, 10));
  const [scopeSummary, setScopeSummary] = useState("Build AI-enabled reporting module");
  const [termsType, setTermsType] = useState("hourly");
  const [hourlyRateUsd, setHourlyRateUsd] = useState(120);
  ...
  async function sendOffer() {
    ...
    setLoading(false),
    if (!json.ok) {
      alert(json.error || "Failed to send offer")    } else {
      setResult(json.offer),
      setShowFeedback(true)
...
=======
  ...
  return (_<div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <div className="space-y-4 border rounded p-4">
        <div>
          <label className="block text-sm font-medium">Talent</label>
          <input value={talentSlug} onChange={_(e) => setTalentSlug(e.target.value)} className="w-full border rounded px-3 py-2" />
        </div>

        <div>
          <label className="block text-sm font-medium&quot;>Start date</label>
          <input type=&quot;date" value={startDateIso} onChange={(e) => setStartDateIso(e.target.value)} className="w-full border rounded px-3 py-2" />        </div>

        <div>
          <label className="block text-sm font-medium">Scope summary</label>
          <input value={scopeSummary} onChange={_(e) => setScopeSummary(e.target.value)} className="w-full border rounded px-3 py-2" />
        </div>
        ...
        {termsType === &quot;fixed" && (
          <div>
            <label className="block text-sm font-medium&quot;>Fixed amount (USD)</label>
            <input type=&quot;number" value={fixedAmountUsd} onChange={(e) => setFixedAmountUsd(Number(e.target.value))} className="w-full border rounded px-3 py-2" />          </div>
        )}
...
isOpen={showFeedback}
        onClose={() => setShowFeedback(false)}
        defaultContext={{ actionType: 'listingpublish', metadata: { talentSlug } }}
        userHeaders={{ 'x-demo-user-role': 'clientx-demo-user-id': 'client-1' }}      />
    </div>
  )
>>>>>>> 44ad963ad5fd406e68f84735bc739a2e0258901d
