  ...
  const Section = ({ title, items, color }: { title: string; items: Service[]; color: string }) => (
    <section style={{ maxWidth: 1400, margin: '0 auto' padding: '32px 20px'}}>
      <div style={{ 
        ...
        <div style={{ 
          width: 4, height: 32, background: color, borderRadius: 2 }}></div>
        <h2 style={{ 
        ...
        {items.map((s) => (
          <a key={s.name} href={s.link} style={{
            ...
            <div style={{ 
              ...
              lineHeight: 1.4, color: '#1e293b'}}>{s.name}</div>
            <div style={{ 
              opacity: 0.8, fontSize: 14, marginBottom: 12, 
              lineHeight: 1.5, color: '#64748b'}}>{s.summary}</div>
            <div style={{ 
      ...
      <section style={{ 
        ...
        <div style={{ display: 'flex' gap: 16, justifyContent: 'center' flexWrap: 'wrap'}}>
          <Link href="/contact" style={{
            ...
            transition: 'all 0.3s ease'}}>Request a Quote</Link>
          <a href="tel: +13024640950" style={{
            display: 'inline-block' 
            background: 'rgba(34, 197, 94, 0.1)' 
            color: '#22c55e' padding: '16px 32px' borderRadius: 12, 
            fontWeight: 700, textDecoration: 'none' fontSize: 16,
            border: '2px solid #22c55e' transition: 'all 0.3s ease'}}>Call +1 302 464 0950</a>
        </div>
      </section>
    </main>
  );
