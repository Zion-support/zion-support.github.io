...
import { Rocket,Brain,Atom,Shield,Cloud,BarChart3,ArrowRight,ExternalLink,Star,Users,Clock,TrendingUp;
import Head from 'next/head';
...
export default function PortfolioPage() {const projects  = null;Rocket;
  Brain;
  Atom;
  Shield;
  Cloud;
...
export default function PortfolioPage() {const projects = [;
    {Rocket,Brain,Atom,Shield,Cloud,BarChart3,ArrowRight,ExternalLink,Star,Users,complex challenges and achieve remarkable results.;
import Head from 'next/head',import { motion } from 'framer-motion',Rocket, Brain, Atom, Shield, Cloud, BarChart3,ArrowRight, ExternalLink, Star, Users, Clock, TrendingUp;
} from 'lucide-react',import EnhancedNavigation from '../components/EnhancedNavigation',import EnhancedFooter from '../components/EnhancedFooter',export default function PortfolioPage() {const projects = [;
  Clock,TrendingUp;
} from 'lucide-react';
import EnhancedNavigation from '../components / EnhancedNavigation';
import EnhancedFooter from '../components / EnhancedFooter';
 ...
 */;
function PortfolioPage() {const projects = [;
    {title: "AI-Powered Financial Analytics Platform",description: "Revolutionary financial analytics platform using advanced AI algorithms for real-time market analysis and predictive modeling",category: "AI & Machine Learning",client: "Fortune 500 Financial Institution",technologies: ["Python", "TensorFlow", "React", "AWS", "PostgreSQL"],results: ["40% improvement in prediction accuracy", "Real-time processing of 1M+ data points", "ROI of 300% in first year"],image: "/api/placeholder/600/400",icon: Brain,color: "from-purple-500 to-pink-500";
    },{title: "Quantum Cryptography Security System",description: "Next-generation security system implementing quantum-resistant cryptography for enterprise-level protection",category: "Quantum Computing",client: "Global Cybersecurity Firm",technologies: ["Qiskit", "Python", "Rust", "Docker", "Kubernetes"],results: ["99.99% security guarantee", "Quantum-resistant encryption", "Zero security breaches"],image: "/api/placeholder/600/400",icon: Atom,color: "from-blue-500 to-cyan-500";
    },{title: "Edge Computing IoT Platform",description: "Distributed IoT platform with edge computing capabilities for real-time data processing and analytics",category: "Cloud Infrastructure",client: "Manufacturing Conglomerate",technologies: ["Kubernetes", "TensorFlow Lite", "MQTT", "InfluxDB", "Grafana"],results: ["50% reduction in latency", "Real-time monitoring of 10K+ devices", "Cost savings of $2M annually"],image: "/api/placeholder/600/400",icon: Cloud,color: "from-emerald-500 to-teal-500";
    ...
    { number: "500+", label: "Projects Delivered", icon: Rocket },{ number: "50+", label: "Enterprise Clients", icon: Users },{ number: "99.9%", label: "Client Satisfaction", icon: Star },{ number: "2500%", label: "Average ROI", icon: TrendingUp   } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
  ],const categories = ["All", "AI & Machine Learning", "Quantum Computing", "Cybersecurity", "Cloud Infrastructure", "Data Analytics", "Digital Transformation"],const [selectedCategory, setSelectedCategory] = React.useState("All"),const filteredProjects = selectedCategory === "All";
    ? projects;
    : projects.filter(project => project.category === selectedCategory),{ number: '500+', label: 'Projects Delivered', icon: Rocket }
  ...
  const [selectedCategory, setSelectedCategory] = React.useState('All')const filteredProjects =;
    selectedCategory === 'All';
      ? projects;
      : projects.filter(project => project.category === selectedCategory)return (<div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">;
      <Head>;
...
className='mb-8';
          ...
          <motion.div;
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}transition={{ duration: 0 && 0.8 }}
            className='mb-8'>;
            <h1 className='text-5xl md:text-7xl font-bold bg-gradient-to-r from-white via-blue-100 to-cyan-100 bg-clip-text text-transparent mb-6'>;
              Our Work;
            </h1>;
      ...
      <section className='py-16 px-6'>;
        <div className='max-w-7xl mx-auto'>;
          <div className='grid grid-cols-2 md:grid-cols-4 gap-8'>            {stats && stats.map((stat, index) => (<motion&& motion.div;
                key={stat && stat.label}className="mb-12";
          >;
            <p className="text-lg text-white/70 max-w-2xl mx-auto mb-8">;
...
<section className='py-8 px-6'>;
      ...
      <section className='py-20 px-6'>;
        <div className='max-w-7xl mx-auto'>;
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>            {filteredProjects && filteredProjects.map((project, index) => (<motion&& motion.div;
                ...
                <div className="p-8">;
                  <div className="flex items-center gap-3 mb-4">;
                    <div className={`w-12 h-12 bg-gradient-to-br ${project.color} rounded-xl flex items-center justify-center`}>;
                      <project.icon className="w-6 h-6 text-white" />;
                        ...
                        </li>;{tech  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                        </span>;
                      ))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                  ...
                  <div className="mb-6">;
                    ...
                    <ul className="space-y-1">;
                      {project.results.map((result) => (<li key={result} className="text-sm text-white/80 flex items-center">;
