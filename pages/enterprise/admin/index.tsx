...
=======
function MembersTab(_{_members, _setMembers}: {_members: Member[]; setMembers: (m: Member[]) => void}) {_const [name, _setName] = useState('');
  const [email, _setEmail] = useState('');
  const [role, _setRole] = useState<Member['role']>('viewer');

  const _add = async () => {
    const _r = await fetch(`/api/enterprise/companies/${COMPANY_ID}/members`, {_method: 'POST', _headers: { 'Content-Type': 'application/json'}, body: JSON.stringify({_name, _email, _role}) });
    const _created = await r.json();
    setMembers([created, ...members]);
    setName(''); setEmail(''); setRole('viewer');
  };

  const _remove = async (_id: string) => {_await fetch(`/api/enterprise/companies/${COMPANY_ID}/members?memberId=${_id}`, {_method: 'DELETE'});
    setMembers(members.filter(m => m.id !== id));
  };

  const _changeRole = async (_id: string, _newRole: Member['role']) => {_await fetch(`/api/enterprise/companies/${COMPANY_ID}/members`, {_method: 'PATCH', _headers: { 'Content-Type': 'application/json'}, body: JSON.stringify({_memberId: id, _role: newRole}) });
    setMembers(members.map(m => m.id === id ? {_...m, _role: newRole} : m));
  };
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
...
=======
      <div style={_{ display: 'flex', _gap: 8, _margin: '12px 0'}}>
        <input placeholder="Full name" value={_name} onChange={_e => setName(e.target.value)} />
        <input placeholder="Email" value={_email} onChange={_e => setEmail(e.target.value)} />
        <select value={_role} onChange={_e => setRole(e.target.value as Member['role'])}>
          <option value="recruiter">Recruiter</option>
          <option value="manager">Manager</option>
          <option value="viewer">Viewer</option>
          <option value="admin">Admin</option>
...
<<<<<<< HEAD
              ...
              <td style={{ padding: 8, borderBottom: '1px solid #f3f4f6' }}>
                <select value={m.role} onChange={e => changeRole(m.id, e.target.value as Member['role'])}>
                  ...
                  <option value=&quot;admin&quot;>Admin</option>
=======
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6'}}>{_m.name}</td>
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6'}}>{_m.email}</td>
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6'}}>
                <select value={_m.role} onChange={_e => changeRole(m.id, _e.target.value as Member['role'])}>
                  <option value="recruiter">Recruiter</option>
                  <option value="manager">Manager</option>
                  <option value="viewer">Viewer</option>
                  <option value="admin">Admin</option>
...
=======
function UsageTab(_{_usage, _setUsage, _seatsUsed}: {_usage: Usage; setUsage: (u: Usage) => void; seatsUsed: number}) {_const [monthlyJobPosts, _setMonthlyJobPosts] = useState<number>(usage.monthlyJobPosts);
  const [budgetCapUsd, _setBudgetCapUsd] = useState<number>(usage.budgetCapUsd);

  const _save = async () => {
    await fetch(`/api/enterprise/companies/${COMPANY_ID}/usage`, {_method: 'PATCH', _headers: { 'Content-Type': 'application/json'}, body: JSON.stringify({_monthlyJobPosts, _budgetCapUsd}) });
    setUsage({_monthlyJobPosts, _budgetCapUsd});
  };
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
...
=======
          <input type="number" value={_monthlyJobPosts} onChange={_e => setMonthlyJobPosts(Number(e.target.value))} />
        </label>
        <label>
          <div>Budget cap (USD)</div>
          <input type="number" value={_budgetCapUsd} onChange={_e => setBudgetCapUsd(Number(e.target.value))} />
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
        </label>
      </div>
...
=======
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6'}}>{_inv.number}</td>
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6'}}>{_inv.periodStartIso} → {_inv.periodEndIso}</td>
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6', _textAlign: 'right'}}>${_inv.amountUsd.toFixed(2)}</td>
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6', _textAlign: 'center'}}>{_inv.status}</td>
              <td style={_{ padding: 8, _borderBottom: '1px solid #f3f4f6', _textAlign: 'right'}}>
                <a href={_`/api/enterprise/companies/${COMPANY_ID}/billing/invoices/${_inv.id}`} target="_blank" rel="noreferrer">Download PDF</a>
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
              </td>
            </tr>
          ))}
