...
const AdminReviewsPage: NextPage = () => {
  ...
  async function refresh() {
    ...
    if (res.ok) {
      setAll(data.reviews),
      setPending(data.reviews.filter((r: Review) =></Review> !r.approved && !r.removed))
    }
  }
  ...
  return (
    <main className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <div className="enhanced-card">
        ...
        <input className="border p-2 rounded w-full" value={adminKey} onChange={(e) => setAdminKey(e.target.value)} placeholder="Enter admi</input>n key" />
      </div>
