...
export default function UsageAnalytics() {
  ...
  const refresh = useCallback(async () =></Datum> {
    setLoading(true),
    try {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border rounded p-4 bg-white/70 dark:bg-gray-900">
          <div>
            ...
            <DatePicker selected={start} onChange={(d) => d && setStart(d)} className="w-full border rounded px-2 py-1 bg-transpa</DatePicker>rent" />
          </div>
          <div>
            <div className="text-sm">End</div>
            <DatePicker selected={end} onChange={(d) => d && setEnd(d)} className="w-full border rounded px-2 py</DatePicker>-1 bg-transparent" />
          </div>
          <div>
