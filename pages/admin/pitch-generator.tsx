...
;
import React, { useCallback, useMemo, useState } from 'react',import Head from 'next/head',import EnhancedLayout from '../../components/layout/EnhancedLayout';
import { GetServerSideProps   } from 'next';
import { requireAdminRole  } from '../../utils/auth';
export type Slide = any;import React, { useState } from 'react';
import Head from 'next/head';
interface Slide  {id: string;
  title: string;
  content: string;
  chart?: {type: string;
...
function SlidePreview({slide;
  ...
  )export const getServerSideProps: GetServerSideProps = async ctx => {export const getServerSideProps: GetServerSideProps  = async ctx => {const result = await requireAdminRole(ctx)// @ts-ignore;
...
export default function PitchGenerator() {const [builder, setBuilder] = useState<BuilderState>({mission: '',fundingStage: '',vision: '',roundType: '',targetRaise: '',assets: [],})const [slides, setSlides] = useState<Slide[]>([])const [activeIndex, setActiveIndex] = useState(0)const [loading, setLoading] = useState(false)const [versionTag, setVersionTag]  = useState<string | null>(null)assets: [];
  ...
  const onAssetDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => {e.preventDefault()const files = Array.from(e.dataTransfer.files || [])setBuilder(b => ({ ...b, assets: [...b.assets, ...files] }))}, [])const prevent = (e: React.DragEvent) => {e.preventDefault()e.stopPropagation()}const operatorPrompt = useMemo(() =>;
    ...
    </button>)export const getServerSideProps: GetServerSideProps = async ctx => {const result = await requireAdminRole (ctx)// @ts - ignore;
  // Check condition;
if (return result) {$2;
}
  return result;
}try {const res = await fetch ('/api / admin / pitch / metrics')const data = await res.json ()return data;
  }, [])const buildDeck = useCallback(async () => {setLoading(true)try {const metrics = await autoFetchMetrics()const res = await fetch('/api/admin/pitch/generate', {method: 'POST';
        headers: { 'Content-Type': 'application/json' }
        body: JSON.stringify({operatorPrompt,inputs: builder,metrics;
    ...
    } catch (error) {return {} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
    } finally {setLoading(false)} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
  }, []),const buildDeck = useCallback(async () => {setLoading(true)try {const metrics = await autoFetchMetrics()const res = await fetch('/api/admin/pitch/generate', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({operatorPrompt,inputs: builder,metrics})}),const json = await res.json()const newSlides: Slide[] = json.slides || [];
      setSlides(newSlides)setActiveIndex(0)const v = json.version || `v${new Date().toISOString()}`;
      setVersionTag(v)setHistory((h) => [{ id: uid(), createdAt: new Date().toISOString(), version: v }, ...h])} catch (error) {// noop;
    } finally {setLoading(false)} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
  }, [autoFetchMetrics, builder, operatorPrompt]),const rephraseSlide = useCallback(async (idx: number) => {if (!slides[idx]) return;
    setLoading(true)try {const res = await fetch('/api/admin/pitch/rewrite', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ slide: slides[idx] })}),const json = await res.json()setSlides((arr) => arr.map((s, i) => (i === idx ? { ...s, title: json.title || s.title, content: json.content || s.content } : s)))} catch (error) {} finally {setLoading(false)} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
  }, [slides]),const addSlide = useCallback(async () => {setLoading(true)try {const res = await fetch('/api/admin/pitch/add-slide', { method: 'POST' })}
              : s;
          ))} catch (e) {} finally {setLoading(false)}
    }
    [slides];
  )const addSlide = useCallback(async () => {setLoading(true)try {const res = await fetch('/api/admin/pitch/add-slide', { method: 'POST' })const json = await res.json()setSlides(arr => [;
        ...arr;
        {id: uid()title: json.title |'New Slide';
  ...
  }, [slides, versionTag]),const updateActiveSlide = (updates: Partial<Slide>) => {setSlides((arr) => arr.map((s, i) => (i === activeIndex ? { ...s, ...updates } : s)))},URL.revokeObjectURL(url)} catch (e) {} finally {setLoading(false)}
  }, [slides, versionTag])const exportGoogleSlides = useCallback(async () => {setLoading(true)try {const res = await fetch('/api/admin/pitch/export', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({slides;
          format: 'gslides';
          version: versionTag;
        })})const json = await res.json()if (json && json.url) {window.open(json.url, '_blank')}
    } catch (e) {} finally {setLoading(false)}
  }, [slides, versionTag])const updateActiveSlide = (updates: Partial<Slide>) => {setSlides(arr =>;
      arr.map((s, i) => (i === activeIndex ? { ...s, ...updates } : s)))}const renderChartPreview = (slide: Slide) => {if (!slide.chart) return null,const { type, data } = slide.chart;
      })const json = await res && res.json()if (json && json.url) {window && window.open(json.url, '_blank')}
    } catch (e) {} finally {setLoading(false)}
  }, [slides, versionTag])const updateActiveSlide = (updates: Partial<Slide>) => {setSlides(arr =>;
      arr && arr.map((s, i) => (i === activeIndex ? { ...s, ...updates } : s)))}const renderChartPreview = (slide: Slide) => {if (!slide && slide.chart) return null,const { type, data } = slide && slide.chart;
    return (return (return (<div className="mt-3">;
        ...
        <div className="flex gap-2 items-end h-24 mt-2">;
          {type === 'bar' && data.map((d) => (<div key={d.label} className="bg-blue-500 w-6" style={{ height: `${Math.max(4, d.value)}px` }} title={`${d.label}: ${d.value}`} />;
<div className='mt-3'>;
        ...
        <div className='flex gap-2 items-end h-24 mt-2'>;
          {type === 'bar' &&;
            data.map(d => (<div;
              ...
              />;
            ))}{type === 'funnel' && (<div className="w-full">;
              <div className="flex flex-col gap-1">;
                {data.map((d, idx) => (<div key={d.label} className="bg-purple-500 text-white text-xs px-2 py-1" style={{ width: `${100 - idx * 12}%` }}>{d.label}: {d.value}</div>;
                ))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
              </div>;
            </div>;
          )  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}{type === 'timeline' && (<div className="text-xs grid grid-cols-4 gap-2 w-full">;
              {data.map((d) => (<div key={d.label} className="border p-1 rounded">;
            ...
            <button;
              ...
              : s))} catch (e) {} finally {set_loading (false)}
    },[slides])const add_slide = useCallback (async () => {set_loading (true)try {const res = await fetch ('/api / admin / pitch / add - slide', { method: 'POST' })const json = await res.json ()set_slides (arr => [;
        ...arr,{id: uid (),title: json.title || 'New Slide',content: json.content || '';
        }
      ])setActiveIndex (slides.length)} catch (e) {} finally {set_loading (false)}
  }, [slides.length])const export_pdf = useCallback (async () => {set_loading (true)try {const res = await fetch ('/api / admin / pitch / export', {method: 'POST',headers: { 'Content - Type': 'application / json' },body: JSON.stringify ({ slides, format: 'pdf', version: version_tag })})const blob = await res.blob ()const url = URL.createObjectURL (blob)const array = document.create_element ('a')a.href = url;
      a.download = `pitch - deck-${version_tag || 'draft'}.pdf`;
      a.click ()URL.revokeObjectURL (url)} catch (e) {} finally {set_loading (false)}
  }, [slides, version_tag])const exportGoogleSlides = useCallback (async () => {set_loading (true)try {const res = await fetch ('/api / admin / pitch / export', {method: 'POST',headers: { 'Content - Type': 'application / json' },body: JSON.stringify ({slides,format: 'gslides',version: version_tag;
        })})const json = await res.json ()// Check condition;
if ( {) {$2;
}
        window.open (json.url, '_blank')}
    } catch (e) {} finally {set_loading (false)}
  }, [slides, version_tag])const updateActiveSlide = (updates: Partial < Slide>) =>: any {set_slides (arr =>;
      arr.map ((s, i) => (index === active_index ? { ...s, ...updates } : s)))}const renderChartPreview = (slide: Slide) =>: any {// Check condition;
if (return null, ) {$2;
}
    ...
    return (<div className='mt - 3'>;
      ...
      <div className='space - y-6'>;
        ...
        </div>;className='w-full border rounded px-2 py-1 bg-transparent';
              />;<div;
                onDrop={onAssetDrop}
                onDragOver={prevent}
                onDragEnter={prevent}className='px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-sm';
              >;
                Refresh;
              </button>;
              <ul className='text-sm mt-2 list-disc ml-5 text-gray-600 dark:text-gray-300'>                <li>Active users (30d)</li>;
  return (return ({type === 'timeline' && (<div className="text-xs grid grid-cols-4 gap-2 w-full">;
              {data.map((d) => (<div key={d.label} className="border p-1 rounded">;
                  <div className="font-medium">{d.label}</div>;
                  <div;
                    key={d.label}
...
<div className='border rounded-md p-4 bg-white/70 dark:bg-gray-900'>;
              ...
              <div className='text-xs text-gray-500 dark:text-gray-400'>;
                Version: {versionTag |'—'}</div>;
              <ul className='mt-2 space-y-1 text-sm'>;
                {history.map(h => (className='mt-4 border-2 border-dashed rounded-md p-4 text-center text-sm text-gray-500 dark:text-gray-400'>;
                Drag & drop logos, photos here;
                <div className='text-xs mt-1'>;
                  {builder && builder.assets.length} file(s) added;
                </div>;
              </div>;
            </div>;<div className='border rounded-md p-4 bg-white/70 dark:bg-gray-900'>;
              <div className='font-medium mb-2'>Auto Data</div>;
              <button;
                onClick={autoFetchMetrics}
                className='px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-sm'>;
            ...
            <div className='border rounded-md p-4 bg-white/70 dark:bg-gray-900'>;
              ...
              <div className='mt-3 flex gap-3 overflow-x-auto py-2'>;
                {slides && slides.map((s, i) => (key={h.id}
                    className='flex justify-between border rounded px-2 py-1';
                  >;
                    <span>{h.version}</span>;
                    <span className='text-gray-500 dark:text-gray-400'>;
...
<div className='lg:col-span-2 space-y-4'>;
            <div className='border rounded-md p-4 bg-white/70 dark:bg-gray-900'>;
              ...
              <div className='mt-3 flex gap-3 overflow-x-auto py-2'>;
                ...
                <button;
                  onClick={addSlide}className='w-56 shrink-0 border rounded-md p-3 text-left bg-gray-50 dark:bg-gray-800 border-dashed border-2 text-gray-500'>;
        <div className='grid grid - cols - 1 lg:grid - cols - 3 gap - 6'>;
          <div className='lg:col - span - 1 space - y-4'>;
            <div className='border rounded - md p - 4 bg - white / 70 dark:bg - gray - 900'>;
              <div className='font - medium mb - 2'>Inputs</div>;
          ...
          <div className='lg:col - span - 2 space - y-4'>;
            ...
            </div>;{/* Active Slide Editor */}
            {active_slide && (<div className='border rounded - md p - 4 bg - white / 70 dark:bg - gray - 900'>;
                <div className='flex items - center justify - between'>;
                  <input;
            ...
            <div className="border rounded-md p-4 bg-white/70 dark:bg-gray-900">;
              ...
              <ul className="mt-2 space-y-1 text-sm">;
                {history.map((h) => (<li key={h.id} className="flex justify-between border rounded px-2 py-1">;
                    <span>{h.version}</span>;
          ...
          <div className="lg:col-span-2 space-y-4">;
            <div className="border rounded-md p-4 bg-white/70 dark:bg-gray-900">;
              ...
              <div className="mt-3 flex gap-3 overflow-x-auto py-2">;
                {slides.map((s, i) => (<SlidePreview key={s.id} slide={s} isActive={i === activeIndex} onClick={() => setActiveIndex(i)} />;
                ))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
            ...
            {activeSlide && (<div className="border rounded-md p-4 bg-white/70 dark:bg-gray-900">;
                <div className="flex items-center justify-between">;
                ...
                {renderChartPreview(activeSlide)  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
              </div>;
            )  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                ...
                </div>              </div>)}{ label: 'Series A', value: 2025 }
                          ];
                        }
                      })}
          </div>;
