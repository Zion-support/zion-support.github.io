...
export default function SupportRequestsPage({ requests: initialRequests }: SupportRequestsPageProps) {
  ...
  const updateRequestStatus = async (requestId: string, status: SupportRequest['status'], notes?: string) => {
    try {
      const response = await fetch('/api/admin/support-requests/update', {
        method: 'POST',
      ...
      if (response.ok) {
        setRequests(requests.map(req => 
          req.id === requestId 
            ? { ...req, status, adminNotes: notes || req.adminNotes, updatedAt: new Date().toISOString() }
            : req
        ));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>Support Requests - Zion Tech Group</title>
      </Head>
      
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Support Requests</h1>
          <p className="mt-2 text-gray-600">Manage customer support requests and tickets</p>
        </div>
        ...
        <div className="space-y-4">
          {filteredRequests.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500">No support requests found for the selected filters.</p>
            </div>
          ) : (
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
              ...
              <div className="flex space-x-2 mt-6">
                <button
                  onClick={() => updateRequestStatus(selectedRequest.id, 'in-progress', adminNotes)}
                ...
                <button
                  onClick={() => updateRequestStatus(selectedRequest.id, 'closed', adminNotes)}
  ...
  try {
    ...
    const requests: SupportRequest[] = [
      ...
      },
      {
        ...
        message: 'Would like to request a new feature for better analytics.',
        status: 'in-progress',
        priority: 'medium',
