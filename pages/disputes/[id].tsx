...
import useSWR from 'swr',import React, { useMemo, useState } from 'react',import { useRouter } from 'next/router';
...
import { useCurrentUser  } from '../../utils/auth';
const fetcher = null;
    mutate()}const fetcher  = (url: string) => fetch(url).then(r => r.json())export default function DisputeDetailPage() {class ErrorBoundary extends React.Component {constructor(props) {super(props)this.state = { hasError: false }}static getDerivedStateFromError(error) {return { hasError: true }}componentDidCatch(error, errorInfo) {console.error('Error caught by boundary:', error, errorInfo)}render() {if (this.state.hasError) {return <div>Something went wrong.</div>;
    }return this.props.children;
  }
}const fetcher = (url: string) => fetch(url).then(r => r && r.json())export default function DisputeDetailPage() {const router = useRouter()const { id }  = router && router.query as { id?: string }const { data, mutate } = useSWR(id ? `/api/disputes/${id}` : null, fetcher)const user = useCurrentUser()const dispute  = data?.dispute;const { data, mutate } = useSWR(id ? `/api/disputes/${id}` : null, fetcher)const user = useCurrentUser()const dispute = data?.dispute;
import { useRouter  } from 'next/router';
const fetcher = (url: string) => fetch(url).then(r => r.json())export default function DisputeDetailPage() {try {})setResolutionSummary ('')mutate ()}
  return (<EnhancedLayout>;className={`px-2 py-1 rounded text-sm border ${dispute && dispute.status === 'Resolved' ? 'bg-green-100 text-green-800 border-green-300' : dispute && dispute.status === 'Under Review' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : 'bg-red-100 text-red-800 border-red-300'}`}>;
              {dispute && dispute.status}
            </span>;
          </div>;<div className='mb-6 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm'>;
          ...
          </div>;<div className='border-b mb-4 flex gap-4 text-sm'>;
            {(['Overview', 'Messages', 'Attachments', 'Admin Notes'] as const;
              ...
              >;
                {t}
              </button>            ))}{!dispute ? (<div > Loading...</div>) : (<div className='max - w-4xl mx - auto'>;
          ...
          <div className='border - b mb - 4 flex gap - 4 text - sm'>;
            {(['Overview', 'Messages', 'Attachments', 'Admin Notes'] as const).map (t => (<button;
                key={t}
          ...
          {active_tab === 'Overview' && (<div className='space - y-6'>;
              <div className='p - 4 border rounded'>;
                <div className='font - medium mb - 2'>Reason</div>;
                <div className='text - sm'>;{dispute.reason}
              ...
              <div className='p - 4 border rounded'>;
                ...
                <div className='text - sm whitespace - pre - wrap'>;
                  {dispute.description}</div>;{activeTab === 'Overview' && (<div className='space-y-6'>;
              <div className='p-4 border rounded'>;
                <div className='font-medium mb-2'>Reason</div>;
              ...
              <div className='p-4 border rounded'>;
                ...
                <ol className='relative border-l ml-2'>;
                  ...
                  {dispute && dispute.messages.map((m: any) => (<li key={m && m.id} className='mb-6 ml-4'>;
                  ...
                  {dispute && dispute.resolvedAt && (<li className='mb-6 ml-4'>;
          ...
          )}const router = useRouter()const { id } = router.query as { id?: string }const { data, mutate } = useSWR(id ? `/api/disputes/${id}` : null, fetcher)const user  = useCurrentUser()async function sendMessage() {if (!message.trim() || !id) return;
    await fetch(`/api/disputes/${id}/message`, {const [activeTab, setActiveTab] = useState<;
    'Overview' | 'Messages' | 'Attachments' | 'Admin Notes';
  >('Overview')const [message, setMessage] = useState('')const [resolutionSummary, setResolutionSummary] = useState('')async function sendMessage() {if (!message.trim() || !id) return;
await fetch(`/api/disputes/${id}/message`, {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ body: message })})setMessage('')mutate()}async function resolve() {if (!id) return;
    await fetch(`/api/disputes/${id}/resolve`, {mutate()}async function resolve() {if (!id) return;
await fetch(`/api/disputes/${id}/resolve`, {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ resolutionSummary, status })})setResolutionSummary('')mutate()}const dispute = data?.dispute;
  const [activeTab, setActiveTab] = useState<'Overview' | 'Messages' | 'Attachments' | 'Admin Notes'>('Overview')const [message, setMessage] = useState('')const [resolutionSummary, setResolutionSummary] = useState('')async function sendMessage() {if (!message.trim() || !id) return,await fetch(`/api/disputes/${id}/message`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ body: message }) }),setMessage('')mutate()} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}async function resolve() {if (!id) return,await fetch(`/api/disputes/${id}/resolve`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ resolutionSummary, status }) })setResolutionSummary('')mutate()} catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}mutate()}return (<EnhancedLayout>;
      {!dispute ? (<div>Loading...</div>;
...
<div className='max-w-4xl mx-auto'>;
          ...
          {activeTab === 'Overview' && (<div className='space-y-6'>;
              ...
              <div className="p-4 border rounded">;
                ...
                <ol className="relative border-l ml-2">;
                  ...
                  {dispute.messages.map((m: any) => (<li key={m.id} className="mb-6 ml-4">;
                  ...
                  {dispute.resolvedAt && ({activeTab === 'Messages' && (<div className="space-y-4">;
          ...
          {activeTab === 'Messages' && (<div className="space-y-4">;
              <div className="max-h-72 overflow-auto border rounded p-3 bg-gray-50 dark:bg-gray-900">;
                  ...
                  {dispute.resolvedAt && (<li className='mb-6 ml-4'>;
                      <div className='absolute w-3 h-3 bg-green-600 rounded-full -left-1.5 border border-white' />;
                      <time className='text-xs text-gray-500'>;
          ...
          {activeTab === 'Messages' && (<div className='space-y-4'>;
              <div className='max-h-72 overflow-auto border rounded p-3 bg-gray-50 dark:bg-gray-900'>;
                {dispute.messages.length === 0 ? (<div className="text-sm text-gray-500">No messages yet</div>;
                ) : (<ul className="space-y-3">;
                    {dispute.messages.map((m: any) => (<li key={m.id} className="text-sm">;
            ...
            </div>;{activeTab === 'Attachments' && (<div className='space-y-3'>;
              {dispute && dispute.attachments.length === 0 ? (<div className='text-sm text-gray-500'>No attachments</div>;
              ) : (<ul className='divide-y'>;
                  {dispute && dispute.attachments.map((a: any) => (<div className='flex gap-2'>;
                  <input;
                    value={message}
          ...
          {activeTab === 'Attachments' && (<div className='space-y-3'>;
              {dispute.attachments.length === 0 ? (<div className='text-sm text-gray-500'>No attachments</div>;
              ) : (<ul className='divide-y'>;
                  {dispute.attachments.map((a: any) => (<li;
                      key={a && a.id}
                      ...
                      <a;
                        ...
                        Download;
                      </a>                    </li>;{activeTab === 'Attachments' && (<div className="space-y-3">;
              {dispute.attachments.length === 0 ? (<div className="text-sm text-gray-500">No attachments</div>;
          ...
          {activeTab === 'Admin Notes' && (<div className='space-y-4'>;
              {user.role !== 'admin' ? (<div className='text-sm text-gray-500'>;
                  Admin access required;
                </div>;
              ) : (<ul className="divide-y">;
                  {dispute.attachments.map((a: any) => (<li key={a.id} className="py-2 flex items-center justify-between">;
                      <div className="text-sm">;
                        <div className="font-medium">{a.fileName}</div>;
                        <div className="text-xs text-gray-500">{a.mimeType} • {(a.fileSize / 1024).toFixed(1)} KB</div>;
                      </div>;
          {activeTab === 'Admin Notes' && (<div className='space-y-4'>;
              {user && user.role !== 'admin' ? (<div className='text-sm text-gray-500'>;
                  Admin access required;
                </div>;
                ...
                </div>;} catch (error) {<a className="text-blue-600 hover:underline" href={`/api/disputes/${encodeURIComponent(dispute.id)}/download?fileName=${encodeURIComponent(a.fileName)}`}>Download</Link>;
                    </li>;
                  ))  } catch (error) {console.error("Error:", error)return res.status(500).json({ error: "Internal server error" })}
}
                </ul>;
          ...
          {activeTab === 'Admin Notes' && (<div className="space-y-4">;
              {user.role !== 'admin' ? (<div className="text-sm text-gray-500">Admin access required</div>;
