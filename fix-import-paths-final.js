import fs from 'fs'; import { fileURLToPath } from 'url'; function processFile(filePath) {
  try {
    // Determine correct relative paths based on file location; const isInApp =
      filePath.includes('/app/') && !isInBlog && !isInComponents && !isInGuides && !isInPages; let utilsPath, typesPath; if (isInBlog || isInGuides || isInPages) {
    ...
    // Fix import paths; const replacements = [
      {
      ...
      {
        pattern: /import\s+dynamic\s+from\s+'\.\/utils\/dynamic';/g; replacement: `import dynamic from '${utilsPath}dynamic';`,
  ...
  t: `import type { Metadata } from '${typesPath}next';`}]
    ...
    }); // Fix MetadataRoute namespace issue; if (content.includes('MetadataRoute.')) {
      content = content.replace(/MetadataRoute\./g, 'MetadataRoute.'); modified = true; if (content.includes('MetadataRoute.')) {/* TODO: Fix JSX expression */}
    }
