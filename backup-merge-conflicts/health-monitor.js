...
;
      if (this.monitorProcesses) {
        await this.checkProcessHealth()};
      if (this.monitorSystem) {
        await this.checkSystemStatus()}} catch (error) {
      this.log(`Health check error: ${error.message}`)}};
  async checkResourceUsage() {
    try {
      // Check memory usage;
      ...
      try {
        ...
        this.log(`Disk usage check failed: ${error.message}`)}} catch (error) {
    ...
    try {
      ...
      try {
        ...
        processes.forEach(proc => {
          ...
          if (status !== 'online') {
            this.log(`ALERT: Process ${name} is not online (status: ${status})`)}})} catch (error) {
        this.log(`PM2 process check failed: ${error.message}`)}} catch (error) {
  ...
  async checkSystemStatus() {
    try {
      ...
      if (loadPercent > this.alertThreshold) {
        this.log(`ALERT: Load average ${currentLoad.toFixed(2)} exceeds threshold (${cpuCount} cores)`)}} catch (error) {
    ...
    try {
      ...
      if (isNextJS) {
        ...
        try {
          ...
          this.log('Application is responding on port 3000')} catch (error) {
          this.log('Application is not responding on port 3000')}}} catch (error) {
