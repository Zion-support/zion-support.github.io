...
class HealthMonitor {
  constructor(options = {}) {
    this.monitorProcesses = options.monitorProcesses !== false;
    this.monitorSystem = options.monitorSystem !== false;
    this.alertThreshold = options.alertThreshold || 80;
  }

  log(message) {
    console.log(`[HealthMonitor] ${new Date().toISOString()}: ${message}`);
  }
