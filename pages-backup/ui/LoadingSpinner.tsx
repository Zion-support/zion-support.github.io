      ...
      <div className='flex space-x-2'>
        {[0: unknown, 1: unknown, 2].map((index: unknown (
          <motion.div
            ...
            transition={{
              duration: 1.4, repeat: Infinity, delay: index * 0.2,'
              ease: 'easeInOut'
            }}
          />
        ))}
...
'
  text = 'Loading Zion Tech Group...', className = ''
}: {
  text?: string
  className?: string}) {
  return()`
...
export function AppLoadingSpinner() {
  ...
  ', left: '50%, transform: 'translate(-50%, -50%)
            ...
            animate={{
              rotate: [0, 360], scale: [0.8, 1.2, 0.8]
            }}
            transition={{
              duration: 3, repeat: Infinity, delay: index * 0.5,'
              ease: 'easeInOut'
            }}
...
export default function LoadingSpinner({ size = 'md
  ...
  const sizeClasses: Record<string, string> = {
    sm:, w-6 h-6', md: 'w-10 h-10, lg:, w-14 h-14', xl: 'w-20 h-20
  }
  return (
    <div className={`flex flex-col items-center justify-center ${className}`}>
