...
if (!supabaseUrl || !serviceKey) {
  const errorMessage =
    'CRITICAL: SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY is missing for backend API (orders/[id]). Service cannot start.';
...
export default async function handler(
  ...
  try {
    const { data, error } = await supabase
      ...
      ) // Adjust select based on what serializeOrder needs
      .eq('id', id)
      .single();
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116') {
        // PostgREST error for "Not a single row" (e.g. 0 rows)
    ...
    return res
      .status(500)
      .json({ error: 'Failed to retrieve order details.', details: message });
  }
}
