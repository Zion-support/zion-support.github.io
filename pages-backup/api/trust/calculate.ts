...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    'Supabase URL or Service Role Key is not defined for calculate endpoint.'
  );
}
...
export default async function handler(req: Request, res: Response) {
  ...
  if (!isAdmin && process.env.NODE_ENV === 'production') {
    // More lenient in dev for testing
    ...
    console.warn(
      'Attempt to access /api/trust/calculate without admin privileges denied.'
    );
    return res
      .status(403)
  ...
  try {
    ...
    if (invokeError) {
      console.error(
        'Error invoking Supabase function calculate-trust-score:',
        invokeError
      );
      ...
      if (
        ...
        typeof (invokeError as any).context === 'object' &&
        (invokeError as any).context !== null &&
      ...
      return res
        .status(statusCode)
        .json({ error: errorMessage, details: invokeError });
    }
    ...
    if (data && data.error) {
      return res
        ...
        .json({ error: data.error, details: data.details || null });
    }
