...
if (!supabaseUrl || !supabaseAnonKey) {
  console.error(
    'Supabase URL or Anon Key is missing for resend-verification-email. Check .env variables.'
  );
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!jwt) {
      const commonSupabaseCookieKey = Object.keys(req.cookies).find(
        key => key.startsWith('sb-') && key.endsWith('-access-token')
      );
    ...
    if (!jwt) {
      console.log(
        'No Supabase auth token cookie found in resend-verification-email.'
      );
      return res
        .status(401)
        .json({ message: 'Unauthorized: Auth token not found.' });
    }
    const {
      data: { user },
      error: authError,
    ...
    if (!user.email) {
      console.error(
        'User object from Supabase does not contain an email for resend.'
      );
      return res
        .status(400)
        .json({ message: 'Bad Request: User email not found in token.' });
    }
    ...
    if (resendError) {
      ...
      if (resendError.message.toLowerCase().includes('rate limit')) {
        // Example: "For security purposes, you can only request this once every X minutes"
        return res
          .status(429)
