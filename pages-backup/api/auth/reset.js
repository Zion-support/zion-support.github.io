...
const db = {
  user: {
    findFirst: async options => {
      ...
      console.log(
        'Mock DB: Searching for user with token',
        options.where.resetToken
      );
      // This is a simplified mock. In a real scenario, you'd query based on the hashed token
      // and check expiry.
      if (
        options.where.resetToken ===
        crypto.createHash('sha256').update('valid-token').digest('hex')
      ) {
        return {
          id: 'user-id-123',
          email: 'exists@example.com',
          resetToken: options.where.resetToken,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!loginResult.success) {
      ...
      console.warn(
        'Password reset successful, but auto-login failed for user:',
        user.id
      );
    }
