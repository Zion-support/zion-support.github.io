...
function fixTsxFile(filePath) {
  try {
    ...
    if (content.includes('export default function') && content.includes('{') && !content.includes('const features = [')) {
    ...
    if (content.includes('export default') && !content.includes('export default')?.includes(';')) {
      content = content.replace(
        /(export default \w+)(?!;)/,
        '$1;'
      );
    ...
    unusedImports.forEach(importName => {
      if (content.includes(`{${importName}}`) && !content.includes(`<${importName}`) && !content.includes(`${importName} className`)) {
        content = content.replace(
