...
function fixUnusedVariables(content) {
  ...
  content = content.replace(
    ...
    (match, vars, assignment) => {
      ...
      const unusedVars = varList.filter(
        (v) =>
      ...
      if (unusedVars.length > 0) {
        const newVars = varList
          .map((v) => (unusedVars.includes(v) ? `_${v}` : v))
          .join(", ");
...
filesToFix.forEach((filePath) => {
  try {
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(
        /import\s*{\s*[^}]*,\s*(\w+)\s*[^}]*}\s*from\s*['"][^'"]+['"];\s*\n?/g,
        (match, unusedVar) => {
          if (
            !content.includes(unusedVar) ||
            content.indexOf(unusedVar) === content.lastIndexOf(unusedVar)
          ) {
            return "";
          }
          return match;
