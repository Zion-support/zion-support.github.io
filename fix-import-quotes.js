#!/usr/bin/env node;
const fs = require('fs')const path  = require('path')function fixImportQuotes() {try {let content = fs.readFileSync(filePath, 'utf8')let modified  = false;// Fix ' in import statements back to single quotes;
    const importRegex = /import\s+.*?from\s+'([^&]+)'/g;
    const newContent = content.replace(importRegex, (match, modulePath) => {modified = true;
      return match.replace(/'/g, "'")})if (modified) {fs.writeFileSync(filePath, newContent, 'utf8')console.log(`Fixed import quotes in: ${filePath}`)return true;
    }return false;
...
for (const file of tsxFiles) {if (fixImportQuotes(file)) {fixedCount++;
  }
