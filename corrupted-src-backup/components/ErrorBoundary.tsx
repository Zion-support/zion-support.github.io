import { Component, ErrorInfo, ReactNode } from 'react'; import { AlertTriangle, RefreshCw, Home } from 'lucide-react'; interface Props {/* TODO: Fix JSX expression */}
}
...
//     // Report error to analytics/monitoring service; this.reportError(error, errorInfo); // Call custom error handler if provided; if (this.props.onError) {
    ...
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'exception', {)
        ...
        custom_map: {,
          ...
          component_stack: errorInfo.componentStack class ErrorBoundary extends Component<Props, State> {/* TODO: Fix JSX expression */}
  r: false }
  }
...
:src/components/ErrorBoundary.tsx {this.state.errorId && (
              <p className="text-gray-400 mb-4 text-sm">
                Error ID: {this.state.errorId}
              </p>
            )}
            <div className="flex flex-col sm: flex-row gap-4 justify-center">,
              <button; <div className="flex flex-col sm:flex-row gap-4 justify-center"></div>
              <button
