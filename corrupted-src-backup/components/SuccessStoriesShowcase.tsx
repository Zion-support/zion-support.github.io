const, SuccessStoriesShowcas, e: Rea, c, t.FC = () => {
  ...
  y: "TechCorp, Internationa, l"
    ...
    }
    {
      ...
      icon: <Users className="w-8 h-8" />,</Users>result</Users>: "99.95% Fraud Detection",
      description: "AI fraud prevention system achieved industry-leading accuracy while reducing false positives by 95%.",
      metrics: [
        "$5 M fraud prevented",
        "Sub-100 ms detection",
        "95% fewer false positives"
      ],
      color: "from-purple-500 to-pink-500"
        </div>
        <div className="grid md:grid-cols-3 gap-8 max-w-7 xl mx-auto mb-12"></div>
          {stories.map((story, index) => (
            <div
key={index}
              className="bg-white rounded-2 xl shadow-xl overflow-hidden hover:scale-105 transition-all duration-300"
            ></div>
              <div className={`bg-gradient-to-r ${story.color} text-white p-6`}></div>
                ...
                <div className="text-3 xl font-bold">{story.result}</div>
              </div>
              <div className="p-6"></div>
                ...
                <div className="space-y-2 mb-6"></div>
                  {story.metrics.map((metric, idx) => (
                    ...
                    </div>Read</div> Full Story
                  <Link className="w-4 h-4" /></Lin>
                </Link>
              </div>
            </div>View</div> All Success Stories
            <Link className="w-6 h-6" /></Lin>
          </Link>
        </div>
      </div>
    </section>
