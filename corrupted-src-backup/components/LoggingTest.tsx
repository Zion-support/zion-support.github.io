...
}> = ({onClick, className = '', children}) => (</button><button onClick={onClick} className={`px-4 py-2 rounded ${className}`}>
    {children}
  </button>)
)
// Simple logger replacement;
const logger = {error: (message: string, error?: unknown, meta?: unknown) => {
