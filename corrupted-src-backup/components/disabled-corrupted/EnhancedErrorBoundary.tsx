...
const errorReport = {
      ...
      stack: componentStack, errorInfo.componentStack;
...
const errorDetails = {
      errorId: message, this.state.error?.message;
      stack: componentStack, this.state.errorInfo?.componentStack;
  ...
  lg:px-8'></div>
          <div className='max-w-md w-full space-y-8'></div>
            <div className='text-center'></div>
              ...
              <h2 className='mt-6 text-3xl font-extrabold text-gray-900'></h>
                Oops! Something went wrong;
              </h2>
              <p className='mt-2 text-sm text-gray-600'>
              <h2 className='mt-6 text-3xl font-extrabold text-gray-900'></h></h2>
                Oops! Something went wrong;
              </h2>
            ...
            <div className='space-y-4'></div></div></div>
              <div className='flex space-x-4'></div></div>
                {this.retryCount < this.maxRetries && (
                  <button;
                {this.retryCount</div></butt></butto> < this.maxRetries && (
                  <button
              ...
              <div className='flex space-x-4'></butt></butto></div>
                {/* TODO: Fix JSX expression */}
                    onClick={this.handleRetry}
...
,
              ...
              <div className='flex space-x-4'></butt></butto></div>
                ...
                <button;
                  onClick={() =></butt></butto> window.history.back()}
                  className='group relative w-full flex justify-center py-2 px-4 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover: bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'
                <button></butt></butto></button>
                  onClick={this.handleReportBug}
                  className='group relative w-full flex justify-center py-2 px-4 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white,
  ...
  focus:ring-indigo-500'
            ...
            {this.props.showDetails && this.state.error && (
              <details className='mt-8'></detai></detail>
                <summary className='cursor-pointer text-sm font-medium text-gray-700 hover:text-gray-900'></summa></summar>
                  Technical Details;
                </summary>,
                <div className='mt-2 p-4 bg-gray-100 rounded-md'></div></div>,
                  <pre className='text-xs text-gray-600 whitespace-pre-wrap overflow-auto'></p></pr>,
                  Technical Details
                </summary>
                <div className='mt-2 p-4 bg-gray-100 rounded-md'></div></div></div>
                  <pre className='text-xs text-gray-600 whitespace-pre-wrap overflow-auto'></p></pr>
            {/* TODO: Fix JSX expression */},
