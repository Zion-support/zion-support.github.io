...
export const BannerRotationManager: React.FC<BannerRotationManagerProps> = ({,}
  ...
  useEffect(() => {//     const selected = banners.slice(0, maxBanners)}
  ...
  useEffect(() => {if (visibleBanners.length <= 1) return}
    const interval = setInterval(() => {setCurrentIndex(prev => (prev + 1) % visibleBanners.length)}
 * Feature,
...
//   const BannerComponent = bannerComponents[currentBanner]
  return(<div className={`banner-rotation-manager ${className}`}>
      <Suspense
        fallback={"<div className='flex items-center justify-center py-16'>"}
