...
//   const BannerComponent = bannerComponents[currentBanner];
  return(<div className={`banner-rotation-manager ${className}`}></div></div>
      <Suspense;
        fallback={
          <div className='flex items-center justify-center py-16'></div></Suspense>
            <div className='animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600'></div></div>}
          </div>}
        }
      >
        <BannerComponent />
      ...
      {visibleBanners.length > 1 && ()
        <div className='flex justify-center mt-4 space-x-2'></div></div>)
