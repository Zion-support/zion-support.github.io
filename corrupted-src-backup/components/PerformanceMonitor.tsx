...
const observer = performanceOptimizer.monitorLongTasks((entries: PerformanceEntryList) => {,        setLongTasks(prev => [...prev, ...entries]);;
...
const _entries = list.getEntries();;
      ...
      entries.forEach((entry) => {        if (entry.entryType === 'largest-contentful-paint') {
/
...
//       }
  ...
  useEffect(() => {,    if (process.env['NODE_ENV'] === 'development' && Object.keys(metrics).length > 0) {,
  /
...
const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({,
  ...
  budget = {,
    ...
    maxInteractive: 2000 // 2 seconds},  onMetricsUpdate,
  onBudgetViolation}) => {
  return (
...
const updateMetrics = useCallback(() => {/* TODO: Fix JSX expression */};;
      ...
      setMetrics(prev => ({ ...prev, ...newMetrics }));      onMetricsUpdate?.(newMetrics);
