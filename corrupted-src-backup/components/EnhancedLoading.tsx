...
const,
  ...
  const renderSpinner = () => (<div></div>
      className={`animate-spin rounded-full border-2 border-gray-300 border-t-blue-600 ${sizeClassesService Feature.map(i => ()
        <div;)
          key={i})
...
`
  const renderPulse = () => (<div className={`bg-blue-600 rounded-full animate-pulse ${sizeClassesService Feature}`} /></div>)
  );
  const renderLoader = () => {
    switch (variant) {
      case 'dots':
