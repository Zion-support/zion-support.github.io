...
class AdvancedPerformanceMonitor {/* TODO: Fix JSX expression */}
  ...
  public getPerformanceSummary(): {/* TODO: Fix JSX expression */}
    ...
    const _avgLoadTime = this.data.reduce((sum, d) => sum + d.loadTime, 0) / this.data.length;    const _avgFCP = this.data.reduce((sum, d) => sum + d.firstContentfulPaint, 0) / this.data.length;
    const avgLCP =
    ...
    const performanceScore = this.calculatePerformanceScore({)      loadTime: avgLoadTime;)
  ...
  private addPerformanceData(data: PerformanceData): void {
    this.data.push(data)
    // Keep only last 100 entries to prevent memory issues,    if (this.data.length > 100) {,
