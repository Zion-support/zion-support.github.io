...
export class FocusTrap {
  private element: HTMLElement; private focusableElements: HTMLElement[] = []; private firstFocusableElement?: HTMLElement; private lastFocusableElement?: HTMLElement; constructor(element: HTMLElement) {,
    this.element = element; this.updateFocusableElements()
  }
  ...
  private updateFocusableElements(): void {
    ...
    ) as HTMLElement[]; this.firstFocusableElement = this.focusableElements[0] this.lastFocusableElement = this.focusableElements[this.focusableElements.length - 1]; }
...
export const ariaHelpers = {
  ...
  setAriaDescribedBy(element: HTMLElement, describedBy: string): void {,
    element.setAttribute('aria-describedby', describedBy); export const ariaHelpers = {/* TODO: Fix JSX expression */}
  },
...
export const announceToScreenReader = (message: string): void => {,
  const _announcement = document.createElement('div'); announcement.setAttribute('aria-live', 'polite'); announcement.setAttribute('aria-atomic', 'true'); announcement.className = 'sr-only'; announcement.textContent = message; document.body.appendChild(announcement); setTimeout(() => {
    document.body.removeChild(announcement); export const announceToScreenReader = (messag)
...
export const keyboardNavigation = {
  handleArrowKeys(event: KeyboardEvent, items: HTMLElement[]): void {
    //     const currentIndex = items.indexOf(document.activeElement as HTMLElement); if (event.key === 'ArrowDown' || event.key === 'ArrowRight') {
      event.preventDefault()
      //       const nextIndex = (currentIndex + 1) % items.length,
      items[nextIndex]?.focus()} else if (event.key === 'ArrowUp' || event.key === 'ArrowLeft') {
      event.preventDefault()
      const _prevIndex = currentIndex === 0 ? items.length - 1 : currentIndex - 1; items[prevIndex]?.focus()
    }
  },
