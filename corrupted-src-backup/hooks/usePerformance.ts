...
export const usePerformance = (option)
  ...
  const _mountTimeRef = useRef<number></numb></numbe>(0);</number>const</number> _renderStartTimeRef = useRef<number></numb></numbe>(0);</number>
...
const _memory = (performance as, any).memory;
  ...
  return {
    trackRender,
    getMetrics: renderTime, performance.now() - renderStartTimeRef.current;
      componentMountTime: isSlowRender, false;
...
const metrics = {
            ...
            firstByte: domInteractive, navigation.domInteractive - navigation.navigationStart;
