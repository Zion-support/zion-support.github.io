...
export const usePerformance = (options: UsePerformanceOptions) => {,
  const {,
    ...
    slowRenderThreshold = 16, // 60fps thresholdexport const usePerformance = (option)
  ...
  useEffect(() => {
    mountTimeRef.current = performance.now()return () => {
  ...
  const trackRender = useCallback(() => {/* TODO: Fix JSX expression */}
    ...
    getMetrics: (): PerformanceMetrics => ({
      ...
      componentMountTime: performance.now() - mountTimeRef.current
      isSlowRender: falsereturn {/* TODO: Fix JSX expression */}
    })}}
/**
...
export const usePageLoadPerformance = () => {/* TODO: Fix JSX expression */}
    ...
    if (document.readyState === 'complete') {
      ...
      // Wait for load event
      window.addEventListener('load', trackPageLoad)return () => window.removeEventListener('load', trackPageLoad)
