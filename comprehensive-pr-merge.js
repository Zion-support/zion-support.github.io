...
const recentBranches = execSync('git for-each-ref --sort=-committerdate refs/remotes/origin --format="%(committerdate:short) %(refname:short)"', { encoding: 'utf8' })
  .split('\n')
  .map(line => line.trim())
...
for (let batch = 0; batch < totalBatches; batch++) {
  ...
  if (batch % 3 === 0 || batch === totalBatches - 1) {
    try {
      ...
      console.log(`⚠️  Warning: Failed to push after batch ${batch + 1}:`, error.message);
    }
  }
}
