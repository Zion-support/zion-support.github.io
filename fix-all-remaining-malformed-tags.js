...
// Function to fix all remaining malformed tags
function fixAllMalformedTags(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');
  let modified = false;

  // Fix all malformed closing tags like <</a>/a> or <</div></div>
  ...
  content = content.replace(/<(\/)([a-z]+)><\/([a-z]+)><\/([a-z]+)><\/([a-z]+)>/g, '</$2>');
