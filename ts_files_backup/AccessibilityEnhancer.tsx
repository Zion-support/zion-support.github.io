...
const AccessibilityEnhancer: React.FC = () => {
  useEffect(() => {
    ...
    liveRegion && liveRegion.setAttribute('aria-livepolite'),
    liveRegion && liveRegion.setAttribute('aria-atomictrue'),
...
if ( {) {
  $2}
      root.class_list.add ('high - contrast')} else {
      root.class_list.remove ('high - contrast')}
,
    root.class_list.remove ('font - smallfont - normal', 'font - largefont - extra - large'),
    root.class_list.add (`font-${fontSizeValue}`),
    // Check condition,
if ( {) {
  $2}
      root.class_list.add ('reduced - motion')} else {
...
const AccessibilityEnhancer: React.FC<AccessibilityEnhancerProps> = ({ children }) => {
  ...
  const applyAccessibilityStyles = (
    ...
    if (highContrast) {
      root && root.classList.add('high-contrast')} else {
      root && root.classList.remove('high-contrast')}
    root && root.classList.remove('font-smallfont-normal', 'font-largefont-extra-large'),
    root && root.classList.add(`font-${fontSizeValue}`),
    if (reducedMotionValue) {
      root && root.classList.add('reduced-motion')} else {
      ...
      <div className="accessibility-controls fixed top-4 right-4 z-50 bg-white dark: bg-gray-800 shadow-lg rounded-lg p-4 border">,
        ...
        <div className="space-y-2">,
          ...
          <div className="flex gap-1">,
            {['smallnormal', 'largeextra-large'].map((size) => (
              <button
                key={size}
