...
const FALLBACK_RESPONSES = [
  ...
  Thanks for reaching out! While Im having trouble connecting to my knowledge base, I can suggest checking our FAQ section or contacting our support team directly.",
  ...
  Im currently experiencing technical difficulties, but I'd be happy to help you get to the right resource. Try browsing our documentation or contacting support.",
...
// Fallback responses when API is unavailable;
const FALLBACK_RESPONSES = [;
  Im here to help! You can browse our help documentation, contact support at support@ziontechgroup.com, or try asking your question in a different way.",
  ...
  I understand you need assistance. For immediate help, please visit our help center or reach out to support@ziontechgroup.com.",
  "I'm currently experiencing technical difficulties, but Id be happy to help you get to the right resource. Try browsing our documentation or contacting support.,
  While I work on resolving my connection issues, you can find helpful information in our help section or contact our support team for immediate assistance.";
],
    ...
    <div className="fixed bottom-4 right-20 bg-zion-blue w-80 max-w-full rounded-lg shadow-xl flex flex-col z-40>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4 style={{ maxHeight: '400px' }}>
        {messages.length === 0 && (
          <ChatMessage 
            role=assistant" 
            message="Hi! Im here to help you with questions about Zion. What can I assist you with today? 

          />

          (FALLBACK_RESPONSES[Math.floor(Math.random() * FALLBACK_RESPONSES.length)] || Im experiencing technical difficulties. Please contact support@ziontechgroup.com for assistance."),
        setMessages(prev => prev.map(m => m.id === botId ? { ...m, message: final } : m));
      }
    } catch (err) {
      logErrorToProduction('Chatbot error:', { data: err }),
      // Provide a helpful fallback response instead of generic error;
      const fallbackResponse = FALLBACK_RESPONSES[Math.floor(Math.random() * FALLBACK_RESPONSES.length)] || "Im experiencing technical difficulties. Please contact support@ziontechgroup.com for assistance.,
        id: Date.now().toString() + -e',
        role: 'assistant,
        message: fallbackResponse;
      },
...
}> <X className=h-5 w-5" /> </Button> </div> <ChatMessage role="assistant message=Hi! Im here to help you with questions about Zion. What can I assist you with today?" />)}{messages && messages.map (m => (<ChatMessagekey= {m && m.id;
}role= {m && m.role;
}message= {m && m.message;
}/>) )})}<divref= {endRef;
}/> </div> </div> </div>)}";}
...
origin/cursor/automate-test-improve-and-merge-code-2533
}> <X className=h - 5 w - 5" /> </Button> </div> <ChatMessage role="assistant message=Hi! Im here to help you with questions about Zion. What can I assist you with today?" />)}{messages.map (m => (<ChatMessage key= {m.id;
}role= {m.role;
}message= {m.message;
}/>) )})}<div ref= {end_ref;
}/> </div> </div> </div>)}";
