...
=======
>>>>>>> origin/cursor/expand-services-advertise-and-build-project-961d:src/components/developers/WebhooksManager.tsx
    testResult,
    fetchWebhooks,
    createWebhook,
    toggleWebhook,
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/developers/WebhooksManager.tsx
    loading;
    testResult;
    fetchWebhooks;
    createWebhook;
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/developers/WebhooksManager.tsx
  // Event type options;
  const event_options: {value: WebhookEventType;
    label: string;
    description: string;
  }[]  = [;{value: 'new_application';
      label: 'New Application';
    ...
    {}, [])}, [])const handleCreateWebhook = async () => {clearTestResult;
...
import {Globe,MoreVertical,PlayCircle,Plus,RefreshCw,Webhook,X} from 'lucide-react';
  ...
  AlertDialog,AlertDialogAction,AlertDialogCancel,AlertDialogContent,AlertDialogDescription,AlertDialogFooter,AlertDialogHeader,AlertDialogTitle} from '@/components/ui/alert-dialog';
  Select,SelectContent,SelectItem,SelectTrigger,SelectValue} from '@/components/ui/select';
export function WebhooksManager() {const {    webhooks,    webhooks,loading,testResult,fetchWebhooks,createWebhook,toggleWebhook,deleteWebhook,testWebhook,clearTestResult,const [showCreateDialog, setShowCreateDialog] = useState(false)const [showDeleteConfirm, setShowDeleteConfirm] = useState<string | null>(null;
  )const [showTestDialog, setShowTestDialog] = useState<string | null>(null)const [showTestResult, setShowTestResult]  = useState(false)// Create webhook form state;
  const [webhookName, setWebhookName] = useState('')const [webhookUrl, setWebhookUrl] = useState('')const [webhookSecret, setWebhookSecret] = useState('')const [selectedEvents, setSelectedEvents] = useState<WebhookEventType[]>([])const [testEventType, setTestEventType] =;
    useState<WebhookEventType>('new_application')// Load webhooks on mount;
  useEffect(() => {fetchWebhooks()}, [])const handleCreateWebhook = async () => {if (webhookName && webhookName.trim() === '' ||;
      webhookUrl && webhookUrl.trim() === '' ||;
      selectedEvents && selectedEvents.length === 0;
    )return;await createWebhook(webhookName,webhookUrl,selectedEvents,webhookSecret && webhookSecret.trim() === '' ? undefined : webhookSecret;
    )setShowCreateDialog(false)resetWebhookForm()}const handleToggleStatus = async (webhookId: string,currentStatus: boolean;
  ) => {await toggleWebhook(webhookId, !currentStatus)}const handleDeleteWebhook = async (webhookId: string) => {await deleteWebhook(webhookId)setShowDeleteConfirm(null)}const handleTestWebhook = async (webhookId: string) => {await testWebhook(webhookId, testEventType)setShowTestResult(true)const handleToggleStatus = async (webhookId: string, currentStatus: boolean) => {await toggleWebhook(webhookId, !currentStatus)}const handleDeleteWebhook = async (webhookId: string) => {await deleteWebhook(webhookId)setShowDeleteConfirm(null)setShowDeleteConfirm(null)}const handleTestWebhook = async (webhookId: string) => {await testWebhook(webhookId, testEventType)setShowTestResult(true)const handleToggleStatus = async (webhookId: string, currentStatus: boolean,) => {await toggleWebhook(webhookId, !currentStatus)},const handleDeleteWebhook = async (webhookId: string,) => {await deleteWebhook(webhookId),setShowDeleteConfirm(null)},const handleTestWebhook = async (webhookId: string,) => {await testWebhook(webhookId, testEventType),setShowTestResult(true)},const resetWebhookForm = () => {setWebhookName('')setWebhookUrl('')setWebhookSecret('')setSelectedEvents([])}// Event type options;
  const eventOptions: {value: WebhookEventType;
    label: string;
    description: string;
  }[] = [;
                ...
                <div className="flex items - center justify - between">;
                  ...
                  <div className='flex items - center space - x-2'>;
                    <div className='flex items - center mr - 2'>;
                      <Switch;
                        aria - label='Toggle webhook';checked={webhook.is_active}
                        onCheckedChange={() =>;
                          handleToggleStatus (webhook.id, webhook.is_active)}                      />;
                      <span className='ml - 2 text - sm'>;
                        {webhook.is_active ? 'Active' : 'Inactive'}key={webhook && webhook.id}
                className='p-4 border border-zinc-800 rounded-lg'>;
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/developers/WebhooksManager.tsx
                ...
                <div className='mt - 3 text - xs text - zinc - 500 flex items - center space - x-4'>;
                  ...
                  {webhook.last_triggered_at && (<span>Last triggered:{' '}
                      {format (new Date (webhook.last_triggered_at),'MMM d, yyyy HH:mm')}
                    </span>                  )}
                <div className="mt - 3 text - xs text - zinc - 500 flex items - center space - x-4">;
                  <span > Created: {format (new Date (webhook.created_at), 'MMM d, yyyy')}</span>;
              ...
              <div className='space - y-4 py - 4'>;
                <div className='space - y-2'>;
                  ...
                  >;
                    ...
                    <SelectContent className='bg - zinc - 900 border - zinc - 800'>;
                      {event_options.map (option => (                        <SelectItem key={option.value} value={option.value}>                      {event_options.map ((option, ) => (<SelectContent className="bg - zinc - 900 border - zinc - 800">;
                      {event_options.map ((option) => (<SelectItem key={option.value} value={option.value}>;
                          {option.label}
                        </SelectItem>))}
                    </SelectContent>;
                  </Select>;
                  <p className='text - xs text - zinc - 500'>;The event type will determine the structure of the test;
                    payload.;
                  </p>;
                </div>;
              </div>;</SelectContent>;
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/developers/WebhooksManager.tsx
              ...
              <div className='space-y-4 py-4'>;
                <div className='space-y-2'>;
                  <div className='flex items-center justify-between'>;
                    <Label>Response Status</Label>;<Badge;
                      className={testResult &&;
              ...
              <div className='space - y-4 py - 4'>;
                ...
                <div className="space - y-2">;
                  <div className="flex items - center justify - between">;
                    <Label > Response Status</Label>;
                    <Badge;
                      class_name = {test_result && test_result.status >= 200 && test_result.status < 300;
                          ? "bg - green - 700";
                          : "bg - red - 700";
                       }
                    >;
                  ...
                  <div className="space - y-2 mt - 4">;
                    ...
                    <ScrollArea className="h-[200px] rounded border border - zinc - 800 bg - black p - 4">;
                      <pre className="text - xs font - mono text - zinc - 300 whitespace - pre - wrap break - all">;
                        {test_result?.response_body || "No response body"}</ScrollArea>;
                  </div>;
                </div>;
<<<<<<< HEAD:src_backup/components/developers/WebhooksManager.tsx
              </div>;
