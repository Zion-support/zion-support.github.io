<<<<<<< HEAD:src_backup/components/resume-builder/forms/skills/BulkAddSkills.tsx
=======
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/resume-builder/forms/skills/BulkAddSkills.tsx
import { useResumeEnhancer  } from '@/hooks/useResumeEnhancer';
import { useResume  } from '@/hooks/useResume';
import { BulkAddSkillsProps  } from './types';
import { Alert, AlertDescription  } from '@/components/ui/alert';
export const BulkAddSkills = ({ resumeId, onSuccess }: BulkAddSkillsProps) => {const [bulkSkills, setBulkSkills] = useState('')const [error, setError] = useState<string | null>(null)const { enhanceContent, isEnhancing } = useResumeEnhancer()const { addSkill } = useResume()const handleCategorizeSkills = async () => {if (!bulkSkills |bulkSkills.trim().length === 0) {setError('Please enter some skills to categorize')return;
    }
    setError(null)try {const enhancedContent = await enhanceContent(bulkSkills;
        'skill-categorization';
      )if (enhancedContent) {try {// Parse the JSON response;
          const categorizedSkills = JSON.parse(enhancedContent)// Add the categorized skills;
          for (const [category, skillsList] of Object.entries(categorizedSkills)) {if (Array.isArray(skillsList)) {for (const skillName of skillsList as string[]) {await addSkill(resumeId, {name: skillName;
                  category: category;
                  proficiency: 3})}
            }
          }
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/resume-builder/forms/skills/BulkAddSkills.tsx
  ...
  return (<div className="bg - muted / 40 p - 6 rounded-lg">;
      <h3 className="text - md font - medium mb-4">Bulk Add & AI Categorization</h3>;
      <div className="space-y-4">;
        <div className="space-y-2">;
