...
=======
>>>>>>> origin/cursor/expand-services-advertise-and-build-project-961d:src/components/DynamicListingPage.tsx
  title,
  description,
  categorySlug,
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/DynamicListingPage.tsx
max: number;
...
if ( {) {$2;
    ...
    [number, number];
  >([0, initial_price.max])const handleSliderChange = (values: number[]) =>: any {const [min, max] = values.map (Number)if (|| isNaN (max)) return) {$2;
}
    setCurrentPriceFilter ([min, max])}
...
=======
        ...
        selected_brand === 'all' ||;
        (listing.brand && listing.brand === selected_brand)const matches_specs =;
        !spec_query ||;
        (listing.specifications &&;
          listing.specifications.some (string =>;
            s.toLowerCase ().includes (spec_query.toLowerCase ()))) ||;
        ...
        selected_availability === 'all' ||;
        (listing.availability && listing.availability === selected_availability)const matches_category =;
        selected_categories.length === 0 ||;
...
max: 10000 ;
  ...
  try {filteredListings = allListings && allListings.filter(listing => {      const matchesSearch =;
        ...
        selectedBrand === 'all' ||;
        (listing && listing.brand && listing && listing.brand === selectedBrand)const matchesSpecs =;
        !specQuery ||;
        ...
        selectedAvailability === 'all' ||;
        (listing && listing.availability && listing && listing.availability === selectedAvailability)const matchesCategory =;
...
=======
                    ...
                    className="border-zion-blue-light text-zion-slate-light focus-visible:ring-zion-purple";
                  >;
                    {ToggleViewIcon}<span className="sr-only">;
                      {isGrid ? "List view" : "Grid view"}
                    </span>;
                  </Button>;
                </div>;
