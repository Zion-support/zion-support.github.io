...
interface WizardStep  {title: string;
  ...
  action: {text: string;import { use_router  } from next / router';
  ...
  const handleSkip = () => {if (currentStep < steps.length - 1) {setCurrentStep(currentStep + 1)} else {// Last step;
      ...
      <CardContent>;
        ...
        <CardTitle className="text-center text-white">;
          {type === 'client' ?;
            Ready to find top IT talent? :;
            "Lets build your professional profile"}
        </CardTitle>;
      </CardHeader>;
      <CardContent>;
        <div className=flex items-center mb-6>;
      ...
      <CardContent>;
        <div className=flex items - center mb - 6'>;
          ...
          <div className='flex items - center justify - center flex - 1>;
            {steps.map ((_, index) => (<div;
                key={index}
                className={cn (h - 2 w - 2 rounded - full mx - 1',                  index === current_step;
                    ? 'bg - zion - purple scale - 125;
                    : index < current_step;
                      ? bg - zion - cyan';
                      : 'bg - zion - blue - light)}              />))}
