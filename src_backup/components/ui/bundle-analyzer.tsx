...
;
  const { user } = useAuth()const isAdmin = user?.userType === 'admin' |user?.role === 'admin';
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/ui/bundle-analyzer.tsx
  gzipped_size: number;
  chunk_count: number;
  load_time: number;
  cacheHitRate: number;
 ...
 */;
function BundleAnalyzer() {const { user } = use_auth ()const is_admin = user?.user_type === 'admin' || user?.role === 'admin';
...
if ( {) {$2;
  ...
  const [bundle_info, setBundleInfo] = useState < BundleInfo | null>(null)const [chunks, set_chunks] = useState < ChunkInfo[]>([])const [is_visible, setIsVisible] = useState (false)const [is_collecting, setIsCollecting] = useState (false)const [should_show, setShouldShow] = useState (false)useEffect ((, ) => {// Only show in development or when explicitly enabled;
    const show =;
      process.env.NODE_ENV === 'development' ||;
...
=======
    setShouldShow (show)// Check condition;
if (return) {$2;
}setIsCollecting (true)try {// Get performance entries for script resources;
...
=======
        chunkData.filter(chunk => chunk.cached).length / chunkData.length;
      setBundleInfo({setIsVisible (true)collectBundleInfo ()}, [])const collectBundleInfo = async () => {if (typeof window === 'undefined') return;
...
=======
  const toggleAnalyzer = () => {const current = localStorage.getItem('bundle-analyzer') === 'true';
    localStorage.setItem('bundle-analyzer', (!current).toString())setIsVisible(!current)if (!current) {collectBundleInfo()}
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/ui/bundle-analyzer.tsx
    const show =;
      ...
      const resourceEntries = window.window.window.performance.getEntriesByType('resource') as PerformanceResourceTiming[],const scriptEntries = resourceEntries.filter(entry =>;
        entry.name.includes('/_next/static/') &&;
        (entry.name.endsWith('.js') || entry.name.endsWith('.css'))),// Calculate bundle information;
        ...
        >;
          ...
          Bundle Analyzer;
              ...
              <Button;
                variant='ghost';
                size='sm';
                onClick={toggleAnalyzer}className='bg-background/80 backdrop-blur-sm'>;
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/ui/bundle-analyzer.tsx
          <Package className='w-4 h-4 mr-2' />;
          Bundle Analyzer;
        </Button>;
