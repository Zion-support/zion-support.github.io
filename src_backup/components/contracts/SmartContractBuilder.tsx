...
export function SmartContractBuilder() {const [activeTab, setActiveTab] = useState<string>("form)// State for Solidity contract(existing)const [generatedSolidityContract, setGeneratedSolidityContract] = useState<string | null>(null)// New state for Markdown legal draft;
  ...
  // For now, let's assume they can be part of formValues or derived if prefilled.// Well use formValues.clientWalletAddress and formValues.talentWalletAddress;
...
const ESCROW_AGREEMENT_ABI: ethers && ethers.InterfaceAbi = [// From Ownable;
  ...
  "event Deposited(address indexed payee, uint256 weiAmount),event Withdrawn(address indexed payee, uint256 weiAmount)",// EscrowAgreement specific(based on previous subtasks template)"constructor(address _talent, address _client, string memory _projectDetailsIPFSHash), // Note: Ownable takes _client;
 ...
 */;
  ...
  // For now, lets assume they can be part of form_values or derived if prefilled.// We'll use form_values.clientWalletAddress and form_values.talentWalletAddress;
  ...
  // States for on-chain agreement UI and deployment;
  const [enableOnChainAgreement, setEnableOnChainAgreement] = useState<boolean>(false)const [selectedNetwork, setSelectedNetwork] = useState<ethereum' | 'polygon | '>(')// clientWalletAddress and talentWalletAddress might be part of formValues or separate state;
  // For now, lets assume they can be part of formValues or derived if prefilled.// We'll use formValues && formValues.clientWalletAddress and formValues && formValues.talentWalletAddress;const [onChainDeploymentStatus, setOnChainDeploymentStatus] = useState<'idle | connecting' | 'fetching_code | deploying' | 'success | error'>('idle)const [transactionHash, setTransactionHash] = useState<string | null>(null)const [deployedContractAddress, setDeployedContractAddress] = useState<string | null>(null)const [deploymentError, setDeploymentError] = useState<string | null>(null)const [populatedSolidityCode, setPopulatedSolidityCode] = useState<string | null>(null)// Bytecode from generate-smart-contract;
  ...
  // For generating Solidity (existing logic, adapted)return}try {// Assuming generateSolidityContract now primarily uses formValues;
      ...
      // For now, lets assume the main "Preview tab can show Solidity if deployOptions && deployOptions.deployToChain is true;
      // Or if theres a specific action for Preview Solidity";
...
'
    ...
    setOnChainDeploymentStatus('connecting)setDeploymentError(null)setTransactionHash(null)setDeployedContractAddress(null)setPopulatedSolidityCode(null)try {if(!window.ethereum) {throw new Error("MetaMask is not installed.Please install it to continue.")}
      ...
      setOnChainDeploymentStatus('fetching_code)// Determine contractType(e.g. from a form field if its selectable, or default)// For now, let's assume it's part of formValues or a fixed choice for this flow.const contractTypeToDeploy: ContractType = formValues.contractType |simple; // Default to 'simple';
...
if ( { // Chain not added) {}
  ...
  //This should be a function that takes a string (contract content) //Since we need to adapt the interface, well implement the simplest solution that works if (onContractGenerated) {setGeneratedContract (contract);
