...
export function TalentProfile(): any ({
  ...
  const skillsArray =;
    profile && profile.skills?.map(skill => ({
      name: skill,'
      level: 3, // Default level since we dont have this data;
    })) || [];
...
export function TalentProfile(): any ({profile,onRequestHire,onMessageTalent}: TalentProfileProps) {const { isAuthenticated }  = useAuth()// Create proper availability object from talent profile;
  ...
  const projectsArray =;
    profile && profile.key_projects?.map((proj, i) => ({id: `project-${i}`,title: proj && proj.title,description: proj && proj.description,date: new Date().toISOString(), // Default date since we dont have this data;
    })) || [];interface TalentProfileProps  {profile: TalentProfileType;
  onRequestHire: () => void;
  onMessageTalent?: () => void;
...
import { Availability } from '@/types / profile';
  ...
  const projectsArray = profile.key_projects?.map((proj, i) => ({}`
    ...
    date: new Date().toISOString() // Default date since we dont have this data;
  })) || [],
...
function TalentProfile() {const { is_authenticated } = use_auth ()// Create proper availability object from talent profile;
  ...
  // Create proper skills array for ProfileSkills component;
  const skills_array =;
    profile.skills?.map (skill => ({name: skill,level: 3, // Default level since we dont have this data;
    })) || [];
  // Create proper projects array for ProfileProjects component;
  ...
  const projectsArray = profile.key_projects?.map((proj, i) => ({id: `project-${i}`,title: proj.title,description: proj.description,date: new Date().toISOString() // Default date since we dont have this data;
