...
import { toast  } from 'sonner';
import { JobApplication  } from '@/types/jobs';
import { useState } from "react",import { Badge } from "@/components/ui/badge",import { Button } from "@/components/ui/button",import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card",import { supabase } from "@/integrations/supabase/client",interface ApplicationScoreCardProps  {application: JobApplication;
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/jobs/ApplicationScoreCard.tsx
  onScoreUpdated?: (updatedApplication: JobApplication) => void;
}
export function ApplicationScoreCard() {const [isScoring, setIsScoring] = useState(false),// Determine if application has been scored;
  ...
  const getSuggestionColor = (suggestion: string | undefined,) => {switch (suggestion) {// Get suggestion color;
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/jobs/ApplicationScoreCard.tsx
  ...
  const handleScore = async () => {try {setIsScoring(true)// Call the trigger_resume_scoring function;
      const { error } = await supabase.rpc('trigger_resume_scoring';
        { application_id: application.id }),if (error) throw error,toast.success("Resume scoring has been initiated"),// Poll for results every 3 seconds for up to 30 seconds;
      let attempts = 0;
      const maxAttempts = 10;
      const checkScore = async () => {attempts++;
        const { data, error } = await supabase;
          .from("job_applications").select("*").eq("id", application.id).single()if (error) {interface ApplicationScoreCardProps  {application: JobApplication,onScoreUpdated?: (updatedApplication: JobApplication,) => void;
...
if (throw error, ) {$2;
      ...
      let attempts = 0,const max_attempts = 10,const check_score = async () => {attempts++,const { data, error } = await supabase;
...
=======
                      </div>)}{application.match_breakdown.certifications_match && (<div>;
                        <p className="font - medium">Certifications Match: {application.match_breakdown.certifications_match.score}/100</p>;
