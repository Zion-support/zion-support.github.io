<<<<<<< HEAD:src_backup/components/jobs/JobMatchCard.tsx
}
...
>>>>>>> origin/cursor/fix-netlify-build-and-merge-to-main-2a0c:src/components/jobs/JobMatchCard.tsx
  talentId: string;
  name: string;
  title: string;
  company: string;
...
}<div className="p-4" > <div className="flex justify-between items-start gap-2 mb-3" > <h3 className="text-lg font-bold line-clamp-2" > {name ";
}</h3> <Badge variant="default" > h-3 w-3" /> </AvatarFallback>)}</Avatar> </Badge>) )}</div> Apply Now </Button>)}</div> </div> </CardContent> </Card>)}";
}
<<<<<<< HEAD:src_backup/components/jobs/JobMatchCard.tsx
