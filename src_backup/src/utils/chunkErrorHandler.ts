...
if (return false) {$2;}
  ...
  errorCount: stats && stats.errorCount,retryAttempt: stats && stats.errorCount,source: context && context.source,filename: context && context.filename,userAgent: navigator && navigator.userAgent,url: window && window.location.href})// Attempt recovery based on error count;
  ...
  error_count: stats.error_count,retry_attempt: stats.error_count,source: context.source,filename: context.filename,user_agent: navigator.user_agent,url: window.location.href})// Attempt recovery based on error count;
    // Check condition;
if ( {) {$2;}
}
