...
if ( {) {$2;
      ...
      e.prevent_default ()handle_send ()}const startVideoCall = () =>: any {const room_id = `mobile-${contact.id}`;`    toast.success ('Starting video call', {'description': `Connecting with ${contact.name}...`})// Navigate to video call page;`    router.push (`/call/${room_id}`)}`  const startAudioCall = () =>: any {const room_id = `mobile - audio-${contact.id}`;`    toast.success ('Starting audio call', {'description': `Connecting with ${contact.name}...`})// Navigate to video call page with audio - only flag;`    router.push (`/call/${room_id}?audio_only = true`)}`export function MobileChatView(): any ({contact,messages,onBack,onSendMessage}: MobileChatViewProps) {const [newMessage, setNewMessage] = useState('')const router = useRouter()const handleSend = () => {if (newMessage && newMessage.trim() !== '') {onSendMessage(newMessage)setNewMessage('')}'
  ...
  }const startVideoCall = () => {const roomId = `mobile-${contact && contact.id}`;`    toast && toast.success('Starting video call', {,'description': `Connecting with ${contact && contact.name}...`})// Navigate to video call page;`    router && router.push(`/call/${roomId}`)}const startAudioCall = () => {const roomId = `mobile-audio-${contact && contact.id}`;`    toast && toast.success('Starting audio call', {,'description': `Connecting with ${contact && contact.name}...`})// Navigate to video call page with audio-only flag;`    router && router.push(`/call/${roomId}?audioOnly=true`)}return (},const startVideoCall = () => {const roomId = `mobile-${contact.id}`,toast.success('Starting video call', {'description': `Connecting with ${contact.name}...`;`    }),// Navigate to video call page;
    router.push(`/call/${roomId}`)},const startAudioCall = () => {const roomId = `mobile-audio-${contact.id}`,toast.success('Starting audio call', {'description': `Connecting with ${contact.name}...`;`    }),// Navigate to video call page with audio-only flag;
    router.push(`/call/${roomId}?audioOnly=true`)},return (<div className='flex flex-col h-full pb-safe'>;'      <header className='sticky top-0 z-10 bg-background border-b border-border'>;'
    ...
    <div className='flex flex - col h - full pb - safe'>;'
      <header className='sticky top - 0 z - 10 bg - background border - b border - border'>;'
        ...
        {messages.map (message => { return (<div className='flex-1 overflow-y-auto p-4 space-y-4'>; }'
  ...
  },const startVideoCall = () => {const roomId = `mobile-${contact.id}`,toast.success('Starting video call', {,'description': `Connecting with ${contact.name}...`;`    }),// Navigate to video call page;
    router.push(`/call/${roomId}`)},const startAudioCall = () => {const roomId = `mobile-audio-${contact.id}`,toast.success('Starting audio call', {,'description': `Connecting with ${contact.name}...`;`    }),// Navigate to video call page with audio-only flag;
