...
interface AuthGuardProps  {children: React.ReactNode;
  ...
  description: 'Please log in to access this feature.',variant: 'destructive'})}const returnTo = encodeURIComponent(router.asPath)router.push(`${redirectTo}?returnTo=${returnTo}`)return;
  ...
  description: `This feature requires ${requireRole.join(' or ')} privileges.`,variant: 'destructive'})}
...
export function useAuthGuard() {const { user, isAuthenticated, isLoading } = useAuth()const router  = useRouter()const requireAuth = (if (isLoading) return false;if (!isAuthenticated) {const redirectTo = options?.redirectTo || '/login';) => {
  return $3;}
}
      const returnUrl  = options?.returnUrl || router.asPath;if (options?.showToast !== false) {toast({title: 'Authentication Required',}
  description: 'Please log in to continue.',variant: 'destructive'})}router.push(`${redirectTo}?returnTo=${encodeURIComponent(returnUrl)}`)return false;
    }return true;
  }const requireRole = (if (!requireAuth({ showToast: false ) => {
  return $3;}
})) return false;const userRoles = user?.role ? [user.role] : [];
  ...
  description: `This feature requires ${roles.join(' or ')} privileges.`,variant: 'destructive'})}router.push(options?.redirectTo || '/dashboard')return false;
    }return true;
