...
const stockStatus =;
  ...
  const productTitle = product.title,const addToCart = (if (!isAuthenticated) {toast({title: 'Login Required',
  description: 'Please log in to add items to your cart.',variant: 'destructive') => {
  return $3;}
}),router.push(`/auth/login?returnTo=${encodeURIComponent(router.asPath)}`),return;
    }
    dispatch(addItem({ id: product.id, title: productTitle, price: product.price ?? 0 })),toast({title: 'Added to cart',}
  description: `${productTitle} has been added to your cart`,action: {label: 'View Cart',}
  onClick: () => router.push('/cart')}})},const imageUrl = Array.isArray(product.images) && product.images.length > 0 ? product.images[0] : null,const imageAltText = productTitle,const handleImageError = (if (!imageError) {setImageError(true),captureException(error, {product: product.id,imageUrl) => {
  return $3;}
})}
        ...
        {onBuy && (<TooltipProvider />;
            <Tooltip />;
              <TooltipTrigger asChild />;
                <Button;}
                  onClick={(e) = /> {e.stopPropagation(),if (onBuy) {setIsRedirecting(true),onBuy().catch(() => {// Error is handled by parent, but we still need to reset loading locally;}
                        }).finally(() => {setIsRedirecting(false), // Always reset loading state;}
