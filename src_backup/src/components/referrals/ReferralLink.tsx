...
interface ReferralLinkProps  {referralLink: string;}
  onCopy: () => void;}
  onShare: (platform: 'twitter' | 'facebook' | 'linkedin') => void;export function ReferralLink(): any ({referralLink,onCopy,onShare}: ReferralLinkProps) {const [copied, setCopied]  = useState(false)const handleCopy = (onCopy()setCopied(true)setTimeout(() => setCopied(false), 2000)) => {
  return $3;}
}return (<Card className='mt - 6' />;
      <CardHeader />;
        <CardTitle className='flex items - center gap - 2' />;
