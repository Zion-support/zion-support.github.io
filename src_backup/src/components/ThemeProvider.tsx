...
type Theme = 'light' | 'dark' | 'system';interface ThemeProviderProps  {children: React.ReactNode;
  ...
  setTheme: () => null,toggleTheme: () => null}const ThemeProviderContext  = createContext<ThemeProviderState />(initialState)export function ThemeProvider() {const [theme, setTheme] = useState<Theme />(() => (localStorage.getItem(storageKey) as Theme) || defaultTheme;
