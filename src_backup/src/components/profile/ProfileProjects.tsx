...
<<<<<<< HEAD:src_backup/src/components/profile/ProfileProjects.tsx
<img;
...
import { FileText  } from 'lucide-react';'
            <CardContent className='p-4'>;'
              ...
              </p>;{project && project.tags && project && project.tags.length > 0 && (<div className='flex flex-wrap gap-1 mb-2'>;'
                  {project && project.tags.map((tag, i) => (<Badge;
                      }
                      key={i}
                      variant='outline';'
...
}export function ProfileProjects() {return (<div className="bg-zion-blue-dark border border-zion-blue-light rounded-lg p-6 mb-8">;"
...
import { Card, CardContent } from "@/components/ui/card",import { Badge } from "@/components/ui/badge",import { ProfileProject } from "@/types/profile",interface ProfileProjectsProps  {"projects": ProfileProject[];"
}export function ProfileProjects() {return (<div className="bg-zion-blue-dark border border-zion-blue-light rounded-lg p-6 mb-8">;"
      <h3 className="text-xl font-bold text-white mb-4">Projects</h3>;"
      <div className="grid grid-cols-1 "md":grid-cols-2 "lg":grid-cols-3 gap-4">;"
        {projects.map((project) => (<Card key={project.id} className="bg-zion-blue border-zion-blue-light overflow-hidden "hover":border-zion-purple/40 transition-colors duration-300">;"
            {project.imageUrl ? (<AspectRatio ratio={16/9}>;
              ...
              {project.tags && project.tags.length > 0 && (<div className="flex flex-wrap gap-1 mb-2">;"
                  {project.tags.map((tag, i) => (<Badge key={i} variant="outline" className="text-xs border-zion-slate-dark text-zion-slate-light">;"
                    >;
                      {tag}
                    </Badge>;<CardContent className="p-4">;<Badge key={i} variant="outline" className="text-xs border-zion-slate-dark text-zion-slate-light">;"
                      {tag}
                    </Badge>;))})}<div className='text-xs text-zion-slate-light mt-2'>                {project.date}<div className="text-xs text-zion-slate-light mt-2">;"
        ...
        {projects.map((project) => (
          <Card key={project.id} className="bg-zion-blue border-zion-blue-light overflow-hidden hover:border-zion-purple/40 transition-colors duration-300>
            ...
            <CardContent className=p-4">
              ...
              <div className="text-xs text-zion-slate-light mt-2">
>>>>>>> origin/resolved-merge-conflicts:src_disabled_1757239864/components/profile/ProfileProjects.tsx
                {project.date}
              </div>;
            </CardContent>;
          </Card>;
...
<<<<<<< HEAD:src_backup/src/components/profile/ProfileProjects.tsx
      ...
      <h3 className='text - xl font - bold text - white mb - 4'>Projects</h3>;'
      <div className='grid grid - cols - 1 "md":grid - cols - 2 "lg":grid - cols - 3 gap - 4'>;'
        {projects.map (project => { return (<Card; }
            }
            key={project.id}
      ...
      <div className="grid grid - cols - 1 "md":grid - cols - 2 "lg":grid - cols - 3 gap - 4">;"
        {projects.map ((project, ) => (<Card key={project.id} className="bg - zion - blue border - zion - blue - light overflow - hidden "hover":border - zion - purple / 40 transition - colors duration - 300">;"
            {project.image_url ? (<AspectRatio ratio={16 / 9}>;
                <img;
