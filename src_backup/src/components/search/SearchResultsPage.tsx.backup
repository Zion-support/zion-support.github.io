  ...
  return (<div on_click={handle_click} className={card_class}>;
      ...
      <div className='flex - 1'>;'
        ...
        <div className='flex items-start justify-between mb-2'>;'
          ...
          {result && result.price && (<div className='text-right'>;'
              <span className='font-bold text-primary'>;'
                {result && result.currency === 'USD' ? '$' : ''}'
                {result && result.price}
              </span>;
              {result && result.type === 'talent' && (<span className='text-sm text-muted-foreground'>/hr</span>;)}'
            </div>;
...
=======
    ...
    : "bg-card border rounded-lg p-4 "hover":shadow-lg transition-shadow cursor-pointer flex gap-4",return (<div onClick={handleClick} className={cardClass}>;"
      {result.image && (<div className={viewMode === 'grid' ? "mb-3" : "flex-shrink-0"}>;"
        ...
        <p className="text-muted-foreground mb-3 line-clamp-2">;"
>>>>>>> origin/resolved-merge-conflicts
          <HighlightText text={result.description} searchTerm={searchTerm} />;<p className='text-muted-foreground mb-3 line-clamp-2'>;'
          <HighlightText text={result && result.description} searchTerm={searchTerm} />;
        </p>;<div className='flex items-center justify-between'>;'
          <div className='flex gap-2 flex-wrap'>;'
            {result && result.category && (<Badge variant='outline' className='text-xs'>;'
                {result && result.category}
              </Badge>;)}</Badge>)}
            {result.tags?.slice (0, 3).map ((tag, index) => (<Badge key={index} variant='outline' className='text - xs'>                <HighlightText text={tag} search_term={search_term} />;'
              </Badge>))}
...
>>>>>>> origin/resolved-merge-conflicts
          ...
          </div>;{result && result.rating && (<div className='flex items-center gap-1'>;'
              <span className='text-yellow-500'>★</span>;'
...
<<<<<<< HEAD
export const 'SearchResultsPage': React.FC = () => {},export const 'SearchResultsPage': React.FC = () => {},// Main Search Results Page Component;
...
>>>>>>> origin/resolved-merge-conflicts
  ...
  // Active filters count;
    ...
    (filters.min_rating > 0 ? 1 : 0)return (<div className='container mx - auto px - 4 py - 6'>;'
      {/* Search Header */}
      <div className='mb - 6'>;'
        <div className='flex gap - 4 mb - 4'>;'
          <div className='flex - 1'>;'
        ...
        {search_term && (<div className='flex items - center justify - between'>;'
            <div>;
              <h1 className='text - 2xl font - bold'>Search Results</h1>;'
              <p className='text - muted - foreground'>;'
                {loading;
...
>>>>>>> origin/resolved-merge-conflicts
              ...
              </div>;{/* Mobile Filter Toggle */}{/* View Mode Toggle */}
...
>>>>>>> origin/resolved-merge-conflicts
                    ...
                    {activeFiltersCount > 0 && `(${activeFiltersCount})`}`                  </Button>;
                  ...
                  <div className='mt - 6'>;'
                    <FilterSidebar;
                      ...
                      filters = {filters }
                      onFiltersChange = {handleFiltersChange }
                      available_categories = {available_categories }<Sheet open={showFilters} onOpenChange={setShowFilters}>;
...
>>>>>>> origin/resolved-merge-conflicts
                  ...
                  {results.map((result) => (<SearchResultCard;
                      ...
                      key={`${result.type}-${result.id}`}`                      result={result}
...
} /> <SearchResultCard key= {`$ {result.type;`}-$ {result.id;
}`;`}result= {result;
}searchTerm= {searchTerm;
}viewMode= {viewMode;
<<<<<<< HEAD
...
export default SearchResultsPage,export default SearchResultsPage;</div>;
  )}export default SearchResultsPage;</div>)}
      </div>;
...
>>>>>>> origin/resolved-merge-conflicts
                ...
                >;
                  {results.map (result => { return (                    <SearchResultCard; }
                      }
                      key={`${result.type}-${result.id}`}`                      result = {result }
                      search_term = {search_term }
                      view_mode = {view_mode } />))}
...
export default SearchResultsPage;
'';'
export default SearchResultsPage,export default SearchResultsPage,{hasMore && ('hasMore': boolean;
=======
} />) )}</div> > {'";"
  }
  loading ? (<> <Loader2 className="h - 4 w - 4 mr - 2 animate - spin" /> Loading... </>) : ('Load More Results')}</Button> </div>)}</>)}</div> </div>)}</div>)}'
