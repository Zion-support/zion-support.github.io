      ...
      <CardContent>;
        <div className="grid grid-cols-1 "sm":grid-cols-2 gap-4">;"
          {actions.map(action => { return (<Button; }key={action.id}
              ...
              variant='outline'              className={`h-auto justify-start p-4 ${action && action.color}`}>;`              <div className='flex flex-col items-start text-left'>;'
                <div className='flex items-center'>;'
                  {action && action.icon}
          ...
          {actions && actions.map(action => { return (<Button; }
              ...
              key={action && action.id}
              variant="outline";"
              className={`h-auto justify-start p-4 ${action && action.color}`}>;`              <div className="flex flex-col items-start text-left">;"
                <div className="flex items-center">;"
                  {action && action.icon}
                  <span>{action && action.label}</span>;
                </div>;
