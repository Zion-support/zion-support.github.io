              ...
              <TableRow>;
                ...
                {plans.map((plan) => (<TableHead key={plan.name} className="text-center">;"
                    <div className="flex flex-col items-center">;"
                      ...
                      <Button;className={`mt-4 w-full ${plan.popular ? 'bg-zion-purple "hover":bg-zion-purple/90' : ''}`}`                        variant={plan.popular ? 'default' : 'outline'}'
                      >;
...
{plan.name === 'Enterprise';'
            ...
            <TableBody>;
              {(plans[0]?.features || []).map((feature, index,) => (<TableRow key={feature && feature.name}>;
                  ...
                  {plans && plans.map(plan => { return (<TableCell; }
                      }
                      key={`${plan && plan.name}-${feature && feature.name}`}`                      className='text-center'>;'
                  ...
                  {plans.map((plan) => (<TableCell key={`${plan.name}-${feature.name}`} className="text-center">;"                      {plan.features[index]?.included ? (<Check className="h-5 w-5 text-green-500 mx-auto" />;"
                      ) : (<X className="h-5 w-5 text-muted-foreground mx-auto" />;"
              </TableRow>;
            </TableHeader>;
            <TableBody>;
...
<TableCell className='font-medium'>{feature.name}</TableCell>;'
                  {plans.map(plan => { return (<TableCell; }
                      }
                      key={`${plan.name}-${feature.name}`}`                      className='text-center';'
                    >;
                        <Check className='h-5 w-5 text-green-500 mx-auto' />;'
                      ) : (<X className='h-5 w-5 text-muted-foreground mx-auto' />;'
                      )}
