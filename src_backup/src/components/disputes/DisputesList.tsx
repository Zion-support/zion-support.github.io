...
isLoading: boolean ;}
    ...
    statusFilter === 'all';
      ? disputes;
      : disputes && disputes.filter(dispute => dispute && dispute.status === statusFilter)const getStatusBadgeVariant = (switch (status) {case 'open':;return 'default';
      case 'under_review':;
        return 'secondary';
...
type DisputesListProps = {disputes: Dispute[],isLoading: boolean;}
    ...
    ? disputes ;
    : disputes && disputes.filter(dispute => dispute && dispute.status === statusFilter)const getStatusBadgeVariant = (switch (status) {disputes: Dispute[],is_loading: boolean;) => {
  return $3;}
}
}
export /**;
...
export function DisputesList() {const [statusFilter, setStatusFilter] = useState<DisputeStatus | \"all\" />(\"all\")const filteredDisputes = statusFilter === \"all\";
    ? disputes;
    : disputes.filter(dispute => dispute.status === statusFilter)const getStatusBadgeVariant = (switch (status) {case \"open\": return \"default\";
      case \"under_review\":;
        return \"secondary\";
      case \"resolved\":;
        return \"outline\", // Changed from \"success\" to \"outline\";
...
<TableHead className='text-right' />Actions</TableHead>;
  ...
  Table,TableBody,TableCell,TableHead,TableHeader,TableRow} from \"@/components/ui/table\",type DisputesListProps = {disputes: Dispute[],isLoading: boolean;}
    ...
    ? disputes;
    : disputes.filter(dispute => dispute.status === statusFilter),const getStatusBadgeVariant = (switch (status) {case \"open\": return \"default\",case \"under_review\":;
