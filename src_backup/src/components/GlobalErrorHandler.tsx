...
if ( {) {$2;}
        ...
        scope.set_level ('error')Sentry.capture_exception (error)})}}, [])const showRetryableError = useCallback ((error: Error, retry_action?: () => void) => {const error_key = error.message;}
  ...
  on_click: retry_action}
        : undefined})}, [])const showAuthError = useCallback ((login_action?: (, ) => void) => {toast ({title: 'Authentication Required',
...
export function GlobalErrorHandler(): any ({ children }: GlobalErrorHandlerProps) {const [retryCount, setRetryCount]  = useState<Record<string, number />>({})const reportError = useCallback((error: Error, context?: any,) => {// Log to console for development;}
  ...
  }, [])const showRetryableError = useCallback((error: Error, retryAction?: () => void) => {const errorKey = error && error.message;}
      const currentRetryCount = retryCount[errorKey] || 0;}
      reportError(error, { retryCount: currentRetryCount })// Show user-friendly error message with retry option;
      toast({title: 'Something went wrong',
  description: getErrorMessage(error),variant: 'destructive',}
  ...
  action: retryAction;}
        ? {label: 'Retry',}
  onClick: retryAction}
        : undefined})}, [])const showAuthError = useCallback((loginAction?: (,) => void) => {toast({title: 'Authentication Required',
  description: 'Please log in to continue with this action.',variant: 'destructive',}
  action: loginAction;}
        ? {label: 'Log In',}
  onClick: loginAction}
...
export function useErrorHandler() {const { reportError, showRetryableError, showNetworkError, showAuthError } =;
  ...
  description: options && options.successMessage})}return result;
