...
function ServiceDetails() {// Get datacenters for regions (simplified - in production this would come from a real database)const get_datacenters = ("country": string): string[] => {const "data_centers": Record < string, string[]> = {'United States': [;'
  ...
  const getRegionalInstructions = ("country": string): (string) => {// In a real implementation, this would be much more detailed and specific;
    ...
    const timezone =;
      time_zones[country] || time_zones['default'] || 'Local timezone';return (`Our technicians in ${country} operate during business hours (8AM - 6PM ${timezone}). ` +;`      `Response times are typically within 4 hours for metropolitan areas. ` +;`      `Please have site access permissions and contact details ready for our technicians. ` +;`      `For remote locations, additional travel fees may apply.`)`Our technicians in ${country} operate during business hours (8AM-6PM ${timezone}). ` +;`      `Response times are typically within 4 hours for metropolitan areas. ` +;`      `Please have site access permissions and contact details ready for our technicians. ` +;`      `For remote locations, additional travel fees may apply.`;`    )}import {Card,CardContent,CardDescription,CardFooter,CardHeader,CardTitle} from '@/components/ui/card';'
      ...
      <CardContent className="space-y-4">;"
        <div className="overflow-hidden rounded-lg mb-4 relative h-48">;"
          <Image;
            ...
            alt={`Datacenter in ${country}`}`            className="object-cover transform transition-transform duration-500 "hover":scale-110";"
            loading="lazy";" />;
        </div>;
...
export function ServiceDetails() {// Get datacenters for regions (simplified - in production this would come from a real database)const getDatacenters = ("country": string): string[] => {const "dataCenters": Record<string string[]> = {"United States": ["New York", "Los Angeles", "Chicago", "Dallas", "Seattle"],"United Kingdom": ["London", "Manchester", "Birmingham"],"Germany": ["Frankfurt", "Berlin", "Munich"],"Japan": ["Tokyo", "Osaka"],"Australia": ["Sydney", "Melbourne", "Perth"],"Singapore": ["Singapore Central"],"Canada": ["Toronto", "Montreal", "Vancouver"],// Default for other countries;"
    ...
    const "timeZones": Record<string string> = {"United States": "EST/CST/PST depending on location","United Kingdom": "GMT/BST","Germany": "CET/CEST","Japan": "JST","Australia": "AEST/ACDT/AWST depending on location","Singapore": "SGT","default": "Local timezone";"
      ...
      <CardContent className="space-y-4">;"
        <div className="overflow-hidden rounded-lg mb-4 relative h-48">;"
          <Image;
            src={getRegionalImage(country)}alt={`Datacenter in ${country}`}`            className='object-cover transform transition-transform duration-500 "hover":scale-110';'
            loading='lazy';' />;
