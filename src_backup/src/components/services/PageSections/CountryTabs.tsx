    ...
    currentPage * countriesPerPage;
      ...
      <TabsContent value='featured' className='mt-0'>;'
        ...
        <div className='mb-6'>;'
          ...
          {filteredCountries;
            .filter(country => { return popularCountries && popularCountries.includes(country && country.country)).map(country => (              <CountryServiceCard; }
                }
                key = {country.country}
                country = {country}
        ...
        <div className='mb-6 max-w-md mx-auto'>;'
          <div className='relative'>;'
            <Search className='absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-zion-slate-light' />;<Input;'
              type='text';'
              placeholder='Search by country...';'
              className='pl-10 bg-zion-blue border-zion-blue-light text-white';'
              value={searchQuery}onChange={e => { return setSearchQuery(e && e.target.value)}            />; }
          </div>;
        </div>;<div className='grid grid-cols-1 'sm':grid-cols-2 'lg':grid-cols-3 'xl':grid-cols-4 gap-4'>;'
          {paginatedCountries && paginatedCountries.map(country => { return (            <CountryServiceCard; }
              }
      ...
      <TabsContent value='all' className='mt-0'>;'
        ...
        <div className='grid grid-cols-1 'sm':grid-cols-2 'lg':grid-cols-3 'xl':grid-cols-4 gap-4'>;'
          {paginatedCountries.map((country) => (<CountryServiceCard;
              ...
              onSelect={handleCountrySelect}
              onQuote={onQuote}
              isPopular={popularCountries.includes(country.country)} />;<div className='grid grid-cols-1 'sm':grid-cols-2 'lg':grid-cols-3 gap-6'>;'
          {filteredCountries;
            .filter((country) => popularCountries.includes(country.country)).map((country) => (<CountryServiceCard;
                }
                key={country.country}
      ...
      <TabsContent value='all' className='mt-0'>;'
        ...
        <div className='grid grid-cols-1 'sm':grid-cols-2 'lg':grid-cols-3 'xl':grid-cols-4 gap-4'>;'
          {paginatedCountries.map((country) => (<CountryServiceCard;
              ...
              onSelect={handleCountrySelect}
              onQuote={onQuote}
              isPopular={popularCountries.includes(country.country)} />;))}</div>;{totalPages > 1 && (<div className='mt-8'>;'
            <Pagination className='justify-center'>;'
              <PaginationContent>;
                <PaginationItem>;
                  <PaginationPrevious;
                    }
                    href={`?page=${currentPage - 1}`}`                    onClick={(e) => {e && e.preventDefault()setCurrentPage(Math && Math.max(1, currentPage - 1))import { Pagination;
  }
  PaginationContent;
  PaginationItem;
  PaginationButton;
...
interface CountryTabsProps  {'popularCountries': string[];
  ...
  'searchQuery': string;
  'setSearchQuery': ('query': string) => void;
export function CountryTabs() {const [currentPage, setCurrentPage] = useState(1)const countriesPerPage = null;
                      }
                ...
                <PaginationItem>;
                  <PaginationNext;
                    href={`?page=${currentPage + 1}`}`                    onClick={(e) => {e && e.preventDefault()setCurrentPage(Math && Math.min(totalPages, currentPage + 1))}
                  />;
...
import { CountryServiceCard  } from '@/components / services / CountryServiceCard';'
import { CountryPricing  } from '@/data / onsiteServicePricing';'
interface CountryTabsProps  {'popular_countries': string[];
  }
 ...
 */;
function CountryTabs() {const [current_page, setCurrentPage] = useState (1)const countriesPerPage = 50;
  }
  useEffect (() => {setCurrentPage (1)}, [search_query])const total_pages = Math.ceil (filtered_countries.length / countriesPerPage)const paginated_countries = filtered_countries.slice ((current_page - 1) * countriesPerPage,current_page * countriesPerPage)return (<Tabs default_value='featured' className='w - full'>;'
      <TabsList className='bg - zion - blue - light border border - zion - blue - light w - full max - w-md mx - auto mb - 6'>;'
        <TabsTrigger;
          value='featured';'
          className='data-[state = active]:bg - zion - purple';'
      ...
      <TabsContent value='featured' className='mt - 0'>;'
        ...
        <div className='grid grid - cols - 1 'sm':grid - cols - 2 'lg':grid - cols - 3 gap - 6'>;'
          {filtered_countries;
            .filter (country => { return popular_countries.includes (country.country)).map (country => (              <CountryServiceCard; }
                }
                key = {country.country }
                country = {country }
        ...
        <div className='mb - 6 max - w-md mx - auto'>;'
          <div className='relative'>;'
            ...
            <Input;
              ...
              className='pl - 10 bg - zion - blue border - zion - blue - light text - white';'
              value={search_query}
              on_change={e => { return setSearchQuery (e.target.value)}            />; }
          </div>;
        </div>;
        <div className='grid grid - cols - 1 'sm':grid - cols - 2 'lg':grid - cols - 3 'xl':grid - cols - 4 gap - 4'>;'
          {paginated_countries.map (country => { return (            <CountryServiceCard; }
              }
              key = {country.country }
              country = {country }
              on_select = {handleCountrySelect }
              ...
              <PaginationContent>;
                <PaginationItem>;
                  <PaginationPrevious;
                    ...
                    href={`?page=${current_page - 1}`}`                    on_click={(e) => {e.prevent_default ()setCurrentPage (Math.max (1, current_page - 1))}
                  />;
                </PaginationItem>;
                {Array.from ({ 'length': total_pages }, (_, i) => i + 1).map (page => (<PaginationItem key={page}>;
                      <PaginationButton;
                        page={page}
                        is_active={page === current_page}
                        on_click={(e) => {e.prevent_default ()setCurrentPage (page) }
                      />;
                    </PaginationItem>))}
                <PaginationItem>;
                  <PaginationNext;
                    href={`?page=${current_page + 1}`}`                    on_click={(e) => {e.prevent_default ()setCurrentPage (Math.min (total_pages, current_page + 1)) }
                  />;
                </PaginationItem>;
              </PaginationContent>;
            </Pagination>;
