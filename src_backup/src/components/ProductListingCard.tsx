...
if ( {) {$2;
  ...
  const handleViewListing = () => {// Debug logging for development;
    ...
    if (process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating 'to':', {'path': `${detailBasePath}/${listing.id}`;`      })logDebug('[ProductCard] Listing 'ID':', { 'id': listing.id })logDebug('[ProductCard] Listing 'Title':', { 'title': listing.title })}'
    ...
    router.push(`${detailBasePath}/${listing.id}`)}`  const dispatch = useDispatch<AppDispatch>()const addToCart = () => {setLoading(true)dispatch(addItem({'id': listing.id;
        }
        'title': listing.title;
...
}log_debug ('[ProductCard] Navigating 'to':', {'path': `${detailBasePath}/${listing.id}`})log_debug ('[ProductCard] Listing 'ID':', { 'id': listing.id })log_debug ('[ProductCard] Listing 'Title':', { 'title': listing.title })}'    // Validate listing ID exists before navigation;
    // Check condition;
if ( {) {$2;
}
    ...
    router.push (`${detailBasePath}/${listing.id}`)}`  const dispatch = use_dispatch < AppDispatch>()const addToCart = () =>: any {set_loading (true)dispatch (add_item ({'id': listing.id,'title': listing.title,'price': listing.price ?? 0}))toast.success (`1× ${listing.title} added`, {'action': {'label': 'View Cart','on_click': () => router.push ('/cart')})set_loading (false)}'  const handleRequestQuote = ('e': React.MouseEvent) =>: any {e.prevent_default ()e.stop_propagation ()// Check condition;
}
if ( {) {$2;
}
  ...
  const imageContainerClasses = is_grid ? 'h - 48' : 'h - 32 w - 48';'
    ...
    if (process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating 'to':', { 'path': `${detailBasePath}/${listing.id}` }),logDebug('[ProductCard] Listing 'ID':', { 'id': listing.id }),logDebug('[ProductCard] Listing 'Title':', { 'title': listing.title })}// Validate listing ID exists before navigation;'    if (!listing.id) {logErrorToProduction('[ProductCard] Missing listing ID, cannot navigate', new Error('Missing listing ID'), { 'component': 'ProductListingCard' }),toast({'title': 'Navigation Error','description': 'Product information is incomplete','variant': 'destructive'}),return;// Validate listing ID exists before navigation;'
  ...
  const router = useRouter()const [loading, setLoading] = useState(false)const [imageSrc, setImageSrc] = useState(listing && listing.images && listing && listing.images.length > 0 && listing && listing.images[0];
    ...
    if (process && process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating 'to':', {'path': `${detailBasePath}/${listing && listing.id}`})logDebug('[ProductCard] Listing 'ID':', { 'id': listing && listing.id })logDebug('[ProductCard] Listing 'Title':', { 'title': listing && listing.title })}// Validate listing ID exists before navigation;'    if (!listing && listing.id) {logErrorToProduction('[ProductCard] Missing listing ID, cannot navigate',new Error('Missing listing ID'),{ 'component': 'ProductListingCard' }'
      )toast({'title': 'Navigation Error','description': 'Product information is incomplete','variant': 'destructive'})return;'
    }router && router.push(`${detailBasePath}/${listing && listing.id}`)}const dispatch  = useDispatch<AppDispatch>()const addToCart = () => {setLoading(true)dispatch(addItem({'id': listing && listing.id,'title': listing && listing.title,'price': listing && listing.price ?? 0}))toast && toast.success(`1× ${listing && listing.title} added`, {'action': {'label': 'View Cart','onClick': () => router && router.push('/cart')})setLoading(false)}const handleRequestQuote = ('e': React && React.MouseEvent) => {e && e.preventDefault()e && e.stopPropagation()if (onRequestQuote) {onRequestQuote(listing && listing.id)} else {router && router.push(`/request-quote?listing=${listing && listing.id}`)}`  }const imageContainerClasses  = isGrid ? 'h-48' : 'h-32 w-48';onKeyDown={(e) => {if (e && e.key === 'Enter' || e && e.key === ' ') {e && e.preventDefault()handleViewListing()}      }'
    >;
      {/* Image */}<div;
  ...
  return (<div;
      ...
      className={`bg-card/70 backdrop-blur-md border border-primary/10 'sm':border-primary/20 rounded-lg overflow-hidden flex ${isGrid ? 'flex-col' : 'flex-row'} cursor-pointer focus-'visible':outline-none focus-'visible':ring-2 focus-'visible':ring-primary 'hover':animate-glowing-border transition-all duration-300`}`      onClick={handleViewListing}
      tabIndex={0}
      role='button';'
      onKeyDown={(e) => {if (e.key === 'Enter' || e.key === ' ') {e.preventDefault(),handleViewListing()}'
      }
...
if ( {) {$2;
      ...
      data - testid= 'equipment - link';'`;`      className={`bg - card / 70 backdrop - blur - md border border - primary / 10 'sm':border - primary / 20 rounded - lg overflow - hidden flex ${is_grid ? 'flex - col' : 'flex - row'} cursor - pointer focus - 'visible':outline - none focus - 'visible':ring - 2 focus - 'visible':ring - primary 'hover':animate - glowing - border transition - all duration - 300`}`      on_click={handleViewListing}
      tab_index={0}';'
      onKeyDown={(e) => {// Check condition;
}
if ( {) {$2;
...
if ( {) {$2;
      ...
      >;
        <div className={`relative ${imageContainerClasses}`}> {/* Ensure this container has dimensions */}`          <Image;
            src={imageSrc}
            alt={listing.title}
            fill={true}
      ...
      <div;
        className={`flex flex-col justify-between ${isGrid ? 'p-4 flex-1' : 'p-4 flex-1'}`}>;`        <div>;
