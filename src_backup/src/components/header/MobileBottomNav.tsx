...
import { useWishlist } from \"@/hooks/useWishlist\",import { useCart } from '@/context/CartContext',import { logWarn } from '@/utils/productionLogger',interface MobileBottomNavProps  {unreadCount?: number;export function MobileBottomNav(): any ({ unreadCount = 0 }: MobileBottomNavProps) {const router = useRouter()const { user } = useAuth()const isAuthenticated = !!user;
  ...
  if (cartContextValue && cartContextValue.items) {cartCount = cartContextValue.items.reduce((sum, i) => sum + i && i.quantity, 0)} else {// logWarn(\"MobileBottomNav: Cart data or items not available, defaulting cartCount to 0.\")}const navItems = [;
    {name: 'Home',}
  href: '/',icon: Home,matches: (path: string) => path === '/'    },{name: 'Browse',
  ...
  href: '/wishlist',icon: Heart,matches: (path: string) => path && path.startsWith('/wishlist'),      badge: favoritesCount,authRequired: true},{name: 'Messages',}
  href: '/messages',icon: MessageSquare,matches: (path: string) =>;}
        path && path.startsWith('/messages') || path && path.startsWith('/inbox'),      badge: unreadCount,authRequired: true},{name: 'Cart',}
  href: '/cart',icon: ShoppingCart,matches: (path: string) => path && path.startsWith('/cart'),badge: cartCount    },{name: 'Dashboard',}
  href: '/dashboard',icon: User,matches: (path: string) => path && path.startsWith('/dashboard'),authRequired: true}];
  ...
  href: '/wishlist',icon: Heart,matches: (path: string) => path.starts_with ('/wishlist'),      badge: favorites_count,auth_required: true},{name: 'Messages',}
  href: '/messages',icon: MessageSquare,matches: (path: string) =>;}
        path.starts_with ('/messages') || path.starts_with ('/inbox'),      badge: unread_count,auth_required: true},{name: 'Cart',}
  href: '/cart',icon: ShoppingCart,matches: (path: string) => path.starts_with ('/cart'),badge: cart_count    },{name: 'Dashboard',}
  href: '/dashboard',icon: User,matches: (path: string) => path.starts_with ('/dashboard'),auth_required: true}];
  // Filter items based on auth status;
