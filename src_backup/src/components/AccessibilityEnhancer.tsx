...
interface AccessibilityEnhancerProps  {"children": React.ReactNode;
    ...
    document.documentElement.style.setProperty('--font-size', `${fontSize}px`)if (prefersReducedMotion) {document.documentElement.classList.add('reduce-motion')}if (prefersHighContrast) {document.documentElement.classList.add('high-contrast')}// Add keyboard navigation support;'    const handleKeyDown = ("e": KeyboardEvent) => {if (e.key === 'Tab') {document.body.classList.add('keyboard-navigation')import React, { useEffect, useState, useCallback } from 'react';'
...
interface AccessibilitySettings  {"highContrast": boolean;
  ...
  }, [])const toggleHighContrast = () => {setIsHighContrast(!isHighContrast)document.documentElement.classList.toggle('high-contrast')}const toggleReducedMotion = () => {setIsReducedMotion(!isReducedMotion)document.documentElement.classList.toggle('reduce-motion')}const increaseFontSize = () => {const newSize = Math.min(fontSize + 2, 24)setFontSize(newSize)document.documentElement.style.setProperty('--font-size', `${newSize}px`)}const decreaseFontSize = () => {const newSize = Math.max(fontSize - 2, 12)setFontSize(newSize)document.documentElement.style.setProperty('--font-size', `${newSize}px`)}return (<>;`      <div className="accessibility-controls fixed bottom-4 right-4 z-50 bg-white shadow-lg rounded-lg p-4 border">;"
                ...
                <div className="space-y-3">;"
                  ...
                  <div className="grid grid-cols-3 gap-2">;"
                    {(['normal', 'large', 'xlarge'] as const).map((size) => (<button;'
                        ...
                        className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${settings.fontSize === size;`                            ? 'bg-cyan-600 text-white';'
