...
interface ChangelogManagerProps  {platform: AppPlatform;}
  ...
  }),const handleAddEntry = (if (!newEntry.version || !newEntry.changes) return,const entry: ChangelogEntry = {...newEntry,id: Math.random().toString(36).substring(2, 9)) => {
  return $3;}
},setEntries([entry, ...entries]),setNewEntry({version: \"\",date: new Date().toISOString().split('T')[0] || new Date().toLocaleDateString('en-CA'),changes: \"\";}
...
import { AppPlatform } from \"./MetadataManager\",interface ChangelogManagerProps  {platform: AppPlatform;}
  ...
  return $3;}
} = e.target,setNewEntry(prev => ({ ...prev, [name]: value }))},return (<Card className=\"bg-zion-blue border-zion-purple/30\" />;
      <CardHeader />;
        <CardTitle />Version History</CardTitle>;
      </CardHeader>;
      <CardContent />;
