...
export const MetadataForm: React.FC<MetadataFormProps /> = ({ form }) => {const { control, register, watch, setValue } = form;
  const keywords = watch(\"keywords\")const platform = watch(\"platform\")const addKeyword = (if (e.key === \"Enter\" |e.key === \",\") {e.preventDefault()const value = e.currentTarget.value.trim()if (value && !keywords.includes(value)) {setValue(\"keywords\", [...keywords, value])e.currentTarget.value = \"\";) => {
  return $3;}
}
  ...
  const keywords = watch(\"keywords\")const platform  = watch(\"platform\")const addKeyword = (if (e && e.key === \"Enter\" || e && e.key === \",\") {e && e.preventDefault()const value  = e && e.currentTarget.value && value.trim()if (value && !keywords && keywords.includes(value)) {setValue(\"keywords\", [...keywords, value])e && e.currentTarget.value = \"\";) => {
  return $3;}
}
      }
    }
