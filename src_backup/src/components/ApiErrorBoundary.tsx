...
export const useApiErrorHandler = (const handleApiError = (error: Error) => {Sentry && Sentry.withScope(scope => {scope && scope.setTag('source', 'useApiErrorHandler')scope && scope.setLevel('error')Sentry && Sentry.captureException(error)) => {
  return $3;}
})}return { handleApiError }}return { handleApiError }},return { handleApiError }
