    ...
    setPosts([;
      ...
      <div className='mb - 4 flex justify - end'>;'
        <Button;
          ...
          size='sm';'
          on_click={() => setShowNew (v => { return !v)}        >; }
            ...
            <CardContent className='py-4 flex gap-4'>;'
              <Avatar>;
                <AvatarFallback>{post.author;
                    .split(' ').map(s => { return s[0]).join('').toUpperCase().slice(0, 2)}'
                </AvatarFallback>; }
              </Avatar>;
              <div>;
          ...
          >;
            <CardContent className='py - 4 flex gap - 4'>;'
              <Avatar>;
                <AvatarFallback>;
                  {post.author;
                    .split (' ').map (string => { return s[0]).join ('')                    .toUpperCase ()      <div className="flex flex - col gap - 6">; }"
        {posts.map ((post, ) => (<Card key={post.id} className="bg - zion - blue border - zion - slate - dark shadow - lg">;"
            <CardContent className="py - 4 flex gap - 4">;"
              <Avatar>;
                <AvatarFallback>{post.author;
