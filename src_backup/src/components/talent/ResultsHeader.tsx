  ...
  'resultCount': number;return (<div className='flex items-center mb-6 text-zion-slate-light'>;'
      {isLoading ? (<div className='flex items-center'>;'
          ...
          {resultCount === 0;
            ...
            : `Found ${resultCount} talent${resultCount !== 1 ? 's' : ''}`}          {resultCount === 0 ? ('No talents found. Try adjusting your filters.';'          ) : (`Found ${resultCount} talent${resultCount !== 1 ? 's' : ''}`;`          )}
      )}</div>;
  ))}}
          <span>Loading talents...</span>;
        </div>;
...
export function ResultsHeader() {return (<div className='flex items-center mb-6 text-zion-slate-light'>;'
  ...
  'resultCount': number;
      ...
      {isLoading ? (<div className='flex items-center'>;'
          ...
          ) : (`Found ${resultCount} talent${resultCount !== 1 ? 's' : ''}`;`          )})}interface ResultsHeaderProps  {'is_loading': boolean;
  }
  'result_count': number;
export /**;
...
function ResultsHeader() {return (<div className='flex items - center mb - 6 text - zion - slate - light'>;'
      {is_loading ? (<div className='flex items - center'>;'
          <Loader2 className='h - 4 w - 4 animate - spin mr - 2' />          <span > Loading talents...</span>;'
            ...
            ? 'No talents found. Try adjusting your filters.';'
            : `Found ${result_count} talent${result_count !== 1 ? 's' : ''}`}        </span>)}`    </div>)}
