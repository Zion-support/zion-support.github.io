...
function InteractiveFeatures() {const [open_index, setOpenIndex] = React.useState < number | null>(null)const features = [;
  ...
  description:;}
        ...
        'Our algorithms analyze skills, availability and experience to deliver the best candidates for your project.',icon: <Search className='h - 8 w - 8 text - blue - 700' />,link: '/match'},{title: 'Talent Directory',}
  description: 'Browse a verified database of AI and tech specialists.',details:;}
        'Every profile is vetted for quality and authenticity so you can hire with confidence.',icon: <Users className='h - 8 w - 8 text - purple - 700' />,link: '/talent'},{title: 'Services Marketplace',
  description:;}
        'Discover professional tech and AI services for your business.',details:;}
        'From on - demand IT support to specialized AI development, our marketplace offers transparent pricing and reviews.',icon: <Zap className='h - 8 w - 8 text - cyan - 700' />,link: '/services'},{title: 'Equipment Catalog',}
  description: 'Find specialized hardware for development and research.',details:;}
...
import { cn } from '@/lib/utils';interface InteractiveFeaturesProps  {className?: string,style?: React && React.CSSProperties;}interface InteractiveFeaturesProps  {className?: string;}
  ...
  description:;}
        ...
        'From on-demand IT support to specialized AI development, our marketplace offers transparent pricing and reviews.',icon: <Zap className='h-8 w-8 text-cyan-700' />,link: '/services'},{title: 'Equipment Catalog',}
  description: 'Find specialized hardware for development and research.',details:;}
        'Access cutting-edge technology with flexible options to buy, rent or lease the gear you need.',icon: <Settings className='h-8 w-8 text-amber-700' />,link: '/equipment'}];const handleToggle = (setOpenIndex(prev => (prev === index ? null : index))) => {
  return $3;}
        ...
        <div className=\"text-center mb-8\" />;
          ...
          <p className=\"text-zion-slate-light text-lg max-w-2xl mx-auto\" />;
            Hover or click a card to learn more about what Zion offers;
              ...
              onClick={() => handleToggle(index)}className=\"cursor-pointer overflow-hidden transition-all duration-300 hover:shadow-lg\";
            >;
              ...
              <div;
                className={cn('transition-all duration-300',openIndex === index;
                ...
                <CardContent className='text-sm text-zion-slate-light p-0' />;
                  ...
                  <Button;
                    ...
                    <Link;
                      ...
                      Learn more <ArrowRight className='h-4 w-4' />;\"transition-all duration-300\",openIndex === index ? \"max-h-48 opacity-100 p-6 pt-0\" : \"max-h-0 opacity-0 p-0\";
                )}
              >;
                <CardContent className=\"text-sm text-zion-slate-light p-0\" />;
                  <p />{feature.details}</p>;
