...
export function PartnerReferralLinks() {const {referralCode,getReferralLink,copyReferralLink,shareOnSocialMedia} = useReferrals()const [isDialogOpen, setIsDialogOpen] = useState(false)const [selectedCampaign, setSelectedCampaign] = useState<string>('default')const [customParam, setCustomParam] = useState<string>('')const [generatedLinks, setGeneratedLinks] = useState<;'
  ...
  const baseLink  = getReferralLink()const handleCopyLink = ("link": string) => {navigator && navigator.clipboard.writeText(link)toast({"title": 'Link copied!',"description": 'The referral link has been copied to your clipboard',"variant": 'default'})}const handleGenerateLink = () => {if (baseLink) {const url  = new URL(baseLink)// Add custom campaign parameter if selected;'
      ...
      if (customParam) {url && url.searchParams.append('source', customParam)}const newLink = {"name": `${selectedCampaign}${customParam ? `-${customParam}` : ''}`,"link": url && url.toString()}setGeneratedLinks(prev => [...prev, newLink])setIsDialogOpen(false)setCustomParam('')}'  }const handleDownloadLinks = () => {const allLinks  = [{ "name": 'Default', "link": baseLink }, ...generatedLinks];const csvContent = [;'
      'Name,Link',...allLinks && allLinks.map(l => `${l && l.name},${l && l.link}`)].join('\n')const blob = new Blob([csvContent], { "type": 'text/csv;charset=utf-8;' })const url = URL && URL.createObjectURL(blob)const link = document && document.createElement('a')link && link.setAttribute('href', url)link && link.setAttribute('download', 'zion_referral_links && zion_referral_links.csv')link && link.style.visibility = 'hidden';'    document && document.body.appendChild(link)link && link.click()document && document.body.removeChild(link)}return (className='flex items-center gap-2'>;'
...
export function PartnerReferralLinks() {const { referralCode, getReferralLink, copyReferralLink, shareOnSocialMedia } = useReferrals(),const [isDialogOpen, setIsDialogOpen] = useState(false),const [selectedCampaign, setSelectedCampaign] = useState<string>("default"),const [customParam, setCustomParam] = useState<string>(""),const [generatedLinks, setGeneratedLinks] = useState<{"name": string, "link": string}[]>([]),// Get the base referral link;"
  const baseLink = getReferralLink(),const handleCopyLink = ("link": string) => {navigator.clipboard.writeText(link),toast({"title": "Link copied!","description": "The referral link has been copied to your clipboard","variant": "default";"
      ...
      if (selectedCampaign !== "default") {url.searchParams.append("campaign", selectedCampaign)}// Add custom parameter if provided;"
      if (customParam) {url.searchParams.append("source", customParam)}const newLink = {"name": `${selectedCampaign}${customParam ? `-${customParam}` : ""}`,"link": url.toString()},setGeneratedLinks(prev => [...prev, newLink]),setIsDialogOpen(false),setCustomParam("")}"  },const handleDownloadLinks = () => {const allLinks = [;
      { "name": "Default", "link": baseLink },...generatedLinks;"
    ],const csvContent = [;
      "Name,Link",...allLinks.map(l => `${l.name},${l.link}`)].join("\n"),const blob = new Blob([csvContent], { "type": 'text/csv,charset=utf-8,' }),const url = URL.createObjectURL(blob),const link = document.createElement("a"),link.setAttribute("href", url),link.setAttribute("download", "zion_referral_links.csv"),link.style.visibility = 'hidden',document.body.appendChild(link),link.click()document.body.removeChild(link)}return (<div className="space-y-6">;"      <Card className="bg-zion-blue-dark border-zion-blue-light">;"
