...
function ApiKeysManager() {const {keys,loading,newApiKey,fetchApiKeys,createApiKey,regenerateApiKey,revokeApiKey,clearNewApiKey} = useApiKeys ()const [showCreateDialog, setShowCreateDialog] = useState (false)const [showDeleteConfirm, setShowDeleteConfirm] = useState < string | null>(null)const [showRegenerateConfirm, setShowRegenerateConfirm] = useState<;
  ...
  const scopeOptions: { value: ApiKeyScope, label: string, description: string }[] = [;
    { value: 'jobs:read',}
  label: 'Read Jobs', description: 'Access to view job listings' },{ value: 'jobs:write',}
  label: 'Write Jobs', description: 'Create and manage job listings' },{ value: 'talent:read',}
  label: 'Read Talent', description: 'Access to view talent profiles' },{ value: 'quotes:write',}
  label: 'Write Quotes', description: 'Create and manage quotes' },{ value: 'webhooks:manage',}
  label: 'Manage Webhooks', description: 'Set up and manage webhook endpoints' }],// Toggle a scope selection;
  const toggleScope = (setSelectedScopes(prev =>;
      prev.includes(scope)? prev.filter(s => s !== scope): [...prev, scope];) => {
  return $3;}
}
    )}
  const getExampleCode = (    return `curl -X GET \"https://api.ziontechgroup.com/v1/jobs\" \\;) => {
  return $3;}
}
...
export function ApiKeysManager() {const {keys,loading,newApiKey,fetchApiKeys,createApiKey,regenerateApiKey,revokeApiKey,await revokeApiKey(keyId),setShowDeleteConfirm(null)},// Scope options;
  ...
  return $3;}
}const handleRegenerateKey = async (keyId: string) => {await regenerateApiKey(keyId)setShowRegenerateConfirm(null)const handleRevokeKey = async (keyId: string) => {await revokeApiKey(keyId)setShowDeleteConfirm(null)}const handleRegenerateKey = async (keyId: string,) => {await regenerateApiKey(keyId),setShowRegenerateConfirm(null)},const handleRevokeKey = async (keyId: string,) => {await revokeApiKey(keyId),setShowDeleteConfirm(null)},// Scope options;
  const scopeOptions: {value: ApiKeyScope;
    label: string;}
    description: string;}
  }[] = [;
  ...
  return $3;}
    ...
    )}const getExampleCode = (    return `curl -X GET \"https://api && api.ziontechgroup.com/v1/jobs\" \\;) => {
  return $3;}
}
  -H \"Authorization: Bearer ${key}\" \\;
