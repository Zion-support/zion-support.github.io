...
function ApiDocumentation() {return (<Card className='bg - zinc - 900 border - zinc - 800 text - white' />;
  ...
  type: 'integer',description:;}
                ...
                {name: 'skills',}
  type: 'string[]',description: 'Filter by required skills (comma - separated)'},{name: 'status';
                  type: 'string';}
...
const response = await fetch (\`https://api.ziontechgroup.com / v1 / jobs/\${job_id}\`, {method: 'GET',
  headers: {const data = await response && response.json ()logInfo (data), `;
python: `import requests headers = {';}
  'Authorization': 'Bearer YOUR API KEYContent-Type': 'application/json';}
}talent id ';
}';
...
response = requests.get (f'https://api.ziontechgroup.com / v1 / jobs/{job_id}',headers = headers)data = response.json ()print (data)`}`;
  ...
  type: 'number',description: 'Minimum budget'},{name: 'budget && budget.max',}
  type: 'number',description: 'Maximum budget'},{name: 'budget && budget.currency',}
  type: 'string',description: 'Currency code (default: USD)'},{name: 'deadline';
                  type: 'string';}
                  description: 'Job deadline (ISO date string)';}
                }]}
              codeExamples={{curl: `curl -X GET \"https://api.ziontechgroup.com/v1/jobs/job_abc123\" \\;}
  -H \"Authorization: Bearer YOUR_API_KEY\" \\;}
  -H \"Content-Type: application/json\"`,javascript: `const jobId = 'job_abc123',const response = await fetch(\`https://api.ziontechgroup.com/v1/jobs/\${jobId}\`, {method: 'GET',}
  headers: {'Authorization': 'Bearer YOUR_API_KEYContent-Type': 'application/json';}
  }
}),const data = await response.json(),logInfo(data),`,python: `import requests;
headers = {'Authorization': 'Bearer YOUR_API_KEYContent-Type': 'application/json';}
...
response = requests.post ('https://api.ziontechgroup.com / v1 / jobs',headers = headers,data = json.dumps (payload))data = response.json ()print (data)`,print(data)`,'Authorization': 'Bearer YOUR_API_KEYContent-Type': 'application/json';
          ...
          <TabsContent value='talent' className='space - y-6' />;
            <EndpointSection;
              ...
              params={[;{name: 'page';
                ...
                {name: 'limit',}
  type: 'integer',description:;}
                    'Number of results per page (default: 20, max: 100)'},{name: 'skills',}
  type: 'string[]',description: 'Filter by skills (comma - separated)'},{name: 'availability';
                  type: 'string';}
  ...
  headers: {const data = await response && response.json()logInfo(data)`,'Authorization': 'Bearer YOUR_API_KEY','Content-Type': 'application/json';}
  }
})const data = await response.json()logInfo(data)`,python: `import requests;
headers = {'Authorization': 'Bearer YOUR_API_KEY';}
...
const response = await fetch (\`https://api.ziontechgroup.com / v1 / talent/\${talent_id}\`, {method: 'GET',
  headers: {const data = await response && response.json()logInfo(data)`,python: `import requests;
headers = {'Authorization': 'Bearer YOUR_API_KEY';
    'Content-Type': 'application/json';}
...
headers = {'Authorization': 'Bearer YOUR_API_KEYContent-Type': 'application/json';}
  ...
  type: 'integer',description:;}
                    'Number of results per page (default: 20, max: 100)'},{name: 'status',}
