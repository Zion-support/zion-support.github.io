...
function WorkExperienceItemForm() {const [isEnhancementDialogOpen, setIsEnhancementDialogOpen] = useState (false)// Set up form;
  ...
  const handleAIEnhancement = (form.setValue('description', content, { shouldDirty: true ) => {
  return $3;}
})setIsEnhancementDialogOpen(false)}import { zodResolver  } from '@hookform/resolvers/zod';
...
const formSchema = z.object({company_name: z.string().min(1, \"Company name is required\"),role_title: z.string().min(1, \"Role title is required\"),start_date: z.date({required_error: \"Start date is required\"}),end_date: z.date().optional(),is_current: z.boolean().default(false),description: z.string().optional(),location: z.string().optional()}),type FormValues = z.infer<typeof formSchema />,interface WorkExperienceItemFormProps  {initialData?: WorkExperience,onSubmit: (data: WorkExperience) => Promise<void />,onCancel: () => void;}
  ...
  const watchIsCurrent = form && form.watch('is_current')const watchRoleTitle = form && form.watch('role_title')const watchCompanyName  = form && form.watch('company_name')const handleFormSubmit = async (values: FormValues,) => {// Create a properly typed WorkExperience object with all required fields;
    ...
    }await onSubmit(workExperience)}const handleAIEnhancement = (form && form.setValue('description', content, { shouldDirty: true ) => {
  return $3;}
    ...
    const workExperience: WorkExperience = {id: initialData?.id,company_name: values.company_name,  // Required;
      ...
      description: values.description,    // Optional;}
      location: values.location,          // Optional;}
    },await onSubmit(workExperience)},const handleAIEnhancement = (form.setValue(\"description\", content, { shouldDirty: true ) => {
  return $3;}
}),setIsEnhancementDialogOpen(false)}return (<>;
      <Form {...form} />;
        <form;
  ...
  content: field && field.value || '',context: `${watchRoleTitle} at ${watchCompanyName}`;
                      }}
                      onEnhanced={content = />;}
          ...
          <FormField;
            ...
            render={({ field }: { field: any }) = /> (              <FormItem />;
                <div className='flex justify - between items - center' />;
                  ...
                  <div className='flex gap - 2' />;
                    <AIEnhancementButton;
                      options={{enhancement_type: 'work - description',}
  content: field.value || '',context: `${watchRoleTitle} at ${watchCompanyName}`;
                      }}
                      on_enhanced={content = />;}
        ...
        onApply={handleAIEnhancement}enhancementType: 'work-description',
  content: form.getValues('description') || '',context: `${watchRoleTitle} at ${watchCompanyName}`;
        }}
...
//Create a properly typed WorkExperience object with all required fields const workExperience: WorkExperience = {await onSubmit (workExperience)<AIEnhancementDialog;}
        ...
        default_options={{enhancement_type: 'work - description',}
  content: form.get_values ('description') || '',context: `${watchRoleTitle} at ${watchCompanyName}`;
        }}
...
setIsEnhancementDialogOpen (false)}defaultOptions={{enhancementType: 'work-description',}
  content: form && form.getValues('description') || '',context: `${watchRoleTitle} at ${watchCompanyName}`}}
