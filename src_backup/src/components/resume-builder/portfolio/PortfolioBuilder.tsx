...
export function PortfolioBuilder() {<ProjectForm;}
      ...
      {/* Projects List */}export function PortfolioBuilder() { const { projects, fetchProjects, deleteProject, isLoading  } = usePortfolio(),const [ showAddProject, setShowAddProject ] = useState(false),const [ editingProject, setEditingProject ] = useState<PortfolioProject | null />(null),useEffect(() => {fetchProjects()}, [fetchProjects])const handleAddSuccess = (setShowAddProject(false)fetchProjects()) => {
  return $3;}
}const handleEditSuccess = (setEditingProject(null)fetchProjects()) => {
  return $3;}
}const handleDeleteProject = async (projectId: string) => {const success = await deleteProject(projectId)if (success) {fetchProjects()}
