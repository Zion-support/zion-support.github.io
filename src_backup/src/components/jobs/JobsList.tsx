...
if ( {) {$2;
    ...
    return (<div className="text - center p - 8 border rounded - md bg - muted / 20">;"
        <p className="text - lg text - muted - foreground">;"
          {filter;
            ? `No jobs with status "${filter}" found.`;: "You haven't posted any jobs yet." }"        </p>;
        ...
        >;
          ...
          <CardFooter className="flex justify - between p - 4 pt - 0 gap - 2">;"
            <Button variant="outline" size="sm" as_child>;"
              <Link href={`/jobs/${job.id}`}>;`                <Eye className="h - 4 w - 4 mr - 1" /> View Details;"
              </Link>;
            </Button>;
            <div className="flex gap - 2">;"
              <Button variant="outline" size="sm" as_child>;"
                <Link href={`/jobs/${job.id}/edit`}>;`                  <Edit className="h - 4 w - 4" />;"
                </Link>;
              </Button>;
