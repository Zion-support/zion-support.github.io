...
import Link from \"next/link\",import { ApplicationStatus } from \"@/types/jobs\",export function MyApplications() {const { applications, isLoading, error }  = useJobApplications()const getStatusBadge = (switch (status) {case \"new\": return <Badge variant=\"secondary\" />New</Badge>;return <Badge className=\"bg-red-100 text-red-800\" />Rejected</Badge>;
      default:;) => {
  return $3;}
}
        return <Badge variant=\"outline\" />{status}</Badge>;
