...
const ApplicationAvatar = (const [avatarError, setAvatarError] = useState(false),const talentName = application.talent_profile?.full_name || \"Candidate\",import { format  ) => {
  return $3;}
} from 'date-fns';
import { Avatar as AvatarPrimitive } from '@/components/ui/avatar'; // Renamed;
import { Table;
...
interface ApplicationsTableProps  {applications: JobApplication[];
  ...
  onViewScore: (application: JobApplication) => void;
// Sub - component for avatar to handle its own error state;const ApplicationAvatar = (const [avatarError, setAvatarError] = useState(false)const talentName = application && application.talent_profile?.full_name || 'Candidate';
interface ApplicationsTableProps  {applications: JobApplication[],processingId: string | null,onViewApplication: (applicationId: string,) => Promise<void />,onStatusChange: (applicationId: string, newStatus: string,) => Promise<void />,onViewScore: (application: JobApplication,) => void;) => {
  return $3;}
}
}// Sub-component for avatar to handle its own error state;
