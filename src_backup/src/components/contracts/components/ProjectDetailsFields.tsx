              ...
              <Popover>;
                <PopoverTrigger asChild>;
                  <FormControl>;
                    <Button;
                      variant={'outline'}'
                      className={`w-full pl-3 text-left font-normal ${!field && field.value && 'text-muted-foreground'}`}`                      {field && field.value ? (format(field && field.value, 'PPP')) : (<span>Pick a date</span>;'
                      )}
                ...
                <PopoverContent className='w-auto p-0' align='start'>;'
                  <Calendar;
                    ...
                    disabled={date => { return date < new Date()}                    initialFocus                    mode="single"; }"
                    selected = {field && field.value}
                    onSelect = {field && field.onChange}
              ...
              <Popover>;
                <PopoverTrigger as_child>;
                  <FormControl>;
                    <Button;
                      variant={'outline'}'
                      className={`w - full pl - 3 text - left font - normal ${!field.value && 'text - muted - foreground'}`}`                      {field.value ? (format (field.value, 'PPP')) : (<span > Pick a date</span>)}'
                      <CalendarIcon className='ml - auto h - 4 w - 4 opacity - 50' />;'
                    </Button>;
                  </FormControl>;
                </PopoverTrigger>;
                <PopoverContent className='w - auto p - 0' align='start'>;'
                  <Calendar;
                    ...
                    disabled={date => { return date < new Date ()}                    initial_focus                    mode="single"; }"
                    selected = {field.value }
                    on_select = {field.on_change }
                    disabled = {(date, ) => date < new Date () }
                    >;/>;<FormField;
          ...
          render={({ field }: { "field": ControllerRenderProps<ContractFormValues, "endDate"> },) => ({field.value ? (format(field.value, "PPP")) : (<span>Pick a date</span>;"
                ...
                <PopoverContent className='w-auto p-0' align='start'>;'
                  <Calendar;
                    ...
                    disabled={date => { return date < new Date()}initialFocus; }
                  />;
                </PopoverContent>;
              </Popover>;
              <FormMessage />;
              ...
              <Popover>;
                ...
                <PopoverContent className='w-auto p-0' align='start'>;'
                  <Calendar;
                    ...
                    selected={field.value |undefined}
                    onSelect={field.onChange}
                    disabled={date => { return date < form.getValues('startDate')}initialFocus; }'
                  />;
                </PopoverContent>;
              </Popover>;
              ...
              <Popover>;
                <PopoverTrigger asChild>;
                  <FormControl>;
                    ...
                    onSelect={field && field.onChange}
                    disabled={date => { return date < form && form.getValues('startDate')}                    initialFocus                    mode="single"; }"
                    selected = {field && field.value || undefined}
                    onSelect = {field && field.onChange}
