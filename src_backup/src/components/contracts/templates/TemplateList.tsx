...
if ( {) {$2;
      ...
      const current_path = router.as_path,router.push (`/auth / login?return_to=${encodeURIComponent (current_path)}`)return;}`    await setDefaultTemplate.mutate_async (template_id)}// useRouter replaces the old useLocation hook from react-router;
import {AlertDialog;
  }
  AlertDialogAction;
  AlertDialogCancel;
...
router.push (`/auth/login?returnTo=$ {encodeURIComponent (currentPath)}`)return;`}await setDefaultTemplate.mutateAsync (templateId)}
...
router.push (`/auth / login?return_to=$ {encodeURIComponent (current_path)}`)return;`}await setDefaultTemplate.mutate_async (template_id)}
<CardContent className="p - 4"> <div className="flex items - center justify - between"> <div className="space - y-1"> <div className="flex items - center gap - 2"> <h3 className="font - medium"> {template.title;"
}</h3> {";"
  }
  template.is default && (<span className="bg - zion - purple / 10 text - zion - purple text - xs px - 2 py - 0.5 rounded - full">Default</span>) ";"
