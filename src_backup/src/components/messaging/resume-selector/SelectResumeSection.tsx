...
export function SelectResumeSection() {return (<div className="space-y-2">;"
      {resumeOptions.length === 0 ? (<p className="text-sm text-zion-slate">No saved resumes found.</p>;"
            ...
            >;
              ...
              key={option.id}
              className={`w-full text-left p-3 rounded-md transition ${selectedResume?.id === option.id;`                  ? 'bg-zion-purple/20 border border-zion-purple';'
            ...
            >;
              ...
              isLoading = {isLoading} />;{selectedResume?.type === 'ai_resume' && selectedResume.resume && (<ResumePreviewCard;'
              }
              resume={selectedResume.resume as Resume}
              onDownload={handleDownloadResume}
              isLoading={isLoading} />;
  ...
  "onSelect": ("resume": Resume) => void;
      ...
      className={`p-4 border rounded-lg cursor-pointer transition-colors ${isSelected;`          ? 'border-blue-500 bg-blue-50';'
