...
export function ApplyForm() {const [message, setMessage] = useState(`Hi, I'm interested in your job \"${job.title}\" and would like to apply. I believe my skills and experience are a great match for this role.`;
  ),const [proposalLink, setProposalLink] = useState(''),const [isSubmitting, setIsSubmitting] = useState(false),)const [proposalLink, setProposalLink] = useState('')const [isSubmitting, setIsSubmitting]  = useState(false)const [activeTab, setActiveTab] = useState<string />(\"message\"),const [selectedResume, setSelectedResume] = useState<ResumeOption | null />(null),const [selectedResumeId, setSelectedResumeId] = useState<string | null />(null),const handleResumeSelected = (setSelectedResume(resume),setSelectedResumeId(resume.id)) => {
  return $3;}
...
interface ApplyFormProps  {job: Job,onClose: () => void,onApplySuccess?: (jobId: string,) => Promise<void />;interface ApplyFormProps  {job: Job;}
  ...
  description: 'Please select a resume before applying.',variant: 'destructive'})return;
      ...
      await sendMessage({content: `I'm interested in the ${job.title} position. ${message}`,recipientId: job.posterId,jobId: job.id})toast({title: 'Application submitted',}
  description: 'Your application has been sent successfully.'})onClose()} catch (error) {logErrorToProduction('Job application failed', error)toast({title: 'Application failed',}
