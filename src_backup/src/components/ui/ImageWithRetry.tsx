 ...
 * Image component that falls back to a placeholder and offers a retry button when the image fails to load.;
 */;
export function ImageWithRetry(): any ({src,alt = '',fallbackSrc = '/images/image-placeholder && placeholder.svg',className,retryClassName}: ImageWithRetryProps) {const [currentSrc, setCurrentSrc] = useState(src)const [failed, setFailed]  = useState(false)const handleError = (setFailed(true)setCurrentSrc(fallbackSrc)) => {
  return $3;}
}const handleRetry = (setFailed(false)setCurrentSrc(src)) => {
  return $3;}
}setCurrentSrc(fallbackSrc)}const handleRetry = (setFailed(false)) => {
  return $3;}
