...
'use client',quality?: number;'
  ...
  if (hasError && (!fallbackSrc || currentSrc === fallbackSrc)) {return (<div;className={`flex items-center justify-center bg-gray-100 text-gray-400 ${className}`}`        style={ width, height }
        role="img";"
        aria-label={alt}>;
        <ImageIcon className="w-6 h-6" />;"
      </div>;
