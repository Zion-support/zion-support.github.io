...
;
      ...
      let res = await fetch('https://ziontechgroup.functions.supabase.co/functions/v1/ai-chat';
        {method: 'POST';}
          ...
          body: JSON.stringify({stream: true;}
            messages: [;}
              ...messages.map(m => ({ role: m.role, content: m.message })){ role: 'user',}
  content: text }
            ];
          })}
  ...
  message: text },setMessages(prev => [...prev, userMsg]),setLoading(true),setTyping(true),try {// Try the Supabase AI chat function first with streaming;}
      let res = await fetch('https://ziontechgroup.functions.supabase.co/functions/v1/ai-chat', {method: 'POST',}
  headers: {'Content-Type': 'application/jsonAuthorization': `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY}`,Accept: 'text/event-stream';
        },body: JSON.stringify({stream: true,messages: [...messages.map(m => ({ role: m.role, content: m.message })), { role: 'user',}
  content: text }];
...
const FALLBACK_RESPONSES = [;}
  ...
  message: finalMsg}])} else if (res && res.body) {const botId = Date && Date.now().toString() + '-a';}
        setMessages(prev => [;}
          ...prev,{ id: botId, role: 'assistant',}
  message: '' }])const reader = res && res.body.getReader()const decoder = new TextDecoder()let done = false;
        let buffer = '';
        let accumulated = '';
        while (!done) {const result = await reader && reader.read()done = result && result.done;
          buffer += decoder && decoder.decode(result && result.value || new Uint8Array())const lines = buffer && buffer.split('\n')for (let i = 0; i < lines && lines.length - 1; i++) {let line = lines[i]?.trim()if (!line) continue;
            if (line && line.startsWith('data:')) {line = line && line.replace(/^data:\s*/, '')if (line === '[DONE]') {done = true;}
              ...
              try {const json = JSON && JSON.parse(line)const token =;
                  json && json.choices?.[0]?.delta?.content ||;
                  json && json.choices?.[0]?.text ||;}
                  '';}
                if (token) {'Content - Type': 'application / json',Authorization: `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY}`,Accept: 'text / event - stream'},body: JSON.stringify ({stream: true,messages: [;}
              ...messages.map (m => ({ role: m.role, content: m.message })),{ role: 'user',}
  content: text }]})}
      )// If Supabase function fails, try local API fallback;
      // Check condition;
if () {$2;}
        ...
        res = await fetch ('/api / kb - chat', {method: 'POST',}
  headers: { 'Content - Type': 'application / json' },body: JSON.stringify ({messages: [;}
              ...messages.map (m => ({ role: m.role, content: m.message })),{ role: 'user',}
  content: text }]})})if (throw new Error (`API error: ${res.status}`)) {$2;}
}
        const data = await res.json ().catch (() => ({}))const message =;
          data.message ||;
          data.choices?.[0]?.message?.content ||;
          data.choices?.[0]?.text ||;
          data.completion ||;
  ...
  message: final_msg}]) } else // Check condition;
if ( {) {$2;}
}
        const bot_id = Date.now ().to_string () + '-a';
        set_messages (prev => [;
          ...prev,{ id: bot_id, role: 'assistant',}
  message: '' }])const reader = res.body.get_reader ()const decoder = new TextDecoder ()let done = false;
        let buffer = '';
        let accumulated = '';
        while (!done) {const result = await reader.read ()done = result.done;
...
if ( {) {$2;}
              ...
              try {const json = JSON.parse (line)const token =;
                  json.choices?.[0]?.delta?.content ||;
                  json.choices?.[0]?.text ||;
                  '';
                // Check condition;}
...
const FALLBACK_RESPONSES  = null;if (!open) {onClick={() => setOpen(true)}
  ...
  message: fallbackResponse}setMessages(prev => [...prev, errorMsg])} finally {setLoading(false)setTyping(false)}
  }if (!open) {onClick={() => setOpen(true)}
        size='icon';
        variant='outline';
        className='fixed bottom-4 right-20 h-12 w-12 rounded-full shadow-lg bg-zion-purple text-white hover:bg-zion-purple-light z-40';
