...
import { Button } from \"./ui/button\",import { toast } from \"./ui/use-toast\",import { useTranslation } from 'react-i18next',export function SocialShareSection() {const { t } = useTranslation(),// Current URL is not available during SSR, guard with typeof check;
  ...
  const shareToTwitter = () = /> {window.open(`https://twitter.com/intent/tweet?url=${shareUrl}&text=${shareText}`, \"_blank\")},const shareToFacebook = (window.open(`https://www.facebook.com/sharer/sharer.php?u=${shareUrl) => {
  return $3;}
}`, \"_blank\")},const shareToLinkedIn = (window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${shareUrl) => {
  return $3;}
}`, \"_blank\")},const copyLinkToClipboard = (navigator.clipboard.writeText(window.location.href).then(() => {toast({title: \"Link Copied!\",description: \"The link has been copied to your clipboard\") => {
  return $3;}
  ...
  return (<section className=\"py-12 bg-zion-blue\" />;
      <div className=\"container mx-auto px-4 sm:px-6 lg:px-8\" />;
        <div className=\"max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 bg-zion-blue-light border border-zion-purple/20 rounded-lg p-6\" />;
          <div />;<h3 className='text-xl font-bold text-white mb-2' />;<h3 className=\"text-xl font-bold text-white mb-2\" />Share Zion with Your Network</h3>;
            <p className=\"text-zion-slate-light\" />Help others discover the future of AI & tech marketplace</p>;
          </div>;
          <div className=\"flex flex-wrap gap-3\" />;
            {shareLinks.map((link, index) => (<Button;}
  ...
  icon: <Twitter className='h - 5 w - 5' aria - hidden='true' />,color: 'bg-[#1DA1F2] hover:bg-[#1DA1F2]/80',
  on_click: shareToTwitter},{name: 'Facebook',
  icon: <Facebook className='h - 5 w - 5' aria - hidden='true' />,color: 'bg-[#4267B2] hover:bg-[#4267B2]/80',}
  on_click: shareToFacebook},{name: 'LinkedIn',
  icon: <Linkedin className='h - 5 w - 5' aria - hidden='true' />,color: 'bg-[#0077B5] hover:bg-[#0077B5]/80',}
  on_click: shareToLinkedIn},{name: 'Copy Link',
  icon: <Link className='h - 5 w - 5' aria - hidden='true' />,color: 'bg - zion - blue - dark hover:bg - zion - blue - dark / 80',}
