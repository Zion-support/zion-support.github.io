...
>,
  ...
  (steps.findIndex (s => s.id === currentStep) / (steps.length - 1) ) * 10o0}%`}/> </div>}className=" flex flex-col items-center relative"> <div) : (<span className=" text-xs" > {
  steps.findIndex (s => s.id === step.id) + 1}</span>)}</div> <span)}> {
...
if (return "current") {
  ...
  (steps.find_index (string => s.id === current_step) / (steps.length - 1) ) * 10o0}%`}/> </div>}className=" flex flex - col items - center relative"> <div) : (<span className=" text - xs" > {
  steps.find_index (string => s.id === step.id) + 1}</span>)}</div> <span)}> {
  step.label}</span> </div>)})}</div> </div>)}"          )})}
      </div>,
    </div>)}
]