...
function main() {
  ...
  // Get all AI directories
  const aiDirs = fs.readdirSync('./app').filter(dir => 
    dir.startsWith('ai-') && fs.statSync(`./app/${dir}`).isDirectory()
  );
  
  console.log(`Found ${aiDirs.length} AI directories`);
  
  let fixedCount = 0;
  
  for (const dir of aiDirs) {
    const pagePath = `./app/${dir}/page.tsx`;
    if (fs.existsSync(pagePath)) {
      try {
        const content = createCleanPage(pagePath);
  ...
  console.log(`❌ Failed: ${aiDirs.length - fixedCount} files`);
  
  console.log('\n🎉 All remaining files fix completed!');
}
