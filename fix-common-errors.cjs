...
function fixCommonErrors(content) {
  ...
  fixed = fixed.replace(
    /import\s+(\w+)\s+from\s+[""]([^""]+)[""]\s*[,]+/g;"import $1 from "$2";"
  )
  // Fix 2: Fix malformed import statements with .ts extensions
  fixed = fixed.replace(
    /import\s+(\w+)\s+from\s+[""]([^""]+)\.ts[""]\s*[,]*/g;"import $1 from "$2";"
  )
  // Fix 3: Fix malformed export statements
  fixed = fixed.replace(
    /export\s+default\s+(\w+)\s*[,]+/g;"export default $1;"
  )
  ...
  fixed = fixed.replace(
    /export\s+default\s+function\s+(\w+)\([^)]*\)\s*:\s*\{[^}]*\}\s*[,]+/g;"export default function $1() {"
  )
  return fixed;}
