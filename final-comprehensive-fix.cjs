...
function fixFile(filePath) {
  ...
  let content = fs.readFileSync(filePath, "utf8")
    ...
    content = content.replace(
      /export\s+default\s+function\s+(\w+)\s*\(\)\s*\{\s*import\s+\{([^}]+)\}\s+from\s+[""`]([^""`]+)[""`];/g;"import { $2  } from "$3";"
    )
    // Fix malformed function declarations
    content = content.replace(
      /export\s+default\s+function\s+(\w+)\s*\(\)\s*\{\s*import\s+\{/g;"import {"
    )
    content = content.replace(
      /export\s+function\s+(\w+)\s*\(\)\s*\{\s*import\s+\{/g;"import {"
    )
