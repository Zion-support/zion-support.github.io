...
# Efficient script to merge remaining unmerged branches in batches
set -e

echo "🚀 Starting batch merge process for remaining unmerged branches..."
...
echo "📋 Fetching all unmerged branches..."
git fetch --all

# Get all unmerged branches (excluding backup branches)
UNMERGED_BRANCHES=$(git branch -r --no-merged main | sed 's/origin\///' | grep -v 'backup' | grep -v 'Backup' | head -10)
TOTAL_BRANCHES=$(echo "$UNMERGED_BRANCHES" | wc -l)

echo "📊 Processing first batch of $TOTAL_BRANCHES unmerged branches..."
...
can_merge_branch() {
    ...
    if [[ "$branch" == *"backup"* ]] || [[ "$branch" == *"Backup"* ]]; then
        return 1
    fi
...
for branch in $UNMERGED_BRANCHES; do
    ...
    if ! can_merge_branch "$branch"; then
        echo "⏭️  Skipping $branch (already merged, doesn't exist, or is a backup branch)"
