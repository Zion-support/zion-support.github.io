...
build: {
  target: 'esnext', minify: 'terser', sourcemap: false, rollupOptions: {
  output: {
  manualChunks: {
  'react-vendor': ['react', 'react-dom'], 'ui-vendor': [ '@radix-ui/react-accordion', '@radix-ui/react-alert-dialog', '@radix-ui/react-avatar', '@radix-ui/react-checkbox', '@radix-ui/react-collapsible', '@radix-ui/react-dialog', '@radix-ui/react-dropdown-menu', '@radix-ui/react-hover-card', '@radix-ui/react-label', '@radix-ui/react-menubar', '@radix-ui/react-navigation-menu', '@radix-ui/react-popover', '@radix-ui/react-progress', '@radix-ui/react-radio-group', '@radix-ui/react-scroll-area', '@radix-ui/react-select', '@radix-ui/react-separator', '@radix-ui/react-slider', '@radix-ui/react-slot', '@radix-ui/react-switch', '@radix-ui/react-tabs', '@radix-ui/react-toast', '@radix-ui/react-toggle', '@radix-ui/react-tooltip', ], 'animation-vendor': ['framer-motion'], 'utils-vendor': [ 'clsx', 'tailwind-merge', 'class-variance-authority', ], 'icons-vendor': ['lucide-react'], 'state-vendor': ['@reduxjs/toolkit', 'react-redux'], 'router-vendor': ['react-router-dom'];
};
...
define: {
  DEV : JSON.stringify (process.env.NODE ENV === 'development');
PROD : JSON.stringify (process.env.NODE ENV === 'production');
'process.env.NODE ENV': JSON.stringify (process.env.NODE ENV);
};
preview: {
  port: 4173, host: true, open: true;
};
define: {
  DEV : JSON.stringify (process.env.NODE ENV === 'development'), PROD : JSON.stringify (process.env.NODE ENV === 'production'), 'process.env.NODE ENV': JSON.stringify (process.env.NODE ENV);
};
envPrefix: ['VITE ', 'ZION '];
experimental: {
