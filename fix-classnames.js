...
function fixClassNames(content) {// Fix className= without quotes
  content = content.replace(/className=([^"'`\s][^"'`\s]*)/g(matchclassName) => {
    // If it doesn't start with quoteadd quotes
...
function processFile(filePath) {try {
    ...
    if (content !== fixedContent) {
      fs.writeFileSync(filePathfixedContent'utf8');
