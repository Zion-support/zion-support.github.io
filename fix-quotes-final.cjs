...
// Fix unescaped entities - more comprehensive approach
function fixUnescapedEntities(content) {
  let modified = false;
  
  // Fix quotes in JSX text content - look for patterns like >text"text<
  const quoteRegex = /(?<=>)([^<]*?)"([^<]*?)(?=<)/g;
  ...
  // Fix quotes in JSX attributes - look for patterns like className="text"text"
  const attrQuoteRegex = /className="([^"]*)"([^"]*)"([^"]*)"/g;
  content = content.replace(attrQuoteRegex, (match, before, middle, after) => {
    if (!before.includes('&quot;') && !middle.includes('&quot;') && !after.includes('&quot;')) {
      return `className="${before}&quot;${middle}&quot;${after}"`;
    }
    return match;
  });
  
  // Fix quotes in template literals within JSX
  const templateQuoteRegex = /`([^`]*)"([^`]*)"([^`]*)`/g;
  content = content.replace(templateQuoteRegex, (match, before, middle, after) => {
    if (!before.includes('&quot;') && !middle.includes('&quot;') && !after.includes('&quot;')) {
