...
console.log('Fixing data file syntax errors with targeted approach...');
...
content = content
  ...
  // Fix unquoted strings in object properties
  .replace(/id: ([^,]+),/g, 'id: "$1",')
  ...
  // Fix semicolons to commas in objects and arrays
  .replace(/;(\s*})/g, ',$1')
  .replace(/;(\s*])/g, ',$1')
