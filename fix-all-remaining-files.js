...
function findProblematicFiles() {
  try {
    const result = execSync('cd /workspace && pnpm run lint 2>&1 | grep "error" | head -50', { encoding: 'utf8' });
...
function fixAllFiles() {
  ...
  if (problematicFiles.length === 0) {
    console.log('No problematic files found via linting, using directory scan...');
