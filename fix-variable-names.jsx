// const fs = require(, fs'); const path = require(' 'path');function toCamelCase(str) {/* TODO: Fix JSX expression */}
  ...
  t: newContent = content.replace(' new RegExp(`const\\s+${filename.replace(/Service Feature/g,\\$&')}\\\\s*:\\\\s*NextPage\\\\s*=\\\\s*\\\\(\\\\)\\\\s*=>\\\\s*{`,g}' ')'} `cons,
  t: ${camelCaseName}: NextPag,`
  e: = () => {`)} //Replac,
  e: the export statement} newConten,`
  t: = newContent.replace(' new RegExp(`export\\s+default\\s+${filename.replace(/Service Feature/g,\\$&')}`;g;' ')'; `expor,`
