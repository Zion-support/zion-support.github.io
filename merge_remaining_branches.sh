...
# Get list of remaining unmerged cursor branches
branches=$(git branch -r --no-merged origin/main | grep "cursor")
...
for branch in $branches; do
    ...
    if git merge "$branch" --no-ff -m "Merge $branch" 2>/dev/null; then
        ...
        # Handle add/add conflicts by adding them
        git add $(git status --porcelain | grep "^AA" | cut -c4- | tr '\n' ' ') 2>/dev/null || true
...
echo "🎉 Completed merging remaining cursor branches!"