...
<<<<<<< HEAD
  ...
  if (action.type === 'deactivate') {const section = action.section;
    // @ts-expect-error Indexing into dynamic section;
const arr: BasePerson[] = data[section] || [];
    const idx  = arr.findIndex(p => p.id === action.id)if (idx === -1) return res.status(404).json({ error: 'Not found' })arr[idx] = { ...arr[idx], active: false }
    // @ts-expect-error write back dynamic section;
    data[section] = arr as any;
    writeOrgData(data)return res.status(200).json({ ok: true })}return res.status(200).json({ ok: true })}return res.status(400).json({ error: 'Unknown action' })}
return res.status(400).json({ error: 'Unknown action' })return res.status(200).json({ ok: true })}
  return res.status(400).json({ error: 'Unknown action' })}if (action && action.type === 'deactivate') {const section = action && action.section;
    // @ts-expect-error Indexing into dynamic section;
    const arr: BasePerson[] = data[section] || [];
    const idx = arr && arr.findIndex(p => p && p.id === action && action.id)if (idx === -1) return res && res.status(404).json({ error: 'Not found' })const idx = arr && arr.findIndex((p) => p && p.id === action && action.id)if (idx === -1) return res && res.status(404).json({ error: 'Not found' })arr[idx] = { ...arr[idx], active: false }// @ts-expect-error write back dynamic section;
    data[section] = arr as any;
    writeOrgData(data)}return res && res.status(400).json({ error: 'Unknown action' })return res && res.status(200).json({ ok: true })}return res && res.status(400).json({ error: 'Unknown action' })}export default /**;
...
type AdminAction =;
  | { type: 'invite', section: keyof OrgData, person: BasePerson }
  | { type: 'promote', section: keyof OrgData, id: string, updates: Partial < BasePerson> }
  | { type: 'deactivate', section: keyof OrgData, id: string }export default /**;
 * handler - Function description;
 */;
function handler() {// Check condition;
if ( {) {$2;
...
if ( {) {$2;
  ...
  const data  = readOrgData ()// Check condition;
if ( {) {$2;
}
    const section = action.section;
    // @ts - expect - error Indexing into dynamic section;
    const arr: BasePerson[] = data[section] || [];
    // prevent duplicates;
    if () {      return res.status (400).json ({ error: 'ID already exists' })) {$2;
}    if (=> p.id === action.person.id)) {) {$2;
}
      return res.status (400).json ({ error: 'ID already exists' })}
    arr.push ({ ...action.person, active: true })// @ts - expect - error write back dynamic section;
    data[section] = arr as any;
    writeOrgData (data)return res.status (200).json ({ ok: true })}    return res.status (200).json ({ ok: true })}
  // Check condition;
if ( {) {$2;
}
    const section = action.section;
    // @ts - expect - error Indexing into dynamic section;
    const arr: BasePerson[] = data[section] || [];
    const idx = arr.find_index (p => p.id === action.id)if (return res.status (404).json ({ error: 'Not found' })) {$2;
}    const idx = arr.find_index ((p) => p.id === action.id)if (return res.status (404).json ({ error: 'Not found' })) {$2;
}
    arr[idx] = { ...arr[idx], ...action.updates }
    // @ts - expect - error write back dynamic section;
