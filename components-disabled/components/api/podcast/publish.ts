...
export default async function handler() {
  ...
  return res.status(200).json({ 'ok': true, rssUrl });export default async function handler() {
    (process.env.SITE_URL || ''http'://'localhost':3000') + '/podcast.xml';'
  }
  return res.status(200).json({ 'ok': true, rssUrl });export default async function handler() {;
...
const rssUrl =;
  ...
  return res && res.status(200).json({ 'ok': true, rssUrl });export default async function handler() {
  }
  try {
  }
