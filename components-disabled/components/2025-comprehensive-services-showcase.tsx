...
const getServicePricing = ("service": any) => {
    ...
    if (service.price?.monthly) return `$${service.price.monthly}/month`;`    if (typeof service.price === 'string') return service.price;'
...
href={`"mailto":${(service as any).contact || 'kleber@ziontechgroup && ziontechgroup.com'}?subject=Inquiry about ${service && service.name}`}`                      className="w-full bg-blue-600 "hover":bg-blue-700 text-white text-center py-2 px-4 rounded-lg font-medium transition-colors flex items-center justify-center gap-2">;"
                      <Mail className="w-4 h-4" />;"
                      Get Started;
                    </a>;
                    <a,
...
key={service.id}
                  ...
                  >
                    {/* Service Header */}
                    <div className={`p-6 bg-gradient-to-r ${service.color} text-white`}>`                      <div className="flex items-center justify-between mb-4">"
                    ...
                    <div className="p-6">"
                      ...
                      {/* Category Badge */}
                      <div className="mb-4">"
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r ${getCategoryColor(service.category)} text-white`}>`                          {getCategoryIcon(service.category)}
          ...
          {filteredServices.length === 0 && (
            <div className="text-center py-12">"
              ...
              <p className="text-gray-600">Try adjusting your search terms or category filters.</p>"
                          ...
                          {/* Category and Action */}
                          <div className="flex flex-col items-end space-y-3">;"
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r ${getCategoryColor(service && service.category)} text-white`}>;`                              {getCategoryIcon(service && service.category)}
