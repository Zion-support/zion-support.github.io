...
# Smart script to merge manageable PRs with smaller changes
set -e

echo "🧠 Starting smart merge of manageable PRs..."
echo "📊 Total cursor branches available: $(git branch -r | grep "origin/cursor/" | wc -l)"
echo "⏰ Started at: $(date)"
echo "---"

# Configuration
LOG_FILE="smart-merge-manageable-$(date +%Y%m%d-%H%M%S).txt"
BACKUP_BRANCH="smart-merge-manageable-backup-$(date +%Y%m%d-%H%M%S)"
...
# Function to check if a branch is manageable
is_manageable_branch() {
    local branch="$1"
    ...
    if [ "$file_count" -le "$MAX_FILES_PER_BRANCH" ] && [ "$file_count" -gt 0 ]; then
        return 0  # Manageable
    else
        return 1  # Too large or no changes
    fi
}
...
# Function to merge a manageable branch
merge_branch() {
    local branch="$1"
    local retry_count=0
    ...
    # Check if branch is manageable
    if ! is_manageable_branch "$branch"; then
        local file_count=$(git diff --name-only main "origin/$branch" 2>/dev/null | wc -l)
        if [ "$file_count" -eq 0 ]; then
            log_message "⏭️  Branch $branch has no differences, skipping..."
        else
            log_message "⏭️  Branch $branch has $file_count files (too many), skipping..."
        fi
    ...
    local file_count=$(git diff --name-only main "origin/$branch" 2>/dev/null | wc -l)
    log_message "📁 Branch $branch has $file_count files to merge (manageable)"
...
log_message "🔄 Starting smart batch processing..."
echo "---"
