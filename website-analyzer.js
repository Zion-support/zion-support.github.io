...
class WebsiteAnalyzer {
  ...
  async analyzeWebsite() {
    try {
      ...
      console.log(`📊 Found ${urlsToAnalyze.length} internal links to analyze`);
  ...
  analyzePageContent(url, $) {
    ...
    const mainContent = $('main, .content, #content, .main-content, article, .page-content')
      .text()
      .trim();
    ...
    this.pageTitles.set(url, title);
    this.pageDescriptions.set(url, description);
  ...
  analyzeNavigation(url, $) {
    $('nav, .navigation, .menu, .navbar').each((index, element) => {
      const navLinks = [];
      $(element)
        .find('a[href]')
        .each((i, link) => {
  ...
  generateReport() {
    const report = {
      ...
      pageTitles: Object.fromEntries(this.pageTitles),
      pageDescriptions: Object.fromEntries(this.pageDescriptions),
    ...
    console.log(`📊 Summary: ${report.summary.totalPagesAnalyzed} pages, ${report.summary.brokenLinks} broken links, ${report.summary.contentIssues} content issues`);
  }
