...
function fixDuplicateDeclarations(filePath) {
  try {
    ...
    content = content.replace(
      /const\s+(\w+)\s*:\s*React\.FC\s*=\s*\(\)\s*=>\s*{[\s\S]*?};\s*const\s+\1\s*:\s*React\.FC\s*=\s*\(\)\s*=>\s*{/g,
      (match, name) => {
    ...
    content = content.replace(
      /const\s+(\w+)\s*=\s*\(\)\s*=>\s*{[\s\S]*?};\s*const\s+\1\s*=\s*\(\)\s*=>\s*{/g,
      (match, name) => {
        return `const ${name} = () => {`;
      },
    );
...
try {
  ...
  console.log(
    "⚠️  Linting found some issues, but duplicates should be resolved",
  );
}
