...
async function mergePR(pr) {
  ...
  try {
    ...
    execSync(
      `git merge ${pr.head.sha} --no-ff -m "Merge PR #${pr.number}: Enhance and expand ziontechgroup.com services and site"`,
      {
        stdio: 'inherit',
      },
    );
    ...
    try {
      ...
      if (status.includes('UU') || status.includes('AA')) {
        ...
        execSync(
          `git commit -m "Resolve merge conflicts for PR #${pr.number}"`,
          { stdio: 'inherit' },
        );
