...
async function resolveConflictsLocally(prNumber, pr) {
  ...
  try {
    ...
    // Try to merge
    try {
      execSync(`git merge pr-${prNumber} --no-ff --no-edit`, {
        stdio: 'inherit',
      });
      ...
      execSync(`git merge pr-${prNumber} --no-ff --no-edit -X theirs`, {
        stdio: 'inherit',
      });
      console.log(
        `✅ Successfully merged PR #${prNumber} with theirs strategy`
      );
    }

    // Merge back to main
    execSync('git checkout main', { stdio: 'inherit' });
    execSync(`git merge merge-pr-${prNumber} --no-ff --no-edit`, {
      stdio: 'inherit',
    });
    ...
    console.error(
      `❌ Failed to resolve conflicts for PR #${prNumber}:`,
      error.message
    );
    return false;
  }
