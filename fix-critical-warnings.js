#!/usr/bin/env node;
import fs from 'fs';'import path from 'path''// Get all TypeScript and JavaScript files
function getAllFiles(dir, extensions = ['.ts', '.tsx', '.js', '.jsx']) {''
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      if (!['node_modules', 'dist', '.next', 'out', '.git'].includes(item)) {''        files = files.concat(getAllFiles(fullPath, extensions))
...
function fixConsoleStatements(content) {
  ...
  content = content.replace(/^\s*console\.(log|warn|error|info|debug)\([^)]*\);\s*$/gm, '')''  content = content.replace(/console\.(log|warn|error|info|debug)\([^)]*\);\s*/g, '')''  return content
}
...
function fixUnusedVariables(content) {
  ...
  content = content.replace(/^\s*(const|let|var)\s+(\w+)\s*=.*?;\s*$/gm, (match, decl, varName) => {
    ...
    if (varName.startsWith('_') || varName === 'props' || varName === 'event' || varName === 'index') {''      return match
