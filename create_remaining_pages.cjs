...
const aiServiceTemplate = (pageName, title, description, keywords) => `import React from 'react';';
...
const ${pageName.charAt(0).toUpperCase() + pageName.slice(1).replace(/-/g, '')}Page = () => {';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">;
      <EnhancedSEO;
        title="${title} - Zion Tech Group | AI-Powered Solutions";
...
missingPages.forEach(pageName => {
  const pageDir = `/workspace/app/${pageName}`;
  ...
  const description = `Transform your business with our ${title.toLowerCase()} solution. AI-powered features and enterprise-grade security for maximum efficiency and growth.`;
  
  const keywords = `${pageName.replace(/-/g, ', ')}, AI solution, business automation, enterprise software`;';
  // Write page file;
  fs.writeFileSync(pageFile, aiServiceTemplate(pageName, title, description, keywords));
  
  global.console.log(`Created page: ${pageName}`);
});

global.console.log(`Created ${missingPages.length} missing pages`);