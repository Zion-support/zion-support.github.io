...
test('shows error banner on failed login', async ({ page }, testInfo) => {'  const serverURL = await skipIfNoServer(testInfo);
  if (!serverURL) return;
  const errors: Error[] = [];
  ...
  await expect(page.getByRole('alert')).toBeVisible();  expect(errors).toHaveLength(0);
});
