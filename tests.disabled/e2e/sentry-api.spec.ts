...
async function pollSentryForEvent(request: unknown) {
  ...
  while (Date.now() < deadline) {
    ...
    if (res.ok()) {
      ...
      if (
        ...
        events.some((e: unknown) => e.message?.includes('Sentry Test API Error'))      ) {
        return;
      }
    }
...
test('Sentry test error endpoint sends error to Sentry', async ({ request }, testInfo) => {'  if (!SENTRY_API_TOKEN || !SENTRY_PROJECT_SLUG || !SENTRY_ORG_SLUG) {
