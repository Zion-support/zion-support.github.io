...
import { Phone, Mail, MapPin, Check, Users } from 'lucide-react';
import UltraAdvancedFuturisticBackground from '../components/ui/UltraAdvancedFuturisticBackground';
import Button from '../components/ui/Button';
import { additionalServices } from '../data/additional-services';

export default function AIRecruitingCopilotPage() {
  const service = additionalServices.find(s => s.id === 'ai-recruiting-copilot');
  ...
  return (
    <UltraAdvancedFuturisticBackground>
      <Head>
        ...
        <link rel="canonical" href="https://ziontechgroup.com/ai-recruiting-copilot" />
      </Head>
      <div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-5xl md:text-6xl font-bold bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent mb-6 flex items-center gap-3"><Users className="w-10 h-10" />{service.name}</h1>
            ...
            <div className="lg:col-span-2 bg-black/30 rounded-2xl border border-pink-500/30 p-6">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {service.features.slice(0, 10).map((feat, i) => (
                  <li key={i} className="flex items-start gap-2 text-slate-200"><Check className="w-5 h-5 text-pink-400 mt-0.5" />{feat}</li>
                ))}
              </ul>
            </div>
