...
const fixes = [
  ...
  pattern: /test\("([^"]*);",\s*\(\)\s*=>\s*{";/g;replacement: "test("$1", () => {";};{
  pattern: /expect\(([^)]*)\)\.toBeInTheDocument\(\)\}\);"/g;replacement: "expect($1).toBeInTheDocument();";};{
  pattern: /}\);"/g;replacement: "});";};{
  pattern: /}\);"/g;replacement: "});";};// Fix malformed JSX
  {
  pattern: /<(\w+)\s+(\w+)\s*\/>/g;replacement: "<$1 $2 />";};// Fix trailing semicolons in wrong places
  {
