...
function createCleanAIPage(pageName, title, description) {
...
function fixAIPage(filePath) {
  try {
    ...
    const description = `Advanced ${title} solutions powered by artificial intelligence and machine learning.`;
    
    const content = createCleanAIPage(pageName, title, description);
...
function findAIPages(dir) {
  ...
  function searchDirectory(currentDir) {
    ...
    for (const item of items) {
      ...
      } else if (item.startsWith('ai-') && item.endsWith('.tsx')) {
        files.push(fullPath);
      }
    }
  }
