...
# Script to merge recent feature branches
set -e

echo "Starting merge of recent feature branches..."

# Get the most recent feature branches (last 50)
RECENT_FEATURE_BRANCHES=$(git branch -r | grep "origin/feature/" | sort -V | tail -50 | sed 's/origin\///')

# Count total branches
TOTAL_BRANCHES=$(echo "$RECENT_FEATURE_BRANCHES" | wc -l)
echo "Found $TOTAL_BRANCHES recent feature branches to process"
...
resolve_conflicts() {
    ...
    for file in $conflicted_files; do
        ...
        # For most conflicts, prefer the incoming version (from feature branch)
...
# Process each branch
for branch in $RECENT_FEATURE_BRANCHES; do
    merge_branch "$branch"
done
