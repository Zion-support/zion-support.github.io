...
# Batch processing script for remaining branches

set -e

echo "Starting batch processing of remaining branches..."

# Get all remote branches that start with cursor/
ALL_BRANCHES=$(git branch -r | grep "origin/cursor/" | sed 's/origin\///' | head -50)
...
echo "Processing first 50 branches..."
