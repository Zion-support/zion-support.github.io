import fs from 'fs'; // Read existing pages; const existingPages = fs.readFileSync('existing_pages.txt', 'utf8').split('\n').filter(page => page.trim() && page !== 'page.tsx'); // Navigation links from the Navigation component; const navigationLinks = [
  // Main navigation; '/about', '/blog', '/case-studies', '/pricing', '/contact', '/services', '/specialized-services',
  
  // Micro SAAS Services; '/ai-project-manager', '/ai-writing-assistant', '/ai-crm', '/ai-healthcare', '/ai-fintech',
  ...
  // AI Services; '/machine-learning', '/ai-business-intelligence', '/ai-supply-chain', '/ai-quality-assurance',
  ...
  // IT Services; '/cloud-services', '/enterprise-security', '/it-infrastructure', '/data-center',
  ...
  // Specialized Services; '/quantum-computing', '/autonomous-systems', '/blockchain', '/iot-edge', '/business-intelligence',
...
const navigationPages = navigationLinks.map(link => link.substring(1)); // Find missing pages; const missingPages = navigationPages.filter(page => !existingPages.includes(page)); console.log('Missing pages: '); missingPages.forEach(page => console.log(`- ${page}`)); console.log(`\nTotal missing pages: ${missingPages.length}`); console.log(`Total existing pages: ${existingPages.length}`); console.log(`Total navigation links: ${navigationPages.length}`); // Write missing pages to file; fs.writeFileSync('missing_pages.txt', missingPages.join('\n'));