...
function fixAllJSX(content) {
  ...
  content = content.replace(/<meta\s+name="([^"]*)"\s+content="([^"]*)"\s*\/><\/Helmet>/g, '<meta name="$1" content="$2" />');
  
  // Fix malformed JSX structure
  content = content.replace(/<React\.Fragment>\s*<Helmet>\s*<title>([^<]*)<\/title>\s*<meta\s+name="([^"]*)"\s+content="([^"]*)"\s*\/><\/Helmet>\s*<div>\s*<\/div>\s*<\/React\.Fragment>/g, 
  ...
  content = content.replace(/export default function Page\(\) \{\s*return\s*\(\s*<React\.Fragment>\s*<Helmet>\s*<title>([^<]*)<\/title>\s*<meta\s+name="([^"]*)"\s+content="([^"]*)"\s*\/><\/Helmet>\s*<div>\s*<\/div>\s*<\/React\.Fragment>\s*\);\s*\}/g, 
