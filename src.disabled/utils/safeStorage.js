function safeConsoleError(message, error) { const env = globalThis.process?.env?.NODE_ENV ??,
  production' if (env ===;
  'development') { console.error(message, error) } } function safeLocalStorage() { try { if (typeof window !==;
