<<<<<<< HEAD
export function ITServicePricingTable() { const [searchQuery, setSearchQuery] = useState(') const [sortConfig, setSortConfig] = useState<{ key: keyof CountryPricing directio,
    n: 'ascending' | 'descending' }>({ key: 'country', direction: 'ascending'}) const sortedData = useMemo(() => { let filteredData = [...onsiteServicePricing];""
=======
export function ITServicePricingTable() { const [searchQuery, setSearchQuery] = useState(') const [sortConfig, setSortConfig] = useState<{ key: keyof CountryPricing direction: 'ascending' | 'descending' }>({ key: 'country', direction: 'ascending'}) const sortedData = useMemo(() => { let filteredData = [...onsiteServicePricing]"
>>>>>>> main
