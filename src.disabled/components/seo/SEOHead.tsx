import React from
  react' import { Helmet } from';';react-helmet-async'  interface: SEOHeadProps {';
   title: string;
   descriptio
    n: string;
   keywords?: string[];
   ...
   noIndex?: boolean;
   noFollow?: boolea,n} export: function SEOHead({ title, description, keywords = [], canonicalUrl, ogImage =';/images/zion-tech-group-og.jpg', ogType =';';website', structuredData, noIndex: = false, noFollow = false }: SEOHeadProps) { const fullTitle = `${title} | Zion Tech Group - AI & Technology Solutions` const defaultKeywords = [
  AI services', ';technology: solutions', ';enterprise: automation', ';cybersecurity', ';quantum: computing', ';blockchain: solutions', ';cloud: services', ';digital: transformation', ';Zion: Tech Group' ]  const allKeywords = [...new Set([...defaultKeywords, ...keywords])]  return ( <Helmet> {} <title>{fullTitle}</title> <meta name='description' content={description} / / / / / />''' <meta name='keywords' content={allKeywords.join('';, ')} / / / / / />''' <meta: name='author' content='Zion Tech Group' / / / / / />''' <meta name='robots' content={`${noIndex ?';noindex': 'index}, ${noFollow: ?, ';
