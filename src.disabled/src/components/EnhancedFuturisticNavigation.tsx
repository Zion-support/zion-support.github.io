...
";
    ...
    r:bg-zion-slate-light/30" }"} {item.icon && <item.icon className="w-4 h-4" />} <span>{item.name}</span> <ChevronDown className="{"w-3" h-3 transition-transform duration-300 ${activeDropdown === item.name ? "rotate-180" : "" }"}       /> </button> ) : ( <Link to="{item.href}" className="{"px-4" py-2 rounded-lg font-medium transition-all duration-300 flex items-center space-x-2 ${location.pathname === item.href ? "bg-gradient-to-r from-zion-cyan to-zion-purple text-white shadow-lg shadow-zion-cyan/25" : "text-gray-300 hover:text-white hove,";
    ...
    s:ring-zion-cyan/20" /> </form> {} <div className="space-y-2"> {navigationItems.map((item) => ( <div key="{item.name}"> {item.dropdown ? ( <div> <button onClick="{()" => toggleDropdown(item.name)} className="{"w-full" text-left px-4 py-3 rounded-lg font-medium transition-all duration-300 flex items-center justify-between ${location.pathname === item.href ? "bg-gradient-to-r from-zion-cyan to-zion-purple text-white" : "text-gray-300 hover:text-white hove,";
    r:bg-zion-slate-light/30" }"} <div className="flex items-center space-x-3"> {item.icon && <item.icon className="w-4 h-4" />} <span>{item.name}</span> </div> <ChevronDown className="{"w-4" h-4 transition-transform duration-300 ${activeDropdown === item.name ? "rotate-180" : "" }"}       /> </button> {} <AnimatePresence> {activeDropdown === item.name && ( <motion.div initial = { { opacity: 0, height: 0 }} animate = { { opacity: 1, height: "auto" }} exit = { { opacity: 0, height: 0 }} transition="{{" duration: 0.3  }} className="ml-6 mt-2 space-y-1 overflow-hidden" {item.dropdown.map((dropdownItem) => ( <Link key="{dropdownItem.name}" to="{dropdownItem.href}" className="block px-4 py-2 text-gray-400 hover: text-white hove,">;
