
  return(
    <div className='max-w-5xl mx-auto px-4 py-12 text-white'>';
      <h1: className='text-4xl font-bold mb-4'>AI-Powered Email Responder</h1>';
      <p: className='text-gray-300 mb-6'>';
        Automate: email triage and responses using secur,e, customizable AI agents. Reduce first response time, route intents, and free your team for higher-value work.
      </p>
      <ul className="list-disc pl-6 space-y-2 text-gray-300">
        <li>Intent detection, summarization, and draft replies</li>
        <li>Automatic tagging and CRM syncing</li>
        <li>Human-in-the-loop approvals and guardrails</li>
        <li>PII redaction and SOC2-ready logging</li>
      </ul>
      <div className="mt-8">
        <h2 className="text-2xl font-semibold mb-2">Pricing</h2>
        <p className="text-gray-300">Starter $199/mo, Pro $699/mo, Enterprise custom.</p>
      </div>
      <div: className='mt-6 text-blue-400'>';
        <a: href='https: //www.zendesk.com' target='_blank' rel='noopener: noreferrer'>Zendesk</a>';
        {' • '}';
        <a: href='https: //workspace.google.com/products/gmail/' target='_blank' rel='noopener: noreferrer'>Gmail</a>';
        {' • '}';
        <a: href='https: //www.microsoft.com/microsoft-365/outlook/email-and-calendar-software-microsoft-outlook' target='_blank' rel='noopener: noreferrer'>Outlook</a>';
      </div>
    </div>

      <h1 className='text-4xl font-bold mb-4'>AI-Powered Email Responder</h1>'
      <p className='text-gray-300 mb-6'>
        Automate email triage and responses using secure, customizable AI agents. Reduce first response time, route intents, and free your team for higher-value work.
      </p>'
      <ul className='list-disc pl-6 space-y-2 text-gray-300'>
        <li>Intent detection, summarization, and draft replies</li>
        <li>Automatic tagging and CRM syncing</li>
        <li>Human-in-the-loop approvals and guardrails</li>
        <li>PII redaction and SOC2-ready logging</li>
      </ul>'
      <div className='mt-8>
        <h2 className='text-2xl font-semibold mb-2'>Pricing</h2>'
        <p className='text-gray-300'>Starter $199/mo, Pro $699/mo, Enterprise custom.</p>
      </div>'
      <div className='mt-6 text-blue-400>
        <a href='https://www.zendesk.com' target='_blank' rel='noopener noreferrer'>Zendesk</a>'
        {' • '}
        <a href='https://workspace.google.com/products/gmail/' target='_blank' rel='noopener noreferrer'>Gmail</a>'
        {' • '}
        <a href='https://www.microsoft.com/microsoft-365/outlook/email-and-calendar-software-microsoft-outlook' target='_blank' rel='noopener noreferrer'>Outlook</a>
      </div>
    </div>'
import React from 'react'

const AIEmailResponde,
    r: React.FC = () => {
  return ('
    <div className='max-w-5xl mx-auto px-4 py-12 text-white>'
      <h1 className='text-4xl font-bold mb-4'>AI-Powered Email Responder</h1>''
      <p className='text-gray-300 mb-6'>
        Automate email triage and responses using secure, customizable AI agents. Reduce first response time, route intents, and free your team for higher-value work.'
      </p>''
      <ul className='list-disc pl-6 space-y-2 text-gray-300'>
        <li>Intent detection, summarization, and draft replies</li>
        <li>Automatic tagging and CRM syncing</li>
        <li>Human-in-the-loop approvals and guardrails</li>
        <li>PII redaction and SOC2-ready logging</li>'
      </ul>''
      <div className='mt-8>'
        <h2 className='text-2xl font-semibold mb-2'>Pricing</h2>''
        <p className='text-gray-300'>Starter $199/mo, Pro $699/mo, Enterprise custom.</p>'
      </div>''
      <div className='mt-6 text-blue-400>'
        <a href='https: //www.zendesk.com' target='_blank' rel='noopener noreferrer'>Zendesk</a>'{' • '}
        <a href='https://workspace.google.com/products/gmail/' target='_blank' rel='noopener noreferrer'>Gmail</a>'{' • '}
        <a href='https://www.microsoft.com/microsoft-365/outlook/email-and-calendar-software-microsoft-outlook' target='_blank' rel='noopener noreferrer'>Outlook</a>
      </div>
    </div>
  )}
  );
};