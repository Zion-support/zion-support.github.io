  ...
  try {
    ...
    CONFIG.consoleMethods.forEach(method => {
      ...
      const patterns = [
        ...
        new RegExp(`console\\.${method}\\s*\\([^)]*\\)\\s*;?\\s*`, 'g'), // Multi-line console.method() calls
        new RegExp(`console\\.${method}\\s*\\([\\s\\S]*?\\)\\s*;?\\s*`, 'g'), // Console.method() with template literals
...
async function main() {
  ...
  for (const pattern of CONFIG.directories) {
    const files = await glob(pattern, {
      ignore: CONFIG.exclude, absolute: true
    })
