...
class BuildOptimizer {
  constructor() {this.projectRoot = process.cwd(),this.reportsDir = path.join(this.projectRoot, 'build-reports'),this.ensureDirectories()}
  ensureDirectories() {
  ...
  analyzeBuildConfig() {
    ...
    try {
      const configFiles = ['vite.config.js','vite.config.ts','next.config.js';
        'next.config.ts';
        'webpack.config.js';
        'webpack.config.ts';
        'rollup.config.js';
      ...
      for (const configFile of configFiles) {
        ...
        if (fs.existsSync(configPath)) {
          ...
          try {
            ...
            configAnalysis[configFile] = {size: content.length,hasOptimization: content.includes('optimization') || content.includes('minify'),hasCompression: content.includes('compression') || content.includes('gzip');
      ...
      return {success: true,foundConfigs,configAnalysis;
  ...
  analyzeBuildOutput() {
    ...
    try {
          ...
          for (const file of files) {
            ...
            if (stat.isDirectory()) {
              ...
              if (stat.size > 100 * 1024) { // Files larger than 100KB
                largeFiles.push({file: path.relative(distPath, filePath),size: stat.size;
                  sizeKB: (stat.size / 1024).toFixed(1)})}
            }
          }
        };
        scanDirectory(dir);
        return {totalSize,fileCount,fileTypes;
          largeFiles}}
      ...
      return {success: true,totalSize: stats.totalSize,totalSizeMB: parseFloat(totalSizeMB);
  ...
  checkBuildPerformance() {
    ...
    try {
      const performanceChecks = {hasSourceMaps: false,hasMinification: false,hasCompression: false;
  ...
  generateReport() {
    ...
    const report = {
      ...
      outputAnalysis;
      performanceCheck;
      summary: {buildSuccessful: buildResult.success,buildTime: buildResult.success ? buildResult.buildTime : null,outputSizeMB: outputAnalysis.success ? outputAnalysis.totalSizeMB : null;
    ...
    return report}
  async run() {
    try {this.log('🚀 Starting Build Optimizer'),const report = this.generateReport(),this.log('✅ Build optimization completed');
...
if (require.main === module) {const optimizer = new BuildOptimizer(),optimizer.run().catch(console.error)}
