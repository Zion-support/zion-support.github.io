...
class BuildHealthCheck {
  ...
  async checkDependencies() {
    try {
      ...
      await this.log(`Dependencies check passed: ${totalDeps} packages expected`, 'INFO');
      ...
      try {
        ...
        await this.log(`Failed to fix dependencies: ${fixError.message}`, 'ERROR');
  ...
  async checkConfigFiles() {
    ...
    for (const config of configs) {
      try {
        ...
        await this.log(`Config file ${config} has issues: ${error.message}`, 'ERROR');
    ...
    try {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          ...
          await this.log(`Build assets found: ${files.length} files in dist/`, 'INFO');
  ...
  async run() {
    try {
      ...
      const invalidConfigs = results.configs.filter(c => c.status === 'invalid').length;
      ...
      await this.log(`Build health check completed: ${results.status} (${results.healthScore}%)`, 'INFO');
