...
class AutomationDashboard {
  ...
  'stopped').length
      ...
      processes: this.processes.map(proc => ({
        ...
        memory: `${Math.round(proc.monit.memory / 1024 / 1024)}MB`,cpu: `${proc.monit.cpu}%',
  ...
  async displayDashboard() {
    ...
    const health = await this.generateHealthReport();// // // // // // // console.log(`📊 Status: ${health.summary.onlineProcesses}/${health.summary.totalProcesses} processes online`);// // // // // // // console.log(`⏰ Last Updated: ${new Date().toLocaleTimeString()}');
    // // // // // // // console.log(,);
    // Display process table
    // // // // // // // console.log('🔄 Automation Processes: ');
    // // // // // // // console.log('─.repeat(80));
    // // // // // // // console.log('Name
  '.padEnd(25) + 'Status.padEnd(10) + 'Memory
  '.padEnd(10) + 'CPU.padEnd(8) + 'Uptime
  '.padEnd(15) + 'Restarts);
    // // // // // // // console.log('─
  '.repeat(80));
  ...
  ' : '🟡;
      // // // // // // // console.log(
        proc.name.padEnd(25) +${statusIcon} ${proc.status}'.padEnd(10) +
  ...
  ') || log.includes('error)) {// // // // // // // console.log('🔴 ${log}`);
        } else if (log.includes('WARN) || log.includes('warn
  ')) {
          // // // // // // // console.log(`🟡 ${log}`);
        } else {// // // // // // // console.log(`ℹ️  ${log}`);
        }
      });
    } catch (error) {
...
console.log('🔴 ${log}`)} else if (log.includes(,
  WARN') || log.includes(warn')) {
          console.log(`🟡 ${log}`)} else {console.log(`ℹ️  ${log}')}
      })} catch (error) {
      console.log(  No recent logs available')}
    console.log('
  ');
  ...
  ');
    ...
    failedProcesses.forEach(proc => {
      try {
        execSync(`pm2 restart ${proc.pm_id}`, { stdio: 'inherit });// // // // // // // console.log(`✅ Restarted ${proc.name}`);
  ...
  ');
      return}
    failedProcesses.forEach(proc => {
      try {execSync(`pm2 restart ${proc.pm_id}`, { stdio: 'inherit });console.log(`✅ Restarted ${proc.name}`)} catch (error) {console.error(`❌ Failed to restart ${proc.name}:', error.message)}
    })}
  async generatePerformanceReport() {
    console.log(,
