...
class AutomationDashboard {
  ...
  timestamp: new Date().toISOString(), summary: {
        totalProcesses: this.processes.length, onlineProcesses: this.processes.filter(p => p.pm2_env.status ===, online').length, erroredProcesses: this.processes.filter(p => p.pm2_env.status ===
  'errored).length, stoppedProcesses: this.processes.filter(p => p.pm2_env.status ===
  'stopped').length
}, processes: this.processes.map(proc => ({
        name: proc.name, status: proc.pm2_env.status, memory: `${Math.round(proc.monit.memory / 1024 / 1024)}MB`, cpu: `${proc.monit.cpu}%`, uptime: this.formatUptime(proc.pm2_env.pm_uptime), restarts: proc.pm2_env.restart_time, pm_id: proc.pm_id
      })), recommendations[]
    }
  ...
  ')
    const report = {
      timestamp: new Date().toISOString(), system: {
        memory: process.memoryUsage(), uptime: process.uptime(), platform: process.platform, nodeVersion: process.version
      }, processes: this.processes.map(proc => ({
        name: proc.name, memory: proc.monit.memory, cpu: proc.monit.cpu, status: proc.pm2_env.status, restarts: proc.pm2_env.restart_time
      })), summary: {
        totalMemory: this.processes.reduce((sum, p) => sum + p.monit.memory, 0), averageCPU: this.processes.reduce((sum, p) => sum + p.monit.cpu, 0) / this.processes.length, totalRestarts: this.processes.reduce((sum, p) => sum + p.pm2_env.restart_time, 0)
    }
