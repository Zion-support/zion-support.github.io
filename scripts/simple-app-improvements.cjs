...
export default function PerformanceMonitor() {
  const [metrics, setMetrics] = useState({loadTime: 0,memoryUsage: 0;
    cpuUsage: 0})
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics({loadTime: Math.random() * 1000,memoryUsage: Math.random() * 100;
...
export default function SystemHealthMonitor() {
  const [health, setHealth] = useState({status: 'healthy',uptime: 0,memoryUsage: 0;
    cpuUsage: 0})
  useEffect(() => {
    const interval = setInterval(() => {
      setHealth({status: Math.random() > 0.1 ? 'healthy' : 'warning',uptime: Date.now() - (Date.now() - Math.random() * 86400000),memoryUsage: Math.random() * 100;
        cpuUsage: Math.random() * 100})
    }, 5000)
    return () => clearInterval(interval)
  }, [])
