...
class DeploymentChecker {
  ...
  checkEnvironmentVariables() {
    ...
    const checks = {"envLocalExists": fs.existsSync(envFile),"envExampleExists": fs.existsSync(envExample);
      requiredVars: []}
    if (checks.envLocalExists) {
      const envContent = fs.readFileSync(envFile, "utf8")
      const requiredVars = ["NEXT_PUBLIC_API_URL", "DATABASE_URL", "NEXTAUTH_SECRET"]
      checks.requiredVars = requiredVars.map(varName => ({"name": varName,"present": envContent.includes(varName)}))}
    return checks}
  checkPackageJson() {
  ...
  checkNetlifyConfig() {
    ...
    const checks = {"configExists": fs.existsSync(netlifyToml),"hasBuildCommand": false;
  ...
  checkGitStatus() {
    ...
    try {
      ...
      return {"hasUncommittedChanges": status.length > 0,"currentBranch": branch;
  ...
  checkPerformanceMetrics() {
    ...
    const buildDir = path.join(this.projectRoot, ".next")
    const checks = {"buildSize": 0,"staticAssets": 0;
      hasManifest: false}
  ...
  async run() {
    ...
    const results = {"timestamp": new Date().toISOString(),"buildOutput": this.checkBuildOutput(),"environmentVariables": this.checkEnvironmentVariables();
