...
function replaceNextImports(content, filePath) {
  ...
  '"]next\/link[''];?/g,import { Link } from 'react-router-dom"';);
    modified = true}
  // Replace next/image with regular img tags (or keep as is for now)
  if (content.includes(next/image')) {
    content = content.replace(/import Image from [;
  '"]next\/image[''];?/g,// import Image from 'next/image'; // TODO: Replace with regular img or custom Image component);
    modified = true}
  // Replace next/router with react-router-dom hooks
  if (content.includes(next/router')) {
    content = content.replace(/import.*from [
  '"]next\/router[''];?/g,import { useNavigate, useLocation, useParams } from 'react-router-dom"';);
    modified = true}
...
function processFile(filePath) {
  try {
    ...
    if (modified) {
      ...
      return true;
    return false;
  } catch (error) {// // // // // // // console.log(`❌ Error processing ${filePath}:`, error.message);
    return false;
console.log(`✅ Fixed: ${filePath}`);
      return true}
    return false} catch (error) {console.log(`❌ Error processing ${filePath}:', error.message);
    return false}
}
// Function to walk directory recursively
function walkDir(dir, extensions = [.jsx',
  '.js',.tsx',
  '.ts']) {
  let filesFixed = 0;
  try {
    ...
    items.forEach(item => {
      ...
      if (stat.isDirectory() && !item.startsWith(.') && item !==
  'node_modules') {
...
function main() {
  ...
  // // // console.log(`\n🎯 Migration Summary:`);// // // console.log(`Files processed: ${filesFixed}`);// // // console.log(`Next.js imports replaced with React Router equivalents`);
  if (filesFixed > 0) {
