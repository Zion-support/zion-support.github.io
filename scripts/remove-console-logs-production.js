  ...
  async removeConsoleLogs() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // Remove console && console.log statements that are not in development blocks
        let newContent = content && content.replace(
          /console\.log\([^)]*\);?\s*/g,
          (match, offset, string) => {
            // Check if this console && console.log is already in a development block
            const beforeMatch = string && string.substring(0, offset);
            const afterMatch = string && string.substring(offset + match && match.length);
