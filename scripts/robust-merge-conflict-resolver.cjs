...
function resolveMergeConflict(filePath) {
  try {
    ...
    if (
      ...
      !content.includes("import React")) {content = import React from "react",\n\n + content}
    ...
    if (
      content.includes("export default") &&
      !content.includes("import React")) {content = "import React from "react",\n\n" + content}
    if (fixed && content !== originalContent) {// Additional cleanup: remove any remaining corrupted content
      content = content.replace(/[^\x00-\x7F]/g, "),// Remove non-ASCII characters
      content = content.replace(/\s+/g, " "), // Normalize whitespace
      fs.writeFileSync(filePath, content, "utf8")
      return true}
...
const colors = {reset: "\x1b[0m",",red: "\x1b[31m",""green: "\x1b[32m",",  yellow": "\x1b[33m",",  blue: "\x1b[34m",",  magenta: "\x1b[35m",""cyan: "\x1b[36m","};";function log(message, color = "reset") {";  console.log(`${colors[color]}${message}${colors.reset}`);`}
function resolveMergeConflict(filePath) {
  try {
    if (!fs.existsSync(filePath)) {
      return false}
    ...
    if (fixed && content !== originalContent) {// Additional cleanup: remove any remaining corrupted content,",      content = content.replace(/[^\x00-\x7F]/g, ""), // Remove non-ASCII characters",      content = content.replace(/\s+/g, " "), // Normalize whitespace"",      fs.writeFileSync(filePath, content, "utf8"),",      return true,"}
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
      .split("\n")";      .filter(line => line.trim()),"} catch (error) {log("Error finding conflicted files", "red"),return [],"}
}
function main() {
