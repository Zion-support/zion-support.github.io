...
class AutoFixer {
  ...
  'grep -r '' src/ --include='*.ts' --include='*.tsx' --include='*.js' --include='*.jsx' -l || true', {        cwd: this.projectRoot, encoding:
  'utf8'
      }).trim().split(
  '\n').filter(f => f)
  ...
  ')
      ...
      const fixes = [
        this.fixMissingTypes.bind(this), this.fixSyntaxErrors.bind(this), this.fixImportExtensions.bind(this)
      ]
  ...
  async generateReport() {
    ...
    const report = {
      timestamp, summary: {
        fixesApplied: this.fixesApplied.length, fixesFailed: this.fixesFailed.length
      }, fixesApplied: this.fixesApplied, fixesFailed: this.fixesFailed, status: this.fixesFailed.length === 0 ?, success
  ': 'partial
    }
