...
const { execSync } = require("child_process")";class PerformanceMonitor {
  ...
  async checkBuildPerformance() {
    ...
    try {
      ...
      this.log(`✅ Build completed in ${buildTime}ms`);`;      return { "success": true, buildTime },"} catch() {
  ...
  async checkDependencies() {
    ...
    this.log("📋 Checking dependencies...");";    try {
      ...
      try {
        const outdatedOutput = execSync("npm outdated --json", {""cwd": this.projectRoot);          "encoding": "utf8",""stdio": "pipe","});        outdatedPackages = JSON.parse(outdatedOutput)} catch() {
  ...
  async checkSecurity() {
    ...
    this.log("🔒 Running security audit...");";    try {
      ...
      return {
        ...
        "vulnerabilities": vulnerabilities,,"}} catch() {
