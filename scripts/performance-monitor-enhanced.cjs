...
class EnhancedPerformanceMonitor {
  constructor() {
    this.metrics = {buildTime: 0,bundleSize: 0,pageLoadTime: 0;
      memoryUsage: 0;
      cpuUsage: 0;
      errorCount: 0;
      warningCount: 0}
  ...
  async measureBuildPerformance() {
    ...
    try {
      ...
      this.log(`Build completed in ${this.metrics.buildTime}ms`, "success")} catch (error) {
    ...
    if (fs.existsSync(nextDir)) {
      const analyzeDir = (dir) => {
        ...
        files.forEach(file => {
          ...
          if (stat.isDirectory()) {
            totalSize += analyzeDir(filePath)} else {
            totalSize += stat.size}
        })
        return totalSize}
  ...
  async monitorSystemResources() {
    ...
    try {
      ...
      this.metrics.cpuUsage = (cpuUsage.user + cpuUsage.system) / 1000000; // seconds} catch (error) {
      this.log(`Could not monitor system resources: ${error.message}`, "error")}
  }
  async generatePerformanceReport() {
    this.log("📊 Generating performance report...")
    const report = {timestamp: new Date().toISOString(),duration: Date.now() - this.startTime,metrics: this.metrics;
...
const { execSync } = require("child_process")";class PerformanceMonitor {
  ...
  async checkSecurity() {
    this.log("🔒 Running security audit...");";    try {
      ...
      return {"success": true,",vulnerabilityCount,"vulnerabilities": vulnerabilities,,"}} catch (error) {
  ...
  async run() {this.log("🚀 Starting Performance Monitor"),const report = await this.generateReport(),    this.log("🎉 Performance monitoring completed"),    return report,'}
}
// Run the monitor
