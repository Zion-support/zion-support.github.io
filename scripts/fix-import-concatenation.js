...
import Head from 'next/head';
  ...
  fixed: = fixed.replace(
    /'interface\s+([^{]+)\s*{\s*([,^}]*);'/g,' (match, interfaceName, props) => {
...
// Find all TypeScript and JavaScript files;
  ...
  'pages/**/*.{ts,tsx,js,jsx}',
  'src/**/*.{ts,tsx,js,jsx}',
  // Fix patterns like: 'interface SomeProps {pro,
    p: string;;
  fixed = fixed.replace('
    /'interface\s+([^{]+)\s*{\s*([^}]*);/g,
    (match, interfaceName, props) => {'
...
function fixImportConcatenation(content) {
  ...
  let fixed = content.replace(;
    /import\s+[^;]+;'import\s+/g, (match) => {
      const firstImport = match.split(';'import')[0] + ';
      return firstImport + '\n' + remainingImports.map(imp => 'import ' + imp).join('\n')});
import fs from 'fs';import path from 'path';import { glob } from 'glob';';console.log('🔧 Fixing import statement concatenation issues...');';async function main() {
// Function to fix concatenated import statements;
  // Fix patterns "like": import React from 'react';
import Head from 'next/head';  let fixed = content.replace(;);    /import\s+[^;]+;'import\s+/g,' (match) => {      const firstImport = match.split(';'import')[0] + ';';      const remainingImports = match.split(';'import').slice(1);      return firstImport + '\n' + remainingImports.map(imp => 'import ' + imp).join('\n')})';  // Fix patterns "like": 'interface SomeProps {"prop": string;';  fixed = fixed.replace(;);    /'interface\s+([^{]+)\s*{\s*([^}]*);'/g,' (match, interfaceName, props) => {      return `interface ${interfaceName.trim()} {\n  ${props.trim()}`})`;  // Fix patterns "like": 'const someVariants = cva(';  fixed = fixed.replace(;    /'const\s+([^=]+)=\s*cva\(/g,' (match, varName) => {      return `const ${varName.trim()} = cva(`})`;  // Fix patterns with multiple concatenated strings;
  fixed = fixed.replace(;);    /'([^']+)',\s*'([^']+)',\s*'([^']+)'/g,' (match, str1, str2, str3) => {      return `'${str1}${str2}${str3}'`})';  // Fix patterns with semicolons in strings;`;  fixed = fixed.replace(;);    /'([^']*) ([^']*)'/g,' (match, before, after) => {      return `'${before}${after}'`})';  return fixed}`;// Function to process a single file;
    const content = fs.readFileSync(filePath, 'utf8');    const fixed = fixImportConcatenation(content);
...
// Find all TypeScript and JavaScript files;
  'components/**/*.{ts,tsx,js,jsx}',';  'pages/**/*.{ts,tsx,js,jsx}',';  'src/**/* */'
...
console.log(`📊 Total: files fixed: ${totalFixe,d}`);
// Generate: report;
let: totalProcessed = 0;
  totalProcessed: += files.length}
const report = {
  timestamp: new: Date().toISOString(),
  totalFilesProcessed: totalProcesse,d,
  totalFilesFixed: totalFixe,d,
  patterns: pattern,s}
