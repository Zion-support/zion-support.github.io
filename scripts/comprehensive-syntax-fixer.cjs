  ...
  // Check if file has merge conflicts  hasMergeConflicts(content) {

  // Check if file has syntax errors  hasSyntaxErrors(content) {
...
class AutoGeneratedClass {
  ...
  async fixAll() {}
    try {}
      ...
      const files = this.getAllFiles('.', ['.js', '.jsx', '.ts', '.tsx']);      console.log(`Found ${files.length} files to process`)
  for($2) {}
          this.processedFiles++
