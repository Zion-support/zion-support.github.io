...
if [ -f "public/sitemap.xml" ]; then
  ...
  # Extract URLs from <loc> tags (works on macOS and Linux)
  ...
  done < <(grep -o '<loc>[^<]*</loc>' public/sitemap.xml | sed -e 's/<\/\?loc>//g')
fi

# Check external links if --external flag
if [ "${1:-}" = "--external" ]; then
  echo "  Checking external links..."
  # Extract URLs from .tsx and .ts files (basic http/https URLs)
  urls=$(grep -roE 'https?://[a-zA-Z0-9./?=_%:-]+' app/ --include='*.tsx' --include='*.ts' 2>/dev/null | grep -v 'ziontechgroup.com' | sort -u)
  while IFS= read -r url; do
    HTTP_CODE=$(curl -sL -o /dev/null -w "%{http_code}" --max-time 10 "$url" 2>/dev/null || echo "000")
    TOTAL=$((TOTAL + 1))
    if [ "$HTTP_CODE" != "200" ] && [ "$HTTP_CODE" != "301" ] && [ "$HTTP_CODE" != "302" ]; then
