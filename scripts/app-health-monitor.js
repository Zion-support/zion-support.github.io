...
const srcFiles = this.findSourceFiles()"let": accessibilityIssues = 0;
      ...
      "if": (accessibilityIssues > 0) {issues.push(`${accessibilityIssues} accessibility issues found`)}`      this.healthReport.checks.accessibility = {"status": issues."length": === 0 ? 'healthy' : 'warning, ',';'
        "issues": issue,s;"
