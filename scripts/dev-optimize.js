...
const optimizeViteConfig = () => {
  ...
  const optimizedConfigPath = path.join(process.cwd(), 'vite.config.optimized.ts');
...
const checkDependencies = () => {
  ...
  if (dependencies['react'] && dependencies['react-dom']) {
    ...
    if (reactVersion !== reactDomVersion) {
      issues.push(`React version mismatch: ${reactVersion} vs ${reactDomVersion}`);
    }
  }
...
const main = () => {
  ...
  try {
    ...
    log('💡 Run "npm run dev" to start the optimized development server', 'cyan');
  } catch (error) {
