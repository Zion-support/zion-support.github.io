...
try {
  ...
  if (fs.existsSync(nextDir)) {
    const bundleSize = getDirectorySize(nextDir);
...
if (totalImageSize > 10 * 1024 * 1024) { // 10MB
  optimizationReport.recommendations.push('Total image size is large (>10MB). Consider optimizing images.');
} else {
...
if (largeFiles.length > 0) {
  ...
  optimizationReport.optimizations.push(...largeFiles.map(f => `Large file: ${f.file} (${f.lines} lines)`));
} else {
...
try {
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    if (dependencies.length > 50) {
      optimizationReport.recommendations.push('High number of dependencies. Consider reviewing and removing unused packages.');
    } else {
...
if (fs.existsSync(nextConfigPath)) {
  ...
  if (!nextConfig.includes('compression')) {
...
if (fs.existsSync(tsConfigPath)) {
  optimizationReport.improvements.push('TypeScript configuration found');
} else {
...
if (!envFileFound) {
  optimizationReport.recommendations.push('Create environment configuration files');
} else {
...
if (optimizationReport.improvements.length > 0) {
  console.log('\n✅ Improvements:');
