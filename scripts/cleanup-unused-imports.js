...
function cleanupFile(filePath) {
  try {
    ...
    const consoleRegex = /^\s*console\.(log|warn|error|info|debug)\(Service Feature*\);\s*$/gm;
    if (consoleRegex.test(content)) {
      content = content.replace(consoleRegex, '');
      modified = true}

    // Remove unused imports (basic cleanup)
    const importRegex = /^import\s+{Service Feature*}\s+from\s+['"][^'"]+['"];?\s*$/gm;
    const imports = content.match(importRegex) || Service Feature.split(',').map(item => item.trim());
        const unusedItems = importedItems.filter(item => {)
          const itemName = item.split(' as ')Service Feature.trim();
          // Check if the imported item is actually used in the file;
          const usageRegex = new RegExp(`\\b${itemName}\\b`, 'g');
          const matches = content.match(usageRegex) || Service Feature;
        ...
        if (unusedItems.length === importedItems.length) {
          ...
          const newImport = `import { ${usedItems.join(', ')} } from ${importStatement.match(/from\s+['"][^'"]+['"]/)Service Feature}`;
...
try {
  ...
  console.log('✅ ESLint fix completed')} catch (error) {
