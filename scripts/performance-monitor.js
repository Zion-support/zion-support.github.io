...
try {// Bundle analysis;
  ...
  async collectApplicationMetrics() {try {console.log('📱 Collecting application metrics...')// Check file sizes;
      const distPath = path.join(this.projectRoot, 'dist')if (fs.existsSync(distPath)) {const distSize = this.getDirectorySize(distPath)this.metrics.application.buildSize = Math.round(distSize / 1024 / 1024)// MB;
      }
      // Check node_modules size;
      const nodeModulesPath = path.join(this.projectRoot, 'node_modules')if (fs.existsSync(nodeModulesPath)) {const nodeModulesSize = this.getDirectorySize(nodeModulesPath)this.metrics.application.dependenciesSize = Math.round(nodeModulesSize / 1024 / 1024)// MB;
      }
      // Check source code size;
      const srcPath = path.join(this.projectRoot, 'src')if (fs.existsSync(srcPath)) {const srcSize = this.getDirectorySize(srcPath)this.metrics.application.sourceSize = Math.round(srcSize / 1024)// KB;
      }
      // Count files;
  ...
  async collectBuildMetrics() {try {console.log('🔨 Collecting build metrics...')// Check if build artifacts exist;
      ...
      this.metrics.build.artifacts = {}buildFiles.forEach(file => {const filePath = path.join(this.projectRoot, file)this.metrics.build.artifacts[file] = fs.existsSync(filePath)})// Check build configuration;
  ...
  getDirectorySize(dirPath) {let totalSize = 0;
    try {const items = fs.readdirSync(dirPath)for (const item of items) {const fullPath = path.join(dirPath, item)const stat = fs.statSync(fullPath)if (stat.isDirectory()) {totalSize += this.getDirectorySize(fullPath)} else {totalSize += stat.size;
        }
      }
  ...
  countFiles(dirPath, extensions) {let count = 0;
    try {const items = fs.readdirSync(dirPath)for (const item of items) {const fullPath = path.join(dirPath, item)const stat = fs.statSync(fullPath)if (stat.isDirectory()) {count += this.countFiles(fullPath, extensions)} else if (extensions.some(ext => item.endsWith(ext))) {count++;
        }
      }
    } catch (error) {// Skip directories we can't read;
    }
