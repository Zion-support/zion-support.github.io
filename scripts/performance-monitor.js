...
/**
 * Performance monitoring script to analyze bundle size and performance metrics
 */
...
function analyzeBundleSize() {
  ...
  fileSizes.slice(0, 10).forEach(file => {
    ...
    console.log(`   ${indicator} ${file.name}: ${file.sizeKB}KB (${sizeMB}MB)`);
  });
  ...
  const largeFiles = fileSizes.filter(f => f.sizeKB > 200);
  ...
  if (totalSizeMB > 2) {
    ...
    console.log('   🟡 Bundle size is moderate (1-2MB)');
  ...
  if (jsFiles.length > 5) {
    console.log('   🟡 Many JS files detected - consider bundling optimization');
  }
  
  if (cssFiles.length > 3) {
    console.log('   🟡 Many CSS files detected - consider CSS optimization');
  }
}
...
function checkDependencies() {
  ...
  const foundProblematic = dependencies.filter(dep => 
    problematicDeps.some(problematic => dep.includes(problematic))
  );

  if (foundProblematic.length > 0) {
    console.log('\n   🟡 Potentially heavy dependencies found:');
    foundProblematic.forEach(dep => {
      console.log(`      - ${dep}`);
    });
    console.log('   Consider lighter alternatives or tree-shaking optimization.');
  } else {
    console.log('\n   🟢 No obviously problematic dependencies detected');
  }
}

function generateReport() {
  console.log('🚀 Zion Tech Group - Performance Analysis Report');
  ...
  console.log('\n📋 Summary:');
  console.log('   This analysis helps identify performance bottlenecks.');
  console.log('   Focus on optimizing the largest files first.');
  console.log('   Consider implementing lazy loading for non-critical components.');
  
  console.log('\n🎯 Next Steps:');
  console.log('   1. Optimize large files identified above');
  console.log('   2. Implement code splitting for better loading performance');
  console.log('   3. Use lazy loading for non-critical components');
