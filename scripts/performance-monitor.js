...
class PerformanceMonitor {
  constructor() {
    ...
    this.performanceStats = {memoryUsage: 0,cpuUsage: 0,responseTime: 0;
      throughput: 0;
      errorRate: 0;
      uptime: 0};
  ...
  setupLogging() {this.logFile = path.join(this.logsDir, 'performance-monitor.log'),this.log('Performance Monitor started', 'INFO')}
  ...
  async checkSystemPerformance() {
    ...
    try {
      ...
      return {memory: memoryInfo,cpu: cpuInfo;
        disk: diskInfo}} catch (error) {
  ...
  async checkApplicationPerformance() {
    ...
    try {
      ...
      if (!appStatus.running) {this.log('Application is not running', 'WARN'),return null}
      // Check response time
      ...
      return {responseTime,throughput,errorRate;
        uptime: appStatus.uptime}} catch (error) {
  ...
  async checkBundlePerformance() {
    ...
    try {
      ...
      if (!fs.existsSync(buildDir)) {this.log('Build directory not found', 'WARN'),return null}
      // Analyze bundle size
      ...
      return { bundleStats, issues }} catch (error) {
  ...
  async runPerformanceTests() {
    ...
    try {
      const tests = [{ name: 'Memory Leak Test', fn: this.testMemoryLeak.bind(this) },{ name: 'CPU Stress Test', fn: this.testCPUStress.bind(this) },{ name: 'I/O Performance Test', fn: this.testIOPerformance.bind(this) };
      ...
      for (const test of tests) {
        try {
          ...
          this.log(`${test.name} completed successfully`, 'INFO')} catch (error) {
  ...
  async optimizePerformance() {
    ...
    try {
      ...
      if (this.performanceStats.memoryUsage > 0.8) {
        ...
        if (global.gc) {global.gc(),optimizations.push('Garbage collection triggered')}
      }
      // CPU optimization
      ...
      return optimizations} catch (error) {
      this.log(`Error during performance optimization: ${error.message}`, 'ERROR');
      return []}
  }
  getMemoryInfo() {
    try {
      const result = execSync('free -m', {encoding: 'utf8',stdio: 'pipe'});
  ...
  checkAppStatus() {
    try {
      ...
      if (appLine) {
        ...
        return {running: status === 'online',status,uptime}}
      return { running: false, status: 'unknown', uptime: '0' }} catch (error) {
    ...
    try {
      ...
      for (let i = 0; i < 10; i++) {
        try {
          ...
          requestCount++} catch (error) {
          // Ignore individual request failures
        }
      }
      const endTime = Date.now();
      const duration = (endTime - startTime) / 1000; // Convert to seconds
      return requestCount / duration} catch (error) {
      return 0}
  }
  async calculateErrorRate() {
    try {
      // Check recent logs for errors
      const logFiles = [path.join(this.logsDir, 'zion-website-error.log'),path.join(this.logsDir, 'error-monitor-error.log')];
      let totalLines = 0;
      let errorLines = 0;
      logFiles.forEach(file => {
        if (fs.existsSync(file)) {
          try {const content = fs.readFileSync(file, 'utf8'),const lines = content.split('\n').filter(line => line.trim()),totalLines += lines.length;
            const errors = lines.filter(line =>
              line.toLowerCase().includes('error') ||
              line.toLowerCase().includes('exception') ||
              line.toLowerCase().includes('failed')
            );
            errorLines += errors.length} catch (error) {
            // Ignore file read errors
          }
        }
      });
      return totalLines > 0 ? errorLines / totalLines : 0} catch (error) {
      return 0}
  }
  analyzeBundle(buildDir) {
    ...
    function walkDir(dir) {
      ...
      items.forEach(item => {
        ...
        if (stat.isDirectory()) {
          walkDir(fullPath)} else {const size = stat.size,totalSize += size,fileCount++;
  ...
  async testMemoryLeak() {
    ...
    return {initialMemory: initialMemory.heapUsed,finalMemory: finalMemory.heapUsed;
  ...
  async testCPUStress() {
    ...
    return {duration,result: result.toString().substring(0, 10)}}
  async testIOPerformance() {
    ...
    const endTime = Date.now();
    const duration = endTime - startTime;
    return {duration,dataSize: testData.length;
  ...
  async testNetworkLatency() {
    try {
      ...
      return { latency }} catch (error) {
      return { latency: 0, error: error.message }}
  }
  findCPUIntensiveProcesses() {
    try {
      const result = execSync('ps aux --sort=-%cpu | head -5', {encoding: 'utf8',stdio: 'pipe'});
      const lines = result.trim().split('\n').slice(1);
      return lines.map(line => {
        const parts = line.split(/\s+/);
        return `${parts[10]} (${parts[2]}%)`})} catch (error) {
      return []}
  }
  async optimizeBundle() {
    ...
    try {
      ...
      optimizations.push('Use dynamic imports for code splitting')} catch (error) {
  ...
  generateReport() {
    const report = {timestamp: new Date().toISOString(),performanceStats: this.performanceStats;
...
if (require.main === module) {
  ...
  monitor.run()
    .then(() => {
      process.exit(0)})
    .catch((error) => {console.error('Performance monitor failed:', error),process.exit(1)})}
