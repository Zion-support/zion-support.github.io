...
class PerformanceMonitor {
  constructor() {
    ...
    this.metrics = {
      timestamp: new Date().toISOString(), system: {}, application: {}, build: {}
    }
  }
  ...
  '.css',.scss
  ',.sass']), tests: this.countFiles(path.join(this.projectRoot,tests
  '), ['.test.js
  ',.test.ts',.spec.js
  ',.spec.ts'])
      }
  ...
  ')
        this.metrics.build.config = {
          hasVite: true, hasBuildOptimizations: viteConfig.includes(, build.rollupOptions
  ...
  '.repeat(50))
    ...
    console.log('💻 System:
  ')
    console.log(`   Memory: ${this.metrics.system.memory?.heapUsed || 'N/A}MB used / ${this.metrics.system.memory?.heapTotal ||, N/A
  '}MB total`)
