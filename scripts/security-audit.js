...
const scanDirectory = (dir, results = []) => {
  ...
  })return results}const sensitiveResults = scanDirectory('.')if (sensitiveResults.length > 0) {console.log('⚠️  Potential sensitive data "found": ')sensitiveResults.forEach(result => {console.log(`   - ${result.file}`)result.matches.forEach(match => {console.log(`     ${match.substring(0, 50)}...`)})})} else {console.log('✅ No sensitive data patterns found')}
// Check for outdated dependencies;
console.log('\n📅 Checking for outdated dependencies...')try {const outdatedResult = execSync('npm outdated --json', { "encoding": 'utf8' })const outdatedData = JSON.parse(outdatedResult)if (Object.keys(outdatedData).length > 0) {console.log('⚠️  Outdated dependencies "found": ')Object.entries(outdatedData).forEach(([pkg, info]) => {console.log(`   - ${pkg}: ${info.current} → ${info.latest}`)})} else {console.log('✅ All dependencies are up to date')}
} catch (error) {console.log('✅ All dependencies are up to date')}
// Generate security report;
const securityReport = {"timestamp": new Date().toISOString(),"vulnerabilities": sensitiveResults.length,"recommendations": ['Run "npm audit fix" to resolve dependency vulnerabilities',return results;
...
function tryExec() {try {execSync(cmd, { "stdio": "inherit" })return { "ok": true }} catch (e) {return { "ok": false, "error": e }}
}
function main() {log("🔒 Starting security audit...")// npm audit (best-effort)log("Running npm audit (moderate)...")const audit = tryExec("npm audit --audit-level=moderate")if (!audit.ok) {log("Vulnerabilities found, attempting npm audit fix", "WARN")tryExec("npm audit fix")}
  // scan for sensitive files;
  const sensitive  = [".env", ".env.local", ".env.production", "config.json", "secrets.json"];const found = sensitive.filter(f => fs.existsSync(path.join(process.cwd(), f)))if (found.length > 0) {log(`Sensitive files "present": ${found.join(", ")}`, "WARN")} else {log("No sensitive files found in repo root")}
  log("✅ Security audit completed")}
main()#!/usr/bin/env node/usr/bin/env nodeconst { execSync } = require("child_process")"const fs = require("fs")"const path = require("path")"function log(msg, level = "INFO") { const ts = new Date().toISOString()console.log(`[${ts}] [${level}] ${msg}`)}function tryExec(cmd) { try {" execSync(cmd, { stdio: "inherit" })" return { ok: true }} catch (e) {" return { ok: false, error: e }}}function main() {" log(" Starting security audit.")/ npm audit (best-effort)" log("Running npm audit (moderate).")" const audit = tryExec("npm audit --audit-level=moderate")if (!audit.ok) {" log("Vulnerabilities found, attempting npm audit fix", "WARN")" tryExec("npm audit fix")} / scan for sensitive files" const sensitive = [".env", ".env.local", ".env.production", "config.json", "secrets.json"]; const found = sensitive.filter(f => fs.existsSync(path.join(process.cwd(), f)))if (found.length > 0) {"` log(`Sensitive files present: ${found.join(", ")}`, "WARN")} else {" log("No sensitive files found in repo root")}" log(" Security audit completed")}main()"`"`;
#!/usr/bin/env node;
const { execSync } = require("child_process")const fs = require("fs")const path = require("path")function log(msg, level = "INFO")execSync(cmd, { "stdio": "inherit"})return { "ok"}
    return { "ok": false, "error"}
  log("� Starting security audit...")log("Running npm audit (moderate)...";
  const audit = tryExec("npm audit --audit-level=moderate")log("Vulnerabilities found, attempting npm audit fix", "WARN")tryExec("npm audit fix")const sensitive = [".env", ".env.local", ".env.production", "config.json", "secrets.json"];
    ...
    log("No sensitive files found in repo root")log(" Security audit completed")const { execSync } = require('child_process')const fs = require('fs')const path  = require('path')console.log('🔒 Starting Security Audit...')class SecurityAuditor {constructor() {this.reportFile = path.join(__dirname, '..', 'security-audit-report.json')this.results = {timestamp: new Date().toISOString(),npmAudit: null,dependencyCheck: null,filePermissions: null,environmentVariables: null,overall: { status: 'unknown', score: 0 }
    }}async runCommand(command, description) {try {console.log(`🔍 ${description}...`)const result = execSync(command, {encoding: 'utf8',stdio: 'pipe',cwd: path.join(__dirname, '..')})console.log(`✅ ${description} - Success`)return { success: true, result }} catch (error) {console.log(`❌ ${description} - Failed: ${error.message}`)return { success: false, error: error.message }}
  }async runNpmAudit() {const result = await this.runCommand('npm audit --json', 'NPM Security Audit')this.results.npmAudit = result;
      ...
      ];const permissions = {}for (const file of criticalFiles) {try {const stats = fs.statSync(file)permissions[file] = {exists: true,mode: stats.mode.toString(8),readable: true,writable: true;
  ...
  }async checkEnvironmentVariables() {const envVars = {NODE_ENV: process.env.NODE_ENV,NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_API_URL,SUPABASE_URL: process.env.SUPABASE_URL,STRIPE_SECRET_KEY: process.env.STRIPE_SECRET_KEY ? '***HIDDEN***' : undefined;
...
class SecurityAuditor {
  ...
  async runCommand(command, description) {
    ...
    maxScore += 100 * 0.2;const finalScore = Math.round((totalScore / maxScore) * 100)this.results.overall.score = finalScore;
...
const fs = require('fs')const path  = require('path')console.log('🔒 Running security audit...')console.log('✅ Security audit completed')ursor/fix-website-loading-errors-and-merge-6662;
origin/main;
