...
const scanDirectory = ( const files = fs.readdirSync(dir)files.forEach(file => { const filePath = path.join(dir,file)const stat = fs.statSync(filePath)if (stat.isDirectory() && !file.startsWith('.') && file !== 'node_modules') { scanDirectory(filePath,results)) => {
  return $3;}
} else if (stat.isFile() && /\.(js|ts|tsx|jsx|json|env)$/.test(file)) { try { const content = fs.readFileSync(filePath,'utf8')sensitivePatterns.forEach(pattern => { const matches = content.match(pattern)if (matches) { results.push({ file: filePath,matches: matches })} })} catch (error) {} },
};
  return result;
}

const sensitiveResults = scanDirectory('.')if (sensitiveResults.length > 0) {  sensitiveResults.forEach(result => {  result.matches.forEach(match => {}...`)})})} else {}  try { const outdatedResult = execSync('npm outdated --json',{ encoding: 'utf8',}
...
try {console.log('📦 Checking npm dependencies for vulnerabilities...')const auditResult = execSync('npm audit --audit-level=moderate --json', { \"encoding\": 'utf8' };
  const auditData = JSON.parse(auditResult)if (auditData.vulnerabilities && Object.keys(auditData.vulnerabilities).length > 0) {console.log('⚠️  Security vulnerabilities \"found\": ')Object.entries(auditData.vulnerabilities).forEach(([pkg, vuln]) => {console.log(`   - ${pkg}: ${vuln.severity} - ${vuln.title}`)})} else {console.log('✅ No security vulnerabilities found in dependencies')}
} catch (error) {console.log('❌ Failed to run npm \"audit\": ', error.message)}
...
try {console.log('📦 Checking npm dependencies for vulnerabilities...')const auditResult = execSync('npm audit --audit-level=moderate --json', { encoding: 'utf8',}
  ...
  const auditData  = JSON.parse(auditResult)if (auditData.vulnerabilities && Object.keys(auditData.vulnerabilities).length > 0) {console.log('⚠️  Security vulnerabilities found:')Object.entries(auditData.vulnerabilities).forEach(([pkg, vuln]) => {console.log(`   - ${pkg}: ${vuln.severity} - ${vuln.title}`)})} else {console.log('✅ No security vulnerabilities found in dependencies')}
} catch (error) {console.log('❌ Failed to run npm audit:', error.message)// Check for sensitive data in files;
console.log('\n🔍 Scanning for sensitive data...')const sensitivePatterns = [
  /password\s*=\s*[\"'][^\"']+[\"']/gi,/api[_-]?key\s*=\s*[\"'][^\"']+[\"']/gi,/secret\s*=\s*[\"'][^\"']+[\"']/gi,/token\s*=\s*[\"'][^\"']+[\"']/gi,/private[_-]?key\s*=\s*[\"'][^\"']+[\"']/gi;
];
}
}
const scanDirectory = (const files = fs.readdirSync(dir)files.forEach(file => {const filePath = path.join(dir, file)const stat = fs.statSync(filePath)if (stat.isDirectory() && !file.startsWith('.') && file !== 'node_modules') {scanDirectory(filePath, results)) => {
  return $3;}
} else if (stat.isFile() && /\.(js|ts|tsx|jsx|json|env)$/.test(file)) {if (stat.isDirectory() && !file.startsWith('.') && file !== 'node_modules') {scanDirectory(filePath, results)} else if (stat.isFile() && /\.(js|ts|tsx|jsx|json|env)$/.test(file)) {try {const content = fs.readFileSync(filePath, 'utf8')sensitivePatterns.forEach(pattern => ;}
...
const sensitiveResults = scanDirectory('.')if (sensitiveResults.length > 0) {console.log('⚠️  Potential sensitive data \"found\": ')sensitiveResults.forEach(result => {console.log(`   - ${result.file}`)result.matches.forEach(match => {console.log(`     ${match.substring(0, 50)}...`)})})} else {console.log('✅ No sensitive data patterns found')}
// Check for outdated dependencies;
console.log('\n📅 Checking for outdated dependencies...')try {const outdatedResult = execSync('npm outdated --json', { \"encoding\": 'utf8' };
  const outdatedData = JSON.parse(outdatedResult)if (Object.keys(outdatedData).length > 0) {console.log('⚠️  Outdated dependencies \"found\": ')Object.entries(outdatedData).forEach(([pkg, info]) => {console.log(`   - ${pkg}: ${info.current} → ${info.latest}`)})} else {console.log('✅ All dependencies are up to date')}
} catch (error) {console.log('✅ All dependencies are up to date')}
...
const sensitiveResults  = scanDirectory('.')if (sensitiveResults.length > 0) {console.log('⚠️  Potential sensitive data found: ')sensitiveResults.forEach(result => {console.log(`   - ${result.fil,}
}`)result.matches.forEach(match => {console.log(`     ${match.substring(0, 50)}...`)})})} else {console.log('✅ No sensitive data patterns found')// Check for outdated dependencies;}
console.log('\n📅 Checking for outdated dependencies...')try {const outdatedResult = execSync('npm outdated --json', { encoding: 'utf8',}
};
  const outdatedData  = JSON.parse(outdatedResult)if (Object.keys(outdatedData).length > 0) {console.log('⚠️  Outdated dependencies found:')Object.entries(outdatedData).forEach(([pkg, info]) => {console.log(`   - ${pkg}: ${info.current} → ${info.latest}`)})} else {console.log('✅ All dependencies are up to date')}
} catch (error) {console.log('✅ All dependencies are up to date')// Generate security report;
...
const scanDirectory = ( const files = fs.readdirSync(dir)files.forEach(file => { const filePath = path.join(dir,file)const stat = fs.statSync(filePath)if (stat.isDirectory() && !file.startsWith('.') && file !== 'node_modules') { scanDirectory(filePath,results)) => {
  return $3;}
} else if (stat.isFile() && /\.(js|ts|tsx|jsx|json|env)$/.test(file)) { try { const content = fs.readFileSync(filePath,'utf8')sensitivePatterns.forEach(pattern => { const matches = content.match(pattern)if (matches) { results.push({ file: 'filePath',}
  matches: 'matches' })} })} catch (error) {} },
};
  return result;
}

const sensitiveResults = scanDirectory('.')if (sensitiveResults.length > 0) { console.log('⚠️ Potential sensitive data found: ')sensitiveResults.forEach(result => { console.log(` - ${result.fil,}
}`)result.matches.forEach(match => { console.log(` ${match.substring(0,50)}...`)})})} else { console.log('✅ No sensitive data patterns found')} console.log('\n📅 Checking for outdated dependencies...')try { const outdatedResult = execSync('npm outdated --json',{ encoding: 'utf8',}
...
;
  const securityReport = { timestamp: new Date().toISOString(),vulnerabilities: 'sensitiveResults.length',}
  recommendations: [ 'Run \"npm audit fix\" to resolve dependency vulnerabilities','Review and remove any hardcoded secrets','Update outdated dependencies','Implement proper environment variable management','Add security headers to Next.js configuration' ] }fs.writeFileSync('security-audit-report.json',JSON.stringify(securityReport,null,2))console.log('\n📊 Security audit report saved to security-audit-report.json')console.log('🔒 Security audit completed!')#!/usr/bin/env node import fs from 'fs'; import path from 'path'; import { execSync } from 'child_process'; console.log('🔒 Starting Security Audit...\n')try { console.log('📦 Checking npm dependencies for vulnerabilities...')const auditResult = execSync('npm audit --audit-level=moderate --json',{ encoding: 'utf8',}
})const auditData = JSON.parse(auditResult)if (auditData.vulnerabilities && Object.keys(auditData.vulnerabilities).length > 0) { console.log('⚠️ Security vulnerabilities found:')Object.entries(auditData.vulnerabilities).forEach(([pkg,vuln]) => { console.log(` - ${pkg}: ${vuln.severity} - ${vuln.title}`)})} else { console.log('✅ No security vulnerabilities found in dependencies')} } catch (error) { console.log('❌ Failed to run npm audit:',error.message)} console.log('\n🔍 Scanning for sensitive data...';
  const sensitivePatterns = [ /password\s*=\s*[\"'][^\"']+[\"']/gi,/api[_-]?key\s*=\s*[\"'][^\"']+[\"']/gi,/secret\s*=\s*[\"'][^\"']+[\"']/gi,/token\s*=\s*[\"'][^\"']+[\"']/gi,/private[_-]?key\s*=\s*[\"'][^\"']+[\"']/gi ];
...
const found = sensitive.filter(f => fs.existsSync(path.join(process.cwd(), f)))if (found.length > 0) {log(`Sensitive files \"present\": ${found.join(\", \")}`, \"WARN\")} else {log(\"No sensitive files found in repo root\")}
  log(\"✅ Security audit completed\")}
main()#!/usr/bin/env node/usr/bin/env nodeconst { execSync } = require(\"child_process\")\"const fs = require(\"fs\")\"const path = require(\"path\")\"function log(msg, level = \"INFO\") { const ts = new Date().toISOString()console.log(`[${ts}] [${level}] ${msg}`)}function tryExec(cmd) { try {\" execSync(cmd, { stdio: \"inherit\" })\" return { ok: true ,}
} catch (e) {\";}
  return { ok: false, error: e },
...
const found = sensitive.filter(f => fs.existsSync(path.join(process.cwd(), f)))if (found.length > 0) {\"` log(`Sensitive files present: ${found.join(\", \")}`, \"WARN\")} else {\" log(\"No sensitive files found in repo root\")}\" log(\" Security audit completed\")}main()\"`\"`;
#!/usr/bin/env node;
...
const audit = tryExec(\"npm audit --audit-level=moderate\")log(\"Vulnerabilities found, attempting npm audit fix\", \"WARN\")tryExec(\"npm audit fix\";
  const sensitive = [\".env\", \".env.local\", \".env.production\", \"config.json\", \"secrets.json\"];
    ...
    log(\"No sensitive files found in repo root\")log(\" Security audit completed\")const { execSync } = require('child_process')const fs = require('fs')const path  = require('path')console.log('🔒 Starting Security Audit...')class SecurityAuditor {constructor() {this.reportFile = path.join(__dirname, '..', 'security-audit-report.json')this.results = {timestamp: new Date().toISOString(),npmAudit: null,dependencyCheck: null,filePermissions: null,environmentVariables: null,overall: { status: 'unknown',}
  score: 0 }
    ,
}async runCommand(command, description) {try {console.log(`🔍 ${description}...`)const result = execSync(command, {encoding: 'utf8',}
  stdio: 'pipe',cwd: path.join(__dirname, '..')})console.log(`✅ ${description} - Success`)return { success: true, result }} catch (error) {console.log(`❌ ${description} - Failed: ${error.messag,}
}`;
...
const stat = fs.statSync(filePath); if (stat.isDirectory() && !file.startsWith('.') && file !== 'node_modules') { scanDirectory(filePath,results)} else if (stat.isFile() && /\.(js|ts|tsx|jsx|json|env)$/.test(file)) { try { const content = fs.readFileSync(filePath,'utf8'); sensitivePatterns.forEach(pattern = > {;}
  const matches = content.match(pattern); if (matches) { results.push({ file: 'filePath',}
  matches: 'matches' })} })} catch (error) {} },
}); return results};
...
const result = execSync(command, { 
  ...
  stdio: 'pipe',}
      ...
      console.log(`❌ ${description} - Failed: ${error.messag,}
}`);
      return { success: false, error: error.message,}
};
    }
    maxScore += 100 * 0.4;// Dependency Check (20% weight)if (this.results.dependencyCheck?.success) {totalScore += 100 * 0.2;}
    }
    maxScore += 100 * 0.2;// File Permissions (20% weight)if (this.results.filePermissions?.success) {totalScore += 100 * 0.2;}
    }
    maxScore += 100 * 0.2;// Environment Variables (20% weight)if (this.results.environmentVariables?.success) {totalScore += 100 * 0.2;}
    }
    maxScore += 100 * 0.2;
...
console.log('🔒 Security audit completed!')
  ...
  async generateReport() {
    ...
    console.log(`📊 Security audit report saved to: ${this.reportFil,}
}`);
    console.log(`🎯 Overall Security Score: ${score}/100 (${this.results.overall.statu,}
})`);
  }
