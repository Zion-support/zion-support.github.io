...
class AutomationManager {
  constructor() {
    this.automationProcesses = [console-error-fixer',
      'link-checker',continuous-improvement',
      'daily-build-test',security-audit',
      'dependency-updates',performance-monitor',
      'quality-checks',link-integrity',
      'front-maximizer',sitemap-runner'
    ];
  }
...
📋 Examples:
  ...
  async getStatus() {
    try {
      ...
      console.log(Name'.padEnd(25) +
        'Status'.padEnd(10) +Memory'.padEnd(10) +
        'CPU'.padEnd(8) +Uptime'.padEnd(15) +
        'Restarts'
      );
      console.log('─'.repeat(80));
      ...
      automationProcesses.forEach(proc => {
        ...
        console.log(
          proc.name.padEnd(25) +${statusIcon} ${proc.pm2_env.status}'.padEnd(10) +
          memory.padEnd(10) + `${proc.monit.cpu}%`.padEnd(8) +
          uptime.padEnd(15) +
      ...
      console.log('');console.log(`📈 Summary: ${onlineCount}/${totalCount} processes online`);
  ...
  async stopAll() {
    ...
    try {
      this.automationProcesses.forEach(processName => {
        try {execSync(`pm2 stop ${processName}`, { stdio: 'pipe' });
        } catch (error) {
  ...
  async restartProcess(processName) {
    if (!this.automationProcesses.includes(processName)) {console.error(`❌ Invalid process name: ${processName}`);
      console.log('Valid processes:', this.automationProcesses.join(', '));
      return;
    }
    console.log(`🔄 Restarting ${processName}...`);
    try {execSync(`pm2 restart ${processName}`, { stdio: 'inherit' });console.log(`✅ ${processName} restarted`);
    } catch (error) {console.error(`❌ Failed to restart ${processName}:`, error.message);
    }
  }
  ...
  async deleteAll() {
    ...
    try {
      this.automationProcesses.forEach(processName => {
        try {execSync(`pm2 delete ${processName}`, { stdio: 'pipe' });
        } catch (error) {
  ...
  async showLogs(processName) {
    ...
    try {execSync(`pm2 logs ${processName} --lines 50`, { stdio: 'inherit' });
    } catch (error) {console.error(`❌ Failed to show logs for ${processName}:`, error.message);
    }
  }
  ...
  async checkHealth() {
    ...
    try {
      ...
      failedProcesses.forEach(proc => {console.log(`  🔴 ${proc.name}: ${proc.pm2_env.status} (restarts: ${proc.pm2_env.restart_time})`);
      });
  ...
  async autoFix() {
    ...
    try {
      ...
      failedProcesses.forEach(proc => {
        try {execSync(`pm2 restart ${proc.name}`, { stdio: 'pipe' });console.log(`✅ Restarted ${proc.name}`);
        } catch (error) {console.error(`❌ Failed to restart ${proc.name}:`, error.message);
        }
      });
  ...
  formatUptime(uptime) {
    ...
    const minutes = Math.floor((seconds % 3600) / 60);return `${hours}h ${minutes}m`;
  }
}
