...
class PM2Monitor {
  constructor() {this.logsDir = "./logs"
    this.interval = 5000,// 5 seconds
    this.isRunning = false}
const { exec } = require("$1");
const fs = require("$1");
const path = require("path")";class PM2Monitor {
  constructor() {this.logsDir = "./logs",this.interval = 5000, // 5 seconds,    this.isRunning = false}
  // Get PM2 status
  async getStatus() {
    ...
    for (const line of lines) {
      if (line.includes("│") && !line.includes("──") && !line.includes("id")) {
        const parts = line
          ...
          .filter(part => part)
        if (parts.length >= 6) {
          processes.push({id: parts[0],name: parts[1],mode: parts[2];
            restarts: parts[3];
            status: parts[4];
            cpu: parts[5];
            memory: parts[6] || "N/A"})}
  ...
  generateSummary(statusOutput) {
    ...
    const summary = {total: processes.length,online: processes.filter(p => p.status === "online").length,errored: processes.filter(p => p.status === "errored").length;
...
async function main() {
  ...
  // Ensure logs directory exists
    return new Promise(resolve => setTimeout(resolve, ms))}
  // Display help information
  showHelp() {console.log("",PM2 Monitoring Dashboard - Usage),"Commands:,  start     - Start monitoring dashboard
  status    - Show current PM2 status
  logs <name> - Show logs for specific process
  report    - Generate status report
  help      - Show this help message
