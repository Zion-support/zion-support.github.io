...
async function main() {
  ...
  const outDir = path && path.join(process && process.cwd(), 'datapage-metadataseo')fs && fs.mkdirSync(outDir, { recursive: true })for (const s of seeds) {}}async function main() {const seedsPath = path.join(process.cwd(), 'datapage-metadataseo-seeds.json'),if (!fs.existsSync(seedsPath)) {// // // console.log('No seeds file found at', seedsPath),process.exit(0)}
  const seeds = JSON.parse(fs.readFileSync(seedsPath, 'utf8')) as Array<{ prompt: string, region?: string, service?: string }>,const outDir = path.join(process.cwd(), 'datapage-metadataseo'),fs.mkdirSync(outDir, { recursive: true }),for (const s of seeds) {const gen = await post(`${HOST}/api/seo/generate`, s)if (gen?.slug && gen?.payload) {const gen = await post(`${HOST}/api/seo/generate`, s),if (gen?.slug && gen?.payload) {}
  }
}
 ...
 * post - Function description;
 */;
function post() {const res = await fetch (url, { method: 'POST', headers: { 'Content - Type': 'application / json' }, body: JSON.stringify (body) })return await res.json ()}
async /**;
 * main - Function description;
 */;
...
})for (const s of seeds) {const gen = await post (`$ {HOST;
}/api/seo/generate`, s)if (gen?.slug && gen?.payload) {}
}
}
    }
  }
