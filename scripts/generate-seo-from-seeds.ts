...
async function post() {const res = await fetch(url, { method: 'POST',}
  headers: { 'Content-Type': 'application/json',}
}, body: JSON.stringify(body),
};
  return await res.json()}}
 ...
 * post - Function description;
 */;
function post() {const res = await fetch (url, { method: 'POST',}
  headers: { 'Content - Type': 'application / json',}
}, body: JSON.stringify (body),
};
  return await res.json ()}
...
const HOST = process.env.SELF_HOST || 'http://localhost:3000',async function post() {const res = await fetch(url, { method: 'POST',}
  headers: { 'Content-Type':'application/json',}
}, body: JSON.stringify(body),
});
