...
const TEMP_DATA_DIR = path.resolve(
...
const JEST_RESULTS_FILE = path.resolve(
  __dirname,
...
function main() {
  ...
  if (!coverageSummaryPath) {
    ...
    writeTestStatus({
      ...
      total: 0,
      coverage: 0,
      error: 'Coverage summary path missing',
    });
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      `Error: Could not read or parse coverage summary at ${coverageSummaryPath}`,
      err
    );
    writeTestStatus({
      passed: 0,
  ...
  const coveragePct =
    coverageData.total && coverageData.total.lines
  ...
  try {
    if (fs.existsSync(JEST_RESULTS_FILE)) {
      const jestOutput = JSON.parse(
        fs.readFileSync(JEST_RESULTS_FILE, 'utf-8')
      );
      jestResultsData = {
  ...
  if (coveragePct < COVERAGE_THRESHOLD) {
    console.error(
      `Error: Coverage (${coveragePct}%) is below threshold of ${COVERAGE_THRESHOLD}%.`
    );
    process.exit(1);
  } else {
