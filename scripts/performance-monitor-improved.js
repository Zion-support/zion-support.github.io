...
class ImprovedPerformanceMonitor {
  constructor() {
    ...
    this.metrics = {
      timestamp: new Date().toISOString(), system: {}, application: {}, build: {}, errors: []
    }
  ...
  async monitor() {
    try {
      ...
      this.metrics.errors.push({
        timestamp: new Date().toISOString(), error: error.message, stack: error.stack
      })
    }
  }
  ...
  async collectSystemMetrics() {
    try {
      ...
      this.metrics.errors.push({
        type: 'system_metrics', error: error.message
      })
    }
  }
  ...
  async collectApplicationMetrics() {
    try {
      ...
      this.metrics.application = {
        buildExists, packageName: packageInfo.name || 'unknown', packageVersion: packageInfo.version || 'unknown', dependencies: Object.keys(packageInfo.dependencies || {}).length, devDependencies: Object.keys(packageInfo.devDependencies || {}).length
      }

    } catch (error) {
      console.error('Error collecting application metrics:', error.message)
      this.metrics.errors.push({
        type: 'application_metrics', error: error.message
      })
    }
  }
  ...
  async collectBuildMetrics() {
    try {
      ...
      if (fs.existsSync(buildPath)) {
        ...
        this.metrics.build = {
          exists: true, size: buildSize, lastModified: fs.statSync(buildPath).mtime
        }
      } else {
        this.metrics.build = {
      ...
      this.metrics.errors.push({
        type: 'build_metrics', error: error.message
      })
    }
  }
  ...
  async saveMetrics() {
    try {
      ...
      this.metrics.errors.push({
        type: 'save_metrics', error: error.message
      })
    }
  }
