...
class AutomatedTesting {
  ...
  async runAllTests() {
    ...
    const tests = [{ name: "Type Check", command: "NODE_OPTIONS="--max-old-space-size=8192" npm run type-check" },{ name: "Lint Check", command: "npm run lint" },{ name: "Build Test", command: "NODE_OPTIONS="--max-old-space-size=8192" npm run build" }]
    for (const test of tests) {
      try {
        console.log(`Running ${test.name}...`)
        execSync(test.command, { cwd: this.projectRoot, stdio: "inherit" })
        this.testResults.push({ name: test.name, status: "PASSED" })
        console.log(`✅ ${test.name} passed`)} catch (error) {
