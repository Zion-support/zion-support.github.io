...
let cleanedContent = content;
  ...
  const result  = cleanConsoleLogs(content, filePath)if (result) {fs.writeFileSync(filePath, result.content, 'utf8')cleanedFiles++;'
      ...
      console.log(`✅ Cleaned ${filePath} (removed ${result.removed} console statements)`;`      )}totalFiles++;
  } catch (error) {console.error(`❌ Error processing ${filePath}:`, error.message)}`}function shouldExcludeFile() {return excludeDirs.some(excludeDir = > filePath.includes(excludeDir))}async function main() {console.log('🧹 Starting console.log cleanup...\n')for (const pattern of directories) {const file of files) {if (!shouldExcludeFile(file)) {processFile(file)}
    }
