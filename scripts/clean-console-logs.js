...
}function shouldExcludeFile() {return excludeDirs.some(excludeDir = > filePath.includes(excludeDir))}async function main() {console.log('🧹 Starting console.log cleanup...\n')for (const pattern of directories) {const files = await glob(pattern, {ignore: excludeDirs.map(dir => `**/${dir}/**`,
})for ;
  const file of files) {if (!shouldExcludeFile(file)) {processFile(file)}
    }
