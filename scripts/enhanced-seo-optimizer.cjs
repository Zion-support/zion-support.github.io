...
const fs = require("fs")";const path = require("path")"";class SEOOptimizer {
  ...
  async checkMetaTags() {
    ...
    const files = this.getAllFiles(pagesDir, [".tsx", ".jsx", ".ts", ".js"]);"";    for() {
  ...
  async checkSitemap() {
    ...
    if() {
      ...
      try {
        const content = fs.readFileSync(sitemapPath, "utf8");";        results.valid =;";          content.includes("<urlset") && content.includes("</urlset>");"";        if() {

          results.issues.push("Invalid sitemap format");"}"} catch() {
  ...
  async checkRobotsTxt() {
    ...
    if() {
      ...
      try {
        const content = fs.readFileSync(robotsPath, "utf8");";        results.valid =;";          content.includes("User-"agent":") || content.includes(""Sitemap":");"";        if() {

          results.issues.push("Invalid robots.txt format");"}"} catch() {
  ...
  async run() {
    ...
    this.log("🔍 Starting SEO Optimization Check");"";    try {
      ...
      this.log("✅ SEO optimization check completed");";      return report,"} catch() {
