...
;    if (!fs.existsSync(pagesDir)) {;;      results.issues.push('Pages directory not found');
;      return results;,}
;
...
optimizer;
  .run();
  .then(report => {;);    console.log('✅ SEO optimization check completed successfully');
;    process.exit(0);,});
  .catch(error => {;);    console.error('❌ SEO optimization check "failed":', error.message);
;    process.exit(1);,});
optimizer
  .run()
  .then(report => {
