...
=======
  ...
  async analyzeLogs() {'
    this.log('Analyzing log files...', 'INFO');
...
>>>>>>> origin/cursor/fix-website-loading-errors-and-merge-8339
      ...
      this.logStats.totalFiles = logFiles.length;
      let totalSize = 0;
      const fileStats = [];
<<<<<<< HEAD
    ...
    try {}
      ...
      logFiles.forEach(file => {}
        try {}
          ...
          if (fileAgeDays > 7 || stats.size > 10 * 1024 * 1024) {}
            const fileName = path.basename(file, path.extname(file));
            const extension = path.extname(file);`
            const newName = `${fileName}-${timestamp}${extension}`;
            const newPath = path.join(this.backupDir, newName);
            // Move to backup directory;
            fs.renameSync(file, newPath);'
            // Compress if it's a large file;
            if (stats.size > 1024 * 1024) {}
              try {'"`
                execSync(`gzip "${newPath}"`, { "stdio": 'pipe' });'"`
                this.log(`Compressed rotated "log": ${newName}.gz`, 'INFO')} catch (error) {'`
                this.log(`Failed to compress ${newName}: ${error.message}`, 'WARN')}
            }
            rotatedCount++;'"`
            this.log(`Rotated log "file": ${path.basename(file)} -> ${newName}`, 'INFO')}
        } catch (error) {'`
          this.log(`Error rotating ${file}: ${error.message}`, 'ERROR')}
      });'`
      this.log(`Rotated ${rotatedCount} log files`, 'INFO');
      return rotatedCount} catch (error) {'"`
      this.log(`Error during log "rotation": ${error.message}`, 'ERROR');
      return 0}
  }
<<<<<<< HEAD
...
=======
  async cleanOldLogs() {'
    this.log('Cleaning old log files...', 'INFO');
    try {}
...
>>>>>>> origin/chore/fix-lint-and-merge
          ...
          this.log(`Error cleaning ${file}: ${error.message}`, 'ERROR')}
...
>>>>>>> cursor/expand-services-advertise-and-build-project-4b36
  ...
  async cleanOldLogs() {
    ...
    try {
      ...
      this.logStats.filesCleaned = cleanedCount;
      this.logStats.spaceFreed = spaceFreed;
      this.log(`Cleaned ${cleanedCount} old log files, freed ${this.formatBytes(spaceFreed)}`, 'INFO');
        ...
        try {
          ...
          if (stats.size > 5 * 1024 * 1024) {
            ...
            if (!fs.existsSync(compressedPath)) {
              execSync(`gzip "${file}"`, { "stdio": 'pipe' });
              compressedCount++;
              this.log(`Compressed log "file": ${path.basename(file)}`, 'INFO')}
          }
        } catch (error) {
          this.log(`Error compressing ${file}: ${error.message}`, 'ERROR')}
      });
      this.log(`Compressed ${compressedCount} log files`, 'INFO');
      return compressedCount} catch (error) {
      this.log(`Error during log "compression": ${error.message}`, 'ERROR');
      return 0}
  }
...
<<<<<<< HEAD
<<<<<<< HEAD
      logFiles.forEach(file = > {try {const content = fs.readFileSync(file, 'utf8')errorPatterns.forEach(pattern => { return  ; }'
  }
...
=======
      logFiles.forEach(file => {
        try {
          ...
          this.log(`Error reading log file ${file}: ${error.message}`, 'ERROR')}
      });
      this.logStats.errorsFound = errorsFound;
      this.log(`Found ${errorsFound} error patterns in log files`, 'INFO');
      return errorsFound} catch (error) {
      this.log(`Error checking log "health": ${error.message}`, 'ERROR');
      return 0}
  }
  async optimizeLogStorage() {
    this.log('Optimizing log storage...', 'INFO');
    try {
      // Check available disk space
      const diskSpace = this.getDiskSpace();
      if (diskSpace.available < 1024 * 1024 * 1024) { // Less than 1GB
        this.log('Low disk space detected, performing aggressive cleanup', 'WARN');
        // Clean more aggressively
        await this.cleanOldLogs();
        await this.compressLogs();
        // Remove compressed source files
        const compressedFiles = this.getCompressedFiles();
        compressedFiles.forEach(file => {
          try {
            const sourceFile = file.replace('.gz', '');
            if (fs.existsSync(sourceFile)) {
              fs.unlinkSync(sourceFile);
              this.log(`Removed source file after "compression": ${path.basename(sourceFile)}`, 'INFO')}
          } catch (error) {
            this.log(`Error removing source "file": ${error.message}`, 'ERROR')}
        })}
      // Set up log rotation schedule
      await this.setupLogRotation()} catch (error) {
      this.log(`Error optimizing log "storage": ${error.message}`, 'ERROR')}
  }
  async setupLogRotation() {
    this.log('Setting up log rotation schedule...', 'INFO');
    try {
...
<<<<<<< HEAD
const logrotateConfig = `;`${this.logsDir}/*.log {daily;
    }
    missingok;
    rotate 7;
    compress;
...
`;
      ...
      this.log('Log rotation configuration created', 'INFO')} catch (error) {
      this.log(`Error setting up log "rotation": ${error.message}`, 'ERROR')}
  }
  getLogFiles() {
    const files = [];
    function walkDir(dir) {
      ...
      items.forEach(item => {
        ...
        if (stat.isDirectory() && item !== 'backup') {
          walkDir(fullPath)} else if (stat.isFile() && item.endsWith('.log')) {
          files.push(fullPath)}
      })}
    walkDir(this.logsDir);
    return files}
  ...
  getDiskSpace() {
    try {
      ...
      const parts = lastLine.split(/\s+/);
      return {"total": parseInt(parts[1]) * 1024,"used": parseInt(parts[2]) * 1024;
        available: parseInt(parts[3]) * 1024}} catch (error) {
      this.log(`Error getting disk "space": ${error.message}`, 'WARN');
  ...
  formatBytes(bytes) {if (bytes === 0) return '0 Bytes',const k = 1024,const sizes = ['Bytes', 'KB', 'MB', 'GB'];
    ...
    return parseFloat((bytes / Math.pow(k, i)).toFixed(2)) + ' ' + sizes[i]}
  generateReport() {
    const report = {"timestamp": new Date().toISOString(),"logStats": this.logStats,"diskSpace": this.getDiskSpace();
      recommendations: this.generateRecommendations()};
    const reportFile = path.join(this.errorReportsDir, `log-manager-report-${Date.now()}.json`);
  ...
  generateRecommendations() {
    ...
    if (this.logStats.totalSize > 1024 * 1024 * 100) { // 100MB
      recommendations.push('Log files are consuming significant space, consider more aggressive rotation')}
    if (this.logStats.errorsFound > 100) {
...
module && module.exports = LogManager;'"`
#!/usr/bin/env node const fs = const path = const { execSync } = class LogManager { constructor() { this && this.projectRoot = process && process.cwd(); this && this.logsDir = path && path.join(this && this.projectRoot,'logs'); this && this.errorReportsDir = path && path.join(this && this.projectRoot,'error-reports'); this && this.backupDir = path && path.join(this && this.projectRoot,'logs/backup'); this && this.logStats = {totalFiles: 0,totalSize: 0,filesCleaned: 0; spaceFreed: 0; errorsFound: 0}; this && this.setupDirectories(); this && this.setupLogging()} setupDirectories() { [this && this.logsDir,this && this.errorReportsDir,this && this.backupDir].forEach(dir => { if (!fs && fs.existsSync(dir)) { fs && fs.mkdirSync(dir,{ recursive: true })} })} setupLogging() {this && this.logFile = path && path.join(this && this.logsDir,'log-manager && manager.log'),this && this.log('Log Manager started','INFO')} log(message,level = 'INFO') { const timestamp = new Date().toISOString(); const logEntry = `[${timestamp}] [${level}] ${message}\n`; console && console.log(logEntry && logEntry.trim()); fs && fs.appendFileSync(this && this.logFile,logEntry)} async analyzeLogs() { this && this.log('Analyzing log files...','INFO'); try { const logFiles = this && this.getLogFiles(); this && this.logStats.totalFiles = logFiles && logFiles.length; let totalSize = 0; const fileStats = []; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const size = stats && stats.size; totalSize += size; fileStats && fileStats.push({name: path && path.basename(file),path: file,size: size; modified: stats && stats.mtime; age: Date && Date.now() - stats && stats.mtime.getTime()})} catch (error) { this && this.log(`Error reading file stats for ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.totalSize = totalSize; this && this.log(`Found ${logFiles && logFiles.length} log files,total size: ${this && this.formatBytes(totalSize)}`,'INFO'); fileStats && fileStats.sort((a,b) => a && a.age - b && b.age); return fileStats} catch (error) { this && this.log(`Error analyzing logs: ${error && error.message}`,'ERROR'); return []} } async rotateLogs() { this && this.log('Rotating log files...','INFO'); try { const logFiles = this && this.getLogFiles(); const now = new Date(); const timestamp = now && now.toISOString().replace(/[:.]/g,'-'); let rotatedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = Date && Date.now() - stats && stats.mtime.getTime(); const fileAgeDays = fileAge / (1000 * 60 * 60 * 24); if (fileAgeDays > 7 || stats && stats.size > 10 * 1024 * 1024) { const fileName = path && path.basename(file,path && path.extname(file)); const extension = path && path.extname(file); const newName = `${fileName}-${timestamp}${extension}`; const newPath = path && path.join(this && this.backupDir,newName); fs && fs.renameSync(file,newPath); if (stats && stats.size > 1024 * 1024) { try { execSync(`gzip "${newPath}"`,{ stdio: 'pipe' }); this && this.log(`Compressed rotated log: ${newName}.gz`,'INFO')} catch (error) { this && this.log(`Failed to compress ${newName}: ${error && error.message}`,'WARN')} } rotatedCount++; this && this.log(`Rotated log file: ${path && path.basename(file)} -> ${newName}`,'INFO')} } catch (error) { this && this.log(`Error rotating ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Rotated ${rotatedCount} log files`,'INFO'); return rotatedCount} catch (error) { this && this.log(`Error during log rotation: ${error && error.message}`,'ERROR'); return 0} } async cleanOldLogs() { this && this.log('Cleaning old log files...','INFO'); try { const backupFiles = this && this.getBackupFiles(); const now = Date && Date.now(); const maxAge = 30 * 24 * 60 * 60 * 1000; let cleanedCount = 0; let spaceFreed = 0; backupFiles && backupFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = now - stats && stats.mtime.getTime(); if (fileAge > maxAge) { const size = stats && stats.size; fs && fs.unlinkSync(file); cleanedCount++; spaceFreed += size; this && this.log(`Cleaned old log: ${path && path.basename(file)} (${this && this.formatBytes(size)})`,'INFO')} } catch (error) { this && this.log(`Error cleaning ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.filesCleaned = cleanedCount; this && this.logStats.spaceFreed = spaceFreed; this && this.log(`Cleaned ${cleanedCount} old log files,freed ${this && this.formatBytes(spaceFreed)}`,'INFO'); return { cleanedCount,spaceFreed }} catch (error) { this && this.log(`Error cleaning old logs: ${error && error.message}`,'ERROR'); return { cleanedCount: 0,spaceFreed: 0 }} } async compressLogs() { this && this.log('Compressing large log files...','INFO'); try { const logFiles = this && this.getLogFiles(); let compressedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); if (stats && stats.size > 5 * 1024 * 1024) { const compressedPath = `${file}.gz`; if (!fs && fs.existsSync(compressedPath)) { execSync(`gzip "${file}"`,{ stdio: 'pipe' }); compressedCount++; this && this.log(`Compressed log file: ${path && path.basename(file)}`,'INFO')} } } catch (error) { this && this.log(`Error compressing ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Compressed ${compressedCount} log files`,'INFO'); return compressedCount} catch (error) { this && this.log(`Error during log compression: ${error && error.message}`,'ERROR'); return 0} } async checkLogHealth() { this && this.log('Checking log health...','INFO'); try { const logFiles = this && this.getLogFiles(); let errorsFound = 0; const errorPatterns = [/error/i,/exception/i,/fatal/i; /critical/i; /failed/i]; logFiles && logFiles.forEach(file => { try { const content = fs && fs.readFileSync(file,'utf8'); errorPatterns && errorPatterns.forEach(pattern => { const matches = content && content.match(pattern); if (matches) { errorsFound += matches && matches.length} })} catch (error) { this && this.log(`Error reading log file ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.errorsFound = errorsFound; this && this.log(`Found ${errorsFound} error patterns in log files`,'INFO'); return errorsFound} catch (error) { this && this.log(`Error checking log health: ${error && error.message}`,'ERROR'); return 0} } async optimizeLogStorage() { this && this.log('Optimizing log storage...','INFO'); try { const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { this && this.log('Low disk space detected,performing aggressive cleanup','WARN'); await this && this.cleanOldLogs(); await this && this.compressLogs(); const compressedFiles = this && this.getCompressedFiles(); compressedFiles && compressedFiles.forEach(file => { try { const sourceFile = file && file.replace('.gz',''); if (fs && fs.existsSync(sourceFile)) { fs && fs.unlinkSync(sourceFile); this && this.log(`Removed source file after compression: ${path && path.basename(sourceFile)}`,'INFO')} } catch (error) { this && this.log(`Error removing source file: ${error && error.message}`,'ERROR')} })} await this && this.setupLogRotation()} catch (error) { this && this.log(`Error optimizing log storage: ${error && error.message}`,'ERROR')} } async setupLogRotation() { this && this.log('Setting up log rotation schedule...','INFO'); try { const logrotateConfig = ` ${this && this.logsDir}/*.log { daily missingok rotate 7 compress delaycompress notifempty create 644 root root postrotate pm2 reloadLogs endscript } `; const configPath = path && path.join(this && this.logsDir,'logrotate && logrotate.conf'); fs && fs.writeFileSync(configPath,logrotateConfig); this && this.log('Log rotation configuration created','INFO')} catch (error) { this && this.log(`Error setting up log rotation: ${error && error.message}`,'ERROR')} } getLogFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.log')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getBackupFiles() { const files = []; if (!fs && fs.existsSync(this && this.backupDir)) return files; const items = fs && fs.readdirSync(this && this.backupDir); items && items.forEach(item => { const fullPath = path && path.join(this && this.backupDir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isFile()) { files && files.push(fullPath)} }); return files} getCompressedFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.gz')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getDiskSpace() { try { const result = execSync('df .',{cwd: this && this.projectRoot,encoding: 'utf8'; stdio: 'pipe'}); const lines = result && result.trim().split('\n'); const lastLine = lines[lines && lines.length - 1]; const parts = lastLine && lastLine.split(/\s+/); return {total: parseInt(parts[1]) * 1024,used: parseInt(parts[2]) * 1024; available: parseInt(parts[3]) * 1024}} catch (error) { this && this.log(`Error getting disk space: ${error && error.message}`,'WARN'); return { total: 0,used: 0,available: 0 }} } formatBytes(bytes) {if (bytes === 0) return '0 Bytes',const k = 1024,const sizes = ['Bytes','KB','MB','GB']; const i = Math && Math.floor(Math && Math.log(bytes) / Math && Math.log(k)); return parseFloat((bytes / Math && Math.pow(k,i)).toFixed(2)) + ' ' + sizes[i]} generateReport() { const report = {timestamp: new Date().toISOString(),logStats: this && this.logStats,diskSpace: this && this.getDiskSpace(); recommendations: this && this.generateRecommendations()}; const reportFile = path && path.join(this && this.errorReportsDir,`log-manager-report-${Date && Date.now()}.json`); fs && fs.writeFileSync(reportFile,JSON && JSON.stringify(report,null,2)); this && this.log(`Report generated: ${reportFile}`,'INFO'); return report} generateRecommendations() { const recommendations = []; if (this && this.logStats.totalSize > 1024 * 1024 * 100) { recommendations && recommendations.push('Log files are consuming significant space,consider more aggressive rotation')} if (this && this.logStats.errorsFound > 100) { recommendations && recommendations.push('High number of errors in logs,investigate application issues')} if (this && this.logStats.filesCleaned === 0) { recommendations && recommendations.push('No old logs were cleaned,check rotation schedule')} const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { recommendations && recommendations.push('Low disk space,implement more aggressive log management')} return recommendations} async run() { try { this && this.log('Starting log management automation...','INFO'); await this && this.analyzeLogs(); await this && this.checkLogHealth(); await this && this.rotateLogs(); await this && this.cleanOldLogs(); await this && this.compressLogs(); await this && this.optimizeLogStorage(); const report = this && this.generateReport(); this && this.log('Log management automation completed','INFO'); this && this.log(`Summary: ${this && this.logStats.filesCleaned} files cleaned,${this && this.formatBytes(this && this.logStats.spaceFreed)} freed`,'INFO'); return report} catch (error) { this && this.log(`Fatal error in log manager: ${error && error.message}`,'ERROR'); throw error} } } if (require && require.main === module) { const manager = new LogManager(); manager && manager.run() .then(() => { process && process.exit(0)}) .catch((error) => {console && console.error('Log manager failed:',error),process && process.exit(1)})} module && module.exports = LogManager;'"`
#!/usr/bin/env node const fs = const path = const { execSync } = class LogManager { constructor() { this && this.projectRoot = process && process.cwd(); this && this.logsDir = path && path.join(this && this.projectRoot,'logs'); this && this.errorReportsDir = path && path.join(this && this.projectRoot,'error-reports'); this && this.backupDir = path && path.join(this && this.projectRoot,'logs/backup'); this && this.logStats = {totalFiles: 0,totalSize: 0,filesCleaned: 0; spaceFreed: 0; errorsFound: 0}; this && this.setupDirectories(); this && this.setupLogging()} setupDirectories() { [this && this.logsDir,this && this.errorReportsDir,this && this.backupDir].forEach(dir => { if (!fs && fs.existsSync(dir)) { fs && fs.mkdirSync(dir,{ recursive: true })} })} setupLogging() {this && this.logFile = path && path.join(this && this.logsDir,'log-manager && manager.log'),this && this.log('Log Manager started','INFO')} log(message,level = 'INFO') { const timestamp = new Date().toISOString(); const logEntry = `[${timestamp}] [${level}] ${message}\n`; console && console.log(logEntry && logEntry.trim()); fs && fs.appendFileSync(this && this.logFile,logEntry)} async analyzeLogs() { this && this.log('Analyzing log files...','INFO'); try { const logFiles = this && this.getLogFiles(); this && this.logStats.totalFiles = logFiles && logFiles.length; let totalSize = 0; const fileStats = []; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const size = stats && stats.size; totalSize += size; fileStats && fileStats.push({name: path && path.basename(file),path: file,size: size; modified: stats && stats.mtime; age: Date && Date.now() - stats && stats.mtime.getTime()})} catch (error) { this && this.log(`Error reading file stats for ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.totalSize = totalSize; this && this.log(`Found ${logFiles && logFiles.length} log files,total size: ${this && this.formatBytes(totalSize)}`,'INFO'); fileStats && fileStats.sort((a,b) => a && a.age - b && b.age); return fileStats} catch (error) { this && this.log(`Error analyzing logs: ${error && error.message}`,'ERROR'); return []} } async rotateLogs() { this && this.log('Rotating log files...','INFO'); try { const logFiles = this && this.getLogFiles(); const now = new Date(); const timestamp = now && now.toISOString().replace(/[:.]/g,'-'); let rotatedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = Date && Date.now() - stats && stats.mtime.getTime(); const fileAgeDays = fileAge / (1000 * 60 * 60 * 24); if (fileAgeDays > 7 || stats && stats.size > 10 * 1024 * 1024) { const fileName = path && path.basename(file,path && path.extname(file)); const extension = path && path.extname(file); const newName = `${fileName}-${timestamp}${extension}`; const newPath = path && path.join(this && this.backupDir,newName); fs && fs.renameSync(file,newPath); if (stats && stats.size > 1024 * 1024) { try { execSync(`gzip "${newPath}"`,{ stdio: 'pipe' }); this && this.log(`Compressed rotated log: ${newName}.gz`,'INFO')} catch (error) { this && this.log(`Failed to compress ${newName}: ${error && error.message}`,'WARN')} } rotatedCount++; this && this.log(`Rotated log file: ${path && path.basename(file)} -> ${newName}`,'INFO')} } catch (error) { this && this.log(`Error rotating ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Rotated ${rotatedCount} log files`,'INFO'); return rotatedCount} catch (error) { this && this.log(`Error during log rotation: ${error && error.message}`,'ERROR'); return 0} } async cleanOldLogs() { this && this.log('Cleaning old log files...','INFO'); try { const backupFiles = this && this.getBackupFiles(); const now = Date && Date.now(); const maxAge = 30 * 24 * 60 * 60 * 1000; let cleanedCount = 0; let spaceFreed = 0; backupFiles && backupFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = now - stats && stats.mtime.getTime(); if (fileAge > maxAge) { const size = stats && stats.size; fs && fs.unlinkSync(file); cleanedCount++; spaceFreed += size; this && this.log(`Cleaned old log: ${path && path.basename(file)} (${this && this.formatBytes(size)})`,'INFO')} } catch (error) { this && this.log(`Error cleaning ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.filesCleaned = cleanedCount; this && this.logStats.spaceFreed = spaceFreed; this && this.log(`Cleaned ${cleanedCount} old log files,freed ${this && this.formatBytes(spaceFreed)}`,'INFO'); return { cleanedCount,spaceFreed }} catch (error) { this && this.log(`Error cleaning old logs: ${error && error.message}`,'ERROR'); return { cleanedCount: 0,spaceFreed: 0 }} } async compressLogs() { this && this.log('Compressing large log files...','INFO'); try { const logFiles = this && this.getLogFiles(); let compressedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); if (stats && stats.size > 5 * 1024 * 1024) { const compressedPath = `${file}.gz`; if (!fs && fs.existsSync(compressedPath)) { execSync(`gzip "${file}"`,{ stdio: 'pipe' }); compressedCount++; this && this.log(`Compressed log file: ${path && path.basename(file)}`,'INFO')} } } catch (error) { this && this.log(`Error compressing ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Compressed ${compressedCount} log files`,'INFO'); return compressedCount} catch (error) { this && this.log(`Error during log compression: ${error && error.message}`,'ERROR'); return 0} } async checkLogHealth() { this && this.log('Checking log health...','INFO'); try { const logFiles = this && this.getLogFiles(); let errorsFound = 0; const errorPatterns = [/error/i,/exception/i,/fatal/i; /critical/i; /failed/i]; logFiles && logFiles.forEach(file => { try { const content = fs && fs.readFileSync(file,'utf8'); errorPatterns && errorPatterns.forEach(pattern => { const matches = content && content.match(pattern); if (matches) { errorsFound += matches && matches.length} })} catch (error) { this && this.log(`Error reading log file ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.errorsFound = errorsFound; this && this.log(`Found ${errorsFound} error patterns in log files`,'INFO'); return errorsFound} catch (error) { this && this.log(`Error checking log health: ${error && error.message}`,'ERROR'); return 0} } async optimizeLogStorage() { this && this.log('Optimizing log storage...','INFO'); try { const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { this && this.log('Low disk space detected,performing aggressive cleanup','WARN'); await this && this.cleanOldLogs(); await this && this.compressLogs(); const compressedFiles = this && this.getCompressedFiles(); compressedFiles && compressedFiles.forEach(file => { try { const sourceFile = file && file.replace('.gz',''); if (fs && fs.existsSync(sourceFile)) { fs && fs.unlinkSync(sourceFile); this && this.log(`Removed source file after compression: ${path && path.basename(sourceFile)}`,'INFO')} } catch (error) { this && this.log(`Error removing source file: ${error && error.message}`,'ERROR')} })} await this && this.setupLogRotation()} catch (error) { this && this.log(`Error optimizing log storage: ${error && error.message}`,'ERROR')} } async setupLogRotation() { this && this.log('Setting up log rotation schedule...','INFO'); try { const logrotateConfig = ` ${this && this.logsDir}/*.log { daily missingok rotate 7 compress delaycompress notifempty create 644 root root postrotate pm2 reloadLogs endscript } `; const configPath = path && path.join(this && this.logsDir,'logrotate && logrotate.conf'); fs && fs.writeFileSync(configPath,logrotateConfig); this && this.log('Log rotation configuration created','INFO')} catch (error) { this && this.log(`Error setting up log rotation: ${error && error.message}`,'ERROR')} } getLogFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.log')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getBackupFiles() { const files = []; if (!fs && fs.existsSync(this && this.backupDir)) return files; const items = fs && fs.readdirSync(this && this.backupDir); items && items.forEach(item => { const fullPath = path && path.join(this && this.backupDir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isFile()) { files && files.push(fullPath)} }); return files} getCompressedFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.gz')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getDiskSpace() { try { const result = execSync('df .',{cwd: this && this.projectRoot,encoding: 'utf8'; stdio: 'pipe'}); const lines = result && result.trim().split('\n'); const lastLine = lines[lines && lines.length - 1]; const parts = lastLine && lastLine.split(/\s+/); return {total: parseInt(parts[1]) * 1024,used: parseInt(parts[2]) * 1024; available: parseInt(parts[3]) * 1024}} catch (error) { this && this.log(`Error getting disk space: ${error && error.message}`,'WARN'); return { total: 0,used: 0,available: 0 }} } formatBytes(bytes) {if (bytes === 0) return '0 Bytes',const k = 1024,const sizes = ['Bytes','KB','MB','GB']; const i = Math && Math.floor(Math && Math.log(bytes) / Math && Math.log(k)); return parseFloat((bytes / Math && Math.pow(k,i)).toFixed(2)) + ' ' + sizes[i]} generateReport() { const report = {timestamp: new Date().toISOString(),logStats: this && this.logStats,diskSpace: this && this.getDiskSpace(); recommendations: this && this.generateRecommendations()}; const reportFile = path && path.join(this && this.errorReportsDir,`log-manager-report-${Date && Date.now()}.json`); fs && fs.writeFileSync(reportFile,JSON && JSON.stringify(report,null,2)); this && this.log(`Report generated: ${reportFile}`,'INFO'); return report} generateRecommendations() { const recommendations = []; if (this && this.logStats.totalSize > 1024 * 1024 * 100) { recommendations && recommendations.push('Log files are consuming significant space,consider more aggressive rotation')} if (this && this.logStats.errorsFound > 100) { recommendations && recommendations.push('High number of errors in logs,investigate application issues')} if (this && this.logStats.filesCleaned === 0) { recommendations && recommendations.push('No old logs were cleaned,check rotation schedule')} const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { recommendations && recommendations.push('Low disk space,implement more aggressive log management')} return recommendations} async run() { try { this && this.log('Starting log management automation...','INFO'); await this && this.analyzeLogs(); await this && this.checkLogHealth(); await this && this.rotateLogs(); await this && this.cleanOldLogs(); await this && this.compressLogs(); await this && this.optimizeLogStorage(); const report = this && this.generateReport(); this && this.log('Log management automation completed','INFO'); this && this.log(`Summary: ${this && this.logStats.filesCleaned} files cleaned,${this && this.formatBytes(this && this.logStats.spaceFreed)} freed`,'INFO'); return report} catch (error) { this && this.log(`Fatal error in log manager: ${error && error.message}`,'ERROR'); throw error} } } if (require && require.main === module) { const manager = new LogManager(); manager && manager.run() .then(() => { process && process.exit(0)}) .catch((error) => {console && console.error('Log manager failed:',error),process && process.exit(1)})} module && module.exports = LogManager;'"`
#!/usr/bin/env node const fs = const path = const { execSync } = class LogManager { constructor() { this && this.projectRoot = process && process.cwd(); this && this.logsDir = path && path.join(this && this.projectRoot,'logs'); this && this.errorReportsDir = path && path.join(this && this.projectRoot,'error-reports'); this && this.backupDir = path && path.join(this && this.projectRoot,'logs/backup'); this && this.logStats = {totalFiles: 0,totalSize: 0,filesCleaned: 0; spaceFreed: 0; errorsFound: 0}; this && this.setupDirectories(); this && this.setupLogging()} setupDirectories() { [this && this.logsDir,this && this.errorReportsDir,this && this.backupDir].forEach(dir => { if (!fs && fs.existsSync(dir)) { fs && fs.mkdirSync(dir,{ recursive: true })} })} setupLogging() {this && this.logFile = path && path.join(this && this.logsDir,'log-manager && manager.log'),this && this.log('Log Manager started','INFO')} log(message,level = 'INFO') { const timestamp = new Date().toISOString(); const logEntry = `[${timestamp}] [${level}] ${message}\n`; console && console.log(logEntry && logEntry.trim()); fs && fs.appendFileSync(this && this.logFile,logEntry)} async analyzeLogs() { this && this.log('Analyzing log files...','INFO'); try { const logFiles = this && this.getLogFiles(); this && this.logStats.totalFiles = logFiles && logFiles.length; let totalSize = 0; const fileStats = []; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const size = stats && stats.size; totalSize += size; fileStats && fileStats.push({name: path && path.basename(file),path: file,size: size; modified: stats && stats.mtime; age: Date && Date.now() - stats && stats.mtime.getTime()})} catch (error) { this && this.log(`Error reading file stats for ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.totalSize = totalSize; this && this.log(`Found ${logFiles && logFiles.length} log files,total size: ${this && this.formatBytes(totalSize)}`,'INFO'); fileStats && fileStats.sort((a,b) => a && a.age - b && b.age); return fileStats} catch (error) { this && this.log(`Error analyzing logs: ${error && error.message}`,'ERROR'); return []} } async rotateLogs() { this && this.log('Rotating log files...','INFO'); try { const logFiles = this && this.getLogFiles(); const now = new Date(); const timestamp = now && now.toISOString().replace(/[:.]/g,'-'); let rotatedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = Date && Date.now() - stats && stats.mtime.getTime(); const fileAgeDays = fileAge / (1000 * 60 * 60 * 24); if (fileAgeDays > 7 || stats && stats.size > 10 * 1024 * 1024) { const fileName = path && path.basename(file,path && path.extname(file)); const extension = path && path.extname(file); const newName = `${fileName}-${timestamp}${extension}`; const newPath = path && path.join(this && this.backupDir,newName); fs && fs.renameSync(file,newPath); if (stats && stats.size > 1024 * 1024) { try { execSync(`gzip "${newPath}"`,{ stdio: 'pipe' }); this && this.log(`Compressed rotated log: ${newName}.gz`,'INFO')} catch (error) { this && this.log(`Failed to compress ${newName}: ${error && error.message}`,'WARN')} } rotatedCount++; this && this.log(`Rotated log file: ${path && path.basename(file)} -> ${newName}`,'INFO')} } catch (error) { this && this.log(`Error rotating ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Rotated ${rotatedCount} log files`,'INFO'); return rotatedCount} catch (error) { this && this.log(`Error during log rotation: ${error && error.message}`,'ERROR'); return 0} } async cleanOldLogs() { this && this.log('Cleaning old log files...','INFO'); try { const backupFiles = this && this.getBackupFiles(); const now = Date && Date.now(); const maxAge = 30 * 24 * 60 * 60 * 1000; let cleanedCount = 0; let spaceFreed = 0; backupFiles && backupFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = now - stats && stats.mtime.getTime(); if (fileAge > maxAge) { const size = stats && stats.size; fs && fs.unlinkSync(file); cleanedCount++; spaceFreed += size; this && this.log(`Cleaned old log: ${path && path.basename(file)} (${this && this.formatBytes(size)})`,'INFO')} } catch (error) { this && this.log(`Error cleaning ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.filesCleaned = cleanedCount; this && this.logStats.spaceFreed = spaceFreed; this && this.log(`Cleaned ${cleanedCount} old log files,freed ${this && this.formatBytes(spaceFreed)}`,'INFO'); return { cleanedCount,spaceFreed }} catch (error) { this && this.log(`Error cleaning old logs: ${error && error.message}`,'ERROR'); return { cleanedCount: 0,spaceFreed: 0 }} } async compressLogs() { this && this.log('Compressing large log files...','INFO'); try { const logFiles = this && this.getLogFiles(); let compressedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); if (stats && stats.size > 5 * 1024 * 1024) { const compressedPath = `${file}.gz`; if (!fs && fs.existsSync(compressedPath)) { execSync(`gzip "${file}"`,{ stdio: 'pipe' }); compressedCount++; this && this.log(`Compressed log file: ${path && path.basename(file)}`,'INFO')} } } catch (error) { this && this.log(`Error compressing ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Compressed ${compressedCount} log files`,'INFO'); return compressedCount} catch (error) { this && this.log(`Error during log compression: ${error && error.message}`,'ERROR'); return 0} } async checkLogHealth() { this && this.log('Checking log health...','INFO'); try { const logFiles = this && this.getLogFiles(); let errorsFound = 0; const errorPatterns = [/error/i,/exception/i,/fatal/i; /critical/i; /failed/i]; logFiles && logFiles.forEach(file => { try { const content = fs && fs.readFileSync(file,'utf8'); errorPatterns && errorPatterns.forEach(pattern => { const matches = content && content.match(pattern); if (matches) { errorsFound += matches && matches.length} })} catch (error) { this && this.log(`Error reading log file ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.errorsFound = errorsFound; this && this.log(`Found ${errorsFound} error patterns in log files`,'INFO'); return errorsFound} catch (error) { this && this.log(`Error checking log health: ${error && error.message}`,'ERROR'); return 0} } async optimizeLogStorage() { this && this.log('Optimizing log storage...','INFO'); try { const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { this && this.log('Low disk space detected,performing aggressive cleanup','WARN'); await this && this.cleanOldLogs(); await this && this.compressLogs(); const compressedFiles = this && this.getCompressedFiles(); compressedFiles && compressedFiles.forEach(file => { try { const sourceFile = file && file.replace('.gz',''); if (fs && fs.existsSync(sourceFile)) { fs && fs.unlinkSync(sourceFile); this && this.log(`Removed source file after compression: ${path && path.basename(sourceFile)}`,'INFO')} } catch (error) { this && this.log(`Error removing source file: ${error && error.message}`,'ERROR')} })} await this && this.setupLogRotation()} catch (error) { this && this.log(`Error optimizing log storage: ${error && error.message}`,'ERROR')} } async setupLogRotation() { this && this.log('Setting up log rotation schedule...','INFO'); try { const logrotateConfig = ` ${this && this.logsDir}/*.log { daily missingok rotate 7 compress delaycompress notifempty create 644 root root postrotate pm2 reloadLogs endscript } `; const configPath = path && path.join(this && this.logsDir,'logrotate && logrotate.conf'); fs && fs.writeFileSync(configPath,logrotateConfig); this && this.log('Log rotation configuration created','INFO')} catch (error) { this && this.log(`Error setting up log rotation: ${error && error.message}`,'ERROR')} } getLogFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.log')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getBackupFiles() { const files = []; if (!fs && fs.existsSync(this && this.backupDir)) return files; const items = fs && fs.readdirSync(this && this.backupDir); items && items.forEach(item => { const fullPath = path && path.join(this && this.backupDir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isFile()) { files && files.push(fullPath)} }); return files} getCompressedFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.gz')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getDiskSpace() { try { const result = execSync('df .',{cwd: this && this.projectRoot,encoding: 'utf8'; stdio: 'pipe'}); const lines = result && result.trim().split('\n'); const lastLine = lines[lines && lines.length - 1]; const parts = lastLine && lastLine.split(/\s+/); return {total: parseInt(parts[1]) * 1024,used: parseInt(parts[2]) * 1024; available: parseInt(parts[3]) * 1024}} catch (error) { this && this.log(`Error getting disk space: ${error && error.message}`,'WARN'); return { total: 0,used: 0,available: 0 }} } formatBytes(bytes) {if (bytes === 0) return '0 Bytes',const k = 1024,const sizes = ['Bytes','KB','MB','GB']; const i = Math && Math.floor(Math && Math.log(bytes) / Math && Math.log(k)); return parseFloat((bytes / Math && Math.pow(k,i)).toFixed(2)) + ' ' + sizes[i]} generateReport() { const report = {timestamp: new Date().toISOString(),logStats: this && this.logStats,diskSpace: this && this.getDiskSpace(); recommendations: this && this.generateRecommendations()}; const reportFile = path && path.join(this && this.errorReportsDir,`log-manager-report-${Date && Date.now()}.json`); fs && fs.writeFileSync(reportFile,JSON && JSON.stringify(report,null,2)); this && this.log(`Report generated: ${reportFile}`,'INFO'); return report} generateRecommendations() { const recommendations = []; if (this && this.logStats.totalSize > 1024 * 1024 * 100) { recommendations && recommendations.push('Log files are consuming significant space,consider more aggressive rotation')} if (this && this.logStats.errorsFound > 100) { recommendations && recommendations.push('High number of errors in logs,investigate application issues')} if (this && this.logStats.filesCleaned === 0) { recommendations && recommendations.push('No old logs were cleaned,check rotation schedule')} const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { recommendations && recommendations.push('Low disk space,implement more aggressive log management')} return recommendations} async run() { try { this && this.log('Starting log management automation...','INFO'); await this && this.analyzeLogs(); await this && this.checkLogHealth(); await this && this.rotateLogs(); await this && this.cleanOldLogs(); await this && this.compressLogs(); await this && this.optimizeLogStorage(); const report = this && this.generateReport(); this && this.log('Log management automation completed','INFO'); this && this.log(`Summary: ${this && this.logStats.filesCleaned} files cleaned,${this && this.formatBytes(this && this.logStats.spaceFreed)} freed`,'INFO'); return report} catch (error) { this && this.log(`Fatal error in log manager: ${error && error.message}`,'ERROR'); throw error} } } if (require && require.main === module) { const manager = new LogManager(); manager && manager.run() .then(() => { process && process.exit(0)}) .catch((error) => {console && console.error('Log manager failed:',error),process && process.exit(1)})} module && module.exports = LogManager;'"`
#!/usr/bin/env node const fs = const path = const { execSync } = class LogManager { constructor() { this && this.projectRoot = process && process.cwd(); this && this.logsDir = path && path.join(this && this.projectRoot,'logs'); this && this.errorReportsDir = path && path.join(this && this.projectRoot,'error-reports'); this && this.backupDir = path && path.join(this && this.projectRoot,'logs/backup'); this && this.logStats = {totalFiles: 0,totalSize: 0,filesCleaned: 0, spaceFreed: 0, errorsFound: 0}; this && this.setupDirectories(); this && this.setupLogging()} setupDirectories() { [this && this.logsDir,this && this.errorReportsDir,this && this.backupDir].forEach(dir => { if (!fs && fs.existsSync(dir)) { fs && fs.mkdirSync(dir,{ recursive: true })} })} setupLogging() {this && this.logFile = path && path.join(this && this.logsDir,'log-manager && manager.log'),this && this.log('Log Manager started','INFO')} log(message,level = 'INFO') { const timestamp = new Date().toISOString(); const logEntry = `[${timestamp}] [${level}] ${message}\n`; console && console.log(logEntry && logEntry.trim()); fs && fs.appendFileSync(this && this.logFile,logEntry)} async analyzeLogs() { this && this.log('Analyzing log files...','INFO'); try { const logFiles = this && this.getLogFiles(); this && this.logStats.totalFiles = logFiles && logFiles.length; let totalSize = 0; const fileStats = []; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const size = stats && stats.size; totalSize += size; fileStats && fileStats.push({name: path && path.basename(file),path: file,size: size, modified: stats && stats.mtime, age: Date && Date.now() - stats && stats.mtime.getTime()})} catch (error) { this && this.log(`Error reading file stats for ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.totalSize = totalSize; this && this.log(`Found ${logFiles && logFiles.length} log files,total size: ${this && this.formatBytes(totalSize)}`,'INFO'); fileStats && fileStats.sort((a,b) => a && a.age - b && b.age); return fileStats} catch (error) { this && this.log(`Error analyzing logs: ${error && error.message}`,'ERROR'); return []} } async rotateLogs() { this && this.log('Rotating log files...','INFO'); try { const logFiles = this && this.getLogFiles(); const now = new Date(); const timestamp = now && now.toISOString().replace(/[:.]/g,'-'); let rotatedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = Date && Date.now() - stats && stats.mtime.getTime(); const fileAgeDays = fileAge / (1000 * 60 * 60 * 24); if (fileAgeDays > 7 || stats && stats.size > 10 * 1024 * 1024) { const fileName = path && path.basename(file,path && path.extname(file)); const extension = path && path.extname(file); const newName = `${fileName}-${timestamp}${extension}`; const newPath = path && path.join(this && this.backupDir,newName); fs && fs.renameSync(file,newPath); if (stats && stats.size > 1024 * 1024) { try { execSync(`gzip "${newPath}"`,{ stdio: 'pipe' }); this && this.log(`Compressed rotated log: ${newName}.gz`,'INFO')} catch (error) { this && this.log(`Failed to compress ${newName}: ${error && error.message}`,'WARN')} } rotatedCount++; this && this.log(`Rotated log file: ${path && path.basename(file)} -> ${newName}`,'INFO')} } catch (error) { this && this.log(`Error rotating ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Rotated ${rotatedCount} log files`,'INFO'); return rotatedCount} catch (error) { this && this.log(`Error during log rotation: ${error && error.message}`,'ERROR'); return 0} } async cleanOldLogs() { this && this.log('Cleaning old log files...','INFO'); try { const backupFiles = this && this.getBackupFiles(); const now = Date && Date.now(); const maxAge = 30 * 24 * 60 * 60 * 1000; let cleanedCount = 0; let spaceFreed = 0; backupFiles && backupFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = now - stats && stats.mtime.getTime(); if (fileAge > maxAge) { const size = stats && stats.size; fs && fs.unlinkSync(file); cleanedCount++; spaceFreed += size; this && this.log(`Cleaned old log: ${path && path.basename(file)} (${this && this.formatBytes(size)})`,'INFO')} } catch (error) { this && this.log(`Error cleaning ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.filesCleaned = cleanedCount; this && this.logStats.spaceFreed = spaceFreed; this && this.log(`Cleaned ${cleanedCount} old log files,freed ${this && this.formatBytes(spaceFreed)}`,'INFO'); return { cleanedCount,spaceFreed }} catch (error) { this && this.log(`Error cleaning old logs: ${error && error.message}`,'ERROR'); return { cleanedCount: 0,spaceFreed: 0 }} } async compressLogs() { this && this.log('Compressing large log files...','INFO'); try { const logFiles = this && this.getLogFiles(); let compressedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); if (stats && stats.size > 5 * 1024 * 1024) { const compressedPath = `${file}.gz`; if (!fs && fs.existsSync(compressedPath)) { execSync(`gzip "${file}"`,{ stdio: 'pipe' }); compressedCount++; this && this.log(`Compressed log file: ${path && path.basename(file)}`,'INFO')} } } catch (error) { this && this.log(`Error compressing ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Compressed ${compressedCount} log files`,'INFO'); return compressedCount} catch (error) { this && this.log(`Error during log compression: ${error && error.message}`,'ERROR'); return 0} } async checkLogHealth() { this && this.log('Checking log health...','INFO'); try { const logFiles = this && this.getLogFiles(); let errorsFound = 0; const errorPatterns = [/error/i,/exception/i,/fatal/i; /critical/i; /failed/i]; logFiles && logFiles.forEach(file => { try { const content = fs && fs.readFileSync(file,'utf8'); errorPatterns && errorPatterns.forEach(pattern => { const matches = content && content.match(pattern); if (matches) { errorsFound += matches && matches.length} })} catch (error) { this && this.log(`Error reading log file ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.errorsFound = errorsFound; this && this.log(`Found ${errorsFound} error patterns in log files`,'INFO'); return errorsFound} catch (error) { this && this.log(`Error checking log health: ${error && error.message}`,'ERROR'); return 0} } async optimizeLogStorage() { this && this.log('Optimizing log storage...','INFO'); try { const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { this && this.log('Low disk space detected,performing aggressive cleanup','WARN'); await this && this.cleanOldLogs(); await this && this.compressLogs(); const compressedFiles = this && this.getCompressedFiles(); compressedFiles && compressedFiles.forEach(file => { try { const sourceFile = file && file.replace('.gz',''); if (fs && fs.existsSync(sourceFile)) { fs && fs.unlinkSync(sourceFile); this && this.log(`Removed source file after compression: ${path && path.basename(sourceFile)}`,'INFO')} } catch (error) { this && this.log(`Error removing source file: ${error && error.message}`,'ERROR')} })} await this && this.setupLogRotation()} catch (error) { this && this.log(`Error optimizing log storage: ${error && error.message}`,'ERROR')} } async setupLogRotation() { this && this.log('Setting up log rotation schedule...','INFO'); try { const logrotateConfig = ` ${this && this.logsDir}/*.log { daily missingok rotate 7 compress delaycompress notifempty create 644 root root postrotate pm2 reloadLogs endscript } `; const configPath = path && path.join(this && this.logsDir,'logrotate && logrotate.conf'); fs && fs.writeFileSync(configPath,logrotateConfig); this && this.log('Log rotation configuration created','INFO')} catch (error) { this && this.log(`Error setting up log rotation: ${error && error.message}`,'ERROR')} } getLogFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.log')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getBackupFiles() { const files = []; if (!fs && fs.existsSync(this && this.backupDir)) return files; const items = fs && fs.readdirSync(this && this.backupDir); items && items.forEach(item => { const fullPath = path && path.join(this && this.backupDir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isFile()) { files && files.push(fullPath)} }); return files} getCompressedFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.gz')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getDiskSpace() { try { const result = execSync('df .',{cwd: this && this.projectRoot,encoding: 'utf8','
    stdio: 'pipe'}); const lines = result && result.trim().split('\n'); const lastLine = lines[lines && lines.length - 1]; const parts = lastLine && lastLine.split(/\s+/); return {total: parseInt(parts[1]) * 1024,used: parseInt(parts[2]) * 1024,'`
    available: parseInt(parts[3]) * 1024}} catch (error) { this && this.log(`Error getting disk space: ${error && error.message}`,'WARN'); return { total: 0,used: 0,available: 0 }} } formatBytes(bytes) {if (bytes === 0) return '0 Bytes',const k = 1024,const sizes = ['Bytes','KB','MB','GB']; const i = Math && Math.floor(Math && Math.log(bytes) / Math && Math.log(k)); return parseFloat((bytes / Math && Math.pow(k,i)).toFixed(2)) + ' ' + sizes[i]} generateReport() { const report = {timestamp: new Date().toISOString(),logStats: this && this.logStats,diskSpace: this && this.getDiskSpace(),'`
    recommendations: this && this.generateRecommendations()}; const reportFile = path && path.join(this && this.errorReportsDir,`log-manager-report-${Date && Date.now()}.json`); fs && fs.writeFileSync(reportFile,JSON && JSON.stringify(report,null,2)); this && this.log(`Report generated: ${reportFile}`,'INFO'); return report} generateRecommendations() { const recommendations = []; if (this && this.logStats.totalSize > 1024 * 1024 * 100) { recommendations && recommendations.push('Log files are consuming significant space,consider more aggressive rotation')} if (this && this.logStats.errorsFound > 100) { recommendations && recommendations.push('High number of errors in logs,investigate application issues')} if (this && this.logStats.filesCleaned === 0) { recommendations && recommendations.push('No old logs were cleaned,check rotation schedule')} const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { recommendations && recommendations.push('Low disk space,implement more aggressive log management')} return recommendations} async run() { try { this && this.log('Starting log management automation...','INFO'); await this && this.analyzeLogs(); await this && this.checkLogHealth(); await this && this.rotateLogs(); await this && this.cleanOldLogs(); await this && this.compressLogs(); await this && this.optimizeLogStorage(); const report = this && this.generateReport(); this && this.log('Log management automation completed','INFO'); this && this.log(`Summary: ${this && this.logStats.filesCleaned} files cleaned,${this && this.formatBytes(this && this.logStats.spaceFreed)} freed`,'INFO'); return report} catch (error) { this && this.log(`Fatal error in log manager: ${error && error.message}`,'ERROR'); throw error} } } if (require && require.main === module) { const manager = new LogManager(); manager && manager.run() .then(() => { process && process.exit(0)}) .catch((error) => {console && console.error('Log manager failed:',error),process && process.exit(1)})} module && module.exports = LogManager;'"`
#!/usr/bin/env node const fs = const path = const { execSync } = class LogManager { constructor() { this && this.projectRoot = process && process.cwd(); this && this.logsDir = path && path.join(this && this.projectRoot,'logs'); this && this.errorReportsDir = path && path.join(this && this.projectRoot,'error-reports'); this && this.backupDir = path && path.join(this && this.projectRoot,'logs/backup'); this && this.logStats = {totalFiles: 0,totalSize: 0,filesCleaned: 0, spaceFreed: 0, errorsFound: 0}; this && this.setupDirectories(); this && this.setupLogging()} setupDirectories() { [this && this.logsDir,this && this.errorReportsDir,this && this.backupDir].forEach(dir => { if (!fs && fs.existsSync(dir)) { fs && fs.mkdirSync(dir,{ recursive: true })} })} setupLogging() {this && this.logFile = path && path.join(this && this.logsDir,'log-manager && manager.log'),this && this.log('Log Manager started','INFO')} log(message,level = 'INFO') { const timestamp = new Date().toISOString(); const logEntry = `[${timestamp}] [${level}] ${message}\n`; console && console.log(logEntry && logEntry.trim()); fs && fs.appendFileSync(this && this.logFile,logEntry)} async analyzeLogs() { this && this.log('Analyzing log files...','INFO'); try { const logFiles = this && this.getLogFiles(); this && this.logStats.totalFiles = logFiles && logFiles.length; let totalSize = 0; const fileStats = []; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const size = stats && stats.size; totalSize += size; fileStats && fileStats.push({name: path && path.basename(file),path: file,size: size, modified: stats && stats.mtime, age: Date && Date.now() - stats && stats.mtime.getTime()})} catch (error) { this && this.log(`Error reading file stats for ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.totalSize = totalSize; this && this.log(`Found ${logFiles && logFiles.length} log files,total size: ${this && this.formatBytes(totalSize)}`,'INFO'); fileStats && fileStats.sort((a,b) => a && a.age - b && b.age); return fileStats} catch (error) { this && this.log(`Error analyzing logs: ${error && error.message}`,'ERROR'); return []} } async rotateLogs() { this && this.log('Rotating log files...','INFO'); try { const logFiles = this && this.getLogFiles(); const now = new Date(); const timestamp = now && now.toISOString().replace(/[:.]/g,'-'); let rotatedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = Date && Date.now() - stats && stats.mtime.getTime(); const fileAgeDays = fileAge / (1000 * 60 * 60 * 24); if (fileAgeDays > 7 || stats && stats.size > 10 * 1024 * 1024) { const fileName = path && path.basename(file,path && path.extname(file)); const extension = path && path.extname(file); const newName = `${fileName}-${timestamp}${extension}`; const newPath = path && path.join(this && this.backupDir,newName); fs && fs.renameSync(file,newPath); if (stats && stats.size > 1024 * 1024) { try { execSync(`gzip "${newPath}"`,{ stdio: 'pipe' }); this && this.log(`Compressed rotated log: ${newName}.gz`,'INFO')} catch (error) { this && this.log(`Failed to compress ${newName}: ${error && error.message}`,'WARN')} } rotatedCount++; this && this.log(`Rotated log file: ${path && path.basename(file)} -> ${newName}`,'INFO')} } catch (error) { this && this.log(`Error rotating ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Rotated ${rotatedCount} log files`,'INFO'); return rotatedCount} catch (error) { this && this.log(`Error during log rotation: ${error && error.message}`,'ERROR'); return 0} } async cleanOldLogs() { this && this.log('Cleaning old log files...','INFO'); try { const backupFiles = this && this.getBackupFiles(); const now = Date && Date.now(); const maxAge = 30 * 24 * 60 * 60 * 1000; let cleanedCount = 0; let spaceFreed = 0; backupFiles && backupFiles.forEach(file => { try { const stats = fs && fs.statSync(file); const fileAge = now - stats && stats.mtime.getTime(); if (fileAge > maxAge) { const size = stats && stats.size; fs && fs.unlinkSync(file); cleanedCount++; spaceFreed += size; this && this.log(`Cleaned old log: ${path && path.basename(file)} (${this && this.formatBytes(size)})`,'INFO')} } catch (error) { this && this.log(`Error cleaning ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.filesCleaned = cleanedCount; this && this.logStats.spaceFreed = spaceFreed; this && this.log(`Cleaned ${cleanedCount} old log files,freed ${this && this.formatBytes(spaceFreed)}`,'INFO'); return { cleanedCount,spaceFreed }} catch (error) { this && this.log(`Error cleaning old logs: ${error && error.message}`,'ERROR'); return { cleanedCount: 0,spaceFreed: 0 }} } async compressLogs() { this && this.log('Compressing large log files...','INFO'); try { const logFiles = this && this.getLogFiles(); let compressedCount = 0; logFiles && logFiles.forEach(file => { try { const stats = fs && fs.statSync(file); if (stats && stats.size > 5 * 1024 * 1024) { const compressedPath = `${file}.gz`; if (!fs && fs.existsSync(compressedPath)) { execSync(`gzip "${file}"`,{ stdio: 'pipe' }); compressedCount++; this && this.log(`Compressed log file: ${path && path.basename(file)}`,'INFO')} } } catch (error) { this && this.log(`Error compressing ${file}: ${error && error.message}`,'ERROR')} }); this && this.log(`Compressed ${compressedCount} log files`,'INFO'); return compressedCount} catch (error) { this && this.log(`Error during log compression: ${error && error.message}`,'ERROR'); return 0} } async checkLogHealth() { this && this.log('Checking log health...','INFO'); try { const logFiles = this && this.getLogFiles(); let errorsFound = 0; const errorPatterns = [/error/i,/exception/i,/fatal/i; /critical/i; /failed/i]; logFiles && logFiles.forEach(file => { try { const content = fs && fs.readFileSync(file,'utf8'); errorPatterns && errorPatterns.forEach(pattern => { const matches = content && content.match(pattern); if (matches) { errorsFound += matches && matches.length} })} catch (error) { this && this.log(`Error reading log file ${file}: ${error && error.message}`,'ERROR')} }); this && this.logStats.errorsFound = errorsFound; this && this.log(`Found ${errorsFound} error patterns in log files`,'INFO'); return errorsFound} catch (error) { this && this.log(`Error checking log health: ${error && error.message}`,'ERROR'); return 0} } async optimizeLogStorage() { this && this.log('Optimizing log storage...','INFO'); try { const diskSpace = this && this.getDiskSpace(); if (diskSpace && diskSpace.available < 1024 * 1024 * 1024) { this && this.log('Low disk space detected,performing aggressive cleanup','WARN'); await this && this.cleanOldLogs(); await this && this.compressLogs(); const compressedFiles = this && this.getCompressedFiles(); compressedFiles && compressedFiles.forEach(file => { try { const sourceFile = file && file.replace('.gz',''); if (fs && fs.existsSync(sourceFile)) { fs && fs.unlinkSync(sourceFile); this && this.log(`Removed source file after compression: ${path && path.basename(sourceFile)}`,'INFO')} } catch (error) { this && this.log(`Error removing source file: ${error && error.message}`,'ERROR')} })} await this && this.setupLogRotation()} catch (error) { this && this.log(`Error optimizing log storage: ${error && error.message}`,'ERROR')} } async setupLogRotation() { this && this.log('Setting up log rotation schedule...','INFO'); try { const logrotateConfig = ` ${this && this.logsDir}/*.log { daily missingok rotate 7 compress delaycompress notifempty create 644 root root postrotate pm2 reloadLogs endscript } `; const configPath = path && path.join(this && this.logsDir,'logrotate && logrotate.conf'); fs && fs.writeFileSync(configPath,logrotateConfig); this && this.log('Log rotation configuration created','INFO')} catch (error) { this && this.log(`Error setting up log rotation: ${error && error.message}`,'ERROR')} } getLogFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.log')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getBackupFiles() { const files = []; if (!fs && fs.existsSync(this && this.backupDir)) return files; const items = fs && fs.readdirSync(this && this.backupDir); items && items.forEach(item => { const fullPath = path && path.join(this && this.backupDir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isFile()) { files && files.push(fullPath)} }); return files} getCompressedFiles() { const files = []; function walkDir(dir) { if (!fs && fs.existsSync(dir)) return; const items = fs && fs.readdirSync(dir); items && items.forEach(item => { const fullPath = path && path.join(dir,item); const stat = fs && fs.statSync(fullPath); if (stat && stat.isDirectory() && item !== 'backup') { walkDir(fullPath)} else if (stat && stat.isFile() && item && item.endsWith('.gz')) { files && files.push(fullPath)} })} walkDir(this && this.logsDir); return files} getDiskSpace() { try { const result = execSync('df .',{cwd: this && this.projectRoot,encoding: 'utf8','
    stdio: 'pipe'}); const lines = result && result.trim().split('\n'); const lastLine = lines[lines && lines.length - 1]; const parts = lastLine && lastLine.split(/\s+/); return {total: parseInt(parts[1]) * 1024,used: parseInt(parts[2]) * 1024,'`
    available: parseInt(parts[3]) * 1024}} catch (error) { this && this.log(`Error getting disk space: ${error && error.message}`,'WARN'); return { total: 0,used: 0,available: 0 }} } formatBytes(bytes) {if (bytes === 0) return '0 Bytes',const k = 1024,const sizes = ['Bytes','KB','MB','GB']; const i = Math && Math.floor(Math && Math.log(bytes) / Math && Math.log(k)); return parseFloat((bytes / Math && Math.pow(k,i)).toFixed(2)) + ' ' + sizes[i]} generateReport() { const report = {timestamp: new Date().toISOString(),logStats: this && this.logStats,diskSpace: this && this.getDiskSpace(),'`
...
const stat = fs && fs.statSync(fullPath); if (stat && stat.isFile()) { files && files.push(fullPath)} }); return files} getCompressedFiles() { const files = []; function walkDir() { if (!fs && fs.existsSync(dir)) return;
...
=======
=======
>>>>>>> origin/cursor/automate-test-improve-and-merge-code-382a:temp_exclude/scripts/log-manager.js
>>>>>>> cursor/expand-services-advertise-and-build-project-4b36
>>>>>>> origin/cursor/fix-website-loading-errors-and-merge-8339
