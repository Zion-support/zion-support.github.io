...
>>>>>>> merged-prs-20250907-203621
  ...
  setupLogging($2) {this.logFile = path.join(this.logsDir, 'log-manager.log'),this.log('Log Manager started', 'INFO')}'
  log($2) {}
    const timestamp = new Date().toISOString();`
...
>>>>>>> merged-prs-20250907-203621
  ...
  getLogFiles() {
    ...
    walkDir(this.logsDir)
    return files}
  formatBytes($2) {if (bytes === 0) return '0 Bytes',const k = 1024,const sizes = ['Bytes', 'KB', 'MB', 'GB']
    const i = Math.floor(Math.log(bytes) / Math.log(k));'
    return parseFloat((bytes / Math.pow(k, i)).toFixed(2)) + ' ' + sizes[i]}
  generateReport() {
    const report = {"timestamp": new Date().toISOString(),"logStats": this.logStats,"diskSpace": this.getDiskSpace();
