...
class SecurityAuditEnhanced {
    ...
    files.forEach(file => {
      ...
      lines.forEach((line, index) => {
        if (line.includes("console.log") && !line.trim().startsWith("//")) {
          this.vulnerabilities.push({type: "code_quality",file: path.relative(this.projectRoot, file),line: index + 1;
            severity: "low";
      ...
      try {
        ...
        secretPatterns.forEach(pattern => {
          if (pattern.test(content)) {
            this.vulnerabilities.push({type: "secrets",file: path.relative(this.projectRoot, file),severity: "high";
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes("http://") && !content.includes("localhost")) {
          this.vulnerabilities.push({type: "insecure_protocol",file: path.relative(this.projectRoot, file),severity: "moderate";
  ...
  checkSecurityHeaders() {
    ...
    if (configExists && !hasSecurityHeaders) {
      this.recommendations.push({type: "configuration",description: "Add security headers to Next.js configuration";
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes("process.env") && content.includes("console.log")) {
          this.vulnerabilities.push({type: "data_exposure",file: path.relative(this.projectRoot, file),severity: "moderate";
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes("debugger") || content.includes("console.debug")) {
          this.vulnerabilities.push({type: "debug_info",file: path.relative(this.projectRoot, file),severity: "low";
      ...
      if (packageJson.scripts) {
        Object.entries(packageJson.scripts).forEach(([scriptName, script]) => {
          if (script.includes("--unsafe") || script.includes("--insecure")) {
            this.vulnerabilities.push({type: "insecure_script",script: scriptName,severity: "high";
  ...
  generateSecurityReport() {
    const report = {
      ...
      summary: {totalVulnerabilities: this.vulnerabilities.length,critical: this.vulnerabilities.filter(v => v.severity === "critical").length,high: this.vulnerabilities.filter(v => v.severity === "high").length;
