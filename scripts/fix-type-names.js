...
#!/usr/bin/env node import fs from 'fs', import path from 'path', import {glob} from 'glob', const typeFixes = [ { pattern: /strin\s+g/g,replacement: 'string' ,},{ pattern: /numbe\s+r/g,replacement: 'number' ,},{ pattern: /boolea\s+n/g,replacement: 'boolean' ,},{ pattern: /ClassValu\s+e/g,replacement: 'ClassValue' ,},{ pattern: /EmergingTechService202\s+7/g,replacement: 'EmergingTechService2027' ,},{ pattern: /EMERGING_TECH_SERVICES_202\s+7/g,replacement: 'EMERGING_TECH_SERVICES_2027' ,} ], function fixFile(filePath) { try { const content = fs && fs.readFileSync(filePath,'utf8'), let fixedContent = content, let hasChanges = false, typeFixes && typeFixes.forEach(fix => { const newContent = fixedContent && fixedContent.replace(fix && fix.pattern,fix && fix.replacement), if (newContent !== fixedContent) { fixedContent = newContent, hasChanges = true} }), if (hasChanges) { fs && fs.writeFileSync(filePath,fixedContent,'utf8'),  return true} return false} catch (error) { console && console.error(`Error fixing ${filePath}:`,error && error.message), return false} } async function main() { const patterns = [ 'src*.tsxsrc*.ts','components*.tsxcomponents*.ts','pages*.tsxpages*.ts' ], let totalFixed = 0, for (const pattern of patterns) { const files = await glob(pattern,{ cwd: process && process.cwd() ,}), for (const file of files) { if (fixFile(file)) { totalFixed++} } } } main().catch(console && console.error);
#!/usr/bin/env node,
import fs from 'fs';
