...
function analyzeBundle() {console.log('🔍 Analyzing bundle...\n');
...
function formatBytes(bytes) {if (bytes === 0) return '0 Bytes';
  const k = 1024;
  const sizes = ['Bytes', 'KB', 'MB', 'GB'];
  const i = Math.floor(Math.log(bytes) / Math.log(k));
  return parseFloat((bytes / Math.pow(k, i)).toFixed(2)) + ' ' + sizes[i];
}
function generateRecommendations(analysis) {const recommendations = [];
  ...
  if (vendorFiles.length > 0) {const vendorSize = vendorFiles.reduce((sum, file) => sum + file.size, 0);
  ...
  if (duplicates.length > 0) {recommendations.push('Duplicate chunks detected. Consider optimizing chunk splitting strategy.');
  }
...
function generateHTMLReport(analysis) {const html = `
<!DOCTYPE html>
<html lang="en" ></html>
<head></head>
    ...
    <style></style>
        ...
        .recommendations {background: #1e293b;
            padding: 20px;
            border-radius: 8px;
            border-left: 4px solid #00d4ff;
        }
        .recommendations h3 {color: #00d4ff;
            margin-top: 0;
        }
