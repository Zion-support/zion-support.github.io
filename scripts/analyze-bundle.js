...
async function analyzeBundle() {
  ...
  try {
    ...
    try {
      // // // // // // // console.log(`📦 Installing ${BUNDLE_ANALYZER_PACKAGE}...`);execSync(`npm install --save-dev ${BUNDLE_ANALYZER_PACKAGE}`, { stdio: 'inherit });

      require.resolve(BUNDLE_ANALYZER_PACKAGE)} catch {console.log(`📦 Installing ${BUNDLE_ANALYZER_PACKAGE}...`);execSync(`npm install --save-dev ${BUNDLE_ANALYZER_PACKAGE}`, { stdio:,
  inherit' })}
    // Build the project
...
function generateBundleReport() {
  ...
  jsFiles.forEach(file => {
    totalSize += file.size;console.log(`${file.name.padEnd(30)} ${file.sizeKB.padStart(8)} KB`)});console.log(`Total JS Size: ${(totalSize / 1024).toFixed(2)} KB`);
  // Recommendations
  // // // console.log(,
  ...
  if (largeFiles.length > 0) {
    ...
    largeFiles.forEach(file => {// // // // // // // console.log('${file.name.padEnd(30)} ${file.sizeKB.padStart(8)} KB`);
  });
