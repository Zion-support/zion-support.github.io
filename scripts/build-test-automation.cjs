...
class BuildTestAutomation {
  ...
  async cleanBuild() {
    try {
      await this.runCommand('rm -rf .next dist build', 'Clean previous builds');
    } catch (error) {
  ...
  async checkBuildOutput() {
    try {
      ...
      this.report.summary.buildOutput = 'verified';
    } catch (error) {
  ...
  async run() {
    try {
      ...
      this.log('Build and Test Automation completed successfully', 'success');
      
    } catch (error) {
      this.log(`Build and Test Automation failed: ${error.message}`, 'error');
    } finally {
