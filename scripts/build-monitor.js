...
class BuildMonitor {
  constructor() {
    ...
    this.buildStats = {buildTime: 0,bundleSize: 0,errorCount: 0;
      warningCount: 0;
      success: false};
  ...
  setupLogging() {this.logFile = path.join(this.logsDir, 'build-monitor.log'),this.log('Build Monitor started', 'INFO')}
  ...
  async checkBuildHealth() {
    ...
    try {
      ...
      if (!fs.existsSync(this.buildDir)) {this.log('Build directory not found, build may have failed', 'WARN'),return false}
      ...
      if (buildFiles.length === 0) {this.log('No build artifacts found', 'WARN'),return false}
  ...
  async optimizeBuild() {
    ...
    try {
      ...
      // Clear cache
      execSync('npm run clean:cache || true', {cwd: this.projectRoot,stdio: 'pipe'});
      // Run optimized build
  ...
  async checkDependencies() {
    ...
    try {
      ...
      if (!buildScripts.build) {this.log('No build script found in package.json', 'WARN'),return false}
  ...
  getBuildLog() {
    try {
      ...
      const logFiles = [path.join(this.logsDir, 'build.log'),path.join(this.logsDir, 'build-monitor.log'),path.join(this.logsDir, 'zion-website.log')];
  ...
  generateReport() {
    const report = {timestamp: new Date().toISOString(),buildStats: this.buildStats,buildHealth: this.buildStats.success;
  ...
  async run() {
    try {
      ...
      if (!isHealthy) {this.log('Build is not healthy, attempting optimization...', 'WARN'),await this.optimizeBuild()}
...
if (require.main === module) {
  ...
  monitor.run()
    .then(() => {
      process.exit(0)})
    .catch((error) => {console.error('Build monitor failed:', error),process.exit(1)})}
