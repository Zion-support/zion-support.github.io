    ...
    if (content.includes('<<<<<<<') || content.includes('') || content.includes('>>>>>>>')) {
      ...
      let resolvedContent = content
        ...
        .replace(/          const parts = match.split('')
          if (parts.length > 1) {const incoming = parts[1].replace(/            return incoming;)} )
          return match;)
        })
...
// Main execution
async function main() {console.log('🚀 Starting comprehensive merge conflict resolution...\n')
  ...
  for (const branch of branchesToMerge) {console.log(`\n🔄 Attempting to merge ${branch} ...`)
    try {
      // Check if branch exists
      const branchExists = execGitCommand(`git show-ref --verify --quiet refs/remotes/origin/${branch} `, `Checking if $ {branch} exists`)
      if (branchExists !== null) {const mergeResult = execGitCommand(`git merge origin/${branch} --no-edit`, `Merging $ {branch}`)
        if (mergeResult) {console.log(`✅ Successfully merged ${branch} `)
        } else {
          ...
          } else {console.log(`❌ Failed to resolve conflicts in ${branch} `)
          }
        }
      } else {
