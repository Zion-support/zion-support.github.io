...
class HealthChecker {
  ...
  'info', `Application health check passed (${responseTime}ms)`)          resolve({
            status: 'healthy, responseTime, statusCode: res.statusCode})
        } else {
          this.log(, warning', `Application health check failed with status ${res.statusCode}`)          resolve({
            status: 'unhealthy, responseTime, statusCode: res.statusCode, reason: `HTTP ${res.statusCode}`})
        }
      })
  ...
  'error', 'Application health check failed', { error: error.message })        resolve({
          status:
  'unhealthy', responseTime, reason: error.message})
      })
  ...
  '
      ...
      this.log('info, `Disk usage: ${usage}`, { status, usagePercent })
            return {
        status, usage: usage, usagePercent, available: diskInfo[3], total: diskInfo[1]}
    } catch (error) {
      this.log(, error
  ...
  '
      ...
      this.log('info, `Memory usage: ${usagePercent}%`, { status, used, total })
            return {
        status, usagePercent, used: `${used}MB`, total: `${total}MB`, available: `${total - used}MB`}
    } catch (error) {
      this.log(, error
  ...
  '
      ...
      this.log('info, `PM2 processes: ${healthyProcesses.length} healthy, ${unhealthyProcesses.length} unhealthy`)
            return {
        status, total: processes.length, healthy: healthyProcesses.length, unhealthy: unhealthyProcesses.length, processes: processes.map(p => ({
          name: p.name, status: p.pm2_env.status, cpu: p.monit.cpu, memory: p.monit.memory, uptime: p.pm2_env.pm_uptime, restarts: p.pm2_env.restart_time}))}
    } catch (error) {
      this.log(, error
  ...
  async checkLogFiles() {
    try {
      ...
      for (const file of logFiles) {
        ...
        logStats.push({
          file, size: `${sizeKB}KB`, sizeBytes: stats.size, modified: stats.mtime})
      }

      const totalSize = logStats.reduce((sum, log) => sum + log.sizeBytes, 0)
      const totalSizeMB = Math.round(totalSize / (1024 * 1024))

      const status = totalSizeMB > 500 ?, warning
  ': 'healthy
  '

      this.log('info, `Log files: ${logFiles.length} files, ${totalSizeMB}MB total`)
            return {
        status, fileCount: logFiles.length, totalSize: `${totalSizeMB}MB`, files: logStats}
    } catch (error) {
      this.log(, error
  ...
  ')
        return {
          status: 'warning, reason: 'node_modules not found
  ', dependencies: Object.keys(packageJson.dependencies || {}).length, devDependencies: Object.keys(packageJson.devDependencies || {})
            .length}
      }
  ...
  ', `Dependencies: ${dependencyCount} production, ${devDependencyCount} development`)
            return {
        status: 'healthy
  ', dependencies: dependencyCount, devDependencies: devDependencyCount, nodeModulesExists}
    } catch (error) {
      this.log(, error
  ...
  async generateReport() {
    ...
    const reportFile = path.join(
      this.logDir, `health-report-${Date.now()}.json`
    )

    const overallHealth = this.determineOverallHealth(this.checks)

    const report = {
      timestamp, overallHealth, checks: this.checks, summary: {
        healthy: this.checks.filter(c => c.status ===, healthy
  ').length, warning: this.checks.filter(c => c.status === 'warning).length, unhealthy: this.checks.filter(c => c.status ===, unhealthy
  ').length, critical: this.checks.filter(c => c.status === 'critical).length, unknown: this.checks.filter(c => c.status === 'unknown
  ').length
      }    }
