...
await runDeploymentStep('Dependency Verification', async () => {
  ...
  try {
    ...
    return { dependenciesInstalled: true, securityAudit: 'passed' };
  } catch (error) {
...
await runDeploymentStep('Post-deployment Verification', async () => {
  ...
  try {
    ...
    verification.pm2Processes = processes.length > 0;
  } catch (error) {
...
const successfulSteps = deployment.steps.filter(step => step.status === 'success').length;
const failedSteps = deployment.steps.filter(step => step.status === 'error').length;
