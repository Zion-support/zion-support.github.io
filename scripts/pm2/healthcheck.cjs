  ...
  async checkSystemHealth() {
    try {
      ...
      const healthReport = {
        ...
        overall: this.calculateOverallHealth(diskUsage, memoryUsage, pm2Status, buildStatus)
      };
      
      // Save health report
      fs.writeFileSync(this.healthReport, JSON.stringify(healthReport, null, 2));
      
      this.log(`Health check completed. Overall health: ${healthReport.overall.status}`);
