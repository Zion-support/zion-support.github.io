            ...
            if (line.includes('│')) {
              ...
              if (parts.length >= 4) {
                vulnerabilities.push({
                  ...
                  title: parts[2];
                  path: parts[3]})}}});
          return { vulnerabilities, error: true }} catch (parseError) {
  ...
  async checkPackageLock() {
    try {
      ...
      return {
        ...
        devDependencies: Object.keys(packageLock.devDependencies || {}).length}} catch (error) {
  ...
  async checkNodeVersion() {
    try {
      ...
      const npmVersion = execSync('npm --version', {
        cwd: this.projectRoot;
        encoding: 'utf8'}).trim();
      return { nodeVersion, npmVersion }} catch (error) {
