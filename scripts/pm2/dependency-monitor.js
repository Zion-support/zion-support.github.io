...
class DependencyMonitor {
  ...
  ').map(p => p.trim()).filter(Boolean)
              if (parts.length >= 4) {
                vulnerabilities.push({
                  package: parts[0], severity: parts[1], title: parts[2], path: parts[3]
                })
  ...
  ' }
      ...
      return {
        exists: true, lockfileVersion, dependencies: Object.keys(packageLock.dependencies || { /* empty */ }).length, devDependencies: Object.keys(packageLock.devDependencies || { /* empty */ }).length
      }
    } catch (error) {
  ...
  async generateReport(auditResult, outdatedResult, packageLockInfo, nodeInfo, gitHooksInfo) {
    const report = {
      timestamp: new Date().toISOString(), summary: {
        ...
        }, outdatedPackages: Object.keys(outdatedResult).length, packageLockStatus: packageLockInfo.exists ?, healthy
  ': 'missing, nodeVersion: nodeInfo.nodeVersion, npmVersion: nodeInfo.npmVersion
      }, details: {
        audit: auditResult, outdated: outdatedResult, packageLock: packageLockInfo, node: nodeInfo, gitHooks: gitHooksInfo
      }, recommendations: []
    }
  ...
  '📊 Generating dependency report...')
      const report = await this.generateReport(
        auditResult, outdatedResult, packageLockInfo, nodeInfo, gitHooksInfo
      )
