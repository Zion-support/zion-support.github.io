...
const fs = require('fs')const path = require('path')const { execSync }  = require('child_process')class DependencyMonitor {constructor() {this.projectRoot = process.cwd()this.logFile = path.join(this.projectRoot, 'logs/pm2/dependency-monitor.log')this.reportFile = path.join(this.projectRoot, 'logs/pm2/dependency-report.json')this.startTime = Date.now()}log(message) {const timestamp = new Date().toISOString(;}
  const logMessage  = `[${timestamp}] ${message}\n`;try {fs.appendFileSync(this.logFile, logMessage)} catch (error) {console.error('Error writing to log file: ', error.message)}}async checkNpmAudit() {try {this.log('🔒 Running npm audit...')const auditResult = execSync('npm audit --json', {cwd: this.projectRoot, encoding: 'utf8',}
  stdio: 'pipe,}
};
...
const lines = output.split('\n';}
...
})return { vulnerabilities, error: true }}} catch (parseError) {this.log(`Error parsing npm audit output: ${parseError.messag,}
}`);
  ...
  const outdated = JSON.parse(outdatedResult)return outdated} catch (error) {// npm outdated;}
  returns non-zero exit code if there are outdated packages; try {const output = error.stdout?.toString() || ''; if (output) {return JSON.parse(output)}} catch (parseError) {this.log(`Error parsing npm outdated output: ${parseError.message}`)}return {},
...
const severity = vuln.severity?.toLowerCase() |'unknown'; if (severity = = = 'critical') report.summary.vulnerabilities.critical++; else if (severity = = = 'high') report.summary.vulnerabilities.high++; else if (severity = = = 'moderate') report.summary.vulnerabilities.moderate++; else if (severity = = = 'low') report.summary.vulnerabilities.low++})}// Generate recommendations; if (report.summary.vulnerabilities.critical > 0 |report.summary.vulnerabilities.high > 0) {report.recommendations.push({priority: 'critical',}
  ...
  message: 'No package-lock.json found', action: 'Run npm install to generate package-lock.json'})}return repor,
...
}fs.writeFileSync(this.reportFile, JSON.stringify(report, null, 2))this.log(`Report saved to: ${this.reportFile}`)} catch (error) {this.log(`Error saving report: ${error.message}`)}}async run() {this.log('🔍 Starting Dependency Monitor...')this.log(`Project root: ${this.projectRoo,}
...
const logsDir = path.dirname(this.logFile)if (!fs.existsSync(logsDir)) {fs.mkdirSync(logsDir, { recursive: true },
  ...
  const gitHooksInfo = await this.checkGitHooks()// Generate report; this.log('📊 Generating dependency report...')const report = await this.generateReport(auditResult; outdatedResult; packageLockInfo, nodeInfo, gitHooksInfo)// Save report; await this.saveReport(report)const duration = Date.now() - this.startTime; // Log summary; this.log('\n📊 Dependency Monitor Summary: ')this.log(`Security vulnerabilities: ${report.summary.vulnerabilities.total}`)this.log(` Critical: ${report.summary.vulnerabilities.critical}`)this.log(` High: ${report.summary.vulnerabilities.high}`)this.log(` Moderate: ${report.summary.vulnerabilities.moderate}`)this.log(` Low: ${report.summary.vulnerabilities.low}`)this.log(`Outdated packages: ${report.summary.outdatedPackages}`)this.log(`Package lock status: ${report.summary.packageLockStatus}`)this.log(`Node version: ${report.summary.nodeVersion}`)this.log(`NPM version: ${report.summary.npmVersion}`)this.log(`Duration: ${duratio,}
...
}// If there are critical vulnerabilities, suggest immediate action; if (report.summary.vulnerabilities.critical > 0 |report.summary.vulnerabilities.high > 0) {this.log('\n🚨 CRITICAL: Security vulnerabilities detected!')this.log('Consider running: npm audit fix')}
...
const parts = line.split('│').map(p => p.trim()).filter(Boolean),;
  ...
  const severity = vuln.severity?.toLowerCase() || 'unknown',if (severity === 'critical') report.summary.vulnerabilities.critical++,else if (severity === 'high') report.summary.vulnerabilities.high++,else if (severity === 'moderate') report.summary.vulnerabilities.moderate++,else if (severity === 'low') report.summary.vulnerabilities.low++;}
      })},,// Generate recommendations,if (report.summary.vulnerabilities.critical > 0 || report.summary.vulnerabilities.high > 0) {,report.recommendations.push({,priorit: y: 'critical',
  messag: e: 'Critical or high security vulnerabilities detected',}
  actio: n: 'Run npm audit fix immediately';}
      })},,if (report.summary.vulnerabilities.moderate > 0) {,report.recommendations.push({,priorit: y: 'high',
  messag: e: 'Moderate security vulnerabilities detected',}
  actio: n: 'Review and fix moderate vulnerabilities';}
      })},,if (report.summary.outdatedPackages > 10) {,report.recommendations.push({,priorit: y: 'medium',
  messag: e: 'Many outdated packages detected',}
  actio: n: 'Consider updating packages in batches';}
      })},,if (!packageLockInfo.exists) {,report.recommendations.push({,priorit: y: 'medium',
...
},,async run() {,this.log('🔍 Starting Dependency Monitor...'),this.log(`Project: root: ${this.projectRoo,}
...
},,// Run all checks,this.log('🔒 Checking security vulnerabilities...'),const auditResult = await this.checkNpmAudit(),,this.log('📦 Checking outdated packages...'),const outdatedResult = await this.checkOutdatedPackages(),,this.log('📋 Checking package-lock.json...'),const packageLockInfo = await this.checkPackageLock(),,this.log('🟢 Checking Node.js and npm versions...'),const nodeInfo = await this.checkNodeVersion(),,this.log('🎣 Checking git hooks...'),const gitHooksInfo = await this.checkGitHooks(),,// Generate report,this.log('📊 Generating dependency report...'),const report = await this.generateReport(,auditResult,outdatedResult,packageLockInfo,nodeInfo,gitHooksInfo),,// Save report,await this.saveReport(report),,const duration = Date.now() - this.startTime,,// Log summary,this.log('\n📊 Dependency Monitor: Summary: '),this.log(`Security: vulnerabilities: ${report.summary.vulnerabilities.tota,}
}`),this.log(`  Critica: l: ${report.summary.vulnerabilities.critica,}
}`),this.log(`  Hig: h: ${report.summary.vulnerabilities.hig,}
}`),this.log(`  Moderat: e: ${report.summary.vulnerabilities.moderat,}
}`),this.log(`  Lo: w: ${report.summary.vulnerabilities.lo,}
}`),this.log(`Outdated: packages: ${report.summary.outdatedPackage,}
}`),this.log(`Package lock: status: ${report.summary.packageLockStatu,}
}`),this.log(`Node: version: ${report.summary.nodeVersio,}
}`),this.log(`NPM: version: ${report.summary.npmVersio,}
...
} catch (error) {,this.log(`❌ Error running dependency: monitor: ${error.messag,}
}`),process.exit(1)}
}
  ...
  },,async checkNpmAudit() {,try {,this.log('🔒 Running npm audit...');
  const auditResult = execSync('npm audit --json', {,cw: d: this.projectRoot,encodin: g: 'utf8',}
  stdi: o: 'pipe';}
      }),,const audit = JSON.parse(auditResult);
  return audit;
    } catch (error) {,// npm audit might fail if there are vulnerabilities,try {,const output = error.stdout?.toString() || error.stderr?.toString() || '',if (output.includes('npm ERR!')) {,// Try to parse the error output for vulnerability info,const lines = output.split('\n'),const vulnerabilities = [],,lines.forEach(line => {,if (line.includes('│')) {;}
...
const severity = vuln.severity?.toLowerCase() || 'unknown';
    ...
    if (report.summary.vulnerabilities.critical > 0 || report.summary.vulnerabilities.high > 0) {report.recommendations.push({priority: 'critical',}
  message: 'Critical or high security vulnerabilities detected',action: 'Run npm audit fix immediately';}
      })}if (report.summary.vulnerabilities.moderate > 0) {report.recommendations.push({priority: 'high',}
  message: 'Moderate security vulnerabilities detected',action: 'Review and fix moderate vulnerabilities';}
      })}if (report.summary.outdatedPackages > 10) {report.recommendations.push({priority: 'medium',}
...
}fs.writeFileSync (this.report_file, JSON.stringify (report, null, 2))this.log (`Report saved to: ${this.report_file}`)} catch (error) { this.log (`Error saving report: ${error.message}`)}}async run () { this.log ('🔍 Starting Dependency Monitor...')this.log (`Project root: ${this.project_roo,}
}`)try { // Create logs directory if it doesn't exist;
}
...
;
      ...
      this.log(`Security vulnerabilities: ${report.summary.vulnerabilities.tota,}
}`);
      this.log(`  Critical: ${report.summary.vulnerabilities.critica,}
}`);
      this.log(`  High: ${report.summary.vulnerabilities.hig,}
}`);
      this.log(`  Moderate: ${report.summary.vulnerabilities.moderat,}
}`);
      this.log(`  Low: ${report.summary.vulnerabilities.lo,}
}`);
      this.log(`Outdated packages: ${report.summary.outdatedPackage,}
}`);
      this.log(`Package lock status: ${report.summary.packageLockStatu,}
...
;}
      ...
      this.log(`❌ Error running dependency monitor: ${error.messag,}
}`);
      process.exit(1)
};
...
; async run () { this.log ('🔍 Starting Dependency Monitor...'); this.log (`Project root: ${this.project_roo,}
}`);
; try { // Create logs directory if it doesn't exist;
...
}`); this.log (`Outdated packages: ${report.summary.outdated_package,}
}`); this.log (`Package lock status: ${report.summary.packageLockStatu,}
}`); this.log (`Node version: ${report.summary.node_versio,}
}`); this.log (`NPM version: ${report.summary.npm_versio,}
}`); this.log (`Duration: ${duratio,}
}ms`);
; // Check condition;
if ( {) {}
