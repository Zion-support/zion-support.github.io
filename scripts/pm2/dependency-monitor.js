...
class DependencyMonitor {
  constructor() {
    ...
    this.logFile = path.join(
      this.projectRoot,
      'logs/pm2/dependency-monitor.log'
    );
    this.reportFile = path.join(
      this.projectRoot,
      'logs/pm2/dependency-report.json'
    );
  ...
  async checkGitHooks() {
    try {
      ...
      const activeHooks = hooks.filter(hook => {
        ...
        return (
          stats.isFile() && (hook.endsWith('.sample') || stats.mode & 0o111)
        );
      });
  ...
  async generateReport(
    ...
    if (
      report.summary.vulnerabilities.critical > 0 ||
      report.summary.vulnerabilities.high > 0
    ) {
  ...
  async run() {
    ...
    try {
      ...
      this.log(
        `Security vulnerabilities: ${report.summary.vulnerabilities.total}`
      );
