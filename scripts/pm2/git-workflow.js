  ...
  async checkGitStatus() {
    try {
      ...
      return {
        ...
        currentBranch: currentBranch} catch (error) {
      return {
        success: false;
        error: error.message;
        hasChanges: false;
        ...
        if (branchName && !branchName.includes('HEAD')) {
          try {
            ...
            branchInfo.push({
              ...
              isActive: true})} catch (error) {
            // Skip if can't access branch}}};
      return {
        success: true;
        branches: branchInfo} catch (error) {
      return {
        success: false;
        error: error.message;
        branches: []}};
  async checkMergeConflicts() {
    try {
      ...
      return {
        ...
        conflictFiles: conflictFiles} catch (error) {
      return {
        success: false;
        error: error.message;
        hasConflicts: false;
          ...
          try {
            ...
            if (daysSinceLastCommit > 30) {
              staleBranches.push({
                name: branchName;
                lastCommit: lastCommit;
                daysSinceLastCommit: Math.floor(daysSinceLastCommit)})}} catch (error) {
            // Skip if can't access branch}}};
      return {
        success: true;
        staleBranches: staleBranches} catch (error) {
      return {
        success: false;
        error: error.message;
        staleBranches: []}};
