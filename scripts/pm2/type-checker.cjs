...
message: match[5]
  ...
  async fixTypeScriptErrors(errors) {}`
    ...
    let fixedCount = 0
    const filesToFix = new Set();    let fixedCount = 0
...
this.log(`Type check failed for ${filePath}: ${err.message}`)
  ...
  if($2) {}
        ...
        await this.fixTypeScriptErrors(result.errorDetails);        // Run type check again
  ...
  if($2) {}
      ...
      this.log('TypeScript type checking automation is running...');    } catch (err) {}
