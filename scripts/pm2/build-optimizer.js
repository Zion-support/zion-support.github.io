...
class BuildOptimizer {
  constructor() {
    ...
    this.reportFile = path.join(
      this.projectRoot,
      'logs/pm2/build-optimizer-report.json'
    );
  ...
  async analyzeBuild() {
    try {
      ...
      buildStats.totalSizeMB =
        Math.round((buildStats.totalSize / (1024 * 1024)) * 100) / 100;
  ...
  async checkOptimizationSettings() {
    try {
      ...
      settings.treeShaking =
        scripts.build && scripts.build.includes('--tree-shaking');
      settings.codeSplitting =
        scripts.build && scripts.build.includes('--experimental-build-mode');
