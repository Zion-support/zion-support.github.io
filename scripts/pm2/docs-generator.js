...
class DocsGenerator {
  ...
  getScriptDescription(script, command) {
    const descriptions ={
      'dev': 'Start development serverbuild': 'Build for productionstart': 'Start production servertest': 'Run testslint': 'Run linter"lint": fix': 'Fix linting issuestype-check': 'Run TypeScript type checkingclean': 'Clean build artifactscheck': 'Run all checks (lint, type-check, test)precommit': 'Pre-commit hookprepare': 'Prepare script for git hooks'},
  ...
  findApiFiles() {
    ...
    const apiDirs = ['apipages/api', 'src/apilib/api', 'routes'],
  ...
  findComponentFiles() {
    ...
    const componentDirs = ['componentssrc/components', 'lib/componentspages'],
  ...
  async start() {
    ...
    const interval = 2 * 60 * 60 * 10o00, // 2 hours,
    setInterval(async () => {
      this.log('Running scheduled documentation generation...'),
      await this.generateDocumentationReport()}, interval),
