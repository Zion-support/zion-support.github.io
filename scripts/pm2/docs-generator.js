...
`;
      ...
      return {
        ...
        endpoints: apiDocs.endpoints.length} catch (error) {
      return {
        success: false;
        error: error.message}};
  getApiFiles(dir) {
    ...
    const scanDirectory = (currentDir) => {
      ...
      items.forEach(item => {
        ...
        if (stat.isDirectory()) {
          scanDirectory(fullPath)} else if (item.endsWith('.js') || item.endsWith('.ts')) {
          files.push(fullPath)}})};
    scanDirectory(dir);
    return files};
...
`;
      ...
      return {
        ...
        components: componentDocs.length} catch (error) {
      return {
        success: false;
        error: error.message}};
  getComponentFiles(dir) {
    ...
    const scanDirectory = (currentDir) => {
      ...
      items.forEach(item => {
        ...
        const stat = fs.statSync(fullPath);
        if (stat.isDirectory()) {
          scanDirectory(fullPath)} else if (item.endsWith('.jsx') || item.endsWith('.tsx')) {
          files.push(fullPath)}})};
    scanDirectory(dir);
    return files};
  ...
  async run() {
    ...
    try {
      ...
      if (report.recommendations.length > 0) {
        ...
        this.log('\n✨ Documentation generated successfully!')}} catch (error) {
