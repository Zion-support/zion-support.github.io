...
class DocsGenerator {
  constructor() {
    ...
    this.reportFile = path.join(
      this.projectRoot,
      'logs/pm2/docs-generator-report.json'
    );
...
`;
  ...
  extractApiEndpoint(filePath, content) {
    ...
    const path =
      relativePath.replace(/\.(js|ts)$/, '').replace(/\/index$/, '') || '/';
...
`;
  ...
  async run() {
    ...
    try {
      ...
      this.log(
        `README: ${report.summary.readmeGenerated ? 'Generated' : 'Failed'}`
      );
      this.log(
        `API Docs: ${report.summary.apiDocsGenerated ? 'Generated' : 'Failed'}`
      );
      this.log(
        `Component Docs: ${report.summary.componentDocsGenerated ? 'Generated' : 'Failed'}`
      );
