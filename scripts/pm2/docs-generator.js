...
---;
Generated by Docs Generator on $ {
  new Date () .toISOString ();
`;
;
...
const props = [];
const propMatch = content.match (/interface\s+\w+Props\s* {
  ([\s\S]*?) 
}/);
if (propMatch) {
  ;
...
this.log (`README: $ {
  report.summary.readmeGenerated ? 'Generated': 'Failed' 
}`);
this.log (`API Docs: $ {
  report.summary.apiDocsGenerated ? 'Generated': 'Failed' 
}`);
this.log (`Component Docs: $ {
  report.summary.componentDocsGenerated ? 'Generated': 'Failed' 
}`);
this.log (`Total Endpoints: $ {
  report.summary.totalEndpoints 
}`);
this.log (`Total Components: $ {
...
this.log ('\n✨ Documentation generated successfully!') 
  ...
  ;
this.log (`❌ Error running docs generator: $ {
  error.message 
}`);
process.exit (1) 
}
