...
const checkMemoryUsage = () => {}
  log('Checking memory usage');  try {}
    ...
    log(`Memory usage: ${memUsagePercent.toFixed(2)}% (${usedMem}MB / ${totalMem}MB)`);    const totalMem = parseInt(memLine[1])
    const usedMem = parseInt(memLine[2])
    const memUsagePercent = (usedMem / totalMem) * 100
    `
...
const main = async () => {}
  ...
  const memoryResults = checkMemoryUsage();  return report
const main = async () => {}
  log('Starting Performance Monitor Process')
  // Run performance checks
  const buildResults = checkBuildPerformance()
  ...
  const report = generatePerformanceReport(results);  const report = generatePerformanceReport(results)
