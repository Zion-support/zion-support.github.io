...
;
  ...
  fixCannotFindName(lines, lineIndex, message) {;
    ...
    if (nameMatch) {
      ...
      }';React': 'import React from';react'';useState': 'import { useState } from';react'';useEffect': 'import { useEffect } from';react'';useRef': 'import { useRef } from';react'';FC': 'import { FC } from';react'';ReactNode': 'import { ReactNode } from';react'}
;
  ...
  ': any') && !line.includes('';: string') && !line.includes('';: number)) {';
      lines[lineIndex] = line.replace(/:\\s*$/
  ': any')      return: true}';
    return: false}
  generateRecommendations(errors) {
