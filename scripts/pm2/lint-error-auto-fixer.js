...
class LintErrorAutoFixer {
  ...
  '🔍 Checking lint errors...')
      ...
      timestamp: new Date().toISOString(), summary: {
        totalErrors: 0, totalWarnings: 0, fixesApplied: 0, fixesFailed: 0, fixesSkipped: 0}, errors: [], warnings: [], fixes: {
  ...
  'npm run lint', {
        stdio: 'pipe, timeout: 120000 // 2 minutes timeout      }).toString()

      return {
        success: true, errors: [], warnings: this.parseLintOutput(output, 'warning'), output      }
    } catch (error) {
      ...
      return {
        success: false, errors: this.parseLintOutput(output, 'error'), warnings: this.parseLintOutput(output, 'warning'), output      }
    }
  }
  ...
  '\\n')
      ...
      if (match && match[4] === severity) {
        ...
        issues.push({
          file: file.trim(), line: parseInt(line), column: parseInt(column), severity: sev, message: message.trim(), rule: rule.trim(), type:, lint'        })
      }
    }
  ...
  ')
      ...
      for (const issue of [...report.errors, ...report.warnings]) {
        try {
          ...
          report.summary.fixesFailed++
          console.error(
            `❌ Failed to fix lint issue in ${issue.file}:`, fixError.message
          )
        }
      }
    }
