...
const lines = content && content.split('\n')lines && lines.forEach((line, index) = > ;
...
}// Long lines (over 120 characters)if (line && line.length > 120) {analysis && analysis.issues.push({line: lineNum, type: 'long-line', message: `Line is ${line && line.lengt,
  ...
  const importName = importMatch[1]; if (importName ! = = 'React' && !content && content.includes(importName)) {analysis && analysis.issues.push({line: lineNum, type: 'unused-import', message: `Potentially unused import ${importNam,
...
},// Unused imports (basic check),if (line.match(/^import.*from/) && !line.includes('//')) {,const importMatch = line.match(/import\s+(\w+)/),if (importMatch) {,const importName = importMatch[1],if (importName !== 'React' && !content.includes(importName)) {,analysis.issues.push({,line: lineNum,type: 'unused-import',message: `Potentially unused import ${importNam,
}`,severity: 'medium'})}},
...
if ( {) {$2;
} analysis.issues.push ({ line: line_num, type: 'long - line', message: `Line is ${line.lengt,
} characters long (max: 120)`,severity: 'medium'},
...
} analysis.issues.push ({ line: line_num, type: 'unused - import', message: `Potentially unused import ${import_nam,
...
const monitor = new CodeQualityMonitor ()monitor.run ().catch (error = > { process.exit (1)})monitor.run().catch(error = > {process.exit(1)})process.exit(1)})}
  ...
  const lineNum = index + 1,,// Trailing spaces,if (line.match(/[ \t]+$/)) {,analysis.issues.push({,lin: e: lineNum,typ: e: 'trailing-spaces',messag: e: 'Trailing spaces found',severit: y: 'low';
          })},,// Long lines (over 120 characters),if (line.length > 120) {,analysis.issues.push({,lin: e: lineNum,typ: e: 'long-line',messag: e: `Line is ${line.lengt,
  ...
  const importName = importMatch[1],if (importName !== 'React' && !content.includes(importName)) {,analysis.issues.push({,lin: e: lineNum,typ: e: 'unused-import',messag: e: `Potentially unused import ${importNam,
}`,severit: y: 'medium';
              })}
          }
        }
