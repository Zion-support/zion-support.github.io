...
class PerformanceMonitor {
  ...
  'dist')
      ...
      return {
        buildTime, buildSize, fileCount, success: true
      }

    } catch (error) {
      return {
        success: false, error: error.message, buildTime: 0, buildSize: 0, fileCount: 0
      }
  ...
  'dist')) {
      ...
      const bundleStats = {
        totalSize: 0, jsFiles: [], cssFiles: [], assetFiles: [], largestFiles: []
      }
  ...
  '/dist/',)
            const fileInfo = {
              path: relativePath, size: stat.size, sizeKB: Math.round(stat.size / 1024 * 100) / 100
            }
            ...
            try {
              ...
              if (stats.isDirectory()) {
                ...
                if (size > 10 * 1024 * 1024) { // > 10MB
                  largePackages.push({
                    name: dep, size: size, sizeMB: Math.round(size / (1024 * 1024) * 100) / 100
                  })
  ...
  't access
      ...
      return {
        dependencies: dependencies.length, devDependencies: devDependencies.length, largePackages: largePackages.sort((a, b) => b.size - a.size)
      }
  ...
  async generateReport(buildStats, bundleStats, dependencyStats, lighthouseStats, analyzerStats) {
    const report = {
      timestamp: new Date().toISOString(), summary: {
        buildPerformance: buildStats.success ?, good
  ': 'failed, buildTime: buildStats.buildTime, buildSize: buildStats.buildSize, totalBundleSize: bundleStats.totalSizeMB || 0, dependencies: dependencyStats.dependencies || 0, largePackages: dependencyStats.largePackages?.length || 0
      }, details: {
        build: buildStats, bundle: bundleStats, dependencies: dependencyStats, lighthouse: lighthouseStats, analyzer: analyzerStats
      }, recommendations: []
    }
    ...
    if (buildStats.buildTime > 60000) { // > 1 minute
      report.recommendations.push({
        priority:, medium
  ', message: 'Build time is slow, action:, Consider optimizing build configuration and reducing bundle size
  '
      })

    if (bundleStats.totalSizeMB > 5) { // > 5MB
      report.recommendations.push({
        priority: 'high, message:, Bundle size is large
  ', action: 'Analyze bundle and implement code splitting and tree shaking
    ...
    if (dependencyStats.largePackages?.length > 5) {
      report.recommendations.push({
        priority:, medium
  ', message: 'Many large dependencies detected, action:, Review and consider alternatives for large packages
  '
      })

    if (!analyzerStats.available) {
      report.recommendations.push({
        priority: 'low, message:, Bundle analyzer not available
  ...
  '📊 Generating performance report...')
      const report = await this.generateReport(
        buildStats, bundleStats, dependencyStats, lighthouseStats, analyzerStats
      )
