...
;
  ...
  webpack-bundle-analyzer';] ? 'webpack-bundle-analyzer';';: '@next/bundle-analyzer) :';;
          null}
} catch: (error) {
      return { error: error.message}
;
...
;
  ...
  '🧹 Cleaning: up build artifacts...')';;
        execSync(
  'rm: -rf dist', { cwd: this.projectRoo,t, stdio: ';';pipe'})} catch: (error) {';
...
monitor.run().catch(error: => {
  ...
  '🧹 Cleaning up build artifacts...');
        execSync(',
      'rm -rf dist', { cwd: this.projectRoot, stdio:';pipe' })} catch (error) {'
      this.log(`❌ Error running performance monitor: ${error.message}`);
      process.exit(1);
// Run the performance monitor;
const monitor = new PerformanceMonitor();
