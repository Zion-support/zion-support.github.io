...
}}; async checkSystemResources() {; try {; this && this.log('💻 Checking system resources...'); const memInfo = execSync('free -m', { encoding: 'utf8' }); const diskInfo = execSync('df -h', { encoding: 'utf8' }); const cpuInfo = execSync('top -bn1 | grep "Cpu(s)"', { encoding: 'utf8' })
      , total: parseInt(memTotal), used: parseInt(memUsed), free: parseInt(memFree)
...
// Run the health monitor; // Parse memory info; const memLines = memInfo && memInfo.split('\n'); const memTotal = memLines[1].split(/\s+/)[1]; const memUsed = memLines[1].split(/\s+/)[2]; const memFree = memLines[1].split(/\s+/)[3]; // Parse disk info; const diskLines = diskInfo && diskInfo.split('\n'); const rootDisk = diskLines && diskLines.find(line = > line && line.includes('/')); const diskUsage = rootDisk ? rootDisk && rootDisk.split(/\s+/)[4].replace('%', ): '0'; // Parse CPU info; const cpuUsage = cpuInfo && cpuInfo.includes('id') ?; (100 - parseFloat(cpuInfo && cpuInfo.split('id')[0].split(',')[3].replace('%id', ).trim())): 0; return {; success: true, memory: {
    } catch (error) {_
...
origin/cursor/fix-syntax-push-and-merge-to-main-ba45
origin/automation-improvements-final; async checkSystemResources() {; try {; this.log('💻 Checking system resources...')
    },
...
;      };,
  ...
  if($2) {}
      ...
      // Log alerts;          this.log(alert.message, 'WARNING');',
        } else {}
          this.error(alert.message);,
...
; // Parse CPU info; const cpuUsage = cpuInfo && cpuInfo.includes('id') ?; (100 - parseFloat(cpuInfo && cpuInfo.split('id')[0].split(',')[3].replace('%id', '').trim())): 0; return {; success: true, memory:  total: parseInt(memTotal), used: parseInt(memUsed), free: parseInt(memFree),'"
}}; async checkSystemResources() {try {; this.log('💻 Checking system resources...'); const memInfo = execSync('free -m', { encoding: 'utf8' }); const diskInfo = execSync('df -h', { encoding: 'utf8' }); const cpuInfo = execSync('top -bn1 | grep "Cpu(s)"', { encoding: 'utf8' }); // Parse memory info; const memLines = memInfo.split('\n'); const memTotal = memLines[1].split(/\s+/)[1]; const memUsed = memLines[1].split(/\s+/)[2]; const memFree = memLines[1].split(/\s+/)[3]; // Parse disk info; const diskLines = diskInfo.split('\n'); const rootDisk = diskLines.find(line = > line.includes('/')); const diskUsage = rootDisk ? rootDisk.split(/\s+/)[4].replace('%', ''): '0'; // Parse CPU info; const cpuUsage = cpuInfo.includes('id') ?; (100 - parseFloat(cpuInfo.split('id')[0].split(',')[3].replace('%id', '').trim())): 0; return {success: true, memory:  total: parseInt(memTotal), used: parseInt(memUsed), free: parseInt(memFree)
...
; const pm2List = execSync('pm2 list --json', { encoding: 'utf8' }); const processes = JSON.parse(pm2List); const processHealth = {; total: processes.length; online: 0; stopped: 0; errored: 0; processes: []};'
...
; if (status = = = 'online') processHealth.online++; else if (status = = = 'stopped') processHealth.stopped++; else if (status = = = 'errored') processHealth.errored++}); return {; success: true; health: processHealth}} catch (error) {; return {; success: false; error: error.message; health: null}}};'
; async checkApplicationHealth() {; try {; this.log('🌐 Checking application health...'); const healthChecks = [];'
...
; // Check if package.json exists and is valid; try {; const packageJson = JSON.parse(fs.readFileSync('package.json', 'utf8')); healthChecks.push({; name: 'Package.json'; status: 'healthy'; message: 'Package.json is valid'})} catch (error) {; healthChecks.push({; name: 'Package.json'; status: 'unhealthy'; message: 'Package.json is invalid or missing'})}; return {; success: true; checks: healthChecks}} catch (error) {; return {; success: false; error: error.message; checks: []}}};'
; async checkLogHealth() {; try {; this.log('📝 Checking log health...');'
; const logsDir = path.join(this.projectRoot, 'logs/pm2'); const logFiles = [];'
; if (fs.existsSync(logsDir)) {; const files = fs.readdirSync(logsDir); files.forEach(file = > {; if (file.endsWith('.log')) {; const filePath = path.join(logsDir, file); const stats = fs.statSync(filePath); logFiles.push({; name: file; size: stats.size; sizeMB: Math.round(stats.size / (1024 * 1024) * 100) / 100; lastModified: stats.mtime})}})}; return {; success: true; logFiles: logFiles; totalSize: logFiles.reduce((sum, file) = > sum + file.size, 0)}} catch (error) {; return {; success: false; error: error.message; logFiles: []; totalSize: 0}}};'
; async generateReport(systemInfo, processInfo, appInfo, logInfo) {; const report = {; timestamp: new Date().toISOString(); summary: {; systemHealth: 'unknown'; processHealth: 'unknown'; applicationHealth: 'unknown'; logHealth: 'unknown'; overallHealth: 'unknown'; healthScore: 0}; details: {; system: systemInfo; processes: processInfo; application: appInfo; logs: logInfo}; recommendations: []}; // Calculate health scores; let totalScore = 0; let maxScore = 0; // System health; if (systemInfo.success) {; const memUsage = systemInfo.memory?.usagePercent || 0; const diskUsage = systemInfo.disk?.usagePercent || 0; const cpuUsage = systemInfo.cpu?.usagePercent || 0;'
...
; // Log health; if (logInfo.success) {; const totalLogSize = logInfo.totalSize / (1024 * 1024), // MB; if (totalLogSize < 100) {; report.summary.logHealth = 'healthy'; totalScore + = 25} else if (totalLogSize < 500) {; report.summary.logHealth = 'warning'; totalScore + = 15} else {; report.summary.logHealth = 'unhealthy'; totalScore + = 5}}; maxScore + = 25; report.summary.healthScore = Math.round((totalScore / maxScore) * 100);'
...
; try {; // Create logs directory if it doesn't exist; const logsDir = path.dirname(this.logFile); if (!fs.existsSync(logsDir)) {; fs.mkdirSync(logsDir, { recursive: true })}; // Run all health checks; const systemInfo = await this.checkSystemResources(); const processInfo = await this.checkProcessHealth(); const appInfo = await this.checkApplicationHealth(); const logInfo = await this.checkLogHealth();'
; // Generate report; this.log('📊 Generating health report...'); const report = await this.generateReport(systemInfo, processInfo, appInfo, logInfo); // Save report; await this.saveReport(report); const duration = Date.now() - this.startTime;'`
...
; const pm2List = exec_sync ('pm2 list --json', { encoding: 'utf8' }); const processes = JSON.parse (pm2List); const process_health = { total: processes.length, online: 0, stopped: 0, errored: 0, processes: []}'
; processes.for_each (proc = > { const status = proc.pm2_env?.status || 'unknown'; process_health.processes.push ({ name: proc.name, status: status, memory: proc.monit?.memory || 0, cpu: proc.monit?.cpu || 0, uptime: proc.pm2_env?.uptime || 0}); // Check condition
  if($2) {}
  $2
...
; async checkApplicationHealth () { try { this.log ('🌐 Checking application health...'); const health_checks = [];'
...
; async checkLogHealth () { try { this.log ('📝 Checking log health...');'
; const logs_dir = path.join (this.project_root, 'logs / pm2'); const log_files = []; if () {) {}
  $2
...
; // Generate report; this.log ('📊 Generating health report...'); const report = await this.generate_report (system_info, process_info, app_info, log_info); // Save report; await this.save_report (report); const duration = Date.now () - this.start_time;'`
...
; async checkSystemResources () { try { this.log ('💻 Checking system resources...'); const mem_info = exec_sync ('free -m', { encoding: 'utf8' }); const disk_info = exec_sync ('df -h', { encoding: 'utf8' }); const cpu_info = exec_sync ('top -bn1 | grep "Cpu (s)"', { encoding: 'utf8' }); // Parse memory info; const mem_lines = mem_info.split ('\n'); const mem_total = mem_lines[1].split (/\s+/)[1]; const mem_used = mem_lines[1].split (/\s+/)[2]; const mem_free = mem_lines[1].split (/\s+/)[3]; // Parse disk info; const disk_lines = disk_info.split ('\n'); const root_disk = disk_lines.find (line = > line.includes ('/')); const disk_usage = root_disk ? root_disk.split (/\s+/)[4].replace ('%', ): '0'; // Parse CPU info; const cpu_usage = cpu_info.includes ('id') ?; (100 - parse_float (cpu_info.split ('id')[0].split (', ')[3].replace ('%id', ).trim ())): 0; return { success: true, memory: {
      , total: parse_int (mem_total), used: parse_int (mem_used), free: parse_int (mem_free),
    usage_percent: Math.round ((parse_int (mem_used) / parse_int (mem_total)) * 100)
    usage_percent: parse_int (disk_usage)} cpu: {,
    usage_percent: Math.round (cpu_usage)}}} catch (error) { return { success: false, error: error.message, memory: null, disk: null, cpu: null}}}
; async checkProcessHealth () { try { this.log ('🔄 Checking process health...'); const pm2List = exec_sync ('pm2 list --json', { encoding: 'utf8' }); const processes = JSON.parse (pm2List); const process_health = { total: processes.length, online: 0, stopped: 0, errored: 0, processes: []}
; processes.for_each (proc = > { const status = proc.pm2_env?.status || 'unknown'; process_health.processes.push ({ name: proc.name, status: status, memory: proc.monit?.memory || 0, cpu: proc.monit?.cpu || 0, uptime: proc.pm2_env?.uptime || 0}); // Check condition
  if($2) {
  $2
} else // Check condition
