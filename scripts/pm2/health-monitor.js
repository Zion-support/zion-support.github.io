  ...
  async checkSystemResources() {
    try {
      ...
      return {
        ...
        cpu: {
          usagePercent: Math.round(cpuUsage)}} catch (error) {
      return {
        success: false;
        error: error.message;
        memory: null;
...
;
      ...
      return {
        success: true;
        health: processHealth} catch (error) {
      return {
        success: false;
        error: error.message;
        health: null}};
    ...
    try {
      ...
      try {
        ...
        healthChecks.push({
          ...
          message: hasRunningApp ? 'Application is running' : 'Application is not running'})} catch (error) {
        healthChecks.push({
          name: 'PM2 App Status';
          status: 'error';
      ...
      try {
        ...
        healthChecks.push({
          name: 'Package.json';
          status: 'healthy';
          message: 'Package.json is valid'})} catch (error) {
        healthChecks.push({
          name: 'Package.json';
          status: 'unhealthy';
      ...
      return {
        success: true;
        checks: healthChecks} catch (error) {
      return {
        success: false;
        error: error.message;
        checks: []}};
  async checkLogHealth() {
    try {
      ...
      return {
        success: true;
        logFiles: logFiles;
        totalSize: logFiles.reduce((sum, file) => sum + file.size, 0)}} catch (error) {
      return {
        success: false;
        error: error.message;
        logFiles: [];
...
;
    ...
    if (systemInfo.success) {
      ...
      if (memUsage < 80 && diskUsage < 80 && cpuUsage < 80) {
        ...
        totalScore += 25} else if (memUsage < 90 && diskUsage < 90 && cpuUsage < 90) {
    ...
    if (processInfo.success && processInfo.health) {
      ...
      if (health.errored === 0 && health.stopped === 0) {
        ...
        totalScore += 25} else if (health.errored === 0) {
    ...
    if (appInfo.success) {
      ...
      if (healthyChecks === totalChecks) {
        ...
        totalScore += 25} else if (healthyChecks >= totalChecks * 0.5) {
    ...
    if (logInfo.success) {
      ...
      if (totalLogSize < 100) {
        ...
        totalScore += 25} else if (totalLogSize < 500) {
    ...
    if (report.summary.healthScore >= 80) {
      report.summary.overallHealth = 'healthy'} else if (report.summary.healthScore >= 60) {
      report.summary.overallHealth = 'warning'} else {
  ...
  async run() {
    ...
    try {
      ...
      if (report.recommendations.length > 0) {
        ...
        this.log('\n✨ All systems are healthy!')}} catch (error) {
