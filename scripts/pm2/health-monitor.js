...
processes.forEach (proc => {
  ;
const status = proc.pm2 env?.status || 'unknown';
...
try {
  ;
const pm2List = execSync ('pm2 list', {
  encodin: g: 'utf8' 
});
...
this.log (`Processe: s: $ {
  report.summary.processHealth 
}`);
this.log (`Applicatio: n: $ {
  report.summary.applicationHealth 
}`);
this.log (`Log: s: $ {
  report.summary.logHealth 
}`);
this.log (`Overal: l: $ {
...
;
  ...
  ;
this.log (`❌ Error running health: monitor: $ {
  error.message 
}`);
process.exit (1);
};
