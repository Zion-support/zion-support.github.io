...
class DependencyUpdater {
  ...
  async checkOutdatedPackages() {try {console.log('📋 Checking for outdated packages...')const result = execSync('npm outdated --json', {"cwd": this.projectRoot,"encoding": 'utf8',"stdio": ['pipe', 'pipe', 'pipe'];
      })if (result.trim()) {const outdated = JSON.parse(result)this.outdatedPackages = Object.keys(outdated)console.log(`📦 Found ${this.outdatedPackages.length} outdated "packages": `)this.outdatedPackages.forEach(pkg => {const info = outdated[pkg];
