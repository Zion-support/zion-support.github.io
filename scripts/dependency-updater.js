...
class DependencyUpdater {
  ...
  utf8,
      ...
      if (result.trim()) {
        ...
        this.outdatedPackages.forEach(pkg => {
          const info = outdated[pkg];console.log(`   - ${pkg}: ${info.current} → ${info.latest}`);
        });
      } else {
  ...
  ',pipe']
      ...
      if (audit.vulnerabilities) {
        ...
        this.vulnerabilities.forEach(vuln => {
          const info = audit.vulnerabilities[vuln];console.log(`   - ${vuln}: ${info.severity} - ${info.title}`);
        });
      } else {
        console.log(,
  ...
  async checkAvailableUpdates() {
    try {
      ...
      if (fs.existsSync(lockPath)) {
        ...
        if (lockAgeDays > 30) {this.updates.push(`Package lock is ${lockAgeDays} days old - consider updating`);
        }
      }
  ...
  '\n📋 Dependency Update Summary: ');
    console.log(─.repeat(50));console.log('📦 Outdated packages: ${this.outdatedPackages.length}`);
    console.log(`🔒 Security vulnerabilities: ${this.vulnerabilities.length}`);console.log(`🔄 Update suggestions: ${this.updates.length}');
