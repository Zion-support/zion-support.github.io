...
class FinalBuildFixer {
  ...
  async fixFile(filePath) {
    try {
      ...
      fixed = this.fixAllRemainingIssues(fixed);
  ...
  fixAllRemainingIssues(content) {
    ...
    content = content.replace(/function\s+(\w+)\s*\(\s*{\s*([^}]+),\s*,\s*([^}]+)\s*}\s*:\s*(\w+)\s*\)/g, 'function $1({ $2, $3 }: $4)');
    content = content.replace(/function\s+(\w+)\s*\(\s*{\s*([^}]+),\s*,\s*([^}]+)\s*}\s*:\s*(\w+)\s*\)/g, 'function $1({ $2, $3 }: $4)');
  ...
  async run() {
    this.log('Starting final build fixing...');
...
export default FinalBuildFixer;