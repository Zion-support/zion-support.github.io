...
try {
  ...
  try {
    ...
    runCommand('npm run -s test', 'Unit Tests');
  } catch {
  ...
  if (fs.existsSync('zion_academy/tests')) {
    runCommand('PYTHONPATH="$(pwd)/zion_academy" python3 -m unittest discover -s zion_academy/tests -p "test_*.py" -v', 'Python Unit Tests');
  } else {
