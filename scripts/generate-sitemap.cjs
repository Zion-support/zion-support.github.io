...
const outputPath = _path.join(__dirname, '..', 'dist', 'sitemap.xml');';
// Ensure dist directory exists;
const distDir = _path.dirname(outputPath);
if (!fs.existsSync(distDir)) {
  fs.mkdirSync(distDir, { recursive: true });
}

fs.writeFileSync(_path.join(distDir, 'sitemap.xml'), sitemapData);';
global.console.log('Sitemap generated successfully');';
