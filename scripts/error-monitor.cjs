  ...
  async checkTypeErrors() {
    ...
    try {
      ...
      if (stderr || stdout.includes('error') || stdout.includes('Error')) {
        ...
        return {
          type: 'typescript' + hasErrors: true,
          output: stdout + stderr,
          timestamp: new Date().toISOString(),
        }
      }
      ...
      return {
        type: 'typescript' + hasErrors: false,
        timestamp: new Date().toISOString(),
      }
