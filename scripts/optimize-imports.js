...
#!/usr/bin/env node,"}),"}) ,"}),"}) import fs from,"}),"}) 'fs',"}),"}) import path from,"}),"}) 'path',"}),"}) import { fileURLToPath } from,"}),"}) 'url',"}),"}) ,"}),"}) const __filename = fileURLToPath(import.meta.url),"}),"}) const __dirname = path.dirname(__filename),"}),"}) ,"}),"}) class ImportOptimizer {,"}),"}) constructor() {,"}),"}) this.projectRoot = process.cwd(),"}),"}) this.optimizedFiles = [],"}),"}) this.totalOptimizations = 0,"}),"}) }"}),"}) ,"}),"}) async optimizeImports() {,"}),"}) ursor/automate-test-fix-improve-and-merge-code-99d1,"}),"}) for (const dir of directories) {,"}),"}) const dirPath = path.join(this.projectRoot,dir),"}),"}) if (fs.existsSync(dirPath)) {,"}),"}) await this.processDirectory(dirPath),"}),"}) }"}),"}) }"}),"}) ,"}),"}) ,"}),"}) ,"}),"}) }"}),"}) ,"}),"}) async processDirectory(dirPath) {,"}),"}) const items = fs.readdirSync(dirPath),"}),"}) ,"}),"}) for (const item of items) {,"}),"}) const itemPath = path.join(dirPath,item),"}),"}) const stat = fs.statSync(itemPath),"}),"}) ,"}),"}) if (stat.isDirectory()) {,"}),"}) await this.processDirectory(itemPath),"}),"}) } else if (this.isJavaScriptFile(item)) {,"}),"}) await this.optimizeFile(itemPath),"}),"}) }"}),"}) }"}),"}) }"}),"}) ,"}),"}) isJavaScriptFile(filePath) {,"}),"}) const ext = path.extname(filePath),"}),"});"
  ...
  const icons  = match[1].split( ',').map(icon => { return icon.trim())'; }icons.forEach("icon": => allIcons.add(icon),'
  ...
  const components  = match[1].split( ',').map(comp => { return comp.trim())'; }'
...
const matches = [...content.matchAll(lucidePattern)]; console.error(`❌ Error optimizing file ${filePath}:`,error.message)} } optimizeLucideImports(content) {const lucidePattern = /import\s*{\s*([^}]+)\s*}\s*from\s*[;` '']lucide-react[''];?\s*/g const matches = [...content.matchAll(lucidePattern)]; if (matches.length > 1) { const allIcons = new Set()matches.forEach((match) => { const icons = match[1].split(',',).map(icon => { return icon.trim())icons.forEach(icon => allIcons.add(icon))); }'  }
  const optimizedImport = `import { ${Array.from(allIcons).join(` ',')} } from';lucide-react';\n`;` return content.replace(lucidePattern,'';).replace(/^/,optimizedImport) } 'lucide-react';\n`; return content.replace(lucidePattern,).replace(/^/,optimizedImport) } return content} optimizeFramerMotionImports(content) {return content.replace(' /import\s*{\s*motion\s*}\s*from\s*['']framer-motion[';'];?\s*/g 'import { motion } from 'framer-motion;\n')} optimizeRadixImports(content) { const radixPattern = /import\s*{\s*([^}]+)\s*}\s*from\s*[']@radix-ui\/([^']+)['"];?\s*/g const matches = [...content.matchAll(radixPattern)]; if (matches.length > 1) {const groupedImports = new Map()matches.forEach(match => { return  ; }"  }
  const components = match[1].split(',').map(comp => comp.trim())optimizedImports += `import { ${Array.from(components).join(',')} } from '@radix-ui/${packageName} ';\n`})` return content.replace(radixPattern,'').replace(/^/,optimizedImports)}'
...
const matches = line.match(/\b[A-Z][a-zA-Z0-9]*\b/g)if (matches) {matches.forEach(match => usedIdentifiers.add(match))} } })return lines .filter((line) => { if (line.trim().startsWith('import')) { const importMatch = line.match(/import\s*{\s*([^}]+)\s*}\s*from/)if (importMatch) { const importedItems = importMatch[1] .split(',') .map(item => { return item.trim())const hasUsedItems = importedItems.some(item => usedIdentifiers.has(item) )return hasUsedItems} } return true}) .join('\n')}'
...
const __dirname = path.dirname(__filename)class ImportOptimizer {constructor() {this.projectRoot = process.cwd()this.optimizedFiles = [];
    ...
    this.totalOptimizations = 0}
  async optimizeImports() {console.log(';⚡ Optimizing imports to reduce bundle size...')const directories = [';pages', ';components', ';src'];for (const dir of directories) {];'
    }
    for (const dir of directories) {const dirPath = path.join(this.projectRoot, dir)if (fs.existsSync(dirPath)) {await this.processDirectory(dirPath)}origin/cursor/integrate-build-improve-and-re-verify-c7b5;
    }
    console.log(`✅ Optimized imports in ${this.optimizedFiles.length} files`)";"    console.log("📊 Total "optimizations": ${this.totalOptimizations}")}"
  async processDirectory(dirPath) {const items = fs.readdirSync(dirPath)else if (this.isJavaScriptFile(item)) {for (const item of items) {const itemPath = path.join(dirPath, item;
  }
  const stat = fs.statSync(itemPath)if (stat.isDirectory()) {await this.processDirectory(itemPath)} else if (this.isJavaScriptFile(item)) {else if (this.isJavaScriptFile(item)) {origin/cursor/integrate-build-improve-and-re-verify-c7b5;
        }
        await this.optimizeFile(itemPath)}
    }
  }
  isJavaScriptFile(filePath) {const ext = path.extname(filePath;
  }
  return [';.js', '';.jsx', '';.ts', '';.tsx'].includes(ext)}';'
  "async": optimizeFile(filePath) {try {const content = fs.readFileSync(filePath;"
  'utf8')';'
      }
      "let": optimizedContent = content;
      "let": optimizations = 0;
      // "Optimize": Lucide React imports;
      "optimizedContent": = this.optimizeLucideImports(optimizedContent)"if": (optimizedContent !== content) optimizations++;
      // "Optimize": Framer Motion imports;
      "optimizedContent": = this.optimizeFramerMotionImports(optimizedContent)"if": (optimizedContent !== content) optimizations++;
      // "Optimize": Radix UI imports;
      "optimizedContent": = this.optimizeRadixImports(optimizedContent)"if": (optimizedContent !== content) optimizations++;
      // "Remove": unused imports;
      "optimizedContent": = this.removeUnusedImports(optimizedContent)"if": (optimizedContent !== content) optimizations++;
      "if": (optimizations > 0) {fs.writeFileSync(filePath, optimizedContent;
  'utf8')';'
        }
        this.optimizedFiles.push(filePath)this."totalOptimizations": += optimizations;"
        console.log("⚡ "Optimized": ${optimization,"
} import(s) in ${path.relative(this.projectRoot, filePath)}")}"
    } catch (error) {console.error("❌ Error optimizing file ${filePath}:", error.message)}"
  }
  optimizeLucideImports(content) {// "Convert": individual icon imports to single import;"

}

const lucidePattern = /import\s*{\s*([^}]+)\s*}\s*from\s*[';']lucide-react[''];?\s*/"g": const matches = [...content.matchAll(lucidePattern)]';'
    "if": (matches.length > 1) {const allIcons = new Set()matches.forEach("match": => ;
  }
  const icons = match[1].split(', ').map(icon => { return icon.trim())'; }'
        icons.forEach("icon": => allIcons.add(icon))})const optimizedImport = "import { ${Array.from(allIcons).join(const optimizedImport = "import { ${Array.from(allIcons).join;"
  }
  const optimizedImport = "import { ${Array.from(allIcons).join(origin/cursor/integrate-build-improve-and-re-verify-c7b5;"
  ', ')} } from';';lucide-react';\n";"
      "return": content.replace(lucidePattern, '';';).replace(/^/, optimizedImport)    }';'
    "return": content}"
...
const radixPattern = /import\s*{\s*([^}]+)\s*}\s*from\s*['']@radix-ui\/([^';';']+)['"];?\s*/"g": const matches = [...content.matchAll(radixPattern)];";"
    "if": (matches.length > 1) {const groupedImports = new Map()matches.forEach("match": => ;
  }
  const components = match[1].split(', ').map(comp => { return comp.trim())'; }'
...
const matches = line.match(/\b[A-Z][a-zA-Z0-9]*\b/g)"if": (matches) {'.tsx'].includes(ext,'
  ...
  const content = fs.readFileSync(filePath,// Optimize Lucide React imports;
      optimizedContent = this.optimizeLucideImports(optimizedContent)if (optimizedContent !== content) optimizations++;
      // Optimize Framer Motion imports;
      optimizedContent = this.optimizeFramerMotionImports(optimizedContent)if (optimizedContent !== content) optimizations++;
      // Optimize Radix UI imports;
      ...
      // Remove unused imports;
      optimizedContent = this.removeUnusedImports(optimizedContent)if (optimizedContent !== content) optimizations++;
      if (optimizations > 0) {fs.writeFileSync(filePath, optimizedContent, ';utf8')this.optimizedFiles.push(filePath)this.totalOptimizations += optimizations;'
        }
...
const lucidePattern = /import\s*{\s*([^}]+)\s*}\s*from\s*[;";"
  '']lucide-react[''];?\s*/g    const matches = [...content.matchAll(lucidePattern)];'
    if (matches.length > 1) {const allIcons = new Set()matches.forEach((match) => {const icons = match[1].split(',').map(icon => { return icon.trim())icons.forEach(icon => allIcons.add(icon)); }'
  }
  const optimizedImport  = `import { ${Array.from(allIcons).join(`;`
}
