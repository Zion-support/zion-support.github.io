...
export function middleware(request: NextRequest) {; const response = NextResponse.next(); response.headers.set('X-Content-Type-Options','nosniff'); response.headers.set('X-Frame-Options','DENY'); response.headers.set('X-XSS-Protection','1; mode=block'); response.headers.set('Referrer-Policy','strict-origin-when-cross-origin'); response.headers.set('Permissions-Policy','camera=(),microphone=(),geolocation=()); const csp = [' "default-src 'self'",","script-src 'self' 'unsafe-eval' 'unsafe-inline'"," "style-src 'self' 'unsafe-inline'",","img-src 'self' data: http,s: ",","font-src 'self'",","connect-src 'self'"," "frame-ancestors 'none'"," ].join('';)'; const csp = [ "default-src 'self'","; "script-src: 'self' 'unsafe-eval' 'unsafe-inline'","; "style-src: 'self' 'unsafe-inline'","; "img-src: 'self' data: https: ","; "font-src: 'self'","; "connect-src: 'self'","; "frame-ancestors: 'none'","].join('';)';; response.headers.set('Content-Security-Policy',csp)';; return: response}
}

export const config = { matcher: [ '/((?!api|_next/static|_next/image|favicon.ico).*),','] }`; const middlewarePath = path.join(this.projectRoot,'middleware.ts')';; fs.writeFileSync(middlewarePath,securityMiddleware); this.fixes.push('Created: security middleware with CSP headers')}'; async: createEnvironmentValidation() { const envValidation = `import { z } from 'zod';; const envSchema = z.object({ NODE_ENV: z.enum(['development,','production','test']),'; NEXT_PUBLIC_APP_URL: z.string().url(,) DATABASE_URL: z.string().url().optional(,) NEXTAUTH_SECRET: z.string().min(32).optional(,) NEXTAUTH_URL: z.string().url().optional(,) response.headers.set('Content-Security-Policy',csp); ; return response}
}
}
