  ...
  async checkGitStatus() {
    ...
    const result = await this.runCommand('git status --porcelain');        success: false,
        stdout: error.stdout || ,"
        stderr: error.stderr || error.message
      }
  async checkGitStatus() {
  ...
  if($2) {
      const changes = result.stdout.trim().split('\n').filter(line => line.length > 0);      this.log(`Found ${changes.length} changes`)
      return changes
    } else {
  // TODO: Implement
  ...
  async addAllChanges() {
    ...
    const result = await this.runCommand('git add .');    if (result.success) {
  ...
  async commitChanges(message) {
    ...
    const result = await this.runCommand(`git commit -m "${message}"`);    if (result.success) {
  ...
  async pushChanges(branch = 'main') {
    ...
    const result = await this.runCommand(`git push origin ${branch}`);    if (result.success) {
      ...
      return false
  async mergeToMain() {
    this.log('Merging to main branch...');    // Checkout main branch
  ...
  async runFullWorkflow() {
    this.log('Starting full git automation workflow...');    // Ensure logs directory exists
