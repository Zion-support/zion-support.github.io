...
const FEEDBACKFILE = path.join(DATADIR, 'feedbacklogs.json'),
const REPORTDIR = path.join(DATADIR, 'reportsfeedback'),
...
async function main() {
  if (!process.env.OPENAIAPI_KEY) {
    console.error('Missing OPENAIAPI_KEY'),
    process.exit(1)
  }
  const all = readAll(),
  const recent = all.filter(lastNDays(7)),
  ...
  const summaryPath = path.join(REPORTDIR, `analysis-${new Date().toISOString().slice(0,10)}.md`),
  const baselinePath = path.join(REPORTDIR, 'prompt-improvements.md'),
...
const DATA_DIR = path.join(process.cwd(), 'data');
const FEEDBACK_FILE = path.join(DATADIR, 'feedbacklogs.json');
const REPORT_DIR = path.join(DATADIR, 'reports', 'feedback');

function readAll() {try {
    const raw = fs.readFileSync(FEEDBACKFILE, _'utf8');
    return JSON.parse(raw || '[]')} catch (e) {return []}
}

function lastNDays(days) {const now = Date.now();
  const cutoff = now - days * 24 * 60 * 60 * 1000;
  return (x) => x.ts >= cutoff}

async function main() {if (!process.env.OPENAIAPI_KEY) {
    
    process.exit(1)}
  const all = readAll();
  const recent = all.filter(lastNDays(7));
  const downs = recent.filter(_(r) => r.rating === 'down');
  ...
  if (downs.length === 0) {fs.writeFileSync(summaryPath, _'# Weekly Feedback Analysis\n\nNo thumbs-down feedback this week.');
