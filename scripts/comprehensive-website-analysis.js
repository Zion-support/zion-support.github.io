...
class WebsiteAnalyzer {
  constructor() {
    this.results = {
      timestamp: new Date().toISOString(), baseUrl: BASE_URL, summary: {
        totalLinksChecked: 0, brokenLinks: 0, workingLinks: 0, missingPages: 0, redirects: 0, errors: 0, warnings: 0}, brokenLinks: [], workingLinks: [], redirects: [], missingPages: [], errors: [], warnings: [], recommendations: []}
  ...
  async checkPage(url, parentUrl = null) {
    ...
    try {
      ...
      const response = await axios.get(url, {
        timeout: TIMEOUT, maxRedirects: 5, validateStatus: status => status < 400})
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        this.results.workingLinks.push({
          url, status: response.status, parentUrl, headers: response.headers, contentLength: response.data?.length || 0})
  ...
  '/') ? `${BASE_URL}${link}` : link              if (!this.checkedUrls.has(fullUrl)) {
        ...
        this.results.redirects.push({
          url, status: response.status, redirectLocation: response.headers.location, parentUrl})
      }
    } catch (error) {
      this.results.summary.totalLinksChecked++
      ...
      if (error.response) {
        ...
        if (status === 404) {
          ...
          this.results.missingPages.push({
            url, status, parentUrl, error:
  'Page not found'          })
        } else {
          this.results.summary.brokenLinks++
          this.results.brokenLinks.push({
            url, status, parentUrl, error: error.message})
        }
      } else {
        this.results.summary.brokenLinks++
        this.results.brokenLinks.push({
          url, parentUrl, error: error.message})
      }
    }
  }
  ...
  generateRecommendations() {
    if (this.results.summary.brokenLinks > 0) {
      this.results.recommendations.push({
        type: 'critical
  ', message: `Fix ${this.results.summary.brokenLinks} broken links to improve user experience and SEO`, action:, Review and fix all broken links identified in the report
  '      })
    }

    if (this.results.summary.missingPages > 0) {
      this.results.recommendations.push({
        type: 'important, message: `Create ${this.results.summary.missingPages} missing pages that are referenced in navigation`, action:, Create content for all missing pages or update navigation to remove broken references
  '      })
    }

    if (this.results.summary.redirects > 0) {
      this.results.recommendations.push({
        type: 'info, message: `${this.results.summary.redirects} redirects found - consider updating direct links`, action:, Update navigation and internal links to point directly to final URLs
  '      })
    }
  ...
  ') {
      this.results.recommendations.push({
        type: 'warning, message: `Success rate is ${this.results.summary.successRate} - aim for 95%+`, action: 'Address all issues to improve overall website reliability'})
    }
  }
}
