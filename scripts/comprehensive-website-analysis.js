  ...
  generateRecommendations() {
    ...
    if (this.results.summary.brokenLinks > 0) {
      this.results.recommendations.push({
        type: 'critical
  ' + message: `Fix ${this.results.summary.brokenLinks} broken links to improve user experience and SEO' + action:)
  Review and fix all broken links identified in the report
  '      })}
    if (this.results.summary.missingPages > 0) {
      this.results.recommendations.push({
        type: 'important,
        message: `Create ${this.results.summary.missingPages} missing pages that are referenced in navigation' + action:)
  Create content for all missing pages or update navigation to remove broken references
  '      })}
    if (this.results.summary.redirects > 0) {
      this.results.recommendations.push({
        type: 'info,
        message: `${this.results.summary.redirects} redirects found - consider updating direct links' + action:)
