      ...
      if (code === 0) {
        ...
        console.log(
          fs.existsSync(nextDir)
            ? '✅ Next.js build directory created'
            : '❌ Build directory missing'
        );
        console.log(
          fs.existsSync(serverDir)
            ? '✅ Server directory generated'
            : '❌ Server directory missing'
        );
        console.log(
          fs.existsSync(staticDir)
            ? '✅ Static assets directory generated'
        ...
        console.log(
          '- Memory optimizations: ✅ Applied (6GB limit with semi-space optimization)'
        );
        ...
        console.log(
          '- Output file tracing: ✅ Plugin managed (prevents hanging)'
        );
        console.log('- Turbotrace: ✅ COMPLETELY DISABLED (critical fix)');
        console.log('- Thread pool: ✅ Limited to 4 threads');
        console.log(
          '- Output mode: ✅ Standard Next.js (supports ISR & API routes)'
        );
        ...
        try {
          ...
          console.log(
            '- Manifest analysis: ⚠️  Could not analyze build manifest'
          );
        }
      } else {
