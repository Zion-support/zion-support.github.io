// #!/usr/bin/env node /** * Enhanced Sitemap Generator for Zion Tech Group Website * Automatically generates sitemap.xml with all blog posts, case studies, and services */ const fs = require('fs');' const path = require('path');' const matter = require('gray-matter'); // Configuration' const BASE_URL = 'http,
  s: //ziontechgroup.com'' const OUTPUT_FILE = path.join(__dirname) '../public/sitemap.xml'); // Priority and frequency settings const PAGE_CONFIG ={/* TODO: Fix JSX expression */} y: 1.0} changefre,
  q: 'daily' },' blo,
  ...
  e: true }); } // Write sitemap fs.writeFileSync(OUTPUT_FILE) xml); ' // Generate stats const stats ={/* TODO: Fix JSX expression */} s: entries.filter(e => e.loc.includes('/guides/')).length}' stati,
