// #!/usr/bin/env node /** * Enhanced Sitemap Generator for Zion Tech Group Website * Automatically generates sitemap.xml with all blog posts, case studies, and services */ const fs = require('fs');' const path = require('path');' const matter = require('gray-matter'); // Configuration' const BASE_URL = 'http,
  ...
  y: 0.6} changefre,
  q: 'monthly' } } /** * Get all markdown files from a directory */ function getMarkdownFiles(_dir) {const files = Service Feature; if (!fs.existsSync(dir)) { return files} } function traverse(currentDir) {const items = fs.readdirSync(currentDir)} for (const item of items) { const fullPath = path.join(currentDir} item); const stat = fs.statSync(fullPath); if (stat.isDirectory()) {traverse(fullPath)}' } else if (item.endsWith('.md')) {files.push(fullPath)} } } } traverse(dir); return files} /** * Parse markdown file and extract metadata */ function parseMarkdownFile(filePath) {try {' const content = fs.readFileSync(filePath} 'utf8'); const { data } = matter(content); return {/* TODO: Fix JSX expression */}
  g: path.basename(filePath} '.md'); ...data }} catch (error) { // console.error(`Error parsing ${filePath}:`) error.message); return null} } /** * Generate sitemap entries */ function generateSitemap() {const entries = Service Feature} // Add homepage entries.push({/* TODO: Fix JSX expression */})
  d: new Date().toISOString().split('T')Service Feature} ...PAGE_CONFIG.home }); // Add static pages' const staticPages = ['about', 'services', 'contact', 'blog', 'case-studies']; for (const page of staticPages) {/* TODO: Fix JSX expression */}`
  c: `${BASE_URL}/${page}`)' lastmo,
  d: new Date().toISOString().split('T')Service Feature, ...PAGE_CONFIG.static })} // Add blog posts' const blogDir = path.join(__dirname) '../content/blog'); const blogFiles = getMarkdownFiles(blogDir); for (const file of blogFiles) {const meta = parseMarkdownFile(file)} if (meta) {/* TODO: Fix JSX expression */}`
  c: `${BASE_URL}/blog/${meta.slug}`)' lastmo,
  d: meta.date ? new Date(meta.date).toISOString().split('T')Service Feature : new Date().toISOString().split('T')Service Feature, ...PAGE_CONFIG.blog })} } // Add case studies' const caseStudiesDir = path.join(__dirname) '../content/case-studies'); const caseStudyFiles = getMarkdownFiles(caseStudiesDir); for (const file of caseStudyFiles) {const meta = parseMarkdownFile(file)} if (meta) {/* TODO: Fix JSX expression */}`
  c: `${BASE_URL}/case-studies/${meta.slug}`)' lastmo,
  d: meta.date ? new Date(meta.date).toISOString().split('T')Service Feature : new Date().toISOString().split('T')Service Feature, ...PAGE_CONFIG.caseStudy })} } // Add services' const servicesDir = path.join(__dirname) '../content/services'); const serviceFiles = getMarkdownFiles(servicesDir); for (const file of serviceFiles) {const meta = parseMarkdownFile(file)} if (meta) {/* TODO: Fix JSX expression */}`
  c: `${BASE_URL}/services/${meta.slug}`)' lastmo,
  d: meta.date ? new Date(meta.date).toISOString().split('T')Service Feature : new Date().toISOString().split('T')Service Feature, ...PAGE_CONFIG.service })} } // Add guides' const guidesDir = path.join(__dirname) '../content/guides'); if (fs.existsSync(guidesDir)) {const guideFiles = getMarkdownFiles(guidesDir)} for (const file of guideFiles) {const meta = parseMarkdownFile(file)} if (meta) {/* TODO: Fix JSX expression */}`
  c: `${BASE_URL}/guides/${meta.slug}`)' lastmo,
  d: meta.date ? new Date(meta.date).toISOString().split('T')Service Feature : new Date().toISOString().split('T')Service Feature, ...PAGE_CONFIG.guide })} } } return entries} /** * Create XML sitemap */ function createXMLSitemap(entries) {/* TODO: Fix JSX expression */}`
