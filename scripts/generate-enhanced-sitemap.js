// #!/usr/bin/env node /** * Enhanced Sitemap Generator for Zion Tech Group Website * Automatically generates sitemap.xml with all blog posts, case studies, and services */ const fs = require('fs');' const path = require('path');' const matter = require('gray-matter')
  s: //ziontechgroup.com'' const OUTPUT_FILE = path.join(__dirname) '../public/sitemap.xml')
  y: 1.0} changefre,
  q: 'daily' },' blo,
  ...
  d: new Date().toISOString().split('T')[0]} ...PAGE_CONFIG.home })
  ...
  p://www.sitemaps.org/schemas/sitemap/0.9">\n' for (const entry of entries) {' xml += ' <url>\n' xml += ` <loc>${entry.loc}</loc>\\n`; xml += ` <lastmod>${entry.lastmod}</lastmod>\\n`; xml += ` <changefreq>${entry.changefreq}</changefreq>\\n`; xml += ` <priority>${entry.priority}</priority>\\n`;' xml += ' </url>\n' } ' xml += '</urlset>' return xml} /** * Main execution */ function main() {' try { const entries = generateSitemap(); const xml = createXMLSitemap(entries)
