...
class ComprehensiveLinkChecker {
  ...
  async checkUrl(url, parentUrl = null, depth = 0) {
    ...
    try {console.log(`Checking: ${url} (depth: ${depth})');
  ...
  '/ai-solutions',/solutions/enterprise',
  '/solutions/healthcare',/research-development',
  '/request-quote',/green-it',
  '/space-tech'    ];
  ...
  '/services/returns-management',/services/llm-content-studio'    ];
    ...
    const promises = serviceUrls.map(url =>this.checkUrl(`${this.baseUrl}${url}', null, 0)
    );
    await Promise.all(promises);
  }
  ...
  '/privacy',/terms'    ];
    ...
    if (missingImportant.length > 0) {
      recommendations.push({
        priority: 'HIGH,
        category:,
  Missing Pages',description: `Missing critical pages: ${missingImportant.join(, ,
  )}',
        actions: [Create missing critical pages,
,
  ...
  'Add SEO meta tags and content'
    ...
    if (orphanedPages.length > 0) {
      recommendations.push({
        ...
        actions: [Review navigation structure',
  'Add breadcrumbs to deep pages',Improve internal linking strategy'
        ]      });
    }
  ...
  async run() {
    console.log(
  '🚀 Starting Comprehensive Link Check for Zion Tech Group');console.log(`Base URL: ${this.baseUrl}');
    ...
    try {
      ...
      console.log(\n📊 Link Check Complete!');      console.log(`Total URLs checked: ${report.summary.totalUrls}`);console.log(`Working links: ${report.summary.workingLinks}`);console.log(`Broken links: ${report.summary.brokenLinks}`);console.log(`Missing pages: ${report.summary.missingPages}');
