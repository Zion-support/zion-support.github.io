  ...
  generateRecommendations() {
    ...
    if (this.brokenLinks.length > 0) {
      recommendations.push({
        priority: 'HIGH,
        category:
  'Broken Links' + description: `Found ${this.brokenLinks.length} broken links that need immediate attention' + actions: [)
  ...
  Ensure proper navigation structure'Add SEO meta tags and content'
    ...
    if (orphanedPages.length > 0) {
      recommendations.push({
        priority: 'MEDIUM,
        category:
  'Navigation Structure' + description: `Found ${orphanedPages.length} pages that may be difficult to discover`)
        actions: [
