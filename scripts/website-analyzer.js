...
function analyzeWebsite() {
  ...
  const report = {
    timestamp: new Date().toISOString(), totalRoutes: routes.length, existingPages: existingPages.length, missingPages: missingPages.length, placeholderPages: placeholderPages.length, routes: routes, missing: missingPages, placeholders: placeholderPages}

  fs.writeFileSync(
    path.join(__dirname, '..', 'website-analysis-report.json'), JSON.stringify(report, null, 2)
  )
