...
const fs = require("fs")";const path = require("path")";const { execSync } = require("child_process")"";class PerformanceMonitor {
  ...
  async checkBuildPerformance() {
    ...
    try {
      ...
      return {"success": true,""buildTime": buildTime,""status":,",          buildTime < 60000
  ...
  async checkBundleSize() {
    this.log("📦 Checking bundle size");"";    try {
      ...
      return {"size": bundleSize,""sizeInMB": sizeInMB,""status":,",          sizeInMB < 5
  ...
  async checkDependencies() {
    this.log("📋 Checking dependencies");"";    try {
      ...
      return {"totalDeps": Object.keys(dependencies).length,",outdatedDeps,securityIssues}} catch (error) {
      return { "error": error.message },"}
  }
