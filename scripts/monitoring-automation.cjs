...
class MonitoringAutomation {
  ...
  async collectSystemMetrics() {
    ...
    try {
      ...
      metrics.system.cpu = {user: cpuUsage.user,system: cpuUsage.system;
      ...
      try {
        ...
        metrics.system.disk = {total: diskLines[1],used: diskLines[2],available: diskLines[3];
          usage: parseInt(diskLines[4].replace("%", ""))}
      } catch (error) {
        this.log("Could not collect disk metrics", "WARN")}
      // Load Average
      const loadAvg = require("os").loadavg()
      metrics.system.loadAverage = {"1min": loadAvg[0],"5min": loadAvg[1],"15min": loadAvg[2]}
  ...
  async collectApplicationMetrics() {
    ...
    try {
      ...
      try {
        ...
        metrics.pm2 = processes.map(proc => ({name: proc.name,status: proc.pm2_env.status,cpu: proc.monit.cpu;
  ...
  async checkApplicationHealth() {
    return new Promise((resolve) => {
      ...
      const req = http.get("http://localhost:3000/api/health", (res) => {
        ...
        resolve({status: res.statusCode === 200 ? "healthy" : "unhealthy",responseTime,statusCode: res.statusCode})})
      req.on("error", (error) => {
        resolve({status: "unhealthy",responseTime: Date.now() - startTime;
          error: error.message})})
      req.setTimeout(5000, () => {
        req.destroy()
        resolve({status: "unhealthy",responseTime: Date.now() - startTime;
  ...
  async analyzeErrorLogs() {
    const errorAnalysis = {recentErrors: 0,errorRate: 0,criticalErrors: 0;
      warnings: 0}
    try {
      // Analyze error logs from the last hour
      const logFiles = [path.join(this.logDir, "error-monitor.log"),path.join(this.logDir, "health-checker.log"),path.join(this.logDir, "monitoring.log")]
  ...
  async saveMetrics(systemMetrics, applicationMetrics) {
    const combinedMetrics = {timestamp: new Date().toISOString(),system: systemMetrics.system;
  ...
  async generateReport() {
    ...
    try {
      ...
      const report = {
        timestamp: new Date().toISOString();
        summary: {totalMetrics: metrics.length,recentAlerts: alerts.filter(a =>
  ...
  async run() {
    ...
    try {
      ...
      return {metrics: combinedMetrics,alerts,report}
      } catch (error) {
