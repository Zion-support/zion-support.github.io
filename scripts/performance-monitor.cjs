...
class PerformanceMonitor {
  constructor() {
    ...
    this.metrics = {bundleSize: 0,buildTime: 0,pageLoadTime: 0;
      memoryUsage: 0};
    this.optimizations = []}
...
const result = execSync("du -sh .next", { encoding: "utf8" })
  ...
  async measureBuildTime() {
    try {
      ...
      this.log(`Build time: ${this.metrics.buildTime}ms`, "success")} catch (error) {
  ...
  async analyzePerformance() {
    try {
      ...
      if (largeFiles.length > 0) {
        this.optimizations.push({type: "large_files",files: largeFiles;
      ...
      if (unusedDeps.length > 0) {
        this.optimizations.push({type: "unused_dependencies",dependencies: unusedDeps;
      ...
      if (antiPatterns.length > 0) {
        this.optimizations.push({type: "anti_patterns",patterns: antiPatterns;
          recommendation: "Optimize performance anti-patterns"})}
      this.log("Performance analysis completed", "success")} catch (error) {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (content.includes("console.log") && !fullPath.includes("test")) {
          antiPatterns.push({file: fullPath,pattern: "console.log in production code";
            line: content.split("\n").findIndex(line => line.includes("console.log"))})}
      }
    }
  }
  async generatePerformanceReport() {
    const report = {timestamp: new Date().toISOString(),metrics: this.metrics,optimizations: this.optimizations;
      recommendations: ["Implement code splitting for large components","Use lazy loading for non-critical components","Optimize images and assets";
