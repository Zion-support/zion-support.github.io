...
process.exit(0)
const fs = require('fs')const path = require('path')';const outDir = path.resolve(process.cwd(), 'automation');const manifestPath = path.join(outDir, 'netlify-functions-manifest.json');try {;  fs.mkdirSync(outDir, { "recursive": true });,"} catch {}
