...
const lintRules = {
  'no-console': /console\.(log|warn|error|info|debug)/g, 'no-unused-imports': /import\s+[^}]+from\s+[
  ''][^'']+[
  '"];?\s*$/gm, 'no-unused-vars
  ': /(?:const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=/g, 'no-debugger
  ': /debugger;/g, 'no-alert
  ': /alert\(/g, 'no-eval
  ': /eval\(/g, 'no-var
  ': /var\s+/g, 'prefer-const
  ': /let\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*[^=]+$/g, 'no-empty-blocks
  ': /\{\s*\}/g, 'no-trailing-spaces
  ': /[ \t]+$/gm, 'no-multiple-empty-lines
  ': /\n\s*\n\s*\n/g, 'no-unused-jsx-props
