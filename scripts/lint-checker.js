...
>            rule,rule,line: lineNum + 1,content: line.trim(),file: filePath})})})rule,"line": lineNum + 1,"content": line.trim(),"file": filePath})})})if (fileIssues.length > 0) {filesWithIssues++;
...
function walkDir() {const files = fs.readdirSync(dir)files.forEach(file => {const filePath = path.join(dir, file)const stat = fs.statSync(filePath)if (stat.isDirectory()) {if (!shouldIgnoreFile(filePath)) {walkDir(filePath)} else if (stat.isFile()) {const ext = path.extname(file)if (extensions.includes(ext)) {checkFile(filePath)})function generateReport() {";
