...
class DeploymentAutomation {
  ...
  async start() {
    ...
    try {
      ...
      console.log('✅ Deployment Automation started successfully');
    } catch (error) {
  ...
  async createBackup(environment) {
    ...
    try {
      ...
      if (environment === 'production') {
        ...
        execSync(`cp -r /var/www/ziontechgroup.com ${backupPath}/`, { stdio: 'pipe' });
      } else {
  ...
  async performHealthCheck(url) {
    try {
      ...
      const success = curlResult.trim() === '200' && responseTime < 5000;
  ...
  async postDeploymentTasks(environment) {
    ...
    try {
      ...
      try {
        ...
        tasks.push({ name: 'sitemap_update', success: true });
      } catch (error) {
      ...
      try {
        ...
        tasks.push({ name: 'cache_clear', success: true });
      } catch (error) {
      ...
      try {
        ...
        tasks.push({ name: 'notification', success: true });
      } catch (error) {
  ...
  async rollback(environment, deploymentId) {
    ...
    try {
      ...
      if (backupExists) {
        if (environment === 'production') {
          execSync(`rsync -avz --delete ${backupPath}/ production-server:/var/www/ziontechgroup.com/`, { stdio: 'pipe' });
        } else {
          execSync(`rsync -avz --delete ${backupPath}/ staging-server:/var/www/ziontechgroup-staging.com/`, { stdio: 'pipe' });
        }
      } else {
        // Deploy previous version from git
        const previousCommit = previousDeployment.steps.find(s => s.name === 'pre_deployment_checks')?.result?.checks?.lastCommit;
        if (previousCommit) {
  ...
  async checkForDeploymentTriggers() {
    ...
    try {
      ...
      if (localCommit !== remoteCommit && this.config.autoDeploy) {
  ...
  async saveDeploymentHistory() {
    try {
      await fs.writeFile('./logs/deployment-history.json', JSON.stringify(this.deploymentHistory, null, 2));
    } catch (error) {
  ...
  async saveRollbackHistory() {
    try {
      await fs.writeFile('./logs/rollback-history.json', JSON.stringify(this.rollbackHistory, null, 2));
    } catch (error) {
