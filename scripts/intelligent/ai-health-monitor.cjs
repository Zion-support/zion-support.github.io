...
class AIHealthMonitor {
  ...
  async detectAnomalies() {
    ...
    if (this.metrics.cpu.length > 3) {
      ...
      if (avgCPU > this.thresholds.cpu * 1.5) {
        anomalies.push({
          type: 'cpu_spike',
          severity: 'high',
          value: avgCPU,
    ...
    if (this.metrics.memory.length > 10) {
      ...
      if (trend === 'increasing' && memoryValues[memoryValues.length - 1] > this.thresholds.memory) {
        anomalies.push({
          type: 'memory_leak',
          severity: 'medium',
          trend: trend,
  ...
  async predictIssues() {
    ...
    if (this.metrics.memory.length > 5) {
      const memoryTrend = this.analyzeTrends().memory;
      if (memoryTrend && memoryTrend.trend === 'increasing' && memoryTrend.average > 70) {
        predictions.push({
