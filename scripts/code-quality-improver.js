...
class CodeQualityImprover {
  ...
  async improve() {
    ...
    try {
      ...
      console.error('❌ Quality improvement failed:', error.message);
      process.exit(1);
    }
  }
  ...
  async removeUnusedCode() {
    ...
    files.forEach(file => {
      const improvements = this.cleanFile(file);
      removedCount += improvements;
    });
    ...
    this.report.improvements.push({
      ...
      message: `Removed ${removedCount} unused items`
    });
    
    console.log(`✅ Removed ${removedCount} unused items`);
  }
  ...
  async optimizeImports() {
    ...
    files.forEach(file => {
      if (this.optimizeFileImports(file)) {
        optimizedCount++;
      }
    });
    ...
    this.report.improvements.push({
      ...
      message: `Optimized imports in ${optimizedCount} files`
    });
    
    console.log(`✅ Optimized imports in ${optimizedCount} files`);
  }

  async addTypeAnnotations() {
    ...
    this.report.improvements.push({
      ...
      status: 'completed',
      annotatedFiles: annotatedCount,
      message: `Added type annotations to ${annotatedCount} files`
    });
    
    console.log(`✅ Added type annotations to ${annotatedCount} files`);
  }
  ...
  async generateQualityReport() {
    ...
    this.report.metrics.qualityScore = score;
  ...
  cleanFile(filePath) {
    ...
    lines.forEach((line, index) => {
      // Skip if line is an import that might be unused
      if (line.trim().startsWith('import ') && line.includes(' from ')) {
        const importName = line.match(/import\s+{([^}]+)}/)?.[1] || 
                          line.match(/import\s+(\w+)/)?.[1];
      ...
      if (line.includes('console.log') && process.env.NODE_ENV === 'production') {
        improvements++;
        return; // Skip this line
      }
  ...
  optimizeFileImports(filePath) {
    ...
    lines.forEach(line => {
      if (inImports && line.trim().startsWith('import ')) {
        ...
        } else if (line.includes('node_modules') || line.includes('@')) {
  ...
  addMissingTypes(filePath) {
    const content = fs.readFileSync(filePath, 'utf8');
    
    // Add return type annotations to functions (simplified)
    let newContent = content;
    let improvements = 0;
    
    // Add React.JSX.Element return type to React components
    const componentPattern = /export default function (\w+)\(\):\s*React\.JSX\.Element/g;
    ...
    const voidPattern = /function (\w+)\([^)]*\)\s*{/g;
    if (voidPattern.test(content) && !content.includes(': void') && !content.includes(': React.JSX.Element')) {
  ...
  isImportUsed(content, importName) {
    // Simple check if import is used (excluding the import line itself)
    const lines = content.split('\n');
    const importLineIndex = lines.findIndex(line => 
      line.includes(importName) && line.trim().startsWith('import ')
    );
    
    const contentAfterImport = lines.slice(importLineIndex + 1).join('\n');
    return contentAfterImport.includes(importName);
  }
  ...
  calculateQualityScore() {
    ...
    this.report.improvements.forEach(improvement => {
      score += 5;
    });
