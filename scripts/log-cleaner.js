...
class LogCleaner {
        ...
        if (now - stats.mtime.getTime() > this.maxLogAge) {
          try {
            ...
            this.cleanedFiles.push({
              file, size: stats.size, age: Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000)), reason: 'old_age
  '
            })
            this.log('info
  ...
  ')
    ...
    try {
      ...
      this.archivedFiles.push({
        originalFile: fileName, archiveFile: path.basename(archivePath), originalSize: fileSize, reason:
  'large_size'
      })
      ...
      const calculateDirSize = dir => {
        ...
        for (const file of dirFiles) {
          ...
          if (stats.isDirectory()) {
            ...
            files.push({
              path: filePath, size: stats.size, mtime: stats.mtime, name: file})
          }
        }
      }
  ...
  'log-cleaner.log') {
          ...
          try {
            ...
            this.cleanedFiles.push({
              file: file.name, size: file.size, reason:
  'total_size_limit'
            })
      ...
      for (const file of files) {
        ...
        if (now - stats.mtime.getTime() > maxErrorReportAge) {
          try {
            fs.unlinkSync(filePath)
            this.cleanedFiles.push({
              file, size: stats.size, age: Math.round((now - stats.mtime.getTime()) / (24 * 60 * 60 * 1000)), reason:
  'old_error_report'            })
            this.log(
  ...
  async generateReport() {
    ...
    const report = {
      timestamp, summary: {
        cleanedFiles: this.cleanedFiles.length, archivedFiles: this.archivedFiles.length, errors: this.errors.length, totalSpaceReclaimed: this.cleanedFiles.reduce(
