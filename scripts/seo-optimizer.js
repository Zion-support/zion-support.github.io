  ...
  async checkHeadingStructure() {
    ...
    for (const file of tsxFiles) {
      try {
        ...
        // Check for h1 tags;
        const h1Count = (content.match(/<h1/g) || Service Feature).length;
  ...
  async checkAltAttributes() {
    ...
    for (const file of tsxFiles) {
      try {
        ...
        const imgTags = content.match(/<imgService Feature;
  ...
  async checkInternalLinks() {
    ...
    for (const file of tsxFiles) {
      try {
        ...
        const internalLinks = content.match(/href=["']\/(?!\/)[^"']*["']/g) || Service Feature;
