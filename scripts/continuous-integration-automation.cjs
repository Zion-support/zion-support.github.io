...
class ContinuousIntegrationAutomation {
  ...
  async runCIStep(stepName, command, description) {
    ...
    const step = {name: stepName,command,description;
      startTime: Date.now();
      status: 'running'}
    try {
  ...
  async runLinting() {this.log('🔍 Running linting checks...')
    return await this.runCIStep(
      'Linting','npm run lint','Check code quality with ESLint'
    )}
  async runTypeChecking() {this.log('🔍 Running TypeScript type checking...')
    return await this.runCIStep(
      'Type Checking','npm run type-check','Check TypeScript types'
    )}
  async runTests() {
    this.log('🧪 Running tests...')
    try {return await this.runCIStep(
        'Tests','npm test','Run unit and integration tests'
      )} catch (error) {
      this.log('⚠️ Tests not configured, skipping...', 'warning')
      return { success: true, output: 'Tests skipped', duration: 0 }
    }
  }
  async runBuild() {this.log('🏗️ Running build...')
    return await this.runCIStep(
      'Build','npm run build','Build the application'
    )}
  async runSecurityAudit() {this.log('🔒 Running security audit...')
    return await this.runCIStep(
      'Security Audit','npm audit --audit-level=moderate','Check for security vulnerabilities'
    )}
  async runPerformanceCheck() {
    this.log('⚡ Running performance check...')
    try {return await this.runCIStep(
        'Performance Check','npm run perf: monitor';
...
`
  ...
  async generateReport() {
    ...
    const report = {
      ...
      summary: {totalSteps: this.ciSteps.length,successfulSteps: successfulSteps.length,failedSteps: failedSteps.length;
  ...
  generateRecommendations() {
    ...
    recommendations.push({type: 'improvement',message: 'Set up automated deployment pipeline'})
    recommendations.push({type: 'improvement',message: 'Add performance monitoring to CI pipeline'})
    recommendations.push({type: 'improvement',message: 'Implement automated rollback mechanisms'})
