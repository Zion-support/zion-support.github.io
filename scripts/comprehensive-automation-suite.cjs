...
async function runComprehensiveAutomation() {
 ...
 ; ;};
  ...
  // Function to run automation phase
 ...
 ; ;};
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      const duration = Date.now() - taskStartTi;m;e;
      ...
      automationSuite.metrics.totalTasks++;
      automationSuite.metrics.successfulTasks++;
      ...
      automationSuite.metrics.totalTasks++;
      automationSuite.metrics.failedTasks++;
  ...
  phase.status = phase.status === 'running' ? 'success' : phase.status
  ...
  if (automationSuite.metrics.successfulTasks / automationSuite.metrics.totalTasks < 0.8) {
    recommendations.push({
      type: 'warning',
  ...
  console.log(`   - Success Rate: ${((automationSuite.metrics.successfulTasks / automationSuite.metrics.totalTasks); * 100).toFixed(1)}%`);
...
runComprehensiveAutomation().then(exitCode => {
  process.exit(exitCode)}).catch(error => {
