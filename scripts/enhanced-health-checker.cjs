...
;
  async checkConfiguration() {;
    this.log("⚙️ Checking configuration files");"";    const results = {;
const fs = require("fs")";const path = require("path")";const { execSync } = require("child_process")"";class HealthChecker {
  ...
  async checkConfiguration() {
    this.log("⚙️ Checking configuration files")"";    const results = {      "packageJson": { "valid": false, "issues": [] },""nextConfig": { "valid": false, "issues": [] },""tsConfig": { "valid": false, "issues": [] },,"}
    // Check package.json;
    // Check next.config.js;
    try {
      const nextConfigPath = path.join(this.projectRoot, "next.config.js");";      if (fs.existsSync(nextConfigPath)) {";        const content = fs.readFileSync(nextConfigPath, "utf8");";        if (content.includes("export default")) {";          results.nextConfig.valid = true,"} else {
          results.nextConfig.issues.push("Invalid export format");"}"} else {
        results.nextConfig.issues.push("File not found");"}"} catch (error) {
    ...
    try {
      const tsConfigPath = path.join(this.projectRoot, "tsconfig.json");";      if (fs.existsSync(tsConfigPath)) {";        const tsConfig = JSON.parse(fs.readFileSync(tsConfigPath, "utf8"));";        results.tsConfig.valid = true;";
        if (!tsConfig.compilerOptions) {;
          results.tsConfig.issues.push("Missing compilerOptions");"}"} else {;
...
;
      const results = {;
    try {
      const packageJson = JSON.parse()        fs.readFileSync(path.join(this.projectRoot, "package.json"), "utf8")")";      results.packageJson.valid = true;
      if (!packageJson.scripts?.build) {
        results.packageJson.issues.push("Missing build script")"}";      if (!packageJson.scripts?.dev) {
        results.packageJson.issues.push("Missing dev script")"}"} catch (error) {
  ...
  async checkDependencies() {
    this.log("📦 Checking dependencies")"";    try {
      ...
      const results = {        "total": Object.keys(dependencies).length,""missing": [],""outdated": [],,"}
      // Check for critical dependencies;
  async run() {
    this.log("🏥 Starting Health Check");"";    try {
