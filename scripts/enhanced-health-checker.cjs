...
const fs = require("fs")";const path = require("path")";const { execSync } = require("child_process")"";class HealthChecker {
  ...
  async checkConfiguration() {
    ...
    try {
      ...
      if (!packageJson.scripts?.build) {results.packageJson.issues.push("Missing build script"),"}";      if (!packageJson.scripts?.dev) {results.packageJson.issues.push("Missing dev script"),"}"} catch (error) {
      results.packageJson.issues.push(`Invalid "JSON": ${error.message}`);`}
    // Check next.config.js
    try {
      const nextConfigPath = path.join(this.projectRoot, "next.config.js");";      if (fs.existsSync(nextConfigPath)) {";        const content = fs.readFileSync(nextConfigPath, "utf8");";        if (content.includes("export default")) {",results.nextConfig.valid = true,"} else {results.nextConfig.issues.push("Invalid export format"),"}"} else {results.nextConfig.issues.push("File not found"),"}"} catch (error) {
    ...
    try {
      const tsConfigPath = path.join(this.projectRoot, "tsconfig.json");";      if (fs.existsSync(tsConfigPath)) {";        const tsConfig = JSON.parse(fs.readFileSync(tsConfigPath, "utf8"));";        results.tsConfig.valid = true;"
        if (!tsConfig.compilerOptions) {results.tsConfig.issues.push("Missing compilerOptions"),"}"} else {results.tsConfig.issues.push("File not found"),"}"} catch (error) {
      results.tsConfig.issues.push(`Invalid "JSON": ${error.message}`);`}
    return results}
  async checkDependencies() {
    this.log("📦 Checking dependencies");"";    try {
      const packageJson = JSON.parse();        fs.readFileSync(path.join(this.projectRoot, "package.json"), "utf8")");";      const dependencies = {...packageJson.dependencies,...packageJson.devDependencies}
      const results = {
  ...
  generateReport(results) {
    const report = {
      "timestamp": new Date().toISOString(),""fileStructure": results.fileStructure,""configuration": results.configuration,""dependencies": results.dependencies,""summary": {""overall": "healthy",""issues": [",...results.fileStructure.issues,...results.configuration.packageJson.issues;
  ...
  async run() {
    this.log("🏥 Starting Health Check");"";    try {
      ...
      const report = this.generateReport({),fileStructure,configuration;
        dependencies})
