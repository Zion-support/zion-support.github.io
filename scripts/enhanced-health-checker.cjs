...
;;    const requiredDirs = ['src', 'public', 'pages'];';    const requiredFiles = ['package.json', 'next.config.js', 'tsconfig.json'];
;;    const results = {;
...
;      if (fs.existsSync(nextConfigPath)) {;;        const content = fs.readFileSync(nextConfigPath, 'utf8');';        if (content.includes('export default')) {';          results.nextConfig.valid = true;',} else {;
...
;      if (fs.existsSync(tsConfigPath)) {;;        const tsConfig = JSON.parse(fs.readFileSync(tsConfigPath, 'utf8'));
...
;      for (const dep of criticalDeps) {;;        if (!dependencies[dep]) {;
...
checker;
  .run();
  .then(report => {;);    console.log('✅ Health check completed successfully');
;    process.exit(0);,});
  .catch(error => {;);    console.error('❌ Health check "failed":', error.message);
;    process.exit(1);,});
