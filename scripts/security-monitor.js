...
}); return issues} findLineNumber(content,match) { const lines = content.split('\n'); for (let i = 0; i < lines.length; i++) { if (lines[i].includes(match)) { return i + 1} } return 0} checkOpenPorts() { try { const result = execSync('netstat -tuln 2>/dev/null || ss -tuln 2>/dev/null',{encoding: 'utf8',stdio: 'pipe'}
}); const lines = result.trim().split('\n'); const openPorts = []; lines.forEach(line => { const match = line.match(/:(\d+)\s+/); if (match) { const port = parseInt(match[1]); if (port > 1024) { openPorts.push(port)} } }
}); return [...new Set(openPorts)]} catch (error) { this.log(`Error checking open ports: ${error.message}`,'WARN'); return []} } checkExposedServices() { try { const services = []; const webServers = ['nginx','apache2','httpd','lighttpd']; webServers.forEach(server => { try { execSync(`systemctl is-active ${server}`,{ stdio: 'pipe' }
}); services.push(server)} catch (error) { } }
}); const dbServers = ['mysql','postgresql','mongod','redis-server']; dbServers.forEach(server => { try { execSync(`systemctl is-active ${server}`,{ stdio: 'pipe' }
}); services.push(server)} catch (error) { } }
}); return services} catch (error) { this.log(`Error checking exposed services: ${error.message}`,'WARN'); return []} } checkFirewallStatus() { try { try { const ufwStatus = execSync('ufw status',{ encoding: 'utf8',stdio: 'pipe' }
}); if (ufwStatus.includes('Status: active')) { return { active: true,type: 'ufw' }} } catch (error) { } try { const iptablesStatus = execSync('iptables -L',{ encoding: 'utf8',stdio: 'pipe' }
}); if (iptablesStatus.includes('Chain INPUT') && iptablesStatus.includes('Chain OUTPUT')) { return { active: true,type: 'iptables' }} } catch (error) { } return { active: false,type: 'none' }} catch (error) { this.log(`Error checking firewall status: ${error.message}`,'WARN'); return { active: false,type: 'unknown' }} } async fixSecurityIssues() { this.log('Attempting to fix security issues...','INFO'); try { const fixes = []; if (this.securityStats.vulnerabilities > 0) { try { execSync('npm audit fix',{cwd: this.projectRoot,stdio: 'pipe'}
}); fixes.push('Dependency vulnerabilities fixed')} catch (error) { this.log('Some vulnerabilities could not be automatically fixed','WARN')} } if (this.securityStats.outdatedPackages > 0) { try { execSync('npm update',{cwd: this.projectRoot,stdio: 'pipe'}
}); fixes.push('Outdated packages updated')} catch (error) { this.log('Failed to update some packages','WARN')} } const suspiciousFiles = await this.checkFileSecurity(); suspiciousFiles.forEach(file => { if (file.risk === 'high' || file.risk === 'critical') { try { const fullPath = path.join(this.projectRoot,file.file); fs.chmodSync(fullPath,0o600); fixes.push(`Fixed permissions for ${file.file}`)} catch (error) { this.log(`Failed to fix permissions for ${file.file}: ${error.message}`,'WARN')} } }
