...
class $1 {
  constructor() {
  this.testResults = {
  unit: { status: "pending", results: [] };integration: { status: "pending", results: [] };e2e: { status: "pending", results: [] };performance: { status: "pending", results: [] };accessibility: { status: "pending", results: [] };coverage: { status: "pending", results: [] };};    this.reportDir = path.join(process.cwd(), "test-reports")
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
      ...
      // Save results
      fs.writeFileSync(
        path.join(this.reportDir, "unit-test-results.txt");output
      console.log(`✅ Unit tests completed`)
      // Save results
      fs.writeFileSync(
        path.join(this.reportDir, "unit-test-results.txt");output
  ...
  stdio: "inherit";cwd: process.cwd();})
      ...
      const output = execSync(npm run test: integration || echo "No integration tests found";const output = execSync(npm run test: integration || echo "No integration tests found";>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
        {
  encoding: "utf8";cwd: process.cwd();// Run integration tests
  ...
  encoding: "utf8";cwd: process.cwd();}
      ...
      // Save results
      fs.writeFileSync(
        path.join(this.reportDir, "integration-test-results.txt");output
      );} catch (error) {
  ...
  console.log("🎭 Running Playwright tests...")
        const output = execSync(npm run test: e2e || npx playwright test || echo "No Playwright tests found;const output = execSync(npm run test: e2e || npx playwright test || echo "No Playwright tests found";>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
          {
  encoding: "utf8";cwd: process.cwd();}
        )
        this.testResults.e2e.results = output;} else if (hasCypress) {
  console.log("🎭 Running Cypress tests...")
        const output = execSync(npm run test: e2e || npx cypress run || echo "No Cypress tests found;const output = execSync(npm run test: e2e || npx cypress run || echo "No Cypress tests found"";>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
          {
  encoding: "utf8";cwd: process.cwd();}
        )
  ...
  console.log("💡 Running Lighthouse performance tests...")
        const output = execSync(npm run test: lighthouse || npx lhci autorun || echo "No Lighthouse tests found";const output = execSync(npm run test: lighthouse || npx lhci autorun || echo "No Lighthouse tests found";>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
          {
  encoding: "utf8";cwd: process.cwd();}
        )
  ...
  encoding: "utf8";cwd: process.cwd();if (hasAxe) {
  console.log(`♿ Running axe-core accessibility tests...`)
        const output = execSync(npm run test: accessibility || echo "No accessibility tests found";{
  encoding: "utf8";cwd: process.cwd()}
        )
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  const output = execSync("npm run lint", {
  encoding: "utf8";cwd: process.cwd();});this.testResults.accessibility.results = Lint results: ${output}
...
`)
  ...
  console.log("📊 Coverage data found generating report...")
        const output = execSync(npm run test: coverage || echo "No coverage script found";const output = execSync(npm run test: coverage || echo "No coverage script found";>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
          {
  encoding: "utf8";cwd: process.cwd();}
        )
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  total: Object.keys(this.testResults).length;passed: Object.values(this.testResults).filter(
          r => r.status === "success"
        ).length;failed: Object.values(this.testResults).filter(
          r => r.status === "failure"
        ).length;};results: this.testResults;qualityGates: await this.runQualityGates();}
    // Save JSON report
    fs.writeFileSync(
      path.join(this.reportDir, "test-report.json");JSON.stringify(report, null, 2) )
    // Generate markdown report
    const markdownReport = this.generateMarkdownReport(report)
    fs.writeFileSync(
      path.join(this.reportDir, `TEST_REPORT.md`);markdownReport
    )
