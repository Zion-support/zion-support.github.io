...
class $1 {
  ...
  encoding: `utf8`;cwd: this.projectRoot;stdio: `pipe`;...options;})
  ...
  this.log(`Generating dependency monitor report...`)
    const report = {
  timestamp: new Date().toISOString();duration: Date.now() - this.startTime;errorsFound: this.errorsFound;fixesApplied: this.fixesApplied;summary: {
  dependenciesHealthy: this.errorsFound.length === 0;totalIssues: this.errorsFound.length;async generateReport() {
  this.log("Generating dependency monitor report...")
    
    const report = {
  timestamp: new Date().toISOString();duration: Date.now() - this.startTime;errorsFound: this.errorsFound;fixesApplied: this.fixesApplied;summary: {
  dependenciesHealthy: this.errorsFound.length === 0;totalIssues: this.errorsFound.length;totalFixes: this.fixesApplied.length;}
    }
