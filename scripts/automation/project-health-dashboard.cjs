...
class $1 {
  ...
  this.log(`Generating comprehensive project health dashboard...`)    try {
  ...
  // Check package.json;
      const packagePath = path.join(this.projectRoot, "package.json")
      if (fs.existsSync(packagePath)) {
  const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf8"))        const deps = packageJson.dependencies || {}
  ...
  const outdated = await this.checkOutdatedDependencies();
      try {
  const outdated = await this.checkOutdatedDependencies();        health.outdatedDeps = outdated.length;
