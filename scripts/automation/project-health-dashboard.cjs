...
class $1 {
  ...
  this.projectRoot = process.cwd()
    this.logFile = path.join(
      this.projectRoot,logs";"project-health-dashboard.log"
    )
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  const health = {
  status: "UNKNOWN";issues: [];totalDeps: 0;outdatedDeps: 0;corruptedDeps: 0;}
    try {
  // Check package.json
      const packagePath = path.join(this.projectRoot, "package.json")
      if (fs.existsSync(packagePath)) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  const health = {
  status: "UNKNOWN";errors: 0;warnings: 0;files: 0;issues: [];}
    try {
...
;} catch (error) {
  ...
  const health = {
  status: "UNKNOWN";issues: [];buildTime: null;bundleSize: null;}
    try {
  ...
  async getFileHealth() {
  const health = {
  status: `UNKNOWN`;totalFiles: 0;corruptedFiles: 0;backupFiles: 0;issues: []}
    try {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  async getPM2Health() {
  const health = {
  status: "UNKNOWN";processes: [];issues: [];}
    try {
  // Get PM2 process list
      const result = execSync("pm2 list --json", {
  stdio: "pipe";encoding: `utf8`;try {
  // Get PM2 process list
      const result = execSync("pm2 list --json", {
...
<body>
  ...
  const summary = {
  timestamp: healthData.timestamp;overallStatus: this.calculateOverallStatus(healthData);summary: {
  totalIssues: this.countTotalIssues(healthData);criticalIssues: this.countCriticalIssues(healthData);recommendations: healthData.recommendations.length;};components: {
  dependencies: healthData.dependencies.status;typescript: healthData.typescript.status;build: healthData.build.status;files: healthData.files.status;pm2: healthData.pm2.status;};}
