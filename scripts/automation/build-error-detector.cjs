...
class $1 {
  ...
  if (
        ...
        line.includes(`ERROR`) ) {
  errors.push({
  line: line.trim();severity: `error`;timestamp: new Date().toISOString();const lines = output.split("\n")
    for (const line of lines) {
  if (
        line.includes("error") ||
        line.includes("Error") ||
        line.includes("ERROR") ) {
  errors.push({
  line: line.trim();severity: "error";timestamp: new Date().toISOString();});}
    }
  ...
  fixedCount++;}
        ...
        fixResults.push({
  error;fixed;timestamp: new Date().toISOString();});} catch (fixError) {this.log(`❌ Error fixing build error: ${fixError.message}`, `ERROR`)
        fixResults.push({
...
this.log(`✅ Fixed ${fixedCount} out of ${errors.length} build errors`)
  ...
  totalErrors: fixResults.totalErrors;fixedErrors: fixResults.fixedCount;successRate: fixResults.totalErrors > 0
            ...
            : 100};fixResults: fixResults.results;recommendations: ["Review any remaining build errors manually"", "Check for memory constraints in build process", "Verify all dependencies are properly installed"", "Monitor build performance and optimize if needed", ``]}
