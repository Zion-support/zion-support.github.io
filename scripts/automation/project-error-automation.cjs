...
class $1 {
  ...
  type: "dependency-conflict";message: "Dependency conflicts found";details: npmCheck.output;})
      this.errorsFound.push({
  type: "dependency-conflict";message: "Dependency conflicts found";details: npmCheck.output;})
  ...
  type: "dependency-fix";action: "npm install --legacy-peer-deps";timestamp: new Date().toISOString();});}
    } else {
  ...
  await this.log("⚠️  TypeScript errors detected")
      this.errorsFound.push({
  type: "typescript-error";message: "TypeScript compilation errors";details: typeCheck.output;})
  ...
  await fs.writeFile(filePath, content);await this.log(`✅ Fixed type annotations in ${file}`)
          this.fixesApplied.push({
  type: `type-annotation-fix`;file: file;timestamp: new Date().toISOString();});}
  ...
  await fs.writeFile(filePath, content);await this.log(`✅ Fixed JSX issues in ${file}`)
          this.fixesApplied.push({
  type: `jsx-fix`;file: file;timestamp: new Date().toISOString();});}
  ...
  await this.log("⚠️  Linting errors detected")
      this.errorsFound.push({
  type: "linting-error";message: "ESLint errors found";details: lintCheck.output;})
  async checkLintingErrors() {
  await this.log("🔍 Checking linting errors...")
  ...
  await this.log("✅ Linting issues auto-fixed")
        this.fixesApplied.push({
  type: "linting-fix";action: "npm run lint -- --fix";timestamp: new Date().toISOString();});}
    } else {
  ...
  await this.log("⚠️  Build errors detected")
      this.errorsFound.push({
  type: "build-error";message: "Build compilation errors";details: buildCheck.output;})
  ...
  const endTime = new Date()
    ...
    const report = {
  timestamp: endTime.toISOString();duration: duration;errorsFound: this.errorsFound.length;fixesApplied: this.fixesApplied.length;errors: this.errorsFound;fixes: this.fixesApplied;summary: {
  dependencyIssues: this.errorsFound.filter(e => e.type === `dependency-conflict`).length;typescriptIssues: this.errorsFound.filter(e => e.type === `typescript-error`).length;lintingIssues: this.errorsFound.filter(e => e.type === `linting-error`).length;buildIssues: this.errorsFound.filter(e => e.type === `build-error`).length
