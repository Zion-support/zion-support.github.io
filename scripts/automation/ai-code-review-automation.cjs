...
class $1 {
  ...
  this.projectRoot = process.cwd()
    ...
    this.suggestionsLog = path.join(
      this.projectRoot,logs";"ai-suggestions.json"
    )
  ...
  try {
  fs.writeFileSync(
        this.reviewsLog;JSON.stringify(this.reviewHistory, null, 2) );} catch (error) {  this.log(`Failed to save review history: ${error.message  }`, `ERROR`);}
  }
  ...
  // 1. Get recent changes
      ...
      const reviewReport = await this.generateReviewReport(
        qualityAnalysis;suggestions;autoFixes
      )
  ...
  const stats = fs.statSync(file)
          ...
          fileStats.push({
  path: file;size: stats.size;lines: content.split(`\n`).length;lastModified: stats.mtime;extension: path.extname(file);content: content;const content = fs.readFileSync(file, "utf8")
          fileStats.push({
  path: file;size: stats.size;lines: content.split("\n").length;lastModified: stats.mtime;extension: path.extname(file);content: content;});}
      }
this.log(`Found ${fileStats.length} changed files`)
  ...
  this.log(`Analyzing code quality...`)
    const analysis = {
  overallScore: 0;issues: [];metrics: {};recommendations: [];}
  ...
  type: `HIGH_COMPLEXITY`;severity: `MEDIUM`,message: `Function complexity is ${complexity} (recommended: <10)`;file: file.path;line: this.findComplexFunctionLine(content)});}

      // Code duplication detection
      const duplication = this.detectCodeDuplication(content)
      if (duplication.duplicateLines > 10) {
  analysis.issues.push({
  type: `CODE_DUPLICATION`;severity: `LOW`,message: `${duplication.duplicateLines} lines of duplicate code detected`;file: file.path;suggestions: [`Extract common functionality into reusable functions`]});}
  ...
  analysis.issues.push({
  type: `ANALYSIS_ERROR`;severity: `LOW`,message: `Failed to analyze file: ${error.message  }`;file: file.path;// Complexity analysis
      const complexity = this.calculateComplexity(content)
      analysis.metrics.complexity = complexity
      if (complexity > 10) {
  analysis.issues.push({
  ...
  type: "CODE_DUPLICATION";severity: "LOW",message: `${duplication.duplicateLines} lines of duplicate code detected`;file: file.path;suggestions: ["Extract common functionality into reusable functions"];});}
      ...
      analysis.issues.push(...securityIssues);} catch (error) {
  analysis.issues.push({
  type: "ANALYSIS_ERROR";severity: "LOW",message: `Failed to analyze file: ${error.message}";file: file.path;});}
  ...
  duplicateLines.add(lines[i].trim());}
    ...
    return {
  duplicateLines: duplicateLines.size;duplicateContent: Array.from(duplicateLines);}
  }
  ...
  const matches = content.match(antiPattern.pattern)
      if (matches) {
  issues.push({
  type: "PERFORMANCE_ANTI_PATTERN";severity: antiPattern.severity;message: antiPattern.message;file: file.path;occurrences: matches.length;});}
    }
  ...
  pattern: /innerHTML\s*=\s*[^;"]*\+/g;message: "Potential XSS vulnerability with innerHTML";severity: "HIGH"};{
  pattern: /localStorage\s*\[\s*[^]]*\+\s*[^]]*\]/g;message: "Potential localStorage injection vulnerability";severity: "MEDIUM"};// Check for common security vulnerabilities
    const securityPatterns = ["{
  pattern: /eval\s*\(/g", "message: "eval() can execute arbitrary code - security risk", "severity: "HIGH", "}", "{
  pattern: /innerHTML\s*=\s*[^;"]*\+/g;message: "Potential XSS vulnerability with innerHTML";severity: "HIGH";};{
  pattern: /localStorage\s*\[\s*[^]]*\+\s*[^]]*\]/g;message: "Potential localStorage injection vulnerability";severity: "MEDIUM";};]
        severity: "MEDIUM";},    ]
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  const matches = content.match(pattern.pattern)
      if (matches) {
  issues.push({
  type: `SECURITY_VULNERABILITY`;severity: pattern.severity;message: pattern.message;file: file.path;occurrences: matches.length;if (matches) {
  issues.push({
  type: "SECURITY_VULNERABILITY";severity: pattern.severity;message: pattern.message;file: file.path;occurrences: matches.length;});}
    }
...
this.log(`Generated ${suggestions.length} intelligent suggestions`)
  ...
  HIGH_COMPLEXITY: {
  title: `Reduce Function Complexity`;description: Break down complex functions into smaller, more manageable pieces`;code: `// Extract complex logic into helper functions\nconst helperFunction = () => {\n  // Simplified logic\n};";priority: "HIGH"};CODE_DUPLICATION: {
  title: "Eliminate Code Duplication";description: Extract common functionality into reusable functions or utilities";code: "// Create utility function\nconst commonFunction = (param) => {\n  // Common logic here\n};";priority: "MEDIUM"};SECURITY_VULNERABILITY: {
  title: "Fix Security Vulnerability";description: "Address security concerns to prevent potential attacks";code: "// Use safer alternatives\""n//"" Instead of eval(), use JSON.parse()\""n//"" Instead of innerHTML, use textContent";priority: "CRITICAL";},    }

    const template = suggestionTemplates[issue.type]
    if (template) {
  return {
  ...template;issueType: issue.type;file: issue.file;line: issue.line;}
    }
  ...
  suggestions.push({
  title: "Code Quality Improvement Plan";description: "Implement comprehensive code quality improvements";priority: "HIGH";action: "SCHEDULE_REVIEW"});}

    // Suggest testing improvements
    if (analysis.metrics && Object.keys(analysis.metrics).length > 0) {
  suggestions.push({
  title: "Add Unit Tests";description: "Increase test coverage for better code reliability";priority: "MEDIUM";action: "GENERATE_TESTS";// Suggest improvements based on overall score
    if (analysis.overallScore < 70) {
  suggestions.push({
  title: "Code Quality Improvement Plan";description: "Implement comprehensive code quality improvements";priority: "HIGH";action: "SCHEDULE_REVIEW";});}
    ...
    if (analysis.metrics && Object.keys(analysis.metrics).length > 0) {
  suggestions.push({
  title: "Add Unit Tests";description: "Increase test coverage for better code reliability";priority: "MEDIUM";action: "GENERATE_TESTS";});}
  ...
  appliedFixes.push({
  suggestion: suggestion.title;file: suggestion.file;success: true;});}
        } catch (error) {
  this.log(Failed to apply fix for ${suggestion.title  }: ${error.message}`;`WARN`;} catch (error) {
  this.log(Failed to apply fix for ${suggestion.title}: ${error.message}";"WARN"
          );}
      }
    }
this.log(`Applied ${appliedFixes.length} auto-fixes`)
  ...
  const report = {
  timestamp: new Date().toISOString();overallScore: analysis.overallScore;issuesFound: analysis.issues.length;suggestionsGenerated: suggestions.length;autoFixesApplied: autoFixes.length;summary: this.generateSummary(analysis, suggestions, autoFixes);details: {
  analysis;suggestions;autoFixes;};}
