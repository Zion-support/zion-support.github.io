...
class AICodeReviewAutomation {
  ...
  async runAICodeReview() {
    ...
    try {
      ...
      const suggestions = await this.generateIntelligentSuggestions(qualityAnalysis);
      
      // 4. Apply auto-fixes where safe
      const autoFixes = await this.applyAutoFixes(suggestions);
      
      // 5. Generate review report
      const reviewReport = await this.generateReviewReport(qualityAnalysis, suggestions, autoFixes);
  ...
  async getRecentChanges() {
    ...
    try {
      ...
      const gitDiff = execSync('git diff --name-only HEAD~5', { encoding: 'utf8' });
  ...
  async analyzeCodeQuality(files) {
    ...
    for (const file of files) {
      if (file.extension === '.js' || file.extension === '.ts' || file.extension === '.tsx') {
  ...
  detectPerformanceAntiPatterns(content) {
    ...
    const antiPatterns = [
      ...
      },
      {
        ...
        message: 'Consider using textContent for better security and performance',
        severity: 'MEDIUM'
      },
      {
  ...
  calculateQualityScore(analysis) {
    ...
    if (analysis.metrics && Object.keys(analysis.metrics).length > 0) score += 5;
  ...
  async generateIntelligentSuggestions(analysis) {
    ...
    const proactiveSuggestions = await this.generateProactiveSuggestions(analysis);
  ...
  async generateReviewReport(analysis, suggestions, autoFixes) {
    ...
    // Save report to file
    const reportPath = path.join(this.projectRoot, 'logs', `ai-review-${Date.now()}.json`);
