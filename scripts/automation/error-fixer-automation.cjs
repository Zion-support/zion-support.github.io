...
class $1 {
  ...
  let content = fs.readFileSync(file, "utf8")        let originalContent = content;
        ...
        // Remove duplicate imports;        // Remove duplicate function declarations;
        content = content.replace(;
          /(export\s+)?(function|const|let|var)\s+(\w+)[^]*;?\s*(export\s+)?(function|const|let|var)\s+\3/g,$1$2 $3";
  ...
  let content = fs.readFileSync(file, "utf8")        let originalContent = content;
        ...
        content = content;
          ...
          // Fix undefined variables;        // Fix common undefined variable issues;
        content = content;
          // Fix navigate function;
          .replace(/\bnavigate\s*\(/g, "router(");
          // Fix reject function;
  ...
  totalFixes: this.fixesApplied.length,
        ...
        successRate: this.fixesApplied.length > 0;
            ? Math.round(                (this.fixesApplied.length /;
