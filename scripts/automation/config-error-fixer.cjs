...
`)
  ...
  const issues = []
    ...
    if (content.includes(`eslint.config.js`)) {  // Check for proper export syntax;
      if (;
...
;
  ...
  const deps = Object.keys(pkg.dependencies)
        const devDeps = Object.keys(pkg.devDependencies)
        const conflicts = deps.filter(dep => devDeps.includes(dep))        // Move conflicts to devDependencies;
