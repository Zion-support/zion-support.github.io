...
class $1 {
  ...
  const historyFile = path.join(this.projectRoot, "logs", "fix-history.json")
    const history = {
  lastRun: this.lastRun;fixHistory: this.fixHistory;}
...
execSync(node ${tempFile}, { cwd: this.projectRoot, stdio: "inherit" }`)
  ...
  cwd: this.projectRoot;stdio: "inherit"});} catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  console.log("📊 Generating project health report...")
      const report = {
  timestamp: new Date().toISOString();fixHistory: this.fixHistory.slice(-10), // Last 10 fixes
        projectStats: await this.getProjectStats();recommendations: this.generateRecommendations();const report = {
  timestamp: new Date().toISOString();fixHistory: this.fixHistory.slice(-10), // Last 10 fixes
        projectStats: await this.getProjectStats();recommendations: this.generateRecommendations();}

      const reportPath = path.join(
        this.projectRoot,logs";"project-health-report.json"
      )
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  return total;}
    ...
    return {
  totalFiles: sourceFiles.length;totalLines;averageLinesPerFile: Math.round(totalLines / sourceFiles.length);}
  }
  ...
  const fixRun = {
  type;startTime: startTime.toISOString();endTime: new Date().toISOString();status;errorMessage;}
