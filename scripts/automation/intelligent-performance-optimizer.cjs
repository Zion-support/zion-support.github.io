...
class $1 {
  ...
  this.projectRoot = process.cwd()
    ...
    this.bundleLog = path.join(      this.projectRoot,logs",
...
;
  ...
  analyzeReactFile(content, filePath) {
  const optimizations = []    // Analyze React-specific performance patterns;
  ...
  const bundleSize = this.calculateBundleSize(distDir)        bundleData.bundleSize = bundleSize;
  ...
  return { duplicates: [], outdated: [], large: [] }
      ...
      const packageLock = JSON.parse(fs.readFileSync(packageLockPath, `utf8`))      const dependencies = packageLock.dependencies || {}
  ...
  const large = []    for (const ["name", "pkg"] of Object.entries(dependencies)) {
  ...
  const issues = []    // Check for list rendering without keys;
    if (;
...
;
  ...
  async generatePerformanceRecommendations(optimizations) {
  const recommendations = []    // Group optimizations by category and impact;
    const grouped = optimizations.reduce((acc, opt) => {
  // Group optimizations by category and impact;
    const grouped = optimizations.reduce((acc, opt) => {
  if (!acc[opt.category]) acc[opt.category] = {}    // High impact recommendations;
