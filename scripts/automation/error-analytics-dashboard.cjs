...
class ErrorAnalyticsDashboard {
  ...
  async collectFromAutomationReports() {
    ...
    for (const file of files) {
      if (file.endsWith('.json')) {
        try {
          const content = fs.readFileSync(path.join(this.reportsPath, file), 'utf8');
  ...
  async collectFromErrorReports() {
    ...
    for (const file of files) {
      if (file.endsWith('.json')) {
        try {
          const content = fs.readFileSync(path.join(this.errorReportsPath, file), 'utf8');
          const data = JSON.parse(content);
  ...
  analyzeTrends() {
    ...
    Object.keys(this.errorData).forEach(errorType => {
      ...
      // Weekly trends
      const weeklyErrors = errors.filter(e => new Date(e.timestamp) >= oneWeekAgo);
      ...
      // Monthly trends
      const monthlyErrors = errors.filter(e => new Date(e.timestamp) >= oneMonthAgo);
  ...
  identifyErrorPatterns() {
    ...
    Object.keys(this.errorData).forEach(errorType => {
      ...
      patterns[errorType] = commonPatterns.map(pattern => ({
        name: pattern.name,
  ...
  async generateDashboard() {
    ...
    const dashboardFile = path.join(this.reportsPath, 'error-analytics-dashboard.json');
  ...
  generateErrorTypeDistribution() {
    ...
    const total = Object.values(distribution).reduce((sum, d) => sum + d.totalErrors, 0);
  ...
  generateErrorTimeline() {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      Object.keys(this.errorData).forEach(errorType => {
        ...
        timeline[dateStr][errorType] = errors.reduce((sum, e) => sum + e.count, 0);
      });
    }
  ...
  generateSourceBreakdown() {
    ...
    Object.values(this.errorData).flat().forEach(error => {
  ...
  async run() {
    ...
    try {
      ...
      this.log(`💥 Error Analytics Dashboard failed: ${error.message}`, 'ERROR');
      throw error;
    }
  }
}
