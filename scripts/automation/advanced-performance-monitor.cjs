...
class AdvancedPerformanceMonitor {
  constructor() {
    ...
    this.alertsLog = path.join(this.projectRoot, 'logs', 'performance-alerts.json');
  ...
  async getBuildMetrics() {
    try {
      const buildLogPath = path.join(this.projectRoot, 'logs', 'build-performance.json');
  ...
  async analyzeMetrics(metrics) {
    ...
    if (metrics.application.build.lastBuildTime > this.alertThresholds.buildTime) {
      issues.push({
        type: 'BUILD_SLOW',
        severity: 'MEDIUM',
    ...
    if (metrics.application.build.lastBuildSize > this.alertThresholds.bundleSize) {
      issues.push({
        type: 'BUNDLE_LARGE',
        severity: 'MEDIUM',
    ...
    try {
      ...
      for (const dir of tempDirs) {
        ...
        if (fs.existsSync(fullPath)) {
          execSync(`rm -rf ${fullPath}`, { cwd: this.projectRoot, stdio: 'pipe' });
        }
      }
  ...
  async fixMemoryUsage() {
    try {
      ...
      if (metrics.system.memory.usage > 95) {
        ...
        return { success: true, message: 'PM2 processes restarted to free memory' };
      }
  ...
  async fixCPUUsage() {
    try {
      ...
      if (metrics.system.cpu.usage > 95) {
        ...
        return { success: true, message: 'PM2 processes scaled down to reduce CPU usage' };
      }
  ...
  async saveAlerts() {
    try {
      fs.writeFileSync(this.alertsLog, JSON.stringify(this.alertHistory, null, 2));
    } catch (error) {
  ...
  getPerformanceSummary() {
    ...
    const avgMemory = recentMetrics.reduce((sum, m) => sum + m.system.memory.usage, 0) / recentMetrics.length;
    const avgDisk = recentMetrics.reduce((sum, m) => sum + m.system.disk.usage, 0) / recentMetrics.length;
