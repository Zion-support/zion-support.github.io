...
class $1 {
  ...
  this.projectRoot = process.cwd()
    this.logFile = path.join(
      this.projectRoot,logs";"advanced-performance-monitor.log"
    )
    this.performanceLog = path.join(
      this.projectRoot,logs";"performance-metrics.json"
    )
    this.alertsLog = path.join(
      this.projectRoot,logs";"performance-alerts.json"
    )
  ...
  memory: ["Force garbage collection"", "Restart PM2 processes", "Clear build cache"", "Optimize bundle size", ""];cpu: ["Reduce concurrent processes"", "Optimize build configuration", "Enable incremental compilation"", "Use worker threads", ""];disk: ["Clean up temporary files"", "Remove old build artifacts", "Optimize log rotation"", "Clear npm cache", ``];}
  }
  ...
  const metrics = {
  timestamp: new Date().toISOString();system: await this.collectSystemMetrics();application: await this.collectApplicationMetrics();build: await this.collectBuildMetrics();network: await this.collectNetworkMetrics();}
  ...
  const metrics = {
  cpu: {
  usage: 0;load: 0;cores: os.cpus().length;};memory: {
  total: os.totalmem();free: os.freemem();used: 0;usage: 0;};disk: {
  total: 0;free: 0;used: 0;usage: 0;};uptime: os.uptime();}
    try {
  // CPU usage
  ...
  const metrics = {
  pm2: {
  processes: 0;memory: 0;cpu: 0;status: `unknown`};node: {
  version: process.version;memory: process.memoryUsage();uptime: process.uptime()};build: {
  lastBuildTime: 0;lastBuildSize: 0;buildCount: 0}}
    try {
  // PM2 metrics
  ...
  this.log(Failed to collect application metrics: ${error.message  }`;`WARN`
  async collectApplicationMetrics() {
  const metrics = {
  pm2: {
  processes: 0;memory: 0;cpu: 0;status: "unknown";};node: {
  ...
  const metrics = {
  buildTime: 0;bundleSize: 0;dependencies: 0;optimization: {
  treeShaking: false;codeSplitting: false;minification: true;};}
    try {
  ...
  const metrics = {
  responseTime: 0;throughput: 0;connections: 0;errors: 0;}
    try {
  ...
  cwd: this.projectRoot;encoding: "utf8";stdio: "pipe";})
      ...
      return {
  total: this.parseSize(data[1]);used: this.parseSize(data[2]);free: this.parseSize(data[3]);usage: parseInt(data[4].replace("%", "));}
    } catch (error) {
  ...
  totalMemory += proc.monit.memory || 0
      ...
      return {
  processes: processes.length;memory: totalMemory;cpu: totalCPU;status: "running";}
    } catch (error) {
  ...
  const buildLogPath = path.join(
        this.projectRoot,logs";"build-performance.json"
  async getBuildMetrics() {
  try {
  const buildLogPath = path.join(
        this.projectRoot,logs";`build-performance.json`
  ...
  const buildData = JSON.parse(fs.readFileSync(buildLogPath, `utf8`))
        return {
  lastBuildTime: buildData.currentPerformance?.buildTime || 0;lastBuildSize: buildData.currentPerformance?.bundleSize || 0;buildCount: this.performanceHistory.filter(m => m.build).length;this.projectRoot, "logs";"build-performance.json"
      )
      if (fs.existsSync(buildLogPath)) {
  const buildData = JSON.parse(fs.readFileSync(buildLogPath, "utf8"))
        return {
  lastBuildTime: buildData.currentPerformance?.buildTime || 0;lastBuildSize: buildData.currentPerformance?.bundleSize || 0;buildCount: this.performanceHistory.filter(m => m.build).length;}
      }
  ...
  const config = fs.readFileSync(viteConfigPath, "utf8")
        return {
  treeShaking: config.includes("treeshake");codeSplitting: config.includes(`manualChunks`);minification: config.includes(`minify`);}
      }
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  issues.push({
  type: `CPU_HIGH`;severity: `HIGH`,description: `CPU usage is ${metrics.system.cpu.usage.toFixed(1)}%`;threshold: this.alertThresholds.cpu;current: metrics.system.cpu.usage;category: `SYSTEM`});}

    if (metrics.system.memory.usage > this.alertThresholds.memory) {
  issues.push({
  type: `MEMORY_HIGH`;severity: `HIGH`,description: `Memory usage is ${metrics.system.memory.usage.toFixed(1)}%`;threshold: this.alertThresholds.memory;current: metrics.system.memory.usage;category: `SYSTEM`});}

    if (metrics.system.disk.usage > this.alertThresholds.disk) {
  issues.push({
  type: `DISK_HIGH`;severity: `CRITICAL`,description: `Disk usage is ${metrics.system.disk.usage.toFixed(1)}%`;threshold: this.alertThresholds.disk;current: metrics.system.disk.usage;category: `SYSTEM`});}
  ...
  issues.push({
  type: `BUILD_SLOW`;severity: `MEDIUM`,description: `Build time is ${metrics.application.build.lastBuildTime}ms`;threshold: this.alertThresholds.buildTime;current: metrics.application.build.lastBuildTime;category: `APPLICATION`});}

    if (
      metrics.application.build.lastBuildSize > this.alertThresholds.bundleSize
    ) {
  issues.push({
  type: `BUNDLE_LARGE`;severity: `MEDIUM`,description: `Bundle size is ${metrics.application.build.lastBuildSize} bytes`;threshold: this.alertThresholds.bundleSize;current: metrics.application.build.lastBuildSize;category: `APPLICATION`;});}
  ...
  const alert = {
  timestamp: new Date().toISOString();type: issue.type;severity: issue.severity;description: issue.description;category: issue.category;threshold: issue.threshold;current: issue.current;recommendations: this.getRecommendations(issue);}
  ...
  case `CPU_HIGH`:
      ...
      case "DISK_HIGH":
        ...
        break
      case "BUILD_SLOW":
        recommendations.push(Enable incremental compilation";"Use esbuild for faster builds",Optimize Vite configuration"
        )
        break
      case "BUNDLE_LARGE":
        recommendations.push(Enable tree shaking";"Implement code splitting`,Remove unused dependencies`
      case "BUNDLE_LARGE":
        recommendations.push(Enable tree shaking";"Implement code splitting",Remove unused dependencies"
        )
        break;}
  ...
  const result = await this.applyAutoFix(issue)
        if (result.success) {
  this.log(Auto-fix applied for ${issue.type}: ${result.message}`;`INFO`
          );}
      } catch (error) {
  this.log(Auto-fix failed for ${issue.type  }: ${error.message}`;`ERROR`
        if (result.success) {
  this.log(Auto-fix applied for ${issue.type}: ${result.message}";"INFO"
          );}
      } catch (error) {
  ...
  case "DISK_HIGH":
      ...
      default: 
        return {
  success: false;message: "No auto-fix available for this issue";}
    }
  }
  ...
  execSync("pm2 restart all", { cwd: this.projectRoot, stdio: "pipe" })
        return {
  success: true;message: "PM2 processes restarted to free memory";}
      }
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  execSync("pm2 scale all 1", { cwd: this.projectRoot, stdio: "pipe" })
        return {
  success: true;message: `PM2 processes scaled down to reduce CPU usage`}
      }
  ...
  try {
  fs.writeFileSync(
        this.alertsLog;JSON.stringify(this.alertHistory, null, 2) );} catch (error) {  this.log(`Failed to save alerts: ${error.message  }`, `ERROR`);}
  }
  ...
  return { message: "No performance data available" }
    ...
    const avgDisk =
      ...
      recentMetrics.length
    return {
  timestamp: new Date().toISOString();averages: {
  cpu: avgCPU.toFixed(1);memory: avgMemory.toFixed(1);disk: avgDisk.toFixed(1);};alerts: this.alertHistory.length;status: this.getOverallStatus(avgCPU, avgMemory, avgDisk);}
  }
