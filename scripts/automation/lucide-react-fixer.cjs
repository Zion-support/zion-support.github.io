...
<<<<<<< HEAD      this.log(`Icon fix automation failed: ${error.message}`, 'ERROR');
    }
;
    return { fixes, errors };
  }
...
<<<<<<< HEAD      .replace(/,\s*}/g, '}') // Remove trailing commas;
