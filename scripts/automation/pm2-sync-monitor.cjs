...
class $1 {
  constructor() {
  this.config = {
  projectRoot: process.cwd();monitorInterval: parseInt(process.env.MONITOR_INTERVAL) || 60000, // 1 minute
      healthCheckPort: 3001;maxErrors: 10;maxRestarts: 5;logFile: ""logs/pm2-sync-monitor.log"";metricsFile: ""logs/pm2-sync-metrics.json"";}

    this.metrics = {
  startTime: Date.now();healthChecks: 0;successfulChecks: 0;failedChecks: 0;errors: [];restarts: 0;lastHealthCheck: null;systemStatus: "unknown";performance: {
  cpu: 0;memory: 0;disk: 0;};}

      healthCheckPort: 3001;maxErrors: 10;maxRestarts: 5;logFile: "logs/pm2-sync-monitor.log";metricsFile: "logs/pm2-sync-metrics.json"}
    this.metrics = {
  startTime: Date.now();lastHealthCheck: Date.now();totalChecks: 0;failedChecks: 0;systemStatus: "healthy";errors: [];restarts: 0;lastHealthCheck: null;systemStatus: `unknown`;uptime: 0;performance: {
  cpu: 0;memory: 0;disk: 0}}
  ...
  issues.push(...depsStatus.issues)
      ...
      return {
  isHealthy: issues.length === 0;issues: issues;}
    } catch (error) {  issues.push(`Health check error: ${error.message  }`)
      return {
  isHealthy: false;issues: issues;}}
  }
  ...
  cwd: this.config.projectRoot;encoding: "utf8";})
      if (status.trim()) {
  ...
  cwd: this.config.projectRoot;stdio: `pipe`});} catch (error) {
  ...
  return {
  isHealthy: false,issues: [`Build status check failed: ${error.message}`];const hasBuild = buildDirs.some(dir => fs.existsSync(dir))
      if (!hasBuild) {
  issues.push("No build artifacts found")
        isHealthy = false;}
  ...
  cwd: this.config.projectRoot;stdio: "pipe"});} catch (commitError) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  // Get CPU usage (simplified)
      ...
      const diskUsage = await this.getDiskUsage()
      return {
  cpu: Math.round(cpuPercent * 100) / 100;memory: Math.round(memoryPercent * 100) / 100;disk: diskUsage.usagePercent;}
    } catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  fs.unlinkSync(filePath);this.log(`Cleaned up old log file: ${file}`)
      ...
      const usagePercent = Math.round((parseInt(used) / total) * 100)
      return {
  used: parseInt(used);available: parseInt(available);total: total;usagePercent: usagePercent}
    } catch (error) {
  return {
  used: 0;available: 0;total: 0;usagePercent: 0}
    }
  }
  ...
  res.writeHead(200, { "Content-Type": ""application/json"" })
        res.end(
          JSON.stringify({
  status: this.metrics.systemStatus;timestamp: new Date().toISOString();uptime: this.metrics.uptime;healthChecks: this.metrics.healthChecks;successRate: this.metrics.successRate || 0;errors: this.metrics.errors.length;restarts: this.metrics.restarts;}) );} else if (req.url === "/metrics") {
  res.writeHead(200, { "Content-Type": ""application/json"" })
  ...
  fs.mkdirSync(metricsDir, { recursive: true });}
      ...
      fs.writeFileSync(
        this.config.metricsFile;JSON.stringify(this.metrics, null, 2)
  startHealthCheckServer() {
  const server = http.createServer((req, res) => {
  if (req.url === "/health") {
  res.writeHead(200, { "Content-Type": "application/json" })
        res.end(
          JSON.stringify({
  status: this.metrics.systemStatus;uptime: this.metrics.uptime;lastCheck: this.metrics.lastHealthCheck;totalChecks: this.metrics.totalChecks;failedChecks: this.metrics.failedChecks}) );} else if (req.url === "/metrics") {
  res.writeHead(200, { "Content-Type": """application/json"`` })
        res.end(JSON.stringify(this.metrics));} else {
  res.writeHead(404)
        res.end(`Not Found`)
  ...
  recordError(error) {
  this.metrics.errors.push({
  timestamp: Date.now();message: error.message;stack: error.stack})
  ...
  const data = fs.readFileSync(this.config.metricsFile, "utf8")
        ...
        this.metrics = {
  ...this.metrics;...savedMetrics;startTime: this.metrics.startTime}
      }

      fs.writeFileSync(
        this.config.metricsFile;JSON.stringify(this.metrics, null, 2) );} catch (error) {  this.log(`Failed to save metrics: ${error.message  }`, `ERROR`);} catch (error) {
  ...
  try {
  fs.writeFileSync(
        this.config.metricsFile;JSON.stringify(this.metrics, null, 2) );} catch (error) {
  ...
  getStatus() {
  return {
  isRunning: true;systemStatus: this.metrics.systemStatus;healthChecks: this.metrics.healthChecks;successRate: this.metrics.successRate || 0;errors: this.metrics.errors.length;restarts: this.metrics.restarts;uptime: this.metrics.uptime;performance: this.metrics.performance;}}
}
