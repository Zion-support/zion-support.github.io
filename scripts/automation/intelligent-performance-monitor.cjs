...
class $1 {
  ...
  this.projectRoot = process.cwd()
    this.logFile = path.join(
      this.projectRoot,logs";"performance-monitor.log"
    )
    this.performanceHistory = path.join(
      this.projectRoot,logs";"performance-history.json"
    );    this.metrics = {
  buildTime: 0;bundleSize: 0;loadTime: 0;memoryUsage: 0;cpuUsage: 0;errorRate: 0;}
    this.thresholds = {
  this.metrics = {
  buildTime: 0;bundleSize: 0;loadTime: 0;memoryUsage: 0;cpuUsage: 0;errorRate: 0;}
    this.thresholds = {
  ...
  this.history = {
  totalMeasurements: 0;averageBuildTime: 0;averageBundleSize: 0;averageLoadTime: 0;performanceTrends: [];bottlenecks: [];optimizations: [];}
      }
    } catch (error) {
  ...
  console.log(``📈 Measuring application performance...`)
    ...
    const measurement = {
  timestamp: new Date().toISOString();buildTime: 0;bundleSize: 0;loadTime: 0;memoryUsage: 0;cpuUsage: 0;errorRate: 0;recommendations: [];}
    try {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  type: `build`;severity: `warning`,message: Build time (${measurement.buildTime}ms) exceeds threshold (${this.thresholds.buildTime}ms);suggestion: `Consider optimizing build configuration, using build caching, or parallel builds`;}`);}
      ;} catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  cwd: this.projectRoot;stdio: "pipe";async runTestBuild() {
  try {
  // Check if it"s a Next.js project
      if (fs.existsSync("next.config.js") || fs.existsSync("next.config.mjs")) {
  const buildStart = Date.now()
        ...
        // Run Next.js build
        execSync("npm run build", {
  cwd: this.projectRoot;stdio: "pipe";timeout: 120000 // 2 minutes timeout;})
        
        return {
  success: true;duration: Date.now() - buildStart;errors: [];}
      }
  ...
  const buildStart = Date.now()
        // Run Vite build
        execSync("npm run build", {
  cwd: this.projectRoot;stdio: "pipe";<<<<<<< HEAD
          timeout: 120000
=======
<<<<<<< HEAD          timeout: 120000
>>>>>>> cursor/automate-test-fix-improve-and-merge-code-1c7d;})
  ...
  success: true;duration: Date.now() - buildStart;errors: [];}
      ...
      // Generic build check
      return {
  success: true;duration: 0;errors: [];}
      ;} catch (error) {
  return {
  success: false;duration: 0;errors: [error.message];}
  ...
  measurement.recommendations.push({
  type: `bundle`;severity: `warning`,message: Bundle size (${this.formatBytes(totalSize)}) exceeds threshold (${this.formatBytes(this.thresholds.bundleSize)});suggestion: `Consider code splitting, tree shaking, or removing unused dependencies`;}`);}
      ;} catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  measurement.recommendations.push({
  type: `memory`;severity: `warning`,message: Memory usage (${this.formatBytes(systemMetrics.memory)}) exceeds threshold (${this.formatBytes(this.thresholds.memoryUsage)});suggestion: `Check for memory leaks, optimize data structures, or increase memory limits`;}`);}
      
      // Check CPU usage
      if (systemMetrics.cpu > this.thresholds.cpuUsage) {
  measurement.recommendations.push({
  type: `cpu`;severity: `warning`,message: `CPU usage (${systemMetrics.cpu}%) exceeds threshold (${this.thresholds.cpuUsage}%)`;suggestion: `Optimize algorithms, implement caching, or use worker threads for heavy operations`;});}
      ;} catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  const buildTimeTrend = this.calculateTrend(
          recentTrends.map(t => t.buildTime);measurement.buildTime
        )
        if (buildTimeTrend > 0.1) { // 10% increase
          measurement.recommendations.push({
  type: "trend";severity: "info";message: "Build time is trending upward";suggestion: "Investigate recent changes that may be affecting build performance";});}
      }
      
      // Calculate trend for bundle size
      if (recentTrends.length > 1) {
  const bundleSizeTrend = this.calculateTrend(
          recentTrends.map(t => t.bundleSize);measurement.bundleSize
        )
        if (bundleSizeTrend > 0.1) { // 10% increase
          measurement.recommendations.push({
  type: "trend";severity: "info";message: "Bundle size is trending upward";suggestion: "Review recent dependencies and code changes for size impact";});}
      }
    }
  }
  ...
  bottlenecks.push({
  type: "build";severity: "critical";message: "Build time is critically slow";impact: "High";suggestion: "Immediate investigation required - check build configuration and dependencies"
  ...
  bottlenecks.push({
  type: "bundle";severity: "critical";message: "Bundle size is critically large";impact: "High";suggestion: "Immediate optimization required - implement code splitting and tree shaking";});}
    
    if (measurement.memoryUsage > this.thresholds.memoryUsage * 1.5) {
  bottlenecks.push({
  type: "memory";severity: "high";message: "Memory usage is significantly high";impact: "Medium";suggestion: "Investigate memory leaks and optimize data handling";});}
  ...
  const optimizations = []
    ...
    if (measurement.buildTime > this.thresholds.buildTime) {
  optimizations.push({
  category: "build";priority: "high";suggestion: "Implement build caching using tools like Turborepo or Nx";expectedImpact: "20-40% reduction in build time";})
      
      optimizations.push({
  category: "build";priority: "medium";suggestion: "Use parallel builds for independent modules";expectedImpact: "15-25% reduction in build time";});}
    
    // Bundle optimizations
    if (measurement.bundleSize > this.thresholds.bundleSize) {
  optimizations.push({
  category: "bundle";priority: "high";suggestion: "Implement dynamic imports and code splitting";expectedImpact: "30-50% reduction in initial bundle size"
  ...
  category: "bundle";priority: "high";suggestion: "Implement dynamic imports and code splitting";expectedImpact: "30-50% reduction in initial bundle size";})
      ...
      optimizations.push({
  category: "bundle";priority: "medium";suggestion: "Remove unused dependencies and implement tree shaking";expectedImpact: "10-20% reduction in bundle size";});}
    
    // Runtime optimizations
    if (measurement.memoryUsage > this.thresholds.memoryUsage) {
  optimizations.push({
  category: "runtime";priority: "high";suggestion: "Implement memory pooling and object reuse";expectedImpact: "25-40% reduction in memory usage";});}
  ...
  timestamp: measurement.timestamp;buildTime: measurement.buildTime;bundleSize: measurement.bundleSize;loadTime: measurement.loadTime;memoryUsage: measurement.memoryUsage;cpuUsage: measurement.cpuUsage;})
  ...
  this.history.performanceTrends = this.history.performanceTrends.slice(-100);}
    ...
    this.history.averageBuildTime = this.calculateRunningAverage(
      this.history.averageBuildTime;measurement.buildTime;this.history.totalMeasurements
    )
    this.history.averageBundleSize = this.calculateRunningAverage(
      this.history.averageBundleSize;measurement.bundleSize;this.history.totalMeasurements
    )
  ...
  buildTime: measurement.buildTime;bundleSize: measurement.bundleSize;loadTime: measurement.loadTime;memoryUsage: measurement.memoryUsage;cpuUsage: measurement.cpuUsage;};recommendations: measurement.recommendations;bottlenecks: measurement.bottlenecks;logPerformanceResults(measurement) {
  const logEntry = {
  timestamp: new Date().toISOString();metrics: {
  buildTime: measurement.buildTime;bundleSize: measurement.bundleSize;loadTime: measurement.loadTime;memoryUsage: measurement.memoryUsage;cpuUsage: measurement.cpuUsage;};recommendations: measurement.recommendations;bottlenecks: measurement.bottlenecks;optimizations: measurement.optimizations;}
