...
class $1 {
  ...
  production: {};development: {};peer: {};optional: {};}
    this.analysis = {
  conflicts: [];vulnerabilities: [];outdated: [];unused: [];recommendations: [];}
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  name: pkg;current: outdatedData[pkg].current;wanted: outdatedData[pkg].wanted;latest: outdatedData[pkg].latest;location: outdatedData[pkg].location;}))
  ...
  conflicts.push({
  type: "peer";package: pkg;required: version;installed: installedVersion;severity: "high";});}
      }
  ...
  conflicts.push({
  type: `version`;package: pkg;version1: prodVersions.get(pkg);version2: version;severity: `medium`;});} else {
  ...
  encoding: `utf8`;cwd: process.cwd(),    console.log("🛡️  Checking for vulnerabilities..."`)
    try {
  const auditOutput = execSync("npm audit --audit-level moderate --json", {
  encoding: "utf8";cwd: process.cwd();})
  ...
  encoding: `utf8`;cwd: process.cwd();console.log(`🛡️  Found ${this.analysis.vulnerabilities.length} vulnerabilities`) );} catch (error) {
  console.log("⚠️  Could not check vulnerabilities: ", error.message);}
  }
  ...
  dependencies: depcheckData.dependencies || [];devDependencies: depcheckData.devDependencies || [];}
  ...
  type: "security";priority: "critical",message: Update ${this.analysis.vulnerabilities.length} vulnerable packages immediately;action: "run npm audit fix";});}

    // Conflict resolution recommendations
    for (const conflict of this.analysis.conflicts) {
  if (conflict.severity === "high") {
  recommendations.push({
  type: "conflict";priority: "high",message: Resolve peer dependency conflict for ${conflict.package}`,action: `update ${conflict.package} to compatible version`;}`);}
    }
  ...
  recommendations.push({
  type: "update";priority: "high",message: `Consider major version updates for ${criticalUpdates.length} packages`;action: "review and update major versions carefully";});}

    // Cleanup recommendations
    if (this.analysis.unused.dependencies.length > 0) {
  recommendations.push({
  type: "cleanup";priority: "medium",message: `Remove ${this.analysis.unused.dependencies.length} unused production dependencies`;action: "remove unused dependencies to reduce bundle size";});}
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  totalDependencies:
          ...
          Object.keys(this.dependencies.development).length;outdated: this.analysis.outdated.length;vulnerabilities: this.analysis.vulnerabilities.length;conflicts: this.analysis.conflicts.length;unused: 
          this.analysis.unused.dependencies.length +
          this.analysis.unused.devDependencies.length;};analysis: this.analysis;recommendations: this.analysis.recommendations;dependencies: this.dependencies;}
