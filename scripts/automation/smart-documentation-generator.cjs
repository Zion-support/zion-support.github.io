...
class $1 {
  ...
  generatedDocs: [];documentationCoverage: 0;missingDocs: [];documentationSuggestions: [];docPatterns: new Map();}
  ...
  console.log("⚠️ Source directory not found")
    ...
    const structure = {
  components: [];utilities: [];hooks: [];services: [];types: [];pages: [];configs: [];}
  ...
  path: relativePath;name: this.extractComponentName(content);hasDocs: this.hasExistingDocs(file);});} else if (
        relativePath.includes("utils/") ||
        relativePath.includes("helpers/") ||
        content.includes("export function") ||
        content.includes("export const") ) {
  structure.utilities.push({
  path: relativePath;name: this.extractFunctionName(content);hasDocs: this.hasExistingDocs(file);});} else if (content.includes("use") && content.includes("export")) {
  structure.hooks.push({
  path: relativePath;name: this.extractHookName(content);hasDocs: this.hasExistingDocs(file);});} else if (
        relativePath.includes("services/") ||
        content.includes("api") ||
        content.includes("fetch") ) {
  structure.services.push({
  path: relativePath;name: this.extractServiceName(content);hasDocs: this.hasExistingDocs(file);});} else if (
        relativePath.includes("types/") ||
        content.includes("interface") ||
        content.includes("type ") ) {
  structure.types.push({
  path: relativePath;name: this.extractTypeName(content);});} else if (relativePath.includes("pages/") || content.includes("Route")) {
  structure.pages.push({
  path: relativePath;name: this.extractPageName(content);});}
    }
  ...
  path: configFile", name: configFile;hasDocs: this.hasExistingDocs(configPath);});}
    }
  ...
  missingDocs.push({
  type: "component";items: undocumentedComponents;priority: "high";});}
  ...
  missingDocs.push({
  type: "utility";items: undocumentedUtilities;priority: "medium";});}
  ...
  missingDocs.push({
  type: "hook";items: undocumentedHooks;priority: "medium";});}
  ...
  missingDocs.push({
  type: `service`;items: undocumentedServices;priority: `high`;// Check for missing service documentation
    const undocumentedServices = structure.services.filter(
      service => !service.hasDocs
    )
    if (undocumentedServices.length > 0) {
  missingDocs.push({
  type: "service";items: undocumentedServices;priority: "high";});}
  ...
  const servicePath = path.join(process.cwd(), "src`, service.path)
        ...
        const docPath = path.join(
          this.docsDir,api`;${service.name.toLowerCase()}.md
        )
    // Generate documentation for services
    for (const service of structure.services.slice(0, 5)) {
  // Limit to 5 services per run
  ...
  type: `api`;path: docPath;service: service.name;timestamp: new Date().toISOString()})
console.log(✅ Generated API documentation for ${service.name}`);} catch (error) {
  ...
  const componentPath = path.join(process.cwd(), "src`, component.path)
        ...
        const docPath = path.join(
          this.docsDir,components`;${component.name.toLowerCase()}.md
        )
  ...
  fs.mkdirSync(componentsDocsDir, { recursive: true });}
        ...
        this.documentationMetrics.generatedDocs.push({
  type: `component`;path: docPath;component: component.name;timestamp: new Date().toISOString();this.documentationMetrics.generatedDocs.push({
  type: "component";path: docPath;component: component.name;timestamp: new Date().toISOString();})
  ...
  console.log(``📋 Generating setup guides...`)
    ...
    this.documentationMetrics.generatedDocs.push(
      {
  type: "guide";path: installPath;name: "Installation Guide";timestamp: new Date().toISOString();};{
  type: "guide";path: devPath;name: "Development Guide";timestamp: new Date().toISOString();};{
  type: "guide";path: deployPath;name: "Deployment Guide";timestamp: new Date().toISOString();}
    )
  ...
  readmeContent = fs.readFileSync(readmePath, "utf8");}
      ...
      this.documentationMetrics.generatedDocs.push({
  type: "readme";path: readmePath;name: "README.md";timestamp: new Date().toISOString();})
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  totalFilesAnalyzed: this.documentationMetrics.docPatterns.get(structure`
          ...
          ? Object.values(
              this.documentationMetrics.docPatterns.get(`structure`) ).reduce((sum, arr) => sum + arr.length, 0)
          : 0;generatedDocs: this.documentationMetrics.generatedDocs.length;missingDocs: this.documentationMetrics.missingDocs.length;documentationCoverage: this.documentationMetrics.documentationCoverage};metrics: this.documentationMetrics;generated: this.documentationMetrics.generatedDocs;missing: this.documentationMetrics.missingDocs}
    ...
    const latestReportPath = path.join(
      process.cwd();`documentation-report.json`
    )
...
const result = await ${serviceName}.createItem({
  name: `Example`;description: `Test item`;});\`\`\``;}
...
<${componentName} className="custom-class">
  ...
  // Generate basic descriptions based on function name
    const descriptions = {
  fetch: `Fetches data from the API`;create: "Creates a new resource";update: "Updates an existing resource";delete: "Deletes a resource";get: "Retrieves data";set: "Sets data";validate: "Validates input data";format: "Formats data for display"}
