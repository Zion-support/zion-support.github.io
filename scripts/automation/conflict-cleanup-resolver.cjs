...
class $1 {
  ...
  this.projectRoot = process.cwd()
    ...
    this.cleanupStats = {
  conflictsResolved: 0;filesCleaned: 0;errors: 0;}
  }
...
`)
  ...
  this.log(⚠️ Failed to remove backup file ${file  }: ${error.message}`;`WARN`
              );}
          }
        } catch (error) {
  ...
  if (!fs.existsSync(filePath)) {this.log(`⚠️ File does not exist: ${filePath}`, `WARN`)
      ...
      if (
        !content.includes("<<<<<<<") &&
        !content.includes(``) &&
        !content.includes(`>>>>>>>`) ) {this.log(`✅ No conflicts in: ${filePath}`)
        return;}
...
this.log(`✅ Resolved conflicts in: ${filePath}`)
  ...
  this.log(❌ Failed to resolve conflicts in ${filePath  }: ${error.message}`;`ERROR`;} catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  this.log(⚠️ Failed to remove deleted file ${file  }: ${error.message}`;`WARN`
          );}
      }
    } catch (error) {
  this.log(❌ Error cleaning up duplicate files: ${error.message  }`;`ERROR`
      );}
  }
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  conflictsResolved: this.cleanupStats.conflictsResolved;filesCleaned: this.cleanupStats.filesCleaned;errors: this.cleanupStats.errors}}
