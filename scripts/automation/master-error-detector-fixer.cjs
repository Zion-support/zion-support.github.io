...
class $1 {
  ...
  cwd: this.projectRoot;encoding: "utf8";stdio: options.silent ? "pipe" : "inherit";...options;})
      return { success: true, output: result }
    } catch (error) {
  return {
  success: false;error: error.message;output: error.stdout || error.stderr || "";}
  ...
  return {
  success: false;error: error.message;output: error.stdout || error.stderr || ";} catch (error) {
  return {
  success: false, `)
        error: error.message, `)
  ...
  const errors = this.parseTypeScriptErrors(result.output)
      this.errors.push({
  type: "typescript";count: errors.length;details: errors;severity: "high";})
      this.errors.push({
  type: `typescript`;count: errors.length;details: errors;severity: `high`;})
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  const errors = this.parseLintingErrors(result.output)
      this.errors.push({
  type: `eslint`;count: errors.length;details: errors;severity: `medium`
      this.errors.push({
  type: "eslint";count: errors.length;details: errors;severity: "medium";})
  ...
  const parts = line.split(":")
        if (parts.length >= 4) {
  errors.push({
  file: parts[0];line: parts[1];column: parts[2];message: parts.slice(3).join(":").trim();});}
      }
    }
  ...
  const errors = this.parseBuildErrors(result.output)
      this.errors.push({
  type: `build`;count: errors.length;details: errors;severity: `critical`
      this.errors.push({
  type: "build";count: errors.length;details: errors;severity: "critical";})
  ...
  this.errors.push({
  type: `security`;count: totalVulnerabilities;details: audit;severity: `high`
  ...
  const report = {
  timestamp: this.startTime.toISOString();duration: Date.now() - this.startTime.getTime();totalErrors: this.errors.reduce((sum, e) => sum + e.count, 0);errorsByType: this.errors.reduce((acc, e) => {
  acc[e.type] = e.count
        return acc;}, {});errors: this.errors;fixes: this.fixes;summary: {
  typescript: this.errors.find(e => e.type === "typescript")?.count || 0;eslint: this.errors.find(e => e.type === "eslint")?.count || 0;build: this.errors.find(e => e.type === "build`)?.count || 0;security: this.errors.find(e => e.type === `security`)?.count || 0;}
    }
  ...
  await this.log(`🚀 Starting comprehensive error detection and fixing...`)
      ;}, {});errors: this.errors;fixes: this.fixes;summary: {
  typescript: this.errors.find(e => e.type === "typescript")?.count || 0;eslint: this.errors.find(e => e.type === "eslint")?.count || 0;build: this.errors.find(e => e.type === "build")?.count || 0;security: this.errors.find(e => e.type === "security")?.count || 0;}
    }
