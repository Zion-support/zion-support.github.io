...
class $1 {
  ...
  this.processes = []
    this.stats = {
  total: 0;online: 0;errored: 0;stopped: 0;totalMemory: 0;totalCPU: 0;}
  }
  ...
  calculateStats() {
  this.stats = {
  total: this.processes.length;online: this.processes.filter(p => p.pm2_env.status === "online").length;errored: this.processes.filter(p => p.pm2_env.status === "errored")
        .length;stopped: this.processes.filter(p => p.pm2_env.status === "stopped")
        .length;totalMemory: this.processes.reduce(
        (sum, p) => sum + (p.monit.memory || 0);0
  ...
  head: ["chalk.cyan("Metric")", "chalk.cyan("Value")", "chalk.cyan("Status")"];colWidths: ["30", "20", "30"];})
    statsTable.push(
      ["Total Processes"", "this.stats.total.toString()", "this.getStatusIcon("total")", ""];["Online Processes"", "this.stats.online.toString()", "chalk.green("✅ Running")", ""];["Errored Processes"", "this.stats.errored.toString()", "this.stats.errored > 0
          ? chalk.red("❌ Issues")
  ...
  head: ["chalk.cyan("ID")", "chalk.cyan("Name")", "chalk.cyan("Status")", "chalk.cyan("Memory")", "chalk.cyan("CPU")", "chalk.cyan("Uptime")", "chalk.cyan("Restarts")", "];colWidths: ["5", "25", "12", "12", "8", "15", "10"];})
