...
class SmartTestingAutomation {
  constructor() {
    ...
    this.logFile = path.join(this.projectRoot, 'logs', 'smart-testing-automation.log');
    this.testResultsLog = path.join(this.projectRoot, 'logs', 'test-results.json');
    this.coverageLog = path.join(this.projectRoot, 'logs', 'test-coverage.json');
  ...
  async runSmartTesting() {
    ...
    try {
      ...
      await this.generateTestReport(testResults, coverageData, generatedTests, fixedTests);
  ...
  analyzeTestFile(content, filePath) {
    ...
    const componentMatches = content.match(/import\s+{?\s*([A-Z][a-zA-Z0-9]*)\s*}?\s+from/g);
    if (componentMatches) {
      componentMatches.forEach(match => {
        const componentName = match.match(/import\s+{?\s*([A-Z][a-zA-Z0-9]*)\s*}?\s+from/)[1];
      ...
      for (const pattern of patterns) {
        ...
        for (const match of matches) {
          ...
          analysis[category].push({
            ...
            line: lineNumber,
            exportType: pattern.pattern.source.includes('default') ? 'default' : 'named'
          });
        }
      }
    }
...
`;
  ...
  getTestFilePath(item) {
    ...
    const testFilePath = path.join(testDir, relativePath.replace(/\.(tsx?|jsx?)$/, '.test.$1'));
  ...
  async generateTestReport(testResults, coverageData, generatedTests, fixedTests) {
    const report = {
      timestamp: new Date().toISOString(),
      summary: {
        testResults,
