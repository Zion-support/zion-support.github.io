...
class $1 {
  ...
  extends: [""next/core-web-vitals"", ""next/typescript""];rules: {@typescript-"eslint/no-unused-vars"": "warn",@typescript-"eslint/no-explicit-any"": "warn","react/react-in-jsx-scope"": "off","react/prop-types"": "off",no-console": "warn",no-unused-vars": "warn",prefer-const": "warn",no-var": "error";};ignorePatterns: ["node_modules/", ".next/", "out/", "dist/", "automation/"];}
      
    try {
  const eslintPath = path.join(this.projectRoot, ".eslintrc.js");const fixedConfig = module.exports = {`)
  extends: ["""next/core-web-vitals""", """next/typescript"""],`)
  rules: {@typescript-""eslint/no-unused-vars""": "warn",@typescript-""eslint/no-explicit-any""": "warn",""react/react-in-jsx-scope""": "off",""react/prop-types""": "off",no-console": "warn",no-unused-vars": "warn",prefer-const": "warn",no-var": "error`);};ignorePatterns: ["node_modules/", ".next/", "out/", "dist/", "automation/"];};`)
      `)
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  type: `eslint-config-fix`;file: `.eslintrc.js`;timestamp: new Date().toISOString()
      this.fixesApplied.push({
  type: "eslint-config-fix";file: ".eslintrc.js";timestamp: new Date().toISOString();});} catch (error) {  await this.log(`❌ Error fixing ESLint config: ${error.message  }`);}
  }
  ...
  await this.log("✅ ESLint auto-fix completed")
      this.fixesApplied.push({
  type: "eslint-auto-fix";action: "npm run lint -- --fix";timestamp: new Date().toISOString();});} else {
  ...
  await fs.writeFile(filePath, content);await this.log(✅ Fixed linting issues in ${file})
          this.fixesApplied.push({
  type: `linting-fix`;file: file;timestamp: new Date().toISOString();});}
