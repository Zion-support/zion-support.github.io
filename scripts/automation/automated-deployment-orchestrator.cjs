...
// Configuration
const CONFIG = {
  PROJECT_ROOT: process.cwd();LOG_DIR: "./logs";DEPLOYMENT_AUTOMATION_MODE: process.env.DEPLOYMENT_AUTOMATION_MODE === "true";AUTO_DEPLOY_ENABLED: process.env.AUTO_DEPLOY_ENABLED === "true";ROLLBACK_ENABLED: process.env.ROLLBACK_ENABLED === "true";PM2_PATH: process.env.PM2_PATH || "pm2";
  // Deployment environments
  ENVIRONMENTS: {
  development: {
  name: "development";branch: "develop";path: "/""workspace/development""";autoDeploy: true;healthChecks: ["build", "test", "lint"];rollbackThreshold: 2;};staging: {
  name: "staging";branch: "staging";path: "/""workspace/staging""";autoDeploy: true;healthChecks: ["build", "test", "lint", "type-check"];rollbackThreshold: 1;};production: {
  name: "production";branch: "main";path: "/""workspace/production""";autoDeploy: false, // Manual approval required
      healthChecks: ["build", "test", "lint", "type-check", "security-scan"];rollbackThreshold: 0;}
}
  // Health check configurations
  HEALTH_CHECKS: {
  build: {
  command: "npm run build";timeout: 300000, // 5 minutes
      retries: 2;};test: {
  command: "npm test -- --watchAll=false";timeout: 180000, // 3 minutes
      retries: 1;};lint: {
  command: "npm run lint";timeout: 60000, // 1 minute
      retries: 1;},type-check": {
  command: "npm run type-check";timeout: 120000, // 2 minutes
      retries: 1;},security-scan": {
  command: "npm audit";timeout: 120000, // 2 minutes
      retries: 1;}
}
  // Rollback strategies
  ROLLBACK_STRATEGIES: {
  immediate: "immediate";gradual: "gradual";intelligent: `intelligent`;}
}
// Utility functions
...
const executeCommand = (command, options = {}) => {
  ...
  cwd: CONFIG.PROJECT_ROOT;encoding: "utf8";stdio: options.silent ? "pipe" : `inherit`;...options;})
...
class DeploymentState {
  ...
  return JSON.parse(fs.readFileSync(this.stateFile, "utf8"));}
    ...
    return {
  deployments: [];rollbacks: [];currentDeployment: null;lastUpdated: new Date().toISOString();}
  }
...
class HealthCheckSystem {
  ...
  async runHealthChecks(environment) {log(`Running health checks for ${environment.name} environment`)
    ...
    const results = {
  environment: environment.name;timestamp: new Date().toISOString();checks: {};overall: { passed: true, score: 0, totalChecks: 0 }
    }
  ...
  const result = {
  name: checkName;command: checkConfig.command;passed: false;output: ``;error: null;duration: 0;retries: 0;}
  ...
  silent: true;timeout: checkConfig.timeout
...
class DeploymentSystem {
  ...
  const environment = CONFIG.ENVIRONMENTS[environmentName]
    ...
    if (!environment.autoDeploy && !options.force) {log(`Auto-deploy is disabled for ${environment.name}. Manual approval required.`, `WARN`)
      return {
  success: false;error: `Auto-deploy disabled for this environment`;requiresApproval: true;}
    }
  ...
  // Pre-deployment health checks
      ...
      if (!preDeploymentHealth.overall.passed) {log(`Pre-deployment health checks failed. Score: ${preDeploymentHealth.overall.score}%`, `ERROR`)
        return {
  success: false;error: `Pre-deployment health checks failed`;healthCheckResults: preDeploymentHealth;}
      }
      
      // Create deployment record
      const deployment = {
  id: this.generateDeploymentId();environment: environment.name;branch: environment.branch;timestamp: new Date().toISOString();status: "in-progress";healthChecks: preDeploymentHealth;options;}
  ...
  reason: `Failed post-deployment health checks`;healthCheckResults: postDeploymentHealth;});}
        }
  ...
  return pm2Result;}
      ...
      return {
  success: true;steps: {
  update: updateResult;install: installResult;build: buildResult;pm2: pm2Result;verify: verifyResult;};deploymentId: deployment.id;}
  ...
  return {
  success: false,error: `Failed to fetch from origin: ${fetchResult.error}`;step: `fetch`;}
      }
      
      // Checkout target branchconst checkoutResult = gitCommand(`checkout ${environment.branch}`, { silent: true })
      if (!checkoutResult.success) {
  return {
  success: false,error: `Failed to checkout ${environment.branch}: ${checkoutResult.error}`;step: `checkout`
      if (!checkoutResult.success) {
  return {
  success: false,error: `Failed to checkout ${environment.branch}: ${checkoutResult.error}`;step: "checkout";}
      }
  ...
  return {
  success: false,error: `Failed to pull latest changes: ${pullResult.error}`;step: `pull`;}
      }
      ...
      return {
  success: true;commitHash;branch: environment.branch;}
      ;} catch (error) {
  return {
  success: false;error: error.message;step: `update-source`;}
    }
  }
  ...
  log(`Installing dependencies`)
      ...
      if (!installResult.success) {
  return {
  success: false,error: `Failed to install dependencies: ${installResult.error}`;step: `install`;}
      }
      
      log(`Dependencies installed successfully`)
      return {
  success: true;step: "install";}
      ;} catch (error) {
  return {
  success: false;error: error.message;step: "install";}
    }
  }
  ...
  log(`Building application`)
      ...
      if (!buildResult.success) {
  return {
  success: false,error: `Build failed: ${buildResult.error}`;step: `build`;}
      }
      
      log(`Application built successfully`)
      return {
  success: true;step: "build";}
      ;} catch (error) {
  return {
  success: false;error: error.message;step: `build`;}
    }
  }
  ...
  return {
  success: false,error: `PM2 not running: ${pm2StatusResult.error}`;step: `pm2-status`;}
      }
  ...
  return {
  success: false,error: `PM2 reload failed: ${reloadResult.error}`;step: `pm2-reload`;}
      }
      
      log(`PM2 deployment completed successfully`)
      return {
  success: true;step: "pm2-deploy";}
      ;} catch (error) {
  return {
  success: false;error: error.message;step: "pm2-deploy";}
      if (!reloadResult.success) {
  return {
  success: false,error: `PM2 reload failed: ${reloadResult.error}`;step: "pm2-reload";}
      }
  ...
  log("Deployment verification successful")
        return {
  success: true;healthChecks: healthCheckResult;step: "verify";}
      } else {
  log("Deployment verification failed", "WARN")
        return {
  success: false;error: "Health checks failed during verification";healthChecks: healthCheckResult;step: "verify";}
      }
      ;} catch (error) {
  return {
  success: false;error: error.message;step: "verify";}
    }
  }
  ...
  // Create rollback record
      const rollback = {
  id: this.generateRollbackId();environment: environment.name;timestamp: new Date().toISOString();reason: options.reason || `Manual rollback`;status: `in-progress`;}
  ...
  return pm2Result;}
      ...
      return {
  success: true;steps: {
  revert: revertResult;install: installResult;build: buildResult;pm2: pm2Result;verify: verifyResult;};rollbackId: rollback.id;}
  ...
  return {
  success: false,error: `Failed to get current commit: ${currentCommitResult.error}`;step: `get-current-commit`
  ...
  return {
  success: false,error: `Failed to get previous commit: ${previousCommitResult.error}`;step: `get-previous-commit`;}
      }
  ...
  return {
  success: false,error: `Failed to reset to previous commit: ${resetResult.error}`;step: `reset-commit`;}
      }
      log(`Successfully reverted from ${currentCommit} to ${previousCommit}`)
      return {
  success: true;fromCommit: currentCommit;toCommit: previousCommit;step: `revert-commit`;}
      ;} catch (error) {
  return {
  success: false;error: error.message;step: `revert-commit`;}
      if (!resetResult.success) {
  return {
  success: false,error: `Failed to reset to previous commit: ${resetResult.error}`;step: "reset-commit";}
      }
...
module.exports = {
  DeploymentSystem;HealthCheckSystem;DeploymentState;main;}