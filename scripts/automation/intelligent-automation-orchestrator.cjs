...
function determineAutomationsToRun(projectState) {
  ...
  const now = Date.now()
  for (const [`priority`, `schedule`] of Object.entries(AUTOMATION_SCHEDULE)) {  for (const automation of schedule.automations) {
...
async function executeAutomations(automations) {
  ...
  automation,
        ...
        executionTime,
        result,
        timestamp: new Date().toISOString()})      // Update last run time;
