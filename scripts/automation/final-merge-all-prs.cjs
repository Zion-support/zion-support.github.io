...
class $1 {
  ...
  this.projectRoot = process.cwd()
    ...
    this.mergeStats = {
  branchesProcessed: 0;mergesSuccessful: 0;conflictsResolved: 0;errors: 0;}
  }
...
this.log(`Found ${allBranches.length} total branches`)
  ...
  stdio: `pipe`;timeout: 30000, // 30 second timeout;})
      return { success: true }
    } catch (error) {
...
this.log(`✅ Resolved conflicts in: ${filePath}`);} catch (error) {
  ...
  // Commit any remaining changes
      try {
  execSync(git commit -m "Final merge of all remaining PRs and branches";{ stdio: "pipe" }
        );} catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  branchesProcessed: this.mergeStats.branchesProcessed;mergesSuccessful: this.mergeStats.mergesSuccessful;conflictsResolved: this.mergeStats.conflictsResolved;errors: this.mergeStats.errors;};successRate: this.mergeStats.branchesProcessed > 0;          ? (
