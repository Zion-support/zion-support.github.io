...
`;
  ...
  async blueGreenDeployment(deployment, envConfig) {
    ...
    try {
      ...
      const currentActive = await this.getCurrentActiveEnvironment(
      ...
      if (healthCheckResult.success) {
        ...
        await this.decommissionEnvironment(
          deployment.environment,
          currentActive
        );
      }
  ...
  async canaryDeployment(deployment, envConfig) {
    ...
    try {
      ...
      const canaryHealth = await this.monitorCanaryHealth(
        deployment,
        envConfig
      );

      if (
        canaryHealth.success &&
        canaryHealth.metrics.score > envConfig.rollbackThreshold
      ) {
  ...
  async rollingDeployment(deployment, envConfig) {
    ...
    try {
      ...
      for (let i = 0; i < targetInstances; i++) {
        ...
        const instanceHealth = await this.checkInstanceHealth(
          deployment.environment,
          i
        );
        ...
        if (instanceHealth.success) {
          ...
          await this.removeInstanceFromLoadBalancer(
            deployment.environment,
            i - 1
          );
  ...
  async rollbackDeployment(deployment, envConfig) {
    ...
    try {
      ...
      const lastSuccessful = this.deploymentHistoryData
        .filter(
          d =>
            d.environment === deployment.environment && d.status === 'SUCCESS'
        )
  ...
  async considerAutomaticRollback(environment, envConfig) {
    ...
    const consecutiveFailures =
      await this.getConsecutiveHealthFailures(environment);
