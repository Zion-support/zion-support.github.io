...
`)
  ...
  const recentTrends = this.maintenanceMetrics.performanceTrends.slice(-3)      const avgBuildTime =;
...
`)
  async calculateSystemHealth() {`)
    console.log(`📊 Calculating system health score...`)    let healthScore = 100;
...
=======
  ...
  const monitor = new PredictiveMaintenanceMonitor()  // Run initial monitoring;
