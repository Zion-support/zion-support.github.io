...
;
      if (buildOutput.includes('error')) {;
        ...
        if (errorMatches) {
<<<<<<< HEAD          buildHealth.errors = errorMatches.slice(0, 5); // Limit to 5 errors;
        }
      }
;
...
<<<<<<< HEAD      const recentTrends = this.maintenanceMetrics.performanceTrends.slice(-3);
      const avgBuildTime =;
