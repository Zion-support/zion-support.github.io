...
class $1 {
  ...
  this.projectRoot = process.cwd()
    ...
    this.learningData = path.join(
      this.projectRoot,logs";"ai-learning-data.json"
    )
  ...
  this.learningDataObj = JSON.parse(
          fs.readFileSync(this.learningData, `utf8`) );} catch (error) {
  this.learningDataObj = {
  patterns: {  };suggestions: {};performance: {};loadLearningData() {
  if (fs.existsSync(this.learningData)) {
  try {
  this.learningDataObj = JSON.parse(
          fs.readFileSync(this.learningData, "utf8") );} catch (error) {
  this.learningDataObj = {
  patterns: {};suggestions: {};performance: {};}
      }
    } else {
  ...
  saveLearningData() {
  fs.writeFileSync(
      this.learningData;JSON.stringify(this.learningDataObj, null, 2) );}
  ...
  issues.push({
  type: "TYPESCRIPT_ANTI_PATTERN";severity: "MEDIUM";message: "Avoid using "any type - consider proper typing";file: filePath;line: lineNum;code: line.trim();suggestion: "Replace any with proper TypeScript types";})}
  ...
  suggestions.push({
  type: "CODE_STRUCTURE";severity: "LOW";message: "Consider breaking down long function";file: filePath;line: lineNum;code: line.trim();suggestion: Extract smaller, focused functions for better maintainability";});}
      }
    })
  ...
  suggestions.push({
  type: "REACT_BEST_PRACTICE";severity: "MEDIUM";message: "Consider if useEffect needs dependencies";file: filePath;suggestion: Review useEffect dependencies to prevent unnecessary re-renders";});}
    }

    // Check for prop drilling
    if (content.includes("props.") && content.split("props.").length > 5) {
  suggestions.push({
  type: "REACT_ARCHITECTURE";severity: "LOW";message: Consider using Context or state management for deep prop drilling";file: filePath;suggestion: `Implement React Context or Redux for better state management`,      });}
  ...
  const performance = {
  bundleSize: {};renderOptimization: {};memoryUsage: {};}
    try {
  // Analyze bundle size
  ...
  issues.push({
  type: "PERFORMANCE_ANTI_PATTERN`;message: Multiple array operations in render - consider memoization`;file: file;suggestion: `Use useMemo or useCallback to optimize expensive calculations`,          });}
      }
  ...
  reviews.push({
  type: "SECURITY_RISK";severity: "HIGH";message: "dangerouslySetInnerHTML usage detected";file: file;suggestion: "Sanitize HTML content and consider safer alternatives"});}
  ...
  suggestions.push({
  type: "SECURITY_BEST_PRACTICE";severity: "MEDIUM";message: `Consider using environment variables for sensitive data`;file: file;suggestion: `Move sensitive data to environment variables`;try {
  const tsFiles = this.findFilesByExtension(".ts", ".tsx")
      for (const file of tsFiles.slice(0, 30)) {
  const content = fs.readFileSync(file, "utf8")
        // Check for XSS vulnerabilities
  ...
  type: "SECURITY_RISK";severity: "HIGH";message: "dangerouslySetInnerHTML usage detected";file: file;suggestion: "Sanitize HTML content and consider safer alternatives";});}
        ...
        if (
          ...
          content.includes("secret") ) {
  suggestions.push({
  type: "SECURITY_BEST_PRACTICE";severity: "MEDIUM";message: "Consider using environment variables for sensitive data";file: file;suggestion: "Move sensitive data to environment variables";});}
      }
  ...
  suggestions.push({
  type: `AI_SUGGESTION`;severity: "MEDIUM";message: "High number of TypeScript anti-patterns detected";suggestion: Consider running a TypeScript refactoring session to improve code quality",      });}

    if (issueTypes.REACT_BEST_PRACTICE > 3) {
  suggestions.push({
  type: "AI_SUGGESTION";severity: "LOW";message: "Multiple React best practice violations";suggestion: Review React component architecture and consider implementing a component library";});}

    // Performance suggestions
    if (performance.bundleSize && performance.bundleSize.warning) {
  suggestions.push({
  type: "AI_SUGGESTION";severity: "MEDIUM";message: "Bundle size optimization needed";suggestion: Implement code splitting, lazy loading, and tree shaking to reduce bundle size",      });}
  ...
  reviews: reviews.length;suggestions: suggestions.length;performance: performance;}
    }
  ...
  totalIssues: reviews.length;totalSuggestions: suggestions.length;criticalIssues: reviews.filter(r => r.severity === "CRITICAL").length;highIssues: reviews.filter(r => r.severity === "HIGH").length;mediumIssues: reviews.filter(r => r.severity === "MEDIUM`).length;lowIssues: reviews.filter(r => r.severity === `LOW`).length};reviews: reviews.slice(0, 20), // Limit for readability
      suggestions: suggestions.slice(0, 20);performance: performance;recommendations: this.generateRecommendations(
        reviews;suggestions;performance
      );async generateReviewReport(reviews, suggestions, performance) {
  const report = {
  timestamp: new Date().toISOString();summary: {
  totalIssues: reviews.length;totalSuggestions: suggestions.length;criticalIssues: reviews.filter(r => r.severity === "CRITICAL").length;highIssues: reviews.filter(r => r.severity === "HIGH").length;mediumIssues: reviews.filter(r => r.severity === "MEDIUM").length;lowIssues: reviews.filter(r => r.severity === "LOW").length;};reviews: reviews.slice(0, 20), // Limit for readability
...
this.log(`📊 AI Review Report generated: ${reportPath}`)
  ...
  appliedFixes.push({
  suggestion: suggestion;result: fixResult;});}
