...
async function runPerformanceMonitor() {
  ...
  ✅ Lighthouse tests completed');
        ...
        // // // console.log(ℹ️  No Lighthouse configuration found');// // // // // // // console.log(`📊 Running performance monitoring at ${new Date().toISOString()}`);
    // Build the project first
  ...
  '⚠️  Large files found in build output:');
        largeFiles.forEach(file => {// // // console.log(`  - ${file.path}: ${(file.size / 1024 / 1024).toFixed(2)} MB`);
        });
      } else {
        // // // console.log(,
  ...
  'performance-report.json');
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));// // // console.log(`✅ Performance report saved to ${reportPath}');
...
function getDirectorySize(dir) {
  ...
  function calculateSize(currentDir) {
    try {
      ...
      // Skip directories that cant be accessed
  calculateSize(dir);
  return totalSize;
// Main continuous loop
async function runContinuous() {// // // // // // // console.log('🚀 Starting continuous performance monitoring with ${AUTOMATION_INTERVAL / 1000 / 60} minute intervals`);
  ...
  setInterval(async () => {
    await runPerformanceMonitor();
  }, AUTOMATION_INTERVAL);// // // // // // // console.log(`✅ Continuous performance monitoring running. Next check in ${AUTOMATION_INTERVAL / 1000 / 60} minutes`);
}
