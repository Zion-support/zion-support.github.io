...
class MergeConflictResolver {
  constructor() {
    ...
    this.conflictsFound = [];
    this.logFile = path.join(this.projectRoot, 'merge-conflict-resolver-report.json');
  ...
  async resolveConflict(conflict) {
    try {
      ...
      resolvedContent = await this.autoResolveConflict(conflict.file, resolvedContent);
  ...
  async commitResolvedConflicts() {
    try {
      ...
      const resolvedFiles = status.filter(line => line.startsWith('M ') || line.startsWith('A '));
