...
class AICodeAnalyzer {
  ...
  calculateScores() {
    ...
    const complexFiles = this.analysisResults.codeSmells.filter(smell => 
      smell.type === 'high_complexity'
    ).length;
  ...
  async generateReport() {
    ...
    const latestReportPath = path.join(process.cwd(), 'ai-analysis-report.json');
  ...
  analyzeImports(content) {
    ...
    const unusedImports = imports.filter(imp => {
      const importName = imp.split('/').pop().replace(/\.(js|ts|tsx|jsx)$/, '');
      return !content.includes(importName) || content.indexOf(importName) === content.indexOf(`import.*${importName}`);
    });
