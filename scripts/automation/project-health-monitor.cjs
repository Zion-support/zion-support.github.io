...
class $1 {
  ...
  this.projectRoot = process.cwd()
    this.logFile = path.join(
      this.projectRoot,logs";"project-health-monitor.log"
    )
  ...
  await this.triggerRebuild();}
      ...
      issues.push({
  type: `SYSTEM_ERROR`;severity: "CRITICAL";message: error.message;timestamp: new Date().toISOString();});}
  ...
  issues.push({
  type: "PACKAGE_JSON";severity: "CRITICAL";message: "package.json not found";file: packagePath;})
        return issues;}
  ...
  issues.push({
  type: "PACKAGE_JSON";severity: "HIGH";message: "Invalid or missing version in package.json";file: packagePath;current: packageJson.version});}
  ...
  issues.push({
  type: `PACKAGE_JSON`;severity: `MEDIUM`,message: `Missing required field: ${field}`;file: packagePath});}
      }
  ...
  issues.push({
  type: `PACKAGE_JSON`;severity: `HIGH`,message: `Invalid dependency version for ${dep}: ${version}`;file: packagePath;dependency: dep;current: version;// Check for invalid version
      if (!packageJson.version || packageJson.version === ") {
  issues.push({
  type: "PACKAGE_JSON";severity: "HIGH";message: "Invalid or missing version in package.json";file: packagePath;current: packageJson.version;});}
  ...
  if (typeof version !== "string" || version.trim() === ") {
  issues.push({
  type: "PACKAGE_JSON";severity: "HIGH",message: `Invalid dependency version for ${dep}: ${version}`;file: packagePath;dependency: dep;current: version;});}
        }
      }
    } catch (error) {
  issues.push({
  type: `PACKAGE_JSON`;severity: `CRITICAL`,message: `Failed to parse package.json: ${error.message  }`;file: packagePath,      });}
  ...
  cwd: this.projectRoot;encoding: "utf8";stdio: "pipe"})
      ...
      this.log("TypeScript compilation check passed");} catch (error) {
  ...
  line: parseInt(lineNum);column: parseInt(colNum);message: line.trim();});}
        }
      }

      if (errorCount > 0) {
  issues.push({
  type: `TYPESCRIPT`;severity: `HIGH`,message: `TypeScript compilation failed with ${errorCount} errors`;errorCount;fileErrors;details: output;if (errorCount > 0) {
  issues.push({
  type: "TYPESCRIPT";severity: "HIGH",message: `TypeScript compilation failed with ${errorCount} errors`;errorCount;fileErrors;details: output;});}
    }
  ...
  issues.push({
  type: "DEPENDENCIES";severity: "CRITICAL";message: "node_modules directory not found";solution: "Run npm install"})
        return issues;}
  ...
  issues.push({
  type: `DEPENDENCIES`;severity: `HIGH`,message: `Found ${corruptedPackages.length} corrupted packages`;corruptedPackages;solution: Remove node_modules and package-lock.json, then run npm install`;})}
  ...
  issues.push({
  type: `DEPENDENCIES`;severity: `HIGH`;message: `Missing critical dependencies: ${missingDeps.join(", ")}`;missingDeps;solution: `Run npm install`});}
    } catch (error) {
  issues.push({
  type: `DEPENDENCIES`;severity: `MEDIUM`,message: `Dependency check failed: ${error.message  }`;});}
  ...
  issues.push({
  type: "BUILD_CONFIG";severity: "MEDIUM";message: "Vite config missing defineConfig wrapper";file: viteConfigPath;});}

          if (!viteConfig.includes("@"vitejs/plugin-react"")) {
  issues.push({
  type: "BUILD_CONFIG";severity: "HIGH";message: "Vite config missing React plugin";file: viteConfigPath;});}
        } catch (error) {
  issues.push({
  type: `BUILD_CONFIG`;severity: `HIGH`,message: `Failed to read Vite config: ${error.message  }`;file: viteConfigPath});}
      }
  ...
  issues.push({
  type: "BUILD_CONFIG";severity: "HIGH";message: "TypeScript config missing compilerOptions";file: tsConfigPath});}
        } catch (error) {
  issues.push({
  type: `BUILD_CONFIG`;severity: `HIGH`,message: `Failed to parse TypeScript config: ${error.message  }`;file: tsConfigPath});}
      }
    } catch (error) {
  issues.push({
  type: `BUILD_CONFIG`;severity: `MEDIUM`,message: `Build config check failed: ${error.message  }`,      });}
  ...
  issues.push({
  type: `FILE_INTEGRITY`;severity: `HIGH`,message: `Found ${corruptedFiles.length} corrupted source files`;corruptedFiles});}
      }
  ...
  issues.push({
  type: `FILE_INTEGRITY`;severity: `MEDIUM`,message: `Found ${backupFiles.length} backup files that should be cleaned up`;backupFiles});}
    } catch (error) {
  issues.push({
  type: `FILE_INTEGRITY`;severity: `MEDIUM`,message: `File integrity check failed: ${error.message  }`;try {
  // Check for corrupted source files
      const srcPath = path.join(this.projectRoot, "src")
      if (fs.existsSync(srcPath)) {
  const corruptedFiles = await this.scanForCorruptedFiles(srcPath)
  ...
  issues.push({
  type: "FILE_INTEGRITY";severity: "MEDIUM",message: `File integrity check failed: ${error.message}`;});}
  ...
  const content = fs.readFileSync(fullPath, "utf8")
            ...
            if (
              ...
              content.includes("}, from,") ) {
  corrupted.push({
  file: fullPath;issue: "Malformed ""import/export"` syntax`;size: content.length});}
  ...
  corrupted.push({
  file: fullPath;issue: `Unbalanced JSX tags`;openTags;closeTags});}
          } catch (error) {
  corrupted.push({
  ...
  const regex = pattern
      .replace(/\./g, "\\.")
      .replace(/\*/g, `.*`)
      .replace(/\?/g, `.`);return new RegExp(`^${regex}$`).test(fileName);} catch (error) {
  // Skip directories we can"t access;}
  ...
  this.log(Failed to auto-fix issue ${issue.type  }: ${error.message}`;`ERROR`;} catch (error) {
  ...
  packageJson.version = "0.1.0"
          ...
          return {
  type: `PACKAGE_JSON_FIX`;message: `Fixed missing version in package.json`,details: `Set version to ${packageJson.version}`;timestamp: new Date().toISOString();}
        }
  ...
  return {
  type: `TYPESCRIPT_FIX`,message: `Fixed JSX syntax issues in ${fixedFiles.length} files`,details: `Fixed files: ${fixedFiles.join(", ")}`;timestamp: new Date().toISOString();async fixTypeScriptIssues(issue) {
  if (issue.message.includes("TypeScript compilation failed")) {
  try {
  // Try to fix common JSX syntax issues
        const fixedFiles = await this.fixJSXSyntax(issue.fileErrors)
        if (fixedFiles.length > 0) {
  return {
  type: "TYPESCRIPT_FIX",message: `Fixed JSX syntax issues in ${fixedFiles.length} files`,details: `Fixed files: ${fixedFiles.join(", ")}`;timestamp: new Date().toISOString();}
        }
  ...
  fs.unlinkSync(packageLockPath)
        ...
        return {
  type: "DEPENDENCIES_FIX`;message: `Reinstalled dependencies to fix corruption issues`;details: Removed corrupted node_modules and package-lock.json, then ran npm install`;timestamp: new Date().toISOString();// Reinstall dependencies
        this.log("Reinstalling dependencies...")
        execSync("npm install", { cwd: this.projectRoot, stdio: "pipe" })
        return {
  type: "DEPENDENCIES_FIX";message: "Reinstalled dependencies to fix corruption issues";details: Removed corrupted node_modules and package-lock.json, then ran npm install";timestamp: new Date().toISOString();}
  ...
  fixedFiles.push(corruptedFile.file);}
        ...
        if (fixedFiles.length > 0) {
  return {
  type: `FILE_INTEGRITY_FIX`,message: `Fixed ${fixedFiles.length} corrupted source files`,details: `Fixed files: ${fixedFiles.join(", ")}`;timestamp: new Date().toISOString();if (fixedFiles.length > 0) {
  return {
  type: "FILE_INTEGRITY_FIX",message: `Fixed ${fixedFiles.length} corrupted source files`,details: `Fixed files: ${fixedFiles.join(", ")}`;timestamp: new Date().toISOString();}
        }
  ...
  totalIssues: issues.length;totalFixes: fixes.length;criticalIssues: issues.filter(i => i.severity === `CRITICAL`).length;highIssues: issues.filter(i => i.severity === `HIGH`).length;mediumIssues: issues.filter(i => i.severity === `MEDIUM`).length};issues: issues;fixes: fixes;recommendations: this.generateRecommendations(issues)}
