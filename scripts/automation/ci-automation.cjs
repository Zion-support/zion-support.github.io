...
class $1 {
  ...
  this.projectRoot = path.resolve(__dirname, "../../")
    this.logFile = path.join(
      this.projectRoot,ci-cd-reports";"ci-automation.log"
    )
    this.reportFile = path.join(
      this.projectRoot,ci-cd-reports";"ci-report.json"
    )
...
`)
  ...
  total: results.length;passed: results.filter(r => r.success).length;failed: results.filter(r => !r.success).length;},    }
  ...
  this.log(Skipping remaining steps due to dependency installation failure";"WARN"
    ...
    results.push({
  step: "lint";success: lintResult;timestamp: new Date().toISOString();})
    // Run type check
    const typeCheckResult = await this.runTypeCheck()
    results.push({
  step: "type-check";success: typeCheckResult;timestamp: new Date().toISOString();})
    // Run build
  ...
  step: `verify-build`;success: verifyResult;timestamp: new Date().toISOString()})
      ...
      results.push({
  step: `tests`;success: testResult;timestamp: new Date().toISOString()});}
