...
class $1 {
  ...
  const hasErrors = error !== null
        ...
        resolve({
  hasErrors;errorCount;warningCount;output: stdout + stderr;});});});}
  ...
  const hasErrors = error !== null
        ...
        resolve({
  hasErrors;errorCount;const errorCount = hasErrors ? (stderr.match(/error "TS/g") || []).length : 0
        
        resolve({
  hasErrors;errorCount;output: stdout + stderr;});});});}
  ...
  const hasErrors = error !== null
        resolve({
  hasErrors;buildSuccess: !hasErrors;output: stdout + stderr;});});});}
  ...
  corruptedFiles.push({
  path: itemPath;relativePath: path.relative(this.projectRoot, itemPath);size: stat.size;issues: this.analyzeFileIssues(content);});}
            } catch (error) {
  // Check for common corruption patterns
              if (
                content.includes("Unterminated string literal") ||
  ...
  corruptedFiles.push({
  path: itemPath;relativePath: path.relative(this.projectRoot, itemPath);error: error.message;issues: ["read_error"];});}
          }
        }
      } catch (error) {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  const timestamp = new Date().toISOString()
    ...
    const report = {
  timestamp;reportId;summary: {
  totalErrors: lintResults.errorCount + typeResults.errorCount;lintErrors: lintResults.errorCount;lintWarnings: lintResults.warningCount;typeErrors: typeResults.errorCount;buildSuccess: buildResults.buildSuccess;corruptedFiles: corruptedFiles.length;};details: {
  lint: lintResults;typeCheck: typeResults;build: buildResults;corruptedFiles;};recommendations: this.generateRecommendations(lintResults, typeResults, buildResults, corruptedFiles);}
  ...
  priority: `high`;action: `fix_corrupted_files`,description: `Fix ${corruptedFiles.length} corrupted files with syntax errors`;files: corruptedFiles.map(f => f.relativePath);});}
    
    if (lintResults.errorCount > 100) {
  recommendations.push({
  priority: `high`;action: `run_lint_fix`,description: `Run `npm run lint --fix` to automatically fix ${lintResults.errorCount} lint errors`;});}
    
    if (typeResults.errorCount > 50) {
  recommendations.push({
  priority: `medium`;action: `fix_type_errors`,description: `Fix ${typeResults.errorCount} TypeScript errors`;});}
    
    if (!buildResults.buildSuccess) {
  recommendations.push({
  priority: `critical`;action: `fix_build_errors";description: "Project build is failing - fix build errors immediately"
