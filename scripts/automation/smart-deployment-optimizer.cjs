  ...
  console.log("📈 Analyzing deployment patterns...")
    ...
    const recentDeployments = this.deploymentMetrics.buildTimes.filter(;
      build =>;
        ...
        new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // Last 7 days;    this.deploymentMetrics.deploymentFrequency = recentDeployments.length;
...
async function $1() {
  const optimizer = new SmartDeploymentOptimizer()  // Run initial optimization;
