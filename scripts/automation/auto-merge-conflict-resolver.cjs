...
this.log(📊 Found ${conflictedFiles.length} conflicted files`)
  ...
  const content = fs.readFileSync(filePath, "utf8")
      ...
      resolvedContent = this.resolveConfigConflicts(resolvedContent, filePath)      // Remove any remaining conflict markers;
  ...
  // Handle component conflicts by keeping the most recent version;
    ...
    return content.replace(componentPattern, (match, headContent, incomingContent) => {  resolveComponentConflicts(content, filePath) {
  // Handle component conflicts by keeping the most recent version;
    const componentPattern = /\s*\n(.*?)\n    ;
  ...
  const headJson = JSON.parse(headMatch[1])
        const incomingJson = JSON.parse(incomingMatch[1])        // Merge dependencies;
...
=======
  ...
  branch: branchName,
          ...
          return true;
} else {this.log(`❌ Failed to commit resolved conflicts in ${branchName}`, `ERROR`)
          return false;
}
      }
