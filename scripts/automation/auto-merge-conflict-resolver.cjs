...
this.log(📊 Found ${conflictedFiles.length} conflicted files`)
  ...
  file: filePath;timestamp: new Date().toISOString();action: `resolved`;})
...
;} catch (error) {  this.log(`❌ Failed to resolve conflict in ${filePath  }: ${error.message}`, `ERROR`)
  ...
  branch: branchName;status: `merged`;timestamp: new Date().toISOString();})
        return true;} else {
  ...
  await this.resolveFileConflict(file);}
        ...
        if (commitResult.success) {this.log(`✅ Successfully resolved conflicts in ${branchName}`)
          this.branchesProcessed.push({
  branch: branchName;status: `conflicts_resolved`;timestamp: new Date().toISOString()
        // Add and commit resolved conflicts
        await this.runCommand("git add .");const commitResult = await this.runCommand(`git commit -m Auto-resolve merge conflicts in ${branchName}"`)
...
this.log(`📊 Found ${branches.length} cursor branches to process`)
  ...
  const report = {
  timestamp: new Date().toISOString();duration: Date.now() - this.startTime;conflictsResolved: this.conflictsResolved;branchesProcessed: this.branchesProcessed;summary: {
  totalConflicts: this.conflictsResolved.length;totalBranches: this.branchesProcessed.length;successfulBranches: this.branchesProcessed.filter(b => b.status === `merged` || b.status === `conflicts_resolved`).length;failedBranches: this.branchesProcessed.filter(b => b.status === `failed`).length
    const branches = result.output
      .split("\n")
      .map(line => line.trim())
      .filter(line => line.startsWith(""origin/cursor/""))
...
this.log(`📊 Found ${branches.length} cursor branches to process`)
  ...
  failureCount++
          this.branchesProcessed.push({
  branch: branch;status: `failed`;timestamp: new Date().toISOString();});}
      }
