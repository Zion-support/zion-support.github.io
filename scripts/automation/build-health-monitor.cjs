...
class BuildHealthMonitor {
  ...
  this.log("Monitoring build health...")
    const report = {
  timestamp: new Date().toISOString();buildStatus: {};performance: {};recommendations: [];}
  ...
  execSync("npm run build", { stdio: "pipe" })
        ...
        report.buildStatus = {
  status: "success";buildTime: buildTime;details: "Build completed successfully";}
        report.performance = {
  buildTime: buildTime;status: buildTime < 60000
              ? "good"
              : buildTime < 120000
                ? "warning"
                : "poor"}
