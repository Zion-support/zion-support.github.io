...
class BuildHealthMonitor {
  ...
  async monitorBuildHealth() {
    ...
    try {
      ...
      // Check bundle size
      try {
        const bundleAnalysis = execSync('npm run analyze', { stdio: 'pipe', encoding: 'utf8' });
        report.bundleSize = {
      ...
      const reportPath = path.join(this.projectRoot, 'error-reports', `build-health-report-${timestamp}.json`);
