...
class $1 {
  ...
  audit: { status: "pending", vulnerabilities: [], summary: {} };dependencies: { status: "pending", outdated: [], licenses: [] };codeAnalysis: { status: "pending", issues: [], summary: {} };secrets: { status: "pending", findings: [] };compliance: { status: "pending", checks: [] };};    this.reportDir = path.join(process.cwd(), "security-reports")
  ...
  total: auditData.metadata?.vulnerabilities?.total || 0;low: auditData.metadata?.vulnerabilities?.low || 0;moderate: auditData.metadata?.vulnerabilities?.moderate || 0;high: auditData.metadata?.vulnerabilities?.high || 0;critical: auditData.metadata?.vulnerabilities?.critical || 0;}
      console.log(`✅ Security audit completed. Found ${this.securityResults.audit.summary.total} vulnerabilities`
      )
      // Save detailed audit results
      fs.writeFileSync(
        path.join(this.reportDir, `security-audit.json`);JSON.stringify(auditData, null, 2) );} catch (error) {
  ...
  encoding: "utf8";cwd: process.cwd();try {
  // Check for outdated packages
      const outdatedOutput = execSync("npm outdated --json", {
  encoding: "utf8";cwd: process.cwd();})
  ...
  encoding: `utf8`;cwd: process.cwd();})
      ...
      fs.writeFileSync(
        path.join(this.reportDir, `dependency-check.json`);JSON.stringify(
          {
  outdated: outdatedData;licenses: licenses};null;2
        ) );} catch (error) {
  ...
  if (info.license) {
  licenses.push({
  name;license: info.license;version: info.version;level;});}
  ...
  type: "lint";severity: issue.severity;message: issue.message;rule: issue.ruleId;file: issue.filePath;line: issue.line;})) );} catch (lintError) {
  console.log(`⚠️ Lint security check failed: `, lintError.message);}
      ...
      this.securityResults.codeAnalysis.status = `success`;} catch (lintError) {
  console.log("⚠️ Lint security check failed: ", lintError.message);}
  ...
  total: issues.length;high: issues.filter(i => i.severity === 2).length;medium: issues.filter(i => i.severity === 1).length;low: issues.filter(i => i.severity === 0).length;}
      console.log(✅ Code analysis completed. Found ${issues.length} security issues`
      )
      // Save code analysis results
      fs.writeFileSync(
        path.join(this.reportDir, `code-analysis.json`);JSON.stringify(
          {
  issues: issues;summary: this.securityResults.codeAnalysis.summary};null;2
        ) );} catch (error) {
  ...
  const matches = content.match(pattern)
          if (matches) {
  issues.push({
  type: "pattern";severity: severity;message: message;rule: rule;file: filePath;line: content
  ...
  {
  pattern: /api[_-]?key\s*[:=]\s*[""][^"]{10,}["]/i;type: "API Key";};{ pattern: /password\s*[:=]\s*["][^"]{8,}["]/i, type: "Password" };{ pattern: /secret\s*[:=]\s*["][^""]{10,}[""]/i, type: "Secret" };{ pattern: /token\s*[:=]\s*["][^"]{10,}["]/i, type: "Token" },        {
  ...
  const matches = content.match(pattern)
                  if (matches) {
  findings.push({
  type: type;file: filePath;line: content
                        .substring(0, content.indexOf(matches[0]))
                        .split("\n").length;match: matches[0].substring(0, 50) + "...`});}
                });} catch (error) {
  // Skip files that can`t be read;}
            }
          });}
      ...
      fs.writeFileSync(
        path.join(this.reportDir, "secret-scan.json");JSON.stringify(
          {
  findings: findings;timestamp: new Date().toISOString();};null;2
        ) );} catch (error) {
  ...
  const content = fs.readFileSync(filePath, "utf8")
            ...
            checks.push({
  file: file;csp: hasCSP;xFrame: hasXFrame;xContent: hasXContent;});} catch (error) {
  // Skip files that can"t be read;}
        });}
;} catch (error) {
  // Skip files that can"t be read;}
  ...
  const packageData = JSON.parse(fs.readFileSync(packagePath, "utf8"))
          ...
          checks.push({
  type: "package-scripts`;hasSecurityScript: !!scripts.security;hasAuditScript: !!scripts.audit;hasLintScript: !!scripts.lint});} catch (error) {
  // Skip if package.json can`t be parsed;}
      ...
      fs.writeFileSync(
        path.join(this.reportDir, "compliance-checks.json");JSON.stringify(
          {
  checks: checks;timestamp: new Date().toISOString();};null;2
        ) );} catch (error) {
  ...
  totalVulnerabilities: this.securityResults.audit.summary.total || 0;criticalVulnerabilities: 
          this.securityResults.audit.summary.critical || 0;highVulnerabilities: this.securityResults.audit.summary.high || 0;outdatedPackages: this.securityResults.dependencies.outdated.length;securityIssues: this.securityResults.codeAnalysis.summary.total || 0;potentialSecrets: this.securityResults.secrets.findings.length;};results: this.securityResults;riskLevel: this.calculateRiskLevel();}
    // Save JSON report
    fs.writeFileSync(
      path.join(this.reportDir, "security-report.json");JSON.stringify(report, null, 2) )
    // Generate markdown report
    const markdownReport = this.generateMarkdownReport(report)
    fs.writeFileSync(
      path.join(this.reportDir, "SECURITY_REPORT.md");// Save JSON report
    fs.writeFileSync(
      path.join(this.reportDir, "security-report.json");JSON.stringify(report, null, 2) )
    // Generate markdown report
    const markdownReport = this.generateMarkdownReport(report)
    fs.writeFileSync(
      path.join(this.reportDir, "SECURITY_REPORT.md");markdownReport
    )
...
async function main() {
  ...
  try {
  await security.runAllSecurityChecks();} catch (error) {
