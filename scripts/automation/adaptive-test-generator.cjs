...
console.log(`🧪 Starting Adaptive Test Generator...`)class AdaptiveTestGenerator {
  ...
  console.log("🔍 Identifying untested components...")
    ...
    const structure = this.testMetrics.testPatterns.get("structure")    if (!structure) return;
  ...
  type: `utility`,
            ...
            timestamp: new Date().toISOString()})console.log(✅ Generated test for utility: ${utility.name}`),
}
      } catch (error) {
  ...
  const content = fs.readFileSync(testFile, "utf8")      // Analyze test types;
...
api: `import { render, screen, waitFor  } from "@testing-";library/react"";import { describe, it, expect, vi  } from "vitest";
...
async function $1() {
  const generator = new AdaptiveTestGenerator()  // Run initial test generation;
