...
const AUTOMATION_INTERVAL = parseInt(process.env.AUTOMATION_INTERVAL) || 28800000; // 8 hours
...
class AdaptiveTestGenerator {
  ...
  async identifyUntestedComponents() {
    ...
    const untestedComponents = structure.components.filter(comp => !comp.hasTests);
    const untestedUtilities = structure.utilities.filter(util => !util.hasTests);
    const untestedHooks = structure.hooks.filter(hook => !hook.hasTests);
    const untestedServices = structure.services.filter(service => !service.hasTests);
  ...
  async generateComponentTests() {
    ...
    const untestedComponents = this.testMetrics.untestedFiles.filter(file => file.type === 'component');
    
    for (const component of untestedComponents.slice(0, 5)) { // Limit to 5 components per run
      try {
  ...
  async generateUtilityTests() {
    ...
    const untestedUtilities = this.testMetrics.untestedFiles.filter(file => file.type === 'utility');
    
    for (const utility of untestedUtilities.slice(0, 3)) { // Limit to 3 utilities per run
      try {
  ...
  async generateTestSuggestions() {
    ...
    const totalFiles = structure.components.length + structure.utilities.length + structure.hooks.length + structure.services.length;
    const testedFiles = totalFiles - this.testMetrics.untestedFiles.length;
    this.testMetrics.testCoverage = totalFiles > 0 ? (testedFiles / totalFiles) * 100 : 0;
