...
class $1 {
  ...
  this.log(`Generating summary...`)
    ...
    const summary = {
  timestamp: new Date().toISOString();duration: Date.now() - this.startTime;totalReports: Object.keys(reports || {}).length;totalErrors: this.errorsFound.length;totalFixes: this.fixesApplied.length;errorCategories: this.categorizeErrors();fixCategories: this.categorizeFixes();reports: reports || {};summary: {
  overallSuccess: this.errorsFound.length === 0;criticalErrors: this.errorsFound.filter(e => e.includes("critical") || e.includes("security")).length;autoFixed: this.fixesApplied.filter(f => f.includes("Auto-fixed") || f.includes("Fixed")).length;manualReviewNeeded: this.errorsFound.filter(e => e.includes(`manual`) || e.includes(`review`)).length;}
    }
  ...
  categorizeErrors() {
  const categories = {
  typescript: 0;eslint: 0;build: 0;dependency: 0;console: 0;other: 0;}
  ...
  categorizeFixes() {
  const categories = {
  autoFixed: 0;manualReview: 0;configuration: 0;other: 0;}
