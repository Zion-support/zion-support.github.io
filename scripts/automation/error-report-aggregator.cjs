  ...
  categorizeErrors() {
    ...
    for (const error of this.errorsFound) {
      if (error.includes('TypeScript') || error.includes('type')) {
        ...
        categories.eslint++;
      } else if (error.includes('build') || error.includes('Build')) {
        categories.build++;
      } else if (error.includes('dependency') || error.includes('npm')) {
        categories.dependency++;
      } else if (error.includes('console') || error.includes('Console')) {
        categories.console++;
      } else {
        categories.other++;
      }
  ...
  categorizeFixes() {
    ...
    for (const fix of this.fixesApplied) {
      if (fix.includes('Auto-fixed') || fix.includes('Fixed')) {
        categories.autoFixed++;
      } else if (fix.includes('manual') || fix.includes('review')) {
        categories.manualReview++;
      } else if (fix.includes('config') || fix.includes('Config')) {
