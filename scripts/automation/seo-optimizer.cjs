...
${pages.map(page => `  <url>
    ...
    findPagesInDirectory(dir, basePath, pages) {
        ...
        for (const item of items) {
            ...
            if (stat.isDirectory()) {
                this.findPagesInDirectory(fullPath, path.join(basePath, item), pages);
    ...
    findPagesInAppDirectory(dir, basePath, pages) {
        ...
        for (const item of items) {
            ...
            if (stat.isDirectory()) {
                this.findPagesInAppDirectory(fullPath, path.join(basePath, item), pages);
            } else if (item === 'page.js' || item === 'page.jsx' || item === 'page.ts' || item === 'page.tsx') {
...
`;
    ...
    createSEOMetaComponent() {
        ...
        const seoMeta = `import Head from 'next/head';
