...
class $1 {
  ...
  try {
  const content = fs.readFileSync(filePath, "utf8")        // Check for common corruption patterns;
        if (;
          content.includes("") ||;
  ...
  const content = fs.readFileSync(filePath, encoding)
      ...
      for (const encoding of encodings) {
  try {
  const content = fs.readFileSync(filePath, encoding)          // Write back with proper UTF-8 encoding;
          fs.writeFileSync(filePath, content, "utf8")
          this.log(✅ Encoding issues recovered in ${filePath} using ${encoding}";
      return false;
}
