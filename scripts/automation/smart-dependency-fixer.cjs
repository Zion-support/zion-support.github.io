  ...
  try {
    ...
    try {
      ...
      if (auditData.vulnerabilities) {
        issues.vulnerabilities = Object.keys(auditData.vulnerabilities).map(pkg => ({
          name: pkg,
    ...
    try {
      ...
      if (lsData.problems) {
        issues.conflicts = lsData.problems.filter(problem => 
    ...
    try {
      const checkOutput = execSync('npm check --json', { stdio: 'pipe' }).toString();
    ...
    const reportPath = path.join(process.cwd(), 'smart-dependency-fixer-report.json');
