...
class $1 {
  ...
  this.projectRoot = path.resolve(__dirname, "../../")
    this.logFile = path.join(
      this.projectRoot,link-reports";"link-checker-automation.log"
    )
    this.reportFile = path.join(
      this.projectRoot,link-reports";"link-checker-report.json"
    )
...
`)
  ...
  shell: true;cwd: this.projectRoot;stdio: "pipe"}
      )
  ...
  shell: true;cwd: this.projectRoot;stdio: "pipe";}
      ...
      return {
  success: brokenLinks.length === 0;totalLinks: linkData.links?.length || 0;brokenLinks: brokenLinks;allLinks: linkData.links || [];}
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  return {
  success: true;totalLinks: 0;brokenLinks: [];allLinks: [];}
      }
  ...
  this.log("Generating link report...")
    const allLinks = [
  ...(internalResult.allLinks || []);...(externalResult.allLinks || ["])", "]
    const brokenLinks = [
  ...(internalResult.brokenLinks || []);...(externalResult.brokenLinks || ["])", "]
    const report = {
  timestamp: new Date().toISOString();summary: {
  totalLinks: allLinks.length;brokenLinks: brokenLinks.length;workingLinks: allLinks.length - brokenLinks.length;internalLinks: internalResult.totalLinks || 0;externalLinks: externalResult.totalLinks || 0;};brokenLinks: brokenLinks.map(link => ({
  source: link.parent || "unknown";target: link.url || "unknown";status: link.status || "unknown";type: link.url?.startsWith("http") ? "external" : "internal";}));recommendations: [];}
    // Generate recommendations
    if (brokenLinks.length > 0) {
  report.recommendations.push({
  priority: "HIGH";action: "Fix broken internal links";details: "Broken internal links affect user experience";// Generate recommendations
    if (brokenLinks.length > 0) {
  report.recommendations.push({
  priority: "HIGH";action: "Fix broken internal links";details: "Broken internal links affect user experience";})
      const externalBroken = brokenLinks.filter(link =>
        link.url?.startsWith("http") )
      if (externalBroken.length > 0) {
  report.recommendations.push({
  priority: `MEDIUM`;action: `Update or remove broken external links`,details: `${externalBroken.length} external links are broken`});}
    }

    report.recommendations.push({
  priority: `LOW`;action: `Regular link validation`;details: "Run link checks weekly to maintain link integrity`})
    // Save detailed report
    fs.writeFileSync(this.reportFile, JSON.stringify(report, null, 2))
    // Generate markdown report
    const markdownReport = this.generateMarkdownReport(report)
    const markdownFile = path.join(
      this.projectRoot,link-reports`;`LINK_REPORT.md`
    )
