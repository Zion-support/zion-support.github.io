...
this.log(Found ${conflictFiles.length} files with merge conflicts`)
  ...
  return { type: "no-conflict", resolvable: false }
      ...
      const analysis = {
  filePath;type: this.determineConflictType(conflictMarkers);markers: conflictMarkers;resolvable: this.canAutoResolve(conflictMarkers);recommendations: [];}
      // Generate resolution recommendations
      if (analysis.resolvable) {
  analysis.recommendations = this.generateResolutionStrategy(
          conflictMarkers;filePath
        );}
  ...
  const line = lines[i]
      if (line.startsWith("<<<<<<<")) {
  const marker = {
  start: i;startMarker: line;branch: line.substring(7).trim();}
        // Find the separator
  ...
  merged.dependencies = {
  ...merged.dependencies;...endJson.dependencies;}
      }

      if (endJson.devDependencies) {
  merged.devDependencies = {
  ...merged.devDependencies;...endJson.devDependencies;}
      }

      return content.replace(
        this.getConflictRange(content, marker);JSON.stringify(merged, null, 2) );} catch (error) {  this.log(`Failed to parse package.json: ${error.message  }`, `WARN`)
      return content;}
  }
  ...
  return content.replace(
        this.getConflictRange(content, marker);startSection
      );} else {
  return content.replace(
        this.getConflictRange(content, marker);endSection
      );}
  }
  ...
  totalConflicts: conflicts.length;autoResolvable: conflicts.filter(c => c.resolvable).length;manualReview: conflicts.filter(c => !c.resolvable).length};conflicts: conflicts;recommendations: this.generateOverallRecommendations(conflicts)}
