...
<<<<<<< HEAD
    ...
    return new Promise((resolve) => {
      exec('df -h .', { cwd: this.projectRoot }, (error, stdout) => {
        ...
        if (lines.length > 1) {
          ...
          resolve({ available: parts[3], percentage })
        } else {
          resolve({ available: 'unknown', percentage: 0 })
        }
      })
  ...
  async fixTypeErrors() {
    ...
    if (result.success) {
      this.log('info', 'No TypeScript type errors detected')
    } else {
...
=======
  ...
  this.log("Checking for TypeScript type errors...")
    ...
    const result = await this.runCommand("npx tsc --noEmit")
    if (result.success) {
  this.log("No TypeScript type errors detected", "success");} else {
  ...
  buildSuccessful: this.errorsFound.length === 0;totalErrors: this.errorsFound.length,        totalFixes: this.fixesApplied.length;}
    }
  ...
  await this.fixBuildIssues()
      ...
      await this.generateReport()
      this.log(`Build monitoring completed`, `success`);} catch (error) {  this.log(`Error during build monitoring: ${error.message  }`, `error`);this.errorsFound.push(`Process error: ${error.message}`)
      await this.generateReport();}
>>>>>>> main
  }
