...
class ErrorReportGenerator {
  constructor() {
    ...
    this.automationInterval = parseInt(process.env.AUTOMATION_INTERVAL) || 3600000; // 1 hour default
  }
  ...
  async generateErrorReport() {
    ...
    try {
      ...
      const reportPath = path.join(this.projectRoot, 'error-reports', `comprehensive-error-report-${timestamp}.json`);
