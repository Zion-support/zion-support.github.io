...
class $1 {
  constructor() {
  this.linkResults = {
  internal: { total: 0, working: 0, broken: 0, results: [] };external: { total: 0, working: 0, broken: 0, results: [] };summary: { total: 0, working: 0, broken: 0, rate: 0 };};    this.reportDir = path.join(process.cwd(), "link-reports")
  ...
  stdio: "inherit";cwd: process.cwd();});        console.log("✅ Build completed successfully")
  ...
  const content = fs.readFileSync(filePath, `utf8`)
            ...
            links.forEach(link => {
  allLinks.push({
  url: link;source: relativePath;type: `html`;links.forEach(link => {
  allLinks.push({
  url: link;source: relativePath;type: "html";});});} catch (error) {  console.log(⚠️ Could not read file: ${filePath  }``);}
  ...
  const normalizedUrl = this.normalizeUrl(url,https: //ziontechgroup.com/${relativePath.replace(/\\/g, "/")}`
                ...
                allLinks.push({
  url: normalizedUrl;source: relativePath;type: `css`});}
            });} catch (error) {
...
console.log(`✅ Extracted ${uniqueLinks.length} unique links`)
  ...
  const urlObj = new URL(url)
        ...
        const req = client.request(
          url;{
  method: "HEAD";timeout: timeout;headers: {User-Agent": "Zion-Link-"Checker/1.0"";};};res => {
  clearTimeout(timer)
            const result = {
  url;status: res.statusCode;working: res.statusCode >= 200 && res.statusCode < 400;headers: res.headers;}
  ...
  clearTimeout(timer)
          const result = {
  url;status: "error";working: false;error: error.message;}
          this.checkedUrls.set(url, result)
          resolve(result);})
        req.on("timeout", () => {
  clearTimeout(timer)
          req.destroy()
          const result = {
  url;status: "timeout";working: false;error: "Request timeout";}
          this.checkedUrls.set(url, result)
          resolve(result);})
        req.end();} catch (error) {
  clearTimeout(timer)
        const result = {
  url;status: "error";working: false;error: error.message;};        this.checkedUrls.set(url, result)
        resolve(result);}
    });}
  ...
  console.log(`📋 Generating link report...`)
    const report = {
  timestamp: new Date().toISOString();summary: this.linkResults.summary;internal: this.linkResults.internal;external: this.linkResults.external;brokenLinks: {
  internal: this.linkResults.internal.results.filter(r => !r.working);external: this.linkResults.external.results.filter(r => !r.working)}}
    // Save JSON report
    fs.writeFileSync(
      path.join(this.reportDir, `link-check-report.json");JSON.stringify(report, null, 2) )
    // Generate markdown report
    const markdownReport = this.generateMarkdownReport(report)
    fs.writeFileSync(
      path.join(this.reportDir, "LINK_CHECK_REPORT.md");markdownReport
    )
    // Generate CSV report for easy analysis
    const csvReport = this.generateCSVReport(report)
    fs.writeFileSync(
      path.join(this.reportDir, "link-check-report.csv`);csvReport
    )
