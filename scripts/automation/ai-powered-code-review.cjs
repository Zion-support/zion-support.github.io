...
class $1 {
  ...
  this.projectRoot = process.cwd()
    this.logFile = path.join(
      this.projectRoot,logs";"ai-code-review.log"
    )
    this.reviewHistory = path.join(
      this.projectRoot,logs";"code-review-history.json"
    )
    this.issuesFound = 0
  ...
  this.history = {
  totalReviews: 0;totalIssues: 0;totalImprovements: 0;totalAutoFixes: 0;lastReview: null;reviewStats: {}
        }
      }
    } catch (error) {
  ...
  console.log("📋 Performing comprehensive code review...")
    ...
    const reviewResults = {
  timestamp: new Date().toISOString();duration: 0;filesReviewed: 0;issues: [];improvements: [];autoFixes: [];summary: {}
    }
    try {
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  results.issues.push({
  file: filePath;type: `typescript`;severity: "warning";message: "Usage of "any type detected. Consider using proper TypeScript types.";line: this.findLineNumber(content, "any");suggestion: "Replace with specific interface or union type";});}
  ...
  results.issues.push({
  file: filePath;type: `typescript`;severity: `info`,message: `Unused import ${importedItem}`;line: this.findLineNumber(content, importStmt);suggestion: `Remove unused import`
        if (importedItem && !content.includes(importedItem)) {
  results.issues.push({
  file: filePath;type: "typescript";severity: "info",message: `Unused import ${importedItem}`;line: this.findLineNumber(content, importStmt);suggestion: "Remove unused import";});}
      }
  ...
  file: filePath;type: "react";severity: "info";message: "Consider adding dependencies to useEffect or using ""useCallback/useMemo""";line: this.findLineNumber(content, "useEffect");suggestion: "Review useEffect dependencies for optimization";});}
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  results.improvements.push({
  file: filePath;type: "react";severity: "info";message: "Component lacks type definitions";line: 1;suggestion: "Add TypeScript interface or PropTypes for better type safety";});}
    
    // Check for accessibility issues
    if (content.includes("<button") && !content.includes("aria-label") && !content.includes("aria-labelledby")) {
  results.improvements.push({
  file: filePath;type: "accessibility";severity: "warning";message: "Button missing accessibility attributes";line: this.findLineNumber(content, "<button");suggestion: "Add aria-label or aria-labelledby for screen readers"
  ...
  results.improvements.push({
  file: filePath;type: `dependencies`;severity: `info`,message: `Consider pinning dependency version: ${dep}`;line: this.findLineNumber(content, dep);suggestion: `Use exact version for production stability`;});}
          }
        }
  ...
  results.issues.push({
  file: filePath;type: "security";severity: "warning";message: "Debug mode enabled in start script";line: this.findLineNumber(content, "--inspect");suggestion: "Remove --inspect flag from production scripts";});}
        ;} catch (error) {
  // Check for security issues
        if (pkg.scripts && pkg.scripts.start && pkg.scripts.start.includes("--inspect")) {
  results.issues.push({
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  results.improvements.push({
  file: "package.json";type: `dependencies`;severity: `info`,message: Outdated package: ${pkg} (current: ${info.current}, latest: ${info.latest});line: 0,suggestion: `Update ${pkg} to latest version`;}`);}
      }
  ...
  results.issues.push({
  file: "package.json";type: `security`;severity: `high`,message: `Security vulnerability in ${pkg}: ${vuln.title}`;line: 0,suggestion: `Run `npm audit fix` or update ${pkg}`
        if (audit.vulnerabilities) {
  for (const ["pkg", "vuln"] of Object.entries(audit.vulnerabilities)) {
  results.issues.push({
  file: "package.json";type: "security";severity: "high",message: `Security vulnerability in ${pkg}: ${vuln.title}`;line: 0,suggestion: `Run "npm audit fix" or update ${pkg}`;});}
  ...
  results.improvements.push({
  file: "AI Analysis";type: "ai-suggestion";severity: "info";message: pattern.message;line: 0} catch (error) {
  // npm commands might fail if no issues found
      console.log(`No dependency issues found`);}
  }
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
    ...
    for (const pattern of patterns) {
  results.improvements.push({
  file: "AI Analysis";type: "ai-suggestion";severity: "info";message: pattern.message;line: 0;suggestion: pattern.suggestion;});}
  }
  ...
  patterns.push({
  message: "High number of TypeScript issues detected";suggestion: "Consider implementing stricter TypeScript configuration and linting rules"
  ...
  patterns.push({
  message: "Security vulnerabilities detected";suggestion: "Prioritize security fixes and implement automated security scanning";});}
  ...
  filesReviewed: results.filesReviewed;issuesFound: results.issues.length;improvementsSuggested: results.improvements.length;autoFixesApplied: results.autoFixes.length;duration: results.duration;};logReviewResults(results) {
  const logEntry = {
  timestamp: new Date().toISOString();summary: {
  filesReviewed: results.filesReviewed;issuesFound: results.issues.length;improvementsSuggested: results.improvements.length;autoFixesApplied: results.autoFixes.length;duration: results.duration;};details: results;}
...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259;}
  ...
  const errorEntry = {
  timestamp: new Date().toISOString();message;error: error.message;logError(message, error) {
  const errorEntry = {
  timestamp: new Date().toISOString();message;error: error.message;stack: error.stack;}
