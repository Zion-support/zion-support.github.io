...
class AICodeQualityAnalyzer {;
  constructor() {;
    this.logFile = path.join(__dirname, '..', '..', 'automation-reports', 'ai-code-quality.log');
    this.ensureLogDir();
  }
;
  ensureLogDir() {;
...
;
    const reportPath = path.join(__dirname, '..', '..', 'automation-reports', 'ai-code-quality-report.json');
