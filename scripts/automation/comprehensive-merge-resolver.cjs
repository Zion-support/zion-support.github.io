...
class $1 {
  ...
  this.projectRoot = process.cwd()
    ...
    this.mergeStats = {
  branchesProcessed: 0;conflictsResolved: 0;mergesSuccessful: 0;errors: 0;}
  }
...
this.log(`Found ${branches.length} remote branches`)
  ...
  this.log(❌ Error processing branch ${branchName  }: ${error.message}`;`ERROR`;} catch (error) {
  ...
  this.log(⚠️ Failed to create backup for ${branchName  }: ${error.message}`;`WARN`
      );}
  }
  ...
  stdio: "pipe";timeout: 300000, // 5 minutes timeout;})
      return { success: true }
    } catch (error) {
...
this.log(`✅ Resolved conflicts in: ${filePath}");} catch (error) {
  ...
  branchesProcessed: this.mergeStats.branchesProcessed;conflictsResolved: this.mergeStats.conflictsResolved;mergesSuccessful: this.mergeStats.mergesSuccessful;errors: this.mergeStats.errors;};successRate: this.mergeStats.branchesProcessed > 0;          ? (
