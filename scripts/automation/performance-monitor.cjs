...
>>>>>>> 8b2501468f72f02648b06a2725c17d2465cef259
  ...
  console.log("ℹ️  Dependency check not available");}
    ...
    const report = {
  timestamp: new Date().toISOString();buildSize: getDirectorySize(distPath);largeFiles: findLargeFiles(distPath);summary: `Performance monitoring completed`}
...
function findLargeFiles(dir, maxSize = 1024 * 1024) {
  ...
  largeFiles.push({
  path: path.relative(process.cwd()`, fullPath);size: stat.size});}
      }
    } catch (error) {
