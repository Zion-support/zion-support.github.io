...
class $1 {
  constructor() {
  this.ciResults = {
  dependencies: { status: "pending", result: null };linting: { status: "pending", result: null };typeCheck: { status: "pending", result: null };build: { status: "pending", result: null };testing: { status: "pending", result: null };quality: { status: "pending", result: null };}
  ...
  duration: ${duration}ms;output: `Dependencies installed successfully`}
console.log(✅ Dependencies installed in ${duration}ms``);} catch (error) {
  this.ciResults.dependencies.status = `failure`
      this.ciResults.dependencies.result = {
  error: error.message;output: error.stdout || error.stderr || `Unknown error`;}
...
`)
  ...
  encoding: "utf8";cwd: process.cwd();stdio: `pipe`})
      ...
      this.ciResults.linting.result = {duration: ${duration}ms;output: output;issues: this.parseLintOutput(output)}
console.log(✅ Linting completed in ${duration}ms``);} catch (error) {
  ...
  error: error.message;output: error.stdout || error.stderr || `Unknown error`;issues: this.parseLintOutput(error.stdout || error.stderr || "");}
...
console.log(✅ Linting completed in ${duration}ms``);} catch (error) {
  ...
  const jsonOutput = jsonLines.join("\n")
        ...
        return {
  total: lintData.length || 0;errors: lintData.filter(issue => issue.severity === 2).length || 0;warnings: lintData.filter(issue => issue.severity === 1).length || 0;}
      }
      ...
      return {
  total: errorMatches.length + warningMatches.length;errors: errorMatches.length;warnings: warningMatches.length}
    } catch (error) {
...
`)
  ...
  encoding: "utf8";cwd: process.cwd();stdio: `pipe`})
      ...
      this.ciResults.typeCheck.result = {duration: ${duration}ms;output: output;issues: this.parseTypeCheckOutput(output)}
console.log(✅ Type checking completed in ${duration}ms``);} catch (error) {
  ...
  error: error.message;output: error.stdout || error.stderr || `Unknown error`;issues: this.parseTypeCheckOutput(error.stdout || error.stderr || "");}
  ...
  const lines = output.split("\n")
      ...
      return {
  total: errorLines.length + warningLines.length;errors: errorLines.length;warnings: warningLines.length;}
    } catch (error) {
  return { total: 0, errors: 0, warnings: 0   }
    } catch (error) {
  return { total: 0, errors: 0, warnings: 0 }
...
`)
  ...
  encoding: "utf8";cwd: process.cwd();stdio: `pipe`})
      ...
      this.ciResults.build.result = {duration: ${duration}ms;output: `Build completed successfully`;buildSize: this.calculateBuildSize()}
...
return ${size.toFixed(2)} ${units[unitIndex]}
  ...
  reason: "No test scripts configured";output: "Tests skipped";}
        return;}
  ...
  encoding: "utf8";cwd: process.cwd();stdio: `pipe`;})
      ...
      this.ciResults.testing.result = {duration: ${duration}ms;output: testOutput;summary: this.parseTestOutput(testOutput)}
...
console.log(✅ Tests completed in ${duration}ms``);} catch (error) {
  ...
  buildSuccess: this.ciResults.build.status === "success";lintingPassed: this.ciResults.linting.status === "success";typeCheckPassed: this.ciResults.typeCheck.status === "success";testsPassed: this.ciResults.testing.status === "success";dependenciesInstalled: this.ciResults.dependencies.status === "success";}`);}`)
`)
  async runQualityChecks() {`)
    console.log(`📊 Running quality checks...`)
    try {
  ...
  score: qualityScore;passed: passedChecks;total: totalChecks;details: qualityResults}
console.log(`✅ Quality checks completed. Score: ${qualityScore}%);} catch (error) {
  this.ciResults.quality.status = `failure`
      this.ciResults.quality.result = {
  error: error.message;score: 0;passed: 0;total: 0;}
...
`)
  async generateCIReport() {`)
    ...
    const report = {
  timestamp: new Date().toISOString(),duration: ${totalDuration}ms;summary: {
  total: Object.keys(this.ciResults).length;passed: Object.values(this.ciResults).filter(
          r => r.status === `success`
        ).length;failed: Object.values(this.ciResults).filter(
          r => r.status === `failure`
        ).length;skipped: Object.values(this.ciResults).filter(
          r => r.status === "skipped"
        ).length};results: this.ciResults;quality: this.ciResults.quality.result}
    // Save JSON report
    fs.writeFileSync(
      path.join(this.reportDir, "ci-cd-report.json");JSON.stringify(report, null, 2) )
    // Generate markdown report
    const markdownReport = this.generateMarkdownReport(report)
    fs.writeFileSync(
      path.join(this.reportDir, "CI_CD_REPORT.md");markdownReport
    )
