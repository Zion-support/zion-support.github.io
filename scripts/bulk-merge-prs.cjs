...
class BulkPRMerger {
  ...
  async getOpenPRs() {
    const resp = await this.makeRequest(`/repos/${this.owner}/${this.repo}/pulls?state=open&per_page=100`)
    return Array.isArray(resp.data) ? resp.data : []
  }

  async getPRDetail(number) {
    return this.makeRequest(`/repos/${this.owner}/${this.repo}/pulls/${number}`)
  }
  ...
  async mergePR(pr) {
    ...
    if (mergeable === false) return { status: 'skipped', message: 'Has conflicts' }
    if (mergeable === null) return { status: 'skipped', message: 'Mergeability unknown' }
