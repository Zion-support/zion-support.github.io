...
global.console.log(`Found ${prs.length} open PRs`);
...
global.console.log(`Found ${duplicatePRs.length} duplicate PRs to close`);

// Close each duplicate PR;
duplicatePRs.forEach(pr => {
  try {
    global.console.log(`Closing PR #${pr.number}: ${pr.title}`);
    ...
    global.console.log(`Deleted branch ${pr.headRefName} for PR #${pr.number}`);
  } catch (_error) {
    global.console._error(`Error processing PR #${pr.number}:`, _error.message);
  }
});

global.console.log('Completed processing duplicate PRs');';