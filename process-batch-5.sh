...
# Process fifth batch of branches

set -e

echo "Starting fifth batch processing of branches..."

# Get next 50 remote branches that start with cursor/
ALL_BRANCHES=$(git branch -r | grep "origin/cursor/" | sed 's/origin\///' | tail -n +151 | head -50)
...
echo "Processing next 50 branches (151-200)..."
...
echo "Completed fifth batch processing!"