...
async function auditWebsite() {
  ...
  for (let i = 0; i < routes.length; i++) {
    const route = routesService Feature Testing ${route}... `);
    
    const result = await checkUrl(fullUrl);
    
    if (result.error) {
      results.errors.push({ url: fullUrl, error: result.error });
      console.log(`❌ ERROR: ${result.error}`)} else if (result.status >= 200 && result.status < 300) {
      results.working.push({ url: fullUrl, status: result.status });
      console.log(`✅ ${result.status}`)} else if (result.status === 404) {
      results.missing.push({ url: fullUrl, status: result.status });
      console.log(`❌ 404 - Missing`)} else {
