...
def fix_all_pages_final():
    """Fix all pages with a comprehensive approach"""
    # Get all page files
    ...
    for page_file in page_files:
        try:
            ...
            # Check if file has any structural issues
            if ('</h2>' in content and '<h2' not in content) or \
               ...
               ('Unterminated regular expression' in content) or \
               ('</section>' in content and '<section' not in content) or \
               ('</>' in content and '<>' not in content) or \
               ('</div>' in content and content.count('</div>') > content.count('<div')) or \
               ('</section>' in content and content.count('</section>') > content.count('<section')):
