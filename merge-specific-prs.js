...
function safeMergePR(pr) {
  try {
    ...
    try {
      execSync(`git merge-tree $(git merge-base HEAD ${pr.branch}) HEAD ${pr.branch}`, { stdio: 'pipe' });
      console.log(`   ✅ No conflicts detected`);
    } catch (error) {
      console.log(`   ⚠️  Potential conflicts detected, will attempt resolution`);
    }
    
    // Attempt to merge
    console.log(`   🔄 Merging ${pr.branch} into main...`);
    execSync(`git merge ${pr.branch} --no-ff -m "Merge PR #${pr.number}: ${pr.title}"`, { stdio: 'inherit' });
    ...
    try {
      ...
      execSync(`git merge ${pr.branch} -X ours --no-ff -m "Auto-merge PR #${pr.number}: ${pr.title}"`, { stdio: 'inherit' });
