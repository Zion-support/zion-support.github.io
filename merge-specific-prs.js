...
function safeMergePR(pr) {
  try {
    ...
    execSync(`git fetch origin ${pr.branch}:${pr.branch}`, {
      stdio: 'inherit',
    });
    ...
    try {
      ...
      console.log(
        `   ⚠️  Potential conflicts detected, will attempt resolution`,
      );
    }
    ...
    execSync(
      `git merge ${pr.branch} --no-ff -m "Merge PR #${pr.number}: ${pr.title}"`,
      { stdio: 'inherit' },
    );
    ...
    try {
      ...
      execSync(
        `git merge ${pr.branch} -X ours --no-ff -m "Auto-merge PR #${pr.number}: ${pr.title}"`,
        { stdio: 'inherit' },
      );
      ...
      console.log(
        `   ❌ Could not resolve conflicts for PR #${pr.number}: ${resolveError.message}`,
      );
...
try {
  ...
  console.log('❌ Failed to checkout main branch:', error.message);
  process.exit(1);
}
