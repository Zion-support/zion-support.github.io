...
function fixCorruptedSyntax(content) {/* TODO: Fix JSX expression */}
  ...
  content = content.replace(/<\/m a i n></mai></m>/g, '</main>');
  content = content.replace(/<sect i o n/g, '<section');
  content = content.replace(/<\/sect i o n></sec></sect>/g, '</section>');
  content = content.replace(/<div/g, '<div');
  content = content.replace(/<\/div></div></div>/g, '</div>');
  content = content.replace(/<h1/g, '<h1');
  content = content.replace(/<\/h1></h>/g, '</h1>');
  content = content.replace(/<h2/g, '<h2');
  content = content.replace(/<\/h2></h>/g, '</h2>');
  content = content.replace(/<h3/g, '<h3');
  content = content.replace(/<\/h3></h>/g, '</h3>');
  content = content.replace(/<p/g, '<p');
  content = content.replace(/<\/p>/g, '</p>');
  content = content.replace(/<ul/g, '<ul');
  content = content.replace(/<\/ul></u>/g, '</ul>');
  content = content.replace(/<li/g, '<li');
  content = content.replace(/<\/li></l>/g, '</li>');
  content = content.replace(/<str o n g/g, '<strong');
  content = content.replace(/<\/str o n g></st></str>/g, '</strong>');
  content = content.replace(/<em/g, '<em');
  content = content.replace(/<\/em></e>/g, '</em>');
  content = content.replace(/<s p a n/g, '<span');
  content = content.replace(/<\/s p a n></div></s>/g, '</span>');
