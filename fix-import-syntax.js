...
function fixImportSyntax(filePath) {
  try {
    ...
    const importSemicolonRegex = /import\s*\{([^}]+)\}\s*from\s*['"][^'"]+['"];?/g;
    ...
    content = content.replace(/function\s*\(([^)]+)\)\s*{/g, (match, params) => {
  ...
  function traverse(currentDir) {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (['node_modules', '.git', '.next', 'dist', 'build', 'out', 'ai-optimization-backups', 'apps.backup', 'backup-merge-conflicts', 'apps'].includes(item)) {
          continue;
        }
        traverse(fullPath);
      } else if (stat.isFile()) {
