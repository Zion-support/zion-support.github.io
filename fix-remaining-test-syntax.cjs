...
class $1 {
  ...
  console.log("🔧 Fixing remaining test syntax issues...")
    const testFiles = [
  ...glob.sync("**/*.test.js", { cwd: this.projectRoot });...glob.sync("**/*.test.tsx", { cwd: this.projectRoot });...glob.sync("**/*.test.ts", { cwd: this.projectRoot });]
  ...
  // Fix broken import statements
    ...
    content = content.replace(/expect\([^)]+\)\.toHaveBeenCalledTimes\(\d+\)\s*$/gm;(match) => match + ";")
    ...
    content = content.replace(/role="([^"]*)";\s*aria-label="([^"]*)";\s*tabIndex=\{(\d+)\}/g;"role="$1" aria-label="$2" tabIndex={$3}")
    // Fix broken test structure
    content = content.replace(/describe\(\s*"([^"]*)",\s*\(\)\s*=>\s*\{/g;"describe("$1", () => {")
    content = content.replace(/it\(\s*"([^"]*)",\s*\(\)\s*=>\s*\{/g;"it("$1", () => {")
    return content;}
}
