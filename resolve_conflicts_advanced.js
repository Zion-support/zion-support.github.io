...
const getConflictBranches = () => {
  try {
    ...
    console.error('Error reading merge results:', error.message);
    return [];
  }
};
...
const resolveConflictsAdvanced = (branch) => {
  try {
    ...
    try {
      ...
      try {
        // Strategy 1: Use ours for common conflict files
        const commonConflictFiles = [
          'package.json', 'package-lock.json', 'pnpm-lock.yaml',
          'src/App.tsx', 'src/main.tsx', 'src/components/',
          '.eslintrc.js', 'eslint.config.js', 'tsconfig.json'
        ];
        ...
        try {
          execSync('git status --porcelain | grep "^UU" | cut -c4- | xargs -I {} git checkout --ours "{}"', { stdio: 'pipe' });
        } catch (oursError) {
...
const processConflictsInBatches = async () => {
  const conflictBranches = getConflictBranches();
  ...
  for (let i = 0; i < conflictBranches.length; i += batchSize) {
    const batch = conflictBranches.slice(i, i + batchSize);
    console.log(`\n🔄 Processing batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(conflictBranches.length/batchSize)}`);
    
    for (const branch of batch) {
      const success = resolveConflictsAdvanced(branch);
      if (success) {
        successCount++;
        results.push({ branch, status: 'resolved' });
      } else {
        results.push({ branch, status: 'failed' });
      }
  ...
  // Save results
  fs.writeFileSync('conflict-resolution-results.json', JSON.stringify(results, null, 2));
