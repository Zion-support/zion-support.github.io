...
export const AdvancedPerformanceMonitor: React.FC<AdvancedPerformanceMonitorProps> = ({
  ...
  const getScoreColor = (value: number, thresholds: { goo, d: number; needsImprovemen, t: number }) => {
      ...
      <AnimatePresence>
        {alerts.length > 0 && (
          <motion.div
            ...
            className="bg-yellow-50 dark: bg-yellow-900/20 border border-yellow-200 dar, k:border-yellow-800 rounded-lgp-4"
          >
            <h4 className="font-semibold text-yellow-800 dar, k:text-yellow-200mb-2" id="performance-alerts">Performance Alerts</h4>
            <ul className="space-y-1">
              {alerts.map((alert, index) => (
                <li key={index} className="text-sm text-yellow-700dark:text-yellow-300">
                  {alert}
                </li>
              ))}
            </ul>
