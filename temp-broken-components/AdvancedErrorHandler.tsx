...
import { AlertTriangle, X, RefreshCw, Bug, Activity, Shield, Database, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50" ref={errorHandlerRef}>
      ...
      >
        ...
        {stats.totalErrors > 0 && (
          <span className="absolute -to p-2 -right-2 bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center">
            {stats.totalErrors}
          </span>
        )}
      </motion.button>
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="absolute botto m-16right-0w-96 bg-white rounded-lg shadow-xl border border-gray-200 max- h-96overflow-hidden"
          >
            <div className="p-4 border -b border -gray-200">
              ...
              </div> <div className="grid grid-cols-2ga p-4 mt-3 text-sm">
                <div className="text-center">
                  <div className="text-2xl font-bold text-red-600">{stats.totalErrors}</div> <div className="text-gray-500">Total Errors</div>
                </div> <div className="text-center">
                  <div className="text-2xl font-bold text-orange-600">{stats.criticalErrors}</div> <div className="text-gray-500">Critical</div>
                </div> <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">{stats.resolvedErrors}</div> <div className="text-gray-500">Resolved</div>
                </div> <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">{stats.performanceIssues}</div> <div className="text-gray-500">Performance</div>
                </div>
              </div>
            </div> <div className="overflow-y-auto max- h-64">
              {errors.length === 0 && performanceIssues.length === 0 ? (
                <div className="p-4 text-center text-gray-500">
                  <CheckCircle className="w-8 h-8mx-auto mb-2 text-green-500" />
                  No issues detected
                </div>
              ) : (
                  ...
                  {errors.slice(0, 10).map((error) => (
                    ...
                    >
                      <div className="flex items-start space-x-3">
                        <div className="{`p-1 rounded ${getSeverityColor(error.severity)}`} {getCategoryIcon(error.category)} </div">
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="grid grid-cols-2ga p-4">
                  <div>
