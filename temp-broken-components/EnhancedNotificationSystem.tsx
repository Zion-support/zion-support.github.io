...
export default function EnhancedNotificationSystem({
  ...
  const handleAction = useCallback((notificationId: string, actionLabel: string) => {
    onAction?.(notificationId, actionLabel);
  }, [onAction]);
  ...
  const renderGroupedNotifications = () => {
    ...
    return Object.entries(groupedNotifications).map(([category, categoryNotifications]) => (
  ...
  return (
    <div className={getPositionClasses()}>
      ...
      {isExpanded && (
        <motion.div
          className="bg-white rounded-full shadow-lg p-3 cursor-pointer"
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
      ...
      <AnimatePresence mode="popLayout">
        {isExpanded ? (
          <motion.div
            className="bg-white rounded-lg shadow-xl border max-h-96 overflow-y-auto"
