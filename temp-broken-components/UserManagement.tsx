...
interface User {
  ...
  statu,
    s: 'active' | 'inactive' | 'pending' | 'suspended';
  avatar?: string;
  lastLogin?: Date;
  createdA,
    t: Date;
...
export const UserManagement: React.FC<UserManagementProps> = ({
  ...
  if (isLoading) {
    return (      <div className = {`bg-white rounded-lg shadow-sm border border-gray-200 p-6 ${className}`}>
        <div className="animate-pulse">          <div className=h-6 bg-gray-300 rounded w-1/4 mb-4""></div>          <div className=space-y-3"">
            {[...Array(5)].map((_, i) => (              <div key = {i} className=flex items-center space-x-4"">                <div className=h-10 w-10 bg-gray-300 rounded-full""></div>                <div className=flex-1 space-y-2"">                  <div className=h-4 bg-gray-300 rounded w-1/4""></div>                  <div className=h-3 bg-gray-300 rounded w-1/3""></div>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className = {`bg-white rounded-lg shadow-sm border border-gray-200 ${className}`}>
      {/* Header */}      <div className=px-6 py-4 border-b border-gray-200"">        <div className=flex items-center justify-between"">          <h2 className=text-lg font-semibold text-gray-900"" id="user-management">User Management</h2>
          <button
            onClick={() => setShowCreateModal(true}            aria-label=setShowCreateModal(true""}            className=bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors""
      ...
      {/* Filters and Search */}      <div className=px-6 py-4 border-b border-gray-200"">        <div className=flex flex-col sm:flex-row gap-4"">          <div className=flex-1"">
            <input              type=text""              placeholder=Search users...""
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}              className=w-full px-3 py-2 border border-gray-300 rounded-md focus: outline-none focu,
    s:ring-2 focu,
    s:ring-blue-500""              aria-label = Search users""
          ...
          <select
            value={filterRole}
            onChange={(e) => setFilterRole(e.target.value)}            className=px-3 py-2 border border-gray-300 rounded-md focus: outline-none focu,
    s:ring-2 focu,
    s:ring-blue-500""
          ...
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}            className=px-3 py-2 border border-gray-300 rounded-md focus: outline-none focu,
    s:ring-2 focu,
    s:ring-blue-500""
      ...
      {/* Bulk Actions */}
      {selectedUsers.length > 0 && (        <div className=px-6 py-3 bg-gray-50 border-b border-gray-200"">          <div className=flex items-center space-x-4"">            <span className=text-sm text-gray-600"">              {selectedUsers.length} user{selectedUsers.length !== 1 ? 's' : ''} selected
            </span>            <div className=flex space-x-2"">
              <button
                onClick={() => handleBulkAction('activate'}            aria-label=handleBulkAction('activate'""}                className=text-sm text-green-600 hover:text-green-700""
              ...
              <button
                onClick={() => handleBulkAction('deactivate'}            aria-label=handleBulkAction('deactivate'""}                className=text-sm text-gray-600 hover:text-gray-700""
              ...
              <button
                onClick={() => handleBulkAction('suspend'}            aria-label=handleBulkAction('suspend'""}                className=text-sm text-yellow-600 hover:text-yellow-700""
              ...
              <button
                onClick={() => handleBulkAction('delete'}            aria-label=handleBulkAction('delete'""}                className=text-sm text-red-600 hover:text-red-700""
      ...
      {/* Users Table */}      <div className=overflow-x-auto"">        <table className=min-w-full divide-y divide-gray-200"">          <thead className=bg-gray-50"">
            <tr>              <th className=px-6 py-3 text-left"">
                <input                  type=checkbox""                  id=select-all-users""
                  checked={selectedUsers.length === sortedUsers.length && sortedUsers.length > 0}
                  onChange={handleSelectAll}                  className=h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded""                  aria-label=Select all users""
                />
            ...
            {sortedUsers.map((user) => (              <tr key={user.id} className=hover:bg-gray-50"">                <td className=px-6 py-4 whitespace-nowrap"">
                  <input                    type=checkbox""
                    id={`user-${user.id}`}
                    checked={selectedUsers.includes(user.id)}
                    onChange={() => handleUserSelect(user.id)}                    className=h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded""
                    aria-label={`Select user ${user.name}`}
                  />
                </td>                <td className=px-6 py-4 whitespace-nowrap"">                  <div className=flex items-center"">                    <div className=flex-shrink-0 h-10 w-10"">
                  ...
                  {user.lastLogin ? user.lastLogin.toLocaleDateString() : 'Never'}
                    ...
                    <button
                      onClick={() => setEditingUser(user}            aria-label=setEditingUser(user""}                      className=text-blue-600 hover:text-blue-900""
                    ...
                    <button
                      onClick={() => handleDeleteUser(user.id}            aria-label=handleDeleteUser(user.id""}                      className=text-red-600 hover:text-red-900""
      ...
      {/* Pagination */}      <div className=px-6 py-4 border-t border-gray-200"">        <div className=flex items-center justify-between"">          <div className=text-sm text-gray-700"">
            Showing {sortedUsers.length} of {users.length} users
          </div>          <div className=flex space-x-2"">            <button className=px-3 py-1 text-sm border border-gray-300 rounded-md hover: bg-gray-50"" aria-label="Previous">
              Previous
            </button>            <button className=px-3 py-1 text-sm border border-gray-300 rounded-md hove,
