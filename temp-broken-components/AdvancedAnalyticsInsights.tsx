...
export default function AdvancedAnalyticsInsights({
  ...
  const generateAnalyticsData = React.useCallback((): AnalyticsData => {
    ...
    return {
      uniqueUsers: Math.floor(890 * baseMultiplier)pageViews: Math.floor(3200 * baseMultiplier)bounceRate: 0.35 + (Math.random() - 0.5) * 0.1avgSessionDuration: 180 + (Math.random() - 0.5) * 60conversionRate: 0.12 + (Math.random() - 0.5) * 0.05,
      traffic: {
        sources: [
