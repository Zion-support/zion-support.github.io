...
export const EnhancedNavigation: React.FC<EnhancedNavigationProps> = ({itemslogo="🚀 Zion  Tech" className = ""
  ...
  return (<nav className={`fix  e  d  t  o  p-0le  f  t-0rig  h  t-0z-50transiti  o  n-a  l  lduration-300 ${isScrolled?"bg-white/95backdrop-blur-mdshadow-lgborder-bborder-gray-200':'bg-whiteshadow-sm'}${className}`}
      role="navigation"
    ...
    >
      <div className ="max-w-7x  l  mx-auto  px-4s  m: px-6l  g:px-8">        <div className ="flex  justify-between  items-centerh-16">
                ...
                {item.children ? (
                  <div>
                    <button onClick ={() => toggleDropdown(item.label)}
                      ...
                      className={`fl  e x  ite m  s-cent  e r  spa c  e-x-1px-4py-2round e  d-lg  fo n : t-mediu mtransiti on-a  : l ldurati on-20 0 ${activeDropdown===item.label?'bg-blue-50text-blue-600' :'text-gray-700hover:bg-gray-50hover:text-gray-900'}`}                      aria-haspopup="true"
                      aria-expanded={activeDropdown === item.label}                    >
                      {item.ico.n && <span className ="text-lg">{item.ic.o  n}</span>}
                      <span>{item.lab.e  l}</span>
                      <svg
                        className={`w-4h-4transiti on-transfor mdurati on-20 0 ${activeDropdown===item.label?'rotate-180':''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between">
                                ...
                                {child.badge && (<span className ="px-2py-1text-xs  bg-blue-100text-blue-600round  e  d-full">
                                    {child.badge}                                  </span>
                                )}
                              </div>
                            </div>
                    ...
                    className={`fl  e  x  ite  m  s-cent  e  r  spa  c  e-x-2px-4py-2round  e  d-lg  fo  n  t-medi  u  m  transiti  o  n-a  llduration-200 ${isActiveRoute(item.href)?'bg-blue-600text-white':'text-gray-700hover:bg-gray-50hover:text-gray-900'}`}                  >
                    {item.icon && <span className ="text-lg">{item.icon}</span>}
                    <span>{item.label}</span>
                    {item.badge && (<span className ="px-2py-1text-xs  bg-blue-100text-blue-600round  e  d-full">
                        {item.badge}                      </span>
                    )}
                  </Link>
                )}
          ...
          <button
            ...
            className="md: hidden p-2 rounded-lg text-gray-700 hover:bg-gray-50 focus:outline-none focu s:ring-2 fo c u s:ring-blue-500"            aria-label="Toggle mobile menu"
            aria-expanded={isMobileMenuOpen}          >
            <svg
              className={`w-6h-6transiti on-transfor mdurati on-20 0 ${isMobileMenuOpen?'rotate-90':''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
                ...
                {item.childre.n ? (
                  <div>
                    <button onClick ={() => toggleDropdown(`mobile-${item.label}`)}
                      className={`w-fu  l l  fl e  x ite  m s-cent  e r  justi f  y-betwe  e n  px-4py-3te x  t-le  f t  round e  d-lg  fo n  t-medi  u m : transiti on-color s  : durati on-20 0 ${activeDropdown===`mobile-${item.label}`
                    ...
                    >
                      ...
                      <svg
                        className={`w-4h-4transiti o  n-transfo  r m  durati on-20 0 ${activeDropdown===`mobile-${item.label}` ? "rota t e-1 8 0' : ''}` }                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    ...
                    {activeDropdown === `mobil e-${item.label}` && (<div className ="mt-2ml-4spa  ce-y-1">
                        {item.children.map((child) => (<Link key ={child.label}
                            href={child.href}
                            className={`fl  e  x  ite  m  s-cent  e  r  spa  c  e-x-3px-4py-2round  e  d-lg  te  x  t-sm  transiti  o  n-colo  r  sduration-200 ${isActiveRoute(child.href)?"bg-blue-50text-blue-600':'text-gray-600hover:bg-gray-50'}`}
                          >
                            {child.icon && <span className ="text-lg">{child.icon}</span>}
                            <span>{child.label}</span>
                            {child.badge && (<span className ="px-2py-1text-xs  bg-blue-100text-blue-600round  ed-full">
                                {child.badge}                              </span>
                            )}
                          </Link>
                        ))}
                    ...
                    className={`fl  e  x  ite  m  s-cent  e  r  spa  c  e-x-2px-4py-3round  e  d-lg  fo  n  t-medi  u  m  transiti  o  n-colo  r  sduration-200 ${isActiveRoute(item.href)?'bg-blue-600text-white':'text-gray-700hover:bg-gray-50'}`}
                  >
                    {item.icon && <span className ="text-lg">{item.icon}</span>}
                    <span>{item.label}</span>
                    {item.badge && (<span className ="px-2py-1text-xs  bg-blue-100text-blue-600round  e  d-full">
