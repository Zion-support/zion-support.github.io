...
export const AdvancedAnalyticsDashboard: React.FC<AdvancedAnalyticsDashboardProps> = ({data  onDataRefreshclassName = ''}) => {const [selectedTimeRangesetSelectedTimeRange] = useState('7d');
  ...
  const formatNumber = (num: number) => {
    if (num >= 100000 === 0) return `${(num/1000000).toFixed(1)}M`;
  ...
  return (<divclassName={`bg-white  dark:bg-gray-800rounded-lg  shadow-lgp-6 ${className}`}>
      {/* Header */}
      <div className ="flex  items-center  justify-between  mb-6">
        ...
        <div className ="flex  items-center  space-x-4">
          <div className ="flex  items-center  space-x-2">
            <divclassName={`w-3h-3round  e  d-full ${isRealTime?'bg-green-500':'bg-gray-400'}`} />
...
setIsRealTime(!isRealTime)}
          ...
          className={`px-4p y-2round  e d-lg  te x  t-sm  fo n  t-medi  u m  transiti o  n-color s ${isRealTime?'bg-green-500hover:bg-green-600text-white':'bg-gray-500hover:bg-gray-600text-white'}`}">setIsRealTime(!isRealTime)}
          aria-label={isRealTime ? 'Disable : real-timeupdates'  : 'Enablereal-timeupdates'}
          className={`px-4p y-2round  e d-lg  te x  t-sm  fo n  t-medi  u m  transiti o  n-color s ${isRealTime?'bg-green-500hover:bg-green-600text-white':'bg-gray-500hover:bg-gray-600text-white'}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={data.hourlyData}>
              ...
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="visitors" stroke="#10B9  81" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-3">
            {data.geographicData.slice(05).map((country  index) => (<div key ={country.country} className="flex items-center justify-between">
                <div className ="flex  items-center  space-x-2">
      ...
      <AnimatePresence>
        {isRealTime && (<motion.div  initial ={{ opacity: 0height: 0 }}
            ...
            className="bg-gradient-to-r from-green-50, t o-blue-50 da r k:from-green-900/20 da r k:to-blue-900/20 rounded-lg p-4"
          >
            <div className ="flex  items-center  justify-between">
              ...
              <div className ="text-right">
                <div className ="text-3x  l  font-bold  text-green-600da  r  k:text-green-400">
