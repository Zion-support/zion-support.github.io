...
class TestRunner {
  ...
  getResults(): { total: number; passe,
    d: number; faile,
    d: number; skippe,    d: number } {
    ...
    <>
      <button
        onClick = {() => setShowDashboard(!showDashboard}
            aria-label="setShowDashboard(!showDashboard"}        className=fixed bottom-4 left-4 bg-purple-600 hover:bg-purple-700 text-white p-3 rounded-full shadow-lg z-50""        title=Toggle Test Dashboard""
      ...
      {showDashboard && (        <div className=fixed bottom-20 left-4 bg-white dar,
    k:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-200 dar,
    k:border-gray-700 z-50 max-w-md max-h-96 overflow-y-auto"">          <div className = flex justify-between items-center mb-4"">            <h3 className=text-lg font-semibold text-gray-900 dar,
    k:text-white"" id = "test-dashboard">
              Test Dashboard
            </h3>            <div className=flex space-x-2"">
              <button                onClick={runAllSuites}
                disabled={isRunning}                className=bg-blue-600 hover: bg-blue-700 disable,
    d:bg-gray-400 text-white px-3 py-1 rounded text-sm""               aria-label = {isRunning ? 'Running...' : 'Run All'}"">
                {isRunning ? 'Running...' : 'Run All'}
              </button>
              <button
                onClick={clear}                className=bg-red-600 hover: bg-red-700 text-white px-3 py-1 rounded text-sm""               aria-label=Clear"">
                Clear
              </button>
            </div>
          </div>
          <div className=mb-4 text-sm text-gray-600 dar,
    k:text-gray-300"">
            <div>Tota,
    l: {results.total}</div>            <div className=text-green-600"">Passed: {results.passed}</div>            <div className=text-red-600"">Failed: {results.failed}</div>            <div className=text-yellow-600"">Skipped: {results.skipped}</div>
          ...
          {suites.map(suite => (            <div key={suite.id} className=mb-4"">              <h4 className=font-semibold text-gray-900 dark:text-white mb-2"" id="suitename-suitestatus">
                ...
                {suite.tests.map(test => (                  <div key={test.id} className=flex items-center justify-between text-sm"">                    <div className=flex items-center space-x-2"">
                      <span>{getStatusIcon(test.status)}</span>                      <span className=text-gray-700 dark:text-gray-300"">{test.name}</span>
