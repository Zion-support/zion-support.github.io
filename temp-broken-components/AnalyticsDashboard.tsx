...
interface AnalyticsData {
  ...
  revenue: number;
  topPages: { page: string; view,    s: number }[];
  trafficSources: { source: string; visitor,    s: number }[];
  deviceTypes: { device: string; percentag,    e: number }[];
  geographicData: { country: string; visitor,    s: number }[];
}
...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ className = "" }) => {;
  ...
  const getChartData = () => {
    ...
    return {
      ...
      datasets: [{
        labe,
    l: selectedMetric = == 'pageViews' ? 'Page Views' : 
...
>>>>>>> 7f723505c7d69fdcdfb649a50c1163e3919b1408:src/components/AnalyticsDashboard.tsx
              >
                ...
                <div className=grid grid-cols-1 md:grid-cols-4 gap-4"">          <div className=text-center"">            <div className=text-3xl font-bold text-blue-600"">
              {getTotalMetric('pageViews').toLocaleString()}
            </div>            <div className=text-sm text-gray-600"">Total Page Views</div>
          </div>          <div className=text-center"">            <div className=text-3xl font-bold text-green-600"">
              {getTotalMetric('uniqueVisitors').toLocaleString()}
            </div>            <div className=text-sm text-gray-600"">Unique Visitors</div>
          </div>          <div className=text-center"">            <div className=text-3xl font-bold text-purple-600"">
              {getAverageMetric('bounceRate').toFixed(1)}%
            </div>            <div className=text-sm text-gray-600"">Avg Bounce Rate</div>
          </div>          <div className=text-center"">            <div className=text-3xl font-bold text-yellow-600"">
              ${getTotalMetric('revenue').toLocaleString()}
            </div>            <div className=text-sm text-gray-600"">Total Revenue</div>
          </div>
        </div>
      </div>
...
>>>>>>> 7f723505c7d69fdcdfb649a50c1163e3919b1408:src/components/AnalyticsDashboard.tsx
                className={`px-3 py-1 rounded-full text-sm font-medium ${
                  selectedMetric === metric
                    ...
                    : 'text-gray-500 hover:text-gray-700'                }`}
              >
                {metric === 'pageViews' ? 'Page Views' :                  metric === 'uniqueVisitors' ? 'Unique Visitors' : 'Revenue'}
              </button>
            ))}
          </div>
        </div>
        ...
        <DataVisualization
          type=""line"
          data={getChartData()}
          title={`${selectedMetric === 'pageViews' ? 'Page Views' :                    selectedMetric === 'uniqueVisitors' ? 'Unique Visitors' : 'Revenue'} Over Time`}
          height={400}
        />
      </div>
      ...
      {/* Charts Grid */}      <div className=grid grid-cols-1 lg:grid-cols-2 gap-6"">
        <DataVisualization          type=bar""
          data={getTopPagesData()}          title=Top Pages""
          height={300}
        />
        <DataVisualization          type=pie""
          data={getTrafficSourcesData()}          title=Traffic Sources""
          height={300}
        />
      </div>

      {/* Device Types and Additional Metrics */}      <div className=grid grid-cols-1 lg:grid-cols-2 gap-6"">
        <DataVisualization          type=doughnut""
          data={getDeviceTypesData()}          title=Device Types""
          height={300}
        />
                <div className=bg-white rounded-lg shadow-sm border border-gray-200 p-6"">          <h3 className=text-lg font-semibold text-gray-900 mb-4"" id="key-metrics">Key Metrics</h3>          <div className=space-y-4"">            <div className=flex justify-between items-center"">              <span className=text-sm text-gray-600"">Avg Session Duration</span>              <span className=text-sm font-medium text-gray-900"">
                {Math.round(getAverageMetric('avgSessionDuration'))}s
              </span>
            </div>            <div className=flex justify-between items-center"">              <span className=text-sm text-gray-600"">Conversion Rate</span>              <span className=text-sm font-medium text-gray-900"">
                {(getAverageMetric('conversionRate') * 100).toFixed(2)}%
              </span>
            </div>            <div className=flex justify-between items-center"">              <span className=text-sm text-gray-600"">Latest Page Views</span>              <span className=text-sm font-medium text-gray-900"">
                {getLatestMetric('pageViews').toLocaleString()}
              </span>
            </div>            <div className=flex justify-between items-center"">              <span className=text-sm text-gray-600"">Latest Unique Visitors</span>              <span className=text-sm font-medium text-gray-900"">
                {getLatestMetric('uniqueVisitors').toLocaleString()}
              </span>
            </div>            <div className=flex justify-between items-center"">              <span className=text-sm text-gray-600"">Latest Revenue</span>              <span className=text-sm font-medium text-gray-900"">
