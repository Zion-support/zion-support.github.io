#!/usr/bin/env python3
"""
Efficient bulk merge script for handling thousands of branches
"""
import subprocess
import json
import sys
...
def get_branches_batch(start: int, count: int) -> List[str]:
    """Get a batch of branches starting from start index"""
...
def merge_branch_fast(branch: str) -> bool:
    """Fast merge with minimal error handling"""
...
def main():
    print("🚀 Efficient bulk merge process...")
    ...
    for chunk_start in range(0, total_branches, 1000):
        ...
        for batch_start in range(chunk_start, min(chunk_start + 1000, total_branches), batch_size):
            batch = get_branches_batch(batch_start + 1, batch_size)
