...
resolve_conflicts() {
    ...
    if git status | grep -q "unmerged paths"; then
        ...
        if [ -n "$conflicted_files" ]; then
            ...
            # Add all resolved files (including ignored ones if necessary)
            for file in $conflicted_files; do
                if [[ "$file" == dist/* ]]; then
...
# Main processing loop - process next batch of PRs
log_message "Starting to process next batch of open PRs..."

# Extract PR information and process each one (skip first 3 that were already processed)
awk '
/"number":/ {
    number = $2
    gsub(/,/, "", number)
...
/"ref":/ && in_head {
    ...
    gsub(/,$/, "", ref)
    gsub(/"/, "", ref)
    if (ref != "ref" && ref != "href" && ref != "archive_url" && ref != "git_refs_url" && count > 3 && count <= 20) {
        printf "%s|%s\n", pr_number, ref
        in_head = 0
    }
}
...
# Final status
echo ""
log_message "Batch processing completed. Checking final status..."
git status

echo ""
log_message "Batch process complete!"
log_message "Summary:"
git log --oneline -10