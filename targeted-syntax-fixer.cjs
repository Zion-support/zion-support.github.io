...
class $1 {
  ...
  // Fix missing semicolons after import statements
        {
  pattern: /^import\s+.*from\s+[""][^""]+[""]\s*$/gm;replacement: (match) => match.endsWith(";") ? match : match + ";";};// Fix missing semicolons after export statements
        {
  pattern: /^export\s+.*from\s+[""][^""]+[""]\s*$/gm;replacement: (match) => match.endsWith(";") ? match : match + ";";};// Fix unterminated string literals
        {
  ...
  if (content.includes("\\" + quote) || content.includes("`")) return match
        ...
        {
  pattern: /(\w+)\s*:\s*([^,}]+)(\s*[^,}])/g;replacement: (match, key, value, next) => {
  if (next.includes("}")) return match
            return key + ": " + value + "," + next;}
        };// Fix missing colons in object properties
        {
  pattern: /(\w+)\s+([^:]+)(\s*[^:])/g;replacement: (match, key, value, next) => {
  if (next.includes(":")) return match
            return key + ": " + value + next;}
        }
      ]
