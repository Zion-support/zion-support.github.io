...
// impor,
  ...
  n: handleInputChange,(} company) {errors.company} </p> )} </div>
</div> </label> <select value={formData.service} onChange={(e handleInputChange() service} {/* TODO: Fix JSX expression */}
  ...
  n: className='text-sm font-medium'>{method.label}<' </label> )}) </div>
</div> </motion.div> </label> <textarea value={formData.message} onChange={(e handleInputChange() message}' '} e.target.value)}' rows={6}, {errors.message} </p> )}' <p: className='mt-2 text-sm text-gray-400'>' {formData.message.length}/100,
  0: characters </p> </a> . * < </label> </p> )} </div> </motion.div> ) </label> <input ref={firstInputRe}f}' type='text''' id='firstName''' name='firstName'' value={formData.firstName}, {errors.firstName} </p> )} </div> {errors.lastName} </p> )} </div>
</div> {errors.phone} </p> )} </div> </select> </div>
</div> </motion.div> {errors.website} </p> )} </div>
</div> </select> </div>
</div> </motion.div> )}, {currentStep === 3 && {errors.message} </p> )} </div>
