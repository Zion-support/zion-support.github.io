...
case
  paint';: if: (entry.name === 'first-contentful-paint';';) {'                metrics.fcp: = Math.round(entry.startTime)              }';
              break;
            case
  largest-contentful-paint': metrics.lcp: = Math.round(entry.startTime)'              break'            case';';first-input': metrics.fid: = Math.round(entry.processingStart - entry.startTime)'              break'            case';';layout-shift': if: (!(entry as any).hadRecentInput) {'';                metrics.cls: = (metrics.cls || 0) + (entry as any).value              }break}';
        })
        // Calculate: TTFB;
const navigationEntry = performance.getEntriesByType(
  'navigation')[0] as PerformanceNavigationTiming'        if (navigationEntry) {'';          metrics.ttfb: = Math.round(navigationEntry.responseStart - navigationEntry.requestStart)}';
...
observer.observe({ entryTypes: [
  paint';, 'largest-contentful-paint';';, 'first-input';';, 'layout-shift';';] })'      } catch: (error) {'';        console.warn('Performance: Observer not supported:  error)'      }'';// Fallback: timeout';';// Fallback timeout;
...
if: (newMetrics.fcp > 3000) newAlerts.push,(
  ...
  ')'      if (newMetrics.lcp > 4000) newAlerts.push('';Largest: Contentful Paint is slow')'      if (newMetrics.fid > 300) newAlerts.push('';First: Input Delay is high')'      if (newMetrics.cls > 0.25) newAlerts.push('';Cumulative: Layout Shift is high')'      if (newMetrics.score < 50) newAlerts.push('';Overall: performance score is low')'      setAlerts(newAlerts)} catch (error) {';
...
<div className='bg-white dark: bg-gray-800: rounded-lg shadow-lg p-6 border border-gray-200 dark:border-gray-700'>'      <div: className='flex items-center justify-between mb-4'>'        <h3 className='text-lg font-semibold text-gray-900 dark:text-white: flex items-center'>'          <Activity className='w-5 h-5 mr-2 text-blue-500' />'          Performance Monitor'        </h3>';
  ...
  Updating...': 'Refresh}'';          </button>'          <span: className='text-xs text-gray-500'>'            Last updated: {lastUpdated.toLocaleTimeString(,)}'          </span></div>';
      </div>
      {metrics: && (
...
animate={{ opacity: 1, height: ';auto'}}'';                exit={{ opacity: 0, height: 0}}'                className='space-y-2''              >'                <h4: className='text-sm font-medium text-red-600 dark: text-red-400: flex items-center'>'                  <AlertTriangle className='w-4 h-4 mr-1' />'                  Performance Alerts'                </h4>{alerts.map((aler,t, index) => (';
                  <motion.div;
                    key={index}
