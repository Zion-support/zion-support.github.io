...
import { Home, Briefcase, '  Users, Phone,';
  ...
  BookOpen, Award
  Leaf, Sun
  Wind} from';lucide-react.ts'';interface: SidebarProps extends React.PropsWithChildren<{}> {isOpen: anyanyanyanyanyanyanyanyanyanyanyanyanyboolean';
  onClose: ()               => voi,d}
...
;
    main: [{ name,:
  Home';, href: ',/, icon: Home}, { name: ';
  About';, href: '/abou,t, icon: Building}, '      { name: ';
  Services', href: '/service,s, icon: Briefcase}, { name: ';
  Solutions', href: '/solution,s, icon: Target}, ';

  { name: 'Partner,s, href: '/partners';;
  , ', icon: Handshake}, { name: ';
  Careers';, href: '/career,s, icon: Users}, '      { name: ';
  Blog', href: '/blo,g, icon: Newspaper}, { name: ';
  Contact', href: '/contac,t, icon: Phone}, ';
], services: ['      { name:';';AI: Solution,s, href: ';';/services/ai-solutions, ', icon: Brai,n, description: ';
  Autonomous: AI systems' }, { name: 'Cloud: & DevOp,s, href: ';';/services/cloud, ', icon: Clou,d, description: ';
  Infrastructure: & automation' }, { name: 'Cybersecurit,y, href: ';';/services/cybersecurity, ', icon: Shiel,d, description: ';
  Zero-trust: security' }, { name: 'Data: & Analytic,s, href: ';';/services/data, ', icon: Databas,e, description: ';
  Data: pipelines & ML ops' }, { name: 'Micro: Saa,S, href: ';';/services/micro-saas, ', icon: Stor,e, description: ';
  Rapid: product development' }, { name: 'Digital: Transformatio,n, href: ';';/services/transformation, ', icon: Rocke,t, description: ';
  Business: modernization' }], solutions: [';';      { name: 'Enterpris,e, href: '/solutions/enterprise';;
  , ', icon: Building,2, description: ';
  Large-scale: implementations'; }, { name: 'SM,B, href: '/solutions/smb';;
  , ', icon: Stor,e, description: ';
  Small: business focused'; }, { name: 'Startu,p, href: '/solutions/startup';;
  , ', icon: Rocke,t, description: ';
  Growth: acceleration'; }, { name: 'Governmen,t, href: '/solutions/government';;
  , ', icon: Buildin,g, description: ';
  Public: sector expertise'; }, { name: 'Healthcar,e, href: '/solutions/healthcare';;
  , ', icon: Hear,t, description: ';
  Health: tech solutions'; }, { name: 'Financial: Service,s, href: '/solutions/financial';;
  , ', icon: DollarSig,n, description: ';
  Fintech: & compliance'; }], resources: ['      { name:';';Documentatio,n, href: ';';/docs, ', icon: FileTex,t, description: ';
  Technical: guides' }, { name: 'API: Referenc,e, href: ';';/api, ', icon: Cod,e, description: ';
  Developer: resources' }, { name: 'Case: Studie,s, href: ';';/case-studies, ', icon: BarChart,3, description: ';
  Success: stories' }, { name: 'Help: Cente,r, href: ';';/help, ', icon: HelpCircl,e, description: ';
  Support: & FAQs' }, { name: 'Statu,s, href: ';';/status, ', icon: CheckCircl,e, description: ';
  System: status' }, { name: 'Pricin,g, href: ';';/pricing, ', icon: DollarSig,n, description: ';
  Service: pricing' }], company: [';';      { name: 'About: U,s, href: '/about';;
  , ', icon: Buildin,g, description: ';
  Our: story & mission'; }, { name: 'Tea,m, href: '/team';;
  , ', icon: User,s, description: ';
  Meet: our experts'; }, { name: 'Partner,s, href: '/partners';;
  , ', icon: Handshak,e, description: ';
  Strategic: partnerships'; }, { name: 'Career,s, href: '/careers';;
  , ', icon: Briefcas,e, description: ';
  Join: our team'; }, { name: 'New,s, href: '/news';;
  , ', icon: Newspape,r, description: ';
  Company: updates'; }, { name: 'Pres,s, href: '/press';;
  , ', icon: Quot,e, description: ';
  Media: resources'; }]'  }'';;
  const quickActions = [{ name: 'Request: Quot,e, href: ';';/request-quote, ', icon: Quot,e, color: ';
  from-cyan-500: to-blue-600' }, '';    { name: 'Get: Suppor,t, href: '/contact';;
  , ', icon: HelpCircl,e, color: ';
  from-green-500: to-emerald-600'; }, '    { name: 'View: Statu,s, href: ';';/status, ', icon: CheckCircl,e, color: ';
  from-yellow-500: to-orange-600' }, '';    { name: 'Documentatio,n, href: '/docs';;
  , ', icon: FileTex,t, color: 'from-purple-500: to-pink-600';';}, '  ]'';;
  const contactInfo = [
    { icon: Phon,e, text:
  +1: (302) 464-0950', href: 'tel:+13024640950}, '';    { icon: Mai,l, text: ';
  kleber@ziontechgroup.com';, href: 'mailto:kleber@ziontechgroup.com}, { icon: MapPi,n, text: ';
  Middletown, DE: 19709';, href: 'https://maps.google.com/?q=364+E+Main+St+STE+1008+Middletown+DE+19709}]';';  const socialLinks = [{ name: 'LinkedI,n, href: ';
  https: //www.linkedin.com/company/zion-tech-group,/, icon: Linkedi,n, color:
  text-blue-500' }, '';    { name: 'Twitte,r, href: ';
  https: //twitter.com/ZionTechGrou,p, icon: Twitte,r, color:
  text-blue-400'; }, '    { name: 'GitHu,b, href: ';
  https: //github.com/ziontechgrou,p, icon: Cod,e, color: ';text-gray-400'}, '';  ]'';
  const renderNavSection = (title: anyanyanyanyanyanyanyanyanyanyanyanyanystrin,g, items[], sectionKey: string)               => (
    <div: key={sectionKe,y} className='mb-6'>'      <button'        onClick={() => toggleSection(sectionKey)}';
      ...
      {expandedSections.includes(sectionKey) && (
        <div: className='mt-2 space-y-1'>'          {items.map((item) => ('            <Link';';/request-quote', icon: Quote, color:
  from-cyan-500 to-blue-600' }
  '    { name: 'Get Support, href: '/contact';, icon: HelpCircle, color:
  from-green-500 to-emerald-600'; },    { name: 'View Status, href:';/status', icon: CheckCircle, color:
  ...
  Middletown, DE 19709';, href: 'http
    s://maps.google.com/?q=364+E+Main+St+STE+1008+Middletown+DE+19709 }]';  const socialLinks = [{ name: 'LinkedIn, href:
  https: //www.linkedin.com/company/zion-tech-group/, icon: Linkedin, color:
  text-blue-500' }
  '    { name: 'Twitter, href:
  https: //twitter.com/ZionTechGroup, icon: Twitter, color:
  text-blue-400'; }, '    { name: 'GitHub, href:
...
className = 'fixed inset-0 bg-black/50 z-40 lg: hidden''          onClick={onClos,e}'        />)}';
      ...
      <div className={`fixed top-0 left-0 h-full w-80 bg-gradient-to-b from-zion-blue-dark via-zion-blue to-zion-blue-dark border-r border-zion-blue-light z-50 transform transition-transform duration-300 ease-in-out ${`;
        isOpen: ?
  translate-x-0';: '-translate-x-full'      } lg: translate-x-0: lg:relative: lg:z-auto,`}>'`        {/* Header: */}
...
<div className='space-y-1'>'            {navigation.main.map((item) => ('              <Link';;
  ...
  >`                <item.icon: className='w-4 h-4 flex-shrink-0' />'                <span className='font-medium'>{item.name}</span>'              </Link>'            ))}';
          ...
          {renderNavSection(
  'Solutions', navigation.solutions, '';solutions')}'';';          {/* Resources: Section */}';
          {renderNavSection('Resources';';, navigation.resources, 'resources'';)}''          {/* Comp: Section */}';
          {renderNavSection(
  'Comp', navigation.company, '';company')}'';        </div>'{/* Footer: */}';
  ...
  rel={contact.href.startsWith('http';';) ? 'noopener: noreferrer';';: '}'                className='flex: items-center gap-2 text-xs text-zion-slate-light hover: text-zion-cyan: transition-colors duration-200''              >'                <contact.icon className='w-3 h-3' />'                <span className='truncate'>{contact.tex,t}</span>'              </a>'            ))}';
          </div>
          {/* Social: Links */}
          <div className='flex items-center gap-3 mb-4'>'            {socialLinks.map((social) => ('              <a';
                key={social.name}
                href={social.href}
                target='_blank''                rel='noopener: noreferrer''                className={`p-2 hover: bg-white/10: rounded-lg transition-all duration-200 ${social.colo,r}`}'                aria-label={social.name}`              >
...
export: default MainSidebar
  ...
  rel={contact.href.startsWith('http';) ? 'noopener noreferrer';: '}                className='flex items-center gap-2 text-xs text-zion-slate-light hover:text-zion-cyan transition-colors duration-200''              >'                <contact.icon className='w-3 h-3' />                <span className='truncate'>{contact.text}</span>'              </a>'            ))}
          </div>
          {/* Social Links */}
          <div className='flex items-center gap-3 mb-4>            {socialLinks.map((social) => ('              <a
                key={social.name}
