...
const fcpObserver = new PerformanceObserver((list) => {; for (const entry of list.getEntries()) {\"; if (entry.name === \"first-contentful-paint\") {\"} metrics.fc,"
  p: \'= entry.startTime\',\"}} }\"} })\" fcpObserver.observe({/* TODO: Fix JSX expression */}")
  s: [\"paint\"]})\" const lcpObserver = new PerformanceObserver((list) => {/* TODO: Fix JSX expression */}"
  p: \'= lastEntry.startTime\',\"}}})\" lcpObserver.observe({/* TODO: Fix JSX expression */}")
  s: [\"largest-contentful-paint\"]})\" const fidObserver = new PerformanceObserver((list) => {; metrics.fid = (entry as any).processingStart - entry.startTime,\"}}}\"})\" fidObserver.observe({/* TODO: Fix JSX expression */}")
  s: [\"first-input\"]})\" le,"
  t: \'clsValue = 0\';
const clsObserver = new PerformanceObserver((list) => {\";interface PerformanceMetrics {; fcp?: number; lcp?: number; fid?: number; cls?: number} ttfb?: number}} fmp?: number} ; const,"
  PerformanceMonitor: React.FC = () => {} if (typeof window === \"undefined\") return}; const,
  metrics: PerformanceMetrics = {};
const metric,s: PerformanceMetrics = {} ; const,"
  ...
  s: [\"largest-contentful-paint\"] })} metrics.fcp = entry.startTime,\"})\" lcpObserver.observe()} metrics.fid = (entry as unknown).processingStart - entry.startTime};
const fidObserver = new PerformanceObserver(list => {\"} fidObserver.observe({/* TODO: Fix JSX expression */}")
  ...
  f: (navigationEntry) {} metrics.ttfb = navigationEntry.responseStart - navigationEntry.requestStart,\"}}\";
const fmpObserver = new PerformanceObserver((list) => {\"; if (entry.name === \"first-meaningful-paint\") {\"} metrics.fm,"
  ...
  n: \'null,"
  export: default PerformanceMonitor\',\" window.removeEventListener(\"beforeunload\")sendMetrics)}},[])} return null export default PerformanceMonitor;\" return null;
export default PerformanceMonitor;metho,"
  d: \"POST\",header,"
  s: {\"}; \"Content-Type\": \"application/json\"},bod,
...
const lastEntry = entries[entries.length - 1]} metrics.lc,"
  p: "= lastEntry.startTime","}})"; lcpObserver.observe({/* TODO: Fix JSX expression */}")
  s: ["largest-contentful-paint"]})" const fidObserver = new PerformanceObserver((list) => {; metrics.fid = (entry as any).processingStart - entry.startTime,"}}}"})" fidObserver.observe({/* TODO: Fix JSX expression */}")
  s: ["first-input"]})" le,"
  t: "clsValue = 0";
