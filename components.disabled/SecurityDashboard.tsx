...
const mockData: SecurityMetrics = {
      overallScor,
    e: Math.floor(Math.random() * 20) + 80, vulnerabilities: {
        critica,
    l: Math.floor(Math.random() * 2), high: Math.floor(Math.random() * 5), medium: Math.floor(Math.random() * 8), low: Math.floor(Math.random() * 15)}, lastScan: new Date()}
    setTimeout(() => {
...
className='bg-white dark: bg-gray-800 p-6 rounded-lg shadow border border-gray-200 dar,
    ...
    d:grid-cols-4 gap-4>        {Object.entries(security.vulnerabilities).map(([type, count]) => ('          <div key={type} className='bg-white dark: bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dar,
    k:border-gray-700 text-center>            <div className='text-3xl font-bold text-red-500'>{count}</div>'            <div className='text-sm text-gray-600 dark:text-gray-400 capitalize>              {type} Vulnerabilities"            </div></div>
        ))}
      </div>
    </div>
  )}
