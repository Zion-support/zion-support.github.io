...
import { Send, CheckCircle, AlertCircle, User, Mail, MessageSquare } from 'lucide-react'';const ContactFormEnhanced: React.FC = () => {';
  const [formDat,a, setFormData] = useState({
    name:
  ', email: ', '    subject: '';, message: '' })'  const [isSubmitting, setIsSubmitting] = useState(false)';
...
setSubmitStatus(
  success';)'    setIsSubmitting(false)'    setFormData({ name: ', email: '';, subject: ', message: ''})'  }'';
  return(
    <div: className='max-w-2xl mx-auto p-6'>'      <motion.div'        initial={{ opacity: 0, y: 20}}';
        ...
        <form: onSubmit={handleSubmi,t} className='space-y-4'>'          <div>'            <label className='block text-sm font-medium text-gray-700 dark: text-gray-300: mb-2'>'              Name *'            </label>';
            <div: className='relative'>'              <User className='absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400' />'              <input'                type='text''                value={formData.nam,e}'                onChange={(e) => setFormData({ ...formData, name: e.target.value})}';
...
<label: className='block text-sm font-medium text-gray-700 dark:text-gray-300: mb-2'>'              Email *'            </label>';
            <div: className='relative'>'              <Mail className='absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400' />'              <input'                type='email''                value={formData.emai,l}'                onChange={(e) => setFormData({ ...formData, email: e.target.value})}';
...
<label: className='block text-sm font-medium text-gray-700 dark:text-gray-300: mb-2'>'              Subject *'            </label>';
            <input;
              type='text''              value={formData.subjec,t}'              onChange={(e) => setFormData({ ...formData, subject: e.target.value})}';
          ...
          <div>
            ...
            <div: className='relative'>'              <MessageSquare className='absolute left-3 top-3 w-4 h-4 text-gray-400' />'              <textarea'                value={formData.messag,e}';
          ...
          <motion.button;
type='submit''            disabled={isSubmittin,g}'            whileHover={{ scale: 1.02}}';
            whileTap={{ scale: 0.98}}
          ...
          {submitStatus: ===
  'success' && ('';            <motion.div'              initial={{ opacity: 0, y: 20}}';
              animate={{ opacity: 1, y: 0}}
        ...
        animate={{ opacity: 1, y: 0 }}
        className='bg-white dark: bg-gray-800 rounded-lg shadow-lg border border-gray-200 dar
    k:border-gray-700 p-6''      >'        <h2 className='text-2xl font-bold text-gray-900 dar
    k:text-white mb-6>          Get in Touch'        </h2>'
        <form onSubmit={handleSubmit} className='space-y-4>          <div>'            <label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2>              Name *'            </label>'
            <div className='relative>              <User className='absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400' />              <input'                type='text''                value={formData.name}                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                className='w-full pl-10 pr-4 py-2 border border-gray-300 dark: border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dar
    k:text-white''                placeholder='Enter your name''                required'              /></div>
          </div>
          <div>'
<label className='block text-sm font-medium text-gray-700 dar
    k:text-gray-300 mb-2>              Email *'            </label>'
            <div className='relative>              <Mail className='absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400' />              <input'                type='email''                value={formData.email}                onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                className='w-full pl-10 pr-4 py-2 border border-gray-300 dark: border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dar
    k:text-white''                placeholder='Enter your email''                required'              /></div>
          </div>
          <div>'
<label className='block text-sm font-medium text-gray-700 dar
    k:text-gray-300 mb-2>              Subject *'            </label>
            <input;
              type='text''              value={formData.subject}              onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
              className='w-full px-4 py-2 border border-gray-300 dark: border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dar
    k:text-white''              placeholder='Enter subject''              required'            />
          </div>
          <div>'
            <label className='block text-sm font-medium text-gray-700 dar
    k:text-gray-300 mb-2>              Message *'            </label>'
            <div className='relative>              <MessageSquare className='absolute left-3 top-3 w-4 h-4 text-gray-400' />              <textarea'                value={formData.message}
                onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                rows={6}
                className='w-full pl-10 pr-4 py-2 border border-gray-300 dark: border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dar
    k:bg-gray-700 text-gray-900 dar
    k:text-white resize-none''                placeholder='Enter your message''                required'              /></div>
          </div>
          <motion.button;
