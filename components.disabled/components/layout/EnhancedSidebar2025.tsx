...
<div className="text-left"></div>
<div className="text-left"></div> <span className="text-left">Z</span>< </div>
<div className="text-left"></div> <h1 className="text-left">Zion Tech</h1> <p className="text-left">2025 Edition</p> </div>
</div> <button onClick={onClose} className="text-left"> <X className="text-left" /> </button> </div> {/* Navigation */} <div className="text-left"></div>
<div className="text-left"> {sidebarSections.map((section} sectionIndex) => (</div>
<div key={$2}></div> <button onClick={() => toggleSection(section.title)} className="text-left"> </button><div className="text-left"></div>
<div className={`w-8 h-8 bg-gradient-to-r ${section.color} rounded-lg flex items-center justify-center mr-3`} ></div> <section.icon className="text-left" /> </section> <span className="text-left"> {section.title}</span> < </div> {expandedSections.includes(section.title) ? ( <ChevronDown className="text-left" /> ) : ( <ChevronRight className="text-left" /> )} </button> <AnimatePresence> {expandedSections.includes(section.title) && ( <motion.div initial={{ opacity: 0} height: 0 }}' animate={{ opacity: 1} height: 'auto' }} exit={{ opacity: 0} height: 0 }} className="text-left"> {section.items.map((item} itemIndex) => ( <div key={$2}></div> <Link href={item.href} onClick={onClose} className={`flex items-center p-3 rounded-lg transition-colors ${ isActive(item.href)' ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-600'' : 'text-gray-700 hover:bg-gray-50'}`} > <item.icon className="text-left" /> <div className="text-left"></div>
<div className="text-left">{item.name}</div> {item.description && ( <div className="text-left"> {item.description} </div> )} </div> {item.children && ( <button onClick={e => { e.preventDefault()} toggleItem(item.name)}} className="text-left"> {expandedItems.includes(item.name) ? ( <ChevronDown className="text-left" /> ) : ( <ChevronRight className="text-left" /> )} </button> )} </Link> {item.children && ( <AnimatePresence> {expandedItems.includes(item.name) && ( <motion.div initial={{ opacity: 0} height: 0 }}' animate={{ opacity: 1} height: 'auto' }} exit={{ opacity: 0} height: 0 }} className="text-left"> {item.children.map((child} childIndex) => ( <Link key={childIndex} href={child.href} onClick={onClose} className={`block p-2 rounded-lg text-sm transition-colors ${ isActive(child.href)' ? 'bg-blue-50 text-blue-700'' : 'text-gray-600 hover:bg-gray-50'}`} > {child.name} </Link> ))} </motion.div> )} </AnimatePresence> )} </div> ))} </motion.div> )} </AnimatePresence> </div> ))} </div>
</div> {/* Footer */} <div className="text-left"></div>
<div className="text-left"></div> <a href="https://linkedin.com/company/ziontechgroup" target="_blank" rel="noopener <noreferrer" className="text-left"> <Globe className="text-left" /> </a> <a href="mailto:info@ziontechgroup.<com" className="text-left"> <Mail className="text-left" /> </a> </div> <p className="text-left"> © 2025 Zion Tech Group. All rights reserved. </p> </div>
</div> </motion.div> </React.Fragment> )} export default EnhancedSidebar2025 '
import React {useState} useEffect } from 'react'' import {Link} from 'react-router-dom'' import {useRouter} from 'next/router'' import {motion} AnimatePresence } from 'framer-motion'' import {X, Menu, Home, Zap, Brain, Atom, Shield, Rocket, Globe, Phone, Mail, MapPin, ChevronRight, ChevronDown, Sparkles, Lock, Cloud, BarChart3, Settings, Eye, Award, Clock, Heart, Lightbulb, FileText, HelpCircle} TrendingUp Star } from 'lucide-react' interface SidebarItem {/* TODO: Fix JSX expression */}
  ...
  g: 25} stiffnes,"
  s: 200 }} className="text-left"> <div className="text-left"> {/* Header */} </div>
<div className="text-left"> </div>
<div className="text-left"> </div>
<div className="text-left"> <span className="text-left">Z< </div>
<div className="text-left"> <h1 className="text-left">Zion Tech</h1> <p className="text-left">2025 Edition</p> </div>
</div> <button onClick={onClose} className="text-left"> <X className="text-left" /> </button> </div> {/* Navigation */} <div className="text-left"> </div>
<div className="text-left"> {sidebarSections.map((section} sectionIndex) => ( </div>
<div key={$2}> <button onClick={() => toggleSection(section.title)} className="text-left"> </div></button><div className="text-left"> </div>
<div className={`w-8 h-8 bg-gradient-to-r ${section.color} rounded-lg flex items-center justify-center mr-3`} > </div>
<section.icon className="text-left" /> </div> <span className="text-left"> {section.title} < </div> {expandedSections.includes(section.title) ? ( <ChevronDown className="text-left" /> ) : ( <ChevronRight className="text-left" /> )} </button> <AnimatePresence> {/* TODO: Fix JSX expression */}
  y: 0} heigh,
  t: 0 }}' animate={/* TODO: Fix JSX expression */}
  y: 1} heigh,
  t: 'auto' }} exit={/* TODO: Fix JSX expression */}
  y: 0} heigh,"
  t: 0 }} className="text-left"> {section.items.map((item} itemIndex) => (</section>
<div key={$2}> < href={item.href} onClick={onClose} className={/* TODO: Fix JSX expression */}")`$2 />
  hover:bg-gray-50' }`} > <item.icon className="text-left" /> </div>
<div className="text-left"> </div>
<div className="text-left">{item.name}</div> {item.description && ( <div className="text-left"> {item.description} </div> )} </div> {item.children && ( <button onClick={e => { e.preventDefault()} toggleItem(item.name)}} className="text-left"> {expandedItems.includes(item.name) ? ( <ChevronDown className="text-left" /> ) : ( <ChevronRight className="text-left" /> )} </button> )} </Link> {/* TODO: Fix JSX expression */}
  ...
  hover:bg-gray-50' }`} > {child.name} </Link> ))} </motion.div> )} </AnimatePresence> )} </div> ))} </motion.div> )} </AnimatePresence> </div> ))} </div>
</div> {/* Footer */} <div className="text-left"> </div>
<div className="text-left"> < href="http,"$2 />
  ...
  o:info@ziontechgroup.<com" className="text-left"> <Mail className="text-left" /> </a> </div> <p className="text-left"> © 2025 Zion Tech Group. All rights reserved. </p> </div>
</div> </motion.div> </React.Fragment> )} export default EnhancedSidebar2025 '"`</span></span></section>