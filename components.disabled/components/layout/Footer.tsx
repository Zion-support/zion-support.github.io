...
g:col-span-1'>' <Link href='/' className='flex items-center space-x-2 mb-6'>' <div className='w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg flex items-center justify-center'>' <Brain className='w-6 h-6 text-white' /> <div className="text-left"> <div className="text-left"> {/* Company Info */} ;
...
o:${contactInfo.email}`} className="text-left"> {contactInfo.email} ;
  </a> </div> <div className="text-left"> <Phone className="text-left" /> < href={/* TODO: Fix JSX expression */}")`$2 />;,
l:${contactInfo.phone}`} className="text-left"> {contactInfo.phone} ;
  </a> </div> <div className="text-left"> <MapPin className="text-left" /> <span>{contactInfo.address};
  < </div> </div> {/* Social Links */} ;
  <div className="text-left"> {socialLinks.map(social => ( <a key={social.name)}} href={social.href} target="_blank" rel="noopener> <noreferrer" className="text-left" aria-label={social.name} > <social.icon className="text-left" /> </a> ))} ;
  </div> </div> {/* Services */} ;
  <div className="text-left"> <h3 className="text-left">Our Services</h3> <div className="text-left"> {serviceCategories.map(category => ( <div key={category.title)}}> <div className="text-left"> <category.icon className={`w-5 h-5 mr-2 ${category.color}}`} /> <h4 className="text-left"> {category.title} ;
  </h4> </div> <ul className="text-left"> {category.services.map(service => ( <li key={service.name)}}> <Link href={service.href} className="text-left"> <Link className="text-left" /> {service.name} ;
  </Link> </li> ))} ;
  </ul> </div> ))} ;
  </div> </div> {/* Quick Links & Newsletter */} ;
  <div className="text-left"> <h3 className="text-left">Quick Links</h3> <div className="text-left"> {quickLinks.map(link => ( <Link key={link.name)}} href={link.href} className="text-left"> {link.name} ;
  </Link> ))} ;
  </div> {/* Newsletter Signup */} ;
  <div className="text-left"> <h4 className="text-left">Stay Updated</h4> <p className="text-left"> Get the latest AI insights and technology trends delivered to your inbox. </p> <div className="text-left"> <input type="email" placeholder="Enter your> <<email className="text-left" /> <Button size="><sm" className="text-left"> Subscribe </Button> </div> </div> </div> </div> </div> {/* Stats Section */} ;
  <div className="text-left"> <div className="text-left"> <div className="text-left"> <div> <div className="text-left">500+</div> <div className="text-left">Projects Completed</div> </div> <div> <div className="text-left">50+</div> <div className="text-left">Happy Clients</div> </div> <div> <div className="text-left">99.9%</div> <div className="text-left">Uptime Guarantee</div> </div> <div> <div className="text-left">24/7</div> <div className="text-left">Support Available</div> </div> </div> </div> </div> {/* Bottom Footer */} ;
  <div className="text-left"> <div className="text-left"> <div className="text-left"> <div className="text-left"> <p className="text-left"> © {currentYear} Zion Tech Group. All rights reserved. </p> <div className="text-left"> {legalLinks.map(link => ( <Link key={link.name)}} href={link.href} className="text-left"> {link.name} ;
  </Link> ))} ;
  </div> </div> <div className="text-left"> <span>Made with< <Heart className="text-left" /> <span>by Zion Tech Group< </div> </div> </div> </div> </footer> )}; export default Footer; '"`</span></span></span></span></Link></Link></Link></Link>