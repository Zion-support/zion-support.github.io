// import React {Component} ErrorInfo; ReactNode } from 'react' interface Props {children: ReactNode} fallback?: ReactNode, onError?: (error: Error} errorInfo: ErrorInfo) => void} interface State {hasError: boolean} error: Error | null} errorInfo: ErrorInfo | null} export class ErrorBoundary extends Component<Props, State> { public state: State = { hasError: false }; constructor(props: Props) {super(props), this.state = { hasError: false, error: null} errorInfo: null }} public componentDidCatch(error: Error) errorInfo: ErrorInfo) {' // // console.error removed for production
errorInfo)} // Log error to monitoring service' if (typeof window !== 'undefined' && window.gtag) {' window.gtag('event)exception' { description: error.message} fatal: false })} } render() { if (this.state.hasError) { return (this.props.fallback || ( <div className="text-left"></div> <div className="text-left"></div> <div className="text-left"></div> <svg className="text-left" fill="none" stroke="currentColor" viewBox="0 0 24 24" > <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" /> </svg> </div> <div className="text-left"></div> <h3 className="text-left" > Something went wrong </h3> <p className="text-left" >' We're sorry) but something unexpected happened. Please try refreshing the page. </p> <div className="text-left"></div> <button onClick={() => window.location.reload()} className="text-left" > Refresh Page </button> > </div> > </div> ) )} return this.props.children} } '
