...
declare global {function gtag(...args: unknown[]): void;
...
}const Analytics: React.FC<AnalyticsProps> = ({ trackingId }) => {useEffect(() => {if (typeof window === 'undefined' || !trackingId) return;
    ...
    document.head.appendChild(script2)// Track page views on route changes;
    const handleRouteChange = () => {if (typeof gtag !== 'undefined') {gtag('config', trackingId, {page_title: document.title,page_location: window.location.href,})}
    }// Listen for route changes (Next.js)window.addEventListener('popstate', handleRouteChange)return () => {window.removeEventListener('popstate', handleRouteChange)}}, [trackingId])// Track page performance;
  useEffect(() => {if (typeof window === 'undefined') return;
    const trackPerformance = () => {if (typeof gtag !== 'undefined' && 'performance' in window) {const perfData = performance.getEntriesByType('navigation';
...
}export const trackPageView = (url: string, title: string) => {if (typeof gtag !== 'undefined') {gtag('config', process.env['NEXT_PUBLIC_GA_TRACKING_ID'] || '', {page_title: title,page_location: url,})}
