<<<<<<< HEAD
'use client"
import React from 'react"
import Head from 'next/head"
=======
'use client";"
import React from 'react";"'
import Head from next/head"
>>>>>>> origin/main
export default function ServicePage() {
<<<<<<< HEAD

  return (
    <>
      <Head>
        <title>Ai Content Generator|Zion Tech Group</title>
        <meta name="description" content="Professional services and solutions for modern businesses." />
        <meta name="robots" content="index, follow" />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Ai Content Generator|Zion Tech Group" />
        <meta property="og:description" content="Professional services and solutions for modern businesses." />
      </Head>
      
=======
  return (
    <>
      <Head>
<<<<<<< HEAD
        <title>Ai Content Generator | Zion Tech Group</title>
        <meta name="description" content="Professional ai content generator services and solutions for modern businesses." />
        <meta name="robots" content="index, follow" />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Ai Content Generator | Zion Tech Group" />
        <meta property="og:description" content="Professional ai content generator services and solutions for modern businesses." /></Head>
>>>>>>> origin/main
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
            Ai Content Generator</h1>
          <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">
            Professional ai content generator services and solutions for modern businesses.</p>
=======
        <title>"Ai Content Generator | Zion Tech Group"</title>
        <meta name="description" content="Professional ai content generator services and solutions for modern businesses." />
        <meta name="robots" content="index, follow /">
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Ai Content Generator | Zion Tech Group /">
        <meta property="og:description" content="Professional ai content generator services and solutions for modern businesses." />
      </Head>
      <div className="min-h-screen" bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20>
        <div className="max-w-7xl" mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center>
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
            "Ai Content Generator"
          </h1>
          <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">"
            Professional ai content generator services and solutions for modern businesses.
          </p>
>>>>>>> origin/main
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link"
              href="/contact"
<<<<<<< HEAD
              className="inline-flex items-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-all duration-300 hover:scale-105
            >
              Get Started"
            </Link><Link"
              href="/ai-services"
              className="inline-flex items-center px-8 py-3 border border-white text-base font-medium rounded-md text-white bg-transparent hover:bg-white hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white transition-all duration-300 hover:scale-105
            >
              Learn More
=======
              className="inline-flex items-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-all duration-300 hover:scale-105"
<<<<<<< HEAD
            >Get Started</Link>
            <Link"
=======
            >
              Get Started"
            </Link>
            <Link
>>>>>>> origin/main
              href="/ai-services"
              className="inline-flex items-center px-8 py-3 border border-white text-base font-medium rounded-md text-white bg-transparent hover:bg-white hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white transition-all duration-300 hover:scale-105
            ">
              "Learn More"
>>>>>>> origin/main
            </Link>
          </div>
        </div>
      </div>
    </div>
<<<<<<< HEAD
  )
<<<<<<< HEAD
}"
"
=======
    </>)"
}
"'
=======
  ");"

    </>
  );"}
}"
"'"
>>>>>>> origin/main
>>>>>>> origin/main
