...
export function useIntersectionObserver({;
  ...
  useEffect(() => {;
    if (prevNode.current) {;
      observer.current?.unobserve(prevNode.current);interface UseIntersectionObserverProps {
  threshold?: number;
  root?: Element | null;
  rootMargin?: string;
  freezeOnceVisible?: boolean}export function useIntersectionObserver({
  threshold = 0,
  root = null,
  rootMargin = '0%',
  freezeOnceVisible = false
  ...
  const prevNode = useRef<Element | null>(null);  useEffect(() => {    if (prevNode.current) {
