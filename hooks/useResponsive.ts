...
const useResponsive = (): ResponsiveState => {;
  ...
  useEffect(() => {
    const updateDimensions = () => {;const width = window.innerWidth;
...
screenWidth: width, screenHeight: height,;
    ...
    updateDimensions();
    window.addEventListener('resize', updateDimensions);'    '    return () => window.removeEventListener('resize', updateDimensions);'  }, []);'return state;
};
