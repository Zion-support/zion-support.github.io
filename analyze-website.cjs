...
class WebsiteAnalyzer {
  ...
  async checkUrl(url, parentUrl = null) {
    return new Promise(resolve => {
      ...
      const req = client.request(
        ...
        res => {
          ...
          if (res.statusCode >= 200 && res.statusCode < 400) {
            ...
            console.log(
              `✗ ${res.statusCode} - ${url} (from ${parentUrl || 'direct'})`
            );
          }
      ...
      req.on('error', error => {
        ...
        this.brokenLinks.push(result);
        console.log(
          `✗ ERROR - ${url}: ${error.message} (from ${parentUrl || 'direct'})`
        );
        resolve(result);
      });
  ...
  generateReport() {
    ...
    const successRate =
      totalLinks > 0 ? ((workingCount / totalLinks) * 100).toFixed(2) : 0;
    ...
    if (this.errors.length > 0) {
      this.report.recommendations.push(
        'Review and fix pages that failed to load'
      );
    }
    ...
    try {
      ...
      if (report.brokenLinks.length > 0) {
        ...
        report.brokenLinks.forEach(link => {
          console.log(
            `  - ${link.url} (${link.status}) ${link.parentUrl ? `from ${link.parentUrl}` : ''}`
          );
        });
      }
