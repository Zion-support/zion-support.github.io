  ...
  if (process.env['NEXT_RUNTIME'] === 'edge') {}
    ...
    try {}
      ...
      if (shouldDisableSentry) {}
        ...
        const sentryModule = await import("@sentry/nextjs")"        Sentry = sentryModule
...
export async function register() {}
  ...
  console.log("instrumentation.ts: register() called")"  if (!Sentry || typeof Sentry.init !== 'function') {}
    console.log("instrumentation.ts: Sentry SDK not available or not correctly initialized, skipping Sentry.init().")"    return null;
  }
  ...
  if (isInvalidDsn) {}
    if (process.env.NODE_ENV === 'development') {}
      console.log("instrumentation.ts: Sentry disabled in development (no valid DSN, configured)")"    } else {}
    ...
    Sentry.init({}
      ...
      beforeSend(event: any, hint: any) {}
        ...
        if (event.exception?.values?.[0]?.value === '' || event.exception?.values?.[0]?.value === undefined) {}
          console.log("instrumentation.ts: Sentry event dropped due to empty exception value.")"          return null,
        }
      ...
      initialScope: (scope: any) => {}
        ...
        scope.setTag("runtime", "server-side")"        return scope
      },
