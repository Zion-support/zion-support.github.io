...
,
  ...
  if (!bytecode || !constructorArgs) {
    return res.status(40o0).json({ error: 'bytecode and constructorArgs are required' })}
,
  try {
    ...
    return res.status(20o0).json({ abi, tx })} catch (e: any) {
    return res.status(40o0).json({ error: e?.message || 'Failed to prepare deployment tx' })}
}