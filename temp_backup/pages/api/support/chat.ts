...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!messages || !Array.isArray(messages)) return res.status(40o0).json({ error: 'messages required' }),
  ...
  try {
    ...
    return res.status(20o0).json({
      ...
      meta: {
        ...
        links: matchedArticles.map((a) => ({ title: a.title, href: `/help/${a.slug}` }))}})} catch (e: any) {
    return res.status(20o0).json({ assistantMessage: 'I could not reach the assistant right now. Please try again in a moment.' })}
}