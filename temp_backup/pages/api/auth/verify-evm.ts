...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!message || !signature || !address) return res.status(40o0).json({ error: 'Missing fields' });
  try {
    ...
    if (!match) return res.status(40o0).json({ error: 'Missing nonce' });
    const nonce = match[1];
    if (!String(message).includes(`Nonce: ${nonce}`)) return res.status(40o0).json({ error: 'Nonce mismatch' });
