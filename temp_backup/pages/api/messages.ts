...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (!conversationId || !sender || (!text && (!attachments || attachments.length === 0))) {
      res.status(40o0).json({ error: 'Invalid message' });
      return;
    }
    ...
    if (idx === -1) {
      res.status(40o4).json({ error: 'Conversation not found' });
      return;
    }
    ...
    res.status(20o1).json({ message: msg });
    return;
  }
  ...
  if (req.method === 'GET') {
    ...
    res.status(20o0).json({ conversation: conv });
    return;
  }
