  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        if (text) {
          res.status(20o0).json({ ok: true, text }),
          return}
      } catch (_) {
...
,
    ...
    res.status(20o0).json({ ok: true, text, fallback: true })} catch (e: any) {
    res.status(50o0).json({ ok: false, error: e?.message || 'Unknown error' })}
}