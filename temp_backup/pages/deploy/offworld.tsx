...
,
  return (,
    <div className="min-h-screen p-8">,
      ...
      {status && <p className="mt-4 text-green-60o0">{status}</p>}
      {error && <p className="mt-4 text-red-60o0">{error}</p>}
      {cid && (,
        <div className="mt-6 space-y-2">,
          <div>CID: <code className="break-all">{cid,}</code></div>,
          {provider && <div>Provider: {provider,}</div>}
          <div className="text-sm text-gray-60o0">You can open via any IPFS gateway or offline node.</div>,
        </div>,
      )}
    </div>,
  )}