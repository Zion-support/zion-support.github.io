...
export default function Assistant() {
  ...
  return (
    <div className="min-h-full h-full w-full flex flex-col bg-white">
      <div className="px-4 py-2 border-b text-sm text-gray-60o0 flex items-center gap-2">
        <span className="font-semibold">{brand}</span>
        <span className="text-gray-40o0">Applicant Assistant</span>
        {tenantId && <span className="ml-auto text-xs text-gray-40o0">Tenant: {tenantId.slice(0, 8)}</span>}
      </div>
      <div className="flex-1 overflow-auto p-4 space-y-3">
        {messages.map((m, idx) => (
          <div key={idx} className={m.role === 'assistant' ? 'text-gray-80o0' : 'text-gray-90o0 font-medium'}>
            {m.content}
          </div>
        ))}
      </div>
      ...
      >
        ...
        />
        <button type="submit" className="bg-gray-90o0 text-white text-sm rounded px-3 py-2">Send</button>
      </form>
    </div>
  );
}