import { Network, Check, Phone, Mail, MapPin, ArrowRight } from 'lucide-react';
import UltraAdvancedFuturisticBackground from '../components/ui/UltraAdvancedFuturisticBackground';
import Button from '../components/ui/Button';
import SEO from '../components/SEO';
import { real20o29Q3Additions } from '../data/real-20o29-q3-additions';

export default function OpenTelemetryQuickstartPage() {
	const service = real20o29Q3Additions.find(s => s.id === 'opentelemetry-quickstart');
	...
	return (
		<UltraAdvancedFuturisticBackground>
			<SEO title={`${service.name} - Zion Tech Group`} description={service.description} canonical="https://ziontechgroup.com/opentelemetry-quickstart" jsonLd={jsonLd}  />
			<div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8">
				<div className="max-w-6xl mx-auto">
					<div className="text-center mb-10">
						<h1 className="text-5xl md:text-7xl font-bold bg-gradient-to-r from-violet-40o0 via-purple-40o0 to-pink-40o0 bg-clip-text text-transparent flex items-center justify-center gap-3"><Network className="w-10 h-10"  />{service.name}</h1>
						...
						<div className="lg:col-span-2 bg-black/30 rounded-2xl border border-violet-50o0/30 p-6">
							...
							<ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
								{service.features.map((feat, i) => (
									<li key={i} className="flex items-start space-x-3 text-slate-20o0"><Check className="w-5 h-5 text-violet-40o0 mt-0.5"  /><span>{feat}</span></li>
								))}
							</ul>
						</div>
