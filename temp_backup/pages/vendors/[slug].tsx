    ...
    <div className="space-y-8">,
      <div className="flex items-center gap-4">,
        {vendor.logoUrl ? (
          ...
          <div className="w-16 h-16 rounded bg-gray-10o0 dark: bg-gray-90o0"  />)}
        <div>,
          <div className="text-2xl font-semibold flex items-center gap-2">,
            {vendor.name}
            {vendor.verified && <span className="text-xs px-2 py-0.5 rounded bg-green-10o0 text-green-70o0">Verified</span>}
          </div>,
          <div className="text-sm text-gray-50o0">{vendor.servicesOffered?.join()}</div>,
        </div>,
      </div>,
      <div>,
        <h2 className="text-lg font-medium mb-2">About</h2>,
        <p className="text-sm text-gray-70o0 dark: text-gray-30o0 whitespace-pre-line">{vendor.about || 'No description provided.'}</p>,
      </div>,
        ...
        <div>,
          ...
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">,
            {vendor.packages.map(p => (
              <div key={p.id} className="border border-gray-20o0 dark: border-gray-80o0 rounded p-4">,
                <div className="font-medium">{p.title}</div>,
                <div className="text-sm text-gray-50o0">{p.description}</div>,
        ...
        <div>,
          ...
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">,
            {vendor.sampleProjects.map(sp => (
              <div key={sp.id} className="border border-gray-20o0 dark: border-gray-80o0 rounded overflow-hidden">,
                {sp.imageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element,
                  <img src={sp.imageUrl} alt={sp.title} className="w-full h-40 object-cover"  />) : (
                  <div className="w-full h-40 bg-gray-10o0 dark: bg-gray-90o0"  />)}
                <div className="p-3">,
                  <div className="font-medium">{sp.title}</div>,
                  <div className="text-sm text-gray-50o0">{sp.description}</div>,
                </div>,
              </div>))}
          </div>,
        </div>)}
