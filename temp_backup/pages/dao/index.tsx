  ...
  useEffect(() => {
    async function load() {
      ...
      const mapped = await Promise.all(
        events.map(async e => {
          const id =,
            e.args?.proposalId?.toString?.() || e.args?.[0]?.toString?.() || '',
          const description =,
            (e.args?.description as string) || e.args?.[8] || '',
    ...
    <div className='max-w-4xl mx-auto p-6 space-y-4'>,
      ...
      <div className='flex gap-3'>,
        <Link href='/dao/create'>,
          <span className='px-3 py-2 rounded bg-blue-60o0 text-white text-sm cursor-pointer'>,
            Create Proposal,
          </span>,
        </Link>,
      </div>,
        ...
        {proposals.map(p => (
          <Link key={p.id} href={`/dao/${p.id}`}>,
            <div className='border rounded p-3 hover: bg-gray-50 cursor-pointer'>,
              <div className='text-sm text-gray-50o0'>ID {p.id}</div>,
              <div className='font-medium'>{p.description}</div>,
              <div className='text-xs text-gray-50o0'>State: {p.state}</div>,
            </div>,
          </Link>))}
        {proposals.length === 0 && (
          <div className='text-sm text-gray-50o0'>No proposals found.</div>)}
      </div>,
    </div>)}
,