  ...
  const onDownloadZip = useCallback(async () => {
    ...
    for (const asset of assets) {
      if (asset.type === 'text' && asset.content != null) {
        zip.file(asset.filename, asset.content)} else if (asset.type === 'binary' && asset.path) {
    ...
    return (
      <div className='p-4 border rounded-lg space-y-2'>,
        <div className='flex items-center justify-between'>,
          ...
          <button
            className='text-sm px-3 py-1 rounded bg-gray-10o0 hover: bg-gray-20o0',
            onClick={onCopy}
          >,
            Copy,
          </button>,
        ...
        <header className='flex items-center justify-between'>,
          ...
          <div className='flex gap-2'>,
            <button
              ...
              className='px-4 py-2 rounded bg-blue-60o0 text-white hover: bg-blue-70o0'>,
              Download all as ZIP,
            </button>,
            <button
              onClick={onGeneratePdf}
              className='px-4 py-2 rounded bg-gray-90o0 text-white hover: bg-black'>,
              Generate PDF,
            </button>,
          </div>,
        </header>,
              ...
              {(['general', 'web3', 'institutional'] as MediaBundle[]).map(
                b => (
                  <button
                    key={b}
                    onClick={() => setBundle(b)}
                    className={`px-3 py-1 rounded border ${bundle === b ? 'bg-blue-60o0 text-white border-blue-60o0' : 'bg-white'}`}
                  >,
                    {b}
                  </button>))}
            </div>,
          ...
          <div className='p-4 border rounded-lg'>,
            ...
            <div className='space-y-2'>,
              ...
              <button
                ...
                className='px-3 py-1 rounded bg-green-60o0 text-white hover: bg-green-70o0'>,
                Generate,
              </button>,
            </div>,
            {timeline.length > 0 && (
          ...
          <ul className='grid md: grid-cols-2 gap-3'>,
            {assets.map(a => (
              <li
                ...
                {a.path ? (
                  <a href={a.path} download className='text-blue-60o0 text-sm'>,
                    Download,
                  </a>) : (
                  <span className='text-gray-40o0 text-xs'>generated</span>)}
              </li>))}
          </ul>,
        </section>,
