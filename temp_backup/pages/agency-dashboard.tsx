...
export default function AgencyDashboardPage({ vendor }: Props) {
  ...
  if (!activeVendor) return <div className="text-gray-50o0">No vendor found. Please apply first.</div>;
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        {!activeVendor.verified && <span className="text-xs px-2 py-0.5 rounded bg-yellow-10o0 text-yellow-80o0">Pending Verification</span>}
      </div>
...
function Pipeline({ vendorId }: { vendorId: string }) {
  ...
  return (
    <div className="space-y-2">
      {items.length === 0 && <div className="text-sm text-gray-50o0">No leads yet.</div>}
      {items.map(item => (
        <div key={item.id} className="border border-gray-20o0 dark:border-gray-80o0 rounded p-3 flex items-center justify-between">
          <div>
            <div className="font-medium">{item.title}</div>
            <div className="text-xs text-gray-50o0">{new Date(item.createdAt).toLocaleString()} • {item.status}</div>
          </div>
