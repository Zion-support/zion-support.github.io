  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      ...
      if (message === 'login_required' && nextUrl) {
        ...
        const product =
          productId && SAMPLE_EQUIPMENT[productId]
            ? SAMPLE_EQUIPMENT[productId]
            : null;
        ...
        if (product) {
          ...
          toast.error(
            'Item details not found for cart. Redirecting to original page.'
          );
          // Clear our specific message before navigating
          params.delete('msg');
          const queryString = params.toString();
          navigate(
            nextUrl.split('?')[0] + `${queryString ? '?' + queryString : ''}`,
            { replace: true }
          );
        }
      } else if (nextUrl) {
