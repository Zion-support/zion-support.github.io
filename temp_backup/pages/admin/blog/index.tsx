...
export default function AdminBlog() {
  ...
  useEffect(() => {
    ...
    setToken(t);
    fetch('/api/blog/posts?status=draft')
      .then(r => r.json())
  ...
  return (
    <div className='mx-auto max-w-5xl'>
      ...
      <div className='mb-6 flex gap-2 items-center'>
        ...
        />
        <button onClick={saveToken} className='px-3 py-2 border rounded'>
          Save token
        </button>
        <button
          onClick={() => startEdit()}
          className='ml-auto px-3 py-2 bg-indigo-60o0 text-white rounded'
        >
          New Post
        </button>
      </div>
          ...
          <ul className='space-y-2'>
            {posts.map(p => (
              <li key={p.id} className='flex items-center justify-between'>
                ...
                <span
                  className={`text-xs px-2 py-1 rounded ${p.status === 'published' ? 'bg-green-10o0' : 'bg-yellow-10o0'}`}
                >
                  {p.status}
                </span>
              </li>
        ...
        <div className='md:col-span-2 border rounded p-4'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
            ...
            <input
              ...
              onChange={e =>
                setEditing(s => ({ ...s, coverImageUrl: e.target.value }))
              }
            />
            <input
              className='border rounded px-3 py-2'
              ...
              onChange={e =>
                setEditing(s => ({ ...s, author: e.target.value }))
              }
            />
            <input
              type='date'
          ...
          <div className='mt-3 flex gap-2'>
            ...
            <button
              onClick={() => save(true)}
              className='px-4 py-2 bg-green-60o0 text-white rounded'
            >
              Publish
            </button>
          </div>
