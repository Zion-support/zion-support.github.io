...
export default function FeedbackInsightsAdminPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/latest-feedback-report')
      .then(r => r.json())
      .then(setReport)
      ...
      <div className='space-y-8'>
        <div className='flex items-center justify-between'>
          ...
          <button
            onClick={runAnalysis}
            disabled={loading}
            className='px-4 py-2 rounded bg-gray-90o0 text-white disabled:opacity-60'
          >
                ...
                <table className='min-w-full text-sm'>
                  ...
                  <tbody>
                    {report.improvementBacklog.map((item, idx) => (
                      <tr
                        key={idx}
                        className='border-t border-gray-20o0 dark:border-gray-80o0 align-top'
                      >
                        ...
                        <td className='py-2 pr-4 text-gray-60o0 dark:text-gray-40o0'>
                          {item.description}
                        </td>
                      </tr>
                    ))}
        ...
        <section>
          <EnhancedCard title='By Feature Area'>
            {!report ||
            !report.groupedByFeature ||
            Object.keys(report.groupedByFeature).length === 0 ? (
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {Object.entries(report.groupedByFeature).map(([area, data]) => (
                  <div
                    key={area}
                    className='rounded border border-gray-20o0 dark:border-gray-80o0 p-3'
                  >
