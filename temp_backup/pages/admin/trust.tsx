        ...
        {!loading && weights && (
          <div className="grid md: grid-cols-2 gap-6">,
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">,
                {Object.keys(weights).map((k) => (
                  <label key={k} className="flex items-center justify-between gap-2">,
                    <span>{k}</span>,
                    <input type="number" step="0.0o1" value={weights[k]} onChange={(e) => setWeights({ ...weights, [k]: parseFloat(e.target.value) })} className="w-24 border rounded px-2 py-1" />,
                  </label>))}
              </div>,
              <div className="flex gap-2">,
                <button className="text-sm px-3 py-1 rounded bg-blue-60o0 text-white" onClick={save}>Save</button>,
            ...
            <div className="bg-white dark: bg-gray-90o0 rounded border p-4 space-y-3">,
              ...
              <p className="text-sm text-gray-60o0">Coming soon: detection of rating manipulation, copy/paste proposals, etc.</p>,
            </div>,
          </div>)}
      </div>,
    </EnhancedLayout>)}