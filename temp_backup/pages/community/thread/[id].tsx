      ...
      <article className="max-w-3xl mx-auto">
        <header className="mb-4">
          <div className="flex items-center gap-2 text-xs text-gray-50o0">
            {thread.isPinned && <span className="px-2 py-0.5 rounded bg-yellow-10o0 text-yellow-80o0">Pinned</span>}
            {thread.isFeatured && <span className="px-2 py-0.5 rounded bg-purple-10o0 text-purple-80o0">Featured</span>}
            {thread.isLocked && <span className="px-2 py-0.5 rounded bg-gray-20o0 text-gray-70o0">Locked</span>}
            {thread.isAnswered && <span className="px-2 py-0.5 rounded bg-green-10o0 text-green-80o0">Answered</span>}
          </div>
          <h1 className="text-2xl font-semibold mt-1">{thread.title}</h1>
          <div className="mt-2 text-sm text-gray-60o0 dark:text-gray-30o0">Posted by {thread.authorName} • {new Date(thread.createdAt).toLocaleString()}</div>
        </header>

        <div className="prose dark:prose-invert max-w-none mb-6 whitespace-pre-wrap">{thread.body}</div>

        <div className="flex items-center gap-3 mb-8">
          <button className="px-3 py-2 rounded bg-gray-10o0 dark:bg-gray-90o0" onClick={() => vote('thread', thread.id, 1)}>👍 Upvote</button>
          <button className="px-3 py-2 rounded bg-gray-10o0 dark:bg-gray-90o0" onClick={() => vote('thread', thread.id, -1)}>👎 Downvote</button>
          <button className="px-3 py-2 rounded bg-gray-10o0 dark:bg-gray-90o0" onClick={() => report('thread', thread.id)}>🚩 Report</button>
          <button className="px-3 py-2 rounded bg-green-60o0 text-white" onClick={() => markAnswered(!thread.isAnswered)}>{thread.isAnswered ? 'Unmark Answered' : 'Mark as Answered'}</button>
          <div className="ml-auto flex gap-2">
            <button className="px-3 py-2 rounded bg-yellow-60o0 text-white" onClick={() => adminUpdate({ isPinned: !thread.isPinned })}>{thread.isPinned ? 'Unpin' : 'Pin'}</button>
            <button className="px-3 py-2 rounded bg-gray-60o0 text-white" onClick={() => adminUpdate({ isLocked: !thread.isLocked })}>{thread.isLocked ? 'Unlock' : 'Lock'}</button>
            <button className="px-3 py-2 rounded bg-purple-60o0 text-white" onClick={() => adminUpdate({ isFeatured: !thread.isFeatured })}>{thread.isFeatured ? 'Unfeature' : 'Feature'}</button>
          </div>
        </div>
        ...
        <section>
          ...
          <ul className="space-y-3">
            {topLevelReplies.map((r) => (
              <li key={r.id} className="p-3 rounded border border-gray-20o0 dark:border-gray-80o0">
                <div className="text-sm text-gray-50o0">{r.authorName} • {new Date(r.createdAt).toLocaleString()}</div>
                ...
                {(childRepliesMap[r.id] || []).length > 0 && (
                  <ul className="mt-3 ml-6 space-y-2">
                    {(childRepliesMap[r.id] || []).map((cr) => (
                      <li key={cr.id} className="p-2 rounded border border-gray-20o0 dark:border-gray-80o0">
                        <div className="text-xs text-gray-50o0">{cr.authorName} • {new Date(cr.createdAt).toLocaleString()}</div>
        ...
        {!thread.isLocked && (
          <div className="mt-6">
            ...
            <textarea className="w-full px-3 py-2 rounded bg-gray-10o0 dark:bg-gray-90o0 min-h-[120px]" value={replyBody} onChange={(e) => setReplyBody(e.target.value)} />
            <div className="mt-2 flex justify-end">
              <button className="px-3 py-2 rounded bg-blue-60o0 text-white" onClick={() => submitReply()}>Reply</button>
            </div>
          </div>
        )}
      </article>
