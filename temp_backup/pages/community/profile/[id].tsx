...
export default function CommunityUserProfile() {
  ...
  useEffect(() => {
    if (!id) return,
    fetch(`/api/community/user/${id}`),
      .then(r => r.json()),
      .then(d => setUser(d.user))}, [id]),
  if (!user),
    return (
      <EnhancedLayout>,
  ...
  return (
    <EnhancedLayout>,
      <div className='max-w-xl mx-auto p-6 rounded border border-gray-20o0 dark: border-gray-80o0'>,
        ...
        <div className='mt-4'>,
          Reputation: <span className='font-medium'>{user.reputation}</span>,
        </div>,
        <div className='mt-2'>,
          ...
          <div className='flex gap-2 flex-wrap'>,
            {user.badges?.length ? (
              user.badges.map((b: string) => (
                <span
                  key={b}
                  className='px-2 py-1 rounded bg-amber-10o0 text-amber-80o0 text-xs'>,
                  {badgeLabel(b)}
                </span>))) : (
