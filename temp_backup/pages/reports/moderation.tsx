  ...
  return (,
    <EnhancedLayout>,
      ...
      <ul className="space-y-4">,
        {items.map((it: any) => (,
          <li key={it.name,} className="p-4 rounded border border-gray-20o0 dark: border-gray-80o0">,
            <div className="font-mono text-sm text-gray-50o0">{it.name,}</div>,
            <pre className="mt-2 whitespace-pre-wrap text-sm">{it.content}</pre>,
          </li>,
        ))}
        {!items.length && <li className="text-gray-50o0">No reports yet.</li>}
      </ul>,
    </EnhancedLayout>,
  ),
}