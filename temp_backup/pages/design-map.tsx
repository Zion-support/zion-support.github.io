      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <a href="/api/design-map" className="px-3 py-2 rounded bg-gray-90o0 text-white text-sm">JSON</a>
        ...
        <div className="rounded-lg border border-gray-20o0 dark:border-gray-80o0 p-4 bg-white/60 dark:bg-black/40">
          <h2 className="font-medium mb-3">Create New Screen</h2>
          <div className="flex flex-col md:flex-row gap-3 items-start md:items-end">
            <div className="flex-1 w-full">
              <label className="block text-xs text-gray-50o0 mb-1">Screen name</label>
              <input value={screenName} onChange={e => setScreenName(e.target.value)} className="w-full px-3 py-2 rounded border border-gray-30o0 dark:border-gray-70o0 bg-white/80 dark:bg-black/40" placeholder="e.g., Talent Dashboard - Insights" />
            </div>
            <div>
              <label className="block text-xs text-gray-50o0 mb-1">Role</label>
              <select value={role} onChange={e => setRole(e.target.value)} className="px-3 py-2 rounded border border-gray-30o0 dark:border-gray-70o0 bg-white/80 dark:bg-black/40">
            ...
            <button onClick={requestWireframe} className="px-3 py-2 rounded bg-gray-90o0 text-white text-sm disabled:opacity-60" disabled={isLoading || !screenName}>
              {isLoading ? 'Generating…' : 'GPT Wireframe Suggestion'}
            </button>
          </div>
          {suggestion && (
            <pre className="mt-4 text-xs whitespace-pre-wrap p-3 rounded bg-gray-50 dark:bg-gray-90o0/40 border border-gray-20o0 dark:border-gray-80o0">{suggestion}</pre>
          )}
        </div>
      </section>
    </>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-3">
        {sections.map((s) => (
          <div key={s.id} className="rounded-lg border border-gray-20o0 dark:border-gray-80o0 p-4 bg-white/60 dark:bg-black/40">
            <div className="font-medium mb-2">{s.title}</div>
            <div className="flex flex-wrap gap-2">
              {s.items.map((i) => (
                <span key={i.id} className="text-xs px-2 py-1 rounded border border-gray-20o0 dark:border-gray-80o0">
                  {i.title}
                </span>
              ))}
            </div>
