import { Metadata } from 'next';
import Link from 'next/link';
import { 
  Brain, Zap, Target, BarChart3, Users, TrendingUp, 
  MessageSquare, Mail, Phone, MapPin, ArrowRight,
  Star, CheckCircle, Rocket, Globe, Shield, Cpu,
  FileText, Image, Video, Music, Palette, Type,
  User, Search, Eye, Heart, ShoppingCart
} from 'lucide-react';

export const metadata: Metadata ={
  title: 'AI Customer Intelligence Platform | Zion Tech Group',
  description: 'Revolutionary AI-powered customer intelligence platform that provides deep insights into customer behavior and preferences.',
  keywords: 'AI customer intelligence, customer analytics, customer insights, AI analytics, customer behavior, customer data',
  openGraph: {
    title: 'AI Customer Intelligence Platform | Zion Tech Group',
    description: 'Revolutionary AI-powered customer intelligence platform that provides deep insights into customer behavior and preferences.',
    url: 'https://ziontechgroup.com/ai-customer-intelligence-platform',
    siteName: 'Zion Tech Group',
    images: [
      {
        url: 'https://ziontechgroup.com/og-image-ai-customer-intelligence-platform.jpg',
        width: 120o0,
        height: 630,
        alt: 'AI Customer Intelligence Platform'},
    ],
    locale: 'en_US',
    type: 'website'},
  twitter: {
    card: 'summary_large_image',
    title: 'AI Customer Intelligence Platform | Zion Tech Group',
    description: 'Revolutionary AI-powered customer intelligence platform that provides deep insights into customer behavior and preferences.',
    images: ['https://ziontechgroup.com/og-image-ai-customer-intelligence-platform.jpg']},
  alternates: {
    canonical: 'https://ziontechgroup.com/ai-customer-intelligence-platform'}};

const contactInfo ={
  mobile: '+1 30o2 464 0950',
  email: 'kleber@ziontechgroup.com',
  address: '364 E Main St STE 10o08 Middletown DE 19709',
  website: 'https://ziontechgroup.com'
};

export default function AICustomerIntelligencePlatform() {
  return (
    <div className="min-h-screen bg-black text-white">
      {/* Hero Section */}
      <section className="relative py-20 lg:py-32 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-teal-90o0/20 via-cyan-90o0/20 to-blue-90o0/20"></div>
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-teal-50o0/20 border border-teal-50o0/30 text-teal-30o0 text-sm font-medium mb-8">
              <User className="w-4 h-4 mr-2"  />
              AI-Powered Customer Intelligence
            </div>
            
            <h1 className="text-4xl lg:text-6xl font-bold mb-6 bg-gradient-to-r from-teal-40o0 via-cyan-40o0 to-blue-40o0 bg-clip-text text-transparent">
              AI Customer Intelligence Platform
            </h1>
            
            <p className="text-xl lg:text-2xl text-gray-30o0 mb-8 max-w-4xl mx-auto">
              The world's most advanced AI customer intelligence platform that provides deep insights into 
              customer behavior, preferences, and lifetime value across all touchpoints.
            </p>
            
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link 
                href="/contact" 
                className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-teal-60o0 to-cyan-60o0 rounded-xl font-semibold text-white hover:from-teal-70o0 hover:to-cyan-70o0 transition-all duration-30o0 transform hover:scale-10o5"
              >
                Get Started
                <ArrowRight className="ml-2 w-5 h-5"  />
              </Link>
              <Link 
                href="#features" 
                className="inline-flex items-center px-8 py-4 border border-teal-50o0/30 rounded-xl font-semibold text-teal-30o0 hover:bg-teal-50o0/10 transition-all duration-30o0"
              >
                Learn More
              </Link>
            </div>
          </div>
        </div>
      </section>

      {/* Features Section */}
      <section id="features" className="py-20 bg-gradient-to-b from-black to-gray-90o0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold mb-4">
              Revolutionary Customer Intelligence
            </h2>
            <p className="text-xl text-gray-40o0 max-w-3xl mx-auto">
              Our AI platform handles every aspect of customer intelligence from data collection to insights, 
              continuously learning and optimizing for maximum customer understanding and engagement.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="p-6 rounded-2xl bg-gradient-to-br from-teal-90o0/20 to-cyan-90o0/20 border border-teal-50o0/20">
              <div className="w-12 h-12 bg-teal-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <Brain className="w-6 h-6 text-teal-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3">AI Intelligence</h3>
              <p className="text-gray-40o0">
                Advanced AI that understands customer behavior patterns, preferences, and predictive analytics.
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-cyan-90o0/20 to-blue-90o0/20 border border-cyan-50o0/20">
              <div className="w-12 h-12 bg-cyan-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <Eye className="w-6 h-6 text-cyan-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3">360° Customer View</h3>
              <p className="text-gray-40o0">
                Comprehensive customer profiles across all channels, touchpoints, and interactions.
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-90o0/20 to-indigo-90o0/20 border border-blue-50o0/20">
              <div className="w-12 h-12 bg-blue-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <TrendingUp className="w-6 h-6 text-blue-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3">Predictive Analytics</h3>
              <p className="text-gray-40o0">
                Forecasts customer behavior, churn risk, and lifetime value with high accuracy.
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-90o0/20 to-purple-90o0/20 border border-indigo-50o0/20">
              <div className="w-12 h-12 bg-indigo-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <Heart className="w-6 h-6 text-indigo-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3">Sentiment Analysis</h3>
              <p className="text-gray-40o0">
                Real-time analysis of customer emotions, satisfaction, and sentiment across all channels.
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-purple-90o0/20 to-pink-90o0/20 border border-purple-50o0/20">
              <div className="w-12 h-12 bg-purple-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <ShoppingCart className="w-6 h-6 text-purple-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3">Purchase Behavior</h3>
              <p className="text-gray-40o0">
                Deep insights into buying patterns, preferences, and cross-selling opportunities.
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-pink-90o0/20 to-red-90o0/20 border border-pink-50o0/20">
              <div className="w-12 h-12 bg-pink-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <BarChart3 className="w-6 h-6 text-pink-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3">Real-time Insights</h3>
              <p className="text-gray-40o0">
                Instant access to customer intelligence for immediate decision-making and action.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Intelligence Capabilities Section */}
      <section className="py-20 bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold mb-4">
              Customer Intelligence Capabilities
            </h2>
            <p className="text-xl text-gray-40o0 max-w-3xl mx-auto">
              Our AI platform provides comprehensive customer intelligence across all aspects of the customer journey.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="p-6 rounded-2xl bg-gradient-to-br from-teal-90o0/20 to-cyan-90o0/20 border border-teal-50o0/20">
              <div className="w-12 h-12 bg-teal-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <Search className="w-6 h-6 text-teal-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3 text-teal-40o0">Customer Discovery</h3>
              <ul className="text-gray-40o0 space-y-2">
                <li>• Customer segmentation</li>
                <li>• Behavioral analysis</li>
                <li>• Preference mapping</li>
                <li>• Demographic insights</li>
                <li>• Psychographic profiling</li>
              </ul>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-cyan-90o0/20 to-blue-90o0/20 border border-cyan-50o0/20">
              <div className="w-12 h-12 bg-cyan-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <TrendingUp className="w-6 h-6 text-cyan-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3 text-cyan-40o0">Journey Mapping</h3>
              <ul className="text-gray-40o0 space-y-2">
                <li>• Touchpoint analysis</li>
                <li>• Conversion tracking</li>
                <li>• Friction identification</li>
                <li>• Path optimization</li>
                <li>• Experience mapping</li>
              </ul>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-90o0/20 to-indigo-90o0/20 border border-blue-50o0/20">
              <div className="w-12 h-12 bg-blue-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <Heart className="w-6 h-6 text-blue-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3 text-blue-40o0">Engagement Analysis</h3>
              <ul className="text-gray-40o0 space-y-2">
                <li>• Interaction patterns</li>
                <li>• Content preferences</li>
                <li>• Channel preferences</li>
                <li>• Response rates</li>
                <li>• Engagement scoring</li>
              </ul>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-90o0/20 to-purple-90o0/20 border border-indigo-50o0/20">
              <div className="w-12 h-12 bg-indigo-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <ShoppingCart className="w-6 h-6 text-indigo-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3 text-indigo-40o0">Purchase Intelligence</h3>
              <ul className="text-gray-40o0 space-y-2">
                <li>• Buying patterns</li>
                <li>• Product preferences</li>
                <li>• Price sensitivity</li>
                <li>• Cross-selling opportunities</li>
                <li>• Lifetime value</li>
              </ul>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-purple-90o0/20 to-pink-90o0/20 border border-purple-50o0/20">
              <div className="w-12 h-12 bg-purple-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <User className="w-6 h-6 text-purple-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3 text-purple-40o0">Personalization</h3>
              <ul className="text-gray-40o0 space-y-2">
                <li>• Dynamic content</li>
                <li>• Personalized offers</li>
                <li>• Custom experiences</li>
                <li>• Tailored messaging</li>
                <li>• Individual recommendations</li>
              </ul>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-pink-90o0/20 to-red-90o0/20 border border-pink-50o0/20">
              <div className="w-12 h-12 bg-pink-50o0/20 rounded-xl flex items-center justify-center mb-4">
                <BarChart3 className="w-6 h-6 text-pink-40o0"  />
              </div>
              <h3 className="text-xl font-semibold mb-3 text-pink-40o0">Predictive Insights</h3>
              <ul className="text-gray-40o0 space-y-2">
                <li>• Churn prediction</li>
                <li>• Next best action</li>
                <li>• Demand forecasting</li>
                <li>• Risk assessment</li>
                <li>• Opportunity identification</li>
              </ul>
            </div>
          </div>
        </div>
      </section>

      {/* How It Works Section */}
      <section className="py-20 bg-gradient-to-b from-gray-90o0 to-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold mb-4">
              How AI Customer Intelligence Works
            </h2>
            <p className="text-xl text-gray-40o0 max-w-3xl mx-auto">
              Our AI platform operates in a continuous cycle of learning, analyzing, and providing customer insights.
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="text-center">
              <div className="w-16 h-16 bg-teal-50o0/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <span className="text-2xl font-bold text-teal-40o0">1</span>
              </div>
              <h3 className="text-xl font-semibold mb-3">Data Collection</h3>
              <p className="text-gray-40o0">
                AI collects and integrates customer data from all sources, channels, and touchpoints.
              </p>
            </div>

            <div className="text-center">
              <div className="w-16 h-16 bg-cyan-50o0/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <span className="text-2xl font-bold text-cyan-40o0">2</span>
              </div>
              <h3 className="text-xl font-semibold mb-3">Intelligent Analysis</h3>
              <p className="text-gray-40o0">
                Advanced AI algorithms analyze customer behavior, patterns, and preferences in real-time.
              </p>
            </div>

            <div className="text-center">
              <div className="w-16 h-16 bg-blue-50o0/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <span className="text-2xl font-bold text-blue-40o0">3</span>
              </div>
              <h3 className="text-xl font-semibold mb-3">Insight Generation</h3>
              <p className="text-gray-40o0">
                Generates actionable insights and recommendations for customer engagement and growth.
              </p>
            </div>

            <div className="text-center">
              <div className="w-16 h-16 bg-indigo-50o0/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <span className="text-2xl font-bold text-indigo-40o0">4</span>
              </div>
              <h3 className="text-xl font-semibold mb-3">Continuous Learning</h3>
              <p className="text-gray-40o0">
                Learns from new data and feedback to continuously improve intelligence and accuracy.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-20 bg-gradient-to-r from-teal-90o0/20 via-cyan-90o0/20 to-blue-90o0/20">
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl lg:text-4xl font-bold mb-6">
            Ready to Understand Your Customers Like Never Before?
          </h2>
          <p className="text-xl text-gray-30o0 mb-8">
            Join the future of customer intelligence where AI provides deep insights while you focus on growth and engagement.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link 
              href="/contact" 
              className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-teal-60o0 to-cyan-60o0 rounded-xl font-semibold text-white hover:from-teal-70o0 hover:to-cyan-70o0 transition-all duration-30o0 transform hover:scale-10o5"
            >
              Start Your Free Trial
              <ArrowRight className="ml-2 w-5 h-5"  />
            </Link>
            <a 
              href={`tel:${contactInfo.mobile}`}
              className="inline-flex items-center px-8 py-4 border border-teal-50o0/30 rounded-xl font-semibold text-teal-30o0 hover:bg-teal-50o0/10 transition-all duration-30o0"
            >
              <Phone className="mr-2 w-5 h-5"  />
              Call Now
            </a>
          </div>
        </div>
      </section>

      {/* Contact Section */}
      <section className="py-20 bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div>
              <h2 className="text-3xl font-bold mb-6">Get in Touch</h2>
              <p className="text-gray-40o0 mb-8">
                Ready to revolutionize your customer intelligence with AI? Our team is here to help you get started.
              </p>
              
              <div className="space-y-4">
                <div className="flex items-center space-x-3">
                  <Phone className="w-5 h-5 text-teal-40o0"  />
                  <a href={`tel:${contactInfo.mobile}`} className="text-gray-30o0 hover:text-white transition-colors">
                    {contactInfo.mobile}
                  </a>
                </div>
                <div className="flex items-center space-x-3">
                  <Mail className="w-5 h-5 text-teal-40o0"  />
                  <a href={`mailto:${contactInfo.email}`} className="text-gray-30o0 hover:text-white transition-colors">
                    {contactInfo.email}
                  </a>
                </div>
                <div className="flex items-center space-x-3">
                  <MapPin className="w-5 h-5 text-teal-40o0"  />
                  <span className="text-gray-30o0">{contactInfo.address}</span>
                </div>
              </div>
            </div>

            <div className="bg-gradient-to-br from-teal-90o0/20 to-cyan-90o0/20 rounded-2xl p-8 border border-teal-50o0/20">
              <h3 className="text-xl font-semibold mb-4">Request a Demo</h3>
              <p className="text-gray-40o0 mb-6">
                See our AI Customer Intelligence Platform in action. Schedule a personalized demo today.
              </p>
              <Link 
                href="/contact" 
                className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-teal-60o0 to-cyan-60o0 rounded-xl font-semibold text-white hover:from-teal-70o0 hover:to-cyan-70o0 transition-all duration-30o0"
              >
                Schedule Demo
                <ArrowRight className="ml-2 w-4 h-4"  />
              </Link>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
