...
export async function getStaticProps() {
  const recDir = path.join(process.cwd(), 'datapolicyrecommendations'),
  ...
  const propDir = path.join(process.cwd(), 'docsgovernanceproposals'),
      ...
      <div className="space-y-6">,
        ...
        {recommendation ? (
          <div className="text-sm space-y-2">,
            ...
            {proposal ? (
              <div className="pt-2"><a className="underline" href={`/docs/governance/proposals/${proposal}`} target="_blank" rel="noreferrer">Open proposal</a></div>) : null}
          </div>) : (
