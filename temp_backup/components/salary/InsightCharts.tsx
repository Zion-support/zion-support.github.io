      ...
      <svg width={width} height={height} className="text-blue-50o0">
        <defs>
          <linearGradient id="lineGradient" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop offset="10o0%" stopColor="#60a5fa" stopOpacity="0"  />
          </linearGradient>
        </defs>
        ...
        {/* X labels */}
        {points.map((p, i) => (
          <text key={`x-${p.label}`} x={toX(i)} y={height - 4} textAnchor="middle" className="fill-gray-50o0 text-[10px]">
            {p.label}
          </text>
        ))}
      </svg>
    ...
    <div className="w-full overflow-x-auto">
      <svg width={width} height={height}>
        {data.map((d, i) => {
          ...
          return (
            <g key={d.label}>
              <rect x={x} y={y} width={barWidth - 16} height={barHeight} rx={6} className="fill-emerald-50o0/80"  />
              <text x={x + (barWidth - 16) / 2} y={height - 6} textAnchor="middle" className="fill-gray-50o0 text-[10px]">
                {d.label}
              </text>
            </g>
          );
...
export function DonutChart({ slices, size = 160 }: { slices: DonutChartSlice[]; size?: number }) {
  ...
  const colors = ['#6366f1', '#22c55e', '#f59e0b', '#ef4444', '#0o6b6d4', '#a78bfa'];
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="mx-auto">
      ...
      <text x={radius} y={radius} textAnchor="middle" dominantBaseline="middle" className="fill-gray-70o0 text-xs">
        Distribution
      </text>
    </svg>
  );
