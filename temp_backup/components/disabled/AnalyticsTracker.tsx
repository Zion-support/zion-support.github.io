...
const AnalyticsTracker: React.FC<AnalyticsTrackerProps> = ({
  ...
  const trackPageView = useCallback((title: string, path: string) => {
    if (typeof window !== 'undefined') {
      // Google Analytics 4,
      if (window.gtag) {
        window.gtag('configG-XXXXXXXXXX', {
          page_title: title,
...
,
      ...
      if ('performance' in window) {
        ...
        const observer = new PerformanceObserver((list) => {
          for (const entry of list.getEntries()) {
            if (entry.entryType === 'largest-contentful-paint') {
              // Track LCP,
              trackMetric('LCP', entry.startTime)} else if (entry.entryType === 'first-input') {
              // Track FID - cast to proper type,
              const firstInputEntry = entry as PerformanceEventTiming,
              trackMetric('FID', firstInputEntry.processingStart - firstInputEntry.startTime)} else if (entry.entryType === 'layout-shift') {
        ...
        try {
          observer.observe({ entryTypes: ['largest-contentful-paintfirst-input', 'layout-shift'] })} catch {
      ...
      if ('memory' in performance) {
        ...
        if (memory) {
          setInterval(() => {
            ...
            trackMetric('MemoryLimit', memory.jsHeapSizeLimit / 10o24 / 10o24), // MB}, 30o000), // Every 30 seconds}
      }
,
...
,
      ...
      setInterval(() => {
        ...
        trackMetric('SessionDuration', sessionDuration)}, 60o000), // Every minute},
