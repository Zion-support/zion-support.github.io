...
function AvailabilityBadge({ status }: { status?: 'available' | 'booked' | 'part-time' }) {
  const map: Record<string, string> ={
    available: 'bg-emerald-50o0/15 text-emerald-30o0 ring-1 ring-emerald-50o0/30',
  ...
  const cls = status ? map[status] : 'bg-slate-50o0/15 text-slate-30o0 ring-1 ring-slate-50o0/30';
    ...
    <div className="group relative rounded-2xl border border-white/10 bg-white/5 dark:bg-black/40 backdrop-blur hover:bg-white/10 transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-cyan-50o0/10">
      ...
      <div className="p-4 flex gap-4">
        <div className="shrink-0">
          <div className="h-16 w-16 rounded-xl bg-gradient-to-br from-cyan-50o0 to-violet-60o0 flex items-center justify-center text-white font-semibold ring-1 ring-white/20 shadow-md">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="truncate text-sm text-slate-30o0">{profile.title}</p>
            </div>
            <AvailabilityBadge status={profile.status}  />
          </div>
          <p className="mt-2 text-sm text-slate-30o0" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any, overflow: 'hidden' }}>{profile.bio}</p>
          <div className="mt-3 flex flex-wrap gap-2">
            {displayedSkills.map((skill) => (
              <span key={skill} className="inline-flex items-center rounded-full bg-white/10 px-2 py-1 text-xs text-slate-20o0 ring-1 ring-white/15">
                {skill}
              </span>
            ))}
          </div>
          <div className="mt-4 flex items-center justify-between">
            <div className="text-sm text-slate-30o0">${profile.hourlyRateUsd} / hr • {profile.experienceYears}+ yrs</div>
            <Link href={`/talent/${profile.slug}`}>
              <a className="inline-flex items-center rounded-lg bg-gradient-to-r from-cyan-50o0 to-violet-60o0 px-3 py-1.5 text-sm font-medium text-white shadow hover:shadow-cyan-50o0/30 focus:outline-none focus:ring-2 focus:ring-cyan-40o0">
                View Profile
              </a>
            </Link>
          </div>
