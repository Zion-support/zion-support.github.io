					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
						{pillars && pillars.map((p) => (;"
							<Card key={p && p.title} className="bg-gradient-to-br from-gray-900/70 to-black/80 border border-gray-700/40 p-6">;"
								<div className="flex items-center gap-3 mb-3 text-white">{p && p.icon}<span className="font-semibold">{p && p.title}</span></div>;"
								<p className="text-gray-300 text-sm">{p && p.desc}</p>
							</Card>
						))}
					</div>
"
					<Card className="bg-gradient-to-br from-gray-900/80 to-black/90 border border-gray-700/50 backdrop-blur-xl p-8">;"
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">;"
							<div className="flex items-center gap-3 text-cyan-400">;"
								...
								<a href={`tel:${contactInfo && contactInfo.mobile.replace(/[^+\d]/g, '')}`} className="hover: text-white">{contactInfo && contactInfo.mobile}</a>
							</div>;"
							<div className="flex items-center gap-3 text-purple-400">;"
								<Mail className="w-5 h-5" />;"`
								<a href={`mailto:${contactInfo && contactInfo.email}`} className="hover:text-white">{contactInfo && contactInfo.email}</a>
							</div>;"
							<div className="flex items-center gap-3 text-green-400">;"
								<MapPin className="w-5 h-5" />;"`
								<a href={`https://maps && maps.google.com/?q=${encodeURIComponent(contactInfo && contactInfo.address)}`} target="_blank" rel="noopener noreferrer" className="hover:text-white">
      ...
      <section className="section-padding bg-gradient-cursor" />;
        <div className="container-cursor" />;
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8" />;
            {achievements.map((achievement, index) => (<div key={index} className="text-center group" />;
                <div className={`text-4xl md: text-5xl font-bold mb-3 ${achievement.color} group-hover:scale-110 transition-transform duration-300`} />{achievement.numbe,}
}</div>;
                <div className="text-gray-400 text-sm md: text-base font-medium" />{achievement.labe,}
}</div>;
              </div>;
            ))}
          </div>;
          ...
          <div className="container-cursor" />;
            ...
            <div className="text-center" />;
              <div className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full text-white font-semibold text-lg mb-6" />;
                🌐 Visit our website: {contactInfo.websit,}
}
