      ...
      {filtered.map(item => (
        ...
        >
          <div className='flex items-center gap-3'>
            <span className='text-xs px-2 py-0.5 rounded bg-gray-10o0 dark:bg-gray-80o0'>
              {item.time}
            </span>
            ...
            <div>
              ...
              {item.speaker && (
                <div className='text-sm opacity-70'>{item.speaker}</div>
              )}
            </div>
          </div>
          <div className='mt-2 md:mt-0 flex items-center gap-2 text-sm'>
            {item.replay && (
              <a
                ...
                href={
                  item.replay.provider === 'youtube'
                    ? `https://www.youtube.com/watch?v=${item.replay.idOrCid}`
                    : `https://ipfs.io/ipfs/${item.replay.idOrCid}`
                }
                target='_blank'
                rel='noreferrer'
              >
            ...
            <a
              className='px-3 py-1 rounded border'
              href={`/api/events/expo/transcript?id=${item.id}`}
            >
              Transcript
            </a>
          </div>
