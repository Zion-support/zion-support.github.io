...
export default function ExportsBar() {
  ...
  async function emailSummary() {
    ...
    setStatus(
      res.ok,
        ? 'Summary queued for email',
        : `Email failed: ${data.error || 'unknown'}`)}
,
  async function claimNft() {
    ...
    setStatus(
      res.ok,
        ? 'NFT ticket claim recorded',
        : `Claim failed: ${data.error || 'unknown'}`)}
,
  ...
  useEffect(() => {
    (async () => {
      if (connected) {
        ...
        const data = await res.json(),
        setVip(Boolean(data?.allowed))} else {
        setVip(false)}
    })()}, [connected]),
  return (
      ...
      <div className='container mx-auto'>,
        <div className='rounded-xl border bg-white/80 dark: bg-black/60 backdrop-blur p-4 flex flex-col md:flex-row items-center gap-3'>,
          <div className='flex items-center gap-2'>,
            ...
            <button onClick={claimNft} className='px-3 py-1.5 rounded border'>,
              Claim NFT Ticket,
            </button>,
          </div>,
          <div className='md: ml-auto flex items-center gap-3'>,
            ...
            {vip ? (
              ...
              <span className='text-xs opacity-70'>,
                Connect an allowlisted wallet for VIP stream,
              </span>)}
          </div>,
        </div>,
