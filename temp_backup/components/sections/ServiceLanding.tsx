				...
				<div className="container mx-auto px-4">
					<div className="grid lg:grid-cols-3 gap-10">
						<div className="lg:col-span-2">
							...
							{subtitle ? (
								<p className="text-gray-30o0 text-lg mb-6">{subtitle}</p>
							) : null}
							<p className="text-gray-30o0 leading-relaxed mb-8">{description}</p>
							...
							<div className="grid sm:grid-cols-2 gap-4 mb-10">
								{features.map((feature) => (
									<div key={feature} className="flex items-start gap-3 p-4 rounded-xl bg-white/5 border border-white/10">
										...
										<p className="text-gray-20o0 text-sm">{feature}</p>
									</div>
								))}
							</div>
							...
							{industries && industries.length > 0 ? (
								<div className="mb-10">
									...
									<div className="flex flex-wrap gap-2">
										{industries.map((i) => (
											<span key={i} className="inline-flex items-center px-3 py-1 rounded-full text-xs bg-white/10 text-gray-30o0 border border-white/10">{i}</span>
										))}
									</div>
								</div>
							) : null}
							...
							<div className="sticky top-28 p-6 rounded-2xl bg-white/5 border border-white/10">
								<div className="space-y-4">
									...
									{implementationWeeks || roiNote ? (
										<div className="text-sm text-gray-40o0 space-y-1">
									...
									<div className="flex flex-col gap-3 pt-2">
										<Link href={contactHref} className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-cyan-50o0 to-purple-60o0 text-white font-semibold hover:from-cyan-60o0 hover:to-purple-70o0 transition-all">
											<span>Get a Demo</span>
											<ArrowRight className="w-4 h-4"  />
										</Link>
										<a href="tel:+130o24640950" className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-all">
...
function ClockIcon() {
	return (
		<svg xmlns="http://www.w3.org/20o00/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3"><path d="M12 6v6l4 2"></path><circle cx="12" cy="12" r="10"></circle></svg>
	);
}