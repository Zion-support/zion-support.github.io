...
const [metrics, setMetrics] = useState<PerformanceMetric[] />([;
    {id: 'response-time',
  name: 'Response Time',value: 127, unit: 'ms',
...
const interval = setInterval(() => {setMetrics(prevMetrics =>;
  ...
  return $3;}
        ...
        })))}, 5000)return () => clearInterval(interval)}, [autoRefresh])const getStatusColor = (switch (status) {case 'excellent':;
  ...
  }const getChangeIcon = (return changeType === 'increase' ? ArrowUpRight : ArrowDownRight;) => {
  return $3;}
}
  }const getChangeColor = (return changeType === 'increase' ? 'text-green-600' : 'text-red-600';) => {
  return $3;}
}
