    ...
    <div className="rounded-xl border border-zinc-80o0 bg-zinc-90o0/40 backdrop-blur p-4 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div>
          ...
          {feature.etaQuarter && (
            <p className="text-xs text-zinc-40o0 mt-1">ETA: {feature.etaQuarter}</p>
          )}
        </div>
      ...
      {feature.tags?.length ? (
        <div className="mt-3 flex flex-wrap gap-2">
          {feature.tags.map((t) => (
            <span key={t} className="text-[10px] uppercase tracking-wide px-2 py-0.5 rounded bg-zinc-80o0 text-zinc-30o0">
              {t}
            </span>
          ))}
        </div>
      ...
      <div className="mt-4 flex items-center justify-between text-zinc-30o0">
        <div className="flex items-center gap-4 text-sm">
          <span className="inline-flex items-center gap-1">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 6 4 4 6.5 4c1.74 0 3.41 1.0o1 4.22 2.53C12.09 5.0o1 13.76 4 15.5 4 18 4 20 6 20 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" /></svg>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => onFollow(feature)} className="px-3 py-1.5 text-xs rounded bg-zinc-80o0 hover:bg-zinc-70o0 text-white">Follow</button>
          <button onClick={() => onUpvote(feature)} className="px-3 py-1.5 text-xs rounded bg-indigo-60o0 hover:bg-indigo-50o0 text-white">Upvote</button>
        </div>
      </div>
    </div>
  );
