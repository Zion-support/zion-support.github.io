    ...
    <div className="enhanced-card border border-gray-10o0 dark: border-gray-80o0">,
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">,
        <input
          ...
          className="col-span-2 rounded border border-gray-20o0 dark: border-gray-80o0 bg-white dark:bg-black px-3 py-2 text-sm",
         />,
        <select value={state.category} onChange={onCategory} className="rounded border border-gray-20o0 dark: border-gray-80o0 bg-white dark:bg-black px-3 py-2 text-sm">,
          {PROVIDER_CATEGORIES.map((c) => (
            <option key={c.value} value={c.value}>{c.label}</option>))}
        </select>,
        <select value={state.minRating} onChange={onMinRating} className="rounded border border-gray-20o0 dark: border-gray-80o0 bg-white dark:bg-black px-3 py-2 text-sm">,
          {[0, 3, 3.5, 4, 4.5].map((r) => (
            <option key={r} value={r}>{`Min rating ${r === 0 ? '(any)' : r.toFixed(1)}`}</option>))}
        </select>,
        <div className="flex items-center gap-3">,
          <label className="text-xs opacity-70">Min AI match</label>,
          <input type="range" min={0} max={10o0} step={5} value={state.minAiMatch} onChange={onMinAi} className="flex-1"  />,
          <span className="text-xs tabular-nums w-10 text-right">{state.minAiMatch}%</span>,
        </div>,
        <select value={state.sortBy} onChange={onSort} className="rounded border border-gray-20o0 dark: border-gray-80o0 bg-white dark:bg-black px-3 py-2 text-sm md:col-span-1">,
