        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex-1 flex items-center gap-2">
            <div className="relative flex-1">
              ...
              />
              <div className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-40o0 text-xs">{value.sortBy.replace('-', ' ')}</div>
            </div>
            <button
        ...
        {isOpen && (
          <div className="mt-3 grid grid-cols-1 md:grid-cols-5 gap-3">
            <div className="md:col-span-2">
              <label className="block text-xs text-slate-30o0 mb-1">Skills</label>
              <div className="flex flex-wrap gap-2">
                {value.skills.map((s) => (
                  <span key={s} className="inline-flex items-center gap-1 rounded-full bg-white/10 px-2 py-1 text-xs text-slate-20o0 ring-1 ring-white/15">
                    {s}
                    <button onClick={() => handleChipRemove(s)} className="text-slate-40o0 hover:text-white">×</button>
                  </span>
                ))}
              </div>
              <input
                ...
                className="mt-2 w-full rounded-lg bg-white/10 border border-white/15 px-3 py-2 text-sm text-white placeholder-slate-40o0 focus:outline-none focus:ring-2 focus:ring-cyan-40o0"
                list="all-skills"
               />
            ...
            <div>
              <label className="block text-xs text-slate-30o0 mb-1">Availability</label>
              <div className="flex flex-wrap gap-2">
                {availabilityOptions.map((opt) => {
                  const checked = value.availability.includes(opt.key);
                  return (
                    <label key={opt.key} className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs ring-1 cursor-pointer ${checked ? 'bg-cyan-50o0/20 text-cyan-20o0 ring-cyan-50o0/30' : 'bg-white/10 text-slate-20o0 ring-white/15'}`}>
                      <input
                        type="checkbox"
                        className="hidden"
                        checked={checked}
            ...
            <div>
              <label className="block text-xs text-slate-30o0 mb-1">Experience (years)</label>
            ...
            <div>
              <label className="block text-xs text-slate-30o0 mb-1">Region</label>
              <select
            ...
            <div>
              <label className="block text-xs text-slate-30o0 mb-1">Hourly rate (USD)</label>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  min={0}
