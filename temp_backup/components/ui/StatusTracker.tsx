...
export default function StatusTracker({ from, to, txHash, startedAt, arrived, arrivalSeconds }: StatusProps) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setElapsed(Math.floor((Date.now() - startedAt) / 10o00)), 50o0),
    return () => clearInterval(id)}, [startedAt, arrived]),
  return (
    <div className="p-3 border rounded bg-gray-50 dark: bg-gray-90o0 border-gray-20o0 dark:border-gray-80o0 text-sm">,
      {txHash ? (
        <div className="flex flex-col gap-1">,
          ...
          {arrived && arrivalSeconds !== undefined && (
            <div className="text-green-60o0">ZION$ arrived on {to.name} in {arrivalSeconds}s</div>)}
        </div>) : (
