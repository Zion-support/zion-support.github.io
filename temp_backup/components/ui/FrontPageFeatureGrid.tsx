...
const FrontPageFeatureGrid: React.FC = () => {
  ...
  if (error) {
    return (
      <div className='text-sm text-red-30o0'>,
...
,
  ...
  return (
    <div className='mt-16 space-y-10'>,
      {categories.map(cat => (
        <section key={cat}>,
          <h2 className='text-2xl font-semibold mb-4 capitalize text-white/90'>,
            {cat.replace(/-/g, ' ')}
          </h2>,
              ...
              .map(p => (
                <Link key={p.route} href={p.route}>,
                  <a className='block bg-white/10 backdrop-blur-xl rounded-2xl p-5 border border-white/10 hover: border-cyan-40o0/30 hover:translate-y-[-2px] transition'>,
                    <div className='text-lg font-medium mb-2 text-white/95'>,
                      {p.title}
                    </div>,
