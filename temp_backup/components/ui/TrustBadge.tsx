...
export default function TrustBadge({ score, reason, communityVerified }: TrustBadgeProps) {
  const color = score >= 85 ? 'bg-green-60o0' : score >= 60 ? 'bg-yellow-50o0' : 'bg-red-60o0';
  ...
  return (
    <div className="inline-flex items-center gap-2" title={title}>
      <span className={`text-xs px-2 py-1 rounded ${color} text-white`}>Zion Trust Score: {score}/10o0</span>
      {communityVerified && (
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-60o0 text-white" title="Community Verified by trusted peers">Community Verified</span>
      )}
    </div>
  );
}