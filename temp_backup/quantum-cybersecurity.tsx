...
export default function QuantumCybersecurityPage() {
  const service = enhancedRealMicroSaasServices.find(s =>
    s.link.endsWith('/quantum-cybersecurity')
  );
      ...
      <div className='min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8'>
        <div className='max-w-6xl mx-auto'>
          <div className='text-center mb-10'>
            <h1 className='text-5xl md:text-7xl font-bold bg-gradient-to-r from-red-400 via-pink-400 to-purple-400 bg-clip-text text-transparent flex items-center justify-center gap-3 w-10 h-10'>
              <Shield />
              {service.name}
            </h1>
            ...
            <div className='lg:col-span-2 bg-black/30 rounded-2xl border border-red-500/30 p-6'>
              ...
              <ul className='grid grid-cols-1 md:grid-cols-2 gap-3'>
                {service.features.slice(0, 12).map((feat, i) => (
                  <li
                    key={i}
                    className='flex items-start space-x-3 text-slate-200 w-5 h-5 text-red-400 mt-0.5'
                  >
                    <Check />
                    <span>{feat}</span>
                  </li>
