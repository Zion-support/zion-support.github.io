...
} catch (error) {; this.log(`Health check error: ${error.message}`)}};
; async checkResourceUsage() {; try {; // Check memory usage; const totalMem = os.totalmem(); const freeMem = os.freemem(); const usedMem = totalMem - freeMem; const memUsagePercent = (usedMem / totalMem) * 100;
...
; if (memUsagePercent > this.alertThreshold) {; this.log(`ALERT: Memory usage ${memUsagePercent.toFixed(2)}% exceeds threshold ${this.alertThreshold}%`)};
; // Check CPU usage; const cpus = os.cpus(); this.log(`CPU cores: ${cpus.length}`);
; // Check disk usage; try {; const diskUsage = execSync('df -h /', { encoding: 'utf8' }); this.log(`Disk usage: \n${diskUsage}`)} catch (error) {; this.log(`Disk usage check failed: ${error.message}`)};
} catch (error) {; this.log(`Resource monitoring error: ${error.message}`)}};
; async checkProcessHealth() {; try {; // Check PM2 processes; try {; const pm2List = execSync('pm2 list --json', { encoding: 'utf8' }); const processes = JSON.parse(pm2List);
; this.log(`PM2 processes: ${processes.length}`);
; processes.forEach(proc = > {; const status = proc.pm2_env?.status; const name = proc.name; const memory = proc.monit?.memory || 0; const cpu = proc.monit?.cpu || 0;
; this.log(` ${name}: ${status} (CPU: ${cpu}%, Memory: ${(memory / 1024 / 1024).toFixed(2)}MB)`);
; if (status ! = = 'online') {; this.log(`ALERT: Process ${name} is not online (status: ${status})`)}});
} catch (error) {; this.log(`PM2 process check failed: ${error.message}`)};
} catch (error) {; this.log(`Process monitoring error: ${error.message}`)}};
; async checkSystemStatus() {; try {; // Check system uptime; const uptime = os.uptime(); const days = Math.floor(uptime / 86400); const hours = Math.floor((uptime % 86400) / 3600); const minutes = Math.floor((uptime % 3600) / 60);
; this.log(`System uptime: ${days}d ${hours}h ${minutes}m`);
; // Check load average; const loadAvg = os.loadavg(); this.log(`Load average: ${loadAvg.map(load = > load.toFixed(2)).join(', ')}`);
; // Check if load is too high; const cpuCount = os.cpus().length; const currentLoad = loadAvg[0]; const loadPercent = (currentLoad / cpuCount) * 100;
; if (loadPercent > this.alertThreshold) {; this.log(`ALERT: Load average ${currentLoad.toFixed(2)} exceeds threshold (${cpuCount} cores)`)};
} catch (error) {; this.log(`System status check error: ${error.message}`)}};
...
; // Set up interval for periodic health checks; setInterval(async () = > {; await this.checkSystemHealth(); await this.checkApplicationHealth()}, 60 * 1000), // Every minute; // Generate health report every hour; setInterval(async () = > {; await this.generateHealthReport()}, 60 * 60 * 1000), // Every hour}};
;
// Start the service;
