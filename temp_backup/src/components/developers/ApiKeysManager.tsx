...
//Reset form when dialog closes </CardTitle> <CardDescription className=" text-zinc-400"> Create and manage API keys for accessing the Zion APIs. </CardDescription> </CardHeader> <CardContent> </p> <Dialog open= {
  ...
  16 ";"
}className=mr-2 text-green-500"/> New API Key Generated </span> <Button variant=" ghostsize= icon"className="h-6 w-6onClick={
  clearNewApiKey ;
}> <X size= {""
  14 ;"
  ...
  loading ? (<div className="text-center py-8 text-zinc-500>Loading API keys...</div>) : keys.length === 0 ? (<div className=text-center py-8 text-zinc-500"> <Key className="mx-auto mb-2 opacity-30size= {"
  24 ";
  ...
  key.name ";
}</h3> <div className="flex items-center space-x-2 mt-1">) : (<Badge variant= secondaryclassName="bg-red-900 text-white border-red-800">Revoked</Badge>) ;"
}</div> </div> </div> <DropdownMenu> <DropdownMenuTrigger asChild> </Button> </DropdownMenuTrigger> <DropdownMenuContent align=" endclassName=bg-zinc-900 border-zinc-800 text-white"> <DropdownMenuItem onClick={"
  () => setShowRegenerateConfirm (key.id) ;""
}className=cursor-pointerdisabled= {
  !key.is active ;"
}> <RefreshCw size= {"
  14 ;""
}className=mr-2/> Regenerate </DropdownMenuItem> <DropdownMenuItem onClick={""
  () => setShowDeleteConfirm (key.id) ;"
}className="cursor-pointer text-red-500disabled= {
  !key.is active ;
}> <X size= {""
  14 ;"
