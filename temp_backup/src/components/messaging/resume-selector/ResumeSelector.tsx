...
//Handle radio option change //Check if its a PDF file if (file.type !== 'application/pdf') {
  toast ({
  return;
...
}className=space-y-3" flex items-center space-x-2"> <RadioGroupItem value= recentid=" recent"/> <Label htmlFor= recentclassName=" text-white">Use most recent AI Resume</Label> </div> <div className= flex items-center space-x-2> <RadioGroupItem value=" select"id= select/> <Label htmlFor=" select"className= text-white>Select from saved versions</Label> </div> <div className=" flex items-center space-x-2"> <RadioGroupItem value= uploadid=" upload"/> <Label htmlFor= uploadclassName=" text-white">Upload a custom resume (PDF) </Label> </div> </RadioGroup> {
