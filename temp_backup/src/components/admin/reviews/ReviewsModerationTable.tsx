...
}</TableCell> <Button size= smvariant=" outline"className= h-8 w-8 p-0onClick={
  () => handleApprove (review.id) ;"
}disabled= {"
  isPending ;""
}> <Check className= h-4 w-4 text-green-500/> </Button> <Button > <X className=" h-4 w-4 text-red-500"/> </Button> </>) ;"
}<DropdownMenu> <DropdownMenuTrigger asChild> <Button variant=" ghostsize= sm"className=" h-8 w-8 p-0> <MoreHorizontal className= h-4 w-4"/> </Button> </DropdownMenuTrigger> Mark as approved </DropdownMenuItem>) ;"
