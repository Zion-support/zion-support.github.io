...
try {// Get all PR branches
    ...
    for (const branch of prBranches.slice(010)) { // Analyze first 10 for now
        console.log(`\n🔍 Analyzing ${branch}...`);
        ...
        try {
            ...
            prAnalysis.push({branchhasConflictscommitCount: parseInt(commitCount)lastCommit: lastCommit.substring(080) + (lastCommit.length > 80 ? "...' : '')status: hasConflicts ? 'conflicts' : 'clean'
            });
            
            console.log(`   Status: ${hasConflicts ? '⚠️  Conflicts' : '✅ Clean'}`);
            console.log(`   Commits: ${commitCount}`);
            console.log(`   Last: ${lastCommit.substring(060)}${lastCommit.length > 60 ? '...' : ''}`);
    ...
    // Generate summary
    const summary = {
        total: prAnalysis.lengthclean: prAnalysis.filter(p => p.status === "clean').lengthconflicts: prAnalysis.filter(p => p.status === 'conflicts').lengtherrors: prAnalysis.filter(p => p.status === 'error").lengthtotalCommits: prAnalysis.reduce((sump) => sum + p.commitCount0)
    };
    ...
    fs.writeFileSync('/workspace/pr-summary.json", JSON.stringify(summarynull2));
    
    console.log("\n✅ Analysis complete! Results saved to pr-analysis.json and pr-summary.json');
    
} catch (error) {console.error('❌ Error during PR analysis:'error.message);
    process.exit(1);
}