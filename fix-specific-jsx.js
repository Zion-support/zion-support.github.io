...
function fixSpecificJSX(filePath) {
  try {
    ...
    content = content.replace(/<div className="([^"]*)"[^>]*><\/div>\s*<div className="([^"]*)"[^>]*><\/div>/g, 
      '<div className="$1">\n        <div className="$2">');
    ...
    const patterns = [
      // Fix h1 tags
      {
        pattern: /<h1 className="([^"]*)"[^>]*><\/h1>\s*([^<]+)\s*<\/h1>/g,
        replacement: '<h1 className="$1">\n            $2\n          </h1>'
      },
      // Fix p tags
      {
        pattern: /<p className="([^"]*)"[^>]*><\/p>\s*([^<]+)\s*<\/p>/g,
        replacement: '<p className="$1">\n            $2\n          </p>'
      }
    ];
