    ...
    const req = https.get(options, res => {
      ...
      res.on('end', () => {
        try {
          ...
          // Save PR data
          fs.writeFileSync('/workspace/open_prs_data.json', JSON.stringify(prs, null, 2));
...
function getBranchesToMerge() {
  ...
  const unmergedBranches = branches.filter(branch => {
    const result = exec(`git log origin/main..origin/${branch} --oneline`, { ignoreError: true });
...
function resolveMergeConflicts() {
  ...
  const conflictFiles = exec('git diff --name-only --diff-filter=U', { ignoreError: true });
  ...
  // Commit the merge
  const commitResult = exec('git commit --no-edit --no-verify', { ignoreError: true });
...
function mergeBranch(branchName) {
  ...
  try {
    ...
    // Attempt merge
    const mergeResult = exec(`git merge origin/${branchName} --no-ff -m "Merge branch '${branchName}' into main"`, { ignoreError: true });
...
async function main() {
  ...
  try {
    ...
    log(`\n🔍 Linting: ${improvements.linting ? '✅ Passed' : '❌ Issues found'}`, improvements.linting ? 'green' : 'yellow');
