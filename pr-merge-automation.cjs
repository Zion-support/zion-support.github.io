...
class PRMergeAutomation {
  ...
  async mergePR(pr) {
    ...
    try {
      ...
      await this.runCommand(
        ...
        `Checkout PR #${pr.number} branch`
      );

      // Pull latest changes
      await this.runCommand(
        `git pull origin ${pr.branch}`,
        `Pull latest changes for PR #${pr.number}`
      );
      ...
      try {
        ...
        if (
          ...
          await this.runCommand(
            'node resolve-all-merge-conflicts-comprehensive.cjs',
          ...
          this.log(
            `✅ Successfully resolved conflicts and merged PR #${pr.number}`
          );
          this.mergedPRs.push(pr);
        }
      } catch (conflictError) {
        this.log(
          `❌ Failed to resolve conflicts for PR #${pr.number}: ${conflictError.message}`
        );
  ...
  async deleteBranch(pr) {
    ...
    try {
      ...
      await this.runCommand(
        ...
        `Delete local branch ${pr.branch}`
      );

      // Delete remote branch
      await this.runCommand(
        `git push origin --delete ${pr.branch}`,
        `Delete remote branch ${pr.branch}`
      );
  ...
  async runComprehensiveImprovements() {
    ...
    try {
      ...
      await this.runCommand(
        'node comprehensive-automation-suite-clean.cjs',
        'Comprehensive Automation Suite'
      );

      // Run final improvements
      await this.runCommand(
        'node final-comprehensive-improvements.cjs',
  ...
  async run() {
    ...
    try {
      ...
      if (this.failedPRs.length === 0) {
        ...
        this.log(
          'The main branch now contains all the latest changes and improvements.'
        );
      } else {
      ...
      this.log(
        `🚨 An error occurred during PR merge automation: ${error.message}`
      );
    }
  }
}
