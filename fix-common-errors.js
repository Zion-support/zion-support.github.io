...
function fixFile(filePath) {
  try {
    ...
    if (content.includes('Database') && !content.includes("import { Database }")) {
      content = content.replace(
        /import { Helmet } from 'react-helmet-async';/,
    ...
    // Fix export names
    content = content.replace(/export default Ai(\w+)Page;/g, 'export default AI$1Page;');
    content = content.replace(/export default It(\w+)Page;/g, 'export default IT$1Page;');
    content = content.replace(/export default MicroSaasPage;/g, 'export default MicroSAASPage;');
