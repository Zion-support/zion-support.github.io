...
def fix_lucide_syntax(file_path):
    ...
    try:
        ...
        print(f"Fixed lucide syntax in: {file_path}")
        return True
...
def main():
    ...
    for file_path in tsx_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                if "from 'lucide-react'" in content and "import " in content and "{" not in content:
                    if fix_lucide_syntax(file_path):
                        fixed_count += 1
        except:
            continue
