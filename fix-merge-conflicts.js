...
function cleanMergeConflicts(content) {
  ...
  cleaned = cleaned.replace(/  cleaned = cleaned.replace(/  cleaned = cleaned.replace(/
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (content.includes("          const cleaned = cleanMergeConflicts(content);"
...
try {
  ...
  execSync("cd /workspace && npx tsc --noEmit --skipLibCheck", { stdio: "inherit"});"
  console.log("TypeScript check passed!")} catch (error) {"
  console.log("TypeScript check found issues, but continuing...")}"
