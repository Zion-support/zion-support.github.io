...
const conflictRegex = /[\s\S]*?[\s\S]*?    if (conflictRegex.test(content)) {
      ...
      content = content.replace(conflictRegex, '')''      modified = true
    }
...
const fixes = [
      ...
      // Fix missing closing tags in JSX
      {
        ...
        pattern: /<meta\s+[^>]*name="[^"]*"[^>]*>/g,""        replacement: (match) => {
  
          if (!match.includes('/>') && !match.includes('</meta>')) {''            return match.replace(/>$/, ' />')''          }
          return match
        }
      },
      // Fix unclosed JSX elements
      {
        ...
        replacement: (match, tagName, attributes) => {
          ...
          if (match.endsWith('/>') || ['img', 'br', 'hr', 'input', 'meta', 'link'].includes(tagName)) {''            return match
          }
          return match
        }
...
const newContent = content.replace(fix.pattern, fix.replacement)
    ...
    if (modified) {
      ...
      fs.writeFileSync(filePath, content, 'utf8')''      console.log(`Fixed parsing errors in: ${filePath}`)
      return true
    }
...
// Main execution
console.log('Starting merge conflict and parsing error fixes...');';'const appDir = path.join(__dirname, 'app');';'const files = findFiles(appDir);
let fixedCount = 0;
let errorCount = 0
