...
function fixMergeConflicts(filePath) {try {
    ...
    for (let i = 0; i < lines.length; i++) {const line = lines[i];
      ...
      if (inConflict) {if (conflictType === 'head') {'
          headContent.push(line)} else if (conflictType === 'other') {'
          otherContent.push(line)}
      } else {result.push(line)}
    }
    
    // Clean up any remaining content
    if (headContent.length > 0) {result.push(...headContent)}
...
function findFilesWithConflicts(dir) {const files = []
...
if (conflictedFiles.length === 0) {console.log('✅ No merge conflicts found!');'
  process.exit(0)}
...
let fixedCount = 0;
for (const file of conflictedFiles) {if (fixMergeConflicts(file)) {
    fixedCount++}
}
...
} catch (error) {console.log('❌ Build failed. There may be other issues to resolve.');'
  process.exit(1)}