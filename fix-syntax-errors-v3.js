...
function fixSyntaxErrors(content) {
  ...
  content = content.replace(/import\s+(\w+)\s+from\s+"(\w+)";\s*import\s+{\s*([^}]+)\s*}\s+from\s+"(\w+)";/g, 
    'import $1 from "$2";\nimport { $3 } from "$4";');
  ...
  content = content.replace(/const\s+(\w+)\s*=\s*\(\s*\)\s*=>\s*{\s*return\s*\(\s*>/g, 'const $1 = () => {\n  return (\n    <');
  ...
  content = content.replace(/const\s+(\w+)\s*:\s*new\s+Date\(\)\.getFullYear\(\);/g, 'const $1 = new Date().getFullYear();');
  
  // Fix malformed object properties
  content = content.replace(/name="([^"]*)"\s*,\s*href="([^"]*)"\s*}/g, 'name: "$1", href: "$2" }');
...
function rewriteProblematicFile(content, filePath) {
  ...
  if (content.includes('import React from "react"; import { Helmet } from react-helmet-async; const PagePage = () =>{ return (</div>')) {
  ...
  for (const pattern of patterns) {
    ...
    for (const file of files) {
      try {
        ...
        if (content.includes('import React from "react"; import { Helmet } from react-helmet-async; const PagePage = () =>{ return (</div>') ||
            content.includes('import React from "react"; import { Link } from react-router-dom; import { Mail, Phone, MapPin, Github, Twitter, Linkedin, Brain, ArrowRight } from lucide-react; const Footer = () =>{ const currentYear : new Date().getFullYear(); const services : [ { name="AI" Solutions, href="/ai-solutions" }') ||
            content.includes('import React from "react"; import { Helmet } from react-helmet-async; export default function Page() { return (<React.Fragment><Helmet><title>5G Implementation - Zion Tech Group</title><meta name="description" content="Professional" 5G implementation services by Zion Tech Group. /></Helmet><div><div><h1 className="text-4xl" font-bold text-white mb-4>5G Implementation</h1><p className="text-gray-300">Coming soon...</p></div></div></React.Fragment>); }</div>')) {
