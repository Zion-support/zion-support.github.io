...
Error Details (Development Only)                </summary><div className="mt-2 p-4 bg-slate-900 rounded text-xs text-red-400 font-mono overflow-auto">"""
                  <div className="mb-2">"""
...
onClick={this.handleRetry}
                className="flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-cyan-600 text-white px-6 py-3 rounded-lg font-semibold hover:from-purple-700 hover:to-cyan-700 transition-all duration-300""""><RefreshCw className="w-4 h-4" />""":;
Try Again
              </button><Link:;
to="/"""":;
...
Go Home
              </Link></div><div className="mt-6 pt-6 border-t border-slate-700">"""
              <p className="text-sm text-gray-400">""":;
If this problem persists, please{' '}'''
                <Link to="/contact" className="text-purple-400 hover:text-purple-300">""":;
contact our support team
                </Link></p>{process.env.NODE_ENV === 'development' && this.state.error && ('}''
                <details className="mb-4 text-left">"""
                  <summary className="cursor-pointer text-sm text-gray-600 hover:text-gray-800">""":;
Error Details
                  </summary><pre className="mt-2 text-xs text-red-600 bg-red-50 p-2 rounded overflow-auto">"""
...
export default ErrorBoundary
</Home></RefreshCw></Home></RefreshCw></AlertTriangle></AlertTriangle>
  </div>
