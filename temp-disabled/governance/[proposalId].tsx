...
const ProposalDetailPage: React.FC = () => {
  ...
  const fetchProposalData = useCallback(async () => {
    ...
    try {
      ...
      if (!proposalRes.ok) {
        ...
        throw new Error(
          errData.detail || `Failed to fetch proposal: ${proposalRes.status}`
        );
      }
      ...
      if (resultsRes.ok) {
        ...
        logWarn('Could not fetch vote results:', {
          data: errData.detail || resultsRes.status,
        });
  ...
  useEffect(() => {
    if (router.isReady && proposalId) {
      // Ensure router is ready and proposalId is available
      fetchProposalData();
    }
  ...
  useEffect(() => {
    if (proposal && connectedWalletAddress) {
      const existingVote = proposal.votes.find(
        v =>
          v.voter_wallet_address?.toLowerCase() ===
            connectedWalletAddress.toLowerCase() ||
          (v.voter &&
            typeof v.voter === 'object' &&
            'id' in v.voter &&
            (v.voter as any).id ===
              /* get platform user id if available */ null)
      );
  ...
  const handleVoteSubmit = async (choice: 'APPROVE' | 'REJECT' | 'ABSTAIN') => {
    ...
    if (
      !tempVotingPower ||
      isNaN(parseFloat(tempVotingPower)) ||
      parseFloat(tempVotingPower) <= 0
    ) {
    ...
    try {
      const response = await fetch(
        `/api/governance/proposals/${proposal.id}/submit_vote/`,
        {
          method: 'POST',
          headers: {
  ...
  const canVote =
    ...
    (!proposal.voting_starts_at ||
      new Date(proposal.voting_starts_at) <= new Date()) &&
    (!proposal.voting_ends_at ||
      new Date(proposal.voting_ends_at) >= new Date()) &&
    !userVote;

  const totalPowerForProgress = results
    ? parseFloat(results.approve_power) +
      parseFloat(results.reject_power) +
      parseFloat(results.abstain_power)
    : 0;
  ...
  return (
    ...
    <div className='container mx-auto p-4 space-y-6'>
      <Link href='/governance'>
        <Button variant='outline'>&larr; Back to Proposals</Button>
      </Link>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className='text-xs sm:text-sm'>
            Proposed by: {proposal.proposer?.username || 'N/A'} | Type:{' '}
            {proposal.proposal_type.replace(/_/g, ' ')} <br />
            Created: {new Date(proposal.created_at).toLocaleString()}
            {proposal.voting_starts_at && (
              <>
                {' '}
                <br className='sm:hidden' />| Voting Starts:{' '}
                {new Date(proposal.voting_starts_at).toLocaleString()}
              </>
            )}
            {proposal.voting_ends_at && (
              <>
                {' '}
                <br className='sm:hidden' />| Voting Ends:{' '}
        ...
        <CardContent className='space-y-4'>
          ...
          {proposal.quorum_percentage !== null &&
            proposal.quorum_percentage !== undefined && (
              <div>
                <h3 className='font-semibold'>Quorum:</h3>
                <p className='text-muted-foreground'>
                  {proposal.quorum_percentage * 100}%
                </p>
              </div>
            )}
          {proposal.funding_ask_amount && proposal.funding_ask_token_symbol && (
            <div>
              <h3 className='font-semibold'>Funding Ask:</h3>
              <p className='text-muted-foreground'>
                {proposal.funding_ask_amount}{' '}
                {proposal.funding_ask_token_symbol}
              </p>
            </div>
          )}
        ...
        <Card>
          ...
          <CardContent className='space-y-3'>
            {canVote ? (
              <>
                <div className='mb-4'>
                  ...
                  <p className='text-xs text-muted-foreground mt-1'>
                    Note: This is a temporary field. In a full system, your
                    ZION$ balance at a specific snapshot block would be used.
                  </p>
                </div>
                <div className='flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3'>
                  <Button
                    onClick={() => handleVoteSubmit('APPROVE')}
                    disabled={isVoting}
                    className='flex-1'
                  >
                  ...
                  <Button
                    onClick={() => handleVoteSubmit('REJECT')}
                    disabled={isVoting}
                    variant='destructive'
                    className='flex-1'
                  >
              ...
              <p>
                You voted: <Badge variant='outline'>{userVote.choice}</Badge>{' '}
                (Voting Power: {userVote.voting_power_at_snapshot} ZION$)
              </p>
            ) : (
              <p className='text-muted-foreground'>
                {proposal.status !== 'ACTIVE'
            ...
            {voteError && (
              <p className='text-red-600 text-sm mt-2'>{voteError}</p>
            )}
          </CardContent>
        </Card>
      )}
      {!isConnected && (
        <Card className='text-center p-6'>
          ...
          <CardDescription className='my-2'>
            To cast your vote or see your voting status, please connect your
            wallet.
          </CardDescription>
      ...
      {results ? (
          ...
          <CardContent className='space-y-4'>
            <div>
              <div className='flex justify-between mb-1 text-sm'>
                <span>Approve</span>
                <span>
                  {results.approve_power} ZION$ ({results.approve_count} votes)
                </span>
              </div>
              <Progress
                value={
                  totalPowerForProgress > 0
                    ? (parseFloat(results.approve_power) /
                        totalPowerForProgress) *
                      100
                    : 0
                }
            ...
            <div>
              <div className='flex justify-between mb-1 text-sm'>
                <span>Reject</span>
                <span>
                  {results.reject_power} ZION$ ({results.reject_count} votes)
                </span>
              </div>
              <Progress
                value={
                ...
                className='w-full h-3 bg-destructive/30'
            ...
            <div>
              <div className='flex justify-between mb-1 text-sm'>
                <span>Abstain</span>
                <span>
                  {results.abstain_power} ZION$ ({results.abstain_count} votes)
                </span>
              </div>
              <Progress
                value={
                ...
                className='w-full h-3 bg-secondary/30'
                indicatorClassName='bg-secondary-foreground'
              />
            </div>
            <p className='text-sm text-muted-foreground pt-2'>
              Total Votes Cast: {results.total_votes_cast}
            </p>
            <p className='text-sm text-muted-foreground'>
              Total Voting Power Cast: {results.total_voting_power_cast} ZION$
            </p>
          </CardContent>
        </Card>
      ) : (
        <Card className='text-center p-6'>
          <CardDescription>
            Vote results are not yet available or could not be loaded.
          </CardDescription>
        </Card>
      )}
    </div>
