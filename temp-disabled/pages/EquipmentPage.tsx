...
export default function EquipmentPage() {
  ...
  const [equipment, setEquipment] = useState<ProductListing[] | undefined>(
    undefined
  );
  const { user } = useAuth();
    ...
    async (
      ...
      if (!res.ok) {
        ...
        const errorData = await res
          ...
          .catch(() => ({
            message:
              'Failed to fetch recommendations, and error response is not JSON.',
          }));
  ...
  const handleRecommendations = async () => {
    ...
    try {
      ...
      const data: ProductListing[] = await fetchRecommendations({
        userId: user.id,
      });
      ...
      toast({
        title: err.message || 'Failed to load recommendations',
        variant: 'destructive',
      });
    }
  };
  ...
  if (delayedError && (!equipment || equipment.length === 0)) {
    return (
      <div
        data-testid='error-state-equipment'
        className='py-12 text-center space-y-4'
      >
        <p className='text-red-400'>
          Failed to load equipment: {delayedError.message}
        </p>
        <Button
          data-testid='retry-button-equipment'
      ...
      <ErrorBoundary
        fallback={
          <p className='text-red-500 text-center'>
            Could not load equipment content. Please try again later.
          </p>
        }
      >
