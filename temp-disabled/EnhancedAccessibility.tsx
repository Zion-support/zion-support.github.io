...
title="Accessibility Options""""">♿ A11y
      </button><div className={`accessibility-panel ${isVisible ? 'visible' : ''}`}>''''`
...
checked={settings.highContrast}
                ...
                className="rounded"""""
              /><span>High Contrast</span></label></div><div><label className="block text-sm font-medium mb-2">Font Size</label>""""
...
value={size}
            ...
            </div></div><div><p className="text-sm text-gray-300">"""":;
Screen Reader: {settings.screenReader ? 'Detected' : 'Not detected'}''''
            </p><p className="text-sm text-gray-300">"""":;
Reduced Motion: {settings.reducedMotion ? 'Enabled' : 'Disabled'}''''
            </p></div><button:;
onClick={toggleVisibility}
            className="w-full bg-purple-600 text-white py-2 px-4 rounded hover:bg-purple-700 transition-colors""""">Close
          </button></div></div></>)
}
