...
const trackPerformance = (if (typeof window.gtag !== 'undefined' && 'performance' in window) ;
  ...
  return $3;}
        ...
        if (perf) {const loadTime = (perf.loadEventEnd || 0) - (perf.fetchStart || 0)window.gtag('event', 'page_load_time', {event_category: 'Performance',}
  event_label: 'Page Load',value: Math.round(loadTime)})}
      }
   ,
...
export const trackEvent = (if (typeof window !== 'undefined' && typeof window.gtag !== 'undefined') {window.gtag('event', action, {event_category: category,event_label: label,value: value,) => {
  return $3;}
})};
}
