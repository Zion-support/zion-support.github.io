...
},const removeExperience = (id: string,) => {setExperiences(experiences && experiences.filter(exp => exp && exp.id !== id);
}

const removeEducation = (id: string,) => {setEducations(educations && educations.filter(edu => edu && edu.id !== id))}
;
  const removeSkill = (id: string,) => {setSkills(skills && skills.filter(skill => skill && skill.id !== id))}})})}";
