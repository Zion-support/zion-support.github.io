...
;
  const removeExperience = (setExperiences(experiences.filter(exp => exp.id !== id))) => {
  return $3;}
}})}Select;
  SelectContent;
  SelectItem;
...
},const removeExperience = (setExperiences(experiences && experiences.filter(exp => exp && exp.id !== id);) => {
  return $3;}
}
}

const removeEducation = (setEducations(educations && educations.filter(edu => edu && edu.id !== id))) => {
  return $3;}
}
;
  const removeSkill = (setSkills(skills && skills.filter(skill => skill && skill.id !== id))) => {
  return $3;}"
}})})}\";"
