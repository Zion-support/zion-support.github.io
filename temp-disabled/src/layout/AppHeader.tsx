...
const [mobileMenuOpen, setMobileMenuOpen] = useState(false);}
  const [loginOpen, setLoginOpen] = useState(false);}
  const isMobile = useIsMobile()const { t } = useTranslation()const { user } = useAuth()const isLoggedIn = useSelector((state: RootState,) => state.auth.isLoggedIn)const router = useRouter(;
  const showTagline = router.pathname === '/';
  // Messaging context (unread message count)const { unreadCount } = useMessaging()const openLoginModal = (// The actual;
  returnToPath is set in the URL by the child components (ResponsiveNavigation, MobileMenu)// using router.push with shallow:true before this function is called.;
...
const [mobileMenuOpen, setMobileMenuOpen] = useState(false),const [loginOpen, setLoginOpen] = useState(false),const isMobile = useIsMobile(),const { t } = useTranslation(),const { user } = useAuth(),const isLoggedIn = useSelector((state: RootState,) => state && state.auth.isLoggedIn),const router = useRouter(),const showTagline  = router && router.pathname === '/',// Messaging context (unread message count)const { unreadCount }  = useMessaging();
  const openLoginModal = (// The actual;
  returnToPath is set in the URL by the child components (ResponsiveNavigation, MobileMenu)// using router && router.push with shallow:true before this function is called.;
    // This function's main job is just to open the modal.;
    // If a returnToPath is passed, we could potentially use it for other logic here if needed in the future.;) => {
  return $3;}
}
      ...
      <header;
       ...
       />;"
        <div className=\"container flex h-16 items-center px-4 sm:px-6\" />;
          ...
          {!isLoggedIn && (<div className=\"ml-4 relative z-10 flex items-center\" />;<Link;"
                href=\"/auth/login\";}"
                className=\"text-sm font-medium text-foreground/70 hover:text-foreground\";}
                aria-label={t('auth.login')}"
                data-testid=\"login-link\";
                onClick = {(e) = /> {e.preventDefault(),// For the main login link, we might not have a specific;
  returnTo beyond current page,// or we could default to dashboard.;}
                  // For consistency with how sub-menus now set it:;}
                  router.push({ pathname: '/auth/login'}
  query: { returnTo: router.asPath }
}, undefined, { shallow: true}
})openLoginModal(router.asPath)onClick = {(e,) => {e && e.preventDefault(),// For the main login link, we might not have a specific;
  returnTo beyond current page,// or we could default to dashboard.;}
                  // For consistency with how sub-menus now set it:;}
                  router && router.push({ pathname: '/auth/login'}
  query: { returnTo: router && router.asPath }
}, undefined, { shallow: true}
          ...
          {/* User avatar menu */}"
          {isLoggedIn && (<div className=\"ml-4\" />;
              <UserMenu />;}
            </div>;}
          )}</div>;
      </header>;{/* Mobile menu - positioned outside of header to prevent overlap issues */}"
...
const [mobileMenuOpen, setMobileMenuOpen] = useState (false),const [login_open, setLoginOpen] = useState (false),const is_mobile = useIsMobile (),const { t } = use_translation (),const { user } = use_auth (),const isLoggedIn = use_selector ((state: RootState, ) => state.auth.isLoggedIn),const router = use_router (),const show_tagline = router.pathname === '/',// Messaging context (unread message count)const { unread_count } = use_messaging ();
  const openLoginModal = (returnToPath?: string, ) =>: any {// The actual;
  returnToPath is set in the URL by the child components (ResponsiveNavigation, MobileMenu)// using router.push with shallow:true before this function is called.;
    // This function's main job is just to open the modal.;}
    // If a returnToPath is passed, we could potentially use it for other logic here if needed in the future.;}
    setLoginOpen (true)},return (<>;
      <header;"
       ...
       />;"
        <div className=\"container flex h - 16 items - center px - 4 sm:px - 6\" />;
          <Logo />;"
          {show_tagline && (<span className=\"ml - 4 hidden text - sm text - muted - foreground md:inline\" />;}
              {t ('home.header_tagline')}
            </span>)}"
          <div className=\"ml - 6 flex - 1 hidden md:block\" />;"
            <nav role=\"navigation\" aria - label=\"Main navigation\" />;
          ...
          <PointsBadge />;"
          {!isLoggedIn && (<div className=\"ml - 4 relative z - 10 flex items - center\" />;
              <Link;"
                href=\"/auth / login\";}"
                className=\"text - sm font - medium text - foreground / 70 hover:text - foreground\";}
                aria - label = {t ('auth.login') }"
                data - testid=\"login - link\";
                on_click = {(e, ) = /> {e.prevent_default (),// For the main login link, we might not have a specific;
  return_to beyond current page,// or we could default to dashboard.;}
                  // For consistency with how sub - menus now set it:;}
                  router.push ({ pathname: '/auth / login'}
  query: { return_to: router.as_path }
}, undefined, { shallow: true}
...
"
        <div className=\"container-responsive\" />\";"
          <div className=\"flex h-20 items-center justify-between\" />;"
            ...
            </div>;{/* Desktop Navigation */}            <nav className=\"hidden lg:flex items-center space-x-8\" />;"
...
"
            ...
            {/* Solutions Dropdown */}<div className=\"relative dropdown-container\" />;<button;
                ...
                aria-expanded={activeDropdown === 'solutions'}</div>;
                            </div>;
                          </Link>))}
                      </div>;
                    </div>;
...
"
                ...
                {activeDropdown === 'solutions' && (<motion&& motion.div;}
                    initial={{ opacity: 0, y: -10 }
}
                    animate={{ opacity: 1, y: 0 }
}
                    ...
                    <div className=\"p-4\" />;"
                      <div className=\"grid grid-cols-1 gap-2\" />;
                        {solutions && solutions.map((solution) => (<Link;}
...
"
            ...
            {/* Resources Dropdown */}<div className=\"relative dropdown-container\" />;<button;
                onClick={() = /> toggleDropdown('resources')}"
                className=\"flex items-center px-3 py-2 rounded-md text-sm font-medium text-zion-slate-light hover:text-zion-cyan hover:bg-zion-cyan/10 transition-colors\";
                aria-expanded={activeDropdown === 'resources'}</div>;
                            </div>;
                          </Link>))}
                      </div>;
                    </div>;
            ...
            <div className=\"relative dropdown - container\" />;
              ...
              <AnimatePresence />;
                ...
                {activeDropdown === 'resources' && (<motion&& motion.div;}
                    initial={{ opacity: 0, y: -10 }
}
                    animate={{ opacity: 1, y: 0 }
}
                    ...
                    <div className=\"p-4\" />;"
                      <div className=\"grid grid-cols-1 gap-2\" />;
                        {resources && resources.map((resource) => (<Link;}
            ...
            {/* Company Dropdown */}<div className=\"relative dropdown-container\" />;<button;
            ...
            <div className=\"relative dropdown - container\" />;
              ...
              <AnimatePresence />;
                ...
                {activeDropdown === 'company' && (<motion&& motion.div;}
                    initial={{ opacity: 0, y: -10 }
}
                    animate={{ opacity: 1, y: 0 }
}
          ...
          <div className=\"flex items-center space-x-4\" />;"
            {/* Search */}<div className=\"relative hidden md:block\" />;"
              <form onSubmit={handleSearch} className=\"relative\" />;<input;"
                  ...
                  className={`w-64 px-4 py-2 pl-10 bg-zion-slate/20 border border-zion-cyan/20 rounded-lg text-white placeholder-zion-slate-light focus:outline-none focus:ring-2 focus:ring-zion-cyan/50 focus:border-zion-cyan transition-all duration-200 ${searchFocused ? 'w-80' : '';}"
            <div className=\"relative hidden md:block\" />;}"
              <form on_submit={handle_search} className=\"relative\" />;
                <input;"
                  ...
                  className={`w - 64 px - 4 py - 2 pl - 10 bg - zion - slate / 20 border border - zion - cyan / 20 rounded - lg text - white placeholder - zion - slate - light focus:outline - none focus:ring - 2 focus:ring - zion - cyan / 50 focus:border - zion - cyan transition - all duration - 200 ${search_focused ? 'w - 80' : '';}
}`}
                />;"
                <Search className=\"absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-zion-slate-light\" />;
              </form>;
            </div>;"
            {/* User Menu */}{user ? (<div className=\"relative dropdown-container\" />;<button;}
                  onClick={() = /> toggleDropdown('user')}"
                  className=\"flex items-center space-x-2 p-2 rounded-lg hover:bg-zion-cyan/10 transition-colors\";"
                  aria-expanded={activeDropdown === 'user'}<div className=\"relative dropdown - container\" />;
                <button;
                  ...
                  <span className=\"text - white text - sm hidden lg: block\" />{user.nam}
}</span>;"
                ...
                <AnimatePresence />;
                  ...
                  <span className=\"text-white text-sm hidden lg: block\" />{user && user.nam}
}</span>;"
                  <ChevronDown className=\"w-4 h-4 text-zion-slate-light\" />;
                </button>;<AnimatePresence />;
                  {activeDropdown === 'user' && (<motion&& motion.div;}
                      initial={{ opacity: 0, y: -10 }
}
                      animate={{ opacity: 1, y: 0 }
}
          ...
          {/* Right side actions */}"
          <div className=\"flex items-center space-x-4\" />;"
            {/* Search */}<form on_submit = {handle_search} className=\"hidden md:flex relative\" /> ;
  const navigation = [
  { name: 'Home'}
  href: '/', current: true}
},{ name: 'About'}
  ...
  href: '/contact', current: false}
}];

const services = [
  { name: 'AI Solutions'}
  href: '/services / ai', description: 'Machine Learning & NLP'}
},{ name: 'Tech Talent'}
  href: '/talent', description: 'Expert Developers & Engineers'}
},{ name: 'Equipment'}
  href: '/equipment', description: 'Infrastructure & Hardware'}
},{ name: 'Consulting'}
  href: '/consulting', description: 'Digital Transformation'}
},{ name: 'Cybersecurity'}
  href: '/services / cybersecurity', description: 'Security & Compliance'}
},{ name: 'Cloud Services'}
  href: '/services / cloud', description: 'DevOps & Infrastructure'}
}];return (<>;"
      <header className=\"sticky top-0 z-50 w-full border-b border-slate-700/20 bg-slate-900/95 backdrop-blur-md\" />;"
        <div className=\"container flex h-16 items-center px-4 sm:px-6\" />;"
          {/* Logo */}<form onSubmit = {handleSearch} className=\"hidden md:flex relative\" /> ;
  const navigation = [
  { name: 'Home'}
  href: '/', current: true}
},{ name: 'About'}
      ...
      <header className=\"sticky top-0 z-50 w-full border-b border-slate-700/20 bg-slate-900/95 backdrop-blur-md\" />;"
        <div className=\"container flex h-16 items-center px-4 sm:px-6\" />;
                    ...
                    <div className=\"grid grid - cols - 1 gap - 2\" />;
                      {services.map ((service) => (<Link;}
                          ...
                          <div className=\"flex - 1\" />;"
                            <div className=\"text - white font - medium group - hover:text - cyan - 400 transition - colors\" />;{service.name}
                            </div>;"
          ...
          <div className=\"ml-6 flex items-center space-x-4\" />;
            ...
            {/* User menu */}<button className=\"p - 2 text - slate - 400 hover:text - cyan - 400 transition - colors\" />;"
              <User className=\"h - 5 w - 5\" />;
            </button>;"
              {mobileMenuOpen ? (<X className=\"w - 6 h - 6 text - white\" />) : (<Menu className=\"w - 6 h - 6 text - zinc - 400\" />              className=\"md:hidden p - 2 text - slate - 400 hover:text - cyan - 400 transition - colors\">;}"
              ...
              <div className=\"mt-6 pt-6 border-t border-zinc-800/50\" />;"
                <div className=\"grid grid-cols-1 gap-3\" />;
                  {quickActions.map((action) => (<Link;}
                      ...
                      {action.name}        {/* Mobile Navigation */}{mobileMenuOpen && (<div className=\"md:hidden\" />;"
            <div className=\"px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-slate-800/95 border-t border-slate-700/20\" />;}
              {navigation.map((item) => (<Link;}
                  key={item.name}{/* Mobile Navigation */}
      <AnimatePresence />;
        {mobileMenuOpen && (<motion&& motion.div;}
            initial={{ opacity: 0, height: 0 }
}
            animate={{ opacity: 1, height: 'auto' }
}
            ...
            <div className=\"container mx-auto px-6 py-6\" />              {/* Mobile Search */}"
              <form onSubmit={handleSearch} className=\"relative\" />;
                ...
                />;"
                <Search className=\"absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-zion-slate-light\" />;
              </form>;{/* Mobile Navigation Links */}"
              <div className=\"space-y-2\" />;
                {navigation && navigation.map((item) => (<Link;}
                    key={item && item.name}
                    to={item && item.href}
                    onClick={closeMobileMenu}
                    className={`block px-3 py-2 rounded-md text-base font-medium transition-colors ${item && item.current;
                        ? 'text-zion-cyan bg-zion-cyan/10';}
                        : 'text-zion-slate-light hover:text-zion-cyan hover:bg-zion-cyan/10';}
}`} />
              ...
              <div className=\"mt-6 pt-6 border-t border-zinc-800/50\" />;"
                <div className=\"grid grid-cols-1 gap-3\" />;
                  {quickActions && quickActions.map((action) => (<Link;}
        ...
        {mobileMenuOpen && (<div className=\"md:hidden\" />;"
            <div className=\"px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-slate-800/95 border-t border-slate-700/20\" />;}
              ...
              {/* Mobile Services */}<div className=\"px-3 py-2\" />;"
              ...
              {/* Mobile Auth */}"
              {!user ? (<div className=\"pt-4 border-t border-zion-slate/20\" />;"
