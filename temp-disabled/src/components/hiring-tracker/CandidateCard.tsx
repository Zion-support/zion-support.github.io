...
const handleSaveNotes = () => ;
  ...
  description: 'Your notes have been saved'})setShowNotes(false
}
...
export function CandidateCard() {const [showNotes, setShowNotes] = useState(false);"
  ...
  return (<>;
      <Draggable draggableId={application.id} index={index} />;
        {(provided) => (<Card;}"
            className=\"mb-2 p-0 shadow-sm border\";}"
import { useState } from \"react\",// Local stub is used in place of the @hello-pangea/dnd package which isn't;
// available in this environment.;
        ...
        {(provided) => (<Card;}"
           ...
           />;
            <CardContent className='p-3' />;"
              {/* Candidate Header */}<div className=\"flex justify-between items-start mb-2\" />;"
              ...
              {application && application.match_score !== null &&;}
                application && application.match_score !== undefined && (<div className='mb-2' />;}
              ...
              {/* Action Buttons */}<div className=\"flex justify-between mt-2 gap-1\" />;
                <Button;"
                  variant=\"outline\";"
                  size=\"sm\";"
                  className=\"flex-1\";
                ...
                </Button>;<Button variant='outline' size='sm' className='flex-1' asChild />;
                  {application && application.resume?.file_url ? (<a;}
                      href={application && application.resume.file_url}
                      target='_blank';
                      rel='noopener noreferrer' />;
