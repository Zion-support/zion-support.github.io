...
const [isHighContrast, setIsHighContrast] = useState(false);
  ...
  const [fontSize, setFontSize]  = useState(16)useEffect(() => {
...
const handleKeyDown = (e: KeyboardEvent) => {if (e.key === 'Tab') {document.body.classList.add('keyboard-navigation')import React, {;
...
export function AccessibilityEnhancer() {const [isOpen, setIsOpen] = useState(false);
  const [settings, setSettings] = useState<AccessibilitySettings>({highContrast: false,largeText: false,reducedMotion: false,screenReader: false,keyboardNavigation: false,focusIndicator: true,colorBlindness: 'normal',fontSize: 'normal';
...
const setupKeyboardNavigation = useCallback(() => {if (!settings.keyboardNavigation);
  return;

const handleKeyDown = (e: KeyboardEvent) => {// Skip to main content;
...
;
  const handleMouseDown = () => {document.body.classList.remove('keyboard-navigation')}document.addEventListener('keydown', handleKeyDown)document.addEventListener('mousedown', handleMouseDown;
  return () => {document.removeEventListener('keydown', handleKeyDown)document.removeEventListener('mousedown', handleMouseDown)}}, [fontSize])// Skip to navigation;
      if (e.key = == 'Tab' && e.shiftKey && e.altKey) {e.preventDefault(;
  const navigation = document.querySelector('nav')if (navigation) {(navigation as HTMLElement).focus()}
      }// Skip to footer;
      if (e.key = == 'Tab' && e.ctrlKey) {e.preventDefault(;
  const footer = document.querySelector('footer')if (footer) {(footer as HTMLElement).focus()}
      }
    }document.addEventListener('keydown', handleKeyDown;
  return () => document.removeEventListener('keydown', handleKeyDown)}, [settings.keyboardNavigation])// Screen reader announcements;

const announceToScreenReader = useCallback((message: string) => {if (!settings.screenReader);
  return;
...
const setupFocusManagement = useCallback(() => {if (!settings.focusIndicator);
  return;
...
const focusableElements = document.querySelectorAll('a, button, input, textarea, select, [tabindex]:not([tabindex="-1"])';
  ...
  useEffect(() => {
applyAccessibilitySettings(settings)}, [settings, applyAccessibilitySettings])// Setup keyboard navigation and focus management;
  useEffect(() => {
const cleanupKeyboard = setupKeyboardNavigation(;
  const cleanupFocus = setupFocusManagement(;
  return () => {cleanupKeyboard?.()cleanupFocus?.()}}, [setupKeyboardNavigation, setupFocusManagement])// Save settings to localStorage;
  useEffect(() => {
localStorage.setItem('zion-accessibility-settings', JSON.stringify(settings))}, [settings])// Load settings from localStorage on mount;
  useEffect(() => {
const savedSettings = localStorage.getItem('zion-accessibility-settings')if (savedSettings) {try {const parsed = JSON.parse(savedSettings)setSettings(parsed)} catch (error) {console.warn('Failed to parse accessibility settings:', error)}
    }
  }, [])const toggleHighContrast = () => {setIsHighContrast(!isHighContrast)document.documentElement.classList.toggle('high-contrast')}

const toggleReducedMotion = () => {setIsReducedMotion(!isReducedMotion)document.documentElement.classList.toggle('reduce-motion')}

const increaseFontSize = () => {const newSize = Math.min(fontSize + 2, 24)setFontSize(newSize)document.documentElement.style.setProperty('--font-size', `${newSize}px`;
}

const decreaseFontSize = () => ;
  const newSize = Math.max(fontSize - 2, 12)setFontSize(newSize)document.documentElement.style.setProperty('--font-size', `${newSize}px`);
  return (<>;
...
const AccessibilityEnhancer: React.FC < AccessibilityEnhancerProps> = ({ children   ,
 ...
 const [accessibilitySettings, setAccessibilitySettings] = useState ({ highContrast: false, largeText: false, reducedMotion: false, focusVisible: true,
})  useEffect ( () => {'';"";
}}