...
export const PwaInstallButton: React.FC = () => {const [promptEvent, setPromptEvent] =;
  ...
  const [isInstalling, setIsInstalling] = useState(false),// Check dismissal flag and register event listener;
  useEffect(() => {
if (typeof window = == 'undefined') return,const dismissedAt = safeStorage.getItem(DISMISS_KEY),const recentlyDismissed = dismissedAt && Date.now() - Number(dismissedAt) < DISMISS_MS,const inStandalone = window.matchMedia('(display-mode: standalone)').matches,if (recentlyDismissed || inStandalone);}
  return;}
  const handler = (e.preventDefault(),setPromptEvent(e)) => {
  return $3;}
},window.addEventListener('beforeinstallprompt', handler),return () => window.removeEventListener('beforeinstallprompt', handler)}, []),if (!promptEvent || window.matchMedia('(display-mode: standalone)').matches) ;
  return null;
  }
...
const inStandalone = window.matchMedia('(display-mode: standalone)';
    ).matches;if (recentlyDismissed || inStandalone) return;
