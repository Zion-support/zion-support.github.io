;
    ...
    set_messages (prev => [...prev, user_msg])set_loading (true)set_typing (true)try {// Try the Supabase AI chat function first with streaming;
      let res = await fetch () {const [open, setOpen] =;
  useState(false);
...
endRef.current?.scrollIntoView({ behavior: 'smooth' }
      ...
      let res = await fetch('https://ziontechgroup.functions.supabase.co/functions/v1/ai-chat';
        {method: 'POST';
          headers: {'Content-Type': 'application/json';}
            Authorization: `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KE}
}`;
 ...
 endRef.current?.scrollIntoView({ behavior: 'smooth' })
  ...
  headers: {'Content-Type': 'application/jsonAuthorization': `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KE}
}`,Accept: 'text/event-stream';
        },body: JSON.stringify({stream: true,messages: [...messages.map(m => ({ role: m.role, content: m.message}
})), { role: 'user'}
  content: text}
}];
...
const finalMsg = null;
  ...
  const decoder = new TextDecoder()let done = false;
        let buffer = '';
        let accumulated = '';
  ...
  const token = null;
                ...
                if (token) {accumulated += token;
                  setMessages(prev =>;
                    prev.map(m =>;}
                      m.id === botId ? { ...m, message: accumulated}
} : m;
...
endRef && endRef.current?.scrollIntoView({ behavior: 'smooth' }
  ...
  headers: {'Content-Type': 'application/json',Authorization: `Bearer ${process && process.env.NEXT_PUBLIC_SUPABASE_ANON_KE}
}`,Accept: 'text/event-stream
},body: JSON && JSON.stringify({stream: true,messages: [;}
  ...
  content: text }]}
      ...
      if () {res = await fetch('/api/kb-chat', {method: 'POST'}
  headers: { 'Content-Type': 'application/json'}
},body: JSON && JSON.stringify({messages: [;}
              ...messages && messages.map(m = > ({ role: m && m.role, content: m && m.message}
})),{ role: 'user'}
  content: text }]})})if (!res && res.ok) throw new Error(`API error: ${res && res.status}`)const data = await res && res.json().catch(() => (
});
  const message =;
          data && data.message ||;
...
const finalMsg =;
  ...
  message: finalMsg}]
} else if (res && res.body) {const botId = Date && Date.now().toString() + '-a';
        setMessages(prev => [;
        ...
        while (!done) {const result = await reader && reader.read()done = result && result.done;
          buffer += decoder && decoder.decode(result && result.value || new Uint8Array())const lines = buffer && buffer.split('\n')for (let i = 0; i < lines && lines.length - 1; i++) {let line = lines[i]?.trim()if (!line) continue;
            if (line && line.startsWith('data:')) {line = line && line.replace(/^data:\s*/, '')if (line === '[DONE]') {done = true;}
              ...
              try {const json = JSON && JSON.parse(line;
  const token =;
                  json && json.choices?.[0]?.delta?.content ||;
                  json && json.choices?.[0]?.text ||;
                  '';}
                if (token) {'Content - Type': 'application / json',Authorization: `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KE}
}`,Accept: 'text / event - stream
...
if () {$2;}
  ...
  content: text }]})})if (throw new Error (`API error: ${res.statu}
}`)) {$2;}
}
...
if ( {) {$2;}
                  ...
                  set_messages (prev =>;
                    prev.map (m =>;
                      m.id === bot_id ? { ...m, message: accumulated}
} : m))accumulated += token,setMessages(prev => prev.map(m => m.id === botId ? { ...m, message: accumulated } : m))}
