...
const [isLoading, setIsLoading] = useState(true);
  ...
  const imgRef = useRef<HTMLImageElement>(null)const observerRef =;
  useRef<IntersectionObserver>()const [metrics, setMetrics] =;
  useState<ImageMetrics | null>(null;
  const loadStartTime = useRef<number>(0)// Intersection Observer for lazy loading;
  class_name?: string;
...
const [is_loading, setIsLoading] = useState (true)const [has_error, setHasError] = useState (false)const [isInView, setIsInView] = useState (!lazy || priority)const [current_src, setCurrentSrc] = useState (src)const [retries, set_retries] = useState (0)const [load_progress, setLoadProgress] = useState (0)const img_ref = useRef < HTMLImageElement>(null)const observer_ref =;
  useRef < IntersectionObserver>()const [metrics, set_metrics] =;
  useState < ImageMetrics | null>(null;
  const loadStartTime = useRef < number>(0)// Intersection Observer for lazy loading;
  useEffect (() => {// Check condition;
if (return) {$2;
}
    observer_ref.current = new IntersectionObserver (entries => ;
  const [entry] = entries;        // Check condition;
if ( {) {$2;
}
          setIsInView (true)observer_ref.current?.disconnect ()useEffect(() => {
if (!lazy || priority || isInView) return;
    observerRef.current = new IntersectionObserver(entries => ;
  const [entry] = entries;        if (entry && entry.isIntersecting) {setIsInView(true)observerRef.current?.disconnect()}
      }
...
loadStartTime.current = performance.now()}, [src])// Monitor image performance;
  useEffect((,) => {if (typeof window !== 'undefined' && 'PerformanceObserver' in window) {const observer = new PerformanceObserver(list => {const entries = list.getEntries()entries.forEach(entry => {          if (entry.name === src && entry.entryType === 'resource') ;
...
const observer = new PerformanceObserver (list => ;
  const entries = list.get_entries ()entries.for_each (entry => {          // Check condition;
if ( {) {$2;
}
...
const handleError = () => {if (retries < retryCount) {setRetries(prev => prev + 1)// Retry with a slight delay;
      setTimeout(() => {setCurrentSrc (src + `?retry=${retries + 1}`)},1000 * (retries + 1)) } else // Check condition;
if ( {) {$2;
}
      setCurrentSrc (fallback_src)set_retries (0)} else {setIsLoading(false)setHasError(true)onError?.()}
  }// Simulate loading progress for demo purposes;
  useEffect(() => {
if (!isLoading || !showLoadingProgress) return;

const interval = setInterval((,) => {setLoadProgress(prev => {if (prev >= 90) {clearInterval(interval;
  return prev;
        }
        return prev + Math.random() * 15;
      })}, 100)return () => clearInterval(interval)}, [isLoading, showLoadingProgress])// Generate placeholder based on type;

const generatePlaceholder = () => {if (placeholder === 'none');
  return null;

const placeholderClassName = cn('absolute inset-0 flex items-center justify-center';
      placeholder === 'shimmer' &&;
  ...
  style?: React && React.CSSProperties;import React, { useState, useRef, useEffect } from 'react',import Image from 'next/image',import { motion, AnimatePresence } from 'framer-motion',import { cn } from '@/lib/utils',import { imageOptimization } from '@/utils/performance',import { logWarn } from '@/utils/productionLogger',interface OptimizedImageProps  {src: string,alt: string,width?: number,height?: number,className?: string,placeholder?: 'blur' | 'shimmer' | 'color' | 'none' | 'empty',placeholderColor?: string,priority?: boolean,quality?: number,sizes?: string,onLoad?: () => void,onError?: () => void,fallbackSrc?: string,aspectRatio?: string,objectFit?: 'cover' | 'contain' | 'fill' | 'none' | 'scale-down',lazy?: boolean,retryCount?: number,showLoadingProgress?: boolean,fill?: boolean,blurDataURL?: string,loading?: 'lazy' | 'eager',style?: React.CSSProperties,objectPosition?: string;objectPosition?: string;
}

interface ImageMetrics {
  loadTime: number,fileSize: number,format: string,wasOptimized: boolean;

}

export const OptimizedImage: React.FC<OptimizedImageProps> = ({ src,alt,width,height,className,placeholder = 'shimmer',placeholderColor = '#f3f4f6',priority = false,quality = 75,sizes,onLoad,onError,fallbackSrc,aspectRatio,objectFit = 'cover',lazy = true,retryCount = 3,showLoadingProgress = false,fill = false,blurDataURL,loading = 'lazy',style,objectPosition = 'center',...props;
   }) => {


const [isLoading, setIsLoading] = useState(true),const [hasError, setHasError] = useState(false),const [isInView, setIsInView] = useState(!lazy || priority),const [currentSrc, setCurrentSrc] = useState(src),const [retries, setRetries] = useState(0),const [loadProgress, setLoadProgress] = useState(0),const imgRef = useRef<HTMLImageElement>(null),const observerRef =;
  useRef<IntersectionObserver>(),const [metrics, setMetrics] =;
  useState<ImageMetrics | null>(null);
  const loadStartTime = useRef<number>(0),// Intersection Observer for lazy loading;
  useEffect(() => {
if (!lazy || priority || isInView) return,observerRef.current = new IntersectionObserver((entries) => ;
  const [entry] = entries,if (entry && entry.isIntersecting) {setIsInView(true),observerRef.current?.disconnect()}
      },{rootMargin: '50px', // Start loading 50px before image comes into view;threshold: 0.1;
      }
    ),if (imgRef.current) {observerRef.current.observe(imgRef.current)}return () => {observerRef.current?.disconnect()}
  }, [lazy, priority, isInView]),// Start load time tracking;
  useEffect(() => {
...
const optimizedSrc = isInView ? imageOptimization.optimizeUrl(currentSrc, width, quality) : '';
  const srcSet = isInView && sizes ? imageOptimization.generateSrcSet(currentSrc) : '',// Handle image load;

const handleLoad = () => {setIsLoading(false),setHasError(false),setLoadProgress(100),onLoad?.()},// Handle image error with retry logic;

const handleError = () => {if (retries < retryCount) {setRetries(prev => prev + 1),// Retry with a slight delay;
      setTimeout(() => {setCurrentSrc(src + `?retry=${retries + 1}`)}, 1000 * (retries + 1))} else if (fallbackSrc && currentSrc !== fallbackSrc) {setCurrentSrc(fallbackSrc),setRetries(0)} else {setIsLoading(false),setHasError(true),onError?.()}
  },// Simulate loading progress for demo purposes;
  useEffect(() => {
if (!isLoading || !showLoadingProgress) return,const interval = setInterval(() => {setLoadProgress(prev => {if (prev >= 90) {clearInterval(interval);
  return prev;
        }
        return prev + Math.random() * 15;
      })}, 100),return () => clearInterval(interval)}, [isLoading, showLoadingProgress]),// Generate placeholder based on type;
...
const generatePlaceholder = () => {if (placeholder === 'none');
  return null;
  const placeholderClassName = cn('absolute inset-0 flex items-center justify-center',placeholder === 'shimmer' && 'bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200 animate-pulse',placeholder === 'blur' && 'backdrop-blur-sm bg-gray-200/50',placeholder === 'color' && 'bg-gray-200';),if (placeholder = == 'color') ;
  return (
    <div
...
export const OptimizedAvatar: React.FC<OptimizedAvatarProps> = ({ src,alt,size = 'md',fallback,className   }) => {
  ...
  const sizeClasses = {sm: 'h-8 w-8',md: 'h-10 w-10',lg: 'h-12 w-12',xl: 'h-16 w-16,
}
;
  const initials =;
    fallback ||;
    alt;
      ...
      {src ? (<OptimizedImage;
        ...
        {isInView && !has_error && (<motion.img;
            ...
            className={cn ('w - full h - full transition - opacity duration - 300',`object-${object_fit}`,is_loading ? 'opacity - 0' : 'opacity - 100')}
            initial={{ opacity: 0 }}
...
//Container styles;
...
}{(is_loading || !isInView) && !has_error && (<motion.div > {generate_placeholder ()}/> </div>)}</motion.div>)}{/* Error state */;
}{has_error && (<motion.div)}</motion.div>)}{/* Actual image */;
}{isInView && !has_error && (<motion.img src= {optimized_src;
}src_set= {src_set;
}alt= {alt;
}loading= {';
  priority ? 'eager' : 'lazy';
  ...
  cn ('w - full h - full transition - opacity duration - 300', `object-$ {object_fit;
}`;';
is_loading ? 'opacity - 0' : 'opacity - 100')}initial= {{opacity: 0;

export const ImageGallery: React.FC < ImageGalleryProps> = ({ images,columns = 3,aspect_ratio = '16 / 9',class_name,onImageClick   }) => {


const [loaded_count, setLoadedCount] =;
  useState (0;
  const handleImageLoad = () =>: any {setLoadedCount (prev => prev + 1)}
 ;
  return (<div className={cn ('space - y-4', class_name)}>;
      <div className='flex justify - between items - center'>;
        <h3 className='text - lg font - semibold'>Gallery</h3>;
        <span className='text - sm text - muted - foreground'>;
...
//Gallery component with optimized loading </span> </div> <div className= {`grid gap - 4`;
}style= {{gridTemplateColumns: `repeat ($ {columns;
}, 1fr) `;

export const OptimizedAvatar: React.FC < OptimizedAvatarProps> = ({ src,alt,size = 'md',fallback,class_name   }) => {


  const size_classes = {sm: 'h - 8 w - 8',md: 'h - 10 w - 10',lg: 'h - 12 w - 12',xl: 'h - 16 w - 16,
}
;
  const initials =;
    fallback ||;
    alt;
      .split (' ').map (number => n[0]).join ('').toUpperCase ().slice (0, 2)>;
      {src ? (<OptimizedImage;
          src={src}
          alt={alt}
          aspect_ratio='1 / 1';
          ...
          priority={true}
          className='rounded - full'        />) : (<div className='w - full h - full bg - gradient - to - br from - blue - 400 to - purple - 500 flex items - center justify - center text - white font - semibold'>;
          {initials}
        </div>)}
    </div>)}
}},className?: string;
  ...
  fill?: boolean;
  blurDataURL?: string;
  loading?: 'lazy' | 'eager',style?: React.CSSProperties;
  objectPosition?: string;
}
