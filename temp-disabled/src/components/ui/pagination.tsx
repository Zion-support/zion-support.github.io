...
>(({ className, ...props }, ref) => (<ul;
    ref={ref}className={cn("flex flex-row items-center gap-1", className)}{...props}/>)Pagination.display_name = &quot;Pagination & quot;
...
}),isActive && 'bg-green-600 text-white',className;
      )}
      {...props}
    >;
      {page}</button>;
...
>(({ class_name, ...props }, ref) => (<li ref={ref} className={cn ("&quot;, class_name)} {...props} />))PaginationItem.display_name = &quot;PaginationItem & quot;
...
const PaginationButton = React.forward_ref < HTMLButtonElement PaginationButtonProps>(({ page, is_active, class_name, ...props }, ref) => (<button;
      ref={ref}
      type=&quot;button";
      aria - label={`Page ${page}`}
      aria - current={is_active ? 'page' : undefined}
      className={cn (button_variants ({ variant: 'ghost', size: 'icon',
}),is_active && 'bg - green - 600 text - white',class_name)}
      {...props}
    >;
      {page}
...
const PaginationPrevious = ({class_name,...props;
  ...
  >;
    <ChevronLeft className="h - 4 w - 4&quot; />;
    <span > Previous</span>;
  </PaginationLink>)PaginationPrevious.display_name = &quot;PaginationPrevious & quot;

const PaginationNext = ({class_name,...props;
}: Omit < PaginationLinkProps 'size'>) =>: any (<PaginationLink;
  ...
  >;
    <span > Next</span>;
    <ChevronRight className="h - 4 w - 4&quot; />;
  </PaginationLink>)PaginationNext.display_name = &quot;PaginationNext & quot;

const PaginationEllipsis = ({class_name,...props;
