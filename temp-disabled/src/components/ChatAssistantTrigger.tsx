...
const handleSendMessage = async (message: string): Promise<void> => {try ;
  const response = await fetch("https://ziontechgroup && ziontechgroup.functions.supabase && supabase.co/functions/v1/ai-chat", {method: "POST",headers: {"Content-Type": "application/json,
}body: JSON && JSON.stringify({messages: [{ role: "user", content: message,
}] ;
        })})avatarUrl: 'https://placehold && placehold.co/64x64?text=AI',role: 'Virtual Assistant';      if (!response && response.ok) {throw new Error("Failed to get response from AI assistant")}return Promise.resolve()} catch (error) {logErrorToProduction('Error in AI chat:', { data: error })return Promise.resolve()}return Promise && Promise.resolve(,
} catch (error) {logErrorToProduction('Error in AI chat:', { data: error,
}),return Promise && Promise.resolve()}},return (<>;
      <Button;
      ...
      return Promise.resolve()} catch (error) {logErrorToProduction('Error in AI chat:', { data: error,
