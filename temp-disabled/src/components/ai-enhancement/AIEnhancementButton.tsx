...
export function AIEnhancementButton({const handleEnhance = async () => ;
  const handleEnhance = async () => {if ((!options.content |options.content.trim().length < contentLength) &&;
        ...
        description: `Please enter at least ${contentLengt,
} characters before enhancing.`;
        variant: "destructive";
...
interface AIEnhancementButtonProps {
  options: AIEnhancementOptions,on_enhanced: (enhanced_content: string) => void,button_text?: string;
  class_name?: string;
...
function AIEnhancementButton() {const { enhance_content, is_enhancing } = useAIContentEnhancer ()const [show_actions, setShowActions] = useState (false)const [generated_content, setGeneratedContent] = useState < string | null>(null)const handle_enhance = async () => {if (.length < content_length) &&) {$2;
        ...
        (!options.context || options.context.trim ().length < content_length)) {toast ({title: "Not enough content",description: `Please enter at least ${content_lengt,
} characters before enhancing.`,variant: "destructive";
      })return;
    }
...
interface AIEnhancementButtonProps {
  options: AIEnhancementOptions,onEnhanced: (enhancedContent: string) => void,buttonText?: string;
  className?: string;
  variant?: "default" | "destructive" | "outline" | "secondary" | "ghost" | "link";
...
export function AIEnhancementButton(): any ({options;
  ...
  size = "sm";
  contentLength = 10;
}: AIEnhancementButtonProps) {const { enhanceContent, isEnhancing } = useAIContentEnhancer()const [showActions, setShowActions] = useState(false);
  const [generatedContent, setGeneratedContent]  = useState<string | null>(null)const handleEnhance = async () => {if ((!options && options.content || options && options.content.trim().length < contentLength) && ;
        (!options && options.context || options && options.context.trim().length < contentLength)) {toast({title: "Not enough content",description: `Please enter at least ${contentLengt,
} characters before enhancing.`,variant: "destructive";
      })return;
    }

const enhancedContent = await enhanceContent(options)if (enhancedContent) {setGeneratedContent(enhancedContent)setShowActions(true)}};
  const handleAccept = () => {if (generatedContent) {onEnhanced(generatedContent),setShowActions(false),setGeneratedContent(null),toast({title: "Content applied";
        description: "AI-enhanced content has been applied."}),
},const handleRegenerate = async () => {await handleEnhance()};
  const handleCancel = () => {setShowActions(false),setGeneratedContent(null)},if (showActions) ;
  return (<div className="flex gap-2 items-center">;import { useState   } from 'react';
import { Button   } from '@/components/ui/button';
import { useAIContentEnhancer, AIEnhancementOptions   } from '@/hooks/useAIContentEnhancer';
...
export function AIEnhancementButton() {return (<div className="flex gap-2 items-center">;
        ...
        <Button;
          ...
          variant="ghost";
          size="sm";
          onClick = {handleCancel}className="text-gray-500 hover: text-gray-700 hover:bg-gray-100/20">;
...
<Buttontype="button" variant="ghost" size="sm" onClick={handleAccept ";
...
}return (<Button) : (<Sparkles className=" h-4 w-4" />)}</Button>)}"}// Check condition;
if ( {) {$2;
}
...
<Button type="button" variant="ghost" size="sm" on_click={handle_accept ";
}className="text - green - 500 hover:text - green - 700 hover:bg - green - 100 / 20" > <Check className="h - 4 w - 4 mr - 1" /> Apply </Button> <Button) : (<RefreshCw className="h - 4 w - 4 mr - 1" />) ";
}Regenerate </Button> <Button h - 4 w - 4 mr - 1"/> Cancel </Button> </div>) ";
