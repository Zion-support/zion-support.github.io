...
const listingSchema = z.object({id: z.string(),title: z.string(),category: z.string(),image: z.string().optional(,
});
  const listingsSchema = z.array(listingSchema),interface ServiceTypeStepProps {
  formData: QuoteFormData,updateFormData: (data: Partial<QuoteFormData>) => void;

}

export function ServiceTypeStep() {const [searchQuery, setSearchQuery] = useState(""),const debouncedQuery = useDebounce(searchQuery, 300),const [listings, setListings] = useState<ListingItem[]>([]),const [loading, setLoading] = useState(false),const [error, setError] =;
  useState<string | null>(null),const isMounted = useIsMounted(),image: z.string().optional(,
};
  const listingsSchema  = z.array(listingSchema)interface ServiceTypeStepProps  {formData: QuoteFormData;
  updateFormData: (data: Partial<QuoteFormData>) => void;

export function ServiceTypeStep() {const [searchQuery, setSearchQuery] = useState('');
  const debouncedQuery = useDebounce(searchQuery, 300)const [listings, setListings] = useState<ListingItem[]>([])const [loading, setLoading] =;
  useState(false);
  const [error, setError] = useState<string | null>(null;
  const isMounted  = useIsMounted()// Fetch services when the service type or query changes;
  useEffect(() => {
if (!formData.serviceType) {setListings([]),return;
import { useEffect, useState } from "react",import { QuoteFormData, ListingItem, ServiceType } from "@/types/quotes",import { Input } from "@/components/ui/input",import { Card } from "@/components/ui/card",import { ListingScoreCard } from "@/components/ListingScoreCard",import { captureException } from "@/utils/sentry",import Skeleton from "@/components/ui/skeleton",import { useDebounce } from "@/hooks/useDebounce",import { useIsMounted } from "@/hooks/useIsMounted",import { z } from "zod",import {logErrorToProduction} from '@/utils/productionLogger',const listingSchema = z.object({id: z.string(),title: z.string(),category: z.string(),image: z.string().optional(,
});
  const listingsSchema = z.array(listingSchema),interface ServiceTypeStepProps {
  formData: QuoteFormData,updateFormData: (data: Partial<QuoteFormData>) => void;
...
const fetchServices = async () => {setLoading(true),setError(null);
  const url = `/api/public/services?category=${encodeURIComponent(formData.serviceType;
      )}&q = ${encodeURIComponent(debouncedQuery)}`,const maxRetries = 3,for (let attempt = 0, attempt < maxRetries, attempt++) {try {const response = await fetch(url),if (!response.ok) throw new Error('Failed to fetch'),const data = await response.json();
  ...
  return;
    ...
    },fetchServices()}, [formData.serviceType, debouncedQuery, isMounted]),const handleTypeSelect = (type: ServiceType) => {updateFormData({ serviceType: type },
};
  const handleItemSelect = (item: ListingItem) => {updateFormData({specificItem: item,serviceCategory: item.category,serviceType: item.category.toLowerCase() as ServiceType;
      ...
      {formData.serviceType && (<div className="space-y-4">;
          ...
          <div className="relative">;
            ...
            <Input;
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}className="pl-10 bg-zion-blue border border-zion-blue-light focus:border-zion-purple";
            />;
          </div>;
          {error && (<div className="text-center text-red-400 text-sm">{error}</div>;
            ...
            ) : filteredListings.length > 0 ? (filteredListings.map((item) => (<div;
                  key={item.id}
