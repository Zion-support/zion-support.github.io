...
"
export function EnhancedNewsletterForm() {const [email, setEmail]  = useState(\"\")export function EnhancedNewsletterForm() {const [email, setEmail]  = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);}
  const [isSubmitted, setIsSubmitted] = useState(false);}
  const { toast } = useToast()const EMAIL_REGEX  = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

const EMAIL_REGEX  = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

const lastSubmit = useRef(0)const handleSubmit = async (e: React.FormEvent) => {e.preventDefault(;
  const now = Date.now()if (now - lastSubmit.current < 1000);
  return;
    lastSubmit.current = now;
"
const trimmed = email.trim()if (!EMAIL_REGEX.test(trimmed)) {toast.error(\"Invalid email\")const trimmed = email.trim()if (!EMAIL_REGEX.test(trimmed)) {toast.error(\"Invalid email\")const trimmed = email.trim()if (!EMAIL_REGEX.test(trimmed)) {toast.error(\"Invalid email\")const trimmed = email.trim()if (!EMAIL_REGEX.test(trimmed)) {toast.error(\"Invalid email\";
  const trimmed = email.trim()if (!EMAIL_REGEX.test(trimmed)) {toast({title: 'Invalid email'}
  description: 'Please enter a valid email address.',variant: 'destructive';}
      })return;"
    }setIsSubmitting(true)try {try {const res = await fetch(\"/api/newsletter\", {method: \"POST\",headers: { \"Content-Type\": \"application/json\"}
},body: JSON.stringify({ email: trimmed }
})ursor/automate-test-improve-and-merge-code-646c;
"
const res = await fetch(\"/api/newsletter\", {method: \"POST\",headers: { \"Content-Type\": \"application/json\"}
},body: JSON.stringify({ email: trimmed })body: JSON.stringify({ email: trimmed })})setIsSubmitting (false)}
...
const res = await fetch('/api/newsletter', {method: 'POST'}
  ...
  description: data.message || 'Thanks for subscribing!';}
        ...
        setIsSubmitted(true)setEmail('')} else {// Handle error responses;}
        logErrorToProduction('Newsletter subscription failed:', { data: data}
})toast({title: 'Subscription failed'}
