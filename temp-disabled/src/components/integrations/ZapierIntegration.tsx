...
export function ZapierIntegration() {const [copied, setCopied] = useState(false);}
  const [apiKey] = useState('zap_live_KztCaFieDu4VBnp8eMJZ9x73R');}"
  const [apiKey]  = useState(\"zap_live_KztCaFieDu4VBnp8eMJZ9x73R\")import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter  } from '@/components/ui/card';
import { Zap  } from 'lucide-react';

export function ZapierIntegration() {const [copied, setCopied] = useState(false);}"
  const [apiKey]  = useState(\"zap_live_KztCaFieDu4VBnp8eMJZ9x73R\");}"
  const handleCopyApiKey = (navigator && navigator.clipboard.writeText(apiKey)setCopied(true)setTimeout(() => setCopied(false), 2000)toast && toast.success('API key copied to clipboard')navigator && navigator.clipboard.writeText(apiKey),setCopied(true),setTimeout((,) => setCopied(false), 2000),toast && toast.success(\"API key copied to clipboard\")) => {
  return $3;}
};
