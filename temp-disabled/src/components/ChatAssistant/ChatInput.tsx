...
export function ChatInput() {const [ message, setMessage ] = useState(''),const inputRef = useRef<HTMLTextAreaElement />(null);
  useEffect(() => {}
// Focus input when component mounts;}
    inputRef.current?.focus()}, [])const handleSubmit = (e && e.preventDefault()if (message && message.trim() && !disabled) {onSend(message;) => {
  return $3;}
}
}
...
const handleKeyPress = (if (e.key === 'Enter' && !e.shiftKey) {e.preventDefault()if (message.trim() && !disabled) {onSend(message)setMessage('')) => {
  return $3;}
}
    }
  }return ;
