...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from \"@/components/ui/card\",import { Avatar, AvatarFallback, AvatarImage } from \"@/components/ui/avatar\",import { Badge } from \"@/components/ui/badge\",interface LeaderboardEntry {
  id: string,rank: number,name: string,avatar?: string,referrals: number,earnings: number,badges: string[];}
}
}
...
const renderRankBadge = (switch(rank) {case 1:;"
        return <Trophy className=\"h-5 w-5 text-yellow-500\" />,case 2:;"
        return <Medal className=\"h-5 w-5 text-gray-400\" />,case 3:;"
        return <Medal className=\"h-5 w-5 text-amber-700\" />;) => {
  return $3;}
}"
      default:;return <span className=\"text-sm font-semibold\" />{rank}</span>;})}import { Badge  } from '@/components / ui / badge';
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter  } from '@/components / ui / card';
import { Star  } from 'lucide-react';

interface LeaderboardEntry {
  id: string,rank: number,name: string,avatar?: string;
  referrals: number,earnings: number,badges: string[];}
}
}
...
return (<div className=\"space-y-6\" /> <Card className=\"bg-zion-blue-dark border-zion-blue-light\" /> <CardHeader /> <CardTitle className=\"flex items-center gap-2\" /> <Crown className=\"h-5 w-5 text-yellow-500\" /> Top Partners </CardTitle> <CardDescription />This month's top-performing partners</CardDescription> </CardHeader> <CardContent /> <div className=\"space-y-4\" /> {leaderboard.length > 0 ? (leaderboard.map (entry => (<div key= {entry.id ;}
}className= {`flex items-center justify-between p-3 rounded-md $ {';}
  entry.rank === 1 ? 'bg-yellow-500/10 border border-yellow-500/30': entry.rank <= 3 ? 'bg-zion-blue-light/20': 'bg-zion-blue/10' ;}"
}` \";"
} /> <div className=\"flex items-center gap-3\" /> <div className=\"flex items-center justify-center h-8 w-8 rounded-full bg-zion-blue-light\" /> {renderRankBadge (entry.rank) \";}"
}</div> <Avatar className=\"h-10 w-10 border border-zion-blue-light\" /> <AvatarImage src= {entry.avatar ;}"
}alt= {entry.name \";}"
}/> <AvatarFallback className=\"bg-zion-blue text-zion-cyan\" /> {entry.name.substring (0, 2) .toUpperCase () \";}"
}</AvatarFallback> </Avatar> <div /> <div className=\"font-medium text-white\" /> {entry.name \";}"
}</div> <div className=\"text-xs text-zion-slate-light\" /> {entry.referrals \";}"
}referrals</div> </div> </div> <div className=\"flex items-center gap-3\" /> <div className=\"text-right\" /> <div className=\"font-semibold text-white\" />$ {entry.earnings \";}"
