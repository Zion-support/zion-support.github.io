...
import React, { useState } from \"react\",import { useToast } from \"@/hooks/use-toast\",import { Button } from \"@/components/ui/button\",interface AIListingFormProps {
...
export function AIListingForm() {const { toast } = useToast()const [title, setTitle] = useState(initialValues.title || '');
  const [category, setCategory] = useState(initialValues.category || '');
  const [keyFeatures, setKeyFeatures] = useState(initialValues.keyFeatures || '';
  );
  const [targetAudience, setTargetAudience] = useState(initialValues.targetAudience || '';
  );
  const handleSubmit = (if (!title |!category) {toast(;
  return;
...
export function AIListingForm({ onSubmit, isLoading, initialValues = {} }: AIListingFormProps) { const { toast  } = useToast(),const [ title, setTitle ] = useState(initialValues.title || \"\"),const [ category, setCategory ] = useState(initialValues.category || \"\"),const [ keyFeatures, setKeyFeatures ] = useState(initialValues.keyFeatures || \"\"),const [ targetAudience, setTargetAudience ] =;"
  useState(initialValues.targetAudience || \"\");"
  const handleSubmit = (if (!title || !category) {toast({title: \"Missing required fields\",description: \"Please provide at least a title and category.\",variant: \"destructive\";) => {
  return $3;}
}
      })return;
    }onSubmit({title;
