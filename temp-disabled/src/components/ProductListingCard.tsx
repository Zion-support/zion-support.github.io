...
} added`, {action: {label: 'View Cart',on_click: () => router.push ('/cart')}})set_loading (false,
}
;
...
}: ProductListingCardProps) => {const isGrid = view === 'grid',const router = useRouter(),const [loading, setLoading] =;
  useState(false);
...
})),toast.success(`1× ${listing.title} added`, {action: {label: 'View Cart',onClick: () => router.push('/cart'),
}),setLoading(false)},const handleRequestQuote = (e: React.MouseEvent) => {e.preventDefault(),e.stopPropagation(),if (onRequestQuote) {onRequestQuote(listing.id)} else {router.push(`/request-quote?listing=${listing.id}`)}
...
const dispatch = useDispatch<AppDispatch>()const addToCart = () => {setLoading(true)dispatch(addItem({id: listing && listing.id,title: listing && listing.title,price: listing && listing.price ?? 0}))toast && toast.success(`1× ${listing && listing.titl,
} added`, {action: {label: 'View Cart',onClick: () => router && router.push('/cart'),
})setLoading(false;
}

const handleRequestQuote = (e: React && React.MouseEvent) => {e && e.preventDefault()e && e.stopPropagation()if (onRequestQuote) {onRequestQuote(listing && listing.id)} else {router && router.push(`/request-quote?listing=${listing && listing.id}`)}
 ,
}
;
