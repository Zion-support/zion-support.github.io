...
const publicRoutes = completeSitemap.filter(route = /> !route.requiredAuth;
  const talentRoutes = completeSitemap.filter(route => route.requiredAuth &&;
        ...
        (route.requiredRoles.includes('jobSeeker') ||;
            route.requiredRoles.includes('creator')))const employerRoutes = completeSitemap.filter(route => route.requiredAuth &&;
        route.requiredRoles &&;
        (route.requiredRoles.includes('employer') ||;
            route.requiredRoles.includes('buyer')))const adminRoutes = completeSitemap.filter(route => route.requiredAuth &&;
        route.requiredRoles &&;
        route.requiredRoles.includes('admin'))return (<div className='min-h-screen bg-gradient-to-br from-zion-slate-dark via-zion-slate to-zion-slate-light' />;
