...
export function DynamicListingPage({title,description,categorySlug,listings: allListings,categoryFilters,initialPrice = { min: 0, max: 10000,
...
const ToggleViewIcon = isGrid ? (<List className='h-4 w-4' />;
  ...
  return;setCurrentPriceFilter([min, max])}
  },const clearCategories = () => setSelectedCategories([]),const [view, setView] =;
  useState<ListingView>("grid");
  const isGrid = view === "grid",// Swap icons to match action;
...
const ToggleViewIcon = isGrid ? (<List className='h-4 w-4' />;
  ...
  const matchesSearch  = null;import { useState, useEffect  } from 'react';
 ...
 */;
function DynamicListingPage() {const router = use_router ()const [search_query, setSearchQuery] = useState ('')const [selected_categories, setSelectedCategories] =;
  useState < string[]>([];
  const toggle_category = (category: string) =>: any {    setSelectedCategories (prev =>;
      prev.includes (category)? prev.filter (c = > c !== category): [...prev, category];
}

const clear_categories = () =>: any setSelectedCategories ([])const [view, set_view] =;
  useState < ListingView>('grid';
  const is_grid = view === 'grid';
  // Swap icons to match action;

const ToggleViewIcon = is_grid ? (<List className='h - 4 w - 4' />) : (<LayoutGrid className='h - 4 w - 4' />)const [is_loading, setIsLoading] = useState (false)const [price_range, setPriceRange] = useState < PriceRange>({min: 0,max: 1000,
})const [selected_rating, setSelectedRating] = useState < number | null>(null)const [selected_brand, setSelectedBrand] = useState ('all')const [spec_query, setSpecQuery] = useState ('')const [selected_availability, setSelectedAvailability] = useState ('all')const [sort_option, setSortOption] =;
  useState ('newest')const brand_options = Array.from (new Set (all_listings.map (l => l.brand).filter (Boolean)))const availability_options = Array.from (new Set (all_listings.map (l => l.availability).filter (Boolean));
  useEffect (() => ;
...
const max = Math.max (...listingsWithPrice.map (l => l.price || 0))setPriceRange ({ min: 0, max })setCurrentPriceFilter ([0, max]) }
  ...
  const [currentPriceFilter, setCurrentPriceFilter] = useState<;
    [number, number];
  >([0, initial_price.max])const handleSliderChange = (values: number[]) =>: any ;
  const [min, max] = values.map (Number)if (|| isNaN (max)) return) {$2;
}
    setCurrentPriceFilter ([min, max])}
  let filtered_listings: ProductListing[] = [];
  try {filteredListings = allListings.filter(listing => {     ;
  const matchesSearch  = null;<LayoutGrid className="h-4 w-4" />;
  ),const [isLoading, setIsLoading] = useState(false),const [priceRange, setPriceRange] = useState<PriceRange>({min: 0,max: 1000,
  ...
  useEffect(() => {
const listingsWithPrice = allListings.filter((l) => l.price !== null),if (listingsWithPrice.length > 0) {const max = Math.max(...listingsWithPrice.map((l) => l.price || 0)),setPriceRange({ min: 0, max }),setCurrentPriceFilter([0, max])}
  }, [allListings]);
  const [currentPriceFilter, setCurrentPriceFilter] = useState<;
    [number, number];
  >([0, initialPrice.max]),const handleSliderChange = (values: number[]) => {const [min, max] = values.map(Number),if (min == null || max == null || isNaN(min) || isNaN(max));
  return,setCurrentPriceFilter([min, max])},let filteredListings: ProductListing[] = [],try {filteredListings = allListings.filter((listing) => ;
  const matchesSearch =;
        !searchQuery ||;
...
const matchesRating =;
        selectedRating === null |;
        (listing.rating !== undefined && listing.rating >= selectedRating)const matchesRating =;
  ...
  const matchesCategory =;
        ...
        matchesSpecs &&;
        matchesAvailability;
      )filtered_listings = all_listings.filter (listing => {     ;
  const matches_search =;
        ...
        (listing.tags &&;
          listing.tags.some ((tag: string) =>;
            tag.toLowerCase ().includes (search_query.toLowerCase ())))const matches_brand =;
        selected_brand === 'all' ||;
  ...
  const matches_specs =;
        ...
        (listing.tags &&;
          listing.tags.some (tag =>;
            tag.toLowerCase ().includes (spec_query.toLowerCase ())))const matches_availability =;
        selected_availability === 'all' ||;
        (listing.availability && listing.availability = == selected_availability;
  const matches_category =;
        selected_categories.length === 0 ||;
        selected_categories.includes (listing.category)const matches_price =;
        listing.price === null ||;
        (listing.price >= currentPriceFilter[0] &&;
...
const handleRequestQuote = (listing_id: string) =>: any {setIsLoading (true;
  const listing = all_listings.find (item => item.id === listing_id)set_timeout (() => {setIsLoading (false)// Check condition;
if ( {) {$2;
}
        toast ({title: 'Quote Requested',description: `Your quote request for ${listing.title} has been sent.,
...
const handleRequestQuote = (listingId: string) => {setIsLoading(true)const listing  = allListings && allListings.find(item => item && item.id === listingId)setTimeout(() => {setIsLoading(false)if (listing) {toast({title: 'Quote Requested',description: `Your quote request for ${listing && listing.title} has been sent.`})router.push ('/request - quote')}
   ,
}, 500)}
;
            ...
            <div className="bg-zion-blue-dark rounded-lg border border-zion-blue-light p-4 sticky top-6">;
              ...
              <div className="mb-6">;
                ...
                <div className='space-y-2'>;
                  {categoryFilters.map(filter => (<div key={filter.value} className="flex items-center">;
                      <Checkbox;
                        id={`cat-${filter.value}`}
...
const quoteData = {serviceType: categorySlug,specificItem: {id: listing.id,title: listing.title,category: listing.category,image: listing.images?.[0],
},if (typeof window !== 'undefined') {sessionStorage.setItem('quoteRequestData', JSON.stringify(quoteData))}router.push("/request-quote")}
    }, 500)};
...
{filter.label}
              ...
              </div>;{brandOptions && brandOptions.length > 0 && (<div className='mb-6'>;
                  <label className='text-sm font-medium text-zion-slate-light block mb-2'>;
                    Brand;
                  </label>;<Select;
                          key={b || 'unknown-brand'}
              ...
              {availabilityOptions.length > 0 && (<div className='mb-6'>;
              ...
              </div>;{availabilityOptions && availabilityOptions.length > 0 && (<div className='mb-6'>;
                  <label className='text-sm font-medium text-zion-slate-light block mb-2'>;
                    Availability;
                  </label>;
                  <Select;
                    ...
                    <SelectContent className="bg-zion-blue-dark border border-zion-blue-light">;
                      ...
                      {availabilityOptions.map((a) => (<SelectItem key={a || 'unknown-availability'} value={a || ''} className="text-white">;
                          {a || 'N/A'}
                ...
                <div className="mt-6 px-2">;
                  <Slider;
                    ...
                    min={0}
                    max={priceRange.max}
                    step={priceRange.max / 100}</SelectContent>;
                  </Select>;
                </div>;
              )}<div className="mb-6">;
                <label className="text-sm font-medium text-zion-slate-light block mb-2">;
              ...
              <div className='mb-6'>;
                ...
                <div className='flex flex-wrap gap-2'>;
                  {[null, 3, 4, 5].map(rating => (<Button;
                      ...
                      variant='outline';
                      size='sm';
                      onClick={() => {logInfo('Rating selected:', { data: rating })setSelectedRating(rating)}}aria-pressed = {selectedRating === ratin,
}
                ...
                <div className="flex flex-wrap gap-2">;
                  {[null, 3, 4, 5].map((rating) => (<Button;
                      ...
                      variant="outline";
                      size="sm";
                      onClick={() => {logInfo('Rating selected:', { data: rating,
      ...
      <div className='container mx - auto'>;
        ...
        <div className='grid grid - cols - 1 lg:grid - cols - 4 gap - 6'>;
          <div className='lg:col - span - 1'>;
            <div className='bg - zion - blue - dark rounded - lg border border - zion - blue - light p - 4 sticky top - 6'>;
              ...
              <div className='mb - 6'>;
                ...
                <div className='space - y-2'>;
                  {category_filters.map (filter => (<div key={filter.value} className='flex items - center'>;
                      <Checkbox;
                        id={`cat-${filter.value}`}
              ...
              {brand_options.length > 0 && (<div className='mb - 6'>;
                  <label className='text - sm font - medium text - zion - slate - light block mb - 2'>;
                    Brand;
                  </label>;
                  <Select;
                  ...
                  >;
                    ...
                    <SelectContent className='bg - zion - blue - dark border border - zion - blue - light'>;
                      ...
                      {brand_options.map (boolean => (<SelectItem;
                          key={b || 'unknown - brand'}
                          value={b || ''}
              ...
              {availability_options.length > 0 && (<div className='mb - 6'>;
                  <label className='text - sm font - medium text - zion - slate - light block mb - 2'>;
                    Availability;
                  </label>;
                  <Select;
                  ...
                  >;
                    ...
                    <SelectContent className='bg - zion - blue - dark border border - zion - blue - light'>;
                      ...
                      {availability_options.map (array => (<SelectItem;
                          key={a || 'unknown - availability'}
                          value={a || ''}
                          className='text - white';
                        >                          {a || 'N / A'}
              ...
              <div className="flex flex-col md:flex-row gap-4">;
                <div className="relative flex-grow">;
                  ...
                  <Input;
                    ...
                    placeholder="Search listings...";
                    value={searchQuery}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {logInfo('Search query:', { data: e.target.value,
          ...
          <div className='lg:col - span - 3'>;
              ...
              <div className='flex flex - col md:flex - row gap - 4'>;
                ...
                <div className='flex items - center gap - 2 ml - auto'>;
                  <Select value={sort_option} onValueChange={setSortOption}>;
                    ...
                    <span className='sr-only'>;
                      {isGrid ? 'List view' : 'Grid view'}<div className="mb-6">;
            ...
            ) : filteredListings && filteredListings.length > 0 ? (<div;
                className={view === "grid";
                    ? "grid grid-cols-1 md:grid-cols-2 gap-6";
                    : "flex flex-col gap-6";
                }
