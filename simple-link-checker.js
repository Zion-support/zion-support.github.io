...
//       //       const result = await this.checkUrl(fullUrl);
  ...
  generateReport() {const valid = this.results.filter(r => r.success);
    const _broken = this.results.filter(r => !r.success);
//     }
//     //     //     if (broken.length > 0) {}
      broken.forEach(link => {
//         `);
