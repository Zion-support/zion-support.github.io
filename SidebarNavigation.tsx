    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-cyan-500/20">
          ...
          <button
            onClick={onClose} className="text-gray-300 hover:text-cyan-400 transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <nav className="mt-8">
          {navigationItems.map((item) => (
            <Link}
              key={item.name} to={item.href} className="flex items-center space-x-3 px-4 py-3 text-gray-300 hover:text-cyan-400 hover:bg-slate-800 transition-colors"
              onClick={onClose} >
