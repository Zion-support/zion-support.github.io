...
function getRecentBranches() {
  try {
    const output = execSync('git for-each-ref --sort=-committerdate refs/remotes/origin --format="%(refname:short) %(committerdate:iso)"', { encoding: 'utf8' });
...
function safeMerge(branch) {
  try {
    ...
    try {
      ...
      return { success: false, branch, error: 'Merge conflict', conflict: true };
    }
  } catch (error) {
...
async function main() {
  ...
  console.log(`Found ${recentBranches.length} recent branches (last 30 days)\n`);
  ...
  if (recentBranches.length === 0) {
    ...
    recentBranches.push(...allBranches.map(branch => ({ branch, date: new Date() })));
  }
  ...
  for (const { branch } of recentBranches.slice(0, 10)) { // Limit to 10 most recent
  ...
  // Save results
  fs.writeFileSync('smart-merge-results.json', JSON.stringify(results, null, 2));
