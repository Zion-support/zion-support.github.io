...
function cleanUnusedImports(filePath) {/* TODO: Fix JSX expression */}
    ...
    if (modified) {
      ...
      console.log(`Cleaned: ${path.relative(__dirname, filePath)}`)} catch (error) {
    ...
    if (newContent !== content) {
      ...
      if (trimmedLine.startsWith('import ')) {/* TODO: Fix JSX expression */}
        ...
        if (importMatch) {/* TODO: Fix JSX expression */}`
                   ...
                   !contentWithoutImport.includes(`{ ${name}`) &&`
                   !contentWithoutImport.includes(`{${name}`);
          if (usedImports.length === 0) {/* TODO: Fix JSX expression */}} else if (usedImports.length < imports.length) {/* TODO: Fix JSX expression */}} else {/* TODO: Fix JSX expression */}`
