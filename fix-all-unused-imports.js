...
function cleanUnusedImports(filePath) {}
  try {}
    ...
    const lucideImportMatch = content.match(/import\s*{\s*([^}]+)\s*}\s*from\s*['"]lucide-react['"];?/)"    if (lucideImportMatch) {}
...
pageFiles.forEach(file => {)
  cleanUnusedImports(file)
    componentImports.forEach(comp => {/* TODO: Fix JSX expression */}")`"      const importRegex = new RegExp(`import\\s+${comp}\\s+from\\s+['"][^'"]+['"];?\\s*`, 'g');`"      if (importRegex.test(content) && !content.includes(`<${comp}`) && !content.includes(`${comp}.`)) {/* TODO: Fix JSX expression */}`
