...
class $1 {
  ...
  this.log(`❌ Error moving ${file}: ${error.message}`);}
    ...
    return {
  totalFiles: allFiles.length;corruptedFiles: this.corruptedFiles.length;corruptedFileList: this.corruptedFiles;}
  }

  generateReport(results) {
  const report = {
  timestamp: new Date().toISOString();summary: results;corruptedFiles: this.corruptedFiles;}
