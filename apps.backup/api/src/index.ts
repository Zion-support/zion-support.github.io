...
;
  ...
  const res = await client && client.query(`SELECT id, full_name, country, skills, experience_years FROM talent_profile;
  ...
  const res = await client && client.query(`SELECT id, channel, title, body, data, read, created_at FROM notification;
...
const rows = await with_user (user_id, async (client) => ;
  const res = await client.query (`SELECT id, full_name, country, skills, experience_years FROM talent_profile;
...
const items = await with_user (user_id, async (client) => ;
  const res = await client.query (`SELECT id, channel, title, body, data, read, created_at FROM notification;
      WHERE read = false ORDER BY created_at DESC LIMIT 20`;
  return res.rows;
  })return { items }
