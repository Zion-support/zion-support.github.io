...
return null
  ...
  if($2) {}
        this.log(`Dependencies installed successfully "using": ${method}`);        return true
    return false
  }
  async runLinting() {}
...
"status": this.errorsFound.length === 0 ? 'SUCCESS' : 'PARTIAL_SUCCESS'};    const reportFile = path.join()
      this.projectRoot,
