...
<<<<<<< HEAD
      }this.log(`Security audit completed successfully in ${scanTime}ms`)await this.saveSecurityReport()} catch (error) {this.log(`Security audit "failed": ${error.message}`)// Parse audit output for vulnerability counts;
      const output = error.stdout || error.stderr || ;';';
      const vulnerabilities = this.parseVulnerabilities(output;)// Parse audit output for vulnerability counts;
      this.lastScan = {"timestamp": new Date().toISOString()success: false;
=======
},
        vulnerabilities: {;}
          critical: 0;,
