...
class AdvancedPerformanceOptimizer {
  ...
  async generateOptimizations() {
    ...
    // Image optimizations
    const largeImages =
      this.optimizationResults.performanceMetrics.issues?.filter(
        i => i.type === 'large_images'
      ) || [];
    if (largeImages.length > 0) {
      optimizations.push({
    ...
    this.optimizationResults.recommendations = optimizations.map(
      opt => opt.action
    );
  }
  ...
  findLargeImages() {
    try {
      const result = execSync(
        `find . -name "*.jpg" -o -name "*.jpeg" -o -name "*.png" -o -name "*.gif" -o -name "*.webp" | xargs ls -la | awk '$5 > 1048576 {print $9}'`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
        }
  ...
  findMissingLazyLoading() {
    try {
      const result = execSync(
        `grep -r '<img' --include="*.jsx" --include="*.tsx" . | grep -v 'loading=' | head -10`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
        }
