...
class SEOOptimizer {
  ...
  findAppPages(dir) {
    ...
    files.forEach(file => {
      ...
      } else if (file === 'page.js' || file === 'page.tsx' || file === 'layout.js' || file === 'layout.tsx') {
        pages.push(filePath);
      }
    });
  ...
  async saveReport() {
    ...
    const reportPath = path.join(logsDir, `seo-optimization-${Date.now()}.json`);
  ...
  async run() {
    ...
    console.log(`✅ SEO optimization completed! Score: ${this.results.seoScore}/100`);
  }
}
