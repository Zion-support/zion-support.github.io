...
const REGISTRY_FILE = path.join(
  LOG_DIR,
  'autonomous-frontend-sync-agents.json'
);
...
function log(message) {
  ...
  try {
    fs.appendFileSync(
      path.join(LOG_DIR, 'autonomous-frontend-sync-factory.log'),
      line
    );
  } catch {}
  console.log(message);
...
function generateAgentsOnce() {
  ...
  fs.writeFileSync(
    REGISTRY_FILE,
    JSON.stringify({ timestamp: new Date().toISOString(), created }, null, 2)
  );
...
function startContinuous() {
  ...
  const watcher = chokidar.watch(watchPaths.filter(Boolean), {
    ignoreInitial: true,
    persistent: true,
  });
