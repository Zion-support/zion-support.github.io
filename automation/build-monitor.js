...
class BuildMonitor {
  ...
  async checkBuildHealth() {
    ...
    try {
      ...
      try {
        ...
        this.log(
          `Build check: FAILED (${this.consecutiveFailures} consecutive failures)`,
          'ERROR'
        );
      }
      ...
      try {
        ...
        results.lint.issues = this.parseLintIssues(
      ...
      try {
        ...
        results.typeCheck.errors = this.parseTypeErrors(
          error.stdout || error.message
        );
  ...
  async sendAlert(results) {
    if (this.consecutiveFailures >= this.alertThreshold) {
      this.log(
        `ALERT: ${this.consecutiveFailures} consecutive build failures!`,
        'CRITICAL'
      );
    ...
    const report = {
      ...
      trends: {
        ...
        improvementSinceLastRun: previousReport
          ? results.build.status === 'success' &&
            previousReport.build.status === 'failed'
          : false,
        degradationSinceLastRun: previousReport
          ? results.build.status === 'failed' &&
            previousReport.build.status === 'success'
          : false,
      },
  ...
  generateRecommendations(results) {
    ...
    if (results.build.duration > 180000) {
      // 3 minutes
  ...
  async run() {
    ...
    try {
      ...
      this.log(
        `Build health check completed. Health score: ${report.healthScore}/100`
      );

      if (report.healthScore < 70) {
        this.log(
          'Build health is below threshold. Consider immediate action.WARN'
        );
      }
    } catch (error) {
