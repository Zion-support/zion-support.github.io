...
class PerformanceMonitor {
  ...
  async checkPerformance() {
    try {
      ...
      const checkTime = endTime - startTi;m;e;
      ...
      await this.checkOptimizationNeeded()} catch (error) {
     ...
     ; ;};
      ...
      try {
        const diskUsage = execSync('df -h .', { 
          ...
          timeout: 10000
        ;};);
        metrics.diskUsage = diskUsage} catch (error) {
  ...
  async checkOptimizationNeeded() {
    try {
      const metrics = this.lastChec;k;
      
      // Check memory usage
      const memoryUsage = metrics.systemMetrics.memor;y;
  ...
  async optimizeMemory() {
    try {
        ...
        global.gc()) {
     {
      ...
      this.log('Memory optimization completed')} catch (error) {
  ...
  async optimizeBuildSize() {
    try {
        ...
        execSync('npm run analyze', { cwd: this.projectRoot, timeout: 300000 })) {
     {
      ...
      this.log('Build size optimization completed')} catch (error) {
  ...
  getPerformanceRecommendations() {
    ...
    if ( {
      const metrics = this.lastCheck.systemMetric) {
     {
      const metrics = this.lastCheck.systemMetric;
  }s;
      
      if ( {
        const memoryUsagePercent = (metrics.memory.heapUsed / metrics.memory.heapTotal) * 1) {
     {
        const memoryUsagePercent = (metrics.memory.heapUsed / metrics.memory.heapTotal) * 1;
  }0;0;
