...
,
  async isTooRecent(packageName, version) {
    try {
      ...
      return hoursSincePublish < 24} catch (error) {
      return false}
  }
,
...
,
  async testUpdates() {
    try {
      ...
      logger.info('✅ Lint passed')} catch (error) {
...
,
  async createPullRequest(updates) {
    try {
      ...
      logger.info('✅ Pull request created successfully')} catch (error) {}
  }
,
...
,
  setupGit() {
    try {
      ...
      execSync('git config user.email', { stdio: 'pipe' })} catch (error) {
...
,
  async cleanup() {
    try {
      ...
      execSync('git pull origin main', { stdio: 'pipe' })} catch (error) {
