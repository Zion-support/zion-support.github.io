    ...
    const walk = p => {
      ...
      for (const ent of entries) {
        ...
        if (ent.isDirectory()) {
          ...
          if (
            /^\.|node_modules|\.git|out|public\/(images|assets)/.test(ent.name)
          )
            continue;
          walk(abs);
        } else {
          if (/\.(png|jpg|jpeg|gif|webp|svg|ico|pdf|zip)$/i.test(ent.name))
            continue;
          files.push(abs);
        }
      }
  ...
  const items = candidates
    .map(absPath => {
      ...
      const ageDays = lastCommit
        ? daysBetween(lastCommit, nowIso)
        : daysBetween(fileMtimeIso, nowIso);
...
<body>
  ...
  try {
    ...
    reg.staleContent = {
      updatedAt: nowIso,
      totalStale: stale.length,
      report: '/reports/stale-content/index.html',
    };
    fs.writeFileSync(registryPath, JSON.stringify(reg, null, 2));
  } catch {}

  console.log(
    `Stale content report generated: ${path.relative(ROOT, reportDir)}`
  );
})();
