...
function isPortInUse(port)  {
  try {
    ...
    return output.trim().length > 0} catch {
    return false}
}
,
function killPort(port)  {
  try {
    ...
    logger.info(`[Watchdog] Killed processes on port ${port}`)} catch (e) {
    // Ignore}
}
,
...
function killAllCursorChats()  {
  try {
    ...
    logger.info('[Watchdog] Killed all completed Cursor chat processes.')} catch (e) {
    // Ignore}
}
,
