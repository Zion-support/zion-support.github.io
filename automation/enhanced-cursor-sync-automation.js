  ...
  getLastSyncTime() {
    ...
    if (fs.existsSync(syncTimeFile)) {
      try {
        ...
        return data.lastSync} catch (error) {
  ...
  async executeCommand(command, options ={}) {
    return new Promise((resolve, reject) => {
      exec(command, {
        ...
        if (error) {
          reject({ error, stdout, stderr })} else {
...
,
      ...
      if (success) {
        metrics.computerStats[this.computerId].successfulSyncs++} else {
...
,
      ...
      if (success) {
        metrics.dailyStats[today].successfulSyncs++} else {
      ...
      fs.writeFileSync(metricsFile, JSON.stringify(metrics, null, 2))} catch (error) {
  ...
  async startContinuousSync() {
    ...
    while (true) {
      try {
        ...
        await this.sleep(this.syncInterval)} catch (error) {
...
if (require.main === module) {
  ...
  if (args.includes('--once') || args.includes('-o')) {
    sync.runOnce().then(success => {
      process.exit(success ? 0 : 1)})} else if (args.includes('--continuous') || args.includes('-c')) {
