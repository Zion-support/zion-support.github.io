...
function generateHtmlReport(report) {
  ...
  html.push(
    '.pill{display:inline-block;margin:2px;padding:2px 8px;border-radius:999px;font-size:12px}'
  );
  ...
  html.push(
    '.build-status{font-size:24px;font-weight:bold;padding:20px;border-radius:12px;text-align:center;margin:20px 0}'
  );
  html.push(
    '.build-success{background:rgba(16,185,129,.2);border:2px solid #10b981}'
  );
  html.push(
    '.build-failure{background:rgba(239,68,68,.2);border:2px solid #ef4444}'
  );
  html.push('</style>');
  ...
  const buildStatusText = summary.canBuild
    ? '✅ BUILD READY'
    : '❌ BUILD WILL FAIL';
  html.push(
    `<div class="build-status ${buildStatusClass}">${buildStatusText}</div>`
  );
  ...
  html.push(
    `<p><strong>Files with Issues:</strong> ${summary.filesWithIssues}</p>`
  );
  ...
  html.push(
    `<p><strong>Build Breaking Files:</strong> ${summary.buildBreakingFiles}</p>`
  );
  html.push(`</div>`);
  ...
  results.forEach(result => {
    if (result.issues.length > 0 || result.warnings.length > 0) {
      html.push(
        `<div class="card ${result.issues.length > 0 ? 'error' : 'warning'}">`
      );
      ...
      if (result.issues.length > 0) {
        ...
        result.issues.forEach(issue => {
          ...
          html.push(
            `<div class="pill error">${issue.code}: ${issue.message}${lineInfo}</div>`
          );
        });
      }
      ...
      if (result.warnings.length > 0) {
        ...
        result.warnings.forEach(warning => {
          ...
          html.push(
            `<div class="pill warning">${warning.code}: ${warning.message}${lineInfo}</div>`
          );
        });
      }
...
function main() {
  ...
  log(
    `   Build status: ${report.summary.canBuild ? '✅ READY' : '❌ WILL FAIL'}`
  );
