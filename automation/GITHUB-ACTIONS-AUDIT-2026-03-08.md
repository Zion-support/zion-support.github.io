...
| **Links & nav** | ai-broken-link-fixer.yml, ai-navigation-audit-fix.yml, ai-site-link-audit-automation | Mon/Thu 06:30, Wed/Sat, manual |
...
1. **ai-weekly-live-link-audit.yml** – Weekly live site link audit (Thu 9 UTC). Runs `site:links:audit` against production, uploads artifact; creates/updates issue if broken links exceed threshold (default 5).
2. **ai-weekly-live-app-audit-auto-fix.yml** – Weekly automated fix run (Sat 9 UTC). Runs live UX audit + site link audit, then broken-link fixer + UX auto-fix, commits and pushes to main. Complements ai-live-app-audit (Fri, manual apply_fix/auto_commit).
3. **ci-cd.yml** – Use `node-version-file: '.nvmrc'` for consistency with deploy-on-push and other workflows; keeps Node version single source of truth.
