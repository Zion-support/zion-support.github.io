...
class SEOOptimizer {
  ...
  scanPages(dir) {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        this.scanPages(filePath)} else if (file.endsWith('.tsx') || file.endsWith('.ts')) {
...
class SecurityScanner {
  ...
  scanCode() {
    const patterns = [
      { pattern: /eval\\(/, description: 'Use of eval() function', severity: 'high' }, { pattern: /innerHTML/, description: 'Potential XSS vulnerability', severity: 'medium' }, { pattern: /localStorage/, description: 'Sensitive data in localStorage', severity: 'low' }
    ];
  ...
  scanDirectory(dir, patterns) {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        this.scanDirectory(filePath, patterns)} else if (file.endsWith('.tsx') || file.endsWith('.ts') || file.endsWith('.js')) {
...
switch (command) {
  ...
  case 'run':
    if (subCommand) {
      factory.runScript(subCommand)} else {
