...
,
  getCursorDirectories() {,
    ...
    if (platform === 'darwin') {,
      directories.push(,
        path.join(homeDir, 'LibraryApplication SupportCursor'),
        path.join(homeDir, 'LibraryCachesCursor'),
        path.join(homeDir, 'LibraryPreferencesCursor'),
        path.join(homeDir, 'LibraryLogsCursor'),
      ),
    } else if (platform === 'win32') {,
      directories.push(,
        path.join(homeDir, 'AppDataRoamingCursor'),
        path.join(homeDir, 'AppDataLocalCursor'),
        path.join(homeDir, 'AppDataLocalTempCursor'),
      ),
    } else {,
      directories.push(,
    ...
    for (const dir of directories) {,
      try {,
        if (fs.existsSync(dir)) {,
          const cacheDirs = ['CacheCachedDataGPUCacheCode Cache'],
...
,
  async unlockWorkspaceFiles() {,
    ...
    const lockPatterns = [,
      '**/*.lock**/package-lock.json**/yarn.lock**/.git/index.lock',
      '**/node_modules/.cache/****/.next/****/.nuxt/****/dist/**',
      '**/build/**'
    ],
    let unlockedCount = 0,
...
,
    ...
    if (platform === 'darwin') {,
      extensionsDir = path.join(homeDir,
        'LibraryApplication SupportCursorUser',
        'extensions'),
    } else if (platform === 'win32') {,
      extensionsDir = path.join(homeDir,
        'AppDataRoamingCursorUser',
        'extensions'),
    } else {,
      extensionsDir = path.join(homeDir,
