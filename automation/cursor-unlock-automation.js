...
,
  getCursorProcesses() {
    ...
    let command = '',
    if (platform === 'darwin') {
      command = 'ps aux | grep -i cursor | grep -v grep'} else if (platform === 'win32') {
      command = 'tasklist /FI "IMAGENAME eq Cursor.exe" /FO CSV'} else {
    ...
    for (const process of processes) {
      try {
        ...
        if (platform === 'darwin') {
          ...
          killCommand = `kill -9 ${pid}`} else if (platform === 'win32') {
          killCommand = 'taskkill /F /IM Cursor.exe'} else {
          const pid = process.split(/\s+/)[1],
          killCommand = `kill -9 ${pid}`}
,
        await this.executeCommand(killCommand),
        killedCount++,
        this.log(`✅ Killed Cursor process: ${process}`)} catch (error) {
