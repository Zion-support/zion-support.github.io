...
class OptimizationAutomation {
  ...
  setupAPIRoutes() {
    ...
    this.app.post('/api/slack/events', async (req, res) => {
      try {
        ...
        process.stderr.write(
          `[${new Date().toISOString()}] ERROR: Slack event error: ${error.message}\n`
        );
    ...
    this.app.post('/api/optimization/trigger', async (req, res) => {
      try {
        ...
        process.stdout.write(
          `[${new Date().toISOString()}] 🚀 Manual optimization triggered: ${target} (reason: ${reason})\n`
        );
        ...
        // Use structured error logging
        process.stderr.write(
          `[${new Date().toISOString()}] ERROR: Optimization trigger error: ${error.message}\n`
        );
        res.status(500).json({
          success: false,
          error: error.message,
    ...
    this.app.get('/api/performance/status', async (req, res) => {
      try {
        ...
        process.stderr.write(
          `[${new Date().toISOString()}] ERROR: Performance status error: ${error.message}\n`
        );
    ...
    this.app.get('/api/performance/metrics', async (req, res) => {
      try {
        ...
        process.stderr.write(
          `[${new Date().toISOString()}] ERROR: Performance metrics error: ${error.message}\n`
        );
        res.status(500).json({ error: error.message });
      }
    });
    ...
    this.app.get('/api/performance/history', async (req, res) => {
      try {
        ...
        process.stderr.write(
          `[${new Date().toISOString()}] ERROR: Performance history error: ${error.message}\n`
        );
        res.status(500).json({ error: error.message });
      }
    });
  ...
  async handleSlackEvent(event) {
    switch (event.type) {
      ...
      default:
        process.stdout.write(
          `[${new Date().toISOString()}] ⚠️ Unhandled event type: ${event.type}\n`
        );
    }
  }
  ...
  async start() {
    try {
      process.stdout.write(
        `[${new Date().toISOString()}] 🚀 Starting Optimization Automation System...\n`
      );
      ...
      this.server = this.app.listen(this.port, () => {
        process.stdout.write(
          `[${new Date().toISOString()}] ⚡ Optimization API server running on port ${this.port}\n`
        );
      });

      const timestamp = new Date().toISOString();
      process.stdout.write(
        `[${timestamp}] ✅ Optimization Automation System started successfully!\n`
      );
      process.stdout.write(
        `[${timestamp}] 📊 Dashboard: http://localhost:${this.port}/dashboard\n`
      );
      process.stdout.write(
        `[${timestamp}] 🔧 API: http://localhost:${this.port}/api\n`
      );
      process.stdout.write(
        `[${timestamp}] 💚 Health: http://localhost:${this.port}/health\n`
      );
    } catch (error) {
      process.stderr.write(
        `[${new Date().toISOString()}] ❌ Failed to start automation system: ${error.message}\n`
      );
      process.exit(1);
    }
  }
  ...
  async stop() {
    try {
      process.stdout.write(
        `[${new Date().toISOString()}] ⏹️ Stopping Optimization Automation System...\n`
      );
      ...
      process.stdout.write(
        `[${new Date().toISOString()}] ✅ Automation system stopped\n`
      );
    } catch (error) {
      process.stderr.write(
        `[${new Date().toISOString()}] ❌ Error stopping automation system: ${error.message}\n`
      );
    }
  }
}
...
if (require.main === module) {
  ...
  automation.start().catch(error => {
    process.stderr.write(
      `[${new Date().toISOString()}] ❌ Failed to start automation: ${error.message}\n`
    );
    process.exit(1);
  });
}
