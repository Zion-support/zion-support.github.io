...
expected = {
    ...
    'latestComplete': metrics.get('latestComplete') or latest.get('latestComplete'),
}
...
for r in records:
    if r.get('event') in ('complete', 'auth_missing'):
        ts = r.get('ts')
        if ts:
            computed['latestComplete'] = ts if computed['latestComplete'] is None else max(computed['latestComplete'], ts)
    src = r.get('summary', r)
...
print('SCHEMA_OK')
print('JSONL_OK')
print(f'METRICS_RECONCILED' if ok else f'METRICS_MISMATCH computed={computed} expected={expected}')
