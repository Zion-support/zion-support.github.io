...
/* eslint-disable no-console */
/**
 * Compare production vs preview AI Lab hub-links smoke.
 *
 * Env:
 *   PROD_REPORT    default automation/reports/ai-lab-hub-links-smoke-prod.json
 *   PREVIEW_REPORT default automation/reports/ai-lab-hub-links-smoke-preview.json
 *   OUT_REPORT     default automation/reports/ai-lab-hub-links-smoke-compare-latest.json
 *   AI_LAB_HUB_COMPARE_FAIL_ON_REGRESSION default "1"
 */
const fs = require('fs');
const path = require('path');
...
function main() {
  const prod = readJsonSafe(PROD_REPORT);
  const preview = readJsonSafe(PREVIEW_REPORT);
  const prodFail = failSet(prod);
  const previewFail = failSet(preview);

  const regressedInPreview = [...previewFail].filter((p) => !prodFail.has(p)).sort();
  const improvedInPreview = [...prodFail].filter((p) => !previewFail.has(p)).sort();

  const out = {
    generatedAt: new Date().toISOString(),
    prodReportPath: path.relative(ROOT, PROD_REPORT),
    previewReportPath: path.relative(ROOT, PREVIEW_REPORT),
    prodPresent: Boolean(prod),
    previewPresent: Boolean(preview),
    prodFailedCount: prodFail.size,
    previewFailedCount: previewFail.size,
    regressedInPreview,
    improvedInPreview,
    ok: regressedInPreview.length === 0,
  };
