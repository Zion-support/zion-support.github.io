...
function log(m) {
  console.log(`[${new Date().toISOString()}] [seo-factory] ${m}`);
}
...
(function main() {
  ...
  for (const focus of focuses) {
    const agent = {
      ...
      focus,
      objective:
        'Improve Core Web Vitals, internal link graph, and metadata completeness',
      guards: ['build'],
      actions:
        focus === 'metadata'
          ? ['generate-missing-meta', 'fix-duplicates']
          : focus === 'links'
            ? ['generate-internal-links', 'fix-broken-links']
