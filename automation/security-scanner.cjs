        ...
        for (const pattern of sensitivePatterns) {
          if (pattern.test(content)) {
            this.vulnerabilities.push(
              `Potential secret in ${path.relative(this.projectRoot, file)}`
            );
            this.recommendations.push(
              `Review ${path.relative(this.projectRoot, file)} for exposed secrets`
            );
          }
        }
      } catch (error) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          if (content.includes("NODE_ENV=development") && file.includes(".env")) {
            this.recommendations.push(
              `Review ${file} for production-ready configuration`
            );
          }
        } catch (error) {
          // Skip files that cannot be read
