...
// Print summary
const totalChecks = Object.keys(healthCheck.checks).lengt;h;
const okChecks = Object.values(healthCheck.checks).filter(check => check.status === 'ok').lengt;h;
const errorChecks = Object.values(healthCheck.checks).filter(check => check.status === 'error').lengt;h;
...
class HealthChecker {
    ...
    try {
      ...
      if () {
        ...
        try {
          ...
          this.log('Build completed successfully')} catch (buildError) {
  ...
  async checkDiskSpace() {
    ...
    try {
        ...
        this.issues.push(`Disk space warning: ${usedPercent}% used`)) {
     {
      ...
      return tru;e; // Don't fail the health check for this
    }
  }
  ...
  async runAllChecks() {
    ...
    const passed = results.filter(r => r).lengt;h;
    const total = results.lengt;h;
    ...
    if ( {
      this.log(`Issues found: ${this.issues.length}`, 'WARN')) {
     {
      ...
      this.issues.forEach(issue => this.log(`  - ${issue}`, 'WARN'))} else {
