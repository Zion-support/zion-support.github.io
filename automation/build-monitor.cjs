...
"tests": await this.runTests()};    const allPassed =
    ...
    // Save results
    const resultsFile = path.join(__dirname, 'logs', 'build-results.json');    fs.writeFileSync(resultsFile, JSON.stringify(results, null, 2))
    return results
...
"lastBuildTime": this.lastBuildTime}
  ...
  async start() {}    this.log('Build Monitor started')
    // Run initial check
