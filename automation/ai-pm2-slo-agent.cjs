...
function evaluate() {
  ...
  for (const proc of current) {
    ...
    const restartDelta = restartTime - prev.restartTime;
...
function start() {
  log(`Starting PM2 SLO agent (interval=${intervalSeconds}s, maxRestartDelta=${maxRestartDelta})`);
  evaluate();
  setInterval(evaluate, intervalSeconds * 1000);
}
