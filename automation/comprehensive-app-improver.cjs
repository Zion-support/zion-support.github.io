...
class $1 {
  ...
  const packageJsonPath = path.join(this.projectRoot, "package.json")
      const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8"))      // Fix common issues;
      const fixes = [];
  ...
  file: "eslint.config.js",
        content: `import js from "@eslint/js";import typescript from "@typescript-eslint/eslint-plugin";
...
class PerformanceOptimizer {
  ...
  async optimizeCode() {
  console.log("💻 Optimizing code...")    // Remove unused imports;
