...
function determineFailoverStrategy(githubActions, pm2Redundancy, netlifyFunctions) {
  ...
  if (pm2Redundancy.available && pm2Redundancy.runningCount > 0) {
    ...
    log("PM2 redundancy system selected as primary")} else if (githubActions.available && githubActions.workflowCount > 0) {
    strategy.primary = "github-actions",
    log("GitHub Actions selected as primary")} else if (netlifyFunctions.available && netlifyFunctions.functionCount > 0) {
...
,
  ...
  if (strategy.primary !== "github-actions" && githubActions.available) {
    ...
    log("GitHub Actions selected as secondary")} else if (strategy.primary !== "pm2-redundancy" && pm2Redundancy.available) {
    strategy.secondary = "pm2-redundancy",
    log("PM2 redundancy selected as secondary")} else if (strategy.primary !== "netlify-functions" && netlifyFunctions.available) {
...
,
  ...
  if (strategy.primary !== "netlify-functions" && strategy.secondary !== "netlify-functions" && netlifyFunctions.available) {
    ...
    log("Netlify functions selected as tertiary")} else if (strategy.primary !== "pm2-redundancy" && strategy.secondary !== "pm2-redundancy" && pm2Redundancy.available) {
    strategy.tertiary = "pm2-redundancy",
    log("PM2 redundancy selected as tertiary")} else if (strategy.primary !== "github-actions" && strategy.secondary !== "github-actions" && githubActions.available) {
