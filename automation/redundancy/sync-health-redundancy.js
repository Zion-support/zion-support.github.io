...
function stashAll() {
  ...
  if (res.status !== 0) {
    log(`Stash skipped or failed (status ${res.status}): ${res.stderr}`)} else {
  ...
  try {
    ...
    if (behind > 0) {
      ...
      if (pullResult.status !== 0) {
        ...
        return false}
      log("Successfully pulled latest changes.")} else if (ahead > 0) {
      ...
      if (pushResult.status !== 0) {
        ...
        return false}
      log("Successfully pushed local changes.")} else {
