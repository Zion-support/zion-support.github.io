...
const StaleCleaner = require('./tasks/StaleCleaner'),
  ...
  try {
    ...
    if (stats.tasks && Object.keys(stats.tasks).length > 0) {
      ...
      results.passed++,
      logger.info('✅ Task scheduler recording works')} else {
...
,
    ...
    if (anomalyStats.total >= 0) {
      ...
      results.passed++,
      logger.info('✅ Anomaly detection works')} else {
...
,
    ...
    try {
      ...
      results.passed++,
      logger.info('✅ Notification system works')} catch (error) {
...
,
    ...
    try {
      ...
      if (report && report.content) {
        ...
        results.passed++,
        logger.info('✅ Report generation works')} else {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (config.autonomous && config.tasks) {
          ...
          results.passed++,
          logger.info('✅ Configuration loading works')} else {
