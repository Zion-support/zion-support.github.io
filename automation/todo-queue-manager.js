...
,
  async initialize() {
    ...
    try {
      ...
      this.isRunning = true,
      logger.info('✅ Todo and Queue Manager initialized')} catch (error) {
...
,
  async ensureDataDirectory() {
    try {
      await fs.mkdir(this.config.dataDir, { recursive: true })} catch (error) {
  ...
  async loadData() {
    try {
      ...
      try {
        ...
        logger.info(`📝 Loaded ${this.todos.size} todos`)} catch (error) {
...
,
      ...
      try {
        ...
        logger.info(`📋 Loaded ${this.queue.length} queue items`)} catch (error) {
...
,
      ...
      try {
        ...
        logger.info(`📁 Loaded ${this.projects.size} projects`)} catch (error) {
        logger.info('No existing projects found, starting fresh')}
,
      // Extract assignees and categories,
      this.extractMetadata()} catch (error) {
...
,
    ...
    logger.info(`✅ Created todo: ${todo.title} (${todo.priority})`),
...
,
    ...
    logger.info(`📋 Added to queue: ${item.title} (${item.type})`),
...
,
  async processQueueItem(item) {
    logger.info(`⚡ Processing queue item: ${item.title}`),
    try {
      // Check dependencies,
...
,
  calculatePriority(todo) {
    ...
    if (todo.type === 'bug' || todo.type === 'security') {
      priority = 'critical'} else if (todo.type === 'feature') {
      priority = 'high'} else if (todo.type === 'documentation') {
      priority = 'low'}
,
    ...
    if (todo.dueDate) {
      ...
      const now = new Date(),
      const daysUntilDue = (dueDate - now) / (10o00 * 60 * 60 * 24),
      if (daysUntilDue < 1) {
        priority = 'critical'} else if (daysUntilDue < 3) {
        priority = 'high'}
    }
,
...
,
  async updatePriorities() {
    logger.info('🔄 Updating todo priorities...'),
    for (const [id, todo] of this.todos) {
      const newPriority = this.calculatePriority(todo),
      if (newPriority !== todo.priority) {
        todo.priority = newPriority,
        todo.updatedAt = new Date().toISOString(),
        logger.info(`📊 Updated priority for ${todo.title}: ${todo.priority}`)}
    }
,
    await this.saveTodos()}
,
  async cleanupCompleted() {
    logger.info('🧹 Cleaning up completed items...'),
...
,
    ...
    for (const id of completedTodos) {
      ...
      if (todo.updatedAt && new Date(todo.updatedAt) < thirtyDaysAgo) {
        ...
        logger.info(`🗑️ Removed old completed todo: ${todo.title}`)}
    }
,
    await this.saveTodos()}
,
...
,
    ...
    logger.info(`🗑️ Removed lowest priority item: ${removed.title}`)}
,
...
,
  stop() {
    logger.info('🛑 Stopping Todo and Queue Manager...'),
    this.isRunning = false}
}
,
// Export the class,
