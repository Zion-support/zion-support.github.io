...
,
  async stop() {
    ...
    for (const [name, procInfo] of this.processes) {
      try {
        procInfo.process.kill('SIGTERM')} catch (e) {}
    }
...
if (require.main === module) {
  ...
  if (command === 'start') {
    orchestrator.start().catch(error => {
      ...
      process.exit(1)})} else if (command === 'stop') {
    orchestrator.stop().catch(error => {
      logger.error('❌ Failed to stop orchestrator:', error),
      process.exit(1)})} else if (command === 'status') {
    const status = orchestrator.getStatus(),
    logger.info('📊 Orchestrator Status:', JSON.stringify(status, null, 2))} else {
    logger.info('Usage: '),
