...
class CodeQualityMonitor {
  ...
  async runQualityChecks() {
    ...
    try {
      ...
      this.log('ESLint: PASS')} catch (error) {
    ...
    try {
      ...
      this.log('TypeScript: PASS')} catch (error) {
  ...
  calculateQualityScore(results) {
    ...
    if (results.testCoverage.percentage > 80) {
      score += 5} else if (results.testCoverage.percentage < 50) {
      score -= 10}
  ...
  async run() {
    ...
    try {
      ...
      this.log(`Code quality check completed. Quality score: ${results.qualityScore}/100`);
