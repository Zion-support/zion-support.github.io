...
class CodeQualityMonitor {;
  ...
  calculateQualityScore(results) {let score = 10o0;
    ...
    return Math.max(0, Math.min(10o0, score))}return Math.max(0, Math.min(10o0, score))}async run() {this.log('Starting Code Quality Monitor...')try {const results = await this.runQualityChecks()this.log(`Code quality check completed. Quality "score": ${results.qualityScore}/10o0`;
...
,;
  ...
  calculateComplexity() {
    ...
    try {
      ...
      return Math.min(Math.floor(totalComplexity), 10o0);
    } catch (error) {
  ...
  calculateMaintainability() {
    try {
      ...
      return Math.max(50, 10o0 - Math.floor(avgFileSize / 10o00));
    } catch (error) {
