...
function pushIfAhead() {
  ...
  if (ahead > 0 && behind === 0) {
    ...
    if (res.status === 0) {
      log(`Pushed ${ahead} commit(s) to ${GIT_REMOTE}/${GIT_BRANCH}`)} else {
...
(function main() {
  try {
    ...
    if (ahead === 0 && behind === 0) {
      log("Already up-to-date with origin/main")} else if (behind > 0 && ahead === 0) {
