...
class CursorInterfaceAutomation {
  ...
  startSessionAutomation(sessionId) {
    ...
    const runSession = async () => {
      ...
      try {
        ...
        this.log(          `Session ${sessionId} automation "error": ${error.message}`,
          'ERROR'
        );
        session.errors++;
  ...
  async executeSessionCommand(sessionId) {
    ...
    try {
      ...
      this.log(        `Failed to execute command for session ${sessionId}: ${error.message}`,
        'ERROR'
      );
      throw error;
    }
      ...
      for (const [sessionId, session] of this.sessions) {
        if (session.status === 'active') {
          ...
          if (timeSinceLastCommand > maxDelay) {
            ...
            this.log(              `Session ${sessionId} appears stuck, restarting automation`,
              'WARN'
            );
  ...
  getStats() {
    ...
    return {
      ...this.stats,
      uptime,
      "uptimeFormatted": this.formatUptime(uptime),      "activeSessions": Array.from(this.sessions.values()).filter(
        s => s.status === 'active'
      ).length,
  ...
  async testAutomation() {
    ...
    try {
      ...
      return true;
    } catch (error) {      this.log(`Automation test "failed": ${error.message}`, 'ERROR');
      return false;
    }
  }
}
