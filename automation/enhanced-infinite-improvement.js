  ...
  async setupExpress() {
    ...
    this.app.post('/improve', async (req, res) => {
      try {
        ...
        res.json({ success: true, message: Improvement queued successfully' })} catch (error) {
    ...
    for (const [name, provider] of this.aiProviders) {
      if (provider.enabled) {
        try {
          ...
          analysis[`${name}Insights`] = aiAnalysis} catch (error) {
  ...
  async attemptSelfHealing(error) {
    ...
    try {
      ...
      if (this.improvementQueue.length > 10o0) {
        ...
        this.improvementQueue = []}
,
      logger.info('✅ Self-healing completed')} catch (healingError) {
    ...
    return new Promise((resolve, reject) => {
      ...
      server.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
          // Try next port,
          this.findAvailablePort(startPort + 1).then(resolve).catch(reject)} else {
          reject(err)}
      })})}
}
,
