...
class SecurityAuditor {
  ...
  async checkCodeSecurity() {
    ...
    try {
      ...
      const dangerousFunctions = this.findPattern(/eval\s*\(|innerHTML\s*=|document\.write\s*\(/g, '**/*.{js,jsx,ts,tsx}');
      ...
      const sqlInjection = this.findPattern(/query\s*\(\s*['"][^'"]*\+/g, '**/*.{js,jsx,ts,tsx}');
  ...
  async checkSensitiveData() {
    ...
    try {
      const sensitivePatterns = [
        ...
        { pattern: /api[_-]?key\s*[:=]\s*['"][^'"]+['"]/gi, type: 'api_key', severity: 'critical' },
      ...
      sensitivePatterns.forEach(({ pattern, type, severity }) => {
        const matches = this.findPattern(pattern, '**/*.{js,jsx,ts,tsx,json,env}');
        if (matches.length > 0) {
          sensitiveData.push({
            type,
            severity,
  ...
  async checkSecurityHeaders() {
    ...
    try {
      ...
      if (fs.existsSync(nextConfig)) {
        ...
        securityConfig.hasSecurityHeaders = configContent.includes('security') || 
  ...
  findPattern(pattern, globPattern) {
    try {
      const result = execSync(`grep -r "${pattern}" --include="*.js" --include="*.jsx" --include="*.ts" --include="*.tsx" --include="*.json" --include="*.env" . || true`, {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  generateRecommendations() {
    ...
    const criticalIssues = this.auditResults.securityIssues.filter(i => i.severity === 'critical');
    if (criticalIssues.length > 0) {
      recommendations.push('Remove hardcoded secrets and sensitive data from code');
    }
    
    const highIssues = this.auditResults.securityIssues.filter(i => i.severity === 'high');
