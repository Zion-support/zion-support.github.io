...
function generateSeries(n, base, volatility = 0.15) {
  const series = [];
  let last = base;
  for (let i = 0; i < n; i++) {
...
function generateMetrics({ includeCompare = true } = {}) {
  const now = new Date();
...
function generateOperatorSummary(metrics) {
...
module.exports = { generateMetrics, generateOperatorSummary };