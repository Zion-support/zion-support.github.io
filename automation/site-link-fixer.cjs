    ...
    const title =
      routePath === '/'
        ...
        : routePath
            ...
            .join(' · ');
    const message =
      'This page was autogenerated to fix a broken internal link. You can customize it later.';
...
function main() {
  ...
  const routes = Array.isArray(data.internalMissingRoutes)
    ? data.internalMissingRoutes
    : [];
  ...
  console.log(
    `Link fixer completed. Created/verified ${results.length} pages. Details: public/automation/link-fixes.json`
  );
}
