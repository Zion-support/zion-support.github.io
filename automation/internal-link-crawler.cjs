...
function extractInternalLinks(text) {
  ...
  for (const re of patterns) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      if (
        /\.(html|json|xml|png|jpg|jpeg|gif|svg|webp|ico|txt)(\?|#|$)/i.test(
          href
        )
      )
        continue;
...
const possiblePageFiles = route => {
  if (route === '/')
    return [
      ...
      path.join(PAGES_DIR, 'index.jsx'),
      path.join(PAGES_DIR, 'index.ts'),
...
function main() {
  ...
  console.log(
    `Internal link scan complete. Routes: ${allRoutes.length}. Missing: ${missing.length}. Report: ${outFile}`
  );
}
