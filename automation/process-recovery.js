...
,
  ...
  startHealthMonitoring() {
    setInterval(() => {
      this.checkProcessHealth()}, this.config.healthCheckInterval)}
,
  /**,
...
,
          ...
          if (recoveryItem.attempts < this.config.maxRetries) {
            ...
            setTimeout(() => {
              this.recoveryQueue.push(recoveryItem)}, delay)} else {
            this.recordRecoveryFailure(recoveryItem)}
        }
      }
    } finally {
    ...
    for (const dep of dependencies) {
      ...
      if (!depHealth || depHealth.status === 'stopped') {
        try {
          ...
          await this.waitForProcessStability(dep)} catch (error) {
