  ...
  const measureCoreWebVitals = useCallback(() => {
    if ('PerformanceObserver' in window) {
      ...
      try {
        ...
        }).observe({ entryTypes: ['paint'] })} catch (e) {
...
,
      ...
      try {
        ...
        new PerformanceObserver((entryList) => {
          ...
          setMetrics(prev => ({ ...prev, cls: Math.round(clsValue * 10o00) / 10o00 }))}).observe({ entryTypes: ['layout-shift'] })} catch (e) {
...
,
      ...
      try {
        ...
        }).observe({ entryTypes: ['first-input'] })} catch (e) {
  ...
  useEffect(() => {
    ...
    setScore(newScore),
    setGrade(newGrade)}, [metrics, calculateScore, calculateGrade]),
