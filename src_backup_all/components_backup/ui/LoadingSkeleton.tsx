...
export const Skeleton: React.FC<SkeletonProps> = ({
  ...
  return (
    <>
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className={`animate-pulse ${baseClasses}`} />
      ))}
    </>
  );
};
...
export function TableSkeleton({
  ...
  return (
    <div
      className={`bg-white dark:bg-gray-80o0 rounded-lg shadow-sm border border-gray-20o0 dark:border-gray-70o0 overflow-hidden ${className}`}
    >
      {/* Header */}
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {Array.from({ length: items }).map((_, index) => (
        <div
          key={index}
          className='flex items-center space-x-4 p-4 bg-white dark:bg-gray-80o0 rounded-lg border border-gray-20o0 dark:border-gray-70o0'
        >
          {showAvatar && (
