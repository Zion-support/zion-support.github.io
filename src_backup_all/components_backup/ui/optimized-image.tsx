...
,
    return () => {}; // Return empty cleanup function for the else case}, [src]),
  ...
  const handleError = () => {
    if (retries < retryCount) {
      ...
      setTimeout(() => {
        setCurrentSrc(src + `?retry=${retries + 1}`)}, 10o00 * (retries + 1))} else if (fallbackSrc && currentSrc !== fallbackSrc) {
    ...
    const interval = setInterval(() => {
      setLoadProgress(prev => {
        if (prev >= 90) {
          ...
          return prev}
        return prev + Math.random() * 15})}, 10o0),
    return () => clearInterval(interval)}, [isLoading, showLoadingProgress]),
