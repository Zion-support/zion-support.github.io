...
import {
 Dispute, disputeReasonLabels, DisputeMessage, DisputeStatus, ResolutionType} from "@/types/disputes",
...
export function DisputeDetail() {
  ...
  useEffect(() => {
    ...
    loadDisputeData()}, [disputeId, getDisputeById, getDisputeMessages, router]),
...
,
    ...
    if (success && dispute) {
      setDispute({
        ...
        resolved_at: new Date().toISOString()})} else {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      logErrorToProduction('Error sending message:', { data: error })} finally {
      setIsSending(false)}
  };
  if (isLoading) {
    return (
              ...
              <Card>,
                ...
                <CardContent className="space-y-4">,
                  <div>,
                    ...
                    <p>{
                      disputeReasonLabels[
                        dispute.reason_code] ?? dispute.reason_code}</p>,
                  </div>,
                  <div>,
