...
,
    ...
    fetch(`/api/search/suggest?q=${encodeURIComponent(debounced)}`, {
      ...
      .then(data => {
        if (Array.isArray(data)) {
          setFilteredSuggestions(data.slice(0, 8))} else {
          setFilteredSuggestions([])}
        setHighlightedIndex(-1)}),
      .catch(() => setFilteredSuggestions([])),
    return () => controller.abort()}, [debounced, searchSuggestions]),
...
,
  ...
  const handleSelectSuggestion = (suggestionObj: SearchSuggestion) => {
    ...
    if (onSelectSuggestion) {
      ...
      onSelectSuggestion(suggestionObj)} else {
      ...
      if (suggestionObj.id) {
        router.push(`/marketplace/listing/${suggestionObj.id}`)} else if (suggestionObj.type === 'doc' && suggestionObj.slug?.startsWith('/')) {
        router.push(suggestionObj.slug)} else if (suggestionObj.type === 'blog' && suggestionObj.slug) {
        router.push(`/blog/${suggestionObj.slug}`)} else {
    ...
    switch (e.key) {
      ...
      case 'Enter':,
        if (isFocused && highlightedIndex !== -1 && filteredSuggestions[highlightedIndex]) {
          ...
          handleSelectSuggestion(filteredSuggestions[highlightedIndex])} else if (value.trim()) {
