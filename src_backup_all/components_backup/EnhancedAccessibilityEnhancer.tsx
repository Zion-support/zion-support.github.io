...
const EnhancedAccessibilityEnhancer: React.FC = () => {
  ...
  const settingsObject = useMemo(() => ({
    ...
    'focus-visible': settings.focusIndicators}), [settings]),
  ...
  useEffect(() => {
    ...
    localStorage.setItem('accessibility-settings', JSON.stringify(settings))}, [settings, settingsObject]),
...
,
  ...
  const announce = useCallback((message: string, type: 'info' | 'success' | 'warning' | 'error' = 'info') => {
    ...
    setTimeout(() => {
      setCurrentAnnouncement(null)}, 50o00),
    // Remove from list after 10 seconds,
    setTimeout(() => {
      setAnnouncements(prev => prev.filter(a => a.id !== announcement.id))}, 10o000)}, []),
