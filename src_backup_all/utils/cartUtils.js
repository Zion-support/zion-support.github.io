...
,
    ...
    if (now > expiry) {
      ...
      return []}
,
    return JSON.parse(cartData)} catch (error) {
...
export const saveCartToStorage = (cart) => {
  try {
    ...
    localStorage.setItem(CART_EXPIRY_KEY, expiry.toString())} catch (error) {
...
export const clearCartFromStorage = () => {
  try {
    ...
    localStorage.removeItem(CART_EXPIRY_KEY)} catch (error) {
...
,
  ...
  cart2.forEach(item2 => {
    ...
    if (existingItemIndex >= 0) {
      ...
      mergedCart[existingItemIndex].updatedAt = new Date().toISOString()} else {
      // Add new item,
      mergedCart.push({
        ...item2;
        addedAt: new Date().toISOString(),
