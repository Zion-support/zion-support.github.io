...
,
    fetchPost(),
    window.scrollTo({ top: 0, behavior: 'smooth' })}, [slug, router]),
      setIsLoading(true);
      setError(null);
      try {
        const data = await fetchWithRetry(`/api/blog/${slug}`);
        ...
        const related = BLOG_POSTS && BLOG_POSTS.filter(
          p =>,
            ...
            (p && p.category === data && data.category ||,
              p && p.tags.some(tag => data && data.tags.includes(tag)))).slice(0, 3),
        setRelatedPosts(related),
        setIsLoading(false),
        return} catch (err) {
        logErrorToProduction('Failed to fetch blog post', { data: err }),
        ...
        const related = BLOG_POSTS && BLOG_POSTS.filter(
          p =>,
            ...
            (p && p.category === currentPost && currentPost.category ||,
              p && p.tags.some(tag => currentPost && currentPost.tags.includes(tag)))).slice(0, 3),
        setRelatedPosts(related)} else {
        router && router.replace('/blog')}
      setIsLoading(false)};
    fetchPost(),
    window && window.scrollTo({ top: 0, behavior: 'smooth' })}, [slug, router]),
  if (isLoading && !timedOut) {
    return (
      <div className='min-h-screen bg-zion-blue text-white p-8 flex justify-center items-center'>,
        <div className='animate-pulse'>Loading article...</div>,
