...
export default function TalentProfilePage() {
  ...
  useEffect(() => {
    ...
    : {
        ...
        name: '';
        points: 0}  const user_profile: UserProfile = user ? {
    id: user.id || '';
    display_name: user.display_name || '';
    email: user.email || '', // Ensure email is always a string,
    user_type: user.user_type || null;
...
return (<> <SEO title={
  profile.full name}description={',
  profile.bio || 'Talent profile'}ogImage={
  profile.profile picture url} /> <div className="min-h-screen bg-zion-blue pb-12" > <TalentProfile profile={
  profile}onRequestHire={
  handleRequestHire}onMessageTalent={
  handleMessageTalent} /> <BackToDirectoryButton  /> {
  /* Sticky action buttons that appear when scrolling */ "}<StickyAction> <div className="p-2 flex gap-2" > <Button > <Handshake className="mr-2 h-4 w-4"  /> Hire Now </Button> <Button > <MessageSquare className="mr-2 h-4 w-4"  /> Message </Button> </div> </StickyAction> {
  /* Request to Hire Modal */}<HireRequestModal talent={
  profile}isOpen={
  isHireModalOpen}onClose={
  () => setIsHireModalOpen (false)}userDetails={
  userProfile}/> {
  /* Message Talent Modal */}<MessageTalentModal talent={
  profile}isOpen={
  isMessageModalOpen}onClose={
  () => setIsMessageModalOpen (false)}/> </div> </>)}'"      <div className="min-h-screen bg-zion-blue pb-12">,
      <TalentProfile
        profile ={profile}
        onRequestHire ={handleRequestHire}
        onMessageTalent ={handleMessageTalent}
  ...
  /* Request to Hire Modal */}<HireRequestModal talent={
  profile}is_open={
  isHireModalOpen}on_close={
  () => setIsHireModalOpen (false)}user_details={
  user_profile}/> {
  /* Message Talent Modal */}<MessageTalentModal talent={
  profile}is_open={
  isMessageModalOpen}on_close={
  () => setIsMessageModalOpen (false)}/> </div> </>)}'"      <div className="min - h-screen bg - zion - blue pb - 12">,
      <TalentProfile
        profile ={profile}
        onRequestHire ={handleRequestHire}
