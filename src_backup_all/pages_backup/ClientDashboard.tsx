...
return (<> <SEO title="Client Dashboard | Zion AI Marketplace" description="Manage your jobs and talent requests in the Zion AI Marketplace."  /> <main className="container mx - auto px - 4 py - 8"> <div className={
  ...
  `flex gap - 2 $ {',
  is_mobile ? 'flex - col': ''}` "}> <Button variant="outline" as_child className={',
  is_mobile ? 'w - full justify - center': '' "}> <Link href="/hiring - tracker"> <Kanban className="h - 4 w - 4 mr - 2"  /> Hiring Pipeline </Link> </Button> <Button as_child className={',
  ...
  selectedJobId "} />) : (<div className="bg - muted / 30 border rounded - lg p - 6 text - center"> <p className="text - muted - foreground"> Select a job to see AI - matched talent suggestions </p> </div>)}</div> </div> </div> </div> </main> </>)}export default /**,
 * ClientDashboard - Function description,
 */,
function ClientDashboard() {
  return (<ProtectedRoute> <ClientDashboardContent  /> </ProtectedRoute> '"}
