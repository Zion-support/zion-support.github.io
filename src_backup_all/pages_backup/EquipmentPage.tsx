...
const EquipmentFilterControls = ({
  sortBy, setSortBy, filterCategory, setFilterCategory, categories, showRecommended, setShowRecommended, loading}: any) => (
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      refresh()}, 10o0), // Small delay to prevent rapid successive refreshes,
    return () => clearTimeout(timeoutId)}, [sortBy, filterCategory, showRecommended, refresh]);
...
,
  ...
  if (error && equipment.length === 0) {
    return (
      ...
      </div>)}, [sortBy, filterCategory, showRecommended, dataSeed]);
  const {
    items: equipment;
    loading;
    error;
              ...
              <EquipmentCard
                ...
                onViewDetails={() => {
                  if (typeof window !== 'undefined') {
                    try {
                      sessionStorage.setItem(`equipment: ${item.id}`, JSON.stringify(item))} catch {
