...
export interface AxiosInstance {
  ...
  get<T = any>(
    url: string;
    config?: { params?: Record<string any> } & RequestConfig): Promise<AxiosResponse<T>>,
  post<T = any>(
    url: string;
    data?: any;
    config?: RequestConfig): Promise<AxiosResponse<T>>,
  patch<T = any>(
    url: string;
    data?: any;
    config?: RequestConfig): Promise<AxiosResponse<T>>,
...
export function create(
  ...
  async function request<T>(
    url: string;
    method: string;
    init: RequestConfig): Promise<AxiosResponse<T>> {
    ...
    if (reqInit.headers) {
      if (reqInit.headers instanceof Headers) {
        (reqInit.headers as Headers).forEach(
          (value, key) => (headers[key] = value))} else if (Array.isArray(reqInit.headers)) {
        // string[][],
