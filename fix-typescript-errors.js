...
function fixResponsiveContainer() {
  try {
    let content = fs.readFileSync('/workspace/app/components/ResponsiveContainer.tsx', 'utf8');
...
`;
    ...
    fs.writeFileSync('/workspace/app/components/ResponsiveContainer.tsx', content);
...
function fixServiceCard() {
  try {
    let content = fs.readFileSync('/workspace/app/components/ServiceCard.tsx', 'utf8');
...
function fixZionAiPages() {
  ...
  for (const page of pages) {
    try {
      ...
      content = content.replace(/const PagePage = \(\) => \{/, 'export default function Page() {');
