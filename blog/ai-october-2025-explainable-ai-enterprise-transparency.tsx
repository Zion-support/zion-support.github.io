import React from 'react';

export default function AIOctober2025ExplainableAIEnterpriseTransparency() {
  return (
    <article className="min-h-screen bg-slate-950 text-white">
      <div className="container mx-auto px-6 py-20">
        {/* Hero Section */}
        <header className="mb-16">
          <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-gradient-to-r from-blue-500/20 to-indigo-500/20 border border-blue-500/30 mb-6">
            <span className="text-blue-400 font-bold text-sm tracking-wider uppercase">
              October 2025 • Explainable AI & Trust
            </span>
          </div>
          
          <h1 className="text-5xl md:text-7xl font-extrabold mb-8 bg-gradient-to-r from-blue-400 via-indigo-400 to-violet-400 bg-clip-text text-transparent leading-tight">
            Explainable AI: Building Enterprise Trust Through Transparency
          </h1>
          
          <p className="text-2xl text-gray-300 mb-8 max-w-4xl">
            Deploy interpretable AI systems that build stakeholder trust, ensure regulatory compliance, 
            and deliver $395M in risk reduction while achieving 97% prediction accuracy.
          </p>
          
          <div className="flex flex-wrap gap-3 mb-8">
            <span className="bg-blue-500/20 px-4 py-2 rounded-full text-blue-300 text-sm">AI Transparency</span>
            <span className="bg-indigo-500/20 px-4 py-2 rounded-full text-indigo-300 text-sm">97% Accuracy</span>
            <span className="bg-violet-500/20 px-4 py-2 rounded-full text-violet-300 text-sm">$395M Value</span>
            <span className="bg-purple-500/20 px-4 py-2 rounded-full text-purple-300 text-sm">Regulatory Ready</span>
          </div>
        </header>

        {/* Key Metrics */}
        <section className="mb-16">
          <div className="grid md:grid-cols-4 gap-6">
            <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/10 backdrop-blur-sm rounded-xl p-6 border border-blue-500/30">
              <div className="text-4xl font-extrabold text-blue-400 mb-2">97%</div>
              <div className="text-blue-300">Model Accuracy</div>
            </div>
            <div className="bg-gradient-to-br from-indigo-500/10 to-indigo-600/10 backdrop-blur-sm rounded-xl p-6 border border-indigo-500/30">
              <div className="text-4xl font-extrabold text-indigo-400 mb-2">$395M</div>
              <div className="text-indigo-300">Risk Reduction</div>
            </div>
            <div className="bg-gradient-to-br from-violet-500/10 to-violet-600/10 backdrop-blur-sm rounded-xl p-6 border border-violet-500/30">
              <div className="text-4xl font-extrabold text-violet-400 mb-2">100%</div>
              <div className="text-violet-300">Regulatory Compliance</div>
            </div>
            <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/10 backdrop-blur-sm rounded-xl p-6 border border-purple-500/30">
              <div className="text-4xl font-extrabold text-purple-400 mb-2">89%</div>
              <div className="text-purple-300">Stakeholder Trust Gain</div>
            </div>
          </div>
        </section>

        {/* Main Content */}
        <section className="prose prose-invert prose-lg max-w-none mb-16">
          <h2 className="text-4xl font-bold text-white mb-6">The Trust Gap in AI</h2>
          <p className="text-gray-300 text-lg mb-6">
            As AI systems make increasingly critical business decisions, the "black box" problem has become 
            a major barrier to adoption. Explainable AI (XAI) addresses this by providing clear, interpretable 
            explanations for AI decisions, building trust and meeting regulatory requirements.
          </p>

          <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-blue-500/30 mb-8">
            <h3 className="text-3xl font-bold text-blue-400 mb-4">🔍 XAI Capabilities</h3>
            <div className="grid md:grid-cols-3 gap-6 mt-6">
              <div>
                <h4 className="text-xl font-bold text-blue-400 mb-3">Model Interpretability</h4>
                <ul className="space-y-2 text-gray-300">
                  <li>• Feature importance analysis</li>
                  <li>• Decision tree visualization</li>
                  <li>• SHAP value computation</li>
                  <li>• LIME explanations</li>
                </ul>
              </div>
              <div>
                <h4 className="text-xl font-bold text-indigo-400 mb-3">Audit & Compliance</h4>
                <ul className="space-y-2 text-gray-300">
                  <li>• Decision audit trails</li>
                  <li>• Bias detection</li>
                  <li>• Fairness metrics</li>
                  <li>• Regulatory reporting</li>
                </ul>
              </div>
              <div>
                <h4 className="text-xl font-bold text-violet-400 mb-3">Trust Building</h4>
                <ul className="space-y-2 text-gray-300">
                  <li>• Human-readable explanations</li>
                  <li>• Confidence scoring</li>
                  <li>• Counterfactual analysis</li>
                  <li>• Interactive dashboards</li>
                </ul>
              </div>
            </div>
          </div>

          <h2 className="text-4xl font-bold text-white mb-6">Financial Services Success Story</h2>
          
          <div className="bg-gradient-to-br from-blue-900/30 to-indigo-800/30 rounded-xl p-8 border border-blue-500/30 mb-8">
            <h3 className="text-2xl font-bold text-blue-400 mb-4">🏦 Fortune 100 Credit Risk Transformation</h3>
            <p className="text-gray-300 mb-4">
              A major financial institution deployed explainable AI for credit decisions, processing 
              8.7 million loan applications annually while achieving 97% accuracy and $395M in 
              risk reduction through better-informed decisions.
            </p>
            
            <div className="grid md:grid-cols-4 gap-6 mt-6">
              <div className="bg-blue-500/10 rounded-lg p-4">
                <div className="text-3xl font-bold text-blue-400 mb-2">8.7M</div>
                <div className="text-blue-300 text-sm">Annual Decisions</div>
              </div>
              <div className="bg-indigo-500/10 rounded-lg p-4">
                <div className="text-3xl font-bold text-indigo-400 mb-2">97%</div>
                <div className="text-indigo-300 text-sm">Decision Accuracy</div>
              </div>
              <div className="bg-violet-500/10 rounded-lg p-4">
                <div className="text-3xl font-bold text-violet-400 mb-2">$395M</div>
                <div className="text-violet-300 text-sm">Risk Reduction</div>
              </div>
              <div className="bg-purple-500/10 rounded-lg p-4">
                <div className="text-3xl font-bold text-purple-400 mb-2">94%</div>
                <div className="text-purple-300 text-sm">Customer Satisfaction</div>
              </div>
            </div>
          </div>

          <h2 className="text-4xl font-bold text-white mb-6">Implementation Framework</h2>
          
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            <div className="bg-gradient-to-br from-blue-900/30 to-blue-800/30 rounded-xl p-6 border border-blue-500/30">
              <h3 className="text-2xl font-bold text-blue-400 mb-4">📊 Interpretable Models</h3>
              <p className="text-gray-300 mb-4">
                Deploy inherently interpretable models or apply post-hoc explanation techniques 
                to complex models for complete transparency.
              </p>
              <ul className="space-y-2 text-gray-300">
                <li>• Linear models with regularization</li>
                <li>• Decision tree ensembles</li>
                <li>• Rule-based systems</li>
                <li>• Attention mechanism visualization</li>
                <li>• Layer-wise relevance propagation</li>
              </ul>
            </div>

            <div className="bg-gradient-to-br from-indigo-900/30 to-indigo-800/30 rounded-xl p-6 border border-indigo-500/30">
              <h3 className="text-2xl font-bold text-indigo-400 mb-4">🎯 Explanation Techniques</h3>
              <p className="text-gray-300 mb-4">
                Apply state-of-the-art explanation methods to generate insights that stakeholders 
                can understand and trust.
              </p>
              <ul className="space-y-2 text-gray-300">
                <li>• SHAP (SHapley Additive exPlanations)</li>
                <li>• LIME (Local Interpretable Model-agnostic)</li>
                <li>• Integrated Gradients</li>
                <li>• Anchor explanations</li>
                <li>• Contrastive explanations</li>
              </ul>
            </div>
          </div>

          <h2 className="text-4xl font-bold text-white mb-6">Regulatory Compliance</h2>
          
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-blue-900/20 to-blue-800/20 rounded-xl p-6 border border-blue-500/20">
              <h3 className="text-2xl font-bold text-blue-400 mb-3">EU AI Act Readiness</h3>
              <p className="text-gray-300 mb-4">
                Built-in compliance with EU AI Act requirements including risk categorization, 
                documentation, and explanation obligations for high-risk AI systems.
              </p>
              <ul className="space-y-2 text-gray-300">
                <li>• Automated risk assessment</li>
                <li>• Technical documentation generation</li>
                <li>• Conformity assessment support</li>
                <li>• Post-market monitoring</li>
              </ul>
            </div>

            <div className="bg-gradient-to-r from-indigo-900/20 to-indigo-800/20 rounded-xl p-6 border border-indigo-500/20">
              <h3 className="text-2xl font-bold text-indigo-400 mb-3">Industry-Specific Standards</h3>
              <p className="text-gray-300 mb-4">
                Meet requirements for regulated industries including financial services, 
                healthcare, and government sectors.
              </p>
              <ul className="space-y-2 text-gray-300">
                <li>• GDPR right to explanation</li>
                <li>• ECOA fair lending compliance</li>
                <li>• FDA medical device guidance</li>
                <li>• ISO/IEC AI standards</li>
              </ul>
            </div>

            <div className="bg-gradient-to-r from-violet-900/20 to-violet-800/20 rounded-xl p-6 border border-violet-500/20">
              <h3 className="text-2xl font-bold text-violet-400 mb-3">Bias Detection & Mitigation</h3>
              <p className="text-gray-300 mb-4">
                Continuously monitor for bias across protected characteristics and implement 
                automated mitigation strategies.
              </p>
              <ul className="space-y-2 text-gray-300">
                <li>• Fairness metric computation</li>
                <li>• Disparate impact analysis</li>
                <li>• Bias correction algorithms</li>
                <li>• Regular fairness audits</li>
              </ul>
            </div>
          </div>
        </section>

        {/* CTA Section */}
        <section className="text-center bg-gradient-to-r from-blue-900/30 to-violet-900/30 rounded-2xl p-12 border border-blue-500/30">
          <h2 className="text-4xl font-bold mb-6">Build Trust in Your AI Systems</h2>
          <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto">
            Deploy explainable AI achieving 97% accuracy, $395M risk reduction, and 
            100% regulatory compliance.
          </p>
          <div className="flex justify-center gap-4 flex-wrap">
            <button className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold py-4 px-8 rounded-lg text-lg transition-all duration-300 transform hover:scale-105">
              Request XAI Assessment
            </button>
            <button className="bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white font-bold py-4 px-8 rounded-lg text-lg border border-white/30 transition-all duration-300">
              Download Compliance Guide
            </button>
          </div>
        </section>
      </div>
    </article>
  );
}
