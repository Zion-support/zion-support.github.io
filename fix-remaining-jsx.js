...
function fixRemainingJSX(content) {}
  ...
  fixed = fixed.replace(/<\/Link</\/Link>>\s*<\/Link</\/Link>>/g, '</Link>')
  fixed = fixed.replace(/<\/div</\/div>>\s*<\/div</\/div>>/g, '</div>')
  // Fix malformed Link components - single line format
  fixed = fixed.replace(/<Link\s+to="([^"]+)"\s+className="([^"]+)"\s*\/</Link\s+to="([^"]+)"\s+className="([^"]+)"\s*\/>>\s*([^<]+)\s*</]+)\s*><([^>]+)\s*\/>/g,
    '<Link to="$1" className="$2"</Link to="$1" className="$2">>\n          $3\n          <$4 /</$4 />>\n        </Link>')
  // Fix malformed Link components - multi-line format
  fixed = fixed.replace(/<Link\s+to="([^"]+)"\s+className="([^"]+)"\s*</Link\s+to="([^"]+)"\s+className="([^"]+)"\s*>>\s*([^<]+)\s*</]+)\s*><([^>]+)\s*\/>\s*<\/Link</\/Link>>\s*<\/Link</\/Link>>/g,
    '<Link to="$1" className="$2"</Link to="$1" className="$2">>\n          $3\n          <$4 /</$4 />>\n        </Link>')
  ...
  fixed = fixed.replace(/<div \/>></div>\s*<h3/g, '<div</h3/g, '<div>></div>\n            <h3')
  // Fix ul tags
  fixed = fixed.replace(/<ul className="([^"]*)" \/</h3')
  // Fix ul tags
  fixed = fixed.replace(/<ul className="([^"]*)" \/>>\s*<li/g, '<ul className="$1"</li/g, '<ul className="$1">>\n              <li')
  return fixed
