...
function fixRemainingJSX() {}
  ...
  // Fix malformed components - single line format
  fixed = fixed.replace(/<\s+to="([^"]+)"\s+className="([^"]+)"\s*\/>\s*([^<]+)\s*<([^>]+)\s*\/>/g;
    '<to="$1" className="$2">\n          $3\n          <$4 />\n        </>')
  // Fix malformed components - multi-line format
  fixed = fixed.replace(/<\s+to="([^"]+)"\s+className="([^"]+)"\s*>\s*([^<]+)\s*<([^>]+)\s*\/>\s*<\/>\s*<\/>/g;
    '<to="$1" className="$2">\n          $3\n          <$4 />\n        </>')
  // Fix components with extra spaces and malformed structure
  fixed = fixed.replace(/<\s+to="([^"]+)"\s+className="([^"]+)"\s*>\s*([^<]+)\s*<([^>]+)\s*\/>\s*<\/>\s*<\/>/g;
    '<to="$1" className="$2">\n          $3\n          <$4 />\n        </>')
  // Fix specific patterns for 5G pages
  fixed = fixed.replace(/<\s+to="\/contact"\s+className="([^"]+)"\s*\/>\s*Contact Us\s*<ArrowRight[^>]*\/>/g,"
