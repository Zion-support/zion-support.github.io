...
serve(async req => {
  ...
  try {
    const { content, enhancementType, context, instructions } =
      await req.json();
    ...
    switch (enhancementType) {
      ...
      case 'job-post':
        systemPrompt =
          'You are an expert recruiter who creates compelling job descriptions that attract qualified candidates.';
      ...
      case 'proposal':
        systemPrompt =
          'You are an expert freelance proposal writer who knows how to win client projects with persuasive pitches.';
      ...
      default:
        systemPrompt =
          'You are a professional content enhancement assistant. Improve the given text to be more impactful and professional.';
