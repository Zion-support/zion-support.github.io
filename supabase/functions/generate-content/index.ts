...
serve(async req => {
  ...
  try {
    ...
    if (contentType === 'blog') {
      ...
      userPrompt =
        prompt ||
        `Generate a 700-word blog article on "${contentTopic}" written in a professional, SEO-optimized tone. 
      ...
      userPrompt =
        prompt ||
        `Create a weekly newsletter for Zion marketplace users featuring:
