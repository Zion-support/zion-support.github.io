...
function detectCurrencySymbol(input: string): {
  currency: string;
  symbol: string;
} {
...
function normalizeBudget(
  raw: QuoteRequestPayload['budgetRange']
):
  | { display: string; currency: string; min?: number; max?: number }
  | undefined {
  ...
  if (typeof raw === 'string') {
    ...
    const normalized = raw.replace(
      /to|–|—|minus|upto|up to|between|and/gi,
      '-'
    );
    ...
    if (parts.length === 1) {
      ...
      return {
        currency,
        display: `${symbol}${raw.replace(/[^0-9kmb\.,\s]/gi, '').trim()}`,
      };
    }
    if (parts.length >= 2) {
...
async function analyzeWithOpenAI(
  projectOverview: string
): Promise<{ summary?: string; projectType?: string }> {
        ...
        input: [
          {
            role: 'system',
            content:
              'You summarize software project briefs and classify the project type. Keep summaries to 2-3 sentences.',
          },
          {
            role: 'user',
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!requesterName)
      return new Response(
        JSON.stringify({ error: 'requesterName is required' }),
        { status: 400 }
      );
    if (!requesterEmail || !isValidEmail(requesterEmail))
      return new Response(
        JSON.stringify({ error: 'Valid requesterEmail is required' }),
        { status: 400 }
      );
    if (!projectOverview)
      return new Response(
        JSON.stringify({ error: 'projectOverview is required' }),
        { status: 400 }
      );
    ...
    if (adminWebhook) {
      const adminText = [
        ...
        body.talentName
          ? `Talent: ${body.talentName}${body.talentId ? ` (${body.talentId})` : ''}`
          : body.talentId
            ? `Talent ID: ${body.talentId}`
            : undefined,
