 ...
 */
export async function normalizeSkillsWithAI(
  skills: string[]
): Promise<string[]> {
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              "You are a skill normalizer for a tech job platform. Normalize the provided skills to their standard industry naming conventions (e.g., 'react js' to 'React.js', 'nodejs' to 'Node.js'). Return only a comma-separated list of the normalized skills, nothing else.",
          },
          {
            role: 'user',
    ...
    const normalizedSkills = normalizedSkillsText
      .split(',')
...
export async function findBestMatches(
  ...
  try {
    ...
    const talentProfilesText = talents
      .map((talent, index) => {
        return `
