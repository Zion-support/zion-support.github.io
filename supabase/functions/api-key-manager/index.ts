...
serve(async req => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      if (path === 'create') {
        const { name, scopes, expiresAt } =
          (await req.json()) as CreateKeyRequest;
...
async function createApiKey(
  ...
  try {
    ...
    const { data: keyData, error: keyGenError } = await supabase.rpc(
      'generate_api_key',
      { prefix }
    );
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .select(
        'id, name, key_prefix, scopes, created_at, expires_at, is_active'
      );
...
async function getUserApiKeys(userId: string) {
  try {
    const { data, error } = await supabase
      .from('api_keys')
      .select(
        'id, name, key_prefix, scopes, created_at, last_used_at, expires_at, is_active'
      )
      .eq('user_id', userId)
...
async function regenerateApiKey(userId: string, keyId: string) {
  try {
    ...
    const { data: newKeyData, error: keyGenError } = await supabase.rpc(
      'generate_api_key',
    ...
    if (keyGenError || !newKeyData) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to generate new API key' }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    if (hashError || !hashData) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to process new API key' }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    return new Response(
      JSON.stringify({
        ...
        message:
          "API key regenerated successfully. Save this key as it won't be shown again.",
      }),
      {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
...
async function revokeApiKey(userId: string, keyId: string) {
  try {
    ...
    if (error || !data || data.length === 0) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to revoke API key or key not found' }),
        {
...
async function getApiLogs(userId: string, _limit = 50, _offset = 0) {
  try {
    ...
    if (keyError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch API logs' }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
