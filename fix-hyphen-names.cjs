...
function fixFile(filePath) {
  try {
    ...
    const camelCaseName = baseName.replace(/-([a-z])/g, (g) => g[1].toUpperCase());
    
    // Fix function names with hyphens
    content = content.replace(new RegExp(`export default function ${baseName}\\(\\)`, 'g'), `export default function ${camelCaseName}()`);
    content = content.replace(new RegExp(`const ${baseName}:`, 'g'), `const ${camelCaseName}:`);
    ...
    content = content.replace(/performanceMonitoring\.test/g, 'performanceMonitoringTest');
    content = content.replace(/accessibilityEnhancer/g, 'accessibilityEnhancer');
