...
class SimplePRMerger {
  ...
  async cleanWorkingDirectory() {
    try {
      // Stash any changes
      await this.runCommand('git stash push -m "Auto-stash before PR merge"', 'Stashing changes')
      return true
    } catch (error) {
  ...
  async processBranch(branchName) {
    try {
      ...
      await this.runCommand('git pull origin main', 'Pulling latest main before merge')
      ...
      try {
        ...
        await this.runCommand('git pull origin main', 'Pulling latest main after error')
      } catch (checkoutError) {
  ...
  async getBranchesToProcess() {
    try {
      ...
      const branches = execSync('git branch -r | grep -E "(codex|fix|feature)" | grep -v "origin/main" | head -20', {
        encoding: 'utf8',
  ...
  generateReport() {
    ...
    // Save report to file
    fs.writeFileSync('simple-pr-merger-report.json', JSON.stringify(report, null, 2))
