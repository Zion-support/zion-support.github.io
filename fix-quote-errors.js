...
// Function to fix quote and syntax errors
function fixQuoteErrors(content, filePath) {
  let fixed = content;
  ...
  // Fix colons instead of equals in JSX attributes
  ...
  fixed = fixed.replace(/<link\s+rel:\s*"([^"]*?)"\s+href:\s*"([^"]*?)"\s*\/>/g, '<link rel="$1" href="$2" />');
  fixed = fixed.replace(/<link\s+rel:\s*"([^"]*?)"\s+type:\s*"([^"]*?)"\s+href:\s*"([^"]*?)"\s*\/>/g, '<link rel="$1" type="$2" href="$3" />');
...
function processFile(filePath) {
  try {
    ...
    const fixed = fixQuoteErrors(content, filePath);
...
export { fixQuoteErrors, processFile };