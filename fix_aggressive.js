...
function fixCorruptedSyntax(text) {
  ...
  fixed = fixed.replace(/const\s*\[([^,]+),\s*([^,]+)\]\s*=\s*useState\s*\(/g, 'const [$1, $2] = useState(');
  fixed = fixed.replace(/const\s*\[([^,]+),\s*([^,]+)\]\s*=\s*useState\s*\(/g, 'const [$1, $2] = useState(');
  ...
  fixed = fixed.replace(/const\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*React\.FC\s*=\s*\(/g, 'const $1: React.FC = (');
  ...
  fixed = fixed.replace(/useStateuseEffectSuspenselazyuseCallback/g, 'useState, useEffect, Suspense, lazy, useCallback');
  ...
  fixed = fixed.replace(/isMobileMenuOpensetIsMobileMenuOpen/g, 'isMobileMenuOpen, setIsMobileMenuOpen');
  fixed = fixed.replace(/showPerformanceMonitorsetShowPerformanceMonitor/g, 'showPerformanceMonitor, setShowPerformanceMonitor');
  fixed = fixed.replace(/performanceMetricssetPerformanceMetrics/g, 'performanceMetrics, setPerformanceMetrics');
