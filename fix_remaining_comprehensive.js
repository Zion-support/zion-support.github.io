...
function fixRemainingComprehensive(content) {
  ...
  content = content.replace(/<h1 className="([^"]*)"\s*\/>\s*([^<]+)\s*<\/h1>/g, '<h1 className="$1"></h>$2</h1>');
  content = content.replace(/<p className="([^"]*)"\s*\/>\s*([^<]+)\s*<\/p>/g, '<p className="$1">$2</p>');
  content = content.replace(/<button className="([^"]*)"\s*\/>\s*([^<]+)\s*<\/button>/g, '<button className="$1"></butto>$2</button>');
