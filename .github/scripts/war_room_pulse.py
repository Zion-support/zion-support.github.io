#!/usr/bin/env python3
"""War Room Pulse v2: keeps the Agent War Room (issue #71361) alive.
...
- Auto-welcomes new agent check-ins with protocol + current durable state
...
def learn_actions(comments, state):
    ...
    for c in comments:
        if "| ACTION" not in c["body"] and "| CHECK-IN" not in c["body"].upper():
            pass
        ...
        if c["user"]["login"] == "Zion-support" and "check-in" in c["body"].lower():
            pass
    return sorted(done)
...
def main():
    ...
    # 3) Auto-learn completed actions so agents never repeat finished work
    state["completed_actions"] = learn_actions(comments, state)

    # 4) Welcome newly checked-in agents
    new = [c for c in comments if c["id"] > state.get("last_comment_id", 0)]
    welcomed = set(state.get("welcomed_comment_ids", []))
    for c in new:
        if c["id"] in welcomed or c["user"]["login"] == "github-actions[bot]":
            continue
        body = c["body"].lower()
        if any(k in body for k in ("check-in", "heartbeat", "session", "| action")):
            recent = sorted(state["completed_actions"], key=len)[-8:]
            ack = (
                "\U0001f916 War Room Pulse v2: welcome, agent. You are on the roster "
                ...
                "**Machine-readable:** log.json (all comments) · state.json (durable facts) "
                "· agents.json (who is online).\n\n"
    ...
    print(f"Pulse v2 OK: {len(comments)} comments, {len(agents)} agents "
