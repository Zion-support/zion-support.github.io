...
// Special template for 404 page
const notFoundTemplate = `import React from 'react';
import { Helmet } from 'react-helmet-async';
...
function rebuildFile(filePath) {
  try {
    ...
    if (isCorrupted(content)) {
      ...
      // Choose template based on file
      let newContent;
      if (fileName === '404.tsx') {
        newContent = notFoundTemplate;
      } else {
        newContent = pageTemplate.replace('Page Title', pageName.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()));
      }
