...
const getBranches = () => {
  try {
    const output = execSync(
      'git branch -r | grep "origin/cursor/fix-errors-and-merge-to-main"',
      { encoding: 'utf8' },
    );
    return output
      .trim()
...
const canMerge = branch => {
  try {
    execSync(`git merge-tree $(git merge-base HEAD ${branch}) HEAD ${branch}`, {
      stdio: 'pipe',
    });
    return true;
  } catch (error) {
...
const mergeAllBranches = async () => {
  ...
  for (const branch of branches) {
    ...
    try {
      ...
      if (canMerge(branchName)) {
        const success = mergeBranch(branchName);
        if (success) {
          successCount++;
    ...
    if ((successCount + conflictCount) % 50 === 0) {
      console.log(
        `📊 Progress: ${successCount} merged, ${conflictCount} conflicts`,
      );
    }
  }
