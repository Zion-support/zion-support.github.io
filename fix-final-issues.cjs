...
function fixPageFile(filePath) {
  try {
    ...
    // Create a valid component name (remove numbers from start)
    let componentName = displayName.replace(/\s+/g, '') + 'Page';
    ...
    const correctImportPath = '../'.repeat(depth) + 'components/ErrorBoundary';
    
    // Create the complete file content
    const newContent = `import ErrorBoundary from '${correctImportPath}';
...
console.log(`Found ${totalCount} page files to fix...`);
