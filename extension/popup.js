...
,
    ...
    if (res && res.answer) {
      outputElement.textContent = res.answer} else if (res && res.error) {
      outputElement.textContent = `Error: ${res.error}`} else {
      outputElement.textContent = No response from background script.'}
  } catch (error) {
    console.error('Failed to contact background script'),
    if (error.message === Request timeout') {
      outputElement.textContent = Request timed out. Please try again.'} else if (error.message?.includes('message channel closed') || error.message?.includes('Could not establish connection') || error.message?.includes('Receiving end does not exist') || error.message?.includes('Extension context invalidated')) {
      outputElement.textContent = Connection lost. Please try again.'} else {
