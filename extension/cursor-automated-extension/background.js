...
,
  startMonitoring() {
    // Start heartbeat,
    setInterval(() => {
      this.sendHeartbeat()}, this.config.heartbeatInterval),
    // Start sync with other computers,
    setInterval(() => {
      this.syncWithOtherComputers()}, this.config.syncInterval),
    // Start file change monitoring,
    setInterval(() => {
      this.monitorFileChanges()}, this.config.fileWatchInterval),
