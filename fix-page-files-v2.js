...
function fixFile(filePath) {
  try {
    ...
    if (orphanedContent.includes('title:') && orphanedContent.includes('description:')) {
      ...
      featuresArray = `const features = [${orphanedContent}];`;
    }
    ...
    // Replace the return; with proper JSX
    content = content.replace(/return;/, jsxReturn);
    
    // Remove the orphaned content and add the features array if it exists
    if (featuresArray) {
    ...
    if (existingImports) {
      ...
      if (currentImports) {
        ...
        const usedImports = ['Zap', 'FileText', 'Users', 'Clock', 'Shield', 'BarChart3', 'Calculator',
          'Target', 'CheckCircle', 'Globe', 'Sparkles', 'Star', 'Box', 'ArrowRight'].filter(imp => content.includes(`<${imp}`));
    ...
    // Fix any syntax errors in the features array
