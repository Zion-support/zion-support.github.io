const fs = require('fs');
const path = require('path');

// Function to fix ultimate remaining errors
function fixUltimateRemainingErrors(filePath) {
  try {
    let content = fs.readFileSync(filePath, 'utf8');
    let modified = false;

    // Fix operator.ts - remove trailing semicolon
    if (filePath.includes('operator.ts')) {
      content = content.replace(/const completion = await openai\.chat\.completions\.create\(\{;/g, 'const completion = await openai.chat.completions.create({');
      modified = true;
    }

    // Fix track.ts - fix import path
    if (filePath.includes('track.ts')) {
      content = content.replace(/import { ensureAdmin } from '\.\.\/\.\.\/\.\.\/utils\/auth';/g, 'import { ensureAdmin } from \'../../../utils/auth\';');
      modified = true;
    }

    // Fix applications.ts - fix object property syntax
    if (filePath.includes('applications.ts')) {
      content = content.replace(/id: uuidv4\(\);/g, 'id: uuidv4(),');
      modified = true;
    }

    // Fix nextauth.ts - remove catch block without try
    if (filePath.includes('nextauth.ts')) {
      content = content.replace(/} catch \(error\) \{[\s\S]*?return res\.status\(500\)\.json\(\{ error: "Internal server error" \}\);\s*}/g, '');
      modified = true;
    }

    // Fix nonce.ts - fix for loop syntax
    if (filePath.includes('nonce.ts')) {
      content = content.replace(/for \(let i = 0, i < length, i\+\+\) res \+= charset\[bytes\[i\] % charset\.length\],;/g, 'for (let i = 0; i < length; i++) res += charset[bytes[i] % charset.length];');
      modified = true;
    }

    if (modified) {
      fs.writeFileSync(filePath, content, 'utf8');
      console.log(`Fixed ultimate remaining errors in: ${filePath}`);
      return true;
    }
  } catch (error) {
    console.error(`Error processing ${filePath}:`, error.message);
  }
  return false;
}

// List of files with ultimate remaining errors
const filesToFix = [
  'pages/api/ai/operator.ts',
  'pages/api/analytics/events/track.ts',
  'pages/api/applications.ts',
  'pages/api/auth/[...nextauth].ts',
  'pages/api/auth/nonce.ts'
];

let fixedCount = 0;

console.log('Fixing ultimate remaining errors...');

filesToFix.forEach(file => {
  const filePath = path.join(process.cwd(), file);
  if (fs.existsSync(filePath)) {
    if (fixUltimateRemainingErrors(filePath)) {
      fixedCount++;
    }
  } else {
    console.log(`File not found: ${filePath}`);
  }
});

console.log(`Fixed ultimate remaining errors in ${fixedCount} files.`);