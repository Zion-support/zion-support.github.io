...
function fixJSXStructure(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === '<>') {
      tagStack.push({ type: 'fragment', line: i });
      result.push(line);
      continue;
    }
    ...
    if (openMatch && !trimmed.includes('/>') && !trimmed.includes('</')) {
    ...
    if (closeMatch) {
      ...
      const tagIndex = tagStack.findIndex(
        item => item.type === 'tag' && item.name === tagName,
      );
      if (tagIndex !== -1) {
...
function fixSpecificPatterns(content) {
  ...
  fixed = fixed.replace(
    /<div[^>]*>\s*<\/div>\s*<div[^>]*>\s*<\/div>\s*<div[^>]*>\s*<\/div>\s*<div[^>]*>\s*<\/div>\s*<div[^>]*>\s*<\/div>\s*<div[^>]*>\s*<\/div>/g,
    '',
  );
