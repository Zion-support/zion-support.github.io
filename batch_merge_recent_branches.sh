...
# Script to efficiently merge recent cursor branches
echo "Starting batch merge of recent cursor branches..."

# Get the most recent 20 branches
branches=$(git for-each-ref --sort=-committerdate refs/remotes/origin --format='%(refname:short)' | grep "cursor/check-fix-push-and-merge-to-main" | head -20)
