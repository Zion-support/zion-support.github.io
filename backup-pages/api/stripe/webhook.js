...
export default async function handler(req, res) {
  ...
  if (!sig) {
    console.warn(
      'Webhook received without stripe-signature header. This is suspicious.'
    );
    return res
      .status(400)
  ...
  try {
    ...
    return res
      .status(400)
      .send(`Webhook Error: Signature verification failed. ${err.message}`);
  }
  // Handle the event
  switch (event.type) {
    case 'checkout.session.completed':
      ...
      console.log(
        `[Stripe Webhook] User ID (from metadata): ${session.metadata?.userId}`
      );
      console.log(
        `[Stripe Webhook] Product ID (from metadata): ${session.metadata?.productId}`
      ); // Assuming you add this to metadata during session creation
      ...
      if (session.payment_status === 'paid') {
          ...
          await sendEmailWithSendGrid({
            ...
            dynamicTemplateData: {
              ...
              totalAmount: session.amount_total
                ...
                : 'N/A', // Stripe amounts are in cents
        ...
        console.log(
          `[Stripe Webhook] Checkout session ${session.id} completed but payment status is ${session.payment_status}. No email action taken.`
        );
      }
      break;
