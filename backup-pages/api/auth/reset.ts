...
const loginUserAfterReset = async (
  ...
  if (error || !data?.session) {
    ...
    return {
      success: false,
      error: error?.message || 'Invalid credentials for auto-login.',
    };
  }
  ...
  res.setHeader(
    'Set-Cookie',
    `authToken=${access_token}; HttpOnly; Path=/; Secure; SameSite=Strict; Max-Age=${maxAge}`
  );

  console.log(
    `User ${supabaseUser.id} (${email}) auto-logged in successfully after password reset.`
  );
...
export default async function handler(
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: 'UID, token, and new password are required.' });
  }
  ...
  try {
    ...
    if (!prismaUser) {
      return res
        .status(400)
        .json({ message: 'Invalid UID or token, or token has expired.' });
    }

    // Step 1: Update password in Supabase Auth using Admin client
    const { data: updateUserResponse, error: updateUserError } =
    ...
    console.log(
      `Password updated in Supabase Auth for user ${updateUserResponse?.user?.id}`
    );
    ...
    const loginResult = await loginUserAfterReset(
      res,
      prismaUser.email,
      newPassword
    );
    ...
    if (!loginResult.success) {
      console.warn(
        `Password reset for user ${prismaUser.id} was successful (Supabase & local DB updated), but auto-login failed. Error: ${loginResult.error}`
      );
    }
