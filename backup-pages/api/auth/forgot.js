...
export default async function handler(req, res) {
  ...
  if (!process.env.SENDGRID_API_KEY) {
    console.error(
      'Missing SENDGRID_API_KEY environment variable. Password reset emails may not be sent.'
    );
  }
  if (!process.env.SENDGRID_FROM_EMAIL) {
    console.error(
      'Missing SENDGRID_FROM_EMAIL environment variable. Password reset emails may not be sent.'
    );
  }
  if (!process.env.NEXT_PUBLIC_APP_URL) {
    console.warn(
      'Missing NEXT_PUBLIC_APP_URL environment variable. Defaulting to http://localhost:3000 for reset URLs.'
    );
  }
  if (req.method !== 'POST') {
