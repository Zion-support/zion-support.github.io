...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!jwt) {
      ...
      const commonSupabaseCookie = Object.keys(req.cookies).find(
        key => key.startsWith('sb-') && key.endsWith('-access-token')
      );
      const supabaseToken =
        jwt ||
        (commonSupabaseCookie ? req.cookies[commonSupabaseCookie] : undefined);
      if (!supabaseToken) {
        console.log(
          'No Supabase auth token cookie found. Checked for "supabase-auth-token" and common patterns like "sb-*-access-token".'
        );
        return res
          .status(401)
      ...
      if (authError || !user) {
        ...
        return res
          .status(401)
          .json({ message: 'Unauthorized: No active session or auth error.' });
      }
      ...
      return res
        .status(200)
        .json({ message: 'Email successfully verified.', user: updatedUser });
    } else {
      ...
      if (!user.email) {
        console.error(
          'User object from Supabase (using "supabase-auth-token") does not contain an email.'
        );
        return res
          .status(400)
          .json({ message: 'Bad Request: User email not found in token.' });
