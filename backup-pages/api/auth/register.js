    ...
    if (data.user) {
      ...
      if (emailVerificationRequired) {
        ...
        if (data.session?.access_token) {
          res.setHeader(
            'Set-Cookie',
            `authToken=${data.session.access_token}; HttpOnly; Path=/; Secure; SameSite=Strict; Max-Age=${data.session.expires_in}`
          );
        }
      ...
      console.error(
        'Unexpected state: No error but no user data after signUp.'
      );
      res
        .status(500)
