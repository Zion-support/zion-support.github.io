...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    'Supabase URL or Service Role Key is not defined for /api/trust/score/[userId] endpoint.'
  );
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        // PostgREST error code for "single row not found"
        return res
          .status(404)
          .json({ error: 'Trust score not found for this user.' });
      }
      console.error('Error fetching trust score:', error);
      return res
        .status(500)
        .json({ error: 'Internal server error while fetching trust score.' });
    }
