...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    'Supabase URL or Service Role Key is not defined for peer review endpoint.'
  );
}
...
const getAuthenticatedUserId = (req: Request): number | null => {
  ...
  console.warn(
    'MOCK_LOGGED_IN_USER_ID not set. Peer review submission will likely fail or use a dummy ID.'
  );
...
export default async function handler(req: Request, res: Response) {
  ...
  if (isNaN(Number(finalReviewerId)) || isNaN(Number(revieweeId))) {
    return res
      .status(400)
      .json({ error: 'reviewerId and revieweeId must be numbers.' });
  }

  if (Number(finalReviewerId) === Number(revieweeId)) {
    return res
      .status(400)
      .json({ error: 'Reviewer and reviewee cannot be the same user.' });
  }

  if (!['endorsement', 'flag'].includes(reviewType)) {
    return res
      .status(400)
      .json({ error: 'Invalid reviewType. Must be "endorsement" or "flag".' });
  }
  ...
  try {
    ...
    const { data: reviewerUser, error: reviewerError } = await supabase
      .from('User')
      .select('id')
      .eq('id', finalReviewerId)
      .single();
    if (reviewerError || !reviewerUser) {
      return res
        .status(404)
        .json({ error: `Reviewer with ID ${finalReviewerId} not found.` });
    }
    ...
    if (revieweeError || !revieweeUser) {
      return res
        .status(404)
        .json({ error: `Reviewee with ID ${revieweeId} not found.` });
    }
    ...
    if (reviewType === 'endorsement') {
      ...
      if (existingError) {
        console.error(
          'Error checking for existing endorsement:',
          existingError
        );
      ...
      if (existingEndorsement) {
        return res
          .status(409)
          .json({ error: 'You have already endorsed this user.' });
      }
    }
    ...
    if (error) {
      ...
      if (error.code === '23503') {
        ...
        if (error.message.includes('PeerReview_reviewerId_fkey')) {
          return res
            .status(400)
            .json({ error: 'Reviewer user does not exist.' });
        }
        if (error.message.includes('PeerReview_revieweeId_fkey')) {
          return res
            .status(400)
            .json({ error: 'Reviewee user does not exist.' });
        }
      }
      return res
        .status(500)
