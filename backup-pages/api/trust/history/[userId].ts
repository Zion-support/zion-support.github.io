...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    'Supabase URL or Service Role Key is not defined for trust history endpoint.'
  );
}
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({
        error: 'Internal server error while fetching trust score history.',
      });
    }
    ...
    if (!trustScoreHistory || trustScoreHistory.length === 0) {
      return res
        .status(404)
        .json({ message: 'No trust score history found for this user.' });
    }
