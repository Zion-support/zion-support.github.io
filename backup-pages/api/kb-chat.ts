...
export default async function handler(
  ...
  try {
    ...
    const orderMatch = lastMessageContent.match(
      /order\s*(?:status)?\s*#?(\d+)/i
    );
    ...
    if (/order status/i.test(lastMessageContent) && orderMatch) {
      ...
      try {
        ...
        console.error(
          `Error fetching internal order status for #${orderId}:`,
          fetchError
        );
    ...
    const answer =
      data.choices?.[0]?.message?.content || 'Sorry, I am not sure.';
