...
import {
  ...
  DialogFooter,
  DialogClose,
} from '@/components/ui/dialog'; // Basic modal for review
...
export default function CommunityProfilePage() {
  ...
  const [reviewType, setReviewType] = useState<'endorsement' | 'flag' | null>(
    null
  );
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      setReviewMessage({
        type: 'success',
        text: `${reviewType.charAt(0).toUpperCase() + reviewType.slice(1)} submitted successfully!`,
      });
      ...
      setReviewMessage({
        type: 'error',
        text: error.message || `An error occurred during ${reviewType}.`,
      });
    } finally {
              ...
              <CardHeader className="text-center">
                ...
                <div className="mt-1">
                  <CommunityVerifiedBadge
                    endorsementCount={mockEndorsementCount}
                  />
                </div>
                {user.role && (
              ...
              <CardContent className="space-y-6">
                ...
                {MOCK_LOGGED_IN_USER_ID !== userId && ( // Don't show buttons on own profile
                  <div className="flex flex-col space-y-2 mt-4 border-t pt-4">
                    <Button
                      onClick={() => openReviewModal('endorsement')}
                      variant="outline"
                      size="sm"
                ...
                {reviewMessage &&
                  !isReviewModalOpen && ( // Show global messages if modal is closed
                    <div
                      className={`p-2 rounded text-sm mt-2 ${reviewMessage.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                    >
                      {reviewMessage.text}
                    </div>
                  )}
