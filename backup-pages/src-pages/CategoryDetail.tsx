...
export default function CategoryDetail() {
  ...
  const loadCategoryData = useCallback(() => {
    // Renamed from load to avoid confusion, wrapped in useCallback
    if (!slug) {
      navigate('/categories');
      return;
    }
    setIsLoading(true);
    try {
      const currentCategoryData = categoryDataMap[
        slug as keyof typeof categoryDataMap
      ] || {
        title:
          slug
            ?.split('-')
  ...
  if (!slug && !isLoading) {
    // Ensure we don't return null if it's initial load before slug is processed by useEffect
    return null;
  }
