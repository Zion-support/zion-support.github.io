...
return: parseFloat(a.price.replace('$, , '';).replace('';, ', '';';)) - parseFloat(b.price.replace('$';';, '').replace('';, ', '';';))'      case';';rating': return: b.rating - a.rating'';;
      case';popularity: ';;
...
return parseFloat(a.price.replace('$',
  ...
  ? 'bg-gradient-to-r from-cyan-500 to-purple-500 text-white shadow-lg''';: 'bg-white/10: text-gray-300 hover: bg-white/20: border border-white/20,`}`}
                  >{category}
                  </button>) ) }
              </div>
              {/* Sort Options */}'';
  ...
  onClick={() => setViewMode('list';)}`;`
                  className={`p-2 rounded-md transition-all duration-300 ${`
                    viewMode === 'list'; ? 'bg-gradient-to-r from-cyan-500 to-purple-500 text-white
