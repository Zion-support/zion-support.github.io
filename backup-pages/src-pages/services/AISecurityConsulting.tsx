...
"
      <section className="py-16 max-w-6xl mx-auto px-6">"
        <div className="grid md:grid-cols-2 gap-10">
          <div>"
            <h2 className="text-3xl font-bold text-gray-900 mb-4">Advanced Security Consulting Features</h2>"
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-6">
          {['50-80% performance improvement',40-70% delivery time reduction',35-65% cost reduction'].map((t) => ("
            <div key={t} className="p-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow">"
              <Brain className="w-6 h-6 text-cyan-600 mb-3"  />"              <div className="text-gray-900 font-semibold">{t}</div>
            </div>
          ))}
        </div>
"
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {useCases.map((useCase) => ("
              <div key={useCase} className="flex items-center p-4 bg-white rounded-lg border border-gray-200">"
                <Target className="w-5 h-5 text-cyan-600 mr-3"  />"                <span className="text-gray-700">{useCase}</span>
              </div>
            ))}
          </div>
        </div>
"
        <div className="mt-16 bg-cyan-50 rounded-xl p-8">"
          <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Measurable Security Consulting Impact</h3>"
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefits.map((benefit) => ("
              <div key={benefit} className="flex items-start p-4 bg-white rounded-lg border border-cyan-200">"
                <Zap className="w-5 h-5 text-cyan-600 mr-3 mt-0.5 flex-shrink-0"  />"                <span className="text-gray-700">{benefit}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {industries.map((industry) => ("
              <div key={industry} className="flex items-center p-4 bg-gray-50 rounded-lg border border-gray-200">"
                <FileText className="w-5 h-5 text-cyan-600 mr-3"  />"                <span className="text-gray-700">{industry}</span>
              </div>
            ))}
          </div>
        </div>
"
        <div className="mt-14 text-center">"
          <div className="text-sm text-gray-600 mb-2">Ready to transform your security consulting with AI?</div>"
