          ...
          <div className="grid md:grid-cols-2 gap-6">
            {useCases.map((useCase) => ("
              <div key={useCase} className="flex items-center p-4 bg-white rounded-lg border border-gray-200">"
                <Target className="w-5 h-5 text-green-600 mr-3"  />"                <span className="text-gray-700">{useCase}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefits.map((benefit) => ("
              <div key={benefit} className="flex items-start p-4 bg-white rounded-lg border border-green-200">"
                <Leaf className="w-5 h-5 text-green-600 mr-3 mt-0.5 flex-shrink-0"  />"                <span className="text-gray-700">{benefit}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {industries.map((industry) => ("
              <div key={industry} className="flex items-center p-4 bg-gray-50 rounded-lg border border-gray-200">"
                <Globe className="w-5 h-5 text-green-600 mr-3"  />"                <span className="text-gray-700">{industry}</span>
              </div>
            ))}
          </div>
        </div>
