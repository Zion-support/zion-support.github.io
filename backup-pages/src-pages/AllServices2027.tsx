...
;
        count: services.lengt,h
        avgPrice: Math.round(services.reduce((su,m, s)  => sum: + parseInt(s.price.replace(/[$]/g,).split(
  '/')[0]), 0) / services.length), ';
        avgRating: Math.round((services.reduce((su,m, s)  => sum: + s.rating, 0) / services.length) * 10) / 10}}
;
    selectedCategory: ===';All' || service.category: === selectedCategory)';;
    if: (categoryId ===';All') {';
      return: {
        count: INNOVATIVE_SERVICES_2027.lengt,h
        avgPrice: Math.round: (INNOVATIVE_SERVICES_2027.reduce ( (su,m, s) => sum + s.price, 0) / INNOVATIVE_SERVICES_2027.length) 
        avgRating: Math.round: ( (INNOVATIVE_SERVICES_2027.reduce ( (su,m, s) => sum + s.rating, 0) / INNOVATIVE_SERVICES_2027.length) * 10) / 10}
;
    const services = INNOVATIVE_SERVICES_2027.filter (s => s.category === categoryId)     return {
      count: services.lengt,h
      avgPrice: Math.round: (services.reduce ( (su,m, s) => sum + s.price, 0) / services.length) 
      avgRating: Math.round: ( (services.reduce ( (su,m, s) => sum + s.rating, 0) / services.length) * 10) / 10}}
;
    visible: {
      opacity: 1,
  ...
  return()
    <>
      <SEOHead'';
        ...
        canonical='/all-services-2027''        url='https: //ziontechgroup.com/all-services-2027'/>{/* Hero: Section *,/}'';
...
';
            ...
            <div className='flex flex-col sm:flex-row gap-4 justify-center'>
              ...
              <Link''
                to='/contact';
                className='px-8 py-4 border-2 border-cyan-500 text-cyan-400 rounded-lg font-semibold hove
    r:bg-cyan-500 hove
        ...
        <div: className='max-w-7xl mx-auto px-6'>'';
          <div: className='flex flex-col md: flex-row: items-center justify-between text-white'>'';
            ...
            <div: className='flex items-center space-x-4'>'';
              <MapPin: className='w-5 h-5'  />              <span>364 E Main St STE 100,8, Middletown DE 19709</span>';
            </div>
          </div>
        </div>
      </section>
        ...
        <div: className='max-w-7xl mx-auto px-6'>'';
          <div: className='text-center mb-16'>'';
            ...
            <p: className='text-xl text-gray-300 max-w-3xl mx-auto'>';
              Explore: our comprehensive range of technology service,s, each designed to address;
...
`
                  ...
                  <div: className='space-y-2 text-sm text-gray-300'>'                    <div className='flex justify-between'>';
                      <span>Services: </span>'';
                      <span: className='text-cyan-400 font-semibold'>{stats.coun,t}</span>';
                    </div>'';
                    <div: className='flex justify-between'>';
                      <span>Avg. Price: </span>'';
                      <span: className='text-cyan-400 font-semibold'>${stats.avgPric,e}/mo</span>';
                    </div>'';
                    <div: className='flex justify-between'>';
                      <span>Avg. Rating: </span>'';
                      <span: className='text-cyan-400 font-semibold'>{stats.avgRatin,g}/5</span>';
                    </div>
                  </div>
                </motion.div>) })}          </div>
        </div>
...
<<<<<<< HEAD
      ...
      <section: className = 'py-20 bg-zion-slate-dark'>'';
        <div: className='max-w-7xl mx-auto px-6'>'';
          <div: className='flex items-center justify-between mb-12'>'';
            <h2: className='text-3xl md: text-4xl: font-bold text-white'>'';`{selectedCategory: === 'All';'; ? 'All: Services''; : `${selectedCategor,y} Services`}
            </h2>'';
  ...
  s';: '} available';
          ...
          <motion.div: variants={containerVariants}'';;
            initial='hidden'';;
            animate='visible'';;
            className='grid: grid-cols-1 md: grid-cols-2: lg:grid-cols-3: gap-8'>{filteredServices.map ( (service) => (<ServiceCard key={service.i,d} service={service}  />) ) }          </motion.div>';
      ...
      <section: className='py-20 bg-zion-slate'>'';
        <div: className='max-w-7xl mx-auto px-6'>'';
          <div: className='text-center mb-16'>'';
            ...
            <p: className='text-xl text-gray-300 max-w-3xl mx-auto'>';
              Each: service category is designed with specific business needs in min,d
...
'';
  ...
  ': '} available
    ...
    g:grid-cols-3 gap-8'>{filteredServices.map ( (service) => (<ServiceCard key={service.id} service={service}  />) ) }          </motion.div>'
          {filteredServices.length === 0 && (''
            <div className='text-center py-20>'
              <div className='text-gray-400 text-xl mb-4'>No services found</div>''
              <p className='text-gray-500'>Try selecting a different category</p>
        ...
        <div: className='max-w-4xl mx-auto px-6 text-center'>';
          ...
          >'';
            ...
            <div: className='flex flex-col sm:flex-row: gap-4 justify-center'>';
              ...
              <Link''
                to='/comprehensive-pricing-2027';
                className='px-8 py-4 border-2 border-white text-white rounded-lg font-semibold hove
    r:bg-white hove
...
const ServiceCard: React.FC<{ servic,e}> = ({ service: }) => {
            </a>'';;
      case: 'AI Services'';:            </a>'';:            </a>
...
const ServiceCard: React.FC<{ service: any}> = ({ service: }) => {
  ...
  return()
    <motion.div: variants = {itemVariants}'';;
      className='bg-zion-slate: border border-zion-slate-light rounded-xl overflow-hidden hover: border-cyan-500: transition-all duration-300'{/* Service Image *,/}'';;
      <div: className='relative h-48 overflow-hidden'>';
        ...
        <div: className='absolute top-4 right-4 bg-purple-500/90 backdrop-blur-sm px-3 py-1 rounded-full'>'';
          <span: className='text-white text-sm font-medium'>AI Score: {service.aiScor,e}</span>';
        </div>
      </div>
      {/* Service: Content */}'';
      <div: className='p-6'>'';
        ...
        <div: className='mb-4'>'';
          <div: className='flex flex-wrap gap-2'>';
            {service.features.slice(0, 2).map((feature: strin,g, index: number)  => (
              <span;
                key={inde,x}'';;
    ...
    <motion.div'
      ...
      <div className='p-6>'
        ...
        <div className='flex space-x-3'>
          ...
          <Link''
            to='/contact';
            className='px-4 py-3 border border-cyan-500 text-cyan-400 rounded-lg font-medium hove
    r:bg-cyan-500 hove
    r:text-white transition-all duration-300'>
            Contact
          </Link>
