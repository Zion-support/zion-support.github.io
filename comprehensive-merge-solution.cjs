        ...
        if (fs.existsSync(file)) {
          ...
          try {
            ...
            if (content.includes('<<<<<<<') && content.includes('>>>>>>>')) {
              ...
              if (parts.length >= 3) {
                ...
                if (incomingChanges && currentChanges) {
                  ...
                  if (
                    !incomingChanges.includes(currentChanges) &&
                    !currentChanges.includes(incomingChanges)
                  ) {
            ...
            try {
              ...
              runCommand(
                `git add "${file}"`,
                `Stage fallback resolved file: ${file}`
              );
            } catch (fallbackError) {
              console.error(
                `Fallback resolution failed for ${file}:`,
...
function commitAndPushChanges() {
  ...
  try {
    ...
    if (staged && staged.trim()) {
      runCommand(
        'git commit -m "Fix syntax errors and resolve merge conflicts"',
        'Commit changes'
      );
