...
function mergePR() {
  ...
  $2}
      ...
      try {
        ...
        return true} catch (commit_error) {
...
function main() {
  ...
  for (const branch of pr_branches) {
    try {
      ...
      // Check if branch exists,
      try {
        exec_sync (`git show - ref --verify --quiet refs / remotes / origin/${branch}`, { stdio: 'pipe' })} catch {
        console.log (`⚠️  Branch ${branch} not found, skipping...`),
        continue}
      // Attempt to merge,
      const success = mergePR(branch),
      if (success) {
        console && // console.log(` Successfully processed ${branch}`)} else {
        console.log (`❌ Failed to process ${branch}`),
        // Abort the merge if it failed,
        try {
          exec_sync ('git merge --abort', { stdio: 'pipe' })} catch (abort_error) {
          // Ignore abort errors}
      }
    } catch (error) {
