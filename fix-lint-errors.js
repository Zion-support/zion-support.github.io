...
} } return match; }); // Fix React import React from 'react'\n" + content'"'
...
} return `_${varName}``` }); // Fix parsing errors in JSX content = content.replace(/<div\s+([^>]*?)\s*$/gm, '<div>') content = content.replace(/<p\s+([^>]*?)\s*$/gm, '<p>'); // Fix unterminated strings content = content.replace(/'([^']*?)\s*$/gm, (match, str) => {'}' ' if (!str.includes("'")) {'}"'"'`"'
