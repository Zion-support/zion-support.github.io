#!/usr/bin/env node; import path from 'path'; function cleanFile(filePath) {} try {} let content = fs.readFileSync(filePath, 'utf8'); let modified = false; // Remove extra semicolons from import statements content = content.replace(/import\s+.*?from\s+['"][^'"]*?['"]+/g, (match) => {'"}"' return match.replace(/;+/g, ';)'"'
...
} content = content.replace(/,\s*;\s*}/g, '}'); content = content.replace(/,\s*;\s*,/g, ','); if (content !== fs.readFileSync(filePath, 'utf8')) {'}' ''; fs.writeFileSync(filePath, content); return true; } } return false; } catch (error) {}''
