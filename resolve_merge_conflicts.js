...
for (const file of conflictedFiles) {
  try {
    ...
    if (!content.includes("      console.log(`✅ No merge conflicts found in: ${file}`);"
      continue}
    // Strategy: Use the version from main (the newer version) for most conflicts
    // This means we"ll take the content after     let resolvedContent = content;"
    // Remove merge conflict markers and keep the main branch version
    resolvedContent = resolvedContent.replace(
      /      (match, headContent, mainContent) => {
        // Use the main branch content (after         return mainContent}
    );
    // Handle any remaining conflict markers
    resolvedContent = resolvedContent.replace(/
    // Clean up any remaining conflict markers
    resolvedContent = resolvedContent.replace(/    resolvedContent = resolvedContent.replace(/    resolvedContent = resolvedContent.replace(/
    // Write the resolved content
    fs.writeFileSync(file, resolvedContent);
    // Add the file to staging
    execSync(`git add "${file}"`, { stdio: "pipe"});"
    resolvedCount++;
    console.log(`✅ Resolved conflicts in: ${file}`)} catch (error) {
