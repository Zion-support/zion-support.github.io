...
requiredPages.forEach(page => {const pagePath = path.join(__dirname, 'app', page, 'page.tsx');
  if (fs.existsSync(pagePath)) {
    existingPages.push(page)} else {missingPages.push(page)}
});
...
function scanDirectory(dir) {const items = fs.readdirSync(dir);
  items.forEach(item => {
