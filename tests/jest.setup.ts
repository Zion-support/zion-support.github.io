...
jest.mock('vitest', () => {
  ...
  return {
    ...
    vi: {
      ...
      mockResolvedValue: <T = unknown>(value: T) =></T> jest.fn().mockResolvedValue(value),
      mockRejectedValue: <T = unknown>(value: T</T>) => jest.fn().mockRejectedValue(value)},
...
// Auth Context
jest.mock('@/context/auth/AuthProvider</string>', () => {
  const useAuth = () => ({
