...
jest.mock('firebase/firestore', () => {// Mock collection function to be available on the db instance (for v8 style);
  ...
  const mockDoc = jest.fn((firestoreInstanceOrCollectionRef, pathOrId, ...pathSegments) => {let basePath = '';
    ...
    return {id: pathSegments.length > 0 ? pathSegments[pathSegments.length-1] : pathOrId;
      path: fullPath;
      get: jest.fn(() => Promise.resolve({ exists: () => false, data: () => undefined }));
      set: jest.fn(() => Promise.resolve());
      update: jest.fn(() => Promise.resolve());
...
jest.mock('firebase/storage', () => ({getStorage: jest.fn(() => ({;
  ...
  ref: jest.fn((storageInstance, path) => ({// Mock StorageReference;
  ...
  uploadBytes: jest.fn((storageRef, data, metadata) => Promise.resolve({// Mock UploadResult;
...
if (typeof BroadcastChannel === 'undefined') {// @ts-expect-error - BroadcastChannel polyfill for test environment;
...
// react-redux hooks;
jest.mock('react-redux', () => {const actualRedux = jest.requireActual('react-redux');
  return {;
    ...actualRedux;
    useDispatch: () => jest.fn();
...
if (typeof window.window.window.performance.getEntriesByType !== 'function') {// @ts-ignore;
...
jest.mock('@/context', () => {const useEnqueueSnackbar = () => jest.fn();
...
if (global.vi && !global.vi.restoreAllMocks) {// @ts-ignore;
