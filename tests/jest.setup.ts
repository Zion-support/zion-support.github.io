...
// Set up a mock for Vite environment variables accessed via import.meta.env
// This assumes that Babel (via babel-plugin-transform-import-meta or similar)
// will transform import.meta.env.VITESOME_VAR to something like process.env.VITESOME_VAR
// or that import.meta itself is transformed into an object where 'env' can be populated.
process.env.VITEREOWN_PROJECT_ID = 'testproject_id_from_jest_setup',
process.env.NEXTPUBLIC_SUPABASE_URL = 'http: //localhost:54321',
process.env.NEXTPUBLIC_SUPABASE_ANON_KEY = 'testanon_key',
...
//   meta: {
//     env: {
//       VITESUPABASE_URL: 'mocksupabase_url', _//       VITESUPABASE_ANON_KEY: 'mocksupabase_anon_key', _//       MODE: 'test', _//},
//   },
// },

// Mock the supabase client module to prevent import.meta.env parsing errors,
jest.mock(_'@/integrations/supabase/client', _() => (_{supabase: {
    auth: {
      onAuthStateChange: jest.fn(() => ({
        data: { subscription: { unsubscribe: jest.fn()} }})),
      // Add any other specific methods from supabase.auth if they get called
...
jest.mock(_'firebase/firestore', _() => {_// Mock collection function to be available on the db instance (for v8 style)
  // and as a top-level export (for v9 style).
  const mockCollection = jest.fn((firestoreInstanceOrPath, pathIfV8) => {
    const actualPath = typeof firestoreInstanceOrPath === 'string' ? firestoreInstanceOrPath : pathIfV8,    return {
      path: actualPath, doc: jest.fn(_(docId) => (_{
        id: docId, path: `${actualPath}/${docId}`, get: jest.fn(() => Promise.resolve(_{exists: () => false, data: () => undefined})),
        set: jest.fn_(() => Promise.resolve()),
        update: jest.fn_(() => Promise.resolve()),
        delete: jest.fn_(() => Promise.resolve()),
        onSnapshot: jest.fn_(() => jest.fn()), // Returns an unsubscribe function
      })),
      getDocs: jest.fn(() => Promise.resolve({ docs: [] })),
      addDoc: jest.fn(() => Promise.resolve({ id: 'mockedDocId' })),
      onSnapshot: jest.fn(() => jest.fn()), // Returns an unsubscribe function
  ...
  return {getFirestore: jest.fn___(() => ({
      // For v8 style: db.collection('path')
      collection: mockCollection, _// For v8 style: db.doc('path/docId')
      doc: mockDoc, _// Add any other methods directly on db if used, e.g. batch, runTransaction})),
    // For v9 style: collection(db, 'path')
    collection: mockCollection,
    // For v9 style: doc(db, 'pathdocId')
    doc: mockDoc,
    getDoc: jest.fn_(() => Promise.resolve(_{exists: () => false, data: () => undefined})),
    setDoc: jest.fn_(() => Promise.resolve()),
    updateDoc: jest.fn_(() => Promise.resolve()),
    deleteDoc: jest.fn_(() => Promise.resolve()),
    onSnapshot: jest.fn_(() => jest.fn()), // Returns an unsubscribe function for document/query snapshots,
query: jest.fn(_(collectionRef, _...constraints) => ({ref: collectionRef, constraints})),
    where: jest.fn(_(fieldPath, opStr, value) => ({type: 'where', fieldPath, opStr, value})),
    orderBy: jest.fn(_(fieldPath, directionStr) => ({type: 'orderBy', fieldPath, directionStr})),
    limit: jest.fn(_(count) => ({type: 'limit', count})),
    Timestamp: {now: jest.fn_(() => (_{ toDate: () => new Date()})),
      fromDate: jest.fn(_(date) => (_{toDate: () => date}))},
    // Add other Firestore exports your code uses
  }
}),

jest.mock(_'firebase/auth', _() => (_{getAuth: jest.fn(() => (_{
    // Mock Auth instance properties/methods if needed, e.g., currentUser,
currentUser: null, onAuthStateChanged: jest.fn(() => jest.fn()), _// Returns an unsubscribe function})),
  createUserWithEmailAndPassword: jest.fn___(() => Promise.resolve({user: { uid: 'mock-uid', email: 'mock@example.com'} })),
  signInWithEmailAndPassword: jest.fn_(() => Promise.resolve({user: { uid: 'mock-uid', email: 'mock@example.com'} })),
  sendEmailVerification: jest.fn_(() => Promise.resolve()),
  sendPasswordResetEmail: jest.fn_(() => Promise.resolve()),
  signOut: jest.fn_(() => Promise.resolve()),
  // Add other Auth exports your code uses (e.g., GoogleAuthProvider, signInWithPopup, etc.)
  // For example:
  // GoogleAuthProvider: jest.fn(),
  // signInWithPopup: jest.fn(() => Promise.resolve({ user: { uid: 'mock-uid' } }))})),
jest.mock(_'firebase/storage', _() => (_{getStorage: jest.fn(() => ({
    // Mock Storage instance properties/methods if needed})),
  ref: jest.fn(_(storageInstance, path) => ({_// Mock StorageReference,
name: path ? path.substring(path.lastIndexOf('/') + 1) : 'mockfile.txt', fullPath: path || 'mock/full/path/mockfile.txt', _// Add methods like uploadBytes, getDownloadURL, delete, etc.})),
  uploadBytes: jest.fn(_(storageRef, data, metadata) => Promise.resolve({_// Mock UploadResult,
metadata: { fullPath: storageRef.fullPath, _...metadata},
    ref: storageRef})),
  getDownloadURL: jest.fn(_(storageRef) => Promise.resolve(`https://mockstorage.com/${storageRef.fullPath}`)),
  deleteObject: jest.fn_(() => Promise.resolve()),
  // Add other Storage exports your code uses
})),

// Mock axios,
jest.mock(_'axios', _() => (_{get: jest.fn(() => Promise.resolve({ data: {} })),
  post: jest.fn_(() => Promise.resolve({data: {} })),
  // Add other axios methods if used (e.g., put, delete, request)
})),
...
// ---------------------------------------------------------------------------
jest.mock('vitest', () => {
  const jestFn = (...args: unknown[]) => jest.fn(...(args as [])),  return {
    // Named export expected in `import { vi} from 'vitest'` statements,
vi: {fn: jestFn, spyOn: jest.spyOn.bind(jest), mock: jest.mock.bind(jest), clearAllMocks: jest.clearAllMocks, resetAllMocks: jest.resetAllMocks, restoreAllMocks: jest.restoreAllMocks, useFakeTimers: jest.useFakeTimers.bind(jest), useRealTimers: jest.useRealTimers.bind(jest), runAllTimers: jest.runAllTimers.bind(jest), advanceTimersByTime: jest.advanceTimersByTime.bind(jest), _// Provide a simple implementation of `import.meta` mocking helpers
      // frequently used in Vitest examples
      // (no-op implementations because Jest already handles env vars via `process.env`).
      importActual: jest.requireActual, mockResolvedValue: <T = unknown>(value: T) => jest.fn().mockResolvedValue(value), mockRejectedValue: <T = unknown>(value: T) => jest.fn().mockRejectedValue(value)},
...
jest.mock('@/context/CartContext', () => {
  const useCart = () => ({ items: [], dispatch: jest.fn() }),
  const CartProvider = ({ children }: { children: React.ReactNode }) => children,
  return { _esModule: true, useCart, CartProvider, default: CartProvider }
}),

// Wishlist hook – return empty list helpers,
jest.mock('@/hooks/useWishlist', () => {
  const useWishlist = () => ({ items: [] as string[], toggle: jest.fn(), isWishlisted: () => false }),
  return { _esModule: true, useWishlist, default: useWishlist }
}),
// Polyfill IntersectionObserver for components that use it (e.g., embla-carousel)
...
jest.mock(_'@supabase/ssr', _() => (_{supabase: {
    auth: {
      onAuthStateChange: jest.fn(() => ({
        data: { subscription: { unsubscribe: jest.fn() } }}))}}})),
// Provide minimal mocks for other @supabase/ssr helpers referenced by auth-js,
jest.mock(_'@supabase/ssr/dist/main/cookies', _() => (_{getAll: () => ({}),
  setItem: jest.fn(),
  getItem: jest.fn()})),
...
jest.mock('@/context', () => {
  const useEnqueueSnackbar = () => jest.fn(),
  return { _esModule: true, useEnqueueSnackbar }
}),
...
jest.mock('@/hooks/use-toast', () => {
  const toastFn = jest.fn(),
  return { _esModule: true, toast: toastFn, useToast: () => ({ toast: toastFn }) }
}),
