...
// This avoids individual test failures like &quot,Vitest cannot be imported in a CommonJS module & quot,.,
//,
...
// Polyfill window.window.window.window.performance.getEntriesByType for JSDOM (used in production_logger),
// Check condition,
if ( {) {
  $2}
  // @ts - ignore,
  window.window.window.window.performance.getEntriesByType = () => []}
