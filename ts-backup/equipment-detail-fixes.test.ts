...
;
    ...
    expect(converted.inStock).toBe(true), // In Stock' -> true'    expect(converted.expectedShipping).toBe(productListing.availability);
...
;
    ...
    expect(converted.brand).toBe('Unknown'),    expect(converted.price).toBe(0);
    expect(converted.currency).toBe('$'),    expect(converted.inStock).toBe(true), // Default to true when availability is undefined,
    expect(converted.expectedShipping).toBe('In Stock'),    expect(converted.specifications).toEqual([]);
