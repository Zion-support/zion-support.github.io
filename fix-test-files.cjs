...
function fixTestFile(content) {
  ...
  content = content.replace(/import\s+.*?from\s+['"]([^'"]*?)['"];''/g, (match, module) => {
    return `import ${match.match(/import\s+([^'"]*?)\s+from/)[1]} from '${module}';`;
  });
  
  // Fix describe and test functions
  content = content.replace(/describe\([^)]*\)\s*\{''/g, 'describe("Test Suite", () => {');
  content = content.replace(/test\([^)]*\)\s*\{''/g, 'test("should work", () => {');
  content = content.replace(/it\([^)]*\)\s*\{''/g, 'it("should work", () => {');
  
  // Fix expect statements
  content = content.replace(/expect\([^)]*\)\.toBeInTheDocument\(\);''/g, 'expect(screen.getByText("Test")).toBeInTheDocument();');
