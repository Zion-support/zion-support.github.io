...
async function processBranch(branchName) {
  try {
    ...
    // Checkout the branch
    await runCommand(`git checkout ${branchName}`, `Checking out ${branchName}`)
    
    // Merge main into the branch
    try {
      await runCommand('git merge main --no-ff -m "Merge main into ' + branchName + '"', `Merging main into ${branchName}`)
    } catch (mergeError) {
      console.log(`⚠️  Merge conflicts in ${branchName}, attempting to resolve...`)
      // Try to resolve conflicts by keeping our changes
      try {
        await runCommand('git checkout --ours .', 'Resolving conflicts by keeping our changes')
        await runCommand('git add .', 'Adding resolved files')
        await runCommand(`git commit -m "Resolve merge conflicts in ${branchName}"`, 'Committing resolved conflicts')
      } catch (resolveError) {
    ...
    await runCommand(`git push origin ${branchName}`, `Pushing updated ${branchName}`)
    
    // Switch back to main
    await runCommand('git checkout main', 'Switching back to main')
    
    // Merge the branch into main
    await runCommand(`git merge ${branchName} --no-ff -m "Merge ${branchName} into main"`, `Merging ${branchName} into main`)
    ...
    try {
      await runCommand(`git push origin --delete ${branchName}`, `Deleting remote branch ${branchName}`)
    } catch (deleteError) {
    ...
    try {
      await runCommand('git checkout main', 'Switching back to main after error')
    } catch (checkoutError) {
