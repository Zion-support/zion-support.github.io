...
function fixFile(filePath) {
  try {
    ...
    content = content.replace(/const\s+(\w+)\s*:\s*React\.FC\s*=\s*\(\s*\)\s*=>/g, 'const $1: React.FC = () =>');
    content = content.replace(/const\s+(\w+)\s*=\s*\(\s*\)\s*=>/g, 'const $1 = () =>');
    
    // Fix specific syntax errors
    if (filePath.includes('services.ts') || filePath.includes('servicesData.ts')) {
      content = content.replace(/export\s+const\s+(\w+)\s*=\s*\(\s*\)\s*=>/g, 'export const $1 = () =>');
    }
    
    if (filePath.includes('global-error.tsx') || filePath.includes('not-found.tsx') || 
        filePath.includes('page-backup.tsx') || filePath.includes('page-optimized.tsx') ||
        filePath.includes('service-template.tsx') || filePath.includes('sitemap-page.tsx')) {
      content = content.replace(/export\s+default\s+function\s+(\w+)\s*\(\s*\)\s*{/g, 'export default function $1() {');
    }
    ...
    if (filePath.includes('types/next.d.ts')) {
      content = content.replace(/declare\s+module\s+(\w+)\s*\(\s*\)\s*{/g, 'declare module "$1" {');
    }
