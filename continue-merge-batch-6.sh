...
# Script to continue merging cursor branches from branch 501
set -e

echo "🚀 Continuing merge process for batch 6 (branches 501-600)..."
echo "📊 Starting from branch 501 (already processed 500 branches)"
...
BATCH_SIZE=100
MAX_RETRIES=3
LOG_FILE="continue-merge-batch-6-log-$(date +%Y%m%d-%H%M%S).txt"

# Initialize counters (starting from where we left off)
SUCCESSFUL_MERGES=500  # Already processed 500
FAILED_MERGES=0
CONFLICT_RESOLUTIONS=0
SKIPPED_BRANCHES=0
TOTAL_PROCESSED=500   # Already processed 500
...
log_message "🔄 Starting batch 6 branch processing..."
log_message "📊 Batch size: $BATCH_SIZE"
log_message "📊 Starting from branch: $TOTAL_PROCESSED"
log_message "---"

# Get all cursor branches (skip the first 500 we already processed)
BRANCHES=$(git branch -r | grep "origin/cursor/" | sed 's/origin\///' | sort | tail -n +501)
...
# Summary
echo ""
log_message "🎉 Batch 6 merge process completed!"
log_message "📊 Summary:"
