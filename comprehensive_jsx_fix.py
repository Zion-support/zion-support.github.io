#!/usr/bin/env python3
"""
Comprehensive JSX syntax fix for all files.
"""
...
def fix_jsx_syntax_comprehensive(content):
    """Comprehensive JSX syntax fix."""
    # Fix malformed JSX elements with const className
    ...
    content = re.sub(r'import\s+(\w+)\s+from\s+[\'"]react[\'"]\s*;', r'import \1 from \'react\';', content)
...
def process_file(file_path):
    ...
    try:
        ...
        if any(pattern in content for pattern in [
            'const className', 'title  >', 'meta name', 'ArrowRight className', 
            'Helmet />', 'Link to=', 'div className', 'className="[^"]*\\s+[^"]*"'
        ]):
            print(f"Processing: {file_path}")
            
            # Fix JSX syntax
            cleaned_content = fix_jsx_syntax_comprehensive(content)
