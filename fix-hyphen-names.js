...
function fixHyphenNames(filePath) {
  try {
    ...
    let fixedContent = content.replace(/const [^:]+: React.FC = \(\) => \{/, `const ${componentName}: React.FC = () => {`);
    fixedContent = fixedContent.replace(/export default [^;]+/, `export default ${componentName}`);
