...
const CartContext = createContext(null)export function useCart() ;
  const ctx = useContext(CartContext)if (!ctx) {throw new Error('useCart must be used within a CartProvider')}
   ;
  return ctx;
}

export function CartProvider() {const { user } = useAuth()const [state, dispatch] = useReducer(cartReducer, initialState)const cartKey = getCartKey(user?.id;
  useEffect(() => {
let items = [];}
}
const stored = safeStorage.getItem(cartKey)if (stored) {try {items = JSON.parse(stored)} catch {items = [];}
            }
        }// Merge guest cart when user logs in;
        if (user?.id) {const guestStored = safeStorage.getItem(getCartKey())if (guestStored) {try ;}
