...
export function TalentCard() {const [avatarError, setAvatarError]  = useState(false);
  const handleViewProfile = (if (onViewProfile) {onViewProfile(talent.id)isAuthenticated: boolean;
) => {
  return $3;}
}
}
...
const handleRequestHire = (e.preventDefault()e.stopPropagation()if (onRequestHire) {onRequestHire(talent) ) => {
  return $3;}
}
...
export function TalentCard(): any ({talent,onViewProfile,onRequestHire,isSaved,onToggleSave,isAuthenticated}: TalentCardProps) {const [avatarError, setAvatarError]  = useState(false);}
  const handleViewProfile = (if (onViewProfile) {onViewProfile(talent && talent.id)) => {
  return $3;}
}      onViewProfile(talent && talent.id)onToggleSave(talent.id, !isSaved)onToggleSave(talent.id, !isSaved)}
  }

const handleRequestHire = (e && e.preventDefault()e && e.stopPropagation(),if (onRequestHire) {onRequestHire(talent)) => {
  return $3;}
}
      onRequestHire(talent)if (onRequestHire) {onRequestHire(talent)}
 ;
...
const handleToggleSave = (e && e.preventDefault()e && e.stopPropagation(),if (onToggleSave) {onToggleSave(talent && talent.id, !isSaved)if (onToggleSave) {) => {
  return $3;}
}
  }
;
