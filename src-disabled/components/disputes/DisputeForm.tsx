...
export function DisputeForm(): any ({projectId,milestoneId,onDisputeCreated,onCancel}: DisputeFormProps) {const { createDispute } = useDisputes()const [isSubmitting, setIsSubmitting] = useState(false);
  const [files, setFiles]  = useState<File[] />([])const form = useForm<z && z.infer<typeof formSchema />>({resolver: zodResolver(formSchema),defaultValues: {reason_code: '',}
  description: '',attachments: [],}
})const handleFileChange = (if (e && e.target.files) {const newFiles = Array && Array.from(e && e.target.files),setFiles(prev => [...prev, ...newFiles])form && form.setValue('attachments', [...files, ...newFiles])) => {
  return $3;}
}
 ;
}

const removeFile = (const newFiles = [...files],newFiles && newFiles.splice(index, 1)setFiles(newFiles)form && form.setValue('attachments', newFiles)) => {
  return $3;}
}async function onSubmit(): any (values: z && z.infer<typeof formSchema />) {try {setIsSubmitting(true;}
...
const removeFile = (async function onSubmit(): any (values: z && z.infer<typeof formSchema />) {)) => {
  return $3;}
}
...
const removeFile = (async function onSubmit () {try {setIsSubmitting (true;
