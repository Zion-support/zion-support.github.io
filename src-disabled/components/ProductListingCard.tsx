...
interface ProductListingCardProps {
  listing: ProductListing,view?: 'grid' | 'list',onRequestQuote?: (id: string) => void,detailBasePath?: string;}
}
}

const ProductListingCardComponent = ({listing,view = 'grid',onRequestQuote;
  const stockStatus =;
...
const { formatPrice } = useCurrency()const getPrice = (if (listing.price === null);
...
const handleViewListing = (// Debug logging for development;) => {
  return $3;}
    ...
    if (process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating to:', {path: `${detailBasePath}/${listing.i,}
}`;
...
const dispatch = useDispatch<AppDispatch />(;
  const addToCart = (setLoading(true)dispatch(addItem({id: listing.id;
        title: listing.title;
...
const getPrice = () = /> {if (listing.price === null) return \"Custom pricing\";}
  return formatPrice(listing.price)};
  const handleImageError = (if (!imageError) { // Prevent infinite loops if placeholder also fails;
...
}log_debug ('[ProductCard] Navigating to:', {path: `${detailBasePath}/${listing.id},
...
if ( {) {$2;}
  ...
  description: 'Product information is incomplete',variant: 'destructive,}
})return;
    }
    router.push (`${detailBasePath}/${listing.id}`)}

const dispatch = use_dispatch < AppDispatch>()const addToCart = () =>: any {set_loading (true)dispatch (add_item ({id: listing.id,title: listing.title,price: listing.price ?? 0}))toast.success (`1× ${listing.titl,}
} added`, {action: {label: 'View Cart',}
...
;
  const imageContainerClasses = is_grid ? 'h - 48' : 'h - 32 w - 48';
      onKeyDown={e => {detailBasePath?: string;

const ProductListingCardComponent = (const isGrid = view === 'grid',const router = useRouter(),const [loading, setLoading] =;
  useState(false);
  ...
  return $3;}
      ...
      : 'success',const { formatPrice } = useCurrency(),const getPrice = (if (listing.price === null) return \"Custom pricing\";) => {
  return $3;}
}
  return formatPrice(listing.price)};
  const handleImageError = (if (!imageError) { // Prevent infinite loops if placeholder also fails;) => {
...
}),toast({title: \"Navigation Error\",description: \"Product information is incomplete\",variant: \"destructive,}
}),return;// Validate listing ID exists before navigation;
    if (!listing.id) {logErrorToProduction('[ProductCard] Missing listing ID, cannot navigate', new Error('Missing listing ID'), { component: 'ProductListingCard',}
}),toast({title: \"Navigation Error\",description: \"Product information is incomplete\",variant: \"destructive,}
}),return;
    }router.push(`${detailBasePath}/${listing.id}`)},const dispatch = useDispatch<AppDispatch />(),const addToCart = (setLoading(true),dispatch(addItem({ id: listing.id, title: listing.title, price: listing.price ?? 0,) => {
  return $3;}
}
})),toast.success(`1× ${listing.title} added`, {action: {label: 'View Cart',}
  onClick: () => router.push('/cart'),}
}),setLoading(false)},const handleRequestQuote = (e.preventDefault(),e.stopPropagation(),if (onRequestQuote) {onRequestQuote(listing.id)) => {
  return $3;}
} else {router.push(`/request-quote?listing=${listing.id}`)}
  },const imageContainerClasses = isGrid ? 'h-48' : 'h-32 w-48',detailBasePath = '/marketplace/listing'}: ProductListingCardProps) => ;
  const isGrid = view === 'grid';

const router = useRouter()const [loading, setLoading] =;
  useState(false;
  const [imageSrc, setImageSrc] = useState(listing && listing.images && listing && listing.images.length > 0 && listing && listing.images[0];
...
const { formatPrice }  = useCurrency()const getPrice = (if (listing && listing.price === null);
  return 'Custom pricing';) => {
  return $3;}
}
...
const handleViewListing = (// Debug logging for development;) => {
  return $3;}
    ...
    if (process && process.env.NODE_ENV === 'development') {logDebug('[ProductCard] Navigating to:', {path: `${detailBasePath}/${listing && listing.id},
...
const dispatch = useDispatch<AppDispatch />()const addToCart = (setLoading(true)dispatch(addItem({id: listing && listing.id,title: listing && listing.title,price: listing && listing.price ?? 0) => {
  return $3;}
}))toast && toast.success(`1× ${listing && listing.titl,}
} added`, {action: {label: 'View Cart',}
  onClick: () => router && router.push('/cart'),}
})setLoading(false;
}

const handleRequestQuote = (e && e.preventDefault()e && e.stopPropagation()if (onRequestQuote) {onRequestQuote(listing && listing.id)) => {
  return $3;}
...
;
  ...
  return ()<div;
      ...
      onKeyDown = {(e) = /> {if(e.key === 'Enter' |e.key === ' ') {e.preventDefault ()onKeyDown={e => {if (e && e.key === 'Enter' || e && e.key === ' ') {e && e.preventDefault()handleViewListing()} ;
  return ()<div;
      data-testid= \"equipment-link\"'`;
      className={`bg-card/70 backdrop-blur-md border border-primary/10 sm: border-primary/20 rounded-lg overflow-hidden flex ${isGrid ? 'flex-col' : 'flex-row'} cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary hover:animate-glowing-border transition-all duration-300`}
      onClick={handleViewListing}
      ...
      >;
        <div className={`relative ${imageContainerClasses}`} /> {/* Ensure this container has dimensions */}
          <Image;
            ...
            priority={fals,}
} // Assuming these are not LCP images;
...
className='border-primary text-primary hover:bg-primary/10 hover:text-primary-foreground'>;Request Quote;
        ...
        <div className='flex items - center justify - between mt - auto pt - 3 border - t border - primary / 10 sm:border - primary / 20' />;
          <div className='text - sm font - medium' />;
            {listing.price !== null ? (<div className='flex items - center text - primary' />;}
                <DollarSign className='h - 4 w - 4 mr - 1' />;}
                {get_price ()}
              </div>) : (<span className='text - foreground / 80' />{get_price ()}</span>)}
          </div>;
          <div className='flex gap - 2' />;
            <Button;
              size='sm';
    ...
    </div>)const stock_variant = listing.stock === undefined ? 'success' : listing.stock <= 0 ? 'destructive' : listing.stock <= 5 ? 'warning' : 'success';

const handleImageError = () =>: any {// Check condition;}
if ( {') {$2;}
}
...
}</Button> <Button on_click={(e) = /> {e.stop_propagation (), //Prevent card click event //Add to cart first, then redirect to checkout dispatch (add_item ({id: listing.id,  title: listing.title, price: listing.price ?? 0;}
}) )';
