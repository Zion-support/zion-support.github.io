...
<<<<<<< HEAD:src-disabled/components/PerformanceOptimized.tsx
=======
...
>>>>>>> origin/cursor/fix-website-loading-errors-and-merge-8339:src/components/PerformanceOptimized.tsx
// Debounced search hook
export const useDebouncedSearch = (value: string, delay: number = 300) = > {const [debouncedValue, setDebouncedValue] = React.useState(value); React.useEffect(() = > {const handler = setTimeout(() = > {; setDebouncedValue(value)}, delay); return () = > {clearTimeout(handler)}}, [value, delay]); return debouncedValue}
// Performance metrics collection
<<<<<<< HEAD:src-disabled/components/PerformanceOptimized.tsx
...
; let OptimizedComponent = Component;
; if (useMemo) {; OptimizedComponent = memo(Component, (prevProps, nextProps) = > {; if (memoDeps) {; const prevDeps = memoDeps(prevProps); const nextDeps = memoDeps(nextProps); return prevDeps.every((dep, index) = > dep = = = nextDeps[index])}; return false, // Always re-render if no custom comparison})};
; if (displayName) {; OptimizedComponent.displayName = displayName};
; return OptimizedComponent};
;
// Hook for expensive calculations;
export const useExpensiveCalculation = <T>(; calculation: () = > T, deps: React.DependencyList,
...
;
// Image optimization component;
export const OptimizedImage: React.FC<{, src: string, alt: string, width?: number, height?: number; className?: string; loading?: 'lazy' | 'eager'; placeholder?: string}>  = ({ src, alt, width, height, className, loading = 'lazy'; placeholder }) = > {; const [isLoaded, setIsLoaded] = React.useState(false); const [hasError, setHasError] = React.useState(false);
...
;
// Debounced search hook;
export const useDebouncedSearch = (value: string, delay: number = 300) = > {; const [debouncedValue, setDebouncedValue] = React.useState(value);
; React.useEffect(() = > {; const handler = setTimeout(() = > {; setDebouncedValue(value)}, delay);
; return () = > {; clearTimeout(handler)}, [value, delay]);
; return debouncedValue};
;
// Performance metrics collection;
export const usePerformanceMetrics = () = > {; const [metrics, setMetrics] = React.useState({; renderCount: 0, lastRenderTime: 0, averageRenderTime: 0});
