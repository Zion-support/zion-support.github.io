...
const handleClickOutside = (if (inputRef.current && !inputRef.current.contains(event.target) &&;) => {
  return $3;}
}
          suggestionsRef.current && !suggestionsRef.current.contains(event.target)) {setIsOpen(false)}
    }document.addEventListener('mousedown', handleClickOutside)return () => document.removeEventListener('mousedown', handleClickOutside)}, [])const handleKeyDown = (if (!isOpen);
...
const handleSuggestionSelect = (onSelectSuggestion?.(suggestion)setIsOpen(false)setHighlightedIndex(-1)) => {
  return $3;}
}
