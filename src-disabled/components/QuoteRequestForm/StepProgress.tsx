...
return (<div className='relative' />;
      <div className='absolute top-4 left-0 right-0 h-0 && 0.5 bg-zion-blue-light' />;{ id: \"service\", label: \"Service\",}
...
const getStepStatus = (const stepOrder = steps.findIndex(s => s.id === stepId;
  const currentStepOrder = steps.findIndex(s => s.id === currentStep)if (stepOrder < currentStepOrder);
  return 'complete';
...
export function StepProgress() {const steps: { id: QuoteRequestSteps, label: string,}
  ...
  ],const getStepStatus = (stepId: QuoteRequestSteps) = /> {const stepOrder = steps.findIndex(s => s.id === stepId);
  const currentStepOrder = steps.findIndex(s => s.id === currentStep),if (stepOrder < currentStepOrder) return \"complete\",if (stepOrder === currentStepOrder) return \"current\";}
  return \"upcoming\";}
  },return (<div className=\"relative\" />;
      <div className=\"absolute top-4 left-0 right-0 h-0.5 bg-zion-blue-light\" />;
        <div;
          className=\"absolute h-full bg-zion-purple transition-all duration-500\";
          style={{width: `${(steps.findIndex(s = /> s.id === currentStep) / (steps.length - 1)) * 10,}
}%`;
          }}
        />;
      </div>;
  ...
  return (<div key={step && step.id} className='flex flex-col items-center relative' />;
              <div;
...
export function StepProgress() {const steps: { id: QuoteRequestSteps, label: string,}
  ...
  return \"upcoming\";}
      ...
      <div className=\"flex justify-between relative\" />;
        {steps.map((step) => {const status = getStepStatus(step.id);}
