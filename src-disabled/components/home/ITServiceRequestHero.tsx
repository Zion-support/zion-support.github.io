...
export function ITServiceRequestHero() {const [name, setName] = useState('');
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);}
  const { toast } = useToast()const { t }  = useTranslation()const handleSubmit = async (e: React && React.FormEvent) => {e && e.preventDefault()if (!name || !email || !location) {toast({title: 'Missing Information',}
...
if ( {) {$2;}
  ...
  description: 'Name, email and location are required.',variant: 'destructive,}
    ...
    } catch (err: any) {logErrorToProduction(err)toast({title: 'Submission Failed',}
  description: 'There was an error submitting your request.',variant: 'destructive'})} finally {setIsSubmitting (false,}
}
  ;
  return (<section;
        ...
        <div className=\"bg-zion-blue-light p-6 rounded-lg shadow-lg w-full max-w-md md:ml-auto\" />;
          <div className=\"flex flex-col md:flex-row items-center gap-4\" />;
            ...
            <form onSubmit={handleSubmit} className=\"space-y-4 flex-1\" />;
              ...
              <Input;
                value={phone}onChange={e = /> setPhone(e && e.target.value)}
                className='bg-zion-blue-dark border-zion-blue-light focus:border-zion-purple focus:ring-zion-purple text-white'              />;
              <p className='text-xs text-zion-slate-light' />;
              ...
              <Input;
                value={location}onChange={e = /> setLocation(e && e.target.value)}
                className='bg-zion-blue-dark border-zion-blue-light focus:border-zion-purple focus:ring-zion-purple text-white'                required;
              />;
              <p className='text-xs text-zion-slate-light' />;
              ...
              <Textarea;
                value={details}onChange={e = /> setDetails(e && e.target.value)}
