...
const createLazyComponent = (importFn: () => Promise<>, fallback?: React.ReactNode) => { ;}
  const LazyComponent = lazy(importFn;}
  return (props: ) => (<Suspense fallback={fallback || <LoadingSpinner /\"  />}><LazyComponent {...prop,}
} /\"  />;
    </Suspense>  )}
...
const createLazyComponent = (importFn: () => Promise<>, fallback?: React.ReactNode) => ;
  const LazyComponent = lazy(importFn) ;
  return(props: ) => (<Suspense fallback={fallback || <LoadingSpinner /\" ,}
} />\";
      <LazyComponent {...props} /\"  />\";
    </Suspense>;
  )}
...
const Terms = createLazyComponent(() => import('./pages/Terms'))';// Service pages';';
