...
export default ${fileName};`;
  ...
  if (filePath.includes('config/errorBoundaryConfig.tsx')) {
    fixed = `import React from "react";

const ErrorBoundaryConfig = () => {
  return (
    <div className="error-boundary-config">
      <h2>Error Boundary Config</h2>
      <p>This component is under development.</p>
    </div>
  );
};
...
export default ErrorBoundaryConfig;`;
  ...
  if (filePath.includes('constants/analytics.ts')) {
    fixed = `export const analytics = {
  ...
  if (filePath.includes('constants/seo.ts')) {
    fixed = `export const seo = {
  // SEO configuration
