...
function fixPageFile(filePath) {
  try {
    ...
    // Skip if already has proper Page component and no duplicates
    if (content.includes('export default function Page()') && !content.includes('function Page() {')) {
      return false;
    }
    ...
    const pageComponent = `function Page() {
  return (
    ...
    // Remove any existing Page function declarations and the export
    let cleanedContent = content.replace(/function Page\(\) \{[^}]*\}/gs, '');
    cleanedContent = cleanedContent.replace(/export default function Page\(\) \{[^}]*\}/gs, '');
    cleanedContent = cleanedContent.replace(/export default function Wrapped\([^}]*\}/gs, '');
    
    // Add the new Page component and export
...
// Main execution
console.log('Starting page component fixes (v2)...');
