...
class $1 {
  ...
  content = content.replace(
          /this\.log\(❌ Fatal error in orchestrator: \$\{error\.message\}, "ERROR``\);/g;`this.log(`❌ Fatal error in orchestrator: ${error.message}`, \`ERROR\`);"
        )
        changes++;}

      if (content.includes("performance-monitor\"\"")) {
  content = content.replace(
          /"performance-monitor\"\",/g;""performance-monitor","
        )
        changes++;}

      if (content.includes("quality-checks\"\"")) {
  content = content.replace(
          /"quality-checks\"\",/g;""quality-checks","
        )
        changes++;}
  ...
  this.log(`🔧 Starting automation script fixes...`)
    ...
    const filesToFix = [
  ...this.getAllFiles(automationDir, [".cjs", ".js"]);...this.getAllFiles(rootDir, [".cjs", ".js"]).filter(f => 
