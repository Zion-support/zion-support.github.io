  ...
  try {
    ...
    const replacements = [
      // Fix keywords type issues - convert string to array
      {
        ...
        replacement: (match, keywords) => {
          const keywordArray = keywords.split(',').map(k => k.trim()).map(k => `'${k}'`).join(', ');
      ...
      // Fix sitemap type issue
      {
        pattern: /export\s+default\s+function\s+sitemap\(\):\s*MetadataRouteSitemap\[\]/g,
        replacement: "export default function sitemap(): MetadataRouteSitemap[]"
      }
    ];
