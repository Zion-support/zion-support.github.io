...
log_message "Starting comprehensive PR merge process..."

# Extract PR information and process each one (process all remaining PRs)
awk '
/"number":/ {
    number = $2
    gsub(/,/, "", number)
...
/"ref":/ && in_head {
    ...
    gsub(/,$/, "", ref)
    gsub(/"/, "", ref)
    if (ref != "ref" && ref != "href" && ref != "archive_url" && ref != "git_refs_url" && count > 20) {
        printf "%s|%s\n", pr_number, ref
        in_head = 0
    }
}
...
echo ""
log_message "Comprehensive merge process complete!"
log_message "Summary:"
git log --oneline -15