  ...
  for (const filePath of files) {
    try {
      ...
      const componentNameMatch = content.match(
        /const\s+([A-Za-z\s]+)Page:\s*React\.FC/,
      );
      if (componentNameMatch) {
        ...
        if (oldName !== newName) {
          content = content.replace(
            new RegExp(
              `const\\s+${oldName.replace(/\s+/g, "\\s+")}Page:\\s*React\\.FC`,
              "g",
            ),
            `const ${newName}Page: React.FC`,
          );
          content = content.replace(
            new RegExp(
              `export\\s+default\\s+${oldName.replace(/\s+/g, "\\s+")}Page`,
              "g",
            ),
            `export default ${newName}Page`,
          );
          modified = true;
        }
      }
      ...
      if (titleMatch) {
        ...
        const newTitle = oldTitle.replace(/\b([a-z])/g, (match, letter) =>
          letter.toUpperCase(),
        );
