...
function findPageFiles(dir) {
  ...
  for (const file of files) {const fullPath = path.join(dir, file.name)
    
    if (file.isDirectory()) {
      findPageFiles(fullPath)} else if (file.name === 'page.tsx' || file.name === 'page.ts') {const relativePath = fullPath.replace(appDir, '').replace(/\\/g, '/');
...
function findComponentFiles(dir) {
  ...
  for (const file of files) {const fullPath = path.join(dir, file.name)
    ...
    if (file.isDirectory()) {
      findComponentFiles(fullPath)} else if (file.name.endsWith('.tsx') || file.name.endsWith('.ts')) {componentFiles.push(fullPath)}
  }
}
...
// Analyze all files
const fileAnalysis = [];
componentFiles.forEach(file => {const analysis = analyzeFile(file);
...
fileAnalysis.forEach(analysis => {const brokenInFile = [];
  ...
  if (brokenInFile.length > 0) {brokenLinksByFile[analysis.file] = brokenInFile}
});
