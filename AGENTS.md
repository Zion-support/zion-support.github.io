...
After EVERY task:
1. **Post-mortem** — What went well? What could be better? One sentence each.
2. **Extract rules** — 1–2 concrete rules in `[CONDITION] → [ACTION]` format.
3. **Append to learning log** — `~/.hermes/learning-log.md`, append-only, dated.
4. **Gap check** — Did you improvise because no rule existed? If yes: add `[GAP: what's missing]` entry.

Before EVERY task:
1. Read `~/.hermes/learning-log.md` and apply relevant rules.
2. Read `~/.hermes/memory/` for recent project context.

## Multi-Agent Coordination

- **Branch**: `hero-carousel` is the single integration branch.
- **Handoff**: When delegating, provide full context (file paths, constraints, background).
- **Verification**: Always verify subagent output before committing.
- **Coordination doc**: See `~/.hermes/multi-agent-coordination.md`.

## Bootstrap for New Agents

When spawning a new agent, provide this project-specific context:
```
Project: zion-support.github.io (Next.js/TypeScript, static export)
Site: https://ziontechgroup.com
Branch: hero-carousel
Working dir: C:\Users\Zion\zion-support.github.io
Build: npm run build
Push: git push origin hero-carousel

Conventions:
- Every page needs metadata with title, description, alternates.canonical
- Components in app/components/ (PascalCase)
- Data in app/data/ (TypeScript modules)
- Dark theme: slate-950 base, purple/pink accents
- Output style: raw compact lines only, no framing
```
