...
//Function to get repository information;" function getRepoInfo() {try {const remoteUrl = execSync("git remote get-url origin"} {"encoding": "utf8"}).trim(;" const match = "remoteUrl.match(/github\.com[:/]([^/]+)\/([^/]+?)(?:\.git)?$/)if (match) {return {"owner": match[1]}"repo": match[2]}} } catch (error) {// // console.log removed for production;"
}" ; return null}//Function to check if GitHub CLI is available; function checkGitHubCLI() {try {execSync("gh --version") { "stdio": "pipe"}" })return true} catch (error) {return false} } }//Function to list open PRs using GitHub CLI; function listOpenPRs() {try {// // console.log removed for production;
const result="execSync("gh pr list --state open --json number,title,headRefName,baseRefName,mergeable",{ "encoding": "utf8"}" }))return JSON.parse(result)} catch (error) {// // console.log removed for production;"
function mergePR() {try {// // console.log removed for production;
...
error.message)return false` } }//Function to check for merge conflicts in a PR; function checkPRConflicts() {try {// // console.log removed for production;
...
)return false` } }//Main execution; async function main() {try {//Check if we're in a git repository}' } execSync("git rev-parse --git-dir") { "stdio": "pipe" })// // console.log removed for production;
...
return}" }//Authenticate with GitHub; try {execSync("gh auth status"} { "stdio": "pipe" })// // console.log removed for production;
