  ...
  if (process.env['NEXT_RUNTIME'] === 'edge') {
    ...
    try {
      ...
      if (shouldDisableSentry) {
        console.log(
          'instrumentation.ts: Sentry DSN invalid or disabled by env var, using Sentry mock for Node.js.',
        );
        ...
        console.log(
          'instrumentation.ts: Valid Sentry DSN found, attempting to load actual @sentry/nextjs for Node.js.',
        );
      ...
      console.warn(
        'instrumentation.ts: Sentry SDK import/init failed for Node.js, falling back to mock:',
        error,
      );
      const mockSentry = await import('./src/utils/sentry-mock');
      Sentry = mockSentry.default;
...
export async function register() {
  ...
  if (isInvalidDsn) {
    if (process.env.NODE_ENV === 'development') {
      ...
      console.warn(
        'instrumentation.ts: Sentry DSN not configured for production environment',
      );
    }
    return;
  }
  ...
  console.log(
    `instrumentation.ts: Initializing Sentry for server-side. Release: ${SENTRY_RELEASE}, Env: ${SENTRY_ENVIRONMENT}`,
  );
  ...
  try {
    Sentry.init({
      ...
      beforeSend(event: any, hint: any) {
        ...
        if (
          ...
          console.log(
            'instrumentation.ts: Sentry event dropped due to empty exception value.',
          );
          return null;
        }
    ...
    console.log(
      'instrumentation.ts: Server-side Sentry initialized successfully',
    );
  } catch (error) {
