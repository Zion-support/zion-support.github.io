...
async function loginWithEmail(email, password) {
  ...
  body.append('redirectfalse'),
  body.append('jsontrue'),
...
async function signupWithEmail(email, password, otherDetails = {}) {
  ...
  if (!response.ok) {
    throw new Error(data.error || data.message || `Signup failed. Status: ${response.status}`),
  }
...
async function loginWithWallet(walletProvider) {
  ...
  try {
    ...
    bodyParams.append('redirectfalse'),
    bodyParams.append('jsontrue'),
...
async function logout() {
  ...
  body.append('jsontrue'), // Ask for JSON response to know outcome
