...
# Script to merge cursor branches in smaller batches
set -e
...
BACKUP_BRANCH="cursor-merge-backup-$(date +%Y%m%d-%H%M%S)"
LOG_FILE="cursor-merge-log-$(date +%Y%m%d-%H%M%S).txt"
...
for ((i=0; i<total_branches; i++)); do
    ...
    # Push changes every 10 branches to avoid losing work
...
# Summary
echo ""
log_message "🎉 Batch merge of cursor branches completed!"
log_message "📊 Summary:"
