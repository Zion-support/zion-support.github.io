...
const Products: NextPage = () => {
  const products = [
    {
      title: 'Revenue AI Copilot',
      description: 'Multi-agent copilots for Sales and CS with guardrails and analytics.',
      icon: Brain,
    ...
    {
      title: 'Cloud FinOps Toolkit',
      description: 'Automated cost governance, rightsizing, and savings plan optimization.',
      icon: Cloud,
      color: 'from-emerald-600 to-cyan-600',
      category: 'Cloud Platforms',
      pricing: 'From $3k/mo',
      href: '/services/it/cloud-finops'
    },
    {
      title: 'SOC-as-a-Service',
      description: '24/7 managed detection, response, and compliance reporting.',
      icon: Shield,
          ...
          <div className="container mx-auto px-4">
            <div className="flex flex-wrap justify-center gap-4">
              {categories.map((category, index) => (
                <button key={index} className="px-4 py-2 rounded-full bg-white text-gray-700 border border-gray-200 hover:border-blue-500 hover:text-blue-600">
                  {category}
                </button>
              ))}
            </div>
              ...
              {products.map((product, index) => (
                <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-2">
                  <div className="relative">
                    ...
                    {product.badge && (
                      <div className="absolute top-3 right-3 bg-white/90 text-blue-600 text-xs font-semibold px-2 py-1 rounded-full border border-blue-200">
                        {product.badge}
                      </div>
                    )}
                  </div>
