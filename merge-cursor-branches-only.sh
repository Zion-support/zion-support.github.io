...
# Merge Cursor Branches Only Script
# This script will merge only the cursor branches that actually exist

set -e

echo "🚀 Starting Merge of Cursor Branches Only..."
echo "📊 Started at: $(date)"
echo "=================================================="

# Configuration
BATCH_SIZE=100
LOG_FILE="cursor-merge-$(date +%Y%m%d-%H%M%S).log"
...
# Get only cursor branches that exist
log_message "📋 Getting list of cursor branches that exist..."
ALL_BRANCHES=$(git branch -r --no-merged main | sed 's/origin\///' | grep -E "^cursor/" | sort)
TOTAL_BRANCHES=$(echo "$ALL_BRANCHES" | wc -l)

log_message "📊 Total cursor branches to process: $TOTAL_BRANCHES"
...
# Final summary
echo ""
log_message "🎉 Cursor branches merge process completed!"
...
echo ""
echo "🎯 All cursor branches have been processed! Check the log file for detailed information."
