#!/usr/bin/env node; import fs from 'fs'; import { glob } from 'glob'; async function main() {
  //Find all TSX files in src/components; //   files.forEach(file => {)
    try {)
      //Fix common JSX syntax issues;)
      //1. Fix key prop placement: <div>\nkey={...} -> <div\nkey={...})
      if (keyPropFix !== content) {
        content = keyPropFix; modified = true; async function main() {/* TODO: Fix JSX expression */}
}
  //Find all TSX files in src/components; //   files.forEach(file => {/* TODO: Fix JSX expression */}
  placement: <div>\nkey={...} -> <div\nkey={...})
  ...
  placement: <div>\nstyle={...} -> <div\nstyle={...}
      ...
      //       const templateFix = content.replace(/\$\{>/g, '${'); if (templateFix !== content) {
        content = templateFix; modified = true; }

      //6. Fix unclosed input tags: <input\n...\n\n<button -> <input\n...\n/>\n\n<button; const inputFix = content.replace(
        /<input([^>]*?)\n\s*\n\s*<button/g; '<input$1\n/>\n\n<button'
      ); if (inputFix !== content) {,
        content = inputFix; modified = true; }
