...
;
    ...
    content = content.replace(/<meta[^></meta>]*\/>;/g, (match) => match.slice(0, -1));
;
...
;
      ...
      line = line.replace(/<meta[^></meta>]*\/>;/g, (match) => match.slice(0, -1));
;
...
;
  ...
  ;
    ...
    console.error(`❌ Error fixing strings in ${filePath}:`;
    return false;
  }
}
;
...
;
  ...
  ;
    ...
    for (const item of items) {;
      ...
      const stat = fs.statSync(fullPath);
      ';
      if (stat.isDirectory() && !['node_modules', '.git', 'dist', 'build', '.next', 'out';
