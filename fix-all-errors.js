...
function fixFile(filePath) {
  ...
  const missingClosePattern = /(\w+):\s*(Service Feature+)\s*$/gm;
  ...
  const memoClosePattern = /(\w+):\s*(Service Feature+)\s*\n\s*\);/g;
...
function getFilesWithErrors() {
  try {
    ...
    const files = new Set();
    output.split('\n').forEach(line => {
      const match = line.match(/^(Service Feature}
}
...
function main() {
  ...
  try {
    ...
    console.log('✅ All TypeScript errors fixed!')} catch (error) {
