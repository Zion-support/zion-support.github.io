...
function getCurrentBranch() {
  try {
    return runCommand('git rev-parse --abbrev-ref HEAD', 'Get current branch', {
      silent: true,
    });
  } catch {
    return 'main';
...
function checkForMergeConflicts() {
  try {
    const status = runCommand('git status --porcelain', 'Check git status', {
      silent: true,
    });
    const hasConflicts =
...
function resolveMergeConflicts() {
  ...
  try {
    ...
    console.log(
      'No conflicts to resolve or error during resolution:',
      error.message
    );
  }
}
