<<<<<<< HEAD
import React from 'react";"
import Link from 'next/link";"
import { Brain, Send, TrendingUp, Zap, BarChart3, Shield, DollarSign, Clock, Mail, ArrowRight, CheckCircle, Star } from 'lucide-react";"
import React from 'react";"
import { ArrowRight, Brain, Mail, CheckCircle, Zap, TrendingUp, Send, BarChart3, Shield, DollarSign, Clock, Star, BarChart } from 'lucide-react";"
import Link from "next/link
    { number: 300%, label: Revenue Increase
    { number: 150%, label: Conversion Boost"
    { number: 20 hrs, label: Time Saved/Week";"
    { "number": '95%', "label": 'Deliverability Rate'
}}}}'"
=======
import React from 'react'
import Link from 'next/link'
import { Brain, Send, TrendingUp, Zap, BarChart3, Shield, DollarSign, Clock, Mail, ArrowRight, CheckCircle, Star } from from 'lucide-react'
import React from 'react'
import { ArrowRight, Brain, Mail, CheckCircle, Zap, TrendingUp, Send, BarChart3, Shield, DollarSign, Clock, Star, BarChart } from from 'lucide-react'
import Link from 'next/link'
    { "number": '300%', "label": 'Revenue Increase'";'"
    { "number": '150%', "label": 'Conversion Boost'";'"
    { "number": '20 hrs', "label": 'Time Saved/Week';";'"
    { "number": '95%', "label": 'Deliverability Rate'";'"
>>>>>>> cursor/fix-errors-and-merge-to-main-eb70
