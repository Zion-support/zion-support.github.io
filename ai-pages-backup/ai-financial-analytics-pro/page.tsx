<<<<<<< HEAD
'use client";"
import Link from 'next/link";"
import { Brain, BarChart3, Shield, Target, Zap, Globe, DollarSign, Calculator, Activity, TrendingUp, Lock, Building2, Database, FileText, CreditCard, Wallet, ArrowRight, CheckCircle, Star, Sparkles, PieChart } from 'lucide-react";"
    { "number": '500+', "label": 'Financial Institutions'"
    { "number": '1 M+', "label": 'Transactions Analyzed'"
    { "number": '99.9%', "label": 'Data Accuracy'"
    { "number": '50%', "label": 'Faster Reporting'"
    { "name": 'QuickBooks'"
    { "name": 'Xero'"
    { "name": 'SAP'"
    { "name": 'Oracle'"
    { "name": 'Salesforce'"
    { "name": 'HubSpot'"
    { "name": 'Stripe";"'"
    { "name": 'PayPal'
}}}}}}}}}}}}'"
=======
'use client'
import Link from 'next/link'
import { Brain, BarChart3, Shield, Target, Zap, Globe, DollarSign, Calculator, Activity, TrendingUp, Lock, Building2, Database, FileText, CreditCard, Wallet, ArrowRight, CheckCircle, Star, Sparkles, PieChart } from from 'lucide-react'
    { "number": '500+', "label": 'Financial Institutions'";'"
    { "number": '1 M+', "label": 'Transactions Analyzed'";'"
    { "number": '99.9%', "label": 'Data Accuracy'";'"
    { "number": '50%', "label": 'Faster Reporting'";'"
    { "name": 'QuickBooks'";'"
    { "name": 'Xero'";'"
    { "name": 'SAP'";'"
    { "name": 'Oracle'";'"
    { "name": 'Salesforce'";'"
    { "name": 'HubSpot'";'"
    { "name": 'Stripe';";'"
    { "name": 'PayPal'";'"
>>>>>>> cursor/fix-errors-and-merge-to-main-eb70
