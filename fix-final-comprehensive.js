import fs from 'fs';'import { fileURLToPath } from 'url''// Files that need specific fixes;
const specificFixes = {
  
  'app/blog/ai-2026-autonomous-business-intelligence-mega-breakthrough/page.tsx': {''    keywords:
...
const keywordArray = fix.keywords
          .split(', ')''function processFile(filePath) {/* TODO: Fix JSX expression */}
          .map(k => `'${k}'`)''          .join(', ')''        if (pattern.test(content)) {/* TODO: Fix JSX expression */}`
  s: [${keywordArray}],`)
          modified = true
        }
      }
...
const replacements = [
      ...
      // Fix import paths for components
      {
        ...
        pattern: /import\s+{\s*useRouter\s*}\s+from\s+'\.\.\/\.\.\/utils\/navigation';/g,''        replacement: "import { useRouter } from '../utils/navigation';"},";'"'      // Fix import paths for root-level files"      {
  
        pattern: /import\s+{\s*Metadata\s*}\s+from\s+'\.\/types\/next';/g,''        replacement: "import { Metadata } from './types/next';"},";'"'      {"  
        pattern: /import\s+Link\s+from\s+'\.\/utils\/link';/g''        replacement: "import Link from './utils/link';",";'"'      {/* TODO: Fix JSX expression */}"  n: /import\s+{\s*useRouter\s*}\s+from\s+'\.\.\/\.\.\/utils\/navigation';/g,''        replacemen,
  t: "import { useRouter } from '../utils/navigation';"},";'"'      // Fix import paths for root-level files"      {/* TODO: Fix JSX expression */}
