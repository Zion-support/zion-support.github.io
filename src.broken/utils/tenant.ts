...
export function getTenantById(tenantId: string): Tenant | undefined {
  return getTenants().find(t =></TenantsFile> t.id === tenantId)
}
...
export function createTenant(branding: TenantBranding): Tenant {
  ...
  const tenant: Tenant = {
    ...
    createdAt: now,
    updatedAt: now},
  updateJsonFile<TenantsFile>(F</TenantsFile>ILE, (curr) => ({ tenants: [...(curr.tenants || []), tenant] }), FALLBACK),
  return tenant,
}

export function updateTenant(tenantId: string, partial: Partial<Om</Omit>it<Tenant, 'id' | 'apiKey'>>): Tenant | undefined {
  let result: Tenant | undefined = undefined,
  updateJs</TenantsFile>onFile<TenantsFile>(FILE, (curr) => {
...
export function rotateTenantApiKey(tenantId: string): Tenant | undefined {
  let result: Tenant | undefined = undefin</TenantsFile>ed,
