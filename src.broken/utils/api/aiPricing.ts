...
function locationCostIndex(location: string): number {
  ...
  if (
    /(brazil|india|pakistan|philippines|vietnam|indonesia|nigeria|egypt)/i.test(
      loc
    )
  )
    return 0.75;
  if (
    /(europe|germany|france|uk|united kingdom|spain|italy|portugal|netherlands|sweden|norway|denmark|finland|austria|switzerland)/i.test(
      loc
    )
  )
    return 1.05;
  if (
    /(usa|united states|canada|australia|new zealand|singapore|hong kong|uae|dubai)/i.test(
      loc
    )
  )
    return 1.15;
...
function baseHourlyForSkillsOrCategory(skillsOrCategory: string[]): number {
  ...
  if (/devops|kubernetes|aws|gcp|azure|terraform|sre|platform/.test(text))
    return 60;
  if (
    /data|ml|machine learning|ai|analytics|big data|spark|pytorch|tensorflow/.test(
      text
    )
  )
    return 65;
  if (/mobile|ios|android|react native|flutter/.test(text)) return 55;
  if (/backend|api|node|django|rails|java|go|rust|spring|express/.test(text))
    return 55;
...
function computeHeuristicClientBudget(
  ...
  const weeks =
    typeof timelineWeeks === 'number' && timelineWeeks > 0 ? timelineWeeks : 2;
...
<<<<<<< HEAD
        {
          role: 'system',
          content: 'You output only JSON. No prose. Be concise and factual.',
        },
...
<<<<<<< HEAD
      confidence:
        (json.confidence as BudgetSuggestion['confidence']) || 'Medium',
...
async function callOpenAIForTalentRate(
  input: TalentRateRequest
): Promise<TalentRateSuggestion | null> {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return null;
  try {
    const client = new OpenAI({ apiKey });
...
<<<<<<< HEAD
      confidence:
        (json.confidence as TalentRateSuggestion['confidence']) || 'Medium',
      rationale: String(json.rationale || ''),
      modelUsed: response.model || process.env.OPENAI_MODEL || 'gpt-4o-mini',
      source: 'openai',
    };
...
export async function generateTalentRateSuggestion(
  ...
  const min = roundMoney((heuristic.min + llm.min) / 2);
  const max = roundMoney((heuristic.max + llm.max) / 2);
  const confidence: TalentRateSuggestion['confidence'] =
