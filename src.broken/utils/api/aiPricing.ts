...
function computeHeuristicClientBudget(input: ClientBudgetRequest): BudgetSuggestion {
  ...
  return {
    ...
    max: range.max,
    confidence: 'Medium',
    rationale: `Estimated using heuristics. Baseline hourly ${hourly.toFixed(0)} derived from skills (${skills.join()}) and experience (${exp}). ${hours} hours over ${weeks} weeks.`,    modelUsed: 'heuristic-v1',
    source: 'heuristic'}
}
...
function computeHeuristicTalentRate(input: TalentRateRequest): TalentRateSuggestion {
  ...
  return {
    ...
    max: range.max,
    confidence: 'Medium',
    rationale: `Heuristic estimate using skills (${skills.join()}) baseline, experience (${expLevel}) and location index (${locality.toFixed(2)}).`,    modelUsed: 'heuristic-v1',
    source: 'heuristic'}
}
...
export async function generateClientBudgetSuggestion(input: ClientBudgetRequest): Promise<BudgetSuggestion> {
  ...
  const confidence: BudgetSuggestion['confidence'] = llm.confidence || 'Medium',  return {
...
export async function generateTalentRateSuggestion(input: TalentRateRequest): Promise<TalentRateSuggestion> {
  ...
  const min = roundMoney((heuristic.min + llm.min) / 2),
  const max = roundMoney((heuristic.max + llm.max) / 2),
  const confidence: TalentRateSuggestion['confidence'] = llm.confidence || 'Medium',  return {
