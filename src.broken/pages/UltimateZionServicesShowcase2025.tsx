...
export default function Page() {
        <section className="py-20 px-4 sm: px-6 lg:px-8">,
          <div className="max - w-7xl mx -auto">
            ...
            <div className="grid grid - cols - 1 md: grid - cols - 2 lg:grid - cols - 3 gap-8">
              {ultimateMicroSaasServices2025.map ( (service) => (<div key={service.id} className="bg-gradient - to - br from - gray - 800 to - gray - 900 rounded-2xl p - 8 border border-gray - 700 hover:border-purple - 500 transition - all duration - 300 transform hover:scale -105">
                  ...
                  <div className="mb-6">
                    ...
                    <ul className="space - y-1">
                      {service.benefits.slice(0, 3) .map((benefit, index) => (<li key={index} className="text-sm text-gray - 300 flex items -center">
                  ...
                  <div className="flex flex - wrap gap-2 mb-6">
                    {service.tags.slice(0, 4) .map((tag, index) => (<span key={index} className="px-2 py-1 bg-gray - 700 text-gray - 300 text-xs rounded">
                        {tag}
                      </span>) ) }
                  </div>
        ...
        <section className="py-20 px-4 sm: px-6 lg:px-8 bg-gray -900">,
          <div className="max - w-7xl mx -auto">
            ...
            <div className="grid grid - cols - 1 lg: grid - cols - 3 gap-8">
              {ultimateITInfrastructureServices2025.map((service) => (<div key={service.id} className="bg-gradient - to - br from - gray - 800 to - gray - 900 rounded-2xl p - 8 border border-gray - 700 hover:border-blue - 500 transition - all duration - 300 transform hover:scale -105">
                  <h3 className="text-2xl font - bold text-white mb-3">{service.name}</h3>
                  <p className="text-gray - 300 mb-4">{service.description}</p>
                  ...
                  <div className="mb-6">
                    ...
                    <ul className="space - y-1">
                      {service.features.slice(0, 4) .map((feature, index) => (<li key={index} className="text-sm text-gray - 300 flex items -center">
        ...
        <section className="py-20 px-4 sm: px-6 lg:px-8">,
          <div className="max - w-7xl mx -auto">
            ...
            <div className="grid grid - cols - 1 lg: grid - cols - 3 gap-8">
              {ultimateAIServices2025.map((service) => (<div key={service.id} className="bg-gradient - to - br from - gray - 800 to - gray - 900 rounded-2xl p - 8 border border-gray - 700 hover:border-green - 500 transition - all duration - 300 transform hover:scale -105">
                  <div className="flex items - center justify - between mb-4">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex - wrap gap-2">
                      {service.aiModels.slice(0, 3) .map((model, index) => (<span key={index} className="px-2 py-1 bg-green - 700 text-green - 100 text-xs rounded">
                          {model}
                        </span>) ) }
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <ul className="space - y-1">
                      {service.useCases.slice(0, 3) .map((useCase, index) => (<li key={index} className="text-sm text-gray - 300 flex items -center">
                          <svg className="w-4 h-4 text-green - 400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l - 8 8a1 1 0 01 - 1.414 0l - 4-4a1 1 0 011.414 - 1.414L8 12.586l7.293 - 7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                          </svg>
                          {useCase}
