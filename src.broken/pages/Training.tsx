            ...
            <div  className="bg-slate - 800 / 50 backdrop - blur - sm rounded-xl p - 6 border border-slate -700 / 50">
              ...
              <div  className="grid grid - cols - 1 md:grid - cols - 3 gap-4">
                ...
                <div>
                  ...
                  <div  className="flex flex - wrap gap-2">
                    {levels.map(level => (<button     key={level.id}
                        ...
                        className={`px-3 py-1 rounded-lg text-sm font - medium transition - colors ${activeLevel === level.id
                            ? 'bg-blue - 500 text-white'
                            : 'bg-slate - 700 / 50 text-gray - 300 hover:bg-slate - 600 / 50'
                        }`}
                      >
                        {level.name}
                        <span className="text-xs opacity - 75 ml-1"> ({level.count}) </span>
                      </button>) ) }
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div  className="flex flex - wrap gap-2">
                    {formats.map(format => (<button     key={format.id}
                        onClick={ () => setActiveFormat(format.id) }
                        className={`px-3 py-1 rounded-lg text-sm font - medium transition - colors ${activeFormat === format.id
                            ? 'bg-blue - 500 text-white'
                            : 'bg-slate - 700 / 50 text-gray - 300 hover:bg-slate - 600 / 50'
                        }`}
                      >
                        {format.name}
                        <span className="text-xs opacity - 75 ml-1"> ({format.count}) </span>
                      </button>) ) }
                  </div>
                </div>
              </div>
        ...
        <section className="py-16 px-4">
          <div  className="max - w-7xl mx -auto">
            <div  className="grid grid - cols - 1 lg:grid - cols - 2 gap-8">
              {filteredPrograms.map((program, index) => (<motion.div
                  key={program.id}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  ...
                  <div  className="flex flex - wrap gap-1 mb-4">
                    {program.tags.slice(0, 3) .map((tag, idx) => (<span
                        key={idx}
                  ...
                  <div  className="mb-4">
                    ...
                    <ul className="space - y-1">
                      {program.features.slice(0, 2) .map((feature, idx) => (<li
                          key={idx}
