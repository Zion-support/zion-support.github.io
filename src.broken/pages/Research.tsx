            ...
            <div  className="bg-slate - 800 / 50 backdrop - blur - sm rounded-xl p - 6 border border-slate -700 / 50">
              ...
              <div  className="grid grid - cols - 1 md:grid - cols - 3 gap-4">
                ...
                <div>
                  ...
                  <div  className="flex flex - wrap gap-2">
                    {categories.map(category => (<button     key={category.id}
                        ...
                        className={`px-3 py-1 rounded-lg text-sm font - medium transition - colors ${activeCategory === category.id
                ...
                <div>
                  ...
                  <div  className="flex flex - wrap gap-2">
                    {statuses.map(status => (<button     key={status.id}
                        onClick={ () => setActiveStatus(status.id) }
                        className={`px-3 py-1 rounded-lg text-sm font - medium transition - colors ${activeStatus === status.id
                            ? 'bg-blue - 500 text-white'
                            : 'bg-slate - 700 / 50 text-gray - 300 hover:bg-slate - 600 / 50'
                        }`}
                      >
                        {status.name}
                        <span className="text-xs opacity - 75 ml-1"> ({status.count}) </span>
                      </button>) ) }
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div  className="flex flex - wrap gap-2">
                    {types.map(type => (<button     key={type.id}
                        onClick={ () => setActiveType(type.id) }
                        className={`px-3 py-1 rounded-lg text-sm font - medium transition - colors ${activeType === type.id
                            ? 'bg-blue - 500 text-white'
                            : 'bg-slate - 700 / 50 text-gray - 300 hover:bg-slate - 600 / 50'
                        }`}
                      >
                        {type.name}
                        <span className="text-xs opacity - 75 ml-1"> ({type.count}) </span>
                      </button>) ) }
                  </div>
                </div>
              </div>
              ...
              {filteredProjects.map((project, index) => (<motion.div
                ...
                >
                  ...
                  <div  className="flex items - start justify - between mb-4">
                    ...
                    {project.featured && (<span className="px-2 py-1 bg-yellow-500 / 20 text-yellow-400 rounded-full text-xs font -medium">
                        Featured
                      </span>) }
                  </div>
                  ...
                  <div  className="flex items - center justify - between mb-4">
                    ...
                    <div  className="flex items - center gap-2">
                      <div  className="w-16 h-2 bg-slate - 700 rounded-full overflow-hidden">
                        <div  className={`h-full ${getProgressColor(project.progress) }`}
                  ...
                  {/* Publications */}
                  {project.publications.length > 0 && (<div  className="mb-4">
                      <h4 className="text-white font - medium text-sm mb-2">
                        Publications({project.publications.length}) </h4>
                      <div  className="space - y-2">
                        {project.publications.slice(0, 2) .map((pub, idx) => (<div  key={idx}
                  ...
                  <div  className="flex flex - wrap gap-1 mb-4">
                    {project.tags.slice(0, 4) .map((tag, idx) => (<span
                        key={idx}
