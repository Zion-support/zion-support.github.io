...
import Cookies from 'js-cookie',
import enTranslation from '../../public/locales/en-US/common.json',
import esTranslation from '../../public/locales/es-ES/common.json',// Initialize i18next
i18n
  .use(LanguageDetector) // Detect user language
  .use(initReactI18next) // Initialize react-i18next
  .init({_resources: {
  ...
  .catch(error => {
    logErrorToProduction('Error initializing i18next or its detector:', { data: error }),    // This helps prevent an unhandled promise rejection if init fails.
  }),
