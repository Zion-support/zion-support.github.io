        ...
        <div className={`flex-1 flex flex-col ${activeChat ? 'w-2/3' : 'w-full'}`}>
          ...
          <div className="flex-1 p-4">"
            <div className="grid grid-cols-2 gap-4 h-full">'`
              {participants.map((participant, index) => (<div key={participant.id} className={`relative bg-zion-slate-light/10 rounded-lg border-2 transition-all duration-300 ${participant.isSpeaking ? 'border-zion-emerald shadow-lg shadow-zion-emerald/30' : 'border-zion-slate-light'}`}>
                  {/* Video Placeholder */}"
...
"
            <div className="p-4 border-t border-zion-slate-light">"
              <div className="flex gap-2">'"
                ...
                <button onClick={sendMessage} disabled={!newMessage.trim()} className="px-4 py-2 bg-zion-purple text-white rounded-lg hover:bg-zion-purple-light transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  Send
                </button>
              </div>
            </div>
