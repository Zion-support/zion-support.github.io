...
export function AccessibilityEnhancer({ enabled = true, showSettings = true }: AccessibilityEnhancerProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {,
      ...
      if(['ArrowUpArrowDown', 'ArrowLeftArrowRight'].includes (event.key) ) {
              ...
              <div className="flex border-b border-gray - 200 dark: border-gray -700">
                {tabs.map((tab) => {
                  ...
                  return (<button     key={tab.id}
                      ...
                      className={`flex - 1 flex items - center justify - center space - x-2 px-4 py-3 text-sm font - medium transition - colors ${isActive
                ...
                <div className="grid grid-cols-2 gap-3">
                  {quickActions.map((action) => (
                    <button
                      ...
                      className={`p-3 rounded-lg border transition-all duration-200 ${action.active
                  ...
                  <div className="flex-1 grid grid-cols-4 gap-2">
                    {fontSizes.map((size) => (
                      <button
                        ...
                        className={`p-2 rounded text-xs font-medium transition-colors ${settings.fontSize === size
