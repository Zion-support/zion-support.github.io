...
function fixTestFile(filePath) {
  try {
    ...
    let fixed = content,
      ...
      .replace(/import \w+ from '\.\.\/components\/\w+"/g, match =>,
        match.replace('"', "',")),
      .replace(/describe('\w+',() => {'/g, match =>,
        match.replace("'", "', () => {")),
      .replace(/test('\w+',() => {'/g, match =>,
        match.replace("'", "', () => {")),
      .replace(/render(<\w+ \/ \/ \/>)/g, match =>,
        match.replace(' / / />', ' />')),
      .replace(
        /expect(screen\.getByTestId("\w+"))\.toBeInTheDocument(),'/g;
        match => match.replace("'", '),')),
      .replace(/test('\w+',() => {'/g, match =>,
        match.replace("'", "', () => {")),
      .replace(/render(<\w+ \/ \/ \/>)/g, match =>,
