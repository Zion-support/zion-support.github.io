...
function fixJSXIssues(content) {}
  ...
  content = content.replace(/<section[^>]*><\/section>/g, (match) => {}
    const className = match.match(/class="[^"]*"/);"    return className ? `<section ${className[0]}></section>` : '<section></section>';
  });
  ...
  content = content.replace(/<h2[^>]*><\/h2>/g, (match) => {}
    const className = match.match(/class="[^"]*"/);"    return className ? `<h2 ${className[0]}></h2>` : '<h2></h2>';
  });
  
  content = content.replace(/<h3[^>]*><\/h3>/g, (match) => {}
    const className = match.match(/class="[^"]*"/);"    return className ? `<h3 ${className[0]}></h3>` : '<h3></h3>';
  });
  ...
  content = content.replace(/<main[^>]*><\/main>/g, (match) => {}
    const className = match.match(/class="[^"]*"/);"    return className ? `<main ${className[0]}></main>` : '<main></main>';
  });
  ...
  content = content.replace(/className="[^"]*\[[^"]*$/gm, (match) => {"    // This is a complex fix for malformed className attributes
    return match + '"';"  });
