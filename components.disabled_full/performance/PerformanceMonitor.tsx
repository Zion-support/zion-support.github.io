...
const "PerformanceMonitor": React.F,"
  C: = () => {, useEffect(() => {} "if": (typeof window = == "undefined") return}const "metrics": PerformanceMetric,"
  s: = {};
const fcpObserver = new PerformanceObserver((list) => {; for (const entry of list.getEntries()) {"; if (entry.name === "first-contentful-paint") {"} metrics."fcp": '= entry.startTime',"}}"} })" fcpObserver.observe({ "entryTypes": ["paint"]})" const lcpObserver = new PerformanceObserver((list) => {;
const entries = list.getEntries();
const lastEntry = entries[entries.length - 1]} metrics."lcp": '= lastEntry.startTime',"}})"; lcpObserver.observe({ "entryTypes": ["largest-contentful-paint"]})" const fidObserver = new PerformanceObserver((list) => {; metrics.fid = (entry as any).processingStart - entry.startTime,"}}}"})" fidObserver.observe({ "entryTypes": ["first-input"]})" "let": 'clsValue = 0';
const clsObserver = new PerformanceObserver((list) => {"interface PerformanceMetrics {;
const fcpObserver = new PerformanceObserver((list) => {; for (const entry of list.getEntries()) {"; if (entry.name === "first-contentful-paint") {"} metrics."fcp": '= entry.startTime'}"} }"} })" fcpObserver.observe({ "entryTypes": ["paint"]})" const lcpObserver = new PerformanceObserver((list) => {;
const entries = list.getEntries();
const lastEntry = entries[entries.length - 1]} metrics."lcp": '= lastEntry.startTime'}"})"; lcpObserver.observe({ "entryTypes": ["largest-contentful-paint"]})" const fidObserver = new PerformanceObserver((list) => {; metrics.fid = (entry as any).processingStart - entry.startTime}"}"})" fidObserver.observe({ "entryTypes": ["first-input"]})" "let": 'clsValue = 0';
const clsObserver = new PerformanceObserver((list) => {"interface PerformanceMetrics {; fcp?: number; lcp?: number; fid?: number} cls?: number}; ttfb?: number} fmp?: number};
const "PerformanceMonitor": React.FC = () => {} if (typeof window === "undefined") return};
const "metrics": PerformanceMetrics = {};
const metric,"s": PerformanceMetrics = {};
const "metrics": PerformanceMetrics = {} ; for (const entry of list.getEntries()) {; if (entry.name = == "first-contentful-paint") {; metrics.fcp = entry.startTime}} })} fcpObserver.observe({ "entryTypes": ["paint"] })} const fcpObserver = new PerformanceObserver()} const lcpObserver = new PerformanceObserver(list => {) metrics.lcp = lastEntry.startTime}})} lcpObserver.observe({ "entryTypes": ["largest-contentful-paint"] })} metrics.fcp = entry.startTime,"})" lcpObserver.observe()} metrics.fid = (entry as unknown).processingStart - entry.startTime};
const fidObserver = new PerformanceObserver(list => {"} fidObserver.observe({ "entryTypes": ["first-input"] })} let clsValue = 0; if (!(entry as any).hadRecentInput) {; clsValue += (entry as any).value} ; metrics.cls = clsValue})" clsObserver.observe({ "entryTypes": ["layout-shift"]})" const navigationEntry = performance.getEntriesByType("navigation")[0] as PerformanceNavigationTiming; "if": (navigationEntry) {} metrics.ttfb = navigationEntry.responseStart - navigationEntry.requestStart}"}";
const fmpObserver = new PerformanceObserver((list) => {"; if (entry.name === "first-meaningful-paint") {"} metrics."fmp": '= entry.startTime'}"}"})} clsObserver.observe({ "entryTypes": ["layout-shift"] })} const clsObserver = new PerformanceObserver()} const navigationEntry = performance.getEntriesByType(" "navigation" )[0] as PerformanceNavigationTiming; if (navigationEntry) {; metrics.ttfb =; navigationEntry.responseStart - navigationEntry.requestStart} metrics.fid = (entry as any).processingStart - entry.startTime}"; if (!(entry as unknown).hadRecentInput) {; clsValue += (entry as unknown).value}"})} } ; if (entry.name = == "first-meaningful-paint") {; metrics.fmp = entry.startTime}}" fmpObserver.observe({ "entryTypes": ["paint"] })})" fmpObserver.observe({ "entryTypes": ["paint"]})" const sendMetrics = () => {"} if (typeof window !== "undefined" && (window as any).gtag) {/* TODO: Fix JSX expression */}"
