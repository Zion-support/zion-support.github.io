...
# Efficient Merge All Cursor Branches
# This script processes cursor branches in large batches with robust error handling

set -e

echo "🚀 Starting efficient merge of all cursor branches..."
...
MAX_RETRIES=2
BACKUP_BRANCH="cursor-merge-efficient-backup-$(date +%Y%m%d-%H%M%S)"
LOG_FILE="cursor-merge-efficient-log-$(date +%Y%m%d-%H%M%S).txt"
FAILED_BRANCHES_FILE="failed-branches-$(date +%Y%m%d-%H%M%S).txt"
...
log_message "🔄 Starting efficient batch processing..."
...
# Summary
echo ""
log_message "🎉 Efficient merge completed!"
