...
const branchOutput = execSync()
    'git branch -r | grep -v backup | grep -E "(cursor|codex|pr|feature|bugfix)"','"'    { encoding: 'utf8' }''// //Step,"  1: Ensure we're on main and up to date,''// try {/* TODO: Fix JSX expression */}
...
//   try {,
    //Fetch the branch
    execSync(`git fetch origin ${branchName}`, { stdio: 'pipe' })''    //Check if branch exists and has commits
    try {
...
//       return { success: false, method: 'not_found' }''    }
    ...
    try { const mergeBase = execSync(`git merge-base HEAD origin/${branchName}`, {)
...
function resolveConflictsAndMerge(branchName) {/* TODO: Fix JSX expression */}
  ...
  o: 'pipe' })''    //Check if branch exists and has commits
    try {/* TODO: Fix JSX expression */}`
...
//     try {
      ...
      execSync('git reset --hard HEAD', { stdio: 'pipe' })''      execSync(
...
//     return {/* TODO: Fix JSX expression */}
  ...
  o: 'pipe' })''      execSync(")`""        `git merge origin/${branchName} -X theirs --no-ff -m "Auto-merge ${branchName} (theirs strategy)"`,""        {/* TODO: Fix JSX expression */}
  o: 'pipe' }''      )
//       return {/* TODO: Fix JSX expression */}
...
//       }
    ...
    try {/* TODO: Fix JSX expression */}
  o: 'pipe' })''      execSync(")`""        `git merge origin/${branchName} -X ours --no-ff -m "Auto-merge ${branchName} (ours strategy)"`,""        {/* TODO: Fix JSX expression */}
  o: 'pipe' }''      )
//       return {/* TODO: Fix JSX expression */}
      ...
      for (const file of conflictedFiles) {
        ...
        if (file.trim()) {
          ...
          try {
            ...
            execSync(`git checkout --theirs "${file}"`, { stdio: 'pipe' })'"'            execSync(`git add "${file}"`, { stdio: 'pipe' })'"'//             } catch (fileError) {"  
      for (const file of conflictedFiles) {/* TODO: Fix JSX expression */}"`""            execSync(`git checkout --theirs "${file}"`, {/* TODO: Fix JSX expression */})""  o: 'pipe' });"`"'"'            execSync(`git add "${file}"`, {/* TODO: Fix JSX expression */})""  o: 'pipe' })''//             } catch (fileError) {/* TODO: Fix JSX expression */}
//             }
        }
      }

      //Complete the merge
      execSync(`git commit -m "Manual conflict resolution for ${branchName}"`, {)""        stdio: 'pipe'),''      //Complete the merge;"`""      execSync(`git commit -m "Manual conflict resolution for ${branchName}"`, {/* TODO: Fix JSX expression */})""      })
//       return {/* TODO: Fix JSX expression */}
