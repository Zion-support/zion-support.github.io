...
class $1 {
  ...
  totalScripts: scriptFiles.length;"fixedScripts": this.fixedCount;runScripts: results.length;"successfulRuns": results.filter(r => r.success).length;failedRuns: results.filter(r => !r.success).length,;};"results": results;errors: this.errors,;}
