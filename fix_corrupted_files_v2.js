...
function isCorrupted(content) {
  ...
  // Check if any line matches corruption patterns
  for (const line of lines) {
    if (corruptionPatterns.some(pattern => pattern.test(line))) {
      return true;
    }
  }
  
  // Check for incomplete component structure
  if (lines.length < 10) {
    return true;
  }
  
  // Check if file starts with incomplete content
  const firstNonEmptyLine = lines.find(line => line.trim().length > 0);
  if (firstNonEmptyLine && (
    firstNonEmptyLine.trim().startsWith('}') ||
    firstNonEmptyLine.trim().startsWith(']') ||
    firstNonEmptyLine.trim().startsWith(')') ||
    firstNonEmptyLine.trim().startsWith('return (') ||
    firstNonEmptyLine.trim().startsWith('<>') ||
    firstNonEmptyLine.trim().startsWith('</>')
  )) {
    return true;
  ...
  if (content.includes('className =') || content.includes('whileHover =') || content.includes('whileTap =')) {
    return true;
  }
  
  // Check if file has incomplete imports
  if (content.includes('import React from \'react\';";";";";')) {
    return true;
  }
  
  // Check if file has incomplete return statements
