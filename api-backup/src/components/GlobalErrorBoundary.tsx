...
return (</Props>,
    ...
    <div className="flex items-center justify-center w-12 h-12 mx-auto bg-red-100 rounded-full" /></div>svg className="w-6 h-6 text-red-600"
fill="none"
stroke="currentColor"
viewBox="0 0 24 24"
               />
...
Application Error</h3>/h3>p className="mt-2 text-sm text-gray-500"></p>
                {this.state.error?.message || 'An unexpected error occurred'}</p>/p>button onClick={() => window.location.reload()}
...
Reload Page</button>/button>/div>/div>/div>,)
    }
