...
import { format  } from 'date - fns';import { use_auth  } from '@/hooks / use_auth';
import { use_projects  } from '@/hooks / use_projects';import { SEO  } from '@/components / SEO';
import { ProtectedRoute  } from '@/components / ProtectedRoute';import { Project, ProjectStatus  } from '@/types / projects';
import { Button  } from '@/components / ui / button';import { logErrorToProduction } from '@/utils / production_logger'; import Link from 'next / link';
import { format   } from './date - fns';
import { use_auth   } from '@/hooks / use_auth';import { use_projects   } from '@/hooks / use_projects';
import { SEO   } from '@/components / SEO';import { ProtectedRoute   } from '@/components / ProtectedRoute';
import { Project, ProjectStatus   } from '@/types / projects';import { Button   } from '@/components / ui / button';
import { logErrorToProduction } from '@/utils / production_logger';import { Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
...
import { Tabs, TabsContent, TabsList, TabsTrigger  } from '@/components / ui / tabs';AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
...
import { toast  } from '@/hooks / use - toast';import { supabase  } from '@/integrations / supabase / client';
import { ProjectReviewSection  } from '@/components / projects / reviews / ProjectReviewSection';AlertCircle,
  Calendar,
  CheckCircle2,
  Clock,
  FileText,
...
import { logErrorToProduction } from '@/utils/productionLogger';import { Card
  CardContent
  CardDescription
  CardFooter
  CardHeader
...
>
          ...
          <div className="order-1 lg:order-2 lg:col-span-1">
            ...
            <Card></Card>
                    ...
                    <Avatar className='h-10 w-10'></Avatar>
                      ...
                      <div className='text - center py - 8'></div>
                        ...
                        <p className='text - sm text - muted - foreground'></p>
                          No documents have been uploaded to this project.</$1>
                </$1>)}
                </CardContent>
                </$1></$1>
              <TabsContent value='notes'></TabsContent>
                <Card></Card>
                  ...
                  <CardContent></CardContent>
                    <div className='space - y-4'></div>
                      <div className='space - y-4 max - h-[400px] overflow - y-auto mb - 4'></div>
                {notes.length > 0 ? (
                          notes.map (note => (
                            <;$2 />
                              key={note.id}
                ...
                {note.content}
                          ...
                          <div className='text - center py - 8'></div>
                            <MessageSquare className='h - 8 w - 8 text - muted - foreground mx - auto mb - 2' />
                            <p className='text - muted - foreground'></p>
                              No notes yet. Add the first note to this project.</$1>
                </$1>)}
                </div>
                {isOfferAccepted && (
                        <div></div>
              ...
              <TabsContent value='reviews'>
                <ProjectReviewSection project={project} /></$1></$1></$1>
          <div className='order - 1 lg:order - 2 lg:col - span - 1'>
            <Card>
              <CardHeader>
          ...
          <div className='order-1 lg:order-2 lg:col-span-1'></div>
            <Card></Card>
              <CardHeader></CardHeader>
                <CardTitle>Project Participants</CardTitle>
                </$1>
...
>
                ...
                {isClient && (
                  ...
                  <div className='flex items-start gap-4'></div>
                    <Avatar className='h-10 w-10'></Avatar>
                        <Button variant='outline' size='sm' as_child></Button>
                          <;$2 />
                            href={project.agreement_url}
                            target='_blank'
                            rel='noopener noreferrer'
...
>
  ...
  if (!project) {
    return (
      <div className="container mx-auto py-8"></div>
        <Card></Card>
          <CardContent className="flex flex-col items-center justify-center py-10"></CardContent>
            <AlertCircle className="h-10 w-10 text-muted-foreground mb-4" />
            <h2 className="text-xl font-bold mb-2">Project Not Found</h2>
            <p className="text-muted-foreground mb-4"></p>
              The project you're looking for doesn't exist or you don't have access to it.</$1>
    ...
    <>
      ...
      <main className="container mx-auto px-4 py-8"></main>
        <div className="mb-6"></div>
          <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-2"></div>
            <div></div>
              ...
              <div className="flex items-center gap-2 mt-1"></div>
                ...
                <>
                  <AlertDialog></AlertDialog>
                    <AlertDialogTrigger asChild></AlertDialogTrigger>
                      <Button variant="default"></Button>
                        <CheckCircle2 className="mr-2 h-4 w-4" /> Accept Offer</$1>
                </$1>
                    <AlertDialogContent></AlertDialogContent>
                      <AlertDialogHeader></AlertDialogHeader>
                        ...
                        <AlertDialogDescription></AlertDialogDescription>
                          ...
                          This will initiate the contract and start the project.</$1>
                </$1>
                  ...
                  <Button variant="outline" onClick={() => handleStatusChange("changes_requested")}>
                    <MessageSquare className="mr-2 h-4 w-4" /> Request Changes</$1>
                </>
              )}
              ...
              {(isClient || isTalent) && project.status === "in_progress" && (
                <AlertDialog></AlertDialog>
                  <AlertDialogTrigger asChild></AlertDialogTrigger>
                    <Button variant="default"></Button>
                      <CheckCircle2 className="mr-2 h-4 w-4" /> Mark as Completed</$1>
                </$1>
                  <AlertDialogContent></AlertDialogContent>
                    <AlertDialogHeader></AlertDialogHeader>
                      ...
                      <AlertDialogDescription></AlertDialogDescription>
                        ...
                        Make sure all deliverables have been provided and approved.</$1>
                </$1>
                    <AlertDialogFooter></AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
              ...
              {isActiveProject && (
                <Button variant="default" asChild></Button>
                  <Link href={`/project/[id]/milestones`}></Link>
                    <Layers className="mr-2 h-4 w-4" /> Milestones</$1>
                </$1>
              )}

              {isActiveProject && (
                <Button variant="outline" asChild></Button>
                  <Link href={`/project/[id]/room`}></Link>
                    <Video className="mr-2 h-4 w-4" /> Project Room</$1>
                </$1>
              )}
              ...
              {(isClient || isTalent) && ["offer_sent", "offer_accepted", "in_progress"].includes(project.status) && (
                ...
                >
                  <MessageSquare className="mr-2 h-4 w-4" /> Message</$1>
              )}
                </div>
                </$1></$1>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8"></div>
          <div className="order-2 lg:order-1 lg:col-span-2"></div>
            <Tabs defaultValue="details" value={activeTab} onValueChange={setActiveTab}></Tabs>
              ...
              <TabsContent value="details"></TabsContent>
                <Card></Card>
                  <CardHeader></CardHeader>
                    ...
                    <CardDescription></CardDescription>
                      Project details and expectations</$1>
                </$1>
                  <CardContent></CardContent>
                    <div className="space-y-4"></div>
                      <div></div>
                        <h3 className="font-semibold mb-2">Project Description</h3>
                        <div className="bg-muted/30 p-4 rounded-md"></div>
                ...
                {project.payment_terms} Payment</$1>
                </$1>
                      <div></div>
              ...
              <TabsContent value="timeline"></TabsContent>
                <Card></Card>
                  <CardHeader></CardHeader>
                    ...
                    <CardDescription></CardDescription>
                      Key dates and milestones</$1>
                </$1>
                  <CardContent></CardContent>
                    <div className="space-y-4"></div>
                      <div className="flex items-start gap-3 p-3 bg-muted/30 rounded-md"></div>
              ...
              <TabsContent value="documents"></TabsContent>
                <Card></Card>
                  <CardHeader></CardHeader>
                    ...
                    <CardDescription></CardDescription>
                      Agreements and relevant files</$1>
                </$1>
                  <CardContent></CardContent>
                {project.agreement_url ? (
                      <div className="flex items-center justify-between bg-muted/30 p-4 rounded-md"></div>
                        <div className="flex items-center gap-3"></div>
                          <FileText className="h-5 w-5 text-primary" />
                          <div></div>
                            <h3 className="font-semibold">Project Agreement</h3>
                            <p className="text-sm text-muted-foreground"></p>
                              Uploaded when project was created</$1>
                </$1></$1>
                        <Button variant="outline" size="sm" asChild></Button>
                          <a href={project.agreement_url} target="_blank" rel="noopener noreferrer"></a>
                            View</$1>
                </$1></$1>
                    ) : (
                      <div className="text-center py-8"></div>
              ...
              <TabsContent value="notes"></TabsContent>
                ...
                {notes.length > 0 ? (
                          notes.map((note) => (
                            <div key={note.id} className="bg-muted/30 p-3 rounded-md"></div>
                              <div className="flex items-center gap-2 mb-2"></div>
                                <Avatar className="h-6 w-6"></Avatar>
                {note.created_by_profile?.avatar_url ? (
                                    <img
...
>
              ...
              {project.status === "changes_requested" && isClient && (
                <CardFooter className="flex-col items-start gap-2 border-t pt-6"></CardFooter>
                  <p className="text-sm text-amber-600 flex items-center gap-1"></p>
              ...
              {project.status === 'completed' && (
                <CardFooter className='flex-col items-start gap-2 border-t pt-6'></CardFooter>
                  <p className='text-sm text-green-600 flex items-center gap-1'></p>
              ...
              {project && project.status === 'offer_sent' && isClient && (
                <CardFooter className='flex-col items-start gap-2 border-t pt-6'></CardFooter>
                  <p className='text-sm text-muted-foreground'></p>
                    Waiting for the talent to accept your offer.</$1>
                </$1>
              )}
              ...
              {project && project.status === 'canceled' && (
                <CardFooter className='flex-col items-start gap-2 border-t pt-6'></CardFooter>
                  <p className='text-sm text-red-600 flex items-center gap-1'></p>
...
>
                    ...
                    className='w - full'                  >
                    <MessageSquare className='mr - 2 h - 4 w - 4' /> Discuss Changes</$1>
                </$1>)}
...
<p>Loading project details...</p>
                ...
                </div> <Card> <CardContent className="flex flex-col items-center justify-center py-10" > <AlertCircle className="h-10 w-10 text-muted-foreground mb-4" /> <h2 className="text-xl font-bold mb-2" >Project Not Found</h2> <p className="text-muted-foreground mb-4" > The project you're looking for doesn't exist or you don't have access to it. </p> <ButtononClick={"
  () => router && router.push ("/dashboard") 
}> Return to Dashboard </Button>
                </CardContent> </Card>
                </div>) "
}//Check if user is either the client or the talent container mx-auto px-4 py-8"> <div className=" mb-6"> <div className=" flex flex-col md:flex-row justify-between md:items-center gap-4 mb-2"> <div> </span>
                </div> </div> <AlertDialog> <AlertDialogTrigger asChild> <Button variant=" default"> <CheckCircle2 className=" mr-2 h-4 w-4"/> Accept Offer </Button>
                </AlertDialogTrigger> <AlertDialogContent> <AlertDialogHeader> <AlertDialogTitle>Accept Project Offer?</AlertDialogTitle> <AlertDialogDescription> By accepting this offer, you agree to the project terms and timeline. This will initiate the contract and start the project. </AlertDialogDescription>
                </AlertDialogHeader> <AlertDialogFooter> <AlertDialogCancel>Cancel</AlertDialogCancel> <AlertDialogActiononClick={"
  () => handleStatusChange (" offer accepted") 
}> Accept Offer </AlertDialogAction>
                </AlertDialogFooter> </AlertDialogContent>
                </AlertDialog> </Button> </>) "
}
                <AlertDialogTrigger asChild> <Button variant=" default"> <CheckCircle2 className=" mr-2 h-4 w-4"/> Mark as Completed </Button>
                </AlertDialogTrigger> <AlertDialogContent> <AlertDialogHeader> <AlertDialogTitle>Mark Project as Completed?</AlertDialogTitle> <AlertDialogDescription> This will finalize the project and mark it as complete. Make sure all deliverables have been provided and approved. </AlertDialogDescription>
                </AlertDialogHeader> <AlertDialogFooter> <AlertDialogCancel>Cancel</AlertDialogCancel> <AlertDialogActiononClick={"
  () => handleStatusChange (" completed") 
}> Mark as Completed </AlertDialogAction>
                </AlertDialogFooter> </AlertDialogContent>
                </AlertDialog>) 
}
                <href= {$2 />
  `/project/[id]/milestones` "
}> <Layers className=" mr-2 h-4 w-4"/> Milestones </Link>
                </Button>) 
}
                <href= {$2 />
  `/project/[id]/room` "
}> <Video className=" mr-2 h-4 w-4"/> Project Room </Link>
                </Button>) "
                ...
                </TabsList> <TabsContent value=" details"> <Card> <CardHeader> <CardTitle>Project Scope</CardTitle> <CardDescription> Project details and expectations </CardDescription>
                </CardHeader> <CardContent> <div className=" space-y-4"> <div> </div>
                </div> <div> </Badge>
                </div> <div> </div>
                </div> </div>
                </CardContent> </Card>
                </TabsContent> <TabsContent value=" timeline"> <Card> <CardHeader> <CardTitle>Project Timeline</CardTitle> <CardDescription> Key dates and milestones </CardDescription>
                </CardHeader> <CardContent> <div className=" space-y-4"> <div className=" flex items-start gap-3 p-3 bg-muted/30 rounded-md"> <Calendar className=" h-5 w-5 text-primary mt-0 && 0.5"/> <div> </div>
                </div> <div className=" flex items-start gap-3 p-3 bg-muted/30 rounded-md"> <Clock className=" h-5 w-5 text-primary mt-0 && 0.5"/> <div> </div>
                </div> </div>
                </div> </CardContent>
                </Card> </TabsContent> <TabsContent value=" documents"> <Card> <CardHeader> <CardTitle>Project Documents</CardTitle> <CardDescription> Agreements and relevant files </CardDescription>
                </CardHeader> <CardContent> <div> <h3 className=" font-semibold">Project Agreement</h3> <p className=" text-sm text-muted-foreground"> Uploaded when project was created </p>
                </div> </div> <Button variant=" outline"size=" sm"asChild> <href= {$2 />
  project && project.agreement url "
}target=" blank"rel=" noopener noreferrer"> View </Link>
                </Button> </div>) : (<div className=" text-center py-8"> <FileText className=" h-10 w-10 text-muted-foreground mx-auto mb-2"/> <h3 className=" font-semibold">No Documents Yet</h3> <p className=" text-sm text-muted-foreground"> No documents have been uploaded to this project. </p>
                </div>) "
}
                </CardContent>
                </Card> </TabsContent> <TabsContent value=" notes"> <Card> <CardHeader> <CardTitle>Project Notes</CardTitle> <CardDescription> Shared notes and updates </CardDescription>
                </CardHeader> <CardContent> />) : (<User className=" h-4 w-4"/>) "
}
                </Avatar>
                </div>) ) ) : (<div className=" text-center py-8"> <MessageSquare className=" h-8 w-8 text-muted-foreground mx-auto mb-2"/> <p className=" text-muted-foreground"> No notes yet. Add the first note to this project. </p>
                </div>) 
}
                </div>
                {"
  ...
  !newNote && newNote.trim () || isSubmittingNote 
                ...
                </Tabs> </div> <div className=" order-1 lg:order-2 lg:col-span-1"> <Card> <CardHeader> <CardTitle>Project Participants</CardTitle>
                </CardHeader> <CardContent> />) : (<User className=" h-6 w-6"/>) 
}
                </Avatar> <div> onClick={
  ...
  project && project.client id 
                ...
                </CardContent> </Card> <CardHeader> <CardTitle>Project Status</CardTitle>
                </CardHeader> <CardContent> </span>
                </div> </div>
                </CardContent> </p> <Buttonvariant=" outline"onClick={
  () => router && router.push (`/messages?talentId=$ {
  project && project.talent id 
}`) "
}className=" w-full"> <MessageSquare className=" mr-2 h-4 w-4" /> Discuss Changes </Button>
                </CardFooter>) 
  ...
  return (
    <>
      <Head></Head>
        ...
        <meta name="description" content="Professional Project Details services"  /></$1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm: px-6 lg:px-8 py-12"></div>
          <div className="text-center"></div>
            ...
            <p className="text-xl text-gray-600 mb-12"></p>
              Professional Project Details services and solutions</$1>
