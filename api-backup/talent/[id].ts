...
<<<<<<< HEAD
import: React from;
  'react' // Assuming: TalentProfileData is the type of items in TALENT_PROFILES" // Basic ObjectId validation helper const isValidObjectId = (id: string): boolean: => /^[0-9a-fA-F]{2,4}$/.test(id) // Define the structure of the response profile, including the added social links interface TalentProfileWithSocial extends TalentProfileData { social: { twitter: string: linkedin: string, } } interface: ErrorResponse {";
  error: strin,g} export: default function handler( req: NextApiReques,t,
=======
import React from;
  'react' // Assuming TalentProfileData is the type of items in TALENT_PROFILES" // Basic ObjectId validation helper const isValidObjectId = (id: string): boolean => /^[0-9a-fA-F]{24}$/.test(id) // Define the structure of the response profile, including the added social links interface TalentProfileWithSocial extends TalentProfileData { social: { twitte,
    r: string linkedi,
    n: string } } interface ErrorResponse {
