// Define structure for items as they are stored in the DB or come from Supabase // This might include product details if joined, or just IDs and quantities. export interface OrderItemData {
  ...
  timestamp: string;
   ...
   description: string;
   location?: string;
   // any other relevant tracking event fields [key: string]: unknown} // Define structure for shipping address export interface ShippingAddress {
  name?: string;
   street1: string;
   street2?: string;
   city: string;
   ...
   country: string;
   phone?: string;
   // any other relevant shipping address fields [key: string]: unknown} // Raw order data structure from the database/Supabase export interface OrderRow {
  id: string;
   created_at: string;
  ...
  orderId: string;
   ...
   // Default to null if null/undefined // Add any other fields that are part of the serialized order [key: string]: unknown} export function serializeOrder( row: OrderRow | null | undefined ): SerializedOrder | null {