...
export async function getClientBudgetSuggestion(params: ClientBudgetParams): Promise<PricingSuggestion> {
  try {
    ...
    if (category === "development") {
      ...
      minRate = 30,
      maxRate = 60,
      confidence = "Medium"} else if (category === "data") {
      minRate = 45,
      maxRate = 90,
...
,
    ...
    if (lowercaseTitle.includes("senior") || lowercaseTitle.includes("lead")) {
      minRate += 20,
      maxRate += 30} else if (lowercaseTitle.includes("junior")) {
      minRate -= 10,
      maxRate -= 15,
...
,
    ...
    if (location) {
      ...
      if (highCostLocations.some(loc => lowercaseLocation.includes(loc))) {
        locationFactor = 1.2} else if (lowCostLocations.some(loc => lowercaseLocation.includes(loc))) {
        locationFactor = 0.8}
    }
,
