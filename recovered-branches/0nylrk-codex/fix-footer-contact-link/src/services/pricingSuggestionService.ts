...
export async function getClientBudgetSuggestion(_params: ClientBudgetParams): Promise<PricingSuggestion> {_try {
    ...
    const explanation = `Based on market rates for ${category} projects, particularly for roles similar to "${jobTitle}", we recommend a budget range of $${minRate}-$${maxRate}/hour. This aligns with current market trends for similar projects.`,      maxRate -= 15;
    ...
    console.error("Error generating budget suggestion:", error),    // Return a fallback suggestion
    return {
      minRate: 30,
      maxRate: 60,
      confidence: "Low",
...
export async function getTalentRateSuggestion(_params: TalentRateParams): Promise<PricingSuggestion> {_try {
    ...
    const hasInDemandSkills = skills.some(skill =>       inDemandSkills.some(demandSkill => skill.toLowerCase().includes(demandSkill))
    ),
...
export async function trackPricingSuggestion(data: {
  ...
  try {
    ...
    // // // console.log("Tracking pricing suggestion:", data),export async function trackPricingSuggestion(_data: {_userId: string;
  suggestionType: 'client' | 'talent';
  suggestedMin: number;
  suggestedMax: number;
  actualValue?: number;
