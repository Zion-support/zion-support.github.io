...
import { useNavigate } from 'react-router-dom',export default function ReferralsPage() {
...
export default function ReferralsPage() {const navigate = useNavigate();
  const { isAuthenticated} = useAuth();
  ...
  useEffect_(() => {if (!isAuthenticated) {
      toast({
        ...
        variant: "destructive"}),
      navigate("/login")        title: "Authentication required", description: "Please login to access the referral program", variant: "destructive"});
      navigate("/login")
    }
