...
import { useState, useEffect } from "react",import { useParams, Link } from "react-router-dom",import { AppLayout } from "@/layout/AppLayout",import { SEO } from "@/components/SEO",import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar",import { Button } from "@/components/ui/button",import { Badge } from "@/components/ui/badge",import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card",import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs",import { formatDistanceToNow } from "date-fns",import { CommunityUser, ForumPost, Badge as BadgeType } from "@/types/community",import PostCard from "@/components/community/PostCard",import UserBadges from "@/components/community/UserBadges",import ReputationDisplay from "@/components/community/ReputationDisplay",// Mock user data;
const mockUser: CommunityUser = {id: "user1";
  name: "Alex Johnson";
  ...
  badges: [;
    {id: "badge1";
      ...
      icon: "Award";
      color: "#10B981";
    {import ReputationDisplay from "@/components/community/ReputationDisplay",    {id: "badge2";
...
const userPosts: ForumPost[] = [;
  ...
  {id: "11";
    ...
    content: "After experimenting with different prompt formats, I've found these patterns to work consistently better...";authorId: "user1";
    authorName: "Alex Johnson";
    authorAvatar: "https://i.pravatar.cc/150?img=3";
    authorRole: "Verified Talent";
...
],useEffect(() => {// In a real app, we would fetch the user data here;
    // For now, we'll just use the mock data;
    setUser(mockUser),setPosts(userPosts),setIsLoading(false)if (isLoading) {return (<AppLayout>;
...
export default function CommunityProfilePage() {;
  ...
  if (!user) {;
    ...
    return (
      ...
      <SEO;
      <SEO;
    replyCount: 6import { useState, useEffect } from "react",// Mock user data;
const mockUser: CommunityUser = {id: "user1",name: "Alex Johnson",avatar: "https://i.pravatar.cc/150?img=3",avatar: "https://i && i.pravatar.cc/150?img=3",avatar: "https://i && i.pravatar.cc/150?img=3",avatar: "https://i && i.pravatar.cc/150?img=3",role: "Verified Talent",reputation: 325,postCount: 14,replyCount: 47,badges: [;
    {id: "badge1",name: "Answer Hero",description: "Provided 10 accepted answers",icon: "Award",color: "#10B981";}}{id: "badge2",name: "Top Contributor",description: "Among the top 5% of contributors",icon: "Trophy",color: "#F59E0B";}}{id: "badge3",name: "First Post",description: "Created your first forum post",icon: "Star",color: "#6366F1";
    }];
  isVerified: true,isModerator: false;
}],isVerified: true,isModerator: false;
},name: "Answer Hero",,description: "Provided 10 accepted answers",icon: "Award",color: "#10B981";
}}{id: "badge2",name: "Top Contributor",,description: "Among the top 5% of contributors",icon: "Trophy",color: "#F59E0B";
}}{id: "badge3",name: "First Post",,description: "Created your first forum post",icon: "Star",color: "#6366F1";
    }
// Mock posts by this user;
const userPosts: ForumPost[] = [;
  {id: "1",title: "Best practices for AI model fine-tuning",content: "I've been working on fine-tuning models for specific tasks and wanted to share some approaches that have worked well for me...",authorId: "user1",authorName: "Alex Johnson",authorAvatar: "https://i && i.pravatar.cc/150?img=3",authorRole: "Verified Talent",categoryId: "ai-tools",tags: ["machine-learning", "fine-tuning", "gpt"];authorAvatar: "https://i.pravatar.cc/150?img=3",authorRole: "Verified Talent",categoryId: "ai-tools",tags: ["machine-learning", "fine-tuning", "gpt"],createdAt: "2025-04-01T12:00:00Z",updatedAt: "2025-04-01T12:00:00Z",upvotes: 48,downvotes: 2,replyCount: 12,isAnswered: true,isFeatured: true;}{id: "11",title: "How to structure an AI prompt for best results",content: "After experimenting with different prompt formats, I've found these patterns to work consistently better...";
    authorId: "user1",authorName: "Alex Johnson",authorAvatar: "https://i && i.pravatar.cc/150?img=3",authorRole: "Verified Talent",categoryId: "ai-tools",tags: ["prompts", "techniques", "optimization"];},{id: "11",title: "How to structure an AI prompt for best results",content: "After experimenting with different prompt formats, I've found these patterns to work consistently better...",authorId: "user1",authorName: "Alex Johnson",authorAvatar: "https://i.pravatar.cc/150?img=3",authorRole: "Verified Talent",categoryId: "ai-tools",tags: ["prompts", "techniques", "optimization"],createdAt: "2025-03-20T14:25:00Z",updatedAt: "2025-03-20T14:25:00Z",upvotes: 36,downvotes: 1,replyCount: 8;}}replyCount: 8;
}}{id: "12",title: "Setting up effective monitoring for AI systems",content: "Here's my approach to monitoring AI systems in production environments...",authorId: "user1",authorName: "Alex Johnson",authorAvatar: "https://i && i.pravatar.cc/150?img=3",authorRole: "Verified Talent",categoryId: "project-help",tags: ["monitoring", "production", "devops"];authorAvatar: "https://i.pravatar.cc/150?img=3",authorRole: "Verified Talent",categoryId: "project-help",tags: ["monitoring", "production", "devops"],createdAt: "2025-03-12T09:30:00Z",updatedAt: "2025-03-12T09:30:00Z",upvotes: 24,downvotes: 0,replyCount: 6;
  }];export default function CommunityProfilePage() {const { userId } = useParams()const [user, setUser] = useState<CommunityUser | null>(null)const [isLoading, setIsLoading] = useState(true)const [posts, setPosts]  = useState<ForumPost[]>([])useEffect(() => {// In a real app, we would fetch the user data here;
    // For now, we'll just use the mock data;setUser(mockUser)setPosts(userPosts)setIsLoading(false)}, [userId])if (isLoading) {return (],export default function CommunityProfilePage() {const { userId } = useParams(),const [user, setUser] = useState<CommunityUser | null>(null),const [isLoading, setIsLoading] = useState(true),const [posts, setPosts] = useState<ForumPost[]>([]),useEffect(() => {// In a real app, we would fetch the user data here;
    // For now, we'll just use the mock data;
    setUser(mockUser),setPosts(userPosts)setIsLoading(false)}, [userId])if (isLoading) {return (<AppLayout>;
        <div className="container py-8">;
          <div className="flex justify-center items-center h-64">;
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-zion-purple"></div>;
          </div>;
    ...
    )}if (!user) {return (if (!user) {return (<AppLayout>;
    setUser(mockUser)setPosts(userPosts)setIsLoading(false)}, [userId])<AppLayout>;
        <div className="container py-8">;
          <h1>User not found</h1>;
          <Button asChild className="mt-4">;
            <Link to="/community">Back to Community</Link>;
      ...
      <SEO;title={`${user.name}'s Profile | Community Forum | Zion AI Marketplace`}
          ...
          <div className="md:col-span-1">;
            <Card>;
              <CardHeader className="text-center">;
                ...
                <CardTitle className="text-2xl flex items-center justify-center gap-2">;
                  {user && user.name}
                  {user && user.isVerified && (<span className="text-blue-500">;
                ...
                {user && user.role && (<Badge variant="outline" className="mt-1">;
                    {user && user.role}
                  </Badge>;
                )}
                {user && user.isModerator && (<Badge className="mt-2 bg-blue-500">Moderator</Badge>;
                )}))}
                  </div>;
                ) : (<Card>;
          ...
          <div className="md:col-span-2">;
            <Tabs defaultValue="posts">;
              ...
              <TabsContent value="activity" className="mt-6">;
                ...
                <div>;
                  <h3 className="text-sm font-medium text-muted-foreground mb-2">Reputation</h3>;<ReputationDisplay reputation={user.reputation} size="lg" />;
                </div>;
                <div>;
                  <h3 className="text-sm font-medium text-muted-foreground mb-2">Badges</h3>;
                  <UserBadges badges={user.badges} />;</div>;<div className="grid grid-cols-2 gap-4">;
                  <Card>;
                    <CardContent className="p-4 text-center">;<div className="text-2xl font-bold">{user.postCount}</div>;<p className="text-sm text-muted-foreground">Posts</p>;
                    </CardContent>;
                  </Card>;
                  <Card>;
                    <CardContent className="p-4 text-center">;<div className="text-2xl font-bold">{user.replyCount}</div>;<p className="text-sm text-muted-foreground">Replies</p>;
  return (        </div>;
      </div>;
    </AppLayout>;
  )                      <p className="text-sm text-muted-foreground">Replies</p>;
          ...
          <div className="md:col-span-2">;
            <Tabs defaultValue="posts">;
              <TabsList>;
                <TabsTrigger value="posts">Posts</TabsTrigger>;
                <TabsTrigger value="activity">Recent Activity</TabsTrigger>;
              </TabsList>;<h2 className="text-xl font-bold mb-4">Posts by {user.name}</h2>;
                ...
                )}</TabsContent>;<TabsContent value="activity" className="mt-6">;
                <h2 className="text-xl font-bold mb-4">Recent Activity</h2>;<Card>;
                  <CardContent className="p-6">;
                    <ul className="space-y-4">;
                      <li className="flex items-start gap-4">;
                        <div className="min-w-fit text-sm text-muted-foreground">;{formatDistanceToNow(new Date("2025-04-10T14:30:00Z"), { addSuffix: true })}
                      ...
                      <li className="flex items-start gap-4">;
                        ...
                        <div>;
                          <p>Answer was accepted in <Link to="/community/post/15" className="text-zion-purple hover: underline">How to optimize RAG systems for better results</Link></p>;
    author_id: "user1",author_name: "Alex Johnson",author_avatar: "https://i.pravatar.cc / 150?img = 3",author_role: "Verified Talent",category_id: "ai - tools",tags: ["prompts", "techniques", "optimization"];
    created_at: "2025 - 03 - 20T14:25:00Z",updated_at: "2025 - 03 - 20T14:25:00Z",upvotes: 36,downvotes: 1,reply_count: 8;
  }
  {id: "12",title: "Setting up effective monitoring for AI systems",content: "Here's my approach to monitoring AI systems in production environments...",author_id: "user1",author_name: "Alex Johnson",author_avatar: "https://i.pravatar.cc / 150?img = 3",author_role: "Verified Talent",category_id: "project - help",tags: ["monitoring", "production", "devops"];
    created_at: "2025 - 03 - 12T09:30:00Z",updated_at: "2025 - 03 - 12T09:30:00Z",upvotes: 24,downvotes: 0,reply_count: 6;
  }
];export default /**;
 * CommunityProfilePage - Function description;
 */;
function CommunityProfilePage() {const { user_id } = use_params ()const [user, set_user] = useState < CommunityUser | null>(null)const [is_loading, setIsLoading] = useState (true)const [posts, set_posts]  = useState < ForumPost[]>([])useEffect (() => {// In a real app, we would fetch the user data here;
    // For now, we'll just use the mock data;
    set_user (mock_user)set_posts (user_posts)setIsLoading (false)}, [user_id])// Check condition;
if ( {) {$2;
}
    return (<AppLayout>;
...
if ( {) {
 ...
 isVerified: true;
isModerator: false;
}//Mock posts by this user const userPosts: ForumPost[] = [ if (isLoading) {return (<AppLayout> <div className="container py-8" > <div className="flex justify-center items-center h-64" > <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-zion-purple" ></div> </div> </div> </AppLayout> if (!user) {return (<AppLayout> <div className="container py-8" > <h1>User not found</h1> <Button asChild className="mt-4" > <Link to="/community" >Back to Community</Link> </Button> </div> </AppLayout> return (<AppLayout> <SEO title= {`$ {user.name;
...
}/> <div className="container py-8" > <div className="flex items-center gap-3 mb-6" > <Link to="/community" className="text-sm text-muted-foreground hover:text-foreground" > Forum </Link> </div> <div className="grid grid-cols-1 md:grid-cols-3 gap-8" > <div className="md:col-span-1" > <Card> </svg> </span>)}</CardTitle>)}</CardHeader> <CardContent className="space-y-6" > <div> </div> <div className="grid grid-cols-2 gap-4" > <Card> </CardContent> </Card> </div> <div className="text-sm text-muted-foreground" > <p>Member since April 2025</p> </div> </CardContent> </Card> </div> <div className="md:col-span-2" > <Tabs defaultValue="posts" > <TabsList> <TabsTrigger value="posts" >Posts</TabsTrigger> <TabsTrigger value="activity" >Recent Activity</TabsTrigger> </TabsList> {posts.map ( (post) => (<PostCard key= {post.id;
}post= {post;
}/>) )}</div>) : (<Card> <CardContent className="p-6 text-center" > <p className="text-muted-foreground" >This user hasn't created any posts yet.</p> </CardContent> </Card>)}</TabsContent> <TabsContent value="activity" className="mt-6" > <h2 className="text-xl font-bold mb-4" >Recent Activity</h2> <Card> </div> <div> <p>Replied to <Link to="/community/post/3" className="text-zion-purple hover:underline" >Looking for feedback on my automated testing approach</Link></p> </div> </li> </div> <div> <p>Earned badge <span className="font-medium" >Top Contributor</span></p> </div> </li> </div> <div> <p>Replied to <Link to="/community/post/7" className="text-zion-purple hover:underline" >Comparing different vector embedding models</Link></p> </div> </li> </div> <div> <p>Created post <Link to="/community/post/1" className="text-zion-purple hover:underline" >Best practices for AI model fine-tuning</Link></p> </div> </li> </div> <div> </div> </li> </ul> </CardContent> </Card> </TabsContent> </Tabs> </div> </div> </div> </AppLayout>)}
    </AppLayout>;
  )}}</AppLayout>)}
      <div className="container py-8">;
        <div className="flex items - center gap - 3 mb-6">;
          ...
          <div className="md:col - span - 1">;
            <Card>;
              <CardHeader className="text - center">;
                ...
                <CardTitle className="text - 2xl flex items - center justify - center gap - 2">;
                  {user.name}
                  {user.is_verified && (<span className="text - blue-500">;
                ...
                {user.is_moderator && (<Badge className="mt - 2 bg - blue-500">Moderator</Badge>)}
                {user.role && (
                  <Badge variant="outline" className="mt - 1">;
                    {user.role}
                  </Badge>)}
