...
import { Tabs, TabsContent, TabsList, TabsTrigger  } from '@/components/ui/tabs';
  ...
  MessageSquare,FileText,Video,Calendar,Users,Settings,X;
  X;
} from "lucide-react";
import { VideoCallRoom  } from '@/components/video/VideoCallRoom';
import { toast  } from 'sonner';
export default function ProjectRoom() {const { projectId } = useParams() as { projectId: string }const [activeTab, setActiveTab]  = useState("chat")const [isInCall, setIsInCall]  = useState(false)const { projectId }  = useParams() as { projectId: string },const [activeTab, setActiveTab] = useState('chat')const [isInCall, setIsInCall] = useState(false)const [callParticipants, setCallParticipants] = useState<;
    Array<{id: string;
import React, { useState } from "react";
import { useParams } from "react-router-dom";
import { Header } from "@/components/Header";
...
export default function ProjectRoom() {
  ...
  ])const startVideoCall = () => {import { MessageSquare, FileText, Video, Calendar, Users, Settings, X } from 'lucide-react';
import { VideoCallRoom } from '@/components / video / VideoCallRoom';
import { toast } from 'sonner';
  const startVideoCall = () => {setIsInCall(true),toast.success("Video call started", {description: "Others can join with the project room link";
    })isMuted: false;
    }
  ])const startVideoCall = () => {import React, { useState } from 'react',import { useParams } from 'react-router-dom',import { Header } from '@/components/Header',import { Footer } from '@/components/Footer',import { SEO } from '@/components/SEO',import { Button } from '@/components/ui/button',import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card',import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs',import { MessageSquare, FileText, Video, Calendar, Users, Settings, X } from 'lucide-react',import { VideoCallRoom } from '@/components/video/VideoCallRoom',import { toast } from 'sonner',export default function ProjectRoom() {const { projectId } = useParams() as { projectId: string },const [activeTab, setActiveTab] = useState('chat'),const [isInCall, setIsInCall] = useState(false),const [callParticipants, setCallParticipants] = useState<Array<{id: string,name: string,avatar?: string,isMuted?: boolean,isVideoEnabled?: boolean,isScreenSharing?: boolean,isHost?: boolean;
  }>>([;
    {id: 'user-1',name: 'You',isHost: true,isVideoEnabled: true,isMuted: false;
    }
...
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components / ui / tabs';
    ...
    if (activeTab !== "video") {setActiveTab("video")}}const endVideoCall = () => {setIsInCall(false)toast.info("Video call ended", {description: "Call duration and participants will be logged";
    ...
    const mockUsers = [;
      { id: "user-2", name: "Alex Chen", isVideoEnabled: true, isMuted: false },{id: "user-3",name: "Taylor Kim",isVideoEnabled: false,isMuted: true;
        isMuted: true;
      },{id: "user-4",name: "Jordan Smith",isVideoEnabled: true,isMuted: false,isScreenSharing: true;
      }isScreenSharing: true;
      }
    ];const randomUser  = mockUsers[Math.floor(Math.random() * mockUsers.length)];if (!callParticipants.find((p) => p.id === randomUser.id)) {setCallParticipants((prev) => [...prev, randomUser])toast(`${randomUser.name} joined the call`)}
  }return (const endVideoCall = () => {setIsInCall(false),toast.info("Video call ended", {description: "Call duration and participants will be logged";
    })setCallParticipants(prev => [...prev, randomUser])toast(`${randomUser && randomUser.name} joined the call`)}]),const startVideoCall = () => {setIsInCall(true),toast.success("Video call started", {,description: "Others can join with the project room link";
    }),// Switch to video tab if not already there;
    ...
    const mockUsers = [;
      { id: 'user-2', name: 'Alex Chen', isVideoEnabled: true, isMuted: false },{ id: 'user-3', name: 'Taylor Kim', isVideoEnabled: false, isMuted: true },{ id: 'user-4', name: 'Jordan Smith', isVideoEnabled: true, isMuted: false, isScreenSharing: true }
    return (<>;
      <SEO;
    }>
  >([
                ...
                {isInCall ? (<div className="space-y-4">;
                    ...
                    <div className="flex justify-center mt-4">;
                      <Button;
                        ...
                        className="text-sm";
                      >;
                        Simulate user joining (demo only)</Button>;
                    </div>;
                  </div>;
        ...
        <Tabs value={active_tab} onValueChange={setActiveTab} className="space-y-4">;
          ...
          <TabsContent value="chat" className="space - y-4">;
            <Card>;
              ...
              <CardContent className="h-[400px]">;
                <div className="flex items - center justify - center h - full">;
                  <p className="text - muted - foreground">Chat functionality will be implemented soon</p>;</div>;
                <div className="flex items - center justify - center h-full">;
                  <p className="text - muted-foreground">Chat functionality will be implemented soon</p>;</CardContent>;
            </Card>;
          </TabsContent>;;
          <TabsContent value="team" className="space-y-4">;
            <Card>;
              ...
              <CardContent className="h-[400px]">;
                <div className="flex items-center justify-center h-full">;
                  <p className="text-muted-foreground">Team management will be implemented soon</p>;<TabsContent value="video" className="space-y-4">;
          ...
          <TabsContent value="settings" className="space - y-4">;
            <Card>;
              ...
              <CardContent className="h-[400px]">;
                <div className="flex items-center justify-center h-full">;
                  <p className="text-muted-foreground">Settings will be implemented soon</p>;<TabsContent value="settings" className="space-y-4">;
            <Card>;
              <CardHeader>;
                <CardTitle > Project Settings</CardTitle>;
                <CardDescription > Configure project parameters</CardDescription>;
