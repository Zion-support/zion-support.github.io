...
const roomId = `msg-${activeConversation.id}`,;`    setActiveCall(roomId),
      ...
      <div className="min-h-screen bg-zion-blue">"
        <div className="container mx-auto py-8 px-4">"
          <div className="flex justify-between items-center mb-6">"
            <h1 className={`text-${isMobile ? '2xl' : '3xl'} font-bold text-white flex items-center gap-2`}>`              <MessageSquare className="h-6 w-6" />"
              Messages
            </h1>
...
useEffect(() => {;
  ...
  const startVideoCall = () => {;
    ...
    const roomId = `msg-${activeConversation && activeConversation.id}`;`    setActiveCall(roomId);
...
if ( {) {
  $2
    ...
    const room_id = `msg-${active_conversation.id}`;`    setActiveCall (room_id);
;
    // Show toast notification;
    toast.success ("Starting video call", {"
      }
...
return (;
    <ProtectedRoute>;
      <div className="min - h-screen bg - zion-blue">;"
        <div className="container mx - auto py - 8 px-4">;"
          <div className="flex justify - between items - center mb-6">;"
            <h1 className={`text-${is_mobile ? '2xl' : '3xl'} font - bold text - white flex items - center gap - 2`}>;`              <MessageSquare className="h - 6 w-6" />;"
              Messages;
            </h1>;
            {active_conversation && (
              <Button;
          ...
          <div className="bg - zion - blue - light / 10 rounded - lg shadow - lg border border - zion - purple / 20 overflow-hidden">;"
            <div className={`flex flex - col "md":flex - row h-[${is_mobile ? '85vh' : '75vh'}]`}>;`              {/* Conversations List */}
              {is_loading ? (
                <div className="flex - 1 flex items - center justify - center p-8">;"
                  <div className="animate-pulse">Loading conversations...</div>;"
                </div>) : (
