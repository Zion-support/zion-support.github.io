...
export default function Blog() {_const [searchQuery, _setSearchQuery] = useState("");
  const [selectedCategory, _setSelectedCategory] = useState("All Categories");
  
  // Filter blog posts based on search and category
  const _filteredPosts = BLOG_POSTS.filter(post => {
    const _matchesSearch = 
      post.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
      post.excerpt.toLowerCase().includes(searchQuery.toLowerCase()) ||
      post.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()));
      
    const _matchesCategory = selectedCategory === "All Categories" || post.category === selectedCategory;
    
    return matchesSearch && matchesCategory;});
  
  // Get featured posts
  const _featuredPosts = BLOG_POSTS.filter(post => post.isFeatured);
    ...
    <>
      ...
      <div className="min-h-screen bg-zion-blue pt-12 pb-20 px-4">
        <div className="container mx-auto">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col justify-center">
                  <span className="text-sm text-zion-cyan bg-zion-blue-dark px-3 py-1 rounded-full inline-block mb-2">
                    {_featuredPosts[0].category}
                  </span>
                  <h3 className="text-3xl font-bold text-white mb-4">
                    {_featuredPosts[0].title}
                  </h3>
                  <p className="text-zion-slate-light mb-6">
                    {_featuredPosts[0].excerpt}
                  </p>
                  <div className="flex items-center mb-6">
                    <img 
                      src={_featuredPosts[0].author.avatarUrl} 
                      alt={_featuredPosts[0].author.name}
                      className="w-10 h-10 rounded-full mr-3"
                    />
                    <div>
                      <p className="text-white font-medium">{_featuredPosts[0].author.name}</p>
                      <p className="text-sm text-zion-slate-light">
                        {_featuredPosts[0].publishedDate} • {_featuredPosts[0].readTime}
                      </p>
                    </div>
                  </div>
                  <Button 
          ...
          {_/* Blog Posts Grid */}
          {_filteredPosts.length > 0 ? (_<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
