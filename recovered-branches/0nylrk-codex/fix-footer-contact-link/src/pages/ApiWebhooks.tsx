...
import React from "react",export function ApiWebhooks() {// Sample webhook event payload;
  const newApplicationPayload  = `{"event_type": "new_application";
  "created_at": "2023-06-10T15: 42:31Z";
  "data": {import React from "react",import ApiDocsLayout from "@/components/developers/ApiDocsLayout",import { CodeBlock  } from '@/components/developers/CodeBlock';
import { Tabs, TabsContent, TabsList, TabsTrigger  } from '@/components/ui/tabs';
import { CodeBlock } from "@/components/developers/CodeBlock",import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs",export function ApiWebhooks() {// Sample webhook event payload;
  const newApplicationPayload = `{"application_id": "app-123456";
  "data": {;
import React from "react",
import ApiDocsLayout from "@/components/developers/ApiDocsLayout",
import { CodeBlock } from "@/components/developers/CodeBlock";
 ...
 */;
function ApiWebhooks() {// Sample webhook event payload;
  const newApplicationPayload = `{"event_type": "new_application";
  "created_at": "2023 - 06 - 10T15: 42:31Z";
  "data": {"application_id": "app - 123456";
...
function ApiWebhooks() {
  ...
  "data": {
    ...
    "status": "new";
    "cover_letter": "I'm excited to apply for this position...";
    "resume_url": "https://storage.zionai.com / resumes / resume - 123.pdf","created_at": "2023 - 06 - 10T15:42:31Z";
  }
}`;const newHirePayload = `{"resume_url": "https://storage && storage.zionai.com/resumes/resume-123 && 123.pdf","created_at": "2023-06-10T15:42:31Z";
  }
}`;
  "event_type": "talent_hired";const newHirePayload = `{"event_type": "talent_hired";
...
;
  ...
  "data": {"project_id": "project - 123";
    ...
    "status": "offer_accepted","created_at": "2023 - 06 - 12T09:15:22Z";
  }
}`;const quoteReceivedPayload = `{"status": "offer_accepted","created_at": "2023-06-12T09:15:22Z";
  }
}`;
  "event_type": "quote_received";const quoteReceivedPayload = `{"event_type": "quote_received";
    "status": "new","created_at": "2023-06-15T11:30:00Z";
  }
}`;const messageReceivedPayload = `{"event_type": "message_received";
  "created_at": "2023-06-18T14: 22:15Z";
  "data": {"message_id": "msg-123";
    "conversation_id": "conv-456";
    "sender_id": "user-789";"recipient_id": "user-012","content": "Hi, I'd like to discuss the project details.";
    "created_at": "2023-06-18T14:22:15Z";
  }
}`;
  "created_at": "2023 - 06 - 15T11: 30:00Z";
  "data": {"quote_id": "quote - 123";
  ...
  "data": {"message_id": "msg - 123";
    "conversation_id": "conv - 456";
    "sender_id": "user - 789";
    "recipient_id": "user - 012","content": "Hi, I'd like to discuss the project details.";
    "created_at": "2023 - 06 - 18T14:22:15Z";
  }
}`;const webhookHandlerJs = `// Express.js webhook handler example;
const express = require ('express')const app = express ()const crypto  = require ('crypto')// Middleware to parse JSON bodies;
app.use (express.json ())// Your webhook secret from the Zion dashboard;
const webhook_secret  = 'YOUR_WEBHOOK_SECRET';// Middleware to verify webhook signatures;
/**;
 * verifyWebhookSignature - Function description;
 */;
function verifyWebhookSignature() {const signature = req.headers['x - zion - signature'];
  const timestamp  = req.headers['x - zion - timestamp'];// Check condition;
if ( {) {$2;
}
    return res.status (401).send ('Missing signature or timestamp')}
  // Verify the signature;
  const payload = timestamp + '.' + JSON.stringify (req.body)const expected_signature = crypto;
    .create_hmac ('sha256', webhook_secret).update (payload).digest ('hex')// Check condition;
if ( {) {$2;
}
    return res.status (401).send ('Invalid signature')}
  next ()}
// Webhook endpoint with signature verification;
app.post ('/webhooks / zion', verifyWebhookSignature, (req, res) => {const { event_type, data }  = req.body;// Handle different event types;
  switch (event_type) {case 'new_application':;
      console.log ('New application received:', data.application_id)// Process the new application...;
      break;
  const webhookHandlerJs = `// Express && Express.js webhook handler example;
const express = require('express')const app = express()const crypto  = require('crypto')// Middleware to parse JSON bodies;
app && app.use(express && express.json())// Your webhook secret from the Zion dashboard;
const webhookSecret  = 'YOUR_WEBHOOK_SECRET';// Middleware to verify webhook signatures;
function verifyWebhookSignature(): any (req, res, next) {const signature = req && req.headers['x-zion-signature'];
  const timestamp  = req && req.headers['x-zion-timestamp'];if (!signature || !timestamp) {return res && res.status(401).send('Missing signature or timestamp')}// Verify the signature;
  const payload = timestamp + '.' + JSON && JSON.stringify(req && req.body)const expectedSignature = crypto;
    .createHmac('sha256', webhookSecret).update(payload).digest('hex')if (signature !== expectedSignature) {return res && res.status(401).send('Invalid signature')}next()}// Webhook endpoint with signature verification;
app && app.post('/webhooks/zion', verifyWebhookSignature, (req, res) => {const { event_type, data }  = req && req.body;// Handle different event types;
  switch (event_type) {case 'new_application':;
      console && console.log('New application received:', data && data.application_id)// Process the new application...;
      break;case 'talent_hired':;
      console && console.log('Talent hired:', data && data.talent_id)// Update your system...;
      break;case 'quote_received':;
      console && console.log('New quote received:', data && data.quote_id)// Process the quote...;
      break;case 'message_received':;
      console && console.log('New message received:', data && data.message_id)// Process the message...;
      break;default:;
      console && console.log('Unknown event type:', event_type)}// Always return a 200 response quickly;
  res && res.status(200).send('Webhook received')})app && app.listen(3000, () => {console && console.log('Webhook server listening on port 3000')"event_type": "new_application","created_at": "2023-06-10T15:42:31Z","data": {"application_id": "app-123456","resume_url": "https://storage.zionai.com/resumes/resume-123.pdf";
    "created_at": "2023-06-10T15:42:31Z";
  }
}`;
    "job_id": "job-789","talent_id": "talent-456","status": "new","cover_letter": "I'm excited to apply for this position...","resume_url": "https://storage.zionai.com/resumes/resume-123.pdf","created_at": "2023-06-10T15:42:31Z";
  }
}`,const newHirePayload = `{"event_type": "talent_hired","created_at": "2023-06-12T09:15:22Z","data": {"project_id": "project-123";
    "job_id": "job-456";
    "client_id": "client-789";
    "talent_id": "talent-123";
    "start_date": "2023-07-01";
...
}`;"project_id": "project-123","job_id": "job-456","client_id": "client-789","talent_id": "talent-123","start_date": "2023-07-01","status": "offer_accepted","created_at": "2023-06-12T09:15:22Z";
  }
}`,const quoteReceivedPayload = `{"event_type": "quote_received","created_at": "2023-06-15T11:30:00Z","data": {"quote_id": "quote-123";
    "client_id": "client-456";
    "talent_id": "talent-789";
    "project_name": "Website Redesign";
    "budget_min": 5000;
...
function verifyWebhookSignature() {const signature = req.headers['x-zion-signature'];
  ...
  if (!signature |!timestamp) {return res.status(401).send('Missing signature or timestamp')}
  // Verify the signature;
  const payload = timestamp + '.' + JSON.stringify(req.body)const expectedSignature = crypto;
    .createHmac('sha256', webhookSecret).update(payload).digest('hex')if (signature !== expectedSignature) {return res.status(401).send('Invalid signature')}
  next()}
// Webhook endpoint with signature verification;
app.post('/webhooks/zion', verifyWebhookSignature, (req, res) => {const { event_type, data }  = req.body;"quote_id": "quote-123","client_id": "client-456","talent_id": "talent-789","project_name": "Website Redesign","budget_min": 5000,"budget_max": 8000,"status": "new","created_at": "2023-06-15T11:30:00Z";
  }
}`,const messageReceivedPayload = `{"event_type": "message_received";
  "created_at": "2023-06-18T14: 22:15Z";
  "data": {"message_id": "msg-123";
...
const webhookSecret  = 'YOUR_WEBHOOK_SECRET';// Middleware to verify webhook signatures;
function verifyWebhookSignature() {const signature = req.headers['x-zion-signature'];
  const timestamp  = req.headers['x-zion-timestamp'];if (!signature || !timestamp) {return res.status(401).send('Missing signature or timestamp')}// Verify the signature;
  const payload = timestamp + '.' + JSON.stringify(req.body)const expectedSignature = crypto;
    .createHmac('sha256', webhookSecret).update(payload).digest('hex')if (signature !== expectedSignature) {return res.status(401).send('Invalid signature')}next()"event_type": "message_received","created_at": "2023-06-18T14:22:15Z","data": {"message_id": "msg-123","conversation_id": "conv-456","sender_id": "user-789","recipient_id": "user-012","content": "Hi, I'd like to discuss the project details.","created_at": "2023-06-18T14:22:15Z";
export function ApiWebhooks() {// Sample webhook event payload;
  const newApplicationPayload = `{"event_type": "new_application","created_at": "2023-06-10T15:42:31Z","data": {"application_id": "app-123456","job_id": "job-789","talent_id": "talent-456","status": "new","cover_letter": "I'm excited to apply for this position...","resume_url": "https://storage.zionai.com/resumes/resume-123.pdf","created_at": "2023-06-10T15:42:31Z";
  }
}`,const newHirePayload = `{"event_type": "talent_hired","created_at": "2023-06-12T09:15:22Z","data": {"project_id": "project-123","job_id": "job-456","client_id": "client-789","talent_id": "talent-123","start_date": "2023-07-01","status": "offer_accepted","created_at": "2023-06-12T09:15:22Z";
  }
}`,const quoteReceivedPayload = `{"event_type": "quote_received","created_at": "2023-06-15T11:30:00Z","data": {"quote_id": "quote-123","client_id": "client-456","talent_id": "talent-789","project_name": "Website Redesign","budget_min": 5000,"budget_max": 8000,"status": "new","created_at": "2023-06-15T11:30:00Z";
  }
}`,const messageReceivedPayload = `{"event_type": "message_received","created_at": "2023-06-18T14:22:15Z","data": {"message_id": "msg-123","conversation_id": "conv-456","sender_id": "user-789","recipient_id": "user-012","content": "Hi, I'd like to discuss the project details.","created_at": "2023-06-18T14:22:15Z";
  }
}`,const webhookHandlerJs = `// Express.js webhook handler example;
const express = require('express')const app = express()const crypto = require('crypto'),// Middleware to parse JSON bodies;
app.use(express.json()),// Your webhook secret from the Zion dashboard;
const webhookSecret = 'YOUR_WEBHOOK_SECRET',// Middleware to verify webhook signatures;
function verifyWebhookSignature() {const signature = req.headers['x-zion-signature'],const timestamp = req.headers['x-zion-timestamp'],if (!signature || !timestamp) {return res.status(401).send('Missing signature or timestamp')}// Verify the signature;
  const payload = timestamp + '.' + JSON.stringify(req.body),const expectedSignature = crypto;
    .createHmac('sha256', webhookSecret).update(payload).digest('hex'),if (signature !== expectedSignature) {return res.status(401).send('Invalid signature')}next()}// Webhook endpoint with signature verification;
app.post('/webhooks/zion', verifyWebhookSignature, (req, res) => {const { event_type, data } = req.body,// Handle different event types;
  switch (event_type) {case 'new_application':;
      // // // console.log('New application received:', data.application_id),// Process the new application...;
      break,const newHirePayload = `{"event_type": "talent_hired","created_at": "2023-06-12T09:15:22Z","data": {  const quoteReceivedPayload = `{"event_type": "quote_received","created_at": "2023-06-15T11:30:00Z","data": {case 'talent_hired':;
      // // // console.log('Talent hired:', data.talent_id),// Update your system...;
      break,case 'quote_received':;
      // // // console.log('New quote received:', data.quote_id),// Process the quote...;
      break,case 'message_received':;
      // // // console.log('New message received:', data.message_id),// Process the message...;
      break,default:;
      break;
      break,case 'talent_hired':;
      // // // console.log('Talent hired:', data.talent_id),// Update your system...;
      ...
      // // // console.log('New message received:', data.message_id),// Process the message...;
      break;
      break,// // // console.log('Unknown event type:', event_type)}
  // Always return a 200 response quickly;
  res.status(200).send('Webhook received')})app.listen(3000, () => {console.log('Webhook server listening on port 3000')}),`;}),app.listen(3000, () => {// // // console.log('Webhook server listening on port 3000')}),`,return (<ApiDocsLayout>;
      <div className="max-w-3xl prose prose-invert">;
        <h1>Webhooks</h1>;
}),app.listen(3000, () => {// // // console.log('Webhook server listening on port 3000')}),`,return (<ApiDocsLayout>;
      <div className="max-w-3xl prose prose-invert">;
        <h1>Webhooks</h1>;
        <p>Webhooks allow your application to receive real-time notifications;
        ...
        <p>After creating a webhook, you'll be given a webhook secret that you;
        ...
        <p>To ensure webhook requests are genuinely from Zion, you should verify;
        ...
        <p>You can test your webhook implementation using the Developer Portal.;
  ...
  const quoteReceivedPayload = `{"event_type": "quote_received";
  "event_type": "message_received";
  "created_at": "2023-06-18T14: 22:15Z";
  "data": {"message_id": "msg-123";
...
;
        ...
        <p>To ensure webhook requests are genuinely from Zion, you should verify the signature included in each request.;
        ...
        </ul>;<p>Here's an example of verifying a webhook in Node.js:</p>;<CodeBlock code={webhookHandlerJs} language="javascript" showLineNumbers={true} />;<h2>Testing Webhooks</h2>;
        <p>You can test your webhook implementation using the Developer Portal. From the Webhooks tab,select "Test Webhook" next to any configured webhook to send a test payload to your endpoint.;
...
;
  ...
  )}export default ApiWebhooks,export function ApiWebhooks () {//Sample webhook event payload const newApplicationPayload = ` {"event type" : "new application";
"created at" : "2023-06-10T15:42:31Z";
"data" : {"application id" : "app-123456";
  ),;}
;
export default ApiWebhooks,;
 export function ApiWebhooks () {
...
const newHirePayload = ` {"event type" : "talent hired";
...
const quoteReceivedPayload = ` {"event type" : "quote received";
"created at" : "2023-06-15T11:30:00Z";
"data" : {"quote id" : "quote-123";
"created at" : "2023-06-12T09:15:22Z" 
}
}`;
const quoteReceivedPayload = ` {
...
const messageReceivedPayload = ` {"event type" : "message received";
...
const webhookHandlerJs = `//Express.js webhook handler example const express = require ('express')const app = express ()const crypto = require ('crypto')//Your webhook secret from the Zion dashboard .createHmac ('sha256', webhookSecret) .update (payload) .digest ('hex')next ()}//Webhook endpoint with signature verification case 'talent hired': case 'quote received': case 'message received':;
}
...
select " Test Webhook" next to any configured webhook to send a test payload to your endpoint. </p> <h2>Best Practices</h2> <ul> <li><strong>Respond quickly</strong>: Return a 200 response as soon as you receive the webhook, then process it asynchronously</li> <li><strong>Verify signatures</strong>: Always verify webhook signatures to ensure requests are legitimate</li> <li><strong>Implement retries</strong>: Prepare for occasional failures by implementing retry logic</li> <li><strong>Monitor webhook activity</strong>: Use the Developer Portal to view webhook delivery history and logs</li> </ul> </div> </ApiDocsLayout>)}export default ApiWebhooks;export default ApiWebhooks;
      break;
}),`;
  return (}
export default ApiWebhooks;export default ApiWebhooks;// Webhook endpoint with signature verification;
app.post('/webhooks/zion', verifyWebhookSignature, (req, res) => {const { event_type, data } = req.body,// Handle different event types;
  switch (event_type) {case 'new_application':;
      // // // console.log('New application received:', data.application_id),// Process the new application...;
      break,case 'talent_hired':;
      ...
      // // // console.log('New message received:', data.message_id),// Process the message...;
      break,default:;
      // // // console.log('Unknown event type:', event_type)}// Always return a 200 response quickly;
  res.status(200).send('Webhook received')}),app.listen(3000, () => {// // // console.log('Webhook server listening on port 3000')}),`,return (}),app.listen(3000, () => {// // // console.log('Webhook server listening on port 3000')}),`,
"created at" : "2023-06-18T14:22:15Z" 
}
}`;
