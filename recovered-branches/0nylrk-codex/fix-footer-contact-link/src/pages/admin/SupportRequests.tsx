...
import { AppLayout } from "@/layout/AppLayout",import { SEO } from "@/components/SEO",// Mock data for support requests;
const MOCK_SUPPORT_REQUESTS = [;
  ...
  const [searchQuery, setSearchQuery] = useState("")const [statusFilter, setStatusFilter] = useState<string | null>(null)const [priorityFilter, setPriorityFilter] = useState<string | null>(null)const [categoryFilter, setCategoryFilter] = useState<string | null>(null)// Apply filters to the request data;
  const filteredRequests = MOCK_SUPPORT_REQUESTS.filter(request => {// Apply search query filter;
    if (searchQuery &&;
...
const MOCK_SUPPORT_REQUESTS = [;
  {id: "SR-1001",userId: "user-123",issue: "Cannot access account after password reset",status: "open",priority: "high",createdAt: "2023-12-15T14:30:00Z",lastUpdated: "2023-12-15T15:45:00Z",category: "authentication";
    userId: "user-456",issue: "Payment failed but funds were deducted",status: "in-progress",priority: "high",createdAt: "2023-12-14T09:15:00Z",lastUpdated: "2023-12-15T13:20:00Z",category: "billing";
    userId: "user-789",issue: "Unable to download invoice PDF",status: "open",priority: "medium",createdAt: "2023-12-15T11:00:00Z",lastUpdated: "2023-12-15T11:00:00Z",category: "billing";
    userId: "user-235",issue: "API integration documentation is outdated",status: "open",priority: "low",userId: "user-567",issue: "Dispute with freelancer over delivered work quality",status: "in-progress",priority: "high",createdAt: "2023-12-12T10:30:00Z",lastUpdated: "2023-12-15T09:15:00Z",category: "disputes";
    userId: "user-890",issue: "Profile verification pending for over 7 days",status: "resolved",priority: "medium",createdAt: "2023-12-08T13:20:00Z",lastUpdated: "2023-12-15T08:30:00Z",category: "verification";
    userId: "user-345",issue: "Cannot upload portfolio images",status: "resolved",priority: "medium",createdAt: "2023-12-10T15:10:00Z",lastUpdated: "2023-12-13T11:25:00Z",category: "profile";
  }
  const [searchQuery, setSearchQuery] = useState(""),const [statusFilter, setStatusFilter] = useState<string | null>(null),const [priorityFilter, setPriorityFilter] = useState<string | null>(null),const [categoryFilter, setCategoryFilter] = useState<string | null>(null),// Apply filters to the request data;
  const filteredRequests = MOCK_SUPPORT_REQUESTS.filter(request => {// Apply search query filter;
    if (searchQuery &&;
  ...
  const filteredRequests = MOCK_SUPPORT_REQUESTS && MOCK_SUPPORT_REQUESTS.filter(request => {// Apply search query filter;
    if (searchQuery && ;
        ...
        !request && request.id.toLowerCase().includes(searchQuery && searchQuery.toLowerCase())) {return false;
    }
    // Apply status filter;
    if (statusFilter && request && request.status !== statusFilter) {return false;
    }
    // Apply priority filter;
    if (priorityFilter && request && request.priority !== priorityFilter) {return false;
    }
    // Apply category filter;
    if (categoryFilter && request && request.category !== categoryFilter) {return false;
    }
  // Count by status for the summary dashboard;
  ...
  const resetFilters = () => {setSearchQuery("")setStatusFilter(null)setPriorityFilter(null)setCategoryFilter(null)}return (<AppLayout>;
      <SEO;
        title="Support Requests | Admin Dashboard";
        description="Manage and track user support requests and issues";
      />;
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">;
              <div className="relative flex-1">;
                ...
                <Input;
                  ...
                  value={searchQuery}<Select value={statusFilter || ""} onValueChange={value => setStatusFilter(value || null)}>;
                ...
                <Table>;
                  ...
                  <TableBody>;
                    {filteredRequests.map((request) => (<TableRow key={request.id}>;
                        ...
                        <TableCell>;
                          <Badgevariant={request && request.priority === 'high';
                              ? 'destructive';
