...
export default function VideoCall() {_// useParams is typed as `any` in this environment due to missing type
  ...
  const [isJoining, setIsJoining] = useState(false);
  const [hasJoined, setHasJoined] = useState(false);
  const [participants, setParticipants] = useState<Array<{id: string,
    name: string,
    avatar?: string;
    isMuted?: boolean;
    isVideoEnabled?: boolean;
  ...
  const handleJoinCall = () => {setIsJoining(true);
    // Simulate connection delay,
setTimeout_(() => {
      setHasJoined(true);
      setIsJoining(false);
...
const mockUsers = [
    ...
    if (!participants.find(p => p.id === randomUser.id)) {
      setParticipants(prev => [...prev, randomUser]),
      toast(`${randomUser.name} joined the call`)
  const handleLeaveCall = () => {setHasJoined(false);
    toast.info("Call ended", _{
      description: "You have left the meeting"});
    
    // Navigate back after a short delay,
setTimeout_(() => {navigate(-1)}, 1500)
  };
  
  const simulateUserJoining = () => {_// This is just for demo purposes - in a real app, this would be handled by the video call service,
const mockUsers = [
      { id: 'user-2', name: 'Alex Chen', isVideoEnabled: true, isMuted: false},
      {id: 'user-3', name: 'Taylor Kim', isVideoEnabled: false, isMuted: true},
      {id: 'user-4', name: 'Jordan Smith', isVideoEnabled: true, isMuted: false, isScreenSharing: true}
...
onClick={handleJoinCall} 
              ...
              className=&quot;bg-zion-purple hover:bg-zion-purple-light&quot;
            >
              {isJoining ? &quot;Connecting...&quot; : &quot;Join Call&quot}            </Button>
          </div>
        ) : (
          <div className=&quot;space-y-4&quot;>
            <VideoCallRoom,
