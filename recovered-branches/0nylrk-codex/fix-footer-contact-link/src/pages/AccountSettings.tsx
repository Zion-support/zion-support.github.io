...
import { Switch } from '@/components/ui/switch',
import { Label } from '@/components/ui/label',
import { toast } from 'sonner',export default function AccountSettings() {
  const { user } = useAuth(),
  const [displayWeb3, setDisplayWeb3] = useState(false),
  const [didHandle, setDidHandle] = useState(''),
  const [enableBackup, setEnableBackup] = useState(false),
...
export default function AccountSettings() {const { user} = useAuth();
  ...
  const handleSave = () => {setIsSubmitting(true);
...
setTimeout_(() => {
      try {
        ...
        setIsSubmitting(false)
          ...
          'accountsettings', JSON.stringify({ displayWeb3, didHandle, enableBackup})
        );
        
        toast.success('Account settings updated successfully')
      } catch (e) {toast.error('Failed to save settings')} finally {setIsSubmitting(false)}
    }, 1000)
  };
