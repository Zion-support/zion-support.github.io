...
className={`flex flex-col ${!isMobile ? ""md":flex-row "md":justify-between "md":items-center" : ""} mb-8 gap-4`}`        >
          <div>
            <h1 className={`text-${isMobile ? "2xl" : "3xl"} font-bold`}>`              My Jobs
            </h1>
            <p className="text-muted-foreground mt-1">"
              Manage your job postings and talent applications
            </p>
          </div>
          <div className={`flex gap-2 ${isMobile ? "flex-col" : ""}`}>`            <Button,
variant="outline";"
              asChild,
...
defaultValue="all""
            ...
            >
              <TabsList className={`mb-6 ${isMobile ? "w-full" : ""}`}>`                <TabsTrigger value="all" className={isMobile ? "flex-1" : ""}>"
                  All
                </TabsTrigger>
...
return (;
    <>;
      ...
      <AppHeader />;
      <main className="container mx - auto px - 4 py-8">;"
        <div className={`flex flex - col ${!is_mobile ? '"md":flex - row "md":justify - between "md":items - center' : ''} mb - 8 gap - 4`}>;`          <div>;
            <h1 className={`text-${is_mobile ? '2xl' : '3xl'} font - bold`}>My Jobs</h1>;`            <p className="text - muted - foreground mt-1">Manage your job postings and talent applications</p>;"
          </div>;
          <div className={`flex gap - 2 ${is_mobile ? 'flex - col' : ''}`}>;`            <Button variant="outline" as_child className={is_mobile ? 'w - full justify - center' : ''}>;'
              <Link to="/hiring - tracker">;"
                <Kanban className="h - 4 w - 4 mr-2" /> Hiring Pipeline;"
              </Link>;
            </Button>;
...
function ClientDashboardContent() {;
  ...
  useEffect(() => {;
    ...
    if (jobs.length > 0 && !selectedJobId) {;
      ...
      <AppHeader />;
      <main className="container mx-auto px-4 py-8">;"
        <div className={`flex flex-col ${!isMobile ? '"md":flex-row "md":justify-between "md":items-center' : ''} mb-8 gap-4`}>;`          <div>;
            <h1 className={`text-${isMobile ? '2xl' : '3xl'} font-bold`}>My Jobs</h1>;`            <p className="text-muted-foreground mt-1">Manage your job postings and talent applications</p>;"
          </div>;
