...
function ClientDashboardContent() {const [activeTab, setActiveTab] = useState<JobStatus | "all">("all");
  const { jobs, isLoading} = useJobs();
  const [selectedJobId, setSelectedJobId] = useState<string | null>(null);
  const [selectedJobTitle, setSelectedJobTitle] = useState<string>("");
  const isMobile = useIsMobile();
  ...
  // Set the first job as selected when jobs are loaded (if any)
  useEffect_(() => {if (jobs.length > 0 && !selectedJobId) {
      setSelectedJobId(jobs[0].id);
      setSelectedJobTitle(jobs[0].title)}
  }, [jobs, selectedJobId]);

  const handleJobSelect = (jobId: string, jobTitle: string) => {setSelectedJobId(jobId);
    setSelectedJobTitle(jobTitle)};
          ...
          <div className="lg:col-span-2&quot;>
            <Tabs defaultValue=&quot;all&quot; onValueChange={(value) => setActiveTab(value as JobStatus | &quot;all")}>
              <TabsList className={`mb-6 ${isMobile ? 'w-full' : ''}`}>
                ...
                <TabsTrigger value="inprogress" className={isMobile ? 'flex-1' : ''}>Active</TabsTrigger>
              ...
              <TabsContent value=&quot;inprogress" className="mt-0&quot;>
                <JobsList filter=&quot;inprogress&quot; onSelectJob={handleJobSelect} />
              </TabsContent>
              <TabsContent value=&quot;filled" className="mt-0&quot;>
                <JobsList filter=&quot;filled&quot; onSelectJob={handleJobSelect} />
              </TabsContent>
              <TabsContent value=&quot;closed" className="mt-0&quot;>
                <JobsList filter=&quot;closed" onSelectJob={handleJobSelect} />              </TabsContent>
            </Tabs>
          </div>
...
export default function ClientDashboard() {return (
    <ProtectedRoute>
