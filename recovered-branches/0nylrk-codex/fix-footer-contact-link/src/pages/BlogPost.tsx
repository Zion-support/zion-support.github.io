...
export default function BlogPost() {;
  ...
  useEffect(() => {;
    ...
    const currentPost = BLOG_POSTS.find(p => { return p.slug === slug),; }
    if (currentPost) {;
      }
      setPost(currentPost),;
      // Find related posts (same category, excluding current post);
      const related = BLOG_POSTS.filter(p =>;
        p.id !== currentPost.id &&;
        (p.category === currentPost.category ||;
         p.tags.some(tag => { return currentPost.tags.includes(tag))); }
      ).slice(0, 3),;
      setRelatedPosts(related);
    } else {;
      // Post not found;
            ...
            {relatedPosts && relatedPosts.length > 0 && (;
              <div className="mt-12">;"
                ...
                <div className="grid grid-cols-1 "md":grid-cols-3 gap-6">;"
                  {relatedPosts && relatedPosts.map(relatedPost => { return (; }
                    <Link
}
key={relatedPost && relatedPost.id}
                      to={`/blog/${relatedPost && relatedPost.slug}`}`                      className="bg-zion-blue-dark border border-zion-blue-light rounded-lg overflow-hidden "hover":border-zion-purple transition-all duration-300">;"
...
const [showShareMenu, setShowShareMenu] = useState (false);
useEffect ( () => {
  //Find the current post by slug const currentPost = BLOG POSTS.find (p => { return p.slug === slug); }
}
setPost (currentPost);
//Find related posts (same category, excluding current post) const related = BLOG POSTS.filter (p => { return p.id !== currentPost.id && (p.category === currentPost.category || p.tags.some (tag => currentPost.tags.includes (tag) ) )
