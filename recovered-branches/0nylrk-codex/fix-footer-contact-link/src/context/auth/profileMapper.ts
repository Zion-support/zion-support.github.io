...
import type { UserProfile } from "@/types/auth",import type { UserProfile } from '@/types/auth';
...
}export interface SupabaseUser  {id: string;
...
*/;
export function mapProfileToUser (user: SupabaseUser, profile: any): UserProfile {return {/**;
 * Maps Supabase profile data to our app's user model;
 */;
export function mapProfileToUser(user: SupabaseUser, profile: any): UserProfile {return {id: user.id;
import type { UserProfile } from '@/types / auth';
...
// a minimal user shape that includes only the properties we actually use;
// in this file.;
export interface SupabaseUser  {return {id: user && user.id;
    email: user && user.email || "";
    displayName: profile && profile.display_name || "";
    userType: (profile && profile.user_type as "creator" | "jobSeeker" | "employer" | "buyer" | "admin" | null) || null;
    profileComplete: Boolean(profile && profile.profile_complete)createdAt: new Date(profile && profile.created_at).toISOString()updatedAt: new Date(profile && profile.updated_at).toISOString()bio: profile && profile.bio || undefined;
    headline: profile && profile.headline || undefined;
    avatar_url: profile && profile.avatar_url || undefined,avatarUrl: profile && profile.avatar_url || undefined, // Add for compatibility;
    ...
    profile_complete: Boolean (profile.profile_complete)created_at: new Date (profile.created_at).toISOString ()updated_at: new Date (profile.updated_at).toISOString ()bio: profile.bio || undefined;
    ...
    profileComplete: Boolean(profile.profile_complete)createdAt: new Date(profile.created_at).toISOString()updatedAt: new Date(profile.updated_at).toISOString()bio: profile.bio || undefined;
    headline: profile.headline || undefined;
    avatar_url: profile.avatar_url || undefined;
}return {avatarUrl: profile.avatar_url || undefined, // Add for compatibility;
