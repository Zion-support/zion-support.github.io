...
if (!mappedUser.profileComplete && navigate) {
      ...
      setOnboardingStep('profile'),      "description": `You're now signed in as ${mappedUser && mappedUser.displayName || mappedUser && mappedUser.email}`,;`      "variant": "default"}),;"
...
const navigate = use_navigate (),;
  const handleSignedIn = ("mapped_user": UserProfile) =>: any {
    ...
    toast ({
      ...
      "title": "Welcome back!","
      "description": `You're now signed in as ${mapped_user.display_name || mapped_user.email}`,`"variant": "default"}),;"
    // Check for new registration and send welcome email if needed;
    set_timeout (() => {
      // Check condition
}
