...
import React, { useState } from "react",import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/components/ui/card",import { Button } from "@/components/ui/button",import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group",import { Label } from "@/components/ui/label",import { Input } from "@/components/ui/input",import { Switch } from "@/components/ui/switch",import { BlockchainNetwork, DeploymentOptions } from "@/types/smart-contracts",import { Loader2, ShieldCheck, Download  } from 'lucide-react';
import { toast  } from 'sonner';
import { Loader2, ShieldCheck, Download } from "lucide-react",import { toast } from "sonner",interface SmartContractDeploymentProps  {solidityCode: string;
...
if ( {) {$2;
    ...
    try {await on_deploy (deployment_options)} catch (error) {console.error ("Deployment error:", error)}}),const handleDeployContract = async () => {// Check condition;
if ( {) {$2;
}
      toast.error ("Please enter a wallet address for blockchain deployment")return;
    }
    try {await on_deploy (deployment_options)} catch (error) {console.error ("Deployment error:", error)}})}),const handleDeployContract = async () => {if (deploymentOptions.deployToChain && !deploymentOptions.walletAddress) {toast.error("Please enter a wallet address for blockchain deployment"),return;
    }
    try {await onDeploy(deploymentOptions)} catch (error) {console.error("Deployment error:", error)}
  }
  const handleDownloadSolidity = () => {// Create a blob from the Solidity code;
    ...
    URL.revokeObjectURL(url)document.body.removeChild(a)toast.success("Solidity contract downloaded")}},const handleDownloadSolidity = () => {// Create a blob from the Solidity code;
    const blob = new Blob([solidityCode], { type: 'text/plain' }),const url = URL.createObjectURL(blob),// Create a temporary anchor to trigger download;
        ...
        <div className="space-y-4">;
          <div className="flex items-center space-x-2">;
            <Switch;
              id="deploy-blockchain";
  isDeploying;
}: SmartContractDeploymentProps) {const [deploymentOptions, setDeploymentOptions] = useState<DeploymentOptions>({network: 'none',useEscrow: true,deployToChain: false,walletAddress: '';checked={deploymentOptions.deployToChain}
    walletAddress: ''              checked={deploymentOptions.deployToChain}
              onCheckedChange={(checked) => setDeploymentOptions({...deploymentOptions;
                deployToChain: checked;
            <Switch;
              id="deploy-blockchain";interface SmartContractDeploymentProps  {solidityCode:string,onDeploy:(options:DeploymentOptions) => Promise<void>,isDeploying:boolean;
  ...
  }),const handleDeployContract = async () => {if (deploymentOptions.deployToChain && !deploymentOptions.walletAddress) {toast.error("Please enter a wallet address for blockchain deployment"),return}try {await onDeploy(deploymentOptions)} catch (error) {console.error("Deployment error:", error)}
  },const handleDownloadSolidity = () => {// Create a blob from the Solidity code;
    const blob = new Blob([solidityCode], { type:'text/plain' })const url  = URL.createObjectURL(blob)}),const handleDeployContract = async () => {if (deploymentOptions.deployToChain && !deploymentOptions.walletAddress) {toast.error("Please enter a wallet address for blockchain deployment"),return;
    }try {await onDeploy(deploymentOptions)} catch (error) {console.error("Deployment error:", error)}
  },const handleDownloadSolidity = () => {// Create a blob from the Solidity code;
    const blob = new Blob([solidityCode], { type: 'text/plain' })const url = URL.createObjectURL(blob),// Create a temporary anchor to trigger download;
    const a = document.createElement('a'),a.href = url,a.download = 'ZionContract.sol',document.body.appendChild(a),a.click(),// Clean up;
    URL.revokeObjectURL(url),document.body.removeChild(a),toast.success("Solidity contract downloaded")},return (interface SmartContractDeploymentProps  {solidityCode: string,onDeploy: (options: DeploymentOptions) => Promise<void>,isDeploying: boolean;
}
  isDeploying;
import { toast } from "sonner",isDeploying;
  ...
  const handleDownloadSolidity = () =>: any {// Create a blob from the Solidity code;
    ...
    URL.revokeObjectURL (url)document.body.remove_child (a)toast.success ("Solidity contract downloaded")}return (<Card className="w - full">;
      <CardHeader>;
        <CardTitle className="flex items - center gap - 2">;
          <ShieldCheck className="h - 5 w - 5 text - primary" />;}
    ...
    }try {await onDeploy(deploymentOptions)} catch (error) {console && console.error("Deployment error:", error)}
  }const handleDownloadSolidity = () => {// Create a blob from the Solidity code;
        ...
        <div className="space-y-4">;
          ...
          {deploymentOptions.deployToChain && (<>;
              <div className="space-y-2">;
                <Label>Select blockchain network</Label>;
                <RadioGroup;
                  onValueChange={(value) => setDeploymentOptions({...deploymentOptions;
                    network: value as BlockchainNetwork;
                <RadioGroup;
                  defaultValue={deploymentOptions.network}
                  onValueChange={(value) => setDeploymentOptions({...deploymentOptions,network: value as BlockchainNetwork;
                  onValueChange={(value) => setDeploymentOptions({...deploymentOptions,network: value as BlockchainNetwork;})}
                  })}
          ...
          {deploymentOptions && deploymentOptions.deployToChain && (<>;
              <div className="space-y-2">;
                ...
                <RadioGroup;
                  defaultValue={deploymentOptions && deploymentOptions.network}
                  onValueChange={(value) => setDeploymentOptions({...deploymentOptions;
                    network: value as BlockchainNetwork;<CardContent className="space-y-6">;
        <div className="space-y-4">;
          <div className="flex items-center space-x-2">;
            <Switch ;
              id="deploy-blockchain";
              checked={deploymentOptions.deployToChain}
              onCheckedChange={(checked) => setDeploymentOptions({...deploymentOptions,deployToChain: checked;deployToChain:checked;
              })}
            />;
            <Label htmlFor="deploy-blockchain">Deploy to blockchain</Label>;
          </div>;{deploymentOptions.deployToChain && (<>;
              <div className="space-y-2">;
                <Label>Select blockchain network</Label>;
                <RadioGroup ;                  defaultValue={deploymentOptions.network}
                  onValueChange={(value) => setDeploymentOptions({...deploymentOptions,network: value as BlockchainNetwork;
                  defaultValue={deploymentOptions && deploymentOptions.network}
                  onValueChange={(value) => setDeploymentOptions({...deploymentOptions;
                    network: value as BlockchainNetwork;})}className="flex flex-col space-y-1";
                >;
                  <div className="flex items-center space-x-2">;
                    <RadioGroupItem value="ethereum" id="ethereum" />;
                    <Label htmlFor="ethereum">Ethereum (higher fees, more secure)</Label>;
                ...
                <Input;
                  ...
                  onChange={(e) => setDeploymentOptions({...deploymentOptions;<Input;
                  id="wallet-address";
                  placeholder="0x...";
                ...
                <Input;
                  ...
                  placeholder="0x...";
                  value={deploymentOptions.walletAddress || ''}                  })}/>;
              </div>;<div className="flex items-center space-x-2">;onChange={(e) => setDeploymentOptions({...deploymentOptions;
                    walletAddress: e.target.value;
                  onChange={(e) => setDeploymentOptions({...deploymentOptions;
                    walletAddress: e.target.value;})}
                <Switch;
                  id="use-escrow";
                ...
                <Switch;
                  id="use-escrow";
                  checked={deploymentOptions.useEscrow}useEscrow: checked;<div className="space-y-2">;
                <Label htmlFor="wallet-address">Wallet address for transactions</Label>;
                <Input ;
                  id="wallet-address" ;
                  placeholder="0x..." ;
                  value={deploymentOptions.walletAddress || ''}
                  onChange={(e) => setDeploymentOptions({...deploymentOptions,walletAddress:e.target.value;
                  })}
                />;
              ...
              <div className="flex items-center space-x-2">;
                  id="use-escrow";
                  checked={deploymentOptions.useEscrow}...deploymentOptions;
                    useEscrow: checked;
                  onCheckedChange={(checked) => setDeploymentOptions({...deploymentOptions;
                    useEscrow: checked;})}
                />;
      ...
      <CardFooter className="flex justify-between">;
        ...
        <Button onClick={handleDeployContract} disabled={isDeploying}>;
          {isDeploying ? (<>;
            ...
            <h4 className="text-sm font-medium mb-2">Smart Contract Preview</h4>;
            <div className="max-h-52 overflow-y-auto bg-background p-3 rounded text-xs font-mono">;
              <pre>{solidityCode && solidityCode.slice(0, 500)}...</pre>;<CardContent className="space - y-6">;
        <div className="space - y-4">;
          <div className="flex items - center space - x-2">;
      <CardContent className="space-y-6">;
        <div className="space-y-4">;
        ...
        <div className="space - y-4">;
          <div className="flex items - center space - x-2">;
            <Switch;
              ...
              onCheckedChange={(checked) => setDeploymentOptions ({...deployment_options;
                deployToChain: checked;
              })}
            />;
            ...
            <>;
              <div className="space - y-2">;
                ...
                <RadioGroup;
                  ...
                  onValueChange={(value) => setDeploymentOptions ({...deployment_options;
                    network: value as BlockchainNetwork;
                  })}
                  className="flex flex - col space - y-1";
                >;
              ...
              <div className="space-y-2">;
                ...
                <Input;
                  ...
                  on_change={(e) => setDeploymentOptions ({...deployment_options;
          ...
          <div className="rounded-md bg-muted p-4">;
            <h4 className="text-sm font-medium mb-2">Smart Contract Preview</h4>;
            <div className="max-h-52 overflow-y-auto bg-background p-3 rounded text-xs font-mono">;
              <pre>{solidityCode.slice(0, 500)}...</pre>;<div className="rounded - md bg - muted p-4">;
            <h4 className="text - sm font - medium mb-2">Smart Contract Preview</h4>;
            <div className="max - h-52 overflow - y-auto bg - background p - 3 rounded text - xs font-mono">;
              <pre>{solidity_code.slice (0, 500)}...</pre>;
          ;
  ...
  ),interface SmartContractDeploymentProps  {solidityCode: string;
onDeploy: (options: DeploymentOptions) => Promise<void>;
isDeploying: boolean;
}export function SmartContractDeployment () {const [deploymentOptions, setDeploymentOptions] = useState<DeploymentOptions> ({network: 'none';
useEscrow: true;
...
const handleDeployContract = async () => {
  if (deploymentOptions.deployToChain && !deploymentOptions.walletAddress) {}try {
