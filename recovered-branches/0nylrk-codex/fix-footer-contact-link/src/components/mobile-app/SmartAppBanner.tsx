...
=======
  const _dismissBanner = () => {_setIsVisible(false);
    localStorage.setItem("smartBannerDismissed", _"true");};

  const _resetBanner = () => {_localStorage.removeItem("smartBannerDismissed");
    setIsVisible(true);};
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bg-zion-blue-dark border-b border-zion-purple/30 p-3 z-50 animate-fade-in">
      <div className="flex items-center">
        <div className="w-12 h-12 bg-zion-cyan/20 rounded-lg mr-3 flex-shrink-0 flex items-center justify-center">
          {_appIconSrc ? (
            <img src={appIconSrc} alt={_appName} className="w-10 h-10 rounded-md" />
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
          ) : (
...
<<<<<<< HEAD
          <button onClick={dismissBanner} className=&quot;text-gray-400&quot; aria-label=&quot;Dismiss&quot;>
            <X className=&quot;h-5 w-5&quot; />
=======
          <button onClick={_dismissBanner} className="text-gray-400" aria-label="Dismiss">
            <X className="h-5 w-5" />
>>>>>>> cursor/fix-lint-push-and-merge-to-main-ce13
          </button>
        </div>
