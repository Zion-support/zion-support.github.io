...
const fileInputRef = useRef<HTMLInputElement> (null);
<CardHeader> <CardTitle className="text-lg" >App Screenshots  <CardContent> <div className= {"`;
  `border-2 border-dashed rounded-lg p-4 mb-4 text-center transition-colors $ {"
  isDragging > <Upload className="mx-auto h-8 w-8 text-gray-400 mb-2" /> <p className="text-sm mb-2" >Drag & drop screenshots here</p> <input > <Plus className="mr-2 h-4 w-4" /> Select Files  </div> > <Trash2 className="h-3 w-3" /> </button> </div>) )"
