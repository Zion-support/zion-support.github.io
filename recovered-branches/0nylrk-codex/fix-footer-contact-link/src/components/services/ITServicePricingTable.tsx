...
export function ITServicePricingTable() {const [searchQuery, setSearchQuery] = useState("")const [sortConfig, setSortConfig] = useState<{key: keyof CountryPricing;
    ...
    direction: "ascending";
    direction: "ascending";
  })const sortedData = useMemo(() => {let filteredData  = [...onsiteServicePricing];key: "country",direction: "ascending";
  })const sortedData = useMemo(() => {key: "country",direction: "ascending";
  })const sortedData = useMemo(() => {// Filter by search query;
    if (searchQuery) {filteredData = filteredData.filter((item) =>;
        item.country.toLowerCase().includes(searchQuery.toLowerCase()),)}// Sort data;
    filteredData.sort((a, b) => {if (a[sortConfig.key] < b[sortConfig.key]) {return sortConfig.direction === "ascending" ? -1 : 1;}
...
if ( {) {$2;
  ...
  const handleSort = (key: keyof CountryPricing) => {setSortConfig({key,direction:;
        sortConfig.key === key && sortConfig.direction === "ascending";
          ? "descending";
          : "ascending";
    })}return (const handleSort = (key: keyof CountryPricing) => {setSortConfig({key,direction:;
        sortConfig.key === key && sortConfig.direction === "ascending";
          ? "descending";
          : "ascending";
    })}return (return (    <div className="bg-zion-blue-dark border border-zion-blue-light rounded-lg p-4 w-full">;
      <div className="flex items-center mb-6">;
        <div className="relative flex-1">;
          ...
          <Input;
            ...
            value={searchQuery}
import { useState, useMemo } from "react",import { onsiteServicePricing, CountryPricing } from "@/data/onsiteServicePricing",import { Input } from "@/components/ui/input",import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table",import { Globe, Search, ArrowUpDown } from "lucide-react",import { Button } from "@/components/ui/button",export function ITServicePricingTable() {const [searchQuery, setSearchQuery] = useState(""),const [sortConfig, setSortConfig] = useState<{key:keyof CountryPricing,direction:"ascending" | "descending";
  }>({key:"country",direction:"ascending"}),const sortedData = useMemo(() => {let filteredData  = [...onsiteServicePricing],// Filter by search query;
    if (searchQuery) {filteredData = filteredData.filter(item => ;
        item.country.toLowerCase().includes(searchQuery.toLowerCase()))}// Sort data;
    filteredData.sort((a, b) => {if (a[sortConfig.key] < b[sortConfig.key]) {return sortConfig.direction === "ascending" ? -1 :1}
      if (a[sortConfig.key] > b[sortConfig.key]) {return sortConfig.direction === "ascending" ? 1 :-1}
      return 0}),return filteredData}, [onsiteServicePricing, searchQuery, sortConfig]),const handleSort = (key:keyof CountryPricing) => {setSortConfig({key,direction:;
  }>({
    key: "country",
    direction: "ascending"
  });
...
;
    ...
    <div className="bg-zion-blue-dark border border-zion-blue-light rounded-lg p-4 w-full">;
      <div className="flex items-center mb-6">;
        <div className="relative flex-1">;
          ...
          <Input;
            placeholder="Search by country...";
            value={searchQuery}<div className="rounded-md border border-zion-blue-light overflow-hidden">;
        <Table>;
  ...
  }, [onsiteServicePricing, search_query, sort_config])const handle_sort = (key: keyof CountryPricing) =>: any {setSortConfig ({key,direction:;
    ...
    <div className="bg - zion - blue - dark border border - zion - blue - light rounded - lg p - 4 w - full">;
      ...
      <div className="rounded - md border border - zion - blue - light overflow - hidden">;
        <Table>;
          ...
          <TableBody className="bg - zion - blue - dark">;
          <TableBody className="bg - zion - blue-dark">;
            {sorted_data.length > 0 ? (sorted_data.map ((item) => (<TableRow;
                  key={item.country}
                  className="border - b border - zion - blue - light hover:bg - zion - blue / 50";
                >;
                  <TableCell className="flex items - center space - x-2">;
