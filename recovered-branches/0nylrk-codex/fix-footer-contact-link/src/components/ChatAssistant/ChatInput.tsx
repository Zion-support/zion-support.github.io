...
export function ChatInput({ onSend, disabled = false }: ChatInputProps) {
  ...
  const handleSubmit = (e: FormEven</HTMLFormElement>t<HTMLFormElement>) => {
    e.preventDefault(),
  ...
  const handleKeyPress = (e</HTMLTextAreaElement>: KeyboardEvent<HTMLTextAreaElement>) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-end gap-2">
      <textarea
        ...
        onKeyDown={handleK</textarea>eyPress}
        rows={1}
        disabled={disabled}
      />
      <Button 
