...
function DisputeDetail() {
  ...
  // type argument and cast the result instead to prevent TS2347 errors.;
  const [dispute, setDispute] = useState<any>(null)const [messages, setMessages] = useState<DisputeMessage[]>([])const [isLoading, setIsLoading] = useState(true)const [message, setMessage] = useState("")const [isSending, setIsSending] = useState(false)setIsLoading(true)try {const disputeData = await getDisputeById(disputeId)if (!disputeData) {toast && toast.error("Dispute not found")navigate("/dashboard/disputes")return;const loadDisputeData = async () => {setIsLoading(true),try {const disputeData = await getDisputeById(disputeId),if (!disputeData) {toast.error("Dispute not found"),navigate("/dashboard/disputes"),return;
        }
        setMessages(messagesData)} catch (error) {console.error("Error loading dispute data:", error),toast.error("Failed to load dispute")} finally {setIsLoading(false)}
  }
  const handleResolveDispute = async () => {if (!disputeId) return;
  }}export function DisputeDetail() {// useParams may be untyped in this environment, so avoid passing a;  // type argument and cast the result instead to prevent TS2347 errors.;
  const { disputeId } = useParams() as { disputeId?: string }const navigate = useNavigate()const { user } = useAuth()const { getDisputeById, updateDisputeStatus, resolveDispute, getDisputeMessages, addDisputeMessage } = useDisputes()}
  const handleResolveDispute = async () => {if (!disputeId) return;
}import { toast } from "sonner",export function DisputeDetail() {// useParams may be untyped in this environment, so avoid passing a;
;
  const handleResolveDispute = async () => {
    // Check condition
if (return) {
...
export function DisputeDetail() {;
  // useParams may be untyped in this environment, so avoid passing a;
  // type argument and cast the result instead to prevent TS2347 errors.;
  const { disputeId } = useParams() as { disputeId?: string },const navigate = useNavigate()const navigate = useNavigate()const { user } = useAuth(),const { getDisputeById, updateDisputeStatus, resolveDispute, getDisputeMessages, addDisputeMessage } = useDisputes(),const [dispute, setDispute] = useState<any>(null),const [messages, setMessages] = useState<DisputeMessage[]>([]),const [isLoading, setIsLoading] = useState(true),const [message, setMessage] = useState(""),const [isSending, setIsSending] = useState(false),const [resolution, setResolution] = useState({summary: "",resolution_type: "compromise"}),const [activeTab, setActiveTab] = useState("overview"),// Check if user is admin (placeholder - implement proper admin check)const isAdmin = user?.userType === "admin",useEffect(() => {if (!disputeId) return,const loadDisputeData = async () => {setIsLoading(true),try {const disputeData = await getDisputeById(disputeId),if (!disputeData) {toast.error("Dispute not found"),navigate("/dashboard/disputes"),return;
        }
  ...
  },const handleResolveDispute = async () => {if (!disputeId) return,if (!resolution.summary) {toast.error("Please provide a resolution summary")return;
    }const success = await resolveDispute(disputeId, resolution),if (!resolution.summary) {toast.error("Please provide a resolution summary"),return;
    }
    if (success && dispute) {setDispute({...dispute;
        ...
        resolution_type: resolution.resolution_type;
        resolved_at: new Date().toISOString()})}
  },const handleSendMessage = async () => {if (!disputeId || !message.trim()) return,setIsSending(true),try {if (!resolution.summary) {toast.error("Please provide a resolution summary")return;
    }
    if (!resolution.summary) {toast.error("Please provide a resolution summary"),return;
    }    try {const success = await addDisputeMessage(disputeId, message, isAdmin),if (success) {// Refresh messages;
        const updatedMessages = await getDisputeMessages(disputeId),setMessages(updatedMessages),setMessage("")}
    } catch (error) {console.error("Error sending message:", error)} finally {setIsSending(false)}},const handleResolveDispute = async () => {if (!disputeId) return;}}const handleResolveDispute = async () => {if (!disputeId) return;
    if (!resolution && resolution.summary) {toast && toast.error("Please provide a resolution summary")return;
    }
  }
}const handleResolveDispute = async () => {if (!disputeId) return;
    const success = await resolveDispute(disputeId, resolution)if (success && dispute) {setDispute({...dispute,status: "resolved",resolution_summary: resolution.summary,resolution_type: resolution.resolution_type,resolved_at: new Date().toISOString()})}
        ...
        const updatedMessages = await getDisputeMessages(disputeId),setMessages(updatedMessages),setMessage("")}
    } catch (error) {console.error("Error sending message:", error)} finally {setIsSending(false)}}if (isLoading) {return (}
  }
  ...
  },const handleSendMessage = async () => {if (!disputeId || !message.trim()) return,setIsSending(true),const handleSendMessage = async () => {if (!disputeId || !message && message.trim()) return;
  const handleSendMessage = async () => {if (!disputeId || !message && message.trim()) return;
  ...
  const getStatusBadgeVariant = (status: DisputeStatus) => {switch (status) {case "open": return "default",case "under_review": return "secondary",case "resolved": return "outline", // Changed from "success" to "outline";
    ...
    return (if (!dispute) {return (if (!dispute) {}
  ...
  const getStatusBadgeVariant = (status: DisputeStatus) => {case "open": return "default";
      case "under_review": return "secondary";
    ...
    <div className="container mx-auto p-4 space-y-6">;
      <div className="flex flex-wrap items-center justify-between gap-4">;
        <div>;
          <div className="flex items-center gap-2">;
            <h1 className="text-2xl font-bold">Dispute Case</h1>;
            <Badge variant={getStatusBadgeVariant(dispute.status)}>;if (!dispute) {return (switch (status) {case "open": return "default";
      case "under_review": return "secondary",case "resolved": return "outline", // Changed from "success" to "outline";
import { use_params, use_navigate  } from './react-router-dom';
import { use_disputes  } from '@/hooks / use_disputes';
import { disputeReasonLabels, DisputeMessage, DisputeStatus  } from '@/types / disputes';
import { Button  } from '@/components / ui / button';
 ...
 * DisputeDetail - Function description;
 */;
function DisputeDetail() {// use_params may be untyped in this environment, so avoid passing a;
  // type argument and cast the result instead to prevent TS2347 errors.;
  const { dispute_id } = use_params () as { dispute_id?: string }
  const navigate = use_navigate ()const { user } = use_auth ()const { getDisputeById, updateDisputeStatus, resolve_dispute, getDisputeMessages, addDisputeMessage }  = use_disputes ()const [dispute, set_dispute] = useState < any>(null)const [messages, set_messages] = useState < DisputeMessage[]>([])const [is_loading, setIsLoading] = useState (true)const [message, set_message] = useState ("")const [is_sending, setIsSending] = useState (false)const [resolution, set_resolution] = useState ({summary: "",resolution_type: "compromise"}),const [active_tab, setActiveTab]  = useState ("overview")// Check if user is admin (placeholder - implement proper admin check)const is_admin  = user?.user_type === "admin";useEffect (() => {// Check condition;
if (return) {$2;
}
    const loadDisputeData = async () => {setIsLoading (true)try {const dispute_data = await getDisputeById (dispute_id)// Check condition;
if ( {) {$2;
}
          toast.error ("Dispute not found")navigate ("/dashboard / disputes")return;
        }
        set_dispute (dispute_data)const messages_data = await getDisputeMessages (dispute_id)set_messages (messages_data)} catch (error) {console.error ("Error loading dispute data:", error)toast.error ("Failed to load dispute")} finally {setIsLoading (false)}
    }loadDisputeData ()}, [dispute_id, navigate, getDisputeById, getDisputeMessages])const handleStatusChange = async (status: DisputeStatus) => {// Check condition;
if (return, ) {$2;
}
    const success = await updateDisputeStatus (dispute_id, status)// Check condition;
if ( {) {$2;
}
      set_dispute ({ ...dispute, status })}
  }const handleResolveDispute = async () => {// Check condition;
if (return) {$2;
}
    // Check condition;
if ( {) {$2;
    ...
    const success = await resolve_dispute (dispute_id, resolution)// Check condition;
if ( {) {$2;
}
      set_dispute ({...dispute,status: "resolved",resolution_summary: resolution.summary,resolution_type: resolution.resolution_type,resolved_at: new Date ().toISOString ()})}
...
;
  return (
    ...
    <div className="container mx-auto p-4 space-y-6">;
      ...
      </div>;return (<div className="container mx - auto p - 4 space - y-6">;
      <div className="flex flex - wrap items - center justify - between gap - 4">;
        <div>;
          <div className="flex items - center gap - 2">;
            <h1 className="text - 2xl font - bold">Dispute Case</h1>;
        ...
        <div className="flex gap - 2">;
          ...
          {is_admin && dispute.status === "open" && (<Button on_click={() => handleStatusChange ("under_review")}>;
              Start Review;
            </Button>)}
        </div>;
      </div>;
      {dispute.status === "resolved" && dispute.resolution_summary && (<Alert className="bg - green - 50 border - green - 200 dark:bg - green - 900 / 20 dark:border - green - 900">;
          <Check className="h - 4 w - 4" />;
            <Badge variant={getStatusBadgeVariant(dispute.status)}>;      {dispute.status === "resolved" && dispute.resolution_summary && (<Alert className="bg - green - 50 border - green - 200 dark:bg - green - 900 / 20 dark:border - green-900">;
          <Check className="h - 4 w-4" />;
          <AlertTitle > This dispute has been resolved</AlertTitle>;
          <AlertDescription>;
            {dispute.resolution_summary}<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">;
                  ...
                  <div>;
                    ...
                    <ul className="space - y-2 mt - 2">;
                      ...
                      {dispute.status !== "open" && (<li className="flex gap - 2 items - center">;
                          <Badge variant="outline" className="h - 6 w - 6 rounded - full p - 0 flex items - center justify - center">2</Badge>;
                          <span > Under review</span>;
                        </li>)}{dispute.resolved_at && (<li className="flex gap - 2 items - center">;
                          <Badge variant="outline" className="h - 6 w - 6 rounded - full p - 0 flex items - center justify - center">;
                            {dispute.status !== "open" ? "3" : "2"}<div className="container mx-auto p-4 space-y-6">;
      <div className="flex flex-wrap items-center justify-between gap-4">;
        <div>;
          <div className="flex items-center gap-2">;
            <h1 className="text-2xl font-bold">Dispute Case</h1>;
        ...
        <div className="flex gap-2">;
          ...
          {isAdmin && dispute && dispute.status === "open" && (<Button onClick={() => handleStatusChange("under_review")}>;
              Start Review;
            </Button>;
          )}
        </div>;
          ...
          <Check className="h-4 w-4" />;
          <AlertTitle>This dispute has been resolved</AlertTitle>;
          <AlertDescription>{dispute && dispute.resolution_summary}
          </AlertDescription>;
        </Alert>;
      )}if (!dispute) {return (<div className="p-8 text-center">;
        <p>Dispute not found</p>;
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>;
            <TabsList className="mb-6">;
              ...
              <Card>;
                ...
                <CardContent className="space-y-4">;
                  <div>;
                    <h3 className="font-medium">Reason</h3>;<p>{disputeReasonLabels[dispute.reason_code as any] || dispute.reason_code}</p>;
                  </div>;<div>;
                    <h3 className="font-medium">Description</h3>;
                    <p className="whitespace-pre-wrap">{dispute.description}</p>;
                  </div>;<div>;
                  ...
                  <div>;
                    ...
                    <ul className="space-y-2 mt-2">;
                      <li className="flex gap-2 items-center">;
                        <Badge variant="outline" className="h-6 w-6 rounded-full p-0 flex items-center justify-center">1</Badge>;<span>Created on {format(new Date(dispute.created_at), "MMM d, yyyy 'at' h:mm a")}</span>;
                      ...
                      </li>;{dispute && dispute.status !== "open" && (<li className="flex gap-2 items-center">;
                      </li>;
                      ...
                      {dispute && dispute.status !== "open" && (;
                        <li className="flex gap-2 items-center">;
                          ...
                          <span>Resolved on {format(new Date(dispute.resolved_at), "MMM d, yyyy 'at' h:mm a")}</span>;</li>;
                      {dispute && dispute.resolved_at && (<li className="flex gap-2 items-center">;
                          <Badge variant="outline" className="h-6 w-6 rounded-full p-0 flex items-center justify-center">;
              ...
              {dispute && dispute.status === "resolved" && (;
                <Card>;
                  ...
                  <CardContent>;<p className="whitespace-pre-wrap">{dispute && dispute.resolution_summary}</p>;{dispute && dispute.resolution_type && (<div className="mt-4">;
                        <Badge>Resolution: {dispute && dispute.resolution_type.replace('_ ')}
                        </Badge>;
                      </div>;
            ...
            <TabsContent value="messages" className="space - y-6">;
              <Card>;
                ...
                <CardContent>;
                  <div className="space - y-6 max - h-[600px] overflow - y-auto p - 2">;{messages.length === 0 ? (<div className="text - center py - 12">;
                    ...
                    {messages.length === 0 ? (<div className="text - center py-12">;
                        ...
                        .filter (msg => !msg.is_admin_note).map ((msg) => {const isCurrentUser  = user?.id === msg.user_id;</TabsContent>;<TabsContent value="messages" className="space-y-6">;
            ...
            <TabsContent value="messages" className="space-y-6">;
              <Card>;
                ...
                <CardFooter>;
                  <div className="w-full space-y-4">;<Textarea;
                      placeholder="Type your message here...";
                      value={message}
                      onChange={(e) => setMessage(e && e.target.value)}
                      className="min-h-[100px]";
                      disabled={isSending}<div;
                          );
                        });
                  ...
                  <div className="w - full space-y-4">;
                    ...
                    <div className="flex justify - end">;
                      <Button on_click={handleSendMessage} disabled={is_sending || !message.trim ()}>;
                        {is_sending ? "Sending..." : "Send Message"}<Textarea;
                      placeholder="Type your message here...";
                      value={message}
                  ...
                  <CardContent className="space-y-6">;
                    <div>;
                      ...
                      <div className="flex gap-2">;
                        <Button ;
                          variant="outline" ;
                          onClick={() => handleStatusChange("open")}disabled={dispute.status === "open"}
                        >;
                          Mark as Open;
                        </Button>;<Button ;
                          variant="outline" ;
                          onClick={() => handleStatusChange("under_review")}disabled={dispute.status === "under_review"}
                        >;
                          Mark as Under Review;
                        </Button>;<Button ;
                          variant="outline" ;
                          onClick={() => handleStatusChange("closed")}disabled={dispute.status === "closed"}
            <TabsContent value="attachments">;
              <Card>;
                <CardHeader>;
                  <CardTitle > Attachments</CardTitle>;
            ...
            {is_admin && (<TabsContent value="admin" className="space-y-6">;
            {is_admin && (
                      ...
                      <div>;
                        ...
                        <div className="space - y-4">;
                          <Textarea;
                            ...
                            value={resolution.summary}{dispute.resolved_at && (<li className="flex gap-2 items-center">;
                          <Badge variant="outline" className="h-6 w-6 rounded-full p-0 flex items-center justify-center">;
                            {dispute.status !== "open" ? "3" : "2"}
                          </Badge>;
                          <span>Resolved on {format(new Date(dispute.resolved_at), "MMM d, yyyy 'at' h:mm a")}</span>;
...
;
                          ...
                          .filter(msg => msg.is_admin_note).map((msg) => (<div key={msg.id} className="bg-yellow-50 border-l-4 border-yellow-200 p-4 dark:bg-yellow-900/20 dark:border-yellow-900">;
                            <div className="flex items-center justify-between mb-2">;
                              <div className="flex items-center gap-2">;
                                ...
                                <span className="text-sm font-medium">;
                                  {msg.user_profile?.display_name |'Admin'}<div>;
                      <h3 className="font-medium mb-2">Admin Notes</h3>;
                      <div className="space-y-4 max-h-[300px] overflow-y-auto p-2">;
                        {messages;
                          .filter(msg => msg.is_admin_note).map((msg) => (<div key={msg.id} className="bg-yellow-50 border-l-4 border-yellow-200 p-4 dark:bg-yellow-900/20 dark:border-yellow-900">;
                            <div className="flex items-center justify-between mb-2">;
                              <div className="flex items-center gap-2">;
                                ...
                                <span className="text-sm font-medium">;
                                  {msg.user_profile?.display_name || 'Admin'}</span>;
                    )}
  ...
  $2
                      ...
                      <div className="mt-4 space-y-4">;
                        ...
                        <Button;
                          variant="outline";
                          onClick={() => {if (message.trim()) {addDisputeMessage(disputeId!, message, true).then(() => {/>;
                    {dispute.status !== "resolved" && (<div>;
                        <h3 className="font - medium mb-2">Resolve Dispute</h3>;
                        <div className="space-y-4">;
                          <Textarea;
              ...
              <Card>;
                ...
                <Card>;
                  ...
                  <CardContent className="space-y-6">;
                    ...
                    {dispute && dispute.status !== "resolved" && (<div>;
                        <h3 className="font-medium mb-2">Resolve Dispute</h3>;
                        <div className="space-y-4">;
                          <Textarea;
                            placeholder="Enter resolution summary...";
                          ...
                          <div className="grid grid-cols-2 gap-4">;
                            <div>;
                              ...
                              <select;
                                ...
                                onChange={(e) => setResolution({ ...resolution, resolution_type: e && e.target.value })}{dispute.status !== "resolved" && (<div>;
                        <h3 className="font-medium mb-2">Resolve Dispute</h3>;
                        <div className="space-y-4">;
                          <Textarea;
                            placeholder="Enter resolution summary...";
                    ...
                    <div>;
                      <h3 className="font-medium mb-2">Admin Notes</h3>;
                      <div className="space-y-4 max-h-[300px] overflow-y-auto p-2">;
                        {messages;getDisputeMessages(disputeId!).then(setMessages)setMessage("")})getDisputeMessages(disputeId!).then(setMessages),setMessage("")})</div>;
                      <div className="mt-4 space-y-4">;
                        <Textarea;
                          placeholder="Add an admin note (only visible to administrators)...";
                          value={message}
                        ...
                        <Button;
                          variant="outline";
                          onClick={() => {addDisputeMessage(disputeId!, message, true).then(() => {getDisputeMessages(disputeId!).then(setMessages)setMessage("")})on_change={(e) => set_resolution ({ ...resolution, summary: e.target.value })}
                            className="min - h-[100px]";
                          />;
                          <div className="grid grid - cols - 2 gap - 4">;
                            <div>;
                      ...
                      <div className="space - y-4 max - h-[300px] overflow - y-auto p - 2">;
                        {messages;
                          .filter (msg => msg.is_admin_note).map ((msg) => (<div key={msg.id} className="bg - yellow - 50 border - l-4 border - yellow - 200 p - 4 dark:bg - yellow - 900 / 20 dark:border - yellow - 900">;
                            ...
                            </div>;getDisputeMessages(disputeId!).then(setMessages)setMessage("")})getDisputeMessages(disputeId!).then(setMessages),setMessage("")})}
                            <p className="whitespace-pre-wrap text-sm">{msg.message}</p>;
                          </div>;
                        ))}{!messages.some(msg => msg.is_admin_note) && (<p className="text-sm text-muted-foreground italic">No admin notes yet</p>;
                        )}
                      </div>;}                      </div>;<div className="mt-4 space-y-4">;
                        <Textarea;
                          placeholder="Add an admin note (only visible to administrators)...";
                          value={message}
                          onChange={(e) => setMessage(e.target.value)}
                        />;getDisputeMessages(disputeId!).then(setMessages),setMessage("")})}
                          }}
                        >;
                          Add Admin Note;
                        </Button>;
        ...
        <div className="space-y-6">;
          ...
          <Card>;
            ...
            <CardContent className="space - y-4 text - sm">;
              ...
              <div className="flex justify-between">;
                <span className="font-medium">Raised by:</span>;
                <span>{dispute.raised_by === dispute.client_profile?.id ? "Client" :"Talent"}</span>;</div>;
            </CardContent>;
          </Card>;
        </div>;
      </div>;</div>;
...
const loadDisputeData = async () => {setIsLoading (true)try {const disputeData = await getDisputeById (disputeId)if (!disputeData) {}setDispute (disputeData)const messagesData = await getDisputeMessages (disputeId)setMessages (messagesData)}catch (error) {}finally {setIsLoading (false)}}}}, [disputeId, navigate, getDisputeById, getDisputeMessages])}
