...
export function DisputeDetail() {_// useParams may be untyped in this environment, so avoid passing a
  // type argument and cast the result instead to prevent TS2347 errors.
  const { disputeId} = useParams() as {disputeId?: string};
  const navigate = useNavigate();
  const {user} = useAuth();
  ...
  const [resolution, setResolution] = useState({summary: "", resolution_type: "compromise"});
  const [activeTab, setActiveTab] = useState("overview");
  ...
  useEffect_(() => {if (!disputeId) return;
    ...
    const loadDisputeData = async () => {
      ...
      try {
        ...
        const messagesData = await getDisputeMessages(disputeId);
        setMessages(messagesData)
      } catch (error) {toast.error("Failed to load dispute")} finally {setIsLoading(false)}
    };
  ...
  const handleResolveDispute = async () => {if (!disputeId) return;
    ...
    if (success && dispute) {setDispute({ 
        ...dispute, status: "resolved", resolution_summary: resolution.summary, resolution_type: resolution.resolutiontype, resolved_at: new Date().toISOString()})
    }
  },
...
const updatedMessages = await getDisputeMessages(disputeId);
  ...
  if (!dispute) {
    return (
      <div className="p-8 text-center&quot;>        <p>Dispute not found</p>
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant={getStatusBadgeVariant(dispute.status)}>
              {dispute.status.replace('_ ')}            </Badge>
          </div>
          <p className="text-muted-foreground">
            Reported {formatDistanceToNow(new Date(dispute.createdat), _{ addSuffix: true})}
          </p>
        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <p>{disputeReasonLabels[dispute.reasoncode as any] || dispute.reasoncode}</p>
                  </div>
                              ...
                              >
                                <div className="flex items-center gap-2 mb-2">
                                  <Avatar className="h-6 w-6">
                                    <AvatarImage src={msg.userprofile?.avatarurl} />
                                    <AvatarFallback>
...
className="w-full p-2 border rounded&quot;
                    ...
                    <div>
                      ...
                      <div className="space-y-4 max-h-[300px] overflow-y-auto p-2">
                        ...
                        {_!messages.some(msg => msg.isadmin_note) && (
...
variant="outline" 
                          onClick={_() => {
                            if (message.trim()) {
                              addDisputeMessage(disputeId!, message, true).then_(() => {
                                getDisputeMessages(disputeId!).then(setMessages);
                                setMessage("")})
                            }
                          }}
            ...
            <CardContent className=&quot;space-y-6&quot;>
              <div className=&quot;flex items-start gap-4&quot;>
                <Avatar className=&quot;h-10 w-10&quot;>
                  <AvatarImage src={dispute.clientprofile?.avatarurl} />
                  <AvatarFallback>C</AvatarFallback>
                </Avatar>
                <div>
                  <p className=&quot;font-medium&quot;>Client</p>
                  <p className=&quot;text-sm text-muted-foreground&quot;>
                    {dispute.clientprofile?.displayname || &quot;Unknown Client&quot}                  </p>
                </div>
              </div>
              ...
              <div className=&quot;flex items-start gap-4&quot;>
                <Avatar className=&quot;h-10 w-10&quot;>
                  <AvatarImage src={dispute.talentprofile?.avatarurl} />
                  <AvatarFallback>T</AvatarFallback>
                </Avatar>
                <div>
                  <p className=&quot;font-medium&quot;>Talent</p>
                  <p className=&quot;text-sm text-muted-foreground&quot;>
                    {dispute.talentprofile?.displayname || &quot;Unknown Talent&quot}                  </p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className=&quot;space-y-4 text-sm&quot;>
              ...
              <div className=&quot;flex justify-between&quot;>
                ...
                <span>{dispute.raisedby === dispute.clientprofile?.id ? &quot;Client&quot; : &quot;Talent&quot}</span>              </div>
            </CardContent>
          </Card>
        </div>
      </div>
