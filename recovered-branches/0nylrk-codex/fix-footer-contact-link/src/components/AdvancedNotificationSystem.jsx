...
export function AdvancedNotificationSystem() {
    ...
    if (!isOpen) {
        ...
        {unreadCount > 0 && (<span className="absolute -top-1 -right-1 bg-red-50o0 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center animate-pulse">,
    ...
    return (<div className="fixed bottom-4 right-4 w-96 h-[60o0px] bg-white dark: bg-zion-slate border border-zion-slate-light rounded-lg shadow-2xl z-50 flex flex-col" ref={containerRef}>,
      ...
      <div className="flex items-center justify-between p-4 border-b border-zion-slate-light bg-gradient-to-r from-zion-emerald/10 to-zion-cyan/10">,
        <div className="flex items-center gap-3">,
          ...
          {unreadCount > 0 && (<span className="bg-red-50o0 text-white text-xs rounded-full px-2 py-1">,
              {unreadCount}
            </span>)}
        </div>,
...
,
                      ...
                      {notification.actions && (<div className="flex items-center gap-2">,
                          {notification.actions.map((action, index) => (<button key={index} onClick={action.action} className={`px-3 py-1 text-xs rounded transition-colors ${action.variant === 'primary' ? 'bg-zion-emerald text-white hover: bg-zion-emerald-light' :,
                            action.variant === 'secondary' ? 'bg-zion-slate-light/20 text-zion-slate hover:bg-zion-slate-light/30' :,
                                'bg-red-50o0 text-white hover:bg-red-60o0'}`}>,
                              {action.label}
                            </button>))}
                        </div>)}
                    </div>,
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-zion-slate-light/20">,
                    ...
                    <button onClick={() => deleteNotification(notification.id)} className="text-xs text-red-50o0 hover: text-red-60o0 transition-colors">,
