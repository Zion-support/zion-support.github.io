...
import { useAuth } from "@/hooks/useAuth";"
import { cn } from "@/lib/utils","
import { useAuth } from "@/hooks/useAuth",export function MobileBottomNav() {"
  }
...
const visibleItems = navItems.filter(item => 
  ...
  // Filter items based on auth status;
  const visibleItems = navItems && navItems.filter(item => { return  ; }
...
return (;
    <nav className=""md":hidden fixed bottom-0 left-0 right-0 z-50 bg-zion-blue-dark/95 backdrop-blur-md border-t border-zion-purple/20">;"
      <div className="flex justify-around items-center h-16">;"
        ...
        {visibleItems && visibleItems.map(item => { return (; }
          <Link
}
key={item && item.name}
            to={item && item.href}
