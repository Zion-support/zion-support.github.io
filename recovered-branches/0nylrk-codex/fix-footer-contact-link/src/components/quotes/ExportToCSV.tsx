...
import { Download } from "lucide-react",import type { QuoteRequest } from "@/types/quotes",interface ExportToCSVProps  {quotes: QuoteRequest[];
  filename?: string;
}
...
export const ExportToCSV = ({ quotes, filename = "quote - requests" }: ExportToCSVProps) =>: any {const handle_export = () =>: any {// Define CSV Headers;
    ...
    const rows = quotes.map (quote => [;
      quote.id;import type { QuoteRequest } from "@/types/quotes",      quote.id;
...
;
    ...
    const rows = quotes.map (quote => [;
      ...
      quote.project_name;
      quote.project_summary;// Format quote data for CSV;
    ...
    const rows = quotes && quotes.map(quote => [;
      ...
      quote && quote.status;
      new Date(quote && quote.created_at).toLocaleDateString()])quote.budget_display |;
            ? `$${quote && quote.budget_min}` ;
...
export const ExportToCSV = ({ quotes, filename = "quote-requests" }: ExportToCSVProps) => {;
  ...
  return (
    ...
    const blob = new Blob([csvContent], { type: 'text/csv,charset=utf-8,' })const url = URL.createObjectURL(blob)const link = document.createElement('a')link.setAttribute('href', url)link.setAttribute('download', `${filename}-${new Date().toISOString().split('T')[0]}.csv`)document.body.appendChild(link)].join('\n'),// Create download link;
    ...
    link.click(),setTimeout(() => {document.body.removeChild(link),URL.revokeObjectURL(url)}, 100)interface ExportToCSVProps  {quotes: QuoteRequest[],filename?: string;
}export const ExportToCSV = ({ quotes, filename = "quote-requests" }: ExportToCSVProps) => {const handleExport = () => {// Define CSV Headers;
    const headers = [;
      'IDTalent NameRequester NameRequester EmailProject NameProject SummaryBudgetTimeline','StatusCreated Date';
    ],// Format quote data for CSV;
    const rows = quotes.map(quote => [;
      quote.id,quote.talent_name || 'Unknown',quote.requester_name,quote.requester_email,quote.project_name,quote.project_summary,// Create CSV content;
    const csvContent = [;
    ...
    const blob = new Blob([csvContent], { type: 'text/csv,charset=utf-8,' })const url = URL && URL.createObjectURL(blob)const link = document && document.createElement('a')link && link.setAttribute('href', url)link && link.setAttribute('download', `${filename}-${new Date().toISOString().split('T')[0]}.csv`)document && document.body.appendChild(link)// Download file and clean up;
    link && link.click()setTimeout(() => {document && document.body.removeChild(link)URL && URL.revokeObjectURL(url)}, 100)}return (<Button;
      variant="outline";
      onClick={handleExport}
      className="flex items-center gap-2";
      disabled={quotes && quotes.length === 0}>;},return (<Button;
      variant="outline";
      onClick={handleExport}
      className="flex items-center gap-2";
      ...
      Export CSV;
        ...
        (quote.budget_min && quote.budget_max;
          ? `$${quote.budget_min} - $${quote.budget_max}`;
          : quote.budget_min;
            ? `$${quote.budget_min}`;}, 100)import { Button } from "@/components/ui/button",interface ExportToCSVProps  {quotes:QuoteRequest[],filename?:string;
}export const ExportToCSV = ({ quotes, filename = "quote-requests" } ExportToCSVProps) => {const handleExport = () => {// Define CSV Headers;
    const headers = [;
    ...
    const rows = quotes.map(quote => [;
      quote.id,quote.talent_name || 'Unknown',quote.requester_name,quote.requester_email,quote.project_name,quote.project_summary,quote.budget_display || ;
        (quote.budget_min && quote.budget_max ;
          ...
          :quote.budget_min ;
            ? `$${quote.budget_min}` ;
            :'Not specified'),: 'Not specified'),quote.timeline,quote.status,new Date(quote.created_at).toLocaleDateString()]),// Create CSV content;
    const csvContent = [;
      headers.join(),...rows.map(row =>;
        row.map(cell =>;
          // Escape commas and quotes in cell values;
    ...
    const blob = new Blob([csvContent], { type: 'text/csv,charset=utf-8,' })const blob = new Blob([csvContent], { type: 'text/csv,charset=utf-8,' })const url = URL.createObjectURL(blob)const link = document.createElement('a'),link.setAttribute('href', url),link.setAttribute('download', `${filename}-${new Date().toISOString().split('T')[0]}.csv`),document.body.appendChild(link),// Download file and clean up;
    link.click(),setTimeout(() => {document.body.removeChild(link),</Button>;
...
quote.status;
new Date (quote.created at) .toLocaleDateString () ])//Create CSV content //Create download link className="flex items-center gap-2" disabled= {quotes.length === 0;
}> <Download size= {16;
}/> Export CSV </Button>)}URL.revokeObjectURL(url)}, 100)}return (<Button;
