  ...
  useEffect(() => {;
    ...
    const runMatching = async () => {;
      ...
      try {;
        ...
        const queryString = `;`          ${formData && formData.projectName} ;
          ${formData && formData.projectDescription} ;
          ${formData && formData.serviceType} ;
          ${formData && formData.budget.type === 'fixed' ? `budget ${formData && formData.budget.amount}` : ''}'          ${formData && formData.timeline}
        `;`        // Get AI matches;
...
export function SummaryStep() {;
  ...
  useEffect(() => {;
      ...
      try {;
        ...
        const queryString = `;`          ${formData.projectName} ;
          ${formData.projectDescription} ;
          ${formData.serviceType} ;
          ${formData.budget.type === 'fixed' ? `budget ${formData.budget.amount}` : ''}'          ${formData.timeline}
        `,`          formData && formData.serviceType;
          3;
        );
import { useEffect, useState } from './react';'
...
if (return) {
  $2
      ...
      try {
        ...
        const query_string = `;`          ${form_data.project_name}
          ${form_data.project_description}
          ${form_data.service_type}
          ${form_data.budget.type === 'fixed' ? `budget ${form_data.budget.amount}` : ''}'          ${form_data.timeline}
        `;`;
        // Get AI matches;
