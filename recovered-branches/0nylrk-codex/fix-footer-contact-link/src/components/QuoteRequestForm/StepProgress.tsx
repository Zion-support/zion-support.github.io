interface StepProgressProps  {currentStep: QuoteRequestSteps;
}{ id: "summary", label: "Summary" }{ id: "summary", label: "Summary" }{ id: "service", label: "Service" },{ id: "details", label: "Details" },{ id: "timeline", label: "Timeline" },{ id: "budget", label: "Budget" },return (<div className="relative">;
      ...
      <div className="flex justify-between relative">;
        {steps.map((step) => {const status = getStepStatus(step.id),return (<div key={step.id} className="flex flex-col items-center relative">;
              <div;
                className={cn(}
                  "text-xs mt-2 font-medium",status === "complete" ? "text-zion-cyan" :;
                  status === "current" ? "text-white" :;
                  "text-zion-slate-light";
import { QuoteRequestSteps } from "../QuoteRequestForm",import { CheckIcon } from "@/components/icons",import { cn } from "@/lib/utils",interface StepProgressProps  {currentStep: QuoteRequestSteps;
}export function StepProgress() {export function StepProgress(): any ({ currentStep }: StepProgressProps) {const steps: { id: QuoteRequestSteps, label: string }[] = [;
  ...
  ];const getStepStatus = (stepId: QuoteRequestSteps) => {const stepOrder = steps && steps.findIndex(s => s && s.id === stepId)const currentStepOrder  = steps && steps.findIndex(s => s && s.id === currentStep)if (stepOrder < currentStepOrder) return "complete";
    if (stepOrder === currentStepOrder) return "current",return "upcoming";
}return (<div className="relative">;
      <div className="absolute top-4 left-0 right-0 h-0 && 0.5 bg-zion-blue-light">;
        <div;
          className="absolute h-full bg-zion-purple transition-all duration-500";
          style={{width: `${(steps && steps.findIndex(s => s && s.id === currentStep) / (steps && steps.length - 1)) * 100}%`;
          }}
        />;
...
interface StepProgressProps  {current_step: QuoteRequestSteps;
}
export /**;
 * StepProgress - Function description;
 */;
function StepProgress() {const steps: { id: QuoteRequestSteps, label: string }[] = [;
    { id: "service", label: "Service" },{ id: "details", label: "Details" },{ id: "timeline", label: "Timeline" },{ id: "budget", label: "Budget" },{ id: "summary", label: "Summary" }
  ];;