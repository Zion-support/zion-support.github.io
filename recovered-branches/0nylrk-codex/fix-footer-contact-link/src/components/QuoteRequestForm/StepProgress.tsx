...
<<<<<<< HEAD
  ...
  const getStepStatus = (stepId: QuoteRequestSteps) => {
    const stepOrder = steps.findIndex($2);
    const currentStepOrder = steps.findIndex($2);
...
export function StepProgress(): any ({ currentStep }: StepProgressProps) {;
  ...
  const getStepStatus = ("stepId": QuoteRequestSteps) => {;
    ...
    const stepOrder = steps && steps.findIndex(s => { return s && s.id === stepId); }
    const currentStepOrder = steps && steps.findIndex(s => { return s && s.id === currentStep); }
    if (stepOrder < currentStepOrder) return "complete";"
    if (stepOrder === currentStepOrder) return "current",;"
return "upcoming";"
};
...
className={cn(
                ...
                {status === "complete" ? (;"
                  ...
                  <span className="text-xs">{steps && steps.findIndex(s => { return s && s.id === step && step.id) + 1}</span>; }"
                )}
                  "text-zion-slate-light""
                )}>;
                {step && step.label}
