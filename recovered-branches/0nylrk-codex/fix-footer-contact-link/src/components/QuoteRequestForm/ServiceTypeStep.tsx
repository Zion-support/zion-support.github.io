...
interface ServiceTypeStepProps  {formData: QuoteFormData;
  updateFormData: (data: Partial<QuoteFormData>) => void;}// Sample data - would come from an API in a real application;
const SAMPLE_LISTINGS: ListingItem[] = [;
  {id: "service-1",title: "AI Development",category: "Services",image:;
      "https://images.unsplash.com/photo-1516192518150-0d8fee5425e3?w=800&auto=format";
      "https://images.unsplash.com/photo-1516192518150-0d8fee5425e3?w=800&auto=format";
  },{id: "service-2",title: "Cloud Migration",category: "Services",image:;
      "https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=800&auto=format";
      "https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=800&auto=format";
  },{id: "talent-1",title: "AI Engineer",category: "Talents",image:;
      "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=800&auto=format";
      "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=800&auto=format";
  },{id: "talent-2",title: "Data Scientist",category: "Talents",image:;
      "https://images.unsplash.com/photo-1573497491765-dccce02b29df?w=800&auto=format";
      "https://images.unsplash.com/photo-1573497491765-dccce02b29df?w=800&auto=format";
  },{id: "equipment-1",title: "Workstation",category: "Equipment",image:;
      "https://images.unsplash.com/photo-1547082299-de196ea013d6?w=800&auto=format";
      "https://images.unsplash.com/photo-1547082299-de196ea013d6?w=800&auto=format";
  },{id: "equipment-2",title: "Server Rack",category: "Equipment",image:;
      "https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&auto=format";
  }
      "https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&auto=format";
  }
];export function ServiceTypeStep() {const [searchQuery, setSearchQuery]  = useState("")const handleTypeSelect = (type: ServiceType) => {updateFormData({ serviceType: type })}const handleItemSelect = (item: ListingItem) => {updateFormData({specificItem: item,serviceCategory: item.category,serviceType: item.category.toLowerCase() as ServiceType;
      serviceType: item.category.toLowerCase() as ServiceType;
    })}import { useState } from "react",import { QuoteFormData, ListingItem, ServiceType } from "@/types/quotes",import { Input } from "@/components/ui/input",import { Card } from "@/components/ui/card",  const handleItemSelect = (item: ListingItem) => {updateFormData({specificItem: item,serviceCategory: item.category,serviceType: item.category.toLowerCase() as ServiceType;
    })}const filteredListings = SAMPLE_LISTINGS.filter((item) => {// Filter by category only when a service type has been selected;
    ...
    return (item.title.toLowerCase().includes(searchQuery.toLowerCase()) ||;
      ...
      {formData.serviceType && (<div className="space-y-4">;
          ...
          <div className="grid grid-cols-1 gap-4 mt-4">;
            {filteredListings.length > 0 ? (filteredListings.map((item) => (<div;
                  ...
                  className={`cursor-pointer transition-all ${formData.specificItem?.id === item.id;
      ...
      )}</div>;))on_click={() => handleTypeSelect ("service")}
          >;
            <h4 className="font - medium text - white">Services</h4>;
            <p className="text - sm text - zion - slate - light">;
          <div className="relative">;
          ...
          <Card;
            className={`p - 4 cursor - pointer border - 2 transition - colors ${form_data.service_type === "talent";
                ? "bg - zion - purple / 20 border - zion - purple";
                : "bg - zion - blue - light / 20 border - zion - blue - light hover:border - zion - purple / 50";
            }`}
          ...
          <Card;
            className={`p - 4 cursor - pointer border - 2 transition - colors ${form_data.service_type === "equipment";
                ? "bg - zion - purple / 20 border - zion - purple";
                : "bg - zion - blue - light / 20 border - zion - blue - light hover:border - zion - purple / 50";
            }`}
      ...
      {form_data.service_type && (<div className="space - y-4">;
          <h3 className="text - xl font - semibold text - white">;
        ...
        <div className="space-y-4">;
          ...
          <div className="grid grid - cols - 1 gap - 4 mt-4">;
            {filtered_listings.length > 0 ? (filtered_listings.map ((item) => (<div;
                  key={item.id}
                  on_click={() => handleItemSelect (item)}
                  className={`cursor - pointer transition - all ${form_data.specific_item?.id === item.id;
                      ? "ring - 2 ring - zion - purple rounded - lg";
                      : "";
                  }`}
                >;
