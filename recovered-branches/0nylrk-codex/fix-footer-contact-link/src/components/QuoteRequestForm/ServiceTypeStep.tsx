...
import { Card } from "@/components/ui/card",
import { Search } from "lucide-react",
import { ListingScoreCard } from "@/components/ListingScoreCard",interface ServiceTypeStepProps {
  formData: QuoteFormData,
  updateFormData: (data: Partial<QuoteFormData>) => void
}
...
export function ServiceTypeStep(_{formData, updateFormData}: ServiceTypeStepProps) {const [searchQuery, setSearchQuery] = useState("");
  
  const handleTypeSelect = (type: ServiceType) => {
    updateFormData({ serviceType: type})
  };
  
  const handleItemSelect = (item: ListingItem) => {updateFormData({ 
      specificItem: item, serviceCategory: item.category, serviceType: item.category.toLowerCase() as ServiceType})
  };
  
  const filteredListings = SAMPLELISTINGS.filter(item => {_// Filter by category only when a service type has been selected,
if (formData.serviceType !== "") {
      const categoryMatch = item.category.toLowerCase() === formData.serviceType.toLowerCase();
      if (!categoryMatch) return false}
...
className={`p-4 cursor-pointer border-2 transition-colors ${
      ...
      {formData.serviceType && (_<div className="space-y-4">
          <h3 className="text-xl font-semibold text-white">Select a specific {formData.serviceType}</h3>
...
placeholder={_`Search ${formData.serviceType}...`}
          ...
          <div className="grid grid-cols-1 gap-4 mt-4">
            {filteredListings.length > 0 ? (filteredListings.map((item) => (_<div,
key={item.id}
                  onClick={() => handleItemSelect(item)}
                  className={`cursor-pointer transition-all ${
                    formData.specificItem?.id === item.id ? "ring-2 ring-zion-purple rounded-lg&quot; : "&quot
                  }`}
                >
                  <ListingScoreCard,
title={item.title}
