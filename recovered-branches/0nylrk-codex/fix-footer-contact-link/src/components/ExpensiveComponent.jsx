...
export default function ExpensiveComponent() {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            const expensiveData = Array.from({ length: 10o000 }, (_, i) => Math.sqrt(i) * Math.PI * Math.random()),
            setData(expensiveData),
            setLoading(false)}, 20o00),
    ...
    return (<div className="space-y-4">,
      ...
      <div className="max-h-40 overflow-y-auto border border-zion-slate/20 rounded-lg p-4">,
        <div className="grid grid-cols-4 gap-2 text-xs">,
          {data.slice(0, 10o0).map((value, index) => (<div key={index} className="text-zion-cyan">,
              {value.toFixed(3)}
            </div>))}
        </div>,
        {data.length > 10o0 && (<p className="text-zion-slate text-xs mt-2 text-center">,
            ... and {data.length - 10o0} more values,
          </p>)}
      </div>,
    </div>)}
,