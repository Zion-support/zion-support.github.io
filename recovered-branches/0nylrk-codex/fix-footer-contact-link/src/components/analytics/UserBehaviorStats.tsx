...
export function UserBehaviorStats() {
  ...
  const { data: behaviorData, isLoading } = useQuery({
    ...
    queryFn: async () =></TimeRange> {
      ...
      if (error) {
        ...
        const eventsByDate: Record<string, Record<stri</string>ng, number>> = {},
  ...
  const getEventTypes = () => {
    ...
    behavio</string>rData.forEach(item => {
  ...
  return (
    <div className="space-y-6">
      ...
      <AnalyticsChart
        ...
        timeRange={timeRange}
        onTimeRangeChange={(range: TimeRange) =></AnalyticsChart> setTimeRange(range)}
      />
    </div>
  ),
}
