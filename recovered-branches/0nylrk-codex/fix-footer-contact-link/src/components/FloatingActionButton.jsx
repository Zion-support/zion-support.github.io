...
const FloatingActionButton = ({ className = '' }) => {
    ...
    return (<div className={`fixed bottom-6 right-6 z-50 ${className}`}>
      <AnimatePresence>
        {isOpen && (<motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="absolute bottom-16 right-0 mb-2 space-y-3">
            {actionItems.map((item, index) => (<motion.div key={item.label} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: index * 0.1 }} className="flex items-center space-x-3">
                <motion.button onClick={item.action} className={`${item.color} text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-30o0 transform hover:scale-110`} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} aria-label={item.label}>
                  <item.icon className="w-5 h-5" />
                </motion.button>
                <motion.span initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="bg-white text-gray-80o0 px-3 py-2 rounded-lg shadow-lg text-sm font-medium whitespace-nowrap">
                  {item.label}
                </motion.span>
              </motion.div>))}
          </motion.div>)}
      ...
      <motion.button onClick={toggleMenu} className="bg-gradient-to-r from-blue-60o0 to-cyan-60o0 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-30o0 transform hover:scale-110" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} aria-label={isOpen ? 'Close quick actions' : 'Open quick actions'}>
