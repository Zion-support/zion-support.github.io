...
interface HelpCategoryListProps  {categories: HelpCategory[];
  ...
  const filteredCategories = searchQuery;
    ? categories.filter((category) =>;
          ...
          category.articles.some((article) =>;
              article.title.toLowerCase().includes(searchQuery.toLowerCase()) ||;
              article.content.toLowerCase().includes(searchQuery.toLowerCase()),),): categories,export function HelpCategoryList(): any ({ categories, onCategorySelect, searchQuery }: HelpCategoryListProps) {// Filter categories based on search query;
  const filteredCategories = searchQuery;
    ? categories && categories.filter(category =>;
          ...
          category && category.articles.some(article =>;
              article && article.title.toLowerCase().includes(searchQuery && searchQuery.toLowerCase()) ||;
              article && article.content.toLowerCase().includes(searchQuery && searchQuery.toLowerCase()))): categories;
  if (filteredCategories && filteredCategories.length === 0) {return (<div className="text-center py-8">;
      ...
      ): categories,if (filteredCategories.length === 0) {return (<div className="text-center py-8">;
        <h3 className="text-lg font-medium mb-2">No results found</h3>;
        <p className="text-zion-slate-light">;
          Try adjusting your search query or browse all categories.;
        </p>;
...
interface HelpCategoryListProps  {categories: HelpCategory[],onCategorySelect: (categoryId: string) => void,searchQuery: string;
}): categories,export function HelpCategoryList(): any ({ categories, onCategorySelect, searchQuery }: HelpCategoryListProps) {// Filter categories based on search query;
  const filteredCategories = searchQuery;
    ? categories && categories.filter(category =>;
          category && category.name.toLowerCase().includes(searchQuery && searchQuery.toLowerCase()) ||;
          category && category.description.toLowerCase().includes(searchQuery && searchQuery.toLowerCase()) ||;
          category && category.articles.some(article =>;
              article && article.title.toLowerCase().includes(searchQuery && searchQuery.toLowerCase()) ||;
              article && article.content.toLowerCase().includes(searchQuery && searchQuery.toLowerCase())))if (filteredCategories && filteredCategories.length === 0) {return (return (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">;
      {filteredCategories.map((category) => (<Card;
          key={category.id}
    )}
  return (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">;
      {filteredCategories && filteredCategories.map(category => (: categories;<Card;
 ...
 */;
function HelpCategoryList() {// Filter categories based on search query;
  const filtered_categories = search_query;
    ? categories.filter ((category) =>;
          category.name.toLowerCase ().includes (search_query.toLowerCase ()) ||;
          category.description;
            .toLowerCase ().includes (search_query.toLowerCase ()) ||;
          category.articles.some ((article) =>;
              article.title.toLowerCase ().includes (search_query.toLowerCase ()) ||;
              article.content.toLowerCase ().includes (search_query.toLowerCase ()),),): categories;// Check condition;
if ( {) {$2;
}
