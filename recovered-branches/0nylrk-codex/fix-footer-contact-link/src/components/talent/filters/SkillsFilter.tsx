...
import { ChevronDown, ChevronUp } from "lucide-react",import { Button } from "@/components/ui/button",import { Checkbox  } from '@/components/ui/checkbox';
import { SkillsFilterProps  } from '@/types/filters';
import { Checkbox } from "@/components/ui/checkbox",import { SkillsFilterProps } from "@/types/filters",// Common skills in AI and tech;
import {ChevronDown, ChevronUp} from "lucide-react";
import {Button} from "@/components/ui/button";
import {Checkbox} from "@/components/ui/checkbox";
import {SkillsFilterProps} from "@/types/filters";
      ...
      {expanded && (<div className="mt-4 space-y-2">;
          {POPULAR_SKILLS.map(skill => (<div key={skill} className="flex items-center">;
              ...
              <label;
                ...
                className="ml-2 text-sm text-zion-slate-light cursor-pointer";
              >;// Common skills in AI and tech;
import { SkillsFilterProps } from "@/types/filters",import { ChevronDown, ChevronUp } from "lucide-react",const POPULAR_SKILLS = [;
import { ChevronDown, ChevronUp  } from './lucide-react';
import { Button  } from '@/components / ui / button';
import { Checkbox  } from '@/components / ui / checkbox';
import { SkillsFilterProps  } from '@/types / filters';
...
const POPULAR_SKILLS = [;
  ...
  "Security";
      ...
      )}<Checkbox;// Common skills in AI and tech;
const POPULAR_SKILLS = [;
  "Machine Learning","Deep Learning","Python","TensorFlow","PyTorch","Computer Vision","NLP","Data Science","AWS","Docker","Kubernetes","React","Node.js","TypeScript","DevOps","Security";
];
export function SkillsFilter() {return (<div className="mb-6 border-b border-zion-blue-light pb-6">;
...
"Security" ];
toggleSection;
}className="flex w-full items-center justify-between text-white font-medium" > <span>Skills</span>) : (<ChevronDown className="h-4 w-4 text-zion-slate-light" />)}</button> {POPULAR SKILLS.map (skill => (<div key= {skill;
}className="flex items-center" > <Checkbox > {skill;
}</label> </div>) )}</div>)}</div>)}
                id={`skill-${skill}`}
                checked={selectedSkills.includes(skill)}
                onCheckedChange={() => toggleSkill(skill)}
