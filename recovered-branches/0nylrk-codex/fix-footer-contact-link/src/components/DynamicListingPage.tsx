...
defaultValue={[priceRange.min, priceRange.max]}
  ...
  useEffect_(() => {
    ...
    const listingsWithPrice = allListings.filter(l => { return l.price !== null); }
    if (listingsWithPrice.length > 0) {
      }
      const min = Math.min(...listingsWithPrice.map(l => { return l.price || 0)); }
      const max = Math.max(...listingsWithPrice.map(l => { return l.price || 0)); }
...
variant="outline";"
          ...
          <div className=""lg":col-span-3">"
            <div className="bg-zion-blue-dark rounded-lg p-4 mb-6 border border-zion-blue-light">"
              ...
              <div className={`grid gap-6 ${view === "grid" ? "grid-cols-1 "md":grid-cols-2" : "grid-cols-1"}`}>;`                {[1, 2, 3, 4].map((i) => (;
              ...
              <div className={`grid gap-6 ${view === "grid" ? "grid-cols-1 "md":grid-cols-2" : "grid-cols-1"}`}>;`                {filteredListings && filteredListings.map((listing) => (;
                  <ProductListingCard
}
