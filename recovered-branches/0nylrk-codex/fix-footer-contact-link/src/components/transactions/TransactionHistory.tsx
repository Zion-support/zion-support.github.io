<<<<<<< HEAD
<<<<<<<< HEAD:recovered-branches/0nylrk-codex/fix-footer-contact-link/src/components/transactions/TransactionHistory.tsx
...
========
<<<<<<< HEAD
>>>>>>>> origin/cursor/expand-services-advertise-and-build-project-dbb7:backup-problematic-files/recovered-branches/0nylrk-codex/fix-footer-contact-link/src/components/transactions/TransactionHistory.tsx
import React, { useState } from "react";
import {useQuery} from "@tanstack/react-query";
import {supabase} from "@/integrations/supabase/client";
import {useAuth} from "@/hooks/useAuth";
...
function TransactionHistory() {
  ...
  const [filter, set_filter] = useState<'all' | 'pending' | 'completed' | 'escrow'>('all');
;
...
if (throw error) {
  $2
    ...
    enabled: !!user}),
  const handleManageTransaction = async (transaction_id: string, action: 'release' | 'refund' | 'cancel') => {
    try {
      const { data, error } = await supabase.functions.invoke ('manage - transaction', {
