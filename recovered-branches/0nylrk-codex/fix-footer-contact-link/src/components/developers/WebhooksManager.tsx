...
export function WebhooksManager() {const {
    webhooks, loading, testResult, fetchWebhooks, createWebhook, toggleWebhook, deleteWebhook, testWebhook, clearTestResult} = useWebhooks();
...
const [webhookName, setWebhookName] = useState(""),
  ...
  const [testEventType, setTestEventType] = useState<WebhookEventType>('newapplication'),
...
const eventOptions: { value: WebhookEventType, label: string, description: string }[] = [
    { value: 'newapplication', label: 'New Application', description: 'When a talent applies to a job' },
    { value: 'quotereceived', label: 'Quote Received', description: 'When a quote is received from talent' },
    { value: 'milestoneapproved', label: 'Milestone Approved', description: 'When a project milestone is approved' },
    { value: 'talenthired', label: 'Talent Hired', description: 'When talent is hired for a project' }],
    setShowCreateDialog(false);
    resetWebhookForm()};

  const handleToggleStatus = async (webhookId: string, currentStatus: boolean) => {await toggleWebhook(webhookId, _!currentStatus)};

  const handleDeleteWebhook = async (webhookId: string) => {await deleteWebhook(webhookId);
    setShowDeleteConfirm(null)};

  const handleTestWebhook = async (webhookId: string) => {await testWebhook(webhookId, testEventType);
    setShowTestResult(true)};

  const resetWebhookForm = () => {setWebhookName("");
    setWebhookUrl("");
    setWebhookSecret("");
    setSelectedEvents([])};
      ...
      <CardContent>
        <div className="flex justify-between items-center mb-6">
          <p className="text-sm text-zinc-400&quot;>
            You have {webhooks.length} {webhooks.length === 1 ? 'webhook' : 'webhooks'}          </p>
          
          <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
            <DialogTrigger asChild>
              <Button variant=&quot;default">
                <Plus size={16} className="mr-1" /> Add Webhook              </Button>
            </DialogTrigger>
...
id=&quot;webhook-name&quot;
                    ...
                    placeholder=&quot;e.g. Application Notifications"                    className="bg-zinc-800 border-zinc-700"
                  />
                </div>
...
id=&quot;webhook-secret&quot;
                    ...
                    placeholder=&quot;••••••••••••••••"                    className="bg-zinc-800 border-zinc-700"
                  />
                  <p className="text-xs text-zinc-500">
...
htmlFor={event.value}
                        ...
                        >
                          {event.label}
                          <span className="block text-xs text-zinc-400 mt-1&quot;>{event.description}</span>                        </Label>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="space-y-4">
          ...
          ) : (webhooks.map((webhook) => (_<div key={webhook.id} className="p-4 border border-zinc-800 rounded-lg">
...
checked={webhook.isactive}
                        onCheckedChange={_() => handleToggleStatus(webhook.id, webhook.isactive)}
                      />
                      <span className="ml-2 text-sm&quot;>
                        {webhook.isactive ? &quot;Active&quot; : &quot;Inactive&quot}                      </span>
                    </div>
...
onClick={_() => setShowDeleteConfirm(webhook.id)}
                ...
                <div className="mt-3 flex flex-wrap gap-2&quot;>
                  {webhook.eventtypes.map((event) => (                    <Badge,
key={event} 
                      variant=&quot;secondary"
                ...
                <div className="mt-3 text-xs text-zinc-500 flex items-center space-x-4">
                  <span>Created: {format(new Date(webhook.createdat), _'MMM d, yyyy')}</span>
                  {webhook.lasttriggered_at && (
                    <span>Last triggered: {format(new Date(webhook.lasttriggered_at), _'MMM d, yyyy HH:mm')}</span>
                  )}
                </div>
              </div>
            ))
...
open={showTestDialog !== null} 
        onOpenChange={_(open) => {
          if (!open) {
            ...
            setTestEventType('newapplication'),
            if (showTestResult) {
...
value={testEventType}
              ...
              <DialogFooter>
                <Button variant=&quot;outline" onClick={() => setShowTestDialog(null)}>                  Cancel
                </Button>
...
className={
                  ...
                  <div className="space-y-2 mt-4">
                    ...
                    <ScrollArea className="h-[200px] rounded border border-zinc-800 bg-black p-4">
                      <pre className="text-xs font-mono text-zinc-300 whitespace-pre-wrap break-all&quot;>
                        {testResult?.responseBody || &quot;No response body&quot}                      </pre>
                    </ScrollArea>
                  </div>
                </div>
              </div>
              ...
              <DialogFooter>
                ...
                <Button variant="outline" onClick={() => {
                  setShowTestResult(false),
                  clearTestResult()                  setShowTestDialog(null);
                  setShowTestResult(false);
                  clearTestResult()}}>
                  Close
                </Button>
