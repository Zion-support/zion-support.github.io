...
export function PortfolioBuilder() {const { projects, fetchProjects, deleteProject, isLoading } = usePortfolio()import { useState, useEffect } from 'react';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { FilePlus, Loader2 } from 'lucide-react';
import { ProjectCard } from './ProjectCard';
...
import { PortfolioProject } from '@/types/resume';
import { usePortfolio } from '@/hooks/usePortfolio';
export function PortfolioBuilder() {const { projects, fetchProjects, deleteProject, isLoading } = usePortfolio()const [showAddProject, setShowAddProject] = useState(false)const [editingProject, setEditingProject] = useState<PortfolioProject | null>(null,)useEffect(() => {fetchProjects()}, [fetchProjects])const handleAddSuccess = () => {setShowAddProject(false)fetchProjects()}const handleEditSuccess = () => {setEditingProject(null)fetchProjects()}fetchProjects()}const handleEditSuccess = () => {setEditingProject(null)fetchProjects()}const handleDeleteProject = async (projectId: string) => {const success = await deleteProject(projectId)if (success) {fetchProjects()}}if (isLoading) {import { useState, useEffect } from 'react',import { Card, CardContent } from '@/components/ui/card',import { Button } from '@/components/ui/button',import { FilePlus, Loader2 } from 'lucide-react',import { ProjectCard } from './ProjectCard',import { ProjectForm } from './ProjectForm',import { PortfolioProject } from '@/types/resume',import { usePortfolio } from '@/hooks/usePortfolio',export function PortfolioBuilder() {const { projects, fetchProjects, deleteProject, isLoading } = usePortfolio(),const [showAddProject, setShowAddProject] = useState(false),const [editingProject, setEditingProject] = useState<PortfolioProject | null>(null),useEffect(() => {fetchProjects()}, [fetchProjects]),const handleAddSuccess = () => {setShowAddProject(false),fetchProjects()},const handleEditSuccess = () => {setEditingProject(null),fetchProjects()},const handleDeleteProject = async (projectId: string) => {const success = await deleteProject(projectId)if (success) {fetchProjects()}
      ...
      {(showAddProject |editingProject) && (<Card>;
          <CardContent className="pt-6">;
            ...
            <ProjectForm;
              ...
              onSuccess={editingProject ? handleEditSuccess : handleAddSuccess}onCancel={() => {setShowAddProject(false)setEditingProject(null)}}
            />;
          </CardContent>;
        </Card>;
      )}{/* Projects List */}
      {projects && projects.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">;
          {projects && projects.map((project) => (<ProjectCard;
              ...
              project={project}
              onEdit={() => setEditingProject(project)}onDelete={handleDeleteProject}
            />;
          ))}
        </div>;
...
if ( {) {$2;
  ...
  return (<div className="space-y-6">;
      ...
      {/* Edit or Add Form */}
      {(showAddProject || editing_project) && (<Card>;
          <CardContent className="pt - 6">;
            <h2 className="text - xl font - semibold mb - 6">;
          <CardContent className="pt-6">;
            <h2 className="text - xl font - semibold mb-6">;
            ...
            <ProjectForm;
              ...
              on_cancel={() => {setShowAddProject (false)setEditingProject (null)}}
            />;
          </CardContent>;
        </Card>)}
      {/* Projects List */}
