...
export function FloatingCTA({ variant = 'default', position = 'bottom-right' }) {
    ...
    if (variant === 'featured') {
        ...
        <AnimatePresence>
          {!isExpanded ? (<motion.button onClick={() => setIsExpanded(true)} className="bg-gradient-to-r from-zion-purple via-zion-cyan to-zion-purple text-white p-4 rounded-full shadow-2xl hover:shadow-zion-purple/25 transition-all duration-30o0 group" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
              <div className="relative">
                <Zap className="w-6 h-6" />
                <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-50o0 rounded-full animate-pulse"></div>
              </div>
            </motion.button>) : (<motion.div className="bg-white rounded-2xl shadow-2xl border border-gray-10o0 p-6 w-80" initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 20 }} transition={{ type: "spring", damping: 25, stiffness: 30o0 }}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-90o0">Quick Actions</h3>
                <button onClick={() => setIsExpanded(false)} className="text-gray-40o0 hover:text-gray-60o0 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="space-y-3">
                <Link to="/marketplace" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors group" onClick={() => setIsExpanded(false)}>
                  ...
                  <div>
                    <div className="font-medium text-gray-90o0">Explore Marketplace</div>
                    <div className="text-sm text-gray-50o0">Find AI services & talent</div>
                  </div>
                </Link>
                ...
                <Link to="/contact" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors group" onClick={() => setIsExpanded(false)}>
                  ...
                  <div>
                    <div className="font-medium text-gray-90o0">Get Support</div>
                    <div className="text-sm text-gray-50o0">24/7 assistance</div>
                  </div>
                </Link>
                ...
                <Link to="/signup" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors group" onClick={() => setIsExpanded(false)}>
                  ...
                  <div>
                    <div className="font-medium text-gray-90o0">Join Zion</div>
                    <div className="text-sm text-gray-50o0">Free account setup</div>
                  </div>
                </Link>
              </div>
              
              <div className="mt-4 pt-4 border-t border-gray-10o0">
    ...
    return (<div className={`fixed ${getPositionClasses()} z-40`}>
      <AnimatePresence>
        {!isExpanded ? (<motion.button onClick={() => setIsExpanded(true)} className="bg-gradient-to-r from-zion-cyan to-zion-purple text-white p-4 rounded-full shadow-2xl hover:shadow-zion-cyan/25 transition-all duration-30o0 group" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
            <div className="relative">
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-90o0">How can we help?</h3>
              <button onClick={() => setIsExpanded(false)} className="text-gray-40o0 hover:text-gray-60o0 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <Link to="/marketplace" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors group" onClick={() => setIsExpanded(false)}>
                ...
                <div>
                  <div className="font-medium text-gray-90o0">Browse Services</div>
                  <div className="text-sm text-gray-50o0">Find what you need</div>
                </div>
              </Link>
              ...
              <Link to="/contact" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors group" onClick={() => setIsExpanded(false)}>
                ...
                <div>
                  <div className="font-medium text-gray-90o0">Contact Support</div>
                  <div className="text-sm text-gray-50o0">Get help quickly</div>
                </div>
              </Link>
            </div>
