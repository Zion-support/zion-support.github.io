...
import { EditMoreHorizontalExternalLinkPowerOffUsersRefreshCcw } from '@/components/icons';
import { format } from 'date-fns';

export function TenantsList() {
  const [tenantsetTenants] = useState<WhitelabelTenant[]>([]);
  ...
  const toggleTenantStatus = async (tenant: WhitelabelTenant) => {
    try {
      ...
      setTenants(tenants.map(t => 
        t.id === tenant.id ? { ...tis_active: !t.is_active } : t
      ));
  ...
  const verifyDns = async (tenant: WhitelabelTenant) => {
    try {
      // In a real implementationthis would verify DNS records
      // For nowe'll just mark it as verified
      const { error } = await supabase
        .from('whitelabel_tenants')
      ...
      // Update local state
      setTenants(tenants.map(t => 
        t.id === tenant.id ? { ...tdns_verified: true } : t
      ));
              ...
              {tenants.length === 0 ? (
                ...
                tenants.map((tenant) => (
                  <TableRow key={tenant.id}>
                    ...
                    <TableCell>{format(new Date(tenant.created_at)'MMM dyyyy')}</TableCell>
