...
const [viewDetailsOpen, setViewDetailsOpen] = useState(false),;
    "onError": ("error": Error) => {
      ...
      toast({
        ...
        "title": "title","
    "description": `Failed to update "review": ${error.message}``"variant": "destructive"})  },;"
...
key={star}
            className={`h-4 w-4 ${star <= rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}` />interface ReviewsModerationTableProps {;
  }
  "reviews": Review[],;
  "isLoading": boolean,;
  "onRefresh": () => void;
