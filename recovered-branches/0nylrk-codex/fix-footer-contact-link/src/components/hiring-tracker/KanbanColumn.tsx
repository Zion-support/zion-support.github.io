...
import { CandidateCard } from "./CandidateCard",interface KanbanColumnProps {
  id: string,
  title: string,
  description: string,
...
interface KanbanColumnProps {id: string,
  title: string,
  description: string,
  applications: JobApplication[];
  count: number}

export function KanbanColumn(_{id, title, description, applications, count}: KanbanColumnProps) {_// Add color based on column type,
const getBadgeVariant = (columnId: string) => {
  ...
  const getColumnBgColor = (columnId: string) => {
    switch (columnId) {
      case "hired": return "bg-green-50",      case "rejected":
        return "bg-red-50",
      default:
        return "bg-muted/30"    }
  },
...
key={application.id}
              ...
              {applications.length === 0 && (
                <div className=&quot;h-full flex items-center justify-center border-2 border-dashed border-muted rounded-md p-4&quot;>
                  <p className=&quot;text-center text-sm text-muted-foreground&quot;>                    Drag candidates here
                  </p>
                </div>
              )}
            </div>
