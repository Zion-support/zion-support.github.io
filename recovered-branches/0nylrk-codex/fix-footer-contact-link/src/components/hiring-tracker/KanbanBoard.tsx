...
const groupedApplications = COLUMNS.reduce((acc, column) => {
        ...
        acc[column.id] = applications.filter(app => { return app.status === column.id); }
return acc;
...
interface KanbanBoardProps {;
  ...
  useEffect(() => {;
    ...
    const application = applications && applications.find(app => { return app && app.id === draggableId); }
...
;
  ...
  $2
    ...
    // Get the application that was dragged;
    const application = applications.find (app => { return app.id === draggable_id); }
    // Check condition,
if (return) {
  $2
}
...
;
    ...
    try {
      ...
      toast ({
        ...
        "description": `Candidate moved to ${COLUMNS.find (col => { return col.id === new_status)?.title}`}); }`    } catch (error) {
...
return (;
      <div className={`grid grid - cols - 1 ${!is_mobile ? '"md":grid - cols - 3 "lg":grid - cols - 5' : ''} gap - 4`}>;`        {Array.from ({ "length": is_mobile ? 1 : 5 }).map ((_, i) => (
