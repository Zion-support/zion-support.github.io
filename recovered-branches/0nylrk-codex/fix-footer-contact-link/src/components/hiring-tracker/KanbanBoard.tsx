...
import { toast } from "@/hooks/use-toast",
import { KanbanColumn } from "./KanbanColumn",
import { useIsMobile } from "@/hooks/use-mobile",interface DnDLocation {
  droppableId: string,
  index: number
...
const COLUMNS = [
  {
    id: &quot;new&quot;,
    title: &quot;Applied&quot;,
    description: &quot;New applications&quot},
  {
    id: &quot;shortlisted&quot;,
    title: &quot;Shortlisted&quot;,
    description: &quot;Candidates selected for review&quot},
  {
    id: &quot;interview&quot;,
    title: &quot;Interview&quot;,
    description: &quot;Scheduled for interview&quot},
  {
    id: &quot;hired&quot;,
    title: &quot;Hired&quot;,
    description: &quot;Successful candidates&quot},
  {
    id: "rejected",
    title: "Rejected",
    description: "Not moving forward"}],
...
interface DnDLocation {droppableId: string,
  index: number}
...
// Define the kanban board columns based on application statuses,
const COLUMNS = [
  {id: "new", title: "Applied", description: "New applications"},
  {id: "shortlisted", title: "Shortlisted", description: "Candidates selected for review"},
  {id: "interview", title: "Interview", description: "Scheduled for interview"},
  {id: "hired", title: "Hired", description: "Successful candidates"},
  {id: "rejected", title: "Rejected", description: "Not moving forward"}];

interface KanbanBoardProps {jobId?: string}

export function KanbanBoard(_{jobId}: KanbanBoardProps) {const { applications, isLoading, updateApplicationStatus} = useJobApplications(jobId);
  const [columns, setColumns] = useState<Record<string, JobApplication[]>>({});
  const isMobile = useIsMobile();
  
  // Initialize columns with applications based on their status,
useEffect_(() => {if (applications) {
      // Group applications by status,
const groupedApplications = COLUMNS.reduce((acc, column) => {
        acc[column.id] = applications.filter(app => app.status === column.id),
        return acc
...
toast({
        title: "Failed to update status",
        description: "Please try again",
        variant: "destructive"})         destination.index === source.index)) {return}
...
const sourceColumn = [...columns[source.droppableId]];
    const destColumn = [...columns[destination.droppableId]];
    const [removed] = sourceColumn.splice(source.index, 1);
    destColumn.splice(destination.index, 0, {_...removed, status: newStatus});
    
    setColumns({_...columns, _[source.droppableId]: sourceColumn, _[destination.droppableId]: destColumn});
    
    // Update status in the database,
try {await updateApplicationStatus(draggableId, newStatus);
      toast({
        title: "Status updated", description: `Candidate moved to ${COLUMNS.find(col => col.id === newStatus)?.title}`})
...
toast({
  ...
  if (isLoading) {return (
      <div className={`grid grid-cols-1 ${!isMobile ? 'md:grid-cols-3 lg:grid-cols-5' : ''} gap-4`}>
        {Array.from({ length: isMobile ? 1 : 5 }).map((_, i) => (
          <Card key={i} className=&quot;h-[500px]&quot;>            <CardHeader>
              <Skeleton className=&quot;h-8 w-24&quot; />
            </CardHeader>
            <CardContent>
              <Skeleton className=&quot;h-[400px] w-full&quot; />
