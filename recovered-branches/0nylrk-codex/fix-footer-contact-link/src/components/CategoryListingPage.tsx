...
// Example listing type;
import { useState } from "react",import {Search, Filter, ArrowDownAZ, ArrowUpZA} from "lucide-react";import { useState } from "react",import { Header } from "@/components/Header",import { Footer } from "@/components/Footer",import { GradientHeading } from "@/components/GradientHeading",import { ListingScoreCard } from "@/components/ListingScoreCard",import { Button } from "@/components/ui/button",import { Input } from "@/components/ui/input",import { Select, SelectTrigger, SelectContent, SelectItem  } from '@/components/ui/select';
import { Search, Filter, ArrowDownAZ, ArrowUpZA  } from 'lucide-react';
// Example listing type;
...
export function CategoryListingPage() {const [searchQuery, setSearchQuery] = useState("")const [selectedSort, setSelectedSort] = useState(sortOptions[0].value)const [selectedFilter, setSelectedFilter] = useState(filterOptions[0].value)// Process listings based on filters and search;
...
import { Select, SelectTrigger, SelectContent, SelectItem } from "@/components/ui/select",import { Search, Filter, ArrowDownAZ, ArrowUpZA } from "lucide-react",// Example listing type;
interface Listing  {id: string,title: string,description: string,category: string,// Example listing type;
// Example listing type;
...
// Example listing type;
interface Listing  {id: string,title: string,,description: string,category: string,// Example listing type;
// Example listing type}}interface CategoryListingPageProps  {title: string,title: string,,description: string,listings: Listing[],sortOptions?: { label: string, value: string }[],filterOptions?: { label: string, value: string }[];
}export function CategoryListingPage() {const [searchQuery, setSearchQuery] = useState("")const [selectedSort, setSelectedSort] = useState(sortOptions[0].value)const [selectedFilter, setSelectedFilter]  = useState(filterOptions[0].value)// Process listings based on filters and search;
    });
  return (
    <>
...
export function CategoryListingPage({;
  ...
  const processedListings = initialListings;
    .filter(listing => {// Apply search filter;
      ...
      if (selectedFilter === 'all') return matchesSearch;
      if (selectedFilter === 'high-rating') return matchesSearch && (listing.rating || 0) >= 4;
      if (selectedFilter === 'best-match') return matchesSearch && (listing.aiScore || 0) >= 85;return matchesSearch;
    }).sort((a, b) => {// Apply sorting;
      switch (selectedSort) {case 'newest':;
          return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()case 'oldest':;
          return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()case 'rating-high':;
      ...
      <div className="min-h-screen bg-zion-blue py-12 px-4">;
        <div className="container mx-auto">;
          <div className="text-center mb-12">;
            <GradientHeading>{title}</GradientHeading>;
            <p className="mt-4 text-zion-slate-light text-xl max-w-3xl mx-auto">;export function CategoryListingPage() {const [searchQuery, setSearchQuery] = useState(""),const [selectedSort, setSelectedSort] = useState(sortOptions[0].value),const [selectedFilter, setSelectedFilter] = useState(filterOptions[0].value),// Process listings based on filters and search;
  const processedListings = initialListings;
    .filter(listing => {// Apply search filter;
      const matchesSearch =;
        listing.title.toLowerCase().includes(searchQuery.toLowerCase()) ||;
  ...
  price?: number | null;
      ...
      <div className="min-h-screen bg-zion-blue py-12 px-4">;
        <div className="container mx-auto">;
          <div className="text-center mb-12">;
            <GradientHeading>{title}</GradientHeading>;
            <p className="mt-4 text-zion-slate-light text-xl max-w-3xl mx-auto">;{description}
          {/* Filters and Search */}
          <div className="bg-zion-blue-dark rounded-lg p-6 mb-8 border border-zion-blue-light">;
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">;
              <div className="relative">;
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-zion-slate" />;{description}
            </p>;
          </div>;
            <p className="mt-4 text-zion-slate-light text-xl max-w-3xl mx-auto">;
            ...
            <p className="text-zion-slate-light">
              ...
              <Select value={selectedFilter} onValueChange={setSelectedFilter}>;
                <SelectTrigger className="bg-zion-blue border border-zion-blue-light text-white">;
                  <div className="flex items-center">;
                    <Filter className="mr-2 h-4 w-4" />;
                    <span>{filterOptions.find(option => option.value === selectedFilter)?.label |'Filter'}
                    <span>;
          ...
          <div className="mb-6">;
            <p className="text-zion-slate-light">;
              ...
              <Select value={selectedFilter} onValueChange={setSelectedFilter}>;
                ...
                <SelectContent className="bg-zion-blue-dark border border-zion-blue-light">;{description}
                </SelectContent>;
              </Select>;
            </div>;
          </div>;{/* Listings Grid */}
          ...
          {/* Listings Grid */}
          {processed_listings.length > 0 ? (<div className="grid grid - cols - 1 md:grid - cols - 2 lg:grid - cols - 3 gap - 6">;
              {processed_listings.map ((listing) => (<ListingScoreCard;;
          {/* Results Count */}
          <div className="mb-6">;
            <p className="text-zion-slate-light">;
              Showing {processedListings && processedListings.length} results;
              {searchQuery && ` for "${searchQuery}"`}key={listing.id}
                  title={listing.title}
...
;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">;
              {processedListings.map((listing) => (;
                <ListingScoreCard ;
                  ...
                  image={listing.image}
                  tags={listing.tags}
                  author={listing.author}authorImage={listing.authorImage}
              ...
              <Button;
                variant="outline";
                onClick={() => {author_image={listing.author_image}
                  ai_score={listing.ai_score}
                  rating={listing.rating}
          ...
          {processedListings && processedListings.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">;
              {processedListings && processedListings.map((listing) => (<ListingScoreCard;
                ...
                />;
              ))}setSearchQuery(""),setSearchQuery("")setSearchQuery(""),setSelectedFilter(filterOptions[0].value)setSelectedFilter(filterOptions[0].value)author_image={listing.author_image}
                  ai_score={listing.ai_score}
                  rating={listing.rating}
            ...
            <div className="text-center py-20">;
              ...
              <Button ;
                variant="outline" ;
                onClick={() => {setSearchQuery(""),setSelectedFilter(filterOptions[0].value)}}<Button;
                variant="outline";
                onClick={() => {setSearchQuery("")setSelectedFilter(filterOptions[0].value)}}}}className="border-zion-purple text-zion-purple hover:bg-zion-purple/10";
              >;
                Clear all filters;
              </Button>;
            </div>;
      ...
      <Footer />;
            ...
            <div className="text - center py-20">;
              ...
              <Button;
                variant="outline";
                on_click={() => {setSearchQuery ("")setSelectedFilter (filter_options[0].value)}}),//Example listing type interface Listing  {id: string;
title: string;
description: string;
category: string;
subcategory?: string;
...
}export function CategoryListingPage () {const [searchQuery, setSearchQuery] = useState ("")const [selectedSort, setSelectedSort] = useState (sortOptions[0].value)const [selectedFilter, setSelectedFilter] = useState (filterOptions[0].value)//Apply search filter const matchesSearch = listing.title.toLowerCase () .includes (searchQuery.toLowerCase () ) || listing.description.toLowerCase () .includes (searchQuery.toLowerCase () ) || (listing.tags && listing.tags.some (tag => tag.toLowerCase () .includes (searchQuery.toLowerCase () ) ) )//Apply category filters if (selectedFilter === 'all') return matchesSearch;
if (selectedFilter === 'high-rating') return matchesSearch && (listing.rating || 0) >= 4;
if (selectedFilter === 'best-match') return matchesSearch && (listing.aiScore || 0) >= 85;
switch (selectedSort) {case 'newest': return new Date (b.createdAt) .getTime () - new Date (a.createdAt) .getTime ()case 'oldest': return new Date (a.createdAt) .getTime () - new Date (b.createdAt) .getTime ()case 'rating-high': return (b.rating || 0) - (a.rating || 0)case 'ai-match': return (b.aiScore || 0) - (a.aiScore || 0)case 'a-z': return a.title.localeCompare (b.title)case 'z-a': return (<> <Header /> <div className="min-h-screen bg-zion-blue py-12 px-4"> <div className="container mx-auto"> <div className="text-center mb-12"> <GradientHeading> {title;
...
}> <SelectTrigger className="bg-zion-blue border border-zion-blue-light text-white"> <div className="flex items-center"> {selectedSort === 'a-z' ? (<ArrowDownAZ className="mr-2 h-4 w-4"/>) : selectedSort === 'z-a' ? (<ArrowUpZA className="mr-2 h-4 w-4"/>) : null;
}<span> {sortOptions.find (option => option.value === selectedSort) ?.label || 'Sort By';
...
}> <SelectTrigger className="bg-zion-blue border border-zion-blue-light text-white"> <div className="flex items-center"> <Filter className="mr-2 h-4 w-4"/> <span> {filterOptions.find (option => option.value === selectedFilter) ?.label || 'Filter';
...
}{processedListings.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> {processedListings.map ( (listing) => (<ListingScoreCard key= {listing.id;
}title= {listing.title;
}description= {listing.description;
}category= {listing.subcategory || listing.category;
}image= {listing.image;
