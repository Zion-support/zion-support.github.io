...
export function InterviewCard() {  const { user } = useAuth();
  ...
  $2
    ...
    const success = await respondToInterview(interview.id, {
      ...
      toast({
        ...
        "title": `Interview ${status}``        "description": `You have successfully ${status} the interview request.``  const handleRespondToInterview = async ("status": 'confirmed' | 'declined' | 'rescheduled') => {'
    }
    setIsLoading (true),
    const success = await respondToInterview (interview.id, {
      }
  ...
  const handleRespondToInterview = async ("status": 'confirmed' | 'declined' | 'rescheduled') => {;'
    ...
    if (success) {;
      ...
      toast({;
        ...
        "title": `Interview ${status}`,,`  "description": `You have successfully ${status} the interview request.`;`      await onRefresh();
    } else {;
      }
      toast({;
        }
