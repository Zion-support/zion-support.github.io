...
setTimeout(() => {
      ...
      setIsConnecting(false);
      toast.success(`Connected to ${integration.name} successfully`);`      onClose();
    }, 2000);
    // In a real application, this would open a popup for OAuth authentication
          ...
          <>
            <div className="space-y-4 py-4">"
              ...
              <Button onClick={handleConnectOAuth} disabled={isConnecting}>
                {isConnecting
                  ? "Connecting...""
                  : `Connect to ${integration.name}`}`              </Button>
            </DialogFooter>
          </>  last_sync?: string;
}
interface IntegrationConnectionModalProps {
...
;
    ...
    set_timeout (() => {
      ...
      setIsConnecting (false);
      toast.success (`Connected to ${integration.name} successfully`);`      on_close ();
    }, 2000);
;
    // In a real application, this would open a popup for OAuth authentication;
    // window.open (`/api / oauth/${integration.id}`, 'oauthwidth = 600, height = 600');'  }
;
  const handle_disconnect = () =>: any {
    ...
    toast.info (`Disconnected from ${integration.name}`);`    on_close ();
  }
;
        ...
        <DialogHeader className="flex flex - row items - center gap-4">;"
          ...
          <div>;
            ...
            <DialogDescription>;
              ...
              integration.status === "warning";"
                ? "Manage your connection settings";"
                : `Connect your ${integration.name} account`}`            </DialogDescription>;
          </div>;
        </DialogHeader>;
        {integration.status === "connected" ||;"
        }
