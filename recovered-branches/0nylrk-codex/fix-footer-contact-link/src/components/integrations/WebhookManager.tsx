...
export function WebhookManager() {const {webhooks;import React, { useEffect, useState } from "react",export function WebhookManager() {const {webhooks;
    loading,import React, { useEffect, useState } from "react",import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/components/ui/card",import { Button } from "@/components/ui/button",import { Input } from "@/components/ui/input",import { Label } from "@/components/ui/label",import { ClickableBadge } from "@/components/ui/clickable-badge",import { PlusCircle, Save, Trash, Play } from "lucide-react",import { useWebhooks, WebhookEventType } from "@/hooks/useWebhooks",import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue  } from '@/components/ui/select';
import { toast  } from 'sonner';
export function WebhookManager() {const {webhooks;loading;
import React, { useEffect, useState } from "react";
import {Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle} from "@/components/ui/card";
import {Button} from "@/components/ui/button";
import {Input} from "@/components/ui/input";
...
export function WebhookManager() {
  const {
    ...
    toggleWebhook;
    deleteWebhook;
  const [newWebhook, setNewWebhook] = useState({loading  const [newWebhook, setNewWebhook] = useState({name: "";
    url: "";
...
export function WebhookManager() {;
  ...
  const [newWebhook, setNewWebhook] = useState({;
    ...
    { value: "quote_received", label: "Quote Request Received" }
    { value: "milestone_approved", label: "Milestone Approved" }
    { value: "talent_hired", label: "Talent Hired" }];useEffect(() => {fetchWebhooks()}, [])const handleAddEvent = () => {],secret: ""  ],useEffect(() => {fetchWebhooks()}, []),const handleAddEvent = () => {if (!newWebhook.selectedEvent) return;
  },if (!newWebhook.selectedEvent) return,if (newWebhook.eventTypes.includes(newWebhook.selectedEvent)) {toast.error("This event is already added")return;
    }
    ...
    })const handleRemoveEvent = (event: WebhookEventType) => {setNewWebhook({...newWebhook;
      eventTypes: newWebhook.eventTypes.filter(e => e !== event)})},const handleCreateWebhook = async () => {if (!newWebhook.name || !newWebhook.url || newWebhook.eventTypes.length === 0) {toast.error("Please fill in all required fields"),newWebhook.name,newWebhook.url,newWebhook.eventTypes,newWebhook.secret || undefined;
    ),// Reset form;
    })    // Reset form;
    setNewWebhook({name: "";
      url: "";
  ...
  const handleTestWebhook = async (webhookId: string, eventType: WebhookEventType) => {await testWebhook(webhookId, eventType)}const handleTestWebhook = async (webhookId: string, eventType: WebhookEventType) => {await testWebhook(webhookId, eventType)}
  },const handleTestWebhook = async (webhookId: string, eventType: WebhookEventType) => {await testWebhook(webhookId, eventType)},const handleTestWebhook = async (webhookId: string, eventType: WebhookEventType) => {await testWebhook(webhookId, eventType)}return (<div className="space-y-8">;
    });
  };
  const handleTestWebhook = async (webhookId: string, eventType: WebhookEventType) => {;
    await testWebhook(webhookId, eventType);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">;
            ...
            <div className="space-y-2">;
              ...
              <Input;
                id="webhook-url";
                placeholder="https://example.com/webhook";
              <Label htmlFor="webhook-url">URL</Label>import React, { useEffect, useState } from "react",import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select",import { toast } from "sonner",export function WebhookManager() {const {webhooks,loading,error,testResult,fetchWebhooks,createWebhook,toggleWebhook,deleteWebhook,testWebhook;
  ...
  const [newWebhook, setNewWebhook] = useState({name: "",url: "",selectedEvent: "" as WebhookEventType,eventTypes: [] as WebhookEventType[],secret: "";
  })const eventOptions: { value: WebhookEventType, label: string }[] = [;
    { value: "new_application", label: "New Application Received" },{ value: "quote_received", label: "Quote Request Received" },{ value: "milestone_approved", label: "Milestone Approved" },{ value: "talent_hired", label: "Talent Hired" }
  ];useEffect(() => {fetchWebhooks()}, [])const handleAddEvent = () => {if (!newWebhook && newWebhook.selectedEvent) return;if (newWebhook && newWebhook.eventTypes.includes(newWebhook && newWebhook.selectedEvent)) {toast && toast.error("This event is already added")return;
    }setNewWebhook({...newWebhook;
      eventTypes: [...newWebhook && newWebhook.eventTypes, newWebhook && newWebhook.selectedEvent];
      selectedEvent: "" as WebhookEventType;
    })}const handleRemoveEvent = (event: WebhookEventType) => {setNewWebhook({...newWebhook,eventTypes: newWebhook && newWebhook.eventTypes.filter(e => e !== event)})}const handleCreateWebhook = async () => {if (!newWebhook && newWebhook.name || !newWebhook && newWebhook.url || newWebhook && newWebhook.eventTypes.length === 0) {toast && toast.error("Please fill in all required fields")return;
    ...
    setNewWebhook({name: "",url: "",selectedEvent: "" as WebhookEventType,eventTypes: [],secret: "";
    })}const handleTestWebhook = async (webhookId: string, eventType: WebhookEventType) => {await testWebhook(webhookId, eventType)}<div className="space-y-8">;
      <Card>;
        <CardHeader>;
          <CardTitle>Create Webhook</CardTitle>;
          <CardDescription>Define webhooks to notify external systems when events occur in Zion.;
  ...
  return (
    <div className="space-y-8">;
      ...
      <div>;
        ...
        <CardContent className="space - y-4">;
          ...
          <div className="space - y-2">;
            ...
            />;
            <p className="text - xs text - muted - foreground">;If provided, this secret will be used to sign the webhook payload.;
            </p>;
          </div>;
        </CardContent>;
        <CardFooter>;
      ...
      <div>;
        <h3 className="text - lg font - medium mb-4">Your Webhooks</h3>;
          <p > Loading webhooks...</p>) : error ? (<p className="text - red-500">{error}</p>) : webhooks.length === 0 ? (<p > No webhooks configured yet. Create your first webhook above.</p>) : (<div className="space-y-4">;
            {webhooks.map (webhook => (<Card key={webhook.id}>;
                <CardHeader className="pb-2">;
                  <div className="flex justify - between items-start">;
                    <div>;
                  ...
                  <div className="flex flex-col space-y-2">;
                    <div>;
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">;
                        {webhook.event_types.map(event => (<ClickableBadge key={event} variant="secondary">;
                            {eventOptions.find(e => e.value === event)?.label || event}{eventOptions.find(e => e.value === event)?.label |event}
              ...
              <Card key={webhook && webhook.id}>;
                ...
                <CardContent className="py - 2">;
                  ...
                  >;
                    ...
                    onValueChange={(value) => handleTestWebhook(webhook && webhook.id, value as WebhookEventType)}<CardFooter className="flex justify - between pt - 2">;
                </CardContent>;
              ...
              <div className="space-y-2">;
                ...
                <div>;
                  ...
                  <pre className="mt-1 p-2 bg-gray-100 rounded text-sm overflow-x-auto">;
                    {testResult.responseBody || '<empty>'}{testResult.responseBody |'<empty>'}{testResult.responseBody || '<empty>'}</pre>;
...
newWebhook.eventTypes;
newWebhook.secret || undefined)//Reset form return (<div className="space-y-8"> <Card> <CardHeader> <CardTitle>Create Webhook</CardTitle> <CardDescription> Define webhooks to notify external systems when events occur in Zion. </CardDescription> </CardHeader> <CardContent className="space-y-4"> <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> <div className="space-y-2"> <Label htmlFor=" webhook-name">Webhook Name</Label> <Input /> </div> </div> <div className="space-y-2"> <Label>Events</Label> <div className="flex flex-wrap gap-2 mb-2"> {newWebhook.eventTypes.map (event => (<ClickableBadge key= {event;
}onRemove= {() => handleRemoveEvent (event)}> {eventOptions.find (e => e.value === event) ?.label || event;
}</ClickableBadge>) )}</div> <div className="flex space-x-2"> <Select value= {newWebhook.selectedEvent;
...
}</SelectItem>) )}</SelectContent> </Select> <PlusCircle className="h-4 w-4 mr-2"/> Add </Button> </div> </div> <div className="space-y-2"> <Label htmlFor=" webhook-secret">Secret (optional) </Label> <Input /> <p className="text-xs text-muted-foreground"> If provided, this secret will be used to sign the webhook payload. </p> </div> </CardContent> <CardFooter> <Button onClick= {handleCreateWebhook;
}> <Save className="h-4 w-4 mr-2"/> Create Webhook </Button> </CardFooter> </Card> <div> <h3 className="text-lg font-medium mb-4">Your Webhooks</h3> {loading ? (<p>Loading webhooks...</p>) : error ? (<p className="text-red-500"> {error;
...
}></div> <span className="text-sm"> {webhook.is active ? 'Active' : 'Inactive';
}</span> </div> <div className="flex-shrink-0"> <Button > {webhook.is active ? 'Disable' : 'Enable';
}</Button> </div> </div> </div> </CardHeader> <CardContent className="py-2"> <div className="flex flex-col space-y-2"> <div> <Label className="text-sm">Events</Label> </ClickableBadge>) )}</div> </div> <div className="text-xs text-muted-foreground"> {webhook.last triggered at ? `Last triggered: $ {new Date (webhook.last triggered at) .toLocaleString ()}` : 'Never triggered';
...
}</span> </div> <div> <span className="font-medium">Response:</span> <pre className="mt-1 p-2 bg-gray-100 rounded text-sm overflow-x-auto"> {testResult.responseBody || '<empty>';
