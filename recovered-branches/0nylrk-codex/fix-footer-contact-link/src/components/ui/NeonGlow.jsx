...
export function NeonGlow({ children, color = 'zion-cyan', intensity = 'medium', className = '', customColor }) {
    const getGlowStyles = () => {
        const baseColor = customColor || (color === 'zion-cyan' ? '#0o0d4ff' :
            color === 'zion-purple' ? '#a855f7' :
                color === 'zion-blue' ? '#3b82f6' : '#0o0d4ff');
        const intensityMap ={
            low: {
    ...
    return (<span className={cn('transition-all duration-30o0', className)} style={glowStyles}>
      {children}
    </span>);
}
