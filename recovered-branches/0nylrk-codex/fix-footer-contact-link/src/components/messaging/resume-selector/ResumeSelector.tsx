...
export function ResumeSelector(_{onResumeSelected}: ResumeSelectorProps) {const [selectedOption, setSelectedOption] = useState<'recent' | 'select' | 'upload'>('recent');
  const [selectedResume, setSelectedResume] = useState<ResumeOption | null>(null);
  const [resumeOptions, setResumeOptions] = useState<ResumeOption[]>([]);
  const [customFile, setCustomFile] = useState<File | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  ...
  // Fetch resume data when component mounts,
useEffect_(() => {const loadResumes = async () => {
      setIsLoading(true);
      try {
        await fetchResume()} catch (error) {} finally {setIsLoading(false)}
    };
...
useEffect_(() => {if (resume) {
      const options: ResumeOption[] = [{
        id: resume.id || 'current',
        title: resume.basicinfo.title,
        type: 'airesume',
        resume: resume
      }],      
...
const handleDownloadResume = async () => {
    if (!selectedResume || selectedResume.type !== 'airesume' || !selectedResume.resume) {
      return
    }
...
const handleGenerateResume = () => {
    window.open('/dashboard/talent/portfolioblank')
  },
...
value={selectedOption} 
        ...
        className=&quot;space-y-3&quot;
  const handleDownloadResume = async () => {if (!selectedResume || selectedResume.type !== 'airesume' || !selectedResume.resume) {
      return}
...
document.body.removeChild(link);
      ...
      toast({title: "Success!", description: "Your resume has been downloaded."})
        ...
        title: "Download failed", description: "There was an error downloading your resume.", variant: "destructive"})
    } finally {setIsLoading(false)}
  };
  
  // Handle "Generate Resume Now" button,
const handleGenerateResume = () => {window.open('/dashboard/talent/portfolio', _'blank')};
