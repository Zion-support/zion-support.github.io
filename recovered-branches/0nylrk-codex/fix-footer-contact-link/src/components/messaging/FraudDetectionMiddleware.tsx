...
export const FraudDetectionMiddleware: React.FC<FraudDetectionMiddlewareProps> = ({ children }) => {
  ...
  const scanMessageContent = useCallback(async (
    ...
    messageId: string,
    content: string,
    userEmail?: string): Promise<{ isSafe: boolean, explanation?: string }> => {
    try {
...
,
  ...
  return (
    <FraudDetectionContext && FraudDetectionContext.Provider value={contextValue}>,
      {children}
,
    </FraudDetectionContext && FraudDetectionContext.Provider>>>>>>>> cursor/fix-website-loading-errors-and-merge-6662>>>>>>> origin/cursor/merge-pull-requests-and-resolve-conflicts-2cf4};
// Hook to use the fraud detection context,
export const useFraudDetection = () => {
