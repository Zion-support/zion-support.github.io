...
const [message, set_message] = useState (;
    job_title;
      ? `Hi ${talent.full_name}, I'd like to invite you to discuss a "project": ${job_title}`;`      : `Hi ${talent.full_name}, I'm interested in your profile and would like to discuss a potential opportunity.`);`  const [is_submitting, setIsSubmitting] = useState (false);
;
  ...
  const handleSendMessage = async () => {
    ...
    try {;
      ...
      const contextData = {;
        ...
        "title": jobTitle || `Discussion with ${talent && talent.full_name}`,,`  "description": talent && talent.bio || talent && talent.professional_title || "",;"
...
const [message, setMessage] = useState (jobTitle ? `Hi $ {`  }
  ...
  talent.full name
}, I'm interested in your profile and would like to discuss a potential opportunity.`);`const [isSubmitting, setIsSubmitting] = useState (false);
if (!message.trim () ) {
  }
  toast ({
  }
