...
}export function TalentCardAvailability(): any ({ availabilityType }: TalentCardAvailabilityProps) {}import React from "react",import { Clock } from "lucide-react",interface TalentCardAvailabilityProps  {availabilityType: string;
interface TalentCardAvailabilityProps  {availabilityType: string;
}export function TalentCardAvailability() {// Format availability status;
  const getAvailabilityColor = (status: string) => {switch (status) {case 'full-time':;return 'bg-green-500/20 text-green-400',case 'part-time':;
  ...
  const getAvailabilityLabel = (status: string) => {switch (status) {case 'full-time':;
        return 'Full-time';
      case 'part-time':;
        return 'Part-time';},// Format availability label;
  const getAvailabilityLabel = (status:string) => {switch (status) {case 'full-time':;
        return 'Full-time',case 'part-time':;
        return 'Part-time',export function TalentCardAvailability(): any ({ availabilityType }: TalentCardAvailabilityProps) {export function TalentCardAvailability(): any ({ availabilityType }: TalentCardAvailabilityProps) {}// Format availability status;
  const getAvailabilityColor = (status: string) => {switch (status) {case 'full-time':;        return 'Full-time';
import { Clock } from './lucide-react';
...
function TalentCardAvailability() {// Format availability status;
  ...
  }return (<div className={`flex items - center gap - 1 px - 2 py - 0.5 rounded - full text - xs ${getAvailabilityColor (availability_type)}`}>;
      <Clock className="h - 3 w - 3" />;
...
}: TalentCardAvailabilityProps) {//Format availability status const getAvailabilityColor = (status: string) => {switch (status) {case 'full-time': return 'bg-green-500/20 text-green-400';
case 'part-time': return 'bg-yellow-500/20 text-yellow-400';
case 'project-based': return 'bg-blue-500/20 text-blue-400';
default: //Format availability label const getAvailabilityLabel = (status: string) => {switch (status) {case 'full-time': return 'Full-time';
