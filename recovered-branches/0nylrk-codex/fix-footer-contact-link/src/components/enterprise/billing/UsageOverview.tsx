...
import React from "react",import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card",import { Progress  } from '@/components/ui/progress';
import { Tabs, TabsContent, TabsList, TabsTrigger  } from '@/components/ui/tabs';
import { Progress } from "@/components/ui/progress",import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs",export function UsageOverview() {// Mock usage data;
  const usage = {jobPosts: {used: 32,total: 100,used: 32;
      total: 100;
      used: 32,total: 100,percentage: 32;
    }percentage: 32;
    }
    },used: 32,total: 100,interviews: {used: 45;
      total: 150;
...
export function UsageOverview() {;
  ...
  return (;
    ...
    }},export function UsageOverview() {// Mock usage data;
  const usage = {jobPosts: {used: 32,total: 100,percentage: 32;
    },interviews: {used: 45,total: 150,percentage: 30;
    },messaging: {used: 1250,total: 5000,percentage: 25;
    },storage: {used: 2.4,total: 10;
      percentage: 24;return (}
  }
  return (}export function UsageOverview() {// Mock usage data;
  const usage = {jobPosts: {used: 32,total: 100,percentage: 32;
    },interviews: {used: 45,total: 150,percentage: 30;
    },messaging: {used: 1250,total: 5000,percentage: 25;
    },storage: {used: 2.4,total: 10;
      percentage: 24;}},return (used: 32,total: 100,percentage: 32;
}percentage: 32;
    }
    },import React from "react",export function UsageOverview() {// Mock usage data;
  const usage = {jobPosts: {used: 32,total: 100,percentage: 32;
    },interviews: {used: 45,total: 150,percentage: 30;
    },messaging: {used: 1250,total: 5000,percentage: 25;
    },storage: {used: 2.4,total: 10;
  ...
  const usage = {jobPosts:{used:32,total:100,percentage:32;
      ...
      <CardContent>;
        <Tabs defaultValue="current">;
          <TabsList className="mb-4">;
            ...
            <div className="space-y-4">;
              <div className="space-y-2">;
                <div className="flex items-center justify-between">;
                  <p className="text-sm font-medium">Job Posts</p>;
                  <p className="text-sm text-muted-foreground">;{usage.jobPosts.used} / {usage.jobPosts.total}
                  </p>;
                </div>;
                <Progress value={usage.jobPosts.percentage} className="h-2" />;
                ...
                <div className="flex items-center justify-between">;
                  <p className="text-sm font-medium">Interviews Conducted</p>;
                  <p className="text-sm text-muted-foreground">;{usage.interviews.used} / {usage.interviews.total}
                  </p>;
                </div>;
                <Progress value={usage.interviews.percentage} className="h-2" />;
              </div>;<div className="space-y-2">;
                <div className="flex items-center justify-between">;
                  <p className="text-sm font-medium">Messages Sent</p>;
                  <p className="text-sm text-muted-foreground">;{usage.messaging.used} / {usage.messaging.total}
                  </p>;
                </div>;
                <Progress value={usage.messaging.percentage} className="h-2" />;
              </div>;<div className="space-y-2">;
                <div className="flex items-center justify-between">;
                  <p className="text-sm font-medium">Storage (GB)</p>;
                  <p className="text-sm text-muted-foreground">;{usage.storage.used} / {usage.storage.total} GB;
                  </p>;
                </div>;
                <Progress value={usage.storage.percentage} className="h-2" />;
              </div>;
          ...
          </TabsContent>;import React from './react';
import { Card, CardContent, CardDescription, CardHeader, CardTitle  } from '@/components / ui / card';
import { Progress  } from '@/components / ui / progress';
import { Tabs, TabsContent, TabsList, TabsTrigger  } from '@/components / ui / tabs';
export /**;
 ...
 */;
function UsageOverview() {// Mock usage data;
  const usage = {job_posts: {used: 32,total: 100,percentage: 32;
    }
    interviews: {used: 45,total: 150,percentage: 30;
    }
    messaging: {used: 1250,total: 5000,percentage: 25;
    ...
    storage: {used: 2.4,total: 10,percentage: 24;}
  }return (<TabsContent value="historical">;
            <div className="py-10 text-center text-muted-foreground">;
              Historical usage data will be available after your first full billing cycle.;
            </div>;
          </TabsContent>;
