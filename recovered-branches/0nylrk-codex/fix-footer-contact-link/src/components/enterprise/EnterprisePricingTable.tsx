...
import React from "react",
import { Button } from "@/components/ui/button",
import { Check, X } from "lucide-react",import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow} from "@/components/ui/table",
import { Badge } from "@/components/ui/badge",export function EnterprisePricingTable() {
  const plans = [
    {
      name: &quot;Teams&quot;,
      price: &quot;$599&quot;,
...
className={_`mt-4 w-full ${plan.popular ? 'bg-zion-purple hover:bg-zion-purple/90' : ''}`}
                        variant={plan.popular ? 'default' : 'outline'}
                      >
                        {plan.name === "Enterprise&quot; ? &quot;Request Quote&quot; : &quot;Get Started"}                      </Button>
                    </div>
                  </TableHead>
                ))}
              </TableRow>
            ...
            <TableBody>
              {plans[0].features.map(_(feature, index) => (_<TableRow key={feature.name}>
                  <TableCell className="font-medium">{feature.name}</TableCell>
                  {plans.map((plan) => (
                    <TableCell key={`${plan.name}-${feature.name}`} className="text-center">
                      {plan.features[index].included ? (
