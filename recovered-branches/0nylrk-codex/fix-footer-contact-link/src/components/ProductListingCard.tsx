...
return (;
    <div className={`bg - zion - blue - dark border border - zion - blue - light rounded - lg overflow - hidden flex ${is_grid ? 'flex - col' : 'flex - row'} cursor - pointer`} on_click={handleViewListing}>;`      {/* Image */}
      <div className={is_grid ? 'block w - full' : 'block w - 1/3'} on_click={handleViewListing}>;'
        <div className={`relative ${is_grid ? 'h - 48' : 'h - full'}`}>;`          <img;
            src={image_url}
            alt={listing.title}
            className="w - full h - full object-cover";"
      ...
      {/* Content */}
      <div className={`flex flex - col justify - between ${is_grid ? 'p - 4 flex - 1' : 'p - 4 flex - 1'}`}>;`        <div>;
          {/* Category & Rating */}
          <div className="flex justify - between items - center mb-2">;"
            <Badge variant="outline" className="bg - zion - blue - light / 20 text - zion - slate - light border - zion - blue-light">;"
              {listing.category}
