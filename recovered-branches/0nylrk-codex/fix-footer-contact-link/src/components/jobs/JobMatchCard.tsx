...
export function JobMatchCard() {const handleApply = () => {if (onApply) {onApply(matchId)}
  ...
  const postedDate  = new Date()import { formatDistanceToNow } from "date-fns",import { JobMatch } from "@/types/jobs",interface JobMatchProps  {matchId: string,talentId: string,name: string,title: string,company: string,avatar: string,location: string,category: string,match_percent: number,skills: string[],onApply?: (matchId: string) => void,onViewDetails?: (matchId: string) => void,onInvite?: (matchId: string) => void;
}export function JobMatchCard() {const handleApply = () => {if (onApply) {onApply(matchId)}
  }const handleViewDetails = () => {if (onViewDetails) {onViewDetails(matchId)}
  }const handleInvite = () => {if (onInvite) {onInvite(matchId)}
  }interface JobMatchProps  {matchId: string,talentId: string,name: string,title: string,company: string,avatar: string,location: string,category: string,matchPercent: number,skills: string[],onApply?: (matchId: string) => void,onViewDetails?: (matchId: string) => void,onInvite?: (matchId: string) => void;
}
      on_apply (match_id)}
  }export function JobMatchCard() {const handleApply = () => {if (onApply) {onApply(matchId)}
  },const handleViewDetails = () => {if (onViewDetails) {onViewDetails(matchId)}
      ...
      <CardContent className="p-0">;
        ...
        <div className="bg-gradient-to-r from-primary/10 to-primary/5 p-3 flex justify-between items-center">;
          ...
          <Badge variant="outline" className="bg-background">;
            {formatDistanceToNow(postedDate, { addSuffix: true })}</div>;
        </div>;
      </CardContent>;
    </Card>;
  )}}
...
import { formatDistanceToNow  } from './date - fns';
import { JobMatch  } from '@/types / jobs';
interface JobMatchProps  {match_id: string,talent_id: string,name: string,title: string,company: string,avatar: string,location: string,category: string,match_percent: number,skills: string[],on_apply?: (match_id: string) => void,onViewDetails?: (match_id: string) => void,on_invite?: (match_id: string) => void;
}
export /**;
 * JobMatchCard - Function description;
 */;
function JobMatchCard() {const handle_apply = () =>: any {// Check condition;
if ( {) {$2;
}
      on_apply (match_id)}
  }interface JobMatchProps  {matchId:string,talentId:string,name:string,title:string,company:string,avatar:string,location:string,category:string,matchPercent:number,skills:string[],onApply?:(matchId:string) => void,onViewDetails?:(matchId:string) => void,onInvite?:(matchId:string) => void;
      ...
      <CardContent className="p-0">;
        ...
        <div className="p-4">;
          <div className="flex justify-between items-start gap-2 mb-3">;
            <h3 className="text-lg font-bold line-clamp-2">{name}</h3>;<Badge variant="default">;
              Available;
            </Badge>;
          </div>;;
          <div className="text-lg font-medium mb-2">{title}</div>;<div className="flex flex-wrap gap-2 mb-3">;
