...
import { toast } from "@/components/ui/use-toast",interface TranslatableJobFormProps {
  ...
  const autoTranslate = async (field: 'title' | 'description' | 'requirements') => {
    ...
    try {
      ...
      toast({
        ...
        variant: "destructive"})      if (field === 'title') {_setTitle(translations);} else if (field === 'description') {_setDescription(translations);} else if (field === 'requirements') {_setRequirements(translations);}
            ...
            <TabsList className=&quot;w-full&quot;>
              {supportedLanguages.map((lang) => (
                <TabsTrigger key={lang.code} value={lang.code} className=&quot;flex-1&quot;>
                  <span className=&quot;mr-1&quot;>{lang.flag}</span> {lang.name}                </TabsTrigger>
              ))}
            </TabsList>
