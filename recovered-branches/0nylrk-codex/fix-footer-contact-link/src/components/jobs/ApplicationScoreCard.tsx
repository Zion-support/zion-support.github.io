    ...
    try {
      ...
      const checkScore = async () => {
        ...
        if (attempts < maxAttempts) {
          ...
          setIsScoring(false);
          toast.info(
            'Scoring is taking longer than expected. Check back later.'
          );
        }
      };
          ...
          <div>
            ...
            <div className='flex items-start'>
              ...
              <div>
                ...
                <Badge
                  className={getSuggestionColor(application.match_suggestion)}
                >
                  ...
                  <div className='mt-2 space-y-2 text-muted-foreground'>
                    {application.match_breakdown.skills_match && (
                      <div>
                        <p className='font-medium'>
                          Skills Match:{' '}
                          {application.match_breakdown.skills_match.score}/100
                        </p>
                        {application.match_breakdown.skills_match.matching && (
                          <p>
                            Matching skills:{' '}
                            {application.match_breakdown.skills_match.matching.join(
                              ', '
                            )}
                          </p>
                        )}
                        {application.match_breakdown.skills_match.missing && (
                          <p>
                            Missing skills:{' '}
                            {application.match_breakdown.skills_match.missing.join(
                              ', '
                            )}
                          </p>
                        )}
                    ...
                    {application.match_breakdown.experience_match && (
                      <div>
                        <p className='font-medium'>
                          Experience Match:{' '}
                          {application.match_breakdown.experience_match.score}
                          /100
                        </p>
                        <p>
                          {
                    ...
                    {application.match_breakdown.education_match && (
                      <div>
                        <p className='font-medium'>
                          Education Match:{' '}
                          {application.match_breakdown.education_match.score}
                          /100
                        </p>
                        <p>
                          {application.match_breakdown.education_match.analysis}
                        </p>
                      </div>
                    )}
                  </div>
