...
htmlFor="interest""
                ...
                <div className="grid grid-cols-1 gap-3 "sm":grid-cols-2 mt-2">"
                  {[
                    ...
                    "Networking","
                    "24/7 Support","
                    "Recycling Services"].map((item, index) => ("
                    <div key={index} className="flex items-center">"
                      <input,
id={`interest-${index}`}`                        type="checkbox""
                        className="h-4 w-4 bg-zion-blue-dark border-zion-purple text-zion-purple "focus":ring-zion-purple/50""
                      <label htmlFor={`interest-${index}`} className="ml-2 text-sm text-zion-slate-light">"import { Button } from "./ui/button";"
import { Input } from "./ui/input";"
import { GradientHeading } from "./GradientHeading",;"
import { useState } from "react",;"
export function QuoteFormSection() {;
