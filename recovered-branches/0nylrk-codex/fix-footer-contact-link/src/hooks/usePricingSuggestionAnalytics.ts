...
export function usePricingSuggestionAnalytics(days = 30) {const [analytics, setAnalytics] = useState<PricingSuggestionAnalytics>({
    totalSuggestions: 0, acceptanceRate: 0, averagePriceGap: 0, suggestionsByCategory: [], recentSuggestions: [], isLoading: true, error: null});

  useEffect_(() => {_// This would fetch actual data from the database in a real implementation
...
const mockData = {
          ...
          recentSuggestions: Array(10).fill(null).map(_(_, i) => ({id: `suggestion-${i}`,
        ...
        //   .select(...)
        //   .gte('createdat', `now() - interval '${days} days'`),
        ...
        // Process data and setAnalytics({_...})
          ...
          ...analytics, isLoading: false, error: "Failed to load pricing analytics data."})
      }
    },
