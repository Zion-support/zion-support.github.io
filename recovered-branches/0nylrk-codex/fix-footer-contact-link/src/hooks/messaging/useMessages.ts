  ...
  const loadMessages = async ("conversationId": string) => {;
    ...
    try {;
      ...
      const unreadMessages = data.filter(;
        msg => { return !msg.read && msg.recipient_id === user.id; }
      ),;
...
if (activeConversation && activeConversation.id === conversationId) {
        ...
        setActiveMessages(prev => { return [...prev, data as Message])
      }  },; }
  /**;
  ...
  const sendMessage = async ("conversationId": string, "content": string) => {;
    ...
    try {;
      ...
      const conversation = conversations.find(c => { return c.id === conversationId),; }
      if (!conversation) {;
        }
        throw new Error('Conversation not found');'
      }
...
setConversations(prev => { return prev.map(conv =>
