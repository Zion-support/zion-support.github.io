  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      const totalUnread = formattedConversations.reduce(
        ...
        0),
      setUnreadCount(totalUnread)} catch (error) {
  ...
  const createConversation = async (
    ...
    try {
      ...
      const { data: existingConversations, error: fetchError } = await supabase,
        ...
        .select('id'),
        .or(
          `and(user_one_id.eq.${user.id},user_two_id.eq.${recipientId}),and(user_one_id.eq.${recipientId},user_two_id.eq.${user.id})`),
...
,
      ...
      return conversationId} catch (error) {
