...
import { TalentProfile } from "@/types/talent",
import { toast } from "@/hooks/use-toast",
import { useAuthStatus } from "@/hooks/talent",export function useSavedTalents() {
  const { isAuthenticated, userDetails } = useAuthStatus(),
  const [savedTalents, setSavedTalents] = useState<TalentProfile[]>([]),
  const [savedTalentIds, setSavedTalentIds] = useState<string[]>([]),
  const [isLoading, setIsLoading] = useState(true),

export function useSavedTalents() {const { isAuthenticated, userDetails} = useAuthStatus();
  const [savedTalents, setSavedTalents] = useState<TalentProfile[]>([]);
  const [savedTalentIds, setSavedTalentIds] = useState<string[]>([]);
  const [isLoading, setIsLoading] = useState(true);
...
const { data: savedData, error: savedError} = await supabase
        ...
        if (savedData) {
          const talentIds = savedData.map(item => item.talentid),
          setSavedTalentIds(talentIds),          
          if (talentIds.length > 0) {
            // Fetch full talent profiles for saved talents,
const { data: talentData, error: talentError} = await supabase
  ...
  // Toggle save talent,
const toggleSaveTalent = async (talent: TalentProfile) => {if (!isAuthenticated || !userDetails.id || !talent.id) {
      toast({
        title: "Authentication required",
        description: "Please log in to save talents to your favorites",
        variant: "destructive"
    ...
    const isSaved = savedTalentIds.includes(talent.id),    
    try {if (isSaved) {
        // Remove from savedtalents,
const { error} = await supabase
          .from('savedtalents')
          .delete()
          .eq('userid', userDetails.id)
          .eq('talentid', talent.id),
        ...
        toast({
          title: "Removed from favorites",
          description: `${talent.fullname} has been removed from your favorites`})      } else {
        // Add to savedtalents,
const { error } = await supabase
          .from('savedtalents')
          .insert({
            ...
            talentid: talent.id}),
        toast({title: "Removed from favorites", description: `${talent.fullname} has been removed from your favorites`})
      } else {_// Add to savedtalents,
const { error} = await supabase
          .from('savedtalents')
          .insert({user_id: userDetails.id, talent_id: talent.id});
        ...
        toast({
          title: "Added to favorites",
          description: `${talent.fullname} has been added to your favorites`})      }
    } catch (error) {
      console.error('Error toggling saved talent:', error),
      toast({
        title: "Error",
        description: "There was a problem updating your favorites. Please try again.",
        variant: "destructive"
      })        toast({title: "Added to favorites", description: `${talent.fullname} has been added to your favorites`})
      }
    } catch (error) {toast({
