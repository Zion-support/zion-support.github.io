...
export const useTalentQuotes = () => {}
  ...
  const updateStatusMutation = useMutation({}
    ...
    onSuccess: (_, variables) => {}
      let message = "Status updated""      if (variables.status === 'in_review') {}
        message = "Quote marked as viewed""      } else if (variables.status === 'responded') {}
    ...
    onSuccess: (_, variables) => {}
      toast({}
        title: variables.isArchived ? "Quote archived" : "Quote unarchived","        description: variables.isArchived ,
          ? "The quote request has been archived" "          : "The quote request has been moved back to active quotes""      })
      queryClient.invalidateQueries({ queryKey: ['quotes', 'talent', talentId] })
    },
    onError: (error: Error) => {}
      toast({}
