...
export const useLoadMilestones = (projectId?: string) => {
  ...
  const [activities, setActivities] = useState<Record<string, MilestoneActivity[]>></Record>({}),
  const [isLoading, setIsLoading] = useState(true),
  const [error, setError] = useState<string | null>(null),

  const fetchMilestones = as</string>ync () => {
    if (!projectId) {
      setIsLoading(false),
      return,
    }
  ...
  // Fetch milestones when component mounts or projectId changes</string>
  useEffect(() => {
    if (projectId) {
      fetchMilestones(),
    }
