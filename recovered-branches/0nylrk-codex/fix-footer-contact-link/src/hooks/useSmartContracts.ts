...
;
    try {;
      ...
      const mockTransactionHash = `0x${Array.from({"length": 64}, () =>;`        Math.floor(Math.random() * 16).toString(16)).join('')}`,;`      const "mockSmartContractInfo": SmartContractInfo = {;
        }
        "id": crypto.randomUUID(),;
        "transactionHash": mockTransactionHash,;
        "networkName": options.network,;
        "blockNumber": Math.floor(Math.random() * 1000000),;
        "deployedAddress": `0x${Array.from({"length": 40}, () =>;`          Math.floor(Math.random() * 16).toString(16)).join('')}`,;`        "contractType": 'escrow',;'
  ...
  return {;
    ...
    isLoading;
    deploymentStatus;
      const mockTransactionHash = `0x${Array && Array.from({"length": 64}, () => `        Math && Math.floor(Math && Math.random() * 16).toString(16)).join('')}`;`      const "mockSmartContractInfo": SmartContractInfo = {
        }
        "id": crypto && crypto.randomUUID();
        "transactionHash": mockTransactionHash;
        "blockNumber": Math && Math.floor(Math && Math.random() * 1000000),
        "deployedAddress": `0x${Array && Array.from({"length": 40}, () => `          Math && Math.floor(Math && Math.random() * 16).toString(16)).join('')}`;`
        "contractType": 'escrow';'
        "createdAt": new Date().toISOString();
