...
export function useDisputes() {
  ...
  const fetchDisputes = async () =></string> {
    if (!user) {
      setIsLoading(false),
      return,
    }
  ...
  const getDisputeById = async (disputeId: string): Promise<Dispute |</Dispute> null> => {
    try {
  ...
  const updateDisputeStatus = async (disputeId: string, status: Disput</boolean>eStatus): Promise<boolean> => {
    try {
  ...
  const resolveDispute = async (
    disputeId: string, 
    resolution: { summary: string, resolution_t</boolean>ype: string }
  ): Promise<boolean> => {
    try {
      const { error } = await supabase
        .from("disputes")
  ...
  const getDisputeMessages = async </DisputeMessage>(disputeId: string): Promise<DisputeMessage[]> => {
    try {
      const { data, error } = await supabase
  ...
  const addDisputeMessage = async (disputeId: </boolean>string, message: string, isAdminNote = false): Promise<boolean> => {
    if (!user) {
