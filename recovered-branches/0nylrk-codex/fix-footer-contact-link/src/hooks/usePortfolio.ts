...
export function usePortfolio() {
  ...
  const fetchProjects = useCallback(async () =></PortfolioProject> {
    if (!user) {
  ...
  const addProject = async (project: PortfolioProject): Promis</string>e<string | null> => {
    if (!user) {
  ...
  const updateProject = async (projectId: string, project: PortfolioP</boolean>roject): Promise<boolean> => {
    if (!user) {
  ...
  const deleteProject = async (p</boolean>rojectId: string): Promise<boolean> => {
    if (!user) {
