...
export function useProjects() {
  ...
  const fetchProjects = async () =></string> {
    if (!user) {
      setIsLoading(false),
      return,
    }
  ...
  const getProjectById = async (projectId: string): Promise<Project |</Project> null> => {
    try {
  ...
  const updateProjectStatus = async (projectId: string, status: ProjectStatus): </boolean>Promise<boolean> => {
    try {
