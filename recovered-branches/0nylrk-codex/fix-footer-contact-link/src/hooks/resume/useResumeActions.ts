...
export function useResumeActions() {
  ...
  const createResume = async (basicInfo: ResumeBasicInfo): Promise<string | null> =></string> {
    if (!user) {
  ...
  const updateBasicInfo = async (resumeId: string, basicInfo: ResumeBasicInfo): Promise<bo</boolean>olean> => {
    if (!user) {
  ...
  const setActiveResume = async (resumeId: string): </boolean>Promise<boolean> => {
    if (!user) {
