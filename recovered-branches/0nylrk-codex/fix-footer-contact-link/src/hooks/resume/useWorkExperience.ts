...
export function useWorkExperience() {
  ...
  const addWorkExperience = async (resumeId: string, work: WorkExperience): Promise<boolean> =></boolean> {
    if (!user) {
  ...
  const updateWorkExperience = async (workId: string, work: WorkExperience): Promise<b</boolean>oolean> => {
    if (!user) {
      setError('You must be logged in to update work experience'),
      return false
    }
  ...
  const deleteWorkExperience = async (workId: string):</boolean> Promise<boolean> => {
    if (!user) {
