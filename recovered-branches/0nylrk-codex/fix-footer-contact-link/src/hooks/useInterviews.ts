...
export function useInterviews() {
  ...
  const requestInterview = async (
    interviewRequest: InterviewRequest): Promise<Interview | null> => {
    if (!user) {
      toast({
...
,
      ...
      return formattedInterviews} catch (err: any) {
...
,
      ...
      if (response.status === 'declined') {
        ...
        message = `Your interview request has been declined`} else if (response.status === 'rescheduled') {
