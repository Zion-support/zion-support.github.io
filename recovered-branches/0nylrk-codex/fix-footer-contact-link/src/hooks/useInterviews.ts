...
export function useInterviews() {
  ...
  const requestInterview = async (interviewRequest: InterviewRequest): Promise<Interview | null> =></Interview> {
    if (!user) {
      toast({
  ...
  const fetchInterviews = async (): Promise<Int</Interview>erview[]> => {
    if (!user?.id) {
      setInterviews([]),
      return [],
    }
  ...
  const cancelInterview = async (intervi</boolean>ewId: string): Promise<boolean> => {
