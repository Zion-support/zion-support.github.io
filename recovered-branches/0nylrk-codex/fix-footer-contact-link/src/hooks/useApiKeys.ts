...
setNewApiKey(result.key),
      toast({
        ...
        "title": "API Key Created","
        "description": "Your new API key has been generated. Save it now, you won't be able to see it again."})} finally {"
      }
      setLoading(false)
    }
  },
...
if ( {) {
  $2
      ...
      set_keys (prev => { return prev.map (key =>; }
...
throw new Error(result && result.error || 'Failed to fetch API logs');'
      ...
      setTotalLogs(result && result.count || 0);
      // Update the key's active status in the list;'
      setKeys(prev => { return prev.map(key =>; }
        key.id === keyId ? { ...key, "is_active": false } : key;
      )),;
      toast({;
        }
