...
import React, { useState } from "react",import { MobileHeader } from '../components/common/MobileHeader';
import { BottomNavigation } from '../components/common/BottomNavigation';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
...
import { Zap, ChevronLeft, ChevronRight } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import {Card, CardContent} from "@/components/ui/card";import React, { useState } from "react",import { MobileHeader } from "../components/common/MobileHeader",import { BottomNavigation } from "../components/common/BottomNavigation",import { Button } from "@/components/ui/button",import { Input } from "@/components/ui/input",import { Textarea } from "@/components/ui/textarea",import { Label } from "@/components/ui/label",Select,SelectContent,SelectItem,SelectTrigger,SelectValue;
} from "@/components/ui/select",import { Zap, ChevronLeft, ChevronRight } from "lucide-react",import { Badge } from "@/components/ui/badge",import { Card, CardContent } from "@/components/ui/card",type JobPostStep = "details" | "requirements" | "budget" | "preview",export function MobileJobPost() {const [currentStep, setCurrentStep] = useState<JobPostStep>("details")export function MobileJobPost() {const [currentStep, setCurrentStep]  = useState<JobPostStep>("details")const goToNextStep = () => {const [currentStep, setCurrentStep] = useState<JobPostStep>("details"),const goToNextStep = () => {if (currentStep === "details") {setCurrentStep("requirements")} else if (currentStep === "requirements") {setCurrentStep("budget")} else if (currentStep === "budget") {setCurrentStep("preview")}const goToPrevStep = () => {if (currentStep === "requirements") {setCurrentStep("details")} else if (currentStep === "budget") {setCurrentStep("requirements")} else if (currentStep === "preview") {setCurrentStep("budget")}
...
function RequirementsStep() {const [skills, setSkills] = useState<string[]>([;
  ...
  ])const [newSkill, setNewSkill] = useState("")<Textarea;
          id="description";
          placeholder="Describe the job role and responsibilities";
import {Select,SelectContent,SelectItem,SelectTrigger,SelectValue;
} from "@/components/ui/select",type JobPostStep = "details" | "requirements" | "budget" | "preview",export function MobileJobPost() {const [currentStep, setCurrentStep]  = useState<JobPostStep>("details")const goToNextStep = () => {if (currentStep === "details") {setCurrentStep("requirements")} else if (currentStep === "requirements") {setCurrentStep("budget")} else if (currentStep === "budget") {setCurrentStep("preview")}const goToPrevStep = () => {if (currentStep === "requirements") {setCurrentStep("details")} else if (currentStep === "budget") {setCurrentStep("requirements")} else if (currentStep === "preview") {setCurrentStep("budget")}
...
function DetailsStep() {return (<div className="space-y-4">;
    ...
    "React","TypeScript","Node.js";
    "Node.js";
  },const removeSkill = (skill: string) => {setSkills(skills.filter(s => s !== skill))},function RequirementsStep() {const [skills, setSkills] = useState<string[]>([;
    "React","TypeScript","Node.js";
  ...
  }const removeSkill = (skill: string) => {setSkills(skills.filter((s) => s !== skill))}setSkills(skills.filter((s) => s !== skill))}return (<div className="space-y-4">;
  ...
  return (
    <div className="space - y-4">;
      ...
      <h2 className="text-lg font-medium">Budget & Timeline</h2>;<div className="space-y-2">;
        <Label htmlFor="paymentType">Payment Type</Label>;
  const [new_skill, setNewSkill]  = useState ("")const add_skill = () =>: any {if () {) {$2;
}
      set_skills ([...skills, new_skill])setNewSkill ("")}
  }const remove_skill = (skill: string) =>: any {set_skills (skills.filter (string => s !== skill))}return (<div className="space - y-4">;
      <h2 className="text - lg font - medium">Job Requirements</h2>;
      <div className="space - y-2">;
        <Label html_for="experience">Experience Level</Label>;function RequirementsStep() {const [skills, setSkills] = useState<string[]>([;
    "React", "TypeScript", "Node.js";
  ]),const [newSkill, setNewSkill] = useState(""),const addSkill = () => {if (newSkill && !skills.includes(newSkill)) {setSkills([...skills, newSkill]),setNewSkill("")}
      ...
      <div className="space - y-2">;
        <Label html_for="education">Education</Label>;<div className="space-y-2">;
        <Label htmlFor="education">Education</Label>;
        <Select>;
          <SelectTrigger>;
            <SelectValue placeholder="Select required education" />;
      ...
      <div className="space-y-2">;
        ...
        <div className="flex flex-wrap gap-2 mb-3">;
          {skills && skills.map((skill) => (<Badge;
              key={skill}function RequirementsStep() {const [skills, set_skills] = useState < string[]>([;
    "React", "TypeScript", "Node.js";
  ])setSkills([...skills, newSkill])setNewSkill("")}
              variant="secondary";
              className="flex items-center gap-1 px-3 py-1">;
              {skill}          rows={3}
        />;
