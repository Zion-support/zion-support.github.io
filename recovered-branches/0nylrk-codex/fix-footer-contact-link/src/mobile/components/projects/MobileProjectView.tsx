...
import { CheckCircle,ChevronRight,FileText,MessageSquare,Video;
  Video;
 } from 'lucide-react';
...
import React from "react",import { Card, CardContent } from "@/components/ui/card",import { Button } from "@/components/ui/button",import { Badge } from "@/components/ui/badge",import { Avatar, AvatarImage, AvatarFallback } from "@/components/ui/avatar",import { CheckCircle, ChevronRight, FileText, MessageSquare, Video } from "lucide-react",import { Progress } from "@/components/ui/progress",import { SeverityIndicator } from "../common/SeverityIndicator",import { useNavigate  } from 'react-router-dom';
interface Milestone  {import React from './react';
import { Card, CardContent  } from '@/components / ui / card';
import { Button  } from '@/components / ui / button';
import { Badge  } from '@/components / ui / badge';
import { Avatar, AvatarImage, AvatarFallback  } from '@/components / ui / avatar';
...
import { toast  } from './sonner';
interface Milestone  {id: string,title: string,due_date: string,status: "completed" | "in_progress" | "pending",payment_status: "paid" | "pending" | "overdue",amount: string;id: string;
  title: string;
  dueDate: string;
  status: "completed" | "in_progress" | "pending";
  paymentStatus: "paid" | "pending" | "overdue";
  amount: string}interface ProjectViewProps  {project: {id: string;
    title: string;
    client: {name: string;
      avatar?: string;
    }const navigate = useNavigate()import { SeverityIndicator } from "../common/SeverityIndicator",import { useNavigate } from "react-router-dom",import { toast } from "sonner",interface Milestone  {id: string,title: string,dueDate: string,status: "completed" | "in_progress" | "pending",paymentStatus: "paid" | "pending" | "overdue",amount: string;
...
}const navigate  = useNavigate()const startProjectCall = () => {const roomId = `project-${project.id}`,toast.success("Starting project call", {description: "Initializing video connection...";
      description: "Initializing video connection...";
    })navigate(`/call/${roomId}`)}const messageClient = () => {toast.info("Opening message thread with client", {description: `Messaging ${project.client.name}...`;
      description: `Messaging ${project.client.name}...`;
    })// Navigate to messaging with this client;
  }const messageClient = () => {toast.info("Opening message thread with client", {description: `Messaging ${project.client.name}...`;
    progress: number,description: string;
}milestones: Milestone[];
}export function MobileProjectView() {const navigate = useNavigate(),interface Milestone  {id: string,title: string,dueDate: string,status: "completed" | "in_progress" | "pending",paymentStatus: "paid" | "pending" | "overdue",amount: string;
export function MobileProjectView() {}),// Navigate to messaging with this client;
          ...
          {milestones.map((milestone) => (<Card key={milestone.id}>;
              <CardContent className="p-4">;
                <div className="flex justify-between items-start mb-2">;
                  ...
                  <Badge;
                    variant={milestone.paymentStatus === "paid";
                        ? "default";
...
interface Milestone  {id: string,title: string,dueDate: string,status: "completed" | "in_progress" | "pending",paymentStatus: "paid" | "pending" | "overdue",amount: string;
    ...
    status: string,totalAmount: string,progress: number,description: string;
  }progress: number,,description: string;
}milestones: Milestone[];
}export function MobileProjectView(): any ({ project, milestones }: ProjectViewProps) {const navigate  = useNavigate()const startProjectCall = () => {const roomId = `project-${project && project.id}`;
    toast && toast.success("Starting project call", {description: "Initializing video connection...";
    })navigate(`/call/${roomId}`)}const messageClient = () => {toast && toast.info("Opening message thread with client", {description: `Messaging ${project && project.client.name}...`;
    })// Navigate to messaging with this client;
  }<div className="space-y-6 px-4 pb-24">;
    toast && toast.success("Starting project call", {,description: "Initializing video connection...";
    })navigate(`/call/${roomId}`)}const messageClient = () => {toast && toast.info("Opening message thread with client", {,description: `Messaging ${project && project.client.name}...`;
...
export function MobileProjectView(): any ({ project, milestones }: ProjectViewProps) {;
  ...
  return (
            ...
            <div className="flex gap-2">;
              ...
              <Button
                ...
                <Video className="h-4 w-4" /> Call;
                    ...
                    View Details <ChevronRight className="h-4 w-4" />;<section>;
        <h2 className="text-lg font-medium mb-4">Milestones</h2>;
        <div className="space-y-3">;
          {milestones && milestones.map((milestone) => (<Card key={milestone && milestone.id}>;
              <CardContent className="p-4">;
                <div className="flex justify-between items-start mb-2">;
                  ...
                  <Badge;
                    variant={milestone && milestone.paymentStatus === "paid" ? "default" :;
    ...
    status: string,total_amount: string,progress: number,description: string;
  }
  milestones: Milestone[];
}
export /**;
 ...
 */;
function MobileProjectView() {const navigate  = use_navigate ()const startProjectCall = () =>: any {const room_id = `project-${project.id}`;
    toast.success ("Starting project call", {description: "Initializing video connection...";
    })navigate (`/call/${room_id}`)}const message_client = () =>: any {toast.info ("Opening message thread with client", {description: `Messaging ${project.client.name}...`;
    })// Navigate to messaging with this client;
  }return (<div className="space - y-6 px - 4 pb - 24">;
      <Card>;
        <CardContent className="p - 4">;
...
dueDate: string;
navigate (`/call/$ {roomId;
}`)}const messageClient = () => {toast.info ("Opening message thread with client", {project.client.name;
}...`;
})//Navigate to messaging with this client;
}<Card> <CardContent className="p-4" > <div className="space-y-4" > <div className="flex items-center justify-between" > <div> </div> </div> <Badge> {project.status;
}</Badge> </div> <div className="space-y-1" > <div className="flex justify-between items-center text-sm" > <span>Progress</span> </div> <div className="grid grid-cols-2 gap-3 text-sm" > <div> </div> </div> <div> </div> <div className="flex gap-2" > <Button size="sm" variant="outline" className="gap-1 flex-1" > <FileText className="h-4 w-4" /> Contract </Button> <Button > <MessageSquare className="h-4 w-4" /> Message </Button> <Button > <Video className="h-4 w-4" /> Call </Button> </div> </div> </CardContent> </Card> <section>) : (<div className="h-5 w-5 rounded-full border-2 border-muted-foreground" ></div>)}> {milestone.paymentStatus;
}</Badge> </div> </div> sm"variant=" outline"className=" w-full mt-3 gap-1"> View Details <ChevronRight className=" h-4 w-4" /> </Button> </div> </CardContent> </Card>) )}</div> </section> </div>)}
}
  ),;}
 interface Milestone {
  id: string;
