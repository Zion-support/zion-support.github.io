...
"headers": { ...corsHeaders, "Content-Type": "application/json" }        }"
    ...
    const openai = new OpenAIApi(configuration),;
    const prompt = `Create a professional and detailed service description for the following "service":;"Title": ${title}`Key "Features": ${keyFeatures || "Not specified"}"
...
5. Include a compelling opening and closing statement`,`    const completion = await openai.chat.completions.create({
      }
      "model": "gpt-4o-mini","
