...
async function generateEmail(
  emailData: EmailData;
  userData: any): Promise<{ subject: string, html: string }> {
    ...
    if (emailData.onboarding_status) {
      ...
      if (user_type === 'jobSeeker' || user_type === 'creator') {
        if (!onboarding.profile_completed) {
          ...
          ctaLink = '/profile',
          ctaText = 'Complete Your Profile'} else if (!onboarding.skills_added) {
          nextAction =,
            'add your skills to get matched with the right opportunities',
          ctaLink = '/profile/skills',
          ctaText = 'Add Your Skills'} else if (!onboarding.availability_set) {
        ...
        if (!onboarding.job_posted) {
          ...
          ctaLink = '/post-job',
          ctaText = 'Post a Job'} else if (!onboarding.talent_invited) {
