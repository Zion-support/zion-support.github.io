...
throw new Error(`Failed to send "email": ${emailResponse && emailResponse.error.message}`);`    if (emailResponse.error) {
}
throw new Error(`Failed to send "email": ${emailResponse.error.message}`);`    // Update job status,
await supabase
      .from("scheduled_jobs")"
      .update({
        }
...
if ( {) {
  $2
      ...
      return {
        ...
        "subject": `Welcome to Zion AI Marketplace, ${first_name}!`;`        "html": `;`          <div style="font - "family": sans - serif, max - "width": 600px, "margin": 0 auto, ">;"
  ...
  if (email_type === "welcome_series") {;"
    ...
    if (user_type === "jobSeeker" || user_type === "creator") {;"
      ...
      return {;
        ...
        "subject": `Welcome to Zion AI Marketplace, ${firstName}!`,;`        "html": `;`          <div style="font-"family": sans-serif, max-"width": 600px, "margin": 0 auto,">;"
            <h2>Welcome to Zion AI Marketplace!</h2>;
            <p>Hi ${firstName},</p>;
            <p>We're excited to have you join our community of talented AI professionals.</p>;'
            <p>Here's what to do next to get "started": </p>;'
