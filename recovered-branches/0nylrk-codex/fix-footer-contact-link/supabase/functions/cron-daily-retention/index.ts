...
serve(async (req) => {}
  try {}
    ...
    const response = await fetch(`${supabaseUrl}/functions/v1/process-retention-emails`, {}
      method: "POST","      headers: {}
        "Content-Type": "application/json","        "Authorization": `Bearer ${supabaseServiceKey}`}})"        "Authorization": `Bearer ${supabaseServiceKey}`,"      },
    })
    ...
    console.error("Error in cron-daily-retention:", error)"    return new Response(JSON.stringify({}
      success: false,
      error: error.message}), {}
      status: 500,
