...
,
    ...
    if (req.method === 'POST') {
      if (path === 'create') {
        ...
        return await createApiKey(user.id, name, scopes, expiresAt)} else if (path === 'regenerate') {
        const { keyId } = (await req.json()) as RegenerateKeyRequest,
        return await regenerateApiKey(user.id, keyId)} else if (path === 'revoke') {
        const { keyId } = (await req.json()) as RegenerateKeyRequest,
        return await revokeApiKey(user.id, keyId)}
    } else if (req.method === 'GET') {
      if (path === 'keys') {
        return await getUserApiKeys(user.id)} else if (path === 'logs') {
