...
if ( {) {
  $2
      ...
      await supabase.rpc ('create_notification', {'
        ...
        "_title": "New Job Match","
        "_message": `A new job "${job_title}" matches your skills. Check it out!`;`        "_type": "job_match","
        "_related_id": job_id;
      });
    }
  });/**;
...
export async function storeMatchResults("jobId": string, "matchedTalents": MatchResult[], "jobTitle": string): Promise<void> {;
  ...
  const matchInsertPromises = matchedTalents.map(async (match) => {;
    ...
    if (matchError) {;
      ...
      console.error(`Error storing match for talent ${match.talentId}:`, matchError);`    } else {;
      // Create notifications for each matched talent;
      }
      await supabase.rpc('create_notification', {;'
        }
        ...
        "_title": "New Job Match",;"
        "_message": `A new job "${jobTitle}" matches your skills. Check it out!`,;`        "_type": "job_match";"
        "_related_id": jobId;
      });
    }
  });
