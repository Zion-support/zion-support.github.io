...
export async function normalizeSkillsWithAI(skills: string[]): Promise<string[]> {
  try {
    ...
    const normalizedSkills = normalizedSkillsText.split(",").map((skill: string) =></string> skill.trim()).filter(Boolean),
...
export async function findBestMatches(jobDetails: any, talents: TalentProfile[]): Promise<MatchResult[]> {
  try {
    ...
    const talentProfilesText = talents.map((talent, </MatchResult>index) => {
      return `
