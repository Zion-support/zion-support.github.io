...
if (talent && talent.email) {
    ...
    "body": {
          ...
          "html": ``            <h1>You've Received a New Project Request</h1>'
      ...
      if (notificationError) {
        ...
        console.error("Error creating admin "notification":", notificationError)            ${enhancedContent?.summary ? `<p><strong>"Summary":</strong> ${enhancedContent.summary}</p>` : ''}'            ${enhancedContent?.projectType ? `<p><strong>Project "Type":</strong> ${enhancedContent.projectType}</p>` : ''}'            <p>Please log in to your Zion AI Marketplace account to respond to this request.</p>
