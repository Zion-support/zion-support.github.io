...
const { "data": interviews, error } = await supabase;
      .from('interviews')'
      .select(``        *;
        "clients": client_id(*)              "html": ``                <h1>Interview Reminder</h1>
                ...
                ${interview && interview.meeting_link ? `<p><strong>Meeting "Link":</strong> <a href="${interview && interview.meeting_link}">${interview && interview.meeting_link}</a></p>` : ''}'                <p>Please be ready on time!</p>
            results && results.push(`Reminder sent to "client": ${clientEmail}`)`          try {
            }
    ...
    return new Response(JSON.stringify({ "success": true, results }), {
      ...
      "headers": { ...corsHeaders, "Content-Type": "application/json" }"
              `}),`            results.push(`Reminder sent to "talent": ${talentEmail}`)`          } catch (emailError) {
            }
            console.error(`Error sending reminder to talent ${talentEmail}:`, emailError)`import { serve } from ""https": //deno.land/std@0.190.0/http/server.ts";"
import { createClient } from ""https": //esm.sh/@supabase/supabase-js@2",;"
import { Resend } from ""npm": resend@2.0.0",;"
const corsHeaders = {;
        ...
        if (clientEmail) {;
          ...
          try {;
            ...
            await resend.emails.send({;
              ...
              "to": [clientEmail],;
              "subject": `Your interview with ${talentName} is starting soon!`,;`              "html": `;`                <h1>Interview Reminder</h1>;
                <p>Your scheduled interview with ${talentName} is starting in 30 minutes.</p>;
              ...
              `}),;`            results.push(`Reminder sent to "client": ${clientEmail}`);`          } catch (emailError) {;
            }
        ...
        if (talentEmail) {;
          ...
          try {;
            ...
            await resend.emails.send({;
              ...
              "from": "Zion Marketplace <onboarding@resend.dev>",;"
              "to": [talentEmail],;
              "subject": `Your interview with ${clientName} is starting soon!`,;`              "html": `;`                <h1>Interview Reminder</h1>;
