...
serve(_async (req) => {_// Handle CORS preflight requests
  ...
  try {_// Use service role key for admin privileges
    ...
    if (interviews && interviews.length > 0) {_for (const interview of interviews) {
        ...
        const interviewDate = new Date(interview.scheduled_date),        const _clientEmail = interview.clients?.email;
          ...
          try {
            await resend.emails.send({
              ...
              html: `
                ...
                ${interview.meeting_link ? `<p><strong>Meeting Link:</strong> <a href=&quot;${interview.meeting_link}&quot;>${interview.meeting_link}</Link></p>` : ''}                <p>Please be ready on time!</p>
              `}),
        ...
        const clientName = interview.clients?.display_name || "Client",            results.push(`Reminder sent to client: ${_clientEmail}`);
          } catch (emailError) {}
        }
