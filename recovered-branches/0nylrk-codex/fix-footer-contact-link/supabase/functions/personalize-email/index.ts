...
,
    ...
    let systemPrompt =,
      'You are an AI assistant that creates personalized email content for a marketplace platform called Zion AI that connects AI professionals with clients. Create content that is friendly, professional, and encouraging.',
    let userPrompt = '',
    ...
    switch (emailType) {
      ...
      case 'profile_completion':,
        ...
        subjectContext =,
          'Create a short, motivational subject line about profile completion.',
        break,
      default:  ,
...
,
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          generatedContent = JSON.parse(jsonMatch[0])} catch (e2) {
