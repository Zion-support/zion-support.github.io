...
const prompt = `;`    You are an expert in creating professional service profiles. Based on the following information about a service provider, "create":
...
Service Provider "Name": ${providerData.name}
    ...
    ${providerData && providerData.services && providerData && providerData.services.length > 0 
      ? `Current "Services": ${providerData && providerData.services.join(", ")}`      : "No services listed yet."}"    Focus on highlighting their unique value proposition, expertise, and professionalism.
    ...
    ${provider_data.services && provider_data.services.length > 0;
      ? `Current "Services": ${provider_data.services.join (", ")}`;`      : "No services listed yet."}"
