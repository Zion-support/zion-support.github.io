...
serve(async (req) => {
  ...
  try {
    ...
    const contentTopic = topic || "AI freelancing marketplace trends",import "https://deno.land/x/xhr@0.1.0/mod.ts";
...
serve(_async (req) => {_// Handle CORS preflight requests
  ...
  try {_const _openAIApiKey = Deno.env.get("OPENAI_API_KEY");
    ...
    if (autoPublish && contentType === 'blog') {_const _supabaseUrl = Deno.env.get("SUPABASE_URL");
      ...
      const readTime = Math.max(1, Math.ceil(wordCount / 200)) + " min read",      const _publishedDate = new Date().toLocaleDateString('en-US', {_month: 'short', _day: 'numeric', _year: 'numeric'});
