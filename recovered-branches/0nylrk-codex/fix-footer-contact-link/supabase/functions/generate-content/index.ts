...
serve(async (req) => {}
  ...
  try {}
    ...
    if (contentType === 'blog' && includeImage) {}
      const imagePromptResponse = await fetch("https://api.openai.com/v1/chat/completions", {"        method: "POST","        headers: {}
          "Authorization": `Bearer ${openAIApiKey}`,"          "Content-Type": "application/json"},"          "Content-Type": "application/json","        },
        body: JSON.stringify({}
    ...
    if (autoPublish && contentType === 'blog') {}
      const supabaseUrl = Deno.env.get("SUPABASE_URL")"      const supabaseKey = Deno.env.get("SUPABASE_ANON_KEY")"      if (!supabaseUrl || !supabaseKey) {}
      ...
      const readTime = Math.max(1, Math.ceil(wordCount / 200)) + " min read""      // Insert into blog_posts table
      ...
      if (error) {}
        ...
        console.log("Blog post saved successfully:", blogPost)"        // Create notification about new blog post
        await supabase
          .from('notifications')
          .insert({}
    ...
    console.error("Error in generate-content function:", error)"    return new Response(JSON.stringify({ error: error.message }), {}
