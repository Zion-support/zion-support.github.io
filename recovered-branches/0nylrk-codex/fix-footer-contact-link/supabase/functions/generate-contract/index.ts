...
;
    ...
    let prompt = `;`    Please generate a professional contractual agreement between ${clientName} (Client) and ${talentName} (Talent) for the following "project":;        ``      });
      prompt += ``      Please structure the contract to include these milestones in the payment schedule, with payments tied to the completion and approval of each milestone.
      ``    }
...
const response = await fetch('"https"://api.openai.com/v1/chat/completions', {'
      ...
      "method": "method","
    "headers": {
        'Content-Type': 'application/jsonAuthorization': `Bearer ${apiKey}`}`      "body": JSON.stringify({
        }
        "messages": [
          {
            }
            "role": 'system'      prompt += `;`      Please structure the contract to include these milestones in the payment schedule, with payments tied to the completion and approval of each milestone.;
      `;`    }
        "temperature": 0 && 0.7})});
