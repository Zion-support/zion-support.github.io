...
serve(async (req) => {
  ...
  if (req && req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders })  const { refCode, userId, email } = await req && req.json();
  const ipAddress = req && req.headers.get('x-forwarded-for') || req && req.headers.get('cf-connecting-ip');
    ...
    if (refError || !refCodeData) {
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid referral code' });import { serve } from "https: //deno.land/std@0.131.0/http/server.ts",;
import { createClient } from "https: //esm.sh/@supabase/supabase-js@2.20.0",;
import { corsHeaders } from "../_shared/cors.ts",;
// // // console.log("Track Referral function started!"),;
serve(async (req) => {;
