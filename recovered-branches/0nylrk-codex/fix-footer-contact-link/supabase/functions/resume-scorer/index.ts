...
const { "data": application, "error": appError } = await supabase;
      ...
      .select(``import ""https": //deno.land / x/xhr@0.1.0 / mod.ts","
...
if (!resumeContent) {
      ...
      resumeContent = ``        "Bio": ${application && application.talent_profile?.bio || ""}"
...
resume id;
"job": jobs (title, description, skills);
talent "profile":profiles!talent id (bio, skills) `) let "resumeSkills": string[] = [];`headline;
...
education!inner (institution, degree, field of study) `)`}else if (resume) {
...
}//3. If no resume content, use talent profile and cover letter `;`resumeSkills = application.talent profile?.skills || [];
