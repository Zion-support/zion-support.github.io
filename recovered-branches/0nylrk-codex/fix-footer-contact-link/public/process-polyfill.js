...
,
  ...
  if (typeof process === 'undefined') {'    if (typeof globalThis !== 'undefined') {'      globalThis.process = processObj} else if (typeof global !== 'undefined') {'      global.process = processObj} else if (typeof window !== 'undefined') {'      window.process = processObj} else if (typeof self !== 'undefined') {'      self.process = processObj}
  }
,
...
,
      ...
      if (typeof input === 'string') {'        // Convert string to Uint8Array,
        ...
        bytes = encoder.encode(input)} else if (input instanceof ArrayBuffer) {
        bytes = new Uint8Array(input)} else if (Array.isArray(input)) {
        bytes = new Uint8Array(input)} else if (input instanceof Uint8Array) {
        bytes = input} else {
