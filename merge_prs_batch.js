...
console.log('🚀 Batch PR Merge System');
console.log('========================');
...
class BatchPRMerger {
  ...
  async getAllRemoteBranches() {
    try {
      ...
      const branches = result.split('\n')
        ...
        .slice(0, 20); // Process only first 20 branches
  ...
  async runAutomation() {
    try {
      this.log('Starting batch PR merge automation...');
  ...
  generateReport() {
    ...
    // Save report to file
    fs.writeFileSync('batch-pr-merge-report.json', JSON.stringify(report, null, 2));

    // Display summary
    console.log('\n🎉 Batch PR Merge Complete!');
    console.log('============================');
    ...
    console.log('\n📊 Detailed report saved to: batch-pr-merge-report.json');
  }
}

// Run the automation
const automation = new BatchPRMerger();
automation.runAutomation().then(() => {
  console.log('\n🚀 Batch PR merge automation completed!');
}).catch(error => {
