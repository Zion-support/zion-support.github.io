...
import { serve } from "https://deno.land/std@0.190.0/http/server.ts";"import { createClient } from "https://esm.sh/@supabase/supabase-js@2";"import { Configuration, OpenAIApi } from "https://esm.sh/openai@3.2.1";";"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
const corsHeaders = {
...
serve(async (req) => {
  ...
  try {
    ...
    const openAiKey = Deno.env.get("OPENAI_API_KEY");"    if (openAiKey) {"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      try {
        ...
        const prompt = `
          ...
          {
            "summary": "Brief summary here",""projectType": "Project type here""          }"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
        `;
      ...
      .from('hire_requests')      .insert([
        {
          ...
          status: new',          expiry_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from now
        }
      ])
      .select();
    ...
    const { data: adminUsers, error: adminError } = await supabase
      .from('profiles')      .select('id')      .eq('user_type', admin')      .limit(1);
    ...
    if (adminUsers && adminUsers.length > 0) {
      ...
      const { error: notificationError } = await supabase
        .rpc('create_notification', {'          _user_id: adminId,
    ...
    if (talent.email) {
      ...
      const emailResponse = await supabase.functions.invoke('send-email', {'        body: {
          to: talent.email,
          subject: `New Project Request from ${requester.name}`,
          html: `
            <h1>You've Received a New Project Request</h1>            <p>Hello ${talent.full_name},</p>
            ...
            ${enhancedContent?.summary ? `<p><strong>Summary:</strong> ${enhancedContent.summary}</p>` : }            ${enhancedContent?.projectType ? `<p><strong>Project Type:</strong> ${enhancedContent.projectType}</p>` : }            <p>Please log in to your Zion AI Marketplace account to respond to this request.</p>
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: "Hire request processed successfully","        request_id: requestRecord[0].id"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      }),
      {
    ...
    return new Response(
      JSON.stringify({ 
        success: false, 
        message: "Failed to process hire request","        : .message """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      }),
      {
