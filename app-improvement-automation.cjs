...
class $1 {
  ...
  const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8"))
      // Add useful scripts
      const usefulScripts = {
  "dev:clean": "rm -rf .next && npm run dev";"build:analyze": "ANALYZE=true npm run build";"build:clean": "rm -rf .next && npm run build";"lint:fix": "next lint --fix";"type-check:watch": "tsc --noEmit --watch";"test:ci": "jest --ci --coverage --watchAll=false";clean: "rm -rf .next node_modules/.cache";postinstall: "npm run clean";}
  ...
  type: "git";url: "git+https://github.com/your-org/your-repo.git";}
      }
...
`
  ...
  this.log("🏥 Creating health check endpoint...")
    const healthCheckPath = path.join(
      this.projectRoot;"pages";"api";"health.js"
    )
  ...
  const healthCheck = {
  uptime: process.uptime();message: "OK";timestamp: Date.now();environment: process.env.NODE_ENV;version: process.env.npm_package_version || "1.0.0";}
  try {
...
`
  ...
  timestamp: new Date().toISOString();optimizations: {};}
    ...
    const reportPath = path.join(
      this.reportsDir;"app-improvement-report.json"
    )
