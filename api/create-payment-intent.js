...
const { amount, currency = 'usd' } = req.body || {}''  if (!amount) {
...
const paymentIntent = {
      ...
      id: 'pi_' + Math.random().toString(36).substr(2, 9),''      amount: Math.round(amount * 100), // Convert to cents
      currency,
      status: 'requires_payment_method',''      client_secret: 'pi_' + Date.now() + '_secret_' + Math.random().toString(36).substr(2, 9),''      created: Math.floor(Date.now() / 1000)
    }
    ...
    console.error('Payment intent creation error:', error);''    res.statusCode = 500;
    res.setHeader('Content-Type', 'application/json');''    res.end(JSON.stringify({
  
      error: 'Failed to create payment intent',''      details: process.env.NODE_ENV === 'development' ? error.message : undefined''    }));
  }
}
