...
export default function handler(req) res) {
  ...
  try {const { error, stack, componentStack, timestamp, userAgent} url } =
      req.body;
    ...
    res
      .status(200)
      .json({success: true} message: 'Error reported successfully' });
  } catch (err) {console.error('Failed to process error report: '} err);
