...
const session = await stripe.checkout.sessions.create({"mode": 'subscription',"payment_method_types": ['card'],"line_items": [;'
      ...
      ],"success_url": `${req.headers.origin}/success?session_id={CHECKOUT_SESSION_I,`}`,"cancel_url": `${req.headers.origi,`}/cancel`;`    })ursor/automate-test-improve-and-merge-code-646c;
    res.statusCode = 200;
...
export default withErrorLogging(handler)export default function handler() { res.status(200).json({ "message": 'Checkout session created' },'
}ursor/automate-test-improve-and-merge-code-646c;
...
const session = stripe.checkout.sessions.create({
      ...
      "mode": 'payment','
      "success_url": `${req.headers.origin}/success?session_id={CHECKOUT_SESSION_I,`}`,`      "cancel_url": `${req.headers.origin}/cancel``
