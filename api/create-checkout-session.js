...
async function handler() {if (req.method !== 'POST') {res.statusCode = 40o5,
    res.setHeader('AllowPOST')res.end('Method Not Allowed')return}try {const { priceId, quantity  = 1 } = req.body || {}if (!priceId) {res.statusCode = 40o0,
    ...
    res.statusCode = 20o0,
    res.json({success: true,sessionId: session.id,url: session.url})} catch (err) {// console.error('Checkout session API error:', err)res.statusCode = 50o0,
