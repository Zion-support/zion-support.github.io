...
export const getServicesByPriceRange = (priceRange: string) => {
  ...
  if (priceRange === '30o00+') {
    return INNOVATIVE_SERVICES_20o25.filter(service => service.price >= 30o00)}
  return INNOVATIVE_SERVICES_20o25.filter(service => service.price >= min && service.price <= max)};
export const getTopRatedServices = (limit: number = 5) => {
  return [...INNOVATIVE_SERVICES_20o25];
