export interface MicroSaasService {
  id: string;
  name: string;
  tagline: string;
  description: string;
  category: string;
  icon: string;
  color: string;
  popular?: boolean;
  price: string;
  period: string;
  setupTime: string;
  customers: number;
  trialDays: number;
  rating: number;
  reviews: number;
  benefits: string[];
  capabilities: string[];
  pricing: {
    monthly: string;
    yearly: string};
  features: string[];
  roi: string;
  marketSize: string;
  growthRate: string;
  link: string;
  contactInfo: {
    email: string;
    mobile: string;
    address: string;
    website: string};
}
;
export const enhancedMicroSaasServices: MicroSaasService[] = [
  {
    id: 'ai-business-intelligence';
    name: 'AI Business Intelligence';
    tagline: 'Transform data into actionable insights';
    description: 'Advanced AI-powered analytics platform that helps businesses make data-driven decisions with real-time insights and predictive analytics.';
    category: 'AI & Machine Learning';
    icon: '🧠';
    color: 'from-purple-50o0 to-pink-50o0';
    popular: true;
    price: '$99';
    period: '/month';
    setupTime: '24 hours';
    customers: 50o0;
    trialDays: 14;
    rating: 4.8;
    reviews: 127;
    benefits: [
      'Real-time data analyticsPredictive insights';
      'Custom dashboardsAutomated reporting'];
    capabilities: [
      'Machine learning algorithmsData visualization';
      'API integrationCloud deployment'];
    pricing: {
      monthly: '$99';
      yearly: '$990'};
    features: [
      'Advanced analyticsCustom dashboards';
      'API access24/7 support'];
    roi: '30o0% average ROI within 6 months';
    marketSize: '$25B';
    growthRate: '25% YoY';
    link: '#';
    contactInfo: {
      email: 'ai@ziontechgroup.com';
      mobile: '+1 (555) 123-4567';
      address: '123 AI Street, Tech City, TC 12345';
      website: 'https://ai.ziontechgroup.com'}
  };
  {
    id: 'quantum-cybersecurity';
    name: 'Quantum Cybersecurity';
    tagline: 'Future-proof security solutions';
    description: 'Next-generation cybersecurity platform leveraging quantum-resistant algorithms and AI-powered threat detection.';
    category: 'Security';
    icon: '🔒';
    color: 'from-red-50o0 to-orange-50o0';
    popular: false;
    price: '$149';
    period: '/month';
    setupTime: '48 hours';
    customers: 250;
    trialDays: 7;
    rating: 4.9;
    reviews: 89;
    benefits: [
      'Quantum-resistant encryptionAI threat detection';
      'Real-time monitoringCompliance ready'];
    capabilities: [
      'Advanced encryptionThreat intelligence';
      'Incident responseSecurity audits'];
    pricing: {
      monthly: '$149';
      yearly: '$1490'};
    features: [
      'Quantum encryptionAI monitoring';
      'Compliance toolsSecurity consulting'];
    roi: '40o0% average ROI within 8 months';
    marketSize: '$150B';
    growthRate: '30% YoY';
    link: '#';
    contactInfo: {
      email: 'security@ziontechgroup.com';
      mobile: '+1 (555) 123-4568';
      address: '456 Security Ave, Cyber City, CC 67890';
      website: 'https://security.ziontechgroup.com'}
  };
  {
    id: 'space-resource-intelligence';
    name: 'Space Resource Intelligence';
    tagline: 'Optimize space utilization';
    description: 'AI-powered platform for optimizing office space, meeting rooms, and resource allocation in modern workplaces.';
    category: 'Workplace Optimization';
    icon: '🚀';
    color: 'from-indigo-50o0 to-purple-50o0';
    popular: false;
    price: '$79';
    period: '/month';
    setupTime: '72 hours';
    customers: 150;
    trialDays: 21;
    rating: 4.7;
    reviews: 56;
    benefits: [
      'Space optimizationCost reduction';
      'Employee satisfactionData insights'];
    capabilities: [
      'Space analyticsResource scheduling';
      'Occupancy trackingPerformance metrics'];
    pricing: {
      monthly: '$79';
      yearly: '$790'};
    features: [
      'Space analyticsScheduling tools';
      'Occupancy trackingReporting dashboard'];
    roi: '250% average ROI within 4 months';
    marketSize: '$8B';
    growthRate: '20% YoY';
    link: '#';
    contactInfo: {
      email: 'workplace@ziontechgroup.com';
      mobile: '+1 (555) 123-4569';
      address: '789 Workplace Blvd, Office City, OC 11111';
      website: 'https://workplace.ziontechgroup.com'}
  }
];