...
const base_url  = 'https://api.zion.os';const v1: ApiDocsSpec = {product: 'Zion OS',
  ...
  version: '1.0.0',description: 'Zion OS API Documentation';}
  },servers: [{ url: baseUrl }],paths: {'/v1/nation/stats': {get: {summary: 'Get Nation Stats',
  ...
  title: 'JWT Login',description: 'Obtain a JWT by providing credentials.',
  path: '/v1/auth/login',method: 'POST',
  visibility: 'public',auth: ['none'],requestBodySchema: {type: 'object',}
  properties: {email: { type: 'string' },password: { type: 'string' }
            },required: ['email', 'password'];
          },responseBodySchema: {type: 'object',}
  properties: {token: { type: 'string' },expiresIn: { type: 'number' }
            }
          },samples: [;
            { language: 'curl',}
  code: `curl -X POST ${baseUrl}/v1/auth/login -H 'Content-Type: application/json' -d '{\"email\":\"user@zion.os\",\"password\":\"***\"}'` }
            { language: 'javascript',}
  code: `await fetch('${baseUrl}/v1/auth/login', { method: 'POST',}
  headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: 'user@zion.os',}
  password: '***' }) }).then(r => r.json()),` }
            { language: 'python',}
  code: `import requests\nresp = requests.post('${baseUrl}/v1/auth/login', json={'email':'user@zion.os','password':'***'})\nprint(resp.json())` }];
          errors: [;
            { code: 'INVALID_CREDENTIALS',}
  httpStatus: 401, message: 'Email or password incorrect' }];
          versions: ['v1']}
        {id: 'oauth-token';
          title: 'OAuth Token';
  ...
  properties: { code: { type: 'string' }, redirect_uri: { type: 'string' } }, required: ['code', 'redirect_uri'] }
          responseBodySchema: { type: 'object',}
  ...
  code: `await fetch('${baseUrl}/v1/oauth/token', { method: 'POST',}
  headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code: 'AUTH_CODE',}
  redirect_uri: 'https://app.partner.com/callback' }) }).then(r => r.json())`;
            },{language: 'python',}
  ...
  items: { type: 'string' } } } }
          samples: [;
            { language: 'curl',}
  code: `curl -X POST ${baseUrl}/v1/messages -H 'Authorization: Bearer <token />' -H 'Content-Type: application/json' -d '{\"to\":\"USER123\",\"body\":\"Hello\"}'` },{ language: 'javascript',}
  ...
  body: 'Hello' }) }).then(r => r && r.json())` },{ language: 'python',}
  ...
  items: { type: 'object' } }, nextPage: { type: 'string' } } }
          samples: [;
const baseUrl  = 'https://api.zion.os';const baseUrl = 'https://api.zion.os';
const v1: ApiDocsSpec = {openapi: '3.0.0',
  ...
  code: `curl '${baseUrl}/v1/wallet/balance' -H 'Authorization: Bearer <token />'` },{ language: 'javascript',}
  code: `await fetch('${baseUrl}/v1/wallet/balance', { headers: { Authorization: 'Bearer <token />' } }).then(r => r && r.json())` },{ language: 'python',}
  code: `import requests\nrequests ;];}];}
ursor/fix-website-loading-errors-and-merge-6662;
}export default v1;
  ];
}export default v1;{language: 'curl',}
  ...
  code: `import requests\nrequests.get('${baseUrl}/v1/jobs', params={'q':'engineer','status':'open'}, headers={'Authorization':'Bearer <token />'}).json()`;
      ...
      endpoints: [;
        {id: 'quotes-create';
          ...
          auth: ['jwt'];}
          requestBodySchema: { type: 'object',}
  properties: { jobId: { type: 'string' }, milestones: { type: 'array',
  items: { type: 'object',}
  properties: { title: { type: 'string' }, amount: { type: 'number' } } } } }, required: ['jobId', 'milestones'] }
          responseBodySchema: { type: 'object',}
  ...
  code: `await fetch('${baseUrl}/v1/quotes', { method: 'POST',}
  headers: { Authorization: 'Bearer <token />', 'Content-Type': 'application/json' }, body: JSON.stringify({ jobId: 'JOB123',
  milestones: [{ title: 'Design',}
  amount: 1000 }] }) }).then(r => r.json())`;
            },{language: 'python',}
  ...
  code: `await fetch('${baseUrl}/v1/messages', { method: 'POST',}
  headers: { Authorization: 'Bearer <token />', 'Content-Type': 'application/json' }, body: JSON.stringify({ to: 'USER123',}
  body: 'Hello' }) }).then(r => r.json())`;
            },{language: 'python',}
  ...
  code: `import requests\nrequests.get('${baseUrl}/v1/treasury/logs', headers={'Authorization':'Bearer <token />'}).json()`;
      ...
      endpoints: [;
        {id: 'nation-stats';
          ...
          auth: ['jwt'];}
          responseBodySchema: { type: 'object',}
  properties: { members: { type: 'number' }, proposals: { type: 'number' } } }
          samples: [;
            {language: 'curl',}
