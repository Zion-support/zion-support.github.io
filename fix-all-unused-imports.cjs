...
for (const file of files) {
  try {
    ...
    const useClientRegex = /'use client';\s*'use client';/g;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.trim().startsWith('import ')) {
        ...
        const destructuredMatch = line.match(/import\s+\{\s*(\w+)\s*\}\s+from/);
        if (destructuredMatch) {
          const importedName = destructuredMatch[1];
          const remainingContent = lines.slice(i + 1).join('\n');
          if (!remainingContent.includes(importedName)) {
            console.log(`Removing unused destructured import: ${importedName} from ${file}`);
            continue; // Skip this line
          }
        }
      }
