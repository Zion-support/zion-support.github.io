...
function fixRemainingUnusedVars(filePath) {
  try {
    ...
    // Fix unused Page variables - more comprehensive pattern matching
    const pagePatterns = [
      // Pattern 1: const Page: React.FC = () => { ... }; const LazyPage = ...; export default LazyPage;
      ...
      // Pattern 2: const SomePage: React.FC = () => { ... }; const LazyPage = ...; export default LazyPage;
      /const (\w+): React\.FC = \(\) => \{[\s\S]*?\};\s*const LazyPage: React\.LazyExoticComponent<React\.ComponentType<any>> = React\.lazy\(\(\) => import\('\.\/page'\)\);\s*export default LazyPage;/g
    ];
    ...
    pagePatterns.forEach(pattern => {
      ...
      if (matches) {
        matches.forEach(match => {
          const componentMatch = match.match(/const (\w+): React\.FC = \(\) => \{[\s\S]*?\};\s*const LazyPage: React\.LazyExoticComponent<React\.ComponentType<any>> = React\.lazy\(\(\) => import\('\.\/page'\)\);\s*export default LazyPage;/);
          if (componentMatch) {
            const componentName = componentMatch[1];
            const replacement = match.replace(/const LazyPage: React\.LazyExoticComponent<React\.ComponentType<any>> = React\.lazy\(\(\) => import\('\.\/page'\)\);\s*export default LazyPage;/, `export default ${componentName};`);
    ...
    if (modified) {
      ...
      console.log(`Fixed remaining unused vars in: ${filePath}`);
    }
  } catch (error) {
...
specificFiles.forEach(file => {
  if (fs.existsSync(file)) {
    fixRemainingUnusedVars(file);
  }
});

console.log('Remaining unused variable fixes completed!');