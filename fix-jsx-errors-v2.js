...
for (const filePath of files) {/* TODO: Fix JSX expression */}
    ...
    content = content.replace(/={\s*\/Service Feature
        .replace(/={\s*\//, '')
        .trim();
      return `={'${value}'}`;
    ...
    content = content.replace(/(\w+):\s*\/Service Feature*$/gm, (match, prop) => {
      if (value.startsWith('/') && !value.endsWith('/')) {
    content = content.replace(/(\w+):\s*\/Service Feature*$/gm, (match, prop) => {/* TODO: Fix JSX expression */}`
        return `${prop}: '${value.substring(1)}'`}
      return match});

    //Fix malformed JSX expressions;
    content = content.replace(/\{\s*\/Service Feature*$/gm, match => {)
    content = content.replace(/\{/* TODO: Fix JSX expression */}`
      return `{'${value}'}`;)
    });
    ...
    content = content.replace(/`Service Feature*$/gm, match => {)
      if (!match.endsWith('`')) {
        return match + '`';
    //Fix malformed template literals;`
    content = content.replace(/`Service Feature*$/gm, match => {/* TODO: Fix JSX expression */}
      }
      return match;)
    });
    ...
    content = content.replace(/(\w+)=\{Service Feature*$/gm, match => {
      if (!match.includes('}')) {
    content = content.replace(/(\w+)=\{Service Feature*$/gm, match => {/* TODO: Fix JSX expression */}
