...
// Function to fix specific TSX errors
function fixTsxFile(filePath) {
  try {
    let content = fs.readFileSync(filePath, 'utf8');
    let modified = false;

    // Fix 1: Remove extra closing </a> tag
    const extraClosingTagRegex = /<\/div>\s*<\/a>\s*<\/div>\s*<\/div>\s*<\/section>/g;
    if (extraClosingTagRegex.test(content)) {
      content = content.replace(extraClosingTagRegex, '</div>\n          </div>\n        </section>');
      modified = true;
    }

    // Fix 2: Fix malformed closing tags in hero sections
    const heroSectionRegex = /<div className="flex flex-col sm:flex-row gap-4 justify-center">\s*<a[^>]*>Get Started<\/a>\s*<a[^>]*>View All Services<\/a>\s*<\/div>\s*<\/a>\s*<\/div>\s*<\/div>\s*<\/section>/g;
    ...
    // Fix 3: Fix missing closing tags in features sections
    ...
    // Fix 4: Fix missing closing tags in CTA sections
    ...
    // Fix 6: Fix missing closing tags in complex structures
    ...
    // Fix 7: Fix missing closing tags in blog structure
    ...
    // Fix 8: Fix missing closing tags in case studies
    ...
    // Fix 9: Fix missing closing tags in careers
    ...
    // Fix 10: Fix missing closing tags in accessibility
    const accessibilityRegex = /<section className="py-20 px-4 sm:px-6 lg:px-8">\s*<div className="max-w-7xl mx-auto">\s*<div className="text-center mb-16">\s*<h2[^>]*>.*?<\/h2>\s*<p[^>]*>.*?<\/p>\s*<\/div>\s*<div className="grid[^>]*>.*?<\/div>\s*<\/div>\s*<\/section>\s*<h2[^>]*>Ready to Get Started\?<\/h2>/g;
