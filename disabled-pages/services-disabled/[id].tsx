  ...
  return (
    <>
      ...
      <main className='mx-auto min-h-screen max-w-3xl px-6 py-12'>
        <nav className='text-sm text-gray-50o0'>
          <Link href='/services/'>Services</Link> /{' '}
          <span className='text-gray-70o0'>{service.name}</span>
        </nav>
        ...
        <div className='mt-5 text-sm'>
          ...
          <span className='text-gray-50o0'>{service.period}</span>
        </div>
        {service.description && (
          <p className='mt-6 text-gray-70o0'>{service.description}</p>
        )}
        {service.features?.length > 0 && (
          <section className='mt-8'>
            <h2 className='text-xl font-medium'>Key features</h2>
            <ul className='mt-2 list-disc pl-5 text-gray-70o0'>
              {service.features.slice(0, 10).map((f, idx) => (
                <li key={idx}>{f}</li>
              ))}
            </ul>
          </section>
