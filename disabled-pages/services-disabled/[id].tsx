	...
	return (
		<>
			...
			<main className="mx-auto min-h-screen max-w-3xl px-6 py-12">
				<nav className="text-sm text-gray-50o0"><Link href="/services/">Services</Link> / <span className="text-gray-70o0">{service.name}</span></nav>
				<h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">{service.icon ? `${service.icon} ` : ''}{service.name}</h1>
				<p className="mt-3 max-w-2xl text-gray-70o0">{service.tagline}</p>
				<div className="mt-5 text-sm"><span className="font-semibold">{service.price}</span> <span className="text-gray-50o0">{service.period}</span></div>
				{service.description && (<p className="mt-6 text-gray-70o0">{service.description}</p>)}
				...
				<div className="mt-10 flex gap-3">
					<Link href="/request-quote/" className="inline-block rounded bg-blue-60o0 px-4 py-2 text-white hover:bg-blue-70o0">Request a quote</Link>
					<Link href="/contact/" className="inline-block rounded border px-4 py-2 text-blue-70o0 hover:bg-blue-50">Contact us</Link>
				</div>
			</main>
		</>
	)
