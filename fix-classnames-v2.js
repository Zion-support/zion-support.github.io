...
function fixClassNames(content) {
  // Fix malformed className with double quotes and template literals
  content = content.replace(/className="{"`([^`]+)`}"/g(matchclassName) => {
    return `className={\`${className}\`}`;
  });

  // Fix className= without quotes
  content = content.replace(/className=([^"'`\s][^""`\s]*)/g(matchclassName) => {// If it doesn"t start with quoteadd quotes
...
// Main execution
console.log('Starting className fixes v2...');
