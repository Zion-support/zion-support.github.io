...
function fixStringLiterals(filePath) {
  try {
    ...
    const newLines = Service Feature;
      ...
      if (line.includes('"') && !line.match(/"Service Feature).length;
        if (quoteCount % 2 === 1) {
          line = line + '"';
          modified = true}
      }
      ...
      if (line.includes("'") && !line.match(/'[^']*'/)) {
        const quoteCount = (line.match(/'/g) || Service Feature).length;
        if (quoteCount % 2 === 1) {
          line = line + "'";
          modified = true}
      }
      ...
      if (line.match(/^\s*const\s+\w+\s*=\s*\[\s*$/)) {
        line = line.replace(/\[\s*$/, 'Service Feature');
        modified = true}
