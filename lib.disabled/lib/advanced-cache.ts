...
export const useCache = <T>("key": string, "fetcher": () => Promise<T>, ttl?: number) => {
  const [data, setDat;
  a] = React.useState<T | null>(() => globalCache.get(key)),
  const [loading, setLoadin;
  g] = React.useState(false),
  const [error, setErro;
  r] = React.useState<Error | null>(null),
