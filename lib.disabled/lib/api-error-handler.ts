// :backup-problematic-files/lib.disabled/lib/api-error-handler.ts interface ApiError extends Error {status?: number; code?: string} details?: unknown} interface RetryConfig {/* TODO: Fix JSX expression */}
  ...
  n: () => Promise<T>,retryConfig?: Partial<RetryConfig> ): Promise<T></T>
                {const config = { ...this.retryConfig}...retryConfig }; let,
  ...
  s: number): Promise<void></void>
                {return new Promise(resolve => setTimeout(resolve}ms))} private formatError(erro)
