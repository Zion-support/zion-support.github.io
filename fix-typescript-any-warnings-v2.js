  ...
  for (const file of files) {
    try {
      ...
      if (content.includes('Record<string, any>')) {
        const newContent = content.replace(/Record<string, any>/g, 'Record<string, unknown>');
        if (newContent !== content) {
          fs.writeFileSync(file, newContent, 'utf8');
          modified = true;
          fixedCount++;
          console.log(`✅ Fixed Record<string, any> in ${file}`);
        }
      }
