...
function getAllFiles(dir, extensions = ['.ts', '.tsx', '.js', '.jsx']) {
  ...
  list.forEach(file => {
    ...
    if (stat && stat.isDirectory()) {
      ...
      if (!['node_modules', '.git', 'dist', '.next', 'out', 'backup-unused-components'].includes(file)) {
...
function cleanFile(filePath) {
  try {
    ...
    // Third pass: remove unused imports and clean up the file
    const newLines = [];
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine.startsWith('import ')) {
        ...
        if (inImportBlock && trimmedLine === '') {
          ...
          if (trimmedLine.includes('const ') && trimmedLine.includes(' = ') && !trimmedLine.includes('export')) {
            ...
            if (varMatch) {
              ...
              if (!remainingContent.includes(varName) && !usedImports.has(varName)) {
                // This variable is not used, remove the line
                modified = true;
                return;
              }
            }
          ...
          if (trimmedLine.includes('function') || trimmedLine.includes('=>')) {
            // This is a function definition, we'll handle it separately if needed
          }
...
const rootFiles = ['./App_clean.tsx', './App.tsx', './main.tsx', './page_clean.tsx'];
