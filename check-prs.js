#!/usr/bin/env node;
const https  = require('https')const options = {hostname: 'api.github.com',path: '/repos/Zion-Holdings/zion.app/pulls?state=open',headers: {'Authorization': 'token ghs_g7Ribzcy3sJGR7qj8KILEG5nEY0aYC1Jg1cH','User-Agent': 'Node.js';
  }
    ...
    } catch (error) {console.error('❌ Error parsing PR data:', error.message)console.log('Raw response:', data)}
  })})req.on('error', (error) => {console.error('❌ Error checking PRs:', error.message)})req.end()