...
export default function SEOOptimized({
  ...
  const fullImage = image ? (image.startsWith('http') ? image : `https://zion.app${image}`) : `https://zion.app${defaultSEO.image}`
  
  const seoTitle = title ? `${title} | ${defaultSEO.title}` : defaultSEO.title
  const seoDescription = description || defaultSEO.description
  const seoKeywords = [...new Set([...(keywords || []), ...defaultSEO.keywords])].join(', ')
  ...
  const breadcrumbStructuredData ={
    ...
    itemListElement: pathname.split('/').filter(Boolean).map((segment, index) => ({
      ...
      item: `https://zion.app/${pathname.split('/').slice(0, index + 2).join('/')}`
    }))
  }
