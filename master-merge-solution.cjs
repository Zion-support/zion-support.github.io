...
function step2_mergeOpenPRs() {
  ...
  try {
    ...
    const branches = runCommand('git branch --list', 'Get local branches', { silent: true });
    ...
    for (const branch of branchList) {
      try {
        ...
        runCommand(`git merge ${branch} --no-ff -m "Merge branch ${branch} into main"`, 
                   `Merge branch ${branch}`, 
                   { allowFailure: true });
        
        // Delete the merged branch
        runCommand(`git branch -d ${branch}`, `Delete merged branch ${branch}`, { allowFailure: true });
...
function step3_proceedWithImprovements() {
  ...
  try {
    ...
    if (staged && staged.trim()) {
      runCommand('git commit -m "Apply code improvements and fixes"', 'Commit improvements');
    }
    ...
    const newStaged = runCommand('git diff --cached --name-only', 'Check new staged files', { 
      silent: true, 
      allowFailure: true 
    });
    
    if (newStaged && newStaged.trim()) {
      runCommand('git commit -m "Apply linting and build improvements"', 'Commit build improvements');
    }
...
function generateReport() {
  ...
  try {
    const currentBranch = runCommand('git branch --show-current', 'Get current branch', { silent: true });
    const lastCommit = runCommand('git log -1 --oneline', 'Get last commit', { silent: true });
