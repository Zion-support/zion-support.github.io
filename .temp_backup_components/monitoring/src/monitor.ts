...
async function runMonitoring() {
  ...
  for (const result of results) {
    // No await' here if we want alerts to be non-blocking for the overall monitoring cycle.'    // However, for logging clarity and ensuring restart attempts are logged within the same run,
    // await' might be preferred. Let's use await.'    await triggerAlerts(result);
  }
  ...
  if (slowestResponses.length > 0) {
    logger.info('Top 5 slowest responses:', { slowestResponses });  } else {
    logger.info('No responses measured or all responses were errors before latency could be determined.');  }

  results.forEach(result => {
    if (result.error) {
      logger.error('Endpoint test failed:', result);    } else {
