...
exports.handler = async () => {
  const steps = [
    'automation/alt-text-improver.cjs',
  ...
  const ok = results.every(r => r.ok || /Alt Text Improver finished|Updated alt text/i.test(r.stdout));
  return {
    statusCode: ok ? 200 : 207,
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ engine: 'alt-text-improver', results, timestamp: new Date().toISOString() })
  };
};