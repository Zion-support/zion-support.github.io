    ...
    if (!pullResult.success) {
      ...
      if (statusResult.success) {
        ...
        for (const file of conflictedFiles) {
          if (file.includes('automation') || file.includes('script') || file.endsWith('.cjs') || file.endsWith('.js')) {
            await this.runCommand(`git checkout --theirs "${file}"`, `Accept incoming changes for ${file}`);
            await this.runCommand(`git add "${file}"`, `Stage resolved file ${file}`);
...
`;
  ...
  async mergePR(pr) {
    ...
    try {
      ...
      await this.runCommand(`git fetch origin ${pr.headRefName}`, `Fetch PR branch ${pr.headRefName}`);
      await this.runCommand(`git checkout ${pr.headRefName}`, `Checkout PR branch ${pr.headRefName}`);
...
`;
  ...
  async run() {
    ...
    this.log(`  - Automation scripts run: ${finalReport.summary.automationScriptsRun}`);
    this.log(`  - Successful automations: ${finalReport.summary.successfulAutomations}`);
  }
}
