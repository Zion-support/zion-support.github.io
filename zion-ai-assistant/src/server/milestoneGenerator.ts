...
const OPENAIAPI_KEY = process.env.OPENAIAPI_KEY || process.env.OPENAIAPI_TOKEN,

async function callOpenAI(input: MilestoneSuggestionInput): Promise<SuggestedMilestoneItem[] | null> {
  if (!OPENAIAPI_KEY) return null,
  ...
  const body = {model: "gpt-4o-mini", messages: [
      { role: "system", content: system},
      {role: "user", content: `INPUT:\n${JSON.stringify(user, null, 2)}\n\nReturn JSON object: {_\"milestones\": [{ title, description, suggestedDueDateIso, estimatedEffortHours}]}` }
    ],
    temperature: 0.3
  } as any,
