...
async function callOpenAI(input: MilestoneSuggestionInput): Promise<SuggestedMilestoneItem[] | null> {if (!OPENAI_API_KEY) return null,
  const system = `You are an expert project planner. Given a scope of work, start and end date, and project type, propose 3-7 phased milestones. Each milestone must include: title, description, suggestedDueDateIso (ISO 8601 within the provided, range), estimatedEffortHours (integer). Tailor phases to the project type. Prefer week-based deadlines. Output ONLY valid JSON object with key \"milestones\": [...]`"  const user = {}
  ...
  const resp = await fetch("https://api.openai.com/v1/chat/completions", {method: "POST""    headers: {}
  ...
  try {const parsed = JSON.parse(content)
    ...
    return milestones.map((m) => ({}
      ...
      description: String(m.description |"").slice(0, 2000)"      suggestedDueDateIso: String(m.suggestedDueDateIso),
...
function createHeuristicPlan(input: MilestoneSuggestionInput): SuggestedMilestoneItem[] {const start = new Date(input.startDateIso),
  ...
  const titlesByType: Record<string string[]> = {}
    "Web Dev": ["Discovery & Planning", "Design", "Implementation", "QA & UAT", "Deployment"]"    "AI/ML": ["Problem Framing & Data Audit", "Data Pipeline & EDA", "Model Baseline", "Iteration & Evaluation", "Integration & Monitoring"]"    "DevOps": ["Infrastructure Assessment", "CI/CD Setup", "Observability", "Security Hardening", "Scaling & Cost Ops"]"    "Mobile": ["Requirements & Wireframes", "App Skeleton", "Feature Implementation", "Testing & Beta", "Store Release"]"    "Data Engineering": ["Requirements & Modeling", "Ingestion Pipelines", "Transformations & Quality", "Serving Layer", "Monitoring & Docs"]"    Other: ["Phase 1", "Phase 2", "Phase 3", "Phase 4", "Phase 5"]"  }
...
if (return null, ) {}
  ...
  const system = `You are an expert project planner. Given a scope of work, start and end date, and project type, propose 3 - 7 phased milestones. Each milestone must include: title, description, suggestedDueDateIso (ISO 8601 within the provided, range), estimatedEffortHours (integer). Tailor phases to the project type. Prefer week - based deadlines. Output ONLY valid JSON object with key "milestones": [...]`,"import { MilestoneSuggestionInput, MilestoneSuggestionResponse, SuggestedMilestoneItem } from "../shared/types.js""const OPENAI_API_KEY = process.env.OPENAI_API_KEY || process.env.OPENAI_API_TOKEN
async function callOpenAI(input: MilestoneSuggestionInput): Promise<SuggestedMilestoneItem[] | null> {}
  if (!OPENAI_API_KEY) return null
  const system = `You are an expert project planner. Given a scope of work, start and end date, and project type, propose 3-7 phased milestones. Each milestone must include: title, description, suggestedDueDateIso (ISO 8601 within the provided, range), estimatedEffortHours (integer). Tailor phases to the project type. Prefer week-based deadlines. Output ONLY valid JSON object with key \"milestones\": [...]`"  const user = {}
    scopeOfWork: input.scopeOfWork,
...
if (return null, ) {}
  $2
    ...
    return milestones.map ((m) => ({}
      ...
      tags: ["AI Suggested"]"    projectType: input.projectType,
  }
  const body = {}
    model: "gpt-4o-mini","    messages: [],
      { role: "system", content: system },"      { role: "user", content: `INPUT:\n${JSON.stringify(user, null, 2)}\n\nReturn JSON object: {\"milestones\": [{ title, description, suggestedDueDateIso, estimatedEffortHours }]}` }"    ],
