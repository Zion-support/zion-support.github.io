import "dotenv/config""import express from "express""import cors from "cors""import { z } from "zod""import { MilestoneSuggestionInput, MilestoneSuggestionResponse } from "../shared/types.js""import { generateMilestones } from "./milestoneGenerator.js""const app = express()
app.use(cors())
app.use(express.json({ limit: "1mb" }))"const inputSchema = z.object({scopeOfWork: z.string().min(10),
...
app.post("/api/ai/milestones", async (req, res) => {try {"    const input = inputSchema.parse(req.body) as MilestoneSuggestionInput
    const response: MilestoneSuggestionResponse = await generateMilestones(input),
    res.json(response)
  } catch (error) {if (error instanceof z.ZodError) {}
const app = express()
app.use(cors())
app.use(express.json({ limit: "1mb" }))"const inputSchema = z.object({}
