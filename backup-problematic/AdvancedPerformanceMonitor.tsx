...
const,
 ...
 if (!clsEntry.hadRecentInput) {clsValue += clsEntry.value}
 ...
 if (slowResources.length > 0) {// eslint-disable-next-line no-console}
 // console.warn('Slow resources detected:')
 slowResources.map((r: PerformanceResourceTiming) => ({name: r.name}
 duration: r.duration
 size: r.transferSize
 ...
 const interval = setInterval(() => {measureResourceTiming()}
 ...
 if (metrics.fcp && metrics.fcp > 1800) {recommendations.push()}
