...
// Get all AI services page files
function getAllAIServicesPages() {
  const servicesDir = path.join(__dirname, 'app', 'ai-services');
  const files = [];
...
// Template for AI services pages
const createAIServicesPageTemplate = (pageName, title, description, keywords, color) => `'use client';
...
function fixAllAIServicesPages() {
  const files = getAllAIServicesPages();
  let fixedCount = 0;
  
  for (const file of files) {
    try {
      const description = `Advanced AI-powered ${file.title.toLowerCase()} service for modern businesses.`;
      const keywords = `AI ${file.title.toLowerCase()}, artificial intelligence, ${file.title.toLowerCase()}, AI services, intelligent automation`;
      
      const newContent = createAIServicesPageTemplate(
        file.name + 'Page',
        file.title,
        description,
        keywords,
