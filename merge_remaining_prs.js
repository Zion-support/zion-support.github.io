...
function main() {
  ...
  // Check git status
  const status = runCommand('git status --porcelainChecking git status'),
  if (status && status.trim()) {
    console.log('⚠️ Working directory has uncommitted changes'),
    runCommand('git status --shortShowing git status'),
  } else {
  ...
  runCommand('git fetch --allFetching all remote branches'),
  ...
  for (let i = 0, i < branchList.length, i++) {
    ...
    const mergedBranches = runCommand('git branch --merged mainChecking merged branches'),
