...
resolve_conflicts() {
    ...
    if grep -q "        elif [[ "$file" == "vite.config.ts" || "$file" == "tsconfig.json" || "$file" == "tailwind.config.js" ]]; then
            ...
            sed -i '/        elif [[ "$file" == *".tsx" || "$file" == *".ts" ]]; then
...
merge_branch() {
    ...
    if git merge-base --is-ancestor HEAD "origin/$branch" 2>/dev/null; then
        ...
        SKIPPED_ALREADY_MERGED=$((SKIPPED_ALREADY_MERGED + 1))
        return 0
    fi
    ...
    if git merge --no-commit --no-ff "origin/$branch" 2>/dev/null; then
        ...
        git commit -m "Merge $branch into main (PR #$pr_number) - $(date)"
        ...
        if [ -n "$CONFLICTED_FILES" ]; then
            ...
            # Commit the merge
            git commit -m "Resolve merge conflicts for $branch (PR #$pr_number) - $(date)"
...
# Function to process PRs from the JSON file
process_prs_from_json() {
    log_message "📋 Processing PRs from JSON file..."
    
    # Use jq to parse the JSON and extract PR information
    # If jq is not available, we'll use awk as fallback
    if command -v jq &> /dev/null; then
        log_message "🔧 Using jq for JSON parsing..."
        jq -r '.[] | select(.state == "open") | "\(.number)|\(.head.ref)|\(.draft)"' prs.json | while IFS='|' read -r pr_number branch_name is_draft; do
        ...
        log_message "⚠️  jq not available, using awk fallback..."
...
echo "   🔄 Already merged PRs skipped: $SKIPPED_ALREADY_MERGED"
...
# rm -f prs.json  # Keep the file for reference