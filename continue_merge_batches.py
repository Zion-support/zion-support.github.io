#!/usr/bin/env python3
"""
Continue merging remaining branches in larger batches
"""
import subprocess
import sys
import time
...
def merge_branch_continue(branch_name, lock, stats):
    """Continue merge function with optimized strategies"""
    try:
        # Try simple merge with auto-resolution
        success, stdout, stderr = run_command(f"git merge origin/{branch_name} -X theirs --no-edit", timeout=10)
        ...
        # Quick abort and try alternative
        run_command("git merge --abort", timeout=3)
        success, stdout, stderr = run_command(f"git merge origin/{branch_name} -X ours --no-edit", timeout=10)
        ...
        # Final attempt with subtree strategy
        run_command("git merge --abort", timeout=3)
        success, stdout, stderr = run_command(f"git merge origin/{branch_name} --strategy=subtree --no-edit", timeout=10)
...
def main():
    ...
    # Get next batch of branches (skip first 100)
    print("\n📋 Getting next batch of branches...")
    success, branches_output, _ = run_command('git branch -r | grep -E "(cursor|codex)" | tail -n +101 | head -200')
    ...
    print(f"📊 Processing next {total_branches} branches...")
    ...
    print(f"⚡ Using high parallelism (10 workers) for speed")
    ...
    for batch_num in range(total_batches):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(merge_branch_continue, branch, lock, stats): branch for branch in batch}
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if success:
                    if completed % 10 == 0:  # Show progress every 10 branches
                        print(f"  ✅ Progress: {completed}/{len(batch)} - {branch_name}")
                else:
                    if completed % 25 == 0:  # Show failures every 25 branches
        ...
        if stats['successful'] > 0:
            ...
            push_success, _, _ = run_command("git push origin main", timeout=180)
            if not push_success:
                print(f"⚠️ Push failed, trying force push...")
                run_command("git push origin main --force", timeout=180)
    ...
    print(f"\n🎉 CONTINUE MERGE BATCHES COMPLETED!")
    print("=" * 50)
    ...
    run_command("git push origin main", timeout=180)
